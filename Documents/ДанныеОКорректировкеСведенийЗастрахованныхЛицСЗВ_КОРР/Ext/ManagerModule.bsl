...
Процедура СоздатьВТПерсональныеДанныеСотрудников(МенеджерВременныхТаблиц, Организация, ОтчетныйПериод, ОкончаниеОтчетногоПериода, ФизическиеЛица = Неопределено, ДатаДокумента = Неопределено) Экспорт 
	...
	ПараметрыПолученияСотрудников.КадровыеДанные = "ФизическоеЛицо, Фамилия, Имя, Отчество, СтраховойНомерПФР, ОформленПоТрудовомуДоговору, ВидЗастрахованногоЛица, Страна, ИНН, ДатаРождения, ДатаУвольнения, ВидЗанятости";
	...
	Если Год(ОтчетныйПериод) >= 2025 И ДатаДокумента <> Неопределено 
		И ДатаДокумента >= ПерсонифицированныйУчет.ДатаПостановленияЕФС1_2026() Тогда 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		КОГДА СотрудникиОрганизации.ВидЗастрахованногоЛица В (&КодМДИГ)
...
Функция ЗапросПоШапкеДляПечати(СписокДокументов)
	...
	|	СведенияОбОрганизациях.КатегорияСтрахователяФизическогоЛица КАК КатегорияСтрахователяФизическогоЛица,
	...
	|			И ДанныеОКорректировкеСведений.Дата = СведенияОбОрганизациях.Период
	|ГДЕ
	...
	|	ДанныеДокументов.КатегорияСтрахователяФизическогоЛица КАК КатегорияСтрахователяФизическогоЛица,
