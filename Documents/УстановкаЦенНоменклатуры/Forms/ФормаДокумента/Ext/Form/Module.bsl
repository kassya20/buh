...
Процедура ЗаполнитьТоварыСервер(Очистить, Обновить, ПоНоменклатуре = Ложь, Группа = Неопределено)
	...
	СписокУсловий.Добавить("НЕ СправочникНоменклатура.ВАрхиве");
	Если НЕ ПоНоменклатуре Тогда
		СписокУсловий.Добавить("ТипыЦенСправочник.Ссылка = &ТипЦен");
	КонецЕсли;
	Если ЗначениеЗаполнено(Группа) Тогда
		СписокУсловий.Добавить("СправочникНоменклатура.Ссылка В ИЕРАРХИИ (&Группа)");
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Текст, "&СписокУсловий", СтрСоединить(СписокУсловий, " И "));
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаТипЦен);
	
	Элементы.ТипЦен.РастягиватьПоГоризонтали = Истина;
	Элементы.ПроводитьНулевыеЗначения.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
