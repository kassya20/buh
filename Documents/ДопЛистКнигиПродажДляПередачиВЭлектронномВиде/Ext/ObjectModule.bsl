...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если НалоговыйПериод >= '2026-01-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеДопЛистовКнигиПродаж_508(ЭтотОбъект);
	...
	Иначе
		ИмяФункции = "Документ.ДопЛистКнигиПродажДляПередачиВЭлектронномВиде.МодульОбъекта.ВыгрузитьДокумент";
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Функция %1:
			|Формирование электронного представления дополнительных листов книги продаж за выбранный налоговый период не предусмотрено'"),
			ИмяФункции);
