...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	ВнешниеДанные = ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчетПриВнешнейЗарплате");
	СохраненныйДокумент = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "Ключ");
	Если ЗначениеЗаполнено(СохраненныйДокумент)Тогда
		ВнешниеДанные = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Параметры.Ключ, "ЗагруженИзВнешнегоИсточника");
	КонецЕсли;
	
	Если ВнешниеДанные Тогда
		СтандартнаяОбработка = Ложь;
		ВыбраннаяФорма = "ФормаДокументаВнешниеДанные";
	КонецЕсли;
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьКадровыйУчетОсновнойИнтерфейс";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.ФункциональныеОпции = "ИспользоватьКадровыйУчет";
	ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "КадровыйДокументВнешнегоИсточника", Ложь);
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчетПриВнешнейЗарплате") Тогда
...
Процедура ЗаполнитьДанныеДокументовДляПечатиРасчетаСреднегоЗаработка(МассивСсылок, ДанныеДокументов, ИмяМакета) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(КадровыеДанныеСотрудников.ДатаПриема, МЕСЯЦ) > НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ВЫБОР
...
Процедура СоздатьВТКадровыеДанныеСотрудниковДокумента(Запрос)
	...
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(Описатель, Истина, "ДатаПриема");
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Представление = СтрШаблон(НСтр("ru = 'Увольнение %1 от %2'"), Данные.НомерПриказа, Формат(Данные.Дата, "ДЛФ=D"));
...
//
// Параметры:
//	Объект                  - ДокументОбъект.Увольнение - исходый документ.
...
Функция АдресПараметровВХранилище(Объект, ДополнительныеПараметры) Экспорт
	...
	ПараметрыРасчетаСреднегоЗаработка.Вставить("ФизическоеЛицо",                    Объект.ФизическоеЛицо);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("ДатаНачалаСобытия",                 Объект.ДатаУвольнения + 86400);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("МесяцНачисления",                   Объект.ПериодРегистрации);
	
	ПараметрыРасчетаСреднегоЗаработка.Вставить("Подразделение",                  ДополнительныеПараметры.Подразделение);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("НачислениеКомпенсацияОтпуска",   ДополнительныеПараметры.НачислениеКомпенсацияОтпуска);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("НачислениеКомпенсацияОтпускаРК", ДополнительныеПараметры.НачислениеКомпенсацияОтпускаРК);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("НачислениеКомпенсацияОтпускаСН", ДополнительныеПараметры.НачислениеКомпенсацияОтпускаСН);
	
	Возврат ПоместитьВоВременноеХранилище(ПараметрыРасчетаСреднегоЗаработка, ДополнительныеПараметры.УникальныйИдентификатор);
	
КонецФункции

// Производит перерасчет начисления компенсации отпуска.
// 
// Параметры:
//  Объект                  - ДокументОбъект.Увольнение - исходый документ.
...
//  ДополнительныеПараметры - Структура - дополнительные параметры из формы документа.
//
Процедура РассчитатьНачисления(Объект, ОбъектФормы, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		НачисленияДокумента = Объект.Начисления.Выгрузить();
		
		Объект.Начисления.Очистить();
		
		СтрокаНачислений = Объект.Начисления.Добавить();
		
		СтрокаНачислений.Начисление    = ДополнительныеПараметры.НачислениеКомпенсацияОтпуска;
		СтрокаНачислений.Сотрудник     = Объект.Сотрудник;
		СтрокаНачислений.Подразделение = ДополнительныеПараметры.Подразделение;
		...
		НачислениеКомпенсацияОтпускаРК = ДополнительныеПараметры.НачислениеКомпенсацияОтпускаРК;
		НачислениеКомпенсацияОтпускаСН = ДополнительныеПараметры.НачислениеКомпенсацияОтпускаСН;
		...
		КалендарныеДниМесяцаБезПраздников = МодульРасчетЗарплатыДляНебольшихОрганизаций.КалендарныеДниМесяцаБезПраздников(ПроизводственныйКалендарь, Объект.ДатаУвольнения);
		МРОТ = ЗарплатаКадрыПовтИсп.МинимальныйРазмерОплатыТрудаРФ(Объект.ДатаУвольнения);
		
		Если Объект.УчитыватьМРОТПриОплатеПоСреднемуЗаработку И Объект.СреднийЗаработок * КалендарныеДниМесяцаБезПраздников <= МРОТ Тогда
		...
		Если Объект.ПланируемаяДатаВыплаты >= ДатаЗаконаПроРКиЕАЭС Тогда
			...
			Если ЗначениеЗаполнено(НачислениеКомпенсацияОтпускаРК) И КомпенсацияОтпускаРК > 0 Тогда
			...
			Если ЗначениеЗаполнено(НачислениеКомпенсацияОтпускаСН) И КомпенсацияОтпускаСН > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из НачисленияДокумента Цикл
			Если СтрокаТаблицы.Начисление = ДополнительныеПараметры.НачислениеКомпенсацияОтпуска
...
// Производит перерасчет начисления за отработанные дни.
// 
// Параметры:
//  Объект                  - ДокументОбъект.Увольнение - исходый документ.
//  ОбъектФормы             - ДокументОбъект.
//  ДополнительныеПараметры - Структура - дополнительные параметры из формы документа.
//
Процедура ВыполнитьРасчетНачислений(Объект, ОбъектФормы, ДополнительныеПараметры) Экспорт
	...
	Если РезультатНачисления.Количество() > 0 Тогда
		...
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ОтработаноДней")
			И Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ОтработаноЧасов") Тогда
			РезультатНачисления.Колонки.Добавить("ОтработаноДней");
			РезультатНачисления.Колонки.Добавить("ОтработаноЧасов");
		КонецЕсли;
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ОплаченоДней")
			И Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ОплаченоЧасов") Тогда
			РезультатНачисления.Колонки.Добавить("ОплаченоДней");
			РезультатНачисления.Колонки.Добавить("ОплаченоЧасов");
		КонецЕсли;
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ДатаНачала")
			И Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(РезультатНачисления.Колонки, "ДатаОкончания") Тогда
		...
		Для Каждого СтрокаТаблицы Из РезультатНачисления Цикл
			...
			Если СтрокаТаблицы.Начисление = ДополнительныеПараметры.НачислениеКомпенсацияОтпуска
				Или СтрокаТаблицы.Начисление = ДополнительныеПараметры.НачислениеКомпенсацияОтпускаРК
				Или СтрокаТаблицы.Начисление = ДополнительныеПараметры.НачислениеКомпенсацияОтпускаСН Тогда
		...
		Для Каждого СтрокаТаблицы Из НачисленияДокумента Цикл
			Если СтрокаТаблицы.Начисление <> ДополнительныеПараметры.НачислениеКомпенсацияОтпуска
				И СтрокаТаблицы.Начисление <> ДополнительныеПараметры.НачислениеКомпенсацияОтпускаРК
				И СтрокаТаблицы.Начисление <> ДополнительныеПараметры.НачислениеКомпенсацияОтпускаСН Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрока = Объект.Начисления.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
		...
		Для Каждого СтрокаТаблицы Из РезультатНачисления Цикл
			СтрокаТаблицы.ОплаченоДней = Мин(СтрокаТаблицы.ОплаченоДней, СтрокаТаблицы.ОтработаноДней);
			СтрокаТаблицы.ОплаченоЧасов = Мин(СтрокаТаблицы.ОплаченоЧасов, СтрокаТаблицы.ОтработаноЧасов);
		КонецЦикла;
...
Процедура ПересчитатьНачисленияСКоэффициентом(Сотрудник, Начисления) Экспорт
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		МодульРасчетЗарплатыДляНебольшихОрганизаций.ПересчитатьНачисленияСКоэффициентом(Сотрудник, Начисления);
	КонецЕсли;
	
КонецПроцедуры

// Производит заполнение по заданным удержаниям.
// 
// Параметры:
//  Объект                  - ДокументОбъект.Увольнение - исходый документ.
//  ОбъектФормы             - ДокументОбъект.
//
Процедура ЗаполнитьРезультатУдержаний(Объект, ОбъектФормы) Экспорт
...
// Производит перерасчет НДФЛ и взносов.
// 
// Параметры:
//  Объект                  - ДокументОбъект.Увольнение - исходый документ.
//  ОбъектФормы             - ДокументОбъект.
//  ДополнительныеПараметры - Структура - дополнительные параметры из формы документа.
//  НДФЛ                    - Булево, признак пересчета НДФЛ
//  Взносы                  - Булево, признак пересчета взносов
//
Процедура ОбновитьНДФЛиВзносыСотрудника(Объект, ОбъектФормы, НДФЛ = Истина, Взносы = Истина) Экспорт
	...
	Если Не Объект.КорректироватьНДФЛ И НДФЛ Тогда
		ПересчитатьНДФЛ(Объект, ОбъектФормы);
	КонецЕсли;
...
Процедура ПересчитатьНДФЛ(Объект, ОбъектФормы) Экспорт
	...
	УчетЗарплаты.ПересчитатьНДФЛ(Объект, ОбъектФормы, Истина);
	
КонецПроцедуры

// Процедура инициирует перерасчет взносов.
...
//  ОбъектФормы - ДокументОбъект.
//
Процедура ПересчитатьВзносы(Объект, ОбъектФормы) Экспорт
	
	ОбновитьНДФЛиВзносыСотрудника(Объект, ОбъектФормы, Ложь, Истина);
