...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ПлатежноеТребование", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура УстановитьСвойстваПоляСостояние(Форма)
	...
	Элементы.ГруппаПредупреждениеОбОтказеБанка.Видимость = 
		 Форма.Состояние = ПредопределенноеЗначение("Перечисление.СостоянияБанковскихДокументов.Отклонено")
	...
	Если Элементы.ГруппаПредупреждениеОбОтказеБанка.Видимость Тогда
		...
		ДанныеПодсказки.Добавить(Новый ФорматированнаяСтрока(
			НСтр("ru = 'Этот документ был отклонен банком. Для повтора платежа создайте новое платежное требование.'")));
...
Функция ПолучитьСостояниеОбмена(Ссылка)
	...
	Результат.Вставить("Состояние", РегистрыСведений.СостоянияБанковскихДокументов.ТекущееСостояниеДокумента(Ссылка));
