...
Функция ВыручкаПоВидамДеятельности(СтруктураПараметров) Экспорт
	...
	РаздельныйУчетНДСНаСчете19 = УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(
		СтруктураПараметров.Организация, СтруктураПараметров.Дата);
	ПлательщикЕНВД = УчетнаяПолитика.ПлательщикЕНВД(СтруктураПараметров.Организация, СтруктураПараметров.Дата);
	...
	Иначе
		...
		ВидыЦенностейВыручкаНеНаТерриторииРФ = Новый Массив;
		ВидыЦенностейВыручкаНеНаТерриторииРФ.Добавить(Перечисления.ВидыЦенностей.ТоварыФизлицамВЕАЭСЭлектронныеПлощадки);
		ВидыЦенностейВыручкаНеНаТерриторииРФ.Добавить(Перечисления.ВидыЦенностей.УслугиНеНаТерриторииРФ);
		...
		Запрос.УстановитьПараметр("ВидыЦенностейВыручкаНеНаТерриторииРФ", ВидыЦенностейВыручкаНеНаТерриторииРФ);
		...
		|			Организация В (&Организация)
		|				И СтавкаНДС = &СтавкаБезНДС
		|				И ВидЦенности В (&ВидыЦенностейВыручкаНеНаТерриторииРФ)) КАК НДСЗаписиКнигиПродажОбороты
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			Организация В (&Организация)
		|				И СтавкаНДС = &СтавкаБезНДС
		|				И НЕ ВидЦенности В (&ВидыЦенностейВыручкаНеНаТерриторииРФ)
...
Процедура ЗаполнитьТабличныеЧастиПоНДСКРаспределению(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	СтрокиСПустымПоставщиком = НДСКРаспределению.НайтиСтроки(ОтборНеЗаполненногоКонтрагента);
	ТЗПоискаПоставщика = НДСКРаспределению.Скопировать(СтрокиСПустымПоставщиком, "СчетФактура");
	
	Если ТЗПоискаПоставщика.Количество() > 0 Тогда
		КонтрагентПоСчетуФактуре = ПолучитьСписокКонтрагентовПоСчетамФактурам(СтруктураПараметров, ТЗПоискаПоставщика);
		КонтрагентПоСчетуФактуре.Индексы.Добавить("СчетФактура");
	КонецЕсли;
	...
	Для Каждого СтрокаДокумента Из НДСКРаспределению Цикл
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.Поставщик) Тогда		
...
Функция ПолучитьВыборкуИзРегистраНДСРаздельныйУчет(СтруктураПараметров)
	...
	|	НДСРаздельныйУчет.АналитикаУчетаНДС КАК АналитикаУчетаНДС,
	|	НДСРаздельныйУчет.Партия КАК Партия,
	|	НДСРаздельныйУчет.СуммаБезНДС КАК СуммаБезНДС,
	|	НДСРаздельныйУчет.НДС КАК НДС
	|ПОМЕСТИТЬ РаспределенныйВЭтомКварталеНДС
	|ИЗ
	|	РегистрНакопления.НДСРаздельныйУчет КАК НДСРаздельныйУчет
	|ГДЕ
	|	НДСРаздельныйУчет.Организация = &Организация
	...
	|	И НДСРаздельныйУчет.СпособУчетаНДС = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределяется)
	|	И НДСРаздельныйУчет.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НДСРаздельныйУчет.Регистратор ССЫЛКА Документ.РаспределениеНДС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	0,
	|	ВТМодернизации.СуммаБезНДС - ЕСТЬNULL(РаспределенныйВЭтомКварталеНДС.СуммаБезНДС, 0),
	|	ВТМодернизации.НДС - ЕСТЬNULL(РаспределенныйВЭтомКварталеНДС.НДС, 0),
	|	ИСТИНА
	|ИЗ
	|	ВТМодернизации КАК ВТМодернизации
	|		ЛЕВОЕ СОЕДИНЕНИЕ РаспределенныйВЭтомКварталеНДС КАК РаспределенныйВЭтомКварталеНДС
	|		ПО ВТМодернизации.АналитикаУчетаЗатрат = РаспределенныйВЭтомКварталеНДС.АналитикаУчетаЗатрат
	|			И ВТМодернизации.АналитикаУчетаНДС = РаспределенныйВЭтомКварталеНДС.АналитикаУчетаНДС
	|			И ВТМодернизации.Партия = РаспределенныйВЭтомКварталеНДС.Партия
	|ГДЕ
	|	ВТМодернизации.СуммаБезНДС - ЕСТЬNULL(РаспределенныйВЭтомКварталеНДС.СуммаБезНДС, 0) >= 0
	|	И ВТМодернизации.НДС - ЕСТЬNULL(РаспределенныйВЭтомКварталеНДС.НДС, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ РаспределенныйВЭтомКварталеНДС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПодготовитьПараметрыПроведения(Ссылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ПлательщикЕНВД", 
		УчетнаяПолитика.ПлательщикЕНВД(РеквизитыДокумента.Организация, РеквизитыДокумента.Дата));
	...
	Запрос.Текст = ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
		...
		+ ТекстЗапросаТаблицаВключенныйВСтоимостьНДСУСН(НомераТаблиц, ПараметрыПроведения, РеквизитыДокумента);
...
Функция ТекстЗапросаТаблицаРаспределяемыйНДС(НомераТаблиц)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	РаспределяемыйНДС.Ссылка.Организация КАК Организация,
	|	РаспределяемыйНДС.СчетФактура КАК СчетФактура,
	|	РаспределяемыйНДС.Поставщик КАК Поставщик,
	|	РаспределяемыйНДС.СуммаБезНДС КАК СуммаБезНДС,
	|	РаспределяемыйНДС.НДС КАК НДС,
	|	РаспределяемыйНДС.СуммаБезНДСПринимаетсяКВычету КАК СуммаБезНДСПринимаетсяКВычету,
	|	РаспределяемыйНДС.НДСПринимаетсяКВычету КАК НДСПринимаетсяКВычету,
	|	РаспределяемыйНДС.СуммаБезНДСУчитываетсяВCтоимости КАК СуммаБезНДСУчитываетсяВCтоимости,
	|	РаспределяемыйНДС.НДСУчитываетсяВCтоимости КАК НДСУчитываетсяВCтоимости,
	|	РаспределяемыйНДС.СуммаБезНДСДляОперацийПо0 КАК СуммаБезНДСДляОперацийПо0,
	|	РаспределяемыйНДС.НДСДляОперацийПо0 КАК НДСДляОперацийПо0,
	|	РаспределяемыйНДС.ВидЦенности КАК ВидЦенности,
	|	РаспределяемыйНДС.СчетУчетаНДС КАК СчетУчетаНДС,
	|	РаспределяемыйНДС.СтавкаНДС КАК СтавкаНДС,
	|	РаспределяемыйНДС.Партия КАК Партия,
	|	РаспределяемыйНДС.СчетЗатрат КАК СчетЗатрат,
	|	РаспределяемыйНДС.Подразделение КАК Подразделение,
	|	ВЫБОР
	...
	|		ИНАЧЕ РаспределяемыйНДС.Субконто3
	|	КОНЕЦ КАК Субконто3,
	|	РаспределяемыйНДС.Количество КАК Количество,
	...
	|	РаспределяемыйНДС.Модернизация КАК Модернизация,
	|	РаспределяемыйНДС.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура
	|ИЗ
	...
	|	РаспределяемыйНДС.ИсправленныйСчетФактура
	|ИЗ
	|	Документ.РаспределениеНДС.РаспределяемыйНДС КАК РаспределяемыйНДС
	|ГДЕ
	|	РаспределяемыйНДС.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаВключенныйВСтоимостьНДСУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	ПрименяетсяУСНДоходыМинусРасходы = УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(
		Реквизиты.Организация, Реквизиты.Дата);
	ПрименяетсяОбщаяСтавкаНДС = УчетНДС.ПрименяетсяОбщаяСтавкаНДС(
		Реквизиты.Организация, Реквизиты.Дата);
	
	Если Не ПрименяетсяУСНДоходыМинусРасходы Или Не ПрименяетсяОбщаяСтавкаНДС Тогда
		ПараметрыПроведения.Вставить("ТаблицаВключенныйВСтоимостьНДСУСН", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВременнаяТаблицаРаспределениеНДС", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВременнаяТаблицаПоступлениеРасходовПоУслугам", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВременнаяТаблицаИзменениеАналитикиЗатрат", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВременнаяТаблицаИзменениеАналитикиЗатратИтог", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВременнаяТаблицаВключенныйВСтоимостьНДС", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ТаблицаВключенныйВСтоимостьНДСУСН", НомераТаблиц.Количество());
	...
	|	РаспределениеНДСРаспределяемыйНДС.Партия КАК Партия,
	|	РаспределениеНДСРаспределяемыйНДС.СчетЗатрат КАК СчетЗатрат,
	|	РаспределениеНДСРаспределяемыйНДС.Субконто1 КАК Субконто1,
	...
	|	РаспределениеНДСРаспределяемыйНДС.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	СУММА(РаспределениеНДСРаспределяемыйНДС.НДС) КАК НДС,
	|	СУММА(РаспределениеНДСРаспределяемыйНДС.НДСУчитываетсяВCтоимости) КАК НДСУчитываетсяВCтоимости
	|ПОМЕСТИТЬ ВТ_РаспределениеНДС
	|ИЗ
	|	Документ.РаспределениеНДС КАК РаспределениеНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеНДС.РаспределяемыйНДС КАК РаспределениеНДСРаспределяемыйНДС
	|		ПО РаспределениеНДС.Ссылка = РаспределениеНДСРаспределяемыйНДС.Ссылка
	|ГДЕ
	|	РаспределениеНДС.Ссылка = &Ссылка
	|	И РаспределениеНДСРаспределяемыйНДС.НДСУчитываетсяВCтоимости > 0
	|	И (РаспределениеНДСРаспределяемыйНДС.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Материалы)
	|			ИЛИ РаспределениеНДСРаспределяемыйНДС.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары)
	|			ИЛИ РаспределениеНДСРаспределяемыйНДС.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги))
	|
	|СГРУППИРОВАТЬ ПО
	|	РаспределениеНДС.Организация,
	|	РаспределениеНДСРаспределяемыйНДС.СчетФактура,
	|	РаспределениеНДСРаспределяемыйНДС.Поставщик,
	|	РаспределениеНДСРаспределяемыйНДС.Партия,
	|	РаспределениеНДСРаспределяемыйНДС.ВидЦенности,
	|	РаспределениеНДСРаспределяемыйНДС.СчетУчетаНДС,
	|	РаспределениеНДСРаспределяемыйНДС.СтавкаНДС,
	|	РаспределениеНДСРаспределяемыйНДС.СчетЗатрат,
	|	РаспределениеНДСРаспределяемыйНДС.Субконто1,
	|	РаспределениеНДСРаспределяемыйНДС.ИсправленныйСчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасходыПриУСНОбороты.Организация КАК Организация,
	|	РасходыПриУСНОбороты.РасчетныйДокумент КАК РасчетныйДокумент,
	|	РасходыПриУСНОбороты.ЭлементРасхода КАК ЭлементРасхода,
	|	РасходыПриУСНОбороты.СчетУчета КАК СчетУчета,
	|	РасходыПриУСНОбороты.НДСПриход КАК НДСПриход
	|ПОМЕСТИТЬ ВТ_ПоступлениеРасходовПоУслугам
	|ИЗ
	...
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ВТ_РаспределениеНДС.Партия
	|					ИЗ
	|						ВТ_РаспределениеНДС КАК ВТ_РаспределениеНДС
	|					ГДЕ
	|						ВТ_РаспределениеНДС.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги))
	...
	|	НДСРаздельныйУчет.НДС КАК НДС
	|ПОМЕСТИТЬ ВТ_ИзмененияАналитикиОбороты
	|ИЗ
	|	РегистрНакопления.НДСРаздельныйУчет КАК НДСРаздельныйУчет
	|ГДЕ
	|	НДСРаздельныйУчет.Организация = &Организация
	|	И НДСРаздельныйУчет.Партия В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ВТ_РаспределениеНДС.Партия
	|			ИЗ
	|				ВТ_РаспределениеНДС КАК ВТ_РаспределениеНДС
	...
	|	И НДСРаздельныйУчет.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И (НДСРаздельныйУчет.СпособУчетаНДС = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределяется)
	|			ИЛИ НДСРаздельныйУчет.СпособУчетаНДСПриВыбытии = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределяется))
	|	И НДСРаздельныйУчет.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|	И НЕ НДСРаздельныйУчет.Регистратор ССЫЛКА Документ.РаспределениеНДС
	|	И НДСРаздельныйУчет.Активность
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ИзмененияАналитикиОбороты.Партия КАК Партия,
	|	РегистрАналитикаУчетаНДС.СчетФактура КАК СчетФактура,
	|	РегистрАналитикаУчетаНДС.ВидЦенности КАК ВидЦенности,
	|	РегистрАналитикаУчетаНДС.СчетУчетаНДС КАК СчетУчетаНДС,
	|	РегистрАналитикаУчетаНДС.СтавкаНДС КАК СтавкаНДС,
	|	РегистрАналитикаУчетаНДС.Поставщик КАК Поставщик,
	|	РегистрАналитикаУчетаНДС.ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	РегистрАналитикаУчетаЗатрат.СчетЗатрат КАК СчетЗатрат,
	|	РегистрАналитикаУчетаЗатрат.Субконто1 КАК Субконто1,
	|	ВТ_ИзмененияАналитикиОбороты.НДС КАК НДС
	|ПОМЕСТИТЬ ВТ_ИзменениеАналитики
	|ИЗ
	|	ВТ_ИзмененияАналитикиОбороты КАК ВТ_ИзмененияАналитикиОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаЗатрат КАК РегистрАналитикаУчетаЗатрат
	|		ПО ВТ_ИзмененияАналитикиОбороты.АналитикаУчетаЗатрат = РегистрАналитикаУчетаЗатрат.КлючАналитики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНДС КАК РегистрАналитикаУчетаНДС
	|		ПО ВТ_ИзмененияАналитикиОбороты.АналитикаУчетаНДС = РегистрАналитикаУчетаНДС.КлючАналитики
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_РаспределениеНДС.Организация КАК Организация,
	|	ВТ_РаспределениеНДС.СчетФактура КАК СчетФактура,
	|	ВТ_РаспределениеНДС.Поставщик КАК Поставщик,
	|	ВТ_РаспределениеНДС.Партия КАК Партия,
	|	ВТ_РаспределениеНДС.СчетЗатрат КАК СчетЗатрат,
	|	ВТ_РаспределениеНДС.Субконто1 КАК Субконто1,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Номенклатура) КАК ВидРасхода,
	|	ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписаноПринято) КАК СтатусыОплатыРасходовУСН,
	|	ВТ_РаспределениеНДС.НДС КАК НДС,
	|	ВТ_РаспределениеНДС.НДСУчитываетсяВCтоимости КАК НДСУчитываетсяВCтоимости
	|ПОМЕСТИТЬ ВТ_ВключенныйВСтоимостьНДС
	|ИЗ
	|	ВТ_РаспределениеНДС КАК ВТ_РаспределениеНДС
	|ГДЕ
	|	ВТ_РаспределениеНДС.Субконто1 ССЫЛКА Справочник.Номенклатура
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТ_РаспределениеНДС.Организация,
	|	ВТ_РаспределениеНДС.СчетФактура,
	|	ВТ_РаспределениеНДС.Поставщик,
	|	ВТ_РаспределениеНДС.Партия,
	|	ВТ_ИзменениеАналитики.СчетЗатрат,
	|	ВТ_ИзменениеАналитики.Субконто1,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Номенклатура),
	|	ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеСписаноПринято),
	|	ВТ_ИзменениеАналитики.НДС,
	|	ВЫБОР
	|		КОГДА ВТ_РаспределениеНДС.НДС <> 0
	|			ТОГДА ВТ_РаспределениеНДС.НДСУчитываетсяВCтоимости * ВТ_ИзменениеАналитики.НДС / ВТ_РаспределениеНДС.НДС
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|ИЗ
	|	ВТ_РаспределениеНДС КАК ВТ_РаспределениеНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ИзменениеАналитики КАК ВТ_ИзменениеАналитики
	|		ПО ВТ_РаспределениеНДС.СчетФактура = ВТ_ИзменениеАналитики.СчетФактура
	|			И ВТ_РаспределениеНДС.ВидЦенности = ВТ_ИзменениеАналитики.ВидЦенности
	|			И ВТ_РаспределениеНДС.СчетУчетаНДС = ВТ_ИзменениеАналитики.СчетУчетаНДС
	|			И ВТ_РаспределениеНДС.СтавкаНДС = ВТ_ИзменениеАналитики.СтавкаНДС
	|			И ВТ_РаспределениеНДС.Поставщик = ВТ_ИзменениеАналитики.Поставщик
	|			И ВТ_РаспределениеНДС.Партия = ВТ_ИзменениеАналитики.Партия
	|			И ВТ_РаспределениеНДС.ИсправленныйСчетФактура = ВТ_ИзменениеАналитики.ИсправленныйСчетФактура
	|			И (НЕ ВТ_РаспределениеНДС.Субконто1 ССЫЛКА Справочник.Номенклатура)
	|			И (ВТ_ИзменениеАналитики.Субконто1 ССЫЛКА Справочник.Номенклатура)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВТ_РаспределениеНДС.Поставщик,
	|	ВТ_РаспределениеНДС.Партия,
	|	ВТ_ПоступлениеРасходовПоУслугам.СчетУчета,
	|	ВТ_ПоступлениеРасходовПоУслугам.ЭлементРасхода,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыРасходовУСН.Услуги),
	|	ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеОплачено),
	|	ВТ_ПоступлениеРасходовПоУслугам.НДСПриход,
	|	ВЫБОР
	|		КОГДА ВТ_РаспределениеНДС.НДС <> 0
	|			ТОГДА ВТ_РаспределениеНДС.НДСУчитываетсяВCтоимости * ВТ_ПоступлениеРасходовПоУслугам.НДСПриход / ВТ_РаспределениеНДС.НДС
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|ИЗ
	|	ВТ_РаспределениеНДС КАК ВТ_РаспределениеНДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПоступлениеРасходовПоУслугам КАК ВТ_ПоступлениеРасходовПоУслугам
	|		ПО ВТ_РаспределениеНДС.Организация = ВТ_ПоступлениеРасходовПоУслугам.Организация
	|			И ВТ_РаспределениеНДС.Партия = ВТ_ПоступлениеРасходовПоУслугам.РасчетныйДокумент
	|			И ВТ_РаспределениеНДС.СчетЗатрат = ВТ_ПоступлениеРасходовПоУслугам.СчетУчета
	|			И ВТ_РаспределениеНДС.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ВключенныйВСтоимостьНДС.Организация КАК Организация,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетФактура КАК РасчетныйДокумент,
	|	ВТ_ВключенныйВСтоимостьНДС.Поставщик КАК Поставщик,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетФактура.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВТ_ВключенныйВСтоимостьНДС.Партия КАК Партия,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетЗатрат КАК СчетУчета,
	|	ВТ_ВключенныйВСтоимостьНДС.Субконто1 КАК ЭлементРасхода,
	|	ВТ_ВключенныйВСтоимостьНДС.ВидРасхода КАК ВидРасхода,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	0 КАК НомерСтрокиДокумента,
	|	ВТ_ВключенныйВСтоимостьНДС.СтатусыОплатыРасходовУСН КАК СтатусыОплатыРасходовУСН,
	|	ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.Принимаются) КАК ОтражениеВУСН,
	|	СУММА(ВТ_ВключенныйВСтоимостьНДС.НДС) КАК НДС,
	|	СУММА(ВТ_ВключенныйВСтоимостьНДС.НДСУчитываетсяВCтоимости) КАК НДСУчитываетсяВCтоимости
	|ИЗ
	|	ВТ_ВключенныйВСтоимостьНДС КАК ВТ_ВключенныйВСтоимостьНДС
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ВключенныйВСтоимостьНДС.Организация,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетФактура,
	|	ВТ_ВключенныйВСтоимостьНДС.Поставщик,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетФактура.ДоговорКонтрагента,
	|	ВТ_ВключенныйВСтоимостьНДС.Партия,
	|	ВТ_ВключенныйВСтоимостьНДС.СчетЗатрат,
	|	ВТ_ВключенныйВСтоимостьНДС.Субконто1,
	|	ВТ_ВключенныйВСтоимостьНДС.ВидРасхода,
	|	ВТ_ВключенныйВСтоимостьНДС.СтатусыОплатыРасходовУСН";
