...
Процедура ВводКомплектовДокументов(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы") Тогда  
		...
		ПараметрыОткрытия.Заголовок = НСтр("ru = 'Список комплектов'");
...
Процедура Подключаемый_ОбновитьСписок()
	...
	Элементы.СписокФильтрСтатусВОбработке.Заголовок = СтрШаблон(НСтр("ru = 'В обработке: %1'"), СостоянияПоСтатусам.ФайловКОбработке);
	...
	Элементы.СписокФильтрСтатусОбработан.Заголовок = СтрШаблон(НСтр("ru = 'Обработанные: %1'"), СостоянияПоСтатусам.ДокументовОбработано);
