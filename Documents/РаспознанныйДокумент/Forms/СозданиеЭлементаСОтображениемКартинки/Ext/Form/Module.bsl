...
Процедура ПроверитьИННКПП(ПроверитьИНН, ПроверитьКПП)
	...
	Если ПроверитьИНН Тогда
		...
		РезультатПроверкиИНН = Новый ФорматированнаяСтрока(РезультатПроверки.ОписаниеОшибки,, ЦветаСтиля.ЦветВыделенияКонтрагентаСОшибкой);
	...
	Если ПроверитьКПП Тогда
		...
		РезультатПроверкиКПП = Новый ФорматированнаяСтрока(РезультатПроверки.ОписаниеОшибки,, ЦветаСтиля.ЦветВыделенияКонтрагентаСОшибкой);
...
Функция ПолучитьДвоичныеДанныеКартинки(ДокументОбъект, ИмяТаблицы, ПараметрыСвязаннойКолонки)
	...
	Если ЗначениеЗаполнено(СтрокаРеквизитов.ОбластьИзображения) Тогда
	...
	Иначе
		...
		СтрокаРеквизитов.КоординатаX0,
		СтрокаРеквизитов.КоординатаY0,
		СтрокаРеквизитов.КоординатаX1,
		СтрокаРеквизитов.КоординатаY1);
