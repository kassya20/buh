...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаписатьИзмененияРеквизитов", Истина);
	Методы.Вставить("ЗаписатьИзмененияРеквизитовТаблицы", Истина);
...
Функция СоздатьРаспознанныйДокументСОшибкой(ДанныеСервиса, ДанныеВыборки, РезультатыОбработки, ОписаниеОшибки = Неопределено, ДатаСоздания = Неопределено, ДанныеСтраниц = Неопределено) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			РаспознаваниеДокументов.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Ошибка, , ,
			НСтр("ru = 'Не удалось записать документ по причине:'") + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())
		);
		Возврат Неопределено;
	КонецПопытки;
	...
	Для Каждого ТекРеквизит Из СтрокиОтбора Цикл
		...
		Для Каждого ТекСоотв Из СоответствиеJSON Цикл
			...
			ИначеЕсли ТекСоотв.Ключ = "bbox" Тогда
				...
				КонецЕсли;
			ИначеЕсли ТекСоотв.Ключ = "refined_bbox" Тогда
				Если ТипЗнч(ТекСоотв.Значение) = Тип("Массив") И ТекСоотв.Значение.Количество() = 4 Тогда
					Координаты = Новый Структура;
					Координаты.Вставить("КоординатаX0", ТекСоотв.Значение[0]);
					Координаты.Вставить("КоординатаY0", ТекСоотв.Значение[1]);
					Координаты.Вставить("КоординатаX1", ТекСоотв.Значение[2]);
					Координаты.Вставить("КоординатаY1", ТекСоотв.Значение[3]);
					СтрТабРек.КоординатыТекста = Новый ХранилищеЗначения(Координаты);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Функция СоздатьРаспознанныйДокументV2(СвойстваРаспознанногоДокумента, РаспознанныйДокумент, ИдентификаторЗадания, СостояниеЗадания) Экспорт
	...
	Для Каждого Строка Из НовыйДокумент.РеквизитыТабличныхЧастей Цикл
		Если Строка.ПорядокСтроки = 0 Тогда
			Строка.ПорядокСтроки = Строка.НомерСтрокиТЧ;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьЗначенияРеквизитаV2(
	...
	Если ЗначениеЗаполнено(Сущность.ОбластьТекста) Тогда
		Области = СтрРазделить(Сущность.ОбластьТекста, ",");
