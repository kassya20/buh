...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("НеТребуетсяЗаполнятьСчетаУчета");
	Методы.Вставить("НеТребуетсяЗаполнятьНастройкиУчетаНДС");
	Методы.Вставить("НеТребуетсяЗаполнятьСтавкуНДСВТаблицеБилетов");
	Методы.Вставить("ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам");
	Методы.Вставить("ЗаполнитьСтавкуНДСВТаблицеБилетов");
	Методы.Вставить("ЗаполнитьАналитикуУчетаКомандировочныхРасходов");
...
Функция РеквизитыЗаСсылками(ВидОперации) Экспорт
	...
	// Прячутся за "НадписьБилеты" (командировка) или за ссылкой в колонке "УчетНДС" (обычный АО)
	АналитикаБилеты = Новый Массив;
	
	АналитикаБилеты.Добавить("Билеты.Билет");
	АналитикаБилеты.Добавить("Билеты.Сумма");	
	...
	Если ВидОперации = Перечисления.ВидыОперацийАвансовыйОтчет.Командировка Тогда
		...
		// Прячется за ссылкой в колонке "УчетНДС". Этот реквизит проверяется механизмом заполнения счетов учета.
		// Другие реквизиты, спрятанные за той же ссылкой, проверяются в модуле АО по условиям.
		РеквизитыЗаСсылками.Вставить("Прочее.СпособУчетаНДС", "УчетНДС");	
	...
	Иначе
		...
		// Прячутся за ссылкой в колонке "УчетНДС"
		Для Каждого ИмяРеквизита Из АналитикаБилеты Цикл
			РеквизитыЗаСсылками.Вставить(ИмяРеквизита, "УчетНДС");
		КонецЦикла;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ОсновнаяСтавкаНДСПоБилетам", УчетНДСКлиентСервер.ОсновнаяСтавкаНДСПоБилетам(Реквизиты.Период));
	...
	СтруктураТаблицДокумента.Вставить("ПогашениеЗадолженностиТаблица",  ПараметрыПроведения.ПогашениеЗадолженностиТаблица);
...
Функция ТекстЗапросаВалютныеАвансыПодотчетногоЛица(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЕстьАвансы Тогда
		...
		|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, АвансовыйОтчетВыданныеАвансы.ДокументАванса.Дата) КАК ДатаДокументаРасчетов,
		...
		|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, АвансовыйОтчетВыданныеАвансы.ДокументАванса.Дата),
		|	ВЫБОР
		|		КОГДА Реквизиты.ВалютаДокумента = &ВалютаРеглУчета
		|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицами)
		|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицамиВал)
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьОплаты Тогда
		...
		|	ТаблицаОплата.Сумма КАК СуммаРуб,
	...
	Если Реквизиты.ЕстьБилеты Тогда
	   ...
	   |	КОНЕЦ КАК СуммаНДС,
	   |	ТаблицаБилеты.ПредъявленСФ КАК ПредъявленСчетФактура,
	   |	ВЫБОР
	   |		КОГДА НЕ &РаздельныйУчетНДСНа19Счете
	   |		И (ТаблицаБилеты.ПредъявленСФ
	   |		ИЛИ НЕ &ПлательщикНДС)
	   |		И Реквизиты.НДСВключенВСтоимость
	   |			ТОГДА ИСТИНА
	   |		ИНАЧЕ ЛОЖЬ
...
Процедура ПодготовитьТаблицыДокументаРасчетыВВалюте(Запрос, Реквизиты)
	...
	Если Реквизиты.ЕстьОплаты Тогда
		СуммыТаблицыОплаты = Результат[НомераТаблиц["СуммыТаблицыОплаты"]].Выгрузить();
		УчетВзаиморасчетов.ПодготовитьТаблицуДокументаРасчетыВВалюте(СуммыТаблицыОплаты, Реквизиты);
		Запрос.УстановитьПараметр("СуммыТаблицыОплаты", СуммыТаблицыОплаты);
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаРасчетыВВалютеОплаты();
	КонецЕсли;
...
Функция ТекстЗапросаРасчетыВВалютеОплаты()
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	СуммыТаблицыОплаты.НомерСтроки КАК НомерСтроки,
	|	СуммыТаблицыОплаты.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	СуммыТаблицыОплаты.СуммаНДСВзаиморасчетов КАК СуммаНДСВзаиморасчетов,
	|	СуммыТаблицыОплаты.СуммаРуб КАК СуммаРуб,
	|	СуммыТаблицыОплаты.СуммаНДСРуб КАК СуммаНДСРуб
	|ПОМЕСТИТЬ СуммыТаблицыОплаты
	|ИЗ
	|	&СуммыТаблицыОплаты КАК СуммыТаблицыОплаты
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	НомерСтроки
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаОплата.Ссылка КАК Ссылка,
	|	ВременнаяТаблицаОплата.НомерСтроки КАК НомерСтроки,
	|	ВременнаяТаблицаОплата.ВидДокВходящий КАК ВидДокВходящий,
	|	ВременнаяТаблицаОплата.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|	ВременнаяТаблицаОплата.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ВременнаяТаблицаОплата.СуммаПлатежа КАК СуммаПлатежа,
	|	СуммыТаблицыОплаты.СуммаРуб КАК СуммаРуб,
	|	ВременнаяТаблицаОплата.Контрагент КАК Контрагент,
	|	ВременнаяТаблицаОплата.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВременнаяТаблицаОплата.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ВременнаяТаблицаОплата.ВидДоговора КАК ВидДоговора,
	|	ВременнаяТаблицаОплата.РасчетыВУсловныхЕдиницах КАК РасчетыВУсловныхЕдиницах,
	|	ВременнаяТаблицаОплата.Содержание КАК Содержание,
	|	СуммыТаблицыОплаты.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	ВременнаяТаблицаОплата.СчетУчетаРасчетовСКонтрагентом КАК СчетУчетаРасчетовСКонтрагентом,
	|	ВременнаяТаблицаОплата.КурсВзаиморасчетов КАК КурсВзаиморасчетов,
	|	ВременнаяТаблицаОплата.КратностьВзаиморасчетов КАК КратностьВзаиморасчетов,
	|	ВременнаяТаблицаОплата.СчетУчетаРасчетовПоАвансам КАК СчетУчетаРасчетовПоАвансам,
	|	ВременнаяТаблицаОплата.Сделка КАК Сделка,
	|	ВременнаяТаблицаОплата.СпособПогашенияЗадолженности КАК СпособПогашенияЗадолженности,
	|	ВременнаяТаблицаОплата.РеквизитыВалютаДокумента КАК РеквизитыВалютаДокумента,
	|	ВременнаяТаблицаОплата.РеквизитыПодразделениеОрганизации КАК РеквизитыПодразделениеОрганизации,
	|	ВременнаяТаблицаОплата.РеквизитыФизЛицо КАК РеквизитыФизЛицо,
	|	ВременнаяТаблицаОплата.РеквизитыДата КАК РеквизитыДата
	|ПОМЕСТИТЬ ТаблицаОплата
	|ИЗ
	|	ВременнаяТаблицаОплата КАК ВременнаяТаблицаОплата
	|		ЛЕВОЕ СОЕДИНЕНИЕ СуммыТаблицыОплаты КАК СуммыТаблицыОплаты
	|		ПО ВременнаяТаблицаОплата.НомерСтроки = СуммыТаблицыОплаты.НомерСтроки";
...
Функция ТекстЗапросаВременныеТаблицыДокументаРасчетыВВалюте(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьОплаты Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПОМЕСТИТЬ ТаблицаОплата", "ПОМЕСТИТЬ ВременнаяТаблицаОплата");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаОплата.Ссылка = &Ссылка", "ТаблицаОплата.Ссылка = &Ссылка
		|
		|ИНДЕКСИРОВАТЬ ПО
		|НомерСтроки");
		НомераТаблиц.Вставить("СуммыТаблицыОплаты", НомераТаблиц.Количество());
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	ВременнаяТаблицаОплата.НомерСтроки,
		|	НЕОПРЕДЕЛЕНО КАК СтавкаНДС,
		|	ВременнаяТаблицаОплата.СуммаВзаиморасчетов,
		|	0 КАК СуммаНДСВзаиморасчетов,
		|	ВременнаяТаблицаОплата.СуммаРуб,
		|	0 КАК СуммаНДСРуб
		|ИЗ
		|	ВременнаяТаблицаОплата КАК ВременнаяТаблицаОплата"
		+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаПогашениеЗадолженности(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаОплата.СуммаРуб КАК СуммаНУ,
	|	0 КАК СуммаБезНДСРуб,
	|	0 КАК СуммаНДСРуб,
	|	0 КАК СуммаНДСВзаиморасчетов,
	|	ТаблицаОплата.ВидДокВходящий,
...
Функция ТекстЗапросаПоступлениеРасходовУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.СуммаСуточных > 0 Тогда
		...
		|	0 КАК НДС,
		|	ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПустаяСсылка) КАК СпособУчетаНДС,
...
Функция ТекстЗапросаБилеты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
   ...
   |	ТаблицаБилеты.СчетФактура КАК СчетФактура,
   |	ТаблицаБилеты.ПредъявленСчетФактура КАК ПредъявленСчетФактура,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.СписокФорм          = "ФормаДокумента,ФормаДокументаКомандировка";
	...
	// Реестр билетов
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "РеестрБилетов";
	КомандаПечати.Представление = НСтр("ru = 'Реестр билетов'");
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
...
Функция ПодготовитьТекстЗапросаДляПечатиСправкиРасчетаРублевыеСуммыДокументовВВалюте(НомераТаблиц) Экспорт
	...
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ОтчетыВал.Ссылка,
	|	ВТ_ОтчетыВал.Организация,
	|	ВТ_ОтчетыВал.Документ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ,
	|	Организация
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм,ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	КонецЕсли;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрБилетов") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
			"РеестрБилетов",
			"Реестр билетов",
			ПечатьРеестраБилетов(МассивОбъектов, ОбъектыПечати),
			,
			"Документ.АвансовыйОтчет.ПФ_MXL_РеестрБилетов");
	КонецЕсли;
...
// Возвращает табличный документ с печатными формами реестров билетов
//
// Параметры:
...
//
// Возвращаемое значение:
//   ТабличныйДокумент   - печатные формы реестров
//
Функция ПечатьРеестраБилетов(МассивОбъектов, ОбъектыПечати)
	
	ОбщегоНазначенияБП.ЗаписатьОперациюБизнесСтатистики("АвансовыйОтчет.ПечатьРеестраБилетов");
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.АвтоМасштаб          = Истина;
	ТабличныйДокумент.ОриентацияСтраницы   = ОриентацияСтраницы.Портрет;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_АвансовыйОтчет_РеестрБилетов";
	...
	|	АвансовыйОтчет.СуммаВключаетНДС КАК СуммаВключаетНДС,
	|	АвансовыйОтчет.ФизЛицо КАК ПодотчетноеЛицо,
	|	АвансовыйОтчетБилеты.НомерСтроки КАК НомерСтроки,
	|	АвансовыйОтчетБилеты.Билет КАК Билет,
	|	АвансовыйОтчетБилеты.Билет.Код КАК БилетКод,
	|	АвансовыйОтчетБилеты.Билет.ДатаПокупки КАК БилетДатаПокупки,
	|	АвансовыйОтчетБилеты.Билет.Маршрут КАК БилетМаршрут,
	|	АвансовыйОтчетБилеты.Билет.ДатаОтправления КАК БилетДатаОтправления,
	|	АвансовыйОтчетБилеты.Билет.Перевозчик.НаименованиеПолное КАК БилетПеревозчик,
	|	АвансовыйОтчетБилеты.Сумма КАК Сумма,
	|	АвансовыйОтчетБилеты.СуммаНДС КАК СуммаНДС
	|ИЗ
	|	Документ.АвансовыйОтчет КАК АвансовыйОтчет
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.АвансовыйОтчет.ПФ_MXL_РеестрБилетов");
	// Области
	ОбластьПриложения    = Макет.ПолучитьОбласть("Приложение");
	...
	// Параметры
	ПараметрыПриложения  = Новый Структура("НомерДокумента,ДатаДокумента");
	ПараметрыШапки       = Новый Структура("НДС");
	ПараметрыСтроки      = Новый Структура;
	ПараметрыСтроки.Вставить("НомерСтроки");
	ПараметрыСтроки.Вставить("БилетКод");
	ПараметрыСтроки.Вставить("БилетДатаПокупки");
	ПараметрыСтроки.Вставить("БилетДатаОтправления");
	ПараметрыСтроки.Вставить("БилетМаршрут");
	ПараметрыСтроки.Вставить("БилетПеревозчик");
	ПараметрыСтроки.Вставить("Сумма");
	ПараметрыСтроки.Вставить("СуммаНДС");
	ПараметрыПодвала     = Новый Структура("Сумма,СуммаНДС");
	ПараметрыПодписи     = Новый Структура("ПредставлениеОрганизации,ПредставлениеПодотчетногоЛица");
	
	ЗаголовокНДС         = НСтр("ru='В том числе НДС'");
	ЗаголовокНДССверху   = НСтр("ru='НДС сверху'");
	ФорматСуммы          = НСтр("ru='ЧДЦ=2; ЧРГ='");
	ФорматДаты           = НСтр("ru='ДФ=''dd.MM.yyyy ""г.""'''");
	...
	Пока Шапка.Следующий() Цикл
		...
		// Приложение
		НомерДокументаДляПечати = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Шапка.Номер, Истина, Ложь);
		ПараметрыПриложения.НомерДокумента = НомерДокументаДляПечати;
		ПараметрыПриложения.ДатаДокумента = Формат(Шапка.ДатаДокумента, ФорматДаты);
		ОбластьПриложения.Параметры.Заполнить(ПараметрыПриложения);
		ТабличныйДокумент.Вывести(ОбластьПриложения);
		// Заголовок
		ТабличныйДокумент.Вывести(ОбластьЗаголовка);
		// Шапка таблицы
		ПараметрыШапки.НДС = ?(Шапка.СуммаВключаетНДС, ЗаголовокНДС, ЗаголовокНДССверху);
		ОбластьШапки.Параметры.Заполнить(ПараметрыШапки);
		ТабличныйДокумент.Вывести(ОбластьШапки);
		
		Билет = Шапка.Выбрать();
		Пока Билет.Следующий() Цикл 
			
			ПараметрыСтроки.НомерСтроки           = Билет.НомерСтроки;
			ПараметрыСтроки.БилетКод              = Билет.БилетКод;
			ПараметрыСтроки.БилетДатаПокупки      = Билет.БилетДатаПокупки;
			ПараметрыСтроки.БилетДатаОтправления  = Билет.БилетДатаОтправления;
			ПараметрыСтроки.БилетМаршрут          = Билет.БилетМаршрут;
			ПараметрыСтроки.БилетПеревозчик       = Билет.БилетПеревозчик;
			ПараметрыСтроки.Сумма                 = Формат(Билет.Сумма, ФорматСуммы);
			ПараметрыСтроки.СуммаНДС              = Формат(Билет.СуммаНДС, ФорматСуммы);
		...
		// Подвал таблицы
		ПараметрыПодвала.Сумма     = Формат(Шапка.Сумма, ФорматСуммы);
		ПараметрыПодвала.СуммаНДС  = Формат(Шапка.СуммаНДС, ФорматСуммы);
		
		ОбластьПодвала.Параметры.Заполнить(ПараметрыПодвала);
		ТабличныйДокумент.Вывести(ОбластьПодвала);
		
		СведенияОбОрганизации = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
		Шапка.Организация, Шапка.ДатаДокумента);
		
		// Подпись
		ПараметрыПодписи.ПредставлениеОрганизации = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(
		СведенияОбОрганизации, "НаименованиеДляПечатныхФорм");
		
		ДанныеПодотчетногоЛица = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(
		Шапка.Организация, Шапка.ПодотчетноеЛицо, Шапка.ДатаДокумента);
		
		ПараметрыПодписи.ПредставлениеПодотчетногоЛица = ДанныеПодотчетногоЛица.Представление;
		
		ОбластьПодписи.Параметры.Заполнить(ПараметрыПодписи);
...
Функция РеквизитыНеРедактируемыеВГрупповойОбработке() Экспорт
	...
	МассивРеквизитов.Добавить("ВыданныеАвансы.ДокументАванса");
...
// Процедура - обработчик обновления
// Находит авансовые отчеты, в которых не указаны документы подтверждения НДС в таблицах прочих расходов и билетов,
// и заполняет необходимые реквизиты документов.
//
// Параметры:
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам(Параметры) Экспорт
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	РасходыКОбработке.Ссылка КАК Ссылка
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		РасходыСотрудника.Ссылка КАК Ссылка,
	|		РасходыСотрудника.Ссылка.Дата КАК Дата
	|	ИЗ
	|		Документ.АвансовыйОтчет.Прочее КАК РасходыСотрудника
	|	ГДЕ
	|		РасходыСотрудника.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийАвансовыйОтчет.Командировка)
	|		И НЕ РасходыСотрудника.Ссылка.НДСВключенВСтоимость
	|		И РасходыСотрудника.ВидДокументаРасхода = ЗНАЧЕНИЕ(Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.ПустаяСсылка)
	|
	|	ОБЪЕДИНИТЬ
	|
	|	ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		РасходыБилеты.Ссылка,
	|		РасходыБилеты.Ссылка.Дата
	|	ИЗ
	|		Документ.АвансовыйОтчет.Билеты КАК РасходыБилеты
	|	ГДЕ
	|		НЕ РасходыБилеты.Ссылка.НДСВключенВСтоимость
	|		И НЕ РасходыБилеты.ПредъявленСФ
	|		И (РасходыБилеты.СуммаНДС <> 0
	|		ИЛИ РасходыБилеты.СуммаНДС = 0
	|		И РасходыБилеты.СпособУчетаНДС <> ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Списывается))) КАК РасходыКОбработке
	|УПОРЯДОЧИТЬ ПО
	|	РасходыКОбработке.Дата УБЫВ";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДокументОбъект.Номер, Истина, Истина);
			
			// Таблицу Прочее обрабатываем только для командировок. Для обычных АО нельзя сказать, 
			// что в таблице Прочее только командировочные расходы.
			Если ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийАвансовыйОтчет.Командировка Тогда
			
				Для каждого СтрокаРасхода Из ДокументОбъект.Прочее Цикл
					
					// Т.к. колонка ВидДокументаРасхода новая, то считаем, что у необработанных документов она неизбежно пустая.
					// В ином случае документ уже был обработан.
					Если ЗначениеЗаполнено(СтрокаРасхода.ВидДокументаРасхода) Тогда
						Продолжить;
					КонецЕсли;	
					
					// До текущих обновлений в АО по командировке все суммы НДС считались принимаемыми к вычету. При этом 
					// номер и дата входящего документа принимались за дату и номер СФ. Для совместимости с таким алгоритмом
					// все существующие в ИБ командировочные расходы с выделенным НДС теперь считаются билетами, 
					// т.к. в текущей логике билеты учитываются аналогично.
					// Расходы без НДС считаются подтвержденными другим документом. 
					Если СтрокаРасхода.СуммаНДС = 0 Тогда
						СтрокаРасхода.ВидДокументаРасхода = Перечисления.ВидыДокументовПодтверждающихКомандировочныеРасходы.ДругойДокумент;
					Иначе
						СтрокаРасхода.ВидДокументаРасхода = Перечисления.ВидыДокументовПодтверждающихКомандировочныеРасходы.Билет;				
						// В графе 7 (дата и номер документа уплаты налога) ранее указывались реквизиты самого авансового отчета, 
						// поэтому для совместимости также оставляем эти данные. 
						СтрокаРасхода.НомерДокументаОплаты = НомерДокумента;
						СтрокаРасхода.ДатаДокументаОплаты = ДокументОбъект.Дата;
					КонецЕсли;
					
					// До текущих обновлений в командировке способ учета НДС всегда  автоматически выставлялся в значение 
					// "Принимается к вычету", даже если флаг ПредъявленСФ выставлялся в значение Ложь.
					Если Не СтрокаРасхода.ПредъявленСФ Тогда
						СтрокаРасхода.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Списывается;
					КонецЕсли;							
			...
			Для каждого СтрокаБилета Из ДокументОбъект.Билеты Цикл
				...
				// Т.к. колонка ПредъявленСФ новая, то считаем, что у необработанных документов в ней неизбежно Ложь.
				// В ином случае документ уже был обработан.				
				Если СтрокаБилета.ПредъявленСФ Тогда
					Продолжить;
				КонецЕсли;	
				
				// Если нет суммы НДС, то флаг ПредъявленСФ не взводим.
				// Но при этом способ учета НДС должен быть "Списывается"				
				Если СтрокаБилета.СуммаНДС = 0 Тогда
					СтрокаБилета.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Списывается;
					Продолжить;
				КонецЕсли;					
				
				// До текущих изменений (добавление колонки ПредъявленСФ) все суммы НДС по билетам, приобретенным организацией, 
				// принимались к вычету. Для сохранения совместимости с прежней версией взводим безусловно флаг ПредъявленСФ.
				СтрокаБилета.ПредъявленСФ = Истина;
				// В графе 7 (дата и номер документа уплаты налога) ранее указывались реквизиты самого авансового отчета, 
				// поэтому для совместимости также оставляем эти данные. 
				СтрокаБилета.НомерДокументаОплаты = НомерДокумента;
				СтрокаБилета.ДатаДокументаОплаты = ДокументОбъект.Дата;				
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедуре Документы.АвансовыйОтчет.ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам() не удалось обработать авансовый отчет по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.АвансовыйОтчет, Выборка.Ссылка, ТекстСообщения);
	...
	Если ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре Документы.АвансовыйОтчет.ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам() не удалось обработать авансовые отчеты: в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.АвансовыйОтчет,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.АвансовыйОтчет.ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам() обработала очередную порцию авансовых отчетов: %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
	
КонецПроцедуры

// Функция проверяет, обработан ли авансовый отчет процедурой обновления (см. ЗаполнитьНастройкиУчетаНДСПоКомандировочнымРасходам()).
//  Определена в свойстве ПроцедураПроверки соответствующего обработчика обновления.
//
// Параметры:
//  Параметры - Структура - см. документацию к БСП
...
//   - Булево - если Истина, то можно редактировать и записывать данные, Ложь - данные недоступны, пока не будут обработаны процедурой обновления
//
Функция НеТребуетсяЗаполнятьНастройкиУчетаНДС(Параметры) Экспорт
	...
	|	Документ.АвансовыйОтчет.Прочее КАК РасходыСотрудника
	|ГДЕ
	|	РасходыСотрудника.Ссылка = &Ссылка
	|	И РасходыСотрудника.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийАвансовыйОтчет.Командировка)
	|	И НЕ РасходыСотрудника.Ссылка.НДСВключенВСтоимость
	|	И РасходыСотрудника.ВидДокументаРасхода = ЗНАЧЕНИЕ(Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Документ.АвансовыйОтчет.Билеты КАК РасходыБилеты
	|ГДЕ
	|	РасходыБилеты.Ссылка = &Ссылка
	|	И НЕ РасходыБилеты.Ссылка.НДСВключенВСтоимость
	|	И НЕ РасходыБилеты.ПредъявленСФ
	|	И РасходыБилеты.СуммаНДС <> 0";
