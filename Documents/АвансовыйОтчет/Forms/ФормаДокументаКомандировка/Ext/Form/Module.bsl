...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Документ.АвансовыйОтчет.Форма.УчетНДС") Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Не Объект.НДСВключенВСтоимость Тогда
		Для Каждого СтрокаРасхода Из Объект.Прочее Цикл
			АвансовыйОтчетФормыКлиентСервер.УстановитьТекстУчетНДС(СтрокаРасхода, "Прочее", РаздельныйУчетНДСНаСчете19);
		КонецЦикла;
	КонецЕсли;
	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		Если НавигационнаяСсылкаФорматированнойСтроки = "#ЗагрузитьЧекиМК" Тогда
			УчетКассовыхЧековПодотчетныхЛицКлиент.НачатьЗагрузкуВДокумент(
				...
				Объект.ФизЛицо,
				ОписаниеФормыДляЗагрузкиКассовыхЧековПодотчетныхЛиц(),
				Новый ОписаниеОповещения("ЗаполнитьСодержимымКассовыхЧеков", ЭтотОбъект, "Прочее"));
		Иначе
...
&НаКлиенте
Процедура КомандировочныеРасходыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "КомандировочныеРасходыУчетНДС" Тогда
...
Процедура КомандировочныеРасходыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не Объект.НДСВключенВСтоимость Тогда
		ТекущиеДанные.ВидДокументаРасхода  = ПредопределенноеЗначение("Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.Чек");	
		АвансовыйОтчетФормыКлиентСервер.УстановитьТекстУчетНДС(ТекущиеДанные, "Прочее", РаздельныйУчетНДСНаСчете19);
	КонецЕсли;
...
&НаКлиенте
Процедура КомандировочныеРасходыВидДокументаРасходаПриИзменении(Элемент)
...
Процедура КомандировочныеРасходыСтавкаНДСПриИзменении(Элемент)
	...
	СтрокаТаблицы = Элементы.КомандировочныеРасходы.ТекущиеДанные;
	ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуНДСТабЧасти(СтрокаТаблицы, Истина);
	Если СтрокаТаблицы.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС") Тогда
		СтрокаТаблицы.ПредъявленСФ = Ложь;
		СтрокаТаблицы.СпособУчетаНДС = ПредопределенноеЗначение("Перечисление.СпособыУчетаНДС.Списывается");
	КонецЕсли;
	АвансовыйОтчетФормыКлиентСервер.УстановитьТекстУчетНДС(СтрокаТаблицы, "Прочее", РаздельныйУчетНДСНаСчете19);			
	
КонецПроцедуры

&НаКлиенте
Процедура КомандировочныеРасходыУчетНДСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;

	ВыбраннаяСтрока = Элементы.КомандировочныеРасходы.ТекущаяСтрока;
	ОткрытьФормуРедактированияСтроки(ВыбраннаяСтрока);
...
Процедура УстановитьУсловноеОформление()
	...
	// Настройки учета НДС не требуются, если организация не является плательщиком НДС или освобождена от него
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КомандировочныеРасходыВидДокументаРасхода");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КомандировочныеРасходыУчетНДС");
	...
	// Для расходов без НДС поля НДС не доступны
	...
	УчетКассовыхЧековПодотчетныхЛиц.УстановитьУсловноеОформлениеНаименованияИзЧека(УсловноеОформление, "КомандировочныеРасходыСодержание");
	
	// Реквизит Содержание не доступен для ручного ввода, но может отображаться, если была загрузка из чеков подотчетных лиц.
	// В основной форме этого документа этот реквизит обязателен для заполнения.
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КомандировочныеРасходыСодержание");
...
Процедура УправлениеФормой(Форма)
	...
	ОбновитьРасчетОбщегоИтога(Форма);
...
Процедура ДатаПриИзмененииНаСервере()
	...
	НДСВключенВСтоимость = Не (УчетнаяПолитика.ПлательщикНДС(Объект.Организация, Объект.Дата) 
		ИЛИ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Объект.Организация, Объект.Дата))
		ИЛИ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(Объект.Организация, Объект.Дата);
	
	Если Не НДСВключенВСтоимость И Объект.НДСВключенВСтоимость Тогда
		// Нужно заполнить дополнительную колонку УчетНДС
		Для Каждого СтрокаРасхода Из Объект.Прочее Цикл
			АвансовыйОтчетФормыКлиентСервер.УстановитьТекстУчетНДС(СтрокаРасхода, "Прочее", РаздельныйУчетНДСНаСчете19);
		КонецЦикла;		
	КонецЕсли;	
...
Процедура ОткрытьФормуРедактированияСтроки(ВыбранноеЗначение)
	...
	ДанныеСтроки = Объект.Прочее.НайтиПоИдентификатору(ВыбранноеЗначение);
			
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИмяТЧ",                "Прочее");
	ПараметрыФормы.Вставить("РаздельныйУчетНДСНаСчете19", РаздельныйУчетНДСНаСчете19);
	ПараметрыФормы.Вставить("ПредъявленСФ",         ДанныеСтроки.ПредъявленСФ);
	ПараметрыФормы.Вставить("ВидДокументаРасхода",  ДанныеСтроки.ВидДокументаРасхода);
	...
	ПараметрыФормы.Вставить("ДатаСФ",               ДанныеСтроки.ДатаСФ);
	ПараметрыФормы.Вставить("НомерСФ",              ДанныеСтроки.НомерСФ);
	ПараметрыФормы.Вставить("СпособУчетаНДС",       ДанныеСтроки.СпособУчетаНДС);
	ПараметрыФормы.Вставить("Поставщик",            ДанныеСтроки.Поставщик);
...
Функция ОписаниеДокументаДляЗагрузкиКассовыхЧековПодотчетныхЛиц()
	...
	ОписаниеДокумента.Поставщик = Ложь;
...
Функция ОписаниеФормыДляЗагрузкиКассовыхЧековПодотчетныхЛиц()
	...
	ОписаниеФормы.КнопкиЗагрузитьЧеки.Добавить("КомандировочныеРасходыЗагрузитьЧеки");
...
Процедура ЗаполнитьСодержимымКассовыхЧековНаСервере(АдресСодержимоеЧеков, ИмяСписка)
	...
	Если ИмяСписка = "Прочее" Тогда
		Для Каждого СтрокаЧека Из ДобавленныеСтроки Цикл
			СтрокаЧека.ВидДокументаРасхода = Перечисления.ВидыДокументовПодтверждающихКомандировочныеРасходы.Чек;
			Если Не Объект.НДСВключенВСтоимость Тогда
				АвансовыйОтчетФормыКлиентСервер.УстановитьТекстУчетНДС(СтрокаЧека, "Прочее", РаздельныйУчетНДСНаСчете19);
			КонецЕсли;	
		КонецЦикла;	
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСчетаУчета(СтрокиДляЗаполненияСчетовУчета, ИмяТаблицы)
...
Процедура МобильныйКлиентАдаптацияФормы()
	...
	АдаптироватьМКСписокФормыКомандировочныеРасходы();
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВМККомандыФормы(Элементы, Элементы.ГруппаВыплатить);
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаПериод);
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаСуточные);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаАванс);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаБилеты);
	...
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.ГруппаШапкаЛевая);
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.ГруппаШапкаПравая);
		
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы,
		Элементы.ГруппаРасходы);
...
Процедура АдаптироватьМККонтекстСписокФормы()
	...
	МобильныйКлиентБПФормаОбъекта.АдаптироватьКонтекстноеМеню(ЭтотОбъект, "", "КомандировочныеРасходы");
	
КонецПроцедуры

&НаСервере
Процедура АдаптироватьМКСписокФормыКомандировочныеРасходы()
	
	КомандыПоля = МобильныйКлиентБПФормаОбъекта.НовыйОписаниеКомандТабличноеПолеФормы();
	КомандыПоля.КомандыТаблицыПрочие.Добавить("КомандировочныеРасходыЗагрузитьЧеки");
	КомандыПоля.ИмяКомандыДобавитьСтроку = "КомандировочныеРасходыДобавить";
	
	ЭлементГруппаСписка = Элементы.ГруппаРасходы;
	
	ЭлементГруппаСписка.Поведение=ПоведениеОбычнойГруппы.Свертываемая;
	ЭлементГруппаСписка.ПутьКДаннымЗаголовка = "Объект.Прочее.КоличествоСтрок";
	
	ОписаниеГруппыПоля = МобильныйКлиентБПФормаОбъекта.НовыйОписаниеГруппыТабличноеПолеФормы();
	ОписаниеГруппыПоля.ЭлементГруппаПоля = ЭлементГруппаСписка;
	
	Элементы.КомандировочныеРасходы.Шапка = Ложь;
	Элементы.КомандировочныеРасходы.Подвал = Ложь;
	Элементы.КомандировочныеРасходы.ПоведениеПриСжатииПоГоризонтали = ПоведениеТаблицыПриСжатииПоГоризонтали.ПереноситьЭлементыПоВажности;
	Элементы.КомандировочныеРасходыНомерСтроки.ВажностьПриОтображении=ВажностьПриОтображении.ОченьВысокая;
	Элементы.КомандировочныеРасходыВидДокументаРасхода.ВажностьПриОтображении=ВажностьПриОтображении.ОченьВысокая;
	Элементы.КомандировочныеРасходыВидДокВходящий.ВажностьПриОтображении=ВажностьПриОтображении.ОченьВысокая;
	Элементы.КомандировочныеРасходыСумма.ВажностьПриОтображении=ВажностьПриОтображении.ОченьВысокая;
	
	МобильныйКлиентБПФормаОбъекта.ПодготовитьТабличноеПолеФормыКоманднаяПанель(ЭтотОбъект,
		Элементы.КомандировочныеРасходы,
		ЭлементГруппаСписка,
		КомандыПоля);
	
	Элементы.КомандировочныеРасходыСумма.Формат = "ЧФ='Сумма: Ч'";
	Элементы.КомандировочныеРасходыСуммаНДС.Формат = "ЧФ='НДС: Ч'";
КонецПроцедуры
