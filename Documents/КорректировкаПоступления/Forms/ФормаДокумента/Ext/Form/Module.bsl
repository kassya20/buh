...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользуетсяПоступлениеНаНесколькоСкладов Тогда
		СкладВСписке = ВидимостьПоляСклад(Объект.ДокументПоступления);
	КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьДанныеОРасхожденияхПриПриемке();
...
Процедура ДатаИсправленияПриИзменении(Элемент)
	...
	Если Объект.ЭтоУниверсальныйДокумент 
	   И ЗначениеЗаполнено(Объект.ДатаИсправления) Тогда
...
Процедура НомерИсправленияПриИзменении(Элемент)
	...
	Если Объект.ЭтоУниверсальныйДокумент 
	   И ЗначениеЗаполнено(Объект.НомерИсправления) Тогда
		ОтобразитьПолеУПД(ЭтотОбъект);
	КонецЕсли;
...
Процедура ДокументПоступленияПриИзменении(Элемент)
	...
	//поступление на несколько складов в основании
	Если ИспользуетсяПоступлениеНаНесколькоСкладов Тогда
		СкладВСписке = ВидимостьПоляСклад(Объект.ДокументПоступления);
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|Сумма, СуммаДоКорректировки, СуммаДоИзменения, СуммаПоДокументу,
		|Малоценка, АналитикаУчета, АналитикаУчетаЗаполнена,
		|СтавкаНДС, СтавкаНДСДоИзменения,
		|СуммаНДС, СуммаНДСДоКорректировки, СуммаНДСДоИзменения, СуммаНДСПоДокументу,
		|НомерГТД, НомерГТДДоИзменения,
		|СтранаПроисхождения, СтранаПроисхожденияДоИзменения,
		|НадписьДоКорректировки, НадписьДоИзменения, НадписьПослеИзменения, НадписьПоДокументу,
		|ОтражениеВУСН, Всего, ВсегоДоКорректировки, ВсегоДоИзменения, ВсегоПоДокументу, СчетУчетаЗабалансовый,
		|ПрослеживаемыйТовар, РНПТ, РНПТДо, ПодсказкаРНПТ, 
		|НоменклатураЕдиницаИзмерения,
		|ИдентификаторСтроки, ПрослеживаемыйКомплект, ПрослеживаемыйТоварПоДокументуОснованию");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, ТекущиеДанные);
	
	ДанныеОбъекта = Новый Структура(
		"Дата, Организация, Склад, ДоговорКонтрагента, ТипЦен, СуммаВключаетНДС,
		|ВалютаДокумента, КорректироватьБУиНУ, ПрименяютсяСтавки4и2, ПрименяетсяМалоценка,
		|ДокументПоступления, ВидОперации");
	...
	ДанныеОбъекта.Вставить("ЕстьРасхождения", ЕстьРасхождения);
...
Процедура ТоварыСуммаДоИзмененияПриИзменении(Элемент)
	...
	ПриИзмененииСуммаНаКлиенте(
		...
		Если ИспользуютсяУпаковки Тогда
		ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
		УпаковкиКлиент.ТоварыСуммаПриИзменении(ТекущаяСтрока, Объект, "ДоИзменения");
	КонецЕсли;
...
Процедура ТоварыСуммаДоКорректировкиПриИзменении(Элемент)
	...
	Если ИспользуютсяУпаковки Тогда
		ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
		УпаковкиКлиент.ТоварыСуммаПриИзменении(ТекущаяСтрока, Объект, "ДоКорректировки");
	КонецЕсли;
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТоварыСчетУчета" 
		...
		Если (ДанныеСтроки.ЕстьВДокументеПоступления И Объект.КорректироватьБУиНУ) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ТоварыЦенаЗаУпаковкуДоИзмененияПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	УпаковкиКлиент.ТоварыЦенаЗаУпаковкуПриИзменении(ТекущаяСтрока, Объект, "ДоИзменения");
...
Процедура ТоварыЦенаЗаУпаковкуДоКорректировкиПриИзменении(Элемент)
	...
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
	УпаковкиКлиент.ТоварыЦенаЗаУпаковкуПриИзменении(ТекущаяСтрока, Объект, "ДоКорректировки");
...
Процедура УслугиНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|Цена, ЦенаДоКорректировки, ЦенаДоИзменения, ЦенаПоДокументу,
		|Сумма, СуммаДоКорректировки, СуммаДоИзменения, СуммаПоДокументу,
		|СтавкаНДС, СтавкаНДСДоИзменения,
		|СуммаНДС, СуммаНДСДоКорректировки, СуммаНДСДоИзменения, СуммаНДСПоДокументу,
		...
		|НадписьДоКорректировки, НадписьДоИзменения, НадписьПослеИзменения, НадписьПоДокументу,
		|Всего, ВсегоДоКорректировки, ВсегоДоИзменения, ВсегоПоДокументу");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, ТекущиеДанные);
...
Процедура АгентскиеУслугиПередУдалением(Элемент, Отказ)
	Для каждого Идентификатор Из Элементы.АгентскиеУслуги.ВыделенныеСтроки Цикл
		СтрокаТаблицы = Объект.АгентскиеУслуги.НайтиПоИдентификатору(Идентификатор);
		Если СтрокаТаблицы.ЕстьВДокументеПоступления И Объект.КорректироватьБУиНУ Тогда
			Отказ = Истина;
			ТекстСообщения = НСтр(
				"ru = 'Для удаления строки исходного документа необходимо очистить все суммовые и количественные показатели.'");
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.АктОРасхожденияхПриПриемке.Видимость = Форма.ЕстьРасхождения;
	...
	ЭтоИсправлениеСобственнойОшибки = ЭтоИсправлениеСобственнойОшибки(Форма);
	...
	ТребуетсяСчетФактура = Форма.ТребуетсяСчетФактура
		ИЛИ ТребуетсяАктуализироватьСчетФактуру(Форма.СчетФактура, Объект.ВидОперации);
	...
	Если ЭтоИсправлениеСобственнойОшибки Тогда
		Если Форма.ТребуетсяСчетФактура И НЕ ЗначениеЗаполнено(Форма.СчетФактура)
			ИЛИ ТребуетсяАктуализироватьСчетФактуру(Форма.СчетФактура, Объект.ВидОперации) Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	ЭтоКорректировка = Документы.КорректировкаПоступления.ЭтоКорректировка(Объект.ВидОперации, Объект.ДокументПоступления);
	...
	ПоступлениеТоваровУслугФормы.ЗаполнитьРеквизитыУПД(ЭтаФорма);
	...
	ЗаполнитьСписокВыбораУслугиОтражениеВУСН();
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ВедетсяУчетПрослеживаемыхТоваров = ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
		И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(ДатаДокументаПоступления);
...
Процедура ОбновитьУсловноеОформление(ЭтотОбъект)
	...
	ЭтотОбъект.УстановитьУсловноеОформлениеРеквизитыДокумента();
...
Процедура УстановитьУсловноеОформлениеВидимость()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыКоличествоУпаковокПоДокументу");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыЦенаЗаУпаковкуПоДокументу");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"ЕстьРасхождения", ВидСравненияКомпоновкиДанных.Равно, Ложь);
    ...
    ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"СкладВСписке", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
...
Процедура УстановитьУсловноеОформлениеТоварыОбщее()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыЦенаЗаУпаковкуДоИзменения");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыЦенаЗаУпаковкуДоКорректировки");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыЦенаЗаУпаковку");
	...
	ГруппаОтбора1 = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(ЭлементУО.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
				"ЭтоКомиссияНаЗакупку", ВидСравненияКомпоновкиДанных.Равно, Ложь);
...
&НаСервере
Процедура УстановитьУсловноеОформлениеРеквизитыДокумента() Экспорт
	...
	// Номер исправления, дата исправления
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НомерИсправления");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ЭтоУниверсальныйДокумент", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НомерИсправления", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
	
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДатаИсправления");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ЭтоУниверсальныйДокумент", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ДатаИсправления", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	...
	ГруппаОтбораИли = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"Объект.ЭтоУниверсальныйДокумент", ВидСравненияКомпоновкиДанных.Равно, Ложь);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"ОтметкаНезаполненногоНомерИсправления", ВидСравненияКомпоновкиДанных.Равно, Ложь);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"Объект.НомерИсправления", ВидСравненияКомпоновкиДанных.Заполнено);
	...
	ГруппаОтбораИли = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"Объект.ЭтоУниверсальныйДокумент", ВидСравненияКомпоновкиДанных.Равно, Ложь);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"ОтметкаНезаполненногоДатаИсправления", ВидСравненияКомпоновкиДанных.Равно, Ложь);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбораИли,
			"Объект.ДатаИсправления", ВидСравненияКомпоновкиДанных.Заполнено);
...
Процедура ОтобразитьПолеУПД(Форма) Экспорт
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(Объект.НомерИсправления)
			И ЗначениеЗаполнено(Объект.ДатаИсправления) Тогда
			НадписьСчетФактураПредставление =  СтрШаблон("(испр.%1) от %2, ",
				Объект.НомерИсправления,
				Формат(Объект.ДатаИсправления, "ДФ=dd.MM.yyyy"));
		КонецЕсли;
...
Функция ВнесеныИзмененияВСтроку(СтруктураСтрокиТабЧасти)
	...
	ИначеЕсли СтруктураСтрокиТабЧасти.Свойство("СтранаПроисхождения") Тогда
		Если СтруктураСтрокиТабЧасти.СтранаПроисхожденияДоИзменения <> СтруктураСтрокиТабЧасти.СтранаПроисхождения Тогда
			Результат = Истина;
		КонецЕсли;
	Иначе
...
Процедура Подключаемый_НапоминатьОКорректировочныхУведомленияхОВвозе()
	...
	ТребуетсяНапоминание = ТребуетсяНапоминаниеОКорректировочномУведомленииОВвозе();
	ПоказыватьБаннер     = ТребуетсяНапоминание И СписокУведомлений.Количество() = 0 
							И НЕ ОтключитьБаннерКорректировочныхУведомленийОВвозе;
...
Процедура ДатаПриИзмененииСервер()
	...
	// Признак прослеживаемости устанавливаем на дату исходного документа поступления.
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		Документы.КорректировкаПоступления.ПерезаполнитьДанныеПрослеживаемостиПриИзмененииОперации(Объект);
	Иначе
	...
	ЗаполнитьСписокВыбораУслугиОтражениеВУСН(Истина);
	УстановитьПараметрыВыбораСтавкиНДС();
...
Процедура ДокументПоступленияПриИзмененииКлиент()
	...
	АктОРасхожденияхПриПриемке          = Неопределено;
...
Процедура ДокументПоступленияПриИзмененииСервер()
	...
	Если НЕ АктОРасхожденияхПриПриемке.Пустая() 
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(АктОРасхожденияхПриПриемке, "ДокументПоступления") <> ДокументПоступленияСсылка Тогда
	...
	// Для возвратов с неустановленной сделкой по шапке привязывааемся к счету фактуре
	Если ТипЗнч(Объект.ДокументПоступления) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
		И ЗначениеЗаполнено(Объект.ДокументПоступления)
		И НЕ ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументПоступления, "Сделка")) Тогда
		
		СчетФактураВозвратПоставщику = УчетНДСПереопределяемый.НайтиПодчиненныйСчетФактуруПолученный(Объект.ДокументПоступления);
	
		Если НЕ ЗначениеЗаполнено(СчетФактураВозвратПоставщику) Тогда
			ТекстСообщения = НСтр("ru = 'Перед добавлением корректировочного счета-фактуры создайте счет-фактуру у документа-основания'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументПоступления", "Объект");
			Возврат;
		КонецЕсли;
		
		Документы.КорректировкаПоступления.ЗаполнитьПоВозвратуПоставщику(ОбъектЗначение, ОбъектЗначение.ДокументПоступления);
		
		ОбъектЗначение.ДокументПоступления = СчетФактураВозвратПоставщику;
...
&НаСервере
Процедура ЗаполнитьДанныеОРасхожденияхПриПриемке()
	...
	|ГДЕ
	|	АктОРасхождениях.ДокументПоступления = &ДокументПоступления
	...
	|		ПО ВТ_АктОРасхождениях.Ссылка = АктОРасхожденияхУслуги.Ссылка
	|ГДЕ
	...
	|		ПО ВТ_АктОРасхождениях.Ссылка = АктОРасхожденияхАгентскиеУслуги.Ссылка
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_РасхожденияПриПоступлении.Ссылка КАК Ссылка
	|ИЗ
	|	ВТ_АктОРасхождениях КАК ВТ_РасхожденияПриПоступлении
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_РасхожденияПриПоступлении.Ссылка КАК Ссылка,
	|	ВТ_РасхожденияПриПоступлении.ИмяТабличнойЧасти КАК ИмяТабличнойЧасти,
	|	ВТ_РасхожденияПриПоступлении.КоличествоПоДокументу КАК КоличествоПоДокументу,
	|	ВТ_РасхожденияПриПоступлении.ЦенаПоДокументу КАК ЦенаПоДокументу,
	|	ВТ_РасхожденияПриПоступлении.СуммаПоДокументу КАК СуммаПоДокументу,
	|	ВТ_РасхожденияПриПоступлении.СуммаНДСПоДокументу КАК СуммаНДСПоДокументу,
	|	ВТ_РасхожденияПриПоступлении.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ИЗ
	|	ВТ_РасхожденияПриПоступлении КАК ВТ_РасхожденияПриПоступлении
	|ИТОГИ ПО
	|	ИмяТабличнойЧасти";
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	ВыборкаДокументыРасхождения = РезультатЗапроса[2].Выбрать();
	
	ЕстьРасхождения = (ВыборкаДокументыРасхождения.Количество() = 1);
	
	Если ЕстьРасхождения Тогда
		ВыборкаДокументыРасхождения.Следующий();
		АктОРасхожденияхПриПриемке = ВыборкаДокументыРасхождения.Ссылка;
		
		ВыборкаПоТабличнымЧастям = РезультатЗапроса[3].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаПоТабличнымЧастям.Следующий() Цикл
			
			ИмяТабличнойЧасти = ВыборкаПоТабличнымЧастям.ИмяТабличнойЧасти;
			Выборка = ВыборкаПоТабличнымЧастям.Выбрать();
			Пока Выборка.Следующий() Цикл
				НайденныеСтроки = Объект[ИмяТабличнойЧасти].НайтиСтроки(Новый Структура("ИдентификаторСтроки", Выборка.ИдентификаторСтроки));
				Если НайденныеСтроки.Количество() > 0 Тогда
					ЗаполнитьЗначенияСвойств(НайденныеСтроки[0], Выборка, , "ИдентификаторСтроки");
					//Если используются Упаковки тогда заполняем количество и цену упаковок
					Если ИмяТабличнойЧасти = "Товары" И ИспользуютсяУпаковки Тогда
						НайденнаяСтрока 								= НайденныеСтроки[0];
						НайденнаяСтрока.КоличествоУпаковокПоДокументу 	= УпаковкиВызовСервера.КоличествоУпаковок(НайденнаяСтрока.КоличествоПоДокументу, НайденнаяСтрока.Упаковка);
						НайденнаяСтрока.ЦенаЗаУпаковкуПоДокументу 		= УпаковкиВызовСервера.ЦенаЗаУпаковку(НайденнаяСтрока.ЦенаПоДокументу, НайденнаяСтрока.Упаковка);
					КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		...
		ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(ЭтотОбъект, 
			СтрокаТаблицы, Объект.СуммаВключаетНДС, ЕстьРасхождения, СведенияОНоменклатуре);
	КонецЦикла;
	
	ЗаполнитьПризнакМалоценка(Объект.Товары, Объект.Организация, ДатаДокументаПоступления);
	
	//При использовании упаковок
	РаботаСУпаковкамиНоменклатуры.ЗаполнитьДанныеУпаковок(Объект);
	//Конец использования упаковок
	...
	БухгалтерскийУчетКлиентСервер.УстановитьНачальныеСвойстваСубконтоТаблицы(
		...
		ПараметрыУстановкиСвойствСубконтоНУ(ЭтотОбъект));
	Для Каждого СтрокаТаблицы Из Объект.Услуги Цикл
		ЗаполнитьДобавленныеКолонкиСтрокиТаблицыУслуги(СтрокаТаблицы, Объект.СуммаВключаетНДС, ЕстьРасхождения);
	КонецЦикла;
	
	Для Каждого СтрокаТаблицы Из Объект.АгентскиеУслуги Цикл
		ЗаполнитьДобавленныеКолонкиСтрокиТаблицыАгентскиеУслуги(СтрокаТаблицы, Объект.СуммаВключаетНДС, ЕстьРасхождения);
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьЗаголовкиСтрок(СтрокаТаблицы, ЕстьРасхождения = Ложь)
	
	СтрокаТаблицы.НадписьДоКорректировки = НСтр("ru = 'до корректировки:'");
	СтрокаТаблицы.НадписьПоДокументу     = НСтр("ru = 'до изменения:'");
	Если ЕстьРасхождения Тогда
		СтрокаТаблицы.НадписьДоИзменения     = НСтр("ru = 'по факту:'");
	Иначе
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта, Знач СчетаУчетаКЗаполнению)
	...
	Если СведенияОНоменклатуре <> Неопределено Тогда
		...
		//Добавляем данные по упаковкам если они используются
		...
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			...
			Если ДанныеОбъекта.ВидОперации <> Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
				Если ЗначениеЗаполнено(ДанныеОбъекта.ДокументПоступления) Тогда
					ДатаОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеОбъекта.ДокументПоступления, "Дата");
				Иначе
	...
	ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(ЭтотОбъект, 
		СтрокаТабличнойЧасти, ДанныеОбъекта.СуммаВключаетНДС, ДанныеОбъекта.ЕстьРасхождения, СведенияОНоменклатуре);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(Форма, СтрокаТаблицы, СуммаВключаетНДС, ЕстьРасхождения, СведенияОНоменклатуре)
	...
	ЗаполнитьЗаголовкиСтрок(СтрокаТаблицы, ЕстьРасхождения);
...
&НаСервереБезКонтекста
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыУслуги(СтрокаТаблицы, СуммаВключаетНДС, ЕстьРасхождения)
...
Процедура АгентскиеУслугиНоменклатураПриИзмененииНаСервере(СтрокаТаблицы, Знач ДанныеОбъекта)
	...
	ЗаполнитьДобавленныеКолонкиСтрокиТаблицыАгентскиеУслуги(СтрокаТаблицы, ДанныеОбъекта.СуммаВключаетНДС, ДанныеОбъекта.ЕстьРасхождения);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыАгентскиеУслуги(СтрокаТаблицы, СуммаВключаетНДС, ЕстьРасхождения)
	...
	СтрокаТаблицы.ВсегоПоДокументу     = СтрокаТаблицы.СуммаПоДокументу
		+ ?(СуммаВключаетНДС, 0, СтрокаТаблицы.СуммаНДСПоДокументу);
...
Процедура ЗарегистрироватьСчетФактуруНаСервере()
	...
	ОбновлятьРеквизиты = Истина;
	
	Если Объект.ЭтоУниверсальныйДокумент Тогда
		Если Не Документы.КорректировкаПоступления.НаличиеСуммыНДС(Объект.Ссылка) Тогда
			Возврат;
		КонецЕсли;
		НомерСФ            = Объект.НомерВходящегоДокумента;
		ДатаСФ             = Объект.ДатаВходящегоДокумента;
		НомерИсправленияСФ = Объект.НомерИсправления;
		ДатаИсправленияСФ  = Объект.ДатаИсправления;
	Иначе
		НомерСФ            = НомерСчетаФактурыПолученного;
		ДатаСФ             = ДатаСчетаФактурыПолученного;
		НомерИсправленияСФ = НомерИсправления;
		ДатаИсправленияСФ  = ДатаСчетаФактурыПолученного;
	КонецЕсли;
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение
		...
		ПараметрыСозданияСчетаФактуры.НомерСчетаФактурыПолученного = НомерСФ;
		ПараметрыСозданияСчетаФактуры.ДатаСчетаФактурыПолученного  = ДатаСФ;
		ПараметрыСозданияСчетаФактуры.ЭтоУниверсальныйДокумент = Объект.ЭтоУниверсальныйДокумент;
		ПараметрыСозданияСчетаФактуры.КодВидаОперации = КодВидаОперации;
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки
		И ЗначениеЗаполнено(НомерИсправленияСФ)
		И ЗначениеЗаполнено(ДатаИсправленияСФ)
		И ВозможностьСозданияСчетаФактуры(Объект.ДокументПоступления) Тогда
		
		ПараметрыСозданияСчетаФактуры = УчетНДСКлиентСервер.НовыеПараметрыСозданияПолученногоСчетаФактуры();
		ПараметрыСозданияСчетаФактуры.Основание = Объект.Ссылка;
		ПараметрыСозданияСчетаФактуры.НомерСчетаФактурыПолученного = НомерИсправленияСФ;
		ПараметрыСозданияСчетаФактуры.ДатаСчетаФактурыПолученного  = ДатаИсправленияСФ;
		ПараметрыСозданияСчетаФактуры.ЭтоУниверсальныйДокумент = Объект.ЭтоУниверсальныйДокумент;
		ПараметрыСозданияСчетаФактуры.КодВидаОперации = КодВидаОперации;
	...
	ЗаполнитьРеквизитыПроСчетФактуру(ЭтотОбъект, Результат, ОбновлятьРеквизиты);
	Если ЭтоИсправлениеСобственнойОшибки(ЭтотОбъект) Тогда
		УстановитьПризнакСчетФактураИСО();
...
Процедура ЗаполнитьСписокВыбораУслугиОтражениеВУСН(ПерезаполнитьНедоступныеЗначения = Ложь)
	
	Если Не (ПрименяетсяУСН И Не ПрименениеУСНДоходы) Тогда
		Возврат;
	КонецЕсли;
	...
	СписокВыбора = Элементы.УслугиОтражениеВУСН.СписокВыбора;
	...
	Если ПерезаполнитьНедоступныеЗначения Тогда
		...
		Для Каждого СтрокаТаблицы Из Объект.Услуги Цикл
			Если Не СтрокаТаблицы.ЕстьВДокументеПоступления
				И ДоступныеОтражения.НайтиПоЗначению(СтрокаТаблицы.ОтражениевУСН) = Неопределено Тогда
...
&НаСервереБезКонтекста
Функция ВидимостьПоляСклад(ДокументПоступления)
	
	Видимость = Ложь;
	
	Если ЗначениеЗаполнено(ДокументПоступления) 
		И ТипЗнч(ДокументПоступления) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПоступления, "СкладВСписке") Тогда
		Видимость = Истина; 
	КонецЕсли;
