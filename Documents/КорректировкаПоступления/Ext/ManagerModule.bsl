...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВсеКорректировкиПоступленияИзЕАЭСПоПрослеживаемымТоварамОбработаны");
	Методы.Вставить("УстановитьРеквизитКорректироватьНДС");
	Методы.Вставить("ОбработчикИсправленияКорректировкиПоступленияИзЕАЭС");
	Методы.Вставить("ЗаполнитьСведенияПрослеживаемостиПриПереходеС20");
	Методы.Вставить("УстановитьРеквизитСуммаВРозницеДоИзменения");
	Методы.Вставить("ЗаполнитьРеквизитыИсправленияКорректировкиПоступления");
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийСоглашенияОбИзмененииСтоимости()
	...
	|   ВЫРАЗИТЬ(КорректировкаПоступления.Количество / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,3)) КАК Количество,
	|	ВЫРАЗИТЬ(КорректировкаПоступления.КоличествоДоИзменения / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,3)) КоличествоДоКорректировки,
	|	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения, ЕСТЬNULL(КорректировкаПоступленияНоменклатура.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка))) КАК ЕдиницаИзмерения,
	|  	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения.Код, ЕСТЬNULL(КорректировкаПоступленияНоменклатура.ЕдиницаИзмерения.Код, """")) КАК ЕдиницаИзмеренияКод, 
	|   ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения.Наименование, ЕСТЬNULL(КорректировкаПоступленияНоменклатура.ЕдиницаИзмерения.Наименование, """")) КАК ЕдиницаИзмеренияНаименование, 
	|	0 КАК КоличествоВОдномМесте,
	|	ВЫРАЗИТЬ(ВЫБОР
	...
	|			КОНЕЦ * ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,3)) КАК ЦенаДоКорректировки,
	|	ВЫБОР
	|		КОГДА ДокументыДляПечати.СуммаВключаетНДС
	|			ТОГДА КорректировкаПоступления.Сумма - КорректировкаПоступления.СуммаНДС
	|		ИНАЧЕ КорректировкаПоступления.Сумма
	...
	|		По КорректировкаПоступления.Упаковка = СправочникУпаковка.Ссылка
