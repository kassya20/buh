...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РеквизитПереченьСчетовОбработан");
	Методы.Вставить("ЗаполнитьПереченьСчетовПоУмолчанию");
...
Функция СчетаДетализацииЗадолженности() Экспорт
	...
	СчетаРасчетовСКонтрагентами(СчетаРасчетов);
	...
	Пока ВыборкаШапок.Следующий() Цикл
		...
		Для Каждого ЧленКомиссии Из ДанныеЧленовКомиссии Цикл
			...
			СтруктураПараметровПодписи.ЗаголовокРазделаПодписей = ?(ВыводитьЗаголовок, 
			                                               ЗаголовокРазделаПодписей.ЧленыКомиссии,
														   "");
			СтруктураПараметровПодписи.Должность                 = ЧленКомиссии.Должность;
			СтруктураПараметровПодписи.НаименованиеЧленаКомиссии = ЧленКомиссии.Фамилия + " " + ЧленКомиссии.Имя + " " + ЧленКомиссии.Отчество;
...
Функция ТекстЗапросаПечатьСправкиИНВ17()
	...
	|	ДебиторскаяЗадолженность.Подтверждено + ДебиторскаяЗадолженность.НеПодтверждено КАК СуммаДт,
	|	0 КАК СуммаКт,
	...
	|	НЕОПРЕДЕЛЕНО,
	|	0,
	|	КредиторскаяЗадолженность.Подтверждено + КредиторскаяЗадолженность.НеПодтверждено,
	...
	|	ДебиторскаяЗадолженность.Подтверждено + ДебиторскаяЗадолженность.НеПодтверждено,
	|	0,
...
Функция ВыбранныеСчетаРасчетов(ИерархияСчетов) Экспорт
...
// Получает таблицы расчетов по задолженностям указанного вида.
//
// Параметры:
//  ПараметрыЗаполнения - Структура - см. НовыеПараметрыЗаполнения().
//
...
Процедура СчетаРасчетовСКонтрагентами(СчетаРасчетов) Экспорт
...
Функция СчетаРасчетовСКонтрагентамиИсключения() Экспорт
	...
	// 76.18 - Приобретение прав на использование результатов интеллектуальной деятельности
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат,
...
Процедура ИсключитьСчетаИзМассива(СчетаРасчетов, ИсключаемыеСчета) Экспорт
	
	Для Каждого ТекущийСчет Из ИсключаемыеСчета Цикл
		ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(СчетаРасчетов, ТекущийСчет);
	КонецЦикла;
