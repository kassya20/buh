...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВыбранныеСчетаБезАналитики = Параметры.ВыбранныеСчетаБезАналитики;
	...
	СчетаРасчетовКПодбору = ДопустимыеСчетаИнвентаризации(СчетаРасчетовССубконто);
	...
	Запрос.УстановитьПараметр("СчетаРасчетов", СчетаРасчетовКПодбору);
	...
	Список.Параметры.УстановитьЗначениеПараметра("СчетаРасчетов", СчетаРасчетовКПодбору);
...
&НаСервереБезКонтекста
Функция ДопустимыеСчетаИнвентаризации(СчетаРасчетов)
	
	СчетаДетализацииЗадолженности = Документы.ИнвентаризацияРасчетовСКонтрагентами.СчетаДетализацииЗадолженности();
	ДопустимыеСчета = СчетаРасчетовССубконто(СчетаДетализацииЗадолженности.ВыгрузитьКолонку("СчетРасчетов"));
	СчетаИсключения = Документы.ИнвентаризацияРасчетовСКонтрагентами.СчетаРасчетовСКонтрагентамиИсключения();
	
	Для Каждого СчетИсключение Из СчетаИсключения Цикл
		ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(ДопустимыеСчета, СчетИсключение);
	КонецЦикла;
	
	Результат = Новый Массив;
	Для Каждого ТекущийСчет Из СчетаРасчетов Цикл
		Если ДопустимыеСчета.Найти(ТекущийСчет) <> Неопределено Тогда
			Результат.Добавить(ТекущийСчет);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИли,
		"Список.СчетРасчетов", ВидСравненияКомпоновкиДанных.ВСписке, ВыбранныеСчетаБезАналитики);
