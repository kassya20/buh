...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ОбъектРасшифровки) Тогда
	...
	Иначе
		...
		Заголовок = СтрШаблон(ЗаголовокШаблон, СчетРасчетов, СвойстваСчета.Наименование);
	КонецЕсли;
...
Процедура ДетальныеЗаписиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ИдСтроки <> ИдентификаторСтроки Тогда
		...
		БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(ТекущиеДанные.СчетРасчетов, ЭтотОбъект, ПоляФормы);
...
Процедура ДетальныеЗаписиПодтвержденоПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ИстекСрокДавности > ТекущиеДанные.Всего Тогда
		...
		ИндексСтроки = ДетальныеЗаписи.Индекс(ТекущиеДанные);
		ИмяПоля = СтрШаблон("ДетальныеЗаписи[%1].ИстекСрокДавности", Формат(ИндексСтроки, "ЧН=0; ЧГ=")); 
		...
		ТекущиеДанные.ИстекСрокДавности = ТекущиеДанные.Всего;
...
Функция ИтогоЗадолженностиПоТекущейСтроке(ТекущаяСтрока)
	...
	Возврат ТекущаяСтрока.Подтверждено + ТекущаяСтрока.НеПодтверждено;
...
Процедура УстановитьОформлениеСубконто()
	...
	СчетаУчетПоНаименованию = Документы.ИнвентаризацияРасчетовСКонтрагентами.СчетаРасчетовУчетПоНаименованию();
	СчетУчитываетсяПоНаименованию = СчетаУчетПоНаименованию.Найти(СчетРасчетов) <> Неопределено;
	
	Элементы.ДетальныеЗаписиСубконто1.Видимость = СчетУчитываетсяПоНаименованию;
	...
	Для НомерСубконто = 1 По 3 Цикл
		...
		Если НомерСубконто = 1 И СчетУчитываетсяПоНаименованию Тогда
			
			// Для счетов расчетов, отображаемых по наименованию в документе в расшифровке первое субконто отображается
			ПоказыватьСубконто = Истина;
			
		ИначеЕсли СчетаРасчетовСПерсоналом.Найти(СвойстваСчета.Ссылка) <> Неопределено Тогда
