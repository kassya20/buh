...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПараметрыОповещения.Вставить("РазделУчета", ПредопределенноеЗначение("Перечисление.РазделыУчетаДляИнвентаризации.РасчетыСКонтрагентами"));
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗаполнениеИзПомощника Тогда
		ЗаполнитьДолгиПоАктуальнымДанным(ПредопределенноеЗначение("Перечисление.ВидыЗадолженности.Дебиторская"));
		ЗаполнитьДолгиПоАктуальнымДанным(ПредопределенноеЗначение("Перечисление.ВидыЗадолженности.Кредиторская"));
	КонецЕсли;
...
Процедура ЗаполнитьДолгиНаКлиенте(ДополнительныеПараметры = Неопределено)
	...
	Если Не ЗаполнитьДолгиНаСервере(ДополнительныеПараметры) Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОжидания.Заголовок            = НаименованиеФоновогоЗадания();
...
Функция НаименованиеФоновогоЗадания()
	...
	Возврат НСтр("ru = 'Заполнить данными расчетов с контрагентами'");
...
Функция ЗаполнитьДолгиНаСервере(ДополнительныеПараметры)
	...
	ОписаниеФоновогоЗадания = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполнения, "Документы.ИнвентаризацияРасчетовСКонтрагентами.ЗаполнитьПоОстаткам", ПараметрыЗаполнения);
...
&НаСервере
Функция ПрочитатьРезультатЗаполнения(АдресХранилищаАктуализации)
...
Процедура СообщитьРезультатЗаполнения(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗаполнения.ОткрытьФормуОшибки Тогда
...
Процедура Подбор(Команда)
	...
	ВыбранныеСчетаБезАналитики = Новый СписокЗначений;
	Для Каждого ТекущаяСтрока Из ЭтотОбъект[ИмяТаблицы] Цикл
		Если ЗначениеЗаполнено(ТекущаяСтрока.Контрагент) Тогда
			ВыбранныеОбъекты.Добавить(ТекущаяСтрока.Контрагент);
		Иначе
			ВыбранныеСчетаБезАналитики.Добавить(ТекущаяСтрока.СчетРасчетов);
		КонецЕсли;
	КонецЦикла;
	...
	ПараметрыОтбора.Вставить("ВыбранныеСчетаБезАналитики", ВыбранныеСчетаБезАналитики);
