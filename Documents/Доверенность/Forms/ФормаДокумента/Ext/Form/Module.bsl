...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеДанныхФизическогоЛица"
		...
		ЕстьПаспортныеДанные = ЕстьПаспортныеДанные(Объект.ФизЛицо);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ДекорацияПаспортныеДанныеОтсутствуют.Видимость = ЗначениеЗаполнено(Объект.ФизЛицо)
		И НЕ Форма.ЕстьПаспортныеДанные;
...
&НаСервереБезКонтекста
Функция ЕстьПаспортныеДанные(ФизическоеЛицо)
	...
	Возврат ДействующийПаспорт <> Неопределено;
КонецФункции
...
&НаСервереБезКонтекста
Функция ПредставлениеСписаниеСРасчетногоСчета(Сделка)
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сделка,
		"НомерВходящегоДокумента, ДатаВходящегоДокумента, ДокументОснование");
	
	Если ЗначениеЗаполнено(Реквизиты.НомерВходящегоДокумента) И ЗначениеЗаполнено(Реквизиты.ДатаВходящегоДокумента) Тогда
		ПредставлениеДокумента = СтрШаблон(
			НСтр("ru = 'Платежное поручение № %1 от %2'"),
			Реквизиты.НомерВходящегоДокумента,
			Формат(Реквизиты.ДатаВходящегоДокумента, "ДЛФ=D"));
	ИначеЕсли ЗначениеЗаполнено(Реквизиты.ДокументОснование)
		И ТипЗнч(Реквизиты.ДокументОснование) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		ПредставлениеДокумента = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(
			Реквизиты.ДокументОснование, СокрЛП(Реквизиты.ДокументОснование.Метаданные().Представление()));
	Иначе
		ПредставлениеДокумента = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(
			Сделка, СокрЛП(Сделка.Метаданные().Представление()));
	КонецЕсли;
...
Функция ПолучитьПредставлениеДокумента(Сделка)
	...
	Если ЗначениеЗаполнено(Сделка) Тогда
		...
		ИначеЕсли ТипЗнч(Сделка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
			ПредставлениеДокумента = ПредставлениеСписаниеСРасчетногоСчета(Сделка);
		Иначе
			ПредставлениеДокумента = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(
				Сделка, СокрЛП(Сделка.Метаданные().Представление()));
		КонецЕсли;
...
Функция ОписаниеТипаРеквизитаСделка()
	...
	Возврат Метаданные.Документы.Доверенность.Реквизиты.Сделка.Тип;
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ДатаДействия);
	...
	Элементы.ГруппаФизЛицо.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаФизЛицо);
...
Процедура АдаптироватьМКСписокФормы()
	...
	ОписаниеКолонокПоля.ВажныеКолонки = "ТоварыНаименованиеТовара";
	ОписаниеКолонокПоля.УстановитьВерхниеЗаголовки = "ТоварыНаименованиеТовара";
