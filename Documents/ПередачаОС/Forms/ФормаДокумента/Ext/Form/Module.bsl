...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	СчетаФактурыНеТребуются = Документы.ПередачаОС.СчетаФактурыНеТребуются(МассивДокументов);
	...
	Для Каждого Строка Из Объект.ОС Цикл
		...
		Строка.ЗаполненыСуммыПоПрослеживаемомуОС = ЗаполнитьПризнакЗаполненыСуммыПоПрослеживаемомуОС(
			Строка.ИдентификаторСтроки, Строка.СтавкаНДС, Строка.ЕдиницаУчета);
	КонецЦикла;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		...
		МассивСсылок.Добавить(Объект.ДокументОснование);
		Оповестить("ИзмененСтатусДокументов", МассивСсылок, Объект.Ссылка);
	КонецЕсли;
...
Процедура ОССтавкаНДСПриИзменении(Элемент)
	...
	Если ТекущаяСтрока.ПрослеживаемыйТовар Тогда
		...
		ТекущаяСтрока.ИдентификаторСтроки, ТекущаяСтрока.СтавкаНДС, ТекущаяСтрока.ЕдиницаУчета);
	КонецЕсли;
...
Процедура ОСКоличествоПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено
		И ТекущиеДанные.ЕдиницаУчета = ПредопределенноеЗначение("Перечисление.ЕдиницыУчетаОС.ГрупповойОбъект") Тогда
		// Необходимо очистить дополнительные сведения
		НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
...
Процедура УстановитьПараметрыВыбораСтавкиНДС()
	...
	УчетНДС.УстановитьПараметрыВыбораСтавкиНДС(ЭтотОбъект, "ОССтавкаНДС");
...
Процедура УстановитьУсловноеОформление()
	...
	// Необходимость заполнения количества и единицы измерения только для групповых ОС
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ОСКоличество");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ОСЕдиницаИзмерения");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		Для Каждого Строка Из Объект.ОС Цикл
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТОС(ЭтаФорма, Строка);
			Строка.ЗаполненыСуммыПоПрослеживаемомуОС =
				ЗаполнитьПризнакЗаполненыСуммыПоПрослеживаемомуОС(
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ПередачаОС.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаОС Из ДобавленныеСтроки Цикл
		...
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			РеквизитыТекущегоОС = ИсторияПрослеживаемостиОС[СтрокаОС.ОсновноеСредство];
			Если РеквизитыТекущегоОС <> Неопределено Тогда
				СтрокаОС.ПрослеживаемыйТовар = РеквизитыТекущегоОС.ПрослеживаемыйТовар;
			Иначе
				СтрокаОС.ПрослеживаемыйТовар = РеквизитыОС[СтрокаОС.ОсновноеСредство];
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьПоНаименованиюСервер(Знач ОсновноеСредство)
	...
	Если УчетОС.ДозаполнитьТабличнуюЧастьОсновнымиСредствамиПоНаименованию(
		...
		Для Индекс = НачальныйИндекс По Объект.ОС.Количество() - 1 Цикл
			...
			ПриДобавленииОС(ЭтаФорма, СтрокаОС);
			Если ВедетсяУчетПрослеживаемыхТоваров Тогда
				СтрокаОС.ПрослеживаемыйТовар = ПрослеживаемостьФормыВызовСервераПереопределяемый.ПолучитьПризнакПрослеживаемости(СтрокаОС.ОсновноеСредство, Объект.Дата);
			КонецЕсли;
...
Процедура ПриДобавленииОС(Форма, СтрокаТабличнойЧасти)
	...
	Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.СтавкаНДС) Тогда
		СтрокаТабличнойЧасти.СтавкаНДС = Форма.СтавкаНДСПриРеализации;
	КонецЕсли;
...
&НаСервере
Функция ЗаполнитьПризнакЗаполненыСуммыПоПрослеживаемомуОС(ИдентификаторСтроки, СтавкаНДС, ЕдиницаУчета)
...
Процедура ЗаполнитьРеквизитыПоПрослеживаемости(ПараметрыЗаполнения)
	...
	ПараметрыЗаполнения.ЗаполненыСуммыПоПрослеживаемомуОС = 
		ЗаполнитьПризнакЗаполненыСуммыПоПрослеживаемомуОС(
		ПараметрыЗаполнения.ИдентификаторСтроки,
		ПараметрыЗаполнения.СтавкаНДС,
		ПараметрыЗаполнения.ЕдиницаУчета);
...
Процедура ОбработкаВыбораРНПТНаКлиенте(ВыбранноеЗначение)
	...
	Если СтрокаТабличнойЧасти <> Неопределено Тогда
		...
		СтрокаТабличнойЧасти.ЗаполненыСуммыПоПрослеживаемомуОС = ЗаполнитьПризнакЗаполненыСуммыПоПрослеживаемомуОС(
			СтрокаТабличнойЧасти.ИдентификаторСтроки, СтрокаТабличнойЧасти.СтавкаНДС, СтрокаТабличнойЧасти.ЕдиницаУчета);
	КонецЕсли;
