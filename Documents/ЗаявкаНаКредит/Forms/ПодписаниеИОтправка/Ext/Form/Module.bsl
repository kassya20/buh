	...
	Если ТипЗнч(Параметры.Банки) = Тип("Массив") Тогда
		Для каждого Банк Из Параметры.Банки Цикл
			...
			ИначеЕсли ТипЗнч(Банк) = Тип("Структура") Тогда
				...
				НоваяСтрока.ИдентификаторКредитногоПродукта = Банк.ИдентификаторКредитногоПродукта;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура НачатьПодписаниеИОтправку()
	...
	ПараметрыПроцедуры.Вставить("КредитныеПродукты",                 Банки.Выгрузить());
	...
	Если ПараметрыКриптографии.ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания =
			НСтр("ru = 'Подписание и отправка заявки на кредит (облачный сертификат)'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ЗаявкаНаКредит.ПодписатьИОтправитьЭлектроннаяПодписьВМоделиСервиса",
			ПараметрыПроцедуры,
			ПараметрыВыполнения);
	...
	Иначе
		...
		ПараметрыВыполнения.ОжидатьЗавершение = 0;
		ПараметрыВыполнения.НаименованиеФоновогоЗадания =
			НСтр("ru = 'Подписание заявки на кредит (локальный сертификат)'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ЗаявкаНаКредит.ПодготовитьФайлыДляОтправки",
			ПараметрыПроцедуры,
			ПараметрыВыполнения);
