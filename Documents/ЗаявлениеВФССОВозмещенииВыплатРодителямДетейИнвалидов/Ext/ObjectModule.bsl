...
Процедура ПриКопировании(ОбъектКопирования)
	...
	// Заполним заново ТЧ Оплаты на основани копии
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Менеджер().ОбновитьФлажокИсправлен(ИсправленныйДокумент, Не ПометкаУдаления);
	Если Не ЭтоНовый()И ДоЗаписи.ИсправленныйДокумент <> ИсправленныйДокумент
...
Функция ЗаполнитьСведенияОСотрудниках(ПараметрыФиксации)
	...
	ОсновныеСотрудникиФизическихЛиц = Оплаты.Выгрузить(Новый Структура("Сотрудник", Справочники.Сотрудники.ПустаяСсылка()));
	Если ОсновныеСотрудникиФизическихЛиц.Количество() > 0 Тогда
		МассивФизическихЛиц = КоллекцииБЗК.УникальныеЗначенияКолонки(ОсновныеСотрудникиФизическихЛиц, "ФизическоеЛицо");
		КоллекцииБЗК.УдалитьПустыеЗначенияМассива(МассивФизическихЛиц);
		ОсновныеСотрудникиФизическихЛиц = КадровыйУчет.ОсновныеСотрудникиФизическихЛиц(МассивФизическихЛиц, Ложь, Организация, Дата);
		МассивНеЗаполненныхСотрудники = КоллекцииБЗК.УникальныеЗначенияКолонки(ОсновныеСотрудникиФизическихЛиц, "Сотрудник");
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСотрудников, МассивНеЗаполненныхСотрудники, Истина);
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из Оплаты Цикл
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.Сотрудник) И ЗначениеЗаполнено(СтрокаТаблицы.ФизическоеЛицо) Тогда
			НайденныеСтроки = ОсновныеСотрудникиФизическихЛиц.НайтиСтроки(
...
Функция ЗаполнитьДанныеРебенка(ПараметрыФиксации)
	...
	Для Каждого СтрокаТаблицы Из Оплаты Цикл
		...
		Если НайденныеСтрокиСправокПоСНИЛС.Количество() > 0 Тогда
			...
			Значения.СправкаДатаУстановления = СтрокаТаблицыСправокПоСНИЛС.ДатаУстановления;
...
Функция ТаблицаСправокПоСНИЛС(ОграничениеПоКоличеству)
	...
	|	СведенияОбИнвалидности.ДатаУстановления КАК ДатаУстановления
