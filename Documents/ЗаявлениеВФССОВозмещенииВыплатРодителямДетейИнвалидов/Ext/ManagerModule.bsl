...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не СЭДОФСС.ДоступенОбменЧерезСЭДО()
		Или Не ПравоДоступа("Изменение", Метаданные.Документы.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов) Тогда
		Возврат; // Нет прав.
	КонецЕсли;
	
	МодульТекущиеДелаСервер = ОбщегоНазначения.ОбщийМодуль("ТекущиеДелаСервер");
	Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта(Метаданные.Документы.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов.ПолноеИмя());
	...
	ДанныеИсправлений = Новый Структура("АдресРасшифровкиИсправлений,КоличествоЗаявлений,Дата,Организации",
		ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор), 0, ТекущаяДатаСеанса(), Новый Массив);
	...
	ЕстьДела = ДанныеИсправлений.КоличествоЗаявлений > 0;
	
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
		Дело.Идентификатор = "ЗаявлениеВСФРВозмещенииВыплат_Исправление_" + СтрЗаменить(Раздел.ПолноеИмя(), ".", "_");
		Дело.ЕстьДела       = ЕстьДела;
		Дело.Важное         = Ложь;
		Дело.Владелец       = Раздел;
		Дело.Представление  = НСтр("ru = 'Исправить заявление в СФР на возмещении выплат'");
		Дело.Количество     = ДанныеИсправлений.КоличествоЗаявлений;
		Дело.Подсказка      = НСтр("ru = 'Найдены исправления оплат, требуется исправить ""Заявление в СФР о возмещении выплат родителям детей-инвалидов"".'");
		Дело.ПараметрыФормы = Новый Структура("АдресРасшифровкиИсправлений", "");
		Дело.Форма          = "Документ.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов.Форма.ИсправлениеЗаявлений";
	КонецЦикла;
...
Функция ТекстXML_120(Документ) Экспорт
	...
	ФильтрОсновныхСтрок = Новый Структура("ЭтоРасшифровка", Ложь);
	ТаблицаСтрок = Документ.Оплаты.Выгрузить(ФильтрОсновныхСтрок);
...
Функция ТабличныйДокументДополнительныеДниОтпуска(МассивСсылок, ОбъектыПечати)
	...
	|	ЕСТЬNULL(ТЧОплаты.НомерСтроки, 0) КАК НомерСтроки,
	|	ЕСТЬNULL(ТЧОплаты.ФизическоеЛицо, ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)) КАК ФизическоеЛицо,
	|	ЕСТЬNULL(ТЧОплаты.Сотрудник, НЕОПРЕДЕЛЕНО) КАК Сотрудник,
	...
	|	ЕСТЬNULL(ТЧОплаты.ЭтоРасшифровка, ЛОЖЬ) КАК ЭтоРасшифровка,
...
Процедура ВывестиЗаявлениеОВозмещении_2024(Выборка, ТабличныйДокумент, ОбъектыПечати)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		// Вывод строк по сотрудникам.
		ИменаПолейШапки = "СотрудникФамилия, СотрудникИмя, СотрудникОтчество, СотрудникСНИЛС, Статус, ФизическоеЛицо,
			|РебенокФамилия, РебенокИмя, РебенокОтчество, РебенокСНИЛС, РебенокДатаРождения, ЭтоРасшифровка,
...
Функция ВложенныеТаблицыПоСотруднику(ТаблицаСтрокПоСотруднику)
	...
	Фильтр              = Новый Структура("ЕстьПриказы, ЭтоРасшифровка", Истина, Ложь);
	...
	Фильтр             = Новый Структура("ЕстьДниЗаМесяц, ЭтоРасшифровка", Истина, Ложь);
	...
	Фильтр             = Новый Структура("ЕстьНакопленные, ЭтоРасшифровка", Истина, Ложь);
	...
	ДниЗаГод = ТаблицаСтрокПоСотруднику.Скопировать(Новый Структура("ЕстьДниЗаГод, ЭтоРасшифровка", Истина, Ложь));
	ДниЗаГод.Свернуть(ВложенныеИзмерения, ВложенныеРесурсы);
	ДниЗаГод.Сортировать("Месяц");
...
Процедура ВывестиШапкуСотрудника_2024(КонтекстПечати, ДанныеШапки, НомерСтроки)
	...
	// Сведения о справке, подтверждающей инвалидность.
	Постфикс = "";
	Если СтрДлина(СокрЛП(ДанныеШапки.СправкаНомер)) > 10 Тогда
		Секция = УправлениеПечатьюБЗК.СекцияПечатнойФормы(КонтекстПечати, "СведенияОСправке_2026");
		Секция.ТабличныйДокумент.Параметры.СправкаНомер = ДанныеШапки.СправкаНомер;
		Постфикс = "2026";
	Иначе
		Секция = УправлениеПечатьюБЗК.СекцияПечатнойФормы(КонтекстПечати, "СведенияОСправке");
		УправлениеПечатьюБЗК.ВывестиПоБуквам(Секция, ДанныеШапки.СправкаНомер, "СправкаНомер");
	КонецЕсли;
	УправлениеПечатьюБЗК.ВывестиДатуПоБуквам(Секция, ДанныеШапки.СправкаДатаВыдачи,       "СправкаДатаВыдачи" + Постфикс);
	УправлениеПечатьюБЗК.ВывестиДатуПоБуквам(Секция, ДанныеШапки.СправкаДатаОкончания,    "СправкаДатаОкончания" + Постфикс);
	УправлениеПечатьюБЗК.ВывестиДатуПоБуквам(Секция, ДанныеШапки.СправкаДатаУстановления, "СправкаДатаУстановления" + Постфикс);
	КонтекстПечати.ТабличныйДокумент.Вывести(Секция.ТабличныйДокумент);
КонецПроцедуры
...
Процедура ВывестиПодвалДокумента_2024(КонтекстПечати, Документ, ТаблицаСтрок)
	...
	МассивСотрудников = КоллекцииБЗК.УникальныеЗначенияКолонки(ТаблицаСтрок, "ФизическоеЛицо");
	КоллекцииБЗК.УдалитьПустыеЗначенияМассива(МассивСотрудников);
	КоличествоПолучателей = МассивСотрудников.Количество();
	ЛистовПриказов        = ТаблицаСтрок.Итог("КоличествоСтраниц");
	УправлениеПечатьюБЗК.ВывестиЧислоПоБуквам(Секция, КоличествоПолучателей,          "КоличествоПолучателей");
...
Функция ЕстьДругоеИсправление(ИсправленныйДокумент, ИсключаемаяСсылка) Экспорт
	...
	|	И НЕ КакимДокументомИсправлен.ПометкаУдаления";
