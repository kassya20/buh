...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ДанныеИсправлений = Новый ФиксированнаяСтруктура(ДанныеИсправлений);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "Запись_ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов" 
		Или ИмяСобытия = "Запись_ОплатаДнейУходаЗаДетьмиИнвалидами" Тогда
		Если ЗначениеЗаполнено(Источник) Тогда
			ОбновитьДанныеИсправлений = ЭтоИсправление(Источник);
		КонецЕсли;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		Организации.Добавить(Организация);
	КонецЕсли;
	ОбновитьДанныеИсправлений = Истина;
	ОбновитьФорму();
КонецПроцедуры
...
&НаКлиенте
Процедура СообщениеОПерерасчетеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура("АдресРасшифровкиИсправлений", ДанныеИсправлений.АдресРасшифровкиИсправлений);
	ОткрытьФорму("Документ.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов.Форма.ИсправлениеЗаявлений", ПараметрыФормы, ЭтотОбъект);
КонецПроцедуры
...
&НаСервере
Процедура ОбновитьДанныеИсправленийОплат()
	ОбновитьДанныеИсправлений = Ложь;
	Если ДанныеИсправлений <> Неопределено Тогда
		СтруктураИсправлений = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(ДанныеИсправлений);
		СтруктураИсправлений.Организации = Организации.ВыгрузитьЗначения();
		СтруктураИсправлений.Дата = ТекущаяДатаСеанса();
		ПрямыеВыплатыПособийСоциальногоСтрахования.ОбновитьДанныеИсправленийОплатДляВозмещенияВыплатРодителямДетейИнвалидов(СтруктураИсправлений);
		ДанныеИсправлений = Новый ФиксированнаяСтруктура(СтруктураИсправлений);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ЭтоИсправление(Источник)
	Возврат ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник, "ИсправленныйДокумент"));
КонецФункции
...
Процедура ОбновитьФорму()
	Если ОбновитьДанныеИсправлений Тогда
		ОбновитьДанныеИсправленийОплат();
	КонецЕсли;
	ОбновитьЭлементыФормы();
	ОбновитьПараметрыСписка();
	ОбновитьНадписьСообщенияОжидаемыеОтСФР();
	УстановитьОтображениеСообщенияОНеобходимостиИсправления();
	Элементы.Список.Обновить();
КонецПроцедуры

&НаСервере
Процедура УстановитьОтображениеСообщенияОНеобходимостиИсправления()
	Элементы.ГруппаПредупреждение.Видимость = ДанныеИсправлений <> Неопределено И ДанныеИсправлений.КоличествоЗаявлений > 0;
	Если Элементы.ГруппаПредупреждение.Видимость Тогда
		КоличествоЗаявленийПредставление = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru = ';%1 заявления;;%1 заявлений;%1 заявлений;'"), ДанныеИсправлений.КоличествоЗаявлений);
		ТекстСообщения = Новый ФорматированнаяСтрока(НСтр("ru='Найдены исправления для '") + КоличествоЗаявленийПредставление + " (",
			Новый ФорматированнаяСтрока(НСтр("ru='Подробнее'") + "...", , , , "Подробнее"), "). ");
		Элементы.СообщениеОПерерасчете.Заголовок = ТекстСообщения;
	КонецЕсли;
КонецПроцедуры
