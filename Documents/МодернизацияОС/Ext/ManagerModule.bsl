...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	ВедетсяУчетПрослеживаемыхТоваров  = ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
		И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Реквизиты.Дата);
	...
	КритерииПримененияВычета.Дата = Реквизиты.Дата;
	КритерииПримененияВычета.МенеджерДокумента = Документы.МодернизацияОС;
...
Функция ЗапрещеноПрименятьНалоговыйВычет(КритерииПримененияВычета) Экспорт
	...
	// Для документа Модернизация ОС нет реквизитов, делающих невозможным применение вычета.
	Возврат Ложь;
...
Функция ТекстЗапросаМодернизацияОС(НомераТаблиц)
	...
	|	ТаблицаОС.СуммаМодернизацииВР КАК СуммаМодернизацииВР,
	|	ТаблицаОС.СуммаКапитальныхВложенийВключаемыхВРасходыНУ КАК СуммаКапитальныхВложенийВключаемыхВРасходыНУ,
	|	ТаблицаОС.ПрименяетсяИнвестиционныйВычет КАК ПрименяетсяИнвестиционныйВычет
	|ИЗ
	|	Документ.МодернизацияОС.ОС КАК ТаблицаОС
	|ГДЕ
	|	ТаблицаОС.Ссылка = &Ссылка
...
Функция ТекстЗапросаИнвестиционныйВычет(НомераТаблиц)
	...
	|	&ПроцентИнвестиционногоВычета КАК ПроцентИнвестиционногоВычета
	|ИЗ
	|	Документ.МодернизацияОС КАК Реквизиты
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка
	|	И &ПроцентИнвестиционногоВычета > 0";
...
Функция ТекстЗапросаНДС(НомераТаблиц)
	...
	|	ТаблицаОС.СуммаМодернизацииБУ КАК Сумма
	|ИЗ
	|	Документ.МодернизацияОС.ОС КАК ТаблицаОС
	|ГДЕ
	|	ТаблицаОС.Ссылка = &Ссылка
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, ВедетсяУчетПрослеживаемыхТоваров)
	...
	Если НЕ ВедетсяУчетПрослеживаемыхТоваров Тогда
		...
		ПараметрыПроведения.Вставить("ПрослеживаемыеОСРасход", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	НомераТаблиц.Вставить("ПрослеживаемыеОСРасход",     НомераТаблиц.Количество());
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СУММА(МодернизацияОССведенияПрослеживаемости.Количество) КАК Количество,
	|	МодернизацияОССведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	СУММА(МодернизацияОССведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	МодернизацияОСОС.Ссылка КАК ДокументОперации,
	|	МодернизацияОССведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|	МодернизацияОС.Дата КАК Дата,
	|	МодернизацияОС.Организация КАК Организация,
	|	МодернизацияОС.ОбъектСтроительства КАК ОбъектСтроительства,
	|	МодернизацияОС.Номер КАК Номер,
	|	МодернизацияОССведенияПрослеживаемости.Номенклатура КАК Комплектующие,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.МодернизацияОС.СведенияПрослеживаемости КАК МодернизацияОССведенияПрослеживаемости
	|		ПО МодернизацияОСОС.Ссылка = МодернизацияОССведенияПрослеживаемости.Ссылка
	|			И МодернизацияОСОС.ИдентификаторСтроки = МодернизацияОССведенияПрослеживаемости.ИдентификаторСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.МодернизацияОС КАК МодернизацияОС
	|		ПО МодернизацияОСОС.Ссылка = МодернизацияОС.Ссылка
	|ГДЕ
	|	МодернизацияОСОС.ПрослеживаемыйТовар
	|	И МодернизацияОСОС.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	МодернизацияОССведенияПрослеживаемости.РНПТ,
	|	МодернизацияОСОС.Ссылка,
	|	МодернизацияОССведенияПрослеживаемости.СтранаПроисхождения,
	|	МодернизацияОС.Дата,
	|	МодернизацияОС.Организация,
	|	МодернизацияОС.ОбъектСтроительства,
	|	МодернизацияОС.Номер,
	|	МодернизацияОССведенияПрослеживаемости.Номенклатура,
	...
	|	ВТ_Прослеживаемость.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВТ_Прослеживаемость.ОбъектСтроительства КАК ОсновноеСредство,
	|	ВТ_Прослеживаемость.Комплектующие КАК Комплектующие
	|ИЗ
	|	ВТ_Прослеживаемость КАК ВТ_Прослеживаемость";
