...
Процедура ОбработкаПроведения(Отказ,РежимПроведения)
	...
	ТаблицаМодернизация = УчетОС.ПодготовитьТаблицуМодернизации(
		ПараметрыПроведения.МодернизацияОСТаблица,
		ПараметрыПроведения.МодернизацияОС);
	
	ТаблицаИнвестиционныйВычет = ИнвестиционныйНалоговыйВычет.ПодготовитьТаблицуИнвестиционныйВычетМодернизацияОС(
		ТаблицаМодернизация,
	...
	ПрослеживаемостьБП.СформироватьДвиженияСписанияОС(
		ПараметрыПроведения.ПрослеживаемыеОСРасход,
		Неопределено,
		ПараметрыПроведения.РеквизитыНДС,
		Движения);
		
	// РНПТ сохраняются по ОС
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВедетсяУчетПрослеживаемыхТоваров И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		ПрослеживаемостьБП.ПодобратьРНПТОССоставное(ЭтотОбъект, Отказ);
	ИначеЕсли Не ВедетсяУчетПрослеживаемыхТоваров Тогда
	...
	Если ТипЗнч(Основание) = Тип("СправочникСсылка.ОсновныеСредства") Тогда
		...
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			СтрокаТабличнойЧасти.ПрослеживаемыйТовар = Справочники.ОсновныеСредства.ПризнакПрослеживаемостиНаДату(
				Основание, НачалоДня(ОбщегоНазначения.ТекущаяДатаПользователя()));
		Иначе
