...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	НДСНеВыделять = Не Документы.ПутевойЛист.ВыделятьНДС(Организация, Дата);
	
	// По умолчанию считаем, что машина отправляется со своего адреса
	НоваяСтрока = Маршрут.Добавить();
	НоваяСтрока.ВидСообщения = Перечисления.ВидыСообщенияПеревозки.Городское;
	НоваяСтрока.ПунктОтправления = Документы.ПутевойЛист.АдресСУчетомКрайнегоРейса(Организация, Дата);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Запоминаем маршрут до изменения ранее проведенного документа
	Если Не Ссылка.Пустая() И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		РеквизитыДоИзменения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Организация, Дата, Маршрут");
		ДополнительныеСвойства.Вставить("РеквизитыДоЗаписи", Новый Структура);
		ДополнительныеСвойства.РеквизитыДоЗаписи.Вставить("Организация", РеквизитыДоИзменения.Организация);
		ДополнительныеСвойства.РеквизитыДоЗаписи.Вставить("Дата", РеквизитыДоИзменения.Дата);
		ДополнительныеСвойства.РеквизитыДоЗаписи.Вставить("Маршрут", РеквизитыДоИзменения.Маршрут.Выгрузить());
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	// Запоминаем сведения о маршруте
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		РеквизитыДоЗаписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "РеквизитыДоЗаписи");
		РегистрыСведений.СведенияОМаршрутеПутевогоЛиста.ЗарегистрироватьСведенияОМаршруте(
			Организация, Дата, Маршрут.Выгрузить(), РеквизитыДоЗаписи);
	ИначеЕсли Не ЭтоНовый И РежимЗаписи = РежимЗаписидокумента.ОтменаПроведения Тогда
		РегистрыСведений.СведенияОМаршрутеПутевогоЛиста.ОтменитьСведенияОМаршруте(
			Организация, Дата, Маршрут.Выгрузить());
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// НДС
	УчетНДС.СформироватьДвиженияПокупкаТопливаПоКассовомуЧеку(
		ПараметрыПроведения.ТаблицаНДС,
		ПараметрыПроведения.Реквизиты,
		Движения,
		Отказ);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ВодительскоеУдостоверение = Документы.ПутевойЛист.ВодительскоеУдостоверениеФизлица(ФизЛицо, Дата);
	СрокДействияУдостоверения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ВодительскоеУдостоверение, "СрокДействия");
	...
	ИначеЕсли ЗначениеЗаполнено(СрокДействияУдостоверения) И СрокДействияУдостоверения < Дата Тогда
		ТекстОшибки = НСтр("ru = 'Истёк срок действия водительского удостоверения сотрудника.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ФизЛицо, "ФизЛицо", "Объект", Отказ);
	КонецЕсли;
	
	Если Документы.ПутевойЛист.ВыводитьСНИЛСиДатуВыдачиВодительскогоУдостоверения(Дата) Тогда
		
		ДанныеФизическогоЛица = КадровыйУчет.КадровыеДанныеФизическогоЛица(Истина, ФизЛицо, "СтраховойНомерПФР", Дата);
		
		Если ДанныеФизическогоЛица = Неопределено Или Не ЗначениеЗаполнено(ДанныеФизическогоЛица.СтраховойНомерПФР) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнен СНИЛС сотрудника.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ФизЛицо, "ФизЛицо", "Объект", Отказ);
		КонецЕсли;
		
		ДатаВыдачиУдостоверения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ВодительскоеУдостоверение, "ДатаВыдачи");
		Если Не ЗначениеЗаполнено(ДатаВыдачиУдостоверения) Тогда
			ТекстОшибки = НСтр("ru = 'Не заполнена дата выдачи водительского удостоверения сотрудника. См. Справочники - Физическое лицо - Все Документы'");
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ФизЛицо, "ФизЛицо", "Объект", Отказ);
		КонецЕсли;
	...
	Если Не Документы.ПутевойЛист.ОтображатьВидСообщения(Дата) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Маршрут.ВидСообщения");
	КонецЕсли;
...
Процедура СообщитьОЗаполненииОбязательногоРеквизита(ИмяПоля, НомерСтроки, ИмяТабличнойЧасти, Отказ)
	...
	ПутьКТабличнойЧасти = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ИмяТабличнойЧасти, НомерСтроки, ИмяПоля);
	ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , ПутьКТабличнойЧасти, "Объект", Отказ);
