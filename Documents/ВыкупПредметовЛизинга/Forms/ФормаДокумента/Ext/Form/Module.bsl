...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ВыкупПредметовЛизинга", ПараметрыЗаписи, Объект.Ссылка);
	Если ЗначениеЗаполнено(СчетФактура) Тогда
...
Функция ДополнительныеСведенияОС(ОсновныеСредства)
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет В (&СчетаАрендныхОбязательств), &ВидыСубконтоКонтрагентыДоговоры, Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	ДоговорКонтрагента
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Сведения.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(Объект.Дата, Не Объект.НДСНеВыделять);
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ВыкупПредметовЛизинга.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
