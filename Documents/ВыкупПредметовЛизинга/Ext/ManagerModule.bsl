...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВыкупПредметовЛизингаОплата.Ссылка КАК Ссылка,
	|	ВыкупПредметовЛизингаОплата.НомерСтроки КАК НомерСтроки,
	|	ВыкупПредметовЛизингаОплата.ОсновноеСредство КАК ОсновноеСредство,
	|	ВыкупПредметовЛизингаОплата.ДатаОплаты КАК ДатаОплаты,
	|	ВыкупПредметовЛизингаОплата.СуммаОплаты КАК СуммаОплаты,
	|	ВыкупПредметовЛизингаОплата.СуммаНДС КАК СуммаНДС
	|ПОМЕСТИТЬ ТаблицаОплат
	|ИЗ
	|	Документ.ВыкупПредметовЛизинга.Оплата КАК ВыкупПредметовЛизингаОплата
	|ГДЕ
	|	ВыкупПредметовЛизингаОплата.Ссылка = &Ссылка"
...
Функция ТекстЗапросаОплатаОсновныхСредствДляУСН(НомераТаблиц)
	...
	|	ТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ТаблицаОС.СпособУчетаНДС КАК СпособУчетаНДС
	|ПОМЕСТИТЬ ВТ_СпособыУчетаНДСОС
	|ИЗ
	|	ТаблицаОС КАК ТаблицаОС
	|;
	|
	...
	|	ТаблицаОплат.НомерСтроки КАК НомерСтроки,
	|	ТаблицаОплат.ОсновноеСредство КАК ОсновноеСредство,
	|	ТаблицаОплат.ДатаОплаты КАК ДатаОплаты,
	|	ТаблицаОплат.СуммаОплаты КАК СуммаОплаты,
	|	ТаблицаОплат.СуммаНДС КАК СуммаНДС,
	|	ЕСТЬNULL(ВТ_СпособыУчетаНДСОС.СпособУчетаНДС, ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПринимаетсяКВычету)) КАК СпособУчетаНДС
	|ИЗ
	|	ТаблицаОплат КАК ТаблицаОплат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СпособыУчетаНДСОС КАК ВТ_СпособыУчетаНДСОС
	|		ПО ТаблицаОплат.ОсновноеСредство = ВТ_СпособыУчетаНДСОС.ОсновноеСредство
	|ГДЕ
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	ВыкупПредметовЛизинга.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	...
	|	И ВыкупПредметовЛизинга.Дата > &УсловиеПоДате";
...
Процедура ЗаполнитьСтавкуНДС(Параметры) Экспорт
	...
	|	Документ.ВыкупПредметовЛизинга.Оплата КАК ВыкупПредметовЛизингаОплата
	|ГДЕ
	|	ВыкупПредметовЛизингаОплата.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементБлокировки = Блокировка.Добавить("Документ.ВыкупПредметовЛизинга");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Выборка.Ссылка);
		Блокировка.Заблокировать();
		
		ВыкупПредметовЛизинга = Выборка.Ссылка.ПолучитьОбъект();
		
		Если ВыкупПредметовЛизинга = Неопределено Тогда
			ОтменитьТранзакцию();
			Продолжить;
		КонецЕсли;
		
		Для Каждого СтрокаОплаты Из ВыкупПредметовЛизинга.Оплата Цикл
		...
		Попытка
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВыкупПредметовЛизинга, Истина);
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Процедуре Документы.ВыкупПредметовЛизинга.ЗаполнитьСтавкуНДС() не удалось обработать документ %1 по причине:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ВыкупПредметовЛизинга.Ссылка,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ВыкупПредметовЛизинга,
				Выборка.Ссылка,
				ТекстСообщения);
		КонецПопытки;
	...
	Если ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре Документы.ВыкупПредметовЛизинга.ЗаполнитьСтавкуНДС() не удалось обработать документы: в %1 из %2 возникли ошибки'",
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ВыкупПредметовЛизинга.ЗаполнитьСтавкуНДС() успешно завершена: обработано %1 документов'", 
					ОбщегоНазначения.КодОсновногоЯзыка()),
				ОбъектовОбработано));
	КонецЕсли;
...
Функция СтавкаНДСЗаполнена(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры.Данные) = Тип("ДокументСсылка.ВыкупПредметовЛизинга") Тогда
		Документ = Параметры.Данные;
	ИначеЕсли ТипЗнч(Параметры.Данные) = Тип("ДокументОбъект.ВыкупПредметовЛизинга")
	...
	|ГДЕ
	|	ВыкупПредметовЛизингаОплата.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	|	И ВыкупПредметовЛизингаОплата.Ссылка = &Документ";
