...
Процедура ПеречислениеНДФЛПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		КонецЕсли;
		Если ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины Тогда
			ТекущиеДанные.Ставка = СтавкаНДФЛ15;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПеречислениеНДФЛФизическоеЛицоПриИзменении(Элемент)
	...
	Иначе
		Если НЕ ЗначениеЗаполнено(ТекущиеДанные.ФизическоеЛицо) Тогда
			ТекущиеДанные.Ставка = СтавкаНДФЛ13;
		Иначе
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыПеречислениеНДФЛСводно

&НаКлиенте
Процедура ПеречислениеНДФЛСводноПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПеречислениеНДФЛСводноПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
...
&НаКлиенте
Процедура ПеречислениеНДФЛСводноПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НоваяСтрока Тогда
		ТекущиеДанные = Элементы.ПеречислениеНДФЛСводно.ТекущиеДанные;
		ТекущиеДанные.НомерСтроки = ПеречислениеНДФЛ.Количество();
		Если ТекущиеДанные.НомерСтроки = 1 Тогда
			ТекущиеДанные.Сумма = Объект.СуммаДокумента;
		КонецЕсли;
...
&НаКлиенте
Процедура ПеречислениеНДФЛСводноПослеУдаления(Элемент)
...
Процедура ЗагрузитьПараметрыВРеквизитыФормы()
	...
	Объект.Вставить("Налог",           Справочники.ВидыНалоговИПлатежейВБюджет.ПустаяСсылка());
	...
	ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины =
		ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины(Объект.Налог,
			Справочники.ВидыНалоговИПлатежейВБюджет.КодВидаДоходаНДФЛАгент15());
		
	СтавкаНДФЛ15 = УчетЗарплаты.СтавкаНДФЛСПревышения();
	СтавкаНДФЛ13 = УчетЗарплаты.СтавкаНДФЛОсновная();
	
	// Для сводного учета - общая сумма документа.
	Если УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
		Если ПеречислениеНДФЛ.Количество() = 0 Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если НЕ УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
		Заголовок = НСтр("ru = 'Разбивка по сотрудникам'");
		Элементы.ГруппаТаблицыНДФЛ.ТекущаяСтраница = Элементы.ГруппаТаблицаНДФЛПоСотрудникам;
	Иначе
		Заголовок = НСтр("ru = 'Разбивка НДФЛ'");
		Элементы.ГруппаТаблицыНДФЛ.ТекущаяСтраница = Элементы.ГруппаТаблицаНДФЛСводно;
	КонецЕсли;
	
	ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли  = УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли();
	ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли = УчетНДФЛ.ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли();
	
	Элементы.ГруппаЗаполнитьЮЛ.Видимость                           = ЭтоЮрЛицо;
	Элементы.ЗаполнитьПоВедомости.Видимость                        = НЕ ЭтоЮрЛицо;
	Элементы.ПеречислениеНДФЛИсчисленоПоДивидендам.Видимость       = ЭтоЮрЛицо;
	Элементы.ПеречислениеНДФЛИсчисленоПоДивидендамСводно.Видимость = ЭтоЮрЛицо;
	Элементы.ПеречислениеНДФЛВключатьВДекларациюПоНалогуНаПрибыль.Видимость = ЭтоЮрЛицо
		И ((Объект.НалоговыйПериод < ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли
				И Объект.НалоговыйПериод >= ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли)
			ИЛИ (Объект.Дата < ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли
				И Объект.Дата >= ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли));
				
	Элементы.ПеречислениеНДФЛСтавка.ТолькоПросмотр = ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины;
...
Процедура ЗаполнитьСписокФизическихЛицПоВедомости(Ведомость)
	...
	Для Каждого СтрокаТаблицы ИЗ ФизическиеЛицаИСтавки Цикл
		...
		Если МассивСтрок.Количество() = 0 Тогда
			...
			Если ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины Тогда
				НоваяСтрока.Ставка = СтавкаНДФЛ15;
			Иначе
				СтавкаСтроки = СтрокаТаблицы.Ставка;
				Если СтавкаСтроки = СтавкаНДФЛ15 Тогда
					СтавкаСтроки = СтавкаНДФЛ13;
				КонецЕсли;
				НоваяСтрока.Ставка = СтавкаСтроки;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьСписокФизическихЛицПоДивидендам(НачислениеДивидендов)
	...
	Если ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины Тогда
		...
		ТекущаяСтрока.Ставка  = СтавкаНДФЛ15;
	Иначе
...
Процедура ОбновитьИтоги(Форма)
	...
	Форма.Элементы.ПеречислениеНДФЛСуммаСводно.ТекстПодвала = Формат(Форма.ПеречислениеНДФЛ.Итог("Сумма"), "ЧЦ=15; ЧДЦ=2; ЧН=' '") ;
