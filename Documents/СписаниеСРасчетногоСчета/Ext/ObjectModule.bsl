...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ЭтоУплатаОбеспечительногоПлатежа = Ложь;
	...
	ИначеЕсли ДанныеЗаполнения <> Неопределено И ТипДанныхЗаполнения <> Тип("Структура")
		...
		ВводНаОсновании = Истина;
		ЭтоУплатаОбеспечительногоПлатежа = ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров");
	...
	Если Не ЭтоУплатаОбеспечительногоПлатежа Тогда
	...
	Если РасшифровкаПлатежа.Количество() > 0 Тогда
		...
		УчетПСН.УстановитьПорядокОтраженияАванса(РасшифровкаПлатежа[0].ПорядокОтраженияДохода, ПараметрыЗаполненияАванса);
	КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ИсполнительныйЛист      = Неопределено;
	ПлатежнаяВедомость      = Неопределено;
	НачислениеДивидендов    = Неопределено;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	БюджетДоходовРасходов.СформироватьДвиженияРасходыФактБюджет(ПараметрыПроведения.БюджетРасходыФакт,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	БюджетДоходовРасходов.СформироватьДвиженияДоходыФактБюджет(ПараметрыПроведения.БюджетДоходыФакт,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
...
Процедура ПроверкаКурсовВалют(СтрокаПлатеж)
	...
	Если СтрокаПлатеж <> Неопределено Тогда
		СтрокаПлатеж.КурсВзаиморасчетов      =
			?(СтрокаПлатеж.КурсВзаиморасчетов      = 0, 1, СтрокаПлатеж.КурсВзаиморасчетов);
		СтрокаПлатеж.КратностьВзаиморасчетов =
			?(СтрокаПлатеж.КратностьВзаиморасчетов = 0, 1, СтрокаПлатеж.КратностьВзаиморасчетов);
	КонецЕсли;
...
Процедура ЗаполнитьПоВедомостям(ПлатежныеВедомости)
	...
	Если УчетЗарплаты.ВедомостьПоДоговоруПодряда(ПлатежнаяВедомость) Тогда
	...
	Иначе
		Для каждого СтрокаВедомости Из ТаблицаВедомостей Цикл
			...
			Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда Тогда
				// Обрабатываем только одну запись из одной ведомости
				Контрагент     = СтрокаВедомости.ФизическоеЛицо;
...
Процедура ЗаполнитьПоДокументуОснованию(Основание, СуммаКОплате = Неопределено, ДанныеПлПоручение = Неопределено)
		...
		Если ЗначениеЗаполнено(СуммаКОплате) И СуммаКОплате <> СуммаПоОснованиям Тогда
			...
			Иначе
				...
				Для каждого СтрокаОснования Из ТаблицаСуммОснования Цикл
					Если СтрокаОснования.СуммаПлатежа <> 0 Тогда
	...
	ИначеЕсли ТипЗначенияОснования = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		...
		ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание,
			...
			|НазначениеПлатежа, ДокументОснование, ПеречислениеВБюджет, ПлатежнаяВедомость, РеестрВыплатСамозанятым, СуммаДокумента, ВидПеречисленияВБюджет, ДоговорКонтрагента,
			|КодОКАТО, КодБК, ПоказательОснования, ПоказательТипа, КПППлательщика, НаправлениеДеятельности, СтатьяДвиженияДенежныхСредств, СтавкаНДС, СуммаНДС,
			|СуммаПроцентов, Сотрудник, ИсполнительныйЛист, КодВалютнойОперации, ИдентификаторОперацииЦР");
		...
		Если Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетОрганизации) Тогда
			НомерВходящегоДокумента = ДанныеОснования.ИдентификаторОперацииЦР;
		Иначе
		...
		НаправлениеДеятельности = ДанныеОснования.НаправлениеДеятельности;
		...
		СтатьяДвиженияДенежныхСредств = ДанныеОснования.СтатьяДвиженияДенежныхСредств;
		НаправлениеДеятельности       = ДанныеОснования.НаправлениеДеятельности;
		КодВалютнойОперации           = ДанныеОснования.КодВалютнойОперации;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым Тогда
			...
			РеестрВыплатСамозанятым = ДанныеОснования.РеестрВыплатСамозанятым;
			СуммаДокумента = ДанныеОснования.СуммаДокумента;
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога Тогда
			...
			Если ЗначениеЗаполнено(ВидНалога)
				...
				И ВидНалога <> Перечисления.ВидыНалогов.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС
				...
				ДанныеОснования.ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.Налог;
				ДанныеОснования.КодБК = Справочники.ВидыНалоговИПлатежейВБюджет.КБК(Налог);
			КонецЕсли;
			
			Если УчетДенежныхСредствКлиентСервер.ЭтоПериодДоЕдиногоНалоговогоПлатежа(Дата) Тогда
				ВидДДС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяДвиженияДенежныхСредств, "ВидДвиженияДенежныхСредств");
				Если Налог = Справочники.ВидыНалоговИПлатежейВБюджет.ЕдиныйНалоговыйПлатеж
					...
					СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ВидОперации, Дата);
				КонецЕсли;
			ИначеЕсли УчетДенежныхСредствКлиентСервер.ЭтоПериодДоЕдиногоНалоговогоПлатежа(ДанныеОснования.Дата) Тогда
				...
				ПравильнаяСтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(КонтекстОперации, Дата);
				Если ЗначениеЗаполнено(ПравильнаяСтатьяДвиженияДенежныхСредств) Тогда
					Если Не ЗначениеЗаполнено(СтатьяДвиженияДенежныхСредств) Тогда
						СтатьяДвиженияДенежныхСредств = ПравильнаяСтатьяДвиженияДенежныхСредств;
					ИначеЕсли СтатьяДвиженияДенежныхСредств <> ПравильнаяСтатьяДвиженияДенежныхСредств Тогда
						ПравильныйВидСтатьи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПравильнаяСтатьяДвиженияДенежныхСредств, "ВидДвиженияДенежныхСредств");
						УстановленныйВидСтатьи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяДвиженияДенежныхСредств, "ВидДвиженияДенежныхСредств");
						Если УстановленныйВидСтатьи <> ПравильныйВидСтатьи Тогда
							СтатьяДвиженияДенежныхСредств = ПравильнаяСтатьяДвиженияДенежныхСредств;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			...
			Если СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.НДФЛ
				...
				Если ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
					Если Справочники.Организации.КППНаДату(Организация, ДанныеОснования.Дата, Истина) <> ДанныеОснования.КПППлательщика
						И Справочники.Организации.КППНаДату(Организация, ДанныеОснования.Дата, Ложь) <> ДанныеОснования.КПППлательщика Тогда
			...
			Если ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровСредствамиБухгалтерии")
				...
				РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НачислениеДивидендов, "Дата, Учредитель, СуммаНалога, СуммаНалогаСПревышения");
				ТекущаяСтрока = ПеречислениеНДФЛ.Добавить();
				ТекущаяСтрока.ФизическоеЛицо        = РеквизитыДокумента.Учредитель;
				Если ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоНДФЛ_ДоходыСвышеПредельнойВеличины(Налог,
						Справочники.ВидыНалоговИПлатежейВБюджет.КодВидаДоходаНДФЛАгент15()) Тогда
					ТекущаяСтрока.Сумма = РеквизитыДокумента.СуммаНалогаСПревышения;
					ТекущаяСтрока.Ставка = УчетЗарплаты.СтавкаНДФЛСПревышения();
				Иначе
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника Тогда
			...
			СуммаДокумента     = ДанныеОснования.СуммаДокумента;
			ИсполнительныйЛист = ДанныеОснования.ИсполнительныйЛист;
		...
		Иначе
			...
			Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалогаЗаТретьихЛиц
				...
				РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента(
					ДоговорКонтрагента, Налогоплательщик, Организация, ВидыДоговоров, ПараметрыПлатежа);
				ЕстьПодходящиеДоговора = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
					ПараметрыПлатежа, "ЕстьПодходящиеДоговора", Ложь);
				
				ВестиУчетПоДоговорам = ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам");
				Если Не ЗначениеЗаполнено(ДоговорКонтрагента)
					И (Не ВестиУчетПоДоговорам Или Не ЕстьПодходящиеДоговора) Тогда
					ПараметрыДоговора = Новый Структура;
					ПараметрыДоговора.Вставить("ВидДоговора", ВидыДоговоров);
					ПараметрыДоговора.Вставить("Организация", Организация);
					ПараметрыДоговора.Вставить("Владелец",    Налогоплательщик);
					ПараметрыСоздания = Новый Структура("ЗначенияЗаполнения", ПараметрыДоговора);
					ДоговорКонтрагента = РаботаСДоговорамиКонтрагентовБПВызовСервера.СоздатьОсновнойДоговорКонтрагента(
						ПараметрыСоздания);
				КонецЕсли;
			...
			СписокВидовДоговора = СписаниеСРасчетногоСчетаФормыВызовСервера.ВидыДоговоровДокумента(ВидОперации);
			...
			ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратЗайма
				...
				Если ДанныеОснования.СуммаПроцентов = 0 Тогда
					...
					СтрокаПлатеж.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию("ПогашениеДолга", Дата);
				ИначеЕсли ДанныеОснования.СуммаДокумента = ДанныеОснования.СуммаПроцентов Тогда
					...
					СтрокаПлатеж.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию("УплатаПроцентов", Дата);
				Иначе
					СтрокаПлатеж.ВидПлатежаПоКредитамЗаймам = Перечисления.ВидыПлатежейПоКредитамЗаймам.ПогашениеДолга;
					...
					СтрокаПроцентов.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию("УплатаПроцентов", Дата);
		...
		Если Не УчетДенежныхСредствКлиентСервер.СписаниеСредствНаДругойСчет(ВидОперации)
	...
	ИначеЕсли ТипЗначенияОснования = Тип("ДокументСсылка.ВыплатыСамозанятым") Тогда
		ЗаполнитьПоРееструВыплатыСамозанятым(Основание);
	ИначеЕсли ТипЗначенияОснования = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров") Тогда
		ВалютаДокумента      = ВалютаРегламентированногоУчета;
		ВидОперации          = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога;
		Налог                = Справочники.ВидыНалоговИПлатежейВБюджет.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС;
		СуммаДокумента       = Окр(Основание.Товары.Итог("ОбеспечительныйПлатеж"));
		НазначениеПлатежа    = Документы.СписаниеСРасчетногоСчета.ПолучитьНазначениеПлатежа(ЭтотОбъект);
...
Процедура ЗаполнитьПоРееструВыплатыСамозанятым(Основание)
...
Процедура ЗаполнитьДаннымиБанка(ДанныеЗаполнения)
	...
	Если ДанныеЗаполнения.ХозяйственнаяОперация <> Неопределено Тогда
		...
		ЕстьОднаСтрокаРасшифровкиПлатежа = РасшифровкаПлатежа.Количество() = 1;
		Если (ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратКредита
				Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратЗайма)
			И ЕстьОднаСтрокаРасшифровкиПлатежа Тогда
		
			СтрокаРасшифровки = РасшифровкаПлатежа[0];
			ЗагрузкаВыпискиПоБанковскомуСчету.ЗаполнитьРеквизит(
				СтрокаРасшифровки, "ВидПлатежаПоКредитамЗаймам", ДанныеЗаполнения.ХозяйственнаяОперация);
		КонецЕсли;
		...
		Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым Тогда
			ЗагрузкаВыпискиПоБанковскомуСчету.ЗаполнитьРеквизит(ЭтотОбъект, "РеестрВыплатСамозанятым", ДанныеЗаполнения.ХозяйственнаяОперация); 
		КонецЕсли;
		
		Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника Тогда
			ЗагрузкаВыпискиПоБанковскомуСчету.ЗаполнитьРеквизит(ЭтотОбъект, "Сотрудник", ДанныеЗаполнения.ХозяйственнаяОперация);
		КонецЕсли;
