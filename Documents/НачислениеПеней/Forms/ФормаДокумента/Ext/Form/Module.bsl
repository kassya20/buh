...
&НаСервереБезКонтекста
Функция УсловияОплатыПоДоговору(Знач ДоговорКонтрагента)
	
	УсловияОплаты = Новый Структура;
	УсловияОплаты.Вставить("СрокОплаты", 0);
	УсловияОплаты.Вставить("ДатаОплаты", Дата(1, 1, 1));
	
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента, "СрокОплаты, ДатаОплаты");
		ЗаполнитьЗначенияСвойств(УсловияОплаты, ДанныеДоговора);
	КонецЕсли; 

	Если УсловияОплаты.СрокОплаты = 0 Тогда
		УсловияОплаты.СрокОплаты = Константы.СрокОплатыПокупателей.Получить();
	КонецЕсли;
...
Процедура РассчитатьПросрочку(ДанныеСтроки, Знач ДанныеОбъекта)
	...
	Перем СрокОплатыПоДоговору, ДатаОплатыПоДоговору;
	
	ПродолжительностьСуток = 86400;
	
	УсловияОплаты = УсловияОплатыПоДоговору(ДанныеОбъекта.ДоговорКонтрагента);
	
	Если НЕ ДанныеОбъекта.Свойство("СрокОплатыПоДоговору", СрокОплатыПоДоговору) Тогда
		СрокОплатыПоДоговору = УсловияОплаты.СрокОплаты;
	КонецЕсли; 
	
	Если Не ДанныеОбъекта.Свойство("ДатаОплатыПоДоговору", ДатаОплатыПоДоговору) Тогда
		ДатаОплатыПоДоговору = УсловияОплаты.ДатаОплаты;
	КонецЕсли;
	
	ДатаСделки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСтроки.Сделка, "Дата");
	
	Если ЗначениеЗаполнено(ДатаОплатыПоДоговору) Тогда
		Если ДатаОплатыПоДоговору >= НачалоДня(ДанныеОбъекта.Дата) Тогда
			ДанныеСтроки.Просрочка = 0;
		Иначе
			ДанныеСтроки.Просрочка = (НачалоДня(ДанныеОбъекта.Дата) - ДатаОплатыПоДоговору) / ПродолжительностьСуток;
		КонецЕсли;
	Иначе
		ДанныеСтроки.Просрочка =
			(НачалоДня(ДанныеОбъекта.Дата) - НачалоДня(ДатаСделки))/ПродолжительностьСуток - СрокОплатыПоДоговору;
	КонецЕсли;
...
Функция ОписаниеТипаРеквизитаСделка()
	...
	Возврат Метаданные.Документы.НачислениеПеней.ТабличныеЧасти.Задолженность.Реквизиты.Сделка.Тип;
