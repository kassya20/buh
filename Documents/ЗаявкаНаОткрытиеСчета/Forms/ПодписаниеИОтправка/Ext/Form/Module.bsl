...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТребуетсяЭП = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ТребуетсяЭП", Ложь);
...
Процедура НачатьПодписаниеИОтправку()
	...
	Если ПараметрыКриптографии.ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		// Передадим в фоновое задание настройки для обращения к серверу криптографии,
		// которые были созданы в пользовательском сеансе.
		ПараметрыПроцедуры.Вставить("МаркерыБезопасности", УниверсальныйОбменСБанками.МаркерыБезопасностиСервисаКриптографии());
	КонецЕсли;
	...
	Если ПараметрыКриптографии.ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания =
			НСтр("ru = 'Подписание и отправка заявки на открытие счета (облачный сертификат)'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ЗаявкаНаОткрытиеСчета.ПодписатьИОтправитьЭлектроннаяПодписьВМоделиСервиса",
			ПараметрыПроцедуры,
			ПараметрыВыполнения);
	...
	Иначе
		...
		ПараметрыВыполнения.ОжидатьЗавершение = 0;
		ПараметрыВыполнения.НаименованиеФоновогоЗадания =
			НСтр("ru = 'Подписание заявки на открытие счета (локальный сертификат)'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ЗаявкаНаОткрытиеСчета.ПодготовитьФайлыДляОтправки",
			ПараметрыПроцедуры,
			ПараметрыВыполнения);
...
Процедура ОбработатьУспешноеЗавершениеШага(Результат, ДополнительныеПараметры)
	...
	ИначеЕсли ДополнительныеПараметры.ИмяШага = "подписание" Тогда
		...
		ИдентификаторВременногоХранилищаТранзакций = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			Результат, 
...
Функция ИнформацияДляПодписания(ЗаявкаНаОткрытиеСчета)
	
	ДляПодписания = Новый Структура;
	ДляПодписания.Вставить("СостояниеПрогресса", ЗаявкиНаОткрытиеСчета.СостояниеПрогрессаПодписанияИОтправки());
	ДляПодписания.Вставить("Организация", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаявкаНаОткрытиеСчета, "Организация"));
...
Процедура Подключаемый_НачатьПодписаниеЛокальныйСертификат()
	...
	ДляПодписания = ИнформацияДляПодписания(Параметры.ЗаявкаНаОткрытиеСчета);
	...
	ПараметрыТранзакции.Предмет           = Параметры.ЗаявкаНаОткрытиеСчета;
	ПараметрыТранзакции.Форма             = ЭтотОбъект;
	ПараметрыТранзакции.Сервис            = Параметры.СервисОбменаСБанками;
	ПараметрыТранзакции.Организация       = ДляПодписания.Организация;
	
	ДобавитьМеткуДоверенногоВремени = Банки.НайтиСтроки(Новый Структура("ДобавитьМеткуДоверенногоВремени", Истина)).Количество() > 0;
	...
	ПараметрыТранзакции.ДобавитьМеткуДоверенногоВремени   = ДобавитьМеткуДоверенногоВремени;
...
Процедура НачатьОтправкуЛокальныйСертификат(РезультатПодписания)
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Отправка заявки на открытие счета (локальный сертификат)'");
	
	Если ДокументооборотыПолучателей <> Неопределено Тогда
		ДокументооборотыПолучателейДляОтправки = Новый Соответствие(ДокументооборотыПолучателей);
	КонецЕсли;

	Получатели = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатПодписания, "Получатели", Новый Массив);
	
	Если Получатели.Количество() = 0 Тогда
		
		Получатели = ОбщегоНазначения.ВыгрузитьКолонку(Банки, "Банк", Истина);
	...
	ПараметрыПроцедуры.Вставить("Банки",                Получатели);
	ПараметрыПроцедуры.Вставить("ДокументооборотыПолучателей", ДокументооборотыПолучателейДляОтправки);
