...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	НДСВключенВСтоимость = НЕ УчетнаяПолитика.ПлательщикНДС(Организация, Дата)
		И НЕ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Организация, Дата);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Если ВыборкаДокументов.ПодакцизныйТовар И ВыборкаДокументов.СуммаАкциза <> 0 И Не ЗначениеЗаполнено(ВыборкаДокументов.СчетАкциза) Тогда
			ТекстСообщения = НСтр("ru = 'Не заполнен ""Счет акциза""'");
			
			Поле = "" + ИмяСписка + "["+ Формат(ВыборкаДокументов.НомерСтроки-1,"ЧГ=") + "].СчетАкциза";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле, "Объект", Отказ);
		КонецЕсли;
...
Функция ПолучитьТаблицыДляПроверкиЗаполнения()
	...
	ТаблицаТовары  = Товары.Выгрузить(, "НомерСтроки, Номенклатура, НомерРаздела, СчетУчетаНДС, СуммаНДС, СпособУчетаНДС, СуммаПошлины, СчетУчетаНУ, СтатьяЗатратНУ, ПодакцизныйТовар, СуммаАкциза, СчетАкциза");
	...
	|	ВремТаблица.НомерРаздела КАК НомерРаздела,
	|	ВремТаблица.СчетУчетаНДС КАК СчетУчетаНДС,
	|	ВремТаблица.СуммаНДС КАК СуммаНДС,
	|	ВремТаблица.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ВремТаблица.СуммаПошлины КАК СуммаПошлины,
	|	ВремТаблица.СчетУчетаНУ КАК СчетУчетаНУ,
	|	ВремТаблица.СтатьяЗатратНУ КАК СтатьяЗатратНУ,
	|	ВремТаблица.ПодакцизныйТовар КАК ПодакцизныйТовар,
	|	ВремТаблица.СуммаАкциза КАК СуммаАкциза,
	|	ВремТаблица.СчетАкциза КАК СчетАкциза
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	|	Разделы.СуммаПошлины КАК СуммаПошлины,
	|	Разделы.СуммаНДС КАК СуммаНДС,
	|	Разделы.СтавкаНДС КАК СтавкаНДС,
	|	ТоварыИтоги.ТоварыКоличествоСтрок КАК ТоварыКоличествоСтрок,
	|	ТоварыИтоги.ТоварыСуммаПошлины КАК ТоварыСуммаПошлины,
	|	ТоварыИтоги.ТоварыСуммаНДС КАК ТоварыСуммаНДС
	|ИЗ
	|	Разделы КАК Разделы
	...
	|	Товары.СчетУчетаНУ КАК СчетУчетаНУ,
	|	ЕСТЬNULL(Счета.СтатьяНУОбязательна, ЛОЖЬ) КАК СтатьяНУОбязательна,
	|	Товары.СтатьяЗатратНУ КАК СтатьяЗатратНУ,
	|	Товары.СчетУчетаНДС КАК СчетУчетаНДС,
	...
	|	Товары.СчетАкциза КАК СчетАкциза
	|ИЗ
	|	Товары КАК Товары
