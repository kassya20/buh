...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьРеквизитСпособЗачетаАвансов");
	Методы.Вставить("ЗаполнитьОтражениеВУСНДокументовПрошлыхЛет");
	Методы.Вставить("ЗаполнитьОтражениеВУСНАктуальныхДокументов");
...
//
Процедура ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, ИмяТабличнойЧасти, ЕстьУчетПодакцизныхТоваров) Экспорт

	ТабличнаяЧасть = Объект[ИмяТабличнойЧасти];
	
	ДанныеОбъекта = Новый Структура("Дата, Организация, Склад, ЕстьУчетПодакцизныхТоваров");
...
Процедура ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабличнойЧасти, СведенияОНоменклатуре) Экспорт
	...
	Если ИмяТабличнойЧасти = "Товары" Тогда
		...
		Если ДанныеОбъекта.ЕстьУчетПодакцизныхТоваров И ЗначениеЗаполнено(СчетаУчета.СчетАкцизаПоИмпорту) Тогда
...
Процедура УстановитьПорядокУчетаНДСПоУмолчанию(Объект) Экспорт
	...
	ВключитьНДСВСтоимость = НЕ УчетнаяПолитика.ПлательщикНДС(Объект.Организация, Объект.Дата)
		И НЕ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Объект.Организация, Объект.Дата);
...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	Набор.Имя = "Документ_ГТДИмпорт";
	Набор.Идентификатор = Новый УникальныйИдентификатор("26815594-8f40-4aec-bee7-4c002b351196");
...
Функция ТоварыПоДаннымПоступления(ДанныеОбъекта, ДокументПоступления, НомерРаздела) Экспорт
	...
	Для каждого СтрокаТоваров Из ТаблицаЗначенийТовары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаРезультата.СчетУчета) Тогда
			...
			Если ЕстьУчетПодакцизныхТоваров Тогда
				СтрокаРезультата.СчетАкциза = СчетаУчета.СчетАкцизаПоИмпорту;
			КонецЕсли;
		Иначе
		...
		СтрокаРезультата.ФактурнаяСтоимость = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
		СтрокаТоваров.Сумма,
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		...
		УчетАкцизов.ЗаполнитьДанныеПоАкцизам(ТаблицаРезультата, ДанныеОбъекта.Организация, ДанныеОбъекта.Дата, ПараметрыЗаполнения);
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Реквизиты.Вставить("ЕстьУчетПодакцизныхТоваров", ПолучитьФункциональнуюОпцию("ИмпортПодакцизныхТоваров"));
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ГТДИмпортТовары.СтатьяЗатратНУ КАК Справочник.ПрочиеДоходыИРасходы).ПринятиеКналоговомуУчету, Истина)
	|		ИНАЧЕ ИСТИНА
	...
	|	ГТДИмпортТовары.Склад КАК Склад
	...
	|	ВЫБОР
	|		КОГДА ТоварыДокумента.ДокументПартии ССЫЛКА Документ.ПоступлениеТоваровУслуг
	|				И ТоварыДокумента.ДокументПартии.СкладВСписке 
	|			ТОГДА ТоварыДокумента.Склад
	|		ИНАЧЕ ТоварыДокумента.ДокументПартии.Склад
	|	КОНЕЦ КАК Склад,
...
Функция ТекстЗапросаНДС(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ТоварыДокумента.ДокументПартии.СкладВСписке
	|			ТОГДА ТоварыДокумента.Склад
	|		ИНАЧЕ ТоварыДокумента.ДокументПартии.Склад 
	|	КОНЕЦ КАК Склад,
	|	ТоварыДокумента.ДокументПартии.ПодразделениеОрганизации КАК Подразделение,
...
Функция ТекстЗапросаАкцизы(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	Если Не Реквизиты.ЕстьУчетПодакцизныхТоваров Тогда
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ГТДИмпорт.Ссылка КАК Регистратор,
	|	ГТДИмпорт.Дата КАК Период,
	|	ГТДИмпорт.Организация КАК Организация,
	|	ГТДИмпорт.Контрагент КАК Контрагент,
	|	ГТДИмпорт.ДоговорКонтрагентаРегл КАК ДоговорКонтрагента,
	...
	|	ГТДИмпортТовары.СчетАкциза КАК СчетАкциза,
	|	ГТДИмпортТовары.ДокументПартии.Склад КАК Склад,
	|	ГТДИмпортТовары.ДокументПартии.ПодразделениеОрганизации КАК Подразделение,
	|	ГТДИмпортТовары.СчетУчета КАК СчетУчета
	|ИЗ
	|	Документ.ГТДИмпорт.Товары КАК ГТДИмпортТовары
	|ГДЕ
	|	ГТДИмпортТовары.Ссылка = &Ссылка
	|	И ГТДИмпортТовары.СуммаАкциза <> 0
	|	И ГТДИмпортТовары.ПодакцизныйТовар = ИСТИНА";
