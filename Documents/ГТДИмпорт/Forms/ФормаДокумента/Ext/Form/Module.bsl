...
Процедура РазделыПередУдалением(Элемент, Отказ)
	Для каждого ИдентификаторСтроки Из Элементы.Разделы.ВыделенныеСтроки Цикл
		СтрокаРаздела = Объект.Разделы.НайтиПоИдентификатору(ИдентификаторСтроки);
		НомерРаздела = Объект.Разделы.Индекс(СтрокаРаздела) + 1;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|ПодакцизныйТовар, СуммаАкциза, СтавкаАкциза, ЕдиницаИзмеренияАкциза,
		|КоэффициентПересчета, ОкруглятьДоЦелого, СчетАкциза");
	...
	ДанныеОбъекта.Вставить("ВидыПроизведенныхПодакцизныхТоваров", ВидыПроизведенныхПодакцизныхТоваров);
...
Процедура ТоварыДокументПартииПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		"Номенклатура, ДокументПартии, СчетУчета, СчетУчетаНДС, СчетУчетаНУ, СтатьяЗатратНУ, СпособУчетаНДС, СчетАкциза"); 
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, ТекущиеДанные);
	
	ДанныеОбъекта = Новый Структура("Дата, Организация, ЕстьУчетПодакцизныхТоваров");
	ЗаполнитьЗначенияСвойств(ДанныеОбъекта, Объект);
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДокументПартии) Тогда
		ТекущиеДанные.Склад = ПолучитьСкладПоДокументуПартии(ТекущиеДанные.ПолучитьИдентификатор());
		ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти();
	КонецЕсли;
...
Процедура ЗаполнитьПоПоступлению(ДокументПоступления, НомерРаздела)
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		...
		УчетАкцизов.ЗаполнитьДанныеПоАкцизам(ТоварыПоДаннымПоступления, ДанныеОбъекта.Организация, ДанныеОбъекта.Дата, ПараметрыЗаполнения);
	КонецЕсли;
...
Функция РассчитатьСуммуНДСГТД(Знач ДанныеСтроки, Знач ДанныеОбъекта, Знач ВалютаРеглУчета)
	...
	ИначеЕсли ДанныеСтроки.НДСВВалюте Тогда  // Надо все пересчитать в валюту расчетов с таможней.
		...
		Иначе
			ТаможеннаяСтоимостьВал = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
				ДанныеСтроки.ТаможеннаяСтоимость,
				ДанныеОбъекта.ВалютаДокумента, ДанныеОбъекта.ВалютаРасчетов,
				ДанныеОбъекта.КурсДокумента, ДанныеОбъекта.КурсВзаиморасчетов,
				ДанныеОбъекта.КратностьДокумента, ДанныеОбъекта.КратностьВзаиморасчетов);
		КонецЕсли;
		...
		Иначе
			...
			Иначе
				СуммаПошлиныВал = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(ДанныеСтроки.СуммаПошлины,
...
Процедура ОбработкаВыбораПодбор(ВыбранноеЗначение, ИмяТаблицы)
	...
	ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, ИмяТаблицы, НомерРаздела, ЕстьУчетПодакцизныхТоваров, ВидыПроизведенныхПодакцизныхТоваров);
	
КонецПроцедуры
 

&НаСервере
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение, ИмяТаблицы, НомерРаздела, ЕстьУчетПодакцизныхТоваров, ВидыПроизведенныхПодакцизныхТоваров)
		...
		Если ЕстьУчетПодакцизныхТоваров Тогда
			...
			Если ЗначениеЗаполнено(КодВидаПодакцизногоТовара) Тогда
				...
				Если СтрокаТабличнойЧасти.ПодакцизныйТовар Тогда
					...
					УчетАкцизовКлиентСервер.РассчитатьСуммуАкцизаВТабЧасти(СтрокаТабличнойЧасти, Ложь, Истина);
				КонецЕсли;
			Иначе
				СтрокаТабличнойЧасти.СуммаАкциза            = 0;
				СтрокаТабличнойЧасти.КоэффициентПересчета   = 0;
...
Процедура ЗаполнитьСчетаУчетаВТабличнойЧастиНаСервере(ИмяТабличнойЧасти = "")
	...
	Если ПустаяСтрока(ИмяТабличнойЧасти) ИЛИ ИмяТабличнойЧасти = "Товары" Тогда
		Документы.ГТДИмпорт.ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, "Товары", ЕстьУчетПодакцизныхТоваров);
	КонецЕсли;
	...
	Если ПараметрыОбработки.Свойство("СтруктураЦеныИВалюта") Тогда
		...
		Если СтруктураЦеныИВалюта.ПересчитатьЦены Тогда
			...
			Для Каждого Строка Из Объект.Товары Цикл
				Строка.ФактурнаяСтоимость = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
	...
	Если ПараметрыОбработки.Свойство("ВалютаРасчетовПередИзменением") Тогда
		...
		Если НЕ (ПараметрыОбработки.ВалютаРасчетовПередИзменением = ВалютаРегламентированногоУчета
			...
			Иначе
				...
				Объект.ТаможенныйСборВал 	= РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(Объект.ТаможенныйСборВал, 
				...
				Объект.ТаможенныйШтрафВал 	= РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(Объект.ТаможенныйШтрафВал,
				ПараметрыОбработки.ВалютаРасчетовПередИзменением, ВалютаРасчетов,
				СтруктураКурсаДокумента.Курс, Объект.КурсВзаиморасчетов, СтруктураКурсаДокумента.Кратность, Объект.КратностьВзаиморасчетов);
...
Процедура ТоварыДокументПартииПриИзмененииСервер(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументПартии) Тогда
		Если ТипЗнч(СтрокаТабличнойЧасти.ДокументПартии) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
			Склад = Документы.ПоступлениеТоваровУслуг.ПолучитьСкладТабличнойЧастиДокументаПоступления(СтрокаТабличнойЧасти.ДокументПартии, СтрокаТабличнойЧасти.Номенклатура);
			ДанныеОбъекта.Вставить("Склад", Склад); 
			Если Не СтрокаТабличнойЧасти.Свойство("Склад") Тогда 
				СтрокаТабличнойЧасти.Вставить("Склад", Склад);
			КонецЕсли;
		Иначе
			ДанныеОбъекта.Вставить("Склад", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТабличнойЧасти.ДокументПартии, "Склад"));
		КонецЕсли;
...
Процедура УправлениеВидимостьюЗакладок()
	...
	Элементы.ГруппаНДС.Видимость = ПлательщикНДС
		И НЕ РаздельныйУчетНДСНаСчете19 
		И (УпрощенныйУчетНДС ИЛИ ВерсияУчетаНДС = 2);
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		...
		УчетАкцизовКлиентСервер.ПриИзмененииНоменклатурыКоличества(ЭтотОбъект, "Товары", Ложь, Истина);
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ГТДИмпорт.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
