...
Функция НаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	|	И (ПоступлениеДопРасходов.СуммаНДС > 0
	|			ИЛИ ПоступлениеДопРасходов.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ПоступлениеДопРасходовТовары.Ссылка = &СсылкаНаДокумент
	|	И ПоступлениеДопРасходовТовары.СуммаНДС > 0";
...
Функция НаличиеСтавкиНДСПоУСН(ТекущийДокумент, СтавкаНДСПоУСН) Экспорт
	...
	|ГДЕ
	|	ПоступлениеДопРасходов.Ссылка = &СсылкаНаДокумент
	|	И ПоступлениеДопРасходов.СтавкаНДС В (&СтавкаНДСПоУСН)";
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|   ВЫБОР
	|		КОГДА ТаблицаТовары.ДокументПартии ССЫЛКА Документ.ПоступлениеТоваровУслуг И ТаблицаТовары.ДокументПартии.СкладВСписке 
	|			ТОГДА ТаблицаТовары.Склад
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаТовары.ДокументПартии.Склад, ЕСТЬNULL(ДополнительныеРеквизитыПартии.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))) 
	|	КОНЕЦ КАК Склад,
...
Функция ТекстЗапросаРасчетыВВалютеТовары()
	...
	|	ВременнаяТаблицаТовары.СчетЗатрат,
	|	ВременнаяТаблицаТовары.ПодразделениеЗатрат,
	|	ВременнаяТаблицаТовары.Субконто1,
	|	ВременнаяТаблицаТовары.Субконто2,
	|	ВременнаяТаблицаТовары.Субконто3,
	|	ВременнаяТаблицаТовары.Малоценка,
...
Функция РаспределениеСуммыДопРасходовАУСН(Знач ДопРасходыРеквизиты, Знач ДопРасходыТаблица) Экспорт

	Если ДопРасходыТаблица = Неопределено Тогда 
		Возврат Неопределено;
	КонецЕсли;	
	
	РаспределенныеРасходы = РаспределениеСуммыДопРасходов(ДопРасходыРеквизиты, ДопРасходыТаблица);
	
	Для Каждого СтрокаРаспределения Из РаспределенныеРасходы Цикл
		Если СтрокаРаспределения.НалоговыйУчет <> Перечисления.ОтражениеВУСН.Принимаются Тогда
			// Заменим недопустимые при УСН-онлайн значения на "Не принимаются".
			СтрокаРаспределения.НалоговыйУчет = Перечисления.ОтражениеВУСН.НеПринимаются;
		КонецЕсли;
	КонецЦикла;
	
	// Приведем таблицу к виду, требуемому для алгоритма зачета аванса поставщику при УСН-онлайн.
	
	РаспределенныеРасходы.Свернуть("Контрагент, ДоговорКонтрагента, ДокументРасчетов, НалоговыйУчет",
		"СуммаВзаиморасчетов");
	
	РаспределенныеРасходы.Колонки.СуммаВзаиморасчетов.Имя = "Сумма";
	
	Возврат РаспределенныеРасходы;
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	ПоступлениеДопРасходов.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	...
	|ГДЕ
	|	ПоступлениеДопРасходов.Ссылка В(&МассивОбъектов)
	|	И ПоступлениеДопРасходов.ДатаВходящегоДокумента > &УсловиеПоДате";
