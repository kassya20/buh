...
Процедура ПриКопировании(ОбъектКопирования)
	...
	СсылкаНаЧек = "";
	НомерЧека = "";
	ЧекСамозанятого = Справочники.ПоступлениеДопРасходовПрисоединенныеФайлы.ПустаяСсылка();
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаРасходовАУСН = Документы.ПоступлениеДопРасходов.РаспределениеСуммыДопРасходовАУСН(
		ПараметрыПроведения.ДопРасходыРеквизиты,
	...
	УчетТоваров.СформироватьДвиженияПоступлениеДопРасходов(
...
Процедура ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Если ЗначениеЗаполнено(ДоговорКонтрагента) 
			И ВидыДоговоров.Найти(РеквизитыДоговора.ВидДоговора) = Неопределено Тогда
			
			СообщениеОбОщибке = НСтр("ru = 'Поступление допрасходов не может быть введено на основании документа ""Поступление (акт, накладная, УПД)"",
...
Процедура ЗаполнитьПоПоступлению(ДанныеЗаполнения) Экспорт
	...
	|	ВЫБОР 
	|		КОГДА Док.Ссылка ССЫЛКА Документ.ПоступлениеТоваровУслуг И Док.Ссылка.СкладВСписке 
	|			ТОГДА Док.Склад
