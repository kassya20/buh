...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ДокументПартии - ПоступлениеТоваровУслуг на несколько складов
	ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти();
	//Конец ПоступлениеТоваровУслуг на несколько складов
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "ОбщаяФорма.УниверсальныйПередаточныйДокумент" Тогда
		...
		УчетНДСКлиентСервер.ОтобразитьПолеУПД(ЭтотОбъект);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОригиналСчетаФактуры И Не ЗначениеЗаполнено(СчетФактура) И Не ЗначениеЗаполнено(ДатаСчетаФактурыПолученного)
		И Не ЗначениеЗаполнено(НомерСчетаФактурыПолученного) Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоУниверсальныйДокумент
	   ...
	   И Документы.ПоступлениеДопРасходов.НаличиеСуммыНДС(Объект.Ссылка) Тогда
		...
		ПараметрыСчетаФактуры.Продавцы = Продавцы;
		ПараметрыСчетаФактуры.КодВидаОперации = КодВидаОперации;
		ПараметрыСчетаФактуры.НДСПредъявленКВычету = НДСПредъявленКВычету;
	...
	ИначеЕсли ЗначениеЗаполнено(НомерСчетаФактурыПолученного)
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИспользуетсяПланированиеПлатежей Тогда
		СрокиОплатыДокументов.ОбновитьСрокОплаты(ТекущийОбъект.Ссылка, СрокОплаты, АвторасчетСрокаОплаты);
		ПараметрыЗаписи.Вставить("СрокОплаты", СрокОплаты);
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	КонтейнерОбъект = Новый Структура("Организация, Дата, СуммаВключаетНДС, СтавкаНДС, ПрименяетсяМалоценка");
	ЗаполнитьЗначенияСвойств(КонтейнерОбъект, Объект);
	КонтейнерОбъект.ПрименяетсяМалоценка = ПрименяетсяМалоценка;

	КонтейнерСтрока = Новый Структура("Номенклатура, Количество, Сумма, СтавкаНДС, СуммаНДС, Малоценка, АналитикаУчета, АналитикаУчетаЗаполнена");
...
Процедура ТоварыДокументПартииПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекСтрока.ДокументПартии) Тогда
		ТекСтрока.Склад = ПолучитьСкладПоДокументуПартии(ТекСтрока.ПолучитьИдентификатор());
		ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти();
		ЗаполнитьСчетаУчетаВСтрокеПоДокументуПартии(ТекСтрока.ПолучитьИдентификатор());
	КонецЕсли;
...
Процедура ТоварыДокументПартииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ТипыДокументов"              , ОписаниеТипаРеквизитаДокументПартии());
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ДоговорУказан Тогда
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДоговорКонтрагента,
			"ВидДоговора,УчетАгентскогоНДС,РасчетыВУсловныхЕдиницах,НДСПоСтавкам4и2,НалогообложениеКонтрагента");

		ЭтоКомиссияНаЗакупку = РеквизитыДоговора.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку;
		ЭтоДоговорССамозанятым = РеквизитыДоговора.НалогообложениеКонтрагента =
		...
		ТребуетсяСчетФактура = ТребуетсяСчетФактура(
			ВестиУчетПоДоговорам, ДоговорУказан, Объект.НДСНеВыделять, УчетАгентскогоНДС, ЭтоДоговорССамозанятым);
	Иначе
		...
		ТребуетсяСчетФактура = ТребуетсяСчетФактура(ВестиУчетПоДоговорам, ДоговорУказан, Объект.НДСНеВыделять);
...
Процедура ДатаОбработатьИзменение()
	...
	Если УчетНДСБП.ВыделятьНДСПоУмолчанию(НалогообложениеКонтрагента) Тогда
		...
		Если Объект.НДСНеВыделять <> НДСНеВыделять Тогда
			...
			Объект.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(Объект.Дата, Не НДСНеВыделять);
			СтавкаНДСПриИзмененииНаСервере();
		КонецЕсли;
	КонецЕсли;
...
Процедура ДоговорКонтрагентаОбработатьИзменение(ПричиныИзменения)
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДоговорКонтрагента,
		"ВалютаВзаиморасчетов,ВидДоговора,УчетАгентскогоНДС,РасчетыВУсловныхЕдиницах,
		|НДСПоСтавкам4и2,НалогообложениеКонтрагента");
		
	ДоговорУказан = ЗначениеЗаполнено(Объект.ДоговорКонтрагента);
	
	Если ДоговорУказан И
		Не УчетНДСБП.ВыделятьНДСПоУмолчанию(РеквизитыДоговора.НалогообложениеКонтрагента) Тогда
		Объект.НДСНеВыделять = Истина;
		Объект.НДСВключенВСтоимость = Истина;
	ИначеЕсли Объект.НДСНеВыделять И ПлательщикНДС Тогда
		Объект.НДСНеВыделять = Ложь;
	КонецЕсли;
	
	ЭтоКомиссияНаЗакупку    = ДоговорУказан
		И РеквизитыДоговора.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку;
	ЭтоДоговорССамозанятым  = ДоговорУказан И РеквизитыДоговора.НалогообложениеКонтрагента =
		Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход;
	...
	СпособЗаполненияСтавкиНДС = ПараметрыДоговора.СпособЗаполненияСтавкиНДС;
	...
	ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры
		И Не УчетАгентскогоНДС
		И Объект.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС Тогда
		Объект.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(Объект.Дата);
		СтавкаНДСИзменилась = Истина;
	ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ОбщаяУСН
		И Не УчетАгентскогоНДС
		И Объект.СтавкаНДС <> УчетНДСКлиентСервер.ОбщаяУСНСтавкаНДС(Объект.Дата) Тогда
		Объект.СтавкаНДС = УчетНДСКлиентСервер.ОбщаяУСНСтавкаНДС(Объект.Дата);
		СтавкаНДСИзменилась = Истина;
	ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН
		И Не УчетАгентскогоНДС
		И Объект.СтавкаНДС <> УчетНДСКлиентСервер.ПониженнаяУСНСтавкаНДС(Объект.Дата) Тогда
		Объект.СтавкаНДС = УчетНДСКлиентСервер.ПониженнаяУСНСтавкаНДС(Объект.Дата);
		СтавкаНДСИзменилась = Истина;
	КонецЕсли;
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(ДанныеСтроки, Знач ДанныеОбъекта, Знач СчетаУчетаКЗаполнению)
	...
	Если ДанныеОбъекта.ПрименяетсяМалоценка
		И ДанныеСтроки.Свойство("Малоценка")
		И ДанныеСтроки.Свойство("СчетУчета") Тогда
		ДанныеСтроки.Малоценка = ДанныеСтроки.СчетУчета = ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыСуммаТовара");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.СпособРаспределения", ВидСравненияКомпоновкиДанных.Равно, Перечисления.СпособыРаспределенияДопРасходов.ПоКоличеству);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Ложь);
	
	// ТоварыКоличествоТовара
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.СпособРаспределения", ВидСравненияКомпоновкиДанных.Равно, Перечисления.СпособыРаспределенияДопРасходов.ПоСумме);
    ...
    ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "УказыватьСкладВТабличнойЧасти", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
...
&НаКлиентеНаСервереБезКонтекста
Функция ТребуетсяСчетФактура(ВестиУчетПоДоговорам, ДоговорУказан, НДСНеВыделять, УчетАгентскогоНДС = Ложь, ЭтоДоговорССамозанятым = Ложь)
	
	Если ДоговорУказан И ЭтоДоговорССамозанятым Тогда
		Результат = Ложь;
	ИначеЕсли ДоговорУказан Тогда
		Результат = Не УчетАгентскогоНДС И Не НДСНеВыделять;
	Иначе
		Результат = Не ВестиУчетПоДоговорам И Не НДСНеВыделять;
	КонецЕсли;
...
Процедура ЗарегистрироватьСчетФактуру(Команда)
	...
	РеквизитыСФ = УчетНДСКлиент.СоздатьСчетФактуруПолученный(ЭтотОбъект, ЭтоКомиссияНаЗакупку);
...
#Область СкладыВТабличнойЧасти

&НаСервере
Функция ПолучитьСкладПоДокументуПартии(ИдентификаторСтроки)
	
	Склад = Справочники.Склады.ПустаяСсылка();
	
	ТекСтрока = Объект.Товары.НайтиПоИдентификатору(ИдентификаторСтроки);
	Если Не ТекСтрока = Неопределено Тогда
		Склад = Документы.ПоступлениеТоваровУслуг.ПолучитьСкладТабличнойЧастиДокументаПоступления(ТекСтрока.ДокументПартии, ТекСтрока.Номенклатура);
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти()
	
	УказыватьСкладВТабличнойЧасти = ЗаполнениеДокументов.ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти(Объект.Товары);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСчетаУчетаВСтрокеПоДокументуПартии(ИдентификаторСтроки)
	...
	ДокументПартии = СтрокаТовары.ДокументПартии;
	Если ТипЗнч(ДокументПартии) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументПартии, "СкладВСписке") Тогда
		Если ЗначениеЗаполнено(СтрокаТовары.Склад) Тогда
			СтруктураПоиска = Новый Структура("Номенклатура, Склад", СтрокаТовары.Номенклатура, СтрокаТовары.Склад);
			СтрокаТоварыПартии = ОбщегоНазначенияБП.НайтиСтрокуТаблицы(ДокументПартии.Товары, СтруктураПоиска);
			Если СтрокаТоварыПартии <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(СтрокаТовары, СтрокаТоварыПартии, "СчетУчета");
				СтрокаТовары.СчетУчетаНУ = СтрокаТоварыПартии.СчетУчета;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ОписаниеТипаРеквизитаДокументПартии()
	...
	Возврат Новый ОписаниеТипов(
		Метаданные.Документы.ПоступлениеДопРасходов.ТабличныеЧасти.Товары.Реквизиты.ДокументПартии.Тип,
		,
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ПоступлениеДопРасходов.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
...
Функция ПараметрыСозданияНовогоДоговора()
	...
	Возврат РаботаСДоговорамиКонтрагентовБП.ПараметрыСозданияНовогоДоговора(ЭтотОбъект,
		?(ЗначениеЗаполнено(Объект.ДатаВходящегоДокумента), "ДатаВходящегоДокумента", "Дата"));
...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.ДатаВходящегоДокумента.Заголовок = НСтр("ru = 'Документ Дата'");
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаВходящиеНомерДата);
	...
	Элементы.ГруппаСумма.Группировка=ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы,
		Элементы.ГруппаГлавное);
	...
	Элементы.ГруппаНомерДатаСчетаФакутры.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	...
	Элементы.СчетФактураСсылка.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	...
	Элементы.Итоги.ГоризонтальноеПоложениеПодчиненных = ГоризонтальноеПоложениеЭлемента.Авто;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаПодвал(Элементы, Элементы.Итоги);
	
	МобильныйКлиентБПФормаОбъекта.УстановитьИсключенияИзАдаптации(ЭтотОбъект, "Сумма,СтавкаНДС,СуммаНДС,СпособРаспределения");
...
Процедура АдаптироватьМКСписокФормы()
	...
	ОписаниеКолонокПоля.СкрытьКолонки = "";
	...
	Элементы.ТоварыСуммаТовара.Формат = "ЧФ='Сумма: Ч'";
	Элементы.ТоварыСумма.Формат = "ЧФ='Доп.расходы: Ч'";
