...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ИспользоватьРасчетПервойПоловиныМесяца = УчетЗарплаты.ИспользоватьРасчетПервойПоловиныМесяца(Объект.Организация, Объект.МесяцНачисления);
		Если ИспользоватьРасчетПервойПоловиныМесяца Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
				Если Параметры.ТолькоОкончательныйРасчет Тогда
					Объект.ПерваяПоловинаМесяца = Ложь;
				Иначе
					...
					КонецЕсли;
				КонецЕсли;
				Если Не Объект.ПерваяПоловинаМесяца Тогда
					...
					Если Объект.Дата > Объект.МесяцНачисления И Объект.Дата > ДатыВыплаты.Зарплата Тогда
						Объект.МесяцНачисления = НачалоМесяца(Объект.Дата);
						Если Не Параметры.ТолькоОкончательныйРасчет Тогда
							Если МодульРасчетЗарплатыДляНебольшихОрганизаций.ЭтоДатаВыплатыАванса(Объект.Организация, Объект.МесяцНачисления, Объект.Дата) Тогда
								Объект.ПерваяПоловинаМесяца = Истина;
							КонецЕсли;
						КонецЕсли;
			...
			КонецЕсли;
		КонецЕсли;
		ИспользуетсяЕдинаяДатаВыплатыДохода = УчетЗарплаты.ИспользуетсяЕдинаяДатаВыплатыДохода(Объект.Организация, Объект.МесяцНачисления);
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			"ДобавитьНачисление",
			"Видимость",
			ПравоДоступа("Редактирование", Метаданные.ПланыВидовРасчета.Начисления));
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ДобавитьДоговорПодряда",
			"Видимость",
			ПравоДоступа("Редактирование", Метаданные.Документы.ДоговорРаботыУслуги));
			
	КонецЕсли;
	
	УстановитьУсловноеОформление();
	УстановитьДоступностьСортировки(ЭтотОбъект);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ТипЗнч(Источник) = Тип("ФормаКлиентскогоПриложения") И Источник.ВладелецФормы = ЭтотОбъект Тогда
		...
		ИначеЕсли ИмяСобытия = "ДобавленДоговор" Тогда
			ДобавитьДоговорНаСервере(Параметр);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьСотрудников(Знач СписокСотрудников)
	...
	Если ДанныеСотрудников <> Неопределено Тогда
		...
		Для Каждого ДобавляемыйСотрудник Из СотрудникиКДобавлению Цикл
			...
			ДополнитьСписокОбновляемыхФизическихЛиц(СтрокаСотрудники.ФизическоеЛицо);
	...
	РезультатНачисления = РасчетЗарплаты.РезультатНачисленияРасчетаЗарплаты(
		...
		?(ЕдинаяДатаВыплатаДокумента, ПланируемаяДатаВыплаты, '00010101'));
		
	Если РезультатНачисления.Количество() > 0 Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			
			Если РезультатНачисления.Колонки.Найти("ОтработаноДней") = Неопределено Тогда
				РезультатНачисления.Колонки.Добавить("ОтработаноДней");
			КонецЕсли;
			Если РезультатНачисления.Колонки.Найти("ОтработаноЧасов") = Неопределено Тогда
				РезультатНачисления.Колонки.Добавить("ОтработаноЧасов");
			КонецЕсли;
			Если РезультатНачисления.Колонки.Найти("ПерваяПоловинаМесяца") = Неопределено Тогда
				РезультатНачисления.Колонки.Добавить("ПерваяПоловинаМесяца");
			КонецЕсли;
	...
	Если Не Объект.ПерваяПоловинаМесяца И ИспользоватьНачисленияПоДоговорам Тогда
		РезультатНачисленияПоДоговорам = РасчетЗарплатыБазовый.РезультатНачисленияПоДоговорам(
			Объект.Организация,
			Объект.МесяцНачисления,
			Объект.Ссылка,
			Объект.Подразделение,
		...
		Если РезультатНачисленияПоДоговорам.Количество() > 0 Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатНачисленияПоДоговорам, Объект.НачисленияПоДоговорам);
...
&НаКлиенте
Процедура ВидНачисленияЧисломПриИзменении(Элемент)
	
	Объект.ПерваяПоловинаМесяца = ВидНачисленияЧислом;
	ВидНачисленияЧисломПриИзмененииНаСервере();
...
Процедура ПланируемаяДатаВыплатыПриИзменении(Элемент)
	...
	Если ПланируемаяДатаВыплаты < Объект.МесяцНачисления Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Дата выплаты не может быть меньше даты начала расчетного месяца.'"));
		ПланируемаяДатаВыплаты = ПланируемаяДатаВыплатыПредыдущая;
	Иначе
		ОбработатьИзменениеДатыВыплаты();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПланируемаяДатаВыплатыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Если Не ЗначениеЗаполнено(ПланируемаяДатаВыплатыПредыдущая) Тогда
		ПланируемаяДатаВыплатыПредыдущая = ПланируемаяДатаВыплаты;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПланируемаяДатаВыплатыОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ЕдинаяДатаВыплатаДокументаПриИзменении(Элемент)
	
	Если ЕдинаяДатаВыплатаДокументаПредыдущая = ЕдинаяДатаВыплатаДокумента Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЕдинаяДатаВыплатаДокумента Тогда
		УправлениеРеквизитамиДатыВыплаты();
	Иначе
		Если ЗначениеЗаполнено(ПланируемаяДатаВыплаты) И ПланируемаяДатаВыплаты = ПланируемаяДатаВыплатыПоДокументу() Тогда
			УправлениеРеквизитамиДатыВыплаты();
		Иначе
			ВопросПриВключенииДатыВыплаты();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ЕдинаяДатаВыплатаДокументаОчистка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ЕдинаяДатаВыплатаДокументаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ЕдинаяДатаВыплатаДокументаПредыдущая = ЕдинаяДатаВыплатаДокумента;
...
&НаКлиенте
Процедура РедактированиеДатыВыплатыЗавершение(Знач РезультатРедактирования, Знач ДополнительныеПараметры) Экспорт
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СотрудникиГруппаКнопокНачисленияДоговоры",
		"Видимость",
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		Не (ПрименяетсяАУСН
		Или Объект.ПерваяПоловинаМесяца));
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СотрудникиПересчитатьВзносы",
		"Видимость",
		Не (ПрименяетсяАУСН
		Или Объект.ПерваяПоловинаМесяца));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ФормаКарточкаУчетаПоСтраховымВзносам",
		"Видимость",
		Не (ПрименяетсяАУСН
		Или Объект.ПерваяПоловинаМесяца));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ФормаРасчетныйЛисток",
		"Видимость",
		Не Объект.ПерваяПоловинаМесяца);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаКнопокНачислить",
		"Видимость",
		Не Объект.ПерваяПоловинаМесяца);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СотрудникиПодробнееОСотруднике",
		"Видимость",
		Не Объект.ПерваяПоловинаМесяца
		И ЕстьУволенныеСотрудники());
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СотрудникиСведенияОСотруднике");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Сотрудники.ДатаУвольнения", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Отображать", Ложь);
...
&НаСервере
Процедура ПланируемаяДатаВыплатыЗарплатыПоНастройкам()
...
&НаСервере
Функция МассивДатыВыплатПоДокументу(Сотрудник = Неопределено)
	
	Если ЗначениеЗаполнено(Сотрудник) Тогда
		ТаблицаНачисления            = Объект.Начисления.Выгрузить(Новый Структура("Сотрудник", Сотрудник));
		ТаблицаНачисленияПоДоговорам = Объект.НачисленияПоДоговорам.Выгрузить(Новый Структура("Сотрудник", Сотрудник));
	Иначе
		ТаблицаНачисления            = Объект.Начисления;
		ТаблицаНачисленияПоДоговорам = Объект.НачисленияПоДоговорам;
	КонецЕсли;
	
	МассивДатВыплаты = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаНачисления,"ПланируемаяДатаВыплаты", Истина);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивДатВыплаты, ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаНачисленияПоДоговорам,"ПланируемаяДатаВыплаты", Истина));
	МассивДатВыплаты = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивДатВыплаты);
	
	Возврат МассивДатВыплаты;
	
КонецФункции

&НаСервере
Функция ПланируемаяДатаВыплатыПоДокументу()
	
	ДатаВыплатыПоДокументу = '00010101';
	
	МассивДатВыплаты = МассивДатыВыплатПоДокументу();
	Если МассивДатВыплаты.Количество() = 1 Тогда
		ДатаВыплатыПоДокументу = МассивДатВыплаты[0];
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ДатаВыплатыПоДокументу) И Не ИспользуетсяЕдинаяДатаВыплатыДохода Тогда
		ДатаВыплатыПоДокументу = КонецМесяца(Объект.МесяцНачисления);
	КонецЕсли;
		
	Возврат ДатаВыплатыПоДокументу;
	
КонецФункции

&НаСервере
Процедура ОпределитьВозможностьУправленияДатойВыплаты()
	
	ЕдинаяДатаВыплатаДокумента = Ложь;
	
	Если ИспользуетсяЕдинаяДатаВыплатыДохода Тогда
		МассивДатВыплаты = МассивДатыВыплатПоДокументу();
		Если МассивДатВыплаты.Количество() = 0 Тогда
			ЕдинаяДатаВыплатаДокумента = Истина;
		Иначе
			ОбщаяДатаВыплаты = МассивДатВыплаты[0];
			ЕдинаяДатаВыплатаДокумента = Не ЗначениеЗаполнено(ОбщаяДатаВыплаты)
				Или (ПланируемаяДатаВыплаты = ОбщаяДатаВыплаты);
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УправлениеРеквизитамиДатыВыплаты()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ИспользоватьРасчетПервойПоловиныМесяца
			И ИспользуетсяЕдинаяДатаВыплатыДохода);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ПланируемаяДатаВыплаты",
		"Видимость",
		ЕдинаяДатаВыплатаДокумента);
	
КонецПроцедуры

&НаСервере
Функция ЕстьУволенныеСотрудники()
	
	ЕстьУволенные = Ложь;
	
	ДатыУвольнения = ОбщегоНазначения.ВыгрузитьКолонку(Объект.Сотрудники, "ДатаУвольнения", Истина);
	Если ДатыУвольнения.Количество() = 1 Тогда
		ЕстьУволенные = ЗначениеЗаполнено(ДатыУвольнения[0]);
	ИначеЕсли ДатыУвольнения.Количество() > 1 Тогда
		ЕстьУволенные = Истина;;
	КонецЕсли;
	
	Возврат ЕстьУволенные;
...
Процедура ДобавитьНачисление(Команда)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СуффиксКоманды) Тогда
			...
			Начисление = СоответствиеКомандНачислений[НомерКоманды-1].Значение;
		Иначе
			Начисление = ПредопределенноеЗначение("ПланВидовРасчета.Начисления.ПустаяСсылка");
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыВвода.Вставить("ДатаВыплатыДохода",          ПланируемаяДатаВыплаты);
	...
	ПараметрыВвода.Вставить("ЕдинаяДатаВыплатаДокумента", ЕдинаяДатаВыплатаДокумента);
...
Процедура ДобавитьДоговорПодряда(Команда)
	...
	ОткрытьФормуВводаДоговора(ПараметрыВвода);
...
Процедура ПересчитатьВзносы(Команда)
	...
	ПересчитатьНДФЛИВзносыСотрудников(Ложь, Истина);
...
Процедура ПересчитатьНДФЛ(Команда)
	...
	ПересчитатьНДФЛИВзносыСотрудников(Истина, Ложь);
...
Процедура РегистрыНалоговогоУчетаПоНДФЛ(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Отчет.РегистрНалоговогоУчетаПоНДФЛ",
		"РегистрНалоговогоУчетаПоНДФЛ",
		РегистрНалоговогоУчетаПоНДФЛ,
		ЭтотОбъект,
		Новый Структура("ЗаголовокФормы", НСтр("ru='Регистры налогового учета по НДФЛ'")));
...
Процедура КарточкиУчетаПоСтраховымВзносам(Команда)
	...
	КарточкаУчета = КарточкаУчетаПоСтраховымВзносамНаСервере();
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(
		...
		Новый Структура("ПечатаемыйДокумент, ЗаголовокФормы",
			КарточкаУчета, НСтр("ru='Карточки учета по страховым взносам'")));
...
Процедура АнализЗарплаты(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Отчет.АнализНачисленийИУдержаний",
		"АнализЗарплатыПоПодразделениямИСотрудникамЗаПериод",
		АнализЗарплатыПодробнее,
		ЭтотОбъект,
		Новый Структура("ЗаголовокФормы", НСтр("ru='Анализ зарплаты'")));
...
Процедура СводНачисленийИУдержаний(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Отчет.АнализНачисленийИУдержаний",
		...
		СводНачисленийИУдержанийПодробнее,
		ЭтотОбъект,
		Новый Структура("ЗаголовокФормы", НСтр("ru='Свод начислений и удержаний'")));
...
Процедура НалогиИВзносы(Команда)
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Отчет.НалогиИВзносыКратко",
		"НалогиИВзносыПодробнее",
		НалогиИВзносыПодробнее,
		ЭтотОбъект,
		Новый Структура("ЗаголовокФормы", НСтр("ru='Налоги и взносы'")));
...
Процедура ПодготовитьФормуНаСервере()
	...
	ВидНачисленияЧислом    = Объект.ПерваяПоловинаМесяца;
	Если Параметры.Ключ.Пустая() Тогда
		ПланируемаяДатаВыплатыЗарплатыПоНастройкам();
	Иначе
		ПланируемаяДатаВыплаты = ПланируемаяДатаВыплатыПоДокументу();
	КонецЕсли;
	...
	ОпределитьВозможностьУправленияДатойВыплаты();
	...
	УстановитьПараметрыОтбораВсехНачислений();
...
Процедура ЗаполнитьНаСервере()
	...
	РасчетЗарплаты.ЗаполнитьДокументНачисленияЗарплаты(Объект, Объект.ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
	ПослеЗаполненияНаСервере();
...
Процедура ОбработатьИзменениеОрганизацииНаСервере()
	...
	Если Не ИспользоватьРасчетПервойПоловиныМесяца Тогда
		Объект.ПерваяПоловинаМесяца = Ложь;
		ВидНачисленияЧислом = Объект.ПерваяПоловинаМесяца;
	КонецЕсли;
	...
	УправлениеДолейЕНВД();
	ОчиститьТабличныеЧасти();
	УстановитьВидимостьКомандыЗаполненияДокумента();
	УстановитьПараметрыОтбораВсехНачислений();
...
Процедура ОбработатьИзменениеМесяцНачисленияНаСервере()
	...
	ОпределитьВозможностьУправленияДатойВыплаты();
	Если Объект.ПерваяПоловинаМесяца Тогда
		Объект.Дата = НачалоДня(ПланируемаяДатаВыплаты);
	Иначе
	...
	Если ЕстьЗаполненныеТабличныеЧасти(ЭтотОбъект) Тогда
		ЗаполнитьНаСервере();
	КонецЕсли; 
...
&НаСервере
Процедура ВидНачисленияЧисломПриИзмененииНаСервере()
	...
	Если Объект.ПерваяПоловинаМесяца Тогда
		...
		Объект.НачисленияПоДоговорам.Очистить();
	Иначе
		ПараметрыРасчета = Новый Структура("НДФЛ, Взносы", Ложь, Истина);
		ПересчитатьДокументНаСервере(ПараметрыРасчета);
	КонецЕсли;
	
	ПланируемаяДатаВыплатыЗарплатыПоНастройкам();
	Если ИспользуетсяЕдинаяДатаВыплатыДохода Тогда
		ЕдинаяДатаВыплатаДокумента = Истина;
		ИзменитьДатуВыплаты(ЭтотОбъект);
	КонецЕсли;
	ПараметрыРасчета = Новый Структура("НДФЛ, Взносы", Истина, Ложь);
	ПересчитатьДокументНаСервере(ПараметрыРасчета);
...
&НаКлиенте
Процедура ВопросПриВключенииДатыВыплаты()
	
	Если ЕстьЗаполненныеТабличныеЧасти(ЭтотОбъект) Тогда
		ТекстВопроса = НСтр("ru = 'Изменение даты выплаты приведет к перерасчету НДФЛ.
			|Продолжить?'");
		Если Объект.Сотрудники.Количество() > 1 Тогда
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Для всех сотрудников будет установлена общая дата выплаты.
				|%1'"), ТекстВопроса);
		КонецЕсли;
		Оповещение = Новый ОписаниеОповещения("ВопросРедактироватьДатыВыплатыСотрудниковЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ОтключениеРедактированияДатыВыплаты();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВопросРедактироватьДатыВыплатыСотрудниковЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Если Не ЗначениеЗаполнено(ПланируемаяДатаВыплаты) Тогда
			ПланируемаяДатаВыплатыЗарплатыПоНастройкам();
		КонецЕсли;
		Если ЗначениеЗаполнено(ПланируемаяДатаВыплаты) Тогда
			ОтключениеРедактированияДатыВыплаты();
			ОбработатьИзменениеДатыВыплаты();
		Иначе
			ПараметрыФормы = Новый Структура("НачалоПериода, КонецПериода", ТекущаяДата(), ТекущаяДата());
			ОписаниеОповещения = Новый ОписаниеОповещения("ВводДатыВыплатыЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ВводДатыВыплатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		ЕдинаяДатаВыплатаДокумента = Ложь;
		УправлениеРеквизитамиДатыВыплаты();
	Иначе
		ПланируемаяДатаВыплаты = Результат.НачалоПериода;
		ОтключениеРедактированияДатыВыплаты();
		ОбработатьИзменениеДатыВыплаты();
	КонецЕсли;
...
&НаСервере
Процедура ОтключениеРедактированияДатыВыплаты()
...
Процедура ПоказатьВсеНачисления(ПараметрыВвода)
	...
	ОткрытьФорму("ПланВидовРасчета.Начисления.ФормаВыбора",
		Новый Структура("Отбор, ДоступенПолныйСписок", ПараметрыОтбораВсехНачислений, Не Объект.ПерваяПоловинаМесяца),
		ЭтотОбъект,
		,
		,
		,
...
&НаСервере
Процедура УстановитьПараметрыОтбораВсехНачислений()
	
	ПараметрыОтбораВсехНачислений = Новый Структура();
	
	Категории = Новый Массив;
	Категории.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.Прочее);
	...
	Если ОрганизацияПрименяетРайонныйКоэффициент Тогда
		Категории.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке);
	КонецЕсли;
	Если ОрганизацияПрименяетСевернуюНадбавку Тогда
		Категории.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиСевернойНадбавкиВСреднемЗаработке);
	КонецЕсли;
	
	МассивОсновныхВидовРасчета = ОсновныеВидыРасчетов();
	
	ПараметрыОтбораВсехНачислений.Вставить("КатегорияНачисленияИлиНеоплаченногоВремени", Категории);
	ПараметрыОтбораВсехНачислений.Вставить("КатегорияДохода",                            КатегорииДохода);
	Если ЗначениеЗаполнено(МассивОсновныхВидовРасчета) Тогда
		ПараметрыОтбораВсехНачислений.Вставить("ОсновнойВидРасчета", МассивОсновныхВидовРасчета);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ОсновныеВидыРасчетов()
	...
	Если ОрганизацияПрименяетРайонныйКоэффициент Или ОрганизацияПрименяетСевернуюНадбавку Тогда
		СтруктураОтбора = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", КатегорииНачисления);
		НачисленияКомпенсацияОтпуска = ПланыВидовРасчета.Начисления.НачисленияПоОтбору(СтруктураОтбора);
	...
	Возврат МассивОсновныхВидовРасчета;
...
Процедура ОткрытьФормуВводаНачисления(Параметры)
	...
	ОткрытьФормуВводаРасчета(Параметры, "ФормаДобавитьНачисление");
...
&НаКлиенте
Процедура ОткрытьФормуВводаДоговора(Параметры)
	
	ОткрытьФормуВводаРасчета(Параметры, "ФормаДобавитьДоговор");
...
Процедура ОткрытьФормуВводаРасчета(Параметры, ИмяФормы)
	...
	ОткрытьФорму("Документ.НачислениеЗарплаты.Форма." + ИмяФормы, 
		ПараметрыОткрытия,
		ЭтотОбъект,
		УникальныйИдентификатор,
		,
...
Процедура ЗаполнитьКнопкиПодменю()
	...
	Если ОрганизацияПрименяетРайонныйКоэффициент Тогда
		КатегорииНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке);
	КонецЕсли;
	Если ОрганизацияПрименяетСевернуюНадбавку Тогда
		КатегорииНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиСевернойНадбавкиВСреднемЗаработке);
	КонецЕсли;
	...
	Если НЕ ПолучитьФункциональнуюОпцию("РасчетЗарплатыДляНебольшихОрганизаций") Тогда
		...
		КатегорииНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.КвартальнаяПремия);
	...
	КатегорииНачисления = Новый ФиксированныйМассив(КатегорииНачислений);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		КатегорииДоходов = МодульРасчетЗарплатыДляНебольшихОрганизаций.КатегорииДоходовНДФЛПриАУСН(Объект.Организация, Объект.МесяцНачисления);
	КонецЕсли;
	КатегорииДоходов.Добавить(Перечисления.КатегорииДоходовНДФЛ.ПустаяСсылка());
	
	КатегорииДохода = Новый ФиксированныйМассив(КатегорииДоходов);
	...
	|ГДЕ
	|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииНачисления)
	|	И Начисления.КатегорияДохода В(&КатегорииДохода)
	|	И НЕ Начисления.ПометкаУдаления
	|	И Начисления.ОсновнойВидРасчета В(&ОсновныеВидыРасчета)
	|	И НЕ Начисления.ВАрхиве
	|
	|УПОРЯДОЧИТЬ ПО
	|	Наименование";
	
	Запрос.УстановитьПараметр("КатегорииНачисления", КатегорииНачисления);
	...
	Если Не ЗначениеЗаполнено(МассивОсновныхВидовРасчета) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.ОсновнойВидРасчета В(&ОсновныеВидыРасчета)", "Истина");
	КонецЕсли;
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Для НомерКонтейнера = 1 По СоответствиеКомандНачислений.Количество() Цикл
		Элементы.Удалить(Элементы["ДобавитьНачисление" + НомерКонтейнера]);
	КонецЦикла;
	
	СоответствиеКомандНачислений.Очистить();
	НомерКонтейнера = 1;
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ПрименяетсяАУСН = 
		УчетнаяПолитика.ПрименяетсяАУСН(Объект.Организация, Объект.МесяцНачисления);
	ПоддержкаСовместительства =
		ПолучитьФункциональнуюОпцию("ПоддержкаСовместительства");
	ИспользоватьРасчетПервойПоловиныМесяца =
		УчетЗарплаты.ИспользоватьРасчетПервойПоловиныМесяца(Объект.Организация, Объект.МесяцНачисления);
	ИспользуетсяЕдинаяДатаВыплатыДохода
		= УчетЗарплаты.ИспользуетсяЕдинаяДатаВыплатыДохода(Объект.Организация, Объект.МесяцНачисления);
...
Процедура УправлениеДолейЕНВД()
	...
	Элементы.ГруппаДоляЕНВД.Видимость = ОтражениеЗарплатыВБухучете.ПлательщикЕНВД(Объект.Организация, Объект.МесяцНачисления)
		И НЕ ЗарплатаКадрыПереопределяемый.ПлательщикТолькоЕНВД(Объект.Организация, Объект.МесяцНачисления);
		
	Элементы.ГруппаДоляЕНВД.Доступность = НЕ ТолькоПросмотр;
...
Процедура УстановитьДоступностьСортировки(Форма)
	
	Элементы = Форма.Элементы;
	Объект = Форма.Объект;
	
	СортировкаДоступна = Объект.Сотрудники.Количество() <> 0;
	Элементы.СотрудникиПереместитьВверх.Доступность                = СортировкаДоступна;
	Элементы.СотрудникиПереместитьВниз.Доступность                 = СортировкаДоступна;
	Элементы.СотрудникиСортироватьПоВозрастанию.Доступность        = СортировкаДоступна;
	Элементы.СотрудникиСортироватьПоУбыванию.Доступность           = СортировкаДоступна;
	Элементы.СотрудникиКонтекстноеМенюПереместитьВверх.Доступность = СортировкаДоступна;
	Элементы.СотрудникиКонтекстноеМенюПереместитьВниз.Доступность  = СортировкаДоступна;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьКомандыЗаполненияДокумента()
	
	// При превышении тарифного ограничения на количество сотрудников не отображается команда автоматического 
	// заполнения документа
	РезультатПроверки = УчетЗарплаты.РезультатПроверкиУсловийТарификацииПоКоличествуСотрудников(Объект.Организация);
	Элементы.Заполнить.Видимость = РезультатПроверки.УсловияВыполнены;
...
Процедура ПерейтиКНачислениям()
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("АдресПараметровВХранилище",  АдресПараметровВХранилище(Сотрудник, ФизическоеЛицо, "Начисления,НачисленияПоДоговорам"));
...
Процедура ПерейтиКНДФЛ()
	...
	ПараметрыФормы.Вставить("ДатаПолученияДохода",                      ПланируемаяДатаВыплаты);
...
Функция СведенияОбНДФЛ(ВходящиеДанные = Неопределено) Экспорт
	...
	Иначе
		...
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("ВходящийОбъект", Новый ОписаниеТипов("ДокументОбъект.НачислениеЗарплаты")));
	...
	Если ЕдинаяДатаВыплатаДокумента Тогда
		ПланируемаяДатаВыплатыРасчета = ПланируемаяДатаВыплаты;
	Иначе
		Если Не ИспользуетсяЕдинаяДатаВыплатыДохода Тогда
			...
			Если КадровыеДанные.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(ДатаУвольнения)
					...
					ПланируемаяДатаВыплатыРасчета = ДатаУвольнения;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ПланируемаяДатаВыплатыРасчета) Тогда
			МассивДатВыплаты = МассивДатыВыплатПоДокументу(Сотрудник);
			Для Каждого ДатаВыплаты Из МассивДатВыплаты Цикл
				ПланируемаяДатаВыплатыРасчета = Макс(ПланируемаяДатаВыплатыРасчета, ДатаВыплаты);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ПланируемаяДатаВыплатыРасчета) Тогда
		ПланируемаяДатаВыплатыРасчета = КонецМесяца(ОбъектФормы.МесяцНачисления);
	КонецЕсли;
...
Процедура ПодобратьСотрудников(МножественныйВыбор)
	...
	КадровыйУчетКлиент.ВыбратьСотрудниковРаботающихВПериодеПоПараметрамОткрытияФормыСписка(
		...
		?(ИспользоватьНачисленияПоДоговорам И Не Объект.ПерваяПоловинаМесяца,
			Новый Структура("ДоступныДГПХ", Истина),
			Неопределено));
...
Процедура ОбновитьДанныеНачисленийНаСервере(АдресВременногоХранилища)
	...
	РасчетЗарплатыБазовый.ЗаменитьСтрокиНаНовыеДанные(Объект.НачисленияПоДоговорам, Параметр.НачисленияПоДоговорам, "Сотрудник", ОтборСтрокСотрудники);
...
&НаСервере
Процедура ДобавитьДоговорНаСервере(АдресВременногоХранилища)
	...
	НоваяСтрока = Объект.НачисленияПоДоговорам.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Параметр.НачисленияПоДоговорам);
...
&НаКлиентеНаСервереБезКонтекста
Функция ЕстьЗаполненныеТабличныеЧасти(Форма)
...
Процедура ЗаполнитьВторичныеДанныеТабличныхЧастей(ВыбранныеСотрудники = Неопределено)
	...
	Для Каждого СтрокаНачисления Из Объект.Начисления Цикл
		СтрокаНачисления.ПерваяПоловинаМесяца = Объект.ПерваяПоловинаМесяца;
	КонецЦикла;
...
Процедура РассчитатьИтогНачислено()
	...
	ИтогНачислено                  = Объект.Сотрудники.Итог("ВсегоНачислено");
...
Процедура ОбновитьНДФЛСотрудников(МассивСотрудников)
	...
	Документы.НачислениеЗарплаты.ОбновитьНДФЛиВзносыСотрудников(Объект, ПолныйМассивСотрудников(МассивСотрудников), Истина, Ложь);
...
Процедура ОбновитьВзносыСотрудников(МассивСотрудников)
	...
	Документы.НачислениеЗарплаты.ОбновитьНДФЛиВзносыСотрудников(Объект, ПолныйМассивСотрудников(МассивСотрудников), Ложь, Истина);
...
Процедура ОбновитьНДФЛиВзносыСотрудников(Сотрудники)
	...
	Документы.НачислениеЗарплаты.ОбновитьНДФЛиВзносыСотрудников(Объект, ПолныйМассивСотрудников(Сотрудники));
...
Функция ПолныйМассивСотрудников(Сотрудники)
	...
	ТаблицаСотрудников = Документы.НачислениеЗарплаты.СотрудникиФизическихЛиц(Объект, МассивФизическихЛиц);
...
&НаКлиенте
Процедура ОбработатьИзменениеДатыВыплаты()
	
	ИзменитьДатуВыплаты(ЭтотОбъект);
	Для Каждого СтрокаТаблицы Из Объект.Начисления Цикл
		ДополнитьСтрокиКРасчету(СтрокаТаблицы.Сотрудник, "НДФЛ");
	КонецЦикла;
	Для Каждого СтрокаТаблицы Из Объект.НачисленияПоДоговорам Цикл
		ДополнитьСтрокиКРасчету(СтрокаТаблицы.Сотрудник, "НДФЛ");
	КонецЦикла;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ИзменитьДатуВыплаты(Форма)
	
	Объект = Форма.Объект;
	
	Для Каждого СтрокаТаблицы Из Объект.Начисления Цикл
		СтрокаТаблицы.ПланируемаяДатаВыплаты = Форма.ПланируемаяДатаВыплаты;
	КонецЦикла;
	Для Каждого СтрокаТаблицы Из Объект.НачисленияПоДоговорам Цикл
		СтрокаТаблицы.ПланируемаяДатаВыплаты = Форма.ПланируемаяДатаВыплаты;
	КонецЦикла;
	
	Форма.ПланируемаяДатаВыплатыПредыдущая = Форма.ПланируемаяДатаВыплаты;
...
Функция АдресПараметровВХранилище(Сотрудник, ФизическоеЛицо, ИменаТаблиц)
	...
	ПараметрыРасчета.Вставить("ДатаВыплатыДохода",                      ПланируемаяДатаВыплаты);
	ПараметрыРасчета.Вставить("ИспользоватьРасчетПервойПоловиныМесяца", ИспользоватьРасчетПервойПоловиныМесяца);
	ПараметрыРасчета.Вставить("ПерваяПоловинаМесяца",                   Объект.ПерваяПоловинаМесяца);
	ПараметрыРасчета.Вставить("ЕдинаяДатаВыплатаДокумента",             ЕдинаяДатаВыплатаДокумента);
...
&НаКлиенте
Процедура ПересчитатьНДФЛИВзносыСотрудников(НДФЛ = Истина, Взносы = Истина)
	
	ПараметрыРасчета = Новый Структура("НДФЛ, Взносы", Ложь, Ложь);
	Если НДФЛ Тогда
		ТабличнаяЧасть       = Объект.НДФЛ;
		КорректироватьРасчет = Объект.КорректироватьНДФЛ;
		ПараметрыРасчета.Вставить("НДФЛ", Истина);
	ИначеЕсли Взносы Тогда
		ТабличнаяЧасть       = Объект.Взносы;
		КорректироватьРасчет = Объект.КорректироватьВзносы;
		ПараметрыРасчета.Вставить("Взносы", Истина);
	Иначе
		Возврат;
	КонецЕсли;
	
	ФиксРасчет = ТабличнаяЧасть.НайтиСтроки(Новый Структура("ФиксРасчет", Истина));
	Если ФиксРасчет.Количество() <> 0
		ИЛИ КорректироватьРасчет Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ВопросПередПересчетомЗавершение", ЭтотОбъект, ПараметрыРасчета);
		ТекстВопроса = СтрШаблон(НСтр("ru='Все корректировки расчета %1 будут отменены.
				|Продолжить?'"),
			?(НДФЛ, НСтр("ru='НДФЛ'"), НСтр("ru='взносов'")));
...
Процедура ПересчитатьДокументНаСервере(ПараметрыРасчета)
	...
	Если ПараметрыРасчета.НДФЛ Тогда
		Объект.КорректироватьНДФЛ   = Ложь;
		Объект.НДФЛ.Очистить();
		ОбновитьРасчетыСотрудников(Новый Массив, МассивСотрудников, Новый Массив)
	Иначе
		Объект.КорректироватьВзносы = Ложь;
		Объект.Взносы.Очистить();
		ОбновитьРасчетыСотрудников(Новый Массив, Новый Массив, МассивСотрудников)
	КонецЕсли;
...
Процедура СформироватьДокументыВыплаты()
	...
	Если МожноСоздатьВедомостиПоРасчетномуДокументу(Объект.Ссылка) Тогда
		...
		ПараметрыФормы.Вставить("РежимВыплатыЗаПериод",    Не Объект.ПерваяПоловинаМесяца);
		ПараметрыФормы.Вставить("РежимВыплатыПоДокументу", Объект.ПерваяПоловинаМесяца);
		ПараметрыФормы.Вставить("ПериодСобытия",           Объект.МесяцНачисления);
		ПараметрыФормы.Вставить("ДатаВыплаты",             ?(ЕдинаяДатаВыплатаДокумента, ПланируемаяДатаВыплаты, ТекущаяДата()));
		ПараметрыФормы.Вставить("Аванс",                   Объект.ПерваяПоловинаМесяца);
