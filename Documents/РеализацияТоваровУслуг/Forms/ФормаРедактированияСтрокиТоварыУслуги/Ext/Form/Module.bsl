...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, Параметры,
		...
		|СчетАкциза, СуммаАкциза, ПодакцизныйТовар, СубконтоАкцизы");
	...
	Элементы.СчетАкциза.Видимость     = ПодакцизныйТовар;
	Элементы.СубконтоАкцизы.Видимость = ПодакцизныйТовар;
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПеренестиВДокумент Тогда
		СтруктураРезультат = Новый Структура("СчетУчета,
			...
			|Субконто, СчетАкциза, СубконтоАкцизы, ИмяТаблицы");
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СчетАкциза");
	...
	ГруппаОтбора1 = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(ЭлементУО.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
			"СуммаАкциза", ВидСравненияКомпоновкиДанных.Равно, 0);
		
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
			"ПодакцизныйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если ПодакцизныйТовар И СуммаАкциза <> 0 И НЕ СчетУчетаЗабалансовый Тогда
		Если НЕ ЗначениеЗаполнено(СчетАкциза) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Заполнение", НСтр("ru = 'Счет акциза'"));
			Поле = "СчетАкциза";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
		КонецЕсли;
	КонецЕсли;
