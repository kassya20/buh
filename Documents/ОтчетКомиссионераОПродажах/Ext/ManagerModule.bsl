...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьСубкомиссионераИСчетФактуруСубкомиссионераВРегистреРеализованныеТоварыКомитентов");
	Методы.Вставить("ЗаполнитьПодразделениеЗатрат");
	Методы.Вставить("ЗаполнитьОтражениеВУСН");
	Методы.Вставить("ОбновитьДокументыПоКомиссии");
...
Процедура ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, ИмяТабличнойЧасти) Экспорт
	...
	ДанныеОбъекта = Новый Структура("Дата, Организация, ВидОперации, Реализация, ДокументБезНДС");
...
Функция ДокументСозданПоПравиламПостановления914(ОтчетКомиссионера) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетКомиссионера)
		И ОтчетКомиссионера.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
		Возврат Истина;
	КонецЕсли;
...
// Определяет статью затрат, использованную в самом свежем документе.
// 
// Возвращаемое значение:
...
Функция ОсновнаяСтатьяЗатрат() Экспорт
	...
	|				ТОГДА ОтчетКомиссионераОПродажах.Субконто1
	|			КОГДА ВидСубконтоСтатьяЗатрат.НомерСтроки = 2
	|				ТОГДА ОтчетКомиссионераОПродажах.Субконто2
	|			КОГДА ВидСубконтоСтатьяЗатрат.НомерСтроки = 3
	|				ТОГДА ОтчетКомиссионераОПродажах.Субконто3
	...
	|		ПО ОтчетКомиссионераОПродажах.СчетУчетаЗатрат = ВидСубконтоСтатьяЗатрат.Ссылка
	|ГДЕ
...
// Функция возвращает текст запроса для печати счетов-фактур
// Параметры: 
//      НомераТаблиц - Структура добавляемых таблиц
//
// Возвращаемое значение:
// Текст - текст запроса
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ОтчетКомиссионераОПродажахВозвраты.Покупатель КАК Контрагент,
	|	ВЫБОР
	...
	|	ОтчетКомиссионераОПродажахВозвраты.ДатаСФ КАК ДатаСФКомиссионера,
	|	ОтчетКомиссионераОПродажахВозвраты.НомерСФ КАК НомерСФКомиссионера,
	|	Реквизиты.Контрагент КАК Комиссионер
	|ПОМЕСТИТЬ ВременнаяТаблицаРеквизиты
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах КАК Реквизиты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.Возвраты КАК ОтчетКомиссионераОПродажахВозвраты
	|		ПО Реквизиты.Ссылка = ОтчетКомиссионераОПродажахВозвраты.Ссылка
	...
	|	И ОтчетКомиссионераОПродажахВозвраты.СчетФактура = &СчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Реквизиты.ДатаСФКомиссионера КАК ДатаСФКомиссионера,
	|	Реквизиты.НомерСФКомиссионера КАК НомерСФКомиссионера,
	...
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка) КАК ТоварыКодТНВЭД,
	...
	|	ТаблицаНоменклатуры.ДокументРеализации КАК ДокументРеализации,
	|	ТаблицаНоменклатуры.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар
	|ПОМЕСТИТЬ ТаблицаНоменклатуры
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.Возвраты КАК Возвраты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ТаблицаНоменклатуры
	|		ПО Возвраты.КлючСтроки = ТаблицаНоменклатуры.КлючСтроки
	...
	|	И Возвраты.СчетФактура = &СчетФактура
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.РНПТ.Код КАК НомерТовара,
	...
	|		ОтчетКомиссионераОПродажахТоварыВозвращенные.Цена КАК ЦенаВВалютеДокумента,
	|		ОтчетКомиссионераОПродажахТоварыВозвращенные.СтавкаНДС КАК СтавкаНДС,
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Количество КАК КоличествоУчетноеВозвращено,
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоТовараВозвращено,
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Сумма КАК СуммаПослеИзменения,
	...
	|			ПО ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Ссылка = ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка
	|				И ОтчетКомиссионераОПродажахСведенияПрослеживаемости.ИдентификаторСтроки = ОтчетКомиссионераОПродажахТоварыВозвращенные.ИдентификаторСтроки
	|	ГДЕ
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Ссылка В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	
	|	ВЫБРАТЬ
	|		ОтчетКомиссионераОПродажахСведенияПрослеживаемости.РНПТ.Код,
	...
	|	ИЗ
	|		Документ.ОтчетКомиссионераОПродажах.СведенияПрослеживаемости КАК ОтчетКомиссионераОПродажахСведенияПрослеживаемости
	|			ПОЛНОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.Товары КАК ОтчетКомиссионераОПродажахТовары
...
Функция СоздатьДокументИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("Ссылка")
		...
		ЭлектронноеВзаимодействиеБП.ПроверитьЗапретИзмененияДанных(НовыйДокумент.Дата, НовыйДокумент.Организация);
		НовыйДокумент.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		НовыйДокумент.Товары.Очистить();
		НовыйДокумент.ТоварыВозвращенные.Очистить();
		НовыйДокумент.СуммаДокумента = 0;
		
	Иначе
		НовыйДокумент = Документы.ОтчетКомиссионераОПродажах.СоздатьДокумент();
		НовыйДокумент.Заполнить(Неопределено);
		НовыйДокумент.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах;
		Новыйдокумент.СуммаВключаетНДС = Истина;
		Новыйдокумент.СпособРасчетаКомиссионногоВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается;
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("Дата") Тогда
		...
		Если ДанныеФайла.Свойство("Комментарий") Тогда
			НовыйДокумент.Комментарий = ДанныеФайла.Комментарий;
		ИначеЕсли ДанныеФайла.ДанныеДокумента.Свойство("ДатаНач") Тогда
			НовыйДокумент.Комментарий = СтрШаблон(
				НСтр("ru = 'Отчет о реализации товаров с %1 по %2'"), 
					Формат(ДанныеФайла.ДанныеДокумента.ДатаНач, "ДФ=""дд.ММ.гггг"""), 
					Формат(ДанныеФайла.ДанныеДокумента.Дата, "ДФ=""дд.ММ.гггг"""));
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("Получатель") Тогда
		Если ДанныеФайла.ДанныеДокумента.Получатель.Свойство("Ссылка") Тогда
			НовыйДокумент.Организация = ДанныеФайла.ДанныеДокумента.Получатель.Ссылка;
		Иначе
			ИНН = "";
			КПП = "";
			...
			КонецЕсли;
			НовыйДокумент.Организация = СсылкаОрганизация;
		КонецЕсли;
	КонецЕсли;
	...
	ИсключатьВыкупы = ИнтеграцияWildberries.ИсключитьВыкупы(НовыйДокумент.Организация, НовыйДокумент.Дата);
	
	НезаполненнаяНоменклатура = Новый Массив;
	
	ЗаполнитьСчетаУчетаРасчетов(, Ложь, НовыйДокумент);
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("ТаблицаТоваров") Тогда
		...
		ДатыРеализацииВозвратов   = Новый Массив;
		СопоставлениеНоменклатуры = Новый Соответствие;
		
		ТаблицаТоваров = ДанныеФайла.ДанныеДокумента.ТаблицаТоваров;
		
		ЕстьНатуральныйИД          = НЕ ТаблицаТоваров.Колонки.Найти("НатуральныйИД") = Неопределено;
		ЕстьИсторияИдентификаторов = НЕ ТаблицаТоваров.Колонки.Найти("ИсторияИдентификаторов") = Неопределено;
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			//Исключим продажи Wildberries в ЕАЭС для плательщиков НДС,
			//т.к. они не являются комиссионной продажей - маркетплейс их выкупает
			Если ВидДокумента = "Отчет_WB_детальный" Тогда
				Если ИсключатьВыкупы И ИнтеграцияWildberries.ПродажаЧерезВыкупТовара(СтрокаТовара.КодСтраны, СтрокаТовара.НазваниеСтраны) Тогда
					ПропуститьЗагрузку = Истина;
				ИначеЕсли Нрег(СтрокаТовара.ВидДвижения) = "возврат" //OZON
					Или ИнтеграцияWildberries.ВозвратТовараМаркетплейсом(СтрокаТовара.ВидДвижения, СтрокаТовара.ТипДокумента) Тогда
					
					Если ЗначениеЗаполнено(СтрокаТовара.ДатаПродажи)
						И Макс(НачалоНедели(НовыйДокумент.Дата), НачалоГода(НовыйДокумент.Дата)) <> 
							Макс(НачалоНедели(СтрокаТовара.ДатаПродажи), НачалоГода(СтрокаТовара.ДатаПродажи)) Тогда
						ДатыРеализацииВозвратов.Добавить(СтрокаТовара.ДатаПродажи);
					КонецЕсли;					
				КонецЕсли;
			...
			Иначе
				...
				Если ЕстьИсторияИдентификаторов Тогда
					СтруктураПоискаНоменклатуры.ИсторияИдентификаторов = СтрокаТовара.ИсторияИдентификаторов;
				ИначеЕсли ЕстьНатуральныйИД И ЗначениеЗаполнено(СтрокаТовара.НатуральныйИД) Тогда
		...
		Для Каждого СтрокиКУдалению из СтрокиКУдалению Цикл
			ТаблицаТоваров.Удалить(СтрокиКУдалению);
		КонецЦикла;
		
		ДокументыРеализацииВозвратов  = НайтиДокументыРеализацииВозвратов(ДатыРеализацииВозвратов, НовыйДокумент.Организация, НовыйДокумент.Контрагент, ВидДокумента);
		...
		УчитыватьДоплату = ЭлектронноеВзаимодействиеБП.УчитыватьДоплатуЗаСчетOzon(НовыйДокумент.Дата);
		
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			
			Номенклатура = СопоставлениеНоменклатуры.Получить(СтрокаТовара.ИД);
			
			Если ЗначениеЗаполнено(СтрокаТовара.ВидДвижения) Тогда
				
				Если Нрег(СтрокаТовара.ВидДвижения) = "продажа" //OZON
					Или ИнтеграцияWildberries.ПродажаТовараМаркетплейсом(СтрокаТовара.ВидДвижения, СтрокаТовара.ТипДокумента) Тогда
					
					Если СтрокаТовара.Сумма = 0 
				...
				ИначеЕсли Нрег(СтрокаТовара.ВидДвижения) = "возврат" //OZON
					...
					СтрокаДокумента = НовыйДокумент.ТоварыВозвращенные.Добавить();
					СтрокаДокумента.Номенклатура = Номенклатура;
					СтрокаДокумента.Количество   = СтрокаТовара.Количество;
					СтрокаДокумента.Сумма        = СтрокаТовара.Сумма;
					...
					Если Не ЗначениеЗаполнено(Номенклатура) И ЭтоАвтозагрузка Тогда
						ОбменСМаркетплейс.ДобавитьНезаполненнуюНоменклатуру(
							НезаполненнаяНоменклатура, СтрокаТовара, "ТоварыВозвращенные", НовыйДокумент.ТоварыВозвращенные.Индекс(СтрокаДокумента));
					КонецЕсли;
					
					Если ВидДокумента = "Отчет_WB_детальный"
						И ЗначениеЗаполнено(СтрокаТовара.ДатаПродажи) Тогда
						
						ДокументРеализации = ДокументыРеализацииВозвратов.Получить(СтрокаТовара.ДатаПродажи);
			...
			Иначе
				...
				Если СтрокаТовара.Количество > 0 Тогда
					...
					СтрокаДокумента.Номенклатура = Номенклатура;
					СтрокаДокумента.Количество   = СтрокаТовара.Количество;
					СтрокаДокумента.Сумма        = ?(УчитыватьДоплату, СтрокаТовара.Сумма + СтрокаТовара.Доплата, СтрокаТовара.Сумма); 
					СтрокаДокумента.Сумма        = СтрокаДокумента.Сумма + СтрокаТовара.ДоплатаПартнеров;
				...
				Если СтрокаТовара.КоличествоВозврат > 0 Тогда
					
					СтрокаДокумента = НовыйДокумент.ТоварыВозвращенные.Добавить();
					СтрокаДокумента.Номенклатура = Номенклатура;
					СтрокаДокумента.Количество   = СтрокаТовара.КоличествоВозврат;
					СтрокаДокумента.Сумма        = ?(УчитыватьДоплату, СтрокаТовара.СуммаВозврат + СтрокаТовара.ДоплатаВозврат, СтрокаТовара.СуммаВозврат);
					СтрокаДокумента.Сумма        = СтрокаДокумента.Сумма + СтрокаТовара.ДоплатаПартнеровВозврат;
		...
		ДанныеОбъекта = Новый Структура("Дата, Организация, Реализация, ВидОперации, ДокументБезНДС, ТипЦен, СуммаВключаетНДС, ВедетсяУчетПрослеживаемыхТоваров");
		...
		Если НовыйДокумент.ТоварыВозвращенные.Количество() > 0 Тогда
			ЗаполнитьСтрокиТЧЗагрузкаИзФайла(НовыйДокумент.ТоварыВозвращенные, "ТоварыВозвращенные", ДанныеОбъекта);
		КонецЕсли;
	...
	ОбменСМаркетплейс.СохранитьНесопоставленнуюНоменклатуру(
		НезаполненнаяНоменклатура, НовыйДокумент.Ссылка, НовыйДокумент.Контрагент, ДанныеФайла);
...
Процедура ЗаполнитьСтрокиТЧЗагрузкаИзФайла(ТаблицаТовары, ИмяТаблицы, ДанныеОбъекта)
	...
	Для Каждого СтрокаТовара Из ТаблицаТовары Цикл
		...
		Если СведенияОНоменклатуре <> Неопределено Тогда
			...
			Если ДанныеОбъекта.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании
				ИЛИ ДанныеОбъекта.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
				СтрокаТовара.ОтражениеВУСН		 = Перечисления.ОтражениеВУСН.НеПринимаются;
			КонецЕсли;
		...
		Если ИмяТаблицы = "ТоварыВозвращенные"
			И ДанныеОбъекта.ВидОперации <> Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
			СтрокаТовара.ПоТекущемуДокументу = Не ЗначениеЗаполнено(СтрокаТовара.ДокументРеализации);
		КонецЕсли;
		
		СтрокаТовара.Цена = ?(СтрокаТовара.Количество = 0, СтрокаТовара.Сумма, Окр(СтрокаТовара.Сумма/СтрокаТовара.Количество, 2, 1));
		
		ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТовара, ИмяТаблицы, СведенияОНоменклатуре);
...
Функция НайтиДокументыРеализацииВозвратов(ДатыРеализацииВозвратов, Организация, Контрагент, ВидДокумента)
	
	ДокументыРеализацииВозвратов = Новый Соответствие;
	ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(ДатыРеализацииВозвратов);
	
	Контрагенты = Новый Массив;
	Контрагенты.Добавить(Контрагент);
	Если ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBДетальный()
		Или ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBОприходование() Тогда
		
		ДанныеКонтрагент = ЭлектронноеВзаимодействиеБП.ПолучитьРеквизитыWB('20240805');
		Если ДанныеКонтрагент.ИНН = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "ИНН") Тогда
	...
	Если ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonОприходование()
		Или ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетWBОприходование() Тогда
		УсловиеПоВидуОперации = "ОтчетКомиссионераОПродажах.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетОСписании)";
	Иначе
		УсловиеПоВидуОперации = "ОтчетКомиссионераОПродажах.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетОСписании)";
	КонецЕсли;
		
	ТаблицаДатРеализации = Новый ТаблицаЗначений;
	ТаблицаДатРеализации.Колонки.Добавить("ДатаПродажи", Новый ОписаниеТипов("Дата", Новый КвалификаторыДаты(ЧастиДаты.ДатаВремя)));
	Для Каждого ДатаРеализации Из ДатыРеализацииВозвратов Цикл
		НоваяСтрока = ТаблицаДатРеализации.Добавить();
		НоваяСтрока.ДатаПродажи = ДатаРеализации;
	КонецЦикла;
		
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументыРеализацииВозвратов", ТаблицаДатРеализации);
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Контрагенты", Контрагенты); 
	Запрос.УстановитьПараметр("ДнейДоОтчета", ДнейДоОтчета);
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ДатаРеализации.ДатаПродажи КАК ДатаРеализации,
	|	КОНЕЦПЕРИОДА(ДатаРеализации.ДатаПродажи, НЕДЕЛЯ) КАК КонецПериода
	|ПОМЕСТИТЬ ДатыРеализации
	|ИЗ
	|	&ДокументыРеализацииВозвратов КАК ДатаРеализации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетКомиссионераОПродажах.Ссылка КАК Ссылка,
	|	ДатыРеализации.ДатаРеализации КАК ДатаРеализации,
	|	ВЫБОР
	|		КОГДА ОтчетКомиссионераОПродажах.Дата ЕСТЬ NULL
	|			ТОГДА &ДнейДоОтчета
	|		ИНАЧЕ РАЗНОСТЬДАТ(ОтчетКомиссионераОПродажах.Дата, ДатыРеализации.ДатаРеализации, ДЕНЬ)
	|	КОНЕЦ КАК ДнейДоОтчета
	|ИЗ
	|	ДатыРеализации КАК ДатыРеализации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах КАК ОтчетКомиссионераОПродажах
	|		ПО ДатыРеализации.ДатаРеализации < ОтчетКомиссионераОПродажах.Дата
	|			И ДатыРеализации.КонецПериода >= ОтчетКомиссионераОПродажах.Дата
	|ГДЕ
	|	ОтчетКомиссионераОПродажах.Организация = &Организация
	|	И ОтчетКомиссионераОПродажах.Контрагент В (&Контрагенты)
	|	И &УсловиеПоВидуОперации
	...
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоВидуОперации", УсловиеПоВидуОперации);
	
	Запрос.Текст = ТекстЗапроса;
	Результат = Запрос.Выполнить();
	ВыборкаДатаРеализации = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаДатаРеализации.Следующий() Цикл
		ВыборкаДетальная = ВыборкаДатаРеализации.Выбрать();
		Если ВыборкаДетальная.Следующий() Тогда
			ДокументыРеализацииВозвратов.Вставить(ВыборкаДетальная.ДатаРеализации, ВыборкаДетальная.Ссылка);
		КонецЕсли;	
	КонецЦикла;
	
	Возврат ДокументыРеализацииВозвратов;
	
КонецФункции

Функция СоздатьДокументСписанияИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Иначе
		НовыйДокумент = Документы.ОтчетКомиссионераОПродажах.СоздатьДокумент();
		НовыйДокумент.Заполнить(Неопределено);
		НовыйДокумент.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании;
		НовыйДокумент.СтатьяПрочихДоходовИРасходов = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.СписаниеТоваровУКомиссионера");
		Новыйдокумент.СуммаВключаетНДС = Истина;
		Новыйдокумент.СпособРасчетаКомиссионногоВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается;
		Новыйдокумент.НаВозвратыВыставляетсяКорректировочныйСчетФактура = Истина;
	КонецЕсли;
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("ТаблицаТоваров") Тогда
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Если ВидДокумента = "Отчет_Ozon_списание"
				Или ИнтеграцияWildberries.СписаниеТовараМаркетплейсом(СтрокаТовара.ВидДвижения, СтрокаТовара.ТипДокумента) Тогда
...
// Определяет есть ли в документа ставка НДС по УСН
//
// Параметры:
...
//
// Возвращаемое значение:
// Булево - если в документе есть ставка НДС по УСН.
Функция НаличиеСтавкиНДСПоУСН(ТекущийДокумент, СтавкаНДСПоУСН) Экспорт
	...
	|	Документ.ОтчетКомиссионераОПродажах КАК ОтчетКомиссионераОПродажах
	|ГДЕ
	|	ОтчетКомиссионераОПродажах.Ссылка = &СсылкаНаДокумент
	|	И ОтчетКомиссионераОПродажах.СтавкаНДСВознаграждения В (&СтавкаНДСПоУСН)";
...
Функция СоздатьДокументОприходованияИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Иначе
		НовыйДокумент = Документы.ОтчетКомиссионераОПродажах.СоздатьДокумент();
		НовыйДокумент.Заполнить(Неопределено);
		НовыйДокумент.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании;
		НовыйДокумент.СтатьяПрочихДоходовИРасходов = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.СписаниеТоваровУКомиссионера");
		Новыйдокумент.СуммаВключаетНДС = Истина;
		Новыйдокумент.СпособРасчетаКомиссионногоВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается;
		Новыйдокумент.НаВозвратыВыставляетсяКорректировочныйСчетФактура = Истина;
	...
	Если ДанныеФайла.ДанныеДокумента.Свойство("ТаблицаТоваров") Тогда
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаТовара.ДатаПродажи)
					И НачалоМесяца(НовыйДокумент.Дата) <> НачалоМесяца(СтрокаТовара.ДатаПродажи) Тогда
					ДатыРеализацииВозвратов.Добавить(СтрокаТовара.ДатаПродажи);
				КонецЕсли;
		...
		Если ТаблицаТоваров.Количество() = 0 Тогда
			ВызватьИсключение НСтр("ru = 'В строках отчета нет данных о декомпенсации'");
		КонецЕсли;
		...
		Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
			...
			Если ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonОприходование()
				Или ИнтеграцияWildberries.ОприходованиеТовараМаркетплейсом(СтрокаТовара.ВидДвижения, СтрокаТовара.ТипДокумента) Тогда
					
				СтрокаДокумента = НовыйДокумент.ТоварыВозвращенные.Добавить();
				СтрокаДокумента.Номенклатура = Номенклатура;
				
				Если ВидДокумента = ЭлектронноеВзаимодействиеБП.ВидДокументаОтчетOzonОприходование() Тогда
					СтрокаДокумента.Количество   = СтрокаТовара.КоличествоВозврат;
					СтрокаДокумента.Сумма        = СтрокаТовара.СуммаВозврат;
				Иначе
					СтрокаДокумента.Количество   = СтрокаТовара.Количество;
					СтрокаДокумента.Сумма        = СтрокаТовара.Сумма;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(СтрокаТовара.ДатаПродажи) Тогда
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ВидОперации) Тогда
		...
		ВидОперации = ВидОперацииПоФункциональнымОпциям();
		
	КонецЕсли;

	СтандартнаяОбработка = Ложь;
	ФормыОтчетаКомиссионера = ПолучитьСоответствиеВидовОперацийФормам();
	ВыбраннаяФорма = ФормыОтчетаКомиссионера[ВидОперации];
...
Функция ПодготовитьТаблицуВзаиморасчетыВознаграждение(ТаблицаВзаиморасчетов, ЗачетАвансовРеквизиты, Отказ) Экспорт
	ТаблицаВзаиморасчетыКомиссия = УчетВзаиморасчетов.ПустаяТаблицаВзаиморасчетовЗачетАвансов();
	Если НЕ ЗначениеЗаполнено(ЗачетАвансовРеквизиты) 
		ИЛИ (ЗачетАвансовРеквизиты[0].СпособЗачетаАвансов = Перечисления.СпособыЗачетаАвансов.НеЗачитывать) Тогда
	...
	Для каждого СтрокаВзаиморасчеты Из ТаблицаВзаиморасчетов Цикл
		Если СтрокаВзаиморасчеты.СчетАвансов = Реквизиты.СчетРасчетов 
...
Функция ТекстЗапросаЗачетЗадолженностиВознаграждение(НомераТаблиц, ПараметрыПроведения, Реквизиты)

	Если НЕ ЗначениеЗаполнено(Реквизиты.СчетУчетаРасчетовЗаПосредническиеУслуги) Тогда
		
		ПараметрыПроведения.Вставить("ЗачетЗадолженностиВознаграждениеРеквизиты", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ЗачетЗадолженностиВознаграждениеРеквизиты",        НомераТаблиц.Количество());
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ЭтоОприходование", Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании);
	...
	Реквизиты.Вставить("ПрименяетсяПСН",
	...
	ТаблицаРеквизиты[0].УчитыватьЗадолженностьУСНПатент = Реквизиты.ПрименяетсяПСН;
	ПараметрыПроведения.Вставить("Реквизиты", ТаблицаРеквизиты);
	
	Запрос.УстановитьПараметр("ПрименяетсяУСН",        Реквизиты.ПрименяетсяУСН);
	Запрос.УстановитьПараметр("ПрименяетсяПСН",        Реквизиты.ПрименяетсяПСН);
	...
	Реквизиты.Вставить("ЭтоСписание", Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании);
	Реквизиты.Вставить("ЭтоОприходование", Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании);
	Запрос.УстановитьПараметр("СпособУчетаНДС", Реквизиты.СпособУчетаНДС);
	
	Реквизиты.Вставить("ОтражатьДоходыУСННаДатуОтчета",
		НалоговыйУчетУСН.ОтражатьДоходыНаДатуОтчетаКомиссионера(Реквизиты.Организация, Реквизиты.Период));
	...
	Запрос.УстановитьПараметр("ЭтоСписание", Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании);
	Запрос.УстановитьПараметр("ЭтоОприходование", Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании);
	...
	Запрос.Текст = ТекстЗапросаСписаниеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаЗачетЗадолженностиВознаграждение(НомераТаблиц, ПараметрыПроведения, Реквизиты)
...
Функция ТекстЗапросаВозвратыТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.ПодразделениеОрганизации КАК Подразделение,
	|	ВЫБОР 
	|		КОГДА &ЭтоОприходование 
	|			ТОГДА ""Оприходование излишков товаров""
	...
	|	ТаблицаВозвраты.ПоТекущемуДокументу КАК ПоТекущемуДокументу,
	...
	|	КОНЕЦ КАК Партия,
	|	ВЫБОР
	|		КОГДА ТаблицаВозвраты.ПоТекущемуДокументу
	|			ТОГДА Реквизиты.Ссылка
	|		ИНАЧЕ ТаблицаВозвраты.Сделка
	|	КОНЕЦ КАК Сделка,
	...
	|		ИНАЧЕ ТаблицаВозвраты.СчетРасходов 
	|	КОНЕЦ КАК КорСчетСписания,
	|	ВЫБОР 
	|		КОГДА &ЭтоОприходование 
	|			ТОГДА Реквизиты.СтатьяПрочихДоходовИРасходов
	|		ИНАЧЕ ТаблицаВозвраты.Субконто 
	...
	|	ЕСТЬNULL(ТаблицаПокупатели.ВыставленСФ, Ложь) КАК ВыставленСФ,
	|	ВЫБОР
	...
	|		ИНАЧЕ ТаблицаВозвраты.СчетДоходов 
	...
	|		ИНАЧЕ ТаблицаВозвраты.СтавкаНДС 
	|	КОНЕЦ КАК СтавкаНДС,
	...
	|	ВЫБОР
	|		КОГДА НЕ &ЗаполнятьДокументРасчетов
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ТаблицаПокупатели.ВыставленСФ
	|			ТОГДА ТаблицаПокупатели.СчетФактура
	|		ИНАЧЕ Реквизиты.Ссылка
...
Функция ТекстЗапросаЗачетАвансовНаВозврат(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	СУММА(ВЫБОР
	|			КОГДА НЕ ТаблицаВозвраты.ДоходЕНВД
	...
	|	ЕСТЬNULL(ТаблицаВозвраты.Сделка, Неопределено) КАК Сделка,
	|	Реквизиты.Контрагент КАК Контрагент,
	|	Реквизиты.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	Реквизиты.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	...
	|ИЗ
	|	Реквизиты КАК Реквизиты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаВозвратыСделки КАК ТаблицаВозвраты
	|		ПО (ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ТаблицаСуммВзаиморасчетов.СуммаВзаиморасчетов ЕСТЬ НЕ NULL" + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаНДСВозвраты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ТекстЗапроса = ТекстЗапроса
		...
		|	ТаблицаТовары.НомерСтроки КАК НомерСтроки,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТаблицаВозвраты.ВыставленСФ, ЛОЖЬ)
		...
		|	ВЫБОР
		|		КОГДА ТаблицаВозвраты.ВыставленСФ ЕСТЬ NULL
		|			ТОГДА ВЫБОР
		|					КОГДА ТаблицаТовары.ПоТекущемуДокументу
		|						ТОГДА ТаблицаТовары.Ссылка
		|					КОГДА НЕ ТаблицаТовары.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионераОПродажах.ПустаяСсылка)
		...
		|		КОГДА НЕ ТаблицаВозвраты.ВыставленСФ
		|				И Реквизиты.НаВозвратыВыставляетсяКорректировочныйСчетФактура
		|				И НЕ ТаблицаТовары.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионераОПродажах.ПустаяСсылка)
		|			ТОГДА ТаблицаТовары.ДокументРеализации
		|		КОГДА Реквизиты.НаВозвратыВыставляетсяКорректировочныйСчетФактура
		|			ТОГДА ТаблицаТовары.Ссылка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.СчетФактураПолученный.ПустаяСсылка)
		|	КОНЕЦ КАК СчетФактура,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТаблицаВозвраты.ВыставленСФ, ЛОЖЬ)
		|			ТОГДА ТаблицаВозвраты.Покупатель
		...
		|		КОГДА НЕ ТаблицаВозвраты.ВыставленСФ
		|				И Реквизиты.НаВозвратыВыставляетсяКорректировочныйСчетФактура
		|				И ТаблицаТовары.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионераОПродажах.ПустаяСсылка)
		|				И ТаблицаТовары.ПоТекущемуДокументу
		|			ТОГДА ""17""
		|		ИНАЧЕ """"
...
Функция ТекстЗапросаРасходыУСНВозвраты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	КонецЕсли;
		...
		|	0 КАК НДС,
		|	ТаблицаВозвраты.СпособУчетаНДС КАК СпособУчетаНДС,
		|	ТаблицаВозвраты.ОтражениеВУСН КАК ОтражениеВУСН
		|ИЗ
		|	ТаблицаВозвраты КАК ТаблицаВозвраты
		|ГДЕ
		|	НЕ ТаблицаВозвраты.УказанДокументОтгрузки
		|	И НЕ ТаблицаВозвраты.ПоТекущемуДокументу
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|		ЛОЖЬ
	|	ИЗ
	|		Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ТаблицаДокумента
	|	ГДЕ
	|		ТаблицаДокумента.Ссылка = &Ссылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|	Реквизиты.НаВозвратыВыставляетсяКорректировочныйСчетФактура КАК НаВозвратыВыставляетсяКорректировочныйСчетФактура,
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьВозвраты Тогда
		...
		|	ТаблицаВозвращенныеТовары.Себестоимость КАК Себестоимость,
		|	ТаблицаВозвращенныеТовары.ДокументРеализации КАК Сделка,
		|	ВЫБОР
		|		КОГДА ТаблицаВозвращенныеТовары.ДокументРеализации = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионераОПродажах.ПустаяСсылка)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		...
		|	ТаблицаВозвращенныеТовары.ОтражениеВУСН КАК ОтражениеВУСН,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТаблицаВозвраты.ВыставленСФ, ЛОЖЬ)
		|			ТОГДА ТаблицаВозвращенныеТовары.СпособУчетаНДС
		|		КОГДА &ЭтоОприходование
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.Распределен)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПринимаетсяКВычету)
		|	КОНЕЦ КАК СпособУчетаНДС,
		|	ВЫБОР
		|		КОГДА ТаблицаВозвращенныеТовары.ПоТекущемуДокументу
		|			ТОГДА ДанныеПервичныхДокументовСсылка.ДатаРегистратора
		|		ИНАЧЕ ДанныеПервичныхДокументов.ДатаРегистратора
		|	КОНЕЦ КАК ДатаРеализации,
		|	ТаблицаВозвращенныеТовары.ПоТекущемуДокументу КАК ПоТекущемуДокументу
		...
		|		ПО (ДанныеПервичныхДокументов.Документ = ТаблицаВозвращенныеТовары.ДокументРеализации)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументовСсылка
		|		ПО ТаблицаВозвращенныеТовары.Ссылка = ДанныеПервичныхДокументовСсылка.Документ
		|ГДЕ
...
Функция ТекстЗапросаРасчетыВВалютеВозвраты()
	...
	|	ВременнаяТаблицаВозвраты.СпособУчетаНДС,
	|	ВременнаяТаблицаВозвраты.ДатаРеализации,
	|	ВременнаяТаблицаВозвраты.ПоТекущемуДокументу
	|ПОМЕСТИТЬ ТаблицаВозвраты
	|ИЗ
	|	ВременнаяТаблицаВозвраты КАК ВременнаяТаблицаВозвраты
...
Функция ТекстЗапросаСписаниеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаПокупатели.ДатаСФ КАК ДатаСФ,
	|	ТаблицаПокупатели.СчетФактура КАК СчетФактура,
	|	ЛОЖЬ КАК ВозвратПоТекущемуДокументу,
	|	ЛОЖЬ КАК РассчитатьСебестоимостьПоТекущимПродажам,
	|	НЕОПРЕДЕЛЕНО КАК КорСчетСписанияВозврат
	|ИЗ
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.ВалютаДокумента КАК ВалютаДокумента,
	|	Реквизиты.СпособЗачетаАвансов КАК СпособЗачетаАвансов,
	|	Реквизиты.СчетУчетаРасчетовЗаПосредническиеУслуги КАК СчетРасчетовСКомиссионером,
	|	&ПрименяетсяУСН КАК УчитыватьЗадолженностьУСН,
	|	&ПрименяетсяПСН КАК УчитыватьЗадолженностьУСНПатент,
	|	""Выбытие"" КАК НаправлениеДвижения,
	|	ЛОЖЬ КАК ЭтоВозврат
...
Функция ТекстЗапросаРеализация(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.ЕстьТовары И НЕ Реквизиты.ЕстьУслуги Тогда
		...
		ПараметрыПроведения.Вставить("КомпенсацияТаблицаДокумента", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("РеализацияРеквизиты",        НомераТаблиц.Количество());
	Если Реквизиты.ЭтоСписание Тогда
		ПараметрыПроведения.Вставить("РеализацияТаблицаДокумента", Неопределено);
		НомераТаблиц.Вставить("КомпенсацияТаблицаДокумента", НомераТаблиц.Количество());
	Иначе
	...
	|	Реквизиты.Ссылка КАК Регистратор,
	|	Реквизиты.Дата КАК Период,
	|	ВЫБОР КОГДА &ЭтоСписание 
	|		ТОГДА ""Списание""
	|	ИНАЧЕ ""Реализация"" КОНЕЦ КАК ТипСписания,
	...
	|	Реквизиты.ВидОперации КАК ВидОперацииДокумента,
	|	Реквизиты.РасчетыВУсловныхЕдиницах,
	|	Реквизиты.ДоговорКонтрагента,
	|	Реквизиты.Контрагент,
	...
	|	ВЫБОР КОГДА &ЭтоСписание 
	|		ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеДоходы)
	|	ИНАЧЕ ТаблицаТовары.СчетДоходов КОНЕЦ КАК СчетДоходов,
	|	ВЫБОР КОГДА &ЭтоСписание 
	|		ТОГДА Реквизиты.СтатьяПрочихДоходовИРасходов
	|	ИНАЧЕ ТаблицаТовары.Субконто КОНЕЦ КАК Субконто,
	|	ВЫБОР КОГДА &ЭтоСписание 
	|		ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|	ИНАЧЕ ТаблицаТовары.СтавкаНДС КОНЕЦ КАК СтавкаНДС,
	|	ТаблицаТовары.СчетУчетаНДСПоРеализации КАК СчетУчетаНДСПоРеализации,
...
Функция ТекстЗапросаТоварыНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.ЕстьТовары Тогда
		...
		ПараметрыПроведения.Вставить("НДСТоварыСписание", Неопределено);
		Возврат "";
	КонецЕсли;
	
	Если Реквизиты.ЭтоСписание Тогда
		ПараметрыПроведения.Вставить("НДСТовары", Неопределено);
		НомераТаблиц.Вставить("НДСТоварыСписание", НомераТаблиц.Количество());
	Иначе
...
// Зачет вознаграждения отраженного этим документом на авансы выданные отдельным документом
...
Функция ТекстЗапросаПогашениеЗадолженностиВознаграждение(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ВЫБРАТЬ
	|	Реквизиты.СчетУчетаРасчетовЗаПосредническиеУслуги КАК СчетРасчетов,
	|	Реквизиты.СчетУчетаРасчетовЗаПосредническиеУслуги КАК СчетАвансов_Отгрузка,
	|	Реквизиты.Контрагент,
	|	Реквизиты.ДоговорКонтрагента,
	|	Реквизиты.ВалютаВзаиморасчетов,
	|	Реквизиты.РасчетыВУсловныхЕдиницах,
...
Функция ТекстЗапросаТаблицаДенежныеСредства(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ПлательщикНДС
		Или ((Реквизиты.ПрименяетсяУСН Или Реквизиты.ПрименяетсяПСН) И Реквизиты.ОтражатьДоходыУСННаДатуОтчета) Тогда
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА (ОтчетКомиссионераОПродажахПокупатели.СчетФактура = ЗНАЧЕНИЕ(Документ.СчетФактураВыданный.ПустаяСсылка)
	|			ИЛИ ОтчетКомиссионераОПродажахПокупатели.СчетФактура ЕСТЬ NULL)
	|			ТОГДА ОтчетКомиссионераОПродажах.Ссылка
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ОтчетКомиссионераОПродажахТовары.СчетУчета.Забалансовый
	...
	|	НАЧАЛОПЕРИОДА(ОтчетКомиссионераОПродажах.Дата, КВАРТАЛ) КАК ПериодОперации,
	|	ВЫБОР
	|		КОГДА &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПустаяСсылка)
	|		КОГДА &ЭтоСписание
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.НедостачаВыявленнаяВХодеИнвентаризации)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ОтчетКомиссионераОПродажахПокупатели.Покупатель ЕСТЬ NULL
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОтчетКомиссионераОПродажахТовары.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И ОтчетКомиссионераОПродажахТовары.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	...
	|	НАЧАЛОПЕРИОДА(ОтчетКомиссионераОПродажах.Дата, КВАРТАЛ),
	|	ВЫБОР
	|		КОГДА &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПустаяСсылка)
	|		КОГДА &ЭтоСписание
	...
	|		ПО ОтчетКомиссионераОПродажахТовары.ИдентификаторСтроки = ТаблицаТовары.ИдентификаторСтроки
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТовары.Ссылка = &Ссылка
...
Функция ТекстЗапросаПрослеживаемыеТоварыВозврат(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не Реквизиты.ВедетсяУчетПрослеживаемыхТоваров Или НЕ Реквизиты.ЕстьВозвраты Тогда
		ПараметрыПроведения.Вставить("ВТ_ПрослеживаемостьВозврат",    Неопределено);
		ПараметрыПроведения.Вставить("ВТ_СуммыБезНДСВозврат",         Неопределено);
	...
	НомераТаблиц.Вставить("ВТ_СуммыБезНДСВозврат",         НомераТаблиц.Количество());
	...
	|	СУММА(ОтчетКомиссионераОПродажахСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ВЫБОР
	|		КОГДА ОтчетКомиссионераОПродажахВозвраты.СчетФактура = НЕОПРЕДЕЛЕНО
	|				ИЛИ ОтчетКомиссионераОПродажахВозвраты.СчетФактура ЕСТЬ NULL
	|			ТОГДА ОтчетКомиссионераОПродажах.Ссылка
	|		ИНАЧЕ ОтчетКомиссионераОПродажахВозвраты.СчетФактура
	|	КОНЕЦ КАК ДокументОперации,
	|	ОтчетКомиссионераОПродажах.Контрагент КАК Комиссионер,
	|	ВЫБОР
	|		КОГДА ОтчетКомиссионераОПродажахТоварыВозвращенные.ПоТекущемуДокументу
	|			ТОГДА ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка
	|		ИНАЧЕ ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации
	...
	|		КОГДА &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС
	|			ТОГДА ВЫБОР
	|					КОГДА ОтчетКомиссионераОПродажахТоварыВозвращенные.СчетУчета.Забалансовый
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокОтраженияВОтчетностиПоПрослеживаемости.ЖурналСФ)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПустаяСсылка)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ОтчетКомиссионераОПродажахВозвраты.Покупатель ЕСТЬ NULL
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ВозвратОтРозничногоПокупателя)
	|				КОГДА ОтчетКомиссионераОПродажахВозвраты.Покупатель.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
	|						И НЕ ОтчетКомиссионераОПродажахВозвраты.Покупатель.ИндивидуальныйПредприниматель
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ВозвратОтРозничногоПокупателя)
	|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПолучениеКомитентомСведенийОВозвратеТовараКомиссионеру)
	|			КОНЕЦ
	|	КОНЕЦ КАК КодОперации,
	|	ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Комитент КАК Комитент,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|	ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Сумма КАК СуммаБезНДС,
	|	ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Номенклатура КАК Комплектующие
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И (НЕ &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах КАК ОтчетКомиссионераОПродажах
	|		ПО ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка = ОтчетКомиссионераОПродажах.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.Возвраты КАК ОтчетКомиссионераОПродажахВозвраты
	...
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ПрослеживаемыйТовар
	|	И ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетКомиссионераОПродажахСведенияПрослеживаемости.РНПТ,
	|	ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Комитент,
	...
	|		ИНАЧЕ ОтчетКомиссионераОПродажахВозвраты.СчетФактура
	|	КОНЕЦ,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ИдентификаторСтроки,
	|	ВЫБОР
	|		КОГДА ОтчетКомиссионераОПродажахТоварыВозвращенные.ПрослеживаемыйКомплект
	|			ТОГДА ОтчетКомиссионераОПродажахСведенияПрослеживаемости.СтранаПроисхождения
	|		ИНАЧЕ ОтчетКомиссионераОПродажахТоварыВозвращенные.СтранаПроисхождения
	...
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ВТ_СуммыБезНДСВозврат
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ОтчетКомиссионераОПродажахТоварыВозвращенные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаВозвраты КАК ТаблицаВозвраты
	|		ПО ОтчетКомиссионераОПродажахТоварыВозвращенные.НомерСтроки = ТаблицаВозвраты.НомерСтроки
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка = &Ссылка
	|	И ОтчетКомиссионераОПродажахТоварыВозвращенные.ПрослеживаемыйТовар
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТ_ПрослеживаемостьВозврат.ПрослеживаемыйКомплект
	|			ТОГДА ВТ_ПрослеживаемостьВозврат.Комплектующие
	|		ИНАЧЕ ВТ_ПрослеживаемостьВозврат.Номенклатура
	|	КОНЕЦ КАК Номенклатура,
	|	ВТ_ПрослеживаемостьВозврат.Количество КАК Количество,
	|	ВТ_ПрослеживаемостьВозврат.РНПТ КАК РНПТ,
	|	ВТ_ПрослеживаемостьВозврат.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ВТ_ПрослеживаемостьВозврат.ДокументОперации КАК ДокументОперации,
	|	ВЫБОР
	|		КОГДА ВТ_ПрослеживаемостьВозврат.ПрослеживаемыйКомплект
	|			ТОГДА ВТ_ПрослеживаемостьВозврат.СуммаБезНДС
	|		ИНАЧЕ ЕСТЬNULL(ВТ_СуммыБезНДСВозврат.СуммаБезНДС, 0)
	|	КОНЕЦ КАК СуммаБезНДС,
	...
	|	ВТ_ПрослеживаемостьВозврат.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ИЗ
	|	ВТ_ПрослеживаемостьВозврат КАК ВТ_ПрослеживаемостьВозврат
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СуммыБезНДСВозврат КАК ВТ_СуммыБезНДСВозврат
	|		ПО ВТ_ПрослеживаемостьВозврат.ИдентификаторСтроки = ВТ_СуммыБезНДСВозврат.ИдентификаторСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ_ПрослеживаемостьВозврат.РНПТ КАК РНПТ,
	|	ВТ_ПрослеживаемостьВозврат.Комиссионер КАК Комиссионер,
	|	ВТ_ПрослеживаемостьВозврат.Комитент КАК Комитент,
	|	ВЫБОР
	|		КОГДА ВТ_ПрослеживаемостьВозврат.Комитент = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыЗапасовПрослеживаемыхТоваров.ТоварыОтКомитентаНаРеализацию)
	|	КОНЕЦ КАК ВидЗапасов,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации,
	|	ВТ_ПрослеживаемостьВозврат.ОснованиеДляВозврата КАК ОснованиеДляВозврата,
	|	ВТ_ПрослеживаемостьВозврат.Комплектующие КАК Комплектующие
	|ИЗ
	|	ВТ_ПрослеживаемостьВозврат КАК ВТ_ПрослеживаемостьВозврат
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВТ_ПрослеживаемостьВозврат.Комиссионер,
	|	ВТ_ПрослеживаемостьВозврат.Комитент,
	|	ВТ_ПрослеживаемостьВозврат.ОснованиеДляВозврата,
	|	ВТ_ПрослеживаемостьВозврат.Комплектующие";
...
Функция ПодготовитьТаблицуСписанныеТоварыОтПокупателя(ТаблицаВозвращенныеСписанныеТовары, СписаниеТоваровСчетаУчета, СписаниеТоваровНоменклатура,ТаблицаРеквизиты, Отказ) Экспорт
	...
	ОтборПоДокументуРеализации = Новый Структура("Сделка, ПоТекущемуДокументу");
	...
	Если ТаблицаРеквизиты <> Неопределено Тогда
		...
		Для каждого СтрокаРеквизиты Из ТаблицаРеквизиты Цикл
			// подбираем партии по каждому документу списания кроме возвратов по текущему документу
			Если СтрокаРеквизиты.ПоТекущемуДокументу Тогда
				Продолжить;
			КонецЕсли; 
			
			ЗаполнитьЗначенияСвойств(ОтборПоДокументуРеализации, СтрокаРеквизиты);
			...
			Если ТаблицаСписанныеТоварыОтПокупателяПоСделке = Неопределено Тогда
				Продолжить;
			КонецЕсли;	
...
Функция ПартииПоСделкамУСН(Реквизиты, МассивСделок, МассивНоменклатуры)
	...
	|
	|ВЫБРАТЬ
	|	ОтчетКомиссионераОПродажахВозвраты.ДокументРеализации,
	...
	|			И ОтчетКомиссионераОПродажахВозвраты.ДокументРеализации = ОтчетКомиссионераОПродажах.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажахВозвраты.ДокументРеализации В(&МассивСделок)
				...
				Для Каждого СтрокаПартии Из НайденныеСтроки Цикл
					...
					Для Каждого СтрокаСписания Из НайденныеСтрокиСписано Цикл
						...
						Для Каждого СтрокаОстатка Из НайденныеСтрокиОстаток Цикл
							...
							НоваяСтрокаПриход.ОтражениеВУСН = СтатусСписания;
							...
							Если (Реквизиты.Период >= Дата("20060101"))
								И (ТоварыПоОтгрузке)
								И (НЕ Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании)
...
Функция ПодготовитьТаблицуЗачетАвансовСУчетовВозврата(ТаблицаДокументаВзаиморасчеты, ТаблицаДокументаВзаиморасчетыВозвраты, ТаблицаРеквизиты, Отказ) Экспорт
	...
	ВзаиморасчетыВозвраты = ТаблицаДокументаВзаиморасчетыВозвраты.НайтиСтроки(Новый Структура("ДокументРасчетов", Реквизиты.Регистратор));
	
	Для Каждого СтрокаАвансыВосстановленные Из ВзаиморасчетыВозвраты Цикл
		...
		ОтборПоСтрокам.СчетРасчетов = СтрокаАвансыВосстановленные.СчетАвансов;
		
		СтрокиТаблицыДокумента = ТаблицаДокументаВзаиморасчеты.НайтиСтроки(ОтборПоСтрокам);
		
		Для Каждого СтрокаТаблицыДокумента Из СтрокиТаблицыДокумента Цикл
			
			Если СтрокаАвансыВосстановленные.СуммаВзаиморасчетов = СтрокаАвансыВосстановленные.ОстатокВзаиморасчетов Тогда
				Прервать;
			КонецЕсли;
			
			Если СтрокаТаблицыДокумента.СуммаВзаиморасчетов = 0 Тогда
				Продолжить;
			КонецЕсли;
			
			СуммаЗачета = Мин(СтрокаТаблицыДокумента.СуммаВзаиморасчетов, СтрокаАвансыВосстановленные.СуммаВзаиморасчетов - СтрокаАвансыВосстановленные.ОстатокВзаиморасчетов);
			
			СтрокаТаблицыДокумента.СуммаВзаиморасчетов 		 	= СтрокаТаблицыДокумента.СуммаВзаиморасчетов - СуммаЗачета;
			СтрокаТаблицыДокумента.СуммаРуб 		 			= СтрокаТаблицыДокумента.СуммаРуб - Мин(СтрокаТаблицыДокумента.СуммаРуб, СтрокаАвансыВосстановленные.СуммаРуб);
			СтрокаТаблицыДокумента.СуммаВзаиморасчетовПатент 	= СтрокаТаблицыДокумента.СуммаВзаиморасчетовПатент - Мин(СтрокаТаблицыДокумента.СуммаВзаиморасчетовПатент, СтрокаАвансыВосстановленные.СуммаВзаиморасчетовПатент);
			СтрокаТаблицыДокумента.СуммаВзаиморасчетовЕНВД 	 	= СтрокаТаблицыДокумента.СуммаВзаиморасчетовЕНВД - Мин(СтрокаТаблицыДокумента.СуммаВзаиморасчетовЕНВД, СтрокаАвансыВосстановленные.СуммаВзаиморасчетовЕНВД);
			СтрокаТаблицыДокумента.СуммаВзаиморасчетовКомитента = СтрокаТаблицыДокумента.СуммаВзаиморасчетовКомитента - Мин(СтрокаТаблицыДокумента.СуммаВзаиморасчетовКомитента, СтрокаАвансыВосстановленные.СуммаВзаиморасчетовКомитента);
			
			СтрокаАвансыВосстановленные.ОстатокВзаиморасчетов   = СтрокаАвансыВосстановленные.ОстатокВзаиморасчетов + СуммаЗачета;
...
Функция ДобавитьАвансыВозвратыКомиссионеру(ТаблицаВзаиморасчетовРеализация, ТаблицаВзаиморасчетовВозвраты, ТаблицаАвансовВозвраты, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаАвансовВозвраты)
		ИЛИ ТаблицаВозвратаВыделяемогоАванса.Количество() = 0 Тогда
	...
	Для каждого СтрокаВозврата Из ТаблицаАвансовВозвраты Цикл
		...
		Если СтрокаВозврата.ДокументАванса <> Реквизиты.Регистратор Тогда
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.СписокФорм    = "ФормаДокументаОбщая,ФормаДокументаТовары,ФормаДокументаСписание";
...
Функция ПечатьОтчетаКомиссионера(МассивОбъектов, ОбъектыПечати)
	...
	|	ОтчетКомиссионераОПродажах.ВидОперации,
	...
	Пока Шапка.Следующий() Цикл
		...
		НазваниеДокумента = ?(Шапка.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании, 
			"Отчет комиссионера о списании", "Отчет комиссионера о продаже");
		ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(Шапка, НазваниеДокумента);
		...
		Если Шапка.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
			ОбластьШапки = ОбластьШапки + "Списание";
		КонецЕсли;
		...
		Если Шапка.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
			ОбластьСтроки = ОбластьСтроки + "Списание";
		КонецЕсли;
		...
		Если НЕ Шапка.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
			// Вывести ИтогоНДС
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ОтчетКомиссионераОПродажахПокупатели.ДатаСФ КАК ДатаСФКоммиссионера,
	|	ОтчетКомиссионераОПродажахПокупатели.НомерСФ КАК НомерСФКомиссионера,
	|	Реквизиты.Контрагент КАК Комиссионер
	...
	|	Реквизиты.ДатаСФКоммиссионера КАК ДатаСФКоммиссионера,
	|	Реквизиты.НомерСФКомиссионера КАК НомерСФКомиссионера,
	|	Реквизиты.Комиссионер КАК Комиссионер
	...
	|	КОНЕЦ КАК АдресПокупателя,
	|	&СчетаФактурыКомиссионера КАК СчетаФактурыКомиссионера,
	|	ВЫБОР
	|		КОГДА &СводныйСФКомиссияПоПродаже
	...
	|	РеквизитыПредварительная.ДатаСФКоммиссионера КАК ДатаСФКоммиссионера,
	|	РеквизитыПредварительная.НомерСФКомиссионера КАК НомерСФКомиссионера,
	|	РеквизитыПредварительная.Комиссионер КАК Комиссионер
	|ИЗ
	...
	|	ТаблицаДокумента.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВЫБОР
	|		КОГДА ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ(ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Количество * (ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) / ТаблицаДокумента.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Количество * ТаблицаДокумента.Цена
	|				КОНЕЦ
	|		ИНАЧЕ ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Сумма
	...
	|			И ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Ссылка = ТаблицаДокумента.Ссылка";
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	ОтчетКомиссионераОПродажах.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	|	"""" КАК ИдентификаторГосКонтракта,
	|	"""" КАК КППСчетаФактуры,
	|	"""" КАК КПППродавца,
	...
	|			ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Ссылка КАК Ссылка,
	|			ИСТИНА КАК ЕстьПрослеживаемыеТовары
	|		ИЗ
	|			Документ.ОтчетКомиссионераОПродажах.СведенияПрослеживаемости КАК ОтчетКомиссионераОПродажахСведенияПрослеживаемости
	|		ГДЕ
	|			ОтчетКомиссионераОПродажахСведенияПрослеживаемости.Ссылка В(&МассивОбъектов)) КАК ПрослеживаемыеТовары
	|		ПО ОтчетКомиссионераОПродажах.Ссылка = ПрослеживаемыеТовары.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажах.Ссылка В(&МассивОбъектов)
	|	И ОтчетКомиссионераОПродажах.ДатаВходящегоДокумента > &УсловиеПоДате
...
Функция ДокументыСПрослеживаемымиТоварами(МассивДокументов) Экспорт
	...
	|	Документ.ОтчетКомиссионераОПродажах.Товары КАК ОтчетКомиссионераОПродажахТовары
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТовары.Ссылка В(&МассивДокументов)
	|	И ОтчетКомиссионераОПродажахТовары.ПрослеживаемыйТовар = ИСТИНА";
...
Функция ОписаниеИсправляемыхДокументов(МассивСтрок) Экспорт
	...
	Для каждого ТекущаяСтрока Из МассивСтрок Цикл
		МассивОснований.Добавить(ТекущаяСтрока.ИсходныйДокумент);
	КонецЦикла;
	СоответствиеРеквизитовСчетаФактуры = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивОснований,
		"Номер,Дата,НомерВходящегоДокумента,ОтредактированНомерСчетаФактуры");
	...
	Для каждого ТекущаяСтрока Из МассивСтрок Цикл
		...
		РеквизитыСчетаФактуры = СоответствиеРеквизитовСчетаФактуры[ТекущаяСтрока.ИсходныйДокумент];
		
		ЭтоПоследнийВыводимыйДокумент = (Индекс = КоличествоДокументов) ИЛИ (Индекс = РазрешенноеКоличествоДокументов);
		
		Если РеквизитыСчетаФактуры.ОтредактированНомерСчетаФактуры Тогда 
			НомерСчетаФактуры = РеквизитыСчетаФактуры.НомерВходящегоДокумента;
		Иначе
			НомерСчетаФактуры = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(РеквизитыСчетаФактуры.Номер, Истина, Ложь);
		КонецЕсли;
		
		ОписаниеДокумента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2%3'"),
			НомерСчетаФактуры,
			Формат(РеквизитыСчетаФактуры.Дата, "ДФ=dd.MM.yyyy"), ?(ЭтоПоследнийВыводимыйДокумент, "", ", "));
...
Процедура ЗаполнитьПоказателиПоСчетамФактурам(ТаблицаВозвраты, ТоварыВозвращенные,
	ДанныеОбъекта, ЗаполнятьТолькоНезаполненные) Экспорт

	ТаблицаОстатков = ТоварыКВозвратуПоОтчетуКомиссионеруПоПродажам(
		ДанныеОбъекта.СчетаФактуры, ДанныеОбъекта.Покупатели, ДанныеОбъекта.ДоговорКонтрагента);
		
	ОтборПоТаблице  = Новый Структура("КлючСтроки,ПоТекущемуДокументу");
	
	Для каждого СтрокаВозврат Из ТаблицаВозвраты Цикл
		
		Покупатель = СтрокаВозврат.Покупатель;
		ОтборПоТаблице.КлючСтроки = СтрокаВозврат.КлючСтроки;
		ОтборПоТаблице.ПоТекущемуДокументу = Ложь;
		НайденныеСтрокиТоварыВозвращенные = ТоварыВозвращенные.НайтиСтроки(ОтборПоТаблице);

		СтруктураОтбора = Новый Структура("Покупатель, Номенклатура, Цена, НомерГТД");

		Для каждого СтрокаТовары Из НайденныеСтрокиТоварыВозвращенные Цикл
		
			Если ЗаполнятьТолькоНезаполненные Тогда
			
				Если ЗначениеЗаполнено(СтрокаТовары.ДокументРеализации) 
					И ЗначениеЗаполнено(СтрокаТовары.ИсправляемыйДокумент) Тогда
			...
			СтруктураОтбора.Покупатель = Покупатель;
		
			ЗаполнитьДанныеПоКорректировкеТоваров(СтруктураОтбора, ТаблицаОстатков, СтрокаТовары, Истина);
...
Функция ТоварыКВозвратуПоОтчетуКомиссионеруПоПродажам(МассивСчетовФактур, Покупатели, ДоговорКонтрагента)
	...
	|	ОтчетКомиссионераОПродажахПокупатели.СчетФактура КАК ИсправляемыйДокумент,
	...
	|ПОМЕСТИТЬ ОстаткиПоНоменклатуре
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.Покупатели КАК ОтчетКомиссионераОПродажахПокупатели
	...
	|ГДЕ
	|	ОтчетКомиссионераОПродажахПокупатели.СчетФактура В(&МассивСчетовФактур)
	|	И ОтчетКомиссионераОПродажахПокупатели.Покупатель В(&Покупатели)
	|	И ОтчетКомиссионераОПродажахПокупатели.Ссылка.ДоговорКонтрагента = &ДоговорКонтрагента
	...
	|ВЫБРАТЬ
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.Номенклатура,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.КоличествоПослеИзменения,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.Цена,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.СуммаПослеИзменения,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.НомерГТД,
	|	ОтчетКомиссионераОПродажахВозвраты.СчетФактура,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.СуммаНДСПослеИзменения,
	|	ОтчетКомиссионераОПродажахВозвраты.ДатаСФ,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации,
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.СчетУчетаНДСПоРеализации,
	|	ОтчетКомиссионераОПродажахВозвраты.Покупатель,
	|	""""
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ОтчетКомиссионераОПродажахТоварыВозвращенные
	...
	|		ПО ОтчетКомиссионераОПродажахВозвраты.Ссылка = ОтчетКомиссионераОПродажах.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажахВозвраты.СчетФактура В(&МассивСчетовФактур)
	|	И ОтчетКомиссионераОПродажахВозвраты.Покупатель В(&Покупатели)
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента = &ДоговорКонтрагента
	...
	|	ОстаткиПоНоменклатуре.Количество КАК Количество,
	|	ОстаткиПоНоменклатуре.Цена КАК Цена,
	|	ОстаткиПоНоменклатуре.Сумма КАК Сумма,
	|	ОстаткиПоНоменклатуре.НомерГТД КАК НомерГТД,
	|	ОстаткиПоНоменклатуре.ИсправляемыйДокумент КАК ИсправляемыйДокумент,
	|	ОстаткиПоНоменклатуре.СуммаНДС КАК СуммаНДС,
	|	ОстаткиПоНоменклатуре.ДатаСФ КАК ДатаСФ,
	|	ОстаткиПоНоменклатуре.Ссылка КАК ДокументРеализации,
	|	ОстаткиПоНоменклатуре.СчетУчетаНДСПоРеализации КАК СчетУчетаНДСПоРеализации,
	|	ОстаткиПоНоменклатуре.Покупатель КАК Покупатель,
	|	ОстаткиПоНоменклатуре.НомерСФ КАК НомерСФ
	|ИЗ
	|	ОстаткиПоНоменклатуре КАК ОстаткиПоНоменклатуре
	|
	|УПОРЯДОЧИТЬ ПО
	|	Покупатель,
	|	ДатаСФ УБЫВ";
	...
	ТоварыКВозвратуПоСчетФактурам = Запрос.Выполнить().Выгрузить();
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ТоварыКВозвратуПоСчетФактурам;
КонецФункции

Процедура ЗаполнитьПоказателиПоВозвратуКомиссионера(СтрокаТовары, ДанныеОбъекта) Экспорт

	ТаблицаОстатков = ТоварыКВозвратуПоОтчетуКомиссионеруПоПродажам(
		ДанныеОбъекта.СчетаФактуры, ДанныеОбъекта.Покупатель, ДанныеОбъекта.ДоговорКонтрагента);
	
	СтруктураОтбора = Новый Структура("Покупатель,Номенклатура, Цена, НомерГТД");
	
	ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаТовары);
	СтруктураОтбора.Покупатель = ДанныеОбъекта.Покупатель;
		
	ЗаполнитьДанныеПоКорректировкеТоваров(СтруктураОтбора, ТаблицаОстатков, СтрокаТовары);
	
КонецПроцедуры

// Процедура записывает данные из ТаблицыОстатков в строку из табличной части ТоварыВозвращенные
// 
// Параметры:
//		СтруктураОтбора - Структура - параметры для отбора
//		ТаблицаОстатков - ТаблицаЗначений - Таблица с ранее проданными товарами
//		СтрокаТовары - ТаблицаЗначений - текущая строка из таблицы ТоварыВозвращенные
//		из ОтчетКомиссионераОПродажах для изменения
//
Процедура ЗаполнитьДанныеПоКорректировкеТоваров(СтруктураОтбора, ТаблицаОстатков, СтрокаТовары, ВозможенОтборПоДокументуРеализации = Ложь)
	...
	Иначе
		Индекс = 0;
		Если ВозможенОтборПоДокументуРеализации И ЗначениеЗаполнено(СтрокаТовары.ДокументРеализации) Тогда
			ИндексПоиска = 0;
			Для каждого ТекСтрокаНайденныеСтроки Из НайденныеСтроки Цикл
				
				Если ТекСтрокаНайденныеСтроки.ДокументРеализации = СтрокаТовары.ДокументРеализации Тогда
		...
		СтрокаОстатки = НайденныеСтроки[Индекс];
		
		Если СтрокаТовары.Количество > СтрокаОстатки.Количество Тогда
		...
		СтрокаТовары.ИсправляемыйДокумент     = СтрокаОстатки.ИсправляемыйДокумент;
		СтрокаТовары.ДокументРеализации       = СтрокаОстатки.ДокументРеализации;
		СтрокаТовары.СчетУчетаНДСПоРеализации = СтрокаОстатки.СчетУчетаНДСПоРеализации;
...
Функция НайтиСчетФактуруПоПараметрамКомиссионера(ПараметрыКомиссионера) Экспорт
	...
	|ГДЕ
	|	ОтчетКомиссионераОПродажахВозвраты.Покупатель = &Покупатель
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.ДоговорКонтрагента = &ДоговорКонтрагента
	|	И ОтчетКомиссионераОПродажахВозвраты.ДатаСФ = &Дата
	|	И ОтчетКомиссионераОПродажахВозвраты.НомерСФ = &Номер
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.Организация = &Организация
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.Проведен
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода";
...
// Возвраты - Таблица возвратов от покупателей 
...
Функция ЕстьСчетФактураВТекущемДокументе(ПараметрыОперации, Покупатели, Возвраты, ОтборПоПокупателю = Ложь) Экспорт
	...
	НайденныеСтрокиПоВозвратам = Возвраты.НайтиСтроки(Фильтр);
	
	Если (НайденныеСтрокиПоПродажам.Количество() > 0) 
		ИЛИ (НайденныеСтрокиПоВозвратам.Количество() > 0) Тогда 
		Возврат Истина;
	КонецЕсли;
...
// Обрабатываем списания, что были возвращены по текущему документу 
//
// Параметры:
//        ТаблицаСписанныеТовары - ТаблицаЗначений - таблица списанных товаров
//        ТаблицаСписанныеТоварыВозврат - ТаблицаЗначений - таблица списанных товаров по возврату
//        ТаблицаРеквизиты - ТаблицаЗначений - таблица реквизитов
//
Процедура ДобавитьСписанияВозвращенныеПоТекущемуДокументу(ТаблицаСписанныеТовары,
		ТаблицаСписанныеТоварыВозврат, ТаблицаРеквизиты) Экспорт
	
	Если Не ЗначениеЗаполнено(ТаблицаСписанныеТовары) Тогда
		Возврат;
	КонецЕсли;
	
	Параметры = ПодготовитьРеквизитыСписанияВозвращенныеПоТекущемуДокументу(ТаблицаРеквизиты, ТаблицаСписанныеТовары);
	Реквизиты = Параметры.Реквизиты[0];
	
	СписаниеБезВозвратовПоТекущемуДокументу
		= ТаблицаСписанныеТовары.НайтиСтроки(Новый Структура("ВозвратПоТекущемуДокументу", Истина));
	
	ТаблицаСписанныеТоварыПоОстаткам
		= Параметры.ТаблицаСписанныеТовары.Скопировать(Новый Структура("РассчитатьСебестоимостьПоТекущимПродажам", Ложь));
	ТаблицаСписанныеТоварыПоОстаткам.Колонки.Добавить("СписаноКоличество", Новый ОписаниеТипов("Число"));
	
	Если Не ЗначениеЗаполнено(СписаниеБезВозвратовПоТекущемуДокументу) Тогда
		Возврат;
	КонецЕсли;
	
	ФильтрПоСписаниюТоваров = Новый Структура("Номенклатура");
	
	Для каждого ТекущаяСтрока Из СписаниеБезВозвратовПоТекущемуДокументу Цикл
		
		ФильтрПоСписаниюТоваров.Номенклатура = ТекущаяСтрока.Номенклатура;
		
		// Если себестоимость не надо рассчитывать, заполняем сразу таблицу возврата
		Если НЕ ТекущаяСтрока.РассчитатьСебестоимостьПоТекущимПродажам Тогда
			
			НоваяСтрока = ТаблицаСписанныеТоварыВозврат.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
			НоваяСтрока.ИмяСписка = НСтр("ru = 'ТоварыВозвращенные'");
			НоваяСтрока.СинонимСписка = НСтр("ru = 'Товары возвращенные'");
			НоваяСтрока.Сделка = Реквизиты.Регистратор;
			НоваяСтрока.КорСчетСписания = ТекущаяСтрока.КорСчетСписанияВозврат;
			
		// Высчитываем стоимость и потом заполняем
		Иначе
			
			НайденныеСтрокиСписания = ТаблицаСписанныеТоварыПоОстаткам.НайтиСтроки(ФильтрПоСписаниюТоваров);
			
			Если НайденныеСтрокиСписания.Количество() > 0 Тогда
				
				Для каждого ТекущаяСтрокаСписания Из НайденныеСтрокиСписания Цикл
					
					ОстатокКоличество = ТекущаяСтрокаСписания.Количество - ТекущаяСтрокаСписания.СписаноКоличество;
					
					КоличествоСписать = Мин(ТекущаяСтрока.Количество, ОстатокКоличество);
					
					Если КоличествоСписать = 0 Тогда
						Продолжить;
					КонецЕсли;
					
					Если ТекущаяСтрока.Количество = ТекущаяСтрокаСписания.Количество
						И ТекущаяСтрокаСписания.СписаноКоличество = 0 Тогда
						
						Если ТекущаяСтрока.РассчитатьСебестоимостьПоТекущимПродажам Тогда
							
							НоваяСтрока = ТаблицаСписанныеТовары.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрокаСписания);
							ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока, "ВозвратПоТекущемуДокументу,КорСчетСписанияВозврат");
							
							ТекущаяСтрока.Количество = 0;
							
							// эта строка может являться реализацией для другого списания
							СписаниеПоОстаткамНоваяСтрока = ТаблицаСписанныеТоварыПоОстаткам.Добавить();
							ЗаполнитьЗначенияСвойств(СписаниеПоОстаткамНоваяСтрока, ТекущаяСтрокаСписания);
					...
					Иначе
						...
						//Списание реализации
						ЦенаСписания = Окр(ТекущаяСтрокаСписания.СуммаСписания/ТекущаяСтрокаСписания.Количество, 2);
						ЦенаСписанияНУ = ТекущаяСтрокаСписания.СуммаСписанияНУ/ТекущаяСтрокаСписания.Количество;
						ЦенаКорСписанияНУ = ТекущаяСтрокаСписания.СуммаКорСписанияНУ/ТекущаяСтрокаСписания.Количество;
						...
						Если ТекущаяСтрока.РассчитатьСебестоимостьПоТекущимПродажам Тогда
							...
							НоваяСтрока.СуммаСписания = КоличествоСписать* ЦенаСписания;
							НоваяСтрока.СуммаСписанияНУ = КоличествоСписать * ЦенаСписанияНУ;
							НоваяСтрока.СуммаКорСписанияНУ = КоличествоСписать * ЦенаКорСписанияНУ;
							
							ТекущаяСтрока.Количество = ТекущаяСтрока.Количество - КоличествоСписать;
							
							// эта строка может являться реализацией для другого списания
							СписаниеПоОстаткамНоваяСтрока = ТаблицаСписанныеТоварыПоОстаткам.Добавить();
							ЗаполнитьЗначенияСвойств(СписаниеПоОстаткамНоваяСтрока, НоваяСтрока);
					...
					ТекущаяСтрокаСписания.СписаноКоличество = ТекущаяСтрокаСписания.СписаноКоличество + КоличествоСписать;
	...
	НайденныеСтроки = ТаблицаСписанныеТовары.НайтиСтроки(Новый Структура("Количество", 0));
	
	Для каждого ТекущаяСтрока Из НайденныеСтроки Цикл
		
		ТаблицаСписанныеТовары.Удалить(ТекущаяСтрока);
		
	КонецЦикла;
		
КонецПроцедуры

Функция ПодготовитьРеквизитыСписанияВозвращенныеПоТекущемуДокументу(ТаблицаРеквизиты, ТаблицаСписанныеТовары)
	...
	СписокОбязательныхКолонок = ""
		...
		+ "СинонимСписка,"                            // <Строка> - синоним списка
		+ "Период,"                                   // <Дата> - дата
		...
		+ "Себестоимость,"                            // <Число,15,3> - себестоимость списанной номенклатуры
		...
		+ "ВидКорСубконто1,"                          // <Характеристика.ВидыСубконтоХозрасчетные> - кор.субконто списания МПЗ
		+ "ВидКорСубконто2,"                          // <Характеристика.ВидыСубконтоХозрасчетные> - кор.субконто списания МПЗ
		+ "ВидКорСубконто3,"                          // <Характеристика.ВидыСубконтоХозрасчетные> - кор.субконто списания МПЗ
		...
		+ "КорПодразделение,"                         // <СправочникСсылка.Подразделения> - кор. подразделение списания МПЗ
		+ "Подразделение,"                            // <СправочникСсылка.Подразделения> - подразделение списания МПЗ
		+ "СуммаСписания,"                            // <Число,15,2> - сумма списания
		+ "СуммаСписанияНУ,"                          // <Число,15,2> - сумма списания НУ
		+ "СуммаСписанияПР,"                          // <Число,15,2> - сумма списания ПР
		+ "СуммаСписанияВР,"                          // <Число,15,2> - сумма списания ВР
		+ "СуммаКорСписанияНУ,"                       // <Число,15,2> - кор. сумма списания НУ
		+ "СуммаКорСписанияПР,"                       // <Число,15,2> - кор. сумма списания ПР
		+ "СуммаКорСписанияВР,"                       // <Число,15,2> - кор. сумма списания ВР
		+ "Содержание,"                               // <Строка> - содержание
		+ "Комитент,"                                 // <СправочникСсылка.> - комитент
		+ "ДоговорКомиссии,"                          // <СправочникСсылка.> - комитент
		+ "СчетРасчетовСКомитентом,"                  // <ПланСчетовСсылка.Хозрасчетный> - счет расчетов с комитентом
		+ "СчетАвансовСКомитентом,"                   // <ПланСчетовСсылка.Хозрасчетный> - авансов. счет расчетов с комитентом
		+ "ВалютаРасчетовСКомитентом,"                // <СправочникСсылка.Валюты> - валюта расчетов с комитентом
		+ "СуммаРасчетовСКомитентом,"                 // <Число,15,2> - сумма расчетов с комитентом
		+ "СуммаПоступленияОтКомитента,"              // <Число,15,2> -  сумма поступления от комитента
		+ "Покупатель,"                               // <СправочникСсылка.Контрагенты> -  покупатель
		+ "ВыставленСФ,"                              // <Булево> -  признак выставления сф
		+ "ДатаСФ,"                                   // <Дата> -  дата сф
		+ "СчетФактура,"                              // <ДокументСсылка.СчетФактура> - счет-фактура
		+ "КлючСтроки,"                               // <Число,5> - ключ строки
		+ "ВозвратПоТекущемуДокументу,"               // <Булево> - признак, что партия списана по текущему документу
		+ "РассчитатьСебестоимостьПоТекущимПродажам," // <Булево> - признак, что по текущей строки необходимо рассчитать себестоимость
		+ "КорСчетСписанияВозврат";                   // <Характеристика.ВидыСубконтоХозрасчетные> - кор.субконто списания МПЗ по возврату
		
	Параметры.Вставить("ТаблицаСписанныеТовары", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		ТаблицаСписанныеТовары, СписокОбязательныхКолонок));
		
	Возврат Параметры;
КонецФункции

Функция ПодготовитьПараметрыТаблицыСписаниеТаблицыТоваров(СписаниеТоваровТаблицаТовары, ВозвратыТоваровТаблицаТовары)
	
	Параметры = Новый Структура;
	
	// Подготовка таблицы Параметры.СписаниеТоваровТаблицаТовары
	...
	СписокОбязательныхКолонок = ""
		...
		+ "Номенклатура,"                             // <СправочникСсылка.*> - списанная номенклатура
		+ "Склад,"                                    // <СправочникСсылка.Склады> - склад списания
		+ "ДокументОприходования,"                    // <ДокументСсылка.*> - документ оприходования
		+ "Себестоимость,"                            // <Число,15,3> - себестоимость списанной номенклатуры
		+ "Количество,"                               // <Число,15,3> - количество списанной номенклатуры
		+ "КорСчетСписания,"                          // <ПланСчетовСсылка.Хозрасчетный> - кор.счет списания МПЗ
		+ "ВидКорСубконто1,"                          // <Характеристика.ВидыСубконтоХозрасчетные> - кор.субконто списания МПЗ
	...
	Параметры.Вставить("СписаниеТоваровТаблицаТовары", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		СписаниеТоваровТаблицаТовары, СписокОбязательныхКолонок));
		
	// Подготовка таблицы Параметры.ВозвратыТоваровТаблицаТовары
	...
	СписокОбязательныхКолонок = ""
		...
		+ "НомерСтроки,"                              // <Число> - номер строки в списка 
		+ "СчетУчета,"                                // <ПланСчетовСсылка.Хозрасчетный> - счет учета МПЗ
		+ "СчетУчетаЗабалансовый,"                                // <ПланСчетовСсылка.Хозрасчетный> - счет учета МПЗ
		+ "Номенклатура,"                             // <СправочникСсылка.*> - списанная номенклатура
		+ "Склад,"                                    // <СправочникСсылка.Склады> - склад списания
		+ "Партия,"                                   // <ДокументСсылка.*> - документ поступления номенклатуры (партия)
		...
		+ "ДатаСФ,"                                   // <Дата> -  дата сф
		+ "СчетФактура,"                              // <ДокументСсылка.СчетФактура> - счет-фактура
		+ "ПоТекущемуДокументу";                      // <Булево> - признак, что партия списана по текущему документу
		
	Параметры.Вставить("ВозвратыТоваровТаблицаТовары", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		ВозвратыТоваровТаблицаТовары, СписокОбязательныхКолонок));
		
	Возврат Параметры;
КонецФункции

// Функция отделяет от реализации товара реализацию, по которой был возврат по текущему документу, 
// по таблице СписаниеТоваровТаблицаТовары.
// Реализация, по которой был возврат, по текущему документу помечается признаком ПоТекущемуДокументу.
//
// Есть вероятность, что такого товара может не быть на остатках.
// Поэтому по этой реализации с установленным признаком ПоТекущемуДокументу 
// устанавливается признак РассчитатьСебестоимостьПоТекущимПродажам. Чтобы можно было 
// рассчитать себестоимость по списанному ранее товару в этом документе
//
// Далее в процедуре УчетТоваров.СписатьОстаткиТоваров признак РассчитатьСебестоимостьПоТекущимПродажам
// может установиться как Ложь, если определена партия списания (определена себестоимость)
//
// Если себестоимость не определена на момент списания, 
// то в процедуре ОтчетКомиссионераОПродажах.ДобавитьСписанияВозвращенныеПоТекущемуДокументу 
// обрабатываются списания, что были возвращены по текущему документу 
// с установленным признаком РассчитатьСебестоимостьПоТекущимПродажам 
// и определяется себестоимость таких возвратов
//
// Параметры:
//         СписаниеТоваровТаблицаТовары - ТаблицаЗначений - таблица реализованного товара
//         ВозвратыТоваровТаблицаТовары - ТаблицаЗначений - таблица возвращенного товара
//
// Возвращаемое значение:
//         ТаблицаЗначений - таблица списанного товара. В этой таблице отделены реализации, по которым был возврат по текущему документу
//
Функция ПодготовитьТаблицуСписаниеТоваровТаблицаТовары(СписаниеТоваровТаблицаТовары, 
		ВозвратыТоваровТаблицаТовары) Экспорт
		
	Если Не ЗначениеЗаполнено(ВозвратыТоваровТаблицаТовары)
		ИЛИ Не ЗначениеЗаполнено(СписаниеТоваровТаблицаТовары) Тогда
		Возврат СписаниеТоваровТаблицаТовары;
	КонецЕсли;
	
	Параметры = ПодготовитьПараметрыТаблицыСписаниеТаблицыТоваров(СписаниеТоваровТаблицаТовары, ВозвратыТоваровТаблицаТовары);
	
	ТаблицаВозвратовПоТекущемуДокументу = Параметры.ВозвратыТоваровТаблицаТовары.Скопировать(
		Новый Структура("ПоТекущемуДокументу", Истина));
		
	Если НЕ ЗначениеЗаполнено(ТаблицаВозвратовПоТекущемуДокументу) Тогда
		Возврат СписаниеТоваровТаблицаТовары;
	КонецЕсли;
	
	ТаблицаВозвратовПоТекущемуДокументу.Свернуть("Номенклатура,КорСчетСписания", "Количество");
	
	ТаблицаВозвратовПоТекущемуДокументу.Колонки.Добавить("Распределено", Новый ОписаниеТипов("Число"));
	
	СписаниеТоваровТаблица = Параметры.СписаниеТоваровТаблицаТовары.СкопироватьКолонки();
	
	ФильтрПоСписаниюТоваров = Новый Структура("Номенклатура");
	
	Для каждого ТекущаяСтрока Из Параметры.СписаниеТоваровТаблицаТовары Цикл
		
		ФильтрПоСписаниюТоваров.Номенклатура = ТекущаяСтрока.Номенклатура;
		КоличествоПоРеализации = ТекущаяСтрока.Количество;
		
		НайденныеСтрокиВозврата = ТаблицаВозвратовПоТекущемуДокументу.НайтиСтроки(ФильтрПоСписаниюТоваров);
		
		// Если возврата нет, то добавляем всю строку полностью
		Если НайденныеСтрокиВозврата.Количество() = 0 Тогда
			
			НоваяСтрока = СписаниеТоваровТаблица.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
			
			НоваяСтрока.РассчитатьСебестоимостьПоТекущимПродажам = Ложь;
			КоличествоПоРеализации = 0;
		Иначе
			Для каждого ТекущаяСтрокаВозврата Из НайденныеСтрокиВозврата Цикл
				
				Если ТекущаяСтрокаВозврата.Распределено = ТекущаяСтрокаВозврата.Количество Тогда
					Продолжить;
				КонецЕсли;
				
				ОстатокНеРаспределенный = ТекущаяСтрокаВозврата.Количество - ТекущаяСтрокаВозврата.Распределено;
				МожноРаспределить = Мин(КоличествоПоРеализации, ОстатокНеРаспределенный);
				
				// В таблицу добавляется реализация, по которой был возврат
				НоваяСтрока = СписаниеТоваровТаблица.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
				НоваяСтрока.Количество = МожноРаспределить;
				
				НоваяСтрока.РассчитатьСебестоимостьПоТекущимПродажам = Истина;
				НоваяСтрока.ВозвратПоТекущемуДокументу = Истина;
				НоваяСтрока.КорСчетСписанияВозврат = ТекущаяСтрокаВозврата.КорСчетСписания;
				КоличествоНеРаспределенное = 0;
				
				// В таблицу добавляется реализация, по которой не было возврата
				Если КоличествоПоРеализации <> МожноРаспределить Тогда
					
					НоваяСтрокаРеализация = СписаниеТоваровТаблица.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаРеализация, ТекущаяСтрока);
					КоличествоНеРаспределенное = ТекущаяСтрока.Количество - МожноРаспределить;
					НоваяСтрокаРеализация.Количество = КоличествоНеРаспределенное;
					
				КонецЕсли;
				
				КоличествоПоРеализации = КоличествоПоРеализации - МожноРаспределить - КоличествоНеРаспределенное;
				ТекущаяСтрокаВозврата.Распределено = ТекущаяСтрокаВозврата.Распределено + МожноРаспределить;
				
				Если КоличествоПоРеализации = 0 Тогда 
					Прервать;
				КонецЕсли;
		...
		// Добавляем строку полностью, когда не было возврата по этой строке
		Если КоличествоПоРеализации > 0 И ТекущаяСтрока.Количество = КоличествоПоРеализации Тогда
	...
	Возврат СписаниеТоваровТаблица;
	
КонецФункции

Процедура ЗаполнитьПоказателиПоТекущемуДокументу(ТаблицаВозвраты, ТоварыВозвращенные, ТаблицаОстатков) Экспорт
	...
	Для каждого СтрокаВозврат Из ТаблицаВозвраты Цикл
		...
		Покупатель = СтрокаВозврат.Покупатель;
		ОтборПоТаблице.КлючСтроки = СтрокаВозврат.КлючСтроки;
		ОтборПоТаблице.ПоТекущемуДокументу = Истина;
		НайденныеСтрокиТоварыВозвращенные = ТоварыВозвращенные.НайтиСтроки(ОтборПоТаблице);
		...
		Для каждого СтрокаТовары Из НайденныеСтрокиТоварыВозвращенные Цикл
			...
			ЗаполнитьДанныеПоКорректировкеТоваров(СтруктураОтбора, ТаблицаОстатков, СтрокаТовары, Истина);
			СтрокаТовары.ДокументРеализации = ""; 
...
Функция ПолучитьСоответствиеВидовОперацийФормам() Экспорт
	...
	ФормыРеализацияТоваровУслуг.Вставить(Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОПродажах,        "ФормаДокументаОбщая");
	ФормыРеализацияТоваровУслуг.Вставить(Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании,        "ФормаДокументаСписание");
	ФормыРеализацияТоваровУслуг.Вставить(Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах, "ФормаДокументаТовары");
	ФормыРеализацияТоваровУслуг.Вставить(Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании,    "ФормаДокументаОприходование");
		
	Возврат ФормыРеализацияТоваровУслуг;

КонецФункции 

Функция ВидОперацииПоФункциональнымОпциям() Экспорт
	ПродажиЧерезМаркетплейс   = ПолучитьФункциональнуюОпцию("ИспользоватьПродажиЧерезМаркетплейс");
	ПродажиЧерезКомиссионеров = ПолучитьФункциональнуюОпцию("ОсуществляетсяРеализацияТоваровУслугЧерезКомиссионеров");
	
	ВидОперации = Неопределено;
	Если ПродажиЧерезМаркетплейс И НЕ ПродажиЧерезКомиссионеров Тогда
		ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах;
	ИначеЕсли НЕ ПродажиЧерезМаркетплейс И ПродажиЧерезКомиссионеров Тогда
...
// Обработчик обновления на версию 3.0.73.41.
// Переносит реквизит Сделка из табличной части Возвраты в Табличную часть ТоварыВозвращенные
//
Процедура ОбновитьДокументыПоКомиссии(Параметры) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка КАК Ссылка,
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата КАК Дата
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ОтчетКомиссионераОПродажахТоварыВозвращенные
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.Возвраты КАК ОтчетКомиссионераОПродажахВозвраты
	|		ПО ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка = ОтчетКомиссионераОПродажахВозвраты.Ссылка
	|			И ОтчетКомиссионераОПродажахТоварыВозвращенные.КлючСтроки = ОтчетКомиссионераОПродажахВозвраты.КлючСтроки
	|			И ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации <> ОтчетКомиссионераОПродажахВозвраты.Сделка
	|ГДЕ
	|	НЕ ОтчетКомиссионераОПродажахВозвраты.Сделка = ЗНАЧЕНИЕ(Документ.ОтчетКомиссионераОПродажах.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка,
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка.Дата
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			ОбработатьДокументПоКомиссии(ДокументОбъект);
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументОбъект);
			ЗафиксироватьТранзакцию();
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Процедуре Документ.ОтчетКомиссионераОПродажах.ОбновитьДокументыПоКомиссии()
					| не удалось обработать документ по причине: %1'"),
	...
	Если ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтрШаблон(
			НСтр("ru = 'Процедуре Документ.ОтчетКомиссионераОПродажах.ОбновитьДокументыПоКомиссии() 
					|не удалось обработать документ: в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.Контрагенты,,
			СтрШаблон(НСтр("ru = 'Процедура Документ.ОтчетКомиссионераОПродажах.ОбновитьДокументыПоКомиссии()
			| обработала очередную количество документов: %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
	
КонецПроцедуры

Процедура ОбработатьДокументПоКомиссии(ДокументОбъект)

	Возвраты = ДокументОбъект.Возвраты;
	ТоварыВозвращенные = ДокументОбъект.ТоварыВозвращенные;

	Для каждого ТекущаяСтрокаВозвраты Из Возвраты Цикл
	
		Сделка = ТекущаяСтрокаВозвраты.Сделка;
		КлючСтроки = ТекущаяСтрокаВозвраты.КлючСтроки;
		
		НайденныеСтроки = ТоварыВозвращенные.НайтиСтроки(Новый Структура("КлючСтроки", КлючСтроки));
		
		Для каждого ТекущаяСтрокаНайденныеСтроки Из НайденныеСтроки Цикл
		
			Если Сделка = ТекущаяСтрокаНайденныеСтроки.ДокументРеализации Тогда
				Продолжить;
			КонецЕсли;
			
			ТекущаяСтрокаНайденныеСтроки.ДокументРеализации = Сделка;
...
Процедура ЗаполнитьВидОперацииПоУмолчанию(Параметры) Экспорт
	...
	|	Документ.ОтчетКомиссионераОПродажах КАК ОтчетКомиссионераОПродажах
	|ГДЕ
	|	ОтчетКомиссионераОПродажах.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ОтчетКомиссионераОПродажах.ЗаполнитьВидОперацииПоУмолчанию() не удалось обработать документ ""Отчет комиссионера"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ОтчетКомиссионераОПродажах.ЗаполнитьВидОперацииПоУмолчанию() не удалось обработать документ ""Отчет комиссионера"": в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ОтчетКомиссионераОПродажах.ЗаполнитьВидОперацииПоУмолчанию() обработала очередную порцию документов ""Отчет комиссионера"": %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
...
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура ЗаполнитьОтражениеВУСН(Параметры) Экспорт
	...
	|	Документ.ОтчетКомиссионераОПродажах.Товары КАК ОтчетКомиссионераОПродажахТовары
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТовары.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|	И ОтчетКомиссионераОПродажахТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетКомиссионера.ОтчетОСписании)
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетКомиссионераОПродажахТовары.Ссылка,
	|	ОтчетКомиссионераОПродажахТовары.Ссылка.Дата
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ";
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТоваров Из ДокументОбъект.Товары Цикл
			Если НЕ ЗначениеЗаполнено(СтрокаТоваров.ОтражениеВУСН) Тогда
				СтрокаТоваров.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
			КонецЕсли;
		КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ОтчетКомиссионераОПродажах.ЗаполнитьОтражениеВУСН() не удалось обработать документ ""Отчет комиссионера"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ОтчетКомиссионераОПродажах, Выборка.Ссылка, ТекстСообщения);
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ОтчетКомиссионераОПродажах.ЗаполнитьОтражениеВУСН() не удалось обработать документ ""Отчет комиссионера"": в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ОтчетКомиссионераОПродажах,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ОтчетКомиссионераОПродажах.ЗаполнитьОтражениеВУСН() обработала очередную порцию документов ""Отчет комиссионера"": %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
...
#Область ИзменениеВидаОперации

Процедура ЗаполнитьНДСВТабличнойЧастиПриИзмененииВидаОперации(Форма, ИмяТабличнойЧасти) Экспорт
	
	ПараметрыСведений = Новый Структура("Дата,Организация", Форма.Объект.Дата, Форма.Объект.Организация);
	Сведения = БухгалтерскийУчетПереопределяемый.ПолучитьСведенияОСпискеНоменклатуры(Форма.Объект.Ссылка[ИмяТабличнойЧасти].ВыгрузитьКолонку("Номенклатура"), ПараметрыСведений);
	
	Для Каждого СтрокаТабличнойЧасти Из Форма.Объект[ИмяТабличнойЧасти] Цикл
		ДанныеНоменклатуры = Сведения.Получить(СтрокаТабличнойЧасти.Номенклатура);
		Если Не ДанныеНоменклатуры = Неопределено Тогда
			СтавкаНДС = ДанныеНоменклатуры.СтавкаНДС;
		Иначе
			СтавкаНДС = УчетНДСКлиентСервер.ОбщаяСтавкаНДС(Форма.Объект.Дата);
		КонецЕсли;
		СтрокаТабличнойЧасти.СтавкаНДС = СтавкаНДС;
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуНДСТабЧасти(СтрокаТабличнойЧасти, Форма.Объект.СуммаВключаетНДС);
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьСуммыВСтрокеПриИзмененииВидаОперации(Форма, ИмяТабличнойЧасти, СтрокаДанных, ОчищатьКлючиСтрок = Ложь)  Экспорт
	
	СтрокаДанных.Всего 	= Форма.Объект[ИмяТабличнойЧасти].Итог("Сумма");
	СтрокаДанных.НДС 	= Форма.Объект[ИмяТабличнойЧасти].Итог("СуммаНДС");
	
	Если ОчищатьКлючиСтрок Тогда
		Для Каждого СтрокаТабличнойЧасти Из Форма.Объект[ИмяТабличнойЧасти] Цикл
			СтрокаТабличнойЧасти.КлючСтроки = 0;	
		КонецЦикла;
	КонецЕсли;
...
#Область УправлениеВидимостьюПодсказки

// Устанавливает флаг в настройках о том что пользователь видел подсказки в панели подсказок
// Если будет добавлена новая подсказка, то необходимо в общем модуле "ОбновленииИнформационнойБазыБП"
// добавить обработчик вызывающий эту процедуру с параметром "Ложь". В этом случае при первом открытии
// пользователем формы списка документов  откроется панель подсказок с новыми подсказками
//
// Параметры: ВиделПодсказку Тип("Булево") -необяз. значение по умолч. "Истина"
//
// Возвращаемое значение: нет
//
Процедура УстановитьПользовательВиделПодсказку(ВиделПодсказку = Истина) Экспорт
	
	КлючОбъекта 		= "ПанельПодсказокФормыСпискаОтчетаКомиссионера";
	КлючНастроек 		= "ПользовательВиделПодсказку";
	
	ХранилищеОбщихНастроек.Сохранить(КлючОбъекта, КлючНастроек, ВиделПодсказку);
	
КонецПроцедуры

Функция ПользовательВиделПодсказку() Экспорт
...
// Получает строковое представление счета-фактуры из сведений о реквизитах
// Параметры:
//  СчетФактура - ДокументСсылка.СчетФактураВыданный или СчетФактураПолученный - документ для которого
//	  формируется строковое представление
//  РеквизитыСчетаФактуры - Структура - извлеченные реквизиты данного счета-фактуры
//    если РеквизитыСчетаФактуры = Неопределено то функция извлекает 
//    реквизиты для данного счета-фактуры самостоятельно
// Возвращаемое значение:
//  - Строка - строковое представление счета-фактуры
//
Функция ОписаниеСчетаФактуры(СчетФактура, РеквизитыСчетаФактуры) Экспорт
	...
	Если ТипЗнч(СчетФактура) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
		...
		Если РеквизитыСчетаФактуры = Неопределено Тогда
			ПараметрыСчетФактуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СчетФактура, 
				"НомерВходящегоДокумента, ДатаВходящегоДокумента");
		Иначе
			ПараметрыСчетФактуры = РеквизитыСчетаФактуры;
		КонецЕсли;
		
		ОписаниеСчетФактуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),
			ПараметрыСчетФактуры.НомерВходящегоДокумента,
			Формат(ПараметрыСчетФактуры.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy"));
	
	ИначеЕсли ТипЗнч(СчетФактура) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		...
		Если ПараметрыСчетФактуры.ОтредактированНомерСчетаФактуры Тогда
			НомерСчетаФактуры = ПараметрыСчетФактуры.НомерВходящегоДокумента;
		Иначе
			НомерСчетаФактуры = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ПараметрыСчетФактуры.Номер, Истина, Ложь);
		КонецЕсли;
...
Процедура СформироватьДвиженияЗачетРасчетовПоВозврату(ТаблицаВзаиморасчеты, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ТаблицаВзаиморасчетыВозврат = ТаблицаВзаиморасчеты.Скопировать(Новый Структура("ДокументРасчетов", Реквизиты.Регистратор));

	Для каждого СтрокаВзаиморасчетов Из ТаблицаВзаиморасчетыВозврат Цикл
		
		СуммаВзаиморасчетов = СтрокаВзаиморасчетов.СуммаВзаиморасчетов - СтрокаВзаиморасчетов.ОстатокВзаиморасчетов;
		...
		Проводка.Сумма       = Окр(СтрокаВзаиморасчетов.СуммаРуб * СуммаВзаиморасчетов / СтрокаВзаиморасчетов.СуммаВзаиморасчетов, 2, 1);
		...
		Проводка.СчетКт = Реквизиты.СчетУчетаРасчетовПоАвансамПолученным;
		...
		Если СвойстваСчетаДт.Валютный Тогда
			Проводка.ВалютаДт        = Реквизиты.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаДт = СуммаВзаиморасчетов;
		КонецЕсли;
		Если СвойстваСчетаКт.Валютный Тогда
			Проводка.ВалютаКт        = Реквизиты.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаКт = СуммаВзаиморасчетов;
		КонецЕсли;
...
// Получает одним запросом сведения о реквизитах счетов-фактур для отображения в табличных частях документов
// Параметры:
//  СписокСчетовФактур - Массив - список документов типа СчетФактураВыданный или СчетФактураПолученный
//	  для которых формируются сведения
// Возвращаемое значение:
//  - Соответствие
//		- Ключ - ДокументСсылка.СчетФактураВыданный или СчетФактураПолученный для которого собираются сведения
//		- Значение - Структура - набор реквизитов документа 
//
Функция ДанныеРеквизитовСчетовФактур(СписокСчетовФактур) Экспорт
	...
	|	СчетФактураВыданный.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаВходящегоДокумента,
	...
	|ГДЕ
	|	СчетФактураВыданный.Ссылка В(&СписокСчетовФактур)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_СчетаФактуры.Ссылка КАК Ссылка,
	|	ВТ_СчетаФактуры.Номер КАК Номер,
	|	ВТ_СчетаФактуры.Дата КАК Дата,
	|	ВТ_СчетаФактуры.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ВТ_СчетаФактуры.ДатаВходящегоДокумента КАК ДатаВходящегоДокумента,
	|	ВТ_СчетаФактуры.ОтредактированНомерСчетаФактуры КАК ОтредактированНомерСчетаФактуры
	|ИЗ
	|	ВТ_СчетаФактуры КАК ВТ_СчетаФактуры";
	
	ДанныеСчетовФактур = Новый Соответствие;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ДанныеСчетовФактур.Вставить(ВыборкаДетальныеЗаписи.Ссылка, 
			ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(ВыборкаДетальныеЗаписи));
