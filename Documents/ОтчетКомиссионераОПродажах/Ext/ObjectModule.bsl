...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ВидОперации = Документы.ОтчетКомиссионераОПродажах.ВидОперацииПоФункциональнымОпциям();
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("Структура")
		...
		ВидОперации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
	...
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
		СуммаВключаетНДС = Истина;		
		
		Если НЕ ЗначениеЗаполнено(СтатьяПрочихДоходовИРасходов) Тогда
			СтатьяПрочихДоходовИРасходов = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.СписаниеТоваровУКомиссионера");
		КонецЕсли;
		
	КонецЕсли;
	
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
		СуммаВключаетНДС = Истина;		
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	РаботаСНоменклатуройБП.ОбновитьПризнакПрослеживаемости(ТоварыВозвращенные, ВедетсяУчетПрослеживаемыхТоваров, Дата);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ТоварыВозвращенные.СтавкаНДС");
		МассивНепроверяемыхРеквизитов.Добавить("ТоварыВозвращенные.Сумма");
	Иначе
		МассивНепроверяемыхРеквизитов.Добавить("СтатьяПрочихДоходовИРасходов");		
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Если НЕ ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
	...
	Для каждого СтрокаТаблицы Из ТоварыВозвращенные Цикл
		...
		Если НЕ ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
		...
		Если СтрокаТаблицы.ПрослеживаемыйТовар
			И Не ЗначениеЗаполнено(СтрокаТаблицы.ДокументРеализации)
	...
	Если ПлательщикНДФЛ 
		И (НЕ ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании) 
		И (НЕ ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ИменаТабличныхЧастей.Добавить("ТоварыВозвращенные");

	ОбщегоНазначенияБПКлиентСервер.ЗаполнитьИдентификаторыСтрок(ЭтотОбъект, ИменаТабличныхЧастей);
	
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
		Услуги.Очистить();
		ДенежныеСредства.Очистить();
		Возвраты.Очистить();
		ТоварыВозвращенные.Очистить();
		ВозвратыИсходныеДокументы.Очистить();
	...
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
		...
		Покупатели.Очистить();
		ВозвратыИсходныеДокументы.Очистить();
		Для Каждого СтрокаТабличнойЧасти Из ЭтотОбъект.ТоварыВозвращенные Цикл
			СтрокаТабличнойЧасти.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
			СтрокаТабличнойЧасти.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Распределен;
			СтрокаТабличнойЧасти.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			СтрокаТабличнойЧасти.СуммаНДС = 0;
		КонецЦикла;
	КонецЕсли;
	...
	Если ЕстьПрослеживаемыйТовар Тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			ПрослеживаемостьБП.СверитьОстаткиРНПТ(ЭтотОбъект, Отказ, "Товары");
		КонецЕсли;
	КонецЕсли;
	
	ПрослеживаемыйВозвращенныйТовар = ТоварыВозвращенные.НайтиСтроки(Новый Структура("ПрослеживаемыйТовар", Истина));
	ЕстьПрослеживаемыйВозвращенныйТовар = ПрослеживаемыйВозвращенныйТовар.Количество() <> 0;
	
	Если ЕстьПрослеживаемыйВозвращенныйТовар Тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			ПрослеживаемостьБП.СверитьОстаткиРНПТ(ЭтотОбъект, Отказ, "ТоварыВозвращенные");
		КонецЕсли;
	КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОПродажах
		И НаВозвратыВыставляетсяКорректировочныйСчетФактура 
		И РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		
		ПроверитьОстаткиПоТекущемуДокументу(Отказ);
		
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах
		И РежимЗаписи = РежимЗаписиДокумента.Проведение 
		И НЕ БезОтраженияВУчете Тогда
		
		ПроверитьОстаткиПоТекущемуДокументуРозница(Отказ);
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам") Тогда
		Документы.ОтчетКомиссионераОПродажах.ЗаполнитьСчетаУчетаРасчетов(Неопределено, Истина, ЭтотОбъект);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	// При загрузке электронного документа не создаем перевыставленные счета-фактуры, т.к. пользователь после формирования 
	// может произвести дополнительные действия в форме документа, например установить флаг "Выписывать счета-фактуры сводно".
	ЭтоЗагрузкаЭлектронногоДокумента = ДополнительныеСвойства.Свойство("ЭтоЗагрузкаЭлектронногоДокумента")
		И ДополнительныеСвойства.ЭтоЗагрузкаЭлектронногоДокумента = Истина;
	
	Если ДополнительныеСвойства.РежимЗаписи = РежимЗаписиДокумента.Запись 
			И НЕ РучнаяКорректировка
			И НЕ ЭтоЗагрузкаЭлектронногоДокумента Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаВзаиморасчетов = УчетВзаиморасчетов.ПодготовитьТаблицуВзаиморасчетовЗачетАвансовРасчетыСКомиссионером(
	...
	// Взаиморасчеты по зачету вознаграждения, полученного отдельным документом, из выручки
	ТаблицаВзаиморасчетыКомиссияПоПродаже = Документы.ОтчетКомиссионераОПродажах.ПодготовитьТаблицуВзаиморасчетыВознаграждение(ТаблицаВзаиморасчетов, 
		ПараметрыПроведения.ЗачетЗадолженностиВознаграждениеРеквизиты, Отказ);
	...
	// Таблицы выручки от реализации: собственных товаров и услуг и отдельно комиссионных
	ТаблицыКомпенсации = УчетДоходовРасходов.ПодготовитьТаблицыВыручкиОтРеализацииКомиссионером(
		ПараметрыПроведения.КомпенсацияТаблицаДокумента, ТаблицаВзаиморасчетовСВозвратом, ТаблицаСписанныеТовары,
		ПараметрыПроведения.РеализацияРеквизиты, Отказ);
	...
	СтруктураТаблицУСН.Вставить("ТаблицаРасчетовПоПриобретению", ТаблицаВзаиморасчетовВознаграждение);
	СтруктураТаблицУСН.Вставить("ТаблицаРасчетовПоРеализации", ТаблицаВзаиморасчетовСВозвратом);
	СтруктураТаблицУСН.Вставить("ТаблицаВознаграждение", ПараметрыПроведения.ВознаграждениеТаблица);
	СтруктураТаблицУСН.Вставить("ДолгПокупателя", ТаблицаВзаиморасчетыКомиссияПоПродаже);
	СтруктураТаблицУСН.Вставить("ДенежныеСредства", ПараметрыПроведения.ДенежныеСредства);
	СтруктураТаблицУСН.Вставить("ТаблицаВозвраты", ПараметрыПроведения.ВозвратыТоваровТаблицаДокумента);
	СтруктураТаблицУСН.Вставить("ТаблицаСобственныеТоварыУслуги", ТаблицыРеализация.СобственныеТоварыУслуги);
	СтруктураТаблицУСН.Вставить("ТаблицаСобственныеТоварыУслугиВозврат", ТаблицыРеализацияВозврат.СобственныеТоварыУслуги);
	...
	УчетТоваров.СформироватьДвиженияСписанияТоваровВозвратОтПокупателя(ТаблицаСписанныеТоварыВозврат,
	...
	УчетВзаиморасчетов.СформироватьДвиженияЗачетАвансов(
		ТаблицаВзаиморасчетыВозврат, ПараметрыПроведения.ЗачетАвансовНаВозвратРеквизиты, Движения, Отказ);
	
	Документы.ОтчетКомиссионераОПродажах.СформироватьДвиженияЗачетРасчетовПоВозврату(ТаблицаВзаиморасчетыВозврат,
	...
	УчетДоходовРасходов.СформироватьДвиженияКомпенсации(
			ТаблицыКомпенсации.СобственныеТоварыУслуги, ТаблицыКомпенсации.ТоварыУслугиКомитентов,
			ТаблицыКомпенсации.РеализованныеТоварыКомитентов, ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
	...
	УчетНДСБП.СформироватьДвиженияРублевыеСуммыДокументовВВалюте(ТаблицыКомпенсации.СобственныеТоварыУслуги, 
		ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
		
	УчетНДСБП.СформироватьДвиженияРублевыеСуммыДокументовВВалюте(ТаблицыКомпенсации.ТоварыУслугиКомитентов, 
		ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
	...
	// Списание товаров и комиссионером
	УчетНДСБП.СформироватьДвиженияСписаниеТоваровНедостача(
		ПараметрыПроведения.НДСТоварыСписание, ТаблицаСписанныеТовары, ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияДоходОтПродажЧерезКомиссионера(
		ТаблицаИПМПЗОтгруженные,
		ТаблицаВзаиморасчетовСВозвратом,
		ПараметрыПроведения.ПогашениеЗадолженностиВознаграждениеТаблицаВзаиморасчетов,
		ПараметрыПроведения.ДенежныеСредства,
		ПараметрыПроведения.Реквизиты,
		Движения,
		Отказ);
	...
	Иначе
		...
		//НДС по возвратам
		ТаблицыРеализацияВозврат = УчетДоходовРасходов.ПодготовитьТаблицыВыручкиОтРеализацииВозвратКомиссионеру(
			ПараметрыПроведения.ВозвратыТоваровТаблицаДокумента, Неопределено, Неопределено,
			ПараметрыПроведения.ВозвратыТоваровРеквизиты, Отказ);
		
		СобственныеТоварыУслугиНДСВозврат = ТаблицыРеализацияВозврат.СобственныеТоварыУслуги;
		СобственныеТоварыУслугиНДСВозврат = УчетНДСРаздельный.ПодготовитьТаблицуСобственныеТоварыУслугиНДСВозвратКомиссионеру(
			СобственныеТоварыУслугиНДСВозврат, ПараметрыПроведения.ТоварыНДС);
		
		ТаблицаСписанныеТоварыВозврат = УчетТоваров.ПолучитьПустуюТаблицуСписанныеТовары();
		Для Каждого СтрокаТаблицы Из СобственныеТоварыУслугиНДСВозврат Цикл
		...
		Если ЗначениеЗаполнено(ТаблицыРеализация.СобственныеТоварыУслуги) Тогда
		...
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах 
		...
		Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
			...
			СуммаВозврат    = СобственныеТоварыУслугиНДСВозврат.Итог("СуммаБезНДСРуб");
			СуммаНДСВозврат = СобственныеТоварыУслугиНДСВозврат.Итог("СуммаНДСРуб");
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
			СуммаДоходов    = ПараметрыПроведения.КомпенсацияТаблицаДокумента.Итог("СуммаБезНДСРуб");
		ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании Тогда
			СуммаВозврат    = ПараметрыПроведения.ВозвратыТоваровТаблицаДокумента.Итог("СуммаБезНДСРуб");
		КонецЕсли;
		
		НаименованияКонтрагента = Справочники.Контрагенты.НаименованияНаДату(Реквизиты.Контрагент, Реквизиты.Период);
		...
		КонецЕсли;
		ОписаниеСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Получен отчет комиссионера ""%1"";'"), ИмяНаПечать);
		...
		Если СуммаДоходов - СуммаВозврат > 0 Тогда
			...
			СтрокаКУдиР.Графа4      = (СуммаДоходов - СуммаВозврат) + (СуммаНДС - СуммаНДСВозврат);
			СтрокаКУдиР.Графа5      = СуммаДоходов - СуммаВозврат;
			СтрокаКУдиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;
			ПояснениеОперации = ?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании,
				НСтр("ru = ' Компенсация за утеряные товары.'"), НСтр("ru = ' Реализованы товары.'"));
			Если СуммаНДС - СуммаНДСВозврат > 0 Тогда
				ПояснениеНДС = СтрШаблон(ШаблонПояснениеНДС, ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаНДС - СуммаНДСВозврат, , , " "));
		...
		Иначе
			...
			Если СуммаВозврат > 0 Тогда
				...
				СтрокаКУдиР.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента;
				ПояснениеОперации = ?(Реквизиты.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОбОприходовании,
					НСтр("ru = ' Сторнирована компенсация за утеряные товары.'"), НСтр("ru = ' Отражен возврат покупателю.'"));
				СтрокаКУдиР.Содержание = ОписаниеСобытия + ПояснениеОперации;
	...
	ЕстьЭкспорт = ПолучитьФункциональнуюОпцию("ВедетсяУчетТаможенныхДекларацийЭкспорт");
	
	Если ЕстьЭкспорт
		И Проведен
		И ЕстьПокупателиИзЕАЭС(Ссылка) Тогда
		
		ПровестиНеПроведенныеЗаявлениеяОВвозеПолученные(Ссылка);
		
	КонецЕсли;
	
	ЗарегистрироватьУстареваниеОперацииНачисленияНалогаПриОтраженииДохода(Истина);
	
КонецПроцедуры

Процедура ОбработкаУдаленияПроведения(Отказ)

	ЗарегистрироватьУстареваниеОперацииНачисленияНалогаПриОтраженииДохода(Ложь);
...
Процедура ПроверитьОстаткиПоТекущемуДокументу(Отказ)
	...
	ТаблицаВозвратовПоТекущемуДокументу.Колонки.Добавить("ВыставленСФ",
	...
	Для Каждого ТекущаяСтрока Из ТаблицаВозвратовПоТекущемуДокументу Цикл
		...
		ЗаполнитьЗначенияСвойств(ТекущаяСтрока, НайденнаяСтрока,"Покупатель,СчетФактура,ВыставленСФ"); 
	...
	ТаблицаТекущихПродаж.Колонки.Добавить("ВыставленСФ", Новый ОписаниеТипов("Булево"));
	...
	Для Каждого ТекущаяСтрока Из Покупатели Цикл
		...
		Для Каждого ТекущаяПродажа Из НайденныеСтроки Цикл
			...
			ЗаполнитьЗначенияСвойств(ТекущаяПродажа, ТекущаяСтрока,"Покупатель,СчетФактура,ВыставленСФ"); 
	...
	Для каждого ТекущаяСтрокаВозвраты Из Возвраты Цикл
		...
		Если НЕ ТекущаяСтрокаВозвраты.ВыставленСФ Тогда
			Продолжить;
		КонецЕсли;
	...
	ТаблицаТекущихПродаж.Свернуть("Покупатель,Номенклатура,Цена,СчетФактура,ВыставленСФ", "Количество");
	...
	ТаблицаВозвратовПоТекущемуДокументу.Свернуть(
		"Номенклатура,Цена,ИсправляемыйДокумент,Покупатель,СчетФактура,ВыставленСФ", "Количество,КоличествоПослеИзменения");
	
	// Проверим, что в возврате правильно установлена галочка ВыставленСФ
	ТаблицаВозвратаСвернутая = ТаблицаВозвратовПоТекущемуДокументу.Скопировать(,"Покупатель,ВыставленСФ");
	ТаблицаВозвратаСвернутая.Свернуть("Покупатель,ВыставленСФ");
	ТаблицаПродажСвернутая = ТаблицаТекущихПродаж.Скопировать(,"Покупатель,ВыставленСФ");
	ТаблицаПродажСвернутая.Свернуть("Покупатель,ВыставленСФ");
	Фильтр = Новый Структура("Покупатель");
	Для Каждого ТекСтрокаВозврата Из ТаблицаВозвратаСвернутая Цикл
		
		ЗаполнитьЗначенияСвойств(Фильтр, ТекСтрокаВозврата);
		
		НайденныеСтроки = ТаблицаПродажСвернутая.НайтиСтроки(Фильтр);	
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			Если НайденныеСтроки[0].ВыставленСФ 
				И НЕ ТекСтрокаВозврата.ВыставленСФ Тогда
				
				ТекстСообщения = НСтр("ru = 'На закладке Возвраты для покупателя %1 не указан признак выставления счета-фактуры'");
				
			ИначеЕсли НЕ НайденныеСтроки[0].ВыставленСФ 
				И ТекСтрокаВозврата.ВыставленСФ Тогда
				
				ТекстСообщения = НСтр("ru = 'На закладке Возвраты для покупателя %1 ошибочно указан признак выставления счета-фактуры'");
				
			КонецЕсли;
			
			Если НайденныеСтроки[0].ВыставленСФ <> ТекСтрокаВозврата.ВыставленСФ Тогда
				
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Фильтр.Покупатель);
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'На закладке Возвраты по покупателю %1 не найдена продажа в текущем документе'");
...
// Возвращаемое значение:
//  СписокЗначений - список ошибок
Процедура ПроверитьОстаткиПоТекущемуДокументуРозница(Отказ)
	...
	ТаблицаТекущихПродаж.Свернуть("Номенклатура", "Количество");
	
	ТаблицаВозвратовПоТекущемуДокументуРазвернутая = ТаблицаВозвратовПоТекущемуДокументу.Скопировать();
	
	ТаблицаВозвратовПоТекущемуДокументу.Свернуть("Номенклатура", "Количество");
	
	Фильтр = Новый Структура("Номенклатура");
	
	Для каждого ТекущаяСтрока Из ТаблицаВозвратовПоТекущемуДокументу Цикл
		
		ЗаполнитьЗначенияСвойств(Фильтр, ТекущаяСтрока);
		КоличествоВозврат = ТекущаяСтрока.Количество;
		...
		ИначеЕсли КоличествоВозврат > 0 Тогда
			...
			ТекстСообщения = НСтр("ru = 'Возвращено номенклатуры: %1 
							|больше, чем реализовано в текущем документе на %2'");
...
//
// Параметры:
//  ДокументПередачи - Массив - список документов на на основании которых заполняется документ
//  КлючСтроки - число - ключ строки таблицы Покупатели по которому будут добавляться товары. 
//               Если Неопределено - то ключ автоматически определяется в максимальный
// СоздаватьЗаписиПокупатели - Булево - создавать новые записи таблицы Покупатели. В случае если 
//               передано Ложь - товары добавятся пот текущую запись с ключом строки из второго параметра
//
Процедура ЗаполнитьТовары(СписокДокументовПередачи = Неопределено, КлючСтроки = Неопределено, СоздаватьЗаписиПокупатели = Истина) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СписокДокументовПередачи",	СписокДокументовПередачи);
	...
	|ГДЕ
	|	РеализацияТоваровУслугТовары.Ссылка В(&СписокДокументовПередачи)
	...
	СоответствиеСведенийОНоменклатуре = БухгалтерскийУчетПереопределяемый.ПолучитьСведенияОСпискеНоменклатуры(
		ОбщегоНазначения.ВыгрузитьКолонку(ОбщаяТаблицаТоваров, "Номенклатура", Истина), ДанныеОбъекта);
		
	Если КлючСтроки = Неопределено Тогда
		КлючСтроки = Документы.ОтчетКомиссионераОПродажах.ПолучитьМаксимальныйКлючСтроки(Покупатели);
	КонецЕсли; 
		
	Для каждого ДокументОтгрузки Из СписокДокументовПередачи Цикл
		
		ТаблицаТоваров = ОбщаяТаблицаТоваров.НайтиСтроки(Новый Структура("Ссылка", ДокументОтгрузки));
		...
		Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл
			...
			Если НЕ ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
		...
		Если СоздаватьЗаписиПокупатели 
			И ВидОперации <> Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
			СтрокаПокупателя = Покупатели.Добавить();
			СтрокаПокупателя.Покупатель	= ТаблицаТоваров[0].Контрагент;
			СтрокаПокупателя.КлючСтроки	= КлючСтроки;
...
Процедура ЗаполнитьПоДокументу(Сделка, Покупатель, КлючСтроки)Экспорт
	...
	Для каждого СтрокаТаблицыВозвраты Из ЗаполненныеСтрокиПоДокументу Цикл
		...
		СтруктураОтбора = Новый Структура("ИдентификаторСтроки", СтрокаТаблицыВозвраты.ИдентификаторСтроки);
	...
	ИсходныеРНПТ = Сделка.СведенияПрослеживаемости;
	...
	Для каждого СтрокаТаблицыТоваров Из ТаблицаТоваров Цикл
		...
		Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
			...
			СтруктураОтбора = Новый Структура("ИдентификаторСтроки", СтрокаТаблицыТоваров.ИдентификаторСтроки);
			МассивСтрокСведенияПрослеживаемости = ИсходныеРНПТ.НайтиСтроки(СтруктураОтбора);
			Для Каждого СтрокаРНПТ Из МассивСтрокСведенияПрослеживаемости Цикл
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента, "ВидДоговора") <> Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
			ВызватьИсключение НСтр("ru='Отчет комиссионера вводится на основании документа ""Реализация (акт, накладная, УПД)"",
		...
		ЗаполнитьТовары(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Основание));
...
Процедура СформироватьСчетаФактуры(Отказ)
	...
	// При загрузке электронного документа не создаем перевыставленные счета-фактуры, т.к. пользователь после формирования 
	// может произвести дополнительные действия в форме документа, например установить флаг "Выписывать счета-фактуры сводно".
	Если ЭтоЗагрузкаЭлектронногоДокумента Тогда
		Возврат;
	КонецЕсли;
...
Процедура НайтиСоздатьПодчиненныеСчетаФактуры(ДокументСсылка, ДанныеСчетовФактур, ПодчиненныеСчетаФактуры, ИмяТаблицы)
	...
	СписокДобавленныхСФ = Новый СписокЗначений;
	...
	Для каждого СтрокаТаблицы Из ДанныеСчетовФактур Цикл
		...
		СтрокаСФ = НайтиСтрокуСчетаФактуры(ПодчиненныеСчетаФактуры, СтрокаТаблицы, ИмяТаблицы, СписокДобавленныхСФ);
		...
		Если СтрокаСФ <> Неопределено Тогда
			...
			Если СтрокаТЧ <> Неопределено Тогда 
				...
				Если ИмяТаблицы = "Покупатели" Тогда
					СписокДобавленныхСФ.Добавить(СтрокаСФ.Ссылка); 	
				КонецЕсли;	
			КонецЕсли;
...
Функция НаличиеСоглашенияЭДСКомиссионером()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Тогда
		...
		ПараметрыЭД = Новый Структура("ТипДокумента,Направление,Организация,Контрагент, Договор");
		ПараметрыЭД.Организация	 = Организация;
		ПараметрыЭД.Контрагент   = Контрагент;
		ПараметрыЭД.Договор		 = ДоговорКонтрагента;
		ПараметрыЭД.ТипДокумента = ОбменСКонтрагентами.ТипыДокументов().СчетФактура;
		ПараметрыЭД.Направление  = ОбменСКонтрагентами.НаправленияДокументов().Исходящий;
...
Функция НайтиСтрокуСчетаФактуры(ТаблицаСФ, СтрокаДокумента, ИмяТаблицы, СписокДобавленныхСФ)
	...
	Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		Если НайденнаяСтрока.Покупатель = СтрокаДокумента.Покупатель
			...
			ИначеЕсли НайденнаяСтрока.КлючСтроки <> Неопределено Тогда
				...
				КонецЕсли;
			ИначеЕсли ИмяТаблицы = "Возвраты" Тогда
				// По возвратам выставляем новый корр. счет-фактуру
				Возврат Неопределено;
			ИначеЕсли СписокДобавленныхСФ.НайтиПоЗначению(НайденнаяСтрока.Ссылка) <> Неопределено Тогда
				Продолжить;
			Иначе
				// найден подходящий счет-фактура
				Возврат НайденнаяСтрока;
...
// Функция проверяет есть ли в отчете комиссионера покупатели из ЕАЭС
//
//Параметры
// Ссылка - ДокументСсылка.ОтчетКомиссионераОПродажах
//
//Возвращаемое значение:
// Булево - Истина, если в табличной части "Покупатели" есть покупатели со страной регистрации не РФ,
// которая является государством-членом ЕАЭС
//
Функция ЕстьПокупателиИзЕАЭС(Ссылка)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОтчетКомиссионераОПродажахПокупатели.Покупатель.СтранаРегистрации КАК ПокупательСтранаРегистрации
	|ПОМЕСТИТЬ СтраныРегистрацииПокупателей
	|ИЗ
	...
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПокупательСтранаРегистрации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	СтраныРегистрацииПокупателей.ПокупательСтранаРегистрации КАК ПокупательСтранаРегистрации
	|ИЗ
	|	СтраныРегистрацииПокупателей КАК СтраныРегистрацииПокупателей
	|ГДЕ
	|	СтраныРегистрацииПокупателей.ПокупательСтранаРегистрации <> ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|	И СтраныРегистрацииПокупателей.ПокупательСтранаРегистрации.УчастникЕАЭС";
...
// Процедура проводит уже созданные на основании счета-фактуры выданного по отчету комиссионера
// заявления о ввозе товаров, полученные от покупателя
Процедура ПровестиНеПроведенныеЗаявлениеяОВвозеПолученные(Ссылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка КАК Ссылка,
	|	ВЫРАЗИТЬ(ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки КАК Документ.СчетФактураВыданный) КАК ДокументОтгрузки
	|ПОМЕСТИТЬ ДокументОтгрузкиСчетФактура
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваровПолученное.ДокументыОтгрузки КАК ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки
	|ГДЕ
	|	НЕ ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка.Проведен
	|	И НЕ ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка.ПометкаУдаления
	|	И ТИПЗНАЧЕНИЯ(ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки) = ТИП(Документ.СчетФактураВыданный)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументОтгрузкиСчетФактура.Ссылка КАК Ссылка
	|ИЗ
	|	ДокументОтгрузкиСчетФактура КАК ДокументОтгрузкиСчетФактура
	|ГДЕ
	|	ДокументОтгрузкиСчетФактура.ДокументОтгрузки.ДокументОснование = &ДокументОснование";
	
	Запрос.Параметры.Вставить("ДокументОснование", Ссылка);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		// Пытаемся установить признак проведения в завлении о ввозе товаров, полученных от покупателя
		Попытка
			ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
			ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Исключение
			// Если признак проведения в документе "Заявление о ввозе товаров (полученное)" установить невозможно,
			// то это не препятствует проведению отчета комиссионера
		КонецПопытки;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗарегистрироватьУстареваниеОперацииНачисленияНалогаПриОтраженииДохода(Проведение)
	
	// Если доход отражается за предыдущий налоговый период (квартал),
	// то необходимо сбросить актуальность операции расчета налога
	...
	Если Не ПрименяетсяУСН И Не ПлательщикНДФЛ Тогда
		Возврат;
	КонецЕсли;
	
	ОтражатьДоходНаДатуОтчета = Ложь;
	Если ПрименяетсяУСН Тогда
		ОтражатьДоходНаДатуОтчета = НалоговыйУчетУСН.ОтражатьДоходыНаДатуОтчетаКомиссионера(Организация, Дата);
	ИначеЕсли ПлательщикНДФЛ Тогда
		ОтражатьДоходНаДатуОтчета = УчетДоходовИРасходовПредпринимателя.ОтражатьДоходыНаДатуОтчетаКомиссионера(Организация, Дата);
	КонецЕсли;
	
	Если Не ОтражатьДоходНаДатуОтчета Тогда
		Возврат;
	КонецЕсли;
	
	Если ПрименяетсяУСН Тогда
		ИмяРегистраОтраженияДохода = "КнигаУчетаДоходовИРасходов";
	ИначеЕсли ПлательщикНДФЛ Тогда
		ИмяРегистраОтраженияДохода = "ИПДоходы";
	КонецЕсли;
	
	Если Проведение Тогда
		ДатыОплаты = Движения[ИмяРегистраОтраженияДохода].Выгрузить(, "Период");
	Иначе
		Движения[ИмяРегистраОтраженияДохода].Прочитать();
		ДатыОплаты = Движения[ИмяРегистраОтраженияДохода].Выгрузить(, "Период");
		Движения[ИмяРегистраОтраженияДохода].Очистить();
	КонецЕсли;
	
	Если ДатыОплаты.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДатыОплаты.Сортировать("Период");
	МинимальнаяДатаДохода = ДатыОплаты[0].Период;
	
	Если КонецКвартала(МинимальнаяДатаДохода) < КонецКвартала(Дата) Тогда
		
		Если ПрименяетсяУСН Тогда
			ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаУСН;
		ИначеЕсли ПлательщикНДФЛ Тогда
			ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНДФЛДляИП;
		КонецЕсли;
		
		Если РаботаСПоследовательностями.ЕстьНеустаревшаяОперация(Организация, КонецКвартала(МинимальнаяДатаДохода), ВидРегламентнойОперации) Тогда
			РегистрыСведений.НеактуальныеРегламентныеОперации.СдвинутьГраницуАктуальностиНазад(
				Организация,
				КонецКвартала(МинимальнаяДатаДохода),
				Перечисления.ВидыРегламентныхОпераций.НомерГруппы(ВидРегламентнойОперации),
