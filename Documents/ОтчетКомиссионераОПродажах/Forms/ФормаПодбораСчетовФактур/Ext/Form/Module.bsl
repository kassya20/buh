...
Процедура СчетаФактурыПриИзменении(Элемент)
	...
	РаспознатьСчетФактуру(ТекущиеДанные, Неопределено);
...
Процедура РаспознатьСчетФактуру(ТекущаяСтрока, РеквизитыСчетовФактур)
	...
	ИначеЕсли РеквизитыСчетовФактур = Неопределено Тогда 
	
		ТекущаяСтрока.ПредставлениеИсходногоДокумента = 
			ОписаниеИсходногоДокумента(ТекущаяСтрока.ИсходныйДокумент, Неопределено);
			
	Иначе
		ТекущаяСтрока.ПредставлениеИсходногоДокумента = 
			ОписаниеИсходногоДокумента(ТекущаяСтрока.ИсходныйДокумент, РеквизитыСчетовФактур[ТекущаяСтрока.ИсходныйДокумент]);
...
Функция ОписаниеИсходногоДокумента(ИсходныйДокумент, РеквизитыСчетовФактур)

	Возврат Документы.ОтчетКомиссионераОПродажах.ОписаниеСчетаФактуры(ИсходныйДокумент, РеквизитыСчетовФактур);
...
Процедура УправлениеФормой(Форма)
	...
	Для каждого ТекущаяСтрокаСчетаФактуры Из Форма.СчетаФактуры Цикл
		МассивСчетовФактур.Добавить(ТекущаяСтрокаСчетаФактуры.ИсходныйДокумент);
	КонецЦикла;
	
	СчетаФактурыДляОтображения = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивСчетовФактур);
	
	РеквизитыСчетовФактур = ДанныеРеквизитовСчетовФактур(СчетаФактурыДляОтображения);
		
	Для Каждого ТекущаяСтрокаСчетаФактуры Из Форма.СчетаФактуры Цикл
		РаспознатьСчетФактуру(ТекущаяСтрокаСчетаФактуры, РеквизитыСчетовФактур);
	КонецЦикла;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДанныеРеквизитовСчетовФактур(СчетаФактуры)
	
	Возврат Документы.ОтчетКомиссионераОПродажах.ДанныеРеквизитовСчетовФактур(СчетаФактуры);
