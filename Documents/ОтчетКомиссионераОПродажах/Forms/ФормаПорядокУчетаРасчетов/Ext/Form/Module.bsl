...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, Параметры,
		...
		|СчетУчетаРасчетовЗаПосредническиеУслуги, СчетУчетаРасчетовПоАвансамВыданным, ПользовательУправляетСчетамиУчета");
	
	Если НЕ Параметры.Свойство("ДоступенЗачетАвансов", ДоступенЗачетАвансов) Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПользовательУправляетСчетамиУчета Тогда
		Если НЕ ЗначениеЗаполнено(СчетУчетаРасчетовПоАвансамПолученным) И ДоступенЗачетАвансов Тогда
		...
		Если НЕ ЗначениеЗаполнено(СчетУчетаРасчетовПоАвансамВыданным) И НЕ ДоступенЗачетАвансов Тогда
			Если СпособЗачетаАвансов <> ПредопределенноеЗначение("Перечисление.СпособыЗачетаАвансов.НеЗачитывать") Тогда
				ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(
					"Поле", "Заполнение", НСтр("ru = 'Счет расчетов по авансам выданным'"));
				Поле = "СчетУчетаРасчетовПоАвансамВыданным";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура ЗачетАвансовДокументАвансаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("ТипыДокументов"       , ОписаниеТипаРеквизитаДокументАванса());
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СчетУчетаРасчетовПоАвансамПолученным.Видимость = Форма.ДоступенЗачетАвансов;
	Элементы.ГруппаШапка.Видимость = Форма.ПользовательУправляетСчетамиУчета;
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ОтчетКомиссионераОПродажах.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
