...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_СчетФактураВыданный" Или ИмяСобытия = "Запись_СчетФактураВыданныйКорректировочный"
	...
	ИначеЕсли ИмяСобытия = "ОбновитьДокументИБПослеЗаполнения" И Параметр.Найти(Объект.Ссылка) <> Неопределено
		ИЛИ ИмяСобытия = "Запись_ЗаявлениеОВвозеТоваровПолученное" Тогда
		Прочитать();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.ОтчетКомиссионераОПродажах.Форма.ФормаПодбораСчетовФактур" Тогда
		
		ОбработкаВыбораПодбораСчетовФактур(ВыбранноеЗначение);
		
		ОбновитьТекущуюСтрокуВозвраты(ЭтотОбъект);
		
		ОбновитьДокументыВТаблицеВозвратов();
		
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.СчетФактураВыданный.Форма.ФормаВыбора" Тогда
		...
		ИначеЕсли ИсточникВыбора.Параметры.КлючНазначенияИспользования = "ТаблицаТоварыВозвращенные" Тогда
		
			ТекущиеДанныеТоварыВозвращенные = Элементы.ТоварыВозвращенные.ТекущиеДанные;
			ТекущиеДанныеВозврат = Элементы.Возвраты.ТекущиеДанные;
			
			ЗаполнитьДаннымиТаблицуТоварыВозвращенныеНаСервере(
				ТекущиеДанныеТоварыВозвращенные.НомерСтроки-1, ТекущиеДанныеВозврат.Покупатель, ВыбранноеЗначение);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для каждого СтрокаТаблицы Из Объект.Покупатели Цикл
		...
		Если СтрокаТаблицы.ВыставленСФ И Не ЗначениеЗаполнено(СтрокаТаблицы.НомерСФ)
			И Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение",
				НСтр("ru = 'Номер'"),
	...
	Если Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура Тогда
		...
		Для каждого СтрокаВозвраты Из Объект.Возвраты Цикл
			
			Если Не СтрокаВозвраты.ВыставленСФ Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаВозвраты.КСчетуФактуреКомиссионераНомер)
				И Не ЗначениеЗаполнено(СтрокаВозвраты.КСчетуФактуреКомиссионераДата)
				И Не СтрокаВозвраты.КСчетуФактуреКомиссионераНомер = НСтр("ru='Сводно'") Тогда
				
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
				"Поле", "Заполнение", НСтр("ru = 'Дата'"));
				ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Объект.Возвраты", СтрокаВозвраты.НомерСтроки, "КСчетуФактуреКомиссионераДата");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ПутьКДанным,, Отказ);
				
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаВозвраты.КСчетуФактуреКомиссионераНомер)
				И ЗначениеЗаполнено(СтрокаВозвраты.КСчетуФактуреКомиссионераДата) Тогда
				
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
				"Поле", "Заполнение", НСтр("ru = 'Номер'"));
				ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(
					"Объект.Возвраты", СтрокаВозвраты.НомерСтроки, "КСчетуФактуреКомиссионераНомер");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, ПутьКДанным,, Отказ);
		...
		ЗаполнитьИсправленныйДокументНаСервере(Истина);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ВедетсяУчетЭкспортаТоваров
		И Объект.Покупатели.Количество() > 0 Тогда
		
		УстановитьЗавленияОВвозеТоваровПолученные();
...
&НаСервере
Процедура ЗаполнитьДаннымиТаблицуТоварыВозвращенныеНаСервере(НомерСтрокиТоварыВозвращенные, Покупатель, СчетФактура)

	ТекущиеДанныеТоварыВозвращенные = Объект.ТоварыВозвращенные[НомерСтрокиТоварыВозвращенные];

	ДанныеОбъекта = Новый Структура("Дата,Ссылка,СчетаФактуры,Покупатель,ДоговорКонтрагента");
	ЗаполнитьЗначенияСвойств(ДанныеОбъекта, Объект);
	СчетаФактуры = Новый Массив();
	СчетаФактуры.Добавить(СчетФактура);
	ДанныеОбъекта.СчетаФактуры = СчетаФактуры;
	ДанныеОбъекта.Покупатель = Покупатель;
	ДанныеОбъекта.ДоговорКонтрагента = Объект.ДоговорКонтрагента;

	Документы.ОтчетКомиссионераОПродажах.ЗаполнитьПоказателиПоВозвратуКомиссионера(
		ТекущиеДанныеТоварыВозвращенные, ДанныеОбъекта);
...
Процедура СпособРегистрацииСчетФактурыПриИзменении()
	...
	ОчиститьПолеСчетовФактурНаСервере();
...
&НаКлиенте
Процедура НадписьСчетФактураРеализацияНажатие(Элемент, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	
	ВидСчетФактуры = ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.Нареализацию");
	ОткрытьСписокСчетовФактур(ВидСчетФактуры);
	
КонецПроцедуры

&НаКлиенте
Процедура НадписьСчетФактураВозвратыНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ВидСчетФактуры = ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.Корректировочный");
	ОткрытьСписокСчетовФактур(ВидСчетФактуры);
...
Процедура ПокупателиТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Копирование Тогда
			...
			ТекДанныеТовары.Всего = 0;
			ТекДанныеТовары.НДС   = 0;
			
			ТекДанныеТовары.ВсегоПоСФ = 0;
			ТекДанныеТовары.НДСПоСФ   = 0;
...
Процедура ПокупателиПослеУдаления(Элемент)
	Если ВедетсяУчетЭкспортаТоваров Тогда
		ВыводитьЗаявлениеОВвозеТоваровПолученное();
		УправлениеФормой(ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПокупателиТоварыВыставленСФПриИзменении(Элемент)
	...
	ДанныеСтроки.НомерСФ = "";
	ДанныеСтроки.ДатаСФ = "";
	ДанныеСтроки.ПредставлениеСчетаФактуры = "";
	ОбновитьИтоги(ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ПокупателиТоварыПокупательСоздание(Элемент, СтандартнаяОбработка)
...
Процедура ПокупателиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ПокупателиПредставлениеСчетаФактуры" И ЗначениеЗаполнено(ТекущаяСтрока.СчетФактура) Тогда
	
		СтандартнаяОбработка = Ложь;
		БухгалтерскийУчетКлиентПереопределяемый.ОткрытьСчетФактуру(ЭтотОбъект, ТекущаяСтрока.СчетФактура, "СчетФактураВыданный");

	КонецЕсли;
	
	Если Поле.Имя = "ПокупателиПредставлениеЗаявленияОВвозеТоваровПолученное" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Если ЗначениеЗаполнено(ТекущаяСтрока.ЗаявлениеОВвозеТоваровПолученное) Тогда
			БухгалтерскийУчетКлиентПереопределяемый.ОткрытьЗаявлениеОВвозеТоваровПолученное(ЭтотОбъект, ТекущаяСтрока.ЗаявлениеОВвозеТоваровПолученное);
		Иначе
			
			ВыбраннаяСтрока = Элементы.Покупатели.ТекущиеДанные;
			Если ВыбраннаяСтрока = Неопределено
				ИЛИ НЕ ВыбраннаяСтрока.ПокупательИзЕАЭС Тогда
				Возврат;
			КонецЕсли;
			
			Индекс = Элементы.Покупатели.ТекущаяСтрока;
			
			Если ЗначениеЗаполнено(ВыбраннаяСтрока.СчетФактура) Тогда
			
				ЗначенияЗаполнения = Новый Структура;
				ЗначенияЗаполнения.Вставить("Ссылка", ВыбраннаяСтрока.СчетФактура);
				ЗначенияЗаполнения.Вставить("Контрагент", ВыбраннаяСтрока.Покупатель);
				ЗначенияЗаполнения.Вставить("Организация", Объект.Организация);
				ЗначенияЗаполнения.Вставить("Провести", Объект.Проведен);
				
				Отбор = Новый Структура ("КлючСтроки", ВыбраннаяСтрока.КлючСтроки);
				НайденныеСтроки = Объект.Товары.НайтиСтроки(Отбор);
				ЗначенияЗаполнения.Вставить("ЕстьТовары", НайденныеСтроки.Количество() > 0);
				
				ПараметрыФормы = Новый Структура;
				ПараметрыФормы.Вставить("ЗначенияЗаполнения", ЗначенияЗаполнения);
				
				Оповещение = Новый ОписаниеОповещения("ВыборЗаявленияОВвозеТоваровПолученногоЗавершение",
									ЭтотОбъект, Элементы.Покупатели.ТекущиеДанные);
				
				ОткрытьФорму("Документ.ЗаявлениеОВвозеТоваровПолученное.Форма.ФормаНовогоЭлемента", ПараметрыФормы, ЭтотОбъект,,,,Оповещение);
			...
			Иначе
				...
				ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка",,
					НСтр("ru = 'Выставленный комиссионеру'"),
					Индекс + 1, ИмяСписка);
				ТекстСообщения = ТекстСообщения + НСтр("ru = '.
					|Заявление о ввозе товаров (полученное) создается на основании счета-фактуры.'");
				
				Поле = СтрШаблон(НСтр("ru = 'Покупатели[%1].ПредставлениеСчетаФактуры'"),
...
Процедура ПокупателиТоварыПокупательПриИзменении(Элемент)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		...
		ЗаполнитьКлючиСтрокРеализацийВЕАЭС(ТекущаяСтрока.КлючСтроки);
		...
		Если ВедетсяУчетЭкспортаТоваров Тогда
			ТекущаяСтрока.ПокупательИзЕАЭС = УчетНДСВызовСервера.ЭтоУчастникЕАЭС(ТекущаяСтрока.Покупатель);
			Если ТекущаяСтрока.ПокупательИзЕАЭС Тогда
				ЕстьКонтрагентыИзЕАЭС = Истина;
				ПараметрыПокупателя = Новый Структура(
				"Покупатель, Счетфактура, ЗаявлениеОВвозеТоваровПолученное, ПредставлениеЗаявленияОВвозеТоваровПолученное");
				ЗаполнитьЗначенияСвойств(ПараметрыПокупателя, ТекущаяСтрока);
				
				УстановитьЗавлениеОВвозеТоваровПолученное(ПараметрыПокупателя);
				ЗаполнитьЗначенияСвойств(ТекущаяСтрока, ПараметрыПокупателя);
			Иначе
				ТекущаяСтрока.ЗаявлениеОВвозеТоваровПолученное = Неопределено;
				ТекущаяСтрока.ПредставлениеЗаявленияОВвозеТоваровПолученное = "";
				ВыводитьЗаявлениеОВвозеТоваровПолученное();
			КонецЕсли;
...
Процедура УслугиНоменклатураПриИзменении(Элемент)
	...
	ДанныеОбъекта = Новый Структура(
		"Дата, Организация, СуммаВключаетНДС, ВидОперации,
		|СпособРасчетаКомиссионногоВознаграждения, ПроцентКомиссионногоВознаграждения, СтавкаНДСВознаграждения,
...
Процедура ВозвратыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Если Копирование Тогда
			ТекДанныеТовары.ВыставленСФ = Ложь;
			ТекДанныеТовары.СчетФактура = Неопределено;
			ТекДанныеТовары.ПояснениеСчетФактура = "";
			ТекДанныеТовары.ПояснениеКСчетамФактурам = "";
			ТекДанныеТовары.НомерСФ =  "";
			ТекДанныеТовары.ДатаСФ = "";
			ТекДанныеТовары.КСчетуФактуреКомиссионераНомер = "";
			ТекДанныеТовары.КСчетуФактуреКомиссионераДата = "";
			ТекДанныеТовары.ПредставлениеСчетаФактуры = "";
			ТекДанныеТовары.ПодборСчетовФактур = "";
...
Процедура ВозвратыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ТекущаяСтрока = Элементы.Возвраты.ТекущиеДанные;
	
	Если Не ТекущаяСтрока.ВыставленСФ Тогда
		Возврат;
	КонецЕсли;
	
	Если Поле.Имя = "ВозвратыПредставлениеСчетаФактуры" Тогда
	...
	ИначеЕсли Поле.Имя = "ВозвратыКСчетуФактуреКомиссионераНомер" Тогда
	...
	ИначеЕсли Поле.Имя = "ВозвратыПодборСчетовФактур" Тогда
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.СчетФактура) Тогда
		...
		Если Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура Тогда
			...
			БухгалтерскийУчетКлиентПереопределяемый.ОткрытьСчетФактуру(
				ЭтотОбъект, ТекущаяСтрока.СчетФактура, "СчетФактураВыданный");

		Иначе
			
			БухгалтерскийУчетКлиентПереопределяемый.ОткрытьСчетФактуру(
				ЭтотОбъект, ТекущаяСтрока.СчетФактура, "СчетФактураПолученный");
...
Процедура НачалоВыбораВозвратыПодборСчетовФактур(ТекущаяСтрокаВозвраты)
	...
	ФильтрПоКлючу = Новый Структура("КлючСтроки", ТекущаяСтрокаВозвраты.КлючСтроки);
...
&НаКлиенте
Процедура ВозвратыПокупательСоздание(Элемент, СтандартнаяОбработка)
...
Процедура ТоварыВозвращенныеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		НайденныеСтрокиТоварыВозвращенные =
			Объект.ТоварыВозвращенные.НайтиСтроки(Новый Структура("КлючСтроки", ДанныеСтрокиВозвраты.КлючСтроки));
		Если НайденныеСтрокиТоварыВозвращенные.Количество() > 0 Тогда 
			
			Если Не ДанныеСтрокиВозвраты.ВыставленСФ И
				НЕ ДанныеСтрокиТовары = Неопределено Тогда
				ДанныеСтрокиТовары.ДокументРеализации = 
				НайденныеСтрокиТоварыВозвращенные[0].ДокументРеализации;
			КонецЕсли;
...
Процедура ТоварыВозвращенныеНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|СуммаНДСДоИзменения, КоличествоПослеИзменения, СуммаПослеИзменения, СуммаНДСПослеИзменения,
		|СуммаВознаграждения, СуммаНДСВознаграждения,
		|СчетУчета, СчетДоходов, Субконто, СчетУчетаНДСПоРеализации, СчетРасходов,
...
Процедура ТоварыВозвращенныеКоличествоПриИзменении(Элемент)
	...
	РассчитатьПоказателиПослеИзменения(ДанныеСтроки);
...
Процедура ДенежныеСредстваПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Объект.ДокументБезНДС Тогда
		ДанныеСтроки = Элементы.ДенежныеСредства.ТекущиеДанные;
		ДанныеСтроки.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.БезНДС");
	КонецЕсли; 
...
&НаКлиенте
Процедура ДенежныеСредстваПокупательСоздание(Элемент, СтандартнаяОбработка)
...
Процедура ЗаполнитьТоварыПоРеализацииЗавершение(РезультатЗакрытия, Дополнительныепараметры) Экспорт
	...
	ЗаполнитьПоРеализации(РезультатЗакрытия, "Заполнить", 0);
...
Процедура ЗаполнитьВозвраты(Команда)
	...
	Если Объект.ТоварыВозвращенные.Количество() > 0 Тогда
	
		ТекстВопроса = НСтр("ru = 'При заполнении табличная часть будет очищена. Заполнить?'");
	
		Оповещение = Новый ОписаниеОповещения("ВозвратыОбработкаЗаполненияЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
		
	Иначе
	
		ЗаполнитьВозвратыПоДокументу();
...
&НаКлиенте
Процедура ВозвратыОбработкаЗаполненияЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьВозвратПоРеализацииЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ДанныеВозвраты <> Неопределено Тогда
		...
		ЗаполнитьПоДокументу(Результат, ДанныеВозвраты.Покупатель, ДанныеВозвраты.КлючСтроки);
		СформироватьНадписьЦеныИВалюта(ЭтаФорма);
...
Процедура ЗаполнитьРНПТ(Команда)
	...
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ЗаполнитьРНПТНаСервере(ТекущиеДанные.КлючСтроки);
		Модифицированность = Истина;
	Иначе
...
Процедура ВставитьСтроки(Команда)
	...
	Если ИмяТаблицы = "ТоварыВозвращенные" Тогда
		ИмяТаблицыПокупатели = "Возвраты";
	Иначе
		ИмяТаблицыПокупатели = "Покупатели";
	КонецЕсли; 
	
	ТекущиеДанные = Элементы[ИмяТаблицыПокупатели].ТекущиеДанные;
	
	Если ТекущиеДанные = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрШаблон(НСтр("ru = 'Не выбрана строка в списке %1'"), ИмяТаблицыПокупатели),
			,ИмяТаблицыПокупатели, "Объект");
		Возврат;
	КонецЕсли;
...
Функция ПолучитьИмяТекущейТабличнойЧасти()
	...
	Если ИмяТекущейСтраницы = "СтраницаВозвраты" Тогда
		Возврат "ТоварыВозвращенные";
	КонецЕсли;
	
	ИмяТаблицы = "";
	
	ИмяТекущейСтраницы = Элементы.ГруппаТоварыУслуги.ТекущаяСтраница.Имя;
...
Процедура ОпределитьСписокСвойствДляВставкиИзБуфера(ПараметрыВставки)
	...
	Если ПараметрыВставки.ИмяТаблицы = "Товары" ИЛИ ПараметрыВставки.ИмяТаблицы = "ТоварыВозвращенные" Тогда
		...
		Если ПараметрыВставки.ИмяТаблицы = "Товары" Тогда
			...
			СписокСвойств.Добавить("ЦенаПередачи");
			СписокСвойств.Добавить("СуммаПередачи");
...
#Область ИнтеграцияBnovo

&НаСервере
Процедура ПодготовитьКЗаполнениюПоДаннымBNovo(ТекущийОбъект)
	Если НЕ ТекущийОбъект.ЗагрузкаИзBnovo Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьПараметрыВыбораСтавкиНДС()
	...
	УчетНДС.УстановитьПараметрыВыбораСтавкиНДС(ЭтотОбъект, "ДенежныеСредстваСтавкаНДС", "Авансы");
...
Процедура УстановитьУсловноеОформлениеВидимость()
	...
	// ТоварыВозвращенныеСебестоимость
	// Поле себестоимость не заполняется, когда в таблице Возвраты ставится галочка ВыставленСФ
	// т.е.перевыставляется корректировочный счет-фактура
	// и поле ДокументРеализация заполняется автоматически, при записи документа
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВозвращенные.ПоТекущемуДокументу", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "УслугиСуммаНДСВознаграждения");
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВозвращенныеНДСВознаграждения");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВозвращенныеСчетУчетаНДСПоРеализации");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДенежныеСредстваСтавкаНДС");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДенежныеСредстваСуммаНДС");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПокупателиПолученныйСчетФактура");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПокупателиТоварыНомерСФ");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПокупателиПредставлениеСчетаФактуры");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВозвращенныеИсправляемыйДокумент");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПояснениеСчетФактура");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПояснениеКСчетамФактурам");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "СпособРегистрацииСчетФактуры");
	...
	УстановитьУсловноеОформлениеЗаявленияОВвозеТоваровПолученного();
...
&НаСервере
Процедура УстановитьУсловноеОформлениеЗаявленияОВвозеТоваровПолученного()

	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПокупателиПредставлениеЗаявленияОВвозеТоваровПолученное");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ПокупательИзЕАЭС", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ЗаявлениеОВвозеТоваровПолученное", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<Ввести>'"));
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ПокупателиПредставлениеЗаявленияОВвозеТоваровПолученное");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ПокупательИзЕАЭС", ВидСравненияКомпоновкиДанных.Равно, Ложь);
...
Процедура УстановитьУсловноеОформлениеКомиссияТовары() Экспорт
	...
	// ПокупателиПредставлениеСчетаФактуры
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ПредставлениеСчетаФактуры", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	...
	// ПокупателиТоварыНомерСФ и ПокупателиТоварыДатаСФ и ПокупателиПредставлениеСчетаФактуры 
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Покупатели.НомерСФ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура", ВидСравненияКомпоновкиДанных.Равно, Истина);
...
Процедура УстановитьУсловноеОформлениеВозвраты() Экспорт
	...
	// ВозвратыПодборСчетовФактур

	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПодборСчетовФактур");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПодборСчетовФактур", ВидСравненияКомпоновкиДанных.НеЗаполнено, "Выбор");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПоТекущемуДокументу", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПоТекущемуДокументу", ВидСравненияКомпоновкиДанных.Равно, Истина);

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НедоступныеДанныеЦвет);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<Создается при записи документа>'"));
	
	// ВозвратыПредставлениеСчетаФактуры

	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПредставлениеСчетаФактуры");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПредставлениеСчетаФактуры", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПояснениеСчетФактура", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Корр. счет-фактура'"));

	// ВозвратыПояснениеКСчетамФактурам
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ПояснениеКСчетамФактурам", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='К счету-фактуре (счетам-фактурам)'"));
	
	// ВозвратыНомерСФ, ВозвратыДатаСФ, ВозвратыПредставлениеСчетаФактуры, ВозвратыПодборСчетовФактур

	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыНомерСФ");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыДатаСФ");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыКСчетуФактуреКомиссионераНомер");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыКСчетуФактуреКомиссионераДата");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПредставлениеСчетаФактуры");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ВозвратыПодборСчетовФактур");
	...
	// ВозвратыКСчетуФактуреКомиссионераДата
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераНомер", ВидСравненияКомпоновкиДанных.Равно, НСтр("ru = 'Сводно'"));

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	// ВозвратыКСчетуФактуреКомиссионераНомер
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераДата", ВидСравненияКомпоновкиДанных.Заполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераНомер", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераДата", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераНомер", ВидСравненияКомпоновкиДанных.Заполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.ВыставленСФ", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Возвраты.КСчетуФактуреКомиссионераНомер", ВидСравненияКомпоновкиДанных.НеРавно, НСтр("ru = 'Сводно'"));
...
Процедура УправлениеФормой(Форма)
	...
	// Заголовок в закладке "Реализация" в табличной части "Покупатели"
	// поля отметки о регистрации заявления о ввозе, полученного от покупателя из ЕАЭС
	Элементы.ПокупателиПредставлениеЗаявленияОВвозеТоваровПолученное.Видимость = Форма.ЕстьКонтрагентыИзЕАЭС;
	...
	Если Объект.ДокументБезНДС Тогда
		...
		// Группа покупатели полученный от комиссионера
		Элементы.ПокупателиПолученныйСчетФактура.Видимость = Ложь;
		
		// Группа возвраты пояснение 
		Элементы.ВозвратыГруппаПояснение.Видимость         = Ложь;
		
		// Группа возвраты полученный и выданный от комиссионера
		Элементы.ВозвратыГруппаПолученныеОтКомиссионера.Видимость = Ложь;
		Элементы.ВозвратыВыставленныеКомиссионеру.Видимость = Ложь;
		
		// Группа выбора вида счета-фактуры при возврате
		Элементы.ВКнигеПокупокРегистрируется.Видимость      = Ложь;
	...
	Иначе
		...
		// Группа покупатели полученный от комиссионера
		Элементы.ПокупателиПолученныйСчетФактура.Видимость = Истина;
		
		// Группа возвраты пояснение 
		Элементы.ВозвратыГруппаПояснение.Видимость         = Истина;
		
		// Группа возвраты полученный и выданный от комиссионера
		Элементы.ВозвратыГруппаПолученныеОтКомиссионера.Видимость = Истина;
		Элементы.ВозвратыВыставленныеКомиссионеру.Видимость = Истина;

		// Группа выбора вида счета-фактуры при возврате
		Элементы.ВКнигеПокупокРегистрируется.Видимость      = Истина;
		
	КонецЕсли;
	
	Если Объект.ДокументБезНДС Тогда
		Элементы.ДенежныеСредстваСумма.Заголовок = НСтр("ru = 'Сумма (руб.)'");
	Иначе
		Элементы.ДенежныеСредстваСумма.Заголовок = НСтр("ru = 'Сумма с НДС (руб.)'");
	КонецЕсли; 
	...
	ЗаполнитьКолонкамиВозврат(Форма);
...
Процедура ЗаполнитьРассчитатьСуммы(Знач ВалютаДоИзменения, КурсДоИзменения, КратностьДоИзменения, ПерезаполнитьЦены = Ложь, ПересчитатьЦены = Ложь, ПересчитатьНДС = Ложь);
	...
	Если Объект.ДокументБезНДС Тогда 
		Для каждого Строка Из Объект.ДенежныеСредства Цикл
			Строка.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			Строка.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
				Строка.Сумма, Истина, УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(Строка.СтавкаНДС));
		КонецЦикла; 
	КонецЕсли;
...
Процедура ЗаполнитьРассчитатьСуммыВСтроке(Строка, ТаблицаЦенНоменклатуры, ВалютаПередИзменением, СтруктураКурса, ПерезаполнитьЦены, ПересчитатьЦены, ПересчитатьНДС, ЗначениеПустогоКоличества)
	...
	Иначе
		Если ПересчитатьЦены Тогда
			...
			Вознаграждение = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(Вознаграждение, ВалютаПередИзменением,
...
Процедура ТоварыВозвращенныеНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)
	...
	ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТоварыВозвращенные(ЭтаФорма, СтрокаТабличнойЧасти);
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьПоРеализации(Реализация, СпособЗаполнения = "Заполнить", МаксимальныйКлючСтроки)
	...
	ДокументОбъект.ЗаполнитьТовары(Реализация, МаксимальныйКлючСтроки);
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(ИмяТаблицы = "", ДобавленныеСтроки = Неопределено)
	...
	Если ПустаяСтрока(ИмяТаблицы) ИЛИ ИмяТаблицы = "ТоварыВозвращенные" Тогда
		Для Каждого СтрокаТаблицы Из ?(ЗначениеЗаполнено(ДобавленныеСтроки), ДобавленныеСтроки, Объект.ТоварыВозвращенные) Цикл
			ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТоварыВозвращенные(ЭтаФорма, СтрокаТаблицы);
		КонецЦикла;
	КонецЕсли;
	
	Если ПустаяСтрока(ИмяТаблицы) ИЛИ ИмяТаблицы = "Услуги" Тогда
		Для Каждого СтрокаТаблицы Из ?(ЗначениеЗаполнено(ДобавленныеСтроки), ДобавленныеСтроки, Объект.Услуги) Цикл
			ЗаполнитьДобавленныеКолонкиСтрокиТаблицыУслуги(СтрокаТаблицы);
		КонецЦикла;
	КонецЕсли;
	
	СписокСчетовФактурПокупатели 	= Объект.Покупатели.Выгрузить(, "СчетФактура").ВыгрузитьКолонку("СчетФактура");
	СписокСчетовФактурВозвраты 		= Объект.Возвраты.Выгрузить(, "СчетФактура").ВыгрузитьКолонку("СчетФактура");
	СписокСчетовФактур				= Новый Массив;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокСчетовФактур, СписокСчетовФактурПокупатели, Истина);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СписокСчетовФактур, СписокСчетовФактурВозвраты, Истина);
	ОбщегоНазначенияКлиентСервер.СвернутьМассив(СписокСчетовФактур);
		
	РеквизитыСчетовФактур = Документы.ОтчетКомиссионераОПродажах.ДанныеРеквизитовСчетовФактур(СписокСчетовФактур);
	...
	Если ПустаяСтрока(ИмяТаблицы) ИЛИ ИмяТаблицы = "Покупатели" Тогда
		...
		Для каждого СтрокаТаблицы Из Объект.Покупатели Цикл
			...
			РеквизитыСчетаФактуры = РеквизитыСчетовФактур[СтрокаТаблицы.СчетФактура];
			
			СтрокаТаблицы.ПредставлениеСчетаФактуры = 
				ОписаниеПеревыставленногоСчетаФактуры(СтрокаТаблицы.СчетФактура, РеквизитыСчетаФактуры);
	...
	Если ПустаяСтрока(ИмяТаблицы) ИЛИ ИмяТаблицы = "Возвраты" Тогда
		...
		ИсправляемыеДокументыПоКлючам = Объект.ТоварыВозвращенные.Выгрузить(,"КлючСтроки, ИсправляемыйДокумент");
		ИсправляемыеДокументыПоКлючам.Свернуть("КлючСтроки, ИсправляемыйДокумент");
		...
		Для каждого СтрокаТаблицы Из Объект.Возвраты Цикл
			...
			ЗаполнитьСтрокуПоСчетуФактуре(СтрокаТаблицы, РеквизитыСчетаФактуры);
...
&НаСервере
Процедура УстановитьЗавленияОВвозеТоваровПолученные()
	
	Запрос = Новый Запрос();
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	Запрос.Параметры.Вставить("ДокументыОтгрузкиКонтрагентов", Объект.Покупатели.Выгрузить(,"Счетфактура, Покупатель, КлючСтроки,"));
	...
	Для каждого Строка Из Объект.Покупатели Цикл
		Если ЗначениеЗаполнено(Строка.Счетфактура)
			И ДокументыОтгрузки.Найти(Строка.Счетфактура) = Неопределено Тогда
			ДокументыОтгрузки.Добавить(Строка.Счетфактура);
		КонецЕсли;
	КонецЦикла;
	
	Запрос.Параметры.Вставить("ДокументыОтгрузки", ДокументыОтгрузки);
	
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДокументыОтгрузкиКонтрагентов.Счетфактура КАК ДокументОтгрузки,
	|	ДокументыОтгрузкиКонтрагентов.Покупатель КАК Контрагент,
	|	ДокументыОтгрузкиКонтрагентов.КлючСтроки КАК КлючСтроки
	|ПОМЕСТИТЬ ДокументыОтгрузкиКонтрагентов
	|ИЗ
	|	&ДокументыОтгрузкиКонтрагентов КАК ДокументыОтгрузкиКонтрагентов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыОтгрузкиКонтрагентов.ДокументОтгрузки КАК ДокументОтгрузки,
	|	ЕСТЬNULL(Контрагенты.Ссылка, ДокументыОтгрузкиКонтрагентов.Контрагент) КАК Контрагент,
	|	ДокументыОтгрузкиКонтрагентов.КлючСтроки КАК КлючСтроки
	|ПОМЕСТИТЬ ДокументыОтгрузкиКонтрагентовЕАЭС
	|ИЗ
	|	ДокументыОтгрузкиКонтрагентов КАК ДокументыОтгрузкиКонтрагентов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
	|		ПО ДокументыОтгрузкиКонтрагентов.Контрагент = Контрагенты.Ссылка
	|ГДЕ
	|	Контрагенты.СтранаРегистрации В(&СтраныЕАЭС)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОтгрузки,
	|	Контрагент
	...
	|	ЗаявлениеОВвозеТоваровПолученное.Контрагент КАК Контрагент,
	|	ЗаявлениеОВвозеТоваровПолученное.Дата КАК Дата,
	|	ЗаявлениеОВвозеТоваровПолученное.НомерВходящегоДокумента КАК НомерВходящегоДокумента,
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗаявлениеОВвозе
	|ИЗ
	...
	|ГДЕ
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки В(&ДокументыОтгрузки)
	|	И НЕ ЗаявлениеОВвозеТоваровПолученное.ПометкаУдаления
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОтгрузки,
	|	Контрагент
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДокументыОтгрузкиКонтрагентов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ЗаявлениеОВвозе.Ссылка, ЗНАЧЕНИЕ(Документ.ЗаявлениеОВвозеТоваровПолученное.ПустаяСсылка)) КАК Ссылка,
	|	ЕСТЬNULL(ЗаявлениеОВвозе.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	|	ЕСТЬNULL(ЗаявлениеОВвозе.НомерВходящегоДокумента, """") КАК НомерВходящегоДокумента,
	|	ДокументыОтгрузкиКонтрагентовЕАЭС.КлючСтроки КАК ДокументОтгрузки,
	|	ДокументыОтгрузкиКонтрагентовЕАЭС.КлючСтроки КАК КлючСтроки
	|ИЗ
	|	ДокументыОтгрузкиКонтрагентовЕАЭС КАК ДокументыОтгрузкиКонтрагентовЕАЭС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗаявлениеОВвозе КАК ЗаявлениеОВвозе
	|		ПО ДокументыОтгрузкиКонтрагентовЕАЭС.ДокументОтгрузки = ЗаявлениеОВвозе.ДокументОтгрузки
	|			И ДокументыОтгрузкиКонтрагентовЕАЭС.Контрагент = ЗаявлениеОВвозе.Контрагент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗаявлениеОВвозе
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ДокументыОтгрузкиКонтрагентовЕАЭС";
	
	Результат = Запрос.Выполнить();
	
	ЕстьКонтрагентыИзЕАЭС = Ложь;
	...
	Пока Выборка.Следующий() Цикл
		...
		Отбор = Новый Структура("КлючСтроки", Выборка.КлючСтроки);
		РезутатПоиска = Объект.Покупатели.НайтиСтроки(Отбор);
		Если РезутатПоиска.Количество() > 0 Тогда
			
			ВыбраннаяСтрока = РезутатПоиска[0];
			ВыбраннаяСтрока.ПокупательИзЕАЭС = Истина; // Покупатель из ЕАЭС, но может быть без заявления о ввозе (полученного)
			ВыбраннаяСтрока.ЗаявлениеОВвозеТоваровПолученное = Выборка.Ссылка;
			
			Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
				ВыбраннаяСтрока.ПредставлениеЗаявленияОВвозеТоваровПолученное = СтрШаблон(НСтр("ru='%1 от %2'"),
								Выборка.НомерВходящегоДокумента,
								Формат(Выборка.Дата,"ДЛФ=Д"));
			Иначе
				ВыбраннаяСтрока.ПредставлениеЗаявленияОВвозеТоваровПолученное = "";
			КонецЕсли;
...
&НаСервере
Процедура УстановитьЗавлениеОВвозеТоваровПолученное(ПараметрыПокупателя)
	
	Запрос = Новый Запрос();
	Запрос.Параметры.Вставить("ДокументОтгрузки", ПараметрыПокупателя.Счетфактура);
	Запрос.Параметры.Вставить("Контрагент", ПараметрыПокупателя.Покупатель);
	...
	|	И НЕ ЗаявлениеОВвозеТоваровПолученное.ПометкаУдаления
	|	И ЗаявлениеОВвозеТоваровПолученное.Контрагент = &Контрагент";
	
	Результат = Запрос.Выполнить();
	
	Если Результат.Пустой() Тогда
		ПараметрыПокупателя.ЗаявлениеОВвозеТоваровПолученное = Документы.ЗаявлениеОВвозеТоваровПолученное.ПустаяСсылка();
		ПараметрыПокупателя.ПредставлениеЗаявленияОВвозеТоваровПолученное = "";
		Возврат;
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		...
		ПараметрыПокупателя.ЗаявлениеОВвозеТоваровПолученное = Выборка.Ссылка;
		ПараметрыПокупателя.ПредставлениеЗаявленияОВвозеТоваровПолученное = СтрШаблон(НСтр("ru='%1 от %2'"),
							Выборка.НомерВходящегоДокумента,
							Формат(Выборка.Дата,"ДЛФ=Д"));
...
&НаКлиенте
Процедура ВыборЗаявленияОВвозеТоваровПолученногоЗавершение(Результат, ДанныеСтрокиТабличнойЧасти) Экспорт
	
	Если ЗначениеЗаполнено(Результат) Тогда
		
		ДанныеСтрокиТабличнойЧасти.ЗаявлениеОВвозеТоваровПолученное = Результат.Ссылка;
		ДанныеСтрокиТабличнойЧасти.ПредставлениеЗаявленияОВвозеТоваровПолученное = Результат.Представление;
...
&НаКлиенте
Процедура ВыводитьЗаявлениеОВвозеТоваровПолученное()
	
	ЕстьКонтрагентыИзЕАЭС = Ложь;
	
	Для Каждого Строка Из Объект.Покупатели Цикл
		Если Строка.ПокупательИзЕАЭС Тогда
			ЕстьКонтрагентыИзЕАЭС = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТоварыВозвращенные(Форма, СтрокаТаблицы)
...
Процедура ДоговорКонтрагентаОбработатьИзменение()
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		...
		ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ОбщаяУСН
			И Объект.СтавкаНДСВознаграждения <> УчетНДСКлиентСервер.ОбщаяУСНСтавкаНДС(Объект.Дата) Тогда
			Объект.СтавкаНДСВознаграждения = УчетНДСКлиентСервер.ОбщаяУСНСтавкаНДС(Объект.Дата);
			СтавкаНДСИзменилась = Истина;
		ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН
			И Объект.СтавкаНДСВознаграждения <> УчетНДСКлиентСервер.ПониженнаяУСНСтавкаНДС(Объект.Дата) Тогда
			Объект.СтавкаНДСВознаграждения = УчетНДСКлиентСервер.ПониженнаяУСНСтавкаНДС(Объект.Дата);
			СтавкаНДСИзменилась = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаВыбораПодборВставкаИзБуфераНаСервере(ВыбранноеЗначение, ИмяТаблицы)
	...
	ЕстьКолонкаЦенаПередачи =  ТаблицаТоваров.Колонки.Найти("ЦенаПередачи") <> Неопределено;
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Иначе
			...
			Если ЭтоВставкаИзБуфера 
				...
				И СведенияОНоменклатуре.Услуга <> (ИмяТаблицы = "Услуги") Тогда
			...
			Если ИмяТаблицы = "Товары" 
				ИЛИ ИмяТаблицы = "ТоварыВозвращенные" Тогда
			...
			// Заполнение цены передачи возможно только для ТЧ "Товары", вставка из буфера обработана ранее
			Если ИмяТаблицы = "Товары" И НЕ ЭтоВставкаИзБуфера Тогда
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ВедетсяУчетЭкспортаТоваров           = ПолучитьФункциональнуюОпцию("ВедетсяУчетТаможенныхДекларацийЭкспорт");
...
Процедура ПриНачалеРедактированияСтрокиТоваровУслуг(ДанныеСтроки, ДанныеСтрокиПокупателя, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		ДанныеСтроки.СтавкаНДС = СтавкаНДСПриРеализации;
...
Процедура УдалитьСвязанныеЗаписиНаСервере()
	...
	ВидыУдаляемыхСтрок.Вставить("ВозвратыИсходныеДокументы", ТаблицаУдаленныхКлючейВозвраты);
	...
	ТаблицаУдаленныхКлючейВозвраты.Очистить();
...
&НаКлиенте
Процедура ОткрытьФормуВыбораРеализации(СпособЗаполнения = "СпособЗаполнения")
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗаполнитьТоварыПоРеализацииЗавершение", ЭтотОбъект, Новый Структура("СпособЗаполнения", СпособЗаполнения));	
...
&НаКлиенте
Процедура ЗаполнитьВозвратыПоДокументу()

	ПараметрыФормы = Новый Структура;
	
	ТекущаяСтрокаВозвраты = Элементы.Возвраты.ТекущиеДанные;
	
	Если ТекущаяСтрокаВозвраты = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ТекущаяСтрокаВозвраты.ВыставленСФ Тогда
		
		ПараметрыФормы.Вставить("СписокДокументов", СписокДокументовРеализации(ТекущаяСтрокаВозвраты.КлючСтроки));
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ЗаполнитьВозвратПоРеализацииЗавершение", ЭтотОбъект);	
	
	ОткрытьФорму("Документ.ОтчетКомиссионераОПродажах.ФормаВыбора",
	ПараметрыФормы, Элементы.ТоварыВозвращенные,,,, ОповещениеОЗакрытии);

КонецПроцедуры

// Процедура очищает видимое поле ПредставлениеСчетаФактуры в таблице Возвраты
// и если есть проведенный счет-фактура, отменяет проведение
//
&НаСервере
Процедура ОчиститьПолеСчетовФактурНаСервере()

	Для каждого ТекущаяСтрокаВозвраты Из Объект.Возвраты Цикл
		Если НЕ ТекущаяСтрокаВозвраты.ВыставленСФ Тогда
			Продолжить;
		КонецЕсли;
		
		ТекущаяСтрокаВозвраты.ПредставлениеСчетаФактуры = "";
		
		Если ЗначениеЗаполнено(ТекущаяСтрокаВозвраты.СчетФактура) Тогда
		
			ДокументОбъект = ТекущаяСтрокаВозвраты.СчетФактура.ПолучитьОбъект();
			Если ДокументОбъект.Проведен Тогда
				ДокументОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			КонецЕсли;
			
			ТекущаяСтрокаВозвраты.СчетФактура = Неопределено;
...
// Функция открывает форму списка счетов-фактур с установленным отбором
&НаКлиенте
Процедура ОткрытьСписокСчетовФактур(ВидСчетФактуры)

	ПараметрыОтбор = Новый Структура;
	ПараметрыОтбор.Вставить("Организация",       Объект.Организация);
	ПараметрыОтбор.Вставить("ДокументОснование", Объект.Ссылка);
	ПараметрыОтбор.Вставить("ВидСчетаФактуры",   ВидСчетФактуры);
...
&НаКлиенте
Процедура ОбновитьДокументыВТаблицеВозвратов()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		НайденныеСтроки = Объект.ТоварыВозвращенные.НайтиСтроки(Новый Структура("КлючСтроки", КлючСтроки));
		
		Для каждого ТекущаяСтрокаНайденныеСтроки Из НайденныеСтроки Цикл
		
			ТекущаяСтрокаНайденныеСтроки.ДокументРеализации = "";
			ТекущаяСтрокаНайденныеСтроки.ИсправляемыйДокумент = "";
			ТекущаяСтрокаНайденныеСтроки.ПоТекущемуДокументу = Ложь;
...
&НаКлиенте
Процедура ВозвратыОбработкаЗаполненияДокументаРеализацииЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт

	Если РезультатВыбора = КодВозвратаДиалога.Да Тогда
	
		ЗаполнитьИсправленныйДокументНаСервере();
...
&НаСервере
Процедура ЗаполнитьИсправленныйДокументНаСервере(ЗаполнятьТолькоНезаполненные = Ложь)

	Если НЕ Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура Тогда
		Возврат;
	КонецЕсли;
	
	ОпределитьСтрокиВозвратаПоТекущемуДокументу();
	
	ЗаполнитьПоказателиПоСчетамФактурамПоРанееВведеннымДокументам(ЗаполнятьТолькоНезаполненные);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИсправленныйДокументПоВсемСтрокам(СписокПокупателей, СписокИсходныхДокументов,
	ЗаполнятьТолькоНезаполненные)

	Если СписокИсходныхДокументов.Количество() = 0
		Или СписокПокупателей.Количество() = 0 Тогда
		
		Возврат;
		
	КонецЕсли;
	
	ДанныеОбъекта = Новый Структура("Дата,Ссылка,СчетаФактуры,Покупатели,ДоговорКонтрагента");
	ЗаполнитьЗначенияСвойств(ДанныеОбъекта, Объект);
	ДанныеОбъекта.СчетаФактуры = СписокИсходныхДокументов;
	ДанныеОбъекта.Покупатели = СписокПокупателей;
	ДанныеОбъекта.ДоговорКонтрагента = Объект.ДоговорКонтрагента;
		
	Документы.ОтчетКомиссионераОПродажах.ЗаполнитьПоказателиПоСчетамФактурам(
		Объект.Возвраты, Объект.ТоварыВозвращенные, ДанныеОбъекта, ЗаполнятьТолькоНезаполненные);
...
Процедура ОткрытьФормуПодбораСчетовФактур(ТекущаяСтрокаВозвраты)
	...
	ПараметрыФормы.Вставить("ДоговорКонтрагента" , Объект.ДоговорКонтрагента);
	ПараметрыФормы.Вставить("КлючСтроки"         , ТекущаяСтрокаВозвраты.КлючСтроки);
	ПараметрыФормы.Вставить("СчетаФактуры"       , Объект.ВозвратыИсходныеДокументы);
	ПараметрыФормы.Вставить("Возвраты"           , Объект.Возвраты);
	ПараметрыФормы.Вставить("Покупатели"         , Объект.Покупатели);
	
	ОткрытьФорму("Документ.ОтчетКомиссионераОПродажах.Форма.ФормаПодбораСчетовФактур", ПараметрыФормы, ЭтаФорма);
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьКолонкамиВозврат(Форма)

	СпособРегистрацииСчетФактуры = Форма.СпособРегистрацииСчетФактуры;
	Элементы = Форма.Элементы;
	Объект = Форма.Объект;
	
	ОтображатьВозврат = Объект.НаВозвратыВыставляетсяКорректировочныйСчетФактура;
	
	Элементы.ВозвратыГруппаПолученныеОтКомиссионера.ОтображатьВШапке = ОтображатьВозврат;
	Элементы.ВозвратыПредставлениеСчетаФактуры.ОтображатьВШапке = НЕ ОтображатьВозврат;
	Элементы.ВозвратыВыставленныеКомиссионеру.ОтображатьВШапке = ОтображатьВозврат;
	Элементы.ВозвратыПодборСчетовФактур.Видимость = ОтображатьВозврат;
	Элементы.ВозвратыЗаголовокСчетФактура.Заголовок = 
		?(ОтображатьВозврат, НСтр("ru = 'Корректировочный счет-фактура'"), НСтр("ru = 'СФ'"));
	
	Элементы.ВозвратыГруппаПояснение.Видимость = ОтображатьВозврат;
	Элементы.ВозвратыНомерСФ.Заголовок = ?(ОтображатьВозврат, НСтр("ru = 'Номер'"), НСтр("ru = 'Номер СФ'"));
	Элементы.ВозвратыДатаСФ.Заголовок = ?(ОтображатьВозврат, НСтр("ru = 'Дата'"), НСтр("ru = 'Дата СФ'"));
	Элементы.ВозвратыКСчетамФактурамКомиссионера.Видимость = ОтображатьВозврат;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ОписаниеПеревыставленногоСчетаФактуры(СчетФактура, РеквизитыСчетаФактуры)
	
	Возврат Документы.ОтчетКомиссионераОПродажах.ОписаниеСчетаФактуры(СчетФактура, РеквизитыСчетаФактуры);

КонецФункции

&НаСервере
Процедура ЗаполнитьСтрокуПоСчетуФактуре(СтрокаТаблицы, РеквизитыСчетаФактуры)
...
Функция ОписаниеИсправляемыхДокументов(Объект, ФильтрПоКлючу)
	...
	Возврат Документы.ОтчетКомиссионераОПродажах.ОписаниеИсправляемыхДокументов(НайденныеСтроки);
	
КонецФункции

&НаСервере
Процедура ОбработкаВыбораСчетаФактуры(ВыбранноеЗначение, ТекущаяСтрока)
	...
	ТекущиеДанные = Объект.Возвраты[ТекущаяСтрока];
	
	НайденныеСтроки = Объект.ВозвратыИсходныеДокументы.НайтиСтроки(
		Новый Структура("КлючСтроки", ТекущиеДанные.КлючСтроки));
...
// Функция проверяет наличие выбранного счета-фактуры в таблицах Покупателели и Возвраты
// 
// Параметры:
//    Покупатель - Справочник.Контрагенты - покупатель
//    СчетФактура - Документы.СчетФактураВыданный - искомый счет-фактура
//
// Возвращаемое значение:
//    Булево - если найден счет-фактура, то истина
&НаСервере
Функция ЕстьВыбранныйСчетФактураВТекущемДокументе(Покупатель, СчетФактура)
	...
	НайденныеСтрокиПоВозвратам = Объект.Возвраты.НайтиСтроки(Фильтр);
...
&НаСервере
Процедура УдалитьИсходныеСчетФактурыПоКлючу(КлючСтроки)
	...
	Для каждого ТекущаяСтрока Из НайденныеСтроки Цикл
		...
		ТекущаяСтрока.ИсправляемыйДокумент = Документы.СчетФактураВыданный.ПустаяСсылка();
		ТекущаяСтрока.КоличествоПослеИзменения = 0;
		ТекущаяСтрока.СуммаПослеИзменения = 0;
		ТекущаяСтрока.СуммаНДСПослеИзменения = 0;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбновитьТекущуюСтрокуВозвраты(Форма, ТекущаяСтрока = Неопределено)
	...
	ФильтрПоКлючу = Новый Структура("КлючСтроки", ТекущиеДанные.КлючСтроки);
...
Процедура ПокупателиПредставлениеСчетаФактурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ВыбраннаяСтрока = Элементы.Покупатели.ТекущаяСтрока;
	
	Если ЗначениеЗаполнено(ВыбраннаяСтрока.СчетФактура) Тогда
	
		БухгалтерскийУчетКлиентПереопределяемый.ОткрытьСчетФактуру(ЭтаФорма,
			ВыбраннаяСтрока.СчетФактура, "СчетФактураВыданный");
...
&НаКлиенте
Процедура ВозвратыПредставлениеСчетаФактурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВозвратыКСчетуФактуреКомиссионераНомерПриИзменении(Элемент)
	
	НайтиПеревыставленныйИсходныйДокумент();
	ОбновитьТаблицуИсходныхДокументов();
	ОбновитьТекущуюСтрокуВозвраты(ЭтотОбъект);
...
&НаКлиенте
Процедура ВозвратыКСчетуФактуреКомиссионераДатаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОбновитьТаблицуИсходныхДокументов()
	...
	ТекущаяСтрокаИсходныхДокументов.КлючСтроки = ТекущиеДанные.КлючСтроки;
...
&НаКлиенте
Процедура НайтиПеревыставленныйИсходныйДокумент()
	...
	ПараметрыКомиссионера = Новый Структура("Покупатель,Дата,Номер,Организация,ДатаДокумента,ДоговорКонтрагента",
		...
		ТекущиеДанные.КСчетуФактуреКомиссионераНомер, Объект.Организация, Объект.Дата, Объект.ДоговорКонтрагента);
...
Процедура РассчитатьПоказателиПослеИзменения(СтрокаТаблицы)
	...
	ДанныеСтроки = Элементы.Возвраты.ТекущиеДанные;
	
	Если ДанныеСтроки <> Неопределено И НЕ ДанныеСтроки.ВыставленСФ Тогда	

		СтрокаТаблицы.КоличествоПослеИзменения = 0;
		СтрокаТаблицы.СуммаПослеИзменения      = 0;
		СтрокаТаблицы.СуммаНДСПослеИзменения   = 0;
...
&НаКлиенте
Процедура ВозвратыВыставленСФПриИзменении(Элемент)
	...
	Иначе
		...
		УдалитьИсходныеСчетФактурыПоКлючу(ТекущиеДанные.КлючСтроки);
...
&НаКлиенте
Процедура ВозвратыПодборСчетовФактурНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВозвратыПриАктивизацииПоля(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ВозвратыПодборСчетовФактур" Тогда
		...
		Если ТекущиеДанные.ВыставленСФ Тогда
			...
			Элементы.Возвраты.ТекущийЭлемент = Элементы.ВозвратыКСчетуФактуреКомиссионераДата;
...
&НаКлиенте
Процедура ТоварыВозвращенныеИсправляемыйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Организация", Объект.Организация);
	ПараметрыФормы.Вставить("Контрагент", ТекущаяСтрокаВозвраты.Покупатель);
	ПараметрыФормы.Вставить("СписокСчетовФактур", СписокСчетовФактур(ТекущаяСтрокаВозвраты.КлючСтроки));
	ПараметрыФормы.Вставить("КлючНазначенияИспользования", "ТаблицаТоварыВозвращенные");
...
&НаСервере
Функция СписокСчетовФактур(КлючСтроки)

	НайденныеСтроки = Объект.ВозвратыИсходныеДокументы.НайтиСтроки(Новый Структура("КлючСтроки", КлючСтроки));
	
	Список = Новый СписокЗначений();

	Для каждого ТекущаяСтрокаМассиваСтрок Из НайденныеСтроки Цикл
	
		Список.Добавить(ТекущаяСтрокаМассиваСтрок.ИсходныйДокумент);
...
&НаКлиенте
Процедура ТоварыВозвращенныеДокументРеализацииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущаяСтрокаВозвраты.ВыставленСФ Тогда
		...
		ОткрытьФорму("Документ.ОтчетКомиссионераОПродажах.Форма.ФормаВыбора", ПараметрыФормы, ЭтотОбъект);
...
&НаСервере
Функция СписокДокументовРеализации(КлючСтроки)
	...
	Для каждого ТекущаяСтрокаНайденныеСтроки Из НайденныеСтроки Цикл
		...
		МассивСчетовФактур.Добавить(ТекущаяСтрокаНайденныеСтроки.ИсходныйДокумент);
		
	КонецЦикла;

	СписокРеализаций = ПолучитьИсходныйДокументРеализации(МассивСчетовФактур, Объект.Организация);
...
&НаСервереБезКонтекста
Функция ПолучитьИсходныйДокументРеализации(МассивСчетовФактур, Организация)
	...
	ТаблицаРеализаций = Результат.Выгрузить();
	
	Если ТаблицаРеализаций.Количество() > 0 Тогда
		
		Возврат ТаблицаРеализаций.ВыгрузитьКолонку("Ссылка");
...
// Процедура выполняет запись счетов-фактур, у которых установлен признак ПоТекущемуДокументу
// для перезаполнения документов оснований и исходных документов
&НаСервере
Процедура ОбновитьСчетаФактуры()
	
	ТаблицаСчетовФактурПоВозвратам = Объект.ВозвратыИсходныеДокументы.Выгрузить(
		Новый Структура("ПоТекущемуДокументу", Истина));
	
	Если ТаблицаСчетовФактурПоВозвратам.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаСчетовФактурПоВозвратам.Свернуть("КлючСтроки");
	ФильтрПоКлючам = Новый Структура("КлючСтроки");
	
	Для каждого ТекущаяСтрока Из ТаблицаСчетовФактурПоВозвратам Цикл
		
		ФильтрПоКлючам.КлючСтроки = ТекущаяСтрока.КлючСтроки;
		
		НайденныеСтроки = Объект.Возвраты.НайтиСтроки(ФильтрПоКлючам);
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			Для каждого ТекущийВозврат Из НайденныеСтроки Цикл
				
				ДокументСФ = ТекущийВозврат.СчетФактура.ПолучитьОбъект();
				ДокументСФ.Записать();
...
// Заполняет исправленный документ по ранее введенным отчетам комиссионера
//
&НаСервере
Процедура ЗаполнитьПоказателиПоСчетамФактурамПоРанееВведеннымДокументам(ЗаполнятьТолькоНезаполненные)
	
	ТаблицаИсходныеДокументы = Объект.ВозвратыИсходныеДокументы.Выгрузить(
		Новый Структура("ПоТекущемуДокументу", Ложь));
	ТаблицаПокупатели = Объект.Возвраты.Выгрузить(,"Покупатель");
	
	ЗаполнитьИсправленныйДокументПоВсемСтрокам(
		ТаблицаПокупатели.ВыгрузитьКолонку("Покупатель"),
		ТаблицаИсходныеДокументы.ВыгрузитьКолонку("ИсходныйДокумент"),
		ЗаполнятьТолькоНезаполненные);
	
КонецПроцедуры

// В этой процедуре отбираются текущие реализации и возвраты
// Строки возврата, по которым есть реализации в текущем документе помечаются истиной в реквизите ПоТекущемуДокументу
//
&НаСервере
Процедура ОпределитьСтрокиВозвратаПоТекущемуДокументу()
	
	ТаблицаИсходныеДокументы = Объект.ВозвратыИсходныеДокументы.Выгрузить(
		Новый Структура("ПоТекущемуДокументу", Истина));
	ТаблицаИсходныеДокументы.Сортировать("ДатаВходящегоИсходногоДокумента");
	
	Если ТаблицаИсходныеДокументы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
		
	СписокНоменклатурыВДокументе = Новый СписокЗначений;
	
	ФильтрПоСчетфактуреКомиссионера = Новый Структура("ДатаСФ,НомерСФ");
	ФильтрПоСчетфактуреКомиссионераПоЗначению = Новый Структура("СчетФактура");
	
	ИменаТаблиц = Новый Массив;
	ИменаТаблиц.Добавить("Покупатели");
	ИменаТаблиц.Добавить("Возвраты");
	
	Для каждого ТекущаяСтрока Из ТаблицаИсходныеДокументы Цикл
		
		КлючСтрокиВозврата = ТекущаяСтрока.КлючСтроки;
		
		СписокНоменклатурыВДокументе.Очистить();

		// Если исходный документ уже создан и заполнен, то ищем по нему
		Если ЗначениеЗаполнено(ТекущаяСтрока.ИсходныйДокумент) Тогда
			
			ФильтрПоСчетфактуреКомиссионераПоЗначению.СчетФактура = ТекущаяСтрока.ИсходныйДокумент;
			
			ДобавитьНоменклатуруПоНайденномуСчетуФактуре(ИменаТаблиц,
				ФильтрПоСчетфактуреКомиссионераПоЗначению, СписокНоменклатурыВДокументе);
			
		// Иначе ищем по номеру и дате счет-фактуры комиссионера
		Иначе
			
			ФильтрПоСчетфактуреКомиссионера.НомерСФ = ТекущаяСтрока.НомерВходящегоИсходногоДокумента;
			ФильтрПоСчетфактуреКомиссионера.ДатаСФ = ТекущаяСтрока.ДатаВходящегоИсходногоДокумента;
		
			Если Не ЗначениеЗаполнено(ФильтрПоСчетфактуреКомиссионера.НомерСФ)
				ИЛИ Не ЗначениеЗаполнено(ФильтрПоСчетфактуреКомиссионера.ДатаСФ) Тогда
				Продолжить;
			КонецЕсли;
		
			ДобавитьНоменклатуруПоНайденномуСчетуФактуре(ИменаТаблиц,
				ФильтрПоСчетфактуреКомиссионера, СписокНоменклатурыВДокументе);
			
		КонецЕсли;
		
		НайденныеСтрокиПоКлючуВозврата = 
		Объект.ТоварыВозвращенные.НайтиСтроки(Новый Структура ("КлючСтроки", КлючСтрокиВозврата));
		
		Для каждого ТекущаяСтрокаПоКлючуВозврата Из НайденныеСтрокиПоКлючуВозврата Цикл
			
			Номенклатура = ТекущаяСтрокаПоКлючуВозврата.Номенклатура; 
			Если СписокНоменклатурыВДокументе.НайтиПоЗначению(Номенклатура) <> Неопределено Тогда
				
				ТекущаяСтрокаПоКлючуВозврата.ПоТекущемуДокументу = Истина;
				
				ТекущаяСтрокаПоКлючуВозврата.ДокументРеализации = 
				Документы.ОтчетКомиссионераОПродажах.ПустаяСсылка();
				ТекущаяСтрокаПоКлючуВозврата.ИсправляемыйДокумент = 
				Документы.СчетФактураВыданный.ПустаяСсылка();
				
				ТекущаяСтрокаПоКлючуВозврата.КоличествоПослеИзменения = 0;
				ТекущаяСтрокаПоКлючуВозврата.СуммаНДСПослеИзменения = 0;
				ТекущаяСтрокаПоКлючуВозврата.СуммаПослеИзменения = 0;
...
&НаСервере
Процедура ДобавитьНоменклатуруПоНайденномуСчетуФактуре(ИменаТаблиц, ФильтрПоСчетфактуреКомиссионера, СписокНоменклатурыВДокументе)
	
	Для каждого ИмяТаблицы Из ИменаТаблиц Цикл
		
		НайденныеСтроки = Объект[ИмяТаблицы].НайтиСтроки(ФильтрПоСчетфактуреКомиссионера);
		
		Если НайденныеСтроки.Количество() > 0 Тогда
			
			ДобавитьНомеклатуруПоНайденнымСтрокам(СписокНоменклатурыВДокументе, НайденныеСтроки, ИмяТаблицы);
...
Функция НайтиСчетФактуру(ПараметрыКомиссионера)
	...
	Результат.ПоТекущемуДокументу = НайденСчетФактураВТекущемДокументе(ПараметрыКомиссионера, Объект.Покупатели, Объект.Возвраты);
	
	Возврат Результат;
	
КонецФункции

// Процедура ищет по КлючуСтроки строки из подчиненной таблицы (Товары, ТоварыВозвращенные)
// и добавляет в массив номенклатуру из найденных строк
//
&НаСервере
Процедура ДобавитьНомеклатуруПоНайденнымСтрокам(СписокНоменклатурыВДокументе, НайденныеСтроки, ИмяТаблицы)
	
	ПодчиненныеТабличныеЧасти = Новый Соответствие();
	ПодчиненныеТабличныеЧасти.Вставить("Покупатели", "Товары");
	ПодчиненныеТабличныеЧасти.Вставить("Возвраты", "ТоварыВозвращенные");
	
	НайденныеСтрокиНоменклатура = 
		Объект[ПодчиненныеТабличныеЧасти.Получить(ИмяТаблицы)].НайтиСтроки(
			Новый Структура("КлючСтроки", НайденныеСтроки[0].КлючСтроки));
	
	Для каждого ТекущаяСтрока Из НайденныеСтрокиНоменклатура Цикл
		
		СписокНоменклатурыВДокументе.Добавить(ТекущаяСтрока.Номенклатура);
...
Функция НайденСчетФактураВТекущемДокументе(ПараметрыОперации, Покупатели, Возвраты)
	...
	Возврат Документы.ОтчетКомиссионераОПродажах.ЕстьСчетФактураВТекущемДокументе(
		ПараметрыОперации, Покупатели, Возвраты, Истина);
...
&НаСервере
Процедура ЗаполнитьРНПТНаСервере(КлючСтроки)
	
	ДанныеЗаполнения = Объект.Товары.НайтиСтроки(Новый Структура("КлючСтроки", КлючСтроки));
	...
	|	И ОтчетКомиссионераОПродажахТовары.КлючСтроки = &КлючСтроки
...
Процедура ОбновитьПризнакПрослеживаемостиПоСтроке(СтрокаТабличнойЧасти, ДатаДокумента)
	...
	Если ВедетсяУчетПрослеживаемыхТоваровПоНоменклатуре Тогда
		РеквизитыПрослеживаемости = Справочники.Номенклатура.ПризнакПрослеживаемостиНаДату(
			СтрокаТабличнойЧасти.Номенклатура, ДатаДокумента);
		Если РеквизитыПрослеживаемости <> Неопределено Тогда
			СтрокаТабличнойЧасти.ПрослеживаемыйТовар    = РеквизитыПрослеживаемости.ПрослеживаемыйТовар
				Или РеквизитыПрослеживаемости.ПрослеживаемыйКомплект;
...
Процедура ПриИзмененииВидаОперацииНаСервере()
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		НоваяСтрокаПокупатели.Покупатель 	= Объект.Контрагент;
		...
		КонецЕсли;
		Документы.ОтчетКомиссионераОПродажах.ЗаполнитьСуммыВСтрокеПриИзмененииВидаОперации(ЭтотОбъект, "Товары", НоваяСтрокаПокупатели, Истина);
	КонецЕсли;
	
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОРозничныхПродажах Тогда
		Если Объект.ТоварыВозвращенные.Количество()>0 Тогда
			Объект.Возвраты.Очистить();
			НоваяСтрокаВозвраты 				= Объект.Возвраты.Добавить();
			НоваяСтрокаВозвраты.Покупатель 		= Объект.Контрагент;
			Документы.ОтчетКомиссионераОПродажах.ЗаполнитьСуммыВСтрокеПриИзмененииВидаОперации(ЭтотОбъект,"ТоварыВозвращенные", НоваяСтрокаВозвраты, Истина);
		КонецЕсли;
	КонецЕсли;
