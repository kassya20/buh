...
Процедура ПоместитьДанныеДляЗаполненияВДанныеДокумента(ДанныеДокумента, ДанныеДляЗаполнения) Экспорт
	...
	ДанныеДокумента.Взносы.Очистить();
...
Функция АдресПараметровВХранилище(Объект, ДополнительныеПараметры) Экспорт
	...
	ПараметрыРасчетаОблагаемойБазы = Новый Структура;
	ПараметрыРасчетаОблагаемойБазы.Вставить("СериализованныйОбъект", ЗарплатаКадры.СериализоватьОбъектВДвоичныеДанные(Объект));
	ПараметрыРасчетаОблагаемойБазы.Вставить("Сотрудник", Объект.Сотрудник);
	ПараметрыРасчетаОблагаемойБазы.Вставить("Организация", Объект.Организация);
	ПараметрыРасчетаОблагаемойБазы.Вставить("ФизическоеЛицо", Объект.ФизическоеЛицо);
	ПараметрыРасчетаОблагаемойБазы.Вставить("МесяцНачисления", Объект.ПериодРегистрации);
	ПараметрыРасчетаОблагаемойБазы.Вставить("КорректироватьРасчет", Объект.КорректироватьВзносы);
	
	Возврат ПоместитьВоВременноеХранилище(ПараметрыРасчетаОблагаемойБазы, ДополнительныеПараметры.УникальныйИдентификатор);
...
Процедура ПересчитатьВзносы(Объект, ОбъектФормы) Экспорт
	...
	Если Объект.КорректироватьВзносы Тогда
		// Не пересчитываем, т.к. взносы в режиме корректировки
		Возврат;
	КонецЕсли; 
	
	СтруктураПараметров = ПараметрыДляПолученияДанныхЗаполнения();
	...
	ДанныеДляЗаполненияИзВременногоХранилищаВДанныеДокумента(Объект, АдресХранилища);
