...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Налог) Тогда
		...
		Элементы.НадписьОткрытьСверкуНалога.Заголовок = СтрШаблон(
			...
			ВыполнениеЗадачБухгалтера.ПредставлениеНалогаРодительныйПадеж(
...
Процедура ЗагрузитьРасчет(Команда)
	...
	СверкаСФНСПоИмущественнымНалогамФормыКлиент.НачатьЗагрузкуРасчетаНалогаИзФайла(ЭтотОбъект, ОтборНалог,, ОповещениеПослеЗагрузкиФайла);
КонецПроцедуры
