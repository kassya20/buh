...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Налог") Тогда
	...
	ПараметрыСверки.Цвета.Вставить("ЦветРасхождений", ЦветаСтиля.ВажноеСобытие);
...
Процедура ОК(Команда)
	...
	Если ПодключенОбработчикОжидания Тогда
		// Не ждем - выполняем сверку расчетов здесь
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Параметры.Свойство("НаименованиеОбъекта") И ЗначениеЗаполнено(Параметры.НаименованиеОбъекта) Тогда
		ТекстНаименования.Добавить(СокрЛП(Параметры.НаименованиеОбъекта));
	КонецЕсли;
	
	Если ЭтоТранспортныйНалог Тогда
		Если Параметры.Свойство("РегистрационныйЗнак") И ЗначениеЗаполнено(Параметры.РегистрационныйЗнак) Тогда
			ТекстНаименования.Добавить(СокрЛП(Параметры.РегистрационныйЗнак));
		КонецЕсли;
	ИначеЕсли ЭтоЗемельныйНалог Или ЭтоНалогНаИмущество Тогда
		Если Параметры.Свойство("КадастровыйНомер") И ЗначениеЗаполнено(Параметры.КадастровыйНомер) Тогда
			ТекстНаименования.Добавить(СтрШаблон(Нстр("ru='кадастровый номер %1'"),
				СокрЛП(Параметры.КадастровыйНомер)));
		КонецЕсли;
	КонецЕсли;
	
	Если Параметры.Свойство("КодПоОКТМО") И ЗначениеЗаполнено(Параметры.КодПоОКТМО) Тогда
		ТекстНаименования.Добавить(СтрШаблон(Нстр("ru='ОКТМО %1'"),
			СокрЛП(Параметры.КодПоОКТМО)));
	КонецЕсли;
	
	Если Параметры.Свойство("КодНалоговогоОргана") И ЗначениеЗаполнено(Параметры.КодНалоговогоОргана) Тогда
...
Функция СкопироватьЭлементФормы(ЭлементИсточник, Родитель, ИмяНовогоЭлемента, ИндексСтрокиТаблицы)
	...
	Если ТипИсточника = Тип("ПолеФормы") Тогда
		...
		// Cумма налога и сумма льгот выводятся в последней строке в целом по объекту, индекса на конце там нет,
...
Процедура ПодсветитьЭлементыРасчетаСРасхождениями(Форма, СторонаРасчета)
	...
	ПодсветитьРасхождения = 
		(СторонаРасчета = "ФНС" И ПравильныйРасчет = 2) // нужно подстветить расхождения в расчете ФНС
		Или (СторонаРасчета = "ВПрограмме" И ПравильныйРасчет = 3) // нужно подсветить расхождения в расчете программы
		Или ПравильныйРасчет = 0; // нужно подстветить расхождения в обоих расчетах
	...
	Для каждого СтрокаРасчета Из Форма.РасчетФНС Цикл
		...
		Если Форма.ЭтоТранспортныйНалог Тогда
			СуммаНалогаФНС = СуммаНалогаФНС
			                 ...
			                 * СтрокаРасчета.КоличествоМесяцевВладения / 12 * СтрокаРасчета.ПовышающийКоэффициент);
		ИначеЕсли Форма.ЭтоНалогНаИмущество Тогда
			СуммаНалогаФНС = СуммаНалогаФНС
			                 ...
			                 * СтрокаРасчета.НалоговаяСтавка / 100 * СтрокаРасчета.КоличествоМесяцевВладения / 12);
		ИначеЕсли Форма.ЭтоЗемельныйНалог И Форма.НалоговыйПериод < '20220101' Тогда
			СуммаНалогаФНС = СуммаНалогаФНС +
			                 Окр((СтрокаРасчета.НалоговаяБаза * СтрокаРасчета.ДоляВПравеЧислитель
			                 / ?(СтрокаРасчета.ДоляВПравеЗнаменатель = 0, 1, СтрокаРасчета.ДоляВПравеЗнаменатель) - СтрокаРасчета.НалоговыйВычет)
			                 * СтрокаРасчета.НалоговаяСтавка / 100 * СтрокаРасчета.КоличествоМесяцевВладения / 12 * СтрокаРасчета.ПовышающийКоэффициент);
...
&НаКлиенте
Процедура СравнитьРасчетыИПодсветитьРасхождения() Экспорт
