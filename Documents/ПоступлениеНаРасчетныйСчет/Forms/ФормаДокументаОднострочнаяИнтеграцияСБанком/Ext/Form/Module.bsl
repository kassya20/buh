...
Процедура ВидОперацииПриИзмененииСервер()
	...
	ВсяСуммаЯвляетсяДоходом = Истина;
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		СчетВРежимеИнтеграции = ИнтеграцияСБанкамиПовтИсп.ИнтеграцияВключена(Объект.СчетОрганизации);
	КонецЕсли;
...
Процедура ПроверитьТолькоОдинСчетВрежимеИнтеграции()
	...
	СловаПредупреждения.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Настройки'"),,,,
		"e1cib/command/Обработка.ПанелиПростойИнтерфейс.Команда.НастройкиБазовый"));
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ПредупреждениеОСчетахВДругихБанках);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаШапка);
	
	Элементы.ГруппаВидОперацииОблагаетсяНалогом.Объединенная = Истина;
	Элементы.ГруппаВидОперацииОблагаетсяНалогом.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	Элементы.ДатаВходящегоДокумента.Заголовок = НСтр("ru = 'По документу от'");
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаПрочее(Элементы,
		Элементы.ГруппаАналитика);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаПрочее(Элементы, Элементы.ГруппаСтатьяДДС);
	...
	Элементы.ВалютаДокумента.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
	...
	Исключения = "Дата,СчетБанк,СуммаУслуг,ПлатежныеКартыСуммаУслуг";
...
&НаКлиенте
Процедура ВсяСуммаЯвляетсяДоходомПриИзменении(Элемент)
	
	Модифицированность = Истина;
	
	Если ВсяСуммаЯвляетсяДоходом Тогда
		Объект.Графа5_УСН = Объект.Графа4_УСН;
	Иначе
		Объект.Графа5_УСН = 0;
	КонецЕсли;
	
	Элементы.ВозвратЗаймаСуммаПроцентов.Видимость = Не ВсяСуммаЯвляетсяДоходом;
