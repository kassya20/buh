...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьПорядокОтраженияАвансаВРасшифровкеПлатежаОтложенно");
	Методы.Вставить("ЗаменитьКонтрагентаНаФизическоеЛицо");
	Методы.Вставить("ОбработатьРучнуюНастройкуУСНПрочиеПоступленияОтложенно");
	Методы.Вставить("ОбработатьКредитыИЗаймы");
	Методы.Вставить("ЗаполнитьОтражениеОплатПокупателяВАУСН");
	Методы.Вставить("ОтражениеОплатыПокупателяВАУСНЗаполнено");
	Методы.Вставить("ПеренестиУслугиНПДВТабличнуюЧасть");
	Методы.Вставить("ЗаполнитьРеквизитыНастройкиУСНДляАктуальныхДокументов");
	Методы.Вставить("ЗаполнитьДоходыУСНПрочиеРасчетыВРасшифровкеПлатежаОтложенно");
	Методы.Вставить("ОбработатьРучнуюНастройкуУСНПрочиеРасчетыОтложенно");
...
// Определяет, требуется ли оформлять кассовый чек по поступлению от указанного плательщика
// с указанного банковского счета.
//
// Параметры:
//  Плательщик - СправочникСсылка.Контрагенты - контрагент, от которого получены средства.
//  СчетПлательщика - СправочникСсылка.БанковскиеСчета - банковский счет контрагента, с которого перечислены средства.
//
// Возвращаемое значение:
//  Булево - если Истина, по платежу требуется оформлять кассовый чек.
//
Функция ТребуетсяОформитьКассовыйЧек(Плательщик, СчетПлательщика) Экспорт
	
	Если ТипЗнч(Плательщик) <> Тип("СправочникСсылка.Контрагенты") Тогда
		// В качестве плательщика указан не контрагент, а физическое лицо. Требуется чек.
		Возврат Истина;
	КонецЕсли;
	
	ТипКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Плательщик, "ЮридическоеФизическоеЛицо");
	...
	Если Не ЗначениеЗаполнено(СчетПлательщика) Тогда
		// Плательщик - физлицо, счет не указан - это платеж от физлица по СБП, QR-коду или онлайн-платеж.
		Возврат Истина;
	КонецЕсли;
	
	// Проверим тип счета плательщика по его номеру.
	
	НомерСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетПлательщика, "НомерСчета");
	БалансовыйСчет = БанковскиеПравила.БалансовыйСчет(НомерСчета);
	
	Возврат БанковскиеПравила.ЭтоСчетПереводовФизическимЛицам(БалансовыйСчет);
...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "ПодразделениеКт", "ПодразделениеБанковскогоСчета");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "БанковскийСчет", "СчетКонтрагента");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеЗапрещено(Правила, "БанковскийСчетЗаполнен");
	
	// - перевод с цифрового кошелька
	СчетаУчетаВДокументах.ДобавитьПравилоЗаполнения(Правила, "РасшифровкаПлатежа", "СчетУчетаРасчетовСКонтрагентом", "БезналичныеДеньгиЦифровые");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ПереводСДругогоСчета");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "БанковскийСчет", "СчетКонтрагента");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ЦифровойСчетКор");
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ПереводСДругогоСчета");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеЗапрещено(Правила, "ЦифровойСчетКор");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеЗапрещено(Правила, "ИспользоватьПереводыВПути");
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "Дата", "Дата");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "Организация", "Организация");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "Контрагент", "Контрагент");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "ДоговорКонтрагента", "ДоговорКонтрагента");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "НазначениеПлатежа", "НазначениеПлатежа");
	...
	// уплата налога
	СчетаУчетаВДокументах.ДобавитьПравилоЗаполнения(Правила, "", "СчетУчетаРасчетовСКонтрагентом", "ВозвратУплатыНалога");
	СчетаУчетаВДокументах.ДобавитьВПравилоУсловиеРазрешено(Правила, "ВозвратНалога");
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "Налог", "Налог");
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "СубконтоКт1", "СубконтоВозвратНалогов1");
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "СубконтоКт2", "СубконтоВозвратНалогов2");
	СчетаУчетаВДокументах.ДобавитьПодчиненноеПравилоЗаполнения(Правила, "СубконтоКт3", "СубконтоВозвратНалогов3");
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоОписаниеДополнительныхДанных(Правила, "ЦифровойСчетКор", "СчетКонтрагента");
...
Процедура ДополнитьДанныеЗаполненияСчетовУчета(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("ЦифровойСчетКор") Тогда
		Если ЗначениеЗаполнено(ДанныеЗаполнения.СчетКонтрагента) Тогда
			ДанныеЗаполнения.ЦифровойСчетКор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				ДанныеЗаполнения.СчетКонтрагента, "ЦифровойСчет");
		КонецЕсли;
		
		Если ДанныеЗаполнения.ЦифровойСчетКор = Неопределено Тогда
			ДанныеЗаполнения.ЦифровойСчетКор = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ВидыОперацийСчетаУчета() Экспорт
	...
	ВидыОпераций.Добавить("ВозвратНалога");
...
Функция ТекстЗапросаДанныеДляОбновленияЦенДокументов() Экспорт
	...
	|ИЗ
	|	Документ.ПоступлениеНаРасчетныйСчет.РасшифровкаПлатежа КАК ТаблицаНоменклатуры
	|ГДЕ
...
Функция ПараметрыФормированияЧекаНПД(ДокументОснование, Организация) Экспорт
	...
	|ИЗ
	|	Документ.ПоступлениеНаРасчетныйСчет.РасшифровкаПлатежа КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &ДокументОснование
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПодготовитьСообщенияПроверкиЗаполненияРасшифровкаПлатежа(Объект, ФормаРасшифровкаПлатежа = Неопределено, Отказ, ПроверяемыеРеквизиты, ПроверятьСчетаУчета = Истина) Экспорт
	...
	Если ЕстьРасшифровкаПлатежа(ВидОперации) Тогда
		...
		ЕстьАвансыУСН            = УчетДенежныхСредствКлиентСервер.ЕстьАвансыПолученные(Объект.ВидОперации);
		...
		ИспользоватьОтражениеАванса = (ПрименяетсяУСН Или ПрименяетсяУСНПатент) И ЕстьАвансыУСН;
		...
		Для каждого СтрокаПлатежа Из РасшифровкаПлатежа Цикл
			...
			Если ИспользоватьОтражениеАванса Тогда
				...
				Если ПрименяетсяУСНПатент И ТипЗнч(СтрокаПлатежа.ПорядокОтраженияАванса) = Тип("СправочникСсылка.Патенты")
					И Не Объект.БезЗакрывающихДокументов Тогда
...
Функция ИнформацияОПроведенииДокумента(ПоступлениеНаРасчетныйСчет) Экспорт
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПоступлениеНаРасчетныйСчет,
		"ВидОперации, Организация, Дата, СчетОрганизации, СчетКонтрагента");
	
	Использовать57Счет = УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(
		РеквизитыДокумента.Организация, РеквизитыДокумента.Дата);
	...
	ИначеЕсли РеквизитыДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
		И НЕ Использовать57Счет И ЗначениеЗаполнено(ПоступлениеНаРасчетныйСчет.СчетКонтрагента) Тогда
		ЦифровойСчетКор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПоступлениеНаРасчетныйСчет.СчетКонтрагента, "ЦифровойСчет");
		Если ЦифровойСчетКор Тогда
			Возврат "";
		Иначе
			Возврат НСтр("ru = 'Документ не формирует движений.
		...
		КонецЕсли;
	ИначеЕсли РеквизитыДокумента.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
		ВыпискаЗагружаетсяПоДепозитномуСчету = ЗначениеЗаполнено(РеквизитыДокумента.СчетОрганизации)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДокумента.СчетОрганизации, "ВыпискаЗагружаетсяПоДепозитномуСчету");
		ВыпискаЗагружаетсяНаДепозитныйСчет = ЗначениеЗаполнено(РеквизитыДокумента.СчетКонтрагента)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыДокумента.СчетКонтрагента, "ВыпискаЗагружаетсяПоДепозитномуСчету");
		
		Возврат ТекстОтсутствияДвиженийПоДепозиту(ВыпискаЗагружаетсяНаДепозитныйСчет, ВыпискаЗагружаетсяПоДепозитномуСчету);
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ТекстОтсутствияДвиженийПоДепозиту(ВыпискаЗагружаетсяНаДепозитныйСчет, ВыпискаЗагружаетсяПоДепозитномуСчету) Экспорт
	
	Результат = "";
	Если ВыпискаЗагружаетсяНаДепозитныйСчет Тогда
		Результат = НСтр("ru = 'Документ не формирует движений.
			|Проводки по поступлению средств на расчетный счет отражаются по счету депозита документом ""Списание с расчетного счета"" с видом операции ""Депозит"".'");
		// проверка выгрузки по депозитному счету должна быть ниже проверки выгрузки на депозитный счет
	ИначеЕсли ВыпискаЗагружаетсяПоДепозитномуСчету Тогда
		Результат = НСтр("ru = 'Документ не формирует движений.
			|Проводки по поступлению средств на расчетный счет отражаются по основному счету документом ""Списание с расчетного счета"" с видом операции ""Депозит"".'");
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ПараметрыПроведения.Вставить("ОрганизацияПрименяетУСН", ПрименяетсяУСН);
	...
	Запрос.УстановитьПараметр("ОрганизацияПрименяетУСН",        ПрименяетсяУСН);
	...
	Запрос.УстановитьПараметр("ПрименяетсяАУСН", ПрименяетсяАУСН);
	Если Реквизиты.БезЗакрывающихДокументов Тогда
		Запрос.УстановитьПараметр("ПрочиеВнереализационныеДоходыРасходы",
			Справочники.ПрочиеДоходыИРасходы.ОсновнаяСтатьяПрочихРасходов());
	КонецЕсли;
	...
	Запрос.Текст = ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
		...
		+ ТекстЗапросаВозвратОтПодотчетногоЛицаВВалюте(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаВзносВУставныйКапитал(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		;
...
Функция ПодготовитьТаблицыРасчетовДляУСН(ТаблицаРеквизиты, ТаблицаВзаиморасчетов, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаРеквизиты) Или Не ЗначениеЗаполнено(ТаблицаВзаиморасчетов) Тогда
	...
	Для Каждого СтрокаТаблицы Из Взаиморасчеты Цикл
		Если СтрокаТаблицы.ДокументРасчетов = Реквизиты.Регистратор Тогда // аванс
			...
			Если Не Реквизиты.ПлательщикНДФЛ И Не Реквизиты.ПлательщикАУСН Тогда
				...
				ЗаполнитьЗначениясвойств(НоваяСтрокаПрочихРасчетов, СтрокаТаблицы);
				НоваяСтрокаПрочихРасчетов.СчетУчета = СтрокаТаблицы.СчетРасчетов;
				НоваяСтрокаПрочихРасчетов.РасчетныйДокумент = СтрокаТаблицы.ДокументРасчетов;
				НоваяСтрокаПрочихРасчетов.Сумма = СтрокаТаблицы.СуммаВзаиморасчетов;
			КонецЕсли;
	...
	ТаблицаПрочихРасчетовУСН.Свернуть("ВидДвижения, МоментВремени, НомерСтроки, ЭтапПроведения, Период, Регистратор,
		|Организация, СчетУчета, Контрагент, РасчетныйДокумент, ДоговорКонтрагента", "Сумма");
	...
	ТаблицаИсточникДанных           = Результат[5].Выгрузить();
	...
	ОстаткиСчетовУСНРеализация = Результат[4].Выгрузить();
	ОстаткиСчетовУСНВозврат    = Результат[5].Выгрузить();
	...
	Для Каждого СтрокаТаблицыВзаиморасчеты Из НераспределенныеВзаиморасчеты Цикл
		...
		// Нет остатков в "Прочих расчетах", учитываем согласно порядку отражения аванса, указанного в документе
		Если НераспределеннаяСумма > 0 Тогда
			...
			Если (Реквизиты.УчитыватьЗадолженностьУСН Или Реквизиты.УчитыватьЗадолженностьУСНПатент)
				И ЗначениеЗаполнено(Реквизиты.Патент) Тогда
				
				НоваяСтрока.СуммаВзаиморасчетовПатент = НоваяСтрока.СуммаВзаиморасчетов;
				НоваяСтрока.СуммаВзаиморасчетовЕНВД = 0;
				НоваяСтрока.СуммаВзаиморасчетовКомитента = 0;
				НоваяСтрока.СуммаВзаиморасчетовТорговыйСбор = 0;
				
				НоваяСтрока.СуммаБУ_Патент = НоваяСтрока.СуммаРуб;
				НоваяСтрока.СуммаБУ_ЕНВД = 0;
				НоваяСтрока.СуммаБУ_Комитента = 0;
				НоваяСтрока.СуммаБУ_ТорговыйСбор = 0;
			...
			Иначе
				...
				КоэффициентРуб = ?(СтрокаТаблицыВзаиморасчеты.СуммаРуб = 0, 0,
					НераспределеннаяСумма / СтрокаТаблицыВзаиморасчеты.СуммаРуб);
			...
			Если Не Реквизиты.ПлательщикНДФЛ И Не Реквизиты.ПлательщикАУСН Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрокаПрочихРасчетов, Реквизиты);
				ЗаполнитьЗначенияСвойств(НоваяСтрокаПрочихРасчетов, СтрокаТаблицыВзаиморасчеты);
				
				НоваяСтрокаПрочихРасчетов.ВидДвижения = ВидДвиженияНакопления.Расход;
				НоваяСтрокаПрочихРасчетов.СчетУчета = СтрокаТаблицыВзаиморасчеты.СчетРасчетов;
				НоваяСтрокаПрочихРасчетов.РасчетныйДокумент = Реквизиты.Регистратор;
				НоваяСтрокаПрочихРасчетов.Сумма = НераспределеннаяСумма;
			КонецЕсли;
...
Процедура ПроверитьСоздатьВидОплаты(Объект, СвойстваПлатежа, ПлательщикНДССВычетами) Экспорт
	...
	Если Объект.ВидОперации
		...
		// Вид оплаты требуется только для операций по эквайрингу.
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СвойстваПлатежа)
		Или ЗначениеЗаполнено(СвойстваПлатежа.ВидОплаты)
		Или Не ЗначениеЗаполнено(СвойстваПлатежа.ВидОплатыРеквизиты)
		Или Не ЗначениеЗаполнено(СвойстваПлатежа.ВидОплатыРеквизиты.ДоговорКонтрагента) Тогда
		// Вид оплаты уже найден в документе, или нет данных для заполнения.
		Возврат;
	КонецЕсли;
	
	ВидОплаты = Справочники.ВидыОплатОрганизаций.ВидОплатыПоДоговору(
		СвойстваПлатежа.ВидОплатыРеквизиты.Организация,
		СвойстваПлатежа.ВидОплатыРеквизиты.Контрагент,
		СвойстваПлатежа.ВидОплатыРеквизиты.ДоговорКонтрагента);
	Если ЗначениеЗаполнено(ВидОплаты) Тогда
		// Вид оплаты по этим реквизитам уже есть в ИБ.
		Возврат;
	КонецЕсли;
	
	ВидОплатыОбъект = Справочники.ВидыОплатОрганизаций.СоздатьЭлемент();
	ВидОплатыОбъект.Заполнить(СвойстваПлатежа.ВидОплатыРеквизиты);
	
	Если Не ЗначениеЗаполнено(ВидОплатыОбъект.СчетУчетаРасчетов) Тогда
		ВидОплатыОбъект.СчетУчетаРасчетов
			= Справочники.ВидыОплатОрганизаций.СчетУчетаРасчетовПоУмолчанию(ВидОплатыОбъект.ТипОплаты);
	КонецЕсли;
	
	ВидОплатыОбъект.Наименование = СтрШаблон(НСтр("ru = 'Эквайринг %1'"),
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СвойстваПлатежа.ВидОплатыРеквизиты.Контрагент, "Наименование"));
	
	Операция = Новый Структура;
	Операция.Вставить("НазначениеПлатежа", Объект.НазначениеПлатежа);
	...
	НаборШаблонов = ЗагрузкаВыпискиПоБанковскомуСчету.НовыйШаблоныКомиссия();
	СуммаУслугБанка = ЗагрузкаВыпискиПоБанковскомуСчету.ВыделитьСуммуКомиссииВозвратаИзНазначенияПлатежа(
		Операция.НазначениеПлатежа, НаборШаблонов);
	
	Если Объект.СуммаДокумента + Объект.СуммаУслуг > 0 И Не ЗначениеЗаполнено(СуммаУслугБанка) Тогда
		ВидОплатыОбъект.ПроцентБанковскойКомиссии = 100 * Объект.СуммаУслуг / (Объект.СуммаДокумента + Объект.СуммаУслуг);
	КонецЕсли;
	
	Если УчетНДСКлиентСервер.ПрименяетсяНДССКомиссииБанка(Объект.Дата) И ПлательщикНДССВычетами И Не ЭтоСБП Тогда
		ВидОплатыОбъект.УчитыватьНДСПоКомиссии = Истина;
	КонецЕсли;
	
	Если ВидОплатыОбъект.ПроверитьЗаполнение() Тогда
		Попытка
			ВидОплатыОбъект.Записать();
		Исключение
			// В случае проблем при записи нового Вида оплаты запишем предупреждение в журнал регистрации.
			ШаблонСообщения = НСтр("ru = 'Не удалось записать справочник ""Виды оплат организаций""
				|%1'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Создание вида оплаты'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Справочники.ВидыОплатОрганизаций,
				ВидОплатыОбъект.Ссылка,
				ТекстСообщения);
			Возврат;
		КонецПопытки;
		
		СвойстваПлатежа.ВидОплаты = ВидОплатыОбъект.Ссылка;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыНастройкиУСНДляАктуальныхДокументов() Экспорт
	...
	ВидыОперацийПрочиеПоступления.Добавить(Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратОтПодотчетногоЛица);
...
Процедура ПеренестиУслугиНПДВТабличнуюЧасть(Параметры) Экспорт
	...
	|	ПоступлениеНаРасчетныйСчет.УдалитьУслугаНПД КАК УслугаНПД
	|ИЗ
	|	Документ.ПоступлениеНаРасчетныйСчет КАК ПоступлениеНаРасчетныйСчет
	|ГДЕ
	|	ПоступлениеНаРасчетныйСчет.УдалитьУслугаНПД <> &ПустаяУслугаНПД
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Иначе
		...
		Если ОбъектовОбработано = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедуре ПоступлениеНаРасчетныйСчет.ПеренестиУслугиНПДВТабличнуюЧасть
				|не удалось перенести услуги НПД в табличную часть в %1 документах Поступление на расчетный счет.'"), 
				ПроблемныхОбъектов);
			ВызватьИсключение ТекстСообщения;
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ПоступлениеНаРасчетныйСчет.ПеренестиУслугиНПДВТабличнуюЧасть
...
// Обработчик обновления 3.0.118.13.
// Заполняет реквизит "ОтражениеВУСН" значением по умолчанию для операций оплаты от покупателя при применении АУСН.
//
// Параметры:
//  Параметры - Структура - параметры обработчика обновления.
//
Процедура ЗаполнитьОтражениеОплатПокупателяВАУСН(Параметры) Экспорт
	...
	|ПОМЕСТИТЬ ВТ_ПериодыНастроекПрименяетсяАУСН
	|ИЗ
	...
	|ГДЕ
	|	НастройкиТекущие.ПрименяетсяАУСН
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВЫБОР
	|		КОГДА ПериодыНастроекПрименяетсяАУСН.ПериодСледующий ЕСТЬ NULL
	|			ТОГДА &КонецПериода
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ПериодыНастроекПрименяетсяАУСН.ПериодСледующий, ДЕНЬ), СЕКУНДА, -1)
	|	КОНЕЦ КАК КонецПериода
	|ПОМЕСТИТЬ ПериодыПримененияАУСН
	|ИЗ
	|	ВТ_НастройкиСистемыНалогообложения КАК НастройкиСистемыНалогообложения
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПериодыНастроекПрименяетсяАУСН КАК ПериодыНастроекПрименяетсяАУСН
	|		ПО НастройкиСистемыНалогообложения.Организация = ПериодыНастроекПрименяетсяАУСН.Организация
	|			И НастройкиСистемыНалогообложения.Период = ПериодыНастроекПрименяетсяАУСН.ПериодТекущий
	|ГДЕ
	...
	|ИЗ
	|	Документ.ПоступлениеНаРасчетныйСчет КАК ПоступлениеНаРасчетныйСчет
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыПримененияАУСН КАК ПериодыПримененияАУСН
	|		ПО ПоступлениеНаРасчетныйСчет.Организация = ПериодыПримененияАУСН.Организация
	|			И ПоступлениеНаРасчетныйСчет.Дата >= ПериодыПримененияАУСН.НачалоПериода
	|			И ПоступлениеНаРасчетныйСчет.Дата <= ПериодыПримененияАУСН.КонецПериода
	|ГДЕ
	|	ПоступлениеНаРасчетныйСчет.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя)
	|	И ПоступлениеНаРасчетныйСчет.ОтражениеВУСН <> ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.Принимаются)";
	...
	ИмяПроцедуры = "Документы.ПоступлениеНаРасчетныйСчет.ЗаполнитьОтражениеОплатПокупателяВАУСН";
...
// Функция проверяет, обработан ли документ процедурой обновления (см. ЗаполнитьОтражениеОплатПокупателяВАУСН).
...
//
Функция ОтражениеОплатыПокупателяВАУСНЗаполнено(Параметры) Экспорт
	...
	ИменаРеквизитов = "Организация, Дата, ВидОперации, ОтражениеВУСН";
	
	ДанныеДокумента = Новый Структура(ИменаРеквизитов);
	
	Если ТипЗнч(Параметры.Данные) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") Тогда
		
		Документ = Параметры.Данные;
		ДанныеДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, ИменаРеквизитов);
		
	ИначеЕсли ТипЗнч(Параметры.Данные) = Тип("ДокументОбъект.ПоступлениеНаРасчетныйСчет")
		Или ТипЗнч(Параметры.Данные) = Тип("ДанныеФормыСтруктура") Тогда
		
		Документ = Параметры.Данные.Ссылка;
		ЗаполнитьЗначенияСвойств(ДанныеДокумента, Параметры.Данные);
	...
	Если ДанныеДокумента.ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		Или Не УчетнаяПолитика.ПрименяетсяАУСН(ДанныеДокумента.Организация, ДанныеДокумента.Дата) Тогда
		Возврат Истина;
	КонецЕсли;
	
	// Обновление завершается, когда во всех необходимых документах будет установлено отражение доходов "Принимаются".
	
	Возврат (ДанныеДокумента.ОтражениеВУСН <> Перечисления.ОтражениеВУСН.Принимаются);
...
Процедура ОбработатьРучнуюНастройкуУСНПрочиеРасчеты(СтрокаПоДокументу)
	...
	Попытка
		...
		Если ДокументОбъект.Графа6_УСН <> 0 Или ДокументОбъект.Графа7_УСН <> 0 Или ДокументОбъект.НДС_УСН <> 0
			Или ДокументОбъект.ДоходыЕНВД_УСН Или ДокументОбъект.УдалитьРасходыЕНВД_УСН Тогда
			ДокументОбъект.РучнаяКорректировка = Истина;
		КонецЕсли;
...
Функция ТекстЗапросаРеквизитыПечатиЧека(ИменаТаблиц)
	...
	|		ИНАЧЕ ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.СчетНаОплату
	|	КОНЕЦ,
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.ДоговорКонтрагента.Владелец,
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Ссылка.Дата
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПередачаОС.Ссылка,
	|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Сделка
	|ИЗ
	|	ВТ_РасшифровкаПлатежа КАК ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаОС КАК ПередачаОС
	|		ПО ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Сделка = ПередачаОС.ДокументОснование
	|ГДЕ
	|	ЕСТЬNULL(ПередачаОС.Проведен, ЛОЖЬ)
	|	И ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Сделка <> ЗНАЧЕНИЕ(Документ.СчетНаОплатуПокупателю.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПодготовитьПараметрыРеквизитыДокумента(Запрос, ПараметрыПроведения, Отказ)
	...
	|	ЕСТЬNULL(Реквизиты.СчетКонтрагента.ВыпискаЗагружаетсяПоДепозитномуСчету, ЛОЖЬ)
	|		ИЛИ ЕСТЬNULL(Реквизиты.СчетОрганизации.ВыпискаЗагружаетсяПоДепозитномуСчету, ЛОЖЬ) КАК ВыпискаЗагружаетсяПоДепозитномуСчету,
	|	ЕСТЬNULL(Реквизиты.СчетКонтрагента.ЦифровойСчет, ЛОЖЬ) КАК ЦифровойСчетКор,
	...
	|	Реквизиты.Графа5_УСН КАК СуммаДоходаУСН,
	...
	|	Реквизиты.СчетКонтрагента КАК СчетКонтрагента,
	|	Реквизиты.ВыпискаЗагружаетсяПоДепозитномуСчету КАК ВыпискаЗагружаетсяПоДепозитномуСчету,
	|	Реквизиты.ЦифровойСчетКор КАК ЦифровойСчетКор,
	|	Реквизиты.СчетОрганизации КАК СчетОрганизации,
	|	Реквизиты.СчетБанк КАК СчетБанк,
	|	Реквизиты.СубконтоКт1 КАК СубконтоКт1,
	|	Реквизиты.СубконтоКт2 КАК СубконтоКт2,
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	&ПрименяетсяАУСН КАК ПлательщикАУСН,
	|	ВЫБОР
	...
	|			ТОГДА &УчитыватьЗадолженностьУСНПатент
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицыДокумента(НомераТаблиц)
	...
	|	&Налог КАК РеквизитыНалог,
	|	&Контрагент КАК РеквизитыКонтрагент,
	|	&СчетКонтрагента КАК РеквизитыСчетКонтрагента,
...
Функция ТекстЗапросаПогашениеЗадолженности(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
		...
		Иначе
			...
			|	ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом,
			|	ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом,
			|	ТаблицаРасшифровкаПлатежа.ПорядокОтраженияАванса,
			|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетУчетаРасчетовСКонтрагентом,
			|	ТаблицаРасшифровкаПлатежа.РеквизитыСубконтоКт1,
			|	ТаблицаРасшифровкаПлатежа.РеквизитыСубконтоКт2,
			|	ТаблицаРасшифровкаПлатежа.РеквизитыСубконтоКт3,
...
Функция ТекстЗапросаВозвратОтПодотчетногоЛицаВВалюте(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Реквизиты.ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратОтПодотчетногоЛица
		Или Реквизиты.ДокументВРублях 
		Или Не УчетДоходовРасходов.ОцениватьПодотчетныеСуммыПоКурсуАвансов(Реквизиты.Дата) Тогда
		ПараметрыПроведения.Вставить("ВозвратОтПодотчетногоЛицаВВалюте", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	|	Реквизиты.Дата КАК Период,
	|	Реквизиты.Организация КАК Организация,
	|	Реквизиты.Контрагент КАК ПодотчетноеЛицо,
	|	Реквизиты.ПодразделениеОрганизации КАК Подразделение,
	|	Реквизиты.СчетБанк КАК КорСчет,
	|	Реквизиты.СчетОрганизации КАК КорСубконто1,
	|	Реквизиты.СтатьяДвиженияДенежныхСредств КАК КорСубконто2,
	|	НЕОПРЕДЕЛЕНО КАК КорСубконто3,
	...
	НомераТаблиц.Вставить("ВозвратОтПодотчетногоЛицаВВалюте", НомераТаблиц.Количество());
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаПрочееПоступление(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	// - ВозвратОтПодотчетногоЛица (с 2020 года - только в рублях)
	// - ПрочееПоступление
	...
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратОтПоставщика
		...
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратОтПодотчетногоЛица 
		И (Реквизиты.ДокументВРублях
		Или Не УчетДоходовРасходов.ОцениватьПодотчетныеСуммыПоКурсуАвансов(Реквизиты.Дата)) Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
	...
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление Тогда
		...
		|ИЗ
		|	ТаблицаРасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа";
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетУчетаРасчетовСКонтрагентом КАК СчетКт,
	...
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаРаботником Тогда
		...
		|ИЗ
		|	ТаблицаРасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа";
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.РеквизитыСчетУчетаРасчетовСКонтрагентом <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
		|			ТОГДА ТаблицаРасшифровкаПлатежа.РеквизитыСчетУчетаРасчетовСКонтрагентом
		|		КОГДА &ДокументВВалюте
		|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДепозитныеСчетаВал)
		|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДепозитныеСчета)
		|	КОНЕЦ КАК СчетКт,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетКонтрагента КАК СубконтоКт1,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСтатьяДвиженияДенежныхСредств КАК СубконтоКт2,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыПодразделениеОрганизации КАК ПодразделениеКт,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетБанк КАК СчетДт,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетОрганизации КАК СубконтоДт1,
		...
		|	ТаблицаРасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа
		|ГДЕ
		|	НЕ &ВыпискаЗагружаетсяПоДепозитномуСчету";
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
		И Не УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Реквизиты.Организация, Реквизиты.Дата)
		И Реквизиты.ЦифровойСчетКор Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
		|			ТОГДА ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом
		|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЦифровойРубль)
		|	КОНЕЦ КАК СчетКт,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСчетКонтрагента КАК СубконтоКт1,
		|	ТаблицаРасшифровкаПлатежа.РеквизитыСтатьяДвиженияДенежныхСредств КАК СубконтоКт2,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
...
Функция ТекстЗапросаПокупкаПродажаВалютыУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не ПараметрыПроведения.ОрганизацияПрименяетУСН
		И Не ПараметрыПроведения.ПрименяетсяАУСН Тогда
...
Функция ТекстЗапросаДанныеРазметкиАУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.СуммаУслуг КАК СуммаУслуг,
	|	Реквизиты.СуммаВозврата КАК СуммаВозврата,
	|	Реквизиты.СуммаДоходаУСН КАК СуммаДоходаУСН,
	|	Реквизиты.ЭтоВозврат КАК ЭтоВозврат,
	|	Реквизиты.ОтражениеВУСН КАК ОтражениеВУСН,
	|	Реквизиты.ИсточникРазметкиАУСН КАК ИсточникРазметкиАУСН
	|ИЗ
...
Функция ТекстЗапросаВзносВУставныйКапитал(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Реквизиты.ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал Тогда
		ПараметрыПроведения.Вставить("ВзносВУставныйКапитал", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	"ВЫБРАТЬ
	|	Реквизиты.ВидОперации КАК ВидОперации,
	|	Реквизиты.СуммаДокумента КАК СуммаВзноса,
	|	Реквизиты.Контрагент КАК Контрагент,
	|	Реквизиты.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	...
	НомераТаблиц.Вставить("ВзносВУставныйКапитал", НомераТаблиц.Количество());
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаРазметкаАУСНБанка(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ИЗ
	|	Документ.ПоступлениеНаРасчетныйСчет.РазметкаАУСНБанка КАК ТаблицаРазметкаАУСНБанка
	|ГДЕ
...
Функция ТекстЗапросаБюджетДоходыФакт(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не Документы.Бюджет.ЕстьБюджетНаПериод(Реквизиты.Организация, Реквизиты.Дата)
		...
		ПараметрыПроведения.Вставить("БюджетДоходы", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	НомераТаблиц.Вставить("БюджетДоходы", НомераТаблиц.Количество());
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Функция ТекстЗапросаБюджетРасходыФакт(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не Документы.Бюджет.ЕстьБюджетНаПериод(Реквизиты.Организация, Реквизиты.Дата)
		...
		ПараметрыПроведения.Вставить("БюджетРасходы", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	НомераТаблиц.Вставить("БюджетРасходы", НомераТаблиц.Количество());
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
...
Процедура ЗачестьВозвратыПоПрочимРасчетам(Реквизиты, ТаблицыВозвратов, ТаблицыРеализаций, ПодготовленныеТаблицы, СчетаУСН)
	...
	Для Каждого СтрокаВозврата Из ОстаткиПрочихРасчетовВозврат Цикл
		...
		Для Каждого СтрокаРеализации Из НайденныеОстаткиРеализации Цикл
			...
			// При совмещении ИП ОСН и ПСН "Прочие расчеты" по зачету возврата формируются при подготовке таблиц ИП
			Если Не Реквизиты.ПлательщикНДФЛ Тогда
				...
				ДобавитьЗачетВозвратаПоПрочимРасчетам(
					ТаблицаПрочихРасчетовУСН, Реквизиты, СтрокаВозврата, СтрокаРеализации, СуммаЗачета);
			КонецЕсли;
...
Функция ПодготовитьПараметрыТаблицРасчетовДляУСН(ТаблицаРеквизиты, ТаблицаВзаиморасчетов)
	...
	СписокОбязательныхКолонок =
		...
		+ "ПлательщикАУСН,"                        // <Булево> - если Истина, то организация является плательщиком АУСН
		+ "Патент,"                                // <СправочникСсылка.Патенты> - патент, для отражения дохода по деятельности на патенте
...
Функция ТекстЗапросаОстаткиПрочихРасчетов()
	...
	|	ВТОстаткиПрочихРасчетов.Подразделение КАК Подразделение,
	|	ВТОстаткиПрочихРасчетов.СчетУчета КАК СчетУчета,
	...
	|		ПО ВТОстаткиПрочихРасчетов.РасчетныйДокумент = ОплатаПлатежнойКартой.Ссылка
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОбщийСрезПрочихРасчетов.Организация КАК Организация,
	|	ВТОбщийСрезПрочихРасчетов.Подразделение КАК Подразделение,
	|	ВТОбщийСрезПрочихРасчетов.СчетУчета КАК СчетУчета,
	|	ВТОбщийСрезПрочихРасчетов.Контрагент КАК Контрагент,
	|	ВТОбщийСрезПрочихРасчетов.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ВТОбщийСрезПрочихРасчетов.ДатаПервичногоДокумента КАК ДатаПервичногоДокумента,
	|	ВТОбщийСрезПрочихРасчетов.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВТОбщийСрезПрочихРасчетов.СуммаОстаток КАК СуммаОстаток,
	|	ВТОбщийСрезПрочихРасчетов.ЭтоПриемРозничнойВыручкиНТТ КАК ЭтоПриемРозничнойВыручкиНТТ
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_АналитикаОстатков.Контрагент КАК Контрагент,
	|	ВТ_АналитикаОстатков.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВТ_АналитикаОстатков.РасчетныйДокумент КАК РасчетныйДокумент
	|ИЗ
	...
	Если БухгалтерскийУчетПереопределяемый.ВестиУчетПоПодразделениям() Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПустоеПодразделение КАК Подразделение",
			"ВЫБОР
			...
			|		КОГДА ТИПЗНАЧЕНИЯ(ПрочиеРасчетыОстатки.РасчетныйДокумент) = ТИП(Документ.ОказаниеУслуг)
			|			ТОГДА ВЫРАЗИТЬ(ПрочиеРасчетыОстатки.РасчетныйДокумент КАК Документ.ОказаниеУслуг).ПодразделениеОрганизации
			|		КОГДА ТИПЗНАЧЕНИЯ(ПрочиеРасчетыОстатки.РасчетныйДокумент) = ТИП(Документ.АктОбОказанииПроизводственныхУслуг)
			|			ТОГДА ВЫРАЗИТЬ(ПрочиеРасчетыОстатки.РасчетныйДокумент КАК Документ.АктОбОказанииПроизводственныхУслуг).ПодразделениеОрганизации
...
Функция ТекстЗапросаОстаткиПоСчетамУСН()
	...
	|	КОНЕЦ КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТВыборкаПосчетамУСНБезАналитики
	|ИЗ
	...
	|				И &УсловиеПоПодразделению) КАК ХозрасчетныйОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	Договор,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТВыборкаПосчетамУСНБезАналитики.Контрагент КАК Контрагент,
	|	ВТВыборкаПосчетамУСНБезАналитики.Договор КАК Договор,
	|	ВТВыборкаПосчетамУСНБезАналитики.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ВТВыборкаПосчетамУСНБезАналитики.Счет КАК Счет,
	|	ВТВыборкаПосчетамУСНБезАналитики.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ВТВыборкаПосчетамУСНБезАналитики.Подразделение КАК Подразделение,
	|	ВТВыборкаПосчетамУСНБезАналитики.СуммаРуб КАК СуммаРуб,
	|	ВТВыборкаПосчетамУСНБезАналитики.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов
	|ПОМЕСТИТЬ ВТВыборкаПоСчетамУСН
	|ИЗ
	|	ВТ_АналитикаОстатков КАК АналитикаОстатков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТВыборкаПосчетамУСНБезАналитики КАК ВТВыборкаПосчетамУСНБезАналитики
	|		ПО АналитикаОстатков.Контрагент = ВТВыборкаПосчетамУСНБезАналитики.Контрагент
	|			И АналитикаОстатков.ДоговорКонтрагента = ВТВыборкаПосчетамУСНБезАналитики.Договор
	|			И АналитикаОстатков.РасчетныйДокумент = ВТВыборкаПосчетамУСНБезАналитики.РасчетныйДокумент
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТВыборкаПосчетамУСНБезАналитики
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТВыборкаПосчетамУСН.Контрагент КАК Контрагент,
	|	ВТВыборкаПосчетамУСН.Договор КАК Договор,
	|	ВТВыборкаПосчетамУСН.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ДанныеПервичныхДокументов.ДатаРегистратора КАК РасчетныйДокументДата,
	|	ВТВыборкаПосчетамУСН.Счет КАК Счет,
	|	ВТВыборкаПосчетамУСН.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
	|	ВТВыборкаПосчетамУСН.Подразделение КАК Подразделение,
	...
	|	ВТ_ОбщиеДанные.Договор КАК Договор,
	|	ВТ_ОбщиеДанные.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ВТ_ОбщиеДанные.РасчетныйДокументДата КАК РасчетныйДокументДата,
	|	ВТ_ОбщиеДанные.Счет КАК Счет,
	|	ВТ_ОбщиеДанные.ВалютаВзаиморасчетов КАК ВалютаВзаиморасчетов,
