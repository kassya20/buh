...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АдресКассовыеЧеки = Параметры.АдресКассовыеЧеки;
...
Процедура КомандаУдалить(Команда)
	...
	Если КассовыеЧеки.Количество() Тогда
		ПоказатьВопросУдалитьЧек(Элементы.КассовыеЧеки.ТекущиеДанные.Ссылка);
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьВопросУдалитьЧек(КассовыйЧек)
	
	Оповещение = Новый ОписаниеОповещения("КассовыеЧекиУдалитьЗавершение", ЭтотОбъект, КассовыйЧек);
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Удалить чек?
		|Это действие нельзя будет отменить.'"), РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

&НаКлиенте
Процедура КассовыеЧекиУдалитьЗавершение(Результат, КассовыйЧек) Экспорт
	...
	ИндексСтроки = Элементы.КассовыеЧеки.ТекущаяСтрока;
	
	УдалитьКассовыйЧек(КассовыйЧек);
	НастроитьДаннымиПодбора();
	
	Если Не МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		ЧислоСтрок = КассовыеЧеки.Количество();
		// После удаления строки табличная часть перезаполняется c назначением новых индексов
		// каждой строке. Cледует установить курсор на прежнее место.
		Элементы.КассовыеЧеки.ТекущаяСтрока = ИндексСтроки + ЧислоСтрок;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура УдалитьКассовыйЧек(КассовыйЧек)
	
	Пользователь = РегистрыСведений.СканированиеКассовыхЧековПодотчетныхЛиц.ПользовательЧека(КассовыйЧек);
	Если ЗначениеЗаполнено(Пользователь) Тогда
		Документы.КассовыйЧекПодотчетногоЛица.УдалитьЧекПользователя(КассовыйЧек, Пользователь)
	КонецЕсли;
	
	ДанныеКассовыхЧеков = ПолучитьИзВременногоХранилища(АдресКассовыеЧеки);
	Для Каждого Чек Из ДанныеКассовыхЧеков.КЗагрузке Цикл
		Если Чек.Ссылка = КассовыйЧек Тогда
			ДанныеКассовыхЧеков.КЗагрузке.Удалить(Чек);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	ПоместитьВоВременноеХранилище(ДанныеКассовыхЧеков, АдресКассовыеЧеки);
...
Процедура НастроитьДаннымиПодбора()
	...
	Если Не ЗначениеЗаполнено(АдресКассовыеЧеки) Тогда
		ВызватьИсключение НСтр("ru = 'Не переданы данные для подбора'");
	КонецЕсли;
	
	ДанныеКассовыхЧеков = ПолучитьИзВременногоХранилища(АдресКассовыеЧеки); // См. УчетКассовыхЧековПодотчетныхЛиц.НовыйДанныеФормыКассовыеЧекиПодотчетныхЛиц
	
	Если ДанныеКассовыхЧеков.КешПодбора <> Неопределено
		И ДанныеКассовыхЧеков.КешПодбора.ПодотчетноеЛицо = ПодотчетноеЛицо Тогда
	...
	Иначе
		...
		ДанныеКассовыхЧеков.КешПодбора.Вставить("ПодотчетноеЛицо",                 ПодотчетноеЛицо);
		ДанныеКассовыхЧеков.КешПодбора.Вставить("ВыводитьПодотчетноеЛицо",         Ложь);
		ДанныеКассовыхЧеков.КешПодбора.Вставить("ПодотчетноеЛицоПредставление",    Строка(ПодотчетноеЛицо));
	...
	ПоместитьВоВременноеХранилище(ДанныеКассовыхЧеков, АдресКассовыеЧеки);
	НачальнаяНастройка = КассовыеЧеки.Количество() = 0;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		СократимНазваниеПоставщика(КассовыеЧеки);
	КонецЕсли;
	
	КешПодбора = ДанныеКассовыхЧеков.КешПодбора;
	НастроитьВариантыОтбора(
		Элементы.ВариантОтбора,
		ВариантОтбора,
		КешПодбора.КоличествоКассовыхЧеков,
		КешПодбора.ПодключенныеМобильныеПриложения,
		?(КешПодбора.ВыводитьПодотчетноеЛицо, КешПодбора.ПодотчетноеЛицо, Неопределено));
...
Процедура НастроитьВариантыОтбора(ЭлементВариантыОтбора, ВыбранныйВариантОтбора,
	...
	Если Не ЗначениеЗаполнено(ВыбранныйВариантОтбора) Тогда
	...
	// Если список выбора уже заполнен, следует его обновить.
	Если ЭлементВариантыОтбора.СписокВыбора.Количество() Тогда
		Для Каждого СуществующийОтбор Из ЭлементВариантыОтбора.СписокВыбора Цикл
			Для Каждого НовыйОтбор Из СписокВыбора Цикл
				Если СуществующийОтбор.Значение = НовыйОтбор.Значение Тогда
					СуществующийОтбор.Представление = НовыйОтбор.Представление;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	Иначе
		Для Каждого НовыйОтбор Из СписокВыбора Цикл
			ЭлементВариантыОтбора.СписокВыбора.Добавить(НовыйОтбор.Значение, НовыйОтбор.Представление);
		КонецЦикла;
...
&НаСервереБезКонтекста
Процедура СократимНазваниеПоставщика(КассовыеЧеки)
	
	Для Каждого Запись Из КассовыеЧеки Цикл
		
		Запись.Поставщик = ОрганизационноПравоваяФормаСокращенно(Запись.Поставщик);
...
&НаСервереБезКонтекста
Функция ОрганизационноПравоваяФормаСокращенно(Знач Наименование)
	//Сократим наиболее популярные формы
	Наименование = СтрЗаменить(Наименование, "ИНДИВИДУАЛЬНЫЙ ПРЕДПРИНИМАТЕЛЬ", "ИП");
	Наименование = СтрЗаменить(Наименование, "ОБЩЕСТВО С ОГРАНИЧЕННОЙ ОТВЕТСТВЕННОСТЬЮ", "ООО");
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	Элементы.ФормаЗагрузитьВсеМК.Видимость = Истина;
	МобильныйКлиентБПФормаОбъекта.АдаптироватьМККоманднаяПанель(ЭтотОбъект);
	МобильныйКлиентБПФормаОбъекта.ПеренестиКомандуВМККомандыФормы(Элементы, Элементы.ФормаЗагрузитьВсеМК);
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы,
		Элементы.ВариантОтбора);
	Элементы.ГруппаКассовыеЧеки.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы,
		Элементы.ГруппаКассовыеЧеки);
...
Процедура АдаптироватьМКСписокФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ПодготовитьТабличноеПолеФормыДокументаМногострочное(ЭтотОбъект, "КассовыеЧеки");
	
	Элементы.КассовыеЧеки.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
	Элементы.КассовыеЧеки.Высота = 0;
	Элементы.КассовыеЧеки.Шапка = Ложь;
	
	Элементы.КассовыеЧеки.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ПоСодержимому;
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементВГруппуМК_ТабличноеПоле_Строка(ЭтотОбъект, Элементы.КассовыеЧекиДата, 1, "КассовыеЧеки");
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементВГруппуМК_ТабличноеПоле_Строка(ЭтотОбъект, Элементы.КассовыеЧекиСумма, 1, "КассовыеЧеки");
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементВГруппуМК_ТабличноеПоле_Строка(ЭтотОбъект, Элементы.КассовыеЧекиПоставщик, 2, "КассовыеЧеки");
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементВГруппуМК_ТабличноеПоле_Строка(ЭтотОбъект, Элементы.КассовыеЧекиПользователи,3, "КассовыеЧеки");
	
	Элементы.КассовыеЧеки.ПоложениеСтрокиПоиска = ПоложениеСтрокиПоиска.ЗаголовокФормы;
...
Процедура АдаптироватьМККонтекстСписокФормы()
	...
	Элементы.КассовыеЧекиКонтекстноеМенюПоказатьЧекМК.Видимость = Истина;
	МобильныйКлиентБПФормаОбъекта.АдаптироватьКонтекстноеМеню(ЭтотОбъект,, "КассовыеЧеки");
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы,
		Элементы.КассовыеЧекиКонтекстноеМенюПоказатьЧекМК,
		Элементы.КассовыеЧеки.КонтекстноеМеню);
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы,
		Элементы.КассовыеЧекиУдалить,
		Элементы.КассовыеЧеки.КонтекстноеМеню);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьЧекМК(Команда)
	ОтобразитьПредварительныйПросмотр(Элементы.КассовыеЧеки.ТекущиеДанные);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьВсеМК(Команда)
	СписокВыгрузки = Новый Массив;
	Для Каждого КассовыйЧек Из КассовыеЧеки Цикл
		СписокВыгрузки.Добавить(КассовыйЧек.Ссылка);
	КонецЦикла;
	Закрыть(СписокВыгрузки);
КонецПроцедуры
