...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, ВедетсяУчетПрослеживаемыхТоваров)
	...
	Если НЕ ВедетсяУчетПрослеживаемыхТоваров Тогда
			ПараметрыПроведения.Вставить("ПрослеживаемыеТоварыСписываемые", Неопределено);
	...
	НомераТаблиц.Вставить("ПрослеживаемыеТоварыСписываемые",   НомераТаблиц.Количество());
	...
	|	КомплектацияНоменклатуры.Номенклатура КАК Комплект,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.Количество КАК Количество,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	КомплектацияНоменклатурыКомплектующие.СтранаПроисхождения КАК СтранаПроисхождения,
	|	НЕОПРЕДЕЛЕНО КАК Комитент,
	|	НЕОПРЕДЕЛЕНО КАК Комиссионер,
	|	НЕОПРЕДЕЛЕНО КАК ВидЗапасов,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации,
	|	НЕОПРЕДЕЛЕНО КАК ОснованиеДляВозврата
	|ПОМЕСТИТЬ ДаннныеПоРНПТ
	|ИЗ
	|	Документ.КомплектацияНоменклатуры КАК КомплектацияНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КомплектацияНоменклатуры.Комплектующие КАК КомплектацияНоменклатурыКомплектующие
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КомплектацияНоменклатуры.СведенияПрослеживаемости КАК КомплектацияНоменклатурыСведенияПрослеживаемости
	|			ПО КомплектацияНоменклатурыКомплектующие.Ссылка = КомплектацияНоменклатурыСведенияПрослеживаемости.Ссылка
	|				И КомплектацияНоменклатурыКомплектующие.ИдентификаторСтроки = КомплектацияНоменклатурыСведенияПрослеживаемости.ИдентификаторСтроки
	|		ПО КомплектацияНоменклатуры.Ссылка = КомплектацияНоменклатурыКомплектующие.Ссылка
	|ГДЕ
	|	КомплектацияНоменклатуры.Ссылка = &Ссылка
	|	И КомплектацияНоменклатурыКомплектующие.ПрослеживаемыйТовар = ИСТИНА
	|
	|СГРУППИРОВАТЬ ПО
	|	КомплектацияНоменклатурыКомплектующие.Номенклатура,
	|	КомплектацияНоменклатуры.Номенклатура,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.Количество,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	КомплектацияНоменклатурыСведенияПрослеживаемости.РНПТ,
	|	КомплектацияНоменклатурыКомплектующие.СтранаПроисхождения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ТОГДА ДаннныеПоРНПТ.Номенклатура
	|		ИНАЧЕ ДаннныеПоРНПТ.Комплект
	|	КОНЕЦ КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА &ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКомплектацияНоменклатуры.Комплектация)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ДаннныеПоРНПТ.Номенклатура
	|	КОНЕЦ КАК Комплектующие,
	|	ДаннныеПоРНПТ.Количество КАК Количество,
	|	ДаннныеПоРНПТ.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ДаннныеПоРНПТ.РНПТ КАК РНПТ,
	|	ДаннныеПоРНПТ.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ДаннныеПоРНПТ.Комитент КАК Комитент,
	|	ДаннныеПоРНПТ.Комиссионер КАК Комиссионер,
	|	ДаннныеПоРНПТ.ВидЗапасов КАК ВидЗапасов,
	|	ДаннныеПоРНПТ.ДатаРеализации КАК ДатаРеализации,
	|	ДаннныеПоРНПТ.ОснованиеДляВозврата КАК ОснованиеДляВозврата
	|ИЗ
	|	ДаннныеПоРНПТ КАК ДаннныеПоРНПТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
