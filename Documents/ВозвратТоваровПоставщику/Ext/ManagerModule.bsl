...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ВЫБОР 
	|		КОГДА СправочникНоменклатура.ЕдиницаИзмерения <> ТаблицаТовары.Упаковка
	|				И ТаблицаТовары.Упаковка <> НЕОПРЕДЕЛЕНО
	|   		ТОГДА ТаблицаТовары.Упаковка.Коэффициент
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК КоэффициентУпаковки,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА ДокументВозвратТоваровПоставщику.КорректировочныйСчетФактураПолученОтПоставщика
	|				И НЕ ДокументВозвратТоваровПоставщику.СуммаВключаетНДС
	|			ТОГДА ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику КАК ДокументВозвратТоваровПоставщику
	|		ПО ТаблицаТовары.Ссылка = ДокументВозвратТоваровПоставщику.Ссылка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВозвратТоваровПоставщикуСведенияПрослеживаемости.РНПТ.Код КАК НомерТовара,
	...
	|	ВЫБОР
	|		КОГДА ВозвратТоваровПоставщикуСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ(ВозвратТоваровПоставщикуСведенияПрослеживаемости.Количество * (ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) / ТаблицаДокумента.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ВозвратТоваровПоставщикуСведенияПрослеживаемости.Количество * ТаблицаДокумента.Цена
	|				КОНЕЦ
	|		ИНАЧЕ ВозвратТоваровПоставщикуСведенияПрослеживаемости.Сумма
	...
	|			И ВозвратТоваровПоставщикуСведенияПрослеживаемости.Ссылка = ТаблицаДокумента.Ссылка";
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|			ВозвратТоваровПоставщикуСведенияПрослеживаемости.Ссылка КАК Ссылка,
	|			ИСТИНА КАК ЕстьПрослеживаемыеТовары
	|		ИЗ
	|			Документ.ВозвратТоваровПоставщику.СведенияПрослеживаемости КАК ВозвратТоваровПоставщикуСведенияПрослеживаемости
	|		ГДЕ
	|			ВозвратТоваровПоставщикуСведенияПрослеживаемости.Ссылка В(&МассивОбъектов)) КАК ПрослеживаемыеТовары
	|		ПО ВозвратТоваровПоставщику.Ссылка = ПрослеживаемыеТовары.Ссылка
	|ГДЕ
	|	ВозвратТоваровПоставщику.Ссылка В(&МассивОбъектов)
	|	И ВозвратТоваровПоставщику.Дата > &УсловиеПоДате
...
// Функция возвращает основание для документа "Возврат товаров поставщику". Возращается документ основание только если он один.
// Если это возврат по нескольким основаниям или возврат без основания - возвращается НЕОПРЕДЕЛЕНО
//
// Параметры:
//  <Ссылка>  - <ДокументСсылка.ВозвратТоваровПоставщику> - документ по которому надо определелить основание
//
// Возвращаемое значение:
//   <ДокументСсылка>, Неопределено   - документ-основание возврата.
//
Функция СделкаПоДокументу(Ссылка) Экспорт
	Запрос = Новый Запрос;
	...
	|	И НЕ ВозвратТоваровПоставщику.Сделка В (&МассивПустыхЗначений)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ВозвратТоваровПоставщикуТовары.Ссылка = &Ссылка
	|	И НЕ ВозвратТоваровПоставщикуТовары.Сделка В (&МассивПустыхЗначений)";
	
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("МассивПустыхЗначений", ОбщегоНазначенияБП.МассивПустыхЗначенийРеквизита(Ссылка.Метаданные().Реквизиты.Сделка));
	
	РезультатВыполненияЗапроса = Запрос.Выполнить().Выбрать();
	Если РезультатВыполненияЗапроса.Количество() = 1 Тогда
		РезультатВыполненияЗапроса.Следующий();
		Возврат РезультатВыполненияЗапроса.Сделка;
	Иначе
		Возврат Неопределено;
	КонецЕсли; 
КонецФункции

Функция ТаблицаОстатковТоваровПоСделкам(ТаблицаТовары, Организация, Дата,Ссылка) Экспорт
	
	ТаблицаРезультата = Новый ТаблицаЗначений;
	ТаблицаРезультата.Колонки.Добавить("Сделка",       Новый ОписаниеТипов("ДокументСсылка.ПоступлениеТоваровУслуг"));
	ТаблицаРезультата.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаРезультата.Колонки.Добавить("Количество",   ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	...
	Запрос.УстановитьПараметр("МоментВремениТекущегоДокумента",       Новый МоментВремени(Дата, Ссылка));
	Запрос.УстановитьПараметр("ТекущийДокумент",                      Ссылка);
	Запрос.УстановитьПараметр("ДатаНачалаУчетаПрослеживаемыхТоваров", ПрослеживаемостьБРУ.ДатаНачалаУчетаПрослеживаемыхТоваров());
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Товары.Сделка КАК Сделка,
	|	ПоступлениеТоваровУслуг.Дата КАК ДатаСделки,
	|	ВТ_Товары.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ ВТ_НоменклатураПоДокументам
	|ИЗ
	|	ВТ_Товары КАК ВТ_Товары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	|		ПО ВТ_Товары.Сделка = ПоступлениеТоваровУслуг.Ссылка
	|ГДЕ
	|	ПоступлениеТоваровУслуг.Организация = &Организация
	|	И НЕ ПоступлениеТоваровУслуг.Ссылка ЕСТЬ NULL
	|	И ПоступлениеТоваровУслуг.Проведен
	|	И ПоступлениеТоваровУслуг.Дата >= &ДатаНачалаУчетаПрослеживаемыхТоваров
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сделка,
	|	Номенклатура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_НоменклатураПоДокументам.Сделка КАК Сделка,
	|	ПоступлениеТоваровУслугТовары.Номенклатура КАК Номенклатура,
	|	ПоступлениеТоваровУслугТовары.Количество КАК Количество
	|ПОМЕСТИТЬ ВТ_ПоступленияВозвраты
	|ИЗ
	|	ВТ_НоменклатураПоДокументам КАК ВТ_НоменклатураПоДокументам
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
	|		ПО ВТ_НоменклатураПоДокументам.Сделка = ПоступлениеТоваровУслугТовары.Ссылка
	|			И ВТ_НоменклатураПоДокументам.Номенклатура = ПоступлениеТоваровУслугТовары.Номенклатура
	|ГДЕ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_НоменклатураПоДокументам.Сделка,
	...
	|		ПО ВТ_НоменклатураПоДокументам.Сделка = ПоступлениеТоваровУслугОборудование.Ссылка
	|			И ВТ_НоменклатураПоДокументам.Номенклатура = ПоступлениеТоваровУслугОборудование.Номенклатура
	|ГДЕ
	|	НЕ ПоступлениеТоваровУслугОборудование.Ссылка ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ // Возвраты по сделке по строкам
	|	ВТ_НоменклатураПоДокументам.Сделка,
	...
	|		ПО ВТ_НоменклатураПоДокументам.Сделка = ВозвратТоваровПоставщикуТовары.Сделка
	|			И ВТ_НоменклатураПоДокументам.Номенклатура = ВозвратТоваровПоставщикуТовары.Номенклатура
	|ГДЕ
	|	НЕ ВозвратТоваровПоставщикуТовары.Ссылка ЕСТЬ NULL
	|	И ВозвратТоваровПоставщикуТовары.Ссылка <> &ТекущийДокумент
	|	И ВозвратТоваровПоставщикуТовары.Ссылка.МоментВремени < &МоментВремениТекущегоДокумента
	|	И ВозвратТоваровПоставщикуТовары.Ссылка.Дата > ВТ_НоменклатураПоДокументам.ДатаСделки
	|	И ВозвратТоваровПоставщикуТовары.Ссылка.Проведен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ  // возвраты по сделке в шапке
	|	ВТ_НоменклатураПоДокументам.Сделка,
	|	ВозвратТоваровПоставщикуТовары.Номенклатура,
	|	-ВозвратТоваровПоставщикуТовары.Количество
	|ИЗ
	...
	|		ПО ВТ_НоменклатураПоДокументам.ДатаСделки < ВозвратТоваровПоставщику.Дата
	|			И ВТ_НоменклатураПоДокументам.Сделка = ВозвратТоваровПоставщику.Сделка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщикуТовары
	|		ПО ВТ_НоменклатураПоДокументам.Номенклатура = ВозвратТоваровПоставщикуТовары.Номенклатура
	|			И (ВозвратТоваровПоставщику.Ссылка = ВозвратТоваровПоставщикуТовары.Ссылка)
	|ГДЕ
	|	НЕ ВозвратТоваровПоставщику.Ссылка ЕСТЬ NULL
	|	И ВозвратТоваровПоставщику.Ссылка <> &ТекущийДокумент
	|	И ВозвратТоваровПоставщику.МоментВремени < &МоментВремениТекущегоДокумента
	...
	|ВЫБРАТЬ
	|	ВТ_НоменклатураПоДокументам.Сделка,
	|	ВТ_НоменклатураПоДокументам.Номенклатура,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияТовары.КоличествоДоКорректировки <> 0
	|			ТОГДА КорректировкаПоступленияТовары.Количество - КорректировкаПоступленияТовары.КоличествоДоКорректировки
	|		ИНАЧЕ КорректировкаПоступленияТовары.Количество - КорректировкаПоступленияТовары.КоличествоДоИзменения
	|	КОНЕЦ
	|ИЗ
	|	ВТ_НоменклатураПоДокументам КАК ВТ_НоменклатураПоДокументам
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПоступления КАК КорректировкаПоступления
	|		ПО ВТ_НоменклатураПоДокументам.Сделка = КорректировкаПоступления.ДокументПоступления
	|			И ВТ_НоменклатураПоДокументам.ДатаСделки < КорректировкаПоступления.Дата
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаПоступления.Товары КАК КорректировкаПоступленияТовары
	|		ПО ВТ_НоменклатураПоДокументам.Номенклатура = КорректировкаПоступленияТовары.Номенклатура
	|			И (КорректировкаПоступления.Ссылка = КорректировкаПоступленияТовары.Ссылка)
	|ГДЕ
	|	НЕ КорректировкаПоступления.Ссылка ЕСТЬ NULL
	|	И КорректировкаПоступления.МоментВремени < &МоментВремениТекущегоДокумента
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПоступленияВозвраты.Сделка КАК Сделка,
	|	ВТ_ПоступленияВозвраты.Номенклатура КАК Номенклатура,
	|	СУММА(ВТ_ПоступленияВозвраты.Количество) КАК Количество
	|ИЗ
	|	ВТ_ПоступленияВозвраты КАК ВТ_ПоступленияВозвраты
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ПоступленияВозвраты.Номенклатура,
	|	ВТ_ПоступленияВозвраты.Сделка";
	
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	Пока РезультатЗапроса.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ТаблицаРезультата.Добавить(), РезультатЗапроса);
	КонецЦикла; 
...
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
	...
	|	И НЕ ВозвратТоваровПоставщику.КорректировочныйСчетФактураПолученОтПоставщика";
	
	Результат = Запрос.ВыполнитьПакет();
	
	// Возвраты из переработки
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ДокументыСчетФактураНеТребуются,
		ОбщегоНазначения.ВыгрузитьКолонку(Результат[1].Выгрузить(), "Ссылка"));
		
	// Возвраты комитенту или по договорам налогового агента
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ДокументыСчетФактураНеТребуются,
		ОбщегоНазначения.ВыгрузитьКолонку(Результат[2].Выгрузить(), "Ссылка"));
		
	// Возвраты с КСФ полученной от поставшика
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ДокументыСчетФактураНеТребуются,
		ОбщегоНазначения.ВыгрузитьКолонку(Результат[3].Выгрузить(), "Ссылка"));
	
	// Прочие возвраты
	ВыборкаВозврат = Результат[4].Выбрать();
	...
	Если Не ЗначениеЗаполнено(ВидОперации) Тогда
		...
		Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
			ВидОперации = РаботаСДоговорамиКонтрагентовБПВызовСервера.ВидОперацииПоДоговору(
				ДоговорКонтрагента, Тип("ДокументСсылка.ВозвратТоваровПоставщику"));
		КонецЕсли;
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	МетаданныеРеквизитСделка = ДокументСсылка.Метаданные().ТабличныеЧасти.Товары.Реквизиты.Сделка;
	
	МассивПустыхЗначенийСделка = ОбщегоНазначенияБП.МассивПустыхЗначенийРеквизита(МетаданныеРеквизитСделка);
	
	Запрос.УстановитьПараметр("МассивПустыхЗначенийСделка", МассивПустыхЗначенийСделка);
	
	НомераТаблиц = Новый Структура;
	Запрос.Текст = ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение);
	Результат    = Запрос.ВыполнитьПакет();
	ТаблицаРеквизиты = Результат[НомераТаблиц["Реквизиты"]].Выгрузить();
	ТаблицаРеквизиты.Колонки.Добавить("РасчетКорректировокСУчетомРекомендацийПоНДС", Новый ОписаниеТипов("Булево"));
	ТаблицаРеквизиты[0].РасчетКорректировокСУчетомРекомендацийПоНДС = УчетВзаиморасчетов.ПрименяетсяРасчетКорректировокВВалютеПоКурсуНаДатуОтгрузки(ТаблицаРеквизиты[0].Организация, ТаблицаРеквизиты[0].Период);
	...
	Запрос.УстановитьПараметр("ПлательщикНДСНаУСНСВычетами", ПлательщикНДСНаУСНСВычетами);
	...
	Реквизиты.Вставить("СпособЗачетаАвансов", ?(Реквизиты.УказанаСделка ИЛИ Реквизиты.ЕстьСделкаПоСтрокам,
	...
	Запрос.УстановитьПараметр("ЕстьСделкаПоСтрокам", Реквизиты.ЕстьСделкаПоСтрокам);
	...
	Запрос.УстановитьПараметр("ЭтоДоговорСКомитентом",             ЭтоДоговорСКомитентом);
	Запрос.УстановитьПараметр("ЭтоДоговорСКомиссионеромНаЗакупку", Реквизиты.ЭтоДоговорСКомиссионеромНаЗакупку);
	...
	Запрос.УстановитьПараметр("ТаблицаВозвратнаяТара", Перечисления.ТабличныеЧастиДокументов.ВозвратнаяТара);
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц, ЭтоОтложенноеПроведение)
	...
	Иначе
		...
		|	МАКСИМУМ(СоставДокумента.ЕстьСделкаПоСтрокам) КАК ЕстьСделкаПоСтрокам,
		...
		|	ГДЕ
		|		ТаблицаДокумента.Ссылка = &Ссылка
		|		И НЕ ТаблицаДокумента.Сделка В (&МассивПустыхЗначенийСделка)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	Реквизиты.СчетУчетаРасчетовПоПретензиям КАК СчетУчетаРасчетовПоПретензиям,
	|	Реквизиты.СчетУчетаРасчетовПоТаре КАК СчетУчетаРасчетовПоТаре,
	|	ЕСТЬNULL(СоставДокумента.ЕстьСделкаПоСтрокам, ЛОЖЬ) КАК ЕстьСделкаПоСтрокам,
...
Процедура ПодготовитьТаблицыДокументаРасчетыВВалюте(Запрос, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		Если Реквизиты.РасчетКорректировокСУчетомРекомендацийПоНДС И Реквизиты.ВалютаРеглУчета <> Реквизиты.ВалютаДокумента Тогда 
		...
		Если Реквизиты.СкладНТТВЦенахПродажи Тогда
			...
			Если Реквизиты.РасчетКорректировокСУчетомРекомендацийПоНДС И Реквизиты.ВалютаРеглУчета <> Реквизиты.ВалютаДокумента Тогда 
				ПодготовитьТаблицуДокументаРасчетыВВалютеСУчетомРекомендацийПоНДС(СуммыТаблицыТоварыВРознице, Реквизиты);
			Иначе 
...
Функция ТекстЗапросаВременныеТаблицыДокументаРасчетыВВалюте(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|	ВременнаяТаблицаТовары.Сделка КАК Сделка,
...
Функция ТекстЗапросаРасчетыВВалютеТовары(Реквизиты)
	...
	|	СуммыТаблицыТовары.СуммаБезНДСРуб
...
Функция ТекстЗапросаСписаниеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику.СведенияПрослеживаемости КАК СведенияПрослеживаемости
	|ГДЕ
	...
	|	ТаблицаТовары.СебестоимостьУказанаВДокументе КАК СебестоимостьУказанаВДокументе,
	|	ВЫБОР
	...
	Иначе
		...
		|		КОГДА НЕ ТаблицаТовары.Сделка В (&МассивПустыхЗначенийСделка)
		|			ТОГДА ТаблицаТовары.Сделка
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.Организация КАК Организация,
	|	Реквизиты.ВалютаДокумента КАК ВалютаДокумента,
	|	&СпособЗачетаАвансов КАК СпособЗачетаАвансов,
	...
	|	ВЫБОР
	|		КОГДА НЕ ТаблицаСуммВзаиморасчетов.Сделка В (&МассивПустыхЗначенийСделка)
	|			ТОГДА ТаблицаСуммВзаиморасчетов.Сделка
...
Функция ТекстЗапросаКорректировкаСтоимостиТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаТовары.Количество,
	|	ВЫБОР
	|		КОГДА Реквизиты.НДСВключенВСтоимость И НЕ &ПлательщикНДСНаУСНСВычетами
...
Функция ПодготовитьТаблицуНДСВключенВСтоимость(ТаблицаТоварыНДС, ТаблицаСписанныеТовары, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если НЕ РаздельныйУчетНДС ИЛИ УпрощенныйУчетНДС ИЛИ РаздельныйУчетНДСБезВычетов Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ПодготовитьПараметрыПодготовитьТаблицуНДСВключенВСтоимость(ТаблицаРеализованныеТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
	...
	//МассивСтрокБезНДС = Параметры.РеализованныеТовары.НайтиСтроки(Новый Структура("СтавкаНДСДокумента", Перечисления.СтавкиНДС.БезНДС));
	//Для каждого СтрокаДляУдаления Из МассивСтрокБезНДС Цикл
	//	Параметры.РеализованныеТовары.Удалить(СтрокаДляУдаления);
	//КонецЦикла; 
...
Функция ПечатьТОРГ12(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Иначе
			УправлениеПечатьюБП.ДоработатьФаксимильнуюПодписьИПечать(ТабличныйДокумент, ОбъектыПечати,
				Шапка.Руководители, Шапка.Ссылка);
		КонецЕсли;
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыНакладная()
	...
	|		КОНЕЦ КАК Артикул,
	|		ВЫБОР
	|			КОГДА Номенклатура.ЕдиницаИзмерения <> Упаковка 
	|			И Упаковка <> НЕОПРЕДЕЛЕНО
	|				ТОГДА ВЫРАЗИТЬ(Количество / Упаковка.Коэффициент КАК ЧИСЛО(15,2))
	|			ИНАЧЕ Количество
	|		КОНЕЦ КАК Количество,
	|		ВЫБОР 
	|			КОГДА Номенклатура.ЕдиницаИзмерения <> Упаковка 
	|			И Упаковка <> НЕОПРЕДЕЛЕНО
	|				ТОГДА Упаковка.Наименование
	|			ИНАЧЕ Номенклатура.ЕдиницаИзмерения.Наименование
	|		КОНЕЦ КАК ЕдиницаИзмерения,
	|		ВЫБОР 
	|			КОГДА Номенклатура.ЕдиницаИзмерения <> Упаковка 
	|			И Упаковка <> НЕОПРЕДЕЛЕНО
	|				ТОГДА ВЫРАЗИТЬ(Цена * Упаковка.Коэффициент КАК ЧИСЛО(15,2))
	|			ИНАЧЕ Цена
	|		КОНЕЦ КАК Цена,
	|		Сумма,
	|		СуммаНДС
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыТОРГ12(НомераТаблиц)
	...
	|			ТОГДА """"
	|	КОНЕЦ КАК ТоварКод,
	|	ЕСТЬNULL(СправочникУпаковки.ЕдиницаИзмерения.Наименование, СправочникНоменклатура.ЕдиницаИзмерения.Наименование) КАК БазоваяЕдиницаНаименование,
	|	ЕСТЬNULL(СправочникУпаковки.ЕдиницаИзмерения.Код, СправочникНоменклатура.ЕдиницаИзмерения.Код) КАК БазоваяЕдиницаКодПоОКЕИ,
	...
	|	ВложенныйЗапрос.Цена * ЕСТЬNULL(СправочникУпаковки.Коэффициент, 1) КАК Цена,
	|	ВложенныйЗапрос.Количество / ЕСТЬNULL(СправочникУпаковки.Коэффициент, 1) КАК Количество,
	|	ВложенныйЗапрос.КоличествоМест КАК КоличествоМест,
	|	ВложенныйЗапрос.Сумма КАК Сумма,
	|	ВложенныйЗапрос.СуммаНДС КАК СуммаНДС,
	|	ВложенныйЗапрос.НомерСтроки КАК НомерСтроки,
	|	1 КАК ID,
	|	ВложенныйЗапрос.ЭтоКомиссия КАК ЭтоКомиссия,
	...
	|		ПО (ВложенныйЗапрос.Упаковка = СправочникУпаковки.Ссылка
...
Функция ПодготовитьТекстЗапросаДляПечатиСправкиРасчетаРублевыеСуммыДокументовВВалюте(НомераТаблиц) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВложенныйЗапрос.ВозвратПоНесколькимПоступлениям, ЛОЖЬ) = ЛОЖЬ
	|				И (ТИПЗНАЧЕНИЯ(ДанныеОбрабатываемогоДокумента.Сделка) <> ТИП(Документ.ПоступлениеТоваровУслуг)
	|					ИЛИ ДанныеОбрабатываемогоДокумента.Сделка ЕСТЬ NULL
	|					ИЛИ ДанныеОбрабатываемогоДокумента.Сделка = ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.ПустаяСсылка))
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|			МАКСИМУМ(ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ВозвратТоваровПоставщикуТовары.Сделка) = ТИП(Документ.ПоступлениеТоваровУслуг)
	|							И НЕ ВозвратТоваровПоставщикуТовары.Сделка ЕСТЬ NULL
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ) КАК ВозвратПоНесколькимПоступлениям,
	|			ВозвратТоваровПоставщикуТовары.Ссылка КАК Ссылка,
	|			ВозвратТоваровПоставщикуТовары.Ссылка.Сделка КАК Сделка
	|		ИЗ
	|			Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщикуТовары
	|		ГДЕ
	...
	|			ВозвратТоваровПоставщикуТовары.Ссылка.Сделка) КАК ВложенныйЗапрос
	|		ПО ДанныеОбрабатываемогоДокумента.Ссылка = ВложенныйЗапрос.Ссылка
	|ГДЕ
...
Функция ДокументыСПрослеживаемымиТоварами(МассивДокументов) Экспорт
	...
	|	И ВозвратТоваровПоставщикуТовары.ПрослеживаемыйТовар = ИСТИНА";
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ЭтоВозвратИзПереработки Или Не Реквизиты.ЕстьТовары Тогда
	...
	|				И НЕ &ЭтоДоговорСКомиссионеромНаЗакупку
	|			ТОГДА ВЫБОР
	|					КОГДА &ЕстьСделкаПоСтрокам
	|						ТОГДА ВозвратТоваровПоставщикуТовары.Сделка
	|					ИНАЧЕ ВозвратТоваровПоставщику.Сделка
	|				КОНЕЦ
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|			ТОГДА ВозвратТоваровПоставщику.Контрагент
	|		ИНАЧЕ ВозвратТоваровПоставщикуТовары.Контрагент
	|	КОНЕЦ КАК Комитент,
	|	ВЫБОР
	...
	|						КОНЕЦ
	|				КОГДА ВозвратТоваровПоставщикуТовары.Контрагент = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.УКДНаУменьшениеПолученный)
	|				ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПолучениеКомиссионеромКорректировкиВСторонуУменьшения)
	|			КОНЕЦ
	|	КОНЕЦ КАК КодОперации,
	|	ВЫБОР
	|		КОГДА &ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС
	|			ТОГДА ВЫБОР
	|					КОГДА ВозвратТоваровПоставщикуТовары.СчетУчета.Забалансовый
	...
	|	ВозвратТоваровПоставщикуСведенияПрослеживаемости.Номенклатура КАК Комплектующие,
	|	ВозвратТоваровПоставщикуСведенияПрослеживаемости.Сумма КАК СуммаБезНДС,
	|	ВозвратТоваровПоставщикуТовары.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	...
	|		ПО ВозвратТоваровПоставщикуТовары.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И ВозвратТоваровПоставщикуТовары.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	...
	|	ВозвратТоваровПоставщикуСведенияПрослеживаемости.Номенклатура,
	|	ВозвратТоваровПоставщикуСведенияПрослеживаемости.Сумма,
	|	ВозвратТоваровПоставщикуТовары.ПрослеживаемыйКомплект
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ВозвратТоваровПоставщикуТовары.НомерСтроки = ТаблицаТовары.НомерСтроки
	|ГДЕ
	|	ВозвратТоваровПоставщикуТовары.Ссылка = &Ссылка
	|	И ВозвратТоваровПоставщикуТовары.ПрослеживаемыйТовар = ИСТИНА
	|;
...
Функция ТекстЗапросаТаблицаПрослеживаемыхТоваровПоОснованию() Экспорт
	...
	|	СУММА(ВЫБОР
	|			КОГДА ВозвратТоваровПоставщику.СуммаВключаетНДС
	...
	|		КОНЕЦ) КАК Сумма,
	|	ВозвратТоваровПоставщику.Организация КАК Организация,
	|	ВозвратТоваровПоставщику.Дата КАК ПериодСобытия,
	|	ВозвратТоваровПоставщикуТовары.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВозвратТоваровПоставщику.Ссылка КАК Ссылка,
	|	ВозвратТоваровПоставщикуТовары.ИдентификаторСтроки КАК ИдентификаторСтрокиТаблицыТовары,
	|	ВЫБОР
	|		КОГДА ВозвратТоваровПоставщикуТовары.Сделка = НЕОПРЕДЕЛЕНО
	|			ТОГДА ВозвратТоваровПоставщику.Сделка
	|		ИНАЧЕ ВозвратТоваровПоставщикуТовары.Сделка
	...
	|	И ВозвратТоваровПоставщикуТовары.ПрослеживаемыйТовар
	|	И ВозвратТоваровПоставщику.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ВозвратТоваровПоставщикуТовары.Номенклатура,
	...
	|				ТОГДА 0
	|			ИНАЧЕ ВТ_ТаблицаТоваровВременная.Сумма / ВТ_ТаблицаТоваровВременная.Количество * ВозвратТоваровПоставщикуСведенияПрослеживаемости.Количество
	|		КОНЕЦ) КАК Сумма,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику.СведенияПрослеживаемости КАК ВозвратТоваровПоставщикуСведенияПрослеживаемости
	|		ПО ВТ_ТаблицаТоваровВременная.Ссылка = ВозвратТоваровПоставщикуСведенияПрослеживаемости.Ссылка
	|			И ВТ_ТаблицаТоваровВременная.ИдентификаторСтрокиТаблицыТовары = ВозвратТоваровПоставщикуСведенияПрослеживаемости.ИдентификаторСтроки
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ПодготовитьТаблицуДокументаРасчетыВВалютеСУчетомРекомендацийПоНДС(СуммыТаблицыТовары, Реквизиты)
	
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	|	СРЕДНЕЕ(ЕСТЬNULL(ПоступлениеТоваровУслуг.СуммаДокумента, 1)) КАК СуммаДокумента,
	|	РублевыеСуммыДокументовВВалюте.Регистратор КАК Сделка,
	...
	|	РегистрСведений.РублевыеСуммыДокументовВВалюте КАК РублевыеСуммыДокументовВВалюте
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	|		ПО РублевыеСуммыДокументовВВалюте.Регистратор = ПоступлениеТоваровУслуг.Ссылка
	|ГДЕ
	|	РублевыеСуммыДокументовВВалюте.Регистратор В(&ДокументОснование)
	|	И РублевыеСуммыДокументовВВалюте.ТабличнаяЧастьДокумента <> &ТаблицаВозвратнаяТара
	|
	|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ТаблицаВозвратнаяТара", Перечисления.ТабличныеЧастиДокументов.ВозвратнаяТара);
	Если Реквизиты.ЕстьСделкаПоСтрокам Тогда
		Запрос.УстановитьПараметр("ДокументОснование", СуммыТаблицыТовары.ВыгрузитьКолонку("Сделка"));
	Иначе
		Запрос.УстановитьПараметр("ДокументОснование", Реквизиты.Сделка);
	КонецЕсли;

	РублевыеСуммы = Запрос.Выполнить().Выгрузить();
	Для каждого Строка Из СуммыТаблицыТовары Цикл 
		ОтобранныеРублевыеСуммы = РублевыеСуммы.Найти(?(Реквизиты.ЕстьСделкаПоСтрокам, Строка.Сделка, Реквизиты.Сделка),
		"Сделка");
		Если ОтобранныеРублевыеСуммы <> Неопределено Тогда
			СреднийКурс          = ОтобранныеРублевыеСуммы.Всего / ОтобранныеРублевыеСуммы.СуммаДокумента;
			КурсПоступления      = ОтобранныеРублевыеСуммы.КурсВзаиморасчетов;
			КратностьПоступления = ОтобранныеРублевыеСуммы.КратностьВзаиморасчетов;
		Иначе
			СреднийКурс = ?(Реквизиты.КратностьВзаиморасчетов = 0, 0, 
			Реквизиты.КурсВзаиморасчетов / Реквизиты.КратностьВзаиморасчетов);
			КурсПоступления      = Реквизиты.КурсВзаиморасчетов;
			КратностьПоступления = Реквизиты.КратностьВзаиморасчетов;
		КонецЕсли;
		КратностьПоступления = ?(КратностьПоступления = 0, 1, КратностьПоступления);
		
		Строка.СуммаРуб = Окр(Строка.СуммаРуб * СреднийКурс, 2);
		...
		Иначе
			Строка.СуммаНДСРуб = Окр((Строка.СуммаВзаиморасчетов * КурсПоступления)/КратностьПоступления /(100 + ПроцентНДС)* ПроцентНДС ,2);
			Строка.СуммаБезНДСРуб = Окр(Строка.СуммаВзаиморасчетов * КурсПоступления / КратностьПоступления /(100 + ПроцентНДС)* 100, 2);
		КонецЕсли;
	КонецЦикла;
