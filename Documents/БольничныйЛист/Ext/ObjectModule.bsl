...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(СпособРасчетовСФизическимиЛицами) И Не ДоступноИзменениеСпособаРасчетов() Тогда
		...
		ПодразделениеУчетаЗатрат = Справочники.ПодразделенияОрганизаций.ПустаяСсылка();
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(СлучайУходаЗаБольнымРебенком) Тогда
		ТаблицаСлучаевУхода = Перечисления.СлучаиУходаЗаБольнымиДетьми.ТаблицаВыбора(ДатаНачалаСобытия);
		Если ТаблицаСлучаевУхода.Найти(СлучайУходаЗаБольнымРебенком) = Неопределено Тогда
			ТекстСообщения = НСтр("ru = 'Выбранная причина нетрудоспособности не может быть использована в указанном периоде.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "ПричинаНетрудоспособностиТекст", , Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоПервичномуБольничномуЛисту(ПервичныйБольничный) Экспорт
	...
	Иначе
		...
		|ДатаНачалаСобытия, СлучайУходаЗаБольнымРебенком, КодПричиныНетрудоспособности,
		...
		|КодНарушенияРежима, СтажЛет, СтажМесяцев";
...
Функция ЗначенияРеквизитовДоЗаписи()
	ИменаРеквизитов = "Организация, ФизическоеЛицо, НомерЛисткаНетрудоспособности, ПометкаУдаления, ГоловнаяОрганизация";
	Если ЭтоНовый() Тогда
