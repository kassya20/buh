...
Функция ДатаНачалаОплатыЗаСчетСФР(Больничный) Экспорт
	Если Больничный.СдвигатьПериодОплаты И Больничный.ИсключаемыеПериоды.Количество() > 0 Тогда
		...
		Если Больничный.Начисления.Количество() > 0 И Не Больничный.ЯвляетсяПродолжениемБолезни Тогда
			...
			Отбор = Новый Структура("Начисление", ВидОплатыЗаСчетРаботодателя(Больничный, Кэш));
...
Функция ДанныеДляРегистрацииВУчетаСтажаПФР(МассивСсылок) Экспорт
	...
	|ИсключаемыеПериоды, ПериодыПростоя, ПериодыУходаЗаРодственниками,
	...
	Пока Выборка.Следующий() Цикл
		...
		Больничный.ПериодыПростоя               = Выборка.ПериодыПростоя.Выгрузить();
...
Процедура ЗаполнитьОтветНаЗапросФССДляРасчетаПособия(БольничныйСсылка, ДокументСЭДО, Реквизиты, ДанныеДляРасчетаСреднегоЗаработка) Экспорт
	...
	Реквизиты.Вставить("ДатаНачала",          Больничный.ДатаНачала);
	Реквизиты.Вставить("ДатаОкончания",       Больничный.ДатаОкончания);
	...
	Кэш = Новый Соответствие;
	ВидОплатыПособия = ВидОплатыПособия(Больничный, Кэш);
	ЕстьОплатаЗаСчетСФР = Больничный.Начисления.Найти(ВидОплатыПособия, "Начисление") <> Неопределено;
	...
	Реквизиты.Вставить("ОтказВНазначенииПособия", Не ЕстьОплатаЗаСчетСФР);
	Реквизиты.Вставить("ОтказВНазначенииПособияПричина", "");
	Если Не ЕстьОплатаЗаСчетСФР Тогда
	...
	Если ЕстьОплатаЗаСчетСФР Тогда
		...
		Фильтр = Новый Структура("ВидРасчета", ВидОплатыЗаСчетРаботодателя(Больничный, Кэш));
	...
	Если ЗначениеЗаполнено(Больничный.ПериодРасчетаСреднегоЗаработкаПервыйГод)
...
Процедура ПриПолученииОсновныхСведенийОДокументеДляСЭДО(БольничныйСсылка, ДокументСЭДО, ОсновныеСведения) Экспорт
	ИменаРеквизитов = "Представление, Проведен, ГоловнаяОрганизация, Организация, Сотрудник, ФизическоеЛицо";
...
Функция ПредставлениеПричиныЗаболевания(ДатаНачалаСобытия, СлучайУходаЗаБольнымРебенком = Неопределено) Экспорт
	...
	ТаблицаЗаболеваний = Новый ТаблицаЗначений;
	ТаблицаЗаболеваний.Колонки.Добавить("Наименование",                 Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(120)));
	ТаблицаЗаболеваний.Колонки.Добавить("ПричинаНетрудоспособности",    Новый ОписаниеТипов("ПеречислениеСсылка.ПричиныНетрудоспособности"));
	ТаблицаЗаболеваний.Колонки.Добавить("СлучайУходаЗаБольнымРебенком", Новый ОписаниеТипов("ПеречислениеСсылка.СлучаиУходаЗаБольнымиДетьми"));
	ТаблицаЗаболеваний.Колонки.Добавить("КодПричиныНетрудоспособности", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(2)));
	ТаблицаЗаболеваний.Колонки.Добавить("Порядок",                      Новый ОписаниеТипов("Число"));
	ТаблицаЗаболеваний.Колонки.Добавить("Недоступен",                   Новый ОписаниеТипов("Булево"));
	
	ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет = УчетПособийСоциальногоСтрахования.ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет(ДатаНачалаСобытия);
	ДействуетПорядокНСПЗ_2022 = УчетПособийСоциальногоСтрахования.ДействуетПорядокНСПЗ_2022(ДатаНачалаСобытия);
	
	Для Каждого СтрокаТаблицы ИЗ Результат Цикл
		
		Если СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ОбщееЗаболевание Тогда
			
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(01) Заболевание'");
			НоваяСтрока.КодПричиныНетрудоспособности = "01";
			НоваяСтрока.Порядок                      = 10;
			
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(02) Травма'");
			НоваяСтрока.КодПричиныНетрудоспособности = "02";
			НоваяСтрока.Порядок                      = 20;
			
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(10) Иное состояние (отравление, проведение манипуляций и др.)'");
			НоваяСтрока.КодПричиныНетрудоспособности = "10";
			НоваяСтрока.Порядок                      = 100;
			
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(11) Заболевание туберкулезом'");
			НоваяСтрока.КодПричиныНетрудоспособности = "11";
		...
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.Карантин Тогда
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(03) Карантин'");
			НоваяСтрока.КодПричиныНетрудоспособности = "03";
		...
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ТравмаНаПроизводстве Тогда
			Если ДействуетПорядокНСПЗ_2022 Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(02) Несчастный случай на производстве или его последствия'");
				НоваяСтрока.КодПричиныНетрудоспособности = "02";
				НоваяСтрока.Порядок                      = 21;
			Иначе
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(04) Несчастный случай на производстве или его последствия'");
				НоваяСтрока.КодПричиныНетрудоспособности = "04";
				НоваяСтрока.Порядок                      = 21;
			КонецЕсли;
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам Тогда
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(05) Отпуск по беременности и родам'");
			НоваяСтрока.КодПричиныНетрудоспособности = "05";
		...
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.Протезирование Тогда
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(06) Протезирование в стационаре'");
			НоваяСтрока.КодПричиныНетрудоспособности = "06";
			НоваяСтрока.Порядок                      = 60;
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.Профзаболевание Тогда
			Если ДействуетПорядокНСПЗ_2022 Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(01) Профессиональное заболевание или его обострение'");
				НоваяСтрока.КодПричиныНетрудоспособности = "01";
				НоваяСтрока.Порядок                      = 11;
			Иначе
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(07) Профессиональное заболевание или его обострение'");
				НоваяСтрока.КодПричиныНетрудоспособности = "07";
				НоваяСтрока.Порядок                      = 70;
			КонецЕсли;
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПособиеПриДолечивании Тогда
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(08) Долечивание в санатории'");
			НоваяСтрока.КодПричиныНетрудоспособности = "08";
			НоваяСтрока.Порядок                      = 80;
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоУходуЗаВзрослым Тогда
			НоваяСтрока = ТаблицаЗаболеваний.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
			НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным взрослым членом семьи'");
			НоваяСтрока.КодПричиныНетрудоспособности = "09";
			НоваяСтрока.Порядок                      = 97;
		ИначеЕсли СтрокаТаблицы.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоУходуЗаРебенком Тогда
			Если СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетАмбулаторно Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком до 7 лет, амбулаторно'");
				НоваяСтрока.КодПричиныНетрудоспособности = "09";
				НоваяСтрока.Порядок                      = 91;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетВСтационаре Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком до 7 лет, в стационаре'");
				НоваяСтрока.КодПричиныНетрудоспособности = "09";
				НоваяСтрока.Порядок                      = 92;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До7ПолныхЛетАмбулаторно Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком 7 лет, амбулаторно'");
					НоваяСтрока.КодПричиныНетрудоспособности = "09";
					НоваяСтрока.Порядок                      = 93;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До7ПолныхЛетВСтационаре Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком 7 лет, в стационаре'");
					НоваяСтрока.КодПричиныНетрудоспособности = "09";
					НоваяСтрока.Порядок                      = 94;
					НоваяСтрока.Недоступен = Не ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетАмбулаторно Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком от 7 до 15 лет, амбулаторно'");
				НоваяСтрока.КодПричиныНетрудоспособности = "09";
				НоваяСтрока.Порядок                      = 95;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетВСтационаре Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(09) Уход за больным ребенком от 7 до 15 лет, в стационаре'");
				НоваяСтрока.КодПричиныНетрудоспособности = "09";
				НоваяСтрока.Порядок                      = 96;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетПриОсобомЗаболеванииАмбулаторно Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(12) Уход за больным ребенком до 7 лет, амбулаторно, заболевание включено в Перечень Минздравсоцразвития'");
				НоваяСтрока.КодПричиныНетрудоспособности = "12";
				НоваяСтрока.Порядок                      = 121;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо7миЛетПриОсобомЗаболеванииВСтационаре Тогда
				НоваяСтрока = ТаблицаЗаболеваний.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.Наименование                 = НСтр("ru = '(12) Уход за больным ребенком до 7 лет, в стационаре, заболевание включено в Перечень Минздравсоцразвития'");
				НоваяСтрока.КодПричиныНетрудоспособности = "12";
				НоваяСтрока.Порядок                      = 122;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ИнвалидомДо8ЛетАмбулаторно Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(13) Уход за больным ребенком-инвалидом до 8 лет, амбулаторно'");
					НоваяСтрока.КодПричиныНетрудоспособности = "13";
					НоваяСтрока.Порядок                      = 125;
					НоваяСтрока.Недоступен = Не ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ИнвалидомДо8ЛетВСтационаре Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(13) Уход за больным ребенком-инвалидом до 8 лет, в стационаре'");
					НоваяСтрока.КодПричиныНетрудоспособности = "13";
					НоваяСтрока.Порядок                      = 126;
					НоваяСтрока.Недоступен = Не ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуЗаРебенкомИнвалидомАмбулаторно Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КодПричиныНетрудоспособности = "13";
				НоваяСтрока.Порядок                      = 131;
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Тогда
					НоваяСтрока.Наименование = НСтр("ru = '(13) Уход за больным ребенком-инвалидом от 8 до 18 лет, амбулаторно'");
				Иначе
					НоваяСтрока.Наименование = НСтр("ru = '(13) Уход за больным ребенком-инвалидом до 18 лет, амбулаторно (до 2015 г. - до 15 лет)'");
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуЗаРебенкомИнвалидомВСтационаре Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КодПричиныНетрудоспособности = "13";
				НоваяСтрока.Порядок                      = 132;
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Тогда
					НоваяСтрока.Наименование = НСтр("ru = '(13) Уход за больным ребенком-инвалидом от 8 до 18 лет, в стационаре'");
				Иначе
					НоваяСтрока.Наименование = НСтр("ru = '(13) Уход за больным ребенком-инвалидом до 18 лет, в стационаре (до 2015 г. - до 15 лет)'");
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До8ЛетПриПоствакцинальномОсложненииАмбулаторно Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(14) Уход за больным ребенком до 8 лет при поствакцинальном осложнении, амбулаторно'");
					НоваяСтрока.КодПричиныНетрудоспособности = "14";
					НоваяСтрока.Порядок                      = 135;
					НоваяСтрока.Недоступен = Не ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До8ЛетПриПоствакцинальномОсложненииВСтационаре Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(14) Уход за больным ребенком до 8 лет при поствакцинальном осложнении, в стационаре'");
					НоваяСтрока.КодПричиныНетрудоспособности = "14";
					НоваяСтрока.Порядок                      = 136;
					НоваяСтрока.Недоступен = Не ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет;
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетПриПоствакцинальномОсложненииАмбулаторно Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КодПричиныНетрудоспособности = "14";
				НоваяСтрока.Порядок                      = 141;
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Тогда
					НоваяСтрока.Наименование = НСтр("ru = '(14) Уход за больным ребенком от 8 до 18 лет при поствакцинальном осложнении, амбулаторно'");
				Иначе
					НоваяСтрока.Наименование = НСтр("ru = '(14) Уход за больным ребенком до 18 лет при поствакцинальном осложнении, амбулаторно (до 2015 г. - до 15 лет)'");
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетПриПоствакцинальномОсложненииВСтационаре Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КодПричиныНетрудоспособности = "14";
				НоваяСтрока.Порядок                      = 142;
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Тогда
					НоваяСтрока.Наименование = НСтр("ru = '(14) Уход за больным ребенком от 8 до 18 лет при поствакцинальном осложнении, в стационаре'");
				Иначе
					НоваяСтрока.Наименование = НСтр("ru = '(14) Уход за больным ребенком до 18 лет при поствакцинальном осложнении, в стационаре (до 2015 г. - до 15 лет)'");
				КонецЕсли;
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.До8ЛетПриВИЧ Тогда
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Или СлучайУходаЗаБольнымРебенком = СтрокаТаблицы.СлучайУходаЗаБольнымРебенком Тогда
					НоваяСтрока = ТаблицаЗаболеваний.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
					НоваяСтрока.Наименование                 = НСтр("ru = '(15) Уход за больным ребенком до 8 лет, ВИЧ-инфицированным, в стационаре'");
					НоваяСтрока.КодПричиныНетрудоспособности = "15";
			...
			ИначеЕсли СтрокаТаблицы.СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетПриВИЧ Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
				НоваяСтрока.КодПричиныНетрудоспособности = "15";
				НоваяСтрока.Порядок                      = 151;
				Если ДействуетПолнаяОплатаПоУходуЗаРебенкомДо8Лет Тогда
					НоваяСтрока.Наименование = НСтр("ru = '(15) Уход за больным ребенком от 8 до 18 лет, ВИЧ-инфицированным, в стационаре'");
				Иначе
					НоваяСтрока.Наименование = НСтр("ru = '(15) Уход за больным ребенком до 18 лет, ВИЧ-инфицированным, в стационаре (до 2015 г. - до 15 лет)'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ТаблицаЗаболеваний.Сортировать("Порядок");
	
	Возврат ТаблицаЗаболеваний;
	...
	Если Больничный.СдвигатьПериодОплаты Тогда
		Если Больничный.ЯвляетсяПродолжениемБолезни Тогда
			КоличествоДнейЗаСчетРаботодателя = ОстатокДнейЗаСчетРаботодателяРекурсивно(
				...
				ВидОплатыЗаСчетРаботодателя(Больничный, Кэш));
		КонецЕсли;
	Иначе
...
Функция ОстатокДнейЗаСчетРаботодателяРекурсивно(ПервичныйБольничныйЛист, КоличествоДнейЗаСчетРаботодателя,
		ДатаНачалаОплатыЗаСчетСФР, ВидОплатыЗаСчетРаботодателя)
	Если Не ЗначениеЗаполнено(ПервичныйБольничныйЛист) Тогда
	...
	Запрос.Текст = 
		...
		|			И БольничныйЛистНачисления.Начисление = &ВидОплатыЗаСчетРаботодателя
		|ГДЕ
		|	БольничныйЛист.Ссылка = &ПервичныйБольничныйЛист
		|
		|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ПервичныйБольничныйЛист", ПервичныйБольничныйЛист);
	Запрос.УстановитьПараметр("ВидОплатыЗаСчетРаботодателя", ВидОплатыЗаСчетРаботодателя);
	...
	Если Выборка.Следующий() Тогда
		...
		Если Выборка.ЯвляетсяПродолжениемБолезни Тогда
			Возврат ОстатокДнейЗаСчетРаботодателяРекурсивно(
				Выборка.ПервичныйБольничныйЛист, КоличествоДнейЗаСчетРаботодателя, ДатаНачалаОплатыЗаСчетСФР, ВидОплатыЗаСчетРаботодателя);
		КонецЕсли;
	КонецЕсли;
	Возврат КоличествоДнейЗаСчетРаботодателя;
КонецФункции
...
Функция ПериодыНачислений(Больничный, Кэш) Экспорт
	...
	Измерения = "ВидРасчета, Категория, ПоловинаОплаты, НарушениеРежима, СреднийПростоя";
	ПериодыНачислений = ОбщегоНазначенияБЗК.ТаблицаПериодовИзТаблицыДат(ДатыНачислений, Измерения);
	ПериодыНачислений.Сортировать("ДатаНачала");
...
Функция ВидОплатыПрофзаболевание(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыПрофзаболевание"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоПрофзаболевание;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			МодульРасчетЗарплатыДляНебольшихОрганизаций.ДополнитьОтборНачислений(Отбор, Больничный.Организация, Больничный.ПериодРегистрации);
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыПрофзаболевание", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции

Функция ВидОплатыПрофзаболеваниеРК(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыПрофзаболеваниеРК"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоПрофзаболевание;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Отбор.Вставить("ОсновнойВидРасчета", ВидОплатыПрофзаболевание(Больничный, Кэш));
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			МодульРасчетЗарплатыДляНебольшихОрганизаций.ДополнитьОтборНачислений(Отбор, Больничный.Организация, Больничный.ПериодРегистрации);
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыПрофзаболеваниеРК", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции

Функция ВидОплатыПрофзаболеваниеСН(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыПрофзаболеваниеСН"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоПрофзаболевание;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Отбор.Вставить("ОсновнойВидРасчета", ВидОплатыПрофзаболевание(Больничный, Кэш));
		...
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыПрофзаболеваниеСН", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции

Функция ВидОплатыНесчастныйСлучайНаПроизводстве(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыНесчастныйСлучайНаПроизводстве"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоНесчастныйСлучайНаПроизводстве;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			МодульРасчетЗарплатыДляНебольшихОрганизаций.ДополнитьОтборНачислений(Отбор, Больничный.Организация, Больничный.ПериодРегистрации);
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыНесчастныйСлучайНаПроизводстве", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции

Функция ВидОплатыНесчастныйСлучайНаПроизводствеРК(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыНесчастныйСлучайНаПроизводствеРК"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоНесчастныйСлучайНаПроизводстве;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Отбор.Вставить("ОсновнойВидРасчета", ВидОплатыНесчастныйСлучайНаПроизводстве(Больничный, Кэш));
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			МодульРасчетЗарплатыДляНебольшихОрганизаций.ДополнитьОтборНачислений(Отбор, Больничный.Организация, Больничный.ПериодРегистрации);
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыНесчастныйСлучайНаПроизводствеРК", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции

Функция ВидОплатыНесчастныйСлучайНаПроизводствеСН(Больничный, Кэш) Экспорт
	Начисление = Кэш["ВидОплатыНесчастныйСлучайНаПроизводствеСН"];
	Если Начисление = Неопределено Тогда
		Категория = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоНесчастныйСлучайНаПроизводстве;
		Отбор = Новый Структура("КатегорияНачисленияИлиНеоплаченногоВремени", Категория);
		Отбор.Вставить("ОсновнойВидРасчета", ВидОплатыНесчастныйСлучайНаПроизводстве(Больничный, Кэш));
		...
		КонецЕсли;
		Начисление = ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор);
		Кэш.Вставить("ВидОплатыНесчастныйСлучайНаПроизводствеСН", Начисление);
	КонецЕсли;
	Возврат Начисление;
КонецФункции
...
Функция АдресПараметровВХранилище(Объект, ДополнительныеПараметры) Экспорт
	...
	ПараметрыРасчетаСреднегоЗаработка.Вставить("СериализованныйОбъект",        ЗарплатаКадры.СериализоватьОбъектВДвоичныеДанные(Объект));
	...
	ПараметрыРасчетаСреднегоЗаработка.Вставить("ИспользоватьПрямыеВыплатыФСС", ДополнительныеПараметры.ИспользоватьПрямыеВыплатыФСС);
...
Процедура РассчитатьНачисления(Объект, ДополнительныеПараметры) Экспорт
	...
	ИспользоватьПрямыеВыплатыФСС = ДополнительныеПараметры.ИспользоватьПрямыеВыплатыФСС;
	ВидНеоплачиваемогоВремени    = ВидНеоплачиваемогоВремени(Объект, ДополнительныеПараметры);
	...
	ТаблицаРасчета.Колонки.Добавить("СреднийПростоя",  ОбщегоНазначения.ОписаниеТипаЧисло(8, 2));
	...
	РассчитыватьДолиРКСН = Не Перечисления.ПричиныНетрудоспособности.ЭтоУходЗаРодственником(Объект.ПричинаНетрудоспособности)
		И Не Объект.ПричинаНетрудоспособности = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам;
	...
	Для Каждого Период Из ПериодыНачислений Цикл
		...
		Пока ДатаНачала <= ДатаОкончания Цикл
			...
			СтрокаНачислений.СреднийПростоя = Период.СреднийПростоя;
	...
	Для НомерСтроки = 0 По ТаблицаРасчета.Количество() - 1 Цикл
		...
		СтрокаСДанными.Дополнительно.Вставить("СреднийПростоя", СтрокаТаблицыРасчета.СреднийПростоя);
		
	КонецЦикла;
	
	ПараметрыРасчета = Новый Структура("ПараметрыРасчетаПособияПоНетрудоспособности");
	ПараметрыРасчета.ПараметрыРасчетаПособияПоНетрудоспособности = ПараметрыОплаты(Объект);
...
Процедура ПересчитатьНДФЛ(Объект, ОбъектФормы) Экспорт
	...
	УчетЗарплаты.ПересчитатьНДФЛ(Объект, ОбъектФормы);
	
КонецПроцедуры

Функция ПараметрыОплаты(Объект)
