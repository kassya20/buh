	...
	Если Не Отказ Тогда
		...
		ДоступноУказаниеПроизводственнойПлощадки =
			(Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БАДы")
				...
				Или Объект.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.СпортивноеПитание"))
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.Операция = ПредопределенноеЗначение("Перечисление.ВидыОперацийИСМП.ВводВОборотПроизводствоВнеЕАЭС")
		И ТребуетсяУказатьДанныеДекларации(ЭтотОбъект)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПроверки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.ЗначениеШтрихкодаУпаковки)
			...
			Если КэшДанныхЗапроса[СтрокаТаблицы.ЗначениеШтрихкодаУпаковки] = Неопределено Тогда
				...
				КонецЕсли;
				СтруктураЗначения = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(ЗначениеШтрихкода, ВидМаркируемойПродукции);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	ЭтоПарфюмерия                = ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия")
	...
	ДоступноУказаниеСодержанияЭтиловогоСпирта = ПрисутствуетОперацияНанесения И (ЭтоПиво Или ЭтоПарфюмерия);
	...
	Элементы.ГруппаПраваяКолонкаДанныеДекларации.Видимость = ЭтоПроизводствоВнеЕАЭС Или ЭтоИмпортСФТС;
	Элементы.ГруппаТаможенныйОрганИРешение.Видимость       = ЭтоПроизводствоВнеЕАЭС;
	...
	Если ТребуетсяУказатьДанныеДекларации Тогда
		...
		Элементы.ПринятоеРешениеПолеВвода.АвтоОтметкаНезаполненного       = ЭтоПроизводствоВнеЕАЭС;
		Элементы.КодТаможенногоОргана.АвтоОтметкаНезаполненного           = ЭтоПроизводствоВнеЕАЭС;
	Иначе
	...
	Элементы.СодержаниеЭтиловогоСпирта.Видимость      = ДоступноУказаниеСодержанияЭтиловогоСпирта И ОтчетПроизводственнойЛинии;
	Элементы.ТоварыКодТНВЭД.Видимость                 = ЭтоВводВОборот;
	
	Элементы.ТоварыСодержаниеЭтиловогоСпирта.Видимость = ДоступноУказаниеСодержанияЭтиловогоСпирта И Не ОтчетПроизводственнойЛинии;
	...
	ОбязательноеУказаниеСодержанияЭтиловогоСпирта =
		ИнтеграцияИСМПКлиентСервер.ОбязательноеУказаниеСодержанияЭтиловогоСпиртаДляВидаПродукцииВОтчетеОНанесении(ВидПродукции);
	...
	Если ЭтоПродукцияСоСрокамиГодности И ПрисутствуетОперацияНанесения Тогда
		...
		Элементы.СрокГодности.АвтоОтметкаНезаполненного = Не ЭтоВидПродукцииСоСрокамиГодностиБезОбязательногоЗаполнения И Не Форма.Объект.ОперацияНанесенияТолькоДляНаборов;
		Элементы.ТоварыСрокГодности.АвтоОтметкаНезаполненного = Не ЭтоВидПродукцииСоСрокамиГодностиБезОбязательногоЗаполнения И Не Форма.Объект.ОперацияНанесенияТолькоДляНаборов;
	...
	Элементы.ДатаПроизводства.Видимость = ЭтоКонтрактноеПроизводство
		...
		Или ЭтоАгрегация И ЭтоПродукцияМОТП;
	...
	Элементы.ДатаПроизводства.АвтоОтметкаНезаполненного =
		...
		И Не ЭтоМолочнаяПродукция И Не ЭтоПродукцияМОТП
	...
	Элементы.ГруппаДанныеОтчетаПроизводственнойЛинии.Видимость = ОтчетПроизводственнойЛинии И Не ЭтоПолучениеПродукцииОтФизическихЛиц;
	...
	ЗависимыеОтСтатусаИСМП.Добавить("ИдентификаторПроизводственногоЗаказа");
...
Процедура ВыбратьЗаказыНаЭмиссиюДляЗаполненияКодовМаркировки(ОповещениеОЗавершении)
	...
	СпособВводаВОборот = ШтрихкодированиеИСМПКлиентСервер.СпособВводаВОборотСУЗПоВидуОперации(Объект.Операция, Объект.ВидПродукции);
...
Функция ОпределитьДоступностьОтчетаПроизводственнойЛинии(ВидПродукции, ОперацииМаркировки, ОперацияНанесения)
	...
	ИначеЕсли ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха") Тогда
		Возврат Ложь;
	ИначеЕсли ОперацииМаркировки.ЭтоМаркировкаОстатков
...
Функция ВидыПродукцииОперацииИмпортСФТС()
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026, Истина);
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.НапиткиССодержаниемМолокаКакаоПрочие, Истина);
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.СпортивноеПитание, Истина);
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ТоварыЛичнойГигиены, Истина);
	Результат.Вставить(Перечисления.ВидыПродукцииИС.БритвыИЛезвия, Истина);
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.РастворимыеЗавариваемыеНапитки, Истина);
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ТоварыИзНатуральногоМеха, Истина);
...
Функция ВидыПродукцииОперацииПроизводствоВнеЕАЭС()
	...
	Результат.Вставить(Перечисления.ВидыПродукцииИС.МорепродуктыПодконтрольныеВЕТИС, Истина); // ЛК
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность, Истина);
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025, Истина);
	Результат.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026, Истина);
	Результат.Вставить(Перечисления.ВидыПродукцииИС.СоковаяПродукция, Истина);
	...
	Если ИнтеграцияИСКлиентСервер.МоторныеМаслаИспользуетПроизводствоВнеЕАЭС() Тогда
		Результат.Вставить(Перечисления.ВидыПродукцииИС.МоторныеМасла, Истина);
	КонецЕсли;
	Если ИнтеграцияИСКлиентСервер.АвтозапчастиИКомплектующиеТранспортныхСредствИспользуетПроизводствоВнеЕАЭС() Тогда
		Результат.Вставить(Перечисления.ВидыПродукцииИС.АвтозапчастиИКомплектующиеТранспортныхСредств, Истина);
	КонецЕсли;
	...
	Если ИнтеграцияИСКлиентСервер.СладостиИКондитерскиеИзделияИспользуетПроизводствоВнеЕАЭС() Тогда
		Результат.Вставить(Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия, Истина);
	КонецЕсли;
	Если ИнтеграцияИСКлиентСервер.ТоварыИзНатуральногоМехаИспользуетПроизводствоВнеЕАЭС() Тогда
		Результат.Вставить(Перечисления.ВидыПродукцииИС.ТоварыИзНатуральногоМеха, Истина);
	КонецЕсли;
