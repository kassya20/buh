...
Функция ОтчетОНанесенииШтрихкодыУпаковокJSON(РезультатЗапроса, ДополнительныеПараметры, Повторно)
	...
	ЭтоПарфюмерия              = Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ПарфюмерныеИКосметическиеСредстваИБытоваяХимия
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ТоварыЛичнойГигиены
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.БритвыИЛезвия;
...
#Область РазборКодовМаркировки
	...
	Если ЭтоТитановаяПродукция Или ЭтоПиво Или ЭтоПарфюмерия Тогда
	...
	Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
		...
		Если (ЭтоТитановаяПродукция Или ЭтоПиво Или ЭтоПарфюмерия)
	...
	Если ЭтоТитановаяПродукция Или ЭтоПиво Или ЭтоПарфюмерия Тогда
		...
		Для Каждого КлючИЗначение Из СоответствиеGTIN Цикл
			...
			ИначеЕсли ЭтоПиво Тогда
				РеквизитыНанесенияНаПиво = Новый Структура;
				РеквизитыНанесенияНаПиво.Вставить("НомерПервичногоДокумента",  "");
				РеквизитыНанесенияНаПиво.Вставить("ДатаПервичногоДокумента",   "");
				РеквизитыНанесенияНаПиво.Вставить("СодержаниеЭтиловогоСпирта", "0");
				Если НайденныеСтроки.Количество() > 0 Тогда
					РеквизитыНанесенияНаПиво.НомерПервичногоДокумента  = НайденныеСтроки[0].НомерПервичногоДокумента;
					РеквизитыНанесенияНаПиво.ДатаПервичногоДокумента   = Формат(НайденныеСтроки[0].ДатаПервичногоДокумента, "ДФ=yyyy-MM-dd");
					РеквизитыНанесенияНаПиво.СодержаниеЭтиловогоСпирта = Формат(НайденныеСтроки[0].СодержаниеЭтиловогоСпирта, "ЧРД=.; ЧН=0;");
				КонецЕсли;
				КлючИЗначение.Значение.Вставить("РеквизитыНанесенияНаПиво", РеквизитыНанесенияНаПиво);
			ИначеЕсли ЭтоПарфюмерия Тогда
				РеквизитыНанесенияНаПарфюмерию = Новый Структура;
				РеквизитыНанесенияНаПарфюмерию.Вставить("СодержаниеЭтиловогоСпирта", "0");
				Если НайденныеСтроки.Количество() > 0 Тогда
					РеквизитыНанесенияНаПарфюмерию.СодержаниеЭтиловогоСпирта = Формат(НайденныеСтроки[0].СодержаниеЭтиловогоСпирта, "ЧРД=.; ЧН=0;");
				КонецЕсли;
				КлючИЗначение.Значение.Вставить("РеквизитыНанесенияНаПарфюмерию", РеквизитыНанесенияНаПарфюмерию);
			КонецЕсли;
		КонецЦикла;
	...
	Если ЭтоТитановаяПродукция Или ЭтоПиво Или ЭтоПарфюмерия Тогда
		...
		Для Каждого ДанныеКодов Из ГруппыКодов Цикл
			Если СоответствиеGTIN[ДанныеКодов.GTIN] = Неопределено Тогда
				...
				ИначеЕсли ЭтоПиво Тогда
					ДанныеGTIN.Вставить("РеквизитыНанесенияНаПиво", Новый Структура);
					ДанныеGTIN.РеквизитыНанесенияНаПиво.Вставить("НомерПервичногоДокумента",  Шапка.НомерПервичногоДокумента);
					ДанныеGTIN.РеквизитыНанесенияНаПиво.Вставить("ДатаПервичногоДокумента",   Формат(Шапка.ДатаПервичногоДокумента, "ДФ=yyyy-MM-dd"));
					ДанныеGTIN.РеквизитыНанесенияНаПиво.Вставить("СодержаниеЭтиловогоСпирта", Формат(Шапка.СодержаниеЭтиловогоСпирта, "ЧРД=.; ЧН=0;"));
				ИначеЕсли ЭтоПарфюмерия Тогда
					ДанныеGTIN.Вставить("РеквизитыНанесенияНаПарфюмерию", Новый Структура);
					ДанныеGTIN.РеквизитыНанесенияНаПарфюмерию.Вставить("СодержаниеЭтиловогоСпирта", Формат(Шапка.СодержаниеЭтиловогоСпирта, "ЧРД=.; ЧН=0;"));
				КонецЕсли;
...
Функция ИменаРеквизитовДляРасчетаДальнейшегоДействия() Экспорт
	...
	Возврат "Ссылка, ОтчетПроизводственнойЛинии, ВидПродукции, Операция, ОперацияНанесения, ВариантЗаполненияДекларации, ВариантВыполненияАгрегации";
...
Функция СообщенияJSONОтчетОНанесении(Шапка, ДополнительныеПараметры)
	...
	ЭтоСоковаяПродукция  = Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.СоковаяПродукция
		...
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.НапиткиССодержаниемМолокаКакаоПрочие;
	ЭтоКреслаКоляски     = Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.КреслаКоляски
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ТехническиеСредстваРеабилитации
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия20;
	ЭтоБАД = Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы
		...
		Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.СпортивноеПитание;
	...
	Для Каждого ДанныеСообщения Из ДополнительныеПараметры.ГруппыКодов Цикл
		...
		ИначеЕсли (ЭтоПродукцияМОТП Или ЭтоБАПиво Или ЭтоСтроительныеМатериалы
			Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.РадиоэлектроннаяПродукция
			Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ПечатныеПлаты
			Или Шапка.ВидПродукции = Перечисления.ВидыПродукцииИС.ТелефоныИНоутбуки)
		...
		Если ЭтоПиво Тогда
			ДанныеGTIN = ДополнительныеПараметры.СоответствиеGTIN[ДанныеСообщения.GTIN];
			Если ДанныеGTIN <> Неопределено Тогда
				Если ЗначениеЗаполнено(ДанныеGTIN.РеквизитыНанесенияНаПиво.НомерПервичногоДокумента)
					Или ЗначениеЗаполнено(ДанныеGTIN.РеквизитыНанесенияНаПиво.ДатаПервичногоДокумента) Тогда
					ТелоЗапроса.Вставить("documentNumber", ДанныеGTIN.РеквизитыНанесенияНаПиво.НомерПервичногоДокумента);
					ТелоЗапроса.Вставить("documentDate",   ДанныеGTIN.РеквизитыНанесенияНаПиво.ДатаПервичногоДокумента);
				КонецЕсли;
				ТелоЗапроса.Вставить("alcoholVolume", ДанныеGTIN.РеквизитыНанесенияНаПиво.СодержаниеЭтиловогоСпирта);
			КонецЕсли;
		КонецЕсли;
		
		Если ЭтоПарфюмерия Тогда
			ДанныеGTIN = ДополнительныеПараметры.СоответствиеGTIN[ДанныеСообщения.GTIN];
			Если ДанныеGTIN <> Неопределено Тогда
				ТелоЗапроса.Вставить("alcoholVolume", ДанныеGTIN.РеквизитыНанесенияНаПарфюмерию.СодержаниеЭтиловогоСпирта);
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого ДанныеОбАгрегации Из ПакетКодов Цикл
		...
		Если ЭтоПродукцияМОТП Тогда
			...
			Если ЗначениеЗаполнено(Шапка.ДатаПроизводства) Тогда
				ТелоЗапроса.Вставить("productionDate", ИнтеграцияИС.ДатаСЧасовымПоясом(Шапка.ДатаПроизводства,,,,""));
			КонецЕсли;
