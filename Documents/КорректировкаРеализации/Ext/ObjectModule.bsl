...
Процедура ЗаполнитьПоДокументуОснованию(Основание, ВидОперацииЗаполнения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Основание) И ТипЗнч(Основание) <> Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(Основание) И ТипЗнч(Основание) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
			Если Основание.ВидОперации = Перечисления.ВидыОперацийОтчетКомиссионера.ОтчетОСписании Тогда
				
				ТекстСообщения = 
				НСтр("ru='Отчета комиссионера о списании не может быть основанием для корректировки реализации!'");
				ВызватьИсключение ТекстСообщения;
				
				Возврат;
			ИначеЕсли НЕ Документы.ОтчетКомиссионераОПродажах.ДокументСозданПоПравиламПостановления914(Основание) Тогда
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПолученный") Тогда
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Организация,ДокументРеализации");
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Организация, Грузоотправитель, Грузополучатель, ОтпускПроизвел, СопроводительныеДокументы, ЭтапГосконтрактаЕИС, АдресДоставки");
	Иначе
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Организация");
	КонецЕсли; 
	...
	КорректироватьБУиНУ = Документы.КорректировкаРеализации.ДоступнаКорректировкаБУиНУ(ДокументРеализации);
	
	КорректироватьНДС = Истина;
	
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.АктОРасхожденияхПолученный") Тогда
		Документы.КорректировкаРеализации.ЗаполнитьПоАктуОРасхождениях(ЭтотОбъект, Основание);
	КонецЕсли; 
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	БанковскийСчетСохраненноеЗначение = БанковскийСчетОрганизации;
	...
	БанковскийСчетОрганизации = БанковскийСчетСохраненноеЗначение;
	...
	Если ЗначениеЗаполнено(ИсходныйДокументРеализации) Тогда
		ДатаОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсходныйДокументРеализации, "Дата");
	Иначе
		ДатаОснования = Дата;
	КонецЕсли;
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда 
		ПередачаТоваровВСоставеРабот = ЭтотОбъект.Товары.Количество() = 0
		И ЭтотОбъект.АгентскиеУслуги.Количество() = 0
		И ЭтотОбъект.СведенияПрослеживаемости.Количество() > 0;
		ИменаТабличныхЧастей = Новый Массив;
		ИменаТабличныхЧастей.Добавить("Товары");
	...
	Если ЕстьПрослеживаемыйТовар И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(ДатаОснования) Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ЭтоОтгрузкаКомиссия = ЭтоКомиссия И ТипЗнч(ИсходныйДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг");
	...
	Если Товары.Количество() > 0 Тогда
		...
		Пока ВыборкаТоваров.Следующий() Цикл
			...
			Если ВыборкаТоваров.ПрослеживаемыйКомплект И Не ЭтоКомиссия Тогда
				СуммаБезНДС = ?(СуммаВключаетНДС, ВыборкаТоваров.Сумма - ВыборкаТоваров.СуммаНДС, ВыборкаТоваров.Сумма);
				Отбор = Новый Структура("ИдентификаторСтроки, ЭтоСтрокаСИсходнымиРНПТ", ВыборкаТоваров.ИдентификаторСтроки, Ложь);
	...
	Если (НЕ ЭтоОтгрузкаКомиссия)
		ИЛИ (НЕ КорректироватьБУиНУ) Тогда
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицыПоВзаиморасчетам = Документы.КорректировкаРеализации.ПодготовитьТаблицыАвансовОплат(ТаблицаВзаиморасчеты, ПараметрыПроведения.ЗачетАвансовРеквизиты);
	
	ТаблицаВзаиморасчетыВозврат = Документы.КорректировкаРеализации.ПодготовитьТаблицуВыделениеАвансов(ТаблицаВзаиморасчетыВозврат, ПараметрыПроведения.ЗачетАвансовРеквизиты, ПараметрыПроведения.СторноРеализацияТаблицаДокумента, ПараметрыПроведения.РеализацияТаблицаДокумента);
		
	ТаблицыДокументаПоКурсуАвансов = Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаПоКурсуАвансов(
		ПараметрыПроведения.РеализацияРеквизиты,
		ПараметрыПроведения.СторноРеализацияТаблицаДокумента,
		ПараметрыПроведения.РеализацияТаблицаДокумента, ТаблицыПоВзаиморасчетам.ТаблицаАвансов);
	...
	ТаблицаВозвратОтКомиссионера = УчетТоваров.ПодготовитьТаблицуСписанныеТоварыСУчетомВозврата(ПараметрыПроведения.ВозвратТоваровОтКомиссионера,
		ТаблицаВозвращенныеТовары, ПараметрыПроведения.ВозвратТоваровРеквизиты, Отказ);
	
	// Таблица товаров возвращенных при отгрузке без перехода права собственности
	ТаблицаВозвратОтгрузкаБезППС = УчетТоваров.ПодготовитьТаблицуСписанныеТоварыСУчетомВозврата(ПараметрыПроведения.ВозвратТоваровОтгрузкаБезППС,
		ТаблицаВозвращенныеТовары, ПараметрыПроведения.ВозвратТоваровРеквизиты, Отказ);
	...
	СтруктураТаблицУСН.Вставить("ТаблицаВозвратОтКомиссионера", ТаблицаВозвратОтКомиссионера);
	СтруктураТаблицУСН.Вставить("ТаблицаВозвратОтгрузкаБезППС", ТаблицаВозвратОтгрузкаБезППС);
	...
	ВозвращенныеМПЗ = Документы.КорректировкаРеализации.ПодготовитьТаблицуСписанныеМПЗИП(
		ТаблицаВозвращенныеТовары, ТаблицаДокументаСторноРеализация, ПараметрыПроведения.РеализацияРеквизитыИП, Истина);
	...
	УчетТоваров.СформироватьДвиженияСписаниеТоваров(ТаблицаВозвратОтКомиссионера, 
		ПараметрыПроведения.ВозвратТоваровРеквизиты, Движения, Отказ);
		
	УчетТоваров.СформироватьДвиженияСписаниеТоваров(ТаблицаВозвратОтгрузкаБезППС,
		ПараметрыПроведения.ВозвратТоваровРеквизиты, Движения, Отказ);
	...
	УчетНДСБП.СформироватьДвиженияВозвратОтКомиссионера(
		ПараметрыПроведения.НДСТоварыВозвратОтКомиссионера, ТаблицаВозвратОтКомиссионера,  
		ПараметрыПроведения.ВозвратТоваровРеквизиты, Движения, Отказ);
	...
	// Возврат товаров, отгруженных без перехода права собственности
	УчетНДСРаздельный.СформироватьДвиженияКорректировкиОтгрузкаБезПереходаПраваСобственности(ПараметрыПроведения.НДСТоварыВозвратОтгрузкиБезППС,
		ТаблицаВозвратОтгрузкаБезППС, ПараметрыПроведения.НДСРеквизиты, Движения, Отказ);
	...
	УчетВзаиморасчетов.СформироватьДвиженияЗадолженностьПоОтгрузке(
		ТаблицыПоВзаиморасчетам.ТаблицаВзаиморасчеты, ПараметрыПроведения.ЗадолженностьПоОтгрузкеРеквизиты, Движения, Отказ);
	
	// Учет прослеживаемых товаров
	ПрослеживаемыеОперации = ПараметрыПроведения.ПрослеживаемыеОперации;
	ПрослеживаемыеОперацииУслуги = ПараметрыПроведения.ПрослеживаемыеОперацииУслуги;
	...
	ПрослеживаемостьБП.СформироватьДвиженияКорректировкаРеализацииТоваров(
	...
	ПрослеживаемостьБП.СформироватьДвиженияКорректировкаРеализацииТоваров(
		ПараметрыПроведения.ПрослеживаемыеТоварыУвеличениеУслуги,
		ПараметрыПроведения.ПрослеживаемыеТоварыУменьшениеУслуги,
		ПрослеживаемыеОперацииУслуги,
		ПараметрыПроведения.НДСРеквизиты,
		Движения);
	...
	УчетАкцизов.СформироватьДвиженияСторноАкцизов(
		ПараметрыПроведения.ТаблицаСторноАкцизы,
		ПараметрыПроведения.ТаблицаРеквизитыАкцизы,
		Движения);
...
Процедура ОбработатьСуммыДокорректировки()
	...
	Если ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
		...
		Товары.ЗагрузитьКолонку(Товары.ВыгрузитьКолонку("СуммаАкцизаДоИзменения"), 	"СуммаАкцизаДоКорректировки");
	...
	ИначеЕсли Не ИсправлениеКорректировки Тогда
		Для Каждого СтрокаТЧ из Товары Цикл
			...
			СтрокаТЧ.СуммаАкцизаДоКорректировки = 0;
		КонецЦикла;
...
Функция ПолучитьДанныеОбъектаДляПроверкиЗаполнения(СтруктураРезультатов)
	...
	Если Товары.Количество() > 0 Тогда
		...
		|	ВремТаблица.ПодакцизныйТовар,
		|	ВремТаблица.СуммаАкциза,
		|	ВремТаблица.СчетАкциза
		...
		|	ТаблицаДокумента.ПодакцизныйТовар,
		|	ТаблицаДокумента.СуммаАкциза,
		|	ТаблицаДокумента.СчетАкциза
		|ИЗ
