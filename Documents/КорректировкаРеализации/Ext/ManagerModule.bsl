...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоОписаниеДополнительныхДанных(Правила, "ОтгрузкаБезПереходаПраваСобственности", "ДокументРеализации");
...
Процедура ДополнитьДанныеЗаполненияСчетовУчета(ДанныеЗаполнения) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("ОтгрузкаБезПереходаПраваСобственности") Тогда
		...
		ДанныеЗаполнения.ОтгрузкаБезПереходаПраваСобственности = ТипЗнч(ДокументРеализацииСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И ЗначениеЗаполнено(ДокументРеализацииСсылка)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализацииСсылка, "ВидОперации") = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьРНПТДо");
	Методы.Вставить("УстановитьРеквизитКорректироватьНДС");
	Методы.Вставить("УстановитьРеквизитИсходныйДокумент");
	Методы.Вставить("УстановитьПризнакПрослеживаемостиДляТоваровСтавшимиПрослеживаемыми");
	Методы.Вставить("ЗаполнитьРеквизитыИсправленияКорректировкиРеализации");
	Методы.Вставить("ЗаполнитьСчетПередачи");
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Реквизиты.Вставить("ЭтоОтгрузкаБезПереходаПраваСобственности", ТипЗнч(ДокументРеализацииСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализацииСсылка, "ВидОперации") = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности);
	Запрос.УстановитьПараметр("ЭтоОтгрузкаБезПереходаПраваСобственности",
		Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности);
	...
	Если Реквизиты.ЭтоДоговорСКомиссионером
	...
	Запрос.УстановитьПараметр("ВидОперацииРеализации",          ДанныеДокументаРеализацииСсылка.ВидОперации);
	...
	Если (Реквизиты.РасчетыВВалюте Или Реквизиты.РасчетыВУсловныхЕдиницах)
		...
		И Реквизиты.ВалютаДокумента <> Реквизиты.ВалютаРеглУчета Тогда
		Если Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
			СчетаРасчетовПоОтгрузке = УчетВзаиморасчетов.СчетаРасчетовПоОтгрузке(Реквизиты);
			СреднийКурсЗачетаОплат = УчетВзаиморасчетов.СреднийКурсЗачетаОплат(
				СчетаРасчетовПоОтгрузке.СчетАвансов,
				Реквизиты.ИсходныйДокументРеализации, Реквизиты.Регистратор, Истина, ДанныеДокументаРеализацииСсылка.Дата, Реквизиты.Период);
			КурсОтгрузки = ?(Реквизиты.КратностьДокументаРеализации = 0,
				0, Реквизиты.КурсДокументаРеализации / Реквизиты.КратностьДокументаРеализации);
...
Функция ТекстЗапросаСписаниеТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|			ТОГДА ""Корректировка отгрузки без перехода права собственности""
	...
	|	&ЭтоОтгрузкаБезПереходаПраваСобственности КАК ЭтоОтгрузкаБезПереходаПраваСобственности,
	|	Реквизиты.ВидДоговора КАК ВидДоговора,
...
Функция ТекстЗапросаВозвратТоваров(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.КорректироватьБУиНУ Тогда
		...
		ПараметрыПроведения.Вставить("ВозвратТоваровОтгрузкаБезППС", Неопределено);
	    Возврат "";
	КонецЕсли;
	...
	ИначеЕсли Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
		...
		НомераТаблиц.Вставить("ВозвратТоваровОтгрузкаБезППС",  НомераТаблиц.Количество());
...
Функция ТекстЗапросаЗачетАвансов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.КорректироватьБУиНУ
		ИЛИ Реквизиты.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
		...
		ПараметрыПроведения.Вставить("ЗадолженностьПоОтгрузкеРеквизиты", Неопределено);
		Возврат "";
	КонецЕсли;
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА &ЭтоОтгрузкаБезПереходаПраваСобственности И ТаблицаСуммВзаиморасчетов.СуммаВзаиморасчетов >= 0
	|			ТОГДА &ДокументРеализацииСсылка
	...
	|	&ОрганизацияПрименяетУСН И НЕ &ЭтоОтгрузкаБезПереходаПраваСобственности КАК УчитыватьЗадолженностьУСН,
	|	&УчитыватьЗадолженностьУСНПатент И НЕ &ЭтоОтгрузкаБезПереходаПраваСобственности КАК УчитыватьЗадолженностьУСНПатент,
	|	&ДеятельностьНаПатенте КАК ДеятельностьНаПатенте,
	...
	|		КОГДА &ЭтоОтгрузкаБезПереходаПраваСобственности
	|			ТОГДА &ДокументРеализацииСсылка
	|		ИНАЧЕ &ПервичныйДокументРеализации
	...
	Если Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
		НомераТаблиц.Вставить("ЗадолженностьПоОтгрузкеРеквизиты", НомераТаблиц.Количество());
		
		ТекстЗапроса = ТекстЗапроса
		+ "ВЫБРАТЬ
		|	Реквизиты.Дата КАК Период,
		|	&ДокументРеализацииСсылка КАК Регистратор,
		|	Реквизиты.Организация КАК Организация,
...
Функция ТекстЗапросаТоварыНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.ЕстьТовары И НЕ Реквизиты.ЕстьУслуги 
		...
		ПараметрыПроведения.Вставить("НДСТоварыВозвратОтгрузкиБезППС", Неопределено);
		Если Реквизиты.ЭтоДоговорСКомиссионером Тогда
			ТекстЗапроса = "";
			...
			|	Реквизиты.КорректироватьБУиНУ КАК КорректироватьБУиНУ,
			|	Реквизиты.ВидДоговора КАК ВидДоговора,
			|	&ВидОперацииРеализации КАК ВидОперацииРеализации,
			|	&ДокументРеализацииСсылка КАК ИсходныйДокументРеализации,
			|	&ЕстьНДС КАК ЕстьНДС
			|ИЗ
			|	Реквизиты КАК Реквизиты"
			+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	...
	Если Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности
		И Реквизиты.ЕстьТовары Тогда
		НомераТаблиц.Вставить("НДСТоварыВозвратОтгрузкиБезППС", НомераТаблиц.Количество());
		ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	""Товары"" КАК ИмяСписка,
		|	&СинонимТовары КАК СинонимСписка,
...
Функция ПодготовитьПараметрыТаблицыСписанныеТоварыКорректировка(ТаблицаТовары, ТаблицаРеквизиты)
	...
	+ "ЭтоОтгрузкаБезПереходаПраваСобственности," // <Булево> - корректировка реализации с видом операции отгрузка без перехода права собственности
	+ "ДатаДокументаРеализации,"      // <Дата> - дата корректируемого документа
...
Процедура ОбработкаОтложенногоПроведения(Параметры, Отказ) Экспорт
	...
	ТаблицыПоВзаиморасчетам = ПодготовитьТаблицыАвансовОплат(ТаблицаВзаиморасчеты, ПараметрыПроведения.ЗачетАвансовРеквизиты);
	
	ТаблицаВзаиморасчетыВозврат = ПодготовитьТаблицуВыделениеАвансов(ТаблицаВзаиморасчетыВозврат, ПараметрыПроведения.ЗачетАвансовРеквизиты);
...
Процедура СформироватьДвиженияВыделениеАванса(ТаблицаВзаиморасчеты, ТаблицаРеквизиты, Движения, Отказ, ЭтоОтложенноеПроведение = Ложь) Экспорт
	...
	Для каждого СтрокаВзаиморасчетов Из Параметры.Взаиморасчеты Цикл
		...
		ИначеЕсли Реквизиты.ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
			...
			// Корректировка задолженности по счету ОТ.х
			Проводка = Проводки.Добавить();

			Проводка.Период      = Реквизиты.Период;
			Проводка.Организация = Реквизиты.Организация;
			Проводка.Содержание  = НСтр("ru = 'Корректировка расчетов по отгрузке без перехода права собственности'");
...
Функция ПодготовитьПараметрыДвиженияВыделениеАванса(ТаблицаВзаиморасчетов, ТаблицаРеквизиты)
	...
	+ "ЭтоОтгрузкаБезПереходаПраваСобственности," // Булево - Истина при корректировке реализации (отгрузка без перехода права собственности)
...
Функция ДоступнаКорректировкаБУиНУ(ДокументРеализации) Экспорт
	...
	Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Возврат (РеквизитыДокумента.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия
					ИЛИ РеквизитыДокумента.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийСоглашенияОбИзмененииСтоимости()
	...
	|			ТОГДА """"
	|	КОНЕЦ КАК НоменклатураКод,
	|	ВЫРАЗИТЬ(КорректировкаРеализации.Количество / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 3)) КАК Количество,
	|	ВЫРАЗИТЬ(КорректировкаРеализации.КоличествоДоИзменения / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 3)) КАК КоличествоДоКорректировки,
	|	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения, ЕСТЬNULL(КорректировкаРеализацииНоменклатура.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка))) КАК ЕдиницаИзмерения, 
	|	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения.Код, ЕСТЬNULL(КорректировкаРеализацииНоменклатура.ЕдиницаИзмерения.Код, """")) КАК ЕдиницаИзмеренияКод, 
	|   ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения.Наименование, ЕСТЬNULL(КорректировкаРеализацииНоменклатура.ЕдиницаИзмерения.Наименование, """")) КАК ЕдиницаИзмеренияНаименование, 
	|	0 КАК КоличествоВОдномМесте,
	|	ВЫРАЗИТЬ(ВЫБОР
	|		КОГДА НЕ ДокументыДляПечати.СуммаВключаетНДС
	|				ИЛИ КорректировкаРеализации.СуммаНДС = 0
	...
	|	КОНЕЦ * ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,2)) КАК ЦенаДоКорректировки,
	|	ВЫБОР
	|		КОГДА ДокументыДляПечати.СуммаВключаетНДС
	|			ТОГДА КорректировкаРеализации.Сумма - КорректировкаРеализации.СуммаНДС
	|		ИНАЧЕ КорректировкаРеализации.Сумма
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиНоменклатуры КАК СправочникУпаковка
	|		ПО КорректировкаРеализации.Упаковка = СправочникУпаковка.Ссылка
	|			И (СправочникУпаковка.Коэффициент > 0)
...
Процедура ЗаполнитьПоДокументу(Объект) Экспорт
	...
	// Для документа Реализация: Отгрузка без перехода права собственности услуги не корректируются
	Если ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И Общегоназначения.ЗначениеРеквизитаОбъекта(Объект.ДокументРеализации, "ВидОперации") = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности Тогда
		ДанныеЗаполнения.Услуги.Очистить();
	КонецЕсли;
...
Функция ТекстЗапросаДанныеЗаполненияПоВозвратуОтПокупателя(НомераТаблиц)
	...
	|ВЫБРАТЬ
	|	ВозвратТоваровОтПокупателяТовары.Номенклатура КАК Номенклатура,
	|	ВозвратТоваровОтПокупателяТовары.Упаковка КАК Упаковка,
	|	ВозвратТоваровОтПокупателяТовары.Цена КАК Цена,
