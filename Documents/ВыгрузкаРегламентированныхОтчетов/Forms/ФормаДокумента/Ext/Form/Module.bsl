...
Процедура СформироватьТекстыВыгрузки(Команда)
	СтруктДопПараметров = Новый Структура(
		"УдалитьВыгрузкуПояснительнойЗапискиКДекларацииПоЕНВД, НеОбновлятьДляФСНБР, НеОбновлятьДляФНСУБ");
...
Процедура СформироватьИЗаписать(ДокументыОснования, ТекстВыгрузки = Неопределено, ПечатаемыеЛисты = Неопределено, ФайлДляСохранения = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, КаталогСохранения = Неопределено, СообщенияОбОшибках = Неопределено, ВВебКлиентеСохранятьФайлВыгрузкиИнтерактивно = Истина, ВыгружатьСОшибками = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, УникальныйИДФормы = Неопределено, ВыполняемоеОповещение = Неопределено, СтруктураДополнительныхПараметров = Неопределено) Экспорт
	...
	Исключение
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), РегламентированнаяОтчетностьКлиент.СформироватьТекстСообщения(ОписаниеОшибки())));
...
Процедура СформироватьТекстыВыгрузкиКлиент(ПечатаемыеЛисты = Неопределено, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, ДляОтправкиВФНС = Ложь, ПервыйДок = Неопределено, СообщенияОбОшибках = Неопределено, ВыгружатьСОшибками = Ложь, СтруктураДополнительныхПараметров = Неопределено)
		...
		Для Каждого Стр Из ОтборФСС Цикл
			...
			Исключение
				...
				Если СообщенияОбОшибках = Неопределено Тогда
					ОбщегоНазначенияКлиент.СообщитьПользователю(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(Стр)));
	...
	Если Отбор500.Количество() <> 0 И (НЕ ВыгрузкаПрервана) Тогда	
		...
		Для Каждого Стр Из Отбор500 Цикл
			...
			Если НЕ ВыгрузкаПрервана Тогда
				Если ТипЗнч(ТекстВыгрузкиВXML) = Тип("Массив") Тогда
					НеОбновлятьТекстыДляФНСУБ = ТипЗнч(СтруктураДополнительныхПараметров) = Тип("Структура")
						И СтруктураДополнительныхПараметров.Свойство("НеОбновлятьДляФНСУБ");
					Для Каждого ЭлементПакетаВыгрузки Из ТекстВыгрузкиВXML Цикл
						Если ТипЗнч(ЭлементПакетаВыгрузки) = Тип("Структура") Тогда
							НовСтр = Объект.Выгрузки.Добавить();
							НовСтр.ИмяФайла = ЭлементПакетаВыгрузки.ИмяФайлаВыгрузки;
							...
							ТекстВыгрузкиФНС = СтрокиПараметровИзСтруктуры(ЭлементПакетаВыгрузки);
							
							ОбновитьТекстВыгрузки = Истина; 
							Если ЭлементПакетаВыгрузки.Свойство("ТипФайлаВыгрузки")
							   И ЭлементПакетаВыгрузки["ТипФайлаВыгрузки"] = "ФормаОценочнаяКартаУБ" Тогда
								ТекстВыгрузкиФНС  = ПолучитьСтрокуИзВременногоХранилища(ЭлементПакетаВыгрузки.АдресФайлаВыгрузки,
									?(ЭлементПакетаВыгрузки.Свойство("КодировкаФайлаВыгрузки"),
										ЭлементПакетаВыгрузки.КодировкаФайлаВыгрузки, "windows-1251"));
								Если Лев(ТекстВыгрузкиФНС, 9) = "Структура" Тогда
									СтруктураПараметровФНС = СтруктураИзСтрокПараметров(ТекстВыгрузкиФНС);
									Если  НеОбновлятьТекстыДляФНСУБ
										И СтруктураПараметровФНС.Свойство("ТипФайлаВыгрузки")
										И СтруктураПараметровФНС["ТипФайлаВыгрузки"] = "ОтчетФНС_ФМ"  Тогда
							...
							Если ОбновитьТекстВыгрузки Тогда
								НовСтр.Текст = ТекстВыгрузкиФНС;
							Иначе
								НовСтр.Текст = НСтр("ru='Оценочная карта УБ была сохранена в каталог в формате XLSM'");
							КонецЕсли;
			...
			Если Стр.Отчет = "Статистика: Форма П-1" И Год(Стр.ДатаНачала) <= 2016 Тогда
				...
				Иначе
					...
					Исключение
						...
						Если СообщенияОбОшибках = Неопределено Тогда
							ОбщегоНазначенияКлиент.СообщитьПользователю(
								СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось выгрузить %1.'"), РегламентированнаяОтчетностьКлиентСервер.ПредставлениеДокументаРеглОтч(СтрДокП1Пр3)));
							ОтобразитьФормуНавигацииПоОшибкам();
						Иначе
							ЗаполнитьСообщенияОбОшибках(СообщенияОбОшибках);
						КонецЕсли;
...
Процедура СформироватьТекстыВыгрузкиСервер(ПервыйДок, ДляОтправкиВФНС, КПП, ОшибкаВыгрузки)
	...
	ОтчетыФНСУБ = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыФНСУгрозаБанкротства();
	...
	ОтчетыМинпромторг = РегламентированнаяОтчетностьПовтИсп.ПолучитьОтчетыМинпромторг();
	...
	Для Каждого Стр Из ВерсииИОтчеты Цикл
		...
		Если ОтчетыСПолнойВнутреннейПроверкой.Найти(ИсточникОтчета) <> Неопределено
		 ...
		 ИЛИ ОтчетыФНСУБ.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыРПН.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
		 ИЛИ ОтчетыМинпромторг.НайтиПоЗначению(ИсточникОтчета) <> Неопределено
			...
			Если ТипЗнч(СтруктураПараметров) = Тип("Структура") И СтруктураПараметров.Свойство("АдресФайлаВыгрузки") Тогда
				Если Лев(СтруктураПараметров.АдресФайлаВыгрузки, 2) = "e1" Тогда
					...
					Попытка
						...
						ИначеЕсли СтруктураПараметров.Свойство("ТипФайлаВыгрузки", ТипФайлаВыгрузки) И ТипФайлаВыгрузки = "ОтчетФНС_ФМ" Тогда
							ТекСтрока.Текст = НСтр("ru='Оценочная карта УБ была сохранена в каталог в формате XLSM'");
...
Процедура ЗавершитьПроверкуОтчетов(СообщенияОбОшибках, ПодавитьВыводСтаройФормыНавигацииПоОшибкам)
	...
	Если СообщенияОбОшибках = Неопределено Тогда
		Если ОшибкаВыгрузки = Истина ИЛИ ТаблицаСообщений.Количество() <> 0 Тогда
			ТекстСообщения = СтрШаблон("В ходе проверки были найдены ошибки%1.",
				?(ОшибкаВыгрузки = Истина, ", препятствующие выгрузке", ""));
