...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСУчредителями Тогда 
		КалендарьБухгалтера.ЗапуститьЗаполнениеВФоне(УникальныйИдентификатор, ТекущийОбъект.Организация, Ложь, Ложь, Истина);
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Объект.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаДляВводаОстатков.РасчетыСУчредителями") Тогда 
...
#Область ОбработчикиСобытийТаблицаБухСправка
...
&НаКлиенте
Процедура БухСправкаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "БухСправкаРНПТ" Тогда
		...
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			...
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "БухСправка", ПараметрыФормыПодбора);
		КонецЕсли;
		
	КонецЕсли;

	Если Поле.Имя = "БухСправкаПрослеживаемыйТовар" Тогда
...
Процедура УстановитьУсловноеОформление()
	...
	// Признак прослеживаемости, РНПТ видимость
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухСправкаПрослеживаемыйТовар");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухСправкаРНПТ");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухСправка.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухСправкаРНПТ");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухСправка.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухСправка.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
...
Процедура ПодготовитьФормуНаСервере()
	...
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации Тогда
		
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.СпецоснасткаВЭксплуатацииВспомогательный);
	...
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ПрочиеСчетаБухгалтерскогоУчета Тогда
		...
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоПрочимУдержаниямРаботников);
		...
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.СпецодеждаВЭксплуатацииВспомогательный);
		...
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВозвратнаяТара);
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВозвратнаяТараНаСкладе);
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВозвратнаяТараУПокупателя);
	...
	ОтборПоПризнакуЗабалансовый	= ?(Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ПрочиеСчетаБухгалтерскогоУчета
		ИЛИ Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации,
		Неопределено, Ложь);
...
Процедура УправлениеФормойСервер()
	...
	ВедетсяУчетПрослеживаемыхТоваров = ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
		...
		И Объект.ОтражатьВБухгалтерскомУчете
		И Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ПрочиеСчетаБухгалтерскогоУчета;
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		Для Каждого СтрокаТаблицы Из Объект.БухСправка Цикл
			Если СтрокаТаблицы.ПрослеживаемыйТовар Тогда 
				ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТОС(
				ЭтотОбъект, СтрокаТаблицы);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ОбработатьИзменениеСубконто(НомерСубконто)
	...
	ИначеЕсли ТипСубконто = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		ПоСчетуМогутБытьОстаткиВПрослеживаемости = 
			СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение")
			Или СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТоварыНаСкладе")
			Или СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТоварыПереданныеНаКомиссию")
			Или СтрокаТаблицы.СчетУчета =
				ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежностиВЭксплуатации");
				
			Если ВедетсяУчетПрослеживаемыхТоваров И ПоСчетуМогутБытьОстаткиВПрослеживаемости
					И ЗначениеЗаполнено(ЗначениеСубконто) Тогда
			ПараметрыПрослеживаемости = ПолучитьПризнакиПрослеживаемости(ЗначениеСубконто);
			СтрокаТаблицы.ПрослеживаемыйТовар = ПараметрыПрослеживаемости.ПрослеживаемыйТовар
				Или ПараметрыПрослеживаемости.ПрослеживаемыйКомплект;
			СтрокаТаблицы.ПрослеживаемыйКомплект = ПараметрыПрослеживаемости.ПрослеживаемыйКомплект;
		Иначе
			СтрокаТаблицы.ПрослеживаемыйТовар = Ложь;
			СтрокаТаблицы.ПрослеживаемыйКомплект = Ложь;
		КонецЕсли;
...
&НаСервере
Функция ПолучитьПризнакиПрослеживаемости(Номенклатура)
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ПрослеживаемыйТовар, ПрослеживаемыйКомплект");
КонецФункции
...
Процедура УдалитьСвязанныеЗаписиНаСервере()
	...
	Для каждого СтрокаКлюча Из ТаблицаУдаленныхСтрок Цикл
		...
		СтрокиНоменклатура = Объект.БухСправка.НайтиСтроки(СтруктураОтбора);
		Если СтрокиНоменклатура.Количество() > 0 Тогда
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТОС(
				ЭтотОбъект, СтрокиНоменклатура[0]);
		КонецЕсли;
...
&НаКлиенте
Процедура БухСправкаРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
