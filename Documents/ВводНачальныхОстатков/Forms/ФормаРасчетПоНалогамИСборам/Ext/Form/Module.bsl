...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасчетыПоНалогамИСборамСтатьяЗатрат");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РасчетыПоНалогамИСборам.СчетУчета", ВидСравненияКомпоновкиДанных.Равно,
	...
	// РасчетыПоНалогамИСборамСуммаВзносаДляУменьшенияНалога
	Если Объект.Дата > УчетСтраховыхВзносовИП.ДатаПереходаНаУпрощенныйЗачетВзносов() Тогда
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(
			ЭлементУО.Поля, "РасчетыПоНалогамИСборамСуммаВзносаДляУменьшенияНалога");
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор, "Объект.РасчетыПоНалогамИСборам.СчетУчета",
			ВидСравненияКомпоновкиДанных.НеРавно, ПланыСчетов.Хозрасчетный.ПФР_ОПС_ИП);
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	Иначе
		Элементы.РасчетыПоНалогамИСборамСуммаВзносаДляУменьшенияНалога.Видимость = Ложь;
	КонецЕсли;
...
Процедура РасчетыПоНалогамИСборамСуммаПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.Сумма <> 0 Тогда
		СтрокаТаблицы.СуммаКт = 0;
		СтрокаТаблицы.СуммаВзносаДляУменьшенияНалога = 0;
	КонецЕсли;
...
Процедура РасчетыПоНалогамИСборамСуммаКтПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.СуммаВзносаДляУменьшенияНалога > СтрокаТаблицы.СуммаКт Тогда
		СтрокаТаблицы.СуммаВзносаДляУменьшенияНалога = СтрокаТаблицы.СуммаКт;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура РасчетыПоНалогамИСборамСуммаВзносаДляУменьшенияНалогаПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.РасчетыПоНалогамИСборам.ТекущиеДанные;
	Если ТекущиеДанные.СуммаВзносаДляУменьшенияНалога > ТекущиеДанные.СуммаКт Тогда
		ТекущиеДанные.СуммаВзносаДляУменьшенияНалога = ТекущиеДанные.СуммаКт;
	КонецЕсли;
