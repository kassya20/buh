...
Процедура МПЗОтгруженныеПередУдалением(Элемент, Отказ)
	...
	ВыделенныеСтроки = Элементы.МПЗОтгруженные.ВыделенныеСтроки;
	...
	Для каждого Строка Из ВыделенныеСтроки Цикл
		ТекДанные = Объект.МПЗОтгруженные.НайтиПоИдентификатору(Строка);
...
Процедура МПЗОтгруженныеСчетУчетаПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости") Тогда
...
Процедура МПЗОтгруженныеНоменклатураПриИзменении(Элемент)
	...
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		РеквизитыНоменклатуры = РеквизитыНоменклатуры(СтрокаТаблицы.Номенклатура, "ЕдиницаИзмерения");
		СтрокаТаблицы.НоменклатураЕдиницаИзмерения = РеквизитыНоменклатуры.ЕдиницаИзмерения;
	Иначе
		СтрокаТаблицы.НоменклатураЕдиницаИзмерения = Неопределено;
	КонецЕсли;
	
	Если Не СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости") Тогда
		
		РеквизитыПрослеживаемости = Новый Структура("СтранаПроисхождения, ПрослеживаемыйТовар, ПрослеживаемыйКомплект");
		ПараметрыОбъекта = Новый Структура("ВедетсяУчетПрослеживаемыхТоваров, Номенклатура",
		ВедетсяУчетПрослеживаемыхТоваров, СтрокаТаблицы.Номенклатура);
...
Процедура МПЗОтгруженныеДоговорПокупателяПриИзменении(Элемент)
	...
	Если СтрокаТаблицы.ДоговорПокупателяВидДоговора <> ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером") Тогда
		// Необходимо очистить дополнительные сведения
		НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
		НоваяСтрока.Ключ = СтрокаТаблицы.Ключ;
...
&НаКлиенте
Процедура МПЗОтгруженныеРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
...
Процедура МПЗОтгруженныеДокументОтгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
		ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
		ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаДокументОтгрузки());
		ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
&НаКлиенте
Процедура МПЗОтгруженныеРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элементы.МПЗОтгруженные.ТекущиеДанные;
	Если ТекущиеДанные.ПрослеживаемыйТовар
		И НЕ (ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости")
		Или ТекущиеДанные.ДоговорПокупателяВидДоговора <> ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером")) Тогда
		...
		ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "МПЗОтгруженные", ПараметрыФормыПодбора);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура МПЗОтгруженныеСтранаПроисхожденияПриИзменении(Элемент)
...
&НаКлиенте
Процедура МПЗОтгруженныеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "МПЗОтгруженныеРНПТ" Тогда
		СтандартнаяОбработка = Ложь;
		ТекущиеДанные = Элементы.МПЗОтгруженные.ТекущиеДанные;
		Если ТекущиеДанные.ПрослеживаемыйТовар
			И НЕ (ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости")
	...
	Если Поле.Имя = "МПЗОтгруженныеСтранаПроисхождения" Тогда
		ТекущиеДанные = Элементы.МПЗОтгруженные.ТекущиеДанные;
		Если НЕ ТекущиеДанные.ПрослеживаемыйТовар 
			Или ТекущиеДанные.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости")
			Или ТекущиеДанные.ДоговорПокупателяВидДоговора <> ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером") Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;

	Если Поле.Имя = "МПЗОтгруженныеПрослеживаемыйТовар" Тогда
		ТекущиеДанные = Элементы.МПЗОтгруженные.ТекущиеДанные;
...
Процедура УстановитьУсловноеОформление()
	...
	// ДанныеПоСФКонтрагент
	
	ДатаНачалаВеденияПолныхЗаписейПоРУ = УчетНДСРаздельный.ДатаНачалаВеденияСамодостаточнойАналитики() - 86400;
	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДанныеПоСФКонтрагент");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	...
	// МПЗОтгруженныеСтранаПроисхождения
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МПЗОтгруженныеСтранаПроисхождения");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.ПрослеживаемыйКомплект", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.СтранаПроисхождения", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.ДоговорПокупателяВидДоговора", ВидСравненияКомпоновкиДанных.Равно,
		Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.СчетУчета", ВидСравненияКомпоновкиДанных.НеРавно, ПланыСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости);
	...
	ГруппаОтбора1 = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(ЭлементУО.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.МПЗОтгруженные.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.МПЗОтгруженные.СчетУчета", ВидСравненияКомпоновкиДанных.Равно, ПланыСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.МПЗОтгруженные.ДоговорПокупателяВидДоговора", ВидСравненияКомпоновкиДанных.НеРавно,
		Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером);
	...
	// МПЗОтгруженныеРНПТ
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МПЗОтгруженныеРНПТ");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МПЗОтгруженные.ДоговорПокупателяВидДоговора", ВидСравненияКомпоновкиДанных.Равно,
		Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МПЗОтгруженныеПрослеживаемыйТовар");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
...
Процедура УправлениеФормойСервер()
	...
	Если Не Объект.ОтражатьВБухгалтерскомУчете Или Не ВедетсяУчетПрослеживаемыхТоваров Тогда
		Элементы.ГруппаПрослеживаемостьМПЗОтгруженные.Видимость = Ложь;
		Объект.СведенияПрослеживаемости.Очистить();
	Иначе
		Элементы.ГруппаПрослеживаемостьМПЗОтгруженные.Видимость = Истина;
	КонецЕсли;
...
Процедура ОбработкаВыбораРНПТНаКлиенте(ВыбранноеЗначение, ИмяТаблицы)
	...
	Если СтрокаТабличнойЧасти <> Неопределено Тогда
		...
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
			ЭтаФорма, СтрокаТабличнойЧасти, Ложь);
	КонецЕсли;
...
Процедура УдалитьСвязанныеЗаписиНаСервере()
	...
	Для каждого СтрокаКлюча Из ТаблицаУдаленныхСтрок Цикл
		...
		СтрокиНоменклатура = Объект.МПЗОтгруженные.НайтиСтроки(СтруктураОтбора);
		Если СтрокиНоменклатура.Количество() > 0 Тогда
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
				ЭтотОбъект, СтрокиНоменклатура[0], Ложь);
		КонецЕсли;
...
Функция ОписаниеТипаРеквизитаРасчетныйДокумент()
	...
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.МПЗОтгруженные.Реквизиты.РасчетныйДокумент.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаДокументОтгрузки()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.МПЗОтгруженные.Реквизиты.ДокументОтгрузки.Тип;
...
Функция ОписаниеТипаРеквизитаПартия()
	...
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.МПЗОтгруженные.Реквизиты.Партия.Тип;
