...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаНачалаВеденияПолныхЗаписейПоРУ = УчетНДСРаздельный.ДатаНачалаВеденияСамодостаточнойАналитики() - 86400;
...
Процедура НоменклатураНаСкладеПередУдалением(Элемент, Отказ)
	...
	ВыделенныеСтроки = Элементы.НоменклатураНаСкладе.ВыделенныеСтроки;
	...
	Для каждого Строка Из ВыделенныеСтроки Цикл
		ТекДанные = Объект.НоменклатураНаСкладе.НайтиПоИдентификатору(Строка);
...
Процедура НоменклатураНаСкладеСчетУчетаПриИзменении(Элемент)
	...
	Если ТипЗнч(СтрокаТаблицы.Номенклатура) <> Тип("Неопределено") Тогда
	...
	Если СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТараПодТоваромИПорожняя")
		Или СтрокаТаблицы.СчетУчета = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.КорректировкаТоваровПрошлогоПериода") Тогда
...
Процедура НоменклатураНаСкладеНоменклатураПриИзменении(Элемент)
	...
	ИначеЕсли ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		РеквизитыНоменклатуры	= РеквизитыНоменклатуры(СтрокаТаблицы.Номенклатура, "Код,Артикул,ЕдиницаИзмерения");
	...
	Иначе
		РеквизитыНоменклатуры	= РеквизитыНоменклатуры(СтрокаТаблицы.Номенклатура, "Код");
		СтрокаТаблицы.НоменклатураКод		= РеквизитыНоменклатуры.Код;
		СтрокаТаблицы.НоменклатураАртикул	= "";
		СтрокаТаблицы.НоменклатураЕдиницаИзмерения = Неопределено;
	КонецЕсли;
	...
	Если ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		РеквизитыПрослеживаемости = Новый Структура("СтранаПроисхождения, ПрослеживаемыйТовар, ПрослеживаемыйКомплект");
		ПараметрыОбъекта = Новый Структура("ВедетсяУчетПрослеживаемыхТоваров, Номенклатура, РазделУчета",
		ВедетсяУчетПрослеживаемыхТоваров, СтрокаТаблицы.Номенклатура, Объект.РазделУчета);
		ПолучитьРеквизитыПрослеживаемости(РеквизитыПрослеживаемости, ПараметрыОбъекта);
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, РеквизитыПрослеживаемости);
		
		Если СтрокаТаблицы <> Неопределено Тогда
			// Необходимо очистить дополнительные сведения
			НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
			НоваяСтрока.Ключ = СтрокаТаблицы.Ключ;
		КонецЕсли;
...
Процедура НоменклатураНаСкладеПартияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаПартия());
...
Процедура НоменклатураНаСкладеРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаРасчетныйДокумент());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
&НаКлиенте
Процедура НоменклатураНаСкладеРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
	Счет0803 = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств");
	СтандартнаяОбработка = Ложь;
	Если ТекущиеДанные.ПрослеживаемыйТовар 
		...
		И ТекущиеДанные.СчетУчета <> ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ТараПодТоваромИПорожняя")
		И ТекущиеДанные.СчетУчета <> ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.КорректировкаТоваровПрошлогоПериода"))
		Или ТекущиеДанные.СчетУчета = Счет0803 Тогда
			...
			ПараметрыФормыПодбора.ПрослеживаемыйКомплект = ТекущиеДанные.ПрослеживаемыйКомплект
					Или ТекущиеДанные.СчетУчета = Счет0803;
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "НоменклатураНаСкладе", ПараметрыФормыПодбора);
		КонецЕсли;
		
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураНаСкладеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "НоменклатураНаСкладеРНПТ" Тогда
		ТекущиеДанные = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
		Счет0803 = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств");
		Если ТекущиеДанные.ПрослеживаемыйТовар
			Или ТекущиеДанные.СчетУчета = Счет0803 Тогда
	...
	Если Поле.Имя = "НоменклатураНаСкладеСтранаПроисхождения" Тогда
		ТекущиеДанные = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
		Если (Объект.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаДляВводаОстатков.Материалы")
			Или Объект.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаДляВводаОстатков.КапитальныеВложения"))
			И НЕ ТекущиеДанные.ПрослеживаемыйТовар Тогда
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если Поле.Имя = "НоменклатураНаСкладеПрослеживаемыйТовар" Тогда
		ТекущиеДанные = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
...
&НаКлиенте
Процедура НоменклатураНаСкладеСтранаПроисхожденияПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
	
	СтранаПроисхожденияПриИзменении(ТекущиеДанные);
	
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураНаСкладеРНПТОчистка(Элемент, СтандартнаяОбработка)
	
	СтрокаТабличнойЧасти = Элементы.НоменклатураНаСкладе.ТекущиеДанные;
...
Процедура СпецодеждаИСпецоснасткаВЭксплуатацииПартияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("Организация",			Объект.Организация);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаПартияСпецодежды());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Процедура СпецодеждаИСпецоснасткаВЭксплуатацииРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаРасчетныйДокументСпецодежды());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Процедура МатериалыПереданныеВПереработкуПередУдалением(Элемент, Отказ)
	...
	ВыделенныеСтроки = Элементы.МатериалыПереданныеВПереработку.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для каждого Строка Из ВыделенныеСтроки Цикл
		ТекДанные = Объект.МатериалыПереданныеВПереработку.НайтиПоИдентификатору(Строка);
		Если ТекДанные <> Неопределено Тогда
			// Необходимо очистить дополнительные сведения
			НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
			НоваяСтрока.Ключ = ТекДанные.Ключ;
...
Процедура МатериалыПереданныеВПереработкуПартияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаПартияМатериаловВПереработку());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Процедура МатериалыПереданныеВПереработкуРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаРасчетныйДокументМатериаловВПереработку());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
&НаКлиенте
Процедура МатериалыПереданныеВПереработкуВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "МатериалыПереданныеВПереработкуРНПТ" Тогда
		СтандартнаяОбработка = Ложь;
		ТекущиеДанные = Элементы.МатериалыПереданныеВПереработку.ТекущиеДанные;
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			ПараметрыФормыПодбора = ПрослеживаемостьФормыКлиентБП.ПодготовитьСтруктуруПараметровДляФормыПодбораРНПТ();
			ПараметрыФормыПодбора.ПрослеживаемыйКомплект = ТекущиеДанные.ПрослеживаемыйКомплект;
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "МатериалыПереданныеВПереработку", ПараметрыФормыПодбора);
		КонецЕсли;
	КонецЕсли;
	
	Если Поле.Имя = "МатериалыПереданныеВПереработкуСтранаПроисхождения" Тогда
		ТекущиеДанные = Элементы.МатериалыПереданныеВПереработку.ТекущиеДанные;
	...
	Если Поле.Имя = "МатериалыПереданныеВПереработкуПрослеживаемыйТовар" Тогда
		ТекущиеДанные = Элементы.МатериалыПереданныеВПереработку.ТекущиеДанные;
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			ПрослеживаемостьФормыКлиентБП.ПоказатьПредупреждениеПрослеживаемыйТовар();
		Конецесли;
...
&НаКлиенте
Процедура МатериалыПереданныеВПереработкуСтранаПроисхожденияПриИзменении(Элемент)
...
&НаКлиенте
Процедура МатериалыПереданныеВПереработкуРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекущиеДанные = Элементы.МатериалыПереданныеВПереработку.ТекущиеДанные;
	Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
		ПараметрыФормыПодбора = ПрослеживаемостьФормыКлиентБП.ПодготовитьСтруктуруПараметровДляФормыПодбораРНПТ();
...
Процедура ТоварыВРозничнойТорговлеАТТПередУдалением(Элемент, Отказ)
	...
	ВыделенныеСтроки = Элементы.ТоварыВРозничнойТорговлеАТТ.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для каждого Строка Из ВыделенныеСтроки Цикл
		ТекДанные = Объект.ТоварыВРозничнойТорговлеАТТ.НайтиПоИдентификатору(Строка);
		Если ТекДанные <> Неопределено Тогда
			// Необходимо очистить дополнительные сведения
			НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
			НоваяСтрока.Ключ = ТекДанные.Ключ;
...
Процедура ТоварыВРозничнойТорговлеАТТПартияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаПартияРозничнойТорговли());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Процедура ТоварыВРозничнойТорговлеАТТРасчетныйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОбъекта.Вставить("СчетУчета",				СтрокаТаблицы.СчетУчета);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаРасчетныйДокументРозничнойТорговли());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
&НаКлиенте
Процедура ТоварыВРозничнойТорговлеАТТВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если Поле.Имя = "ТоварыВРозничнойТорговлеАТТРНПТ" Тогда
		ТекущиеДанные = Элементы.ТоварыВРозничнойТорговлеАТТ.ТекущиеДанные;
		Если ТекущиеДанные.ПрослеживаемыйТовар Тогда
			СтандартнаяОбработка = Ложь;
			ПараметрыФормыПодбора = ПрослеживаемостьФормыКлиентБП.ПодготовитьСтруктуруПараметровДляФормыПодбораРНПТ();
			ПараметрыФормыПодбора.ПрослеживаемыйКомплект = ТекущиеДанные.ПрослеживаемыйКомплект;
			ПрослеживаемостьФормыКлиентБП.ОткрытьФормуПодбораРНПТ(ЭтаФорма, "ТоварыВРозничнойТорговлеАТТ", ПараметрыФормыПодбора);
		КонецЕсли;
		
	КонецЕсли;
	
	Если Поле.Имя = "ТоварыВРозничнойТорговлеАТТПрослеживаемыйТовар" Тогда
...
&НаКлиенте
Процедура ТоварыВРозничнойТорговлеАТТРНПТНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ТекущиеДанные = Элементы.ТоварыВРозничнойТорговлеАТТ.ТекущиеДанные;
	СтандартнаяОбработка = Ложь;
	ПараметрыФормыПодбора = ПрослеживаемостьФормыКлиентБП.ПодготовитьСтруктуруПараметровДляФормыПодбораРНПТ();
	ПараметрыФормыПодбора.ПрослеживаемыйКомплект = ТекущиеДанные.ПрослеживаемыйКомплект;
...
&НаКлиенте
Процедура ТоварыВРозничнойТорговлеАТТСтранаПроисхожденияПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТоварыВРозничнойТорговлеАТТРНПТОчистка(Элемент, СтандартнаяОбработка)
	
	СтрокаТабличнойЧасти = Элементы.ТоварыВРозничнойТорговлеАТТ.ТекущиеДанные;
...
Процедура УстановитьУсловноеОформлениеНоменклатураНаСкладе() Экспорт
	...
	// ДанныеПоСФНоменклатураНаСкладеКонтрагент
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДанныеПоСФНоменклатураНаСкладеКонтрагент");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	...
	// НоменклатураНаСкладеСтранаПроисхождения
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеСтранаПроисхождения");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.ПрослеживаемыйКомплект", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.СтранаПроисхождения", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, Перечисления.РазделыУчетаДляВводаОстатков.Товары);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеСтранаПроисхождения");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	
	ГруппаОтбора1 = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(ЭлементУО.Отбор.Элементы, ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, Перечисления.РазделыУчетаДляВводаОстатков.Материалы);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.НоменклатураНаСкладе.СчетУчета", ВидСравненияКомпоновкиДанных.Равно, ПланыСчетов.Хозрасчетный.ТараПодТоваромИПорожняя);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.НоменклатураНаСкладе.СчетУчета", ВидСравненияКомпоновкиДанных.Равно, ПланыСчетов.Хозрасчетный.КорректировкаТоваровПрошлогоПериода);

	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НедоступныеДанныеЦвет);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не требуется>'"));
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	//СтранаПроисхождения по готовой продукции
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеСтранаПроисхождения");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);

	
	// НоменклатураНаСкладеРНПТ
	// Если это Материалы
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеРНПТ");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
		"Объект.НоменклатураНаСкладе.СчетУчета", ВидСравненияКомпоновкиДанных.НеРавно,
		ПланыСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств);
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, 
		Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения);
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно, Перечисления.РазделыУчетаДляВводаОстатков.Товары);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.НомерГТД", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.РазделУчета", ВидСравненияКомпоновкиДанных.Равно,
	Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеРНПТ");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.ПрослеживаемыйКомплект", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НоменклатураНаСкладе.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НоменклатураНаСкладеПрослеживаемыйТовар");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
...
Процедура УстановитьУсловноеОформлениеМатериалыПереданныеВПереработку() Экспорт
	...
	// МатериалыПереданныеВПереработкуСтранаПроисхождения
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МатериалыПереданныеВПереработкуСтранаПроисхождения");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.СтранаПроисхождения", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.ПрослеживаемыйКомплект", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МатериалыПереданныеВПереработкуСтранаПроисхождения");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.НедоступныеДанныеЦвет);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='<не требуется>'"));
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	
	// МатериалыПереданныеВПереработкуРНПТ
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МатериалыПереданныеВПереработкуРНПТ");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.МатериалыПереданныеВПереработку.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
	
	// Признак прослеживаемости
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "МатериалыПереданныеВПереработкуПрослеживаемыйТовар");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"ВедетсяУчетПрослеживаемыхТоваров", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
...
Процедура УстановитьУсловноеОформлениеТоварыВРозничнойТорговлеАТТ() Экспорт
	...
	// ДанныеПоСФТоварыВРозничнойТорговлеАТТКонтрагент
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();

	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДанныеПоСФТоварыВРозничнойТорговлеАТТКонтрагент");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
			"Объект.Дата", ВидСравненияКомпоновкиДанных.Меньше, ДатаНачалаВеденияПолныхЗаписейПоРУ);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);

	// ТоварыВРозничнойТорговлеАТТСтранаПроисхождения
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВРозничнойТорговлеАТТСтранаПроисхождения");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйКомплект", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.СтранаПроисхождения", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
	
	// ТоварыВРозничнойТорговлеАТТРНПТ
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВРозничнойТорговлеАТТРНПТ");
	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.СтранаПроисхождения", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.НомерГТД", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ТоварыВРозничнойТорговлеАТТ.РНПТ", ВидСравненияКомпоновкиДанных.НеЗаполнено, "");
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
		...
		// Признак прослеживаемости
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыВРозничнойТорговлеАТТПрослеживаемыйТовар");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"ВедетсяУчетПрослеживаемыхТоваров", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
...
Процедура ПодготовитьФормуНаСервере()
	...
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы Тогда
		...
		МассивИсключений.Добавить(ПланыСчетов.Хозрасчетный.ТопливоВБаке);
		...
		МассивСпец = Новый Массив;
		МассивСпец.Добавить(ПланыСчетов.Хозрасчетный.ТопливоВБаке);
		БухгалтерскийУчетКлиентСервер.ИзменитьПараметрыВыбораСчета(Элементы.ТопливоСчетУчета, МассивСпец);
	...
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы Тогда
		...
		Элементы.ГруппаТопливо.Видимость = ПолучитьФункциональнуюОпцию("ВедетсяУчетПоПутевымЛистам");
...
Процедура УправлениеФормойСервер()
	...
	Если ОтражатьГТДВместеСОстатками И НЕ ВедетсяУчетПрослеживаемыхТоваров Тогда 
		ТекстЗаголовкаРНПТ = НСтр("ru = 'Таможенная декларация'");
		Элементы.НоменклатураНаСкладеПодсказкаРНПТ.Видимость		= Ложь;
		Элементы.ТоварыВРозничнойТорговлеАТТПодсказкаРНПТ.Видимость = Ложь;
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Товары Тогда
		Элементы.НоменклатураНаСкладеПодсказкаРНПТ.Видимость		 = Истина;
		Элементы.ТоварыВРозничнойТорговлеАТТПодсказкаРНПТ.Видимость  = Истина;
	ИначеЕсли Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы
		Или Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения
		Или Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты Тогда
		ТекстЗаголовкаРНПТ = НСтр("ru = 'РНПТ'");
		Элементы.НоменклатураНаСкладеПодсказкаРНПТ.Видимость = Ложь;
	КонецЕсли;
	Элементы.ГруппаРНПТВАТТ.Заголовок					= ТекстЗаголовкаРНПТ;
	Элементы.ГруппаРНПТНоменклатураНаСкладе.Заголовок 	= ТекстЗаголовкаРНПТ;
	
	Если Не Объект.ОтражатьВБухгалтерскомУчете Или Не ОтражатьГТДВместеСОстатками Тогда
		Элементы.ГруппаПрослеживаемостьНоменклатураНаСкладе.Видимость = Ложь;
		Элементы.ГруппаПрослеживаемостьМатериалыПереданныеВПереработку.Видимость = Ложь;
		Элементы.ГруппаПрослеживаемостьТоварыВАТТ.Видимость = Ложь;
		Объект.СведенияПрослеживаемости.Очистить(); 
	ИначеЕсли Объект.ОтражатьВБухгалтерскомУчете
		И ОтражатьГТДВместеСОстатками
		И НЕ ВедетсяУчетПрослеживаемыхТоваров
		И (Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы 
		Или Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения
		Или Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты) Тогда
		Элементы.ГруппаПрослеживаемостьНоменклатураНаСкладе.Видимость = Ложь;
		Элементы.ГруппаПрослеживаемостьМатериалыПереданныеВПереработку.Видимость = Ложь;
		...
		Объект.СведенияПрослеживаемости.Очистить();
	Иначе
		Элементы.ГруппаПрослеживаемостьНоменклатураНаСкладе.Видимость = ВедетсяУчетПрослеживаемыхТоваров;
		Элементы.ГруппаПрослеживаемостьМатериалыПереданныеВПереработку.Видимость = ВедетсяУчетПрослеживаемыхТоваров;
		Элементы.ГруппаПрослеживаемостьТоварыВАТТ.Видимость = ВедетсяУчетПрослеживаемыхТоваров;
	КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	ОбъектыДляРеквизитов = Новый Массив(2);
	ОбъектыДляРеквизитов[0] = Новый Массив; // для ссылок на Номенклатура
	ОбъектыДляРеквизитов[1] = Новый Массив; // для ссылок на другие типы
	Для каждого СтрокаТаблицы Из Объект.НоменклатураНаСкладе Цикл
		Если ЗначениеЗаполнено(СтрокаТаблицы.Номенклатура) И ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета) Тогда
			ИндексСписка = ?(ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура"), 0, 1);
			ОбъектыДляРеквизитов[ИндексСписка].Добавить(СтрокаТаблицы.Номенклатура);
		КонецЕсли;
	КонецЦикла;
	КешРеквизитовНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ОбъектыДляРеквизитов[0], "Код,Артикул,ЕдиницаИзмерения");
	ОбъектыДляРеквизитов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ОбъектыДляРеквизитов[1], "Код");
	Для Каждого ОбъектДляРеквизитов Из ОбъектыДляРеквизитов Цикл
		КешРеквизитовНоменклатуры.Вставить(ОбъектДляРеквизитов.Ключ, Новый Структура("Код", ОбъектДляРеквизитов.Значение));
	КонецЦикла;
	
	ПараметрыДокумента = Новый Структура;
	ПараметрыДокумента.Вставить("ВалютаРегламентированногоУчета", ВалютаРегламентированногоУчета);
	ПараметрыДокумента.Вставить("РазделУчета",                    Объект.РазделУчета);
	ПараметрыДокумента.Вставить("Организация",                    Объект.Организация);
	ПараметрыДокумента.Вставить("РаздельныйУчетНДС",              РаздельныйУчетНДС);
	ПараметрыДокумента.Вставить("КешРеквизитовНоменклатуры",      КешРеквизитовНоменклатуры);
	...
	Для каждого СтрокаТаблицы Из Объект.НоменклатураНаСкладе Цикл
		...
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
			ЭтотОбъект, СтрокаТаблицы, Объект.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Товары);
	...
	Для каждого СтрокаТаблицы Из Объект.МатериалыПереданныеВПереработку Цикл
		...
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокаТаблицы, Ложь);
	...
	Для каждого СтрокаТаблицы Из Объект.ТоварыВРозничнойТорговлеАТТ Цикл
		...
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокаТаблицы, Истина);
...
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицы_НоменклатураНаСкладе(СтрокаТаблицы, ПараметрыДокумента)
	...
	Перем КешРеквизитовНоменклатуры;
	...
	СчетаКапВложенийСУчетомПоРУ = Документы.ВводНачальныхОстатков.СчетаКапВложенийДляУчетаПоРУ();
	...
	Если НЕ СтрокаТаблицы.СчетУчетаСубконтоПартия И ПараметрыДокумента.РаздельныйУчетНДС Тогда
		ПроверитьВхождение =  СчетаКапВложенийСУчетомПоРУ.Найти(СтрокаТаблицы.СчетУчета); 
		Если НЕ ПроверитьВхождение = Неопределено Тогда
			СтрокаТаблицы.СчетУчетаСубконтоПартия = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	ПараметрыДокумента.Свойство("КешРеквизитовНоменклатуры", КешРеквизитовНоменклатуры);
	РеквизитыОбъекта = ?(КешРеквизитовНоменклатуры = Неопределено, Неопределено, КешРеквизитовНоменклатуры[СтрокаТаблицы.Номенклатура]);
	ЭтоСправочникНоменклатура = (ТипЗнч(СтрокаТаблицы.Номенклатура) = Тип("СправочникСсылка.Номенклатура"));
	Если РеквизитыОбъекта = Неопределено Тогда
		СписокРеквизитов = ?(ЭтоСправочникНоменклатура, "Код,Артикул,ЕдиницаИзмерения", "Код");
		РеквизитыОбъекта = РеквизитыНоменклатуры(СтрокаТаблицы.Номенклатура, СписокРеквизитов);
	КонецЕсли;
	СтрокаТаблицы.НоменклатураКод = РеквизитыОбъекта.Код;
	Если ЭтоСправочникНоменклатура Тогда
		СтрокаТаблицы.НоменклатураАртикул = РеквизитыОбъекта.Артикул;
		СтрокаТаблицы.НоменклатураЕдиницаИзмерения = РеквизитыОбъекта.ЕдиницаИзмерения;
	КонецЕсли;
...
Процедура ДанныеПоСФСчетФактураНачалоВыбора(Элемент)
	...
	ПараметрыОбъекта.Вставить("Организация",			Объект.Организация);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаСчетФактура());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Процедура ОборотыНачалоВыбора(Элемент)
	...
	ПараметрыОбъекта.Вставить("Организация",			Объект.Организация);
	ПараметрыОбъекта.Вставить("ОстаткиОбороты",			"Дт");
	ПараметрыОбъекта.Вставить("ТипыДокументов",			ОписаниеТипаРеквизитаПартияОборотов());
	ПараметрыОбъекта.Вставить("РежимОтбораДокументов",	ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоРеквизитам"));
...
Функция ПоляСтрокиТабличнойЧасти(СтрокаТаблицы)
	...
	ПараметрыСтроки = Новый Структура("СчетУчета, СчетУчетаНалоговыйУчет, СчетУчетаКоличественный,
			...
			|Номенклатура, НоменклатураКод, НоменклатураАртикул, НоменклатураЕдиницаИзмерения,
...
&НаСервереБезКонтекста
Функция РеквизитыНоменклатуры(Ссылка, Реквизиты)
...
Процедура УдалитьСвязанныеЗаписиНаСервере()
	...
	Для каждого СтрокаКлюча Из ТаблицаУдаленныхСтрок Цикл
		...
		СтрокиНоменклатура = Объект.НоменклатураНаСкладе.НайтиСтроки(СтруктураОтбора);
		Если СтрокиНоменклатура.Количество() > 0 Тогда
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
				ЭтотОбъект, СтрокиНоменклатура[0], Объект.РазделУчета=Перечисления.РазделыУчетаДляВводаОстатков.Товары);
		КонецЕсли;
		
		СтрокиМатериалыВПереработке = Объект.МатериалыПереданныеВПереработку.НайтиСтроки(СтруктураОтбора);
		Если СтрокиМатериалыВПереработке.Количество() > 0 Тогда
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокиМатериалыВПереработке[0], Ложь);
		КонецЕсли;
		
		СтрокиТоварыВАТТ = Объект.ТоварыВРозничнойТорговлеАТТ.НайтиСтроки(СтруктураОтбора);
		Если СтрокиТоварыВАТТ.Количество() > 0 Тогда
			ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(ЭтотОбъект, СтрокиТоварыВАТТ[0], Истина);
		КонецЕсли;
...
&НаСервере
Процедура ОбработкаВыбораРНПТНаСервере(Ключ, ВыбранноеЗначение)
	
	СтрокиСРНПТ = Объект.СведенияПрослеживаемости.НайтиСтроки(Новый Структура("Ключ", Ключ));
...
Процедура ОбработкаВыбораРНПТНаКлиенте(ВыбранноеЗначение, ИмяТаблицы)
	...
	Если СтрокаТабличнойЧасти <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.Ключ) Тогда
			СтрокаТабличнойЧасти.Ключ = Новый УникальныйИдентификатор;
		КонецЕсли;
		ОбработкаВыбораРНПТНаСервере(СтрокаТабличнойЧасти.Ключ, ВыбранноеЗначение);
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
			ЭтаФорма, СтрокаТабличнойЧасти, 
			Объект.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыУчетаДляВводаОстатков.Товары"));
	КонецЕсли;
...
&НаКлиенте
Процедура СтранаПроисхожденияПриИзменении(ТекущиеДанные)
	
	 Если ТекущиеДанные <> Неопределено Тогда
		// Необходимо очистить дополнительные сведения
		НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
		НоваяСтрока.Ключ = ТекущиеДанные.Ключ;
		ПрослеживаемостьФормыКлиентСерверБП.ЗаполнитьПредставлениеРНПТ(
...
&НаСервереБезКонтекста
Процедура ПолучитьРеквизитыПрослеживаемости(ПараметрыСтроки, ПараметрыОбъекта)
	
	Если ПараметрыОбъекта.ВедетсяУчетПрослеживаемыхТоваров Тогда
		РеквизитыНоменклатурыПоПрослеживаемости = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ПараметрыОбъекта.Номенклатура, "СтранаПроисхождения, ПрослеживаемыйТовар, ПрослеживаемыйКомплект");
		ПараметрыСтроки.СтранаПроисхождения    = РеквизитыНоменклатурыПоПрослеживаемости.СтранаПроисхождения;
		Если ПараметрыОбъекта.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты Тогда
			ПараметрыСтроки.ПрослеживаемыйТовар = РеквизитыНоменклатурыПоПрослеживаемости.ПрослеживаемыйКомплект;
		Иначе
			ПараметрыСтроки.ПрослеживаемыйТовар    = РеквизитыНоменклатурыПоПрослеживаемости.ПрослеживаемыйТовар
			Или РеквизитыНоменклатурыПоПрослеживаемости.ПрослеживаемыйКомплект;
		КонецЕсли;
		ПараметрыСтроки.ПрослеживаемыйКомплект = РеквизитыНоменклатурыПоПрослеживаемости.ПрослеживаемыйКомплект;
	Иначе
		ПараметрыСтроки.СтранаПроисхождения    = ПредопределенноеЗначение("Справочник.СтраныМира.ПустаяСсылка");
		ПараметрыСтроки.ПрослеживаемыйТовар    = Ложь;
		ПараметрыСтроки.ПрослеживаемыйКомплект = Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаПартия()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.НоменклатураНаСкладе.Реквизиты.Партия.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаРасчетныйДокумент()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.НоменклатураНаСкладе.Реквизиты.РасчетныйДокумент.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаПартияСпецодежды()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.СпецодеждаИСпецоснасткаВЭксплуатации.Реквизиты.Партия.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаРасчетныйДокументСпецодежды()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.СпецодеждаИСпецоснасткаВЭксплуатации.Реквизиты.РасчетныйДокумент.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаПартияМатериаловВПереработку()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.МатериалыПереданныеВПереработку.Реквизиты.Партия.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаРасчетныйДокументМатериаловВПереработку()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.МатериалыПереданныеВПереработку.Реквизиты.РасчетныйДокумент.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаПартияРозничнойТорговли()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.ТоварыВРозничнойТорговлеАТТ.Реквизиты.Партия.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаРасчетныйДокументРозничнойТорговли()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.ТоварыВРозничнойТорговлеАТТ.Реквизиты.РасчетныйДокумент.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаСчетФактура()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.ДанныеПоСФ.Реквизиты.СчетФактура.Тип;
	
КонецФункции

&НаСервереБезКонтекста
Функция ОписаниеТипаРеквизитаПартияОборотов()
	
	Возврат Метаданные.Документы.ВводНачальныхОстатков.ТабличныеЧасти.Обороты.Реквизиты.Партия.Тип;
