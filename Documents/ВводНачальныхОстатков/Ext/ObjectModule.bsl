...
Процедура ПроверкаЗаполненияКолонокТабличныхЧастейПоСубконто(
	...
	СубконтоСпособыСтроительства = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СпособыСтроительства;
	Если Не ОтражатьВБухгалтерскомУчете И НЕ ОтражатьВНалоговомУчете Тогда
		ДвиженияТолькоПоСпецРегистрам = Истина;
	Иначе
		ДвиженияТолькоПоСпецРегистрам = Ложь;
	КонецЕсли;
		...
		Для Каждого ТекущийВидСубконто Из ВсеСубконтоСчета Цикл
			...
			Если (СоответствующаяВидуСубконтоКолонка <> Неопределено) И (НЕ ПустаяСтрока(СоответствующаяВидуСубконтоКолонка)) Тогда
				...
				Если НЕ ЗначениеЗаполнено(Значение) Тогда
					Если  ДвиженияТолькоПоСпецРегистрам И ТекущийВидСубконто.ВидСубконто = СубконтоСпособыСтроительства   Тогда
						Продолжить;
					Иначе
						Префикс = "" + ИмяТабличнойЧасти + "[" + Формат(ТекущаяСтрока.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].";
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПроверкаТабличнойЧастиОС(Отказ)
	...
	Для Каждого Строка Из ОС Цикл 
		...
		Если Строка.ПорядокВключенияСтоимостиВСоставРасходовНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету
		...
		// Для полученных в лизинг (остатки до ФСБУ 25) объектов ОС необходимо указание контрагента-арендодателя и договора аренды,
		// а также способа отражения расходов по арендным платежам для плательщиков налога на прибыль 
		Если Строка.СпособПоступленияРегл = Перечисления.СпособыПоступленияАктивов.ПоДоговоруЛизинга 
			И Не РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг
...
Процедура ПроверкаОСНеПринятыКУчетуВДругихОрганизациях(Отказ)
	...
	Если Метаданные.Документы.Найти("АвизоОСВходящее") <> Неопределено Тогда 
		ДопУсловие = " И НЕ Рег.Регистратор ССЫЛКА Документ.АвизоОСВходящее";
	Иначе
		ДопУсловие = "";
	КонецЕсли;
	...
	Запрос.Текст = "
		...
		|	И Рег.Организация <> &Организация" + ДопУсловие + "
		|ИНДЕКСИРОВАТЬ ПО
		|	Организация,
		|	ОсновноеСредство,
		|	ДатаСостояния
...
Процедура СформироватьСчетаФактуры(Отказ)
	...
	|	И НЕ &РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости)
	|	И ВЫБОР
	|			КОГДА Док.Ссылка.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСПоПриобретеннымЦенностям)
	|				ТОГДА ВЫБОР
	|						КОГДА Док.ПредъявленСФ
	...
	|	Док.КодВидаОперации
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НДСУчтенныйВСтоимости КАК Док
	|ГДЕ
	|	Док.Ссылка = &Ссылка
	|	И Док.СчетФактураДокумент В(&МассивПустыхСсылок)
	|	И &РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА Док.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСпоАвансамИПредоплатам)
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда  
		КолонкиГруппировок	= "Контрагент, ДоговорКонтрагента, СчетФактура, ДатаСФ, НомерСФ, ВалютаДокумента, Продавец, КодВидаОперации, КорректировочныйСчетФактура";
		КолонкиСуммирования	= "";
	Иначе 
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаСчетовФактур Цикл
		...
		Попытка
			...
			ИначеЕсли РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда
...
Функция СформироватьСчетФактуруПолученный(СтрокаТаблицы)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ДатаСФ)
		И РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда 
		НовыйДокумент.Дата						= СтрокаТаблицы.ДатаСФ;
	Иначе 
...
Процедура СинхронизироватьСостояниеСчетаФактуры()
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда 
		...
		|	И Док.СчетФактураДокумент.СформированПриВводеНачальныхОстатковНДС
		|
		|СГРУППИРОВАТЬ ПО
		...
		|						ТОГДА ""ОтменитьПроведениеУстановитьПометкуУдаления""
		|					ИНАЧЕ ""УстановитьПометкуУдаления""
		|				КОНЕЦ
		|		КОГДА &Проведен
		|				И НЕ СчетФактураПолученный.Проведен
		|			ТОГДА ВЫБОР
		|					КОГДА СчетФактураПолученный.ПометкаУдаления
		|						ТОГДА ""СнятьПометкуУдаленияПровести""
		|					ИНАЧЕ ""Провести""
		|				КОНЕЦ
		|		КОГДА СчетФактураПолученный.ПометкаУдаления
		|			ТОГДА ""СнятьПометкуУдаления""
		|		КОГДА СчетФактураПолученный.Проведен
		|			ТОГДА ""ОтменитьПроведение""
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК Действие
		|ИЗ
		|	ВТ_СчетаФактуры КАК ВТ_СчетаФактуры
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный КАК СчетФактураПолученный
		|		ПО ВТ_СчетаФактуры.Ссылка = СчетФактураПолученный.Ссылка
		|ГДЕ
		|	(СчетФактураПолученный.ПометкаУдаления <> &ПометкаУдаления
		|			ИЛИ СчетФактураПолученный.Проведен <> &Проведен)";
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл 
			
			Если Выборка.Действие = "" Тогда 
				Продолжить;
			КонецЕсли;
			...
			Если Выборка.Действие = "ОтменитьПроведениеУстановитьПометкуУдаления" Тогда  
			...
			ИначеЕсли Выборка.Действие = "УстановитьПометкуУдаления" Тогда  
				СчетФактураОбъект.УстановитьПометкуУдаления(Истина);
			ИначеЕсли Выборка.Действие = "СнятьПометкуУдаленияПровести" Тогда  
				СчетФактураОбъект.ПометкаУдаления = Ложь;
				СчетФактураОбъект.Записать(РежимЗаписиДокумента.Проведение); 
			ИначеЕсли Выборка.Действие = "Провести" Тогда 
				СчетФактураОбъект.Записать(РежимЗаписиДокумента.Проведение);
			ИначеЕсли Выборка.Действие = "СнятьПометкуУдаления" Тогда 
				СчетФактураОбъект.УстановитьПометкуУдаления(Ложь);
			ИначеЕсли Выборка.Действие = "ОтменитьПроведение" Тогда 
				СчетФактураОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
			КонецЕсли;
...
Процедура ОбновитьСчетаФактуры(Отказ)
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	&Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТРегламентныеДокументы
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫРАЗИТЬ(РегламентнаяОперацияПоРегистратору.РегламентныйДокумент КАК Документ.ВводНачальныхОстатков)
	|ИЗ
	|	РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК РегламентнаяОперацияПоРегистратору
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК ВыполнениеРегламентныхОперацийНДС
	|		ПО (&РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости))
	|			И (РегламентнаяОперацияПоРегистратору.Организация = &Организация)
	|			И ((ВЫРАЗИТЬ(РегламентнаяОперацияПоРегистратору.РегламентныйДокумент КАК Документ.ВводНачальныхОстатков)) = &Ссылка)
	|			И РегламентнаяОперацияПоРегистратору.Организация = ВыполнениеРегламентныхОперацийНДС.Организация
	|			И РегламентнаяОперацияПоРегистратору.РегламентнаяОперация = ВыполнениеРегламентныхОперацийНДС.РегламентнаяОперация
	|			И РегламентнаяОперацияПоРегистратору.ПериодРегистрации = ВыполнениеРегламентныхОперацийНДС.ПериодРегистрации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетФактураДокумент КАК СчетФактураДокумент
	...
	|	И НЕ ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетФактураДокумент В (&МассивПустыхСсылок)
	|	И ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетФактураДокумент.СформированПриВводеНачальныхОстатковНДС
	|
	|СГРУППИРОВАТЬ ПО
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетФактураДокумент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Док.СтавкаНДС КАК СтавкаНДС,
	|	Док.Партия КАК СчетФактура,
	|	СУММА(Док.СуммаБезНДС) КАК СуммаБезНДС,
	|	СУММА(Док.НДС) КАК НДС,
	...
	|	Док.НомерИсправленияИсходногоДокумента КАК НомерИсправленияИсходногоДокумента,
	|	Док.ДатаИсправленияИсходногоДокумента КАК ДатаИсправленияИсходногоДокумента,
	|	Док.Контрагент КАК Продавец,
	|	Док.КодВидаОперации КАК КодВидаОперации
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НДСУчтенныйВСтоимости КАК Док
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументовДокументОплаты
	|		ПО (ДанныеПервичныхДокументовДокументОплаты.Организация = &Организация)
	|			И Док.СчетФактураДокумент = ДанныеПервичныхДокументовДокументОплаты.Документ
	|ГДЕ
	|	&РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости)
	|	И Док.Ссылка В
	|			(ВЫБРАТЬ
	|				ВТРегламентныеДокументы.Ссылка КАК Ссылка
	|			ИЗ
	|				ВТРегламентныеДокументы КАК ВТРегламентныеДокументы)
	|	И Док.СчетФактураДокумент В
	|			(ВЫБРАТЬ
	|				ВТСчетаФактуры.СчетФактураДокумент КАК СчетФактураДокумент
	|			ИЗ
	...
	|ГДЕ
	|	НЕ &РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости)
	|	И Док.Ссылка = &Ссылка
	|	И НЕ Док.СчетФактураДокумент В (&МассивПустыхСсылок)
	|
	|СГРУППИРОВАТЬ ПО
...
// Процедура обновляет содерджимое полученного счета-фактуры
//
Процедура ОбновитьСчетФактуруПолученный(ДокументСсылка, ДанныеСчетаФактуры, Отказ)
	...
	Попытка
		...
		ДанныеДоИзменений = ДанныеСчетаФактуры(СчетФактура);
		...
		ДанныеПослеИзменений = ДанныеСчетаФактуры(СчетФактура);
		
		Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ДанныеДоИзменений.Реквизиты, ДанныеПослеИзменений.Реквизиты)
			ИЛИ НЕ ОбщегоНазначения.КоллекцииИдентичны(ДанныеДоИзменений.ДокументыОснования, ДанныеПослеИзменений.ДокументыОснования)
			ИЛИ НЕ ОбщегоНазначения.КоллекцииИдентичны(ДанныеДоИзменений.Авансы, ДанныеПослеИзменений.Авансы) Тогда 
...
// Процедура обновляет содерджимое выданного счета-фактуры
	...
	Попытка
		...
		Пока ДанныеСчетаФактуры.Следующий() Цикл
			...
			Если СчетФактура.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаАванс Тогда
				...
				СчетФактура.СуммаДокумента = СчетФактура.СуммаДокумента + ДанныеСчетаФактуры.СуммаБезНДС + ДанныеСчетаФактуры.НДС;
				СчетФактура.СуммаНДСДокумента = СчетФактура.СуммаНДСДокумента + ДанныеСчетаФактуры.НДС;
				СчетФактура.ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
...
Процедура ЗаполитьДокументСформированнымиCчетамиФактурами(ТаблицаСчетовФактур)
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаСчетовФактур Цикл
		СтруктураОтбора = Новый Структура("Контрагент, СчетФактура, НомерСФ, ДатаСФ");
		ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаТаблицы);
		
		Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда
			СтруктураОтбора.Вставить("Партия",СтруктураОтбора.СчетФактура);
			СтруктураОтбора.Удалить("СчетФактура");
			МассивСтрокДокумента = НДСУчтенныйВСтоимости.НайтиСтроки(СтруктураОтбора);
		Иначе 
...
Процедура ПроверитьЗаполнениеРеквизитовПоПрослеживаемости(ИмяТабличнойЧасти, Отказ)
	...
	Если ИмяТабличнойЧасти = "БухСправка" Тогда
		ОстаткиПоЗабалансу = Истина;
		
		СубконтоПартии = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Партии;
		
		Счет00401 = ПланыСчетов.Хозрасчетный.ТоварыНаСкладе;
		Счет00402 = ПланыСчетов.Хозрасчетный.ТоварыПереданныеНаКомиссию;
		Счет002 = ПланыСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение;
	Иначе
		ОстаткиПоЗабалансу = Ложь;
	КонецЕсли;
	
	СчетИсключенияПриПроверке = ПланыСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости; // 45.04
	Для Каждого ТекущаяСтрока Из ЭтотОбъект[ИмяТабличнойЧасти] Цикл
		Если Не ТекущаяСтрока.ПрослеживаемыйТовар Тогда
			Продолжить;
		КонецЕсли;
		НужнаПроверка = Ложь;
		Если ИмяТабличнойЧасти = "МПЗОтгруженные" Тогда 
			Если ТекущаяСтрока.ПрослеживаемыйТовар
				И ТекущаяСтрока.ДоговорПокупателя.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером 
				И Не ТекущаяСтрока.СчетУчета = СчетИсключенияПриПроверке Тогда
				НужнаПроверка = Истина;
			КонецЕсли;
		ИначеЕсли ТекущаяСтрока.ПрослеживаемыйТовар Тогда
			НужнаПроверка = Истина;
		КонецЕсли;
		
		Если НужнаПроверка Тогда 
			Префикс = "" + ИмяТабличнойЧасти + "[" + Формат(ТекущаяСтрока.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].";
			Если Не ЭтоОС И Не ОстаткиПоЗабалансу
				И Не ТекущаяСтрока.ПрослеживаемыйКомплект
				И Не ЗначениеЗаполнено(ТекущаяСтрока.СтранаПроисхождения) Тогда
				ИмяРеквизита = "СтранаПроисхождения";
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			КонецЕсли;
			ИмяРеквизита = "РНПТ";
			СтруктураОтбора = Новый Структура("Ключ", ТекущаяСтрока.Ключ);
			МассивСтрокСведенияПрослеживаемости = СведенияПрослеживаемости.НайтиСтроки(СтруктураОтбора);
			КоличествоСтрокСРНПТ = МассивСтрокСведенияПрослеживаемости.Количество();
			
			Если Не ЭтоОС И ТекущаяСтрока.Количество > 0 Тогда
				
				Если ТекущаяСтрока.ПрослеживаемыйКомплект И КоличествоСтрокСРНПТ = 0 Тогда
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнены РНПТ для комплекта'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность", НСтр("ru = 'РНПТ'"),
					ТекущаяСтрока.НомерСтроки, ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				ИначеЕсли Не ТекущаяСтрока.ПрослеживаемыйКомплект И (КоличествоСтрокСРНПТ = 0 
					ИЛИ ПрослеживаемостьБП.КоличествоРНПТКСтроке(МассивСтрокСведенияПрослеживаемости) <> ТекущаяСтрока.Количество) Тогда
			...
			ИначеЕсли ЭтоОС Тогда
				...
				Если КоличествоСтрокСРНПТ = 0 Тогда
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнены РНПТ для основного средства'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
						НСтр("ru = 'Основное средство'"), ТекущаяСтрока.НомерСтроки,
						ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				КонецЕсли;
			КонецЕсли;
			
			Если ЭтоОС И МассивСтрокСведенияПрослеживаемости.Количество() > 1 Тогда
				НужноЗаполнениеНоменклатуры = Истина;
			ИначеЕсли Не ЭтоОС И ТекущаяСтрока.ПрослеживаемыйКомплект Тогда
				НужноЗаполнениеНоменклатуры = Истина;
			Иначе
				НужноЗаполнениеНоменклатуры = Ложь;
			КонецЕсли;
			
			Если ЭтоОС Или ИмяТабличнойЧасти = "БухСправка" Тогда
				НужноЗаполнениеСтраныПроисхождения = Истина;
			ИначеЕсли Не ЭтоОС И ТекущаяСтрока.ПрослеживаемыйКомплект Тогда
				НужноЗаполнениеСтраныПроисхождения = Истина;
			Иначе
				НужноЗаполнениеСтраныПроисхождения = Ложь;
			КонецЕсли; 
			
			Для Каждого Строка Из МассивСтрокСведенияПрослеживаемости Цикл
				
				Если НужноЗаполнениеНоменклатуры И Не ЗначениеЗаполнено(Строка.Номенклатура) Тогда 
					
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнена колонка ""Комплектующие"" в данных по РНПТ'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
					НСтр("ru = 'Комплектующие'"), ТекущаяСтрока.НомерСтроки,
					ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				...
				Если НужноЗаполнениеСтраныПроисхождения И Не ЗначениеЗаполнено(Строка.СтранаПроисхождения) Тогда
					
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнена колонка ""Страна происхождения"" в данных по РНПТ'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
					НСтр("ru = 'Страна происхождения'"), ТекущаяСтрока.НомерСтроки,
					ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				...
				Если (ЭтоОС Или (Не ЭтоОС И ТекущаяСтрока.ПрослеживаемыйКомплект)) И Не ЗначениеЗаполнено(Строка.Количество) Тогда
					
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнена колонка ""Количество"" в данных по РНПТ'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
					НСтр("ru = 'Количество'"), ТекущаяСтрока.НомерСтроки,
					ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				...
				Если Не ЗначениеЗаполнено(Строка.РНПТ) Тогда 
					ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнена колонка ""РНПТ"" в данных по РНПТ'");
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
					НСтр("ru = 'РНПТ'"), ТекущаяСтрока.НомерСтроки,
					ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифрвка);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
					Отказ	= Истина;
				КонецЕсли;
		...
		Если ИмяТабличнойЧасти = "БухСправка" Тогда
			Если ТекущаяСтрока.СчетУчета = Счет00401
				Или ТекущаяСтрока.СчетУчета = Счет00402
				Или ТекущаяСтрока.СчетУчета = Счет002 Тогда
				ДанныеСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ТекущаяСтрока.СчетУчета);
				
				Для Индекс = 1 По 3 Цикл
					Если Индекс <= ДанныеСчета.КоличествоСубконто Тогда
						Если Не ЗначениеЗаполнено(ТекущаяСтрока["Субконто" + Индекс]) Тогда
							ИмяРеквизита = "Субконто" + Индекс;
							ТекстСообщенияРасшифровка =
							СтрШаблон(НСтр("ru = 'Для корректного отражения последующих операций, необходимо заполнить субконто ""%1""'"),
							ДанныеСчета["ВидСубконто" + Индекс + "Наименование"]);
							ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность", 
							НСтр("ru = 'Субконто'"), ТекущаяСтрока.НомерСтроки, ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифровка);
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
							Отказ = Истина;
						КонецЕсли;
						
						Если ДанныеСчета["ВидСубконто" + Индекс] = СубконтоПартии Тогда
							СубконтоПартииСчетаУчета = "Субконто" + Индекс;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
			...
			Если ТекущаяСтрока.СчетУчета = Счет00401
				Или ТекущаяСтрока.СчетУчета = Счет00402
				И СубконтоПартииСчетаУчета <> Неопределено Тогда
					Партия = ТекущаяСтрока[СубконтоПартииСчетаУчета];
					ИмяРеквизита = СубконтоПартииСчетаУчета;
			Иначе
				Партия = Неопределено;
			КонецЕсли;
			
			Если Партия <> Неопределено Тогда
				Если Метаданные.НайтиПоТипу(ТипЗнч(Партия)).Реквизиты.Найти("Контрагент") <> Неопределено Тогда
					Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Партия, "Контрагент");
					Если Не ЗначениеЗаполнено(Контрагент) Тогда
						ТекстСообщенияРасшифровка =
						НСтр("ru = 'Для корректного отражения последующих операций, необходимо заполнить значение ""Контрагент"" в Партии'");
						ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка","Корректность",
						НСтр("ru = 'Партия'"), ТекущаяСтрока.НомерСтроки, ПредставлениеТабличнойЧасти, ТекстСообщенияРасшифровка);
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Префикс + ИмяРеквизита, , Отказ);
						Отказ = Истина;
					Иначе
						Для Каждого СтрокаСведений Из МассивСтрокСведенияПрослеживаемости Цикл
							СтрокаСведений.Комитент = Контрагент;
						КонецЦикла;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ДобавитьНепроверяемыеРеквизитыПоОперациямИзмененияНалоговогоРежима(МассивНепроверяемыхРеквизитов);
	
	// ПРОВЕРКА ОС

	Если Не РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства 
		И Не РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг Тогда
	...
	// ПРОВЕРКА АРЕНДА И ЛИЗИНГ
	
	Если Не РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг Тогда
		
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.Контрагент");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.ДоговорКонтрагента");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СпособОценкиАрендыБУ");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СтавкаДисконтирования");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаРасчетов");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаНДС");
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаПроцентов");
		МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов");
		МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов.ДатаНачисления");
		МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов.СуммаПроцентов");
		
	Иначе
		
		ЕстьДисконтирование = УчетнаяПолитика.ПоддерживаетсяДисконтированиеОбязательств();
		СтрокаРасчеты = ?(РасчетыАрендаЛизинг.Количество() > 0, РасчетыАрендаЛизинг[0], Неопределено);
		
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.Контрагент");
		Если СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.Контрагент) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Контрагент");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Контрагент", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.ДоговорКонтрагента");
		Если СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.ДоговорКонтрагента) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Договор");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "ДоговорКонтрагента", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СпособОценкиАрендыБУ");
		Если ЕстьДисконтирование
			И (СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.СпособОценкиАрендыБУ)) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Оценка в БУ");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "СпособОценкиАрендыБУ", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СтавкаДисконтирования");
		Если СтрокаРасчеты <> Неопределено 
			И ЕстьДисконтирование
			И Не ЗначениеЗаполнено(СтрокаРасчеты.СтавкаДисконтирования)
			И СтрокаРасчеты.СпособОценкиАрендыБУ = Перечисления.СпособыОценкиБУ.РассчитываетсяПоСтавке Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Ставка дисконтирования");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "СтавкаДисконтирования", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаРасчетов");
		Если СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.СчетУчетаРасчетов) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Счет расчетов");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "СчетУчетаРасчетов", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаНДС");
		Если СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.СчетУчетаНДС) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Счет НДС");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "СчетУчетаНДС", , Отказ);
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("РасчетыАрендаЛизинг.СчетУчетаПроцентов");
		Если ЕстьДисконтирование
			И СтрокаРасчеты <> Неопределено 
			И СтрокаРасчеты.СпособОценкиАрендыБУ = Перечисления.СпособыОценкиБУ.РассчитываетсяПоСтавке
			И Не ЗначениеЗаполнено(СтрокаРасчеты.СчетУчетаПроцентов) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , "Счет процентов");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "СчетУчетаПроцентов", , Отказ);
		КонецЕсли;
		Если Не ЕстьДисконтирование
			Или СтрокаРасчеты = Неопределено 
			Или Не ЗначениеЗаполнено(СтрокаРасчеты.СпособОценкиАрендыБУ)
			Или СтрокаРасчеты.СпособОценкиАрендыБУ = Перечисления.СпособыОценкиБУ.РавнаСуммеДоговора Тогда
			МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов");
			МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов.ДатаНачисления");
			МассивНепроверяемыхРеквизитов.Добавить("ГрафикПроцентныхРасходов.СуммаПроцентов");
		КонецЕсли;
	...
	Иначе
		...
		Для Каждого ТекущаяСтрока Из ЭтотОбъект[ИмяТабличнойЧасти] Цикл
			...
			// Для счета 68.21 НДФЛ ИП не требуется вводить статью затрат
			Если ПлательщикНДФЛ И ТекущаяСтрока.СчетУчета <> ПланыСчетов.Хозрасчетный.НДФЛ_ИП
				И Не ЗначениеЗаполнено(ТекущаяСтрока.СтатьяЗатрат) Тогда
				
				ТекстСообщения = СтрШаблон(НСтр("ru='В строке %1 списка %2 не заполнена статья затрат'"),
					ТекущаяСтрока.НомерСтроки, ПредставлениеТабличнойЧасти);
				Поле = СтрШаблон("%1[%2].СтатьяЗатрат", ИмяТабличнойЧасти, Формат(ТекущаяСтрока.НомерСтроки - 1, "ЧН=0; ЧГ="));
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле, "Объект", Отказ);
	...
	Если РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.НезавершенноеПроизводство
		...
		И РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации
	...
	ИначеЕсли УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, ДатаУчетнойПолитики)
		И ОтражатьПоСпециальнымРегистрам
		И НЕ ОтражатьВБухгалтерскомУчете
		И НЕ ОтражатьВНалоговомУчете Тогда
		// Остатки раздельный учет.
		МассивНепроверяемыхРеквизитов.Добавить("НоменклатураНаСкладе.ОтражениеВУСН");
		МассивНепроверяемыхРеквизитов.Добавить("НоменклатураНаСкладе.СтатусыОплатыРасходовУСН");
	КонецЕсли;
	...
	// Проверка реквизитов прослеживаемости 
	...
	Если ВедетсяУчетПрослеживаемыхТоваров И ОтражатьВБухгалтерскомУчете 
		И Документы.ВводНачальныхОстатков.УчетПрослеживаемостиВедетсяПоРазделуУчета(РазделУчета) Тогда
		
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("НоменклатураНаСкладе", Отказ);
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("МатериалыПереданныеВПереработку", Отказ);
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("ТоварыВРозничнойТорговлеАТТ", Отказ);
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("МПЗОтгруженные", Отказ);
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("ОС", Отказ);
		ПроверитьЗаполнениеРеквизитовПоПрослеживаемости("БухСправка", Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ПараметрыПроведения = Документы.ВводНачальныхОстатков.ПодготовитьПараметрыПроведения(Ссылка, ЭтотОбъект.ДополнительныеСвойства, Отказ);
	...
	Документы.ВводНачальныхОстатков.СформироватьДвижения_АрендаЛизинг(
		ПараметрыПроведения,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
	...
	Документы.ВводНачальныхОстатков.СформироватьДвижения_НДСУчтенныйВСтоимости(
		ПараметрыПроведения,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
	...
	// Прослеживаемость
	Документы.ВводНачальныхОстатков.СформироватьДвижения_Прослеживаемость(
	...
	Документы.ВводНачальныхОстатков.СформироватьДвиженияПрочиеРасчетыУСН(ПараметрыПроведения.ТаблицаРасчетыПоПлатежнымКартамУСН,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
		
	// Остаток взноса подлежащего уплате, уменьшающего налог
	Документы.ВводНачальныхОстатков.СформироватьДвижения_ОстатокВзносаДляУменьшенияНалога(ПараметрыПроведения,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СтруктураОтбора = Новый Структура("ПрослеживаемыйТовар", Истина);
	ПрослеживаемыеОС = ОС.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйОС = ПрослеживаемыеОС.Количество() <> 0;
	
	ПрослеживаемыеТовары = НоменклатураНаСкладе.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйТовар = ПрослеживаемыеТовары.Количество() <> 0;
	
	ПрослеживаемыеТоварыВРознице = ТоварыВРозничнойТорговлеАТТ.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйТоварВРознице = ПрослеживаемыеТоварыВРознице.Количество() <> 0;

	ПрослеживаемыеМатериалы = МатериалыПереданныеВПереработку.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйМатериал = ПрослеживаемыеМатериалы.Количество() <> 0;

	ПрослеживаемыеТоварыОтгруженные = МПЗОтгруженные.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйТоварОтгруженный = ПрослеживаемыеТоварыОтгруженные.Количество() <> 0;

	ПрослеживаемыеТоварыЗабаланс = БухСправка.НайтиСтроки(СтруктураОтбора);
	ЕстьПрослеживаемыйТоварЗабаланс = ПрослеживаемыеТоварыЗабаланс.Количество() <> 0;

	Если (НЕ ЕстьПрослеживаемыйОС
		И НЕ ЕстьПрослеживаемыйТовар
		И НЕ ЕстьПрослеживаемыйТоварВРознице
		И НЕ ЕстьПрослеживаемыйМатериал
		И НЕ ЕстьПрослеживаемыйТоварОтгруженный
		И НЕ ЕстьПрослеживаемыйТоварЗабаланс)
		ИЛИ НЕ ОтражатьВБухгалтерскомУчете Тогда
		СведенияПрослеживаемости.Очистить();
	КонецЕсли;

	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда 
		РегистрыСведений.ВыполнениеРегламентныхОперацийНДС.СброситьФактВыполненияРегламентнойОперации(Организация,
								Дата+1, Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС);
	КонецЕсли;
	
	ОбработатьПередЗаписьюДанныеПоНДС();
	
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг Тогда
		
		Для каждого СтрокаРасчеты Из РасчетыАрендаЛизинг Цикл
			
			Если Не СтрокаРасчеты.Лизинг Тогда
				СтрокаРасчеты.НалоговыйУчетИмуществаВедетЛизингополучатель = Ложь;
			КонецЕсли;
			Если Не УчетнаяПолитика.ПоддерживаетсяДисконтированиеОбязательств() Тогда
				СтрокаРасчеты.СпособОценкиАрендыБУ = Перечисления.СпособыОценкиБУ.РавнаСуммеДоговора;
				СтрокаРасчеты.СтавкаДисконтирования = 0;
				ГрафикПроцентныхРасходов.Очистить();
			КонецЕсли;
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства
		...
		И ПолучитьФункциональнуюОпцию("КомпонентыАмортизацииОС") Тогда
		УчетРемонтовОС.ЗаполнитьДокументыРемонтовПриВводеОстатков(ЭтотОбъект, Отказ);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСНачисленный
		ИЛИ РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСПоАвансамПолученным
		ИЛИ РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСПоПриобретеннымЦенностям
		ИЛИ РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСПерсоналомПоОплатеТрудаИДепонентам
		И Не ДополнительныеСвойства.Свойство("ПропуститьЗаписьВедомостей") Тогда
...
Процедура ДобавитьНепроверяемыеРеквизитыПоОперациямИзмененияНалоговогоРежима(НепроверяемыеРеквизиты)
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ДокументыОперацийИзмененияНалоговогоРежима.ДокументОперации = &ДокументОперации";
	
	Запрос.УстановитьПараметр("ДокументОперации", Ссылка);
	
	Если Запрос.Выполнить().Пустой() Тогда
		Возврат;
	КонецЕсли;
	
	НепроверяемыеРеквизиты.Добавить("НоменклатураНаСкладе");
	НепроверяемыеРеквизиты.Добавить("НоменклатураНаСкладе.Количество");
	НепроверяемыеРеквизиты.Добавить("НоменклатураНаСкладе.Склад");
	НепроверяемыеРеквизиты.Добавить("НоменклатураНаСкладе.Партия");
	
КонецПроцедуры

//Функция возвращает структуру с реквизитами и данными табличных частей "ДокументыОснования" и "Авансы"
//
Функция ДанныеСчетаФактуры(СчетФактура)
	
	СписокРеквизитов = "СформированПриВводеНачальныхОстатковНДС, 
	|СуммаДокумента, 
	|СуммаНДСДокумента, 
	|СуммаУвеличение, 
	|СуммаНДСУвеличение, 
	...
	Результат.Вставить("Реквизиты", Новый Структура(СписокРеквизитов));
	ЗаполнитьЗначенияСвойств(Результат.Реквизиты, СчетФактура);
	Результат.Вставить("ДокументыОснования", СчетФактура.ДокументыОснования.Выгрузить());
	Результат.Вставить("Авансы", СчетФактура.Авансы.Выгрузить());
