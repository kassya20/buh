...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьРаботниковПоСпискуСотрудников");
	Методы.Вставить("ЗаполнитьКодВидаОперацииВДанныеПоНДСОтложенно");
	Методы.Вставить("ЗаполнитьКодВидаОперацииИСуммыВЖурналеУчетаСФОтложенно");
	Методы.Вставить("ЗаполнитьПервоначальнуюСтоимостьНУПоСпецодежде");
...
Функция СчетаКапВложенийДляУчетаПоРУ() Экспорт
	
	ПланСчетов = ПланыСчетов.Хозрасчетный;
	// Массив счетов, по которым ведется только суммовой учет и по которым необходимо создавать запись в регистр НДС Раздельный учет	
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, ДополнительныеСвойства, Отказ) Экспорт
	...
	Если НЕ ДополнительныеСвойства.Свойство("РегистрацияНачальныхДанныхНулевойОтчетности") Тогда
	...
	ОтражатьГТДВместеСОстатками = ПолучитьФункциональнуюОпцию("ВедетсяУчетИмпортныхТоваров")
		И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Реквизиты.Период);
	Реквизиты.Вставить("ВедетсяУчетПрослеживаемыхТоваров", ВедетсяУчетПрослеживаемыхТоваров);
	Реквизиты.Вставить("ОтражатьГТДВместеСОстатками",	   ОтражатьГТДВместеСОстатками);
	...
	МассивСчетов = Новый Массив;
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам);
	СчетаРасчетовПоПлатежнымКартам = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(МассивСчетов);
	...
	Запрос.УстановитьПараметр("СчетаПлатежныеКарты", СчетаРасчетовПоПлатежнымКартам);
	...
	Запрос.УстановитьПараметр("РаздельныйУчетНДСБезВычетов", УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период + 86400));
	...
	Запрос.УстановитьПараметр("МассивСчетовКапВложенийДляРУ", СчетаКапВложенийДляУчетаПоРУ());
	Запрос.УстановитьПараметр("Счет4111",					  ПланыСчетов.Хозрасчетный.ТоварыВРозничнойТорговлеВПродажныхЦенахАТТ);
	Запрос.УстановитьПараметр("СокращеннаяАналитика",
		УчетНДСРаздельный.СокращеннаяАналитика(Реквизиты.Период + 86400));
	Запрос.Текст =
		ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
		+ ТекстЗапросаОС(НомераТаблиц, Реквизиты)
		+ ТекстЗапросаАрендаЛизинг(НомераТаблиц, Реквизиты)
		...
		+ ТекстЗапросаТопливо(НомераТаблиц, Реквизиты)
		+ ТекстЗапросаПрослеживаемость(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		+ ТекстЗапросаПрослеживаемостьОС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		+ ТекстЗапросаГТД(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		+ ТекстЗапросаРасчетыПоПлатежнымКартамУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		+ ТекстЗапросаНДСУчтенныйВСтоимости(НомераТаблиц, ПараметрыПроведения, Реквизиты);
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	&РаздельныйУчетНДСБезВычетов КАК РаздельныйУчетНДСБезВычетов
	|ИЗ
	|	Документ.ВводНачальныхОстатков КАК Реквизиты
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка";
...
Функция ТекстЗапросаСчетаИСубконто(НомераТаблиц, Реквизиты)
	...
	ТекстЗапроса = "
		...
		|ГДЕ
		|	Хозрасчетный.Ссылка В(&МассивСчетовКапВложенийДляРУ)		
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	СчетУчета
		|";
...
Функция ТекстЗапросаОС(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства 
		И Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг Тогда
		Возврат "";
	КонецЕсли;
	...
	НомераТаблиц.Вставить("ТаблицаЗатратыНаРемонтОС", НомераТаблиц.Количество());
	...
	|	Док.Обесценение                         			  КАК Обесценение,
	...
	|	ВЫБОР 
	|		КОГДА &РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.АрендаЛизинг) 
	|			ТОГДА 0
	|		ИНАЧЕ Док.КорректировкаСтоимостиНУ 
	...
	|	Док.Ремонт КАК Ремонт,
	|	Док.ТекущаяСтоимостьБУ КАК ТекущаяСтоимостьБУ,
	|	Док.НакопленнаяАмортизацияБУ КАК НакопленнаяАмортизацияБУ,
	|	Док.ОстатокСрокаАмортизацииБУ КАК ОстатокСрокаАмортизацииБУ
	|ИЗ
	|	Документ.ВводНачальныхОстатков.ЗатратыНаРемонтОС КАК Док
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.ОС КАК ОС
	|		ПО (&РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.ОсновныеСредства))
	|			И Док.Ссылка = ОС.Ссылка
	|			И Док.Ключ = ОС.Ключ
	|ГДЕ
	|	Док.Ссылка = &Ссылка
	|	И ОС.Ссылка = &Ссылка
...
Функция ТекстЗапросаАрендаЛизинг(НомераТаблиц, Реквизиты)
	
	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг Тогда
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВременнаяТаблицаКурсАрендаЛизинг", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ТаблицаРасчетыАрендаЛизинг", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ТаблицаОСАрендаЛизинг", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ТаблицаПроцентыАрендаЛизинг", НомераТаблиц.Количество());
	
	ТекстЗапроса =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Расчеты.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК Валюта,
	|	ВЫБОР
	|		КОГДА Расчеты.КурсВзаиморасчетов = 0
	|			ТОГДА 1
	|		ИНАЧЕ Расчеты.КурсВзаиморасчетов
	|	КОНЕЦ КАК Курс,
	|	ВЫБОР
	|		КОГДА Расчеты.КратностьВзаиморасчетов = 0
	|			ТОГДА 1
	|		ИНАЧЕ Расчеты.КратностьВзаиморасчетов
	|	КОНЕЦ КАК Кратность
	|ПОМЕСТИТЬ КурсАрендаЛизинг
	|ИЗ
	|	Документ.ВводНачальныхОстатков.РасчетыАрендаЛизинг КАК Расчеты
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	Расчеты.Ссылка КАК Регистратор,
	|	&Период КАК Период,
	|	Расчеты.Контрагент КАК Контрагент,
	|	Расчеты.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	Расчеты.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК Валюта,
	|	Расчеты.СчетУчетаРасчетов КАК СчетУчетаРасчетов,
	|	Расчеты.СчетУчетаНДС КАК СчетУчетаНДС,
	|	Расчеты.СчетУчетаПроцентов КАК СчетУчетаПроцентов,
	|	Расчеты.СпособОценкиАрендыБУ КАК СпособОценкиАрендыБУ,
	|	Расчеты.СтавкаДисконтирования КАК СтавкаДисконтирования,
	|	Расчеты.Лизинг КАК Лизинг,
	|	Расчеты.НалоговыйУчетИмуществаВедетЛизингополучатель КАК НалоговыйУчетИмуществаВедетЛизингополучатель
	|ИЗ
	|	Документ.ВводНачальныхОстатков.РасчетыАрендаЛизинг КАК Расчеты
	|ГДЕ
	|	Расчеты.Ссылка = &Ссылка
	...
	|	ОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ОС.СуммаАрендныхПлатежей КАК СуммаАрендныхПлатежей,
	|	ОС.СуммаНДСАрендныхПлатежей КАК СуммаНДСАрендныхПлатежей,
	|	ВЫРАЗИТЬ(ОС.СуммаАрендныхПлатежей * КурсВалюты.Курс / КурсВалюты.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаАрендныхПлатежейРуб,
	|	ВЫРАЗИТЬ(ОС.СуммаНДСАрендныхПлатежей * КурсВалюты.Курс / КурсВалюты.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаНДСАрендныхПлатежейРуб,
	|	(ВЫРАЗИТЬ(ОС.СуммаАрендныхПлатежей * КурсВалюты.Курс / КурсВалюты.Кратность КАК ЧИСЛО(15, 2))) - (ВЫРАЗИТЬ(ОС.СуммаНДСАрендныхПлатежей * КурсВалюты.Курс / КурсВалюты.Кратность КАК ЧИСЛО(15, 2))) - (ОС.ТекущаяСтоимостьНУ - ОС.НакопленнаяАмортизацияНУ) КАК КорректировкаСтоимостиНУ
	|ИЗ
	|	Документ.ВводНачальныхОстатков.ОС КАК ОС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КурсАрендаЛизинг КАК КурсВалюты
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ОС.Ссылка = &Ссылка
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""ГрафикПроцентныхРасходов"" КАК ИмяСписка,
	|	Проценты.Ссылка КАК Регистратор,
	|	&Период КАК Период,
	|	Проценты.НомерСтроки КАК НомерСтроки,
	|	Проценты.ДатаНачисления КАК ДатаНачисления,
	|	Проценты.СуммаПроцентов КАК СуммаПроцентов,
	|	ВЫРАЗИТЬ(Проценты.СуммаПроцентов * КурсВалюты.Курс / КурсВалюты.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаПроцентовРуб
	|ИЗ
	|	Документ.ВводНачальныхОстатков.ГрафикПроцентныхРасходов КАК Проценты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КурсАрендаЛизинг КАК КурсВалюты
	|		ПО (ИСТИНА)
	|ГДЕ
...
Функция ТекстЗапросаНМА(НомераТаблиц, Реквизиты)
	...
	|	Док.ОбесценениеДоступноеДляВосстановления,
...
Функция ТекстЗапросаРасчетыПоНалогамИСборам(НомераТаблиц, Реквизиты)
	...
	|	Док.Сумма КАК Сумма,
	|	Док.СуммаКт КАК СуммаКт,
	|	Док.СуммаВзносаДляУменьшенияНалога КАК СуммаВзносаДляУменьшенияНалога,
...
Функция ТекстЗапросаБухСправка(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.ПрочиеСчетаБухгалтерскогоУчета
		...
		И Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации
...
Функция ТекстЗапросаТоварыОтгруженныеНДС(НомераТаблиц, Реквизиты)
	...
	ТекстЗапроса = "
		...
		|	ДокПартии.НетПартииНаОстатках            КАК НетПартииНаОстатках,
		...
		|	ВЫБОР
		|		КОГДА НЕ &СокращеннаяАналитика ТОГДА
		|			ДокСФ.Контрагент
...
Функция ТекстЗапросаТоварыВРозничнойТорговлеАТТНДС(НомераТаблиц, Реквизиты)
	...
	ТекстЗапроса = "
		|/////////////////////////////////////////////////////////////////////////////////////
		|// Строки со счетом учета 41.11, другие счета не должны попадать
		|ВЫБРАТЬ
...
Функция ТекстЗапросаНДС(НомераТаблиц, Реквизиты)
	...
	|	Док.СчетУчета КАК СчетУчета,
	|	Док.СтавкаНДС КАК СтавкаНДС,
	|	Док.ВидЦенности КАК ВидЦенности,
	|	Док.Контрагент КАК Контрагент,
	|	Док.ДоговорКонтрагента КАК ДоговорКонтрагента,
	...
	|	Док.ПредъявленСФ КАК ПредъявленСФ,
	|	Док.СуммаБезНДС КАК СуммаБезНДС,
	|	Док.НДС КАК НДС,
	|	Док.СуммаБезНДСОплата КАК СуммаБезНДСОплата,
	|	Док.НДСОплата КАК НДСОплата,
	|	Док.СуммаБезНДСВключеноВСтоимость КАК СуммаБезНДСВключеноВСтоимость,
	|	Док.НДСВключеноВСтоимость КАК НДСВключеноВСтоимость,
	|	Док.СуммаБезНДССтавка0 КАК СуммаБезНДССтавка0,
	|	Док.НДССтавка0 КАК НДССтавка0,
	|	Док.СуммаБезНДСПредъявлено КАК СуммаБезНДСПредъявлено,
	|	Док.НДСПредъявлено КАК НДСПредъявлено,
	|	Док.СуммаБезНДСПредъявлено0 КАК СуммаБезНДСПредъявлено0,
	|	Док.НДСПредъявлено0 КАК НДСПредъявлено0,
	|	Док.ВалютаАванса КАК ВалютаАванса,
	|	Док.ВалютнаяСуммаСНДС КАК ВалютнаяСуммаСНДС,
	|	Док.СуммаБезНДСПодтверждено0 КАК СуммаБезНДСПодтверждено0,
	|	Док.НДСПодтверждено0 КАК НДСПодтверждено0,
	|	Док.СуммаБезНДСНеПодтверждено0 КАК СуммаБезНДСНеПодтверждено0,
	|	Док.НДСНеПодтверждено0 КАК НДСНеПодтверждено0,
	|	Док.СуммаБезНДСПредъявленоНеПодтверждено0 КАК СуммаБезНДСПредъявленоНеПодтверждено0,
	|	Док.НДСПредъявленоНеПодтверждено0 КАК НДСПредъявленоНеПодтверждено0,
	|	Док.ДокументОтгрузки КАК ДокументОтгрузки,
	|	Док.ДокументОплаты КАК ДокументОплаты,
	|	ВЫБОР
	...
	|	Док.СтавкаНДСНеПодтверждена0 КАК СтавкаНДСНеПодтверждена0,
	...
	|	ТаблицаДанныеПоНДС.Контрагент КАК Контрагент,
	|	ТаблицаДанныеПоНДС.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ТаблицаДанныеПоНДС.СчетФактура КАК СчетФактура,
	|	ТаблицаДанныеПоНДС.СчетФактураДата КАК СчетФактураДата,
	|	ТаблицаДанныеПоНДС.ВидЦенности КАК ВидЦенности,
	...
	|	ТаблицаДанныеПоНДС.ДокументОтгрузки КАК ДокументОтгрузки,
	...
	|	ТаблицаДанныеПоНДС.СтавкаНДС КАК СтавкаНДС,
	|	ТаблицаДанныеПоНДС.СчетУчета КАК СчетУчетаНДС,
	|	&Период КАК ДатаСобытия,
	|	ТаблицаДанныеПоНДС.ДокументОплаты КАК ДокументОплаты,
	|	ТаблицаДанныеПоНДС.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДанныеПоНДС.Регистратор КАК Регистратор,
	|	&Период КАК Период,
	|	ТаблицаДанныеПоНДС.ДатаОплаты КАК ДатаОплаты,
	|	ТаблицаДанныеПоНДС.Исправление КАК Исправление,
	|	ТаблицаДанныеПоНДС.НомерИсправления КАК НомерИсправления,
	|	ТаблицаДанныеПоНДС.ДатаИсправления КАК ДатаИсправления,
	|	ТаблицаДанныеПоНДС.КорректировочныйСчетФактура КАК КорректировочныйСчетФактура,
	|	ТаблицаДанныеПоНДС.НомерИсходногоДокумента КАК НомерИсходногоДокумента,
	|	ТаблицаДанныеПоНДС.ДатаИсходногоДокумента КАК ДатаИсходногоДокумента,
	|	ТаблицаДанныеПоНДС.УчитыватьИсправлениеИсходногоДокумента КАК УчитыватьИсправлениеИсходногоДокумента,
	|	ТаблицаДанныеПоНДС.НомерИсправленияИсходногоДокумента КАК НомерИсправленияИсходногоДокумента,
	|	ТаблицаДанныеПоНДС.ДатаИсправленияИсходногоДокумента КАК ДатаИсправленияИсходногоДокумента
	|ИЗ
	...
	|	СУММА(ТаблицаДанныеПоНДС.НДСНеПодтверждено0) КАК НДСНеПодтверждено0,
	|	&Период КАК ДатаСобытия,
	|	ТаблицаДанныеПоНДС.СтавкаНДСНеПодтверждена0 КАК СтавкаНДСНеПодтверждена0,
	|	ТаблицаДанныеПоНДС.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДанныеПоНДС.Регистратор КАК Регистратор,
	|	&Период КАК Период,
	...
	|	СУММА(ТаблицаДанныеПоНДС.НДСВключеноВСтоимость) КАК НДСВключеноВСтоимость,
	|	СУММА(ТаблицаДанныеПоНДС.СуммаБезНДСВключеноВСтоимость) КАК СуммаБезНДСВключеноВСтоимость,
	|	ТаблицаДанныеПоНДС.ВалютаАванса КАК ВалютаАванса,
...
Функция ТекстЗапросаРасчетыСПерсоналом(НомераТаблиц, Реквизиты)
	...
	|	Док.НДФЛСПревышения КАК НДФЛСПревышения
	|ИЗ
...
Функция ТекстЗапросаТоварыОтгруженныеУСН(НомераТаблиц, Реквизиты)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА Док.СтатусыОплатыРасходовУСН = ЗНАЧЕНИЕ(Перечисление.СтатусыРасходовУСН.НеОплачено)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.СтатусыПартийУСН.Списанные)
	|					ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтатусыПартийУСН.НаРеализации)
	|				КОНЕЦ
	|		ИНАЧЕ Док.СтатусыПартийУСН
	|	КОНЕЦ КАК СтатусыПартийУСН,
	|	Док.РасчетныйДокумент КАК РасчетныйДокумент,
	|	Док.СтатусыОплатыРасходовУСН КАК СтатусыОплатыРасходовУСН,
	|	Док.ОтражениеВУСН КАК ОтражениеВУСН,
	|	Док.Валюта КАК Валюта,
	|	Док.Количество КАК Количество,
	|	ВЫБОР
...
Функция ТекстЗапросаТопливо(НомераТаблиц, Реквизиты)
	...
	|	0 КАК СуммаВР
	|ИЗ
	|	Документ.ВводНачальныхОстатков.Топливо КАК Док
	|ГДЕ
	|	Док.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаРасчетыПоПлатежнымКартамУСН(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не УчетнаяПолитика.ПрименяетсяУСН(Реквизиты.Организация, Реквизиты.Период + 86400) Тогда
		ПараметрыПроведения.Вставить("ТаблицаРасчетыПоПлатежнымКартамУСН", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ТаблицаРасчетыПоПлатежнымКартамУСН", НомераТаблиц.Количество());
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""БухСправка"" КАК ИмяСписка,
	|	ВводНачальныхОстатковБухСправка.Ссылка КАК Регистратор,
	|	&Период КАК Период,
	|	ВводНачальныхОстатковБухСправка.НомерСтроки КАК НомерСтроки,
	|	ВводНачальныхОстатковБухСправка.СчетУчета КАК СчетУчета,
	|	ВводНачальныхОстатковБухСправка.Субконто1 КАК Контрагент,
	|	ВводНачальныхОстатковБухСправка.Субконто2 КАК ДоговорКонтрагента,
	|	ВводНачальныхОстатковБухСправка.Ссылка КАК РасчетныйДокумент,
	|	ВводНачальныхОстатковБухСправка.Сумма КАК Сумма,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения
	|ИЗ
	|	Документ.ВводНачальныхОстатков.БухСправка КАК ВводНачальныхОстатковБухСправка
	|ГДЕ
	|	ВводНачальныхОстатковБухСправка.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковБухСправка.СчетУчета В (&СчетаПлатежныеКарты)
...
Функция ПодготовитьПараметрыПроведенияНоменклатуры_Материалы(
	...
	// Подготовка таблицы Параметры.ТаблицаТопливо
	...
	ОбязательныеКолонки.Добавить("Сумма");                // Число (15,2)
	ОбязательныеКолонки.Добавить("СуммаНУ");              // Число (15,2)
	ОбязательныеКолонки.Добавить("СуммаПР");              // Число (15,2)
	ОбязательныеКолонки.Добавить("СуммаВР");              // Число (15,2)
	
	СписокОбязательныхКолонок = СтрСоединить(ОбязательныеКолонки, ",");
	
	Параметры.Вставить("ТаблицаТопливо", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		ТаблицаТопливо, СписокОбязательныхКолонок));
...
Процедура СформироватьДвижения_Номенклатура(
	...
	Если Реквизиты.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы Тогда
		...
		Если (ТаблицаСпецодеждаИСпецоснасткаВЭксплуатации.Количество() = 0)
			...
			И (ТаблицаНоменклатураНаСкладе.Количество() = 0)
			И Не ЗначениеЗаполнено(ПараметрыПроведения.ТаблицаТопливо) Тогда
			Возврат;
		КонецЕсли;
		...
		Если Реквизиты.ОтражатьВБухгалтерскомУчете ИЛИ Реквизиты.ОтражатьВНалоговомУчете Тогда
			...
			СформироватьДвижения_Номенклатура_Топливо(Реквизиты, Параметры.ТаблицаТопливо, Движения);
		КонецЕсли;
...
Процедура СформироватьДвижения_Номенклатура_ПроводкиОбщие(Реквизиты, ТаблицаНоменклатуры, Движения)
	...
	Для Каждого ТекущаяСтрока Из ТаблицаНоменклатуры Цикл
		...
		Если НЕ Проводка = Неопределено Тогда
			...
			Если СчетУчета = ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов Тогда
				
				ВыбытиеМалоценногоОборудованияИЗапасов(ТекущаяСтрока, Движения.Хозрасчетный, Реквизиты, ОценкаМПЗПоСредней);
...
Процедура ВыбытиеМалоценногоОборудованияИЗапасов(Знач ТекущаяСтрока, Движения, Реквизиты, ОценкаМПЗПоСредней);
	
	ПлательщикНалогаНаПрибыль	= УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Реквизиты.Организация, Реквизиты.Период + 86400);
	ПоддержкаПБУ18				= УчетнаяПолитика.ПоддержкаПБУ18(Реквизиты.Организация, Реквизиты.Период + 86400);
	
	ТекущаяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов;
	Проводка = ПолучитьЗаполненнуюПроводкуБезСубконто(Реквизиты, Движения, ТекущаяСтрока, Истина);
	...
	Если НЕ Проводка = Неопределено Тогда
		...
		Если ПлательщикНалогаНаПрибыль Тогда
			Если ПоддержкаПБУ18 Тогда
				Проводка.СуммаВРКт = ТекущаяСтрока.СуммаНУ - ТекущаяСтрока.СуммаПР;
			КонецЕсли;
...
Процедура СформироватьДвижения_Номенклатура_Топливо(Реквизиты, ТаблицаТопливо, Движения)

	Для Каждого ТекущаяСтрока Из ТаблицаТопливо Цикл
...
Функция ПодготовитьПараметрыПроведенияОС(ТаблицаОС, ТаблицаЗатратыНаРемонтОС, ТаблицаРеквизиты)
	...
	+ "Обесценение,"                                  // <Число(15, 2, Неотрицательный)>
	...
	// Подготовка таблицы Параметры.ТаблицаЗатратыНаРемонтОС
	...
	+ "НомерСтроки,"                                  // <Число>
	+ "ОсновноеСредство,"                             // <СправочникСсылка.ОсновныеСредства>
	+ "ВидРемонта,"                                   // <СправочникСсылка.ВидыРемонтовОС>
	+ "Ремонт,"                                       // <ОпределяемыйТип.ДокументыРемонтаОС>
	+ "ТекущаяСтоимостьБУ,"                           // <Число(15, 2, Неотрицательный)>
	+ "НакопленнаяАмортизацияБУ,"                     // <Число(15, 2)>
	+ "ОстатокСрокаАмортизацииБУ";                    // <Число(5, 0, Неотрицательный)>

	Параметры.Вставить("ТаблицаЗатратыНаРемонтОС",
		ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(ТаблицаЗатратыНаРемонтОС, СписокОбязательныхКолонок));
...
Процедура СформироватьДвижения_ОС(
	...
	ТаблицаЗатратыНаРемонтОС = ПараметрыПроведения.ТаблицаЗатратыНаРемонтОС;

	Если ТаблицаОС.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Параметры = ПодготовитьПараметрыПроведенияОС(ТаблицаОС, ТаблицаЗатратыНаРемонтОС, ТаблицаРеквизиты);
	...
	Если Реквизиты.ОтражатьВБухгалтерскомУчете ИЛИ Реквизиты.ОтражатьВНалоговомУчете Тогда
		...
		Если Реквизиты.ОтражатьВБухгалтерскомУчете Тогда
			УчетРемонтовОС.СформироватьНачальныеОстатки(
				Параметры.ТаблицаЗатратыНаРемонтОС, СодержаниеПроводки, Движения, Отказ);
		КонецЕсли;
...
Процедура СформироватьДвижения_ОС_Проводки(ТаблицаОС, СодержаниеПроводки, Движения, Отказ)
	...
	Для Каждого СтрокаОС Из ТаблицаОС Цикл
		...
		// 3. Корректировка стоимости НУ (по разделу ОсновныеСредства)
		...
		// 4. Обесценение ОС
		Если СтрокаОС.Обесценение <> 0 Тогда
			...
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ОбесценениеОС_01;
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ОсновныеСредства", СтрокаОС.ОсновноеСредство);
...
// ДВИЖЕНИЯ ПО АРЕНДЕ И ЛИЗИНГУ

Процедура СформироватьДвижения_АрендаЛизинг(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	НачалоПримененияФСБУ25 = УчетнаяПолитика.НачалоПримененияФСБУ25(Реквизиты.Организация);
	Если Реквизиты.Период < НачалоПримененияФСБУ25 Тогда
		ТекстОшибки = НСтр("ru='Ввод остатков по разделу Аренда и лизинг применяется только после перехода на ФСБУ 25.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Реквизиты.Регистратор, "Дата", , Отказ);
		Возврат;
	КонецЕсли;
	
	ТаблицаРасчеты = ПараметрыПроведения.ТаблицаРасчетыАрендаЛизинг;
	ТаблицаОС = ПараметрыПроведения.ТаблицаОСАрендаЛизинг;
	ТаблицаПроценты = ПараметрыПроведения.ТаблицаПроцентыАрендаЛизинг;
	
	Если ТаблицаРасчеты.Количество() = 0 
		Или ТаблицаОС.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если Реквизиты.ОтражатьПоСпециальнымРегистрам Тогда
		СформироватьДвижения_АрендаЛизинг_ДанныеПредметовАренды(ТаблицаОС, ТаблицаРасчеты, Реквизиты, Движения);
		СформироватьДвижения_АрендаЛизинг_ГрафикиПроцентныхРасходов(ТаблицаПроценты, ТаблицаРасчеты, ТаблицаОС, Реквизиты, Движения);
	КонецЕсли;
	
	Если Реквизиты.ОтражатьВБухгалтерскомУчете Тогда
		СодержаниеПроводки = ПолучитьСодержаниеПроводки(Реквизиты.РазделУчета);
		СформироватьДвижения_АрендаЛизинг_Проводки_Расчеты(
			ТаблицаРасчеты, ТаблицаОС, ТаблицаПроценты, СодержаниеПроводки, Реквизиты, Движения);
	КонецЕсли;
	
	Если Реквизиты.ОтражатьВНалоговомУчете Тогда
		СодержаниеПроводки = ПолучитьСодержаниеПроводки(Реквизиты.РазделУчета);
		СформироватьДвижения_АрендаЛизинг_Проводки_КорректировкаСтоимостиНУ(
			ТаблицаОС, СодержаниеПроводки, Реквизиты, Движения);
	КонецЕсли;
	
КонецПроцедуры

Процедура СформироватьДвижения_АрендаЛизинг_ДанныеПредметовАренды(ТаблицаОС, ТаблицаРасчеты, Реквизиты, Движения)
	
	СтрокаРасчеты = ТаблицаРасчеты[0];
	
	Для каждого СтрокаОС Из ТаблицаОС Цикл
		
		Запись = Движения.ДанныеПредметовАренды.Добавить();
		Запись.Период = Реквизиты.Период;
		Запись.Организация = Реквизиты.Организация;
		Запись.Контрагент = СтрокаРасчеты.Контрагент;
		Запись.ДоговорКонтрагента = СтрокаРасчеты.ДоговорКонтрагента;
		Запись.ПредметАренды = СтрокаОС.ОсновноеСредство;
		
		Запись.СуммаВзаиморасчетов = СтрокаОС.СуммаАрендныхПлатежей;
		Запись.СуммаРуб = СтрокаОС.СуммаАрендныхПлатежейРуб;
		Запись.СуммаНДСВзаиморасчетов = СтрокаОС.СуммаНДСАрендныхПлатежей;
		Запись.Лизинг = СтрокаРасчеты.Лизинг;
		Запись.НалоговыйУчетИмуществаВедетЛизингополучатель = СтрокаРасчеты.НалоговыйУчетИмуществаВедетЛизингополучатель;
		Запись.СтавкаДисконтирования = СтрокаРасчеты.СтавкаДисконтирования;
...
Процедура СформироватьДвижения_АрендаЛизинг_ГрафикиПроцентныхРасходов(ТаблицаПроценты, ТаблицаРасчеты, ТаблицаОС, Реквизиты, Движения)
	
	Если Не ЗначениеЗаполнено(ТаблицаПроценты) Тогда
		Возврат;
	КонецЕсли;
	
	ОсновныеСредства = ТаблицаОС.ВыгрузитьКолонку("ОсновноеСредство");
	СуммыПлатежей = ТаблицаОС.ВыгрузитьКолонку("СуммаАрендныхПлатежей");
	СтрокаРасчеты = ТаблицаРасчеты[0];
	
	Для каждого СтрокаПроценты Из ТаблицаПроценты Цикл
		
		СуммыПроцентовОС = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(СтрокаПроценты.СуммаПроцентов, СуммыПлатежей);
		Если СуммыПроцентовОС = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Для Индекс = 0 По СуммыПроцентовОС.ВГраница() Цикл
			
			Запись = Движения.ГрафикиПроцентныхРасходов.Добавить();
			
			Запись.Период = Реквизиты.Период;
			Запись.Организация = Реквизиты.Организация;
			Запись.Контрагент = СтрокаРасчеты.Контрагент;
			Запись.ДоговорКонтрагента = СтрокаРасчеты.ДоговорКонтрагента;
			Запись.ПредметНачисления = ОсновныеСредства[Индекс];
			Запись.ДатаНачисления = СтрокаПроценты.ДатаНачисления;
			Запись.ПериодНачисления = КонецМесяца(СтрокаПроценты.ДатаНачисления);
			Запись.СуммаПроцентов = СуммыПроцентовОС[Индекс];
			Запись.СтавкаДисконтирования = СтрокаРасчеты.СтавкаДисконтирования;
...
Процедура СформироватьДвижения_АрендаЛизинг_Проводки_Расчеты(ТаблицаРасчеты, ТаблицаОС, ТаблицаПроценты, СодержаниеПроводки, Реквизиты, Движения)
	
	СуммаАрендныхПлатежейВал = ТаблицаОС.Итог("СуммаАрендныхПлатежей");
	СуммаНДСАрендныхПлатежейВал = ТаблицаОС.Итог("СуммаНДСАрендныхПлатежей");
	СуммаПроцентовВал = ТаблицаПроценты.Итог("СуммаПроцентов");
	СуммаАрендныхПлатежейРуб = ТаблицаОС.Итог("СуммаАрендныхПлатежейРуб");
	СуммаНДСАрендныхПлатежейРуб = ТаблицаОС.Итог("СуммаНДСАрендныхПлатежейРуб");
	СуммаПроцентовРуб = ТаблицаПроценты.Итог("СуммаПроцентовРуб");
	...
	Проводка.СчетКт = СтрокаРасчеты.СчетУчетаРасчетов;
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты", СтрокаРасчеты.Контрагент);
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Договоры", СтрокаРасчеты.ДоговорКонтрагента);
	БухгалтерскийУчет.УстановитьПодразделениеПроводки(Проводка, Реквизиты.ПодразделениеОрганизации, "Кт");
	
	Проводка.Сумма = СуммаАрендныхПлатежейРуб;
	СвойстваСчетаКт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаРасчеты.СчетУчетаРасчетов);
	Если СвойстваСчетаКт.Валютный Тогда
		Проводка.ВалютаКт = СтрокаРасчеты.Валюта;
		Проводка.ВалютнаяСуммаКт = СуммаАрендныхПлатежейВал;
	КонецЕсли;
	
	// НДС
	
	Если СуммаНДСАрендныхПлатежейВал <> 0 Тогда
		...
		Проводка.СчетДт = СтрокаРасчеты.СчетУчетаНДС;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты", СтрокаРасчеты.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Договоры", СтрокаРасчеты.ДоговорКонтрагента);
		БухгалтерскийУчет.УстановитьПодразделениеПроводки(Проводка, Реквизиты.ПодразделениеОрганизации, "Дт");
		
		Проводка.Сумма = СуммаНДСАрендныхПлатежейРуб;
		СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаРасчеты.СчетУчетаНДС);
		Если СвойстваСчетаДт.Валютный Тогда
			Проводка.ВалютаДт = СтрокаРасчеты.Валюта;
			Проводка.ВалютнаяСуммаДт = СуммаНДСАрендныхПлатежейВал;
		КонецЕсли;
	...
	Если СуммаПроцентовВал <> 0 Тогда
		...
		Проводка.СчетДт = СтрокаРасчеты.СчетУчетаПроцентов;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты", СтрокаРасчеты.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Договоры", СтрокаРасчеты.ДоговорКонтрагента);
		БухгалтерскийУчет.УстановитьПодразделениеПроводки(Проводка, Реквизиты.ПодразделениеОрганизации, "Дт");
		
		Проводка.Сумма = СуммаПроцентовРуб;
		СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаРасчеты.СчетУчетаПроцентов);
		Если СвойстваСчетаДт.Валютный Тогда
			Проводка.ВалютаДт = СтрокаРасчеты.Валюта;
...
Процедура СформироватьДвижения_АрендаЛизинг_Проводки_КорректировкаСтоимостиНУ(ТаблицаОС, СодержаниеПроводки, Реквизиты, Движения)
...
Процедура СформироватьДвижения_НМА_Проводки(ТаблицаНМА, Реквизиты, СодержаниеПроводки, Движения, Отказ)
	...
	Для Каждого СтрокаНМА Из ТаблицаНМА Цикл
		...
		Если СтрокаНМА.ЭтоНМА Тогда
			...
			УчетНМА.ДобавитьПроводкуДоступноеОбесценение(
				НаборЗаписейДвижения,
				СтрокаНМА,
				СтрокаНМА.НематериальныйАктив,
				Реквизиты.ПодразделениеОрганизации,
				СодержаниеПроводки,
				ПланыСчетов.Хозрасчетный.Вспомогательный,
				СтрокаНМА.ОбесценениеДоступноеДляВосстановления);
...
Процедура СформироватьДвижения_БухСправка(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Реквизиты.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ДенежныеСредства
		...
		ИЛИ Реквизиты.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации
...
Функция ПодготовитьПараметрыПроведения_НДС_Партиии(
	...
	+ "РаздельныйУчетНДСБезВычетов";    // <Булево>
	...
	Если Реквизиты.РаздельныйУчетНДСНаСчете19
		ИЛИ Реквизиты.РаздельныйУчетНДСБезВычетов Тогда
...
Процедура СформироватьДвиженияПрочиеРасчетыУСН(ТаблицаПрочиеРасчеты, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.ДенежныеСредства
		И Не УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Реквизиты.Организация, Реквизиты.Период + 86400) Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДвижения_РасчетыПоНалогамИСборамИП(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	// Задолженность
	СчетаНДФЛ = Новый Массив;
	СчетаНДФЛ.Добавить(ПланыСчетов.Хозрасчетный.НДФЛ);
	СчетаНДФЛ.Добавить(ПланыСчетов.Хозрасчетный.НДФЛ_ИП);
	
	Запрос.УстановитьПараметр("СчетаНДФЛ", СчетаНДФЛ);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТаблицаРасчетыПоНалогамИСборам.СчетУчета,
	...
	|	И НЕ ТаблицаРасчетыПоНалогамИСборам.СчетУчета В (&СчетаНДФЛ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура СформироватьДвижения_РасчетыПоЗаработнойПлатеИП(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	|	ТаблицаРасчетыПоЗаработнойПлате.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	|	ТаблицаРасчетыПоЗаработнойПлате.ХарактерДеятельности КАК ХарактерДеятельности,
	|	ТаблицаРасчетыПоЗаработнойПлате.СтатьяЗатрат КАК СтатьяЗатрат,
	|	ТаблицаРасчетыПоЗаработнойПлате.Регистратор КАК Регистратор,
	...
	|	КОНЕЦ КАК СчетУчета,
	|	ТаблицаРасчетыПоЗаработнойПлате.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ Удержано
	|ИЗ
	|	&ТаблицаРасчетыПоЗаработнойПлате КАК ТаблицаРасчетыПоЗаработнойПлате
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Удержано.ФизЛицо КАК ФизЛицо,
	|	Удержано.Сумма КАК Сумма,
	|	Удержано.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	|	Удержано.ХарактерДеятельности КАК ХарактерДеятельности,
	|	Удержано.СтатьяЗатрат КАК СтатьяЗатрат,
	|	Удержано.Регистратор КАК Регистратор,
	|	Удержано.ВидНачисления КАК ВидНачисления,
	|	Удержано.Контрагент КАК Контрагент,
	|	Удержано.СчетУчета КАК СчетУчета
	|ИЗ
...
Функция ПодготовитьТаблицуТоваровИП(ТаблицаТМЦ, УчетнаяПолитикаНУ, Регистратор)
	...
	ЕстьКолонкаПартия = (ТаблицаТМЦ.Колонки.Найти("Партия") <> Неопределено);
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаТМЦ Цикл
		...
		Если СтрокаТаблицы.Оплачен Тогда
			Если ЕстьКолонкаПартия
				И ЗначениеЗаполнено(СтрокаТаблицы.Партия)
				И ТипЗнч(СтрокаТаблицы.Партия) = Тип("ДокументСсылка.РасходыПредпринимателя") Тогда
				СтрокаТаблицы.ДокументОплаты = СтрокаТаблицы.Партия;
			Иначе
...
Функция ПодготовитьТаблицуРеализованоИП(МПЗРеализованныеИП, СтатусыОплатыРасходовИПСписок)
	...
	|	ВЫБОР
	|		КОГДА МПЗРеализованныеИП.РасчетныйДокумент = НЕОПРЕДЕЛЕНО
...
Процедура СформироватьДвижения_ОстатокВзносаДляУменьшенияНалога(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Реквизиты = ТаблицаРеквизиты[0];
	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.РасчетыПоНалогамИСборам 
		Или ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Реквизиты.Организация)
		Или Реквизиты.Период < УчетСтраховыхВзносовИП.ДатаПереходаНаУпрощенныйЗачетВзносов() Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаНалоговИВзносов = ПараметрыПроведения.ТаблицаРасчетыПоНалогамИСборам;
	
	Отбор = Новый Структура("СчетУчета, ВидПлатежаВБюджет");
	Отбор.СчетУчета = ПланыСчетов.Хозрасчетный.ПФР_ОПС_ИП;
	Отбор.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела;
	СтрокиВзносовСвышеПредела = ТаблицаНалоговИВзносов.НайтиСтроки(Отбор);
	
	ОстатокВзносаДляУменьшенияНалога = 0;
	ВзносовНачислено = 0;
	
	Для Каждого СтрокаВзносаСвышеПредела Из СтрокиВзносовСвышеПредела Цикл
		ОстатокВзносаДляУменьшенияНалога = ОстатокВзносаДляУменьшенияНалога
			+ СтрокаВзносаСвышеПредела.СуммаВзносаДляУменьшенияНалога;
		ВзносовНачислено = ВзносовНачислено + СтрокаВзносаСвышеПредела.СуммаКт;
	КонецЦикла;
	
	Движения.УменьшениеНалогаНаСтраховыеВзносыИП.Очистить();
	Если ОстатокВзносаДляУменьшенияНалога >= 0 И ОстатокВзносаДляУменьшенияНалога < ВзносовНачислено Тогда
		...
		Движение.СтраховойПериод = Реквизиты.Период;
		Движение.Сумма = Макс(0, ВзносовНачислено - ОстатокВзносаДляУменьшенияНалога);
	КонецЕсли;
...
Функция ПолучитьСоответствиеРазделовФормам()
	...
	СоответствиеРазделыУчета.Вставить(Перечисления.РазделыУчетаДляВводаОстатков.СпецоснасткаВЭксплуатации, "ФормаБухСправка");
	...
	СоответствиеРазделыУчета.Вставить(Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг, "ФормаАрендаЛизинг");
	СоответствиеРазделыУчета.Вставить(Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости, "ФормаНДСУчтенныйВСтоимости");
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если Параметры.Ключ.ОтражатьВБухгалтерскомУчете
			И НЕ Параметры.Ключ.ОтражатьВНалоговомУчете
			И НЕ Параметры.Ключ.ОтражатьПоСпециальнымРегистрам Тогда
			Если Обработки.ВводНачальныхОстатков.МожноИспользоватьПримитивнуюФормуПомощника(Параметры.Ключ.Организация) Тогда
...
Функция ПодготовитьТаблицуДляОтраженияВРегистреРаздельныйУчетНДС(Реквизиты, ТаблицаПартийНДС)
	...
	Для каждого СтрокаТаблицы Из ДанныеДвижений Цикл
		...
		Если Реквизиты.РаздельныйУчетНДСБезВычетов Тогда
			СтрокаТаблицы.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПустаяСсылка();
		...
		Для Ном = 1 По СвойстваСчета.КоличествоСубконто Цикл
			...
			ИначеЕсли СвойстваСчета["ВидСубконто" + Ном] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Партии Тогда
				Если ПартионныйУчет И НЕ СтрокаТаблицы.НетПартииНаОстатках Тогда
...
Функция ТекстЗапросТаблицаСтруктураСтоимостиЗапасов(НомераТаблиц, Реквизиты)
	...
	|	МАКСИМУМ(СтруктураСтоимостиЗапасов.НомерСтроки) КАК НомерДетальнойЗаписи
	|ИЗ
	...
	|	БухСправка.Ключ,
	|	МАКСИМУМ(СтруктураСтоимостиЗапасов.НомерСтроки)
	|ИЗ
	|	Документ.ВводНачальныхОстатков.СтруктураСтоимостиЗапасов КАК СтруктураСтоимостиЗапасов
...
Процедура СформироватьДвижения_ОС_СпособыОтраженияРасходовПоАренднымПлатежамНал(ТаблицаОС, Движения, Отказ)
	...
	Для Каждого СтрокаОС Из ТаблицаОС Цикл
		Если СтрокаОС.СпособПоступленияРегл <> Перечисления.СпособыПоступленияАктивов.ПоДоговоруЛизинга 
			И СтрокаОС.СпособПоступленияРегл <> Перечисления.СпособыПоступленияАктивов.ПоДоговоруАренды Тогда
			Продолжить;
		КонецЕсли;
...
Функция ТекстЗапросаГТД(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ОтражатьГТДВместеСОстатками Тогда
		ПараметрыПроведения.Вставить("ГТД", Неопределено);
		Возврат "";
	КонецЕсли;
	НомераТаблиц.Вставить("ГТД", НомераТаблиц.Количество());
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Номенклатура КАК Номенклатура,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Количество КАК Количество,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Сумма КАК Сумма,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.НомерГТД КАК НомерГТД,
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ГТД) КАК СчетУчета
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НоменклатураНаСкладе КАК ВводНачальныхОстатковНоменклатураНаСкладе
	|ГДЕ
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковНоменклатураНаСкладе.ПрослеживаемыйТовар = ЛОЖЬ
	|	И НЕ ВводНачальныхОстатковНоменклатураНаСкладе.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|	И НЕ ВводНачальныхОстатковНоменклатураНаСкладе.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	|	И НЕ ВводНачальныхОстатковНоменклатураНаСкладе.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТараПодТоваромИПорожняя)
	|	И НЕ ВводНачальныхОстатковНоменклатураНаСкладе.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.КорректировкаТоваровПрошлогоПериода)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Номенклатура,
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Количество,
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Сумма,
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.СтранаПроисхождения,
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.НомерГТД,
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ГТД)
	|ИЗ
	|	Документ.ВводНачальныхОстатков.ТоварыВРозничнойТорговлеАТТ КАК ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ
	|ГДЕ
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйТовар = ЛОЖЬ
	|	И НЕ ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.СтранаПроисхождения = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|	И НЕ ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.НомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)";
	
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
КонецФункции

Функция ТекстЗапросаПрослеживаемость(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ВедетсяУчетПрослеживаемыхТоваров Тогда
		ПараметрыПроведения.Вставить("ПрослеживаемаяНоменклатура", Неопределено);
		ПараметрыПроведения.Вставить("РНПТ",                       Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВТ_Прослеживаемость",        НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ПрослеживаемаяНоменклатура", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("РНПТ",                       НомераТаблиц.Количество());
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Номенклатура КАК Номенклатура,
	|	СУММА(ВводНачальныхОстатковСведенияПрослеживаемости.Количество) КАК Количество,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковНоменклатураНаСкладе.ПрослеживаемыйКомплект
	|			ТОГДА ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения
	|		ИНАЧЕ ВводНачальныхОстатковНоменклатураНаСкладе.СтранаПроисхождения
	|	КОНЕЦ КАК СтранаПроисхождения,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	СУММА(ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Комиссионер,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	...
	|ПОМЕСТИТЬ ВТ_ПрослеживаемостьНоменклатураНаСкладе
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НоменклатураНаСкладе КАК ВводНачальныхОстатковНоменклатураНаСкладе
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковНоменклатураНаСкладе.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковНоменклатураНаСкладе.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковНоменклатураНаСкладе.ПрослеживаемыйТовар
	|	И ВводНачальныхОстатковНоменклатураНаСкладе.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		ИНАЧЕ ВводНачальныхОстатковНоменклатураНаСкладе.СтранаПроисхождения
	|	КОНЕЦ,
	|	ВводНачальныхОстатковНоменклатураНаСкладе.ПрослеживаемыйКомплект,
	...
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйКомплект
	|			ТОГДА ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения
	|		ИНАЧЕ ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.СтранаПроисхождения
	|	КОНЕЦ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйКомплект,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	Документ.ВводНачальныхОстатков.ТоварыВРозничнойТорговлеАТТ КАК ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.ПрослеживаемыйТовар
	|	И ВводНачальныхОстатковТоварыВРозничнойТорговлеАТТ.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковМатериалыПереданныеВПереработку.Номенклатура,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Количество,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковМатериалыПереданныеВПереработку.ПрослеживаемыйКомплект
	|			ТОГДА ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения
	|		ИНАЧЕ ВводНачальныхОстатковМатериалыПереданныеВПереработку.СтранаПроисхождения
	|	КОНЕЦ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|	ВводНачальныхОстатковМатериалыПереданныеВПереработку.ПрослеживаемыйКомплект,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	Документ.ВводНачальныхОстатков.МатериалыПереданныеВПереработку КАК ВводНачальныхОстатковМатериалыПереданныеВПереработку
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковМатериалыПереданныеВПереработку.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковМатериалыПереданныеВПереработку.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковМатериалыПереданныеВПереработку.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковМатериалыПереданныеВПереработку.ПрослеживаемыйТовар
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковМПЗОтгруженные.Номенклатура,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Количество,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковМПЗОтгруженные.ПрослеживаемыйКомплект
	|			ТОГДА ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения
	|		ИНАЧЕ ВводНачальныхОстатковМПЗОтгруженные.СтранаПроисхождения
	|	КОНЕЦ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	ВводНачальныхОстатковМПЗОтгруженные.Покупатель,
	|	ВводНачальныхОстатковМПЗОтгруженные.ПрослеживаемыйКомплект,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	Документ.ВводНачальныхОстатков.МПЗОтгруженные КАК ВводНачальныхОстатковМПЗОтгруженные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковМПЗОтгруженные.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковМПЗОтгруженные.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковМПЗОтгруженные.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковМПЗОтгруженные.ПрослеживаемыйТовар = ИСТИНА
	|	И ВводНачальныхОстатковМПЗОтгруженные.ДоговорПокупателя.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ВводНачальныхОстатковБухСправка.Субконто1) = ТИП(Справочник.Номенклатура)
	|			ТОГДА ВводНачальныхОстатковБухСправка.Субконто1
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(ВводНачальныхОстатковБухСправка.Субконто2) = ТИП(Справочник.Номенклатура)
	|					ТОГДА ВводНачальныхОстатковБухСправка.Субконто2
	|				ИНАЧЕ ВводНачальныхОстатковБухСправка.Субконто3
	|			КОНЕЦ
	|	КОНЕЦ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Количество,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковБухСправка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПереданныеНаКомиссию)
	|			ТОГДА ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ВводНачальныхОстатковБухСправка.Субконто1) = ТИП(Справочник.Контрагенты)
	|						ТОГДА ВводНачальныхОстатковБухСправка.Субконто1
	|					ИНАЧЕ ВЫБОР
	|							КОГДА ТИПЗНАЧЕНИЯ(ВводНачальныхОстатковБухСправка.Субконто2) = ТИП(Справочник.Контрагенты)
	|								ТОГДА ВводНачальныхОстатковБухСправка.Субконто2
	|							ИНАЧЕ ВводНачальныхОстатковБухСправка.Субконто3
	|						КОНЕЦ
	|				КОНЕЦ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВводНачальныхОстатковБухСправка.ПрослеживаемыйКомплект,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковБухСправка.СчетУчета = ЗНАЧЕНИЕ(Плансчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение)
	|			ТОГДА ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ВводНачальныхОстатковБухСправка.Субконто1) = ТИП(Справочник.Контрагенты)
	|						ТОГДА ВводНачальныхОстатковБухСправка.Субконто1
	|					ИНАЧЕ ВЫБОР
	...
	|						КОНЕЦ
	|				КОНЕЦ
	|		ИНАЧЕ ВводНачальныхОстатковСведенияПрослеживаемости.Комитент
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВводНачальныхОстатковБухСправка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыЗапасовПрослеживаемыхТоваров.ТоварыЗакупленныеДляКомитента)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ВводНачальныхОстатковБухСправка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыНаСкладе)
	|						ИЛИ ВводНачальныхОстатковБухСправка.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыПереданныеНаКомиссию)
	...
	|	Документ.ВводНачальныхОстатков.БухСправка КАК ВводНачальныхОстатковБухСправка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковБухСправка.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковБухСправка.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковБухСправка.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковБухСправка.ПрослеживаемыйТовар = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Номенклатура КАК Номенклатура,
	|	СУММА(ВТ_ПрослеживаемостьНоменклатураНаСкладе.Количество) КАК Количество,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.РНПТ КАК РНПТ,
	|	СУММА(ВТ_ПрослеживаемостьНоменклатураНаСкладе.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комитент КАК Комитент,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комиссионер КАК Комиссионер,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.ВидЗапасов КАК ВидЗапасов,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комплектующие КАК Комплектующие,
	|	НЕОПРЕДЕЛЕНО КАК ОснованиеДляВозврата
	|ИЗ
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе КАК ВТ_ПрослеживаемостьНоменклатураНаСкладе
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Номенклатура,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.РНПТ,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.СтранаПроисхождения,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комиссионер,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комплектующие,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.Комитент,
	|	ВТ_ПрослеживаемостьНоменклатураНаСкладе.ВидЗапасов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
// Отложенный обработчик заполняет в документах ввода остатков суммы НУ по счету 55.23 у организаций-плательщиков налога на прибыль
//
Процедура ЗаполнитьСуммыНУПоСчетуДепозитныеСчетаВал(Параметры) Экспорт 
	...
	// Получаем все документы, где сумма НУ по 55.23 = 0.
	// Фильтр по плательщикам налога на прибыль - при обходе выборки.
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	Документ.ВводНачальныхОстатков.БухСправка КАК ТаблицаБухСправка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков КАК ВводНачальныхОстатков
	|		ПО ТаблицаБухСправка.Ссылка = ВводНачальныхОстатков.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО (ВводНачальныхОстатков.Организация = Организации.Ссылка)
	|ГДЕ
	|	ВводНачальныхОстатков.РазделУчета = &ДенежныеСредства
	|	И ТаблицаБухСправка.СчетУчета = &ДепозитныеСчетаВал
	|	И ТаблицаБухСправка.Сумма <> 0
	|	И ТаблицаБухСправка.СуммаНУ = 0";
	Запрос.УстановитьПараметр("ДенежныеСредства", Перечисления.РазделыУчетаДляВводаОстатков.ДенежныеСредства);
	...
	Пока Выборка.Следующий() Цикл
		...
		ПлательщикНалогаНаПрибыль = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Выборка.Организация, КонецДня(Выборка.Дата) + 1);
		...
		Попытка
			...
			Для каждого СтрокаТаблицы Из Объект.БухСправка Цикл
				...
				Если СтрокаТаблицы.СчетУчета <> ПланыСчетов.Хозрасчетный.ДепозитныеСчетаВал
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ВводНачальныхОстатков.ЗаполнитьСуммыНУПоСчетуДепозитныеСчетаВал() не удалось обработать документ по причине:
					|%1'"),
	...
	Если ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ВводНачальныхОстатков.ЗаполнитьСуммыНУПоСчетуДепозитныеСчетаВал() не удалось обработать часть документов: в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Документы.ВводНачальныхОстатков, ,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ВводНачальныхОстатков.ЗаполнитьСуммыНУПоСчетуДепозитныеСчетаВал() обработала очередную порцию: %1 документов'"), 
				ОбъектовОбработано));
	КонецЕсли;
...
// ДВИЖЕНИЯ ПО ПРОСЛЕЖИВАЕМОСТИ

Процедура СформироватьДвижения_Прослеживаемость(ПараметрыПроведения, ТаблицаРеквизиты, Движения, Отказ)Экспорт
	
	Реквизиты = ТаблицаРеквизиты[0];
	
	Если НЕ Реквизиты.ОтражатьВБухгалтерскомУчете Тогда 
		Возврат;
	КонецЕсли;
	
	Если НЕ УчетПрослеживаемостиВедетсяПоРазделуУчета(Реквизиты.РазделУчета) Тогда
		Возврат;
	КонецЕсли;
	
	ПрослеживаемостьБП.СформироватьДвиженияПоступлениеТоваров(
		ПараметрыПроведения.ПрослеживаемаяНоменклатура,
		Неопределено,
	...
	СформироватьДвиженияПоГТД(ПараметрыПроведения.ГТД, Реквизиты, Движения);
	
КонецПроцедуры

Процедура СформироватьДвиженияПоГТД(ТаблицаГТД, Реквизиты, Движения)
	
	Если Не Реквизиты.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Товары Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекущаяСтрока Из ТаблицаГТД Цикл
		...
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Номенклатура", ТекущаяСтрока["Номенклатура"]);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "СтраныПроисхождения", ТекущаяСтрока["СтранаПроисхождения"]);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "НомераГТД", ТекущаяСтрока["НомерГТД"]);
		
	КонецЦикла;

КонецПроцедуры

Функция ТекстЗапросаПрослеживаемостьОС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	"ВЫБРАТЬ
	|	СУММА(ВводНачальныхОстатковСведенияПрослеживаемости.Количество) КАК Количество,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	СУММА(ВводНачальныхОстатковСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура КАК Комплектующие,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ПО ВводНачальныхОстатковОС.Ссылка = ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка
	|			И ВводНачальныхОстатковОС.Ключ = ВводНачальныхОстатковСведенияПрослеживаемости.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковОС.ПрослеживаемыйТовар
	|	И ВводНачальныхОстатковОС.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВводНачальныхОстатковСведенияПрослеживаемости.РНПТ,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.Номенклатура,
	|	ВводНачальныхОстатковОС.ОсновноеСредство,
	|	ВводНачальныхОстатковСведенияПрослеживаемости.СтранаПроисхождения
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Номенклатура
	|ИЗ
	|	Документ.ВводНачальныхОстатков.СведенияПрослеживаемости КАК ВводНачальныхОстатковСведенияПрослеживаемости
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.НоменклатураНаСкладе КАК ВводНачальныхОстатковНоменклатураНаСкладе
	|		ПО ВводНачальныхОстатковСведенияПрослеживаемости.Ключ = ВводНачальныхОстатковНоменклатураНаСкладе.Ключ
	|ГДЕ
	|	ВводНачальныхОстатковНоменклатураНаСкладе.Ссылка = &Ссылка
	|	И ВводНачальныхОстатковНоменклатураНаСкладе.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СтроительствоОбъектовОсновныхСредств)
	|	И ВводНачальныхОстатковСведенияПрослеживаемости.Ссылка = &Ссылка";
	
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
КонецФункции

Функция УчетПрослеживаемостиВедетсяПоРазделуУчета(РазделУчета) Экспорт
	
	УчетПоПрослеживаемостиВедется = Ложь;
	...
	Если РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.Материалы
		...
		ИЛИ РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.КапитальныеВложения 
		...
		УчетПоПрослеживаемостиВедется =  Истина;
	КонецЕсли;

	Возврат УчетПоПрослеживаемостиВедется;
	
КонецФункции

Функция ТекстЗапросаНДСУчтенныйВСтоимости(НомераТаблиц, ПараметрыПроведения, Реквизиты)

	Если Реквизиты.РазделУчета <> Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ТаблицаДанныеПоНДСУчтенномуВСтоимости", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ТаблицаНДСПредъявленныйПоНДСУчтенномуВСтоимости", НомераТаблиц.Количество());
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетУчетаНДС КАК СчетУчетаНДС,
	...
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Комиссионер КАК Комиссионер,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Склад КАК Склад,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Партия КАК Партия,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.ДокументПоступления КАК ДокументПоступления,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Номенклатура КАК Номенклатура,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СтатьяЗатрат КАК СтатьяЗатрат,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.НДС КАК СуммаНДС
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НДСУчтенныйВСтоимости КАК ВводНачальныхОстатковНДСУчтенныйВСтоимости
	|ГДЕ
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Ссылка = &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Партия КАК СчетФактура,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.ВидЦенности КАК ВидЦенности,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СтавкаНДС КАК СтавкаНДС,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетУчетаНДС КАК СчетУчетаНДС,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Контрагент КАК Контрагент,
	|	МАКСИМУМ(ВводНачальныхОстатковНДСУчтенныйВСтоимости.ДатаПартии) КАК ДатаОплаты,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	МАКСИМУМ(ВводНачальныхОстатковНДСУчтенныйВСтоимости.ИсправленныйСчетФактура) КАК ИсправленныйСчетФактура,
	|	СУММА(ВводНачальныхОстатковНДСУчтенныйВСтоимости.СуммаБезНДС) КАК СуммаБезНДС,
	|	СУММА(ВводНачальныхОстатковНДСУчтенныйВСтоимости.НДС) КАК НДС,
	|	МАКСИМУМ(ВводНачальныхОстатковНДСУчтенныйВСтоимости.ДатаПартии) КАК ДатаСобытия,
	|	0 КАК СуммаБезНДСВключеноВСтоимость,
	|	0 КАК НДСВключеноВСтоимость
	|ИЗ
	|	Документ.ВводНачальныхОстатков.НДСУчтенныйВСтоимости КАК ВводНачальныхОстатковНДСУчтенныйВСтоимости
	|ГДЕ
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.ВидЦенности,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СчетУчетаНДС,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.СтавкаНДС,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.Контрагент,
	|	ВводНачальныхОстатковНДСУчтенныйВСтоимости.ДоговорКонтрагента,
...
// Движения по НДС учтенный в стоимости
Процедура СформироватьДвижения_НДСУчтенныйВСтоимости(ПараметрыПроведения,
									ТаблицаРеквизиты, Движения, Отказ) Экспорт   

	Реквизиты = ТаблицаРеквизиты[0];

	Если НЕ Реквизиты.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.НДСУчтенныйВСтоимости Тогда
		Возврат;
	КонецЕсли;
	
	ТаблицаНДСУчтенныйВСтоимости = ПараметрыПроведения.ТаблицаДанныеПоНДСУчтенномуВСтоимости;
	
	Если ТаблицаНДСУчтенныйВСтоимости.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаНДС ИЗ ТаблицаНДСУчтенныйВСтоимости Цикл
		Проводка = НаборЗаписейДвижения.Добавить();
		Проводка.Период      = Реквизиты.Период;
		Проводка.Организация = Реквизиты.Организация;
		Проводка.Содержание  = СодержаниеПроводки;
		...
		Проводка.СчетДт = СтрокаНДС.СчетУчетаНДС;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Контрагенты",СтрокаНДС.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "СФПолученные",СтрокаНДС.Партия);
		БухгалтерскийУчет.УстановитьПодразделениеПроводки(Проводка, Реквизиты.ПодразделениеОрганизации, "Дт");
		Если Реквизиты.РаздельныйУчетНДСНаСчете19 Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "СпособыУчетаНДС", Перечисления.СпособыУчетаНДС.ПринимаетсяКВычету);
		КонецЕсли;
		...
		//Проводка.СчетКт
		Проводка.СчетКт =  СтрокаНДС.СчетУчета;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Номенклатура",         СтрокаНДС.Номенклатура);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "ОбъектыСтроительства", СтрокаНДС.Номенклатура);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "НематериальныеАктивы", СтрокаНДС.Номенклатура);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "РасходыНаНИОКР",       СтрокаНДС.Номенклатура);
		Если ВедетсяУчетПоПартиям Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Партии", СтрокаНДС.ДокументПоступления);
		КонецЕсли;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Склады",СтрокаНДС.Склад);
		Если ЗначениеЗаполнено(СтрокаНДС.Комиссионер) Тогда 
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты",СтрокаНДС.Комиссионер);
		Иначе 
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты",СтрокаНДС.Контрагент);
		КонецЕсли;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "СтатьиЗатрат",СтрокаНДС.СтатьяЗатрат);
	...
	ТаблицаНДСПредъявленныйПоНДСУчтенномуВСтоимости = ПараметрыПроведения.ТаблицаНДСПредъявленныйПоНДСУчтенномуВСтоимости;
	СформироватьДвижения_НДС_НДСПредъявленный(ТаблицаНДСПредъявленныйПоНДСУчтенномуВСтоимости, Реквизиты, Движения, Отказ);
