...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	Набор.Имя = "Документ_УведомлениеОбОстаткахПрослеживаемыхТоваров";
	Набор.Идентификатор = Новый УникальныйИдентификатор("0e6eeb28-be4e-11ed-afa1-0242ac120002");
	Набор.Используется  = ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров");
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если (Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ)) Тогда
		...
		ПараметрыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.Ключ, "НомерКорректировки,ВидОперации");
		
		ВидУведомления= ОпределитьВидУведомления(ПараметрыДокумента);
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.Текст = ПрослеживаемостьПереопределяемый.ТекстЗапросаТаблицаТоваровУведомлениеОбОстатках(
					...
					+ ПрослеживаемостьПереопределяемый.ТекстЗапросаТаблицаОСУведомлениеОбОстатках(
																								НомераТаблиц, ДокументСсылка);
...
Функция ТекстЗапросаТаблицаПрослеживаемыхТоваровПоОснованию() Экспорт
	...
	Возврат ПрослеживаемостьПереопределяемый.ТекстЗапросаТаблицаПрослеживаемыхТоваровПоОснованиюУведомленияОбОстатках();
...
Функция ЗаполнитьДокументДанными(Период, Организация, ДокументОснование = Неопределено, ДокументИсключение = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА УведомлениеОбОстаткахПрослеживаемыхТоваров.ПервичныйДокумент = НЕОПРЕДЕЛЕНО
	|			ТОГДА УведомлениеОбОстаткахПрослеживаемыхТоваров.Ссылка
	|		ИНАЧЕ УведомлениеОбОстаткахПрослеживаемыхТоваров.ПервичныйДокумент
	|	КОНЕЦ КАК ПервичныйДокумент,
	|	&ДокументОснование КАК ОснованиеКорректировки,
	...
	|	УведомлениеОбОстаткахПрослеживаемыхТоваров.НаименованиеПервичногоДокумента КАК НаименованиеПервичногоДокумента,
	|	УведомлениеОбОстаткахПрослеживаемыхТоваров.НомерПервичногоДокумента КАК НомерПервичногоДокумента,
	|	УведомлениеОбОстаткахПрослеживаемыхТоваров.ДатаПервичногоДокумента КАК ДатаПервичногоДокумента
	|ИЗ
	|	Документ.УведомлениеОбОстаткахПрослеживаемыхТоваров КАК УведомлениеОбОстаткахПрослеживаемыхТоваров
	|ГДЕ
...
Функция ПолучитьСоответствиеВидовУведомленийФормам() Экспорт
	...
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.УведомлениеПоТоварам, "ФормаДокументаОсновная");
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.УведомлениеПоОС, "ФормаДокументаОсновная");
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.УведомлениеПоКомплектующимОС, "ФормаДокументаОсновная");
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.КорректировочноеУведомлениеПоТоварам, "ФормаДокументаКорректировочная");
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.КорректировочноеУведомлениеПоОС, "ФормаДокументаКорректировочная");
	ФормыУведомленияОбОстатках.Вставить(ВидыУведомлений.КорректировочноеУведомлениеПоКомплектующимОС, "ФормаДокументаКорректировочная");
	
	Возврат ФормыУведомленияОбОстатках;

КонецФункции 

Функция ОпределитьВидУведомления(ПараметрыДокумента)

	ВидыУведомлений = ПрослеживаемостьБРУ.ВидыУведомленийОбОСтатках();
	
	Если ПараметрыДокумента.НомерКорректировки > 0 Тогда
		
		Если ПараметрыДокумента.ВидОперации = Перечисления.ВидыОперацийУведомленияОбОстаткахПрослеживаемыхОбъектов.ОсновныеСредства Тогда
			
			Возврат ВидыУведомлений.КорректировочноеУведомлениеПоОС;
			
		ИначеЕсли ПараметрыДокумента.ВидОперации = Перечисления.ВидыОперацийУведомленияОбОстаткахПрослеживаемыхОбъектов.КомплектующиеОС Тогда
			
			Возврат ВидыУведомлений.КорректировочноеУведомлениеПоКомплектующимОС;
			
		КонецЕсли;
		
		Возврат ВидыУведомлений.КорректировочноеУведомлениеПоТоварам;
		
	КонецЕсли;
	
	Если ПараметрыДокумента.ВидОперации = Перечисления.ВидыОперацийУведомленияОбОстаткахПрослеживаемыхОбъектов.ОсновныеСредства Тогда
			
		Возврат ВидыУведомлений.УведомлениеПоОС;
		
	ИначеЕсли ПараметрыДокумента.ВидОперации = Перечисления.ВидыОперацийУведомленияОбОстаткахПрослеживаемыхОбъектов.КомплектующиеОС Тогда
		
		Возврат ВидыУведомлений.УведомлениеПоКомплектующимОС;
		
	КонецЕсли;

	Возврат ВидыУведомлений.УведомлениеПоТоварам;
