...
//  ВедетсяУчетПрослеживаемыхТоваров - Булево
// 
Процедура ЗаполнитьМатериалыПоПродукции(Материалы, Продукция, ПрямыеРасходыУчитываютсяПоПродукции,
	                                    ВедетсяУчетПрослеживаемыхТоваров) Экспорт
	...
	Запрос.УстановитьПараметр("ПрямыеРасходыУчитываютсяПоПродукции", ПрямыеРасходыУчитываютсяПоПродукции);
...
Функция ТекстЗапросаТаблицыДокумента(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаВозвращенныеМатериалы.Ссылка КАК Ссылка,
	...
	|	ТаблицаВозвращенныеМатериалы.СчетПередачи КАК СчетПередачи
...
Функция ТекстЗапросаПоступлениеУслуг(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	//
	// На вкладке "Услуги" можно указывать продукцию (предполагается, что это изготовленная переработчиком продукция).
	// То есть, вместо услуги "Изготовление продукции А" мы просто указываем "А",
	// и тогда это значение можно использовать для учета затрат на счете 20.
	// Но иногда услуга может быть указана без деления по наименованиям продукции, тогда в поле "Номенклатура"
	// указывается услуга и это значение нельзя использовать для учета затрат на счете 20.
	//
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Реквизиты.Ссылка КАК Регистратор,
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Номенклатура КАК Номенклатура,
	|	СУММА(ПоступлениеИзПереработкиСведенияПрослеживаемости.Количество) КАК Количество,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ПоступлениеИзПереработкиСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	СУММА(ПоступлениеИзПереработкиСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка КАК ДокументОперации,
	|	НАЧАЛОПЕРИОДА(ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка.Дата, КВАРТАЛ) КАК ПериодОперации,
	...
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Продукция КАК Продукция
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеИзПереработки.СведенияПрослеживаемости КАК ПоступлениеИзПереработкиСведенияПрослеживаемости
	|		ПО ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка = ПоступлениеИзПереработкиСведенияПрослеживаемости.Ссылка
	|			И ПоступлениеИзПереработкиИспользованныеМатериалы.ИдентификаторСтроки = ПоступлениеИзПереработкиСведенияПрослеживаемости.ИдентификаторСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	...
	|ГДЕ
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.ПрослеживаемыйТовар
	|	И ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.СтранаПроисхождения,
	|	ПоступлениеИзПереработкиСведенияПрослеживаемости.РНПТ,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Номенклатура,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка,
	|	НАЧАЛОПЕРИОДА(ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка.Дата, КВАРТАЛ),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """"),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.ИдентификаторСтроки,
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Продукция
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ПоступлениеИзПереработкиПродукция.Номенклатура КАК Продукция,
	...
	|		ПО ВТ_Прослеживаемость.Продукция = ПоступлениеИзПереработкиПродукция.Номенклатура
	|ГДЕ
	|	ПоступлениеИзПереработкиПродукция.Ссылка = &Ссылка
	|	И ПоступлениеИзПереработкиПродукция.ПрослеживаемыйКомплект
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПодготовитьТаблицыМатериаловПродукцииИП(ТаблицаСписанныеМПЗ, ТаблицаВозвращенныеМатериалы, ВыпускПродукцииТаблица, ТаблицаРеквизиты, Движения) Экспорт
	...
	СтруктураТаблиц = НоваяСтруктураТаблицМатериаловПродукцииИП();
	...
	ВремТаблицаСписанныеМПЗ.Колонки.Добавить("СчетЗатрат",
		Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	ВремТаблицаСписанныеМПЗ.Колонки.Добавить("НоменклатурнаяГруппа",
		Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));
	ВремТаблицаСписанныеМПЗ.Колонки.Добавить("СтатьяЗатрат",
		Новый ОписаниеТипов(МассивТиповЗатрат));
	
	ВремТаблицаСписанныеМПЗ.ЗагрузитьКолонку(
		ВремТаблицаСписанныеМПЗ.ВыгрузитьКолонку("КорСчетСписания"), "СчетЗатрат");
	ВремТаблицаСписанныеМПЗ.ЗагрузитьКолонку(
		ВремТаблицаСписанныеМПЗ.ВыгрузитьКолонку("КорСубконто1"), "НоменклатурнаяГруппа");
	ВремТаблицаСписанныеМПЗ.ЗагрузитьКолонку(
		ВремТаблицаСписанныеМПЗ.ВыгрузитьКолонку("КорСубконто2"), "СтатьяЗатрат");
	
	КолонкаБазаРаспределенияВыпускПродукции = ?(ИспользоватьПлановуюСебестоимость, "ПлановаяСтоимость", "Количество");
	
	// Заполняем таблицу "ПолученоПродукции"
	Для Каждого Продукция Из ВыпускПродукцииТаблица Цикл
		
		Если Продукция[КолонкаБазаРаспределенияВыпускПродукции] = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяСтрока = СтруктураТаблиц.ПолученоПродукции.Добавить();
	...
	СтруктураТаблиц.ПолученоПродукции.Свернуть(
		"СчетЗатрат, НоменклатурнаяГруппа, Продукция, СчетУчетаПродукции", "КоличествоПродукции, ПлановаяСтоимость");
	...
	Для Каждого Материал Из ВремТаблицаСписанныеМПЗ Цикл
		...
		МатериалПродукция = УчетТоваров.ЗначениеКорСубконто(
			Материал, ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Продукция);
		
		МассивКоэффициентов = КоэффициентыРаспределенияНаПродукцию(
			СтруктураТаблиц.ПолученоПродукции, МатериалПродукция, КешБазыРаспределения, КолонкаБазаРаспределения);
		
		Если МассивКоэффициентов.Количество() > 0 Тогда
			
			МассивКоличество = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
				Материал.Количество, МассивКоэффициентов, 3);
			...
			Для Индекс = 0 По МассивКоэффициентов.ВГраница() Цикл
				...
				МатериалПоПродукции = СтруктураТаблиц.ТаблицаМатериалов.Добавить();
				ЗаполнитьЗначенияСвойств(МатериалПоПродукции, Материал);
				ЗаполнитьЗначенияСвойств(МатериалПоПродукции, СтруктураТаблиц.ПолученоПродукции[Индекс]);
				
				МатериалПоПродукции.СчетЗатрат = СтруктураТаблиц.ПолученоПродукции[Индекс].СчетУчетаПродукции;
	...
	Для Каждого Услуга Из Движения.ИПМПЗ Цикл
		...
		МассивКоэффициентов = КоэффициентыРаспределенияНаПродукцию(
			СтруктураТаблиц.ПолученоПродукции, Услуга.Номенклатура, КешБазыРаспределения, КолонкаБазаРаспределения);
		
		Если МассивКоэффициентов.Количество() > 0 Тогда
			
			МассивКоличество = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
				Услуга.Количество, МассивКоэффициентов, 3);
			МассивСумма = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
				Услуга.Сумма, МассивКоэффициентов, 2);
			МассивНДС = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
				Услуга.НДС, МассивКоэффициентов, 2);
			...
			Для Индекс = 0 По МассивКоэффициентов.ВГраница() Цикл
				...
				УслугаПоПродукции = СтруктураТаблиц.ПолученныеУслуги.Добавить();
				ЗаполнитьЗначенияСвойств(УслугаПоПродукции, Услуга);
				ЗаполнитьЗначенияСвойств(УслугаПоПродукции, СтруктураТаблиц.ПолученоПродукции[Индекс]);
		...
		Иначе
			...
			ИМР = СтруктураТаблиц.УслугиОтнесенныеНаИМР.Добавить();
	...
	СтруктураТаблиц.ТаблицаМатериалов = УчетДоходовИРасходовПредпринимателя.ДополнитьТаблицуСписанияМатериалов(
		СтруктураТаблиц.ТаблицаМатериалов, ТаблицаРеквизиты);
	
	Для Каждого СтрокаВозвращенныеМатериалы Из ТаблицаВозвращенныеМатериалы Цикл
		
		НоваяСтрока = СтруктураТаблиц.ВозвращенныеМатериалы.Добавить();
		НоваяСтрока.ИмяСписка = СтрокаВозвращенныеМатериалы.ИмяСписка;
		НоваяСтрока.СинонимСписка = СтрокаВозвращенныеМатериалы.СинонимСписка;
		НоваяСтрока.НомерСтроки = СтрокаВозвращенныеМатериалы.НомерСтроки;
		НоваяСтрока.СчетУчета = СтрокаВозвращенныеМатериалы.СчетУчета;
		НоваяСтрока.Номенклатура = СтрокаВозвращенныеМатериалы.Номенклатура;
		НоваяСтрока.Партия = СтрокаВозвращенныеМатериалы.Партия;
		НоваяСтрока.Себестоимость = СтрокаВозвращенныеМатериалы.СуммаСписания;
		НоваяСтрока.Количество = СтрокаВозвращенныеМатериалы.Количество;
		НоваяСтрока.КорСчетСписания = СтрокаВозвращенныеМатериалы.КорСчетСписания;
...
//@skip-check module-structure-method-in-regions
Функция НоваяСтруктураТаблицМатериаловПродукцииИП()
	...
	ВозвращенныеМатериалы = Новый ТаблицаЗначений;
	ВозвращенныеМатериалы.Колонки.Добавить("ИмяСписка", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	ВозвращенныеМатериалы.Колонки.Добавить("СинонимСписка", ОбщегоНазначения.ОписаниеТипаСтрока(50));
	ВозвращенныеМатериалы.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0));
	ВозвращенныеМатериалы.Колонки.Добавить("СчетУчета", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	ВозвращенныеМатериалы.Колонки.Добавить("Номенклатура", Справочники.ТипВсеСсылки());
	ВозвращенныеМатериалы.Колонки.Добавить("Партия", Документы.ТипВсеСсылки());
	ВозвращенныеМатериалы.Колонки.Добавить("Себестоимость", ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	ВозвращенныеМатериалы.Колонки.Добавить("Количество", ОбщегоНазначения.ОписаниеТипаЧисло(15, 3));
	ВозвращенныеМатериалы.Колонки.Добавить("КорСчетСписания", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	
	СтруктураТаблиц = Новый Структура;
	СтруктураТаблиц.Вставить("ТаблицаМатериалов", ТаблицаМатериалов);
	СтруктураТаблиц.Вставить("ПолученоПродукции", ПолученоПродукции);
	СтруктураТаблиц.Вставить("ПолученныеУслуги", ПолученныеУслуги);
	СтруктураТаблиц.Вставить("УслугиОтнесенныеНаИМР", УслугиОтнесенныеНаИМР);
	СтруктураТаблиц.Вставить("ВозвращенныеМатериалы", ВозвращенныеМатериалы);
...
Функция ПечатьМ4(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Шапка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Пока Шапка.Следующий() Цикл
			...
			Если НЕ ЗначениеЗаполнено(Шапка.Номенклатура) Тогда
				Продолжить;
			КонецЕсли;
...
Функция ДокументыСПрослеживаемымиТоварами(МассивДокументов) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПоступлениеИзПереработки.ИспользованныеМатериалы КАК ПоступлениеИзПереработкиИспользованныеМатериалы
	|ГДЕ
	|	ПоступлениеИзПереработкиИспользованныеМатериалы.Ссылка В(&МассивДокументов)
	|	И ПоступлениеИзПереработкиИспользованныеМатериалы.ПрослеживаемыйТовар = ИСТИНА";
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	ПоступлениеИзПереработки.Контрагент КАК Контрагент,
	|	ПоступлениеИзПереработки.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ПоступлениеИзПереработки.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	...
	|ГДЕ
	|	ПоступлениеИзПереработки.Ссылка В(&МассивОбъектов)
	|	И ПоступлениеИзПереработки.Дата > &УсловиеПоДате";
...
Функция НаличиеСтавкиНДСПоУСН(ТекущийДокумент, СтавкаНДСПоУСН) Экспорт
	...
	|ГДЕ
	|	ПоступлениеИзПереработкиУслуги.Ссылка = &СсылкаНаДокумент
	|	И ПоступлениеИзПереработкиУслуги.СтавкаНДС В (&СтавкаНДСПоУСН)
	|
	|СГРУППИРОВАТЬ ПО
	|	ПоступлениеИзПереработкиУслуги.Ссылка";
