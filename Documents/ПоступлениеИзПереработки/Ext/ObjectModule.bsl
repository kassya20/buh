...
Процедура ПриКопировании(ОбъектКопирования)
	...
	РаботаСНоменклатуройБП.ОбновитьПризнакПрослеживаемости(ИспользованныеМатериалы, ВедетсяУчетПрослеживаемыхТоваров, Дата);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Склад нужен только тогда, когда есть движения материальных ценностей, использующих
	// эту аналитику.
	Если Продукция.Количество() + ВозвращенныеМатериалы.Количество() + ВозвратнаяТара.Количество() = 0 Тогда
		// Помимо табличных частей Продукция, ВозвращенныеМатериалы, ВозвратнаяТара, материальные ценности
		// заполняются в табличной части ИспользованныеМатериалы.
		// У счетов учета в этой табличной части обычно нет субконто Склады.
		// Если это действительно так, заполнение поля Склад для данных этой табличной части не требуется.
		СчетаУчетаИспользованныеМатериалы =
			ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИспользованныеМатериалы.ВыгрузитьКолонку("СчетУчета"));
		ИспользуетсяСубконтоСклад = Ложь;
		Для каждого СчетИспользованныеМатериалы Из СчетаУчетаИспользованныеМатериалы Цикл
			Если БухгалтерскийУчет.ВедетсяУчетПоСкладам(СчетИспользованныеМатериалы) Тогда
				ИспользуетсяСубконтоСклад = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если Не ИспользуетсяСубконтоСклад Тогда
	...
	Если ЕстьПрослеживаемыйКомплект Тогда
		Для Каждого Строка Из ПрослеживаемыйКомплект Цикл
			...
			ПрослеживаемыеМатериалыПоКомплекту = ИспользованныеМатериалы.НайтиСтроки(СтруктураОтбора);
			Если ПрослеживаемыеМатериалыПоКомплекту.Количество() = 0 Тогда
				...
				ТекстСообщенияРасшифрвка = НСтр("ru = 'Не заполнены прослеживаемые материалы, входящие в состав продукции'")
					+ Символы.ПС + НСтр("ru = 'Заполните колонку ""Продукция"" таблицы ""Использованные материалы""'");
	...
	Для Каждого СтрокаТаблицы Из ИспользованныеМатериалы Цикл
		Префикс = "ИспользованныеМатериалы[" + Формат((СтрокаТаблицы.НомерСтроки - 1), "ЧН=0; ЧГ=") + "].";
		ИмяСписка = НСтр("ru = 'Использованные материалы'");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого СтрокаТабличнойЧасти Из ЭтотОбъект.ИспользованныеМатериалы Цикл
	...
	ПрослеживаемыйМатериал = ИспользованныеМатериалы.НайтиСтроки(Новый Структура("ПрослеживаемыйТовар", Истина));
	ЕстьПрослеживаемыйМатериал = ПрослеживаемыйМатериал.Количество() <> 0;
	
	Если ЕстьПрослеживаемыйМатериал Тогда
		Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
			ПрослеживаемостьБП.ПодобратьРНПТ(ЭтотОбъект, Отказ, "ИспользованныеМатериалы");
		КонецЕсли;
	Иначе
		СведенияПрослеживаемости.Очистить();
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ТаблицаПоступленияУслугПереработкиИП <> Неопределено Тогда
		...
		Если ТаблицыСписанияМПЗИП.СтоимостьПродукции = Неопределено Тогда
			...
			ТаблицыСписанияМПЗИП.СтоимостьПродукции.Колонки.Добавить("ПартияПоступившейНоменклатуры", Документы.ТипВсеСсылки());
		...
		Для Каждого УслугаПереработкиИП Из ТаблицаПоступленияУслугПереработкиИП Цикл
			...
			ПродукцияИП.ПартияПоступившейНоменклатуры = УслугаПереработкиИП.Партия;
	...
	ТаблицаИзмененияВидаМПЗИП = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицуИзмененияВидаМПЗ(
		ТаблицыМатериаловПродукцииИП.ВозвращенныеМатериалы, ПараметрыПроведения.СписаниеМатериаловРеквизиты, Отказ);
	...
	ПрослеживаемыеОперации = ПрослеживаемостьБП.ПодготовитьТаблицуПоПрослеживаемымОперациям(
		ПараметрыПроведения.ПрослеживаемыеОперации, МатериалыСОценкойСтоимости.СписаниеМатериалов);
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияИзмененияВидаМПЗ(
		ТаблицаИзмененияВидаМПЗИП, ПараметрыПроведения.СписаниеМатериаловРеквизиты, Движения, Отказ);
