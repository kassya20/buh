...
Процедура СформироватьДвижения(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПризнаниеВНУАрендныхПлатежей Тогда
		ПризнаниеВНУАрендныхПлатежей(СтруктураШапкиДокумента, Отказ, Ошибки);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеПроцентовПоКредитамЗаймам Тогда
		НачислениеПроцентовПоКредитамИЗаймам(СтруктураШапкиДокумента, Отказ, Ошибки);	
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.АмортизацияНМАИсписаниеРасходовПоНИОКР Тогда
		...
		НачислениеАмортизацииНМА_НИОКР(СтруктураШапкиДокумента, Отказ, Ошибки);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеВзносовОтНесчастныхСлучаев Тогда
		// НАЧИСЛЕНИЕ ФЗНОСОВ ОТ НЕСЧАСТНЫХ СЛУЧАЕВ ПРИ АУСН
		НачислениеВзносовОтНесчастныхСлучаев(СтруктураШапкиДокумента, Отказ);			
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетТорговогоСбора Тогда
		...
		// ТУРИСТИЧЕСКИЙ НАЛОГ
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетТуристическогоНалога Тогда
		ВыполнитьРасчетТуристическогоНалога(СтруктураШапкиДокумента, Отказ, Ошибки);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.КлассификацияЗадолженностиПоСрокам Тогда 
		// КЛАССИФИКАЦИЯ ДЕБИТОРСКОЙ ЗАДОЛЖЕННОСТИ И ОБЯЗАТЕЛЬСТВ
		КлассифицироватьЗадолженностьПоСрокам(СтруктураШапкиДокумента, Отказ);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетОтложенногоНалога Тогда
		...
		РассчитатьОтложенныйНалогНаПрибыль(СтруктураШапкиДокумента, Движения);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереводОСВМалоценноеОборудование Тогда
		...
		// ОЧИСТКА ПАРТИЙ ПРИ ПЕРЕХОДЕ С РАСЧЕТА СЕБЕСТОИМОСТИ ПО ФИФО НА УЧЕТ ПО СРЕДНЕЙ
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаПартийПриПереходеСФИФОНаУчетПоСредней Тогда
		
		КорректировкаПартийПриПереходеСФИФОНаУчетПоСредней(СтруктураШапкиДокумента, Отказ);
	...
	ИначеЕсли СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ЗакрытиеУчетныхРегистровУСН Тогда
		ЗакрытьРасчетныеРегистрыУСН(СтруктураШапкиДокумента);
...
Процедура НачислениеАмортизацииИИзносаОС(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ПараметрыНачисленияАмортизации = УчетОС.ПодготовитьТаблицыАмортизацииОСИСуммАмортизационнойПремии(Неопределено,
		ПараметрыПроведения.НачислениеАмортизацииОС, Отказ, Истина, Ошибки);
...
//
Процедура НачислениеАмортизацииНМА_НИОКР(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ПараметрыНачисленияАмортизации = УчетНМА.ПодготовитьТаблицыАмортизацииНМА(Неопределено,
		ПараметрыПроведения.НачислениеАмортизацииНМА, Отказ, Ошибки);
...
//
Процедура ПризнаниеВНУАрендныхПлатежей(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ПараметрыНачисленияАмортизации = УчетОС.ПодготовитьТаблицыАмортизацииОСИСуммАмортизационнойПремии(Неопределено,
		ПараметрыПроведения.АрендныеПлатежи, Отказ, , Ошибки);
...
// ПРОЦЕДУРЫ И ФУНКЦИИ ПО РАСЧЕТУ ВЗНОСОВ ОТ НЕСЧАСТНЫХ СЛУЧАЕВ

Процедура НачислениеВзносовОтНесчастныхСлучаев(СтруктураШапкиДокумента, Отказ)
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ИНФОРМАЦИОННОЙ БАЗЫ
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыНачислениеВзносовОтНесчастныхСлучаев(
		СтруктураШапкиДокумента, Отказ);
...
Процедура КорректировкаСтоимостиНоменклатуры(Период, Организация, Ссылка)
	...
	Если Результат.Следующий() Тогда
		ДокументИзмененияПартии = Результат.Ссылка.ПолучитьОбъект();
		ДокументИзмененияПартии.ОтменитьОперацию();
	КонецЕсли;
...
Процедура РассчитатьОтложенныйНалогНаПрибыль(СтруктураШапкиДокумента, Движения)
...
Процедура РассчитатьРасходыУменьшающиеОтдельныеНалоги(СтруктураШапкиДокумента, Отказ)
	...
	УчетПСН.ОтразитьРасходыНаОнлайнКассу(СтруктураШапкиДокумента, Движения);
	
	УчетПСН.ОтразитьИспользованныеСтраховыеВзносы(СтруктураШапкиДокумента, Движения);
...
// ПРОЦЕДУРЫ И ФУНКЦИИ ПО НАЧИСЛЕНИЮ ТУРИСТИЧЕСКОГО НАЛОГА

Процедура ВыполнитьРасчетТуристическогоНалога(СтруктураШапкиДокумента, Отказ, Ошибки)
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ДОКУМЕНТА
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыРасчетТуристическогоНалога(СтруктураШапкиДокумента, Отказ);
	...
	ТаблицыНачислениеНалога = ТуристическийНалогСервер.ПодготовитьТаблицыРасчета(
		ПараметрыПроведения.ТаблицаРеквизитыРасчетТуристическогоНалога, Отказ, Ошибки);
	...
	ТуристическийНалогСервер.СформироватьДвиженияРасчета(
		ТаблицыНачислениеНалога.ТаблицаПроводок,
		ТаблицыНачислениеНалога.ТаблицаПроводокИсключениеТурНалога,
		ТаблицыНачислениеНалога.ТаблицаСправкиРасчета,
		ПараметрыПроведения.ТаблицаРеквизитыРасчетТуристическогоНалога,
		Движения,
		Отказ);
		
	
	УчетУСН.СформироватьДвиженияОтражениеПоТуристическомуНалогу(
		ТаблицыНачислениеНалога.ТаблицаПроводокИсключениеТурНалога,
		ПараметрыПроведения.ТаблицаРеквизитыРасчетТуристическогоНалога,
		ЭтотОбъект,
		Отказ);
	...
	ТуристическийНалогСервер.ОбработатьЗадачиПоТуристическомуНалогу(СтруктураШапкиДокумента.КонДата, СтруктураШапкиДокумента.Организация); 
...
Процедура РассчитатьНалогУСН(СтруктураШапкиДокумента, Отказ)
	...
	УчетУСН.СформироватьДвиженияНачислениеНалогаУСН(
		...
		ДанныеРасчета.ТаблицаУменьшениеНалогаНаСтраховыеВзносыИП,
...
Процедура НачислитьНалогАУСН(СтруктураШапкиДокумента, Отказ)
	...
	Документы.КорректировкаЕНС.УдалитьАвтоматическиеКорректировкиАУСН(КонтекстРасчета);
...
Процедура ЗакрытьРасчетныеРегистрыУСН(СтруктураШапкиДокумента)
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыЗакрытияРасчетныхРегистровУСН(
		СтруктураШапкиДокумента);
	
	УчетУСН.СформироватьСторнирующиеДвиженияРасчетныхРегистров(
		ПараметрыПроведения.РасходыПриУСН, "РасходыПриУСН", Движения);
	УчетУСН.СформироватьСторнирующиеДвиженияРасчетныхРегистров(
		ПараметрыПроведения.ПрочиеРасчеты, "ПрочиеРасчеты", Движения);
	УчетУСН.СформироватьСторнирующиеДвиженияРасчетныхРегистров(
		ПараметрыПроведения.ОплатыНМА, "ОплатыНМАДляУСН", Движения);
	УчетУСН.СформироватьСторнирующиеДвиженияРасчетныхРегистров(
		ПараметрыПроведения.ОплатыОС, "ОплатыОСДляУСН", Движения);
	
КонецПроцедуры

// Корректировка партий при изменении способа расчета себестоимости
Процедура КорректировкаПартийПриПереходеСФИФОНаУчетПоСредней(СтруктураШапкиДокумента, Отказ)
	...
	ВедетсяСкладскойУчет = ПараметрыУчета.СкладскойУчет;
	
	Запрос.УстановитьПараметр("СчетаЗапасов", 					СчетаЗапасов);
	Запрос.УстановитьПараметр("ГраницаОстатков", 				СтруктураШапкиДокумента.КонГраница);
	Запрос.УстановитьПараметр("Организация", 					СтруктураШапкиДокумента.Организация);
	
	// если учет по складам ведется только количеству,
	// тогда остатки по сумме и по количеству будут разными строками
	// и проводки тогда тоже будут двумя строками
	// для исправления необходимо свернуть остатки
	// субконто склад может отсутствовать, и может быть любым по счету
	Если ВедетсяСкладскойУчет = 1 Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ХозрасчетныйВидыСубконто.Ссылка КАК Счет,
		|	ХозрасчетныйВидыСубконто.НомерСтроки КАК НомерСтроки
		|ПОМЕСТИТЬ СчетаССубконтоПартия
		|ИЗ
		...
		|			Счет В
		|				(ВЫБРАТЬ
		|					СчетаССубконтоПартия.Счет
		|				ИЗ
		|					СчетаССубконтоПартия),
		|			,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОстаткиПоСчетам.Счет КАК Счет,
		|	ОстаткиПоСчетам.Субконто1 КАК Субконто1,
		|	ОстаткиПоСчетам.Субконто2 КАК Субконто2,
		|	ОстаткиПоСчетам.Субконто3 КАК Субконто3,
		...
		|	ОстаткиПоСчетам.СуммаНУ КАК СуммаНУ,
		|	ОстаткиПоСчетам.СуммаПР КАК СуммаПР,
		|	ОстаткиПоСчетам.СуммаВР КАК СуммаВР,
		|	НомерСубконтоПартия.НомерСтроки КАК НомерСубконтоПартии
		|ПОМЕСТИТЬ ОстаткиПоПартии
		|ИЗ
		|	ОстаткиПоСчетам КАК ОстаткиПоСчетам
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаССубконтоПартия КАК НомерСубконтоПартия
		|		ПО ОстаткиПоСчетам.Счет = НомерСубконтоПартия.Счет
		|ГДЕ
		|	ВЫБОР
		|			КОГДА НомерСубконтоПартия.НомерСтроки = 1
		|				ТОГДА ОстаткиПоСчетам.Субконто1 <> НЕОПРЕДЕЛЕНО
		|			КОГДА НомерСубконтоПартия.НомерСтроки = 2
		|				ТОГДА ОстаткиПоСчетам.Субконто2 <> НЕОПРЕДЕЛЕНО
		|			КОГДА НомерСубконтоПартия.НомерСтроки = 3
		|				ТОГДА ОстаткиПоСчетам.Субконто3 <> НЕОПРЕДЕЛЕНО
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ОстаткиПоПартии.Счет КАК Счет,
		|	ОстаткиПоПартии.Субконто1 КАК Субконто1,
		|	ОстаткиПоПартии.Субконто2 КАК Субконто2,
		|	ОстаткиПоПартии.Субконто3 КАК Субконто3,
		|	ОстаткиПоПартии.Организация КАК Организация,
		|	ОстаткиПоПартии.Подразделение КАК Подразделение,
		|	ОстаткиПоПартии.Количество КАК Количество,
		|	ОстаткиПоПартии.Сумма КАК Сумма,
		|	ОстаткиПоПартии.СуммаНУ КАК СуммаНУ,
		|	ОстаткиПоПартии.СуммаПР КАК СуммаПР,
		|	ОстаткиПоПартии.СуммаВР КАК СуммаВР,
		|	ОстаткиПоПартии.НомерСубконтоПартии КАК НомерСубконтоПартии
		|ИЗ
		|	ОстаткиПоПартии КАК ОстаткиПоПартии
		|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаСоСкладами КАК СчетаСоСкладами
		|		ПО ОстаткиПоПартии.Счет = СчетаСоСкладами.Ссылка
		|ГДЕ
		|	СчетаСоСкладами.Ссылка ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ОстаткиПоПартии.Счет,
		|	МАКСИМУМ(ОстаткиПоПартии.Субконто1),
		|	ОстаткиПоПартии.Субконто2,
		|	ОстаткиПоПартии.Субконто3,
		|	ОстаткиПоПартии.Организация,
		|	ОстаткиПоПартии.Подразделение,
		|	СУММА(ОстаткиПоПартии.Количество),
		|	СУММА(ОстаткиПоПартии.Сумма),
		|	СУММА(ОстаткиПоПартии.СуммаНУ),
		|	СУММА(ОстаткиПоПартии.СуммаПР),
		|	СУММА(ОстаткиПоПартии.СуммаВР),
		|	ОстаткиПоПартии.НомерСубконтоПартии
		|ИЗ
		|	ОстаткиПоПартии КАК ОстаткиПоПартии
		|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаСоСкладами КАК СчетаСоСкладами
		|		ПО ОстаткиПоПартии.Счет = СчетаСоСкладами.Ссылка
		|ГДЕ
		|	СчетаСоСкладами.НомерСтроки = 1
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиПоПартии.Счет,
		|	ОстаткиПоПартии.Субконто2,
		...
		|ВЫБРАТЬ
		|	ОстаткиПоПартии.Счет,
		|	ОстаткиПоПартии.Субконто1,
		|	МАКСИМУМ(ОстаткиПоПартии.Субконто2),
		|	ОстаткиПоПартии.Субконто3,
		|	ОстаткиПоПартии.Организация,
		|	ОстаткиПоПартии.Подразделение,
		|	СУММА(ОстаткиПоПартии.Количество),
		...
		|	ОстаткиПоПартии.Субконто1,
		|	ОстаткиПоПартии.Субконто2,
		|	МАКСИМУМ(ОстаткиПоПартии.Субконто3),
		|	ОстаткиПоПартии.Организация,
		|	ОстаткиПоПартии.Подразделение,
		|	СУММА(ОстаткиПоПартии.Количество),
		|	СУММА(ОстаткиПоПартии.Сумма),
	...
	Пока Выборка.Следующий() Цикл
		...
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Выборка.Счет);
		...
		КонецЕсли;
		Если Выборка.Субконто1 <> Неопределено И Не Выборка.НомерСубконтоПартии = 1 Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, СвойстваСчета.ВидСубконто1, Выборка.Субконто1);
		КонецЕсли;
	
		Если Выборка.Субконто2 <> Неопределено И Не Выборка.НомерСубконтоПартии = 2 Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, СвойстваСчета.ВидСубконто2, Выборка.Субконто2);
		КонецЕсли;
		
		Если Выборка.Субконто3 <> Неопределено И Не Выборка.НомерСубконтоПартии = 3 Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, СвойстваСчета.ВидСубконто3, Выборка.Субконто3);
		КонецЕсли;
	
		Проводка.СчетКт = Выборка.Счет;
		
		Если Выборка.Субконто1 <> Неопределено Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, СвойстваСчета.ВидСубконто1, Выборка.Субконто1);
		КонецЕсли;
	
		Если Выборка.Субконто2 <> Неопределено Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, СвойстваСчета.ВидСубконто2, Выборка.Субконто2);
		КонецЕсли;
		
		Если Выборка.Субконто3 <> Неопределено Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, СвойстваСчета.ВидСубконто3, Выборка.Субконто3);
		КонецЕсли;
		...
		Если СвойстваСчета.НалоговыйУчет Тогда
			Проводка.СуммаНУДт = Выборка.СуммаНУ;
			Проводка.СуммаНУКт = Выборка.СуммаНУ;
			Проводка.СуммаПРДт = Выборка.СуммаПР;
			Проводка.СуммаПРКт = Выборка.СуммаПР;
			Проводка.СуммаВРДт = Выборка.СуммаВР;
			Проводка.СуммаВРКт = Выборка.СуммаВР;
		КонецЕсли;
		
		Проводка.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Переход на расчет себестоимости запасов по средней в связи с изменением учетной политики с %1'"),
			Формат(СтруктураШапкиДокумента.Дата + 86400, "ДЛФ=Д"));
	...
	УчетНДСРаздельный.ИзменениеАналитикиПоПартиямПриПереходеСФИФОНаУчетПоСредней(СтруктураШапкиДокумента, Движения, Отказ);
	
КонецПроцедуры

// Выполняет начисление процентов в БУ и НУ по кредитам и займам
//
Процедура НачислениеПроцентовПоКредитамИЗаймам(СтруктураШапкиДокумента, Отказ, Ошибки)
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ДОКУМЕНТА
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыНачислениеПроцентовПоКредитамИЗаймам(
		СтруктураШапкиДокумента, Отказ);
	...
	ТаблицаРеквизиты = ПараметрыПроведения.НачислениеПроцентовПоКредитамИЗаймам;
	
	// ПОДГОТОВКА ПРОВЕДЕНИЯ ПО ДАННЫМ ИНФОРМАЦИОННОЙ БАЗЫ
	
	ТаблицыНачислениеНачислениеПроцентовПоКредитамИЗаймам = Новый Структура;
	УчетКредитовЗаймов.ЗаполнитьТаблицыНачислениеПроцентовПоКредитамИЗаймам(
		ТаблицыНачислениеНачислениеПроцентовПоКредитамИЗаймам, ТаблицаРеквизиты, Отказ, Ошибки);
	...
	ТаблицаПроценты = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ТаблицыНачислениеНачислениеПроцентовПоКредитамИЗаймам, "ТаблицаПроценты");
	ТаблицаРасчет = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ТаблицыНачислениеНачислениеПроцентовПоКредитамИЗаймам, "ТаблицаРасчет");
		
	// ФОРМИРОВАНИЕ ДВИЖЕНИЙ
	
	УчетКредитовЗаймов.СформироватьДвиженияНачислениеПроцентовПоКредитамИЗаймам(
		Движения, ТаблицаПроценты, ТаблицаРеквизиты);
		
	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		Движения, ТаблицаРасчет, ТаблицаРеквизиты, "РасчетНачисленияПроцентовПоКредитамЗаймам");
	
КонецПроцедуры

// КЛАССИФИКАЦИЯ ДЕБИТОРСКОЙ ЗАДОЛЖЕННОСТИ И ОБЯЗАТЕЛЬСТВ ПО СРОКАМ ПОГАШЕНИЯ: ДОЛГОСРОЧНАЯ И КРАТКОСРОЧНАЯ
Процедура КлассифицироватьЗадолженностьПоСрокам(СтруктураШапкиДокумента, Отказ)
	
	ПараметрыПроведения = Документы.РегламентнаяОперация.ПодготовитьПараметрыКлассификацияЗадолженности(СтруктураШапкиДокумента, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;

	ТаблицаРеквизиты = ПараметрыПроведения.ТаблицаРеквизиты;
	
	КонтекстОперации = Новый Структура;
	Организации = Новый Массив;
	Организации.Добавить(СтруктураШапкиДокумента.Организация);
	КонтекстОперации.Вставить("Организации", Организации);
	КонтекстОперации.Вставить("НачалоПериода", СтруктураШапкиДокумента.НачДата);
	КонтекстОперации.Вставить("КонецПериода", СтруктураШапкиДокумента.КонГраница);
	КонтекстОперации.Вставить("КонецПериодаДата", СтруктураШапкиДокумента.КонДата);
	КонтекстОперации.Вставить("ДатаНачалаКраткосрочногоПериода", КонецМесяца(СтруктураШапкиДокумента.КонДата) + 1);
	КонтекстОперации.Вставить("ДатаОкончанияКраткосрочногоПериода", КонецМесяца(ДобавитьМесяц(СтруктураШапкиДокумента.КонДата, 12)));
	КонтекстОперации.Вставить("ДатаНачалаДолгосрочногоПериода", КонецМесяца(ДобавитьМесяц(СтруктураШапкиДокумента.КонДата, 12)) + 1);

	ДанныеЗадолженности = КлассификацияЗадолженности.ДанныеЗадолженностиПоСрокам(КонтекстОперации);
	
	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		Движения, ДанныеЗадолженности.ДебиторскаяЗадолженностьПоСрокам, ТаблицаРеквизиты, "КлассификацияПоСрокамОтдельныхВидовЗадолженности");
		
	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		Движения, ДанныеЗадолженности.КредиторскаяЗадолженностьПоСрокам, ТаблицаРеквизиты, "КлассификацияПоСрокамОтдельныхВидовЗадолженности");

	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		Движения, ДанныеЗадолженности.РасходыБудущихПериодов, ТаблицаРеквизиты, "КлассификацияПоСрокамРасходовБудущихПериодов");
