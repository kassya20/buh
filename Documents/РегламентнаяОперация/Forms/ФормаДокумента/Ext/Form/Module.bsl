...
Процедура СправкаРасчет(Команда)
	...
	ФормаОтчета.ОткрытьИСформировать(
		Новый Структура("Организация, НачалоПериода, КонецПериода, Период, НаборПоказателейОтчета, ВыводитьЗаголовок, ВыводитьПодвал, СНачалаГода, ВидОперации",
		Объект.Организация,
...
Процедура ПроверкаДляОбособленныхПодразделений()
	...
	Если ЗначениеЗаполнено(Объект.Организация) 
		...
		Если Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереносУбытковПоНалогуНаПрибыль		
			ИЛИ Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.ЗакрытиеСчетов90_91 
			ИЛИ Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетНалогаНаПрибыль
			ИЛИ Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.РасчетОтложенногоНалога
			ИЛИ Объект.ВидОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога Тогда
