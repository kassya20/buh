...
Процедура СписокОшибокОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Строка")
		И (СтрНачинаетсяС(Расшифровка, "http:") Или СтрНачинаетсяС(Расшифровка, "https:")) Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура ОбработатьРасшифровку(Элемент, Действие)
	...
	ИначеЕсли Действие.СпособРасшифровки = "ФормаСписка" Тогда
		...
		Если Действие.Свойство("ПараметрыФормы")
			И ЗначениеЗаполнено(Действие.ПараметрыФормы) Тогда
			Для Каждого КлючЗначение Из Действие.ПараметрыФормы Цикл
				ПараметрыОткрываемойФормы.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
			КонецЦикла;
		КонецЕсли;
