...
Процедура ДатаВходящегоДокументаПриИзменении(Элемент)
	Если НачалоДня(Объект.ДатаВходящегоДокумента) = НачалоДня(ТекущаяДатаВходящегоДокумента) Тогда
		// Изменение времени не влияет на поведение документа.
		ТекущаяДатаВходящегоДокумента = Объект.ДатаВходящегоДокумента;
		Возврат;
	КонецЕсли;
...
Процедура ДоговорКонтрагентаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	ПредлагатьНовыйДоговорСчет = ЗначениеЗаполнено(Объект.ДатаВходящегоДокумента)
		И ЗначениеЗаполнено(Объект.НомерВходящегоДокумента);
	
	РаботаСДоговорамиКонтрагентовБПКлиент.ЗаполнитьСписокВыбора(
		Элемент, Текст, ПредлагатьНовыйДоговор, СтандартнаяОбработка, , ПредлагатьНовыйДоговорСчет);
...
Процедура ДатаВходящегоДокументаПриИзмененииНаСервере()
	РаботаСНоменклатуройБП.ОбновитьСодержаниеУслуг(Объект.Товары, Объект.ДатаВходящегоДокумента, ТекущаяДатаВходящегоДокумента);
КонецПроцедуры
...
Функция ПараметрыСозданияНовогоДоговора()
	...
	Возврат РаботаСДоговорамиКонтрагентовБП.ПараметрыСозданияНовогоДоговора(ЭтотОбъект,
		...
		?(ЗначениеЗаполнено(Объект.НомерВходящегоДокумента), "НомерВходящегоДокумента", ""));
...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.ГруппаСтатусОплаты.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	Элементы.ГруппаСтатусОплаты.Объединенная = Истина;
...
Процедура АдаптироватьМКСписокФормыТовары()
	...
	ОписаниеКолонокПоля.ВажныеКолонки = "ТоварыГруппаСодержание,ТоварыНоменклатура,ТоварыСодержание";
	ОписаниеКолонокПоля.УстановитьВерхниеЗаголовки = "ТоварыНоменклатура,ТоварыСодержание";
