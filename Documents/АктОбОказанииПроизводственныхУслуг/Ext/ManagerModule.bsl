...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Документ, ТаблицаТовары.Ссылка) КАК Ссылка,
	...
	|		ПО ТаблицаТовары.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	И АктОбОказанииПроизводственныхУслуг.Дата > &УсловиеПоДате";
...
Функция СчетаФактурыНеТребуются(МассивДокументов) Экспорт
	...
	|			ИЛИ АктОбОказанииПроизводственныхУслуг.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|	ТаблицаУслуги.ПлановаяСтоимость КАК ПлановаяСтоимость,
	...
	|		ПО (&ПлательщикЕНВД)
	|			И ТаблицаУслуги.СчетДоходов = СчетаЕНВД.Счет
...
Функция ТекстЗапросаРеализация(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицыДокументаРеализации.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ИЗ
	|	Реквизиты КАК Реквизиты
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Строка Из СтрокиДокумента Цикл
			...
			Если ИспользоватьПериодичностьУслуг Тогда
				СтрокаТаблицыДокумента.НоменклатураНаименование = РаботаСНоменклатуройКлиентСерверБП.СодержаниеУслуги(
...
Функция ОпределитьПодписантов(СписокДокументов) Экспорт
	...
	|	Документ.АктОбОказанииПроизводственныхУслуг КАК АктОбОказанииПроизводственныхУслуг
	|ГДЕ
	|	АктОбОказанииПроизводственныхУслуг.Ссылка В(&Ссылки)";
