...
Функция ПроверитьВедениеУчетаПоДокументамРасчетов(СчетУчета)
	...
	Для Н = 1 По СвойстваСчета.КоличествоСубконто Цикл
		...
		Если НЕ СвойстваСчета["ВидСубконто" + Н + "ТолькоОбороты"]
		   И (СвойстваСчета["ВидСубконто" + Н] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами
		   Или СвойстваСчета["ВидСубконто" + Н] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СФПолученные) Тогда
...
Функция ПолучитьДанныеСпискаПоРеквизитам(СтруктураОтбора)
	...
	Если СтруктураОтбора.Свойство("ПометкаУдаления") Тогда
		УсловиеЗапросаПометкаУдаления = " И Док.ПометкаУдаления = "
	...
	Для Каждого ТипДокумента Из ТипыДокументов.Типы() Цикл
		...
		ТекстЗапроса = ТекстЗапроса +  УсловиеЗапросаПометкаУдаления;
	...
	Для Н = 1 По СвойстваСчета.КоличествоСубконто Цикл
		...
		Если НЕ СвойстваСчета["ВидСубконто" + Н + "ТолькоОбороты"] Тогда
			...
			ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами
				Или ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СФПолученные Тогда
...
Функция ПолучитьДанныеСпискаПоОборотам(СтруктураОтбора)
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		...
		ИначеЕсли БухгалтерскийУчет.НаСчетеВедетсяУчетСчетамФактурамПолученным(Счет) Тогда
			ИспользованоСубконто = ИспользованоСубконто + 1;
...
Процедура СформироватьСервер()
	...
	Если ПомеченныеНаУдалениеСервер.СкрыватьПомеченныеНаУдаление() Тогда
	...
	ИначеЕсли РежимОтбораДокументов = Перечисления.РежимОтбораДокументов.ПоОстаткам Тогда
		...
		СтруктураОтбора.Вставить("ОстаткиОбороты", ОстаткиОбороты);
		СтруктураОтбора.Вставить("ТипыДокументов", ТипыДокументов);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ОткрытьФорму("Документ.ДокументРасчетовСКонтрагентом.ФормаОбъекта", Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения), ЭтотОбъект);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыОбъекта") Тогда
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры.ПараметрыОбъекта);
...
Процедура МобильныйКлиентАдаптацияФормы()
	...
	Элементы.РежимОтбора.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.РежимОтбораДокументов);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.ФормироватьСписокПриОткрытии);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.Организация);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.Контрагент);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.ДоговорКонтрагента);
	Элементы.ГруппаОтборПоСчету.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.ГруппаОтборПоСчету);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКШапка(ЭтотОбъект, Элементы.Актуализация);
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	КомандыПоля.КомандаСоздать = "СписокДобавить";
	...
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКСоздать(ЭтотОбъект, Элементы.СписокДобавить);
	МобильныйКлиентБПФормаСписка.ПеренестиКомандыВМККомандыФормы(ЭтотОбъект, Элементы.Список.КоманднаяПанель.ПодчиненныеЭлементы);
	
	Элементы.Переместить(Элементы.СписокСформировать, Элементы.Список.КоманднаяПанель);
	Элементы.Список.КоманднаяПанель.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
...
Процедура АдаптироватьМККонтекстСписокФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы, Элементы.СписокВыбрать, Элементы.Список.КонтекстноеМеню);
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы, Элементы.СписокПереместитьВверх, Элементы.Список.КонтекстноеМеню);
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы, Элементы.СписокПереместитьВниз, Элементы.Список.КонтекстноеМеню);
...
Процедура АдаптироватьМКСписокФормы()
	...
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_ДатаНомер(ЭтотОбъект, Элементы.СписокНомер);
	
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_СуммаДокумента(ЭтотОбъект, Элементы.СписокСумма);
	
	Элементы.СписокВидДокумента.РастягиватьПоГоризонтали = Истина;
	Элементы.СписокВидДокумента.АвтоВысотаЯчейки = Истина;
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Важное(ЭтотОбъект, Элементы.СписокВидДокумента);
	
	Элементы.СписокВалюта.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.СписокВалюта.ОтображатьВШапке = Ложь;
	Элементы.СписокВалюта.РастягиватьПоГоризонтали = Ложь;
	Элементы.СписокВалюта.Ширина = 3;
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_СуммаДокумента(ЭтотОбъект, Элементы.СписокВалюта);
	...
	Элементы.СписокДатаВходящегоДокумента.ЦветТекста = ЦветаСтиля.ЦветТекстаСтрокаСпискаМК;
	Элементы.СписокДатаВходящегоДокумента.Формат = "ДФ='''Вх. дата/номер:'' dd.MM.yyyy'";
	Элементы.СписокНомерВходящегоДокумента.ЦветТекста = ЦветаСтиля.ЦветТекстаСтрокаСпискаМК;
	
	Элементы.Переместить(Элементы.СписокДатаВходящегоДокумента, ГруппаКолонокМК_ВходящиеДатаНомер);
	Элементы.Переместить(Элементы.СписокНомерВходящегоДокумента, ГруппаКолонокМК_ВходящиеДатаНомер);
	
	Элементы.СписокОстаток.Формат = "ЧФ='Остаток(сумма): Ч'";
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.СписокОстаток, 2);
