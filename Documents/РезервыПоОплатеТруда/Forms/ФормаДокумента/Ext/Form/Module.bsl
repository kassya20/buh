...
Процедура ЗаполнитьУволенными(Команда)
	...
	Если Объект.ОценочныеОбязательстваПоСотрудникам.Количество() > 0
		Или Объект.ОценочныеОбязательства.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект, Новый Структура("ИмяПоказателяДопОтбора, Период", "Уволен", КонецМесяца(Объект.МесяцНачисления)));
		ТекстВопроса = НСтр("ru = 'Табличные части документа будут очищены. Продолжить?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЗавершение(КодВозвратаДиалога.Да, Новый Структура("ИмяПоказателяДопОтбора, Период", "Уволен", КонецМесяца(Объект.МесяцНачисления)));
	КонецЕсли;
...
Процедура ЗаполнитьПереведенными(Команда)
	...
	Если Объект.ОценочныеОбязательстваПоСотрудникам.Количество() > 0
		Или Объект.ОценочныеОбязательства.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект, Новый Структура("ИмяПоказателяДопОтбора, Период", "Перемещен", КонецМесяца(Объект.МесяцНачисления)));
		ТекстВопроса = НСтр("ru = 'Табличные части документа будут очищены. Продолжить?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЗавершение(КодВозвратаДиалога.Да, Новый Структура("ИмяПоказателяДопОтбора, Период", "Перемещен", КонецМесяца(Объект.МесяцНачисления)));
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьЗачисленными(Команда)
	...
	Если Объект.ОценочныеОбязательстваПоСотрудникам.Количество() > 0
		Или Объект.ОценочныеОбязательства.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьЗавершение", ЭтотОбъект, Новый Структура("ИмяПоказателяДопОтбора, Период", "Зачислен", НачалоМесяца(Объект.МесяцНачисления)));
		ТекстВопроса = НСтр("ru = 'Табличные части документа будут очищены. Продолжить?'");
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьЗавершение(КодВозвратаДиалога.Да, Новый Структура("ИмяПоказателяДопОтбора, Период", "Зачислен", НачалоМесяца(Объект.МесяцНачисления)));
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_РасчетРезервовУдалитьСотрудника(Команда)
	
	ТекущиеДанные = Элементы.РасчетРезервов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда 
		УдалитьДанныеСотрудникаНаСервере(ТекущиеДанные.Сотрудник);
		Если РассчитыватьДокументыПриРедактировании Тогда
			ОбновитьОтражениеВУчетеНаКлиенте();
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ОценочныеОбязательстваПоСотрудникамУдалитьСотрудника(Команда)
...
Процедура УправлениеФормой();
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ИспользоватьОбособленныеПодразделения
			И (ЭтоСписание Или ЭтоКорректировка));
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ЗаполнитьЗачисленными",
		"Видимость",
		ИспользоватьОбособленныеПодразделения
			И ЭтоКорректировка);
...
Процедура ЗаполнитьЗавершение(Ответ, ДополнительныеПараметры) Экспорт 
	...
	ЗаполнитьДанныеФормыНаКлиенте(ДополнительныеПараметры);
...
Процедура ДанныеДляЗаполненияВДанныеФормы(ДанныеЗаполнения, Объект, ПозицииВставки = Неопределено, Сотрудники = Неопределено, ЗаполнятьПоказатели)
	...
	//Оценочные обязательства по сотрудникам
	РезервыПоОплатеТруда.ДополнитьТаблицуРасчетаКадровымСостоянием(ДанныеЗаполнения.ОценочныеОбязательстваПоСотрудникам, Объект.Организация, Объект.МесяцНачисления);
	РезервыПоОплатеТрудаФормы.ОценочныеОбязательстваПоСотрудникамВДанныеФормы(Объект.ОценочныеОбязательстваПоСотрудникам,
		ДанныеЗаполнения.ОценочныеОбязательстваПоСотрудникам,
		ПозицииВставки);
	
	//Оценочные обязательства
	НастройкаРезерва = РезервыПоОплатеТруда.НастройкиРезерва(Объект.Организация, Объект.Резерв, Объект.МесяцНачисления);
	...
	РезервыПоОплатеТрудаФормы.ОценочныеОбязательстваВДанныеФормы(Объект.ОценочныеОбязательства,
		...
		Не Дополнять, ?(Дополнять, РаспределятьВыявленныеИзлишки, Ложь));
...
Функция СодержимоеДокументаПослеРасчета(АдресХранилища, ОписаниеТаблиц, ЗаполнятьПоказатели)
	...
	СодержимоеДокумента.Вставить("Резерв",          СтруктураПараметров.Резерв);
...
Процедура ДанныеВРеквизиты(ТекущийОбъект)
	...
	РезервыПоОплатеТруда.ДополнитьТаблицуРасчетаКадровымСостоянием(Объект.ОценочныеОбязательстваПоСотрудникам, Объект.Организация, Объект.МесяцНачисления);
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ПараметрыФО = Новый Структура("Организация",
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Организация, "ГоловнаяОрганизация"));
	ИспользоватьОбособленныеПодразделения =
		ПолучитьФункциональнуюОпцию("ИспользоватьОбособленныеПодразделения", ПараметрыФО);
...
Функция СформироватьОписаниеДокумента()
	...
	ОписаниеДокумента = Новый Структура("Ссылка,
		|Организация, Резерв, ВидОперации, Дата, МесяцНачисления,
...
Процедура ЗагрузитьРезультат()
	...
	Если Результат.Свойство("Объект") Тогда
		...
		Объект.РасчетРезервов.Загрузить(РезультатОбъект.РасчетРезервов);
		Объект.ОценочныеОбязательства.Загрузить(РезультатОбъект.ОценочныеОбязательства);
		Объект.ОценочныеОбязательстваПоСотрудникам.Загрузить(РезультатОбъект.ОценочныеОбязательстваПоСотрудникам);
		Объект.ФизическиеЛица.Загрузить(РезультатОбъект.ФизическиеЛица);
