...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьНомераИСуммыСчетаФактуры");
	Методы.Вставить("УстановитьКодВидаОперацииСводныйКомиссионный");
	Методы.Вставить("ПереопределитьПризнакСводныйКомиссионный");
	Методы.Вставить("УстановитьВидОперацииНаАвансКомитентуНаЗакупку");
...
// Проверяет есть ли в измененных пользователем макетах счета-фактуры, УПД, УКД до изменений по постановлению 534
// Если имеются измененные макеты, то пользователя необходимо
// предупредить о необходимости перенести свои изменения в новый макет.
//
// Возвращаемое значение:
//  Булево - признак того, что есть измененные макеты
//           Истина - есть измененные макеты
//           Ложь   - нет измененных макетов

Функция ТребуетсяПеренестиИзмененияВМакеты534() Экспорт
	...
	|ГДЕ
	|	(ПользовательскиеМакетыПечати.Объект = ""ОбщийМакет""
	|				И (ПользовательскиеМакетыПечати.ИмяМакета = ""ПФ_MXL_СчетФактура981""
	|					ИЛИ ПользовательскиеМакетыПечати.ИмяМакета = ""ПФ_MXL_КорректировочныйСчетФактура981""
	|					ИЛИ ПользовательскиеМакетыПечати.ИмяМакета = ""ПФ_MXL_КорректировочныйСчетФактура981Аванс"")
	|			ИЛИ ПользовательскиеМакетыПечати.Объект = ""Обработка.ПечатьУКД""
	|				И ПользовательскиеМакетыПечати.ИмяМакета = ""ПФ_MXL_УниверсальныйКорректировочныйДокумент981""
	|			ИЛИ ПользовательскиеМакетыПечати.Объект = ""Обработка.ПечатьУПД""
	|				И ПользовательскиеМакетыПечати.ИмяМакета = ""ПФ_MXL_УниверсальныйПередаточныйДокумент981"")";
...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
    ...
    |    И ЧтениеСпискаРазрешено(ДокументыОснования.ДокументОснование)
    |;
    |РазрешитьИзменениеЕслиРазрешеноЧтение
    |ГДЕ
    |    ЗначениеРазрешено(Организация)
    |    И ИзменениеСпискаРазрешено(ДокументыОснования.ДокументОснование)";
...
//
// Параметры:
//   ДокументСсылка - ДокументСсылка.СчетФактураВыданный - документ
//   ВидСчетаФактуры - ПеречислениеСсылка.ВидСчетаФактурыВыставленного - вид документа
//
// Возвращаемое значение:
//   Структура - структура представления:
//    * СчетФактураКраткоеПредставление - Строка - содержит номер и дату
//    * СчетФактураПредставление        - Строка - содержит название документа, номер и дату
Функция ПолучитьПредставлениеДокумента(ДокументСсылка, ВидСчетаФактуры) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументСсылка) И ОбщегоНазначения.СсылкаСуществует(ДокументСсылка) Тогда
		РеквизитыСчетаФактуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
			"Дата, Номер, Исправление, НомерИсправления, НомерИсправляемогоКорректировочногоДокумента, НомерВходящегоДокумента, ОтредактированНомерСчетаФактуры,
...
// Определяет, является ли счет-фактура универсальным передаточным документом
// 
// Параметры:
//  ДокументСсылка - ДокументСсылка.СчетФактураВыданный - документ для которого осуществляется проверка
// Возвращаемое значение:
//  Булево - Истина, если это УПД.
Функция ЭтоУниверсальныйДокумент(ДокументСсылка) Экспорт
	...
	|		ПО ДокументыОснования.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И ДокументыОснования.ДокументОснование = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	|	ДокументыОснования.Ссылка = &ТекущийДокумент
	|	И ДанныеПервичныхДокументов.ЭтоУниверсальныйДокумент";
	...
	Если Не Результат.Пустой() Тогда
		ЭтоУниверсальныйДокумент = Истина;
	КонецЕсли;
	
	Возврат ЭтоУниверсальныйДокумент;

КонецФункции

Функция ОснованияСчетовФактур(СчетаФактуры) Экспорт
	...
	ОснованияСчетовФактур = Новый Соответствие;
	
	ВыборкаСчетовФактур = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаСчетовФактур.Следующий() Цикл
		ОснованияСчетаФактуры = Новый Массив;
		Выборка = ВыборкаСчетовФактур.Выбрать();
		Пока Выборка.Следующий() Цикл
			ОснованияСчетаФактуры.Добавить(Выборка.ДокументОснование);
		КонецЦикла;
		ОснованияСчетовФактур.Вставить(ВыборкаСчетовФактур.Ссылка, ОснованияСчетаФактуры);
	КонецЦикла;
	
	Возврат ОснованияСчетовФактур;
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаСписка"
		ИЛИ ВидФормы = "ФормаВыбора"
		ИЛИ ВидФормы = "ФормаДанныхВерсииИсторииДанных"
		ИЛИ ВидФормы = "ФормаИсторииИзмененийИсторииДанных"
		ИЛИ ВидФормы = "ФормаРазличийВерсийИсторииДанных" Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Параметры.Свойство("Основание") 
		...
		ВидСчетаФактуры = 
			УчетНДСПереопределяемый.ОпределитьВидСчетаФактурыВыданногоПоТипуОснования(Параметры.Основание, Истина);
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.СводнаяСправка Тогда
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Данные.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.КорректировочнаяСправка Тогда
		СтандартнаяОбработка = Ложь;
		Представление = СтрШаблон(НСтр("ru = '%1 %2 от %3'"),
			"Корр. сводная справка", 
			Данные.ПредставлениеНомера,
			Данные.Дата);
...
Процедура ДобавитьКомандыОтправки(КомандыОтправки) Экспорт
	...
	Если ПравоДоступа("Использование", Метаданные.Обработки.ПечатьУКД) Тогда
		...
		КомандаОтправки.Обработчик    = "ОтправитьУКД";
	КонецЕсли;
...
Функция СоздатьДокументНаОсновании(ПараметрыСоздания) Экспорт
	...
	Если ЭтоУниверсальныйДокумент Тогда
		СчетФактура.КодВидаОперации = КодВидаОперации;
	КонецЕсли;
...
Функция ПолучитьПредставлениеПоРеквизитам(РеквизитыСчетаФактуры = Неопределено, ТекстОсновноеПредставление = "") Экспорт
	...
	Иначе
		...
		ИначеЕсли РеквизитыСчетаФактуры.ОтредактированНомерСчетаФактуры Тогда
			ТекстПредставлениеКраткое = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),
				РеквизитыСчетаФактуры.НомерВходящегоДокумента, Формат(РеквизитыСчетаФактуры.Дата,"ДЛФ=Д"));
			ТекстПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 %2'"), ТекстОсновноеПредставление, ТекстПредставлениеКраткое);
		Иначе
			ТекстПредставлениеКраткое = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),
...
Функция ДатыНомераПлатежноРасчетныхДокументов(МассивОснований, Контрагент = Неопределено, ДоговорКонтрагента = Неопределено) Экспорт
	...
	ДатыНомера.Колонки.Добавить("НомерДокумента", Метаданные.РегистрыСведений.ДанныеПервичныхДокументов.Ресурсы.Номер.Тип);
	ДатыНомера.Колонки.Добавить("ДатаДокумента",  Метаданные.РегистрыСведений.ДанныеПервичныхДокументов.Ресурсы.Дата.Тип);
	ДатыНомера.Колонки.Добавить("НомерСчетаФактурыНаАванс",  Метаданные.РегистрыСведений.ЖурналУчетаСчетовФактур.Ресурсы.НомерСчетаФактуры.Тип);
	ДатыНомера.Колонки.Добавить("ДатаСчетаФактурыНаАванс",  Метаданные.РегистрыСведений.ЖурналУчетаСчетовФактур.Ресурсы.ДатаСчетаФактуры.Тип);
	ДатыНомера.Колонки.Добавить("НомерИсправленияСчетаФактурыНаАванс",  Метаданные.РегистрыСведений.ЖурналУчетаСчетовФактур.Ресурсы.НомерИсправления.Тип);
	ДатыНомера.Колонки.Добавить("ДатаИсправленияСчетаФактурыНаАванс",  Метаданные.РегистрыСведений.ЖурналУчетаСчетовФактур.Ресурсы.ДатаИсправления.Тип);

	УникальныеОснования = ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(МассивОснований, Истина);
	
	УстановитьПривилегированныйРежим(Истина);
	РеквизитыОснований = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(УникальныеОснования, "Дата");
	УстановитьПривилегированныйРежим(Ложь);
	
	МинДата  = '2999-12-31';
	МаксДата = '0001-01-01';
	Для каждого КлючИЗначение Из РеквизитыОснований Цикл
		ДатаДокументаОснования = КлючИЗначение.Значение;
		Если ЗначениеЗаполнено(ДатаДокументаОснования) Тогда
			МинДата  = Мин(МинДата, ДатаДокументаОснования);
			МаксДата = Макс(МаксДата, ДатаДокументаОснования);
		КонецЕсли;
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("МассивОснований", УникальныеОснования);
	...
	Запрос.УстановитьПараметр("МассивСчетовАвансов", БухгалтерскийУчет.СформироватьМассивСубсчетов(МассивСчетовАвансов));
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫБОР
	|		КОГДА ХозрасчетныйДвиженияССубконто.СубконтоДт3 ССЫЛКА Документ.ПоступлениеНаРасчетныйСчет
	|				ИЛИ ХозрасчетныйДвиженияССубконто.СубконтоДт3 ССЫЛКА Документ.ПриходныйКассовыйОрдер
	...
	|		ПО ХозрасчетныйДвиженияССубконто.Организация = СчетФактураДокументыОснования.Ссылка.Организация
	|			И ХозрасчетныйДвиженияССубконто.СубконтоДт3 = СчетФактураДокументыОснования.ДокументОснование
	|			И ХозрасчетныйДвиженияССубконто.Период >= СчетФактураДокументыОснования.Ссылка.Дата
	|			И (СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаАванс))
	...
	Если Контрагент <> Неопределено И ДоговорКонтрагента <> Неопределено Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоКонтрагентуИДоговору", 
			"СубконтоДт1 = &Контрагент И СубконтоДт2 = &ДоговорКонтрагента ");
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ДатыНомера.Добавить(), Выборка);
	КонецЦикла;
...
Функция НоменклатураСчетаНаОплату(СчетНаОплату) Экспорт
	...
	Для каждого СтрокаНоменклатуры Из НоменклатураСчета Цикл
		...
		СтрокаНоменклатуры.СтавкаНДС = Перечисления.СтавкиНДС.РасчетнаяСтавкаНДСПоОбычной(СтрокаНоменклатуры.СтавкаНДС);
	КонецЦикла;
...
Функция РаспределитьСуммыАвансовПоНоменклатуреСчетовНаОплату(АвансыПоСчетамНаОплату, НоменклатураСчетовНаОплату, ВалютаРеглУчета) Экспорт
	...
	Для каждого СтрокаАванса Из АвансыПоСчетамНаОплату Цикл
		...
		Если ЗначениеЗаполнено(СтрокаАванса.СчетНаОплату) Тогда
			...
			// Таблица АвансыПоСчетамНаОплату содержит только строки с расчетными ставками - например, 20/120, 18/118, 10/110
			// Добавляем строки с "обычными" ставками (например, 20%, 18%, 10%)
			ОтборСтрокНоменклатуры.СтавкаНДС = Перечисления.СтавкиНДС.ОбычнаяСтавкаНДСПоРасчетной(СтрокаАванса.СтавкаНДС);
			...
			Если СтрокаАванса.ВалютаСчетаНаОплату <> ВалютаРеглУчета Тогда
				...
				Для каждого СтрокаНоменклатуры Из НоменклатураСчетаНаОплату Цикл
					СтрокаНоменклатуры.Сумма = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаНоменклатуры.Сумма,
...
Функция ТоварыУслугиЗакупленныеКомитентам(СтруктураПараметров) Экспорт
	...
	Для Индекс = 0 По Результат.Количество() - 1 Цикл
		...
		Результат[Индекс].СтавкаНДС = Перечисления.СтавкиНДС.РасчетнаяСтавкаНДСПоОбычной(Результат[Индекс].СтавкаНДС);
...
Функция ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировки(
	...
	Если СчетФактура.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.Корректировочный 
		ИЛИ НЕ ЭтоКорректировочныйДокумент(ДокументОснование) Тогда
	...
	Иначе
		...
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			...
			Если СчетФактура.Исправление Тогда 
				ИсходныйДокументРеализации = УчетНДСПереопределяемый.ПолучитьИсправляемыйДокументРеализации(
					ДокументОснование, ПолучитьИсходный);
					
				Если ТипЗнч(ИсходныйДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					ИсходныйДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
						ИсходныйДокументРеализации, "ДокументРеализации");
				КонецЕсли; 
				
			Иначе
				ИсходныйДокументРеализации = ?(ЗначениеЗаполнено(ИсходныйДокумент), ИсходныйДокумент, ДокументРеализации);
			КонецЕсли;
		...
		РеквизитыИсходногоСчетаФактуры = ПолучитьРеквизитыСчетаФактурыВыданного(
			ИсходныйДокументРеализации, ПолучатьПредставление);
...
Функция ПолучитьРеквизитыСчетаФактурыВыданного(ДокументОснование, ПолучатьПредставление = Ложь, ВидСчетаФактуры = Неопределено) Экспорт
	...
	|		КОГДА СчетФактураВыданный.ОтредактированНомерСчетаФактуры
	|			ТОГДА СчетФактураВыданный.НомерВходящегоДокумента
...
Функция ПодготовитьПараметрыЗаполненияАванс(ДокументОснование, ДокументСсылка, ДоговорКонтрагента = Неопределено, Отказ) Экспорт
	...
	РасчетнаяСтавкаНДСПоУмолчанию = УчетНДСКлиентСервер.РасчетнаяСтавкаНДСПоАвансам(
		РеквизитыОснования.Организация, РеквизитыОснования.Дата);
	
	СчетаУчетаАвансовПриОтгрузке = Новый Массив;
	СчетаУчетаАвансовПриОтгрузке.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиРуб);
	СчетаУчетаАвансовПриОтгрузке.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиВал);
	СчетаУчетаАвансовПриОтгрузке.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВСчетОтгрузкиУЕ);
	...
	Запрос.УстановитьПараметр("СчетаУчетаАвансовПриОтгрузке", СчетаУчетаАвансовПриОтгрузке);
	
	СредиДоговоровОплатыЕстьГосКонтракты = ЕстьОплатаГосКонтрактов(ДокументОснование);
	...
	Если СредиДоговоровОплатыЕстьГосКонтракты Тогда
		Запрос.Текст = Запрос.Текст +ТекстЗапросаДанныеПлатежаПоДоговорам(НомераТаблиц);
	Иначе
		Запрос.Текст = Запрос.Текст + ТекстЗапросаДанныеПлатежа(НомераТаблиц);
	КонецЕсли;
	Запрос.Текст = Запрос.Текст + ТекстЗапросаНоменклатураСчетовНаОплату(НомераТаблиц);
	...
	Для каждого СтрокаБезНДС Из АвансыБезНДС Цикл
		Если СтрокаБезНДС.НДСИсчисляетсяНалоговымАгентом Тогда
			СтрокаБезНДС.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСИсчисляетсяНалоговымАгентом(СтрокаБезНДС.Дата, СтрокаБезНДС.Организация);
		Иначе
	...
	Иначе
		ГосКонтракт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыОплаты.ДоговорКонтрагента, "ГосударственныйКонтракт");
		Если СредиДоговоровОплатыЕстьГосКонтракты ИЛИ ЗначениеЗаполнено(ГосКонтракт) Тогда
			ЗаполнитьЗначенияСвойств(Реквизиты, РеквизитыОплаты);
		Иначе
...
Функция ТекстЗапросаСуммыАвансовПоВзаиморасчетам(НомераТаблиц)
	...
	|			,
	|			ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(&ДокументОснование) = ТИП(Документ.ДокументРасчетовСКонтрагентом)
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ Регистратор = &ДокументОснование
	|				КОНЕЦ
	...
	|					ИНАЧЕ Регистратор = &ДокументОснование
	|				КОНЕЦ
	|				И СчетКт В (&СчетаУчетаАвансовПриОтгрузке)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыАвансовПоДокументу.Контрагент КАК Контрагент,
	|	СуммыАвансовПоДокументу.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	СуммыАвансовПоДокументу.СчетАвансов КАК СчетАвансов,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыАвансовПредварительная.Контрагент КАК Контрагент,
	|	СуммыАвансовПредварительная.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	СуммыАвансовПредварительная.СчетАвансов КАК СчетАвансов,
	|	СуммыАвансовПредварительная.Сумма КАК Сумма
...
Функция ТекстЗапросаДанныеПлатежа(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	...
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
...
Функция ПодготовитьПараметрыЗаполненияНалоговыйАгент(ДокументОснование, ДокументСсылка, ДоговорКонтрагента = Неопределено, Отказ) Экспорт
	...
	Запрос.Текст = Документы[ДокументОснование.Метаданные().Имя].ТекстЗапросаСчетФактураВыданныйНалоговыйАгентРасшифровкаПлатежа(НомераТаблиц)
		+ ТекстЗапросаРеквизитыИСуммыОплатПоВзаиморасчетам(НомераТаблиц);
	Результат = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	СуммыОплатПредварительная = Результат[НомераТаблиц.ВТ_СуммыОплатПредварительная].Выгрузить();
	
	Если СуммыОплатПредварительная.Количество() = 0 Тогда
		Отказ = Истина;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ТекстЗапросаРеквизитыИСуммыОплатПоВзаиморасчетам(НомераТаблиц)
	
	// Временная таблица договоров из ВТ РасшифровкаПлатежа 
	НомераТаблиц.Вставить("ВТ_ДоговорыПлатежа", НомераТаблиц.Количество());

	// Расшифровка платежа документа. Данные для заполнения реквизитов
	НомераТаблиц.Вставить("РасшифровкаПлатежа", НомераТаблиц.Количество());
	
	// Предварительная таблица оплат из проводок оплат документа основания или по договору,
	//	если документ основание - Документ расчетов с контрагентом
	НомераТаблиц.Вставить("ВТ_СуммыОплатПредварительная", НомераТаблиц.Количество());
	
	// Временная таблица оплат по документам, на которые не были зарегистрированны счета-фактуры
	НомераТаблиц.Вставить("ВТ_СуммыОплатБезСчетовФактур", НомераТаблиц.Количество());
	
	// Суммы оплат из ВТ СуммыОплат. Данные для заполнения ТЧ Авансы
	...
	|	РасшифровкаПлатежа.Номенклатура КАК Номенклатура,
	|	ТИПЗНАЧЕНИЯ(&ДокументОснование) = ТИП(Документ.ДокументРасчетовСКонтрагентом) КАК ОснованиеДокументРасчетовСКонтрагентом
	|ИЗ
	|	РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|ГДЕ
	|	НЕ РасшифровкаПлатежа.ДоговорКонтрагента.ВидАгентскогоДоговора В (&ВидыДоговоровРеализацииТоваров)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА Проводки.СубконтоДт3 = НЕОПРЕДЕЛЕНО
	...
	|ПОМЕСТИТЬ ВТ_СуммыОплатПредварительная
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(
	|			,
	|			,
	|			ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(&ДокументОснование) = ТИП(Документ.ДокументРасчетовСКонтрагентом)
	|						ТОГДА СчетКт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Касса))
	|								ИЛИ СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетныеСчета)
	|								ИЛИ СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВалютныеСчета)
	|								ИЛИ СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЦифровойРубль)
	|								ИЛИ СчетКт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СпециальныеСчета))
	|					ИНАЧЕ Регистратор = &ДокументОснование
	|				КОНЕЦ
	|				И ВидСубконтоДт1 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты)
	|				И ВидСубконтоДт2 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры)
	...
	|				И ВЫРАЗИТЬ(СубконтоДт2 КАК Справочник.ДоговорыКонтрагентов) В
	|					(ВЫБРАТЬ
	|						ВТ_ДоговорыПлатежа.ДоговорКонтрагента
	|					ИЗ
	|						ВТ_ДоговорыПлатежа КАК ВТ_ДоговорыПлатежа)
	...
	|	СуммыОплатПредварительная.ДокументРасчетов КАК ДокументРасчетов,
	|	СуммыОплатПредварительная.Сумма КАК Сумма,
	|	СуммыОплатПредварительная.Организация КАК Организация,
	|	СуммыОплатПредварительная.Регистратор КАК Регистратор,
	|	СуммыОплатПредварительная.СчетДт КАК СчетДт,
	|	СуммыОплатПредварительная.СчетКт КАК СчетКт
	|ПОМЕСТИТЬ ВТ_СуммыОплатБезСчетовФактур
	|ИЗ
	|	ВТ_СуммыОплатПредварительная КАК СуммыОплатПредварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ПО (НЕ СчетФактураВыданный.ПометкаУдаления)
	|			И (НЕ СчетФактураВыданный.Ссылка = &ДокументСсылка)
	|			И СуммыОплатПредварительная.Регистратор = СчетФактураВыданный.ДокументОснование
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыОплатБезСчетовФактур.Организация КАК Организация,
	|	СуммыОплатБезСчетовФактур.Контрагент КАК Контрагент,
	|	СуммыОплатБезСчетовФактур.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1)) КАК ДокументРасчетовДата,
	|	СУММА(СуммыОплатБезСчетовФактур.Сумма) КАК Сумма
	|ИЗ
	|	ВТ_СуммыОплатБезСчетовФактур КАК СуммыОплатБезСчетовФактур
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДанныеДоговораКонтрагента
	|		ПО СуммыОплатБезСчетовФактур.ДоговорКонтрагента = ДанныеДоговораКонтрагента.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО СуммыОплатБезСчетовФактур.Организация = ДанныеПервичныхДокументов.Организация
	|			И СуммыОплатБезСчетовФактур.ДокументРасчетов = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	|	ДанныеДоговораКонтрагента.ВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПоставщиком)
	|
	|СГРУППИРОВАТЬ ПО
	|	СуммыОплатБезСчетовФактур.Контрагент,
	|	СуммыОплатБезСчетовФактур.ДоговорКонтрагента,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1)),
	|	СуммыОплатБезСчетовФактур.Организация";
...
Функция ТекстЗапросаРеквизитыСчетаФактурыИсходногоДокумента(НомераТаблиц)
	...
	|	ВЫБОР
	|		КОГДА СчетФактураВыданныйДокументыОснования.Ссылка.ОтредактированНомерСчетаФактуры
	|			ТОГДА СчетФактураВыданныйДокументыОснования.Ссылка.НомерВходящегоДокумента
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура") Тогда
		ТабличныйДокумент = УчетНДСБП.ПечатьСчетовФактур(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура"]),
			ПараметрыПечати,
			ПараметрыВывода);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура1137") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур1137(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура1137"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура981") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур981(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура981"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура534"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			...
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура534");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура534Прослеж"], Ложь, Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактура534Прослеж",
			"Счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура534Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2023Кв4") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2023кв4(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2023Кв4"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2023Кв4Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2023кв4Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2023Кв4Прослеж"], Ложь, Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактура2023Кв4Прослеж",
			"Счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура2023Кв4Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура1096") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур1096(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура1096"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			...
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура1096");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура1096Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур1096Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура1096Прослеж"], Ложь, Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактура1096Прослеж",
			"Счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура1096Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2026") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2026(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2026"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			...
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура2026");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2026Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2026Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2026Прослеж"], Ложь, Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактура2026Прослеж",
			"Счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура2026Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура26") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур26(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура26"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			...
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура26");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура26Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур26Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура26Прослеж"], Ложь, Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"СчетФактура26Прослеж",
			"Счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_СчетФактура26Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура") Тогда
		ТабличныйДокумент = УчетНДСБП.ПечатьКорректировочныхСчетовФактур(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура"]), 
			ПараметрыПечати,
			ПараметрыВывода);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура1137") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур1137(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура1137"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура1137",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура952") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур952(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура952"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура952",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура981") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур981(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура981"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура981",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура981Аванс") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур981Аванс(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура981Аванс"]), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура981Аванс",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534"]), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура534",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура534");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534Прослеж"],Ложь,Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура534Прослеж",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура534Прослеж");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2023Кв4") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2023Кв4(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2023Кв4"]), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура2023Кв4",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2023Кв4");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2023Кв4Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2023Кв4Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2023Кв4Прослеж"],Ложь,Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура2023Кв4Прослеж",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2023Кв4Прослеж");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура1096") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур1096(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура1096"]), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура1096",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура1096");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура1096Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур1096Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура1096Прослеж"], Ложь, Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура1096Прослеж",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура1096Прослеж");
	КонецЕсли; 
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2026") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2026(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2026"]), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура2026",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2026");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2026Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2026Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2026Прослеж"], Ложь, Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура2026Прослеж",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура2026Прослеж");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура26") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур26(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура26"]),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура26",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура26");
	КонецЕсли;
		
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура26Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур26Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура26Прослеж"], Ложь, Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КорректировочныйСчетФактура26Прослеж",
			"Корректировочный счет-фактура",
			ТабличныйДокумент,
			,
			"ОбщийМакет.ПФ_MXL_КорректировочныйСчетФактура26Прослеж");
	КонецЕсли;
...
Функция ТекстЗапросаПечатьСчетовФактур(ВерсияПостановленияНДС1137 = Неопределено, ДляУПД = Ложь, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|		КОГДА СчетФактура.ОтредактированНомерСчетаФактуры
	|			ТОГДА СчетФактура.НомерВходящегоДокумента
	...
	|	СчетФактура.ОтредактированыДокументыОтгрузки КАК ОтредактированыДокументыОтгрузки
	|ИЗ
	...
	Если НЕ ДляУПД Тогда
		...
		|	СчетФактура.Исправление КАК Исправление,
		|	СчетФактура.НомерИсправления КАК НомерИсправления,
		|	СчетФактура.Дата КАК Дата1,
		|	СчетФактура.Дата КАК Дата2,
		|	ИСТИНА КАК Поле2,
		|	ИСТИНА КАК Поле3,
		|	ЛОЖЬ КАК Поле4,
		|	СчетФактура.Ссылка КАК Ссылка1,
		|	1 КАК Поле5,
		|	ВЫБОР
		|		КОГДА СчетФактура.Контрагент.ОбособленноеПодразделение
		|				И СчетФактура.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
		...
		|	ЛОЖЬ КАК Поле9,
		|	СчетФактура.СводныйКомиссионный КАК СводныйКомиссионный,
		|	ЕСТЬNULL(СчетФактура.ДоговорКонтрагента.СчетаФактурыОтИмениОрганизации, ЛОЖЬ) КАК Поле10,
		|	СчетФактура.ОтредактированыДокументыОтгрузки
		|ИЗ
		|	Документ.СчетФактураВыданный КАК СчетФактура
		|ГДЕ
		|	СчетФактура.Ссылка В(&МассивОбъектов)
	...
	"ВЫБРАТЬ
	|	СчетФактураВыданныйДокументыОбОтгрузке.Ссылка КАК СчетФактура,
	|	СчетФактураВыданныйДокументыОбОтгрузке.НомераСтрок КАК НомераСтрок,
	...
	|ГДЕ
	|	СчетФактураВыданныйДокументыОбОтгрузке.Ссылка В(&МассивОбъектов)
	|	И СчетФактураВыданныйДокументыОбОтгрузке.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаРеализацию)";
	
	УчетНДС.ДополнитьТекстЗапросаУсловиемПоДатеПримененияПостановленияНДС1137(ВерсияПостановленияНДС1137, ТекстЗапроса);
	...
	ИначеЕсли ЕстьПрослеживаемыеТовары Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЕстьПрослеживаемыеТовары", "СчетФактура.ЕстьПрослеживаемыеТовары = ИСТИНА");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЕстьПрослеживаемыеТовары", "СчетФактура.ЕстьПрослеживаемыеТовары = ЛОЖЬ");
	КонецЕсли;
...
Функция ТекстЗапросаПечатьКорректировочныхСчетовФактур(ВерсияПостановленияНДС1137 = Неопределено, ДляУКД = Ложь, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	СчетФактураДокументыОснования.Ссылка КАК СчетФактура,
	|	СчетФактураДокументыОснования.Ссылка.Дата КАК Дата,  
	|	ВЫБОР
	|		КОГДА СчетФактураДокументыОснования.Ссылка.ОтредактированНомерСчетаФактуры
	|			ТОГДА СчетФактураДокументыОснования.Ссылка.НомерВходящегоДокумента
	|		ИНАЧЕ СчетФактураДокументыОснования.Ссылка.Номер
	|	КОНЕЦ КАК Номер,
	|	СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры КАК ВидСчетаФактуры,
	|	СчетФактураДокументыОснования.Ссылка.Исправление КАК Исправление,
	|	СчетФактураДокументыОснования.Ссылка.НомерИсправления КАК НомерИсправления,
	|	СчетФактураДокументыОснования.Ссылка.Дата КАК ДатаИсправления,
	|	СчетФактураДокументыОснования.УчитыватьИсправлениеИсходногоДокумента КАК УчитыватьИсправлениеИсходногоДокумента,
	|	СчетФактураДокументыОснования.Ссылка.НомерИсправляемогоКорректировочногоДокумента КАК НомерИсправляемогоКорректировочногоДокумента,
	|	СчетФактураДокументыОснования.Ссылка.ДатаИсправляемогоКорректировочногоДокумента КАК ДатаИсправляемогоКорректировочногоДокумента,
	|	СчетФактураДокументыОснования.Ссылка.Дата КАК ДатаСведений,
	|	СчетФактураДокументыОснования.Ссылка.СчетФактураБезНДС КАК СчетФактураБезНДС,
	|	ВЫБОР
	|		КОГДА СчетФактураДокументыОснования.Ссылка.Контрагент.ОбособленноеПодразделение
	|				И СчетФактураДокументыОснования.Ссылка.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|			ТОГДА СчетФактураДокументыОснования.Ссылка.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ СчетФактураДокументыОснования.Ссылка.Контрагент
	|	КОНЕЦ КАК Контрагент,
	|	СчетФактураДокументыОснования.Ссылка.КППКонтрагента КАК КППСчетаФактуры,
	...
	|	СчетФактураДокументыОснования.Ссылка.Руководитель КАК Руководитель,
	|	СчетФактураДокументыОснования.Ссылка.ГлавныйБухгалтер КАК ГлавныйБухгалтер,
	|	СчетФактураДокументыОснования.Ссылка.ИдентификаторГосКонтракта КАК ИдентификаторГосКонтракта,
	|	СчетФактураДокументыОснования.ИсходныйДокумент КАК ИсходныйДокумент,
	|	СчетФактураВыданный.ЕстьПрослеживаемыеТовары КАК ЕстьПрослеживаемыеТовары
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураДокументыОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ПО СчетФактураДокументыОснования.Ссылка = СчетФактураВыданный.Ссылка
	|ГДЕ
	|	СчетФактураДокументыОснования.Ссылка В(&МассивОбъектов)
	|	И СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.Корректировочный)
	|	И ВЫБОР
	|			КОГДА СчетФактураДокументыОснования.Ссылка.Исправление
	|				ТОГДА СчетФактураДокументыОснования.Ссылка.ДатаИсправляемогоКорректировочногоДокумента > &УсловиеПоДате
	|			ИНАЧЕ СчетФактураДокументыОснования.Ссылка.Дата > &УсловиеПоДате
	|		КОНЕЦ
	|	И &ЕстьПрослеживаемыеТовары
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|		ИНАЧЕ СчетФактураДокументыОснования.Ссылка.Номер
	|	КОНЕЦ,
	|	СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры,
	|	СчетФактураДокументыОснования.Ссылка.Исправление,
	|	СчетФактураДокументыОснования.Ссылка.НомерИсправления,
	|	СчетФактураДокументыОснования.Ссылка.Дата,
	|	ЛОЖЬ,
	|	"""""""",
	|	ДАТАВРЕМЯ(1, 1, 1),
	|	СчетФактураДокументыОснования.Ссылка.Дата,
	|	СчетФактураДокументыОснования.Ссылка.СчетФактураБезНДС,
	|	ВЫБОР
	|		КОГДА СчетФактураДокументыОснования.Ссылка.Контрагент.ОбособленноеПодразделение
	|				И СчетФактураДокументыОснования.Ссылка.Контрагент.ГоловнойКонтрагент.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|			ТОГДА СчетФактураДокументыОснования.Ссылка.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ СчетФактураДокументыОснования.Ссылка.Контрагент
	|	КОНЕЦ,
	|	СчетФактураДокументыОснования.Ссылка.КППКонтрагента,
	|	ИСТИНА,
	|	ИСТИНА,
	|	СчетФактураДокументыОснования.Ссылка,
	|	СчетФактураДокументыОснования.НомерСтроки,
	|	ИсходныйСчетФактураРеквизиты.НомерСчетаФактуры,
	|	ИсходныйСчетФактураРеквизиты.ДатаСчетаФактуры,
	|	"""""""",
	|	ДАТАВРЕМЯ(1, 1, 1),
	|	СчетФактураДокументыОснования.Ссылка.Руководитель,
	|	СчетФактураДокументыОснования.Ссылка.ГлавныйБухгалтер,
	|	СчетФактураДокументыОснования.Ссылка.ИдентификаторГосКонтракта,
	|	НЕОПРЕДЕЛЕНО,
	|	СчетФактураВыданный.ЕстьПрослеживаемыеТовары
	|ИЗ
	|	Документ.СчетФактураВыданный.Авансы КАК СчетФактураДокументыОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЖурналУчетаСчетовФактур КАК ИсходныйСчетФактураРеквизиты
	|		ПО СчетФактураДокументыОснования.КорректируемыйСчетФактура = ИсходныйСчетФактураРеквизиты.СчетФактура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ПО СчетФактураДокументыОснования.Ссылка = СчетФактураВыданный.Ссылка
	|ГДЕ
	|	СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.КорректировочныйНаАванс)
	|	И СчетФактураДокументыОснования.Ссылка В(&МассивОбъектов)
	|	И &ЕстьПрослеживаемыеТовары
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	...
	|ГДЕ
	|	СчетФактураВыданныйКорректируемыеНомераСтрок.Ссылка В(&МассивОбъектов)
	...
	Если ВерсияПостановленияНДС1137 = Неопределено Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЕстьПрослеживаемыеТовары", "ИСТИНА");
	ИначеЕсли ЕстьПрослеживаемыеТовары Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЕстьПрослеживаемыеТовары", "СчетФактураВыданный.ЕстьПрослеживаемыеТовары = ИСТИНА");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЕстьПрослеживаемыеТовары", "СчетФактураВыданный.ЕстьПрослеживаемыеТовары = ЛОЖЬ");
	КонецЕсли;
...
Функция ТекстЗапросаДанныеДляПечатиКорректировочныхСчетовФактур(НомераТаблиц) Экспорт
	...
	|	НЕОПРЕДЕЛЕНО КАК СтранаПроисхожденияДо,
	|	НЕОПРЕДЕЛЕНО КАК РегистрационныйНомерТДДо,
	|	НЕОПРЕДЕЛЕНО КАК РегистрационныйНомерТДПосле,
...
Функция ОпределитьПодписантов(СписокДокументов) Экспорт
	...
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|ГДЕ
	|	СчетФактураВыданный.Ссылка В(&Ссылки)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Руководитель) Тогда
			...
			ПодписиДокумента.Вставить("РуководительСчетФактура", Выборка.Руководитель);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.ГлавныйБухгалтер) Тогда
			ПодписиДокумента.Вставить("ГлавныйБухгалтер", Выборка.ГлавныйБухгалтер);
			ПодписиДокумента.Вставить("ГлавныйБухгалтерСчетФактура", Выборка.ГлавныйБухгалтер);
		КонецЕсли;
...
Функция ТекстЗапросаРеквизитыДокумента()
	...
	|	Реквизиты.Дата КАК Дата,
	|	ВЫБОР
	|		КОГДА Реквизиты.ОтредактированНомерСчетаФактуры
	|			ТОГДА Реквизиты.НомерВходящегоДокумента
	|		ИНАЧЕ Реквизиты.Номер
	|	КОНЕЦ КАК Номер,
	|	Реквизиты.Организация КАК Организация,
	...
	|		ИНАЧЕ Реквизиты.СуммаНДСУменьшение
...
Функция ТекстЗапросаЗаписьЖурналаУчетаСчетовФактур(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ИначеЕсли НЕ (Реквизиты.СводныйКомиссионный 
		...
		|				И НЕ Реквизиты.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.СводнаяСправка)
...
Функция ТекстЗапросаНомераДокументовОплаты(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|ГДЕ
	|	ВТ_ЗаписиКнигиПродажПредварительная.СтавкаНДС <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПодготовитьДанныеСводнойСправки(СтруктураПараметров)
	...
	|			ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.ПриходныйКассовыйОрдер).Дата
	|		КОГДА НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОплатаПлатежнойКартой
	|			ТОГДА ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.ОплатаПлатежнойКартой).Дата
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	...
	|			ИЛИ НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|			ИЛИ НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОплатаПлатежнойКартой)
	...
	|	ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.Регистратор КАК Документ.ОтчетКомиссионераОПродажах).Дата,
	...
	|	И НДСЗаписиКнигиПродаж.СчетФактура ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|
	|СГРУППИРОВАТЬ ПО
	|	НДСЗаписиКнигиПродаж.Регистратор,
	|	ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.Регистратор КАК Документ.ОтчетКомиссионераОПродажах).Дата
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НДСЗаписиКнигиПродаж.СчетФактура,
	|	ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.СчетФактура КАК Документ.ОтчетКомиссионераОПродажах).Дата,
	|	СУММА(НДСЗаписиКнигиПродаж.СуммаБезНДС)
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПродаж КАК НДСЗаписиКнигиПродаж
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОтчетыКомиссионеровРеализацияРозница КАК ВТ_ОтчетыКомиссионеровРеализацияРозница
	|		ПО НДСЗаписиКнигиПродаж.СчетФактура = ВТ_ОтчетыКомиссионеровРеализацияРозница.Ссылка
	|ГДЕ
	|	НДСЗаписиКнигиПродаж.Организация = &Организация
...
Функция РеквизитыРегламентнойОперации(ДокументСсылка) Экспорт
	...
	РеквизитыОперации.Вставить("НаАванс", Ложь);
...
Функция ПодготовитьДанныеКорректировочнойСправки(СтруктураПараметров)
	...
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации КАК ДокументРеализации
	...
	|		ПО ОтчетКомиссионераОПродажахВозвраты.КлючСтроки = ОтчетКомиссионераОПродажахТоварыВозвращенные.КлючСтроки
	|			И ОтчетКомиссионераОПродажахВозвраты.Ссылка = ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации В
	|			(ВЫБРАТЬ
	|				ОснованияИсходнойСправки.ДокументОснование
	|			ИЗ
	|				ОснованияИсходнойСправки)
	...
	|			И ОтчетКомиссионераОПродажахВозвраты.Ссылка = ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка В
	|			(ВЫБРАТЬ
	|				ОснованияИсходнойСправки.ДокументОснование
	|			ИЗ
	|				ОснованияИсходнойСправки)
	|	И ОтчетКомиссионераОПродажахТоварыВозвращенные.ПоТекущемуДокументу
	|	И ОтчетКомиссионераОПродажахВозвраты.Ссылка.Проведен
	|	И НЕ ОтчетКомиссионераОПродажахВозвраты.ВыставленСФ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	Документ.ОтчетКомиссионераОПродажах КАК ОтчетКомиссионераОПродажах
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетКомиссионераОПродажах.ТоварыВозвращенные КАК ОтчетКомиссионераОПродажахТоварыВозвращенные
	|		ПО ОтчетКомиссионераОПродажах.Ссылка = ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка
	|ГДЕ
	|	ОтчетКомиссионераОПродажахТоварыВозвращенные.ДокументРеализации В
	|			(ВЫБРАТЬ
	|				ОснованияИсходнойСправки.ДокументОснование
	...
	|				ИЗ
	|					ВТ_ДокументыОснования)
	|	И НЕ НДСЗаписиКнигиПокупок.СчетФактура ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|
	|СГРУППИРОВАТЬ ПО
...
Функция РеквизитыИсправляемогоСчетаФактуры(ВидСчетаФактуры, Исправление, ИсправляемыйСчетФактура) Экспорт
	
	Если Не Исправление Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	РеквизитыИсправляемогоДокумента = Новый Структура;
	РеквизитыИсправляемогоДокумента.Вставить("НомерИсходногоДокумента", "");
	РеквизитыИсправляемогоДокумента.Вставить("ДатаИсходногоДокумента", '00010101');
	РеквизитыИсправляемогоДокумента.Вставить("НомерИсправляемогоКорректировочногоДокумента", "");
	РеквизитыИсправляемогоДокумента.Вставить("ДатаИсправляемогоКорректировочногоДокумента", '00010101');
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.Корректировочный Тогда
		...
		РеквизитыИсправляемогоДокумента.НомерИсправляемогоКорректировочногоДокумента =
			ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(РеквизитыИсправляемогоСчетаФактуры.Номер, Истина, Ложь);
		РеквизитыИсправляемогоДокумента.ДатаИсправляемогоКорректировочногоДокумента =
			РеквизитыИсправляемогоСчетаФактуры.Дата;
		
	Иначе
		
		РеквизитыИсправляемогоДокумента.НомерИсходногоДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(РеквизитыИсправляемогоСчетаФактуры.Номер, Истина, Ложь);
		РеквизитыИсправляемогоДокумента.ДатаИсходногоДокумента  = РеквизитыИсправляемогоСчетаФактуры.Дата;
		
	КонецЕсли;
	
	Возврат РеквизитыИсправляемогоДокумента;
...
// Счет-фактура на аванс
Функция ЕстьОплатаГосКонтрактов(ДокументОснование)
		
	ЕстьОплатаГосКонтрактов = Ложь;

	Если ТипЗнч(ДокументОснование) <> Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") Тогда
			Возврат ЕстьОплатаГосКонтрактов;
	КонецЕсли;

	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	| ИСТИНА КАК ЕстьОплатаГосКонтрактов
	|ИЗ
	...
	| И ЕСТЬNULL(ДоговорыКонтрагентов.ГосударственныйКонтракт, ЗНАЧЕНИЕ(Справочник.ГосКонтракты.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ГосКонтракты.ПустаяСсылка)";

	Запрос.УстановитьПараметр("ДокументОснование",ДокументОснование);
	Результат = Запрос.Выполнить();
	Если НЕ Результат.Пустой() Тогда
		ЕстьОплатаГосКонтрактов = Истина;
	КонецЕсли;
...
Функция ТекстЗапросаДанныеПлатежаПоДоговорам(НомераТаблиц)
	
	// Временная таблица договоров из ВТ РасшифровкаПлатежа
	НомераТаблиц.Вставить("ВТ_ДоговорыПлатежа", НомераТаблиц.Количество());
	
	// Таблица договоров для отложенного проведения
	НомераТаблиц.Вставить("ДоговорыПлатежа", НомераТаблиц.Количество());
	...
	|ИЗ
	|	РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДоговорыСФ КАК ДоговорыСчетаФактуры
	|		ПО РасшифровкаПлатежа.ДоговорКонтрагента = ДоговорыСчетаФактуры.ДоговорКонтрагента
	|ГДЕ
	|	РасшифровкаПлатежа.СчетНаОплату <> ЗНАЧЕНИЕ(Документ.СчетНаОплатуПокупателю.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|	СуммыАвансов КАК СуммыАвансов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДоговорыСФ КАК ДоговорыСчетаФактуры
	|		ПО СуммыАвансов.ДоговорКонтрагента = ДоговорыСчетаФактуры.ДоговорКонтрагента
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДоговорыСФ КАК ДоговорыСФ
	|		ПО РасшифровкаПлатежа.ДоговорКонтрагента = ДоговорыСФ.ДоговорКонтрагента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыСчетаФактуры
	|		ПО РасшифровкаПлатежа.ДоговорКонтрагента = ДоговорыСчетаФактуры.Ссылка";
...
// Функция проверяет тип документа
// Параметры:
// ДокументОснование - документ, тип которого нужно проверить
// Возвращаемое значение:
// Булево - если относится к проверяемомму типу документа, то истина
Функция ЭтоКорректировочныйДокумент(ДокументОснование)
