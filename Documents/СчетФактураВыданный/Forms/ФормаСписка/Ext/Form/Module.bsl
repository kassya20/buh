...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыОтбораСписка") Тогда
		...
		Если ПараметрыОтбораСписка.Свойство("ДокументОснование") Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				Список, "ДокументОснование", ПараметрыОтбораСписка.ДокументОснование,
	...
	// При включенной роли ДобавлениеИзменениеСчетовФактурВыданных справки не включаем в список
	Если НЕ Пользователи.ЭтоПолноправныйПользователь( ,Истина)
		И Пользователи.РолиДоступны("ДобавлениеИзменениеСчетовФактурВыданных")
		И НЕ Пользователи.РолиДоступны(
			"ДобавлениеИзменениеДанныхБухгалтерии,ЧтениеДанныхБухгалтерии,ЧтениеСчетовФактурВыданных") Тогда
		
		ВидыСчетовФактурСправки = Новый Массив();
		ВидыСчетовФактурСправки.Добавить(Перечисления.ВидСчетаФактурыВыставленного.СводнаяСправка);
		ВидыСчетовФактурСправки.Добавить(Перечисления.ВидСчетаФактурыВыставленного.КорректировочнаяСправка);
		
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
				ЭтотОбъект.Список, "ВидСчетаФактуры", 
				ВидыСчетовФактурСправки,
				ВидСравненияКомпоновкиДанных.НеВСписке, , Истина, 
				РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Авто);
				
		Элементы.СоздатьКорректировочнуюСправку.Видимость = Ложь;
		Элементы.СоздатьСводнуюСправку.Видимость = Ложь;
	КонецЕсли;
...
Процедура Подключаемый_КомандаМКСоздать(Команда)
	...
	СписокВыбораОперации.Добавить("На реализация");
...
Процедура КомандыМК_Создать_Завершение(ВыбранныйЭлемент, ДопПараметры) Экспорт
	...
	Если ВыбранныйЭлемент.Значение = "На реализация" Тогда
		СоздатьСчетФактуруРеализация(ДопПараметры.Команда);
...
Процедура АдаптироватьМКСписокФормы()
	...
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.ВидСчетаФактуры, 1);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.ДокументОснование, 2);
