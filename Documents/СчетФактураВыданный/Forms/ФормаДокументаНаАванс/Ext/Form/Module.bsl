...
Процедура АвансыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		РасчетнаяСтавкаНДС = УчетНДСКлиентСервер.РасчетнаяСтавкаНДСПоАвансам(Объект.Организация, Объект.Дата);
...
Процедура АвансыНоменклатураПриИзменении(Элемент)
	...
	НовыеПараметры = ПолучитьДанныеНоменклатураПриИзменении(
		ТекущиеДанные.Номенклатура, Объект.Организация, ТекущаяДатаДокумента, ПокупательНалоговыйАгентПоНДС, СтавкаНДСИсчисляетсяНалоговымАгентом);
...
Процедура АвансыКорректируемыйСчетФактураПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.КорректируемыйСчетФактура)
		...
		ДанныеСтроки = Новый Структура("КорректируемыйСчетФактура,Сумма,СтавкаНДС,СуммаНДС,Дата");
		ЗаполнитьЗначенияСвойств(ДанныеСтроки, СтрокаТабличнойЧасти);
		ЗаполнитьЗначенияСвойств(ДанныеСтроки, ЭтотОбъект.Объект, "Дата");
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		И Объект.КодВидаОперации = "01" Тогда // переопределим не "авансовый" код
		Объект.КодВидаОперации = ПолучитьКодВидаОперации();
	КонецЕсли;
...
Процедура УстановитьПараметрыВыбораСтавкиНДС()
	...
	УчетНДС.УстановитьПараметрыВыбораСтавкиНДС(ЭтотОбъект, "АвансыСтавкаНДСДоКорректировки", "Авансы");
	УчетНДС.УстановитьПараметрыВыбораСтавкиНДС(ЭтотОбъект, "АвансыСтавкаНДСПослеКорректировки", "Авансы");
...
Функция ДатаПриИзмененииНаСервере()
	...
	РаботаСНоменклатуройБП.ОбновитьСодержаниеУслуг(Объект.Авансы, Объект.Дата, ТекущаяДатаДокумента);
...
Функция ПолучитьДанныеНоменклатураПриИзменении(Номенклатура, Организация, Дата, ПокупательНалоговыйАгентПоНДС, СтавкаНДСИсчисляетсяНалоговымАгентом)
	...
	Если ПараметрыНоменклатуры.Услуга Тогда
		НовыеПараметры.Вставить("Содержание", РаботаСНоменклатуройКлиентСерверБП.СодержаниеУслуги(
	...
	Если ПокупательНалоговыйАгентПоНДС Тогда
		НовыеПараметры.Вставить("СтавкаНДС", СтавкаНДСИсчисляетсяНалоговымАгентом);
	...
	Иначе
		НовыеПараметры.Вставить("СтавкаНДС", УчетНДСКлиентСервер.РасчетнаяСтавкаНДСПоАвансам(Организация, Дата));
	КонецЕсли;
...
Функция ПараметрыДоплатыКАвансу(ДанныеСтроки)
	...
	Запрос.УстановитьПараметр("УстаревшаяСтавкаНДС", УчетНДСКлиентСервер.ОбщаяСтавкаНДС(ПредыдущийГод));
	Запрос.УстановитьПараметр("УстаревшаяРасчетнаяСтавкаНДС", УчетНДСКлиентСервер.ОбщаяРасчетнаяСтавкаНДС(ПредыдущийГод));
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ВидСчетаФактуры);
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаШапкаЛевая);
	МобильныйКлиентБПФормаОбъекта.ПеренестиЭлементыВГруппаМКФормаПрочее(Элементы, Элементы.ГруппаШапкаПравая);
	
	Элементы.ГруппаКонтрагент.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	...
	Элементы.ДокументОснование.ПодсказкаВвода= НСтр("ru = 'Нажмите для выбора'");
...
Процедура АдаптироватьМКСписокФормы()
	...
	КомандыПоля.ИмяКомандыДобавитьСтроку = "АвансыКнопкаДобавить";
	КомандыПоля.КомандыТаблицыПрочие.Добавить("АвансыГруппаЗаполнить");
	
	ОписаниеГруппыПоля = МобильныйКлиентБПФормаОбъекта.НовыйОписаниеГруппыТабличноеПолеФормы();
	ОписаниеГруппыПоля.ЭлементГруппаПоля = Элементы.ГруппаАвансы;
	ОписаниеГруппыПоля.ПутьКДаннымЗаголовка = "Объект.Авансы.КоличествоСтрок";
	...
	ОписаниеКолонокПоля.КолонкаНомерСтроки = "АвансыНомерСтроки";
	ОписаниеКолонокПоля.ВажныеКолонки = "АвансыКорректируемыйСчетФактура,АвансыНоменклатура,АвансыКонтрагент";
	ОписаниеКолонокПоля.УстановитьВерхниеЗаголовки = "АвансыКорректируемыйСчетФактура,АвансыКонтрагент,"
		+ "АвансыНоменклатура,АвансыСодержание";
	ОписаниеКолонокПоля.СкрытьКолонки = "АвансыОтступ,АвансыНадписьНадписьДоИзменения,АвансыНадписьПослеИзменения";
	
	МобильныйКлиентБПФормаОбъекта.ПодготовитьТабличноеПолеФормыДокумента(ЭтотОбъект,
		Элементы.Авансы, ОписаниеГруппыПоля, КомандыПоля, ОписаниеКолонокПоля);
		
	Элементы.АвансыСуммаДоКорректировки.Формат = "ЧФ='Сумма: Ч'";
	Элементы.АвансыСуммаНДСДоКорректировки.Формат = "ЧФ='НДС: Ч'";
