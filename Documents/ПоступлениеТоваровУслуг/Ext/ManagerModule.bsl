...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьСчетаРасчетовСКомитентами");
	...
	Методы.Вставить("ЗаполнитьПустыеКоличествоЦенаОсновныхСредств");
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|				КОНЕЦ / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 3)) КАК Количество, 
	|	ВЫРАЗИТЬ(ВЫБОР
	|				КОГДА ТаблицаТовары.Ссылка.ЕстьРасхождения
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыПоступлениеТоваров()
	...
	|	ЗапросПоОборудованию.Номенклатура КАК Номенклатура,
	...
	|	ЗапросПоОборудованию.Коэффициент КАК Коэффициент,
	|	ЗапросПоОборудованию.Цена КАК Цена,
	|	ЗапросПоОборудованию.Сумма КАК Сумма,
	|	ЗапросПоОборудованию.СуммаНДС КАК СуммаНДС,
	|	ЗапросПоОборудованию.СтавкаНДС КАК СтавкаНДС,
	|	ЗапросПоОборудованию.НомерСтроки КАК НомерСтроки,
	|	1 КАК ID
	|ИЗ
	...
	|			ТОГДА """"
	|	КОНЕЦ,
	|	ЕСТЬNULL(СправочникУпаковка.Ссылка, СправочникНоменклатура.ЕдиницаИзмерения), 
	|	ЗапросПоТоварам.ЕдиницаИзмерения.Представление,
	|	ЗапросПоТоварам.КоличествоМест,
	|	ВЫРАЗИТЬ(ЗапросПоТоварам.Количество / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 3)),
	|	ЗапросПоТоварам.Коэффициент,
	|	ВЫРАЗИТЬ(ЗапросПоТоварам.Цена * ЕСТЬNULL(ЗапросПоТоварам.Упаковка.Коэффициент, 1) КАК ЧИСЛО(15, 2)),
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ЗапросПоТоварам.Номенклатура = СправочникНоменклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиНоменклатуры КАК СправочникУпаковка
	|		ПО (ЗапросПоТоварам.Упаковка = СправочникУпаковка.Ссылка
	|				И СправочникУпаковка.Коэффициент > 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийТОРГ12()
	...
	|			ТОГДА """"
	|	КОНЕЦ,
	|	ПоступлениеТоваровУслуг.Количество / ЕСТЬNULL(СправочникУпаковки.Коэффициент, 1),
	|	ПоступлениеТоваровУслуг.КоличествоМест,
	|	ЕСТЬNULL(СправочникУпаковки.Ссылка, СправочникНоменклатура.ЕдиницаИзмерения),
	|	ЕСТЬNULL(СправочникУпаковки.ЕдиницаИзмерения.Код, СправочникНоменклатура.ЕдиницаИзмерения.Код),
	|	ЕСТЬNULL(СправочникУпаковки.ЕдиницаИзмерения.Наименование, СправочникНоменклатура.ЕдиницаИзмерения.Наименование),
	|	ПоступлениеТоваровУслуг.ЕдиницаИзмерения,
	|	ПоступлениеТоваровУслуг.Коэффициент,
	|	ПоступлениеТоваровУслуг.Коэффициент,
	|	ПоступлениеТоваровУслуг.Цена * ЕСТЬNULL(СправочникУпаковки.Коэффициент, 1),
	|	ВЫБОР
	|		КОГДА ДокументыДляПечати.СуммаВключаетНДС
	|			ТОГДА ПоступлениеТоваровУслуг.Сумма - ПоступлениеТоваровУслуг.СуммаНДС
	|		ИНАЧЕ ПоступлениеТоваровУслуг.Сумма
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|			ПО ПоступлениеТоваровУслуг.Номенклатура = СправочникНоменклатура.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиНоменклатуры КАК СправочникУпаковки
	|			ПО ПоступлениеТоваровУслуг.Упаковка = СправочникУпаковки.Ссылка
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыТОРГ4()
	...
	|	КОНЕЦ КАК ТоварКод,
	|	ПоступлениеТоваровУслуг.КоличествоМест КАК КоличествоМест,
	|	ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Количество / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,3)) КАК КоличествоШтук,
	|	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения, СправочникНоменклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияНаименование,
	|	ЕСТЬNULL(СправочникУпаковка.ЕдиницаИзмерения.Код, СправочникНоменклатура.ЕдиницаИзмерения.Код) КАК ЕдиницаИзмеренияКодПоОКЕИ,
	|	ПоступлениеТоваровУслуг.Коэффициент КАК КоличествоВОдномМесте,
	|	ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Цена * ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15,3)) КАК Цена,
	...
	|		ПоступлениеТоваровУслуг.Упаковка) КАК ПоступлениеТоваровУслуг
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ПоступлениеТоваровУслуг.Номенклатура = СправочникНоменклатура.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УпаковкиНоменклатуры КАК СправочникУпаковка
	|		ПО ПоступлениеТоваровУслуг.Упаковка = СправочникУпаковка.Ссылка
	|			И (СправочникУпаковка.Коэффициент > 0)
	|ГДЕ
...
Функция ПолучитьТекстЗапросаДляФормированияПечатнойФормыМ4()
	...
	|		ИНАЧЕ ПоступлениеТоваровУслуг.Номенклатура.Код
	|	КОНЕЦ КАК ТоварКод,
	|	ЕСТЬNULL(ПоступлениеТоваровУслуг.Упаковка.ЕдиницаИзмерения, СправочникНоменклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмеренияНаименование,
	|	ЕСТЬNULL(ПоступлениеТоваровУслуг.Упаковка.ЕдиницаИзмерения.Код, СправочникНоменклатура.ЕдиницаИзмерения.Код) КАК ЕдиницаИзмеренияКод,
	|	ПоступлениеТоваровУслуг.СтавкаНДС КАК СтавкаНДС,
	|	ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Цена * ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 2)) КАК Цена,
	|	СУММА(ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Количество / ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 3))) КАК Количество,
	|	СУММА(ВЫБОР
	|			КОГДА ДокументПоступлениеТоваровУслуг.ВалютаДокумента = &ВалютаРегламентированногоУчета
	|				ТОГДА ПоступлениеТоваровУслуг.Сумма
	|			КОГДА ДокументПоступлениеТоваровУслуг.КратностьВзаиморасчетов = 0
	...
	|	ПоступлениеТоваровУслуг.Номенклатура,
	|	ПоступлениеТоваровУслуг.ЕдиницаИзмерения,
	|	ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Цена * ЕСТЬNULL(СправочникУпаковка.Коэффициент, 1) КАК ЧИСЛО(15, 2)),
	|	ПоступлениеТоваровУслуг.СтавкаНДС,
	|	ЕСТЬNULL(ПоступлениеТоваровУслуг.Упаковка.ЕдиницаИзмерения, СправочникНоменклатура.ЕдиницаИзмерения),
	|	ЕСТЬNULL(ПоступлениеТоваровУслуг.Упаковка.ЕдиницаИзмерения.Код, СправочникНоменклатура.ЕдиницаИзмерения.Код),
	|	ПоступлениеТоваровУслуг.Номенклатура.НаименованиеПолное,
	|	ВЫБОР
	|		КОГДА &ДополнительнаяКолонкаПечатныхФормДокументов = ЗНАЧЕНИЕ(Перечисление.ДополнительнаяКолонкаПечатныхФормДокументов.Артикул)
	|			ТОГДА ПоступлениеТоваровУслуг.Номенклатура.Артикул
...
Функция ПечатьМ4(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ТаблицаПоТоварам.Колонки.Добавить("НДСРуб",                     ОписаниеТиповЧисло15_2);
	ТаблицаПоТоварам.Колонки.Добавить("СуммаБезНДСРуб",             ОписаниеТиповЧисло15_2);
	ТаблицаПоТоварам.Колонки.Добавить("Упаковка");
