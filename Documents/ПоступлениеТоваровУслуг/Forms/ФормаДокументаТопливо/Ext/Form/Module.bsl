...
Процедура УстановитьУсловноеОформлениеОбщее()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТопливоСуммаНДС");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТопливоСчетУчетаНДС");
...
Процедура СчетНаОплатуПоставщикаПриИзменении(Элемент)
	...
	ОбработкаЗаполненияПоСчетуНаКлиенте(Объект.СчетНаОплатуПоставщика, ИмяТекущейТабличнойЧасти());
КонецПроцедуры
...
Процедура АдаптироватьМКСписокФормы()
	...
	КомандыПоля.ИмяКомандыПодбор ="ТопливоПодборТопливо";
	КомандыПоля.ИмяКомандыДобавитьСтроку = "ТопливоДобавить";
	КомандыПоля.КомандыТаблицыПрочие.Добавить("ТопливоЗаполнить");
	...
	ОписаниеГруппыПоля.ПутьКДаннымЗаголовка = "Объект.Топливо.КоличествоСтрок";
	...
	ОписаниеКолонокПоля.ВажныеКолонки = "ТопливоТранспортноеСредство,ТопливоНоменклатура";
	ОписаниеКолонокПоля.УстановитьВерхниеЗаголовки = "ТопливоТранспортноеСредство,ТопливоНоменклатура";
	...
	ГруппаКолонокМКТранспортТопливо = Элементы.Добавить("ГруппаКолонокМКТранспортТопливо",
		Тип("ГруппаФормы"),
		Элементы.Топливо);
	ГруппаКолонокМКТранспортТопливо.Вид = ВидГруппыФормы.ГруппаКолонок;
	ГруппаКолонокМКТранспортТопливо.Группировка = ГруппировкаКолонок.Вертикальная;
	ГруппаКолонокМКТранспортТопливо.ВажностьПриОтображении = ВажностьПриОтображении.ОченьВысокая;
	ГруппаКолонокМКТранспортТопливо.ОтображатьВШапке = Ложь;
	ГруппаКолонокМКТранспортТопливо.РастягиватьПоГоризонтали = Истина;
	Элементы.Переместить(ГруппаКолонокМКТранспортТопливо,
		Элементы.Топливо,
		Элементы.ТопливоТранспортноеСредство);
	
	Элементы.Переместить(Элементы.ТопливоТранспортноеСредство, ГруппаКолонокМКТранспортТопливо);
	Элементы.Переместить(Элементы.ТопливоНоменклатура, ГруппаКолонокМКТранспортТопливо);
	
	Элементы.ТопливоТранспортноеСредство.РастягиватьПоГоризонтали = Истина;
	Элементы.ТопливоНоменклатура.Ширина = 20;
