...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.ЗаголовокФормы= НСтр("ru = 'Реестр документов ""Передача ОС арендатору""'");
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|ПОМЕСТИТЬ Реквизиты
	|ИЗ
	|	Документ.ПередачаОСВАренду КАК Реквизиты
	|ГДЕ
...
Функция ПечатьОС1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ИмяМакета)
	...
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПередачаОСВАренду_ОС1";
	...
	|	ПередачаОСВАренду.Дата КАК Дата,
	|	ПередачаОСВАренду.Номер КАК НомерАкта,
	|	ПередачаОСВАренду.Ответственный,
	|	ПередачаОСВАренду.ПодразделениеОрганизации КАК ПодрСдатчика,
	|	ПередачаОСВАренду.Контрагент.НаименованиеПолное КАК ОрганизацияПолучатель,
	|	ПередачаОСВАренду.Контрагент КАК Получатель,
	|	ПередачаОСВАренду.Организация КАК Сдатчик,
	|	ПередачаОСВАренду.ПодразделениеОрганизации КАК ПодразделениеОрганизации,
	|	ПРЕДСТАВЛЕНИЕ(ПередачаОСВАренду.ДоговорКонтрагента) КАК ДоговорПередачи,
	|	ПередачаОСВАренду.ДоговорКонтрагента.Дата КАК ДатаДоговора,
	|	ПередачаОСВАренду.ДоговорКонтрагента.Номер КАК НомерДоговора,
	|	ПередачаОСВАренду.Организация КАК Организация,
	|	ПередачаОСВАренду.МоментВремени КАК СсылкаМоментВремени,
	|	ПередачаОСВАренду.СчетУчета
	|ИЗ
	|	Документ.ПередачаОСВАренду КАК ПередачаОСВАренду
	|ГДЕ
	|	ПередачаОСВАренду.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	|	Ссылка
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПередачаОСВАрендуОС.ОсновноеСредство КАК ОсновноеСредство
	|ПОМЕСТИТЬ СписокОС
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПередачаОСВАрендуОС.НомерСтроки КАК НомерСтроки,
	...
	|	ПередачаОСВАрендуОС.ОсновноеСредство.ГруппаОС КАК ГруппаОС,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.НаименованиеПолное КАК НаименованиеОс,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.АмортизационнаяГруппа.Порядок + 1 КАК НомерГруппы,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.ЗаводскойНомер КАК ЗаводскойНомер,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.ДатаВыпуска КАК ГодВыпуска,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.Изготовитель КАК Изготовитель,
	|	ПередачаОСВАрендуОС.ОсновноеСредство.КодПоОКОФ.Код КАК КодОКОФ,
	|	МестонахождениеОС.Местонахождение,
	...
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = МестонахождениеОС.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПараметрыАмортизации КАК ПараметрыАмортизации
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = ПараметрыАмортизации.ОсновноеСредство
	...
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = ПервоначальныеСведенияОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследнийКапитальныйРемонт КАК ПоследнийКапитальныйРемонт
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = ПоследнийКапитальныйРемонт.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоследняяМодернизация КАК ПоследняяМодернизация
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = ПоследняяМодернизация.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ СтоимостьОС КАК СтоимостьОС
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = СтоимостьОС.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ АмортизацияОС КАК АмортизацияОС
	|		ПО ПередачаОСВАрендуОС.ОсновноеСредство = АмортизацияОС.ОсновноеСредство
	|ГДЕ
	|	ПередачаОСВАрендуОС.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ЗаполнитьДанныеОрганизацииСдатчика(ПараметрыДокумента, ОбластьМакета)
	...
	СведенияОбОрганизации = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(ПараметрыДокумента.Сдатчик, ПараметрыДокумента.Дата, , , , Истина);
