...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Документы.ПоступлениеНМА.ЗаполнитьПодразделениеТабличнойЧастиЕслиОбъектНеОбработан(Объект);
	// Конец Обновление
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ЭтоУниверсальныйДокумент
	   ...
	   И Документы.ПоступлениеНМА.НаличиеСуммыНДС(Объект.Ссылка) Тогда
...
Процедура НематериальныеАктивыНематериальныйАктивПриИзменении(Элемент)
	...
	НематериальныйАктивПриИзмененииНаСервере(ДанныеСтроки, Объект.Дата, ПлательщикНДС, СпособЗаполненияСтавкиНДС);
...
Процедура ПодготовитьФормуНаСервере()
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, УчетНМА.СпискиСчетовУчетаНМА());
	
	НовыйМассивПараметров = Новый Массив;
	НовыйМассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", Новый ФиксированныйМассив(СчетаУчетаПоНМА.ВыгрузитьЗначения())));
	
	НовыйМассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ЗапретитьИспользоватьВПроводках", Ложь));
	Элементы.НематериальныеАктивыСчетУчета.ПараметрыВыбора = Новый ФиксированныйМассив(НовыйМассивПараметров);
...
Процедура НематериальныйАктивПриИзмененииНаСервере(СтрокаТаблицы, Период, ПлательщикНДС, СпособЗаполненияСтавкиНДС)

	Документы.ПоступлениеНМА.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(СтрокаТаблицы, Период, ПлательщикНДС, СпособЗаполненияСтавкиНДС);
...
Процедура ОбновитьУсловноеОформление(Форма)
	...
	Если Форма.НастройкиУсловногоОформления = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеОбщее()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыСтатьяЗатрат");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НематериальныеАктивы.СчетУчета", ВидСравненияКомпоновкиДанных.НеВСписке, СчетаУчетаПоСтатьямЗатрат);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НематериальныеАктивы.СчетУчета", ВидСравненияКомпоновкиДанных.ВСписке, СчетаУчетаПоНМА);
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "НематериальныеАктивыПодразделениеЗатрат");

	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НематериальныеАктивы.СчетУчета", ВидСравненияКомпоновкиДанных.НеВСписке, СчетаУчетаПоПодразделениям);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.НематериальныеАктивы.СчетУчета", ВидСравненияКомпоновкиДанных.ВСписке, СчетаУчетаПоНМА);
...
Процедура ОбновитьИтоги(Форма)
	...
	Форма.ИтогиВсегоНДС = Объект.НематериальныеАктивы.Итог("СуммаНДС");
...
Функция ОписаниеТипаРеквизитаДокументАванса()
	...
	Возврат Метаданные.Документы.ПоступлениеНМА.ТабличныеЧасти.ЗачетАвансов.Реквизиты.ДокументАванса.Тип;
