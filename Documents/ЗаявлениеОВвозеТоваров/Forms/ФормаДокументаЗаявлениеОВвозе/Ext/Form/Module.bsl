...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(Параметры.ДокументОснование) Тогда
			ЗаполнитьДокументПоПоступлениюТоваров(Параметры.ДокументОснование);
		КонецЕсли;
		Объект.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе;
...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	Если ЕстьУчетПодакцизныхТоваров Тогда
		...
		УчетАкцизовКлиентСервер.РассчитатьСуммуАкцизаВТабЧастиПоЗаявлениюОВвозе(ТекДанные);
		ДанныеСтрокиТаблицы = Новый Структура(
		"НалоговаяБазаНДС,ДатаПринятияНаУчет,СтатСтоимостьДолларыСША");
...
Процедура ТоварыДокументПартииПриИзменении(Элемент)
	...
	//ДокументПартии-ДокументПоступление на несколько складов
...
Процедура ТоварыСчетУчетаПриИзменении(Элемент)
	...
	Если НЕ ПлательщикНДС
		И НЕ ТекущийСчетУчетаЗабалансовый Тогда
		
		ТекущиеДанные.СчетУчетаНДС = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.НДСУплачиваемыйПриИмпортеИзТаможенногоСоюза");
...
Процедура ПодготовитьФормуНаСервере()
	...
	ЗабалансовыеСчета = ПланыСчетов.Хозрасчетный.СчетаЗабалансовогоУчетаЗапасов(Истина);
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	ТекстЗаголовка = НСтр("ru = 'Заявление о ввозе товаров'");
...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)
	...
	Если ДанныеОбъекта.ЕстьУчетПодакцизныхТоваров Тогда
		...
		Если ЗначениеЗаполнено(КодВидаПодакцизногоТовара) И Не СведенияОНоменклатуре.МаркируемаяПродукция Тогда
			...
			Если СтрокаТабличнойЧасти.ПодакцизныйТовар Тогда
				...
				УчетАкцизовКлиентСервер.РассчитатьСуммуАкцизаВТабЧастиПоЗаявлениюОВвозе(СтрокаТабличнойЧасти);
...
Процедура РасчитатьСуммыВТабличнойЧастиНаСервере(СтрокаТабличнойЧасти, ДанныеОбъекта)
	...
	Документы.ЗаявлениеОВвозеТоваров.РасчитатьСуммыВТабличнойЧастиЗаявленияОВвозе(СтрокаТабличнойЧасти, ДанныеОбъекта);
	ОбновитьИтоги(ЭтаФорма);
...
&НаСервере
Процедура ЗаполнитьДокументПоПоступлениюТоваров(ДокументПоступления)
	
	Объект.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе;
	
	РеквизитыПоступления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументПоступления,
	...
	ДокументыПоступления.Добавить(ДокументПоступления);
