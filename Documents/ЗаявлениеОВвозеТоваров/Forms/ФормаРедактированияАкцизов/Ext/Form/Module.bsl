...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Параметры,
		"Номенклатура,ДатаПринятияНаУчет,Организация,ВидОперации,КБК,КодВидаПодакцизногоТовара,МаркируемаяПродукция,
		|НалоговаяБазаАкцизы,ТвердаяСтавкаАкциза,АдвалорнаяСтавкаАкциза,ЕдиницаИзмеренияАкциза,
		|СуммаАкциза, КоэффициентПересчета, ОкруглятьДоЦелого, ЕстьУчетПодакцизныхТоваров");
	
	Если ЕстьУчетПодакцизныхТоваров Тогда
		Если ЕдиницаИзмеренияАкциза <> "" Тогда
			НаименованиеЕдиницыИзмеренияАкциза = Справочники.КлассификаторЕдиницИзмерения.НайтиПоКоду(ЕдиницаИзмеренияАкциза);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаДОПП.Видимость = Форма.ВидАкциза <> 0 
		И Форма.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП");
...
Процедура УстановитьУсловноеОформление()
	...
	ГруппаОтбора1 = КомпоновкаДанныхКлиентСервер.ДобавитьГруппуОтбора(
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
			"ВидАкциза", ВидСравненияКомпоновкиДанных.Равно, 0);
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаОтбора1,
			"ВидОперации", ВидСравненияКомпоновкиДанных.НеРавно, Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП);
	...
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "КодВидаПодакцизногоТовара");
...
Функция ПроверитьЗаполнениеНаКлиенте()
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП")Тогда
		...
		Если Не ЗначениеЗаполнено(КодВидаПодакцизногоТовара) Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
				"Поле", "Заполнение", НСтр("ru = 'Код вида подакцизного товара'"));
			Поле = "КодВидаПодакцизногоТовара";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , Поле, "", Отказ);
		КонецЕсли;
