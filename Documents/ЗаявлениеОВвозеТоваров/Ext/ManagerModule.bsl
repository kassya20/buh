...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	Набор.Имя = "Документ_ЗаявлениеОВвозеТоваров";
	Набор.Идентификатор = Новый УникальныйИдентификатор("d88afef3-1aff-469a-b1e6-34908860a542");
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если Не ЗначениеЗаполнено(ВидОперации) Тогда
			// Ранее созданный документ с пустым видом операции.
	...
	ФормыВвозТоваровИзЕАЭС = ПолучитьСоответствиеВидовОперацийФормам();
	ВыбраннаяФорма = ФормыВвозТоваровИзЕАЭС[ВидОперации];
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Данные.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП Тогда
		СтандартнаяОбработка = Ложь;
	...
	ИначеЕсли Данные.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе Тогда
		СтандартнаяОбработка = Ложь;
		Представление = СтрШаблон(НСтр("ru = '%1 %2 от %3'"),
...
Функция ТоварыПоДОПП(ДОПП) Экспорт
	...
	Результат.Колонки.Добавить("Коэффициент",            ОбщегоНазначения.ОписаниеТипаЧисло(10, 3));
	Результат.Колонки.Добавить("Количество",             ОбщегоНазначения.ОписаниеТипаЧисло(10, 3));
	...
	Результат.Колонки.Добавить("СуммаНДС",               ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
	Результат.Колонки.Добавить("НомерГТД",               Новый ОписаниеТипов("СправочникСсылка.НомераГТД"));
	Результат.Колонки.Добавить("СтранаПроисхождения",    Новый ОписаниеТипов("СправочникСсылка.СтраныМира"));
	Результат.Колонки.Добавить("ОтражениеВУСН",          Новый ОписаниеТипов("ПеречислениеСсылка.ОтражениеВУСН"));
	Результат.Колонки.Добавить("ПрослеживаемыйТовар",    Новый ОписаниеТипов("Булево"));
	Результат.Колонки.Добавить("ПрослеживаемыйКомплект", Новый ОписаниеТипов("Булево"));
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДОПП", ДОПП);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	ЗаявлениеОВвозеТоваровТовары.ФактурнаяСтоимость КАК Сумма,
	|	ЗаявлениеОВвозеТоваровТовары.ФактурнаяСтоимость / ЗаявлениеОВвозеТоваровТовары.Количество КАК Цена,
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0) КАК СтавкаНДС,
	|	ЗаявлениеОВвозеТоваровТовары.СчетУчета КАК СчетУчета,
	|	ЗаявлениеОВвозеТоваровТовары.НомерТД КАК НомерГТД,
	|	ЗаявлениеОВвозеТоваровТовары.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	...
	|ГДЕ
	|	ЗаявлениеОВвозеТоваровТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП)
	|	И ЗаявлениеОВвозеТоваровТовары.Ссылка = &ДОПП";
	Товары = Запрос.Выполнить().Выгрузить();
	Для каждого СтрокаДОПП Из Товары Цикл
		НоваяСтрока = Результат.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДОПП);
...
Функция КодВидаТранспортаДляЗаявленияОВвозе(ДОПП) Экспорт
	
	КодВидаТранспорта = "";
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДОПП);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ЗаявлениеОВвозеТоваров.РегистрационныйЗнакПрицепаДополнительный <> """"
	|			ТОГДА ""32""
	|		КОГДА ЗаявлениеОВвозеТоваров.РегистрационныйЗнакПрицепа <> """"
	|			ТОГДА ""31""
	|		ИНАЧЕ ""30""
	|	КОНЕЦ КАК КодВидаТранспорта
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		КодВидаТранспорта = Выборка.КодВидаТранспорта;
	КонецЕсли;
	
	Возврат КодВидаТранспорта;
...
Функция ОпределитьВидОперацииПоДокументуОснованию(Основание)
	
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") Тогда
		Результат = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП;
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		Результат = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе;
	Иначе
		Результат = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе;
	КонецЕсли;
...
Функция ТоварыПоДаннымПоступления(ДанныеОбъекта, ДокументыПоступления) Экспорт
	...
	Запрос.УстановитьПараметр("ЕстьУчетПодакцизныхТоваров", ЕстьУчетПодакцизныхтоваров);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Товары.Номенклатура КАК Номенклатура,
	|	Товары.Количество КАК Количество,
	|	Товары.Количество * ЕСТЬNULL(Товары.Номенклатура.ВесПоСертификатуТовара, 0) КАК ВесНеттоКг,
	|	ВЫБОР
	...
	|					ТОГДА Товары.Количество * ЕСТЬNULL(Товары.Номенклатура.ВесПоСертификатуТовара, 0)
	|				ИНАЧЕ 0
	|			КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА Товары.СчетУчета.Забалансовый = ИСТИНА
	|			ТОГДА Товары.СчетРасчетов
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСУплачиваемыйПриИмпортеИзТаможенногоСоюза)
	|	КОНЕЦ КАК СчетУчетаНДС,
	|	ВЫБОР
	|		КОГДА Товары.СчетУчета.Забалансовый = ИСТИНА
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПустаяСсылка)
	|		ИНАЧЕ Товары.СпособУчетаНДС
	...
	|	ВЫБОР
	|		КОГДА &ЕстьУчетПодакцизныхТоваров
	|				И СправочникНоменклатура.КодВидаПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КодыВидовПодакцизныхТоваров.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.АкцизыПоОплаченнымМатериальнымЦенностям)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СчетАкциза,
	|	ВЫБОР
	|		КОГДА &ЕстьУчетПодакцизныхТоваров
	|				И СправочникНоменклатура.КодВидаПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КодыВидовПодакцизныхТоваров.ПустаяСсылка)
	|			ТОГДА Товары.Ссылка.Организация.РегистрацияВНалоговомОргане
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СубконтоАкцизы,
	...
	|	Оборудование.Количество * ЕСТЬNULL(Оборудование.Номенклатура.ВесПоСертификатуТовара, 0),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КлассификаторТНВЭД.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка)) = ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ПустаяСсылка))
	|			ТОГДА Оборудование.Количество
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(КлассификаторТНВЭД.ЕдиницаИзмерения.Код, """") = ""166""
	|					ТОГДА Оборудование.Количество * ЕСТЬNULL(Оборудование.Номенклатура.ВесПоСертификатуТовара, 0)
	|				ИНАЧЕ 0
	|			КОНЕЦ
	|	КОНЕЦ,
	|	Оборудование.Сумма,
	...
	|	ВЫБОР
	|		КОГДА Оборудование.СчетУчета.Забалансовый = ИСТИНА
	...
	|		КОГДА Оборудование.СчетУчета.Забалансовый = ИСТИНА
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.ПустаяСсылка)
	|		ИНАЧЕ Оборудование.СпособУчетаНДС
	|	КОНЕЦ,
	|	0,
	|	0,
	|	0,
	...
	|		КОГДА &ЕстьУчетПодакцизныхТоваров
	|				И СправочникНоменклатура.КодВидаПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КодыВидовПодакцизныхТоваров.ПустаяСсылка)
	|			ТОГДА Оборудование.Ссылка.Организация.РегистрацияВНалоговомОргане
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ,
	|	NULL
	|ИЗ
	...
	Для каждого СтрокаТоваров Из ТаблицаЗначенийТовары Цикл
		...
		РасчитатьСуммыВТабличнойЧастиЗаявленияОВвозе(СтрокаТоваров, ДанныеОбъекта);
	КонецЦикла;
...
Процедура РасчитатьСуммыВТабличнойЧастиЗаявленияОВвозе(СтрокаТабличнойЧасти, ДанныеОбъекта) Экспорт
...
Процедура РасчитатьСуммыВТабличнойЧастиДОПП(СтрокаТабличнойЧасти, ДанныеОбъекта) Экспорт
	...
	Если ДанныеОбъекта.ВалютаДокумента <> ДанныеОбъекта.ВалютаРегламентированногоУчета Тогда
		СтруктураКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ДанныеОбъекта.ВалютаДокумента, ДатаДляРасчетаКурса);
		Если СтруктураКурса.Кратность = 0 Тогда
			СтруктураКурса.Курс = 1;
			СтруктураКурса.Кратность = 1;
		КонецЕсли;
...
Функция КБКПоКодуВидаПодакцизногоТовара(КодВидаПодакцизногоТовара) Экспорт
	
	СоответствиеКБК = Новый Соответствие;
	СоответствиеКБК.Вставить("224", "18210402110011000110");
	СоответствиеКБК.Вставить("225", "18210402110011000110");
	СоответствиеКБК.Вставить("226", "18210402110011000110");
	СоответствиеКБК.Вставить("227", "18210402110011000110");
	СоответствиеКБК.Вставить("228", "18210402110011000110");
	СоответствиеКБК.Вставить("229", "18210402110011000110");
	
	СоответствиеКБК.Вставить("231", "18210402130011000110");
	
	СоответствиеКБК.Вставить("232", "18210402120011000110");
	
	СоответствиеКБК.Вставить("251", "18210402021011000110");
	СоответствиеКБК.Вставить("252", "18210402021011000110");
	СоответствиеКБК.Вставить("253", "18210402021011000110");
	СоответствиеКБК.Вставить("254", "18210402021011000110");
	СоответствиеКБК.Вставить("255", "18210402021011000110");
	СоответствиеКБК.Вставить("256", "18210402021011000110");
	
	СоответствиеКБК.Вставить("274", "18210402091011000110");
	СоответствиеКБК.Вставить("275", "18210402091011000110");
	СоответствиеКБК.Вставить("288", "18210402091011000110");
	
	СоответствиеКБК.Вставить("276", "18210402090011000110");
	СоответствиеКБК.Вставить("277", "18210402090011000110");
	СоответствиеКБК.Вставить("287", "18210402090011000110");
	СоответствиеКБК.Вставить("289", "18210402090011000110");
	...
	СоответствиеКБК.Вставить("410", "18210402030011000110");
	СоответствиеКБК.Вставить("412", "18210402030011000110");
	СоответствиеКБК.Вставить("413", "18210402030011000110");
	СоответствиеКБК.Вставить("414", "18210402030011000110");
	СоответствиеКБК.Вставить("415", "18210402030011000110");
	СоответствиеКБК.Вставить("420", "18210402030011000110");
	СоответствиеКБК.Вставить("430", "18210402030011000110");
	СоответствиеКБК.Вставить("440", "18210402030011000110");
	СоответствиеКБК.Вставить("441", "18210402030011000110");
	СоответствиеКБК.Вставить("442", "18210402030011000110");
	СоответствиеКБК.Вставить("450", "18210402030011000110");
	СоответствиеКБК.Вставить("460", "18210402030011000110");
	
	СоответствиеКБК.Вставить("471", "18210402190011000110");
	
	СоответствиеКБК.Вставить("472", "18210402200011000110");
	...
	КБК = СоответствиеКБК.Получить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КодВидаПодакцизногоТовара, "Код"));
...
Функция ТоварыПоСчетуПоставщика(ДанныеОбъекта, ДокументОснование) Экспорт
	...
	|	ЕСТЬNULL(СправочникНоменклатура.КодТНВЭД, """") КАК КодТНВЭД,
	|	ЕСТЬNULL(СправочникНоменклатура.ВидСтавкиНДС, ЗНАЧЕНИЕ(Перечисление.ВидыСтавокНДС.ПустаяСсылка)) КАК ВидСтавкиНДС,
	|	ЕСТЬNULL(СправочникНоменклатура.СтранаПроисхождения, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) КАК СтранаПроисхождения,
	...
	Для каждого СтрокаТоваров Из ТаблицаЗначенийТовары Цикл
		...
		РасчитатьСуммыВТабличнойЧастиДОПП(СтрокаТоваров, ДанныеОбъекта);
	КонецЦикла;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	ВедетсяПростойУчетЕНП = УчетНДСПереопределяемый.ВедетсяПростойУчетЕНП(Реквизиты.Период, Реквизиты.Организация);
	...
	Запрос.УстановитьПараметр("ВедетсяПростойУчетЕНП", ВедетсяПростойУчетЕНП);
	...
	Запрос.Текст = ТекстЗапросаТаблицаТовары();
	ТаблицаТоварыПредварительная = Запрос.Выполнить().Выгрузить();
	// С даты начала ведения простого учета ЕНП суммы по налогу округляем до целого рубля.
	Если ВедетсяПростойУчетЕНП Тогда
		...
		Если КоличествоСтрок > 0 
			...
			И Реквизиты.СуммаДокумента <> ТаблицаТоварыПредварительная.Итог("СуммаНДС") Тогда
			МассивКоэффициентов = ТаблицаТоварыПредварительная.ВыгрузитьКолонку("СуммаНДС");
			МассивСумм = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(
			Реквизиты.СуммаДокумента, МассивКоэффициентов);
			Для НомерСтроки = 0 По КоличествоСтрок - 1 Цикл
				ТаблицаТоварыПредварительная[НомерСтроки].СуммаНДС = МассивСумм[НомерСтроки];
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	Запрос.УстановитьПараметр("ТаблицаТовары", ТаблицаТоварыПредварительная);
	Запрос.УстановитьПараметр("СчетРасчетовПоАкцизам", ПланыСчетов.Хозрасчетный.Акцизы);
	Реквизиты.Вставить("ИмпортПодакцизныхТоваров", ПолучитьФункциональнуюОпцию("ИмпортПодакцизныхТоваров"));
	НомераТаблиц = Новый Структура;
	...
	Запрос.Текст = ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
		...
		+ ТекстЗапросаРезервированиеОбеспечительногоПлатежа(НомераТаблиц, ПараметрыПроведения, Реквизиты);
...
Процедура СформироватьПроводкуРезервированиеОбеспечительногоПлатежа(ТаблицаРезервированиеОбеспечительногоПлатежа, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Если ТаблицаРезервированиеОбеспечительногоПлатежа = Неопределено Или ТаблицаРеквизиты = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Реквизиты = ТаблицаРеквизиты[0];
	
	// Проводка Дт 68.42 Кт 68.91 на сумму обеспечительного платежа по документу
	Для каждого СтрокаТаблицы Из ТаблицаРезервированиеОбеспечительногоПлатежа Цикл
		
		Если СтрокаТаблицы.СуммаДокумента = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Проводка.Содержание = НСтр("ru = 'Резервирование обеспечительного платежа для уплаты косвенных налогов при ввозе из ЕАЭС'");
		
		Проводка.СчетДт = ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате; // 68.42
		...
		Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС; // 68.91
		
		Проводка.Сумма = СтрокаТаблицы.СуммаДокумента;
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидОперации", Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе);
	КомандаПечати.СписокФорм     = "ФормаДокументаЗаявлениеОВвозе,ФормаСписка";
...
Функция ПодготовитьТаблицыЗаявленияОВвозеТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	Запрос.УстановитьПараметр("РасширенныйФункционал", ПолучитьФункциональнуюОпцию("РасширенныйФункционал"));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА &РасширенныйФункционал
	|				И Реквизиты.Организация.КрупнейшийНалогоплательщик
	|			ТОГДА Реквизиты.Организация.КрупнейшийНалогоплательщикКПП
	...
	|	Реквизиты.ВалютаДокумента.Код КАК ВалютаДокументаКод,
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		Если СтрокаШапки.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП Тогда
			Продолжить;
		КонецЕсли;
		...
		СведенияОПокупателе = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(СтрокаШапки.Организация, СтрокаШапки.Дата, , , , Истина);
		...
		Иначе
			РеквизитыРаздел1.ПокупательНаимОрг = СведенияОПокупателе.ПолноеНаименование;
		КонецЕсли; 
		...
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			Для каждого СтрокаТоваров Из НайденныеСтрокиТовары Цикл
				...
				Если БезАкцизов Тогда
					...
					Если СтрокаТоваров.П000010000118 = Перечисления.СтавкиНДС.БезНДС Тогда
						МногострочнаяЧасть.Параметры.П000010000116 = НСтр("ru='Льгота'");
						МногострочнаяЧасть.Параметры.П000010000117 = НСтр("ru='Льгота'");
						МногострочнаяЧасть.Параметры.П000010000118 = НСтр("ru='Льгота'");
					Иначе
...
Функция ПодготовитьТаблицыСтатистическойФормыУчетаПеремещенияТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	|	ЗаявлениеОВвозеТоваров.ВидОперации КАК ВидОперации
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров КАК ЗаявлениеОВвозеТоваров
	|ГДЕ
	...
	|	ЗаявлениеОВвозе.НомерСтроки КАК НомерСтроки
	|ИЗ
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			ТаблицаТоварыДляВыгрузки.Колонки.Добавить("НомерДекларацииНаТовары"); // Номер декларации на товары (без порядкового номера товара) для печатной формы.
			...
			Для каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
				...
				УчетНДСФормированиеОтчетности.ЗаполнитьРеквизитыТаможеннойДекларации(СтрокаТовары, НоваяСтрокаВыгрузки);
				УчетНДСФормированиеОтчетности.ЗаполнитьСведенияОПроизводителе(СтрокаТовары, НоваяСтрокаВыгрузки, Правила891, РеквизитыШапка.Дата);
				УчетНДСФормированиеОтчетности.ЗаполнитьСведенияОМаркируемыхТоварах(СтрокаТовары, НоваяСтрокаВыгрузки, Правила891, ШтрихкодыУпаковок);
...
Функция ПолучитьСоответствиеВидовОперацийФормам() Экспорт
	...
	СоответствиеВидовОперацийФормам = Новый Соответствие;
	СоответствиеВидовОперацийФормам.Вставить(Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП, "ФормаДокументаДОПП");
	СоответствиеВидовОперацийФормам.Вставить(Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе, "ФормаДокументаЗаявлениеОВвозе");
		
	Возврат СоответствиеВидовОперацийФормам;
...
Процедура ПроверитьВозможностьВыгрузки_ПараметрыВыгрузки(СтатусПроверки, ДанныеДокумента, СтруктураРеквизитовВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(КодИФНС) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не заполнен код налогового органа. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(СокрЛП(КодИФНС))<> 4 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Неверно заполнен код налогового органа. Требуется заполнить 4 символами в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(НаимОтпр) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнено наименование организации отправителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ИННЮЛОтпр) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнен ИНН организации отправителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(КППОтпр) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнен КПП организации отправителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(ФамилияФЛОтпр) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнена фамилия отправителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ИмяФЛОтпр) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнено имя отправителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ИмпортерСовпадаетСОтправителем Тогда
		...
		Если ИмпортерЯвляетсяЮЛ Тогда
			Если НЕ ЗначениеЗаполнено(НаимИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено наименование организации импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ИННЮЛИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнен ИНН организации импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(КППИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнен КПП организации импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
		Иначе
			Если НЕ ЗначениеЗаполнено(ФамилияФЛИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнена фамилия импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ИмяФЛИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнено имя импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ИННФЛИмпорт) Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = НСтр("ru='Не заполнен ИНН импортера. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
				Сообщение.Сообщить();
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФамилияПодп) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не заполнена фамилия подписанта. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ИмяПодп) Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Не заполнено имя подписанта. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
		Сообщение.Сообщить();
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПрПодп = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовПодп) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнено наименование документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(НомерДовПодп) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнен номер документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(ДатаДовПодп) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не заполнена дата документа, подтверждающего полномочия представителя. Требуется заполнить в форме ""Сведения об отправителе, импортере и подписантах"", вызываемой из командного меню документа.'");
			Сообщение.Сообщить();
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВозможностьВыгрузки_Товары(СтатусПроверки, ДанныеДокумента)
	...
	Для Каждого СтрТабл Из ТаблицаРаздел1 Цикл
		...
		// Здесь упоминаются ставки НДС: Перечисления.СтавкиНДС.НДС10, Перечисления.СтавкиНДС.НДС18, Перечисления.СтавкиНДС.НДС20, Перечисления.СтавкиНДС.НДС22
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|	ЗаявлениеОВвозеТоваров.ВалютаДокумента КАК ВалютаДокумента,
	|	ЗаявлениеОВвозеТоваров.СуммаДокумента КАК СуммаДокумента,
	|	ЗаявлениеОВвозеТоваров.ВидОперации КАК ВидОперации
	|ПОМЕСТИТЬ Реквизиты
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров КАК ЗаявлениеОВвозеТоваров
	...
	|	Реквизиты.СуммаДокумента КАК СуммаДокумента,
	|	ВЫБОР
	|		КОГДА Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ДОПП)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоДОПП
	|ИЗ
...
Функция ТекстЗапросаТаблицаТовары()
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ЗаявлениеТовары.Ссылка КАК Ссылка,
	|	ЗаявлениеТовары.НомерСтроки КАК НомерСтроки,
	|	ЗаявлениеТовары.Номенклатура КАК Номенклатура,
	|	ЗаявлениеТовары.Количество КАК Количество,
	|	ЗаявлениеТовары.ФактурнаяСтоимость КАК ФактурнаяСтоимость,
	|	ЗаявлениеТовары.НалоговаяБазаНДС КАК НалоговаяБазаНДС,
	|	ЗаявлениеТовары.СтавкаНДС КАК СтавкаНДС,
	|	ЗаявлениеТовары.СуммаНДС КАК СуммаНДС,
	|	ЗаявлениеТовары.СчетУчета КАК СчетУчета,
	|	ЗаявлениеТовары.Контрагент КАК Контрагент,
	|	ЗаявлениеТовары.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ЗаявлениеТовары.СчетУчетаНДС КАК СчетУчетаНДС,
	|	ЗаявлениеТовары.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ЗаявлениеТовары.ДокументПартии КАК ДокументПартии,
	|	ВЫБОР 
	|		КОГДА ЗаявлениеТовары.ДокументПартии ССЫЛКА Документ.ПоступлениеТоваровУслуг
	|				И ЗаявлениеТовары.ДокументПартии.СкладВСписке
	|			ТОГДА ЗаявлениеТовары.Склад
	|		ИНАЧЕ ЗаявлениеТовары.ДокументПартии.Склад
	|	КОНЕЦ КАК Склад,
	|	ЗаявлениеТовары.ДокументПартии.ПодразделениеОрганизации КАК Подразделение,
	|	ВЫБОР
	|		КОГДА НЕ &ВедетсяПростойУчетЕНП
	|			ТОГДА ЗаявлениеТовары.Ссылка.Дата
	|		КОГДА ЗаявлениеТовары.ДатаПринятияНаУчет = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЗаявлениеТовары.ДокументПартии.Дата
	|		ИНАЧЕ ЗаявлениеТовары.ДатаПринятияНаУчет
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|	ЗаявлениеТовары.СуммаНДС КАК СуммаНДС,
	|	ЗаявлениеТовары.СчетУчета КАК СчетУчета,
	|	ЗаявлениеТовары.Контрагент КАК Комитент,
	|	ЗаявлениеТовары.ДоговорКонтрагента КАК ДоговорКомитента,
	|	ЗаявлениеТовары.СчетУчетаНДС КАК СчетУчетаНДС,
	|	ЗаявлениеТовары.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ЗаявлениеТовары.ДокументПартии КАК ДокументПартии,
	|	ЗаявлениеТовары.Склад КАК Склад,
	|	ЗаявлениеТовары.Подразделение КАК Подразделение,
	|	ЗаявлениеТовары.ДатаПринятияНаУчет КАК ДатаПринятияНаУчет
	|ПОМЕСТИТЬ ТаблицаТовары
	|ИЗ
	|	&ТаблицаТовары КАК ЗаявлениеТовары";
...
Функция ТекстЗапросаТаблицаНачисленияНДС(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаТовары.ДоговорКомитента КАК ДоговорКомитента,
	...
	|	НАЧАЛОПЕРИОДА(ТаблицаТовары.ДатаПринятияНаУчет, МЕСЯЦ) КАК ДатаПринятияНаУчет
	|ИЗ
...
Функция ТекстЗапросаТаблицаВключенияНДСВСтоимость(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаТовары.ДоговорКомитента,
...
Функция ТекстЗапросаАкцизы(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не Реквизиты.ИмпортПодакцизныхТоваров
	    Или Реквизиты.ЭтоДОПП Тогда
		ПараметрыПроведения.Вставить("ТаблицаАкцизы",             Неопределено);
		ПараметрыПроведения.Вставить("ТаблицаРеквизитыПоАкцизам", Неопределено);
		Возврат "";
	КонецЕсли;

	НомераТаблиц.Вставить("ТаблицаРеквизитыПоАкцизам", НомераТаблиц.Количество());
	...
	|	ЗаявлениеОВвозеТоваров.Контрагент КАК Контрагент,
	|	ЗаявлениеОВвозеТоваров.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	&СчетРасчетовПоАкцизам КАК СчетУчетаРасчетовСКонтрагентом
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров КАК ЗаявлениеОВвозеТоваров
	|ГДЕ
	|	ЗаявлениеОВвозеТоваров.Ссылка = &Ссылка
	...
	|ВЫБРАТЬ
	|	ЗаявлениеОВвозеТоваровТовары.Номенклатура КАК Номенклатура,
	|	ЗаявлениеОВвозеТоваровТовары.ДокументПартии КАК Партия,
	|	ЗаявлениеОВвозеТоваровТовары.СуммаАкциза КАК СуммаАкциза,
	|	ЗаявлениеОВвозеТоваровТовары.СчетАкциза КАК СчетАкциза,
	|	ЗаявлениеОВвозеТоваровТовары.ДокументПартии.Склад КАК Склад,
	|	ЗаявлениеОВвозеТоваровТовары.ДокументПартии.ПодразделениеОрганизации КАК Подразделение,
	|	ЗаявлениеОВвозеТоваровТовары.СчетУчета КАК СчетУчета,
	|	ЗаявлениеОВвозеТоваровТовары.СубконтоАкцизы КАК СубконтоАкцизы,
	|	НАЧАЛОПЕРИОДА(ВЫБОР
	|			КОГДА НЕ &ВедетсяПростойУчетЕНП
	|				ТОГДА ЗаявлениеОВвозеТоваровТовары.Ссылка.Дата
	|			КОГДА ЗаявлениеОВвозеТоваровТовары.ДатаПринятияНаУчет = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ЗаявлениеОВвозеТоваровТовары.ДокументПартии.Дата
	|			ИНАЧЕ ЗаявлениеОВвозеТоваровТовары.ДатаПринятияНаУчет
	|		КОНЕЦ, МЕСЯЦ) КАК ДатаПринятияКУчету
	|ИЗ
	|	Документ.ЗаявлениеОВвозеТоваров.Товары КАК ЗаявлениеОВвозеТоваровТовары
	|ГДЕ
	|	ЗаявлениеОВвозеТоваровТовары.Ссылка = &Ссылка
	|	И ЗаявлениеОВвозеТоваровТовары.СуммаАкциза <> 0
	|	И ЗаявлениеОВвозеТоваровТовары.ПодакцизныйТовар = ИСТИНА
	|	И ЗаявлениеОВвозеТоваровТовары.ДоговорКонтрагента = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)";
	
	Возврат ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
КонецФункции

Функция ТекстЗапросаРезервированиеОбеспечительногоПлатежа(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ЭтоДОПП Тогда
		ПараметрыПроведения.Вставить("ТаблицаРезервированиеОбеспечительногоПлатежа", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ТаблицаРезервированиеОбеспечительногоПлатежа", НомераТаблиц.Количество());
...
Процедура ЗаполнитьВидОперацииПоУмолчанию(Параметры) Экспорт
	...
	|	Документ.ЗаявлениеОВвозеТоваров КАК ЗаявлениеОВвозеТоваров
	|ГДЕ
	|	ЗаявлениеОВвозеТоваров.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВвозТоваровИзЕАЭС.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументОбъект = Неопределено 
		 Или ЗначениеЗаполнено(ДокументОбъект.ВидОперации) Тогда
			ОбъектовОбработано = ОбъектовОбработано - 1;
			Продолжить;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(ДокументОбъект.ВидОперации) Тогда
			ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВвозТоваровИзЕАЭС.ЗаявлениеОВвозе;
		КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.ЗаполнитьВидОперацииПоУмолчанию() не удалось обработать документ ""Ввоз товаров из ЕАЭС"" по причине:
					|%1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ЗаявлениеОВвозеТоваров, Выборка.Ссылка, ТекстСообщения);
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В процедуре Документы.ЗаявлениеОВвозеТоваров.ЗаполнитьВидОперацииПоУмолчанию() не удалось обработать документ ""Ввоз товаров из ЕАЭС"": в %1 из %2 возникли ошибки'"),
				ПроблемныхОбъектов,
				ОбъектовОбработано);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура Документы.ЗаявлениеОВвозеТоваров.ЗаполнитьВидОперацииПоУмолчанию() обработала очередную порцию документов ""Ввоз товаров из ЕАЭС"": %1 документов'"), ОбъектовОбработано));
	КонецЕсли;
