...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаСписаннаяТара         = УчетТоваров.ПодготовитьТаблицуСписанныеТовары(
		ПараметрыПроведения.СписаниеТарыТаблица, ПараметрыПроведения.СписаниеТарыРеквизиты, Отказ);
	...
	ОбщегоНазначенияБПВызовСервера.ЗагрузитьВТаблицуЗначений(ТаблицаСписаннаяТара, ТаблицаСписанныеМатериалы);
	...
	ТаблицыМатериаловПродукцииИП_Тара = Документы.ПередачаМатериаловВЭксплуатацию.ПодготовитьТаблицыМатериаловПродукцииИП(
		ТаблицаСписаннаяТара, ПараметрыПроведения.ПередачаТарыТаблица, ПараметрыПроведения.СписаниеТарыРеквизиты);
	...
	ТаблицыСписанияМПЗИП_Тара = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицыСписанияМПЗ(
		ТаблицыМатериаловПродукцииИП_Тара.ТаблицаМатериалов, ПараметрыПроведения.СписаниеТарыРеквизиты, Отказ);
	...
	ТаблицаПоступлениеПродукцииИП_Тара = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицуПоступлениеПродукции(
		ТаблицыМатериаловПродукцииИП_Тара.ПолученоПродукции, ПараметрыПроведения.СписаниеТарыРеквизиты);
	...
	Если Дата < УчетМатериаловВЭксплуатации.ДатаОтменыСпецодежды() Тогда
	...
	УчетТоваров.СформироватьДвиженияСписаниеТоваров(
		...
		Движения, Отказ);
	УчетТоваров.СформироватьДвиженияПоступлениеТарыВЭксплуатацию(ТаблицаСписаннаяТара,
		ПараметрыПроведения.ПередачаТарыТаблица,
		ПараметрыПроведения.ПередачаТарыРеквизиты, Движения, Отказ);
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияСписаниеМПЗ(
		ТаблицыСписанияМПЗИП_Тара, ПараметрыПроведения.СписаниеТарыРеквизиты, Движения, Отказ);
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияПоступлениеПродукции(
		ТаблицаПоступлениеПродукцииИП_Тара, ТаблицыСписанияМПЗИП_Тара.СтоимостьПродукции, 
	...
	ТаблицаРегистрации2 = РаботаСПоследовательностями.ПодготовитьТаблицуСчетовТоваровДляАнализа(
		ТаблицаРегистрации, ТаблицаСписанныйИнвентарь);
	РаботаСПоследовательностями.ЗарегистрироватьОтложенныеРасчетыВПоследовательности(
		ЭтотОбъект, Отказ, ,
		РаботаСПоследовательностями.ПодготовитьТаблицуСчетовТоваровДляАнализа(ТаблицаРегистрации2, ТаблицаСписаннаяТара));
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ДоступнаАмортизацияСпецодежды = Дата < УчетМатериаловВЭксплуатации.ДатаОтменыСпецодежды();
	
	Если ДоступнаАмортизацияСпецодежды Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Спецодежда.СпособОтраженияРасходов");
		МассивНепроверяемыхРеквизитов.Добавить("Спецоснастка.СпособОтраженияРасходов");
	Иначе
		МассивНепроверяемыхРеквизитов.Добавить("Спецодежда.НазначениеИспользования");
		МассивНепроверяемыхРеквизитов.Добавить("Спецоснастка.НазначениеИспользования");
		МассивНепроверяемыхРеквизитов.Добавить("Спецодежда.СчетПередачи");
		МассивНепроверяемыхРеквизитов.Добавить("Спецоснастка.СчетПередачи");
	КонецЕсли;
		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			...
			Если ЗначениеЗаполнено(ТекСтрокаТовары.СчетУчета)
				И (ТекСтрокаТовары.СчетУчета = Счет10_09 
					ИЛИ БухгалтерскийУчетПовтИсп.СчетВИерархии(ТекСтрокаТовары.СчетУчета, Счет10_09)
					ИЛИ ТекСтрокаТовары.СчетУчета = ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов) Тогда
			...
			ИначеЕсли СчетаУчета<> Неопределено
				И (БухгалтерскийУчетПовтИсп.СчетВИерархии(ТекСтрокаТовары.СчетУчета, ПланыСчетов.Хозрасчетный.Тара)
				ИЛИ БухгалтерскийУчетПовтИсп.СчетВИерархии(ТекСтрокаТовары.СчетУчета, ПланыСчетов.Хозрасчетный.ТараПодТоваромИПорожняя)) Тогда
...
Функция ВыполнитьЗпросПроверкиТабличныхЧастей()
	...
	|	И Спецодежда.НазначениеИспользования <> ЗНАЧЕНИЕ(Справочник.НазначенияИспользованияНоменклатуры.ПустаяСсылка)
	...
	|	И Спецоснастка.НазначениеИспользования <> ЗНАЧЕНИЕ(Справочник.НазначенияИспользованияНоменклатуры.ПустаяСсылка)
	|	И (
