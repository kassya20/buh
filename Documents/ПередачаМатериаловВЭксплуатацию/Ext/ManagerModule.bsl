...
Процедура ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, ИмяТабличнойЧасти, ЗаполнятьТолькоНезаполенные = Ложь) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти Из ТабличнаяЧасть Цикл
		...
		ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабличнойЧасти, СчетаУчета, ЗаполнятьТолькоНезаполенные);
	КонецЦикла;
...
Процедура ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТабличнойЧасти, ИмяТабличнойЧасти, СведенияОНоменклатуре, ЗаполнятьТолькоНезаполенные = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(СчетаУчета.СчетУчета) Тогда
		Если Не ЗаполнятьТолькоНезаполенные
			Или Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.СчетУчета) Тогда
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Если Выборка.Период < УчетМатериаловВЭксплуатации.ДатаОтменыСпецодежды() Тогда
		
		ТекстЗапроса = ТекстЗапросаСписаниеСпецодежды(НомераТаблиц)
	...
	Иначе
		...
		ТекстЗапроса = ТекстЗапросаСписаниеСпецодежды2021(НомераТаблиц)
			+ ТекстЗапросаСписаниеСпецоснастки2021(НомераТаблиц)
			+ ТекстЗапросаПередачаСпецодежды2021(НомераТаблиц)
			+ ТекстЗапросаПередачаСпецоснастки2021(НомераТаблиц);
	...
	Запрос.Текст = ТекстЗапроса
		...
		+ ТекстЗапросаПередачаТары(НомераТаблиц)
...
Функция ТекстЗапросаСписаниеСпецодежды2021(НомераТаблиц)
	...
	|		ПО ТаблицаСпецодежда.СпособОтраженияРасходов = СпособыОтраженияРасходов.Ссылка
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаСписаниеСпецоснастки2021(НомераТаблиц)
	...
	|		ПО ТаблицаСпецоснастка.Ссылка = Реквизиты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОтраженияРасходовПоАмортизации.Способы КАК СпособыОтраженияРасходов
	|		ПО ТаблицаСпецоснастка.СпособОтраженияРасходов = СпособыОтраженияРасходов.Ссылка
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаСписаниеТары(НомераТаблиц)
	...
	НомераТаблиц.Вставить("СписаниеТарыТаблица", НомераТаблиц.Количество());
	...
	|	""Возвратная тара"" КАК СинонимСписка,
	|	Реквизиты.Дата КАК Период,
	...
	|	0 КАК СуммаРасчетовСКомитентом
	|ИЗ
	|	Документ.ПередачаМатериаловВЭксплуатацию.ВозвратнаяТара КАК ТаблицаВозвратнаяТара
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаМатериаловВЭксплуатацию КАК Реквизиты
	|		ПО ТаблицаВозвратнаяТара.Ссылка = Реквизиты.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОтраженияРасходовПоАмортизации.Способы КАК СпособыОтраженияРасходов
	|		ПО ТаблицаВозвратнаяТара.СпособОтраженияРасходов = СпособыОтраженияРасходов.Ссылка
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТекстЗапросаПередачаСпецодежды2021(НомераТаблиц)
	...
	|	ТаблицаСпецодежда.ФизЛицо КАК ФизЛицо,
	|	Реквизиты.Местонахождение КАК Подразделение,
	|	ТаблицаСпецодежда.СпособОтраженияРасходов КАК СпособОтраженияРасходов,
	|	ТаблицаСпецодежда.Количество КАК КоличествоМЦ
	|ИЗ
	|	Документ.ПередачаМатериаловВЭксплуатацию.Спецодежда КАК ТаблицаСпецодежда
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаМатериаловВЭксплуатацию КАК Реквизиты
...
Функция ТекстЗапросаПередачаСпецоснастки2021(НомераТаблиц)
	...
	|	НЕОПРЕДЕЛЕНО КАК ФизЛицо,
	|	Реквизиты.Местонахождение КАК Подразделение,
	|	ТаблицаСпецоснастка.СпособОтраженияРасходов КАК СпособОтраженияРасходов,
	|	ТаблицаСпецоснастка.Количество КАК КоличествоМЦ
	|ИЗ
	|	Документ.ПередачаМатериаловВЭксплуатацию.Спецоснастка КАК ТаблицаСпецоснастка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаМатериаловВЭксплуатацию КАК Реквизиты
...
Функция ТекстЗапросаПередачаТары(НомераТаблиц)
	
	НомераТаблиц.Вставить("ПередачаТарыРеквизиты", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ПередачаТарыТаблица", НомераТаблиц.Количество());
	...
	|	НЕОПРЕДЕЛЕНО КАК СпособПогашенияСтоимости,
	|	ТаблицаВозвратнаяТара.СпособОтраженияРасходов,
	|	""Передача тары в эксплуатацию"" КАК Содержание,
...
Функция ТекстЗапросаНДС(НомераТаблиц)
	...
	НомераТаблиц.Вставить("ТараНДС",         НомераТаблиц.Количество());
	...
	|ВЫБРАТЬ
	|	""ВозвратнаяТара"" КАК ИмяСписка,
	|	ТаблицаВозвратнаяТара.НомерСтроки КАК НомерСтрокиДокумента,
	|	ТаблицаВозвратнаяТара.Номенклатура КАК Номенклатура,
	|	ТаблицаВозвратнаяТара.СчетУчета КАК СчетУчета,
	|	ТаблицаВозвратнаяТара.СпособОтраженияРасходов КАК СпособОтраженияРасходов,
	|	ТаблицаВозвратнаяТара.Количество КАК Количество
	|ИЗ
	|	Документ.ПередачаМатериаловВЭксплуатацию.ВозвратнаяТара КАК ТаблицаВозвратнаяТара
	|ГДЕ
...
Функция ПечатьМ11(МассивОбъектов, ОбъектыПечати)
	...
	|		4,
	|		МИНИМУМ(ВозвратнаяТара.НомерСтроки),
	|		ВозвратнаяТара.Номенклатура,
	|		ВозвратнаяТара.СчетУчета,
	|		СУММА(ВозвратнаяТара.Количество)
	|	ИЗ
	|		Документ.ПередачаМатериаловВЭксплуатацию.ВозвратнаяТара КАК ВозвратнаяТара
	|	ГДЕ
	|		ВозвратнаяТара.Ссылка В(&МассивОбъектов)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВозвратнаяТара.Ссылка,
	|		ВозвратнаяТара.Номенклатура,
