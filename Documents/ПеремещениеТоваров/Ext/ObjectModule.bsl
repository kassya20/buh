...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЭтоНовый()
		...
		ВидОперации = Перечисления.ВидыОперацийПеремещениеТоваров.ПередачаМеждуСотрудниками;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаСписанныеМатериалыЗаказчика = УчетТоваров.ПодготовитьТаблицуСписанныеТовары(
		ПараметрыПроведения.ТаблицаМатериалыЗаказчика, ПараметрыПроведения.ПеремещениеРеквизиты, Отказ);
		
	ТаблицаСписанныеССотрудникаМатериалы = УчетМатериаловВЭксплуатации.ПодготовитьТаблицуСписанныеССотрудникаМатериалы(
		ПараметрыПроведения.ПеремещениеМатериаловСотрудниковМатериалы,
		ПараметрыПроведения.ПеремещениеМатериаловСотрудниковРеквизиты,
		Отказ);
	...
	ТаблицыПеремещенияТоваровИП = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицыПеремещенияТоваров(
		ТаблицаСписанныеТовары, ПараметрыПроведения.ПеремещениеРеквизиты, Отказ);
	
	СписанныеВНТТМПЗИП = Документы.ПеремещениеТоваров.ПодготовитьТаблицуСписанныеВНТТМПЗИП(
	...
	ТаблицыСписанияТоваровИП = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицыСписанияМПЗ(
		СписанныеВНТТМПЗИП, ПараметрыПроведения.ИПРеквизиты, Отказ);
	
	ВозвращенныеИзНТТМПЗИП = Документы.ПеремещениеТоваров.ПодготовитьТаблицуВозвращенныеИзНТТМПЗИП(ТаблицаСписанныеТоварыНТТ, ПараметрыПроведения.ИПРеквизиты);
	
	ТаблицаТоваровИП = УчетДоходовИРасходовПредпринимателя.ПодготовитьТаблицуПоступленияМПЗ(
		ВозвращенныеИзНТТМПЗИП, ПараметрыПроведения.ИПРеквизиты);
	...
	УчетТоваров.СформироватьДвиженияПеремещениеТоваров(ТаблицаСписанныеТоварыНаКомиссии,
		ПараметрыПроведения.ПеремещениеРеквизиты, Движения, Отказ);
	УчетТоваров.СформироватьДвиженияПеремещениеТоваров(ТаблицаСписанныеМатериалыЗаказчика,
		ПараметрыПроведения.МатериалыЗаказчикаРеквизиты, Движения, Отказ);
	...
	// Перемещение материалов сотрудников
	УчетМатериаловВЭксплуатации.СформироватьДвиженияПеремещениеМатериаловСотрудников(
		ТаблицаСписанныеССотрудникаМатериалы,
		ПараметрыПроведения.ПеремещениеМатериаловСотрудниковРеквизиты, Движения, Отказ);
	
	// Структура таблиц для отражения в налоговом учете УСН
	СтруктураТаблицУСН = Новый Структура("ТаблицаТМЦ, ТаблицаСписанныеПартииНДС", ТаблицаСписанныеТовары, СписанныеПартииНДС);
	
	НалоговыйУчетУСН.СформироватьДвиженияУСН(ЭтотОбъект, СтруктураТаблицУСН);
	
	// Учет доходов и расходов ИП
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияПеремещенияТоваров(
		ТаблицыПеремещенияТоваровИП, ПараметрыПроведения.ПеремещениеРеквизиты, Движения, Отказ);
	...
	УчетДоходовИРасходовПредпринимателя.СформироватьДвиженияПоступлениеМПЗ(
		СтруктураТаблицМПЗ, , , ПараметрыПроведения.ИПРеквизиты, Движения, Отказ);
...
Процедура ЗаполнитьПоПоступлению(ДанныеЗаполнения)
	...
	|	И ПоступлениеТоваровУслугТовары.Ссылка.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	ВыборкаМатериалыЗаказчика = Результат[4].Выбрать();
	Пока ВыборкаМатериалыЗаказчика.Следующий() Цикл
		НоваяСтрока	= МатериалыЗаказчика.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаМатериалыЗаказчика);
	КонецЦикла;
