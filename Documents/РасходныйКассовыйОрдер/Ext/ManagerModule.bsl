...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "БанковскийСчет", "СчетОрганизации");
...
Процедура ЗаполнитьПоИнвентаризацииКассы(Объект, Основание) Экспорт
	
	Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ПрочийРасход;
	Объект.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.НедостачиИПотериОтПорчиЦенностей;
	Объект.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию("НедостачаДенежныхСредств", Объект.Дата);

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетаКасса", БухгалтерскийУчет.СчетаНаличныхДенежныхСредств());
	Запрос.УстановитьПараметр("Основание", Основание);

	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ИнвентаризацияКассыЦенности.СчетУчета КАК СчетКасса,
		|	СУММА(ИнвентаризацияКассыЦенности.ВалютнаяСуммаУчет - ИнвентаризацияКассыЦенности.ВалютнаяСумма) КАК СуммаДокумента,
		...
		|	ИнвентаризацияКассыЦенности.Ссылка.ПодразделениеОрганизации КАК ПодразделениеДт
		|ИЗ
		|	Документ.ИнвентаризацияКассы.Ценности КАК ИнвентаризацияКассыЦенности
		|ГДЕ
		|	ИнвентаризацияКассыЦенности.СчетУчета В(&СчетаКасса)
	...
	Если ВыборкаРасхождений.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(Объект, ВыборкаРасхождений);
	КонецЕсли;
...
Функция ПодготовитьСообщенияПроверкиЗаполненияРасшифровкаПлатежа(Объект, ФормаРасшифровкаПлатежа = Неопределено, Отказ, ПроверяемыеРеквизиты, ПроверятьСчетаУчета = Истина) Экспорт
	...
	Если ЕстьРасшифровкаПлатежа(ВидОперации) Тогда
		...
		Для каждого СтрокаПлатежа Из РасшифровкаПлатежа Цикл
			...
			Если ПрименяетсяУСНДоходыМинусРасходы И УчетУСН.ЕстьПрочиеРасчетыПоРасходам(ВидОперации) Тогда
...
Функция ТекстЗапросаРасходыНаУплатуПроцентовПоЗаймам(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидОперации <> Перечисления.ВидыОперацийРКО.ВозвратЗайма Тогда
...
Процедура ПодготовитьПараметрыРеквизитыДокумента(Запрос, ПараметрыПроведения, Отказ)
	...
	|	Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда) КАК ЭтоВыплатаПоДоговоруПодряда,
	|	ВЫБОР
...
Функция ПолучитьТаблицыОплатыДоговоровПодряда(ПараметрыПроведения, ДокументСсылка)
	...
	ДанныеДокумента = Новый Структура("СтатьяДвиженияДенежныхСредств,
		|Подразделение, СчетУчета, ВидОперации,
		|ФизическоеЛицо, ПлатежнаяВедомость");
...
Процедура ЗаполнитьОтражениеВУСНОтложенно(Параметры) Экспорт
	...
	ВидыОперацийПрочиеРасчеты.Добавить(Перечисления.ВидыОперацийРКО.РасчетыПоКредитамИЗаймам);
	ВидыОперацийПрочиеРасчеты.Добавить(Перечисления.ВидыОперацийРКО.ВозвратЗайма);
	ВидыОперацийПрочиеРасчеты.Добавить(Перечисления.ВидыОперацийРКО.ВозвратКредита);
	ВидыОперацийПрочиеРасчеты.Добавить(Перечисления.ВидыОперацийРКО.ВыдачаЗаймаКонтрагенту);
	...
	|			ПО РасходныйКассовыйОрдер.Организация = ПериодыПримененияУСНДоходыМинусРасходы.Организация
	|				И РасходныйКассовыйОрдер.Дата >= ПериодыПримененияУСНДоходыМинусРасходы.НачалоПериода
	|				И РасходныйКассовыйОрдер.Дата <= ПериодыПримененияУСНДоходыМинусРасходы.КонецПериода
	|	ГДЕ
	|		РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ПрочийРасход)
	|		И РасходныйКассовыйОрдер.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ ПЕРВЫЕ 1000
	...
	|		РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.УплатаНалога)
	|		И РасходныйКассовыйОрдер.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|		И РасходныйКассовыйОрдер.Налог.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ПрочиеНалогиИСборы)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ ПЕРВЫЕ 1000
	...
	|				И РасходныйКассовыйОрдер.Дата <= ПериодыПримененияУСНДоходыМинусРасходы.КонецПериода
	|	ГДЕ
	|		РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда)
	|		И РасходныйКассовыйОрдер.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	...
	|		Документ.РасходныйКассовыйОрдер.РасшифровкаПлатежа КАК РасходныйКассовыйОрдерРасшифровкаПлатежа
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПериодыПримененияУСНДоходыМинусРасходы КАК ПериодыПримененияУСНДоходыМинусРасходы
	|			ПО РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.Организация = ПериодыПримененияУСНДоходыМинусРасходы.Организация
	|				И РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.Дата >= ПериодыПримененияУСНДоходыМинусРасходы.НачалоПериода
	|				И РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.Дата <= ПериодыПримененияУСНДоходыМинусРасходы.КонецПериода
	|	ГДЕ
	|		РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.ВидОперации В(&ВидыОперацийПрочиеРасчеты)
	|		И РасходныйКассовыйОрдерРасшифровкаПлатежа.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|	
	|	УПОРЯДОЧИТЬ ПО
	|		ДатаДокумента УБЫВ,
	...
	ИмяПроцедуры = "Документы.РасходныйКассовыйОрдер.ЗаполнитьОтражениеВУСНОтложенно";
		...
		Попытка
			...
			Иначе
				...
				ИначеЕсли ДокументОбъект.УдалитьРасходыЕНВД_УСН Тогда
					...
					Если ДокументОбъект.Графа7_УСН <> СуммаПлатежаРуб Тогда
						ДокументОбъект.РучнаяКорректировка = Истина;
					КонецЕсли;
				Иначе
...
Функция ОтражениеВУСНЗаполнено(Параметры) Экспорт
	...
	Если ТипЗнч(Параметры.Данные) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
		
		Документ = Параметры.Данные;
		ОсновныеДанные = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документ, "Организация, Дата");
		
	ИначеЕсли ТипЗнч(Параметры.Данные) = Тип("ДокументОбъект.РасходныйКассовыйОрдер")
	...
	|	И РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ПрочийРасход)
	|	И РасходныйКассовыйОрдер.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ГДЕ
	|	РасходныйКассовыйОрдер.Ссылка = &Ссылка
	|	И РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда)
	|	И РасходныйКассовыйОрдер.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)
	|	И РасходныйКассовыйОрдер.ПлатежнаяВедомость = НЕОПРЕДЕЛЕНО
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	И РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.ВидОперации В(&ВидыОперацийПрочиеРасчеты)
	|	И РасходныйКассовыйОрдерРасшифровкаПлатежа.ОтражениеВУСН = ЗНАЧЕНИЕ(Перечисление.ОтражениеВУСН.ПустаяСсылка)";
...
Функция ОпределитьКодыСчетСубсчет(Шапка)
	...
	ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратРозничномуПокупателю Тогда
		СтрокаДебет = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Шапка.СчетУчетаРасчетовСКонтрагентом).Код;
		// При проведении возврата розничной выручки данные из реквизита СчетУчетаРасчетовСКонтрагентом в проводках не используются.
		// Проводок в этом случае в документе может вообще не быть - они делаются в другом документе - ОРП,
		// а РКО нужен только для отражения в Кассовой книге.
		...
		Если СтрНачинаетсяС(СтрокаДебет, "90.01") Тогда
			СтрокаДебет = "90.01";
		КонецЕсли;
...
Функция ПечатьПД4(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Если Не ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяСФР(РеквизитыПлатежа.КодБК) Тогда
...
Функция СчетДоходовИзОтчетаОРозничныхПродажах(Основание)
	Запрос = Новый Запрос;
	...
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Возвраты КАК ОтчетОРозничныхПродажахТовары
	|ГДЕ
	...
	Возврат РезультатЗапроса.СчетДоходов;
КонецФункции
...
Процедура ЗаполнитьПоОтчетуОРозничныхПродажах(Объект, Основание) Экспорт
	...
	СчетДоходовОРП = СчетДоходовИзОтчетаОРозничныхПродажах(Основание);
	Если ЗначениеЗаполнено(СчетДоходовОРП) Тогда
		Объект.СчетУчетаРасчетовСКонтрагентом = СчетДоходовОРП;
	Иначе
	...
	// Вычитаем оплату по безналичному расчету и прибавляем возвраты предоплат
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	СУММА(ОтчетОРозничныхПродажахПредоплаты.СуммаВозврат) КАК СуммаПлатежа,
	...
	|ГДЕ
	|	ОтчетОРозничныхПродажахПредоплаты.Ссылка = &Основание
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетОРозничныхПродажахПредоплаты.СтавкаНДС
	|
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(СУММА(ОтчетОРозничныхПродажахПредоплаты.СуммаВозврат), 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(СУММА(ОтчетОРозничныхПродажахОплата.СуммаОплаты), 0) > 0");
	...
	ВыборкаВозвратыПредоплат = Результат[0].Выбрать();
	Пока ВыборкаВОзвратыПредоплат.Следующий() Цикл
	
		НоваяСтрока = ТаблицаСуммОснования.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаВозвратыПредоплат);
		НоваяСтрока.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
			НоваяСтрока.СуммаПлатежа, Истина, УчетНДСПереопределяемый.ПолучитьСтавкуНДС(НоваяСтрока.СтавкаНДС))
