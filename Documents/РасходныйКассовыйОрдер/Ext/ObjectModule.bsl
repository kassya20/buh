...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям
		...
		Ведомости = ОбщегоНазначения.ВыгрузитьКолонку(ВыплатаЗаработнойПлаты, "Ведомость", Истина);
		Если Ведомости.Количество() > 0 Тогда
			...
			Если ВыплатаЗаработнойПлаты.Итог("СуммаКВыплате") <> СуммаДокумента
	...
	Если (ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику
		...
		Если ЗначениеЗаполнено(ПлатежнаяВедомость) И ЗначениеЗаполнено(Контрагент) Тогда
			...
			Если ТаблицаВедомостей.Количество() <> 0 Тогда
				СуммаПоВедомости = ТаблицаВедомостей.Итог("СуммаКВыплате") + ТаблицаВедомостей.Итог("КомпенсацияЗаЗадержкуЗарплаты");
			Иначе
				СуммаПоВедомости = 0;
			КонецЕсли;
...
Процедура ПроверитьОбновитьРеквизитыПлатежаВБюджет(ОбъектКопирования)
	...
	ЭтоУплатаОпределенногоНалога = ВидОперации = Перечисления.ВидыОперацийРКО.УплатаНалога
	...
	Если Не ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(Дата)
		Или ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(
			ОбъектКопирования.Дата) = ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(Дата)
...
Процедура ЗаполнитьПоВедомостям(ПлатежныеВедомости)
	...
	ЭтоВыплатаПоГПХ = Ложь;
	Если УчетЗарплаты.ВедомостьПоДоговоруПодряда(ПлатежнаяВедомость) Тогда
		ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда;
		ЭтоВыплатаПоГПХ = Истина;
	КонецЕсли;
	...
	ИсходнаяТаблицаВедомостей = ТаблицаВедомостей.Скопировать();
	
	Если ВидОперации <> Перечисления.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда Тогда
		ТаблицаВедомостей.Свернуть("Ведомость", "СуммаКВыплате, КомпенсацияЗаЗадержкуЗарплаты");
	КонецЕсли;
	
	ОднаВедомость = ТаблицаВедомостей.Количество() = 1;
	ОдинСотрудник = ИсходнаяТаблицаВедомостей.Количество() = 1;
	...
	Иначе
		Для каждого СтрокаВедомости Из ТаблицаВедомостей Цикл
			Если УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если Не ОдинСотрудник И ЭтоВыплатаПоГПХ Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документ %1 оформлен по списку ДГПХ!'"), СтрокаВедомости.Ведомость);
				ВызватьИсключение ТекстСообщения;
			КонецЕсли;
			...
			Если ОднаВедомость Или ЭтоВыплатаПоГПХ Тогда
				Основание = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(
					СтрокаВедомость.Ведомость, СокрЛП(СтрокаВедомость.Ведомость.Метаданные().Представление()));
				Если ОдинСотрудник Или ЭтоВыплатаПоГПХ Тогда
					ПараметрыОбъекта = Новый Структура;
					ПараметрыОбъекта.Вставить("Организация", Организация);
					ПараметрыОбъекта.Вставить("ВыдатьФизЛицо",
						?(ЭтоВыплатаПоГПХ, СтрокаВедомости.ФизическоеЛицо, ИсходнаяТаблицаВедомостей[0].ФизическоеЛицо));
					ПараметрыОбъекта.Вставить("Дата", ОбщегоНазначения.ТекущаяДатаПользователя());
					ДанныеФизЛица = РасходныйКассовыйОрдерФормы.НовыеПараметрыПриВыбореВыдать(ПараметрыОбъекта);
					Выдать        = ДанныеФизЛица.Выдать;
					ПоДокументу   = ДанныеФизЛица.ПоДокументу;
				КонецЕсли;
			КонецЕсли;
			...
			Если ЭтоВыплатаПоГПХ Тогда
				...
				СуммаДокумента = СтрокаВедомость.СуммаКВыплате;
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
		...
		Для каждого СтрокаПлатеж Из ТаблицаПлатежей Цикл
			...
			Если ЗначениеЗаполнено(ДоговорКонтрагента)
				...
				Если Основание.ВалютаДокумента = ВалютаРегламентированногоУчета Тогда
					...
					СтрокаПлатеж.СуммаНДС = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаПлатеж.СуммаНДС,
						ВалютаРегламентированногоУчета, ВалютаВзаиморасчетовДоговора,
						1, Основание.КурсВзаиморасчетов, 1, Основание.КратностьВзаиморасчетов);
				КонецЕсли;
	...
	ИначеЕсли ТипЗначенияОснования = Тип("ДокументСсылка.ИнвентаризацияКассы") Тогда
		Документы.РасходныйКассовыйОрдер.ЗаполнитьПоИнвентаризацииКассы(ЭтотОбъект, Основание);
	КонецЕсли;
...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения)
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) И Не ЗначениеЗаполнено(ВалютаДокумента) Тогда
