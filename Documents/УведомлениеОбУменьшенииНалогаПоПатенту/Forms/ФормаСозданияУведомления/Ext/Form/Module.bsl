...
Процедура ОткрытьПомощникПодготовкиУведомления()
	...
	Если ИмяОтчета = "РегламентированноеУведомлениеУменьшениеНалогаНаСтраховыеВзносы" Тогда
		...
		Если ВыбранаАктуальнаяФормаУведомления(ПолеРедакцияФормы) Тогда
			...
			ПараметрыПомощника = Новый Структура("Организация", Организация);
			
			ОткрытьФорму("Обработка.ПомощникЗаполненияУведомленияОбУменьшенииНалогаПоПатенту.Форма", ПараметрыПомощника);
