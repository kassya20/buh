...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если ПараметрыПроверки = Неопределено Тогда
		...
		ТекстСообщения = ТекстОшибкиНеНайденыНастройки();
		ФинОтчетностьВБанкиКлиент.ПоказатьОшибку(ТекстСообщения, ЭтотОбъект);
		Возврат;
...
Функция РезультатФоновогоЗадания(Результат, СохранитьАдресРезультата = Ложь)
	...
	Если РезультатДляКлиента = Неопределено Тогда
...
Функция НайтиОписаниеПоВидуОтчета(ВидОтчета, ОписаниеВидовОтчетов, Период = '00010101')

	Если НЕ ЗначениеЗаполнено(ВидОтчета) ИЛИ НЕ ЗначениеЗаполнено(ОписаниеВидовОтчетов) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// Исключение для декларации 3-НДФЛ: для разных периодов действует разная версия рег.отчета
	Если СтрНайти(ВидОтчета, "3НДФЛ") Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("ВидОтчета", ВидОтчета);
		Отбор.Вставить("ИсточникОтчетаБРО", ПомощникЗаполнения3НДФЛ.ИмяРегламентированногоОтчета(Период));
		Возврат ОписаниеВидовОтчетов.НайтиСтроки(Отбор)[0];
	КонецЕсли;
...
Функция ПараметрыФормыВыбораРегламентированногоОтчета(ИдентификаторСтрокиДерева)
	...
	Если ЗначениеЗаполнено(СтрокаДереваОтчетов.ВидОтчета) Тогда
		...
		СтрокаВидаОтчета = НайтиОписаниеПоВидуОтчета(СтрокаДереваОтчетов.ВидОтчета,
			ОписаниеВидовОтчетов,
			СтрокаДереваОтчетов.ДатаНачала);
		...
		Для каждого СтрокаОтчета Из СтрокаВидаОтчетов.ПодготовленныеОтчеты Цикл
			...
			Если ЗначениеЗаполнено(СтрокаОтчета.УточнениеНаименования) Тогда
				...
				Если ГруппаПоУточнениюНаименования = Неопределено Тогда
					...
					Если ЗначениеЗаполнено(СтрокаНастройки.ПодробноеПредставление) Тогда
						// Запомним, что у отчета есть описание с порядком заполнения.
						ГруппаПоУточнениюНаименования.Описание = ДействиеОписание();
					КонецЕсли;
...
Процедура СформироватьПредставлениеОтчета(СтрокаДереваОтчетов, НастройкиКомплекта, ОписаниеВидовОтчетов)
	...
	СтрокаВидаОтчета = НайтиОписаниеПоВидуОтчета(СтрокаДереваОтчетов.ВидОтчета, ОписаниеВидовОтчетов, Объект.Дата);
	...
	Если ЗначениеЗаполнено(СтрокаДереваОтчетов.ИдентификаторСтрокиКомплекта) Тогда
		...
		ПредставлениеЗаданоВРодительскойСтроке = (ЭтоОтчетПоФормеБанка
			...
			Или (ЗначениеЗаполнено(СтрокаДереваОтчетов.Описание) 
					И ПолеУказаноВРодительскойСтроке(СтрокаДереваОтчетов, "Описание", СтрокаДереваОтчетов.Описание));
...
Процедура ПодписатьФинОтчет(ДополнительныеПараметры)
	...
	Если ПараметрыПодписания = Неопределено Тогда
...
Функция НачатьПодписаниеНаСервере(Знач УникальныйИдентификатор, Знач ПараметрыПодписания)
	...
	Если ПараметрыПодписания.ДействиеПослеПодписания = "Отправить" Тогда
		...
		ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Отправка пакета'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ФинОтчетВБанк.ПодписатьИОтправитьВБанкВФоне",
			ПараметрыПодписания,
			ПараметрыВыполнения);
	...
	Иначе
		...
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.ФинОтчетВБанк.ПодписатьОтчетыВФоне",
			ПараметрыПодписания,
			ПараметрыВыполнения);
...
Функция СостояниеПередВыгрузкой()
	...
	НастройкиКомплекта = НастройкиКомплекта();
	Если НастройкиКомплекта = Неопределено Тогда
		СделатьПередВыгрузкой.ТекстСообщения = ТекстОшибкиНеНайденыНастройки();
		Возврат СделатьПередВыгрузкой;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ТекстОшибкиНеНайденыНастройки()

	Возврат НСтр("ru = 'Не найдены настройки выгрузки и подписания комплекта отчетности.
		|Вероятно, данный комплект отчетности более не принимается банком.'");
