...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПодготовитьАрхивВФоне", Истина);
	Методы.Вставить("ПодписатьИОтправитьВБанкВФоне", Истина);
	Методы.Вставить("ЗаполнитьОтчетыВФоне", Истина);
	Методы.Вставить("ЗагрузитьОтчетыВФоне", Истина);
	Методы.Вставить("ПроверитьГотовностьКОтправкеВФоне", Истина);
	Методы.Вставить("ПодписатьОтчетыВФоне", Истина);
	Методы.Вставить("ПрикрепитьПодписиВФоне", Истина);
	Методы.Вставить("ПодготовитьПодписываемыеОтчетыВФоне", Истина);
	Методы.Вставить("ОтправитьВБанкВФоне", Истина);
	
	Методы.Вставить("ЗаполнитьНедостающиеРеквизитыПериодОтложенно");
...
Функция ПараметрыПодписания(Объект, КлючСтрокиСубъекта, НастройкиКомплекта) Экспорт
	...
	ПараметрыПодписания.Вставить("Сейчас",                  ОбщегоНазначенияБП.ТекущаяДатаНаСервере()); // чтобы одинаковые дату и время использовать во всех процедурах
...
Функция ГотовностьКОтправке(ПараметрыПроверки, ТолькоПроверка, Прогрессор) Экспорт
	...
	Пока ДанныеДляПроверки.Следующий() Цикл
		...
		Если Отчет.ВариантВыгрузки = "Файл" Тогда // отдельный файл и его подпись
			...
			ПараметрыПомещения.Сжать = ?(ЗаполнениеФинОтчетностиВБанки.ЭтоИмяФайлаАрхива(Отчет.Представление), 
				Ложь, ПараметрыПроверки.СжиматьФайлы);
...
Процедура ДобавитьОтчетДекларацияНДС(ПараметрыЗаполнения, СтрокаНастройкиКомплекта, СоставОтчетности, ДатаНачала, ДатаОкончания)
	...
	ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(ПараметрыЗаполнения.Субъект, НачалоПериода)
		И УчетнаяПолитика.ПлательщикНДС(ПараметрыЗаполнения.Субъект, КонецПериода);
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыЗаполнения.Субъект, НачалоПериода)
			И УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыЗаполнения.Субъект, КонецПериода);
...
Процедура ПодобратьРегламентированныеОтчеты(ПараметрыЗаполнения, РезультатЗаполнения)
	...
	Если ПараметрыПоискаНалоговыхДеклараций.Количество() > 0 Тогда
		...
		ИгнорируемыеКолонки.Вставить("МаксВерсияФормыРеглОтчета");
	...
	Пока Подписи.Количество() > 0 Цикл
		...
		Если РезультатПодписания.КлючСтрокиОтчета <> ТекущийКлюч 
			...
			Если ТекущийКлюч <> 0 Тогда
				...
				ПараметрыПомещения.Сжать = ?(КонтекстПодписания.ЭтоАрхив 
					Или ЗаполнениеФинОтчетностиВБанки.ЭтоИмяФайлаАрхива(ПутьКПоследнемуФайлу), Ложь, ПараметрыПодписания.СжиматьФайлы);
...
Процедура МашиночитаемыеДоверенностиПакета(ПараметрыПодписания, РезультатПодготовки)
	...
	Пока ИндексТекущейДоверенности < КоличествоДоверенностей Цикл
		...
		ДатаОкончанияСрокаДействия = ?(ТипЗнч(ТекущаяДоверенность.Доверенность)=Тип("СправочникСсылка.МЧД003"), "СрокДействия", "ДатаОкончания");
		...
		Если СокращенныйПериодБезУИД Тогда
			...
			ПараметрыФайлаОтчета.ПрефиксФайла           = "ON_DOV";
		...
		Если ТипыСправочниковМЧД.РегламентированнаяОтчетность.Найти(ТипСправочникаМЧД) <> Неопределено Тогда
			...
			Если ТипЗнч(РезультатВыгрузки) = Тип("Массив") Тогда
				...
				Для Каждого ФайлРезультатаВыгрузки Из РезультатВыгрузки Цикл
					...
					ИмяФайлаСРасширением = ИмяФайлаДоверенности(
						СокращенныйПериодБезУИД, ПараметрыФайлаОтчета, РасширениеФайла, ФайлРезультатаВыгрузки.Имя);
			...
			Иначе
				...
				ИмяФайлаСРасширением = ИмяФайлаДоверенности(
						СокращенныйПериодБезУИД, ПараметрыФайлаОтчета, "xml", РезультатВыгрузки.ИмяФайла);
		...
		ИначеЕсли ТипыСправочниковМЧД.СтандартныеПодсистемы.Найти(ТипСправочникаМЧД) <> Неопределено Тогда
			...
			Для Каждого ДоверенностиДляВыгрузки Из РезультатВыгрузки Цикл
				...
				Для Каждого ФайлРезультатаВыгрузки Из ФайлыДляВыгрузки Цикл
					...
					КонецЕсли;
					ИмяФайлаСРасширением = ИмяФайлаДоверенности(
						СокращенныйПериодБезУИД, ПараметрыФайлаОтчета, РасширениеФайла, ФайлРезультатаВыгрузки.ИмяФайла);
		...
		ИначеЕсли ТипыСправочниковМЧД.ЭлектронноеВзаимодействие.Найти(ТипСправочникаМЧД) <> Неопределено Тогда
			...
			Для Каждого ЭлементАрхива Из ЧтениеАрхива.Элементы Цикл
				...
				КонецЕсли;
				ИмяФайлаСРасширением = ИмяФайлаДоверенности(
						СокращенныйПериодБезУИД, ПараметрыФайлаОтчета, РасширениеФайла, ЭлементАрхива.Имя);
...
Функция ИмяФайлаДоверенности(СокращенныйПериодБезУИД, ПараметрыФайлаОтчета, РасширениеФайла, ИмяФайлаИзПодсистемы)
	
	Если Не СокращенныйПериодБезУИД Тогда
		
		Возврат ИмяФайлаИзПодсистемы;
		
	КонецЕсли;
		
	ПозицияПодчеркивания = СтрНайти(ИмяФайлаИзПодсистемы, "_", НаправлениеПоиска.СНачала, , 2);
	Если ПозицияПодчеркивания <> 0 Тогда
		ПараметрыФайлаОтчета.ПрефиксФайла = Лев(ИмяФайлаИзПодсистемы, ПозицияПодчеркивания - 1);
	КонецЕсли;					
	ИмяФайлаБезРасширения = УниверсальныйОбменСБанкамиОбщегоНазначения.ПолучитьИмяФайлаОтчета(ПараметрыФайлаОтчета);
	
	Возврат ИмяФайлаБезРасширения + "." + РасширениеФайла;
