	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		Если НавигационнаяСсылкаФорматированнойСтроки = "#ЗагрузитьЧекиМК" Тогда
			...
			Новый ОписаниеОповещения(
				"ЗаполнитьСодержимымКассовыхЧеков", 
				ЭтотОбъект, 
				Новый Структура("ИмяСписка, СуммаВключаетНДС", "Расходы", Объект.СуммаВключаетНДС)));
		Иначе
...
Процедура РасходыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура("Номенклатура, КассовыйЧек", ТекущиеДанные.Номенклатура);
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.КассовыйЧек) Тогда
		Изменения.Удалить("СтавкаНДС");
		Изменения.Удалить("Цена");
	КонецЕсли;
...
Процедура РасходыНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	НоменклатураНаименование = Элементы.Расходы.ТекущиеДанные.Описание;
	ВидыНоменклатуры = "Товары, Материалы, Услуги";
...
Процедура УстановитьУсловноеОформление()
	...
	УчетКассовыхЧековПодотчетныхЛиц.УстановитьУсловноеОформлениеНаименованияИзЧека(УсловноеОформление, "РасходыОписание");
...
Процедура ОбработатьИзмененияПоКнопкеЦеныИВалюты()
	...
	Если НЕ ПлательщикНДС И НЕ Объект.НДСНеВыделять Тогда
...
Функция ОписаниеФормыДляЗагрузкиКассовыхЧековПодотчетныхЛиц()
	...
	ОписаниеФормы.КнопкиЗагрузитьЧеки.Добавить("РасходыЗагрузитьЧеки");
...
Процедура ЗаполнитьСчетаУчета(ДобавленныеСтроки, ИмяТаблицы)
	...
	СчетаУчетаВДокументах.ЗаполнитьСтроки(СтрокиДляЗаполненияСчетовУчета, ИмяТаблицы, Объект, Документы.РасходыПредпринимателя);
	
	Для Каждого Строка Из СтрокиДляЗаполненияСчетовУчета Цикл
		Если Строка.Свойство("СчетУчета") И ЗначениеЗаполнено(Строка.СчетУчета) Тогда
			БухгалтерскийУчетКлиентСервер.УстановитьСвойстваСубконтоСтрокиПриИзмененииСчета(
			ЭтотОбъект,
			Строка,
			ПараметрыУстановкиСвойствСубконто(ЭтотОбъект));
			
			УстановитьДоступностьСубконто(Строка);
		КонецЕсли;
	КонецЦикла;
...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.ИтогиВсего.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
...
Процедура АдаптироватьМКСписокФормыТовары()
	...
	КомандыПоля.КомандыТаблицыПрочие.Добавить("РасходыЗагрузитьЧеки");
	...
	ОписаниеГруппыПоля.ЗаголовокГруппы = "Расходы";
	ОписаниеГруппыПоля.ПутьКДаннымЗаголовка = "Объект.Расходы.КоличествоСтрок";
	...
	ОписаниеКолонокПоля.КолонкаНомерСтроки = "РасходыНомерСтроки";
	ОписаниеКолонокПоля.ВажныеКолонки = "РасходыНоменклатура,РасходыОписание";
	ОписаниеКолонокПоля.УстановитьВерхниеЗаголовки = 
		"РасходыНоменклатура,РасходыОписание,РасходыДокументРасхода,РасходыПодразделениеЗатрат";
	...
	Элементы.ГруппаНоменклатура.Высота = 2;
	Элементы.РасходыОписание.Ширина = 20;
	
	Элементы.РасходыКоличество.Формат = "ЧФ='Кол-во: Ч'";
	Элементы.РасходыЦена.Формат = "ЧФ='Цена: Ч'";
	Элементы.РасходыСумма.Формат = "ЧФ='Сумма: Ч'";
	Элементы.РасходыСуммаНДС.Формат = "ЧФ='НДС: Ч'";
	Элементы.РасходыВсего.Формат = "ЧФ='Всего: Ч'";
КонецПроцедуры
