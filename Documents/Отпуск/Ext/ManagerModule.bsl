...
Процедура ЗаполнитьДанныеДокументовДляПечатиРасчетаСреднегоЗаработка(МассивСсылок, ДанныеДокументов, ИмяМакета) Экспорт
	...
	|	ВЫБОР
	|		КОГДА НАЧАЛОПЕРИОДА(КадровыеДанныеСотрудников.ДатаПриема, МЕСЯЦ) > ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(Документ.ДатаНачалаОсновногоОтпуска, МЕСЯЦ), МЕСЯЦ, -12)
	|			ТОГДА НАЧАЛОПЕРИОДА(КадровыеДанныеСотрудников.ДатаПриема, МЕСЯЦ)
	|		ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(Документ.ДатаНачалаОсновногоОтпуска, МЕСЯЦ), МЕСЯЦ, -12)
	...
	|	Документ.НачалоПериодаЗаКоторыйПредоставляетсяОтпуск КАК НачалоПериодаЗаКоторыйПредоставляетсяОтпуск,
	|	Документ.КонецПериодаЗаКоторыйПредоставляетсяОтпуск КАК КонецПериодаЗаКоторыйПредоставляетсяОтпуск,
	...
	|			И Документ.ДатаНачалаСобытия = КадровыеДанныеСотрудников.Период
	|ГДЕ
...
Функция ПланируемаяДатыВыплатыОтпуска(ДатаНачалаСобытия, Организация) Экспорт
	...
	ПланируемаяДатыВыплатыОтпуска = УчетЗарплаты.БлижайшийРабочийДеньДляВыплаты(Организация, ПланируемаяДатыВыплатыОтпуска);
...
Функция АдресПараметровВХранилище(Объект, ДополнительныеПараметры) Экспорт
	...
	ПараметрыРасчетаСреднегоЗаработка.Вставить("НачислениеОтпускРК",    ДополнительныеПараметры.НачислениеОтпускРК);
	ПараметрыРасчетаСреднегоЗаработка.Вставить("НачислениеОтпускСН",    ДополнительныеПараметры.НачислениеОтпускСН);
...
Процедура РассчитатьНачисления(Объект, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		НачислениеОтпускРК = ДополнительныеПараметры.НачислениеОтпускРК;
		НачислениеОтпускСН = ДополнительныеПараметры.НачислениеОтпускСН;
		...
		Пока ЗначениеЗаполнено(ДатаНачала) И ДатаНачала <= ДатаОкончания Цикл
			...
			Если Объект.ПланируемаяДатаВыплаты >= ДатаЗаконаПроРКиЕАЭС Тогда
				Если ЗначениеЗаполнено(НачислениеОтпускСН) Тогда
					СреднийЗаработокСН = Объект.СреднийЗаработокСН;
				КонецЕсли;
				Если ЗначениеЗаполнено(НачислениеОтпускРК) Тогда
			...
			КалендарныеДниМесяцаБезПраздников = МодульРасчетЗарплатыДляНебольшихОрганизаций.КалендарныеДниМесяцаБезПраздников(ПроизводственныйКалендарь, Объект.ДатаНачалаСобытия);
			МРОТ = ЗарплатаКадрыПовтИсп.МинимальныйРазмерОплатыТрудаРФ(Объект.ДатаНачалаСобытия);
			...
			Если Объект.УчитыватьМРОТПриОплатеПоСреднемуЗаработку И Объект.СреднийЗаработок * КалендарныеДниМесяцаБезПраздников <= МРОТ Тогда
				Отпускные   = МРОТ / КалендарныеДниМесяцаБезПраздников * СтрокаНачислений.ОплаченоДней;
				ОтпускныеРК = 0;
				ОтпускныеСН = 0;
			Иначе
				Отпускные   = (Объект.СреднийЗаработок - СреднийЗаработокРК - СреднийЗаработокСН) * СтрокаНачислений.ОплаченоДней;
				ОтпускныеРК = СреднийЗаработокРК * СтрокаНачислений.ОплаченоДней;
				ОтпускныеСН = СреднийЗаработокСН * СтрокаНачислений.ОплаченоДней;
			КонецЕсли;
			
			СтрокаНачислений.Результат = Отпускные;
			...
			Если Объект.ПланируемаяДатаВыплаты >= ДатаЗаконаПроРКиЕАЭС Тогда
				...
				Если ЗначениеЗаполнено(НачислениеОтпускРК) И ОтпускныеРК > 0 Тогда
					...
					СтрокаНачисленийРК.Результат 		= ОтпускныеРК;
					
				КонецЕсли;
				
				Если ЗначениеЗаполнено(НачислениеОтпускСН) И ОтпускныеСН > 0 Тогда
					...
					СтрокаНачисленийСН.Результат 		= ОтпускныеСН;
