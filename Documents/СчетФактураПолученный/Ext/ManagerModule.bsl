...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработкаСчетФактурПолученныхБезНДС");
	Методы.Вставить("ОбработатьНомераИСуммыСчетаФактуры");
	Методы.Вставить("ОбработатьСчетаФактурыИзПредыдущейВерсии");
...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
    ...
    |    И (ЧтениеСпискаРазрешено(ДокументыОснования.ДокументОснование) Или ДокументОснование = Неопределено)
    |;
    |РазрешитьИзменениеЕслиРазрешеноЧтение
    |ГДЕ
    |    ЗначениеРазрешено(Организация)
    |    И (ИзменениеСпискаРазрешено(ДокументыОснования.ДокументОснование) Или ДокументОснование = Неопределено)";
...
Функция СчетаФактурыВыданныеПокупателям(ПараметрыЗапроса) Экспорт
	...
	ИначеЕсли ПараметрыЗапроса.Исправление И НЕ ПараметрыЗапроса.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаАванс Тогда
		
		ВидКорректировочногоДокумента = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.ИсправлениеОшибки;
		Запрос.УстановитьПараметр("ВидКорректировочногоДокумента", ВидКорректировочногоДокумента);
		Запрос.Текст = ТекстЗапросаИсправление();
		
	ИначеЕсли ПараметрыЗапроса.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда 
		
		ВидКорректировочногоДокумента = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение;
		Запрос.УстановитьПараметр("ВидКорректировочногоДокумента", ВидКорректировочногоДокумента);
		Запрос.Текст = ТекстЗапросаКорректировка();
...
//
// Параметры:
//   ДокументСсылка - ДокументСсылка.СчетФактураПолученный - документ
//   ВидСчетаФактуры - ПеречислениеСсылка.ВидСчетаФактурыПолученного - вид документа
//   ЗаявлениеОВвозеТоваровОтКомиссионера - Булево - необязательный параметр, Истина, если при регистрации поступления товаров
//                                                   вместо счета-фактуры используется заявление о ввозе товаров,
//                                                   полученное от комиссионера.
//
...
Функция ПолучитьПредставлениеДокумента(ДокументСсылка, ВидСчетаФактуры, ЗаявлениеОВвозеТоваровОтКомиссионера = Ложь) Экспорт
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаПоступление
		И НЕ ЗаявлениеОВвозеТоваровОтКомиссионера Тогда
		ТекстОсновноеПредставление = НСтр("ru = 'Счет-фактура полученный на поступление'");
	ИначеЕсли ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаПоступление
		И ЗаявлениеОВвозеТоваровОтКомиссионера Тогда
		ТекстОсновноеПредставление = НСтр("ru = 'Счет-фактура (заявление о ввозе товаров)'");
	...
	Если ЗначениеЗаполнено(ДокументСсылка) И ОбщегоНазначения.СсылкаСуществует(ДокументСсылка) Тогда
		...
		РеквизитыСчетаФактуры.Вставить("ЗаявлениеОВвозеТоваровОтКомиссионера", ЗаявлениеОВвозеТоваровОтКомиссионера);
...
Функция ПолучитьКодВидаОперации(Параметры) Экспорт
	...
	Если ВерсияКодовВидовОпераций >= 6 И ПараметрыОпределенияКВО.НДСИсчисляетсяНалоговымАгентом 
		И ПараметрыОпределенияКВО.ВидАгентскогоДоговора = Перечисления.ВидыАгентскихДоговоров.РеализацияМедныхПолуфабрикатов Тогда 
		...
		ИначеЕсли ПараметрыОпределенияКВО.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаАванс Тогда
			Возврат "58";
		КонецЕсли;
	ИначеЕсли ВерсияКодовВидовОпераций >= 4 И ПараметрыОпределенияКВО.НДСИсчисляетсяНалоговымАгентом Тогда
		Если ПараметрыОпределенияКВО.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаПоступление Тогда
			Возврат "42";
		ИначеЕсли ПараметрыОпределенияКВО.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаАванс Тогда
			Возврат "41";
...
Функция ПолучитьПорядокОтраженияВычетаПоУмолчанию(СтруктураПараметров) Экспорт
	...
	ПлательщикНДСНаУСНСВычетами = УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(СтруктураПараметров.Организация, СтруктураПараметров.Дата);
	ПлательщикНДС               = УчетнаяПолитика.ПлательщикНДС(СтруктураПараметров.Организация, СтруктураПараметров.Дата) ИЛИ ПлательщикНДСНаУСНСВычетами;
	Если НЕ ПлательщикНДС Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ЕстьФункцияНаличиеСуммыНДС(ТекущийДокумент) Экспорт
	...
	Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
	 ...
	 Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
...
Функция ПолучитьПредставлениеПоРеквизитам(РеквизитыСчетаФактуры = Неопределено, ТекстОсновноеПредставление = "") Экспорт 
	...
	ПараметрыСчетаФактуры.Вставить("БланкСтрогойОтчетности", Ложь);
	ПараметрыСчетаФактуры.Вставить("ЗаявлениеОВвозеТоваровОтКомиссионера", Ложь);
	ПараметрыСчетаФактуры.Вставить("Исправление", Ложь);
	ПараметрыСчетаФактуры.Вставить("НомерИсправления", 0);
	ПараметрыСчетаФактуры.Вставить("ДатаИсправления", '00010101');
	ПараметрыСчетаФактуры.Вставить("НомерВходящегоДокумента", "");
	ПараметрыСчетаФактуры.Вставить("ДатаВходящегоДокумента", '00010101');
	...
	Иначе
		...
		ЗаполнитьЗначенияСвойств(ПараметрыСчетаФактуры, РеквизитыСчетаФактуры);
		
		Если ПараметрыСчетаФактуры.БланкСтрогойОтчетности = Истина Тогда
			ТекстОсновноеПредставление = НСтр("ru = 'Счет-фактура (бланк строгой отчетности)'");
		ИначеЕсли ПараметрыСчетаФактуры.ЗаявлениеОВвозеТоваровОтКомиссионера = Истина Тогда
			ТекстОсновноеПредставление = НСтр("ru = 'Счет-фактура (заявление о ввозе товаров)'");
		КонецЕсли;
		
		Если ПараметрыСчетаФактуры.Исправление = Истина Тогда
			ТекстПредставлениеКраткое = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 (испр. %2) от %3'"),
				ПараметрыСчетаФактуры.НомерВходящегоДокумента, ПараметрыСчетаФактуры.НомерИсправления, Формат(ПараметрыСчетаФактуры.ДатаИсправления,"ДЛФ=Д"));
		...
		Иначе
			ТекстПредставлениеКраткое = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 от %2'"),
				ПараметрыСчетаФактуры.НомерВходящегоДокумента, Формат(ПараметрыСчетаФактуры.ДатаВходящегоДокумента,"ДЛФ=Д"));
			ТекстПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 %2'"), ТекстОсновноеПредставление, ТекстПредставлениеКраткое);	
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьРеквизитыИсходногоСчетаФактурыДляКорректировки(ДокументОснование, СчетФактура, ПолучатьПредставление = Ложь, 
	Знач ИсходныйСчетФактура= Неопределено, Знач ДокументПоступления = Неопределено) Экспорт
	...
	Иначе
		Если ЗначениеЗаполнено(ДокументПоступления) Тогда
			// Ничего не делаем - документ поступления определили ранее.
...
Функция НовыйПараметрыОпределенияКВО()
	...
	ПараметрыОпределенияКВО.Вставить("ВидАгентскогоДоговора", Перечисления.ВидыАгентскихДоговоров.ПустаяСсылка());
...
Функция ПодготовитьПараметрыЗаполненияАванс(ДокументОснование, ДокументСсылка, ДоговорКонтрагента = Неопределено, Отказ) Экспорт
	...
	Для каждого СтрокаАванса Из АвансыПоСчетамНаОплату Цикл
		Если СтрокаАванса.НДСИсчисляетсяНалоговымАгентом Тогда
			СпособЗаполненияСтавкиНДС = РаботаСДоговорамиКонтрагентовБП.СпособЗаполненияСтавкиНДСПоДоговору(СтрокаАванса.ДоговорКонтрагента);
			Если СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ОбщаяУСН Тогда
				СтрокаАванса.СтавкаНДС = УчетНДСКлиентСервер.РасчетнаяУСНСтавкаНДС(РеквизитыОснования.Дата);
			ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН Тогда
				СтрокаАванса.СтавкаНДС = УчетНДСКлиентСервер.ПониженнаяРасчетнаяУСНСтавкаНДС(РеквизитыОснования.Дата);
			Иначе
...
Функция ТекстЗапросаДанныеПлатежа(НомераТаблиц)
	...
	|				И ДоговорыСчетаФактуры.ВидАгентскогоДоговора В (&ВидыДоговоровРеализацииТоваров)
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка,
		"Организация, Дата, ИсправляемыйСчетФактура, ИсправлениеСобственнойОшибки, ДокументОснование");
	Запрос.УстановитьПараметр("Дата",                    РеквизитыДокумента.Дата);
	Запрос.УстановитьПараметр("ДатаОтраженияВПрослеживаемости", КонецДня(РеквизитыДокумента.Дата));
	...
	Реквизиты.Вставить("ИностранныйСчетФактура", Справочники.Контрагенты.КонтрагентРезидентТаможенногоСоюза(Реквизиты.Контрагент));
	...
	ПараметрыПроведения.Вставить("РеквизитыПрослеживаемости", ТаблицаРеквизиты);
...
Функция ТекстЗапросаРеквизитыДокумента()
	...
	|ПОМЕСТИТЬ ПоступлениеКомандировочныхРасходов
	|ИЗ
	|	(ВЫБРАТЬ
	|		Прочее.ДатаДокументаОплаты КАК ДатаДокументаОплаты,
	|		Прочее.НомерДокументаОплаты КАК НомерДокументаОплаты
	|	ИЗ
	|		Документ.АвансовыйОтчет.Прочее КАК Прочее
	|	ГДЕ
	|		Прочее.Ссылка = &ДокументОснование
	|		И Прочее.СчетФактура = &Ссылка
	|		И Прочее.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийАвансовыйОтчет.Командировка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		Билеты.ДатаДокументаОплаты,
	|		Билеты.НомерДокументаОплаты
	|	ИЗ
	|		Документ.АвансовыйОтчет.Билеты КАК Билеты
	|	ГДЕ
	|		Билеты.Ссылка = &ДокументОснование
	|		И Билеты.СчетФактура = &Ссылка) КАК КомандировочныеРасходы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				И (ЕСТЬNULL(Реквизиты.ДоговорКонтрагента.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) В (&ВидыДоговоровРеализацииТоваров))
		|	КОНЕЦ КАК НДСИсчисляетсяНалоговымАгентом,
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДеталиПоступления.НомерДокументаОплаты, """") <> """"
	|				И Реквизиты.Дата >= ДАТАВРЕМЯ(2014, 10, 1)
	|			ТОГДА ДеталиПоступления.НомерДокументаОплаты
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДеталиПоступления.ДатаДокументаОплаты, ДАТАВРЕМЯ(1, 1, 1)) <> ДАТАВРЕМЯ(1, 1, 1)
	|				И Реквизиты.Дата >= ДАТАВРЕМЯ(2014, 10, 1)
	|			ТОГДА ДеталиПоступления.ДатаДокументаОплаты
	|		КОГДА Реквизиты.ДокументОснование ССЫЛКА Документ.АвансовыйОтчет
	|				И Реквизиты.Дата >= ДАТАВРЕМЯ(2014, 10, 1)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоступлениеКомандировочныхРасходов КАК ДеталиПоступления
	|		ПО (ИСТИНА)
	|ГДЕ
...
Функция ТекстЗапросаОшибочныеРеквизитыКонтрагентов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если НЕ Реквизиты.ИсправлениеСобственнойОшибки Тогда
		ПараметрыПроведения.Вставить("ВТ_ТаблицаОшибочныеРеквизитыКонтрагентов", Неопределено);
		ПараметрыПроведения.Вставить("ТаблицаОшибочныеРеквизитыКонтрагентов", Неопределено);
		Возврат "";
	КонецЕсли;

	НомераТаблиц.Вставить("ВТ_ТаблицаОшибочныеРеквизитыКонтрагентов", НомераТаблиц.Количество());
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОшибочныеРеквизитыКонтрагентов.Период КАК Период,
	|	ОшибочныеРеквизитыКонтрагентов.Регистратор КАК Регистратор,
	...
	|ИЗ
	|	ОшибочныеРеквизитыКонтрагентов КАК ОшибочныеРеквизитыКонтрагентов";
...
Функция ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Не Реквизиты.РеализацияТоваровКомитента
		ИЛИ Не Реквизиты.ВедетсяУчетПрослеживаемыхТоваров Тогда
		ПараметрыПроведения.Вставить("ВТ_СписаниеРНПТ",        Неопределено);
		ПараметрыПроведения.Вставить("ВТ_СуммыПоРНПТ",         Неопределено);
		ПараметрыПроведения.Вставить("ВТ_Операции",            Неопределено);
		ПараметрыПроведения.Вставить("ПрослеживаемыеОперации", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВТ_СписаниеРНПТ",        НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВТ_СуммыПоРНПТ",         НомераТаблиц.Количество());
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ПрослеживаемыеТоварыДвижения.Регистратор КАК Регистратор,
	|	ПрослеживаемыеТоварыДвижения.Номенклатура КАК Номенклатура,
	|	ПрослеживаемыеТоварыДвижения.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ПрослеживаемыеТоварыДвижения.Комиссионер КАК Комиссионер,
	|	ПрослеживаемыеТоварыДвижения.Комитент КАК Комитент,
	|	ПрослеживаемыеТоварыДвижения.ВидЗапасов КАК ВидЗапасов,
	|	ПрослеживаемыеТоварыДвижения.РНПТ КАК РНПТ,
	|	СУММА(ПрослеживаемыеТоварыДвижения.Количество) КАК Количество,
	|	СУММА(ПрослеживаемыеТоварыДвижения.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	СчетаФактурыВыданныеПокупателям.Ссылка КАК ДокументОперации,
	|	СчетаФактурыВыданныеПокупателям.Ссылка.Дата КАК Дата,
	|	ПрослеживаемыеТоварыДвижения.Комплектующие КАК Комплектующие,
	|	СправочникНоменклатура.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ПОМЕСТИТЬ СписаниеРНПТ
	|ИЗ
	|	Документ.СчетФактураПолученный.СчетаФактурыВыданныеПокупателям КАК СчетаФактурыВыданныеПокупателям
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйДокументыОснования
	|		ПО СчетаФактурыВыданныеПокупателям.СчетФактура = СчетФактураВыданныйДокументыОснования.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПрослеживаемыеТовары КАК ПрослеживаемыеТоварыДвижения
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|			ПО ПрослеживаемыеТоварыДвижения.Номенклатура = СправочникНоменклатура.Ссылка
	|		ПО (СчетФактураВыданныйДокументыОснования.ДокументОснование = ПрослеживаемыеТоварыДвижения.Регистратор)
	|			И СчетаФактурыВыданныеПокупателям.Ссылка.Контрагент = ПрослеживаемыеТоварыДвижения.Комитент
	|ГДЕ
	|	СчетаФактурыВыданныеПокупателям.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ПрослеживаемыеТоварыДвижения.Регистратор,
	|	ПрослеживаемыеТоварыДвижения.Номенклатура,
	|	ПрослеживаемыеТоварыДвижения.СтранаПроисхождения,
	|	ПрослеживаемыеТоварыДвижения.Комиссионер,
	|	ПрослеживаемыеТоварыДвижения.Комитент,
	|	ПрослеживаемыеТоварыДвижения.ВидЗапасов,
	|	ПрослеживаемыеТоварыДвижения.РНПТ,
	|	СчетаФактурыВыданныеПокупателям.Ссылка,
	|	СчетаФактурыВыданныеПокупателям.Ссылка.Дата,
	|	ПрослеживаемыеТоварыДвижения.Комплектующие,
	|	СправочникНоменклатура.ПрослеживаемыйКомплект
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписаниеРНПТ.Регистратор КАК Регистратор,
	|	ОперацииСПрослеживаемымиТоварами.РНПТ КАК РНПТ,
	|	СписаниеРНПТ.Номенклатура КАК Номенклатура,
	...
	|	СписаниеРНПТ.Регистратор,
	|	СписаниеРНПТ.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СписаниеРНПТ.Комиссионер КАК Комиссионер,
	|	СписаниеРНПТ.Комитент КАК Комитент,
	|	СписаниеРНПТ.ВидЗапасов КАК ВидЗапасов,
	...
	|	СписаниеРНПТ.Комитент КАК Контрагент,
	|	СписаниеРНПТ.Дата КАК ПериодОперации,
	...
	|	СписаниеРНПТ.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ПОМЕСТИТЬ ВТ_Операции
	|ИЗ
	|	СписаниеРНПТ КАК СписаниеРНПТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ СуммыПоРНПТ КАК СуммыПоРНПТ
	|		ПО СписаниеРНПТ.Регистратор = СуммыПоРНПТ.Регистратор
	|			И СписаниеРНПТ.РНПТ = СуммыПоРНПТ.РНПТ
	|			И СписаниеРНПТ.Номенклатура = СуммыПоРНПТ.Номенклатура
	|ГДЕ
	|	НЕ СписаниеРНПТ.Регистратор ЕСТЬ NULL
	|;
	|
	...
	|	ВТ_Операции.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ВТ_Операции.Комиссионер КАК Комиссионер,
	|	ВТ_Операции.Комитент КАК Комитент,
	|	ВТ_Операции.ВидЗапасов КАК ВидЗапасов,
	|	ВТ_Операции.РНПТ КАК РНПТ,
	|	ВТ_Операции.Количество КАК Количество,
	|	ВТ_Операции.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	ВТ_Операции.СуммаБезНДС КАК СуммаБезНДС,
	|	ВТ_Операции.ОтражениеВОтчетности КАК ОтражениеВОтчетности,
	|	ВТ_Операции.КодОперации КАК КодОперации,
	|	ВТ_Операции.ДокументОперации КАК ДокументОперации,
	|	ВТ_Операции.Контрагент КАК Контрагент,
	|	ВТ_Операции.ПериодОперации КАК ПериодОперации,
	|	ВТ_Операции.НомерДокумента КАК НомерДокумента,
	|	ВТ_Операции.ДатаДокумента КАК ДатаДокумента,
	|	ВТ_Операции.ТипДокументаВПрослеживаемости КАК ТипДокументаВПрослеживаемости,
	|	ВТ_Операции.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ВТ_Операции.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ИЗ
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ЗаявлениеОВвозеТоваровОтКомиссионера", Ложь);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура534Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2023Кв4Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2023кв4Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2023Кв4Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура1096Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур1096Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура1096Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура2026Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур2026Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура2026Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура26Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьСчетовФактур26Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьСчетовФактур(ПорядокПримененияФормСФ["СчетФактура26Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура534Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур534Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура534Прослеж"],Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2023Кв4Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2023Кв4Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2023Кв4Прослеж"], Истина),
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура1096Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур1096Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура1096Прослеж"], Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура2026Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур2026Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура2026Прослеж"], Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КорректировочныйСчетФактура26Прослеж") Тогда
		ТабличныйДокумент = УчетНДС.ПечатьКорректировочныхСчетовФактур26Прослеж(
			МассивОбъектов,
			ОбъектыПечати,
			ТекстЗапросаПечатьКорректировочныхСчетовФактур(ПорядокПримененияФормСФ["КорректировочныйСчетФактура26Прослеж"], Истина), 
			ПараметрыПечати,
			ПараметрыВывода);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
...
Функция ТекстЗапросаПечатьСчетовФактур(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	СчетФактура.ДатаВходящегоДокумента КАК Дата,
	...
	|			ТОГДА СчетФактура.ДатаИсправления
	|		КОГДА СчетФактура.ДатаВходящегоДокумента <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА СчетФактура.ДатаВходящегоДокумента
	...
	|	И СчетФактура.ДатаВходящегоДокумента > &УсловиеПоДате
	|	И НЕ СчетФактура.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.НаАванс)
	|	И НЕ СчетФактура.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.НаАвансКомитента)
	|	И НЕ СчетФактура.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.Корректировочный)
	|	И НЕ СчетФактураДокументыОснования.ДокументОснование ССЫЛКА Документ.ОтчетКомитентуОПродажах
	...
	"ВЫБРАТЬ
	|	СчетФактураПолученныйДокументыОбОтгрузке.Ссылка КАК СчетФактура,
	|	СчетФактураПолученныйДокументыОбОтгрузке.НомераСтрок КАК НомераСтрок,
	...
	|ИЗ
	|	Документ.СчетФактураПолученный.ДокументыОбОтгрузке КАК СчетФактураПолученныйДокументыОбОтгрузке
	|ГДЕ
	|	СчетФактураПолученныйДокументыОбОтгрузке.Ссылка В(&МассивОбъектов)
	|	И СчетФактураПолученныйДокументыОбОтгрузке.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.НаПоступление)";
...
Функция ТекстЗапросаПечатьКорректировочныхСчетовФактур(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	СчетФактура.Исправление КАК Исправление,
	|	СчетФактура.НомерИсправления КАК НомерИсправления,
	|	СчетФактура.ДатаИсправления КАК ДатаИсправления,
	...
	|	СчетФактура.НомерИсправления КАК НомерИсправленияКорректировочного,
	|	СчетФактура.ДатаИсправления КАК ДатаИсправленияКорректировочного,
	|	ВЫБОР
	|		КОГДА СчетФактура.Исправление
	|			ТОГДА СчетФактура.ДатаИсправления
	|		КОГДА СчетФактура.ДатаВходящегоДокумента <> ДАТАВРЕМЯ(1, 1, 1)
	...
	|ГДЕ
	|	СчетФактураДокументыОснования.Ссылка В(&МассивОбъектов)
	|	И СчетФактураДокументыОснования.Ссылка.ДатаВходящегоДокумента > &УсловиеПоДате
	|	И СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.Корректировочный)
	|	И &ЕстьПрослеживаемыеТовары
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетФактураПолученныйКорректируемыеНомераСтрок.Ссылка КАК СчетФактура,
	|	СчетФактураПолученныйКорректируемыеНомераСтрок.НомерСтроки КАК НомерСтроки,
	|	СчетФактураПолученныйКорректируемыеНомераСтрок.НомерИсходнойСтроки КАК НомерИсходнойСтроки
	|ИЗ
	|	Документ.СчетФактураПолученный.КорректируемыеНомераСтрок КАК СчетФактураПолученныйКорректируемыеНомераСтрок
	|ГДЕ
	|	СчетФактураПолученныйКорректируемыеНомераСтрок.Ссылка В(&МассивОбъектов)
	|	И СчетФактураПолученныйКорректируемыеНомераСтрок.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыПолученного.Корректировочный)";
...
Функция СоздатьДокументНаОсновании(ПараметрыСоздания, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина, ОригиналСчетаФактуры = Неопределено) Экспорт
	...
	Продавец                     = ПараметрыСоздания.Продавец;
	...
	ЗаявлениеОВвозеТоваровОтКомиссионера = ПараметрыСоздания.ЗаявлениеОВвозеТоваровОтКомиссионера;
	...
	Если ПараметрыСоздания.ЭтоУниверсальныйДокумент = Истина Тогда
		СчетФактура.КодВидаОперации = КодВидаОперации;
		СчетФактура.НДСПредъявленКВычету = НДСПредъявленКВычету;
	ИначеЕсли ЗаявлениеОВвозеТоваровОтКомиссионера = Истина Тогда
		СчетФактура.КодВидаОперации = КодВидаОперации;
		СчетФактура.ЗаявлениеОВвозеТоваровОтКомиссионера = ЗаявлениеОВвозеТоваровОтКомиссионера;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Продавцы) И Продавцы.Количество() > 0 Тогда
		СчетФактура.Продавец = Продавцы[0].Продавец;
...
Функция ТекстЗапросаИсправление()
	
	// Суммы берем из комиссионных сумм счетов-фактур выставленных покупателю за день,
	// по которому наш комитент перевыставляет счета-фактуры. 
	// Для сложных случаев (например корректируются документы реализации товаров нескольких комитентов) 
	// предполагается корректировка автозаполненных сумм.
	ТекстЗапроса = ТекстЗапросаДляИсправленияКорректировки() + 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	СчетФактураОснования.СуммаУвеличение КАК СуммаУвеличение,
	|	СчетФактураОснования.СуммаУменьшение КАК СуммаУменьшение,
	|	СчетФактураОснования.СуммаНДСУвеличение КАК СуммаНДСУвеличение,
	|	СчетФактураОснования.СуммаНДСУменьшение КАК СуммаНДСУменьшение
	|ИЗ
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактураШапка.Ссылка,
	|	СчетФактураШапка.СуммаДокументаКомиссия,
	|	СчетФактураШапка.СуммаНДСДокументаКомиссия,
	|	СчетФактурыПоСубкомиссии.СубКомиссионер,
	|	СчетФактураШапка.Контрагент,
	...
	|ИЗ
	|	СчетФактурыПоСубкомиссии КАК СчетФактурыПоСубкомиссии
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураОснования
	|		ПО СчетФактурыПоСубкомиссии.ДокументРеализации = СчетФактураОснования.ДокументОснование
	|			И СчетФактурыПоСубкомиссии.СчетФактура = СчетФактураОснования.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураШапка
	|		ПО СчетФактурыПоСубкомиссии.СчетФактура = СчетФактураШапка.Ссылка
	|ГДЕ
	|	НЕ СчетФактураШапка.ПометкаУдаления";
...
Функция ТекстЗапросаКорректировка()
	...
	// Суммы увеличения у уменьшения берем из регистра "РеализованныеТоварыКомитентов",
	// который двигается корректировочными документами, выставленными покупателю, 
	// которому мы реализовали товары комитента.
	// Сумма документа и НДС не используются для заполнения журнала,
	// записываем дельту между увеличением и уменьшением этих показателей.
	ТекстЗапроса = ТекстЗапросаДляИсправленияКорректировки() + 
	"ВЫБРАТЬ
	|	СчетФактураОснования.ДокументОснование КАК ДокументОснование,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетФактураОснования.ДокументОснование,
	...
	|	СчетФактураШапка.Контрагент КАК Покупатель,
	|	СУММА(ВЫБОР
	|			КОГДА РеализованныеТоварыКомитентов.Выручка > 0
	|				ТОГДА РеализованныеТоварыКомитентов.Выручка
	|			ИНАЧЕ 0
	...
	|	СУММА(ВЫБОР
	|			КОГДА РеализованныеТоварыКомитентов.СуммаНДС > 0
	|				ТОГДА РеализованныеТоварыКомитентов.СуммаНДС
	|			ИНАЧЕ 0
	...
	|ПОМЕСТИТЬ ВыданныеСчетаФактурыПредварительная
	|ИЗ
	|	ДокументыРеализации КАК ДокументыРеализации
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РеализованныеТоварыКомитентов КАК РеализованныеТоварыКомитентов
	|		ПО (РеализованныеТоварыКомитентов.Контрагент = &Комитент)
	|			И (РеализованныеТоварыКомитентов.ДоговорКонтрагента = &ДоговорКомитента)
	|			И ДокументыРеализации.ДокументРеализации = РеализованныеТоварыКомитентов.Регистратор
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаСчетовФактур КАК СчетФактураОснования
	|		ПО ДокументыРеализации.ДокументРеализации = СчетФактураОснования.ДокументОснование
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураШапка
	|		ПО (СчетФактураОснования.Ссылка = СчетФактураШапка.Ссылка)
	|ГДЕ
	...
	|ИЗ
	|	СчетФактурыПоСубкомиссии КАК СчетФактурыПоСубкомиссии
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РеализованныеТоварыКомитентов КАК РеализованныеТоварыКомитентов
	|		ПО (РеализованныеТоварыКомитентов.Контрагент = &Комитент)
	|			И (РеализованныеТоварыКомитентов.ДоговорКонтрагента = &ДоговорКомитента)
	|			И СчетФактурыПоСубкомиссии.ДокументРеализации = РеализованныеТоварыКомитентов.Регистратор
	|			И СчетФактурыПоСубкомиссии.СчетФактура = РеализованныеТоварыКомитентов.ДокументРасчетов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК СчетФактураШапка
	|		ПО СчетФактурыПоСубкомиссии.СчетФактура = СчетФактураШапка.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВыданныеСчетаФактурыПредварительная.СчетФактура КАК СчетФактура,
	|	ВыданныеСчетаФактурыПредварительная.Субкомиссионер КАК Субкомиссионер,
	|	ВыданныеСчетаФактурыПредварительная.Покупатель КАК Покупатель,
	|	ВыданныеСчетаФактурыПредварительная.СуммаУвеличение КАК СуммаУвеличение,
	|	ВыданныеСчетаФактурыПредварительная.СуммаУменьшение КАК СуммаУменьшение,
	|	ВыданныеСчетаФактурыПредварительная.СуммаНДСУвеличение КАК СуммаНДСУвеличение,
	|	ВыданныеСчетаФактурыПредварительная.СуммаНДСУменьшение КАК СуммаНДСУменьшение,
	|	ВЫБОР
	|		КОГДА ВыданныеСчетаФактурыПредварительная.СуммаУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаУменьшение > 0
	|			ТОГДА ВыданныеСчетаФактурыПредварительная.СуммаУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаУменьшение
	|		ИНАЧЕ -(ВыданныеСчетаФактурыПредварительная.СуммаУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаУменьшение)
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА ВыданныеСчетаФактурыПредварительная.СуммаНДСУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаНДСУменьшение > 0
	|			ТОГДА ВыданныеСчетаФактурыПредварительная.СуммаНДСУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаНДСУменьшение
	|		ИНАЧЕ -(ВыданныеСчетаФактурыПредварительная.СуммаНДСУвеличение - ВыданныеСчетаФактурыПредварительная.СуммаНДСУменьшение)
	|	КОНЕЦ КАК НДС
	|ИЗ
	|	ВыданныеСчетаФактурыПредварительная КАК ВыданныеСчетаФактурыПредварительная";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ТекстЗапросаДляИсправленияКорректировки()
	
	// Временные таблицы:
	// ПокупателиПоОтчету - все покупатели, отраженные в отчете, которым был реализован товар комитента.
	// ДокументыРеализации - корректировочные документы, выставленные покупателям за день,
	// по которому наш комитент перевыставляет счета-фактуры.
	...
	|	И ДокументКорректировкиРеализации.ВидОперации = &ВидКорректировочногоДокумента
	...
	|	И НАЧАЛОПЕРИОДА(ВозвратТоваровОтПокупателя.Дата, ДЕНЬ) = &ДатаВходящегоДокумента
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетКомиссионераОПродажахВозвраты.СчетФактура КАК СчетФактура,
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка КАК ДокументРеализации,
	|	СУММА(ОтчетКомиссионераОПродажахТоварыВозвращенные.Сумма) КАК СуммаУменьшение,
	|	СУММА(ОтчетКомиссионераОПродажахТоварыВозвращенные.СуммаНДС) КАК СуммаНДСУменьшение,
	|	0 КАК СуммаУвеличение,
	|	0 КАК СуммаНДСУвеличение,
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка.Контрагент КАК СубКомиссионер
	|ПОМЕСТИТЬ СчетФактурыПоСубкомиссии
	|ИЗ
	...
	|		ПО ОтчетКомиссионераОПродажахВозвраты.Ссылка = ОтчетКомиссионераОПродажахТоварыВозвращенные.Ссылка.Ссылка
	...
	|	И ОтчетКомиссионераОПродажах.НаВозвратыВыставляетсяКорректировочныйСчетФактура
	|	И ОтчетКомиссионераОПродажах.Проведен
	|	И ОтчетКомиссионераОПродажахВозвраты.ВыставленСФ
	|	И НАЧАЛОПЕРИОДА(ОтчетКомиссионераОПродажахВозвраты.ДатаСФ, ДЕНЬ) = &ДатаВходящегоДокумента
	|	И ОтчетКомиссионераОПродажахВозвраты.Покупатель В
	|			(ВЫБРАТЬ
	|				ПокупателиПоОтчету.Покупатель
	|			ИЗ
	|				ПокупателиПоОтчету)
	...
	|	ОтчетКомиссионераОПродажахВозвраты.Ссылка.Контрагент
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура";
...
Функция ЕстьФункцияНаличиеСтавкиНДСПоУСН(ТекущийДокумент) Экспорт

	Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ПоступлениеДопРасходов")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ПоступлениеНМА")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ПоступлениеИзПереработки")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
		Или ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
