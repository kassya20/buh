...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	КомментарииДокументовЗакрытогоПериодаВызовСервера.ПриЧтенииНаСервере(ЭтаФорма, ТекущийОбъект);
...
&НаКлиенте
Процедура ОтредактированыНомераКорректируемыхСтрокПриИзменении(Элемент)
...
&НаКлиенте
Процедура КорректируемыеНомераСтрокПриИзменении(Элемент)
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ЗаполнятьНомераКорректируемыхСтрок = ВерсияПостановленияНДС1137 >= 5
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления");
...
Процедура УстановитьВидимость()
	...
	Если ИспользуетсяПостановлениеНДС1137
		...
		// Скроем сумму по документу, т.к. сумма по корректировочному счету-фактуре в журнале не заполняется.
	...
	Элементы.ГруппаКорректируемыеСтроки.Видимость = ЗаполнятьНомераКорректируемыхСтрок;
...
Процедура УправлениеФормой(ЭтотОбъект)
	...
	Элементы.НадписьИзменитьОснованиеСчетаФактуры.Видимость = 
		ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику");
		
	Если ЭтотОбъект.ЗаполнятьНомераКорректируемыхСтрок Тогда
		Если Объект.КорректируемыеНомераСтрок.Количество() = 0 Тогда
			НадписьКорректируемыеСтроки = НСтр("ru = '(не указаны номера корректируемых строк)'");
		ИначеЕсли Объект.КорректируемыеНомераСтрок.Количество() = 1 Тогда
			НадписьКорректируемыеСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(корректируется строка №%1)'"),
			?(Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки = 0, "--", Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки));
		ИначеЕсли Объект.КорректируемыеНомераСтрок.Количество() = 2 Тогда
			НадписьКорректируемыеСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(корректируются строки №%1 и №%2)'"),
			?(Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки = 0, "--", Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки),
			?(Объект.КорректируемыеНомераСтрок[1].НомерИсходнойСтроки = 0, "--", Объект.КорректируемыеНомераСтрок[1].НомерИсходнойСтроки));
		Иначе
			НадписьКорректируемыеСтроки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(корректируются строки №%1,№%2 и еще %3)'"),
			?(Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки = 0, "--", Объект.КорректируемыеНомераСтрок[0].НомерИсходнойСтроки),
			?(Объект.КорректируемыеНомераСтрок[1].НомерИсходнойСтроки = 0, "--", Объект.КорректируемыеНомераСтрок[1].НомерИсходнойСтроки),
			Объект.КорректируемыеНомераСтрок.Количество() - 2);
		КонецЕсли;
		Элементы.ГруппаКорректируемыеСтрокиТаблица.Заголовок  = НадписьКорректируемыеСтроки;
		Элементы.КорректируемыеНомераСтрок.Доступность = Объект.ОтредактированыНомераКорректируемыхСтрок;
	КонецЕсли;
