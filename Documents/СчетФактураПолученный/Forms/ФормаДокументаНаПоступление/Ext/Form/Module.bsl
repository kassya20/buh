...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	КомментарииДокументовЗакрытогоПериодаВызовСервера.ПриЧтенииНаСервере(ЭтаФорма, ТекущийОбъект, "ГруппаКомментарийОтветственный");
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПредставлениеДокумента = Документы.СчетФактураПолученный.ПолучитьПредставлениеДокумента(Объект.Ссылка, Объект.ВидСчетаФактуры,
		Объект.ЗаявлениеОВвозеТоваровОтКомиссионера);
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
		...
		НДСИсчисляетсяНалоговымАгентом = Перечисления.ВидыАгентскихДоговоров.НДСИсчисляетсяНалоговымАгентом(СвойстваДоговора.ВидАгентскогоДоговора);
	Иначе
	...
	Элементы.ПредставлениеКППКонтрагента.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьИзменитьПродавцов.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьНесколькоПродавцовВыбор.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьНесколькоПродавцовСсылка.Доступность = Не ТолькоПросмотр;	
	Элементы.НадписьИзменитьСчетаФактуры.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьСчетаФактурыВыданныеПокупателю.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьСчетФактураВыбор.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьВыборДокументаОснования.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьИзменить.Доступность = Не ТолькоПросмотр;
	Элементы.НадписьДокументыОснования.Доступность = Не ТолькоПросмотр;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.КодСпособаПолучения.Видимость = Форма.ИспользуетсяПостановлениеНДС1137 
											И НЕ Объект.ЗаявлениеОВвозеТоваровОтКомиссионера;
	...
	Элементы.КодВидаОперации.ТолькоПросмотр = Объект.ЗаявлениеОВвозеТоваровОтКомиссионера
		И СокрЛП(Объект.КодВидаОперации) = "46";
		
	Если Объект.ЗаявлениеОВвозеТоваровОтКомиссионера Тогда
		Элементы.Номер.Заголовок = НСтр("ru = '№ отметки о регистрации заявления'");
		Элементы.СуммаДокумента.Заголовок = НСтр("ru = 'Налоговая база (НДС)'");
		Элементы.СуммаНДСДокумента.Заголовок = НСтр("ru = 'НДС'");
	Иначе
		Элементы.Номер.Заголовок = НСтр("ru = 'Счет-фактура №'");
		Элементы.СуммаДокумента.Заголовок = НСтр("ru = 'Сумма'");
		Элементы.СуммаНДСДокумента.Заголовок = НСтр("ru = 'НДС (в т.ч.)'");
	КонецЕсли;
...
Процедура ДатаПриИзмененииНаСервере()
	...
	УстановитьЗаявлениеОВвозеТоваровОтКомиссионера(Истина);
...
Процедура УстановитьКодВидаОперацииНаСервере()
	...
	СтруктураПараметров.Вставить("ЗаявлениеОВвозеТоваровОтКомиссионера", Объект.ЗаявлениеОВвозеТоваровОтКомиссионера);
...
&НаСервере
Процедура УстановитьЗаявлениеОВвозеТоваровОтКомиссионера(РасчитатьНалоговуюБазуНДС = Ложь)
	МассивПродавцов = Новый Массив;
	Для каждого СтрокаПродавец Из Объект.Продавцы Цикл
		МассивПродавцов.Добавить(СтрокаПродавец.Продавец);
	КонецЦикла;
	
	Объект.ЗаявлениеОВвозеТоваровОтКомиссионера = Объект.ДатаВходящегоДокумента >= '20240101'
		И ВидДоговораКонтрагента = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку
		И Справочники.Контрагенты.ЕстьКонтрагентыИзЕАЭС(МассивПродавцов);
		
	ПредставлениеДокумента = Документы.СчетФактураПолученный.ПолучитьПредставлениеДокумента(Объект.Ссылка, Объект.ВидСчетаФактуры,
		Объект.ЗаявлениеОВвозеТоваровОтКомиссионера);
	УстановитьЗаголовокФормы(ЭтотОбъект, ПредставлениеДокумента);
	
	Если РасчитатьНалоговуюБазуНДС Тогда
		ПараметрыСчетаФактуры = ПолучитьПараметрыСчетаФактуры();
		Объект.СуммаДокумента = ПараметрыСчетаФактуры.СуммаДокумента;
	КонецЕсли;
...
Процедура ДокументыОснованияДокументОснованиеПриИзмененииНаСервере()
	...
	УстановитьЗаявлениеОВвозеТоваровОтКомиссионера();
...
Процедура ЗаполнитьЗначенияСвойствОбъекта(Объект, ПараметрыСчетаФактуры)
	...
	СтатусДокументооборота = ОбменСКонтрагентами.СтатусДокументооборота(Объект.Ссылка);
...
Процедура ВыбратьПродавцов()
	...
	ПараметрыФормы.Вставить("ЗаявлениеОВвозеТоваровОтКомиссионера", Объект.ЗаявлениеОВвозеТоваровОтКомиссионера);
	ПараметрыФормы.Вставить("ДатаВходящегоДокумента", Объект.ДатаВходящегоДокумента);
	ПараметрыФормы.Вставить("ВидДоговораКонтрагента", ВидДоговораКонтрагента);
...
Процедура ОбработкаВыбораПродавцовНаСервере(ВыбранноеЗначение)
	...
	Если Объект.ЗаявлениеОввозеТоваровОтКомиссионера <> ВыбранноеЗначение.ЗаявлениеОввозеТоваровОтКомиссионера Тогда
		
		Объект.ЗаявлениеОввозеТоваровОтКомиссионера = ВыбранноеЗначение.ЗаявлениеОввозеТоваровОтКомиссионера;
...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.Дата.Заголовок = НСтр("ru = 'Счет-фактура Дата'");
	Элементы.ДатаИсправления.Заголовок = НСтр("ru = 'Исправление Дата'");
	Элементы.ДатаИсправления.ПоложениеЗаголовка=ПоложениеЗаголовкаЭлементаФормы.Авто;
	Элементы.ДатаВходящегоДокумента.Заголовок = НСтр("ru = 'Вх. Дата'");
	Элементы.ДатаВходящегоДокумента.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
	
	МобильныйКлиентБПФормаОбъекта.АдаптироватьНомерДатаФормаДокумента(Элементы, Элементы.ГруппаТекущий);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаТекущий);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.НадписьСчетФактура);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаРеквизитыИсправления);
	
	
	Элементы.ГруппаНомераИДатыНижняя.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	Элементы.ОригиналПолучен.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Авто;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаНомераИДатыНижняя);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ОригиналПолучен);
	...
	Элементы.ГруппаНесколькоПродавцов.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаНесколькоПродавцов);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаСчетФактураСубкомиссионер);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаСводныйСчетФактура);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.НадписьВыборДокументаОснования);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаДокументОснования);
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.НадписьДокументыОснования);
	...
	Элементы.ГруппаВидаОперации.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаПрочее(Элементы,
		Элементы.ГруппаПолучениеСчетаФактуры);
	...
	Элементы.ГруппаЕстьКомиссия.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаПодвал(Элементы, Элементы.ГруппаИтогиСКомиссией);
	
	МобильныйКлиентБПФормаОбъекта.УстановитьИсключенияИзАдаптации(ЭтотОбъект, "ДатаПолучения,ДатаИсправления,ДатаВходящегоДокумента");
