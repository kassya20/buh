...
Функция ВыгрузитьДокумент(УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если НалоговыйПериод >= '2026-01-01' Тогда
		Результат = УчетНДСФормированиеОтчетности.ЭлектронноеПредставлениеКнигиПродаж_508(ЭтотОбъект);
	...
	Иначе
		ИмяФункции = "Документ.КнигаПродажДляПередачиВЭлектронномВиде.МодульОбъекта.ВыгрузитьДокумент";
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Функция %1:
			|Формирование электронного представления книги продаж за выбранный налоговый период не предусмотрено'"),
			ИмяФункции);
