...
&НаКлиенте
Перем ОтправкаВДиректБанкИзФормы;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ДокументОплачен = ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОплачено(Состояние);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОбновитьСостояниеОбменСБанками" Тогда
		...
		ПлатежноеПоручениеФормыКлиент.ОповеститьОЗаписиПлатежногоПорученияНаУплатуНалога(
			Объект, ДокументОплачен);
		ДокументОтправлен = ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеУспешнойОтправки(Состояние);
		Если ОтправкаВДиректБанкИзФормы = Истина И ДокументОтправлен  И Открыта() Тогда
			Закрыть();
		КонецЕсли;
...
Процедура ДатаПриИзменении(Элемент)
	...
	ИначеЕсли ПлатежиВБюджетКлиентСервер.ДействуетПриказ2020_199н_Отложенный(ПредыдущаяДата)
		<> ПлатежиВБюджетКлиентСервер.ДействуетПриказ2020_199н_Отложенный(Объект.Дата) Тогда
		ИсправитьПоказательОснования();
...
Процедура ОтправитьВБанк(Команда)
	...
	Если Параметры.Ключ.Пустая() Или Модифицированность Тогда
		...
		РезультатПроверки = Записать(ПараметрыЗаписи);
	КонецЕсли;
	Если РезультатПроверки Тогда
		ОтправкаВДиректБанкИзФормы = Истина;
...
Процедура Выгрузить(Команда)
	...
	Если РезультатПроверки Тогда
		ДокументыОплатыВФайл = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка);
		Оповещение = Новый ОписаниеОповещения("ВыгрузитьДокументЗавершение", ЭтотОбъект);
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если ЭтоИнтерфейсИнтеграцииСБанком Тогда
		НомерПлатежногоПоручения = УчетДенежныхСредствКлиентСервер.НомерОбъектаБезПрефикса(НомерИнтеграции);
	КонецЕсли;
	УстановитьЗаголовокФормыПриИнтерфейсеИнтеграцииСБанком(ЭтотОбъект);
...
Процедура УстановитьРежимИнтерфейсаИнтеграцииСБанком()
	...
	Элементы.НомерИнтеграции.Видимость            = НЕ ЭтоИнтерфейсИнтеграцииСБанком;
...
Процедура ПолучитьИПоказатьНомерДокументаИнтеграции()
	...
	Если Документы.ПлатежноеПоручение.НомерПлатежногоПорученияИзвестен(Объект.Ссылка, Объект.СчетОрганизации) Тогда
		...
		АвтоЗаголовок = Не ЭтоИнтерфейсИнтеграцииСБанком;
	Иначе
		АвтоЗаголовок = Ложь;
...
&НаСервере
Процедура ИсправитьПоказательОснования()
	
	ПлатежиВБюджетКлиентСервер.ИсправитьПоказательОснования(
		Объект.ВидПеречисленияВБюджет, Объект.Дата, Объект.ПоказательОснования);
...
Процедура ОтобразитьСостояниеОбменаСБанком()
	...
	ВидимостьОшибкиОтправки     = ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОшибки(Состояние);
	ВидимостьУспешнойОтправки   = ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеУспешнойОтправки(Состояние);
	ВидимостьОплачено           = ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОплачено(Состояние);
	
	Если ЭтоИнтерфейсИнтеграцииСБанком Тогда
		Элементы.ФормаВыгрузить.Видимость = Ложь;
		Элементы.ФормаОтправитьВБанк.Видимость = Ложь;
		Элементы.ФормаПровестиИЗакрыть.Видимость = Истина;
		Элементы.ФормаЗаписать.Видимость = Ложь;
		
	ИначеЕсли НастроенПрямойОбменСБанком Тогда
		ВидимостьГотовностьОтправки = Не (ВидимостьОшибкиОтправки Или ВидимостьУспешнойОтправки Или ВидимостьОплачено);
		
		Элементы.ФормаВыгрузить.Видимость = Ложь;
		Элементы.ФормаОтправитьВБанк.Видимость = ВидимостьГотовностьОтправки;
		Элементы.ФормаОтправитьВБанк.КнопкаПоУмолчанию = ВидимостьГотовностьОтправки;
		Элементы.ФормаПровестиИЗакрыть.Видимость = Не ВидимостьГотовностьОтправки;
		Элементы.ФормаПровестиИЗакрыть.КнопкаПоУмолчанию = Не ВидимостьГотовностьОтправки;
	Иначе
		ВидимостьГотовностьОтправки = Не (ВидимостьОшибкиОтправки Или ВидимостьОплачено);
		
		Элементы.ФормаОтправитьВБанк.Видимость = Ложь;
		Элементы.ФормаВыгрузить.Видимость = ВидимостьГотовностьОтправки;
		Элементы.ФормаВыгрузить.КнопкаПоУмолчанию = ВидимостьГотовностьОтправки;
		Элементы.ФормаПровестиИЗакрыть.Видимость = Не ВидимостьГотовностьОтправки;
		Элементы.ФормаПровестиИЗакрыть.КнопкаПоУмолчанию = Не ВидимостьГотовностьОтправки;
	КонецЕсли;
...
&НаСервере
Процедура НастроитьКоманднуюПанель()
	...
	Если КоманднаяПанельПодменюДиректБанк <> Неопределено Тогда
		...
		Если КоманднаяПанельПодменюДиректБанкОбычное <> Неопределено Тогда
			ПеренестиКнопкиВДополнительноеПодменю(КоманднаяПанельПодменюДиректБанкОбычное);
		КонецЕсли;
		Элементы.Переместить(КоманднаяПанельПодменюДиректБанк, Элементы.ГруппаКоманднаяПанель);
	КонецЕсли;
	
	ПеренестиКнопкиВДополнительноеПодменю(Элементы.ПодменюПечать);
	ПеренестиКнопкиВДополнительноеПодменю(Элементы.КомандыЭДО);
	ПеренестиКнопкиВДополнительноеПодменю(Элементы.ПодменюЗаполнить);
	ПеренестиКнопкиВДополнительноеПодменю(Элементы.ПодменюОтчеты);
	
КонецПроцедуры

&НаСервере
Процедура ПеренестиКнопкиВДополнительноеПодменю(Подменю)
	
	Для Каждого ЭлементФормы Из Подменю.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(ЭлементФормы) = Тип("ГруппаФормы") Тогда
			ПеренестиКнопкиВДополнительноеПодменю(ЭлементФормы);
		Иначе
			ЭлементФормы.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ВыгрузитьДокументЗавершение(ПлатежныеПоручения, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ПровестиИЗакрытьНаКлиенте()
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	ИменаКомандМК = "ФормаОтправитьВБанк,ФормаВыгрузить,ФормаПровестиИЗакрыть,ФормаЗаписать";
