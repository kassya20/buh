...
Процедура ОбновитьСписокПодбора()
	...
	Если ПомеченныеНаУдалениеСервер.СкрыватьПомеченныеНаУдаление() Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ОтборПометкаУдаления", "И ПлатежноеПоручение.ПометкаУдаления = ЛОЖЬ");
	КонецЕсли;
...
Процедура СформироватьСтрокиВыпискиСервер(ТекстПредупреждения)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ДокументСписания.Дата = ОтборДата; // для корректного подбора курса валюты для договоров в у.е.
			ДокументСписания.Заполнить(Выборка.Ссылка);
			ДокументСписания.Дата = ОтборДата; // вернем дату, т.к. в предыдущей строке она могла быть затерта при заполнении
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
		...
		Если ЗначениеЗаполнено(Источник) Тогда
			...
			Если СтрокаДокументаПоИсточнику.Количество() > 0 Тогда
				...
				Если УчетДенежныхСредствКлиентСервер.СписаниеСредствНаДругойСчет(СтрокаДокумента.ВидОперации) Тогда
