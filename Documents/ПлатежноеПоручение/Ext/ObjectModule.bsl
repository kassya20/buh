...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения <> Неопределено Тогда
		...
		ИначеЕсли ТипДанныхЗаполнения = Тип("Массив")
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения[0]) = Тип("ДокументСсылка.ВыплатыСамозанятым") Тогда
				ЗаполнитьДокументПоРееструВыплатыСамозанятым(ДанныеЗаполнения[0]);
			Иначе
	...
	Если ДанныеЗаполнения = Неопределено Тогда
		...
		ЭтоЕНП = Документы.ПлатежноеПоручение.ЭтоУплатаЕНП(ПеречислениеВБюджет, Налог, КодБК);
		...
		АвтоЗначенияРеквизитов = УчетДенежныхСредствБП.СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(
			Организация, СчетОрганизации, Контрагент, СчетКонтрагента, ПеречислениеВБюджет, Дата, , , ЭтоЕНП);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	УплатаНалогаЗаТретьеЛицо
		= ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалогаЗаТретьихЛиц;
	ПеречислениеПоИспЛисту
		= ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника
			Или ПлатежиВБюджетКлиентСервер.ЭтоПлатежПоИсполнительномуЛисту(СтатусСоставителя);
	ЭтоПереводНаДругойСчет = УчетДенежныхСредствКлиентСервер.СписаниеСредствНаДругойСчет(ВидОперации);
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Иначе
			...
			Если НЕ РезультатПроверки.СоответствуетТребованиям Тогда
				...
				Если НЕ((ИННПлательщика = ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение() ИЛИ РезультатПроверкиФизЛицо.СоответствуетТребованиям)
					И ПеречислениеПоИспЛисту ИЛИ УплатаНалогаЗаТретьеЛицо) Тогда
					МассивНепроверяемыхРеквизитов.Добавить("ИННПлательщика");
					Если Не (ПеречислениеВБюджет И ПеречислениеПоИспЛисту) Тогда
		...
		Если ПеречислениеВБюджет И Не ПеречислениеПоИспЛисту Тогда
			Если ПустаяСтрока(КПППлательщика) Тогда
				...
				Иначе
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "Организация", "Объект", Отказ);
				КонецЕсли;
			ИначеЕсли Не ПлатежиВБюджетКлиентСервер.РеквизитЗаполнен(КПППлательщика) Тогда
			...
			Если Не УчетнаяПолитика.ПлательщикЕНП(Организация, Дата) Тогда
				Если ЗначениеЗаполнено(СчетКонтрагента) Тогда
					ЗначенияРеквизитовЕдиногоНалоговогоСчета = ЕдиныйНалоговыйСчет.ЗначенияРеквизитовЕдиногоНалоговогоСчета();
					НомерСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетКонтрагента, "НомерСчета");
					Если НомерСчета = ЗначенияРеквизитовЕдиногоНалоговогоСчета.РасчетныйСчет Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru = 'Указан счет единого налогового платежа, но организация не является плательщиком ЕНП'"),
						ЭтотОбъект, "СчетКонтрагента", "Объект", Отказ);
					КонецЕсли;
				КонецЕсли;
				Если ЗначениеЗаполнено(Налог) Тогда
					Если Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж) Тогда
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru = 'Организация не является участником эксперимента по уплате единого налогового платежа'"),
						ЭтотОбъект, "Налог", "Объект", Отказ);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Если ПустаяСтрока(ИННПолучателя) Тогда
			...
			Если КонтрагентЮрЛицо И Не КонтрагентНерезидент Тогда
			...
			КонецЕсли;
		Иначе
			Если КонтрагентНерезидент
				И (СтрДлина(СокрП(ИННПолучателя)) = 5 Или ИННПолучателя = ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение()) Тогда
				// Для нерезидента может указываться КИО, который состоит из 5 знаков или "0".
			ИначеЕсли КонтрагентЮрЛицо Или ПлатежиВБюджетКлиентСервер.РеквизитЗаполнен(ИННПолучателя) Тогда
				...
				РезультатПроверки = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(
					ИННПолучателя, КонтрагентЮрЛицо);
				Если Не РезультатПроверки.СоответствуетТребованиям И ЗначениеЗаполнено(РезультатПроверки.ОписаниеОшибки) Тогда
					МассивНепроверяемыхРеквизитов.Добавить("ИННПолучателя");
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						РезультатПроверки.ОписаниеОшибки, ЭтотОбъект, "Контрагент", "Объект", Отказ);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым Тогда
		Если ЗначениеЗаполнено(РеестрВыплатСамозанятым) Тогда
			...
			Иначе
				...
				Если СуммаЗадолженности <> СуммаДокумента Тогда
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Сумма не соответствует реестру выплат самозанятым, размер задолженности %1 руб.'"), СуммаЗадолженности);
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Поле", "Корректность", НСтр("ru = 'Сумма документа'"), , , ТекстСообщения);
					ОбщегоНазначения.СообщитьПользователю(
	...
	Если Документы.ПлатежноеПоручение.ДокументМожетБытьОформленВЦифровыхРублях(ВидОперации, Дата) Тогда
		Если ЗначениеЗаполнено(СчетОрганизации) Тогда
			Если Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетОрганизации) Тогда
				МассивНепроверяемыхРеквизитов.Добавить("СчетКонтрагента");
				Если ЗначениеЗаполнено(Контрагент) И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
					Если ЗначениеЗаполнено(СчетКонтрагента) Тогда
						Если Не Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетКонтрагента) Тогда
							ТекстСообщения = НСтр("ru = 'В оплате с цифрового счета у контрагента должен быть указан цифровой счет'");
							ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
								"Поле", "Корректность", НСтр("ru = 'Счет получателя'"),,, ТекстСообщения);
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								ТекстОшибкиЗаполнения, ЭтотОбъект, "СчетКонтрагента", "Объект", Отказ);
						КонецЕсли;
					Иначе
						ОГРН = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "РегистрационныйНомер");
						Если Не ЗначениеЗаполнено(ОГРН) Тогда
							ТекстСообщения = НСтр("ru = 'Заполните поле ОГРН в карточке контрагента'");
							ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
								"Поле", "Корректность", НСтр("ru = 'Контрагент'"),,, ТекстСообщения);
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								ТекстОшибкиЗаполнения, ЭтотОбъект, "Контрагент", "Объект", Отказ);
							
							ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
								"Поле", , НСтр("ru = 'Счет получателя'"));
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
								ТекстОшибкиЗаполнения, ЭтотОбъект, "СчетКонтрагента", "Объект", Отказ);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			Иначе
				Если ЗначениеЗаполнено(Контрагент) И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты")
					И ЗначениеЗаполнено(СчетКонтрагента)
					И Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетКонтрагента) Тогда
					ТекстСообщения = НСтр("ru = 'Оплата на счет цифрового рубля не поддерживается'");
					ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Поле", "Корректность", НСтр("ru = 'Счет получателя'"),,, ТекстСообщения);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						ТекстОшибкиЗаполнения, ЭтотОбъект, "СчетКонтрагента", "Объект", Отказ);
	...
	Иначе
		Если ЗначениеЗаполнено(СчетОрганизации)
			И Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетОрганизации) Тогда
			ТекстСообщения = НСтр("ru = 'Оплата со счета цифрового рубля не поддерживается'");
			ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
				"Поле", "Корректность", НСтр("ru = 'Счет организации'"),,, ТекстСообщения);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибкиЗаполнения, ЭтотОбъект, "СчетОрганизации", "Объект", Отказ);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СчетКонтрагента)
			И Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетКонтрагента) Тогда
			ТекстСообщения = НСтр("ru = 'Оплата контрагенту на счет цифрового рубля не поддерживается'");
			ТекстОшибкиЗаполнения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
				"Поле", "Корректность", НСтр("ru = 'Счет получателя'"),,, ТекстСообщения);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ТекстОшибкиЗаполнения, ЭтотОбъект, "СчетКонтрагента", "Объект", Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЦифровойСчетОрганизации = Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетОрганизации);
	ЦифровойСчетКонтрагента = Справочники.БанковскиеСчета.ЭтоЦифровойСчет(СчетКонтрагента);
	ЭтоЦифровойСчет = ЦифровойСчетОрганизации Или ЦифровойСчетКонтрагента;
	Если ЭтоЦифровойСчет И Не ЗначениеЗаполнено(ИдентификаторОперацииЦР) Тогда
		ИдентификаторОперацииЦР = Новый УникальныйИдентификатор;
	ИначеЕсли Не ЭтоЦифровойСчет И ЗначениеЗаполнено(ИдентификаторОперацииЦР) Тогда
		ИдентификаторОперацииЦР = "";
	КонецЕсли;
	
	ПроверитьУдалитьПравилоРегулярногоПлатежа();
	
	ВыполнениеЗадачБухгалтера.ПередЗаписьюПлатежа(ЭтотОбъект);
...
Процедура ПриЗаписи(Отказ)
	...
	РегистрыСведений.СостоянияБанковскихДокументов.УстановитьСостояниеДокумента(ЭтотОбъект, Состояние, , Истина);
	...
	КонецЕсли;
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника Тогда
		РеквизитыПеречисления = РегистрыСведений.РеквизитыПеречисленийПоИсполнительнымЛистам.СтруктураРеквизитов();
		ЗаполнитьЗначенияСвойств(РеквизитыПеречисления, ЭтотОбъект);
...
Процедура ПроверитьОбновитьРеквизитыПлатежаВБюджет(ОбъектКопирования)
	...
	ЭтоУплатаОпределенногоНалога = ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога
		И ВидПеречисленияВБюджет = Перечисления.ВидыПеречисленийВБюджет.НалоговыйПлатеж
		И Налог <> Справочники.ВидыНалоговИПлатежейВБюджет.ПрочиеНалогиИСборы;
	
	СменаНалогаНаЕНП = ПлатежиВБюджетКлиентСервер.ДействуетПереходНаЕНП(ОбъектКопирования.Дата)
		<> ПлатежиВБюджетКлиентСервер.ДействуетПереходНаЕНП(Дата)
		Или ПлатежиВБюджетКлиентСервер.ДействуетПереходныйПериодЕНП(ОбъектКопирования.Дата)
			<> ПлатежиВБюджетКлиентСервер.ДействуетПереходныйПериодЕНП(Дата);
	
	ЭтоНалоговыйПлатеж = ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(КодБК);
	
	ВидНалога = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Налог, "ВидНалога");
	Если Не ПлатежиВБюджетКлиентСервер.ДействуетПереходНаЕНП(ОбъектКопирования.Дата)
		Или СменаНалогаНаЕНП
		Или ЭтоУплатаОпределенногоНалога Тогда
		НеактуальныеНалоги = ПлатежиВБюджетКлиентСерверПереопределяемый.НеактуальныеНалоги(Дата);
	ИначеЕсли Не ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(Дата)
		Или ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(ОбъектКопирования.Дата) = ПлатежиВБюджетКлиентСервер.ДействуетПриказ2017_90н(Дата)
		Или Не ЭтоУплатаОпределенногоНалога Тогда
		НеактуальныеНалоги = Новый Соответствие;
	Иначе
		НеактуальныеНалоги = ПлатежиВБюджетКлиентСерверПереопределяемый.НеактуальныеНалоги(Дата);
	КонецЕсли;
	
	ПереходНаЕНП = УчетнаяПолитика.ПлательщикЕНП(Организация, Дата) И СменаНалогаНаЕНП;
	
	НалогиУплачиваемыеОтдельно = ЕдиныйНалоговыйСчетПовтИсп.МассивДоступныхВидовНалогов(Дата);
	НалогУплачиваетсяОтдельно  = НалогиУплачиваемыеОтдельно.Найти(ВидНалога) <> Неопределено;
	
	НеактуальныйВзносПФРФСС = НеактуальныеНалоги[ВидНалога] <> Неопределено
		И (НалогУплачиваетсяОтдельно Или Не ПереходНаЕНП);
	
	Если НеактуальныйВзносПФРФСС Тогда
		...
		Если Контрагент <> РеквизитыВБюджетПоУмолчанию.Получатель Тогда
	...
	Если УчетнаяПолитика.ПлательщикЕНП(Организация, Дата) И СменаНалогаНаЕНП Тогда
		НалогиУплачиваемыеОтдельно = ЕдиныйНалоговыйСчетПовтИсп.МассивДоступныхВидовНалогов(Дата);
		Если ЗначениеЗаполнено(ВидНалога)
			И ВидНалога <> Перечисления.ВидыНалогов.ПрочиеНалогиИСборы Тогда
			СчетКонтрагента = Неопределено; // очищаем, тогда счет контрагента будет перезаполнен актуальным значением
			Если НалогиУплачиваемыеОтдельно.Найти(ВидНалога) = Неопределено Тогда
		...
		РеквизитыПлатежногоПоручения = Документы.ПлатежноеПоручение.РеквизитыПлатежногоПорученияВБюджетПоУмолчанию(
			Дата,
			Организация,
			ПеречислениеВБюджет,
			Налог,
		...
		Если ЗначениеЗаполнено(РеквизитыПлатежногоПоручения.КодБК) Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыПлатежногоПоручения);
			Контрагент = РеквизитыПлатежногоПоручения.Получатель;
			СчетКонтрагента = РеквизитыПлатежногоПоручения.СчетПолучателя;
			АвтоЗначенияРеквизитов = УчетДенежныхСредствБП.СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(
				Организация,
				СчетОрганизации,
				Контрагент,
				...
				Дата);
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, АвтоЗначенияРеквизитов, "ИННПолучателя, КПППолучателя, ТекстПолучателя");
			РеквизитыОрганизации = ЕдиныйНалоговыйСчет.КППОКТМООрганизацииДляУплатыВБюджет(Организация, Налог, Дата);
			КПППлательщика = РеквизитыОрганизации.КПП;
			КодОКАТО       = РеквизитыОрганизации.ОКТМО;
		КонецЕсли;
	ИначеЕсли ЭтоНалоговыйПлатеж И Дата > ПлатежиВБюджетКлиентСервер.ДатаСменыНаименованияМИДол() Тогда
		Реквизиты = ЕдиныйНалоговыйСчет.РеквизитыМИДол();
		МИДол     = ДанныеГосударственныхОрганов.ГосударственныйОрган(Реквизиты.Вид, Реквизиты.Код);
		ЗаполнитьЗначенияСвойств(Реквизиты, МИДол, "Ссылка");
		Если ЗначениеЗаполнено(Реквизиты.Ссылка) И Контрагент <> Реквизиты.Ссылка Тогда
			Контрагент = Реквизиты.Ссылка;
			СчетКонтрагента = ЕдиныйНалоговыйСчет.ЕдиныйБанковскийСчетГосударственногоОргана(Контрагент);
			ТекстПолучателя = Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа;
			ИННПолучателя   = Реквизиты.ИНН;
			КПППолучателя   = Реквизиты.КПП;
		ИначеЕсли ТекстПолучателя <> Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа Тогда
			ТекстПолучателя = Реквизиты.ПлатежныеРеквизиты.ПолучательПлатежа;
		КонецЕсли;
	КонецЕсли;
	
	ПлатежиВБюджетКлиентСервер.ПроверитьИсправитьНедопустимыеРеквизитыПлатежаВБюджет(
		ВидПеречисленияВБюджет, Дата, СтатусСоставителя, ПоказательОснования);
	
	Если ПеречислениеВБюджет
		И ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н(Дата)
		И Не ПлатежиВБюджетКлиентСервер.ДействуетПриказ58н(ОбъектКопирования.Дата) Тогда
		ЭтоИП = Не ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация);
		УплатаНалогаЗаТретьихЛиц =
			ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалогаЗаТретьихЛиц;
		ЭтоЕНП = Документы.ПлатежноеПоручение.ЭтоУплатаЕНП(ПеречислениеВБюджет, Налог, КодБК);
		
		Плательщик = ?(УплатаНалогаЗаТретьихЛиц, Налогоплательщик, Организация);
		АвтоЗначенияРеквизитов = УчетДенежныхСредствБП.СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(
			Плательщик,
			СчетОрганизации,
			Контрагент,
		...
		Если ЭтоИП Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, АвтоЗначенияРеквизитов, "ТекстПлательщика");
		КонецЕсли;
		
		Если ЭтоЕНП Тогда
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, АвтоЗначенияРеквизитов, "КПППлательщика");
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.ВыплатыСамозанятым") Тогда
		
		ЗаполнитьДокументПоРееструВыплатыСамозанятым(Основание);
	...
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.АвансовыйОтчет") Тогда
		...
		ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "ФизЛицо, Номер, Дата, СуммаДокумента");
		
		Контрагент = ДанныеОснования.ФизЛицо;
		
		СтруктураПеречисления =
			Новый Структура("Организация, ФизическоеЛицо, ВидОперации, БанковскийСчет, НазначениеПлатежа");
		ЗаполнитьЗначенияСвойств(СтруктураПеречисления, ЭтотОбъект);
		СтруктураПеречисления.ФизическоеЛицо = Контрагент;
		ОсновнойВариантПеречисления =
			РегистрыСведений.ОсновныеВариантыПеречисленияСотруднику.ПолучитьОсновнойВариантПеречисления(СтруктураПеречисления);
		
		СчетКонтрагента = ОсновнойВариантПеречисления.БанковскийСчет;
		Если ЗначениеЗаполнено(СчетКонтрагента) Тогда
			ВладелецСчета = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетКонтрагента, "Владелец");
			Если ВладелецСчета <> Контрагент Тогда
				Банк = ВладелецСчета;
			КонецЕсли;
		Иначе
			УчетДенежныхСредствБП.УстановитьБанковскийСчет(СчетКонтрагента, Контрагент, ВалютаДокумента);
		...
		ТекстНазначенияПлатежа  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонНазначенияПлатежа,
		...
		ПерерасходАванса = Макс(0, ДанныеОснования.СуммаДокумента - СуммаАванса);
		...
		СтандартныйТекстНазначения = НСтр("ru = 'Перечисление подотчетной суммы'");
		НазначениеПлатежа = СтрЗаменить(НазначениеПлатежа, СтандартныйТекстНазначения, ТекстНазначенияПлатежа);
	...
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.НачислениеДивидендов") Тогда
		...
		ВалютаДокумента      = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
		СуммаДокумента       = Основание.СуммаДохода - Основание.СуммаНалога - Основание.СуммаНалогаСПревышения;
...
Процедура ЗаполнитьДокументПоВедомости(Основание)
	...
	Если УчетЗарплаты.ВедомостьПоДоговоруПодряда(ДокументОснование) Тогда
		ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникамПоДоговорамПодряда;
	КонецЕсли;
	...
	КодВидаДохода = УчетЗарплаты.КодВидаДоходаПоВидуДоходаИсполнительногоПроизводства(
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Основание,
	...
	Если УчетДенежныхСредствКлиентСервер.ДействуетУказаниеБанкаРоссии5286У(Дата)
		И Не ЗначениеЗаполнено(КодВидаДохода) Тогда
		КодВидаДохода = "1";
	КонецЕсли;
...
Процедура ЗаполнитьДокументПоРееструВыплатыСамозанятым(Основание)
	
	ДокументОснование = Основание;
	
	ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВыплатыСамозанятым;
	
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Организация, ПодразделениеОрганизации, СуммаДокумента");
	Организация = СтруктураРеквизитов.Организация;
	ПодразделениеОрганизации = СтруктураРеквизитов.ПодразделениеОрганизации;
	СуммаДокумента = СтруктураРеквизитов.СуммаДокумента;
	
	ВалютаРегламентированногоУчета = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	УчетДенежныхСредствБП.УстановитьБанковскийСчет(СчетОрганизации, Организация, ВалютаРегламентированногоУчета);
	
	РеестрВыплатСамозанятым = Основание;
	
	АвтоЗначенияРеквизитов = УчетДенежныхСредствБП.СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(
		Организация, СчетОрганизации, Контрагент, СчетКонтрагента, Ложь, Дата);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, АвтоЗначенияРеквизитов);
...
Процедура ЗаполнитьПоСписаниюСРасчетногоСчета(Основание)
	...
	РеестрВыплатСамозанятым            = Основание.РеестрВыплатСамозанятым;
	Налог                              = Основание.Налог;
	Сотрудник                          = Основание.Сотрудник;
	ИсполнительныйЛист                 = Основание.ИсполнительныйЛист;
	ПеречислениеПоИсполнительномуЛисту =
		Основание.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника;
	...
	КодВалютнойОперации            = Основание.КодВалютнойОперации;
	...
	ВозвратКредитаЗаймаСуммаПроцентов = 0;
	Если Основание.РасшифровкаПлатежа.Количество() = 1 Тогда
		...
		Если Основание.РасшифровкаПлатежа[0].ВидПлатежаПоКредитамЗаймам
			= Перечисления.ВидыПлатежейПоКредитамЗаймам.УплатаПроцентов Тогда
			ВозвратКредитаЗаймаСуммаПроцентов = СуммаДокумента;
		КонецЕсли;
	ИначеЕсли Основание.РасшифровкаПлатежа.Количество() > 1 Тогда
		...
		НазначениеПлатежаБылоИзмененоВручную = ЗаполненоНазначениеПлатежа;
		Для Каждого СтрокаОснования Из Основание.РасшифровкаПлатежа Цикл
			Если СтрокаОснования.ВидПлатежаПоКредитамЗаймам = Перечисления.ВидыПлатежейПоКредитамЗаймам.УплатаПроцентов Тогда
				ВозвратКредитаЗаймаСуммаПроцентов = ВозвратКредитаЗаймаСуммаПроцентов + СтрокаОснования.СуммаПлатежа;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДивидендов Тогда
		...
		КонецЕсли;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратКредита
		Или ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратЗайма Тогда
		СуммаПроцентов = ВозвратКредитаЗаймаСуммаПроцентов;
	КонецЕсли;
	...
	АвтоЗначенияРеквизитов = УчетДенежныхСредствБП.СформироватьАвтоЗначенияРеквизитовПлательщикаПолучателя(
		...
		СчетОрганизации,
		?(УчетДенежныхСредствКлиентСервер.СписаниеСредствНаДругойСчет(ВидОперации), Организация, Контрагент),
		СчетКонтрагента,
		ПеречислениеВБюджет,
		Дата,
		РегистрацияВНалоговомОргане,
	...
	Если ПеречислениеВБюджет Тогда
		...
		ИсточникДанныхКонтекста = Документы.ПлатежноеПоручение.ИсточникДанныхКонтекста(
			Дата, Организация, ПеречислениеВБюджет, Налог, ВидНалоговогоОбязательства, РегистрацияВНалоговомОргане, СчетКонтрагента, ПеречислениеПоИсполнительномуЛисту);
...
Процедура ЗаполнитьПоСтруктуре(ДанныеЗаполнения)
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента)
		И (Не ЗначениеЗаполнено(Контрагент) Или Не ЗначениеЗаполнено(ВалютаДокумента)) Тогда
		...
		Если РеквизитыДоговора.ОплатаВВалюте И Не ЗначениеЗаполнено(ВалютаДокумента) Тогда
	...
	Если НЕ ЗначениеЗаполнено(ВидОперации) Тогда
		...
		ИначеЕсли ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И ЗначениеЗаполнено(Контрагент) Тогда
			...
			КонецЕсли;
		ИначеЕсли ДанныеЗаполнения.Свойство("КодБК") И ЗначениеЗаполнено(ДанныеЗаполнения.КодБК) Тогда
			ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога;
			Налог = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоКБК(ДанныеЗаполнения.КодБК);
		КонецЕсли;
	КонецЕсли;
	...
	Если ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога Тогда
		...
		Если Не ЗначениеЗаполнено(Контрагент) И ПравилаЗаполнения.ЗаполнятьПолучателя И ЗначениеЗаполнено(Налог) Тогда
			
			ВидИКодГосударственногоОргана = ДанныеГосударственныхОрганов.ВидИКодГосударственногоОрганаПоНалогу(
				Налог, Организация, Дата);
			Если ЗначениеЗаполнено(ВидИКодГосударственногоОргана.Код) Тогда
				ПолучательПлатежа = Документы.ПлатежноеПоручение.ПолучательДляПлатежаГосударственномуОргануПоПравиламУплаты(
					Организация, Дата, ВидИКодГосударственногоОргана.Вид, ВидИКодГосударственногоОргана.Код, Налог);
				Контрагент      = ПолучательПлатежа.Контрагент;
				СчетКонтрагента = ПолучательПлатежа.БанковскийСчет;
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(Контрагент) И ЗначениеЗаполнено(ИННПолучателя) И ЗначениеЗаполнено(КПППолучателя) Тогда
				ДанныеКонтрагента = Новый Структура;
				ДанныеКонтрагента.Вставить("НаименованиеПолное",        ТекстПолучателя);
				ДанныеКонтрагента.Вставить("Наименование",              ТекстПолучателя);
				ДанныеКонтрагента.Вставить("ИНН",                       ИННПолучателя);
				ДанныеКонтрагента.Вставить("КПП",                       КПППолучателя);
				ДанныеКонтрагента.Вставить("ЮридическоеФизическоеЛицо", Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо);
				ДанныеКонтрагента.Вставить("ГосударственныйОрган",      Истина);
				ДанныеКонтрагента.Вставить("ВидГосударственногоОргана", ВидИКодГосударственногоОргана.Вид);
				ДанныеКонтрагента.Вставить("КодНалоговогоОргана",       ВидИКодГосударственногоОргана.Код);
				
				Контрагент = Справочники.Контрагенты.НайтиСоздатьНалоговуюИнспекцию(ДанныеКонтрагента);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Контрагент) И Не ЗначениеЗаполнено(СчетКонтрагента)
			И ДанныеЗаполнения.Свойство("РеквизитыСчетаПолучателя") Тогда
			
			РеквизитыСчета = ДанныеЗаполнения.РеквизитыСчетаПолучателя;
			СчетКонтрагентаПоРеквизитам = ДанныеГосударственныхОрганов.БанковскийСчетПоРеквизитам(
				Контрагент,
				РеквизитыСчета.НомерСчета,
				РеквизитыСчета.БИК,
				РеквизитыСчета.КоррСчет,
				РеквизитыСчета.НаименованиеБанка,
				Истина);
				
			Если ЗначениеЗаполнено(СчетКонтрагентаПоРеквизитам) Тогда
				СчетКонтрагента = СчетКонтрагентаПоРеквизитам;
			КонецЕсли;
			
		КонецЕсли;
		
		ИсточникДанныхКонтекста = Документы.ПлатежноеПоручение.ИсточникДанныхКонтекста(
			Дата, Организация, ПеречислениеВБюджет, Налог, ВидНалоговогоОбязательства, РегистрацияВНалоговомОргане, СчетКонтрагента, Ложь);
	...
	Иначе
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, АвтоЗначенияРеквизитов
			,, ?(ЗначениеЗаполнено(КПППлательщика), "КПППлательщика", ""));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Налог) Тогда
		...
		ДанныеЗаполненияИННПолучателя = "";
		ДанныеЗаполненияКПППолучателя = "";
		
		Если ДанныеЗаполнения.Свойство("ИННПолучателя", ДанныеЗаполненияИННПолучателя)
			И ДанныеЗаполнения.Свойство("КПППолучателя", ДанныеЗаполненияКПППолучателя)
			И ЗначениеЗаполнено(ДанныеЗаполненияИННПолучателя)
			И ЗначениеЗаполнено(ДанныеЗаполненияКПППолучателя) Тогда
			
			ИННПолучателя = ДанныеЗаполненияИННПолучателя;
			КПППолучателя = ДанныеЗаполненияКПППолучателя;
		КонецЕсли;
		
		Если ДанныеЗаполнения.Свойство("ТекстПолучателя") И ЗначениеЗаполнено(ДанныеЗаполнения.ТекстПолучателя) Тогда
...
Процедура ЗаполнитьПоМассиву(ДанныеЗаполнения)
	...
	Для Каждого Основание Из ДанныеЗаполнения Цикл
		...
		РеквизитыДоговора = "ВалютаВзаиморасчетов, РасчетыВУсловныхЕдиницах, УчетАгентскогоНДС";
		Если ЗначениеЗаполнено(ДанныеОснования.ДоговорКонтрагента) Тогда
			ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеОснования.ДоговорКонтрагента, РеквизитыДоговора);
		Иначе
			ДанныеДоговора = Новый Структура(РеквизитыДоговора);
			ДанныеДоговора.ВалютаВзаиморасчетов = ВалютаДокумента;
			ДанныеДоговора.РасчетыВУсловныхЕдиницах = Ложь;
			ДанныеДоговора.УчетАгентскогоНДС = Ложь;
		КонецЕсли;
