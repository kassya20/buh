...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Дата < ПерсонифицированныйУчет.ДатаПостановленияАДВ2025() Тогда 
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИмяФайлаДляПФР) Тогда
		ИмяФайлаДляПФР = Документы.ПачкаДокументовСЗВ_К.ИмяФайла(Организация, Дата);
	Иначе		
		...
		КонецЕсли;	
		ИмяФайлаДляПФР = Документы.ПачкаДокументовСЗВ_К.ИмяФайла(Организация, Дата, УИДИзИмениФайла);
	КонецЕсли;
					...
					Если УказаныТерриториальныеУсловия Тогда
						Если Запись.ТерриториальныеУсловия = Справочники.ТерриториальныеУсловияПФР.РКС
							...
							Если НЕ УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий РКС, МКС, РКСР, МКС-РКСР, МКСР и РКСМ должен быть указан районный коэффициент.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						Иначе
							Если УказанРК Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'для территориальных условий, отличных от РКС, МКС, РКСР, МКС-РКСР, МКСР и РКСМ, районный коэффициент указываться не должен.'"), НомерСтроки, Запись.СотрудникНаименование);
								СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
