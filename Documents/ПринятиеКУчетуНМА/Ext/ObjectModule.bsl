...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаИнвестиционныйВычет = ИнвестиционныйНалоговыйВычет.ПодготовитьТаблицуИнвестиционныйВычетПринятиеКУчетуНМА(
		ПараметрыПроведения.ПринятиеКУчетуНМА);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПрименениеПБУ18 Тогда
		Если СтоимостьНУ = 0 И СтоимостьПР = 0 И СтоимостьВР = 0 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "СтоимостьНУ");
		КонецЕсли;
		МассивНепроверяемыхРеквизитов.Добавить("СтоимостьНУ");
	КонецЕсли;
	
	Если Не РаздельныйУчетНДСНаСчете19 
		Или ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуНМА.ПринятиеКУчетуНаОснованииИнвентаризации Тогда
	...
	Если ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуНМА.ПринятиеКУчетуНаОснованииИнвентаризации Тогда
