...
Функция ОбновитьВторичныеДанные(ПараметрыФиксации = Неопределено) Экспорт
	...
	Если ЗаполнитьИсключаемыеПериодыПримечание(ПараметрыФиксации) Тогда
		Модифицирован = Истина;
	КонецЕсли;
...
Функция ЗаполнитьИсключаемыеПериодыПримечание(ПараметрыФиксации)
	Реквизиты = Новый Структура("ИсключаемыеПериодыПримечание,ИсключаемыеПериодыВыгружатьПримечание", "", Ложь);
	Если ИсключаемыеПериоды.Количество() = 0 Тогда
		Возврат ФиксацияВторичныхДанныхВДокументах.ОбновитьДанныеШапки(Реквизиты, ЭтотОбъект, ПараметрыФиксации);
	КонецЕсли;
	
	Отбор = Новый Структура("Причина", Перечисления.ПричиныНеоплатыБольничных.ПределОплаты5МесяцевДляИнвалидов);
	ПределОплаты = ИсключаемыеПериоды.НайтиСтроки(Отбор);
	КоличествоСтрок = ПределОплаты.Количество();
	
	Если КоличествоСтрок > 0 И ЗначениеЗаполнено(Сотрудник) Тогда
		Представления = Новый Массив;
		ТекущаяДатаОкончания = Неопределено;
		Реквизиты.ИсключаемыеПериодыВыгружатьПримечание = Истина;
		КадровыеДанные = КадровыеДанныеСотрудника();
		ФорматДня = "ДФ=""д""";
		ФорматМесяц = "ДФ=""д ММММ""";
		Для Каждого Период Из ПределОплаты Цикл
			Если ТекущаяДатаОкончания <> Неопределено Тогда
				ФорматДаты = ?(ФорматДаты = ФорматМесяц Или Месяц(ТекущаяДатаОкончания) <> Месяц(Период.ДатаНачала),
					ФорматМесяц, ФорматДня);
				Представление = Представление + Формат(ТекущаяДатаОкончания, ФорматДаты);
				Представления.Добавить(Представление);
			КонецЕсли;
			КоличествоСтрок = КоличествоСтрок - 1;
			ФорматДаты = ?(Период.ДатаНачала = Период.ДатаОкончания
				Или Месяц(Период.ДатаНачала) = Месяц(Период.ДатаОкончания) Или КоличествоСтрок > 0,
					ФорматДня, ФорматМесяц);
			Если Период.ДатаНачала = Период.ДатаОкончания Тогда
				Представление = ?(КоличествоСтрок = 0, Формат(Период.ДатаОкончания, ФорматМесяц), "");
			Иначе
				Представление = ?(ТекущаяДатаОкончания = Неопределено, НСтр("ru = 'С '"), НСтр("ru = 'с '"))
					+ Формат(Период.ДатаНачала, ФорматДаты) + НСтр("ru = ' по '")
					+ ?(КоличествоСтрок = 0, Формат(Период.ДатаОкончания, ФорматМесяц), "");
			КонецЕсли;
			ТекущаяДатаОкончания = Период.ДатаОкончания;
		КонецЦикла;
		Представления.Добавить(Представление);
		ГруппаИнвалидности = ?(ЗначениеЗаполнено(КадровыеДанные.ИнвалидностьГруппаИнвалидности),
			Лев(Строка(КадровыеДанные.ИнвалидностьГруппаИнвалидности), 1) + НСтр("ru = '-ой группы инвалидности'"),
			НСтр("ru = 'инвалида'"));
		Реквизиты.ИсключаемыеПериодыПримечание = СтрСоединить(Представления, ", ")
			+ СтрШаблон(НСтр("ru = ' превышен предел оплаты 5 месяцев больничного у сотрудника %1.'"),
				ГруппаИнвалидности);
	КонецЕсли;
