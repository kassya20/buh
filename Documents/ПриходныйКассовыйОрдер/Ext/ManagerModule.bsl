...
Процедура УстановитьПравилаЗаполненияСчетовУчета(Правила) Экспорт
	...
	СчетаУчетаВДокументах.ДобавитьВПравилоИсточникДанныхРеквизитОбъекта(Правила, "БанковскийСчет", "Контрагент");
...
Функция ТекстЗапросаДанныеДляОбновленияЦенДокументов() Экспорт
	...
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер.РасшифровкаПлатежа КАК ТаблицаНоменклатуры
	|ГДЕ
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	|		И &УчитыватьЗадолженностьУСНПатент КАК УчитыватьЗадолженностьУСНПатент,
	|	Реквизиты.Основание КАК Содержание,
	|	Реквизиты.Приложение КАК Приложение,
	|	Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПКО.ВозвратОтПоставщика)
	|		И НЕ Реквизиты.БезЗакрывающихДокументов КАК ЭтоВозврат,
...
Функция ТекстЗапросаВозвратОтПодотчетногоЛицаВВалюте(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.СтатьяДвиженияДенежныхСредств КАК КорСубконто1,
	|	НЕОПРЕДЕЛЕНО КАК КорСубконто2,
	|	НЕОПРЕДЕЛЕНО КАК КорСубконто3,
...
Функция ТекстЗапросаПрочееПоступление(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	ИначеЕсли Реквизиты.ВидОперации = Перечисления.ВидыОперацийПКО.ВозвратОтПодотчетногоЛица 
...
Функция ТекстЗапросаВзносВУставныйКапитал(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидОперации <> Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда
...
Процедура ПеренестиУслугиНПДВТабличнуюЧасть(Параметры) Экспорт
	...
	|	ПриходныйКассовыйОрдер.УдалитьУслугаНПД КАК УслугаНПД
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер КАК ПриходныйКассовыйОрдер
	|ГДЕ
	|	ПриходныйКассовыйОрдер.УдалитьУслугаНПД <> &ПустаяУслугаНПД
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Иначе
		...
		Если ОбъектовОбработано = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедуре ПриходныйКассовыйОрдер.ПеренестиУслугиНПДВТабличнуюЧасть
				|не удалось перенести услуги НПД в табличную часть в %1 документах Поступление наличных.'"), 
				ПроблемныхОбъектов);
			ВызватьИсключение ТекстСообщения;
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ПриходныйКассовыйОрдер.ПеренестиУслугиНПДВТабличнуюЧасть
				|обработала очередную порцию документов Поступление наличных: %1 элементов'"), 
				ОбъектовОбработано);
...
Функция ОпределитьКодыСчетСубсчет(ВалютаРегламентированногоУчета, Шапка)
	...
	ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда 
		СтрокаКредит = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПланыСчетов.Хозрасчетный.РасчетыПоВкладамВУставныйКапитал).Код;
...
Функция ТекстЗапросаРеквизитыПечатиЧека(ИменаТаблиц)
	...
	|		ПО ВТ_РасшифровкаПлатежа.Сделка = ПередачаОС.ДокументОснование
	|ГДЕ
...
Процедура ЗаполнитьПоОтчетуОРозничныхПродажах(Объект, Основание) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	СУММА(СуммыОплаты.Сумма) КАК СуммаБезналичнойОплаты
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		ОтчетОРозничныхПродажахЗачетАвансов.Ссылка = &Основание) КАК СуммыОплаты
	|
	|ИМЕЮЩИЕ
	|	ЕСТЬNULL(СУММА(СуммыОплаты.Сумма), 0) > 0");
...
Процедура ЗаполнитьПоИнвентаризацииКассы(Объект, Основание) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(ИнвентаризацияКассыЦенности.ВалютнаяСумма - ИнвентаризацияКассыЦенности.ВалютнаяСуммаУчет) КАК СуммаДокумента,
		...
		|	ИнвентаризацияКассыЦенности.Ссылка.ПодразделениеОрганизации КАК ПодразделениеОрганизацииКт,
		|	ЗНАЧЕНИЕ(Справочник.ПрочиеДоходыИРасходы.ПрочиеВнереализационныеДоходыРасходы) КАК СубконтоКт1
		|ИЗ
