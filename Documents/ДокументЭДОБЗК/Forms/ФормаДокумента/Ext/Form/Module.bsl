...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(Объект.ВидДокумента) Тогда
			Если Объект.ВидДокумента.ЭтоГруппа Тогда
				Объект.ВидДокумента = Неопределено;
			КонецЕсли;
		КонецЕсли;
		
		ПриПолученииДанныхНаСервере();
		УстановитьОтображениеПолейТребованияПодписи(
			ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи());
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ИзменитьВариантПодписания Тогда
		ТекущийОбъект.ВариантПодписания = ВариантПодписания;
		ТекущийОбъект.ТипЭлектроннойПодписи = ТипЭлектроннойПодписи;
	Иначе
		ТекущийОбъект.ВариантПодписания = Неопределено;
		ТекущийОбъект.ТипЭлектроннойПодписи = Неопределено;
	КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьВариантПодписанияПриИзменении(Элемент)
	
	Если Не ИзменитьВариантПодписания Тогда
		ОбновитьДанныеФормыПоВидуДокумента(Истина);
	Иначе
		УстановитьОтображениеЭлементовВариантаПодписания(ЭтотОбъект);
	КонецЕсли;
...
Процедура ВариантПодписанияПриИзменении(Элемент)
	...
	Если ВариантПодписания = ПредопределенноеЗначение("Перечисление.ВариантыПодписанияДокументовЭДОБЗК.Требуется")
		И Не ЗначениеЗаполнено(ТипЭлектроннойПодписи) Тогда
		
		ТипЭлектроннойПодписи = ПредопределенноеЗначение("Перечисление.ТипыЭлектронныхПодписейЭДОБЗК.УНЭП");
	Иначе
		ТипЭлектроннойПодписи = Неопределено;
	КонецЕсли;
	УстановитьОтображениеЭлементовВариантаПодписания(ЭтотОбъект);
...
Процедура ПередатьВ1СКабинетСотрудника(Команда)
	...
	ДополнительныеПараметры = Новый Структура("Действия", ДействияСДокументом());
...
Процедура ПодписатьИПередатьДокумент(Ответ, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если ТребуетсяПодписьОрганизации Тогда
			...
			ЭДОБЗККлиент.ПодписатьПоНастройкамДокумент(
				ЭтотОбъект,
				Объект.Ссылка,
				Объект.ВидДокумента,
...
Процедура ПослеПодписанияФайлаНаСервере(Действие)
	...
	ПрочитатьЭлектронныеПодписи(Ложь);
...
Процедура ВыбратьСотрудников(Команда)
	...
	ПараметрыОткрытия.Вставить("Отбор", Новый Структура("ПодключенКЭДО", Истина));
	
	КадровыйУчетКлиент.ВыбратьСотрудниковРаботающихНаДатуПоПараметрамОткрытияФормыСписка(
		Элементы.ЭлектронныеПодписиСотрудников, Объект.Организация, , Объект.Дата, Истина, АдресСпискаПодобранных(), ПараметрыОткрытия);
...
Процедура НастроитьИПодписать(Команда)
	...
	ДополнительныеПараметры = Новый Структура("Действия", ДействияСДокументом());
	Если Модифицированность Тогда
		ТекстВопроса = НСтр("ru = 'Перед настройкой подписания и передачей документа, его необходимо записать.
			|Продолжить?'");
		Оповещение = Новый ОписаниеОповещения("НастроитьПодписатьИПередатьДокумент", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена);
	Иначе
		НастроитьПодписатьИПередатьДокумент(КодВозвратаДиалога.Да, ДополнительныеПараметры);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НастроитьПодписатьИПередатьДокумент(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ОповещениеЗавершения = Новый ОписаниеОповещения("ПослеПодписанияФайла", ЭтотОбъект, ДополнительныеПараметры);
		ЭДОБЗККлиент.НастроитьИПодписатьДокумент(
			ЭтотОбъект,
			Объект.Ссылка,
			Объект.ВидДокумента,
			ДополнительныеПараметры.Действия[0],
			ОповещениеЗавершения);
...
Процедура Подписать(Команда)
	...
	ЭлектронныеДокументы =
		ЭДОБЗКВызовСервера.ЭлектронныеДокументыПоСсылкамНаДокументыЭДОБЗК(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка));
	ЭДОБЗККлиент.ПодписатьЭлектронныеДокументы(ЭтотОбъект, ЭлектронныеДокументы);
...
&НаСервере
Процедура ПриПолученииДанныхНаСервере(ОбновитьДанныеОбъекта = Истина)
	...
	Если ЗначениеЗаполнено(Объект.ВариантПодписания) Тогда
		ВариантПодписания = Объект.ВариантПодписания;
		Если Объект.ВариантПодписания = Перечисления.ВариантыПодписанияДокументовЭДОБЗК.Требуется Тогда
			Если ЗначениеЗаполнено(Объект.ТипЭлектроннойПодписи) Тогда
				ТипЭлектроннойПодписи = Объект.ТипЭлектроннойПодписи;
			Иначе
				ТипЭлектроннойПодписи = Перечисления.ТипыЭлектронныхПодписейЭДОБЗК.УНЭП;
			КонецЕсли;
		КонецЕсли;
		ИзменитьВариантПодписания = Истина;
	КонецЕсли;
	
	ОбновитьДанныеФормыПоВидуДокумента(ОбновитьДанныеОбъекта);
	
	ПрочитатьДанныеФайла();
	ПрочитатьЭлектронныеПодписи(ОбновитьДанныеОбъекта);
	ПрочитатьСтатусыПРР();
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"СписокСотрудниковКоманднаяПанель",
		"Видимость",
		Объект.Внешний);
...
&НаСервере
Процедура ПрочитатьЭлектронныеПодписи(ОбновитьДанныеОбъекта = Истина)
	...
	Для Каждого СотрудникПодписи Из ПодписиФизическихЛицДокумента Цикл
		...
		Если ЗначениеЗаполнено(СотрудникПодписи.ДатаПодписи) Тогда
			Если Не ЗначениеЗаполнено(СотрудникПодписи.ИдентификаторПриложения) Тогда
				СтрокаПодписи.РезультатПроверкиКраткий = НСтр("ru = 'Требуется собственноручная подпись'");
			ИначеЕсли СтрокаПодписи.РезультатСогласования = Перечисления.РезультатыСогласованияБЗК.Согласовано Тогда
				СтрокаПодписи.РезультатПроверкиКраткий = НСтр("ru = 'Простая электронная подпись'");
				ЕстьПодписиДокумента = Истина;
			КонецЕсли;
		КонецЕсли;
		СтрокаПодписи.ПодписьСотрудника = Истина;
	...
	СостояниеДокумента = РегистрыСведений.СостоянияДокументовЭДОБЗК.СостояниеДокумента(Объект.Ссылка);
	Если ЕстьПодписиДокумента Тогда
		ИдентификаторыКоманд["БЗК_ДокументСВерсиейДляПечати"] = СостояниеДокумента.ЕстьВерсияДляПечати;
	Иначе
		ИдентификаторыКоманд["БЗК_ДокументСВерсиейДляПечати"] = Ложь;
	КонецЕсли;
	
	РезультатыПубликации = РегистрыСведений.РезультатыПубликацииДокументовЭДОБЗК.РезультатыПубликацииДокумента(Объект.Ссылка);
	Опубликован = ЗначениеЗаполнено(РезультатыПубликации.ДатаПубликации) И Не ЗначениеЗаполнено(РезультатыПубликации.ДатаУдаления);
	...
	Если ОбновитьДанныеОбъекта Тогда
...
Процедура ПрочитатьСтатусыПРР()
	...
	УстановитьПривилегированныйРежим(Истина);
	ИдентификаторДокумента = РегистрыСведений.ИдентификаторыДокументовНаРаботаРоссии.ИдентификаторДокумента(Объект.ЭлектронныйДокумент);
...
Процедура УстановитьОтображениеЭлементовФормы(УправляемаяФорма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"ФормаПередатьВ1СКабинетСотрудника",
		"Доступность",
		Не УправляемаяФорма.ОчередностьПодписанияПользователя
			И (ДанныеФайлаЗаполнены(УправляемаяФорма)
...
Процедура ОбновитьЗначениеТребуетсяПодписьОрганизации()
	...
	ЕстьНаправленияНаПодпись = Ложь;
	ОчередностьПодписанияПользователя = Ложь;
	...
	Если ИспользоватьЭлектронныеПодписи Тогда
		Если Объект.Внешний Тогда
			ТребуетсяПодписьОрганизацииВидаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				Объект.ВидДокумента, "ТребуетсяПодписьОрганизации");
			Если ТребуетсяПодписьОрганизацииВидаДокумента Тогда
				...
				Если ЗначениеЗаполнено(НаправленияНаПодпись) Тогда
					ОчередностьПодписанияПользователя = НаправленияНаПодпись[0].Исполнитель = Пользователи.ТекущийПользователь();
					ЕстьНаправленияНаПодпись = Истина;
					ТребуетсяПодписьОрганизации = Ложь;
					Исполнители = Новый Массив;
					Для Каждого ДанныеНаправленияНаПодпись Из НаправленияНаПодпись Цикл
						Исполнители.Добавить(ДанныеНаправленияНаПодпись.Исполнитель);
					КонецЦикла;
					ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
						Элементы,
						"ОжидаетПодписанияДекорация",
						"Заголовок",
						СтрШаблон(НСтр("ru = 'Ожидает подписания (%1)'"), СтрСоединить(Исполнители, ", ")));
				КонецЕсли;
			Иначе
				ТребуетсяПодписьОрганизации = Ложь;
			КонецЕсли;
	...
	УстановитьОтображениеПолейТребованияПодписи(ИспользоватьЭлектронныеПодписи, ЕстьНаправленияНаПодпись);
...
&НаСервере
Процедура ОбновитьДанныеФормыПоВидуДокумента(ОбновитьДанныеОбъекта)
	...
	РеквизитыВидаДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ВидДокумента,
		"ТребуетсяПодписьОрганизации,СодержимоеДокумента,ВариантПодписания,ТипЭлектроннойПодписи");
	Если Не ИзменитьВариантПодписания Тогда
		ВариантПодписания = РеквизитыВидаДокумента.ВариантПодписания;
		ТипЭлектроннойПодписи = РеквизитыВидаДокумента.ТипЭлектроннойПодписи;
	КонецЕсли;
	Если ОбновитьДанныеОбъекта И ЗначениеЗаполнено(Объект.ВидДокумента) Тогда
		Если ЗначениеЗаполнено(РеквизитыВидаДокумента.СодержимоеДокумента)Тогда
			Если ЭДОБЗК.ДоступныеСодержанияДокументовПользователя(
				Пользователи.ТекущийПользователь()).Найти(РеквизитыВидаДокумента.СодержимоеДокумента) <> Неопределено Тогда
				
				Объект.СодержимоеДокумента = РеквизитыВидаДокумента.СодержимоеДокумента;
			КонецЕсли;
		КонецЕсли;
		ТребуетсяПодписьОрганизации = РеквизитыВидаДокумента.ТребуетсяПодписьОрганизации;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтображениеЭлементовВариантаПодписания(УправляемаяФорма)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		УправляемаяФорма.Элементы,
		"Шапка",
		"Доступность",
		ЗначениеЗаполнено(УправляемаяФорма.Объект.ВидДокумента));
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		УправляемаяФорма.Элементы,
		"ИзменитьВариантПодписания",
		"Видимость",
		УправляемаяФорма.Объект.Внешний);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		УправляемаяФорма.Элементы,
		"ВариантПодписанияГруппа",
		"ТолькоПросмотр",
		Не УправляемаяФорма.ИзменитьВариантПодписания);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		УправляемаяФорма.ИспользуетсяВерсияDTO_3_3
			И УправляемаяФорма.ВариантПодписания = ПредопределенноеЗначение("Перечисление.ВариантыПодписанияДокументовЭДОБЗК.Требуется"));
	...
	Если УправляемаяФорма.Опубликован
		И (УправляемаяФорма.ВидДокументаПриОткрытии <> УправляемаяФорма.Объект.ВидДокумента
			Или УправляемаяФорма.ВариантПодписания <> УправляемаяФорма.Объект.ВариантПодписания
			Или УправляемаяФорма.ТипЭлектроннойПодписи <> УправляемаяФорма.Объект.ТипЭлектроннойПодписи) Тогда
		
		ТекстПодсказки = НСтр("ru = 'Для обновления вида и варианта подписания документа сотрудниками в своих кабинетах, необходимо повторить передачу документа в 1С:Кабинет сотрудника.'");
	...
	ЗарплатаКадрыКлиентСервер.УстановитьРасширеннуюПодсказкуЭлементуФормы(
		УправляемаяФорма, "ВидДокументаГруппа", ТекстПодсказки);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьОтображениеПолейТребованияПодписи(ИспользоватьЭлектронныеПодписи, ЕстьНаправленияНаПодпись = Ложь)
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ТребуетсяПодписьОрганизации",
		"Видимость",
		ИспользоватьЭлектронныеПодписи И Не ЕстьНаправленияНаПодпись);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ОжидаетПодписанияДекорация",
		"Видимость",
		ИспользоватьЭлектронныеПодписи И ЕстьНаправленияНаПодпись);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ФормаПодписать",
		"Видимость",
		ИспользоватьЭлектронныеПодписи И ОчередностьПодписанияПользователя);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ФормаПередатьВ1СКабинетСотрудника",
		"Видимость",
		ИспользоватьЭлектронныеПодписи И Не ОчередностьПодписанияПользователя);
	
	Если ИспользоватьЭлектронныеПодписи И ОчередностьПодписанияПользователя Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ФормаПодписать",
			"КнопкаПоУмолчанию",
...
Функция ДействияСДокументом()
	...
	КонецЕсли;
	Если ПолучитьФункциональнуюОпциюФормы("ИспользоватьИнтеграциюСРаботаРоссии") Тогда
		Действия.Добавить(ПредопределенноеЗначение("Перечисление.ДействияСФайламиДокументовЭДОБЗК.ПередатьНаРаботаРоссии"));
		
		Если Действия.Количество() = 1 И Не ОрганизацияИспользуетПРР(Объект.Организация) Тогда
			ПоказатьПредупреждение(, ИнтеграцияСРаботаРоссииКлиент.ТекстПредупрежденияОрганизацияНеИспользуетПРР());
