...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеУчастника")
		И ТипЗнч(Параметры.ДанныеУчастника) = Тип("Структура") Тогда 
		
		ДанныеУчастника =  Параметры.ДанныеУчастника;
		
		Если ЗначениеЗаполнено(ДанныеУчастника.Контакт)
			И Объект.Контрагент <> ДанныеУчастника.Контакт Тогда
			ЗаполнитьНаОснованииУчастника(ДанныеУчастника);
		Иначе
			УстановитьКонтактнуюИнформацию(ДанныеУчастника);
		КонецЕсли;
	...
	Если Параметры.Свойство("ЗначенияЗаполнения") И ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения) Тогда
		...
		Если ТипЗнч(ЗначенияЗаполнения) = Тип("Структура") Тогда
			...
			Если ЗначенияЗаполнения.Свойство("ПредметВзаимодействия")
				...
				СтрокаПредмет = Объект.ДокументыОснования.Добавить();
				СтрокаПредмет.ДокументОснование = ЗначенияЗаполнения.ПредметВзаимодействия;
	...
	Если ПустаяСтрока(АдресПолучателяСкрытойКопииИзНастроек) Тогда
		ПараметрыПисьмаПоУмолчанию = ОтправкаПочтовыхСообщенийВызовСервера.ПараметрыПисьмаПоУмолчанию();
		АдресПолучателяСкрытойКопииИзНастроек = ПараметрыПисьмаПоУмолчанию.АдресПолучателяСкрытойКопии;
	КонецЕсли;
	
	// Письма отправляем сразу
...
Процедура ПодписьИПечатьПриИзменении(Элемент)
	...
	ЗаполнитьВложенияФаксимиле();
	
КонецПроцедуры

&НаКлиенте
Процедура КонтактноеЛицоПриИзменении(Элемент)
	КонтактноеЛицоЗаполнено = ЗначениеЗаполнено(Объект.КонтактноеЛицо);
	Если Не ПисьмоОтправлено(Объект.СтатусПисьма) Тогда
		ОчиститьКонтактнуюИнформацию(ЭтотОбъект, Не КонтактноеЛицоЗаполнено);
	КонецЕсли;
	Если КонтактноеЛицоЗаполнено Тогда
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если Не ПисьмоОтправлено(Объект.СтатусПисьма) Тогда
		ОчиститьКонтактнуюИнформацию(ЭтотОбъект, Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура ТемаОчистка(Элемент, СтандартнаяОбработка)
	
	Если ПисьмоОтправлено(Объект.СтатусПисьма) Тогда
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ОткрытьДокументыОснования(Команда)
	...
	Если Объект.ДокументыОснования.Количество() <> 0 Тогда
		...
		ПараметрыФормы.Вставить("АдресВХранилищеДокументовОснований", АдресВХранилищеДокументовОснований);
		ПараметрыФормы.Вставить("ТолькоПросмотр", ТолькоПросмотр);
		
		ОповещениеОЗакрытииФормыВыбора =  Новый ОписаниеОповещения("ОткрытьДокументыОснованияЗавершение", ЭтотОбъект, ПараметрыФормы);
		ОткрытьФорму("Документ.ЭлектронноеПисьмоИсходящее.Форма.ДокументыОснованияПисьма", ПараметрыФормы
	...
	Иначе
		...
		СписокВыбора.ПоказатьВыборЭлемента(ОповещениеПослеВыбора, НСтр("ru = 'Выберите тип данных'"));
	КонецЕсли;
...
Процедура ОпределитьВидимостьДоступностьЭлементовВЗависимостиОтСтатусаПисьма()
	...
	Иначе
		...
		Элементы.ДобавитьВложениеКоманда.Видимость = Ложь;
...
Процедура ОпределитьСпособРедактированияПисьма()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыРаботыПользователяПользователя = ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
		...
		// Всегда включаем тело исходного письма при ответе
...
Процедура ОбработатьПереданныеПараметры(ПереданныеПараметры)
	...
	ИначеЕсли ПереданныеПараметры.Свойство("Получатели") И ПереданныеПараметры.Получатели <> Неопределено 
		И ПереданныеПараметры.Получатели.Количество() > 0 Тогда
		Получатель = ПереданныеПараметры.Получатели[0].ЭлектроннаяПочта
	КонецЕсли;
	
	Если Получатель <> Неопределено Тогда
		
		// Если передан параметр "Получатели", то эта табличная часть очищается перед заполнением согласно данным параметра.
		
		Объект.ПолучателиПисьма.Очистить();
		СписокПолучателей.Очистить();
		
		Если ТипЗнч(Получатель) = Тип("Строка") И Не ПустаяСтрока(Получатель) Тогда
			Объект.СписокПолучателейПисьма = Получатель;
			НоваяСтрока = Объект.ПолучателиПисьма.Добавить();
			НоваяСтрока.Адрес = Получатель;
			
		ИначеЕсли ТипЗнч(Получатель) = Тип("СписокЗначений") Тогда
			
			Для Каждого ЭлементСписка Из Получатель Цикл
				НоваяСтрока = Объект.ПолучателиПисьма.Добавить();
		...
		ИначеЕсли ТипЗнч(Получатель) = Тип("Массив") Тогда
			...
			Для Каждого ЭлементМассива Из Получатель Цикл
				...
				Для Каждого Адрес Из МассивАдресов Цикл
					...
					Если Контрагент = Неопределено
						И ТипЗнч(ЭлементМассива.ИсточникКонтактнойИнформации) = Тип("СправочникСсылка.Контрагенты") Тогда
						Контрагент = ЭлементМассива.ИсточникКонтактнойИнформации;
					ИначеЕсли КонтактноеЛицо = Неопределено
						И ТипЗнч(ЭлементМассива.ИсточникКонтактнойИнформации) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
						КонтактноеЛицо = ЭлементМассива.ИсточникКонтактнойИнформации;
					ИначеЕсли Контрагент <> Неопределено
						И ТипЗнч(ЭлементМассива.ИсточникКонтактнойИнформации) = Тип("СправочникСсылка.Контрагенты")
						И Контрагент <> ЭлементМассива.ИсточникКонтактнойИнформации Тогда
			...
			Если ЗначениеЗаполнено(КонтактноеЛицо) Тогда
				...
				Если Не ЗначениеЗаполнено(Контрагент) Тогда
					Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КонтактноеЛицо, "ОбъектВладелец");
				КонецЕсли;
				Объект.Контрагент = Контрагент;
			ИначеЕсли ЗначениеЗаполнено(Контрагент) Тогда
				Объект.Контрагент = Контрагент;
				Объект.КонтактноеЛицо = Справочники.КонтактныеЛица.КонтактноеЛицоПоУмолчанию(Контрагент);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Объект.КонтактноеЛицо) Или ЗначениеЗаполнено(Объект.Контрагент) Тогда
				КонтактноеЛицоЗаполнено = ЗначениеЗаполнено(Объект.КонтактноеЛицо);
				ОчиститьКонтактнуюИнформацию(ЭтотОбъект, Не КонтактноеЛицоЗаполнено);
				УстановитьКонтактнуюИнформацию();
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(АдресПолучателяСкрытойКопииИзНастроек) Тогда
		...
		Для Каждого Получатель Из СписокПолучателейСкрытойКопии Цикл
			Если Не ПустаяСтрока(Получатель.ОписаниеОшибки) Тогда
				...
				Возврат Результат;
			КонецЕсли;
			Если Не ПараметрыПисьма.Свойство("СкрытыеКопии") Тогда
				ПараметрыПисьма.Вставить("СкрытыеКопии", Новый Массив);
			КонецЕсли;
			ПараметрыПисьма.СкрытыеКопии.Добавить(
				Новый Структура("Адрес, Представление", Получатель.Адрес, Получатель.Псевдоним));
		КонецЦикла;
	КонецЕсли;
	
	ДополнитьПараметрыПисьмаПолучателямиСообщения(ПараметрыПисьма);
...
Процедура ОтправитьПисьмоКлиент()
	...
	Если Результат.ПисьмоОтправлено И Результат.ТекстСообщения = "" Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Письмо отправлено:'"), ПолучитьНавигационнуюСсылку(Объект.Ссылка),
			?(ПустаяСтрока(Объект.Тема), НСтр("ru = '<Без темы>'"), Объект.Тема), БиблиотекаКартинок.Информация32);
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьПодпись(ПредыдущаяУчетнаяЗапись, НоваяУчетнаяЗапись)
	...
	ПараметрыПредыдущаяУЗ = ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
		ПредыдущаяУчетнаяЗапись,
		ФорматСообщения);
	
	ПараметрыНоваяУЗ = ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
		НоваяУчетнаяЗапись,
		ФорматСообщения);
...
Процедура ПриИзмененииФорматаПодписиНаHTMLНаСервере(УчетнаяЗапись)
	...
	ПараметрыУЗ = ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
		УчетнаяЗапись,
		ФорматСообщения);
	...
	Если ПараметрыУЗ.Подпись <> Неопределено Тогда
		...
		ДобавитьФорматированныйДокументКФорматированномуДокументу(ТекстПисьмаФорматированныйДокумент,
			ПараметрыУЗ.Подпись);
...
&НаСервере
Процедура ДополнитьПараметрыПисьмаПолучателямиСообщения(ПараметрыПисьма)
	
	Если Объект.ПолучателиПисьма.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаПолучателя = Объект.ПолучателиПисьма[0];
	...
	Для Каждого Адрес Из СписокАдресовЭП Цикл
		ДанныеКонтакта = Адрес.Значение;
		Если ДанныеКонтакта.Адрес = СтрокаПолучателя.Адрес Тогда
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(Объект.КонтактноеЛицо) Тогда
		ПараметрыПолучателя.ИсточникКонтактнойИнформации = Объект.КонтактноеЛицо;
	Иначе
		ПараметрыПолучателя.ИсточникКонтактнойИнформации = Объект.Контрагент;
	КонецЕсли;
	ПараметрыПисьма.Вставить("ПолучателиСообщения", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыПолучателя));
	...
	Если Параметры.Свойство("АдресИсходногоВложения") Тогда
		...
		Если ЗначениеЗаполнено(АдресИсходногоВложения) Тогда
			...
			Если ПараметрыФаксимиле.Свойство("Организация") Тогда
				УправлениеПечатьюБП.ОформитьВыводРеквизитаПодписьИПечать(Элементы.ПодписьИПечатьБП,
					Элементы.ГруппаПодписьИПечатьБП, ПараметрыФаксимиле);
				Если ПараметрыФаксимиле.ВыводитьПодписьИПечать И ЗначениеЗаполнено(ПараметрыФаксимиле.Организация) Тогда
					Организация = ПараметрыФаксимиле.Организация;
				КонецЕсли;
				НастройкиПодписиИПечати = УправлениеПечатьюБП.НастройкиПодписиИПечати();
				Если НастройкиПодписиИПечати.ВыводитьФаксимиле = Истина И ПараметрыФаксимиле.ВыводитьПодписьИПечать
...
Функция ПолучитьСписокВыбораОснований()
	...
	СписокВыбора.Добавить("Документ.ПрочееВзаимодействие.ФормаВыбора", НСтр("ru = 'Прочее взаимодействие'"));
	СписокВыбора.Добавить("Документ.ТелефонныйЗвонок.ФормаВыбора", НСтр("ru = 'Звонок'"));
	СписокВыбора.Добавить("Документ.Встреча.ФормаВыбора", НСтр("ru = 'Встреча'"));
	СписокВыбора.Добавить("Документ.ЭлектронноеПисьмоВходящее.ФормаВыбора", НСтр("ru = 'Электронное письмо входящее'"));
	СписокВыбора.Добавить("Документ.ЭлектронноеПисьмоИсходящее.ФормаВыбора", НСтр("ru = 'Электронное письмо исходящее'"));
	СписокВыбора.Добавить("Документ.СчетНаОплатуПокупателю.ФормаВыбора", НСтр("ru = 'Счет покупателю'"));
	СписокВыбора.Добавить("Документ.КоммерческоеПредложение.ФормаВыбора", НСтр("ru = 'Коммерческое предложение'"));
	СписокВыбора.Добавить("Документ.РеализацияТоваровУслуг.ФормаВыбора", НСтр(
		"ru = 'Реализация (акты, накладные, УПД)'"));
...
Процедура ВыборТипаОснованияЗавершение(ВыбранноеИмяФормы, Параметры) Экспорт
	...
	Если ВыбранноеИмяФормы <> Неопределено Тогда
		...
		Если ВыбранноеИмяФормы.Значение = "Документ.СчетНаОплатуПокупателю.ФормаВыбора" Тогда
...
Процедура УстановитьЗаголовокГиперссылкиДокументовОснований()
	...
	ОснованияПрописью = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		";%1 основание;;%1 основания;%1 оснований;%1 основания", Объект.ДокументыОснования.Количество() - 1);
	Элементы.ОткрытьДокументыОснования.Заголовок = СтрШаблон(НСтр("ru = '%1 и еще %2.'"), ПервыйДокумент,
		ОснованияПрописью);
...
Функция ПоместитьДокументыОснованияВХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(
		Объект.ДокументыОснования.Выгрузить( , "ДокументОснование"), УникальныйИдентификатор);
...
Процедура ЗаполнитьНаОснованииУчастника(ДанныеУчастника)
	...
	Получатель.Контакт = ДанныеУчастника.Контакт;
	Получатель.Адрес = ДанныеУчастника.КакСвязаться;
	УстановитьКонтактнуюИнформацию(ДанныеУчастника);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьКонтактнуюИнформацию(ДанныеУчастника = Неопределено)
	
	Если Не ЗначениеЗаполнено(ДанныеУчастника) Или ПустаяСтрока(ДанныеУчастника.КакСвязаться) Тогда
	
		ВзаимодействияФормыБП.УстановитьКонтактнуюИнформацию(ЭтотОбъект);

	Иначе
		ЭлектроннаяПочта = ДанныеУчастника.КакСвязаться;
		Если Объект.ПолучателиПисьма.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(Объект.КонтактноеЛицо) Тогда
			СтрокаПолучатели.Представление = ОбработанноеПредставлениеАдресата(Строка(Объект.КонтактноеЛицо));
			СтрокаПолучатели.Контакт = Объект.КонтактноеЛицо;
		Иначе
			СтрокаПолучатели.Представление = ОбработанноеПредставлениеАдресата(Строка(Объект.Контрагент));
			СтрокаПолучатели.Контакт = Объект.Контрагент;
		КонецЕсли;
		СтрокаПолучатели.Адрес = ЭлектроннаяПочта;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОчиститьКонтактнуюИнформацию(Форма, ДобавитьНовуюСтроку = Ложь)
	
	Объект = Форма.Объект;
	
	Объект.ПолучателиПисьма.Очистить();
	Форма.СписокПолучателей.Очистить();
	
	Если ДобавитьНовуюСтроку = Истина Тогда
		
		ОбъектПолучатели = Объект.ПолучателиПисьма.Добавить();
		
		СтрокаСпискаПолучателей = Форма.СписокПолучателей.Добавить();
		СтрокаСпискаПолучателей.ВариантОтправки = ВзаимодействияКлиентСерверБП.Кому();
...
&НаСервере
Процедура ЗаполнитьВложенияФаксимиле()
	
	ВложенияОбъект = РеквизитФормыВЗначение("Вложения", Тип("ТаблицаЗначений"));
	ВложенияБезПодписиИПечатиОбъект = РеквизитФормыВЗначение("ВложенияБезПодписиИПечати", Тип("ТаблицаЗначений"));
	Если ПодписьИПечать Тогда

		Для Каждого Вложение Из ВложенияПодписьИПечать Цикл
			НайденнаяСтрока = ВложенияОбъект.Найти(Вложение.Представление);
			// Не нашли исходный файл - пропускаем
			Если НайденнаяСтрока = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			СтрокаСохранения = ВложенияБезПодписиИПечатиОбъект.Найти(Вложение.Представление);
			Если СтрокаСохранения = Неопределено Тогда
				СтрокаСохранения = ВложенияБезПодписиИПечатиОбъект.Добавить();
				СтрокаСохранения.Представление = Вложение.Представление;
			КонецЕсли;
			СтрокаСохранения.АдресВоВременномХранилище = НайденнаяСтрока.ИмяФайлаНаКомпьютере;
			НайденнаяСтрока.ИмяФайлаНаКомпьютере = Вложение.АдресВоВременномХранилище;
		КонецЦикла;

	Иначе
		
		Для Каждого Вложение Из ВложенияБезПодписиИПечати Цикл
			НайденнаяСтрока = ВложенияОбъект.Найти(Вложение.Представление);
			// Не нашли исходный файл - пропускаем
			Если НайденнаяСтрока = Неопределено Тогда
				Продолжить;
	...
	ЗначениеВРеквизитФормы(ВложенияОбъект, "Вложения");
	ЗначениеВРеквизитФормы(ВложенияБезПодписиИПечатиОбъект, "ВложенияБезПодписиИПечати");
...
&НаКлиентеНаСервереБезКонтекста
Функция ПисьмоОтправлено(СтатусПисьма)
	
	Возврат СтатусПисьма = ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Отправлено");
...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.СписокПолучателейВариантОтправки.Заголовок = НСтр("ru = 'Вид получателя'");
	МобильныйКлиентБПФормаОбъекта.АдаптироватьМККолонкуТабличногоПоля(
		Элементы.СписокПолучателейВариантОтправки,
		Истина,
		ВажностьПриОтображении.ОченьВысокая);
	Элементы.СписокПолучателейПредставление.Заголовок = НСтр("ru = 'Получатель'");
	МобильныйКлиентБПФормаОбъекта.АдаптироватьМККолонкуТабличногоПоля(
		Элементы.СписокПолучателейПредставление,
		Истина,
		ВажностьПриОтображении.ОченьВысокая);
		
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ПредупреждениеОНеотправленныхПисьмах);
	...
	Элементы.СтраницаОбычныйТекст.ЦветФона = ЦветаСтиля.ЦветФонаФормы;
	Элементы.СтраницаФорматированныйДокумент.ЦветФона = ЦветаСтиля.ЦветФонаФормы;
...
&НаСервере
Функция ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(УчетнаяЗапись, ФорматСообщения)
	
	ДляНового = ?(Объект.ВзаимодействиеОснование = Неопределено, Истина, Ложь);
	
	ПараметрыРаботыПользователя =
			ВзаимодействияБП.ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
			УчетнаяЗапись,
			ФорматСообщения,
			ДляНового);
	
	// Заполняем подпись из настроек взаимодействия, если не задана в персональных настройках
	Если ПараметрыРаботыПользователя.Подпись = Неопределено Тогда
		ПараметрыРаботыПользователяВзаимодействия =
			Взаимодействия.ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
			УчетнаяЗапись,
			ФорматСообщения,
			ДляНового);
		ПараметрыРаботыПользователя.Подпись = ПараметрыРаботыПользователяВзаимодействия.Подпись;
	КонецЕсли;
	
	Возврат ПараметрыРаботыПользователя;
