...
Функция ДальнейшееДействиеПоУмолчанию(СтруктураПараметров = Неопределено) Экспорт
	...
	ИначеЕсли СтруктураПараметров.Свойство("ОбъектРасчета") И ЗначениеЗаполнено(СтруктураПараметров.ОбъектРасчета) Тогда
		ВидПродукции = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтруктураПараметров.ОбъектРасчета, "ВидПродукции");
	ИначеЕсли СтруктураПараметров.Свойство("ВидПродукции") И ЗначениеЗаполнено(СтруктураПараметров.ВидПродукции) Тогда
		ВидПродукции = СтруктураПараметров.ВидПродукции;
	Иначе
		ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво;
	КонецЕсли;
	
	Если Документы.РазрешениеНаОтгрузкуИСМП.ДоступныеВидыПродукцииИСМП().Найти(ВидПродукции) <> Неопределено Тогда
...
Функция ДоступныеВидыПродукцииИСМП() Экспорт
	...
	Для Каждого ДоступныйВидПродукции Из ВидыПродукцииВнутреннегоПеремещенияМеждуМОД Цикл
		Если УчитываемыеВидыПродукции.Найти(ДоступныйВидПродукции) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		МассивПродукции.Добавить(ДоступныйВидПродукции);
	КонецЦикла;
...
Функция ПолучитьДанныеДляЭД(ДокументСсылка) Экспорт
	...
	Запрос.Текст =
		...
		|	ПеремещениеМеждуМОДИСМП.ВидПродукции                                    КАК ВидПродукции,
...
Функция ОформлениеПеремещения(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	|	Шапка.Дата                               КАК Дата,
	|	ВЫБОР
	|		КОГДА Шапка.ВидПродукции = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.Пиво)
	|		ИНАЧЕ Шапка.ВидПродукции
