...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСМП.НастроитьВидПродукцииПриСозданииНаСервере(ЭтотОбъект, ДоступныеВидыПродукцииИС.ВыгрузитьЗначения());
...
&НаКлиенте
Процедура ВидПродукцииОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДекорацияОтсканироватьПроверитьМаркируемуюПродукциюОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ВидыПродукцииПоДокументу.Количество() = 1 Тогда
			ПроверкаИПодборПродукцииИСМПКлиент.ОткрытьФормуПроверкиИПодбора(ЭтотОбъект, ВидыПродукцииПоДокументу[0].Значение, ПараметрыУказанияСерий);
		Иначе
			
			ПослеВыбораИзСписка = Новый ОписаниеОповещения("ПослеВыбораВидаПродукцииДляОткрытияФормыПроверкиИПодбора", ЭтотОбъект);
			ВидыПродукцииПоДокументу.ПоказатьВыборЭлемента(ПослеВыбораИзСписка);
...
Процедура ОбновитьСтатусИСМП()
	...
	СтруктураПараметров.Вставить("ВидПродукции",  Объект.ВидПродукции);
...
Процедура НастроитьЭлементыФормы(Форма)
	...
	Если ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе <> Неопределено
		И ЗначениеЗаполнено(Объект.ВидПродукции) Тогда
		...
		ПараметрыУказанияМОД = ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции.Получить(Объект.ВидПродукции);
...
Процедура ПолучитьКодФИАС(Префикс)
	...
	ПараметрыЗапросаДанных.Параметры = Новый Структура("ВидПродукции", Объект.ВидПродукции);
...
Процедура ОбновитьИнформациюОткрытияФормыПроверкиПодбора()
	...
	ИначеЕсли СтатусИСМП = Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.Черновик
		Или СтатусИСМП = Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.РазрешениеОшибка
		Или СтатусИСМП = Перечисления.СтатусыОбработкиПеремещенийМеждуМОДИСМП.ОформлениеПеремещенияОшибка Тогда
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ДоступныеВидыПродукцииИС.Очистить();
	...
	ЗаполнитьВидПродукцииПоУмолчанию();
...
Процедура ИнициализироватьПараметрыУказанияДанныхПоМОД()
	...
	ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе = ЭлектронноеВзаимодействиеИСМП.ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(Объект.ВидПродукции,, Истина);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьВидПродукцииПоУмолчанию()
	...
	ИначеЕсли Не РедактированиеФормыНедоступно Тогда
		...
		Если ВидыПродукцииПоДокументу.Количество() = 1 Тогда
			Объект.ВидПродукции = ВидыПродукцииПоДокументу[0].Значение;
		ИначеЕсли ДоступныеВидыПродукцииИС.Количество() Тогда
			Объект.ВидПродукции = ДоступныеВидыПродукцииИС[0].Значение;
		КонецЕсли;
	...
	Элементы.ВидПродукции.АвтоОтметкаНезаполненного = ПроверятьЗаполнениеВидаПродукции;
	Элементы.ВидПродукции.Видимость                 = Элементы.ВидПродукции.Видимость И ПоказыватьВыборВидаПродукции;
...
Процедура ЗаполнитьДанныеВидаПродукцииДействияОСУПоТаблице()
	...
	ВидыПродукцииПоДокументу.Очистить();
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТабличнойЧасти Из СтрокиТабличнойЧасти Цикл
			...
			Если ВидыПродукцииПоДокументу.НайтиПоЗначению(Выборка.ВидПродукции) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ВидыПродукцииПоДокументу.Добавить(Выборка.ВидПродукции);
...
&НаСервере
Процедура ПриИзмененииВидаПродукцииНаСервере()
...
Процедура ПриИзмененииВидаПродукции()
	...
	ПриИзмененииВидаПродукцииНаСервере();
...
&НаКлиенте
Процедура ПослеВыбораВидаПродукцииДляОткрытияФормыПроверкиИПодбора(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПроверкаИПодборПродукцииИСМПКлиент.ОткрытьФормуПроверкиИПодбора(ЭтотОбъект, Результат.Значение, ПараметрыУказанияСерий);
