...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЭтоМаркировкаОстатков Тогда
		...
		Если ЭтоБылаРасширеннаяМаркировкаОстатков(ЭтотОбъект) Тогда
...
Процедура НастроитьЭлементыФормыНаСервере()
	...
	ДоступностьСпособаВвода.Вставить(
		...
		ИнтеграцияИСКлиентСервер.ДоступенСпособВводаВОборотМаркировкаОстатков(Объект.ВидПродукции));
	ДоступностьСпособаВвода.Вставить(
		Перечисления.СпособыВводаВОборотСУЗ.Перемаркировка,
		ИнтеграцияИСКлиентСервер.ДоступенСпособВводаВОборотПеремаркировка(Объект.ВидПродукции));
	ДоступностьСпособаВвода.Вставить(
	...
	ДоступностьСпособаВвода.Вставить(
		Перечисления.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта,
		ИнтеграцияИСКлиентСервер.ВидПродукцииПодлежитМаркировкеВнеПроизводстваИлиИмпорта(Объект.ВидПродукции));
	...
	Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(Объект.ВидПродукции)
		И Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ДокументОбрабатываетсяИлиОбработанВСУЗ(Объект.Ссылка) Тогда
...
Процедура РасчетПризнакаПолногоОписанияОстатков(СтрокиДляОбработки, СброситьПолноеОписание = Истина, НовыеСтроки = Ложь)
	...
	Если Не НовыеСтроки
		И ЭтоБылаРасширеннаяМаркировкаОстатков(ЭтотОбъект) Тогда
		ПолноеОписание = Неопределено;
	Иначе
		ПолноеОписание = ИнтеграцияИСКлиентСервер.ИспользованиеСпособаОписанияОстатков(Объект.ВидПродукции);
	КонецЕсли;
...
Процедура ОбновитьВидимостьДоступностьЭлементовФормы(Форма)
	...
	ЭтоЛегкаяПромышленность = Объект.ВидПродукции = ПредопределенноеЗначение(
		...
		Или Объект.ВидПродукции = ПредопределенноеЗначение(
		"Перечисление.ВидыПродукцииИС.ЛегкаяПромышленность2026");
	...
	ЭтоБАД                  = Объект.ВидПродукции = ПредопределенноеЗначение(
		...
		Или Объект.ВидПродукции = ПредопределенноеЗначение(
		"Перечисление.ВидыПродукцииИС.СпортивноеПитание");
	...
	ЭтоМоторныеМасла        = Объект.ВидПродукции = ПредопределенноеЗначение(
		"Перечисление.ВидыПродукцииИС.МоторныеМасла");
	...
	ЭтоБылаРасширеннаяМаркировкаОстатков = ЭтоБылаРасширеннаяМаркировкаОстатков(Форма);
	
	Если Форма.ЭтоМаркировкаОстатков
		И ЭтоБылаРасширеннаяМаркировкаОстатков Тогда
		...
		Элементы.БезУчетаНоменклатуры.Видимость = Объект.БезУчетаНоменклатуры
			И Не ЭтоАльтернативныйТабак И Не ЭтоДухи
			И Не ЭтоАнтисептики И Не ЭтоБАД И Не ЭтоКреслаКоляски И Не ЭтоВелосипеды И Не ЭтоМоторныеМасла;
		
		БылДоступен = Объект.ВвезенПослеДатыОбязательнойМаркировки
			И (ЭтоДухи Или ЭтоШины Или ЭтоЛегкаяПромышленность Или ЭтоОбувь);
		Элементы.ВвезенПослеДатыОбязательнойМаркировки.Видимость = БылДоступен;
		
		БылДоступен = Объект.ПриобретенПроизведенПослеДатыОбязательнойМаркировки
			И (ЭтоДухи Или ЭтоШины Или ЭтоЛегкаяПромышленность Или ЭтоОбувь);
		Элементы.ПриобретенПроизведенПослеДатыОбязательнойМаркировки.Видимость = БылДоступен;
	...
	Если ИспользуетсяТипОплатыКодовМаркировки Тогда
		Если Объект.СпособВводаВОборот = ПредопределенноеЗначение("Перечисление.СпособыВводаВОборотСУЗ.МаркировкаВнеПроизводстваИмпорта")
			И Объект.ВидПродукции <> ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.РадиоэлектроннаяПродукция") Тогда
	...
	Элементы.ТоварыВладелецGTINСтрокой.Видимость =
		ИнтеграцияИСМПКлиентСервер.ИспользуетсяВладелецGTINВЗаказеНаЭмиссию(Форма.Объект.СпособВводаВОборот)
		И Не ЭтоБылаРасширеннаяМаркировкаОстатков;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоБылаРасширеннаяМаркировкаОстатков(Форма)
...
Процедура ПроверитьОчиститьВладельцаGTIN(Форма, Данные = Неопределено)
	...
	ИспользуетсяВладелецGTIN = ИнтеграцияИСМПКлиентСервер.ИспользуетсяВладелецGTINВЗаказеНаЭмиссию(Форма.Объект.СпособВводаВОборот);
