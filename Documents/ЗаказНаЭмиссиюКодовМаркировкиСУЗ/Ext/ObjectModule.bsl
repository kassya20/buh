...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоМаркировкаВнеПроизводстваИмпорта Тогда
		ДоступныеПотребительскиеШаблоны = ИнтеграцияИСМПКлиентСервер.ШаблоныКодовПоВидуПродукции(ВидПродукции, 0);
	КонецЕсли;
	...
	Если ЭтоТабачнаяПродукция Тогда
		...
		Если Документы.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.ДокументОбрабатываетсяИлиОбработанВСУЗ(Ссылка) Тогда
			МассивНепроверяемыхРеквизитов.Добавить("СпособВводаВОборот");
		КонецЕсли;
	...
	КонтролироватьЕМРЦ = (ВидПродукции = Перечисления.ВидыПродукцииИС.Табак) И Не ДокументОтправлен;
	...
	Для Каждого СтрокаТовары Из Товары Цикл
		...
		ИначеЕсли ЭтоМаркировкаВнеПроизводстваИмпорта Тогда
			
			Если ДоступныеПотребительскиеШаблоны.НайтиПоЗначению(СтрокаТовары.Шаблон) = Неопределено Тогда
				ТекстСообщения = СтрШаблон(
...
Процедура ПроверитьАктуальностьСпособаВводаВОборот()
	...
	Если СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.МаркировкаОстатков
		И Не ИнтеграцияИСКлиентСервер.ДоступенСпособВводаВОборотМаркировкаОстатков(ВидПродукции) Тогда
