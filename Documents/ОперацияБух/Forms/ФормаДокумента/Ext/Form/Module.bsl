...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ПодготовитьФормуНаСервере(Параметры.ЗначениеКопирования, Истина);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПодготовитьФормуНаСервере(ТекущийОбъект.Ссылка, Ложь);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для каждого СтрокаРегистраБухгалтерии Из ХозрасчетныйНаборЗаписей Цикл
		СтрокаРегистраБухгалтерии.УточнениеПериода = ?(ТекущийОбъект.МежотчетныйПериод,
...
Процедура ОбработатьИзменениеСчета(Форма, ДтКт)
	...
	БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(СтрокаТаблицы["Счет" + ДтКт], Форма, ПоляФормы);
...
&НаСервере
Процедура ПодготовитьФормуНаСервере(ДокументДвижений, Копирование)
	...
	ПрочитатьДвиженияДокумента(ДокументДвижений, Копирование);
...
&НаСервере
Процедура ПрочитатьДвиженияДокумента(ДокументДвижений, Копирование)
...
Процедура СформироватьДвиженияСторноНаСервере()
	...
	УстановитьОтображениеВТаблицеРегистров(РегистрыСДвижениями);
	СоздатьРеквизитыФормы();
	ПрочитатьДвиженияДокумента(Объект.СторнируемыйДокумент, Ложь);
...
Процедура НастроитьФормуПоОрганизацииПериоду()
	...
	БухгалтерскийУчет.НастроитьПоляУчетаРазниц(ПоляРазниц, ПоляЗаголовков, ПериодДок, Организация);
