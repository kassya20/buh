...
Функция ПечатьБухгалтерскаяСправка(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ОперацияБух.МежотчетныйПериод КАК МежотчетныйПериод,
	...
	Пока Выборка.Следующий() Цикл
		...
		// (когда это возможно и разрешено учетной политикой).
		
		ИмяОбластиЗаголовокТаблицы = "ЗаголовокТаблицы";
		ИмяОбластиСтрокаТаблицы    = "СтрокаТаблицы";
		
		Если ЕстьОбластиПБУ18
			И (Выборка.НаличиеПРДт ИЛИ Выборка.НаличиеПРКт ИЛИ Выборка.НаличиеВРДт ИЛИ Выборка.НаличиеВРКт)
			И БухгалтерскийУчетКлиентСервер.ИспользоватьСуммыРазниц(
				БухгалтерскийУчет.ИспользоватьСуммыНалогНаПрибыль(Выборка.ОрганизацияДокумента, Выборка.Дата)) Тогда
			
			ИмяОбластиЗаголовокТаблицы = "ЗаголовокТаблицыПБУ18";
			ИмяОбластиСтрокаТаблицы     = "СтрокаТаблицыПБУ18";
			
		КонецЕсли;
		
		ЗаголовокТаблицы = Макет.ПолучитьОбласть(ИмяОбластиЗаголовокТаблицы);
		СтрокаТаблицы    = Макет.ПолучитьОбласть(ИмяОбластиСтрокаТаблицы);
		...
		МежотчетныйПериод = ?(Выборка.МежотчетныйПериод, СтрШаблон(" %1", НСтр("ru = '(межотчетный период)'")), "");
		...
		СтруктураШапки.Вставить("ДатаДокумента",  Формат(Выборка.Дата, "ДЛФ=D") + МежотчетныйПериод);
