...
Функция ДанныеВедомостиНаВыплатуЗарплатыВБанк(МассивДокументов, ДатаПолученияДанных = Неопределено, ПлатежныйДокумент = Неопределено, ТолькоПроведенные = Истина) Экспорт
	...
	|	СписокФизическихЛиц.ВидМестаВыплаты КАК Вид,
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВедомостьНаВыплатуЗарплатыВБанк.ДолжностьГлавногоБухгалтера, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(ВТОтветственныеЛица.ДолжностьГлавногоБухгалтера, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка))
	|		ИНАЧЕ ВедомостьНаВыплатуЗарплатыВБанк.ДолжностьГлавногоБухгалтера
	|	КОНЕЦ КАК ГлавныйБухгалтерДолжность,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК Бухгалтер,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ВедомостьНаВыплатуЗарплатыВБанк.ДолжностьБухгалтера, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(ВТОтветственныеЛица.ДолжностьБухгалтера, ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка))
	|		ИНАЧЕ ВедомостьНаВыплатуЗарплатыВБанк.ДолжностьБухгалтера
	|	КОНЕЦ КАК БухгалтерДолжность,
	|	ВЫБОР
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Пока ВыборкаСтрокДокументов.НайтиСледующий(ВыборкаДокументов.Документ, "Документ") Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаСтрокДокументов.БанковскаяКарта) Тогда
...
Функция ВидМестаВыплаты(Ведомость = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Ведомость) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ведомость, "ВидМестаВыплаты");
	Иначе// объект или данные формы
		Возврат Ведомость.ВидМестаВыплаты;
	КонецЕсли;
КонецФункции
