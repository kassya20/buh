...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Дата < УчетУСНКлиентСервер.ДатаНачалаФормирования4РазделаКУДиР()
		Или Дата > УчетУСНКлиентСервер.ДатаОтменыФормирования4РазделаКУДиР()
		Или ДеятельностьНаПатенте
		Или Не ПрименяетсяУСНДоходы Тогда
	...
	Если Дата < УчетУСНКлиентСервер.ДатаНачалаИспользованияУпрощенногоУчетаРасходовПоБольничным()
		Или Не (ПрименяетсяУСНДоходы Или ДеятельностьНаПатенте)
		Или Не РасчетыСРаботникамиВедутсяСводно Тогда
		УменьшениеНалогаНаБольничные.Очистить();
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДеятельностьНаПатенте Тогда
		Если НЕ УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, Дата) Тогда
			...
			Если Дата < УчетПСН.ДатаНачалаДействияПатентнойСистемы() Тогда
	...
	Иначе
		...
		МассивНепроверяемыхРеквизитов.Добавить("ОС.ДатаОплаты");
		МассивНепроверяемыхРеквизитов.Добавить("ОС.Графа5_ПервоначальнаяСтоимость");
		МассивНепроверяемыхРеквизитов.Добавить("ОС.Графа13_СуммаРасходовЗаКвартал");
		
		МассивНепроверяемыхРеквизитов.Добавить("НМА.НематериальныйАктив");
		МассивНепроверяемыхРеквизитов.Добавить("НМА.ДатаОплаты");
		МассивНепроверяемыхРеквизитов.Добавить("НМА.Графа5_ПервоначальнаяСтоимость");
		МассивНепроверяемыхРеквизитов.Добавить("НМА.Графа13_СуммаРасходовЗаКвартал");
	КонецЕсли;
