...
//
// Параметры:
//   ДокументСсылка - ДокументСсылка.АктОРасхождениях - документ
//
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ2(МассивОбъектов, СоответствиеОбъектовПечати = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(АктОРасхожденияхТовары.Номенклатура.ЕдиницаИзмерения, НЕОПРЕДЕЛЕНО) КАК ЕдиницаИзмерения,
		|	ЕСТЬNULL(АктОРасхожденияхТовары.Номенклатура.ЕдиницаИзмерения.Код, """") КАК КодПоОКЕИ,
		|	ЕСТЬNULL(АктОРасхожденияхТовары.Номенклатура.ЕдиницаИзмерения.Наименование, """") КАК ЕдиницаИзмеренияНаименование,
		|	ВЫБОР
		...
		|		ПО (АктОРасхожденияхАгентскиеУслуги.Номенклатура = НоменклатураПоставщиков.Номенклатура)
		|ГДЕ
		|	АктОРасхожденияхАгентскиеУслуги.КоличествоПоДокументу > 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки
