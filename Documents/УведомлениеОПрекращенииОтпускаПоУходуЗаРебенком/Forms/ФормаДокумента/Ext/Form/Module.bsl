...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НеобходимоОформитьДругиеДокументыГруппа.Видимость = Ложь;
...
Процедура ФиксацияЗаполнитьРеквизитыОбъекта()
	...
	Если ДокументОбъект.ОбновитьВторичныеДанные() Тогда
		Неучтенные = ОбщегоНазначенияБЗККлиентСервер.ЗначениеСвойства(ДокументОбъект.ДополнительныеСвойства, "ДанныеОДетях");
		Если Неучтенные <> Неопределено И Неучтенные.Количество() > 1 Тогда
			Элементы.НеобходимоОформитьДругиеДокументыГруппа.Видимость = Истина;
			Элементы.НеобходимоОформитьДругиеДокументыНадпись.Заголовок = ЗаголовокДляПрочихНеучтенныхДокументов(Неучтенные);
		КонецЕсли;
...
Процедура Подключаемый_ЗафиксироватьИзменениеРеквизитаВФорме(Элемент, СтандартнаяОбработка = Ложь) Экспорт
	...
	ОсновныеРеквизиты = "Организация, ДокументОснование, Сотрудник, ОтветНаЗапрос, ДатаПрекращенияОплаты, Ребенок,
	|ОтправлятьЧерезСЭДО, ТипСообщения";
...
Процедура ПриОпределенииСвойствСвертываемойГруппы(Группа, Представление, Заполнена, Развернута, Видимость) Экспорт
	...
	Если Группа = Элементы.ПодтверждающийДокументГруппа Тогда
		ОбновитьЭлементыГруппыПодтверждающийДокумент(Представление, Заполнена, Развернута, Видимость);
	...
	ИначеЕсли Группа = Элементы.АдресРегистрацииГруппа Тогда
		Видимость = Объект.ТипСообщения = 104;
	...
	ИначеЕсли Группа = Элементы.РебенокГруппа Тогда
		ОбновитьЭлементыГруппыРебенок(Представление, Заполнена, Развернута, Видимость);
		
	ИначеЕсли Группа = Элементы.ПрекращаемыеОтпускаГруппа Тогда
		ОбновитьЭлементыГруппыПрекращаемыеОтпуска(Представление, Заполнена, Развернута, Видимость);
	...
	ИначеЕсли Группа = Элементы.ПредставительГруппа Тогда
		ОбновитьЭлементыГруппыПредставитель(Представление, Заполнена, Развернута, Видимость);
...
&НаСервере
Процедура ОбновитьЭлементыГруппыРебенок(Представление, Заполнена, Развернута, Видимость)
	Видимость = (Объект.ТипСообщения = 10410);
	Если Не Видимость Тогда
		Возврат;
	КонецЕсли;
	Заполнена = (ЗначениеЗаполнено(Объект.РебенокФамилия)
		И (ЗначениеЗаполнено(Объект.РебенокДатаРождения) Или ЗначениеЗаполнено(Объект.РебенокСНИЛС)));
...
&НаСервере
Процедура ОбновитьЭлементыГруппыПодтверждающийДокумент(Представление, Заполнена, Развернута, Видимость)
	...
	Если Развернута Тогда
		Элементы.ДатаПредставленияПакетаДокументов.Видимость = (Объект.ТипСообщения <> 10410);
...
&НаСервере
Процедура ОбновитьЭлементыГруппыПрекращаемыеОтпуска(Представление, Заполнена, Развернута, Видимость)
	Если Не ФормаОткрыта Тогда
		Развернута = Истина;
	КонецЕсли;
	Видимость = (Объект.ТипСообщения = 104);
...
#Область ПредставительСФР

Процедура ОбновитьЭлементыГруппыПредставитель(Представление, Заполнена, Развернута, Видимость);
	ВидимостьФИО = (Объект.ТипСообщения = 10410);
	ЕстьФИО = ВидимостьФИО И ЗначениеЗаполнено(Объект.ПредставительФамилия);
	ЕстьТелефон = ЗначениеЗаполнено(ТелефонСтрахователяПредставление);
	ЕстьПочта = ЗначениеЗаполнено(Объект.АдресЭлектроннойПочтыСтрахователя);
	Заполнена = ЕстьФИО Или ЕстьТелефон Или ЕстьПочта;
	Если Развернута Тогда
		Элементы.ПредставительФИОГруппа.Видимость = ВидимостьФИО;
	Иначе
		Представления = Новый Массив;
		Если ЕстьФИО Тогда
			Представления.Добавить(СокрЛП(Объект.ПредставительФамилия + " " + Объект.ПредставительИмя + " " + Объект.ПредставительОтчество));
		КонецЕсли;
...
Процедура ОбновитьЭлементыФормы()
	...
	ЭтоТип104 = (Объект.ТипСообщения = 104);
	ЭтоТип10410 = (Объект.ТипСообщения = 10410);
	Элементы.ТипСообщения.Видимость = ЭтоТип10410 Или (
		ЭтоТип104 И СЭДОФСС.ТекущаяДатаФонда() >= СЭДОФСС.ДатаПереходаПособия5НаЕЦП());
	Элементы.ОтправлятьЧерезПрямыеВыплаты.Видимость = Объект.ОтправлятьЧерезПрямыеВыплаты
		Или Не Объект.ОтправлятьЧерезСЭДО
		Или Не Элементы.ТипСообщения.Видимость;
	Элементы.СотрудникИНН.Видимость = ЭтоТип104;
	Элементы.СотрудникДатаРождения.Видимость = ЭтоТип104;
	Элементы.КодСтраныГражданства.Видимость = ЭтоТип104;
	Элементы.ИдентификаторСтрокиРеестра.Видимость = ЭтоТип10410;
	Элементы.Ребенок.Видимость = ЭтоТип10410 И СЭДОФСС.ВидимостьПоляРодственник();
	Элементы.ОтветНаЗапрос.Видимость = ЭтоТип10410;
...
Процедура ЗаполнитьПрекращаемыеОтпуска()
	...
	Если Не ЗначениеЗаполнено(Объект.Сотрудник) Или Не ЗначениеЗаполнено(Объект.ДатаПрекращенияОплаты) Тогда
		Возврат;
	КонецЕсли;
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("ОтветНаЗапрос");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("РебенокФамилия");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("РебенокИмя");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("РебенокОтчество");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("РебенокДатаРождения");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("РебенокСНИЛС");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("Ребенок");
	ОчиститьИВключитьЗаполнениеРеквизитаШапки("ИдентификаторСтрокиРеестра");
	Если Объект.ТипСообщения = 10410 Тогда
		// Заполнение реквизитов шапки при обновлении вторичных данных.
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ОчиститьИВключитьЗаполнениеРеквизитаШапки(ИмяРеквизита)
	Объект[ИмяРеквизита] = Неопределено;
	ЭтотОбъект[ИмяРеквизита + "Фикс"] = Ложь;
КонецПроцедуры
...
&НаСервере
Функция ЗаголовокДляПрочихНеучтенныхДокументов(Неучтенные)
	Если ЗначениеЗаполнено(Объект.ОтветНаЗапрос) Тогда
		КоллекцииБЗК.УдалитьСтрокиТаблицыПоЗначению(Неучтенные, "ОтветНаЗапрос", Объект.ОтветНаЗапрос);
	ИначеЕсли ЗначениеЗаполнено(Объект.РебенокСНИЛС) Тогда
		КоллекцииБЗК.УдалитьСтрокиТаблицыПоЗначению(Неучтенные, "РебенокСНИЛС", Объект.РебенокСНИЛС);
	Иначе
		Неучтенные.Удалить(0);
	КонецЕсли;
	Количество = Неучтенные.Количество();
	Если Количество = 0 Тогда
		Возврат "";
	КонецЕсли;
	Тексты = Новый Массив;
	ПредставлениеКоличестваДокументов = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru = ';%1 Уведомление о прекращении отпуска;;%1 Уведомления о прекращении отпуска;%1 Уведомлений о прекращении отпуска;'"),
		Количество);
	Текст = СтрШаблон(НСтр("ru = 'После записи этого документа останется оформить еще %1 по сотруднику %2 на основании документов:'"),
		ПредставлениеКоличестваДокументов,
		Объект.Сотрудник);
	Тексты.Добавить(Текст);
	Для Каждого СтрокаТаблицы Из Неучтенные Цикл
		Если Не ЗначениеЗаполнено(СтрокаТаблицы.ОтветНаЗапрос) Тогда
			Продолжить;
		КонецЕсли;
		Тексты.Добавить(Символы.ПС + "• ");
		Тексты.Добавить(Новый ФорматированнаяСтрока(Строка(СтрокаТаблицы.ОтветНаЗапрос), , , , ПолучитьНавигационнуюСсылку(СтрокаТаблицы.ОтветНаЗапрос)));
	КонецЦикла;
	Возврат Новый ФорматированнаяСтрока(Тексты);
КонецФункции
...
Процедура ОбновитьПанельСостоянияДокумента()
	...
	Если ЕстьПравоОтправки Тогда
		Элементы.ОтправитьВФонд.Видимость = ЕстьПравоЭДОФСС И Объект.ОтправлятьЧерезСЭДО
			И Объект.Проведен И Не ЗначениеЗаполнено(Объект.ДатаОтправки);
