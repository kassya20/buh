...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтатусОтгрузки = СтатусыДокумента.ДополнительныйСтатус;
	...
	Для Индекс = 1 По КоллекцияСтатусов.Количество() - 1 Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтатусОтгрузки) И СтатусОтгрузки <> СтатусыДокумента.ДополнительныйСтатус Тогда
			СтатусОтгрузки = Перечисления.СтатусыОтгрузки.ПустаяСсылка();
		КонецЕсли;
	КонецЦикла;
...
Функция СохранитьИзменения()
	...
	Если УстановитьСтатусДокументовНаСервере(СписокДокументов.ВыгрузитьЗначения(), СтатусОплаты, СтатусОтгрузки) Тогда
		...
		Если Не ВладелецФормы.ОтображатьСтатусыДокументов Тогда
			ОбщегоНазначенияБПКлиент.ОповеститьОбИзмененииСтатусовДокументов(
				ВладелецФормы, СписокДокументов, СтатусОплаты, СтатусОтгрузки);
		КонецЕсли;
...
Функция УстановитьСтатусДокументовНаСервере(Знач МассивДокументов, Знач СтатусОплаты, Знач СтатусОтгрузки)
	...
	СтатусИзменен = РегистрыСведений.СтатусыДокументов.УстановитьСтатусыДокументов(МассивДокументов, СтатусыДокумента);
	
	Если СтатусИзменен
		И ИнтеграцияCRMПовтИсп.ИнтеграцияВИнформационнойБазеВключена() Тогда
		РегистрыСведений.ДокументыИнтеграцииCRM.ЗаполнитьДанныеКОтправкеИОповеститьОбИзмененияхВФоне(МассивДокументов);
	КонецЕсли;
