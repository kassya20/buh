...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.РеализацияТоваровУслуг.Форма.ФормаДоставка" Тогда
		ОбработкаВыбораРеквизитыФормаДоставка(ВыбранноеЗначение);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	СчетаФактурыНеТребуются = Документы.ПередачаТоваров.СчетаФактурыНеТребуются(МассивДокументов);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|СчетУчета,НомерГТД, СтранаПроисхождения, ПрослеживаемыйТовар,
		|ПодсказкаРНПТ, РНПТ, ИдентификаторСтроки, ПрослеживаемыйКомплект");
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиТаблицы, ТекущиеДанные);
	
	ДанныеОбъекта = Новый Структура(
		"Дата, ВидОперации, Организация, Склад, ТипЦен, СуммаВключаетНДС, ВедетсяУчетПрослеживаемыхТоваров");
...
&НаКлиенте
Процедура ОбработкаВыбораРеквизитыФормаДоставка(ВыбранноеЗначение)
...
Процедура ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(Форма, СтрокаТабличнойЧасти, ПараметрыОбъекта)
...
Процедура ДатаПриИзмененииНаСервере()
	...
	УстановитьФункциональныеОпцииФормы();
	УстановитьПараметрыВыбораСтавкиНДС();
	УстановитьПараметрДокументБезНДС();
...
&НаСервере
Процедура УстановитьПараметрДокументБезНДС()
	
	Если ПлательщикНДС Тогда 
		Объект.ДокументБезНДС   = Ложь;
		Объект.СуммаВключаетНДС = БухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию(
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Иначе
			...
			Если ИмяТаблицы = "Товары" Тогда
				...
				ЗаполнитьДобавленныеКолонкиСтрокиТаблицыТовары(ЭтаФорма, СтрокаТабличнойЧасти, ПараметрыОбъекта);
			КонецЕсли;
		КонецЕсли;
