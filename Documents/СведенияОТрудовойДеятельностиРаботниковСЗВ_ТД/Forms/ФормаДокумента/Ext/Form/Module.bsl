...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Организация) И Параметры.Свойство("Организация") Тогда
			Объект.Организация = Параметры.Организация;
		КонецЕсли;
		
		Если Параметры.Свойство("ДатаСобытия") Тогда
			Объект.Дата = Параметры.ДатаСобытия;
		КонецЕсли;
		...
		ИначеЕсли Параметры.Свойство("ОкончаниеПериодаЗаполнения") Тогда
			// Заполнение по периоду
			ЗаполнитьМероприятиеПоПериоду(Параметры.ОкончаниеПериодаЗаполнения);
			ЭлектронныйДокументооборотДоступен = ИнтерфейсыВзаимодействияБРО.ПодключенДокументооборотСКонтролирующимОрганом(Объект.Организация, "ПФР", Истина);
		КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДанныеПоПериоду(ОкончаниеПериодаЗаполнения)
	...
	ГраницаЗаполнения = ОкончаниеПериодаЗаполнения;
	Если КонецДня(ГраницаЗаполнения) > КонецДня(Объект.Дата) Тогда
		ГраницаЗаполнения = Объект.Дата;
	КонецЕсли;
	
	ПеренестиДанныеТрудовойДеятельностиСотрудниковВДокумент(
		ЭлектронныеТрудовыеКнижки.ДанныеТрудовойДеятельностиСотрудников(
			Истина, Объект.Организация, , ЭлектронныеТрудовыеКнижки.ДатаНачалаУчета(),
			ГраницаЗаполнения, Истина, Объект.Ссылка, ПараметрыЗаполнения),
		Ложь);
...
&НаСервере
Процедура ЗаполнитьМероприятиеПоПериоду(ОкончаниеПериодаЗаполнения)
	
	ЗарплатаКадрыКлиентСервер.ЗаполнитьМесяцПоДате(ЭтотОбъект, "Объект.ОтчетныйПериод", "ОтчетныйПериодПредставление");
	ЗаполнитьДанныеПоПериоду(ОкончаниеПериодаЗаполнения);
	ЗаписатьПровестиОтправитьПриСоздании = Истина;
...
Процедура УстановитьОтображениеПоДатеДокумента()
	...
	ЭтаФорма.Заголовок = ?(ЭтоЕФС1, "", НСтр("ru = 'Сведения о трудовой деятельности, СЗВ-ТД'") + ?(ЗначениеЗаполнено(Объект.Ссылка), 
...
Процедура ПрежнийСтраховательРегистрационныйНомерПФРНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	СписокЗначений.Добавить("9999999999", НСтр("ru = 'Рег. номер СФР (9999999999)'"), Элемент.Маска = "9999999999");
	СписокЗначений.Добавить("999-999-999999", НСтр("ru = 'Рег. номер ПФР (999-999-999999)'"), Элемент.Маска = "999-999-999999");
