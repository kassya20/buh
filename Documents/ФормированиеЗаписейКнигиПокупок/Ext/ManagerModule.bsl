...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("НачисленияИВычеты", Истина);
	
	Методы.Вставить("УстановитьКодВидаОперации");
...
Процедура НачисленияИВычеты(ПараметрыЗадачи, АдресХранилища) Экспорт
	...
	Показатели = УчетНДСФормированиеОтчетности.ПоказателиДекларацииНДС(
		...
		СписокОрганизаций,
		НачалоКвартала(ПараметрыЗадачи.Период), 
		КонецКвартала(ПараметрыЗадачи.Период),
		Ложь);
...
Функция ТекстЗапросаПоАвансамПолученным(НомераТаблиц)
	...
	|				И ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) В (&ВидыДоговоровРеализацииТоваров)
	|			ТОГДА ВЫБОР
	...
	|		КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.УчетАгентскогоНДС, ЛОЖЬ) = ИСТИНА
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.РеализацияТоваров)
	|						ТОГДА ""43""
	|					КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.РеализацияМедныхПолуфабрикатов)
	|						ТОГДА ""60""
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	...
	|							ИНАЧЕ ""26""
	|						КОНЕЦ
	|				КОГДА ЕСТЬNULL(СчетаФактурыДокументы.КодВидаОперации, ""22"") = ""52""
	|					ТОГДА ""54""
	|				ИНАЧЕ ""22""
	|			КОНЕЦ
	|	КОНЕЦ КАК КодВидаОперации,
...
Функция ПодготовитьДанныеРазделаВычетПоПриобретеннымЦенностям(СтруктураПараметров) Экспорт
	...
	СтруктураПараметров.Вставить("КлассифицироватьНоменклатуруПоОперациям0", УчетНДСВызовСервера.КлассифицироватьНоменклатуруПоОперациям0(СтруктураПараметров.Дата, СтруктураПараметров.Организация));
	...
	Если СтруктураПараметров.ПредъявленНДСКВычету0 Тогда
		...
		Если СтруктураПараметров.КлассифицироватьНоменклатуруПоОперациям0 Тогда
			...
			ВычетПоПриобретеннымЦенностям.Колонки.Добавить("СобытиеНДСПродажи", Новый ОписаниеТипов("ПеречислениеСсылка.СобытияПоНДСПродажи"));
		КонецЕсли;
	Иначе
	...
	Если СтруктураПараметров.ПредъявленНДСКВычету0 
		...
		ДатаПоискаСФ = ПолучитьДатуПоискаСчетовФактурПоПриобретеннымЦенностям(СтруктураПараметров.Дата);
		ТаблицаСФ = ОпределитьНаличиеСчетовФактурПолученных(ДатаПоискаСФ, МенеджерВТ_СписокСчетовФактур);
	...
	Для Каждого СтрокаСчетФактура Из ТаблицаНДСкВычету Цикл
		...
		Если СписокСчетовФактур.НайтиПоЗначению(СтрокаСчетФактура.СчетФактура) <> Неопределено 
			...
			ВычетОтражаетсяВРазделе5 = СтруктураПараметров.ПредъявленНДСКВычету0 И СтрокаСчетФактура.ПредъявленВычетВПоследующихПериодах;
			...
			Если НоваяСтрока.Состояние = Перечисления.НДССостоянияРеализация0.НеПодтвержденаРеализация0 
				И КонецКвартала(НоваяСтрока.ДокументОтгрузки.Дата) <> КонецКвартала(СтруктураПараметров.Дата)
				И СтруктураПараметров.Дата < '20240101'
				И НЕ ВычетОтражаетсяВРазделе5 Тогда
	...
	Если СтруктураПараметров.ПредъявленНДСКВычету0
		И СтруктураПараметров.КлассифицироватьНоменклатуруПоОперациям0 Тогда
...
Функция ПараметрыПоискаСчетовФактур(ПараметрыФормированияКниги, ДокументыОснования)
	...
	Если ЗначениеЗаполнено(ПараметрыФормированияКниги.Организация) Тогда
		СписокДоступныхОрганизаций = ОбщегоНазначенияБП.СписокДоступныхОрганизаций(ПараметрыФормированияКниги.Организация);
...
Функция ЗаполнитьНДСКВычетуПоДаннымРегистраНДСПредъявленныйРеализация0(СтруктураПараметров)
	...
	Если НЕ СтруктураПараметров.КлассифицироватьНоменклатуруПоОперациям0 Тогда
		...
		|			ИЛИ НДСПредъявленныйРеализация0Остатки.СчетФактура ССЫЛКА Документ.ГТДИмпорт
	...
	Иначе
		...
		|	ИСТИНА КАК НДСРаспределенный
		|ПОМЕСТИТЬ ВТ_НДСРаспределенный
		|ИЗ
		|	НДСПредъявленныйРеализация0Остатки КАК НДСПредъявленныйРеализация0Остатки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.НДСПредъявленныйРеализация0 КАК НДСПредъявленныйРеализация0
		|		ПО НДСПредъявленныйРеализация0Остатки.СчетФактура = НДСПредъявленныйРеализация0.СчетФактура
		|			И НДСПредъявленныйРеализация0Остатки.ДокументОтгрузки = НДСПредъявленныйРеализация0.ДокументОтгрузки
		|ГДЕ
		|	НДСПредъявленныйРеализация0.Регистратор ССЫЛКА Документ.РаспределениеНДС
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	СчетФактура,
		|	ДокументОтгрузки
		...
		|	НДСПредъявленныйРеализация0Остатки КАК НДСПредъявленныйРеализация0Остатки
		|ГДЕ
		|	НЕ (НДСПредъявленныйРеализация0Остатки.СчетФактура, НДСПредъявленныйРеализация0Остатки.ДокументОтгрузки) В
		|				(ВЫБРАТЬ
		|					ВТ_НДСРаспределенный.СчетФактура,
		|					ВТ_НДСРаспределенный.ДокументОтгрузки
		|				ИЗ
		|					ВТ_НДСРаспределенный КАК ВТ_НДСРаспределенный)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВТ_НДСРаспределенный.СчетФактура,
		|	ВТ_НДСРаспределенный.СчетФактураДата,
		|	ВТ_НДСРаспределенный.ВидЦенности,
		|	ВТ_НДСРаспределенный.СтавкаНДС,
		|	ВТ_НДСРаспределенный.СчетУчетаНДС,
		|	ВТ_НДСРаспределенный.СуммаБезНДС,
		|	ВТ_НДСРаспределенный.НДС,
		|	ВТ_НДСРаспределенный.ДокументОтгрузки,
		|	ВТ_НДСРаспределенный.Состояние,
		|	ВТ_НДСРаспределенный.Поставщик,
		|	ВТ_НДСРаспределенный.ИсправленныйСчетФактура,
		|	ВТ_НДСРаспределенный.ПредъявленВычетВПоследующихПериодах,
		|	ВТ_НДСРаспределенный.СобытиеНДСПродажи,
		|	ВТ_НДСРаспределенный.НДСРаспределенный
		|ИЗ
		|	ВТ_НДСРаспределенный КАК ВТ_НДСРаспределенный
		|
		|ИНДЕКСИРОВАТЬ ПО
	...
	Если НЕ СтруктураПараметров.КлассифицироватьНоменклатуруПоОперациям0 Тогда
		...
		|				ИЛИ ТИПЗНАЧЕНИЯ(ВТ_НДСПродажи.Регистратор) = ТИП(Документ.ТребованиеНакладная)
		|				ИЛИ ТИПЗНАЧЕНИЯ(ВТ_НДСПродажи.Регистратор) = ТИП(Документ.ОтчетПроизводстваЗаСмену)
		|				ИЛИ ВТ_НДСПредъявленныйРеализация0Остатки.ДокументОтгрузки = ВТ_НДСПродажи.Регистратор
		|				ИЛИ ВТ_НДСПредъявленныйРеализация0Остатки.ДокументОтгрузки ССЫЛКА Документ.СчетФактураВыданный
...
Процедура ИсключитьНДСпоНепринятымКУчетуНМА(СтруктураПараметров, ТаблицаНДСкВычету)
	...
	Запрос.УстановитьПараметр("РегламентнаяОперация", Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости);
	...
	|ПОМЕСТИТЬ ВТ_ДокументыПодготовкиКВычету
	|ИЗ
	|	РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК ВыполнениеРегламентныхОперацийНДС
	|ГДЕ
	|	ВыполнениеРегламентныхОперацийНДС.РегламентнаяОперация = &РегламентнаяОперация
	|	И ВыполнениеРегламентныхОперацийНДС.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НДСПредъявленныйОбороты.Поставщик КАК Поставщик,
	|	НДСПредъявленныйОбороты.СчетФактура КАК СчетФактура
	|ПОМЕСТИТЬ ВТ_НДСПодготовленныйКВычетуПоОстаткам
	|ИЗ
	...
	|			Регистратор,
	|			Организация = &Организация
	|				И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НМА)
	|				И НЕ СчетФактура ССЫЛКА Документ.КорректировкаРеализации
	|				И НЕ СчетФактура ССЫЛКА Документ.АвансовыйОтчет) КАК НДСПредъявленныйОбороты
	|ГДЕ
	|	НДСПредъявленныйОбороты.Регистратор В
	|			(ВЫБРАТЬ
	|				ДокументыПодготовкиКВычету.РегламентныйДокумент КАК РегламентныйДокумент
	|			ИЗ
	|				ВТ_ДокументыПодготовкиКВычету КАК ДокументыПодготовкиКВычету)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаНДСкВычету.Поставщик КАК Поставщик,
	|	ВТ_ТаблицаНДСкВычету.СчетФактура КАК СчетФактура
	...
	|	И НЕ (ВТ_ТаблицаНДСкВычету.Поставщик, ВТ_ТаблицаНДСкВычету.СчетФактура) В
	|				(ВЫБРАТЬ
	|					ВТ_НДСПодготовленныйКВычетуПоОстаткам.Поставщик КАК Поставщик,
	|					ВТ_НДСПодготовленныйКВычетуПоОстаткам.СчетФактура КАК СчетФактура
	|				ИЗ
	|					ВТ_НДСПодготовленныйКВычетуПоОстаткам КАК ВТ_НДСПодготовленныйКВычетуПоОстаткам)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_СгруппированныеДанныеПоНМА.Поставщик КАК Поставщик,
	|	ВТ_СгруппированныеДанныеПоНМА.СчетФактура КАК СчетФактура,
	|	ВТ_СгруппированныеДанныеПоНМА.СуммаПоступленияНМА КАК СуммаПоступленияНМА,
	|	ВТ_СгруппированныеДанныеПоНМА.ОстатокНеПринятоКУчетуНМА КАК ОстатокНеПринятоКУчетуНМА,
	...
	СтруктураПоиска = Новый Структура("Поставщик, СчетФактура, ВидЦенности");
	ТаблицаНДСкВычету.Индексы.Добавить("Поставщик, СчетФактура, ВидЦенности");
	
	СтруктураПоиска.ВидЦенности = Перечисления.ВидыЦенностей.НМА;
...
Процедура ПолучитьДанныеОДокументахОплатыИмпорт(СтруктураПараметров, ТаблицаРезультатов)
	...
	|			И (ХозрасчетныйСубконтоКт.УточнениеПериода = 0)
	|ГДЕ
	...
	КлючеваяОперация = "ИндексТаблицыСубконто.КнигаПокупок_ДанныеОДокументахОплатыИмпорт";
...
Процедура ПолучитьДанныеОДокументахОплатыКомандировочныеРасходы(СтруктураПараметров, ТаблицаРезультатов)
	...
	|	ВТКомандировочныеРасходы.СчетФактура.ДокументОснование КАК ДокументОснование
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Прочее КАК Прочее
	|			ПО СчетаФактуры.СчетФактура = Прочее.СчетФактура
	|			И СчетаФактуры.ДокументОснование = Прочее.Ссылка
	|	ГДЕ
	|		Прочее.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийАвансовыйОтчет.Командировка)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|	ИЗ
	|		СчетаФактуры КАК СчетаФактуры
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Билеты КАК Билеты
	|			ПО СчетаФактуры.СчетФактура = Билеты.СчетФактура
	|			И СчетаФактуры.ДокументОснование = Билеты.Ссылка) КАК КомандировочныеРасходы
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
...
Процедура ПолучитьДанныеОДокументахОплатыЭлектронныеУслуги(СтруктураПараметров, ТаблицаРезультатов)
	...
	ВремяНачала = ОценкаПроизводительности.НачатьЗамерВремени();
	КлючеваяОперация = "ИндексТаблицыСубконто.КнигаПокупок_ДанныеОДокументахОплатыЭлектронныеУслуги";
...
Функция РаспределитьВычетыПоКодамОпераций(ВычетПоПриобретеннымЦенностям, Дата)
	...
	Для Каждого КолонкаТабличнойЧасти Из Метаданные.Документы.ФормированиеЗаписейКнигиПокупок.ТабличныеЧасти.ВычетПоПриобретеннымЦенностям.Реквизиты Цикл
		ВычетПоПриобретеннымЦенностямКодамОпераций.Колонки.Добавить(КолонкаТабличнойЧасти.Имя, КолонкаТабличнойЧасти.Тип);
	КонецЦикла;
	...
	Для Каждого СтрокаТаблицыКодовОпераций0 Из ТаблицаКодовОпераций0 Цикл
		Если ПредыдущийКод <> СтрокаТаблицыКодовОпераций0.Код
			...
			И (СтрокаТаблицыКодовОпераций0.ДатаОкончания = '00010101'
					И СтрокаТаблицыКодовОпераций0.ДатаНачала <= Дата
				ИЛИ Дата <= СтрокаТаблицыКодовОпераций0.ДатаОкончания
					И Дата >= СтрокаТаблицыКодовОпераций0.ДатаНачала) Тогда
	...
	|			ПО (ПодтверждениеНулевойСтавкиНДССостав.СчетФактураВыданный = НДСЗаписиКнигиПродаж.СчетФактура)
	...
	Пока ВыборкаНДСРаспределенный.Следующий() Цикл
		...
		Пока ВыборкаПоДокументуОтгрузки.Следующий() Цикл
			...
			Пока ВыборкаПоДокументуПриобретения.Следующий() Цикл
				...
				ВыборкаПоВидамЦенностей = ВыборкаПоДокументуПриобретения.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				Пока ВыборкаПоВидамЦенностей.Следующий() Цикл 
					
					ВыборкаПоСтавкамНДС = ВыборкаПоВидамЦенностей.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
					Пока ВыборкаПоСтавкамНДС.Следующий() Цикл
						...
						ВыборкаРаспределяемыхСумм = ВыборкаПоСтавкамНДС.Выбрать();
						Пока ВыборкаРаспределяемыхСумм.Следующий() Цикл
						
							НДСДляРаспределения = НДСДляРаспределения + ВыборкаРаспределяемыхСумм.НДС;
							СуммаБезНДСДляРаспределения = СуммаБезНДСДляРаспределения + ВыборкаРаспределяемыхСумм.СуммаБезНДС;
							
							Коэффициенты.Добавить(ВыборкаРаспределяемыхСумм.Коэффициент);
							
						КонецЦикла;
						
						МассивНДС = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(НДСДляРаспределения, Коэффициенты);
						МассивСуммаБезНДС = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СуммаБезНДСДляРаспределения, Коэффициенты);
						
						Если МассивНДС <> Неопределено И
							МассивСуммаБезНДС <> Неопределено Тогда
	...
	ВычетПоПриобретеннымЦенностямКодамОпераций.Сортировать("ДатаПолученияСчетаФактуры,СчетФактура,ДокументОтгрузки", Новый СравнениеЗначений);
...
Функция ПолучитьДанныеОЗачтенныхАвансахПоСпискуСФ_УпрощенныйУчетНДС(СтруктураПараметров, МенеджерВТ_СписокСчетовФактур)
	...
	|			И (ХозрасчетныйСубконтоКТ1.УточнениеПериода = 0)
	...
	|			И (ХозрасчетныйСубконтоКТ3.УточнениеПериода = 0)
	|ГДЕ
	|	Хозрасчетный.Период <= &КонецПериода
...
Процедура ПолучитьДанныеОбОтгрузочныхСчетахФактурах(СтруктураПараметров, ТаблицаРезультатов, МенеджерВТ_СписокСчетовФактур)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СУММА(ХозрасчетныйОбороты.СуммаОборотДт) КАК СуммаЗачета,
	|	ХозрасчетныйОбороты.Регистратор КАК ДокументЗачетаАванса,
	|	ХозрасчетныйОбороты.Период КАК ДатаСобытия,
	|	ХозрасчетныйОбороты.Субконто1 КАК СчетФактура,
	|	ХозрасчетныйОбороты.Субконто2 КАК Контрагент,
	|	ХозрасчетныйОбороты.Организация КАК Организация
	...
	|					ИЗ
	|						СписокСчетовФактур),
	|			НЕ КорСчет В (&СчетаВозвратаДенежныхСредств),
	|			) КАК ХозрасчетныйОбороты
	|ГДЕ
	|	ХозрасчетныйОбороты.СуммаОборотДт > 0
	|
	...
	|	ДокументыЗачетаАванса.Контрагент КАК Контрагент,
	...
	|			И ДокументыЗачетаАванса.Контрагент = СчетФактураДокументыОснования.Ссылка.Контрагент
	|			И (СчетФактураДокументыОснования.Ссылка.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаРеализацию))
	...
	ЗачтенныйНДС.Индексы.Добавить("СчетФактура,Контрагент,ДатаСобытия");
	Отбор = Новый Структура("СчетФактура,Контрагент,ДатаСобытия");
	...
	Для каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		Отбор.СчетФактура = СтрокаТаблицы.СчетФактура;
		Отбор.ДатаСобытия = СтрокаТаблицы.ДатаСобытия;
		Отбор.Контрагент  = СтрокаТаблицы.Покупатель;
...
Функция ПодготовитьДанныеРазделаВычетНДСПоНалоговомуАгенту(СтруктураПараметров) Экспорт
	...
	Если ТаблицаРезультатов.Количество() = 0 
		И ТаблицаРезультатаПриВозвратеАванса.Количество() = 0 Тогда
		// Дальнейшая обработка не требуется, не обнаружен НДС, который может быть принят к вычету.
	...
	Если ТаблицаРезультатаПриВозвратеАванса.Количество() <> 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаРезультатаПриВозвратеАванса Цикл
			...
			НоваяСтрока.СчетУчетаНДС   = ПланыСчетов.Хозрасчетный.РасчетыНДСНалоговогоАгента;
		КонецЦикла;
	...
	ВычетНДСПоНалоговомуАгенту.Свернуть(
		...
		|ИсправленныйСчетФактура",
		"СуммаБезНДС,НДС, ВозвратАвансовВыданных");
	
	ВычетНДСПоНалоговомуАгенту.Сортировать("ВозвратАвансовВыданных, СчетФактураДата,СчетФактура,ДатаОплаты", Новый СравнениеЗначений);
...
Функция ТаблицаВычетаПриВозвратеАвансаНерезидента(СтруктураПараметров)
	...
	|ПОМЕСТИТЬ ОстаткиПоСчету
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаГраница, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыНДСНалоговогоАгента), &ВидыСубконтоНалоговыйАгент, Организация = &Организация) КАК ХозрасчетныйОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО ХозрасчетныйОстатки.Субконто2 = ДоговорыКонтрагентов.Ссылка
	|ГДЕ
	|	ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.Нерезидент)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	ДоговорКонтрагента,
	...
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ОстаткиПоСчету.Контрагент,
	|						ОстаткиПоСчету.ДоговорКонтрагента
	|					ИЗ
	|						ОстаткиПоСчету КАК ОстаткиПоСчету),
	|			,
	|			) КАК ХозрасчетныйОбороты
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ХозрасчетныйОбороты.Регистратор) = ТИП(Документ.ПоступлениеНаРасчетныйСчет)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВозвратыЗаПериод.Поставщик КАК Поставщик,
	|	ВозвратыЗаПериод.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВозвратыЗаПериод.ДокументОплаты КАК ДокументОплаты,
	|	ВозвратыЗаПериод.Организация КАК Организация,
	|	ВозвратыЗаПериод.СуммаВозврата КАК СуммаВозврата
	|ИЗ
	|	ВозвратыЗаПериод КАК ВозвратыЗаПериод";
	
	ВидыСубконтоНалоговыйАгент = Новый Массив;
	ВидыСубконтоНалоговыйАгент.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	ВидыСубконтоНалоговыйАгент.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	...
	ВидыСубконтоАвансы = Новый Массив();
	ВидыСубконтоАвансы.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	ВидыСубконтоАвансы.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	ВидыСубконтоАвансы.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами);
	
	Запрос.УстановитьПараметр("Организация",                СтруктураПараметров.Организация);
	Запрос.УстановитьПараметр("ДатаГраница",                Новый Граница(КонецДня(СтруктураПараметров.Дата), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ВидыСубконтоНалоговыйАгент", ВидыСубконтоНалоговыйАгент);
	Запрос.УстановитьПараметр("МассивСчетовАвансов",        МассивСчетовАвансов);
	Запрос.УстановитьПараметр("ВидыСубконтоАвансы",         ВидыСубконтоАвансы);
	// При отказе от товаров/услуг к вычету подлежат суммы НДС не позднее года с момента отказа.
	Запрос.УстановитьПараметр("НачалоПериода",              ДобавитьМесяц(НачалоКвартала(СтруктураПараметров.Дата), -12));
	...
	|			&ДатаГраница,
	|			,
	|			(Организация, Покупатель, ДоговорКонтрагента, СчетФактура) В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ВозвратыЗаПериод.Организация,
	|						ВозвратыЗаПериод.Поставщик,
	|						ВозвратыЗаПериод.ДоговорКонтрагента,
	|						ВозвратыЗаПериод.ДокументОплаты
	|					ИЗ
	|						ВозвратыЗаПериод КАК ВозвратыЗаПериод)
	|				И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НалоговыйАгентИностранцы)) КАК НДСЗаписиКнигиПродажОбороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			&ДатаГраница,
	|			,
	|			(Организация, Поставщик, ДоговорКонтрагента, ДокументОплаты) В
	|					(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|						ВозвратыЗаПериод.Организация,
	|						ВозвратыЗаПериод.Поставщик,
	|						ВозвратыЗаПериод.ДоговорКонтрагента,
	...
	|					ИЗ
	|						ВозвратыЗаПериод КАК ВозвратыЗаПериод)
	|				И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.НалоговыйАгентИностранцы)) КАК НДСЗаписиКнигиПокупокОбороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	Начисления.Покупатель КАК Контрагент,
	|	Начисления.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	Начисления.СчетФактура КАК СчетФактура,
	|	Начисления.ВидЦенности КАК ВидЦенности,
	|	СУММА(Начисления.СуммаБезНДСОборот - ЕСТЬNULL(Вычеты.СуммаБезНДСОборот, 0)) КАК СуммаБезНДС,
	|	СУММА(Начисления.НДСОборот - ЕСТЬNULL(Вычеты.НДСОборот, 0)) КАК НДС,
	|	Начисления.ДатаСобытия КАК ДатаСобытия,
	|	Начисления.СтавкаНДС КАК СтавкаНДС
	|ПОМЕСТИТЬ НепогашенныеНачисленияПредварительная
	|ИЗ
	|	Начисления КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Вычеты КАК Вычеты
	|		ПО Начисления.Организация = Вычеты.Организация
	|			И Начисления.Покупатель = Вычеты.Поставщик
	|			И Начисления.ДоговорКонтрагента = Вычеты.ДоговорКонтрагента
	|			И Начисления.СчетФактура = Вычеты.ДокументОплаты
	|ГДЕ
	|	Начисления.НДСОборот - ЕСТЬNULL(Вычеты.НДСОборот, 0) > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	Начисления.ДоговорКонтрагента,
	|	Начисления.Организация,
	|	Начисления.Покупатель,
	|	Начисления.СчетФактура,
	|	Начисления.ВидЦенности,
	|	Начисления.ДатаСобытия,
	|	Начисления.СтавкаНДС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Контрагент,
	...
	|					СчетаАвансов КАК СчетаАвансов),
	|			&ВидыСубконтоАвансы,
	|			(Организация, Субконто1, Субконто2, Субконто3) В
	|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|					НепогашенныеНачисленияПредварительная.Организация,
	|					НепогашенныеНачисленияПредварительная.Контрагент,
	|					НепогашенныеНачисленияПредварительная.ДоговорКонтрагента,
	|					НепогашенныеНачисленияПредварительная.СчетФактура
	|				ИЗ
	|					НепогашенныеНачисленияПредварительная КАК НепогашенныеНачисленияПредварительная)) КАК ХозрасчетныйОстатки
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НепогашенныеНачисленияПредварительная.Организация КАК Организация,
	|	НепогашенныеНачисленияПредварительная.Контрагент КАК Поставщик,
	|	НепогашенныеНачисленияПредварительная.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	НепогашенныеНачисленияПредварительная.СчетФактура КАК СчетФактура,
	|	НепогашенныеНачисленияПредварительная.ВидЦенности КАК ВидЦенности,
	|	НепогашенныеНачисленияПредварительная.СуммаБезНДС КАК СуммаБезНДС,
	|	НепогашенныеНачисленияПредварительная.НДС КАК НДС,
	|	ЕСТЬNULL(ОстаткиАвансов.Сумма, 0) КАК СуммаАванса,
	|	НепогашенныеНачисленияПредварительная.ДатаСобытия КАК ДатаСобытия,
	|	НепогашенныеНачисленияПредварительная.СтавкаНДС КАК СтавкаНДС,
	|	НепогашенныеНачисленияПредварительная.СчетФактура КАК ДокументОплаты,
	|	НепогашенныеНачисленияПредварительная.СуммаБезНДС + НепогашенныеНачисленияПредварительная.НДС КАК СНДС,
	|	ЕСТЬNULL(ВозвратыЗаПериод.СуммаВозврата, 0) КАК СуммаВозврата
	|ИЗ
	|	НепогашенныеНачисленияПредварительная КАК НепогашенныеНачисленияПредварительная
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиАвансов КАК ОстаткиАвансов
	|		ПО НепогашенныеНачисленияПредварительная.Организация = ОстаткиАвансов.Организация
	|			И НепогашенныеНачисленияПредварительная.Контрагент = ОстаткиАвансов.Поставщик
	|			И НепогашенныеНачисленияПредварительная.ДоговорКонтрагента = ОстаткиАвансов.ДоговорКонтрагента
	|			И НепогашенныеНачисленияПредварительная.СчетФактура = ОстаткиАвансов.СчетФактура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВозвратыЗаПериод КАК ВозвратыЗаПериод
	|		ПО НепогашенныеНачисленияПредварительная.Организация = ВозвратыЗаПериод.Организация
	|			И НепогашенныеНачисленияПредварительная.СчетФактура = ВозвратыЗаПериод.ДокументОплаты
	|			И НепогашенныеНачисленияПредварительная.Контрагент = ВозвратыЗаПериод.Поставщик
	|			И НепогашенныеНачисленияПредварительная.ДоговорКонтрагента = ВозвратыЗаПериод.ДоговорКонтрагента
	|ГДЕ
	|	ЕСТЬNULL(ОстаткиАвансов.Сумма, 0) = 0
	|	И ЕСТЬNULL(ВозвратыЗаПериод.СуммаВозврата, 0) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаСобытия УБЫВ,
	|	СчетФактура
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиПоСчету.Счет КАК Счет,
	|	ОстаткиПоСчету.Контрагент КАК Поставщик,
	|	ОстаткиПоСчету.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ОстаткиПоСчету.СуммаОстатокДт КАК СуммаОстатокДт,
	|	ОстаткиПоСчету.Организация КАК Организация
	|ИЗ
	|	ОстаткиПоСчету КАК ОстаткиПоСчету";
	
	Результаты = Запрос.ВыполнитьПакет();
	
	Результат        = Результаты[4].Выгрузить();
	Остатки76НА      = Результаты[5].Выгрузить();
	ТекущийПоставщик = Неопределено;
	ТекущийДоговор   = Неопределено;
	Отбор            = Новый Структура("Поставщик, ДоговорКонтрагента");
	ОстатокНДС       = 0; // остаток на 76.НА
	ТекущаяСуммаНДС  = 0;
	
	ТаблицаВычета = Результат.СкопироватьКолонки();
	// Обходим таблицу начислений, сравнивая, что начисления не больше суммы возврата и не превышают остатки на 76.НА
	Для Каждого СтрокаТаблицы Из Результат Цикл
		
		// Остатки начислений не должны превышать сумму возврата.
		Если СтрокаТаблицы.СуммаБезНДС > СтрокаТаблицы.СуммаВозврата Тогда
			МассивСуммБезНДС = Новый Массив;
			МассивСуммБезНДС.Добавить(СтрокаТаблицы.СуммаБезНДС - СтрокаТаблицы.СуммаВозврата);
			МассивСуммБезНДС.Добавить(СтрокаТаблицы.СуммаВозврата);
			СуммыНДС = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(СтрокаТаблицы.НДС, МассивСуммБезНДС);
			СтрокаТаблицы.СуммаБезНДС = СтрокаТаблицы.СуммаВозврата;
			СтрокаТаблицы.НДС         = СуммыНДС[1];
		КонецЕсли;
		
		// Остатки начислений не должны превышать остатки на 76.НА.
		Если СтрокаТаблицы.Поставщик <> ТекущийПоставщик
			ИЛИ СтрокаТаблицы.ДоговорКонтрагента <> ТекущийДоговор Тогда
			ТекущийПоставщик = СтрокаТаблицы.Поставщик;
			ТекущийДоговор   = СтрокаТаблицы.ДоговорКонтрагента;
			ОстатокНДС       = 0;
			ТекущаяСуммаНДС  = 0;
			ЗаполнитьЗначенияСвойств(Отбор, СтрокаТаблицы);
			Строки76НА = Остатки76НА.НайтиСтроки(Отбор);
			Если Строки76НА.Количество() > 0 Тогда
				ОстатокНДС = Строки76НА[0].СуммаОстатокДт;
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		Если ТекущаяСуммаНДС >= ОстатокНДС Тогда
			Продолжить;
		КонецЕсли;
		
		ТекущаяСуммаНДС = ТекущаяСуммаНДС + СтрокаТаблицы.НДС;
		Если ТекущаяСуммаНДС > ОстатокНДС Тогда
			ПревышающийОстатокНДС = ТекущаяСуммаНДС - ОстатокНДС;
			ДоступныйНДСКВычету = СтрокаТаблицы.НДС - ПревышающийОстатокНДС;
			МассивСуммНДС = Новый Массив;
			МассивСуммНДС.Добавить(СтрокаТаблицы.НДС - ДоступныйНДСКВычету);
			МассивСуммНДС.Добавить(ДоступныйНДСКВычету);
			СуммыБезНДС = ОбщегоНазначенияБПКлиентСервер.РаспределитьПропорционально(СтрокаТаблицы.СуммаБезНДС, МассивСуммНДС);
			СтрокаТаблицы.НДС         = ДоступныйНДСКВычету;
			СтрокаТаблицы.СуммаБезНДС = СуммыБезНДС[1];
		КонецЕсли;
		
		СтрокаТаблицы.СтавкаНДС = Перечисления.СтавкиНДС.ОбычнаяСтавкаНДСПоРасчетной(СтрокаТаблицы.СтавкаНДС);
		
		НоваяСтрока = ТаблицаВычета.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
		
	КонецЦикла;
	
	Возврат ТаблицаВычета;
...
Процедура ИсключитьНДСНалоговогоАгентаНеоплаченныйВБюджет(СтруктураПараметров, ТаблицаРезультатов)
	...
	Если УчетНДС.ПравоНаВычетНДСНалоговогоАгентаДоОплатыВБюджет(СтруктураПараметров.Дата) Тогда
		Возврат;
	КонецЕсли;
