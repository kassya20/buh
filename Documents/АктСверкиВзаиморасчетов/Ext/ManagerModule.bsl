...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьПризнакПодписан");
...
// 
// Параметры:
//    Документ - ДокументОбъект.АктСверкиВзаиморасчетов - документ, для которого устанавливаются значения реквизитов.
//    ДатаНачала - Дата - Дата начала периода сверки.
//    ДатаОкончания - Дата - Дата окончания периода сверки.
//
Процедура УстановитьПериодСверки(Документ, ДатаНачала, ДатаОкончания) Экспорт
...
Процедура СопоставитьДанныеОрганизацииИКонтрагента(Объект) Экспорт
	...
	Если СтрокаРезультата.ЕстьРасхождения Тогда
...
Функция ПодготовитьЗапрос(СтруктураПараметров, СчетаИсключения)
	...
	Запрос.УстановитьПараметр("Счет62Р",          ПланыСчетов.Хозрасчетный.РасчетыСРозничнымиПокупателями);
...
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		// Проверяем наличие расхождений по-разному для электронного и бумажного документа для целей исправления ошибки, когда
		// для бумажного документа ошибочно не возводился признак ЕстьРасхождения;
		// для электронного документа не актуализировался реквизит Расхождение при получении данных второй стороны.
		ЕстьРасхождения = Шапка.ЭтоЭлектронныйДокумент И Шапка.ЕстьРасхождения И Шапка.Расхождение <> 0
			ИЛИ НЕ Шапка.ЭтоЭлектронныйДокумент И Шапка.Расхождение <> 0;
		Если ЕстьРасхождения И Шапка.СверкаСогласована Тогда
