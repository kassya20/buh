...
Функция ПечатьСписокУчредителей(МассивОбъектов, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТаблицы Из ТаблицаУчредителей Цикл
			...
			ПараметрыСтрока.Вставить("ДанныеУчастника",    ОписаниеУчредителя(СтрокаТаблицы, Выборка.Дата));
...
Функция ОписаниеУчредителя(Учредитель, Дата)
	
	ОписаниеУчредителя = "";
	Если Учредитель.ЭтоЮридическоеЛицо Тогда
		
		СведенияОбУчредителе = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(Учредитель.Учредитель, Дата);
	...
	Иначе
		...
		ДанныеФизическогоЛица = УчетЗарплаты.ДанныеФизическихЛиц( , Учредитель.Учредитель, Дата);
