...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьРучнуюНастройкуУСНДляАктуальныхДокументов");
	Методы.Вставить("ОбработатьРучнуюНастройкуУСНОтложенно");
...
Функция ПолучитьЗакупленные(Реквизиты) Экспорт
	...
	|	ПоступлениеТовары.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ПоступлениеТовары.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	...
	|	ПоступлениеТовары.ПрослеживаемыйТовар,
	|	ПоступлениеТовары.ПрослеживаемыйКомплект
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	СУММА(ВЫБОР
	|			КОГДА ПоступлениеАгентскиеУслуги.Количество = 0
	|				ТОГДА 1
	|			ИНАЧЕ ПоступлениеАгентскиеУслуги.Количество
	|		КОНЕЦ),
	|	ПоступлениеАгентскиеУслуги.Цена,
	|	"""",
	|	ДАТАВРЕМЯ(1, 1, 1),
	|	НЕОПРЕДЕЛЕНО,
	|	ЛОЖЬ,
	...
	|	ВТ_СтрокиДокументов.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ВТ_СтрокиДокументов.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ИЗ
	|	ВТ_ЗакупленныеТоварыКомитентовОстатки КАК ВТ_ЗакупленныеТоварыКомитентовОстатки
	...
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ВТ_СтрокиДокументов.Партия
	|			ИЗ
	|				ВТ_СтрокиДокументов)
	|	И НЕ ПоступлениеТоваровУслугТовары.ПрослеживаемыйТовар
	|	И ПоступлениеТоваровУслугТовары.Контрагент = &Контрагент
	|	И ПоступлениеТоваровУслугТовары.ДоговорКонтрагента = &ДоговорКонтрагента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПоступлениеТоваровУслугСведенияПрослеживаемости.Количество,
	...
	|	ПоступлениеТоваровУслугТовары.ПрослеживаемыйКомплект,
...
Функция ДокументыСПрослеживаемымиТоварами(МассивДокументов) Экспорт
	...
	|ГДЕ
	|	ОтчетКомитентуОПродажахТовары.Ссылка В(&МассивДокументов)
	|	И ОтчетКомитентуОПродажахТовары.ПрослеживаемыйТовар = ИСТИНА";
...
Функция РНПТРеализованныхПрослеживаемыхТоваров(ОтчетКомитентуОПродажах) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ОтчетКомитентуОПродажах", ОтчетКомитентуОПродажах);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОтчетКомитентуОПродажахТовары.Номенклатура КАК Номенклатура,
	|	ОтчетКомитентуОПродажахТовары.ДокументРасчетов КАК ДокументРасчетов,
	|	ЕСТЬNULL(НомераГТД.Код, """") КАК РНПТ,
	|	СУММА(ЕСТЬNULL(ПрослеживаемыеТовары.Количество, 0)) КАК Количество,
	|	СУММА(ЕСТЬNULL(ПрослеживаемыеТовары.КоличествоПрослеживаемости, 0)) КАК КоличествоПрослеживаемости,
	|	МАКСИМУМ(ЕСТЬNULL(СтраныМира.Наименование, """")) КАК СтранаПроисхождения
	|ИЗ
	|	Документ.ОтчетКомитентуОПродажах.Товары КАК ОтчетКомитентуОПродажахТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПрослеживаемыеТовары КАК ПрослеживаемыеТовары
	|		ПО ОтчетКомитентуОПродажахТовары.ДокументРасчетов = ПрослеживаемыеТовары.Регистратор
	|			И ОтчетКомитентуОПродажахТовары.Номенклатура = ПрослеживаемыеТовары.Номенклатура
	|			И ОтчетКомитентуОПродажахТовары.Ссылка.Контрагент = ПрослеживаемыеТовары.Комитент
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НомераГТД КАК НомераГТД
	|		ПО (ПрослеживаемыеТовары.РНПТ = НомераГТД.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтраныМира КАК СтраныМира
	|		ПО (ПрослеживаемыеТовары.СтранаПроисхождения = СтраныМира.Ссылка)
	|ГДЕ
	|	ОтчетКомитентуОПродажахТовары.Ссылка = &ОтчетКомитентуОПродажах
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетКомитентуОПродажахТовары.Номенклатура,
	|	ОтчетКомитентуОПродажахТовары.ДокументРасчетов,
	|	ЕСТЬNULL(НомераГТД.Код, """")";
...
Процедура ОбработатьРучнуюНастройкуУСН(СтрокаПоДокументу)
	...
	Попытка
		...
		Если Не ДокументОбъект.РучнаяКорректировка
			И (ДокументОбъект.Графа4_УСН <> 0 Или ДокументОбъект.ДоходыЕНВД_УСН Или ДокументОбъект.УдалитьРасходыЕНВД_УСН
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаСписка"
		...
		ИЛИ ВидФормы = "ФормаИсторииИзмененийИсторииДанных" Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаНачалаУчетаПрослеживаемыхТоваров", 
	...
	Запрос.Текст = ТекстЗапросаТаблицаУслуги(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		...
		+ ТекстЗапросаПрослеживаемыеТоварыЗакупленные(НомераТаблиц, ПараметрыПроведения, Реквизиты)
		+ ТекстЗапросаПрослеживаемыеТоварыРеализованные(НомераТаблиц, ПараметрыПроведения, Реквизиты);
...
Функция ТекстЗапросаВременныеТаблицыДокумента(НомераТаблиц)
	...
	|	ТаблицаТовары.ДатаРеализации КАК ДатаРеализации,
	|	ТаблицаТовары.ДокументРасчетов КАК ДокументРасчетов,
	|	ТаблицаТовары.СуммаПоступления КАК СуммаПоступления,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаВознаграждение.Ссылка КАК Ссылка,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаСуммыПоступления.Ссылка КАК Ссылка,
	|	ТаблицаСуммыПоступления.НомерСтроки КАК НомерСтроки,
	|	ТаблицаСуммыПоступления.Покупатель КАК Покупатель,
	|	ТаблицаСуммыПоступления.ДатаРеализации КАК ДатаРеализации,
	|	ТаблицаСуммыПоступления.ДокументРасчетов КАК ДокументРасчетов,
	|	ТаблицаСуммыПоступления.Услуга КАК Услуга,
...
Функция ТекстЗапросаРасчетыВВалютеСуммыПоступления()
	...
	|	ВременнаяТаблицаСуммыПоступления.ДокументРасчетов,
...
Функция ТекстЗапросаСписаниеТоваровКомитента(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Иначе
		...
		|	ТаблицаСуммаПоступления.ДокументРасчетов,
...
Функция ТекстЗапросаВознаграждение(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	Реквизиты.Дата КАК Период,
	|	ВЫБОР
	|		КОГДА Реквизиты.УдержатьВознаграждение
	|			ТОГДА ""УдержаниеВознаграждения""
	|		ИНАЧЕ ""ОтражениеВознаграждения""
	...
	|ВЫБРАТЬ
	|	ТаблицаУслуги.Ссылка КАК Ссылка,
	|	ТаблицаУслуги.СчетРасчетов КАК СчетРасчетов,
	|	ТаблицаУслуги.СчетАвансов КАК СчетАвансов,
...
Функция ТекстЗапросаПрослеживаемыеТоварыЗакупленные(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ВедетсяУчетПрослеживаемыхТоваров 
		ИЛИ Реквизиты.ВидОперации <> Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОЗакупках Тогда
		ПараметрыПроведения.Вставить("ПрослеживаемыеОперацииПоЗакупке", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ПрослеживаемыеОперацииПоЗакупке", НомераТаблиц.Количество());
	
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахТовары.ПрослеживаемыйКомплект
	|			ТОГДА ОтчетКомитентуОПродажахСведенияПрослеживаемости.Номенклатура
	|		ИНАЧЕ ОтчетКомитентуОПродажахТовары.Номенклатура
	|	КОНЕЦ КАК Номенклатура,
	|	СУММА(ОтчетКомитентуОПродажахСведенияПрослеживаемости.Количество) КАК Количество,
	|	ОтчетКомитентуОПродажахТовары.СтранаПроисхождения КАК СтранаПроисхождения,
	|	ОтчетКомитентуОПродажахСведенияПрослеживаемости.РНПТ КАК РНПТ,
	|	СУММА(ОтчетКомитентуОПродажахСведенияПрослеживаемости.КоличествоПрослеживаемости) КАК КоличествоПрослеживаемости,
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПередачаСведенийОЗакупкеКомитенту)
	|	КОНЕЦ КАК КодОперации,
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	|			ТОГДА ОтчетКомитентуОПродажахПоставщики.СчетФактура
	|		ИНАЧЕ ОтчетКомитентуОПродажахПоставщики.Ссылка
	|	КОНЕЦ КАК ДокументОперации,
	|	ОтчетКомитентуОПродажахТовары.Ссылка.Контрагент КАК Контрагент,
	|	НАЧАЛОПЕРИОДА(ОтчетКомитентуОПродажахПоставщики.ДатаСФ, КВАРТАЛ) КАК ПериодОперации,
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	|			ТОГДА """"
	|		ИНАЧЕ ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """")
	...
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ТипыДокументов.ОтчетКомитенту)
	|	КОНЕЦ КАК ТипДокументаВПрослеживаемости,
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахТовары.ПрослеживаемыйКомплект
	|			ТОГДА ОтчетКомитентуОПродажахСведенияПрослеживаемости.Сумма
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаТовары.СуммаРуб, 0) - ЕСТЬNULL(ТаблицаТовары.СуммаНДСРуб, 0)
	...
	|ИЗ
	|	Документ.ОтчетКомитентуОПродажах.Товары КАК ОтчетКомитентуОПродажахТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтчетКомитентуОПродажах.СведенияПрослеживаемости КАК ОтчетКомитентуОПродажахСведенияПрослеживаемости
	|		ПО ОтчетКомитентуОПродажахТовары.Ссылка = ОтчетКомитентуОПродажахСведенияПрослеживаемости.Ссылка
	|			И ОтчетКомитентуОПродажахТовары.ИдентификаторСтроки = ОтчетКомитентуОПродажахСведенияПрослеживаемости.ИдентификаторСтроки
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаТовары КАК ТаблицаТовары
	|		ПО ОтчетКомитентуОПродажахТовары.КлючСтроки = ТаблицаТовары.КлючСтроки
	|			И ОтчетКомитентуОПродажахТовары.НомерСтроки = ТаблицаТовары.НомерСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОтчетКомитентуОПродажахТовары.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И ОтчетКомитентуОПродажахТовары.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|ГДЕ
	|	ОтчетКомитентуОПродажахТовары.ПрослеживаемыйТовар
	|	И ОтчетКомитентуОПродажахТовары.Ссылка = &Ссылка
	|	И ОтчетКомитентуОПродажахПоставщики.ДатаСФ >= &ДатаНачалаУчетаПрослеживаемыхТоваров
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетКомитентуОПродажахСведенияПрослеживаемости.РНПТ,
	|	НАЧАЛОПЕРИОДА(ОтчетКомитентуОПродажахПоставщики.ДатаСФ, КВАРТАЛ),
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	|			ТОГДА ОтчетКомитентуОПродажахПоставщики.СчетФактура
	|		ИНАЧЕ ОтчетКомитентуОПродажахПоставщики.Ссылка
	...
	|	ОтчетКомитентуОПродажахТовары.СтранаПроисхождения,
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахПоставщики.ПолученСФ
	|			ТОГДА """"
	|		ИНАЧЕ ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """")
...
Функция ТекстЗапросаПрослеживаемыеТоварыРеализованные(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	
	Если Не Реквизиты.ВедетсяУчетПрослеживаемыхТоваров 
		ИЛИ Реквизиты.ВидОперации <> Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОПродажах Тогда
		ПараметрыПроведения.Вставить("ПрослеживаемыеОперацииПоПродаже", Неопределено);
		Возврат "";
	КонецЕсли;
	
	НомераТаблиц.Вставить("ВТ_ОтчетКомитентуОПродажах",      НомераТаблиц.Количество());
	НомераТаблиц.Вставить("СписаниеРНПТ",                    НомераТаблиц.Количество());
	НомераТаблиц.Вставить("СуммыПоРНПТ",                     НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВТ_Операции",                     НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ПрослеживаемыеОперацииПоПродаже", НомераТаблиц.Количество());
	...
	|	И ОтчетКомитентуОПродажахТовары.СуммаНДС = 0
	|	И НЕ ОтчетКомитентуОПродажахТовары.ДокументРасчетов.Ссылка ЕСТЬ NULL
	|	И ОтчетКомитентуОПродажахТовары.ПрослеживаемыйТовар = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВТ_ОтчетКомитентуОПродажах.Ссылка КАК ДокументОперации,
	|	ВТ_ОтчетКомитентуОПродажах.Дата КАК Дата,
	|	ВТ_ОтчетКомитентуОПродажах.ЭтоВозврат КАК ЭтоВозврат,
	|	ПрослеживаемыеТоварыДвижения.Организация КАК Организация,
	...
	|		ПО ВТ_ОтчетКомитентуОПродажах.ДокументРасчетов = ПрослеживаемыеТоварыДвижения.Регистратор
	|			И ВТ_ОтчетКомитентуОПродажах.Контрагент = ПрослеживаемыеТоварыДвижения.Комитент
	|			И ВТ_ОтчетКомитентуОПродажах.Номенклатура = ПрослеживаемыеТоварыДвижения.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ВТ_ОтчетКомитентуОПродажах.Дата,
	|	ВТ_ОтчетКомитентуОПродажах.ЭтоВозврат,
	|	ПрослеживаемыеТоварыДвижения.Организация,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОперацииСПрослеживаемымиТоварами.Регистратор КАК Регистратор,
	|	ОперацииСПрослеживаемымиТоварами.РНПТ КАК РНПТ,
	|	ОперацииСПрослеживаемымиТоварами.Номенклатура КАК Номенклатура,
	...
	|	ОперацииСПрослеживаемымиТоварами.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА СписаниеРНПТ.ЭтоВозврат
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПередачаКомиссионеромКомитентуСведенийОВозвратеТовара)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.КодыОперацийПрослеживаемости.ПередачаСведенийОРеализацииКомитенту)
	|	КОНЕЦ КАК КодОперации,
	...
	|	ЗНАЧЕНИЕ(Справочник.ТипыДокументов.ОтчетКомитенту) КАК ТипДокументаВПрослеживаемости,
	...
	|			И СписаниеРНПТ.Регистратор = СуммыПоРНПТ.Регистратор
	|			И СписаниеРНПТ.Номенклатура = СуммыПоРНПТ.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО СписаниеРНПТ.ДокументОперации = ДанныеПервичныхДокументов.Документ
	|			И СписаниеРНПТ.Организация = ДанныеПервичныхДокументов.Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СписаниеРНПТ.КоличествоПрослеживаемости КАК КоличествоПрослеживаемости,
	|	СписаниеРНПТ.ДокументОперации КАК ДокументОперации,
	|	СписаниеРНПТ.Дата КАК Дата,
	|	СписаниеРНПТ.ЭтоВозврат КАК ЭтоВозврат,
	|	СписаниеРНПТ.Организация КАК Организация,
	|	СписаниеРНПТ.Комплектующие КАК Комплектующие,
	|	СписаниеРНПТ.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект
	|ИЗ
	|	СписаниеРНПТ КАК СписаниеРНПТ";
...
Функция ТекстЗапросаРеализация(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаУслуги.ДокументРасчетов КАК ДокументРасчетов,
...
Функция ПодготовитьПараметрыСписаниеРеализованныхТоваровКомитента(ТаблицаТоварыКомитента, ТаблицаРеквизиты)
	...
	+ "ДокументРасчетов,"   // <ДокументСсылка> - Документ реализации
...
Процедура СформироватьДвиженияСписаниеРеализованныхТоваровКомитента(ТаблицаТоварыКомитента, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ТаблицаТоваров.Свернуть("Номенклатура, Услуга, Покупатель, ДатаРеализации, ДокументРасчетов",
	...
	Для Каждого Строка Из ТаблицаТоваров Цикл
		...
		// В документе КорректировкаРеализации исправили только сумму
		Если Строка.Количество = 0 И (НЕ Строка.Услуга)
			 И ТипЗНЧ(Строка.ДокументРасчетов) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		...
		ИначеЕсли Строка.Количество = 0 И (НЕ Строка.Услуга) Тогда
			
			Продолжить;
			
		ИначеЕсли Строка.Количество < 0 
			ИЛИ (Строка.Количество = 0 И Строка.Выручка < 0) Тогда
		...
		Для НомерФильтра = 1 По ?(ЗначениеЗаполнено(Строка.Покупатель), 2, 1) Цикл
			...
			Если ЗначениеЗаполнено(Строка.ДокументРасчетов) И НомерФильтра = 1 Тогда
				СтруктураОтбора.Вставить("ДокументРасчетов", Строка.ДокументРасчетов);
			КонецЕсли;
			...
			Для Каждого Запись Из НайденныеСтроки Цикл
				...
				Если Строка.Услуга Тогда
					...
					Если Списать > 0 Тогда
						...
						Движение.ДокументРасчетов     = Запись.ДокументРасчетов;
...
Функция ПолучитьОстаткиРеализованныеТоварыКомитентов(СтруктураПараметров)
	...
	|	РеализованныеТоварыКомитентовОстатки.ДокументРасчетов КАК ДокументРасчетов
	|ИЗ
	...
	|	РеализованныеТоварыКомитентовОстатки.ДокументРасчетов
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура,
	|	Покупатель,
...
Функция ТекстЗапросаДанныеДляПечатиСчетовФактур(НомераТаблиц) Экспорт
	...
	|	Реквизиты.УслугаПоВознаграждению.КодОКВЭД2
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НЕОПРЕДЕЛЕНО КАК НоменклатураКодТНВЭД,
	|	НЕОПРЕДЕЛЕНО КАК СсылкаОтчет,
	...
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВТ_КлючиСтрок.ДатаСФ < ДАТАВРЕМЯ(2021, 7, 1)
	|			ТОГДА ЕСТЬNULL(ВТ_ГТД.СтранаПроисхождения, ТаблицаТовары.Номенклатура.СтранаПроисхождения)
	|		ИНАЧЕ ТаблицаТовары.СтранаПроисхождения
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВТ_КлючиСтрок.ДатаСФ < ДАТАВРЕМЯ(2021, 7, 1)
	|			ТОГДА ЕСТЬNULL(ВТ_ГТД.СтранаПроисхождения, ТаблицаТовары.Номенклатура.СтранаПроисхождения.НаименованиеПолное)
	|		ИНАЧЕ ТаблицаТовары.СтранаПроисхождения.НаименованиеПолное
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВТ_КлючиСтрок.ДатаСФ < ДАТАВРЕМЯ(2021, 7, 1)
	|			ТОГДА ЕСТЬNULL(ВТ_ГТД.НомерГТД, ТаблицаТовары.Номенклатура.НомерГТД)
	|		ИНАЧЕ ТаблицаТовары.НомерГТД
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВТ_КлючиСтрок.ДатаСФ < ДАТАВРЕМЯ(2021, 7, 1)
	|			ТОГДА ЕСТЬNULL(ВТ_ГТД.НомерГТД.Представление, ТаблицаТовары.Номенклатура.НомерГТД.Представление)
	|		ИНАЧЕ ТаблицаТовары.НомерГТД.Представление
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ВТ_КлючиСтрок.ДатаСФ < ДАТАВРЕМЯ(2021, 7, 1)
	|			ТОГДА ЕСТЬNULL(ВТ_ГТД.НомерГТД.Представление, ТаблицаТовары.Номенклатура.НомерГТД.РегистрационныйНомер)
	...
	|	ТаблицаТовары.Номенклатура.КодТНВЭД,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетКомитентуОПродажахСведенияПрослеживаемости.РНПТ.Код КАК НомерТовара,
	|	ОтчетКомитентуОПродажахСведенияПрослеживаемости.Количество КАК КоличествоУчетное,
	|	ОтчетКомитентуОПродажахСведенияПрослеживаемости.КоличествоПрослеживаемости КАК Количество,
	...
	|	ВЫБОР
	|		КОГДА ОтчетКомитентуОПродажахСведенияПрослеживаемости.Сумма = 0
	|			ТОГДА ВЫБОР
	|					КОГДА ТаблицаДокумента.СуммаВключаетНДС
	|						ТОГДА ВЫРАЗИТЬ(ОтчетКомитентуОПродажахСведенияПрослеживаемости.Количество * (ТаблицаДокумента.Сумма - ТаблицаДокумента.СуммаНДС) / ТаблицаДокумента.Количество КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ ОтчетКомитентуОПродажахСведенияПрослеживаемости.Количество * ТаблицаДокумента.Цена
	|				КОНЕЦ
	|		ИНАЧЕ ОтчетКомитентуОПродажахСведенияПрослеживаемости.Сумма
	...
	|		ПО (ТаблицаДокумента.ИдентификаторСтроки = ОтчетКомитентуОПродажахСведенияПрослеживаемости.ИдентификаторСтроки)
	|			И ОтчетКомитентуОПродажахСведенияПрослеживаемости.Ссылка = ТаблицаДокумента.СсылкаОтчет";
...
Функция ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(ВерсияПостановленияНДС1137 = Неопределено, ЕстьПрослеживаемыеТовары = Ложь) Экспорт
	...
	|	ОтчетКомитентуОПродажах.ДоговорКонтрагента.ВидДоговора КАК ВидДоговора,
	...
	|			ОтчетКомитентуОПродажахСведенияПрослеживаемости.Ссылка КАК Ссылка,
	|			ИСТИНА КАК ЕстьПрослеживаемыеТовары
	|		ИЗ
	|			Документ.ОтчетКомитентуОПродажах.СведенияПрослеживаемости КАК ОтчетКомитентуОПродажахСведенияПрослеживаемости
	|		ГДЕ
	|			ОтчетКомитентуОПродажахСведенияПрослеживаемости.Ссылка В(&МассивОбъектов)) КАК ПрослеживаемыеТовары
	|		ПО ОтчетКомитентуОПродажах.Ссылка = ПрослеживаемыеТовары.Ссылка
	|ГДЕ
	|	ОтчетКомитентуОПродажах.Ссылка В(&МассивОбъектов)
	|	И ОтчетКомитентуОПродажах.Дата > &УсловиеПоДате
...
Пока ВыборкаДокументов.Следующий() Цикл
	...
	СведенияОбОрганизации    = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(ВыборкаДокументов.Организация, ВыборкаДокументов.Дата, , , , Истина);
...
Если НЕ ОтчетНапечатан Тогда 
	...
	СведенияОбОрганизации    = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(Шаблон.Организация, Шаблон.Дата, , , , Истина);
	ПредставлениеОрганизации = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,");
...
Функция ПечатьАкта(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПериодичностьУслуг") Тогда
			...
			ДанныеПечатиНоменклатуры.Вставить("Товар", РаботаСНоменклатуройКлиентСерверБП.СодержаниеУслуги(
		...
		ДанныеПечатиИтогоНДС = ПечатьТорговыхДокументов.ДанныеПечатиИтоговНДС(
			ИтогоНДС, 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Шапка.СтавкаНДСВознаграждения),
