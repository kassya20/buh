...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// Дополним содержание УСН сведениями по НДС
	Если ПараметрыУСН.ПрименениеУСНДоходы Тогда
		СуммаИсключаемогоНДС = Товары.Итог("СуммаНДСВознаграждения");
		Если СуммаИсключаемогоНДС > 0 Тогда
			Содержание_УСН = СтрШаблон(
				НСтр("ru = '%1 НДС в сумме %2 руб. исключен из доходов.'"),
				Содержание_УСН,
				Формат(СуммаИсключаемогоНДС, "ЧДЦ=2; ЧРГ=' '"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ПараметрыПроведения.Реквизиты) Тогда
		ПрослеживаемостьБП.СформироватьДвиженияОперацииСПрослеживаемымиТоварами(
			ПараметрыПроведения.ПрослеживаемыеОперацииПоПродаже,
			ПараметрыПроведения.Реквизиты[0],
			Движения);
		ПрослеживаемостьБП.СформироватьДвиженияОперацииСПрослеживаемымиТоварами(
			ПараметрыПроведения.ПрослеживаемыеОперацииПоЗакупке,
			ПараметрыПроведения.Реквизиты[0],
			Движения);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Если НЕ ЭтоУслуга И НЕ ЗначениеЗаполнено(СтрокаТаблицы.Количество) 
			И НЕ ТипЗНЧ(СтрокаТаблицы.ДокументРасчетов) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
			ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Колонка", "Заполнение",
		...
		Если ВидОперации = Перечисления.ВидыОперацийОтчетКомитентуОПродажах.ОтчетОЗакупках // отчет о продажах не содержит РНПТ
			И СтрокаТаблицы.ПрослеживаемыйТовар 
			...
			Если СтрокаТаблицы.ПрослеживаемыйКомплект И МассивСтрокСведенияПрослеживаемости.Количество() = 0 Тогда
				ТекстСообщенияРасшифровка = НСтр("ru = 'Данные по РНПТ не заполнены'");
...
Процедура ЗаполнитьПоДокументуПоступление(ДанныеЗаполнения)
	...
	СтавкаНДСПриРеализации = УчетНДСКлиентСервер.СтавкаНДСПриРеализации(Дата, Организация);
	...
	Пока ВыборкаПоПоставщикам.Следующий() Цикл
		...
		Пока ВыборкаПоПартиям.Следующий() Цикл
			...
			Для каждого СтрокаТовары Из ТаблицаТовары Цикл
				...
				ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧастиТовары, СтрокаТовары);
				// Дополним сведениями об импортном товаре.
				СтруктураОтбора = Новый Структура("Партия, ПрослеживаемыйТовар, Номенклатура");
				ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаТабличнойЧастиТовары);
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					СтрокаТабличнойЧастиТовары.СтранаПроисхождения = НайденныеСтроки[0].СтранаПроисхождения;
					Если СтрокаТабличнойЧастиТовары.ПрослеживаемыйТовар Тогда
						Если Не ЗначениеЗаполнено(СтрокаТабличнойЧастиТовары.ИдентификаторСтроки) Тогда
							СтрокаТабличнойЧастиТовары.ИдентификаторСтроки = Новый УникальныйИдентификатор;
						КонецЕсли;
						Для Каждого СтрокаСРНПТ Из НайденныеСтроки Цикл
							СтрокаСведенийПрослеживаемости = СведенияПрослеживаемости.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаСведенийПрослеживаемости, СтрокаСРНПТ, "РНПТ, Количество, КоличествоПрослеживаемости");
							СтрокаСведенийПрослеживаемости.ИдентификаторСтроки = СтрокаТабличнойЧастиТовары.ИдентификаторСтроки;
						...
						КонецЦикла;
					Иначе
						СтрокаТабличнойЧастиТовары.НомерГТД = НайденныеСтроки[0].НомерГТД;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Если НЕ ПолучитьФункциональнуюОпцию("ОсуществляетсяРеализацияТоваровУслугКомитентов")
			И НЕ ПолучитьФункциональнуюОпцию("ОсуществляетсяЗакупкаТоваровУслугДляКомитентов") Тогда
			ВызватьИсключение НСтр("ru = 'Отчет комитенту вводится на основании документа ""Поступление (акт, накладная, УПД)"",
		...
		ИначеЕсли РеквизитыДоговора.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПоставщиком
			...
			Если МассивДоговоров.Количество() = 0 Тогда
				...
				ВызватьИсключение НСтр("ru = 'Отчет комитенту не может быть введен на основании документа ""Поступление (акт, накладная, УПД)"",
		...
		Иначе
			...
			СообщениеОбОщибке = НСтр("ru = 'Отчет комитенту не может быть введен на основании документа ""Поступление (акт, накладная, УПД)"",
				|в котором указан договор ""%1"".'");
		
			ВызватьИсключение СтрШаблон(СообщениеОбОщибке, РеквизитыДоговора.ВидДоговора);
