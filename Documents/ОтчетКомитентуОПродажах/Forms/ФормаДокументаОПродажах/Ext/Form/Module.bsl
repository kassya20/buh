...
&НаКлиенте
Процедура ТоварыДокументРасчетовПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;

	ЗаполнитьДанныеПоТекущейСтроке(ТекущиеДанные.НомерСтроки-1);
...
Процедура ЗаполнитьРеализованнымПоДоговору(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru = 'Перед заполнением табличная часть будет очищена%ПредупреждениеОЗаписиДокумента% Заполнить?'") ;
	ИначеЕсли Модифицированность Тогда
		ТекстВопроса = НСтр("ru = ' Перед заполнением документ будет записан! Продолжить?'") ;
	Иначе
	...
	ТекстВопроса = СтрЗаменить(ТекстВопроса, "%ПредупреждениеОЗаписиДокумента%", 
		?(Модифицированность, Символы.ПС + НСтр("ru=' (документ будет записан!)'"), "."));
...
Процедура ЗаполнитьРассчитатьСуммыВСтроке(Строка, ТаблицаЦенНоменклатуры, ВалютаДоИзменения, СтруктураКурса, ПерезаполнитьЦены, ПересчитатьЦены, ПересчитатьНДС, ЕстьНДС, ЗначениеПустогоКоличества)
	...
	Иначе
		Если ПересчитатьЦены Тогда
			...
			ЦенаПоступления = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
				ЦенаПоступления,
				ВалютаДоИзменения, Объект.ВалютаДокумента, 
				СтруктураКурса.Курс, Объект.КурсВзаиморасчетов, 
				СтруктураКурса.Кратность, Объект.КратностьВзаиморасчетов);
			
			Строка.СуммаВознаграждения  = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
...
Процедура УстановитьПараметрыВыбораСтавкиНДС()
	...
	УчетНДС.УстановитьПараметрыВыбораСтавкиНДС(ЭтотОбъект, "СтавкаНДСВознаграждения");
...
Процедура ПриИзмененииТоварыНоменклатураНаСервере(СтрокаТабличнойЧасти, Знач ИмяТабличнойЧасти, Знач Объект, Знач ВалютаРегламентированногоУчета)
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		СтрокаТабличнойЧасти.ПрослеживаемыйТовар = ПараметрыНоменклатуры.ПрослеживаемыйТовар
			Или ПараметрыНоменклатуры.ПрослеживаемыйКомплект;
	Иначе
...
Процедура УстановитьУсловноеОформлениеТовары() Экспорт
	...
	// ТоварыДокументРасчетов
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыДокументРасчетов");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Товары.ПрослеживаемыйТовар", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Товары.ДокументРасчетов", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", Истина);
...
Процедура ЗаполнитьОтражениеВУСН(Форма)
	...
	Если ЗаполнятьОтражениеВУСН Тогда 
		...
		СуммаДляКУДиР = Объект.Товары.Итог("СуммаВознаграждения")
			- ?(Объект.СуммаВключаетНДС, Объект.Товары.Итог("СуммаНДСВознаграждения"), 0);
		...
		Объект.Содержание_УСН = НСтр("ru = 'Комиссионное вознаграждение удержано из выручки комитента.'");
...
Процедура ЗаполнитьПоДоговоруНаСервере()
	...
	|	ВЫРАЗИТЬ(ХозрасчетныйОборотыДтКт.СубконтоДт1 КАК Справочник.Номенклатура).ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ВЫРАЗИТЬ(ХозрасчетныйОборотыДтКт.СубконтоДт1 КАК Справочник.Номенклатура).ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
	|	1 КАК Коэффициент,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			СтрокаТабличнойЧасти.ПрослеживаемыйТовар = Выборка.ПрослеживаемыйТовар
				Или Выборка.ПрослеживаемыйКомплект;;
		Иначе
			СтрокаТабличнойЧасти.ПрослеживаемыйТовар    = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьРеализованнымНаСервере()
	...
	|	СУММА(РеализованныеТоварыКомитентовОстатки.КоличествоОстаток) КАК Количество,
	|	СУММА(РеализованныеТоварыКомитентовОстатки.СуммаПоступленияОстаток) КАК СуммаПоступления,
	|	СУММА(РеализованныеТоварыКомитентовОстатки.ВыручкаОстаток) КАК Сумма,
	|	СУММА(РеализованныеТоварыКомитентовОстатки.СуммаНДСОстаток) КАК СуммаНДС,
	...
	|ИМЕЮЩИЕ
	|	(ЕСТЬNULL(РеализованныеТоварыКомитентовОстатки.Номенклатура.Услуга, ИСТИНА)
	|		ИЛИ СУММА(РеализованныеТоварыКомитентовОстатки.КоличествоОстаток) <> 0)";

	ТаблицаТовары = Запрос.Выполнить().Выгрузить();
	Объект.Товары.Загрузить(ТаблицаТовары);
	
	// Отдельно получим даты разных типов документов.
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		
		ДокументыРасчетов = ТаблицаТовары.Скопировать(, "ДокументРасчетов").ВыгрузитьКолонку("ДокументРасчетов");
		КоличествоДокументов = ДокументыРасчетов.Количество();
		Для ИндексСтроки = 1 По КоличествоДокументов Цикл
			Если Не ЗначениеЗаполнено(ДокументыРасчетов[КоличествоДокументов - ИндексСтроки]) Тогда
				ДокументыРасчетов.Удалить(КоличествоДокументов - ИндексСтроки);
			КонецЕсли;
		КонецЦикла;
		ДатыДокументовРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ДокументыРасчетов, "Дата");
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		...
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			...
			ДатаРасчетов = ?(ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументРасчетов),
				ДатыДокументовРасчетов[СтрокаТабличнойЧасти.ДокументРасчетов],
				СтрокаТабличнойЧасти.ДатаРеализации);
			РеквизитыТекущейНоменклатуры = Справочники.Номенклатура.ПризнакПрослеживаемостиНаДату(СтрокаТабличнойЧасти.Номенклатура, ДатаРасчетов);
		...
		Если УчетАгентскогоНДС 
			ИЛИ (НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.СуммаНДС) И Не ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Объект.Дата)) Тогда
			// Ресурс "Сумма НДС" может быть не заполнен только в документах, созданных в версии 3.0.42 и ранее
			// В документах, начиная с 1 июля 2021 года считаем сумму заполненной если отгрузка товаров комитента-продавца производилась с НДС
			// и не заполненной, если отгрузка производилась без начисления НДС (комитент-продавец применяет УСН или освобождение от НДС.
			СтрокаТабличнойЧасти.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
				СтрокаТабличнойЧасти.Сумма,
		...
		СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.Сумма - ?(Объект.СуммаВключаетНДС, 0, СтрокаТабличнойЧасти.СуммаНДС);
		Если СтрокаТабличнойЧасти.Количество = 0 Тогда
			СтрокаТабличнойЧасти.ЦенаПоступления  = ?(СтрокаТабличнойЧасти.ЭтоУслуга, СтрокаТабличнойЧасти.СуммаПоступления, 0);
			СтрокаТабличнойЧасти.Цена             = ?(СтрокаТабличнойЧасти.ЭтоУслуга, СтрокаТабличнойЧасти.Сумма, 0);
		Иначе
...
Процедура ЗаполнитьПоПоступлениюНаСервере(Поступление)
	...
	|	ПоступлениеТоваровУслугТовары.Номенклатура.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	ПоступлениеТоваровУслугТовары.Номенклатура.ПрослеживаемыйКомплект КАК ПрослеживаемыйКомплект,
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	ОтслеживаютсяПрослеживаемыеТовары = ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Объект.Дата);
	ВедетсяУчетПрослеживаемыхТоваров = 
		ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
		И ОтслеживаютсяПрослеживаемыеТовары;
...
// Процедура, в которой в текущей строке таблицы Товары по документу расчетов заполняется дата реализации
//
// Параметры:
// НомерСтроки - Число -  номер текущей строки
//
&НаСервере
Процедура ЗаполнитьДанныеПоТекущейСтроке(НомерСтроки)
	...
	ТекущиеДанные.ДатаРеализации = ОпределитьДатуПоДокументуРасчетов(Документ);

КонецПроцедуры

// Функция возвращает дату реализации
//
// Параметры:
// Документ - Документ, по которому необходимо определить дату реализации
//
// Возвращаемое значение:
// Дата - дата реализации
//
&НаСервереБезКонтекста
Функция ОпределитьДатуПоДокументуРасчетов(Документ)

	ДатаРеализации = '20010101';
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		ИЛИ ТипЗнч(Документ) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
		...
		Сделка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "Сделка");
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		...
		ДокументРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "ДокументРеализации");
		Если ЗначениеЗаполнено(ДокументРеализации) Тогда
			
			ДатаРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализации, "Дата");
		
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат ДатаРеализации;
