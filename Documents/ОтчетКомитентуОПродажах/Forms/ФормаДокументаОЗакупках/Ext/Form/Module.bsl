...
Процедура ТоварыПриАктивизацииСтроки(Элемент)
	...
	Если ТекущаяКолонкаТаблицы <> Неопределено
		И ТекущаяКолонкаТаблицы.Имя = "ТоварыРНПТ" 
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураСтроки = ПолучитьСтруктуруДанныхСтроки(ТекущиеДанныеТовары, "Товары");
	...
	СтруктураСтроки.Вставить("НомерГТД", ТекущиеДанныеТовары.НомерГТД);
	СтруктураСтроки.Вставить("ИдентификаторСтроки", ТекущиеДанныеТовары.ИдентификаторСтроки);
	СтруктураСтроки.Вставить("РНПТ", ТекущиеДанныеТовары.РНПТ);
	СтруктураСтроки.Вставить("ПодсказкаРНПТ", ТекущиеДанныеТовары.ПодсказкаРНПТ);
	СтруктураСтроки.Вставить("ПрослеживаемыйКомплект", ТекущиеДанныеТовары.ПрослеживаемыйКомплект);
	
	ПриИзмененииТоварыНоменклатураНаСервере(СтруктураСтроки);

	ЗаполнитьЗначенияСвойств(ТекущиеДанныеТовары, СтруктураСтроки, , "НомерСтроки" );
	
	ПересчитатьСуммуСтроки(ТекущиеДанныеТовары, "Товары");
	
	Если ТекущиеДанныеТовары <> Неопределено Тогда
		// Необходимо очистить дополнительные сведения
		НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
		НоваяСтрока.ИдентификаторСтроки = ТекущиеДанныеТовары.ИдентификаторСтроки;
	КонецЕсли;
...
&НаСервере
Процедура ПриИзмененииТоварыНоменклатураНаСервере(СтрокаТабличнойЧасти)
	...
	Если ЗаполнятьСведенияОбИмпортномТоваре Тогда
		СтрокаТабличнойЧасти.НомерГТД            = ПараметрыНоменклатуры.НомерГТД;
		СтрокаТабличнойЧасти.СтранаПроисхождения = ПараметрыНоменклатуры.СтранаПроисхождения;
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			...
			СтрокаТабличнойЧасти.ПрослеживаемыйКомплект = ПараметрыНоменклатуры.ПрослеживаемыйКомплект;
		Иначе
...
Процедура УстановитьУсловноеОформлениеВидимость()
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"ЗаполнятьСведенияОбИмпортномТоваре", ВидСравненияКомпоновкиДанных.Равно, Ложь);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
...
Процедура УстановитьУсловноеОформлениеТовары() Экспорт
	...
	// Недоступные страна проихождения и РНПТ для услуг
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыСтранаПроисхождения");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыРНПТ");
...
Процедура ДанныеПоставщикиУстановитьОтборСтрок()
	...
	Если Элементы.Поставщики.ТекущаяСтрока = Неопределено
		И Объект.Поставщики.Количество() > 0 Тогда
		// Установим текущей первую строку.
		Элементы.Поставщики.ТекущаяСтрока = Объект.Поставщики[0].ПолучитьИдентификатор();
	КонецЕсли;
	...
	Если ТекДанныеПоставщики <> Неопределено Тогда
		...
		КонецЕсли;
		ЗаполнятьСведенияОбИмпортномТовареДо = ЗаполнятьСведенияОбИмпортномТоваре;
		Если ЗначениеЗаполнено(ТекДанныеПоставщики.ДатаСФ) Тогда
			ЗаполнятьСведенияОбИмпортномТоваре = ТекДанныеПоставщики.ДатаСФ >= ДатаНачалаУчетаПрослеживаемыхТоваров;
			Если ЗаполнятьСведенияОбИмпортномТовареДо <> ЗаполнятьСведенияОбИмпортномТоваре Тогда
				НастройкиУсловногоОформления.Вставить("ТоварыПроинициализировано", Ложь);
				ОбновитьУсловноеОформление(ЭтотОбъект);
			КонецЕсли;
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьЗакупленнымиНаСервере(СпособЗаполнения = "ЗаполнитьЗакупленнымиПоДоговору", Поступление = Неопределено)
	...
	ЗакупленныеТовары = Документы.ОтчетКомитентуОПродажах.ПолучитьЗакупленные(СтруктураПараметров);
	РезультатЗапроса = ЗакупленныеТовары[3];
	...
	ДопИнформацияОбИмпортныхТоварах = ЗакупленныеТовары[4].Выгрузить();
	ДопИнформацияОбИмпортныхТоварах.Индексы.Добавить("Партия, ПрослеживаемыйТовар, Номенклатура");
		...
		Пока ВыборкаПоПартиям.Следующий() Цикл
			...
			Пока ВыборкаПоНоменклатуре.Следующий() Цикл
				...
				Пока ВыборкаПоСтрокам.Следующий() Цикл
					...
					// Дополним сведениями об импортном товаре.
					СтруктураОтбора = Новый Структура("Партия, ПрослеживаемыйТовар, Номенклатура, ПрослеживаемыйКомплект");
					ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаТабличнойЧасти);
					СтруктураОтбора.Партия = ВыборкаПоПартиям.Партия;
					НайденныеСтроки = ДопИнформацияОбИмпортныхТоварах.НайтиСтроки(СтруктураОтбора);
					Если НайденныеСтроки.Количество() > 0 Тогда
						СтрокаТабличнойЧасти.СтранаПроисхождения = НайденныеСтроки[0].СтранаПроисхождения;
						Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
							...
							Для Каждого СтрокаСРНПТ Из НайденныеСтроки Цикл
								СтрокаСведенийПрослеживаемости = Объект.СведенияПрослеживаемости.Добавить();
								ЗаполнитьЗначенияСвойств(СтрокаСведенийПрослеживаемости, СтрокаСРНПТ, "РНПТ, Количество, КоличествоПрослеживаемости");
								СтрокаСведенийПрослеживаемости.ИдентификаторСтроки = СтрокаТабличнойЧасти.ИдентификаторСтроки;
								Если СтрокаСРНПТ.ПрослеживаемыйКомплект Тогда
									СтрокаСведенийПрослеживаемости.Номенклатура = СтрокаСРНПТ.Комплектующие;
									СтрокаСведенийПрослеживаемости.Сумма = СтрокаСРНПТ.СуммаБезНДС;
								КонецЕсли;
							КонецЦикла;
						Иначе
							СтрокаТабличнойЧасти.НомерГТД = НайденныеСтроки[0].НомерГТД;
						КонецЕсли;
					КонецЕсли;
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	// С даты вступления в силу закона о прослеживаемости заполняем в документе сведения о таможенной декларации или РНПТ.
	ЗаполнятьСведенияОбИмпортномТоваре = ОтслеживаютсяПрослеживаемыеТовары;
...
Процедура УдалитьСвязанныеЗаписиНаСервере()
	...
	// Очистим товары, связанные с поставщиками.
	Для каждого СтрокаКлюча Из ТаблицаУдаленныхКлючей Цикл
		...
		Для каждого СтрокаТЧ Из МассивСтрокТовары Цикл
			// Добавим к удалению связанные с товарами строки со сведениями прослеживаемости.
			КлючПоиска = Новый Структура("ИдентификаторСтроки", СтрокаТЧ.ИдентификаторСтроки);
			СтрокиСведенийПрослеживаемостиКУдалению = ТаблицаУдаленныхСтрок.НайтиСтроки(КлючПоиска);
			Если СтрокиСведенийПрослеживаемостиКУдалению.Количество() = 0 Тогда
				НоваяСтрока = ТаблицаУдаленныхСтрок.Добавить();
	...
	// Очистим сведения о прослеживаемости, связанные с товарами.
