...
&НаСервере
Процедура УстановитьПараметрыВыбораОсновногоНачисления(ЯвляетсяКатегориейОплатыДолиРКСН, КатегорияДохода) Экспорт
	
	Если Не ЯвляетсяКатегориейОплатыДолиРКСН 
		ИЛИ Не ЗначениеЗаполнено(КатегорияДохода) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		Если Не МодульРасчетЗарплатыДляНебольшихОрганизаций.ПрименяетсяАУСН() Тогда
			Возврат;
		КонецЕсли;
		КатегорииДоходов = Перечисления.КатегорииДоходовНДФЛ.ДоходыАУСН();
		Если КатегорииДоходов.Найти(КатегорияДохода) = Неопределено Тогда
			КатегорииДоходов.Очистить();
			КатегорииДоходов.Добавить(Перечисления.КатегорииДоходовНДФЛ.ПрочиеДоходыВДенежнойФормеОтТрудовойДеятельности);
		Иначе
			КатегорииДоходов.Очистить();
			КатегорииДоходов.Добавить(Перечисления.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН);
			КатегорииДоходов.Добавить(Перечисления.КатегорииДоходовНДФЛ.ПрочиеДоходыАУСН);
		КонецЕсли;
		
		ПараметрВыбора = Новый ПараметрВыбора("Отбор.КатегорияДохода", Новый ФиксированныйМассив(КатегорииДоходов));
...
Процедура УстановитьНачальноеОтображениеЭлементовУправления()
	...
	ЯвляетсяКатегориейОплатыДолиРКСН = Объект.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке
											Или Объект.КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиСевернойНадбавкиВСреднемЗаработке;
	УстановитьПараметрыВыбораОсновногоНачисления(ЯвляетсяКатегориейОплатыДолиРКСН, Объект.КатегорияДохода);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ОсновнойВидРасчета",
		"Видимость",
		ЯвляетсяКатегориейОплатыДолиРКСН);
...
Процедура КодДоходаНДФЛПриИзмененииНаСервере()
	...
	УстановитьСвойстваКатегорииДоходаНДФЛ(ЭтотОбъект);
	УчетНДФЛВызовСервера.СтрокаРаздела2Расчета6НДФЛПоКодуДохода(Объект.КодДоходаНДФЛ);
