...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ПометкаУдаления
		И Не ВАрхиве
		И ЗначениеЗаполнено(ОсновнойВидРасчета)
		И (КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке
			Или КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиСевернойНадбавкиВСреднемЗаработке) Тогда
		Отбор = Новый Структура;
		Отбор.Вставить("КатегорияНачисленияИлиНеоплаченногоВремени", КатегорияНачисленияИлиНеоплаченногоВремени);
		Отбор.Вставить("ОсновнойВидРасчета", ОсновнойВидРасчета);
		Отбор.Вставить("КатегорияДохода", КатегорияДохода);
		...
		Если Начисления.Количество() > 0 И Начисления.Найти(Ссылка) = Неопределено Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				КатегорияНачисленияИлиНеоплаченногоВремени,
				ОсновнойВидРасчета);
			СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ЭтотОбъект, "ОсновнойВидРасчета", ТекстСообщения);
		КонецЕсли;
	КонецЕсли;
	...
	Если КатегорияНачисленияИлиНеоплаченногоВремени = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаДолиРайонногоКоэффициентаВСреднемЗаработке
		...
		Если ЗначениеЗаполнено(ОсновнойВидРасчета) Тогда
			ВидОперацииПоЗарплате = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойВидРасчета, "ВидОперацииПоЗарплате");
		ИначеЕсли Не ЗначениеЗаполнено(ВидОперацииПоЗарплате) Тогда
			ВидОперацииПоЗарплате = Перечисления.ВидыОперацийПоЗарплате.НачисленоДоход;
		КонецЕсли;
	ИначеЕсли ЯвляетсяДоходомВНатуральнойФорме И ВидОперацииПоЗарплате <> Перечисления.ВидыОперацийПоЗарплате.НатуральныйДоход Тогда
