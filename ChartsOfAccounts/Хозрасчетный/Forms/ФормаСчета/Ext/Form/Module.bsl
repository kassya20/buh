...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьВидимостьПредупрежденияУчетПоПродукции();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменениеНастройкиПланаСчетов" Тогда
		...
		Прочитать();
		Если Элементы.ВидыСубконтоЗапрещенногоСчета.Видимость Тогда
...
Функция ОграничениеСпискаСубконто(Знач Счет = Неопределено)
	...
	УчетЗапасов[8] = ПланыСчетов.Хозрасчетный.ВозвратнаяТара;                         // 012
	...
	УчетДвиженияДенежныхСредств = Новый Массив(7);
	...
	УчетДвиженияДенежныхСредств[3] = ПланыСчетов.Хозрасчетный.ЦифровойРубль;    // 53
	УчетДвиженияДенежныхСредств[4] = ПланыСчетов.Хозрасчетный.СпециальныеСчета; // 55
	УчетДвиженияДенежныхСредств[5] = ПланыСчетов.Хозрасчетный.ПереводыВПути;    // 57.01
	УчетДвиженияДенежныхСредств[6] = ПланыСчетов.Хозрасчетный.ПереводыВПутиВал; // 57.21
	...
	УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев = Новый Массив(2);
	УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев[0] = ПланыСчетов.Хозрасчетный.ПлатежиПоДобровольномуСтрахованиюРаботников;               // 76.01.2
	УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев[1] = ПланыСчетов.Хозрасчетный.РасходыБудущихПериодовНаДобровольноеСтрахованиеРаботников; // 97.02
	УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев = БухгалтерскийУчет.СформироватьМассивСубсчетов(УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев);
	
	УчетРемонтовОсновныхСредств = Новый Массив(2);
	УчетРемонтовОсновныхСредств[0] = ПланыСчетов.Хозрасчетный.ЗатратыНаРемонтОС;           // 01.05
	УчетРемонтовОсновныхСредств[1] = ПланыСчетов.Хозрасчетный.АмортизацияЗатратНаРемонтОС; // 02.05
	
	СчетПрочиеНалогиИСборы = ПланыСчетов.Хозрасчетный.ПрочиеНалогиИСборы; // 68.10
	АналитикаСчетаПрочиеНалогиИСборы = БухгалтерскийУчет.АналитикаСчета(СчетПрочиеНалогиИСборы);
	УчетПрочихНалоговИСборов = Неопределено; 
	Если АналитикаСчетаПрочиеНалогиИСборы.Получить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыНалогов) <> Неопределено Тогда
		УчетПрочихНалоговИСборов = Новый Массив(1);
		УчетПрочихНалоговИСборов[0] = СчетПрочиеНалогиИСборы;
	КонецЕсли;
	...
	ЗапрещенныеСчета.Вставить("УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев",     УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев);
	ЗапрещенныеСчета.Вставить("УчетРемонтовОсновныхСредств",                      УчетРемонтовОсновныхСредств);
	Если ЗначениеЗаполнено(УчетПрочихНалоговИСборов) Тогда
		ЗапрещенныеСчета.Вставить("УчетПрочихНалоговИСборов", УчетПрочихНалоговИСборов);
	КонецЕсли;
...
Функция ПолучитьСсылкуНастройкиСубконто(ИдентификаторОграниченияСпискаСубконто)
	...
	ИначеЕсли ИдентификаторОграниченияСпискаСубконто = "УчетРасчетовПоСтрахованиюОтНесчастныхСлучаев" Тогда
		ЗаголовокФормы = НСтр("ru='Учет расчетов по страхованию от несчастных случаев'");
	Иначе
...
Процедура УстановитьДоступностьРедактированияВидовСубконто(ИдентификаторОграниченияСпискаСубконто = "")
	...
	Если ИдентификаторОграниченияСпискаСубконто = "УчетРемонтовОсновныхСредств"
			Или ИдентификаторОграниченияСпискаСубконто = "УчетПрочихНалоговИСборов" Тогда
		Элементы.ПредупреждениеЗапрещенногоСчета.Заголовок = Новый ФорматированнаяСтрока(
			НСтр("ru='   Состав видов субконто на этом счете не редактируется.'"));
	ИначеЕсли Не МожноРедактировать Тогда
		Элементы.ПредупреждениеЗапрещенногоСчета.Заголовок = Новый ФорматированнаяСтрока(
...
Процедура УстановитьВидимостьПредупрежденияУчетПоПодразделениям()
	...
	СчетаЗатрат.Добавить(ПланыСчетов.Хозрасчетный.БракВПроизводстве);           // 28
	СчетаЗатрат.Добавить(ПланыСчетов.Хозрасчетный.ОбслуживающиеПроизводства);   // 29
	СчетаЗатрат.Добавить(ПланыСчетов.Хозрасчетный.РасходыНаПродажу);            // 44
...
&НаСервере
Процедура УстановитьВидимостьПредупрежденияУчетПоПродукции()
	
	ВидимостьПредупреждения = Объект.Ссылка = ПланыСчетов.Хозрасчетный.ОсновноеПроизводство
		Или Объект.Ссылка = ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства;
		
	Элементы.ПредупреждениеУчетПоПродукции.Видимость = ВидимостьПредупреждения;
