...
Процедура ПередЗаписью(Отказ)
	...
	// Проверка неиспользуемых счетов.
	// У таких счетов ИмяПредопределенныхДанных начинается с Удалить...
	// Поэтому требуется скорректировать реквизиты:
	//   - Порядок = "Я";
	//   - КодБыстрогоВыбора - начинается с "Я".
	Если ЗначениеЗаполнено(ИмяПредопределенныхДанных) И СтрНачинаетсяС(СокрЛП(ИмяПредопределенныхДанных), НСтр("ru='Удалить'")) Тогда
		Порядок = НСтр("ru='Я'");
		КодБыстрогоВыбора = СокрЛП(СтрЗаменить(Код, ".", ""));
	КонецЕсли;
