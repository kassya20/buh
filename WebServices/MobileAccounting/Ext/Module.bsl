...
Функция ConfirmGettingFile(УзелОбменаКод, НомерСообщения, СообщениеОбОшибке)
	...
	УзелОбъект.ПервичнаяИнициализацияВыполнена = Истина;
	УзелОбъект.Записать();
КонецФункции
...
Функция SendTaxNotificationToEmail(ID, ErrorMessage)
	Возврат МобильнаяБухгалтерия.ОтправитьУведомлениеНаEmail(ID, ErrorMessage);
КонецФункции
...
Функция RegTransferConfigStart(ИдентификаторАбонента, ИдентификаторДокументооборота, ИдентификаторСертификата, СообщениеОбОшибке)
	...
	Возврат МобильнаяБухгалтерия.НачатьНастройкуОтчетности(ИдентификаторАбонента, ИдентификаторДокументооборота, ИдентификаторСертификата, СообщениеОбОшибке);
...
Функция RegTransferConfigFinish(ИдентификаторАбонента, ИдентификаторДокументооборота, ИдентификаторСертификата, ВременныйПароль, СообщениеОбОшибке)
	...
	Возврат МобильнаяБухгалтерия.ЗавершитьНастройкуОтчетности(ИдентификаторАбонента, ИдентификаторДокументооборота, ИдентификаторСертификата, ВременныйПароль, СообщениеОбОшибке);
...
Функция TaxPayRegistration(Docs, Tax, ErrorMessage)
	...
	Для Каждого ДокументОплаты Из МассивДокументов Цикл
		...
		Если Не ЗначениеЗаполнено(ДокументСсылка)
			Или Не ЗначениеЗаполнено(Правило) Тогда
			Продолжить;
		КонецЕсли;
		
		ЭтоУведомление = ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов")
			Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОперацияПоЕдиномуНалоговомуСчету")
			Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.УведомлениеОбУменьшенииНалогаПоПатенту");
...
Функция AdvancedMode(УзелОбменаКод, ПроверятьАктуальностьЗакрытиеМесяца)
	...
	//Актуализацию нужно запланировать при любом режиме расчета налога
	//Выполняется только в разделенном режиме. Иначе приводит к вызову исключения при обращении к очереди заданий
	Если ПроверятьАктуальностьЗакрытиеМесяца
		И ОбщегоНазначения.РазделениеВключено() Тогда
		МобильнаяБухгалтерия.ПроверитьАктуализироватьЗакрытиеМесяца(РеквизитыУзла.Организация, ТекущаяДата());
	КонецЕсли;
	
	//Если при подключении в СоздатьУзелОбмена был установлен расчет в БП - сохраним режим в МБ
	//Для изменения настройки, полученной при подключении, нужно отключить ф-ть и заново подключить мобильное
	Если РеквизитыУзла.РасчетНалоговВБП Тогда
		Возврат Истина;
	КонецЕсли;
	
	ТребуетсяРасчетНалоговВБП = МобильнаяБухгалтерия.РасширеннаяФункциональность(РеквизитыУзла.Организация,
		ТекущаяДата(), Ложь);
	
	//Если в процессе работы включили расширенная ф-ть - учтем в настройках БП и передадим в МБ команду меняться
	//При отключении лишней ф-ти не можем включить Расчет в МБ, потому что часть документов в МБ не отражена в учете
	//Для изменения настройки нужно отключить лишнюю ф-ть и заново подключить мобильное
	Если ТребуетсяРасчетНалоговВБП Тогда
...
Функция RegDocOptions(ВидОтчета, ПолноеИмяПравила, КонецПериода)
	...
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(Неопределено, Ложь) Тогда
		ПараметрыОтчета.Вставить("ПечатьСоШтрихкодомPDF417НаСервере", Ложь);
	КонецЕсли; 
...
Функция AllowedOrganisation()
	...
	Если МобильнаяБухгалтерия.ИспользованиеМобильнойБухгалтерииОграниченоТарифом() Тогда
...
Функция GetGoodsBalances(Parameters)
	Возврат МобильнаяБухгалтерия.ОстаткиНоменклатуры(Parameters);
КонецФункции
...
Функция GetTaxDeduction(Parameters)
	Возврат МобильнаяБухгалтерияЗарплата.ЗаявленияНаСтандартныеВычеты(Parameters);
КонецФункции

Функция GetQualifierTaxDeduction(Parameters)
	Возврат МобильнаяБухгалтерияЗарплата.ВидыВычетовНДФЛ(Parameters);
КонецФункции

Функция SetTaxDeduction(Parameters)
	Возврат МобильнаяБухгалтерияЗарплата.СоздатьИзменитьЗаявленияНаСтандартныеВычеты(Parameters);
КонецФункции

Функция GetAppVersion()
	Возврат МобильнаяБухгалтерия.ВерсияСервиса();
КонецФункции

Функция GetSmallBusinessInsuranceRate(Parameters)
	Возврат МобильнаяБухгалтерияЗарплата.ВключенТарифДляСубъектовМалогоБизнесаКоронавирус(Parameters);
КонецФункции

Функция SetInsuranceRate(Parameters)
	Возврат МобильнаяБухгалтерияЗарплата.УправлениеТарифамиСтраховыхВзносовКоронавирус(Parameters);
КонецФункции

Функция PaymentLink(ID, AccessDenied)
	Возврат МобильнаяБухгалтерия.ПлатежнаяСсылка(ID, AccessDenied);
КонецФункции


Функция CheckAccessYandexKassa(ID, AccessPage)
	Возврат МобильнаяБухгалтерия.ЕстьДоступКОнлайнОплатам(ID, AccessPage);
КонецФункции

Функция TaxPatentData(СтруктураПараметров, СообщениеОбОшибке)
	Возврат МобильнаяБухгалтерия.ПараметрыРасчетаПатентаКУплате(СтруктураПараметров, СообщениеОбОшибке);
КонецФункции


Функция GetTariffRestrictions(Parameters)
	
	Возврат МобильнаяБухгалтерия.ПолучитьТарифныеОграничения(Parameters);
	
КонецФункции

Функция TaxENСData(Parameters, ErrorMessage)
	Возврат МобильнаяБухгалтерия.СведенияОЕдиномНалоговСчете(Parameters, ErrorMessage);
КонецФункции

#Область ВзаимодействиеСАдреснымКлассификатором

Функция GetAdressMethod(Method, Parameters, ErrorMessage)
	
	ПараметрыМетода = Parameters.Получить();
	
	Результат = АдресныйКлассификаторБПМобильный.ВыполнитьМетодАдресногоКлассификатора(Method, ПараметрыМетода, ErrorMessage);
