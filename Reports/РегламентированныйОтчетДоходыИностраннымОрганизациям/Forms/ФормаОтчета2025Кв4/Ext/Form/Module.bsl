...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	ИменаПодчиненныхРазделов.Добавить("Раздел34");
	...
	СтруктураРаздела.Вставить("Многостраничность");
	СтруктураРаздела.Вставить("Подчиненность");
	СтруктураРаздела.Вставить("НаимГруппыСтраниц", "Подраздел 3.4");
	СтруктураРаздела.Вставить("НаимСтраницы", "Подраздел 3.4");
	мСтруктураМногоуровневыхРазделов.Вставить("Раздел34", СтруктураРаздела);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П001100002003", 20, ,
		"Выбор кода бюджетной классификации", КоллекцияСписковВыбора["КБК"]);
	...
	//Раздел 3.4.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000340002003", 2, ,
	"Выбор кода дохода", КоллекцияСписковВыбора["КодыДоходаПодр34"]);
...
Процедура ВосстановитьСохраненныеДанныеОтчетаСтарогоОбразца(СписокСохранения)
	...
	КонецЕсли;
	НастройкаРаздела34 = ДеревоНастройкиСтраниц.Строки.Найти("Раздел34", "ИмяСтраницы");
	Если НастройкаРаздела34 <> Неопределено Тогда
		ДеревоНастройкиСтраниц.Строки.Удалить(НастройкаРаздела34);
	КонецЕсли;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000320001603" ИЛИ ИмяПоказателя = "П000420002003" Тогда
		...
		Если ЗначениеЗаполнено(НомерВедомости) Тогда
			...
			"ru='Номер ведомости банковского контроля должен соответствовать шаблону ""ХХХХХХХХ/ХХХХ/ХХХХ/Х/Х"" (X - число от 0 до 9) или ""ХХХХХХХХ/ХХХХ/GUХХ/Х/Х"" (X - число от 0 до 9, GU - подстрока ""GU""). Значение очищено'");
			...
			Иначе
				...
				Для Ном = 1 По 22 Цикл
					...
					//символы "GU" - из номера паспорта-сделки, которые актуальны и сейчас,
					//так как эти контракты-сделки длительные по времени и могут быть действующими
					ИначеЕсли Ном = 15 Тогда
						НомерВедомостиСоответствуетТребованиям = СтрНайти("G1234567890", ТекСимв) > 0;
					ИначеЕсли Ном = 16 Тогда
						НомерВедомостиСоответствуетТребованиям = СтрНайти("U1234567890", ТекСимв) > 0;
	...
	Если ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел32" ИЛИ ИмяРаздела = "Раздел34" ИЛИ ИмяРаздела = "Раздел42" Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел32" Тогда
		...
		Если Обл <> Неопределено Тогда
			Обл.Формат = ?(ВалютаВыплатыДохода_Рубли, "ЧЦ=15;ЧДЦ=0;ЧН=-", "ЧЦ=23;ЧДЦ=8;ЧРД=,;ЧН=-");
			Обл.Значение = ?(ВалютаВыплатыДохода_Рубли, Окр(Обл.Значение), Обл.Значение);
		КонецЕсли;
		
		// Переопределение свойств ячейки с показателем строки 100.
		Обл = ТДО.Найти("П000320010003");
		Если Обл <> Неопределено Тогда
			Обл.Формат = ?(ВалютаВыплатыДохода_Рубли, "ЧЦ=15;ЧДЦ=0;ЧН=-", "ЧЦ=23;ЧДЦ=8;ЧРД=,;ЧН=-");
			Обл.Значение = ?(ВалютаВыплатыДохода_Рубли, Окр(Обл.Значение), Обл.Значение);
		КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел33" Тогда
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли ИмяРаздела = "Раздел34" Тогда
		
		// Переопределение свойств ячейки с показателем строки 060.
		Обл = ТДО.Найти("П000340006003");
		Если Обл <> Неопределено Тогда
			Если Форма.СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
				РазрешитьВводДанныхВОбласть(Форма, Обл);
			Иначе
...
Процедура ВывестиДанныеВТабличныйДокумент(ИмяТекРаздела, НовыеНомераСтрокМногоуровнегоРаздела = Неопределено)
	...
	Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
		...
		Если Обл <> Неопределено Тогда
			Если ИмяТекРаздела = "Раздел32" ИЛИ ИмяТекРаздела = "Раздел33" ИЛИ ИмяТекРаздела = "Раздел34" Тогда
...
Процедура ДобавитьСтраницуНаСервере(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел31" Тогда
		ДеревоРаздела = ДеревоДанныхРаздела;
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если НЕ ЗначениеЗаполнено(ТДО["П000320009003"].Значение) Тогда
					...
					Иначе
						ТДО["П000320010003"].Значение = Окр(Окр(ТДО["П000320004003"].Значение, 8) * СтавкаДляРасчета / 100, 8);
					КонецЕсли;
				Иначе
				...
				Иначе
					...
					= Окр(Окр(ТДО["П000320004003"].Значение, 8) * Окр(ТДО["П000320012003"].Значение, 8));
					ТДО["П000320014003"].Значение
					= Окр(Окр(ТДО["П000320010003"].Значение, 8) * Окр(ТДО["П000320012003"].Значение, 8));
				КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел34" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Ставка = ТДО["П000340005003"].Значение;
				СтавкаДляРасчета = ?(Ставка = 0 ИЛИ Ставка = 99.99, 0, Ставка);
				
				ТДО["П000340006003"].Значение
					= Окр(ТДО["П000340003003"].Значение * СтавкаДляРасчета / 100);
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Иначе
					...
					= Окр(Окр(ТДО["П000420005003"].Значение, 8) * Окр(ТДО["П000420008003"].Значение, 8));
				КонецЕсли;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
		...
		Для каждого Ветка_31 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка_32 Из Ветка_31.Строки[0].Строки Цикл
				...
				Если НЕ ЗначениеЗаполнено(Ветка_32.Данные["П000320009003"]) Тогда
					...
					Иначе
						Ветка_32.Данные.Вставить("П000320010003",
						Окр(Окр(Ветка_32.Данные["П000320004003"], 8) * СтавкаДляРасчета / 100, 8));
					КонецЕсли;
				Иначе
					// Ручной ввод значения в стр.100.
				КонецЕсли;
				...
				Иначе
					...
					Окр(Окр(Ветка_32.Данные["П000320004003"], 8) * Окр(Ветка_32.Данные["П000320012003"], 8)));
					Ветка_32.Данные.Вставить("П000320014003",
					Окр(Окр(Ветка_32.Данные["П000320010003"], 8) * Окр(Ветка_32.Данные["П000320012003"], 8)));
				КонецЕсли;
	...
	ИначеЕсли ИмяРаздела = "Раздел34" Тогда
		...
		Для каждого Ветка_31 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка_34 Из Ветка_31.Строки[1].Строки Цикл
				
				Если Ветка_34 = АктивнаяВетка Тогда
					Для каждого Пок Из Ветка_34.Данные Цикл
						Ветка_34.Данные.Вставить(
						Пок.Ключ, Форма.ТабличныйДокумент.Области[Пок.Ключ].Значение);
					КонецЦикла;
				КонецЕсли;
				
				Ставка = Ветка_34.Данные["П000340005003"];
				СтавкаДляРасчета = ?(Ставка = 0 ИЛИ Ставка = 99.99, 0, Ставка);
				Ветка_34.Данные.Вставить("П000340006003",
				Окр(Ветка_34.Данные["П000340003003"] * СтавкаДляРасчета / 100));
				
				Если Ветка_34 = АктивнаяВетка Тогда
					ТДО["П000340006003"].Значение = Ветка_34.Данные["П000340006003"];
				КонецЕсли;
		...
		Для каждого Ветка_41 Из ДеревоДанных.Строки Цикл
			...
			Для каждого Ветка_42 Из Ветка_41.Строки[0].Строки Цикл
				...
				Иначе
					...
					Окр(Окр(Ветка_42.Данные["П000420005003"], 8) * Окр(Ветка_42.Данные["П000420008003"], 8)));
				КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ЗаполненРаздел1(ДеревоРаздела) Тогда
		...
		ЗначПокР1_020 = СокрЛП(Дан["П001100002003"]);
		ЗначПокР1_060 = Дан["П001100006003"];
		Если НЕ ЗначениеЗаполнено(ЗначПокР1_020) И ЗначениеЗаполнено(ЗначПокР1_060) Тогда
			СообщитьВТС("Раздел 1: при заполненной сумме налога по сроку по п. 6.1 ст. 105.3 НК РФ не задан КБК", "Раздел1", "П001100002003", НомерСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если ЗаполненРаздел3(ДеревоРаздела31) Тогда
		...
		ВозможныеКодыДохода34 = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000340002003");
		...
		Для каждого СтраницаРаздела31 Из ДеревоРаздела31.Строки Цикл
			...
			// Проверка на заполненность подраздела 3.4.
			// Чтобы номер дохода был ПО ПОРЯДКУ в рамках подраздела
			// мы должны либо выгружать все страницы подраздела 3.4,
			// либо не выгружать подраздел 3.4 вообще при наличии пустых добавленных страниц
			Проверять34 = Ложь;
			Для каждого СтраницаРаздела34 Из СтраницаРаздела31.Строки[1].Строки Цикл
				Данные34 = СтраницаРаздела34.Данные;
				Для каждого Пок Из Данные34 Цикл
					Если ЗначениеЗаполнено(Пок.Значение) Тогда
						Проверять34 = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если Проверять34 = Истина Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если Проверять34 Тогда
				ВозможныеКодыПериода34Стр = "34,46,50"; //только за налоговый период
				ВозможныеКодыПериода34 = СтрРазделить(ВозможныеКодыПериода34Стр, ",", Ложь);
				Если ВозможныеКодыПериода34.Найти(ПараметрыВыгрузки.Период) = Неопределено Тогда
					СообщитьВТС("Подраздел 3.4 заполняется при указании на титульном листе следующих кодах отчетного (налогового) периода: "
					+ ВозможныеКодыПериода34Стр, "Титульный", "Период");
					СтатусПроверки = Ложь;
					Проверять34 = Ложь;
				КонецЕсли;
			КонецЕсли;
			...
			Если Проверять34 Тогда
				...
				Для каждого СтраницаРаздела34 Из СтраницаРаздела31.Строки[1].Строки Цикл
					...
					ИндСтраницы34       = СтраницаРаздела31.Строки[1].Строки.Индекс(СтраницаРаздела34);
					НомерСтраницы34     = ИндСтраницы34 + 1;
					НомерСтраницы34_Стр = Формат(НомерСтраницы34, "ЧГ=");
					
					КоординатыСтраницы34 = НомерСтраницы31_Стр + ",2," + НомерСтраницы34_Стр;
					
					ИмяПокКодДох_Р34_020    = "П000340002003";
					ИмяПокДатаВыпл_Р34_040  = "П000340004003";
					ИмяПокСтавкаНал_Р34_050 = "П000340005003";
					ИмяПокСумНал_Р34_060    = "П000340006003";
					ИмяПокСрокУпл_Р34_070   = "П000340007003";
					
					КодДох_Р34_020    = СокрЛП(Данные34[ИмяПокКодДох_Р34_020]);
					ДатаВыпл_Р34_040  = Данные34[ИмяПокДатаВыпл_Р34_040];
					СтавкаНал_Р34_050 = Данные34[ИмяПокСтавкаНал_Р34_050];
					СумНал_Р34_060    = Данные34[ИмяПокСумНал_Р34_060];
					СрокУпл_Р34_070   = Данные34[ИмяПокСрокУпл_Р34_070];
					
					Если НЕ ЗначениеЗаполнено(КодДох_Р34_020) Тогда
						СообщитьВТС("Подраздел 3.1. Страница " + НомерСтраницы31_Стр + " -> Подраздел 3.4. Страница "
						+ НомерСтраницы34_Стр + ": не задан код дохода", "Раздел34", ИмяПокКодДох_Р34_020, КоординатыСтраницы34);
						СтатусПроверки = Ложь;
					ИначеЕсли ВозможныеКодыДохода34.МассивЗначений.Найти(КодДох_Р34_020) = Неопределено Тогда
						СообщитьВТС("Подраздел 3.1. Страница " + НомерСтраницы31_Стр + " -> Подраздел 3.4. Страница "
						+ НомерСтраницы34_Стр + ": некорректно задан код дохода. Возможны значения: "
						+ ВозможныеКодыДохода34.СтрокаЗначений, "Раздел34", ИмяПокКодДох_Р34_020, КоординатыСтраницы34);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если НЕ ЗначениеЗаполнено(ДатаВыпл_Р34_040) Тогда
						СообщитьВТС("Подраздел 3.1. Страница " + НомерСтраницы31_Стр + " -> Подраздел 3.4. Страница "
						+ НомерСтраницы34_Стр + ": не задана дата выплаты дохода",
						"Раздел34", ИмяПокДатаВыпл_Р34_040, КоординатыСтраницы34);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					//ОдновременноЗаполненыР34_050_070 = ЗначениеЗаполнено(СтавкаНал_Р34_050) И ЗначениеЗаполнено(СумНал_Р34_060) И ЗначениеЗаполнено(СрокУпл_Р34_070);
					ОдновременноЗаполненыР34_050_070 = ЗначениеЗаполнено(СтавкаНал_Р34_050) И ЗначениеЗаполнено(СрокУпл_Р34_070);
					ОдновременноНЕЗаполненыР34_050_070 = НЕ ЗначениеЗаполнено(СтавкаНал_Р34_050) И НЕ ЗначениеЗаполнено(СумНал_Р34_060) И НЕ ЗначениеЗаполнено(СрокУпл_Р34_070);
					Если НЕ (ОдновременноЗаполненыР34_050_070 ИЛИ ОдновременноНЕЗаполненыР34_050_070) Тогда
						СообщитьВТС("Подраздел 3.1. Страница " + НомерСтраницы31_Стр + " -> Подраздел 3.4. Страница "
						+ НомерСтраницы34_Стр + ": должны быть одновремено заполнены или одновременно пусты следующие показатели: ставка налога, сумма налога, срок уплаты налога",
						"Раздел34", ИмяПокСтавкаНал_Р34_050, КоординатыСтраницы34);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Функция ЗаполненРаздел3(ДеревоРаздела31)
	...
	Для каждого СтраницаРаздела31 Из ДеревоРаздела31.Строки Цикл
		...
		Иначе
			...
			Для каждого СтраницаРаздела34 Из СтраницаРаздела31.Строки[1].Строки Цикл
				...
				Для каждого Пок Из Данные34 Цикл
					Если ЗначениеЗаполнено(Пок.Значение) Тогда
						ЕстьЗаполненныйРаздел34 = Истина;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если ЕстьЗаполненныйРаздел34 Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Возврат ЕстьЗаполненныйРаздел31 ИЛИ ЕстьЗаполненныйРаздел32 ИЛИ ЕстьЗаполненныйРаздел33 ИЛИ ЕстьЗаполненныйРаздел34;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для каждого СтрокаМнЧ1 Из ДанныеМнЧ1.Строки Цикл
		...
		КонецЦикла;
	КонецЦикла;
	СумНалПУ6_1Код = ПолучитьПодчиненныйЭлемент(СумНалПУ, "СумНалПУ6.1Код");
	Для каждого Эл Из СумНалПУ6_1Код.Строки Цикл
		ВывестиПоказательВXML(Эл, Ветка1.Данные[Эл.Ключ]);
	КонецЦикла;
	...
	Если ЗаполненРаздел3(ДеревоДанныхРаздел31) Тогда
		...
		Для каждого Ветка31 Из ДеревоДанныхРаздел31.Строки Цикл
				...
				Для каждого Эл Из ЭкзДохРасчНал.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						Если Эл.Код = "НомерДох" Тогда
							...
							// для "номера дохода" текущего экземпляра Раздела 3.
						...
						ИначеЕсли Эл.Код = "СумДохДоНал" ИЛИ Эл.Код = "СумНалРЕПО" ИЛИ Эл.Код = "СумНалогВал" Тогда
							...
							"ЧЦ=15;ЧДЦ=0;ЧРД=.;ЧН=0;ЧГ=;", "ЧЦ=23;ЧДЦ=8;ЧРД=.;ЧН=0;ЧГ=;");
			...
			// Подраздел 3.4.
			ДохРасчНал_1 = ПолучитьПодчиненныйЭлемент(ЭкзРасчВыплДох, "ДохРасчНал6.1");
			Для каждого Ветка34 Из Ветка31.Строки[1].Строки Цикл
				
				ЭкзДохРасчНал = СкопироватьУзел(ЭкзРасчВыплДох, ДохРасчНал_1);
				Для каждого Эл Из ЭкзДохРасчНал.Строки Цикл
					Если Эл.Строки.Количество() = 0 Тогда
						Если Эл.Код = "СумНал" Тогда
							СтавкаНалЗ4 = Число(Ветка34.Данные["П000340005003"]);
							СумНалЗ4 = Число(Ветка34.Данные["П000340006003"]);
							Если (СтавкаНалЗ4 > 0) И (СумНалЗ4 = 0) Тогда
								Эл.Обязательность = "О";
							Иначе
								Эл.Обязательность = "Н";
							КонецЕсли;
							ВывестиПоказательВXML(Эл, Ветка34.Данные[Эл.Ключ]);
						ИначеЕсли Эл.Код = "НомерДох" Тогда
							Для каждого Пок Из Ветка34.Данные Цикл
								Если ЗначениеЗаполнено(Пок.Значение) Тогда
									// Генерирование "порядковых номеров" экземпляров Подраздела 3.4
									// для "номера дохода" текущего экземпляра Раздела 3.
									ВывестиПоказательВXML(Эл, Ветка31.Строки[1].Строки.Индекс(Ветка34) + 1);
									Прервать;
								КонецЕсли;
							КонецЦикла;
						Иначе
...
Процедура ОчиститьНаСервере() Экспорт
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел32"
		...
		ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел34"
		ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел42" Тогда
		СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела.Добавить(1);
		СтруктураРеквизитовФормы.НомераСтрокМногоуровнегоРаздела.Добавить(1);
	КонецЕсли;
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел31" Тогда
		...
		МакетРаздела34 = ОбъектОтчета(ИмяФормы).ПолучитьМакет(Сред(
		ИмяФормы, СтрНайти(ИмяФормы, "ФормаОтчета")) + "_" + "Раздел34");
		...
		Для НомЛиста31 = 1 По КолЛистовРаздела31 Цикл
				...
				Для каждого Пок Из Данные32 Цикл
					...
					Если Обл <> Неопределено Тогда
						...
						ИначеЕсли Пок.Ключ = "П000320004003"
							...
							Иначе
								Обл.Значение = Окр(Пок.Значение, 8);
							КонецЕсли;
			...
			// Вывод листов подраздела 3.4.
			КолЛистовРаздела34 = ДеревоДанныхРаздела.Строки[НомЛиста31 - 1].Строки[1].Строки.Количество();
			Для НомЛиста34 = 1 По КолЛистовРаздела34 Цикл
				
				Данные34 = ДеревоДанныхРаздела.Строки[НомЛиста31 - 1].Строки[1].Строки[НомЛиста34 - 1].Данные;
				
				ПорядНомДоходаСтрока = Формат(НомЛиста34, "ЧГ=;ЧВН=;ЧЦ=12");
				
				ТабличныйДокумент.Очистить();
				ТабличныйДокумент.Вывести(МакетРаздела34);
				...
				Для каждого ЭлСтруктурыПараметров Из СтруктураПараметров Цикл
					СтруктураПараметровРаздела3.Вставить(ЭлСтруктурыПараметров.Ключ, ЭлСтруктурыПараметров.Значение);
				КонецЦикла;
				СтруктураПараметровРаздела3.Вставить("ИмяСтраницы", "Раздел34");
				ТД = ТабличныйДокумент.ПолучитьОбласть();
				ВывестиНаПечатьЛист(СтруктураПараметровРаздела3, ОбщиеРеквизитыРазделов, СтрПоля);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПрефиксИменМакетов = "ПечатныйБланк2025Кв4_";
				...
				Если ТекАдресЗначенияОбласти.ИндексЗнакоместа <> 0 Тогда
					...
					Если РезультатПоиска <> Неопределено Тогда
						Если ТипЗнч(РезультатПоиска.Значение)=Тип("Число") Тогда
							...
							Если (РезультатПоиска.Имя = "П000320007003" ИЛИ РезультатПоиска.Имя = "П000340005003") И РезультатПоиска.Значение = 0 Тогда
							...
							Иначе
								...
								ИначеЕсли ТекАдресЗначенияОбласти.ИндексЗнакоместа > ИндексТочкиЧислаНаМЧБ Тогда
									...
									Если (РезультатПоиска.Имя = "П000320007003" ИЛИ РезультатПоиска.Имя = "П000340005003")
										И РезультатПоиска.Значение = Цел(РезультатПоиска.Значение) Тогда
...
Процедура ЗагрузитьПодготовленныеДанные() Экспорт
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел32"
		ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел33"
		ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел34" Тогда
...
Процедура СформироватьСлужебныеДанные()
	...
	ИменаРазделов.Вставить("Р34", "Раздел34");
	...
	ПредставленияРазделов.Вставить("Р34", "Подраздела 3.4 Раздела 3");
	...
	ЗаменаНекорректныхСтрок.Вставить("УникальныйНомер", "001");
...
Процедура ПолучитьДанныеСтраницы(СтруктураПриемник, 
	...
	Если ТипЗнч(СтруктураИсточник) = Тип("СтрокаДереваЗначений") И НЕ СтруктураИсточник.Строки.Количество() = 0 Тогда
		...
		Если ВходящийРаздел.Наименование = "Раздел31" И ВходящийРаздел.Уровень = 1 Тогда
			...
			ПолучитьДанныеСтраницы(СтруктураПриемник, СтруктураИсточник.Строки[1],
				Новый Структура("Наименование, Уровень", ВходящийРаздел.Наименование + "4", 2), ВходящаяМнЧасть); 
				
		ИначеЕсли ВходящийРаздел.Наименование = "Раздел41" И ВходящийРаздел.Уровень = 1 Тогда
			ПолучитьДанныеСтраницы(СтруктураПриемник, СтруктураИсточник.Строки[0],
				Новый Структура("Наименование, Уровень", ВходящийРаздел.Наименование, 2), ВходящаяМнЧасть); 
		...
		Иначе
			...
			Для каждого СтраницаПодраздела Из СтруктураИсточник.Строки Цикл
				...
				Если ВходящийРаздел.Уровень = 2 ИЛИ ВходящийРаздел.Уровень = 4 Тогда 
					Если ВходящийРаздел.Наименование = "Раздел314" Тогда
						ДанныеСтраницы.Вставить("НомерСтраницы", СтруктураПриемник.НомерСтраницы + ",2," +
...
Процедура ПодготовитьНастройки(Заголовки)
	...
	МассивКС25г = СтрРазделить(
		"1.01.1,1.01.2,1.01.3,1.01.4,1.02,1.06,1.07,1.14,1.24,1.34.1,1.34.2,1.44.а,1.44.б,1.44.в", ",");
	
	Если НЕ МассивКС25г.Найти(НастройкиКС.НомерКС) = Неопределено Тогда 
		НастройкиКС.Основание = "Письмо ФНС России от 13.11.2025 N СД-4-3/10190@";
	КонецЕсли;
...
Процедура ПроверкаКС_17_1_1_к()
	...
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@'")); 
...
Процедура ПроверкаКС_17_3_к()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='∑ стр.(030, 040, 050,060) Раздела 1 = ∑ стр.040 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.030, 040, 050 Раздела 1'"));
	Заголовки.Вставить("Основание"			, 
		НСтр("ru='Приказ ФНС России от 29.02.2024 № ЕД-7-3/164@, Приказ ФНС России от 25.11.2025 N ЕД-7-3/1010@'")); 
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей.Вставить("Р01С040"	, "П000100004003_");
	ИменаПоказателей.Вставить("Р01С050"	, "П000100005003_");
	ИменаПоказателей.Вставить("Р01С060"	, "П001100006003");
	...
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Р01С060, ДанныеКС.Раздел1);
...
&НаСервере
Процедура ПроверкаКС_1_01_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 1");
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.01.3'"));
	Заголовки.Вставить("Описание"			, НСтр("ru='Сумма налога в стр.050 Раздела 1 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Укажите сумму налога в стр.050 Раздела 1 >= 0'"));
...
&НаСервере
Процедура ПроверкаКС_1_01_4()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, "Раздел 1");
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.01.4'"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р01С001"		, "П001100006003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
...
Процедура ПроверкаКС_1_02()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='∑ стр.(030, 040, 050, 060) Раздела 1 = ∑ стр.140 подр.3.2 Раздела 3 + ∑ стр.060 подр.3.4 Раздела 3'"));
	...
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните показатели в стр.030, 040, 050, 060 Раздела 1'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''")); 
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей.Вставить("Р01С060"	, "П001100006003"); 
	ИменаПоказателей.Вставить("Р32С140"	, "П000320014003");
	ИменаПоказателей.Вставить("Р34С060"	, "П000340006003");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел312 Цикл
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р32С140, СтраницаПодраздела);
		КонецЦикла;
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел3142 Цикл
			ЗаполнитьИРасшифроватьПоказатель(Показатели.Р34С060, СтраницаПодраздела);
		КонецЦикла;
	КонецЦикла;
	
	РасчетнаяСумма1 = Показатели.Р01С030.Итог + Показатели.Р01С040.Итог + Показатели.Р01С050.Итог 
		+ Показатели.Р01С060.Значение;  
	РасчетнаяСумма2 = Показатели.Р32С140.Итог + Показатели.Р34С060.Итог;
	НастройкиКС.ЭтоОшибка = НЕ РасчетнаяСумма1 = РасчетнаяСумма2;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
		РасчетнаяСумма1, Показатели.Р01С030.Итог, Показатели.Р01С040.Итог, Показатели.Р01С050.Итог,
		Показатели.Р01С060.Значение, РасчетнаяСумма2, Показатели.Р32С140.Итог, Показатели.Р34С060.Итог);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_06()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='∑ стр.140 подр.3.2 Раздела 3 + ∑ стр.060 подр.3.4 Раздела 3 по соответствующим '") +
		НСтр("ru='кодам дохода в стр.020 подр.3.2 и 3.4 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4) по коду дохода %5'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей.Вставить("Р32С140"	, "П000320014003");
	ИменаПоказателей.Вставить("Р34С020"	, "П000340002003");
	ИменаПоказателей.Вставить("Р34С060"	, "П000340006003"); 
	...
	Для каждого КодДохода Из ДанныеКС.МассивКДРаздела2 Цикл
		...
		Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл
			...
			КонецЦикла;
			Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел3142 Цикл
				ЗаполнитьПоказатель(Показатели.Р34С020, СтраницаПодраздела);
				Если Показатели.Р34С020.Значение = КодДохода Тогда
					Если НЕ КодР34Расшифрован Тогда
						РасшифроватьПоказатель(Показатели.Р34С020);
						КодР34Расшифрован = Истина;
					КонецЕсли;
					ЗаполнитьИРасшифроватьПоказатель(Показатели.Р34С060, СтраницаПодраздела);
				КонецЕсли;
			КонецЦикла;
		...
		РасчетнаяСумма = Показатели.Р32С140.Итог + Показатели.Р34С060.Итог;
		НастройкиКС.ЭтоОшибка = НЕ Показатели.Р21С040.Итог = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
			Показатели.Р21С040.Итог, РасчетнаяСумма, Показатели.Р32С140.Итог, Показатели.Р34С060.Итог, 
			ЗначСтр(КодДохода));
		ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_07()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='∑ стр.040 подр.2.1 Раздела 2 = ∑ стр.140 подр.3.2 Раздела 3 + ∑ стр.060 подр.3.4 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.040 подр.2.1 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
...
Процедура ПроверкаКС_1_14()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.070 подр.2.2 Раздела 2 = ∑ стр.140 подр.3.2 Раздела 3 + ∑ стр.060 '") +
		НСтр("ru='подр.3.4 Раздела 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.070 подр.2.2 Раздела 2'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	...
	НастройкиКС.ЭтоОшибка = НЕ Показатели.Р22С070.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
		Показатели.Р22С070.Значение, РасчетнаяСумма, Показатели.Р32С140.Итог, Показатели.Р34С060.Итог);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_24()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Раздела 3 = 01 - 13, 16, 18, 21, 23, 24, 26, 28, 29, 31, 32, 39, 41, 43'"));
	...
	МассивКодов = СтрРазделить(
		"01,02,03,04,05,06,07,08,09,10,11,12,13,16,18,21,23,24,26,28,29,31,32,39,41,43", ",");
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел312 Цикл
			...
			РасчетнаяСумма = Окр(Показатели.Р32С002.Значение * Показатели.Р32С003.Значение / 100, 8);
...
Процедура ПроверкаКС_1_25()
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел312 Цикл
			...
			РасчетнаяСумма = Окр(Показатели.Р32С003.Значение * Показатели.Р32С004.Значение / 100, 8);
...
Процедура ПроверкаКС_1_34_1()
	...
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='12, 28, 29, 31, 32, 39, 43 и стр.160 подр.3.2 Раздела 3 не заполнена'"));
	...
	НастройкиКС.Вставить("МассивКодов"		, СтрРазделить("09,11,12,28,29,31,32,39,43", ","));
	ПроверкаКС_1_31_1_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_1_34_2()
	...
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.070 подр.3.2 Раздела 3 <= 25%, если стр.020 подр.3.2 Раздела 3 = 09, 11, '") +
		НСтр("ru='12, 28, 29, 31, 32, 39, 43 и стр.160 подр.3.2 Раздела 3 заполнена'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <= 25, если %2 = 09, 11, 12, 28, 29, 39'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните ставку налога в стр.070 подр.3.2 Раздела 3'"));
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 29.07.2024 № СД-4-3/13606@'"));
...
&НаСервере
Процедура ПроверкаКС_1_44а()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Раздел 3'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.44.а'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Значение уникального номера в стр.001 подр.3.4 Раздела 3 должно быть заполнено'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Значение %1 не заполнено'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно некорректное значение показателя отчета'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Заполните уникальный номер в стр.001 подр.3.4 Раздела 3'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.3.4 Раздела 3'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р31С001"		, "П000310000103");
	ИменаПоказателей.Вставить("Р34С001"		, "УникальныйНомер");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл 
		
		ЗаполнитьПоказатель(Показатели.Р31С001, СтраницаРаздела);
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел3142 Цикл
		
			ЗарегистрироватьПроверку(); 
			Показатели.Р34С001.Значение = Показатели.Р31С001.Значение;
			Показатели.Р34С001.ЗначениеСтр = Показатели.Р31С001.ЗначениеСтр;
			Показатели.Р34С001.НомерСтраницы = СтраницаПодраздела.НомерСтраницы;
			РасшифроватьПоказатель(Показатели.Р34С001);
		
			НастройкиКС.ЭтоОшибка = СтраницаПодраздела.РазделЗаполнен И НЕ ЗначениеЗаполнено(Показатели.Р31С001.Значение);
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
				Показатели.Р31С001.ЗначениеСтр);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(СтраницаПодраздела.НомерСтраницы));
...
&НаСервере
Процедура ПроверкаКС_1_44б()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Раздел 3'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.44.б'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.060 подр.3.4 Раздела 3 = стр.(030 х 050)/100 подр.3.4 Раздела 3, если стр.050 '") +
		НСтр("ru='подр.3.4 Раздела 3 > 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4 / 100), если %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Уточните сумму налога в стр.060 подр.3.4 Раздела 3'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.3.4 Раздела 3'"));
	ПодготовитьНастройки(Заголовки);
	...
	ПроверкаКС_1_44х_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_1_44х_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р34С001"		, "П000340006003");
	ИменаПоказателей.Вставить("Р34С002"		, "П000340007003");
	ИменаПоказателей.Вставить("Р34С003"		, "П000340003003");
	ИменаПоказателей.Вставить("Р34С004"		, "П000340005003");
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Показатели.Р34С002.Расшифровывать = НЕ НастройкиКС.БольшеНуля;
	Показатели.Р34С003.Расшифровывать = НастройкиКС.БольшеНуля;
	...
	Для каждого СтраницаРаздела Из ДанныеКС.Раздел31 Цикл 
		Для каждого СтраницаПодраздела Из СтраницаРаздела.Раздел3142 Цикл
			...
			Если НастройкиКС.БольшеНуля Тогда
				УсловиеВыполнено = Показатели.Р34С004.Значение > 0;
				РасчетнаяСумма = Окр(Показатели.Р34С003.Значение * Показатели.Р34С004.Значение / 100);
				НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.Р34С001.Значение = РасчетнаяСумма;
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					Показатели.Р34С001.Значение, РасчетнаяСумма, Показатели.Р34С003.Значение, Показатели.Р34С004.Значение);
			Иначе
				УсловиеВыполнено = Показатели.Р34С004.Значение = 0;
				НастройкиКС.ЭтоОшибка = УсловиеВыполнено И (НЕ Показатели.Р34С001.Значение = 0 
					ИЛИ ЗначениеЗаполнено(Показатели.Р34С002.Значение));
				НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации,
					Показатели.Р34С001.Значение, Показатели.Р34С002.ЗначениеСтр, Показатели.Р34С004.Значение);
			КонецЕсли;
...
&НаСервере
Процедура ПроверкаКС_1_44в()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Раздел 3'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='1.44.в'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.060 подр.3.4 Раздела 3 = 0 и стр.070 подр.3.4 Раздела 3 не заполнена, если '") +
		НСтр("ru='стр.050 подр.3.4 Раздела 3 = 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0 и дата %2 не заполнена, если %3 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные показатели'"));
	Заголовки.Вставить("Рекомендации"		, НСтр("ru='Проверьте значения в стр.060, 070 подр.3.4 Раздела 3'"));
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 подр.3.4 Раздела 3'"));
	ПодготовитьНастройки(Заголовки);
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "Раздел1" ИЛИ Ячейка.Раздел = "Раздел2" ИЛИ Ячейка.Раздел = "Раздел31" ИЛИ Ячейка.Раздел = "Раздел41" ИЛИ Ячейка.Раздел = "Раздел5" Тогда
	...
	ИначеЕсли Ячейка.Раздел = "Раздел32" ИЛИ Ячейка.Раздел = "Раздел33" ИЛИ Ячейка.Раздел = "Раздел34" ИЛИ Ячейка.Раздел = "Раздел42" Тогда
							...
							Если НайденныйПодчРаздел.КолонкаНомерСтраницыРазделаОтчета = Число(НомераСтрокМногоуровнегоРаздела[1]) Тогда
								Для каждого НайденнаяПодчСтраница Из НайденныйПодчРаздел.ПолучитьЭлементы() Цикл
									Если НайденнаяПодчСтраница.КолонкаНомерСтраницыРазделаОтчета = Число(НомераСтрокМногоуровнегоРаздела[2]) Тогда
										...
										Если Ячейка.Раздел = "Раздел33" Тогда
