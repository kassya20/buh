...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "31/08/2025";
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2025Кв3_ФормаОтчета"));
...
Процедура ПриОткрытии(Отказ)
	...
	ПронумероватьСтрокиДляПечати(ЭтотОбъект, Ложь);
...
Процедура ПронумероватьСтрокиДляПечати(Форма, УстМодифицированность = Истина)
	Если УстМодифицированность = Истина Тогда
		Форма.Модифицированность = Истина;
	КонецЕсли;	
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (Не (Область.Защита = Истина)) И СтрНайти(Область.Имя, "_") = 14 И СтруктураРеквизитовФормы.мАвтоВыборКодов = Ложь Тогда
		...
		Если СтрНайти(Область.Имя, "П010000000702") = 1
			Или СтрНайти(Область.Имя, "П010000000703") = 1 Тогда 
		...
		ИначеЕсли СтрНайти(Область.Имя, "П010000000502") = 1 Тогда
							
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ВеществаАтмосфера");
			ОбластьДляВвода     = ПолеТабличногоДокументаФормаОтчета.Области["П010000000502" + Сред(Область.Имя, 14)];
			ОбластьНаименование = ПолеТабличногоДокументаФормаОтчета.Области["П010000000502" + Сред(Область.Имя, 14)];
			ВвестиПоказатель(ОбластьДляВвода, ПараметрВводаПоля.Показатель, ПараметрВводаПоля.ТекстПриВыборе,
							ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка,ОбластьНаименование);
							
		ИначеЕсли СтрНайти(Область.Имя, "П010000000603") = 1 Тогда
							
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "ВеществаВода");
			ОбластьДляВвода     = ПолеТабличногоДокументаФормаОтчета.Области["П010000000603" + Сред(Область.Имя, 14)];
			ОбластьНаименование = ПолеТабличногоДокументаФормаОтчета.Области["П010000000603" + Сред(Область.Имя, 14)];
			ВвестиПоказатель(ОбластьДляВвода, ПараметрВводаПоля.Показатель, ПараметрВводаПоля.ТекстПриВыборе,
							ПараметрВводаПоля.ТаблицаЗначений, СтандартнаяОбработка,ОбластьНаименование);
		КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2025Кв3_ФормаОтчета"));
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ИмяПечатнойФормы = "Декларация о воздействии на окружающую среду";
	
	СтруктураРеквизитовФормы.мПечатныеФормы.Очистить();
	ОбщиеРеквизитыРазделов = Новый Структура;
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("НаименованиеЛиста",  ИмяПечатнойФормы);
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ТитульныйЧасть1"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ТитульныйЧасть2"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, ИмяПечатнойФормы);
	...
	Для Инд = 1 По 8 Цикл
		...
		МассивПроверкиВывода.Добавить(ТабличныйДокумент.ПолучитьОбласть("ЗаголовокТабЧасть" + Инд));
		МассивПроверкиВывода.Добавить(ТабличныйДокумент.ПолучитьОбласть(ТабличныйДокумент.Область("ЗаголовокТабЧасть" + Инд).Низ + 1, ,ТабличныйДокумент.Область("ЗаголовокТабЧасть" + Инд).Низ + 1));
		...
		ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ЗаголовокТабЧасть" + Инд));
		
		ПерваяСтр = ТабличныйДокумент.Область("ЗаголовокТабЧасть" + Инд).Низ + 1;
		ПослСтр = ТабличныйДокумент.Область("ДобавлениеСтрокиЧасть" + Инд).Верх - 1;
		...
		Если ОП <> Неопределено Тогда
			...
			Если Не ПечатнаяФорма.ПроверитьВывод(МассивПроверкиВывода) Тогда
				...
				ПечатнаяФорма = НоваяСтраница();
			Иначе
            	ПечПС = ТабличныйДокумент.ПолучитьОбласть("ПустаяСтрока");
				НомПечПС = 0;
				Пока ПечатнаяФорма.ПроверитьВывод(МассивПроверкиВывода) Цикл
					НомПечПС = НомПечПС + 1;
					МассивПроверкиВывода.Добавить(ПечПС);
				КонецЦикла;
			КонецЕсли;
			Для НомПС = 1 По НомПечПС - 1 Цикл
				ПечатнаяФорма.Вывести(ПечПС);
			КонецЦикла;
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("Списки2025Кв3");
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ВеществаАтмосфера", 512, , "Выбор загрязняющего вещества для атмосферы", КоллекцияСписковВыбора["ВеществаАтмосфера"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "ВеществаВода", 512, , "Выбор загрязняющего вещества для водных объектов", КоллекцияСписковВыбора["ВеществаВода"]);
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2025Кв3");
...
Функция ПроверитьПоказатели() Экспорт
	...
	РегламентированнаяОтчетность.СформироватьСтруктуруПоказателей(ЭтаФорма);
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2025Кв3");
