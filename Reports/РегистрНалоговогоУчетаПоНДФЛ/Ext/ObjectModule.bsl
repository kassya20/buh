...
Процедура ПолучитьДанныеДляПостроенияОтчета(СписокФизЛицОрганизация)
	...
	|	НДФЛСведенияОДоходах.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН
	...
	|ПОМЕСТИТЬ ВТДоходыПереданныеВБанкНаАУСН
	|ИЗ
	|	ВТДоходыПоСтавкам КАК Доходы
	|ГДЕ
	|	Доходы.ЗарегистрированоДляПередачиВБанкНаАУСН
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Доходы.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане
	|ПОМЕСТИТЬ ВТМесяцыДоходовПереданныхВБанкНаАУСН
	|ИЗ
	|	ВТДоходыПереданныеВБанкНаАУСН КАК Доходы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо ЕСТЬ НЕ NULL 
	|		И НДФЛИмущественныеВычетыФизлиц.КодВычета.ГруппаВычета = ЗНАЧЕНИЕ(Перечисление.ГруппыВычетовПоНДФЛ.Стандартные) КАК ЗарегистрированоДляПередачиВБанкНаАУСН
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМесяцыДоходовПереданныхВБанкНаАУСН КАК ДоходыПереданныеВБанкНаАУСН
	|		ПО СписокФизЛицОрганизация.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
	|			И СписокФизЛицОрганизация.ГоловнаяОрганизация = ДоходыПереданныеВБанкНаАУСН.ГоловнаяОрганизация
	|			И (НАЧАЛОПЕРИОДА(НДФЛИмущественныеВычетыФизлиц.Период, МЕСЯЦ) = ДоходыПереданныеВБанкНаАУСН.МесяцНалоговогоПериода)
	|			И (НДФЛИмущественныеВычетыФизлиц.РегистрацияВНалоговомОргане = ДоходыПереданныеВБанкНаАУСН.РегистрацияВНалоговомОргане)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	КОНЕЦ,
	|	ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо ЕСТЬ НЕ NULL 
	|		И НДФЛИмущественныеВычетыФизлиц.КодВычета.ГруппаВычета = ЗНАЧЕНИЕ(Перечисление.ГруппыВычетовПоНДФЛ.Стандартные)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСтатусыФизЛицНаКонецНалоговогоПериода КАК СтатусыФизЛицНаКонецНалоговогоПериода
	|		ПО ВТСписокФизЛицОрганизация.ФизическоеЛицо = СтатусыФизЛицНаКонецНалоговогоПериода.ФизическоеЛицо
	|			И (ГОД(НДФЛПредоставленныеСтандартныеВычетыФизЛиц.МесяцНалоговогоПериода) = СтатусыФизЛицНаКонецНалоговогоПериода.Год)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМесяцыДоходовПереданныхВБанкНаАУСН КАК ДоходыПереданныеВБанкНаАУСН
	|		ПО ВТСписокФизЛицОрганизация.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
	|			И ВТСписокФизЛицОрганизация.ГоловнаяОрганизация = ДоходыПереданныеВБанкНаАУСН.ГоловнаяОрганизация
	|			И (НАЧАЛОПЕРИОДА(НДФЛПредоставленныеСтандартныеВычетыФизЛиц.МесяцНалоговогоПериода, МЕСЯЦ) = ДоходыПереданныеВБанкНаАУСН.МесяцНалоговогоПериода)
	|			И (НДФЛПредоставленныеСтандартныеВычетыФизЛиц.РегистрацияВНалоговомОргане = ДоходыПереданныеВБанкНаАУСН.РегистрацияВНалоговомОргане)
	|
	|СГРУППИРОВАТЬ ПО
	|	НДФЛПредоставленныеСтандартныеВычетыФизЛиц.ГоловнаяОрганизация,
	|	НДФЛПредоставленныеСтандартныеВычетыФизЛиц.ФизическоеЛицо,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	База.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	...
	|		И НЕ ДоходыПоСтавкам.ЗарегистрированоДляПередачиВБанкНаАУСН
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		СтандартныеИмущественныеВычеты.ГоловнаяОрганизация,
	...
	|		И НЕ СтандартныеИмущественныеВычеты.ЗарегистрированоДляПередачиВБанкНаАУСН
	...
	Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
		...
		|			И СведенияОДоходахНДФЛ.ГоловнаяОрганизация = СотрудникиСПревышением.ГоловнаяОрганизация
		|ГДЕ
		|	СведенияОДоходахНДФЛ.ОблагаетсяУНалоговогоАгента
		|	И СведенияОДоходахНДФЛ.ДатаПолученияДохода < &ДатаЗакона176ФЗ
		|	И НЕ СведенияОДоходахНДФЛ.ЗарегистрированоДляПередачиВБанкНаАУСН
		|
		|СГРУППИРОВАТЬ ПО
		...
		Если Не Результат.Пустой() Тогда
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	СтандартныеИмущественныеВычеты.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
			...
			|			И СтандартныеИмущественныеВычеты.ГоловнаяОрганизация = СотрудникиСПревышением.ГоловнаяОрганизация
			...
			Если ИскатьВычеты И ТаблицаВычетов.Количество() > 100 Тогда
				ТаблицаВычетов.Индексы.Добавить("ГоловнаяОрганизация,ФизическоеЛицо,НалоговаяБаза,НачалоМесяца");
			КонецЕсли;
			ПоискВычетов = Новый Структура("ГоловнаяОрганизация,ФизическоеЛицо,НачалоМесяца,НалоговаяБаза");
			...
			Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
				Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяГоловнаяОрганизация <> СтрокаТаблицы.ГоловнаяОрганизация Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Или ТекущийГод <> СтрокаТаблицы.Год Тогда
					ОблагаемыйДоходСНачалаГода = 0;
					ТекущаяГоловнаяОрганизация = СтрокаТаблицы.ГоловнаяОрганизация;
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	РасчетНарастающимИтогом.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
			...
			|	ДоходыПоСтавкам.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН,
			...
			|			И ДоходыПоСтавкам.ГоловнаяОрганизация = РасчетНарастающимИтогом.ГоловнаяОрганизация
			...
			|	РасширенныеДанныеОДоходах.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН,
			...
			|	РасширенныеДанныеОДоходах.ЗарегистрированоДляПередачиВБанкНаАУСН,
			...
			Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
				Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяГоловнаяОрганизация <> СтрокаТаблицы.ГоловнаяОрганизация Или ТекущийГод <> СтрокаТаблицы.Год Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Тогда
			...
			|	ДоходыСУточненнымиСтавками.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН,
			...
			|	ДоходыСУточненнымиСтавками.ЗарегистрированоДляПередачиВБанкНаАУСН,
			...
			|	ВременнаяТаблица.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН,
		...
		Если Не Результат.Пустой() Тогда
			...
			Если ИскатьВычеты И ТаблицаВычетов.Количество() > 100 Тогда
				ТаблицаВычетов.Индексы.Добавить("ГоловнаяОрганизация,ФизическоеЛицо,НалоговаяБаза,НачалоМесяца");
			КонецЕсли;
			ПоискВычетов = Новый Структура("ГоловнаяОрганизация,ФизическоеЛицо,НалоговаяБаза,НачалоМесяца");
	...
	|	НДФЛРасчетыСБюджетом.СрокПеречисления КАК СрокПеречисления,
	|	НДФЛРасчетыСБюджетом.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо ЕСТЬ НЕ NULL КАК ЗарегистрированоДляПередачиВБанкНаАУСН
	|ПОМЕСТИТЬ ВТДанныеОНалогах
	|ИЗ
	|	ВТСтрокиНалоговСоСтавками КАК НДФЛРасчетыСБюджетом
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыПереданныеВБанкНаАУСН КАК ДоходыПереданныеВБанкНаАУСН
	|		ПО НДФЛРасчетыСБюджетом.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
	|			И НДФЛРасчетыСБюджетом.ГоловнаяОрганизация = ДоходыПереданныеВБанкНаАУСН.ГоловнаяОрганизация
	|			И НДФЛРасчетыСБюджетом.МесяцНалоговогоПериода = ДоходыПереданныеВБанкНаАУСН.ДатаПолученияДохода
	|			И НДФЛРасчетыСБюджетом.РегистрацияВНалоговомОргане = ДоходыПереданныеВБанкНаАУСН.РегистрацияВНалоговомОргане
	|ГДЕ
	...
	|	И РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.Период < ДАТАВРЕМЯ(2024, 1, 1, 0, 0, 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ВычетыПоСтавкам.ЗарегистрированоДляПередачиВБанкНаАУСН
	|			ТОГДА ""АУСН""
	|		ИНАЧЕ ВычетыПоСтавкам.НалоговаяБаза
	|	КОНЕЦ КАК НалоговаяБаза,
	...
	|		КОГДА Доходы.ДатаПолученияДохода >= &ДатаЗакона176ФЗ
	|			ТОГДА ВЫБОР
	|					КОГДА Доходы.Раздел = ""Раздел4_Ставка13_АУСН""
	|						ТОГДА 1
	...
	|		И НЕ ДоходыПоМесяцам.ЗарегистрированоДляПередачиВБанкНаАУСН
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		""Раздел4_Ставка13_АУСН"",
	|		ДоходыПоМесяцам.Месяц,
	|		ЕСТЬNULL(ДоходыДоПолученияСтатусаРезидента.СуммаДохода, 0),
	|		ЕСТЬNULL(ДоходыДоПолученияСтатусаРезидента.СуммаВычета, 0),
	|		ЕСТЬNULL(ДоходыДоПолученияСтатусаРезидента.СуммаДохода, 0) - ЕСТЬNULL(ДоходыДоПолученияСтатусаРезидента.СуммаВычета, 0),
	...
	|	ГДЕ
	|		ДоходыПоМесяцам.ДоходРазделаПо13
	|		И ДоходыПоМесяцам.ЗарегистрированоДляПередачиВБанкНаАУСН
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ДанныеОНалогах.Ставка КАК Ставка,
	|	ВЫБОР
	|		КОГДА ДанныеОНалогах.ЗарегистрированоДляПередачиВБанкНаАУСН
	|			ТОГДА ""АУСН""
	|		ИНАЧЕ ДанныеОНалогах.НалоговаяБаза
	|	КОНЕЦ КАК НалоговаяБаза,
	...
	Если ВыборкаДоходовПоМесяцамПоРазделам.НайтиСледующий(ГоловнаяОрганизация, "ГоловнаяОрганизация") Тогда
		...
		Если ВыборкаФизЛиц.НайтиСледующий(ФизическоеЛицо, "ФизическоеЛицо") Тогда
			...
			Пока ВыборкаРегистраций.Следующий() Цикл
				...
				ВыводимыеОбласти.Очистить();
				Если ВыборкаПоРазделам.НайтиСледующий("Раздел4_Ставка13_АУСН", "Раздел") Тогда
					
					ИсчисленоШапка = Макет.ПолучитьОбласть("ИсчисленоШапкаБезДивидендов");
					ИсчисленоСтрока = Макет.ПолучитьОбласть("ИсчисленоСтрокаБезДивидендов");
					
					РасчетыНалогаПоСтавке13Заголовок.Параметры.ОписаниеСтавки = "13%";
					РасчетыНалогаПоСтавке13Заголовок.Параметры.НалоговаяБаза = "АУСН";
					РасчетыНалогаПоСтавке13Шапка = Макет.ПолучитьОбласть("НалогПоСтавке13ШапкаАУСН");
					РасчетыНалогаПоСтавке13Строка = Макет.ПолучитьОбласть("НалогПоСтавке13СтрокаАУСН");
					РасчетыНалогаПоСтавке13Итог = Макет.ПолучитьОбласть("НалогПоСтавке13ИтогАУСН");
					...
					Пока ВыборкаПоНалоговымБазам.Следующий() Цикл
						...
						Пока ВыборкаПоМесяцамНалоговогоПериода.Следующий() Цикл
							...
							ВсегоСтандартногоВычета = ?(ВычетыПоСтавке13.Получить(ВыборкаПоМесяцамНалоговогоПериода.МесяцНалоговогоПериода) = Неопределено, 0, ВычетыПоСтавке13.Получить(ВыборкаПоМесяцамНалоговогоПериода.МесяцНалоговогоПериода));
							...
							Пока ВыборкаПоДатамПолученияДохода.Следующий() Цикл
								...
								РасчетыНалогаПоСтавке13Строка.Параметры.ДатаВыплаты = Формат(ВыборкаПоДатамПолученияДохода.ДатаВыплаты, "ДЛФ=D");
								РасчетыНалогаПоСтавке13Строка.Параметры.ДатаПолученияДохода = Формат(ВыборкаПоДатамПолученияДохода.ДатаПолученияДохода, "ДЛФ=D");
								СуммаСтандартногоВычета = Мин(ВсегоСтандартногоВычета, ВыборкаПоДатамПолученияДохода.СуммаДохода - ВыборкаПоДатамПолученияДохода.СуммаВычета);
								ВсегоСтандартногоВычета = ВсегоСтандартногоВычета - СуммаСтандартногоВычета;
								ОблагаемаяСуммаДохода = ВыборкаПоДатамПолученияДохода.СуммаДохода - ВыборкаПоДатамПолученияДохода.СуммаВычета - СуммаСтандартногоВычета;
								НалогНачислено = Окр(ОблагаемаяСуммаДохода * 0.13);
								РасчетыНалогаПоСтавке13Строка.Параметры.СуммаСтандартногоВычета = СуммаСтандартногоВычета;
								РасчетыНалогаПоСтавке13Строка.Параметры.ОблагаемаяСуммаДохода = ОблагаемаяСуммаДохода;
								РасчетыНалогаПоСтавке13Строка.Параметры.НалогНачислено = НалогНачислено;
								
								УстановитьШрифтПоСумме(РасчетыНалогаПоСтавке13Строка, "НалогПоСтавке13СтрокаАУСНСуммаВычета", ВыборкаПоДатамПолученияДохода.СуммаВычета, СтандартныйШрифт, ШрифтРазмер8, ШрифтРазмер7);
								...
								ДокументРезультат.Вывести(РасчетыНалогаПоСтавке13Строка);	
								...
								СуммаСтандартногоВычетаИтог = СуммаСтандартногоВычетаИтог + СуммаСтандартногоВычета;
								ОблагаемаяСуммаДоходаИтог = ОблагаемаяСуммаДоходаИтог + ОблагаемаяСуммаДохода;
								...
								СтрокаТаблицыИтогов.НалоговаяБаза = ОблагаемаяСуммаДохода;
						...
						РасчетыНалогаПоСтавке13Итог.Параметры.Заполнить(ВыборкаПоМесяцамНалоговогоПериода);
						РасчетыНалогаПоСтавке13Итог.Параметры.СуммаДохода = СуммаДоходаИтог;
						РасчетыНалогаПоСтавке13Итог.Параметры.СуммаВычета = СуммаВычетовИтог;
						РасчетыНалогаПоСтавке13Итог.Параметры.СуммаСтандартногоВычета = СуммаСтандартногоВычетаИтог;
						РасчетыНалогаПоСтавке13Итог.Параметры.ОблагаемаяСуммаДохода = ОблагаемаяСуммаДоходаИтог;
						РасчетыНалогаПоСтавке13Итог.Параметры.НалогНачислено = НалогНачисленоИтог;
						
						УстановитьШрифтПоСумме(РасчетыНалогаПоСтавке13Итог, "НалогПоСтавке13ИтогАУСНСуммаВычета", СуммаВычетовИтог, СтандартныйШрифт, ШрифтРазмер8, ШрифтРазмер7);
						
						ДокументРезультат.Вывести(РасчетыНалогаПоСтавке13Итог);	
