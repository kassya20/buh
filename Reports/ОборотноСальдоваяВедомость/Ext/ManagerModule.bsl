...
Процедура СформироватьСтруктуруОтчета(КомпоновщикНастроек, ПараметрыОтчета, ПараметрыАлгоритма)
	...
	Поле = УсловноеОформление.Поля.Элементы.Добавить();
	Поле.Поле = Новый ПолеКомпоновкиДанных("Показатели.СверкаНУ");
	Поле = УсловноеОформление.Поля.Элементы.Добавить();
...
Процедура ДоработатьНаборДанных_ПоСубконтоРазвернутое(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	// Подготовим полный список соответствий счетов и субконто
	// из всех подзапросов
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		Если ОписаниеПодзапроса.ПоСубсчетам Тогда
			СчетаИтогов = БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаПодзапроса);
		Иначе  
			СчетаИтогов = СчетаПодзапроса;
		КонецЕсли;     
	...
	// Массив включенных счетов в подзапросы 
	СчетаВключенныеВПодзапрос = Новый Массив;   
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		Для Каждого Счет Из СчетаПодзапроса Цикл
			...
			Если Исключения <> Неопределено Тогда
				Для Каждого СчетИсключения Из Исключения Цикл
					...
					// что счет и его субсчет могут быть отнесены к одному подзапросу,
					// то исключать субсчет не следует.
					Если СчетаПодзапроса.Найти(СчетИсключения) = Неопределено 
						 И СчетаСПодходящимСоставомСубконто.НайтиПоЗначению(СчетИсключения) = Неопределено Тогда     
		...
		// Исключим ранее включенные счета
		СчетаРазвернутоеСальдо = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаРазвернутоеСальдо, СчетаВключенныеВПодзапрос);

		// Запомним счета включенные в подзапрос   
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаВключенныеВПодзапрос, СчетаРазвернутоеСальдо, Истина);
