...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Стр = 702 По 708 Цикл
		...
		Для Гр = 4 По 9 Цикл
			...
			КонецЕсли;
			Графа	  = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаПоГр = СуммаПоГр + Области["П0130010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0130010" + Стр + "03"].Значение = СуммаПоГр;
	КонецЦикла;
	...
	Для Гр = 3 По 9 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 702 По 708 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0130010" + Стр + Графа].Значение;
		КонецЦикла;
		Области["П0130010701" + Графа].Значение = СуммаПоСтр;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Гр = 3 По 9 Цикл
		...
		Для Стр = 701 По 708 Цикл
			Если Не (Гр > 3 И Стр > 701) Тогда
				мФормаОтчета.Области["П0130010" + Стр + Графа].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0130010" + Стр + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_5"));
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть8_6"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 2-Наука");
