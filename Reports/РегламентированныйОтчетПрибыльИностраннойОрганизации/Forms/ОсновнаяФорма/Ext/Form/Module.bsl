...
Процедура ПоказатьПериод(Форма)
	...
	Если ГодПериода >= 2011 И ГодПериода < 2026 Тогда
	...
	ФормаУтратилаСилу = ГодПериода >= 2026;
	...
	Иначе
		...
		Форма.ОписаниеНормативДок = ?(ФормаУтратилаСилу, НСтр(
			"ru='Утратила силу в связи с приказом ФНС России от 02.10.2024 № ЕД-7-3/830@.'"),
	...
	Если ГодПериода < 2026 Тогда
		Форма.Элементы.НадписьКтоСдаетОтчет.Видимость = Истина;
		Форма.НадписьКтоСдаетОтчет = НСтр("ru='Иностранные организации (абз.3 п.1 ст.246 НК РФ).'");
	Иначе
		Форма.Элементы.НадписьКтоСдаетОтчет.Видимость = Ложь;
	КонецЕсли;
	
	// Для периодов ранее 2013 года ссылку Изменения законадательства скрываем.
	Форма.Элементы.ПолеСсылкаИзмененияЗаконодательства.Видимость = ГодПериода > 2012 И ГодПериода < 2026;
