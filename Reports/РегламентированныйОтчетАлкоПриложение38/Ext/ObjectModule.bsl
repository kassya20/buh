...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоЛицензии" Тогда
		...
		РегламентированнаяОтчетностьАЛКО.ОбработкаСохраненияАдреса(
						Запись.П0000000VNUTR, Запись.П0000000Адрес, СтруктураАдреса);
		...
		Если НЕ АдресVNUTR = Запись.П0000000VNUTR Тогда
			...
			Запись.П000000000205 = СтруктураАдреса.Город 
								+ ?(ЗначениеЗаполнено(СтруктураАдреса.НаселенныйПункт), " / ", "")
								+ СтруктураАдреса.НаселенныйПункт;
			Запись.П000000000206 = СтруктураАдреса.Улица;
			Запись.П000000000207 = СтруктураАдреса.Дом
								+ ?(ЗначениеЗаполнено(СтруктураАдреса.Корпус), ", ", "")
								+ СтруктураАдреса.Корпус
								+ ?(ЗначениеЗаполнено(СтруктураАдреса.Литера), ", ", "")
								+ СтруктураАдреса.Литера;
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение38.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение38.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение38.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение38.ДеревоФормИФорматов();
