...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Отчет.ПоказыватьБыстрыеНастройки = Ложь;
...
Процедура РезультатВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Область.ГиперСсылка
		...
		Если ЕстьПравоПосмотраНастройкиПользователейИПрав() Тогда
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.ПоказательКоличество              = Отчет.ПоказательКоличество 
		И КлючТекущегоВарианта <> "ПродажиПоКонтрагентамПоОплате";
...
Функция ПолучитьЗапрещенныеПоля(Режим = "") Экспорт
	...
	Если Не ИспользуетсяНесколькоОтветственных Или КлючТекущегоВарианта = "ПродажиПоКонтрагентамПоОплате" Тогда
...
Процедура ПоказатьБыстрыеНастройкиНаСервере()
	...
	Если Элементы.МакетОформленияБыстрыеНастройки.СписокВыбора.Количество() > 0
		И (ПустаяСтрока(МакетОформления) Или МакетОформления = "МакетОформленияОтчетовЗеленый") Тогда
		МакетОформления = Элементы.МакетОформленияБыстрыеНастройки.СписокВыбора[0].Значение;
	КонецЕсли;
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") Тогда
		Если ЗаполняемыеНастройки.Группировка Тогда
			// Добавление группировок в соответствии с выбранным счетом	
			...
			НоваяСтрока = Отчет.Группировка.Добавить();
			НоваяСтрока.Поле           = "Ответственный";
			НоваяСтрока.Использование  = КлючТекущегоВарианта = "ПродажиПоОтветственным";
			НоваяСтрока.Представление  = НСтр("ru = 'Ответственный'");
			НоваяСтрока.ТипГруппировки = 0;
			
			Если КлючТекущегоВарианта = "ПродажиПоКонтрагентам" 
				Или КлючТекущегоВарианта = "ПродажиПоКонтрагентамПоОплате" Тогда
				НоваяСтрока = Отчет.Группировка.Добавить();
			...
			Если КлючТекущегоВарианта = "ПродажиПоНоменклатуре" Тогда
				НоваяСтрока = Отчет.Группировка.Добавить();
			...
			Если КлючТекущегоВарианта = "ПродажиПоНоменклатурнымГруппам" Тогда
				НоваяСтрока = Отчет.Группировка.Добавить();
				НоваяСтрока.Поле           = "НоменклатурнаяГруппа";
				НоваяСтрока.Использование  = Истина;
				НоваяСтрока.Представление  = НСтр("ru = 'Номенклатурная группа'");
				НоваяСтрока.ТипГруппировки = 0;
			КонецЕсли;
...
Процедура УправлениеВидимостьюПоляОтветственный()
	...
	Если КлючТекущегоВарианта = "ПродажиПоОтветственным" Тогда
		Возврат;
	КонецЕсли;
...
Функция ЕстьПравоПосмотраНастройкиПользователейИПрав()
