...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ПараметрыОтчета.ПоказательКоличество = ПараметрыОтчета.ПоказательКоличество
		И Не ПараметрыОтчета.ПоказательОпределятьСуммуПоОплате;
	...
	ТекстЗапроса = Схема.НаборыДанных.Продажи.Запрос;

	Схема.НаборыДанных.Продажи.Запрос = СтрЗаменить(ТекстЗапроса, 
		"НАЧАЛОПЕРИОДА(ЕСТЬNULL(ВозвратТоваровОтПокупателя.Дата, РеализацияТоваровУслуг.Дата), ДЕНЬ)",
		"НАЧАЛОПЕРИОДА(ЕСТЬNULL(ВозвратТоваровОтПокупателя.Дата, РеализацияТоваровУслуг.Дата), "
		+ ПериодичностьОтчета.Получить(Периодичность) + ")");
	...
	Если ИспользоватьНесколькоОтветственных Тогда
		ТекстПоляОптоваяПродажаОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"ОптоваяПродажа",
			"Регистратор",
		...
		ТекстПоляРозничнаяПродажаОтветственный = "ВЫБОР
			|КОГДА РозничнаяПродажа.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
			|	ТОГДА ВЫРАЗИТЬ(РозничнаяПродажа.Регистратор КАК Документ.ОтчетОРозничныхПродажах).Ответственный
			|ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
			|КОНЕЦ";
		ТекстПоляКорректировкиОтветственный = "ЕСТЬNULL(ВозвратТоваровОтПокупателя.Ответственный, РеализацияТоваровУслуг.Ответственный)";
	Иначе
		ТекстПоляОптоваяПродажаОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
		ТекстПоляРозничнаяПродажаОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
		ТекстПоляКорректировкиОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстПоляРозничнаяПродажаОтветственный",
		ТекстПоляРозничнаяПродажаОтветственный);
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстПоляКорректировкиОтветственный",
		ТекстПоляКорректировкиОтветственный);
	Схема.НаборыДанных.Продажи.Запрос = ТекстЗапроса;
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.КонецПериода) Тогда
		...
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "КонецМесяца", КонецМесяца(ПараметрыОтчета.КонецПериода));
	Иначе
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ПараметрПериод", КонецДня(ТекущаяДатаСеанса()));
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "КонецМесяца", КонецМесяца(ТекущаяДатаСеанса()));
	КонецЕсли;
		...
		Если ВыводитьДиаграмму И ПараметрыОтчета.ПоказательСумма Тогда
			...
			Для Каждого ПолеВыбраннойГруппировки Из ПараметрыОтчета.Группировка Цикл 
				Если ПолеВыбраннойГруппировки.Использование Тогда
					Если ПолеВыбраннойГруппировки.Поле = "Организация" Тогда
	...
	Если ДиаграммаКоличество <> Неопределено Тогда
		...
		Если ВыводитьДиаграмму И ПараметрыОтчета.ПоказательКоличество 
...
Процедура НастроитьВариантыОтчета(Настройки, ОписаниеОтчета) Экспорт
	...
	ПродажиПоОтветственным = ВариантыОтчетов.ОписаниеВарианта(Настройки, ОписаниеОтчета, "ПродажиПоОтветственным");
	ПродажиПоОтветственным.ФункциональныеОпции.Добавить("ИспользоватьНесколькоОтветственных");
	ПродажиПоОтветственным.Размещение.Вставить(Метаданные.Подсистемы.Руководителю.Подсистемы.Продажи, "");
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	ДополнительныеСвойства.Вставить("КонецМесяца",             КонецМесяца(ДанныеОтчета.Объект.КонецПериода));
	...
	ДополнительныеСвойства.Вставить("ПоказательКоличество",    ДанныеОтчета.Объект.ПоказательКоличество 
		И Не ДанныеОтчета.Объект.ПоказательОпределятьСуммуПоОплате);
	...
	Если ЗначениеЗаполнено(Период) Тогда
		...
		ДополнительныеСвойства.Вставить("КонецМесяца", КонецМесяца(БухгалтерскиеОтчетыКлиентСервер.КонецПериода(Период, Периодичность)));
	...
	Для Каждого ЗначениеРасшифровки Из Данные_Расшифровки Цикл
		Если ЗначениеРасшифровки.Ключ <> "Период" Тогда
			ТипЗначения = ТипЗнч(ЗначениеРасшифровки.Значение);
			ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначения);
			ВидСравненияОтбор = ВидСравненияКомпоновкиДанных.Равно;
			Если ОбъектМетаданных <> Неопределено 
				И ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) 
				И ЗначениеРасшифровки.Значение.ЭтоГруппа Тогда
					ВидСравненияОтбор = ВидСравненияКомпоновкиДанных.ВИерархии;
			КонецЕсли;
			БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ОтборПоЗначениямРасшифровки, ЗначениеРасшифровки.Ключ,
				ЗначениеРасшифровки.Значение, ВидСравненияОтбор);
		КонецЕсли;
	КонецЦикла;
...
Функция ВариантыНастроек() Экспорт
	...
	Массив.Добавить(Новый Структура("Имя, Представление","ПродажиПоОтветственным", НСтр("ru = 'Продажи по ответственным'")));
...
Процедура ВывестиПримечания(ПараметрыОтчета, Результат)
	...
	Если ПараметрыОтчета.КлючТекущегоВарианта = "ПродажиПоОтветственным" Тогда
		Примечания.Параметры.Ссылка = "e1cib/command/Обработка.ПанельАдминистрированияБСП.Команда.НастройкиПользователейИПрав";
	КонецЕсли;
	...
	Если ПараметрыОтчета.КлючТекущегоВарианта = "ПродажиПоОтветственным" Тогда
		ПримечаниеОтветственныйПоПродажам = Примечания.Область("ИзменитьЗаполнениеОтветственногоПоПродажам");
		
		Результат.ВставитьОбласть(ПримечаниеОтветственныйПоПродажам,
