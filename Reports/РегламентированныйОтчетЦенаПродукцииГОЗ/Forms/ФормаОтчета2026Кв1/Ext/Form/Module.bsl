...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "08/04/2026";
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = ИзвлечьСтруктуруXML("СхемаВыгрузки2026Кв1");
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = ИзвлечьСтруктуруXML("СхемаВыгрузки2026Кв1");
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	ДеревоВыгрузки = ИзвлечьСтруктуруXML("СхемаВыгрузки2026Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Не ЗначениеЗаполнено(Титульный.ReportDate) Тогда 
		СообщитьВТС("Не дата указана составления кассового плана.", "Титульный", "ReportDate");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Титульный.Type) Тогда 
		...
		СтатусПроверки = Ложь;
	КонецЕсли;
	Если ПараметрыВыгрузки.Type = "07" Тогда 
	...
	БлокДанных = "InvoiceNum,InvoiceDate,InvoiceComment,ActNum,ActDate,ActComment,BillNum,BillDate,BillComment";
	Если Титульный.Type = "07" Или УведомлениеОСпецрежимахНалогообложения.БлокЗаполнен(Титульный, БлокДанных) Тогда 
...
Функция ВыгрузитьДеревоВXML(ДеревоВыгрузки, ПараметрыВыгрузки)
	ПотокXML = РегламентированнаяОтчетность.СоздатьНовыйПотокXML("utf-8");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПостфиксПФ = ?(СтруктураДанныхТитульный.Type = "08" , "Н", "П");
	
	МакетПФ = ОбъектОтчета(ИмяФормы).ПолучитьМакет("ПечатныйБланк2026Кв1");
	ПечатнаяФорма = НовыйТабличныйДокументДляПечати(ОриентацияСтраницы.Портрет);
	ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("Заголовок1"));
	ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("Заголовок2"));
	ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("Заголовок3"));
	...
	ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("Заголовок5" + ПостфиксПФ));
	...
	ЗаголовокТЧ = МакетПФ.ПолучитьОбласть("КалькуляцияЗаголовок");
	Для Каждого Стр Из Калькуляция.Строки Цикл
		...
		ПечатнаяФорма.Вывести(МакетПФ.ПолучитьОбласть("КалькуляцияНиз"));
