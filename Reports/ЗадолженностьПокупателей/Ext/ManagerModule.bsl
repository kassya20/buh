...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ИспользоватьНесколькоОтветственных Тогда
		ТекстПоляХозрасчетныйОборотыОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"ХозрасчетныйОбороты",
			"Субконто3",
		...
		ТекстПоляНачальныеОстаткиОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"НачальныеОстатки",
			"Субконто3",
			ОтчетыРуководителя.ТипыДокументовОптовойПродажи(),
			,
		...
		ТекстПоляКонечныеОстаткиОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"КонечныеОстатки",
			"Субконто3",
			ОтчетыРуководителя.ТипыДокументовОптовойПродажи(),
			,
	...
	Иначе
		ТекстПоляХозрасчетныйОборотыОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
		ТекстПоляНачальныеОстаткиОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
		ТекстПоляКонечныеОстаткиОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
	КонецЕсли;
	...
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстПоляНачальныеОстаткиОтветственный",
		ТекстПоляНачальныеОстаткиОтветственный);
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстПоляКонечныеОстаткиОтветственный",
		ТекстПоляКонечныеОстаткиОтветственный);
...
Функция ПолучитьЗадолженностьПокупателейДляМонитораРуководителя(Организация, ДатаЗадолженности) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И ВЫРАЗИТЬ(Субконто2 КАК Справочник.ДоговорыКонтрагентов).ВидДоговора В (ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СПокупателем), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СТранспортнойКомпанией), ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку))) КАК ХозрасчетныйОстатки
	               |ГДЕ
