...
Процедура ПоказатьПериод(Форма)
	...
	Если Форма.ДатаПодписи < '20260613'
		И Форма.РезультирующаяТаблица.Количество() = 2
		И СокрЛП(Форма.РезультирующаяТаблица[0].ФормаОтчета) = "ФормаОтчета2026Кв2"
		И СокрЛП(Форма.РезультирующаяТаблица[1].ФормаОтчета) = "ФормаОтчета2026Кв1" Тогда
