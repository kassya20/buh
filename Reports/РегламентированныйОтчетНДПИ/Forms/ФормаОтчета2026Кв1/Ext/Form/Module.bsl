...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаПредставлениеФормы.Видимость
			= КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= КонецДня('20260531');
	КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел3" Тогда
		...
		Если НЕ ЗначениеЗаполнено(ОбрабатываемаяОбласть.Значение) Тогда
			ЗапретитьВводДанныхВОбласть(Форма, ОбрабатываемаяОбласть);
		КонецЕсли;
		
		ТДО["СекцияПояснение096"].Видимость = КодВидаПИ <> "03200";
	...
	ИначеЕсли ИмяРаздела = "Раздел33" Тогда
		...
		ТДО["СекцияПояснениеРекомендФНС"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение390"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение400"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение410"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение430"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение450"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
		ТДО["СекцияПояснение480"].Видимость
			= Форма.СтруктураРеквизитовФормы["РодительскаяСтраницаРаздела3_КодВидаПИ"] <> "03200";
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел5" ИЛИ ИмяРаздела = "Раздел51" ИЛИ ИмяРаздела = "Раздел52" ИЛИ ИмяРаздела = "Раздел53" Тогда
		...
		Для каждого Ветка_5 Из ДеревоДанных.Строки Цикл
			...
			//	СуммаСтр135 = СуммаСтр135 + Окр(Ветка_54.Данные["П000540034003"], 2);
			//КонецЦикла;
			...
			Для каждого Ветка_51 Из Ветка_5.Строки[0].Строки Цикл
				...
				//// Стр.135.
				//Ветка_51.Данные.Вставить("П000510013503", СуммаСтр135);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если КонецДня(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) >= КонецДня('20260531')
		И СтруктураРеквизитовФормы.мСохраненныйДок.Вид = 0 И ТекущаяДатаСеанса() >= '20260613' Тогда
		
		СообщитьВТС("С 13.06.2026 представление первичной декларации, начиная с отчета за период Май 2026 года, "
			+ "доступно только по форме в редакции приказа ФНС России от 10.03.2026 № ЕД-1-3/155@",
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел3") Тогда
		...
		Для каждого Ветка_3 Из ДеревоРаздела.Строки Цикл
			...
			Если Окр(Ветка_3.Данные["П000030009603"]) <> 0 Тогда
				СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы_3
				+ ": строка 096 не должна быть заполнена (с 01.01.2026 утратил силу п. 18 ст. 343 НК РФ)",
				"Раздел3", "П000030009603", НомСтраницы_3);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого Ветка_32 Из Ветка_3.Строки[1].Строки Цикл
				...
				Для каждого Пок Из Ветка_32.Данные Цикл
					Если СтрНачинаетсяС(Пок.Ключ, "П0") И ЗначениеЗаполнено(Пок.Значение) Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы_3 + " -> Подраздел 3.2 -> Страница " + НомСтраницы_32
							+ ": подраздел 3.2 не должен быть заполнен (с 01.01.2026 утратил силу п. 18 ст. 343 НК РФ)",
							"Раздел32", Пок.Ключ, Координаты_32);
						СтатусПроверки = Ложь;
						
						ЗаполненПодр32 = Истина;
						
					КонецЕсли;
					
					Если ЗаполненПодр32 Тогда
						Прервать;
					КонецЕсли;
				...
				Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
					...
					Для каждого Пок Из СтрокаМнЧ.Данные Цикл
						...
						Если СтрНачинаетсяС(Пок.Ключ, "П0") И ЗначениеЗаполнено(Пок.Значение) Тогда
							...
							СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы_3 + " -> Подраздел 3.2 -> Страница " + НомСтраницы_32
							+ ": подраздел 3.2 не должен быть заполнен (с 01.01.2026 утратил силу п. 18 ст. 343 НК РФ)",
							"Раздел32", Пок.Ключ + "_" + НомСтроки, Координаты_32);
							СтатусПроверки = Ложь;
			...
			ИначеЕсли ЗаполненПодр33 Тогда
				...
				Для каждого Ветка_33 Из Ветка_3.Строки[2].Строки Цикл
					...
					Если Ветка_33.Данные["П000330043003"] <> 0 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы_3 + " -> Подраздел 3.3 -> Страница " + НомСтраницы_33
						+ ": значение в строке 430 должно быть равно 0 (с 01.01.2026 внесены изменения в п. 19 ст. 343 НК РФ)",
						"Раздел33", "П000330043003", Координаты_33);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если Ветка_33.Данные["П000330048003"] <> 0 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы_3 + " -> Подраздел 3.3 -> Страница " + НомСтраницы_33
						+ ": значение в строке 480 должно быть равно 0 (с 01.01.2026 внесены изменения в п. 19 ст. 343 НК РФ)",
						"Раздел33", "П000330048003", Координаты_33);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	Для каждого Ветка_3 Из ДеревоРаздела.Строки Цикл
		...
		// 3.2.
		// С 01.01.2026 утратил силу п. 18 ст. 343 НК РФ. Подраздел 3.2 не заполняется.
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	|от 13.11.2025 № ЕД-7-3/976@ будет реализована
...
Процедура ПечатьРаздел5(КонтекстПечати)
	Показатели511 = "П000510007003,П000510009003,П000510010003,П000510010503,П000510011003,П000510012003,П000510013003,"
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	ИначеЕсли Ячейка.Раздел = "Раздел21" ИЛИ Ячейка.Раздел = "Раздел211" ИЛИ Ячейка.Раздел = "Раздел212"
		...
		ИЛИ Ячейка.Раздел = "Раздел54" ИЛИ Ячейка.Раздел = "Раздел71" ИЛИ Ячейка.Раздел = "Раздел81" Тогда
