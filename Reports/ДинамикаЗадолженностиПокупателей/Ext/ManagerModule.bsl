...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	КолонкиЗадолженности.Добавить("Документ");
	КолонкиЗадолженности.Добавить("Ответственный");
	...
	Для Каждого Период Из ТаблицаПериоды Цикл
		ВременнаяТаблица = СрокиОплатыДокументов.ПросроченнаяЗадолженностьПокупателей(
			ПараметрыОтчета.Организация,
			Период.ПериодКонец,
			ПараметрыОтчета.ВключатьОбособленныеПодразделения,
			,
			ИспользоватьНесколькоОтветственных);
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ТекстЗапроса = Схема.НаборыДанных.НаборДанныхОбъединение.Элементы[0].Запрос;
	
	ИспользоватьНесколькоОтветственных = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОтветственных");
	Если ИспользоватьНесколькоОтветственных Тогда
		ТекстПоляЗадолженностьОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"Задолженность",
			"Субконто3",
	...
	Иначе
		ТекстПоляЗадолженностьОтветственный = ОтчетыРуководителя.ТекстПоляОтветственныйПоУмолчанию();
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(
		ТекстЗапроса,
		"&ТекстПоляЗадолженностьОтветственный",
		ТекстПоляЗадолженностьОтветственный);
	Схема.НаборыДанных.НаборДанныхОбъединение.Элементы[0].Запрос = ТекстЗапроса;
