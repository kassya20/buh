...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0040010001", ";", Истина);
КонецФункции
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли ТекГруппа = "П000101" Тогда
			...
			КонецЕсли;
			Модифицированность = Истина;
		ИначеЕсли (СтрНайти(Область.Имя, "П004001000101_") = 1 Или СтрНайти(Область.Имя, "П004001000103_") = 1) Тогда
			...
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникВидыДеятельности");
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	ИмяЯчейкиНомерСроки = "П004001000102_1";
	Если Не ЗначениеЗаполнено(Области[ИмяЯчейкиНомерСроки].Значение) Тогда
		Области[ИмяЯчейкиНомерСроки].Значение = "4";
	КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ИП");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1_5"));
	
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть2")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ИП");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникВидыДеятельности",
		512, , "Выбор вида деятельности", КоллекцияСписковВыбора["ВидыДеятельности"]);
...
Функция ЗаполнитьАвтоНаСервере()
	...
	Контейнер.Вставить("ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1",
		...
		ЭтотОбъект, ЭтотОбъект.П0040010001, Ложь));
