...
Процедура Проверить(Команда)
	...
	|от 26.11.2025 № ЕД-7-3/1017@, будет реализована
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбъектОтчета(ИмяФормы).СформироватьСтруктуруРеквизитовФормы(Параметры.мВыбраннаяФорма, СтруктураРеквизитовФормы);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено
		...
		Если НЕ СтруктураРеквизитовФормы.ИП Тогда
			...
			СтруктураДанныхРаздел21.Вставить("П002110015003", 0);
			СтруктураДанныхРаздел21.Вставить("П002110016003", 0);
			СтруктураДанныхРаздел21.Вставить("П002110016103", 0);
			СтруктураДанныхРаздел21.Вставить("П002110016203", 0);
			
			СтруктураДанныхРаздел22.Вставить("П000220029003", 0);
			СтруктураДанныхРаздел22.Вставить("П000220030003", 0);
			СтруктураДанныхРаздел22.Вставить("П000220031003", 0);
			СтруктураДанныхРаздел22.Вставить("П000220032003", 0);
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.декл.УСН 25к1",
		'20260115') Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[СтруктураРеквизитовФормы.НаименованиеОтчета].ПолучитьМакет("СпискиВыбора2025Кв1");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СписокСохранения.Свойство("ВерсияФормы", ВерсияФормы) Тогда
		ВерсияФормы = ?(ВерсияФормы = "01/01/2024", "01/01/2025", ВерсияФормы);
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если СтрРазделить("П002110012003,П002110012103,П002110012203,П002110012303", ",", Ложь).Найти(ИмяПоказателя)
		<> Неопределено И ЗначПоказателя > 6 Тогда
			Область.Значение = 6;
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Значение налоговой ставки не должно превышать 6%'"));
	КонецЕсли;
	
	Если СтрРазделить("П000220026003,П000220026103,П000220026203,П000220026303", ",", Ложь).Найти(ИмяПоказателя)
		<> Неопределено И ЗначПоказателя > 15 Тогда
			Область.Значение = 15;
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Значение налоговой ставки не должно превышать 15%'"));
	КонецЕсли;
	...
	Если ИмяПоказателя = "П000040005003" И Окр(ЗначПоказателя) > 28000 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
			"ru='Сумма расходов на приобретение экземпляра контрольно-кассовой техники не должна превышать 28 000 рублей'"));
	КонецЕсли;
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет =Отчеты[СтруктураРеквизитовФормы.НаименованиеОтчета].ПолучитьМакет("СхемаВыгрузки509");
...
Процедура ПоказатьПодсказку(Знач ИмяПоказателя)
	...
	ЗаголовокПодсказки = ?(
		Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Титульный", 
		ПодсказкаСвойства.Наименование, 
		ДинамическийЗаголовокПодсказки(ПодсказкаСвойства));
...
&НаКлиенте
Функция ДинамическийЗаголовокПодсказки(ПодсказкаСвойства)
				
	ПодстрокаВидВладельца = "";
	ПодстрокаВладелец = "";
	Если ЗначениеЗаполнено(ПодсказкаСвойства.КодСтроки) Тогда
		ПодстрокаВидВладельца = "Строка";
		ПодстрокаВладелец = ПодсказкаСвойства.КодСтроки;
	ИначеЕсли ЗначениеЗаполнено(ПодсказкаСвойства.КодКолонки) Тогда
		ПодстрокаВидВладельца = "Графа";
		ПодстрокаВладелец = ПодсказкаСвойства.КодКолонки;
	КонецЕсли;
	
	ИмяПоказателя = ?(
		ЗначениеЗаполнено(ПодсказкаСвойства.КодСтроки), 
		СтрШаблон("%1 %2", ПодстрокаВидВладельца, ПодсказкаСвойства.КодСтроки), "");
	
	ЗаголовокПодсказки = СтрШаблон("%1 %2", ПодстрокаВидВладельца, ПодстрокаВладелец);
	
	Возврат ЗаголовокПодсказки;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяСтраницыПанели = "")
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел21" Тогда
		...
		Если НЕ Форма.СтруктураРеквизитовФормы.ИП Тогда
			...
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П002110015003);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П002110016003);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П002110016103);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П002110016203);
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел22" Тогда
		...
		Если НЕ Форма.СтруктураРеквизитовФормы.ИП Тогда
			...
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П000220029003);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П000220030003);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П000220031003);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО.П000220032003);
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ОбъектОтчета(ИмяФормы).УстановитьКонтекстныеПодсказки(ЭтотОбъект, ИмяТекРаздела);
