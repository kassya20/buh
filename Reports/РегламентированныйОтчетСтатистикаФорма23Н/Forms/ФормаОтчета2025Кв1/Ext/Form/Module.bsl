...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010010001;П0030010002;П0030010004;П0040010001", ";", Истина);
КонецФункции
...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		ИначеЕсли Стр = "П0040010001" Тогда
			...
			КолКолонок = 5;
			Для Гр = 1 По КолКолонок Цикл
				Если Гр = 1 Или Гр = 2 Или Гр = 3 Тогда
					ТипКолонки = Новый ОписаниеТипов("Строка");
				ИначеЕсли Гр = 4 Или Гр = 5 Тогда
					ТипКолонки = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(21, 3));
				КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНайти(Область.Имя, "П004001000103_") > 0 Тогда
...
Функция ПечатьНаСервере(ВидПечати)
	...
	МассивПроверкиВывода = Новый Массив;
	МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_1"));
	МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_2"));
	МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_3"));
	...
	Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
		...
		МассивПроверкиВывода = Новый Массив;
		МассивПроверкиВывода.Добавить(ТекущаяОбластьМногострочнойЧасти);
		МассивПроверкиВывода.Добавить(ТабДок.ПолучитьОбласть("ОсновнаяЧасть4_4"));
		Если ПечатнаяФорма.ПроверитьВывод(МассивПроверкиВывода) Тогда
			ПечатнаяФорма.Вывести(ТекущаяОбластьМногострочнойЧасти);
		Иначе
			СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 23-Н");
