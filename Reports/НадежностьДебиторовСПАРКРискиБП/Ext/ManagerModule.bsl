...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ГруппировкаБаннер = БухгалтерскиеОтчеты.НайтиПоИмени(КомпоновщикНастроек.Настройки.Структура, "Баннер1СПАРК");
...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	// Основной вариант отчета (НадежностьДебиторовСПАРКРискиБП) настраивается по умолчанию.
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
		...
		Иначе
			...
			Для Каждого ПолеРасшифровки ИЗ ЗначениеРасшифровки.ПолучитьПоля() Цикл
				...
				ИначеЕсли ПолеРасшифровки.Поле = "СводныйИндикатор" Тогда
					ПараметрыРасшифровки.Вставить("ОткрытьФорму", Истина);
					ПараметрыРасшифровки.Вставить("Форма", "Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАКРРиски");
					ПараметрыРасшифровки.Вставить("ПараметрыФормы", Новый Структура("ВидФормы", "SPARK:WhatIsCompositeIndex"));
				ИначеЕсли ПолеРасшифровки.Поле = "ИндексДолжнойОсмотрительности" Тогда
					ПараметрыРасшифровки.Вставить("ОткрытьФорму", Истина);
					ПараметрыРасшифровки.Вставить("Форма", "Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАКРРиски");
					ПараметрыРасшифровки.Вставить("ПараметрыФормы", Новый Структура("ВидФормы", "SPARK:WhatIsIndexOfDueDiligence"));
				ИначеЕсли ПолеРасшифровки.Поле = "ИндексФинансовогоРиска" Тогда
					ПараметрыРасшифровки.Вставить("ОткрытьФорму", Истина);
					ПараметрыРасшифровки.Вставить("Форма", "Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАКРРиски");
					ПараметрыРасшифровки.Вставить("ПараметрыФормы", Новый Структура("ВидФормы", "SPARK:WhatIsFailureScore"));
				ИначеЕсли ПолеРасшифровки.Поле = "ИндексПлатежнойДисциплины" Тогда
					ПараметрыРасшифровки.Вставить("ОткрытьФорму", Истина);
					ПараметрыРасшифровки.Вставить("Форма", "Обработка.СправкаСПАРКРиски.Форма.РасшифровкаДанныхСПАКРРиски");
					ПараметрыРасшифровки.Вставить("ПараметрыФормы", Новый Структура("ВидФормы", "SPARK:WhatIsPaymentIndex"));
	...
	СписокПунктовМеню.Добавить("НадежностьДебиторовСПАРКРискиБП", НСтр("ru='Надежность дебиторов'"));
	
	НастройкиРасшифровки = Новый Структура();
	НастройкиРасшифровки.Вставить("НадежностьДебиторовСПАРКРискиБП", ПользовательскиеНастройки);
...
Функция ВариантыНастроек() Экспорт
	...
	Массив.Добавить(Новый Структура("Имя, Представление",
		"НадежностьДебиторовСПАРКРиски", НСтр("ru = 'Надежность дебиторов'")));
	Массив.Добавить(Новый Структура("Имя, Представление",
		"СписокКонтрагентовДляОбновления", НСтр("ru = 'Список данных для обновления'")));
	Массив.Добавить(Новый Структура("Имя, Представление",
		"Расшифровка", НСтр("ru = 'Расшифровка дебиторской задолженности'")));
