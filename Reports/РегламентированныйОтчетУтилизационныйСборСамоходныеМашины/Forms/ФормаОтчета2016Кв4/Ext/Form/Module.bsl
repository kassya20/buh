...
Процедура ПроверитьКС_2_03(СтруктураРезультата)
	...
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Постановление РФ от 06.02.2016 № 81'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru='Базовая ставка в строке 110 Раздела 2 должна иметь значение '") + XMLСтрока(БазоваяСтавка));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1'"));
	Заголовки.Вставить("ШаблонРасшифровки"	, 
...
Процедура ПроверитьКС_2_04(СтруктураРезультата)
	...
	Для НомерСтраницы = 1 По мДанныеОтчета.Количество Цикл
		...
		Если ПоказателиТитульного.КатегорияПлательщика.Значение = НСтр("ru='1'") И
			ДобавитьМесяц(ПоказателиТитульного.ДатаНачалаПериода.Значение, -36) > ПоказателиРаздела2.Р2С050.Значение
			Или ПоказателиТитульного.КатегорияПлательщика.Значение = НСтр("ru='2'") И
		...
		НСтр("ru='ДатаРасчета, КатегорияПлательщика'"), НСтр("ru='КатегорияПлательщика, ДатаНачалаПериода'"));
