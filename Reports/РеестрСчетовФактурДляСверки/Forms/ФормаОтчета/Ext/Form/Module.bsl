...
Функция СформироватьОтчетНаСервере()
	...
	РезультатВыполнения.Вставить("Статус", "ОтказПроверкиЗаполнения");
	...
	РезультатВыполнения.АдресРезультата = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
	...
	ОписаниеЗаданияФормированияОтчета = ДлительныеОперации.ВыполнитьПроцедуру(
		ПараметрыЗадания, "СверкаДанныхУчетаНДС.СформироватьРеестрСчетовФактур",
		ПараметрыОтчета, РезультатВыполнения.АдресРезультата);
...
Процедура ЗагрузитьПодготовленныеДанные(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	ЗагрузитьПодготовленныеДанныеНаСервере(ДополнительныеПараметры.АдресРезультата);
	
	Если ДополнительныеПараметры.ОтправитьОтчетПочтой Тогда
...
Процедура ОтправитьПоЭлектроннойПочте(Команда)
	...
	Если ОтчетСформирован() Тогда
		ОтправитьПоЭлектроннойПочтеСформированныйОтчет();
	Иначе
		НачатьФормированиеОтчета(Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура НачатьФормированиеОтчета(ОтправитьОтчетПочтой = Ложь)
	
	ОчиститьСообщения();
	
	РезультатВыполнения = СформироватьОтчетНаСервере();
	
	Если РезультатВыполнения.Статус = "ОтказПроверкиЗаполнения" Тогда
		Возврат;
	...
	ДополнительныеПараметры.Вставить("АдресРезультата", РезультатВыполнения.АдресРезультата);
	ДополнительныеПараметры.Вставить("ОтправитьОтчетПочтой", ОтправитьОтчетПочтой);
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗагрузитьПодготовленныеДанные", ЭтотОбъект, ДополнительныеПараметры);
