...
Процедура СформироватьТабличныйДокумент(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ПараметрыОтчета.ВыводитьЗаголовок Тогда
		...
		ТекстОрганизация = БухгалтерскиеОтчетыВызовСервера.ПолучитьТекстОрганизация(ПараметрыОтчета.Организация,
			Ложь, ПараметрыОтчета.ДатаСведений);
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА НачислениеАмортизацииОСНалоговыйУчет.Регистратор ССЫЛКА Документ.ВводНачальныхОстатков
		|			ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаПринятияКУчету, МЕСЯЦ, 1), МЕСЯЦ)
		|		ИНАЧЕ НачислениеАмортизацииОСНалоговыйУчет.Период
		|	КОНЕЦ КАК Период,
		|	НачислениеАмортизацииОСНалоговыйУчет.Регистратор КАК Регистратор,
		|	НачислениеАмортизацииОСНалоговыйУчет.НачислятьАмортизацию КАК НачислятьАмортизацию
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПериодыЗначащихЗаписейПоМесяцам.Месяц КАК Месяц,
		...
		|	НачислениеАмортизацииОССпециальныйКоэффициентНалоговыйУчет.Регистратор КАК Регистратор,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СпециальныеКоэффициентыПоМесяцам.СпециальныйКоэффициент КАК СпециальныйКоэффициент,
