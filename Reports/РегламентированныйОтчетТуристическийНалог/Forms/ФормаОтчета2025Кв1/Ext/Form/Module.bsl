...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаПредставлениеФормы.Видимость = Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2026;
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) >= 2026
		И СтруктураРеквизитовФормы.мСохраненныйДок.Вид = 0 И ТекущаяДатаСеанса() >= '20260424' Тогда
		
		СообщитьВТС("С 24.04.2026 представление первичной декларации, начиная с отчета за период 1 квартал 2026 года, "
			+ "доступно только по форме в редакции приказа ФНС России от 19.12.2025 № ЕД-7-3/1228@",
