...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.НалоговыйОрган) = Тип("СправочникСсылка.РегистрацииВНалоговомОргане") 
		И Не Параметры.НалоговыйОрган.Пустая() Тогда
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаПредставлениеФормы.Видимость = ТекущаяДатаСеанса() < '20260424';
	КонецЕсли;
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	// Группа П00002М2.
	ИменаГраф = Новый Массив;
	ИменаГраф.Добавить("П00002М200501");
	ИменаГраф.Добавить("П00002М207003");
	ИменаГраф.Добавить("П00002М207503");
	ИменаГраф.Добавить("П00002М208003");
	ИменаГраф.Добавить("П00002М208503");
	ИменаГраф.Добавить("П00002М208603");
	ИменаГраф.Добавить("П00002М208703");
	ИменаГраф.Добавить("П00002М209003");
	ИменаГраф.Добавить("П00002М210003");
	ИменаГраф.Добавить("П00002М211003");
	ИменаГраф.Добавить("П00002М212003");
	ИменаГраф.Добавить("П00002М213003");
	ИменаГраф.Добавить("П00002М214003");
	ИменаГраф.Добавить("П00002М214503");
	ИменаГраф.Добавить("П00002М215003");
	ИменаПодчиненныхГрупп = Новый Массив;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020005001", 2, ,
		"Выбор типа средства размещения", КоллекцияСписковВыбора["КодыТиповСР"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020006501", 1, ,
		"Выбор признака изменения ставки", КоллекцияСписковВыбора["ПрИзмененияСтавки"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00002М200501", 2, ,
		"Выбор номера месяца", КоллекцияСписковВыбора["НомераМесяцаНалПериода_"
...
Функция ДополнительныйТекстПриДобавленииУдаленииСтрок(Знач ИмяГруппы)
	...
	Если ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппыР2М1 Тогда
		Подстрока = "здание (строение) / сооружение";
		ТекстВопроса = НСтр("ru='Удалить выбранное здание (строение) / сооружение?'");
	ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппыР2М2 Тогда
		Подстрока = "сведения за месяц налогового периода";
		ТекстВопроса = НСтр("ru='Удалить выбранные сведения за месяц налогового периода?'");
	Иначе
		Подстрока = "строку";
...
Функция СоответствиеПоказателейСтраницыРеквизитамАдреса(ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		// Многострочные данные (здание (строение) / сооружение).
		//
		Здания = Новый Массив;
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если мСтруктураМногоуровневыхРазделов.Свойство(ИмяТекРаздела) Тогда
			...
			Если ТипЗнч(ТекВетка) = Тип("СтрокаДереваЗначений") Тогда
				...
				// Особая обработка: возможно не более 3 строк множественного показателя "Здание (строение) / сооружение".
				//
				Если ИмяГлРаздела = "Раздел2" Тогда
					...
					Пока ДеревоМногострочнойЧасти.Строки.Количество() > 3 Цикл
						...
						"ru='Возможно не более 3 строк множественного показателя ""Здание (строение) / сооружение""'");
					КонецЦикла;
				КонецЕсли;
...
Процедура ДобавитьСтроку(Команда)
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
		...
		Если СтрНайти(ИмяТекОбласти, СтруктураРеквизитовФормы.мИдГруппыР2М1) > 0 Тогда
			...
			Если ЗначениеЗаполнено(ИмяПервойГрафы)
				И ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_3") <> Неопределено Тогда
				ТекстПредупреждения = НСтр("ru = 'Возможно не более 3 строк показателя ""Здание (строение) / сооружение""'");
			КонецЕсли;
		КонецЕсли;
		
		Если СтрНайти(ИмяТекОбласти, СтруктураРеквизитовФормы.мИдГруппыР2М2) > 0 Тогда
			ИмяПервойГрафы = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[
				СтруктураРеквизитовФормы.мИдГруппыР2М2]["ИменаГраф"][0];
			Если ЗначениеЗаполнено(ИмяПервойГрафы)
				И ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_3") <> Неопределено Тогда
				ТекстПредупреждения = НСтр("ru = 'Возможно не более 3 строк сведений за месяц налогового периода'");
			КонецЕсли;
		КонецЕсли;
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				РегламентированнаяОтчетностьКлиентСервер.УстановитьИндекс(Индекс, ИндексСтр);
				Обл = ТДО.Найти("П00002М200501_" + ИндексСтр);
				Пока Обл <> Неопределено Цикл
					
					ТДО["П00002М210003_" + ИндексСтр].Значение
						= Окр(ТДО["П00002М207003_" + ИндексСтр].Значение) - Окр(ТДО["П00002М208003_" + ИндексСтр].Значение)
						- Окр(ТДО["П00002М208603_" + ИндексСтр].Значение) - Окр(ТДО["П00002М209003_" + ИндексСтр].Значение);
					
					ТДО["П00002М212003_" + ИндексСтр].Значение
						= Окр(Окр(ТДО["П00002М210003_" + ИндексСтр].Значение) * ТДО["П00002М211003_" + ИндексСтр].Значение / 100);
					
					ТДО["П00002М215003_" + ИндексСтр].Значение
						= Окр(ТДО["П00002М212003_" + ИндексСтр].Значение) + Окр(ТДО["П00002М213003_" + ИндексСтр].Значение)
						- Окр(ТДО["П00002М214003_" + ИндексСтр].Значение) - Окр(ТДО["П00002М214503_" + ИндексСтр].Значение);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			ДерМнЧ = Ветка.ДанныеМногострочныхЧастей[Форма.СтруктураРеквизитовФормы.мИдГруппыР2М2];
			
			Для каждого СтрМнЧ Из ДерМнЧ.Строки Цикл
				
				СтрМнЧ.Данные.Вставить("П00002М210003"
					, Окр(СтрМнЧ.Данные["П00002М207003"]) - Окр(СтрМнЧ.Данные["П00002М208003"])
					- Окр(СтрМнЧ.Данные["П00002М208603"]) - Окр(СтрМнЧ.Данные["П00002М209003"]));
				
				СтрМнЧ.Данные.Вставить("П00002М212003",
					Окр(Окр(СтрМнЧ.Данные["П00002М210003"]) * СтрМнЧ.Данные["П00002М211003"] / 100));
				
				СтрМнЧ.Данные.Вставить("П00002М215003"
					, Окр(СтрМнЧ.Данные["П00002М212003"]) + Окр(СтрМнЧ.Данные["П00002М213003"])
					- Окр(СтрМнЧ.Данные["П00002М214003"]) - Окр(СтрМнЧ.Данные["П00002М214503"]));
...
Процедура СобратьДанные()
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		Элементы.ТабличныйДокумент.ТекущаяОбласть = ТабличныйДокумент.Область("П00001М101000_1");
	КонецЕсли;
...
Процедура СформироватьРаздел1()
	...
	Для каждого Ветка Из ДеревоР2.Строки Цикл
		...
		ПрИзмененияСтавки = СокрЛП(Ветка.Данные["П000020006501"]);
		
		ДерМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М2];
		
		ТекСумма = 0;
		Для каждого СтрМнЧ Из ДерМнЧ.Строки Цикл
			ТекСумма = ТекСумма + Окр(СтрМнЧ.Данные["П00002М215003"]);
		КонецЦикла;
		
		НоваяСтрокаТЗ = ТЗ.Добавить();
		НоваяСтрокаТЗ.ОКТМО = СокрЛП(Ветка.Данные["П000020006001"]);
		НоваяСтрокаТЗ.Сумма = ТекСумма;
	...
	ТЗ.Свернуть("ОКТМО", "Сумма");
	ТЗ.Сортировать("ОКТМО");
	...
	Для Инд = 0 По ТЗ.Количество() - 1 Цикл
		...
		СтрокаМнЧР1М1.Данные.Вставить("П00001М101000", СтрокаТЗ.ОКТМО);
		СтрокаМнЧР1М1.Данные.Вставить("П00001М102000", СтрокаТЗ.Сумма);
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П00002М210003");
	Структ.Вставить("П00002М212003");
	Структ.Вставить("П00002М215003");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ТекущаяДатаСеанса() < '20260424' Тогда
		
		СообщитьВТС("До 23.04.2026 (включительно) представление декларации доступно только по форме, "
		+ "утвержденной приказом ФНС России от 05.11.2024 № ЕД-7-3/992@", "Титульный", "НомерКорректировки");
	...
	Если ЗаполненРаздел(ДеревоРаздела, "Раздел2") Тогда
		...
		ВозмКодыТипСрРазм = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000020005001");
		ВозмКодыПрИзмСтав = ВозможныеЗначенияИзСпискаВыбораПоказателя("П000020006501");
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Если КолСтрокР2М1 > 3 Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
					+ ": возможно не более 3 строк множественного показателя ""Здание (строение) / сооружение""",
				...
				Для каждого СтрокаМнЧР2М1 Из ДеревоМнЧР2М1.Строки Цикл
					...
					Если ЗначениеЗаполнено(ЗданиеТип) ИЛИ ЗначениеЗаполнено(ЗданиеНомер) Тогда
						...
						Если НЕ ЗначениеЗаполнено(ЗданиеТип) Тогда
							СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ":" + ?(КолСтрокР2М1 > 1, " в строке " + НомСтрМнЧ, "")
								+ " не задан тип здания (строения) / сооружения", "Раздел2", "П00002М102013_" + НомСтрМнЧ, НомСтраницы);
							СтатусПроверки = Ложь;
						КонецЕсли;
						Если НЕ ЗначениеЗаполнено(ЗданиеНомер) Тогда
							СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ":" + ?(КолСтрокР2М1 > 1, " в строке " + НомСтрМнЧ, "")
								+ " не задан номер здания (строения) / сооружения", "Раздел2", "П00002М102014_" + НомСтрМнЧ, НомСтраницы);
							СтатусПроверки = Ложь;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			...
			Если ЗначениеЗаполнено(ПомещЗданияТип) ИЛИ ЗначениеЗаполнено(ПомещЗданияНомер) Тогда
				Если НЕ ЗначениеЗаполнено(ПомещЗданияТип) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
						+ ": не задан тип помещения в пределах здания (строения), сооружения / машино-места",
						"Раздел2", "П000020002015", НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(ПомещЗданияНомер) Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
						+ ": не задан номер помещения в пределах здания (строения), сооружения / машино-места",
			...
			ЗначПок = СокрЛП(Ветка.Данные["П000020005001"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": не задан код типа средства размещения",
				"Раздел2", "П000020005001", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмКодыТипСрРазм.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
				+ ": код типа средства размещения должен принимать одно из значений: "
				+ ВозмКодыТипСрРазм.СтрокаЗначений, "Раздел2", "П000020005001", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			ПрИзмСтав = СокрЛП(Ветка.Данные["П000020006501"]);
			Если НЕ ЗначениеЗаполнено(ПрИзмСтав) Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
					+ ": не задан признак изменения налоговой ставки", "Раздел2", "П000020006501", НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ВозмКодыПрИзмСтав.МассивЗначений.Найти(ПрИзмСтав) = Неопределено Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы
					+ ": признак изменения налоговой ставки должен принимать одно из значений: "
					+ ВозмКодыПрИзмСтав.СтрокаЗначений, "Раздел2", "П000020006501", НомСтраницы);
				СтатусПроверки = Ложь;
			Иначе
				ДеревоМнЧР2М2 = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М2];
				КолСтрокР2М2 = ДеревоМнЧР2М2.Строки.Количество();
				
				Если ПрИзмСтав = "1" Тогда
					
					ВыполненоУсловиеЗаполнения = КолСтрокР2М2 = 1 И СокрЛП(ДеревоМнЧР2М2.Строки[0].Данные["П00002М200501"]) = "00";
					Если НЕ ВыполненоУсловиеЗаполнения Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": если признак изменения налоговой ставки равен 1, "
							+ "на странице должна быть единственная группа строк 005, 070-150, при этом строка 005 должна быть равна 00",
							"Раздел2", "П000020006501", НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
				Иначе// ПрИзмСтав = "2"
					
					ВыполненоУсловиеЗаполнения = Ложь;
					Если КолСтрокР2М2 = 3 Тогда
						НомМес_1 = СокрЛП(ДеревоМнЧР2М2.Строки[0].Данные["П00002М200501"]);
						НомМес_2 = СокрЛП(ДеревоМнЧР2М2.Строки[1].Данные["П00002М200501"]);
						НомМес_3 = СокрЛП(ДеревоМнЧР2М2.Строки[2].Данные["П00002М200501"]);
						Мас_НомМес = СтрРазделить("01,02,03", ",", Ложь);
						ВыполненоУсловиеЗаполнения
							= Мас_НомМес.Найти(НомМес_1) <> Неопределено
							И Мас_НомМес.Найти(НомМес_2) <> Неопределено
							И Мас_НомМес.Найти(НомМес_3) <> Неопределено
							И НомМес_1 <> НомМес_2
							И НомМес_1 <> НомМес_3
							И НомМес_2 <> НомМес_3;
					КонецЕсли;
					...
					Если НЕ ВыполненоУсловиеЗаполнения Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": если признак изменения налоговой ставки равен 2, "
							+ "на странице должно быть три группы строк 005, 070-150, при этом строки 005 должны быть равны 01, 02 и 03",
							"Раздел2", "П000020006501", НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	СумНал = ПолучитьПодчиненныйЭлемент(СумНалПодУпл, "СумНал");
	...
	Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
		ЗаполнитьДаннымиУзел(СтрокаМнЧ.Данные,
			СкопироватьУзел(СумНалПодУпл, СумНал, СумНалПодУпл.Строки.Количество() - 1));
	КонецЦикла;
	...
	Иначе
		...
		ИначеЕсли СтрокаДерева.Код = "СвОбктНалог" Тогда
			...
			Для каждого Ветка Из ДеревоРаздела.Строки Цикл
				...
				ПотокXML.ЗаписатьАтрибут("ТипСрРазм",  СокрЛП(Ветка.Данные["П000020005001"]));
				ПотокXML.ЗаписатьАтрибут("ОКТМО",      СокрЛП(Ветка.Данные["П000020006001"]));
				ПотокXML.ЗаписатьАтрибут("ПрИзмСтав",  СокрЛП(Ветка.Данные["П000020006501"]));
				...
				ДеревоМнЧР2М2 = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР2М2];
				Для каждого СтрокаМнЧР2М2 Из ДеревоМнЧР2М2.Строки Цикл
					ПотокXML.ЗаписатьНачалоЭлемента("РасчетНалог");
					ПотокXML.ЗаписатьАтрибут("НомМес",           СокрЛП(СтрокаМнЧР2М2.Данные["П00002М200501"]));
					ПотокXML.ЗаписатьАтрибут("СтмУсл",           СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М207003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("КолЛицВрПрож",     СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М207503"], Ч_6_0)));
					ПотокXML.ЗаписатьАтрибут("СтмУслИск",        СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М208003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("КолЛицПрДок",      СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М208503"], Ч_6_0)));
					ПотокXML.ЗаписатьАтрибут("СтмУслИск3",       СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М208603"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("КолЛицПрДок3",     СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М208703"], Ч_6_0)));
					ПотокXML.ЗаписатьАтрибут("СтмУслИсч",        СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М209003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("НалогБаз",         СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М210003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("СтавНал",          СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М211003"], Ч_3_2)));
					ПотокXML.ЗаписатьАтрибут("СумИсчНалБез",     СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М212003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("СумИсчНал",        СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М213003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("СумНалОсвУп",      СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М214003"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("СумНалОсвУп418.5", СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М214503"], Ч_15_0)));
					ПотокXML.ЗаписатьАтрибут("ИтогСумНал",       СокрЛП(Формат(СтрокаМнЧР2М2.Данные["П00002М215003"], Ч_15_0)));
...
Процедура УстановитьФорматВыводаНаСервере(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		Для Каждого Обл Из ТДО Цикл
			Если СтрНачинаетсяС(Обл.Имя, "П00001М102000") Тогда
				Постфикс = Сред(Обл.Имя, 14);
				Обл010 = ТДО.Найти("П00001М101000" + Постфикс);
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	ИначеЕсли ИмяТекРаздела = "Раздел2" Тогда
		...
		Для каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
			...
			// Лист 1.
			//
			ИмяГруппыМнЧ1 = СтруктураРеквизитовФормы.мИдГруппыР2М1;
			СтрокВБлокеМнЧ1 = 3;
			...
			// Лист 2.
			//
			ИмяГруппыМнЧ2 = СтруктураРеквизитовФормы.мИдГруппыР2М2;
			СтрокВБлокеМнЧ2 = 1;
			...
			Для каждого СтрокаМнЧ2 Из ДанныеМнЧ2.Строки Цикл
				...
				Для каждого ИмяГрафы Из ИменаГрафМнЧ2 Цикл
					ДанныеМнЧДляПечати.Вставить(ИмяГрафы + СтрокаСИндексомПоказателя(НомерСтрокиВБлокеМнЧ2),
						СтрокаМнЧ2.Данные[ИмяГрафы]);
				КонецЦикла;
			...
			Для НомЛиста = 1 По МассивДанныхМнЧДляПечати.Количество() Цикл
				...
				Для Ном2 = 1 По СтрокВБлокеМнЧ2 - 1 Цикл
					ИмяОбластиПреемника = "ДобавлениеСтроки" + ИмяГруппыМнЧ2 + СтрокаСИндексомПоказателя(Ном2);
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Область Из ИсходныйМакет.Области Цикл
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Прямоугольник
			...
			Если ПараметрыПечати.ИмяСтраницы = "Раздел2_Лист2"
				И СтрНачинаетсяС(Область.Имя, "П00002М211003") И Область.Значение = 0 Тогда
