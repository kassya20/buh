...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ГруппаДокумент.Видимость = ЭтоПБОЮЛ;
...
Процедура ОбособленноеПодразделениеНажатие(Элемент = Неопределено,
	...
	ПараметрыИзФормы.Вставить("НуженВидПродажи", Ложь);
	СтандартнаяОбработка = Ложь;
	
	РегламентированнаяОтчетностьАЛКОКлиент.ОбособленноеПодразделениеНажатиеАЛКО(
						ЭтаФорма, , СтандартнаяОбработка, ПараметрыИзФормы);
...
&НаСервере
Процедура ТумблерРежимаНавигацииПриИзмененииНаСервере()
	РегламентированнаяОтчетностьАЛКО.ТумблерРежимаНавигацииПриИзмененииАЛКО(ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТумблерРежимаНавигацииПриИзмененииНаКлиенте(Элемент)
	ТумблерРежимаНавигацииПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура ОбновитьМестоОсуществленияДеятельностиНаСервере(Результат) Экспорт
	...
	// В СтруктураДанныхТитульный.МестоДеятельности хранится представление адреса,
	// и если не заполнен, там пустая строка. 
	// А в ОрганизацияМестоОсуществленияДеятельности - если не заполнен адрес хранится "Заполнить".
	АдрМЖРукРФПредставление = ?(ЭтоПБОЮЛ, СтруктураДанныхТитульный.МестоДеятельности, "");
	
	Если СтруктураДанныхТитульный.Свойство("АдрМЖРукРФПредставление") Тогда

		Если СтруктураДанныхТитульный["АдрМЖРукРФПредставление"] <> АдрМЖРукРФПредставление Тогда

			СтруктураДанныхТитульный.Вставить("АдрМЖРукРФПредставление", АдрМЖРукРФПредставление);
...
&НаСервере
Процедура ДобавитьСтраницыПоОПвДекларацииНаСервере(НаименованиеОП, ПредставлениеАдреса, АдресXML, КПП, 
					ВидРозничнойПродажи = 0, ОтсутствуетИнтернет = Ложь)

	РегламентированнаяОтчетностьАЛКОВызовСервера.ДобавитьСтраницыПоОПвДекларации(ЭтаФорма, 
...
&НаКлиенте
Процедура ДобавитьСтраницыПоОПвДекларации() Экспорт
	
	Перем СтруктураНовогоОП;
	
	ОбщийПараметрПроцедурНаКлиенте.Свойство("СтруктураНовогоОП", СтруктураНовогоОП);
	
	Если СтруктураНовогоОП = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	ДобавитьСтраницыПоОПвДекларацииНаСервере(
					СтруктураНовогоОП["НаименованиеОП"], СтруктураНовогоОП["ПредставлениеАдреса"], 
					СтруктураНовогоОП["АдресXML"], СтруктураНовогоОП["КПП"]);
					
	ОбщийПараметрПроцедурНаКлиенте.Удалить("СтруктураНовогоОП");
...
&НаКлиенте
Процедура МасштабироватьОтчетАЛКОНаКлиенте(СписокМасштабируемыхИменЭлементов = Неопределено) Экспорт
	
	// Серверный вызов ради скорости исполнения, 
	// на клиенте намного медленнее из-за постоянного обновления формы.
	МасштабироватьОтчетАЛКОНаСервере(СписокМасштабируемыхИменЭлементов);

КонецПроцедуры

&НаСервере
Процедура МасштабироватьОтчетАЛКОНаСервере(СписокМасштабируемыхИменЭлементов = Неопределено) Экспорт

	РегламентированнаяОтчетностьАЛКОКлиентСервер.МасштабироватьОтчетАЛКО(ЭтаФорма, СписокМасштабируемыхИменЭлементов);
...
Процедура УстановитьПараметрыДекларации() Экспорт
	...
	// Подробно - см. описание к РегламентированнаяОтчетностьАЛКОКлиентСервер.ИнициализироватьUIФормы().
	// 
	
	// Настройка по умолчанию достигается 
	// установкой НастройкаUI = Неопределено или НастройкаUI = Новый Соответствие без дальнейших настроек.
	// См. РегламентированнаяОтчетностьАЛКОКлиентСервер.НастройкаUIПоУмолчанию().
	
	НастройкаUI = Новый Соответствие;
		
	СтруктураРеквизитовФормы.Вставить("НастройкаUI", НастройкаUI); 
