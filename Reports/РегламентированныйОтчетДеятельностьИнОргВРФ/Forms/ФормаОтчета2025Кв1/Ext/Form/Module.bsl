...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	ИменаГраф.Добавить("П00001М200002");
	ИменаПодчиненныхГрупп = Новый Массив;
...
Процедура СформироватьСпискиВыбора()
	...
	//Раздел 1.
	//
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010003003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010004003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010005003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010006003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010007003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000010008003", 1, ,
	"Выбор признака наличия показателя", КоллекцияСписковВыбора["ПризнакиДаНет"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "КодГос"
		ИЛИ Область.Имя = "КодНалРез" Тогда
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				//110	
				Если СтрНайти(СтруктЗависимыхЯчеек.П000020011003, ОбластьИмя) > 0
					ИЛИ СтрНайти(СтруктЗависимыхЯчеек.П000020011004, ОбластьИмя) > 0
					ИЛИ СтрНайти(СтруктЗависимыхЯчеек.П000020011005, ОбластьИмя) > 0 Тогда
									
					Для НомГрафы = 3 По 5 Цикл
						Стр010 = Форма.ТабличныйДокумент.Области["П00002000100" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр020 = Форма.ТабличныйДокумент.Области["П00002000200" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр030 = Форма.ТабличныйДокумент.Области["П00002000300" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр040 = Форма.ТабличныйДокумент.Области["П00002000400" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр050 = Форма.ТабличныйДокумент.Области["П00002000500" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр060 = Форма.ТабличныйДокумент.Области["П00002000600" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр070 = Форма.ТабличныйДокумент.Области["П00002000700" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр080 = Форма.ТабличныйДокумент.Области["П00002000800" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр090 = Форма.ТабличныйДокумент.Области["П00002000900" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр100 = Форма.ТабличныйДокумент.Области["П00002001000" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр110 = Стр010 + Стр020 + Стр030 + Стр040 + Стр050 + Стр060 + Стр070 + Стр080 + Стр090 + Стр100;
						Форма.ТабличныйДокумент.Области["П00002001100" + Формат(НомГрафы, "ЧГ=")].Значение = Стр110;			
					КонецЦикла;
					
				//180
				ИначеЕсли СтрНайти(СтруктЗависимыхЯчеек.П000020018003, ОбластьИмя) > 0
					ИЛИ СтрНайти(СтруктЗависимыхЯчеек.П000020018004, ОбластьИмя) > 0
					ИЛИ СтрНайти(СтруктЗависимыхЯчеек.П000020018005, ОбластьИмя) > 0	
				Тогда
									
					Для НомГрафы = 3 По 5 Цикл
						Стр120 = Форма.ТабличныйДокумент.Области["П00002001200" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр130 = Форма.ТабличныйДокумент.Области["П00002001300" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр140 = Форма.ТабличныйДокумент.Области["П00002001400" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр150 = Форма.ТабличныйДокумент.Области["П00002001500" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр160 = Форма.ТабличныйДокумент.Области["П00002001600" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр170 = Форма.ТабличныйДокумент.Области["П00002001700" + Формат(НомГрафы, "ЧГ=")].Значение;
						Стр180 = Стр120 + Стр130 + Стр140 + Стр150 + Стр160 + Стр170;
						Форма.ТабличныйДокумент.Области["П00002001800" + Формат(НомГрафы, "ЧГ=")].Значение = Стр180;			
					КонецЦикла;
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел2" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			//110
			Для НомГрафы = 3 По 5 Цикл
				Стр010 = Ветка.Данные["П00002000100" + Формат(НомГрафы, "ЧГ=")];
				Стр020 = Ветка.Данные["П00002000200" + Формат(НомГрафы, "ЧГ=")];
				Стр030 = Ветка.Данные["П00002000300" + Формат(НомГрафы, "ЧГ=")];
				Стр040 = Ветка.Данные["П00002000400" + Формат(НомГрафы, "ЧГ=")];
				Стр050 = Ветка.Данные["П00002000500" + Формат(НомГрафы, "ЧГ=")];
				Стр060 = Ветка.Данные["П00002000600" + Формат(НомГрафы, "ЧГ=")];
				Стр070 = Ветка.Данные["П00002000700" + Формат(НомГрафы, "ЧГ=")];
				Стр080 = Ветка.Данные["П00002000800" + Формат(НомГрафы, "ЧГ=")];
				Стр090 = Ветка.Данные["П00002000900" + Формат(НомГрафы, "ЧГ=")];
				Стр100 = Ветка.Данные["П00002001000" + Формат(НомГрафы, "ЧГ=")];
				Стр110 = Стр010 + Стр020 + Стр030 + Стр040 + Стр050 + Стр060 + Стр070 + Стр080 + Стр090 + Стр100;
				Ветка.Данные.Вставить("П00002001100" + Формат(НомГрафы, "ЧГ="), РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр110));
			КонецЦикла;
			
			//180
			Для НомГрафы = 3 По 5 Цикл
				Стр120 = Ветка.Данные["П00002001200" + Формат(НомГрафы, "ЧГ=")];
				Стр130 = Ветка.Данные["П00002001300" + Формат(НомГрафы, "ЧГ=")];
				Стр140 = Ветка.Данные["П00002001400" + Формат(НомГрафы, "ЧГ=")];
				Стр150 = Ветка.Данные["П00002001500" + Формат(НомГрафы, "ЧГ=")];
				Стр160 = Ветка.Данные["П00002001600" + Формат(НомГрафы, "ЧГ=")];
				Стр170 = Ветка.Данные["П00002001700" + Формат(НомГрафы, "ЧГ=")];
				Стр180 = Стр120 + Стр130 + Стр140 + Стр150 + Стр160 + Стр170;
				Ветка.Данные.Вставить("П00002001800" + Формат(НомГрафы, "ЧГ="), РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Стр180));
			КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ.Вставить("П000020011003", "П000020001003,П000020002003,П000020003003,П000020004003,П000020005003,П000020006003,П000020007003,П000020008003,П000020009003,П000020010003");
	Структ.Вставить("П000020011004", "П000020001004,П000020002004,П000020003004,П000020004004,П000020005004,П000020006004,П000020007004,П000020008004,П000020009004,П000020010004");
	Структ.Вставить("П000020011005", "П000020001005,П000020002005,П000020003005,П000020004005,П000020005005,П000020006005,П000020007005,П000020008005,П000020009005,П000020010005");
	Структ.Вставить("П000020018003", "П000020012003,П000020013003,П000020014003,П000020015003,П000020016003,П000020017003");
	Структ.Вставить("П000020018004", "П000020012004,П000020013004,П000020014004,П000020015004,П000020016004,П000020017004");
	Структ.Вставить("П000020018005", "П000020012005,П000020013005,П000020014005,П000020015005,П000020016005,П000020017005");
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("КодГос", СокрЛП(Титульный.КодГос));
	ПараметрыВыгрузки.Вставить("КодНалРез", СокрЛП(Титульный.КодНалРез));
	...
	ПараметрыВыгрузки.Вставить("ИмяКлючевогоУзлаСодержательнойЧасти", "ГодОтчет");
	
	ПараметрыВыгрузки.Вставить("ИдФайл", ИдентификаторФайлаВыгрузки(ПараметрыВыгрузки, "NO_GODOTIO"));
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	НаимПок = "код государства (территории) регистрации";
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодГос) Тогда
		СообщитьВТС("Не задан " + НаимПок, "Титульный", "КодГос");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодГос) <> 3 Тогда
		СообщитьВТС("Задан " + НаимПок + ", не состоящий из 3 цифр", "Титульный", "КодГос");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.КодНалРез) Тогда
		СообщитьВТС("Не задан код государства (территории) налогового резидентства иностранной организации", "Титульный", "КодНалРез");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(ПараметрыВыгрузки.КодНалРез) <> 3 Тогда
		СообщитьВТС("Задан задан код государства (территории) налогового резидентства иностранной организации, не состоящий из 3 цифр", "Титульный", "КодНалРез");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ПараметрыВыгрузки.ПрПодп = "2" И НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.НаимДокПред) Тогда
		СообщитьВТС("Не заданы наименование и реквизиты документа, подтверждающего полномочия представителя руководителя",
		"Титульный", "ДокУпПред");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, ПараметрыВыгрузки)
	...
	СведДеят = ПолучитьПодчиненныйЭлемент(УзелРодитель, "СведДеят");
	
	// Немногострочные данные.
	ЗаполнитьДаннымиУзел(Ветка.Данные, СведДеят);
		
	// Многострочная часть "Перечень наименований регистров бухгалтерского, учета,
	// применяемых отделением при ведении бухгалтерского учета".
	ПерНаимРег = ПолучитьПодчиненныйЭлемент(СведДеят, "ПерНаимРег");
	НаимРег = ПолучитьПодчиненныйЭлемент(ПерНаимРег, "НаимРег");
	...
	Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
		ВывестиПоказательВXML(СкопироватьУзел(ПерНаимРег, НаимРег), СтрокаМнЧ.Данные["П00001М100001"]);
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(НаимРег);

	// Многострочная часть "Перечень применяемых отделением счетов бухгалтерского учета".
	ПерСчет = ПолучитьПодчиненныйЭлемент(СведДеят, "ПерСчет");
	СведНомНаим = ПолучитьПодчиненныйЭлемент(ПерСчет, "СведНомНаим");
	ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР1М2];
	Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
		ЗаполнитьДаннымиУзел(СтрокаМнЧ.Данные, СкопироватьУзел(ПерСчет, СведНомНаим, ПерСчет.Строки.Количество() - 1));
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(СведНомНаим);		

	// Многострочная часть "Перечень наименований регистров налогового учета, применяемых отделением".
	ПерРег = ПолучитьПодчиненныйЭлемент(СведДеят, "ПерРег");
	НаимРег = ПолучитьПодчиненныйЭлемент(ПерРег, "НаимРег");
	ДанныеМнЧ = Ветка.ДанныеМногострочныхЧастей[СтруктураРеквизитовФормы.мИдГруппыР1М3];
	Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
		ВывестиПоказательВXML(СкопироватьУзел(ПерРег, НаимРег), СтрокаМнЧ.Данные["П00001М300001"]);
	КонецЦикла;
	РегламентированнаяОтчетность.УдалитьУзел(НаимРег);
	...
	ЗаполнитьДаннымиУзел(ДеревоРаздела.Строки[0].Данные, ПолучитьПодчиненныйЭлемент(УзелРодитель, "ФинДан"));
...
Функция ВыгрузкаОтчета(ТекстВыгрузки, ПараметрыВыгрузки)
	...
	РезультатВыгрузки.Вставить("ТипФайлаВыгрузки", "ДеятельностьИнОргВРФ");
...
Процедура ВывестиНаПечатьДанныеМногоуровневогоРаздела(СтруктураПараметров, ОбщиеРеквизитыРазделов, СтрПоля)
	...
	Если ИмяТекРаздела = "Раздел1" Тогда
		...
		// Раздел 1 на 4-х листах. Лист 1. Печать одиночного листа без допстрок
		...
		НомЛиста_Печать = 1;
		ИмяРаздела_Печать = ИмяТекРаздела + "_" + Строка(НомЛиста_Печать);
		
		СтруктураПараметровРаздела = ОбщегоНазначения.СкопироватьРекурсивно(СтруктураПараметров);
		СтруктураПараметровРаздела.Вставить("ИмяСтраницы", ИмяРаздела_Печать);
		ВывестиНаПечатьЛист(СтруктураПараметровРаздела, ОбщиеРеквизитыРазделов, СтрПоля);
		
		
		// Раздел 1 на 4-х листах. Лист 2-4. Печать Многострочки. Одна многострочка на одном листе.
		
		НомЛистаНач_Печать = 2;
		НомЛистаКон_Печать = 4;
		Для НомЛиста_Печать = НомЛистаНач_Печать По НомЛистаКон_Печать Цикл
			
			Если НомЛиста_Печать = 2 Тогда
				ИмяГруппыМнЧ_Печать = СтруктураРеквизитовФормы.мИдГруппыР1М1;
				СтрокВБлокеМнЧ_Печать = 25;
			ИначеЕсли НомЛиста_Печать = 3 Тогда
				ИмяГруппыМнЧ_Печать = СтруктураРеквизитовФормы.мИдГруппыР1М2;
				СтрокВБлокеМнЧ_Печать = 8;
			ИначеЕсли НомЛиста_Печать = 4 Тогда
				ИмяГруппыМнЧ_Печать = СтруктураРеквизитовФормы.мИдГруппыР1М3;
				СтрокВБлокеМнЧ_Печать = 25;
			КонецЕсли;
			
			ИмяРаздела_Печать = ИмяТекРаздела + "_" + Строка(НомЛиста_Печать);
			ИменаГрафМнЧ_Печать = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[ИмяГруппыМнЧ_Печать]["ИменаГраф"];
		
			МассивДанныхМнЧДляПечати = Новый Массив;
			ДанныеМнЧДляПечати = Новый Структура;
			
			НомерСтрокиВБлокеМнЧ_Печать = 0;
			
			ДанныеМнЧ_Печать = ДеревоДанныхРаздела.Строки[0].ДанныеМногострочныхЧастей[ИмяГруппыМнЧ_Печать];
			Для каждого СтрокаМнЧ_Печать Из ДанныеМнЧ_Печать.Строки Цикл
				
				НомерСтрокиВБлокеМнЧ_Печать = НомерСтрокиВБлокеМнЧ_Печать + 1;
				Если НомерСтрокиВБлокеМнЧ_Печать > СтрокВБлокеМнЧ_Печать Тогда
					
					МассивДанныхМнЧДляПечати.Добавить(ДанныеМнЧДляПечати);
					ДанныеМнЧДляПечати = Новый Структура;
					
					НомерСтрокиВБлокеМнЧ_Печать = 1;
					
				КонецЕсли;
				
				Для каждого ИмяГрафы Из ИменаГрафМнЧ_Печать Цикл
					ДанныеМнЧДляПечати.Вставить(ИмяГрафы + СтрокаСИндексомПоказателя(НомерСтрокиВБлокеМнЧ_Печать),
					СтрокаМнЧ_Печать.Данные[ИмяГрафы]);
				КонецЦикла;
			...
			Для НомЛиста = 1 По МассивДанныхМнЧДляПечати.Количество() Цикл
				...
				Для Ном_Печать = 1 По СтрокВБлокеМнЧ_Печать - 1 Цикл
					ИмяОбластиПреемника = "ДобавлениеСтроки" + ИмяГруппыМнЧ_Печать + СтрокаСИндексомПоказателя(Ном_Печать);
					ВставитьДопСтроку(ИмяГруппыМнЧ_Печать, ИмяОбластиПреемника);
				КонецЦикла;
