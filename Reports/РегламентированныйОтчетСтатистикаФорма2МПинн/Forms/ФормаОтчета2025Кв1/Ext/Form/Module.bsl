...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли СтрНайти(Область.Имя, "П0040010136") = 1 Или СтрНайти(Область.Имя, "П0040010137") = 1 Тогда
		Модифицированность = Истина;
		...
		Иначе
			Область.Значение = 0;
		КонецЕсли;
	ИначеЕсли СтрНайти(Область.Имя, "П006") = 1 Тогда
	...
	ИначеЕсли Область.Имя = "П003001013103" Или Область.Имя = "П003001013203" Или Область.Имя = "П003001013303"
		  Или Область.Имя = "П004001013403" Или Область.Имя = "П004001013503" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//стр. 103 = сумме строк 104, 107-115;
	Итог = 0;
	Для Инд = 104 По 115 Цикл
		Если Инд = 105 Или Инд = 106 Тогда
			Продолжить;
		КонецЕсли;
		Итог = Итог + Области["П0010010" + Инд + "03"].Значение;
	КонецЦикла;
	Области.П001001010303.Значение = Итог;
