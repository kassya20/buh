...
Процедура ЗаполнитьСведенияОПодписантах()
	...
	Если НЕ (ЗначениеЗаполнено(Фам) ИЛИ ЗначениеЗаполнено(Имя) ИЛИ ЗначениеЗаполнено(Отч)) Тогда
		...
		СведОбОрг = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			...
			"ФамилияПредставительСтрахователя, ИмяПредставительСтрахователя, ОтчествоПредставительСтрахователя");
	...
	Если НЕ (ЗначениеЗаполнено(Фам) ИЛИ ЗначениеЗаполнено(Имя) ИЛИ ЗначениеЗаполнено(Отч)) Тогда
		...
		Руководитель = СокрЛП(?(ЗначениеЗаполнено(СведОбОрг.ФИОРук),
		СведОбОрг.ФИОРук, СведОбОрг.ФИО));
		
	Иначе
		
		Руководитель = Фам + " " + Имя + " " + Отч;
		СведОбОрг = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		СтруктураРеквизитовФормы.Организация, ДатаПодписи, "ФИОБух");
		
	КонецЕсли;
	
	Главбух = СокрЛП(СведОбОрг.ФИОБух);
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если СтрЧислоВхождений(ТабличныйДокумент.ТекущаяОбласть.Имя, "П0000200001") > 0 Тогда
