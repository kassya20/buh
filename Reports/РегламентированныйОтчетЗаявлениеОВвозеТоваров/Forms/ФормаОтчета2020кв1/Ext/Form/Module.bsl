...
Процедура ПроверитьВозможностьВыгрузки_ПараметрыВыгрузки(СтатусПроверки)
	...
	Если НЕ ЗначениеЗаполнено(КодНО) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнен код налогового органа'"), "Раздел1");
		СтатусПроверки = Ложь;
	ИначеЕсли СтрДлина(СокрЛП(КодНО))<> 4 Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах неверно заполнен код налогового органа'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ОтправительЯвляетсяЮЛ Тогда
		Если НЕ ЗначениеЗаполнено(НаимОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнено наименование организации отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИННЮЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнен ИНН организации отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(КППОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнен КПП организации отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(ФамилияФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнена фамилия отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИмяФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнено имя отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ИННФЛОтпр) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнен ИНН отправителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ИмпортерСовпадаетСОтправителем Тогда
		...
		Если ИмпортерЯвляетсяЮЛ Тогда
			Если НЕ ЗначениеЗаполнено(НаимИмпорт) Тогда
				...
				"ru='В сведениях об отправителе, импортере и подписантах не заполнено наименование организации импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ИННЮЛИмпорт) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр(
				"ru='В сведениях об отправителе, импортере и подписантах не заполнен ИНН организации импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(КППИмпорт) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр(
				"ru='В сведениях об отправителе, импортере и подписантах не заполнен КПП организации импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
		Иначе
			Если НЕ ЗначениеЗаполнено(ФамилияФЛИмпорт) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр(
				"ru='В сведениях об отправителе, импортере и подписантах не заполнена фамилия импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ИмяФЛИмпорт) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр(
				"ru='В сведениях об отправителе, импортере и подписантах не заполнено имя импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ИННФЛИмпорт) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр(
				"ru='В сведениях об отправителе, импортере и подписантах не заполнен ИНН импортера'"), "Раздел1");
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ФамилияПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнена фамилия подписанта'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ИмяПодп) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнено имя подписанта'"), "Раздел1");
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если ПрПодп = 3 Тогда
		Если НЕ ЗначениеЗаполнено(НаимДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнено наименование документа, подтверждающего полномочия представителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(НомерДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнен номер документа, подтверждающего полномочия представителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ДатаДовПодп) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, НСтр("ru='В сведениях об отправителе, импортере и подписантах не заполнена дата документа, подтверждающего полномочия представителя'"), "Раздел1");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
