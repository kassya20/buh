...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил22Раздел1Поступления" 
		ИЛИ ИмяРегистра = "СведенияРеглОтчетАлкоПрил22Раздел2Возвраты" Тогда

		ИдГруппы = ?(ИмяРегистра = "СведенияРеглОтчетАлкоПрил22Раздел1Поступления", "П0000200003", "П0000300003");
...
Процедура ПроверитьЗаполнениеДекларации(СтатусПроверки)
	...
	ИдГруппыРаздел1 = СтруктураРеквизитовФормы.мИдГруппы22;	
	ИдГруппыРаздел2 = СтруктураРеквизитовФормы.мИдГруппы32;
...
Процедура ПроверитьЗаполнениеТаблицы(СтатусПроверки, ИдГруппы,
	...
	ИмяТаблицы = ?(ИдГруппы = СтруктураРеквизитовФормы.мИдГруппы22, "Раздел 1", "Раздел 2");
	...
	ИмяАдресаПоставщика = ИдГруппы + "07";
	ИмяАдресаXMLПоставщика = ИдГруппы + "07XML";
	ИмяИННПоставщика = ИдГруппы + "08";
	ИмяКПППоставщика = ИдГруппы + "09";
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаДекларации Из РезЗапросаДанных Цикл
			...
			ИмяПоказателяАдреса = ИмяАдресаПоставщика + "_" + НомерСтрокиСтроковый;
			
			АдресПоставщика = СокрЛП(СтрокаДекларации[ИмяАдресаПоставщика]);
			АдресПоставщикаXML = СокрЛП(СтрокаДекларации[ИмяАдресаXMLПоставщика]);
			
			СтруктураАдресПоставщика = РегламентированнаяОтчетностьАЛКО.СтруктураАдресаИзСтрокиСтруктурыХранения(
																	АдресПоставщикаXML);
																					
			КодСтраныПоставщика = СтруктураАдресПоставщика.КодСтраны;
						
			РоссийскийРезидент = Ложь;
			РезидентЕАЭС = Ложь;
	
			РегламентированнаяОтчетностьАЛКО.ОпределитьПринадлежностьЕАЭСиРФ(ИННПоставщика, КПППоставщика, НаименованиеПоставщика,
											КодСтраныПоставщика, РоссийскийРезидент, РезидентЕАЭС, РежимОтладки);
											
			////////////////////////////////////////////////////////////////////////////////								
			// Контроль Адреса поставщика.
			...
			ПроверкаАдресаПройдена = РегламентированнаяОтчетностьАЛКО.ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, 
										ТаблицаСообщенийДляСведения, Истина, СтруктураАдресПоставщика, 
										ВспомогательнаяСтруктураДанных, РежимОтладки, СоответствиеКешАдресов);
			...
			СтруктураСведенийИННКПП.Вставить("КодСтраны", КодСтраныПоставщика);
			СтруктураСведенийИННКПП.Вставить("ТолькоЕАЭС", ТолькоЕАЭС);
			СтруктураСведенийИННКПП.Вставить("ТолькоРФ", ТолькоРФ); 
			СтруктураСведенийИННКПП.Вставить("МаксДлинаИННпоXSD", МаксДлинаИННпоXSD); 	// По умолчанию 20.
			СтруктураСведенийИННКПП.Вставить("МинДлинаИННпоXSD", МинДлинаИННпоXSD);		// По умолчанию 1.
			...
			Если ПоставщикРоссийский Тогда			
				КодСтраныПоставщика = "643";			
			КонецЕсли; 
			...
			ЛицензияЗаполнена = ЗначениеЗаполнено(ВидДеятельностиЛицензии);
			//
			////////////////////////////////////////////////////////////////////////////////
			...
			Если ЕстьИдОП И ТекущееКоличествоОшибок = 0 Тогда
				...
				ДанныеДвижений_ПоПоставщикам = ДанныеДвижений_ПоОП_ПоКоду[ИдКодПродПроизв]["Поставщики"];
				...
				Если СправочникиОтчета["Поставщики"].Получить(ИдПоставщикаЛиц) = Неопределено Тогда
					...
					Если ПоставщикРоссийский Тогда
						// Россия.
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("П000000000008", СтруктураАдресПоставщика);
						
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("ЮЛ", ЗначениеЗаполнено(КПППоставщика));
						...
						Если ЛицензияЗаполнена Тогда					
							СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("П000000000011", ВидДеятельностиЛицензии);					
						КонецЕсли;
						
					Иначе
						
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("П000000000081", КодСтраныПоставщика);
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("П000000000082", АдресПоставщика);
						СправочникиОтчета["Поставщики"][ИдПоставщикаЛиц].Вставить("Номер", ИННПоставщика);
				...
				Если ДанныеДвижений_ПоПоставщикам.Получить(ИдПоставщикаЛиц) = Неопределено Тогда
					
					ДанныеДвижений_ПоПоставщикам.Вставить(ИдПоставщикаЛиц, Новый Соответствие);

					СтруктураПоставщика = ДанныеДвижений_ПоПоставщикам[ИдПоставщикаЛиц];
					...
					СтруктураПоставщика.Вставить("Закупка", Новый Соответствие);
					СтруктураПоставщика.Вставить("Возврат", Новый Соответствие);
					
				КонецЕсли;	

				СтруктураПоставщика = ДанныеДвижений_ПоПоставщикам[ИдПоставщикаЛиц];
				
				ЗакупкаИлиВозврат = ?(ИдГруппы = СтруктураРеквизитовФормы.мИдГруппы22, "Закупка", "Возврат");
				СтруктураНакладных = СтруктураПоставщика[ЗакупкаИлиВозврат];
				...
				СтруктураНакладной.Вставить("П000000000018", СтрокаДекларации[ИмяДатыТТН]);
				СтруктураНакладной.Вставить("П000000000019", СтрокаДекларации[ИмяНомерТТН]);
				СтруктураНакладной.Вставить("П000000000020", СтрокаДекларации[ИмяНомерГТД]);
				СтруктураНакладной.Вставить("П000000000021", СтрокаДекларации[ИмяОбъемПоТТН]);
...
Процедура ЗаполнитьСправочникиДекларации(УзелСправочники)
	...
	Для Каждого ЭлементСправочникиПоставщики Из СправочникиОтчета["Поставщики"] Цикл
		...
		Если СтрокаПоставщика.Получить("ЮЛ") = Неопределено Тогда
		
			// Иностранец.
			РегламентированнаяОтчетность.УдалитьУзел(Узел_Резидент);
			
			УстановитьЗначениеЭлемента(Узел_Иностр, "П000000000081", СтрокаПоставщика["П000000000081"]);
			УстановитьЗначениеЭлемента(Узел_Иностр, "П000000000082", СтрокаПоставщика["П000000000082"]);
			УстановитьЗначениеЭлемента(Узел_Иностр, "Номер", СтрокаПоставщика["Номер"]);
		...
		Иначе
			...
			СтруктураАдреса = СтрокаПоставщика["П000000000008"];
			...
			Если СтрокаПоставщика["ЮЛ"] Тогда
			...
			Если НЕ ПустаяСтрока(СтрокаПоставщика["П000000000011"]) Тогда
				УстановитьЗначениеЭлемента(Узел_Резидент, "П000000000011", СтрокаПоставщика["П000000000011"]);		
			КонецЕсли;
	...
	Для Каждого ЭлементДвиженияПоОП Из ДанныеДвиженийПоОП Цикл
		...
		Для Каждого Элемент_ДанныеДвижений_ПоОП_ПоКоду Из ДанныеДвижений_ПоОП_ПоКоду Цикл
			...
			Для Каждого ЭлементПоставщика Из Поставщики Цикл
				...
				ОбразецУзлаПоставка = ПолучитьПодчиненныйЭлемент(НовыйУзелПоставщик, "Закупка");
				Поставки = СтруктураПоставщика["Закупка"];
				...
				Для Каждого СтрокаПродукции Из Поставки Цикл
					...
					Для Счетчик = 18 По 21 Цикл
	
						КолонкаСтрокаПродукции = Формат(Счетчик, "ЧЦ=2; ЧН=; ЧГ=0; ЧВН="); // 18, ..21.
												
						// П000000000018 .. П000000000021.
				...
				Возвраты = СтруктураПоставщика["Возврат"];
				...
				Для Каждого СтрокаПродукции Из Возвраты Цикл
					...
					Для Счетчик = 18 По 21 Цикл
						...
						УстановитьЗначениеЭлемента(НовыйУзелВозврат, КлючВыгрузки, СтрокаПоставки[КлючВыгрузки]);
...
Функция ПроверитьВыгрузку(ПараметрыДокумента, ЭтоВыгрузка = Ложь) Экспорт
	...
	ВсегоСтрокПоРазделу1 = ПолучитьКоличествоСтрокПоГруппе(СтруктураРеквизитовФормы.мИдГруппы22, Истина);
	ВсегоСтрокПоРазделу2 = ПолучитьКоличествоСтрокПоГруппе(СтруктураРеквизитовФормы.мИдГруппы32, Истина);
	...
	ОписаниеСтруктурыАдресаИзОтчета = РегламентированнаяОтчетностьАЛКО.ПолучитьПустуюСтруктуруАдреса();
	Для каждого КлючЗначение Из ОписаниеСтруктурыАдресаИзОтчета Цикл
		ОписаниеСтруктурыАдресаИзОтчета[КлючЗначение.Ключ] = 
			Новый Структура("ПредставлениеПоля, МинДлина, МаксДлина, ТолькоЦифры");
	КонецЦикла;
	
	// Описание длин из описания адресов схем xsd выгрузок алкодеклараций.
	ОписаниеСтруктурыАдресаИзОтчета.Индекс.ПредставлениеПоля = """Индекс""";
	ОписаниеСтруктурыАдресаИзОтчета.Индекс.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Индекс.МаксДлина = 10;
	ОписаниеСтруктурыАдресаИзОтчета.Индекс.ТолькоЦифры = Истина;

	ОписаниеСтруктурыАдресаИзОтчета.Район.ПредставлениеПоля = """Район""";
	ОписаниеСтруктурыАдресаИзОтчета.Район.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Район.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Район.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Город.ПредставлениеПоля = """Город""";
	ОписаниеСтруктурыАдресаИзОтчета.Город.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Город.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Город.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.НаселенныйПункт.ПредставлениеПоля = """Населенный Пункт""";
	ОписаниеСтруктурыАдресаИзОтчета.НаселенныйПункт.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.НаселенныйПункт.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.НаселенныйПункт.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Улица.ПредставлениеПоля = """Улица""";
	ОписаниеСтруктурыАдресаИзОтчета.Улица.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Улица.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Улица.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Дом.ПредставлениеПоля = """Дом""";
	ОписаниеСтруктурыАдресаИзОтчета.Дом.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Дом.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Дом.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Корпус.ПредставлениеПоля = """Корпус""";
	ОписаниеСтруктурыАдресаИзОтчета.Корпус.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Корпус.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Корпус.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Литера.ПредставлениеПоля = """Литера""";
	ОписаниеСтруктурыАдресаИзОтчета.Литера.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Литера.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Литера.ТолькоЦифры = Ложь;

	ОписаниеСтруктурыАдресаИзОтчета.Квартира.ПредставлениеПоля = """Квартира""";
	ОписаниеСтруктурыАдресаИзОтчета.Квартира.МинДлина = 0;
	ОписаниеСтруктурыАдресаИзОтчета.Квартира.МаксДлина = 50;
	ОписаниеСтруктурыАдресаИзОтчета.Квартира.ТолькоЦифры = Ложь;
...
Процедура ВывестиНаПечатьТитульный(СтруктураПараметровСтраницы)
	...
	НастройкиСтраницы.Вставить("ПолеСлева", 10);
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение37.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение37.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение37.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение37.ДеревоФормИФорматов();
