...
Функция ПолучитьПараметрыИсполненияОтчета() Экспорт
	...
	Манифест.Вставить("ИспользоватьПриВыводеПодвала",       Истина);
...
Процедура ПередКомпоновкойМакета(Контекст, Схема, КомпоновщикНастроек) Экспорт
	...
	Контекст.ВыполнениеОтчета.Вставить("Данные", ДанныеОтчета(Контекст.КонецПериода, Контекст.Организация));
...
//
// Возвращаемое значение:
//  Структура - внешние наборы данных. Ключ - имя набора. Значение - набор данных (таблица значений)
//
...
Функция ДанныеОтчета(Знач Период, Организация)
	...
	Запрос.УстановитьПараметр("НомерРасчета",  0);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВЫБОР
	...
	|	И Расчет.НомерРасчета = &НомерРасчета
	|	И Расчет.Активность
	|
	|УПОРЯДОЧИТЬ ПО
	|	ГруппаПоказателей,
...
Процедура ВывестиПримечания(Результат, Примечания, Данные)
	...
	Для Каждого Примечание Из Примечания Цикл
		...
		ИначеЕсли Примечание.Ключ = "ВыполнитьРегламентнуюОперацию" Тогда
			ПредставлениеРегламентнойОперации = Строка(Перечисления.ВидыРегламентныхОпераций.РасчетОтложенногоНалога);
			Текст = СтрШаблон(
