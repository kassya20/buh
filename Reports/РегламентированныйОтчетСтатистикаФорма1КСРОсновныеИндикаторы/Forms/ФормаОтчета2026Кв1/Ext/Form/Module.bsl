...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Инд = 1 По 7 Цикл
		...
		Для Гр = 1 По КолКолонокРазделов Цикл
			Если Гр <= 2 Или Гр = 10 Тогда
		...
		мСтруктураКолвоКолонокРазделов.Вставить(ИмяДобавляемогоРеквизита, КолКолонокРазделов);
...
Процедура ДоступностьРазделов()
	...
	Если ДоступКРазделамЗапрещен() Тогда
		
		Для Каждого ЭлементСтруктуры Из ЭтотОбъект.мСтруктураМногострочныхРазделов Цикл
			ИмяГруппы = ЭлементСтруктуры.Ключ;
			
			Индекс = 0;
			Пока Истина Цикл
				Индекс = Индекс + 1;
				Если ПолеТабличногоДокументаФормаОтчета.Области.Найти(ИмяГруппы + "01_" + Индекс) = Неопределено Тогда
					Прервать;
				КонецЕсли;
				
				Для Гр = 1 По ЭтотОбъект.мСтруктураКолвоКолонокРазделов[ИмяГруппы] Цикл
					Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
					ПоказательИмя = ИмяГруппы + Графа + "_" + Индекс;
					Если Гр = 1 Тогда
						ПолеТабличногоДокументаФормаОтчета.Области["УдалитьСтроку" + ПоказательИмя].Гиперссылка = Ложь;
						ПолеТабличногоДокументаФормаОтчета.Области["УдалитьСтроку" + ПоказательИмя].Текст = "";
					КонецЕсли;
		...
		Для Секция = 1 По 7 Цикл
			Если ПолеТабличногоДокументаФормаОтчета.Области.Найти("ДобавлениеСтроки" + Секция) <> Неопределено Тогда
				ПолеТабличногоДокументаФормаОтчета.УдалитьОбласть(
					ПолеТабличногоДокументаФормаОтчета.Области["ДобавлениеСтроки" + Секция], ТипСмещенияТабличногоДокумента.ПоВертикали);
			КонецЕсли;
		КонецЦикла;
		...
		Для НомСтр = 1 По 7 Цикл
			Для Гр = 3 По 9 Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				ПолеТабличногоДокументаФормаОтчета.Области["П04040" + НомСтр + Графа].ЦветФона = Новый Цвет();
				ПолеТабличногоДокументаФормаОтчета.Области["П04040" + НомСтр + Графа].Защита   = Истина;
				ПолеТабличногоДокументаФормаОтчета.Области["П04040" + НомСтр + Графа].Значение = "";
			КонецЦикла;
		КонецЦикла;
		
		Для НомСтр = 501 По 507 Цикл
			Для Гр = 1 По 7 Цикл
				Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
				ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + Графа].ЦветФона = Новый Цвет();
				ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + Графа].Защита   = Истина;
				ПолеТабличногоДокументаФормаОтчета.Области["П0050010" + НомСтр + Графа].Значение = "";
			КонецЦикла;
		КонецЦикла;
...
&НаСервере
Функция ДоступКРазделамЗапрещен()
	Возврат Не (Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3
		Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 6
		Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 9
		Или Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12);
КонецФункции
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если Не ДоступКРазделамЗапрещен() Тогда
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П001") Тогда
		...
		Если Прч >= 101 И Прч <= 117 Тогда
			...
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 118 И Прч <= 119 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 118 по 119 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 120 И Прч <= 126 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 120 по 126 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 127 И Прч <= 129 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 127 по 129 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 130 И Прч <= 132 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 130 по 132 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		КонецЕсли;
...
Процедура Расчет(Форма)
	...
	//Раздел 3
	//Строка 301 графа 1 = строка 302 графа 1+ строка 303 графа 1;
	Области["П003001030101"].Значение
		= Области["П003001030201"].Значение
		+ Области["П003001030301"].Значение;
	
	//Строка 301 графа 2 = сумме строк 302 графа 2, 303 графа 2;
	Области["П003001030102"].Значение
		= Области["П003001030202"].Значение
		+ Области["П003001030302"].Значение;
	
	//Строка 304 графа 1 = строка 305 графа 1 + строка 306 графа 1;
	Области["П003001030401"].Значение
		= Области["П003001030501"].Значение
		+ Области["П003001030601"].Значение;
	
	//Строка 304 графа 2 = сумме строк 305 графа 2, 306 графа 2;
	Области["П003001030402"].Значение
		= Области["П003001030502"].Значение
		+ Области["П003001030602"].Значение;
		
	//Строка 307 графа 1 = строка 308 графа 1 + строка 309 графа 1;
	Области["П003001030701"].Значение
		= Области["П003001030801"].Значение
		+ Области["П003001030901"].Значение;
		
	//Строка 310 графа 1 = строка 311 графа 1 + строка 312 графа 1;
	Области["П003001031001"].Значение
		= Области["П003001031101"].Значение
		+ Области["П003001031201"].Значение;
		
	//Строка 313 графа 1 = сумме строк 314–316 графа 1;
	Области["П003001031301"].Значение
		= Области["П003001031401"].Значение
		+ Области["П003001031501"].Значение
		+ Области["П003001031601"].Значение;
	...
	//Раздел 5
	//Сумма граф 1, 2, 3, 4, 5, 6 по строке 501-507 = строке 501-507 гр. 7;
	Для НомСтр = 501 По 506 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 1 По 6 Цикл
			Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
			СуммаПоГрафам = СуммаПоГрафам + Области["П0050010" + НомСтр + Графа].Значение;
		КонецЦикла;
		Области["П0050010" + НомСтр + "07"].Значение = СуммаПоГрафам;
	КонецЦикла;
	
	//Сумма строк 501, 502, 503, 504, 505, 506 по графе 1-7 = строке 507 гр. 1-7;
	Для Гр = 1 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П003001030102"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П003001030102"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	мФормаОтчета.Области["П003001030402"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П003001030402"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	мФормаОтчета.Области["П003001031301"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П003001031301"].Защита   = Не ОтклАвтоРасчетЗначение;
	...
	Для Гр = 1 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		Для НомСтр = 501 По 507 Цикл
			Если НомСтр = 507 Или Гр = 7 Тогда
				мФормаОтчета.Области["П0050010" + НомСтр + Графа].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0050010" + НомСтр + Графа].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Инд = 1 По 7 Цикл
		...
		Если ДоступКРазделамЗапрещен() Тогда
			НизМногострочнойЧасти  = ПолеТабличногоДокументаФормаОтчета.Области["Секция" + (Инд+1)].Верх - 1;
		Иначе
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("Секция8")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-КСР (основные индикаторы)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("Секция8"));
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001012701"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД01001012701", "1"); //1 Капитальное
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001012801"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД01001012701", "2.АГ"); //2.АГ Некапитальное
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013001"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "2"); //2 Некапитальное модульное
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013101"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "3"); //3 Некапитальное немодульное
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013201"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "4"); //4 Некапитальное модульное и немодульное
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001012901"].Значение) Тогда 
		ПараметрыВыгрузки.Вставить("ПД01001012701", "3.АГ"); //3.АГ Смешанное
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013001"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "5"); //5 Смешанное модульное
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013101"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "6"); // 6 Смешанное немодульное
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013201"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013001", "7"); // 7 Смешанное модульное и немодульное
		КонецЕсли;
	КонецЕсли;
