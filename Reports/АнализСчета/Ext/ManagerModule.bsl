...
Функция ПолучитьПараметрыИсполненияОтчета() Экспорт
	...
	Результат.Вставить("КоличествоКолонокПоказателей",       2);
	Результат.Вставить("НастроитьШиринуКолонкиИтогов",       Истина);
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ГруппаЕстьДт = КомпоновщикНастроек.Настройки.Выбор.Элементы.Добавить(Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));
	ГруппаЕстьДт.Заголовок     = НСтр("ru = 'Есть Дебет'");
	ГруппаЕстьДт.Использование = Ложь;
	ГруппаЕстьКт = КомпоновщикНастроек.Настройки.Выбор.Элементы.Добавить(Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));
	ГруппаЕстьКт.Заголовок     = НСтр("ru = 'Есть Кредит'");
	ГруппаЕстьКт.Использование = Ложь;
	...
	Для Каждого ИмяПоказателя Из МассивПоказателей Цикл
		Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда 
			...
			ВыбранноеПоле = Новый ПолеКомпоновкиДанных("ОборотыЗаПериод.Есть" + ИмяПоказателя + "ОборотДт");
			ВыводимыеПоля.Добавить(ВыбранноеПоле);
			БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаЕстьДт, ВыбранноеПоле);
			ВыбранноеПоле = Новый ПолеКомпоновкиДанных("ОборотыЗаПериод.Есть" + ИмяПоказателя + "ОборотКт");
			ВыводимыеПоля.Добавить(ВыбранноеПоле);
			БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ГруппаЕстьКт, ВыбранноеПоле); 
	...
	// Кор.подразделение.
	УстановитьПараметрКорПодразделение(ПараметрыОтчета, КомпоновщикНастроек);
	
	// Период. 
	ПараметрыОтчета.Вставить("ПрефиксПоказателяНаличияОборота", "Есть");
	...
	КорСчетаБезПодразделения = Новый Массив;
	Для Каждого СтрокаГруппировки Из ПараметрыОтчета.ГруппировкаКор Цикл
		...
		Если ИндексПодразделения <> ПозицияКорПодразделения И ИндексПодразделения >= 1 Тогда
			// Если в таблице подразделения находятся на разных позициях - то подразделение всегда выводится первым.
			ПозицияКорПодразделения = ?(ПозицияКорПодразделения = 0, ИндексПодразделения, -1);
		КонецЕсли;
		
		Если ИндексПодразделения <> 0.5 Тогда
			КорСчетаСПодразделением.Добавить(СтрокаГруппировки.Счет);
		Иначе
			КорСчетаБезПодразделения.Добавить(СтрокаГруппировки.Счет);
		КонецЕсли;
	...
	КонецЦикла;
	КорСчетаСПодразделением = СчетаВИерархии(КорСчетаСПодразделением);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "КорСчетаСПодразделением", КорСчетаСПодразделением);
	КорСчетаБезПодразделения = СчетаВИерархии(КорСчетаБезПодразделения);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "КорСчетаБезПодразделения", КорСчетаБезПодразделения);
	...
	Для Индекс = 1 По КоличествоКорГруппировок Цикл 
		...
		ПолеГруппировки = Структура.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
		ПолеГруппировки.Использование  = Истина;
		Если Индекс = ПозицияКорПодразделения Тогда
			ПолеГруппировки.Поле           = Новый ПолеКомпоновкиДанных("КорПодразделение");
		Иначе
	...
	Если ПозицияКорПодразделения <> 0 Тогда
		...
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(УсловноеОформление.Отбор, "КорСчет", КорСчетаСПодразделением, ВидСравненияКомпоновкиДанных.НеВСписке);
...
Процедура ПроверитьКорректностьИтогов(ПараметрыОтчета, Результат) Экспорт
	...
	// В табличном документе нет данных
	Если Результат.ВысотаТаблицы < ОбластьИтогов.ПерваяИзСтрокИтогов Тогда
		Возврат;
	КонецЕсли;
	...
	Если КонтрольноеСоотношение <> 0 Тогда
		...
		Если БухгалтерскиеОтчеты.ИспользуетсяОтборПоРесурсам(ЭлементыОтбора) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ДобавитьУсловиеПоВыбраннымРесурсам(ПараметрыОтчета, Схема)
	...
	ТекстыУсловийДт = Новый Массив;
	ТекстыУсловийКт = Новый Массив;
	
	ШаблонРесурса = "ХозрасчетныйОстаткиИОбороты.Сумма%1Оборот%2";
	Для Каждого ИмяПоказателя Из ПолучитьНаборПоказателей() Цикл
		
		Если Не ПараметрыОтчета["Показатель" + ИмяПоказателя]
		 Или ИмяПоказателя = "ВалютнаяСумма" Или ИмяПоказателя = "Количество" Или ИмяПоказателя = "Контроль" Или ИмяПоказателя = "СверкаНУ" Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстыУсловийДт.Добавить(СтрШаблон(ШаблонРесурса, ?(ИмяПоказателя = "БУ", "", ИмяПоказателя), "Дт"));
		ТекстыУсловийКт.Добавить(СтрШаблон(ШаблонРесурса, ?(ИмяПоказателя = "БУ", "", ИмяПоказателя), "Кт"));
		
	КонецЦикла;
	
	Если ТекстыУсловийДт.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	// Если есть остатки или обороты по счету и набору аналитик (из ОсновнойНаборДанных), то суммы должны попадать в отчет.
	// Если есть лишь обороты по кор.счетам (из набора Обороты), в сумме дающие ноль, то такие обороты отсекаем.
	ТекстУсловияДляПодстановкиВЗапрос =
	"ГДЕ
	|	((" + СтрСоединить(ТекстыУсловийДт, " - ") + ") ЕСТЬ НЕ NULL
	|			ИЛИ (" + СтрСоединить(ТекстыУсловийКт, " - ") + ") ЕСТЬ НЕ NULL)";
...
Функция ДополнитьТекстЗапросаОборотыПоКорсчетам(ГруппировкаКор, ЗначенияПараметровМакета, Знач ИсходныйТекстЗапроса)
	...
	Для Каждого СтрокаТаблицы Из ГруппировкаКор Цикл 
		...
		// Удаляем лишние условия на кор.субконто в секции ГДЕ. 
		ПозицияУсловия = СтрНайти(ЗавершающаяЧастьЗапроса, "ГДЕ");
		Если ПозицияУсловия <> 0 Тогда
			ПозицияУсловия = СтрНайти(ЗавершающаяЧастьЗапроса, " ", НаправлениеПоиска.СНачала, ПозицияУсловия);
			ПозицияКонцаУсловия = СтрДлина(ЗавершающаяЧастьЗапроса); 
		    УсловиеЗапроса = Сред(ЗавершающаяЧастьЗапроса, ПозицияУсловия + 1, ПозицияКонцаУсловия - ПозицияУсловия - 1);
			...
			Для ИндексКор = НачальныйИндексКор + 1 По 3 Цикл
				...
				Пока ПозицияНачалаСтроки <> 0 И ПозицияКонцаСтроки <> 0 Цикл
					...
					// Если отбор задан по несуществущему кор.субконто, то условие должно быть ЛОЖЬ.
					// Поэтому для сохранения структуры запроса заменяем на сравенение с NULL
					УсловиеЗапроса = ?(ПозицияНачалаСтроки = 1, "", Лев(УсловиеЗапроса, ПозицияНачалаСтроки - 1))
						+ "NULL" + Сред(УсловиеЗапроса, ПозицияКонцаСтроки);
		...
		Если БылоУдаленоУсловиеНаКорСубконто Тогда
			...
			КонечныйТекстЗапроса.Добавить(Лев(ЗавершающаяЧастьЗапроса, ПозицияУсловия));
			КонечныйТекстЗапроса.Добавить(УсловиеЗапроса)
...
Процедура УстановитьПараметрКорПодразделение(ПараметрыОтчета, КомпоновщикНастроек)
	
	ИспользоватьКорПодразделение = Ложь;
	
	Для Каждого ПолеВыбраннойГруппировки Из ПараметрыОтчета.Группировка Цикл 
		
		Если ПолеВыбраннойГруппировки.Использование
		   И СтрНачинаетсяС(ПолеВыбраннойГруппировки.Поле, "КорПодразделение") Тогда
			// КорПодразделение должно безусловно вычисляться, если использовано в группировке или отборе.
			// Группировка будет выводиться для всех счетов кроме указанных в таблице "Группировка по кор. счетам".
			ИспользоватьКорПодразделение = Истина;
			Прервать;
	...
	Если Не ИспользоватьКорПодразделение Тогда
		
		ИменаИскомыхПолей = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("КорПодразделение");
		Если БухгалтерскиеОтчеты.ЕстьОтборПоКорПолю(КомпоновщикНастроек.Настройки.Отбор.Элементы, ИменаИскомыхПолей) Тогда
			// КорПодразделение должно безусловно вычисляться, если использовано в группировке или отборе.
			// Отбор будет использовать значение кор.подразделения (а не пустое значение) для всех счетов
			// кроме указанных в таблице "Группировка по кор. счетам".
			ИспользоватьКорПодразделение = Истина;
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "КорПодразделениеВГруппировке", ИспользоватьКорПодразделение);
