...
Функция ПоляПоВидамУчета()
	...
	ПоляПоВидамУчета.Вставить("СуммаСверкаНУ",          "СверкаНУ");
...
Процедура ДобавитьДанныеПрошлогоМесяцаВРегистр(НачалоПериода, КонецПериода, Организация)
	...
	|	ЕСТЬNULL(ВыпускПродукции.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК Подразделение,
	...
	|		ЕСТЬNULL(Выпуск.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК Подразделение,
	...
	|	РаспределениеВыпускаПоПродукции.Подразделение КАК Подразделение,
	|	РаспределениеВыпускаПоПродукции.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	|	РаспределениеВыпускаПоПродукции.Продукция КАК Продукция,
	...
	|		ЕСТЬNULL(КонечныйОстатокНЗП.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)),
	...
	|	ЕСТЬNULL(Материалы.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК Подразделение,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЭлементыЗатратКорСчета.Элемент КАК Элемент,
	...
	|		ЕСТЬNULL(Расходы.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК Подразделение,
	...
	|		ЕСТЬNULL(НЗППрошлогоПериода.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)),
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ПоляПоВидамУчета = ПоляПоВидамУчета();
	
	НастроитьРесурсыПоВидамУчета(
		КомпоновщикНастроек.Настройки,
		ПараметрыОтчета.СоставНабораПоказателей,
		ПоляПоВидамУчета);
	...
	БухгалтерскиеОтчеты.НастроитьОбъединениеЗаголовкаГруппировокТаблицы(
		...
		"КалькуляцияПродукции");
	
	БухгалтерскиеОтчетыВызовСервера.ДобавитьОтборПоОрганизации(ПараметрыОтчета, КомпоновщикНастроек);
	
	ГруппировкаДляОтключения = БухгалтерскиеОтчеты.НайтиПоИмени(КомпоновщикНастроек.ПользовательскиеНастройки.Элементы[0].Структура, "ВспомогательнаяОтключаемаяГруппировка");
	Если ГруппировкаДляОтключения <> Неопределено Тогда
		ГруппировкаДляОтключения.Состояние = СостояниеЭлементаНастройкиКомпоновкиДанных.Отключен;
	КонецЕсли;
...
Процедура НастроитьРесурсыПоВидамУчета(НастройкиКомпоновки, ВидыУчета, ПоляПоВидамУчета)
	
	Ресурсы = БухгалтерскиеОтчетыКлиентСервер.НайтиРесурсы(НастройкиКомпоновки, ПоляПоВидамУчета);
	
	ИспользуемыеРесурсы = Новый Соответствие;
	
	Для Каждого ЭлементРесурса Из Ресурсы Цикл
		
		ТипЭлемента = ТипЗнч(ЭлементРесурса);
		...
		Если ТипЭлемента = Тип("ВыбранноеПолеКомпоновкиДанных") И ЭлементРесурса.Использование Тогда
			РесурсыПоИмени = ИспользуемыеРесурсы[ВидУчета];
			Если РесурсыПоИмени = Неопределено Тогда
				РесурсыПоИмени = Новый Массив;
				ИспользуемыеРесурсы.Вставить(ВидУчета, РесурсыПоИмени);
			КонецЕсли;
			РесурсыПоИмени.Добавить(ЭлементРесурса);
		КонецЕсли;
		
	КонецЦикла;
	
	УстановитьРасширенныеЗаголовкиСуммБухгалтерскогоУчета(ИспользуемыеРесурсы, НастройкиКомпоновки);
	
КонецПроцедуры

// Если наряду с бухгалтерской суммой выводятся и другие показатели,
// то заголовок показателей бухгалтерских сумм дополняется указанием на то, что это бухгалтерская сумма
//
Процедура УстановитьРасширенныеЗаголовкиСуммБухгалтерскогоУчета(ИспользуемыеРесурсы, НастройкиКомпоновки)
	
	Если ИспользуемыеРесурсы.Количество() < 2 Тогда
		// Сложно перепутать
		Возврат;
	КонецЕсли;
	
	ПоляБухгалтерскихСумм = ИспользуемыеРесурсы["БУ"];
	
	Если Не ЗначениеЗаполнено(ПоляБухгалтерскихСумм) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ВыбранноеПоле Из ПоляБухгалтерскихСумм Цикл
		
		Заголовок = ВыбранноеПоле.Заголовок;
		Если ПустаяСтрока(Заголовок) Тогда
			
			ИсточникПоля = НастройкиКомпоновки.ДоступныеПоляВыбора.НайтиПоле(ВыбранноеПоле.Поле);
			
			Если ИсточникПоля = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Заголовок = ИсточникПоля.Заголовок;
			
		КонецЕсли;
		
		ВыбранноеПоле.Заголовок = СтрШаблон(НСтр("ru = '%1 (БУ)'"), Заголовок);
