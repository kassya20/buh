...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	Возврат ПараметрыОтчета.СписокСформированныхЛистов;
...
Процедура ПолучитьНастройкиОтчета(ОписаниеОбъекта) Экспорт
	...
	ОписаниеОбъекта.РасходыНаНДСПоОплатеПоставщику = ОписаниеОбъекта.Отчет.КонецПериода <= УчетУСНКлиентСервер.ДатаНачалаПримененияНДС()
		И ПорядокПризнанияРасходовПоНДС = Перечисления.ПорядокПризнанияРасходовПоНДС.ПоОплатеПоставщику;
...
Функция ПодготовитьЛистыОтчета(ПараметрыОтчета)
	...
	Если ПараметрыОтчета.ПрименяетсяУСНДоходыМинусРасходы Тогда
		Если Не ПараметрыОтчета.РежимИнтеграцииСБанком Тогда
...
Процедура ПечатьТитульногоЛиста(ПараметрыОтчета)
	...
	Если Данные.БанковскиеСчета <> Неопределено Тогда
		...
		Пока БанковскиеСчетаВыборка.Следующий() Цикл
			...
			Если БанковскиеСчетаВыборка.ЦифровойСчет Тогда
				Счета = Счета + НСтр("ru = 'Цифровой рубль: '") + СокрЛП(БанковскиеСчетаВыборка.ИдентификаторЦифровогоСчета) + ", ";
			Иначе
...
Процедура ПечатьРаздела1(ПараметрыОтчета)
	...
	КолКварталов = НомерКвартала(ПараметрыОтчета.КонецПериода);
...
Функция НомерКвартала(ДатаВКвартале)
	...
	Возврат ОбщегоНазначенияБПКлиентСервер.НомерКвартала(ДатаВКвартале);
...
Функция ПодготовитьДанныеОтчета(ПараметрыОтчета)
	...
	Запрос.Текст = ТекстЗапросаБанковскиеСчета(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		...
		+ ТекстЗапросаПодсчетСтрокРазделаУменьшенияНалогаДоТекущегоКвартала(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		+ ТекстЗапросаПодсчетСтрокРазделаТорговогоСбораДоТекущегоКвартала(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		+ ТекстЗапросаОборотыРаздела1ПоКварталам(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		+ ТекстЗапросаОборотыРазделаУменьшенияНалогаПоКварталам(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		+ ТекстЗапросаОборотыРазделаТорговогоСбораПоКварталам(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		...
		+ ТекстЗапросаЗаписиРазделУменьшенияНалога(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		+ ТекстЗапросаЗаписиРазделТорговогоСбора(НомераТаблиц, ДанныеОтчета, ПараметрыОтчета)
		;
...
Функция ДвоичныеДанныеОтчета(ПараметрыОтчета)
	...
	ТипФайла   = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "ТипФайла");
	Расширение = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "Расширение");
	Если Не ЗначениеЗаполнено(ТипФайла) Или Не ЗначениеЗаполнено(Расширение) Тогда
		ТипФайла   = ТипФайлаТабличногоДокумента.PDF;
		Расширение = "pdf";
	КонецЕсли;
	
	ПрефиксИмени = СтрШаблон(НСтр("ru = 'КУДиР %1'"),
		ПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода, "ФП = Истина"));
