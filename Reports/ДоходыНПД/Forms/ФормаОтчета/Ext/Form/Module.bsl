...
Процедура УстановитьТекстОшибки(Текст);
	...
	Элементы.ТекстОшибки.Заголовок = СтроковыеФункции.ФорматированнаяСтрока(ТекстОшибки);
...
Процедура ВыполнитьЗапросыОНачислениях()
	...
	Если Не ИнтеграцияСПлатформойСамозанятыеВызовСервера.ПриложениеПодключено(Отчет.Организация) Тогда
...
Процедура УстановитьСостояниеПоляТабличногоДокументаПолучениеДанных(ПолеТабличногоДокумента, Состояние = "ПолучениеДанных") Экспорт
	...
	Если ТипЗнч(ПолеТабличногоДокумента) = Тип("ПолеФормы") 
		...
		ОтображениеСостояния = ПолеТабличногоДокумента.ОтображениеСостояния;
			...
			ИначеЕсли ВРег(Состояние) = "ОБНОВЛЕНИЕЧЕКОВ" Тогда
				
				ОтображениеСостояния.Видимость                      = Истина;
				ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;
				ОтображениеСостояния.Картинка                       = БиблиотекаКартинок.ДлительнаяОперация48;
				ОтображениеСостояния.Текст                          = НСтр("ru = 'Обновление сведений о чеках...'");
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	БухгалтерскиеОтчеты.ЗаписатьОперациюБизнесСтатистики(ЭтотОбъект, "СформироватьОтчет");
...
Процедура ПослеПолученияРезультатаВыполненияМетода(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если Ответ.Статус = СтатусыЗапросов.Выполнено Тогда
		...
		ЗагрузитьПолученныеРезультаты(Ответ.Результат, Ответ.ВремяОжидания);
...
&НаКлиенте
Процедура ЗагрузитьПолученныеРезультаты(РезультатВыполнения, ВремяОжидания)
	
	Если ТипЗнч(РезультатВыполнения) <> Тип("Структура") Тогда
		ДанныеПолученыСписокДоходов = Истина;
		Возврат;
	КонецЕсли;
	
	ЗаполнитьТаблицуПолученнымРезультатом(РезультатВыполнения);
	
	Если ЧекиНПДВызовСервера.ЕстьПравоОбновленияЧековНПД() Тогда
		УстановитьСостояниеПоляТабличногоДокументаПолучениеДанных(Элементы.Результат, "ОбновлениеЧеков");
		ПодключитьОбработчикОжидания("Подключаемый_ОбновитьЧекиПоСпискуДоходов", ВремяОжидания, Истина);
	Иначе
		ДанныеПолученыСписокДоходов = Истина;
		ОбновитьСтатусФоновогоЗадания();
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПолученнымРезультатом(РезультатВыполнения)
	...
	УдалитьИзВременногоХранилища(РезультатВыполнения.АдресВременногоХранилища);
	...
	Для Каждого КлючИЗначение Из Значение Цикл 
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("ТаблицаЗначений") Тогда
			...
			Отчеты.ДоходыНПД.ЗаполнитьТаблицуДоходов(СписокЧеков, КлючИЗначение.Значение);
...
&НаКлиенте
Процедура Подключаемый_ОбновитьЧекиПоСпискуДоходов() Экспорт
	
	ДлительнаяОперация = ОбновитьЧекиПоСпискуДоходовВФонеНаСервере();
	Если ДлительнаяОперация = Неопределено Тогда
		ДанныеПолученыСписокДоходов = Истина;
		ОбновитьСтатусФоновогоЗадания();
		Возврат;
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОжидатьОбновлениеЧековПоСпискуДоходовЗавершение", ЭтотОбъект);
...
&НаСервере
Функция ОбновитьЧекиПоСпискуДоходовВФонеНаСервере()
	
	СписокЧеков = ПолучитьИзВременногоХранилища(АдресСписокЧеков);
	Если Не ЗначениеЗаполнено(СписокЧеков) Или ТипЗнч(СписокЧеков) <> Тип("ТаблицаЗначений") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	АннулированныеЧеки = СписокЧеков.Скопировать(Новый Структура("Состояние", Перечисления.СостоянияЧековНПД.Аннулирован));
	...
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполнения,
		"РегистрыСведений.ЧекиНПД.ОбновитьЧекиПоСпискуДоходовВФоне",
		АннулированныеЧеки,
		Отчет.Организация);
...
&НаКлиенте
Процедура ОжидатьОбновлениеЧековПоСпискуДоходовЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДлительнаяОперация)
		...
		ОбновленныеЧеки = ПолучитьРезультатИзВременногоХранилища(ДлительнаяОперация.АдресРезультата);
		Если ЗначениеЗаполнено(ОбновленныеЧеки) Тогда
			Оповестить(ЧекиНПДКлиент.ИмяСобытияОбновленияСведенийОСформированныхЧеках(), Отчет.Организация);
		КонецЕсли;
		ДанныеПолученыСписокДоходов = Истина;
