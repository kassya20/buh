...
Функция ИзвлечьСтруктуруXML(ИмяФормы) Экспорт
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
		...
		Макет = ЭтотОбъект.ПолучитьМакет("СхемаВыгрузки2026Кв1");
...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
		...
		Если Дан2["П000210000103"] > 0 И НЕ (Дан2["П000020000200"] > 0) Тогда
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Раздел 2: численность работающих застрахованных лиц по обязательному социальному страхованию от несчастных "
				+ "случаев на производстве и профессиональных заболеваний должна быть больше нуля, если значение по графе 3 "
				+ "строки 1 подраздела 2.1 больше нуля", "Раздел2", "П000020000200", 1);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если Дан2["П000210000403"] > 0 И НЕ (Дан2["П000020000300"] > 0) Тогда
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Раздел 2: численность работающих инвалидов должна быть больше нуля, если значение по графе 3 "
				+ "строки 4 подраздела 2.1 больше нуля", "Раздел2", "П000020000300", 1);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (Дан2["П000020000300"] <= Дан2["П000020000200"]) Тогда
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Раздел 2: численность работающих инвалидов должна быть меньше либо равна численности работающих "
				+ "застрахованных лиц по обязательному социальному страхованию от несчастных случаев на производстве "
				+ "и профессиональных заболеваний", "Раздел2", "П000020000300", 1);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
		Если НЕ (Дан2["П000020000400"] <= Дан2["П000020000200"]) Тогда
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Раздел 2: численность работников, занятых на работах с вредными и (или) опасными производственными факторами, "
				+ "должна быть меньше либо равна численности работающих застрахованных лиц по обязательному социальному "
				+ "страхованию от несчастных случаев на производстве и профессиональных заболеваний",
				"Раздел2", "П000020000400", 1);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если ЗаполненРаздел(ИмяФормы, ДерРазд, "Раздел211", СтруктураРеквизитовФормы, СтруктураМногоуровневыхРазделов) Тогда
		...
		Если ТипСтрахователя = "1" ИЛИ ТипСтрахователя = "2" ИЛИ ТипСтрахователя = "3" Тогда
			...
			Если ИмяЗаполненногоПоказателя <> "" Тогда
				...
				СообщитьВТС(ИмяФормы, РезультатВыгрузки,
					"Подраздел 2.1: строки 5-8 не должны быть заполнены при типе страхователя """
					+ ТипСтрахователя + """ в подразделе 2.1.1", "Раздел2", ИмяЗаполненногоПоказателя, НомерСтраницы);
				СтатусПроверки = Ложь;
		...
		Если ТипСтрахователя = "1" И ДеревоМнЧ.Строки.Количество() > 1 Тогда
			
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Подраздел 2.1.1 -> Блок ""Основной ВЭД"" может содержать не более одной строки при типе страхователя """
				+ ТипСтрахователя + """", "Раздел211", "П00211М100002_2", НомерСтраницы);
			СтатусПроверки = Ложь;
			
		ИначеЕсли ТипСтрахователя = "2" И ДеревоМнЧ.Строки.Количество() > 2 Тогда
			
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
				"Подраздел 2.1.1 -> Блок ""Основной ВЭД"" может содержать не более двух строк при типе страхователя """
				+ ТипСтрахователя + """", "Раздел211", "П00211М100002_3", НомерСтраницы);
			СтатусПроверки = Ложь;
			
		Иначе
			
			ОКВЭД_ДляСравнения = ?(ЗначениеЗаполнено(ПараметрыВыгрузки.ОКВЭД),
				ПараметрыВыгрузки.ОКВЭД, ПараметрыВыгрузки.Предст_ОКВЭД);
			...
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				...
				ИначеЕсли НЕ РегламентированнаяОтчетностьКлиентСервер.ОКВЭДСоответствуетТребованиям(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли ТипСтрахователя = "2" И ЗначПок <> ОКВЭД_ДляСравнения Тогда
					СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Подраздел 2.1.1 -> Основной ВЭД: в " + НомерСтроки
						+ " строке код ОКВЭД должен соответствовать указанному на Титульном листе коду по ОКВЭД страхователя"
						+ " (коду по ОКВЭД страхователя, за которого представляются сведения, - если заполнены сведения о"
						+ " страхователе, за которого представляются сведения) при типе страхователя """ + ТипСтрахователя + """",
						"Раздел211", "П00211М100003_" + НомерСтроки, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
		...
		Если ЗаполненаМногострочнаяЧасть(ИмяФормы, ДеревоМнЧ, "Раздел211",
			...
			Если ТипСтрахователя = "2" ИЛИ ТипСтрахователя = "3" Тогда
				
				СообщитьВТС(ИмяФормы, РезультатВыгрузки,
					"Подраздел 2.1.1 -> Блок ""СКЕ"" не должен быть заполнен при типе страхователя """ + ТипСтрахователя + """",
					"Раздел211", "П002110000100", НомерСтраницы);
				СтатусПроверки = Ложь;
		...
		Иначе
			...
			Если ТипСтрахователя = "1" Тогда
				...
				СообщитьВТС(ИмяФормы, РезультатВыгрузки,
					"Подраздел 2.1.1 -> Блок ""СКЕ"" должен быть заполнен при типе страхователя """ + ТипСтрахователя + """",
					"Раздел211", "П002110000100", НомерСтраницы);
				СтатусПроверки = Ложь;
	...
	Если ЗаполненРаздел(ИмяФормы, ДерРазд, "Раздел22", СтруктураРеквизитовФормы, СтруктураМногоуровневыхРазделов) Тогда
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			...
			ИначеЕсли ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2024Кв1") Тогда
				...
				Если ЗначПок = ПараметрыВыгрузки.Предст_РегНомер Тогда
					СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Подраздел 2.2: в " + НомерСтроки
						+ " строке регистрационный номер не должен быть равен указанному на Титульном листе"
						+ " регистрационному номеру страхователя",
						"Раздел22", "П00022М100002_" + НомерСтроки, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если ЗначПок = ПараметрыВыгрузки.РегНомер Тогда
					СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Подраздел 2.2: в " + НомерСтроки
						+ " строке регистрационный номер не должен быть равен указанному на Титульном листе"
						+ " регистрационному номеру страхователя, за которого представляются сведения",
						"Раздел22", "П00022М100002_" + НомерСтроки, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			...
			Иначе
				...
				Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1")
					...
					Если ИНН = ПараметрыВыгрузки.Предст_ИНН Тогда
						СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Подраздел 2.2: в " + НомерСтроки
							+ " строке ИНН не должен быть равен указанному на Титульном листе ИНН страхователя",
							"Раздел22", "П00022М100003_" + НомерСтроки, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					
					Если ИНН = ПараметрыВыгрузки.ИНН Тогда
						СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Подраздел 2.2: в " + НомерСтроки
							+ " строке ИНН не должен быть равен указанному на Титульном листе ИНН страхователя, за которого представляются сведения",
							"Раздел22", "П00022М100003_" + НомерСтроки, НомерСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
...
Функция ВозможныеЗначенияИзСпискаВыбораПоказателя(ИмяФормы, ИмяГруппыЗначений)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
		...
		МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СпискиВыбора2026Кв1");
...
Функция ИдентификаторФайлаВыгрузки(ИмяФормы, ПараметрыВыгрузки)
	...
	Если СтрДлина(ПараметрыВыгрузки.Предст_РегНомер) = 10 Тогда
		...
		Возврат Преф + "_" + ПараметрыВыгрузки.Предст_РегНомер
		+ "_ЕФС-1_" + Формат(ТекущаяДатаСеанса(), "ДФ=ггггММдд") + "_" + ПараметрыВыгрузки.ГУИД;
		
	Иначе
		
		Возврат Преф + "_" + ПараметрыВыгрузки.Предст_РегНомер + "_" + ПараметрыВыгрузки.КодОргана
		+ "_ЕФС-1_" + Формат(ТекущаяДатаСеанса(), "ДФ=ггггММдд") + "_" + ПараметрыВыгрузки.ГУИД;
...
Процедура ЗаписатьУзелДереваВXML(ИмяФормы,
	...
	Если ТипЗнч(СтрокаДерева) = Тип("ДеревоЗначений") Тогда
		...
		Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
			...
			ПотокXML.ЗаписатьНачалоЭлемента("ЭДСФР");
			
			ПотокXML.ЗаписатьАтрибут("xmlns", "http://пф.рф/ЕФС-1/2026-01-01");
			
			ПотокXML.ЗаписатьАтрибут("xmlns:УТ8",  "http://пф.рф/УТ/2025-01-01");
			ПотокXML.ЗаписатьАтрибут("xmlns:ЕФС8", "http://пф.рф/ВС/ЕФС/2026-01-01");
			ПотокXML.ЗаписатьАтрибут("xmlns:ВС8",  "http://пф.рф/ВС/типы/2025-01-01");
			ПотокXML.ЗаписатьАтрибут("xmlns:АФ8",  "http://пф.рф/АФ/2025-01-01");
			
		Иначе// "ФормаОтчета2024Кв1", "ФормаОтчета2023Кв1"
...
Функция ПечатныйБланк(ПараметрыПечати) Экспорт
	...
	ИмяФайлаПечатногоБланка = НСтр("ru='ЕФС-1 Раздел 2 за '")
...
Процедура ВывестиНаПечатьЛист(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов, СтруктураМногоуровневыхРазделов)
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
		...
		Макет = ЭтотОбъект.ПолучитьМакет("ПечатныйБланк2026Кв1");
