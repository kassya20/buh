...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0601040";
...
Процедура ДополнитьРеквизитыФормы()
	...
	КатегорииКлассификатора =
		СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010010001;П0020010001", ";", Истина);
	...
	ИсхКолвоКолонокРазделов  = 4;
	
	ТипКолонкиСтрока    = Новый ОписаниеТипов("Строка");
	ТипКолонкиЧисло19_1 = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(19, 1));
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		Если Стр = "П0010010001" Тогда
			...
			Для Инд = 1 По 4 Цикл
				...
				Иначе
					ТипКолонки = ТипКолонкиЧисло19_1;
				КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОКПД2",
		512, , "Выбор кода ОКПД2", КоллекцияСписковВыбора["S_OKPD2"]);
...
Процедура ПоказатьПериод()
	...
	СтрПериод = ВРег(Формат(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг'"));
	
	СтрПериодТабДок = "ЗА " + СтрПериод + " ГОД";
	ПолеТабличногоДокументаФормаОтчета.Области.ПериодОтчета.Значение = СтрПериодТабДок;
	
	СтрПериодЗаголовок = СтрПериод + " г.";
	СтруктураРеквизитовФормы.СтрПериодОтчета = СтрПериодЗаголовок;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Обл = Области.Найти("П001001000102_1");
	Если Обл <> Неопределено И Не ЗначениеЗаполнено(Обл.Значение) Тогда
		Обл.Значение = "6100";
	КонецЕсли;
	
	Обл2 = Области.Найти("П002001000102_1");
	Если Обл2 <> Неопределено И Не ЗначениеЗаполнено(Обл2.Значение) Тогда
		Обл2.Значение = "7100";
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если СтрНайти(Область.Имя, "П00" + Сред(Область.Имя, 4, 1) + "001000" + Сред(Область.Имя, 11, 1) + "01") > 0
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-предприятие приложение ТЗВ");
