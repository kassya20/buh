...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ИсточникДанных) Тогда
			ТекстСообщения = СтрЗаменить(НСтр("ru='В заполнении показателя применен неизвестный источник данных. Возможно,
				|форма заполнена в прошлых версиях программы. Рекомендуется выполнить заполнение формы по данным
				|информационной базы еще раз.'"), Символы.ПС, " ");
			ВызватьИсключение ТекстСообщения;
		КонецЕсли;
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("РазделИсточникаДанных"            , РазделИсточникаДанных);
...
Процедура ЗагрузитьПодготовленныеДанныеНаСервере(РезультатВыполнения)
	...
	РезультатХранилища = ПолучитьИзВременногоХранилища(РезультатВыполнения.АдресРезультата);
	ДанныеРасшифровкиОтчета = РезультатХранилища.ДанныеРасшифровки;
...
Процедура ВывестиТаблицуНастроек(); 
	...
	Если ДеревоНастроек.Количество() > 0 Тогда
		...
		ДанныеРасшифровкиНастроек = ПоместитьВоВременноеХранилище(ДанныеДляРасшифровки, ЭтотОбъект.УникальныйИдентификатор);
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	Настройки.ДополнительныеСвойства.Вставить("ВыводитьЗаголовок", Истина);
...
Процедура ИзменениеСхемыКомпоновкиДанныхНаСервере() Экспорт
	...
	Если ЗначениеЗаполнено(ИсточникДанных) И ЗначениеЗаполнено(ХарактеристикаОбъектаНаблюдения) Тогда
		...
		СхемаКомпоновкиДанных = ПоместитьВоВременноеХранилище(Схема, ЭтотОбъект.УникальныйИдентификатор);
