...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.РазделИсточникаДанных = "ПрибыльУбыток" Тогда
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
			КомпоновщикНастроек.Настройки,
			"ПрименяетсяУСН", 
			УчетнаяПолитика.ПрименяетсяУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода));
	КонецЕсли;	
...
Процедура ЗаполнитьПараметрыРасшифровкиОтчета(Адрес, Расшифровка, ПараметрыРасшифровки) Экспорт
	...
	ИначеЕсли Данные_Расшифровки.Количество() > 0 Тогда
		...
		Если Данные_Расшифровки.Количество() > 0 Тогда
			...
			// Если значение пустое (null, неопределено, пустая ссылка), то его открытие может выглядеть как ошибка.
			// В этом случае не добавляем команду открытия значения в меню.
			// При расшифровке будет открыт текст сообщения о пустом значении расшифровки.
			...
			Иначе	
				ПараметрыРасшифровки.Вставить("Значение",  НСтр("ru='Выбранный показатель не расшифровывается'"));
			КонецЕсли; 
