...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Декларацию представляют: %1'"), КтоСдаетОтчет()));
	КонецЕсли;
...
Функция ФормаПрименяетсяВНовойРедакции()
	...
	Возврат ТекущаяДатаСеанса() >= Отчеты.РегламентированныйОтчет3НДФЛ.ДатаПримененияФормыВНовойРедакции();
...
Процедура УправлениеФормой(Форма)
	...
	Если КоличествоФорм = 0 И Год(Форма.мДатаКонцаПериодаОтчета) >= 2025 И ФормаПрименяетсяВНовойРедакции() Тогда
		
		Элементы.ОписаниеНормативДок.Видимость = Истина;
		Форма.ОписаниеНормативДок = НСтр(
			"ru='Утратила силу. Основание: приказ ФНС России от 20.10.2025 № ЕД-7-11/913@. "
			+ "С отчетности за 2025 г. применяется новая форма ""3-НДФЛ (с 2025 года)""'");
		
		Элементы.ФормаСоздатьБезПомощника.Видимость = Ложь;
		
	ИначеЕсли КоличествоФорм = 0 И Год(Форма.мДатаКонцаПериодаОтчета) >= 2026 И НЕ ФормаПрименяетсяВНовойРедакции() Тогда
		
		Элементы.ОписаниеНормативДок.Видимость = Истина;
		Форма.ОписаниеНормативДок = НСтр(
			"ru='Утратила силу. Основание: приказ ФНС России от 20.10.2025 № ЕД-7-11/913@. "
			+ "С 01.01.2026, начиная с отчетности за 2025 г., будет применяться новая форма ""3-НДФЛ (с 2025 года)""'");
