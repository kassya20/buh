...
Функция ПолучитьКурсВалютыНаДату(КодВалюты,ДатаКурса)
	...
	Если Валюта <> Неопределено Тогда
		...
		Возврат Результат.Курс / ?(ЗначениеЗаполнено(Результат.Кратность), Результат.Кратность, 1);
	Иначе
		Возврат 0;
	КонецЕсли;
	...
	Для Каждого ТекущийЛист Из ТаблицаСтраницыРаздел1 Цикл
		...
		Для Каждого ТекущееЗначение Из ТекущийЛист.Данные Цикл
			...
			Пока НачальныйИндекс * МаксимальноеКоличествоСтрок <= МаксимальныйИндекс Цикл
				...
				НоваяСтраница.Представление = СтрШаблон(Нстр("ru = 'Лист N %1'"), ТаблицаСтраницыРаздел1.Количество());
