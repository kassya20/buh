	...
	Если ЕстьДанныеПоАвансам Тогда
		...
		Для Каждого ИмяЯчейки Из ЯчейкиОКТМО Цикл
			...
			Если ПустаяСтрока(ДанныеСтраницы[ИмяЯчейки]) Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Раздела 1 не указан код ОКТМО!'"), НомерСтраницы),
					"Раздел1",
					ИмяЯчейки,
					НомерСтраницы);
			Иначе
...
Процедура ПроверитьСтраницуПриложенияКРазделу1(ДанныеСтраницы, НомерСтраницы)
	...
	Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаКодПериода]) 
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 код периода не соответствует периоду декларации!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаКодПериода,
				НомерСтраницы);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(КБКЗачет) Тогда 
		...
		Если Не ЗначениеЗаполнено(ДанныеСтраницы["П002100001005"]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 не указан номер заявления!'"), НомерСтраницы),
				"Раздел1Приложение",
				"П002100001005",
				НомерСтраницы);
		КонецЕсли;
		...
		Если ПустаяСтрока(ДанныеСтраницы[ЯчейкаОКТМО]) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 не указан код ОКТМО!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
		Иначе
			...
			Если ДлинаОКТМО <> 8 И ДлинаОКТМО <> 11 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 указан код ОКТМО некорректной длины!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
			КонецЕсли;
			Если МассивОКТМОРаздела1.Найти(СокрЛП(ДанныеСтраницы[ЯчейкаОКТМО])) = Неопределено Тогда
				 РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
					СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 код ОКТМО не соответствует ни одному из кодов ОКТМО, указанных в строках 030 Раздела 1!'"), НомерСтраницы),
					"Раздел1Приложение",
					ЯчейкаОКТМО,
					НомерСтраницы);
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(КБКВозврат) Тогда
		...
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаФамилия])
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 фамилия получателя не соответствует фамилии, указанной на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаФамилия,
				НомерСтраницы);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаИмя])
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 имя получателя не соответствует имени, указанному на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаИмя,
				НомерСтраницы);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеСтраницы[ЯчейкаОтчество]) 
			...
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				СтрШаблон(НСтр("ru = 'На странице %1 Приложения к Разделу 1 отчество получателя не соответствует отчеству, указанному на Титульном листе!'"), НомерСтраницы),
				"Раздел1Приложение",
				ЯчейкаОтчество,
				НомерСтраницы);
		КонецЕсли;
