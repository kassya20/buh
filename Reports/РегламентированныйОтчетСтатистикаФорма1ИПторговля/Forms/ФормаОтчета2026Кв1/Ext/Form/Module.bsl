...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы["ИмяМакетаТабличногоДокумента"] = "ФормаОтчета2026Кв1";
...
Процедура ИнициализироватьМногострочныеЧасти()
	...
	СтруктураРеквизитовФормы.мИдГруппыР141М1  = "П001001";
	
	СтруктураРеквизитовФормы.ДлинаИмениГруппы = 7;
	...
	ИменаГраф.Добавить("П001001000901");
	ИменаГраф.Добавить("П001001000902");
	ИменаГраф.Добавить("П001001000903");
	ИменаГраф.Добавить("П001001000904");
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКодОКВЭД2", 254, ,
	"Выбор кода ОКВЭД", КоллекцияСписковВыбора["КодОКВЭД2"]);
...
Процедура ПоказатьПериод()
	...
	ПериодОтчета = "сентябрь " + СтрПериодОтчета;
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Лев(Область.Имя, 14) = "П001001000901_" Или Лев(Область.Имя, 14) = "П001001000902_" Тогда
			Область = ТабличныйДокумент.Области.Найти(Лев(Область.Имя, 12) + "1" + Сред(Область.Имя, 14));
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникКодОКВЭД2");
		ИначеЕсли Лев(Область.Имя, 8) = "П0010010" И СтрНайти(Область.Имя, "_") = 0 И Число(Сред(Область.Имя, 10, 2)) < 8 Тогда
			СтандартнаяОбработка = Ложь;
			Модифицированность   = Истина;
			НомерСтроки = Сред(Область.Имя, 10, 2);
			Если НомерСтроки = "01" Или НомерСтроки = "02" Тогда
				РегламентированнаяОтчетностьФСГСКлиент.УстановитьОтметкуВГруппеЯчеек(
					ЭтотОбъект,Область, 1, 2, 1, 1, 1, "1", "ТабличныйДокумент");
			ИначеЕсли НомерСтроки = "03" Или НомерСтроки = "04" Тогда
				РегламентированнаяОтчетностьФСГСКлиент.УстановитьОтметкуВГруппеЯчеек(
					ЭтотОбъект,Область, 3, 4, 1, 1, 1, "1", "ТабличныйДокумент");
			ИначеЕсли НомерСтроки = "05" Или НомерСтроки = "06" Или НомерСтроки = "07" Тогда
				РегламентированнаяОтчетностьФСГСКлиент.УстановитьОтметкуВГруппеЯчеек(
					ЭтотОбъект,Область, 5, 7, 1, 1, 1, "1", "ТабличныйДокумент");
			КонецЕсли;
		КонецЕсли;
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		Возврат;
	Иначе
		Если Лев(Область.Имя, 12) = "П00100100090" И Сред(Область.Имя, 13, 1) = "1" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "СправочникКодОКВЭД2");
		КонецЕсли;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	Для Каждого СтрокаУровня1 Из мДеревоВыбранныхСтраниц.ПолучитьЭлементы() Цикл
		...
		СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-ИП (торговля). " + СтрокаУровня1.Представление);
		...
		Если мСтруктураМногоуровневыхРазделов.Свойство(СтрокаУровня1.ИмяСтраницы) Тогда
			...
			Если ИмяТекРаздела = "Раздел1" Тогда
				...
				Для Каждого Ветка Из ДеревоДанныхРаздела.Строки Цикл
					...
					НизМногострочнойЧасти = ТабличныйДокумент.Области.ОсновнаяЧасть1_3.Верх - 2;
					...
					Пока ТекущийВерх <= НизМногострочнойЧасти Цикл
						...
						Иначе
							СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-ИП (торговля)");
							ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
							ПечатнаяФорма.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
		...
		Иначе
			...
			ИначеЕсли СтрокаУровня1.ИмяСтраницы = "Подписи" Тогда
				ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
				ПечатнаяФорма.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
				ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("ОсновнаяЧасть1"));
			КонецЕсли;
