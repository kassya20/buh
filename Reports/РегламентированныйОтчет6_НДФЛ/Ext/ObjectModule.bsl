...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("БухгалтерияПредприятияПодсистемы.ПроверкаКодовПоКлассификаторам") Тогда
			ОбъекПроверки = СтруктураРеквизитовФормы.Организация;
			МодульПроверкаКодовПоКлассификаторамПереопределяемый = ОбщегоНазначения.ОбщийМодуль("ПроверкаКодовПоКлассификаторамПереопределяемый");
			МодульПроверкаКодовПоКлассификаторамПереопределяемый.ПриЗаполненииОъектаПроверкиОтчета6НДФЛ(
				ОбъекПроверки, ПараметрыВыгрузки.КПП, ПараметрыВыгрузки.КодНО);
			...
			ПараметрыПроверки.КодПоОКТМО.Объект = ОбъекПроверки;
