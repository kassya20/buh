...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	// Производитель
		...
		Если НЕ ЗначениеЗаполнено(Запись.ПроизводительИмпортер) Тогда
			...
			Если ЗначениеЗаполнено(Запись.ПроизводительИмпортер) Тогда
				Запись.П000010000303 = Запись.ПроизводительИмпортер[ИмяРеквизитаНаименование];
				Запись.П000010000305 = КПП;
			КонецЕсли;
			
		КонецЕсли;

		НаименованиеПолное = Запись.П000010000303;
		
		СтруктураХешИдПроизводителя = РегламентированнаяОтчетностьАЛКО.ПолучитьВсеИдПроизводителя(
														КодПродукции, НаименованиеПолное, ИНН, КПП);
	...
	// Производитель
		...
		Если НЕ ЗначениеЗаполнено(Запись.ПроизводительИмпортер) Тогда
			...
			Если ЗначениеЗаполнено(Запись.ПроизводительИмпортер) Тогда
				Запись.П000020000303 = Запись.ПроизводительИмпортер[ИмяРеквизитаНаименование];
				Запись.П000020000305 = КПП;
			КонецЕсли;
			
		КонецЕсли;
		
		НаименованиеПолное = Запись.П000020000303;
		...
		Если НЕ ЗначениеЗаполнено(Запись.Поставщик) Тогда
			...
			Если ЗначениеЗаполнено(Запись.Поставщик) Тогда
				Запись.П000020000306 = Запись.Поставщик[ИмяРеквизитаНаименование];
				Запись.П000020000308 = КПП;
			КонецЕсли;
			
		КонецЕсли;
		
		НаименованиеПолное = Запись.П000020000306;
		...
		СтруктураСведений.Вставить("Наименование", НаименованиеПолное);
...
Процедура ПроверитьЗаполнениеРаздел2(СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		РегламентированнаяОтчетностьАЛКО.ВерноеЗаполнениеОП(ДокументСсылкаАлкоОтчет, 
...
// В структуре СтруктураПараметровСтраницы уже установлены свойства 
	...
	СписокПереносимыхПолей.Добавить(26, "П000010000301");
	СписокПереносимыхПолей.Добавить(26, "П000010000303");
...
// В структуре СтруктураПараметровСтраницы уже установлены свойства 
	...
	// Действуют и на общую часть по организации и на часть по ОП.
	СписокПереносимыхПолей = Новый СписокЗначений;
	СписокПереносимыхПолей.Добавить(26, "П000020000301");
	СписокПереносимыхПолей.Добавить(26, "П000020000303");
	СписокПереносимыхПолей.Добавить(26, "П000020000306");
	СписокПереносимыхПолей.Добавить(26, "П000020000312");
	СписокПереносимыхПолей.Добавить(26, "П000020000314");
	СписокПереносимыхПолей.Добавить(26, "П000020000315");
...
Функция ДеревоФормИФорматов() Экспорт
	...
	ОпределитьФорматВДеревеФормИФорматов(Форма20140408, "4.31", , , '2015-09-01', '2020-12-31');
