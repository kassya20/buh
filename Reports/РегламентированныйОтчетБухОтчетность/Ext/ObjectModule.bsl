...
Процедура ВыполнитьОбработчикиОбновленияФормаОтчета2025Кв1(СохраненныеДанные,
	СохраненнаяВерсия, СохраненныеДанныеИзменены, ПараметрыОбновления)
	...
	Если ВыполнитьПриПереходеНаВерсию("БРО.2026.04.01", СохраненнаяВерсия) Тогда
		ОбновитьСтруктурыФормаОтчета2025Кв1_20260401(СохраненныеДанные, ПараметрыОбновления);
		СохраненныеДанныеИзменены = Истина;
	КонецЕсли;
	
	Если ВыполнитьПриПереходеНаВерсию("БРО.2026.04.02", СохраненнаяВерсия) Тогда
		ОбновитьСтруктурыФормаОтчета2025Кв1_20260402(СохраненныеДанные);
		СохраненныеДанныеИзменены = Истина;
	КонецЕсли;
...
Процедура ОбновитьСтруктурыФормаОтчета2025Кв1_20260401(СохраненныеДанные, ПараметрыОбновления)
	
	Если СохраненныеДанные.ЭтоБалансНекоммерческойОрганизации Тогда
		ДеревоДанныхРазделов = СохраненныеДанные.ДеревоДанныхРазделов;
		ДанныеРаздела = ВеткаРаздела(ДеревоДанныхРазделов, "Баланс");
		
		НайденныеСтроки = ДанныеРаздела.Строки.НайтиСтроки(Новый Структура("Тип, Идентификатор", "Показатель", "КодНО"));
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтрокаПоказателяКодНО = ДанныеРаздела.Строки.Добавить();
			СтрокаПоказателяКодНО.Тип = "Показатель";
			СтрокаПоказателяКодНО.Идентификатор = "КодНО";
			СтрокаПоказателяКодНО.Данные = ПараметрыОбновления.КодНалоговогоОргана;
		КонецЕсли;
		
		НайденныеСтроки = ДанныеРаздела.Строки.НайтиСтроки(Новый Структура("Тип, Идентификатор", "Показатель", "КПП"));
		Если НайденныеСтроки.Количество() = 0 Тогда
			СтрокаПоказателяКПП = ДанныеРаздела.Строки.Добавить();
			СтрокаПоказателяКПП.Тип = "Показатель";
			СтрокаПоказателяКПП.Идентификатор = "КПП";
			СтрокаПоказателяКПП.Данные = СокрЛП(ПараметрыОбновления.Организация.КПП);
		КонецЕсли;
...
Процедура ОбновитьСтруктурыФормаОтчета2025Кв1_20260402(СохраненныеДанные)
	
	// Настройки видимости фиксированных строк по умолчанию.
	ДеревоДанныхРазделов = СохраненныеДанные.ДеревоДанныхРазделов;
	
	ДанныеРаздела = ВеткаРаздела(ДеревоДанныхРазделов, "ОФР");
	СтрокиСУправляемойВидимостью = "2900, 2910";
	СкрытыеСтроки = "";
...
Функция СохраненныеДанные(СохраненныйДокумент) Экспорт
	...
	ПараметрыОбновления.Вставить("Организация", СохраненныйДокумент.Организация);
	ПараметрыОбновления.Вставить("КодНалоговогоОргана", СохраненныйДокумент.КодНалоговогоОргана);
	ОбновитьСтруктурыДанныхСохраненногоОтчета(СохраненныйДокумент.ВыбраннаяФорма, СохраненныеДанные, ПараметрыОбновления);
