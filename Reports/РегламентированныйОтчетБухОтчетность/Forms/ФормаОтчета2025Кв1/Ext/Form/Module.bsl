...
Процедура Проверить(Команда)
	...
	ЗафиксироватьДействиеСФормой("Команда.Проверить");
	ДополнительныеПараметры = Новый Структура;
...
Процедура УдалитьПустыеСтрокиРасшифровок(Команда)
	...
	ЗафиксироватьДействиеСФормой("Команда.УдалитьПустыеСтрокиРасшифровок");
...
Процедура УдалитьВсеСтрокиРасшифровок(Команда)
	...
	ЗафиксироватьДействиеСФормой("Команда.УдалитьВсеСтрокиРасшифровок");
...
Процедура СформироватьСтруктуруРаздела_ОФР(ДеревоДанныхРазделов)
	...
	СтрокиСУправляемойВидимостью = "2110, 2120, 2210, 2220, 2310, 2320, 2330, 2340, 2350,
		|2410, 2420, 2460, 2510, 2520, 2530, 2900, 2910";
	СкрытыеСтроки = "2460";
...
Процедура СобратьДанныеТабличногоПоля(ТабличноеПоле, ХранилищеПоказателей, ХранилищеМСЧ)
	...
	Для Каждого ОбластьТД Из ТабличноеПоле.Области Цикл
		...
		ЭтоСлужебнаяОбласть = СтрНачинаетсяС(ИмяПоказателя, "ДобавитьСтроку")
		                      ...
		                      ИЛИ СтрНачинаетсяС(ИмяПоказателя, "ДобавитьСиблинга")
		                      ИЛИ СтрНачинаетсяС(ИмяПоказателя, "ДобавитьПодстроку")
		                      ИЛИ СтрНачинаетсяС(ИмяПоказателя, "Скрыть");
...
Процедура ОчиститьРазделыОтчета(ИмяРаздела = "")
	...
	СписокПоказателейБаланса = СписокУниверсальныхПоказателей + ",
		...
		|НаименованиеУтверждающегоОрганаВЗаголовке, КодНО, КПП";
...
Процедура ОбработатьПоляНалоговогоОргана()
	...
	ЭтоНеРазделБаланса = ОбластьКодНО = Неопределено;
	Если ЭтоНеРазделБаланса Тогда
		Возврат;
	КонецЕсли;
...
Процедура СформироватьДанныеОтчета()
	...
	СохраненныеДанные = ДанныеОтчетаДляКонтрольныхСоотношений();
...
Процедура ПоместитьДеревоРазделовВКэш(ДеревоДанныхРазделов)
	...
	Если НЕ ТипЗнч(ДеревоДанныхРазделов) = Тип("ДеревоЗначений") Тогда
		ИмяПроцедуры = "Отчет.РегламентированныйОтчетБухОтчетность.ФормаОтчета2025Кв1.ПоместитьДеревоРазделовВКэш";
...
&НаСервере
Функция ДанныеОтчетаДляКонтрольныхСоотношений()
	
	СохраненныйДокумент = СтруктураРеквизитовФормы.мСохраненныйДок;
	СохраненныеДанные = ОбъектОтчета().СохраненныеДанные(СохраненныйДокумент);
	
	// В отчете может присутствовать паразитный набор данных вписываемых строк, собранный с экранных форм,
	// приводящий к некорректному расчету контрольных соотношений.
	ДеревоДанныхРазделов = СохраненныеДанные.ДеревоДанныхРазделов;
	УдаляемыеПоказатели = Новый Массив;
	
	ДанныеРазделов = БухгалтерскаяОтчетностьБРО.НайтиСтрокиПоПорядку(ДеревоДанныхРазделов.Строки,
		Новый Структура("Тип", "Раздел"));
	Для Каждого ДанныеРаздела Из ДанныеРазделов Цикл
		Показатели = БухгалтерскаяОтчетностьБРО.НайтиСтрокиПоПорядку(ДанныеРаздела.Строки,
			Новый Структура("Тип", "Показатель"));
		Для Каждого Показатель Из Показатели Цикл
			Если СтрНачинаетсяС(Показатель.Идентификатор, "П10001") Тогда
	...
	Для Каждого УдаляемыйПоказатель из УдаляемыеПоказатели Цикл
		УдаляемыйПоказатель.Родитель.Строки.Удалить(УдаляемыйПоказатель);
	КонецЦикла;
