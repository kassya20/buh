...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	//Раздел 1
	//гр.1 по строкам 02-04, 06-12, 20 = (гр.2 + гр.3 + гр.4) по строкам 02-04, 06-12, 20;
	Для Стр = 2 По 20 Цикл
		Если Стр = 5 Или (Стр >= 13 И Стр < 20) Тогда
			Продолжить;
		КонецЕсли;
		
		НомСтр = Формат(Стр, "ЧЦ=2; ЧВН=");
		
		Если Стр = 8 Или Стр = 10 Или Стр = 12 Тогда
			
			СтрНом = 0;
			Пока Истина Цикл
				Знч = 0;
				СтрНом = СтрНом + 1;
				Если Области.Найти("П00100000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "01_" + СтрНом) = Неопределено Тогда
					Прервать;
				КонецЕсли;
				
				Знч = Знч
					+ Области["П00100000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "02_" + СтрНом].Значение
					+ Области["П00100000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "03_" + СтрНом].Значение
					+ Области["П00100000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "04_" + СтрНом].Значение;
				
				Области["П00100000" + Формат(Стр, "ЧЦ=2; ЧВН=") + "01_" + СтрНом].Значение = Знч;
			КонецЦикла;
		Иначе
	...
	//стр. 07 = ∑стр. 8 по гр. 1-7 по всем строкам;
	Для Гр = 1 По 7 Цикл
		
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ГрафаМнг = Формат(?((Гр = 6 Или Гр = 7), Гр + 3, Гр), "ЧЦ=2; ЧВН=");
		
		Знч = 0;
		СтрНом = 0;
		Пока Истина Цикл
			СтрНом = СтрНом + 1;
			Если Области.Найти("П0010000008" + ГрафаМнг + "_" + СтрНом) = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Знч = Знч + Области["П0010000008" + ГрафаМнг + "_" + СтрНом].Значение;
		КонецЦикла;
		
		Области["П0010010007" + Графа].Значение = Знч;
	КонецЦикла;
	
	//стр. 09 = ∑ стр. 10 по гр. 1-7 по всем строкам;
	Для Гр = 1 По 7 Цикл
		...
		Пока Истина Цикл
			СтрНом = СтрНом + 1;
			Если Области.Найти("П0010000010" + ГрафаМнг + "_" + СтрНом) = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			Знч = Знч + Области["П0010000010" + ГрафаМнг + "_" + СтрНом].Значение;
		КонецЦикла;
		
		Области["П0010010009" + Графа].Значение = Знч;
	КонецЦикла;
	
	//стр. 11 = ∑ стр. 12 по гр. 1–7 по всем строкам;
	Для Гр = 1 По 7 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
	...
	//стр. 01 = стр. (02+03+04+06+07+09+11+13+20) по гр. 1, 2, 4;
	Для Гр = 1 По 4 Цикл
		Если Гр = 3 Тогда
			Продолжить;
		КонецЕсли;
