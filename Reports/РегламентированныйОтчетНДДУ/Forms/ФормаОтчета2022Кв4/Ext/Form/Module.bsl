...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ФормаВРедакцииОт20230919", Ложь);
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если ТекущаяДатаСеанса() >= '20260512' Тогда
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20230919"] = Ложь;
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20220808"] = Ложь;
	ИначеЕсли Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2024 Тогда
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20230919"] = Истина;
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20220808"] = Ложь;
	ИначеЕсли Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2024 Тогда
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20230919"] = Ложь;
		СтруктураРеквизитовФормы["ФормаВРедакцииОт20220808"] = Истина;
	КонецЕсли;
...
Процедура СформироватьСпискиВыбора()
		...
		Если Область.ТипОбласти = ТипОбластиЯчеекТабличногоДокумента.Строки Тогда
			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					Если Область.Имя = "КодыОснованияНалогообложения"
	...
	ИмяСекции_КодыОснованияВосстановленияФактическихРасходов = ?(ТекущаяДатаСеанса() >= '20260512',
		"КодыОснованияВосстановленияФактическихРасходов_20260512", "КодыОснованияВосстановленияФактическихРасходов");
	...
	КоллекцияСписковВыбора[ИмяСекции_КодыОснованияВосстановленияФактическихРасходов]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П001070065003"
		...
		И Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) <= 2026 Тогда
		...
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2026 Тогда
			СписокКоэффициентов.Добавить(1.520, "1,520");
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Титульный" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы["ФормаВРедакцииОт20230919"] Тогда
			
			Обл = ТДО.Найти("РеквизитыПриказа");
			Если Обл <> Неопределено Тогда
				Обл.Текст = НСтр("ru='Утверждена приказом ФНС России
					|от 30.03.2021 № КВ-7-3/234@
					|(в редакции приказа ФНС России
					|от 19.09.2023 № ЕД-7-3/650@)'");
			КонецЕсли;
			
		ИначеЕсли Форма.СтруктураРеквизитовФормы["ФормаВРедакцииОт20220808"] Тогда
...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Если ПараметрыПечати.ИмяСтраницы = "Титульный" Тогда
		...
		// Особая обработка: реквизиты приказа.
		Если СтруктураРеквизитовФормы["ФормаВРедакцииОт20230919"] Тогда
		...
		ИначеЕсли СтруктураРеквизитовФормы["ФормаВРедакцииОт20220808"] Тогда
