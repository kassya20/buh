...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.РазделыОтчета.РежимВыделения = РежимВыделенияТаблицы.Множественный;
...
Процедура СформироватьСпискиВыбора()
	...
	//Прил. 3.1 к разд. 1.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П00131М100101", 1, ,
	"Выбор кода основания исчисления", КоллекцияСписковВыбора["КодыПлательщикаРаздел1Прил31"]);
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если ЗначениеЗаполнено(ИмяГруппы) Тогда
		...
		Координаты = РегламентированнаяОтчетностьКлиентСервер.СтрокаСКоординатамиПоказателя(
			ТабличныйДокумент.ТекущаяОбласть.Имя);
		...
		Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2Подр1" Тогда
			...
			Если Обл <> Неопределено И СокрЛП(Обл.Значение) = "2"
				И СтрНачинаетсяС(ТабличныйДокумент.ТекущаяОбласть.Имя, "П00021М2") Тогда
				Элементы.ТабличныйДокументКонтекстноеМенюУдалитьСтроку.Доступность = Ложь;
				Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку.Доступность = Ложь;
			КонецЕсли;
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел1Подр1" ИЛИ ИмяРаздела = "Раздел1Подр3" ИЛИ ИмяРаздела = "Раздел1Подр4"
		ИЛИ ИмяРаздела = "Раздел1Прил2" ИЛИ ИмяРаздела = "Раздел1Прил3" ИЛИ ИмяРаздела = "Раздел1Прил5"
		ИЛИ ИмяРаздела = "Раздел2" ИЛИ ИмяРаздела = "Раздел4" ИЛИ ИмяРаздела = "Раздел4Подр1"
		ИЛИ ИмяРаздела = "Раздел4Прил1" Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил31" Тогда
		...
		Пока Обл <> Неопределено Цикл
			...
			Если СокрЛП(Обл.Значение) = "2" Тогда
				...
				ЗапретитьВводДанныхВОбласть(Форма, ТДО.Найти("П00131М101001" + Постфикс));
				ЗапретитьВводДанныхВОбласть(Форма, ТДО.Найти("П00131М102001" + Постфикс));
				
				ЗапретитьВводДанныхВОбласть(Форма, ТДО.Найти("П00131М103001" + Постфикс));
				Инд = АвтоРасчПоказатели.Найти("П00131М103001" + Постфикс);
			...
			Иначе
				...
				РазрешитьВводДанныхВОбласть(Форма, ТДО.Найти("П00131М101001" + Постфикс));
				РазрешитьВводДанныхВОбласть(Форма, ТДО.Найти("П00131М102001" + Постфикс));
				
			КонецЕсли;
			
			Ном = Ном + 1;
			Постфикс = РегламентированнаяОтчетностьКлиентСервер.СтрокаСИндексомПоказателя(Ном);
			Обл = ТДО.Найти("П00131М100101" + Постфикс);
...
Процедура ДобавитьСтроку(Команда)
	...
	ИначеЕсли СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил31" Тогда
		
		ИмяПервойГрафы = СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[
		СтруктураРеквизитовФормы.мИдГруппы131М1]["ИменаГраф"][0];
		Если ТабличныйДокумент.Области.Найти(ИмяПервойГрафы + "_2") <> Неопределено Тогда
			ТекстПредупреждения = НСтр(
			"ru = 'Возможно не более двух групп строк 001-030 в приложении 3.1 к разделу 1'");
		КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку()
	...
	Если СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей.Свойство(ИмяГруппы) Тогда
		...
		Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1Прил31"
			ИЛИ СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2Подр1"
	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил31" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Если СокрЛП(ТДО["П00131М100101" + _Инд].Значение) = "2" Тогда
					ТДО["П00131М103001" + _Инд].Значение = 0;
				Иначе
					Знаменатель = ТДО["П00131М101001" + _Инд].Значение;
					ТДО["П00131М103001" + _Инд].Значение = ?(Знаменатель = 0,
						0, Окр(ТДО["П00131М102001" + _Инд].Значение / Знаменатель * 100, 2));
				КонецЕсли;
				
				Знаменатель = ТДО["П00131М101002" + _Инд].Значение;
				ТДО["П00131М103002" + _Инд].Значение = ?(Знаменатель = 0,
					0, Окр(ТДО["П00131М102002" + _Инд].Значение / Знаменатель * 100, 2));
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Раздел1Прил31" Тогда
		...
		ИмяГруппыМнЧ = Форма.СтруктураРеквизитовФормы.мИдГруппы131М1;
		СтруктураГруппыМнЧ = Форма.СтруктураРеквизитовФормы.СтруктураМногострочныхЧастей[ИмяГруппыМнЧ];
		ИменаГрафМнЧ = СтруктураГруппыМнЧ["ИменаГраф"];
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			...
			Если СокрЛП(ДанСтр["П00131М100101"]) = "2" Тогда
				ДанСтр.Вставить("П00131М103001", 0);
			Иначе
				Знаменатель = ДанСтр["П00131М101001"];
				ДанСтр.Вставить("П00131М103001", ?(Знаменатель = 0, 0, Окр(ДанСтр["П00131М102001"] / Знаменатель * 100, 2)));
			КонецЕсли;
			
			Знаменатель = ДанСтр["П00131М101002"];
			ДанСтр.Вставить("П00131М103002", ?(Знаменатель = 0, 0, Окр(ДанСтр["П00131М102002"] / Знаменатель * 100, 2)));
		КонецЦикла;
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ = Новый Структура;
	Структ.Вставить("П00131М103001");
	Структ.Вставить("П00131М103002");
...
Функция УниверсальнаяВыгрузка(ТекстВыгрузки = Неопределено, ПроверятьБлокировки = Истина, ПроверятьВозможностьВыгрузки = Истина) Экспорт
	...
	Если СтруктураРеквизитовФормы.мПроверятьСоотношенияПриПечатиИВыгрузки = Истина Тогда // возможны 3 состояния
		...
		СтруктураПараметров.Вставить("ИмяФормы"			, "ФормаОтчета2026Кв1"); 
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Если ВидПечати <> "ПоказатьРеестр" Тогда
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru='%1. Вывод на печать прерван'"), ТекстСообщения));
			Возврат;
		КонецЕсли;
...
Функция ПечатьРеестраНаСервере(ВидПечати, ЗаголовокПечатнойФормы)
	...
	ИмяМакета = "ПечатныйБланк2026Кв1_Реестр";
...
Процедура Проверить(Команда)
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 2026 Тогда
		...
		"ru='Проверка контрольных соотношений для 2027 и более поздних 
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) < 2026 Тогда
		...
		ПоказатьПредупреждение(, НСтр(
		"ru='Проверка контрольных соотношений для 2025 и более ранних 
	...
	ДополнительныеПараметры.Вставить("ИмяФормы"			, "ФормаОтчета2026Кв1"); 
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.Сценарий = "Основной" Тогда 
		// длительная операция
		Возврат ДлительныеОперации.ВыполнитьВФоне("РегламентированнаяОтчетность.ПроверитьКонтрольныеСоотношенияДлительнаяОперация",
...
Функция ВыполнитьДокументированиеНаСервере()
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("НаименованиеФормы", "2026Кв1");
	
	Возврат ОбъектОтчета.ВыполнитьДокументированиеОбъект2026(ДополнительныеПараметры);
...
Функция ВыполнитьПоискКопийРеглОтчетовНаСервере(ДополнительныеПараметры)
	...
	|	И (Документ.ВыбраннаяФорма = ""ФормаОтчета2025Кв1"" ИЛИ Документ.ВыбраннаяФорма = ""ФормаОтчета2026Кв1"") 
