...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли РегламентированнаяОтчетностьКлиент.ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(ЭтаФорма, Область, СтандартнаяОбработка) Тогда
		Возврат;
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П0010010") Тогда
		...
		ИначеЕсли Прч >= 122 И Прч <= 123 Тогда
			...
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 124 И Прч <= 130 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 124 по 130 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 131 И Прч <= 133 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 131 по 133 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		ИначеЕсли Прч >= 134 И Прч <= 136 Тогда
			НЗ = ?(ЗначениеЗаполнено(Область.Значение), "", "1");
			Для Инд = 134 по 136 Цикл
				ПолеТабличногоДокументаФормаОтчета.Области["П0010010" + Инд + "01"].Значение = "";
			КонецЦикла;
			Область.Значение = НЗ;
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Строка 311 = сумме строк 312, 313 по графе 1;
	Области["П000030131101"].Значение
		= Области["П000030131201"].Значение
		+ Области["П000030131301"].Значение;
	
	//Строка 314 = сумме строк 315, 316 по графе 1;
	Области["П000030131401"].Значение
		= Области["П000030131501"].Значение
		+ Области["П000030131601"].Значение;
	
	//Справка 2.
	//Строка 318 = сумме строк 319–321.
	Области["П003002031801"].Значение
		= Области["П003002031901"].Значение
		+ Области["П003002032001"].Значение
		+ Области["П003002032101"].Значение;
...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Области["П000030100102_1"].Значение = "317";
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	мФормаОтчета.Области["П000030131401"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П000030131401"].Защита   = Не ОтклАвтоРасчетЗначение;
	
	мФормаОтчета.Области["П003002031801"].ЦветФона = ЦветФонаЗначение;
	мФормаОтчета.Области["П003002031801"].Защита   = Не ОтклАвтоРасчетЗначение;
КонецПроцедуры
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки) Экспорт 
	...
	Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013101"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД01001013101", "1");
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013201"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД01001013101", "2.АГ");
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013401"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "2");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013501"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "3");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013601"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "4");
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013301"].Значение) Тогда
		ПараметрыВыгрузки.Вставить("ПД01001013101", "3.АГ");
		Если ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013401"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "5");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013501"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "6");
		ИначеЕсли ЗначениеЗаполнено(ПолеТабличногоДокументаФормаОтчета.Области["П001001013601"].Значение) Тогда
			ПараметрыВыгрузки.Вставить("ПД01001013401", "7");
		КонецЕсли;
	КонецЕсли;
