...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ИначеЕсли ИдентификаторМакета = "РасшифровкаВосстановлениеНДС" Тогда
		ПолучитьНаборДанныхВосстановлениеНДС(Запрос, СтруктураВнешнихНаборовДанных);
	...
	ИначеЕсли ИдентификаторМакета = "РасшифровкаСтроки125Декларации" Тогда
		ПолучитьНаборДанныхВычетПоПриобретеннымЦенностямРасшифровка125СтрокиДекларации(Запрос, СтруктураВнешнихНаборовДанных);
...
Функция ПолучитьПараметрыЗапроса(ПараметрыОтчета)
	...
	ПлательщикНДСНаУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода);
	Если ПлательщикНДСНаУСН Тогда
		ВидСтавкиПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода);
		СтавкаНДСОрганизации = Перечисления.СтавкиНДС.СтавкаНДС(ВидСтавкиПриУСН, ПараметрыОтчета.КонецПериода); 
		СтавкаНДСПредположительная = Перечисления.СтавкиНДС.РасчетнаяСтавкаНДСПоОбычной(СтавкаНДСОрганизации);
	Иначе
		СтавкаНДСОрганизации = УчетНДСКлиентСервер.ОбщаяСтавкаНДС(ПараметрыОтчета.КонецПериода);
		СтавкаНДСПредположительная = УчетНДСКлиентСервер.ОбщаяРасчетнаяСтавкаНДС(ПараметрыОтчета.КонецПериода);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("КодОперацииПоУмолчанию", 
		УчетНДСВызовСервера.КодОперацииПоСтавке0ПоУмолчанию(Запрос.Параметры.КонецПериода, Запрос.Параметры.Организация));
	
	Запрос.УстановитьПараметр("СтавкаНДСКакМножитель",      УчетНДСПереопределяемый.СтавкаНДСКакМножитель(СтавкаНДСОрганизации));
	Запрос.УстановитьПараметр("СтавкаНДСПредположительная", СтавкаНДСПредположительная);
	
	Запрос.УстановитьПараметр("ОбщаяРасчетнаяСтавкаНДС", УчетНДСКлиентСервер.ОбщаяРасчетнаяСтавкаНДС(ПараметрыОтчета.КонецПериода));
	Запрос.УстановитьПараметр("ОбщаяРасчетнаяУСН",       УчетНДСКлиентСервер.РасчетнаяУСНСтавкаНДС(ПараметрыОтчета.КонецПериода));
	Запрос.УстановитьПараметр("ПониженнаяРасчетнаяУСН",  УчетНДСКлиентСервер.ПониженнаяРасчетнаяУСНСтавкаНДС(ПараметрыОтчета.КонецПериода));
	
	Запрос.УстановитьПараметр("ОбщаяРасчетнаяСтавкаНДСКАКМножитель", УчетНДСПереопределяемый.СтавкаНДСКакМножитель(УчетНДСКлиентСервер.ОбщаяРасчетнаяСтавкаНДС(ПараметрыОтчета.КонецПериода)));
	Запрос.УстановитьПараметр("ОбщаяРасчетнаяУСНКАКМножитель",       УчетНДСПереопределяемый.СтавкаНДСКакМножитель(УчетНДСКлиентСервер.РасчетнаяУСНСтавкаНДС(ПараметрыОтчета.КонецПериода)));
	Запрос.УстановитьПараметр("ПониженнаяРасчетнаяУСНКАКМножитель",  УчетНДСПереопределяемый.СтавкаНДСКакМножитель(УчетНДСКлиентСервер.ПониженнаяРасчетнаяУСНСтавкаНДС(ПараметрыОтчета.КонецПериода)));
...
Процедура ПолучитьНаборДанныхНачислениеРеализация(Запрос, СтруктураВнешнихНаборов)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА КнигаПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
	|					КОГДА КнигаПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
	|					КОГДА КнигаПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	|					КОГДА КнигаПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|						ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|					КОГДА КнигаПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	...
	|	И СчетФактураВыданныйДокументыОснования.Ссылка.ВидСчетаФактуры В (ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаРеализацию), ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.Корректировочный), ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.СводнаяСправка))
	|	И НЕ СчетФактураВыданныйДокументыОснования.ДокументОснование ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|	И ВЫБОР
...
Процедура ПолучитьНаборДанныхНачислениеНалоговыйАгент(Запрос, СтруктураВнешнихНаборов) 
	...
	|	&СтавкаНДСПредположительная КАК СтавкаНДС,
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям(Запрос, СтруктураВнешнихНаборов, ТолькоВключенные, ТолькоПеренесенные)
	...
	Иначе
		...
		|	И НЕ СчетФактураВыданный.ДокументОснование ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	...
	Иначе
		...
		Иначе
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ТаблицаДокумента.Ссылка КАК ДокументРегистратор,
			|	ВЫБОР
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2019, 1, 1)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2026, 1, 1)
			|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
			|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
			|	КОНЕЦ КАК СтавкаНДС,
			|	ВЫБОР
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2019, 1, 1)
			|			ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента * 18 / 118 КАК ЧИСЛО(15, 2))
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2026, 1, 1)
			|			ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента * 20 / 120 КАК ЧИСЛО(15, 2))
			|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента * 22 / 122 КАК ЧИСЛО(15, 2))
			|	КОНЕЦ КАК СуммаНДС,
			|	ВЫБОР
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2019, 1, 1)
			|			ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента - ТаблицаДокумента.СуммаДокумента * 18 / 118 КАК ЧИСЛО(15, 2))
			|		КОГДА ТаблицаДокумента.Дата < ДАТАВРЕМЯ(2026, 1, 1)
			|			ТОГДА ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента - ТаблицаДокумента.СуммаДокумента * 20 / 120 КАК ЧИСЛО(15, 2))
			|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаДокумента.СуммаДокумента - ТаблицаДокумента.СуммаДокумента * 22 / 122 КАК ЧИСЛО(15, 2))
			|	КОНЕЦ КАК Сумма,
			|	1 КАК КоличествоДокументов,
			...
			|	И ТаблицаДокумента.Организация В(&Организация)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	&ТекстНеОтраженыВКнигеПокупок
			|ИЗ
			|	Документ.ПоступлениеНМА КАК ТаблицаДокумента
			|ГДЕ
			|	ТаблицаДокумента.Ссылка В
			|			(ВЫБРАТЬ
			|				НДСВключенныйВСтоимость.СчетФактура
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностямРасшифровка130СтрокиДекларации(Запрос, СтруктураВнешнихНаборов) 
	...
	Запрос.Текст = ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностямРасшифровкаСтрок125и130Декларации();
...
Процедура ПолучитьНаборДанныхВычетНалоговыйАгент(Запрос, СтруктураВнешнихНаборов) 
	...
	|
	|ВЫБРАТЬ
	|	ЗачетПредоплаты.КорСубконто1,
	|	ЗачетПредоплаты.Субконто1
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(
	|			,
	|			&КонецПериодаГраница,
	...
	|					ИЗ
	|						НДСПредъявленный КАК НДСПредъявленный),
	|			КорСчет В (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоАвансамВыданным), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоАвансамВыданнымВал), ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоАвансамВыданнымУЕ)),
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами)) КАК ЗачетПредоплаты
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
	...
	|					ТОГДА &ТекстСФНалоговогоАгентаНеВыписан
	|				КОГДА НЕ ОснованияСчетаФактуры.Проведен
	|					ТОГДА &ТекстСФНалоговогоАгентаНеПроведен
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный КАК ОснованияСчетаФактуры
	|			ПО ТаблицаСвязиОплат.Регистратор = ОснованияСчетаФактуры.ДокументОснование
	|				И (НЕ ОснованияСчетаФактуры.ПометкаУдаления)
	...
	|			И НДСПредъявленный.ДоговорКонтрагента = СчетФактураПолученный.Ссылка.ДоговорКонтрагента
	|			И (НЕ СчетФактураПолученный.Ссылка.ПометкаУдаления)
	|ГДЕ
	|	(НДСПредъявленный.СуммаБезНДСОстаток > 0
	|			ИЛИ НДСПредъявленный.НДСОстаток > 0)
	|	И ЕСТЬNULL(ОснованияСчетаФактуры.ВидСчетаФактуры, ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НалоговыйАгент)) = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НалоговыйАгент)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			И НДСЗаписиКнигиПокупокОбороты.ДоговорКонтрагента = СчетФактураПолученный.Ссылка.ДоговорКонтрагента";
...
Процедура ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТРеализации.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	...
	|		КОГДА ВТРеализации.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|		КОГДА ВТРеализации.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	...
	|	&СтавкаНДСПредположительная КАК СтавкаНДСПредположительная,
	|	ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт КАК Сумма,
	|	ВЫРАЗИТЬ(ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт - ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт * &СтавкаНДСКакМножитель КАК ЧИСЛО(15, 2)) КАК СуммаБезНДС,
	|	ВЫРАЗИТЬ(ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстатокКт * &СтавкаНДСКакМножитель КАК ЧИСЛО(15, 2)) КАК СуммаНДС
	...
	|						ТОГДА ВЫБОР
	|								КОГДА СуммыПлатежей.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	|									ТОГДА ВТАвансыПолученныеВременнаяТаблица.Сумма * 22 / 122
	...
	|								КОГДА СуммыПлатежей.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|									ТОГДА ВТАвансыПолученныеВременнаяТаблица.Сумма * 5 / 105
	|								КОГДА СуммыПлатежей.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|									ТОГДА ВТАвансыПолученныеВременнаяТаблица.Сумма * 7 / 107
	|								ИНАЧЕ 0
	|							КОНЕЦ
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностямУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	Документ.ПоступлениеДопРасходов КАК ТаблицаДокумента
	|ГДЕ
	|	ТаблицаДокумента.НДСВключенВСтоимость
	|	И ТаблицаДокумента.Проведен
	|	И ТаблицаДокумента.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ТаблицаДокумента.Организация В(&Организация)
	|	И НЕ ТаблицаДокумента.Ссылка В
...
Процедура ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов, ТолькоВключенные) 
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	|						ТОГДА &ОбщаяРасчетнаяСтавкаНДС
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры)
	|						ТОГДА &ОбщаяРасчетнаяСтавкаНДС
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН)
	|						ТОГДА &ОбщаяРасчетнаяУСН
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН)
	|						ТОГДА &ПониженнаяРасчетнаяУСН
	|					ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДС
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры)
	|					ТОГДА &ОбщаяРасчетнаяСтавкаНДС
	|				КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН)
	|					ТОГДА &ОбщаяРасчетнаяУСН
	|				КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН)
	|					ТОГДА &ПониженнаяРасчетнаяУСН
	|				ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДС
	|			КОНЕЦ
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	|						ТОГДА &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры)
	|						ТОГДА &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН)
	|						ТОГДА &ОбщаяРасчетнаяУСНКАКМножитель
	|					КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН)
	|						ТОГДА &ПониженнаяРасчетнаяУСНКАКМножитель
	|					ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры)
	|					ТОГДА &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	...
	|				ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|			КОНЕЦ
	|	КОНЕЦ * ХозрасчетныйОбороты.СуммаОборотДт КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.Автоматически)
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	...
	|				ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|			КОНЕЦ
	|	КОНЕЦ * ХозрасчетныйОбороты.СуммаОборотДт КАК СуммаНДС
	...
	|		ПО ХозрасчетныйОбороты.Субконто2 = ПараметрыНДСПоДоговорам.ДоговорКонтрагента
	|ГДЕ
	...
	|						КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	|							ТОГДА ИСТИНА
	|						КОГДА ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС В (ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры), ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН), ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН))
	|							ТОГДА ИСТИНА
	|						ИНАЧЕ ЛОЖЬ
	|					КОНЕЦ
	|			ИНАЧЕ ВЫБОР
	|					КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС В (ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ИзКарточкиНоменклатуры), ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ОбщаяУСН), ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН))
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ
	|		КОНЕЦ
	...
	|			КОНЕЦ
	|	КОНЕЦ КАК СтавкаНДСПредположительная,
	|	ХозрасчетныйОстатки.СуммаОстатокДт - ВЫБОР
	|		КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.Автоматически)
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	|						ТОГДА &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	...
	|				ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|			КОНЕЦ
	|	КОНЕЦ * ХозрасчетныйОстатки.СуммаОстатокДт КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА СвойстваДоговора.СпособЗаполненияСтавкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыЗаполненияСтавкиНДС.Автоматически)
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ПараметрыНДСПоДоговорам.СпособЗаполненияСтавкиНДС, 0) = 0
	...
	|				ИНАЧЕ &ОбщаяРасчетнаяСтавкаНДСКАКМножитель
	|			КОНЕЦ
	|	КОНЕЦ * ХозрасчетныйОстатки.СуммаОстатокДт КАК СуммаНДС
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыНДСПоДоговорам КАК ПараметрыНДСПоДоговорам
	|		ПО ХозрасчетныйОстатки.Субконто2 = ПараметрыНДСПоДоговорам.ДоговорКонтрагента
	|ГДЕ
...
Процедура ПолучитьНаборДанныхВычетСАвансовПолученныхУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов, ТолькоВключенные, ТолькоПеренесенные)
	...
	Иначе
		...
		+ УчетНДСОтчетыПереопределяемый.ТекстЗапросаРеквизитыСчетовФактурПостановление981КнигаПокупок()
		+ "ВЫБРАТЬ
...
Процедура ПолучитьНаборДанныхНачислениеРеализация0(Запрос, СтруктураВнешнихНаборов) 
	...
	Запрос.Текст = УчетНДСПереопределяемый.ТекстЗапросаНаборДанныхНачислениеРеализация0(Запрос);
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям0(Запрос, СтруктураВнешнихНаборов) 
	...
	Запрос.Текст = УчетНДСПереопределяемый.ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностям0(Запрос);
...
Функция ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностямРасшифровкаСтрок125и130Декларации()
...
//   Структура   - структура параметров для формирования карты отчета
//
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностямРасшифровка125СтрокиДекларации(Запрос, СтруктураВнешнихНаборов) 
	
	Запрос.Текст = ТекстЗапросаНаборДанныхВычетПоПриобретеннымЦенностямРасшифровкаСтрок125и130Декларации();
	ВычетПоПриобретеннымЦенностямРасшифровка125СтрокиДекларации = Запрос.Выполнить();
	
	ВнешнийНаборДанныхВычетПриобретенныеЦенности = Новый Структура;
	ВнешнийНаборДанныхВычетПриобретенныеЦенности.Вставить("ВычетПоПриобретеннымЦенностямРасшифровка125СтрокиДекларации", 
		ВычетПоПриобретеннымЦенностямРасшифровка125СтрокиДекларации);
	
	СтруктураВнешнихНаборов.Вставить("ВнешнийНаборДанныхСтроки125Декларации", 
									 ВнешнийНаборДанныхВычетПриобретенныеЦенности);
	
КонецПроцедуры

// Выполняет запрос к базе данных для получения набора данных, формирующего расшифровку по восстановленному НДС
//
// Параметры
//  Запрос  - Запрос - запрос к базе данных
//  СтруктураВнешнихНаборов - структура внешних источников данных для расшифровки.
...
//   Структура   - структура параметров для формирования карты отчета
//
Процедура ПолучитьНаборДанныхВосстановлениеНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	ВосстановлениеНДС = Запрос.Выполнить();
	
	ВнешнийНаборДанныхВосстановлениеНДС = Новый Структура;
	ВнешнийНаборДанныхВосстановлениеНДС.Вставить("ВосстановлениеНДС", ВосстановлениеНДС);
	СтруктураВнешнихНаборов.Вставить("ВнешнийНаборДанныхВосстановлениеНДС", ВнешнийНаборДанныхВосстановлениеНДС);
	
КонецПроцедуры // ПолучитьНаборДанныхВосстановлениеНДС()
...
Процедура ПолучитьНаборДанныхПереченьЗаявленийОВвозеТоваров(Запрос, СтруктураВнешнихНаборов)
	...
	|	ЕСТЬNULL(СУММА(НДСЗаписиКнигиПродаж.СуммаБезНДС), 0) КАК Сумма,
	...
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка.Дата КАК ДатаВходящегоДокумента,
	|	НДСЗаписиКнигиПродаж.Покупатель КАК Контрагент,
	|	ВЫБОР
	|		КОГДА ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки ССЫЛКА Документ.РеализацияТоваровУслуг
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки.СтранаРеализации, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|						ТОГДА ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.ДокументОтгрузки.СтранаРеализации
	|					ИНАЧЕ НДСЗаписиКнигиПродаж.Покупатель.СтранаРегистрации
	|				КОНЕЦ
	|		ИНАЧЕ НДСЗаписиКнигиПродаж.Покупатель.СтранаРегистрации
	|	КОНЕЦ КАК СтранаНалоговогоОргана,
	|	НДСЗаписиКнигиПродаж.Покупатель.НалоговыйНомер КАК ИдентификационныйНомерПокупателя
	|ИЗ
	...
	|	НДСЗаписиКнигиПродаж.СтавкаНДС,
	|	НДСЗаписиКнигиПродаж.Событие,
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка,
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка.НомерВходящегоДокумента,
	|	ЗаявлениеОВвозеТоваровПолученноеДокументыОтгрузки.Ссылка.Дата,
