...
//  Структура - структура с результатом выполнения отчета.
//
Функция СформироватьОтчет(Знач ПараметрыОтчета) Экспорт
	...
	РезультатВыполнения.Вставить("ДанныеРасшифровки", ПараметрыОтчета.ДанныеРасшифровки);
	РезультатВыполнения.Вставить("КонтрольноеСоотношениеИтоговВыполняется", Истина);
...
Функция ПроверитьНаличиеЧековНПД(ПараметрыОтчета)
	
	ПериодПроверки = ИнтеграцияСтатусСамозанятогоБП.ПериодПроверкиНаличияЧековНПД();
	Если ПараметрыОтчета.НачалоПериода > ПериодПроверки.ДатаОкончания Тогда
		// Самозанятый должен предоставить чеки до 9-го числа следующего месяца, в текущем месяце не проверяем
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(СтатусыДокументов.СтатусЧекаНПД, Значение(Перечисление.СтатусыЧековНПД.ПустаяСсылка)) КАК СтатусЧекаНПД,
		|	ЕСТЬNULL(СтатусыСамозанятых.ПлательщикНПД, НЕОПРЕДЕЛЕНО) КАК ПлательщикНПД,
		|	ЕСТЬNULL(СтатусыСамозанятых.ДатаРегистрации, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаРегистрации
		|ИЗ
		|	Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыСамозанятых КАК СтатусыСамозанятых
		|		ПО ПоступлениеТоваровУслуг.Контрагент = СтатусыСамозанятых.Контрагент
		|		И НАЧАЛОПЕРИОДА(ВЫБОР
		|			КОГДА ПоступлениеТоваровУслуг.ДатаВходящегоДокумента > ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА ПоступлениеТоваровУслуг.ДатаВходящегоДокумента
		|			ИНАЧЕ ПоступлениеТоваровУслуг.Дата
		|		КОНЕЦ, ДЕНЬ) = СтатусыСамозанятых.Период
		...
		|	И (ЕСТЬNULL(СтатусыДокументов.СтатусЧекаНПД,
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыЧековНПД.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековНПД.НеПолучен)
		|	ИЛИ ЕСТЬNULL(СтатусыДокументов.СтатусЧекаНПД,
		|		ЗНАЧЕНИЕ(Перечисление.СтатусыЧековНПД.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековНПД.УтраченСтатусПлательщикаНПД))
		...
		|	ЕСТЬNULL(СтатусыДокументов.СтатусЧекаНПД, Значение(Перечисление.СтатусыЧековНПД.ПустаяСсылка)),
		|	ЕСТЬNULL(СтатусыСамозанятых.ПлательщикНПД, НЕОПРЕДЕЛЕНО),
		|	ЕСТЬNULL(СтатусыСамозанятых.ДатаРегистрации, ДАТАВРЕМЯ(1, 1, 1))
		|ИЗ
		|	Документ.ПоступлениеДопРасходов КАК ПоступлениеДопРасходов
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыСамозанятых КАК СтатусыСамозанятых
		|		ПО ПоступлениеДопРасходов.Контрагент = СтатусыСамозанятых.Контрагент
		|		И НАЧАЛОПЕРИОДА(ВЫБОР
		|			КОГДА ПоступлениеДопРасходов.ДатаВходящегоДокумента > ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА ПоступлениеДопРасходов.ДатаВходящегоДокумента
		|			ИНАЧЕ ПоступлениеДопРасходов.Дата
	...
	Если ПараметрыОтчета.КонецПериода > ПериодПроверки.ДатаОкончания Тогда
		КонецПериода = ПериодПроверки.ДатаОкончания;
	Иначе
	...
	Иначе
		...
		ДобавитьСтатусыСамозанятых(Результат);
	КонецЕсли;
...
Процедура ДобавитьСтатусыСамозанятых(Результат)
	
	Результат.Колонки.Добавить("СтатусСамозанятого", ОбщегоНазначения.ОписаниеТипаСтрока(254));
	Для Каждого СтрокаДанных Из Результат Цикл
		Если СтрокаДанных.ПлательщикНПД <> Неопределено Тогда
			СтрокаДанных.СтатусСамозанятого = ИнтеграцияСтатусСамозанятогоКлиентСервер.ОписаниеСтатуса(
				СтрокаДанных.ПлательщикНПД, СтрокаДанных.ДатаРегистрации);
		Иначе
			СтрокаДанных.СтатусСамозанятого
				= НСтр("ru = 'Проверка статуса плательщика налога на профессиональный доход не выполнялась'");
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьОписанияПроверок(ТаблицаПроверок, ПараметрыОтчета)
	...
	Для НомСтроки = НомПервойСтрокиПравил По НомПоследнейСтрокиПравил Цикл
		...
		ПроверкаАктуальна = ПроверкаАктуальна(ИмяОбластиСтроки, ПараметрыОтчета.КонецПериода, ПараметрыОтчета.Организация);
		Если Не ПроверкаАктуальна Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПроверкаАктуальна(ИмяПроверки, Период, Организация)
	
	Если ИмяПроверки = "ДиректКостингИСчет26" Тогда
		Возврат Период < БухгалтерскийУчетКлиентСервер.ДатаОбязательногоПримененияФСБУ5();
	КонецЕсли;
...
Процедура ВыполнитьВсеПроверки(ТаблицаПроверок, ПараметрыОтчета)
	...
	ПодготовленныеДанныеДляПроверок = ПодготовитьДанныеПроверок(ТаблицаПроверок, ПараметрыОтчета);
	
	Для каждого СтрокаПроверки Из ТаблицаПроверок Цикл
		Если ЕстьДоступ Тогда
			ВыполнитьПроверку(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок);
		Иначе
...
Функция ПодготовитьДанныеПроверок(ТаблицаПроверок, ПараметрыОтчета);
	
	ПодготовленныеДанныеДляПроверок = Новый Структура;
	
	Если ТаблицаПроверок.Найти("РазделЕдиныйНалоговыйСчет", "ИдентификаторРаздела") <> Неопределено Тогда
		// Для всех проверок уведомлений по ЕНС требуются одни и те же данные:
		//  - задачи по уведомлениям
		//  - созданные для задач уведомления вместе с суммами
		//  - рассчитанные суммы налогов по уведомлениям.
		// Поэтому, чтобы не рассчитывать эти данные несколько раз,
		// рассчитаем их один раз и добавим в подготовленные данные.
		// Проверки уже будут их использвать, и не будут обращаться к базе данных.
		ПодготовленныеДанныеДляПроверок.Вставить("ПодготовленныеДанныеУведомленияЕНС",
			ПодготовленныеДанныеУведомленийЕНС(ПараметрыОтчета));
	КонецЕсли;
	
	Возврат ПодготовленныеДанныеДляПроверок;
	
КонецФункции

Процедура ВыполнитьПроверку(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок)
	...
	// Раздел "Расчеты с контрагентами"
	Если СтрокаПроверки.Идентификатор = "НаличиеЧековНПД" Тогда
		Результат = ПроверитьНаличиеЧековНПД(ПараметрыОтчета);
	КонецЕсли;
		...
		Иначе
			...
			Если СтрокаПроверки.ОбнаруженыОшибки Тогда
				...
				Если ТипЗнч(СтрокаПроверки.ТаблицаОшибок) = Тип("ТаблицаЗначений")
					...
					ИначеЕсли СтрокаПроверки.Идентификатор = "МинусыПоКассе" 
						И ВестиУчетПоПодразделениям Тогда
		...
		Если НЕ ПерваяЗапись И Выборка.Период <> НачалоМесяца(НачалоГодаУП) Тогда
			...
			Для Каждого ЭлементУчетнойПолитики Из СинонимыРесурсов Цикл
				...
				Если ИзменилсяЭлементУчетнойПолитики Тогда
					// Проверим переход на АУСН, он может быть осуществлён в любой момент в течение года.
					Если Не ЭтоУчетнаяПолитикаБУ И НовоеЗначение = Перечисления.СистемыНалогообложения.АУСН Тогда
						Продолжить;
					КонецЕсли;
...
Функция ПолучитьСтруктуруИменНеизменныхРесурсовБУ()
	...
	ИменаРесурсовБУ.Вставить("ВариантУчетаОтложенногоНалога", "Год");
...
Функция ПроверитьДиректКостингИСчет26(СтрокаПроверки, ПараметрыОтчета)
	...
	СчетаРасходыПериода = Справочники.НастройкиУчетаЗатрат.СчетаРасходыПериода(
		ПараметрыОтчета.НачалоПериода,
		ПараметрыОтчета.Организация);
		
	Если Не ЗначениеЗаполнено(СчетаРасходыПериода) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	КалькуляционныеСчета = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(КалькуляционныеСчета, УчетЗатрат.ПредопределенныеКалькуляционныеСчета());
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(КалькуляционныеСчета, УчетЗатрат.ПредопределенныеСчетаОбслуживающиеПроизводства());
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачалоПериода",        ПараметрыОтчета.НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода",         КонецДня(ПараметрыОтчета.КонецПериода));
	Запрос.УстановитьПараметр("Организация",          ПараметрыОтчета.Организация);
	Запрос.УстановитьПараметр("СчетаРасходыПериода",  СчетаРасходыПериода);
	Запрос.УстановитьПараметр("КалькуляционныеСчета", УчетЗатрат.СчетаРасходов(КалькуляционныеСчета));
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, МЕСЯЦ, Счет В (&СчетаРасходыПериода), , Организация = &Организация, КорСчет В (&КалькуляционныеСчета), ) КАК ХозрасчетныйОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Для Каждого СтрокаТабРезультата Из Результат Цикл
		СтрокаТабРезультата.Расшифровка = Новый Структура();
		СтрокаТабРезультата.Расшифровка.Вставить("Идентификатор", СтрокаПроверки.Идентификатор);
...
Функция ПроверитьПрименениеПБУ18Счета(СтрокаПроверки, ПараметрыОтчета)
	...
	Если НЕ УчетнаяПолитика.Существует(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода) Тогда
		СтрокаПроверки.КомментарийРезультата = НСтр("ru = 'Проверка не может быть проведена, так как не задана учетная политика организации'");
		Возврат Истина;
	КонецЕсли;
	
	ВариантУчета = НалогНаПрибыльБухгалтерскийУчет.ВариантУчетаОтложенногоНалога(
		ПараметрыОтчета.НачалоПериода,
		ПараметрыОтчета.Организация);
		
	ЗапрещенныеСчета = Новый Массив;
	
	Если ВариантУчета = Перечисления.ВариантыУчетаОтложенногоНалога.Нет Тогда
		
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ОтложенныеНалоговыеАктивы);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ОтложенныеНалоговыеОбязательства);
		
	КонецЕсли;
	
	Если ВариантУчета <> Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы
		И ВариантУчета <> Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод Тогда
		
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.РасчетНалогаНаПрибыль);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.УсловныйРасходПоНалогуНаПрибыль);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.УсловныйДоходПоНалогуНаПрибыль);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПостоянноеНалоговоеОбязательство);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиОтПересчетаОНА_ОНО);
		
	КонецЕсли;
	
	Если ВариантУчета <> Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод Тогда
		
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ОтложенныйНалогНаПрибыль);
		ЗапрещенныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ТекущийНалогНаПрибыль);
		
	КонецЕсли;
	
	Возврат ПроверитьОтсутствиеОстатковИОборотов(СтрокаПроверки, ПараметрыОтчета, ЗапрещенныеСчета);
...
Функция ПроверитьНаличиеСложногоНДС(СтрокаПроверки, ПараметрыОтчета)
	...
	Если Не ПроверитьНеприменениеАУСН(СтрокаПроверки, ПараметрыОтчета) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверитьНастроеноИспользованиеСчетовПроизводства(СтрокаПроверки, ПараметрыОтчета)
	...
	СчетаРасходов = УчетЗатрат.ПредопределенныеКалькуляционныеСчета();
	НастройкиУчетаЗатрат = Справочники.НастройкиУчетаЗатрат.БазовыеНастройки(НачалоПериода, Организация);
	
	СчетаРасходовНеВНастройке
		= ОбщегоНазначенияКлиентСервер.РазностьМассивов(УчетЗатрат.СчетаРасходов(СчетаРасходов), НастройкиУчетаЗатрат.СчетаУчета);
	
	Запрос.УстановитьПараметр("НачалоПериода",             НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода",              КонецПериода);
	Запрос.УстановитьПараметр("Организация",               Организация);
	Запрос.УстановитьПараметр("СчетаРасходовНеВНастройке", СчетаРасходовНеВНастройке);
	...
	|	ХозрасчетныйОбороты.СуммаОборотКт КАК СуммаОборотКт
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, МЕСЯЦ, Счет В (&СчетаРасходовНеВНастройке), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПериодМесяц,
...
Функция ПроверитьНетОстатковПоСчетамПодлежащимЗакрытиюПоТипуЗакрытия(СтрокаПроверки, ПараметрыОтчета, ТипЗакрытия)
	...
	ТаблицаОшибок.Колонки.Добавить("СуммаОстаток", 		БухгалтерскийУчетКлиентСервер.ТипСумма());
...
Функция ПроверитьПереоценкаВалютыПустойДокументРасчетов(СтрокаПроверки, ПараметрыОтчета)
	...
	ТаблицаОшибок.Колонки.Добавить("СуммаВРублях", 		ОписаниеТиповЧисло);
	ТаблицаОшибок.Колонки.Добавить("СуммаВВалюте", 		ОписаниеТиповЧисло);
	ТаблицаОшибок.Колонки.Добавить("Ошибка", 			ОписаниеТиповСтрока);
	ТаблицаОшибок.Колонки.Добавить("Расшифровка");
...
Функция ПолучитьНедопустимыеПроводкиДляСчета(РеквизитыОрганизации, КодСчета, ДтКт, ТаблицаНедопустимыхПроводок)
...
Функция ПроверитьКорректныеКорреспондецииСчетов(СтрокаПроверки, ПараметрыОтчета)
	...
	ТаблицаОшибок.Колонки.Добавить("Сумма", 			ОписаниеТиповЧисло);
	ТаблицаОшибок.Колонки.Добавить("Ошибка", 			ОписаниеТиповСтрока);
	...
	ИсключаемыеСчета.Добавить(ПланыСчетов.Хозрасчетный.Вспомогательный); // 000
	
	// При изменении учетной политики, при исправлении ошибок с 84 счетом может корреспондировать любой балансовый счет,
	// поэтому проводки с 84 не проверяем.
	...
	|ГДЕ
	|	НЕ Хозрасчетный.СчетДт В (&ИсключаемыеСчета)
	|	И НЕ Хозрасчетный.СчетКт В (&ИсключаемыеСчета)
	...
	|	И Хозрасчетный.СчетДт <> Хозрасчетный.СчетКт
	|
	|СГРУППИРОВАТЬ ПО
	|	Хозрасчетный.Период,
	...
	Пока ВыборкаПоДт.Следующий() Цикл
		...
		Пока ВыборкаПоКт.Следующий() Цикл
			...
			МассивСтрокТаблицыНедопустимыхПроводок = ПолучитьНедопустимыеПроводкиДляСчета(РеквизитыОрганизации, ВыборкаПоКт.СчетДтКод, "Дт", ТаблицаНедопустимыхПроводок);
			Если МассивСтрокТаблицыНедопустимыхПроводок.Количество() > 0 Тогда
			...
			Иначе
				...
				МассивСтрокТаблицыНедопустимыхПроводок = ПолучитьНедопустимыеПроводкиДляСчета(РеквизитыОрганизации, ВыборкаПоКт.СчетКтКод, "Кт", ТаблицаНедопустимыхПроводок);
				Если МассивСтрокТаблицыНедопустимыхПроводок.Количество() > 0 Тогда
			...
			Если СтрокаТаблицыНедопустимыхПроводок <> Неопределено Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					ЭтоЗакрытиеГода = Месяц(Выборка.Период) = 12;
					
					Если Не СтрокаТаблицыНедопустимыхПроводок.ДопустимаТолькоПриЗакрытииГода
						ИЛИ Не ЭтоЗакрытиеГода Тогда
...
Функция ПроверитьОстаточнаяСтоимостьОС(СтрокаПроверки, ПараметрыОтчета)
	...
	ТаблицаОшибок.Колонки.Добавить("СуммаОстаток01_03", ОписаниеТиповЧисло);
	ТаблицаОшибок.Колонки.Добавить("СуммаОстаток02_010", ОписаниеТиповЧисло);
	ТаблицаОшибок.Колонки.Добавить("ОстаточнаяСтоимость", ОписаниеТиповЧисло);
...
Функция ПроверитьОкруглениеНачисленияНалогов(СтрокаПроверки, ПараметрыОтчета)
	...
	Если УчетнаяПолитика.ПрименяетсяНалогНаПрофессиональныйДоход(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода) Тогда
		Возврат	ТаблицаОшибок;
	КонецЕсли;
...
Функция ПроверитьПодотчетТолькоРаботникам(СтрокаПроверки, ПараметрыОтчета)
	...
	Если ПолучитьФункциональнуюОпцию("УчетЗарплатыИКадровВоВнешнейПрограмме")
		И Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчетПриВнешнейЗарплате") Тогда
		
		СтрокаПроверки.КомментарийРезультата = 
			НСтр("ru = 'При отключенном кадровом учете и учете зарплаты во внешней программе проверка не осуществляется'");
		Возврат Ложь;
	...
	// Не проверяем выдачи в подотчетных сумм самому ИП.
	...
	Если РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Запрос.УстановитьПараметр("ФизЛицоИП", РеквизитыОрганизации.ИндивидуальныйПредприниматель);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоФизЛицуИП", "СубконтоДт1 <> &ФизЛицоИП");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоФизЛицуИП", "ИСТИНА");
	КонецЕсли;
...
Функция ТекстЗапросаСчетаФактурыПоРеализации()
	...
	|ПОМЕСТИТЬ ВТ_СводныеСправкиПоОтчетуКомиссионера
	|ИЗ
	|	Документ.СчетФактураВыданный КАК СчетФактураВыданный
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураВыданныйДокументыОснования
	|		ПО СчетФактураВыданный.Ссылка = СчетФактураВыданныйДокументыОснования.Ссылка
	|ГДЕ
	|	СчетФактураВыданный.ВидСчетаФактуры = ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.СводнаяСправка)
	...
	|	ВЫБОР
	|		КОГДА НЕ ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура ЕСТЬ NULL
	|			ТОГДА ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура
	|		ИНАЧЕ ОтчетКомиссионераОПродажахПокупатели.СчетФактура
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура.Проведен, ЛОЖЬ)
	|		ИНАЧЕ ЕСТЬNULL(ОтчетКомиссионераОПродажахПокупатели.СчетФактура.Проведен, ЛОЖЬ)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура ЕСТЬ NULL
	|			ТОГДА ЕСТЬNULL(ВТ_СводныеСправкиПоОтчетуКомиссионера.СчетФактура.Дата, ДАТАВРЕМЯ(1, 1, 1))
	|		ИНАЧЕ ЕСТЬNULL(ОтчетКомиссионераОПродажахПокупатели.СчетФактура.Дата, ДАТАВРЕМЯ(1, 1, 1))
	|	КОНЕЦ,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СводныеСправкиПоОтчетуКомиссионера КАК ВТ_СводныеСправкиПоОтчетуКомиссионера
	|		ПО ОтчетКомиссионераОПродажахПокупатели.Ссылка = ВТ_СводныеСправкиПоОтчетуКомиссионера.ДокументОснование
	|ГДЕ
...
Функция ПроверитьСоответствие90_НДС_БУ(СтрокаПроверки, ПараметрыОтчета)
	...
	|	И НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор
	|;
	...
	|			ИНАЧЕ ВЫРАЗИТЬ(ВТ_РеализацияВВалюте.ВалютнаяСуммаКорОборотКт * КурсыВалют.Курс / КурсыВалют.Кратность КАК ЧИСЛО(31, 2))
	...
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.РеализацияТоваровУслуг).НДССМежценовойРазницы
	|							И НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.РеализацияТоваровУслуг).ДоговорКонтрагента.УчетАгентскогоНДСПокупателем
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.РеализацияОтгруженныхТоваров
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.РеализацияОтгруженныхТоваров).ДоговорКонтрагента.УчетАгентскогоНДСПокупателем
	...
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПоступлениеНаРасчетныйСчет).БезЗакрывающихДокументов
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ПриходныйКассовыйОрдер).БезЗакрывающихДокументов
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ОплатаПлатежнойКартой
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ОплатаПлатежнойКартой).БезЗакрывающихДокументов
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.РасходныйКассовыйОрдер
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.РасходныйКассовыйОрдер).БезЗакрывающихДокументов
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.СписаниеСРасчетногоСчета
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.СписаниеСРасчетногоСчета).БезЗакрывающихДокументов
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.РеализацияТоваровУслуг).БезОтраженияВУчете
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ВозвратТоваровОтПокупателя).БезОтраженияВУчете
	|				КОГДА ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|					ТОГДА НЕ ВЫРАЗИТЬ(ХозрасчетныйОбороты.Регистратор КАК Документ.ОтчетКомиссионераОПродажах).БезОтраженияВУчете
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ
	...
	Результат = Запрос.Выполнить();
	ТаблицаРезультата = Результат.Выгрузить();
	ТаблицаРезультата.Колонки.Добавить("СуммаБезНДС", БухгалтерскийУчетКлиентСервер.ТипСумма());
	ТаблицаРезультата.Колонки.Добавить("НДСРассчитанный", БухгалтерскийУчетКлиентСервер.ТипСумма());
	ТаблицаРезультата.Колонки.Добавить("ОбнаруженыОшибки", Новый ОписаниеТипов("Булево"));
	
	РасчетнаяСуммаРаздОкр = ДопустимокОтклонениеРассчитаннойСуммы();
	Для Каждого СтрокаТабРезультата Из ТаблицаРезультата Цикл
		...
		СтрокаТабРезультата.ОбнаруженыОшибки = Не СтрокаТабРезультата.ИсключитьДокументИзПроверкиСоответствия
			И Не (Окр(СтрокаТабРезультата.НДСРассчитанный - СтрокаТабРезультата.НДСНачисленный) < РасчетнаяСуммаРаздОкр
				И Окр(СтрокаТабРезультата.НДСРассчитанный - СтрокаТабРезультата.НДСНачисленный) > -РасчетнаяСуммаРаздОкр);
...
Функция ПроверитьСоответствие91_НДС_БУ(СтрокаПроверки, ПараметрыОтчета)
	...
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода);
	Если ПлательщикНДСПрименяющийУСН Тогда
		ВидСтавкиПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(
			ПараметрыОтчета.Организация, КонецДня(ПараметрыОтчета.КонецПериода));
		СтавкаНДСОрганизации = Перечисления.СтавкиНДС.СтавкаНДС(ВидСтавкиПриУСН, КонецДня(ПараметрыОтчета.КонецПериода)); 
	Иначе
		СтавкаНДСОрганизации = УчетНДСКлиентСервер.ОбщаяСтавкаНДС(КонецДня(ПараметрыОтчета.КонецПериода));
	КонецЕсли;

	Запрос.УстановитьПараметр("РезультатПоСуммовымРазницам", РезультатПоСуммовымРазницам);
	Запрос.УстановитьПараметр("СтавкаНДСПредположительная", СтавкаНДСОрганизации);
	...
	|						ИЛИ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	...
	|	И ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	|				ТОГДА НЕ ВЫРАЗИТЬ(НДСЗаписиКнигиПродаж.Регистратор КАК Документ.РеализацияТоваровУслуг).НДССМежценовойРазницы
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА &СтавкаНДСПредположительная
	...
	|	И ЕСТЬNULL(ВТ_НДСЗаписиКнигиПродаж.СтавкаНДС, &СтавкаНДСПредположительная) <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
	|	И НЕ ВТ_Обороты91.Регистратор ССЫЛКА Документ.ПодтверждениеНулевойСтавкиНДС
	|	И ВТ_НДСРеализация0.Регистратор ЕСТЬ NULL
	|	И (ВТ_НДСЗаписиКнигиПродаж.Регистратор ЕСТЬ NULL
	|			ИЛИ ВТ_НДСЗаписиКнигиПродаж.КоличествоРазныхСтавок = 1)
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для Каждого СтрокаТабРезультата Из ТаблицаРезультата Цикл
		...
		Если БазаДляНДС = 0 
			И ТипЗнч(СтрокаТабРезультата.Документ) = Тип("ДокументСсылка.ОтражениеНачисленияНДС") Тогда
			// При произвольном отражении НДС есть только проводка по начислению самого НДС,
			// по счету 91.01 может не быть отражений, поэтому пропускаем.
			Продолжить;
		КонецЕсли;
...
Функция ПроверитьСоответствие90и91_НДС_НУ(СтрокаПроверки, ПараметрыОтчета) Экспорт
	...
	Запрос.УстановитьПараметр("РасчетнаяСуммаРаздОкр",    ДопустимокОтклонениеРассчитаннойСуммы() - 0.5); // с учетом округления 0.5 до 1
	
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода);
	Запрос.УстановитьПараметр("ПлательщикНДСПрименяющийУСН", ПлательщикНДСПрименяющийУСН);
	Если ПлательщикНДСПрименяющийУСН Тогда
		ВидСтавкиПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(
			ПараметрыОтчета.Организация, КонецДня(ПараметрыОтчета.КонецПериода));
		СтавкаНДСОрганизации = Перечисления.СтавкиНДС.СтавкаНДС(ВидСтавкиПриУСН, КонецДня(ПараметрыОтчета.КонецПериода)); 
	...
	|	ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.КорректировкаРеализации).ИсходныйДокументРеализации КАК ИсходныйДокументРеализации
	|ПОМЕСТИТЬ ВТ_КорректировкиОтгрузкиБезПереходаПраваСобственности
	|ИЗ
	...
	|ГДЕ
	|	НДСПродажи.Регистратор ССЫЛКА Документ.КорректировкаРеализации
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НДСПредъявленный.Регистратор,
	|	ВЫРАЗИТЬ(НДСПредъявленный.Регистратор КАК Документ.КорректировкаРеализации).ИсходныйДокументРеализации
	|ИЗ
	...
	|			Регистратор,
	|			Организация = &Организация
	|				И НЕ ВидЦенности В (&ИсключаемыеВидыЦенностей)) КАК НДСПредъявленный
	|ГДЕ
	|	НДСПредъявленный.Регистратор ССЫЛКА Документ.КорректировкаРеализации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкиРеализации.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ ВТ_ДокументыКорректировкиРеализации
	|ИЗ
	|	ВТ_КорректировкиОтгрузкиБезПереходаПраваСобственности КАК КорректировкиРеализации
	|ГДЕ
	|	КорректировкиРеализации.ИсходныйДокументРеализации ССЫЛКА Документ.РеализацияТоваровУслуг
	|	И ВЫРАЗИТЬ(КорректировкиРеализации.ИсходныйДокументРеализации КАК Документ.РеализацияТоваровУслуг).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеНДС.СчетФактура КАК СчетФактура,
	|	ДанныеНДС.Регистратор КАК Регистратор,
	|	ДанныеНДС.Контрагент КАК Контрагент,
	|	ДанныеНДС.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ДанныеНДС.БазаНДСпоРегистру) КАК БазаНДСпоРегистру,
	|	СУММА(ДанныеНДС.СуммаНДСпоРегистру) КАК СуммаНДСпоРегистру
	...
	|		И НЕ НДСПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|		И НЕ НДСПродажи.Регистратор ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|		И ВЫБОР
	|				КОГДА НДСПродажи.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	|					ТОГДА НЕ ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.РеализацияТоваровУслуг).НДССМежценовойРазницы
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ
	|		И ВЫБОР
	|				КОГДА НДСПродажи.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|					ТОГДА ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.ОтчетОРозничныхПродажах).ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОтчетОРозничныхПродажах.ОтчетНТТОПродажах)
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ
	|		И ВЫБОР
	|				КОГДА НДСПродажи.Регистратор ССЫЛКА Документ.РеализацияТоваровУслуг
	|					ТОГДА НЕ ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.РеализацияТоваровУслуг).БезОтраженияВУчете
	|				КОГДА НДСПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|					ТОГДА НЕ ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.ВозвратТоваровОтПокупателя).БезОтраженияВУчете
	|				КОГДА НДСПродажи.Регистратор ССЫЛКА Документ.ОтчетКомиссионераОПродажах
	|					ТОГДА НЕ ВЫРАЗИТЬ(НДСПродажи.Регистратор КАК Документ.ОтчетКомиссионераОПродажах).БезОтраженияВУчете
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|				Регистратор,
	|				Организация = &Организация
	|					И НЕ ВидЦенности В (&ИсключаемыеВидыЦенностей)) КАК НДСПродажи
	|	ГДЕ
	|		НДСПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		НДСПродажи.Регистратор ССЫЛКА Документ.КорректировкаРеализации
	|		И НДСПродажи.Регистратор В
	|				(ВЫБРАТЬ
	|					ВТ_ДокументыКорректировкиРеализации.Регистратор
	|				ИЗ
	|					ВТ_ДокументыКорректировкиРеализации)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		НДСПредъявленный.Регистратор,
	|		ВЫБОР
	|			КОГДА НДСПредъявленный.Поставщик = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ НДСПредъявленный.Поставщик
	|		КОНЕЦ,
	|		НДСПредъявленный.СтавкаНДС,
	|		-НДСПредъявленный.СуммаБезНДСПриход,
	|		-НДСПредъявленный.НДСПриход
	|	ИЗ
	|		РегистрНакопления.НДСПредъявленный.Обороты(
	|				&НачалоПериода,
	|				&КонецПериода,
	...
	|		НДСПокупки.Регистратор,
	|		ВЫБОР
	|			КОГДА НДСПокупки.Поставщик = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|			ИНАЧЕ НДСПокупки.Поставщик
	|		КОНЕЦ,
	|		НДСПокупки.СтавкаНДС,
	|		-НДСПокупки.СуммаБезНДСОборот,
	|		-НДСПокупки.НДСОборот
	|	ИЗ
	...
	|				Регистратор,
	|				Организация = &Организация
	|					И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Возврат)) КАК НДСПокупки
	|	ГДЕ
	|		(НДСПокупки.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|				ИЛИ ВЫРАЗИТЬ(НДСПокупки.Регистратор КАК Документ.ВозвратТоваровОтПокупателя).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийВозвратТоваровОтПокупателя.ОтгруженныеТовары))) КАК ДанныеНДС
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеНДС.СчетФактура,
	|	ДанныеНДС.Регистратор,
	|	ДанныеНДС.Контрагент,
	|	ДанныеНДС.СтавкаНДС
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	КОНЕЦ КАК Документ,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВТ_НДСПродажи.СтавкаНДС) КАК КоличествоРазныхСтавок
	|ПОМЕСТИТЬ ВТ_РазличныеСтавкиНДСПоДокументу
	|ИЗ
	...
	|			ИЛИ &ПлательщикНДСПрименяющийУСН)
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Регистратор,
	|	ХозрасчетныйОбороты.Субконто1
	...
	|		ИНАЧЕ &СтавкаНДСПредположительная
	|	КОНЕЦ,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВТ_Данные.БазаНДСпоБУ + ВТ_Данные.БазаНДСпоОтгрузкеБезПереходаСобственности - ВТ_Данные.БазаНДСпоРегистру) < &РасчетнаяСуммаРаздОкр И
	|	СУММА(ВТ_Данные.БазаНДСпоБУ + ВТ_Данные.БазаНДСпоОтгрузкеБезПереходаСобственности - ВТ_Данные.БазаНДСпоРегистру) > -&РасчетнаяСуммаРаздОкр И
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РазличныеСтавкиНДСПоДокументу КАК ВТ_РазличныеСтавкиНДСПоДокументу
	|		ПО ВТ_Данные.Документ = ВТ_РазличныеСтавкиНДСПоДокументу.Документ
	|ГДЕ
	|	ВТ_ПоДокументам.Документ ЕСТЬ NULL
	|	И (ВТ_РазличныеСтавкиНДСПоДокументу.Документ ЕСТЬ NULL
	|			ИЛИ ВТ_РазличныеСтавкиНДСПоДокументу.КоличествоРазныхСтавок = 1)
	...
	|						ИЛИ ВТ_Данные.БазаНДСпоБУ + ВТ_Данные.БазаНДСпоОтгрузкеБезПереходаСобственности - ВТ_Данные.БазаНДСпоРегистру < &РасчетнаяСуммаРаздОкр
	|							И ВТ_Данные.БазаНДСпоБУ + ВТ_Данные.БазаНДСпоОтгрузкеБезПереходаСобственности - ВТ_Данные.БазаНДСпоРегистру > -&РасчетнаяСуммаРаздОкр))
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТ_Данные.Документ,
	|	ВТ_Данные.СтавкаНДС
...
Функция ПроверитьНачислениеНДСсАвансов(СтрокаПроверки, ПараметрыОтчета) Экспорт
	...
	ПараметрыРегистрацииСФНаАванс = УчетНДСБП.ПараметрыЗаполненияРегистрацииСчетовФактур();
	...
	|		ПО ВТ_ТаблицаАвансов.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
...
Функция ПроверитьПолнотаНДСАгенты(СтрокаПроверки, ПараметрыОтчета)
	...
	|				И ЕСТЬNULL(НЕ ВЫРАЗИТЬ(СубконтоДт2 КАК Справочник.ДоговорыКонтрагентов).ВидАгентскогоДоговора В (&ВидыДоговоровРеализацииТоваров), ЛОЖЬ)) КАК ХозрасчетныйОборотыДтКт
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПроверитьНДСсСМРхозспособомВКнигеПродаж(СтрокаПроверки, ПараметрыОтчета)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Сводный.Период КАК Период,
	|	СУММА(Сводный.СуммаСМР) КАК СуммаСМР,
	|	СУММА(Сводный.БазаНДСпоРегистру) КАК БазаНДСпоРегистру,
	|	СУММА(Сводный.СуммаНДСпоРегистру) КАК СуммаНДСпоРегистру
	...
	|				Организация = &Организация
	|					И Субконто1 = ЗНАЧЕНИЕ(Перечисление.СпособыСтроительства.Хозспособ)
	|					И КорСубконто1 = ЗНАЧЕНИЕ(Перечисление.СпособыСтроительства.Хозспособ),
	|				КорСчет В (&Счета08_03),
	|				ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СпособыСтроительства)) КАК ХозрасчетныйОбороты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				КВАРТАЛ,
	|				Организация = &Организация
	|					И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.СМРСобственнымиСилами)) КАК НДСПродажи) КАК Сводный
	|
	|СГРУППИРОВАТЬ ПО
	|	Сводный.Период
	|;
	...
	|	ВТ_Результат.СуммаСМР КАК СуммаСМР,
	|	ВТ_Результат.БазаНДСпоРегистру КАК БазаНДСпоРегистру,
	|	ВТ_Результат.СуммаНДСпоРегистру КАК СуммаНДСпоРегистру
	|ИЗ
...
// Отклонение рассчитанной суммы налога на меньшее количество рублей не считается ошибкой учета.
// По аналогии с проверкой контрольного соотношения в процедуре ПроверкаКС_1_28() из модуля формы отчета РегламентированныйОтчетНДС.
//
Функция ДопустимокОтклонениеРассчитаннойСуммы()
...
Функция ПроверитьПолнотаСФПолученных(СтрокаПроверки, ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("ТекстНетСчетаФактуры", НСтр("ru='Нет счета-фактуры'"));
	Запрос.УстановитьПараметр("ТекстСчетФактураНеПроведен", НСтр("ru='Счет-фактура не проведен'"));
	Запрос.УстановитьПараметр("ТекстСчетФактураНеЗаполненыРеквизиты", НСтр("ru='В счете-фактуре не заполнен входящий номер или дата'")); 
	...
	|	ТаблицаДокументов.СчетФактураПроведен КАК СчетФактураПроведен,
	...
	|			ТОГДА &ТекстНетСчетаФактуры
	|		КОГДА ВТ_ТаблицаДокументов.СчетФактураПроведен = ЛОЖЬ
	|			ТОГДА &ТекстСчетФактураНеПроведен
	|		ИНАЧЕ &ТекстСчетФактураНеЗаполненыРеквизиты
	|	КОНЕЦ КАК Ошибка
	|ИЗ
...
Функция ПроверитьВсеКосвРасходыРаспределены(СтрокаПроверки, ПараметрыОтчета)
	...
	Если НЕ ПроверитьНеприменениеУСН(СтрокаПроверки, ПараметрыОтчета) Тогда
		...
		// Проверка не проводится, но исправим предмет контроля,
		// чтобы не показывать пользователю служебные символы 
		...
		ИсправитьПредметКонтроляВсеКосвРасходыРаспределены(СтрокаПроверки, ТекстПредметКонтроля, ТекстРегистрКонтроля, ТекстРаспределения, ТекстКомментарий);
	...
	Если ПрименяетсяРаздельныйУчетНДС Тогда
		...
		ТаблицаОшибок.Колонки.Добавить("НераспределеннаяСумма", БухгалтерскийУчетКлиентСервер.ТипСумма());
...
Функция ПроверитьНетОтрицательногоНДСПредъявленного(СтрокаПроверки, ПараметрыОтчета) Экспорт
	...
	ТаблицаОшибок.Колонки.Добавить("СуммаОстатка", БухгалтерскийУчетКлиентСервер.ТипСумма());
...
Функция ПроверитьОстаткиНДСПредъявленного_БУ_НУ(СтрокаПроверки, ПараметрыОтчета) Экспорт
	...
	ТаблицаОшибок.Колонки.Добавить("СуммаНДСпоСчету19", БухгалтерскийУчетКлиентСервер.ТипСумма());
	ТаблицаОшибок.Колонки.Добавить("СуммаНДСпоРегистрам", БухгалтерскийУчетКлиентСервер.ТипСумма());
	
	Если УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода)
		И НЕ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода) Тогда
		Возврат ТаблицаОшибок;
	КонецЕсли;
...
Функция ПроверитьВычетНДСсАвансов_БУ_НУ(СтрокаПроверки, ПараметрыОтчета)
	...
	|		0 КАК СуммаПоРегистру
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСпоАвансамИПредоплатам), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты
	|	ГДЕ
	|		НЕ ХозрасчетныйОбороты.Регистратор ССЫЛКА Документ.СписаниеНДС
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
...
Процедура ИсправитьПредметКонтроляВсеКосвРасходыРаспределены(СтрокаПроверки, ТекстПредметКонтроля, ТекстРегистрКонтроля, ТекстРаспределения, ТекстКомментарий)
...
#Область ПроверкиРазделаУведомленияЕНС

Функция ПодготовленныеДанныеУведомленийЕНС(ПараметрыОтчета)
	
	// Выбираем все задачи, срок выполнения которых наступает в выбранном периоде.
	// По каждой такой задаче ищем свяазнные с ней уведомления и заполняем таблицу.
	...
	|	ЗадачиБухгалтера.Наименование КАК НаименованиеЗадачиВСписке,
	...
	|ПОМЕСТИТЬ ЗадачиПоУведомлениям
	|ИЗ
	...
	|	И ЗадачиБухгалтера.Срок МЕЖДУ &НачалоПериода И &КонецПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗадачиПоУведомлениям.Организация КАК Организация,
	|	ЗадачиПоУведомлениям.Правило КАК Правило,
	|	ЗадачиПоУведомлениям.ПериодСобытия КАК ПериодСобытия,
	|	ЗадачиПоУведомлениям.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ЗадачиПоУведомлениям.Действие КАК Действие,
	|	ЗадачиПоУведомлениям.Статус КАК Статус,
	|	ЗадачиПоУведомлениям.Срок КАК Срок,
	|	ВидыНалоговИПлатежейВБюджет.Ссылка КАК Налог
	|ПОМЕСТИТЬ ЗадачиПоУведомлениямСНалогами
	|ИЗ
	|	ЗадачиПоУведомлениям КАК ЗадачиПоУведомлениям
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаПредставленияОтчетовУплатыНалогов КАК ПравилаПредставленияОтчетовУплатыНалогов
	|		ПО ЗадачиПоУведомлениям.Правило = ПравилаПредставленияОтчетовУплатыНалогов.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ЗадачиБухгалтера КАК ЗадачиБухгалтераСправочник
	|		ПО (ЗадачиБухгалтераСправочник.Ссылка = ПравилаПредставленияОтчетовУплатыНалогов.Владелец)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВидыКодыНалогов КАК ВидыКодыНалогов
	|		ПО (ВидыКодыНалогов.КодНалога = ЗадачиБухгалтераСправочник.Код)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
	|		ПО (ВидыКодыНалогов.ВидНалога = ВидыНалоговИПлатежейВБюджет.ВидНалога)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗадачиПоУведомлениям.Организация КАК Организация,
	|	ЗадачиПоУведомлениям.Правило.Владелец КАК Задача,
	|	ЗадачиПоУведомлениям.Правило.Владелец.Код КАК ИдентификаторЗадачи,
	|	ЗадачиПоУведомлениям.Правило.Владелец.Наименование КАК НаименованиеЗадачи,
	|	ЗадачиПоУведомлениям.Правило КАК Правило,
	|	ЗадачиПоУведомлениям.Правило.Код КАК ИдентификаторПравила,
	|	ЗадачиПоУведомлениям.Правило.ЕстьИнформацияНаИТС КАК ЕстьИнформацияНаИТС,
	|	ЗадачиПоУведомлениям.ПериодСобытия КАК ПериодСобытия,
	|	ЗадачиПоУведомлениям.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ЗадачиПоУведомлениям.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОрганеУведомления,
	|	ЗадачиПоУведомлениям.Действие КАК Действие,
	|	ЗадачиПоУведомлениям.Статус КАК Статус,
	|	ЗадачиПоУведомлениям.Срок КАК Срок,
	|	ЗадачиПоУведомлениям.НаименованиеЗадачиВСписке КАК НаименованиеЗадачиВСписке,
	|	ЗадачиПоУведомлениям.НачалоВыполнения КАК НачалоВыполнения,
	|	ЗадачиПоУведомлениям.Периодичность КАК Периодичность,
	|	ЗадачиПоУведомлениям.РегистрацияВНалоговомОргане <> ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка) КАК ВыполняетсяПоРегистрации
	|ИЗ
	|	ЗадачиПоУведомлениям КАК ЗадачиПоУведомлениям
	|;
	|
	...
	|	УведомлениеОбИсчисленныхСуммахНалогов.Дата КАК ДатаУведомления,
	|	ЗадачиПоУведомлениямСНалогами.Организация КАК Организация,
	|	ЗадачиПоУведомлениямСНалогами.Правило КАК Правило,
	|	ЗадачиПоУведомлениямСНалогами.ПериодСобытия КАК ПериодСобытия,
	|	ЗадачиПоУведомлениямСНалогами.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ЗадачиПоУведомлениямСНалогами.Действие КАК Действие,
	...
	|	УведомлениеОбИсчисленныхСуммахНалогов.Уведомление КАК УведомлениеБРО,
	...
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОрганеУведомления
	|ИЗ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗадачиПоУведомлениямСНалогами КАК ЗадачиПоУведомлениямСНалогами
	|		ПО (ЗадачиПоУведомлениямСНалогами.Организация = УведомлениеОбИсчисленныхСуммахНалогов.Организация)
	|			И (ЗадачиПоУведомлениямСНалогами.Налог = УведомлениеОбИсчисленныхСуммахНалоговНалоги.Налог)
	|			И (КОНЕЦПЕРИОДА(ЗадачиПоУведомлениямСНалогами.ПериодСобытия, ДЕНЬ) = КОНЕЦПЕРИОДА(УведомлениеОбИсчисленныхСуммахНалоговНалоги.КонецПериода, МЕСЯЦ))
	|			И (ЗадачиПоУведомлениямСНалогами.РегистрацияВНалоговомОргане = УведомлениеОбИсчисленныхСуммахНалоговНалоги.РегистрацияВНалоговомОргане)
	|			И (ЗадачиПоУведомлениямСНалогами.РегистрацияВНалоговомОргане <> ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка))
	|ГДЕ
	...
	|	ЗадачиПоУведомлениямСНалогами.Организация,
	|	ЗадачиПоУведомлениямСНалогами.Правило,
	|	ЗадачиПоУведомлениямСНалогами.ПериодСобытия,
	|	ЗадачиПоУведомлениямСНалогами.РегистрацияВНалоговомОргане,
	|	ЗадачиПоУведомлениямСНалогами.Действие,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.Налог,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.Сумма,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.НомерСтроки,
	|	УведомлениеОбИсчисленныхСуммахНалогов.Уведомление,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.КодБК,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.ВидПлатежаВГосБюджет,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.УровеньБюджета,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.КодПоОКТМО,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.КонецПериода,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.СрокУплаты,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.НачалоПериода,
	|	УведомлениеОбИсчисленныхСуммахНалоговНалоги.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОрганеУведомления
	|ИЗ
	|	Документ.УведомлениеОбИсчисленныхСуммахНалогов КАК УведомлениеОбИсчисленныхСуммахНалогов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.УведомлениеОбИсчисленныхСуммахНалогов.Налоги КАК УведомлениеОбИсчисленныхСуммахНалоговНалоги
	...
	|ГДЕ
	|	УведомлениеОбИсчисленныхСуммахНалогов.Организация = &Организация
	|	И УведомлениеОбИсчисленныхСуммахНалогов.Проведен";
	
	ТаблицаДанныхУведомленийЕНС = НовыйТаблицаУведомленийЕНС();
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	ЗадачиПоУведомлениям = РезультатЗапроса[РезультатЗапроса.Количество() - 2].Выгрузить();
	ВсеСозданныеУведомления = РезультатЗапроса[РезультатЗапроса.Количество() - 1].Выгрузить();
	
	ЗаполнитьСтатусУведомленийЕНС(ВсеСозданныеУведомления);
	
	ЗадачиПоУведомлениям.Колонки.Добавить("ИдентификаторЗапроса", Новый ОписаниеТипов("УникальныйИдентификатор"));
	
	ТаблицаЗадачДляРасчетов = Обработки.ПомощникУплатыНалога.НовыйТаблицаЗадачДляСведенийОРасчетах();
	Для Каждого ЗадачаПоУведомлению Из ЗадачиПоУведомлениям Цикл
		ЗадачаПоУведомлению.ИдентификаторЗапроса = Новый УникальныйИдентификатор();
		СтрокаТаблицыЗадач = ТаблицаЗадачДляРасчетов.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыЗадач, ЗадачаПоУведомлению);
	КонецЦикла;
	
	РасчетыНалогов = Обработки.ПомощникУплатыНалога.ТаблицаРасчетовПоСпискуЗадач(
		Перечисления.ВидыДействийКалендаряБухгалтера.Уведомление, ЗадачиПоУведомлениям);
	
	Для Каждого ЗадачаПоУведомлению Из ЗадачиПоУведомлениям Цикл
		
		СтрокаУведомления = ТаблицаДанныхУведомленийЕНС.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаУведомления, ЗадачаПоУведомлению);
		
		РассчитанныеСуммыУведомлений = РассчитанныеСуммыУведомлений(ЗадачаПоУведомлению, РасчетыНалогов);
		
		СтрокаУведомления.НалогРассчитан = РассчитанныеСуммыУведомлений.НалогРассчитан;
		СтрокаУведомления.РасчитаннаяСуммаНалога = РассчитанныеСуммыУведомлений.СуммаУведомления;
		
		СтрокаУведомления.ПараметрыОткрытияЗадачи = ПараметрыОткрытияЗадачи(ЗадачаПоУведомлению);
		
		СтрокаУведомления.СозданныеУведомления = НовыйСозданныеУведомления();
		ЗаполнитьСозданныеУведомления(ЗадачаПоУведомлению, СтрокаУведомления.СозданныеУведомления, ВсеСозданныеУведомления);
		
	КонецЦикла;
	
	Возврат ТаблицаДанныхУведомленийЕНС;
	
КонецФункции

Процедура ЗаполнитьСтатусУведомленийЕНС(ВсеСозданныеУведомления)
	
	УникальныеУведомления = Новый Соответствие;
	Для Каждого СозданноеУведомление Из ВсеСозданныеУведомления Цикл
		УникальныеУведомления.Вставить(СозданноеУведомление.УведомлениеБРО, Истина);
	КонецЦикла;
	
	СтатусыУведомлений = Новый Соответствие;
	Для Каждого КлючЗначение Из УникальныеУведомления Цикл
		
		Если Не ЗначениеЗаполнено(КлючЗначение.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		
		СтатусБРО = ИнтерфейсыВзаимодействияБРО.СостояниеДокумента(КлючЗначение.Ключ);
		Если Не ЗначениеЗаполнено(СтатусБРО) Тогда
			Продолжить;
		КонецЕсли;
		
		СтатусыУведомлений.Вставить(КлючЗначение.Ключ, СтатусБРО.Представление);
		
	КонецЦикла;
	
	ВсеСозданныеУведомления.Колонки.Добавить("СтатусОтправки", Метаданные.РегистрыСведений.ЖурналОтправокВКонтролирующиеОрганы.Ресурсы.СтатусОтправки.Тип);
	
	Для Каждого СозданноеУведомление Из ВсеСозданныеУведомления Цикл
		СозданноеУведомление.СтатусОтправки = СтатусыУведомлений.Получить(СозданноеУведомление.УведомлениеБРО);
	КонецЦикла;
	
КонецПроцедуры

Функция РассчитанныеСуммыУведомлений(ЗадачаПоУведомлению, РасчетыНалогов)
	
	РассчитанныеСуммыУведомлений = Новый Структура;
	РассчитанныеСуммыУведомлений.Вставить("НалогРассчитан", Ложь);
	РассчитанныеСуммыУведомлений.Вставить("СуммаУведомления", 0);
	
	РасчетНалога = ОпределитьСведенияОРасчетеНалога(РасчетыНалогов, ЗадачаПоУведомлению.ИдентификаторЗапроса);
	Если Не ЗначениеЗаполнено(РасчетНалога) Тогда
		Возврат РассчитанныеСуммыУведомлений;
	КонецЕсли;
	
	РассчитанныеСуммыУведомлений.НалогРассчитан = РасчетНалога.РасчетСуммыВыполнен;	
	
	Если Не ЗначениеЗаполнено(РасчетНалога.ТаблицаРасчета) Тогда
		Возврат РассчитанныеСуммыУведомлений;
	КонецЕсли;
	
	Для Каждого СтрокаРасчета Из РасчетНалога.ТаблицаРасчета Цикл
		Если СтрокаРасчета.ЭтоИтоговаяСтрока Тогда
			Продолжить;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаРасчета.КонецПериода)
			И КонецМесяца(СтрокаРасчета.КонецПериода) <> КонецМесяца(ЗадачаПоУведомлению.ПериодСобытия) Тогда
			Продолжить;
		КонецЕсли;
		РассчитанныеСуммыУведомлений.СуммаУведомления = РассчитанныеСуммыУведомлений.СуммаУведомления +
			СтрокаРасчета.СуммаУведомления;
	КонецЦикла;
...
Функция ОпределитьСведенияОРасчетеНалога(РасчетыНалогов, ИдентификаторЗапроса)
	
	Для Каждого РасчетНалогов Из РасчетыНалогов Цикл
		Если РасчетНалогов.СведенияОРасчете.ИдентификаторЗапроса = ИдентификаторЗапроса Тогда
			Возврат РасчетНалогов.СведенияОРасчете;
		КонецЕсли;
	КонецЦикла;
...
Функция КлючевыеПоляУведомлений()
	
	КлючевыеПоляУведомлений = Документы.УведомлениеОбИсчисленныхСуммахНалогов.КлючевыеПоляУведомлений();
	// Для случаев нескольких уведомлений по разным ИФНС 
	КлючевыеПоляУведомлений = КлючевыеПоляУведомлений + ",РегистрацияВНалоговомОрганеУведомления";

	Возврат КлючевыеПоляУведомлений;
	
КонецФункции

Функция НовыйТаблицаУведомленийЕНС()
	
	МетаданныеРегистраЗадачиБухгалтера = Метаданные.РегистрыСведений.ЗадачиБухгалтера;
	
	ДанныеУведомлений = Новый ТаблицаЗначений;
	ДанныеУведомлений.Колонки.Добавить("Организация", МетаданныеРегистраЗадачиБухгалтера.Измерения.Организация.Тип);
	ДанныеУведомлений.Колонки.Добавить("Правило", МетаданныеРегистраЗадачиБухгалтера.Измерения.Правило.Тип);
	ДанныеУведомлений.Колонки.Добавить("ПериодСобытия", МетаданныеРегистраЗадачиБухгалтера.Измерения.ПериодСобытия.Тип);
	ДанныеУведомлений.Колонки.Добавить("РегистрацияВНалоговомОргане", МетаданныеРегистраЗадачиБухгалтера.Измерения.РегистрацияВНалоговомОргане.Тип);
	ДанныеУведомлений.Колонки.Добавить("Действие", МетаданныеРегистраЗадачиБухгалтера.Измерения.Действие.Тип);
	ДанныеУведомлений.Колонки.Добавить("Статус", МетаданныеРегистраЗадачиБухгалтера.Ресурсы.Статус.Тип);
	ДанныеУведомлений.Колонки.Добавить("Срок", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Срок.Тип);
	ДанныеУведомлений.Колонки.Добавить("НаименованиеЗадачиВСписке", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Наименование.Тип);
	ДанныеУведомлений.Колонки.Добавить("ПараметрыОткрытияЗадачи", Новый ОписаниеТипов("Структура"));
	ДанныеУведомлений.Колонки.Добавить("СозданныеУведомления", Новый ОписаниеТипов("ТаблицаЗначений"));
	ДанныеУведомлений.Колонки.Добавить("НалогРассчитан", Новый ОписаниеТипов("Булево"));
	ДанныеУведомлений.Колонки.Добавить("РасчитаннаяСуммаНалога", Метаданные.ОпределяемыеТипы.ДенежнаяСуммаЛюбогоЗнака.Тип);
	
	Возврат ДанныеУведомлений;
	
КонецФункции

Функция НовыйСозданныеУведомления()
	
	МетаданныеУведомлениеНалоги = Метаданные.Документы.УведомлениеОбИсчисленныхСуммахНалогов.ТабличныеЧасти.Налоги;
	
	СозданныеУведомления = Новый ТаблицаЗначений;
	СозданныеУведомления.Колонки.Добавить("ДатаУведомления", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	СозданныеУведомления.Колонки.Добавить("Уведомление", Новый ОписаниеТипов("ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов"));
	СозданныеУведомления.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0));
	СозданныеУведомления.Колонки.Добавить("Налог", МетаданныеУведомлениеНалоги.Реквизиты.Налог.Тип);
	СозданныеУведомления.Колонки.Добавить("КодБК", МетаданныеУведомлениеНалоги.Реквизиты.КодБК.Тип);
	СозданныеУведомления.Колонки.Добавить("ВидПлатежаВГосБюджет", МетаданныеУведомлениеНалоги.Реквизиты.ВидПлатежаВГосБюджет.Тип);
	СозданныеУведомления.Колонки.Добавить("УровеньБюджета", МетаданныеУведомлениеНалоги.Реквизиты.УровеньБюджета.Тип);
	СозданныеУведомления.Колонки.Добавить("КодПоОКТМО", МетаданныеУведомлениеНалоги.Реквизиты.КодПоОКТМО.Тип);
	СозданныеУведомления.Колонки.Добавить("СтатусОтправки", Метаданные.РегистрыСведений.ЖурналОтправокВКонтролирующиеОрганы.Ресурсы.СтатусОтправки.Тип);
	СозданныеУведомления.Колонки.Добавить("КонецПериода", МетаданныеУведомлениеНалоги.Реквизиты.КонецПериода.Тип);
	СозданныеУведомления.Колонки.Добавить("РегистрацияВНалоговомОргане", МетаданныеУведомлениеНалоги.Реквизиты.РегистрацияВНалоговомОргане.Тип);
	СозданныеУведомления.Колонки.Добавить("СрокУплаты", МетаданныеУведомлениеНалоги.Реквизиты.СрокУплаты.Тип);
	СозданныеУведомления.Колонки.Добавить("Сумма", МетаданныеУведомлениеНалоги.Реквизиты.Сумма.Тип);
	СозданныеУведомления.Колонки.Добавить("НачалоПериода", МетаданныеУведомлениеНалоги.Реквизиты.НачалоПериода.Тип);
	СозданныеУведомления.Колонки.Добавить("РегистрацияВНалоговомОрганеУведомления", МетаданныеУведомлениеНалоги.Реквизиты.РегистрацияВНалоговомОргане.Тип);

	Возврат СозданныеУведомления;
	
КонецФункции

Процедура ЗаполнитьСозданныеУведомления(ЗадачаПоУведомлению, СозданныеУведомления, ВсеСозданныеУведомления)
	
	ОтборПоЗадачам = Новый Структура("Организация,Правило,ПериодСобытия,РегистрацияВНалоговомОргане,Действие");
	ЗаполнитьЗначенияСвойств(ОтборПоЗадачам, ЗадачаПоУведомлению);
	
	КлючевыеПоляУведомлений = Новый Структура(КлючевыеПоляУведомлений()); 
	ДополненныеКлючевыеПоляУведомлений = Новый Структура(КлючевыеПоляУведомлений());
	ДополненныеКлючевыеПоляУведомлений.Вставить("Уведомление");
	
	СтрокиВсеСозданныеУведомления = ВсеСозданныеУведомления.НайтиСтроки(ОтборПоЗадачам);
	Для Каждого СтрокаВсеСозданныеУведомления Из СтрокиВсеСозданныеУведомления Цикл
		
		ДобавитьЗапись = Ложь;
		ЗаменитьЗаписи = Ложь;
		
		Если ВыполнениеЗадачБухгалтера.УведомлениеСодержитПромежуточныйНДФЛ(СтрокаВсеСозданныеУведомления.Уведомление) Тогда
			Продолжить;
		КонецЕсли;
		
		Если УведомлениеНеСоответствуетПравилуНДФЛ(СтрокаВсеСозданныеУведомления) Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(КлючевыеПоляУведомлений, СтрокаВсеСозданныеУведомления);
	
		СтрокаУведомления = НайтиСтрокуУведомленияПоКлючевымПолям(СозданныеУведомления, КлючевыеПоляУведомлений);
		
		// Уведомлений по одному налогу может быть несколько. Поэтому нужно сравнить уже добавленное уведомление
		// с добавляемым и решить, какое из них более приоритетное.
		// Кроме того в одном уведомлении может быть несколько записей по одному налогу. В этом случае их надо суммировать;
		Если СтрокаУведомления = Неопределено Тогда
			ДобавитьЗапись = Истина;
		Иначе
			ЗаполнитьЗначенияСвойств(ДополненныеКлючевыеПоляУведомлений, СтрокаВсеСозданныеУведомления); 
			
			ДополненнаяСтрокаУведомления = НайтиСтрокуУведомленияПоКлючевымПолям(СозданныеУведомления, ДополненныеКлючевыеПоляУведомлений);
			Если ДополненнаяСтрокаУведомления <> Неопределено Тогда
				ДобавитьЗапись = Истина;
			Иначе
				Если СуществующееУведомлениеБолееПриоритетное(СтрокаВсеСозданныеУведомления, СтрокаУведомления) Тогда
					// Уже записанное уведомление более приоритетное, поэтому не перезатираем его.
					Продолжить;
				Иначе
					ЗаменитьЗаписи = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если ЗаменитьЗаписи Тогда
			УдаляемыеСтроки = СозданныеУведомления.НайтиСтроки(КлючевыеПоляУведомлений);
			Для Каждого СтрокаТаблицы Из УдаляемыеСтроки Цикл
				СозданныеУведомления.Удалить(СтрокаТаблицы);
			КонецЦикла;
			ДобавитьЗапись = Истина;
		КонецЕсли;
		Если ДобавитьЗапись Тогда
			СтрокаУведомления = СозданныеУведомления.Добавить();
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(СтрокаУведомления, СтрокаВсеСозданныеУведомления);
		
	КонецЦикла;
	
	СозданныеУведомления.Сортировать("ДатаУведомления,НомерСтроки");
	
КонецПроцедуры

Функция УведомлениеНеСоответствуетПравилуНДФЛ(СтрокаТаблицыУведомлений)
	
	ПолноеИмяПравила = Справочники.ПравилаПредставленияОтчетовУплатыНалогов.ПолноеИмяПравила(СтрокаТаблицыУведомлений.Правило);
	
	ЭтоПравилоТринадцатогоНДФЛ
		= ВыполнениеЗадачБухгалтера.ЭтоПравилоТринадцатогоНДФЛ(ПолноеИмяПравила);
	УведомлениеСодержитНДФЛЗаТринадцатыйПериод
		= ВыполнениеЗадачБухгалтера.УведомлениеСодержитНДФЛЗаТринадцатыйПериод(СтрокаТаблицыУведомлений.Уведомление); 
	ЭтоПравилоНДФЛКонецМесяца
		= ВыполнениеЗадачБухгалтера.ЭтоПравилоНДФЛКонецМесяца(ПолноеИмяПравила);
	УведомлениеСодержитНДФЛЗаКонецМесяца
		= ВыполнениеЗадачБухгалтера.УведомлениеСодержитНДФЛЗаКонецМесяца(СтрокаТаблицыУведомлений.Уведомление, СтрокаТаблицыУведомлений.ПериодСобытия);
	ЭтоПравилоУведомлениеПоНДФЛ
		= ВыполнениеЗадачБухгалтера.ЭтоПравилоУведомлениеПоНДФЛ(СтрокаТаблицыУведомлений.Правило);
	УведомлениеСодержитНДФЛЗаНачалоМесяца
		= ВыполнениеЗадачБухгалтера.УведомлениеСодержитНДФЛЗаНачалоМесяца(СтрокаТаблицыУведомлений.Уведомление, СтрокаТаблицыУведомлений.ПериодСобытия);
		
	Возврат ((ЭтоПравилоТринадцатогоНДФЛ И Не УведомлениеСодержитНДФЛЗаТринадцатыйПериод) 
				Или (Не ЭтоПравилоТринадцатогоНДФЛ И УведомлениеСодержитНДФЛЗаТринадцатыйПериод))
			Или (ЭтоПравилоУведомлениеПоНДФЛ И Не ЭтоПравилоТринадцатогоНДФЛ И
				((ЭтоПравилоНДФЛКонецМесяца И Не УведомлениеСодержитНДФЛЗаКонецМесяца)
				Или (Не ЭтоПравилоНДФЛКонецМесяца И УведомлениеСодержитНДФЛЗаКонецМесяца И Не УведомлениеСодержитНДФЛЗаНачалоМесяца)));
	
КонецФункции

Функция НайтиСтрокуУведомленияПоКлючевымПолям(СозданныеУведомления, КлючевыеПоляУведомлений)
	
	НайденныеСтроки = СозданныеУведомления.НайтиСтроки(КлючевыеПоляУведомлений);
	Если НайденныеСтроки.Количество() > 0 Тогда
...
Функция СуществующееУведомлениеБолееПриоритетное(СтрокаНовогоУведомления, СтрокаСуществующегоУведомления)
	
	// Уведомление более приоритетное, если у него статус "Сдано", а у второго - не "Сдано".
	// Если же приоритет статусов одинаков, то приоритетным является то уведомление, которое создано позже.
	
	Если СтрокаНовогоУведомления.СтатусОтправки <> СтрокаСуществующегоУведомления.СтатусОтправки Тогда
		// Статусы отличаются, при этом если у одного из них статус "Сдано" - то значит, это уведомление лучше.
		Если СтрокаНовогоУведомления.СтатусОтправки = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусСданоСтрокой() Тогда
			Возврат Ложь;
		КонецЕсли;
		
		Если СтрокаСуществующегоУведомления.СтатусОтправки = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусСданоСтрокой() Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	
	// Уведомления имеют либо одинаковый статус, либо ни одно из них не сдано.
	// Поэтому приоритет по статусу у них одинаковый, и нужно посмотреть на дату уведомления и выбрать более позднее.
	Возврат СтрокаНовогоУведомления.ДатаУведомления < СтрокаСуществующегоУведомления.ДатаУведомления;
	
КонецФункции

Функция ПроверитьПравильностьСозданныхУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок)
	
	Если ПроверитьПрименениеНПД(СтрокаПроверки, ПараметрыОтчета) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПодготовленныеДанныеУведомленияЕНС = ПодготовленныеДанныеДляПроверок.ПодготовленныеДанныеУведомленияЕНС;
	
	ТаблицаОшибок = НовыйТаблицаОшибокСозданныхУведомленийЕНС();
	
	Для Каждого ДанныеУведомленияЕНС Из ПодготовленныеДанныеУведомленияЕНС Цикл
		
		Если ДанныеУведомленияЕНС.НалогРассчитан
			И ДанныеУведомленияЕНС.РасчитаннаяСуммаНалога = 0 Тогда
			// Налог рассчитан и уведомления не нужны. Поэтому не показываем этот налог в проблемах
			Продолжить;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ДанныеУведомленияЕНС.СозданныеУведомления) Тогда
			Продолжить;
		КонецЕсли;
		
		Ошибка = ТаблицаОшибок.Добавить();
		ЗаполнитьЗначенияСвойств(Ошибка, ДанныеУведомленияЕНС);
		
		Расшифровка = Новый Структура();
		Расшифровка.Вставить("Идентификатор", "СозданиеУведомленийЕНС");
		Расшифровка.Вставить("ПараметрыОткрытияЗадачи", ДанныеУведомленияЕНС.ПараметрыОткрытияЗадачи);
		
		Ошибка.Расшифровка = Расшифровка;
		
	КонецЦикла;
	
	ТаблицаОшибок.Сортировать("Срок,НаименованиеЗадачиВСписке");
	
	Возврат ТаблицаОшибок;
	
КонецФункции

Функция НовыйТаблицаОшибокСозданныхУведомленийЕНС()
	
	МетаданныеРегистраЗадачиБухгалтера = Метаданные.РегистрыСведений.ЗадачиБухгалтера;
	
	ТаблицаОшибок = Новый ТаблицаЗначений;
	ТаблицаОшибок.Колонки.Добавить("НаименованиеЗадачиВСписке", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Наименование.Тип);
	ТаблицаОшибок.Колонки.Добавить("Срок", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Срок.Тип);
	ТаблицаОшибок.Колонки.Добавить("Расшифровка", Новый ОписаниеТипов("Структура"));
	
	Возврат ТаблицаОшибок;
	
КонецФункции

Функция ПроверитьСтатусУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок)
	...
	ТаблицаОшибок = НовыйТаблицаОшибокСтатусУведомленийЕНС();
	
	Для Каждого ДанныеУведомленияЕНС Из ПодготовленныеДанныеУведомленияЕНС Цикл
		
		Для Каждого СозданноеУведомление Из ДанныеУведомленияЕНС.СозданныеУведомления Цикл
			Если СозданноеУведомление.СтатусОтправки = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусСданоСтрокой() Тогда
				Продолжить;
			КонецЕсли;
			
			Если ТаблицаОшибок.Найти(СозданноеУведомление.Уведомление, "Уведомление") <> Неопределено Тогда
				// Это уведомление уже обработано в другом налоге.
				// Добавлять его повторно не требуется.
				Продолжить;
			КонецЕсли;
			
			Ошибка = ТаблицаОшибок.Добавить();
			ЗаполнитьЗначенияСвойств(Ошибка, СозданноеУведомление);
			
		КонецЦикла;
		
	КонецЦикла;
	
	ТаблицаОшибок.Сортировать("ДатаУведомления,Уведомление");
	
	Возврат ТаблицаОшибок;
	
КонецФункции

Функция НовыйТаблицаОшибокСтатусУведомленийЕНС()
	
	ТаблицаОшибок = Новый ТаблицаЗначений;
	ТаблицаОшибок.Колонки.Добавить("ДатаУведомления", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	ТаблицаОшибок.Колонки.Добавить("Уведомление", Новый ОписаниеТипов("ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов"));
	ТаблицаОшибок.Колонки.Добавить("СтатусОтправки", Метаданные.РегистрыСведений.ЖурналОтправокВКонтролирующиеОрганы.Ресурсы.СтатусОтправки.Тип);
	
	Возврат ТаблицаОшибок;
	
КонецФункции

Функция ПроверитьСуммыУведомленийЕНС(СтрокаПроверки, ПараметрыОтчета, ПодготовленныеДанныеДляПроверок)
	...
	ТаблицаОшибок = НовыйТаблицаОшибокСуммыУведомленийЕНС();
	
	Для Каждого ДанныеУведомленияЕНС Из ПодготовленныеДанныеУведомленияЕНС Цикл
		
		Если НЕ ЗначениеЗаполнено(ДанныеУведомленияЕНС.СозданныеУведомления) Тогда
			// Если уведомлений нет совсем - то будет показано предупреждение
			// в проверке СозданиеУведомленийЕНС. Здесь их показывать не нужно.
			Продолжить;
		КонецЕсли;
		
		СуммаУведомлений = ДанныеУведомленияЕНС.СозданныеУведомления.Итог("Сумма");
		
		Если ДанныеУведомленияЕНС.РасчитаннаяСуммаНалога = СуммаУведомлений Тогда
			Продолжить;
		КонецЕсли;
		
		Ошибка = ТаблицаОшибок.Добавить();
		ЗаполнитьЗначенияСвойств(Ошибка, ДанныеУведомленияЕНС);
		Ошибка.СуммаУведомлений = СуммаУведомлений;
...
Функция НовыйТаблицаОшибокСуммыУведомленийЕНС()
	...
	ТаблицаОшибок.Колонки.Добавить("НаименованиеЗадачиВСписке", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Наименование.Тип);
	ТаблицаОшибок.Колонки.Добавить("Срок", МетаданныеРегистраЗадачиБухгалтера.Реквизиты.Срок.Тип);
	ТаблицаОшибок.Колонки.Добавить("РасчитаннаяСуммаНалога", Метаданные.ОпределяемыеТипы.ДенежнаяСуммаЛюбогоЗнака.Тип);
	ТаблицаОшибок.Колонки.Добавить("СуммаУведомлений", Метаданные.ОпределяемыеТипы.ДенежнаяСуммаЛюбогоЗнака.Тип);
	ТаблицаОшибок.Колонки.Добавить("Расшифровка", Новый ОписаниеТипов("Структура"));
...
Функция ПараметрыОткрытияЗадачи(ЗадачаПоУведомлению)
	
	ПараметрыОткрытияЗадачи = Новый Структура("Задача,ИдентификаторЗадачи,Организация,
	|РегистрацияВНалоговомОргане,Срок,НачалоВыполнения,Правило,ИдентификаторПравила,
	|ПериодСобытия,Периодичность,Наименование,Действие,НачальныйСрок");
	ЗаполнитьЗначенияСвойств(ПараметрыОткрытияЗадачи, ЗадачаПоУведомлению);
	ПараметрыОткрытияЗадачи.Наименование = ЗадачаПоУведомлению.НаименованиеЗадачиВСписке;
	Возврат ПараметрыОткрытияЗадачи;
...
Функция ПроверитьНеприменениеУСН(СтрокаПроверки, ПараметрыОтчета)
	...
	ПрименяетсяОсвобождениеОтУплатыНДС      = УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода);
	
	Если ПрименяетсяУСН 
		И Не ПлательщикНДСПрименяющийУСН Тогда
	...
	Если ПрименяетсяОсобыйПорядокНалогообложения Тогда
		...
		ТекстКомментария = НСтр("ru = 'Проверка не может быть выполнена, так как организация на %1 применяет особый порядок налогообложения.'");
		СтрокаПроверки.КомментарийРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстКомментария, Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy"));
	...
	Если ПрименяетсяОсвобождениеОтУплатыНДС Тогда
	
		ТекстКомментария = НСтр("ru = 'Проверка не может быть выполнена, так как организация на %1 применяет освобождение от уплаты НДС по ст. 145 или ст. 145.1 НК РФ.'");
		СтрокаПроверки.КомментарийРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстКомментария, Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy"));
...
Функция ПроверитьНеприменениеАУСН(СтрокаПроверки, ПараметрыОтчета)
	...
	ПрименяетсяАУСН = УчетнаяПолитика.ПрименяетсяАУСН(ПараметрыОтчета.Организация, ПараметрыОтчета.НачалоПериода);
	Если ПрименяетсяАУСН  Тогда
	
		ТекстКомментария = НСтр("ru = 'Проверка не может быть выполнена, так как организация на %1 применяет автоматизированную упрощенную систему налогообложения.'");
		СтрокаПроверки.КомментарийРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстКомментария, Формат(ПараметрыОтчета.НачалоПериода, "ДФ=dd.MM.yyyy"));
