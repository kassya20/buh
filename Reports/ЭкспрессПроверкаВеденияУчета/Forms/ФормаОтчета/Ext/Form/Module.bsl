...
Процедура РезультатВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Расшифровка = "НастроеноИспользованиеСчетовПроизводства" Тогда
		ОткрытьФормуСпискаУчетнойПолитики();
...
Процедура ЗаполнитьДеревоПроверок()
	...
	ИспользуетсяУСНСНДС = ПолучитьФункциональнуюОпцию("ИспользуетсяУСНСНДС");
	...
	ЭтоНекоммерческаяОрганизация = Справочники.Организации.ЭтоНекоммерческаяОрганизация(Отчет.Организация);
	...
	Для НомСтроки = НомПервойСтрокиПравил По НомПоследнейСтрокиПравил Цикл
		...
		Если ЕстьИмяУСтроки Тогда
			...
			ПропускатьПроверкуДляНКО = МакетПравил.Область(ИмяОбластиСтроки + "|ПропускатьПроверкуДляНКО").Текст = "1";
			ИмяФункциональнойОпции = МакетПравил.Область(ИмяОбластиСтроки + "|ФункциональнаяОпция").Текст;
			
			Если ЗначениеЗаполнено(ИмяФункциональнойОпции)
				И Не ПолучитьФункциональнуюОпцию(ИмяФункциональнойОпции) Тогда
				Продолжить;
			КонецЕсли;
			
			Если ПроверятьТолькоПриНаличииНДС Тогда
				Если НЕ (ИспользуетсяОСНО ИЛИ ИспользуетсяНДФЛИП ИЛИ ИспользуетсяУСНСНДС) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если ПропускатьПроверкуДляНКО И ЭтоНекоммерческаяОрганизация Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого Строка Из ГруппыПроверок Цикл
		Если Строка.ПолучитьЭлементы().Количество() = 0 Тогда
			УдаляемыеГруппы.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого УдаляемаяСтрока Из УдаляемыеГруппы Цикл
		ГруппыПроверок.Удалить(УдаляемаяСтрока);
	КонецЦикла;
...
Функция СформироватьОтчетНаСервере() Экспорт
	...
	ОписаниеЗаданияФормированияОтчета = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыЗадания, "Отчеты.ЭкспрессПроверкаВеденияУчета.СформироватьОтчет", ПараметрыОтчета);
...
&НаКлиенте
Процедура ОткрытьЗадачуБухгалтера(Расшифровка)
	
	ПараметрыКоманды = ВыполнениеЗадачБухгалтераКлиентСервер.НовыеПараметрыКомандЗадачи();
	ЗаполнитьЗначенияСвойств(ПараметрыКоманды, Расшифровка.ПараметрыОткрытияЗадачи);
