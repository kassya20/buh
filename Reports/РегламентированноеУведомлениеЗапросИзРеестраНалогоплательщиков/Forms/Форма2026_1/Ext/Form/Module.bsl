...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ЗапросИзРеестраНалогоплательщиков;
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Иначе
		СтрокаСведений = "ИННФЛ,ФИО,ТелДом,ФамилияИП,ИмяИП,ОтчествоИП,КодУдЛичн,СерияУдЛичн,НомерУдЛичн,ДатаРожд";
		СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			Объект.Организация, Объект.ДатаПодписи, СтрокаСведений);
		...
		ДанныеУведомленияТитульный.Вставить("ПрЗаяв", "2");
		Если Не ЗначениеЗаполнено(СведенияОбОрганизации.ИННФЛ) Тогда 
			...
			ДанныеУведомленияТитульный.Вставить("СерНомДок",
				СокрЛП(СведенияОбОрганизации.СерияУдЛичн + " " + СведенияОбОрганизации.НомерУдЛичн));
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Запрос выписки" + Символы.ПС + "о себе";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "ЗапрВыпСеб_2026";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Истина;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЗапрВыпСеб";
	Стр001.МногострочныеЧасти.Добавить("МнгСтр1");
	Стр001.МногострочныеЧасти.Добавить("МнгСтр2");
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Запрос выписки" + Символы.ПС + "о третьем лице";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "ЗапрВыпТрЛицо_2026";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЗапрВыпТрЛицо";
КонецПроцедуры
...
Процедура ПоказатьТекущуюСтраницу(ИмяМакета, МногострочныеЧасти, ПредУИД)
	...
	Если ЭтотОбъект["ТекущееИДНаименования"] = "Титульная" Тогда 
		...
		КонецЕсли;
	ИначеЕсли ЭтотОбъект["ТекущееИДНаименования"] = "ЗапрВыпСеб" Тогда
