...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.прибыль 25к4",
		'20260101') Тогда
...
Процедура СформироватьСпискиВыбора()
	...
	// Лист 05.1.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000510000100", 1, ,
	"Выбор метода списания стоимости ЦВ", КоллекцияСписковВыбора["МетодыСписанияСтоимостиЦВ"]);
	...
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиУменьшенияУвеличения"]);
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000800008503", 1, ,
	"Выбор признака", КоллекцияСписковВыбора["ПризнакиНетДа"]);
...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "П000250006103" ИЛИ Область.Имя = "П000250006104"
	...
	ИначеЕсли Область.Имя = "П000220013503" ИЛИ Область.Имя = "П000510000200" Тогда
...
Процедура ТабличныйДокументПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если ИмяПоказателя = "П000020000200" ИЛИ ИмяПоказателя = "П000240000200" ИЛИ ИмяПоказателя = "П000250000200" Тогда
		...
		Если ПризнакНП = "03" Тогда
			...
			Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= 2026 Тогда
				...
				= СтрДлина(НомерДок) >= 2 И СтрНайти("1234568", Лев(НомерДок, 1)) > 0 И Сред(НомерДок, 2, 1) = "/";
				...
				Иначе
					...
					+ "(цифра 1-6, 8), ""Н"" - строка длиной 0-19 символов. Значение очищено";
			...
			Иначе
				...
				Иначе
					...
					ТекстСообщения = "Если код признака налогоплательщика равен ""03"", номер документа должен соответствовать "
					+ "шаблону ""К/Н"", где ""К"" - код типа особых (свободных) экономических зон или свободного порта Владивосток "
					+ "(цифра 1-8), ""Н"" - строка длиной 0-19 символов. Значение очищено";
					ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(НСтр("ru='%1'"), ТекстСообщения));
				КонецЕсли;
	...
	Если СтрНачинаетсяС(ИмяПоказателя, "П00008М200008") Тогда
		
		ТекстСообщенияОбщий
			= НСтр("ru='Уникальный номер контракта / кредитного договора должен соответствовать одному из шаблонов: "
	...
	Если ИмяПоказателя = "ПоМесту" ИЛИ (ИмяРаздела = "Лист02_5" И ИмяПоказателя = "ПризнакВключения") Тогда
		...
		// Заполнение стр.160 Листа 02 не предусмотрено, если есть обособленные подразделения.
		// Ее обнуление выполняется в "ОбработатьНаличиеОтсутствиеОбособленныхПодразделений".
		//
	...
	Если СтруктураЗависимыхЯчеекДляАвторасчета.Свойство(ИмяРаздела)
		...
		Если ИмяПоказателя = "ПризнакВключения" ИЛИ СтрНачинаетсяС(ИмяРаздела, "Лист02") ИЛИ ИмяРаздела = "Лист05_1" Тогда
...
Процедура УстановитьДоступностьОбластей(Форма, ИмяРаздела)
	...
	Если ТДО.Найти("ПризнакВключения") <> Неопределено Тогда
		...
		Если НЕ ВключитьРаздел Тогда
			Если ИмяРаздела = "Лист02" Тогда
				СекцияПояснение370_390 = ТДО.Найти("СекцияПояснение370_390");
				Если СекцияПояснение370_390 <> Неопределено Тогда
					СекцияПояснение370_390.Видимость = Ложь;
				КонецЕсли;
			...
			ИначеЕсли ИмяРаздела = "Приложение2" Тогда
				СекцияПояснение_РазделБ = ТДО.Найти("СекцияПояснение_РазделБ");
				Если СекцияПояснение_РазделБ <> Неопределено Тогда
					СекцияПояснение_РазделБ.Видимость = Ложь;
				КонецЕсли;
	...
	Если ИмяРаздела = "Лист02" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.ТекущийЛист02_ЕстьОбособленныеПодразделения = Истина Тогда
			// Заполнение стр.160 не предусмотрено.
	...
	ИначеЕсли ИмяРаздела = "Лист08" Тогда
		...
		Иначе
			...
			СтруктЗависимыхЯчеек = Форма.СтруктураЗависимыхЯчеекДляАвторасчета[ИмяРаздела];
			Для каждого Пок Из СтруктЗависимыхЯчеек Цикл
				Если Пок.Ключ = "П000800005004" Тогда
					АвтоРасчПоказатели.Добавить(Пок.Ключ);
				КонецЕсли;
			КонецЦикла;
		...
		ПрНетДа = СокрЛП(ТДО["П000800008503"].Значение);
		
		Если ПрНетДа = "1" Тогда
			// Заполнение стр.070 и стр.080 не предусмотрено.
			ЗапретитьВводДанныхВОбласть(Форма, ТДО["П000800007003"]);
			ЗапретитьВводДанныхВОбласть(Форма, ТДО["П000800008003"]);
		Иначе
			РазрешитьВводДанныхВОбласть(Форма, ТДО["П000800007003"]);
			СтруктЗависимыхЯчеек = Форма.СтруктураЗависимыхЯчеекДляАвторасчета[ИмяРаздела];
			Для каждого Пок Из СтруктЗависимыхЯчеек Цикл
				Если Пок.Ключ = "П000800008003" Тогда
					АвтоРасчПоказатели.Добавить(Пок.Ключ);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура Расчет(Форма, ИмяРаздела = "", ОбластьИмя = "") Экспорт
	...
	ИначеЕсли ИмяРаздела = "Лист05_1" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяРаздела Тогда
			...
			Если ЗначениеЗаполнено(ОбластьИмя) И ЗначениеЗаполнено(ТДО["ПризнакВключения"].Значение) Тогда
				...
				Если ПрНетДа = "1" Тогда
					...
					// Заполнение стр.080 не предусмотрено.
					
				Иначе
					
					ТДО["П000800008003"].Значение = ОкрЧФ(Форма,
						ОкрЧФ(Форма, ТДО["П000800006003"].Значение) * ТДО["П000800007003"].Значение / 100);
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Лист02" Тогда
		...
		Для каждого Лист02 Из Листы02 Цикл
			...
			Если ЗначениеЗаполнено(Лист02.Данные["ПризнакВключения"]) Тогда
				...
				УчитыватьНаТекущемЛисте02_ПрНП_01_ДанныеДругихЛистовВЦеломПоОрганизации = Ложь;
				...
				Если Лист02_ПрНП_01 <> Неопределено Тогда
					...
					УчитыватьНаТекущемЛисте02_ПрНП_01_ДанныеДругихЛистовВЦеломПоОрганизации = Лист02 = Лист02_ПрНП_01;;
				...
				Если УчитыватьНаТекущемЛисте02_ПрНП_01_ДанныеДругихЛистовВЦеломПоОрганизации Тогда
					...
					Лист05_1 = ПолучитьИзВременногоХранилища(
						Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхЛист05_1"]).Строки[0];
					
					Лист05_1_Стр550 = ОкрЧФ(Форма, Лист05_1.Данные["П000510055003"]);
				...
				Если ОтчетЗаКвартал Тогда
					...
					Если СтрРазделить("01,02,03,04,06,07,08,09,10,11,12,14,15,16,17,18,19,20,21,22",
				...
				Стр100 = Стр060 - Стр070 - Лист02_2_Стр400 + Лист05_Стр100 + Лист05_1_Стр550
				...
				Если ОтчетныйМесяц = 3 ИЛИ ОтчетныйМесяц = 12 Тогда
					...
					Если СтрРазделить("01,03,04,06,07,08,15,16,17,18,19,20,21,22", ",", Ложь).Найти(ПризнакНП) <> Неопределено Тогда
				...
				Иначе
					...
					Стр200 = ОкрЧФ(Форма, Стр120 * Стр160 / 100);
			...
			Для каждого Лист02_4 Из Листы02_4 Цикл
				...
				Если ЗначениеЗаполнено(Лист02_4.Данные["ПризнакВключения"]) Тогда
					...
					Если ЗначениеЗаполнено(ПризнакНП) И ПризнакНП <> "05" Тогда
						Если ПризнакНП = "51" И Лист02_ПризнакНП = "01" Тогда
							Лист05_1 = ПолучитьИзВременногоХранилища(
								Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхЛист05_1"]).Строки[0];
							Лист05_1_Стр500 = ОкрЧФ(Форма, Лист05_1.Данные["П000510050003"]);
							Лист02_4.Данные.Вставить("П000240014003", Макс(0, Лист05_1_Стр500));
						ИначеЕсли ПризнакНП = Лист02_ПризнакНП Тогда
					...
					Если ОтчетныйМесяц = 12 Тогда
						...
						ДеревоДанныхЛист05 = ПолучитьИзВременногоХранилища(
						Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхЛист05"]);
						ДеревоДанныхЛист05_1 = ПолучитьИзВременногоХранилища(
						Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхЛист05_1"]);
						...
						Если СтрРазделить("01,03,04,06,07,08,15,16,17,18,19,20,21,22", ",", Ложь).Найти(ПризнакНП) <> Неопределено Тогда
							...
							ЕстьЛист05ИлиЛист05_1ИлиЛист06 = Ложь;
							...
							Если РегламентированнаяОтчетностьКлиентСервер.ВеткаУдовлетворяющаяУсловиямПоиска(
								...
								ДеревоДанныхЛист05_1.Строки, Усл) <> Неопределено
								ИЛИ РегламентированнаяОтчетностьКлиентСервер.ВеткаУдовлетворяющаяУсловиямПоиска(
								ДеревоДанныхЛист06.Строки, Усл) <> Неопределено Тогда
								
								ЕстьЛист05ИлиЛист05_1ИлиЛист06 = Истина;
								
							КонецЕсли;
							
							Если Стр140 < 0 ИЛИ (Стр140 = 0 И Лист02_Стр100 < 0) Тогда
								
								Если ЕстьЛист05ИлиЛист05_1ИлиЛист06 Тогда
									...
									Учитывать_Лист05_Стр100   = Ложь;
									Учитывать_Лист05_1_Стр550 = Ложь;
									Учитывать_Лист06_Стр530   = Ложь;
									
									Если Лист02_4_ПрНП_01 <> Неопределено Тогда
										Учитывать_Лист05_Стр100   = Лист02_4 = Лист02_4_ПрНП_01;
										Учитывать_Лист05_1_Стр550 = Лист02_4 = Лист02_4_ПрНП_01;
										Учитывать_Лист06_Стр530   = Лист02_4 = Лист02_4_ПрНП_01;
									ИначеЕсли Лист02_4_ПрНП_03 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_03;
									ИначеЕсли Лист02_4_ПрНП_06 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_06;
									ИначеЕсли Лист02_4_ПрНП_04 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_04;
									ИначеЕсли Лист02_4_ПрНП_07 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_07;
									ИначеЕсли Лист02_4_ПрНП_08 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_08;
									ИначеЕсли Лист02_4_ПрНП_15 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_15;
									ИначеЕсли Лист02_4_ПрНП_16 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_16;
									ИначеЕсли Лист02_4_ПрНП_17 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_17;
									ИначеЕсли Лист02_4_ПрНП_18 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_18;
									ИначеЕсли Лист02_4_ПрНП_19 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_19;
									ИначеЕсли Лист02_4_ПрНП_20 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_20;
									ИначеЕсли Лист02_4_ПрНП_21 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_21;
									ИначеЕсли Лист02_4_ПрНП_22 <> Неопределено Тогда
										Учитывать_Лист06_Стр530 = Лист02_4 = Лист02_4_ПрНП_22;
									КонецЕсли;
									...
									Если Учитывать_Лист05_Стр100 Тогда
										Лист05_Стр100 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(
											Форма, ДеревоДанныхЛист05.Строки, "П000050010003");
									КонецЕсли;
									Если Учитывать_Лист05_1_Стр550 Тогда
										Лист05_1_Стр550 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(
											Форма, ДеревоДанныхЛист05_1.Строки, "П000510055003");
									КонецЕсли;
									Если Учитывать_Лист06_Стр530 Тогда
									...
									Стр160 = Макс(0, Стр010 - ((Лист02_Стр060 - Лист02_Стр070)
									+ Макс(0, Лист05_Стр100) + Макс(0, Лист05_1_Стр550) + Макс(0, Лист06_Стр530)));
							...
							ИначеЕсли Стр140 > 0 Тогда
								...
								Стр160 = Макс(0, Стр010 - Стр150);
								
							Иначе
								
								Стр160 = Макс(0, Стр010);
						...
						ИначеЕсли ПризнакНП = "51" Тогда
							...
							Иначе
								...
								Лист05_1_Стр550 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(
									Форма, ДеревоДанныхЛист05_1.Строки, "П000510055003");
								Если Лист05_1_Стр550 <= 0 Тогда
									Лист05_1_Стр520 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(
										Форма, ДеревоДанныхЛист05_1.Строки, "П000510052003");
								КонецЕсли;
								
								Стр160 = Макс(0, Стр010 + Лист05_1_Стр520);
	...
	ИначеЕсли ИмяРаздела = "Лист05_1" Тогда
		...
		Если ЗначениеЗаполнено(Данные["ПризнакВключения"]) Тогда
			...
			// Стр.100 = стр.105 + стр.110 + стр.115 + стр.120 + стр.125.
			Данные.Вставить("П000510010003", ОкрЧФ(Форма, Данные["П000510010503"])
				+ ОкрЧФ(Форма, Данные["П000510011003"]) + ОкрЧФ(Форма, Данные["П000510011503"])
				+ ОкрЧФ(Форма, Данные["П000510012003"]) + ОкрЧФ(Форма, Данные["П000510012503"]));
			
			// Стр.300 = стр.305 + стр.310 + стр.315 + стр.320 + стр.325 + стр.330.
			Данные.Вставить("П000510030003", ОкрЧФ(Форма, Данные["П000510030503"])
				+ ОкрЧФ(Форма, Данные["П000510031003"]) + ОкрЧФ(Форма, Данные["П000510031503"])
				+ ОкрЧФ(Форма, Данные["П000510032003"]) + ОкрЧФ(Форма, Данные["П000510032503"])
				+ ОкрЧФ(Форма, Данные["П000510033003"]));
			
			// Стр.400 = стр.405 + стр.410 + стр.415 + стр.420 + стр.425.
			Данные.Вставить("П000510040003", ОкрЧФ(Форма, Данные["П000510040503"])
				+ ОкрЧФ(Форма, Данные["П000510041003"]) + ОкрЧФ(Форма, Данные["П000510041503"])
				+ ОкрЧФ(Форма, Данные["П000510042003"]) + ОкрЧФ(Форма, Данные["П000510042503"]));
			
			ПрибыльУбыток
				= ОкрЧФ(Форма, Данные["П000510010003"]) + ОкрЧФ(Форма, Данные["П000510020003"])
				- ОкрЧФ(Форма, Данные["П000510030003"]) - ОкрЧФ(Форма, Данные["П000510040003"]);
			
			// Стр.500 = стр.100 + стр.200 - стр.300 - стр.400, если результат >= 0.
			Данные.Вставить("П000510050003", Макс(0, ПрибыльУбыток));
			
			// Стр.520 = стр.100 + стр.200 - стр.300 - стр.400, если результат < 0.
			Данные.Вставить("П000510052003", -Мин(0, ПрибыльУбыток));
			
			// Стр.530 = стр.150 Прил. 4 к Листу 02 с кодом "51" в реквизите "Признак налогоплательщика (код)".
			Листы02 = ПолучитьИзВременногоХранилища(
				Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхЛист02"]).Строки;
			Лист02_ПрНП_01 = РегламентированнаяОтчетностьКлиентСервер.ВеткаУдовлетворяющаяУсловиямПоиска(
				Листы02, Новый Структура("П000020000100", "01"));
			Если Лист02_ПрНП_01 <> Неопределено Тогда
				Листы02_4 = Лист02_ПрНП_01.Строки[3].Строки;
				Лист02_4_Стр150 = РегламентированнаяОтчетностьКлиентСервер.СуммаПоказателяПоВеткам(Форма,
					Листы02_4, "П000240015003", Новый Структура("П000240000100", "51"));
				Данные.Вставить("П000510053003", Макс(0, Лист02_4_Стр150));
			Иначе
				Данные.Вставить("П000510053003", 0);
			КонецЕсли;
			
			// Стр.550 = стр.500 - стр.530.
			Данные.Вставить("П000510055003", ОкрЧФ(Форма, Данные["П000510050003"]) - ОкрЧФ(Форма, Данные["П000510053003"]));
	...
	ИначеЕсли ИмяРаздела = "Лист08" Тогда
		...
		Для каждого Ветка Из ДеревоДанных.Строки Цикл
			...
			Если ЗначениеЗаполнено(Данные["ПризнакВключения"]) Тогда
				...
				ПрНетДа = СокрЛП(Данные["П000800008503"]);
				...
				Иначе
					...
					Данные.Вставить("П000800008003", ОкрЧФ(Форма,
						ОкрЧФ(Форма, Данные["П000800006003"]) * Данные["П000800007003"] / 100));
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	Структ = Новый Структура;
	Структ.Вставить("П000510010003");
	Структ.Вставить("П000510030003");
	Структ.Вставить("П000510040003");
	Структ.Вставить("П000510050003");
	Структ.Вставить("П000510052003");
	Структ.Вставить("П000510053003");
	Структ.Вставить("П000510055003");
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("Лист05_1", Структ);
...
Процедура Проверить(Команда)
	...
	Если НЕ СтруктураРеквизитовФормы.мВыбраннаяФорма = "ФормаОтчета2025Кв4" Тогда
...
#Область Основные
	...
	ПроверкаКС_91_02_5_1();
	ПроверкаКС_91_02_5_2();
	ПроверкаКС_92_02_5();
	ПроверкаКС_93_02_5();
	...
	ПроверкаКС_96_02_7();
	ПроверкаКС_97_02_7(); 
	ПроверкаКС_99_02_7_1(); 
	ПроверкаКС_99_02_7_2(); 
...
Процедура СформироватьСлужебныеДанные()
	...
	ИменаРазделов.Вставить("П7Д", "Лист02_7Д");
	...
	ИменаРазделов.Вставить("Л51", "Лист05_1");
	...
	ПредставленияРазделов.Вставить("П7Д", "Приложения 7 к Листу 02 (Раздел Д)");
	...
	ПредставленияРазделов.Вставить("Л51", "Листа 05.1");
	...
	ЦелочисленныеРазделы.Добавить("П7Д");
	...
	ЦелочисленныеРазделы.Добавить("Л51");
...
Процедура СформироватьДанныеОтчета()
	...
	ДанныеКС.Вставить("Лист03Б"		, ДанныеКС.Лист03Б[0]);
	ДанныеКС.Вставить("Лист05_1"	, ДанныеКС.Лист05_1[0]);
	...
	Для Счетчик = 1 По ДанныеКС.Раздел1_3.КолвоСтрокП00013М1 Цикл
		...
		Если ЗначениеЗаполнено(Показатели.Р13С001.Значение) 
			И МассивКодовР13.Найти(Показатели.Р13С001.Значение) = Неопределено Тогда
		    МассивКодовР13.Добавить(Показатели.Р13С001.Значение);
		КонецЕсли;
	КонецЦикла;
	
	ДанныеКС.Вставить("МассивКодовР13", МассивКодовР13);
...
Процедура ПолучитьДанныеСтраницы(СтруктураПриемник, 
	...
	Если НЕ СтруктураИсточник.Строки.Количество() = 0 Тогда
		Если ВходящийРаздел.Наименование = "Лист02" И ВходящийРаздел.Уровень = 1 Тогда
			...
			СтруктураПриемник.Вставить("Лист02_7Б"	, СтруктураПриемник.Лист02_7Б[0]);
			СтруктураПриемник.Вставить("Лист02_7Д"	, СтруктураПриемник.Лист02_7Д[0]);
...
Процедура СформироватьПриоритетыРазделов()
	...
	ПриоритетыРазделов.Добавить("Приложение 7 к Листу 02");
	...
	ПриоритетыРазделов.Добавить("Лист 05.1");
...
Процедура ПодготовитьНастройки(Заголовки)
	...
	Если НЕ НастройкиКС.Критичность = 3 Тогда
		НастройкиКС.Основание 	= "Письмо ФНС России от 25.03.2026 N СД-36-3/2302@";
	КонецЕсли;
...
&НаСервере
Функция КодНайден(ЗначениеКода, Знач СтрокаКодов, Разделитель = ",")
...
Функция НомерСтраницы(Знач НомерСтрокой)
	...
	МассивЭлементов = СтрРазделить(НомерСтрокой, ",", Ложь);
	МаксИндекс = МассивЭлементов.ВГраница();
	СтрокаИсключений = "1,3";
	
	Если МаксИндекс = -1 Тогда
		Возврат "";
	КонецЕсли;
	
	Для ОбратныйИндекс = 0 По МаксИндекс Цикл
		ТекИндекс = МаксИндекс - ОбратныйИндекс;
		Если НЕ Найти(СтрокаИсключений, ТекИндекс) = 0 Тогда
			МассивЭлементов.Удалить(ТекИндекс);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверкаКС_1_01_1()
	...
	НастройкиКС.Вставить("Показатель1"	, "П00011М104003_");
	НастройкиКС.Вставить("Показатель2"	, "П000020027003");
...
Процедура ПроверкаКС_1_01_1_код()
	...
	ИменаПоказателей.Вставить("Л02С001"		, НастройкиКС.Показатель2);
	...
	НастройкиКС.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел1_1 
		...
		И НЕ Показатели.Р11С001.Итог = Показатели.Л02С001.Значение;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р11С001.Итог, Показатели.Л02С001.Значение);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_1_01_2_код()
	...
	ИменаПоказателей.Вставить("Р11С001"		, НастройкиКС.Показатель1);
	ИменаПоказателей.Вставить("Л02С001"		, НастройкиКС.Показатель2);
	ИменаПоказателей.Вставить("Л02С002"		, НастройкиКС.Показатель3);
	...
	НастройкиКС.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел1_1
		...
		И ?(НастройкиКС.Безусловно, Истина, ДанныеКС.БезОП)
		И РасчетнаяСумма > 0
		И НЕ Показатели.Р11С001.Итог = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р11С001.Итог, РасчетнаяСумма, Показатели.Л02С001.Итог, Показатели.Л02С002.Итог);
	ДополнитьТаблицуРезультата();
...
Процедура ПроверкаКС_2_01_1()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='2.01.1'"));
	Заголовки.Вставить("Описание"			, 
	...
	НастройкиКС.Вставить("Показатель1"	, "П00011М105003_");
	НастройкиКС.Вставить("Показатель2"	, "П000020028003");
	НастройкиКС.Вставить("Безусловно"	, Истина);
...
Процедура ПроверкаКС_2_01_2()
	...
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='2.01.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_3_01_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='3.01.1'"));
	Заголовки.Вставить("Описание"			, 
	...
	НастройкиКС.Вставить("Показатель1"	, "П00011М107003_");
	НастройкиКС.Вставить("Показатель2"	, "П000020027103");
...
&НаСервере
Процедура ПроверкаКС_3_01_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='3.01.2'"));
	Заголовки.Вставить("Описание"			, 
		...
		НСтр("ru='Приложение 5 к Листу 02) и количество Листов 02 > 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.070 подр.1.1 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 01.07.2024 N СД-4-3/7429@'"));
...
Процедура ПроверкаКС_4_01()
	...
	Заголовки.Вставить("Описание"			,	
		НСтр("ru='Сумма налога к доплате в бюджет субъекта в стр.070 подр.1.1 Раздела 1 = стр.100 '") +
		НСтр("ru='Приложений 5 к Листу 02 с кодом 1 по реквизиту ""Расчет составлен"", или стр.070 подр.1.1 '") +
		НСтр("ru='Раздела 1 = стр.100 Приложений 5 к Листу 02 с кодом 4 по реквизиту ""Расчет '") +
		НСтр("ru='составлен"", если организация имеет обособленные подразделения (заполнено Приложение 5 к '") + 
		НСтр("ru='Листу 02), 5 и 6 знаки КПП = 01. '") + 
		НСтр("ru='КС проверяется, если ""Признак налогоплательщика"" Листа 02 = ""Признак налогоплательщика '") +
		НСтр("ru='Приложения 5 к Листу 02'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 или %1 = %3'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.070 подр.1.1 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 01.07.2024 N СД-4-3/7429@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
...
&НаСервере
Процедура ПроверкаКС_4_01_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С001"		, НастройкиКС.ПоказательР11);
	...
	ИменаПоказателей.Вставить("П25С005"		, НастройкиКС.ПоказательЛ21);
	...
	НастройкиКС.ЭтоОшибка = ДанныеКС.РазделЗаполнен_Раздел1_1 И УсловиеВыполнено 
		И НЕ ДанныеКС.БезОП И (Показатели.П25С004.Итог > 0 ИЛИ  Показатели.П25С005.Итог > 0) 
		И НЕ (Показатели.Р11С001.Итог = Показатели.П25С004.Итог ИЛИ Показатели.Р11С001.Итог = Показатели.П25С005.Итог);
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Р11С001.Итог, Показатели.П25С004.Итог, Показатели.П25С005.Итог);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_5_01_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='5.01.1'"));
	Заголовки.Вставить("Описание"			, 
	...
	НастройкиКС.Вставить("Показатель1"	, "П00011М108003_");
	НастройкиКС.Вставить("Показатель2"	, "П000020028103");
	НастройкиКС.Вставить("Безусловно"	, Ложь);
...
&НаСервере
Процедура ПроверкаКС_5_01_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.1 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='5.01.2'"));
	Заголовки.Вставить("Описание"			, 
...
Процедура ПроверкаКС_6_01()
	...
	Заголовки.Вставить("Описание"			,	
		НСтр("ru='Сумма налога к уменьшению из бюджета субъекта в стр.080 подр.1.1 Раздела 1 = стр.110 '") +
		НСтр("ru='Приложений 5 к Листу 02 с кодом 1 по реквизиту ""Расчет составлен"", или стр.080 подр.1.1 '") +
		НСтр("ru='Раздела 1 = стр.110 Приложений 5 к Листу 02 с кодом 4 по реквизиту ""Расчет '") +
		НСтр("ru='составлен"", если организация имеет обособленные подразделения (заполнено Приложение 5 к '") + 
		НСтр("ru='Листу 02), 5 и 6 знаки КПП = 01. '") +
		НСтр("ru='КС проверяется, если ""Признак налогоплательщика"" Листа 02 = ""Признак налогоплательщика '") +
		НСтр("ru='Приложения 5 к Листу 02'"));
...
Процедура ПроверкаКС_7_01()
	...
	Заголовки.Вставить("Описание"			, НСтр("ru='5 и 6 знаки КПП в стр.056 подр.1.1 Раздела 1 <> 50'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 50'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная значение КПП'"));
	Заголовки.Вставить("Рекомендации"		, "Проверьте правильность КПП в стр.056 подр.1.1 Раздела 1");
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р11С001"		, "П00011М105603_");
	...
	Для Счетчик = 1 По ДанныеКС.Раздел1_1.КолвоСтрокП00011М1 Цикл  
		...
		Знаки56 = Сред(Показатели.Р11С001.Значение, 5, 2);	
		НастройкиКС.ЭтоОшибка = Знаки56 = "50";
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Знаки56);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_8_01_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='8.01.1'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_8_01_1_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_8_01_1_код()
...
&НаСервере
Процедура ПроверкаКС_8_01_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='8.01.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_8_01_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='8.01.3'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_8_01_3_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_8_01_3_код()
...
&НаСервере
Процедура ПроверкаКС_11_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='11.01'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_12_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='12.01'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_12_01_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_12_01_код()
...
&НаСервере
Процедура ПроверкаКС_13_01_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='13.01.1'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_13_01_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='13.01.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_13_01_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='13.01.3'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_14_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='14.01'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_16_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='16.01'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_16_01_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_16_01_код()
...
&НаСервере
Процедура ПроверкаКС_17_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='17.01'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_18_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='18.01'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_18_01_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_18_01_код()
...
&НаСервере
Процедура ПроверкаКС_19_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='19.01'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_19_01_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_19_01_код()
...
&НаСервере
Процедура ПроверкаКС_20_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='20.01'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_21_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.2 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='21.01'"));
	Заголовки.Вставить("Описание"			, НСтр("ru='5 и 6 знаки КПП в стр.206 подр.1.2 Раздела 1 <> 50'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 50'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная значение КПП'"));
	Заголовки.Вставить("Рекомендации"		, "Проверьте правильность КПП в стр.206 подр.1.2 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р12С001"		, "П00012М120603_");
	...
	Для Счетчик = 1 По ДанныеКС.Раздел1_2.КолвоСтрокП00012М1 Цикл  
		...
		Знаки56 = Сред(Показатели.Р12С001.Значение, 5, 2);	
		НастройкиКС.ЭтоОшибка = Знаки56 = "50";
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Знаки56);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_22_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.3 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='22.01'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Если код дохода в стр.020 подр.1.3 Раздела 1 = 11, то стр.070 подр.1.3 Раздела 1 = '") +
		НСтр("ru='стр.080 Листа 04 с аналогичным кодом дохода, а стр.030 подр.1.3 Раздела 1 = '") + 
		ДанныеКС.КБК.Проценты_Облигации);
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 по коду дохода 11, %3 = '") + 
		ДанныеКС.КБК.Проценты_Облигации);
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога или значение КБК'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните показатели в стр.030, 070 подр.1.3 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("КодДохода"		, "11");
	НастройкиКС.Вставить("КБК"				, ДанныеКС.КБК.Проценты_Облигации);
	
	ПроверкаКС_22_01_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_22_01_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р13С001"		, "П00013М102003_");
	ИменаПоказателей.Вставить("Р13С002"		, "П00013М103003_");
	ИменаПоказателей.Вставить("Р13С003"		, "П00013М107003_");
	ИменаПоказателей.Вставить("Л04С001"		, "П000040000100");
	ИменаПоказателей.Вставить("Л04С002"		, "П000040008003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	
	Для каждого КодДоходаР13 Из ДанныеКС.МассивКодовР13 Цикл 
		
		Если НЕ КодДоходаР13 = НастройкиКС.КодДохода Тогда
			Продолжить;
		КонецЕсли;
		...
		НеверныйКБК = Ложь;
		НеверныйКБКЗначение = "";
		Показатели.Р13С003.Итог = 0;
		Показатели.Л04С002.Итог = 0;
		
		Для Счетчик = 1 По ДанныеКС.Раздел1_3.КолвоСтрокП00013М1 Цикл
			
			ЗаполнитьПоказатель(Показатели.Р13С001, ДанныеКС.Раздел1_3, Счетчик);
			Если Показатели.Р13С001.Значение = КодДоходаР13 Тогда
				
				РасшифроватьПоказатель(Показатели.Р13С001);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р13С002, ДанныеКС.Раздел1_3, Счетчик);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р13С003, ДанныеКС.Раздел1_3, Счетчик);
				Если НЕ Показатели.Р13С002.Значение = НастройкиКС.КБК Тогда 
					НеверныйКБК = Истина;
					НеверныйКБКЗначение = Показатели.Р13С002.ЗначениеСтр; 
				КонецЕсли;
			КонецЕсли; 
		КонецЦикла;
		...
		Для каждого Лист04 Из ДанныеКС.Лист04 Цикл  
			...
			ЗаполнитьПоказатель(Показатели.Л04С001, Лист04);
			Если Показатели.Л04С001.Значение = НастройкиКС.КодДохода Тогда
				РасшифроватьПоказатель(Показатели.Л04С001);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Л04С002, Лист04);
			КонецЕсли;
		КонецЦикла;
		
		НастройкиКС.ЭтоОшибка = НеверныйКБК ИЛИ НЕ Показатели.Р13С003.Итог = Показатели.Л04С002.Итог;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р13С003.Итог, Показатели.Л04С002.Итог, 
			?(НеверныйКБК, НеверныйКБКЗначение, НастройкиКС.КБК));
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_23_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.3 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='23.01'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Если код дохода в стр.020 подр.1.3 Раздела 1 = 20, то стр.(040 + 050 + 060) подр.1.3 Раздела 1 = '") +
		НСтр("ru='стр.120 Листа 03 (Раздел А) с аналогичным кодом дохода, а стр.030 подр.1.3 Раздела 1 = '") + 
		ДанныеКС.КБК.Дивиденды_ОтРосКРос);
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 (%2 + %3 + %4) = %5 по коду дохода 20, %6 = '") + 
		ДанныеКС.КБК.Дивиденды_ОтРосКРос);
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные суммы налога или значение КБК'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните показатели в стр.030, 040, 050, 060 подр.1.3 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("КодДохода"		, "20");
	НастройкиКС.Вставить("КБК"				, ДанныеКС.КБК.Дивиденды_ОтРосКРос);
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Р13С001"		, "П00013М102003_");
	ИменаПоказателей.Вставить("Р13С002"		, "П00013М103003_");
	ИменаПоказателей.Вставить("Р13С003"		, "П00013М104003_"); 
	ИменаПоказателей.Вставить("Р13С004"		, "П00013М105003_"); 
	ИменаПоказателей.Вставить("Р13С005"		, "П00013М106003_"); 
	ИменаПоказателей.Вставить("Л3АС001"		, "П0003А0000600");
	ИменаПоказателей.Вставить("Л3АС002"		, "П0003А0012003");
	...
	Для каждого КодДоходаР13 Из ДанныеКС.МассивКодовР13 Цикл
		...
		НеверныйКБКЗначение = "";
		Показатели.Р13С003.Итог = 0;
		Показатели.Р13С004.Итог = 0;
		Показатели.Р13С005.Итог = 0;
		Показатели.Л3АС002.Итог = 0;
		...
		Для Счетчик = 1 По ДанныеКС.Раздел1_3.КолвоСтрокП00013М1 Цикл
			...
			Если Показатели.Р13С001.Значение = КодДоходаР13 Тогда
				...
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р13С003, ДанныеКС.Раздел1_3, Счетчик);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р13С004, ДанныеКС.Раздел1_3, Счетчик);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Р13С005, ДанныеКС.Раздел1_3, Счетчик);
				Если НЕ Показатели.Р13С002.Значение = НастройкиКС.КБК Тогда 
					НеверныйКБК = Истина;
					НеверныйКБКЗначение = Показатели.Р13С002.ЗначениеСтр; 
				КонецЕсли;
		...
		Для каждого Лист03А Из ДанныеКС.Лист03А Цикл
			...
			ЗаполнитьПоказатель(Показатели.Л3АС001, Лист03А);
			Если Показатели.Л3АС001.Значение = НастройкиКС.КодДохода Тогда
				РасшифроватьПоказатель(Показатели.Л3АС001);
				ЗаполнитьИРасшифроватьПоказатель(Показатели.Л3АС002, Лист03А);
			КонецЕсли;
		КонецЦикла;
		
		РасчетнаяСумма = Показатели.Р13С003.Итог + Показатели.Р13С004.Итог + Показатели.Р13С005.Итог;
		НастройкиКС.ЭтоОшибка = НеверныйКБК ИЛИ НЕ РасчетнаяСумма = Показатели.Л3АС002.Итог;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			РасчетнаяСумма, Показатели.Р13С003.Итог, Показатели.Р13С004.Итог, Показатели.Р13С005.Итог, 
			Показатели.Л3АС002.Итог, ?(НеверныйКБК, НеверныйКБКЗначение, НастройкиКС.КБК));
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_24_01()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.3 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='24.01'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Если код дохода в стр.020 подр.1.3 Раздела 1 = 21, то стр.070 подр.1.3 Раздела 1 = '") +
		НСтр("ru='стр.080 Листа 04 с аналогичным кодом дохода, а стр.030 подр.1.3 Раздела 1 = '") + 
		ДанныеКС.КБК.Дивиденды_ОтИнКРос);
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 по коду дохода 21, %3 = '") + 
		ДанныеКС.КБК.Дивиденды_ОтИнКРос);
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога или значение КБК'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните показатели в стр.030, 070 подр.1.3 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	...
	НастройкиКС.Вставить("КодДохода"		, "21");
...
&НаСервере
Процедура ПроверкаКС_25_01_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.3 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='25.01.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.070 подр.1.3 Раздела 1 = 0, если стр.(040 + 050 + 060) подр.1.3 Раздела 1 > 0. '") +
		НСтр("ru='КС не проверяется, если реквизит ""По месту нахождения"" Титульного листа = 351'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 (%3 + %4 + %5) > 0 по коду дохода %6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.070 подр.1.3 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	...
	НастройкиКС.Вставить("Стр070Ноль"		, Истина);
...
Процедура ПроверкаКС_25_01_код()
	...
	ИменаПоказателей.Вставить("Р13С003"		, "П00013М105003_");
	ИменаПоказателей.Вставить("Р13С004"		, "П00013М106003_");
	ИменаПоказателей.Вставить("Р13С005"		, "П00013М107003_");
	...
	Для каждого КодДоходаР13 Из ДанныеКС.МассивКодовР13 Цикл
		...
		УсловиеВыполнено = НЕ Показатели.Р00С001.Значение = "351";
		
		РасчетнаяСумма = Показатели.Р13С002.Итог + Показатели.Р13С003.Итог + Показатели.Р13С004.Итог;
		Если НастройкиКС.Стр070Ноль Тогда
			НастройкиКС.ЭтоОшибка = УсловиеВыполнено И РасчетнаяСумма > 0 И НЕ Показатели.Р13С005.Итог = 0;
		Иначе
			НастройкиКС.ЭтоОшибка = УсловиеВыполнено И Показатели.Р13С005.Итог > 0 И НЕ РасчетнаяСумма = 0;
		КонецЕсли;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Р13С005.Итог, РасчетнаяСумма, Показатели.Р13С002.Итог, Показатели.Р13С003.Итог, 
			Показатели.Р13С004.Итог, КодДоходаР13); 
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_25_01_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='подраздел 1.3 Раздела 1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='25.01.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.(040, 050, 060) подр.1.3 Раздела 1 = 0, если стр.070 подр.1.3 Раздела 1 > 0. '") +
		НСтр("ru='КС не проверяется, если реквизит ""По месту нахождения"" Титульного листа = 351'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%2 (%3 + %4 + %5) = 0, если %1 > 0 по коду дохода %6'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните суммы налога в стр.(040, 050, 060) подр.1.3 Раздела 1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	...
	НастройкиКС.Вставить("Стр070Ноль"		, Ложь);
...
&НаСервере
Процедура ПроверкаКС_26_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='26.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 8%, а в стр.160 Листа 02 = 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 01, стр.120 Листа 02 > 0, реквизит ""Номер документа"" не заполнен '") +
		НСтр("ru='и не заполнено Приложение 2. Для организаций без обособленных подразделений (не заполнено '") +
		НСтр("ru='Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 8, %2 = 17, если %3 = 01 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. КС не применяется в отношении'") +
		НСтр("ru='налогоплательщиков, для которых установлены повышенные ставки и личных фондов, применяющих '") + 
		НСтр("ru='ставку, установленную п.1.18 ст.284 НК РФ'")); 
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л02С001"		, "П000020015003");
	ИменаПоказателей.Вставить("Л02С002"		, "П000020016003");
	ИменаПоказателей.Вставить("Л02С003"		, "П000020000100");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0 И НЕ ЗначениеЗаполнено(Показатели.Л02С005.Значение)
			И НЕ ДанныеКС.РазделЗаполнен_Приложение2;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 8 ИЛИ Показатели.Л02С002.Значение <> 17);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
...
&НаСервере
Процедура ПроверкаКС_27_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='27.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, а в стр.160 Листа 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 02, 07 - 12, 14, 18 и стр.120 Листа 02 > 0. Для организаций без '") + 
		НСтр("ru='обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, %2 <> 17, если %3 = 02, 07 - 12, 14, 18 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 0 ИЛИ Показатели.Л02С002.Значение = 17);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
			Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
...
&НаСервере
Процедура ПроверкаКС_28_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='28.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, а в стр.160 Листа 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 06, стр.120 Листа 02 > 0 и стр.359 Листа 02 > 0. Для организаций без '") + 
		НСтр("ru='обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, %2 <> 17, если %3 = 06, %4 > 0 и %5 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	ИменаПоказателей.Вставить("Л02С003"		, "П000020000100");
	ИменаПоказателей.Вставить("Л02С004"		, "П000020012003");
	ИменаПоказателей.Вставить("Л02С005"		, "П000020035903");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0 И Показатели.Л02С005.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 0 ИЛИ Показатели.Л02С002.Значение = 17);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
...
&НаСервере
Процедура ПроверкаКС_29_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='29.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 02, 07 - 12, 14, 18 и стр.120 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 = 02, 07 - 12, 14, 18 и %3 > 0'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
			И Показатели.Л02С001.Значение <> 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_29_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='29.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.060 Приложения 5 к Листу 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 02, 07 - 12, 14, 18 и стр.120 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 17, если %2 = 02, 07 - 12, 14, 18 и %3 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.060 Приложения 5 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 5 к Листу 02'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С001.Значение) = Неопределено И
			Показатели.Л02С002.Значение > 0;
		...
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.П25С001.Значение, Показатели.Л02С001.Значение, Показатели.Л02С002.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(Лист02_5.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_30_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='30.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 06, стр.120 Листа 02 > 0 и стр.359 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 = 06, %3 > 0 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
			И Показатели.Л02С001.Значение <> 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, 
			Показатели.Л02С004.Значение, Показатели.Л02С005.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_30_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='30.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.060 Приложения 5 к Листу 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 06, стр.120 Листа 02 > 0 и стр.359 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 17, если %2 = 06, %3 > 0 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.060 Приложения 5 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 5 к Листу 02'"));
	...
	ИменаПоказателей.Вставить("Л02С001"		, "П000020000100");
	ИменаПоказателей.Вставить("Л02С002"		, "П000020012003");
	ИменаПоказателей.Вставить("Л02С003"		, "П000020035903");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		ЗаполнитьПоказатель(Показатели.Л02С001, Лист02);
		ЗаполнитьПоказатель(Показатели.Л02С002, Лист02);
		ЗаполнитьПоказатель(Показатели.Л02С003, Лист02);
		
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С001.Значение) = Неопределено И
			Показатели.Л02С002.Значение > 0 И Показатели.Л02С002.Значение > 0;
		...
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			РасшифроватьПоказатель(Показатели.Л02С001);
			РасшифроватьПоказатель(Показатели.Л02С002);
			РасшифроватьПоказатель(Показатели.Л02С003);
...
&НаСервере
Процедура ПроверкаКС_31_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='31.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 5%, а в стр.160 Листа 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 17 и стр.120 Листа 02 > 0. Для организаций без '") + 
		НСтр("ru='обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 5, %2 = 0, если %3 = 17 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 5 ИЛИ Показатели.Л02С002.Значение <> 0);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
			Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
...
&НаСервере
Процедура ПроверкаКС_32_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='32.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 5%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 17 и стр.120 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 5, если %2 = 17 и %3 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
			И Показатели.Л02С001.Значение <> 5;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_32_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='32.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.060 Приложения 5 к Листу 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 17 и стр.120 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 = 17 и %3 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.060 Приложения 5 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 5 к Листу 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
				И НЕ Показатели.П25С001.Значение = 0;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.П25С001.Значение, Показатели.Л02С001.Значение, Показатели.Л02С002.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(Лист02_5.НомерСтраницы));
...
&НаСервере
Процедура ПроверкаКС_33_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='33.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 25%, а в стр.160 Листа 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 04 и стр.120 Листа 02 > 0. Для организаций без '") + 
		НСтр("ru='обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 25, %2 = 0, если %3 = 04 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 25 ИЛИ Показатели.Л02С002.Значение <> 0);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
			Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
...
&НаСервере
Процедура ПроверкаКС_33_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='33.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 25%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 04 и стр.120 Листа 02 > 0. Для организаций с '") + 
		НСтр("ru='обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 25, если %2 = 04 и %3 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
			И Показатели.Л02С001.Значение <> 25;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_34_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='34.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 3%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 15 и стр.120 Листа 02 > 0'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 3, если %2 = 15 и %3 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И Показатели.Л02С001.Значение <> 3;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_35_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='35.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, а в стр.160 Листа 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 5, 7, 8 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций без обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, %2 <> 17, если %3 = 03, %4 = 5, 7, 8 и %5 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. КС не применяется в отношении'") +
		НСтр("ru='налогоплательщиков, применяющих общие ставки налогообложения'")); 
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НомерДокумента = Сред(Показатели.Л02С005.Значение, 1, 1);
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			НЕ МассивНомеров.Найти(НомерДокумента) = Неопределено И Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 0 ИЛИ Показатели.Л02С002.Значение = 17);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
			НомерДокумента, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_36_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='36.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 5, 7, 8 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций с обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 = 03, %3 = 5, 7, 8 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. КС не применяется в отношении '") +
...
&НаСервере
Процедура ПроверкаКС_36_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='36.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.060 Приложения 5 к Листу 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 5, 7, 8 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций с обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 17, если %2 = 03, %3 = 5, 7, 8 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.060 Приложения 5 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 5 к Листу 02. КС не применяется в '") +
		НСтр("ru='отношении налогоплательщиков, применяющих общие ставки налогообложения'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		ЗаполнитьПоказатель(Показатели.Л02С001, Лист02);
		ЗаполнитьПоказатель(Показатели.Л02С002, Лист02);
		ЗаполнитьПоказатель(Показатели.Л02С005, Лист02); 
		
		НомерДокумента = Сред(Показатели.Л02С005.Значение, 1, 1);
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С001.Значение) = Неопределено И
			НЕ МассивНомеров.Найти(НомерДокумента) = Неопределено И Показатели.Л02С002.Значение > 0;
		...
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			ЗаполнитьИРасшифроватьПоказатель(Показатели.П25С001, Лист02_5);
			РасшифроватьПоказатель(Показатели.Л02С001);
			РасшифроватьПоказатель(Показатели.Л02С005);
			РасшифроватьПоказатель(Показатели.Л02С002);
			
			НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
				И Показатели.П25С001.Значение = 17;
			НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
				Показатели.П25С001.Значение, Показатели.Л02С001.Значение, НомерДокумента, Показатели.Л02С002.Значение);
			НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
				НомерСтраницы(Лист02_5.НомерСтраницы));
			ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_37_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='37.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0% или 4%, а в стр.160 Листа 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 6 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций без обособленных подразделений (не заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, 4, %2 <> 17, если %3 = 03, %4 = 6 и %5 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные ставки налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставки налога в стр.150, 160 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. КС не применяется в отношении'") +
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			НЕ МассивНомеров.Найти(НомерДокумента) = Неопределено И Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 0 И Показатели.Л02С001.Значение <> 4 ИЛИ Показатели.Л02С002.Значение = 17);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.Значение, 
			НомерДокумента, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
...
&НаСервере
Процедура ПроверкаКС_38_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='38.02.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.150 Листа 02 = 0% или 4%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 6 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций с обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, 4, если %2 = 03, %3 = 6 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.150 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. КС не применяется в отношении'") +
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		УсловиеВыполнено = НЕ МассивПризнаков.Найти(Показатели.Л02С003.Значение) = Неопределено И
			НЕ МассивНомеров.Найти(НомерДокумента) = Неопределено И Показатели.Л02С004.Значение > 0;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
			И (Показатели.Л02С001.Значение <> 0 И Показатели.Л02С001.Значение <> 4);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С003.Значение, 
			НомерДокумента, Показатели.Л02С004.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
...
&НаСервере
Процедура ПроверкаКС_38_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='38.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Ставка налога в стр.060 Приложения 5 к Листу 02 <> 17%, если реквизит ""Признак '") +
		НСтр("ru='налогоплательщика"" (код) = 03, реквизит ""Номер документа"" = 6 и стр.120 Листа 02 > 0. '") + 
		НСтр("ru='Для организаций с обособленными подразделениями (заполнено Приложение 5 к Листу 02)'")); 
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 <> 17, если %2 = 03, %3 = 6 и %4 > 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная ставка налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните ставку налога в стр.060 Приложения 5 к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 5 к Листу 02. КС не применяется в '") +
...
&НаСервере
Процедура ПроверкаКС_39_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='39.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_40_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='40.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_41_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='41.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_42_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='42.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Расходы, уменьшающие сумму доходов от реализации в стр.030 Листа 02 = стр.130 '") +
		НСтр("ru='Приложения 2 к Листу 02 (для всех, кроме негосударственных пенсионных фондов)'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, 
...
&НаСервере
Процедура ПроверкаКС_43_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='43.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_44_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='44.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_45_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='45.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_46_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='46.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Налоговая база в стр.100 Листа 02 = стр.060 - стр.070 - стр.400 Приложения 2 к Листу 02 '") +
		НСтр("ru='+ стр.100 Листов 05 + стр.550 Листа 05.1 + стр.530 Листа 06 + стр.050 Листов 08 '") +
		НСтр("ru='- стр.010 Листа 04 с кодом дохода 37'"));
	...
	ИменаПоказателей.Вставить("Л51С001"		, "П000510055003");
	...
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Л51С001, ДанныеКС.Лист05_1);
	ЗаполнитьИРасшифроватьПоказатель(Показатели.Л06С001, ДанныеКС.Лист06);
	...
	Детализация2 = СтрШаблон(НастройкиКС.ШаблонДетализации2,
	    Показатели.Л02С002.Итог, -Показатели.Л02С003.Итог, -Показатели.П22С001.Итог, 
		Показатели.Л05С001.Итог, Показатели.Л51С001.Итог, Показатели.Л06С001.Итог, Показатели.Л08С001.Итог, 
		-Показатели.Л04С002.Итог);
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Л02С001.Итог, РасчетнаяСумма, Детализация2);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_47_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='47.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Если налоговая база в стр.100 Листа 02 <= 0, то стр.110, 120, 180, 190, 200, 270, 271, '") +
		НСтр("ru='290, 300, 310 Листа 02 = 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы и налога'"));
	Заголовки.Вставить("Рекомендации"		, 
		"Укажите значения в стр.110, 120, 180, 190, 200, 270, 271, 290, 300, 310 Листа 02 = 0");
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 01.07.2024 N СД-4-3/7429@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	ПодготовитьНастройки(Заголовки);
...
&НаСервере
Процедура ПроверкаКС_48_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='48.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.110 Листа 02 = стр.150 Приложения 4 к Листу 02 для отчета за 1 отчетный'") +
		НСтр("ru='период (1 квартал или 1 месяц) или отчетный налоговый период'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная 
			И (ДанныеКС.МесОтчета = 1 ИЛИ ДанныеКС.МесОтчета = 3 ИЛИ ДанныеКС.МесОтчета = 12) 
...
&НаСервере
Процедура ПроверкаКС_49_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='49.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 01.07.2024 N СД-4-3/7429@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. Кроме налогоплательщиков с реквизитом'") +
		НСтр("ru='""По месту нахождения"" Титульного листа = 331, 332, 333'"));	
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателейТЛ = Новый Структура;
	ИменаПоказателейТЛ.Вставить("Р00С001"		, "ПоМесту");

	Показатели = СформироватьПоказатели(ИменаПоказателей);
	ПоказателиТЛ = СформироватьПоказатели(ИменаПоказателейТЛ);
	Показатели.Л02С003.Инверсия = Истина;
	
	МассивКодов = СтрРазделить(
		"331,332,333", ",");
	
	ЗаполнитьПоказатель(ПоказателиТЛ.Р00С001, ДанныеКС.Титульный);
	УсловиеВыполнено = МассивКодов.Найти(ПоказателиТЛ.Р00С001.Значение) = Неопределено;
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		РасчетнаяСумма = СуммаПоказателей(Показатели) - Показатели.Л02С001.Значение;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено И РасчетнаяСумма >= 0 
			И НЕ Показатели.Л02С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
...
&НаСервере
Процедура ПроверкаКС_50_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='50.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Налоговая база в стр.120 Листа 02 = стр.100 - стр.110 + стр.370 Листа 02, но не менее стр.370 '") +
		НСтр("ru='Листа 02, если стр.370 > 0 и реквизит ""По месту нахождения"" Титульного листа = 331, 332, 333'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %5 (%2 - %3 + %4), %5 >= %4, если %6 = 331, 332, 333'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налоговой базы в стр.120 Листа 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02'"));
	...
	ИменаПоказателейЛ2 = Новый Структура;
	ИменаПоказателейЛ2.Вставить("Л02С001"		, "П000020012003");
	ИменаПоказателейЛ2.Вставить("Л02С002"		, "П000020010003");
	ИменаПоказателейЛ2.Вставить("Л02С003"		, "П000020011003");
	ИменаПоказателейЛ2.Вставить("Л02С004"		, "П000020037003");
	ИменаПоказателейТЛ = Новый Структура;
	ИменаПоказателейТЛ.Вставить("Р00С001"		, "ПоМесту");
	
	ПоказателиЛ2 = СформироватьПоказатели(ИменаПоказателейЛ2);
	ПоказателиТЛ = СформироватьПоказатели(ИменаПоказателейТЛ);
	ПоказателиЛ2.Л02С003.Инверсия = Истина;
	
	МассивКодов = СтрРазделить(
		"331,332,333", ",");
	
	ЗаполнитьПоказатель(ПоказателиТЛ.Р00С001, ДанныеКС.Титульный);
	УсловиеВыполнено = НЕ МассивКодов.Найти(ПоказателиТЛ.Р00С001.Значение) = Неопределено;
	
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
	
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатели(ПоказателиЛ2, Лист02);
		РасшифроватьПоказатель(ПоказателиТЛ.Р00С001);
		
		
		РасчетнаяСумма = СуммаПоказателей(ПоказателиЛ2) - ПоказателиЛ2.Л02С001.Значение;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено И ПоказателиЛ2.Л02С004.Значение > 0 
			И НЕ (ПоказателиЛ2.Л02С001.Значение = РасчетнаяСумма И РасчетнаяСумма >= ПоказателиЛ2.Л02С004.Значение);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			ПоказателиЛ2.Л02С001.Значение, ПоказателиЛ2.Л02С002.Значение, -ПоказателиЛ2.Л02С003.Значение, 
			ПоказателиЛ2.Л02С004.Значение, РасчетнаяСумма, ПоказателиТЛ.Р00С001.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_51_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='51.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_52_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='52.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_53_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='53.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма исчисленного налога в стр.200 Листа 02 = стр.(120 х 160) Листа 02 / 100, '") +
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		РасчетнаяСумма = Окр(Показатели.Л02С002.Значение * Показатели.Л02С004.Значение / 100);
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяБезОП И НЕ Показатели.Л02С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С004.Значение, РасчетнаяСумма);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_54_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='54.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_54_02_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_54_02_код()
...
&НаСервере
Процедура ПроверкаКС_55_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='55.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_55_02_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_55_02_код()
...
&НаСервере
Процедура ПроверкаКС_56_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='56.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_56_02_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_56_02_код()
...
&НаСервере
Процедура ПроверкаКС_57_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='57.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_58_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='58.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_59_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='59.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_60_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='60.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_61_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='61.02'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма уменьшения налога в федеральный бюджет в стр.268 Листа 02 = ∑ стр.050 всех '") +
		НСтр("ru='Приложений 7 к Листу 02 (раздел Г) + стр.170 Приложения 7 к Листу 02 (раздел Д). '") +
		НСтр("ru='КС проверяется, если в отчете не заполнено Приложение 2'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4)'"));
	...
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0017003");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П7ДС001, Лист02.Лист02_7Д);
		РасчетнаяСумма = Показатели.П7ГС001.Итог + Показатели.П7ДС001.Значение;
		
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И НЕ ДанныеКС.РазделЗаполнен_Приложение2 
			И НЕ Показатели.Л02С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л02С001.Значение, РасчетнаяСумма, Показатели.П7ГС001.Итог, Показатели.П7ДС001.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", Лист02.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_62_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='62.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И НЕ ДанныеКС.РазделЗаполнен_Приложение2
			И НЕ Показатели.Л02С001.Значение = Показатели.П7АС001.Итог;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
...
&НаСервере
Процедура ПроверкаКС_63_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='63.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_64_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='64.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_65_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='65.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	ПроверкаКС_65_02_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_65_02_код()
...
&НаСервере
Процедура ПроверкаКС_66_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='66.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_67_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='67.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_68_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='68.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_69_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='69.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_70_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='70.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_71_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='71.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_72_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='72.02'"));
	Заголовки.Вставить("Описание"			, 
	...
	Заголовки.Вставить("Основание"			, НСтр("ru='Письмо ФНС России от 01.07.2024 N СД-4-3/7429@'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 02. Кроме налогоплательщиков с реквизитом'") +
		НСтр("ru='""По месту нахождения"" Титульного листа = 331, 332, 333, а также в случаях, когда '") +
		НСтр("ru='налогоплательщик вправе не начислять авансовые платежи'"));
	ПодготовитьНастройки(Заголовки);
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено И ДанныеКС.МесОтчета = 3 И ДанныеКС.Периодичность = 3
...
&НаСервере
Процедура ПроверкаКС_73_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='73.03'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_74_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='74.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_75_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='75.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_76_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='76.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_77_02()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='77.02'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_78_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 1 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='78.02.1'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_79_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 1 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='79.02.1'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_80_02_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 1 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='80.02.1'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_81_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='81.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_82_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='82.02.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.047 Приложения 2 к Листу 02 = 0, если реквизит '") +  
		НСтр("ru='""По месту нахождения"" Титульного листа <> 331, 332, 333'"));  
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0, если %2 <> 331, 332, 333'"));
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		...
		НастройкиКС.ЭтоОшибка = УсловиеВыполнено И НЕ Показатели.П22С001.Значение = 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.П22С001.Значение, ПоказателиТЛ.Р00С001.ЗначениеСтр);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_83_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='83.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_84_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='84.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_85_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='85.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_86_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='86.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_87_02_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 2 к Листу 02'")); 
	Заголовки.Вставить("Критичность"		, -1);
	Заголовки.Вставить("НомерКС"			, НСтр("ru='87.02.2'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_88_02_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 3 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='88.02.3'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_89_02_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 3 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='89.02.3'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_90_02_3()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 3 к Листу 02'")); 
	Заголовки.Вставить("НомерКС"			, НСтр("ru='90.02.3'"));
	Заголовки.Вставить("Описание"			, 
...
&НаСервере
Процедура ПроверкаКС_91_02_5_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 5 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='91.02.5.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налоговой базы в стр.050 Приложения 5 к Листу 02 = стр.030 х стр.040 / 100, '") +
		НСтр("ru='если стр.030 Приложения 5 к Листу 02 > 0, стр.031 Приложения 5 к Листу 02 = 0 '") +
		НСтр("ru='и реквизит ""Расчет составлен"" <> 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %4 (%2 х %3 / 100)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	...
	ИменаПоказателей.Вставить("П25С005"		, "П000250000300");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			УсловиеВыполнено = Показатели.П25С002.Значение > 0 И Показатели.П25С004.Значение = 0 
				И НЕ Показатели.П25С005.Значение = "3";
...
&НаСервере
Процедура ПроверкаКС_91_02_5_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 5 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='91.02.5.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.070 Приложения 5 к Листу 02 = стр.050 х стр.060 / 100, '") +
		НСтр("ru='если стр.030 Приложения 5 к Листу 02 > 0, стр.051 Приложения 5 к Листу 02 = 0 '") +
		НСтр("ru='и реквизит ""Расчет составлен"" <> 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %4 (%2 х %3 / 100)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	...
	ИменаПоказателей.Вставить("П25С006"		, "П000250003003");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			УсловиеВыполнено = Показатели.П25С006.Значение > 0 И Показатели.П25С004.Значение = 0 
				И НЕ Показатели.П25С005.Значение = "3";
			РасчетнаяСумма = Окр(Показатели.П25С002.Значение * Показатели.П25С003.Значение / 100);
			НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И УсловиеВыполнено 
				И НЕ Показатели.П25С001.Значение = РасчетнаяСумма;
...
&НаСервере
Процедура ПроверкаКС_92_02_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 5 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='92.02.5'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.100 Приложения 5 к Листу 02 = стр.(070 - 080 - 090 - 097 - 098) '") +
		НСтр("ru='Приложения 5 к Листу 02, если стр.070 > стр.(080 + 090 + 097 + 098) Приложения 5 к Листу 02 '") +
		НСтр("ru='и реквизит ""Расчет составлен"" <> 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, 
		НСтр("ru='%1 = %7 (%2 - %3 - %4 - %5 - %6), если %2 > %8 (%3 + %4 + %5 + %6)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	...
	ПроверкаКС_92_02_5_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_92_02_5_код()
	...
	ИменаПоказателей.Вставить("П25С007"		, "П000250000300");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Лист02_5 Из Лист02.Лист02_5 Цикл
			...
			УсловиеВыполнено = НЕ Показатели.П25С007.Значение = "3";
			...
			Если НастройкиКС.БольшеНуля Тогда
				...
				НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловнаяСОП И Лист02_5.РазделЗаполнен_Лист02_5 И УсловиеВыполнено 
...
&НаСервере
Процедура ПроверкаКС_93_02_5()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 5 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='93.02.5'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.110 Приложения 5 к Листу 02 = стр.(080 + 090 + 097 + 098 - 070) '") +
		НСтр("ru='Приложения 5 к Листу 02, если стр.070 < стр.(080 + 090 + 097 + 098) Приложения 5 к Листу 02 '") +
		НСтр("ru='и реквизит ""Расчет составлен"" <> 3'"));
	Заголовки.Вставить("ШаблонДетализации"	, 
		НСтр("ru='%1 = %7 (%3 + %4 + %5 + %6 - %2), если %2 < %8 (%3 + %4 + %5 + %6)'"));
...
&НаСервере
Процедура ПроверкаКС_94_02_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 7 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='94.02.7'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.130 или стр.170 Приложения 7Д к Листу 02 = 0, '") +
		НСтр("ru='если реквизит ""Признак налогоплательщика"" Листа 02 <> 01 и количество Листов 02 = 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = 0 или %2 = 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные показатели отчета'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните значения показателей в стр.130, 170 Приложения 7Д к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 7Д к Листу 02'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0013003");
	ИменаПоказателей.Вставить("П7ДС002"		, "П0027Д0017003");
	ИменаПоказателей.Вставить("Л02С003"		, "П000020000100");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл 
		...
		ЗаполнитьПоказатель(Показатели.Л02С003, Лист02);
		УсловиеВыполнено = НЕ Показатели.Л02С003.Значение = "01" И ДанныеКС.КолвоСтраницЛист02 = 1;
		
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П7ДС001, Лист02.Лист02_7Д);
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П7ДС002, Лист02.Лист02_7Д);
		РасшифроватьПоказатель(Показатели.Л02С003);
		
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ (Показатели.П7ДС001.Значение = 0 ИЛИ Показатели.П7ДС002.Значение = 0);
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.П7ДС001.Значение, Показатели.П7ДС002.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			НомерСтраницы(Лист02.Лист02_7Д.НомерСтраницы));
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_96_02_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 7 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='96.02.7'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.120 Приложения 7Д к Листу 02 = стр.190 Листа 02 с реквизитом '") +
		НСтр("ru='""Признак налогоплательщика"" = 01'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2, если %3 = 01'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.120 Приложения 7Д к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 7Д к Листу 02'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0012003");
	ИменаПоказателей.Вставить("Л02С002"		, "П000020019003");
	ИменаПоказателей.Вставить("Л02С003"		, "П000020000100");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл 
		...
		УсловиеВыполнено = Показатели.Л02С003.Значение = "01" И ЗначениеЗаполнено(Лист02.Лист02_7Д.ПризнакВключения);
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.П7ДС001.Значение = Показатели.Л02С002.Значение;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.П7ДС001.Значение, Показатели.Л02С002.Значение, Показатели.Л02С003.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			НомерСтраницы(Лист02.Лист02_7Д.НомерСтраницы));
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_97_02_7()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 7 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='97.02.7'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.130 Приложения 7Д к Листу 02 = стр.(135 + 145 + 155 + 165) '") +
		НСтр("ru='Приложения 7Д к Листу 02'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4 + %5 + %6)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма расходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму расходов в стр.130 Приложения 7Д к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 7Д к Листу 02'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0013003");
	ИменаПоказателей.Вставить("П7ДС002"		, "П0027Д0013503");
	ИменаПоказателей.Вставить("П7ДС003"		, "П0027Д0014503");
	ИменаПоказателей.Вставить("П7ДС004"		, "П0027Д0015503");
	ИменаПоказателей.Вставить("П7ДС005"		, "П0027Д0016503");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл 
		...
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатели(Показатели, Лист02.Лист02_7Д);
		
		УсловиеВыполнено = ЗначениеЗаполнено(Лист02.Лист02_7Д.ПризнакВключения);
		РасчетнаяСумма = СуммаПоказателей(Показатели) - Показатели.П7ДС001.Значение;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.П7ДС001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.П7ДС001.Значение, РасчетнаяСумма, Показатели.П7ДС002.Значение, Показатели.П7ДС003.Значение,
			Показатели.П7ДС004.Значение, Показатели.П7ДС005.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			НомерСтраницы(Лист02.Лист02_7Д.НомерСтраницы));
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_99_02_7_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 7 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='99.02.7.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма в стр.200 Приложения 7Д к Листу 02 = стр.(100 + 130 - 170) '") +
		НСтр("ru='Приложения 7Д к Листу 02'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4 - %5)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный показатель отчета'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму в стр.200 Приложения 7Д к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Приложения 7Д к Листу 02'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0020003");
	ИменаПоказателей.Вставить("П7ДС002"		, "П0027Д0010003");
	ИменаПоказателей.Вставить("П7ДС003"		, "П0027Д0013003");
	ИменаПоказателей.Вставить("П7ДС004"		, "П0027Д0017003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Показатели.П7ДС004.Инверсия = Истина;
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл 
...
&НаСервере
Процедура ПроверкаКС_99_02_7_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Приложение 7 к Листу 02'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='99.02.7.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма в стр.200 Приложения 7Д к Листу 02 = сумме подчиненных строк'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочный показатель отчета'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму в стр.200 Приложения 7Д к Листу 02");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("П7ДС001"		, "П0027Д0020003");
	ИменаПоказателей.Вставить("П7ДС002"		, "П0027ДМ200003_");
	ИменаПоказателей.Вставить("П7ДС003"		, "П0027ДМ200001_");
	ИменаПоказателей.Вставить("П7ДС004"		, "П0027ДМ200002_");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		
		МассивРасшифровки = Новый Массив;
		Показатели.П7ДС002.Итог = 0;
		
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатель(Показатели.П7ДС001, Лист02.Лист02_7Д);
		
		Для Счетчик = 1 По Лист02.Лист02_7Д.КолвоСтрокП0027ДМ2 Цикл
			
			ЗаполнитьПоказатель(Показатели.П7ДС002, Лист02.Лист02_7Д, Счетчик); 
			ЗаполнитьПоказатель(Показатели.П7ДС003, Лист02.Лист02_7Д, Счетчик); 
			ЗаполнитьПоказатель(Показатели.П7ДС004, Лист02.Лист02_7Д, Счетчик);
			
			Показатели.П7ДС002.Представление = "Строка " + Показатели.П7ДС004.ЗначениеСтр + 
				" Приложения 7 к Листу 02 (Раздел Д) (за " + Показатели.П7ДС003.ЗначениеСтр +
				" год)";
			РасшифроватьПоказатель(Показатели.П7ДС002);
			МассивРасшифровки.Добавить(Показатели.П7ДС002.Значение);
		
		КонецЦикла;
		
		СтрокаРасшифровки = СтрСоединить(МассивРасшифровки, " + ");
		УсловиеВыполнено = ЗначениеЗаполнено(Лист02.Лист02_7Д.ПризнакВключения);
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.П7ДС001.Значение = Показатели.П7ДС002.Итог;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.П7ДС001.Значение, Показатели.П7ДС002.Итог, СтрокаРасшифровки);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			НомерСтраницы(Лист02.Лист02_7Д.НомерСтраницы));
		ДополнитьТаблицуРезультата();
...
#Область КС_Проверки_Листа_04

&НаСервере
Процедура ПроверкаКС_102_04_1()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 04'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='102.04.1'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.080 Листа 04 = стр.(040 - 050 - 060 - 070) Листа 04'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4 - %5 - %6)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.080 Листа 04");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 04'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л04С001"		, "П000040008003");
	ИменаПоказателей.Вставить("Л04С002"		, "П000040004003");
	ИменаПоказателей.Вставить("Л04С003"		, "П000040005003");
	ИменаПоказателей.Вставить("Л04С004"		, "П000040006003");
	ИменаПоказателей.Вставить("Л04С005"		, "П000040007003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Показатели.Л04С003.Инверсия = Истина;
	Показатели.Л04С004.Инверсия = Истина;
	Показатели.Л04С005.Инверсия = Истина;
	
	Для каждого Лист04 Из ДанныеКС.Лист04 Цикл 
		
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатели(Показатели, Лист04);
		
		УсловиеВыполнено = ЗначениеЗаполнено(Лист04.ПризнакВключения); 
		РасчетнаяСумма = СуммаПоказателей(Показатели) - Показатели.Л04С001.Значение;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.Л04С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л04С001.Значение, РасчетнаяСумма, Показатели.Л04С002.Значение, 
			-Показатели.Л04С003.Значение, -Показатели.Л04С004.Значение, -Показатели.Л04С005.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист04.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_102_04_2()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 04'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='102.04.2'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.080 Листа 04 >= 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.080 Листа 04");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	...
	Для каждого Лист04 Из ДанныеКС.Лист04 Цикл 
		...
		УсловиеВыполнено = ЗначениеЗаполнено(Лист04.ПризнакВключения); 
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И Показатели.Л04С001.Значение < 0;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л04С001.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист04.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
#Область КС_Проверки_Листа_05

&НаСервере
Процедура ПроверкаКС_103_05()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='103.05'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма доходов в стр.010 Листа 05 >= стр.(011 + 012 + 013 + 014 + 015) Листа 05'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3 + %4 + %5 + %6 + %7)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма доходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму доходов в стр.010 Листа 05");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л05С001"		, "П000050001003");
	ИменаПоказателей.Вставить("Л05С002"		, "П000050001103");
	ИменаПоказателей.Вставить("Л05С003"		, "П000050001203");
	ИменаПоказателей.Вставить("Л05С004"		, "П000050001303");
	ИменаПоказателей.Вставить("Л05С005"		, "П000050001403");
	ИменаПоказателей.Вставить("Л05С006"		, "П000050001503");
	...
	Для каждого Лист05 Из ДанныеКС.Лист05 Цикл 
		...
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатели(Показатели, Лист05);
		
		УсловиеВыполнено = ЗначениеЗаполнено(Лист05.ПризнакВключения); 
		РасчетнаяСумма = СуммаПоказателей(Показатели) - Показатели.Л05С001.Значение;
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И Показатели.Л05С001.Значение < РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л05С001.Значение, РасчетнаяСумма, Показатели.Л05С002.Значение, 
			Показатели.Л05С003.Значение, Показатели.Л05С004.Значение, Показатели.Л05С005.Значение,
			Показатели.Л05С006.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист05.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_104_05()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='104.05'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.020 Листа 05 >= стр.(021 + 022 + 023 + 024 + 025) Листа 05'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3 + %4 + %5 + %6 + %7)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма расходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму расходов в стр.020 Листа 05");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л05С001"		, "П000050002003");
	ИменаПоказателей.Вставить("Л05С002"		, "П000050002103");
	ИменаПоказателей.Вставить("Л05С003"		, "П000050002203");
	ИменаПоказателей.Вставить("Л05С004"		, "П000050002303");
	ИменаПоказателей.Вставить("Л05С005"		, "П000050002403");
	ИменаПоказателей.Вставить("Л05С006"		, "П000050002503");
...
&НаСервере
Процедура ПроверкаКС_105_05()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='105.05'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налоговой базы в стр.060 Листа 05 = стр.(040 - 050) Листа 05, но не менее 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4) >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налоговой базы в стр.060 Листа 05");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000050006003");
	НастройкиКС.Вставить("Показатель2"		, "П000050004003");
	НастройкиКС.Вставить("Показатель3"		, "П000050005003");
	НастройкиКС.Вставить("БольшеНуля"		, Истина);
	
	ПроверкаКС_105_05_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_105_05_код()
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л05С001"		, НастройкиКС.Показатель1);
	ИменаПоказателей.Вставить("Л05С002"		, НастройкиКС.Показатель2);
	ИменаПоказателей.Вставить("Л05С003"		, НастройкиКС.Показатель3);
	...
	Для каждого Лист05 Из ДанныеКС.Лист05 Цикл 
		...
		УсловиеВыполнено = ЗначениеЗаполнено(Лист05.ПризнакВключения); 
		РасчетнаяСумма = Показатели.Л05С002.Значение - Показатели.Л05С003.Значение;
		РасчетнаяСумма = ?(НастройкиКС.БольшеНуля, Макс(0, РасчетнаяСумма), РасчетнаяСумма);
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.Л05С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л05С001.Значение, РасчетнаяСумма, Показатели.Л05С002.Значение, 
			Показатели.Л05С003.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист05.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_106_05()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='106.05'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма прибыли / убытка в стр.040 Листа 05 = стр.(010 - 020) Листа 05'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма прибыли / убытка'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли / убытка в стр.040 Листа 05");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000050004003");
	НастройкиКС.Вставить("Показатель2"		, "П000050001003");
	НастройкиКС.Вставить("Показатель3"		, "П000050002003");
	НастройкиКС.Вставить("БольшеНуля"		, Ложь);
...
&НаСервере
Процедура ПроверкаКС_107_05()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='107.05'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налоговой базы в стр.100 Листа 05 = стр.(060 - 080) Листа 05, но не менее 0'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4) >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налоговой базы в стр.100 Листа 05");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель1"		, "П000050010003");
	НастройкиКС.Вставить("Показатель2"		, "П000050006003");
	НастройкиКС.Вставить("Показатель3"		, "П000050008003");
	НастройкиКС.Вставить("БольшеНуля"		, Истина);
...
#Область КС_Проверки_Листа_051

&НаСервере
Процедура ПроверкаКС_108_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='108.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма доходов в стр.100 Листа 05.1 >= стр.(105 + 110 + 115 + 120 + 125) Листа 05.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма доходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму доходов в стр.100 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "П000510010003");
	НастройкиКС.Вставить("Показатели"		, "105,110,115,120,125");
	
	ПроверкаКС_108_51_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_108_51_код()
	
	ИменаПоказателей1 = Новый Структура;
	ИменаПоказателей1.Вставить("Л51С001"		, НастройкиКС.Показатель);
	Показатели1 = СформироватьПоказатели(ИменаПоказателей1);
	
	ИменаПоказателей2 = Новый Структура;
	МассивСтрок = СтрРазделить(
		НастройкиКС.Показатели, ",");
	Для Счетчик = 0 По МассивСтрок.ВГраница() Цикл
		ИменаПоказателей2.Вставить("Л51С00" + (Счетчик + 1), "П0005100" + МассивСтрок[Счетчик] + "03");		
	КонецЦикла;
	Показатели2 = СформироватьПоказатели(ИменаПоказателей2);
		
	ЗарегистрироватьПроверку();
	ЗаполнитьИРасшифроватьПоказатели(Показатели1, ДанныеКС.Лист05_1);
	ЗаполнитьИРасшифроватьПоказатели(Показатели2, ДанныеКС.Лист05_1);
	
	МассивРасшифровки = Новый Массив;
	Для каждого ЭлементСтруктуры Из Показатели2 Цикл
	    МассивРасшифровки.Добавить(ЭлементСтруктуры.Значение.Значение);
	КонецЦикла;
	
	УсловиеВыполнено = ЗначениеЗаполнено(ДанныеКС.Лист05_1.ПризнакВключения); 
	РасчетнаяСумма = СуммаПоказателей(Показатели2);
	НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
		И Показатели1.Л51С001.Значение < РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели1.Л51С001.Значение, РасчетнаяСумма, 
		СтрСоединить(
			МассивРасшифровки, " + ")); 
	НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
		ДанныеКС.Лист05_1.НомерСтраницы);
	ДополнитьТаблицуРезультата();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_109_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='109.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма доходов в стр.200 Листа 05.1 >= стр.(205 + 210) Листа 05.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма доходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму доходов в стр.200 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "П000510020003");
	НастройкиКС.Вставить("Показатели"		, "205,210");
...
&НаСервере
Процедура ПроверкаКС_110_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='110.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.300 Листа 05.1 >= стр.(305 + 310 + 315 + 320 + 325 + 330) Листа 05.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма расходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму расходов в стр.300 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "П000510030003");
	НастройкиКС.Вставить("Показатели"		, "305,310,315,320,325,330");
...
&НаСервере
Процедура ПроверкаКС_111_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='109.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма расходов в стр.400 Листа 05.1 >= стр.(405 + 410 + 415 + 420 + 425) Листа 05.1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 >= %2 (%3)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма расходов'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму расходов в стр.400 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "П000510040003");
	НастройкиКС.Вставить("Показатели"		, "405,410,415,420,425");
...
&НаСервере
Процедура ПроверкаКС_112_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='112.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма прибыли в стр.500 Листа 05.1 = стр.(100 + 200 - 300 - 400) Листа 05.1, '") +  
		НСтр("ru='если результат >= 0'"));  
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4 - %5 - %6) >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма прибыли'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму прибыли в стр.500 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "500");
	
	ПроверкаКС_112_51_код();
	
КонецПроцедуры

&НаСервере
Процедура ПроверкаКС_112_51_код()
	
	ИмяПоказателя = ?(НастройкиКС.Показатель = "500", "П000510050003", "П000510052003");
	
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л51С001"	, ИмяПоказателя);
	ИменаПоказателей.Вставить("Л51С002"	, "П000510010003");
	ИменаПоказателей.Вставить("Л51С003"	, "П000510020003");
	ИменаПоказателей.Вставить("Л51С004"	, "П000510030003");
	ИменаПоказателей.Вставить("Л51С005"	, "П000510040003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
		
	ЗарегистрироватьПроверку();
	ЗаполнитьИРасшифроватьПоказатели(Показатели, ДанныеКС.Лист05_1);
	
	УсловиеВыполнено = ЗначениеЗаполнено(ДанныеКС.Лист05_1.ПризнакВключения); 
	РасчетнаяСумма = Показатели.Л51С002.Значение + Показатели.Л51С003.Значение - Показатели.Л51С004.Значение - 
		Показатели.Л51С005.Значение;
		
	Если НастройкиКС.Показатель = "500" Тогда
		РасчетнаяСумма = Макс(0, РасчетнаяСумма);		
	Иначе
		РасчетнаяСумма = - Мин(0, РасчетнаяСумма);		
	КонецЕсли; 
		
	НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
		И НЕ Показатели.Л51С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Л51С001.Значение, РасчетнаяСумма, Показатели.Л51С002.Значение, Показатели.Л51С003.Значение, 
		Показатели.Л51С004.Значение, Показатели.Л51С005.Значение); 
	НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
		ДанныеКС.Лист05_1.НомерСтраницы);
	ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_113_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='113.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.520 Листа 05.1 = стр.(100 + 200 - 300 - 400) Листа 05.1, '") +  
		НСтр("ru='если результат <= 0'"));  
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 + %4 - %5 - %6) <= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма убытка'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму убытка в стр.520 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	НастройкиКС.Вставить("Показатель"		, "520");
...
&НаСервере
Процедура ПроверкаКС_114_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='114.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма убытка в стр.530 Листа 05.1 = стр.150 Приложения 4 к Листу 02 с реквизитом , '") +  
		НСтр("ru='""Признак налогоплательщика"" = 51 (для отчетов за 1 квартал и год)'"));  
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма убытка'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму убытка в стр.530 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л51С001"		, "П000510053003");
	ИменаПоказателей.Вставить("П24С001"		, "П000240015003");
	ИменаПоказателей.Вставить("П24С002"		, "П000240000100");
	...
	Для каждого Лист02 Из ДанныеКС.Лист02 Цикл
		Для каждого Приложение4 Из Лист02.Лист02_4 Цикл
			ЗаполнитьПоказатель(Показатели.П24С002, Приложение4);
			Если Показатели.П24С002.Значение = "51" Тогда 
				ЗаполнитьИРасшифроватьПоказатель(Показатели.П24С001, Приложение4);
				КодНайден = Истина;
			КонецЕсли;
		КонецЦикла;
	...
	УсловиеВыполнено = ЗначениеЗаполнено(ДанныеКС.Лист05_1.ПризнакВключения) И КодНайден
		И (ДанныеКС.МесОтчета = 3 ИЛИ ДанныеКС.МесОтчета = 12); 
	НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
		И НЕ Показатели.Л51С001.Значение = Показатели.П24С001.Итог;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Л51С001.Значение, Показатели.П24С001.Итог); 
	НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
		ДанныеКС.Лист05_1.НомерСтраницы);
	ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_115_51()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 05.1'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='115.51'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налоговой базы в стр.550 Листа 05.1 = стр.(500 - 530) Листа 05.1, но не менее 0'"));  
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 - %4) >= 0'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налоговой базы'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налоговой базы в стр.550 Листа 05.1");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 05.1'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л51С001"	, "П000510055003");
	ИменаПоказателей.Вставить("Л51С002"	, "П000510050003");
	ИменаПоказателей.Вставить("Л51С003"	, "П000510053003");
	...
	УсловиеВыполнено = ЗначениеЗаполнено(ДанныеКС.Лист05_1.ПризнакВключения); 
	РасчетнаяСумма = Макс(0, Показатели.Л51С002.Значение - Показатели.Л51С003.Значение);
	НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
		И НЕ Показатели.Л51С001.Значение = РасчетнаяСумма;
	НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
		Показатели.Л51С001.Значение, РасчетнаяСумма, Показатели.Л51С002.Значение, Показатели.Л51С003.Значение); 
...
#Область КС_Проверки_Листа_08

&НаСервере
Процедура ПроверкаКС_116_08()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 08'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='116.08'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Сумма налога в стр.080 Листа 08 = стр.(060 х 070 / 100) Листа 08'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='%1 = %2 (%3 х %4 / 100)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочная сумма налога'"));
	Заголовки.Вставить("Рекомендации"		, "Уточните сумму налога в стр.080 Листа 08");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 08'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л08С001"		, "П000800008003");
	ИменаПоказателей.Вставить("Л08С002"		, "П000800006003");
	ИменаПоказателей.Вставить("Л08С003"		, "П000800007003");
	
	Показатели = СформироватьПоказатели(ИменаПоказателей);
	Показатели.Л08С003.Округление = Ложь;
	
	Для каждого Лист08 Из ДанныеКС.Лист08 Цикл 
		
		ЗарегистрироватьПроверку();
		ЗаполнитьИРасшифроватьПоказатели(Показатели, Лист08);
		
		УсловиеВыполнено = ЗначениеЗаполнено(Лист08.ПризнакВключения); 
		РасчетнаяСумма = Окр(Показатели.Л08С002.Значение * Показатели.Л08С003.Значение / 100);
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ Показатели.Л08С001.Значение = РасчетнаяСумма;
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л08С001.Значение, РасчетнаяСумма, Показатели.Л08С002.Значение, 
			Показатели.Л08С003.Значение);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист08.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
&НаСервере
Процедура ПроверкаКС_117_08()
	
	Заголовки = Новый Структура;
	Заголовки.Вставить("Раздел"				, НСтр("ru='Лист 08'"));
	Заголовки.Вставить("НомерКС"			, НСтр("ru='117.08'"));
	Заголовки.Вставить("Описание"			, 
		НСтр("ru='Строки 090, 095 Листа 08 должны быть заполнены, если стр.085 Листа 08 = 1'"));
	Заголовки.Вставить("ШаблонДетализации"	, НСтр("ru='Значения %1, %2 заполнены, если %3 = 1'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Возможно ошибочные показатели отчета'"));
	Заголовки.Вставить("Рекомендации"		, "Заполните показатели в стр.090, 095 Листа 08");
	Заголовки.Вставить("Основание"			, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru='Страница %1 Листа 08'"));
	ПодготовитьНастройки(Заголовки);
	...
	ИменаПоказателей = Новый Структура;
	ИменаПоказателей.Вставить("Л08С001"		, "П000800009003");
	ИменаПоказателей.Вставить("Л08С002"		, "П000800009503");
	ИменаПоказателей.Вставить("Л08С003"		, "П000800008503");
	...
	Для каждого Лист08 Из ДанныеКС.Лист08 Цикл 
		...
		УсловиеВыполнено = ЗначениеЗаполнено(Лист08.ПризнакВключения) И Показатели.Л08С003.Значение = "1"; 
		НастройкиКС.ЭтоОшибка = ДанныеКС.ЭтоГоловная И УсловиеВыполнено 
			И НЕ (ЗначениеЗаполнено(Показатели.Л08С001.Значение) И ЗначениеЗаполнено(Показатели.Л08С002.Значение));
		НастройкиКС.Детализация = СтрШаблон(НастройкиКС.ШаблонДетализации, 
			Показатели.Л08С001.ЗначениеСтр, Показатели.Л08С002.Значение, 
			Показатели.Л08С003.ЗначениеСтр);
		НастройкиКС.ДопСведения = СтрЗаменить(НастройкиКС.ШаблонДопСведений, "%1", 
			Лист08.НомерСтраницы);
		ДополнитьТаблицуРезультата();
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "Раздел1_1" ИЛИ Ячейка.Раздел = "Раздел1_2" ИЛИ Ячейка.Раздел = "Раздел1_3"
		ИЛИ Ячейка.Раздел = "Лист03Б" ИЛИ Ячейка.Раздел = "Лист05_1" ИЛИ Ячейка.Раздел = "Лист06"
		ИЛИ Ячейка.Раздел = "Лист07" ИЛИ Ячейка.Раздел = "Приложение1" Тогда
	...
	ИначеЕсли Ячейка.Раздел = "Лист02" ИЛИ Ячейка.Раздел = "Лист03А" ИЛИ Ячейка.Раздел = "Лист04"
		ИЛИ Ячейка.Раздел = "Лист05" ИЛИ Ячейка.Раздел = "Лист08" ИЛИ Ячейка.Раздел = "Лист09"
