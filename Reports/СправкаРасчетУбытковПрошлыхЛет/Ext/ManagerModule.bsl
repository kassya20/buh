...
Процедура ДобавитьПримечаниеЛимитУменьшенияПрибыли(Результат, Контекст)
	...
	НачалоПериодаОграничения = НалоговыйУчет.ПериодНачалаОграниченияНаПереносУбытковПрошлыхЛетПоДолеПрибыли();
	КонецПериодаОграничения = КонецГода(НалоговыйУчет.ПериодПродленияОграниченияНаПереносУбытковПрошлыхЛетПоДолеПрибыли());
	
	ФорматДаты = "ДЛФ=DD";
	
	ТекстПримечания = СтрШаблон(
		НСтр("ru = '""В отчетные (налоговые) периоды с %1 по %2 налоговая база по налогу за текущий отчетный (налоговый) период ... не может быть уменьшена на сумму убытков, полученных в предыдущих налоговых периодах, более чем на 50 процентов"" (п. 2.1 ст. 283 НК РФ)'"),
		Формат(НачалоПериодаОграничения, ФорматДаты),
		Формат(КонецПериодаОграничения, ФорматДаты));
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	ГруппировкаПодразделение = БухгалтерскиеОтчеты.НайтиПоИмени(КомпоновщикНастроек.Настройки.Структура, "ПодразделениеУбытков");
	Таблица                  = БухгалтерскиеОтчеты.НайтиПоИмени(ГруппировкаПодразделение.Структура, "Убытки");
...
Функция ПолучитьНаборПоказателей() Экспорт
	...
	Возврат ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("НУ");
