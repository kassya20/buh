...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ИначеЕсли КлючВарианта = "НаправлениеДляПостановкиНаВоинскийУчет" Тогда
		
		// Параметры документа
		ДокументРезультат.ТолькоПросмотр = Истина;
		ДокументРезультат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_НаправлениеДляПостановкиНаВоинскийУчет";
		ДокументРезультат.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
		...
		ВывестиМакетНаправлениеДляПостановкиНаВоинскийУчет(ДокументРезультат, РезультатКомпоновки, ДатаОтчета, НастройкиОтчета);
...
#Область НаправлениеДляПостановкиНаВоинскийУчет

Функция ВывестиМакетНаправлениеДляПостановкиНаВоинскийУчет(ДокументРезультат, РезультатКомпоновки, ДатаОтчета, НастройкиОтчета)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Отчет.ВоинскийУчетОбщий.ПФ_MXL_НаправлениеДляПостановкиНаВоинскийУчет");
	ЛицеваяСторона = Макет.ПолучитьОбласть("ЛицеваяСторона");
	ОборотнаяСторона = Макет.ПолучитьОбласть("ОборотнаяСторона");
	...
	Для Каждого ТекСтрока Из ДанныеОтчета.Строки Цикл 
		...
		ПараметрыМакета = ПараметрыЗаголовкаСтруктура(ДатаОтчета, ТекСтрока.Организация);
		
		ДанныеПользовательскихПолейСсылки = ЗарплатаКадрыОтчеты.ЗначенияЗаполненияПользовательскихПолей(ИдентификаторыМакета, ТекСтрока);
		ЗарплатаКадрыОтчеты.ЗаполнитьПараметрыОбластиМакета(ЛицеваяСторона, ПараметрыМакета, ТекСтрока, ДанныеПользовательскихПолейСсылки);
		
		ЛицеваяСторона.Параметры.ДатаНаправления = ЗарплатаКадрыОтчеты.ФорматДатыЧислоВКавычкахМесяцПрописью(ДатаОтчета);
		
		Если ТолькоАдресПоПрописке Тогда 
			ЛицеваяСторона.Параметры.АдресМестаПроживанияПредставление = ТекСтрока.АдресПоПропискеПредставление;
		КонецЕсли;
		...
		Для Каждого ДанныеТрудовогоДоговора Из КадровыеДанныеСотрудниковПринятыхНаРаботуПоДоговорам Цикл 
			...
			Для Каждого ДанныеСобытия Из ДанныеСотрудника Цикл 
				Если ПринятыеНаРаботуПоДоговорам.Найти(ДанныеСобытия.Регистратор, "Регистратор") <> Неопределено Тогда
					Если Не ЗначениеЗаполнено(ДанныеТрудовогоДоговора.ПриказОПриемеВидДоговора) Тогда
