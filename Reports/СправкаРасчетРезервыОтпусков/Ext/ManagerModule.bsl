...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	ЧастиЗаголовка.Добавить(НСтр("ru = 'Справка-расчет резервов отпусков'"));
	ЧастиЗаголовка.Добавить(ПредставлениеПериода);
	
	Если ЗначениеЗаполнено(ПараметрыОтчета.НазваниеНабораПоказателейОтчета) Тогда
		ОписаниеНабораПоказателей =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '(%1)'"), ПараметрыОтчета.НазваниеНабораПоказателейОтчета);
		ЧастиЗаголовка.Добавить(ОписаниеНабораПоказателей);
	КонецЕсли;
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Таблица = БухгалтерскиеОтчеты.НайтиПоИмени(КомпоновщикНастроек.Настройки.Структура, "РезервыОтпусков");
	...
	Если ПараметрыОтчета.ПоказательНУ Тогда
		...
		Для Каждого ИмяГруппировки Из МассивНазванийГруппировок Цикл
			МассивГруппировок.Добавить(
				БухгалтерскиеОтчеты.НайтиПоИмени(Таблица.Строки, ИмяГруппировки + "НУ" + СуффиксГруппировки));
		КонецЦикла;
	...
	ИначеЕсли ПараметрыОтчета.ПоказательВР Тогда
		...
		Для Каждого ИмяГруппировки Из МассивНазванийГруппировок Цикл
			МассивГруппировок.Добавить(
				БухгалтерскиеОтчеты.НайтиПоИмени(Таблица.Строки, ИмяГруппировки + "СРазницами" + СуффиксГруппировки));
		КонецЦикла;
	Иначе
		Если Инвентаризация Тогда
			СуффиксГруппировки = "МО";
		...
		Для Каждого ИмяГруппировки Из МассивНазванийГруппировок Цикл
			МассивГруппировок.Добавить(
				БухгалтерскиеОтчеты.НайтиПоИмени(Таблица.Строки, ИмяГруппировки + "БУ" + СуффиксГруппировки));
		КонецЦикла;
	КонецЕсли;
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ЭлементТела = БухгалтерскиеОтчеты.ПодобратьЭлементыИзТелаМакета(
						МакетКомпоновки, "ГруппировкаФизическоеЛицо", ПараметрыПоиска);
