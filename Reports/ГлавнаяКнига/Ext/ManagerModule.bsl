...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	СтруктураНастроекОтчета.Вставить("ВидПериода" , ПараметрыОтчета.ВидПериода);
...
Функция СформироватьЗаголовок(ПараметрыОтчета)
	...
	ОписаниеПериода = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(
		ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода, Ложь, ПараметрыОтчета.ВидПериода);
...
Функция ВыполнитьОбщийЗапрос(ПараметрыОтчета)
	...
	Запрос.УстановитьПараметр("НачалоПериода", БухгалтерскиеОтчеты.НачалоПериодаСУчетомВидаПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.ВидПериода));
	...
	Если ПараметрыОтчета.КонецПериода <> '00010101000000' Тогда
		Запрос.УстановитьПараметр("КонецПериода", БухгалтерскиеОтчеты.КонецПериодаСУчетомВидаПериода(КонецДня(ПараметрыОтчета.КонецПериода), ПараметрыОтчета.ВидПериода));
	Иначе
