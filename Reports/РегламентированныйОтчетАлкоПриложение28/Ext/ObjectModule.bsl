...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил28Декларация" Тогда
...
Процедура ПроверитьЗаполнениеДекларации(СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		// Итог считается автоматически, поскольку в макете СоставПоказателей поле П000010000310 таблицы
		// имеет установленный признак подсчета итогов.
		СуммаПоКодам = 0;
		...
		ЗаполненыДанныеВиноградника = (П000000000002 + П000000000004 + П000000000003
										+ П000000000005 + П000000000006 + П000000000007
										+ П000000000008 > 0);
...
Процедура ПроверитьЗаполнениеТаблицыДекларация(СтатусПроверки, ИдГруппы,
	...
	ИмяКодПродукции  		= ИдГруппы + "08";
	ИмяОбъемПроизводства 	= ИдГруппы + "10";
...
Функция ЗаполнитьПоВсейОрганизацииДекларация(
	...
	|ВЫБРАТЬ
	|АлкоПрил28Декларация.П000010000308 КАК П000010000308,
	|АлкоПрил28Декларация.П000010000309 КАК П000010000309,
	|
	|СУММА(АлкоПрил28Декларация.П000010000310) КАК П000010000310
	|
	|ИЗ
	|	
	|(ВЫБРАТЬ
	|РегистрСведенийАлкоПрил28Декларация.*
	|
	|ИЗ
	|РегистрСведений.СведенияРеглОтчетАлкоПрил28Декларация КАК РегистрСведенийАлкоПрил28Декларация
	|
	|ГДЕ (РегистрСведенийАлкоПрил28Декларация.ИДДокИндСтраницы В (&МассивИДДокИндСтраницы)) 
	|И (РегистрСведенийАлкоПрил28Декларация.Активно = Истина)) КАК АлкоПрил28Декларация
	|	
    |СГРУППИРОВАТЬ ПО 
	|АлкоПрил28Декларация.П000010000308, 
	|АлкоПрил28Декларация.П000010000309
	|
	|УПОРЯДОЧИТЬ ПО
	|АлкоПрил28Декларация.П000010000308 ВОЗР
	|";
	
	ЗапросДекларация = Новый Запрос;
	ЗапросДекларация.Текст = ТекстЗапросаДекларация;
	
	СервисРегистровДекларация = СтруктураСервисРегистров["СведенияРеглОтчетАлкоПрил28Декларация"];
...
Процедура ЗаполнитьДвижения(ИдГруппы, НомерЛиста, ИдОП)
	...
	|	РегистрСведенийАлкоПрил28Декларация.*
	|ИЗ
	|	РегистрСведений." + ИмяРегистраСведений + " КАК РегистрСведенийАлкоПрил28Декларация
	|ГДЕ
	|	РегистрСведенийАлкоПрил28Декларация.ИДДокИндСтраницы = &ИДДокИндСтраницы
	|   И РегистрСведенийАлкоПрил28Декларация.Активно = Истина
	|	И РегистрСведенийАлкоПрил28Декларация.ИндексСтроки > &ТекущийИндексСтроки";
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение28.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение28.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение28.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение28.ДеревоФормИФорматов();
