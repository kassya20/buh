...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(Настройки)
	...
	ОписаниеНастроекРеестра = НовыйОписаниеНастроек();
	...
	Иначе
		ХранилищеНастроекОтчетовУдалить(ОписаниеНастроекРеестра);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуПоказателей(ПоказателиИсточник, ПоказателиПриемник, УсловноеОформление, СохраненныеНастройки = Неопределено)
	...
	Для Каждого Показатель Из ПоказателиПриемник.Элементы Цикл
		...
		ДобавитьПоказатель(
			ТаблицаПоказателей, Показатель, ДоступноеПоле, УсловноеОформление, ПорядокПоказателя, ИспользованиеПоказателя);
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьПоказатель(ТаблицаПоказателей, Показатель, ДоступноеПоле, УО, Приоритет, Использование = Ложь)
	...
	Если ДоступноеПоле = Неопределено Тогда
		Если СтрНайти(ПоказательИмя, ".") > 0 Тогда
			Если СтрНайти(ПоказательИмя, " ") = 0 Тогда
				ДобавитьПредставлениеДочернегоПоля(НоваяСтрока, Показатель, УО);
			Иначе
				НоваяСтрока.Представление = ВыделитьЗаголовокИзУсловногоОформления(Показатель.Поле, УО);
			КонецЕсли;
			
			Если ПустаяСтрока(НоваяСтрока.Представление) Тогда
				НоваяСтрока.Представление = ПоказательИмя;
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(Показатель.Заголовок) Тогда
...
Процедура ДобавитьДоступныеПоля(ТаблицаПоказателей, ПоказателиИсточник, УсловноеОформление, Приоритет)
	...
	Для Каждого Показатель Из ПоказателиИсточник Цикл
		...
		ДобавитьПоказатель(ТаблицаПоказателей, Показатель, ДоступноеПоле, УсловноеОформление, Приоритет);
...
&НаСервере
Процедура ДобавитьПредставлениеДочернегоПоля(НоваяСтрока, Показатель, УсловноеОформление)
	
	ТекстУО = ВыделитьЗаголовокИзУсловногоОформления(Показатель.Поле, УсловноеОформление);
	
	ПоказательИмя = Строка(Показатель.Поле);
	ЭлементыПоля  = СтрРазделить(ПоказательИмя, ".");
	
	Если ПустаяСтрока(ТекстУО) Тогда
		ТекстУО = ЭлементыПоля[ЭлементыПоля.ВГраница()];
		ЭлементыПоля.Удалить(ЭлементыПоля.ВГраница());
		ПредставлениеРодителей = СтрСоединить(ЭлементыПоля, ", ");
	Иначе
		Индекс = 0;
		СоставПредставления = Новый Массив;
		ЗаполнитьСоставПредставленияПоля(СоставПредставления, ЭлементыПоля, УсловноеОформление, Индекс);
		ПредставлениеРодителей = СтрСоединить(СоставПредставления, ", ");
	КонецЕсли;
	
	НоваяСтрока.ПредставлениеВОтчете = ТекстУО;
	НоваяСтрока.Представление = ТекстУО + " (" + ПредставлениеРодителей + ")";
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСоставПредставленияПоля(СоставПредставления, ЭлементыПоля, УсловноеОформление, Индекс)
	
	Если ЭлементыПоля.ВГраница() = Индекс Тогда
		Возврат;
	КонецЕсли;
	
	Родитель = ЭлементыПоля[Индекс];
	РодительСсылка = НСтр("ru = 'Ссылка'; en = 'Ref'", ОбщегоНазначения.КодОсновногоЯзыка());
	Если Родитель = РодительСсылка Тогда
		ПредставлениеОсновногоПоля = РодительСсылка;
	Иначе
		ДоступноеПолеРодитель = Отчет.КомпоновщикНастроек.Настройки.Выбор.ДоступныеПоляВыбора.Элементы.Найти(Родитель);
		Если ДоступноеПолеРодитель = Неопределено Тогда
			СоставТекущегоПоля = Новый Массив;
			Для ИндексПоля = 0 По Индекс Цикл
				СоставТекущегоПоля.Добавить(ЭлементыПоля[ИндексПоля]);
			КонецЦикла;
			
			ПредставлениеОсновногоПоля = ВыделитьЗаголовокИзУсловногоОформления(
				Новый ПолеКомпоновкиДанных(СтрСоединить(СоставТекущегоПоля, ".")), УсловноеОформление);
		Иначе
			ПредставлениеОсновногоПоля = ДоступноеПолеРодитель.Заголовок;
		КонецЕсли;
	КонецЕсли;
	
	СоставПредставления.Добавить(ПредставлениеОсновногоПоля);
...
Функция НовыйОписаниеНастроек()
	...
	Возврат ОписаниеНастроекРеестра;
	
КонецФункции

&НаСервереБезКонтекста
Процедура ХранилищеНастроекОтчетовУдалить(ОписаниеНастроекРеестра)
...
&НаСервере
Процедура ЗаполнитьПоказателиОтчета(ПоказателиКуда, ТаблицаПоказателей, ИспользоватьКолонкуПредставлениеВОтчете = Ложь)
	...
	Для Каждого Показатель Из ТаблицаПоказателей Цикл
		...
		Если ИспользоватьКолонкуПредставлениеВОтчете И ЗначениеЗаполнено(Показатель.ПредставлениеВОтчете) Тогда
			ВыбранноеПоле = БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(
				ПоказателиКуда, ПоказательПоле, Показатель.ПредставлениеВОтчете);
		Иначе
			ВыбранноеПоле = БухгалтерскиеОтчетыКлиентСервер.ДобавитьВыбранноеПоле(
				ПоказателиКуда, ПоказательПоле, Показатель.Представление);
		КонецЕсли;
...
Функция СформироватьНаСервере()
	...
	ЗаполнитьПоказателиОтчета(Отчет.КомпоновщикНастроек.Настройки.Структура[0].Выбор, Отчет.Показатели, Истина);
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.ГруппаСтандартныеКнопки);
