...
Процедура НастройкиДинамическогоСписка(Форма, ИмяТаблицыФормы = "Список", ИмяВФорме = "") Экспорт
	...
	// Имя команды для вызова отчета формируется в БСП и может меняться со временем.
	// К тому же, из некоторых форм списков могут передаваться вариации стандартного значения.
	// Чтобы меньше зависеть от изменений и при необходимости обрабатывать их в одном месте,
	// проверка имени команды вынесена в отдельную функцию.
	Если Не ЗначениеЗаполнено(ИмяВФорме)
		Или УправлениеПечатьюБПКлиентСервер.ЭтоИмяКомандыРеестрДокументов(ИмяВФорме) Тогда
		Для Каждого СтрокаТаблицыКоманды Из ТаблицаКоманды Цикл
			МенеджерПечати = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				СтрокаТаблицыКоманды.ДополнительныеПараметры, "МенеджерПечати");
			Если УправлениеПечатьюБПКлиентСервер.ЭтоИмяКомандыРеестрДокументов(СтрокаТаблицыКоманды.ИмяВФорме)
				И СтрНайти(МенеджерПечати, "ДополнительныеОтчетыИОбработки") = 0 Тогда
				СтрокаТаблицы = СтрокаТаблицыКоманды;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе
		СтрокаТаблицы = ТаблицаКоманды.Найти(ИмяВФорме, "ИмяВФорме");
	КонецЕсли;
...
Функция НомерНаПечать(Номер, Ссылка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		ПредставлениеНомера = "";
		Если Документы.ПлатежноеПоручение.НомерПлатежногоПорученияИзвестен(Ссылка) Тогда
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.СчетНаОплатуПокупателю")
		Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		ИдентификаторКонтракта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			Ссылка, "ДоговорКонтрагента.ГосударственныйКонтракт.Код");
		ПредставлениеНомера = ?(ЗначениеЗаполнено(ИдентификаторКонтракта), ИдентификаторКонтракта + "/", "")
...
Процедура НастроитьСтруктуруОтчета(Настройки, ПараметрыОтчета, Схема, СхемаКомпоновкиДанныхЭталон)
	...
	Для каждого УО Из СтруктураИтоги.УсловноеОформление.Элементы Цикл
		Если ЗначениеЗаполнено(УО.Отбор) И СтрНайти(Строка(УО.Отбор), "Сумма") > 0 Тогда
			...
			Если УО.Использование Тогда
				УО.ИспользоватьВГруппировке = ИспользованиеУсловногоОформленияКомпоновкиДанных.Использовать;
			КонецЕсли;
