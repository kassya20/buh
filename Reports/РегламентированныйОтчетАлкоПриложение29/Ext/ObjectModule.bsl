...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил29Производство" Тогда
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил29Поставщики" Тогда
		
		// Поставщик
		Запись.П000010000304 = СокрЛП(Запись.П000010000304);
		НаименованиеПолное = Запись.П000010000304;
		Запись.П0000100003041 = СокрЛП(Запись.П0000100003041);
		Наименование = Запись.П0000100003041;
		...
		Если НЕ ЗначениеЗаполнено(Запись.Поставщик) Тогда
			...
			Запись.Поставщик = ОпределитьПоставщика(ИНН, КПП, НаименованиеПолное);
			
			МетаданныеКонтрагенты = Метаданные.Справочники.Контрагенты;
			ЕстьРеквизитПолноеНаименование = ОбщегоНазначения.ЕстьРеквизитОбъекта(
										"НаименованиеПолное", МетаданныеКонтрагенты);
			ИмяРеквизитаНаименование = ?(ЕстьРеквизитПолноеНаименование, 
										"НаименованиеПолное", "Наименование");
			
			Если ЗначениеЗаполнено(Запись.Поставщик) Тогда 
				Запись.П000010000304 = Запись.Поставщик[ИмяРеквизитаНаименование];
				Запись.П0000100003041 = Запись.Поставщик["Наименование"];
		...
		НаименованиеВсеВместе = НаименованиеПолное + " " + НаименованиеОП;
...
Процедура ПроверитьЗаполнениеТаблицыДекларацияПоставщики(СтатусПроверки, ИдГруппы, НомерЛиста, ИдОП)
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаДекларации Из РезЗапросаДанных Цикл
			...
			// Допускается для иностранцев не заполнять.
...
Функция ЗаполнитьПоВсейОрганизацииДекларация(
	...
	|ВЫБРАТЬ
	|АлкоПрил29Декларация.П000011000313 КАК П000010000313,
	|АлкоПрил29Декларация.П000011000314 КАК П000010000314,
	|
	|СУММА(АлкоПрил29Декларация.П000011000315) КАК П000010000315
	|
	|ИЗ
	|	
	|(ВЫБРАТЬ
	|РегистрСведенийАлкоПрил29Декларация.*
	|
	|ИЗ
	|РегистрСведений.СведенияРеглОтчетАлкоПрил29Производство КАК РегистрСведенийАлкоПрил29Декларация
	|
	|ГДЕ (РегистрСведенийАлкоПрил29Декларация.ИДДокИндСтраницы В (&МассивИДДокИндСтраницы)) 
	|И (РегистрСведенийАлкоПрил29Декларация.Активно = Истина)) КАК АлкоПрил29Декларация
	|	
    |СГРУППИРОВАТЬ ПО 
	|АлкоПрил29Декларация.П000011000313, 
	|АлкоПрил29Декларация.П000011000314
	|
	|УПОРЯДОЧИТЬ ПО
	|АлкоПрил29Декларация.П000011000313 ВОЗР
	|";
	
	ЗапросДекларация = Новый Запрос;
	ЗапросДекларация.Текст = ТекстЗапросаДекларация;
	
	СервисРегистровДекларация = СтруктураСервисРегистров["СведенияРеглОтчетАлкоПрил29Производство"];
	...
	ТекстЗапросаДекларация = "
	|ВЫБРАТЬ
	|АлкоПрил29Декларация.П000010000304 КАК П000010000304,
	|АлкоПрил29Декларация.П0000100003042 КАК П0000100003042,
	|АлкоПрил29Декларация.П000010000305 КАК П000010000305,
	|АлкоПрил29Декларация.П000010000306 КАК П000010000306,
	|АлкоПрил29Декларация.ХешИдПоставщика КАК ХешИдПоставщика,
	|
	|СУММА(АлкоПрил29Декларация.П000010000307) КАК П000010000307
	|
	|ИЗ
	|	
	|(ВЫБРАТЬ
	...
	|
	|ИЗ
	|РегистрСведений.СведенияРеглОтчетАлкоПрил29Поставщики КАК РегистрСведенийАлкоПрил29Декларация
	|
	|ГДЕ (РегистрСведенийАлкоПрил29Декларация.ИДДокИндСтраницы В (&МассивИДДокИндСтраницы)) 
	|И (РегистрСведенийАлкоПрил29Декларация.Активно = Истина)) КАК АлкоПрил29Декларация
	|	
    |СГРУППИРОВАТЬ ПО 
	|АлкоПрил29Декларация.П000010000304, 
	|АлкоПрил29Декларация.П0000100003042, 
	|АлкоПрил29Декларация.П000010000305, 
	|АлкоПрил29Декларация.П000010000306,
	|АлкоПрил29Декларация.ХешИдПоставщика
	|
	|УПОРЯДОЧИТЬ ПО
	|АлкоПрил29Декларация.П000010000304 ВОЗР, 
	|АлкоПрил29Декларация.П0000100003042 ВОЗР, 
	|АлкоПрил29Декларация.П000010000305 ВОЗР, 
	|АлкоПрил29Декларация.П000010000306 ВОЗР
	|";
	
	ЗапросДекларация = Новый Запрос;
	ЗапросДекларация.Текст = ТекстЗапросаДекларация;
	
	СервисРегистровДекларация = СтруктураСервисРегистров["СведенияРеглОтчетАлкоПрил29Поставщики"];
	МассивИндексовСтраниц = СервисРегистровДекларация.ВыгрузитьКолонку("ИндексСтраницыРегистра");
	...
	ТекстЗапросаДекларация = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|АлкоПрил29Декларация.П0000100003041 КАК П0000100003041,
	|АлкоПрил29Декларация.ХешИдПоставщика КАК ХешИдПоставщика
	|
	|ИЗ
	|	
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|АлкоПрил29Декларация.ХешИдПоставщика ВОЗР,
	|АлкоПрил29Декларация.П0000100003041 УБЫВ
	|";
...
Процедура ЗаполнитьДанныеПроизводство(ИдГруппы, НомерЛиста, ИдОП)
	...
	|	РегистрСведенийАлкоПрил29Декларация.*
	|ИЗ
	|	РегистрСведений." + ИмяРегистраСведений + " КАК РегистрСведенийАлкоПрил29Декларация
	|ГДЕ
	|	РегистрСведенийАлкоПрил29Декларация.ИДДокИндСтраницы = &ИДДокИндСтраницы
	|   И РегистрСведенийАлкоПрил29Декларация.Активно = Истина
	|	И РегистрСведенийАлкоПрил29Декларация.ИндексСтроки > &ТекущийИндексСтроки";
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение29.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение29.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение29.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение29.ДеревоФормИФорматов();
