...
Функция ПолучитьОстаткиДенежныхСредствДляМонитораРуководителя(Организация, Дата) Экспорт
	...
	СписокСчетов.Добавить(ПланыСчетов.Хозрасчетный.ЦифровойРубль);    // 53
	...
	ПорядокСубконтоБезналичные.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.БанковскиеСчета);
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ВедетсяУчетПоВалютам Тогда
		ЗаголовокИтого = НСтр("ru = 'Итого, руб'");
	Иначе
		ЗаголовокИтого = НСтр("ru = 'Итого'");
	КонецЕсли;
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ЗаголовокИтого", ЗаголовокИтого);
...
Процедура АдаптироватьНастройки(Настройки) Экспорт
	...
	// Добавим поле "Период" в настройки 
	Настройки.Вставить("Период", Настройки.НачалоПериода);
