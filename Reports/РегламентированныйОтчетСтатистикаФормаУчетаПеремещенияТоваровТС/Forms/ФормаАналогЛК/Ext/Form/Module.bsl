...
Процедура ПроверитьПоказателиОтчета(Команда)
	...
	Для Каждого Стр Из ДокументыОснования Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.Дата) Тогда
			...
			Возврат;
		ИначеЕсли КонецМесяца(Стр.Дата) > КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда 
			ПутьКПолю = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("ДокументыОснования", ДокументыОснования.Индекс(Стр) + 1, "Дата");
			ОбщегоНазначенияКлиент.СообщитьПользователю("Дата документа позднее периода статформы", , ПутьКПолю);
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Функция КоличествоСтрокПоМетодикеФТС()
	...
	Для Каждого Стр Из Товары Цикл 
		...
		ДекларацияНаТовары = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПВТ, "Товар_ДекларацияНаТовары", Новый ТаблицаЗначений);
		СредстваИдентификации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПВТ, "Товар_СредстваИдентификации", Новый ТаблицаЗначений);
		
		ВсегоПозиций = ВсегоПозиций 
			+ ?(ДекларацияНаТовары.Количество() > 0, ДекларацияНаТовары.Количество() - 1, 0)
			+ ?(СредстваИдентификации.Количество() > 0, СредстваИдентификации.Количество() - 1, 0);
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ДеревоВыгрузки = РегламентированнаяОтчетность.ИзвлечьСтруктуруXML(ИмяФормы, "СхемаВыгрузкиXMLЛК_5_27_0");
...
Процедура ЗаполнитьДанными(ДеревоВыгрузки, ПараметрыВыгрузки)
	...
	Для Каждого Стр Из Товары Цикл
		...
		СредстваИдентификации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ПараметрыВыгрузкиСтрокиТовара, "Товар_СредстваИдентификации", Новый ТаблицаЗначений);
		
		ПараметрыВыгрузкиСтрокиТовара.Вставить("Товары_CIMQuantity", СредстваИдентификации.Количество());
		...
		Если СредстваИдентификации.Количество() = 0 Тогда
...
Асинх Процедура ЗаполнитьДаннымиПервичногоДокументаПослеВыбора(Результат, ДополнительныеПараметры) Экспорт 
	...
	РезультатВыполнения = ЗаполнитьДаннымиПервичногоДокумента(Результат);
	Если НЕ РезультатВыполнения.ЗаданиеВыполнено Тогда
		Доступность = Ложь;
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗаполнитьАвтоЗавершениеПослеЗаполнения", ЭтотОбъект);
		ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
		ПараметрыОжидания.ТекстСообщения = НСтр("ru = 'Выполняется заполнение по данным первичных документов.'");
...
&НаСервере
Функция ЗаполнитьДаннымиПервичногоДокумента(Ссылки)
	Попытка
		СписокОснований.Очистить();
		...
		Иначе
			...
			СписокОснований.Добавить(СсылкаДокументаОснования);
		КонецЕсли;
		СтруктураРеквизитовФормы.Вставить("АдресХранилищаМассивДокументов",
			ПоместитьВоВременноеХранилище(МассивДокументов, Новый УникальныйИдентификатор));
		РазложениеИмениФормы = СтрРазделить(ИмяФормы, ".");
		
		ПараметрыОтчетаВФоне = Новый Структура;
		ПараметрыОтчетаВФоне.Вставить("ИДОтчета", РазложениеИмениФормы[1]);
		ПараметрыОтчетаВФоне.Вставить("ИДРедакцииОтчета", РазложениеИмениФормы[3]);
		ПараметрыОтчетаВФоне.Вставить("ПараметрыОтчета",
				Новый Структура("МассивДокументов,НаправлениеПеремещения", МассивДокументов, ?(Графа4 = 1, "ИМ", "ЭК")));
		ПараметрыОтчетаВФоне.Вставить("Контейнер", Неопределено);
		...
		РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
			...
			ПараметрыОтчетаВФоне,
			ПараметрыВыполненияВФоне);
		
		РезультатВыполнения.Вставить("ЗаданиеВыполнено", НРег(РезультатВыполнения.Статус) = "выполнено");
		СтруктураРеквизитовФормы.Вставить("АдресХранилища", РезультатВыполнения.АдресРезультата);
...
Процедура ЗагрузитьПодготовленныеДанные()
	Попытка
		МассивДокументов = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы.АдресХранилищаМассивДокументов);
		ДанныеПервичногоДокумента = ПолучитьИзВременногоХранилища(СтруктураРеквизитовФормы.АдресХранилища).Контейнер;
		...
		КонецЕсли;
		ТаблицаТоварыДляВыгрузки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеПервичногоДокумента, "ТаблицаТоварыДляВыгрузки");
		Если ТипЗнч(ТаблицаТоварыДляВыгрузки) = Тип("ТаблицаЗначений") Тогда 
			Если ТаблицаТоварыДляВыгрузки.Колонки.Найти("ПрослеживаемыйТовар") = Неопределено Тогда 
				ТаблицаТоварыДляВыгрузки.Колонки.Добавить("ПрослеживаемыйТовар", Новый ОписаниеТипов("Булево"));
			КонецЕсли;
			Для Каждого КолонкаТаблицы Из СтрРазделить("ДопСведения,ДопСведенияКод,КолвоПрослеживаемогоТовараВЕдИзмеренияТНВЭД," 
				...
				Если ТаблицаТоварыДляВыгрузки.Колонки.Найти(КолонкаТаблицы) = Неопределено Тогда 
					ТаблицаТоварыДляВыгрузки.Колонки.Добавить(КолонкаТаблицы);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Исключение
		...
		Основание = Неопределено;
		СписокОснований.Очистить();
		ОбщегоНазначения.СообщитьПользователю("Не удалось выполнить заполнение");
	КонецПопытки;
	Модифицированность = Истина;
КонецПроцедуры
