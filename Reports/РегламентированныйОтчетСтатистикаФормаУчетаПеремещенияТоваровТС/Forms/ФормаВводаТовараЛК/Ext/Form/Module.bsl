...
Процедура НоменклатураВыборЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(РезультатВыбора) = Тип("Структура") Тогда 
		...
		Если Не ЗначениеЗаполнено(MeasureUnitQualifierName) И Не ЗначениеЗаполнено(MeasureUnitQualifierCode) Тогда 
			GoodsQuantity = 0;
		КонецЕсли;
		...
		ОКАТО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатВыбора, "ПроизводительОКАТО");
		...
		СредстваИдентификации.Очистить();
