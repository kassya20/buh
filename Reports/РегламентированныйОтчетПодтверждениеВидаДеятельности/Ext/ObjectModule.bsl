...
Процедура СформироватьСтруктуруРеквизитовФормы(ИмяФормы, СтруктураРеквизитовФормы) Экспорт
	...
	СтруктураРеквизитовФормы.Вставить("мВерсияФормы", "25/12/2025");
	...
	СтруктураРеквизитовФормы.Вставить("Редакция75н", Ложь);
...
Функция СтруктураСтраницОтчета(ИмяФормы) Экспорт
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда 
		Возврат СтруктураСтраницОтчетаФормаОтчета2025Кв1();
	ИначеЕсли ИмяФормы = "ФормаОтчета2013Кв1" Тогда 
		Возврат СтруктураСтраницОтчетаФормаОтчета2013Кв1();
	Иначе
		ВызватьИсключение "Неправильный параметр вызова СтруктураСтраницОтчета(...)";
	КонецЕсли;
КонецФункции

Функция СтруктураСтраницОтчетаФормаОтчета2013Кв1() Экспорт
...
Функция СтруктураСтраницОтчетаФормаОтчета2025Кв1() Экспорт
	ДеревоСтраницОтчета = НовыйДеревоСтраницОтчета();
...
Процедура ЗаполнитьПустыеСтруктурыДанных(ИмяФормы, РазделыОтчета, СтруктураРеквизитовФормы)
	...
	Для Каждого Раздел Из РазделыОтчета Цикл
		...
		Если Раздел.Значение.Данные.Количество() = 0 Тогда
			...
			Если Раздел.Ключ = "Приложение3" И СтруктураРеквизитовФормы.Редакция75н Тогда
...
Функция ЗаписатьДокумент(ИмяФормы, ПараметрыЗаписи) Экспорт
	...
	СтруктураХранения = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыЗаписи, "СтруктураХраненияФайлаПояснительнойЗаписки");
...
Функция УниверсальнаяВыгрузка(Параметры) Экспорт
	...
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда 
		ОбщегоНазначения.СообщитьПользователю("Выгрузка будет реализована в будущих версиях программы");
	...
	Если НЕ ВыгрузкаВозможна(ИмяФормы, ДанныеОтчета, ПараметрыВыгрузки, ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы) Тогда
		РезультатВыгрузки.Вставить("РеглОтчетВыгружен", Ложь);
		Возврат РезультатВыгрузки;
	КонецЕсли;
	
	ЗаполнитьДанными(ИмяФормы, ДанныеОтчета, ПараметрыВыгрузки, ДеревоВыгрузки);
...
Функция ПроверитьДанные(ИмяФормы, ДеревоВыгрузки, РезультатВыгрузки) Экспорт
	...
	Для Каждого Стр из ДеревоВыгрузки.Строки Цикл
		СтатусПроверки = (СтатусПроверки И ПроверитьДанные(ИмяФормы, Стр, РезультатВыгрузки));
	КонецЦикла;
...
Функция ВыгрузкаВозможна(ИмяФормы, ДанныеОтчета, ПараметрыВыгрузки, ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы) Экспорт 
	Если ИмяФормы = "ФормаОтчета2013Кв1" Тогда 
		Возврат ВыгрузкаВозможнаФормаОтчета2013Кв1(ДанныеОтчета, ПараметрыВыгрузки,
			ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы);
	ИначеЕсли ИмяФормы = "ФормаОтчета2025Кв1" Тогда
		Возврат ВыгрузкаВозможнаФормаОтчета2025Кв1(ДанныеОтчета, ПараметрыВыгрузки,
			ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы);
	Иначе
		ВызватьИсключение "Неправильный параметр ВыгрузкаВозможна(...)";
	КонецЕсли;
КонецФункции

Функция ВыгрузкаВозможнаФормаОтчета2025Кв1(ДанныеОтчета, ПараметрыВыгрузки, ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы)
	РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(
		РезультатВыгрузки, "Выгрузка отчета будет реализована в будущих версиях программы", "Приложение1", "");
	Возврат Ложь;
КонецФункции

Функция ВыгрузкаВозможнаФормаОтчета2013Кв1(ДанныеОтчета, ПараметрыВыгрузки, ДеревоВыгрузки, РезультатВыгрузки, СтруктураРеквизитовФормы)
	СтатусПроверки = Истина;
	Приложение2 = ДанныеОтчета.ПолеТабличногоДокументаПриложение2;
...
Функция СформироватьСтруктуруПараметров(ИмяФормы, ДанныеОтчета, СтруктураРеквизитовФормы) Экспорт
	Если ИмяФормы = "ФормаОтчета2013Кв1" Тогда
		Возврат СформироватьСтруктуруПараметровФормаОтчета2013Кв1(ДанныеОтчета, СтруктураРеквизитовФормы);
	ИначеЕсли ИмяФормы = "ФормаОтчета2025Кв1" Тогда
		Возврат СформироватьСтруктуруПараметровФормаОтчета2025Кв1(ДанныеОтчета, СтруктураРеквизитовФормы);
	Иначе
		ВызватьИсключение "Неправильный параметр вызова СформироватьСтруктуруПараметров(...)";
	КонецЕсли;
КонецФункции

Функция СформироватьСтруктуруПараметровФормаОтчета2025Кв1(ДанныеОтчета, СтруктураРеквизитовФормы)
	Возврат Новый Структура;
КонецФункции

Функция СформироватьСтруктуруПараметровФормаОтчета2013Кв1(ДанныеОтчета, СтруктураРеквизитовФормы)
	...
	ДатаНачХозДеят = "";
	ДатаНачалаХозДеятельности = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Приложение2, "ДатаНачалаХозДеятельности");
	Если ЗначениеЗаполнено(ДатаНачалаХозДеятельности) Тогда
		Если ТипЗнЧ(ДатаНачалаХозДеятельности) = Тип("Строка") Тогда
		...
		ИначеЕсли ТипЗнЧ(ДатаНачалаХозДеятельности) = Тип("Дата") Тогда
			ДатаНачХозДеят = ?(ЗначениеЗаполнено(ДатаНачалаХозДеятельности),
			Формат(ДатаНачалаХозДеятельности, "ДФ=""гггг-ММ-дд"""), Неопределено);
		КонецЕсли;
	КонецЕсли;
...
Функция ИзвлечьСтруктуруXML(ИмяФормы) Экспорт
	...
	Для НомерСтроки = 2 По ВысотаТаблицы Цикл
		...
		Если Макет.Область(НомерСтроки, 0, НомерСтроки, 0).Видимость = Неопределено Тогда
			ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), НомерСтроки));
		КонецЕсли;
	КонецЦикла;
...
Функция ПечатныйБланк(ПараметрыПечати) Экспорт
	...
	Если ДанныеРазделов.Приложение2.Количество() = 4 И ДанныеРазделов.Приложение3.Количество() = 2 Тогда
		РегламентированнаяОтчетность.ЗаполнитьСтруктуруДанныхРазделаЗначениямиИзДопСтрок(
...
Процедура ВывестиНаПечатьПриложение1(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов)
	...
	ЗаполняемыйМакет = ЭтотОбъект.ПолучитьМакет("Печатная" + ИмяФормы + "_Приложение1");
	
	ЗаполнитьПечатнуюФорму(ИмяФормы, СтруктураПараметров, ЗаполняемыйМакет, ДанныеРазделов, СтруктураРеквизитовФормы);
	Если ЗначениеЗаполнено(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеРазделов.Приложение1, "ПечататьСтроку2")) Тогда 
		ЗаполняемыйМакет.УдалитьОбласть(ЗаполняемыйМакет.Область("СекцияПечататьСтроку2"), ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
...
Процедура ВывестиНаПечатьПриложение2(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов)
	СписокПечатныхФорм = Новый СписокЗначений;
	ЗаполняемыйМакет = ЭтотОбъект.ПолучитьМакет("Печатная" + ИмяФормы + "_Приложение2");
...
Процедура ВывестиНаПечатьПриложение3(ИмяФормы, СтруктураПараметров, СтруктураРеквизитовФормы, ДанныеРазделов)
	...
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда 
		ЗаполняемыйМакет = ЭтотОбъект.ПолучитьМакет("ПечатнаяФормаОтчета2025Кв1_Приложение3_1");
	ИначеЕсли СтруктураРеквизитовФормы.Редакция75н Тогда
	...
	Если ИмяФормы = "ФормаОтчета2025Кв1" Тогда 
		МакетОбразец = ЭтотОбъект.ПолучитьМакет("ПечатнаяФормаОтчета2025Кв1_Приложение3_2");
	ИначеЕсли СтруктураРеквизитовФормы.Редакция75н Тогда
	...
	ЗаполнитьПечатнуюФорму(ИмяФормы, СтруктураПараметров, МакетОбразец, ДанныеРазделов, СтруктураРеквизитовФормы);
	...
	Для Инд = 10 По КоличествоСтрок Цикл
		...
		Для Каждого Колонка Из МногострочнаяЧасть.Колонки Цикл
			...
			КонецЕсли;
		КонецЦикла;
		ЗаполнитьПечатнуюФорму(ИмяФормы, СтруктураПараметров, ОбластьМногострочнаяЧасть, ДанныеРазделов, СтруктураРеквизитовФормы);
	...
	Для Каждого СформированныйЛист Из СписокПечатныхФорм Цикл
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(СформированныйЛист.Значение, СтруктураПараметров.НаименованиеЛиста);
	КонецЦикла;
КонецПроцедуры
...
Функция ПолучитьЗначениеМЧБ(ИмяФормы, Показатель, ПараметрыПечати, СтруктураРеквизитовФормы)
	...
	Если ИмяФормы <> "ФормаОтчета2013Кв1" 
		Или ПараметрыПечати.ИмяСтраницы <> "Приложение3"
		Или Не СтруктураРеквизитовФормы.Редакция75н Тогда
		
		МакетФормыОтчета = ЭтотОбъект.ПолучитьМакет(ИмяФормы + "_" + ПараметрыПечати.ИмяСтраницы);
	Иначе
		МакетФормыОтчета = ЭтотОбъект.ПолучитьМакет("ФормаОтчета2013Кв1_Приложение3_ВРедакцииПриказа75н");
	КонецЕсли;
	
	Область = МакетФормыОтчета.Области.Найти(?(ТипЗнч(Показатель) = Тип("КлючИЗначение"), Показатель.Ключ, Показатель.Имя));
