...
Функция ДоляВычетовОтНачисленийНДС(Организация, КонецПериода, КоличествоКварталов = 4) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСТр("ru = 'Ошибка получения сведений о вычетах НДС'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПодготовитьСведенияПредельныхЗначений(ПараметрыОтчета)
	...
	Иначе
		...
		ВозможныеКодыОКВЭД = КлассификаторОКВЭДБП.КодВИерархии(КодОКВЭД);
...
Процедура НалоговаяНагрузка(ПараметрыОтчета, ДанныеУчетнойПолитики, СведенияПредельныхЗначений, 
	...
	ИначеЕсли ТекущаяУчетнаяПолитика.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая Тогда
		...
		ДанныеРегламентированногоОтчетаПрибыль = НовыйДанныеРегламентированногоОтчета();
		Если Год(КонецПериода) >= 2025 Тогда
			ИсточникОтчета = "РегламентированныйОтчет3_НДФЛ";
		Иначе
...
Функция ПроверитьВидЮридическогоЛицаНаПрименениеЕНВД(ДанныеУчетнойПолитики, Организация, ДанныеЗаполненияМакета, 
	...
	Если СистемаНалогообложенияНалогоплательщика = Перечисления.СистемыНалогообложения.АУСН Тогда
		ДанныеЗаполненияМакета.Вставить("НеПроверяется", "НеПроверяетсяНалоговаяНагрузкаАУСН");
		ЗаполнитьМакетОтчетаНеПроверяется(
			РезультатПроверкиКритериев, 
			РезультатПроверкиКритериевПФ, 
			ДанныеЗаполненияМакета);
		Возврат Истина;
	ИначеЕсли Не ЭтоЮридическоеЛицо 
		И (СистемаНалогообложенияНалогоплательщика = Перечисления.СистемыНалогообложения.ОсобыйПорядок
		Или СистемаНалогообложенияНалогоплательщика = Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход) Тогда
...
Процедура ЗаполнитьОтчетыНетДанных(ОтчетыНетДанных, РезультатЗапроса, ПериодыОтчета, ИсточникОтчета)
	...
	Если Год(ПериодыОтчета.КонецПериода) >= 2025 Тогда
		ГодовойОтчет.Добавить("РегламентированныйОтчет3_НДФЛ");
	Иначе
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если ТипЗнч(ГодовойОтчет.НайтиПоЗначению(ИсточникОтчета)) <> Тип("Неопределено") Тогда
			...
			Если ИсточникОтчета = "РегламентированныйОтчет3НДФЛ"
				Или ИсточникОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
...
Функция ПолучитьНаименованиеОтчета(ИсточникОтчета)
	...
	ИначеЕсли ИсточникОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		НаименованиеОтчета = НСтр("ru='декларацию 3-НДФЛ за'") + " ";
...
Процедура СоздатьВТПрочихДоходовИРасходов(МенеджерВременныхТаблиц)
	...
	ЗначениеРеквизита_2310_2320_2340.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыКПолучению);
	ЗначениеРеквизита_2310_2320_2340.Добавить(
	...
	ЗначениеРеквизита_2330_2350.Добавить(Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыКУплате);
...
Процедура ЗначительныеВычетыПоНДС(ПараметрыОтчета, РезультатыПроверки, РезультатПредставлениеПроверки,
	...
	МожноКонтролироватьДолюВычетаНДС = 
		КонецПериода >= УчетНДСКлиентСервер.НачалоРаботыКонтроляДолиВычетов();
	...
	ПредельноеЗначениеВычетовНДС = УчетнаяПолитика.БезопаснаяДоляВычета(Организация, КонецПериода);
	МинимальноеЗначениеДолиВычета = 0;
...
Процедура ТемпыРостаРасходов(ПараметрыОтчета, ДанныеУчетнойПолитики, РезультатПроверкиКритериев, 
	...
	КоэффициентРостаПоНалоговойОтчетности = ?(ТемпРостаДоходовНалоговойОтчетности <> 0,
		Окр(ТемпРостаРасходовНалоговойОтчетности / ТемпРостаДоходовНалоговойОтчетности, 1, 1),
		0);
	КоэффициентРостаПоБухгалтерскойОтчетности = ?(ТемпРостаДоходовБухгалтерскойОтчетности <> 0,
		Окр(ТемпРостаРасходовБухгалтерскойОтчетности / ТемпРостаДоходовБухгалтерскойОтчетности, 1, 1),
		0);
...
Процедура ПолучитьДанныеДоходыУСН(ПериодыОтчета, Организация, ДанныеПолученныхДоходов, 
	...
	Иначе
		РасшифровкаЗначения.Вставить("ЛимитДоходовПредставление",
			СтрШаблон(НСтр("ru = '%1 млн. руб.'"), Формат(ГраницаПримененияУСНПоДоходам, "ЧДЦ=; ЧС=6")));
	КонецЕсли;
...
Процедура ЗаполнитьРасшифровкуСреднесписочнойЧисленности(ДанныеСреднесписочнойЧисленности, 
	...
	Для ИндексСтроки = 0 По МаксимальныйИндекс Цикл
		...
		ПараметрыРасшифровки.Вставить("ПредельныйРазмерЧисленностиПредставление",
			СтрШаблон(НСтр("ru = '%1 человек'"), ТекущаяСтрока.ПредельныйРазмер));
...
Функция ПолучитьСтоимостьАктивов(Организация, ПериодыОтчета)
	...
	СКД1_1.Добавить(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства_);
	...
	СКД1_1.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеПравНаИспользованиеРезультатовИнтеллектуальнойДеятельности);
	
	СКД1_2 = Новый Массив;
	СКД1_2.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеОС);
	СКД1_2.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеМЦ);
	СКД1_2.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеНематериальныхАктивов);
	...
	СКК1_1.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияНематериальныхАктивов_);
