...
Функция ТекущаяСхемаКомпоновкиДанных(ПараметрыВыбораСхемы) Экспорт
	...
	ТолькоОсновнаяСистема = УчетнаяПолитика.ТолькоОсновнаяСистемаНалогообложенияЗаПериод(
		ПараметрыВыбораСхемы.Организация,
		ПараметрыВыбораСхемы.НачалоПериода,
		ПараметрыВыбораСхемы.КонецПериода);
	
	Если ТолькоОсновнаяСистема Тогда
		Возврат "СправкаРасчетТолькоОсновнаяСистемаНалогообложения";
	Иначе
		Возврат "СправкаРасчетПрименяетсяОсобыйПорядокНалогообложения";
	КонецЕсли;
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	СубконтоОС = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.РеализацияОсновныхСредств");
	
	Если СубконтоОС <> Неопределено Тогда
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СубконтоОС", СубконтоОС);
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ФункциональностьОСВключена", Истина);
	КонецЕсли;
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ИспользуетсяЕНВД", ПрименяютсяСпециальныеРежимы);
