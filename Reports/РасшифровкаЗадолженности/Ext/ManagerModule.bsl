...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	Запрос.УстановитьПараметр("СчетаАвансыПоставщикам", СчетаАвансыПоставщикам());
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	УстановитьЗапросыНаборовДанных(ПараметрыОтчета, Схема);
...
Функция СчетаРасчетовПоУмолчанию(ВключитьСчетаБезИнвентаризацииВАкте = Ложь) Экспорт
	...
	Если ВключитьСчетаБезИнвентаризацииВАкте Тогда
		...
		УдалитьСчетаПроцентыИНДСПоАренде(Результат)
...
Функция ВариантыНастроек() Экспорт
	...
	Результат.Добавить(Новый Структура("Имя, Представление",
		"РасшифровкаЗадолженности", НСтр("ru = 'Расшифровка задолженности'")));
	Результат.Добавить(Новый Структура("Имя, Представление",
		"РасшифровкаЗадолженностиПоСтрокамБаланса", НСтр("ru = 'Расшифровка задолженности по строкам баланса'")));
...
Процедура ВывестиПредставлениеОтбораСчетаРасчетов(ПараметрыОтчета, Результат)
	...
	Если ОбластьОтборов = Неопределено Тогда
		...
		СтрокаВставкиОбластиОтборов = Результат.Области.Заголовок.Верх + 1;
...
Функция ТекстЗапросаОтчета(ВариантДетализации, Интервалы, ПоляВыбора)
	...
	|				И Данные.СчетРасчетов В (&СчетаАвансыПоставщикам)
	|			ТОГДА ВЫБОР
	|					КОГДА Данные.Договор.ВидАктива = ЗНАЧЕНИЕ(Перечисление.ВидыАктивовДляАвансов.ОсновныеСредства)
	|						ТОГДА ""1190""
	|					КОГДА Данные.Договор.ВидАктива В (ЗНАЧЕНИЕ(Перечисление.ВидыАктивовДляАвансов.Запасы), ЗНАЧЕНИЕ(Перечисление.ВидыАктивовДляАвансов.ПрочиеОборотные))
	|						ТОГДА ""1260""
	|					ИНАЧЕ ""1230""
	|				КОНЕЦ
	|		КОГДА Данные.ВидЗадолженности = &ДебиторскаяЗадолженность
	...
	|						ТОГДА ВЫРАЗИТЬ(ДанныеПоАктам.Субконто2 КАК Справочник.ДоговорыКонтрагентов)
	|					КОГДА ДанныеПоАктам.Субконто3 ССЫЛКА Справочник.ДоговорыКонтрагентов
	|						ТОГДА ВЫРАЗИТЬ(ДанныеПоАктам.Субконто3 КАК Справочник.ДоговорыКонтрагентов)
	|					ИНАЧЕ NULL
	|				КОНЕЦ
	|		КОНЕЦ КАК Договор,
	|		ВЫБОР
...
Процедура УстановитьПараметры(ПараметрыОтчета, Схема, КомпоновщикНастроек)
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаСтроки1190", СчетаСтроки1190());
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаПроцентыИНДСПоАренде", СчетаПроцентыИНДСПоАренде());
...
Процедура УстановитьЗапросыНаборовДанных(ПараметрыОтчета, Схема)

	// В случае вывода варианта "по строкам баланса" НДС с авансов выданных необходимо
	// исключать из разных строк баланса в зависимости от признака "Вид актива" в договоре с контрагентом.
	// Поскольку на 76.ВА нет субконто договора, то определяем договор из документа-основания счета-фактуры (субконто2).
	// Для этого составм список документов-оснований, которые фактически есть в остатках 76.ВА.
	
	Если ПараметрыОтчета.КлючВарианта <> "РасшифровкаЗадолженностиПоСтрокамБаланса" Тогда
		// Расшифровка по строкам баланса не выполняется, ничего делать не требуется.
		Возврат;
	КонецЕсли;
	
	НаборНДСсАвансов = Схема.НаборыДанных.ДанныеОтчета.Элементы.НДСсАвансов;
	
	ТекстКомментарияВставки = "// ОбъединитьВсеДокументыОснованияСчетовФактур76ВА";
	
	Если СтрНайти(НаборНДСсАвансов.Запрос, ТекстКомментарияВставки) = 0 Тогда
		// Схема уже была изменена ранее, повторно не требуется.
		Возврат;
	КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организации", ПараметрыОтчета.СписокОрганизаций);
	Запрос.УстановитьПараметр("Период",      КонецДня(ПараметрыОтчета.Период) + 1);
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТИПЗНАЧЕНИЯ(ХозрасчетныйОстатки.Субконто2) КАК ТипДокумента
	|ИЗ
	...
	|	ШапкаДокумента.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ШапкаДокумента.Ссылка КАК ДокументОснование
	|ИЗ
	|	ВТ_Остатки76ВА КАК ВТ_Остатки76ВА
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ %ИмяДокумента% КАК ШапкаДокумента
	|	ПО ВТ_Остатки76ВА.Организация = ШапкаДокумента.Организация
	|		И ВТ_Остатки76ВА.Субконто1 = ШапкаДокумента.Контрагент
	|		И ВТ_Остатки76ВА.Субконто2 = ШапкаДокумента.Ссылка
	|ГДЕ
	|	ШапкаДокумента.Дата <= &ДатаОтчета
	|	И ШапкаДокумента.Организация В (&Организация)
	|{ГДЕ
	|	Контрагент.* КАК ДебиторКредитор,
	|	Ссылка.* КАК Документ
	|}";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МетаданныеДокумента = Неопределено
			ИЛИ НЕ Метаданные.Документы.Содержит(МетаданныеДокумента) Тогда
			Продолжить;
		КонецЕсли;
	...
	НаборНДСсАвансов.Запрос = СтрЗаменить(
		НаборНДСсАвансов.Запрос,
		ТекстКомментарияВставки,
		СтрСоединить(ТекстыПодзапросов));

КонецПроцедуры

#Область ПредставлениеСтрокБаланса

Функция СчетаАвансыПоставщикам()

	Результат = Новый Массив;
	
	// 60.02 - Расчеты по авансам выданным
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным);

	// 60.22 - Расчеты по авансам выданным (в валюте)
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданнымВал);

	// 60.32 - Расчеты по авансам выданным (в у.е.)
	Результат.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданнымУЕ);
	
	Результат = БухгалтерскийУчет.СформироватьМассивСубсчетов(Результат);
...
Функция СчетаБезИнвентаризацииВАкте()
	...
	Результат = СчетаСтроки1190();
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, СчетаСтроки1260());
...
Функция СчетаПрочиеОборотныеАктивы()
	...
	// 76.27.9 - НДС по арендным обязательствам (в валюте)
	Результат.Добавить(ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствамВал);
...
Функция СчетаСтроки1190()
	
	Результат = Новый Массив;
	
	// 08.06 - Перевод молодняка животных в основное стадо
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат,
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПереводМолоднякаЖивотныхВОсновноеСтадо));
	
	// 08.07 - Приобретение взрослых животных
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат,
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПриобретениеВзрослыхЖивотных));
...
Процедура УдалитьСчетаПроцентыИНДСПоАренде(ТаблицаСчетов)
	
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.РасчетыПоАренде);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.РасчетыПоАрендеВал);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.РасчетыПоАрендеУЕ);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.ПроцентыПоАренде);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствам);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеВал);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствамВал);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеУЕ);
	УдалитьСчетИзТаблицы(ТаблицаСчетов, ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствамУЕ);
	
КонецПроцедуры

Процедура УдалитьСчетИзТаблицы(ТаблицаСчетов, СчетРасчетов)
	
	СтрокаТаблицы = ТаблицаСчетов.Найти(СчетРасчетов, "СчетРасчетов");
...
Функция СчетаПроцентыИНДСПоАренде()

	Результат = Новый Массив;
	Результат.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоАренде);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеВал);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеУЕ);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствам);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствамВал);
	Результат.Добавить(ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствамУЕ);
