...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета   = "0613001";
...
Функция ПолучитьМассивКатегорий()
	Возврат СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("П0010010001;П0010010002", ";", Истина);
КонецФункции
...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		НомСтроки = Прав(Стр, 1);
		
		ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		СтруктураРеквизитовФормы.Вставить("мИдГруппы" + НомСтроки, ИмяДобавляемогоРеквизита);
		...
		СтруктураРеквизитовФормы.мСоответствияМнгЧ.Вставить(ИмяДобавляемогоРеквизита, "МногострочнаяЧасть" + НомСтроки);
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникСекторыДеятельности",
		512, , "Выбор кода сектора деятельности", КоллекцияСписковВыбора["s_sekdel"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникКатегорииСоглашений",
		512, , "Выбор кода категории соглашения", КоллекцияСписковВыбора["s_org"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникОКВЭД2",
		512, , "Выбор кода ОКВЭД", КоллекцияСписковВыбора["S_OKVED2"]);
		
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "СправочникПереченьСтранМира",
		512, , "Выбор кода страны по ОКСМ", КоллекцияСписковВыбора["s_1000016075"]);
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Иначе
		...
		Если Область.Имя = "ОргКодЛКСД" Тогда
			ИмяСправочника = "СправочникСекторыДеятельности";
		ИначеЕсли СтрНайти(Область.Имя, "П001001000" + Сред(Область.Имя, 11, 1) + "01") > 0
			  Или СтрНайти(Область.Имя, "П001001000" + Сред(Область.Имя, 11, 1) + "05") > 0 Тогда
			  
			ИмяСправочника = "СправочникОКВЭД2";
			
			Если СтрНайти(Область.Имя, "_") > 0 Тогда
				ОбластьКод	 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "01_", Сред(Область.Имя, 1, 11) + "05_"));
				ОбластьНаим  = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "05_", Сред(Область.Имя, 1, 11) + "01_"));
			Иначе
				ОбластьКод	 = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, Сред(Область.Имя, 1, 11) + "01", Сред(Область.Имя, 1, 11) + "05"));
			КонецЕсли;
			
		ИначеЕсли СтрНайти(Область.Имя, "П001001000" + Сред(Область.Имя, 11, 1) + "04") > 0 Тогда
			ИмяСправочника = "СправочникКатегорииСоглашений";
		ИначеЕсли СтрНайти(Область.Имя, "П001001000" + Сред(Область.Имя, 11, 1) + "07") > 0 Тогда
			ИмяСправочника = "СправочникПереченьСтранМира";
		КонецЕсли;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНайти(ОбластьКод.Имя, "_") > 0 И Сред(ОбластьКод.Имя, 13, 1) <> "7" Тогда
		НаименованиеОбщ = "";
		Наименование1   = "";
		Наименование2   = "";
		
		Для Каждого Элемент Из СвойстваПоказателей Цикл
			Если Элемент.Показатель = "СправочникОКВЭД2" Тогда
				ОбластьГр5 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКод.Имя, 12) + "5" + Сред(ОбластьКод.Имя, 14)];
				Если ЗначениеЗаполнено(ОбластьГр5.Значение) Тогда
					МассивВыбора = Элемент.ТаблицаЗначений.НайтиСтроки(Новый Структура("Код", ОбластьГр5.Значение));
					Если МассивВыбора.Количество() = 1 Тогда
						Наименование1 = МассивВыбора[0].Название;
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли Элемент.Показатель = "СправочникКатегорииСоглашений" Тогда
				ОбластьГр4 = ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКод.Имя, 12) + "4" + Сред(ОбластьКод.Имя, 14)];
		...
		ПолеТабличногоДокументаФормаОтчета.Области[Лев(ОбластьКод.Имя, 12) + "1" + Сред(ОбластьКод.Имя, 14)].Значение = НаименованиеОбщ;
	КонецЕсли;
...
Процедура Расчет(Форма)
	...
	//Графа 9 = ∑ граф 10, 11, 12, 13.
	Для НомСтр = 1 По 2 Цикл
		
		Инд = 0;
		Пока Истина Цикл
			Инд = Инд + 1;
			НомИнд  = Формат(Инд, "ЧГ=");
			
			Обл = Области.Найти("П001001000" + НомСтр + "09_" + НомИнд);
			Если Обл = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			СуммаПоГр = 0;
			Для Гр = 10 По 13 Цикл
				НомГр = Формат(Гр, "ЧЦ=2; ЧВН=");
				СуммаПоГр = СуммаПоГр + Области["П001001000" + НомСтр + НомГр + "_" + НомИнд].Значение;
			КонецЦикла;
			Области["П001001000" + НомСтр + "09_" + НомИнд].Значение = СуммаПоГр;
		КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для НомСтр = 1 По 2 Цикл
		...
		Инд = 0;
		СтрокаИмяМнг = "П001001000" + НомСтр + "09_";
		Пока Истина Цикл
			Инд = Инд + 1;
			Обл = мФормаОтчета.Области.Найти(СтрокаИмяМнг + Формат(Инд, "ЧГ="));
			Если Обл = Неопределено Тогда
				Прервать;
			КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	СписокПоказателей.Добавить(ВыбТабПоле.Области["ОргКодЛКСД"].Значение,                "ОргКодЛКСД");
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Лицензия");
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_4")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Лицензия");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ПустаяСтрока"));
	КонецЕсли;
	...
	ВерхМногострочнойЧасти = ТабДок.Области["ОсновнаяЧасть1_4"].Низ + 1;
	...
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_5")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Лицензия");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
	ПечатнаяФорма.Вывести(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_5"));
	
	Если Не ПечатнаяФорма.ПроверитьВывод(ТабДок.ПолучитьОбласть("ОсновнаяЧасть1_6")) Тогда
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Лицензия");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргКодЛКСД) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
			"Не указан сектор деятельности по ЛКСД!",
			ИмяПоляТаблДокумента, "ОргКодЛКСД");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция СформироватьСтруктуруПараметров() Экспорт
	...
	ПараметрыВыгрузки.Вставить("ОргКодЛКСД", СокрЛП(ТабДокумент.ОргКодЛКСД));
