...
Процедура ПоказателиОценкиРискаВыезднойНалоговойПроверки(ТаблицаПоказателей, ОписаниеОтчета) Экспорт 
	...
	Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.Раздел1 Цикл
		...
		Пока ДанныеЛиста.Свойство("П000110002030" + Постфикс) Цикл
			ЗначениеПоказателя = ЗначениеПоказателя
			                   + РегламентированнаяОтчетность.ПоказательОтчета(ДанныеЛиста, "П000110002030" + Постфикс)
			                   - РегламентированнаяОтчетность.ПоказательОтчета(ДанныеЛиста, "П000110002040" + Постфикс);
	...
	Для Каждого Лист Из СведенияОтчета.ДанныеМногостраничныхРазделов.Раздел12 Цикл
		...
		Пока ДанныеЛиста.Свойство("П000120002030" + Постфикс) Цикл
			ЗначениеПоказателя = ЗначениеПоказателя
			                   + РегламентированнаяОтчетность.ПоказательОтчета(ДанныеЛиста, "П000120002030" + Постфикс)
			                   - РегламентированнаяОтчетность.ПоказательОтчета(ДанныеЛиста, "П000120002040" + Постфикс);
			НомерСтроки = НомерСтроки + 1;
			Постфикс = "_" + РегламентированнаяОтчетность.ЧислоВСтрокуЧГ0(НомерСтроки);
		КонецЦикла;
	КонецЦикла;
