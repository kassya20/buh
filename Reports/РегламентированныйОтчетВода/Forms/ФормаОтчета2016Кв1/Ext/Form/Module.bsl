...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("КоэффициентДефлятор");
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		СтруктураРеквизитовФормы.Вставить("КоэффициентДефлятор", 1);
	КонецЕсли;
	...
	Если НЕ БезОткрытияФормы Тогда
		...
		Элементы.ГруппаПредставлениеФормы.Видимость
			= КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) >= КонецДня('20260630');
	КонецЕсли;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если КонецДня(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) >= КонецДня('20260630')
		И СтруктураРеквизитовФормы.мСохраненныйДок.Вид = 0 И ТекущаяДатаСеанса() >= '20260705' Тогда
		
		СообщитьВТС("С 05.07.2026 представление первичной декларации, начиная с отчета за 2 квартал 2026 года, "
			+ "доступно только по форме, утвержденной приказом ФНС России от 26.03.2026 № ЕД-1-3/205@",
			...
			Если ЗначениеЗаполнено(ОбластьИмя)
				...
				Форма.ТабличныйДокумент.Области.П000210016003.Значение
					= РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма,
					  Форма.ТабличныйДокумент.Области.П000210010103.Значение
					* Окр(Форма.ТабличныйДокумент.Области.П000210011003.Значение
					* Стр130 * Стр140 * Стр150 * Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"])
					+ Форма.ТабличныйДокумент.Области.П000210010203.Значение
					* Окр(Форма.ТабличныйДокумент.Области.П000210012003.Значение
					* Стр130 * Стр140 * Стр150 * Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]));
			КонецЕсли;
			...
			Если Страница.АктивнаяСтраница И Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = ИмяЛиста Тогда
				...
				Если ГруппаКодов = "2" Тогда
					Строка130
						...
						* Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]) / 4;
				ИначеЕсли ГруппаКодов = "3" Тогда
					Строка130
						= Форма.ТабличныйДокумент.Области.П000220008003.Значение
						* Окр(Форма.ТабличныйДокумент.Области.П000220011003.Значение
						* Форма.ТабличныйДокумент.Области.П000220012003.Значение
						* Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]);
				ИначеЕсли ГруппаКодов = "4" Тогда
					Строка130
						...
						* Окр(Форма.ТабличныйДокумент.Области.П000220011003.Значение
						* Форма.ТабличныйДокумент.Области.П000220012003.Значение
						* Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]) / 100;
				КонецЕсли;
			...
			Иначе
				...
				Если ГруппаКодов = "2" Тогда
					Строка130
						...
						* Данные.П000220012003 * Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]) / 4;
				ИначеЕсли ГруппаКодов = "3" Тогда
					Строка130
						= Данные.П000220008003
						* Окр(Данные.П000220011003
						* Данные.П000220012003 * Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]);
				ИначеЕсли ГруппаКодов = "4" Тогда
					Строка130
						= Данные.П000220009003
						* Данные.П000220010003
						* Окр(Данные.П000220011003
						* Данные.П000220012003 * Форма.СтруктураРеквизитовФормы["КоэффициентДефлятор"]) / 100;
				КонецЕсли;
				Строка130 = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Строка130);
			КонецЕсли;
...
Процедура ПроверитьКС_2_10(СтруктураРезультата)
	...
	// Устарело. Отключено. Требуется включение к-дефлятора.
	Возврат;
...
Процедура ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, ОписаниеКС, ОбщееЧислоОшибок,
		ОбщееЧислоБезОшибок, Расшифровка, ЭтоОшибка, Описание = Неопределено, 
		Нарушение = "Письмо ФНС России от 07.09.2016 № СД-4-3/16671@",
