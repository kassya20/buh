...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	ИмяОбласти = ОбрабатываемаяОбласть.Имя;
	
	Если Лев(ИмяОбласти, 1) = "П"
		И ОбрабатываемаяОбласть.СодержитЗначение = Истина// здесь исключаем значение Неопределено
		И ТипЗнч(ОбрабатываемаяОбласть.Значение) = Тип("Число") Тогда
		ЦветФонаЯчейки = ОбрабатываемаяОбласть.ЦветФона;
