...
&НаСервере
Перем МенеджерОтчета; // ОтчетМенеджер.БухгалтерскаяОтчетностьВБанк - кэш объекта отчета для обращения
//                       к методам модуля менеджера без использования основного реквизита формы.
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Если ТабличныйДокумент.ТекущаяОбласть = Неопределено Тогда
		ОбрабатываемаяОбласть = ТабличныйДокумент.Область(1, 1);
	Иначе
		ОбрабатываемаяОбласть = ТабличныйДокумент.ТекущаяОбласть;
	КонецЕсли;
	...
	УстановитьДоступностьДопстрокВКонтекстномМеню(ОбрабатываемаяОбласть,
...
Процедура ПредставлениеБухОтчетностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПреставлениеПериодаБухотчетности = ПреставлениеПериодаБухОтчетности(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	ТекстЗаголовка = СтрШаблон(НСтр("ru = 'Выбор бухгалтерской отчетности за %1'"), ПреставлениеПериодаБухотчетности);
	
	ПараметрыФормыВыбора = Новый Структура;
	ПараметрыФормыВыбора.Вставить("ИсточникОтчета", "РегламентированныйОтчетБухОтчетность");
	ПараметрыФормыВыбора.Вставить("Организация", СтруктураРеквизитовФормы.Организация);
	ПараметрыФормыВыбора.Вставить("Заголовок", ТекстЗаголовка);
	ПараметрыФормыВыбора.Вставить("ДатаНачала", НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	ПараметрыФормыВыбора.Вставить("ДатаОкончания", КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		СтруктураРеквизитовФормы.ВерсияФормата = "5.13.02";
...
Процедура ИнициализироватьМногоуровневыеРазделы()
	...
	ИменаПодчиненныхРазделов.Добавить("Операции53");
	...
	СтруктураРаздела.Вставить("Подчиненность");
	СтруктураРаздела.Вставить("КорневойРаздел", "ОперацииПоБанковскимСчетам");
	мСтруктураМногоуровневыхРазделов.Вставить("Операции53", СтруктураРаздела);
...
Процедура ИнициализироватьДеревоРазделовОтчета()
	...
	ЗначениеНастройки = МенеджерОтчета().ЗначенияНастройкиВидимостиРазделов();
	...
	ПоказыватьЗаКвартал = ?(ЭтоОтчетностьЗаКвартал, ЗначениеНастройки.Показать, ЗначениеНастройки.Исключить);
	ДоступнаЗаКвартал = ?(ЭтоОтчетностьЗаКвартал, ЗначениеНастройки.НеПоказывать, ЗначениеНастройки.Исключить);
	
	ПоказыватьЗаКварталНеИП = ?(ЭтоОтчетностьЗаКвартал
		И НЕ ЭтоИП, ЗначениеНастройки.Показать, ЗначениеНастройки.Исключить);
	...
	СтрокаУровня1.ПоказатьСтраницу         = ЗначениеНастройки.Показать;
	...
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = ЗначениеНастройки.Исключить;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
	СтрокаУровня1.АвторасчетПоказателей    = Истина;
	СтрокаУровня1.СтраницаВывода           = "СтраницаОтчета";
	...
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = ЗначениеНастройки.Показать;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
	СтрокаУровня2.АвторасчетПоказателей    = Истина;
	СтрокаУровня2.СтраницаВывода           = "СтраницаОтчета";
	...
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = ЗначениеНастройки.Исключить;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
	СтрокаУровня2.АвторасчетПоказателей    = Истина;
	СтрокаУровня2.СтраницаВывода           = "СтраницаОтчета";
	...
	СтрокаУровня1.Представление            = "ОСВ (сводная)";
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = ПоказыватьЗаКвартал;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
	...
	СтрокаУровня1.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = ЗначениеНастройки.НеПоказывать;
	СтрокаУровня1.ВыгрузитьСтраницу        = 2;
	СтрокаУровня1.АвторасчетПоказателей    = Истина;
	СтрокаУровня1.СтраницаВывода           = "СтраницаОтчета";
	...
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = ЗначениеНастройки.НеПоказывать;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
	СтрокаУровня2.АвторасчетПоказателей    = Истина;
	СтрокаУровня2.СтраницаВывода           = "СтраницаОтчета";
	...
	СтрокаУровня2 = СтрокаУровня1.ПолучитьЭлементы().Добавить();
	СтрокаУровня2.ИмяСтраницы              = "Операции53";
	СтрокаУровня2.Представление            = "Карточка счета 53 «Счет цифрового рубля»";
	СтрокаУровня2.ОриентацияСтраницы       = "Портрет";
	СтрокаУровня2.ВыводНаПечать            = 1;
	СтрокаУровня2.ПоказатьСтраницу         = ЗначениеНастройки.НеПоказывать;
	СтрокаУровня2.ВыгрузитьСтраницу        = 2;
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СписокСохранения.Вставить("ВерсияСтруктурыДанных", МенеджерОтчета().ВерсияСтруктурыДанных(
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СведенияОтчета = МенеджерОтчета().СведенияОтчета(СтруктураРеквизитовФормы.мСохраненныйДок);
	СписокСохранения = СведенияОтчета.СохраненныеДанные;
	...
	ВосстановитьСохраненныеНастройкиДереваРазделов(СведенияОтчета.ДеревоНастройкиСтраниц);
...
Функция СборныйМакетРаздела(ИмяРаздела, ДанныеРаздела)
	...
	ИначеЕсли ИмяРаздела = "Операции51" ИЛИ ИмяРаздела = "Операции52"
	          ИЛИ ИмяРаздела = "Операции53" ИЛИ ИмяРаздела = "Операции55" Тогда
		Если ДанныеРаздела.ДополнительныеДанные <> Неопределено Тогда
...
Процедура СформироватьСекциюАнализаСчета_5_11_01(МакетРаздела, Шаблон, ДанныеРаздела, НомерСекции = 1)
	...
	ДатаКонцаОтчета  = КонецМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	Если ДанныеРаздела.ДополнительныеДанные <> Неопределено Тогда
		ДатаНачалаОтчета = НачалоМесяца(ДанныеРаздела.ДополнительныеДанные.НачалоПериода);
		ДатаКонцаОтчета  = КонецМесяца(ДанныеРаздела.ДополнительныеДанные.КонецПериода);
	КонецЕсли;
...
&НаСервере
Процедура ВосстановитьСохраненныеНастройкиДереваРазделов(СохраненноеДеревоНастройкиСтраниц)
	
	Если ТипЗнч(СохраненноеДеревоНастройкиСтраниц) <> Тип("ДеревоЗначений") Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияНастройки = МенеджерОтчета().ЗначенияНастройкиВидимостиРазделов();
	ИсходноеДеревоСтраниц = РеквизитФормыВЗначение("мДеревоСтраницОтчета");
	СтрокиИсходногоДерева = РегламентированнаяОтчетность.ВсеСтрокиДереваЗначений(ИсходноеДеревоСтраниц);
	СтрокиСохраненногоДерева = РегламентированнаяОтчетность.ВсеСтрокиДереваЗначений(СохраненноеДеревоНастройкиСтраниц);
	
	// Исключение всех разделов, чтобы видны были только разделы, включенные в сохраненый отчет,
	// включая те, которые были видимы в старых версиях отчета.
	Для Каждого СтрокаДерева Из СтрокиИсходногоДерева Цикл
		СтрокаДерева.ПоказатьСтраницу = 0;
	КонецЦикла;
	
	ИсключаемыеСвойства = "Представление";
	Если СохраненноеДеревоНастройкиСтраниц.Колонки.Найти("ЗаполняетсяПоИБ") <> Неопределено Тогда
		ИсключаемыеСвойства = ИсключаемыеСвойства + ", ЗаполняетсяПоИБ";
	КонецЕсли;
	
	Для Каждого СтрокаНастройкиСтраницы Из СтрокиСохраненногоДерева Цикл
	...
	// Исключение раздела замещающего бухгалтерскую отчетность с раздельными отчетама на выбор отчета
	// РегламентированныйОтчетБухОтчетность в старых версиях отчета.
	Если НЕ ДействуетСВерсииФормата("5.12.05") Тогда
		...
		Если РазделБухОтчетность <> Неопределено Тогда
			РазделБухОтчетность.ПоказатьСтраницу = ЗначенияНастройки.Исключить;
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыАвтозаполнения(ЗаполнятьВсеРазделы)
	...
	ДополнитьПараметрыАвтозаполнения_Операции53(ПараметрыАвтозаполнения, УсловияВключенияРазделов);
...
Процедура ДополнитьПараметрыАвтозаполнения_ОСВ(ПараметрыАвтозаполнения, УсловияВключенияРазделов)
	...
	Если Месяц(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета) % 3 <> 0 Тогда
		// Поддержка работы отчетов, сохраненных и формируемых за месяц.
		ПериодЗаполненияОтчета = НовыйПериодЗаполненияОтчета();
		ПериодЗаполненияОтчета.КонецПериода = КонецМесяца(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.НачалоПериода = НачалоМесяца(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.ПрефиксСегментов = "ОСВ";
		ПериодыЗаполнения.Добавить(ПериодЗаполненияОтчета);
	Иначе
		ПериодЗаполненияОтчета = НовыйПериодЗаполненияОтчета();
		ПериодЗаполненияОтчета.КонецПериода = КонецКвартала(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.НачалоПериода = НачалоКвартала(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.ПрефиксСегментов = "ОСВ";
		ПериодыЗаполнения.Добавить(ПериодЗаполненияОтчета);
	КонецЕсли;
...
Функция ДобавитьПериодЗаполненияОСВПоСчету(ПериодыЗаполнения)
	...
	ПериодЗаполнения = НовыйПериодЗаполненияОтчета();
...
Процедура ДополнитьПараметрыАвтозаполнения_АнализСчета(ПараметрыАвтозаполнения, УсловияВключенияРазделов)
	...
	Если Месяц(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета) % 3 <> 0 Тогда
		...
		ПериодЗаполненияОтчета.КонецПериода = КонецМесяца(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.НачалоПериода = НачалоМесяца(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
		ПериодЗаполненияОтчета.ПрефиксСегментов = "АнализСчета";
		ПериодыЗаполнения.Добавить(ПериодЗаполненияОтчета);
	Иначе
		ПериодЗаполненияОтчета = НовыйПериодЗаполненияОтчета();
		ПериодЗаполненияОтчета.КонецПериода = КонецКвартала(ПараметрыАвтозаполнения.мДатаКонцаПериодаОтчета);
...
&НаСервере
Процедура ДополнитьПараметрыАвтозаполнения_Операции53(ПараметрыАвтозаполнения, УсловияВключенияРазделов)
	
	ИмяРаздела = "Операции53";
	НомерСтраницы = УсловияВключенияРазделов.НомерСтраницыРаздела;
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ДвоичныеДанныеФайла, ПараметрыФормированияXML)
	...
	СтрокиДереваРазделов = РегламентированнаяОтчетность.ВсеСтрокиДереваЗначений(ДеревоРазделов);
	...
	Для Каждого СтрокаДерева Из СтрокиДереваРазделов Цикл
		Если СтрокаДерева.ИмяСтраницы = "Операции51" ИЛИ СтрокаДерева.ИмяСтраницы = "Операции52"
		     ИЛИ СтрокаДерева.ИмяСтраницы = "Операции53" ИЛИ СтрокаДерева.ИмяСтраницы = "Операции55" Тогда
...
Процедура ПроверитьЗаполнение_ОперацииПоБанковскимСчетам(СтатусПроверки, ПараметрыВыгрузки)
	...
	ДанныеРаздела53 = ДанныеМетаРаздела.Строки[0].Строки[2].Строки[0];
	ДанныеРаздела55 = ДанныеМетаРаздела.Строки[0].Строки[3].Строки[0];
	...
	Если РазделВключенВОтчет("Операции53", ПараметрыВыгрузки.ДеревоНастройкиСтраниц)
	     И ДанныеРаздела53.ДополнительныеДанные = Неопределено Тогда
		ОписаниеОшибки = "Операции по банковским счетам на счете 53 не заполнены";
		ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, ОписаниеОшибки, "Операции53");
	КонецЕсли;
...
Процедура ПроверитьЗаполнение_БухОтчетность(СтатусПроверки, ПараметрыВыгрузки)
	...
	ДанныеОтчета = КонтекстОбъектаОтчета.СохраненныеДанные(СтруктураРеквизитовФормы.Отчеты_Бухотчетность);
	
	// В Сбербанк передается только обычная отчетность. Для формы по ФСБУ 4/2023 вид сдачи отчетности не заполняется.
...
Процедура ВывестиСтраницуВУзел_ОСВ(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	НомерМесяца = Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	НомерКвартала = Месяц(КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)) / 3;
	Если НомерМесяца % 3 <> 0 Тогда
		// Поддержка работы отчетов, сохраненных и формируемых за месяц.
		КодПериода = КодМесяца(НомерМесяца);
	Иначе
		КодПериода = КодКварталаОСВ(НомерКвартала);
	КонецЕсли;
	...
	ПараметрыУзла.Период = КодПериода;
...
Процедура ВывестиУзел_ОперБанкСч(Компоновщик, ПараметрыВыгрузки)
	...
	ВывестиУзел_ОперБанкСч_СчетУчета(Компоновщик, ПараметрыВыгрузки, "Операции53");
...
Процедура ВывестиУзел_ОперБанкСч_СчетУчета(Компоновщик, ПараметрыВыгрузки, ИмяРаздела)
	...
	ИначеЕсли ИмяРаздела = "Операции52" Тогда
		ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[1].Строки[0];
	ИначеЕсли ИмяРаздела = "Операции53" Тогда
		ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[2].Строки[0];
	ИначеЕсли ИмяРаздела = "Операции55" Тогда
		ДанныеРаздела = ДанныеМетаРаздела.Строки[0].Строки[3].Строки[0];
	КонецЕсли;
	...
	Для Каждого СтрокаСчета Из СтрокиСчетов Цикл
		...
		Если ОписаниеТипаБанковскиеСчета.СодержитТип(ТипЗнч(СтрокаСчета.Счет)) Тогда
			...
			Если НЕ ТребуетсяРазыменование Тогда
				...
				Если ДействуетСВерсииФормата("5.13.02")
				     И РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(СтрокаСчета, "ЭтоЦифровойСчет")
				     И СтрокаСчета.ЭтоЦифровойСчет Тогда
					ВывестиНеобязательныйАтрибут(Компоновщик, "ИдКошелькаЦР", СтрокаСчета.ИдентификаторЦифровогоСчета, "Т,70");
				КонецЕсли;
						...
						Для Каждого СтрокаСекции Из СтрокиСекции Цикл
							...
							Если ЗначениеЗаполнено(СтрокаСекции.Контрагент) Тогда
								...
								Если ДействуетСВерсииФормата("5.11.01") Тогда
									...
									Если ДействуетСВерсииФормата("5.13.02")
									     И СтрокаСекции.СчетКонтрагентаЭтоЦифровойСчет Тогда
										ВывестиНеобязательныйАтрибут(Компоновщик, "ИдКошелькаЦР", СтрокаСекции.СчетКонтрагентаИдЦифровогоСчета, "Т,70");
									КонецЕсли;
...
Функция КодКвартала(НомерКвартала)
	...
	Иначе
		ИмяФункции = "Отчет.БухгалтерскаяОтчетностьВБанк.ФормаОтчета2017Кв3.КодКвартала";
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Функция %1:
			|Непредусмотренный номер квартала'"), ИмяФункции);
...
Функция КодКварталаОСВ(НомерКвартала)
	...
	Иначе
		ИмяФункции = "Отчет.БухгалтерскаяОтчетностьВБанк.ФормаОтчета2017Кв3.КодКварталаОСВ";
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Функция %1:
			|Непредусмотренный номер квартала'"), ИмяФункции);
...
Процедура ОткрытьФормуВыбораСтраниц(ВариантВыбора)
	...
	НастройкиСтраниц = РегламентированнаяОтчетностьКлиентСервер.РеквизитДеревоЗначенийВСтруктуру(мДеревоСтраницОтчета,
		"Представление, ВыгрузитьСтраницу, ПоказатьСтраницу, ВыводНаПечать, ОриентацияСтраницы,
	...
	ПараметрыФормы.Вставить("НастройкиСтраниц", НастройкиСтраниц);
	...
	ОткрытьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкиОтчета",
...
Процедура НастроитьОтчетПоСчетам(ИмяРаздела)
	...
	СписокСтраницРаздела = ЭтотОбъект["СписокСтраниц" + ИмяРаздела];
	
	СчетаДляВыбора = РегламентированнаяОтчетностьКлиентСервер.РеквизитТаблицаЗначенийВМассив(СписокСтраницРаздела,
		"Включен, Счет, Наименование");
	ПараметрыФормы = Новый Структура("СчетаДляВыбора", СчетаДляВыбора);
	...
	ОткрытьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкаСпискаСчетов",
		ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СохранитьПредставлениеОтчетаВXLSX(СтрокаРаздела, ПараметрыСохраненияПредставлений, СформированныеФайлы)
	...
	ИначеЕсли ИмяРаздела = "ФНС" ИЛИ ИмяРаздела = "ДополнительныеФайлы" ИЛИ ИмяРаздела = "Титульный"
	          ИЛИ ИмяРаздела = "Операции51" ИЛИ ИмяРаздела = "Операции52"
	          ИЛИ ИмяРаздела = "Операции53" ИЛИ ИмяРаздела = "Операции55"
...
&НаКлиенте
Процедура УстановитьДоступностьДопстрокВКонтекстномМеню(АктивнаяОбласть,
	КонтекстноеМенюУдалитьСтроку, КонтекстноеМенюДобавитьСтроку)
	
	КонтекстноеМенюУдалитьСтроку.Доступность = Ложь;
	КонтекстноеМенюДобавитьСтроку.Доступность = Ложь;
	
	ИмяГруппы = МногострочнаяЧастьТекущейОбласти(ЭтотОбъект, АктивнаяОбласть.Имя);
...
// Конструктор описания периода заполнения отчета.
// 
// Возвращаемое значение:
//  Структура:
//   * НачалоПериода - Дата - 
//   * КонецПериода - Дата - 
//   * ПрефиксСегментов - Строка -
//
&НаСервере
Функция НовыйПериодЗаполненияОтчета()
...
// Вовзращает ссылку на менеджер отчета в памяти для вызова методов модуля менеджера этого объекта.
// 
// Возвращаемое значение:
...
Функция МенеджерОтчета()
	...
	Если МенеджерОтчета = Неопределено Тогда
		ИмяОтчета = СтрРазделить(ИмяФормы, ".")[1];
		МенеджерОтчета = РегламентированнаяОтчетностьВызовСервера.ОтчетОбъект(ИмяОтчета);
	КонецЕсли;
	
	Возврат МенеджерОтчета;
