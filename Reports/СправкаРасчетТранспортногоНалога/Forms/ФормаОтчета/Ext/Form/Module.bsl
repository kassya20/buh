...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки)
	...
	Если Не Отчет.РежимРасшифровки Тогда
		...
		Если Отчет.Группировка.НайтиСтроки(Новый Структура("Поле", "Группировка.ДатаСведений")).Количество() = 0 Тогда
			ИнициализироватьГруппировки();
		КонецЕсли;
		УстановитьПериодОтчета();
		ОбновитьТекстЗаголовка(ЭтотОбъект);
...
Процедура НадписьПереходОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОткрытьФорму("Отчет.СправкаРасчетТранспортногоНалогаДо2020.Форма", ПараметрыФормы);
...
Процедура ГруппировкаПриИзменении(Элемент)
	...
	// Отменим перемещение предопределенных группировок.
	// Последние 2 группировки всегда предопределенные.
	КоличествоГруппировок = Отчет.Группировка.Количество();
	
	// Предпоследняя
	Если Не Отчет.Группировка[КоличествоГруппировок - 2].Предопределенная Тогда
		Отчет.Группировка.Сдвинуть(КоличествоГруппировок - 2, -1);
	КонецЕсли;
	
	// Последняя
	Если Не Отчет.Группировка[КоличествоГруппировок - 1].Предопределенная Тогда
		Отчет.Группировка.Сдвинуть(КоличествоГруппировок - 1, -2);
	КонецЕсли;
