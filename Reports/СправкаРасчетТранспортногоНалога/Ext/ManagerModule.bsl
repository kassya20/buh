...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	// В структуре настроек есть таблицы для разных случаев.
	// Выберем нужную, остальные удалим ниже.
	ИмяГруппировки = "";
	
	Если КонецКвартала(ПараметрыОтчета.Период) <> КонецГода(ПараметрыОтчета.Период) Тогда
		ИмяГруппировки = "РасчетТранспортногоНалога_Аванс";
	Иначе
		ИмяГруппировки = "РасчетТранспортногоНалога";
	КонецЕсли;
...
Процедура ПослеВыводаРезультата(ПараметрыОтчета, Результат) Экспорт
	...
	// Графы 4-8: вид ТС, идентификационный номер, марка, ОКТМО и код НО
	...
	// Графы 9-11: даты изменений
	НомерПервойКолонки = 9;
	НомерПоследнейКолонки = 11;
