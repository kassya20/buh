...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СформироватьПриОткрытии = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "СформироватьПриОткрытии", Ложь);
	...
	Если Параметры.Свойство("СформироватьОбложку") Тогда
		Отчет.СформироватьОбложку = Параметры.СформироватьОбложку;
	КонецЕсли;
	...
	Если Не Отчет.РежимРасшифровки
		И БухгалтерскийУчетПереопределяемый.ВестиУчетПоПодразделениям()
		И НЕ ЗначениеЗаполнено(Отчет.ПодразделениеОрганизации) Тогда
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки, ИспользуютсяСтандартныеНастройки)
	...
	Если Отчет.РежимРасшифровки Или ИспользуютсяСтандартныеНастройки Тогда
...
Процедура РезультатВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если Результат.Области.Найти("ОповеститьОПроблеме") = Неопределено
	 Или Область.Верх <> Результат.Области.ОповеститьОПроблеме.Верх Тогда
		Возврат;
	КонецЕсли;
...
Функция СформироватьОтчетНаСервере(Знач ГраницыПересчетаЛистов = Неопределено)
	...
	Отчеты.КассоваяКнига.ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, ЭтотОбъект);
	...
	ОписаниеЗаданияФормированияОтчета = ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыЗадания, "Отчеты.КассоваяКнига.ПодготовитьОтчет", ПараметрыОтчета);
...
Процедура ЗагрузитьПодготовленныеДанные(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Статус = "Ошибка" Тогда
		...
		Сообщить(РезультатВыполнения.КраткоеПредставлениеОшибки, СтатусСообщения.Важное);
	...
	ОповеститьОПересчете = ПересчитатьНомераЛистов;
	
	ЗагрузитьПодготовленныеДанныеНаСервере(РезультатВыполнения);
	
	Если ОповеститьОПересчете Тогда
...
Процедура ЗагрузитьПодготовленныеДанныеНаСервере(РезультатВыполнения)
	...
	ВозвращаемыеПараметры = ПолучитьИзВременногоХранилища(РезультатВыполнения.АдресРезультата);
	...
	Результат = ВозвращаемыеПараметры.Результат;
	ВозвращаемыеПараметры.Результат = Неопределено;
	Результат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КассоваяКнига";
	//В мобильном клиенте не поддерживается изменение в компактном режиме
	Если Не ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Результат.ТолькоПросмотр = Истина;
	КонецЕсли;
	
	ПоказатьОповещениеОНумерации = ВозвращаемыеПараметры.ПоказатьОповещениеОНумерации;
	ПоказатьТабличныйДокумент(Результат);
	...
	Если ПересчитатьНомераЛистов Тогда
		...
		ПараметрыОтчета = Отчеты.КассоваяКнига.ПустыеПараметрыКомпоновкиОтчета();
		Отчеты.КассоваяКнига.ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, ЭтотОбъект);
		НомерЛистаНачало = ПараметрыОтчета.НомерЛистаНачало;
...
Процедура ПоказатьТабличныйДокумент(ТабДокумент)
	...
	Если ПоказатьОповещениеОНумерации Тогда
		ОбластьПредупреждения = ТабДокумент.Область(2, 1, ТабДокумент.ВысотаТаблицы, ТабДокумент.ШиринаТаблицы);
		ТабДокумент.ОбластьПечати  = ОбластьПредупреждения;
...
Функция ПолучитьДополнительныеПараметры(Знач ПараметрыОтчета)
	
	Результат = Новый Структура("Кассир, ГлавныйБухгалтер, НомерЛистаНачало");
	
	ОтветственныеЛица = Отчеты.КассоваяКнига.ПолучитьОтветственныхИУполномоченных(ПараметрыОтчета);
	ЗаполнитьЗначенияСвойств(Результат, ОтветственныеЛица);
	Результат.НомерЛистаНачало = Отчеты.КассоваяКнига.ПолучитьНомерЛистаНачало(ПараметрыОтчета);
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаОрганизация);
	...
	ПолеОрганизация = Элементы.Найти("ПолеОрганизация");
	Если ПолеОрганизация <> Неопределено Тогда
		ПолеОрганизация.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	КонецЕсли;
