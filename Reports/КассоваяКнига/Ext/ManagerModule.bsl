...
Процедура ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, Форма) Экспорт
	...
	ПараметрыОтчета.ПодразделениеОрганизации   = Отчет.ПодразделениеОрганизации;
	ПараметрыОтчета.ФормироватьПоПодразделению = Форма.ФормироватьПоПодразделению;
	
	ПараметрыОтчета.ПроверитьНомера            = Форма.ПроверитьНомераЛистовСНачалаГода;
	ПараметрыОтчета.ПересчитатьНомераЛистов    = Форма.ПересчитатьНомераЛистов;
	ПараметрыОтчета.ПоследнийЛист              = Отчет.ПоследнийЛист;
	ПараметрыОтчета.НомерЛистаНачало           = Форма.НомерЛистаНачало;
	ПараметрыОтчета.СписокСформированныхЛистов = Форма.СписокЛистов;
	
	ПараметрыОтчета.ВыводитьОснования          = Отчет.ВыводитьОснования;
	ПараметрыОтчета.СформироватьОбложку        = Отчет.СформироватьОбложку;
	
	ПараметрыОтчета.ПлатежныйАгент             = Отчет.ПлатежныйАгент;
	ПараметрыОтчета.ГлавныйБухгалтер           = Форма.ГлавныйБухгалтер;
	ПараметрыОтчета.Кассир                     = Форма.Кассир;
...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	ВозвращаемыеПараметры = ПодготовитьОтчет(ПараметрыОтчета);
...
Функция ЗаполнитьПараметрыОтчета(ПараметрыОтчета) Экспорт
	...
	ОтветственныеЛица = Отчеты.КассоваяКнига.ПолучитьОтветственныхИУполномоченных(ПараметрыОтчета);
	ЗаполнитьЗначенияСвойств(ПараметрыОтчета, ОтветственныеЛица);
	
	ПараметрыОтчета.НомерЛистаНачало = ПолучитьНомерЛистаНачало(ПараметрыОтчета);
	...
	Если КонецДня(ПараметрыОтчета.КонецПериода) = КонецГода(ПараметрыОтчета.КонецПериода) Тогда
		ПараметрыОтчета.ПоследнийЛист       = 2;
	ИначеЕсли КонецДня(ПараметрыОтчета.КонецПериода) = КонецМесяца(ПараметрыОтчета.КонецПериода) Тогда
		ПараметрыОтчета.ПоследнийЛист       = 1;
	Иначе
		ПараметрыОтчета.ПоследнийЛист       = 0;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыОтчета.ПодразделениеОрганизации) Тогда
		ПараметрыОтчета.ФормироватьПоПодразделению = Истина;
	КонецЕсли;
...
Функция ПодготовитьОтчет(ПараметрыОтчета) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		ВозвращаемыеПараметры.КраткоеПредставлениеОшибки = НСтр("ru = 'Не установлен отбор по организации'");
	...
	ОтветственныеЛица = ПолучитьФИООтветственныеЛицаКассовойКниги(ПараметрыОтчета,, КонецДня(ПараметрыОтчета.КонецПериода));
	...
	Если ПараметрыОтчета.НачалоПериода > НачалоГода
		И (Не ПараметрыОтчета.ПроверитьНомера Или ПараметрыОтчета.ПересчитатьНомераЛистов) Тогда
		...
		Иначе
			...
			Если ТекНомера.Количество() > 0 И ТекНомера[0].Период >= НачалоГода И ТекНомера[0].НомерЛиста <> ПараметрыОтчета.НомерЛистаНачало Тогда
				НумерацияЛистовИзменилась               = Истина;
				ЛистовЗаГод         = ПараметрыОтчета.НомерЛистаНачало - 1;
	...
	Иначе
		ДатаНачальногоЛиста = НачалоГода;
		ПараметрыОтчета.НомерЛистаНачало = 1;
	КонецЕсли;
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ВТ_КассовыеДокументы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВозвратРозничномуПокупателю)
	|						ТОГДА ВЫБОР
...
//		ЛистовЗаГод = 1 стр
		...
		Для каждого Документ Из ТаблицаДокументы Цикл
			...
			Если (ЦелаяСтраница = КоличествоСтраниц ИЛИ
				...
				Если ДатаЛиста >= ПараметрыОтчета.НачалоПериода Тогда
					...
					ВозвращаемыеПараметры.СписокСформированныхЛистов.Добавить("3" + ИмяОбласти, "Лист " + Формат(НомерЛистаДляПерехода, "ЧН=; ЧГ="));
		...
		Если ДатаЛиста >= ПараметрыОтчета.НачалоПериода Тогда
			...
			ОтветственныеЛица = ПолучитьФИООтветственныеЛицаКассовойКниги(ПараметрыОтчета, ОтветственныеЛица, КонецДня(ДатаЛиста));
		...
		Если Ошибка Тогда
			ВозвращаемыеПараметры.ДатыЛистовСОшибками.Добавить(ДатаЛиста);
		КонецЕсли;
		...
		Если Не ПараметрыОтчета.ПроверитьНомера Или ДатаЛиста >= ПараметрыОтчета.НачалоПериода Тогда
			ВывестиЛисты = Истина;
			...
			ЛистыКассовойКниги.Вывести(ДокументРезультат);
			ВозвращаемыеПараметры.СписокСформированныхЛистов.Добавить("3" + ИмяОбласти, "Лист " + Формат(НомерЛиста, "ЧН=; ЧГ="));
	...
	Если СведенияОНарушеннойНумерации.СтекСообщенийОНумерации.Количество() > 0 Тогда
		ОповеститьОПроблемеСНумерацией(ВозвращаемыеПараметры.Результат, ПараметрыОтчета, СведенияОНарушеннойНумерации);
		ВозвращаемыеПараметры.ПоказатьОповещениеОНумерации = Истина;
	КонецЕсли;
	
	// Печать обложки и титульного листа
	СформироватьОбложку(ВозвращаемыеПараметры.Результат, ПараметрыОтчета, ДатаЛиста, ПредставлениеПодразделения);
	
	Если ВывестиЛисты Тогда
		ВозвращаемыеПараметры.Результат.Вывести(ЛистыКассовойКниги);
	КонецЕсли;
	
	// Последний лист кассовой книги
	СформироватьЗавершающийЛист(ВозвращаемыеПараметры.Результат, ПараметрыОтчета, ЛистовЗаГод, ОтветственныеЛица);
	
	Для Каждого Лист Из ПараметрыОтчета.СписокСформированныхЛистов Цикл
		ВозвращаемыеПараметры.СписокСформированныхЛистов.Добавить(Лист.Значение, Лист.Представление);
	КонецЦикла;
	
	ВозвращаемыеПараметры.СписокСформированныхЛистов.СортироватьПоЗначению();
	ВозвращаемыеПараметры.Результат.ОтображатьСетку     = Ложь;
	ВозвращаемыеПараметры.Результат.ОтображатьЗаголовки = Ложь;
	ВозвращаемыеПараметры.Результат.ТолькоПросмотр      = Истина;
...
Функция НовыйВозвращаемыеПараметры()
	...
	ВозвращаемыеПараметры.Вставить("ДатыЛистовСОшибками",          Новый Массив);
	ВозвращаемыеПараметры.Вставить("ПоказатьОповещениеОНумерации", Ложь);
	
	// Итоговый табличный документ листа кассовой книги
...
Функция ПолучитьОтветственныхИУполномоченных(ПараметрыОтчета) Экспорт
	
	ОтветственныеЛица = Новый Структура("ГлавныйБухгалтер, Кассир, НомерЛистаНачало");
...
Функция ПолучитьНомерЛистаНачало(ПараметрыОтчета) Экспорт
	
	Возврат РегистрыСведений.НомераЛистовКассовойКниги.ПолучитьНомерЛистаНачало(ПараметрыОтчета);
	
КонецФункции

Функция ПолучитьФИООтветственныеЛицаКассовойКниги(Параметры, ТекущиеОтветственные = Неопределено, ДатаСреза)
