...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил34Декларация" Тогда
		
		Запись.П000010000302 = СокрЛП(Запись.П000010000302);
		Запись.П000010000313 = СокрЛП(Запись.П000010000313);
		
		Таб = ЗагрузитьТаблицу("ТаблицаПоставщиков", Запись);
		...
		Для каждого СтрокаТЗ Из Таб Цикл
			...
			СтрокаТЗ.Наименование = СокрЛП(СтрокаТЗ.Наименование);
			СтрокаТЗ.ИНН = СокрЛП(СтрокаТЗ.ИНН);
			СтрокаТЗ.КПП = СокрЛП(СтрокаТЗ.КПП);
			
			Запись.П000010000304 = Запись.П000010000304 + ?(НомерСтроки = 1, "", Символы.ПС) + СтрокаТЗ.Наименование;
			Запись.П000010000305 = Запись.П000010000305 + ?(НомерСтроки = 1, "", Символы.ПС) + СтрокаТЗ.ИНН;
			Запись.П000010000306 = Запись.П000010000306 + ?(НомерСтроки = 1, "", Символы.ПС) + СтрокаТЗ.КПП;
...
Функция ЗагрузитьТаблицу(ИмяТаблицы, Запись)
	...
	Если ЗначениеЗаполнено(Запись[ИмяТаблицы])
		// Возможен вызов при заполнении из таблицы, а не из регистра, поэтому проверяем тип значения.
		И ТипЗнч(Запись[ИмяТаблицы]) = Тип("Строка")
...
Процедура РасчетРаздел1(Запись)
	...
	Для Индекс = 2 По 13 Цикл
		...
		Если Индекс = 2 ИЛИ (Индекс >= 4 И Индекс <= 6) ИЛИ Индекс = 13 Тогда
			Запись[ИмяРесурса] = ?(Запись[ИмяРесурса] = Неопределено, "", Запись[ИмяРесурса]);
		Иначе
	...
	Запись["ТаблицаПоставщиков"] = ?(Запись["ТаблицаПоставщиков"] = Неопределено, "", Запись["ТаблицаПоставщиков"]);
	
	// Всего для производства.
	Запись.П000010000308 	= Запись.П000010000309 + Запись.П000010000310 + Запись.П000010000311;
	
	// По схеме выгрузки возможно или заполнение поля Закуплено и таблицы Поставщиков,
	// или - заполнение поля Собрано.
	
	ТаблицаПоставщиков = ЗагрузитьТаблицу("ТаблицаПоставщиков", Запись);
	
	Если Запись.П000010000303 > 0 Тогда
	
		Запись.П000010000307 = 0;
		Запись.ТаблицаПоставщиков = "";
		
	ИначеЕсли Запись.П000010000307 > 0 ИЛИ ТаблицаПоставщиков.Количество() > 0 Тогда
		Запись.П000010000303 = 0;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеТаблицыДекларация(СтатусПроверки, ИдГруппы,
	...
	ИмяСобрано  = ИдГруппы + "03";
	ИмяНаименованиеПоставщикаСумма = ИдГруппы + "04";
	ИмяИННПоставщикаСумма = ИдГруппы + "05";
	ИмяКПППоставщикаСумма = ИдГруппы + "06";
	ИмяЗакуплено = ИдГруппы + "07";
	ИмяИспользованоВсего = ИдГруппы + "08";
	ИмяИспользованоВиноГеогрУказания	 = ИдГруппы + "09";
	ИмяИспользованоВиноНаименМеста	 = ИдГруппы + "10";
	ИмяИспользованоДисстилятов	 = ИдГруппы + "11";
	ИмяПоставлено	 = ИдГруппы + "12";
	ИмяНомерПартии	 = ИдГруппы + "13";
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаДекларации Из РезЗапросаДанных Цикл
			...
			// Проверяем одновременное заполнение Собрано и Закуплено.
			Собрано = СтрокаДекларации[ИмяСобрано];
			Закуплено = СтрокаДекларации[ИмяЗакуплено]; 
			ЕстьСтрокиПоставщиков = ЗначениеЗаполнено(СтрЗаменить(СтрокаДекларации[ИмяИННПоставщикаСумма], Символы.ПС, ""));
			
			ИмяПоказателя = ИмяСобрано + "_" + НомерСтрокиСтроковый;
			
			Если Собрано > 0 И (Закуплено > 0 ИЛИ ЕстьСтрокиПоставщиков) Тогда
			    
				ТекстОписанияОшибки = СтрокаГде + " одновременно заполненны объем собранного винограда и объем закупленного (или не пуст список поставщиков)!";
				...
				СтруктураОшибки.Вставить("Показатель", ИмяПоказателя);
				СтруктураОшибки.Вставить("ЛистРаздела", НомерЛиста);
				СтруктураОшибки.Вставить("ТипОшибки", "ОдновременноСобраноЗакуплено");
			...
			// Проверяем наличие Поставщиков при заполненном поле Закуплено.
			ИмяПоказателя = ИмяЗакуплено + "_" + НомерСтрокиСтроковый;
			
			Если Закуплено > 0 И (НЕ ЕстьСтрокиПоставщиков) Тогда
			    			
				ТекстОписанияОшибки = СтрокаГде + " заполнено поле объем закупленного винограда, но нет заполненных строк в таблице поставщиков!";
				...
				СтруктураОшибки.Вставить("Показатель", ИмяПоказателя);
				СтруктураОшибки.Вставить("ЛистРаздела", НомерЛиста);
				СтруктураОшибки.Вставить("ТипОшибки", "НетСтрокПоставщиков");
			...
			Иначе
				...
				Если Закуплено > 0 Тогда
					
					ИмяПоказателяНаименования = ИмяНаименованиеПоставщикаСумма + "_" + НомерСтрокиСтроковый;
					ИмяПоказателяИНН = ИмяИННПоставщикаСумма + "_" + НомерСтрокиСтроковый;
					ИмяПоказателяКПП = ИмяКПППоставщикаСумма + "_" + НомерСтрокиСтроковый;
					
					ТаблицаПоставщиков = ЗагрузитьТаблицу("ТаблицаПоставщиков", СтрокаДекларации);
					
					КолСтрокПоставщиков = ТаблицаПоставщиков.Количество();
					
					Для ИндСтрокиПоставщиков = 0 По КолСтрокПоставщиков - 1 Цикл
						
						ТекСтрокаПоставщиков =  ТаблицаПоставщиков[ИндСтрокиПоставщиков];
						
						Если ЗначениеЗаполнено(ТекСтрокаПоставщиков.Наименование 
												+ ТекСтрокаПоставщиков.ИНН 
												+ ТекСтрокаПоставщиков.КПП) 
							Тогда
						
							// Проверяем верность заполнения Наименования, ИНН, КПП
							НомСтрокиТабПоставщиков = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(
																					ИндСтрокиПоставщиков + 1);
							СтрокаГдеТабПоставщиков = СтрокаГде + " в таблице Поставщиков, в строке " 
													+ НомСтрокиТабПоставщиков + ",";
							//
							////////////////////////////////////////////////////////////////////////////////
							...
							МинДлинаИННпоXSD = 1;
							МаксДлинаИННпоXSD = 12;
							
							КПП56СимволыМогутБытьБуквами = Ложь;
							
							ИННLiteТип = (СтрДлина(ТекСтрокаПоставщиков.ИНН) = 10) И ТолькоРФ;
							КППLiteТип = (СтрДлина(ТекСтрокаПоставщиков.ИНН) = 10) И ТолькоРФ;
							
							СтруктураСведенийИННКПП = Новый Структура;

							СтруктураСведенийИННКПП.Вставить("ИмяРаздела", ИмяРаздела);

							СтруктураСведенийИННКПП.Вставить("СтрокаГде", СтрокаГдеТабПоставщиков);
							...
							СтруктураСведенийИННКПП.Вставить("ИмяТаблицы", "в таблице Поставщики");
							...
							СтруктураСведенийИННКПП.Вставить("Наименование", ТекСтрокаПоставщиков.Наименование);

							СтруктураСведенийИННКПП.Вставить("ИмяПоказателяИНН", ИмяПоказателяИНН);
							СтруктураСведенийИННКПП.Вставить("ИНН", ТекСтрокаПоставщиков.ИНН);
							СтруктураСведенийИННКПП.Вставить("ИмяПоказателяКПП", ИмяПоказателяКПП);
							СтруктураСведенийИННКПП.Вставить("КПП", ТекСтрокаПоставщиков.КПП);
							...
							СтруктураСведенийИННКПП.Вставить("КодСтраны", "643");
			...
			ИмяПоказателяПустаяСтрока = ИмяСобрано + "_" + НомерСтрокиСтроковый;
			...
			// Всего использовано должно быть не больше суммы собранного и полученного винограда за минусом поставленного. 
			ИспользованоВсего 	= Число(СтрокаДекларации[ИмяИспользованоВсего]);
			Собрано 			= Число(СтрокаДекларации[ИмяСобрано]);
			Закуплено 			= Число(СтрокаДекларации[ИмяЗакуплено]);
			Поставлено			= Число(СтрокаДекларации[ИмяПоставлено]);
			
			Если ИспользованоВсего > Собрано + Закуплено - Поставлено Тогда
			
				ТекстОписанияОшибки = СтрокаГде + " использовано для производства больше винограда, чем собрано и закуплено за минусом поставленного!";
			...
			Если ЕстьИдОП Тогда
				...
				Если ТекущееКоличествоОшибок = 0 Тогда
					...
					Для Счетчик = НачальныйИндекс По КоличествоКолонокВРазделе Цикл
						...
						Если Счетчик >= 4 И Счетчик <= 6 Тогда
							// Колонки для печати поставщиков.
							Продолжить;
						КонецЕсли;
						...
						Если Счетчик = 2 ИЛИ Счетчик = 3 ИЛИ Счетчик = 7 Тогда							
						...
						Если Счетчик = 3 Тогда
							...
							ДанныеДвижения.Вставить("ОбъемСобр", Новый Соответствие);
							ДанныеДвижения["ОбъемСобр"].Вставить(КлючКолонки, ЗначениеПоКолонке);
							
						ИначеЕсли Счетчик = 7 Тогда
							
							ДанныеДвижения.Вставить("Поставщик", Новый Соответствие);
							ДанныеДвижения["Поставщик"].Вставить(КлючКолонки, ЗначениеПоКолонке);
					...
					Если Закуплено > 0 Тогда 
						...
						ДанныеДвижения["Поставщик"].Вставить("Поставщики", Новый Соответствие);
						...
						Для ИндСтрокиПоставщиков = 0 По КолСтрокПоставщиков - 1 Цикл
							...
							ТекСтрокаПоставщиков =  ТаблицаПоставщиков[ИндСтрокиПоставщиков];
							ИндСтрокиПоставщиковСтроковый = РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(ИндСтрокиПоставщиков);
							...
							Если ЗначениеЗаполнено(ТекСтрокаПоставщиков.Наименование 
								...
								ИдПоставщика  = "X" + РегламентированнаяОтчетностьАЛКО.ПолучитьХешИдОрганизации(
																ТекСтрокаПоставщиков.Наименование,
																ТекСтрокаПоставщиков.ИНН,
																ТекСтрокаПоставщиков.КПП);
								...
								Если СправочникиОтчета["Поставщики"].Получить(ИдПоставщика) = Неопределено Тогда
									...
									СправочникиОтчета["Поставщики"][ИдПоставщика].Вставить("П000000000004", СокрЛП(ТекСтрокаПоставщиков.Наименование));
									СправочникиОтчета["Поставщики"][ИдПоставщика].Вставить("П000000000005", СокрЛП(ТекСтрокаПоставщиков.ИНН));
									СправочникиОтчета["Поставщики"][ИдПоставщика].Вставить("П000000000006", СокрЛП(ТекСтрокаПоставщиков.КПП));

								КонецЕсли;
								
								ИдПостав = СправочникиОтчета["Поставщики"][ИдПоставщика]["ИдПостав"];
								
								ДанныеДвижения["Поставщик"]["Поставщики"].Вставить(
												ИндСтрокиПоставщиковСтроковый + ИдПоставщика, ИдПостав); 
...
Процедура ЗаполнитьСправочникиДекларации(УзелСправочники)
	...
	Для Каждого ЭлементСправочникиПоставщики Из СправочникиОтчета["Поставщики"] Цикл
		...
		УстановитьЗначениеЭлемента(Узел_Поставщики, "П000000000004", СтрокаПоставщика["П000000000004"]);
		
		УказанКПП = НЕ ПустаяСтрока(СтрокаПоставщика["П000000000006"]);
		...
		Если УказанКПП Тогда
			УстановитьЗначениеЭлемента(Узел_ЮЛ, "П000000000005", СтрокаПоставщика["П000000000005"]);
			УстановитьЗначениеЭлемента(Узел_ЮЛ, "П000000000006", СтрокаПоставщика["П000000000006"]);
			РегламентированнаяОтчетность.УдалитьУзел(Узел_ФЛ);
		Иначе
			УстановитьЗначениеЭлемента(Узел_ФЛ, "П000000000005", СтрокаПоставщика["П000000000005"]);
	...
	Для Каждого ЭлементДвиженияПоОП Из ДанныеДвиженийПоОП Цикл
		...
		Для каждого ЭлМассива Из МассивДвиженийПоИдУчетныйНомерНомерПартии Цикл
			...
			Для каждого КлючЗначение  Из ДанныеДвижения Цикл
				...
				Если КлючЗначение.Ключ = "ОбъемСобр" Тогда
					
					НовыйУзелОбъемСобр = ПолучитьПодчиненныйЭлемент(НовыйУзелОборот, "ОбъемСобр");
					
					Для каждого КлючЗначениеОбъемСобр  Из КлючЗначение.Значение Цикл						
						УстановитьЗначениеЭлемента(НовыйУзелОбъемСобр, КлючЗначениеОбъемСобр.Ключ, КлючЗначениеОбъемСобр.Значение);
					КонецЦикла;	
					
				ИначеЕсли КлючЗначение.Ключ = "Поставщик" Тогда
					
					НовыйУзелПоставщик = ПолучитьПодчиненныйЭлемент(НовыйУзелОборот, "Поставщик");
					ОбразецУзла_Поставщики = ПолучитьПодчиненныйЭлемент(НовыйУзелПоставщик, "Поставщики");
					
					Для каждого КлючЗначениеПоставщик  Из КлючЗначение.Значение Цикл
						
						Если КлючЗначениеПоставщик.Ключ = "Поставщики" Тогда
							
							Для каждого КлючЗначениеПоставщики Из КлючЗначениеПоставщик.Значение Цикл
								
								НовыйУзел_Поставщики = НовыйУзелИзПрототипа(ОбразецУзла_Поставщики);
								
								ИдПостав = КлючЗначениеПоставщики.Значение;
								УстановитьЗначениеЭлемента(НовыйУзел_Поставщики, "ИдПостав", ИдПостав);
							
							КонецЦикла;
							
						Иначе	
						    УстановитьЗначениеЭлемента(НовыйУзелПоставщик, КлючЗначениеПоставщик.Ключ, КлючЗначениеПоставщик.Значение);
						КонецЕсли;
						
					КонецЦикла;
					
					РегламентированнаяОтчетность.УдалитьУзел(ОбразецУзла_Поставщики);
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, КорневойУзел, ПараметрыВыгрузки)
	...
	УзелСправочники = ПолучитьПодчиненныйЭлемент(Узел_Документ, "Справочники");
...
Процедура ВывестиНаПечатьДекларация(СтруктураПараметровСтраницы)
	...
	СтруктураВысот.Вставить("ВысотаСтраницыВСтроках", 70);
	...
	СписокПереносимыхПолей.Добавить(33, "02");
	СписокПереносимыхПолей.Добавить(33, "04");
	СписокПереносимыхПолей.Добавить(33, "13");
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение34.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение34.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение34.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение34.ДеревоФормИФорматов();
