...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПеренестиПользовательскиеНастройкиВНовыйВариантОтчета");
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ИспользоватьНесколькоОтветственных Тогда
		ТекстПоляХозрасчетныйОборотыОтветственный = ОтчетыРуководителя.ТекстПоляОтветственный(
			"ХозрасчетныйОбороты",
			"Регистратор",
			ОтчетыРуководителя.ТипыДокументовПродажи(),
...
Процедура УстановитьПараметрыВыводаДиаграммы(Диаграмма, ПараметрыОтчета)
	...
	Если ПараметрыОтчета.ПериодичностьСравнения = Перечисления.Периодичность.Месяц
		...
		Если ПараметрыОтчета.ВПределахОдногоГода И АктивнаяГруппировка <> Неопределено Тогда
			...
			// Тип группировки "в иерархии"(ТипГруппировки = 1) удваивает значение серии
			Если АктивнаяГруппировка.ТипГруппировки = 1 Тогда
				ОписаниеНовойСерии.Вставить("ТипГруппировки", 0);
			Иначе
				ОписаниеНовойСерии.Вставить("ТипГруппировки", АктивнаяГруппировка.ТипГруппировки);
			КонецЕсли;
			
			ОписаниеНовойСерии.Вставить("Поле", АктивнаяГруппировка.Поле);
		Иначе
			ОписаниеНовойСерии.Вставить("ТипГруппировки", 0);
