...
Функция СформироватьОтчет(Знач ПараметрыОтчета) Экспорт
	...
	ПараметрыОтчета.СписокТаблиц = ПараметрыОтчета.СписокТаблиц.Скопировать();
	Если ПроверитьЗаполнение(ПараметрыОтчета) Тогда
		ПараметрыОтчета.СписокТаблиц = ПодготовитьОтчет(ПараметрыОтчета);
	КонецЕсли;
	
	Возврат ПараметрыОтчета.СписокТаблиц;
	
КонецФункции

Функция ПроверитьЗаполнение(ПараметрыОтчета)
	
	Организация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "Организация");
	НачалоПериода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "НачалоПериода");
	КонецПериода = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, "КонецПериода");
...
Функция ПодготовитьОтчет(Знач ПараметрыОтчета) Экспорт
...
Процедура ПодготовитьДанныеОтчета(Знач ПараметрыОтчета) Экспорт
	...
	Если ПараметрыОтчета.НачалоПериода > ПараметрыОтчета.КонецПериода Тогда
		ВызватьИсключение НСтр("ru = 'Отчет.КнигаУчетаДоходовИРасходовПредпринимателя.ПодготовитьДанныеОтчета(): Установлены некорректные параметры отчета - дата начала периода не может быть больше даты конца'");
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, Форма) Экспорт
	...
	ПараметрыОтчета.ВидДеятельности = Отчет.ВидДеятельности;
	
	Для Каждого Таблица Из Отчет.СписокТаблиц Цикл
		ПараметрыОтчета.СписокСформированныхЛистов.Добавить(Таблица.АдресХранилища,
			?(ПустаяСтрока(Таблица.ЗаголовокТаблицы), Таблица.ИмяМакета, Таблица.ЗаголовокТаблицы));
	КонецЦикла;
	...
	ПараметрыОтчета.Вставить("СписокТаблиц");
	
	ЗаполнитьПараметрыОтчета(ПараметрыОтчета); // заполняем доп.параметры и значения по умолчанию
	
	ПараметрыОтчета.ОблагаетсяНДС = Отчет.ОблагаетсяНДС; // восстановим значение из настроек отчета
...
Функция ЗаполнитьПараметрыОтчета(ПараметрыОтчета) Экспорт
	...
	ПараметрыОтчета.ОблагаетсяНДС = Не УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(
	...
	ПараметрыОтчета.Вставить("ФормироватьВсеТаблицы", Истина);
	
	Если Не ЗначениеЗаполнено(ПараметрыОтчета.Заголовок) Тогда
		...
		ПараметрыОтчета.Заголовок = СтрШаблон(НСтр("ru = 'Книга доходов и расходов ИП %1'"), ПериодОтчета);
	КонецЕсли;
	
	Если Не ПараметрыОтчета.Свойство("СписокТаблиц") Тогда
		СписокТаблиц = Новый ТаблицаЗначений;
		СписокТаблиц.Колонки.Добавить("КодТаблицы", ОбщегоНазначения.ОписаниеТипаЧисло(2, 0, ДопустимыйЗнак.Неотрицательный));
		СписокТаблиц.Колонки.Добавить("Формировать", Новый ОписаниеТипов("Булево"));
		СписокТаблиц.Колонки.Добавить("АдресХранилища", ОбщегоНазначения.ОписаниеТипаСтрока(0));
		...
		СписокТаблиц.Колонки.Добавить("НаименованиеПолное", ОбщегоНазначения.ОписаниеТипаСтрока(0));
		ОбновитьСписокТаблиц(СписокТаблиц);
		
		Для Каждого Таблица Из СписокТаблиц Цикл
			Если Таблица.Формировать Тогда
				Таблица.АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено);
			Иначе
...
Функция ПустыеПараметрыКомпоновкиОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ИдентификаторОтчета", "КнигаУчетаДоходовИРасходовПредпринимателя"); // имя объекта метаданных
	...
	ПараметрыОтчета.Вставить("ВидДеятельности", Справочники.ВидыДеятельностиПредпринимателей.ПустаяСсылка());
	ПараметрыОтчета.Вставить("ОблагаетсяНДС", Ложь);
...
Функция ПодготовитьОбщийФайлОтчета(ПараметрыОтчета) Экспорт
	...
	ПараметрыДляФормирования = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыОтчета);
	ПараметрыОтчета.Удалить("ПараметрыОтчета");
	
	СписокТаблиц = ПодготовитьОтчет(ПараметрыДляФормирования);
...
Функция ТекстЗапросаОборотыМПЗ(ПараметрыОтчета, НомераТаблиц, ИмяТаблицы, УловиеПоВидуМПЗ)
	...
	|ГДЕ
	|	МПЗ.Оплачено
	|	И (МПЗ.СуммаРасход <> 0 ИЛИ МПЗ.СуммаПриход <> 0 ИЛИ МПЗ.НДСРасход <> 0 ИЛИ МПЗ.НДСПриход <> 0)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПериодОтчета,
	|	МПЗ.Период,
...
Функция ТекстЗапросаОС(ПараметрыОтчета, НомераТаблиц)
	...
	НомераТаблиц.Вставить("ВТНДСВСтоимости", НомераТаблиц.Количество());
	...
	|	ИПМПЗОстаткиИОбороты.НДСНачальныйОстаток + ИПМПЗОстаткиИОбороты.НДСПриход КАК НДС
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОстаткиИОборотыОС.Номенклатура КАК Номенклатура,
	|	ВТОстаткиИОборотыОС.Партия КАК Партия,
	|	ВТОстаткиИОборотыОС.ДокументОплаты КАК ДокументОплаты,
	|	ВТОстаткиИОборотыОС.НДС КАК НДС
	|ПОМЕСТИТЬ ВТНДСВСтоимости
	|ИЗ
	...
	|		ПО (ВТОстаткиИОборотыОС.Партия ССЫЛКА Документ.ПоступлениеТоваровУслуг)
	|			И ВТОстаткиИОборотыОС.Партия = ПоступлениеТоваровУслугОсновныеСредства.Ссылка
	|			И ВТОстаткиИОборотыОС.Номенклатура = ПоступлениеТоваровУслугОсновныеСредства.ОсновноеСредство
	|ГДЕ
	|	ПоступлениеТоваровУслугОсновныеСредства.СпособУчетаНДС = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.УчитываетсяВCтоимости)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Партия,
	...
	|	КОНЕЦ + ЕСТЬNULL(НДСВСтоимости.НДС, 0) КАК ПервоначальнаяСтоимость,
	...
	|							ТОГДА 0
	|						ИНАЧЕ ЕСТЬNULL(ИППрочиеРасходыПредПериод.СуммаВсего, 0) + ЕСТЬNULL(ИППрочиеРасходыТекПериод.СуммаВсего, 0)
	|					КОНЕЦ + ЕСТЬNULL(НДСВСтоимости.НДС, 0)) / ПараметрыАмортизацииОС.СрокПолезногоИспользования КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ 0
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНДСВСтоимости КАК НДСВСтоимости
	|		ПО ОстаткиИОборотыОС.Номенклатура = НДСВСтоимости.Номенклатура
	|			И ОстаткиИОборотыОС.Партия = НДСВСтоимости.Партия
	|			И ОстаткиИОборотыОС.ДокументОплаты = НДСВСтоимости.ДокументОплаты
...
Функция ТекстЗапросаРасходыНаОплатуТруда(ПараметрыОтчета, НомераТаблиц)
	...
	|	ЕСТЬNULL(РасходыНаОплатуТруда.ФизЛицо.Наименование, """") КАК ФизЛицоПредставление,
...
Функция ТекстЗапросаПрочиеРасходы(ПараметрыОтчета, НомераТаблиц)
	...
	|	ПрочиеРасходы.СтатьяЗатрат КАК СтатьяЗатрат,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК СтатьяЗатратПредставление,
	|	ПрочиеРасходы.РасчетныйДокумент КАК РасчетныйДокумент,
	|	ПрочиеРасходы.ДокументОплаты КАК ДокументОплаты,
	|	ЕСТЬNULL(РеквизитыДокументовОплаты.Номер, """") КАК НомерДокументаОплаты,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(РеквизитыДокументовОплаты.Номер, """") = """"
	|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
	|		ИНАЧЕ ЕСТЬNULL(РеквизитыДокументовОплаты.Дата, ДАТАВРЕМЯ(1, 1, 1))
...
Функция ТекстЗапросаРасходыБудущихПериодов(ПараметрыОтчета, НомераТаблиц)
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ИПРБП.Период КАК Период,
	|	ИПРБП.Регистратор КАК Регистратор,
	|	ИПРБП.ВидДвижения КАК ВидДвижения,
	|	ИПРБП.ВидРасхода КАК ВидРасхода,
	|	ИПРБП.Партия КАК Партия,
	|	ИПРБП.ДокументОплаты КАК ДокументОплаты,
	|	СУММА(ВЫБОР
	|			КОГДА ИПРБП.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ТОГДА ИПРБП.Сумма
	|			ИНАЧЕ -ИПРБП.Сумма
	|		КОНЕЦ) КАК СуммаОборот,
	|	СУММА(ВЫБОР
	|			КОГДА ИПРБП.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|				ТОГДА ИПРБП.НДС
	|			ИНАЧЕ -ИПРБП.НДС
	|		КОНЕЦ) КАК НДСОборот
	...
	|	И ИПРБП.ХарактерДеятельности В(&СписокХарактеровДеятельности)
	|	И ИПРБП.НоменклатурнаяГруппа В(&СписокНоменклатурныхГрупп)
	|	И ИПРБП.ВидРасхода ССЫЛКА Справочник.ПрочиеДоходыИРасходы
	...
	Если НЕ ПараметрыОтчета.СНачалаНалоговогоПериода Тогда
		...
		|	NULL КАК Партия,
		|	NULL КАК ДокументОплаты,
		|	ИПРБПОбороты.СуммаОборот КАК СуммаОборот,
		|	ИПРБПОбороты.НДСОборот КАК НДСОборот
		|ИЗ
		...
		|				И ХарактерДеятельности В (&СписокХарактеровДеятельности)
		|				И НоменклатурнаяГруппа В (&СписокНоменклатурныхГрупп)
		|				И ВидРасхода ССЫЛКА Справочник.ПрочиеДоходыИРасходы
		|				И ВидРасхода В (&СтатьиЗатратПринимаемые)
		|				И ДокументОплаты <> НЕОПРЕДЕЛЕНО) КАК ИПРБПОбороты";
...
Функция СформироватьТаблицу16_17(ПараметрыОтчета, КодТаблицы)
	...
	Если ПараметрыОтчета.ОблагаетсяНДС Тогда
		ОбластьШапкаКон.Параметры.ОбщаяСтавкаНДС = УчетНДСКлиентСервер.ОбщаяСтавкаНДС(ПараметрыОтчета.НачалоПериода);
		ОбластьШапкаКон.Параметры.ЛьготнаяСтавкаНДС = УчетНДСКлиентСервер.ПониженнаяСтавкаНДС(ПараметрыОтчета.НачалоПериода);
	КонецЕсли;
	ТабличныйДокумент.Присоединить(ОбластьШапкаКон);
...
Функция СформироватьТаблицу50(ПараметрыОтчета, КодТаблицы)
	...
	Если НЕ РасходыНаОплатуТруда.Пустой() Тогда
		...
		ДокументыОплаты = РасходыНаОплатуТруда.Выгрузить().ВыгрузитьКолонку("ДокументОплаты");
		ДокументыОплаты = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДокументыОплаты);
		ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияТипаИзМассива(ДокументыОплаты, Тип("Null"));
		ДоговорыПодрядаСотрудников = УчетЗарплаты.ОплаченныеДоговорыПодрядаСотрудниковИП(
			ПараметрыОтчета.Организация, ФизическиеЛица, ДокументыОплаты);
		ДоговорыПодрядаСотрудников.Индексы.Добавить("ФизическоеЛицо");
		ПараметрыОтчета.Вставить("ДоговорыПодрядаСотрудников", ДоговорыПодрядаСотрудников);
...
Процедура СформироватьТаблицу50заМесяц(Таблица, ТабличныйДокумент, Макет, ВыборкаМесяц, ПараметрыОтчета)
	...
	ДоговорыПодрядаСотрудников = ПараметрыОтчета.ДоговорыПодрядаСотрудников;
	...
	Пока ВыборкаДатаДокументаОплаты.Следующий() Цикл
		...
		Пока ВыборкаФизЛицо.Следующий() Цикл
			...
			Если ДатаДоговора = '00010101' Тогда
				НайденныеСтроки = ДоговорыПодрядаСотрудников.НайтиСтроки(Новый Структура("ФизическоеЛицо", ВыборкаФизЛицо.ФизЛицо));
				Если НайденныеСтроки.Количество() <> 0 Тогда
					ДатаДоговора = "";
					Для ИндексСтроки = 0 По НайденныеСтроки.Количество() - 1 Цикл
						ДатаДоговора = ДатаДоговора + ?(ИндексСтроки = 0, "", ", ") + Формат(НайденныеСтроки[ИндексСтроки].ДатаДоговора, "ДФ=dd.MM.yyyy");
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
