	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил18Декларация" Тогда
		...
		Если НЕ ЗначениеЗаполнено(Запись.Получатель) Тогда		
			...
			Если ЗначениеЗаполнено(Запись.Получатель) Тогда
				Запись.П000010000304 = Запись.Получатель[ИмяРеквизитаНаименование];
		...
		Если ЗначениеЗаполнено(Запись.Получатель) Тогда
			...
			Запись.П000010000305XML 	= СтруктураРезультата.ПолеСтруктурыАдреса;
			Запись.П000010000305		= СтруктураРезультата.Представление;
			
		Иначе 
			РегламентированнаяОтчетностьАЛКО.ОбработкаСохраненияАдреса(Запись.П000010000305XML, Запись.П000010000305);			
		КонецЕсли;
...
Процедура ПроверитьЗаполнениеТаблицыДекларация(СтатусПроверки, ИдГруппы, НомерЛиста, ИдОП = "")
	...
	Пока Истина Цикл
		...
		Для каждого СтрокаДекларации Из РезЗапросаДанных Цикл
			...
			СтруктураАдресКонтрагента = РегламентированнаяОтчетностьАЛКО.СтруктураАдресаИзСтрокиСтруктурыХранения(
																	АдресКонтрагентаXML);
			...
			ПроверкаАдресаПройдена = РегламентированнаяОтчетностьАЛКО.ВерныйАдрес(ДокументСсылкаАлкоОтчет, ТаблицаСообщений, 
										ТаблицаСообщенийДляСведения, СтатусПроверки, СтруктураАдресКонтрагента, 
...
Процедура ВывестиНаПечатьДекларация(СтруктураПараметровСтраницы)
	...
	СписокПереносимыхПолей.Добавить(26, "П000010000304");
	СписокПереносимыхПолей.Добавить(26, "П000010000305");
	СписокПереносимыхПолей.Добавить(26, "П000010000308");
...
Функция ДеревоФормИФорматов() Экспорт
	...
	ОпределитьФорматВДеревеФормИФорматов(Форма20180101, "4.33", , , '2017-12-29', '2020-12-31');
