...
Процедура ПодготовитьДанныеБухгалтерскойОтчетности(ДанныеФинансовогоАнализа, ПроверкаАктуализации)
	...
	// Баланс
	ПоказателиБаланса = ПоказателиБаланса();
	СтруктураИтогов.Вставить("П1100", ПоказателиБаланса.Раздел1);
	СтруктураИтогов.Вставить("П1200", ПоказателиБаланса.Раздел2);
	СтруктураИтогов.Вставить("П1300", ПоказателиБаланса.Раздел3);
	СтруктураИтогов.Вставить("П1400", ПоказателиБаланса.Раздел4);
	СтруктураИтогов.Вставить("П1500", ПоказателиБаланса.Раздел5);
	
	ДобавитьВСтруктуруМассивПоказателей(СтруктураИтогов, "П1600", "П1100;П1200"); // Баланс Актив
	ДобавитьВСтруктуруМассивПоказателей(СтруктураИтогов, "П1700", "П1300;П1400;П1500"); // Баланс Пассив
	
	// ОФР
	ПоказателиОФР = ПоказателиОФР();
	СтруктураИтогов.Вставить("П2100", ПоказателиОФР.ВаловаяПрибыль);
	СтруктураИтогов.Вставить("П2200", ПоказателиОФР.ПрибыльОтПродаж);
	СтруктураИтогов.Вставить("П2300", ПоказателиОФР.ПрибыльДоНалогообложения);
	СтруктураИтогов.Вставить("П2400", ПоказателиОФР.ЧистаяПрибыль);
	
	// Добавим так же накопительный итог по каждому разделу
	СтруктураИтогов.П2200.Добавить("П2100");
	СтруктураИтогов.П2300.Добавить("П2200");
	СтруктураИтогов.П2400.Добавить("П2300");
	...
	Для Год = 0 По 4 Цикл
		Для Каждого КлючИЗначение Из СтруктураИтогов Цикл
			...
			Для Каждого ИмяПоказателя Из КлючИЗначение.Значение Цикл
				ИмяПоказателяСтроки = СтрШаблон("%1_%2", 
...
Функция ДанныеЗаполненияБухгалтерскойОтчетности(Организация, ДатаНачала, ДатаОкончания, ЗаполнятьБаланс)
	...
	ПараметрыАвтозаполнения = ЗаполнениеБухгалтерскойОтчетности.ПараметрыАвтозаполненияБухОтчетности(
	...
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(
		ИДОтчета, ФормаОтчета, ПараметрыОтчета, ПараметрыАвтозаполнения.Контейнер);
...
Функция ФормыБухОтчетностиДействующиеВВыбранныйПериод(ВсеДоступныеФормы, ДатаКонцаПериода)
	...
	Для Каждого ДоступнаяФорма Из ВсеДоступныеФормы Цикл
		...
		ИначеЕсли ДоступнаяФорма.ДатаКонецДействия = '20241231' Тогда
			// Продлеваем действие формы
			ДатаКонцаДействияФормы = '20991231';
		Иначе
...
Процедура ВывестиОтчетОФинансовомРезультате(ТабличныйДокумент, ОбластиМакета, ДанныеФинансовогоАнализа, ЭтоОсновнаяФормаОтчета, СтрокаТаблицы)
	...
	Для Каждого Строка Из ПоказателиПрибылиУбытки Цикл
		ВыводитьСтроку = Ложь;
		КоличествоПериодов = ДанныеФинансовогоАнализа.СтруктураГодов.Количество();
		Для Год = 1 По КоличествоПериодов Цикл
			ИмяПоказателя = СтрШаблон("%1_%2", Строка, Год);
			Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтрокаТаблицы.Показатели, ИмяПоказателя, 0) <> 0 Тогда
				ВыводитьСтроку = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		Если ВыводитьСтроку Тогда
			...
			ОбластьСтрока.Параметры.Заполнить(СтрокаТаблицы.Показатели);
			Для Год = 1 По КоличествоПериодов Цикл
				ОбластьСтрока.Параметры[СтрШаблон("АнализДоходовИРасходов_%1", Год)] = Новый Структура("Действие, Год",
				"ОткрытьОтчет", ДанныеФинансовогоАнализа.СтруктураГодов[СтрШаблон("Год_%1", Год)]);
			КонецЦикла;
			ТабличныйДокумент.Вывести(ОбластьСтрока);
		КонецЕсли;
	КонецЦикла;
...
Функция ПоказателиБаланса()
	...
	ДобавитьВСтруктуруМассивПоказателей(ПоказателиБаланса, "Раздел4", "П1410;П1420;П1450");
...
Процедура ПодготовитьДанныеАнализаОтчетности(ДанныеФинансовогоАнализа)
	...
	ДобавитьВСтруктуруМассивПоказателей(РасчетПоказателей, "ЧистыеАктивы", "1600;-1410;-1420;-1450;-1510;-1520;-1540;-1550");
	...
	Для Каждого ИмяПоказателя Из ИменаПоказателей Цикл
		...
		Если ЕстьДанные Тогда
			...
			Если ЗначениеПоказателя1 > ЗначениеПоказателя2 Тогда
				ВыбраннаяКартинка = БиблиотекаКартинок.ПродажиУвеличениеПоказателя;
			ИначеЕсли ЗначениеПоказателя1 < ЗначениеПоказателя2 Тогда
				ВыбраннаяКартинка = БиблиотекаКартинок.ПродажиУменьшениеПоказателя;
			Иначе
...
Процедура ДобавитьПоказателиРентабельностьАктивов(Показатели, СтрокаОтчетности, ДанныеФинансовогоАнализа)
	...
	Для Каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		РентабельностьПоОтрасли = СредняяРентабельностьПоОтрасли(
			"РентабельностьАктивов", ДанныеФинансовогоАнализа.ВидыДеятельности, ДанныеГода.Год);
...
Процедура ДобавитьПоказателиРентабельностьПродаж(Показатели, СтрокаОтчетности, ДанныеФинансовогоАнализа)
	...
	Для Каждого ДанныеГода Из ДанныеПоГодам Цикл
		...
		// Средняя цифра по отрасли
		РентабельностьПоОтрасли = СредняяРентабельностьПоОтрасли(
			"РентабельностьПродаж", ДанныеФинансовогоАнализа.ВидыДеятельности, ДанныеГода.Год);
...
Функция СредняяРентабельностьПоОтрасли(ИмяПоказателя, ВидыДеятельности, Год)
	...
	Если ИспользоватьОКВЭД2 Тогда
		...
		ДатаЗначений = Дата(Число(Год), 1, 1);
		Показатели = ДанныеНагрузкиИРентабельности.ЗначениеПоказателейПоКоду(ОсновнойКодОКВЭД, ДатаЗначений);
		Если Показатели <> Неопределено
			И Показатели[ИмяПоказателя] <> Неопределено Тогда
			Результат.ВидДеятельности = Показатели.НаименованиеВидаДеятельности;
			Результат.Рентабельность = Показатели[ИмяПоказателя];
		КонецЕсли;
	...
	Иначе
		...
		ОбластьРентабельность = Макет.Область(ИмяПоказателя);
		ОбластьКолонки = Макет.НайтиТекст(ИскомыйГод, , ОбластьРентабельность, , Истина);
...
Процедура УстановитьЦветЯчейкиПоОценке(Область, ИмяОбласти, Оценка);
	...
	Если Оценка = ПлохаяОценка() Тогда
		Область.Области[ИмяОбласти].ЦветТекста = ЦветаСтиля.ПродажиПлохаяОценкаЦвет;
	ИначеЕсли Оценка = ХорошаяОценка() Тогда
		Область.Области[ИмяОбласти].ЦветТекста = ЦветаСтиля.ПродажиХорошаяОценкаЦвет;
	КонецЕсли;
...
Функция ПроверкаАктуальностьДанныхУчета(Организация, Период)
	...
	ПроверкаАктуальности = Обработки.ЗакрытиеМесяца.ПроверитьАктуальность(ПараметрыПроверкиАктуальности);
