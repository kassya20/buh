...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002М125003") И НЕ СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		СписокСтавок.Добавить(18.03, "18,03");
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Пока Обл245 <> Неопределено Цикл
					...
					Пока Обл240 <> Неопределено Цикл
						...
						Если Обл220 <> Неопределено И СокрЛП(Обл220.Значение) = "643" Тогда
							...
							Если Обл210 <> Неопределено Тогда
								Обл240.Значение = Макс(0, Окр(Обл210.Значение));
							КонецЕсли;
						КонецЕсли;
						Сумма245 = Сумма245 + Макс(0, Окр(Обл240.Значение));
						РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(ИндексПодч, ИндексПодчСтр);
						Обл240 = Форма.ТабличныйДокумент.Области.Найти("П00002М224003_" + ИндексСтр + "_" + ИндексПодчСтр);
					КонецЦикла;
					...
					Если Обл250 <> Неопределено И Обл260 <> Неопределено Тогда
						Обл260.Значение = Макс(0, Окр(Сумма245 * Обл250.Значение / 100));
					...
					Пока Обл340 <> Неопределено Цикл
						...
						Если Обл320 <> Неопределено И СокрЛП(Обл320.Значение) = "643" Тогда
							...
							Если Обл310 <> Неопределено Тогда
								Обл340.Значение = Макс(0, Окр(Обл310.Значение));
							КонецЕсли;
						КонецЕсли;
						Сумма345 = Сумма345 + Макс(0, Обл340.Значение);
						РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(ИндексПодч, ИндексПодчСтр);
						Обл340 = Форма.ТабличныйДокумент.Области.Найти("П00003М234003_" + ИндексСтр + "_" + ИндексПодчСтр);
					КонецЦикла;
			...
			Если ЗначениеЗаполнено(ОбластьИмя) Тогда
				...
				Пока Обл450 <> Неопределено Цикл
					...
					Пока Обл440 <> Неопределено Цикл
						...
						Если Обл420 <> Неопределено И СокрЛП(Обл420.Значение) = "643" Тогда
							...
							Если Обл410 <> Неопределено Тогда
								Обл440.Значение = Макс(0, Окр(Обл410.Значение));
							КонецЕсли;
						КонецЕсли;
						Сумма450 = Сумма450 + Макс(0, Окр(Обл440.Значение));
						РегламентированнаяОтчетностьКлиентСервер.УвеличитьИндекс(ИндексПодч, ИндексПодчСтр);
						Обл440 = Форма.ТабличныйДокумент.Области.Найти("П00004М244003_" + ИндексСтр + "_" + ИндексПодчСтр);
					КонецЦикла;
					...
					Если Обл460 <> Неопределено И Обл470 <> Неопределено Тогда
						...
						Сумма480 = Сумма480 + Обл470.Значение;
					КонецЕсли;
				...
				Обл480 = Форма.ТабличныйДокумент.Области.Найти("П000040048003");
				Обл480.Значение = Сумма480;
				
				Обл490 = Форма.ТабличныйДокумент.Области.Найти("П000040049003");
				Обл500 = Форма.ТабличныйДокумент.Области.Найти("П000040050003");
				Обл500.Значение = Макс(0, Окр(Обл480.Значение) - Окр(Обл490.Значение));
				...
				Пока Обл550 <> Неопределено Цикл
					...
					Если Обл530 <> Неопределено И СокрЛП(Обл530.Значение) = "643" Тогда
						...
						Если Обл520 <> Неопределено Тогда
							Обл550.Значение = Макс(0, Окр(Обл520.Значение));
						КонецЕсли;
					КонецЕсли;
					
					Сумма560 = Сумма560 + Макс(0, Окр(Обл550.Значение));
...
Процедура РасчетВДеревеЗначений(Форма, ИмяРаздела)
	...
	Если ИмяРаздела = "Раздел1" Тогда
		...
		Для каждого ВеткаР Из ДеревоР.Строки Цикл
			Сумма120 = Сумма120 + Макс(0, ВеткаР.Данные["П000020027003"]);
		КонецЦикла;
		
		ДеревоР = ПолучитьИзВременногоХранилища(
			Форма.СтруктураРеквизитовФормы["АдресВоВрХранилищеДеревоДанныхРаздел4"]);
		Для каждого ВеткаР Из ДеревоР.Строки Цикл
			Сумма120 = Сумма120 + Макс(0, ВеткаР.Данные["П000040050003"]);
		КонецЦикла;
			...
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				...
				Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
					Если СокрЛП(СтрокаПодчМнЧ.Данные["П00002М222003"]) = "643" Тогда
						...
						СтрокаПодчМнЧ.Данные.Вставить("П00002М224003", Макс(0, Окр(СтрокаПодчМнЧ.Данные["П00002М221003"])));
					КонецЕсли;
					Сумма245 = Сумма245 + Макс(0, Окр(СтрокаПодчМнЧ.Данные["П00002М224003"]));
				КонецЦикла;
				
				СтрокаМнЧ.Данные.Вставить("П00002М124503", Сумма245);
				
				СтрокаМнЧ.Данные.Вставить("П00002М126003", Макс(0, Окр(Сумма245 * СтрокаМнЧ.Данные["П00002М125003"] / 100)));
			...
			Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
				...
				Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
					Если СокрЛП(СтрокаПодчМнЧ.Данные["П00003М232003"]) = "643" Тогда
						...
						СтрокаПодчМнЧ.Данные.Вставить("П00003М234003", Макс(0, Окр(СтрокаПодчМнЧ.Данные["П00003М231003"])));
					КонецЕсли;
					Сумма345 = Сумма345 + Макс(0, СтрокаПодчМнЧ.Данные["П00003М234003"]);
				КонецЦикла;
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			...
			Для каждого СтрокаПодчМнЧ Из ДеревоПодчМнЧ.Строки Цикл
				Если СокрЛП(СтрокаПодчМнЧ.Данные["П00004М242003"]) = "643" Тогда
					...
					СтрокаПодчМнЧ.Данные.Вставить("П00004М244003", Макс(0, Окр(СтрокаПодчМнЧ.Данные["П00004М241003"])));
				КонецЕсли;
				Сумма450 = Сумма450 + Макс(0, Окр(СтрокаПодчМнЧ.Данные["П00004М244003"]));
			КонецЦикла;
			...
			Сумма480 = Сумма480 + СтрокаМнЧ.Данные["П00004М147003"];
			
		КонецЦикла;
		
		Ветка.Данные.Вставить("П000040048003", Сумма480);
		
		Ветка.Данные.Вставить("П000040050003", Макс(0,
			Окр(Ветка.Данные["П000040048003"]) - Окр(Ветка.Данные["П000040049003"])));
	...
	ИначеЕсли ИмяРаздела = "Раздел5" Тогда
		...
		Для каждого СтрокаМнЧ Из ДеревоМнЧ.Строки Цикл
			Если СокрЛП(СтрокаМнЧ.Данные["П00005М153003"]) = "643" Тогда
				...
				СтрокаМнЧ.Данные.Вставить("П00005М155003", Макс(0, Окр(СтрокаМнЧ.Данные["П00005М152003"])));
			КонецЕсли;
			Сумма560 = Сумма560 + Макс(0, Окр(СтрокаМнЧ.Данные["П00005М155003"]));
		КонецЦикла;
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
		СообщитьВТС("В разделе 1 не задан код по ОКТМО", "Раздел1", "П000010010003", НомСтраницы);
		СтатусПроверки = Ложь;
	ИначеЕсли ЗначПок <> "45348000" Тогда
		СообщитьВТС("В разделе 1 код по ОКТМО должен принимать значение 45348000", "Раздел1", "П000010010003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	ЗначПок = СокрЛП(Ветка.Данные["П000010011003"]);
	Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
		СообщитьВТС("В разделе 1 не задан код бюджетной классификации", "Раздел1", "П000010011003", НомСтраницы);
		СтатусПроверки = Ложь;
	ИначеЕсли ЗначПок <> "18210301000011000110" Тогда
		СообщитьВТС("В разделе 1 код бюджетной классификации должен принимать значение 18210301000011000110",
			"Раздел1", "П000010011003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	
	Если Окр(Ветка.Данные["П000010012003"]) < 0 Тогда
		СообщитьВТС("В разделе 1 значение в строке 120 не должно быть отрицательным",
			"Раздел1", "П000010012003", НомСтраницы);
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если ЗаполненРаздел2 Тогда
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Если Окр(Ветка.Данные["П000020027003"]) < 0 Тогда
				СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": значение в строке 270 не должно быть отрицательным",
					"Раздел2", "П000020027003", НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					...
					СтатусПроверки = Ложь;
				ИначеЕсли ЗначПок <> 18.03 И ЗначПок <> 16.67 Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
					+ "расчетная налоговая ставка должна принимать одно из значений: 18,03; 16,67",
					"Раздел2", "П00002М125003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если Окр(СтрокаМнЧ.Данные["П00002М124503"]) < 0 Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
						+ "значение в строке 245 не должно быть отрицательным", "Раздел2", "П00002М124503_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если Окр(СтрокаМнЧ.Данные["П00002М126003"]) < 0 Тогда
					СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
						+ "значение в строке 260 не должно быть отрицательным", "Раздел2", "П00002М126003_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого СтрокаПодчМнЧ Из ДанныеПодчМнЧ.Строки Цикл
					...
					Если Окр(СтрокаПодчМнЧ.Данные["П00002М221003"]) < 0 Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 210 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел2", "П00002М221003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если СтрокаПодчМнЧ.Данные["П00002М223003"] < 0 Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 230 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел2", "П00002М223003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если Окр(СтрокаПодчМнЧ.Данные["П00002М224003"]) < 0 Тогда
						СообщитьВТС("Раздел 2 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 240 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел2", "П00002М224003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
	...
	Если ЗаполненРаздел3 Тогда
		...
		Для каждого Ветка Из ДеревоРаздела.Строки Цикл
			...
			Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
				...
				Если Окр(СтрокаМнЧ.Данные["П00003М134503"]) < 0 Тогда
					СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
						+ "значение в строке 345 не должно быть отрицательным", "Раздел3", "П00003М134503_" + НомСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				...
				Для каждого СтрокаПодчМнЧ Из ДанныеПодчМнЧ.Строки Цикл
					...
					Если Окр(СтрокаПодчМнЧ.Данные["П00003М231003"]) < 0 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 310 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел3", "П00003М231003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					...
					Если СтрокаПодчМнЧ.Данные["П00003М233003"] < 0 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 330 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел3", "П00003М233003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
					Если Окр(СтрокаПодчМнЧ.Данные["П00003М234003"]) < 0 Тогда
						СообщитьВТС("Раздел 3 -> Страница " + НомСтраницы + ": " + Текст_СтрокаГруппыСтрок
							+ "значение в строке 340 не должно быть отрицательным " + Текст_СтрокаГруппыПодчСтрок,
							"Раздел3", "П00003М234003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
						СтатусПроверки = Ложь;
					КонецЕсли;
	...
	Если ЗаполненРаздел4 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
			СообщитьВТС("В разделе 4 не задан признак иностранного налогоплательщика",
			"Раздел4", "П000040040003", НомСтраницы);
			СтатусПроверки = Ложь;
		ИначеЕсли ВозмПризнакиИнПрод.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
			СообщитьВТС("В разделе 4 признак иностранного налогоплательщика должен принимать одно из значений: "
		...
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			...
			ИндСтрокиМнЧ = ДанныеМнЧ.Строки.Индекс(СтрокаМнЧ);
			НомСтроки = Формат(ИндСтрокиМнЧ + 1, "ЧГ=");
			Текст_СтрокаГруппыСтрок = ?(КолСтрокМнЧ = 1, "", "в строке " + НомСтроки + " группы строк 410-470 ");
			
			ЗначПок = СтрокаМнЧ.Данные["П00004М146003"];
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок
				+ "не задана расчетная налоговая ставка", "Раздел4", "П00004М146003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			ИначеЕсли ЗначПок <> 9.09 И ЗначПок <> 16.67 И ЗначПок <> 18.03 Тогда
				СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок
				+ "расчетная налоговая ставка должна принимать одно из значений: 9,09; 16,67; 18,03",
			...
			Для каждого СтрокаПодчМнЧ Из ДанныеПодчМнЧ.Строки Цикл
				...
				Если Окр(СтрокаПодчМнЧ.Данные["П00004М241003"]) < 0 Тогда
					СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "значение в строке 410 не должно быть отрицательным "
						+ Текст_СтрокаГруппыПодчСтрок, "Раздел4", "П00004М241003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				ЗначПок = СокрЛП(СтрокаПодчМнЧ.Данные["П00004М242003"]);
				Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
					СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "не задан код валюты " + Текст_СтрокаГруппыПодчСтрок,
						"Раздел4", "П00004М242003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
				Если СтрокаПодчМнЧ.Данные["П00004М243003"] < 0 Тогда
					СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "значение в строке 430 не должно быть отрицательным "
						+ Текст_СтрокаГруппыПодчСтрок, "Раздел4", "П00004М243003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				Если Окр(СтрокаПодчМнЧ.Данные["П00004М244003"]) < 0 Тогда
					СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "значение в строке 440 не должно быть отрицательным "
						+ Текст_СтрокаГруппыПодчСтрок, "Раздел4", "П00004М244003_" + НомСтроки + "_" + НомПодчСтроки, НомСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
				
			КонецЦикла;
			
			Если Окр(СтрокаМнЧ.Данные["П00004М145003"]) < 0 Тогда
				СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "значение в строке 450 не должно быть отрицательным",
					"Раздел4", "П00004М145003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Окр(СтрокаМнЧ.Данные["П00004М147003"]) < 0 Тогда
				СообщитьВТС("В разделе 4 " + Текст_СтрокаГруппыСтрок + "значение в строке 470 не должно быть отрицательным",
					"Раздел4", "П00004М147003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
		КонецЦикла;
		
		Если Окр(Ветка.Данные["П000040048003"]) < 0 Тогда
			СообщитьВТС("В разделе 4 значение в строке 480 не должно быть отрицательным",
			"Раздел4", "П000040048003", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Окр(Ветка.Данные["П000040049003"]) < 0 Тогда
			СообщитьВТС("В разделе 4 значение в строке 490 не должно быть отрицательным",
			"Раздел4", "П000040049003", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		Если Окр(Ветка.Данные["П000040050003"]) < 0 Тогда
			СообщитьВТС("В разделе 4 значение в строке 500 не должно быть отрицательным",
			"Раздел4", "П000040050003", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если ЗаполненРаздел5 Тогда
		...
		Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
			СообщитьВТС("В разделе 5 не задан признак иностранного налогоплательщика",
			"Раздел5", "П000050051003", НомСтраницы);
			СтатусПроверки = Ложь;
		ИначеЕсли ВозмПризнакиИнПрод.МассивЗначений.Найти(ЗначПок) = Неопределено Тогда
			СообщитьВТС("В разделе 5 признак иностранного налогоплательщика должен принимать одно из значений: "
		...
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			...
			Если Окр(СтрокаМнЧ.Данные["П00005М152003"]) < 0 Тогда
				СообщитьВТС("В разделе 5 " + Текст_СтрокаГруппыСтрок + "значение в строке 520 не должно быть отрицательным",
					"Раздел5", "П00005М152003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ЗначПок = СокрЛП(СтрокаМнЧ.Данные["П00005М153003"]);
			Если НЕ ЗначениеЗаполнено(ЗначПок) Тогда
				СообщитьВТС("В разделе 5 " + Текст_СтрокаГруппыСтрок + "не задан код валюты",
				"Раздел5", "П00005М153003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			Если СтрокаМнЧ.Данные["П00005М154003"] < 0 Тогда
				СообщитьВТС("В разделе 5 " + Текст_СтрокаГруппыСтрок + "значение в строке 540 не должно быть отрицательным",
					"Раздел5", "П00005М154003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			Если Окр(СтрокаМнЧ.Данные["П00005М155003"]) < 0 Тогда
				СообщитьВТС("В разделе 5 " + Текст_СтрокаГруппыСтрок + "значение в строке 550 не должно быть отрицательным",
					"Раздел5", "П00005М155003_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
		КонецЦикла;
		
		Если Окр(Ветка.Данные["П000050056003"]) < 0 Тогда
			СообщитьВТС("В разделе 5 значение в строке 560 не должно быть отрицательным",
				"Раздел5", "П000050056003", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
		
	КонецЕсли;
	
	// Проверка заполнения хотя бы одного из разделов 2-5.
	//
...
Процедура Проверить(Команда)
	...
	ПоказатьПредупреждение(, НСтр(
		...
		|от 19.12.2025 № ЕД-7-3/1229@ будет реализована
...
Процедура АктивизироватьЯчейку(Ячейка) Экспорт
	...
	Если Ячейка.Раздел = "Раздел1" ИЛИ Ячейка.Раздел = "Раздел4" ИЛИ Ячейка.Раздел = "Раздел5" Тогда
