...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОтчета.Период) Тогда
		ПараметрыОтчета.Период = РегиональныеСтавкиНалогов.ПериодПоУмолчанию();
	КонецЕсли;
...
Процедура ДокументРезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	ИначеЕсли Действие = "ПрименитьСтавку" Тогда
		...
		Если Режим = "НалогиИОтчеты" Тогда
			...
			Если ФормаЗакрыта(ВладелецФормы, "ОбщаяФорма.НалогиИОтчеты") Тогда
				Возврат;
			КонецЕсли;
		...
		ИначеЕсли Режим = "СравнениеРежимовНалогообложения" Тогда
			...
			Если ФормаЗакрыта(ФормаСтавкиНалогов, "Обработка.СравнениеРежимовНалогообложения.Форма.ФормаСтавкиНалогов") Тогда
				Возврат;
			КонецЕсли;
		...
		ИначеЕсли Налог = "ПСН" И Режим = "Помощник" Тогда
			...
			Если ФормаЗакрыта(ВладелецФормы, "Обработка.ПомощникЗаполненияЗаявленияНаПолучениеПатента.Форма.Форма") Тогда
				Возврат;
			КонецЕсли;
			ВладелецФормы.Открыть();
		...
		ИначеЕсли Налог = "УСН" И Режим = "НастройкаСтавкиУСН" Тогда

			Если ФормаЗакрыта(ВладелецФормы, "РегистрСведений.НастройкиУчетаУСН.Форма.ФормаЗаписиСтавкиНалога") Тогда
				Возврат;
			КонецЕсли;
			
			ВладелецФормы.Открыть();
			Оповестить("ИзменениеСтавкиУСН", Расшифровка, ЭтотОбъект);
...
&НаКлиенте
Функция ФормаЗакрыта(Форма, ИмяФормы)

	Возврат ТипЗнч(Форма) <> Тип("ФормаКлиентскогоПриложения") 
		Или Форма.ИмяФормы <> ИмяФормы
		Или Не Форма.Открыта();
