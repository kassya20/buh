...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СформироватьОтчетУСН", Истина);
	Методы.Вставить("СформироватьОтчетПСН", Истина);
...
Функция ДанныеОтчетаУСН(Параметры)
	...
	Для каждого ОбъектНалогообложения Из Информация.ОбъектыНалогообложения Цикл
		...
		ВывестиИнформациюПовышеннаяСтавкаУСН(
...
Процедура ВывестиИнформациюПовышеннаяСтавкаУСН(ОбъектНалогообложения, ДокументРезультат, Области, ПараметрыОтчета)
	
	Перем ПовышеннаяСтавка;
	
	Если Не ОбъектНалогообложения.Свойство("ПовышеннаяСтавка", ПовышеннаяСтавка)
		Или Не ПовышеннаяСтавка.Применяется Тогда
		Возврат;
	КонецЕсли;
	
	ТекстЗаголовка = НСтр("ru='Повышенная ставка'");
	...
	Области.ПовышеннаяСтавка.Параметры.СтавкаПредставление = СтрШаблон("%1%%", ПовышеннаяСтавка.СтавкаНалога);
	Области.ПовышеннаяСтавка.Параметры.Описание = ПовышеннаяСтавка.Описание;
	ДокументРезультат.Вывести(Области.ПовышеннаяСтавка);
...
Процедура ВывестиИнформациюОсновнаяСтавкаУСН(ОбъектНалогообложения, ДокументРезультат, Области, ПараметрыОтчета)
	...
	ТекстЗаголовка = НСтр("ru='Базовая ставка'");
	ОсновнаяСтавка = ОбъектНалогообложения.ОсновнаяСтавка;
	Области.ЗаголовокРаздела.Параметры.ЗаголовокРаздела = ВРег(ТекстЗаголовка);
	ДокументРезультат.Вывести(Области.ЗаголовокРаздела);
	ДокументРезультат.Вывести(Области.Разделитель);
	
	ЕстьУсловия = ОсновнаяСтавка.Свойство("Условия", Условия) И Условия.Количество() > 0;
	ТекущаяОбласть = ?(ЕстьУсловия, Области.ОсновнаяСтавка, Области.ОсновнаяСтавкаБезУсловий);
	ТекущаяОбласть.Параметры.СтавкаПредставление = СтрШаблон("%1%%", ОсновнаяСтавка.СтавкаНалога);
	Если Не ЕстьУсловия Тогда
		ТекущаяОбласть.Параметры.Описание = ОсновнаяСтавка.Описание;
	КонецЕсли;
	...
	Если ПараметрыОтчета.ПоказыватьПрименить 
		...
		Если ЕстьУсловия Тогда
			...
			ТекущаяОбласть.Параметры.Условие = Условия[0];
		Иначе
			ТекущаяОбласть = Области.ПрименитьБезУсловий;
		КонецЕсли;
		...
		ДокументРезультат.Вывести(ТекущаяОбласть);
	ИначеЕсли ЕстьУсловия Тогда
		Области.Условие.Параметры.Условие = Условия[0];
		ДокументРезультат.Вывести(Области.Условие);
	КонецЕсли;
	
	Если ЕстьУсловия Тогда
		Для Номер = 1 По Условия.ВГраница() Цикл
			Области.Условие.Параметры.Условие = Условия[Номер];
			ДокументРезультат.Вывести(Области.Условие);
		КонецЦикла;
	КонецЕсли;
...
Процедура ВывестиИнформациюОсновнаяСтавкаПСН(Информация, ДокументРезультат, Области, ПараметрыОтчета)
	...
	Области.ОсновнаяСтавкаБезУсловий.Параметры.СтавкаПредставление = СтрШаблон("%1%%", ИнформацияПоСтавке.СтавкаНалога);
	Области.ОсновнаяСтавкаБезУсловий.Параметры.Описание = ИнформацияПоСтавке.Описание;
	ДокументРезультат.Вывести(Области.ОсновнаяСтавкаБезУсловий);
