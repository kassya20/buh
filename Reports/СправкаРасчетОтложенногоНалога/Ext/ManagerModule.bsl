...
Процедура ПередКомпоновкойМакета(Контекст, Схема, КомпоновщикНастроек) Экспорт
	...
	НомерРасчета = ?(Контекст.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога,
		1, 
		0);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
		КомпоновщикНастроек,
...
Функция ПолучитьВнешниеНаборыДанных(Контекст, МакетКомпоновки) Экспорт
	...
	ПрименяетсяФСБУ25 = УчетнаяПолитика.ПрименяетсяФСБУ25(Контекст.Организация, Контекст.КонецПериода);
	КонтекстРасчета = Новый Структура("ПрименяетсяФСБУ25", ПрименяетсяФСБУ25);
	ВнешниеНаборы.Вставить("ГруппыБалансовыхСчетов", ГруппыБалансовыхСчетов(КонтекстРасчета));
...
Процедура ПриВыводеЗаголовка(Контекст, КомпоновщикНастроек, Результат) Экспорт
	...
	Если Контекст.ВидРегламентнойОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога Тогда
		Контекст.ПредставлениеПериода = Контекст.ПредставлениеПериода + ", " + НСтр("ru='включая межотчетный период'");
	КонецЕсли;
...
Функция ГруппыБалансовыхСчетов(КонтекстРасчета)
	...
	ПравилаГруппировки = НалогНаПрибыльБухгалтерскийУчет.ПравилаГруппировкиАктивовОбязательств(КонтекстРасчета);
