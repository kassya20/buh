...
Процедура ТабличныйДокументВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ВидОтчетности" Тогда
		СтандартнаяОбработка = Ложь;
		Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < ДатаТолькоЛиквидационная() Тогда
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ОбработатьПоляВыбораВидаОтчетности();
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= ДатаТолькоЛиквидационная() Тогда
		УстановитьВидСдачиОтчетности(3);
	Иначе
...
&НаСервере
Процедура ОбработатьПоляВыбораВидаОтчетности()
	
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < ДатаТолькоЛиквидационная()
	     ИЛИ ТабличныйДокумент.Области.Найти("ВидОтчетности") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Блокировка поля выбора вида отчетности.
	ОбластьВидОтчетности = ТабличныйДокумент.Области["ВидОтчетности"];
	
	ОбластьВидОтчетности.Защита = Истина;
	ОбластьВидОтчетности.ЦветФона = Новый Цвет();
	
КонецПроцедуры

// Дата, с которой возможно предоставление только ликвидационной отчетности.
// 
// Возвращаемое значение:
//  Дата - Дата
// 
&НаКлиентеНаСервереБезКонтекста
Функция ДатаТолькоЛиквидационная()
