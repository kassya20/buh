...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	СчетаКассы = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.Касса);
	
	ИсключаемыеСчетаКассы = Новый Массив;
	ДенежныеДокументы = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ДенежныеДокументы);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаКассы, ДенежныеДокументы);
	
	ДенежныеДокументыВал = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ДенежныеДокументыВал);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаКассы, ДенежныеДокументыВал);
	
	СчетаКассы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаКассы, ИсключаемыеСчетаКассы);
	
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаКассы", СчетаКассы);
	
	СчетаУчетаПодотчет = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицами_);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаУчетаПодотчет", СчетаУчетаПодотчет);
	
	СчетаРасчетовПоНалогам = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РасчетыПоНалогам);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаРасчетовПоНалогам", СчетаРасчетовПоНалогам);
	...
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаРасчетовПоСоциальномуСтрахованию",
		СчетаРасчетовПоСоциальномуСтрахованию);

	СчетаПереводовВПути = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПереводыВПути_);
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаПереводовВПути", СчетаПереводовВПути);

	СчетаКассОрганизации = Новый Массив;
	
	КассаОрганизации = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаОрганизации);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаКассОрганизации, КассаОрганизации);
	
	КассаОрганизацииВал = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаОрганизацииВал);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаКассОрганизации, КассаОрганизацииВал);
	
	БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "СчетаКассОрганизации", СчетаКассОрганизации);

	СчетаУчетаБезналичные = Новый Массив;
	
	РасчетныеСчета = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РасчетныеСчета);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаБезналичные, РасчетныеСчета);
	
	ВалютныеСчета = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ВалютныеСчета);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаБезналичные, ВалютныеСчета);
	
	СчетЦифровогоРубля = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ЦифровойРубль);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаБезналичные, СчетЦифровогоРубля);

	СпециальныеСчета = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.СпециальныеСчета);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаБезналичные, СпециальныеСчета);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаУчетаБезналичные, СчетаПереводовВПути);

	ИсключаемыеСчетаУчетаБезналичные = Новый Массив;
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаУчетаБезналичные, ПриобретениеИностраннойВалюты);
	
	ПродажиПоПлатежнымКартам = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаУчетаБезналичные, ПродажиПоПлатежнымКартам);
	
	РеализацияИностраннойВалюты = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.РеализацияИностраннойВалюты);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаУчетаБезналичные, РеализацияИностраннойВалюты);
	
	СчетаУчетаБезналичные = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаУчетаБезналичные, ИсключаемыеСчетаУчетаБезналичные);
	...
	ОперационнаяКасса = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ОперационнаяКасса);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, ОперационнаяКасса);
	
	КассаПлатежногоАгента = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаПлатежногоАгента);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, КассаПлатежногоАгента);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, СчетаКассОрганизации);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, СчетаУчетаБезналичные);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, СчетаУчетаПодотчет);
	
	ПереводыВПути = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПереводыВПути);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, ПереводыВПути);

	ПереводыВПутиВал = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ПереводыВПутиВал);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаДС, ПереводыВПутиВал);

	ИсключаемыеСчетаДС = Новый Массив;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаДС, ДенежныеДокументы);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаДС, ДенежныеДокументыВал);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсключаемыеСчетаДС, СчетаПереводовВПути);

	СчетаДС = ОбщегоНазначенияКлиентСервер.РазностьМассивов(СчетаДС, ИсключаемыеСчетаДС);
	...
	МассивПолей = БухгалтерскиеОтчетыВызовСервера.ПолучитьМассивПолейРасшифровки(Расшифровка, ДанныеРасшифровки, КомпоновщикНастроек, Истина);
 	...
 	Для Каждого Отбор Из МассивПолей Цикл
		Если ТипЗнч(Отбор) = Тип("ЗначениеПоляРасшифровкиКомпоновкиДанных") тогда
			...
			Иначе
				Если Отбор.Поле = "ВидДенежныхСредств" Тогда
					Значение = "Наличные";
					Если Отбор.Значение = Отчеты.ПоступленияДенежныхСредств.ВидДенежныхСредствДеньгиНаРасчетныхСчетах() Тогда
						Значение = "Безналичные";
					ИначеЕсли Отбор.Значение = Отчеты.ПоступленияДенежныхСредств.ВидДенежныхСредствДеньгиУПодотчетныхЛиц() Тогда
						Значение = "Подотчетные лица";
					КонецЕсли;
				Иначе
				...
				Если Отбор.Иерархия Тогда
					БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, Отбор.Поле, Значение, ВидСравненияКомпоновкиДанных.ВИерархии);
				Иначе
					БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, Отбор.Поле, Значение);
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Отбор) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			ИначеЕсли Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидДенежныхСредств")
				И Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
				
				Значение = "Наличные";
				
				Если Отбор.ПравоеЗначение = Отчеты.ПоступленияДенежныхСредств.ВидДенежныхСредствДеньгиНаРасчетныхСчетах() Тогда
					Значение = "Безналичные";
				ИначеЕсли Отбор.ПравоеЗначение = Отчеты.ПоступленияДенежныхСредств.ВидДенежныхСредствДеньгиУПодотчетныхЛиц() Тогда
					Значение = "Подотчетные лица";
				КонецЕсли;
				
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ПользовательскиеОтборы, Отбор.ЛевоеЗначение, Значение, Отбор.ВидСравнения);
