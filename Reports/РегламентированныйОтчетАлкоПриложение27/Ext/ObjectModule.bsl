...
Процедура ОбработкаЗаписи(ИмяРегистра, Запись,
	...
	Если ИмяРегистра = "СведенияРеглОтчетАлкоПрил27Декларация" Тогда
...
Функция ЗаполнитьПоВсейОрганизацииДекларация(П000010000301, П000010000302, П000010000303)
	...
	|ВЫБРАТЬ
	|АлкоПрил27Декларация.П000010000305 КАК П000010000305,
	|АлкоПрил27Декларация.П0000100003051 КАК П0000100003051,
	|АлкоПрил27Декларация.П000010000306 КАК П000010000306,
	|АлкоПрил27Декларация.П000010000307 КАК П000010000307,
	|
	|СУММА(АлкоПрил27Декларация.П000010000304) КАК П000010000304
	|
	|ИЗ
	|	
	|(ВЫБРАТЬ
	|РегистрСведенийАлкоПрил27Декларация.*
	|
	|ИЗ
	|РегистрСведений.СведенияРеглОтчетАлкоПрил27Декларация КАК РегистрСведенийАлкоПрил27Декларация
	|
	|ГДЕ (РегистрСведенийАлкоПрил27Декларация.ИДДокИндСтраницы В (&МассивИДДокИндСтраницы)) 
	|И (РегистрСведенийАлкоПрил27Декларация.Активно = Истина)) КАК АлкоПрил27Декларация
	|	
    |СГРУППИРОВАТЬ ПО 
	|АлкоПрил27Декларация.П000010000305, 
	|АлкоПрил27Декларация.П0000100003051, 
	|АлкоПрил27Декларация.П000010000306, 
	|АлкоПрил27Декларация.П000010000307
	|
	|УПОРЯДОЧИТЬ ПО
	|АлкоПрил27Декларация.П000010000305 ВОЗР, 
	|АлкоПрил27Декларация.П0000100003051 ВОЗР, 
	|АлкоПрил27Декларация.П000010000306 ВОЗР, 
	|АлкоПрил27Декларация.П000010000307 ВОЗР
	|";
	
	ЗапросДекларация = Новый Запрос;
	ЗапросДекларация.Текст = ТекстЗапросаДекларация;
	
	СервисРегистровДекларация = СтруктураСервисРегистров["СведенияРеглОтчетАлкоПрил27Декларация"];
...
Процедура ЗаполнитьДвижения(ИдГруппы, НомерЛиста, ИдОП)
	...
	|	РегистрСведенийАлкоПрил27Декларация.*
	|ИЗ
	|	РегистрСведений." + ИмяРегистраСведений + " КАК РегистрСведенийАлкоПрил27Декларация
	|ГДЕ
	|	РегистрСведенийАлкоПрил27Декларация.ИДДокИндСтраницы = &ИДДокИндСтраницы
	|   И РегистрСведенийАлкоПрил27Декларация.Активно = Истина
	|	И РегистрСведенийАлкоПрил27Декларация.ИндексСтроки > &ТекущийИндексСтроки";
...
Функция ВерсияФорматаВыгрузки(Знач НаДату = Неопределено, ВыбраннаяФорма = Неопределено) Экспорт
	...
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение27.ВерсияФорматаВыгрузки(НаДату, ВыбраннаяФорма);
		
КонецФункции

Функция ТаблицаФормОтчета() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение27.ТаблицаФормОтчета();
	
КонецФункции

Функция ДанныеРеглОтчета(ЭкземплярРеглОтчета) Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение27.ДанныеРеглОтчета(ЭкземплярРеглОтчета);
	
КонецФункции

Функция ДеревоФормИФорматов() Экспорт
	
	Возврат Отчеты.РегламентированныйОтчетАлкоПриложение27.ДеревоФормИФорматов();
