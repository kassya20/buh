...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы._ИмяФайлТреб.КнопкаВыбора = 
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(_РеестрДокПОбНЛ.Количество()) Тогда 
		Элементы._РеестрДокПОбНЛ.ТекущаяСтрока = _РеестрДокПОбНЛ[0].ПолучитьИдентификатор();
	КонецЕсли;
КонецПроцедуры
...
Процедура СинхронизироватьСтроки()
	...
	Для Каждого Стр Из КУдалению Цикл 
		_ПредТипДог.ПолучитьЭлементы().Удалить(_ПредТипДог.ПолучитьЭлементы().Индекс(Стр));
	КонецЦикла;
	
	Для Каждого Стр Из ПредТипДогТек.ПолучитьЭлементы() Цикл 
		Найдено = _ПредТипДог.ПолучитьЭлементы().Добавить();
		СкопироватьРекурсивно(Найдено, Стр);
	КонецЦикла;
КонецПроцедуры
...
Функция ПроверкаДереваНаСервере()
	...
	РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
		"Отчеты.РегламентированноеУведомлениеРеестрДокументовПодтверждающихЛьготы.ПроверкаОтчета",
...
Функция ЗаполнитьНаОснованииФайловНаСервере(ФайлыВыгрузки, Очищать)
	...
	ПараметрыВыполненияВФоне.ЗапуститьВФоне = Истина;
	РезультатВыполнения = ДлительныеОперации.ВыполнитьВФоне(
		"Отчеты.РегламентированноеУведомлениеРеестрДокументовПодтверждающихЛьготы.КонсолидироватьВФоне",
