...
Процедура СформироватьОтчетыНаКлиенте()
	...
	// Для случаев, когда имеются старые формы регламентированной отчетности,
	// не поддерживающие формирование отчетов на сервере,
	// нужно вызывать получение формы отчета с имитацией заполнения на клиенте.
	// Формируются по умолчанию нулевые декларации.
	...
	Для Каждого ЗадачаОтчет Из ТаблицаОтчеты Цикл
		...
		Если ЗадачаОтчет.ФормироватьНаКлиенте
			...
			ЗадачаОтчет.Статус   = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусВРаботеСтрокой();
			ЗадачаОтчет.Обновить = Не Результат.ОтчетСформирован;
			ЗадачаОтчет.РегламентированныйОтчет = Результат.СохраненныйОтчет;
			ЗадачаОтчет.НулеваяДекларация = Истина;
			ЗадачаОтчет.СуммаПоДекларации = 0;
...
Функция СформироватьОтчетНаКлиенте(ЗадачаОтчет)
	...
	Результат.СохраненныйОтчет = СформироватьАвтоматическиРеглОтчет(ЗадачаОтчет, Истина);
...
&НаКлиенте
Функция СформироватьАвтоматическиРеглОтчет(ЗадачаОтчет, ЗаполнитьПриСоздании)
	
	Если ЗадачаОтчет.ИсточникОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		Возврат РегламентированнаяОтчетностьКлиент.СформироватьАвтоматическиРеглОтчет(
			ЗадачаОтчет.ИсточникОтчета,
			Объект.Организация,
			ЗадачаОтчет.КонецПериода,
			ЗадачаОтчет.НачалоПериода);
	КонецЕсли;
	...
	ФормаОтчета = ПолучитьФорму(СтрШаблон("Отчет.%1.Форма.%2", ЗадачаОтчет.ИсточникОтчета, ЗадачаОтчет.ВыбраннаяФорма),
		ПараметрыФормы, , Истина);
	
	ФормаОтчета.СохранитьНаКлиенте();
	
	Возврат ФормаОтчета.СтруктураРеквизитовФормы.мСохраненныйДок;
...
Функция ПараметрыТестаПрошлыхПериодов()
	...
	Результат.Вставить("Организация",                 Объект.Организация);
	Результат.Вставить("НачальныйВопрос",             "ГруппаТест1");
	Результат.Вставить("ОтчетныйПериод",              ОтчетныйПериод);
	Результат.Вставить("Правило",                     Правило);
	Результат.Вставить("ГосударственныйОрган",        Перечисления.ТипыКонтролирующихОрганов.ФНС);
