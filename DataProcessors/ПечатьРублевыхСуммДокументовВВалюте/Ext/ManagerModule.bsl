...
Процедура ЗаполнитьТабличныйДокументСправки(ТабличныйДокумент, ВыборкаДокументов, ВыборкаСуммПоДокументам, ЕстьПредоплаты, ВыборкаПредоплат, ВалютаРегламентированногоУчета, Макет)
	...
	ВозвратТоваровОтПокупателяПоКурсуОснования = Ложь;
	ВозвратТоваровПоставщикуПоКурсуОснования   = Ложь;
	
	Если ТипСсылки = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")
		Или ТипСсылки = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		Если ВыборкаДокументов.ВВозвратеУчитываетсяКурсДокументаОснования Тогда
			
			УчетВозвратаПоКурсуДокументаОснования =
				УчетВзаиморасчетов.ПрименяетсяРасчетКорректировокВВалютеПоКурсуНаДатуОтгрузки(ВыборкаДокументов.Организация, ВыборкаДокументов.Дата);
			ВозвратТоваровОтПокупателяПоКурсуОснования = ТипСсылки = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя")
				И УчетВозвратаПоКурсуДокументаОснования;
			ВозвратТоваровПоставщикуПоКурсуОснования = ТипСсылки = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
				И УчетВозвратаПоКурсуДокументаОснования;
		КонецЕсли;
	...
	СреднийКурсДокументаОтгрузки = 0;
	Если ВозвратТоваровОтПокупателяПоКурсуОснования Тогда
		Если СуммаПредоплатыВал = 0 Тогда
			СуммаПредоплатыРуб = 0;
		Иначе
			СреднийКурсДокументаОтгрузки = Окр(ВыборкаСуммПоДокументам.ВсегоРуб / СуммаДокументаВал, 4);
			СуммаПредоплатыРуб = СуммаПредоплатыВал * СреднийКурсДокументаОтгрузки;
		КонецЕсли;
	Иначе
	...
	Если ЗначениеЗаполнено(ВыборкаДокументов.КурсВзаиморасчетов) И ЗначениеЗаполнено(ВыборкаДокументов.КратностьВзаиморасчетов) Тогда
		Если ВозвратТоваровПоставщикуПоКурсуОснования Или  ВозвратТоваровОтПокупателяПоКурсуОснования Тогда
			КурсДокумента = Окр(ВыборкаСуммПоДокументам.ВсегоРуб / СуммаДокументаВал, 4);
	...
	Если СуммаНеоплаченоВал <> 0 Тогда
		...
		СуммаНеоплаченоРуб = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СуммаНеоплаченоВал,
	...
	КонецЕсли;
	Если ВозвратТоваровОтПокупателяПоКурсуОснования Тогда
		ОбластьПоказатели.Параметры.ЗаголовокДляСтроки1_6 = НСтр("ru = 'Средний курс валюты по документу отгрузки'");
	ИначеЕсли ВозвратТоваровПоставщикуПоКурсуОснования Тогда
		ОбластьПоказатели.Параметры.ЗаголовокДляСтроки1_6 = НСтр("ru = 'Средний курс валюты по документам поступлений'");
	Иначе
