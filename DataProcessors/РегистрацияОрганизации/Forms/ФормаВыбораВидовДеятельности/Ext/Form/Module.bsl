...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ЮридическоеФизическоеЛицо", ЮридическоеФизическоеЛицо);
...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("Подключаемый_ПриВыбореВидаДеятельности", 0.1, Истина);
...
Процедура ЗаполнитьРазделы()
	...
	Раздел.Порядок = -1; // Строка должна быть первой всегда
...
Процедура ЗаполнитьПопулярныеКодыВЭД()
	...
	ЭтоЮрЛицо = ЭтоЮрЛицо(ЮридическоеФизическоеЛицо);
	
	Если Не ЗначениеЗаполнено(ЮридическоеФизическоеЛицо) Или Не ЭтоЮрЛицо Тогда
		МакетыСКатегориями.Добавить(ПолучитьОбщийМакет("КатегорииОКВЭД_ИП"));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ЮридическоеФизическоеЛицо) Или ЭтоЮрЛицо Тогда
...
Процедура ОтобразитьНайденные()
	...
	Если ОтобразитьНайденные Тогда
		РезультатПоиска = КлассификаторОКВЭДБП.НайтиДанные(СтрокаПоиска, Истина);
		КлассификаторПоиск.Очистить();
		Для Каждого СтрокаРезультата Из РезультатПоиска Цикл
			СтрокаКлассификаторПоиск = КлассификаторПоиск.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаКлассификаторПоиск, СтрокаРезультата);
			СтрокаКлассификаторПоиск.Выбран = ЗначениеЗаполнено(
				Выбранные.НайтиСтроки(Новый Структура("Код", СтрокаРезультата.Код)));
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ПриВыбореВидаДеятельности()
	
	ВидимостьПодсказкиОСудимости = Ложь;
	
	Если Не ЭтоЮрЛицо(ЮридическоеФизическоеЛицо) Тогда
		
		ВыбранныеВидыДеятельности = Новый Массив;
		Для Каждого ТекущийОКВЭД Из Выбранные Цикл
			ВыбранныеВидыДеятельности.Добавить(ТекущийОКВЭД.Код);
		КонецЦикла;
		
		ВидимостьПодсказкиОСудимости = ТребуетсяСправкаОбОтсутствииСудимости(ВыбранныеВидыДеятельности);
		
	КонецЕсли;
	
	Элементы.ПодсказкаСправкаОСудимости.Видимость = ВидимостьПодсказкиОСудимости;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоЮрЛицо(ВидЛица)
	
	Возврат (ВидЛица = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо"));
