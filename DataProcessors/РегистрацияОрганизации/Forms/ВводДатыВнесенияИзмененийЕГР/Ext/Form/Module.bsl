...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИзмененияВРеестре = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеПомощникаЗаполнения, "Изменено");
	ДанныеОрганизации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеПомощникаЗаполнения, "ДанныеОрганизации");
	
	ДатаИзмененийЕГР = ТекущаяДатаСеанса();
	
	Если ЗначениеЗаполнено(ИзмененияВРеестре) Тогда
		
		ЭтоИзмененияВРесстре = Истина;
		...
		Если ИзмененияВРеестре.Руководитель Тогда
		...
		Если ИзмененияВРеестре.ОсновнойВидДеятельности Тогда
			КодОКВЭД = ДанныеПомощникаЗаполнения.КодОКВЭД2Новый;
		ИначеЕсли ИзмененияВРеестре.ВидыДеятельности Тогда
			КодОКВЭД = ДанныеОрганизации.КодОКВЭД2;
		КонецЕсли;
	...
	Иначе
		...
		ЭтоИзмененияВРесстре = Ложь;
		ДанныеИП = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОрганизации, "ИндивидуальныйПредприниматель");
		АдресПропискиJSON = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеИП, "АдресПропискиЗначениеJSON", "");
		СтруктураУчетнойПолитики = РегистрацияОрганизацииСервер.СтруктураУчетнойПолитикиПоДаннымОрганизации(
			ДанныеОрганизации,
			ДатаИзмененийЕГР);
		Если ЗначениеЗаполнено(СтруктураУчетнойПолитики) Тогда
			СистемаНалогообложения = ОрганизацииФормыКлиентСервер.ПредставлениеСистемыНалогообложения(СтруктураУчетнойПолитики);
		КонецЕсли;
		
		КодОКВЭД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОрганизации, "КодОКВЭД2");
		НаименованиеПолное = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОрганизации, "Ссылка");
		ЮридическийАдрес = УправлениеКонтактнойИнформацией.ПредставлениеАдреса(АдресПропискиJSON);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоИзмененияВРесстре Тогда
...
Процедура ОбновитьРеквизиты(Команда)
	...
	Результат.Вставить("ДатаИзмененийЕГР", ДатаИзмененийЕГР);
...
Процедура УправлениеФормой()
	...
	Если ЭтоИзмененияВРесстре Тогда
		...
		Элементы.СистемаНалогообложения.Видимость = Ложь;
	Иначе
		Элементы.ЗаголовокНаименованиеПолное.Заголовок = НСтр("ru = 'Индивидуальный предприниматель'");
		Элементы.ЗаголовокЮридическийАдрес.Заголовок = НСтр("ru = 'Адрес регистрации'");
		Элементы.ДатаИзмененийЕГР.Заголовок = НСтр("ru = 'Дата регистрации в реестре'");
		Элементы.ОГРН.РасширеннаяПодсказка.Заголовок = РегистрацияОрганизацииКлиентСервер.ТекстПодсказкиДляОГРНИП(ДатаИзмененийЕГР);
		Элементы.ГруппаНаименованиеСокращенное.Видимость = Ложь;
		Элементы.ГруппаРуководитель.Видимость = Ложь;
		Элементы.ГруппаРуководительДолжность.Видимость = Ложь;
		Элементы.СписокУчредителей.Видимость = Ложь;
		Элементы.ОГРН.Видимость = Истина;
		Элементы.СистемаНалогообложения.Видимость = ЗначениеЗаполнено(СистемаНалогообложения);
	КонецЕсли;
