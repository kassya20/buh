...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоАдресВременногоХранилища(Параметры.КодыОКВЭДИзЕГРЮЛ) Тогда
		СписокКодовОКВЭДИзЕГРЮЛ = ПолучитьИзВременногоХранилища(Параметры.КодыОКВЭДИзЕГРЮЛ);
		Если ТипЗнч(СписокКодовОКВЭДИзЕГРЮЛ) = Тип("Массив") Тогда
			КодыОКВЭДИзЕГРЮЛ.ЗагрузитьЗначения(СписокКодовОКВЭДИзЕГРЮЛ);
		КонецЕсли;
	КонецЕсли;
	...
	УстановитьВидимостьПодсказкиСправкаОСудимости(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьПодсказкиСправкаОСудимости(Форма)
	
	ВидимостьПодсказки = Ложь;
	
	Если Не ЭтоЮрЛицо(Форма.ЮридическоеФизическоеЛицо) Тогда
		
		ПроверяемыеОКВЭД = Новый Массив;
		Для Каждого ТекущийВид Из Форма.ВидыДеятельности Цикл
			ПроверяемыеОКВЭД.Добавить(ТекущийВид.Код);
		КонецЦикла;
		
		// Убираем из проверки коды ОКВЭД, которые уже есть в ЕГРЮЛ.
		// По умолчанию, считаем, что по действующим ИП налоговая уже проверила судимость
		// и выслала уведомления о запрете на ведение деятельности.
		Для Каждого ТекущийОКВЭД Из Форма.КодыОКВЭДИзЕГРЮЛ Цикл
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ПроверяемыеОКВЭД, ТекущийОКВЭД.Значение);
		КонецЦикла;
		
		ВидимостьПодсказки = ТребуетсяСправкаОбОтсутствииСудимости(ПроверяемыеОКВЭД);
		
	КонецЕсли;
	
	Форма.Элементы.ПодсказкаСправкаОСудимости.Видимость = ВидимостьПодсказки;
