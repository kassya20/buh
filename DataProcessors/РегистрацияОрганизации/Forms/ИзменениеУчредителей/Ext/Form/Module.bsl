...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СуммаУставногоКапитала = Параметры.УставныйКапитал;
	СуммаУставногоКапиталаДоИзменения = Параметры.УставныйКапиталДоИзменения;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		ПроверитьДублиВТабличнойЧасти(Отказ);
		...
		// Уставный капитал
		СуммаУставногоКапитала = Учредители.Итог("СуммаВзноса");
		Если СуммаУставногоКапитала < СуммаУставногоКапиталаДоИзменения Тогда
		...
		Иначе
			...
			Если СуммаУставногоКапитала < МинимальныйУставныйКапитал Тогда
...
&НаКлиенте
Процедура УчредителиСсылкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если ЗначениеЗаполнено(ВыбранноеЗначение)
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ВыбранноеЗначение, "ФизическоеЛицо")
		И Элементы.Учредители.ТекущиеДанные <> Неопределено Тогда
		
		Элементы.Учредители.ТекущиеДанные.Ссылка = ВыбранноеЗначение.ФизическоеЛицо;
		Элементы.Учредители.ТекущиеДанные.Наименование = ВыбранноеЗначение.Наименование;
		СтандартнаяОбработка = Ложь;
...
Процедура ПересчитатьДолиУчастия()
	...
	МассивДолей = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(100, МассивКоэффициентов, 15);
...
Процедура РаспределитьУставныйКапитал()
	...
	МассивСумм = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СуммаУставногоКапитала, МассивКоэффициентов, 2);
...
Процедура ОповеститьОВыбореИЗакрыть()
	...
	РезультатВыбора.Вставить("УставныйКапитал", СуммаУставногоКапитала);
...
&НаСервере
Функция ПроверитьДублиВТабличнойЧасти(Отказ)
	
	Результат = Ложь;
	
	ТаблицаУчредители = Учредители.Выгрузить( , "Ссылка");
	ТаблицаУчредители.Свернуть("Ссылка");
	
	Если Учредители.Количество() > ТаблицаУчредители.Количество() Тогда
		
		Результат = Истина;
		
		Для Каждого ТекущийУчредитель Из ТаблицаУчредители Цикл
			
			Отбор = Новый Структура("Ссылка", ТекущийУчредитель.Ссылка);
			НайденныеСтроки = Учредители.НайтиСтроки(Отбор);
			КоличествоУчредителей = НайденныеСтроки.Количество();
			
			Если КоличествоУчредителей > 1 Тогда
				
				Для Индекс = 1 По КоличествоУчредителей - 1 Цикл
					
					НомерСтроки = Учредители.Индекс(НайденныеСтроки[Индекс]) + 1;
					ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(
						"Колонка", "Корректность",
						"Учредитель",
						НомерСтроки,
						НСтр("ru = 'Учредители'"),
						СтрШаблон(НСтр("ru = 'Учредитель %1 уже был указан ранее'"), НайденныеСтроки[Индекс].Наименование));
					Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Учредители", НомерСтроки, "Ссылка");
