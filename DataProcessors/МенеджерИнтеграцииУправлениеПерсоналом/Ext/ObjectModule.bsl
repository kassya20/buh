...
Функция РесурсОбновлениеДанныхТарифа()

	Возврат "/subscribers/plans/updates";

КонецФункции

Функция РесурсАктивацияИС()

	Возврат "/information-systems/{ID}/restore";
...
Функция ИнформацияОбАбоненте() Экспорт
	...
	ИначеЕсли Ответ.КодСостояния = 200 Тогда
		...
		ЧтениеJSON.УстановитьСтроку(Ответ.ПолучитьТелоКакСтроку());
		Попытка
			ОбъектDTO = ПрочитатьJSON(ЧтениеJSON, Истина, ИменаСвойствСервисаСоЗначениямиДата());
		Исключение
			СообщениеОбОшибке = НСтр("ru='Ошибка чтения JSON.'");
			Результат.СообщениеОбОшибке = СтрШаблон("%1 %2", СообщениеОбОшибке, ПодробностиВЖурналеРегистрации());
			ЗаписатьОшибкуЧтенияJSON(Ответ.ПолучитьТелоКакСтроку(), ОписаниеОшибки());
...
Функция ОбновитьАдминистратораПриложения(ИдентификаторПриложения, ОписаниеАдминистратора) Экспорт
	...
	Результат = Новый Структура("ОписаниеАдминистратора,АдминистраторИзменен,СообщениеОбОшибке", Ложь);
	...
	ИначеЕсли Ответ.КодСостояния = 200 Тогда
		...
		Исключение
			...
			Возврат Результат;
		КонецПопытки;
		Результат.ОписаниеАдминистратора = ОписаниеАдминистратораПоОбъектуDTO(ОбъектDTO);
	ИначеЕсли Ответ.КодСостояния = 400 Тогда
		СообщениеОбОшибке = НСтр("ru='Некорректный запрос.'");
		Результат.СообщениеОбОшибке = СтрШаблон("%1 %2", СообщениеОбОшибке, ПодробностиВЖурналеРегистрации());	
	ИначеЕсли Ответ.КодСостояния = 404 Тогда
...
Функция ОбновитьСвойстваПриложения(ИдентификаторПриложения, ИмяПриложения) Экспорт

	Результат = Новый Структура("ОписаниеПриложения,СообщениеОбОшибке");
	
	ОбъектDTO = Новый Соответствие;
	ОбъектDTO.Вставить("name", ИмяПриложения);
	СтрокаТела = ИнтеграцияУправлениеПерсоналом.СформироватьJSON(ОбъектDTO);
	
	АдресРесурса = СтрЗаменить(РесурсЗапросПриложение(),"{ID}",ИдентификаторПриложения);
	ПараметрыЗапроса = ПараметрыЗапроса(АдресРесурса, "PATCH", СтрокаТела);
...
Функция ОбновитьДанныеТарифа() Экспорт
	
	Результат = Новый Структура("СообщениеОбОшибке");

	АдресРесурса = РесурсОбновлениеДанныхТарифа();
	ПараметрыЗапроса = ПараметрыЗапроса(АдресРесурса, "POST");
...
Функция ПолучениеКодаАктивации(ИдентификаторИС) Экспорт
	
	Результат = Новый Структура("КодАктивации,СообщениеОбОшибке");
	
	АдресРесурса = СтрЗаменить(РесурсАктивацияИС(),"{ID}",ИдентификаторИС);
	ПараметрыЗапроса = ПараметрыЗапроса(АдресРесурса,"GET");
	...
	ИначеЕсли Ответ.КодСостояния = 200 Тогда
		...
		Исключение
			...
			Возврат Результат;
		КонецПопытки;
		Результат.КодАктивации = ОбъектDTO["code"];
	ИначеЕсли Ответ.КодСостояния = 400 Тогда
		СообщениеОбОшибке = НСтр("ru='Некорректный запрос.'");
		Результат.СообщениеОбОшибке = СтрШаблон("%1 %2", СообщениеОбОшибке, ПодробностиВЖурналеРегистрации());	
	ИначеЕсли Ответ.КодСостояния = 404 Тогда
...
Функция ПодключениеИСПоКодуАктивации(ИдентификаторИС, КодАктивации) Экспорт
	...
	ОбъектDTO = Новый Соответствие;
	ОбъектDTO.Вставить("code", КодАктивации);
	СтрокаТела = ИнтеграцияУправлениеПерсоналом.СформироватьJSON(ОбъектDTO);
	
	ПараметрыЗапроса = ПараметрыЗапроса(АдресРесурса, "POST", СтрокаТела);
	...
	ИначеЕсли Ответ.КодСостояния = 200 Тогда
		...
		Исключение
			...
			Возврат Результат;
		КонецПопытки;
		Результат.ИнформационнаяСистема = ИнформационнаяСистемаПоОбъектуDTO(ОбъектDTO);
	ИначеЕсли Ответ.КодСостояния = 400 Тогда
		СообщениеОбОшибке = НСтр("ru='Некорректный запрос.'");
		Результат.СообщениеОбОшибке = СтрШаблон("%1 %2", СообщениеОбОшибке, ПодробностиВЖурналеРегистрации());
	ИначеЕсли Ответ.КодСостояния = 403 Тогда
...
Функция ОбъектРазвертыванияВОбъектDTO(ОписаниеОбъекта)
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ОбъектФизическоеЛицо = ФизическоеЛицоАдминистраторОбъектDTO(ОписаниеОбъекта.ФизическоеЛицо);
		ОбъектDTO.Вставить("person", ОбъектФизическоеЛицо);
	Иначе
...
Функция ОписаниеАдминистратораПоОбъектуDTO(ОбъектDTO)
	
	Описание = НовоеОписаниеАдминистратора();
	
	Описание.Логин 						= ОбъектDTO["username"];
	Описание.Телефон 					= ОбъектDTO["phone"];
	Описание.АдресЭлектроннойПочты 		= ОбъектDTO["email"];
	Описание.СсылкаДляАдминистратора 	= ОбъектDTO["logonUrl"];
	Описание.ФизическоеЛицо 			= ОбъектDTO["personID"];
