...
//      * ПрикладнаяЗагрузка - Булево - признак, что загрузка выполняется при вводе начальных остатков.
//      * ВестиУчетПоДоговорам - Булево - признак, что в программе ведется учет по договорам.
//      * Организация - СправочникСсылка.Организации - организация, для которой выполняется ввод начальных остатков.
//      * ВидДоговора - ПеречислениеСсылка.ВидыДоговоровКонтрагентов - вид договора, для поиска договора контрагента при вводе начальных остатков.
//   АдресРезультата - УникальныйИдентификатор - адрес во временном хранилище для сохранения результатов.
//
Процедура ПолучитьТаблицуДанныхДляЗагрузки(ПараметрыЗадания, АдресРезультата) Экспорт
	...
	Для НомерСтроки = 2 По ВысотаТаблицы Цикл
		...
		Для НомерКолонки = 1 По ШиринаТаблицы Цикл
			...
			Если ТекущаяОбласть.ПараметрРасшифровки <> "НеЗагружать" Тогда
				ТекстТекущейОбласти = СокрЛП(ТекущаяОбласть.Текст);
				Если ЗначениеЗаполнено(ТекстТекущейОбласти)
					И ИмяКолонки = "Задолженность" Или ИмяКолонки = "Аванс" Тогда
					НоваяСтрока.ЗначенияКолонок.Вставить(ИмяКолонки, ПолучитьЧислоИзСтроки(ТекстТекущейОбласти));
				Иначе
					НоваяСтрока.ЗначенияКолонок.Вставить(ИмяКолонки, ТекстТекущейОбласти);
				КонецЕсли;
			КонецЕсли;
		...
		НоваяСтрока.Представление = СтрШаблон(НСтр("ru = 'Новый: %1'"), НоваяСтрока.Наименование);
		
		Если ПустаяСтрока(НоваяСтрока.Договор) Тогда
			НоваяСтрока.Договор = РаботаСДоговорамиКонтрагентовБПКлиентСервер.НаименованиеБезДоговора();
		КонецЕсли;
		
		НоваяСтрока.ПредставлениеДоговора = СтрШаблон(НСтр("ru = 'Новый: %1'"), НоваяСтрока.Договор);
		
		НоваяСтрока.Статус = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовый();
		НоваяСтрока.СтатусКонтрагента = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовый();
		НоваяСтрока.СтатусДоговора = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовый();
	...
	Если ПараметрыЗадания.ПрикладнаяЗагрузка И ПараметрыЗадания.ВестиУчетПоДоговорам Тогда
		НайтиДоговорыКонтрагентов(ЗагружаемыеДанные, ПараметрыЗадания.Организация, ПараметрыЗадания.ВидДоговора);
	КонецЕсли;
...
Процедура ЗагрузитьДанныеИзФайла(ПараметрыЗадания, АдресРезультата) Экспорт
	...
	ВестиУчетПоДоговорам = ПараметрыЗадания.ВестиУчетПоДоговорам;
	Организация = ПараметрыЗадания.Организация;
	ВидДоговора = ПараметрыЗадания.ВидДоговора;
	
	ПараметрыДоговора = НовыеПараметрыДоговора();
	ПараметрыДоговора.Организация = Организация;
	ПараметрыДоговора.ВидДоговора = ВидДоговора;
	
	ПоляПоискаКонтрагента = "Наименование, ИНН, КПП, Контрагент";
	ТаблицаПоискаКонтрагента = ЗагружаемыеДанные.СкопироватьКолонки(ПоляПоискаКонтрагента);
	
	ПоляПоискаДоговора = "Контрагент, Договор, ДоговорКонтрагента";
	ТаблицаПоискаДоговора = ЗагружаемыеДанные.СкопироватьКолонки(ПоляПоискаДоговора);
	...
	Для Каждого СтрокаДанных Из ЗагружаемыеДанные Цикл
		...
		СоздатьКонтрагента = Не ЗначениеЗаполнено(СтрокаДанных.Контрагент);
		СоздатьДоговор = ПрикладнаяЗагрузка И ВестиУчетПоДоговорам И Не ЗначениеЗаполнено(СтрокаДанных.ДоговорКонтрагента);
		
		Если СоздатьКонтрагента Тогда
			// Попробуем найти контрагента в уже обработанных данных, чтобы не загружать дубли
			Если ЗначениеЗаполнено(СтрокаДанных.ИНН) Тогда
				ПараметрыПоискаКонтрагента = Новый Структура("ИНН", СтрокаДанных.ИНН);
				Если ЗначениеЗаполнено(СтрокаДанных.КПП) Тогда
					ПараметрыПоискаКонтрагента.Вставить("КПП", СтрокаДанных.КПП);
				КонецЕсли;

				НайденныеСтрокиКонтрагента = ТаблицаПоискаКонтрагента.НайтиСтроки(ПараметрыПоискаКонтрагента);
				Если НайденныеСтрокиКонтрагента.Количество() > 0 Тогда
					СтрокаДанных.Контрагент = НайденныеСтрокиКонтрагента[0].Контрагент;
				КонецЕсли;
			Иначе
				ПараметрыПоискаКонтрагента = Новый Структура("Наименование, ИНН", СтрокаДанных.Наименование, "");
				НайденныеСтрокиКонтрагента = ТаблицаПоискаКонтрагента.НайтиСтроки(ПараметрыПоискаКонтрагента);
				Если НайденныеСтрокиКонтрагента.Количество() > 0 Тогда
					СтрокаДанных.Контрагент = НайденныеСтрокиКонтрагента[0].Контрагент;
				КонецЕсли;
		...
		Если СоздатьДоговор И Не СоздатьКонтрагента Тогда
			// Попробуем найти договор в уже обработанных данных, чтобы не загружать дубли
			ПараметрыПоискаДоговора = Новый Структура("Контрагент, Договор",
				СтрокаДанных.Контрагент, СтрокаДанных.Договор);
			НайденныеСтрокиДоговора = ТаблицаПоискаДоговора.НайтиСтроки(ПараметрыПоискаДоговора);
			Если НайденныеСтрокиДоговора.Количество() > 0 Тогда
				СтрокаДанных.ДоговорКонтрагента = НайденныеСтрокиДоговора[0].ДоговорКонтрагента;
			КонецЕсли;
			СоздатьДоговор = Не ЗначениеЗаполнено(СтрокаДанных.ДоговорКонтрагента);
		КонецЕсли;
		
		Если Не СоздатьКонтрагента И Не СоздатьДоговор Тогда
			Продолжить;
		КонецЕсли;
		...
		Попытка
			Если СоздатьКонтрагента Тогда
				ИНН = СтрокаДанных.ИНН;
				РеквизитыКонтрагентаИзСервиса = Неопределено;
				Если ЗначениеЗаполнено(ИНН) Тогда
					...
					Если Не ЗначениеЗаполнено(ДанныеСервиса.ОписаниеОшибки)
						...
						РеквизитыКонтрагентаИзСервиса = ПолучитьИзВременногоХранилища(ДанныеСервиса.АдресРеквизитовКонтрагента);
					КонецЕсли;
				КонецЕсли;

				ДанныеЗаполненияКонтрагента = ДанныеЗаполненияКонтрагента(СтрокаДанных, РеквизитыКонтрагентаИзСервиса);
				...
				Если ДанныеЗаполненияКонтрагента.Свойство("ДанныеКонтактногоЛица") Тогда
					НовыйКонтрагент.ДополнительныеСвойства.Вставить("ДанныеКонтактногоЛица",
						ДанныеЗаполненияКонтрагента.ДанныеКонтактногоЛица);
				КонецЕсли;
				...
				ОсновнойБанковскийСчет = СоздатьБанковскийСчет(
					НовыйКонтрагент.Ссылка, ДанныеЗаполненияБанковскихСчетов, Ошибки);
			
				// При создании банковского счета, начинающегося с 40802 для владельца физического лица реквизит ИндивидуальныйПредприниматель
				// устанавливается в значение Истина, поэтому необходимо обновить объект НовыйКонтрагент
				НовыйКонтрагент = НовыйКонтрагент.Ссылка.ПолучитьОбъект();
				...
				ЗаполнитьЗначенияСвойств(ТаблицаПоискаКонтрагента.Добавить(), СтрокаДанных);
			КонецЕсли;
			
			Если СоздатьДоговор Тогда
				Если ПустаяСтрока(СтрокаДанных.Договор) Тогда
					НаименованиеДоговора = РаботаСДоговорамиКонтрагентовБПКлиентСервер.НаименованиеБезДоговора();
				Иначе
					НаименованиеДоговора = СтрокаДанных.Договор;
				КонецЕсли;
				
				ЗаполнитьНомерИДатуДоговора(ПараметрыДоговора, НаименованиеДоговора);
				
				ПараметрыДоговора.Владелец = СтрокаДанных.Контрагент;
				ПараметрыДоговора.Наименование = НаименованиеДоговора;
				...
				СтрокаДанных.ДоговорКонтрагента = ДоговорКонтрагента;
				
				ЗаполнитьЗначенияСвойств(ТаблицаПоискаДоговора.Добавить(), СтрокаДанных);
			КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(ТекстОшибки, СтрокаДанных.Наименование, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "", ТекстСообщения, "");
			
			ТекстСообщения = СтрШаблон(ТекстОшибки, СтрокаДанных.Наименование, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ПрикладнаяЗагрузка Тогда
		ОстаткиВзаиморасчетов = ЗагружаемыеДанные.Скопировать(, "Контрагент, ДоговорКонтрагента, Задолженность, Аванс");
		ОстаткиВзаиморасчетов.Свернуть("Контрагент, ДоговорКонтрагента", "Задолженность, Аванс");
		
		ПодготовитьВзаиморасчетыДляВводаОстатков(ОстаткиВзаиморасчетов);
		
		Результат.Вставить("ОстаткиВзаиморасчетов", ОбщегоНазначения.ТаблицаЗначенийВМассив(ОстаткиВзаиморасчетов));
	КонецЕсли;
...
Функция ОписаниеЗагружаемыхКолонок(ПрикладнаяЗагрузка = Ложь) Экспорт
	...
	Если ПрикладнаяЗагрузка Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам") Тогда
			// Договор
			НоваяСтрока = ОписаниеКолонок.Добавить();
			НоваяСтрока.Идентификатор = "Договор";
			НоваяСтрока.ПредставлениеКолонки = НСтр("ru = 'Договор'");
			НоваяСтрока.ПодходящиеЗаголовкиФайла = ПодходящиеЗаголовкиДоговор();
		КонецЕсли;
		
		// Задолженность
		НоваяСтрока = ОписаниеКолонок.Добавить();
		НоваяСтрока.Идентификатор = "Задолженность";
		НоваяСтрока.ПредставлениеКолонки = НСтр("ru = 'Задолженность'");
		НоваяСтрока.ПодходящиеЗаголовкиФайла = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("Задолженность");
	
		// Аванс
		НоваяСтрока = ОписаниеКолонок.Добавить();
		НоваяСтрока.Идентификатор = "Аванс";
		НоваяСтрока.ПредставлениеКолонки = НСтр("ru = 'Аванс'");
		НоваяСтрока.ПодходящиеЗаголовкиФайла = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("Аванс");
...
Функция ПодходящиеЗаголовкиДоговор()
	
	МассивЗаголовков = Новый Массив;
	МассивЗаголовков.Добавить("Договор");
	МассивЗаголовков.Добавить("Договор контрагента");
...
Процедура НайтиКонтрагентов(ЗагружаемыеДанные)
	...
	Для Каждого СтрокаТаблицы Из ЗагружаемыеДанные Цикл
		...
		Если Выборка.НайтиСледующий(ПараметрыПоиска) Тогда
			...
			СтрокаТаблицы.СтатусКонтрагента = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНайденный();
		КонецЕсли;
...
Процедура НайтиДоговорыКонтрагентов(ЗагружаемыеДанные, Организация, ВидДоговора)
	
	Если Не ЗначениеЗаполнено(Организация) Или Не ЗначениеЗаполнено(ВидДоговора) Тогда
		Возврат;
	КонецЕсли;
	...
	"ВЫБРАТЬ
	|	ТаблицаДляПоиска.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДляПоиска.Контрагент КАК Контрагент,
	|	ТаблицаДляПоиска.Договор КАК Договор
	|ПОМЕСТИТЬ ВТ_ТаблицаДляПоиска
	|ИЗ
	|	&ТаблицаДляПоиска КАК ТаблицаДляПоиска
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаДляПоиска.НомерСтроки КАК НомерСтроки,
	|	ВТ_ТаблицаДляПоиска.Контрагент КАК Контрагент,
	|	ВТ_ТаблицаДляПоиска.Договор КАК Договор,
	|	ДоговорыКонтрагентов.Ссылка КАК ДоговорКонтрагента
	|ИЗ
	|	ВТ_ТаблицаДляПоиска КАК ВТ_ТаблицаДляПоиска
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО ВТ_ТаблицаДляПоиска.Контрагент = ДоговорыКонтрагентов.Владелец
	|		И ВТ_ТаблицаДляПоиска.Договор = ДоговорыКонтрагентов.Наименование
	|		И ВТ_ТаблицаДляПоиска.Договор <> &ПустаяСтрока
	|ГДЕ
	...
	Для Каждого СтрокаТаблицы Из ЗагружаемыеДанные Цикл
		...
		Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
			СтрокаТаблицы.ДоговорКонтрагента = ДоговорКонтрагента;
			СтрокаТаблицы.СтатусДоговора = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНайденный();
		Иначе
			СтрокаТаблицы.Статус = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовыйДоговор();
		КонецЕсли;
...
Функция ДанныеЗаполненияКонтрагента(СтрокаДанных, ДанныеИзСервиса)
	...
	ОГРН = ЗначениеКолонкиПоИдентификатору(СтрокаДанных.ЗначенияКолонок, "ОГРН");
	...
	ДанныеЗаполнения.Вставить("РегистрационныйНомер", ОГРН);
	...
	Если ДанныеИзСервиса <> Неопределено Тогда
		...
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ЮридическийАдрес) И ДанныеИзСервиса.Свойство("ЮридическийАдрес")
		...
		Если ЭтоФизическоеЛицо И ДанныеИзСервиса.Свойство("СвидетельствоОРегистрации") Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ОГРН) И ДанныеИзСервиса.Свойство("РегистрационныйНомер") Тогда
...
Процедура ЗаполнитьНомерИДатуДоговора(ПараметрыДоговора, НаименованиеДоговора)
	
	ПараметрыДоговора.Номер = "";
	ПараметрыДоговора.Дата = Дата('00010101');
	
	Если НаименованиеДоговора = РаботаСДоговорамиКонтрагентовБПКлиентСервер.НаименованиеБезДоговора() Тогда
		Возврат;
	КонецЕсли;
	
	МассивДанных = СтрРазделить(НаименованиеДоговора, " ", Ложь);
	
	Если МассивДанных.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИндексДанных = МассивДанных.Найти(НСтр("ru = 'от'"));
	Если ИндексДанных <> Неопределено Тогда
		
		Если ИндексДанных <> 0 И МассивДанных.ВГраница() >= ИндексДанных - 1 Тогда
			Номер = МассивДанных[ИндексДанных - 1];
			ЕстьЦифры = СтрРазделить(Номер, "0123456789", Истина).Количество() > 1;
			Если ЕстьЦифры Тогда
				ПараметрыДоговора.Номер = Номер;
			КонецЕсли;
		КонецЕсли;
		
		Если МассивДанных.ВГраница() >= ИндексДанных + 1 Тогда
			Дата = СтроковыеФункцииКлиентСервер.СтрокаВДату(МассивДанных[ИндексДанных + 1]);
			ПараметрыДоговора.Дата = Дата;
		КонецЕсли;
...
Функция НовыеПараметрыДоговора()
	
	ПараметрыДоговора = Новый Структура;
	ПараметрыДоговора.Вставить("Организация", Неопределено);
	ПараметрыДоговора.Вставить("ВидДоговора", Неопределено);
	ПараметрыДоговора.Вставить("Владелец", Неопределено);
	ПараметрыДоговора.Вставить("Наименование", "");
	ПараметрыДоговора.Вставить("Номер", "");
	ПараметрыДоговора.Вставить("Дата", '00010101');
	
	Возврат ПараметрыДоговора;
	
КонецФункции

Процедура ПодготовитьВзаиморасчетыДляВводаОстатков(ОстаткиВзаиморасчетов)
	
	ВалютаРегламентированногоУчета = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	ОписаниеТипаЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(15, 2);
	
	ОстаткиВзаиморасчетов.Колонки.Добавить("ЗадолженностьОстаток", ОписаниеТипаЧисло);
	ОстаткиВзаиморасчетов.Колонки.Добавить("АвансОстаток", ОписаниеТипаЧисло);
	ОстаткиВзаиморасчетов.Колонки.Добавить("Валюта", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
	
	Для Каждого СтрокаОстатка Из ОстаткиВзаиморасчетов Цикл
		
		СтрокаОстатка.Валюта = ВалютаРегламентированногоУчета;
		
		Если СтрокаОстатка.Задолженность > СтрокаОстатка.Аванс Тогда
			СтрокаОстатка.Задолженность = СтрокаОстатка.Задолженность - СтрокаОстатка.Аванс;
			СтрокаОстатка.Аванс = 0;
		ИначеЕсли СтрокаОстатка.Аванс > СтрокаОстатка.Задолженность Тогда
			СтрокаОстатка.Аванс = СтрокаОстатка.Аванс - СтрокаОстатка.Задолженность;
			СтрокаОстатка.Задолженность = 0;
		Иначе
			СтрокаОстатка.Задолженность = 0;
			СтрокаОстатка.Аванс = 0;
		КонецЕсли;
		
		СтрокаОстатка.ЗадолженностьОстаток = СтрокаОстатка.Задолженность;
		СтрокаОстатка.АвансОстаток = СтрокаОстатка.Аванс;
