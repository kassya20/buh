...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ФормаПоказатьЖурналОбмена.Видимость = ПравоДоступа("Просмотр", Метаданные.Обработки.ЖурналРегистрации);
...
&НаКлиенте
Процедура ПолучитьФайлОбмена(Команда)
	
	Результат = ПолучитьФайлОбменаНаСервере();
	Если ЗначениеЗаполнено(Результат.ОписаниеОшибки) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрСоединить(Результат.ОписаниеОшибки, ","));
	КонецЕсли;
	
	Если ЭтоАдресВременногоХранилища(Результат.ФайлОбменаВоВременномХранилище) Тогда
		ФайловаяСистемаКлиент.ОткрытьФайл(Результат.ФайлОбменаВоВременномХранилище,, "ФайлОбменаСИнтернетМагазином.txt");
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось получить файл обмена'"));
	КонецЕсли;
...
Функция ПолучитьФайлОбменаНаСервере()
	...
	Возврат ОбменСИнтернетМагазином.ПолучитьФайлОбменаСИнтернетМагазином();
