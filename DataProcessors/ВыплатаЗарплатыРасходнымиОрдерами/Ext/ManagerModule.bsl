...
Процедура Автозаполнение(ПараметрыЗаполнения, АдресХранилища) Экспорт
	...
	РежимПомощника     = Объект.РежимПомощника;
	...
	Если РежимПомощника Тогда
		ТаблицаВыплаченнойЗарплаты = УчетЗарплаты.ПолучитьДанныеВедомостейДляОплатыТаблично(,Ведомости,,Истина);
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВыплаченнойЗарплаты, ТаблицаВыплатыЗарплаты);
	КонецЕсли;
	...
	|			И (РасходныйКассовыйОрдер.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда)))
