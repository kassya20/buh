...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПрочитатьДанныеДляЗагрузки", Истина);
...
Функция НовоеОписаниеЗагружаемыхФайлов()
	...
	ОписаниеЗагружаемыхФайлов.Колонки.Добавить("Представление");
...
Процедура ЗаполнитьОписаниеИменамиФайловДляЗагрузкиИзТекстовыхФайлов(Описание)
	...
	ДобавитьСтрокуВОписаниеФайлов(
		Описание, ИмяИсходящиеДокументы(), "исходящие документы", НСтр("ru = 'Исходящие документы'"));
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяДокументыДвиженияДенег(), "деньги");
	ДобавитьСтрокуВОписаниеФайлов(
		Описание, ИмяВходящиеДокументы(), "входящие документы", НСтр("ru = 'Входящие документы'"));
	
КонецПроцедуры

Процедура ЗаполнитьОписаниеИменамиФайловДляЗагрузкиИзЭлектронныхТаблиц(Описание)
	
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяКонтрагенты(), "названиеконтрагента");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяКонтрагенты(), "паспортныеданные");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяНоменклатура(), "наименованиетовара");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяНоменклатура(), "типноменклатуры");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяКассоваяКнига(), "кассоваякнига");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяДокументы(), "типдокумента");
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяДокументыБезРазбивки(), "документ:");
	ДобавитьСтрокуВОписаниеФайлов(
		Описание, ИмяОборотноСальдоваяВедомость(), ПризнакЗаголовкаОборотноСальдовойВедомостиПоСчету());
	ДобавитьСтрокуВОписаниеФайлов(Описание, ИмяОсновныеСредства(), "типос", НСтр("ru = 'Основные средства'"));
...
Процедура ДобавитьВОписаниеФайлыЭлектронныхТаблиц(Файлы, ОписаниеФайлов)
	...
	Для Каждого СтрокаФайла Из Файлы Цикл
		...
		ГраницаСтрока = Мин(Таблица.ПолучитьРазмерОбластиДанныхПоВертикали(), МаксимальныйНомерСтроки);
		ГраницаКолонка = Мин(Таблица.ПолучитьРазмерОбластиДанныхПоГоризонтали(), МаксимальныйНомерКолонки);
		...
		Для НомерСтроки = 1 По ГраницаСтрока Цикл
			Для НомерКолонки = 1 По ГраницаКолонка Цикл
				...
				Если Не ЗначениеЗаполнено(ТекстЯчейки) Тогда
					Продолжить;
				КонецЕсли;
				ПриведенныйТекст = СтрЗаменить(НРег(ТекстЯчейки), " ", "");
				СтрокаОписания = Неопределено;
				Для Каждого Строка Из ОписаниеФайлов Цикл
					Если СтрЧислоВхождений(ПриведенныйТекст, Строка.Заголовок) > 0 Тогда
...
Функция ПрочитатьДанныеИзТекстовогоФайла(СтрокаОписания, Ошибки, ЗагружаемыеДанные, Организация)
	...
	Иначе
		...
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ПрочитанныеДанные, ЗагружаемыеДанные);
	КонецЕсли;
...
Функция ПрочитатьДанныеИзЭлектроннойТаблицы(СтрокаОписания, Ошибки, ЗагружаемыеДанные)
	...
	Если СтрокаОписания.ИмяФайла = ИмяДокументы() Тогда
		...
		Для Каждого Строка Из ПрочитанныеДанные Цикл
			...
			КонецЕсли;
		КонецЦикла;
		КолонкиДляКопирования = СтрШаблон("Дата,Номер,ТипДокумента,%1", ИмяКолонкиИдентифицирующейКоличествоОбъектов());
		ДанныеПоКоличествуДокументов = ПрочитанныеДанные.Скопировать( , КолонкиДляКопирования);
		ДанныеПоКоличествуДокументов.Свернуть(КолонкиДляКопирования);
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ДанныеПоКоличествуДокументов, ЗагружаемыеДанные);
	ИначеЕсли СтрокаОписания.ИмяФайла = ИмяНоменклатура() Тогда
		КолонкиДляКопирования = СтрШаблон("%1,%2",
			ИмяКолонкиИдентифицирующейНоменклатуру(), ИмяКолонкиИдентифицирующейКоличествоОбъектов());
		ДанныеПоКоличествуНоменклатуры = ПрочитанныеДанные.Скопировать( , КолонкиДляКопирования);
		ДанныеПоКоличествуНоменклатуры.Свернуть(КолонкиДляКопирования);
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ДанныеПоКоличествуНоменклатуры, ЗагружаемыеДанные);
	ИначеЕсли СтрокаОписания.ИмяФайла = ИмяОборотноСальдоваяВедомость() Тогда
		КолонкиДляКопирования = СтрШаблон("ТипДокумента,Счет,%1", ИмяКолонкиИдентифицирующейКоличествоОбъектов());
		ДанныеПоКоличествуОбъектов = ПрочитанныеДанные.Скопировать( , КолонкиДляКопирования);
		ДанныеПоКоличествуОбъектов.Свернуть(КолонкиДляКопирования);
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ДанныеПоКоличествуОбъектов, ЗагружаемыеДанные);
		ЗаполнитьФункциональныеОпцииНачальныхОстатков(ДанныеПоКоличествуОбъектов, ЗагружаемыеДанные);
	Иначе
		ЗаполнитьКоличествоОбъектов(СтрокаОписания, ПрочитанныеДанные, ЗагружаемыеДанные);
	КонецЕсли;
	
	ДополнитьФункциональныеОпцииЗагружаемыхДанных(ЗагружаемыеДанные);
...
Функция ПрочитатьТаблицуДанныхИзТекстовогоФайла(СтрокаОписанияФайла, Ошибки)
	...
	Если СтрокаШапкаТаблицы = 0 Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Неверный формат файла %1'"), СтрокаОписанияФайла.ИмяФайла);
	...
	Иначе
		ЗаполнитьТаблицуДанныхПоТекстовомуФайлу(
			ТаблицаДанных, СоставДанных, СодержимоеФайла, СтрокаШапкаТаблицы, ИмяФайла, СтрокаОписанияФайла.Представление);
	КонецЕсли;
...
Процедура ЗаполнитьКоличествоОбъектов(Описание, ТаблицаДанных, ЗагружаемыеДанные)
	
	ИмяТаблицы = Описание.ИмяФайла;
	...
	Если ТаблицаДанных.Колонки.Найти("ТипДокумента") = Неопределено Тогда
		...
		Если НайденнаяСтрока = Неопределено Тогда
			...
			НовСтрока.ПредставлениеОбъекта = ?(ЗначениеЗаполнено(Описание.Представление), Описание.Представление, ИмяТаблицы);
...
Процедура ЗаполнитьФункциональныеОпцииНачальныхОстатков(УникальныеСчета, ЗагружаемыеДанные)
	
	СтрокаВводНачальныхОстатков = ЗагружаемыеДанные.Найти("ВводНачальныхОстатков", "ИмяОбъекта");
	Если СтрокаВводНачальныхОстатков = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ФункциональныеОпции = Новый Массив;
	Для Каждого СтрокаСчет Из УникальныеСчета Цикл
		Если СтрокаСчет.Счет = "01" Тогда
			ФункциональныеОпции.Добавить("ВедетсяУчетОсновныхСредств");
		ИначеЕсли СтрокаСчет.Счет = "60" Или СтрокаСчет.Счет = "62" Тогда
			ФункциональныеОпции.Добавить("ВестиУчетРасчетовСКонтрагентами");
		КонецЕсли;
	КонецЦикла;
	
	СтрокаВводНачальныхОстатков.ИмяФункциональнойОпции = СтрСоединить(ФункциональныеОпции, ",");
	
КонецПроцедуры

Процедура ДополнитьФункциональныеОпцииЗагружаемыхДанных(ЗагружаемыеДанные)
	
	СтрокаОсновныхСредств = ЗагружаемыеДанные.Найти(ИмяОсновныеСредства(), "ИмяОбъекта");
	Если СтрокаОсновныхСредств <> Неопределено Тогда
		СтрокаОсновныхСредств.ИмяФункциональнойОпции = "ВедетсяУчетОсновныхСредств";
	КонецЕсли;
...
Функция ПрочитатьЭлектроннуюТаблицу(СтрокаОписанияФайла, Ошибки)
	...
	СоставДанных = НоваяТаблицаРеквизитов();
	ИмяФайла = СтрокаОписанияФайла.ИмяФайла;
	ПредставлениеФайла = СтрокаОписанияФайла.Представление;
	...
	КонецЕсли;
	Если ИмяФайла = ИмяОборотноСальдоваяВедомость() Тогда
		Возврат ПрочитатьОборотноСальдовуюВедомость(СоставДанных, Таблица);
	КонецЕсли;
	...
	КонецЕсли;
	Если ИмяФайла = ИмяНоменклатура() Или ИмяФайла = ИмяКонтрагенты() Тогда
		// Поскольку есть различные варианты формата файла с номенклатурой, некоторые колонки дублируются,
		// т.к. ищутся по разным признакам
		УдалитьДублирующиесяКолонки(СоставДанных);
	КонецЕсли;
	ТаблицаДанных = НоваяТаблицаДанных(СоставДанных);
	...
	Иначе
		...
		ЗаполнитьТаблицуДанных(
			...
			ОбязательныеРеквизиты, ПрекращатьЧтениеВСлучаеПустойСтроки, , ПредставлениеФайла);
		Если ИмяФайла = ИмяОсновныеСредства() Тогда
			ОбработатьПрочитанныеДанныеДляОсновныхСредств(ТаблицаДанных);
		КонецЕсли;
	КонецЕсли;
...
Функция НоваяТаблицаРеквизитов()
	...
	ТаблицаРеквизитов.Колонки.Добавить(ИмяРеквизитаВФайле(), 
		Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(1024)));
	ТаблицаРеквизитов.Колонки.Добавить(ТипЗначенияРеквизита(), 
		Новый ОписаниеТипов("Тип"));
	ТаблицаРеквизитов.Колонки.Добавить("Значение");
	Возврат ТаблицаРеквизитов;
	
КонецФункции

Процедура ЗаполнитьТаблицуДанныхПоТекстовомуФайлу(ТаблицаДанных, СоставДанных, Текст, СтрокаШапкиТаблицы, ИмяФайла, Представление = "")
	
	Если Не ЗначениеЗаполнено(Представление) Тогда
		Представление = ИмяФайла;
	КонецЕсли;
	Начало = СтрокаШапкиТаблицы + 1;
	КоличествоСтрок = СтрЧислоСтрок(Текст);
	НеобходимыеРеквизитыТабличнойЧасти = Новый Массив;
	ОбязательныеРеквизитыШапкиОбъекта = Новый Массив;
	Если ИмяФайла = ИмяКонтрагенты() Тогда
		ОбязательныеРеквизитыШапкиОбъекта = РеквизитыКонтрагентовОбязательныеКЗаполнению();
	ИначеЕсли ИмяФайла = ИмяИсходящиеДокументы() Или ИмяФайла = ИмяВходящиеДокументы() Тогда
		ОписаниеТабличнойЧасти = СоставДанных.Найти("ТабличнаяЧасть", ИмяРеквизита1С());
		Если ОписаниеТабличнойЧасти <> Неопределено Тогда
			КолонкиТабличнойЧасти = ОписаниеТабличнойЧасти.Значение;
			ИменаКолонок = ИменаНеобходимыхРеквизитовТабличнойЧасти();
			Для Каждого ИмяКолонки Из ИменаКолонок Цикл
				СтрокаВложенногоРеквизита = КолонкиТабличнойЧасти.Найти(ИмяКолонки, ИмяРеквизита1С());
				Если СтрокаВложенногоРеквизита <> Неопределено И СтрокаВложенногоРеквизита.Значение <> Неопределено Тогда
					НеобходимыеРеквизитыТабличнойЧасти.Добавить(ИмяКолонки);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	ВыводимоеСообщение = СтрШаблон(НСтр("ru = 'Выполняется чтение файла ""%1""'"), Представление);
	Если КоличествоСтрок > РекомендуемоеКоличествоОповещений() Тогда
		ШагОповещения = Окр(КоличествоСтрок / РекомендуемоеКоличествоОповещений(), 0);
	Иначе
		ШагОповещения = 0;
	КонецЕсли;
	НомерСтрокиОповещения = Начало + ШагОповещения;
	Для Сч = Начало По КоличествоСтрок Цикл
		Если ШагОповещения > 0 И Сч >= НомерСтрокиОповещения Тогда
			ДлительныеОперации.СообщитьПрогресс(Окр(Сч / КоличествоСтрок * 100), ВыводимоеСообщение);
			НомерСтрокиОповещения = НомерСтрокиОповещения + ШагОповещения;
		КонецЕсли;
		...
		Для Каждого ОписаниеРеквизита Из СоставДанных Цикл
			...
			Если ТипЗнч(ОписаниеРеквизита.Значение) = Тип("Число") И ОписаниеРеквизита.Значение <= Подстроки.ВГраница() Тогда
				...
				Если ЗначениеЗаполнено(НайденноеЗначение) Тогда
					...
					КонецЕсли;
					ПриведенноеЗначение = ПривестиСтрокуКТипу(НайденноеЗначение, ОписаниеРеквизита[ТипЗначенияРеквизита()]);
					ДанныеОбъекта[ИмяРеквизита] = ПриведенноеЗначение;
				КонецЕсли;
			ИначеЕсли ТипЗнч(ОписаниеРеквизита.Значение) = Тип("ТаблицаЗначений") Тогда
				...
				Если ИмяРеквизита = "ТабличнаяЧасть" Тогда
					ПройденаПроверкаЗаполнения = Ложь;
				Иначе
					ПройденаПроверкаЗаполнения = Истина;
				КонецЕсли;
				Для Каждого ОписаниеВложенногоРеквизита Из ОписаниеРеквизита.Значение Цикл
					...
					Иначе
						Продолжить;
					КонецЕсли;
					ИмяВложенногоРеквизита = ОписаниеВложенногоРеквизита[ИмяРеквизита1С()];
					Если Не ЗначениеЗаполнено(ПрочитанноеЗначение) Тогда
						Продолжить;
					ИначеЕсли Не ПройденаПроверкаЗаполнения
						И НеобходимыеРеквизитыТабличнойЧасти.Найти(ИмяВложенногоРеквизита) <> Неопределено Тогда
						ПройденаПроверкаЗаполнения = Истина;
					КонецЕсли;
					Если СтрокаВложенногоРеквизита = Неопределено Тогда
						СтрокаВложенногоРеквизита = ДанныеОбъекта[ИмяРеквизита].Добавить();
					КонецЕсли;
					ПриведенноеЗначение = ПривестиСтрокуКТипу(ПрочитанноеЗначение,
						ОписаниеВложенногоРеквизита[ТипЗначенияРеквизита()]);
					СтрокаВложенногоРеквизита[ИмяВложенногоРеквизита] = ПриведенноеЗначение;
				КонецЦикла;
				Если Не ПройденаПроверкаЗаполнения И СтрокаВложенногоРеквизита <> Неопределено Тогда
					ДанныеОбъекта[ИмяРеквизита].Удалить(СтрокаВложенногоРеквизита);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Для Каждого Реквизит Из ОбязательныеРеквизитыШапкиОбъекта Цикл
	...
	ИначеЕсли ИмяФайла = ИмяКонтрагенты() Тогда
		ОбработатьДанныеКонтрагентов(ТаблицаДанных);
	КонецЕсли;
...
Функция ПрочитатьЭлектроннуюТаблицуВТабличныйДокумент(Путь)
	...
	Таблица.Прочитать(Путь, СпособЧтенияЗначенийТабличногоДокумента.Значение);
...
Процедура ЗаполнитьСоответствияРеквизитовЭлектронныхТаблиц(ИмяФайла, СоставДанных)
	...
	ИначеЕсли ИмяФайла = ИмяДокументыБезРазбивки() Тогда
		ЗаполнитьСоответствияРеквизитовДокументовБезРазбивки(СоставДанных);
	ИначеЕсли ИмяФайла = ИмяОборотноСальдоваяВедомость() Тогда
		ЗаполнитьСоответствияРеквизитовОборотноСальдоваяВедомость(СоставДанных);
	ИначеЕсли ИмяФайла = ИмяОсновныеСредства() Тогда
		ЗаполнитьСоответствияРеквизитовОсновныеСредства(СоставДанных);
	КонецЕсли;
...
Функция ОпределитьНомераКолонокВЭлектроннойТаблице(
	...
	Если ЗначениеЗаполнено(ИмяОбласти) Тогда
		...
		Если Область = Неопределено Тогда
			Начало = 1;
			Окончание = Таблица.ПолучитьРазмерОбластиДанныхПоВертикали();
		Иначе
	...
	Если Не ЗначениеЗаполнено(ПоследняяКолонка) Тогда
		ПоследняяКолонка = Таблица.ПолучитьРазмерОбластиДанныхПоГоризонтали();
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуДанных(
	...
	ПрекращатьЧтениеВСлучаеПустойСтроки, ЗначенияДляЗаполнения = Неопределено, Представление = "")
	...
	СтрокиКУдалению = Новый Массив;
	НПП = 1;
	ВыводимыйТекст = СтрШаблон(НСтр("ru = 'Выполняется чтение файла ""%1""'"), Представление);
	Если Окончание > РекомендуемоеКоличествоОповещений() Тогда
		ШагОповещения = Окр(Окончание / РекомендуемоеКоличествоОповещений(), 0);
	Иначе
		ШагОповещения = 0;
	КонецЕсли;
	НомерСтрокиОповещения = Начало + ШагОповещения;
	Для НомерСтроки = Начало По Окончание Цикл
		Если ШагОповещения > 0 И НомерСтроки >= НомерСтрокиОповещения Тогда
			ДлительныеОперации.СообщитьПрогресс(Окр(НомерСтроки / Окончание * 100), ВыводимыйТекст);
			НомерСтрокиОповещения = НомерСтрокиОповещения + ШагОповещения;
		КонецЕсли;
		ДанныеОбъекта = Неопределено;
		Для каждого ОписаниеРеквизита Из СоставДанных Цикл
			...
			Ячейка = Таблица.Область(НомерСтроки, ОписаниеРеквизита.Значение);
			Если Ячейка.СодержитЗначение Тогда
				Значение = Ячейка.Значение;
			Иначе
				Значение = ПривестиСтрокуКТипу(Ячейка.Текст, ОписаниеРеквизита[ТипЗначенияРеквизита()]);
			КонецЕсли;
			Если ДанныеОбъекта = Неопределено Тогда
				ДанныеОбъекта = ПрочитанныеДанные.Добавить();
			КонецЕсли;
			ДанныеОбъекта[ОписаниеРеквизита[ИмяРеквизита1С()]] = Значение;
		КонецЦикла;
		Если ДанныеОбъекта = Неопределено Тогда
...
Функция ПривестиСтрокуКТипу(Строка, Тип)
	
	Результат = Строка;
	
	Если Тип = Тип("Число") Тогда
		Результат = ОбработатьЧисловойРеквизит(Строка);
	КонецЕсли;
...
Процедура ЗагрузитьДанныеИзЭлектронныхТаблиц(ДанныеЗагрузки, Результат, АдресРезультата)
	...
	Если ДанныеЗагрузки.Свойство(ИмяОсновныеСредства()) Тогда
		НайтиСоздатьОсновныеСредства(ДанныеЗагрузки, Результат.Ошибки);
		ДлительныеОперации.СообщитьПрогресс( , ИмяОсновныеСредства());
	КонецЕсли;
	...
	Если ДанныеЗагрузки.Свойство(ИмяОборотноСальдоваяВедомость()) Тогда
		ЗагрузитьНачальныеОстатки(ДанныеЗагрузки, Результат.Ошибки);
		ДлительныеОперации.СообщитьПрогресс( , "ВводОстатков");
	КонецЕсли;
	...
	Попытка
		...
		Если Не ЮрЛицо Тогда
			ОрганизацияОбъект.НаименованиеПолное =
				ОбщегоНазначенияБПКлиентСервер.ПолноеНаименованиеИндивидуальногоПредпринимателя(
				РеквизитыОрганизации.Фамилия, РеквизитыОрганизации.Имя, РеквизитыОрганизации.Отчество);
			ОрганизацияОбъект.Наименование =
				ОбщегоНазначенияБПКлиентСервер.НаименованиеИндивидуальногоПредпринимателя(
				РеквизитыОрганизации.Фамилия, РеквизитыОрганизации.Имя, РеквизитыОрганизации.Отчество);
			ОрганизацияОбъект.НаименованиеСокращенное =
				ОбщегоНазначенияБПКлиентСервер.СокращенноеНаименованиеИндивидуальногоПредпринимателя(
				РеквизитыОрганизации.Фамилия, РеквизитыОрганизации.Имя, РеквизитыОрганизации.Отчество);
		Иначе
		...
		Если Не ЗначениеЗаполнено(ОрганизацияОбъект.КодОрганаПФР) Тогда
			...
			ПериодИсторииПФР = НачалоКвартала(
				РегистрыСведений.ИсторияРегистрацийВОрганеПФР.ДатаОтсчетаПериодическихСведений());
			НаборЗаписейИсторияПФР.Отбор.Организация.Установить(Организация);
			НаборЗаписейИсторияПФР.Отбор.Период.Установить(ПериодИсторииПФР);
			НаборЗаписейИсторияПФР.Прочитать();
			Если ЗначениеЗаполнено(НаборЗаписейИсторияПФР) Тогда
				ЗаписьИсторияПФР = НаборЗаписейИсторияПФР[0];
			Иначе
			...
			ЗаписьИсторияПФР.Период = ПериодИсторииПФР;
			НаборЗаписейИсторияПФР.ОбменДанными.Загрузка = Истина;
...
Функция СформироватьТаблицуРеквизитовОрганизации(Текст)
	...
	ПрочитанныеРеквизиты = ОбщегоНазначенияБПКлиентСервер.РазобратьТекстВФорматеЗначенийСРазделителями(Текст, ";");
	ПорядковыйНомерСчета = 1; // Используется для создания нескольких счетов организации
	Для Каждого ЧастиСтроки Из ПрочитанныеРеквизиты Цикл
		...
		Строка = ПрочитатьСтрокуРеквизитовОрганизации(ЧастиСтроки);
		Если ЗначениеЗаполнено(Строка.ИмяОсновногоРеквизита) Тогда
			...
			ИначеЕсли Реквизит[ИмяРеквизита1С()] = ИмяРеквизитаДляРасчетныхСчетов() Тогда
				...
				Реквизит = ДобавитьДанныеНовогоСчета(ТаблицаРеквизитовОрганизации, ПорядковыйНомерСчета);
			КонецЕсли;
...
Функция ПрочитатьСтрокуРеквизитовОрганизации(ЧастиСтроки)
	
	ДанныеСтроки = Новый Структура;
	ДанныеСтроки.Вставить("ИмяОсновногоРеквизита", НРег(СокрЛП(ЧастиСтроки[0])));
	ДанныеСтроки.Вставить("ИмяВложенногоРеквизита", НРег(СокрЛП(ЧастиСтроки[1])));
	ДанныеСтроки.Вставить("Значение", СокрЛП(ЧастиСтроки[2]));
...
Функция СоздатьБанковскиеСчета(Владелец, ДанныеСчетов)
	
	ОсновнойБанковскийСчет = Неопределено;
	
	Для Каждого ПараметрыСчета Из ДанныеСчетов Цикл
		
		Если ТипЗнч(ПараметрыСчета) = Тип("СтрокаТаблицыЗначений") Тогда
			РеквизитыСчета = ПрочитатьРеквизитыРасчетногоСчетаИзТаблицы(ПараметрыСчета);
		ИначеЕсли Не ЗначениеЗаполнено(ПараметрыСчета) Тогда
			Продолжить;
		Иначе
...
Функция СформироватьДанныеРасчетныхСчетовИзСтроки(ПрочитанныеДанные)
	...
	Для каждого ДанныеРасчетногоСчета Из ДанныеРасчетныхСчетов Цикл
		ДанныеРасчетногоСчета = СтрЗаменить(ДанныеРасчетногоСчета, ":", "");
...
Функция ДобавитьДанныеНовогоСчета(ТаблицаРеквизитов, ПорядковыйНомер)
	
	Реквизит = ТаблицаРеквизитов.Добавить();
	Реквизит[ИмяРеквизита1С()] = СтрШаблон("%1%2", ИмяРеквизитаДляРасчетныхСчетов(), Формат(ПорядковыйНомер, "ЧГ="));
...
Процедура НайтиСоздатьОсновныеСредства(ЗагружаемыеДанные, Ошибки)
	
	ДанныеОС = ЗагружаемыеДанные[ИмяОсновныеСредства()];
	...
	Для Каждого ПрочитанныеДанные Из ДанныеОС Цикл
		СтрокаПоиска = ДанныеПоиска.Добавить();
		СтрокаПоиска.Наименование = ПрочитанныеДанные.Наименование;
		СтрокаПоиска.ИнвентарныйНомер = ПрочитанныеДанные.ИнвентарныйНомер;
	КонецЦикла;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДанныеПоиска.Наименование КАК Наименование,
		|	ДанныеПоиска.ИнвентарныйНомер КАК ИнвентарныйНомер
		|ПОМЕСТИТЬ ВТ_ПрочитанныеДанные
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ПрочитанныеДанные.Наименование КАК Наименование,
		|	ВТ_ПрочитанныеДанные.ИнвентарныйНомер КАК ИнвентарныйНомер,
		|	МАКСИМУМ(ЕСТЬNULL(ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство, ОсновныеСредства.Ссылка)) КАК Ссылка
		|ИЗ
		|	ВТ_ПрочитанныеДанные КАК ВТ_ПрочитанныеДанные
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПервоначальныеСведенияОСБухгалтерскийУчет КАК ПервоначальныеСведенияОСБухгалтерскийУчет
		|		ПО ВТ_ПрочитанныеДанные.Наименование = ПервоначальныеСведенияОСБухгалтерскийУчет.ОсновноеСредство.Наименование
		|			И ВТ_ПрочитанныеДанные.ИнвентарныйНомер = ПервоначальныеСведенияОСБухгалтерскийУчет.ИнвентарныйНомер
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ОсновныеСредства КАК ОсновныеСредства
		|		ПО ВТ_ПрочитанныеДанные.Наименование = ОсновныеСредства.Наименование
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ПрочитанныеДанные.Наименование,
		|	ВТ_ПрочитанныеДанные.ИнвентарныйНомер";
	
	Запрос.УстановитьПараметр("ДанныеПоиска", ДанныеПоиска);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Фильтр = Новый Структура("Наименование, ИнвентарныйНомер");
		ЗаполнитьЗначенияСвойств(Фильтр, Выборка);
		НайденныеСтроки = ДанныеОС.НайтиСтроки(Фильтр);
		Если Не ЗначениеЗаполнено(НайденныеСтроки) Тогда
			Продолжить;
		КонецЕсли;
		СтрокаОС = НайденныеСтроки[0];
		Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
			СтрокаОС.Ссылка = Выборка.Ссылка;
		Иначе
			НовыйОС = Справочники.ОсновныеСредства.СоздатьЭлемент();
			НовыйОС.Наименование = СтрокаОС.Наименование;
			НовыйОС.НаименованиеПолное = СтрокаОС.Наименование;
			Если СтрокаОС.ТипОС = "Земельный участок" Тогда
				НовыйОС.ГруппаОС = Перечисления.ГруппыОС.ЗемельныеУчастки;
			ИначеЕсли СтрокаОС.ТипОС = "Транспортное средство" Тогда
				НовыйОС.ГруппаОС = Перечисления.ГруппыОС.ТранспортныеСредства;
			Иначе
				НовыйОС.ГруппаОС = Перечисления.ГруппыОС.МашиныИОборудование;
			КонецЕсли;
			НовыйОС.ТипОС = Перечисления.ТипыОС.ОбъектОС;
			НовыйОС.Записать();
			СтрокаОС.Ссылка = НовыйОС.Ссылка;
		КонецЕсли;
	КонецЦикла;
...
Процедура НайтиСоздатьКонтрагентов(ДанныеКонтрагентов, Ошибки)
	...
	Для Каждого ПрочитанныеДанные Из ДанныеКонтрагентов Цикл
		...
		ИНН = ПрочитанныеДанные.ИНН;
		РеквизитыКонтрагента = Неопределено;
		Если ЗначениеЗаполнено(ИНН) Тогда
			ДанныеСервиса = КонтрагентыФормы.ДанныеЕдиныхГосРеестровПоИНН(
				ИНН, ИдентификационныеНомераНалогоплательщиков.ЭтоИННЮридическогоЛица(ИНН));
			Если Не ЗначениеЗаполнено(ДанныеСервиса.ОписаниеОшибки)
				И ДанныеСервиса.Свойство("АдресРеквизитовКонтрагента") Тогда
				РеквизитыКонтрагента = ПолучитьИзВременногоХранилища(ДанныеСервиса.АдресРеквизитовКонтрагента);
			КонецЕсли;
		КонецЕсли;
		
		ДанныеЗаполнения = ДанныеЗаполненияКонтрагента(ПрочитанныеДанные, РеквизитыКонтрагента);
		
		РеквизитыСчета = ПрочитанныеДанные[ИмяРеквизитаДляРасчетныхСчетов()];
		Если ЗначениеЗаполнено(РеквизитыСчета) И ТипЗнч(РеквизитыСчета) = Тип("Строка") Тогда
				ДанныеРасчетныхСчетов = СформироватьДанныеРасчетныхСчетовИзСтроки(РеквизитыСчета);
		ИначеЕсли ЗначениеЗаполнено(РеквизитыСчета) Тогда
			ДанныеРасчетныхСчетов = РеквизитыСчета;
		Иначе
...
Функция ДанныеЗаполненияКонтрагента(СтрокаДанных, ДанныеИзСервиса)
	...
	ЭтоФизическоеЛицо = ИдентификационныеНомераНалогоплательщиков.ЭтоИННФизическогоЛица(ДанныеЗаполнения.ИНН);
	Если ЭтоФизическоеЛицо Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДанных.ОГРН) Тогда
			ДанныеЗаполнения.Вставить("ИндивидуальныйПредприниматель", Истина);
		КонецЕсли;
	Иначе
	...
	Если ДанныеИзСервиса <> Неопределено Тогда
		Если ЭтоФизическоеЛицо Тогда
			ДанныеЗаполнения.Вставить("ИндивидуальныйПредприниматель", Истина);
		КонецЕсли;
		Если ДанныеИзСервиса.Свойство("КодСтраныГражданства") Тогда
			ДанныеЗаполнения.Вставить("СтранаРегистрации", ДанныеИзСервиса.КодСтраныГражданства);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаДанных.ЮридическийАдрес) И ДанныеИзСервиса.Свойство("ЮридическийАдрес")
			И ТипЗнч(ДанныеИзСервиса.ЮридическийАдрес) = Тип("Структура") Тогда
			ДанныеЗаполнения.Вставить("ЮридическийАдресЗначениеJSON", ДанныеИзСервиса.ЮридическийАдрес.КонтактнаяИнформация);
		КонецЕсли;
		Если ДанныеИзСервиса.Свойство("Руководитель") И ЗначениеЗаполнено(ДанныеИзСервиса.Руководитель) Тогда
			ДанныеЗаполнения.Вставить("Руководитель", ДанныеИзСервиса.Руководитель.Представление);
			ДанныеЗаполнения.Вставить("ДолжностьРуководителя", ДанныеИзСервиса.Руководитель.Должность);
		КонецЕсли;
		Если ЭтоФизическоеЛицо И ДанныеИзСервиса.Свойство("СвидетельствоОРегистрации") Тогда
			СвидетельствоОРегистрации = ДанныеИзСервиса.СвидетельствоОРегистрации;
			Если СвидетельствоОРегистрации <> Неопределено Тогда
				ДанныеЗаполнения.Вставить("СвидетельствоСерияНомер", СтрШаблон(
					НСтр("ru = '%1 №%2'"), СвидетельствоОРегистрации.Серия, СвидетельствоОРегистрации.Номер));
				ДанныеЗаполнения.Вставить("СвидетельствоДатаВыдачи", СвидетельствоОРегистрации.Дата);
			КонецЕсли;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(СтрокаДанных.ОГРН) И ДанныеИзСервиса.Свойство("РегистрационныйНомер") Тогда
			ДанныеЗаполнения.Вставить("РегистрационныйНомер", ДанныеИзСервиса.РегистрационныйНомер);
		КонецЕсли;
	КонецЕсли;
...
Функция СоставРеквизитовКонтрагентов()
	...
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(Реквизиты, "Наименование", , "название");
	...
	ДобавитьСтрокуРеквизитаДляНесколькихФорматов(
		Реквизиты, ИмяРеквизитаДляРасчетныхСчетов(), "расчетный счет", "банковские реквизиты");
...
Функция НовыеУниверсальныеРеквизитыКонтрагентов()
	...
	Реквизиты.Колонки.Добавить(
		ТипЗначенияРеквизита(), Новый ОписаниеТипов("Тип"));
	Возврат Реквизиты;
...
Процедура ЗагрузитьНачальныеОстатки(ЗагружаемыеДанные, Ошибки)
	
	ТаблицаДанных = ЗагружаемыеДанные[ИмяОборотноСальдоваяВедомость()];
	Если Не ЗначениеЗаполнено(ТаблицаДанных) Тогда
		Возврат;
	КонецЕсли;
	
	УникальныеСчета = ТаблицаДанных.Скопировать( , "Счет");
	УникальныеСчета.Свернуть("Счет");
	
	СчетаНоменклатуры = СчетаНоменклатуры();
	СчетаРасчетовСКонтрагентами = СчетаРасчетовСКонтрагентами();
	
	Для Каждого Строка Из УникальныеСчета Цикл
		ФильтрПоиска = Новый Структура;
		ФильтрПоиска.Вставить("Счет", Строка.Счет);
		НайденныеСтроки = ТаблицаДанных.НайтиСтроки(ФильтрПоиска);
		Если СчетаНоменклатуры.Найти(Строка.Счет) <> Неопределено Тогда
			ЗагрузитьОстаткиПоНоменклатуре(НайденныеСтроки, ЗагружаемыеДанные, Ошибки);
		ИначеЕсли Строка.Счет = "01" Тогда
			ЗагрузитьОстаткиПоОсновнымСредствам(НайденныеСтроки, ЗагружаемыеДанные, Ошибки);
		ИначеЕсли Строка.Счет = "51" Тогда
			ЗагрузитьОстаткиПоРасчетнымСчетам(НайденныеСтроки, ЗагружаемыеДанные, Ошибки);
		ИначеЕсли СчетаРасчетовСКонтрагентами.Найти(Строка.Счет) <> Неопределено Тогда
			ЗагрузитьОстаткиПоРасчетамСКонтрагентами(НайденныеСтроки, ЗагружаемыеДанные, Ошибки);
		КонецЕсли;
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗагрузитьОстаткиПоНоменклатуре(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	
	НоменклатураДокумента = НовыйНоменклатураДокумента();
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		СтрокаНоменклатуры = НоменклатураДокумента.Добавить();
		СтрокаНоменклатуры[ИмяКолонкиИдентифицирующейНоменклатуру()] = Строка.Аналитика;
	КонецЦикла;
	НоменклатураДокумента.ЗаполнитьЗначения("ВводНачальныхОстатков", "ТипДокумента");
	ЗагружаемыеДанные.Вставить("НоменклатураДокумента", НоменклатураДокумента);
	ЗагрузитьНоменклатуру("НоменклатураДокумента", ЗагружаемыеДанные, Ошибки);
	
	Организация = ЗагружаемыеДанные.Организация;
	Дата = ПрочитанныеДанные[0].Дата;
	Счет = СтрШаблон("Счет%1", ПрочитанныеДанные[0].Счет);
	
	ПроверитьУстановитьЗаписьУчетнойПолитики(Организация, Дата);
	
	Документ = Документы.ВводНачальныхОстатков.СоздатьДокумент();
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", Организация);
	ДанныеЗаполнения.Вставить("ПодразделениеОрганизации", ОсновноеПодразделениеОрганизации(Документ.Организация));
	Документ.Заполнить(ДанныеЗаполнения);
	Документ.Дата = Дата;
	СоответствияСчетовИРазделовУчета = СоответствияСчетовНоменклатурыРазделамУчета();
	РазделУчета = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		СоответствияСчетовИРазделовУчета, Счет, Перечисления.РазделыУчетаДляВводаОстатков.Товары);
	Документ.РазделУчета = РазделУчета;
	ПрефиксНомера = ПрефиксацияОбъектовБПСобытия.ПрефиксИнформационнойБазыОрганизацииПодразделения(
		Организация, ДанныеЗаполнения.ПодразделениеОрганизации, "");
	Документ.УстановитьНовыйНомер(ПрефиксНомера);
	
	ТаблицаНоменклатуры = ЗагружаемыеДанные[ИмяНоменклатура()];
	КолонкаНоменклатура = ИмяКолонкиИдентифицирующейНоменклатуру();
	ДокументПартии = Неопределено;
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		НайденнаяНоменклатура = ТаблицаНоменклатуры.Найти(Строка.Аналитика, КолонкаНоменклатура);
		Если НайденнаяНоменклатура <> Неопределено Тогда
			НоваяСтрока = Документ.НоменклатураНаСкладе.Добавить();
			...
			Иначе
				СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(
					Организация, НоваяСтрока.Номенклатура);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СчетаУчета);
			КонецЕсли;
			Если НужнаПартия Тогда
				Если ДокументПартии = Неопределено Тогда
					НоваяПартия = Документы.ДокументРасчетовСКонтрагентом.СоздатьДокумент();
					НоваяПартия.Организация = Организация;
					НоваяПартия.Дата = Дата;
					НоваяПартия.Записать();
					ДокументПартии = НоваяПартия.Ссылка;
				КонецЕсли;
				НоваяСтрока.Партия = ДокументПартии;
			КонецЕсли;
			НоваяСтрока.Сумма = Строка.ДебетСумма - Строка.КредитСумма;
			НоваяСтрока.ВалютнаяСумма = НоваяСтрока.Сумма;
			НоваяСтрока.СуммаНУ = НоваяСтрока.Сумма;
			НоваяСтрока.Количество = Строка.ДебетКоличество - Строка.КредитКоличество;
		КонецЕсли;
	КонецЦикла;
	...
	СтрокаДокументКПроведению = ЗагружаемыеДанные.ДокументыКПроведению.Добавить();
	СтрокаДокументКПроведению.Дата = Документ.Дата;
	СтрокаДокументКПроведению.Ссылка = Документ.Ссылка;
	
КонецПроцедуры

Процедура ЗагрузитьОстаткиПоРасчетамСКонтрагентами(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	
	Организация = ЗагружаемыеДанные.Организация;
	Дата = ПрочитанныеДанные[0].Дата;
	Счет = ПрочитанныеДанные[0].Счет;
	...
	Документ.Дата = Дата;
	Если Счет = "60" Тогда
		Документ.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСПоставщикамиИПодрядчиками;
		СчетУчетаДебет = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным;
		СчетУчетаКредит = ПланыСчетов.Хозрасчетный.РасчетыСПоставщиками;
	ИначеЕсли Счет = "62" Тогда
		Документ.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСПокупателямиИЗаказчиками;
		СчетУчетаДебет = ПланыСчетов.Хозрасчетный.РасчетыСПокупателями;
		СчетУчетаКредит = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным;
	КонецЕсли;
	ПрефиксНомера = ПрефиксацияОбъектовБПСобытия.ПрефиксИнформационнойБазыОрганизацииПодразделения(
		Организация, ДанныеЗаполнения.ПодразделениеОрганизации, "");
	Документ.УстановитьНовыйНомер(ПрефиксНомера);
	
	ИменаСуммовыхРеквизитов = Новый Структура;
	ИменаСуммовыхРеквизитов.Вставить("ДебетСумма", СчетУчетаДебет);
	ИменаСуммовыхРеквизитов.Вставить("КредитСумма", СчетУчетаКредит);
	
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		Для Каждого Элемент Из ИменаСуммовыхРеквизитов Цикл
			Если Не ЗначениеЗаполнено(Строка[Элемент.Ключ]) Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрока = Документ.РасчетыСКонтрагентами.Добавить();
			НоваяСтрока.Контрагент = КонтрагентПоНаименованию(ЗагружаемыеДанные[ИмяКонтрагенты()], Строка.Аналитика, Ошибки);
			НоваяСтрока.СчетУчета = Элемент.Значение;
			НоваяСтрока.Сумма = Строка[Элемент.Ключ];
			...
			ДокументРасчета.Контрагент = НоваяСтрока.Контрагент;
			ДокументРасчета.Дата = Дата;
			ДокументРасчета.СуммаДокумента = НоваяСтрока.Сумма;
			ДокументРасчета.Записать();
			НоваяСтрока.Документ = ДокументРасчета.Ссылка;
		КонецЦикла;
	КонецЦикла;
...
Процедура ЗагрузитьОстаткиПоРасчетнымСчетам(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	
	Если Не ЗначениеЗаполнено(ПрочитанныеДанные) Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", Организация);
	ДанныеЗаполнения.Вставить("ПодразделениеОрганизации", ОсновноеПодразделениеОрганизации(Организация));
	НовыйДокумент.Заполнить(ДанныеЗаполнения);
	НовыйДокумент.Дата = ПрочитанныеДанные[0].Дата;
	НовыйДокумент.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ДенежныеСредства;
	НовыйДокумент.УстановитьНовыйНомер();
	
	ПроверитьУстановитьЗаписьУчетнойПолитики(Организация, НовыйДокумент.Дата);
	
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		РасчетныйСчет = Справочники.БанковскиеСчета.СоздатьЭлемент();
		РасчетныйСчет.Владелец = Организация;
		РасчетныйСчет.Наименование = Строка.Аналитика;
		РасчетныйСчет.ОбменДанными.Загрузка = Истина;
		РасчетныйСчет.Записать();
		
		СтрокаОстатки = НовыйДокумент.БухСправка.Добавить();
		СтрокаОстатки.СчетУчета = ПланыСчетов.Хозрасчетный.РасчетныеСчета;
		СтрокаОстатки.Субконто1 = РасчетныйСчет.Ссылка;
		СтрокаОстатки.Сумма = Строка.ДебетСумма - Строка.КредитСумма;
		СтрокаОстатки.Валюта = ВалютаРегламентированногоУчета;
		СтрокаОстатки.ВалютнаяСумма = СтрокаОстатки.Сумма;
	КонецЦикла;
	
	НовыйДокумент.ОбменДанными.Загрузка = Истина;
	НовыйДокумент.Записать(РежимЗаписиДокумента.Запись);
	
	СтрокаКПроведению = ЗагружаемыеДанные.ДокументыКПроведению.Добавить();
	СтрокаКПроведению.Дата = НовыйДокумент.Дата;
	СтрокаКПроведению.Ссылка = НовыйДокумент.Ссылка;
...
Функция ПрочитатьОборотноСальдовуюВедомость(СоставДанных, Таблица)
	
	ТаблицаДанных = НоваяТаблицаДанных(СоставДанных);
	РеквизитыШапкиВедомости = РеквизитыШапкиОборотноСальдовойВедомости(Таблица);
	СтрокаШапкиТаблицы =
		ОпределитьНомераКолонокВЭлектроннойТаблице(
		СоставДанных, Таблица, "", РеквизитыШапкиВедомости.НомерСтроки + 1);
	ТекстКонечныхОстатков = "конечноесальдо";
	МаксКолонка = Таблица.ПолучитьРазмерОбластиДанныхПоГоризонтали();
	Для НомерКолонки = 1 По МаксКолонка Цикл
		ТекстЯчейки = Таблица.Область(СтрокаШапкиТаблицы, НомерКолонки).Текст;
		Если Не ЗначениеЗаполнено(ТекстЯчейки) Тогда
			Продолжить;
		КонецЕсли;
		ТекстЯчейкиФорматированный = СтрЗаменить(ТекстЯчейки, " ", "");
		ТекстЯчейкиФорматированный = НРег(ТекстЯчейкиФорматированный);
		Если СтрЧислоВхождений(ТекстЯчейкиФорматированный, ТекстКонечныхОстатков) > 0 Тогда
			Для Каждого Строка Из СоставДанных Цикл
				Если Строка.ИмяРеквизита1С = "ДебетСумма" Тогда
					Строка.Значение = НомерКолонки;
				ИначеЕсли Строка.ИмяРеквизита1С = "ДебетКоличество" Тогда
					Строка.Значение = НомерКолонки + 1;
				ИначеЕсли Строка.ИмяРеквизита1С = "КредитСумма" Тогда
					Строка.Значение = НомерКолонки + 2;
				ИначеЕсли Строка.ИмяРеквизита1С = "КредитКоличество" Тогда
					Строка.Значение = НомерКолонки + 3;
				КонецЕсли;
			КонецЦикла;
			Прервать;
		КонецЕсли;
	...
	ЗначенияЗаполнения.Вставить("ТипДокумента", "ВводНачальныхОстатков");
	ЗначенияЗаполнения.Вставить("Дата", РеквизитыШапкиВедомости.Дата);
	ЗначенияЗаполнения.Вставить("Счет", РеквизитыШапкиВедомости.Счет);
	ПредставлениеФайла = СтрШаблон(НСтр("ru = 'Оборотно-сальдовая ведомость по счету %1'"), РеквизитыШапкиВедомости.Счет);
	ЗаполнитьТаблицуДанных(
		ТаблицаДанных, СоставДанных, Таблица, СтрокаШапкиТаблицы + 2, "", Неопределено, Ложь,
		ЗначенияЗаполнения, ПредставлениеФайла);
	// Удаляем подвал с итогами
	ТаблицаДанных.Удалить(ТаблицаДанных.Количество() - 1);
	
	СтрокиКУдалению = Новый Массив;
	Для Каждого Строка Из ТаблицаДанных Цикл
		Если НРег(Строка.Аналитика) = "аналитика не указана" Тогда
...
Процедура ПрочитатьЛистКассовойКнигиВОдностраничномФормате(
	...
	ПредставлениеФайла = НСтр("ru = 'Кассовая книга'");
	
	// Читаем саму таблицу
	ДанныеДляЗаполнения = Новый Структура("Дата", ДатаДокумента);
	ЗаполнитьТаблицуДанных(
		ТаблицаДанных, СоставДанных, Таблица, СтрокаНачалаТаблицы,
		Область.Имя, ОбязательныеРеквизиты, Истина, ДанныеДляЗаполнения, ПредставлениеФайла);
	
	СтрокиКУдалению = Новый Массив;
	Для Каждого КассовыйДокумент Из ТаблицаДанных Цикл
		Если Не ЗначениеЗаполнено(КассовыйДокумент.СуммаПоступления)
			И Не ЗначениеЗаполнено(КассовыйДокумент.СуммаСписания) Тогда
			СтрокиКУдалению.Добавить(КассовыйДокумент);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПрочитатьЛистКассовойКнигиВДвухстраничномФормате(
	...
	Для Каждого ПрочитаннаяСтрока Из ТаблицаДанных Цикл
		Если Не ЗначениеЗаполнено(ПрочитаннаяСтрока.СуммаПоступления)
			И Не ЗначениеЗаполнено(ПрочитаннаяСтрока.СуммаСписания) Тогда
			СтрокиКУдалению.Добавить(ПрочитаннаяСтрока);
			Если ЗначениеЗаполнено(ПрочитаннаяСтрока.Контрагент) И ПредыдущаяСтрока <> Неопределено Тогда
				ПредыдущаяСтрока.Контрагент = ПредыдущаяСтрока.Контрагент + " " + ПрочитаннаяСтрока.Контрагент;
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ПрочитаннаяСтрока.Номер) Тогда
...
Процедура ОбработатьДанныеКонтрагентов(Данные)
	
	Если Данные.Колонки.Найти(ИмяРеквизитаДляРасчетныхСчетов()) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИменаРасчетныхСчетов = Новый Массив;
	Для Каждого Колонка Из Данные.Колонки Цикл
		Если СтрНачинаетсяС(Колонка.Имя, ИмяРеквизитаДляРасчетныхСчетов()) Тогда
			ИменаРасчетныхСчетов.Добавить(Колонка.Имя);
		КонецЕсли;
	КонецЦикла;
	
	Данные.Колонки.Добавить(ИмяРеквизитаДляРасчетныхСчетов());
	Для Каждого РеквизитыКонтрагента Из Данные Цикл
		Для Каждого ИмяСчета Из ИменаРасчетныхСчетов Цикл
			РеквизитыСчета = РеквизитыКонтрагента[ИмяСчета];
			Если ЗначениеЗаполнено(РеквизитыСчета) Тогда
				Если Не ЗначениеЗаполнено(РеквизитыКонтрагента[ИмяРеквизитаДляРасчетныхСчетов()]) Тогда
					РеквизитыКонтрагента[ИмяРеквизитаДляРасчетныхСчетов()] = РеквизитыСчета;
				Иначе
					Для Каждого Строка Из РеквизитыСчета Цикл
						ЗаполнитьЗначенияСвойств(
							РеквизитыКонтрагента[ИмяРеквизитаДляРасчетныхСчетов()].Добавить(), Строка);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбработатьПрочитанныеДанныеДляОсновныхСредств(Данные)
	
	Для Каждого ОсновноеСредство Из Данные Цикл
		Если ТипЗнч(ОсновноеСредство.ИнвентарныйНомер) = Тип("Число") Тогда
			ОсновноеСредство.ИнвентарныйНомер = Строка(ОсновноеСредство.ИнвентарныйНомер);
		КонецЕсли;
		ОсновноеСредство.НаименованиеВВедомости = СтрШаблон(
			"%1%2",
			?(ЗначениеЗаполнено(ОсновноеСредство.ИнвентарныйНомер), ОсновноеСредство.ИнвентарныйНомер + " ", ""),
			...
			Если НовыйДокумент.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратЗайма Тогда
				Если ПрочитанныеДанные.ВидОперации = "Выплата процентов по кредиту" Тогда
					...
					СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств =
						УчетДенежныхСредствВызовСервера.СтатьяДДСПоУмолчанию("УплатаПроцентов", НовыйДокумент.Дата);
				Иначе
					СтрокаРасшифровкаПлатежа.ВидПлатежаПоКредитамЗаймам = Перечисления.ВидыПлатежейПоКредитамЗаймам.ПогашениеДолга;
					СтрокаРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств =
						УчетДенежныхСредствВызовСервера.СтатьяДДСПоУмолчанию("ПогашениеДолга", НовыйДокумент.Дата);
				КонецЕсли;
...
Функция НовыйПараметрыДокумента()
	...
	ПараметрыДокумента.Вставить("ЕстьНомерВходящегоДокумента");
	ПараметрыДокумента.Вставить("ЕстьДатаВходящегоДокумента");
...
Процедура ЗагрузитьВходящиеИсходящиеДокументы(ТаблицаДанных, ЗагружаемыеДанные, Ошибки, ПараметрыДокумента)
	...
	Для Каждого Строка Из НайденныеСтроки Цикл
		...
		РеквизитыДокумента.Организация = Организация;
		РеквизитыДокумента.СуммаДокумента = Строка.Сумма;
		...
		Если ПараметрыДокумента.ЕстьНомерВходящегоДокумента Тогда
			НовыйДокумент.НомерВходящегоДокумента = РеквизитыДокумента.Номер;
		КонецЕсли;
		Если ПараметрыДокумента.ЕстьДатаВходящегоДокумента Тогда
		...
		Если ПараметрыДокумента.ЭтоСчетФактура Тогда
			ЗаполнитьРеквизитыСФ(НовыйДокумент, Строка);
		КонецЕсли;
		
		Если Не ПустаяСтрока(ПараметрыДокумента.ИмяТабЧасти) Тогда
			ЗаполнитьТабличнуюЧасть(
				НовыйДокумент, ПараметрыДокумента, Строка.ТабличнаяЧасть, ЗагружаемыеДанные.Номенклатура);
		КонецЕсли;
...
Процедура ЗагрузитьДокументыПокупкиПродажиИзЭлектроннойТаблицы(
	...
	Для Каждого СтрокаШапки Из ШапкаДокумента Цикл
		...
		Если Не ПустаяСтрока(ПараметрыДокумента.ИмяТабЧасти) Тогда
			ЗаполнитьТабличнуюЧасть(
				НовыйДокумент, ПараметрыДокумента, ТабличнаяЧастьДокумента, ЗагружаемыеДанные[ИмяНоменклатура()]);
		КонецЕсли;
	...
	Для НомерСтроки = Область.Верх По Область.Низ Цикл
		Для НомерКолонки = 1 По МаксКолонка Цикл
			...
			Если СокрЛП(ТекстЯчейки) = ШапкаОбластиКассовойКниги() Тогда
				Для Сч = НомерКолонки + 1 По МаксКолонка Цикл
...
Функция ВидыОперацийДокументов()
	...
	ВидыОпераций.Вставить("входящие_документы_упд", Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары);
	...
	ВидыОпераций.Вставить("исходящие_документы_упд", Перечисления.ВидыОперацийРеализацияТоваров.Товары);
	ВидыОпераций.Вставить("исходящие_документы_счет", Перечисления.ВидыОперацийСчетаПокупателю.ТоварыИУслуги);
	ВидыОпераций.Вставить("исходящие_документы_счёт", Перечисления.ВидыОперацийСчетаПокупателю.ТоварыИУслуги);
	ВидыОпераций.Вставить(
...
Процедура ДополнитьПараметрыВходящихИсходящихДокументов(ПараметрыДокумента)
	...
	ПараметрыДокумента.ЕстьНомерВходящегоДокумента = ОбщегоНазначения.ЕстьРеквизитОбъекта(
		"НомерВходящегоДокумента", МетаданныеДокумента);
	ПараметрыДокумента.ЕстьДатаВходящегоДокумента = ОбщегоНазначения.ЕстьРеквизитОбъекта(
		"ДатаВходящегоДокумента", МетаданныеДокумента);
...
Процедура ЗаполнитьРеквизитыСФ(СчетФактура, ДанныеСФ)
...
Процедура ЗаполнитьТабличнуюЧасть(
	НовыйДокумент, ПараметрыДокумента, ДанныеТабличнойЧасти, ТаблицаНоменклатуры)
	...
	Для Каждого Строка Из ДанныеТабличнойЧасти Цикл
		...
		Если ПараметрыДокумента.ЕстьСтавкаНДС Тогда
			ПрочитаннаяСтавкаНДС = ОбработатьСтроковоеЗначениеСтавкиНДС(Строка.СтавкаНДС);
			НоваяСтрока.СтавкаНДС = УчетНДС.СтавкаНДСПоПредставлению(ПрочитаннаяСтавкаНДС);
...
Функция НовыйРезультатЧтенияШапкиОборотноСальдовойВедомости()
...
Процедура ЗаполнитьСоответствияРеквизитовВходящихИсходящихДокументов(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Сумма", "сумма документа", Тип("Число"));
...
Процедура ЗаполнитьСоответствияРеквизитовДокументовПокупкиПродажи(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "Количество", "кол-во", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ФактическоеКоличество", "фактическое кол-во", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "Цена", "цена без ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаБезНДС", "сумма без ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СтавкаНДС", "ндс");
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаНДС", "сумма ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаСНДС", "сумма с ндс", Тип("Число"));
	ДобавитьСоответствие(
		ТаблицаРеквизитов, ИмяКолонкиИдентифицирующейРазделПокупки(), "наличие несоответствия по количеству / качеству");
	
КонецПроцедуры

Процедура ЗаполнитьСоответствияРеквизитовДокументовБезРазбивки(ТаблицаРеквизитов)
	
	ДобавитьСоответствие(ТаблицаРеквизитов, "Дата", "дата");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Номер", "номер");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипДокумента", "документ");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Контрагент", "контрагент");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Состояние", "операция");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Сумма", "всего, ₽", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "Комментарий", "комментарий");
...
Процедура ЗаполнитьСоответствияРеквизитовДенежныхДокументов(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаПоступления", "поступило", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаСписания", "списано", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаУслуг", "комиссия", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДоходыУСН", "учитывается в налоге усн (доход)", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "РасходыУСН", "учитывается в налоге усн (расход)", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаПатент", "учитывается в патенте", Тип("Число"));
...
Процедура ЗаполнитьСоответствияРеквизитовОборотноСальдоваяВедомость(ТаблицаРеквизитов)
	
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "товар");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "продукция");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "материал");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "основное средство");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "расчетный счет");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "поставщик, договор");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "клиент, договор");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "поставщик");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Аналитика", "клиент");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДебетСумма", "сумма", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ДебетКоличество", "количество", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "КредитСумма", "сумма", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "КредитКоличество", "количество", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипДокумента");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Дата");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Счет");
	
КонецПроцедуры

Процедура ЗаполнитьСоответствияРеквизитовОсновныеСредства(ТаблицаРеквизитов)
	
	ДобавитьСоответствие(ТаблицаРеквизитов, "Наименование", "название");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ИнвентарныйНомер", "инвентарный номер");
	ДобавитьСоответствие(ТаблицаРеквизитов, "СчетЗатрат", "счет затрат");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипОС", "тип ос");
	ДобавитьСоответствие(ТаблицаРеквизитов, "СрокЭксплуатации", "срок службы, мес");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Стоимость", "первоначальная стоимость, руб", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "Ссылка");
	ДобавитьСоответствие(ТаблицаРеквизитов, "НаименованиеВВедомости");
...
Функция СоответствияТабличнойЧастиДокументов()
	...
	ТаблицаРеквизитов = НоваяТаблицаРеквизитов();
	ДобавитьСоответствие(ТаблицаРеквизитов, ИмяКолонкиИдентифицирующейНоменклатуру(), "наименование");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Количество",               "количество", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "ЕдиницаИзмерения",         "ед. измерения");
	ДобавитьСоответствие(ТаблицаРеквизитов, "Цена",                     "цена за ед. товара", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаСНДС",                "сумма с ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СуммаНДС",                 "ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "СтавкаНДС",                "ставка ндс");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ПроцентСкидки",            "процент скидки", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "Сумма",                    "сумма", Тип("Число"));
...
Функция НоваяТаблицаНоменклатуры()
	...
	Для Каждого Строка Из ОписаниеРеквизитов Цикл
		Если Номенклатура.Колонки.Найти(Строка[ИмяРеквизита1С()]) = Неопределено Тогда
...
Процедура ЗаполнитьСоответствияРеквизитовНоменклатуры(ТаблицаРеквизитов)
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, ИмяКолонкиИдентифицирующейНоменклатуру(), "название");
	...
	ДобавитьСоответствие(ТаблицаРеквизитов, "ЦенаПродажи",              "цена продажи", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "НДС",                      "ндс", Тип("Число"));
	ДобавитьСоответствие(ТаблицаРеквизитов, "НДС",                      "ставка ндс");
	ДобавитьСоответствие(ТаблицаРеквизитов, "ТипНоменклатуры",          "тип номенклатуры");
...
Процедура ЗагрузитьНоменклатуру(ИмяРаздела, ПрочитанныеДанные, Ошибки)
	...
	Для Каждого СтрокаНоменклатура Из НоваяНоменклатура Цикл
		...
		Иначе
			Продолжить;
		КонецЕсли;
		// Вид номенклатуры заполняется в зависимости от формата загружаемого файла
		Если Не ЗначениеЗаполнено(СтрокаНоменклатура.ВидНоменклатуры)
			И ЗначениеЗаполнено(СтрокаНоменклатура.ТипНоменклатуры) Тогда
			СтрокаНоменклатура.ВидНоменклатуры = СтрокаНоменклатура.ТипНоменклатуры;
		КонецЕсли;
...
Процедура СоздатьНоменклатуру(СтрокаНоменклатура, ГруппыНоменклатуры, ПараметрыСоздания, Ошибки)
	...
	Если ЗначениеЗаполнено(СтрокаНоменклатура.Родитель) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Наименование = СтрокаНоменклатура[РеквизитНаименование];
	НоменклатураОбъект.НаименованиеПолное = Наименование;
	НоменклатураОбъект.Наименование = ?(СтрДлина(Наименование) > 100, Лев(Наименование, 100), Наименование);
	Если ЗначениеЗаполнено(СтрокаНоменклатура.ВидНоменклатуры) Тогда
		Если НРег(СтрокаНоменклатура.ВидНоменклатуры) = ИмяВидаНоменклатурыУслуга() Тогда
	...
	СтавкаНДС = УчетНДС.СтавкаНДСПоПредставлению(ПрочитаннаяСтавкаНДС);
...
Функция ИмяКолонкиИдентифицирующейКоличествоОбъектов()
...
Функция ИмяДокументыБезРазбивки()
	
	Возврат "ДокументыБезРазбивки";
	
КонецФункции

Функция ИмяОсновныеСредства()
	
	Возврат "ОсновныеСредства";
	
КонецФункции

Функция ИмяОборотноСальдоваяВедомость()
...
Функция ТипЗначенияРеквизита()
	
	Возврат "ТипЗначенияРеквизита";
...
Функция ПризнакЗаголовкаОборотноСальдовойВедомостиПоСчету()
	
	Возврат "оборотно-сальдоваяведомостьпосчету";
...
Процедура ДобавитьСтрокуВОписаниеФайлов(Описание, Имя, Заголовок, Представление = "")
...
Функция НоваяТаблицаДанных(СоставДанных)
	...
	ТаблицаДанных.Колонки.Добавить("НПП", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10)));
	Возврат ТаблицаДанных;
...
Процедура УдалитьДублирующиесяКолонки(СоставДанных)
	
	УдаляемыеСтроки = Новый Массив;
	Для Каждого Строка Из СоставДанных Цикл
		Если ЗначениеЗаполнено(Строка.ИмяРеквизитаВФайле) И Не ЗначениеЗаполнено(Строка.Значение) Тогда
			УдаляемыеСтроки.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		СоставДанных.Удалить(УдаляемаяСтрока);
	КонецЦикла;
...
Функция ДобавитьСоответствие(Таблица, ИмяРеквизита1С, ИмяРеквизитаВФайле = "", Тип = Неопределено)
	...
	Строка[ТипЗначенияРеквизита()] = Тип;
	Возврат Строка;
...
Процедура ДобавитьСтрокуРеквизитаДляНесколькихФорматов(
	Реквизиты, ИмяРеквизита1С, ЗначениеТекстовыйФормат = "", ЗначениеФорматЭлектроннойТаблицы = "", Тип = Неопределено)
...
Функция ОбработатьЧисловойРеквизит(ИсходнаяСтрока)
	...
	Результат = СтрокаВЧисло(ЧислоСтрокой);
...
Функция СимволДопустимВЧисловомРеквизите(Символ)
	
	Код = КодСимвола(Символ);
	// 44 и 46 - коды символов точки и запятой
	// 45 - код знака "минус"
	Возврат Код = 44 Или Код = 46 Или Код = 45;
...
Функция СимволПробела(Символ)
	...
	Возврат КодСимвола(Символ) = 32 Или Символ = Символы.НПП;
	
КонецФункции

Функция РеквизитыШапкиОборотноСальдовойВедомости(Таблица)
	
	Результат = НовыйРезультатЧтенияШапкиОборотноСальдовойВедомости();
	...
	Для НомерСтроки = 1 По ГраницаСтрока Цикл
		Для НомерКолонки = 1 По ГраницаКолонка Цикл
			...
			ТекстЯчейки = НРег(СтрЗаменить(ТекстЯчейки, " ", ""));
			Если Не ЗначениеЗаполнено(ТекстЯчейки) Тогда
				Продолжить;
			КонецЕсли;
			Если СтрНачинаетсяС(ТекстЯчейки, ПризнакЗаголовкаОборотноСальдовойВедомостиПоСчету()) Тогда
				Результат.Счет = СтрЗаменить(ТекстЯчейки, ПризнакЗаголовкаОборотноСальдовойВедомостиПоСчету(), "");
			ИначеЕсли СтрНачинаетсяС(ТекстЯчейки, "период:") Тогда
				ДатаСоставная = Новый Массив;
				СборнаяЦифра = Новый Массив;
				ДлинаСтроки = СтрДлина(ТекстЯчейки);
				Для Индекс = 0 По ДлинаСтроки - 1 Цикл
					ОчереднойСимвол = Сред(ТекстЯчейки, ДлинаСтроки - Индекс, 1);
					Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОчереднойСимвол) Тогда
						СборнаяЦифра.Вставить(0, ОчереднойСимвол);
					Иначе
						ДатаСоставная.Добавить(СтрСоединить(СборнаяЦифра));
						СборнаяЦифра.Очистить();
						Если ДатаСоставная.Количество() = 3 Тогда
							Прервать;
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
				Попытка
					Результат.Дата = Дата(ДатаСоставная[0], ДатаСоставная[1], ДатаСоставная[2]);
				Исключение
					Результат.Дата = НачалоГода(ТекущаяДатаСеанса()) - 1;
				КонецПопытки;
				Результат.НомерСтроки = НомерСтроки;
			КонецЕсли;
		КонецЦикла;
...
Функция НовыйНоменклатураДокумента()
	
	Номенклатура = Новый ТаблицаЗначений;
	Номенклатура.Колонки.Добавить(ИмяКолонкиИдентифицирующейНоменклатуру());
	Номенклатура.Колонки.Добавить("ТипДокумента");
	
	Возврат Номенклатура;
	
КонецФункции

Функция СчетаНоменклатуры()
...
Функция СоответствияСчетовНоменклатурыРазделамУчета()
	
	Результат = Новый Структура;
	Результат.Вставить("Счет41", Перечисления.РазделыУчетаДляВводаОстатков.Товары);
	Результат.Вставить("Счет10", Перечисления.РазделыУчетаДляВводаОстатков.Материалы);
	Результат.Вставить("Счет43", Перечисления.РазделыУчетаДляВводаОстатков.ГотоваяПродукцияИПолуфабрикаты);
...
Процедура ЗагрузитьОстаткиПоОсновнымСредствам(ПрочитанныеДанные, ЗагружаемыеДанные, Ошибки)
	
	Если Не ЗначениеЗаполнено(ПрочитанныеДанные) Тогда
		Возврат;
	КонецЕсли;
	
	Амортизация = ПрочитанныеДанныеАмортизации(ЗагружаемыеДанные);
	
	ОсновныеСредства = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ЗагружаемыеДанные, ИмяОсновныеСредства(), Неопределено);
	Если ОсновныеСредства = Неопределено Тогда
		ТекстОшибки = НСтр(
			"ru = 'Для загрузки начальных остатков по основным средствам, добавьте файл со списком основных средств'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "", ТекстОшибки, "");
		Возврат;
	КонецЕсли;
	Организация = ЗагружаемыеДанные.Организация;
	Подразделение = ОсновноеПодразделениеОрганизации(Организация);
	Дата = ПрочитанныеДанные[0].Дата;
	СостояниеПринятия = СостояниеПринятияПоУмолчанию();
	
	НовыйДокумент = Документы.ВводНачальныхОстатков.СоздатьДокумент();
	
	ДанныеЗаполнения = Новый Структура;
	ДанныеЗаполнения.Вставить("Организация", Организация);
	ДанныеЗаполнения.Вставить("ПодразделениеОрганизации", Подразделение);
	НовыйДокумент.Заполнить(ДанныеЗаполнения);
	НовыйДокумент.Дата = Дата;
	
	НовыйДокумент.УстановитьНовыйНомер();
	НовыйДокумент.РазделУчета = Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства;
	КэшСпособовОтраженияРасходовПоАмортизации = Новый Соответствие;
	НомерДокументаПринятияКУчету = 1;
	
	Для Каждого Строка Из ПрочитанныеДанные Цикл
		НайденноеОС = ОсновныеСредства.Найти(Строка.Аналитика, "НаименованиеВВедомости");
		Если НайденноеОС = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		СуммаАмортизации = 0;
		Если ЗначениеЗаполнено(Амортизация) Тогда
			СтрокаАмортизация = Амортизация.Найти(Строка.Аналитика, "Аналитика");
			Если СтрокаАмортизация <> Неопределено Тогда
				СуммаАмортизации = СтрокаАмортизация.КредитСумма;
			КонецЕсли;
		КонецЕсли;
		СтрокаДокумента = НовыйДокумент.ОС.Добавить();
		СтрокаДокумента.ОсновноеСредство = НайденноеОС.Ссылка;
		СтрокаДокумента.ПервоначальнаяСтоимостьБУ = Строка.ДебетСумма - Строка.КредитСумма;
		СтрокаДокумента.ПервоначальнаяСтоимостьНУ = Строка.ДебетСумма - Строка.КредитСумма;
		СтрокаДокумента.ТекущаяСтоимостьБУ = СтрокаДокумента.ПервоначальнаяСтоимостьБУ - СуммаАмортизации;
		СтрокаДокумента.ТекущаяСтоимостьНУ = СтрокаДокумента.ТекущаяСтоимостьБУ;
		СтрокаДокумента.НакопленнаяАмортизацияБУ = СуммаАмортизации;
		СтрокаДокумента.НакопленнаяАмортизацияНУ = СуммаАмортизации;
		СтрокаДокумента.ИнвентарныйНомерРегл = НайденноеОС.ИнвентарныйНомер;
		СтрокаДокумента.СпособНачисленияАмортизацииБУ = СпособНачисленияАмортизации(НайденноеОС);
		СтрокаДокумента.СпособПоступленияРегл = Перечисления.СпособыПоступленияАктивов.ПриобретениеЗаПлату;
		СтрокаДокумента.СостояниеПринятияКУчетуРегл = СостояниеПринятия;
		СрокПолезногоИспользования = Число(НайденноеОС.СрокЭксплуатации);
		Если Не ЗначениеЗаполнено(СрокПолезногоИспользования) Тогда
			СрокПолезногоИспользования = 60;
		КонецЕсли;
		СтрокаДокумента.СрокПолезногоИспользованияБУ = СрокПолезногоИспользования;
		СтрокаДокумента.ДатаПринятияКУчетуРегл = НачалоГода(НачалоГода(Дата) - 1);
		СтрокаДокумента.НазваниеДокументаПринятияКУчетуРегл = НСтр("ru = 'Поступление основных средств'");
		СтрокаДокумента.НомерДокументаПринятияКУчетуРегл = НомерДокументаПринятияКУчету;
		НомерДокументаПринятияКУчету = НомерДокументаПринятияКУчету + 1;
		СтрокаДокумента.СостояниеПринятияКУчетуРегл = СостояниеПринятияПоУмолчанию();
		СтрокаДокумента.СчетУчетаБУ = ПланыСчетов.Хозрасчетный.ОСвОрганизации;
		СтрокаДокумента.СчетАмортизацииБУ = ПланыСчетов.Хозрасчетный.АмортизацияОсновныхСредств;
		СпособОтражения = КэшСпособовОтраженияРасходовПоАмортизации.Получить(НайденноеОС.СчетЗатрат);
		Если СпособОтражения = Неопределено Тогда
			СпособОтражения = СпособОтраженияРасходовПоАмортизации(НайденноеОС.СчетЗатрат);
			КэшСпособовОтраженияРасходовПоАмортизации.Вставить(НайденноеОС.СчетЗатрат, СпособОтражения);
		КонецЕсли;
		СтрокаДокумента.СпособОтраженияРасходовПоАмортизации = СпособОтражения;
		СтрокаДокумента.ПорядокПогашенияСтоимостиБУ = Перечисления.ПорядокПогашенияСтоимостиОС.НачислениеАмортизации;
		СтрокаДокумента.СпособНачисленияАмортизацииБУ = Перечисления.СпособыНачисленияАмортизацииОС.Линейный;
	КонецЦикла;
...
Функция ПрочитанныеДанныеАмортизации(ЗагружаемыеДанные)
	
	Если Не ЗагружаемыеДанные.Свойство(ИмяОборотноСальдоваяВедомость()) Тогда
		Возврат Неопределено;
	КонецЕсли;

	Результат = Новый Массив;
	ДанныеВедомостей = ЗагружаемыеДанные[ИмяОборотноСальдоваяВедомость()];
	Для Каждого Строка Из ДанныеВедомостей Цикл
		Если СтрНачинаетсяС(Строка.Счет, "02") Тогда
			Результат.Добавить(Строка);
		КонецЕсли;
	КонецЦикла;
	
	Возврат ДанныеВедомостей.Скопировать(Результат);
	
КонецФункции

Функция СпособНачисленияАмортизации(НайденноеОС)
...
Функция СостояниеПринятияПоУмолчанию()
...
Функция СпособОтраженияРасходовПоАмортизации(СчетЗатратСтрокой)
	
	СчетЗатрат = СчетЗатратИзСтроковогоЗначения(СчетЗатратСтрокой);
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Иначе
		...
		ЗначенияАналитики.Вставить("Организация");
		ЗначенияАналитики.Вставить("СчетЗатрат", СчетЗатрат);
		Возврат Справочники.СпособыОтраженияРасходовПоАмортизации.СоздатьСпособОтраженияРасходовПоЗначениямАналитики(
			ЗначенияАналитики);
...
Функция СчетЗатратИзСтроковогоЗначения(СчетЗатратСтрокой)
	
	СчетЗатратСтрокой = СокрЛП(СчетЗатратСтрокой);
	ДлинаОписанияСчета = СтрДлина(СчетЗатратСтрокой);
	СоставноеЗначениеСчета = Новый Массив;
	Для Сч = 1 По ДлинаОписанияСчета Цикл
		ОчереднойСимвол = Сред(СчетЗатратСтрокой, Сч, 1);
		Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОчереднойСимвол) Тогда
			СоставноеЗначениеСчета.Добавить(ОчереднойСимвол);
		Иначе
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	СчетЗатратПоУмолчанию = ПланыСчетов.Хозрасчетный.ОбщехозяйственныеРасходы;
	СчетЗатрат = Неопределено;
	Если ЗначениеЗаполнено(СоставноеЗначениеСчета) Тогда
		КодСчета = СтрСоединить(СоставноеЗначениеСчета);
		СчетЗатрат = ПланыСчетов.Хозрасчетный.НайтиПоКоду(КодСчета);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СчетЗатрат) Тогда
		СчетЗатрат = СчетЗатратПоУмолчанию;
	КонецЕсли;
...
Функция РекомендуемоеКоличествоОповещений()
	
	Возврат 100;
	
КонецФункции

Функция ИменаНеобходимыхРеквизитовТабличнойЧасти()
...
Функция СтрокаВЧисло(ЧислоСтрокой)
	...
	ЗапятаяДробнойЧасти = "(\d*),(\d{1,2})$";
	ТочкаВДробнойЧасти = "$1.$2";
	ЧислоСтрокойСТочкой = СтрЗаменитьПоРегулярномуВыражению(ЧислоСтрокой, ЗапятаяДробнойЧасти, ТочкаВДробнойЧасти);
	Результат = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ЧислоСтрокойСТочкой);
	Возврат Результат;
