...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АдресФайла) Или Не ЭтоАдресВременногоХранилища(АдресФайла) Тогда
		ТекстИсключения = НСтр("ru = 'Не выбран файл для загрузки.
			|Откройте обработку из формы списка лидов с помощью команды ""Загрузить"".'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	ОписаниеКолонок.Загрузить(Обработки.ЗагрузкаЛидовИзФайла.ОписаниеЗагружаемыхКолонок());
...
&НаКлиенте
Процедура ЗагружаемыеДанныеЛидПриИзменении(Элемент)
	
	ТабличнаяЧастьИзменена = Истина;
	ТекущиеДанные = Элементы.ЗагружаемыеДанные.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекущиеДанные.Лид) Тогда
		ЗаполнитьЗначенияСвойств(ТекущиеДанные, ПолучитьДанныеЛида(ТекущиеДанные.Лид));
...
Процедура УстановитьУсловноеОформление()
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ЗагружаемыеДанныеЛид");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.ЗагружаемыеДанные.Лид", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст",
		Новый ПолеКомпоновкиДанных("Объект.ЗагружаемыеДанные.Представление"));
...
Процедура ИзменитьТекстПереключателяОтбора()
	...
	СписокВыбора[0].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(
		НСтр("ru = 'Все'"), Статистика.Всего);
	СписокВыбора[1].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(
		НСтр("ru = 'Найденные'"), Статистика.Найденные);
	СписокВыбора[2].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(
		НСтр("ru = 'Новые'"), Статистика.Новые);
...
Функция ПодготовитьТабличныйДокументСервер(Знач АдресФайла, Знач РасширениеФайла, Знач ИдентификаторФормы, Знач ОписаниеКолонок)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подготовка табличного документа к загрузке лидов.'");
	
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаЛидовИзФайла.ОбработатьДанныеИзФайла",
...
Функция УдалитьЛишнееСервер()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаЛидовИзФайла.УдалитьВсеНенужныеСтрокиТаблицы",
		ПараметрыЗадания, ПараметрыВыполнения);
...
Функция ПодготовитьТаблицуДанныхДляЗагрузкиНаСервере()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Подготовка таблицы данных при загрузке лидов.'");
	
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаЛидовИзФайла.ПолучитьТаблицуДанныхДляЗагрузки",
		ПараметрыЗадания, ПараметрыВыполнения);
...
Функция ЗагрузитьДанныеИзФайлаНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаЛидовИзФайла.ЗагрузитьДанныеИзФайла",
		ПараметрыЗадания, ПараметрыВыполнения);
...
Процедура ЗагрузкаДанныхИзФайлаЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		СтруктураПараметров.Вставить("Ответственный", ПользователиКлиент.ТекущийПользователь());
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru = 'Загрузка лидов произведена.'"));
			Закрыть();
		КонецЕсли;
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		ВызватьИсключение ДлительнаяОперация.КраткоеПредставлениеОшибки;
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеЛида(Лид)
