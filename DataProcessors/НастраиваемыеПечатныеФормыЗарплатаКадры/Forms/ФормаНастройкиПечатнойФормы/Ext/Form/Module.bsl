...
Процедура УстановитьОтображениеПолейРедактора(УстановитьМакетПоУмолчанию = Ложь, АдресШаблонаНаСервере = Неопределено)
	...
	Иначе
		ЗарплатаКадрыОтчеты.ЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, ОбъектОтчета, КлючВарианта, ЭтотОбъект);
	КонецЕсли;
...
Процедура СохранитьИзменения()
	...
	Если Не ПустаяСтрока(АдресСхемы) Тогда
	...
	Если Не РежимРедактированияШаблонаДляКабинетаСотрудников
		И ЗначениеЗаполнено(ПутьКВариантуОтчета) Тогда
...
Процедура ВставитьПараметрыВОбласть(ОбластьЯчеек, ИменаРеквизитов)
	...
	Для Каждого ИмяРеквизита Из ИменаРеквизитов Цикл
		ОбластьЯчеек.Текст = Строка(ОбластьЯчеек.Текст) + " [" + ИмяРеквизита + "]";
	КонецЦикла;
...
Процедура УдалитьПользовательскийМакет()
	...
	КонецЕсли;
	ЗарплатаКадрыОтчеты.ОбновитьМакетДокументаНастроекВариантовОтчета(ВариантОтчета, Неопределено);
