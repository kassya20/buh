...
Процедура ЗаполнитьРеквизитыШапкиТТН(ДанныеПечати, Макет, ТабличныйДокумент, СведенияОКонтрагентах)
	...
	Если ДанныеПечати.Организация <> ДанныеПечати.Грузоотправитель Тогда
		...
		ПараметрыМакета.Вставить("ОрганизацияПоОКПО",    СведенияОКонтрагентах.Грузоотправитель.КодПоОКПО);
	ИначеЕсли ДанныеПечати.ОбособленноеПодразделение Тогда
		ПараметрыМакета.Вставить("ПредставлениеОрганизации", УчетНДСПереопределяемый.СведенияОПодразделенииОрганизации(ДанныеПечати.ПодразделениеОрганизации));
	Иначе
...
Процедура ЗаполнитьРеквизитыПодвалаТТН(ДанныеПечати, ИтоговыеСуммы, Макет, ТабличныйДокумент, ЕдиницаИзмеренияВеса = Неопределено, КоэффициентПересчетаВТонны = 0)
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеПечати.Склад) Тогда 
		МОЛ = ОтветственныеЛицаБП.ОтветственноеЛицоНаСкладе(ДанныеПечати.Склад, ДанныеПечати.Дата);
	...
	ПараметрыМакета.Вставить("ГрузПринялДолжность", ДанныеПечати.ВодительДолжность);
...
Процедура ЗаполнитьРеквизитыТранспортногоРазделаТТН(ДанныеПечати, Макет, ОбластьМакета, СведенияОКонтрагентах)
	...
	Если ДанныеПечати.ОбособленноеПодразделение Тогда
		СтруктураАдреса = УправлениеКонтактнойИнформациейБП.КонтактнаяИнформацияНаДату(
			ДанныеПечати.ПодразделениеОрганизации, Справочники.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций);
		
		ПараметрыМакета.Вставить("ПунктПогрузки",  ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураАдреса,"Представление", ""));
	Иначе
...
Процедура ЗаполнитьТабличныйДокументТТН(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеПечати.АдресДоставки) Тогда
			СведенияОГрузополучателе.Вставить("ФактическийАдрес", ДанныеПечати.АдресДоставки);
		КонецЕсли; 
		...
		Если УправлениеПечатьюБП.ЭтоПечатнаяФормаЗаПоставщика(ДанныеПечати.Ссылка) Тогда
			УправлениеПечатьюБП.ОчиститьФаксимильнуюПодписьИПечатьИзМакета(ТабличныйДокумент, ОбъектыПечати, ДанныеПечати.Ссылка);
		Иначе
			УправлениеПечатьюБП.ДоработатьФаксимильнуюПодписьИПечать(ТабличныйДокумент, ОбъектыПечати,
				ДанныеПечати.Организация, ДанныеПечати.Ссылка);
		КонецЕсли;
