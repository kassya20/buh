...
Процедура СоздатьПлатежныеПоручения(ПараметрыСоздания, АдресХранилища) Экспорт
	...
	КодВидаДохода                  = УчетЗарплаты.КодВидаДоходаПоВидуДоходаИсполнительногоПроизводства(
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПлатежнаяВедомость,
			"ВидДоходаИсполнительногоПроизводства"));
	
	Если УчетЗарплаты.ВедомостьПоДоговоруПодряда(ПлатежнаяВедомость) Тогда
		ВидОперации        = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеСотрудникуПоДоговоруПодряда;
		НазначениеПлатежа  = НСтр("ru = 'Оплата работ по договору ГПХ за %1 %2 г.: %3 '");
	...
	Для Каждого СтрокаТаблицы Из ПлатежныеПоручения Цикл
		...
		Если СтрокаТаблицы.Отметка И НЕ ЗначениеЗаполнено(СтрокаТаблицы.ПлатежноеПоручение) Тогда
			...
			ПлатежноеПоручение.ОчередностьПлатежа            = ОчередностьПлатежа;
			...
			КонецЕсли;
			ПлатежноеПоручение.КодВидаДохода                 = КодВидаДохода;
