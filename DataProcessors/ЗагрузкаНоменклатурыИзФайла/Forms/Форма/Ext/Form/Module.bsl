...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПрикладнаяЗагрузка Тогда
		...
		ДатаУстановкиЦен = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			Параметры.ПараметрыПрикладнойЗагрузки, "Период", ТекущаяДатаСеанса());
		...
		ПараметрВыбора = Новый ПараметрВыбора("Отбор.ВалютаЦены", Валюта);
		ПараметрыВыбораТипЦен = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрВыбора);
		Элементы.ТипЦен.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбораТипЦен);
		
		ОписаниеЗагружаемыхКолонок = Обработки.ЗагрузкаНоменклатурыИзФайла.ОписаниеЗагружаемыхКолонок(
			ПрикладнаяЗагрузка, Валюта);
	Иначе
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ВидимостьКолонки("Цена", ОписаниеКолонок)
		Или (ПрикладнаяЗагрузка И Не ИспользоватьТипыЦенНоменклатуры) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ТипЦен");
		МассивНепроверяемыхРеквизитов.Добавить("ДатаУстановкиЦен");
	КонецЕсли;
	
	Если Не (ПрикладнаяЗагрузка И ВедетсяСкладскойУчет) Тогда
	...
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		...
		Если ВидимостьКолонки("ЕдиницаИзмерения", ОписаниеКолонок) Тогда
		...
		Если ВидимостьКолонки("СтавкаНДС", ОписаниеКолонок) Тогда
...
Процедура ТипЦенПриИзменении(Элемент)
	...
	Если Не ПрикладнаяЗагрузка Тогда
		Валюта = ?(ЗначениеЗаполнено(ТипЦен),
			ПолучитьВалютуИзТипаЦенСервер(ТипЦен),
...
Процедура НадписьСсылкиУстановкиРеквизитовНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ЕстьКолонкаЕдиницаИзмерения", ВидимостьКолонки("ЕдиницаИзмерения", ОписаниеКолонок));
	ПараметрыФормы.Вставить("ЕстьКолонкаСтавкаНДС", ВидимостьКолонки("СтавкаНДС", ОписаниеКолонок));
...
&НаКлиенте
Процедура ДекорацияНастройкаПоискаНажатие(Элемент)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияФормыНастройкиПоиска", ЭтотОбъект);
	ОткрытьФорму("Обработка.ЗагрузкаНоменклатурыИзФайла.Форма.ФормаНастройкиПоиска",
		Новый Структура("ПоляПоиска", ПоляПоиска), ЭтотОбъект,,,, ОписаниеОповещения);
...
Процедура ДанныеФайлаВыборЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранаКолонкаСЦеной Тогда
		...
		Если Не ПрикладнаяЗагрузка Тогда
			Валюта = ?(ЗначениеЗаполнено(СведенияОТипеЦен.Валюта),
				СведенияОТипеЦен.Валюта,
				ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
		КонецЕсли;
	КонецЕсли;
...
Процедура ТоварыНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.Статус = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовый() Тогда // будет создана новая номенклатура
		СтандартнаяОбработка = Ложь;
		ДанныеВыбора = Новый СписокЗначений;
		ТекстПодбора = СтрШаблон(НСтр("ru = 'Создать: %1'"), ТекущиеДанные.Наименование);
		Если Не ПустаяСтрока(ТекущиеДанные.Артикул) Тогда
			ТекстПодбора = СтрШаблон(НСтр("ru = '%1, %2'"), ТекстПодбора, ТекущиеДанные.Артикул);
		КонецЕсли;
		Если ВидимостьКолонки("ЕдиницаИзмерения", ОписаниеКолонок) Тогда
			Если Не ПустаяСтрока(ТекущиеДанные.ЕдиницаИзмерения) Тогда
				ТекстПодбора = СтрШаблон(НСтр("ru = '%1, %2'"), ТекстПодбора, ТекущиеДанные.ЕдиницаИзмерения);
			КонецЕсли;
		Иначе
			ТекстПодбора = СтрШаблон(НСтр("ru = '%1, %2'"), ТекстПодбора, ЕдиницаИзмерения);
		КонецЕсли;
...
Процедура ТоварыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") И СтрНайти(ВыбранноеЗначение, НСтр("ru = 'Создать:'")) > 0 Тогда
		...
		ДанныеЗаполнения.Вставить("СтавкаНДС",
			?(ВидимостьКолонки("СтавкаНДС", ОписаниеКолонок), ТекДанные.СтавкаНДС, СтавкаНДС));
		ДанныеЗаполнения.Вставить("ЕдиницаИзмерения",
			?(ВидимостьКолонки("ЕдиницаИзмерения", ОписаниеКолонок), ТекДанные.ЕдиницаИзмерения, ЕдиницаИзмерения));
...
Процедура УправлениеФормой(Форма)
	...
	ЕстьЦена = ВидимостьКолонки("Цена", Форма.ОписаниеКолонок);
	
	Элементы.ТоварыЦена.Видимость = ЕстьЦена;
	Элементы.ТоварыЕдиницаИзмерения.Видимость = ВидимостьКолонки("ЕдиницаИзмерения", Форма.ОписаниеКолонок);
	Элементы.ТоварыШтрихкод.Видимость = ВидимостьКолонки("Штрихкод", Форма.ОписаниеКолонок);
	Элементы.ТоварыСтавкаНДС.Видимость = ВидимостьКолонки("СтавкаНДС", Форма.ОписаниеКолонок);
	Элементы.ТоварыКоличество.Видимость = ВидимостьКолонки("Количество", Форма.ОписаниеКолонок);
	Элементы.ТоварыСумма.Видимость = ВидимостьКолонки("Сумма", Форма.ОписаниеКолонок);
	
	ИспользованиеТипаЦен = ЕстьЦена
		И (Не Форма.ПрикладнаяЗагрузка Или Форма.ИспользоватьТипыЦенНоменклатуры);
	Элементы.ТипЦен.Видимость = ИспользованиеТипаЦен;
	Элементы.ДатаУстановкиЦен.Видимость = ИспользованиеТипаЦен;
	Элементы.Валюта.Видимость = ЕстьЦена И ЗначениеЗаполнено(Форма.ТипЦен) И Не ЗначениеЗаполнено(Форма.Валюта);
	Элементы.Склад.Видимость = Форма.ПрикладнаяЗагрузка И Форма.ВедетсяСкладскойУчет;
...
Процедура УстановитьУсловноеОформление()
	...
	// Поиск по УИД и изменение номенклатуры
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ТоварыНоменклатура");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.Товары.НайденоПоУИД", ВидСравненияКомпоновкиДанных.Равно, Истина);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.Товары.Представление"));
...
Процедура ЗаголовокСсылкиУстановкиРеквизитов()
	...
	СписокРеквизитов = "Родитель,ВидНоменклатуры,НоменклатурнаяГруппа"
		+ ?(ВидимостьКолонки("ЕдиницаИзмерения", ОписаниеКолонок), "", ",ЕдиницаИзмерения")
		+ ?(ВидимостьКолонки("СтавкаНДС", ОписаниеКолонок), "", ",СтавкаНДС");
...
Процедура ЗаполнитьТабличныйДокументСервер(АдресРезультата)
	...
	Если Не ПрикладнаяЗагрузка Тогда
		Валюта = ?(ЗначениеЗаполнено(ЗагруженныеДанные.Валюта),
			ЗагруженныеДанные.Валюта,
			ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
	КонецЕсли;
...
Функция ПодготовитьТаблицуТоваровСервер()
	...
	ПараметрыЗадания.Вставить("ПоляПоиска", ПоляПоиска);
	ПараметрыЗадания.Вставить("ОписаниеКолонок", ОписаниеКолонок.Выгрузить());
...
Процедура ЗаполнитьТаблицуТоваров(АдресРезультата)
	...
	Результат = ПолучитьИзВременногоХранилища(АдресРезультата);
	ПоляПоиска = Результат.ПоляПоиска;
	ОписаниеКолонок.Загрузить(Результат.ОписаниеКолонок);
...
Функция ЗагрузитьНаСервере()
	...
	ПараметрыЗадания.Вставить("ИспользоватьТипыЦенНоменклатуры", ИспользоватьТипыЦенНоменклатуры);
...
Процедура ЗагрузкаДанныхИзФайлаЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		Если Результат.Ошибки <> Неопределено Тогда
		...
		ИначеЕсли ПрикладнаяЗагрузка Тогда
			...
			Закрыть(Результат.ТаблицаОстатков);
...
&НаКлиенте
Процедура ПослеЗакрытияФормыНастройкиПоиска(Результат, ДополнительныеПараметры) Экспорт
	...
	ДлительнаяОперация = ВыполнитьПоискНоменклатуры();
	ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
	Обработчик = Новый ОписаниеОповещения("ВыполнитьПоискНоменклатурыЗавершение", ЭтотОбъект);
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация, Обработчик, ПараметрыОжидания);
	
КонецПроцедуры

&НаСервере
Функция ВыполнитьПоискНоменклатуры()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Поиск номенклатуры после изменения настроек.'");
	
	Возврат ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ЗагрузкаНоменклатурыИзФайла.ПроизвестиПоискНоменклатурыВФоне",
		ПараметрыЗадания, ПараметрыВыполнения);
	
КонецФункции

&НаКлиенте
Процедура ВыполнитьПоискНоменклатурыЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		ЗаполнитьТаблицуТоваровПослеПоиска(ДлительнаяОперация.АдресРезультата);
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		ВызватьИсключение ДлительнаяОперация.КраткоеПредставлениеОшибки;
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТаблицуТоваровПослеПоиска(АдресРезультата)
...
Функция ВидимостьКолонки(ИмяКолонки, ОписаниеКолонок)
	...
	СтруктураПоиска = Новый Структура("Идентификатор", ИмяКолонки);
	Колонка = ОписаниеКолонок.НайтиСтроки(СтруктураПоиска);
	Если Колонка.Количество() > 0 Тогда
		Возврат Колонка[0].Видимость;
	Иначе
		Возврат Ложь;
	КонецЕсли;
