...
Процедура ЗаполнитьСписокЗаказов()
	...
	|	МАКСИМУМ(ТаблицаРегистра.ДокументОснование <> &ДокументОснование) КАК ЕстьИноеОснование
	|ИЗ
	...
	|	ТаблицаРегистра.ДокументОснование КАК ДокументОснование
	|ИЗ
	|	РегистрСведений.ДанныеЗаказовСервисДоставки КАК ТаблицаРегистра
	|ГДЕ
	|	ТаблицаРегистра.ДокументОснование В(&СписокОснований)
	...
	ВыборкаОснование = РезультатЗапроса[0].Выбрать();
	ЕстьИныеОснования = Ложь;
	КоличествоФоновыхЗаданийСостоянияЗаказов = 0;
	Пока ВыборкаОснование.Следующий() Цикл
		ЕстьИныеОснования = ЕстьИныеОснования ИЛИ ВыборкаОснование.ЕстьИноеОснование;
	...
	ВыборкаОрганизация = РезультатЗапроса[1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
