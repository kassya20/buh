...
&НаКлиенте
Процедура ТарифыДетализацияСтоимостиПоУслугамПлательщикРольПриИзменении(Элемент)
...
Процедура УстановитьУсловноеОформление()
	...
	// Представление Плательщика
	Для каждого ЗначениеСписка Из Элементы.ТарифыДетализацияСтоимостиПоУслугамПлательщикРоль.СписокВыбора Цикл
		Элемент = УсловноеОформление.Элементы.Добавить();
		ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
		ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ТарифыДетализацияСтоимостиПоУслугамПлательщикРоль.Имя);
		ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Тарифы.ДетализацияСтоимостиПоУслугам.ПлательщикРоль");
		ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура УстановитьВидимостьДоступность();
	...
	Элементы.ТарифыДетализацияСтоимостиПоУслугамПлательщикРоль.Видимость = Кэш.ЭтоДеловыеЛинии;
...
Процедура СформироватьОтборыТарифов(ЗаполнитьИспользоватьПоТарифам = Ложь)
	...
	Элементы.ГруппаОтборы.Видимость = ПоказыватьОтборыТарифов();
	
КонецПроцедуры

&НаСервере
Функция ПоказыватьОтборыТарифов()
	Возврат Элементы.СпособОтгрузки.СписокВыбора.Количество() > 1
		Или Элементы.СпособДоставки.СписокВыбора.Количество() > 1
		Или Элементы.ГруппаОтборыОсновная.Видимость;
КонецФункции
...
Функция ТабличныйДокументИтоговаяИнформация()
	...
	Если ОплатаСтоимостьУслуг.Количество() > 0 И Кэш.ЭтоДеловыеЛинии Тогда
		СписокВыбораПлательщиков = Элементы.ТарифыДетализацияСтоимостиПоУслугамПлательщикРоль.СписокВыбора;
		...
		Для Каждого ПлательщикУслуги Из ОплатаСтоимостьУслуг Цикл
			...
			НайденныйЭлемент = СписокВыбораПлательщиков.НайтиПоЗначению(ПлательщикУслуги.ПлательщикРоль);
			Если НайденныйЭлемент <> Неопределено Тогда
				ОбластьМакета.Параметры.ПлательщикРоль = НайденныйЭлемент.Представление;
			КонецЕсли;
...
Процедура ЗаполнитьЗначенияСписковВыбораКонтактнойИнформацииУчастника(Контрагент, ЭтоОрганизация, КонтактноеЛицо,
	...
	ДополнитьВладельцевКонтактнойИнформации(Контрагент, МассивВладельцевКонтактнойИнформации);
...
&НаСервереБезКонтекста
Процедура ДополнитьВладельцевКонтактнойИнформации(Контрагент, МассивВладельцев)
	СервисДоставкиПереопределяемый.ДополнитьВладельцевКонтактнойИнформации(Контрагент, МассивВладельцев);
КонецПроцедуры
