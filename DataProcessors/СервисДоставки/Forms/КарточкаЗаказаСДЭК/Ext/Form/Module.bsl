...
#Область ОбработчикиСобытийЭлементовТаблицыФормыКомпенсацияДоставки
...
&НаКлиенте
Процедура КомпенсацияДоставкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
	
	ОчиститьСообщения();
	
	Если КомпенсацияДоставки.Количество() > 0 Тогда
		Сообщение = НСтр("ru = 'Несколько условий доступно только с услугой ""Частичная доставка"".'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(Сообщение, , "КомпенсацияДоставки", , Отказ);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура КомпенсацияДоставкиПриИзменении(Элемент)
	
	Если Элемент.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Элемент.ТекущиеДанные.СтавкаНДС = "" Тогда
		Элемент.ТекущиеДанные.СтавкаНДС = Элементы.КомпенсацияДоставкиСтавкаНДС.СписокВыбора.Получить(0).Значение;
	КонецЕсли;
...
Функция ПараметрыЗапросаСоздатьИзменитьЗаказНаДоставку(ПараметрыОперации, Отказ)
	...
	КонецЦикла;
	ДанныеФормы.Вставить("КомпенсацияДоставки", КомпенсацияДоставки.Выгрузить());
...
Процедура ОбработатьИзменениеРеквизитаФормы(ИмяРеквизита, ОбработкаНаСервере = Истина)
	...
	ИначеЕсли ИмяРеквизита = "ТипЗаказа" Тогда
		...
		ЗаполнитьСписокВыбораГрузВидРасшифровки(Элементы.ГрузВидРасшифровки.СписокВыбора, ТипЗаказа = Кэш.ТипыЗаказов.Доставка);
		Элементы.ГруппаОсновноеПодвал.Видимость = ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин;
		Элементы.КомпенсацияДоставки.Видимость = ТипЗаказа = Кэш.ТипыЗаказов.ИнтернетМагазин;
...
Процедура ОбработатьИзменениеРеквизитаФормыНаСервере(ИмяРеквизита, Отказ = Ложь)
	...
	ИначеЕсли ИмяРеквизита = "ОтправительКонтрагентСсылка"
		...
		Если ИмяРеквизита = "ОтправительКонтрагентСсылка" И РежимИзмененияСоставаОснований = 2  Тогда
			// Адрес по умолчанию берем из документа-основание
			ПараметрыУчастника.Удалить("Адрес");
		КонецЕсли;
...
Функция ПолучитьУслугиТарифов(Знач Идентификатор = "", Знач ТолькоВыбранные = Истина)
	...
	КонецЕсли;
	Если Кэш.ДеревоУслугТарифовЗаполнено И ТипЗнч(Идентификатор) = Тип("Строка") И Не ПустаяСтрока(Идентификатор) Тогда
...
Процедура ЗаполнитьНаложенныйПлатежЗапрещенПоДокументуОснованию()
	...
	Для Каждого ЭлементСписка Из ДокументыОснования Цикл
		ТипОснования = ТипЗнч(ЭлементСписка.Значение);
		Если СервисДоставкиПереопределяемый.ТипыОснованийДляНаложенногоПлатежаСДЭК().Найти(ТипОснования) = Неопределено Тогда
...
Процедура ПересчитатьКомпенсацияДоставкиСуммаНДС()
	...
	ТекущиеДанные = Элементы.КомпенсацияДоставки.ТекущиеДанные;
