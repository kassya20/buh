...
Процедура ПолучательКонтактноеЛицоТелефонПриИзменении(Элемент)
	...
	Модифицированность = Истина;
	ПолучательКонтактноеЛицоТелефонЗначение = 
		СервисДоставкиВызовСервера.КонтактнаяИнформацияПоПредставлению(ПолучательКонтактноеЛицоТелефонПредставление,
			"Телефон");
...
Процедура ПроверитьНомерТелефона(Префикс, ЕстьОшибки, ВыводитьПредупреждения = Истина)
	...
	ОчиститьМаскуТелефонаПриНеобходимости(Элементы[Префикс], Представление);
...
Процедура ОбработатьПараметры(Параметры, Префикс = "")
	...
	Для Каждого КлючЗначение Из ПараметрыДляФормы Цикл
		Если СтрНайти(КлючЗначение.Ключ, "ТелефонПредставление") > 0 Тогда
			ИмяЭлементаФормы = СтрЗаменить(КлючЗначение.Ключ, "Представление", "");
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Элементы, ИмяЭлементаФормы) Тогда
				ОчиститьМаскуТелефонаПриНеобходимости(Элементы[ИмяЭлементаФормы], КлючЗначение.Значение);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьСуммыОплаты(Отказ = Ложь)
	...
	ИначеЕсли СуммаНаложенногоПлатежа + СуммаНаложенногоПлатежаДополнительный > 0 
		И Не ЗначениеЗаполнено(НаложенныйПлатежВидОплаты) Тогда
			
		ТекстОшибки = НСтр("ru = 'Не заполнен способ оплаты.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки,, "НаложенныйПлатежВидОплаты",, Отказ);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция НомерТелефонаСоответствуетМаске(НомерТелефона, Маска) 
	...
	Для НомерСимвола = 1 По КоличествоСимволов Цикл
		...
		Если СтрНайти(ПроверяемыеСимволыМаски, СимволМаски) = 0 И СимволМаски <> СимволНомераТелефона Тогда
			Возврат Ложь;	
		КонецЕсли;		
	КонецЦикла; 
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОчиститьМаскуТелефонаПриНеобходимости(ЭлементФормы, Телефон)
	Маска = ЭлементФормы.Маска;
	Если ЗначениеЗаполнено(Маска) И Не НомерТелефонаСоответствуетМаске(Телефон, Маска) Тогда
		ЭлементФормы.Маска = "";
	КонецЕсли;
КонецПроцедуры
