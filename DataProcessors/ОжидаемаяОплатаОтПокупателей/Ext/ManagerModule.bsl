...
Процедура ЗаполнитьПланируемыеПлатежи(Параметры, АдресХранилища) Экспорт
	...
	ПланируемыеПлатежи = ОстаткиПоДокументам(Параметры.Организация, Неопределено, Ложь, Параметры.ДатаОплаты);
...
Функция ОстаткиПоДокументам(Организация = Неопределено, ДатаОстатков = Неопределено, ДляКалендаря = Ложь, ДатаОплаты = Неопределено)
	...
	Если ЗначениеЗаполнено(ДатаОплаты) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоСрокуОплаты", "Результат.СрокОплаты = &ДатаОплаты");
		Запрос.УстановитьПараметр("ДатаОплаты", ДатаОплаты);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоСрокуОплаты", "ИСТИНА");
	КонецЕсли;
...
Функция ТекстЗапросаПлатежи(ДляКалендаря, ДоступныОстаткиПоДокументамРасчетов, ДоступныОстаткиПоСчетамПокупателя)
	...
	Если ДоступныОстаткиПоДокументамРасчетов Тогда
		...
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС +
			...
			|		ИНАЧЕ ВЫБОР
			|				КОГДА ОстаткиПоДокументамРасчетов.Договор.УстановленСрокОплаты И ОстаткиПоДокументамРасчетов.Договор.ДатаОплаты <> ДатаВремя(1, 1, 1)
	...
	Иначе
		...
		|	Результат.СчетНаОплату КАК СчетНаОплату,
		|	Результат.СуммаСчета КАК СуммаСчета,
		|	Результат.ДокументРасчетов КАК ДокументРасчетов,
		|	Результат.Организация КАК Организация,
		|	Результат.Контрагент КАК Контрагент,
