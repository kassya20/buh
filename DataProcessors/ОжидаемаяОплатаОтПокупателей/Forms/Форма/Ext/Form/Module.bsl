...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаОплаты = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "ДатаОплаты", Дата(1, 1, 1));
...
Функция ПодготовитьПараметрыЗаполнениеОстатков()
	...
	ПараметрыЗадания.Вставить("ДатаОплаты", ДатаОплаты);
...
Процедура СформироватьПисьмоСНапоминанием(Знач ВыбранныеСтроки)
	...
	Если ВыбранныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не выбраны задолженности для напоминания'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , "Объект.БухОстатки");
		Возврат;
	КонецЕсли;
...
Процедура МобильныйКлиентАдаптацияФормы()
	...
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКСписок(ЭтотОбъект, Элементы.БухОстатки);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМКСписок(ЭтотОбъект, Элементы.ГруппаИтогиОстатки);
	
	Элементы.Переместить(Элементы.ИтогоКОплате, Элементы.ГруппаИтогиОстатки);
	Элементы.Переместить(Элементы.НастройкаСчетовУчетаРасчетов, Элементы.ГруппаИтогиОстатки);
	Элементы.ГруппаИтогиОстатки.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
...
Процедура АдаптироватьМККонтекстСписокФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ПереместитьЭлементФормы(Элементы,
		Элементы.ФормаНапомнить,
		Элементы.БухОстатки.КонтекстноеМеню,
		Элементы.БухОстаткиКонтекстноеМенюОткрытьДокумент);
...
Процедура АдаптироватьМКСписокФормы()
	...
	МобильныйКлиентБПФормаСписка.ПодготовитьСписокФормыСправочника(ЭтотОбъект, "БухОстатки");
	
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Важное(ЭтотОбъект, Элементы.БухОстаткиСрокОплаты);
	Элементы.БухОстаткиОсталосьДней.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Важное(ЭтотОбъект, Элементы.БухОстаткиОсталосьДней);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.БухОстаткиКонтрагент, 1);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.БухОстаткиНомерТелефона, 1);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.БухОстаткиЭлектроннаяПочта, 1);
	Элементы.БухОстаткиСуммаПредоплата.Формат = "ЧФ='Сумма по счету: Ч'";
	Элементы.БухОстаткиСуммаПредоплата.ГоризонтальноеПоложениеВШапке = ГоризонтальноеПоложениеЭлемента.Лево;
	Элементы.БухОстаткиСуммаДолг.Формат = "ЧФ='Сумма по акту, накладной: Ч'";
	Элементы.БухОстаткиСуммаДолг.ГоризонтальноеПоложениеВШапке = ГоризонтальноеПоложениеЭлемента.Лево;
	
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.БухОстаткиСуммаПредоплата, 2);
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементВГруппуМК_Строка(ЭтотОбъект, Элементы.БухОстаткиСуммаДолг, 2);
	
	МобильныйКлиентБПФормаСписка.ВстроитьЭлементыВГруппуМК_Прочее(ЭтотОбъект, "БухОстатки");
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухОстаткиСуммаПредоплата");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухОстатки.СуммаПредоплата", ВидСравненияКомпоновкиДанных.Равно, 0);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "БухОстаткиСуммаДолг");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ЭлементУО.Отбор,
		"Объект.БухОстатки.СуммаДолг", ВидСравненияКомпоновкиДанных.Равно, 0);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
