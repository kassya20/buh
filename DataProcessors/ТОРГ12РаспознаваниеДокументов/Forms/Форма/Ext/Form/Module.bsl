...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СуммаВключаетНДСРеквизит = РаспознаваниеДокументовСлужебныйКлиентСервер.РеквизитДокумента(Объект, "СуммаВключаетНДС");
	СуммаВключаетНДС = СуммаВключаетНДСРеквизит.Значение;
	...
	РаспознаваниеДокументовПереопределяемый.ОпределитьПрименяетсяМалоценка(ЭтотОбъект, ПрименяетсяМалоценка);
	...
	РаспознаваниеДокументовПереопределяемый.ОпределитьНеобходимостьРасчетаСуммыВРознице(ЭтотОбъект, РассчитыватьСуммаВРознице);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если РаспознаваниеДокументовСлужебныйКлиент.ЭтоФормаВыбораАналитикиУчета(ИсточникВыбора) Тогда
...
Процедура НаправлениеПриИзменении(Элемент)
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПослеВопросаОСменеНаправления", ЭтотОбъект, Новый Структура("Направление", Объект.Направление));
...
Процедура Подключаемый_ОбработкаВыбораПоля(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = "РаспознатьФрагмент" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Если Объект.Дата < Дата("20250825") Тогда
			ВызватьИсключение НСтр(
				"ru = 'Распознавание с фрагмента изображения доступно для документов, загруженных после 25 августа 2025 г.'");
		КонецЕсли;
		
		ЭтоТаблица = (ТекущийЭлемент = Элементы.ТаблицаДокумента);
		
		ПорядокСтроки = Неопределено;
		НомерСтроки = Неопределено;
		Если ЭтоТаблица Тогда
			
			Если ТекущийЭлемент.ТекущиеДанные.НомерСтроки = 0 Тогда
				ЗаполнитьНомераСтрокНаСервере();
			КонецЕсли;
			
			ПорядокСтроки = ТекущийЭлемент.ТекущиеДанные.ПорядокСтроки;
			НомерСтроки = ТекущийЭлемент.ТекущиеДанные.НомерСтроки;
		КонецЕсли;
		
		ДанныеЭлементаДляРаспознавания = Новый Структура;
		ДанныеЭлементаДляРаспознавания.Вставить("ИмяЭлемента", Элемент.Имя);
		ДанныеЭлементаДляРаспознавания.Вставить("ЭтоТаблица", ЭтоТаблица);
		ДанныеЭлементаДляРаспознавания.Вставить("ПорядокСтроки", ПорядокСтроки);
		ДанныеЭлементаДляРаспознавания.Вставить("НомерСтроки", НомерСтроки);
		
		БаннерТекст = Элементы.ПолеПросмотра.Документ.getElementById("selectionModeMessage");
		БаннерТекст.textContent = СтрШаблон(
			НСтр("ru = 'Выделите фрагмент изображения для распознавания в поле ""%1""'"),
			Элемент.Заголовок);
			
		РежимВыделенияВключен = Булево(Элементы.ПолеПросмотра.Документ.getElementById("cropMode").value);
		Если Не РежимВыделенияВключен Тогда
			Элементы.ПолеПросмотра.Документ.getElementById("btn-cropmode").click();
		КонецЕсли;
		
		ПодключитьОбработчикОжидания("ЖдатьКоординаты", 1, Истина);
...
&НаКлиенте
Процедура ТаблицаДокументаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
...
Процедура ПриАктивизацииЯчейкиТаблицы(Элемент)
	...
	НомерСтрокиТЧ = ?(Элемент.ТекущиеДанные.НомерСтроки = 0, Элемент.ТекущиеДанные.ПорядокСтроки, Элемент.ТекущиеДанные.НомерСтроки);
	...
	Если Свойства = Неопределено Тогда
		...
		ЭлементКолонка.СписокВыбора.Добавить("РаспознатьФрагмент", "Распознать с изображения",,
...
&НаКлиенте
Процедура ПослеВопросаОСменеНаправления(Результат, Параметры) Экспорт
	
	Если Результат <> КодВозвратаДиалога.Да Тогда
		
		Если Параметры.Направление = ПредопределенноеЗначение("Перечисление.НаправленияРаспознанногоДокумента.Входящий") Тогда
			Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияРаспознанногоДокумента.Исходящий");
		Иначе
...
Процедура НастроитьВидимостьПоФункциональнымОпциям()
	...
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюВедетсяПроизводственнаяДеятельность(ВедетсяПроизводственнаяДеятельность);
	
	Элементы.ФормаСоздатьПоступлениеВПереработку.Видимость = ВедетсяПроизводственнаяДеятельность;
	
	ВедетсяУчетОсновныхСредств = Ложь;
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюВедетсяУчетОсновныхСредств(ВедетсяУчетОсновныхСредств);
	
	Элементы.ФормаСоздатьПоступлениеОборудование.Видимость = ВедетсяУчетОсновныхСредств;
	
	ВедетсяУчетЛизинговогоИмущества = Ложь;
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюВедетсяУчетЛизинговогоИмущества(ВедетсяУчетЛизинговогоИмущества);
	
	Элементы.ФормаСоздатьПоступлениеУслугиЛизинга.Видимость = ВедетсяУчетЛизинговогоИмущества;
	
	ИспользоватьФакторинг = Ложь;
	РаспознаваниеДокументовПереопределяемый.ПолучитьФункциональнуюОпциюИспользоватьФакторинг(ИспользоватьФакторинг);
	
	Элементы.ФормаСоздатьПоступлениеУслугиФакторинга.Видимость = ИспользоватьФакторинг;
...
Процедура ЗаполнитьТаблицуДокумента()
	...
	Для Каждого Запись Из Объект.РеквизитыТабличныхЧастей Цикл
		...
		Если Запись.ИмяРеквизита = "Номенклатура" Тогда
			...
			ДобавитьУсловноеОформлениеНеСопоставлен(Запись);
...
&НаСервере
Процедура ДобавитьУсловноеОформлениеНеСопоставлен(РеквизитТаблицыДокумента)
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(РеквизитТаблицыДокумента.ИмяРеквизита);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаДокумента.НомерСтроки");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = РеквизитТаблицыДокумента.НомерСтрокиТЧ;
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаДокумента." + РеквизитТаблицыДокумента.ИмяРеквизита);
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", Новый Цвет(255, 0, 0));
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", СтрШаблон(НСтр("ru = 'Не сопоставлен: %1'"), РеквизитТаблицыДокумента.РаспознанныйТекст));
...
Процедура НастроитьИзменяемыйЭлемент(РеквизитШапки, ИзменяемыйЭлемент, ДанныеДляВыбораРеквизитов)
	...
	Если РеквизитШапки.ИмяРеквизита <> "Склад" Тогда
		...
		Если ИзменяемыйЭлемент.СписокВыбора.НайтиПоЗначению("РаспознатьФрагмент") = Неопределено Тогда
			ИзменяемыйЭлемент.СписокВыбора.Добавить("РаспознатьФрагмент", "Распознать с изображения",, БиблиотекаКартинок.РаспознаваниеДокументов);
		КонецЕсли;
...
Функция СоздатьДокумент(ТипДокументаСтрокой, ПараметрыЗаполнения)
	...
	ПроводитьДокумент = Не ТекущиеНастройки.ОткрыватьСозданныйДокументДляПроверки;
...
Процедура ЗаполнитьАналитикуУчета(СтрокаТабличнойЧасти, ПараметрыОбъекта)
	...
	РаспознаваниеДокументовКлиентСерверПереопределяемый.ЗаполнитьАналитикуУчета(СтрокаТабличнойЧасти, ПараметрыОбъекта);
...
&НаКлиенте
Процедура ЖдатьКоординаты()
	
	ДанныеКоординат = Элементы.ПолеПросмотра.Документ.getElementById("cropData");
	Если Не ЗначениеЗаполнено(ДанныеКоординат.dataset.x0) Тогда
		ПодключитьОбработчикОжидания("ЖдатьКоординаты", 1, Истина);
		Возврат;
	КонецЕсли;
	...
	Координаты.Вставить("ИмяЭлемента", ДанныеЭлементаДляРаспознавания.ИмяЭлемента);
	Координаты.Вставить("КоординатаX0", Окр(ДанныеКоординат.dataset.x0));
	Координаты.Вставить("КоординатаY0", Окр(ДанныеКоординат.dataset.y0));
	Координаты.Вставить("КоординатаX1", Окр(ДанныеКоординат.dataset.x1));
	Координаты.Вставить("КоординатаY1", Окр(ДанныеКоординат.dataset.y1));
	
	ПослеПолученияКоординатНаСервере(Координаты);
	
	Если ДанныеЭлементаДляРаспознавания.ЭтоТаблица Тогда
		СвойстваЯчеекТаблицы = ПолучитьИзВременногоХранилища(АдресСвойствЯчеекТаблицы);
		ТекущийЭлемент = Элементы.ТаблицаДокумента;
		ПриАктивизацииЯчейкиТаблицы(Элементы.ТаблицаДокумента);
		Подключаемый_ПриИзмененииКолонки(Элементы[ДанныеЭлементаДляРаспознавания.ИмяЭлемента]);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ПослеПолученияКоординатНаСервере(Координаты)
	
	РаспознанныйТекст = РаспознаваниеДокументов.РаспознанныйТекстПоКоординатам(Объект.ИдентификаторРезультата, Координаты);
	
	Если ДанныеЭлементаДляРаспознавания.ЭтоТаблица Тогда
		ОбновитьДанныеРеквизитаТаблицыПоРаспознанномуТексту(РаспознанныйТекст, Координаты);
	Иначе
		ОбновитьДанныеРеквизитаПоРаспознанномуТексту(РаспознанныйТекст, Координаты);
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьДанныеРеквизитаПоРаспознанномуТексту(РаспознанныйТекст, Координаты)
	
	ИмяРеквизита = ДанныеЭлементаДляРаспознавания.ИмяЭлемента;
	
	Если СтрЗаканчиваетсяНа(ИмяРеквизита, "Белый") Или СтрЗаканчиваетсяНа(ИмяРеквизита, "Красный") Тогда
		ИмяРеквизита = СтрЗаменить(ИмяРеквизита, "Белый", "");
		ИмяРеквизита = СтрЗаменить(ИмяРеквизита, "Красный", "");
	КонецЕсли;
	
	РеквизитДокумента = РаспознаваниеДокументовСлужебныйКлиентСервер.РеквизитДокумента(Объект, ИмяРеквизита);
	
	РеквизитДокумента.РаспознанныйТекст = РаспознанныйТекст;
	
	ЗаполнитьЗначенияСвойств(РеквизитДокумента, Координаты, "КоординатаX0, КоординатаY0, КоординатаX1, КоординатаY1");
	
	ТипРеквизита = ТипЗнч(РеквизитДокумента.Значение);
	Если РаспознаваниеДокументовКлиентСервер.ЭтоПримитивныйТип(ТипРеквизита) Тогда
		РеквизитДокумента.Значение = РаспознаваниеДокументовСериализацияСлужебныйКлиентСервер.ПривестиТип(РаспознанныйТекст, ТипРеквизита);
	Иначе
		ДокументОбъект = РеквизитФормыВЗначение("Объект", Тип("ДокументОбъект.РаспознанныйДокумент"));
		
		ОписаниеТипа = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипРеквизита));
		РеквизитДокумента.Значение = ОписаниеТипа.ПривестиЗначение();
		
		РаспознаваниеДокументов.ЗаполнитьСписокВыбораРеквизита(ДокументОбъект, ДанныеЭлементаДляРаспознавания.ИмяЭлемента);
		Документы.РаспознанныйДокумент.ЗаполнитьЗначенияРаспознанногоДокумента(ДокументОбъект, ДокументОбъект.Ссылка);
	...
	ДанныеДляВыбораРеквизитов = РаспознаваниеДокументовСлужебный.ПолучитьДанныеДляСпискаВыбора(Объект.Ссылка);
	НастроитьИзменяемыйЭлемент(РеквизитДокумента, Элементы[ДанныеЭлементаДляРаспознавания.ИмяЭлемента], ДанныеДляВыбораРеквизитов);
	
	Если СтрЗаканчиваетсяНа(ДанныеЭлементаДляРаспознавания.ИмяЭлемента, "Белый") Тогда
		НастроитьИзменяемыйЭлемент(
			РеквизитДокумента, 
			Элементы[СтрЗаменить(ДанныеЭлементаДляРаспознавания.ИмяЭлемента, "Белый", "Красный")],
			ДанныеДляВыбораРеквизитов);
	КонецЕсли;
	
	Если СтрЗаканчиваетсяНа(ДанныеЭлементаДляРаспознавания.ИмяЭлемента, "Красный") Тогда
		НастроитьИзменяемыйЭлемент(
			РеквизитДокумента, 
			Элементы[СтрЗаменить(ДанныеЭлементаДляРаспознавания.ИмяЭлемента, "Красный", "Белый")],
			ДанныеДляВыбораРеквизитов);
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьДанныеРеквизитаТаблицыПоРаспознанномуТексту(РаспознанныйТекст, Координаты)
	
	РеквизитТаблицыДокумента = РаспознаваниеДокументовСлужебныйКлиентСервер.РеквизитТаблицыДокумента(
		Объект, ДанныеЭлементаДляРаспознавания.НомерСтроки, ДанныеЭлементаДляРаспознавания.ИмяЭлемента);
	
	РеквизитТаблицыДокумента.РаспознанныйТекст = РаспознанныйТекст;
	
	ЗаполнитьЗначенияСвойств(РеквизитТаблицыДокумента, Координаты, "КоординатаX0, КоординатаY0, КоординатаX1, КоординатаY1");
	
	НайденныеСтроки = ТаблицаДокумента.НайтиСтроки(Новый Структура("НомерСтроки", ДанныеЭлементаДляРаспознавания.НомерСтроки));
	Если НайденныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ТипРеквизита = ТипЗнч(СтрокаТаблицы[ДанныеЭлементаДляРаспознавания.ИмяЭлемента]);
	Если РаспознаваниеДокументовКлиентСервер.ЭтоПримитивныйТип(ТипРеквизита) Тогда
		РеквизитТаблицыДокумента.Значение = РаспознаваниеДокументовСериализацияСлужебныйКлиентСервер.ПривестиТип(РаспознанныйТекст, ТипРеквизита);
	Иначе
		ОписаниеТипа = Новый ОписаниеТипов(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипРеквизита));
		РеквизитТаблицыДокумента.Значение = ОписаниеТипа.ПривестиЗначение();
		
		ДокументОбъект = РеквизитФормыВЗначение("Объект", Тип("ДокументОбъект.РаспознанныйДокумент"));
		
		РаспознаваниеДокументов.ЗаполнитьСписокВыбораРеквизитаТабличнойЧасти(
			ДокументОбъект, ДанныеЭлементаДляРаспознавания.ИмяЭлемента, ДанныеЭлементаДляРаспознавания.НомерСтроки);
		Документы.РаспознанныйДокумент.ЗаполнитьЗначенияРаспознанногоДокумента(ДокументОбъект, ДокументОбъект.Ссылка);
		
		ЗначениеВРеквизитФормы(ДокументОбъект, "Объект");
	КонецЕсли;
	
	Если РеквизитТаблицыДокумента.ИмяРеквизита = "Номенклатура" Тогда
		ДобавитьУсловноеОформлениеНеСопоставлен(РеквизитТаблицыДокумента);
	КонецЕсли;
	
	ВсеСвойстваЯчеекТаблицы = РаспознаваниеДокументовСлужебный.СвойстваЯчеекТаблицы(Объект);
	АдресСвойствЯчеекТаблицы = ПоместитьВоВременноеХранилище(ВсеСвойстваЯчеекТаблицы, УникальныйИдентификатор);
	
	СтрокаТаблицы[ДанныеЭлементаДляРаспознавания.ИмяЭлемента] = РеквизитТаблицыДокумента.Значение;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьНомераСтрокНаСервере()
	
	ТаблицаКЗаполнению = ТаблицаДокумента.Выгрузить();
	РаспознаваниеДокументовСлужебный.СохранитьТаблицуДокумента(Объект, ТаблицаКЗаполнению);
	
	Для Каждого Строка Из ТаблицаКЗаполнению Цикл
		
		НайденныеСтроки = ТаблицаДокумента.НайтиСтроки(Новый Структура("ПорядокСтроки", Строка.ПорядокСтроки));
		Если НайденныеСтроки.Количество() > 0 Тогда
			НайденныеСтроки[0].НомерСтроки = Строка.НомерСтроки;
		КонецЕсли;
