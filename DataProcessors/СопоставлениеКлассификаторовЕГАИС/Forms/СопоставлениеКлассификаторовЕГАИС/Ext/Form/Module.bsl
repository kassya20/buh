...
Процедура СоздатьНесопоставленные_Завершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаКонтрагенты Из Объект.Организации Цикл
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			СтрокаКонтрагенты.ТорговыйОбъект = ИнтеграцияЕГАИСВызовСервера.ТорговыйОбъектКонтрагента(ВыбранноеЗначение);
		КонецЕсли;
		
		СтрокаКонтрагенты.Сопоставлено = ЗначениеЗаполнено(СтрокаКонтрагенты.Контрагент)
		                             И (ЗначениеЗаполнено(СтрокаКонтрагенты.ТорговыйОбъект) ИЛИ Не ИспользоватьТорговыеОбъектыКонтрагентов);
		СтрокаКонтрагенты.ЕстьИзменения = Истина;
