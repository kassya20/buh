...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьПерепроведение", Истина);
...
Функция ПараметрыПерепроведения() Экспорт
	...
	Результат.Вставить("ОтменятьПередПроведением", 	       Ложь);
	...
	Результат.Вставить("ТолькоПерепроведениеДокументов",   Ложь);
	Результат.Вставить("ЭтоИнтерактивныйЗапуск",           Ложь); // Запуск выполнен из интерактивного сеанса - пользователем.
	Результат.Вставить("ПроверятьАктивностьПользователей", Ложь);
	Результат.Вставить("ИнтервалПроверкиВСекундах", 5);
	Результат.Вставить("ОчищатьАдресСОшибками",            Истина);
	
	Возврат Результат;

КонецФункции

// Описание для запущенного фонового задания, перепроводящего документы.
//
// Возвращаемое значение:
//   Строка - представление задания.
//
Функция НаименованиеФоновогоЗадания() Экспорт
	
	Возврат НСтр("ru = 'Групповое перепроведение документов'");
...
//
// Параметры:
//	ПараметрыПерепроведения - см. ПараметрыПерепроведения().
//
// Возвращаемое значение:
...
Функция ЗапуститьПерепроведение(ПараметрыПерепроведения) Экспорт
	...
	МоментНачала = ?(ПараметрыПерепроведения.МоментНачала = Неопределено,
		ПараметрыПерепроведения.МоментНачалаАктуализацииРасчетовСКонтрагентами,
		ПараметрыПерепроведения.МоментНачала);
	
	КлючЗадания = "";
	Если ПараметрыПерепроведения.ЭтоПомощникПоУчетуНДС Тогда
		
		КлючЗадания = ПомощникРасчетаНДС.СобратьКлючЗаданияДляФоновогоЗадания(
				ПараметрыПерепроведения.Организация,
				ПользователиКлиентСервер.ТекущийПользователь(),
				ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий().Проведение,
				ПараметрыПерепроведения.ДатаОкончания,
				МоментНачала.Дата);
				
	КонецЕсли;
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(ПараметрыПерепроведения.УникальныйИдентификаторФормы);
...
Функция ВыполнитьПерепроведение(Параметры) Экспорт
	...
	РезультатОбработки.Вставить("ПериодСОшибкойВыполнения",					'00010101');
	...
	РезультатОбработки.Вставить("СозданДокументНачисленияРезервовПремий",   Ложь);
	РезультатОбработки.Вставить("ВыполнениеОстановлено",                    Ложь);
	РезультатОбработки.Вставить("ЭтоИнтерактивныйЗапуск",                   Параметры.ЭтоИнтерактивныйЗапуск);
	...
	ПроверкаАктивности.ЭтоИнтерактивныйЗапуск           = Параметры.ЭтоИнтерактивныйЗапуск;
	ПроверкаАктивности.ПроверятьАктивностьПользователей = Параметры.ПроверятьАктивностьПользователей;
	ПроверкаАктивности.ИнтервалПроверкиВСекундах        = Параметры.ИнтервалПроверкиВСекундах;
	...
	ОчищатьАдресСОшибками = Параметры.ОчищатьАдресСОшибками;
	...
	Если НЕ Параметры.ДанныеЗаблокированы Тогда
		...
		Если ПроведениеПрервано Тогда
			...
			ДобавитьСообщенияВРезультатОбработки(ТаблицаСообщений, РезультатОбработки, ДатаНачала, ДатаОкончания, ОчищатьАдресСОшибками);
	...
	Запрос.УстановитьПараметр("ВедетсяУчетРасчетовПоЗарплатеСводно",        ПолучитьФункциональнуюОпцию("ВедетсяУчетРасчетовПоЗарплатеСводно"));
	...
	Пока ТекущийМесяц <= ДатаОкончания Цикл
		...
		ОтменитьПроведениеРегламентныхОпераций(
			...
			РегистрыТребующиеОчисткиПоОрганизациям,
			ТаблицаСообщений,
		...
		Для Каждого СтрокаОрганизации Из ТаблицаОрганизаций Цикл
			...
			Если НЕ ВыполнятьТолькоАктуализациюРасчетовСКонтрагентами Тогда
				...
				Пока ВыборкаПоДокументам.Следующий() Цикл
					...
					ТребуетсяЗавершение = ЗакрытиеМесяца.ВыполнениеОперацииОстановленоПользователем(ТекущаяОрганизация, ПроверкаАктивности);
					Если ТребуетсяЗавершение Тогда
						СтрокаОрганизации.ПроведенВХронологическойПоследовательности = Ложь;
						СтрокаОрганизации.НоваяДатаНарушенияПоследовательности = ВыборкаПоДокументам.Дата;
						РезультатОбработки.ВыполнениеОстановлено = Истина;
						РезультатОбработки.ПериодСОшибкойВыполнения = ТекущаяДатаПроведения;
					...
					Если СтрокаОрганизации.ЭтоПервыйДокументПоОрганизации Тогда
						...
						Если МоментНарушения <> Неопределено Тогда
							...
							// Т.к. проводится изменение границы последовательности, то необходимо у операций по формированию остатков
							// сбросить признак актуальности операций. Если последним документом в границе последовательности является сам
							// документ Ввода остатков, тогда актуальность выполненных операций не будет сбрасываться.
							УчетНДСРаздельныйФормированиеНачальныхОстатков.СброситьАктуальностьОстатков(ТекущаяОрганизация, МоментНарушения);
					...
					Иначе
						...
						Если Параметры.ОтменятьПередПроведением Тогда
							
							Успешно = ОтменитьПроведениеПервичногоДокумента(ВыборкаПоДокументам, СтрокаОрганизации, ТаблицаСообщений, РегистрыТребующиеОчисткиПоДокументам);
							Если Успешно И Не СтрокаОрганизации.ПропуститьПроверкуЗапретаИзменения Тогда
								// После первого успешной отмены проведения уже не нужна проверка доступности периода для изменений.
					...
					Иначе
						...
						Если Не ЗначениеЗаполнено(РезультатОбработки.ПериодСОшибкойВыполнения) Тогда
							РезультатОбработки.ПериодСОшибкойВыполнения = ТекущаяДатаПроведения;
						КонецЕсли;
				...
				// Проверим необходимость завершения после перепроведения первичных документов, т.к. документов за месяц могло и не быть.
				ТребуетсяЗавершение = ЗакрытиеМесяца.ВыполнениеОперацииОстановленоПользователем(ТекущаяОрганизация, ПроверкаАктивности);
				Если ТребуетсяЗавершение Тогда
					РезультатОбработки.ВыполнениеОстановлено = Истина;
					РезультатОбработки.ПериодСОшибкойВыполнения = ТекущаяДатаПроведения;
		...
		Если Параметры.ЭтоЗакрытиеМесяца ИЛИ Параметры.ЭтоПомощникПоУчетуНДС Тогда
			...
			Если Не Параметры.ТолькоПерепроведениеДокументов Тогда
				ВосстановитьСостояниеРегламентныхОпераций(
					ТаблицаРегламентныеОперации, ТаблицаОрганизаций, РегистрыТребующиеОчисткиПоОрганизациям,
					ТекущийМесяц, ТаблицаСообщений);
			КонецЕсли;
		...
		Иначе
			...
			// всех первичных документов по всем организациям, если позволяют условия.
			Если НЕ ПроведениеПрервано И Не Параметры.ТолькоПерепроведениеДокументов Тогда
				Успешно = ВыполнитьРегламентныеОперацииЗаМесяц(
					ТаблицаРегламентныеОперации, ТаблицаОрганизаций, РегистрыТребующиеОчисткиПоОрганизациям,
					РезультатОбработки, ТаблицаСообщений, Параметры.ОстанавливатьсяПоОшибке);
	...
	Для Каждого СтрокаОрганизации Из ТаблицаОрганизаций Цикл
		...
		ИначеЕсли СтрокаОрганизации.ПроведенВХронологическойПоследовательности Тогда
			// Были успешно проведены документы по организации за месяц, поэтому в следующий
			// раз начинаем с даты окончания, если вручную не выполнят регламентные операции.
			НоваяДатаНарушенияПоследовательности = ДатаОкончания;
		Иначе
	...
	// СтандартныеПодсистемы.ОценкаПроизводительности
	ОценкаПроизводительности.ЗакончитьЗамерВремени("ЗакрытиеМесяцаПерепроведениеДокументов",
		НачалоВыполнения, РезультатОбработки.ПроведеноДокументов);
...
Функция ТипыДокументовПоддерживающихОтложенноеПроведение() Экспорт
	...
	ИменаДокументов.Добавить("ПутевойЛист");
...
Функция ИсключаемыеИзПерепроведенияДокументыБезусловно() Экспорт
	...
	// Список безуcловно исключаемых из перепроведения документов состоит из документов, которые не влияют на движения первичных документов
	// и операции закрытия месяца. Также эти документы отсутствуют в последовательности ДокументыОрганизаций.
	// В него не входят документы, для которых в метаданных свойство Проведение = Запретить (например, ОперацияБух). Такие документы
	// отсекаются в цикле при построении текста запроса.

	Результат = Новый Структура();
	
	Результат.Вставить("ВыгрузкаРегламентированныхОтчетов");
	Результат.Вставить("ИнвентаризацияНЗП");
	Результат.Вставить("ПрочиеКонтролируемыеСделки");
	Результат.Вставить("РозничнаяПродажа");
	...
	// Документы библиотеки БЗКБ
	Результат.Вставить("АвансовыйПлатежИностранцаПоНДФЛ");
	Результат.Вставить("АктПроверкиСтраховыхВзносов");
	Результат.Вставить("БронированиеГражданПребывающихВЗапасе");
	Результат.Вставить("ВедомостьНаВыплатуЗарплатыВБанк");
	Результат.Вставить("ВедомостьНаВыплатуЗарплатыВКассу");
	Результат.Вставить("ВозвратНДФЛ");
	Результат.Вставить("ДоговорРаботыУслуги");
	Результат.Вставить("ДополнительныеСведенияДляОплатыОтпускаСФР");
	Результат.Вставить("ЗаявлениеНаПредоставлениеСтандартныхВычетовПоНДФЛ");
	Результат.Вставить("ЗаявленияОПредоставленииСведенийОТрудовойДеятельности");
	Результат.Вставить("ИсполнительныйЛист");
	Результат.Вставить("КадровыйПеревод");
	Результат.Вставить("ЛистокСообщенияДляВоенкомата");
	Результат.Вставить("ОперацияНалоговогоУчетаПоНДФЛ");
	Результат.Вставить("ОперацияУчетаПоСтраховымВзносам");
	Результат.Вставить("ОтзывСогласияНаОбработкуПерсональныхДанных");
	Результат.Вставить("ОтменаБронированияГражданПребывающихВЗапасе");
	Результат.Вставить("ОтпускБезСохраненияОплаты");
	Результат.Вставить("ОтзывЗаявленийОПредоставленииСведенийОТрудовойДеятельности");
	...
	Результат.Вставить("ПерерасчетНДФЛ");
	Результат.Вставить("ПерерасчетСтраховыхВзносов");
	Результат.Вставить("ПодтверждениеЗачисленияЗарплаты");
	Результат.Вставить("ПрекращениеСтандартныхВычетовНДФЛ");
	Результат.Вставить("ПриемНаРаботу");
	Результат.Вставить("РегистрацияТрудовойДеятельности");
	Результат.Вставить("РеестрДСВ_3");
	Результат.Вставить("РеестрСЗВ_6_2");
	Результат.Вставить("СведенияДляОплатыОтпускаСФР");
	Результат.Вставить("СведенияОПриемеПереводеИУвольненииДляВоенкомата");
	Результат.Вставить("СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД");
	Результат.Вставить("СверкаСВоеннымКомиссариатом");
	Результат.Вставить("СверкаСДокументамиВоинскогоУчета");
	Результат.Вставить("СогласиеНаОбработкуПерсональныхДанных");
	Результат.Вставить("СправкиНДФЛДляПередачиВНалоговыйОрган");
	Результат.Вставить("УведомлениеОПравеНаИмущественныйВычетДляНДФЛ");
	Результат.Вставить("Увольнение");
	Результат.Вставить("ХодатайствоОБронированииГражданПребывающихВЗапасе");
	...
	// Документы библиотеки ЗЕРНО
	Результат.Вставить("ЗапросОстатковПартийЗЕРНО");
	Результат.Вставить("ОформлениеСДИЗЗЕРНО");
	Результат.Вставить("ПогашениеСДИЗЗЕРНО");
	Результат.Вставить("СписаниеПартийЗЕРНО");
	Результат.Вставить("ФормированиеПартийЗЕРНО");
	Результат.Вставить("ФормированиеПартийИзДругихПартийЗЕРНО");
	Результат.Вставить("ФормированиеПартийПриПроизводствеЗЕРНО");
...
Функция СформироватьТекстыЗапросовВсеДокументы(Параметры, НомераТаблиц, НомераТаблицОтложенноеПроведение)
	...
	|	ВЫБОР
	|		КОГДА ПроверенныеДокументы.Документ ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК СчитатьРеквизитыПроверенными
	|ИЗ
	|	ЖурналДокументов.РегламентныеОперацииНДС КАК Журнал
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПроверенныеДокументы КАК ПроверенныеДокументы
	|		ПО (ПроверенныеДокументы.Организация = &Организация)
	|			И Журнал.Ссылка = ПроверенныеДокументы.Документ
	|ГДЕ
	|	Журнал.Проведен
	...
	Если ЕстьОтборПоСсылке Тогда
		...
		// Операции межотчетного периода попадают в последовательность следующего месяца и только в том случае, когда
		// дата начала совпадает с началом месяца.
		ТекстОперацияБухУсловияПериода1 = "((Журнал.Дата = &ДатаНачала И Журнал.Ссылка >= &НачальнаяСсылка ИЛИ Журнал.Дата > &ДатаНачала)
			| И Журнал.Дата <= &ДатаОкончания И НЕ Журнал.МежотчетныйПериод
			| ИЛИ Журнал.Дата = ДОБАВИТЬКДАТЕ(&ДатаНачала, СЕКУНДА, -1) И Журнал.МежотчетныйПериод)";
		ТекстОперацияБухУсловияПериода2 = "(Журнал.Дата >= &ДатаНачала И Журнал.Дата <= &ДатаОкончания И НЕ Журнал.МежотчетныйПериод
			| ИЛИ Журнал.Дата = ДОБАВИТЬКДАТЕ(&ДатаНачала, СЕКУНДА, -1) И Журнал.МежотчетныйПериод)";
	Иначе
		...
		ТекстОперацияБухУсловияПериода1 = "(Журнал.Дата >= &ДатаНачала И Журнал.Дата <= &ДатаОкончания И НЕ Журнал.МежотчетныйПериод
			| ИЛИ Журнал.Дата = ДОБАВИТЬКДАТЕ(&ДатаНачала, СЕКУНДА, -1) И Журнал.МежотчетныйПериод)";
		ТекстОперацияБухУсловияПериода2 = ТекстОперацияБухУсловияПериода1;
	КонецЕсли;
	...
	Для Счетчик = 0 По 1 Цикл 
		...
		ТекстыЗапросовПервыйМесяц[Счетчик] = СтрЗаменить(ТекстыЗапросовПервыйМесяц[Счетчик], 
			"&ОперацияБухУсловиеПериода", ТекстОперацияБухУсловияПериода1);
		...
		ТекстыЗапросовПоследующиеМесяцы[Счетчик] = СтрЗаменить(ТекстыЗапросовПоследующиеМесяцы[Счетчик], 
			"&ОперацияБухУсловиеПериода", ТекстОперацияБухУсловияПериода2);
	КонецЦикла;
...
Функция ЗаблокироватьДанныеПередПроведением(МассивОрганизаций, ТаблицаСообщений)
	...
	Для Каждого Организация ИЗ МассивОрганизаций Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заблокировать данные по организации %1 для группового перепроведения.
						   |Возможные причины: Выполняется групповое перепроведение, актуализация, закрытие месяца " 
						 + "(в т.ч. закрытие месяца по расписанию) в каком-либо отчете/обработке, или в другом сеансе.%2 %3'"),
				Организация,
				Символы.ПС,
...
Функция ТекстЗапросаРегламентныеОперации()
	...
	|	Журнал.Состояние КАК Состояние,
	|	ВЫБОР
	|		КОГДА Журнал.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийРегламентныхОпераций.Выполняется)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВыполненаОтменаПроведения
	|ИЗ
...
Процедура ОтменитьПроведениеРегламентныхОпераций(Параметры, РегДокументы, РегистрыТребующиеОчисткиПоОрганизациям,
												ТаблицаСообщений, ВыполнятьТолькоАктуализациюРасчетовСКонтрагентами)
	...
	// Меняем состояние рег.операции на "Выполняется", чтобы в случае сбоя знать, какие операции требуется выполнить заново.
	...
	// Определим те регистры, по которым нет ни одного движения за период. Их не нужно будет очищать при перепроведении.
	РегистрыТребующиеОчистки = РегистрыТребующиеОчисткиДляРегламентныхОпераций(РегистрыТребующиеОчисткиПоОрганизациям);
	...
	Для Каждого СтрокаДокумента Из РегДокументы Цикл
		...
		Попытка	
			...
			// Возможно что из-за ранее случившегося аварийного завершения приложения рег.операция уже в состоянии Выполняется.
			// Однако пометить ее в регистре как неактуальную все-равно стоит.
			...
			ОперацияОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
			ОперацияОбъект.ДополнительныеСвойства.Вставить("РегистрыТребующиеОчистки", РегистрыТребующиеОчистки[СтрокаДокумента.Организация]);
...
Функция ВыполнитьРегламентныеОперацииЗаМесяц(ТаблицаРегламентныеОперации, ТаблицаОрганизаций, РегистрыТребующиеОчисткиПоОрганизациям,
												РезультатОбработки, ТаблицаСообщений, ОстанавливатьсяПоОшибке)
	...
	Для Каждого СтрокаДокумента Из ТаблицаРегламентныеОперации Цикл
		...
		Если СтрокаОрганизации <> Неопределено И СтрокаОрганизации.ПропуститьПроверкуЗапретаИзменения Тогда
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("РегистрыТребующиеОчистки", РегистрыТребующиеОчистки[СтрокаДокумента.Организация]);
...
// Восстанавливаем состояние регламентных операций, которое было до начала перепроведения.
//
Процедура ВосстановитьСостояниеРегламентныхОпераций(РегДокументы, ТаблицаОрганизаций, РегистрыТребующиеОчисткиПоОрганизациям,
													Период, ТаблицаСообщений)
	...
	Если Успешно И ЗначениеЗаполнено(ВидПервойОперации) Тогда
		Для Каждого СтрокаОрганизации Из ТаблицаОрганизаций Цикл
			Если ЕстьОперацииНеТребующиеВыполнения Тогда
				...
				РегистрыСведений.НеактуальныеРегламентныеОперации.СдвинутьГраницуАктуальностиНазад(
					СтрокаОрганизации.Организация, НачалоМесяца(Период), НомерГруппы, ВидПервойОперации, Истина);
			Иначе
				РегистрыСведений.НеактуальныеРегламентныеОперации.СдвинутьГраницуАктуальностиНазад(
					СтрокаОрганизации.Организация, НачалоМесяца(Период), 0, Неопределено);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция РегистрыТребующиеОчисткиДляРегламентныхОпераций(РегистрыТребующиеОчисткиПоОрганизациям)
	Перем РегистрыПоТипуДокументов;
	
	РегистрыТребующиеОчистки = Новый Соответствие;
	
	МетаданныеОбъекта = Метаданные.Документы.РегламентнаяОперация;
	Для Каждого РегистрыТребующиеОчисткиПоДокументам Из РегистрыТребующиеОчисткиПоОрганизациям Цикл
		
		Если Не РегистрыТребующиеОчисткиПоДокументам.Значение.Свойство("РегламентнаяОперация", РегистрыПоТипуДокументов) Тогда
			РегистрыПоТипуДокументов = ПроведениеСервер.РегистрыТребующиеОчисткиДляТипа(РегистрыТребующиеОчисткиПоДокументам.Значение, МетаданныеОбъекта);
		КонецЕсли;
		РегистрыТребующиеОчистки.Вставить(РегистрыТребующиеОчисткиПоДокументам.Ключ, РегистрыПоТипуДокументов);
...
Функция ПровестиРегламентныеДокументыЗаМесяцПоНДС(РегДокументыНДС, РезультатОбработки, ТаблицаСообщений, ОстанавливатьсяПоОшибке, ПропуститьПроверкуЗапретаИзменения)
	...
	Для Каждого СтрокаДокумента Из РегДокументыНДС Цикл		
		...
		Попытка
			...
			Если СтрокаДокумента.СчитатьРеквизитыПроверенными Или ДокументОбъект.ПроверитьЗаполнение() Тогда
				ДокументОбъект.ДополнительныеСвойства.Вставить("СчитатьРеквизитыПроверенными", Истина);
...
Функция ТекстыЗапросовПоПервичнымДокументам(Параметры, НомераТаблиц, НомерТаблицОтложенноеПроведение)
	...
	НомераТаблиц.Вставить("ВТ_ВсеДокументыДляПерепроведения",НомераТаблиц.Количество());
	...
	// а также "Операции Бух" и "Счет на оплату покупателю", которые не проводятся, но тоже двигают последовательность.
	// Счета на оплату регистрируются в последовательность только для плательщиков туристического налога.
	//
	...
	|	И (ТИПЗНАЧЕНИЯ(СтрокиТЧ.Ведомость) = ТИП(Документ.ВедомостьНаВыплатуЗарплатыВКассу)
	|			ИЛИ ТИПЗНАЧЕНИЯ(СтрокиТЧ.Ссылка.ПлатежнаяВедомость) = ТИП(Документ.ВедомостьНаВыплатуЗарплатыВКассу))
	...
	|ГДЕ
	|	&ВедетсяУчетРасчетовПоЗарплатеСводно
	|	И (ТИПЗНАЧЕНИЯ(СтрокиТЧ.Ведомость) = ТИП(Документ.ВедомостьНаВыплатуЗарплатыВБанк)
	|			ИЛИ ТИПЗНАЧЕНИЯ(СтрокиТЧ.Ссылка.ПлатежнаяВедомость) = ТИП(Документ.ВедомостьНаВыплатуЗарплатыВБанк))
	|	И СтрокиТЧ.Ссылка.Организация = &Организация
	|	И &ТабличнаяЧастьУсловиеПериода
	|;
	|
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Журнал.МежотчетныйПериод
	|				И Журнал.Дата = КОНЕЦПЕРИОДА(Журнал.Дата, МЕСЯЦ)
	|			ТОГДА ДОБАВИТЬКДАТЕ(Журнал.Дата, СЕКУНДА, 1)
	|		ИНАЧЕ Журнал.Дата
	|	КОНЕЦ,
	|	Журнал.Ссылка,
	|	Журнал.Организация,
	...
	|		ПО Журнал.Регистратор = СчетНаОплатуПокупателю.Ссылка
	|ГДЕ
	|	Журнал.Организация = &Организация
	...
	|	ИСТИНА,
	|	ВЫБОР
	|		КОГДА ИнвентаризацияНЗП.Проведен
	|			ТОГДА 1
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	ЛОЖЬ
	|ИЗ
	|	Последовательность.ДокументыОрганизаций КАК Журнал
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИнвентаризацияНЗП КАК ИнвентаризацияНЗП
	|		ПО Журнал.Регистратор = ИнвентаризацияНЗП.Ссылка
	|ГДЕ
	|	Журнал.Организация = &Организация
	|	И Журнал.СостояниеПроведения = ЗНАЧЕНИЕ(Перечисление.СостоянияПроведенияВПоследовательности.ПроведенСНарушениемПоследовательности)
	|	И &ПоследовательностьУсловиеПериода
	...
	|				ИЛИ (&ВедетсяУчетРасчетовПоЗарплатеСводно
	|					ИЛИ НЕ &ИспользуетсяПодсистемаУчетаЗарплатыИКадров)
	|					И (Журнал.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям)
	|						ИЛИ Журнал.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику))
	|					И (&УчетЗарплатыИКадровВоВнешнейПрограмме
	|						ИЛИ ВТ_ВыплатаЗарплатыВКассу.Ссылка ЕСТЬ НЕ NULL )
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ,
	|	ИСТИНА
	...
	|				ИЛИ (&ВедетсяУчетРасчетовПоЗарплатеСводно
	|					ИЛИ НЕ &ИспользуетсяПодсистемаУчетаЗарплатыИКадров)
	|					И (Журнал.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗП)
	|						ИЛИ Журнал.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗаработнойПлатыРаботнику))
	|					И (&УчетЗарплатыИКадровВоВнешнейПрограмме
	|						ИЛИ ВТ_ВыплатаЗарплатыВБанк.Ссылка ЕСТЬ НЕ NULL )
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ,
	|	ИСТИНА
	...
	ПраваОграничены = Не РольДоступна("ПолныеПрава");
	...
	Для Каждого МетаданныеДокумента Из Метаданные.Документы Цикл
		...
		Если ИмяДокумента = "НачислениеЗарплаты"
			...
			ИЛИ ИмяДокумента = "НачислениеОценочныхОбязательствПоОтпускам" Тогда
			// Зарплатные документы проводятся по месяцу начисления, а не по дате, поэтому их обрабатываем отдельно.
			Продолжить;
		ИначеЕсли ИмяДокумента = "РасходныйКассовыйОрдер"
			ИЛИ ИмяДокумента = "СписаниеСРасчетногоСчета" Тогда
			// Ввиду того что документами может выплачиваться зарплата в зависимости от способа учета зарплаты в/вне программы,
			// их обрабатываем отдельно.
			Продолжить;
		ИначеЕсли СтрНачинаетсяС(ИмяДокумента, "Удалить") Тогда
			// Отсечь документы с префиксом "Удалить".
			Продолжить;
		ИначеЕсли ДокументыНеТребующиеПерепроведения.Свойство(ИмяДокумента) Тогда
			// Не влияют на другие документы и не зависят от них. Отсутствуют в последовательности ДокументыОрганизаций.
			Продолжить;
		КонецЕсли;
        
		Если МетаданныеДокумента.Проведение <> Метаданные.СвойстваОбъектов.Проведение.Разрешить
		 Или МетаданныеДокумента.Движения.Количество() = 0 Тогда
			// Документ не проводится.
			Продолжить;
		ИначеЕсли МетаданныеДокумента.Реквизиты.Найти("Организация") = Неопределено Тогда
			// Документы без организации могут быть добавлены на конкретном внедрении у пользователей, но они не могут влиять на последовательность
			// и учетные данные в регистрах, т.к. организация является ключевым измерением. Поэтому такие документы не перепроводим.
			Продолжить;
		КонецЕсли;
		
		Если ПраваОграничены И Не ПравоДоступа("Чтение", МетаданныеДокумента) Тогда
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПроверенныеДокументы КАК ПроверенныеДокументы
	|		ПО (ПроверенныеДокументы.Организация = &Организация)
	|			И ВТ_ВсеДокументыДляПерепроведения.Ссылка = ПроверенныеДокументы.Документ
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата,
	|	Ссылка";
...
Функция ДокументВходитВПоследовательность(СсылкаНаДокумент, КэшСоставДокументовПоследовательности)
	...
	Если КэшСоставДокументовПоследовательности = Неопределено Тогда
		...
		Для Каждого МетаДок Из МетаданныеПоследовательности Цикл
			...
			Если Не РаботаСПоследовательностями.ДокументВходитВПоследовательностьИРегистрируется(МетаДок) Тогда
...
Функция ОтменитьПроведениеПервичногоДокумента(ВыборкаПоДокументам, СостояниеПоОрганизации, ТаблицаСообщений, РегистрыТребующиеОчисткиПоДокументам)
	...
	Попытка
		...
		// При отмене проведения документ может выдать информационные сообщения, которые помогут с расследованием невозможности проведения.
		// Запомним их для вывода в отчет.
		ПроведениеСервер.ЗапомнитьСообщенияПользователю(
			ТаблицаСообщений,
			ВыборкаПоДокументам.Организация,
			"",
...
Функция ПровестиПервичныйДокумент(ВыборкаПоДокументам, СостояниеПоОрганизации, ТаблицаСообщений, РегистрыТребующиеОчисткиПоДокументам)
	...
	Попытка
		...
		ДокументОбъект.ДополнительныеСвойства.Вставить("ГрупповоеПерепроведение",       Истина);
		ДокументОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьРасчетСтатуса",      Истина);
		ДокументОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьРасчетСтатусаГосИС", Истина);
		ДокументОбъект.ДополнительныеСвойства.Вставить("ПроведенВХронологическойПоследовательности",
														СостояниеПоОрганизации.ПроведенВХронологическойПоследовательности);
		Если Не ДокументОбъект.ПометкаУдаления Тогда // не был помечен за время выполнения группового перепроведения
			ДокументОбъект.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
		КонецЕсли;
		Если СостояниеПоОрганизации.ПропуститьПроверкуЗапретаИзменения Тогда
			ДокументОбъект.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
		...
		Если ВыборкаПоДокументам.СчитатьРеквизитыПроверенными Или ДокументОбъект.ПроверитьЗаполнение() Тогда
			ДокументОбъект.ДополнительныеСвойства.Вставить("СчитатьРеквизитыПроверенными", Истина);
			ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
			// При проведении документ может выдать информационные сообщения, 
			// не мешающие проведению, запомним их для вывода в отчет.
...
Функция ИсключитьИзПоследовательности(ВыборкаПоДокументам, ТаблицаСообщений)
	...
	НаборЗаписейПоследовательности = Последовательности.ДокументыОрганизаций.СоздатьНаборЗаписей();
	НаборЗаписейПоследовательности.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
...
Функция ВыполнитьОтложенныеРасчеты(Организация, ДатаОкончания, РезультатОбработки, ТаблицаСообщений, Параметры)
	...
	ПараметрыРасчета.ГрупповоеПерепроведение    = Истина;
...
Процедура ПроверитьНачислениеЗарплатыИРезервовОтпусков(Параметры, МесяцНачисления, Организация, РезультатОбработки)
	...
	// Не нужно создавать зарплатные документы ранее даты ввода начальных остатков.
	ДатаВводаНачальныхОстатков = РегистрыСведений.ДатыВводаНачальныхОстатков.ДатаВводаНачальныхОстатков(Организация);
	Если ЗначениеЗаполнено(ДатаВводаНачальныхОстатков)
	   И КонецМесяца(ДатаВводаНачальныхОстатков) >= МесяцНачисления Тогда
		Возврат;
	КонецЕсли;
	...
	ЕстьРезервыПремий		= УчетЗарплаты.ЕстьДокументНачисленияРезервовПремий(МесяцНачисления, Организация);
	
	// Если уже все начислено, то ничего не делаем.
	Если ЕстьНачислениеЗарплаты И ЕстьРезервыОтпусков И ЕстьРезервыПремий Тогда
		Возврат;
	КонецЕсли;
	...
	Если НЕ ЕстьНачислениеЗарплаты Тогда
		...
		Если ЗначениеЗаполнено(ДокументНачисленияЗарплаты) Тогда
			...
			Попытка
				...
				Иначе
					Объект.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
				КонецЕсли;
	...
	Если УчетЗарплаты.ВедетсяУчетРезервовОтпусков(Организация, МесяцНачисления)
		...
		Если ЗначениеЗаполнено(ДокументНачисленияРезервовОтпусков) Тогда
			...
			Исключение
				...
				РезервыОтпусковАрхивные = Не РезервыПоОплатеТруда.ЕстьНастройкиРезерваОтпусков(Объект.Организация, Объект.ПериодРегистрации);
				Если РезервыОтпусковАрхивные Тогда
					ПредставлениеДокумента = НСтр("ru = '""Резервы отпусков""'");
				Иначе
					ПредставлениеДокумента = НСтр("ru = '""Резервы по оплате труда"" (отпуск)'");
				КонецЕсли;
				
				ИнфоОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Не удалось провести документ %1 за %2 по %3 по причине:%4'"),
	...
	// Создадим документ начисления резервов отпусков.
	Если УчетЗарплаты.ВедетсяУчетРезервовПремий(Организация, МесяцНачисления)
		И НЕ ЕстьРезервыПремий Тогда

		ДокументНачисленияРезервовПремий = УчетЗарплаты.ПолучитьДокументНачисленияРезерваПремий(МесяцНачисления, Организация);
		Если ЗначениеЗаполнено(ДокументНачисленияРезервовПремий) Тогда

			Объект = ДокументНачисленияРезервовПремий.ПолучитьОбъект();
			Попытка
				Если Объект.ПометкаУдаления Тогда
					Объект.ПометкаУдаления = Ложь;
				КонецЕсли;
				...
				// Сообщим основному процессу закрытия месяца, что самостоятельно создали документ начисления резервов отпусков.
				РезультатОбработки.СозданДокументНачисленияРезервовПремий = Истина;
				
			Исключение
				ИнфоОбОшибке = ИнформацияОбОшибке();
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Не удалось провести документ ""Резервы по оплате труда"" (премия) за %1 по %2 по причине:%3'"),
					Формат(МесяцНачисления, "ДФ='ММММ гггг'"),
					Организация,
					ПодробноеПредставлениеОшибки(ИнфоОбОшибке));
...
Процедура ДобавитьСообщенияВРезультатОбработки(ТаблицаСообщений, РезультатОбработки, ДатаНачала, ДатаОкончания, ОчищатьАдресСОшибками)
	
	Если ТаблицаСообщений.Количество() > 0 Тогда
		
		РезультатОбработки.ВывестиИнформациюУведомлений = Истина;
		СообщениеСОшибкой = ТаблицаСообщений.Найти(0, "ТипСообщения");
		Если СообщениеСОшибкой <> Неопределено Тогда
			РезультатОбработки.ПериодСОшибкойВыполнения = СообщениеСОшибкой.Дата;
		КонецЕсли;
	...
	ИначеЕсли ОчищатьАдресСОшибками Тогда
