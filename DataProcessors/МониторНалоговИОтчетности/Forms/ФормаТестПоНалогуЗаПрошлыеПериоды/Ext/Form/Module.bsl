...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиНДФЛПредприниматель() Тогда
		ШаблонЗаголовок = НСтр("ru = 'Была ли у вас задолженность по НДФЛ на начало %1 года?'");
	Иначе
		ШаблонЗаголовок = НСтр("ru = 'Была ли у вас задолженность по УСН на начало %1 года?'");
	КонецЕсли;
	
	Элементы.НадписьВопрос.Заголовок = СтрШаблон(ШаблонЗаголовок, Формат(ГодЗаголовкаОпроса, "ЧГ=0"));
	
	РазрешеноРедактирование = ПравоДоступа("Изменение", Метаданные.Документы.ВводНачальныхОстатков);
	Элементы.ГруппаВопрос.ТолькоПросмотр = Не РазрешеноРедактирование;
	Элементы.КнопкаГотово.Доступность = РазрешеноРедактирование;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПереключательДа = ЗначениеВключенПереключательДа() Тогда
		Если НалогЗадолженность + ПеняЗадолженность + ШтрафЗадолженность = 0 Тогда
