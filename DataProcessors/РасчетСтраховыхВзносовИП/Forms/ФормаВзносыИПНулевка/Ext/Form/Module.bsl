...
&НаКлиенте
Процедура ТекущийГодДолгЕТПриИзменении(Элемент)
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ТекущийГодДолгПС.Видимость = Форма.ПерешлиНаЕдиныйТариф;
	Элементы.ТекущийГодДолгМС.Видимость = Форма.ПерешлиНаЕдиныйТариф;
		...
		Если ЗначениеЗаполнено(РезультатТестирования) Тогда
			...
			Если РезультатТестирования = РезультатЕстьДолги() Тогда
				...
				Для Каждого ЭлементСписка Из ОплаченныеГоды Цикл
					...
					ПрошлыеГодыДолгЕТ = ПрошлыеГодыДолгЕТ + ФиксированныеСтраховыеВзносыКУплате.СуммаВзносаЕдиныйТариф;
			...
			ИначеЕсли РезультатТестирования = РезультатВведенаЗадолженность() Тогда
				...
				ПрошлыеГодыДолгЕТ = ВходныеПараметры.ДолгЕдиныйТариф;
				ПрошлыеГодыПеняЕТ = ВходныеПараметры.ПеняЕдиныйТариф;
				ПрошлыеГодыШтрафЕТ = ВходныеПараметры.ШтрафЕдиныйТариф;
...
Процедура РассчитатьВзносы()
	...
	ПрошлыеГодыДолгЕТ  = 0;
	ПрошлыеГодыПеняЕТ  = 0;
	ПрошлыеГодыШтрафЕТ = 0;
	
	ТекущийГодДолгПС = 0;
	ТекущийГодДолгМС = 0;
	ТекущийГодДолгЕдиныйТариф = 0;
	...
	ТекущийГодДолгЕдиныйТариф = ФиксированныеСтраховыеВзносыКУплате.СуммаВзносаЕдиныйТариф;
	...
	Для Каждого СтрокаДолга Из ТаблицаДолговПоВзносам Цикл
		...
		Если СтрокаДолга.ВидПлатежа = Перечисления.ВидыПлатежейВГосБюджет.Налог Тогда
			...
			ИначеЕсли СтрокаДолга.Счет = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП Тогда
				ПрошлыеГодыДолгЕТ = СуммаОстаток;
			Иначе
		...
		ИначеЕсли СтрокаДолга.ВидПлатежа = Перечисления.ВидыПлатежейВГосБюджет.ПениАкт Тогда
			...
			ИначеЕсли СтрокаДолга.Счет = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП Тогда
				ПрошлыеГодыПеняЕТ = СуммаОстаток;
			Иначе
		...
		ИначеЕсли СтрокаДолга.ВидПлатежа = Перечисления.ВидыПлатежейВГосБюджет.Штраф Тогда
			...
			ИначеЕсли СтрокаДолга.Счет = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП Тогда
				ПрошлыеГодыШтрафЕТ = СуммаОстаток;
			Иначе
...
Процедура ПересчитатьИтогПоВзносам(Форма)
	...
	Элементы = Форма.Элементы;
	Форма.ТекущийГодИтого  = Форма.ТекущийГодДолгПС + Форма.ТекущийГодДолгМС + Форма.ТекущийГодДолгЕдиныйТариф;
	Форма.ПрошлыеГодыИтого = Форма.ПрошлыеГодыДолгПС + Форма.ПрошлыеГодыПеняПС + Форма.ПрошлыеГодыШтрафПС
		...
		+ Форма.ПрошлыеГодыДолгЕТ + Форма.ПрошлыеГодыПеняЕТ + Форма.ПрошлыеГодыШтрафЕТ;
...
Функция СоздатьПлатежныеДокументыНаСервере(СпособОплаты)
	...
	Если ОплатитьПрошлыеГоды Тогда
		...
		СтруктураПараметров.НалоговыйПериод = Дата(ГодТекущий - 1, 1, 1);
		СтруктураПараметров.ПериодСобытия   = КонецГода(Дата(ГодТекущий - 1, 1, 1));
	...
	Если ОплатитьТекущийГод Тогда
		...
		СтруктураПараметров.Организация     = Организация;
		СтруктураПараметров.НалоговыйПериод = Дата(ГодТекущий, 1, 1);
		СтруктураПараметров.Платежи         = ПоместитьВоВременноеХранилище(ТаблицаПлатежей);
		СтруктураПараметров.СпособОплаты    = СпособОплаты;
		СтруктураПараметров.СчетОрганизации = СчетОрганизации;
...
Функция УстановитьСостояниеПослеОплаты()
	...
	СчетаВзносовИП.Добавить(ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП); // 69.06.6
	...
	ТекущиеВзносыИПУплачено = ВзносыИП.СуммаВзносаПФРУплачено + ВзносыИП.СуммаВзносаФССУплачено
		+ ВзносыИП.СуммаВзносаФФОМСУплачено + ВзносыИП.СуммаВзносаПФРДобровольноУплачено;
...
Функция ПодготовитьТаблицуПлатежейПрошлыхГодов() Экспорт
	...
	// ПФР 69.06.6
	// Налог
	Если ПрошлыеГодыДолгЕТ <> 0 Тогда
		...
		НоваяСтрока.Сумма     = ПрошлыеГодыДолгЕТ;
	КонецЕсли;
	
	// Пеня
	Если ПрошлыеГодыПеняЕТ <> 0 Тогда
		НоваяСтрока = ТаблицаПлатежей.Добавить();
		НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП;
		НоваяСтрока.Субконто1 = Перечисления.ВидыПлатежейВГосБюджет.ПениАкт;
		НоваяСтрока.Сумма     = ПрошлыеГодыПеняЕТ;
	КонецЕсли;
	
	// Штраф
	Если ПрошлыеГодыШтрафЕТ <> 0 Тогда
		НоваяСтрока = ТаблицаПлатежей.Добавить();
		НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП;
		НоваяСтрока.Субконто1 = Перечисления.ВидыПлатежейВГосБюджет.Штраф;
		НоваяСтрока.Сумма     = ПрошлыеГодыШтрафЕТ;
	КонецЕсли;
...
Функция ПодготовитьТаблицуТекущегоГода() Экспорт
	...
	// Единый тариф 69.06.6 Налог
	Если ТекущийГодДолгЕдиныйТариф <> 0 Тогда
		НоваяСтрока = ТаблицаПлатежей.Добавить();
		НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП;
		НоваяСтрока.Субконто1 = Перечисления.ВидыПлатежейВГосБюджет.Налог;
		НоваяСтрока.Сумма     = ТекущийГодДолгЕдиныйТариф;
	КонецЕсли;
...
Процедура НачислитьВзносыЗаПрошлыеГоды(ЗапуститьНеВФоне = Ложь)
	...
	ПараметрыЗадания.Вставить("ДолгЕТ",                  ПрошлыеГодыДолгЕТ);
	ПараметрыЗадания.Вставить("ПеняЕТ",                  ПрошлыеГодыПеняЕТ);
	ПараметрыЗадания.Вставить("ШтрафЕТ",                 ПрошлыеГодыШтрафЕТ);
...
Функция НачислитьВзносыЗаПрошлыеГодыВФоне(Знач ПараметрыЗадания)
	...
	Если ПараметрыЗадания.ДолгЕТ > 0 Тогда
		...
		СтрокаСтраховыеВзносы.ЗначениеСубконтоДт = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("РасходыПоУплатеСтраховыхВзносовИП");
		СтрокаСтраховыеВзносы.СчетКт             = ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП;
		...
		СтрокаСтраховыеВзносы.Сумма              = ПараметрыЗадания.ДолгЕТ;
		СтрокаСтраховыеВзносы.Содержание         = НСтр("ru = 'Начислены страховые взносы за прошлые годы'");
		
	КонецЕсли;
	
	Если ПараметрыЗадания.ПеняЕТ > 0 Тогда
	...
	Если ПараметрыЗадания.ШтрафЕТ > 0 Тогда
		...
		СтрокаСтраховыеВзносы.Сумма              = ПараметрыЗадания.ШтрафЕТ;
