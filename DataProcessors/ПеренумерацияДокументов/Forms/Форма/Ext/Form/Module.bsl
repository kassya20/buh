...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НачальныйНомерДляСФНаАванс.Видимость = ОтдельнаяНумерацияСчетовФактурНаАванс;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ВидДокументов.Типы().Количество() = 0 Тогда
		ТекстИсключения = НСтр("ru = 'Обработка ""Перенумерация документов"" может быть открыта только из экспресс-проверки ведения учета.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	Если НЕ ДоступнаРольПолныеПрава Тогда
		ТекстИсключения = НСтр("ru = 'Проводить перенумерацию документов может только пользователь с ролью ""Полные права"".'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	Если НачалоГода(НачалоПериода) <> НачалоГода(КонецПериода) Тогда
		ТекстИсключения = НСтр("ru = 'Проводить перенумерацию документов возможно только в пределах года.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
...
Процедура ЗаполнитьСписокОбъектов()
	...
	|	&СчетаФактурыНаАванс КАК СФНаАванс,
	...
	Если ПолноеИмяДокумента = "Документ.СчетФактураВыданный" Тогда
		...
		Если ОтдельнаяНумерацияСчетовФактурНаАванс Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
				"&СчетаФактурыНаАванс",
				"ВЫБОР
				|		КОГДА ТаблицаОбъектов.ВидСчетаФактуры В (&СчетаФактурыНаАванс)
				|			ТОГДА ИСТИНА
				|		ИНАЧЕ ЛОЖЬ
				|	КОНЕЦ");
			СчетаФактурыНаАванс = Перечисления.ВидСчетаФактурыВыставленного.СчетаФактурыНаАванс();
		Иначе
			СчетаФактурыНаАванс = Ложь;
		КонецЕсли;
	Иначе
		ТекстУсловия = "";
		Если ПеренумероватьПоПодразделению = ПоОбособленномуПодразделению() Тогда
	...
	Запрос.УстановитьПараметр("СчетаФактурыНаАванс", 		СчетаФактурыНаАванс);
	Выборка = Запрос.Выполнить().Выбрать();

	НачальныйНомер = 1000000000;
	НачальныйНомерДляСФНаАванс = 1000000000;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Документ.СФНаАванс Тогда
			НачальныйНомерДляСФНаАванс = Мин(Документ.СтарыйНомерЧислом, НачальныйНомерДляСФНаАванс);
		Иначе
...
Процедура СоставитьПланЗаменыНомеров()
	...
	ТаблицаДат = Список.Выгрузить(Новый Структура("СФНаАванс", Ложь), "Дата");
	ЗаполнитьНовыеНомера(ПервыйНомерДокументаВПериоде, ТаблицаДат, Ложь);
	
	ПервыйНомерДокументаВПериоде = НачальныйНомерДляСФНаАванс;
	ТаблицаДат = Список.Выгрузить(Новый Структура("СФНаАванс", Истина), "Дата");
	ЗаполнитьНовыеНомера(ПервыйНомерДокументаВПериоде, ТаблицаДат, Истина);
	
	Список.Сортировать("СФНаАванс,НовыйНомерЧислом");
...
&НаСервере
Процедура ЗаполнитьНовыеНомера(ПервыйНомерДокументаВПериоде,ТаблицаДат, СФНаАванс)
	
	ТаблицаДат.Свернуть("Дата");
	ДатыДокументов = ТаблицаДат.ВыгрузитьКолонку("Дата");
	Для Каждого День Из ДатыДокументов Цикл
		ДокументыДня = Список.НайтиСтроки(Новый Структура("Дата,СФНаАванс", День, СФНаАванс));
		...
		Для Номер = ПервыйНомерДокументаВПериоде По ПоследнийНомерДокументаВПериоде Цикл
			НайденныеДокументы = Список.НайтиСтроки(Новый Структура("Дата,СтарыйНомерЧислом,СФНаАванс", День, Номер, СФНаАванс));
			...
			КонецЕсли;
		КонецЦикла;
		ДокументыСИзменяемымНомером = Список.НайтиСтроки(Новый Структура("Дата,НовыйНомер,СФНаАванс", День, "", СФНаАванс));
		Для Индекс = 0 По ДокументыСИзменяемымНомером.Количество() - 1 Цикл
			...
			ДлинаНомера = СтрДлина(Документ.СтарыйНомер) - ?(СФНаАванс,1,0);
			Документ.НовыйНомер = Документ.Префикс + ?(СФНаАванс,"А","") + 
				СтроковыеФункцииКлиентСервер.ДополнитьСтроку(Формат(Документ.НовыйНомерЧислом, "ЧГ=0"), 
				ДлинаНомера - СтрДлина(Документ.Префикс), "0", "Слева");
...
Процедура Подключаемый_ОбновитьКоманды()
	ИсточникКоманды = ТекущийСписок(ЭтотОбъект);
	Если ИсточникКоманды <> Неопределено Тогда
		ПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(ЭтотОбъект, ИсточникКоманды);
	КонецЕсли;
КонецПроцедуры
// Конец СтандартныеПодсистемы.ПодключаемыеКоманды
