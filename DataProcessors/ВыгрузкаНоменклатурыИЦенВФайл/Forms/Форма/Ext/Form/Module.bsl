...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИспользоватьСкрытиеНоменклатуры Тогда
		ПоляНаборДанных = Схема.НаборыДанных.НоменклатураСЦенами.Поля;
		ПолеВАрхиве = ПоляНаборДанных.Найти("ВАрхиве");
		ПолеВАрхиве.ОграничениеИспользования.Условие = Истина;
		ПолеВАрхиве.ОграничениеИспользованияРеквизитов.Условие = Истина;
	КонецЕсли;
...
Процедура УстановитьНастройкиФормы(Схема)
	...
	СохраненныеНастройки = ХранилищеОбщихНастроек.Загрузить("ВыгрузкаНоменклутарыИЦенВФайл", "Настройки");
	...
	Если СохраненныеНастройки <> Неопределено Тогда
		...
		КомпоновкаДанныхКлиентСервер.СкопироватьЭлементы(Настройки.Отбор, СохраненныеНастройки.Отбор);
		
		Если СохраненныеНастройки.ДополнительныеСвойства.Свойство("ДополнительныеПоля", КоллекцияПолей) Тогда
		...
		Для каждого ЭлементОтбора Из ЭлементыОтбора Цикл
			Если ЗначениеЗаполнено(ЭлементОтбора.ПравоеЗначение) 
				И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЭлементОтбора.ПравоеЗначение)) 
				И НЕ ОбщегоНазначения.СсылкаСуществует(ЭлементОтбора.ПравоеЗначение) Тогда
		...
		Если СохраненныеНастройки.ДополнительныеСвойства.Свойство("ИспользоватьТипыЦен", ИспользоватьТипыЦенИзНастроек)
		...
		Если ИспользоватьСкрытиеНоменклатуры Тогда
			
			ПолеВАрхиве = Новый ПолеКомпоновкиДанных("ВАрхиве");
			ПолеВАрхивеНайдено = Ложь;
			Для Каждого Элемент Из ЭлементыОтбора Цикл
				Если ТипЗнч(Элемент) = Тип("ЭлементОтбораКомпоновкиДанных") И Элемент.ЛевоеЗначение = ПолеВАрхиве Тогда
					ПолеВАрхивеНайдено = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если НЕ ПолеВАрхивеНайдено Тогда
			
				НовоеПоле = ЭлементыОтбора.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
				НовоеПоле.ЛевоеЗначение = ПолеВАрхиве;
				НовоеПоле.ПравоеЗначение = Истина;
				НовоеПоле.Представление = НСтр("ru = 'Исключая скрытые товары'");
	...
	Если НЕ ИспользоватьСкрытиеНоменклатуры Тогда
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(Настройки.Отбор, "ВАрхиве");
	КонецЕсли;
