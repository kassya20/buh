...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Документ.ПоступлениеНаРасчетныйСчет.Форма.ФормаВыбора"
		Или ИсточникВыбора.ИмяФормы = "Документ.ПриходныйКассовыйОрдер.Форма.ФормаВыбора"
		Или ИсточникВыбора.ИмяФормы = "Документ.КорректировкаРеализации.Форма.ФормаВыбора"
		Или ИсточникВыбора.ИмяФормы = "Документ.ВозвратТоваровОтПокупателя.Форма.ФормаВыбора" Тогда
...
Процедура АвансыПокупателейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элементы.АвансыПокупателей.ТекущиеДанные.ДокументОплаты) Тогда
...
Процедура АвансыПокупателейПередУдалением(Элемент, Отказ)
	...
	ДанныеКУдалению.Вставить("НомерСтроки",     ТекущиеДанные.НомерСтроки);
...
Процедура АвансыПокупателейПередУдалениемНаСервере(ТекущаяСтрока)
	...
	Если Не ЗначениеЗаполнено(ТекущаяСтрока.ДокументОплаты) Тогда
		Возврат;
	КонецЕсли;
	
	// Проверим, что удаляемая строка не дублирует другую имеющуюся строку в таблице
	Для Каждого СтрокаСравнения Из АвансыПокупателей Цикл
		
		Если ТекущаяСтрока.НомерСтроки <> СтрокаСравнения.НомерСтроки
			И АналитикаСтрокСовпадает(ТекущаяСтрока, СтрокаСравнения) Тогда
	...
	// Запишем информацию об удаляемой строке
...
Функция НовыйТаблицаУдаленныеСтроки()
	...
	ТипыДокументаОплаты = Метаданные.РегистрыНакопления.ИППрочиеДоходы.Измерения.ДокументОплаты.Тип.Типы();
