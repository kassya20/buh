...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПараметрыВыбораКодаДохода(Элементы.ДоходыКодДохода, Декларация3НДФЛВыбраннаяФорма);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Индекс = 0 По Доходы.Количество() - 1 Цикл
		...
		Если ПустаяСтрока(СтрокаДоходы.ВидДохода) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , ПутьКТабличнойЧасти, , Отказ);
		КонецЕсли;
...
Процедура ДоходыКодВычетаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если МассивДоступныхВычетов = Неопределено Тогда
		МассивДоступныхВычетов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			ПредопределенноеЗначение("Справочник.ВидыВычетовНДФЛ.ПустаяСсылка"));
	КонецЕсли;
...
Функция СохранитьРезультаты()
	...
	Информация = НСтр("ru = 'Ценные бумаги, облигации, вклады (%1)'");
...
Функция СохранитьДанныеФормы()
	...
	КодыДоходовИИС = ПомощникЗаполнения3НДФЛ.КодыВидовДоходовЦенныеБумаги(Декларация3НДФЛВыбраннаяФорма).ИИС;
...
Функция СохранитьДанныеДекларации()
	...
	КодыВидовДоходовРФ = ПомощникЗаполнения3НДФЛ.ВидыДоходов(Декларация3НДФЛВыбраннаяФорма);
	...
	ТаблицаДоходов.Колонки.Добавить("ВидДоходаДекларация", ОбщегоНазначения.ОписаниеТипаСтрока(3));
	...
	СоответствиеКодовДоходов = Обработки.ПомощникЗаполнения3НДФЛ.СоответствиеДоходовИсточникам(Декларация3НДФЛВыбраннаяФорма);
	...
	Для Каждого СтрокаДоходов Из ТаблицаДоходов Цикл
		...
		СтрокаДоходов.ВидДоходаДекларация = СоответствиеКодовДоходов.Получить(СтрокаДоходов.ВидДохода);
		
		Если Не ЗначениеЗаполнено(СтрокаДоходов.ВидДоходаДекларация) Тогда
			СтрокаДоходов.ВидДоходаДекларация = КодыВидовДоходовРФ.ОперацииСЦеннымиБумагами;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ТаблицаДоходов.Количество() = 1 Тогда
		ДанныеДекларации.Вставить("КодВидаДохода", ТаблицаДоходов[0].ВидДоходаДекларация);
	Иначе
...
&НаСервереБезКонтекста
Процедура УстановитьПараметрыВыбораКодаДохода(Элемент, ВыбраннаяФорма)
	
	МассивВидовДоходов = КодыДоходовПоЦеннымБумагам(ВыбраннаяФорма);
...
&НаСервереБезКонтекста
Функция КодыДоходовПоЦеннымБумагам(ВыбраннаяФорма)
	
	КодыВидовДоходов = ПомощникЗаполнения3НДФЛ.КодыВидовДоходовЦенныеБумаги(ВыбраннаяФорма);
	
	КодыДоходов = Новый Массив;
	КодыДоходов.Добавить(КодыВидовДоходов.ЦенныеБумаги);
	
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма) Тогда
		Для Каждого ТекущийИдентификатор Из КодыВидовДоходов Цикл
			КодыДоходов.Добавить(ТекущийИдентификатор.Значение);
		КонецЦикла;
	ИначеЕсли ПомощникЗаполнения3НДФЛКлиентСервер.ВычетыРаспределяютсяПоДоходам(ВыбраннаяФорма) Тогда
		КодыДоходов.Добавить(КодыВидовДоходов.Вклады);
		КодыДоходов.Добавить(КодыВидовДоходов.Облигации);
		КодыДоходов.Добавить(КодыВидовДоходов.ЦифровыеАктивы);
	КонецЕсли;
	
	СтрокаКодовДоходовПоЦеннымБумагам = СтрСоединить(КодыДоходов, ",");
	...
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма) Тогда
		...
		|ГДЕ
		|	ВидыДоходовНДФЛ.КодПрименяемыйВНалоговойОтчетностиС2025Года В(&КодыДоходов)";
	Иначе
		Запрос.Текст =
		"ВЫБРАТЬ
...
Процедура УправлениеФормойПриСозданииНаСервере()
	...
	Элементы.ДоходыДобавить.Доступность = Не ТолькоПросмотр;
	
	Элементы.ГруппаБрокер.ТолькоПросмотр = ТолькоПросмотр;
	Элементы.ГруппаСведенияОДоходах.ТолькоПросмотр = ТолькоПросмотр;
