...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокОбъектовНедвижимости = ОбъектыНедвижимости(Период);
	Для Каждого ТекущийВычет Из СписокОбъектовНедвижимости Цикл
		Элементы.ОбъектНедвижимости.СписокВыбора.Добавить(ТекущийВычет.Значение, ТекущийВычет.Представление);
	КонецЦикла;
	...
	ВычетПрименяетсяВпервые = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеФормы,
		"ВычетПрименяетсяВпервые", Ложь);
	
	ВычетПрошлыхЛетСтоимость = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеФормы, "ВычетПрошлыхЛетСтоимость", 0);
	ВычетПрошлыхЛетПроценты = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеФормы, "ВычетПрошлыхЛетПроценты", 0);
...
Процедура ДатаАктаПередачиПравПриИзменении(Элемент)
	...
	ПриИзмененииНачалаПримененияВычета();
...
Процедура НачалоПримененияВычетаРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(НачалоПримененияВычета) Тогда
		...
		НачалоПримененияВычета = ДобавитьМесяц(НачалоГода(Период), Направление * 12);
		ПриИзмененииНачалаПримененияВычета();
	КонецЕсли;
...
Процедура ЗаполнитьОбъектНедвижимостиИзСохраненныхДанных(ДанныеФормы)
	...
	Если ДанныеФормы.Свойство("ОбъектНедвижимости", СохраненныйОбъектНедвижимости)
		...
		КодыОбъектаНедвижимости = ПомощникЗаполнения3НДФЛ.КодыНаименованияОбъектаНедвижимости(ВыбраннаяФорма);
...
Функция ПолНалогоплательщика(Налогоплательщик)
	...
	Если ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(КадровыеДанныеФизЛиц[0].Отчество) Тогда
			Результат = ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.ПустаяСсылка");
			ОрганизацииФормыКлиентСервер.ОпределитьПолПоОтчеству(Результат, КадровыеДанныеФизЛиц[0].Отчество);
		КонецЕсли;
	КонецЕсли;
...
Функция СтруктураДанныхДекларации()
	...
	СтруктураДанныхДекларации.ФормаСобственности = Формат(ФормаСобственности, "ЧГ=0;");
...
Функция НовыйСтруктураДанныхДекларации()
	...
	СтруктураДанныхДекларации.Вставить("ФормаСобственности", "");
...
Функция РассчитатьСуммуВозможногоВычета(Форма)
	...
	Если Форма.ВычетПрименяетсяВпервые Тогда
		...
		ДоступныйВычетПроценты  = Мин(ПределыВычетов.НаПриобретениеНедвижимостиПроценты, Форма.СуммаПроцентовЗаВсеГоды);
	Иначе
		...
		Если Форма.КредитОформленДо2014Года Тогда
			ДоступныйВычетПроценты = Форма.СуммаПроцентовЗаВсеГоды; 
		Иначе
			ДоступныйВычетПроценты = Мин(Форма.СуммаПроцентовЗаВсеГоды, ПределыВычетов.НаПриобретениеНедвижимостиПроценты);
		КонецЕсли;
	КонецЕсли;
	
	СуммаВычета = Мин(Форма.Стоимость + Форма.СуммаРасходов, ДоступныйВычетСтоимость);
	Если Форма.ИспользоватьВычетПоПроцентам Тогда
		СуммаВычета = СуммаВычета + Макс(0, ДоступныйВычетПроценты - Форма.ВычетПрошлыхЛетПроценты);
	КонецЕсли;
...
Функция ПределыВычетов(ВыбраннаяФорма)
	Возврат ПомощникЗаполнения3НДФЛ.ПределыВычетов(ВыбраннаяФорма);
КонецФункции

&НаСервереБезКонтекста
Функция КодОбъектаНедвижимости(Знач ОбъектНедвижимости, Знач ВыбраннаяФорма)
	
	Возврат ПомощникЗаполнения3НДФЛ.КодОбъектаНедвижимости(ОбъектНедвижимости, ВыбраннаяФорма);
...
Функция КодСпособаПриобретенияЖилогоДома(Знач СпособПриобретения, Знач ВыбраннаяФорма)
	...
	СпособыПриобретенияНедвижимости = ПомощникЗаполнения3НДФЛ.СпособыПриобретенияНедвижимости(ВыбраннаяФорма);
...
Функция КодыПризнакаНалогоплательщика(ВыбраннаяФорма)
	Возврат ПомощникЗаполнения3НДФЛ.КодыПризнакаНалогоплательщикаДляИмущественногоВычета(ВыбраннаяФорма);
КонецФункции
...
&НаСервереБезКонтекста
Функция ОбъектыНедвижимости(Период)
	
	УчитыватьЖилойДомСУчастком = (Период < '20240101');
	УчитыватьДолюДома = (Период >= '20250101');
	
	Результат = Новый СписокЗначений;
	Результат.Добавить("Квартира", "Квартира");
	Результат.Добавить("Комната", "Комната");
	Результат.Добавить("ЖилойДом", "Жилой дом");
	
	Если УчитыватьДолюДома Тогда
		Результат.Добавить("ДоляВДоме", "Доля в доме");
	КонецЕсли;
	
	Если УчитыватьЖилойДомСУчастком Тогда
		Результат.Добавить("ДоляВКвартире", "Доля в доме, квартире, комнате");
	ИначеЕсли УчитыватьДолюДома Тогда
		Результат.Добавить("ДоляВКвартире", "Доля в квартире, комнате, земельном участке");
	Иначе
		Результат.Добавить("ДоляВКвартире", "Доля в доме, квартире, комнате, земельном участке");
	КонецЕсли;
	
	Результат.Добавить("ЗемельныйУчастокПодСтроительство", "Земельный участок под строительство жилого дома");
	Результат.Добавить("ЗемельныйУчасток", "Земельный участок с расположенным на нем домом");
	
	Если УчитыватьЖилойДомСУчастком Тогда
		Результат.Добавить("ДоляВЗемельномУчастке", "Доля в земельном участке");
		Результат.Добавить("ЖилойДомСЗемельнымУчастком", "Дом с земельным участком");
	КонецЕсли;
...
&НаКлиенте
Процедура ПриИзмененииНачалаПримененияВычета()
