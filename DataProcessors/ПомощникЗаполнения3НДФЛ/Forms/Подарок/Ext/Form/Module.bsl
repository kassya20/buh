...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПомощникЗаполнения3НДФЛ.МожноНеУказыватьНаименованиеИсточникаДохода(ВыбраннаяФорма) Тогда
		Элементы.ВидКонтрагента.СписокВыбора.Добавить(Перечисления.ЮридическоеФизическоеЛицо.ПустаяСсылка(),
			НСтр("ru = 'Неизвестен'"));
	КонецЕсли;
...
Функция СохранитьДанныеДекларации()
	...
	Если ПомощникЗаполнения3НДФЛКлиентСервер.УпрощеннаяФормаДекларации(ВыбраннаяФорма) Тогда
		ДанныеДекларации.Вставить("КодВидаДохода", КодыВидовДоходовРФ.ПродажаИмущества);
	ИначеЕсли ПомощникЗаполнения3НДФЛКлиентСервер.ВычетыРаспределяютсяПоДоходам(ВыбраннаяФорма) Тогда
		ДанныеДекларации.Вставить("КодВидаДохода", КодыВидовДоходовРФ.ПодарокСтраховаяВыплата);
	Иначе
