...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ (ЗначениеЗаполнено(ОстатокУбыткаЦБ) Или ЗначениеЗаполнено(ОстатокУбыткаПФИ)) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, , "ОстатокУбыткаЦБ", , Отказ);
	КонецЕсли;
...
Функция ГодНачалаДействияВычета(ВидИнвестиционногоСчета)
	...
	Возврат ПомощникЗаполнения3НДФЛ.ГодНачалаДействияВычетаУбыткиПрошлыхЛетПоЦеннымБумагам(ВидИнвестиционногоСчета);
