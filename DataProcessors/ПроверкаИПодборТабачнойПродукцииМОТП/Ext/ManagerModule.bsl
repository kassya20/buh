				...
				Для Каждого Строка Из ДанныеНоменклатурыПоДаннымУПД.СтрокиТекущейТоварнойГруппы Цикл
					Если ДобавленныеШтрихкоды[Строка.Штрихкод] = Неопределено Тогда
						ДанныеШтрихкодовСписок.Добавить(
							Новый Структура("Штрихкод, Количество, ЗагруженИзЭлектронногоДокумента", Строка.Штрихкод, 1, Истина));
...
Процедура ЗафиксироватьРезультатПроверкиИПодбора(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		РазрешенОборотНемаркированныхОстатков = ОбщегоНазначенияИСМППовтИсп.РазрешенОборотНемаркируемыхОстатковПродукции(ПараметрыОкончанияПроверки.ВидПродукцииИС);
		...
		Для Каждого СтрокаДерева Из Параметры.ПодобраннаяМаркируемаяПродукция Цикл
			...
			Если РазрешенОборотНемаркированныхОстатков И 
				(СтрокаДерева.ИндексКодаМаркировки = 6 Или СтрокаДерева.ИндексКодаМаркировки = 7) Тогда
		...
		НастройкаКонтроляПоВидуПродукции = ПроверкаИПодборПродукцииИСМП.СтруктураДанныхКонтроляПоРазделеннымДатам(ПараметрыСканирования, Параметры.ВидМаркируемойПродукции);
		ПараметрыОкончанияПроверки.ДанныеКонтроляВидаПродукцииПоРазделеннымДатам = НастройкаКонтроляПоВидуПродукции;
...
Процедура ДополнитьКолонкамиРозничнойПродажи(Таблица)
	...
	Таблица.Колонки.Добавить("РазрешительныйРежимМинимальнаяРозничнаяЦена",  ОбщегоНазначения.ОписаниеТипаЧисло(15, 3, ДопустимыйЗнак.Неотрицательный));
...
Процедура ЗаполнитьШтрихкодыВерхнегоУровняИдентификаторамиЗапросовГИСМТ(СтрокиДерева, ТаблицаШтрихкодов)
	...
	Для Каждого Строка Из СтрокиДерева Цикл
		...
		Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			СтрокаТаблицы.РазрешительныйРежимМинимальнаяРозничнаяЦена  = Строка.РазрешительныйРежимМинимальнаяРозничнаяЦена;
...
Процедура ДобавитьСтрокуДереваМаркированнойПродукцииПриЗагрузке(СтрокаИсточника, КоллекцияСтрокПриемника, ДанныеДокумента, ПараметрыСканирования)
	...
	НоваяСтрока.РазрешительныйРежимМинимальнаяРозничнаяЦена = СтрокаИсточника.МинимальнаяРозничнаяЦена;
...
Функция ДеревоМаркированнойПродукции()
	...
	ДеревоМаркированнойПродукции.Колонки.Добавить("РазрешительныйРежимМинимальнаяРозничнаяЦена",  ТипЧисло);
