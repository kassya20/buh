...
Процедура ЗаполнитьКодыМаркировки(Команда)
	...
	Если ПроверкаИПодборПродукцииИСМПКлиентСервер.ЭтоДокументРеализации(ПроверяемыйДокумент)
		Или ИнтеграцияИСМПКлиентСервер.ОперацииУточненияСведенийОКодахМаркировки().Найти(ВидОперацииИСМП) <> Неопределено  Тогда
...
Функция ОбновитьСтатусыКодовМаркировки(СообщатьОбОшибке = Истина, ПараметрыПроверки = Неопределено)
	...
	Для Каждого КлючИЗначение Из СоответствиеШтрихкодовСтрокДерева Цикл
		...
		СтрокаДерева.РазрешительныйРежимМинимальнаяРозничнаяЦена  = СтатусКодаМаркировки.МинимальнаяРозничнаяЦена;
...
Процедура ДобавитьИЗаполнитьКолонкиПриВосстановленииДерева(ДеревоМаркированнойПродукцииОбъект, СоответствиеПараметров)
	...
	ДополнитьОписаниеКолонок(Коллекция, "РазрешительныйРежимМинимальнаяРозничнаяЦена",  ТипЧисло);
...
Процедура ДобавитьНовуюТабачнуюПродукциюВДерево(ДанныеШтрихкода, ДобавлятьВУпаковку = Неопределено)
	...
	НоваяСтрока.РазрешительныйРежимМинимальнаяРозничнаяЦена = ДанныеШтрихкода.МинимальнаяРозничнаяЦена;
	...
	Если СтрНачинаетсяС(РезультатВопроса, "ПоЗаказамНаЭмиссию") Тогда
		...
		Если КодыМаркировки.Количество() = 0 Тогда
			...
			ОповещениеОЗавершении = Новый ОписаниеОповещения(
				"Подключаемый_ПослеВыбораОснованийЗаполененияКодаМаркировки",
				ЭтотОбъект,
...
&НаКлиенте
Процедура Подключаемый_ПослеВыбораОснованийЗаполененияКодаМаркировки(ВыбранныеЗаказы, РежимПолученияКодов) Экспорт
