...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("АктуализироватьСБлокировкойПараллельности", Истина);
	Методы.Вставить("ПроверитьАктуальность", Истина);
	Методы.Вставить("ВыполнитьЗакрытиеМесяца", Истина);
	Методы.Вставить("ВыполнитьРегламентнуюОперацию", Истина);
	Методы.Вставить("ВыполнитьПереносГраницыАктуальности", Истина);
	Методы.Вставить("АктуализироватьВФонеДляРасчетаНалога", Истина);
	Методы.Вставить("АктуализироватьВФоне", Истина);
...
Функция ВыполнитьРегламентнуюОперацию(Период, Организация, ВидОперации, Ссылка, Состояние, ВыполненныеПроверки) Экспорт
	...
	Результат.Вставить("ЭтоИнтерактивныйЗапуск", ВыполненныеПроверки.ЭтоИнтерактивныйЗапуск);
	...
	Если ЗакрытиеМесяцаКлиентСервер.ИспользуетсяДокументРегламентнаяОперация(ВидОперации) Тогда
		...
		Если Не ВыполненныеПроверки.СтатусСледующих Тогда
			// Пометить как неактуальные следующие документы (сдвинуть границу актуальности).
			Объект.ДополнительныеСвойства.Вставить("СброситьСостояниеСледующих", Истина);
		КонецЕсли;

		Если Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено
		 Или Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.Выполняется Тогда
			
			Результат.Отказ = Не Объект.ВыполнитьОперацию();
			Если Не Результат.Отказ И Не ВыполненныеПроверки.СтатусСледующих Тогда
				// Последовательность сброшена, поэтому для оптимизации в дальнейшем можно уже последовательность не сбрасывать.
		...
		Иначе
			...
			Иначе
				...
				Попытка
					...
					Если ВыполненныеПроверки.ПроверятьАктивностьПользователей Тогда
						// Требуется записать объект со статусом группового перепроведения
						Объект.ДополнительныеСвойства.Вставить("ГрупповоеПерепроведение", Истина);
					КонецЕсли;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыРегламентныхОпераций.НачислениеРезервовПоОплатеТруда
		И ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты")
		И УчетЗарплаты.ИспользуетсяПодсистемаУчетаЗарплатыИКадров()
		И (УчетЗарплаты.ВедетсяУчетРезервовОтпусков(Организация, НачалоМесяца(Период))
			Или УчетЗарплаты.ВедетсяУчетРезервовПремий(Организация, НачалоМесяца(Период))) Тогда
		
		Если Состояние <> Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено
		   И Состояние <> Перечисления.ВидыСостоянийРегламентныхОпераций.Выполняется Тогда
			// Начисление резервов по оплате труда "пропускать" не умеем
		Иначе
			
			ЕстьДокументНачисленияРезервовПоОплатеТруда = УчетЗарплаты.ЕстьДокументНачисленияРезервовПоОплатеТруда(НачалоМесяца(Период), Организация);
			Если НЕ ЕстьДокументНачисленияРезервовПоОплатеТруда Тогда
				Попытка
					УчетЗарплаты.СформироватьДокументыНачисленияРезервовПоОплатеТруда(НачалоМесяца(Период), Организация);
					ЕстьДокументНачисленияРезервовПоОплатеТруда = УчетЗарплаты.ЕстьДокументНачисленияРезервовПоОплатеТруда(НачалоМесяца(Период), Организация);
					Если ЕстьДокументНачисленияРезервовПоОплатеТруда Тогда
						Результат.Вставить("Состояние", Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено);
					Иначе
						Результат.Вставить("Состояние", Перечисления.ВидыСостоянийРегламентныхОпераций.НеВыполнено);
						Результат.Вставить("Отказ",     Истина);
...
Функция ВыполнитьЗакрытиеМесяца(Период, Организация, СтатусЗакрытияМесяца, Состояние, ФинишнаяОперация = Неопределено,
	ТолькоОперацииОбособленногоПодразделения = Ложь, ВыполненныеПроверки = Неопределено, СообщатьПрогрессВыполнения = Истина) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Выполнено", Истина);
	Результат.Вставить("Операции",  СтатусЗакрытияМесяца.СкопироватьКолонки("Организация, ВидОперации, Состояние, Ссылка"));
	Результат.Вставить("ВыполнениеОстановлено", Ложь);
	
	Если ВыполненныеПроверки = Неопределено Тогда
		Результат.Вставить("ЭтоИнтерактивныйЗапуск", Ложь);
	Иначе
		Результат.Вставить("ЭтоИнтерактивныйЗапуск", ВыполненныеПроверки.ЭтоИнтерактивныйЗапуск);
	КонецЕсли;
	...
	ПроверкаАктивности = ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыПроверкиАктивностиСеансов();
	Если ВыполненныеПроверки <> Неопределено Тогда
		ПроверкаАктивности.ЭтоИнтерактивныйЗапуск           = ВыполненныеПроверки.ЭтоИнтерактивныйЗапуск;
		ПроверкаАктивности.ПроверятьАктивностьПользователей = ВыполненныеПроверки.ПроверятьАктивностьПользователей;
		ПроверкаАктивности.ИнтервалПроверкиВСекундах        = ВыполненныеПроверки.ИнтервалПроверкиВСекундах;
	КонецЕсли;
	...
	Отбор = Новый Структура("ПоОрганизацииВЦелом", Истина);
	ОперацииПоОрганизации = СтатусЗакрытияМесяца.НайтиСтроки(Отбор);
	
	Для Каждого СтрокаТаблицы Из СтатусЗакрытияМесяца Цикл
		
		// Проверка на необходимость остановить выполнение.
		ТребуетсяЗавершение = ЗакрытиеМесяца.ВыполнениеОперацииОстановленоПользователем(Организация, ПроверкаАктивности);
		Если ТребуетсяЗавершение Тогда
			Результат.ВыполнениеОстановлено = Истина;
			Прервать; // Пользователи записывают документы последовательности, или нажали кнопку отмены.
		КонецЕсли;
		...
		Если ИндексСтартовойОперации = Неопределено 
		 ...
		 И СтрокаТаблицы.Организация = Организация Тогда
			...
			// Проверим состояние закрытия месяца по организации в целом.
			Если СтрокаТаблицы.ПоОрганизацииВЦелом = Ложь Тогда
				Для Каждого СтрокаПоОрганизации Из ОперацииПоОрганизации Цикл
					Если СтрокаТаблицы.НомерГруппы > СтрокаПоОрганизации.НомерГруппы
					 И ЗакрытиеМесяцаКлиентСервер.ОперацияВРаботе(СтрокаПоОрганизации) Тогда
					 	// Сначала необходимо выполнить операции предыдущей группы по организации в целом.
					 	ИндексСтартовойОперации = Неопределено;
						ИндексФинишнойОперации  = Неопределено;
						Прервать;
					КонецЕсли;
...
Функция ВыполнитьПереносГраницыАктуальности(ПараметрыПереноса) Экспорт
	...
	Результат.Вставить("ЭтоИнтерактивныйЗапуск",  ПараметрыПереноса.ЭтоИнтерактивныйЗапуск);
	...
	ПараметрыСбросаАктуальностиОстатков = Новый Структура;
	ПараметрыСбросаАктуальностиОстатков.Вставить("Дата", 	ПараметрыПереноса.Период);
	ПараметрыСбросаАктуальностиОстатков.Вставить("Ссылка",  Неопределено);
	УчетНДСРаздельныйФормированиеНачальныхОстатков.СброситьАктуальностьОстатков(Организация,
		ПараметрыСбросаАктуальностиОстатков);
	...
	Попытка
		...
		Пока ДатаНачалаПереносаГраницы <= ДатаОкончанияЗакрытияМесяца Цикл
			...
			Для каждого РегОперация Из СтатусЗакрытияМесяца Цикл
				...
				ИначеЕсли ЗначениеЗаполнено(РегОперация.Ссылка)
				   И РегОперация.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.Выполняется Тогда
					
					ОперацияОбъект = РегОперация.Ссылка.ПолучитьОбъект();
					Попытка
						
						Успешно = ОперацияОбъект.ВосстановитьОперацию(Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено);
						Если Не Успешно Тогда
							ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось восстановить после сбоя операцию, имеющую движения с отключенной активностью
												|%1'"), РегОперация.ВидОперации);
						КонецЕсли;
	...
	Исключение
		...
		ТекстПричины = СтрШаблон(НСтр("ru = 'Не удалось отметить как актуальные регламентные операции до %1.'"),
			НРег(Формат(Выборка.Период, "ДФ='ММММ гггг'")));
...
Функция ПроверитьАктуальность(ПараметрыПроверки) Экспорт
	...
	Если ТребуетсяПолнаяАктуализация ИЛИ НЕ ИспользуетсяОтложенноеПроведение Тогда
		...
		// Если	подразделение было снято с учета, тогда требуется выполнять актуализацию только до конца года с даты снятия учета.
		Если БухгалтерскийУчетПереопределяемый.ОбособленноеПодразделение(Организация) Тогда
			ДатаСнятияСУчета = Справочники.Организации.ДатаСнятияСУчета(Организация, ПределАктуализацииЗакрытияМесяца);
			Если ЗначениеЗаполнено(ДатаСнятияСУчета) И ПределАктуализацииЗакрытияМесяца > КонецГода(ДатаСнятияСУчета) Тогда
				ПределАктуализацииЗакрытияМесяца = КонецГода(ДатаСнятияСУчета);
			КонецЕсли;
		КонецЕсли;
		...
		Если МоментНачалаПоследовательности <> Неопределено Тогда // В базе есть проведенные документы.
			...
			Пока ПериодРегистрации >= НачалоМесяца(МоментНачалаПоследовательности.Дата) Цикл
				...
				Выполнено = Выполнено(Организация, ПериодРегистрации);
				// Для ежеквартальных операций недостаточно проверить наличие невыполненных операций в прошлом месяце.
				// Может сложиться такая ситуация, что в прошлом месяце все операции выполнены, а в прошлом квартале
				// выполнены не все необходимые операции - к примеру, была сдвинута граница актуальности на несколько 
				// кварталов назад. Поэтому, необходимо проверять наличие невыполненных операций не только в прошлом 
				// месяце, но и в прошлом квартале.
				// Проверяем, чтобы не было невыполненных операций в прошлом месяце - для ежемесячных операций,
				// и в конце прошлого квартала - для ежеквартальных операций.
				ПериодПрошлыйКвартал = НачалоМесяца(НачалоКвартала(ПериодРегистрации) - 1);
				Если Выполнено 
					И ПериодПрошлыйКвартал >= ПервыйНезакрытыйПериод Тогда
					Выполнено = Выполнено(Организация, ПериодПрошлыйКвартал);
					Если Не Выполнено Тогда
						ПериодРегистрации = ПериодПрошлыйКвартал;
					КонецЕсли;
				КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДатаАктуальности) Тогда // Требуется актуализация итоговых данных.
		...
		Если ПараметрыПроверки.АктуализацияДляРасчетаНалога Тогда
			Результат.ТребуетсяАктуализация = Не ЗакрытиеМесяца.ДанныеДляРасчетаНалогаАктуальны(ПараметрыПроверки);
		Иначе
...
// Выполняет перепроведение документов и выполнение регламентных операций. Проверяет перед запуском, что нет ранее запущенной актуализации.
//
// Параметры:
...
//
Функция АктуализироватьСБлокировкойПараллельности(ПараметрыЗадания) Экспорт
	
	РезультатАктуализации = ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации();
	РезультатАктуализации.ПолнаяАктуализация = Истина;
	
	Если ЗначениеЗаполнено(ПараметрыЗадания.ИдентификаторЗадания)
	   И ЗавершеноОжиданиеРанееЗапущенногоЗадания(ПараметрыЗадания, РезультатАктуализации) Тогда
		// В другом сеансе было запущено фоновое задание. В этом сеансе просто подождали его завершения.
	...
	ЕстьОбособленные = Справочники.Организации.ЕстьОбособленныеПодразделения(ПараметрыЗадания.Организация);
	Если ЕстьОбособленные Тогда
		КлючеваяОперация = "АктуализацияДанныхВыполненаАктуализацияПоОбособленным";
		РезультатАктуализации = АктуализироватьПоОрганизацииСОбособленными(ПараметрыЗадания);
	Иначе
		КлючеваяОперация = "АктуализацияДанныхВыполненаАктуализацияЗаПериод";
...
Процедура АктуализироватьВФоне(ПараметрыЗадания, АдресХранилища) Экспорт
	...
	РезультатАктуализации = АктуализироватьСБлокировкойПараллельности(ПараметрыЗадания);
...
Функция МожноРедактироватьОперациюНДС(Период, Организация) Экспорт
	...
	Возврат НЕ ДатыЗапретаИзмененияБП.ЗапрещеноРедактированиеПериода(Период, Организация);
	...
	Если Не ЗначениеЗаполнено(ПараметрыОтчета)
	 Или ТипЗнч(ПараметрыОтчета[0]) = Тип("ДокументСсылка.РегламентнаяОперация") Тогда // отчет для основной формы
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ВыполненныеОперации", 
			НСтр("ru = 'Отчет о выполнении операций за месяц'"), 
...
Функция ОтчетОВыполненииОпераций(Параметры, ОбъектыПечати)
	...
	ОбластьМакета.Параметры.Организация    = БухгалтерскиеОтчетыВызовСервера.ПолучитьТекстОрганизация(Организация,
		ВсеОбособленныеПодразделения, Период);
...
Функция ОперацияДолжнаБытьВыполнена(Операция, ФинишнаяОперация, Организация, ТолькоОперацииОбособленногоПодразделения, ФинишнаяГруппа = Неопределено)
	...
	Если Не Операция.Требуется
	   И Операция.Состояние <> Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено
	   И Операция.Состояние <> Перечисления.ВидыСостоянийРегламентныхОпераций.Выполняется Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ПроверкиВозможностиВыполненияОперации() Экспорт
	...
	ВыполненныеПроверки.Вставить("СтатусСледующих",                  Ложь);
	ВыполненныеПроверки.Вставить("ПроверятьАктивностьПользователей", Ложь);
	ВыполненныеПроверки.Вставить("ИнтервалПроверкиВСекундах",        5);
	ВыполненныеПроверки.Вставить("ЭтоИнтерактивныйЗапуск",           Ложь);
...
Функция АктуализироватьЗаПериод(ПараметрыЗадания)
	...
	РезультатАктуализации = ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации();
	РезультатАктуализации.Организация = Организация;
	РезультатАктуализации.ПолнаяАктуализация = ТребуетсяПолнаяАктуализация Или НЕ ИспользуетсяОтложенноеПроведение;
	РезультатАктуализации.ЭтоИнтерактивныйЗапуск = ПараметрыЗадания.ЭтоИнтерактивныйЗапуск;
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось заблокировать данные по организации %1 для актуализации.
						   |Возможные причины: Выполняется групповое перепроведение, актуализация, закрытие месяца " 
						 + "(в т.ч. закрытие месяца по расписанию) в каком-либо отчете/обработке, или в другом сеансе.'"),
				Организация);
	...
	Если ИнтеграцияСБанкамиПовтИсп.ИнтеграцияОрганизацииВключена(Организация) Тогда
		Если Не ДождалисьВозможностиБыстройАктуализацииИнтеграцииСБанком(Организация, УникальныйИдентификаторФормы) Тогда
			СообщенияПользователю = Новый Массив;
			СообщенияПользователю.Добавить(
				НСтр("ru='Выполняется актуализация данных Банком. Попробуйте выполнить операцию позже.'"));
			РезультатАктуализации.Выполнено = Ложь;
			РезультатАктуализации.ВывестиИнформациюУведомлений = Истина;
			РезультатАктуализации.Вставить("СообщенияПользователю", СообщенияПользователю);
			Возврат РезультатАктуализации;
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыЗадания.ДатаНачалаАктуализации) Тогда // дата начала актуализации указана явным образом
		
		ДатаАктуальности = ПараметрыЗадания.ДатаНачалаАктуализации;
	...
	Иначе
		...
		// Перевыполнение регламентной операции в предыдущем месяце должно приводить к перепроведению документов в следующем месяце.
		ДатаНачалаПерепроведения = Мин(ДатаНачалаПерепроведения, НачалоМесяца(ДобавитьМесяц(ДатаАктуальности, 1)));
	...
	ВыполненныеПроверки = ПроверкиВозможностиВыполненияОперации();
	ВыполненныеПроверки.ЭтоИнтерактивныйЗапуск           = ПараметрыЗадания.ЭтоИнтерактивныйЗапуск;
	ВыполненныеПроверки.ПроверятьАктивностьПользователей = ПараметрыЗадания.ПроверятьАктивностьПользователей;
	ВыполненныеПроверки.ИнтервалПроверкиВСекундах        = ПараметрыЗадания.ИнтервалПроверкиВСекундах;
	...
	Пока ДатаАктуальности <= ПределАктуализации Цикл
		...
		Если КонецМесяца(ДатаАктуальности) >= ДатаНачалаПерепроведения Тогда
			...
			ПараметрыПерепроведения.ОчищатьАдресСОшибками            = ПараметрыЗадания.ОчищатьАдресСОшибками;
			ПараметрыПерепроведения.ЭтоИнтерактивныйЗапуск           = ПараметрыЗадания.ЭтоИнтерактивныйЗапуск;
			ПараметрыПерепроведения.ПроверятьАктивностьПользователей = ПараметрыЗадания.ПроверятьАктивностьПользователей;
			ПараметрыПерепроведения.ИнтервалПроверкиВСекундах        = ПараметрыЗадания.ИнтервалПроверкиВСекундах;
			
			РезультатОперации = Обработки.ГрупповоеПерепроведениеДокументов.ВыполнитьПерепроведение(ПараметрыПерепроведения);
			РезультатАктуализации.ПериодСОшибкойВыполнения = РезультатОперации.ПериодСОшибкойВыполнения;
			РезультатАктуализации.ВыполнениеОстановлено    = РезультатОперации.ВыполнениеОстановлено;
			...
			Если Не РезультатАктуализации.Выполнено Или РезультатАктуализации.ВыполнениеОстановлено Тогда
				
				Если ЭтоЗакрытиеМесяца Тогда
					
					ПоследовательностьВосстановлена = РезультатАктуализации.Выполнено;
					РезультатАктуализации.Вставить("ПоследовательностьВосстановлена", ПоследовательностьВосстановлена);
					РезультатАктуализации.Вставить("АдресХранилищаСОшибками", РезультатОперации.АдресХранилищаСОшибками);
		...
		Если ДатаАктуальности <= ПределАктуализацииЗакрытияМесяца Тогда
			...
			Если РезультатОперации.ВыполнениеОстановлено Тогда
				РезультатАктуализации.ВыполнениеОстановлено = Истина;
				РезультатАктуализации.ПериодСОшибкойВыполнения = ДатаАктуальности;
...
Процедура АктуализироватьВФонеДляРасчетаНалога(ПараметрыЗадания, АдресХранилища) Экспорт
	...
	Если ИнтеграцияСБанкамиПовтИсп.ИнтеграцияОрганизацииВключена(Организация) Тогда
		...
		Если Не ДождалисьВозможностиБыстройАктуализацииИнтеграцииСБанком(Организация, ПараметрыЗадания.УникальныйИдентификаторФормы) Тогда
			СообщенияПользователю = Новый Массив;
			СообщенияПользователю.Добавить(
				НСтр("ru='Выполняется актуализация данных Банком. Попробуйте выполнить операцию позже.'"));
			РезультатАктуализации.Выполнено = Ложь;
	...
	ОперацииДляРасчета = ЗакрытиеМесяца.ОперацииДляРасчетаНалога();
	
	РезультатАктуализации = АктуализироватьРегламентныеОперации(ПараметрыЗадания, ОперацииДляРасчета);
	
	Если Не ЗначениеЗаполнено(РезультатАктуализации) Тогда
		СообщенияПользователю.Добавить(НСтр("ru='Не удалось актуализировать регламентные операции.'"));
		РезультатАктуализации = ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации();
		РезультатАктуализации.Выполнено = Ложь;
		РезультатАктуализации.ВывестиИнформациюУведомлений = Истина;
		РезультатАктуализации.Вставить("СообщенияПользователю", СообщенияПользователю);
...
Функция АктуализироватьПоОрганизацииСОбособленными(ПараметрыЗадания)
	
	ГоловнаяОрганизация = ОбщегоНазначенияБПВызовСервераПовтИсп.ГоловнаяОрганизация(ПараметрыЗадания.Организация);
	
	ПараметрыПодразделения = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыЗадания);
	ПараметрыГоловная      = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыЗадания);
	ПараметрыГоловная.Организация = ГоловнаяОрганизация;
	
	// Определим минимальную дату актуальности, с которой и начнем выполнение
	ПараметрыПроверки = ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыПроверкиАктуальности();
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ПараметрыГоловная);
	ПараметрыПроверки.ТребуетсяПолнаяАктуализация = Истина;
	
	Если ЗначениеЗаполнено(ПараметрыЗадания.ДатаНачалаАктуализации) Тогда
		ОбособленныеПодразделения = ОбособленныеПодразделенияДляАктуализации(ГоловнаяОрганизация, ПараметрыЗадания.ДатаНачалаАктуализации);
	Иначе
		РезультатПроверки = ПроверитьАктуальность(ПараметрыПроверки);
		Если Не РезультатПроверки.ТребуетсяАктуализация Тогда
			// Пока выполнялось текущее задание в фоне, где-то в другом месте уже выполнили актуализацию.
			РезультатАктуализации = ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации();
			РезультатАктуализации.Выполнено = Истина;
			РезультатАктуализации.ПолнаяАктуализация = Истина;
			РезультатАктуализации.Вставить("ПределАктуализации", КонецМесяца(ПараметрыЗадания.Период));
			Возврат РезультатАктуализации;
		КонецЕсли;
		// Определим дату актуальности ГО, и получим на эту дату список подразделений не снятых с учета.
		ОбособленныеПодразделения = ОбособленныеПодразделенияДляАктуализации(ГоловнаяОрганизация, РезультатПроверки.ДатаАктуальности);
	КонецЕсли;
	
	ДатыАктуальности = ПолучитьДатыАктуальностиПоОбособленнымПодразделениям(ПараметрыЗадания, ОбособленныеПодразделения);
	
	НарушенаПоследовательностьВГоловной  =
		ЗначениеЗаполнено(ДатыАктуальности.ДатаАктуальностиПоОрганизации)
		И Не ЗначениеЗаполнено(ДатыАктуальности.ДатаАктуальностиПоПодразделениям);
	
	ТребуетсяВыполнениеОперацийВГоловной =
		ЗначениеЗаполнено(ДатыАктуальности.ДатаАктуальностиПоОрганизации)
		И ДатыАктуальности.ДатаАктуальностиПоОрганизации <= ДатыАктуальности.ДатаАктуальностиПоПодразделениям;
	
	Если НарушенаПоследовательностьВГоловной Или ТребуетсяВыполнениеОперацийВГоловной Тогда
		// Если в подразделениях дата актуальности будет больше, чем в головной организации, то
		// выполнение в подразделениях завершится с ошибкой. Для этого выполним группу 0-1
		// в головной организации, чтобы даты актуальности в подразделениях были сброшены,
		// а группы 0-1 в ГО были выполнены, если это требует состояние учета.
		ПараметрыГоловная.Период = КонецМесяца(ДатыАктуальности.ДатаАктуальностиПоОрганизации);
		РезультатАктуализации = АктуализироватьЗаПериод(ПараметрыГоловная);
		Если РезультатАктуализации.ВыполнениеОстановлено Тогда
			Возврат РезультатАктуализации;
		КонецЕсли;

		ДатаАктуальности = ДатыАктуальности.ДатаАктуальностиПоОрганизации;
	Иначе
		ДатаАктуальности = ДатыАктуальности.ДатаАктуальностиПоПодразделениям;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДатаАктуальности) Тогда
		
		// Для успешного выполнения требуется запускать закрытие месяца по определенному порядку:
		// Подразделения - группы 0-1, ГО - группы 0-2, подразделения - группа 3
		// ГО - группа 3-4, подразделения - группа 5, ГО - группа 5-6
		// и в исключительных случаях после выполнения группы 6 в ГО, требуется повторно выполнить группу 5 в подразделениях.
		ПределАктуализации = КонецМесяца(ПараметрыЗадания.Период);
		
		Пока ДатаАктуальности <= ПределАктуализации Цикл
			
			ПараметрыГоловная.Период      = КонецМесяца(ДатаАктуальности);
			ПараметрыПодразделения.Период = КонецМесяца(ДатаАктуальности);
			
			// Получим даты актуальности на следующий период, который был увеличен в цикле.
			ДатыАктуальности = ПолучитьДатыАктуальностиПоОбособленнымПодразделениям(ПараметрыГоловная, ОбособленныеПодразделения);
			
			// Выполним актуализацию поочередно в подразделениях, затем в ГО, пока все не будет актуализировано за месяц.
			Пока ЗначениеЗаполнено(ДатыАктуальности.ДатаАктуальностиПоОрганизации)
			 Или ЗначениеЗаполнено(ДатыАктуальности.ДатаАктуальностиПоПодразделениям) Цикл
				
				// Выполнение в подразделениях.
				Для Каждого Подразделение Из ОбособленныеПодразделения Цикл
					ПараметрыПодразделения.Организация = Подразделение;
					РезультатАктуализации = АктуализироватьЗаПериод(ПараметрыПодразделения);
					Если НЕ РезультатАктуализации.Выполнено Или РезультатАктуализации.ВыполнениеОстановлено Тогда
						Возврат РезультатАктуализации;
					КонецЕсли;
				КонецЦикла;
				
				// Выполнение в головной организации.
				РезультатАктуализации = АктуализироватьЗаПериод(ПараметрыГоловная);
				Если НЕ РезультатАктуализации.Выполнено Или РезультатАктуализации.ВыполнениеОстановлено Тогда
					Возврат РезультатАктуализации;
				КонецЕсли;
				
				// Снова получим даты актуальности. Если даты актуальности пустые, значит всё было актуализировано за этот месяц.
				ДатыАктуальности = ПолучитьДатыАктуальностиПоОбособленнымПодразделениям(ПараметрыГоловная, ОбособленныеПодразделения);
				
			КонецЦикла;
			
			ДатаАктуальности = КонецМесяца(ДобавитьМесяц(ДатаАктуальности, 1));
			
			// Если актуализация выполняется с прошлого года и переходит на следующий год, то нужно получить
			// новый список подразделений на следующий год, т.к. подразделения могли быть сняты с учета.
			Если ДатаАктуальности = КонецМесяца(НачалоГода(ДатаАктуальности)) Тогда
				ОбособленныеПодразделения = ОбособленныеПодразделенияДляАктуализации(ГоловнаяОрганизация, ДатаАктуальности);
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Закрытие требуется только для головной организации
		РезультатАктуализации = АктуализироватьЗаПериод(ПараметрыГоловная);
	КонецЕсли;
...
Функция ОбособленныеПодразделенияДляАктуализации(ГоловнаяОрганизация, Период)

	Результат = БухгалтерскийУчетПереопределяемый.ВсяОрганизация(ГоловнаяОрганизация, Период);
	Результат.Удалить(Результат.Найти(ГоловнаяОрганизация));
	
	Возврат Результат;
	
КонецФункции

Функция ПолучитьДатыАктуальностиПоОбособленнымПодразделениям(ПараметрыЗадания, ОбособленныеПодразделения)
	
	Результат = Новый Структура;
	Результат.Вставить("ДатаАктуальностиПоОрганизации", '00010101');
	Результат.Вставить("ДатаАктуальностиПоПодразделениям", '00010101');
	
	ПараметрыПроверки = ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыПроверкиАктуальности();
	ЗаполнитьЗначенияСвойств(ПараметрыПроверки, ПараметрыЗадания);
	ПараметрыПроверки.ТребуетсяПолнаяАктуализация = Истина;
	
	ДатаАктуальностиПоОрганизации    = '00010101';
	ДатаАктуальностиПоПодразделениям = '00010101';
	РезультатПроверки = ПроверитьАктуальность(ПараметрыПроверки);
	Если РезультатПроверки.ТребуетсяАктуализация Тогда
		Результат.ДатаАктуальностиПоОрганизации = РезультатПроверки.ДатаАктуальности;
	КонецЕсли;
	
	Для Каждого Подразделение Из ОбособленныеПодразделения Цикл
		ПараметрыПроверки.Организация = Подразделение;
		РезультатПроверки = ПроверитьАктуальность(ПараметрыПроверки);
		Если Не РезультатПроверки.ТребуетсяАктуализация Тогда
			Продолжить;
		КонецЕсли;
		Если ЗначениеЗаполнено(Результат.ДатаАктуальностиПоПодразделениям) Тогда
			Результат.ДатаАктуальностиПоПодразделениям =
				Мин(Результат.ДатаАктуальностиПоПодразделениям, РезультатПроверки.ДатаАктуальности);
		Иначе
			Результат.ДатаАктуальностиПоПодразделениям = РезультатПроверки.ДатаАктуальности;
		КонецЕсли;
	КонецЦикла;
...
Функция ДождалисьВозможностиБыстройАктуализацииИнтеграцииСБанком(Организация, УникальныйИдетификатор)
	Заблокировано = Ложь;
	Для НомерПопытки = 1 По 6 Цикл
		Попытка
			ЗаблокироватьДанныеДляРедактирования(Организация,, УникальныйИдетификатор);
			Заблокировано = Истина;
			Прервать;
		Исключение
			ОбщегоНазначенияБТС.Пауза(НомерПопытки * ИнтеграцияСБанками.ВремяОжиданияАктуализации());
		КонецПопытки
	КонецЦикла;
...
// Выполняет только регламентные операции, необходимые для расчета фиксированнных взносов ИП.
//
// Параметры:
//  ПараметрыЗадания - Структура - см. ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыАктуализации()
//
...
//   Структура - см. ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации()
//
Функция АктуализироватьДляРасчетаФиксированныхВзносов(ПараметрыЗадания) Экспорт
	
	ВремяНачала = ОценкаПроизводительности.НачатьЗамерВремени();
	
	ОперацииДляРасчета = Новый Массив;
	ОперацииДляРасчета.Добавить(Перечисления.ВидыРегламентныхОпераций.РаспределениеРасходовПоВидамДеятельностиДляИП);
	ОперацииДляРасчета.Добавить(Перечисления.ВидыРегламентныхОпераций.РаспределениеРасходовПоВидамДеятельностиДляУСН);
	ОперацииДляРасчета.Добавить(Перечисления.ВидыРегламентныхОпераций.НачислениеСтраховыхВзносовИП);
	
	РезультатАктуализации = АктуализироватьРегламентныеОперации(ПараметрыЗадания, ОперацииДляРасчета);
	
	ОценкаПроизводительности.ЗакончитьЗамерВремени("АктуализацияДанныхВыполненаАктуализацияРасчетаФиксированныхВзносов", ВремяНачала);
	
	Возврат РезультатАктуализации;
	
КонецФункции

// Выполняет только указанные регламентные операции за переданный период актуализации.
//
// Параметры:
//  ПараметрыЗадания - Структура - см. ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыАктуализации()
//  ВидыОпераций - Массив - виды регламентных операций, которые требуется выполнить
//  АдресХранилища - Строка - для возврата результата.
//
Функция АктуализироватьРегламентныеОперации(ПараметрыЗадания, ОперацииДляРасчета)
	...
	// Проверим конфликты параллельного выполнения двух быстрых актуализаций.
	Запрос = Новый Запрос;
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	КОНЕЦПЕРИОДА(РегламентнаяОперация.Дата, МЕСЯЦ) КАК КонецМесяца,
	|	РегламентнаяОперация.ВидОперации КАК ВидОперации,
	|	РегламентнаяОперация.ВидОперации.Порядок КАК ПорядокОперации,
	...
	АктуализируемыйМесяц = КонецМесяца(ГраницаАктуализацииНачало);
	...
	ПараметрыОтменыРегОпераций = Документы.РегламентнаяОперация.НовыеПараметрыОтменыОперации();
	ПараметрыОтменыРегОпераций.УдалитьДвижения = Истина;
	ПараметрыОтменыРегОпераций.СдвигатьМоментНарушенияПоследовательности = Ложь;
	ПараметрыОтменыРегОпераций.ОбновитьГраницуАктуальности = Ложь;
	ПараметрыОтменыРегОпераций.СброситьСостояниеОперации = Истина;
	
	СчетчикМесяцев = 0;
	
	Пока АктуализируемыйМесяц <= ГраницаАктуализацииКонец Цикл
		
		СтатусОпераций = СтатусЗакрытияМесяца(Организация, АктуализируемыйМесяц, ОперацииДляРасчета);
		
		РезультатАктуализации.Выполнено = Истина; // Сбрасывается с прерыванием обработки, если при актуализации месяца возникнут ошибки.
		
		Для Каждого СтрокаОперации Из СтатусОпераций Цикл
			
			Если Не СтрокаОперации.Требуется
				И ЗначениеЗаполнено(СтрокаОперации.Ссылка)
				И Не СтрокаОперации.РучнаяКорректировка Тогда
				ОперацияОбъект = СтрокаОперации.Ссылка.ПолучитьОбъект();
				ОперацияОбъект.ОтменитьОперацию( , , ПараметрыОтменыРегОпераций);
				Продолжить;
			КонецЕсли;
			
			Результат = ВыполнитьРегламентнуюОперацию(
				НачалоМесяца(АктуализируемыйМесяц),
				Организация,
				СтрокаОперации.ВидОперации,
				СтрокаОперации.Ссылка,
				Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено,
				ПараметрыПроверок);
			...
			Если Не РезультатАктуализации.Выполнено Тогда
				РезультатАктуализации.ВывестиИнформациюУведомлений = Истина;
				РезультатАктуализации.Вставить("ВидОперации", СтрокаОперации.ВидОперации);
				РезультатАктуализации.Вставить("ПериодРегистрации", НачалоМесяца(АктуализируемыйМесяц));
				СообщенияПользователю = ПолучитьСообщенияПользователю(Истина);
				РезультатАктуализации.Вставить("СообщенияПользователю", ОбработатьСообщенияПользователю(СообщенияПользователю));
		...
		ДлительныеОперации.СообщитьПрогресс(Окр(СчетчикМесяцев / МесяцевПериода * 100));
		
		Если Не РезультатАктуализации.Выполнено Тогда
			Прервать;
		КонецЕсли;
		
		АктуализируемыйМесяц = КонецМесяца(ДобавитьМесяц(АктуализируемыйМесяц, 1));
...
Функция СтатусЗакрытияМесяца(Организация, Период, ВидыОпераций = Неопределено)
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	// Получаем детальный статус закрытия месяца
	ЗакрытиеМесяца.СоздатьДетальныйСтатусЗакрытияМесяца(МенеджерВременныхТаблиц, Период, Организация, , ВидыОпераций);
...
// Используется для синхронизации с ранее запущенным в другом сеансе заданием актуализации, чтобы не запускать два одновременно.
//
// Параметры:
//  ПараметрыЗадания - Структура - см. ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыАктуализации()
//  РезультатАктуализации - Структура - ЗакрытиеМесяцаКлиентСервер.НовыйРезультатАктуализации()
//
// Возвращаемое значение:
//   Булево - Ложь, если ранее запущенное задание актуализации сейчас уже не выполняется.
//            Истина, если нам нужно было подождать ранее запущенное задание актуализации. Может быть два варианта:
//            * подождали меньше часа и дождались завершения того задания;
//            * прошел час, не дождались, и то задание всё еще выполняется.
//
Функция ЗавершеноОжиданиеРанееЗапущенногоЗадания(ПараметрыЗадания, РезультатАктуализации)
	
	// Если в течение часа не дождались окончания ранее запущенного фонового задания, то завершаем ожидание
	// и из основного сеанса снова запустим проверку актуальности.
	ДопустимыйТаймаут = 3600;
	
	УстановитьПривилегированныйРежим(Истина);
	ВыполняемоеЗаданиеАктуализации = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ПараметрыЗадания.ИдентификаторЗадания);
	Если ВыполняемоеЗаданиеАктуализации = Неопределено Тогда // предыдущее фоновое задание закончилось
		ОписаниеЗаданияАктуализации = ЗакрытиеМесяцаВызовСервера.НайтиФоновоеЗаданиеАктуализацииПоОрганизации(
			ПараметрыЗадания.Организация, ПараметрыЗадания.УникальныйИдентификаторФормы);
		Если ОписаниеЗаданияАктуализации.УникальныйИдентификатор <> Неопределено Тогда
			ВыполняемоеЗаданиеАктуализации = ФоновыеЗадания.НайтиПоУникальномуИдентификатору(ОписаниеЗаданияАктуализации.УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
	Если ВыполняемоеЗаданиеАктуализации = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
		
	ВыполняющиесяЗадания = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ВыполняемоеЗаданиеАктуализации);
	ВыполняющиесяЗадания = ФоновыеЗадания.ОжидатьЗавершенияВыполнения(ВыполняющиесяЗадания, ДопустимыйТаймаут);
	
	РезультатАктуализации.Выполнено = ВыполняющиесяЗадания.Количество() > 0
									И ВыполняющиесяЗадания[0].Состояние <> СостояниеФоновогоЗадания.Активно;
	Если Не РезультатАктуализации.Выполнено Тогда
	
		РезультатАктуализации.СообщенияПользователю.Добавить(НСтр("ru='Актуализация не завершена.'"));
		РезультатАктуализации.ВывестиИнформациюУведомлений = Истина;
