...
Процедура СоздатьПлатежноеПоручение(Команда)
	...
	ИмяТекущейКомандыСозданияДокумента = Команда.Имя;
	
	ПараметрыОперации = РаспознаваниеДокументовКомплектыКлиентСервер.НовыеПараметрыОперации();
	ПараметрыОперации.ТипДокумента = РаспознаваниеДокументовСлужебныйВызовСервера.ИмяДокументаПоТипу(
		"ПлатежноеПоручениеБРД");
...
&НаКлиенте
Процедура СоздатьСчетИПлатежноеПоручение(Команда)
...
Процедура УправлениеФормой()
	...
	Элементы.СоздатьПлатежноеПоручение.Видимость = ДоступноСозданиеПлатежныхПоручений
		И (Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий);
	Элементы.СоздатьСчетИПлатежноеПоручение.Видимость = ДоступноСозданиеПлатежныхПоручений
		И (Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий);
...
Процедура Подключаемый_СозданиеТипового(ПараметрыОперации) Экспорт
	...
	Если ПараметрыОперации.ТипДокумента = "ПлатежноеПоручение" Тогда
		
		ПараметрыЗаполнения = ПолучитьПараметрыОткрытияФормы(ПараметрыОперации.ТипДокумента);
		Если ЗначениеЗаполнено(СозданныйСчетНаОплатуПоставщика) Тогда
			ПараметрыЗаполнения.Вставить("ДокументОснование", СозданныйСчетНаОплатуПоставщика);
		КонецЕсли;
		
		Если ИмяТекущейКомандыСозданияДокумента = "СоздатьПлатежноеПоручение" Тогда
			СледующийДокумент();
		КонецЕсли;
		
		ПараметрыОткрытия = Новый Структура;
		ПараметрыОткрытия.Вставить("Основание", ПараметрыЗаполнения);
		ПараметрыОткрытия.Вставить("РаспознанныйДокументОснование", Объект.Ссылка);
		
		ОткрытьФорму("Документ.ПлатежноеПоручение.Форма.ФормаДокумента", ПараметрыОткрытия, ЭтотОбъект);
	...
	Иначе
		...
		Если СозданныйДокумент <> Неопределено Тогда
			...
			Если ИмяТекущейКомандыСозданияДокумента = "СоздатьСчетИПлатежноеПоручение" Тогда
				СозданныйСчетНаОплатуПоставщика = СозданныйДокумент;
			КонецЕсли;
...
Процедура НайтиИЗаполнитьСозданныеДокументы()
	...
	Иначе
		...
		НаличиеТоваровУслуг = РаспознаваниеДокументовСлужебныйВызовСервера.НаличиеТоваровУслуг(ТаблицаДокумента);
		ДанныеКомплекта.ПараметрыОперации    = РаспознаваниеДокументовКомплектыКлиентСервер.ТипДокументаИВидОперацииПоУмолчанию(
			Объект.Направление, ДанныеКомплекта.ТипКомплекта, НаличиеТоваровУслуг);
