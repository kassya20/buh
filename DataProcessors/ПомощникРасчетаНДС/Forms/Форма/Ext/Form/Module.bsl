...
Процедура ПриОткрытии(Отказ)
	...
	СервисСверкиРасчетовСверкаДекларацийНДСКлиент.ПриОткрытииОтчета(ЭтотОбъект, "ПомощникПоНДС");
	Если ИспользуетсяСервисСверкиРасчетов И ЗначениеЗаполнено(Объект.Организация) Тогда
		ВыполнитьСверку();
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "СостояниеРегламентнойОперации" Тогда 
		...
		ПроверитьСтатусРасчетаНалога();
		ПолучитьПоказателиДляРасчетаНалога();
		ОперацияВыполнена =ПредопределенноеЗначение("Перечисление.ВидыСостоянийРегламентныхОпераций.Выполнено");
		Если ТипЗнч(Источник)= Тип("ДокументСсылка.ФормированиеЗаписейКнигиПокупок")
			И Параметр = ОперацияВыполнена
			И ИспользуетсяСервисСверкиРасчетов Тогда
			ВыполнитьСверку(Истина);
		ИначеЕсли ТипЗнч(Источник)= Тип("ДокументСсылка.ФормированиеЗаписейКнигиПродаж")
			И Параметр = ОперацияВыполнена
			И ИспользуетсяСервисСверкиРасчетов Тогда
			ВыполнитьСверку(Ложь, Истина);
		КонецЕсли;
		
	ИначеЕсли ИмяСобытия = "ОстаткиАктуализированы" Тогда 
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ЭтоФоновоеЗаданиеДругогоПомощника И НЕ ЗначениеЗаполнено(ПользовательФоновогоЗадания) Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗавершениеРаботы Тогда
		
		ПодключитьОбработчикОжидания("Подключаемый_ОтменитьФоновыеЗадания", 1, Истина);
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ОпределитьИспользованиеСверки();
	Если ЗначениеЗаполнено(ИдентификаторЗаданияСверки) Тогда
		СервисСверкиРасчетовБПКлиент.ОтменитьВыполнениеСверкиВПомощнике(ЭтотОбъект);
	КонецЕсли;
	Если ИспользуетсяСервисСверкиРасчетов И ЗначениеЗаполнено(Объект.Организация) Тогда
		ВыполнитьСверку();
	КонецЕсли;
...
&НаКлиенте
Процедура ПревышениеДолиВычетовОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "БезопаснаяДоляВычета" Тогда
		КлючЗаписи = ПолучитьДанныеНастроекНДС(Объект.Организация, Объект.Период);
		...
		КонецЕсли;
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "КакПеренестиВычет" Тогда
		АдресСтатьиОПереносеВычета = "https://its.1c.ru/db/accnds#content:1508:hdoc";
		ПерейтиПоНавигационнойСсылке(АдресСтатьиОПереносеВычета);
	КонецЕсли;
...
&НаКлиенте
Процедура ОшибкаСверкиСНалогомНаПрибыльНажатие(Элемент, СтандартнаяОбработка) 
	
	СтандартнаяОбработка = Ложь;
	
	ОткрытьФорму("Отчет.СверкаДекларацийПрибыльНДС.Форма.ФормаОтчета",
		СверкаСНалогомНаПрибыль,ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.Независимый);

КонецПроцедуры

&НаКлиенте
Процедура ЕстьНДСУчтенныйВСтоимостиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПомощникПодготовкиКВычетуНДСВключенногоВСтоимость" Тогда
		СтандартнаяОбработка = Ложь;
...
&НаКлиенте
Процедура СообщениеНалогКВозмещениюПростойПомощникОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПревышениеДолиВычетовОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки);
	
КонецПроцедуры

&НаКлиенте
Процедура СообщениеПревышенаДоляВычетовПростойПомощникОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СообщениеПревышенаДоляВычетовСложныйПомощникОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура СообщениеНалогКВозмещениюСложныйПомощникОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДоляВычетовНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ДоляВычетовНажатиеНаКлиенте();
	
КонецПроцедуры

&НаКлиенте
Процедура ДоляВычетовСложныйПомощникНажатие(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДоляВычетовНажатиеНаКлиенте()
	
	ПараметрыОтчета  = Новый Структура();
	ПараметрыОтчета.Вставить("Организация", Объект.Организация);
	ПараметрыОтчета.Вставить("КонецПериода", КонецКвартала(Объект.Период));
	ПараметрыОтчета.Вставить("НачалоПериода", НачалоКвартала(Объект.Период));
	ПараметрыОтчета.Вставить("СформироватьПриОткрытии", Истина);
	ПараметрыОтчета.Вставить("ТолькоНДС", Истина);
	ПараметрыОтчета.Вставить("РежимРасшифровки", Истина);
	
	ОткрытьФорму("Отчет.ОценкаРискаНалоговойПроверки.Форма", 
		ПараметрыОтчета, ЭтотОбъект, УникальныйИдентификатор,,,, РежимОткрытияОкнаФормы.Независимый);
...
&НаКлиенте
Процедура СформироватьОтчетПоДокументамФункциональности(ЗаголовокМетода, ИмяОпции)
...
Процедура ПриИзмененииУсловийРасчета()
	...
	ЭтоФоновоеЗаданиеДругогоПомощника = Ложь;
	ОтменаРасчетаНДС = Ложь;
	ПользовательФоновогоЗадания = ПредопределенноеЗначение("Справочник.Пользователи.ПустаяСсылка");
...
Процедура ЗапуститьПерепроведение()
	...
	ВидДлительнойОперации = "ПерепровестиДокументыАвто";
	...
	ОповещениеОПрогрессеВыполнения = Новый ОписаниеОповещения("Подключаемый_ОбновитьФормуДлительнойОперации", ЭтотОбъект);
...
Процедура Подключаемый_ПроверитьВыполнениеЗаданияПерепроведение()
	...
	Состояние = СостояниеЗадания(ОписаниеФоновогоЗадания.ИдентификаторЗадания);
	Если Состояние = "Выполнено" И ЭтоФоновоеЗаданиеДругогоПомощника
	 Или Состояние = "Отменено" Тогда
		
		ОписаниеФоновогоЗадания = Неопределено;
		ОбновитьПоказателиРасчетаПослеОтключенияОтЗадания();
		
	ИначеЕсли Состояние = "Выполнено" Тогда
		
		Если ВидДлительнойОперации = "ПерепровестиДокументыАвто" Тогда
			// Выполнение запущенного нами фонового задания обрабатывается в ПоказатьРезультатДлительнойОперации() вызовом из механизма БСП.
			Возврат;
		Иначе
			// Ожидали запущенного не нами фонового задания.
			ОписаниеФоновогоЗадания.Вставить("Статус", Состояние);
			ПоказатьРезультатДлительнойОперации(ОписаниеФоновогоЗадания, Неопределено);
...
Процедура ПолучитьПоказателиДляРасчетаНалога()
	...
	Если ВыполненоПодключениеКФоновомуЗаданию() Тогда
		Возврат;
	КонецЕсли;
	
	СверкаНП = ПлательщикНалогаНаПрибыль И Объект.Период >= '20230101';
	
	СоставРазделов = Новый Структура("Операции, Расчет, Уведомление, Уплата, СверкаСНалогомНаПрибыль",
		НЕ АвтоматическийПомощник, АвтоматическийПомощник, ПлательщикЕНП, Истина, СверкаНП);

	ЗаполнитьПомощник(СоставРазделов, Истина);
...
Функция ВыполнитьРасчетНаСервере()
	...
	ПараметрыВыполнения.КлючФоновогоЗадания = ПомощникРасчетаНДС.СобратьКлючЗаданияДляФоновогоЗадания(
			ПараметрыЗадачи.Организация,
			ПользователиКлиентСервер.ТекущийПользователь(),
			ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий().РасчетНалога,
...
Процедура Подключаемый_ПроверитьВыполнениеЗаданияРасчет()
	...
	КонецЕсли;
	 ...
	 Состояние = СостояниеЗадания(ИдентификаторЗаданияРасчетНалога);
...
&НаКлиенте 
Процедура ОбновитьПоказателиРасчетаПослеОтключенияОтЗадания()
	
	ЭтоФоновоеЗаданиеДругогоПомощника = Ложь;
	ПроверитьСтатусРасчетаНалога();
	
	Если РассчитыватьНалогАвтоматически <> ДоступенАвтоматическийРасчетНДС Тогда
		ПриИзмененииУсловийРасчета();
	Иначе
		ПолучитьПоказателиДляРасчетаНалога();
	КонецЕсли;
...
&НаКлиенте
Функция ОбновитьСтатусФоновогоЗадания(ДлительнаяОперация = Неопределено)
	
	ЧастиТекста = Новый Массив;
	
	Если ЗначениеЗаполнено(ПользовательФоновогоЗадания) Тогда
		
		ШаблонТекста = НСтр("ru = 'Идет расчет НДС пользователем %1.'");

		ТекстФоновоеЗаданиеДругогоПользователя = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
		ПользовательФоновогоЗадания);
		
		ЧастиТекста.Добавить(ТекстФоновоеЗаданиеДругогоПользователя);
		ЧастиТекста.Добавить(Символы.ПС);
		
	КонецЕсли;
	
	// Если пользователь не указан, значит текущий пользователь выполняет расчет НДС
	Если НЕ ЗначениеЗаполнено(ПользовательФоновогоЗадания) 
			И НЕ ЗначениеЗаполнено(ИдентификаторЗаданияПолучениеПоказателей) Тогда
		
		СостояниеКнопкиОтменыЗадания(Истина);
		
	КонецЕсли;
	
	Если ОтменаРасчетаНДС Тогда
		
		ЧастиТекста.Добавить(НСтр("ru = 'Пожалуйста, подождите, расчет НДС отменяется...'"));
		СостояниеКнопкиОтменыЗадания(Истина, Ложь);
		
	ИначеЕсли ОписаниеФоновогоЗадания <> Неопределено И СтрНачинаетсяС(ВидДлительнойОперации, "ПерепровестиДокументы") Тогда
		
		Процент = 0;
		Прогресс = ЗакрытиеМесяцаВызовСервера.ПрочитатьПрогресс(
			?(ДлительнаяОперация = Неопределено, ОписаниеФоновогоЗадания.ИдентификаторЗадания, ДлительнаяОперация));
		Если ТипЗнч(Прогресс) = Тип("Структура") 
			И Прогресс.Свойство("ДополнительныеПараметры") Тогда
			
			ДостигнутоеЗначение = Прогресс.ДополнительныеПараметры.ДостигнутоеЗначение;
			
			Если ТипЗнч(ДостигнутоеЗначение) = Тип("Дата") Тогда
				Числитель   = (НачалоДня(ДостигнутоеЗначение) - НачалоДня(МоментНарушенияПоследовательности)) / 86400 + 1;
				Знаменатель = (НачалоДня(КонецКвартала(Объект.Период)) - НачалоДня(МоментНарушенияПоследовательности)) / 86400 + 1;
			ИначеЕсли ТипЗнч(ДостигнутоеЗначение) = Тип("Число") И ТипЗнч(Прогресс.ДополнительныеПараметры.КонечноеЗначение) = Тип("Число") Тогда
				Числитель   = ДостигнутоеЗначение;
				Знаменатель = ?(Прогресс.ДополнительныеПараметры.КонечноеЗначение = 0, Числитель, Прогресс.ДополнительныеПараметры.КонечноеЗначение);
			Иначе
				Числитель   = 1;
				Знаменатель = 1;
			КонецЕсли;
		...
		ЧастиТекста.Добавить(НСтр("ru = 'Выполняется перепроведение документов...'") + Процент);
	...
	ИначеЕсли ЗначениеЗаполнено(ИдентификаторЗаданияРасчетНалога) Тогда
		...
		ЧастиТекста.Добавить(НСтр("ru = 'Выполняется расчет налога...'") + Процент);
	...
	ИначеЕсли ЗначениеЗаполнено(ИдентификаторЗаданияПолучениеПоказателей) Тогда
		...
		ЧастиТекста.Добавить(НСтр("ru = 'Выполняется получение показателей...'"));
		
		СостояниеКнопкиОтменыЗадания(Ложь);
...
Функция ПолучитьПоказателиДляРасчетаНалогаНаСервере(СоставРазделов)
	...
	ПараметрыЗадачи.КонтролироватьДолюВычета = КонтролироватьДолюВычета;
...
&НаСервере
Процедура ЗагрузитьРезультатВыполненияПерепроведения(РезультатЗадания)
	
	АдресХранилищаПерепроведение = РезультатЗадания.АдресРезультата;
	Результат = ПолучитьИзВременногоХранилища(АдресХранилищаПерепроведение);
	УдалитьИзВременногоХранилища(АдресХранилищаПерепроведение);
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Если ДоступенАвтоматическийРасчетНДС И РассчитыватьНалогАвтоматически Тогда
		...
		Элементы.ГруппаРасчетДолиВычета.Видимость            = КонтролироватьДолюВычета;
		Элементы.ОтчетныеФормы.Видимость                     = Истина;
		Элементы.ОплатаНалога.Видимость                      = Истина;
		Элементы.ДекларацияНеактуальна.Видимость             = Ложь;
		Элементы.ГруппаНДСКУплатеПошаговыйПомощник.Видимость = Ложь;
	...
	Иначе
		...
		Элементы.РеестрКРазделу7Декларации.Видимость    =
			УчетНДСПереопределяемый.ЗаполняетсяРаздел7ДекларацииНДС(Объект.Период)
		...
		Элементы.ГруппаНДСКУплатеПошаговыйПомощник.Видимость = ПростойУчетЕНС;
	...
	Элементы.СуммаНалоговыйАгентНДС.Заголовок    = ЗаголовокНалоговыйАгент();
	
	Элементы.ГруппаНДСТаможенныйСоюз.Видимость = СуммаТаможенныйСоюзНДС <> 0 И НЕ ПростойУчетЕНС;
	
	Элементы.ПодробнееНаИТС.Видимость = ЗначениеЗаполнено(Объект.Период);
	
	УстановитьКнопкуПоУмолчанию();
	
	Элементы.ГруппаУведомлениеОНалогах.Видимость = Не ПравилоУплатыВыполняетсяЕдинымПомощником;
	Элементы.ОплатаНалога.Видимость = Не ПравилоУплатыВыполняетсяЕдинымПомощником;
	
	Элементы.СверкаСНалогомНаПрибыль.Видимость = ПлательщикНалогаНаПрибыль И Объект.Период >= '20230101';
...
Процедура НадписьОтчетность()
	...
	Иначе
		...
		СрокПодачиОтчета = Обработки.ПомощникРасчетаНДС.СрокПодачиОтчета(Платежи[0].Дата);
		ТекстПорядокСдачиОтчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекста,
			Формат(СрокПодачиОтчета, "ДФ='дд ММММ'"));
	КонецЕсли;
...
&НаСервере
Функция ЗаголовокНалоговыйАгент()
	
	ТекстЗаголовка = НСтр("ru = 'Сумма НДС по операциям налогового агента'");
	Если ПростойУчетЕНС Тогда
		ТекстЗаголовка = НСтр("ru = 'в т.ч. НДС по операциям налогового агента'");
	КонецЕсли;
...
Процедура НастроитьБлокРасчет()
	...
	ВыполняетсяРасчет = ЗначениеЗаполнено(ИдентификаторЗаданияРасчетНалога)
		Или ОписаниеФоновогоЗадания <> Неопределено;
	...
	ПоказыватьСообщениеОВозмещении     = Ложь;
	ПоказыватьСообщениеОПревышенииДоли = Ложь;
	...
	Если Не ДекларацияСдана
		...
		Элементы.СообщениеПревышенаДоляВычетовПростойПомощник.Видимость = Ложь;
		Элементы.СообщениеНалогКВозмещениюПростойПомощник.Видимость     = Ложь;
	Иначе
		Элементы.ТребуетсяРасчет.Видимость = Ложь;
		ПоказыватьСообщениеОПревышенииДоли = Не ДекларацияСдана
			И ЗначениеЗаполнено(СообщениеПревышенаДоляВычетов);
		Элементы.СообщениеПревышенаДоляВычетовПростойПомощник.Видимость = ПоказыватьСообщениеОПревышенииДоли;
		ПоказыватьСообщениеОВозмещении = Не ДекларацияСдана
			И ЗначениеЗаполнено(СообщениеНалогКВозмещению);
		Элементы.СообщениеНалогКВозмещениюПростойПомощник.Видимость = ПоказыватьСообщениеОВозмещении;
	КонецЕсли;
	
	УстановитьВысотуСообщенийОПревышении(ПоказыватьСообщениеОПревышенииДоли);
	ВыделитьБлокСПревышением(ПоказыватьСообщениеОВозмещении, ПоказыватьСообщениеОПревышенииДоли);
...
Процедура УстановитьКнопкуПоУмолчанию()
	...
	Если ДоступенАвтоматическийРасчетНДС И РассчитыватьНалогАвтоматически Тогда
		КнопкаПоУмолчаниюДекларация = Не ДекларацияСдана
			И (Не РасчетВыполнен Или ТребуетсяПерепроведение);
		УстановитьВидПоУмолчаниюОформлением(ЭтотОбъект, Элементы.Рассчитать, КнопкаПоУмолчаниюДекларация);
	Иначе
		КнопкаПоУмолчаниюДекларация = Не ЗначениеЗаполнено(Декларация);
		УстановитьВидПоУмолчаниюОформлением(ЭтотОбъект, Элементы.СформироватьДекларацию, КнопкаПоУмолчаниюДекларация);
	КонецЕсли;
	
	КнопкаПоУмолчаниюУведомление = Не КнопкаПоУмолчаниюДекларация И ПлательщикЕНП И Не УведомлениеПодготовлено;
	...
	КнопкаПоУмолчаниюОтправитьВБанк = Не КнопкаПоУмолчаниюДекларация И Не КнопкаПоУмолчаниюУведомление
	...
	КнопкаПоУмолчаниюУплата = Не КнопкаПоУмолчаниюДекларация И Не КнопкаПоУмолчаниюУведомление
...
&НаСервере
Процедура УстановитьВысотуСообщенийОПревышении(ПоказыватьСообщениеОПревышенииДоли)
	
	ВысотаБлокаСПредупреждением = 2;
	ВысотаБлокаСРекомендацией   = 3;
	
	// Высота сообщений зависит от состава предупреждений о превышении вычетов.
	Если ПоказыватьСообщениеОПревышенииДоли Тогда
		Элементы.СообщениеНалогКВозмещениюПростойПомощник.Высота = ВысотаБлокаСПредупреждением;
		Элементы.СообщениеНалогКВозмещениюСложныйПомощник.Высота = ВысотаБлокаСПредупреждением;
		Элементы.СообщениеПревышенаДоляВычетовПростойПомощник.Высота = ВысотаБлокаСПредупреждением + ВысотаБлокаСРекомендацией;
		Элементы.СообщениеПревышенаДоляВычетовСложныйПомощник.Высота = ВысотаБлокаСПредупреждением + ВысотаБлокаСРекомендацией;
	Иначе
		Элементы.СообщениеНалогКВозмещениюПростойПомощник.Высота = ВысотаБлокаСПредупреждением + ВысотаБлокаСРекомендацией;
		Элементы.СообщениеНалогКВозмещениюСложныйПомощник.Высота = ВысотаБлокаСПредупреждением + ВысотаБлокаСРекомендацией;
		Элементы.СообщениеПревышенаДоляВычетовПростойПомощник.Высота = ВысотаБлокаСПредупреждением;
		Элементы.СообщениеПревышенаДоляВычетовСложныйПомощник.Высота = ВысотаБлокаСПредупреждением;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ВыделитьБлокСПревышением(ПоказыватьСообщениеОВозмещении, ПоказыватьСообщениеОПревышенииДоли)
	
	ЦветБлокаСПревышением  = ЦветаСтиля.ЦветФонаНекорректногоКонтрагента;
	ЦветБлокаБезПревышения = Новый Цвет;
	
	Если ПоказыватьСообщениеОВозмещении И ПоказыватьСообщениеОПревышенииДоли Тогда
		// Выделяем общую группу со всеми превышенными показателями.
		Элементы.ГруппаПревышениеЗаГодИКварталПростойПомощник.ЦветФона = ЦветБлокаСПревышением;
		Элементы.ГруппаПревышениеЗаГодИКварталСложныйПомощник.ЦветФона = ЦветБлокаСПревышением;
		// Сбрасываем выделение для прочих групп по которым контролиркуется вычет.
		Элементы.ГруппаНалогКВозмещениюПростойПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаНалогКВозмещениюСложныйПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышенаДоляПростойПомощник.ЦветФона    = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышенаДоляСложныйПомощник.ЦветФона    = ЦветБлокаБезПревышения;
	ИначеЕсли ПоказыватьСообщениеОВозмещении Тогда 
		// Выделяем блок с сообщением о возмещении.
		Элементы.ГруппаНалогКВозмещениюПростойПомощник.ЦветФона = ЦветБлокаСПревышением;
		Элементы.ГруппаНалогКВозмещениюСложныйПомощник.ЦветФона = ЦветБлокаСПревышением;
		// Сбрасываем выделение общей группы и группы о превышении доли вычета.
		Элементы.ГруппаПревышениеЗаГодИКварталПростойПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышениеЗаГодИКварталСложныйПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышенаДоляПростойПомощник.ЦветФона           = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышенаДоляСложныйПомощник.ЦветФона           = ЦветБлокаБезПревышения;
	ИначеЕсли ПоказыватьСообщениеОПревышенииДоли Тогда 
		// Выделяем блок с сообщением о превышении доли вычета.
		Элементы.ГруппаПревышенаДоляПростойПомощник.ЦветФона = ЦветБлокаСПревышением;
		Элементы.ГруппаПревышенаДоляСложныйПомощник.ЦветФона = ЦветБлокаСПревышением;
		// Сбрасываем выделение общей группы и группы о возмещении.
		Элементы.ГруппаПревышениеЗаГодИКварталПростойПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышениеЗаГодИКварталСложныйПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаНалогКВозмещениюПростойПомощник.ЦветФона        = ЦветБлокаБезПревышения;
		Элементы.ГруппаНалогКВозмещениюСложныйПомощник.ЦветФона        = ЦветБлокаБезПревышения;
	Иначе
		// Сбрасываем выделение всех групп с контролируемыми показателями.
		Элементы.ГруппаПревышенаДоляПростойПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышенаДоляСложныйПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышениеЗаГодИКварталПростойПомощник.ЦветФона = ЦветБлокаБезПревышения;
		Элементы.ГруппаПревышениеЗаГодИКварталСложныйПомощник.ЦветФона = ЦветБлокаБезПревышения;
...
Процедура ОбработатьУплату(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СоставРазделов = Новый Структура("Операции, Расчет, Уведомление, Уплата", Ложь, Ложь, Ложь, Истина);
	ЗаполнитьПомощник(СоставРазделов);
...
Процедура ОбработатьУведомление(РезультатЗакрытия = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СоставРазделов = Новый Структура("Операции, Расчет, Уведомление, Уплата", Ложь, Ложь, Истина, Ложь);
	ЗаполнитьПомощник(СоставРазделов);
...
Процедура БыстраяПроверкаДанныхНДС()
	...
	СообщениеТребуетсяИсправитьОшибки = СтрокаСообщенияНеЗаполненыДанныеДляОтчетности(ОшибкиПоДаннымНДС);
	СообщениеПревышенаДоляВычетов     = СтрокаСообщенияПревышенаБезопаснаяДоляВычета();
	СообщениеНалогКВозмещению         = СтрокаСообщенияНалогКВозмещению();
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция СтрокаСообщенияНеЗаполненыДанныеДляОтчетности(ОшибкиПоДаннымНДС) Экспорт
...
&НаСервере
Функция СтрокаСообщенияПревышенаБезопаснаяДоляВычета()
	
	Если КонтролироватьДолюВычета
		И ДоляВычетов > БезопаснаяДоляВычета Тогда
		
		ТекстСообщенияБезопаснаяДоля = НСтр("ru = '   Доля вычетов превышает '");
		ГиперссылкаБезопаснаяДоляВычета = Новый ФорматированнаяСтрока("" + БезопаснаяДоляВычета + "%",,,,"БезопаснаяДоляВычета");
		ТекстСообщенияПредупреждение = НСтр("ru = '
			|   Это может привести к назначению выездной налоговой проверки'");
		
		СообщениеОПревышении = Новый Массив;
		
		СообщениеОПревышении.Добавить(ТекстСообщенияБезопаснаяДоля);
		СообщениеОПревышении.Добавить(ГиперссылкаБезопаснаяДоляВычета);
		СообщениеОПревышении.Добавить(ТекстСообщенияПредупреждение);
		
		ДополнитьСообщениеИнформациейОПереносеВычета(СообщениеОПревышении);
		
		Возврат Новый ФорматированнаяСтрока(СообщениеОПревышении);
...
&НаСервере
Функция СтрокаСообщенияНалогКВозмещению()
	
	Если Объект.Период >= УчетНДСКлиентСервер.НачалоРаботыКонтроляДолиВычетов() 
		И НалогКВозмещению > 0 Тогда 
		
		ТекстСообщения = НСтр("ru = '   В текущем квартале налог к вычету превышает начисленный 
			|   Это приведет к назначению камеральной проверки (предоставлению документов в ИФНС и т.д.)'");
		
		СообщениеОПревышении = Новый Массив;
		СообщениеОПревышении.Добавить(ТекстСообщения);
		// Если выводится только сообщение о налоге к возмещению, то добавим ссылку на ИТС по переносу.
		Если НЕ (КонтролироватьДолюВычета
			И ДоляВычетов > БезопаснаяДоляВычета) Тогда 
			ДополнитьСообщениеИнформациейОПереносеВычета(СообщениеОПревышении);
		КонецЕсли;
...
&НаСервере
Процедура ДополнитьСообщениеИнформациейОПереносеВычета(СообщениеОПревышении)
	
	ШаблонПредупреждение = НСтр("ru = '
		|
		|Перенос части вычета на следующий налоговый период поможет снизить риски.'");
	ТекстПредупреждение = Новый ФорматированнаяСтрока(ШаблонПредупреждение, Новый Шрифт(,,,Истина));
	ТекстСообщенияСсылкаНаИТС = НСтр("ru = '
		|Как перенести вычет на следующий налоговый период?'");
	ГиперссылкаКакПеренестиВычет = Новый ФорматированнаяСтрока(
	ТекстСообщенияСсылкаНаИТС, Новый Шрифт(,,,Истина),,,"КакПеренестиВычет");
	СообщениеОПревышении.Добавить(ТекстПредупреждение);
	СообщениеОПревышении.Добавить(ГиперссылкаКакПеренестиВычет);
...
Процедура УстановитьНастройкиРасчета()
	...
	УСНщикНаНДС           = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Объект.Организация, Объект.Период);
	УСНщикНаНДСБезВычетов = УСНщикНаНДС 
		И РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(Объект.Организация, Объект.Период) <> Перечисления.ВидыСтавокНДС.Общая;	
	КонтролироватьДолюВычета = ПолучитьФункциональнуюОпцию("КонтролироватьДолюВычетаНДС", 
		Новый Структура("Организация, Период", Объект.Организация, Объект.Период))
		И Объект.Период >= УчетНДСКлиентСервер.НачалоРаботыКонтроляДолиВычетов()
		И НЕ УСНщикНаНДСБезВычетов;
	БезопаснаяДоляВычета = УчетнаяПолитика.БезопаснаяДоляВычета(Объект.Организация, Объект.Период);
	
	Если Не КонтекстныйВызов Или Не ЗначениеЗаполнено(Срок) Тогда
		ВидНалога = Перечисления.ВидыНалогов.НДС;
		ПериодСобытия = КонецКвартала(Объект.Период);
		...
		Если ПорядокПредоставленияОтчета <> Неопределено Тогда
			...
			Срок    = ПорядокПредоставленияОтчета.Срок;
		Иначе
			Срок    = ПериодСобытия;
		КонецЕсли;
	КонецЕсли;
	...
	ПлательщикНалогаНаПрибыль = УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Объект.Организация, Объект.Период);
...
Процедура ЗаполнитьТаблицуОперацийПошаговогоПомощника()
	...
	СтрокаТаблицы.РегламентнаяОперация = Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС;
	СтрокаТаблицы.ИмяОперации = "ВводОстатковРУ";
	...
	СтрокаТаблицы = ТаблицаОпераций.Добавить();
	СтрокаТаблицы.РегламентнаяОперация = Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости;
	СтрокаТаблицы.ИмяОперации = "ПодготовкаКВычетуНДСУчтенногоВСтоимости";
...
Процедура ОбновитьЭлементыРегламентныхОпераций()
	...
	Для Каждого СтрокаУзла ИЗ ПолноеДеревоОпераций Цикл
		...
		Если РегламентнаяОперация = Перечисления.РегламентныеОперации.ФормированиеДекларации Тогда 
			Элементы.ДекларацияНеактуальна.Видимость = НЕ СтрокаУзла.Актуальна 
				И ЗначениеЗаполнено(Декларация)
				И НЕ ДекларацияСдана;
			ПоказыватьСообщениеОПревышенииДоли = ЗначениеЗаполнено(СообщениеПревышенаДоляВычетов)
				И СтрокаУзла.Актуальна
				И НЕ ДекларацияСдана;
			Элементы.СообщениеПревышенаДоляВычетовСложныйПомощник.Видимость =  ПоказыватьСообщениеОПревышенииДоли;
			ПоказыватьСообщениеОВозмещении = ЗначениеЗаполнено(СообщениеНалогКВозмещению)
				И СтрокаУзла.Актуальна
				И НЕ ДекларацияСдана;
			Элементы.СообщениеНалогКВозмещениюСложныйПомощник.Видимость = ПоказыватьСообщениеОВозмещении;
			// Долю вычета, сумму к возмещению в сложном помощнике показываем только в случае превышения.
			Элементы.ГруппаРасчетДолиВычетаСложныйПомощник.Видимость = ПоказыватьСообщениеОПревышенииДоли;
			Элементы.НалогКВозмещениюСложныйПомощник.Видимость       = ПоказыватьСообщениеОВозмещении;
			УстановитьВысотуСообщенийОПревышении(ПоказыватьСообщениеОПревышенииДоли);
			ВыделитьБлокСПревышением(ПоказыватьСообщениеОВозмещении, ПоказыватьСообщениеОПревышенииДоли);
		КонецЕсли;
		...
		Если РегламентнаяОперация = Перечисления.РегламентныеОперации.РегистрацияСчетовФактурНаАванс
		 ...
		 ИЛИ РегламентнаяОперация = Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости Тогда 
		...
		ИначеЕсли РегламентнаяОперация = Перечисления.РегламентныеОперации.ПереходНаРаздельныйУчетНДСНаСчете19
			  ...
			  ИЛИ РегламентнаяОперация = Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС
		...
		//Установка заголовка расширенной подсказки
		Если РегламентнаяОперация = Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости Тогда 
			
			Элементы[ТекущаяСтрока.ИмяОперации].РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(
				Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'При переходе на уплату НДС по общей ставке (%1).
					|Налог, включенный в стоимость остатков на счетах '"),
				УчетНДСКлиентСервер.ОбщаяСтавкаНДС(ОбщегоНазначения.ТекущаяДатаПользователя()))),
				Новый ФорматированнаяСтрока("07, 08, 10, 41 и 45",,,,"ОткрытьСписокСчетовПоиска"),
				НСтр("ru = ', можно принять к вычету.'"));
		КонецЕсли;
...
Процедура ОбработкаНажатияСсылки(ИмяОперации)
	...
	Если ТекущаяСтрока.ИмяОперации = "ВводОстатковРУ" Тогда
		
		ОбработкаВыбранногоДействияВводОстатковРУ();
		Возврат;
	...
	Если ТекущаяСтрока.ИмяОперации = "ПодготовкаКВычетуНДСУчтенногоВСтоимости" Тогда
		
		ОбработкаВыбранногоДействияПодготовкаКВычетуНДСУчтенногоВСтоимости();
		Возврат;
...
Процедура ВвестиДокумент(РегламентнаяОперация, ИмяОперации, ОпределитьНаличиеДокументов, ПараметрыФормы)
	...
	ИначеЕсли ИмяОперации = "ФормированиеСводнойСправки" Тогда
		ПараметрыЗаполнения = Новый Структура("ПараметрыЗаполненияФормы", ПараметрыФормы);
		ПолноеИмяФормы = "Обработка.РегистрацияСводныхСправок.Форма";
	Иначе
...
Процедура ПроверитьВозможностьВыполненияЗадачи(БлижайшийНалоговыйПериод)
	...
	ИначеЕсли НЕ (УчетнаяПолитика.ПлательщикНДС(Объект.Организация, КонецКвартала(Объект.Период)) 
		ИЛИ УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Объект.Организация, КонецКвартала(Объект.Период))) Тогда
...
Процедура ОтобразитьПричиныНедоступностиАвтоматическогоРасчета()
	...
	Элементы.ЕстьНДСУчтенныйВСтоимости.Видимость           = Ложь;
	...
	Иначе
		...
		Элементы.ЕстьНДСУчтенныйВСтоимости.Видимость = ПричиныНедоступности.Найти("ЕстьНДСУчтенныйВСтоимости") <> Неопределено;
		Элементы.ЕстьНДСУчтенныйВСтоимости.Заголовок = Новый ФорматированнаяСтрока(
		Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'При переходе на уплату НДС по общей ставке (%1).
				|Налог, включенный в стоимость остатков на счетах '"),
			УчетНДСКлиентСервер.ОбщаяСтавкаНДС(ОбщегоНазначения.ТекущаяДатаПользователя()))),
			Новый ФорматированнаяСтрока("07, 08, 10, 41 и 45",,,,"ОткрытьСписокСчетовПоиска"),
			Новый ФорматированнаяСтрока(НСтр("ru = ', можно принять к вычету.'")),   
			Символы.ПС,
...
Процедура ПерепровестиДокументы(ТекущийМесяц, ДатаНачалаПерепроведения)
	...
	ВидДлительнойОперации = "ПерепровестиДокументы";
...
Функция ЗапуститьПереносГраницыАктуальности(ТекущийМесяц, СамостоятельнаяОперация)
	...
	ПараметрыПереноса.Вставить("Период",                  ?(СамостоятельнаяОперация, ТекущийМесяц, ДобавитьМесяц(ТекущийМесяц, -1)));
	ПараметрыПереноса.Вставить("Организация",             Объект.Организация);
	ПараметрыПереноса.Вставить("АдресХранилищаСОшибками", ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор));
	...
	ПараметрыВыполнения.КлючФоновогоЗадания = ПомощникРасчетаНДС.СобратьКлючЗаданияДляФоновогоЗадания(
			Объект.Организация,
			ПользователиКлиентСервер.ТекущийПользователь(),
			ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий().Проведение,
			КонецКвартала(Объект.Период),
			МоментНарушенияПоследовательности);
...
Процедура Подключаемый_ПроверитьПрогрессЗадания()
	...
	ЗакрытиеМесяцаКлиент.ПроверитьПрогрессЗадания(ФормаДлительнойОперации, ОписаниеФоновогоЗадания);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьФормуДлительнойОперации(ДлительнаяОперация, ДополнительныеПараметры) Экспорт

	Если ВидДлительнойОперации <> "ПерепровестиДокументыАвто" Тогда
		Возврат;
	КонецЕсли;
	
	ОбновитьСтатусФоновогоЗадания(ДлительнаяОперация);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьРезультатДлительнойОперации(РезультатЗадания, ДополнительныеПараметры) Экспорт
	...
	ПерепровестиДокументыАвтоматически = (ВидДлительнойОперации = "ПерепровестиДокументыАвто");
	
	// По-умолчанию после завершения длительной операции мы закроем форму с индикатором.
	ЗакрытьФормуДлительнойОперации = Истина;
	Если ПустаяСтрока(ВидДлительнойОперации)
	 Или ПерепровестиДокументыАвтоматически
	 Или ВидДлительнойОперации = "ПереносГраницыВРамкахПроведения" Тогда
		// Оставим открытой форму с индикатором, если она не открывалась, или если после завершения одного фонового задания
		// должно автоматически начаться следующее.
		ЗакрытьФормуДлительнойОперации = Ложь;
		
	КонецЕсли;
	
	Отказ = Ложь;
	Если РезультатЗадания.Статус = "Отменено"
	 Или РезультатЗадания.Статус = "Выполнено" И ЭтоФоновоеЗаданиеДругогоПомощника Тогда
		// Пользователь отменил запущенное им самим задание, или при запуске обнаружили, что есть задание в другом сеансе.
		Если ПерепровестиДокументыАвтоматически Тогда
			ОбновитьПоказателиРасчетаПослеОтключенияОтЗадания();
		КонецЕсли;
		Отказ = Истина;
	...
	Если ПустаяСтрока(ВидДлительнойОперации) // если контроль фонового задания был обработчиком ожидания
	 Или ПерепровестиДокументыАвтоматически Тогда
	
		ЗагрузитьРезультатВыполненияПерепроведения(РезультатЗадания);
		Если ТребуетсяПерепроведение Тогда 
	...
	Иначе
		...
		Результат = ОбновитьДанныеПоРезультатуДлительнойОперации(РезультатЗадания);
		ОбработатьЗавершениеДлительнойОперацииНаКлиенте(Результат);
...
&НаСервере
Функция ОбновитьДанныеПоРезультатуДлительнойОперации(РезультатЗадания)
...
Процедура ОбработатьРезультатПерепроведенияДокументовНаКлиенте(РезультатПерепроведения)
	...
	Иначе
		...
		// Закроем форму длительной операции, она больше не требуется.
		ЗакрытиеМесяцаКлиент.ЗакрытьФормуДлительнойОперации(ФормаДлительнойОперации);
...
// Функция ищет фоновые задания, запущенные обработкой Помощник по расчету НДС
// 
// Параметры:
//  Организация - СправочникСсылка.Организация - Организация, 
//                                 по которой будет производиться поиск фонового
//                                 задания
//  ДатаОкончания - Дата - Дата окончания расчета НДС. Дата нужна для отбора фонового задания 
// 
...
//               под которым было запущено фоновое задание
//               * МоментНарушенияПоследовательности  - Дата - момент нарушения последовательности
//                на стороне Помощника расчета НДС, запустившего фоновое задание
&НаСервереБезКонтекста
Функция НайтиФоновоеЗаданиеПомощникаРасчетаНДС(Организация, ДатаОкончания)
	...
	Для каждого ФоновоеЗадание Из МассивФоновыхЗаданиях Цикл
		...
		Если Ключ.Количество() > 1 Тогда
			...
			Если НЕ ЭтоФоновоеЗаданиеПомощникаНДС(ПрефиксКлючаЗадания) Тогда
				Продолжить;
			КонецЕсли;
			...
			ДатаОкончанияРасчетаНДС = Дата(Ключ[3]);
			
			Если ОрганизацияЗадания = Организация И КонецДня(ДатаОкончанияРасчетаНДС) = ДатаОкончания Тогда
				
				МоментНарушенияПоследовательности = Дата(Ключ[4]);
				...
				СвойстваВыполняемогоЗадания.Вставить("ПрефиксКлючаЗадания", ПрефиксКлючаЗадания);
				СвойстваВыполняемогоЗадания.Вставить("МоментНарушенияПоследовательности", МоментНарушенияПоследовательности);
...
#Область ВводОстатковРУ

&НаКлиенте
Процедура ОбработкаВыбранногоДействияВводОстатковРУ()
	
	ПараметрыДляОткрытияФормы = Новый Структура;
	ПараметрыДляОткрытияФормы.Вставить("Организация",                       ЭтотОбъект.Объект.Организация);
	ПараметрыДляОткрытияФормы.Вставить("ДоступенОтборПоОрганизации",        Ложь);
	ПараметрыДляОткрытияФормы.Вставить("ВключатьОбособленныеПодразделения", Ложь);
	
	ОткрытьФорму("ОбщаяФорма.ВводНачальныхОстатковДляВеденияРаздельногоУчетаНДС", ПараметрыДляОткрытияФормы, ЭтотОбъект);
...
Процедура ВыполнитьДействияПоПереходуНаРаздельныйУчетНДСНаСчете19НаСервере()
	...
	Если СообщенияОбОшибках <> Неопределено Тогда
		Для каждого ОшибкаОбработки Из СообщенияОбОшибках Цикл
			ОшибкаОбработки.ПутьКДанным = "Объект";
			ОшибкаОбработки.Сообщить();
		КонецЦикла;
	Иначе
...
Процедура ВыполнитьДействияПоОтключениюРаздельногоУчетаНДСНаСервере()
	...
	Обработки.ПомощникРасчетаНДС.ОтменитьДействияПоОтключениюРаздельногоУчетаНДС(
		Объект.Организация, Объект.Период, СообщенияОбОшибках);
	Если СообщенияОбОшибках <> Неопределено Тогда
		Для каждого СтрОшибки Из СообщенияОбОшибках Цикл
			СтрОшибки.Сообщить();
		КонецЦикла;
	Иначе
...
&НаКлиенте
Процедура Подключаемый_ОтменитьФоновыеЗадания()
...
&НаКлиенте
Процедура ОтменитьФоновыеЗаданияНаКлиенте()
...
Процедура ОтменитьФоновыеЗаданияНаСервере()
	...
	// Если это фоновое задание другого помощника и запущенное другим пользователем
	Если ЭтоФоновоеЗаданиеДругогоПомощника И ЗначениеЗаполнено(ПользовательФоновогоЗадания) Тогда
	...
	Если ОписаниеФоновогоЗадания <> Неопределено Тогда
		...
		Обработки.ПомощникРасчетаНДС.ОтменитьФоновоеЗаданиеНаСервере(ОписаниеФоновогоЗадания.ИдентификаторЗадания);
		ОписаниеФоновогоЗадания = Неопределено;
...
// Функция возвращает состояние фонового задания 
//
// Параметры:
// ИдентификаторЗадания - идентификатор фонового задания
// 
// Возвращаемое значение:
// Строка - состояние задания (Выполнено, Отменено, Активно)
&НаСервереБезКонтекста
...
// Функция возвращает Истину, если было подключение к фоновому заданию, запущенному другим помощником НДС
//
// Возвращаемое значение:
//
// - Булево - истина, если было выполнено подключение к другому помощнику НДС
//
&НаКлиенте
Функция ВыполненоПодключениеКФоновомуЗаданию()
	
	СтруктураНайденныеФоновыеЗадания = НайтиФоновоеЗаданиеПомощникаРасчетаНДС(
		Объект.Организация, КонецКвартала(Объект.Период));
	
	// Если есть запущенные фоновые задания, то ждем выполнения
	Если СтруктураНайденныеФоновыеЗадания <> Неопределено Тогда
		
		ЭтоФоновоеЗаданиеДругогоПомощника = Истина;
		
		// Пользователь фонового задания заполняется, когда отличается от текущего пользователя
		Если СтруктураНайденныеФоновыеЗадания.Свойство("Пользователь")
			И ЗначениеЗаполнено(СтруктураНайденныеФоновыеЗадания.Пользователь)
			И СтруктураНайденныеФоновыеЗадания.Пользователь <> ПользователиКлиентСервер.ТекущийПользователь() Тогда
			
			ПользовательФоновогоЗадания = СтруктураНайденныеФоновыеЗадания.Пользователь;
			
		КонецЕсли;
		
		Если СтруктураНайденныеФоновыеЗадания.ПрефиксКлючаЗадания = "ПроведениеДокументов" Тогда
				
			ОписаниеФоновогоЗадания = Новый Структура;
			ОписаниеФоновогоЗадания.Вставить("ИдентификаторЗадания", СтруктураНайденныеФоновыеЗадания.УникальныйИдентификатор);
			ОписаниеФоновогоЗадания.Вставить("АдресРезультата", "");
			
			МоментНарушенияПоследовательности = СтруктураНайденныеФоновыеЗадания.МоментНарушенияПоследовательности;
		...
		Иначе
			...
			ИдентификаторЗаданияРасчетНалога =  СтруктураНайденныеФоновыеЗадания.УникальныйИдентификатор;
			АдресХранилищаРасчет       = "";
...
&НаКлиенте
Процедура СостояниеКнопкиОтменыЗадания(ПоказыватьКнопкуОтмена, ДоступностьКнопки = Истина)
	
	Если Элементы.СтраницыПомощник.ТекущаяСтраница = Элементы.СтраницаПустая Тогда
		
		Элементы.ОтменитьЗаданиеПриОткрытии.Видимость = ПоказыватьКнопкуОтмена;
		Элементы.ОтменитьЗаданиеПриОткрытии.Доступность = ДоступностьКнопки;
		
	ИначеЕсли Элементы.СтраницыПомощник.ТекущаяСтраница = Элементы.СтраницаРабочая Тогда
		
		Элементы.ОтменитьЗадание.Видимость = ПоказыватьКнопкуОтмена;
		Элементы.ОтменитьЗадание.Доступность = ДоступностьКнопки;
		
	КонецЕсли;
	
	Элементы.СтатусФоновогоЗадания.РастягиватьПоВертикали = НЕ ПоказыватьКнопкуОтмена;
	Элементы.СтатусФоновогоЗаданияПриОткрытии.РастягиватьПоВертикали = НЕ ПоказыватьКнопкуОтмена;
...
Процедура ОтменитьЗадание(Команда)
	...
	ОтменаРасчетаНДС = Истина;
	ОбновитьСтатусФоновогоЗадания();
	ОтменитьЗаданиеНаСервере(Объект.Организация, КонецКвартала(Объект.Период));
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОтменитьЗаданиеНаСервере(Организация, ДатаОкончания)
	...
	Для каждого ФоновоеЗадание Из МассивФоновыхЗаданиях Цикл
		...
		Если Ключ.Количество() > 1 Тогда
			...
			Если ОрганизацияЗадания = Организация И КонецДня(ДатаОкончанияРасчетаНДС) = ДатаОкончания Тогда
				...
				Обработки.ПомощникРасчетаНДС.ОтменитьФоновоеЗаданиеНаСервере(
...
// Функция возвращает Истина, если префикс ключа совпадает 
// с префиксами ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий()
// 
// Параметры:
//            ПрефиксКлючаЗадания - Строка- Префикс фонового задания
//
// Возвращаемое значение:
//      Булево - возвращает Истина, когда префикс совпал с префиксом фонового задания помощника НДС
//
&НаСервереБезКонтекста
Функция ЭтоФоновоеЗаданиеПомощникаНДС(ПрефиксКлючаЗадания)
	
	Если (ПрефиксКлючаЗадания = ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий().Проведение
		ИЛИ ПрефиксКлючаЗадания = ПомощникРасчетаНДС.ПрефиксыКлючаФоновыхЗаданий().РасчетНалога) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ОписаниеТаблицыНалоги()
	...
	ИндексОчередногоУведомления = 0;
	Пока ИндексОчередногоУведомления < Уведомления.Количество() Цикл
		Если Не ЗначениеЗаполнено(Уведомления[ИндексОчередногоУведомления].Уведомление) Тогда
			Прервать;
		Иначе
			ИндексОчередногоУведомления = ИндексОчередногоУведомления + 1;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ПомощникРасчетаНДС.ОписаниеТаблицыНалоги(ЭтотОбъект, ИндексОчередногоУведомления);
...
Процедура ОпределитьСвязанныеПравила()
	...
	Иначе
		ПравилоУплатыВыполняетсяЕдинымПомощником = УчетНДСПереопределяемый.ВедетсяПростойУчетЕНП(Срок, Объект.Организация);
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьПомощник(СоставРазделов, ФиксироватьПоказателиПолучены = Ложь)
	
	Результат = ПолучитьПоказателиДляРасчетаНалогаНаСервере(СоставРазделов);
		
	Если Результат.Статус = "Выполнено" Тогда
		Если ФиксироватьПоказателиПолучены Тогда
			ПоказателиПолучены = Истина;
		КонецЕсли;
		ЗагрузитьРезультатПоказателиРасчета();
	Иначе
...
&НаКлиенте
Процедура ОпределитьИспользованиеСверки()
	
	СервисСверкиРасчетовСверкаДекларацийНДСКлиент.ПроверкаИспользованияСверки(ЭтотОбъект, "ПомощникПоНДС");
	Если ИспользуетсяСервисСверкиРасчетов И ДанныеДляСверкиДеклараций = Неопределено Тогда
		СервисСверкиРасчетовСверкаДекларацийНДСКлиентСервер.ИнициализацияРеквизитовСверкиДекларации(ЭтотОбъект, "ПомощникПоНДС");
	КонецЕсли;
	
	Элементы.ГруппаОжиданиеСверки.Видимость               = Ложь;
	Элементы.ГруппаОжиданиеСверкиКнигиПокупок.Видимость   = Ложь;
	Элементы.ГруппаОжиданиеСверкиКнигиПродаж.Видимость    = Ложь;
	Элементы.ГруппаРезультатыСверки.Видимость             = Ложь;
	Элементы.ГруппаРезультатыСверкиКнигиПокупок.Видимость = Ложь;
	Элементы.ГруппаРезультатыСверкиКнигиПродаж.Видимость  = Ложь;
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьСверку(ТолькоКнигаПокупок = Ложь, ТолькоКнигаПродаж = Ложь)
	
	Если Не ИспользуетсяСервисСверкиРасчетов Тогда
		Возврат;
	КонецЕсли;
	
	Если РассчитыватьНалогАвтоматически И НЕ РасчетВыполнен Тогда
		Возврат;
	КонецЕсли;
	...
	СписокОрганизацийОтбора.Добавить(Объект.Организация);
	...
	ДанныеДляПроверкиКонтрагентов.Вставить("ЕстьДоступКВебСервисуФНС"        , Ложь);
	...
	ПараметрыОтчета.Вставить("ДополнительныеЛистыЗаТекущийПериод", Ложь);
	ПараметрыОтчета.Вставить("ГруппироватьПоКонтрагентам"        , Ложь);
	ПараметрыОтчета.Вставить("КонтрагентДляОтбора");
	ПараметрыОтчета.Вставить("ВыводитьТолькоДопЛисты"            , Ложь);
	ПараметрыОтчета.Вставить("ВыводитьПродавцовПоАвансам"        , Ложь);
	ПараметрыОтчета.Вставить("ВыводитьПокупателейПоАвансам"      , Ложь);
	...
	ПараметрыОтчета.Вставить("ОтбиратьПоКонтрагенту"             , Ложь);
	ПараметрыОтчета.Вставить("СписокОрганизаций"                 , СписокОрганизацийОтбора);
	...
	ПараметрыОтчета.Вставить("ФормироватьТабличныйДокумент"      , Ложь);
	ПараметрыОтчета.Вставить("ДанныеДляПроверкиКонтрагентов"     , ДанныеДляПроверкиКонтрагентов);
	ПараметрыОтчета.Вставить("ИспользуетсяСервисСверкиРасчетов"  , Истина);
	ПараметрыОтчета.Вставить("ДанныеДляСверкиДеклараций"         , ДанныеДляСверкиДеклараций);
	
	СервисСверкиРасчетовБПКлиент.СверитьКнигиДляПомощникаУчетаНДС(ЭтотОбъект, ПараметрыОтчета, ТолькоКнигаПокупок, ТолькоКнигаПродаж);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаВыбранногоДействияПодготовкаКВычетуНДСУчтенногоВСтоимости()
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("Организация",	Объект.Организация);
	ПараметрыЗаполнения.Вставить("Период",		Объект.Период);
	
	ОткрытьФорму("Обработка.ПодготовкаКВычетуНДСУчтенногоВСтоимости.Форма",ПараметрыЗаполнения);
