...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Ошибка Из МассивОшибок Цикл
		КонтекстОшибкиГОСТ34122018 = Неопределено;
		Ошибка.Свойство("ПриОшибкеГОСТ34122018", КонтекстОшибкиГОСТ34122018);
		Если ЗначениеЗаполнено(КонтекстОшибкиГОСТ34122018) Тогда
			Элементы.ОписаниеОшибкиСсылка.Высота = 4;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОшибкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Перем КонтекстРасшифровки, КонтекстОбработки, КонтекстАрхива, КонтекстОшибкиГОСТ34122018;
	...
	Если Поле.Имя = "ОписаниеОшибки" 
		...
		ИначеЕсли Контекст <> Неопределено И Контекст.Свойство("ПриИзвлеченииАрхива", КонтекстАрхива) Тогда
			...
			Если КонтекстЭДО = Неопределено Тогда 
				ПоказатьПредупреждение(, "Не удалось инициализировать контекст ЭДО, форма управления нерасшифровываемыми сообщениями не может быть открыта");				
			Иначе 
				КонтекстЭДО.ПолучениеПеремещенныхФайлов(КонтекстАрхива, ОрганизацияАрхива, "ДлительнаяОперация", Неопределено);
			КонецЕсли;
			
			Возврат;
			
		ИначеЕсли Контекст <> Неопределено И Контекст.Свойство("ПриОшибкеГОСТ34122018", КонтекстОшибкиГОСТ34122018) Тогда
			СтандартнаяОбработка = Ложь;
			ТипКриптопровайдера = ?(ЗначениеЗаполнено(КонтекстОшибкиГОСТ34122018)
				И КонтекстОшибкиГОСТ34122018[0].Свойство("ТипКриптопровайдера"),
				КонтекстОшибкиГОСТ34122018[0].ТипКриптопровайдера, Неопределено);
			
			ПараметрыФормыПредупреждения = Новый Структура("ТипКриптопровайдера", ТипКриптопровайдера);
			ОткрытьФорму(
...
Процедура ЗаполнитьДеревоОшибок(МассивОшибок)
	...
	Для каждого Ошибка Из МассивОшибок Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(КонтекстОбработки) Тогда
			НоваяОшибка.Контекст = Новый Структура;
		...
		ИначеЕсли ЗначениеЗаполнено(КонтекстОшибкиГОСТ34122018) Тогда
			НоваяОшибка.Контекст = Новый Структура;
			НоваяОшибка.Контекст.Вставить("ПриОшибкеГОСТ34122018", КонтекстОшибкиГОСТ34122018);
			НоваяОшибка.Контекст.Вставить("ПриОшибкеГОСТ34122018Организация", Ошибка.Организация);
			НоваяОшибка.Контекст.Вставить("ОписаниеОшибки", НоваяОшибка.ОписаниеОшибки);
			
			Если КонтекстОшибкиГОСТ34122018.Количество() > 1 Тогда
				НоваяОшибка.ОписаниеОшибки =
					СтрШаблон("%1 (%2 ошиб%3).",
						УбратьТочкуВКонце(НоваяОшибка.ОписаниеОшибки),
						КонтекстОшибкиГОСТ34122018.Количество(),
						КонтекстЭДО.ПростоеСклонение(КонтекстОшибкиГОСТ34122018.Количество(), "ка", "ки", "ок"));
			КонецЕсли;
		КонецЕсли;
