...
Функция ИзвлечьСведенияИзЗашифрованногоПакета(ИмяФайла, РабочийКаталог, РазмерДокумента)
	...
	Если WshShell = Неопределено Тогда 
		...
		КодЗавершения = ФайловаяСистема.ЗапуститьПрограмму(
			КоманднаяСтрока + ПараметрыКоманднойСтроки,
			ПараметрыЗапускаПрограммы).КодВозврата;
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Для Каждого Направление Из РезультатыДиагностики.Автонастройка.Получатели Цикл
		...
		ИначеЕсли Направление.Значение = "ФТС" Тогда 
			...
			Урл = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("/FtsPersonalCabinetWeb", "{FTS}/FtsPersonalCabinetWeb");
		...
		ИначеЕсли Направление.Значение = "ФСС" Тогда 
			...
			Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(
				ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.Отчет4ФСС"));
			Порт = 80;
			...
			Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(
				ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.РеестрСведенийФСС"));
			Порт = 80;
	...
	Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(
		ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ДанныеЭЛНФСС"),, Организация);
	ОтпечаткиСертификатов = КонтекстЭДОКлиент.ПолучитьОтпечаткиСертификатовИзНастроекОрганизацииДляФССНаСервере(
		Организация);
	ЭтоТестовыйСерверФССЭЛН = ОтпечаткиСертификатов <> Неопределено И ОтпечаткиСертификатов.ТестовыйСерверФССЭЛН = Истина;
	Порт = 443;
	Урл = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С(?(ЭтоТестовыйСерверФССЭЛН,
		"/eln/ws-insurer-crypto/api/soap/FileOperationsLnService?wsdl", "/ws-insurer-crypto-v20/FileOperationsLnService?wsdl"),
		"{FSS}/ws-insurer-crypto-v20/FileOperationsLnService?wsdl");
