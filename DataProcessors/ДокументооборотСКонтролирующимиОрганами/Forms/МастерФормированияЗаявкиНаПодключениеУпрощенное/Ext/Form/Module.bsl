...
Функция ПараметрыФормыНаправленийСервер() Экспорт
	...
	Результат = Мультирежим.ПараметрыОткрытияФормыНаправленийИзГлавногоОкна(ЭтотОбъект);
	Результат.Вставить("ЭтоОблако", ЭтоОблако(ЭтотОбъект) Или ЭтоОблачнаяПодпись(ЭтотОбъект));
	Возврат Результат;
...
Функция ЕстьОшибкиВЗаполненииЗаявления()
	...
	ОбработкаЗаявленийАбонентаКлиентСервер.ПроверитьАлгоритмШифрования(ЭтотОбъект, МастерДалее);
...
Процедура ИнициализироватьОкружениеИКонстанты()
	...
	АлгоритмШифрованияОбязателен =
		ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Об.К.ФНС ГОСТ 34.12-2018", '20260401');
...
Процедура ПрорисоватьПанельГОСТ34122018ДляФНС()
	...
	Если ЭтоОблако(ЭтотОбъект) Или ЭтоОблачнаяПодпись(ЭтотОбъект) Тогда
		Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ПрорисоватьПанельГОСТ34122018ДляФНСПослеПолученияАлгоритмовШифрования",
		ЭтотОбъект);
		
	КриптографияЭДКОКлиент.ПолучитьАлгоритмыШифрования(
		Оповещение,
		Ложь,,,,
		Новый Структура);
...
&НаКлиенте
Процедура ПрорисоватьПанельГОСТ34122018ДляФНСПослеПолученияАлгоритмовШифрования(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат <> Неопределено И Результат.Выполнено Тогда
		Для каждого СвойстваАлгоритмаШифрования Из Результат.АлгоритмыШифрования Цикл
			Если ВРег(СвойстваАлгоритмаШифрования.Имя) = "GR 34.12-15 M" Тогда
				Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = Ложь;
				Возврат;
			КонецЕсли;
		КонецЦикла;
	...
	ТипКриптопровайдераИнициализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат,
		"ТипКриптопровайдераИнициализации");
	Если НЕ ЗначениеЗаполнено(ТипКриптопровайдераИнициализации) Тогда
		Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	АлгоритмШифрованияУстарел = Истина;
	
	ПараметрыПриложения["БРО.ПредупредитьОГОСТ34122018ДляФНСПодключение"] =
		Новый Структура("ТипКриптопровайдераИнициализации", ТипКриптопровайдераИнициализации);
	Элементы.ПанельГОСТ34122018ДляФНС.Заголовок =
		КриптографияЭДКОСлужебныйКлиентСервер.ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018(
		ТипКриптопровайдераИнициализации,
		ГОСТ34122018ViPNetCSPДоступен,
		Истина);
	
	Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = АлгоритмШифрованияУстарел И СдаватьВФНС;
	Элементы.СкрытьПанельГОСТ2018.Видимость = Не АлгоритмШифрованияОбязателен;
...
Процедура ПанельГОСТ34122018ДляФНСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПредупредитьОГОСТ34122018ДляФНС" Тогда
		...
		ПредупредитьОГОСТ34122018ДляФНС = ПараметрыПриложения["БРО.ПредупредитьОГОСТ34122018ДляФНСПодключение"];
...
&НаКлиенте
Процедура СкрытьПанельГОСТ34122018ДляФНС(Команда)
