...
Процедура ЗаполнитьОписаниеСсылок()
	...
	Если ПолучитьФункциональнуюОпцию("УстановкаМобильногоКлиентаБизнесСтарт") Тогда
		ВсеСсылки.Вставить("ДругиеВозможности.МобильноеПриложение", 
			"e1cib/app/ОбщаяФорма.УстановкаМобильногоКлиентаБизнесСтарт");
	Иначе
		ВсеСсылки.Вставить("ДругиеВозможности.МобильноеПриложение", 
			"e1cib/app/ОбщаяФорма.УстановкаМобильногоКлиента");
	КонецЕсли;
...
Процедура ПерейтиПоСсылке(ПолнаяСсылкаHTML)
	...
	Иначе
		...
		ИначеЕсли СсылкаHTML = "Отчетность.НовыйПатент" Тогда
			...
			ЗначенияЗаполнения = Новый Структура("Организация", Организация);
			ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
			ОткрытьФорму("Документ.ОперацияСПатентом.ФормаОбъекта", ПараметрыФормы);
		...
		ИначеЕсли СсылкаHTML = "Документы.ЭДО" Тогда
			
			ОбменСКонтрагентамиКлиент.ПомощникПодключенияКСервису1СЭДО();
			
		ИначеЕсли СсылкаHTML = "Документы.РПД" Тогда
		...
		ИначеЕсли СсылкаHTML = "Помощь.ОбращениеВСлужбуПоддержки" Тогда
