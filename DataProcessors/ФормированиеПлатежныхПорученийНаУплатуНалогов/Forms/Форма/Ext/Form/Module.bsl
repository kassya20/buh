...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПлательщикЕНП = УчетнаяПолитика.ПлательщикЕНП(Объект.Организация, Объект.ДатаОстатков);
	...
	Если НЕ ПустаяСтрока(Параметры.Платежи) Тогда
		...
		УстановитьОплатуСтрокТаблицы(ЭтотОбъект, Истина, Истина);
...
Процедура ЗаполнитьСписокВыбораСпособовОплатыНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		БанковскиеСчетаОрганизации = Справочники.БанковскиеСчета.БанковскиеСчетаОрганизации(
			Объект.Организация, ВалютаРегламентированногоУчета, , Ложь);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОплатуСтрокТаблицы(Форма, ЗначениеОтметки, ДанныеЗаполнения = Ложь)
	
	Для каждого СтрокаТаблицы Из Форма.Объект.БухОстатки Цикл
		Если ДанныеЗаполнения И ЗначениеОтметки И Форма.ПлательщикЕНП Тогда
			СтрокаТаблицы.Оплатить = СтрокаТаблицы.Налог = Форма.ЕдиныйНалоговыйПлатеж;
		Иначе
...
Функция СоздатьПлатежныеДокументыНаСервере(АктуализированыРеквизитыПлатежаВБюджет = Неопределено)
	...
	СтруктураПараметров.СчетОрганизации = Объект.СчетОрганизации;
	СтруктураПараметров.СпособОплаты    = Объект.СпособОплаты;
	СтруктураПараметров.Платежи =
