...
Процедура Конвертация_ПослеЗагрузкиОбъекта(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	ИначеЕсли ИмяТипаОбъекта = "Документ" Тогда
		...
		Если Константы.ВыполнятьСинхронизациюV77.Получить() Тогда
			...
			СинхронизацияДанныхV77ВызовСервера.ДобавитьСтрокуВТаблицуСоответствий(Параметры.ТаблицаСоответствийДокументов, Объект);
		Иначе
			Если ТипЗнч(Объект) = Тип("ДокументОбъект.ВводНачальныхОстатков") Тогда
...
Процедура Конвертация_ПередЗагрузкойДанных(ФайлОбмена, Отказ) Экспорт
	...
	НаборКонстант = Константы.СоздатьНабор("ИспользоватьВыпускПродукции, ВедетсяПроизводственнаяДеятельность, ВедетсяРозничнаяТорговля, ИспользуетсяОСНО, 
	...
	НаборКонстант.ИспользоватьВыпускПродукции = Истина;
	...
	Если Константы.ВыполнятьСинхронизациюV77.Получить() Тогда
		ТаблицаСоответствийДокументов = Новый ТаблицаЗначений;
		ТаблицаСоответствийДокументов.Колонки.Добавить("Документ77");
		ТаблицаСоответствийДокументов.Колонки.Добавить("Объект");
		ТаблицаСоответствийДокументов.Колонки.Добавить("ПредставлениеОбъекта");
		Параметры.Вставить("ТаблицаСоответствийДокументов",ТаблицаСоответствийДокументов);
	КонецЕсли;
	
	//*********************************************************************************************
	
	ВидНоменклатуры = Справочники.ВидыНоменклатуры;
	
	ВидНоменклатуры.СоздатьЭлементыСпецодежда();
	ВидНоменклатуры.СоздатьЭлементыВозвратнаяТара();
	ВидНоменклатуры.СоздатьЭлементыОС();
	ВидНоменклатуры.СоздатьЭлементыКомиссияНаПродажу();
	ВидНоменклатуры.СоздатьЭлементыПроизводство();
	ВидНоменклатуры.СоздатьЭлементыУслугаЕНВД();
	ВидНоменклатуры.СоздатьЭлементыМатериал();
	ВидНоменклатуры.НайтиСоздатьЭлементыТовар();
	ВидНоменклатуры.НайтиСоздатьЭлементыУслуга();
...
Процедура Конвертация_ПослеЗагрузкиДанных() Экспорт
	...
	Если Константы.ВыполнятьСинхронизациюV77.Получить() Тогда
		...
		Параметры.Вставить("АдресТаблицыСоответствийДокументов",АдресТаблицыСоответствий);
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	
	Организация = Неопределено;
	Если ТипЗнч(Параметры) = Тип("Структура") И Параметры.Свойство("Организация") Тогда
		Организация = Параметры.Организация;
	КонецЕсли;
	
	Попытка
		РегистрыСведений.УчетнаяПолитика.УстановитьИспользованиеСчета40(Организация);
	Исключение
		// Здесь принято не оставлять следов
	КонецПопытки;
	...
	Для Каждого Запись Из НаборЗаписей Цикл
		Запись.ОсновноеРабочееМестоВОрганизации = Истина;
	КонецЦикла;
	...
	НастройкиУчета.УстановитьКонстантуИспользуетсяКассоваяКнига();
	...
	Если ТипЗнч(Параметры) = Тип("Структура") И Параметры.Свойство("Организация") <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "<> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)", "= &Организация");
		Запрос.УстановитьПараметр("Организация", Параметры.Организация);
	КонецЕсли;
	
	ТекущееКоличествоСотрудников = Запрос.Выполнить().Выгрузить();

	Если ТекущееКоличествоСотрудников.Количество() > 0 Тогда
		МаксимальноеКоличествоСотрудниковВОрганизации = ТекущееКоличествоСотрудников[0].КоличествоСотрудников;
		
		Если МаксимальноеКоличествоСотрудниковВОрганизации > 60 
			И Константы.РасчетЗарплатыДляНебольшихОрганизаций.Получить() Тогда
...
Процедура ПКО_ПослеЗагрузки_Материалы(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован,
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
	
	Если НЕ Объект.ЭтоГруппа Тогда
		ВидыНоменклатуры = Справочники.ВидыНоменклатуры;
		
		Если Объект.Комментарий = "10.9" Тогда
			НаименованиеДляПоиска = "Инвентарь и хозяйственные принадлежности";
		ИначеЕсли Объект.Комментарий = "10.10" Тогда
			НаименованиеДляПоиска = "Материалы спец. назначения (10.10)";
		Иначе
			НаименованиеДляПоиска = ?(Объект.Комментарий = "10.1", "Материалы", "Материалы (" + Объект.Комментарий + ")");
		КонецЕсли;
		
		СчетУчета = ПланыСчетов.Хозрасчетный.СырьеИМатериалы;
		
		Если Объект.Комментарий = "10.2" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.ПокупныеПолуфабрикатыИКомплектующие;
		ИначеЕсли Объект.Комментарий = "10.3" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.Топливо;
		ИначеЕсли Объект.Комментарий = "10.4" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.Тара;
		ИначеЕсли Объект.Комментарий = "10.5" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.ЗапасныеЧасти;
		ИначеЕсли Объект.Комментарий = "10.6" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.ПрочиеМатериалы;
		ИначеЕсли Объект.Комментарий = "10.7" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.МатериалыПереданныеВПереработку;
		ИначеЕсли Объект.Комментарий = "10.8" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.СтроительныеМатериалы;
		ИначеЕсли Объект.Комментарий = "10.9" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежности;
		ИначеЕсли Объект.Комментарий = "10.10" Тогда
			СчетУчета = ПланыСчетов.Хозрасчетный.СпецоснасткаИСпецодеждаНаСкладе;
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ВидыНоменклатуры.НайтиПоНаименованию(НаименованиеДляПоиска, Истина)) Тогда
			...
			ЭлементСсылка = ВидыНоменклатуры.НайтиПоНаименованию(НаименованиеДляПоиска, Истина);
		Иначе
			ЭлементСсылка = ВидыНоменклатуры.НайтиПоНаименованию(НаименованиеДляПоиска, Истина);
			...
			Если НЕ ЭлементСсылка.Пустая() Тогда
				...
				|ГДЕ
				|	СчетаУчетаНоменклатуры.ВидНоменклатуры = &ЭлементСсылка";
				...
				Если Результат.Количество() = 0 Тогда
					...
					Запись.ВидНоменклатуры                      = ЭлементСсылка;
					Запись.СчетУчета                            = СчетУчета;
					Запись.СчетУчетаНДСПоПриобретеннымЦенностям = ПланыСчетов.Хозрасчетный.НДСпоПриобретеннымМПЗ;
					Запись.СчетУчетаНДСПоРеализации             = ПланыСчетов.Хозрасчетный.ПрочиеРасходы;
					Запись.СчетУчетаНДСУплаченногоНаТаможне     = ПланыСчетов.Хозрасчетный.НДСуплачиваемыйТаможеннымОрганам;
					...
					Набор.Записать(Ложь);
				Иначе
					Набор.Отбор.ВидНоменклатуры.Значение = ЭлементСсылка;
					Набор.Прочитать();
		...
		Если НЕ ЭлементСсылка.Пустая() Тогда
			Объект.ВидНоменклатуры = ЭлементСсылка;
			Объект.Записать();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_Номенклатура(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если НЕ Объект.ЭтоГруппа Тогда
		...
		ЭлементСсылка = ВидНоменклатуры.НайтиПоНаименованию(Объект.Комментарий, Истина);
                Объект.Комментарий = "";
...
Процедура ПКО_ПослеЗагрузки_Оборудование(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
	
	Если НЕ Объект.ЭтоГруппа Тогда
		ВидНоменклатуры = Справочники.ВидыНоменклатуры;
		
		ЭлементСсылка = ВидНоменклатуры.НайтиПоНаименованию("Оборудование к установке", Истина);
...
Процедура ПКО_ПослеЗагрузки_ОбъектыСтроительства_08_4(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
                                           ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
	
	Если НЕ Объект.ЭтоГруппа Тогда
		ВидНоменклатуры = Справочники.ВидыНоменклатуры;
		
		ЭлементСсылка = ВидНоменклатуры.НайтиПоНаименованию("Оборудование (объекты основных средств)", Истина);
...
Процедура ПКО_ПослеЗагрузки_ИсправлениеСчетаФактуры_СчетФактура(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если Объект.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию
		И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации")
		И ТипЗнч(Объект.ДокументОснование.ДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование.ДокументРеализации, "ДокументРеализации, ВидОперации");
...
Процедура ПКО_ПослеЗагрузки_ОперацияБух(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Для каждого Корреспонденция из Объект.Движения.Хозрасчетный Цикл
		...
		Если НЕ ПолучитьФункциональнуюОпцию("ВестиУчетПоПодразделениям") Тогда
			Если ЗначениеЗаполнено(Корреспонденция.ПодразделениеДт) Тогда
				Корреспонденция.ПодразделениеДт = Неопределено;
			КонецЕсли;
			Если ЗначениеЗаполнено(Корреспонденция.ПодразделениеКт) Тогда
				Корреспонденция.ПодразделениеКт = Неопределено;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПКО_ПослеЗагрузки_ПлатежноеПоручение(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Если Объект.ПеречислениеВБюджет Тогда
		...
		НалогиИПлатежи = Справочники.ВидыНалоговИПлатежейВБюджет;
		Результат = НалогиИПлатежи.НайтиПоРеквизиту("КодБК", Объект.КодБК);
		
		Если Результат = НалогиИПлатежи.ПустаяСсылка() Тогда
			Если ЗначениеЗаполнено(СсылкаПредопределенного) Тогда
				Объект.Налог = СсылкаПредопределенного;
			КонецЕсли;
		Иначе
			Объект.Налог = Результат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПКО_ПослеЗагрузки_ИсторияРегистрацийВНалоговомОргане(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	...
	Набор.Отбор.СтруктурнаяЕдиница.Установить(Объект.СтруктурнаяЕдиница);
	Набор.Прочитать();
...
Процедура ПКО_ПослеЗагрузки_ИсторияРегистрацийВНалоговомОрганеВторичный(ФайлОбмена, Отказ, Ссылка, Объект, ПараметрыОбъекта, ОбъектМодифицирован, 
	ИмяТипаОбъекта, ОбъектНайден, НаборЗаписей) Экспорт
