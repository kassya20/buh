...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПодготовкаНулевойОтчетности Тогда
		Элементы.ГруппаШапка.Видимость = НЕ ЗначениеЗаполнено(Объект.Организация);
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРазделыВводаОстатков
...
Процедура ОбновитьФормуНаСервере()
	...
	Если ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком() Тогда
		Элементы.ДатаВводаОстатков.ТолькоПросмотр = Истина;
		Элементы.ДатаВводаОстатков.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	КонецЕсли;
...
Процедура ПолучитьРазделыОстатков()
	...
	СписокИспользуемыхРазделов = Перечисления.РазделыУчетаДляВводаОстатков.ДанныеВыбораПоФункциональнымОпциям();
	...
	// Топливо в баке - только при включенной функциональности
	Если ПолучитьФункциональнуюОпцию("ВедетсяУчетПоПутевымЛистам") Тогда
		НоваяСтрока = Объект.РазделыВводаОстатков.Добавить();
		НоваяСтрока.Раздел             = НСтр("ru='Топливо'");
		НоваяСтрока.ИмяРаздела         = "Топливо";
		НоваяСтрока.ЕстьОстаткиРаздела = МенеджерОбработки.ЕстьОстаткиРаздела(НоваяСтрока.ИмяРаздела, ПараметрыПроверки);
		НоваяСтрока.Описание           = НСтр("ru='Топливо в баке автомобиля, учитываемое по путевым листам'");
	КонецЕсли;
	
	// Основные средства - только при включенной функциональности
	Если СписокИспользуемыхРазделов.НайтиПоЗначению(Перечисления.РазделыУчетаДляВводаОстатков.ОсновныеСредства) <> Неопределено Тогда
		НоваяСтрока = Объект.РазделыВводаОстатков.Добавить();
	...
	Если СписокИспользуемыхРазделов.НайтиПоЗначению(Перечисления.РазделыУчетаДляВводаОстатков.НематериальныеАктивыИНИОКР) <> Неопределено Тогда
		НоваяСтрока = Объект.РазделыВводаОстатков.Добавить();
	...
	// Платежные карты - только при включенной функциональности
	Если ПолучитьФункциональнуюОпцию("ВестиУчетРасчетовСКонтрагентами")
		И ПолучитьФункциональнуюОпцию("ИспользоватьОплатуПоПлатежнымКартам") Тогда
		НоваяСтрока = Объект.РазделыВводаОстатков.Добавить();
		НоваяСтрока.Раздел     = НСтр("ru='Платежные карты'");
		НоваяСтрока.ИмяРаздела = "ПлатежныеКарты";
		НоваяСтрока.ЕстьОстаткиРаздела = МенеджерОбработки.ЕстьОстаткиРаздела(НоваяСтрока.ИмяРаздела, ПараметрыПроверки);
		НоваяСтрока.Описание   = НСтр("ru='Оплаты по картам, находящиеся в обработке у банка'");
	КонецЕсли;
	
	// Зарплата и Подотчет - только при включенной функциональности
	//            а для ИП при условии что есть наемные работники
	Если СписокИспользуемыхРазделов.НайтиПоЗначению(Перечисления.РазделыУчетаДляВводаОстатков.РасчетыСПерсоналомПоОплатеТрудаИДепонентам) <> Неопределено Тогда
		
		Если Не ЭтоПредприниматель Или УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Объект.Организация) Тогда
			НоваяСтрока = Объект.РазделыВводаОстатков.Добавить();
