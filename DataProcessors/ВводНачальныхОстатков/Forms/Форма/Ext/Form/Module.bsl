...
Процедура УправлениеФормой()
	...
	ПлательщикНДСПрименяющийУСН = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Объект.Организация, ДатаУчетнойПолитики);
	...
	Элементы.НДСПоРеализации.Видимость = (ПлательщикНДС ИЛИ ПлательщикНДСПрименяющийУСН) И НЕ УпрощенныйУчетНДС;

КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьСписокСчетовИсключений(ДатаВводаОстатков)

	СписокИсключений = Новый Массив;
	СписокИсключений.Добавить(Планысчетов.Хозрасчетный.СпецодеждаВЭксплуатацииВспомогательный);
	СписокИсключений.Добавить(Планысчетов.Хозрасчетный.СпецоснасткаВЭксплуатацииВспомогательный);
	...
	И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(ДатаВводаОстатков );
...
Процедура ЗаполнитьСписокРазделов()
	...
	СтрокаРаздела.КодыСчетов     = ",76.01.2,76.01.9,76.18,";
	...
	СтрокаРаздела.КодыИсключений = ",76.03,76.04,76.01.1,76.07.1,76.07.5,76.07.9,76.10,76.14,76.21,76.27.1,76.27.5,76.27.9,76.37.1,76.37.5,76.37.9,76.41,76.49,76.АВ,76.ВА,76.Н,76.ОТ,76.51,76.52,";
	...
	ИспользуетсяУчетЗарплатыИКадров   = УчетЗарплаты.ИспользуетсяПодсистемаУчетаЗарплатыИКадров();
	...
	Если ВедетсяУчетОсновныхСредств Тогда
		...
		Если ПрименяетсяФСБУ25 Тогда
			...
			СтрокаРаздела.КодыИсключений = СтрокаРаздела.КодыИсключений + "01.03,02.03,01.К,";
			
			СтрокаРаздела = СписокРазделов.Добавить();
			СтрокаРаздела.РазделУчета    = Перечисления.РазделыУчетаДляВводаОстатков.АрендаЛизинг;
			СтрокаРаздела.КодыГрупп      = "";
			СтрокаРаздела.КодыСчетов     = ",01.03,02.03,01.К,76.07.1,76.07.5,76.07.9,76.27.1,76.27.5,76.27.9,76.37.1,76.37.5,76.37.9,";
			СтрокаРаздела.КодыИсключений = "";
	...
	Если ВедетсяУчетНМА Тогда
		...
		СтрокаРаздела.КодыИсключений = ",04.09,";
		
	КонецЕсли;
	
	Если ВедетсяУчетОсновныхСредств Или ВедетсяУчетНМА Тогда
		...
		Если Не ВедетсяУчетНМА Тогда
			СтрокаРаздела.КодыИсключений = ",08.05,08.11,";
		ИначеЕсли Не ВедетсяУчетОсновныхСредств Тогда
			СтрокаРаздела.КодыГрупп   = ",07,";
			СтрокаРаздела.КодыСчетов  = ",08.05,08.08,08.11,";
			СтрокаРаздела.КодыИсключений = ",08,";
		КонецЕсли;
	...
	Если Не ВедетсяУчетСпецодеждыСпецоснастки Тогда
		СтрокаРаздела.КодыИсключений = ",10.10,10.11.1,10.11.2,10.21.1,10.21.2,";
	КонецЕсли;
	...
	Если Не ВедетсяУчетОсновныхСредств Тогда
		СтрокаРаздела.КодыИсключений = ",19.01" + ?(ВедетсяУчетНМА, "", ",19.02") + ",19.08,";
	ИначеЕсли Не ВедетсяУчетНМА Тогда
		СтрокаРаздела.КодыИсключений = ",19.02,";
	КонецЕсли;
	
	Если ИспользуетсяУчетЗарплатыИКадров Тогда
	...
	Если ВедетсяУчетСпецодеждыСпецоснастки Тогда
...
Процедура ЗаполнитьСпискиСчетов()
	...
	ПрименяетсяФСБУ25 = УчетнаяПолитика.ПрименяетсяФСБУ25(Объект.Организация, Объект.ДатаВводаОстатков);
	
	ЗаполнитьСписокРазделов();

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СписокИсключений", ПолучитьСписокСчетовИсключений(Объект.ДатаВводаОстатков));
...
Процедура ОбновитьОстатки()
	...
	СтавкиНачисленияНДС = Новый Массив();
	СтавкиНачисленияНДС.Добавить(Перечисления.СтавкиНДС.НДС10_110);
	СтавкиНачисленияНДС.Добавить(Перечисления.СтавкиНДС.НДС18_118);
	СтавкиНачисленияНДС.Добавить(Перечисления.СтавкиНДС.НДС20_120);
	СтавкиНачисленияНДС.Добавить(Перечисления.СтавкиНДС.НДС22_122);
	...
	Запрос.УстановитьПараметр("Подразделение", Объект.ПодразделениеОрганизации);
	Запрос.УстановитьПараметр("СписокИсключений", ПолучитьСписокСчетовИсключений(Объект.ДатаВводаОстатков));
	Запрос.УстановитьПараметр("СтавкиНачисленияНДС", СтавкиНачисленияНДС);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Если ПлательщикНДС И НЕ УпрощенныйУчетНДС Тогда
		...
		|						КОГДА ВводНачальныхОстатковДанныеПоСФНДС.СтавкаНДСНеПодтверждена0 В (&СтавкиНачисленияНДС)
		|							ТОГДА ВводНачальныхОстатковДанныеПоСФНДС.НДСПодтверждено0
		|						ИНАЧЕ 0
		|					КОНЕЦ)
		|			ИНАЧЕ 0
