...
// Определяет, используются ли склады при вводе начальных остатков
//
// Возвращаемое значение:
//  Булево
Функция ВедетсяСкладскойУчет() Экспорт
	...
	Для НомерСубконто = 1 По СвойстваСчета.КоличествоСубконто Цикл
		Если СвойстваСчета["ВидСубконто" + НомерСубконто] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Склады Тогда
			ВедетсяСкладскойУчет = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ВедетсяСкладскойУчет И ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоСкладовБухгалтерскийУчет");
...
Функция ПодготовитьТабличнуюЧасть_ПлатежныеКарты(ТаблицаДанных, ПараметрыДокумента) Экспорт
	...
	Запрос.УстановитьПараметр("РазделУчета",         ПараметрыДокумента.РазделУчета);
	Запрос.УстановитьПараметр("СчетаПлатежныеКарты", РазрешенныеСчета("ПлатежныеКарты"));
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ТаблицаДанных.Ссылка КАК Ссылка,
	|	ВидыОплатОрганизаций.СчетУчетаРасчетов КАК СчетУчета,
	|	ВидыОплатОрганизаций.Контрагент КАК Субконто1,
	|	ВидыОплатОрганизаций.ДоговорКонтрагента КАК Субконто2,
	|	ВТ_ТаблицаДанных.Сумма КАК Сумма,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	ВТ_ТаблицаДанных.Сумма КАК ВалютнаяСумма,
	|	ВТ_ТаблицаДанных.Сумма КАК СуммаНУ
	|ИЗ
	|	ВТ_ТаблицаДанных КАК ВТ_ТаблицаДанных
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыОплатОрганизаций КАК ВидыОплатОрганизаций
	|		ПО ВТ_ТаблицаДанных.ВидОплаты = ВидыОплатОрганизаций.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|		ПО ВводНачальныхОстатковБухСправка.Ссылка = ВводНачальныхОстатков.Ссылка
	|ГДЕ
	...
	ДанныеПоПлатежнымКартам = РезультатЗапроса[1].Выгрузить();
	
	ВыборкаДокументов = РезультатЗапроса[2].Выбрать();
	Если ВыборкаДокументов.Следующий() Тогда
		СсылкаНаДокумент = ВыборкаДокументов.Ссылка;
		
		Для Каждого СтрокаПлатежнойКарты Из ДанныеПоПлатежнымКартам Цикл
			Если Не ЗначениеЗаполнено(СтрокаПлатежнойКарты.Ссылка) Тогда
				СтрокаПлатежнойКарты.Ссылка = СсылкаНаДокумент;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ДанныеПоПлатежнымКартам;
	
КонецФункции

Процедура ДобавитьСтрокуПоОстаткуВзносовДляУменьшенияНалога(ТаблицаНалогиИВзносы, Организация, СуществующиеДокументы, ДатаВводаОстатков) Экспорт
	
	Если ДатаВводаОстатков < УчетСтраховыхВзносовИП.ДатаПереходаНаУпрощенныйЗачетВзносов() Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из ТаблицаНалогиИВзносы Цикл
		ВзносыСвышеПредела = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.ВидНалога, "ВидНалога")
			= Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела;
		ДобавитьСтрокуПослеТекущей = ВзносыСвышеПредела
			И СтрокаТаблицы.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела
			И СтрокаТаблицы.СчетУчета = ПланыСчетов.Хозрасчетный.ПФР_ОПС_ИП;
		Если ДобавитьСтрокуПослеТекущей Тогда
			СтрокаОстаткаВзносов = ТаблицаНалогиИВзносы.Вставить(ТаблицаНалогиИВзносы.Индекс(СтрокаТаблицы) + 1);
			ЗаполнитьЗначенияСвойств(СтрокаОстаткаВзносов, СтрокаТаблицы);
			СтрокаОстаткаВзносов.ВидНалога = НСтр("ru='      в том числе остаток для уменьшения налога'");
			СтрокаОстаткаВзносов.Задолженность = СтрокаОстаткаВзносов.СуммаВзносаДляУменьшенияНалога;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
#Область ОбслуживаниеУпрощенногоИнтерфейса
...
Функция РазрешенныеСчета(Раздел = Неопределено) Экспорт
	...
	Если Раздел = "Банк" Или Раздел = Неопределено Тогда
		...
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ЦифровойРубль);                          // 53
	КонецЕсли;
	...
	Если Раздел = "ПлатежныеКарты" Или Раздел = Неопределено Тогда
		МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам);               // 57.03);
	КонецЕсли;
...
Функция РазрешенныеСчетаНулевка(Раздел = Неопределено) Экспорт
...
Функция ЕстьДокументыСЗапрещеннымиСчетами(Организация) Экспорт
	...
	Если НулеваяОтчетность.ДоступноУпрощенноеФормирование(Организация, ТекущаяДатаСеанса()) Тогда
		РазрешенныеСчета = РазрешенныеСчетаНулевка();
	Иначе
		РазрешенныеСчета = РазрешенныеСчета();
	КонецЕсли;
	...
	|	ВводНачальныхОстатков.Ссылка
	|ИЗ
	|	Документ.ВводНачальныхОстатков.Топливо КАК ВводНачальныхОстатков
	|ГДЕ
	|	ВводНачальныхОстатков.Ссылка.Проведен
	|	И ВводНачальныхОстатков.Ссылка.Организация = &Организация
	|	И ВводНачальныхОстатков.СчетУчета <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТопливоВБаке)
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Функция ЕстьМногострочныеДокументы(Организация) Экспорт
	
	МассивТабличныхЧастей = Новый Массив;
	МассивТабличныхЧастей.Добавить("ОС");
	МассивТабличныхЧастей.Добавить("НМА");
	МассивТабличныхЧастей.Добавить("РасчетыСКонтрагентами");
	МассивТабличныхЧастей.Добавить("РасчетыПоНалогамИСборам");
	МассивТабличныхЧастей.Добавить("РасчетыПоЗаработнойПлате");
	МассивТабличныхЧастей.Добавить("БухСправка");
	МассивТабличныхЧастей.Добавить("РасчетыСПодотчетнымиЛицами");
	МассивТабличныхЧастей.Добавить("МПЗОтгруженные");
	МассивТабличныхЧастей.Добавить("РасходыБудущихПериодов");
	МассивТабличныхЧастей.Добавить("НоменклатураНаСкладе");
	МассивТабличныхЧастей.Добавить("СпецодеждаИСпецоснасткаВЭксплуатации");
	МассивТабличныхЧастей.Добавить("МатериалыПереданныеВПереработку");
	МассивТабличныхЧастей.Добавить("ТоварыВРозничнойТорговлеАТТ");
	МассивТабличныхЧастей.Добавить("ТоварыВРозничнойТорговлеНТТ");
	МассивТабличныхЧастей.Добавить("ДанныеПоНДС");
	...
	|	ВводНачальныхОстатков.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВводНачальныхОстатков.#ИмяТаблицы# КАК ВводНачальныхОстатков
	|ГДЕ
	|	ВводНачальныхОстатков.Ссылка.Проведен
	|	И ВводНачальныхОстатков.Ссылка.Организация = &Организация
	|	И ВводНачальныхОстатков.НомерСтроки = 2";
	
	ТекстЗапроса = "";
	Для Каждого ИмяТаблицы ИЗ МассивТабличныхЧастей Цикл
...
Функция МожноИспользоватьПростуюФормуПомощника(Организация) Экспорт
	...
	//      организация не должна применять ПБУ 18 (в варианте, требующем учета ПР и ВР)
	//      в программе не включена функциональная опция Вести учет прослеживаемых товаров 
	//          или дата ввода остатков меньше даты начала ведения учета прослеживаемых товаров
	Если УчетнаяПолитика.ПрименяетсяУСНДоходыМинусРасходы(Организация, ДатаПроверки)
		...
		Или БухгалтерскийУчетКлиентСервер.ИспользоватьСуммыРазниц(
				БухгалтерскийУчет.ИспользоватьСуммыНалогНаПрибыль(Организация, ДатаПроверки))
		ИЛИ (ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров")
			И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(МенеджерЗаписиРегистра.ДатаВводаНачальныхОстатков)) Тогда
		Результат = Ложь;
	КонецЕсли;
...
Функция МожноИспользоватьПримитивнуюФормуПомощника(Организация) Экспорт
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ДатаРегистрации) Тогда
			ДатаПроверки = ПредельнаяДата;
		Иначе
			ДатаПроверки = Макс(ДатаРегистрации, ПредельнаяДата);
		КонецЕсли;
	КонецЕсли;
	
	Результат = Истина;
	
	// Примитивная форма доступна в случае если это "нулевка"
	Если НЕ НулеваяОтчетность.ДоступноУпрощенноеФормирование(Организация, ТекущаяДатаСеанса()) Тогда
		Результат = Ложь;
	КонецЕсли;
	
	// Примитивная форма доступна в случае если нет документов с необслуживаемыми по умолчанию счетами
	Если ЕстьДокументыСЗапрещеннымиСчетами(Организация) Тогда
		Результат = Ложь;
	КонецЕсли;
	
	// Примитивная форма доступна в случае если в документах только по одной строке
	Если ЕстьМногострочныеДокументы(Организация) Тогда
		Результат = Ложь;
	КонецЕсли;
...
Функция ЕстьОстаткиРаздела(ИмяРаздела, ПараметрыПроверки) Экспорт
	...
	Если ИмяРаздела = "Топливо" Тогда
		...
		|	И ВводНачальныхОстатков.Ссылка.ОтражатьПоСпециальнымРегистрам
		|	И ВводНачальныхОстатков.Ссылка.Проведен
		|	И ВводНачальныхОстатков.Ссылка.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.Материалы)";
		
		Возврат Не Запрос.Выполнить().Пустой();
		
	КонецЕсли;
	
	Если ИмяРаздела = "ПлатежныеКарты" Тогда
		
		Запрос.УстановитьПараметр("СчетаПлатежныеКарты", РазрешенныеСчета("ПлатежныеКарты"));
		
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ВводНачальныхОстатковБухСправка.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ВводНачальныхОстатков.БухСправка КАК ВводНачальныхОстатковБухСправка
		|ГДЕ
		|	ВводНачальныхОстатковБухСправка.Ссылка.Организация = &Организация
		|	И ВводНачальныхОстатковБухСправка.Ссылка.ОтражатьВБухгалтерскомУчете
		|	И ВводНачальныхОстатковБухСправка.Ссылка.ОтражатьВНалоговомУчете
		|	И ВводНачальныхОстатковБухСправка.Ссылка.ОтражатьПоСпециальнымРегистрам
		|	И ВводНачальныхОстатковБухСправка.Ссылка.Проведен
		|	И ВводНачальныхОстатковБухСправка.Ссылка.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.ДенежныеСредства)
		|	И ВводНачальныхОстатковБухСправка.СчетУчета В (&СчетаПлатежныеКарты)
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ОбновитьОстатки(ТаблицаРаздела, ИмяРаздела, ПараметрыОстатков, СуществующиеДокументы = Неопределено, ВспомогательнаяТаблицаРаздела = Неопределено ) Экспорт
	...
	Запрос.УстановитьПараметр("Нулевка",                                    ПараметрыОстатков.Свойство("Нулевка"));
	...
	Если ИмяРаздела = "Деньги" Тогда
		...
		|	И ВводНачальныхОстатков.Ссылка.ОтражатьВБухгалтерскомУчете
		|	И (&Нулевка
		|			ИЛИ ВводНачальныхОстатков.Ссылка.ОтражатьВНалоговомУчете
		|				И ВводНачальныхОстатков.Ссылка.ОтражатьПоСпециальнымРегистрам)
		|	И ВводНачальныхОстатков.Ссылка.Проведен
		|	И ВводНачальныхОстатков.Ссылка.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаДляВводаОстатков.ДенежныеСредства)
		|	И ВводНачальныхОстатков.СчетУчета В(&СчетаКасса)
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_Деньги.Ссылка КАК Ссылка,
		|	ВТ_Деньги.СчетУчета КАК СчетУчета,
		|	ВТ_Деньги.БанковскийСчет КАК БанковскийСчет,
		|	ВТ_Деньги.Сумма КАК Сумма,
		|	ВТ_Деньги.Валюта КАК Валюта,
		|	ВТ_Деньги.ВалютнаяСумма КАК ВалютнаяСумма,
		|	ВТ_Деньги.НаименованиеПоказателя КАК НаименованиеПоказателя,
		|	ВТ_Деньги.СуммаОстаток КАК СуммаОстаток,
	...
	Если ИмяРаздела = "Запасы" Тогда
		...
		|	ВводНачальныхОстатков.Партия КАК Партия
		|ИЗ
		|	Документ.ВводНачальныхОстатков.НоменклатураНаСкладе КАК ВводНачальныхОстатков
		|ГДЕ
		|	ВводНачальныхОстатков.Ссылка.Организация = &Организация
	...
	Если ИмяРаздела = "ОсновныеСредства" Тогда
		...
		|	ВводНачальныхОстатков.ВыработкаКоличествоБУ КАК ВыработкаКоличествоБУ,
		|	ВводНачальныхОстатков.ГрафикАмортизацииБУ КАК ГрафикАмортизацииБУ,
		|	ВводНачальныхОстатков.ДатаПоследнейМодернизацииРегл КАК ДатаПоследнейМодернизацииРегл,
		|	ВводНачальныхОстатков.ДатаПринятияКУчетуРегл КАК ДатаПринятияКУчетуРегл,
		|	ВводНачальныхОстатков.ДатаПриобретенияУСН КАК ДатаПриобретенияУСН,
		|	ВводНачальныхОстатков.ИнвентарныйНомерРегл КАК ИнвентарныйНомерРегл,
		|	ВводНачальныхОстатков.КоэффициентАмортизацииБУ КАК КоэффициентАмортизацииБУ,
		|	ВводНачальныхОстатков.КоэффициентАмортизацииНУ КАК КоэффициентАмортизацииНУ,
		|	ВводНачальныхОстатков.КоэффициентУскоренияБУ КАК КоэффициентУскоренияБУ,
		|	ВводНачальныхОстатков.МетодНачисленияАмортизацииНУ КАК МетодНачисленияАмортизацииНУ,
		|	ВводНачальныхОстатков.МОЛРегл КАК МОЛРегл,
		|	ВводНачальныхОстатков.НазваниеДокументаМодернизацииРегл КАК НазваниеДокументаМодернизацииРегл,
		|	ВводНачальныхОстатков.НазваниеДокументаПринятияКУчетуРегл КАК НазваниеДокументаПринятияКУчетуРегл,
		...
		|	ВводНачальныхОстатков.НачислятьАмортизациюБУ КАК НачислятьАмортизациюБУ,
		|	ВводНачальныхОстатков.НачислятьАмортизациюНУ КАК НачислятьАмортизациюНУ,
		|	ВводНачальныхОстатков.НомерДокументаМодернизацииРегл КАК НомерДокументаМодернизацииРегл,
		|	ВводНачальныхОстатков.НомерДокументаПринятияКУчетуРегл КАК НомерДокументаПринятияКУчетуРегл,
		|	ВводНачальныхОстатков.ОбъемПродукцииРаботБУ КАК ОбъемПродукцииРаботБУ,
		|	ВводНачальныхОстатков.ОбъемПродукцииРаботДляВычисленияАмортизацииБУ КАК ОбъемПродукцииРаботДляВычисленияАмортизацииБУ,
		|	ВводНачальныхОстатков.ПараметрВыработкиБУ КАК ПараметрВыработкиБУ,
		...
		|	ВводНачальныхОстатков.ПорядокВключенияСтоимостиВСоставРасходовНУ КАК ПорядокВключенияСтоимостиВСоставРасходовНУ,
		|	ВводНачальныхОстатков.ПорядокВключенияСтоимостиВСоставРасходовУСН КАК ПорядокВключенияСтоимостиВСоставРасходовУСН,
		|	ВводНачальныхОстатков.ПорядокПогашенияСтоимостиБУ КАК ПорядокПогашенияСтоимостиБУ,
		|	ВводНачальныхОстатков.РеквизитыДокументаОплаты КАК РеквизитыДокументаОплаты,
		|	ВводНачальныхОстатков.СостояниеМодернизацииРегл КАК СостояниеМодернизацииРегл,
		|	ВводНачальныхОстатков.СостояниеПринятияКУчетуРегл КАК СостояниеПринятияКУчетуРегл,
		|	ВводНачальныхОстатков.СпециальныйКоэффициентНУ КАК СпециальныйКоэффициентНУ,
		|	ВводНачальныхОстатков.СпособНачисленияАмортизацииБУ КАК СпособНачисленияАмортизацииБУ,
		|	ВводНачальныхОстатков.СпособОтраженияРасходовПоАмортизации КАК СпособОтраженияРасходовПоАмортизации,
		|	ВводНачальныхОстатков.СпособПоступленияРегл КАК СпособПоступленияРегл,
		|	ВводНачальныхОстатков.СрокИспользованияДляВычисленияАмортизацииБУ КАК СрокИспользованияДляВычисленияАмортизацииБУ,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияБУ КАК СрокПолезногоИспользованияБУ,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияНУ КАК СрокПолезногоИспользованияНУ,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияУСН КАК СрокПолезногоИспользованияУСН,
		...
		|	ВводНачальныхОстатков.СчетАмортизацииБУ КАК СчетАмортизацииБУ,
		|	ВводНачальныхОстатков.СчетУчетаБУ КАК СчетУчетаБУ,
		...
		|	ВводНачальныхОстатков.ФактическийСрокИспользованияДо2009 КАК ФактическийСрокИспользованияДо2009,
		|	ВводНачальныхОстатков.Контрагент КАК Контрагент,
		|	ВводНачальныхОстатков.ДоговорКонтрагента КАК ДоговорКонтрагента,
		|	ВводНачальныхОстатков.СпособОтраженияРасходовПоАренднымПлатежам КАК СпособОтраженияРасходовПоАренднымПлатежам,
		|	ВводНачальныхОстатков.Ссылка.Номер КАК Номер,
		|	ВводНачальныхОстатков.НомерСтроки КАК НомерСтроки
		|ИЗ
		|	Документ.ВводНачальныхОстатков.ОС КАК ВводНачальныхОстатков
	...
	Если ИмяРаздела = "НематериальныеАктивы" Тогда
		...
		|	ВводНачальныхОстатков.СчетУчетаБУ КАК СчетУчетаБУ,
		|	ВводНачальныхОстатков.СчетНачисленияАмортизацииБУ КАК СчетНачисленияАмортизацииБУ,
		|	ВводНачальныхОстатков.ПервоначальнаяСтоимостьБУ КАК ПервоначальнаяСтоимостьБУ,
		|	ВводНачальныхОстатков.ПервоначальнаяСтоимостьНУ КАК ПервоначальнаяСтоимостьНУ,
		|	ВводНачальныхОстатков.ПервоначальнаяСтоимостьУСН КАК ПервоначальнаяСтоимостьУСН,
		|	ВводНачальныхОстатков.НакопленнаяАмортизацияБУ КАК НакопленнаяАмортизацияБУ,
		|	ВводНачальныхОстатков.НакопленнаяАмортизацияНУ КАК НакопленнаяАмортизацияНУ,
		|	ВводНачальныхОстатков.НакопленнаяАмортизацияПР КАК НакопленнаяАмортизацияПР,
		|	ВводНачальныхОстатков.НакопленнаяАмортизацияВР КАК НакопленнаяАмортизацияВР,
		|	ВводНачальныхОстатков.НакопленнаяАмортизацияУСН КАК НакопленнаяАмортизацияУСН,
		|	ВводНачальныхОстатков.ДатаПринятияКУчетуРегл КАК ДатаПринятияКУчетуРегл,
		|	ВводНачальныхОстатков.ДатаПриобретения КАК ДатаПриобретения,
		|	ВводНачальныхОстатков.ВидОбъектаУчета КАК ВидОбъектаУчета,
		|	ВводНачальныхОстатков.СпособОтраженияРасходов КАК СпособОтраженияРасходов,
		|	ВводНачальныхОстатков.НачислятьАмортизациюБУ КАК НачислятьАмортизациюБУ,
		|	ВводНачальныхОстатков.НачислятьАмортизациюНУ КАК НачислятьАмортизациюНУ,
		|	ВводНачальныхОстатков.ПорядокСписанияНИОКРНаРасходыНУ КАК ПорядокСписанияНИОКРНаРасходыНУ,
		|	ВводНачальныхОстатков.ТекущаяСтоимостьБУ КАК ТекущаяСтоимостьБУ,
		|	ВводНачальныхОстатков.ТекущаяСтоимостьНУ КАК ТекущаяСтоимостьНУ,
		|	ВводНачальныхОстатков.ТекущаяСтоимостьПР КАК ТекущаяСтоимостьПР,
		|	ВводНачальныхОстатков.ТекущаяСтоимостьВР КАК ТекущаяСтоимостьВР,
		|	ВводНачальныхОстатков.КоэффициентБУ КАК КоэффициентБУ,
		|	ВводНачальныхОстатков.АмортизацияДо2009 КАК АмортизацияДо2009,
		|	ВводНачальныхОстатков.ФактическийСрокИспользованияДо2009 КАК ФактическийСрокИспользованияДо2009,
		|	ВводНачальныхОстатков.ОбъемПродукцииРаботДляВычисленияАмортизации КАК ОбъемПродукцииРаботДляВычисленияАмортизации,
		|	ВводНачальныхОстатков.ПорядокВключенияСтоимостиВСоставРасходовУСН КАК ПорядокВключенияСтоимостиВСоставРасходовУСН,
		|	ВводНачальныхОстатков.РеквизитыДокументаОплаты КАК РеквизитыДокументаОплаты,
		|	ВводНачальныхОстатков.СпециальныйКоэффициентНУ КАК СпециальныйКоэффициентНУ,
		|	ВводНачальныхОстатков.СпособНачисленияАмортизацииБУ КАК СпособНачисленияАмортизацииБУ,
		|	ВводНачальныхОстатков.МетодНачисленияАмортизацииНУ КАК МетодНачисленияАмортизацииНУ,
		|	ВводНачальныхОстатков.СпособПоступления КАК СпособПоступления,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияБУ КАК СрокПолезногоИспользованияБУ,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияНУ КАК СрокПолезногоИспользованияНУ,
		|	ВводНачальныхОстатков.СрокПолезногоИспользованияУСН КАК СрокПолезногоИспользованияУСН,
		|	ВводНачальныхОстатков.Ссылка.Номер КАК Номер
	...
	Если ИмяРаздела = "НалогиИВзносы" Тогда
		...
		|	ВидыНалоговИПлатежейВБюджет.Код КАК Код,
		|	ВЫБОР
		...
		|	ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосбюджет.Налог) КАК ВидПлатежаВБюджет
		|ПОМЕСТИТЬ ВТ_ВидыНалогов
		|ИЗ
		|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
		|ГДЕ
		|	ВидыНалоговИПлатежейВБюджет.СчетУчета В(&СчетаНалогиИВзносы)
		|	И ВидыНалоговИПлатежейВБюджет.СчетУчета <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПФР_ОПС_ИП)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ЗНАЧЕНИЕ(Перечисление.УровниБюджетов.ПустаяСсылка),
		|	ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосбюджет.ВзносыСвышеПредела)
		|ИЗ
		|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
		|ГДЕ
		|	ВидыНалоговИПлатежейВБюджет.СчетУчета В(&СчетаНалогиИВзносы)
		|	И ВидыНалоговИПлатежейВБюджет.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВидыНалоговИПлатежейВБюджет.СчетУчета В(&СчетаНалогиИВзносы)
		|	И ВидыНалоговИПлатежейВБюджет.ВидНалога = ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ФиксированныеВзносы_СвышеПредела)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ВидыНалогов.ВидНалога КАК ВидНалога,
		|	ВТ_ВидыНалогов.УровеньБюджета КАК УровеньБюджета,
		|	ВТ_ВидыНалогов.ВидПлатежаВБюджет КАК ВидПлатежаВБюджет
		...
		|			И ВводНачальныхОстатков.ВидПлатежаВБюджет = ВТ_ВидыНалогов.ВидПлатежаВБюджет
		|ГДЕ
		|	ВводНачальныхОстатков.Ссылка.Организация = &Организация
		|	И ВводНачальныхОстатков.Ссылка.ОтражатьВБухгалтерскомУчете
		|	И (&Нулевка
		...
		|	ВТ_ВидыНалогов.ВидПлатежаВБюджет
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	СУММА(ВводНачальныхОстатков.СуммаКт) КАК Задолженность,
		|	СУММА(ВводНачальныхОстатков.СуммаВзносаДляУменьшенияНалога) КАК СуммаВзносаДляУменьшенияНалога,
		|	ВТ_ВидыНалогов.ВидНалога КАК ВидНалога,
		...
		|	СУММА(ВводНачальныхОстатков.СуммаВзносаДляУменьшенияНалога),
		|	ВводНачальныхОстатков.ВидНалога,
		|	ВводНачальныхОстатков.ВидПлатежаВБюджет,
		|	ВводНачальныхОстатков.РегистрацияВНалоговомОргане,
		...
		|			И ВТ_ВидыНалогов.ВидПлатежаВБюджет = ВТ_ИспользуемыеВидыНалогов.ВидПлатежаВБюджет
		|ГДЕ
		|	ВТ_ВидыНалогов.СчетУчета <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
		|	И ВТ_ИспользуемыеВидыНалогов.ВидНалога ЕСТЬ NULL
	...
	Если ИмяРаздела = "КредитыИЗаймы" Тогда
		...
		|	ВТ_Сводная.СчетУчета КАК СчетУчета,
		|	ВТ_Сводная.Контрагент КАК Контрагент,
		|	ВТ_Сводная.ДоговорКонтрагента КАК ДоговорКонтрагента,
		|	СУММА(ВТ_Сводная.Сумма) КАК Сумма,
		|	ВТ_Сводная.Валюта КАК Валюта,
		|	СУММА(ВТ_Сводная.ВалютнаяСумма) КАК ВалютнаяСумма,
		|	СУММА(ВТ_Сводная.СуммаПроценты) КАК СуммаПроценты,
		|	ВТ_Сводная.СчетУчетаПроценты КАК СчетУчетаПроценты,
		|	ВТ_Сводная.ВидОбязательства КАК ВидОбязательства,
		|	СУММА(ВТ_Сводная.СуммаОстаток) КАК СуммаОстаток,
		|	СУММА(ВТ_Сводная.СуммаПроцентыОстаток) КАК СуммаПроцентыОстаток,
		|	ВТ_Сводная.ПодсказкаПроценты КАК ПодсказкаПроценты,
		|	ВТ_Сводная.ВидПлатежаПоКредитамЗаймамПроценты КАК ВидПлатежаПоКредитамЗаймамПроценты,
		|	ВТ_Сводная.ВидПлатежаПоКредитамЗаймамТело КАК ВидПлатежаПоКредитамЗаймамТело,
	...
	Если ИмяРаздела = "Капитал" Тогда
		...
		|	ВЫБОР
		|		КОГДА &Нулевка
		|			ТОГДА ВводНачальныхОстатков.Ссылка
		|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.ВводНачальныхОстатков.ПустаяСсылка)
		|	КОНЕЦ КАК СсылкаДляГруппировки
	...
	Если ИмяРаздела = "Топливо" Тогда
		...
		|	ВводНачальныхОстатков.Ссылка КАК Ссылка,
		|	ВводНачальныхОстатков.СчетУчета КАК СчетУчета,
		|	ВводНачальныхОстатков.ТранспортноеСредство КАК ТранспортноеСредство,
		|	ВводНачальныхОстатков.Номенклатура КАК Номенклатура,
		|	СУММА(ВводНачальныхОстатков.Количество) КАК Количество,
		|	СУММА(ВводНачальныхОстатков.Сумма) КАК Сумма
		|ИЗ
		...
		|	ВводНачальныхОстатков.СчетУчета,
		|	ВводНачальныхОстатков.Ссылка,
		|	ВводНачальныхОстатков.ТранспортноеСредство,
		|	ВводНачальныхОстатков.Номенклатура";
	...
	Если ИмяРаздела = "ПлатежныеКарты" Тогда
		...
		|	СУММА(ВводНачальныхОстатковБухСправка.Сумма) КАК Сумма
		|ИЗ
		|	Документ.ВводНачальныхОстатков.БухСправка КАК ВводНачальныхОстатковБухСправка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыОплатОрганизаций КАК ВидыОплатОрганизаций
		|		ПО ВводНачальныхОстатковБухСправка.СчетУчета = ВидыОплатОрганизаций.СчетУчетаРасчетов
		|		И ВводНачальныхОстатковБухСправка.Субконто1 = ВидыОплатОрганизаций.Контрагент
		|		И ВводНачальныхОстатковБухСправка.Субконто2 = ВидыОплатОрганизаций.ДоговорКонтрагента
...
Процедура ЗаписатьНаСервереДокументы(СтруктураПараметровДокументов, ТаблицаДанных, ИмяТаблицыДокумента) Экспорт
	...
	Если СтруктураПараметровДокументов.РазделУчета
		= Перечисления.РазделыУчетаДляВводаОстатков.РасчетыПоНалогамИСборам Тогда
		СвернутьСтрокиПоВзносамСвышеПредела(ТаблицаДанных);
	КонецЕсли;
	...
	Для Каждого ДокументОстатков Из СписокДокументов Цикл
		...
		Иначе
			// В режиме "Нулевки" регистрируются только
			// укрупненные показатели баланса и только в бухгалтерском учете
			...
			ДокументОбъект.ОтражатьВНалоговомУчете        = НЕ СтруктураПараметровДокументов.Свойство("Нулевка");
			ДокументОбъект.ОтражатьПоСпециальнымРегистрам = НЕ СтруктураПараметровДокументов.Свойство("Нулевка");
		КонецЕсли;
...
Процедура СвернутьСтрокиПоВзносамСвышеПредела(ТаблицаДанных)
	...
	СтрокиВзносов = ТаблицаДанных.НайтиСтроки(Отбор);
	Для Каждого СтрокаВзносов Из СтрокиВзносов Цикл
		Если ТипЗнч(СтрокаВзносов.ВидНалога) = Тип("Строка") Тогда
			ОстатокВзноса = СтрокаВзносов.СуммаКт;
			СтрокаДляУдаления = СтрокаВзносов;
		КонецЕсли;
	КонецЦикла;
	Если СтрокаДляУдаления <> Неопределено Тогда
		ТаблицаДанных.Удалить(СтрокаДляУдаления);
	КонецЕсли;
	
	СтрокиВзносов = ТаблицаДанных.НайтиСтроки(Отбор);
	Для Каждого СтрокаВзносов Из СтрокиВзносов Цикл
		СтрокаВзносов.СуммаВзносаДляУменьшенияНалога = ОстатокВзноса;
	КонецЦикла;
...
Процедура ОбновитьФинансовыйРезультат(СтруктураПараметровДокументов, ФинансовыйРезультат) Экспорт
	
	ОбновитьОстатки(ФинансовыйРезультат, "ФинансовыйРезультат",
			Новый Структура("Организация, ДатаВводаОстатков, Нулевка",
				СтруктураПараметровДокументов.Организация,СтруктураПараметровДокументов.Дата, СтруктураПараметровДокументов.Свойство("Нулевка")));
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Организация) И Обработки.ВводНачальныхОстатков.МожноИспользоватьПримитивнуюФормуПомощника(Организация) Тогда
		СтандартнаяОбработка = Ложь;
		ВыбраннаяФорма = "ФормаПримитивная";
	ИначеЕсли ЗначениеЗаполнено(Организация) И Обработки.ВводНачальныхОстатков.МожноИспользоватьПростуюФормуПомощника(Организация) Тогда
		СтандартнаяОбработка = Ложь;
