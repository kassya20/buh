...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Завершение отправки в контролирующий орган" 
		...
		Если ЕстьСогласиеНаПолучениеДанныхЕНС Тогда
			...
			КонецЕсли;
			Если ПоказатьСтраницуБыстраяЗагрузкаВключена Тогда
				Элементы.ГруппаДанныеСтраницы.ТекущаяСтраница = Элементы.СтраницаБыстраяЗагрузкаВключена;
			КонецЕсли;
			Если ПоказатьСтраницуОжидатьСогласиеДляБыстройЗагрузки Тогда
				ПоказатьСтраницуОтсутствуетСогласие = Ложь;
				УправлениеФормой();
			КонецЕсли;
		Иначе
			Элементы.ГруппаДанныеСтраницы.ТекущаяСтраница = Элементы.СтраницаОшибкаНеУдалосьОтправитьСогласие;
			ПоказатьСтраницуБыстраяЗагрузкаВключена = Ложь;
			ПоказатьСтраницуОжидатьСогласиеДляБыстройЗагрузки = Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ТекстПояснениеПроблемыСОтсутствиемСогласияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "СогласиеНаРаскрытиеНалоговойТайны" Тогда
		СоздатьСогласиеНаРаскрытиеНалоговойТайны();
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
Процедура ВключитьБыстуюЗагрузку(Команда)
	...
	ОтправитьСогласиеНаРаскрытиеНалоговойТайны();
	ПоказатьСтраницуБыстраяЗагрузкаВключена = Истина;
...
&НаКлиенте
Процедура ПодатьСогласие(Команда)
	
	ОтправитьСогласиеНаРаскрытиеНалоговойТайны();
	ПоказатьСтраницуСРекламойСогласия = Ложь;
	ПереключитьСтраницыПередПолучениемДанных();
	ПоказатьСтраницуОжидатьСогласиеДляБыстройЗагрузки = Истина;
...
Процедура ПодготовитьФормуНаСервере(КонтролироватьНаличиеДанных = Ложь)
	...
	Если Не ИспользоватьСервисДанныхЕНС Тогда
...
Функция ПослеАвторизацииНаСервисеEnsIntegration(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			...
			Если Не РезультатАвторизации.ЕстьСогласиеНаПолучениеДанныхЕНС И ИспользоватьСервисДанныхЕНС Тогда
				ДатаПодачиСогласия = ДатаПодачиСогласия(Объект.Организация);
				Если ДатаПодачиСогласия < ТекущаяДата() - 5*24*60*60 Тогда
					ПоказатьСтраницуОтсутствуетСогласие = Истина;
				Иначе
					ПоказатьСтраницуОжидатьСогласиеДляБыстройЗагрузки = Истина;
				КонецЕсли;
				УправлениеФормой();
...
&НаСервере
Функция ДатаПодачиСогласия(Организация)
	
	Возврат ЕдиныйНалоговыйСчетЛичныйКабинет.ДатаПодачиСогласия(Организация);
