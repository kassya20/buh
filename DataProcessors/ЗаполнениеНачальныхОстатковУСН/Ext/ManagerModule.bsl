...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьНачальныеОстаткиУСН", Истина);
	
КонецПроцедуры

Функция ЗаполнитьНачальныеОстаткиУСН(ПараметрыДляОстатков, АдресХранилища = Неопределено) Экспорт
...
Функция ТекстЗапросаПоВзаиморасчетам(СтруктураПараметров, ДополнительныеПараметрыЗапроса, ВременныеТаблицыДляУничтожения)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВзаиморасчетыКЗ.Контрагент КАК Контрагент,
	|	ВзаиморасчетыКЗ.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВзаиморасчетыКЗ.ДокументРасчетов КАК ДокументРасчетов,
	|	ВзаиморасчетыКЗ.Валюта КАК Валюта,
	|	ВзаиморасчетыКЗ.Сумма КАК Сумма,
	|	ВзаиморасчетыКЗ.ВалютнаяСумма КАК ВалютнаяСумма,
	...
	|		КОГДА ХозрасчетныйОборотыДтКт.СчетКт.Валютный
	|				И ХозрасчетныйОборотыДтКт.ВалютнаяСуммаОборотКт <> 0
	|			ТОГДА ХозрасчетныйОборотыДтКт.ВалютнаяСуммаОборотКт
	|		ИНАЧЕ ХозрасчетныйОборотыДтКт.СуммаОборот
...
Функция ТекстЗапросаПоОстаткамМПЗ(СтруктураПараметров, ДополнительныеПараметрыЗапроса, ВременныеТаблицыДляУничтожения)
	...
	Если УчетнаяПолитикаДоПерехода.СложныйУчетНДС Тогда // остатки из регистра НДСпоПриобретеннымЦенностям - легко и "прямо"
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто1) = ТИП(Справочник.Номенклатура)
		|			ТОГДА КлючиАналитикиУчетаЗатрат.Субконто1
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто2) = ТИП(Справочник.Номенклатура)
		|			ТОГДА КлючиАналитикиУчетаЗатрат.Субконто2
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто3) = ТИП(Справочник.Номенклатура)
		|			ТОГДА КлючиАналитикиУчетаЗатрат.Субконто3
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|	КОНЕЦ КАК Номенклатура,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто1) = ТИП(Справочник.Номенклатура)
		|			ТОГДА ВЫРАЗИТЬ(КлючиАналитикиУчетаЗатрат.Субконто1 КАК Справочник.Номенклатура).Наименование
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто2) = ТИП(Справочник.Номенклатура)
		|			ТОГДА ВЫРАЗИТЬ(КлючиАналитикиУчетаЗатрат.Субконто2 КАК Справочник.Номенклатура).Наименование
		|		КОГДА ТИПЗНАЧЕНИЯ(КлючиАналитикиУчетаЗатрат.Субконто3) = ТИП(Справочник.Номенклатура)
		|			ТОГДА ВЫРАЗИТЬ(КлючиАналитикиУчетаЗатрат.Субконто3 КАК Справочник.Номенклатура).Наименование
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК НоменклатураНаименование,
		|	КлючиАналитикиУчетаЗатрат.СчетЗатрат КАК СчетУчета,
		|	ВЫБОР
		|		КОГДА &УчетЗапасовПоСредней
		|			ТОГДА ЗНАЧЕНИЕ(Документ.ДокументРасчетовСКонтрагентом.ПустаяСсылка)
		|		ИНАЧЕ НДСРаздельныйУчетОстатки.Партия
		|	КОНЕЦ КАК Партия,
		|	КлючиАналитикиУчетаНДС.СчетФактура КАК ДокументРасчетов,
		...
		|	СУММА(НДСРаздельныйУчетОстатки.СуммаБезНДСОстаток + ВЫБОР
		|			КОГДА НДСРаздельныйУчетОстатки.СпособУчетаНДС = ЗНАЧЕНИЕ(Перечисление.СпособыУчетаНДС.УчитываетсяВCтоимости)
		|				ТОГДА НДСРаздельныйУчетОстатки.НДСОстаток
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК Сумма,
		|	СУММА(0) КАК СуммаНДС
		|ПОМЕСТИТЬ ПартииМПЗиНДС
		|ИЗ
		|	РегистрНакопления.НДСРаздельныйУчет.Остатки(&ДатаОстатков, Организация = &Организация) КАК НДСРаздельныйУчетОстатки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаЗатрат КАК КлючиАналитикиУчетаЗатрат
		|		ПО НДСРаздельныйУчетОстатки.АналитикаУчетаЗатрат = КлючиАналитикиУчетаЗатрат.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНДС КАК КлючиАналитикиУчетаНДС
		|		ПО НДСРаздельныйУчетОстатки.АналитикаУчетаНДС = КлючиАналитикиУчетаНДС.Ссылка
		|ГДЕ
		|	КлючиАналитикиУчетаНДС.ВидЦенности В(&ВидыЦенностейМПЗ)
		|	И КлючиАналитикиУчетаЗатрат.СчетЗатрат В(&СчетаУчетаМПЗ_БезГрупп)
		|
		|СГРУППИРОВАТЬ ПО
