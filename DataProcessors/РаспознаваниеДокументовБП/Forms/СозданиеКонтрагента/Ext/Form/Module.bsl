...
Процедура ПриОткрытии(Отказ)
	...
	ОтправитьОповещениеВОсновнуюФорму(ЭтотОбъект.ТекущийЭлемент.Имя);
...
Процедура КомандаОК(Команда)
	...
	Если Записать(ПараметрыЗаписи) Тогда
		ЗначениеОповещения = Новый Структура("Ссылка, СозданВФормеБРД", Объект.Ссылка, Истина);
...
Процедура УправлениеФормой(Форма)
	...
	Если Объект.ОбособленноеПодразделение 
		...
		Элементы.КПППрочие.Видимость = ДоступенКПП;
		Элементы.КПППрочиеКартинка.Видимость = ДоступенКПП;
	...
	Иначе
		...
		Элементы.КППЮрФизЛица.Видимость = ДоступенКПП;
		Элементы.КППЮрФизЛицаКартинка.Видимость = ДоступенКПП;
...
Функция ПолучитьДвоичныеДанныеКартинки(ДокументОбъект, ИмяТаблицы, ПараметрыСвязаннойКолонки)
	...
	СтрокаРеквизитов = ДокументОбъект[ИмяТаблицы].Получить(ПараметрыСвязаннойКолонки.НомерСтроки - 1);
	
	ДвоичныеДанные = СтрокаРеквизитов.ОбластьИзображения.Получить();
	Если ДвоичныеДанные = Неопределено Тогда
		
		Координаты = СтрокаРеквизитов.КоординатыТекста.Получить();
		Если Координаты = Неопределено Тогда
			Координаты = СтрокаРеквизитов;
		КонецЕсли;
		
		ДвоичныеДанныеОбласти = РаспознаваниеДокументовСлужебный.ВырезатьОбластьИзИзображения(
		ДокументОбъект.ИсходноеИзображение,
		Координаты.КоординатаX0,
		Координаты.КоординатаY0,
		Координаты.КоординатаX1,
		Координаты.КоординатаY1);
		
		Если ДвоичныеДанныеОбласти <> Неопределено Тогда
			ДвоичныеДанные = ДвоичныеДанныеОбласти.Получить();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтобразитьРеквизиты()
	...
	Для Каждого РеквизитСоздаваемогоОбъекта Из РеквизитыСоздаваемогоОбъекта Цикл
			...
			Если РеквизитСоздаваемогоОбъекта.Имя = "Наименование" Тогда
				...
				Элементы.ГруппаКонтрагентыНаименованиеЮрФизЛицаКартинка.Видимость = РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 0;
				Если РеквизитСоздаваемогоОбъекта.ВысотаКартинки > 1 Тогда
					Элементы.КонтрагентыНаименованиеЮрФизЛицаКартинка.Высота = Цел(РеквизитСоздаваемогоОбъекта.ВысотаКартинки / 2) + 1;
				КонецЕсли;
				
				Объект.Наименование = РеквизитСоздаваемогоОбъекта.Значение;
				Объект.НаименованиеПолное = РеквизитСоздаваемогоОбъекта.Значение;
			...
			ИначеЕсли РеквизитСоздаваемогоОбъекта.Имя = "ИНН" Тогда
				Объект.ИНН = РаспознаваниеДокументовСлужебный.ИзвлечьЦифрыИзСтроки(РеквизитСоздаваемогоОбъекта.Значение);
			...
			ИначеЕсли РеквизитСоздаваемогоОбъекта.Имя = "КПП" Тогда
				Объект.КПП = РаспознаваниеДокументовСлужебный.ИзвлечьЦифрыИзСтроки(РеквизитСоздаваемогоОбъекта.Значение);
...
&НаКлиенте
Процедура ОтправитьОповещениеВОсновнуюФорму(Знач ИмяЭлемента)
	
	Если СтрНачинаетсяС(ИмяЭлемента, "Наименование") Тогда
		ИмяЭлемента = "Наименование";
	КонецЕсли;
	
	Если СтрНачинаетсяС(ИмяЭлемента, "ИНН") Тогда
		ИмяЭлемента = "ИНН";
	КонецЕсли;
	
	Если СтрНачинаетсяС(ИмяЭлемента, "КПП") Тогда
		ИмяЭлемента = "КПП";
	КонецЕсли;
