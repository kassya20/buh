...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = НавигационнаяСсылкаРезультатЗапросаИОН() Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьВыпискуОперацийПоРасчетамСБюджетом();
	КонецЕсли;
...
Процедура ОбновитьДанныеФормыНаСервере(АдресРезультата)
	...
	ДатаПоследнейСверки = Новый ФорматированнаяСтрока(Формат(ДанныеПоследнейСверки.Дата, "ДЛФ=D"),,,, НавигационнаяСсылкаРезультатЗапросаИОН());
	СсылкаНаЗапросИОН   = ДанныеПоследнейСверки.СсылкаНаЗапросИОН;
...
Процедура УстановитьВидимостьКомандыОтправкиСверки(ВидимостьКомандыОтправки)
	...
	ДоступенЗапросВыписки = ДоступенЗапросВыпискиРасчетовСБюджетом();
	
	Элементы.ГруппаЗапросСверки.Видимость           = ВидимостьКомандыОтправки И ДоступенЗапросВыписки;
	Элементы.ДекорацияОтправленЗапросВФНС.Видимость = Не ВидимостьКомандыОтправки И ДоступенЗапросВыписки;
	Элементы.ГруппаНедоступенЗапросСверки.Видимость = Не ДоступенЗапросВыписки;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Функция НавигационнаяСсылкаРезультатЗапросаИОН()
	
	Возврат "НавигационнаяСсылкаРезультатЗапросаИОН";
	
КонецФункции

&НаКлиенте
Процедура ПоказатьВыпискуОперацийПоРасчетамСБюджетом()
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьВыпискуОперацийПоРасчетамСБюджетомЗавершение", ЭтотОбъект);
	ДокументооборотСКОКлиент.ПолучитьКонтекстЭДО(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьВыпискуОперацийПоРасчетамСБюджетомЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДО.ПоказатьОтветНаЗапросИОН(СсылкаНаЗапросИОН);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДоступенЗапросВыпискиРасчетовСБюджетом()
	
	Возврат ТекущаяДатаСеанса() < НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж();
