...
Процедура СформироватьДокументыВводаОстатковПоРегиструБухгалтерии(СтрокаСписка, Параметры)
	...
	Запрос.УстановитьПараметр("ДатаСвертки",Новый Граница(
		Новый МоментВремениСУточнениемПериода(Параметры.ПериодСвертки - 1, БухгалтерскийУчет.ВерхняяГраницаУточненияПериода()),
		ВидГраницы.Включая));
	Запрос.УстановитьПараметр("ВыбранныеОрганизации",Параметры.Организации);
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если ((ВыборкаПоСчетам.Счет.Вид =  ВидСчета.Активный) ИЛИ (ВыборкаПоСчетам.Счет.Вид =  ВидСчета.АктивноПассивный))
						...
						Для Каждого ТекВидСубконто ИЗ ВыборкаПоСчетам.Счет.ВидыСубконто Цикл
							...
							Иначе 
								НоваяЗапись.СубконтоДт[ТекВидСубконто.ВидСубконто] = ТекВидСубконто.ВидСубконто.ТипЗначения.ПривестиЗначение(Неопределено);
							КонецЕсли;
					...
					Если ((ВыборкаПоСчетам.Счет.Вид =  ВидСчета.Пассивный) ИЛИ (ВыборкаПоСчетам.Счет.Вид =  ВидСчета.АктивноПассивный))
						...
						Для Каждого ТекВидСубконто ИЗ ВыборкаПоСчетам.Счет.ВидыСубконто Цикл
							...
							Иначе
								НоваяЗапись.СубконтоКт[ТекВидСубконто.ВидСубконто] = ТекВидСубконто.ВидСубконто.ТипЗначения.ПривестиЗначение(Неопределено);
							КонецЕсли;
...
Процедура УдалитьДвиженияПоПодчиненномуРегистру(ПараметрыРегистра, ПараметрыОбработки, ПериодическийРегистрСведений = Истина)
	...
	Если ПериодическийРегистрСведений Тогда
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТабРегистраторы.Регистратор КАК Регистратор,
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ДвиженияПослеСверки.Период >= &ПериодСвертки
		|				ТОГДА ДвиженияПослеСверки.НомерСтроки
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК НомерСтроки
		|ИЗ
		|	ТабРегистраторы КАК ТабРегистраторы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный КАК ДвиженияПослеСверки
		|		ПО ТабРегистраторы.Регистратор = ДвиженияПослеСверки.Регистратор
		|
		|СГРУППИРОВАТЬ ПО
	...
	Пока Истина Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ПараметрыРегистра.СпособСвертки <> "Не сворачивать" Тогда
				...
				Если ПериодическийРегистрСведений
					И ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.НомерСтроки) Тогда
					НаборЗаписей.Прочитать();
					
					НомерТекущейЗаписи = НаборЗаписей.Количество();
					Пока НомерТекущейЗаписи > 0 Цикл
						Если (НаборЗаписей[НомерТекущейЗаписи-1].Период < ПараметрыОбработки.ПериодСвертки) Тогда
							НаборЗаписей.Удалить(НомерТекущейЗаписи-1);
						Иначе
