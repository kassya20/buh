...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолноеОписаниеРежимаРаботы = Новый Структура;
	ПолноеОписаниеРежимаРаботы.Вставить("Файловый", ОбщегоНазначения.ИнформационнаяБазаФайловая());
	ПолноеОписаниеРежимаРаботы.Вставить("Автономный", ОбщегоНазначения.ЭтоАвтономноеРабочееМесто());
	...
	ОтображатьОстаткиИЦеныВСпискеТоваров = Константы.ОтображатьОстаткиИЦеныВСпискеТоваров.Отображать();
	ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов = Константы.ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов.Отображать();
	...
	Для Каждого ИмяРегламентногоЗадания Из ИменаРегламентныхЗаданий() Цикл
		РегламентныеЗаданияПрочитать(ИмяРегламентногоЗадания);
	КонецЦикла;
	...
	УстановитьВидимостьДоступностьВнутреннегоЭДО();
	// Конец одсистема "ОбменСКонтрагентами".
...
&НаКлиенте
Процедура ОтображатьОстаткиИЦеныВСпискеТоваровПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОтображатьДополнительнуюИнформациюВСпискеКонтрагентовПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОтображатьДополнительнуюИнформациюВСпискеИмуществоПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВариантОтображенияДополнительнойИнформацииВСпискеКонтрагентыПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВариантОтображенияДополнительнойИнформацииВСпискеТоваровПриИзменении(Элемент)
...
&НаКлиенте
Процедура ОтображатьИтогиПоНачислениямИВыплатамВСпискеСотрудниковПриИзменении(Элемент)
...
&НаКлиенте
Процедура АвтоматическиВосстанавливатьОтборыПоПроизвольнымПолямВСпискахПриИзменении(Элемент)
...
&НаКлиенте
Процедура УдалениеПомеченныхОбъектовИспользованиеПриИзменении(Элемент)

	РегламентныеЗаданияИспользованиеПриИзменении("УдалениеПомеченныхОбъектов", Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура УдалениеПомеченныхОбъектовИспользованиеРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Функция СохранитьЗначениеРеквизита(РеквизитПутьКДанным)
	...
	Если РеквизитПутьКДанным = "ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов"
		Или РеквизитПутьКДанным = "ОтображатьОстаткиИЦеныВСпискеТоваров" Тогда
		
		КонстантаИмя = РеквизитПутьКДанным;
		КонстантаМенеджер = Константы[КонстантаИмя];
		Если ЭтотОбъект[КонстантаИмя] Тогда
			КонстантаЗначение = Перечисления.ВариантыОтображенияДополнительнойИнформацииВСписках.ВПодвале;
		Иначе
			КонстантаЗначение = Перечисления.ВариантыОтображенияДополнительнойИнформацииВСписках.НеОтображать;
		КонецЕсли;
		КонстантаМенеджер.Установить(КонстантаЗначение);
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	// Отображение дополнительной информации в списке "Контрагенты"
	Если (РеквизитПутьКДанным = "ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов"
		ИЛИ РеквизитПутьКДанным = "") Тогда
		Элементы.ВариантОтображенияДополнительнойИнформацииВСпискеКонтрагенты.Доступность = ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов;
	КонецЕсли;
	
	// Отображаение остатков и цен в списке "Товары"
	Если (РеквизитПутьКДанным = "ОтображатьОстаткиИЦеныВСпискеТоваров"
		ИЛИ РеквизитПутьКДанным = "") Тогда
		Элементы.ВариантОтображенияДополнительнойИнформацииВСпискеТоваров.Доступность = ОтображатьОстаткиИЦеныВСпискеТоваров;
	КонецЕсли;
...
Функция ИменаРегламентныхЗаданий()
	...
	Результат.Добавить(Метаданные.РегламентныеЗадания.ОбновлениеЗадачБухгалтера.Имя);
	Результат.Добавить(Метаданные.РегламентныеЗадания.ОбновлениеПроверокКонтролирующимиОрганами.Имя);
	Результат.Добавить(Метаданные.РегламентныеЗадания.УдалениеПомеченныхОбъектов.Имя);
...
Процедура ОпределитьНедоступныеНастройки()
	...
	Если РаботаВМоделиСервиса.РазделениеВключено() Тогда // Разделение включено
	...
	Если РаботаВМоделиСервиса.РазделениеВключено() Или РежимРаботы.Автономный Тогда // Разделение включено или АРМ
		НедоступныеНастройки.Вставить("ИспользоватьПроверкуКонтрагентов");
	КонецЕсли;
	
	Если Не ОбщегоНазначенияБП.ЭтоПростойИнтерфейс() Тогда
		НедоступныеНастройки.Вставить("ОтображатьОстаткиИЦеныВСпискеТоваров");
		НедоступныеНастройки.Вставить("ОтображатьДополнительнуюИнформациюВСпискеКонтрагентов");
		НедоступныеНастройки.Вставить("ОтображатьДополнительнуюИнформациюВСпискеИмущество");
		НедоступныеНастройки.Вставить("ОтображатьИтогиПоНачислениямИВыплатамВСпискеСотрудников");
	КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимостьДоступностьВнутреннегоЭДО()
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ЭлектронныеДокументы.ВнутренниеДокументы")
	   И ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Тогда
		ЭтотОбъект.ИспользоватьВнутренниеДокументыЭДО = Константы["ИспользоватьВнутренниеДокументыЭДО"].Получить();
	Иначе
		Элементы.ГруппаИспользованиеВнутреннегоЭДО.Видимость = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьВнутренниеДокументыЭДОПриИзменении(Элемент)
	
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСКонтрагентами.ЭлектронныеДокументы.ВнутренниеДокументы") Тогда
		Если ЭтотОбъект.ИспользоватьВнутренниеДокументыЭДО Тогда
			ОписаниеОповещения = Новый ОписаниеОповещения("ИспользоватьВнутренниеДокументыЭДОПриИзмененииЗавершение", ЭтотОбъект);
			ТекстПредупреждения = 
				НСтр("ru = 'При использовании функциональности внутреннего электронного документооборота возможно замедление работы системы на медленных компьютерах.'");
			ПоказатьПредупреждение(ОписаниеОповещения, ТекстПредупреждения);
		Иначе
			МодульОбменСКонтрагнетамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбменСКонтрагентамиКлиент");
			МодульОбменСКонтрагнетамиКлиент.УстановитьИспользованиеВнутреннегоЭДО(ЭтотОбъект.ИспользоватьВнутренниеДокументыЭДО);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ИспользоватьВнутренниеДокументыЭДОПриИзмененииЗавершение(Результат) Экспорт
	МодульОбменСКонтрагнетамиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОбменСКонтрагентамиКлиент");
	МодульОбменСКонтрагнетамиКлиент.УстановитьИспользованиеВнутреннегоЭДО(ЭтотОбъект.ИспользоватьВнутренниеДокументыЭДО);
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьВнутренниеДокументыЭДОРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
