...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РежимАвтоматическогоЗакрытияМесяца.ТолькоПросмотр = Не ЭтоПолноправныйПользователь;
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций Тогда
		СписокВыбора = Элементы.РежимАвтоматическогоЗакрытияМесяца.СписокВыбора;
		СписокВыбора.Очистить();
		СписокВыбора.Добавить("ПоОтдельнымОрганизациям", "Включено");
		СписокВыбора.Добавить("Отключено", "Отключено");
	КонецЕсли;
	
	ФО_РасширенныйФункционал = ПолучитьФункциональнуюОпцию("РасширенныйФункционал");
	Элементы.ГруппаРазрешитьВыполнениеОперацииБезКонтроляПредыдущих.Видимость = ФО_РасширенныйФункционал;
	
	УстановитьПривилегированныйРежим(Истина);
	РежимАвтоматическогоЗакрытияМесяца = ЗакрытиеМесяцаВызовСервера.ТекущийРежимАвтоматическогоЗакрытияМесяца();
	УстановитьПривилегированныйРежим(Ложь);
	
	ПрочитатьИспользоватьОтложенноеПроведение();
	ОбновитьПодсказкуИспользоватьОтложенноеПроведение(); 
	
	Если НаборКонстант.Свойство("РазрешитьВыполнениеОперацииБезКонтроляПредыдущих") Тогда
		// Доступно только в расширенном функционале, поэтому может скрываться функциональной опцией.
		РазрешитьВыполнениеОперацииБезКонтроляПредыдущих = ?(НаборКонстант.РазрешитьВыполнениеОперацииБезКонтроляПредыдущих, 1, 0);
		Элементы.ГруппаРазрешитьВыполнениеОперацииБезКонтроляПредыдущих.ТолькоПросмотр =
			НЕ ПравоДоступа("Изменение", Метаданные.Константы["РазрешитьВыполнениеОперацииБезКонтроляПредыдущих"]);
...
Процедура ПриОткрытии(Отказ)
	...
	НачатьОбновлениеПодсказкиСледующегоЗапуска();
...
&НаКлиенте
Процедура РазрешитьВыполнениеОперацииБезКонтроляПредыдущихКонтролироватьПриИзменении(Элемент)
	
	РазрешитьВыполнениеОперацииБезКонтроляПредыдущихПриИзменении(Элемент, Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура РазрешитьВыполнениеОперацииБезКонтроляПредыдущихНеКонтролироватьПриИзменении(Элемент)
	
	РазрешитьВыполнениеОперацииБезКонтроляПредыдущихПриИзменении(Элемент, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура РежимАвтоматическогоЗакрытияМесяцаПриИзменении(Элемент)
	
	ПереключитьРежимАвтоматическогоЗакрытияМесяца();
	Если РежимАвтоматическогоЗакрытияМесяца = "ПоОтдельнымОрганизациям" И ИспользуетсяНесколькоОрганизаций Тогда
		ПоказатьПодсказкуОНеобходимостиНастройкиРасписания();
	Иначе
		НачатьОбновлениеПодсказкиСледующегоЗапуска();
	КонецЕсли;
...
&НаКлиенте
Процедура СледующийЗапускОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если Не ЭтоПолноправныйПользователь И РежимАвтоматическогоЗакрытияМесяца = "ПоВсемОрганизациям" Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Для изменения настроек по всем организациям обратитесь к администратору.'"));
		Возврат;
	КонецЕсли;
	
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("НастройкиЗакрытияМесяцаЗавершение", ЭтотОбъект);
	ИмяФормыЗаписи = "РегистрСведений.НастройкиАвтоматическогоЗакрытияМесяца.Форма.ФормаЗаписи";
	ОткрытьФорму(ИмяФормыЗаписи, , , , , , ОповещениеОЗакрытии);
...
Процедура ОбновитьПодсказкуИспользоватьОтложенноеПроведение()
	...
	Если ИспользоватьОтложенноеПроведение = 1 Или Не ИспользуетсяНесколькоОрганизаций Тогда
		...
		Если ТаблицаОрганизаций.Количество() > 0 Тогда
			...
			Иначе
				...
				Если СтрокаТаблицы.ЕстьПериодыИспользованияОтложенногоПроведения Тогда
					// Если есть периоды применения отложенного проведения, то их даты описаны в тексте причины.
...
&НаКлиенте
Процедура РазрешитьВыполнениеОперацииБезКонтроляПредыдущихПриИзменении(Элемент, Разрешить)

	НаборКонстант.РазрешитьВыполнениеОперацииБезКонтроляПредыдущих = Разрешить;
...
&НаСервере
Процедура ПереключитьРежимАвтоматическогоЗакрытияМесяца()
	
	МенеджерРегистра = РегистрыСведений.НастройкиАвтоматическогоЗакрытияМесяца;
	МенеджерРегистра.ПереключитьРежимАвтоматическогоЗакрытияМесяца(РежимАвтоматическогоЗакрытияМесяца);
	
КонецПроцедуры

#Область ПодсказкаЗапускаАвтоматическогоЗакрытияМесяца

&НаКлиенте
Процедура ПоказатьПодсказкуОНеобходимостиНастройкиРасписания()
	
	Элементы.СледующийЗапускЗадания.Заголовок =
		НСтр("ru = 'Не будет выполнен, требуется настроить расписание как минимум для одной организации.'");
	Элементы.ГруппаРасписание.ЦветФона = ЗакрытиеМесяцаВызовСервера.ПолучитьПредупреждающийЦвет();
	ДобавитьГиперссылкуДляОткрытияНастроек();
	
КонецПроцедуры

&НаКлиенте
Процедура НачатьОбновлениеПодсказкиСледующегоЗапуска()
	
	ВыполнениеВключено = РежимАвтоматическогоЗакрытияМесяца <> "Отключено";
	
	Если НЕ ИспользуетсяНесколькоОрганизаций И ВыполнениеВключено Тогда
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПроверкаАктуальностиЗавершение",
			ЭтотОбъект, Новый Структура("Форма", ЭтотОбъект));
		ЗакрытиеМесяцаКлиент.ЗапуститьПроверкуАктуальностиДляПодсказки(
			ЭтотОбъект, ОповещениеОЗавершении, ОсновнаяОрганизация());
	Иначе
...
&НаКлиенте
Процедура НастройкиЗакрытияМесяцаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если ЗакрытиеМесяцаВызовСервера.ТекущийРежимАвтоматическогоЗакрытияМесяца() = "Отключено" Тогда
		ПоказатьПодсказкуОНеобходимостиНастройкиРасписания();
	Иначе
		НачатьОбновлениеПодсказкиСледующегоЗапуска();
	КонецЕсли;
...
Процедура ОбновитьПодсказкуСледующегоЗапуска()
	...
	ВыполнениеВключено = РежимАвтоматическогоЗакрытияМесяца <> "Отключено";
	ПараметрыЗадания = ПараметрыЗаданияАвтоматическогоЗакрытияМесяца(Организация);
	ПараметрыЗадания.ВыполнениеВключено = ПараметрыЗадания.ВыполнениеВключено Или ВыполнениеВключено;
	ПараметрыЗадания.Вставить("РежимАвтоматическогоЗакрытияМесяца", РежимАвтоматическогоЗакрытияМесяца);
	
	ЗакрытиеМесяцаКлиент.ОбновитьПодсказкуСледующегоЗапуска(ЭтотОбъект, ПараметрыЗадания);
	
	Если ВыполнениеВключено  Тогда
		ДобавитьГиперссылкуДляОткрытияНастроек();
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьГиперссылкуДляОткрытияНастроек()
	
	ТекстОткрытияНастроек = НСтр("ru = 'Настройки автоматического закрытия месяца.'");
	ОткрытиеНастроек = Новый ФорматированнаяСтрока(ТекстОткрытияНастроек, , , , "ОткрытьНастройкиЗакрытияМесяца");
	
	МассивСтрок = Новый Массив;
	МассивСтрок.Добавить(Элементы.СледующийЗапускЗадания.Заголовок);
	МассивСтрок.Добавить(Символы.ПС);
	МассивСтрок.Добавить(ОткрытиеНастроек);
	Элементы.СледующийЗапускЗадания.Заголовок = Новый ФорматированнаяСтрока(МассивСтрок);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПараметрыЗаданияАвтоматическогоЗакрытияМесяца(Организация)
	
	Возврат РегистрыСведений.НастройкиАвтоматическогоЗакрытияМесяца.НастройкиРегламентногоЗадания(Организация);
