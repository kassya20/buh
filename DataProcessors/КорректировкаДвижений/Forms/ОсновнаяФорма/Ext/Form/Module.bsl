...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// В операциях межотчетного периода запрещается изменять дату проводки
	ЭтоОперацияМежотчетногоПериода = БухгалтерскийУчет.ЭтоОперацияМежотчетногоПериода(ДокументДвижений);
	Элементы.ХозрасчетныйПериод.ТолькоПросмотр = ЭтоОперацияМежотчетногоПериода;
...
Процедура ЗаполнитьРеквизитыОбъекта(МетаданныеДокумента)
	...
	Если Не Объект.Проведен
	   И БухгалтерскийУчетКлиентСерверПереопределяемый.ЭтоРегламентнаяОперация(ДокументДвижений) Тогда
		ИменаРеквизитов = ИменаРеквизитов + ",Состояние";
	КонецЕсли;
...
Процедура НастроитьФормуПоОрганизацииПериоду()
	...
	БухгалтерскийУчет.НастроитьПоляУчетаРазниц(ПоляРазниц, ПоляЗаголовков, Период, ОрганизацияНабораХозрасчетный);
...
Процедура ХозрасчетныйПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если ИдСтроки <> ИдентификаторСтрокиПроводки Тогда
		...
		БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(ТекущиеДанные.СчетДт, ЭтотОбъект, ПоляФормы);
	
		ПоляФормы = Новый Структура("Субконто1, Субконто2, Субконто3",
			"ХозрасчетныйСубконтоКт1", "ХозрасчетныйСубконтоКт2", "ХозрасчетныйСубконтоКт3");
		БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(ТекущиеДанные.СчетКт, ЭтотОбъект, ПоляФормы);
	...
	Если ЭтоОперацияМежотчетногоПериода Тогда
		ТекущиеДанные.УточнениеПериода = БухгалтерскийУчетКлиентСервер.ОсновноеЗначениеУточненияМежотчетногоПериода();
	КонецЕсли;
...
Процедура Подключаемый_ТаблицаРегистраПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока И Не Копирование Тогда
		...
		Если ТекущиеДанные.Свойство("Период") И ЗначениеЗаполнено(Объект.Дата) Тогда
			ТекущиеДанные.Период = Объект.Дата;
		КонецЕсли;

		Если ТекущиеДанные.Свойство("Организация") И ЗначениеЗаполнено(Объект.Организация) Тогда
			ТекущиеДанные.Организация = Объект.Организация;
		КонецЕсли;
		
		Если ТекущиеДанные.Свойство("НоменклатурнаяГруппа") И ТекущиеДанные.Свойство("ХарактерДеятельности") Тогда
			ТекущиеДанные.НоменклатурнаяГруппа = ИПОсновнаяНоменклатурнаяГруппа;
			ТекущиеДанные.ХарактерДеятельности = ИПОсновнойХарактерДеятельности;
		КонецЕсли;
		
		Если ТекущиеДанные.Свойство("Валюта") И ТекущиеДанные.Свойство("ОтражениеВУСН") Тогда // регистр накопления "Расходы при УСН"
...
Функция ПолучитьСписокПараметров(Форма, ТекущиеДанные, ШаблонИмяПоляОбъекта, ИмяПоляСчетУчета)
	...
	БухгалтерскийУчетКлиентСервер.ДополнитьСписокПараметровСубконто(СписокПараметров, ТекущиеДанные, ШаблонИмяПоляОбъекта);
...
Процедура ОбработатьИзменениеСчета(ДтКт)
	...
	БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(ТекущиеДанные["Счет" + ДтКт], ЭтотОбъект, ПоляФормы);
	...
	Если РучнаяКорректировка Тогда
		...
		Попытка
			...
			Если НЕ ДокОбъект.РучнаяКорректировка Тогда
				...
				ДокОбъект.ДополнительныеСвойства.Вставить("РучнаяКорректировкаДвиженийДокумента", Истина);
				ДокОбъект.Записать(РежимЗаписиДокумента.Запись);
			КонецЕсли;
			
			ДокОбъектМетаданных = ДокОбъект.Метаданные();
			
			АктивностьДвижений = ДокОбъект.Проведен
				Или Не ДокОбъект.ПометкаУдаления И ДокОбъектМетаданных.Проведение = Метаданные.СвойстваОбъектов.Проведение.Запретить;
			Если АктивностьДвижений
			   И БухгалтерскийУчетКлиентСерверПереопределяемый.ЭтоРегламентнаяОперация(ДокументДвижений)
			   И ДокОбъект.РучнаяКорректировка
			   И ДокОбъект.Состояние = Перечисления.ВидыСостоянийРегламентныхОпераций.НеВыполнено Тогда
			...
			Для каждого СтрокаРегистра Из Регистры Цикл
				...
				Если СтрокаРегистра.ЕстьРеквизит Тогда
					...
					НаборЗаписей.УстановитьАктивность(АктивностьДвижений);
			...
			Если РаботаСПоследовательностями.ДокументВходитВПоследовательностьИРегистрируется(ДокОбъектМетаданных) Тогда
				РаботаСПоследовательностями.СброситьСостояниеПоследовательностиДокумента(ДокументДвижений, ДокументДвижений.Дата, ДокументДвижений.Организация);
			КонецЕсли;
		...
		Попытка
			...
			Если БухгалтерскийУчетКлиентСерверПереопределяемый.ЭтоРегламентнаяОперация(ДокументДвижений) Тогда
				...
				Если ВыполнениеБезКонтроля Тогда			
					ДокОбъект.ДополнительныеСвойства.Вставить("ПропуститьПроверкуПоследовательности", Истина);
					ДокОбъект.ДополнительныеСвойства.Вставить("ПропуститьПроверкуСтатусаПредыдущих", Истина);
					ДокОбъект.ДополнительныеСвойства.Вставить("СброситьСостояниеСледующих", Истина);
				КонецЕсли;
