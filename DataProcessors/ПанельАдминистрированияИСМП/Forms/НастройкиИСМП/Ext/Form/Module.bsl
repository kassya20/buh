...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВерсияБПОПоддерживаетТСПИоТ = ОбщегоНазначенияИСМП.ВерсияБПОПоддерживаетПроверкуКМЧерезТСПИоТ();
	...
	Если ВерсияБПОПоддерживаетТСПИоТ Тогда
		
		ИмяСправочникаТСПИоТ = "ТСПИоТ";
		ПросмотрНастроекПодключенияТСПИоТ = ПравоДоступа(
			"Просмотр", Метаданные.Справочники[ИмяСправочникаТСПИоТ]);
			
	Иначе
		ПросмотрНастроекПодключенияТСПИоТ = Истина;
	КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Элементы.НастройкиПодключенияТСПИоТ.Видимость = ПросмотрНастроекПодключенияТСПИоТ;
