...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДнейПланирования") И Параметры.ДнейПланирования <> Неопределено Тогда
		Объект.ДнейПланирования = Параметры.ДнейПланирования;
	КонецЕсли;
...
Процедура РезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Расшифровка) = Тип("Структура") Тогда // Это задача бухгалтера
		...
		Если Расшифровка.Свойство("СписокВедомостей") Тогда
			ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ВыборДокументаЗавершение", ЭтотОбъект);
			ПоказатьВыборИзМеню(ОписаниеОповещенияОЗавершении, Расшифровка.СписокВедомостей);
		Иначе
...
Функция ПодготовитьПараметрыЗаполненияКалендаря()
	...
	ПараметрыЗадания.ДнейПланирования = Объект.ДнейПланирования;
...
Функция НачатьЗаполнениеКалендаряНаСервере()
	...
	ИмяКлючевойОперации = СтрШаблон(
		"СтатистикаБП.Отчеты.ПлатежныйКалендарь.Сформировать.Горизонт_%1", // Не локализуется.
		Формат(Объект.ДнейПланирования, "ЧГ=0"));
	
	ПоказателиБизнесСтатистики = ПоказателиБизнесСтатистики();
	
	ЦентрМониторинга.ЗаписатьОперациюБизнесСтатистики(ИмяКлючевойОперации, 1, ПоказателиБизнесСтатистики);
	СтатистикаПоПоказателямКлиентСервер.ДобавитьСобытие(ИмяКлючевойОперации, ПоказателиБизнесСтатистики);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Параметры.ДнейПланирования <> 0 Тогда
		Объект.ДнейПланирования = Параметры.ДнейПланирования;
	КонецЕсли;
...
Процедура АдаптироватьМККоманднаяПанельФормы()
	...
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.ПеренестиНаДругуюДату);
	МобильныйКлиентБПФормаОбъекта.СкрытьЭлементФормы(Элементы, Элементы.РезультатКонтекстноеМенюПеренестиНаДругуюДату);
...
Процедура АдаптироватьМКШапкаФормы()
	...
	МобильныйКлиентБПФормаОбъекта.ВстроитьЭлементФормыВГруппаМКФормаВажное(Элементы, Элементы.ГруппаПериодПланирования);
