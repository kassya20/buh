...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьКалендарь", Истина);
...
Процедура ЗаполнитьКалендарь(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.ОтключитьВыводВТабличныйДокумент Тогда
		Календарь = Неопределено;
	Иначе
		Календарь =  ВывестиКалендарь(Данные);
	КонецЕсли;

	РезультатВыполнения = Новый Структура;
	
	НепросроченныеПлатежи = Данные.ВсеПлатежи.Скопировать(
		Новый Структура("Просрочен", Ложь), "НомерРаздела, ДатаПлатежа, Сумма, Документ");
	
	РезультатВыполнения.Вставить("Платежи",   НепросроченныеПлатежи);	
	РезультатВыполнения.Вставить("Календарь", Календарь);
...
// Возвращает массив номеров раздела платежного календаря в зависимости от назначения.
//
// Параметры:
//	ВиджетПлатежиСегодня - Булево - ВиджетПлатежиСегодня - для формирования виджета по платежам сегодня.
//	Поступления - Булево - для формирования виджета по платежам сегодня, Истина - раздел поступлений, Ложь - раздел списаний.
//
// Возвращаемое значение:
//	Массив.
//
Функция НомераРазделовДляГруппировкиПлатежей(ВиджетПлатежиСегодня = Ложь, Поступления = Ложь) Экспорт
	
	НомераРазделовДляГруппировкиПлатежей = Новый Массив;
	Если ВиджетПлатежиСегодня И Поступления Тогда
		НомераРазделовДляГруппировкиПлатежей.Добавить(1);
		НомераРазделовДляГруппировкиПлатежей.Добавить(2);
	ИначеЕсли ВиджетПлатежиСегодня Тогда
		НомераРазделовДляГруппировкиПлатежей.Добавить(3);
		НомераРазделовДляГруппировкиПлатежей.Добавить(4);
		НомераРазделовДляГруппировкиПлатежей.Добавить(5);
		НомераРазделовДляГруппировкиПлатежей.Добавить(6);
	Иначе
		НомераРазделовДляГруппировкиПлатежей.Добавить(1);
		НомераРазделовДляГруппировкиПлатежей.Добавить(4);
	КонецЕсли;
...
Функция ДанныеКалендаря(Параметры)
	
	Организация = Параметры.Организация;
	ДнейПланирования = Параметры.ДнейПланирования;
	...
	Данные.ОстатокДенег = 0;
	
	Если Параметры.ПолучатьОстаток Тогда
...
Процедура ЗаполнитьВсеПлатежи(Данные)
	...
	ДлительныеОперации.СообщитьПрогресс(90);
...
Функция НовыеПараметрыЗаполнения() Экспорт
	...
	ПараметрыЗаполнения.Вставить("ДнейПланирования", 0);  
	ПараметрыЗаполнения.Вставить("ОтключитьВыводВТабличныйДокумент", Ложь);
	ПараметрыЗаполнения.Вставить("ПолучатьОстаток", Истина);
