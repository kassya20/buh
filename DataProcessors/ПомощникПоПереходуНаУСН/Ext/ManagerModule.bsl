...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьВосстановлениеНДС", Истина);
	Методы.Вставить("ВключитьАвансыВДоходыУСН", Истина);
	Методы.Вставить("ОчиститьОстаткиРегистровНУ", Истина);
	
КонецПроцедуры

Функция ВыполнитьВосстановлениеНДС(СтруктураПараметров, АдресХранилища = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ДокОбъект.СчетСписанияНДС) Тогда
		...
		ДокОбъект.СубконтоСписанияНДС1 = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("СписаниеНДСНаПрочиеРасходы");
	КонецЕсли;
...
Функция ВключитьАвансыВДоходыУСН(СтруктураПараметров, АдресХранилища = Неопределено) Экспорт
...
Функция ОчиститьОстаткиРегистровНУ(СтруктураПараметров, АдресХранилища = Неопределено) Экспорт
...
// Определяет, возможен ли переход на УСН в простом интерфейсе.
//
// Параметры:
//   Организация - СправочникСсылка.Организации - организация в простом интерфейсе.
//   ИнтерфейсТаксиПростой - Булево, Неопределено - значение функциональной опции, определяющей использование простого интерфейса.
//                           В случае, если передано Неопределено, значение будет получено внутри функции.
//
// Возвращаемое значение:
//   Булево - возможен или нет переход на УСН.
Функция ВозможенПереходНаУСН(Организация, ИнтерфейсТаксиПростой = Неопределено) Экспорт
	
	Если ИнтерфейсТаксиПростой = Неопределено Тогда
		ИнтерфейсТаксиПростой = ПолучитьФункциональнуюОпцию("ИнтерфейсТаксиПростой");
	КонецЕсли;
	
	Если Не (ИнтерфейсТаксиПростой И ТарификацияБП.РазрешенаУСН()) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	СтруктураПараметров = ПараметрыПереходаНаУСН(Организация);
	УстановитьПривилегированныйРежим(Ложь);
	Возврат ЗначениеЗаполнено(СтруктураПараметров.Период) 
		Или СтруктураПараметров.ПрименяетсяОСНО;
...
Процедура СформироватьДвиженияПоРегиструХозрасчетный(СтруктураПараметров)
	...
	Если СтруктураПараметров.Свойство("Курсор", Курсор) Тогда
		
		СозданКурсор = Ложь;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Курсор",
			"(ХозрасчетныйОстатки.Счет >= &КурсорСчет
			|			ИЛИ ХозрасчетныйОстатки.Организация >= &КурсорОрганизация
			|			ИЛИ ЕСТЬNULL(ХозрасчетныйОстатки.Подразделение, НЕОПРЕДЕЛЕНО) >= &КурсорПодразделение)
			|			И
			|			ВЫБОР КОГДА ХозрасчетныйОстатки.Счет = &КурсорСчет
			|				ТОГДА (ЕСТЬNULL(ХозрасчетныйОстатки.Субконто1, НЕОПРЕДЕЛЕНО) > &КурсорСубконто1
			|						ИЛИ ЕСТЬNULL(ХозрасчетныйОстатки.Субконто2, НЕОПРЕДЕЛЕНО) > &КурсорСубконто2
			|						ИЛИ ЕСТЬNULL(ХозрасчетныйОстатки.Субконто3, НЕОПРЕДЕЛЕНО) > &КурсорСубконто3)
			|				КОГДА ХозрасчетныйОстатки.Счет > &КурсорСчет
			|				ТОГДА(ЕСТЬNULL(ХозрасчетныйОстатки.Субконто1, НЕОПРЕДЕЛЕНО) >= &КурсорСубконто1
			|						ИЛИ ЕСТЬNULL(ХозрасчетныйОстатки.Субконто2, НЕОПРЕДЕЛЕНО) >= &КурсорСубконто2
			|						ИЛИ ЕСТЬNULL(ХозрасчетныйОстатки.Субконто3, НЕОПРЕДЕЛЕНО) >= &КурсорСубконто3)
			|				ИНАЧЕ ЛОЖЬ
		...
		Запрос.УстановитьПараметр("КурсорСчет",          Курсор.Счет);
		Запрос.УстановитьПараметр("КурсорСубконто1",     Курсор.Субконто1);
		Запрос.УстановитьПараметр("КурсорСубконто2",     Курсор.Субконто2);
		Запрос.УстановитьПараметр("КурсорСубконто3",     Курсор.Субконто3);
		Запрос.УстановитьПараметр("КурсорОрганизация",   Курсор.Организация);
		Запрос.УстановитьПараметр("КурсорПодразделение", Курсор.Подразделение);
		
	Иначе
		
		СозданКурсор = Истина;
		Курсор = Новый Структура; // позиция выборки, чтобы избежать зацикливания
		СтруктураПараметров.Вставить("Курсор", Курсор);
		Запрос.УстановитьПараметр("Курсор", Истина);
		
		Курсор.Вставить("Счет");
		Курсор.Вставить("Субконто1");
		Курсор.Вставить("Субконто2");
		Курсор.Вставить("Субконто3");
		Курсор.Вставить("Организация");
		Курсор.Вставить("Подразделение");
	...
	Если Результат.Пустой() Тогда
		...
		Если СозданКурсор Тогда
			СтруктураПараметров.Удалить("Курсор");
		КонецЕсли;
		Возврат;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Курсор.Счет          = ВыборкаДетальныеЗаписи.Счет;
		Курсор.Субконто1     = ВыборкаДетальныеЗаписи.Субконто1;
		Курсор.Субконто2     = ВыборкаДетальныеЗаписи.Субконто2;
		Курсор.Субконто3     = ВыборкаДетальныеЗаписи.Субконто3;
		Курсор.Организация   = ВыборкаДетальныеЗаписи.Организация;
		Курсор.Подразделение = ВыборкаДетальныеЗаписи.Подразделение;
...
Процедура СформироватьДвиженияПоРегиструНДСПоПриобретеннымЦенностям(СтруктураПараметров)
	...
	|	&Период КАК Период
	|ИЗ
	|	РегистрНакопления.НДСРаздельныйУчет.Остатки(&Граница, Организация = &Организация) КАК НДСРаздельныйУчетОстатки
	|ГДЕ
...
Функция ПараметрыПереходаНаУСН(Организация) Экспорт
	
	СтруктураПараметров = Новый Структура();
	СтруктураПараметров.Вставить("Период", '00010101');
	СтруктураПараметров.Вставить("ДатаПереходаНаУСН", '00010101');
	СтруктураПараметров.Вставить("ПрименяетсяУСНДоходыМинусРасходы", Ложь);
	СтруктураПараметров.Вставить("ПрименяетсяОСНО", Ложь);
	СтруктураПараметров.Вставить("ВосстанавливатьНДС", Истина);
	...
	ПрименяетсяОСНО        = СистемаНалогообложения = Перечисления.СистемыНалогообложения.Общая;
	
	СтруктураПараметров.ПрименяетсяОСНО = ПрименяетсяОСНО;
	СтруктураПараметров.ВосстанавливатьНДС = Не УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Организация, Период);
	
	Если Не ПрименяетсяУСН И Не ПрименяетсяОСНО Тогда
	...
	Иначе
		// ищем переход на УСН "в будущем"
		Запрос.Текст =
		"ВЫБРАТЬ
		|	НастройкиСистемыНалогообложенияСрезПервых.Период КАК Период,
		|	НастройкиСистемыНалогообложенияСрезПервых.Организация КАК Организация,
		|	НастройкиСистемыНалогообложенияСрезПервых.ДатаПереходаНаУСН КАК ДатаПереходаНаУСН,
		|	НастройкиСистемыНалогообложенияСрезПервых.ПрименяетсяУСНДоходыМинусРасходы КАК ПрименяетсяУСНДоходыМинусРасходы
		|ИЗ
