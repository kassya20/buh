...
Процедура ЗаполнитьСтрокиДокумента(СтруктураПараметров, ТаблицаРезультатов)
	...
	|	СУММА(ХозрасчетныйОплата.СуммаОборот) КАК СуммаОборот,
	...
	|ГДЕ
	|	НЕ ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.ПустаяСсылка)) В (&ВидыДоговоровРеализацииТоваров)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезультатов Цикл
		...
		Иначе
			СтрокаТаблицы.СтавкаНДС = УчетНДСКлиентСервер.ОбщаяРасчетнаяСтавкаНДС(СтрокаТаблицы.Дата);
			СтавкаНДСЧислом = УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(УчетНДСКлиентСервер.ОбщаяСтавкаНДС(СтрокаТаблицы.Дата));
			СтрокаТаблицы.СуммаНДС  = УчетНДСКлиентСервер.РассчитатьСуммуНДС(СтрокаТаблицы.СуммаОборот, Ложь, СтавкаНДСЧислом);
			СтрокаТаблицы.Сумма     = СтрокаТаблицы.СуммаОборот + СтрокаТаблицы.СуммаНДС;
		КонецЕсли;
	КонецЦикла;
