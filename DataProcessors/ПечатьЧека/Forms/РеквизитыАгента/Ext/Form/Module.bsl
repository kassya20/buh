...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	// Не проверяем ИНН состоящие полностью из нулей, так как это служебные значения для иностранных контрагентов
	Если ЗначениеЗаполнено(СтрЗаменить(ИННПоставщика, "0", "")) Тогда
	...
	РеквизитыТелефон = Новый Структура;
	РеквизитыТелефон.Вставить("ТелефонПоставщика",                НСтр("ru = 'Телефон поставщика'"));
	РеквизитыТелефон.Вставить("ТелефонОператораПеревода",         НСтр("ru = 'Телефон оператора перевода'"));
	РеквизитыТелефон.Вставить("ТелефонОператораПоПриемуПлатежей", НСтр("ru = 'Телефон оператора по приему платежей'"));
	РеквизитыТелефон.Вставить("ТелефонПлатежногоАгента",          НСтр("ru = 'Телефон платежного агента'"));
	
	ОписаниеОшибки = НСтр("ru = 'Телефон должен начинаться с ""+"" и содержать только цифры.'");
	
	Для каждого Реквизит Из РеквизитыТелефон Цикл
		
		ИмяРеквизита = Реквизит.Ключ;
		
		Если ЗначениеЗаполнено(ЭтотОбъект[ИмяРеквизита]) 
			И (Лев(ЭтотОбъект[ИмяРеквизита], 1) <> "+" 
			ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ЭтотОбъект[ИмяРеквизита], 2))) Тогда
			
			ТекстСообщенияПользователю = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения("Поле", "Корректность", Реквизит.Значение,,,ОписаниеОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияПользователю,,ИмяРеквизита, , Отказ);
...
Процедура КомандаОК(Команда)
	...
	Результат.Вставить("ДанныеАгента",     ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыДанныеАгента());
	Результат.Вставить("ДанныеПоставщика", ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыДанныеПоставщика());
