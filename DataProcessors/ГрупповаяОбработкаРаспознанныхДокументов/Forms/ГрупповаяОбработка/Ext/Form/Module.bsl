...
Процедура ОбработкаВыбораПоляТаблицаДопРеквизитов(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.РаспознаваниеДокументов.Форма.СозданиеЭлементаСОтображениемКартинки", ПараметрыОткрытия, Элемент);
...
Процедура ОбработкаВыбораПоляТаблицаНоменклатуры(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ДатаДокумента = РаспознаваниеДокументовСлужебныйВызовСервера.ЗначениеРеквизитаОбъекта(
		ДанныеЭтойСтроки.Ссылка, "ДатаДокумента");
	...
	ДополнительныеДанные.Вставить("ДатаДокумента", ДатаДокумента);
...
Процедура Подключаемый_ОбработкаВыбораПоля(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИмяФормыСоздания = "Обработка.РаспознаваниеДокументов.Форма.СозданиеЭлементаСОтображениемКартинки";
	РаспознаваниеДокументовКлиентПереопределяемый.ИмяФормыСозданияЭлемента(ИмяРеквизита, ИмяФормыСоздания);
	
	ОткрытьФорму(ИмяФормыСоздания, ПараметрыОткрытия, Элемент);
...
Функция ЗаполнениеСписковВыбораШапкиНаСервере(ИменаРеквизитов, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		Результат.ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"РаспознаваниеДокументовСлужебный.ЗаполнитьСпискиВыбораРеквизитаШапкиДокументов",
			ПараметрыОперации,
			ПараметрыВыполнения);
	КонецЕсли;
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовШапкиНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		Результат.ОперацияЗапущена = Истина;
		Результат.ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.РаспознанныйДокумент.ЗаписатьИзмененияРеквизитов",
			ПараметрыОперации,
			ПараметрыВыполнения);
	КонецЕсли;
...
Процедура СоздатьКомплектыНаСервере(ДанныеОбработки)
	...
	Для НомерКомплекта = 1 По КоличествоКомплектов Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Элементы, ИмяГруппыШапкиКомплекта) Тогда
			...
			ДокументОбъект = СтрокиКомплекта[0].Ссылка.ПолучитьОбъект();
			...
			ПараметрыОперации = РаспознаваниеДокументовКомплектыКлиентСервер.ТипДокументаИВидОперацииПоУмолчанию(
				СтрокиКомплекта[0].Направление, СтрокиКомплекта[0].ТипКомплекта, НаличиеТоваровУслуг);
...
Процедура ПриАктивизацииЯчейкиТаблицы(ТаблицаЭлемент, ИмяРеквизита, ОбновитьПолнуюКартинку)
	...
	Если ИмяРеквизита <> "ЕдиницаИзмерения" Тогда // список заполняется через "Быстрый выбор"
		...
		Если ТаблицаЭлемент.Имя = "ТаблицаНоменклатуры" И ИмяРеквизита = "СтавкаНДС" Тогда
			ЭлементКолонка.РежимВыбораИзСписка = Истина;
			СтавкиНДСПоДатеРаспознанногоДокумента(ТекущиеДанные.Ссылка, СписокДляВыбора);
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура СтавкиНДСПоДатеРаспознанногоДокумента(РаспознанныйДокумент, СписокДляВыбора)
	
	ДатаДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РаспознанныйДокумент, "ДатаДокумента");
	РаспознаваниеДокументовПереопределяемый.СтавкиНДСПоДатеРаспознанногоДокумента(ДатаДокумента, СписокДляВыбора);
...
Функция ЗапуститьФоновоеЗаданиеЗаписиРеквизитовТаблицыНаСервере(ПараметрыОперации, ИдентификаторФормы)
	...
	Если Не Отказ Тогда
		...
		Результат.ОперацияЗапущена = Истина;
		Результат.ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"Документы.РаспознанныйДокумент.ЗаписатьИзмененияРеквизитовТаблицы",
			ПараметрыОперации,
			ПараметрыВыполнения);
	КонецЕсли;
...
Процедура СоздатьОдиночныеДокументыНаСервере(ДанныеОбработки)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаИтогоОдиночныеДокументы Цикл
		...
		Если ИмяТипа = "ПоступлениеТоваровУслугБРД" Тогда
			Если ПараметрыЗаполнения.Товары.Количество() > 0
				И ПараметрыЗаполнения.Услуги.Количество() > 0 Тогда				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия;
			ИначеЕсли ПараметрыЗаполнения.Услуги.Количество() > 0 Тогда
				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Услуги;
			Иначе
				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары;
			КонецЕсли;
		КонецЕсли;

		Если ИмяТипа = "РеализацияТоваровУслугБРД" Тогда
			Если ПараметрыЗаполнения.Товары.Количество() > 0
				И ПараметрыЗаполнения.Услуги.Количество() > 0 Тогда
				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия;
			ИначеЕсли ПараметрыЗаполнения.Услуги.Количество() > 0 Тогда
				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Услуги;
			Иначе
				ПараметрыЗаполнения.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Товары;
			КонецЕсли;
		КонецЕсли;
		...
		Если СтрокаТаблицы.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.УПД Тогда
			СчетФактураСсылка = СоздатьСчетФактуруПоПараметрам(СоздаваемыйДокумент, ПараметрыЗаполнения.ТипДокументаСтрокой,
				ПараметрыЗаполнения, РаспознанныйОбъект, АдресКартинки);
...
&НаСервере
Функция СоздатьСчетФактуруПоПараметрам(ОснованиеСчетаФактуры, ТипДокументаСтрокой, ПараметрыЗаполнения, РаспознанныйОбъект, АдресКартинки)
	...
	Попытка
		...
		Если СчетФактураСсылка <> Неопределено Тогда
			РегистрыСведений.СвязанныеОбъектыРаспознаниеДокументов.ЗаписатьЗначения(СчетФактураСсылка, РаспознанныйОбъект.Ссылка, Ложь);
			РаспознаваниеДокументовСлужебный.ДобавитьПрисоединенныйФайл(РаспознанныйОбъект, СчетФактураСсылка, АдресКартинки);
		КонецЕсли;
