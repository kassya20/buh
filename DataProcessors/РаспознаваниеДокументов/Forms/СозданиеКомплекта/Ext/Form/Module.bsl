...
Процедура ОбработатьДокументы(Команда)
	...
	Если Не ТребуетсяВключениеСкидок() Тогда
		Закрыть(Параметры.ПараметрыСоздания);
	Иначе
		ПараметрыФормы = Новый Структура("Направление", НаправлениеДокумента);
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьДокументыЗавершение", ЭтотОбъект, НаправлениеДокумента);
...
&НаКлиенте
Процедура ОбработатьДокументыЗавершение(Результат, Направление) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Если Направление = ПредопределенноеЗначение("Перечисление.НаправленияРаспознанногоДокумента.Входящий") Тогда
			РаспознаваниеДокументовСлужебныйВызовСервера.УстановитьЗначениеКонстантыОпцииПоСкидкам("ФункциональнаяОпцияИспользоватьРучныеСкидкиНаценкиЗакупки", Истина);
		Иначе
			РаспознаваниеДокументовСлужебныйВызовСервера.УстановитьЗначениеКонстантыОпцииПоСкидкам("ФункциональнаяОпцияИспользоватьРучныеСкидкиНаценкиПродажи", Истина);
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеФормой()
		...
		Иначе
			...
			Элементы["СоздатьДокумент_" + Ид].Заголовок = Элементы["СоздатьДокумент_" + Ид].Заголовок +
				" """ + ТипДокумента + """" + СтрШаблон(НСтр("ru = ' № %1 от %2'"),
					...
					Формат(ДанныеРаспознанного.ДатаДокумента, "ДЛФ=D")
				);
		КонецЕсли;
	...
	ЭтотОбъект.Заголовок = СтрШаблон(НСтр("ru = '%1 от %2 %3'"),
		СтрокаТипДокумента, Формат(ДанныеЗаголовка.ДатаДокумента, "ДЛФ=D"), ВОтКонтрагента);
...
&НаСервере
Функция ТребуетсяВключениеСкидок()
	
	Результат = Ложь;
	
	Для Каждого СтрокаРеквизитов Из РеквизитыДокументовНаФорме Цикл
		
		Если Не СтрокаРеквизитов.СоздатьДокумент Тогда
			Продолжить;
		КонецЕсли;
		
		Если Не Метаданные.ОпределяемыеТипы.СчетНаОплатуПокупателюБРД.Тип.СодержитТип(СтрокаРеквизитов.ТипДокумента)
			И Не Метаданные.ОпределяемыеТипы.СчетНаОплатуПоставщикаБРД.Тип.СодержитТип(СтрокаРеквизитов.ТипДокумента) Тогда
			Продолжить;
		КонецЕсли;
		
		РаспознанныйДокумент = Параметры.ПараметрыСоздания.РаспознанныеДокументыПоТипам[Перечисления.ТипыДокументовРаспознаваниеДокументов.СчетНаОплату];
		
		РеквизитыДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РаспознанныйДокумент, "РеквизитыДокумента");
		ВидСкидки = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(
			Новый Структура("РеквизитыДокумента", РеквизитыДокумента.Выгрузить()), "ВидСкидки", 
			РаспознаваниеДокументовСлужебныйКлиентСервер.ВидыСкидок().НеПредоставлена);
		
		РаспознаваниеДокументовПереопределяемый.ТребуетсяВключениеФункциональныхОпцийПоСкидкам(ВидСкидки,
			НаправлениеДокумента, Результат);
