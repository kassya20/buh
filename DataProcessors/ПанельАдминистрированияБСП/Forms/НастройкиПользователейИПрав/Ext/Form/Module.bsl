...
&НаКлиенте
Процедура ИспользоватьНесколькоОтветственныхПриИзменении(Элемент)
	
	Подключаемый_ПриИзмененииРеквизита(Элемент);
	
	Если Не НаборКонстант.ИспользоватьНесколькоОтветственных Тогда
		ИспользоватьНесколькоОтветственныхПриИзмененииНаСервере();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнятьОтветственногоПоПродажамИзКонтрагентаПриИзменении(Элемент)
...
Функция СохранитьЗначениеРеквизита(РеквизитПутьКДанным)
	...
	Если ИмяКонстанты = "ИспользоватьДатыЗапретаИзменения"
		И КонстантаЗначение = Ложь Тогда
		КомментарииДокументовЗакрытогоПериодаВызовСервера.РазрешитьРедактированиеКомментариевПользователям(Ложь);
	КонецЕсли;
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьНесколькоОтветственных"
		Или РеквизитПутьКДанным = "" Тогда
		Элементы.ЗаполнятьОтветственногоПоПродажамИзКонтрагента.Доступность = НаборКонстант.ИспользоватьНесколькоОтветственных;
	КонецЕсли;
	
	Если РеквизитПутьКДанным = "НаборКонстант.ИспользоватьДатыЗапретаИзменения" Тогда
		Если Не Элементы.Найти("ФормаРазрешитьРедактироватьКомментарий") = Неопределено Тогда
			ЭтотОбъект.РазрешитьРедактированияКомментарияДокументовЗакрытогоПериода = 
				ПолучитьФункциональнуюОпцию("РазрешитьРедактированияКомментарияДокументовЗакрытогоПериода");
			Элементы.ФормаРазрешитьРедактироватьКомментарий.Доступность = 
...
&НаСервере
Процедура ИспользоватьНесколькоОтветственныхПриИзмененииНаСервере()
	
	НаборКонстант.ЗаполнятьОтветственногоПоПродажамИзКонтрагента = Константы.ЗаполнятьОтветственногоПоПродажамИзКонтрагента.Получить();
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_РазрешитьРедактированияКомментарияПриИзменении(Элемент) 
	
	Разрешить = ЭтотОбъект.РазрешитьРедактированияКомментарияДокументовЗакрытогоПериода;
	КомментарииДокументовЗакрытогоПериодаВызовСервера.РазрешитьРедактированиеКомментариевПользователям(Разрешить);
