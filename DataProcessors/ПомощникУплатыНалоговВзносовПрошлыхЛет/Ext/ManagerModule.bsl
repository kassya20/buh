...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОтразитьЗадолженностьПоНалогамВзносам", Истина);
...
Функция ДокументыУплаты(Организация, Период, ВидыНалогов = Неопределено, ВидыНалоговыхОбязательств = Неопределено) Экспорт
	...
	ПараметрыУплатыНалогов.КодыЗадач.Добавить(ЗадачиБухгалтераКлиентСервер.КодЗадачиНДФЛПредприниматель());
...
//
Функция ТаблицаЗадолженностиДляВводаНачальныхОстатков(СтруктураЗадолженности, Организация, Период) Экспорт
	...
	Для Каждого Показатель Из СтруктураПоказателей Цикл
		...
		ВидНалога = РасчетыСБюджетом.ВидНалогаПоСчетуУчета(Показатель.Значение, Организация, Период);
		...
		Для Каждого ВидПлатежа Из ВидыПлатежей Цикл
			...
			СтрокаОстатков.ВидНалога = Налог;
...
Процедура ОтразитьЗадолженностьПоНалогамВзносам(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДокументов Цикл
			ЭлементБлокировки = БлокировкаДанных.Добавить("Документ.ВводНачальныхОстатков");
		...
		// Перенос на ЕНП
		ДатаПереходаНаЕНП = ДатаВводаОстатков + 1;
		ПлательщикЕНП = УчетнаяПолитика.ПлательщикЕНП(Параметры.Организация, ДатаПереходаНаЕНП);
		
		Если ПлательщикЕНП Тогда
			ОтразитьЗадолженностьПоНалогамВзносамНаСчетеЕНС(
				Параметры.Организация, ДатаПереходаНаЕНП, Параметры.ТаблицаОстатков, Отказ);
		КонецЕсли;
		
		Если Отказ Тогда
			ИнформацияОбОшибке = НСтр("ru = 'Ошибка переноса задолженности на счет ЕНС'");
			ЗаписатьСообщениеОбОшибкеОтраженияЗадолженности(ИнформацияОбОшибке);
			ОтменитьТранзакцию();
		Иначе
	...
	Исключение
		...
		ЗаписатьСообщениеОбОшибкеОтраженияЗадолженности(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВидыНалоговПоКодуЗадачи(Организация, КодЗадачи) Экспорт
	...
	ИначеЕсли КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиСтраховыеВзносыИП() Тогда
		...
		Результат.Добавить(Перечисления.ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф);
		Результат.Добавить(Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела);
	ИначеЕсли КодЗадачи = ЗадачиБухгалтераКлиентСервер.КодЗадачиНДФЛПредприниматель() Тогда
		Результат.Добавить(Перечисления.ВидыНалогов.НДФЛ_ИП);
	КонецЕсли;
	Результат.Добавить(Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж);
	
	Возврат Результат;
	
КонецФункции

Процедура ОтразитьЗадолженностьПоНалогамВзносамНаСчетеЕНС(Организация, ДатаПерехода, ТаблицаОстатков, Отказ)
	
	ТаблицаОстатковПоНалогам = ТаблицаОстатков.СкопироватьКолонки();
	ТаблицаОстатковПоШтрафамИПеням = ТаблицаОстатков.СкопироватьКолонки();
	
	Для Каждого СтрокаОстатка Из ТаблицаОстатков Цикл
		
		Если СтрокаОстатка.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Налог
			Или СтрокаОстатка.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела Тогда
			СтрокаНалога = ТаблицаОстатковПоНалогам.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаНалога, СтрокаОстатка);
		ИначеЕсли СтрокаОстатка.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.ПениСам
			Или СтрокаОстатка.ВидПлатежаВБюджет = Перечисления.ВидыПлатежейВГосБюджет.Штраф Тогда
			СтрокаШтрафаИПени = ТаблицаОстатковПоШтрафамИПеням.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаШтрафаИПени, СтрокаОстатка);
		КонецЕсли;
	...
	СписокНалогов.ЗагрузитьЗначения(ТаблицаОстатковПоНалогам.ВыгрузитьКолонку("ВидНалога"));
	
	ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеНалогов;
	
	ПеренестиЗадолженностьПоНалогамВзносамНаСчетЕНС(Организация, ДатаПерехода, ВидОперации, СписокНалогов, Отказ);
	...
	// Штрафы и пени
	СписокНалогов = Новый СписокЗначений;
	СписокНалогов.ЗагрузитьЗначения(ТаблицаОстатковПоШтрафамИПеням.ВыгрузитьКолонку("ВидНалога"));
	
	ВидОперации = Перечисления.ВидыОперацийПоЕдиномуНалоговомуСчету.НачислениеПенейШтрафов;
...
Функция ПодготовитьТаблицуНачальныхОстатковКЗаписи(НачальныеОстатки, ТаблицаИсточник)
	...
	ТаблицаИсточникСвернутая.Свернуть("СчетУчета, ВидНалога, ВидПлатежаВБюджет", "Задолженность, Переплата");
...
Процедура ПеренестиЗадолженностьПоНалогамВзносамНаСчетЕНС(Организация, ДатаПерехода, ВидОперации, СписокНалогов, Отказ)
	...
	СтруктураПараметров.Вставить("ВводитсяЗадолженностьПрошлыхЛет", Истина);
	СтруктураПараметров.Вставить("СписокНалогов", СписокНалогов);
	...
	Отказ = Не СтруктураДанных.УспешноВыполнено Или СтруктураДанных.ТребуетсяКонтроль;
	Если Не Отказ Тогда
		СтруктураПараметров.Вставить("Состояние",
			ПредопределенноеЗначение("Перечисление.ВидыСостоянийРегламентныхОпераций.Выполнено"));
		
		ДокументыОперации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураДанных, "ДокументыОперации");
		Если ТипЗнч(ДокументыОперации) = Тип("Массив") Тогда
			СтруктураПараметров.Вставить("ДокументыОперации", ДокументыОперации);
		КонецЕсли;
		
		Обработки.ПомощникПереходаНаЕНС.ОтразитьВыполнениеОперацииПереходаНаЕНС(СтруктураПараметров, Истина, Отказ);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаписатьСообщениеОбОшибкеОтраженияЗадолженности(ИнформацияОбОшибке)

	ШаблонСообщения = НСтр("ru = 'Не удалось отразить задолженность прошлых лет
	|%1'");
	ТекстСообщения = СтрШаблон(ШаблонСообщения, ИнформацияОбОшибке);
	
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Отражение задолженности прошлых лет'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,
		Метаданные.Обработки.ПомощникУплатыНалоговВзносовПрошлыхЛет,,
		ТекстСообщения);
