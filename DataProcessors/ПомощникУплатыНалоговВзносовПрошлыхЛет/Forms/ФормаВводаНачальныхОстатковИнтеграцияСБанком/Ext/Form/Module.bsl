...
Процедура УправлениеКомандамиОплаты(Форма)
	...
	ЗадолженностьОплаченаПолностью = Не ЗадолженностьОплаченаПолностью(Форма);
	...
	УстановитьВидПоУмолчаниюОформлением(Форма, Элементы.ОплатитьСБанковскогоСчета, Не ЗадолженностьОплаченаПолностью);
...
Функция СоздатьПлатежныеДокументыНаСервере(СпособОплаты, СчетОрганизации)
	...
	Если СуммыКУплате.Всего > 0 Тогда
		...
		СтруктураПараметров.Правило         = ПорядокУплаты.Правило;
...
Функция ТаблицаНачальныхОстатков()
	...
	Возврат МенеджерОбработки.ТаблицаЗадолженностиДляВводаНачальныхОстатков(
		СтруктураЗадолженности,
		Объект.Организация,
		КонецМесяца(Объект.Период) + 1);
