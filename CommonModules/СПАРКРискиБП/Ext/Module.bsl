...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьИндексыСПАРКПередФормированиемОтчета", Истина);
	
КонецПроцедуры

// Обновляет кэш индексов сервиса 1СПАРК по настройкам отчетов НадежностьДебиторовСПАРКРискиБП и ВходящийНДССПАРКРиски.
...
//
Функция ОбновитьИндексыСПАРКПередФормированиемОтчета(ПараметрыОтчета) Экспорт

	АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор);
	ОбновитьИндексыСПАРКПоНастройкамОтчета(ПараметрыОтчета, АдресХранилища);
...
#Область ПрограммныйИнтерфейсУстарело
...
Процедура ОбновитьИндексыСПАРКПоНастройкамОтчета(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	РезультатВызоваВебСервиса = СПАРКРискиСервис.НовыйРезультатВызоваСервиса();
...
Процедура СПАРКРискиПриИзмененииОпцииПриЗаписи(Источник, Отказ) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		УстановитьОпциюОтображатьКоманды1СПАРКРиски();
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьОпциюОтображатьКоманды1СПАРКРиски() Экспорт
	
	ОтображатьКоманды1СПАРКРиски = Константы.ИспользоватьСервисСПАРКРиски.Получить()
		И ОбщегоНазначенияБП.ЭтоПростойИнтерфейс();
	
	Константы.ОтображатьКоманды1СПАРКРиски.Установить(ОтображатьКоманды1СПАРКРиски);
