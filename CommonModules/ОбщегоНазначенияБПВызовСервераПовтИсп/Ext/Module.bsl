...
Функция ЭтоИП(Организация) Экспорт
	...
	Возврат Справочники.Организации.ЭтоИП(Организация);
...
// Устарела. Следует использовать Справочники.Организации.ВсяОрганизация.
...
// Для эффективной работы кэша повторно используемых значений запрещено передавать период, отличный от начала года.
//
// Параметры:
// Организация - СправочникСсылка.Организации - головная организация.
// Период - Дата - начало года, до которого необходимо исключить из выборки закрытые подразделения. Если указан Период,
...
Функция ВсяОрганизация(Организация, Период = '00010101') Экспорт
	...
	Возврат Новый ФиксированныйМассив(БухгалтерскийУчетПереопределяемый.ВсяОрганизация(Организация, Период));
...
Функция ДатаЗакрытияОрганизации(Организация) Экспорт
	...
	Возврат Справочники.Организации.ДатаЗакрытияОрганизации(Организация);
...
Функция ОрганизацииРазрешенныеПользователю(Пользователь = Неопределено) Экспорт
	...
	Если Не УправлениеДоступом.ОграничиватьДоступНаУровнеЗаписей()
		...
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоОрганизации", "Истина");
		КонецЕсли;
		РазрешенныеЗначенияАктуальные = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	...
	Иначе
		...
		РазрешенныеЗначенияАктуальные = Новый Массив;
		Если ОбщегоНазначенияБП.ИспользуетсяАрхив() Тогда
			Для Каждого РазрешенноеЗначение Из РазрешенныеЗначения Цикл
				Если Не РазрешенноеЗначение.ВАрхиве Тогда
					РазрешенныеЗначенияАктуальные.Добавить(РазрешенноеЗначение);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	...
	Возврат Новый ФиксированныйМассив(РазрешенныеЗначенияАктуальные);
