...
// Выполнить частичную выгрузку данных на офлайн ККМ
//
// Параметры:
...
// Выполнить полную выгрузку данных на офлайн ККМ
//
// Параметры:
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование
//   УникальныйИдентификатор - ФормаКлиентскогоПриложения -идентификатор формы.
...
// Выполнить выгрузку настроек на офлайн ККМ
//
// Параметры:
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование
//   УникальныйИдентификатор - ФормаКлиентскогоПриложения -идентификатор формы.
...
// Выполнить очистку прайс-листа на офлайн ККМ
//
// Параметры:
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование
//   УникальныйИдентификатор - ФормаКлиентскогоПриложения -идентификатор формы.
...
// Выполнить загрузку данных из офлайн ККМ
//
// Параметры:
//   ИдентификаторУстройства - СправочникСсылка.ПодключаемоеОборудование
//   УникальныйИдентификатор - ФормаКлиентскогоПриложения -идентификатор формы.
...
Процедура НачатьЗагрузкуДанныхИзККМЗавершение(РезультатКоманды, Параметры) Экспорт
	...
	Если РезультатКоманды.Результат Тогда
		...
		Если РезультатПередачиДанных.Успешно Тогда
			...
			НесопоставленныеТоварыОФД = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				РезультатПередачиДанных.ДополнительныеПараметры, "НесопоставленныеТоварыОФД", Неопределено);
			
			Если РезультатПередачиДанных.ЭтоОбработчикОФД 
				И ЗначениеЗаполнено(НесопоставленныеТоварыОФД) Тогда
				...
				Если Параметры.СледующееОповещение <> Неопределено Тогда
					...
					РезультатВыполнения.Вставить("НесопоставленныеТоварыОФД", НесопоставленныеТоварыОФД);
			...
			Иначе
				...
				Параметры.Вставить("ДополнительныеПараметры", РезультатПередачиДанных.ДополнительныеПараметры);
...
Процедура НачатьЗагрузкуДанныхИзККМПослеУстановкиФлагаОбработанности(РезультатКоманды, Параметры) Экспорт
	...
	Если Параметры.СледующееОповещение <> Неопределено Тогда
		...
		РезультатВыполнения.Вставить("ДополнительныеПараметры", 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры,"ДополнительныеПараметры", Новый Структура));
...
//
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено, ОткрыватьФормуНастройки = Истина) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ФормаНастройки) И ФормаНастройки = "ОФД" Тогда
		...
		ПараметрыОборудования.Вставить("ОповещениеПриЗавершении", ОповещениеПриЗавершении);
		...
		Если ОткрыватьФормуНастройки Тогда
