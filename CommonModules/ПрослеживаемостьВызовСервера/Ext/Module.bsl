...
Функция ИзменениеЗапрещено(ПервичныйДокумент, ОписаниеОшибки) Экспорт
	
	Отказ = ДатыЗапретаИзменения.ИзменениеЗапрещено(ПервичныйДокумент.Метаданные().ПолноеИмя(), ПервичныйДокумент, ОписаниеОшибки);
	
	Если Отказ Тогда
		
		ПредставлениеУведомления = ПрослеживаемостьПереопределяемый.ПредставлениеДокумента(ПервичныйДокумент);
	
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, Строка(ПервичныйДокумент), ПредставлениеУведомления);
	КонецЕсли;	
...
Функция ЗначениеРеквизитаДатаПервичногоДокумента(ПервичныйДокумент) Экспорт
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПервичныйДокумент, "Дата");
