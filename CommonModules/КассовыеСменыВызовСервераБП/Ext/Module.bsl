...
//
Функция ДанныеЗакрытияКассовойСмены(РабочееМесто, КлючФормы, Знач КомпоновщикНастроек = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	
	ДанныеЗакрытия = НовыеДанныеЗакрытияКассовойСмены();
	
	ДанныеЗакрытия.ОткрытыеСмены = ОткрытыеСмены(РабочееМесто, ИдентификаторУстройства);
	...
	Если КоличествоОткрытыхСмен = 1
		...
		ДанныеЗакрытия.КассирИНН = ИННКассира(Организация);
...
Процедура ЗаполнитьПараметрыОперацииДляОрганизацииПоУмолчанию(ПараметрыВыполненияОперации) Экспорт
	ПараметрыВыполненияОперации.Кассир    = ПредставлениеКассираДляОрганизацииПоУмолчанию();
	ПараметрыВыполненияОперации.КассирИНН = ИННКассираДляОрганизацииПоУмолчанию();
КонецПроцедуры
...
Функция НовыеДанныеЗакрытияКассовойСмены()
	...
	ДанныеЗакрытия.Вставить("КассирИНН",     ""); // информация о кассире, которая выводится в кассовые документы
...
Функция ОткрытыеСмены(РабочееМесто, ИдентификаторУстройства = Неопределено)
	...
	|		ПО КассоваяСмена.ФискальноеУстройство = ПодключаемоеОборудование.Ссылка
	|ГДЕ
	|	КассоваяСмена.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыКассовойСмены.Открыта)
	|	И ПодключаемоеОборудование.Ссылка = &ИдентификаторУстройства
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПодключаемоеОборудование.Ссылка = &ИдентификаторУстройства", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РозничнаяПродажа.ИдентификаторУстройства = &ИдентификаторУстройства", "ИСТИНА");
	КонецЕсли;
