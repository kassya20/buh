...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗагрузитьЗаказыИнтернетМагазинаВФоне", Истина);
...
Функция ПолучитьДанныеССервера(Соединение, СтрокаЗапроса, Заголовки = "", ОписаниеОшибки = "")
	...
	Если HTTPОтветСервера.КодСостояния<>200 Тогда
		ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Ошибка получения данных с сайта. Код ответа: '") + HTTPОтветСервера.КодСостояния);
		Если ЗначениеЗаполнено(ОтветСервера) Тогда
			ДобавитьОписаниеОшибки(ОписаниеОшибки, НСтр("ru = 'Ответ сервера: '") + ОтветСервера);
		КонецЕсли;
		Возврат Неопределено;
	КонецЕсли;
	...
	НомерПопытки = 1;
	ТестовыйСимвол = Символ(65533);
	Пока СтрНайти(ОтветСервера, ТестовыйСимвол)>0 И НомерПопытки<=5 Цикл
...
Процедура ЗаполнитьСчетПоДаннымДокументаXDTO(Заказ, СвойстваЗаказа, Параметры)
	...
	ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(Параметры.Организация, ДатаДокумента)
		Или УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Параметры.Организация, ДатаДокумента);
	
	Если НЕ ЗначениеЗаполнено(СведенияОНалогах.СтавкаНДС) Тогда
		СведенияОНалогах.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПриРеализации(ДатаДокумента, Параметры.Организация);
	КонецЕсли;
...
Функция ПолучитьСтавкуНДСПеречислением(СтавкаНДС)
	...
	Возврат УчетНДС.СтавкаНДСПоПредставлению(СтавкаНДС);
...
Функция НовыйСтатистикаЗагрузки()
	...
	ДанныеОнлайнОплат = Новый Структура;
	ДанныеОнлайнОплат.Вставить("Обработано", 0);
	ДанныеОнлайнОплат.Вставить("Создано"   , Новый Массив);
	ДанныеОнлайнОплат.Вставить("Обновлено" , Новый Массив);
	ДанныеОнлайнОплат.Вставить("Пропущено" , Новый Соответствие); // Ключ - оплата, Значение - причина
	
	СтатистикаЗагрузки.Заказы = Заказы;
	СтатистикаЗагрузки.ОнлайнОплаты = ДанныеОнлайнОплат;
