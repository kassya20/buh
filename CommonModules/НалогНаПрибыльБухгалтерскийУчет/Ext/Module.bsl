...
// С декабря 2024 года настоятельно рекомендуем переходить с затратного метода учета ПБУ 18 на балансовый метод.
// Метод возвращает признак того, является ли этот период переходным с затратного метода на балансовый.
//
// Параметры:
//  Период - Дата
...
//  Булево
//
Функция ВыполняетсяПереходНаБалансовыйМетод(Период, Организация) Экспорт
	
	Если Не ВозможенПереходВМежотчетномПериоде(Период) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УчетнаяПолитика.ВариантУчетаОтложенногоНалога КАК ВариантУчетаОтложенногоНалога
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &Организация) КАК УчетнаяПолитика";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Не Выборка.Следующий() Или Не Перечисления.ВариантыУчетаОтложенногоНалога.ЭтоЗатратныйМетод(Выборка.ВариантУчетаОтложенногоНалога) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат БудущийВариантУчетаОтложенногоНалога(Период, Организация) <> Неопределено;
...
Процедура ИнициализироватьРасчетОтложенногоНалога(Движения, ПериодРасчета, Организация, ВидОперации) Экспорт
	
	КонтекстРасчета = НовыйКонтекстРасчета(Организация, ПериодРасчета, ВидОперации);
	Если КонтекстРасчета = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не Перечисления.ВариантыУчетаОтложенногоНалога.ЭтоБалансовыйМетод(КонтекстРасчета.ВариантУчетаОтложенногоНалога)
		И КонтекстРасчета.ИзменениеУчетнойПолитики = Неопределено Тогда
		// Используется только при переходе на балансовый метод
		Возврат;
	КонецЕсли;
...
Функция ВозможенПереходВМежотчетномПериоде(Период)
	
	ГодНачалаПримененияФункциональности = 2024;
	Возврат Год(Период) >= ГодНачалаПримененияФункциональности;
	
КонецФункции

Функция БудущийВариантУчетаОтложенногоНалога(Период, Организация)
	...
	Запрос.УстановитьПараметр("Период",      КонецГода(Период) + 1);
	Запрос.УстановитьПараметр("Организация", Организация);
	...
	Если Не Перечисления.ВариантыУчетаОтложенногоНалога.ЭтоБалансовыйМетод(Выборка.ВариантУчетаОтложенногоНалога) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ОтразитьПоследствияНачисленияНалога(Движения, Период, Организация, ВидОперации, Ссылка) Экспорт
	
	КонтекстРасчета = НовыйКонтекстРасчета(Организация, Период, ВидОперации);
	...
	КонтекстРасчета.Вставить("ВидОперации", ВидОперации);
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога Тогда
		КонтекстРасчета.ПрименяетсяФСБУ25 = УчетнаяПолитика.ПрименяетсяФСБУ25(Организация, КонецГода(Период) + 1);
		КонтекстРасчета.НомерРасчета = 1;
		КонтекстРасчета.УточнениеПериода = БухгалтерскийУчетКлиентСервер.ОсновноеЗначениеУточненияМежотчетногоПериода();
	КонецЕсли;
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереходНаФСБУ6 Тогда
		КонтекстРасчета.НомерРасчета = 2;
		КонтекстРасчета.УточнениеПериода = БухгалтерскийУчетКлиентСервер.ОсновноеЗначениеУточненияМежотчетногоПериода();
	КонецЕсли;
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.ПереводОСВМалоценноеОборудование Тогда
		КонтекстРасчета.НомерРасчета = 3;
		КонтекстРасчета.УточнениеПериода = БухгалтерскийУчетКлиентСервер.ОсновноеЗначениеУточненияМежотчетногоПериода();
	КонецЕсли;
	...
	ЗаписатьОтражениеИнвестиционногоВычета(Движения.Хозрасчетный, РасчетРасходаПоНалогу, КонтекстРасчета);
	...
	РегистрыСведений.ПримененныеСтавкиОтложенногоНалога.ЗаписатьРасчетСтавокНалога(
		...
		КонтекстРасчета.Организация, 
		,
		РасчетРасходаПоНалогу.НомерРасчета);
...
Функция СчетаТекущегоНалога() Экспорт
	...
	СчетаТекущегоНалога.Добавить(ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль); // 99.02.И Инвестиционный вычет
...
Функция НовыйКонтекстРасчета(Организация, ПериодРасчета, ВидОперации)
	...
	КонтекстРасчета.Вставить("ИзменениеУчетнойПолитики", Неопределено);
	
	Если ВидОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога
		И ВозможенПереходВМежотчетномПериоде(ПериодРасчета) Тогда
		
		БудущийВариантУчета = БудущийВариантУчетаОтложенногоНалога(ПериодРасчета, Организация);
		Если БудущийВариантУчета <> КонтекстРасчета.ВариантУчетаОтложенногоНалога Тогда
			КонтекстРасчета.ИзменениеУчетнойПолитики = БудущийВариантУчета;
		КонецЕсли;
...
Функция НовыйИзменениеОтложенногоНалога()
	...
	Изменение.Колонки.Добавить("НомерРасчета",                        Измерения.НомерРасчета.Тип);
	// на отчетную дату
...
Функция СчетаКапитала()
	
	// Временные разницы определяются в стоимости активов и обязательств (п. 8 ПБУ 18).
	// Суммы на счетах капитала (включая промежуточные счета формирования прибыли) приниматься в расчет не должны.
	
	СчетаКапитала = Новый Массив;
	
	// - собственно капитал
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.УставныйКапитал);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.СобственныеАкции);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.РезервныйКапитал);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.ДобавочныйКапитал);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.НераспределеннаяПрибыль);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.ЦелевоеФинансирование);
	
	// - доходы и расходы
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.Продажи);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеДоходыИРасходы);
	СчетаКапитала.Добавить(ПланыСчетов.Хозрасчетный.ПрибылиИУбытки);
	
	Возврат СчетаКапитала;
...
Функция СчетаБезНалоговойСтоимости()
	...
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.ОбесценениеОС_01);
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.ОбесценениеНематериальныхАктивов);
	...
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.ГрупповыеОбъектыОС);
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияГрупповыхОбъектовОС);
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.ЗатратыНаРемонтОС);
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияЗатратНаРемонтОС);
	СчетаБезНалоговойСтоимости.Добавить(ПланыСчетов.Хозрасчетный.ВложенияВРемонтыОсновныхСредств);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		СчетаБезНалоговойСтоимости, 
...
// Описывает правила группировки активов обязательств
//
// Параметры:
//  КонтекстРасчета - см. РасчетНалогаНаПрибыль.НовыйКонтекстРасчета, НовыйКонтекстРасчета
// 
// Возвращаемое значение:
//  см. НовыйПравилаГруппировкиАктивовОбязательств
//
Функция ПравилаГруппировкиАктивовОбязательств(КонтекстРасчета) Экспорт
	
	Правила = НовыйПравилаГруппировкиАктивовОбязательств();
	ЗаполнитьПравилаГруппировкиВидовАктивовИОбязательств(Правила);
	ПрименитьКонтекстРасчетаПравилГруппировки(Правила, КонтекстРасчета);
	
	Возврат Правила;
	
КонецФункции

Функция НовыйПравилаГруппировкиАктивовОбязательств()
...
Процедура ЗаполнитьПравилаГруппировкиВидовАктивовИОбязательств(Правила)
	...
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияГрупповыхОбъектовОС);
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияЗатратНаРемонтОС);
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ОбесценениеОС_01);
	
	// АрендныеОбязательства
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.КорректировкаСтоимостиАрендованногоИмущества;
	...
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.АмортизацияНематериальныхАктивов_);
	...
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства_;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.НезавершенноеПроизводство;
	
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ВыполнениеРемонтовОС;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ВнеоборотныеАктивы;
	...
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ПриобретениеПравНаИспользованиеРезультатовИнтеллектуальнойДеятельности;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ДебиторскаяЗадолженность;
	
	// Курсовые разницы
	СчетаРасчетов = УчетВзаиморасчетов.ОсновныеСчетаРасчетов();

	СчетаРасчетовВУсловныхЕдиницах = ОбщегоНазначенияБПКлиентСервер.ЗначенияКоллекции(
		СчетаРасчетов[Перечисления.ВидыРасчетовПоДоговорам.РасчетыВУсловныхЕдиницах],
		Истина);

	Для Каждого СчетУчета Из СчетаРасчетовВУсловныхЕдиницах Цикл
		Правило = Правила.Добавить();
		Правило.ОсновнойСчет           = СчетУчета;
		Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.КурсовыеРазницыПоРасчетамВУЕ;
	КонецЦикла;

	СчетаРасчетовВИностраннойВалюте = ОбщегоНазначенияБПКлиентСервер.ЗначенияКоллекции(
		СчетаРасчетов[Перечисления.ВидыРасчетовПоДоговорам.РасчетыВИностраннойВалюте],
		Истина);

	Для Каждого СчетУчета Из СчетаРасчетовВИностраннойВалюте Цикл
		Правило = Правила.Добавить();
		Правило.ОсновнойСчет           = СчетУчета;
	...
	// Проценты по обязательствам по рассрочке
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочке;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПроцентыПоОбязательствам;
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочкеВал);
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочкеУЕ);
	
	// Проценты по обязательствам по аренде
	Правило = Правила.Добавить();
	Правило.ОсновнойСчет           = ПланыСчетов.Хозрасчетный.ПроцентыПоАренде;
	Правило.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПроцентыПоОбязательствам;
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеВал);
	Правило.ДополнительныеСчета.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоАрендеУЕ);
...
Процедура ПрименитьКонтекстРасчетаПравилГруппировки(Правила, КонтекстРасчета)
	
	ПрименяетсяФСБУ25 = КонтекстРасчета.ПрименяетсяФСБУ25;
	ПоддерживаетсяДисконтирование = УчетнаяПолитика.ПоддерживаетсяДисконтированиеОбязательств();
	
	Если ПрименяетсяФСБУ25 И ПоддерживаетсяДисконтирование Тогда
		Возврат;
	КонецЕсли;
	
	УдаляемыеСтроки = Новый Массив;
	
	Для каждого СтрокаПравил Из Правила Цикл
		
		Если Не ПрименяетсяФСБУ25 
			И СтрокаПравил.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.АрендныеОбязательства Тогда 
			УдаляемыеСтроки.Добавить(СтрокаПравил);
		КонецЕсли;
		
		Если СтрокаПравил.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПроцентыПоОбязательствам
			И СтрокаПравил.ОсновнойСчет = ПланыСчетов.Хозрасчетный.ПроцентыПоАренде Тогда
			УдаляемыеСтроки.Добавить(СтрокаПравил);
		КонецЕсли;
		
		Если Не ПоддерживаетсяДисконтирование
			И СтрокаПравил.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.ПроцентыПоОбязательствам
			И СтрокаПравил.ОсновнойСчет = ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочке Тогда
			УдаляемыеСтроки.Добавить(СтрокаПравил);
		КонецЕсли;
	...
	Для каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		Правила.Удалить(УдаляемаяСтрока);
	КонецЦикла;
...
Функция РассчитатьОтложенныйНалог(КонтекстРасчета, ОбщаяСуммаНалога)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МАКСИМУМ(НачальныйРасчет.НомерРасчета) КАК МаксимальныйНомерРасчета
	|ПОМЕСТИТЬ НачальныйРасчет
	|ИЗ
	|	РегистрСведений.ПримененныеСтавкиОтложенногоНалога КАК НачальныйРасчет
	|ГДЕ
	|	НачальныйРасчет.ОтчетнаяДата = &НачальныеДанныеОтчетнаяДата
	|	И НачальныйРасчет.Организация = &Организация
	|	И НачальныйРасчет.ПериодРасчета = &НачальныеДанныеПериодРасчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ НачальныйРасчет КАК НачальныйРасчет
	|			ПО (ИСТИНА)
	|	ГДЕ
	...
	|				ИЛИ НачальныеДанные.ОтложенныйНалог <> 0)
	|		И НачальныеДанные.НомерРасчета = НачальныйРасчет.МаксимальныйНомерРасчета) КАК Изменения
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если КонтекстРасчета.НомерРасчета > 0 Тогда
		Расчет.ОтложенныйНалог.ЗаполнитьЗначения(КонтекстРасчета.НомерРасчета, "НомерРасчета");
		Расчет.ВременныеРазницы.ЗаполнитьЗначения(КонтекстРасчета.НомерРасчета, "НомерРасчета");
		Расчет.ЭффектИзмененияСтавокНалогаНаПрибыль.ЗаполнитьЗначения(КонтекстРасчета.НомерРасчета, "НомерРасчета");
	КонецЕсли;
...
Процедура СоздатьСальдоВременныхРазниц(МенеджерВременныхТаблиц, ОтчетнаяДата, КонтекстРасчета)
	...
	ВариантВедетсяУчетРазниц = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы;
	
	ВедетсяУчетРазниц = (КонтекстРасчета.ВариантУчетаОтложенногоНалога = ВариантВедетсяУчетРазниц);
	Если КонтекстРасчета.ИзменениеУчетнойПолитики <> Неопределено Тогда
		ВедетсяУчетРазниц = (КонтекстРасчета.ИзменениеУчетнойПолитики = ВариантВедетсяУчетРазниц);
	КонецЕсли;
	
	ПравилаГруппировки = ПравилаГруппировкиАктивовОбязательств(КонтекстРасчета);
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	// При получении остатков на начало следующего года, если операция выполняется в отчетном периоде,
	// остатки должны получаться без захвата межотчетного периода.
	ЭтоНачалоСледующегоГода = ОтчетнаяДата = КонецГода(КонтекстРасчета.КонецПериода) + 1;
	ЭтоОперацияОтчетногоПериода = КонтекстРасчета.УточнениеПериода = 0;
	Запрос.УстановитьПараметр("ОтчетнаяДата", ?(ЭтоНачалоСледующегоГода И ЭтоОперацияОтчетногоПериода,
...
Процедура ЗаписатьИзменениеОтложенногоНалога(Проводки, РасчетОтложенногоНалога, КонтекстРасчета)
	...
	Если КонтекстРасчета.ВидОперации = Перечисления.ВидыРегламентныхОпераций.КорректировкаОтложенногоНалога Тогда
		
		СчетРасчетаНалога = ПланыСчетов.Хозрасчетный.ПрибыльПодлежащаяРаспределению; // 84.01
		СчетПересчетаОтложенногоНалога = ПланыСчетов.Хозрасчетный.ПрибыльПодлежащаяРаспределению;
		
	ИначеЕсли ОтдельныеПроводкиИзмененияСтавкиНалога Тогда
...
Функция ОтдельныеПроводкиИзмененияСтавкиНалога(КонтекстРасчета)
	...
	Возврат (КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы)
		Или (КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод
			И КонтекстРасчета.ИзменениеУчетнойПолитики = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы);
...
Функция КорректировкиОтложенногоНалога(ИзменениеОтложенногоНалога, КонтекстРасчета, ОтдельныеПроводкиИзмененияСтавкиНалога)
	...
	Запрос.УстановитьПараметр("СчетаОтложенныйНалог",   СчетаОтложенныйНалог());
	Запрос.УстановитьПараметр("НеиспользованныеАктивы", Перечисления.ВидыАктивовИОбязательств.НеиспользованныеНалоговыеУбыткиВычеты());
	// При получении остатков на конец года, если операция выполняется в отчетном периоде,
	// остатки должны получаться без захвата межотчетного периода.
	ЭтоКонецГода = КонецДня(КонтекстРасчета.КонецПериода) = КонецГода(КонтекстРасчета.КонецПериода);
	ЭтоОперацияОтчетногоПериода = КонтекстРасчета.УточнениеПериода = 0;
	Запрос.УстановитьПараметр("ОтчетнаяДата",           ?(ЭтоКонецГода И ЭтоОперацияОтчетногоПериода,
	                                                      Новый Граница(КонтекстРасчета.КонецПериода, ВидГраницы.Включая),
	...
	Запрос.УстановитьПараметр("Организации",            КонтекстРасчета.Организации);
	Запрос.УстановитьПараметр("НомерРасчета",           КонтекстРасчета.НомерРасчета);
	...
	ПереходСЗатратногоМетода = (КонтекстРасчета.ИзменениеУчетнойПолитики <> Неопределено
		И КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод);
	
	Для Каждого Корректировка Из Корректировки Цикл
		
		Если ПереходСЗатратногоМетода Тогда
			// При применении затратного метода проводки формируются исходя из оборотов каждого конкретного месяца,
			// таким образом, в общем случае данные об оборотах ничего не значат - на них ориентироваться нельзя.
			ИзменениеОтложенногоНалога = Корректировка.УвеличениеОтложенногоНалога + Корректировка.УменьшениеОтложенногоНалога;
			Если ИзменениеОтложенногоНалога > 0 Тогда
				Корректировка.УвеличениеОтложенногоНалога = ИзменениеОтложенногоНалога;
				Корректировка.УменьшениеОтложенногоНалога = 0;
			Иначе
				Корректировка.УвеличениеОтложенногоНалога = 0;
				Корректировка.УменьшениеОтложенногоНалога = ИзменениеОтложенногоНалога;
			КонецЕсли;
		КонецЕсли;
...
Процедура ДобавитьПроводкуОтложенныйНалог(Проводки, Сумма, Содержание, ВидДвижения, Счет, ВидАктивовИОбязательств, КорСчет, КонтекстРасчета)
	...
	Проводка.УточнениеПериода = КонтекстРасчета.УточнениеПериода;
...
Функция БалансовыеСчетаВременныхРазниц(ВедетсяУчетРазниц)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетаКапитала",              СчетаКапитала());
	...
	|ГДЕ
	|	НЕ Счета.Ссылка В ИЕРАРХИИ (&СчетаКапитала)
...
Функция ПланГруппировкиАктивовОбязательств(ПравилаГруппировки, ВедетсяУчетРазниц)
	...
	ПланГруппировки.Колонки.Добавить("НомерВариантаИсключения", ОбщегоНазначения.ОписаниеТипаЧисло(1)); // максимальное значение = 2^3 - 1 = 7
...
Функция НовыйРасчетРасходаПоНалогу()
	...
	// Основной расчет - 0, возможны перерасчеты в том же периоде при изменении учетной политики, 
	// например, при начале применения нового ФСБУ
	Расчет.Вставить("НомерРасчета", 0); 
...
Функция РассчитатьРасходПоНалогу(КонтекстРасчета, РасчетОтложенногоНалога, НачисленныйНалог)
	...
	Расчет.НомерРасчета    = КонтекстРасчета.НомерРасчета;
	
	// Порядок расчета текущего налога (см. НачисленныйНалог)
	Если КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы
		Или КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод Тогда
	...
	// См. СуммарноеИзменениеОтложенногоНалога: в расчет включаем все последствия,
	// как обозначенные явно ("эффекты"), так и нет (разного рода погрешности, расхождения и т.п.)
...
Процедура ЗаписатьРасчетНалога(Проводки, РасчетТекущегоНалога, СуммаНачисленногоНалога, КонтекстРасчета)
	...
	Если КонтекстРасчета.ИзменениеУчетнойПолитики <> Неопределено Тогда
		// В контексте перехода не требуется
		Возврат;
	КонецЕсли;
...
// Формирует проводки по инвестиционному вычету
// 
// Параметры:
//  Проводки - РегистрБухгалтерииНаборЗаписей.Хозрасчетный
//  РасчетРасходаПоНалогу - см. НовыйРасчетРасходаПоНалогу
//  КонтекстРасчета - см. НовыйКонтекстРасчета
Процедура ЗаписатьОтражениеИнвестиционногоВычета(Проводки, РасчетРасходаПоНалогу, КонтекстРасчета)
	
	Если КонтекстРасчета.ВариантУчетаОтложенногоНалога <> Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод
		И КонтекстРасчета.ВариантУчетаОтложенногоНалога <> Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы Тогда
		Возврат;
	КонецЕсли;

	Если Не КонтекстРасчета.ПрименяетсяИнвестиционныйВычет Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("СчетОтложенногоНалога", ПланыСчетов.Хозрасчетный.ОтложенныеНалоговыеАктивы);
	Запрос.УстановитьПараметр("СубконтоВидыАктивовИОбязательств",
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыАктивовИОбязательств);
	Запрос.УстановитьПараметр("ВидАктивовИнвестиционныйВычет", Перечисления.ВидыАктивовИОбязательств.ФедеральныйИнвестиционныйВычет);
	...
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецПериода, Счет = &СчетВычета, , Организация = &Организация) КАК ХозрасчетныйОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				&КонецПериода,
	|				Счет = &СчетОтложенногоНалога,
	|				&СубконтоВидыАктивовИОбязательств,
	|				Организация = &Организация
	|					И Субконто1 = &ВидАктивовИнвестиционныйВычет) КАК ХозрасчетныйОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				&СубконтоВидыАктивовИОбязательств,
	|				Организация = &Организация
	|					И Субконто1 = &ВидАктивовИнвестиционныйВычет,
	|				,
	|				) КАК ХозрасчетныйОбороты) КАК Остатки
	|
	|ИМЕЮЩИЕ
	...
	Если Выборка.Следующий() Тогда
		...
		СодержаниеПогашение  = НСтр("ru = 'Погашение отложенного налогового актива'", Метаданные.ОсновнойЯзык.КодЯзыка);
		...
		Если Выборка.ВычетОстаток > Выборка.ОтложенныйНалоговыйАктивОстаток Тогда
			...
			// Нужно увеличить остаток на счете 09 на разницу между остатком на счете ИНВ.Ф и остатком на счете 09,
			// но, может быть, это можно сделать путем уменьшения суммы погашения налогового актива
			// текущего периода (сторнировать проводку Дт 99.02.О Кт 09).
			...
			Если Выборка.ОтложенныйНалоговыйАктивПогашенВТекущемПериоде > 0 Тогда
				КорректировкаПогашения =
					Макс(Мин(Выборка.ВычетОстаток - Выборка.ОтложенныйНалоговыйАктивОстаток, Выборка.ОтложенныйНалоговыйАктивПогашенВТекущемПериоде), 0);
				СуммаПризнания = СуммаПризнания - КорректировкаПогашения;
			КонецЕсли;
		Иначе
			// Нужно уменьшить остаток на счете 09 (сумма должна быть такой же, как на счете ИНВ.Ф),
			// но, может быть, это можно сделать путем уменьшения суммы признания налогового актива
			// текущего периода (сторнировать проводку Дт 09 Кт 99.02.О).
			...
			Если Выборка.ОтложенныйНалоговыйАктивПризнанВТекущемПериоде > 0 Тогда
				КорректировкаПризнания =
					Макс(Мин(Выборка.ОтложенныйНалоговыйАктивОстаток - Выборка.ВычетОстаток, Выборка.ОтложенныйНалоговыйАктивПризнанВТекущемПериоде), 0);
		...
		Если СуммаПризнания > 0 Тогда
			ДобавитьПроводкуОтложенныйНалог(
				...
				СуммаПризнания,
				СодержаниеПризнание,
				ВидДвиженияПризнание,
				ПланыСчетов.Хозрасчетный.ОтложенныеНалоговыеАктивы,
				Перечисления.ВидыАктивовИОбязательств.ФедеральныйИнвестиционныйВычет,
				ПланыСчетов.Хозрасчетный.ОтложенныйНалогНаПрибыль,
				КонтекстРасчета);
		КонецЕсли;
	...
	РасчетИнвестиционногоВычета = РасчетИнвестиционногоВычета(КонтекстРасчета);
	
	Если РасчетИнвестиционногоВычета.ИнвестиционныйВычетЗаМесяц <> 0 Тогда
	
		Проводка = Проводки.Добавить();
		Проводка.Период           = КонтекстРасчета.КонецПериода;
		Проводка.Организация      = КонтекстРасчета.Организация;

		Проводка.Сумма       = РасчетИнвестиционногоВычета.ИнвестиционныйВычетЗаМесяц;
		...
		Если КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы Тогда
			...
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль; // 99.02.И
		
		Иначе
			Проводка.СчетДт = ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль; // 99.02.И
			Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ТекущийНалогНаПрибыль; // 99.02.Т
		КонецЕсли;
	...
	// Если не ведется учет постоянных разниц, то меняем расчет показателя с кодом (7):
	// он будет определяться как сумма постоянного налогового расхода (теперь вместо кода (7) это код (7А)) и примененного инвестиционного вычета (7Б).
	// Если ведется учет постоянных разниц, то применный инвестиционный вычет выделяется как отдельный показатель,
	// расчет показателя (7) не меняется. Для единообразия в обоих случаях код показателя примененного инвестиционного вычета - (7Б).
	
	// 7Б. Примененный инвестиционный Вычет
	РасчетРасходаПоНалогу.ИнвестиционныйВычет = РасчетИнвестиционногоВычета.ИнвестиционныйВычетЗаПериод;
		
	ПорядокРасчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Оборот по кредиту счета %1'", РасчетРасходаПоНалогу.КодЯзыка),
		КодСчета(ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль));
		
	ДобавитьЗаписьПротоколаРасчета(РасчетРасходаПоНалогу, "ИнвестиционныйВычет", ПорядокРасчета);
	
	Если КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод Тогда
		// Меняется порядок расчета: формула должна учитывать проводку Дт 68.04.1 Кт 99.02.И
		ПорядокРасчетаТекущийНалог = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Оборот по счету %1 в корреспонденции со счетами %2 и %3'", РасчетРасходаПоНалогу.КодЯзыка),
			КодСчета(ПланыСчетов.Хозрасчетный.ТекущийНалогНаПрибыль),
			КодСчета(ПланыСчетов.Хозрасчетный.РасчетыСБюджетом),
			КодСчета(ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль));

		ДобавитьЗаписьПротоколаРасчета(РасчетРасходаПоНалогу, "ТекущийНалог", ПорядокРасчетаТекущийНалог);
		
		Возврат;
	КонецЕсли;
	
	// 7А. Переносим ранее рассчитанное значение постоянного налогового расхода (дохода).
	РасчетРасходаПоНалогу.ПостоянныйНалоговыйРасходПоСуммеРазниц = РасчетРасходаПоНалогу.ПостоянныйНалоговыйРасход;
	ПорядокРасчета = РасчетРасходаПоНалогу.ПорядокРасчета[Перечисления.ПоказателиОтложенногоНалога.ПостоянныйНалоговыйРасход];
	ДобавитьЗаписьПротоколаРасчета(РасчетРасходаПоНалогу, "ПостоянныйНалоговыйРасходПоСуммеРазниц", ПорядокРасчета);
		
	// (7) = (7А) + (7Б)
	РасчетРасходаПоНалогу.ПостоянныйНалоговыйРасход =
		РасчетРасходаПоНалогу.ПостоянныйНалоговыйРасходПоСуммеРазниц + РасчетРасходаПоНалогу.ИнвестиционныйВычет;

	ПорядокРасчетаПостоянныйНалоговыйРасход = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '(%1) + (%2)'", РасчетРасходаПоНалогу.КодЯзыка),
		Перечисления.ПоказателиОтложенногоНалога.Код(Перечисления.ПоказателиОтложенногоНалога.ПостоянныйНалоговыйРасходПоСуммеРазниц, РасчетРасходаПоНалогу.КодыПоказателей),
		Перечисления.ПоказателиОтложенногоНалога.Код(Перечисления.ПоказателиОтложенногоНалога.ИнвестиционныйВычет, РасчетРасходаПоНалогу.КодыПоказателей));
	
	ДобавитьЗаписьПротоколаРасчета(РасчетРасходаПоНалогу, "ПостоянныйНалоговыйРасход", ПорядокРасчетаПостоянныйНалоговыйРасход);
	
КонецПроцедуры

Функция РасчетИнвестиционногоВычета(КонтекстРасчета)
	
	РасчетИнвестиционногоВычета = НовыйРасчетИнвестиционногоВычета();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачалоПериода",          КонтекстРасчета.НачалоНалоговогоПериода);
	Запрос.УстановитьПараметр("НачалоМесяца",           КонтекстРасчета.НачалоМесяца);
	Запрос.УстановитьПараметр("КонецПериода",           КонтекстРасчета.КонецПериода);
	Запрос.УстановитьПараметр("Организация",            КонтекстРасчета.Организация);
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(ХозрасчетныйОбороты.ИнвестиционныйВычетЗаПериод) КАК ИнвестиционныйВычетЗаПериод,
	|	СУММА(ХозрасчетныйОбороты.ИнвестиционныйВычетЗаМесяц) КАК ИнвестиционныйВычетЗаМесяц
	|ИЗ
	|	(ВЫБРАТЬ
	|		ЦЕЛ(ХозрасчетныйОбороты.СуммаНУОборотКт) КАК ИнвестиционныйВычетЗаПериод,
	|		0 КАК ИнвестиционныйВычетЗаМесяц
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка), , СчетКт = &СчетВычета, , Организация = &Организация) КАК ХозрасчетныйОбороты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		0,
	|		ЦЕЛ(ХозрасчетныйОбороты.СуммаНУОборотКт)
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоМесяца, &КонецПериода, , СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка), , СчетКт = &СчетВычета, , Организация = &Организация) КАК ХозрасчетныйОбороты) КАК ХозрасчетныйОбороты";
	
	ВыборкаИнвестиционныйВычет = Запрос.Выполнить().Выбрать();
	
	Если ВыборкаИнвестиционныйВычет.Следующий() Тогда
		ЗаполнитьЗначенияСвойств(РасчетИнвестиционногоВычета, ВыборкаИнвестиционныйВычет);
	КонецЕсли;
	
	Возврат РасчетИнвестиционногоВычета;
	
КонецФункции

Функция НовыйРасчетИнвестиционногоВычета()

	РасчетИнвестиционногоВычета = Новый Структура;
	РасчетИнвестиционногоВычета.Вставить("ИнвестиционныйВычетЗаМесяц", 0);
	РасчетИнвестиционногоВычета.Вставить("ИнвестиционныйВычетЗаПериод", 0);
...
Функция НачисленныйНалог(КонтекстРасчета)
	...
	УточнениеПериода = ?(КонтекстРасчета.УточнениеПериода = 0, 0, БухгалтерскийУчет.ВерхняяГраницаУточненияПериода());
	...
	Запрос.УстановитьПараметр("СчетИнвестиционногоВычета", ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль); // 99.02.И
		
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(СуммаНачисленногоНалога.НачисленныйНалог) КАК НачисленныйНалог
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	
	|	ВЫБРАТЬ
	|		-ЕСТЬNULL(СУММА(ХозрасчетныйОборотыДтКт.СуммаОборот), 0)
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСБюджетом), , СчетКт = &СчетИнвестиционногоВычета, , Организация = &Организация) КАК ХозрасчетныйОборотыДтКт) КАК СуммаНачисленногоНалога";
...
Процедура ЗаписатьРасчетПоказателейОтложенногоНалога(Движения, РасчетРасходаПоНалогу, РеквизитыЗаписи)
	...
	Для Каждого ОписаниеПоказателя Из Метаданные.Перечисления.ПоказателиОтложенногоНалога.ЗначенияПеречисления Цикл
		...
		ИначеЕсли Показатель = Перечисления.ПоказателиОтложенногоНалога.ПостоянныйНалоговыйРасходПоСуммеРазниц И Сумма > 0 Тогда
			Показатель = Перечисления.ПоказателиОтложенногоНалога.ПостоянныйНалоговыйДоходПоСуммеРазниц;
		КонецЕсли;
		...
		Запись.Сумма           = Сумма;
		Запись.НомерРасчета    = РасчетРасходаПоНалогу.НомерРасчета;
...
Функция СправкиРасчеты() Экспорт
	...
	ИменаОтчетов.Добавить("СправкаРасчетНалогаНаПрибыль");
	
	Возврат Новый ФиксированныйМассив(ИменаОтчетов);
	
КонецФункции

Функция СправкиРасчетыКорректировка() Экспорт
...
Процедура ДобавитьСправкиРасчеты(СписокВыбора, СправкиРасчеты) Экспорт
...
// См. также вариант с кешем (например, для использования в алгоритмах проведения) - НалоговыйУчетПовтИсп.ЭтоСчетБезНалоговойСтоимости
//
// Параметры:
...
Функция ЭтоСчетБезНалоговойСтоимости(Счет) Экспорт
	...
	Счета = СчетаБезНалоговойСтоимости();
	Возврат Счета.Найти(Счет) <> Неопределено;
