...
Процедура ПослеЗаписи(Форма, ПараметрыЗаписи) Экспорт
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику") Тогда
		...
		Оповестить("ИзменениеОплатыВедомости", ПараметрыЗарплаты, Объект.ПлатежнаяВедомость);
	КонецЕсли;
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям") Тогда
		...
		Для Каждого СтрокаТаблицы ИЗ Объект.ВыплатаЗаработнойПлаты Цикл
			...
			Оповестить("ИзменениеОплатыВедомости", , СтрокаТаблицы.Ведомость);
		КонецЦикла;
		ПараметрыЗарплаты = Новый Структура;
		ПараметрыЗарплаты.Вставить("Организация", Объект.Организация);
...
Процедура КонтрагентСоздание(Форма, Элемент, НазваниеКонтрагента, СтандартнаяОбработка) Экспорт
	
	РаботаСКонтрагентамиБПКлиент.КонтрагентСоздание(Элемент, НазваниеКонтрагента, СтандартнаяОбработка);
...
Процедура РеквизитыОрганизацииСсылкаОбработкаНавигационнойСсылки(Форма, Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	ПроверкаРеквизитовОрганизацииКлиент.РеквизитыОрганизацииСсылкаОбработкаНавигационнойСсылки(Форма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
...
Процедура РасшифровкаПлатежаПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если НоваяСтрока И (Форма.ПрименениеУСН И Не Форма.ПрименениеУСНДоходы) Тогда
...
Процедура РасшифровкаПлатежаДоговорКонтрагентаОткрытие(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	ВидыДоговора   = РасходныйКассовыйОрдерВызовСервера.ВидыДоговоровДокумента(Объект.ВидОперации);
...
Процедура РасшифровкаПлатежаСделкаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	ПараметрыОбъекта.Вставить("ТипыДокументов",        РасходныйКассовыйОрдерВызовСервера.ОписаниеТипаРеквизитаСделка());
...
Процедура ВыплатаЗаработнойПлатыВедомостьПриИзменении(Форма, Элемент) Экспорт
	...
	Если ЭтоОднострочнаяФорма Тогда
		...
		Ведомость = Форма.ВыплатаЗаработнойПлатыВедомость;
	...
	Иначе
		...
		Ведомость = СтрокаПлатеж.Ведомость;
		
	КонецЕсли;
	
	Если Не ФормаВыплатаЗаработнойПлаты(Форма)
		И ЗначениеЗаполнено(Ведомость) И Объект.ВыплатаЗаработнойПлаты.Количество() = 1 Тогда
		Объект.Основание = ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(
			Ведомость, НСтр("ru = 'Ведомость в кассу'"));
	КонецЕсли;
...
Процедура ОткрытьФормуРедактированияРеквизитовПлатежаВБюджет(Форма, ИсправитьОшибки = Ложь, АдресОшибок = "")
	...
	// Заполним показатели
	РеквизитыОбъекта   = ПлатежиВБюджетКлиентСерверПереопределяемый.НовыеРеквизитыПлатежаВБюджет(Объект);
	...
	Для каждого ОписаниеРеквизита Из РеквизитыДокумента Цикл
		РеквизитыПлатежаВБюджет[ОписаниеРеквизита.Ключ] = РеквизитыОбъекта[ОписаниеРеквизита.Значение];
	КонецЦикла;
	
	// Заполним контекст
	СвойстваКонтекста = ПлатежиВБюджетКлиентСервер.НовыйКонтекстПлатежногоДокумента();
	СвойстваКонтекста.Период                      = Объект.Дата;
	СвойстваКонтекста.Организация                 = Объект.Организация;
	СвойстваКонтекста.СчетПолучателя              = Объект.СчетКонтрагента;
	СвойстваКонтекста.Налог                       = Объект.Налог;
	СвойстваКонтекста.ВидНалоговогоОбязательства  = Объект.ВидНалоговогоОбязательства;
	СвойстваКонтекста.ПеречислениеВБюджет         = Истина;
...
Процедура ЗадатьВопросОПроверкеПравильностиРеквизитов(Форма, РезультатПроверки, ПараметрыЗаписи, Отказ) Экспорт
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ВопросЗаписатьСОшибкамиЗавершение", Форма, ПараметрыЗаписи),
		ТекстВопроса,
		РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры

Функция ФормаВыплатаЗаработнойПлаты(Форма)
	
	Возврат СтрНайти(Форма.ИмяФормы, "ФормаВыплатаЗаработнойПлаты") > 0;
...
Процедура ОбработкаРедактированияРеквизитовПлатежаВБюджетЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для каждого РеквизитПлатежаВБюджет Из Результат Цикл
		...
		Если ИмяРеквизита <> Неопределено Тогда
			РеквизитыОбъекта[ИмяРеквизита] = РеквизитПлатежаВБюджет.Значение;
		КонецЕсли;
	КонецЦикла;
