...
Процедура ПриРасшифровкеТранзакции(Транзакция, Результат) Экспорт
	...
	МодульСервиса = ОбщегоНазначения.ОбщийМодуль(ИмяМодуля);
	МодульСервиса.ПриРасшифровкеТранзакции(РеквизитыТранзакции, Результат);
...
Процедура ПослеЗагрузкиДанныхУслугиБанка(УслугаБанка, ИмяСервиса) Экспорт
	...
	ИначеЕсли ИмяСервиса = "ЗаявкиНаОткрытиеСчета" Тогда
		ИмяМодуля = "ЗаявкиНаОткрытиеСчета";
	ИначеЕсли ИмяСервиса = "ФинУслуги" Тогда
		ИмяМодуля = "ФинУслуги";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ПеречислениеХарактеристикСервиса(Сервис, Перечисление) Экспорт
	...
	ИмяСервиса = ОбщегоНазначения.ИмяЗначенияПеречисления(Сервис);
	Если ИмяСервиса = "ЗаявкиНаКредит" Или ИмяСервиса = "ЗаявкиНаЛизинг" Тогда
		ИмяПеречисления = "ХарактеристикиСервисаЗаявкиНаКредит";
	ИначеЕсли ИмяСервиса = "ФинансоваяОтчетность" Тогда
		ИмяПеречисления = "ХарактеристикиСервисаФинансовойОтчетности";
	ИначеЕсли ИмяСервиса = "ЗаявкиНаОткрытиеСчета" Тогда
		ИмяПеречисления = "ХарактеристикиСервисаЗаявкиНаОткрытиеСчета";
	ИначеЕсли ИмяСервиса = "ФинУслуги" Тогда
		ИмяПеречисления = "ХарактеристикиСервисаФинУслуги";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура МакетСервиса(Сервис, Макет) Экспорт
	...
	ИмяСервиса = ОбщегоНазначения.ИмяЗначенияПеречисления(Сервис);
	Если ИмяСервиса = "ЗаявкиНаКредит" Тогда
		ИмяМакета = "ЗаявкиНаКредит";
	ИначеЕсли ИмяСервиса = "ЗаявкиНаЛизинг" Тогда
		ИмяМакета = "ЗаявкиНаЛизинг";
	ИначеЕсли ИмяСервиса = "ФинансоваяОтчетность" Тогда
		ИмяМакета = "ФинОтчетностьВБанки";
	ИначеЕсли ИмяСервиса = "ЗаявкиНаОткрытиеСчета" Тогда
		ИмяМакета = "ЗаявкиНаОткрытиеСчета";
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ГруппаНовогоСобытияДокументооборота(Сервис, ПараметрыСобытия, Группа) Экспорт
	...
	МодульСервиса = ОбщегоНазначения.ОбщийМодуль(ИмяМодуля);
	Группа = МодульСервиса.ГруппаНовогоСобытияДокументооборота(
...
Процедура ТребуетсяРасшифроватьДанныеХарактеристики(Сервис, Характеристика, Результат) Экспорт
	...
	// Характеристики, которые поставляются как архив файлов 
	// не расшифровываются на данном этапе.
	// Расшифровка происходит в модуле соответствующего сервиса.
	ИмяСервиса = ОбщегоНазначения.ИмяЗначенияПеречисления(Сервис);
	...
	Если ИмяСервиса = "ЗаявкиНаОткрытиеСчета"
		И (ИмяХарактеристики = "УсловияОбслуживания"
			...
			ИЛИ ИмяХарактеристики = "ДополнительныеФайлы"
			ИЛИ ИмяХарактеристики = "СогласияНаОбработкуДанных") Тогда
			
		Результат = Истина;
		
	ИначеЕсли ИмяСервиса = "ФинУслуги" И ИмяХарактеристики <> "ФайлыКреативов" Тогда
		
		Результат = Истина;
		
	ИначеЕсли ИмяСервиса = "ЗаявкиНаКредит" И ИмяХарактеристики = "СогласиеНаОбработкуДанных" Тогда
		
		Результат = Истина;
		
	ИначеЕсли ИмяСервиса = "ЗаявкиНаЛизинг" И ИмяХарактеристики = "СогласиеНаОбработкуДанных" Тогда
...
Процедура ТребуетсяРасшифроватьДанныеСервиса(Сервис, Результат) Экспорт
	...
	ИмяСервиса = ОбщегоНазначения.ИмяЗначенияПеречисления(Сервис);
	Если ИмяСервиса = "ЗаявкиНаОткрытиеСчета" Или ИмяСервиса = "ФинУслуги" Или ИмяСервиса = "ЗаявкиНаКредит" Или ИмяСервиса = "ЗаявкиНаЛизинг" Тогда
		Результат = Истина;
	КонецЕсли;
