...
Функция ОписаниеФормыРеглОтчета(Требование, Организация, РегистрацияВНалоговомОргане, Знач НачалоПериода, Знач КонецПериода, Знач Периодичность) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ОписаниеФормыРеглОтчета.Имя) Тогда
		...
		ОписаниеФормыНовогоРеглОтчета = ОписаниеФормыНовогоРеглОтчета(
			ИмяРеглОтчета, Организация, РегистрацияВНалоговомОргане, НачалоПериода, КонецПериода, Периодичность);
		
		Если ОписаниеФормыНовогоРеглОтчета = Неопределено Тогда
			Возврат Неопределено;
		КонецЕсли;
		
		ОписаниеФормыРеглОтчета.Имя = ОписаниеФормыНовогоРеглОтчета.Имя;
		ОписаниеФормыРеглОтчета.Параметры = ОписаниеФормыНовогоРеглОтчета.Параметры;
...
// Описывает форму нового регламентированного отчета.
//
// Параметры:
//   ИмяРеглОтчета - Строка - имя регламентированного отчета, описание действия для которого нужно получить.
//   Организация   - СправочникСсылка.Организации - ссылка на элемент справочника "Организации".
//   РегистрацияВНалоговомОргане - СправочникСсылка.РегистрацииВНалоговомОргане - ссылка на элемент справочника "РегистрацииВНалоговомОргане".
//   НачалоПериода - Дата - дата начала периода.
//   КонецПериода  - Дата - дата окончания периода.
...
//     * Параметры - Структура - параметры формы.
//
Функция ОписаниеФормыНовогоРеглОтчета(ИмяРеглОтчета, Организация, РегистрацияВНалоговомОргане, НачалоПериода, КонецПериода, Периодичность) Экспорт
	...
	ПолноеИмяФормыОтчета = СтрШаблон("%1Отчет.%2.Форма.%3",
		?(РегламентированнаяОтчетность.ЭтоВнешнийОтчет(ИмяРеглОтчета), "Внешний", ""),
		ИмяРеглОтчета,
		ПараметрыОткрытия.мВыбраннаяФорма);
	
	ОписаниеФормыРеглОтчета.Вставить("Имя", ПолноеИмяФормыОтчета);
	ОписаниеФормыРеглОтчета.Вставить("Параметры", ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыОткрытия));
...
//                        "ФормаОтчета2026Кв1" (необязательное свойство; не используется при
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СформироватьАвтоматическиРеглОтчетВФоне", Истина);
КонецПроцедуры
