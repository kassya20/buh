...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбработатьУчетнуюПолитикуРаздельныйУчетНДСДо2014Года");
	Методы.Вставить("ПодготовитьПараметрыКнигиПокупок", Истина);
	Методы.Вставить("ПодготовитьПараметрыКнигиПродаж", Истина);
...
Функция ТекстЗапросаПодготовкаСведенийРНПТЖурнал() Экспорт
	...
	Возврат УчетНДСБП.ТекстЗапросаПодготовкаСведенийРНПТЖурнал();
	
КонецФункции

Процедура ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, СтруктураПараметров, Запрос) Экспорт

	УчетНДСБП.ДополнитьИнформациейОСчетахФактурахНаОтгрузку(ДополнительнаяИнформация, СтруктураПараметров, Запрос);
...
Функция РеквизитыДляНадписиОСчетеФактуреВыданном(Знач Документ, СтруктураОтбора = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СчетФактура) Тогда
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СчетФактура,
			"Номер,Дата,ПредставлениеНомера,ВидСчетаФактуры,Исправление,НомерИсправления,НомерВходящегоДокумента,ОтредактированНомерСчетаФактуры,
		...
		Если ЗначенияРеквизитов.Исправление Тогда
			...
			КонецЕсли;
		ИначеЕсли ЗначенияРеквизитов.ОтредактированНомерСчетаФактуры Тогда
			РеквизитыСФ.НомерСчетаФактуры = ЗначенияРеквизитов.НомерВходящегоДокумента;
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(СчетФактура.ДоговорКонтрагента) Тогда
			...
			СтруктураПараметров.Вставить("НДСИсчисляетсяНалоговымАгентом",
				...
				И Перечисления.ВидыАгентскихДоговоров.НДСИсчисляетсяНалоговымАгентом(СвойстваДоговора.ВидАгентскогоДоговора));
		КонецЕсли; 
...
Функция ОпределитьНаличиеСчетовФактурПолученных(ПараметрыПоискаСчетовФактур) Экспорт
	...
	|				И НЕ НДСПредъявленныйОбороты.ДоговорКонтрагента.ВидАгентскогоДоговора В (&ВидыДоговоровРеализацииТоваров))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция НовыеПараметрыСостоянияСчетаФактуры(Флаг, ОбъектОснование = Неопределено, ВидСчетаФактуры = "СчетФактураВыданный") Экспорт
	...
	ПараметрыДействия.Вставить("ВалютаДоговора");       // значение для контроля, нужно ли перепроводить при групповом перепроведении
	...
	// См. ПроведениеСервер.УстановитьПризнакПроверкиРеквизитов()
	ПараметрыДействия.Вставить("СчитатьРеквизитыПроверенными", Ложь);
	...
	Если ОбъектОснование <> Неопределено Тогда
		...
		Если ПараметрыДействия.ГрупповоеПерепроведение
		   И ЗначениеЗаполнено(ОбъектОснование.ДоговорКонтрагента) Тогда
			ПараметрыДействия.ВалютаДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектОснование.ДоговорКонтрагента, "ВалютаВзаиморасчетов");
		КонецЕсли;
		...
		ПараметрыДействия.СчитатьРеквизитыПроверенными = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ОбъектОснование.ДополнительныеСвойства, "СчитатьРеквизитыПроверенными", Ложь);
...
Процедура УстановитьСостояниеСчетаФактуры(ПараметрыДействия, Отказ = Ложь, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина) Экспорт
	...
	Если ПараметрыДействия.ГрупповоеПерепроведение
		И ПараметрыДействия.ВалютаДоговора = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета() Тогда
	...
	Иначе
		...
		СчетФактураБезПометкиУдаления = Неопределено;
		Если ПараметрыДействия.ВидСчетаФактуры = "СчетФактураВыданный" Тогда
			...
			Если ПометкаУдаления Тогда
				СчетФактураБезПометкиУдаления = НайтиПодчиненныйСчетФактуруВыданныйНаРеализацию(
					ПараметрыДействия.ДокументОснование);
			КонецЕсли;
		Иначе
			...
			Если ПометкаУдаления Тогда
				СчетФактураБезПометкиУдаления = НайтиПодчиненныйСчетФактуруПолученный(
					ПараметрыДействия.ДокументОснование,
					ПараметрыДействия.ДокументОснование,
					Ложь,
					ПараметрыДействия.СтруктураОтбора);
		...
		Если Не ЗначениеЗаполнено(СчетФактура)
			ИЛИ ЗначениеЗаполнено(СчетФактураБезПометкиУдаления) Тогда
			Возврат;
		КонецЕсли;
	...
	Иначе // изменение состояния "Проведен"
		...
		Если ПараметрыДействия.СостояниеФлага Тогда // проведение
			...
			Если Не СчетФактураОбъект.ПроверитьЗаполнение() Тогда
				Возврат;
			КонецЕсли;
			...
			Если ПараметрыДействия.ГрупповоеПерепроведение Тогда
				...
				ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
	...
	КонецЕсли;
	Если ПараметрыДействия.СчитатьРеквизитыПроверенными Тогда
		СчетФактураОбъект.ДополнительныеСвойства.Вставить("СчитатьРеквизитыПроверенными", ПараметрыДействия.СчитатьРеквизитыПроверенными);
	КонецЕсли;
	
	Если СчетФактураНаходятсяВЗакрытомПериоде(СчетФактура, СчетФактураОбъект, ТекстСообщенияВСлучаеОшибки) Тогда 
		Возврат;
	КонецЕсли;
...
Процедура СинхронизироватьРеквизитыСчетаФактурыПолученного(ДокОбъект, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина) Экспорт
	...
	СинхронизироватьСтатусСчетаФактуры(СчетФактура, ДокОбъект, ОбновлятьСтатусСчетаФактурыПоДокументу);
	
КонецПроцедуры

Процедура ПроверитьСоответствиеРеквизитовСчетаФактурыВыданного(ДокОбъект, СтруктураОтбора = Неопределено, ОбновлятьСтатусСчетаФактурыПоДокументу = Истина) Экспорт
	...
	СинхронизироватьСтатусСчетаФактуры(СчетФактура, ДокОбъект, ОбновлятьСтатусСчетаФактурыПоДокументу, Истина);
...
//
// Параметры: 
//  ДокументПоступления - документ-основание корректировочного счета-фактуры.
//  ОбработанныеДокументы - массив обработанных документов, чтобы повторно не обрабатывать уже обработанные документы.
//
// Возвращаемое значение:
//  ДокументРеализации - ДокументСсылка.
//  Для корректировки поступления вернет либо последнюю корректировку поступления на основании которой составляется
//  исправленный счет-фактура, либо поступление товаров и услуг.
//  Для возврата поставщику возвращает документ поступления.
//  Для прочих документов вернет переданный документ поступления.
Функция ПолучитьОснованиеКорректируемогоСчетаФактурыПолученного(ДокументПоступления, ОбработанныеДокументы = Неопределено) Экспорт
	...
	СогласованноеИзменение = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение;
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументПоступления, "ВидОперации,ДокументПоступления");
	
	Если РеквизитыДокумента.ВидОперации <> СогласованноеИзменение 
		И ОпределитьВидСчетаФактурыПолученногоПоТипуОснования(ДокументПоступления) <> Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда
			Возврат ДокументПоступления;
	Иначе
		Если ОбработанныеДокументы = Неопределено Тогда
			ОбработанныеДокументы = Новый Массив;
		...
		Возврат ПолучитьОснованиеКорректируемогоСчетаФактурыПолученного(РеквизитыДокумента.ДокументПоступления, ОбработанныеДокументы);
	КонецЕсли;
...
Функция ПолучитьСуммуДокументаСНДС(ДокументОбъект, ИмяТабличнойЧасти = Неопределено, НеУчитыватьТару = Истина, Отбор = Неопределено) Экспорт
	...
	Если ИмяТабличнойЧасти <> Неопределено Тогда
		...
		ЕстьСуммаСкидки = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СуммаСкидки", МетаданныеДокумента, ИмяТабличнойЧасти);
	...
	Иначе
		...
		Для каждого ТЧОбъекта Из ДокументОбъект.Метаданные().ТабличныеЧасти Цикл
			...
			ИначеЕсли МетаданныеДокумента.Имя = "ОтчетОРозничныхПродажах" И ТЧОбъекта.Имя = "Возвраты"  Тогда
				// возвраты обрабатыватся отдельно
				Продолжить;
			ИначеЕсли МетаданныеДокумента.Имя = "ОтчетОРозничныхПродажах" 
				И (ТЧОбъекта.Имя = "ПодарочныеСертификаты" ИЛИ ТЧОбъекта.Имя = "Предоплаты") Тогда
				// НДС по проданным подарочным сертификатам и принятым предоплатам определяется расчетным методом
				ДобавитьСуммуСНДСПоПодарочнымСертификатам(ДокументОбъект, СуммаДокументаПоСтавкам, ТЧОбъекта.Имя);
				Продолжить;
			ИначеЕсли МетаданныеДокумента.Имя = "РеализацияТоваровУслуг" И ТЧОбъекта.Имя = "Смета" Тогда
				Продолжить;
			КонецЕсли;
...
Процедура СформироватьДвиженияВыбытиеТоваров(СписанныеПартииНДС, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ) Экспорт
	...
	РаздельныйУчетНДСБезВычетов = УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период);
	
	Если РаздельныйУчетНДСНаСчете19
		ИЛИ РаздельныйУчетНДСБезВычетов Тогда
...
Процедура СформироватьДвиженияРеализацияБезНДС(СписанныеПартииНДС, РеализованныеТоварыУслуги, Реквизиты, Движения, Отказ) Экспорт
	...
	Если ЗаполняетсяРаздел7ДекларацииНДС Тогда
		
		КодОперацииПоСделке = Реквизиты.КодОперацииПоСделке;
		
		Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
			И Реквизиты.ЭлектроннаяТорговаяПлощадка = Истина Тогда
			
			Код1010836 = Справочники.КодыОперацийРаздела7ДекларацииПоНДС.НайтиПоКоду("1010836");
			
			Если ЗначениеЗаполнено(Код1010836) Тогда
				Реквизиты.КодОперацииПоСделке = Код1010836;
			Иначе
				Событие =  НСтр("ru = 'при проведении документа'");
			...
			РеализованныеТоварыЧерезЭТПФизлицуВЕАЭС = РеализованныеТоварыУслуги.Скопировать(
				Новый Структура("РеализацияТоваровЧерезЭлектроннуюТорговуюПлощадкуФизлицуВЕАЭС", Истина));
				
			УчетНДСРаздельный.СформироватьДвиженияРеализацияБезНДС(СписанныеПартииНДС,
				РеализованныеТоварыЧерезЭТПФизлицуВЕАЭС, Реквизиты, Движения, Отказ);
			
				Если РаздельныйУчетНДСНаСчете19
					ИЛИ РаздельныйУчетНДСБезВычетов Тогда
				
				Реквизиты.КодОперацииПоСделке = КодОперацииПоСделке;
				
				ТоварыУслуги = РеализованныеТоварыУслуги.Скопировать(
					Новый Структура("РеализацияТоваровЧерезЭлектроннуюТорговуюПлощадкуФизлицуВЕАЭС", Ложь));
				
				УчетНДСРаздельный.СформироватьДвиженияРеализацияБезНДС(СписанныеПартииНДС, ТоварыУслуги, Реквизиты, Движения, Отказ);
				
			КонецЕсли;
			
		ИначеЕсли РаздельныйУчетНДСНаСчете19 ИЛИ РаздельныйУчетНДСБезВычетов Тогда
...
Функция СоответствиеВерсийПостановленияФормамСчетовФактур() Экспорт
	...
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура1096",        ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура1096Прослеж", ВерсияПостановленияНДС1137('20241001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура2026",        ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура2026Прослеж", ВерсияПостановленияНДС1137('20260101'));

	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура26",          ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("СчетФактура26Прослеж",   ВерсияПостановленияНДС1137('20260401'));
	...
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура1096",        ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура1096Прослеж", ВерсияПостановленияНДС1137('20241001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура2026",        ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура2026Прослеж", ВерсияПостановленияНДС1137('20260101'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура26",          ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("КорректировочныйСчетФактура26Прослеж",   ВерсияПостановленияНДС1137('20260401'));
	...
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_1096",           ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_1096",           ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_1096Прослеж",    ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_1096Прослеж",    ВерсияПостановленияНДС1137('20241001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_2026",           ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_2026",           ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_2026Прослеж",    ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_2026Прослеж",    ВерсияПостановленияНДС1137('20260101'));

	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_26",             ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_26",             ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус1_26Прослеж",      ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УПДСтатус2_26Прослеж",      ВерсияПостановленияНДС1137('20260401'));
	...
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_2023Кв4",        ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_2023Кв4Прослеж", ВерсияПостановленияНДС1137('20231001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_1096",           ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_1096",           ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_1096Прослеж",    ВерсияПостановленияНДС1137('20241001'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_1096Прослеж",    ВерсияПостановленияНДС1137('20241001'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_2026",           ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_2026",           ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_2026Прослеж",    ВерсияПостановленияНДС1137('20260101'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_2026Прослеж",    ВерсияПостановленияНДС1137('20260101'));
	
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_26",             ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_26",             ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус1_26Прослеж",      ВерсияПостановленияНДС1137('20260401'));
	СоответствиеВерсийПостановленияФормам.Вставить("УКДСтатус2_26Прослеж",      ВерсияПостановленияНДС1137('20260401'));
...
Процедура СформироватьДвиженияОприходованиеТоваров(ТаблицаТовары, Реквизиты, Движения, Отказ) Экспорт
	...
	РаздельныйУчетНДСНаСчете19 = УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Реквизиты.Организация, Реквизиты.Период)
		ИЛИ УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период);
...
// РЕАЛИЗАЦИЯ АВТОМАТИЧЕСКИ ВЫПУЩЕННОЙ ПРОДУКЦИИ

Процедура СформироватьДвиженияОприходованияИПродажиПродукции(ТаблицаТовары, Реквизиты, Движения, Отказ) Экспорт
	...
	Если РаздельныйУчетНДСНаСчете19 Тогда
		...
		УчетНДСРаздельный.СформироватьДвиженияНДСРаздельныйУчет(ТаблицаТовары, Реквизиты, ВидДвиженияНакопления.Приход, Движения);
		УчетНДСРаздельный.СформироватьДвиженияНДСРаздельныйУчет(ТаблицаТовары, Реквизиты, ВидДвиженияНакопления.Расход, Движения);
		
	Иначе
		
		УчетНДСБП.СформироватьДвиженияНДСПоПриобретеннымЦенностямОприходованиеТоваров(ТаблицаТовары, Реквизиты, Движения, Отказ);
		УчетНДСБП.СформироватьДвиженияНДСПоПриобретеннымЦенностямСписаниеАвтоматическиВыпущеннойПродукции(
			ТаблицаТовары, Реквизиты, Движения, Отказ);
	КонецЕсли;
...
Функция ПараметрыСчетаФактуры(СчетФактура) Экспорт
	...
	Результат = Новый Структура(
		...
		|ИдентификаторГосКонтракта, ЕстьПрослеживаемыеТовары, РеквизитыДокументовОтгрузки");
	...
	РеквизитыДокументовОтгрузки = Новый ТаблицаЗначений;
	РеквизитыДокументовОтгрузки.Колонки.Добавить("НаименованиеДокумента",         ОбщегоНазначения.ОписаниеТипаСтрока(255));
	РеквизитыДокументовОтгрузки.Колонки.Добавить("НомерДокумента",                ОбщегоНазначения.ОписаниеТипаСтрока(255));
	РеквизитыДокументовОтгрузки.Колонки.Добавить("ДатаДокумента",                 ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	
	Результат.РеквизитыОснований          = РеквизитыОснований;
	Результат.РеквизитыДокументовОтгрузки = РеквизитыДокументовОтгрузки;
	...
	ЗаявлениеОВвозеТоваровОтКомиссионера = Ложь;
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.СчетФактураПолученный") тогда
		ЭтоПолученныйСФ = Истина;
		ЗаявлениеОВвозеТоваровОтКомиссионера = СчетФактура.ЗаявлениеОВвозеТоваровОтКомиссионера;
	...
	Если ДокументыОснования.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ЭтоУниверсальныйДокумент", МетаданныеДокумента) Тогда
			ЭтоУниверсальныйДокумент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументыОснования[0], "ЭтоУниверсальныйДокумент");
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли СчетФактура.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный
		И ТипЗнч(СчетФактура.ДокументОснование) <> Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
		// Запрос объединяющий данные возврата поставщику и корректировки поступления
		Запрос.Текст = ТекстЗапросаКорректировочныйСчетФактураПолученный();
	...
	Иначе
		Для каждого ТипОснования Из ТипыОснований Цикл
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
				// Для исправленного счета-фактуры
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
				...
				РеквизитыСчетаФактуры.Вставить("ДокументОснование",              Неопределено);
				РеквизитыСчетаФактуры.Вставить("ДоговорКонтрагентаВидДоговора",  Неопределено);
				РеквизитыСчетаФактуры.Вставить("ИсходныйДокументРеализацииДата", Неопределено);
				
				Если ДокументыОснования.Количество() > 0
					И ЗначениеЗаполнено(ДокументыОснования[0])
					И ТипЗнч(ДокументыОснования[0]) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
					РеквизитыСчетаФактуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументыОснования[0], "ИсходныйДокументРеализации, ДоговорКонтрагента.ВидДоговора");
					РеквизитыСчетаФактуры.Вставить("ДокументОснование", ДокументыОснования[0]);
					Если РеквизитыСчетаФактуры.ИсходныйДокументРеализации = Неопределено
						Или РеквизитыСчетаФактуры.ИсходныйДокументРеализации = NULL
						Или Не ЗначениеЗаполнено(РеквизитыСчетаФактуры.ИсходныйДокументРеализации) Тогда
						РеквизитыСчетаФактуры.Вставить("ИсходныйДокументРеализацииДата", Неопределено);
					Иначе
						РеквизитыИсходногоДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыСчетаФактуры.ИсходныйДокументРеализации, "Дата");
						РеквизитыСчетаФактуры.Вставить("ИсходныйДокументРеализацииДата", РеквизитыИсходногоДокумента);
					КонецЕсли;
				КонецЕсли;
				...
				Запрос.УстановитьПараметр("ТоварыВРаботахВыводятсяВОтдельныхСтроках",
					?(РеквизитыСчетаФактуры.ИсходныйДокументРеализацииДата = Неопределено, Ложь,
					НЕ ВерсияПостановленияНДС1137(РеквизитыСчетаФактуры.ИсходныйДокументРеализацииДата) >= 6));
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
				И Не ЭтоПолученныйСФ Тогда
				
				ТекстЗапроса = ТекстЗапросаВозвратПоставщикуВыданныйСФ();
				
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
				Если ЭтоПолученныйСФ Тогда 
					ТекстЗапроса = ТекстЗапросаВозвратОтПокупателяПолученныйСФ();
				Иначе
					...
					ТекстЗапроса = ТекстЗапросаВозвратОтПокупателяВыданныйСФ();
				КонецЕсли;
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда 
				...
				Запрос.УстановитьПараметр("ЗаявлениеОВвозеТоваровОтКомиссионера", ЗаявлениеОВвозеТоваровОтКомиссионера);
			...
			ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.ПередачаТоваров") Тогда 
	...
	Иначе
		...
		ЗаполнятьДокументыОбОтгрузке    = РезультатЗапроса.Колонки.Найти("ЕстьТовары") <> Неопределено;
		Корректировка       = Ложь;
		ЭтоКорректировочный = Ложь;
		
		Если СчетФактура.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный
			И ТипЗнч(СчетФактура.ДокументОснование) <> Тип("ДокументСсылка.ОтчетКомитентуОПродажах")
			ИЛИ ТипДокументаОснования = Тип("ДокументСсылка.КорректировкаРеализации") 
		...
		Если Корректировка Тогда
			...
			Если Не ЭтоКорректировочный Тогда
				ИсправляемыйСчетФактура = СчетФактура.ИсправляемыйСчетФактура;
				Если ЗначениеЗаполнено(ИсправляемыйСчетФактура) Тогда
					Для Каждого ИсходнаяСтрокаДокументаОтгрузки Из ИсправляемыйСчетФактура.ДокументыОбОтгрузке Цикл
						СтрокаДокументаОтгрузки = РеквизитыДокументовОтгрузки.Добавить();
						СтрокаДокументаОтгрузки.НаименованиеДокумента = ИсходнаяСтрокаДокументаОтгрузки.НаименованиеДокумента; 
						СтрокаДокументаОтгрузки.НомерДокумента = ИсходнаяСтрокаДокументаОтгрузки.НомерДокумента;
						СтрокаДокументаОтгрузки.ДатаДокумента  = ИсходнаяСтрокаДокументаОтгрузки.ДатаДокумента;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			...
			Пока Выборка.Следующий() Цикл
				...
				Иначе
					Если ЗаполнятьПризнакПослеживаемости 
						И Результат.ЕстьПрослеживаемыеТовары = Ложь Тогда
				...
				Если ЗаполнятьДокументыОбОтгрузке
					И ТекущийДокументОснование <> Выборка.Ссылка Тогда
					ТекущийДокументОснование = Выборка.Ссылка;
					СтрокаДокументаОтгрузки = РеквизитыДокументовОтгрузки.Добавить();
					
					Если Выборка.ЕстьТовары Тогда
						СтрокаДокументаОтгрузки.НаименованиеДокумента = Справочники.ТипыДокументов.Накладная.ПолноеНаименование; 
					Иначе
						СтрокаДокументаОтгрузки.НаименованиеДокумента = Справочники.ТипыДокументов.Акт.ПолноеНаименование;
					КонецЕсли;
					Если ТипЗнч(ТекущийДокументОснование) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
						СтрокаДокументаОтгрузки.НомерДокумента = Выборка.НомерДокументаОтгрузки
							+ ПостфиксОтгрузочногоДокументаПоОказаниюУслуг(ТекущийДокументОснование, Ссылка);
					Иначе
						Если ЭтоУниверсальныйДокумент И ТипЗнч(Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
							НомерДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Ссылка, "Номер");
							СтрокаДокументаОтгрузки.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(НомерДокумента, Истина, Ложь);
						Иначе
							СтрокаДокументаОтгрузки.НомерДокумента = Выборка.НомерДокументаОтгрузки;
						КонецЕсли;
					КонецЕсли;
					
					СтрокаДокументаОтгрузки.ДатаДокумента  = Выборка.ДатаДокументаОтгрузки;
				КонецЕсли;
				
			КонецЦикла;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(РеквизитыДокументовОтгрузки)
			И (СчетФактура.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию
			ИЛИ СчетФактура.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.НаПоступление) Тогда
			// Для документов у которых нет отдельных запросов для получения параметоров собираем отгрузочные документы по основаниям.
			ЗаполнитьОтгрузочныеДокументыПоОснованиям(ДокументыОснования, РеквизитыДокументовОтгрузки);
		КонецЕсли;
	...
	Если ЗаявлениеОВвозеТоваровОтКомиссионера Тогда
		Результат.СуммаДокумента = Результат.СуммаДокумента - Результат.СуммаНДСДокумента;
	КонецЕсли;
...
Функция ПостфиксОтгрузочногоДокументаПоОказаниюУслуг(ДокументСсылка, СчетФактураСсылка)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОказаниеУслугКонтрагенты.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	Документ.ОказаниеУслуг.Контрагенты КАК ОказаниеУслугКонтрагенты
	|ГДЕ
	|	ОказаниеУслугКонтрагенты.Ссылка = &ДокументСсылка
	|	И ОказаниеУслугКонтрагенты.СчетФактура = &СчетФактураСсылка";
	
	Запрос.УстановитьПараметр("ДокументСсылка",    ДокументСсылка);
	Запрос.УстановитьПараметр("СчетФактураСсылка", СчетФактураСсылка);
...
Процедура СформироватьДвиженияВозвратТоваровВРознице(ТаблицаВозвращаемыеТовары, ТаблицаПартииВозвратовРаздельныйУчет, Реквизиты, Движения, Отказ, ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу = Неопределено) Экспорт
	
	УчетНДСРаздельный.СформироватьДвиженияВозвратТоваровВРознице(ТаблицаВозвращаемыеТовары, ТаблицаПартииВозвратовРаздельныйУчет, Реквизиты, Движения, Отказ, ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу);
...
Процедура НачислитьЕНППриВвозеТоваровИзЕАЭС(Реквизиты, СтрокаТаблицыНачислений, НачислениеНалоговНаЕНС) Экспорт
	
	НоваяСтрока = НачислениеНалоговНаЕНС.Добавить(); 
	
	НоваяСтрока.СчетУчета = ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате; // 68.42
	
	ЕНПНеплательщикаНДС  = НЕ УчетнаяПолитика.ПлательщикНДС(Реквизиты.Организация, Реквизиты.Период)
		И НЕ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Реквизиты.Организация, Реквизиты.Период);
	Если ЕНПНеплательщикаНДС Тогда
		НоваяСтрока.СуммаНалога = СтрокаТаблицыНачислений.СуммаБУ;
		НоваяСтрока.Начислено = СтрокаТаблицыНачислений.СуммаБУ;
		НоваяСтрока.Субконто2 = СтрокаТаблицыНачислений.КорСубконто2;
	Иначе
		НоваяСтрока.СуммаНалога = СтрокаТаблицыНачислений.НДС;
		НоваяСтрока.Начислено = СтрокаТаблицыНачислений.НДС;
		НоваяСтрока.Субконто2 = Реквизиты.ДоговорКонтрагента;
	КонецЕсли;
	НоваяСтрока.Сумма = НоваяСтрока.Начислено;
	НоваяСтрока.Субконто1 = Реквизиты.Контрагент;
	НоваяСтрока.Субконто3 = Реквизиты.Регистратор;
	
	МесяцУплаты               = ДобавитьМесяц(СтрокаТаблицыНачислений.ДатаПринятияНаУчет, 1);
	...
	НоваяСтрока.СрокУплаты    = ГрафикПереноса[СрокУплаты]; 
	
	НоваяСтрока.НачалоПериода = НачалоМесяца(СтрокаТаблицыНачислений.ДатаПринятияНаУчет);
	НоваяСтрока.КонецПериода = КонецМесяца(СтрокаТаблицыНачислений.ДатаПринятияНаУчет);
	
	НоваяСтрока.Содержание = НСтр("ru = 'Единый налоговый счет: Начисление налога за товары из Таможенного союза'",
		ОбщегоНазначения.КодОсновногоЯзыка());
	
КонецПроцедуры

// Возвращает признак ведения простого учета единого налогового платежа.
//
// Параметры
//  Период  -  тип дата, в данном параметре передается
//             дата на которую необходимо определить признак ведения простого учета.
//  Организация - СправочникСсылка.Организации - организация, для которой определяется ведение простого учета.
// Возвращаемое значение:
//  Булево   -  Истина если ведется простой учет и Ложь в противном случае.
//
Функция ВедетсяПростойУчетЕНП(Период, Организация) Экспорт
	
	Возврат ЕдиныйНалоговыйСчет.ЭтоПростойУчет(Период, Организация);
	
КонецФункции

Процедура ЗаполнитьОтгрузочныеДокументыПоОснованиям(ДокументыОснования, РеквизитыДокументовОтгрузки)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДанныеПервичныхДокументов.Документ КАК Документ,
	|	ДанныеПервичныхДокументов.Номер КАК НомерДокументаОтгрузки,
	|	ДанныеПервичныхДокументов.Дата КАК ДатаДокументаОтгрузки
	|ИЗ
	|	РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|ГДЕ
	|	ДанныеПервичныхДокументов.Документ В (&ДокументыОснования)";
	Запрос.УстановитьПараметр("ДокументыОснования", ДокументыОснования);
	Выборка = Запрос.Выполнить().Выбрать();
	
	НаименованиеДокумента = Справочники.ТипыДокументов.Накладная.ПолноеНаименование;
	
	Пока Выборка.Следующий() Цикл
		СтрокаДокументаОтгрузки = РеквизитыДокументовОтгрузки.Добавить();
		СтрокаДокументаОтгрузки.НаименованиеДокумента = НаименованиеДокумента;
		СтрокаДокументаОтгрузки.НомерДокумента = Выборка.НомерДокументаОтгрузки;
		СтрокаДокументаОтгрузки.ДатаДокумента  = Выборка.ДатаДокументаОтгрузки;
	КонецЦикла;
...
Функция НаименованиеУПДДляПечати() Экспорт
	
	Возврат Справочники.ТипыДокументов.УПД.ПолноеНаименование;
...
//			СуммаНДС	- Сумма НДС по ставке
//
Процедура ДобавитьСуммуСНДСПоПодарочнымСертификатам(ДокументОбъект, ТаблицаСумм, ИмяТабличнойЧасти)

	ПлательщикНДС     = УчетнаяПолитика.ПлательщикНДС(ДокументОбъект.Организация, ДокументОбъект.Дата);
	РеквизитыСтроки = Новый Структура("Сумма, СтавкаНДС", 0, УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(ДокументОбъект.Дата, ПлательщикНДС));
	
	Для каждого СтрокаТЧ Из ДокументОбъект[ИмяТабличнойЧасти]  Цикл
		
		ЗаполнитьЗначенияСвойств(РеквизитыСтроки, СтрокаТЧ);
		
		СтрокаТаблицыИтогов = ТаблицаСумм.Найти(РеквизитыСтроки.СтавкаНДС,"СтавкаНДС");
		Если СтрокаТаблицыИтогов = Неопределено тогда
			СтрокаТаблицыИтогов = ТаблицаСумм.Добавить();
			СтрокаТаблицыИтогов.СтавкаНДС = РеквизитыСтроки.СтавкаНДС;
		КонецЕсли;
		
		СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
			РеквизитыСтроки.Сумма, Истина, ПолучитьСтавкуНДС(РеквизитыСтроки.СтавкаНДС));
		
		СтрокаТаблицыИтогов.Сумма    = СтрокаТаблицыИтогов.Сумма    + РеквизитыСтроки.Сумма;
		СтрокаТаблицыИтогов.СуммаНДС = СтрокаТаблицыИтогов.СуммаНДС + СуммаНДС;
	КонецЦикла;
КонецПроцедуры
...
Функция ТекстЗапросаДокументРасчетовСКонтрагентом()
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ЛОЖЬ КАК ЕстьТовары,
	|	НДСПредъявленныйОстатки.СчетФактура КАК Ссылка
	|ИЗ
...
Функция ТекстЗапросаВозвратТоваровЧерезКомиссионера()
	...
	|	ВТ_ОтчетКомиссионераОПродажахВозвраты.Контрагент КАК Контрагент,
...
Функция ТекстЗапросаПоступлениеДопРасходов()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО Таблица.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И Таблица.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаРеализацияОтгруженныхТоваров()
	...
	|	ВТ_РеализацияОтгруженныхТоваров.ЕстьТовары КАК ЕстьТовары
	...
	|	ВТ_РеализацияОтгруженныхТоваров.ЕстьТовары
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(ВТ_РеализацияОтгруженныхТоваров.ЕстьТовары) КАК ЕстьТовары,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """""""""""""""") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ВТ_РеализацияОтгруженныхТоваров.Ссылка КАК Ссылка
	|ИЗ
	|	ВТ_РеализацияОтгруженныхТоваров КАК ВТ_РеализацияОтгруженныхТоваров
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ВТ_РеализацияОтгруженныхТоваров.Организация = ДанныеПервичныхДокументов.Организация
	|			И ВТ_РеализацияОтгруженныхТоваров.Ссылка = ДанныеПервичныхДокументов.Документ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """"""""""""""""),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|	ВТ_РеализацияОтгруженныхТоваров.Ссылка";
...
Функция ТекстЗапросаНачислениеНДСпоСМР()
	...
	Возврат 
	"ВЫБРАТЬ
	|	НачислениеНДСпоСМРхозспособом.Ссылка.Организация КАК Организация,
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """""""""""""""") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	НачислениеНДСпоСМРхозспособом.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.НачислениеНДСпоСМРхозспособом.СМРхозспособом КАК НачислениеНДСпоСМРхозспособом
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО НачислениеНДСпоСМРхозспособом.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И НачислениеНДСпоСМРхозспособом.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаКорректировкаРеализации(ТолькоПредварительнуюТаблицу = Ложь)
	...
	Иначе
		...
		|	МАКСИМУМ(ВЫБОР
		|			КОГДА ЕСТЬNULL(СведенияПрослеживаемостиДо.Количество, 0) - ЕСТЬNULL(СведенияПрослеживаемостиПосле.Количество, 0) <> 0
		|					ИЛИ ЕСТЬNULL(СведенияПрослеживаемостиДо.Сумма, 0) - ЕСТЬNULL(СведенияПрослеживаемостиПосле.Сумма, 0) <> 0
		|					ИЛИ ЕСТЬNULL(СведенияПрослеживаемостиДо.СуммаНДС, 0) - ЕСТЬNULL(СведенияПрослеживаемостиПосле.СуммаНДС, 0) <> 0
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ
		...
		|	И СведенияПрослеживаемостиДо.Ссылка В(&ДокументыОснования)
		|	И СведенияПрослеживаемостиПосле.Ссылка В(&ДокументыОснования)
		|
		|СГРУППИРОВАТЬ ПО
		|	СведенияПрослеживаемостиПосле.Ссылка,
		|	СведенияПрослеживаемостиПосле.ИдентификаторСтроки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВЫБОР
		|		КОГДА &КорректировочныйСчетФактура
		|				И &ТоварыВРаботахВыводятсяВОтдельныхСтроках
		|				И ВТ_ИзмененияПоСведениямПрослеживаемости.ЕстьИзменения
		|			ТОГДА ИСТИНА
		|		КОГДА &КорректировочныйСчетФактура
		|				И НЕ &ТоварыВРаботахВыводятсяВОтдельныхСтроках
		|				И (Услуги.Сумма - Услуги.СуммаДоИзменения <> 0
		|					ИЛИ Услуги.СуммаНДС - Услуги.СуммаНДСДоИзменения <> 0)
		|			ТОГДА ЕСТЬNULL(ВТ_ИзмененияПоСведениямПрослеживаемости.ТоварыВРаботах, ЛОЖЬ)
		|		КОГДА НЕ &КорректировочныйСчетФактура
		|			ТОГДА Услуги.Ссылка.ПередачаТоваровВСоставеРабот
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ
		|ИЗ
		...
		|		ПО Услуги.Ссылка = ВТ_ИзмененияПоСведениямПрослеживаемости.Ссылка
		|			И Услуги.ИдентификаторСтроки = ВТ_ИзмененияПоСведениямПрослеживаемости.ИдентификаторСтроки
		|ГДЕ
...
Функция ТекстЗапросаКорректировочныйСчетФактураПолученный()
	...
	|	Товары.Ссылка.ДоговорКонтрагента,
	|	"""",
	|	0,
	|	ВЫБОР
	|		КОГДА Товары.Ссылка.СуммаВключаетНДС
	...
	|			КОНЕЦ
	|	КОНЕЦ,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Документ, Товары.Ссылка.Сделка),
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеПервичныхДокументов.Документ, Товары.Ссылка.Сделка) ССЫЛКА Документ.ПоступлениеТоваровУслуг
	|			ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ДанныеПервичныхДокументов.Документ, Товары.Ссылка.Сделка) КАК Документ.ПоступлениеТоваровУслуг).ЕстьРасхождения
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	...
	|	ТаблицыКорректировки.ЕстьПрослеживаемыеТовары КАК ЕстьПрослеживаемыеТовары
	|ИЗ
...
Функция ТекстЗапросаВыданныеСчетаФактурыПеревыставленныеКомитенту()
	...
	|	НЕ ЕСТЬNULL(ОтчетКомитентуОПродажахТовары.Номенклатура.Услуга, ЛОЖЬ) КАК ЕстьТовары
	|ПОМЕСТИТЬ ВТ_Суммы
	|ИЗ
	...
	|	НЕ ЕСТЬNULL(ОтчетКомитентуОПродажахТовары.Номенклатура.Услуга, ЛОЖЬ)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	КлючСтроки
	...
	|	МАКСИМУМ(ВТ_Суммы.ЕстьПрослеживаемыеТовары) КАК ЕстьПрослеживаемыеТовары,
	|	МАКСИМУМ(ВТ_Суммы.ЕстьТовары) КАК ЕстьТовары,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """""""") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ВТ_ОтчетКомитентуОПродажах.Ссылка КАК Ссылка
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ВТ_ОтчетКомитентуОПродажах.Организация = ДанныеПервичныхДокументов.Организация
	|			И ВТ_ОтчетКомитентуОПродажах.Ссылка = ДанныеПервичныхДокументов.Документ
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """"""""),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|	ВТ_ОтчетКомитентуОПродажах.Ссылка";
...
Функция ТекстЗапросаВыданныеСчетаФактурыПокупателямПоОтчетуКомиссионера()
	...
	|	КОНЕЦ КАК СуммаДокументаКомиссия,
	|	МАКСИМУМ(ВТ_Суммы.ЕстьПрослеживаемыеТовары) КАК ЕстьПрослеживаемыеТовары,
	|	ВТ_Суммы.ЕстьТовары КАК ЕстьТовары,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """""""") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ВТ_ОтчетКомиссионераОПродажах.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.Покупатели КАК ОтчетКомиссионераОПродажахПокупатели
	...
	|		ПО ОтчетКомиссионераОПродажахПокупатели.КлючСтроки = ВТ_Суммы.КлючСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОтчетКомиссионераОПродажахПокупатели.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И ОтчетКомиссионераОПродажахПокупатели.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	|	ОтчетКомиссионераОПродажахПокупатели.СчетФактура = &ТекущийДокумент
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """"""""),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|	ВТ_ОтчетКомиссионераОПродажах.Ссылка";

КонецФункции

Функция ТекстЗапросаПодтверждениеНулевойСтавкиНДС()

	// Наличие товаров проверяем только для типа документа отгрузки "РеализацияТоваровУслуг".
	Возврат 
	"ВЫБРАТЬ
	...
	|	ЛОЖЬ КАК ЕстьПрослеживаемыеТовары,
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.КоличествоТоваров > 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЕстьТовары,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ПодтверждениеНулевойСтавки.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ПодтверждениеНулевойСтавкиНДС.Состав КАК ПодтверждениеНулевойСтавки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ПодтверждениеНулевойСтавки.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И ПодтверждениеНулевойСтавки.ДокументОтгрузки = ДанныеПервичныхДокументов.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СУММА(ЕСТЬNULL(РеализацияТоваровУслугТовары.Количество, 0)) КАК КоличествоТоваров,
	|			ПодтверждениеНулевойСтавкиНДССостав.Ссылка КАК Ссылка,
	...
	|				ПО ПодтверждениеНулевойСтавкиНДССостав.ДокументОтгрузки = РеализацияТоваровУслугТовары.Ссылка
	|		ГДЕ
	|			ПодтверждениеНулевойСтавкиНДССостав.Ссылка В(&ДокументыОснования)
	|		
	|		СГРУППИРОВАТЬ ПО
	|			ПодтверждениеНулевойСтавкиНДССостав.Ссылка,
	|			ПодтверждениеНулевойСтавкиНДССостав.ДокументОтгрузки) КАК ВложенныйЗапрос
	|		ПО ПодтверждениеНулевойСтавки.Ссылка = ВложенныйЗапрос.Ссылка
	|			И ПодтверждениеНулевойСтавки.ДокументОтгрузки = ВложенныйЗапрос.ДокументОтгрузки
	|ГДЕ
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """"),
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)),
	|	ПодтверждениеНулевойСтавки.Ссылка,
...
Функция ТекстЗапросаОказаниеУслуг()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОказаниеУслуг.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И ОказаниеУслуг.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаАвансовыйОтчет()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО АвансовыйОтчетПрочее.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И АвансовыйОтчетПрочее.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО АвансовыйОтчетТовары.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И АвансовыйОтчетТовары.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО АвансовыйОтчетБилеты.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И АвансовыйОтчетБилеты.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаНаОснованииМетаданных(ДокументыОснования, ЭтоПолученныйСФ, ПараметрыЗапроса, ТипДокументаОснования)
	...
	ИгнорироватьТЧ.Добавить("СведенияПрослеживаемости"); // Для исключения этой таблицы при регистрации СФ полученного к документу "Отчет комитенту" с видом операции "Отчет о продажах"
	...
	Если НЕ ЗначениеЗаполнено(ТекстЗапроса) Тогда // в документе не оказалось табличных частей
		...
		Если МетаданныеДокумента.Реквизиты.Найти("СуммаНДС") <> Неопределено Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
			|		ПО ДокОснование.Организация = ДанныеПервичныхДокументов.Организация
			|			И ДокОснование.Ссылка = ДанныеПервичныхДокументов.Документ
			|ГДЕ
		...
		Иначе
			...
			+ "1 Как ЕстьНДС,
			| Ложь Как ЕстьПрослеживаемыеТовары,
			|	"""" КАК НомерДокументаОтгрузки,
			|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаДокументаОтгрузки,
			|	ЛОЖЬ КАК ЕстьТовары,
...
Функция ТекстЗапросаВозвратОтПокупателяВыданныйСФ()
	Возврат 
	"ВЫБРАТЬ
	|	СчетФактураВыданныйДокументыОснования.ДокументОснование КАК ДокументОснование,
...
Функция ТекстЗапросаВозвратОтПокупателяПолученныйСФ()
	...
	|	Таблица.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВозвратТоваровОтПокупателя.Товары КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО Таблица.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И Таблица.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Функция ТекстЗапросаВозвратПоставщикуВыданныйСФ()
...
Функция ТекстЗапросаКорректировочныеВыданныеСчетаФактурыПоОтчетуКомиссионера()
	...
	|		ОтчетКомиссионераОВозвратахТовары.ПрослеживаемыйТовар КАК ЕстьПрослеживаемыеТовары
	|	ИЗ
	...
	|	ВТ_Суммы.ЕстьПрослеживаемыеТовары КАК ЕстьПрослеживаемыеТовары
	|ИЗ
	|	Документ.ОтчетКомиссионераОПродажах.Возвраты КАК ОтчетКомиссионераОВозвратахПокупатели
...
Функция ТекстЗапросаПоступлениеТоваровУслуг()
	...
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """") КАК НомерДокументаОтгрузки,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаОтгрузки,
	|	ТаблицыПоступлениеТоваровУслуг.ЕстьТовары КАК ЕстьТовары,
	|	ТаблицыПоступлениеТоваровУслуг.Ссылка КАК Ссылка
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|							КОГДА ТаблицаТовары.Ссылка.СуммаВключаетНДС
	|									ИЛИ &ЗаявлениеОВвозеТоваровОтКомиссионера
	|								ТОГДА 0
	|							ИНАЧЕ ТаблицаТовары.СуммаНДС
	|						КОНЕЦ
	|				ИНАЧЕ 0
	...
	|		"""",
	|		СУММА(ТаблицаПрава.Сумма + ВЫБОР
	|				КОГДА ПоступлениеТоваровУслуг.СуммаВключаетНДС
	|					ТОГДА 0
	|				ИНАЧЕ ТаблицаПрава.СуммаНДС
	|			КОНЕЦ),
	|		ПоступлениеТоваровУслуг.ВалютаДокумента,
	|		СУММА(ТаблицаПрава.СуммаНДС),
	|		0,
	|		0,
	|		СУММА(ВЫБОР
	|				КОГДА ТаблицаПрава.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
	|					ТОГДА 0
	|				ИНАЧЕ 1
	|			КОНЕЦ),
	|		НЕОПРЕДЕЛЕНО,
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Права КАК ТаблицаПрава
	|			ПО ПоступлениеТоваровУслуг.Ссылка = ТаблицаПрава.Ссылка
	|	ГДЕ
	|		ПоступлениеТоваровУслуг.Ссылка В(&ДокументыОснования)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ПоступлениеТоваровУслуг.Организация,
	|		ПоступлениеТоваровУслуг.Контрагент,
	|		ПоступлениеТоваровУслуг.ДоговорКонтрагента,
	|		ПоступлениеТоваровУслуг.ВалютаДокумента,
	|		ПоступлениеТоваровУслуг.Ссылка) КАК ТаблицыПоступлениеТоваровУслуг
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ТаблицыПоступлениеТоваровУслуг.Организация = ДанныеПервичныхДокументов.Организация
	|			И ТаблицыПоступлениеТоваровУслуг.Ссылка = ДанныеПервичныхДокументов.Документ";
...
Функция ТекстЗапросаРеализацияТоваровУслуг()
	...
	|		ГДЕ
	|			РеализацияТоваровУслугСведенияПрослеживаемости.Ссылка В(&ДокументыОснования)) КАК ВложенныйЗапрос
	|		ПО Таблица.Ссылка = ВложенныйЗапрос.Ссылка
	|ГДЕ
	|	Таблица.Ссылка В(&ДокументыОснования)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО Таблица.Ссылка.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка
	|			И Таблица.Ссылка.Организация = ДоговорыКонтрагентов.Организация
	|			И Таблица.Ссылка = ДоговорыКонтрагентов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентовКомиссия
	|		ПО Таблица.ДоговорКонтрагента = ДоговорыКонтрагентовКомиссия.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО Таблица.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
...
Функция ПредставлениеСчетФактураПолученный(РеквизитыСФ)
	...
	Если РеквизитыСФ.ИсправлениеСобственнойОшибки = Истина Тогда
	...
	ИначеЕсли РеквизитыСФ.Исправление = Истина Тогда
...
Функция ТекстЗапросаПередачаОС()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ПередачаОСОС.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И ПередачаОСОС.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
...
Процедура СинхронизироватьСтатусСчетаФактуры(СчетФактура, ДокОбъект, ОбновлятьСтатусСчетаФактурыПоДокументу, УстановитьГрупповоеПерепроведение = Ложь)
	...
	ТекстСообщенияВСлучаеОшибки = НСтр("ru='Реквизиты документа ""%1"" автоматически не перезаполнены и могут быть неактуальными'");
	...
	Попытка
		...
		Если УстановитьГрупповоеПерепроведение И ПроведениеСервер.ГрупповоеПерепроведение(ДокОбъект) Тогда
			СчетФактураОбъект.ДополнительныеСвойства.Вставить("ГрупповоеПерепроведение", Истина);
		КонецЕсли;
		СчетФактураОбъект.Записать();
	Исключение
		ТекстСообщения = СтрШаблон(ТекстСообщенияВСлучаеОшибки, СчетФактура);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, СчетФактура);
	КонецПопытки;
	
КонецПроцедуры

Функция СчетФактураНаходятсяВЗакрытомПериоде(СчетФактура, СчетФактураОбъект, ТекстСообщенияВСлучаеОшибки)
	
	Результат = ДатыЗапретаИзмененияСлужебный.ПроверитьДатыЗапретаИзмененияЗагрузкиДанных(
		СчетФактураОбъект, Ложь, Истина, Ложь);
		
	Если Результат.ИзменениеЗапрещено Тогда
		
		ТекстСообщенияВСлучаеОшибки = СтрШаблон(ТекстСообщенияВСлучаеОшибки, СчетФактура)+
			Символы.ПС + Результат.ОписаниеОшибки;
			
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияВСлучаеОшибки);
...
Процедура ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета) Экспорт
	...
	УчетНДСБП.ТекстЗапросаРеестрНДСПриложение(Запрос, ПараметрыОтчета);
	
КонецПроцедуры

Функция ТекстЗапросаРаздел4(ПараметрыОтчета) Экспорт
	
	Возврат УчетНДСБП.ТекстЗапросаРаздел4(ПараметрыОтчета);
