...
Процедура СоздатьМенеджерКриптографииПослеПолученияИнформацииОМодуле(ИнформацияМодуля, Контекст) Экспорт
	...
	ТипУниверсальногоКриптопровайдера = УниверсальныйОбменСБанкамиКлиентСервер.КриптопровайдерMicrosoftBaseCryptographicProvider().ТипКриптопровайдера;
	Если НЕ Контекст.СвойстваПрограммы.ТипКриптопровайдера = ТипУниверсальногоКриптопровайдера Тогда
		Попытка
			// Принудительно устанавливаем алгоритм шифрования.
		...
		Исключение
			...
			ОписаниеОшибки = НСтр("ru = 'Не удалось установить алгоритм шифрования для криптопровайдера ""%1"":'");
			ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, Контекст.СвойстваПрограммы.ТипКриптопровайдера)
				+ Символы.ПС + КраткоеОписание;
			КодОсновногоЯзыка = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
			ВариантСобытия = НСтр("ru = 'Создание менеджера криптографии'", КодОсновногоЯзыка);
...
Процедура Подписать(ОповещениеОбратногоВызова, ПараметрыПодписи) Экспорт
	...
	ДополнительныеПараметры.Вставить("ПодписанныеДанные", Неопределено);
	ДополнительныеПараметры.Вставить("ПовторноеПодписание", Ложь);
...
Процедура ПодписатьПослеСозданияМенеджераКриптографии(Результат, ДополнительныеПараметры) Экспорт
	...
	Перем ТипИспользуемойПодписи;

	#Если Не МобильныйКлиент Тогда
		ТипИспользуемойПодписи = ТипПодписиКриптографии.CAdEST;
	#КонецЕсли
	...
	Оповещение = Новый ОписаниеОповещения("ПроверитьПослеПодписания",
		ЭтотОбъект,
		ДополнительныеПараметры,
...
Процедура ПроверитьПослеПодписания(Результат, ДополнительныеПараметры) Экспорт
	
	ДополнительныеПараметры.ПодписанныеДанные = Результат;
	Оповещение = Новый ОписаниеОповещения("ПроверитьПослеПодписанияЗавершение",
		ЭтотОбъект,
		ДополнительныеПараметры);
	
	ПроверитьПодпись(Оповещение, ДополнительныеПараметры.Данные, Результат);
	
КонецПроцедуры

Процедура ПроверитьПослеПодписанияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		ВыполнитьОбработкуОповещения(Оповещение, ДополнительныеПараметры.ПодписанныеДанные);
		
	ИначеЕсли Не ДополнительныеПараметры.ПовторноеПодписание Тогда
		...
		ДополнительныеПараметры.ПовторноеПодписание = Истина;
