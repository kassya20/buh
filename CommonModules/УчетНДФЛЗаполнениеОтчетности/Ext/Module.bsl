...
Процедура РасчетПоказателей_6НДФЛ_2024Кв2(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Запрос.УстановитьПараметр("АУСН", Перечисления.КатегорииДоходовНДФЛ.ДоходыАУСН(ОкончаниеПериода));
...
Функция ДанныеОДоходахНалогахВычетах(Сотрудники, НалоговыйПериод, Организация, РегистрацияВНалоговомОргане, ДатаАктуальности, КППОтправки, ВРазрезеРегистраторов, ДополнительныйОтбор) Экспорт
	...
	|	СведенияОДоходахНДФЛ.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН
	|ПОМЕСТИТЬ ВТДоходы
	|ИЗ
	|	РегистрНакопления.СведенияОДоходахНДФЛ КАК СведенияОДоходахНДФЛ
	...
	|		ПО Сотрудники.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
	|			И (НАЧАЛОПЕРИОДА(ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.МесяцНалоговогоПериода, МЕСЯЦ) = ДоходыПереданныеВБанкНаАУСН.МесяцНалоговогоПериода)
	|			И (ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.РегистрацияВНалоговомОргане = ДоходыПереданныеВБанкНаАУСН.РегистрацияВНалоговомОргане)
	|ГДЕ
	...
	|	ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛОбороты.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН
	|ПОМЕСТИТЬ ВТВычеты
	|ИЗ
	...
	|	КОНЕЦ,
	|	ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо ЕСТЬ НЕ NULL 
	|		И ИмущественныеВычетыНДФЛ.КодВычета.ГруппаВычета = ЗНАЧЕНИЕ(Перечисление.ГруппыВычетовПоНДФЛ.Стандартные)
	|ИЗ
	...
	|			И (&ДополнительныйОтбор)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыПереданныеВБанкНаАУСН КАК ДоходыПереданныеВБанкНаАУСН
	|		ПО СотрудникиОрганизаций.ФизическоеЛицо = ДоходыПереданныеВБанкНаАУСН.ФизическоеЛицо
	|			И (НАЧАЛОПЕРИОДА(ИмущественныеВычетыНДФЛ.Период, МЕСЯЦ) = ДоходыПереданныеВБанкНаАУСН.МесяцНалоговогоПериода)
	|			И (ИмущественныеВычетыНДФЛ.РегистрацияВНалоговомОргане = ДоходыПереданныеВБанкНаАУСН.РегистрацияВНалоговомОргане)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Иначе
		...
		|	Доходы.ДатаПолученияДохода КАК ДатаПолученияДохода,
		|	Доходы.ЗарегистрированоДляПередачиВБанкНаАУСН КАК ЗарегистрированоДляПередачиВБанкНаАУСН,
		...
		|	ГДЕ
		|		НЕ СтандартныеИмущественныеВычеты.ЗарегистрированоДляПередачиВБанкНаАУСН) КАК База
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПределыДляНалоговыхБаз КАК ПределыДляНалоговыхБаз
		|		ПО База.НалоговаяБаза = ПределыДляНалоговыхБаз.НалоговаяБаза
		|
		|СГРУППИРОВАТЬ ПО
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			|			И СведенияОДоходахНДФЛ.НалоговаяБаза = СотрудникиСПревышением.НалоговаяБаза
			|ГДЕ
			|	НЕ СведенияОДоходахНДФЛ.ЗарегистрированоДляПередачиВБанкНаАУСН
			|
			|СГРУППИРОВАТЬ ПО
			|	СведенияОДоходахНДФЛ.ФизическоеЛицо,
			|	СведенияОДоходахНДФЛ.ДатаПолученияДохода,
			...
			|ГДЕ
			|	НЕ СтандартныеИмущественныеВычеты.ЗарегистрированоДляПередачиВБанкНаАУСН
			|
			|СГРУППИРОВАТЬ ПО
			|	СтандартныеИмущественныеВычеты.ФизическоеЛицо,
			|	СтандартныеИмущественныеВычеты.НалоговаяБаза,
...
Функция ДополнительныеСтрокиПревышения(ТаблицаРасчетаНарастающимИтогом, ЗаполнятьСтавкуЧислом, ЗаполнятьКБК) Экспорт
	...
	Если ТаблицаРасчетаНарастающимИтогом.Колонки.Найти("ГоловнаяОрганизация") = Неопределено Тогда
		ТаблицаРасчетаНарастающимИтогом.Колонки.Добавить("ГоловнаяОрганизация", Новый ОписаниеТипов("СправочникСсылка.Организации"))	
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
		Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяГоловнаяОрганизация <> СтрокаТаблицы.ГоловнаяОрганизация Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Или ТекущийПериод <> СтрокаТаблицы.Период Тогда
