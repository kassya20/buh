...
Функция БухучетСтраховыхВзносов(Организация, Период, ТаблицаВзносов, БухучетНачислений) Экспорт
	...
	Если БухучетНачислений.Колонки.Найти("БюджетныйТарифНСиПЗ") = Неопределено Тогда
		БухучетНачислений.Колонки.Добавить("БюджетныйТарифНСиПЗ", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	...
	ОтражениеЗарплатыВУчете.СоздатьВТНачислениеУдержаниеВидОперации(Запрос.МенеджерВременныхТаблиц);
	...
	|	ТаблицаВзносов.БюджетныйТарифНСиПЗ КАК БюджетныйТарифНСиПЗ,
	...
	|	БухучетНачислений.БюджетныйТарифНСиПЗ КАК БюджетныйТарифНСиПЗ,
	...
	|	БухучетНачислений.Начисление,
	|	БухучетНачислений.ОблагаетсяЕНВД,
	|	БухучетНачислений.БюджетныйТарифНСиПЗ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И БухучетНачислений.БюджетныйТарифНСиПЗ = БухучетНачисленийСвод.БюджетныйТарифНСиПЗ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	|	ФизическоеЛицо,
	...
	|	ЕСТЬNULL(ЕСТЬNULL(БухучетНачислений.ВидОперации, НачислениеУдержаниеВидОперации.ВидОперации), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.НачисленоДоход)) КАК ВидОперацииНачисления,
	...
	|			И СтраховыеВзносы.БюджетныйТарифНСиПЗ = БухучетНачислений.БюджетныйТарифНСиПЗ
...
Процедура УпорядочитьДанныеДляОтраженияЗарплатыВБухучете(РезультатЗаполнения) Экспорт
	...
	Для каждого ЭлементКоллекции Из РезультатЗаполнения Цикл
		...
		ЕстьПолеРасчетПервойПоловиныМесяца = Ложь;
		Для каждого КолонкаТЗ Из ЭлементКоллекции.Значение.Колонки Цикл
			...
			КонецЕсли;
			Если ВРег(ИмяПоля) = ВРег("РасчетПервойПоловиныМесяца") Тогда
				ЕстьПолеРасчетПервойПоловиныМесяца = Истина;
			КонецЕсли;
		КонецЦикла;
		...
		Если ЕстьПолеРасчетПервойПоловиныМесяца Тогда
			ТекстУпорядочивания = ТекстУпорядочивания + ",РасчетПервойПоловиныМесяца";
		КонецЕсли;
...
Функция НоваяТаблицаБухучетНачисленнаяЗарплатаИВзносы() Экспорт
	...
	Таблица.Колонки.Добавить("РасчетПервойПоловиныМесяца",  Новый ОписаниеТипов("Булево"));
