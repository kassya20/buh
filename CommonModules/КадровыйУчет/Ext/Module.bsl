...
//				* ОтбиратьПоГоловнойОрганизации - Булево - если Истина, то отбор по головной организации, иначе по текущей организации.
//													По умолчанию - ЛОЖЬ
...
//				* СписокФизическихЛиц - Массив из СправочникСсылка.ФизическиеЛица - если параметр опустить,
...
//				* Отборы				- Массив из Структура - описывающих отборы сотрудников
...
//    * УченыеСтепени - Массив из Структура: см. РегистрыСведений.УченыеСтепениФизическихЛиц.НоваяУченаяСтепень
//    * УченыеЗвания - Массив из Структура: см. РегистрыСведений.УченыеЗванияФизическихЛиц.НовоеУченоеЗвание
...
Функция НовыеДанныеФизическогоЛица() Экспорт
	...
	ДанныеФизическогоЛица.Вставить("ДатаВременнойРегистрации", Дата(1, 1, 1));
	ДанныеФизическогоЛица.Вставить("ДатаФактическогоПроживания", Дата(1, 1, 1));
	...
	ДанныеФизическогоЛица.Вставить("УченыеЗвания", Новый Массив);
...
Функция НоваяКонтактнаяИнформация() Экспорт
	...
	КонтактнаяИнформация.Вставить("Дата", Перечисления.ТипыКонтактнойИнформации.ПустаяСсылка());
...
// Возвращает идентификатор группы проверок ведения кадрового учета.
// 
// Возвращаемое значение:
//  Строка
...
// Возвращает идентификатор группы проверок контроля оформления кадровых приказов.
// 
// Возвращаемое значение:
//  Строка
...
// Возвращает идентификатор группы проверок контроля заполнения личных данных.
// 
// Возвращаемое значение:
//  Строка
...
Функция ЗапросВТСотрудникиПоКритериямПоиска(КритерииПоискаСотрудников, ТолькоРазрешенные, Знач ОбластьПоиска = "Справочник.ФизическиеЛица", ИмяВТСотрудникиПоКритериямПоиска = "ВТСотрудникиПоКритериямПоиска") Экспорт
	...
	Для каждого КритерийПоиска Из КритерииПоискаСотрудников Цикл
		...
		Если ОбластьПоиска = ОбластьПоискаПоСотрудникам
			...
			ИначеЕсли ВРег(КритерийПоиска.Ключ) = ВРег("РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника") Тогда
				ИмяПоляФизическоеЛицоТаблицы = "Ссылка";
			Иначе
...
Функция КритерийПоискаФизическогоЛица(Знач ПолноеИмяОбъектаМетаданных)
	...
	ИначеЕсли ПолноеИмяОбъектаМетаданныхВерхнемРегистре = ВРег("РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника") Тогда
		ПоискФизическогоЛица = Истина;
	Иначе
	...
	Иначе
		...
		Иначе
			...
			Если КритерийПоискаФизическогоЛица(ПолноеИмяОбъектаМетаданныхВерхнемРегистре) Тогда
				...
				ИначеЕсли ПолноеИмяОбъектаМетаданныхВерхнемРегистре = ВРег("РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника") Тогда
					Запрос.Текст = СтрЗаменить(Запрос.Текст, "Т.Сотрудник КАК Сотрудник", "Т.Ссылка КАК Ссылка");
				Иначе
...
Функция ТаблицаНачисленийСотрудниковПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод = "Период", ИмяПоляСотрудник = "Сотрудник", ИмяПоляДатаОкончания = "", ИспользоватьКраткиеПредставления = Истина, ВыводитьДенежныеПоказателиСВалютой = Ложь, ФормироватьСПериодичностьДень = Истина, ИмяПоляНачисление = "") Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплаты")
		...
		ОписанияНачислений = ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник, ИмяПоляДатаОкончания, ИспользоватьКраткиеПредставления, ВыводитьДенежныеПоказателиСВалютой, ФормироватьСПериодичностьДень, ИмяПоляНачисление);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ОтключитьБизнесЛогику Тогда
				...
				Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
					...
					РегистрыСведений.ТекущиеКадровыеДанныеСотрудников.УстановитьОсновноеРабочееМестоВОрганизации(Набор, Замещение);
...
Функция НеобходимыПостоянныеКадровыеДанныеФизическихЛиц(Знач ИмяПоля) Экспорт
	...
	КадровыйУчетПереопределяемый.ПриОпределенииПостоянныхКадровыхДанныхФизическихЛиц(
		ИмяПоля, СтандартнаяОбработка);
...
Функция НеобходимыПостоянныеКадровыеДанныеСотрудников(Знач ИмяПоля) Экспорт
	...
	СтандартнаяОбработка = Истина;
	КадровыйУчетПереопределяемый.ПриОпределенииПостоянныхКадровыхДанныхСотрудников(
		ИмяПоля, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод = "Период", ИмяПоляСотрудник = "Сотрудник", ИмяПоляДатаОкончания = "", ИспользоватьКраткиеПредставления = Истина, ВыводитьДенежныеПоказателиСВалютой = Ложь, ФормироватьСПериодичностьДень = Истина, ИмяПоляНачисление = "") Экспорт
	
	Возврат КадровыйУчетВнутренний.ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник, ИмяПоляДатаОкончания, ИспользоватьКраткиеПредставления, ВыводитьДенежныеПоказателиСВалютой, ФормироватьСПериодичностьДень, ИмяПоляНачисление);
...
Функция НеобходимыКадровыеДанныеФизическогоЛица(Знач ИмяПоля) Экспорт
	...
	СтандартнаяОбработка = Истина;
	КадровыйУчетПереопределяемый.ПриОпределенииПериодическихКадровыхДанныхФизическихЛиц(
		ИмяПоля, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура ЗапомнитьСотрудниковДляОбновленияКадровыхДанных(Источник, Замещение) Экспорт
	
	СотрудникиДляОбновления = СотрудникиДляОбновления(Источник, Замещение);
...
Процедура ЗапомнитьСотрудниковДляОбновленияДанныхОбОплатеТруда(Источник, Замещение) Экспорт
...
Процедура ОбновитьВторичныеКадровыеДанныеСотрудников(НаборЗаписей, Замещение) Экспорт
	...
	КадровыйУчетВнутренний.ОбновитьВторичныеКадровыеДанныеСотрудников(НаборЗаписей, Замещение);
	
КонецПроцедуры

Функция СотрудникиДляОбновления(НаборЗаписей, Замещение)
	
	СписокСотрудников = Новый Массив;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		Изменения = РегистрыБЗК.ИзменениеЗаписейНабора(РегистрыБЗК.ЗаписиНабораИзБазыДанных(НаборЗаписей, Замещение), НаборЗаписей, Замещение, Истина);
		СтруктурныеЕдиницыКОбновлениюЗависимыхВидовСтажа = Новый Соответствие;
		Для Каждого СтрокаТаблицы Из Изменения Цикл
			Если СтрокаТаблицы.ВидИзмененияСтроки <> 0 Тогда
...
Процедура КадроваяИсторияСотрудниковПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	КадровыйУчетВнутренний.КадроваяИсторияСотрудниковПередЗаписью(НаборЗаписей, Отказ, Замещение);
	
КонецПроцедуры

Процедура КадроваяИсторияСотрудниковПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчетВнутренний.КадроваяИсторияСотрудниковПриЗаписи(НаборЗаписей, Отказ, Замещение);
	
КонецПроцедуры

Процедура ПлановыеНачисленияПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчетВнутренний.ПлановыеНачисленияПередЗаписью(НаборЗаписей, Отказ, Замещение);
	
КонецПроцедуры

Процедура ПлановыеНачисленияПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчетВнутренний.ПлановыеНачисленияПриЗаписи(НаборЗаписей, Отказ, Замещение);
	...
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			Если СтрокиНачисленийСотрудника.Количество() > 0 Тогда
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеДетальныхЗаписей, "РаботаСведенияОбОплатеТрудаТарифнаяСтавкаНаПечать")
					И Не ЗначениеЗаполнено(ДанныеДетальныхЗаписей.РаботаСведенияОбОплатеТрудаТарифнаяСтавкаНаПечать) Тогда
				...
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеДетальныхЗаписей, "РаботаСведенияОбОплатеТрудаНадбавкаНаПечать") Тогда
					ДанныеНачислений.Вставить("РаботаСведенияОбОплатеТрудаНадбавкаНаПечать", СтрокиНачисленийСотрудника[0].Надбавка);
				КонецЕсли;
			...
			ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т1", НомерСтрокиНачалаОбласти,
...
Процедура ВывестиНаПечатьТ5(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т5", НомерСтрокиНачалаОбласти,
				ДанныеДетальныхЗаписей, ДанныеНаПечать.СсылкаНаОбъект);
...
Процедура ВывестиНаПечатьТ8(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Для Каждого ДанныеНаПечать Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеДетальныхЗаписей Из ДанныеНаПечать.Строки Цикл
			...
			ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т8", НомерСтрокиНачалаОбласти,
				ДанныеДетальныхЗаписей, ДанныеНаПечать.СсылкаНаОбъект);
...
Процедура ВывестиНаПечатьТ6(ПараметрыПечати, ДокументРезультат, ДанныеСсылок, МакетПечатнойФормы, ИдентификаторыМакета = Неопределено, ОбъектыПечати = Неопределено) Экспорт
	...
	Для Каждого ДанныеСотрудников Из ДанныеСсылок Цикл
		...
		Для Каждого ДанныеНаПечать Из ДанныеСотрудников.Строки Цикл
			...
			ЭДОБЗК.ЗадатьДетальнуюОбластьПечати(ПараметрыПечати, ДокументРезультат, "ПФ_MXL_Т6", НомерСтрокиНачалаОбласти,
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ОснованияУвольнения) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.35.40";
		Обработчик.Процедура = "Справочники.ОснованияУвольнения.ЗагрузитьКлассификатор";
		Обработчик.ОбщиеДанные = Истина;
	КонецЕсли;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.62";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("729489e3-9220-4093-8019-cb44aa9043a7");
	Обработчик.Процедура = "РегистрыСведений.СведенияОбИнвалидностиФизическихЛиц.ЗаполнитьРеквизитДатаУстановления";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Дата установления"".'");
	
	ЭлектронныеТрудовыеКнижки.ЗарегистрироватьОбработчикиОбновления(Обработчики);
	ЭДОБЗК.ЗарегистрироватьОбработчикиОбновления(Обработчики);
...
Процедура ПриЗаписиНастроекКадровогоУчета(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчетВнутренний.ПриЗаписиНастроекКадровогоУчета(НаборЗаписей, Отказ, Замещение)
...
Функция ОснованиеДляРеестраПоДатеИНомеруПриказа(ДатаПриказа, НомерПриказа) Экспорт 
	...
	Возврат СтрШаблон(НСтр("ru='Пр. № %1 от %2'"), НомерПриказа, Формат(ДатаПриказа, "ДЛФ=D"));
	
КонецФункции

Функция ДоступноОбъединениеДублейПерсональныхДанных() Экспорт
	
	Возврат КадровыйУчетВнутренний.ДоступноОбъединениеДублейПерсональныхДанных();
...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	КадровыйУчетВнутренний.ПриОпределенииДоступныхДляПереводаМакетов(Макеты);
	
КонецПроцедуры

#КонецОбласти

// Возвращает количество календарных дней исполнения обязанностей единоличного руководителя.
//
// Параметры:
//  Сотрудник - СправочникСсылка.Сотрудники
//  Месяц     - Дата, в месяце для которого получаются сведения
//
// Возвращаемое значение:
//  Число
//
Функция КалендарныеДниНаДолжностиЕдиноличногоРуководителя(Сотрудник, Месяц) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		СтрокаОтбора.ДатаНачала = НачалоМесяца(Месяц);
		СтрокаОтбора.ДатаОкончания = КонецМесяца(Месяц);
		
		ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(ТаблицаОтбора);
		ПараметрыПостроения = ЗарплатаКадрыОбщиеНаборыДанных.ПараметрыПостроенияДляСоздатьВТИмяРегистра();
		ПараметрыПостроения.ВключатьЗаписиНаНачалоПериода = Истина;
		...
		Запрос.Текст =
			...
			|	КадроваяИсторияСотрудников.Должность.ЯвляетсяДолжностьюЕдиноличногоРуководителяКоммерческойОрганизации КАК ЯвляетсяДолжностьюЕдиноличногоРуководителя,
			|	ЕСТЬNULL(ДОБАВИТЬКДАТЕ(МИНИМУМ(КадроваяИсторияСотрудниковСледующие.Период), ДЕНЬ, -1), &КонецМесяца) КАК ДатаОкончания
			|ИЗ
			|	ВТКадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадроваяИсторияСотрудников КАК КадроваяИсторияСотрудниковСледующие
			|		ПО КадроваяИсторияСотрудников.Период < КадроваяИсторияСотрудниковСледующие.Период
			|ГДЕ
			|	КадроваяИсторияСотрудников.ВидСобытия <> ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
			|
			|СГРУППИРОВАТЬ ПО
			|	КадроваяИсторияСотрудников.Период,
			|	КадроваяИсторияСотрудников.Должность.ЯвляетсяДолжностьюЕдиноличногоРуководителяКоммерческойОрганизации";
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Если Выборка.ЯвляетсяДолжностьюЕдиноличногоРуководителя Тогда
				КалендарныеДни = КалендарныеДни + ЗарплатаКадрыКлиентСервер.ДнейВПериоде(Выборка.ДатаНачала, Выборка.ДатаОкончания);
			КонецЕсли;
		КонецЦикла;
		
	Иначе
		КадровыеДанные = КадровыеДанныеСотрудника(Истина, Сотрудник, "ДатаПриема,ДатаЗавершенияРаботы,ЕдиноличныйРуководительКоммерческойОрганизации", Месяц);
		Если КадровыеДанные <> Неопределено
			И КадровыеДанные.ЕдиноличныйРуководительКоммерческойОрганизации
			И (Не ЗначениеЗаполнено(КадровыеДанные.ДатаЗавершенияРаботы)
				Или КадровыеДанные.ДатаЗавершенияРаботы > НачалоМесяца(Месяц)) Тогда
			
			ДатаОкончания = КонецМесяца(Месяц);
			Если ЗначениеЗаполнено(КадровыеДанные.ДатаЗавершенияРаботы) Тогда
				ДатаОкончания = Мин(КадровыеДанные.ДатаЗавершенияРаботы, ДатаОкончания);
			КонецЕсли;
			
			ДатаНачала = НачалоМесяца(Месяц);
			Если КадровыеДанные.ДатаПриема > НачалоМесяца(Месяц) Тогда
				ДатаНачала = КадровыеДанные.ДатаПриема;
			КонецЕсли;
			
			КалендарныеДни = ЗарплатаКадрыКлиентСервер.ДнейВПериоде(ДатаНачала, ДатаОкончания);
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыСправокОСреднемЗаработкеДляПособияПоБезработице()
	...
	ПараметрыСправок.Вставить("ДатаСправки", ОбщегоНазначения.ТекущаяДатаПользователя());
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	КадровыйУчетПереопределяемый.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля)
	...
	КадровыйУчетПереопределяемый.ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
