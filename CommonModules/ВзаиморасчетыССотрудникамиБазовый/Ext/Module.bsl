...
Процедура СоздатьВТЗарплатаКВыплате(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники) Экспорт
	...
	ИначеЕсли Параметры.СпособПолучения  = Перечисления.СпособыПолученияЗарплатыКВыплате.ОкончательныйРасчет Тогда
		Если Параметры.Свойство("ДокументыОснования") И Параметры.ДокументыОснования.Количество() <> 0 Тогда
...
Процедура СоздатьВТПлановыйАванс(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники, КадровыеДанные) Экспорт
	...
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(
		ОписательВТ, ТолькоРазрешенные, "Подразделение, ФОТ, СпособРасчетаАванса, Аванс, Организация" + ?(ПустаяСтрока(КадровыеДанные), "", "," + КадровыеДанные));
...
Функция ВидВзаиморасчетовССотрудникамиПоХарактеруВыплатыЗарплаты(ПорядокВыплаты) Экспорт
	...
	ИначеЕсли ПорядокВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Зарплата
		Или ПорядокВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Межрасчет Тогда
...
Процедура СоздатьВТЗарплатаКВыплатеАванс(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники)
	...
	|		ПлановыйАванс.Организация КАК Организация,
...
Процедура СоздатьВТЗарплатаКВыплатеМежрасчет(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники)
	...
	Если ТипЗнч(ДокументыОснования) = Тип("ДокументСсылка.НачислениеЗарплаты") 
		ИЛИ (ТипЗнч(ДокументыОснования) = Тип("Массив")
				И ДокументыОснования.Количество() > 0
				И ТипЗнч(ДокументыОснования[0]) = Тип("ДокументСсылка.НачислениеЗарплаты")) Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТИПЗНАЧЕНИЯ(ЗарплатаКВыплате.ДокументОснование) В (&ДопустимыеТипы)", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТИПЗНАЧЕНИЯ(ДокументОснование) В (&ДопустимыеТипы)", "");
	ИначеЕсли ЗначениеЗаполнено(ДокументыОснования) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТИПЗНАЧЕНИЯ(ЗарплатаКВыплате.ДокументОснование) В (&ДопустимыеТипы)", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТИПЗНАЧЕНИЯ(ДокументОснование) В (&ДопустимыеТипы)", "");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ДокументОснование В (&ДокументыОснования)", "");
