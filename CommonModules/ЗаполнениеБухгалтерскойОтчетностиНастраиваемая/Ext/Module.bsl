...
//                        Описание структуры отчета (раздела отчета) и правил его заполнения
...
Процедура ЗаполнитьОтчет(РезультатЗаполнения, ЗаполняемыеПоказатели, Раздел, Контекст) Экспорт
	...
	Процессор.ПредставлениеПериода = БухгалтерскийУчет.ПредставлениеПериодаБухгалтерскойОтчетности(
		Контекст.Период.КонецДата,
		Процессор.КодЯзыка);
	...
	Для Каждого ГруппаПоказателей Из Процессор.ЗаполняемыеПоказатели.ГруппыПоказателей Цикл
		...
		Для ИндексСоставаГруппы = 0 По РасчетГруппы.Состав.ВГраница() Цикл
			ПорядковыйНомерПоказателя = ?(РасчетГруппы.СодержитЕдинственныйПоказатель, 0, ИндексСоставаГруппы + 1);
			ДобавитьОписаниеПоказателяОтчета(РасчетГруппы, ИндексСоставаГруппы, ПорядковыйНомерПоказателя);
		КонецЦикла;
...
// Параметры:
//  ЗаполняемыеПоказатели - см. БухгалтерскаяОтчетностьБРО.НовыйЗаполняемыеПоказателиРаздела
//                          Заполняемая коллекция с описанием структуры отчета (раздела отчета) и правил его заполнения
...
Процедура ОпределитьЗаполняемыеПоказатели(ЗаполняемыеПоказатели, Раздел, Контекст) Экспорт
	...
	ПроцессорЗаполнения.КонтекстОтчета = Контекст;
...
//    * Ключ - Строка - имя раздела из БухгалтерскаяОтчетностьБРО.РазделыОтчета
//    * Значение - Массив из Строка - коды строк из приложения 10 к ФСБУ 4
//
Функция КодыЗаполняемыхСтрок(Контекст) Экспорт
...
// Конструктор коллекции с перечнем классифицируемых объектов.
// См. также КлассифицируемыеОбъектыДляВывода
// 
// Возвращаемое значение:
//  Структура:
// * ПрименяетсяУточнениеОбъектов - Булево - Истина, если уточняющие объекты фактически применяются (например, введены группы справочника) 
// * ОбъектыПоказателей - Соответствие из КлючИЗначение:
//   ** Ключ - Число - идентификатор показателя
//   ** Значение - СписокЗначений Из ЛюбаяСсылка - объекты и их представления;
//       Тип значений см. в ЗаполнениеБухгалтерскойОтчетностиПравила.ТипЗначенияКлассифицируемыхОбъектов
//       Пометка соответствует активным объектам; Неактивные объекты выводятся только при наличии значения
//       Картинка применяется для идентификации уточняющих объектов - см. КартинкаИдентификаторУточняющийОбъект
Функция НовыйКлассифицируемыеОбъекты() Экспорт
	
	КлассифицируемыеОбъекты = Новый Структура;
	
	КлассифицируемыеОбъекты.Вставить("ПрименяетсяУточнениеОбъектов", Ложь);
	КлассифицируемыеОбъекты.Вставить("ОбъектыПоказателей", Новый Соответствие);
...
// 
// Настройка отчета также выполняется в терминах классифицируемых объектов, но в частном случае это могут быть не все объекты,
// выводимые в отчет - см. КлассифицируемыеОбъекты
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Структура - см. НовыйКлассифицируемыеОбъекты 
Функция КлассифицируемыеОбъектыДляВывода(МенеджерВременныхТаблиц, Алгоритм, ОсновнойПоказатель) Экспорт
...
// Описывает порядок настройки отчета в части перечня классифицируемых объектов.
// 
// Параметры:
//  ЗаполняемыеПоказатели - см. БухгалтерскаяОтчетностьБРО.НовыйЗаполняемыеПоказатели
// 
// Возвращаемое значение:
//  Соответствие из КлючИЗначение:
//    * Ключ - Строка, ЛюбаяСсылка - идентификатор алгоритма заполнения
//    * Значение - см. НовыйКлассифицируемыеОбъекты
Функция ПорядокНастройкиКлассифицируемыхОбъектов(ЗаполняемыеПоказатели) Экспорт

	ПорядокНастройки = Новый Соответствие; 
	...
	Для Каждого ПравилаПолученияДанныхАлгоритма Из ПравилаПолученияДанных Цикл
		...
		Если Не ЗаполнениеБухгалтерскойОтчетностиПравила.РазрешенаНастройкаПользователем(ПравилаПолученияДанныхАлгоритма.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		...
		КлассифицируемыеОбъекты = КлассифицируемыеОбъекты(
			МенеджерВременныхТаблиц,
			ПравилаПолученияДанныхАлгоритма.Ключ,
			Истина,
			ОсновнойПоказатель);
			
		ПорядокНастройки.Вставить(ПравилаПолученияДанныхАлгоритма.Ключ, КлассифицируемыеОбъекты);
		
	КонецЦикла;
	
	Возврат ПорядокНастройки;
...
//  ЗаполняемыеПоказатели - см. БухгалтерскаяОтчетностьБРО.НовыйЗаполняемыеПоказателиРаздела
//   	В этой коллекции можно снять флаг Вывод, другие модификации недопустимы.
...
// 
// Возвращаемое значение:
//  СтрокаТаблицыЗначений из см. БухгалтерскаяОтчетностьБРО.НовыйЗаполняемыеПоказателиРаздела.Показатели - добавленный показатель
//
...
// 
// Возвращаемое значение:
//  см. БухгалтерскаяОтчетностьБРО.НовыйАлгоритмЗаполнения
//
...
// Определяет картинку, используемую для идентификации уточняющих классифицируемых объектов при передаче информации об объектах в списке значений.
// 
// Возвращаемое значение:
//  Картинка
Функция КартинкаИдентификаторУточняющийОбъект() Экспорт
	Возврат БиблиотекаКартинок.УвеличитьМасштаб;
КонецФункции
...
Процедура СоздатьКлассифицируемыеОбъекты(МенеджерВременныхТаблиц, Алгоритм, ПравилаПолученияДанных)
	
	ТипКлассифицируемыхОбъектов = ЗаполнениеБухгалтерскойОтчетностиПравила.ТипЗначенияКлассифицируемыхОбъектов(Алгоритм);
	
	Если ТипКлассифицируемыхОбъектов = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	ЗаполнениеБухгалтерскойОтчетностиПравила.СоздатьКлассифицируемыеОбъекты(
		МенеджерВременныхТаблиц,
		Алгоритм,
		ПравилаПолученияДанных);
	
КонецПроцедуры

Функция КлассифицируемыеОбъекты(МенеджерВременныхТаблиц, Алгоритм, ТолькоНастраиваемые, ОсновнойПоказатель)
	
	КлассифицируемыеОбъекты = НовыйКлассифицируемыеОбъекты();
	
	Если Не ЗначениеЗаполнено(МенеджерВременныхТаблиц.Таблицы) Тогда
		Возврат КлассифицируемыеОбъекты;
	КонецЕсли;
	
	ЗаполнениеБухгалтерскойОтчетностиПравила.УстановитьПрименяетсяУточнениеОбъектов(
		КлассифицируемыеОбъекты.ПрименяетсяУточнениеОбъектов,
		Алгоритм,
		МенеджерВременныхТаблиц);
	...
	ИдентификаторыУточненияОбъектов = Новый Соответствие;
	ИдентификаторыУточненияОбъектов.Вставить(Ложь, Новый Картинка);
	ИдентификаторыУточненияОбъектов.Вставить(Истина, КартинкаИдентификаторУточняющийОбъект());
	ИдентификаторУточняющегоОбъекта = ИдентификаторыУточненияОбъектов[Ложь];
	...
	Пока ВыборкаПоказателей.Следующий() Цикл
		...
		КлассифицируемыеОбъекты.ОбъектыПоказателей.Вставить(ИдентификаторПоказателя, КлассифицируемыеОбъектыПоказателя);
		
		ВыборкаОбъектов = ВыборкаПоказателей.Выбрать();
		ОстатокЛимитаОбъектов = ЛимитКлассифицируемыхОбъектов();
		Пока ВыборкаОбъектов.Следующий() Цикл
			
			МожетБытьРазрешеноИсчерпаниеЛимита = Ложь;
			РазрешеноИсчерпаниеЛимита = Ложь;
			ОстатокЛимитаОбъектов = ОстатокЛимитаОбъектов - 1;
			
			Если КлассифицируемыеОбъекты.ПрименяетсяУточнениеОбъектов Тогда
				
				МожетБытьРазрешеноИсчерпаниеЛимита = Истина;
				РазрешеноИсчерпаниеЛимита = ВыборкаОбъектов.Уточняемый;
				ИдентификаторУточняющегоОбъекта = ИдентификаторыУточненияОбъектов[Не ВыборкаОбъектов.Уточняемый];
				
			КонецЕсли;
			
			Если ОстатокЛимитаОбъектов < 0 И Не РазрешеноИсчерпаниеЛимита Тогда
				Если МожетБытьРазрешеноИсчерпаниеЛимита Тогда
					Продолжить;
				Иначе
					Прервать;
				КонецЕсли;
			...
			КлассифицируемыеОбъектыПоказателя.Добавить(
				ВыборкаОбъектов.Объект,
				ВыборкаОбъектов.Представление,
				ВыборкаОбъектов.Активный,
				ИдентификаторУточняющегоОбъекта);
...
// Определяет технологическое ограничение количества объектов классификации, применяемое для настройки.
// Избыточное количество объектов может привести к неработоспособности форм настройки,
// при этом не имеет практического смысла.
// Если достигнуто ограничение, значит справочник объектов классификации организован неверно:
// следует реорганизовать его так, чтобы объекты были лучше сгруппированы в уточняемые.
// 
// Возвращаемое значение:
//  Число - предельное количество уточняемых объектов в составе показателя.
Функция ЛимитКлассифицируемыхОбъектов()
...
Процедура ВыполнитьШагиРасчета(ЗаписьРодитель, ВыборкаРодитель, ОписаниеПорядкаЗаполнения, ШагиРасчета, Процессор, ВыполненныеШагиРасчета, КешПредставлений = Неопределено)
	...
	ИмяШага = ШагРасчета.Представление;
	ПеременныйНаборШаговРасчета = Ложь;
	Если СтрЗаканчиваетсяНа(ИмяШага, "?") Тогда
		ПеременныйНаборШаговРасчета = Истина;
	...
	ОстатокЛимитаЗаписейРасчета = ЛимитКлассифицируемыхОбъектов();
	СводнаяЗаписьРасчет = Неопределено; // Применяется при достижении лимита показателей
	КоличествоПропущенныхЗаписей = 2; // Сводная запись включает как минимум две
	
	// Поиск в дереве не оптимизирован, поэтому используем "самодельный" индекс
	ИндексПредметыЗаписи = Новый Соответствие;
	Для Каждого ИндексируемоеЗначение Из ЗаписьРодитель.Строки Цикл
		ИндексПредметыЗаписи.Вставить(ИндексируемоеЗначение.ПредметЗаписи, ИндексируемоеЗначение);
	КонецЦикла;
	
	Пока Выборка.Следующий() Цикл
		
		ДостигнутШагРасчета = (ИмяШага = Выборка.Группировка());
		Если Не ДостигнутШагРасчета Тогда
		...
		ПропуститьШагРасчета = ПеременныйНаборШаговРасчета И Не ЗначениеЗаполнено(ПредметЗаписи);
		Если ПропуститьШагРасчета Тогда
			// Пропускаем шаг расчета
		...
		Если ЗначениеЗаполнено(ФиксированныеЗначения) Тогда
			ЗаписьРасчет = ИндексПредметыЗаписи[ПредметЗаписи];
			...
			КонецЕсли;
		КонецЕсли;
		Если ЗаписьРасчет = Неопределено И Не СводнаяЗаписьРасчет = Неопределено Тогда
			ЗаписьРасчет = СводнаяЗаписьРасчет;
			ЗаписьРасчет.Описание = СтрШаблон(НСтр("ru = '(Остальные %1)'", Процессор.КодЯзыка), КоличествоПропущенныхЗаписей);
			ЗаписьРасчет.Отчет = Неопределено;
		...
		Если ЗаписьРасчет = Неопределено Тогда
			...
			ОстатокЛимитаЗаписейРасчета = ОстатокЛимитаЗаписейРасчета - 1;
			Если ОстатокЛимитаЗаписейРасчета < 0 Тогда
				СводнаяЗаписьРасчет = ЗаписьРасчет;
			КонецЕсли;
		...
		ЗаписьРасчет.Значение = ЗаписьРасчет.Значение + Выборка.Значение;
	...
	Для Каждого ОписаниеСубконтоОтчета Из ПостроительОписанияОтчетов.Субконто Цикл
		...
		ИначеЕсли ПостроительОписанияОтчетов.ОтборыПоказателя.Свойство(ИмяПоля) Тогда
			...
			Если ТипЗначенияОтбора = Тип("Массив") Тогда
				Если ЗначениеОтбора.Количество() < ЛимитКлассифицируемыхОбъектов() Тогда
					ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
						Отчет,
						ОписаниеСубконтоСчета.Имя,
						ЗначениеОтбора,
			...
			ИначеЕсли ТипЗначенияОтбора = Тип("Структура") Или ТипЗначенияОтбора = Тип("Соответствие") Тогда
				Для Каждого ОтборЧерезТочку Из ЗначениеОтбора Цикл
					ТипЗначенияОтбора = ТипЗнч(ОтборЧерезТочку.Значение);
					Если (ТипЗначенияОтбора = Тип("Массив") Или ТипЗначенияОтбора = Тип("СписокЗначений"))
						И ОтборЧерезТочку.Значение.Количество() >= ЛимитКлассифицируемыхОбъектов() Тогда
						Продолжить;
					КонецЕсли;
					ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
						Отчет,
...
Функция НовыйРасчетГруппыОтчета()
	...
	РасчетГруппы.Вставить("Состав", Новый Массив); // Запись из Показатели
	РасчетГруппы.Вставить("РасчетПоказателей", Новый Массив); // Запись из ЗначенияПоказателей; синхронизирован по индексу с Состав
...
Функция НачатьРасчетГруппыОтчета(ГруппаПоказателей, ПроцессорЗаполненияОтчета)
	...
	ПолныйСоставГруппы = БухгалтерскаяОтчетностьБРО.СоставГруппыПоказателей(ГруппаПоказателей, ПроцессорЗаполненияОтчета.ЗаполняемыеПоказатели);
	Для Каждого Показатель Из ПолныйСоставГруппы Цикл
		...
		РасчетГруппы.Состав.Добавить(Показатель);
		РасчетГруппы.РасчетПоказателей.Добавить(ПротоколЗначенияПоказателя);
		
	КонецЦикла;
	
	Много = 2;
	Если РасчетГруппы.Состав.Количество() < Много Тогда
		РасчетГруппы.СодержитЕдинственныйПоказатель = Истина;
	КонецЕсли;
...
Процедура ЗакончитьРасчетГруппыОтчета(РасчетГруппы)
	...
	Если Не РасчетГруппы.СодержитЕдинственныйПоказатель Тогда
		...
		ДобавитьПоясняющийЗаголовокРасшифровкиПоказателей(РасчетГруппы);
	КонецЕсли;
...
Процедура ДобавитьПоясняющийЗаголовокРасшифровкиПоказателей(РасчетГруппы)
	
	ЗаписьРасшифровки = ДобавитьЗаписьРасшифровкиГруппы(РасчетГруппы);
	
	ЗаписьРасшифровки.ТипЗаписи = "РазделительТаблиц";
	ЗаписьРасшифровки.Описание  = НСтр("ru = 'Ниже приведен подробный расчет каждого слагаемого'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ДобавитьОписаниеПоказателяОтчета(РасчетГруппы, ИндексСоставаГруппы, ПорядковыйНомерПоказателя)
	
	ПроцессорЗаполненияОтчета = РасчетГруппы.ПроцессорЗаполнения;
	Показатель = РасчетГруппы.Состав[ИндексСоставаГруппы];
	ПротоколЗначенияПоказателя = РасчетГруппы.РасчетПоказателей[ИндексСоставаГруппы];
	ИдентификаторПоказателя = БухгалтерскаяОтчетностьБРО.ИдентификаторПоказателя(Показатель);
	...
	ТекстЗаголовкаПоказателя = Показатель.Наименование;
	Если ПорядковыйНомерПоказателя > 0 Тогда
		// Дополняем наименование порядковым номером, чтобы показать связь строк в начальной таблице описания показателя отчета,
		// и пояснений заполнения
		ТекстЗаголовкаПоказателя = СтрШаблон("%1. %2", ПорядковыйНомерПоказателя, ТекстЗаголовкаПоказателя);
	КонецЕсли;
	
	НачатьОписаниеПоказателяОтчета(РасчетГруппы, Показатель, Значение, ТекстЗаголовкаПоказателя);
...
Процедура НачатьОписаниеПоказателяОтчета(РасчетГруппы, Показатель, Значение, ТекстЗаголовкаПоказателя)
	...
	Иначе
		...
		ЗаписьРасшифровки.Описание = ТекстЗаголовкаПоказателя;
...
Процедура ДобавитьОписаниеСлагаемыхПоказателяОтчета(РасчетГруппы, Слагаемые, Уровень)
	...
	Для Каждого Слагаемое Из Слагаемые Цикл
		...
		ВыводитьПодчиненныеСлагаемые = КоличествоПодчиненныхСлагаемых > 1
			Или (КоличествоПодчиненныхСлагаемых = 1 И (ПредметЗаписиЗаполнен(Слагаемое.Строки[0].ПредметЗаписи)));
...
Функция ПредметЗаписиЗаполнен(ПредметЗаписи)
	Возврат ТипЗнч(ПредметЗаписи) = Тип("Число") Или ЗначениеЗаполнено(ПредметЗаписи);
КонецФункции
...
Функция НовыйОписаниеОбобщеннойГруппыПоказателейУпрощенногоОтчета()
	...
	ОбобщеннаяГруппаПоказателей.Вставить("Итоговая", Ложь); // см. ОпределитьИтоговуюГруппуПоказателей 
...
Функция ПланСокращенияГруппПоказателейУпрощенногоОтчета(ЗаполняемыеПоказатели, Раздел)
	...
	Для Каждого ОбобщеннаяГруппаПоказателей Из СхемаСокращения Цикл
		...
		ОпределитьИтоговуюГруппуПоказателей(
			ОписаниеОбобщеннойГруппыПоказателей,
			ОсновнойКодСтроки,
...
#Область ИтоговыеГруппыПоказателей

// В некоторых отчетах образцами и составом кодов в Приложении 10 к ФСБУ 4 предполагается,
// что коды строк есть не только у групп показателей, но и у показателей в составе группы.
// В таком случае группа показателей содержит арифметический итог вложенных показателей, далее ее называем "итоговой".
// Итоговые группы можно отличить по "вложенной" нумерации кодов - в последнем разряде кода группа кодируется значением 0,
// а показатели - последовательными значениями (1,2,3...).
// Например, в ОЦИС в группе "Расходы на целевые мероприятия" с кодом 6310
// описаны показатели с кодами 6311, 6312, 6313 (по видам мероприятий).
// Итоговая группа (в примере - 6310) заведомо не имеет значения и при сворачивании в целом избыточна.
// Для того, чтобы в расшифровке она не вызывала вопросов, устраняем ее из отчета.
// При этом для устранения недостаточно удалить его только из состава обобщенной группы (списка сокращаемых групп),
// так как в расшифровку уже включена информация о нем, необходимая для полной отчетности.
// См. обращения к Итоговая

Функция ЭтоИтоговаяГруппаПоказателей(КодСтрокиОбобщенногоПоказателя, СокращаемыеГруппыПоказателей, ЗаполняемыеПоказатели)
	...
	ПризнакГруппы = Сред(КодСтрокиОбобщенногоПоказателя, ДлинаПрефиксаКода, 1);
	Если ПризнакГруппы = "0" Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПрефиксКодаСтроки = Лев(КодСтрокиОбобщенногоПоказателя, ДлинаПрефиксаКода);

	Для Каждого ИдентификаторСокращаемойГруппаПоказателей Из СокращаемыеГруппыПоказателей Цикл
		СокращаемаяГруппаПоказателей = БухгалтерскаяОтчетностьБРО.ГруппаПоказателей(ИдентификаторСокращаемойГруппаПоказателей, ЗаполняемыеПоказатели);
		Если Лев(СокращаемаяГруппаПоказателей.КодСтроки, ДлинаПрефиксаКода) <> ПрефиксКодаСтроки Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОпределитьИтоговуюГруппуПоказателей(ОписаниеОбобщеннойГруппыПоказателей, ОсновнойКодСтроки, ИдентификаторОбобщеннойГруппыПоказателей, ЗаполняемыеПоказатели)
	
	КодыПоказателейВложены = ЭтоИтоговаяГруппаПоказателей(
		ОсновнойКодСтроки,
		ОписаниеОбобщеннойГруппыПоказателей.Состав,
		ЗаполняемыеПоказатели);
	Если Не КодыПоказателейВложены Тогда
		Возврат;
	КонецЕсли;
	
	ОписаниеОбобщеннойГруппыПоказателей.Итоговая = Истина;
...
Процедура ДобавитьРаскрывающиеПоказателиУпрощенногоОтчета(ЗаполняемыеПоказатели, ПланСокращенияГруппПоказателей)
	...
	Для Каждого ПланОбобщеннойГруппыПоказателей Из ПланСокращенияГруппПоказателей Цикл
		...
		Если ОбобщеннаяГруппаПоказателей.Показатель <> Неопределено Тогда
			...
			Если ОписаниеОбобщеннойГруппыПоказателей.Итоговая Тогда
				// Итоговая группа самостоятельного значения иметь не может,
				// поэтому не должна содержать показателя "имени себя"
				// (содержит только сокращаемые, см. далее)
				ОбобщеннаяГруппаПоказателей.Состав.Очистить();
			КонецЕсли;
...
Процедура СократитьЗначенияЗаполненияУпрощенногоОтчета(Процессор)
	...
	Для Каждого ПланОбобщеннойГруппыПоказателей Из ПланСокращения Цикл
		...
		Если ОписаниеОбобщеннойГруппыПоказателей.Итоговая Тогда
			// Итоговая группа самостоятельного значения иметь не может.
			// Однако, ее расшифровка уже может быть выведена.
			// В полной отчетности она проблем не создает, а в упрощенной выглядит как дублирование.
			ЗначенияИтоговойГруппы = Процессор.ЗначенияЗаполненияОтчета.Строки.Найти(
				ИдентификаторОбобщеннойГруппыПоказателей,
				"Показатель");
			Если ЗначенияИтоговойГруппы <> Неопределено И ЗначенияИтоговойГруппы.Значение = 0 Тогда
				ЗначенияИтоговойГруппы.Расшифровка.Очистить();
			КонецЕсли;
		КонецЕсли;
...
#Область КоллекцияЗаполняемыхПоказателей
...
//  Структура - см. тело функции
//
Функция НовыйПроцессорОписанияЗаполняемыеПоказатели() // Здесь сокращается до ПроцессорОписания. В контексте составления описания - до Процессор
	
	Процессор = Новый Структура;
	
	// Допускается определение контекста не полностью. Обязательные для заполнения свойства: Период, ОтчетныйПериод, Упрощенная
	Процессор.Вставить("КонтекстОтчета"); // См. БухгалтерскаяОтчетностьБРО.НовыйКонтекстОтчета
