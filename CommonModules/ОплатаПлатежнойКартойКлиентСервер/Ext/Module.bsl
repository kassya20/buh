...
Функция ТекущиеПараметрыОбъекта(Форма) Экспорт
	...
	ПараметрыОбъекта = Новый Структура(
		...
		|СчетНаОплату, СчетУчетаРасчетовСКонтрагентом, СчетУчетаРасчетовПоАвансам,
		|ПорядокОтраженияАванса, ПлательщикНДСПриУСН, СтавкаНДСПриУСН, Ссылка");
	...
	ИначеЕсли ОбщегоНазначенияклиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "РасшифровкаПлатежа") 
		И ЗначениеЗаполнено(Форма.РасшифровкаПлатежа) Тогда
	...
	Если СтрокаРасшифровки <> Неопределено Тогда
		...
		ПараметрыОбъекта.ПорядокОтраженияАванса = СтрокаРасшифровки.ПорядокОтраженияАванса;
	КонецЕсли; 
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Если ЭтоРозничнаяВыручка Тогда 
		...
		Если УчетБезЗакрывающих И ДокументПроведен
			...
			Или НЕ РозницаВключена И Не Объект.БезЗакрывающихДокументов) Тогда
	...
	КомандаВозвратСБПВидимость            = Форма.ЭтоВозвратПоСБП И Форма.ДоступностьВозвратПоСБП;
	КомандаВозвратОнлайнЗаказыВидимость   = Форма.ЭтоВозвратОнлайнЗаказы И Форма.ДоступностьВозвратОнлайнЗаказы;
	...
	Если КомандаПечатьЧека <> Неопределено Тогда
		...
		КомандаПечатьЧекаВидимость = НЕ БаннерПечатьЧекаАктивен
			И НапечататьЧекВидимость
			И Не Форма.ПлательщикНПД;
		
		// Если это возврат по СБП и подключено торговое оборудование, то должна отображаться только одна команда.
		Если КомандаВозвратСБПВидимость И КомандаПечатьЧекаВидимость И Форма.ЕстьПравоПодключаемоеОборудование Тогда
			КомандаПечатьЧека.Заголовок = НСтр("ru = 'Возврат СБП и печать чека'");
			КомандаВозвратСБПВидимость = Ложь;
		ИначеЕсли КомандаВозвратОнлайнЗаказыВидимость И КомандаПечатьЧекаВидимость И Форма.ЕстьПравоПодключаемоеОборудование Тогда
			КомандаПечатьЧека.Заголовок = НСтр("ru = 'Возврат на карту и печать чека'");
			КомандаВозвратОнлайнЗаказыВидимость = Ложь;
		Иначе
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЕстьФискальныеОперацииПоДокументу") Тогда
				КомандаПечатьЧека.Заголовок = ?(Форма.ЕстьФискальныеОперацииПоДокументу,
					НСтр("ru = 'Чек коррекции'"), НСтр("ru = 'Чек'"));
			Иначе
				КомандаПечатьЧека.Заголовок = НСтр("ru = 'Чек'");
			КонецЕсли;
		КонецЕсли;
		
		КомандаПечатьЧека.Видимость = КомандаПечатьЧекаВидимость;
		
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ВернутьСБП", "Видимость", КомандаВозвратСБПВидимость);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ВернутьОнлайнЗаказы", "Видимость", КомандаВозвратОнлайнЗаказыВидимость);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаВозвратСредствПоСБПВыполнен", "Видимость", Форма.ВозвратСредствПоСБПВыполнен);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаПредупреждениеВозвратСредствПоСБП", "Видимость", ЗначениеЗаполнено(Форма.ТекстПредупрежденияСБП));
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаВозвратОнлайнЗаказыВыполнен", "Видимость", Форма.ВозвратСредствОнлайнЗаказыВыполнен);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаПредупреждениеВозвратОнлайнЗаказы", "Видимость", ЗначениеЗаполнено(Форма.ТекстПредупрежденияОнлайнЗаказы));
...
Функция ЭтоРасчетыСКонтрагентом(ВидОперации) Экспорт
	...
	Возврат ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя")
		ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ПлатежПоРеестру")
		Или ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаСчетаЧерезСБП")
...
// Возвращает результат сравнения вида операции:
// Истина - вид операции равен "Оплата покупателя", иначе ЛОЖЬ.
//
// Возвращаемое значение:
//   Булево.
//
Функция ЭтоОплатаПокупателя(ВидОперации) Экспорт
...
Функция СостояниеВозвратаСБП(СостояниеВозврата) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ДоступностьВозвратПоСБП",     Ложь);
	Результат.Вставить("ВозвратСредствПоСБПВыполнен", Ложь);
	Результат.Вставить("ТекстПредупрежденияСБП",       "");
	
	Результат.ДоступностьВозвратПоСБП     = СостояниеВозврата.Доступен И Не СостояниеВозврата.Исполнен;
	Результат.ВозвратСредствПоСБПВыполнен = СостояниеВозврата.Доступен И СостояниеВозврата.Исполнен;
	
	Если Не СостояниеВозврата.Доступен Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ТекстПредупрежденияСБП = СостояниеВозврата.ТекстПредупреждения;
	
	Возврат Результат;
	
КонецФункции

Функция СостояниеВозвратаОнлайнЗаказы(СостояниеВозврата) Экспорт

	Результат = Новый Структура;
	Результат.Вставить("ДоступностьВозвратОнлайнЗаказы",     Ложь);
	Результат.Вставить("ВозвратСредствОнлайнЗаказыВыполнен", Ложь);
	Результат.Вставить("ТекстПредупрежденияОнлайнЗаказы",       "");
	
	Результат.ДоступностьВозвратОнлайнЗаказы     = СостояниеВозврата.Доступен И Не СостояниеВозврата.Исполнен;
	Результат.ВозвратСредствОнлайнЗаказыВыполнен = СостояниеВозврата.Доступен И СостояниеВозврата.Исполнен;
	
	Если Не СостояниеВозврата.Доступен Тогда
		Возврат Результат;
	КонецЕсли;
	
	Результат.ТекстПредупрежденияОнлайнЗаказы = СостояниеВозврата.ТекстПредупреждения;
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	ЧекСОшибкой = ЗначениеЗаполнено(Форма.СведенияОЧекеНПД)
		И Не ПустаяСтрока(Форма.СведенияОЧекеНПД.СообщениеОбОшибке);
	
	ВидимостьУслугНПД = ВидимостьЭлементовНПД И (ВключенаИнтеграция Или ЧекСформирован) И Не ЭтоВозвратПокупателю;
	...
	Элементы.ГруппаЧекОжидаетОтправкиВФНС.Видимость = ВидимостьЭлементовНПД И Не ЭтоВозвратПокупателю
		И ЧекОжидаетОтправки;
	...
	Если ЭтоВозвратПокупателю Тогда
		...
		Элементы.АннулироватьЧекНПД.Видимость    = Не ЧекАннулирован И ВключенаИнтеграция;
	...
	Иначе
		...
		Элементы.ПоказатьЧекНПД.Видимость = ЧекСформирован;
		
		Если ЧекСформирован Тогда
			Элементы.ПоказатьЧекНПД.Заголовок = Форма.СведенияОЧекеНПД.ОписаниеЧека;
		КонецЕсли;
		
		Если ЧекСОшибкой Тогда
			Элементы.ПоказатьЧекНПД.ЦветТекста = ОбщегоНазначенияВызовСервера.ЦветСтиля("ЦветТекстаОшибкиЧекиНПД");
		Иначе
			Элементы.ПоказатьЧекНПД.ЦветТекста = ОбщегоНазначенияВызовСервера.ЦветСтиля("ЦветГиперссылки");
		КонецЕсли;
