...
Процедура ПолучитьКодОрганаФСГСПодразделения(Знач Подразделение, КодФСГС) Экспорт
	...
	РеквизитыПодразделения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Подразделение, "Владелец,КодОрганаФСГС");
	
	Если ЗначениеЗаполнено(РеквизитыПодразделения.КодОрганаФСГС) Тогда
		КодФСГС = РеквизитыПодразделения.КодОрганаФСГС;
	Иначе
		КодФСГС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыПодразделения.Владелец, "КодОрганаФСГС");
	КонецЕсли;
...
Процедура ПолучитьНастройкиВидимостиЭлементовФормы(ИДОтчета, ИДРедакцииОтчета, НастройкиВидимости) Экспорт
	...
	Если ИДОтчета = "РегламентированныйОтчетБухОтчетность" Тогда
		НастройкиВидимости.ПостояннаяВидимость = Истина;
		НастройкиВидимости.ПоказыватьВФорме = Истина;
		НастройкиВидимости.ОбщаяФормаСсылка = "https://v8.1c.ru/lawmonitor/bukhgalterskaya_otchetnost/";
	КонецЕсли;
	
	Если ИДОтчета = "РегламентированныйОтчетБухОтчетностьМП" Тогда
		НастройкиВидимости.ПостояннаяВидимость = Истина;
		НастройкиВидимости.ПоказыватьВФорме = Истина;
		НастройкиВидимости.ОбщаяФормаСсылка = "https://v8.1c.ru/lawmonitor/bukhgalterskaya_otchetnost/";
	КонецЕсли;
	
	Если ИДОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		НастройкиВидимости.ПостояннаяВидимость = Истина;
		НастройкиВидимости.ПоказыватьВФорме = Истина;
		НастройкиВидимости.ОбщаяФормаСсылка = "https://v8.1c.ru/lawmonitor/ndfl_ip_za_sebya/";
	КонецЕсли;
...
Функция ПолучитьСведенияОПоказателяхОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетКосвенныеНалогиТамСоюз" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв3" Тогда
			...
			УчетНДСФормированиеОтчетности.ПолучитьСведенияОПоказателяхОтчетаКосвенныеНалогиПриИмпортеФормаОтчета2023Кв3(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПереченьЗаявленийОВвозеТоваров" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2015Кв2" Тогда
			...
			УчетНДСФормированиеОтчетности.ПолучитьСведенияОПоказателяхОтчетаПереченьЗаявленийОВвозеТоваровФормаОтчета2015Кв2(ПоказателиОтчета);
		
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС" Тогда
		
		Если ИДРедакцииОтчета = "ФормаАналогЛК" Тогда
			
			УчетНДСФормированиеОтчетности.ПолучитьСведенияОПоказателяхОтчетаСтатистикаФормаУчетаПеремещенияТоваровТСФормаАналогЛК(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПрибыль" Тогда
		...
		Иначе
			...
			ИначеЕсли РегламентированныйОтчетПрибыль.ВерсияКонтейнера(ИДРедакцииОтчета) >= '2019-10-01'
				И РегламентированныйОтчетПрибыль.ВерсияКонтейнера(ИДРедакцииОтчета) < '2025-01-01' Тогда
				УставныйКапитал.ОписаниеПоказателей_РасчетПоНалогуНаПрибыль_2019Кв4(ПоказателиОтчета);
			ИначеЕсли РегламентированныйОтчетПрибыль.ВерсияКонтейнера(ИДРедакцииОтчета) >= '2025-01-01' Тогда
				УставныйКапитал.ОписаниеПоказателей_РасчетПоНалогуНаПрибыль_2025Кв1(ПоказателиОтчета);
			КонецЕсли;
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетУСН" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв1"
			Или ИДРедакцииОтчета = "ФормаОтчета2023Кв1"
			Или ИДРедакцииОтчета = "ФормаОтчета2024Кв1"
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеСообщениеТранспортЗемля" Тогда
		
		УправлениеВнеоборотнымиАктивами.ПолучитьСведенияОПоказателяхСообщенияОНаличииТранспортаИЗемли(ПоказателиОтчета);	
	
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеГибельТС" Тогда
		
		УправлениеВнеоборотнымиАктивами.ПолучитьСведенияОПоказателяхУведомленияОГибелиТС(ПоказателиОтчета);	
		
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеСубсидияПострадавшимМСП" Тогда
		
		УчетЗарплаты.ПолучитьСведенияОПоказателяхРегламентированноеУведомлениеСубсидияПострадавшимМСП(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетИмущество" Тогда
		...
		Иначе
			...
			УправлениеВнеоборотнымиАктивами.ПолучитьСведенияОПоказателяхОтчетаИмуществоФормаОтчета2019_2025(ПоказателиОтчета, ИДРедакцииОтчета, ПараметрыОтчета);
			
		КонецЕсли;
	
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеЛьготаПоНалогуНаИмущество" Тогда
		
		УправлениеВнеоборотнымиАктивами.ПолучитьСведенияОПоказателяхЗаявленияОЛьготеПоНалогуНаИмущество(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет3НДФЛ" Или ИДОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		
		ПомощникЗаполнения3НДФЛ.СведенияОПоказателяхОтчета3НДФЛ(ИДРедакцииОтчета, ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетОперацииСДенежнымиСредствамиНФО" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв3" Тогда
			...
			УчетНФО.ПолучитьСведенияОПоказателяхОтчетаОперацииСДенежнымиСредствамиНФО2021Кв3(ПоказателиОтчета);
			
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2022Кв2" Тогда
			
			УчетНФО.ПолучитьСведенияОПоказателяхОтчетаОперацииСДенежнымиСредствамиНФО2022Кв2(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеБумажныеФормы" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ОсвобождениеОтУплатыНДС_502" Тогда
			
			УведомлениеОбОсвобожденииОтУплатыНДСВызовСервера.ПолучитьСведенияОПоказателях(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПрослеживаемыеТоварыОтчетОбОперациях" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2021Кв3" Тогда
			...
			Если ПараметрыОтчета.Свойство("ПараметрыБаннера") Тогда
				ПрослеживаемостьБП.ПараметрыБаннера(ПоказателиОтчета);
			Иначе
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" Или ИДРедакцииОтчета = "ФормаОтчета2024Кв2" Тогда
			
			УчетДенежныхСредствБП.ПолучитьСведенияОПоказателяхДвиженияхСредствЗаПределамиРФ(ПоказателиОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДоходыИностраннымОрганизациям" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" ИЛИ ИДРедакцииОтчета = "ФормаОтчета2025Кв4" Тогда
			УчетДенежныхСредствБП.ПолучитьСведенияОПоказателяхДоходыИностраннымОрганизациям(ПоказателиОтчета);
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетТуристическийНалог" Тогда
		
		Если ИДРедакцииОтчета = "ФормаОтчета2025Кв1" Или ИДРедакцииОтчета = "ФормаОтчета2026Кв1" Тогда
			ТуристическийНалогСервер.ПолучитьСведенияОПоказателяхОтчетаТуристическийНалог(ПоказателиОтчета, ИДРедакцииОтчета);
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетОценочнаяКартаУБ" Тогда
		
		ЗаполнениеОценочнойКарты.ПолучитьСведенияОПоказателяхОтчетаОценочнаяКарта(ПоказателиОтчета);
	...
	Иначе
		...
		Если ИДОтчета = "РегламентированныйОтчет4ФСС"
			...
			Если ИДРедакцииОтчета = "ФормаОтчета2017Кв1"
				ИЛИ ИДРедакцииОтчета = "ФормаОтчета2017Кв3"
				ИЛИ ИДРедакцииОтчета = "ФормаОтчета2022Кв2" Тогда
				УчетЗарплаты.ПолучитьСведенияОПоказателях4ФСС(ПоказателиОтчета, ИДРедакцииОтчета);
			КонецЕсли;
		Иначе
...
// Если не разрешен учет регулярной деятельности заполняется только ограниченный набор отчетов.
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетКосвенныеНалогиТамСоюз" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв3" Тогда
			...
			УчетНДСФормированиеОтчетности.ЗаполнитьОтчетКосвенныеНалогиПриИмпортеФормаОтчета2023Кв3(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПереченьЗаявленийОВвозеТоваров" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2015Кв2" Тогда
			...
			УчетНДСФормированиеОтчетности.ЗаполнитьОтчетПереченьЗаявленийОВвозеТоваровФормаОтчета2015Кв2(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаАналогЛК" Тогда
			...
			УчетНДСФормированиеОтчетности.ЗаполнитьОтчетСтатистикаФормаУчетаПеремещенияТоваровТСФормаАналогЛК(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПрибыль" Тогда
		...
		Иначе
			...
			// Сведения о расчетах по доходам, в отношении которых
			// организация выступает в качестве налогового агента
			ВерсияКонтейнера = РегламентированныйОтчетПрибыль.ВерсияКонтейнера(ИДРедакцииОтчета);
			Если ИДРедакцииОтчета = "ФормаОтчета2016Кв4" Тогда
				УставныйКапитал.ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2016Кв4(ПараметрыОтчета, Контейнер);
			ИначеЕсли ВерсияКонтейнера >= '2025-01-01' Тогда
				УставныйКапитал.ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2025Кв1(ПараметрыОтчета, Контейнер);
			ИначеЕсли ВерсияКонтейнера >= '2019-10-01' Тогда
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеСообщениеТранспортЗемля" Тогда
		...
		УправлениеВнеоборотнымиАктивами.ЗаполнитьСообщениеОНаличииТранспортаИЗемли(ПараметрыОтчета, Контейнер);	
	
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеГибельТС" Тогда
		
		УправлениеВнеоборотнымиАктивами.ЗаполнитьУведомлениеОГибелиТС(ПараметрыОтчета, Контейнер);			
		
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеСубсидияПострадавшимМСП" Тогда
		
		УчетЗарплаты.ЗаполнитьРегламентированноеУведомлениеСубсидияПострадавшимМСП(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетИмущество" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2019Кв1" 
			...
			УправлениеВнеоборотнымиАктивами.ЗаполнитьОтчетИмуществоФормаОтчета2019_2021(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
			
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2022Кв4" Тогда
			
			Если РасчетИмущественныхНалоговПереопределяемый.ИспользоватьПрежнийРасчетНалогаНаИмуществоВПереходныйПериод(
				 ПараметрыОтчета.Организация,
			 	 ПараметрыОтчета.мДатаКонцаПериодаОтчета) Тогда
				УправлениеВнеоборотнымиАктивами.ЗаполнитьОтчетИмуществоФормаОтчета2022_ПереходныйПериод(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
			Иначе
				УправлениеВнеоборотнымиАктивами.ЗаполнитьОтчетИмуществоФормаОтчета2022_2025(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
			КонецЕсли;
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеЛьготаПоНалогуНаИмущество" Тогда
		...
		УправлениеВнеоборотнымиАктивами.ЗаполнитьЗаявлениеОЛьготеПоНалогуНаИмущество(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетПодтверждениеВидаДеятельности" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2025Кв1" Тогда
			ЗаполнениеФормСтатистики.ЗаполнитьПодтверждениеВидаДеятельности(ПараметрыОтчета, Контейнер);
		Иначе
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчет3НДФЛ" Или ИДОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
		...
		ПомощникЗаполнения3НДФЛ.ЗаполнитьОтчет3НДФЛ(ИДРедакцииОтчета, ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетОперацииСДенежнымиСредствамиНФО" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2021Кв3" Тогда
			...
			УчетНФО.ЗаполнитьРегламентированныйОтчетОперацииСДенежнымиСредствамиНФО2021Кв3(ПараметрыОтчета, Контейнер);
			
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2022Кв2" Тогда
			
			УчетНФО.ЗаполнитьРегламентированныйОтчетОперацииСДенежнымиСредствамиНФО2022Кв2(ПараметрыОтчета, Контейнер);
	...
	ИначеЕсли ИДОтчета = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ПолучитьИмяОтчетаПоВидуУведомления(
		...
		Если ИДРедакцииОтчета = "Форма2014_1" Или ИДРедакцииОтчета = "Форма2019_1" Тогда
			УчетПСН.ЗаполнитьЗаявлениеУтратаПраваПатентаФорма2014_1(ПараметрыОтчета, Контейнер);
		ИначеЕсли ИДРедакцииОтчета = "Форма2026_1" Тогда
			УчетПСН.ЗаполнитьЗаявлениеУтратаПраваПатентаФорма2026_1(ПараметрыОтчета, Контейнер);
		КонецЕсли;
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеФормаР13014" Тогда
		...
		Если ИДРедакцииОтчета = "Форма2020_4" Или ИДРедакцииОтчета = "Форма2021_4" Тогда
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеФормаР21001" Тогда
		...
		Если ИДРедакцииОтчета = "Форма2014_1" Или ИДРедакцииОтчета = "Форма2020_4" Или ИДРедакцииОтчета = "Форма2024_1" Тогда
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеФормаР26001" Тогда
		
		Если ИДРедакцииОтчета = "Форма2020_4" Тогда
			Обработки.РегистрацияОрганизации.ЗаполнитьЗаявлениеОПрекращенииДеятельностиИП(ПараметрыОтчета, Контейнер);
		КонецЕсли;
	...
	ИначеЕсли ИДОтчета = "РегламентированноеУведомлениеБумажныеФормы" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОсвобождениеНДС" Тогда
			УведомлениеОбОсвобожденииОтУплатыНДСВызовСервера.ЗаполнитьУведомление(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
		ИначеЕсли ИДРедакцииОтчета = "ОсвобождениеОтУплатыНДС_502" Тогда
			УведомлениеОбОсвобожденииОтУплатыНДСВызовСервера.ЗаполнитьУведомление(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ" Тогда
		...
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" Или ИДРедакцииОтчета = "ФормаОтчета2024Кв2" Тогда
			УчетДенежныхСредствБП.ЗаполнитьРегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ(ПараметрыОтчета, Контейнер);
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетДоходыИностраннымОрганизациям" Тогда
		
		Если ИДРедакцииОтчета = "ФормаОтчета2023Кв4" ИЛИ ИДРедакцииОтчета = "ФормаОтчета2025Кв4" Тогда
			УчетДенежныхСредствБП.ЗаполнитьРегламентированныйОтчетДоходыИностраннымОрганизациям(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетТуристическийНалог" Тогда
		
		Если ИДРедакцииОтчета = "ФормаОтчета2025Кв1" Или ИДРедакцииОтчета = "ФормаОтчета2026Кв1" Тогда
			ТуристическийНалогСервер.ЗаполнитьРегламентированныйОтчетТуристическийНалог(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
		КонецЕсли;
		
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетОценочнаяКартаУБ" Тогда
		
		ЗаполнениеОценочнойКарты.ЗаполнитьОтчетОценочнаяКарта(ПараметрыОтчета, Контейнер);
	...
	Иначе
		...
		Если ИДОтчета = "РегламентированныйОтчет4ФСС"
			...
			Если ИДРедакцииОтчета = "ФормаОтчета2017Кв1"
				ИЛИ ИДРедакцииОтчета = "ФормаОтчета2017Кв3"
				ИЛИ ИДРедакцииОтчета = "ФормаОтчета2022Кв2" Тогда
				УчетЗарплаты.ЗаполнитьРегламентированныйОтчет4ФСС(ПараметрыОтчета, Контейнер, ИДРедакцииОтчета);
			КонецЕсли;
		Иначе
...
Функция ПолучитьСписокРегламентированныхОтчетов() Экспорт
	...
	Если РегламентированнаяОтчетностьБП.ДоступнаСтатистическаяОтчетностьБанкРоссии() Тогда
...
Процедура ФормаРегламентированнойОтчетности_ПриСозданииНаСервере(Форма) Экспорт
	...
	МобильныйКлиентБПФормаОбъекта.ФормаРегламентированнойОтчетности_ПриСозданииНаСервере(Форма);
...
Процедура ПолучитьПараметрыФормированияДекларацийПоОбособленнымПодразделениямВПомощнике(ДанныеДляЗаполнения, Организация, ДатаКонцаПериода) Экспорт
	...
	Обработки.ПомощникУплатыНалога.ПолучитьПараметрыФормированияДекларацийПоОбособленнымПодразделениямВПомощнике(ДанныеДляЗаполнения, Организация, ДатаКонцаПериода);
...
Процедура ЗаполнитьРасшифровкуПоОбособленномуПодразделению(ПараметрыАвтоформирования, ДанныеРеглОтчета) Экспорт
	...
	НаименованиеПоказателя = НСтр("ru = 'Прил. 5 к Листу 02'");
	НаименованиеСлагаемого = СтрШаблон(НСтр("ru = 'Декларация по организации в целом - Прил. 5 к Листу 02 - Стр. %1'"), ПараметрыАвтоформирования.НомерСтраницы);
	ТаблицаРасшифровки = Неопределено;
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("Ссылка", ПараметрыАвтоформирования.ОсновнаяДекларация);
	ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьСтрокуРасшифровки
		(ТаблицаРасшифровки, "", НаименованиеПоказателя, "", НаименованиеСлагаемого, 0, "", ДополнительныеПараметры);
...
Процедура ПолучитьДоступныеЗначенияВидовНалогов(Отбор, СтрокаПоиска, ДанныеВыбора) Экспорт
	...
	Исключения.Добавить(Перечисления.ВидыНалогов.НДФЛ_Дивиденды);
	Исключения.Добавить(Перечисления.ВидыНалогов.НДФЛ_Дивиденды_ДоходыСвышеПредельнойВеличины);
	Исключения.Добавить(Перечисления.ВидыНалогов.НДФЛ_ДоходыСвышеПредельнойВеличины);
	
	Исключения.Добавить(Перечисления.ВидыНалогов.НДФЛ_ИП_НалоговаяБазаСвышеПредельнойВеличины);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьОтчет", Истина);
...
Процедура ЗаполнениеДанныхПоДоговорамДляПроверкиВРЭЦ(ПараметрыЗаполнения) Экспорт
	...
	УчетНДСБП.ЗаполнениеДанныхПоДоговорамДляПроверкиВРЭЦ(ПараметрыЗаполнения);
...
Процедура ПодготовитьПараметрыСверкиДекларацииНДС(Форма) Экспорт
	...
	СервисСверкиРасчетовСверкаДекларацийНДС.ПолучитьСтраницыРазделовДекларации(Форма);
...
Процедура СверкаДекларацийПриСозданииНаСервереДекларации(Форма) Экспорт
	...
	СервисСверкиРасчетовСверкаДекларацийНДС.ПриСозданииНаСервереДекларации(Форма);
