...
Процедура ПолучитьПризнакВеденияУчетТаможенныхДекларацийЭкспорт(ЗначениеПоУмолчанию) Экспорт
	...
	ЗначениеПоУмолчанию = УчетНДСБП.ПолучитьПризнакВеденияУчетаТаможенныхДекларацийЭкспорт();
...
Процедура ПолучитьВидДоговораСПокупателем(Значение) Экспорт
	...
	Значение = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем;
...
Процедура ПолучитьФизЛицоПоСНИЛС(СНИЛС, ФизЛицоРезультат) Экспорт
	...
	|	И ФизическиеЛица.СтраховойНомерПФР = &СНИЛС
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если Выборка.Следующий() Тогда
		ФизЛицоРезультат = Выборка.Ссылка;
	КонецЕсли;
	
КонецПроцедуры

// Метод создает физ. лицо по ФИО и ИНН
//
// Параметры:
//  ФИО				 - Строка - ФИО создаваемого физ. лица
//  ИНН				 - Строка - ИНН создаваемого физ. лица
//  ФизЛицоРезультат - СправочникСсылка.ФизическиеЛица - созданное физ.лицо
//
// Пример:
...
// ФизическоеЛицоОбъект.ИНН = ИНН;
// ФизическоеЛицоОбъект.Записать();
//
// ЧастиФИО = ДокументооборотСКОВызовСервера.РазложитьФИО(ФИО);
//
...
// МенеджерЗаписи.Фамилия = ЧастиФИО.Фамилия;
// МенеджерЗаписи.Имя = ЧастиФИО.Имя;
// МенеджерЗаписи.Отчество = ЧастиФИО.Отчество;
// МенеджерЗаписи.Период = ТекущаяДатаСеанса();
// МенеджерЗаписи.Записать();
//
// ФизЛицоРезультат = ФизическоеЛицоОбъект.Ссылка;

Процедура СоздатьФизЛицоПоФИОиИНН(ФИО, ИНН, ФизЛицоРезультат) Экспорт
	
	
	ФизЛицоРезультат = УчетЗарплаты.СоздатьФизическоеЛицо(ФИО, ИНН);
...
Процедура ОпределитьВладельцаЭлектронногоДокумента(ЭД, ВладелецЭД) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Тогда
		...
		СведенияОДокументе = ОбменСКонтрагентами.СведенияОбЭлектронномДокументе(ЭД);
		ВладелецЭД = СведенияОДокументе.ОбъектыУчета;
...
Процедура ОпределитьСвойстваВладельцевЭлектронныхДокументов(СвойстваВладельцевЭД, МассивВладельцевЭД = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Тогда
		СвойстваВладельцевЭД = ОбменСКонтрагентами.СвойстваОбъектовУчетаЭлектронныхДокументовДляВыгрузкиВФНС(МассивВладельцевЭД);
	КонецЕсли;
