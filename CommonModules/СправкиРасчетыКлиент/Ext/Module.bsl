...
Функция НастроитьПериодИнтервал(Форма, ЦелыйИнтервал = Неопределено, Периодичность = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Периодичность) Тогда
		ТребуетсяВызовСервера.Вставить("НастроитьПериодОтчета");
	КонецЕсли;
...
// Выполняет действия после вывода результата в табличный документ на форме справки отчета.
//
// Параметры:
//  Форма        - УправляемаяФорма - место вывода результата отчета.
Процедура ПослеФормированияОтчета(Форма)
