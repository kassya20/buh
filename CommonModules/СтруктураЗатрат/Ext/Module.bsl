...
// Содержит перечень колонок начального сальдо, которые поддерживаются в текстах запросов этого модуля.
//
Функция ДопустимыеКолонкиНачальногоСальдо()
	
	СоставНачальногоСальдо = Новый Структура;
	Для НомерСубконто = 1 По БухгалтерскийУчет.МаксимальноеКоличествоСубконто() Цикл
		СоставНачальногоСальдо.Вставить("СальдоСубконто" + XMLСтрока(НомерСубконто));
	КонецЦикла;
	
	Возврат СоставНачальногоСальдо;
...
Функция СоставКлючаКалькуляцииСебестоимости() Экспорт
	...
	СоставКлюча.Вставить("ПрямыеЗатраты",    Новый ОписаниеТипов("Булево"));
...
Процедура СоздатьКлючи(МенеджерВременныхТаблиц, ВариантыСтруктурыЗатрат, Затраты, ЭлементыПротокола) Экспорт 
	...
	Для Каждого Вариант Из ВариантыСтруктурыЗатрат Цикл
		...
		|					ТОГДА КлючиВершин.Субконто3
		|			КОНЕЦ КАК Справочник.Номенклатура)) <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка), ЛОЖЬ) КАК ПрямыеЗатраты,
		|	ВЫБОР
		...
		|	ЭлементыСтруктурыЗатрат.ПрямыеЗатраты КАК ПрямыеЗатраты,
		...
		|			И КлючиАналитики.ПрямыеЗатраты = ЭлементыСтруктурыЗатрат.ПрямыеЗатраты
...
Процедура ЗаполнитьДаннымиВнешнихИсточников(Вершины, Вариант, МенеджерВременныхТаблиц)
	...
	|	ИсходныеДанные.ПрямыеЗатраты КАК ПрямыеЗатраты,
	...
	|			И ИсходныеДанные.ПрямыеЗатраты = КлючиСтруктурыЗатрат.ПрямыеЗатраты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сумма УБЫВ,
	...
	Если Не ЭтоСтандартныйВариант(Вариант) Тогда
		...
		// Удаляем колонки, относящиеся к аналитике начального сальдо
		УдалитьКолонкиЗапроса(ЗапросИсходныеДанные, Вариант.СоставКлюча, ДопустимыеКолонкиНачальногоСальдо());
...
Процедура Исправить(Суммы, Вариант, Отбор = Неопределено)
	...
	Для Каждого ИмяСвойства Из ИменаСвойств Цикл
		...
		Если СуммыВсехЭлементов[ИмяСвойства] < 0 Тогда
			ОсновноеПолеСортировки = ИмяСвойства;
		Иначе
			ОсновноеПолеСортировки = ИмяСвойства + " УБЫВ";
		КонецЕсли;
		// Поля сортировки дополняем ключевым полем (содержащим уникальные значения),
		// чтобы результат сортировки не зависел от окружения
		Элементы.Сортировать(СтрШаблон("%1, Идентификатор", ОсновноеПолеСортировки));
