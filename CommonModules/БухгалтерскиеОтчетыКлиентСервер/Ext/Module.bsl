...
Функция НовыйУниверсальныеНастройки() Экспорт
	...
	Настройки.Вставить("ВидПериода",						ВидыПериода().ОтчетныйПериод);
...
// Возвращает строку с представлением периода с начала года.
//
// Параметры:
//	Период       - Дата   - Конец периода.
//	ПоМесяцам    - Булево - Если Истина, возвращаемая строка будет иметь вид "Январь - Март", иначе "1 квартал".
//	ДобавлятьГод - Булево - Если Истина, содержать год вида "0001 г.".
//
// Возвращаемое значение:
//  Строка - Текст представления периода.
//
Функция ПолучитьПредставлениеПериодаСНачалаГода(Период, ПоМесяцам = Истина, ДобавлятьГод = Ложь) Экспорт
	
	Если ПоМесяцам Тогда
		
		СтрокаФорматирования = ?(ДобавлятьГод, "ДФ='ММММ гггг ''г.'''", "ДФ='ММММ'");
		
		Если Месяц(Период) = 1 Тогда
			Возврат Формат(Период, СтрокаФорматирования);
		Иначе
			Возврат СтрШаблон(НСтр("ru='%1 - %2'"), 
				Формат(НачалоГода(Период), "ДФ='ММММ'"), Формат(Период, СтрокаФорматирования));
		КонецЕсли;
		
	Иначе
		
		Если Не ДобавлятьГод Тогда
			
			Если Месяц(Период) = 3 Тогда
				Возврат НСтр("ru = '1 квартал'");
			ИначеЕсли Месяц(Период) = 6 Тогда
				Возврат НСтр("ru = '1 полугодие'");
			ИначеЕсли Месяц(Период) = 9 Тогда
				Возврат НСтр("ru = '9 месяцев'");
		...
		Возврат ПолучитьПредставлениеПериода(НачалоГода(Период), Период, Истина);
...
Процедура УстановитьВидимостьФлаговПоказателей(Форма, ЭлементыПоказателей, НалоговыйУчет, Валютный, Количественный) Экспорт
	...
	ПроверитьКорректностьЗначенийФлаговПоказателей(Форма, Валютный, Количественный);
...
Процедура УправлениеВидимостьюВидПериода(Элемент, Отчет) Экспорт
	...
	ЭтоКонецГода = КонецДня(Отчет.КонецПериода) = КонецГода(Отчет.КонецПериода);
	Элемент.Видимость = ЭтоКонецГода;
	
	ВидыПериода = ВидыПериода();
	
	Если Отчет.ВидПериода = ВидыПериода.МежотчетныйПериод Тогда
		Элемент.Подсказка = НСтр("ru = 'Вид периода: межотчетный период'");
		Элемент.Картинка = БиблиотекаКартинок.ВидПериодаМежотчетныйПериод;
	ИначеЕсли Отчет.ВидПериода = ВидыПериода.ОтчетныйИМежотчетныйПериод Тогда
		Элемент.Подсказка = НСтр("ru = 'Вид периода: отчетный и межотчетный периоды'");
		Элемент.Картинка = БиблиотекаКартинок.ВидПериодаОтчетныйИМежотчетныйПериод;
	Иначе
		Элемент.Подсказка = НСтр("ru = 'Вид периода: отчетный период'");
		Элемент.Картинка = БиблиотекаКартинок.ВидПериодаОтчетныйПериод;
	КонецЕсли;
	
КонецПроцедуры

Процедура ПроверитьКорректностьЗначенийФлаговПоказателей(Форма, Валютный, Количественный)
	
	Отчет = Форма.Отчет;
	
	// Если доступность показателя отключена
	// проверим его значение
	
	Если Не Валютный  И Отчет.ПоказательВалютнаяСумма Тогда 
		Отчет.ПоказательВалютнаяСумма = Ложь;
	КонецЕсли; 
	
	Если Не Количественный И Отчет.ПоказательКоличество Тогда 
...
// Если Отбор <> Неопределено, ресурсы ищутся по сокращенному имени - см. КореньРесурса.
//
// Параметры:
...
//  Отбор              - Неопределено
//                     - Соответствие - ресурсы ищутся по сокращенному имени - см. КореньРесурса:
//                        * Ключ: сокращенное имя ресурса
...
//
Функция НайтиРесурсы(Настройки, Отбор = Неопределено, ОтобранныеЭлементы = Неопределено) Экспорт
	...
	Для Каждого Поле Из ВыбранныеПоля Цикл
		Если Отбор = Неопределено Или Отбор[КореньРесурса(Поле.Поле)] <> Неопределено Тогда
	...
	Для Каждого ЭлементОтбора Из ЭлементыОтбора Цикл
		Если Отбор = Неопределено Или Отбор[КореньРесурса(ЭлементОтбора.ЛевоеЗначение)] <> Неопределено Тогда
	...
	Для Каждого ЭлементУпорядочивания Из ЭлементыУпорядочивания Цикл
		Если Отбор = Неопределено Или Отбор[КореньРесурса(ЭлементУпорядочивания.Поле)] <> Неопределено Тогда
