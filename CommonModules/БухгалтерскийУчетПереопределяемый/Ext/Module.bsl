...
Функция ПолучитьРеквизитыДоговораКонтрагента(ДоговорКонтрагента) Экспорт
	...
	СтруктураРеквизиты.Вставить("ОплатаВВалюте",        Ложь);
	...
	СтруктураРеквизиты.Вставить("НалогообложениеКонтрагента", Перечисления.СистемыНалогообложения.ПустаяСсылка());
...
Функция ПолучитьСведенияОСпискеНоменклатуры(Знач СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	ПрименяютсяСтавкиНДСДляУСН = РегистрыСведений.НастройкиУчетаНДС.ПрименяютсяСтавкиНДСДляУСН(Организация, Дата);
	...
	Если ЗначениеЗаполнено(Дата) Тогда
		...
		ДатаОпределенияДанныхИзИстории = Дата;
		
	Иначе
		
		ДатаОпределенияДанныхИзИстории = ОбщегоНазначения.ТекущаяДатаПользователя();
	...
	ИначеЕсли ПараметрыОбъекта.Свойство("ДоговорКонтрагента")
		И ЗначениеЗаполнено(ПараметрыОбъекта.ДоговорКонтрагента) Тогда 
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПараметрыОбъекта.ДоговорКонтрагента,
			"УчетАгентскогоНДС, Владелец, ВидАгентскогоДоговора");
	КонецЕсли;
	...
	ИменаРеквизитов =
		...
		|Услуга, УслугаРазмещения, НоменклатурнаяГруппа,
		|СтранаПроисхождения, НомерГТД, ПризнакПредметаРасчета,
		|СтатьяЗатрат, ОсновнаяСпецификацияНоменклатуры,
		|ВидМаркировки, ПериодичностьУслуги, ПрослеживаемыйТовар, ПрослеживаемыйКомплект,
		|КодВидаПодакцизногоТовара, КоэффициентПересчета, ЕдиницаИзмеренияДляРасчетаАкциза, УслугаРазмещения";
	...
	Если ВедетсяУчетПрослеживаемыхТоваров Тогда
		СоответствиеСведенийПризнакаПрослеживаемостиИзИстории = Справочники.Номенклатура.ПризнакПрослеживаемостиНаДатуИзИстории(
			СписокНоменклатурыБезПустыхЗначений, ДатаОпределенияДанныхИзИстории);
	Иначе
		СоответствиеСведенийПризнакаПрослеживаемостиИзИстории = Неопределено;
	КонецЕсли;
	
	Если ПараметрыОбъекта.Свойство("ЕстьУчетПодакцизныхТоваров") И ПараметрыОбъекта.ЕстьУчетПодакцизныхТоваров Тогда
		СоответствиеВидовПодакцизныхТоваровНоменклатуре = Новый Соответствие;
		УчетАкцизов.ПроверкаДействияАкцизаПоСпискуНоменклатуры(
			СписокНоменклатурыБезПустыхЗначений, ДатаОпределенияДанныхИзИстории,
			СоответствиеВидовПодакцизныхТоваровНоменклатуре);
	Иначе
		СоответствиеВидовПодакцизныхТоваровНоменклатуре = Неопределено;
	КонецЕсли;
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		Иначе
			ВидПродукцииИС = СведенияОНоменклатуре.ВидМаркировки;
		КонецЕсли;
		...
		СведенияОНоменклатуре.НаименованиеПолное = РаботаСНоменклатуройКлиентСерверБП.СодержаниеУслуги(
		...
		ИначеЕсли Реализация 
			...
			Если ДоходЕНВД ИЛИ ПрименяетсяОсвобождениеОтУплатыНДС Тогда
				СведенияОНоменклатуре.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			ИначеЕсли ПрименяютсяСтавкиНДСДляУСН Тогда
				// Для плательщика УСН с НДС, применяющего ставки НДС 5% или 7%
				// ставку 20% или 10% заменяем на ставку из настроек налогов.
				// При этом ставку 0% или БезНДС сохраняем и не замеяем на ставку из настроек налогов,
				// так как подобная ставка может применяться и на УСН и является характеристикой номенклатурной позиции.
				Если СведенияОНоменклатуре.СтавкаНДС <> Перечисления.СтавкиНДС.БезНДС
				   И СведенияОНоменклатуре.СтавкаНДС <> Перечисления.СтавкиНДС.НДС0 Тогда
					ВидСтавкиНДСНаУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(Организация, Дата);
					СведенияОНоменклатуре.СтавкаНДС = Перечисления.СтавкиНДС.СтавкаНДС(ВидСтавкиНДСНаУСН, ДатаОпределенияВидаСтавкиНДС);
				КонецЕсли;
		...
		ИначеЕсли ПараметрыОбъекта.Свойство("СпособЗаполненияСтавкиНДС") Тогда
			...
			Если Перечисления.ВидыАгентскихДоговоров.ЭтоРеализацияТоваров(ПараметрыОбъекта.ВидАгентскогоДоговора) Тогда 
				Если СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ОбщаяУСН Тогда
					СведенияОНоменклатуре.СтавкаНДС = УчетНДСКлиентСервер.ОбщаяУСНСтавкаНДС(Дата);
				ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.ПониженнаяУСН Тогда
					СведенияОНоменклатуре.СтавкаНДС = УчетНДСКлиентСервер.ПониженнаяУСНСтавкаНДС(Дата);
				Иначе
		...
		ИначеЕсли НЕ Реализация 
			...
			Если ЗначениеЗаполнено(ПараметрыОбъекта.ДоговорКонтрагента) Тогда 
				...
				КонтрагентРезидентТаможенногоСоюза = Справочники.Контрагенты.КонтрагентРезидентТаможенногоСоюза(РеквизитыДоговора.Владелец);
				Если Перечисления.ВидыАгентскихДоговоров.ЭтоРеализацияТоваров(РеквизитыДоговора.ВидАгентскогоДоговора) Тогда
					СведенияОНоменклатуре.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(Дата);
				ИначеЕсли КонтрагентРезидентТаможенногоСоюза И НЕ РеквизитыДоговора.УчетАгентскогоНДС Тогда
					СведенияОНоменклатуре.СтавкаНДС = Перечисления.СтавкиНДС.НДС0;
				ИначеЕсли СпособЗаполненияСтавкиНДС = Перечисления.СпособыЗаполненияСтавкиНДС.БезНДС
		...
		Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольныеЗнакиГИСМ")
		 ИЛИ НЕ ИнтеграцияГИСМКлиентСерверБП.МожноИспользоватьКонтрольныеЗнакиГИСМ(Дата)	
		 Или СведенияОНоменклатуре.ВидМаркировки <> Перечисления.ВидыПродукцииИС.ПродукцияИзНатуральногоМеха Тогда
			СведенияОНоменклатуре.Вставить("ПродукцияМаркируемаяДляГИСМ", Ложь);
		КонецЕсли;
		//Конец ИнтеграцияГИСМ
		
		Если ВедетсяУчетПрослеживаемыхТоваров Тогда
			СведенияОПризнакеПрослеживаемостиИзИстории = СоответствиеСведенийПризнакаПрослеживаемостиИзИстории.Получить(Номенклатура);
			Если СведенияОПризнакеПрослеживаемостиИзИстории = Неопределено Тогда
				ПрослеживаемыйТовар    = СведенияОНоменклатуре.ПрослеживаемыйТовар;
				ПрослеживаемыйКомплект = СведенияОНоменклатуре.ПрослеживаемыйКомплект;
			Иначе
				ПрослеживаемыйТовар    = СведенияОПризнакеПрослеживаемостиИзИстории.ПрослеживаемыйТовар;
				ПрослеживаемыйКомплект = СведенияОПризнакеПрослеживаемостиИзИстории.ПрослеживаемыйКомплект;
			КонецЕсли;
			
			СведенияОНоменклатуре.Вставить("ПрослеживаемыйТовар",    ПрослеживаемыйТовар);
			СведенияОНоменклатуре.Вставить("ПрослеживаемыйКомплект", ПрослеживаемыйКомплект);
		КонецЕсли;
		
		Если НЕ ВедетсяУчетПрослеживаемыхТоваров
			ИЛИ СведенияОНоменклатуре.ПрослеживаемыйТовар = Неопределено Тогда
			СведенияОНоменклатуре.Вставить("ПрослеживаемыйТовар", Ложь);
		КонецЕсли;
		
		Если НЕ ВедетсяУчетПрослеживаемыхТоваров
			ИЛИ СведенияОНоменклатуре.ПрослеживаемыйКомплект = Неопределено Тогда
			СведенияОНоменклатуре.Вставить("ПрослеживаемыйКомплект", Ложь);
		КонецЕсли;
		
		Если ПараметрыОбъекта.Свойство("ЕстьУчетПодакцизныхТоваров") И ПараметрыОбъекта.ЕстьУчетПодакцизныхТоваров Тогда
			СведенияОбАкцизеСУчетомСроков = СоответствиеВидовПодакцизныхТоваровНоменклатуре.Получить(Номенклатура);
			Если СведенияОбАкцизеСУчетомСроков = Неопределено Тогда
				КодВидаПодакцизногоТовара        = СведенияОНоменклатуре.КодВидаПодакцизногоТовара;
				КоэффициентПересчета             = СведенияОНоменклатуре.КоэффициентПересчета;
				ЕдиницаИзмеренияДляРасчетаАкциза = СведенияОНоменклатуре.ЕдиницаИзмеренияДляРасчетаАкциза;
			Иначе 
				КодВидаПодакцизногоТовара        = СведенияОбАкцизеСУчетомСроков.КодВидаПодакцизногоТовара;
				КоэффициентПересчета             = ?(СведенияОбАкцизеСУчетомСроков.АкцизСчитаетсяНаДату = Ложь, 0, СведенияОНоменклатуре.КоэффициентПересчета);
				ЕдиницаИзмеренияДляРасчетаАкциза = СведенияОНоменклатуре.ЕдиницаИзмеренияДляРасчетаАкциза;
			КонецЕсли;
			СведенияОНоменклатуре.Вставить("КодВидаПодакцизногоТовара",        КодВидаПодакцизногоТовара);
			СведенияОНоменклатуре.Вставить("КоэффициентПересчета",             КоэффициентПересчета);
			СведенияОНоменклатуре.Вставить("ЕдиницаИзмеренияДляРасчетаАкциза", ЕдиницаИзмеренияДляРасчетаАкциза);
			
		Иначе
			
			СведенияОНоменклатуре.Вставить("КодВидаПодакцизногоТовара",        Неопределено);
			СведенияОНоменклатуре.Вставить("КоэффициентПересчета",             0);
			СведенияОНоменклатуре.Вставить("ЕдиницаИзмеренияДляРасчетаАкциза", ЕдиницаИзмеренияДляРасчетаАкциза);
...
Функция ПодготовитьСоответствиеНоменклатураЦенаПоСкладам(СтруктураПараметров) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(НоменклатураСклад.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ВЫРАЗИТЬ(НоменклатураСклад.ТипЦен КАК Справочник.ТипыЦенНоменклатуры) КАК ТипЦен
	|ПОМЕСТИТЬ втНоменклатураСклад
	|ИЗ
	|	&НоменклатураСклад КАК НоменклатураСклад
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втНоменклатураСклад.Номенклатура КАК Номенклатура,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалют КАК КурсыВалют
	|		ПО ЦеныНоменклатурыСрезПоследних.Валюта = КурсыВалют.Валюта
	|		ЛЕВОЕ СОЕДИНЕНИЕ втНоменклатураСклад КАК втНоменклатураСклад
	|		ПО ЦеныНоменклатурыСрезПоследних.Номенклатура = втНоменклатураСклад.Номенклатура
	|			И ЦеныНоменклатурыСрезПоследних.ТипЦен = втНоменклатураСклад.ТипЦен";
	
	Запрос.УстановитьПараметр("Период", Период);
	Запрос.УстановитьПараметр("НоменклатураСклад", Номенклатура); 
...
Функция СкладВТабличнойЧасти(Реквизиты) Экспорт
	
	Возврат ?((ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
			 И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Регистратор, "СкладВСписке")), Истина, Ложь);
...
Функция ТаблицаСуммСписанияАктивов(МассивРегистраторов, ДатаНач, ДатаКон) 
	...
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов);
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	Возврат БухгалтерскийУчет.ОписаниеОрганизации(СписокСведений, Список, СПрефиксом);
...
//
// Возвращаемое значение:
//  Массив Из СправочникСсылка.Организации - перечень структурных частей переданной головной организации
//
Функция ВсяОрганизация(Организация, Период = '00010101') Экспорт
	...
	Если ЗначениеЗаполнено(Период) Тогда
		...
		|ИЗ
		|	Справочник.Организации КАК Организации
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияРегистрацийВНалоговомОргане.СрезПоследних(&Период, СтруктурнаяЕдиница.ГоловнаяОрганизация = &Организация) КАК ИсторияРегистраций
...
//  ГосКонтракт		 - СправочникСсылка.ГосКонтракты - гос.контракт реквизиты по которому нужно включить в реквизиты компании
...
//
Функция СведенияОЮрФизЛице(Компания, Период = '00010101', Знач БанковскийСчет = Неопределено, ПлатежВБюджет = Ложь, ГосКонтракт = Неопределено, КрупнейшийНалогоплательщик = Ложь, ЦифровойСчет = Ложь) Экспорт
	...
	Если ТипЗнч(Компания) = Тип("СправочникСсылка.Организации") Тогда
		ЗаполнитьРеквизитыОрганизации(Реквизиты, Компания, Период, ПлатежВБюджет Или КрупнейшийНалогоплательщик);
		...
		Если Не ЗначениеЗаполнено(БанковскийСчет) Тогда
			...
			Если ЦифровойСчет Тогда
				...
				БанковскиеСчетаЦифровые = Справочники.БанковскиеСчета.БанковскиеСчетаОрганизации(
					Компания, ВалютаРеглУчета, , Истина);
				Если БанковскиеСчетаЦифровые.Количество() > 0 Тогда
					БанковскийСчет = БанковскиеСчетаЦифровые[0].Ссылка;
				КонецЕсли;
			КонецЕсли;
	...
	Если ПлатежВБюджет
		И ТипЗнч(Компания) = Тип("СправочникСсылка.Организации")
		И Реквизиты.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ЮрАдресИзJSON = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
			Реквизиты.ЗначениеJSONЮридическийАдрес, Перечисления.ТипыКонтактнойИнформации.Адрес);
		Если ЮрАдресИзJSON.addressType = РаботаСАдресамиКлиентСервер.МуниципальныйАдрес() Тогда
			ЮрАдресИзJSON.addressType = РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес();
			РаботаСАдресами.ОбновитьПредставлениеАдреса(ЮрАдресИзJSON, Ложь);
			Реквизиты.ЮридическийАдресИП = ЮрАдресИзJSON.value;
		Иначе
			Реквизиты.ЮридическийАдресИП = Реквизиты.ЮридическийАдрес;
		КонецЕсли;
	КонецЕсли;
...
Функция НовыйРеквизитыКомпании() Экспорт
	...
	Реквизиты.Вставить("ЮридическийАдресИП",                 "");
	...
	Реквизиты.Вставить("ДатаПрисвоенияОГРН",                 ""); // Нужен для ИП при заполнении УПД
	...
	Реквизиты.Вставить("КонтрагентИП",                       Ложь); // Только для контрагентов-ИП
	...
	Реквизиты.Вставить("ИдентификаторЦифровогоСчета",        "");
	...
	Реквизиты.Вставить("ЦифровойСчет",                       Ложь);
	Реквизиты.Вставить("ЯвляетсяБанкомРФ",                   Истина);
	...
	Реквизиты.Вставить("ЭтоСамозанятый",                     Ложь);
	Реквизиты.Вставить("ВАрхиве",                            Ложь);
	
	Возврат Реквизиты;
	
КонецФункции

Процедура ЗаполнитьРеквизитыОрганизации(Реквизиты, Организация, Период, РеквизитыКрупнейшегоНалогоплательщика = Ложь)
	...
	ИменаРеквизитовОбъекта = "ЮридическоеФизическоеЛицо, Представление, ИНН, ОГРН, ДатаРегистрации, ДатаПрисвоенияОГРН, КодПоОКПО, ВАрхиве, "
	...
	Если Реквизиты.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если РегистрацияОрганизацииКлиентСервер.ПрименяетсяОГРНИП(Реквизиты.ДатаРегистрации) Тогда
			Реквизиты.ДатаПрисвоенияОГРН = Реквизиты.ДатаРегистрации;
		КонецЕсли;
	Иначе // Юридическое лицо
		Реквизиты.КПП = Справочники.Организации.КППНаДату(Организация, Период, Не РеквизитыКрупнейшегоНалогоплательщика);
		...
		Если РегистрацияОрганизацииКлиентСервер.ПрименяетсяОГРНЮЛ(Реквизиты.ДатаРегистрации) Тогда
			Реквизиты.ДатаПрисвоенияОГРН = Реквизиты.ДатаРегистрации;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыКонтрагента(Реквизиты, Контрагент, Период)
	...
	ИменаРеквизитовКонтрагентаИП = "ИндивидуальныйПредприниматель";
	ИменаРеквизитовПредпринимателяРасширенное = ИменаРеквизитовПредпринимателя + ", " + ИменаРеквизитовКонтрагентаИП;
	ИменаРеквизитовОбъекта = "ЮридическоеФизическоеЛицо, Представление, ИНН, КодПоОКПО, РегистрационныйНомер, "
		+ "СтранаРегистрации, НалоговыйНомер, ДатаРегистрации, " + ИменаРеквизитовПредпринимателяРасширенное;
	...
	Реквизиты.КонтрагентИП                     = ЗначенияРеквизитовОбъекта.ИндивидуальныйПредприниматель;
	
	ЗаполнитьЗначенияСвойств(Реквизиты, ЗначенияРеквизитовОбъекта, ,
		ИменаРеквизитовПредпринимателяРасширенное); // Все, кроме реквизитов предпринимателя
	...
	Если Реквизиты.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ФИОПредпринимателя = ОбщегоНазначенияБПКлиентСервер.ФамилияИмяОтчествоПоНаименованиюИП(Реквизиты.ПолноеНаименование);
		
		Реквизиты.ФамилияИнициалыФизлица = ?(ФИОПредпринимателя <> Неопределено,
			ФизическиеЛицаКлиентСервер.ФамилияИнициалы(ФИОПредпринимателя),
...
Процедура ЗаполнитьРеквизитыБанковскогоСчета(Реквизиты, БанковскийСчет)
	...
	|	ЕСТЬNULL(БанковскиеСчета.БанкДляРасчетов.Страна, БанковскиеСчета.Банк.Страна) КАК Страна,
	...
	Если Выборка.Следующий() Тогда
		...
		Если Реквизиты.ЦифровойСчет Тогда
			Реквизиты.Банк = "";
		КонецЕсли;
		Реквизиты.ЯвляетсяБанкомРФ = Выборка.Страна = Справочники.СтраныМира.Россия;
	КонецЕсли;
...
Функция СвидетельствоОРегистрации(РеквизитыОрганизации, ДатаДокумента = '00010101') Экспорт
	...
	ДатаПрисвоенияОГРН = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыОрганизации, "ДатаПрисвоенияОГРН");
	...
	Если ОГРНВместоСвидетельстваОРегистрации(ДатаРегистрации, ДатаДокумента) Тогда
		...
		Если ЗначениеЗаполнено(ДатаПрисвоенияОГРН) Тогда
			ДатаПрисвоенияОГРНСтрокой = Формат(ДатаПрисвоенияОГРН, "ДЛФ=D") + " " + НСтр("ru = 'г.'");
		Иначе
			ДатаПрисвоенияОГРНСтрокой = НСтр("ru = '"" __ "" ______________ 20__ г.'");
		КонецЕсли;
		
		Если ЭтоФизЛицо Тогда
			Результат = СтрШаблон(НСтр("ru = 'ОГРНИП %1, %2'"),
				СокрЛП(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыОрганизации, "ОГРН")),
				ДатаПрисвоенияОГРНСтрокой);
		Иначе
			Результат = СтрШаблон(НСтр("ru = 'ОГРН %1, %2'"),
				СокрЛП(ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыОрганизации, "ОГРН")),
				ДатаПрисвоенияОГРНСтрокой);
		КонецЕсли;
	Иначе
...
// Функция возвращает 1, если используется учет зарплаты , иначе 0.
//
Функция ВедетсяУчетЗарплатыКадров() Экспорт
	
	Возврат ?(Константы.ВедетсяУчетЗарплатыКадров.Получить(), 1, 0);

КонецФункции // ВедетсяУчетЗарплатыКадров()
...
Функция ПолучитьСчетаУчетаВНТТ(Организация, Склад, Дата, УчетнаяПолитикаНУ) Экспорт
	...
	Запрос.УстановитьПараметр("РозничнаяТорговляОблагаетсяЕНВД", УчетнаяПолитикаНУ.РозничнаяТорговляОблагаетсяЕНВД);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПорядокНалогообложенияТорговыхТочекСрезПоследних.ОсобыйПорядокНалогообложения КАК ОсобыйПорядокНалогообложения,
	|	1 КАК Приоритет
	|ПОМЕСТИТЬ ВТ_ПорядокНалогообложенияТорговыхТочек
	|ИЗ
	|	РегистрСведений.ПорядокНалогообложенияТорговыхТочек.СрезПоследних(&Период, Склад = &Склад) КАК ПорядокНалогообложенияТорговыхТочекСрезПоследних
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВЫБОР
	|		КОГДА ВТ_ПорядокНалогообложенияТорговыхТочек.ОсобыйПорядокНалогообложения
	...
	|ИЗ
	|	ВТ_ПорядокНалогообложенияТорговыхТочек КАК ВТ_ПорядокНалогообложенияТорговыхТочек
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВТ_ПорядокНалогообложенияТорговыхТочек.Приоритет";
...
Функция ПолучитьСчетаУчетаСпискаНоменклатуры(Организация, СписокНоменклатуры, Склад = Неопределено, Знач Дата = Неопределено, Знач ОсобыйПорядокНалогообложения = Ложь) Экспорт
	...
	Для Каждого Номенклатура Из СписокНоменклатуры Цикл
		...
		СтруктураВозврата.Вставить("СчетАкциза",                    ПустойСчет);
		СтруктураВозврата.Вставить("СчетАкцизаПоИмпорту",           ПустойСчет);
	...
	|	И СчетаУчетаНоменклатуры.Номенклатура.ЭтоГруппа";
	
	ЕстьГруппыНоменклатуры = НЕ Запрос.Выполнить().Пустой();
	...
	|	СчетаУчетаНоменклатуры.СпособУчетаНДС КАК СпособУчетаНДС,
	|	СчетаУчетаНоменклатуры.СчетУчетаАкцизаПоРеализации КАК СчетАкциза,
	|	СчетаУчетаНоменклатуры.СчетУчетаАкцизаПоИмпорту КАК СчетАкцизаПоИмпорту
	|ИЗ
	...
	|	СчетаУчетаНоменклатуры.СчетУчетаАкцизаПоИмпорту
	|ИЗ
	|	ВТТаблицаИерархииСВидами КАК ТаблицаИерархии
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СчетаУчетаНоменклатуры КАК СчетаУчетаНоменклатуры
	|		ПО ТаблицаИерархии.Родитель = СчетаУчетаНоменклатуры.Номенклатура
...
Функция СчетаУчетаЗатратПоПодразделениям() Экспорт
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.СобственныеИсследованияИРазработки); // 20.04
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства_);       // 23
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства);        // 23.01
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВыполнениеРемонтовОС);               // 23.05
...
Функция ЗабалансовыеСчетаПодлежащиеПереоценке()
	...
	Счета.Добавить(ПланыСчетов.Хозрасчетный.НДСРасчетыПоОтгрузке);
...
Функция СчетаРасходовЕНВД() Экспорт
	...
	МассивСчетов = СчетаРасходовНаПродажу();
	...
	ТаблицаСчетов = Запрос.Выполнить().Выгрузить();
	МассивСчетов = ТаблицаСчетов.ВыгрузитьКолонку("Счет");
...
Функция СчетаРасходовНаПродажу() Экспорт
	
	СчетаРасходов = Новый Массив;
	СчетаРасходов.Добавить(ПланыСчетов.Хозрасчетный.СебестоимостьПродаж); // 90.02
	СчетаРасходов.Добавить(ПланыСчетов.Хозрасчетный.Продажи_РасходыНаПродажу); // 90.07
	СчетаРасходов.Добавить(ПланыСчетов.Хозрасчетный.Продажи_УправленческиеРасходы); // 90.08
...
Функция ПолучитьЗначениеПоУмолчанию(Настройка, ИмяПользователяИБ = Неопределено) Экспорт
	...
	ИначеЕсли НастройкаВРег = ВРег("ОсновнаяСтавкаНДС") Тогда
		ПустоеЗначение = Перечисления.СтавкиНДС.ПустаяСсылка();
		ИмяОбъекта = "Перечисление.СтавкиНДС";
		НастройкаТипаСсылка = Истина;
	ИначеЕсли НастройкаВРег = ВРег("ОсновнойВариантРасчетаВходящегоНДС") Тогда
		ПустоеЗначение = Перечисления.ВариантыРасчетаНДС.НДССверху;
		ИмяОбъекта = "Перечисление.ВариантыРасчетаНДС";
		НастройкаТипаСсылка = Истина;
	ИначеЕсли НастройкаВРег = ВРег("ОсновнойВариантРасчетаИсходящегоНДС") Тогда
		ПустоеЗначение = Перечисления.ВариантыРасчетаНДС.НДСВСумме;
		ИмяОбъекта = "Перечисление.ВариантыРасчетаНДС";
		НастройкаТипаСсылка = Истина;
	ИначеЕсли НастройкаВРег = ВРег("ОсновнаяГостиница") Тогда
		ПустоеЗначение = Справочники.Гостиницы.ПустаяСсылка();
		ИмяОбъекта = "Справочник.Гостиницы";
	ИначеЕсли НастройкаВРег = ВРег(ОповещенияПользователейОСобытияхКлиентСервер.ИмяНастройкиОповещенияПриИзмененииСтатусаОплатыСчета()) Тогда
		ПустоеЗначение = Ложь;
	ИначеЕсли НастройкаВРег = ВРег(ОповещенияПользователейОСобытияхКлиентСервер.ИмяНастройкиОповещенияПриИзмененииСтатусаОтгрузкиСчета()) Тогда
		ПустоеЗначение = Ложь;
	ИначеЕсли НастройкаВРег = ВРег(ОповещенияПользователейОСобытияхКлиентСервер.ИмяНастройкиОповещенияОПоступленииНовыхЗадач()) Тогда
		// По умолчанию оповещения о новых задачах включено для всех пользователей
...
Функция ПоказыватьСчетаУчетаВДокументахПоУмолчанию()
	...
	// В Простом интерфейсе скрываем счета учета для всех
	Возврат Не ОбщегоНазначенияБП.ЭтоПростойИнтерфейс() И Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком();

КонецФункции

#КонецОбласти

#Область НастройкиЭлектроннойПочты
...
Процедура ОбработатьРезультатПроверкиИтогов(Форма, РезультатВыполнения) Экспорт
	...
	ФлагКонтрольногоСоотношения = Новый Структура("КонтрольноеСоотношениеИтоговВыполняется",
		РезультатВыполнения.КонтрольноеСоотношениеИтоговВыполняется);
	ЗаполнитьЗначенияСвойств(Форма, ФлагКонтрольногоСоотношения);
	
	// Если контроль успешен, или в отчете нет контроля.
	Если ФлагКонтрольногоСоотношения.КонтрольноеСоотношениеИтоговВыполняется Тогда
		Возврат;
	КонецЕсли;
...
Функция ДоступенБухгалтерскийУчетНекоммерческихОрганизаций() Экспорт
	
	// Данная конфигурация не предназначена для ведения учета в некоммерческих организациях
	Возврат Ложь;
...
Функция СчетаПодлежащиеПереоценке() Экспорт
	...
	// Исключаются счета, у которых в регистре "Счета с особым порядком переоценки" 
	// установлен порядок "Не переоценивать автоматически".
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ЗабалансовыеСчетаПодлежащиеПереоценке",
		БухгалтерскийУчет.СформироватьМассивСубсчетов(ЗабалансовыеСчетаПодлежащиеПереоценке()));
	...
	|		ПО Хозрасчетный.Ссылка = СчетаСОсобымПорядкомПереоценкиБУ.Счет
	|			И (СчетаСОсобымПорядкомПереоценкиБУ.ПорядокПереоценки = ЗНАЧЕНИЕ(Перечисление.ОсобыйПорядокПереоценки.НеПереоцениватьАвтоматически))
	|ГДЕ
	|	СчетаСОсобымПорядкомПереоценкиБУ.Счет ЕСТЬ NULL
	|	И Хозрасчетный.Валютный
	|	И (НЕ Хозрасчетный.Забалансовый
	|			ИЛИ Хозрасчетный.Ссылка В (&ЗабалансовыеСчетаПодлежащиеПереоценке))";
...
Функция ОГРНВместоСвидетельстваОРегистрации(ДатаРегистрации, ДатаДокумента)
	...
	Возврат РегистрацияОрганизацииКлиентСервер.ВыдаетсяЛистЗаписиЕГР(ДатаРегистрации);
