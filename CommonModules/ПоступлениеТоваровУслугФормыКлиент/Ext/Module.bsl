...
Процедура ПорядокУчетаРасчетовНажатие(Форма, Элемент, СтандартнаяОбработка, АдресХранилищаЗачетАвансов) Экспорт
	...
	ВидДоговораСКомиссионером = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СКомиссионером");
	ВидДоговораСМаркетплейсом = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СТранспортнойКомпанией");
	
	ЭтоУслугиКомиссионера = (ПоступлениеТоваровУслугФормыКлиентСервер.ПолучитьРеквизитФормы(Форма, "ВидДоговора") = ВидДоговораСКомиссионером)
							ИЛИ (ПоступлениеТоваровУслугФормыКлиентСервер.ПолучитьРеквизитФормы(Форма, "ВидДоговора") = ВидДоговораСМаркетплейсом);
	ЭтоКомиссия           = ПоступлениеТоваровУслугФормыКлиентСервер.ПолучитьРеквизитФормы(Форма, "ЭтоКомиссия") ИЛИ ЭтоУслугиКомиссионера;
	...
	ПараметрыФормы.Вставить("ТипыДокументов",
		ПоступлениеТоваровУслугФормыВызовСервера.ОписаниеТипаРеквизитаДокументАванса());
	...
	ПараметрыФормы.Вставить("ПользовательУправляетСчетамиУчета", СчетаУчетаВДокументахВызовСервераПовтИсп.ПользовательУправляетСчетамиУчета());
	
	Если ЭтоУслугиКомиссионера Тогда
		ПараметрыФормы.Вставить("СчетУчетаРасчетовЗаПосредническиеУслуги", Объект.СчетУчетаРасчетовСКонтрагентом);
		ПараметрыФормы.Вставить("СчетУчетаРасчетовПоАвансамВыданным",      Объект.СчетУчетаРасчетовПоАвансам);
		ПараметрыФормы.Вставить("СчетУчетаРасчетовСКонтрагентом",          Объект.СчетУчетаРасчетовСКомиссионером);
		
		ОткрытьФорму("Документ.ОтчетКомиссионераОПродажах.Форма.ФормаПорядокУчетаРасчетов", ПараметрыФормы, Форма);
	Иначе
...
Процедура ГрузоотправительГрузополучательНажатие(Форма, Элемент) Экспорт
	...
	ПараметрыФормы.Вставить("ДОПП",                              Объект.ДОПП);
	ПараметрыФормы.Вставить("Организация",                       Объект.Организация);
	ПараметрыФормы.Вставить("Контрагент",                        Объект.Контрагент);
...
Процедура НомерСчетаФактурыПолученногоПриИзменении(Форма, Элемент) Экспорт
	...
	// Признак, что вместо счета-фактуры, полученного от комиссионера регистрируется заявление о ввозе товаров
	...
	Если Заявление.ЗаявлениеОВвозеТоваровОтКомиссионера = Истина Тогда
		УчетНДСКлиент.ДатаОтметкиОРегистрацииЗаявленияОВвозеТоваров(Форма.ДатаСчетаФактурыПолученного,
...
Функция ПодготовитьПараметрыОбработкиТоварыНоменклатураПриИзменении(Форма, СтрокаТабличнойЧасти) Экспорт
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		...
		|Упаковка, УпаковкаЕдиницаИзмерения, КоличествоУпаковок, ЦенаЗаУпаковку, КоэффициентУпаковки,
	...
	ДанныеОбъекта = Новый Структура(
		...
		|ВедетсяУчетПрослеживаемыхТоваров, ИспользуютсяУпаковки");
...
Функция ПолучитьПараметрыОбработкиТабличнойЧастиТовары(Форма, АдресХранилищаТовары)
	...
	ПараметрыОбработки.Вставить("ИспользуютсяУпаковки",         ПоступлениеТоваровУслугФормыКлиентСервер.ПолучитьРеквизитФормы(Форма, "ИспользуютсяУпаковки"));
...
Процедура ОбработатьПередЗаписью(Форма, Отказ, ПараметрыЗаписи) Экспорт
	...
	Если Форма.ОригиналСчетаФактуры И Не ЗначениеЗаполнено(Форма.СчетФактура)
...
Процедура ТоварыСкладНачалоВыбора(ЭлементФормы) Экспорт
	...
	ЭлементОтбора.ПравоеЗначение 	= ПоступлениеТоваровУслугФормыВызовСервера.СкладыНТТ();
	ЭлементОтбора.Использование 	= Истина;
	ЭлементОтбора.РежимОтображения 	= РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
	
	ПараметрыФормы = Новый Структура("ФиксированныеНастройки", НастройкиКомпоновки);
	ОткрытьФорму("Справочник.Склады.Форма.ФормаВыбора", ПараметрыФормы, ЭлементФормы, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
