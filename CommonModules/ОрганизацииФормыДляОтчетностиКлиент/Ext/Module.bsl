...
Процедура ВыбратьКодИзКлассификатора(ИмяКлассификатора, ИмяРеквизитаКод, ИмяРеквизитаНаименование, Форма, Объект, СтандартнаяОбработка, ОбработчикВыбора = Неопределено) Экспорт
	...
	Если ИмяКлассификатора = "ОКОПФ"
		...
		ОповещениеОЗакрытии = Новый ОписаниеОповещения(
			"ВыбратьКодИзПоставляемогоКлассификатораЗавершение",
	...
	ИначеЕсли ИмяКлассификатора = "ОКВЭД2" Тогда
		ОповещениеОЗакрытии = Новый ОписаниеОповещения("ВыбратьКодИзОКВЭД2Завершение", ЭтотОбъект, ПараметрыЗавершенияВыбора);
		ПараметрыФормы = КлассификаторОКВЭДКлиент.НовыйПараметрыВыбора();
		ПараметрыФормы.ОписаниеОповещенияОЗакрытии = ОповещениеОЗакрытии;
		ПараметрыФормы.Владелец = Форма.Элементы[ИмяРеквизитаКод];
		ПараметрыФормы.ТекущийКод = Объект[ИмяРеквизитаКод];
...
Процедура ВыбратьКодИзПоставляемогоКлассификатораЗавершение(РезультатЗакрытия, ПараметрыЗавершенияВыбора) Экспорт
	...
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") Тогда
		...
		ПараметрыЗавершенияВыбора.Объект[ПараметрыЗавершенияВыбора.ИмяРеквизитаКод]          = РезультатЗакрытия.Код;
		ПараметрыЗавершенияВыбора.Объект[ПараметрыЗавершенияВыбора.ИмяРеквизитаНаименование] = РезультатЗакрытия.Наименование;
		
		ВыбранныйКод = РезультатЗакрытия.Код;
		
	Иначе
		
		ВыбранныйКод = Неопределено;
...
&НаКлиенте
Процедура ВыбратьКодИзОКВЭД2Завершение(РезультатЗакрытия, ПараметрыЗавершенияВыбора) Экспорт
	
	Если ТипЗнч(РезультатЗакрытия) = Тип("Структура") Тогда
		ПараметрыЗавершенияВыбора.Форма.ПодсказкаОКВЭД2 = "";
		ПараметрыЗавершенияВыбора.Форма.Модифицированность = Истина;
		
		ДанныеОКВЭД2 = РезультатЗакрытия.ВыбранныеЗначения[0];
		ПараметрыЗавершенияВыбора.Объект[ПараметрыЗавершенияВыбора.ИмяРеквизитаКод] = ДанныеОКВЭД2.Код;
		ПараметрыЗавершенияВыбора.Объект[ПараметрыЗавершенияВыбора.ИмяРеквизитаНаименование] = ДанныеОКВЭД2.Наименование;
		
		ВыбранныйКод = ДанныеОКВЭД2.Код;
	Иначе
		ВыбранныйКод = Неопределено;
	КонецЕсли;
