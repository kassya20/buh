...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ВыгрузкаВАрхив", Истина);
КонецПроцедуры
...
Процедура ПоказатьМногострочныеЧасти(Форма, Макет, МногострочныеЧасти, ЧисловыеФорматы = Неопределено) Экспорт 
	Если ТипЗнч(МногострочныеЧасти) = Тип("СписокЗначений") 
		...
		Для Каждого МнгСтр Из МногострочныеЧасти Цикл 
			...
			ИсхСтр = ОбщегоНазначения.СкопироватьРекурсивно(Форма.ДанныеДопСтрокСтраницы[МнгСтр.Значение][0].Значение);
...
Функция СформироватьСправку(Объект, УИДТекущаяСтраница, ИмяРаздела) Экспорт
	СписокИспользуемыхМакетов = ЗагрузитьМакетыИзАрхива(Объект, "БланкиПечати_");
	...
	ИННКПП = ТиповаяСтруктураИННКППДляПечати(Объект, СтруктураПараметров.ДанныеУведомления.Титульная);
...
Процедура УдалитьСтруктурыДанных(Форма, СтрокаДляУдаления)
	...
	Для Каждого КЗ Из Форма.ДанныеДопСтрок Цикл 
		Таблица = ПолучитьИзВременногоХранилища(КЗ.Значение);
		СтрокиДляУдаления = Таблица.НайтиСтроки(Новый Структура("УИД", СтрокаДляУдаления.УИД));
		Если СтрокиДляУдаления.Количество() > 0 Тогда 
			Для Каждого Стр Из СтрокиДляУдаления Цикл 
				Таблица.Удалить(Стр);
			КонецЦикла;
			Форма.ДанныеДопСтрок[КЗ.Ключ] = ПоместитьВоВременноеХранилище(Таблица, Новый УникальныйИдентификатор);
		КонецЕсли;
	КонецЦикла;
...
Процедура СкопироватьСтрокуВДеревеЗначенийРек(СтрокаИсточник, СтрокаПриемник, Форма, УИДТекСтр, Уровень)
	...
	Если Уровень > 0 
		...
		СписокСтраниц.Добавить(ОбщегоНазначения.СкопироватьРекурсивно(СписокСтраниц[0].Значение));
