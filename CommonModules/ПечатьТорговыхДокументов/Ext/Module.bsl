...
Функция ПолучитьОписаниеАктаОбОказанииУслуг() Экспорт
	...
	ТипДокументы.Добавить(Тип("ДокументСсылка.РеализацияУслугПоПереработке"));
...
Функция ПечатьАктаОбОказанииУслуг(СведенияАктаОбОказанииУслуг, ОбъектыПечати, СтруктураПараметровПечати) Экспорт
	...
	Для Каждого СведенияОДокументе ИЗ СведенияАктаОбОказанииУслуг Цикл
		...
		ПредставлениеПоставщика = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбПоставщике,
			"НаименованиеДляПечатныхФорм,ИНН,ЮридическийАдрес,Телефоны,НомерСчета,ИдентификаторЦифровогоСчета,Банк,БИК,КоррСчет");
...
Функция ПечатьСоглашенияОбИзмененииСтоимости(СведенияСоглашенияОбИзмененииСтоимости, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияСоглашенияОбИзмененииСтоимости Цикл
		...
		СведенияОПокупателе       = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
				СведенияОДокументе.Покупатель, СведенияОДокументе.ДатаДляПолученияСведений);
...
Функция ПолучитьОписаниеСчетаНаОплату() Экспорт
	...
	Таблица.Колонки.Добавить("НастройкаИнтеграцииСБП",
		Новый ОписаниеТипов("СправочникСсылка.НастройкиПодключенияКСистемеБыстрыхПлатежей"));
		
	Таблица.Колонки.Добавить("ИдентификаторПлатежаСБП", Новый ОписаниеТипов("Строка"));
	...
	Таблица.Колонки.Добавить("СуммаВключаетНДС", 		Новый ОписаниеТипов("Булево"));
	Таблица.Колонки.Добавить("ДокументБезНДС", 			Новый ОписаниеТипов("Булево"));
	...
	Таблица.Колонки.Добавить("ЗаголовокСчета",          Новый ОписаниеТипов("Строка"));
	...
	Таблица.Колонки.Добавить("ИдентификаторЦифровогоСчета", Новый ОписаниеТипов("Строка"));
	Таблица.Колонки.Добавить("ЦифровойСчет", 			Новый ОписаниеТипов("Булево"));
	...
	Таблица.Колонки.Добавить("ВидОперации",               Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОперацийСчетаПокупателю"));
	
	Таблица.Колонки.Добавить("ТаблицаДокумента");
	
	Таблица.Колонки.Добавить("ДополнительныеРеквизитыQRКода");
	
	Таблица.Колонки.Добавить("ВариантПечатиQRКода",        Новый ОписаниеТипов("ПеречислениеСсылка.ВариантПечатиQRКода"));
...
Функция ПолучитьОписаниеТаблицыСчетаНаОплату() Экспорт
	...
	Таблица.Колонки.Добавить("Номенклатура", 					Новый ОписаниеТипов("СправочникСсылка.Номенклатура, СправочникСсылка.ОсновныеСредства"));
...
Функция ПечатьСчетаНаОплату(СведенияСчетаНаОплату, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		ЭтоПечатнаяФормаЗаПоставщика = УправлениеПечатьюБП.ЭтоПечатнаяФормаЗаПоставщика(СведенияОДокументе.Документ);
		ЕстьФаксимиле = Ложь;
		
		// Получим логотип организации
		ДвоичныеДанныеКартинки = Неопределено;
		Если ТипЗнч(СведенияОДокументе.Получатель) = Тип("СправочникСсылка.Организации") 
			И ОбщегоНазначения.СсылкаСуществует(СведенияОДокументе.Получатель) Тогда
			...
			ЕстьФаксимиле = НЕ ЭтоПечатнаяФормаЗаПоставщика 
				И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СведенияОДокументе.Получатель, "ФайлПечать"));
		КонецЕсли;
		...
		ЦифровойСчет = СведенияОДокументе.ЦифровойСчет;
		
		ПечатьQRВозможна = СведенияОДокументе.ВариантПечатиQRКода <> Перечисления.ВариантПечатиQRКода.НеПечатать
			И Не СчетВВалюте И Не ЦифровойСчет;
		
		Если Не ПечатьQRВозможна Тогда
			ДвоичныеДанныеQRКода = Неопределено;
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.ПлатежныеРеквизиты Тогда
			ДвоичныеДанныеQRКода = QRКодРеквизитыПлатежа(СведенияОДокументе, ДанныеПечатиЗаголовокСчета, ДанныеПечатиИтоговНДС);
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.ОнлайнЗаказы Тогда
			ДвоичныеДанныеQRКода = QRКодОнлайнЗаказы(СведенияОДокументе);
		ИначеЕсли СведенияОДокументе.ВариантПечатиQRКода = Перечисления.ВариантПечатиQRКода.СБП Тогда
			ПечатьQRКодаСБП = Ложь;
			Если ТипЗнч(СведенияОДокументе.БанковскийСчетПродавца) = Тип("СправочникСсылка.БанковскиеСчета")
				И ТипЗнч(СведенияОДокументе.Получатель) = Тип("СправочникСсылка.Организации")
				И ПереводыСБПc2b.ПереводыСБПДоступны() Тогда
				КонтрагентЮрлицоИлиИП = СведенияОПокупателе.КонтрагентИП
					Или СведенияОПокупателе.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо;
				ВидСБП = ?(КонтрагентЮрлицоИлиИП, "b2b", "c2b");
				СведенияОДокументе.НастройкаИнтеграцииСБП = ИнтеграцияССБПБППовтИсп.НастройкаИнтеграцииСБППоБИК(
					СведенияОДокументе.Получатель, СведенияОДокументе.БикБанкаПолучателя, ВидСБП);
				
				Если ЗначениеЗаполнено(СведенияОДокументе.НастройкаИнтеграцииСБП) Тогда
		...
		Если Макет.Области.Найти("QRКод") <> Неопределено Тогда
			...
			ИмяРисунка   = "ЛоготипСчета";
			
			Если ЕстьЛоготип Тогда
				ИмяЗаголовка = ИмяЗаголовка + "СЛоготипом";
			КонецЕсли;
			
			Если ЦифровойСчет Тогда
				ИмяЗаголовка = ИмяЗаголовка + "ЦифровойСчет";
				ИмяРисунка   = ИмяРисунка   + "ЦифровойСчет";
			КонецЕсли;
			...
			Если ЕстьЛоготип Тогда
				...
				Попытка
					ОбластьРеквизиты.Рисунки[ИмяРисунка].Картинка = Новый Картинка(ДвоичныеДанныеКартинки);
				Исключение
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Печать счета'"),
						УровеньЖурналаРегистрации.Ошибка,
						Метаданные.Документы.СчетНаОплатуПокупателю,
						СведенияОДокументе.Документ,
		...
		ЗаголовокСчета = ?(Не ПустаяСтрока(СведенияОДокументе.ЗаголовокСчета),
			СведенияОДокументе.ЗаголовокСчета,
			ПечатьТорговыхДокументовКлиентСервер.ЗаголовокСчетаНаОплатуПоУмолчанию());
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 № %2 от %3 г.'"),
			ЗаголовокСчета, НомерДокумента, Формат(СведенияОДокументе.ДатаДокумента, "ДФ='дд ММММ гггг'"));
		...
		Если ЕстьФаксимиле И НЕ ТабличныйДокумент.ПроверитьВывод(ОбластьМакета) Тогда
			ТабличныйДокумент.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если ЭтоПечатнаяФормаЗаПоставщика Тогда
			УправлениеПечатьюБП.ОчиститьФаксимильнуюПодписьИПечатьИзМакета(ТабличныйДокумент, ОбъектыПечати, СведенияОДокументе.Документ);
		Иначе
...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ИмяМакета) Экспорт
	...
	Если СведенияОПолучателе.ЦифровойСчет Тогда
		ДанныеПечатиЗаголовокСчета.Вставить("СчетПолучателяПредставление", СведенияОПолучателе.ИдентификаторЦифровогоСчета);
	Иначе
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='%1 № 1 от %2'"),
		ПечатьТорговыхДокументовКлиентСервер.ЗаголовокСчетаНаОплатуПоУмолчанию(), Формат(ТекущаяДатаСеанса(), "ДФ=dd.MM.yyyy"));
	ОбластьМакета.Параметры.Заполнить(Новый Структура("ТекстЗаголовка", ТекстЗаголовка));
	ТабличныйДокумент.Вывести(ОбластьМакета);
	...
	ПодписиИПечатиОрганизации = Справочники.Организации.ПодписиИПечатиОрганизации(Организация);
	
	ФаксимилеДляУстановки = Новый Структура;
	...
	Если ЭтоЮридическоеЛицо Тогда 
		...
		ФаксимилеДляУстановки.Вставить("ФаксимилеПечать", "ФаксимилеПечать");
		ФаксимилеДляУстановки.Вставить("ФаксимилеРуководитель", "ФаксимилеРуководитель");
		ФаксимилеДляУстановки.Вставить("ФаксимилеГлавныйБухгалтер", "ФаксимилеГлавныйБухгалтер");
		// Факсимильную печать можно смело использовать в одном ряду с печатью, подписью руководителя
		// и подписью главного бухгалтера. Т.к. Справочники.Организации.ПодписиИПечатиОрганизации 
		// возвращает ФаксимилеФаксимильнаяПечать только в том случае, если не заполнена ни одна подпись или печать.
		ФаксимилеДляУстановки.Вставить("ФаксимилеФаксимильнаяПечать", "ФаксимилеФаксимильнаяПечать");
		
	Иначе
		ОбластьМакета = Макет.ПолучитьОбласть("ПодвалСчетаПредприниматель");
		
		ФаксимилеДляУстановки.Вставить("ФаксимилеПечатьПредприниматель", "ФаксимилеПечать");
		ФаксимилеДляУстановки.Вставить("ФаксимилеПредприниматель", "ФаксимилеРуководитель");
		ФаксимилеДляУстановки.Вставить("ФаксимилеФаксимильнаяПечатьПредприниматель", "ФаксимилеФаксимильнаяПечать");
	...
	НомерФаксимиле = 1;
	Для Каждого ИмяФаксимиле Из ФаксимилеДляУстановки Цикл
		Если Не ПодписиИПечатиОрганизации.Свойство(ИмяФаксимиле.Значение) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ОбластьМакета.Области.Найти(ИмяФаксимиле.Ключ) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОбластьМакета.Рисунки[ИмяФаксимиле.Ключ].Картинка =
			ПодписиИПечатиОрганизации[ИмяФаксимиле.Значение];
		ОбластьМакета.Рисунки[ИмяФаксимиле.Ключ].ЦветФона = ЦветФонаБелый;
		ОбластьМакета.Рисунки[ИмяФаксимиле.Ключ].Линия    = ЛинияРамки;
		// Изменим имя картинки для того, чтобы печать БСП
		// не считала, что здесь есть факсимиле и не выводила кнопку факсимиле.
		ОбластьМакета.Рисунки[ИмяФаксимиле.Ключ].Имя = "ПредварительныйПросмотрКартинка_" + Формат(НомерФаксимиле, "ЧДЦ=0; ЧГ=0");
		
		НомерФаксимиле = НомерФаксимиле + 1;
		
	КонецЦикла;
	
	ТабличныйДокумент.Вывести(ОбластьМакета);
	
	ДокументПечатнойФормы = Документы.СчетНаОплатуПокупателю.ПустаяСсылка();
	
	// В табличном документе зададим имя области, в которую был 
	// выведен объект. Нужно для возможности печати покомплектно.
	УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, 
		1, ОбъектыПечати, ДокументПечатнойФормы);
	
	// Мы переименовали картинки факсимиле в том случае, если для факсимиле есть картинка.
	// Факсимиле, для которых нет картинок, остается имя с Факсимиле, поэтому мы их должны удалить.
	УправлениеПечатьюБП.ОчиститьФаксимильнуюПодписьИПечатьИзМакета(ТабличныйДокумент, ОбъектыПечати, ДокументПечатнойФормы);
...
Функция QRКодРеквизитыПлатежа(ДанныеДокумента, ДанныеПечатиЗаголовокСчета, ДанныеПечатиИтоговНДС)
	...
	// QR-код
	РеквизитыПлатежа = РеквизитыПлатежа(ДанныеДокумента, ДанныеПечатиЗаголовокСчета, ДанныеПечатиИтоговНДС);
	
	QRСтрока = QRСтрокаРеквизитыПлатежа(РеквизитыПлатежа);
...
Функция QRСтрокаРеквизитыПлатежа(РеквизитыПлатежа) Экспорт
	ОграничитьДлинуРеквизитовПлатежаДляQRКода(РеквизитыПлатежа);
	
	// QR-код
	Возврат УправлениеПечатьюРФ.ФорматнаяСтрокаУФЭБС(РеквизитыПлатежа);
КонецФункции 

Функция РеквизитыПлатежа(ДанныеДокумента, ДанныеПечатиЗаголовокСчета, ДанныеПечатиИтоговНДС) Экспорт
	...
	Если ДанныеДокумента.ДополнительныеРеквизитыQRКода <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(
			РеквизитыПлатежа, ДанныеДокумента.ДополнительныеРеквизитыQRКода, Истина);
	КонецЕсли;
	...
	СуммаКОплате = ДанныеДокумента.ТаблицаДокумента.Итог("Сумма") - СкидкиПоДокументу;
	Если НЕ ДанныеДокумента.СуммаВключаетНДС Тогда
		СуммаКОплате = СуммаКОплате + ДанныеДокумента.ТаблицаДокумента.Итог("СуммаНДС");
	КонецЕсли; 
...
Функция QRКодОнлайнЗаказы(ДанныеДокумента)
	НастройкиПодключения = ОнлайнЗаказы.НастройкиПодключения(ДанныеДокумента.Организация);
	Если НастройкиПодключения.НастройкиПодключения.Количество() = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Для формирования QR кода со ссылкой на онлайн заказ, необходимо настроить подключение к сервису.'"));
		Возврат Неопределено;
	КонецЕсли;

	// QR-код
	РезультатПолучения = ОнлайнЗаказы.URLЗаказа(ДанныеДокумента.Документ, НастройкиПодключения.НастройкиПодключения[0]);
	QRСтрока = РезультатПолучения.URLЗаказа;
	Если ПустаяСтрока(QRСтрока) Тогда
		Возврат Неопределено; // Возникли ошибки
	КонецЕсли;
...
Функция ПечатьАктаНаПередачуПрав(СведенияАктаНаПередачуПрав, ОбъектыПечати, СтруктураПараметровПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияАктаНаПередачуПрав Цикл
		...
		СписокЛицензиар  = "НаименованиеДляПечатныхФорм,ИНН,Свидетельство,ЮридическийАдрес,Телефоны,НомерСчета,ИдентификаторЦифровогоСчета,Банк,БИК,КоррСчет";
		СписокЛицензиат  = "НаименованиеДляПечатныхФорм,ИНН,Свидетельство,ЮридическийАдрес,Телефоны,НомерСчета,ИдентификаторЦифровогоСчета,Банк,БИК,КоррСчет";
		СписокПлательщик = "НаименованиеДляПечатныхФорм,ИНН,Свидетельство,ЮридическийАдрес,Телефоны,НомерСчета,ИдентификаторЦифровогоСчета,Банк,БИК,КоррСчет";
...
Функция ДанныеПечатиИтоговНДС(СуммаНДС, СтавкиНДС, СуммаВключаетНДС, Разделитель = "") Экспорт
	...
	Если СуммаНДС <> 0 Тогда
		...
		Если СтавкиНДС.Количество() = 1 Тогда
			СтавкаНДССтрокой = СтрШаблон(" %1%%",УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтавкиНДС[0]));
...
Процедура ОграничитьДлинуРеквизитовПлатежаДляQRКода(РеквизитыПлатежа) 
	
	ОграниченияСтрок = Новый Соответствие;
	ОграниченияСтрок.Вставить("ТекстПолучателя", 160);
	ОграниченияСтрок.Вставить("НаименованиеБанкаПолучателя", 45);
	
	Для Каждого Ограничение Из ОграниченияСтрок Цикл
		Если РеквизитыПлатежа.Свойство(Ограничение.Ключ) Тогда
			РеквизитыПлатежа[Ограничение.Ключ] = Лев(РеквизитыПлатежа[Ограничение.Ключ], Ограничение.Значение);
		КонецЕсли;
	КонецЦикла;
