...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
    Методы.Вставить("УстановитьПараметрСеансаТекущийСеансДокументооборотаСКО", Истина);
    Методы.Вставить("ПеренестиТранспортныеСообщенияВПрисоединенныеФайлы", Истина);
    Методы.Вставить("УстановитьКонстантуНачалаРаботыО1СО");
    Методы.Вставить("ВыполнитьОбмен", Истина);
    Методы.Вставить("ПереносЗаявленийГосРегистрацииВЕГРЮЛ");
    Методы.Вставить("ОбъединитьВTIFСканы", Истина);
КонецПроцедуры
...
Функция СоздатьТекстИнформацииБРО(ПараметрыОтображения, Тариф) Экспорт
	...
	Если Создать Тогда
		...
		Если Тариф = Перечисления.ТарифыОператораЭДО.КадровыеРешения Тогда
			Если ЭтоСЭДО Тогда
				Текст = НСтр("ru = 'Подключите обмен с СФР бесплатно на 3 месяца! Попробуйте Социальный документооборот и другие возможности.'");
			Иначе
				Текст = НСтр("ru = 'Подключите обмен с СФР бесплатно на 3 месяца!'");
			КонецЕсли;
