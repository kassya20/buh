...
//  Отказ  - <Булево> - флаг отказа от проведения
//
Функция ПодготовитьТаблицуВзаиморасчетовЗачетАвансовРасчетыСКомиссионером(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Параметры = ПодготовитьПараметрыТаблицыВзаиморасчетовЗачетАвансовРасчетыСКомиссионером(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	
	// Ддя расчетов с комиссионером добавим отдельную строку по комиссионному вознаграждению
	ТаблицаДокумента = Параметры.ТаблицаДокумента.СкопироватьКолонки();
	Для каждого СтрокаТаблицы Из Параметры.ТаблицаДокумента Цикл
		...
		СтрокаКомиссионер = ТаблицаДокумента.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаКомиссионер, СтрокаТаблицы);
		СтрокаКомиссионер.СчетАвансов = Реквизиты.СчетРасчетовСКомиссионером;
	КонецЦикла; 

	ОстаткиАвансов = ПолучитьОстаткиАвансов(ТаблицаДокумента, Реквизиты, Отказ);
	...
	ТаблицаДокумента = Параметры.ТаблицаДокумента.СкопироватьКолонки();
	НераспределенныеСтроки = ТаблицаВзаиморасчетов.НайтиСтроки(Новый Структура("ДокументРасчетов", Реквизиты.Регистратор));
	Для каждого СтрокаНераспределеннойЗадолженности Из НераспределенныеСтроки Цикл
		СтрокаДокумента = ТаблицаДокумента.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДокумента, СтрокаНераспределеннойЗадолженности);
		СтрокаДокумента.СчетАвансов = Реквизиты.СчетРасчетовСКомиссионером;
		ТаблицаВзаиморасчетов.Удалить(СтрокаНераспределеннойЗадолженности);
	КонецЦикла; 
	
	ТаблицаВзаиморасчетовВрем = ПустаяТаблицаВзаиморасчетовЗачетАвансов();
	ЗаполнитьТаблицуВзаиморасчетовЗачетАвансов(
		ТаблицаДокумента, 
		Параметры.ДокументыАвансов, 
		Реквизиты, 
		ТаблицаВзаиморасчетовВрем, 
		ОстаткиАвансов, 
		Ложь,
		Отказ);
		
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВзаиморасчетовВрем, ТаблицаВзаиморасчетов);
				...
				Если НЕ ЭтоОтложенноеПроведение И СтрокаДокумента.ВалютаВзаиморасчетов = ВалютаРегламентированногоУчета Тогда
					...
					Для Каждого СтрокаТаблицыАвансов Из ДокументыАвансов Цикл
						...
						Если Реквизиты.УчитыватьЗадолженностьУСН ИЛИ Реквизиты.УчитыватьЗадолженностьУСНПатент Тогда
							СтрокаОстатка.СуммаВзаиморасчетовКомитента = 0;
							СтрокаОстатка.СуммаВзаиморасчетовТорговыйСбор = 0;
							СтрокаОстатка.СуммаВзаиморасчетовПатент = 0;
							СтрокаОстатка.СуммаВзаиморасчетовЕНВД = 0;
						КонецЕсли;
...
Процедура СформироватьДвиженияЗачетАвансов(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаВзаиморасчетов Из Параметры.Взаиморасчеты Цикл
		...
		Если Не ЭтоСчетАвансовПоОтгрузке(СтрокаВзаиморасчетов.СчетРасчетов) Тогда
			СформироватьПроводкуПоСчетамУСНЗачетАвансов(СтрокаВзаиморасчетов, Реквизиты, ДтКтУСН, ЗнакСуммУСН, Движения, Отказ);
		ИначеЕсли (СтрокаВзаиморасчетов.ДокументРасчетов <> Реквизиты.Регистратор) И (Не ЭтоВозвратОтПокупателя) Тогда
			СформироватьПроводкуПоСчетамУСНПереквалификацияАвансаПоОтгрузке(
				СтрокаВзаиморасчетов, Реквизиты, Движения, Отказ);
		КонецЕсли;
...
// Формируются проводки по специальным счетам УСН.
// При реализации функционала по отгрузке без перехода права собственности.
Процедура СформироватьПроводкуПоСчетамУСНПереквалификацияАвансаПоОтгрузке(СтрокаВзаиморасчетов, Реквизиты, Движения, Отказ)
	...
	Если ФормироватьПроводкиУСН 
		...
		Если СтрокаВзаиморасчетов.РасчетыВВалюте Тогда
			...
			Проводка.ВалютнаяСуммаКт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовЕНВД;
		Иначе
		...
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "ДокументыРасчетовСКонтрагентами", ДокументРасчетовВПроводку);
		
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Контрагенты", СтрокаВзаиморасчетов.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Договоры", СтрокаВзаиморасчетов.ДоговорКонтрагента);
		...
		Если СвойстваСчета.УчетПоПодразделениям Тогда
			...
			Проводка.ПодразделениеКт = СтрокаВзаиморасчетов.ПодразделениеРасчетов;
		КонецЕсли;
	...
	// Проводка по вспомогательному счету УСН по задолженности ПСН
	...
	Если ФормироватьПроводкиУСНПатент 
		...
		Если СтрокаВзаиморасчетов.РасчетыВВалюте Тогда
			...
			Проводка.ВалютаДт        = СтрокаВзаиморасчетов.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаДт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовПатент;
			
			Проводка.СчетКт          = ПланыСчетов.Хозрасчетный.УСНРасчетыСПокупателямиВалПатент;
			Проводка.ВалютаКт        = СтрокаВзаиморасчетов.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаКт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовПатент;
		Иначе
		...
		Проводка.Сумма = СтрокаВзаиморасчетов.СуммаРуб_Патент;
	...
	Если ФормироватьПроводкиУСН 
		...
		Если СтрокаВзаиморасчетов.РасчетыВВалюте Тогда
			...
			Проводка.ВалютаКт        = СтрокаВзаиморасчетов.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаКт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовКомитента;
		Иначе
	...
	Если ФормироватьПроводкиУСН
		...
		Если СтрокаВзаиморасчетов.РасчетыВВалюте Тогда
			Проводка.СчетДт          = ПланыСчетов.Хозрасчетный.УСНРасчетыСПокупателямиВалТорговыйСбор;
			Проводка.ВалютаДт        = СтрокаВзаиморасчетов.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаДт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовТорговыйСбор;
			
			Проводка.СчетКт          = ПланыСчетов.Хозрасчетный.УСНРасчетыСПокупателямиВалТорговыйСбор;
			Проводка.ВалютаКт        = СтрокаВзаиморасчетов.ВалютаВзаиморасчетов;
			Проводка.ВалютнаяСуммаКт = СтрокаВзаиморасчетов.СуммаВзаиморасчетовТорговыйСбор;
		Иначе
		...
		Проводка.Сумма = СтрокаВзаиморасчетов.СуммаРуб_ТорговыйСбор;
...
// Возвращает таблицу с данными о незакрытых переплатах от эквайера.
// 
// Параметры:
//  ТаблицаИсточник - ТаблицаЗначений - Таблица с данными для отбора переплат
//  ВидыОтборов - Структура - Структура, определяющая отбор
// * В ключах - имена колонок источника, в значениях - имена соответствующих измерений регистра "Прочие расчеты", по которым необходимо отбирать переплаты
//  Реквизиты - СтрокаТаблицыЗначений - Реквизиты документа, на момент которого получаются остатки
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Переплаты от эквайера см.ПолучитьОстаткиПрочихРасчетов
Функция ПереплатыОтЭквайера(ТаблицаИсточник, ВидыОтборов, Реквизиты) Экспорт
	
	ПереплатыОтЭквайера = ПолучитьОстаткиПрочихРасчетов(ТаблицаИсточник, ВидыОтборов, Реквизиты, Истина);
	
	// Переплатами считаем поступления по платежным картам, по которым сумма остатка меньше 0.
	// Также считаем переплатами возвраты по эквайрингу, превышающие выручку по эквайрингу.
	СтрокиДляУдаления = Новый Массив;
	Для Каждого СтрокаПереплаты Из ПереплатыОтЭквайера Цикл
		Если СтрокаПереплаты.СуммаОстаток < 0
			И (ТипЗнч(СтрокаПереплаты.РасчетныйДокумент) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет")
			Или ТипЗнч(СтрокаПереплаты.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах")) Тогда
			СтрокаПереплаты.СуммаОстаток = - СтрокаПереплаты.СуммаОстаток;
		Иначе
			СтрокиДляУдаления.Добавить(СтрокаПереплаты);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаДляУдаления Из СтрокиДляУдаления Цикл
		ПереплатыОтЭквайера.Удалить(СтрокаДляУдаления);
	КонецЦикла;
	
	Возврат ПереплатыОтЭквайера;
	...
	Для Каждого СтрокаРеализации Из Параметры.ТаблицаРеализация Цикл
		...
		Для Каждого СтрокаВзаиморасчетов Из ВзаиморасчетыПоДоговору Цикл
			...
			Для Каждого НезачисленныйАванс Из НезачисленныеАвансыПоДокументу Цикл
				...
				// При совмещении ИП ОСН и ПСН "Прочие расчеты" формируются при подготовке таблиц ИП,
				// кроме документа "Оказание услуг"
				Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.ОказаниеУслуг") Или Не ПлательщикНДФЛ Тогда
...
// Конструктор таблицы проводок по счетам вспомогательных расчетов.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Пустая таблица проводок:
// * Сумма - Число - Сумма проводки
// * Содержание - Строка - Содержание проводки
// * СчетДт - ПланСчетовСсылка.Хозрасчетный - Счет Дт проводки
// * ПодразделениеДт - СправочникСсылка.ПодразделенияОрганизаций - Подразделение в Дт проводки
// * КонтрагентДт - СправочникСсылка.Контрагенты - Контрагент в Дт проводки
// * ДоговорКонтрагентаДт - СправочникСсылка.ДоговорыКонтрагентов - Договор с контрагентом в Дт проводки
// * ДокументРасчетовДт - ДокументСсылка - Документ расчетов с контрагентом в Дт проводки
// * СчетКт - ПланСчетовСсылка.Хозрасчетный - Счет Кт проводки
// * ПодразделениеКт - СправочникСсылка.ПодразделенияОрганизаций - Подразделение в Кт проводки
// * КонтрагентКт - СправочникСсылка.Контрагенты - Контрагент в Кт проводки
// * ДоговорКонтрагентаКт - СправочникСсылка.ДоговорыКонтрагентов - Договор с контрагентом в Кт проводки
// * ДокументРасчетовКт - ДокументСсылка - Документ расчетов с контрагентом в Кт проводки
Функция НоваяТаблицаПроводокВспомогательныеРасчеты() Экспорт
...
Процедура СформироватьДвиженияРасчетыПоЭквайрингуЗачетАвансов(ТаблицаПрочихРасчетов, ТаблицаПроводок, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаРеквизиты)
		Или (Не ЗначениеЗаполнено(ТаблицаПрочихРасчетов) И Не ЗначениеЗаполнено(ТаблицаПроводок)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьДвиженияПрочиеРасчетыПоЭквайрингуПогашениеЗадолженности(Взаиморасчеты, Реквизиты, Движения)
	...
	Если Реквизиты.ЭтоВозврат Тогда
		ДобавитьДвиженияПрочиеРасчетыПоЭквайрингуПогашениеЗадолженностиВозврат(Взаиморасчеты, Реквизиты, Движения);
	Иначе
		ДобавитьДвиженияПрочиеРасчетыПоЭквайрингуПогашениеЗадолженностиОплата(Взаиморасчеты, Реквизиты, Движения);
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьДвиженияПрочиеРасчетыПоЭквайрингуПогашениеЗадолженностиОплата(Взаиморасчеты, Реквизиты, Движения)
	...
	СтруктураКУДиР = НоваяСтруктураКУДиРУСН();
	...
	ИменаОтборов = Новый Структура("Контрагент, ДоговорКонтрагента", "Контрагент", "ДоговорКонтрагента");
	ПереплатыОтЭквайера = ПереплатыОтЭквайера(ТаблицаОтбор, ИменаОтборов, Реквизиты);
	...
	ОстаткиПоСчетамУСН = ОстаткиРасчетовПоСчетамУСН(ПереплатыОтЭквайера, СчетаРасчетовУСН, Реквизиты, Истина);
	
	ПереплатыОтЭквайера.Индексы.Добавить("Контрагент, ДоговорКонтрагента, СчетУчета");
	ОстаткиПоСчетамУСН.Индексы.Добавить("Контрагент, Договор, РасчетныйДокумент");
	
	ОтборПереплат = Новый Структура("Контрагент, ДоговорКонтрагента, СчетУчета");
	ОтборОстатковПоСчетамУСН = Новый Структура("Контрагент, Договор, РасчетныйДокумент");
	
	ОтборПереплат.Контрагент = Реквизиты.Эквайер;
	ОтборПереплат.ДоговорКонтрагента = Реквизиты.ДоговорЭквайринга;
	
	Для Каждого СтрокаВзаиморасчетов Из Взаиморасчеты Цикл
		
		СуммаОплаты = СтрокаВзаиморасчетов.СуммаБУ;
		
		ОтборПереплат.СчетУчета = СтрокаВзаиморасчетов.КорСчет;
		
		НайденныеСтрокиПереплат = ПереплатыОтЭквайера.НайтиСтроки(ОтборПереплат);
		
		Для Каждого СтрокаПереплаты Из НайденныеСтрокиПереплат Цикл
			...
			РаспределеннаяСумма = Мин(СуммаОплаты, СуммаПереплаты);
			
			НоваяСтрока = Движения.ПрочиеРасчеты.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Реквизиты);
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПереплаты);
			...
			Если ОстаткиСчетовУСНПоДокументу.Количество() > 0 Тогда
				...
				КоэффициентСписанияПереплаты = РаспределеннаяСумма / СуммаПереплаты;
				...
				Для Каждого ОстатокПоСчетамУСН Из ОстаткиСчетовУСНПоДокументу Цикл
					...
					ДобавитьПроводкиВспомогательныеРасчетыПоЭквайрингуЗачетПереплаты(
						СтрокаВзаиморасчетов, ОстатокПоСчетамУСН, СуммаПоСчетамУСН, СчетаУСН, Реквизиты, Движения);
			...
			Если СтрокаВзаиморасчетов.СуммаБУ_Патент > 0 Тогда
				...
				СуммаКорректировкиДохода = ?(СтрокаВзаиморасчетов.СуммаБУ = 0, 0,
					Окр(РаспределеннаяСумма * СтрокаВзаиморасчетов.СуммаБУ_Патент / СтрокаВзаиморасчетов.СуммаБУ, 2, 1));
				...
				Если СуммаКорректировкиДохода > 0 Тогда
					...
					СтруктураКУДиР.Графа5 = СтруктураКУДиР.Графа5 - СуммаКорректировкиДохода;
					СтруктураКУДиР.ДоходЕНВД = СтруктураКУДиР.ДоходЕНВД + СуммаКорректировкиДохода;
					
					СодержаниеОперации = СтрШаблон(
						НСтр("ru = 'Аванс на сумму %1 руб. отнесен к деятельности на патенте.'", КодЯзыка),
						ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаКорректировкиДохода, , , " "));
					
					СтруктураКУДиР.Пояснение.Добавить(СодержаниеОперации);
					
					Проводка = Движения.Хозрасчетный.Добавить();
					Проводка.Период      = Реквизиты.Период;
					Проводка.Организация = Реквизиты.Организация;
					Проводка.Содержание = НСтр("ru = 'Расчеты по деятельности на патенте'", КодЯзыка);
					Проводка.СчетКт = ПланыСчетов.Хозрасчетный.УСНРасчетыСПокупателямиПатент;

					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,
						Проводка.СубконтоКт, "Контрагенты", Реквизиты.Эквайер);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,
						Проводка.СубконтоКт, "Договоры", Реквизиты.ДоговорЭквайринга);
					БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт,
						Проводка.СубконтоКт, "ДокументыРасчетовСКонтрагентами", СтрокаВзаиморасчетов.ДокументРасчетов);
					...
					Проводка.Сумма = СуммаКорректировкиДохода;
					
				КонецЕсли;
				
			КонецЕсли;
			
			Если СтрокаВзаиморасчетов.СуммаБУ_Патент < СтрокаВзаиморасчетов.СуммаБУ И ДоходыПоСпецРежимам > 0 Тогда
				
				СуммаКорректировкиДохода = ?(СтрокаВзаиморасчетов.СуммаБУ = 0, 0,
					Окр(РаспределеннаяСумма * (СтрокаВзаиморасчетов.СуммаБУ - СтрокаВзаиморасчетов.СуммаБУ_Патент) / СтрокаВзаиморасчетов.СуммаБУ, 2, 1));

				ЗачтеноНеПризнанныеДоходы = Мин(СуммаКорректировкиДохода, ДоходыПоСпецРежимам);
				СуммаКорректировкиДохода = ЗачтеноНеПризнанныеДоходы;
				ДоходыПоСпецРежимам = ДоходыПоСпецРежимам - ЗачтеноНеПризнанныеДоходы;
				...
				СодержаниеОперации = СтрШаблон(
					НСтр("ru = 'Аванс на сумму %1 руб. отнесен к деятельности УСН.'", КодЯзыка),
					ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаКорректировкиДохода, , , " "));
						...
						Если СтрокаВзаиморасчетов.СуммаБУ_Патент > 0 Тогда
							...
							Если ЗначениеЗаполнено(ПатентРеализации) И (ПатентРеализации <> ПатентАванса) Тогда
								...
								ЗаписьКУДиРПатент.Содержание =  НалоговыйУчетУСН.СодержаниеОперацииКУДПоПатентуОплатаОтПокупателя(
									СтрокаВзаиморасчетов.Контрагент, СтрокаВзаиморасчетов.ДоговорКонтрагента, Реквизиты.Период, КодЯзыка);
								ЗаписьКУДиРПатент.Графа4 = ?(СтрокаВзаиморасчетов.СуммаБУ = 0, 0,
									Окр(РаспределеннаяСумма * СтрокаВзаиморасчетов.СуммаБУ_Патент / СтрокаВзаиморасчетов.СуммаБУ, 2, 1));
								ЗаписьКУДиРПатент.Контрагент = СтрокаВзаиморасчетов.Контрагент;
								ЗаписьКУДиРПатент.ДоговорКонтрагента = СтрокаВзаиморасчетов.ДоговорКонтрагента;
			...
			СуммаОплаты = СуммаОплаты - РаспределеннаяСумма;
			
			СтрокаПереплаты.СуммаОстаток = СтрокаПереплаты.СуммаОстаток - РаспределеннаяСумма;
			
		КонецЦикла;
		
		Если СуммаОплаты > 0 И ОграничениеТипаДокументов.СодержитТип(ТипЗнч(СтрокаВзаиморасчетов.ДокументРасчетов)) Тогда
			НоваяСтрока = Движения.ПрочиеРасчеты.Добавить();
	...
	Если Реквизиты.УчитыватьЗадолженностьУСН И (СтруктураКУДиР.Графа5 <> 0 Или СтруктураКУДиР.ДоходЕНВД <> 0) Тогда
		
		ЗаписьКУДиР = Движения.КнигаУчетаДоходовИРасходов.Добавить();
		ЗаписьКУДиР.Период = Реквизиты.Период;
		ЗаписьКУДиР.Регистратор = Реквизиты.Регистратор;
		...
		ЗаписьКУДиР.Графа5 = СтруктураКУДиР.Графа5;
		ЗаписьКУДиР.ДоходЕНВД = СтруктураКУДиР.ДоходЕНВД;
		ЗаписьКУДиР.ДоходТорговыйСбор = СтруктураКУДиР.ДоходТорговыйСбор;
		
		ЗаписьКУДиР.Содержание = СтрШаблон(
			НСтр("ru = 'Получена оплата от покупателя по платежной карте. %1'", КодЯзыка),
			СтрСоединить(СтруктураКУДиР.Пояснение, " "));
		
	КонецЕсли;
	
	Если Реквизиты.УчитыватьЗадолженностьУСНПатент И ДвиженияКУДиРПатент.Количество() > 0 Тогда
		
		УчетПСН.СформироватьДвиженияКнигаУчетаДоходовПатент(ДвиженияКУДиРПатент, Движения, Ложь);
...
Процедура ДобавитьДвиженияПрочиеРасчетыПоЭквайрингуПогашениеЗадолженностиВозврат(Взаиморасчеты, Реквизиты, Движения)
...
Функция ПодготовитьТаблицуВзаиморасчетовПогашениеЗадолженности(РасшифровкаПлатежа, ТаблицаРеквизиты, Отказ, ВидДокумента = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(РасшифровкаПлатежа) ИЛИ Не ЗначениеЗаполнено(ТаблицаРеквизиты) Тогда
		Возврат ТаблицаВзаиморасчетов;
	КонецЕсли;
...
Функция ПодготовитьТаблицуВзаиморасчетовБезналичнаяРозничнаяВыручка(ТаблицаБезналичныхОплат, ТаблицаРеквизиты, Отказ) Экспорт
	...
	// дополним остатки авансов данными по излишне перечисленным деньгам от эквайера
	ТаблицаОплат.Индексы.Добавить("ТипОплаты");
	Отбор = Новый Структура("ТипОплаты", Перечисления.ТипыОплат.ПлатежнаяКарта);
	АналитикаАвансовПоПлатежнымКартам = ТаблицаОплат.Скопировать(Отбор);
	
	ОстаткиАвансовПоПлатежнымКартам = ОстаткиАвансовПоПлатежнымКартам(
		АналитикаАвансовПоПлатежнымКартам, Реквизиты, Отказ);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ОстаткиАвансовПоПлатежнымКартам, ОстаткиАвансов);
...
Процедура ЗаполнитьТаблицуВзаиморасчетовБезналичнаяРозничнаяВыручка(ТаблицаОплат, Реквизиты, ТаблицаВзаиморасчетов, ОстаткиАвансов, Отказ) Экспорт
	...
	Для каждого СтрокаДокумента Из ТаблицаОплат Цикл
		...
		Если СтрокаДокумента.СпособЗачетаАвансов = Перечисления.СпособыЗачетаАвансов.Автоматически
			Или СтрокаДокумента.ТипОплаты = Перечисления.ТипыОплат.ПлатежнаяКарта Тогда
...
Функция ПодготовитьТаблицуВзаиморасчетовПереносЗадолженности(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаВзаиморасчетов)
	 Или Не ЗначениеЗаполнено(ТаблицаРеквизиты) Тогда
		Возврат ПустаяТаблицаВзаиморасчетовПереносЗадолженности(Ложь);
	КонецЕсли;
...
// Возвращает таблицу остатков по счетам УСН.хх для определения расчетов с особым налогообложением.
// 
// Параметры:
//  ТаблицаИсточник - ТаблицаЗначений - Таблица с данными для отбора остатков:
//   * Контрагент - СправочникСсылка.Контрагенты - Контрагент, по которому выполняется отбор
//   * ДоговорКонтрагента - СправочникСсылка.ДоговорыКонтрагентов - Договор, по которому выполняется отбор
//   * РасчетныйДокумент - ДокументСсылка - Расчетный документ, по которому выполняется отбор
//  СчетаРасчетовУСН - Массив из ПланСчетовСсылка.Хозрасчетный - Счета вспомогательных расчетов УСН
//  Реквизиты - СтрокаТаблицыЗначений - Реквизиты документа, на момент которого получаются остатки:
//   * Период - Дата - Дата документа
//   * Организация - СправочникСсылка.Организации - Организация документа
//   * Регистратор - ДокументСсылка - Документ, на момент которого получаются остатки
//  УстанавливатьБлокировку - Булево - Если Истина, то необходимо установить управляемую блокировку таблицы регистра
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Остатки расчетов по вспомогательным счетам УСН:
//   * Контрагент - СправочникСсылка.Контрагенты
...
Функция ОстаткиРасчетовПоСчетамУСН(ТаблицаИсточник, СчетаРасчетовУСН, Реквизиты, УстанавливатьБлокировку = Ложь) Экспорт
	
	Если УстанавливатьБлокировку Тогда
		
		Блокировка = Новый БлокировкаДанных;
		
		ТаблицаИсточникДанных = ТаблицаИсточник.Скопировать( , "Контрагент, ДоговорКонтрагента");
		ТаблицаИсточникДанных.Свернуть("Контрагент, ДоговорКонтрагента");

		Для Каждого СубсчетУСН Из СчетаРасчетовУСН Цикл
	...
	Запрос.УстановитьПараметр("Контрагенты",
		ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточник, "Контрагент", Истина));
	Запрос.УстановитьПараметр("ДоговораКонтрагента",
		ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточник, "ДоговорКонтрагента", Истина));
	Запрос.УстановитьПараметр("РасчетныеДокументы",
		ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточник, "РасчетныйДокумент", Истина));
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&МоментВремениОстатков, Счет В (&СчетаУСН), &ВидыСубконто,
	|		Организация = &Организация
	|	И Субконто1 В (&Контрагенты)
	|	И Субконто2 В (&ДоговораКонтрагента)
	|	И Субконто3 В (&РасчетныеДокументы)) КАК ХозрасчетныйОстатки";
...
Функция ПодготовитьПараметрыТаблицыВзаиморасчетовЗачетАвансов(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты) Экспорт
	...
	Если ТаблицаРеквизиты.Колонки.Найти("РасчетКорректировокСУчетомРекомендацийПоНДС")<> Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок
		+ ", РасчетКорректировокСУчетомРекомендацийПоНДС,"     // <Булево> - Истина при расчете валютных сумм с учетом рекомендаций по письму ФНС
...
Функция ПодготовитьПараметрыТаблицыВзаиморасчетовЗачетАвансовРасчетыСКомиссионером(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты) Экспорт
	...
	+ "ВалютаДокумента,"           // <СправочникСсылка.Валюты>
	+ "СпособЗачетаАвансов,"       // <ПеречислениеСсылка.СпособыЗачетаАвансов>
	+ "СчетРасчетовСКомиссионером," // <ПланСчетовСсылка.Хозрасчетный> - счет учета расчетов с комиссионером
	+ "УчитыватьЗадолженностьУСН," // <Булево> - Истина для расчетов с покупателями по организации, применяющей УСН
	+ "УчитыватьЗадолженностьУСНПатент," // <Булево> - Истина для расчетов с покупателями по организации, применяющей УСН патент
	+ "НаправлениеДвижения,"       // <Строка> - "Поступление" при зачете авансов, выданных поставщикам (при поступлении материальных ценностей)
	                               //          - "Выбытие" при зачете авансов, полученных от покупателей (при выбытии материальных ценностей)
...
Функция ПолучитьОстаткиАвансов(ТаблицаДокумента, Реквизиты, Отказ)
	...
	Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ОперацияСБилетом") Тогда
		...
		Документы.ОперацияСБилетом.УпорядочитьТаблицуВзаиморасчетов(ОстаткиАвансов, Реквизиты.Регистратор);
		
	Иначе
		// Если по документу не было данных в ДанныеПервичныхДокументов и дата пустая, то возьмем ее напрямую из документа.
		ОбщегоНазначенияБПВызовСервера.ЗаполнитьДатыВТаблице(ОстаткиАвансов, "ДокументРасчетов", "ДатаДокументаРасчетов");
...
Функция ТекстЗапросаОстаткиАвансов(УчитыватьЗадолженностьУСН)
	...
	Если УчитыватьЗадолженностьУСН Тогда
		...
		|			КОГДА Остатки.Счет.Забалансовый
		|				ТОГДА 0
		|			ИНАЧЕ Остатки.СуммаНУОстаток * &Множитель
		|		КОНЕЦ) КАК СуммаНУ,
		|	СУММА(ВЫБОР
		...
		|			И Остатки.Субконто3 = ДанныеПервичныхДокументов.Документ
		|ГДЕ
		|	ВЫБОР
		|			КОГДА Остатки.Счет.Валютный
	...
	Иначе
		...
		|			ТОГДА Остатки.ВалютнаяСуммаОстаток
		|		ИНАЧЕ Остатки.СуммаОстаток
		|	КОНЕЦ * &Множитель КАК СуммаВзаиморасчетов,
...
Процедура ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, ОстаткиАвансов, СтрокаДокумента, Реквизиты)
	...
	Если Реквизиты.Владелец().Колонки.Найти("РасчетКорректировокСУчетомРекомендацийПоНДС") = Неопределено Тогда 
		РасчетКорректировокСУчетомРекомендацийПоНДС = Ложь;
	Иначе
		РасчетКорректировокСУчетомРекомендацийПоНДС = Реквизиты.РасчетКорректировокСУчетомРекомендацийПоНДС;
	КонецЕсли;
	
	Отбор = Новый Структура("ДоговорКонтрагента", СтрокаДокумента.ДоговорКонтрагента);
	ВзаиморасчетыПоДоговору = ТаблицаВзаиморасчетов.Скопировать(Отбор);
	
	РаспределеннаяСуммаРуб = ВзаиморасчетыПоДоговору.Итог("СуммаРуб");
	РаспределеннаяСуммаВал = ВзаиморасчетыПоДоговору.Итог("СуммаВзаиморасчетов");
	НераспределеннаяСуммаРуб = СтрокаДокумента.СуммаРуб - РаспределеннаяСуммаРуб;
	НераспределеннаяСуммаВал = СтрокаДокумента.СуммаВзаиморасчетов - РаспределеннаяСуммаВал;
	...
	Для каждого СтрокаОстатка Из ОстаткиАвансов Цикл
		...
		Если РасчетКорректировокСУчетомРекомендацийПоНДС
		...
		Если СтрокаОстатка.СуммаВзаиморасчетов = ЗачитываемыйАванс 
			И ТипЗнч(Реквизиты.Регистратор) <> Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			СуммаРубПоКурсуАванса = СтрокаОстатка.СуммаБУ;
		Иначе
			СуммаРубПоКурсуАванса = Окр(ЗачитываемыйАванс * КоэффициентКурсаАванса, 2);
		КонецЕсли;
		
		СуммаРуб              = Окр(ЗачитываемыйАванс * КоэффициентРуб, 2);
		
		ЗачетПоТекущемуКурсу = СтрокаДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомитентомНаЗакупку
			И Не ЭтоСчетРасчетовСПокупателями(СтрокаДокумента.СчетАвансов)
			Или СтрокаДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионеромНаЗакупку
			И Не ЭтоСчетРасчетовСПоставщиками(СтрокаДокумента.СчетАвансов);
		Если ЗачетПоТекущемуКурсу Тогда
			СуммаРубПоТекущемуКурсу = Окр(ЗачитываемыйАванс * Окр(КоэффициентРуб, 4), 2);
			СтрокаВзаиморасчетов.СуммаРубПоКурсуАванса = СуммаРубПоТекущемуКурсу;
		Иначе
			СтрокаВзаиморасчетов.СуммаРубПоКурсуАванса = СуммаРубПоКурсуАванса;
		КонецЕсли;
		...
		Если ПроведениеСервер.ИспользуетсяОтложенноеПроведение(Реквизиты.Организация, Реквизиты.Период) Тогда
			СтрокаВзаиморасчетов.СуммаРуб = СуммаРуб;
		Иначе
			НераспределеннаяСуммаВал = НераспределеннаяСуммаВал - ЗачитываемыйАванс;
			Если НераспределеннаяСуммаВал = 0 Тогда
				СтрокаВзаиморасчетов.СуммаРуб = НераспределеннаяСуммаРуб;
			Иначе
				СтрокаВзаиморасчетов.СуммаРуб = Мин(СуммаРуб, НераспределеннаяСуммаРуб);
			КонецЕсли;
			НераспределеннаяСуммаРуб = НераспределеннаяСуммаРуб - СтрокаВзаиморасчетов.СуммаРуб;
		КонецЕсли;
		
		СтрокаОстатка.СуммаВзаиморасчетов = СтрокаОстатка.СуммаВзаиморасчетов - СтрокаВзаиморасчетов.СуммаВзаиморасчетов;
		СтрокаОстатка.СуммаБУ             = СтрокаОстатка.СуммаБУ - СуммаРубПоКурсуАванса;
...
Процедура ДобавитьСтрокуЗадолженности(ТаблицаВзаиморасчетов, НераспределеннаяСумма, СтрокаДокумента, Реквизиты)
	...
	Если РасчетКорректировокСУчетомРекомендацийПоНДС И НЕ Реквизиты.ЭтоВозврат Тогда
		КоэффициентРуб = Реквизиты.КурсВзаиморасчетов / ?(Реквизиты.КратностьВзаиморасчетов = 0,
			1, Реквизиты.КратностьВзаиморасчетов);
	Иначе
		КоэффициентРуб = ?(НЕ СтрокаДокумента.РасчетыВВалюте, 1,
...
Функция СодержаниеПроводокПоЗачетуАвансов(НаправлениеДвижения, ЭтоВозврат, ВидДоговора, СчетРасчетов) Экспорт
	...
	Если НаправлениеДвижения = "Поступление" Тогда
		...
		ИначеЕсли БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомиссионером(ВидДоговора) Тогда
			Содержание = "Зачет аванса комиссионеру (агенту)";
		ИначеЕсли БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомиссионеромНаЗакупку(ВидДоговора) Тогда
			Если ЭтоСчетРасчетовСПоставщиками(СчетРасчетов) Тогда
				Содержание = "Зачет аванса комиссионеру (агенту)";
			Иначе
				Содержание = "Зачет задолженности комиссионера (принципала)";
			КонецЕсли;
	...
	Иначе //"Выбытие"
		...
		ИначеЕсли БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомитентом(ВидДоговора) Тогда
			Содержание = "Зачет аванса комитента (принципала)";
		ИначеЕсли БухгалтерскийУчетПереопределяемый.ЭтоВидДоговораСКомитентомНаЗакупку(ВидДоговора) Тогда
			Если ЭтоСчетРасчетовСПокупателями(СчетРасчетов) Тогда
				Содержание = "Зачет аванса комитента (принципала)";
			Иначе
				Содержание = "Зачет задолженности перед комитентом (принципалом)";
			КонецЕсли;
		Иначе
			Содержание = "Зачет аванса";
		КонецЕсли;
...
// Возвращает флаг принадлежности счета субсчетам счета 60, 76.05, 76.25, 76.35
//
// Параметры:
...
//   - Булево
//
Функция ЭтоСчетРасчетовСПоставщиками(Счет) Экспорт
	...
	ЭтоСчетРасчетовСПоставщиками = Не Запрос.Выполнить().Пустой();
	Возврат ЭтоСчетРасчетовСПоставщиками;

КонецФункции

// Возвращает флаг принадлежности счета субсчетам счета 62, 76.06, 76.26, 76.36
//
// Параметры:
//  Счет - ПланСчетовСсылка.Хозрасчетный - проверяемый счет
// 
...
//   - Булево
//
Функция ЭтоСчетРасчетовСПокупателями(Счет) Экспорт
	...
	|ГДЕ
	|	Хозрасчетный.Ссылка = &Счет 
	|	И Хозрасчетный.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками), 
	...
	ЭтоСчетРасчетовСПокупателями = Не Запрос.Выполнить().Пустой();
	Возврат ЭтоСчетРасчетовСПокупателями;
...
Функция ПолучитьОстаткиЗадолженности(РасшифровкаПлатежа, Реквизиты, Отказ) Экспорт
	...
	ОбщегоНазначенияБПВызовСервера.ЗаполнитьДатыВТаблице(ОстаткиЗадолженности, "ДокументРасчетов", "ДатаДокументаРасчетов");
...
Функция ТекстЗапросаОстаткиЗадолженности(УчитыватьЗадолженностьУСН)
	...
	Если УчитыватьЗадолженностьУСН Тогда
		...
		|	Остатки.Валюта,
		|	ВЫБОР
		|		КОГДА НЕ Остатки.Счет В (&СчетаРасчетовПоДокументам)
		|			ТОГДА 0
		|		КОГДА Остатки.Счет.Валютный
		|			ТОГДА Остатки.ВалютнаяСуммаОстаток * &Множитель
		|		ИНАЧЕ Остатки.СуммаОстаток * &Множитель
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(втОстатки.СчетРасчетов) КАК СчетРасчетов,
		|	ЕСТЬNULL(втОстатки.Валюта, ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)) КАК Валюта,
		|	ЕСТЬNULL(втОстатки.Подразделение, &ПустоеПодразделение) КАК Подразделение,
		|	втОстатки.Контрагент КАК Контрагент,
		|	втОстатки.ДоговорКонтрагента КАК ДоговорКонтрагента,
		|	втОстатки.ДокументРасчетов КАК ДокументРасчетов,
		|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаРасчетов,
		|	СУММА(втОстатки.СуммаВзаиморасчетов) КАК СуммаВзаиморасчетов,
		|	СУММА(втОстатки.СуммаВзаиморасчетов) КАК ОстатокВзаиморасчетов,
		|	СУММА(втОстатки.СуммаБУ) КАК СуммаБУ,
		|	СУММА(втОстатки.СуммаНУ) КАК СуммаНУ,
		|	СУММА(втОстатки.СуммаВзаиморасчетовЕНВД) КАК СуммаВзаиморасчетовЕНВД,
		|	СУММА(втОстатки.СуммаБУ_ЕНВД) КАК СуммаБУ_ЕНВД,
		|	СУММА(втОстатки.СуммаВзаиморасчетовПатент) КАК СуммаВзаиморасчетовПатент,
		|	СУММА(втОстатки.СуммаБУ_Патент) КАК СуммаБУ_Патент,
		|	СУММА(втОстатки.СуммаВзаиморасчетовКомитента) КАК СуммаВзаиморасчетовКомитента,
		|	СУММА(втОстатки.СуммаБУ_Комитента) КАК СуммаБУ_Комитента,
		|	СУММА(втОстатки.СуммаВзаиморасчетовТорговыйСбор) КАК СуммаВзаиморасчетовТорговыйСбор,
		|	СУММА(втОстатки.СуммаБУ_ТорговыйСбор) КАК СуммаБУ_ТорговыйСбор
		|ИЗ
		|	втОстатки КАК втОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
		|		ПО (ДанныеПервичныхДокументов.Организация = &Организация)
		|			И втОстатки.ДокументРасчетов = ДанныеПервичныхДокументов.Документ
		|
		|СГРУППИРОВАТЬ ПО
		|	ЕСТЬNULL(втОстатки.Подразделение, &ПустоеПодразделение),
		|	втОстатки.Контрагент,
		|	втОстатки.ДоговорКонтрагента,
		|	ЕСТЬNULL(втОстатки.Валюта, ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)),
		|	втОстатки.ДокументРасчетов,
		|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1))
		|
		|ИМЕЮЩИЕ
		|	СУММА(втОстатки.СуммаВзаиморасчетов) > 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаДокументаРасчетов,
		|	ДокументРасчетов";
	...
	Иначе
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втОстатки.СчетРасчетов КАК СчетРасчетов,
		|	ЕСТЬNULL(втОстатки.Валюта, ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка)) КАК Валюта,
		|	втОстатки.Контрагент КАК Контрагент,
		|	втОстатки.ДоговорКонтрагента КАК ДоговорКонтрагента,
		|	втОстатки.ДокументРасчетов КАК ДокументРасчетов,
		|	ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаДокументаРасчетов,
		|	ЕСТЬNULL(втОстатки.Подразделение, &ПустоеПодразделение) КАК Подразделение,
		|	втОстатки.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
		|	втОстатки.СуммаВзаиморасчетов КАК ОстатокВзаиморасчетов,
		...
		|			И втОстатки.ДокументРасчетов = ДанныеПервичныхДокументов.Документ
		|ГДЕ
		|	втОстатки.СуммаВзаиморасчетов > 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаДокументаРасчетов,
		|	ДокументРасчетов";
...
Процедура ЗаполнитьПараметрыВзаимозачета(СтрокаВзаимозачета, СтрокаДебиторскойЗадолженности, СтрокаКредиторскойЗадолженности, Реквизиты)
	...
	Если РасчетыВВалюте Тогда
		...
		Если СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетовПатент > 0 Тогда
			
			КоэффициентДляСуммыПатента = ?(СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетов = 0, 0, СуммаСоответствия /
				СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетов);
			
			СтрокаВзаимозачета.СуммаВзаиморасчетовПатент =
				Окр(СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетовПатент * КоэффициентДляСуммыПатента, 2);
			
			СтрокаВзаимозачета.СуммаБУ_Патент =
				Окр(СтрокаДебиторскойЗадолженности.СуммаБУ_Патент * КоэффициентДляСуммыПатента, 2);
	...
	Иначе
		...
		Если СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетовПатент > 0 Тогда
			...
			СтрокаВзаимозачета.СуммаБУ_Патент = СтрокаВзаимозачета.СуммаВзаиморасчетовПатент;
	...
	СтрокаДебиторскойЗадолженности.СуммаБУ_Патент = СтрокаДебиторскойЗадолженности.СуммаБУ_Патент -
		СтрокаВзаимозачета.СуммаБУ_Патент;
	СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетовПатент = СтрокаДебиторскойЗадолженности.СуммаВзаиморасчетовПатент -
		СтрокаВзаимозачета.СуммаВзаиморасчетовПатент;
...
Функция ПолучитьТаблицуСчетовУчетаВзаиморасчетов(ВключатьГруппыСчетов = Ложь, РаскрыватьГруппыСчетов = Истина) Экспорт
	...
	СписокСчетов.Добавить(ПланыСчетов.Хозрасчетный.АрендныеПлатежи);

	СписокСчетовИсключить = Новый Массив;
	СписокСчетовИсключить.Добавить(ПланыСчетов.Хозрасчетный.РасчетыНДСНалоговогоАгента);
	СписокСчетовИсключить.Добавить(ПланыСчетов.Хозрасчетный.АрендныеОбязательства);
	СписокСчетовИсключить.Добавить(ПланыСчетов.Хозрасчетный.НДСпоАренднымОбязательствам); 
	
	// Счета финансовых вложений с требуемыми субконто, которых нет в счете-группе.
	ОсобыеСчетаФинансовыхВложений = Новый Массив;
	ОсобыеСчетаФинансовыхВложений.Добавить(ПланыСчетов.Хозрасчетный.ПредоставленныеЗаймы);
	ОсобыеСчетаФинансовыхВложений.Добавить(ПланыСчетов.Хозрасчетный.ВкладыПоДоговоруПростогоТоварищества);
	ОсобыеСчетаФинансовыхВложений.Добавить(ПланыСчетов.Хозрасчетный.ПриобретенныеПрава);

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СписокСчетов", 	СписокСчетов);
	Запрос.УстановитьПараметр("СписокСчетовИсключить", СписокСчетовИсключить);
	Запрос.УстановитьПараметр("ОсобыеСчетаФинансовыхВложений", ОсобыеСчетаФинансовыхВложений);
	...
	Если ВключатьГруппыСчетов=Истина Тогда
		...
		Иначе
			// Только группы счетов и счета, которые имеют требуемые виды субконто внутри группы.
			ТекстУсловия = "И (Хозрасчетный.ЗапретитьИспользоватьВПроводках=Истина ИЛИ Хозрасчетный.Ссылка В ИЕРАРХИИ(&ОсобыеСчетаФинансовыхВложений))"; 
		КонецЕсли;
	Иначе
...
// Возвращает текст запроса по состоянию взаиморасчетов с контрагентами
//
// Возвращаемое значение:
// ТекстЗапроса   - строка текста запроса
Функция ТекстЗапросаРасчетыСКонтрагентами() Экспорт
	...
	|											И ХозрасчетныйОбороты.КорСчет <> &Счет62Р
...
Функция ТекстЗапросаОстатокЗадолженностиПоОтгрузке()
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО (ДанныеПервичныхДокументов.Организация = &Организация)
	|			И (ДанныеПервичныхДокументов.Документ = &ДокументОтгрузки)
	|ГДЕ
...
Процедура СформироватьДвиженияПогашениеЗадолженностиПоОтгрузке(ТаблицаВзаиморасчетов, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Параметры = ПодготовитьПараметрыПогашениеЗадолженностиПоОтгрузке(ТаблицаВзаиморасчетов, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	Взаиморасчеты = Параметры.ТаблицаВзаиморасчетов;
	
	ОстатокЗадолженности = ОстатокЗадолженностиПоОтгрузке(Реквизиты);
	СформироватьПроводкиПогашениеЗадолженностиПоОтгрузкеНаСчетеОТ(Взаиморасчеты, ОстатокЗадолженности, Реквизиты, Движения, Отказ);
	
КонецПроцедуры

Функция ПодготовитьПараметрыПогашениеЗадолженностиПоОтгрузке(ТаблицаВзаиморасчетов, ТаблицаРеквизиты)
...
Процедура СформироватьПроводкиПогашениеЗадолженностиПоОтгрузкеНаСчетеОТ(ТаблицаВзаиморасчеты, ТаблицаПогашениеЗадолженности, Реквизиты, Движения, Отказ)
	
	ДтКтРасчетов = "Кт";
	
	СчетРасчетовПоОтгрузке = СчетаРасчетовПоОтгрузке(Реквизиты).СчетРасчетов;
	
	Для Каждого СтрокаВзаиморасчеты Из ТаблицаВзаиморасчеты Цикл
		
		Если НЕ СтрокаВзаиморасчеты.ДокументРасчетов = Реквизиты.Регистратор Тогда
			Продолжить;
		Конецесли;
	
		Для Каждого СтрокаЗадолженность Из ТаблицаПогашениеЗадолженности Цикл
			
			СуммаБУ             = Мин(СтрокаВзаиморасчеты.СуммаРуб, СтрокаЗадолженность.СуммаБУ);
			СуммаНУ             = Мин(СтрокаВзаиморасчеты.СуммаРуб, СтрокаЗадолженность.СуммаНУ);
			СуммаВзаиморасчетов = Мин(СтрокаВзаиморасчеты.СуммаВзаиморасчетов, СтрокаЗадолженность.СуммаВзаиморасчетов);
			
			Если СуммаБУ = 0 И СуммаНУ = 0 И СуммаВзаиморасчетов = 0 Тогда
				Продолжить;
			Иначе
				СтрокаЗадолженность.СуммаБУ             = СтрокаЗадолженность.СуммаБУ - СуммаБУ;
				СтрокаЗадолженность.СуммаНУ             = СтрокаЗадолженность.СуммаНУ - СуммаНУ;
				СтрокаЗадолженность.СуммаВзаиморасчетов = СтрокаЗадолженность.СуммаВзаиморасчетов - СуммаВзаиморасчетов;
			КонецЕсли;
			...
			Если СвойстваСчетаОТ.Валютный Тогда
				Проводка["Валюта" + ДтКтРасчетов]        = СтрокаЗадолженность.ВалютаВзаиморасчетов;
				Проводка["ВалютнаяСумма" + ДтКтРасчетов] = СуммаВзаиморасчетов;
			КонецЕсли;
			Если СвойстваСчетаОТ.УчетПоПодразделениям Тогда
				Проводка["Подразделение" + ДтКтРасчетов] = СтрокаЗадолженность.ПодразделениеРасчетов;
			КонецЕсли;
...
Функция ОстатокЗадолженностиПоОтгрузке(Реквизиты)
	...
	ОбщегоНазначенияБПВызовСервера.ЗаполнитьДатыВТаблице(ОстатокЗадолженности, "ДокументРасчетов", "ДатаДокументаРасчетов");
...
// Возвращает признак возможности применения алгоритма расчета по корректировкам в валюте по курсу на дату отгрузки
//
// Входящие параметры
// Дата - тип дата, в данном параметре передается
//        дата на которую необходимо определить возможность применения методологии 
// Возвращаемое значение:
//  Булево - признак применения
//           Истина - применяется
//           Ложь   - не применяется
// 
Функция ВозможенУчетКорректировокВВалютеПоКурсуНаДатуОтгрузки(Дата) Экспорт
	
	ВозможенУчетКорректировокВВалютеПоКурсуНаДатуОтгрузки = '20240101';
	
	Если Дата < ВозможенУчетКорректировокВВалютеПоКурсуНаДатуОтгрузки Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ПрименяетсяРасчетКорректировокВВалютеПоКурсуНаДатуОтгрузки(Организация, Дата) Экспорт
	
	Если Не ВозможенУчетКорректировокВВалютеПоКурсуНаДатуОтгрузки(Дата) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	"ВЫБРАТЬ
	|	НастройкиВариантаРасчетаКорректировокВВалютеСрезПоследних.РассчитыватьКорректировкуНаУвеличениеПоКурсуНаДатуОтгрузки КАК РассчитыватьКорректировкуНаУвеличениеПоКурсуНаДатуОтгрузки
	|ИЗ
	|	РегистрСведений.НастройкиВариантаРасчетаКорректировокВВалюте.СрезПоследних(&Период, Организация = &Организация) КАК НастройкиВариантаРасчетаКорректировокВВалютеСрезПоследних";
	...
	Иначе
		...
		Выборка.Следующий();
		Возврат Выборка.РассчитыватьКорректировкуНаУвеличениеПоКурсуНаДатуОтгрузки;
	КонецЕсли;
	
КонецФункции

// Возвращает средний курс зачета оплат
// Этот курс используется для выделения аванса при корректировках в валюте 
//
// Входящие параметры
//  СчетУчетаРасчетовСКонтрагентом- <ПланСчетовСсылка.Хозрасчетный> - Счет учета расчетов с контрагентом
//  ДокументРасчетов      - <ДокументСсылка> - Документ расчетов с контрагентами
//  Регистратор           - <ДокументССылка> - Регистратор
//  ЭтоРеализация         - <Булево> - Истина - если корректировка реализации, Ложь - если корректировка поступления
//  НачДата               - <Дата> - Дата документа расчетов (документа поступления или реализации)
//  КонДата               - <Дата> - Дата регистратора (документ возврата или корректировки)
// 
// Возвращаемые параметры
//  СреднийКурсЗачетаОплат - средний курс по которому были зачтены оплаты по документу расчетов с контрагентом
//
Функция СреднийКурсЗачетаОплат(СчетУчетаРасчетовСКонтрагентом, ДокументРасчетов, Регистратор, ЭтоРеализация, НачДата, КонДата) Экспорт
	...
	Если ЭтоРеализация Тогда 
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ВложенныйЗапрос.ВалютнаяСумма, 1)) КАК ВалютнаяСумма,
		|	СУММА(ЕСТЬNULL(ВложенныйЗапрос.СуммаРуб, 0)) КАК СуммаРуб,
		|	ВложенныйЗапрос.СчетКт КАК СчетКт
		|ИЗ
		|	(ВЫБРАТЬ
		|		ХозрасчетныйОборотыДтКт.ВалютнаяСуммаОборотКт КАК ВалютнаяСумма,
		|		ХозрасчетныйОборотыДтКт.СуммаОборот КАК СуммаРуб,
		|		ХозрасчетныйОборотыДтКт.СчетКт КАК СчетКт,
		|		ХозрасчетныйОборотыДтКт.СубконтоКт3 КАК СубконтоКт3,
		|		ХозрасчетныйОборотыДтКт.СчетДт КАК СчетДт
		|	ИЗ
		|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачДата, &КонДата, Регистратор, , , СчетКт = &СчетРасчетов, &СубконтоРасчетов, СубконтоКт3 = &ДокументРасчетов) КАК ХозрасчетныйОборотыДтКт
		|	ГДЕ
		|		ХозрасчетныйОборотыДтКт.ВалютнаяСуммаОборотКт <> 0
		|		И ХозрасчетныйОборотыДтКт.Регистратор <> &Регистратор) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
	...
	Иначе
		...
		|		ХозрасчетныйОборотыДтКт.СчетКт КАК СчетКт
		|	ИЗ
		|		РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачДата, &КонДата, Регистратор, СчетДт = &СчетРасчетов, &СубконтоРасчетов, , , СубконтоДт3 = &ДокументРасчетов) КАК ХозрасчетныйОборотыДтКт
		|	ГДЕ
		|		ХозрасчетныйОборотыДтКт.ВалютнаяСуммаОборотДт <> 0
		|		И ХозрасчетныйОборотыДтКт.Регистратор <> &Регистратор) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
	...
	СубконтоРасчетов = Новый Массив();
	СубконтоРасчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты);
	СубконтоРасчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	СубконтоРасчетов.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами);
	
	Запрос.УстановитьПараметр("ДокументРасчетов",  ДокументРасчетов);
	Запрос.УстановитьПараметр("СчетРасчетов",      СчетУчетаРасчетовСКонтрагентом);
	Запрос.УстановитьПараметр("СубконтоРасчетов",  СубконтоРасчетов);
	...
	Если Результат.Следующий() Тогда
		Если Результат.ВалютнаяСумма = 0 Тогда
			СреднийКурсЗачетаОплат = 0;
		Иначе
			СреднийКурсЗачетаОплат = Результат.СуммаРуб / Результат.ВалютнаяСумма;
		КонецЕсли;
	КонецЕсли;
	
	Возврат СреднийКурсЗачетаОплат;
КонецФункции

#Область СлужебныеПроцедурыИФункции

Функция ОстаткиАвансовПоПлатежнымКартам(ТаблицаДокумента, Реквизиты, Отказ)
	
	ОстаткиАвансов = ПустаяТаблицаОстаткиАвансов();
	...
	ИменаОтборов = Новый Структура("СчетАвансов, Контрагент, ДоговорКонтрагента", "СчетУчета", "Контрагент", "ДоговорКонтрагента");
	ПереплатыОтЭквайера = ПереплатыОтЭквайера(ТаблицаДоговоров, ИменаОтборов, Реквизиты);
	...
	Для Каждого СтрокаОплаты Из ТаблицаДокумента Цикл
		
		СуммаОплат = СтрокаОплаты.СуммаРуб;
		
		КурсВзаиморасчетов = ?(СтрокаОплаты.СуммаВзаиморасчетов = 0, 0, СтрокаОплаты.СуммаРуб / СтрокаОплаты.СуммаВзаиморасчетов);
		
		ОтборПереплат.Контрагент = СтрокаОплаты.Контрагент;
		ОтборПереплат.ДоговорКонтрагента = СтрокаОплаты.ДоговорКонтрагента;
		ОтборПереплат.СчетУчета = СтрокаОплаты.СчетАвансов;
		...
		Для Каждого СтрокаПереплаты Из НайденныеСтрокиПереплат Цикл
			...
			РаспределеннаяСумма = Мин(СуммаОплат, СуммаПереплаты);
			
			НоваяСтрока = ОстаткиАвансов.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОплаты);
			
			НоваяСтрока.Валюта = СтрокаОплаты.ВалютаВзаиморасчетов;
			НоваяСтрока.ДокументРасчетов = СтрокаПереплаты.РасчетныйДокумент;
			НоваяСтрока.ДатаДокументаРасчетов = СтрокаПереплаты.РасчетныйДокументДата;
			НоваяСтрока.Счет = СтрокаПереплаты.СчетУчета;
			
			НоваяСтрока.СуммаБУ             = РаспределеннаяСумма;
			НоваяСтрока.СуммаНУ             = РаспределеннаяСумма;
			...
			Если ОстаткиСчетовУСНПоДокументу.Количество() > 0 Тогда
				...
				КоэффициентСписанияПереплаты = ?(СтрокаПереплаты.СуммаОстаток = 0, 0, РаспределеннаяСумма / СтрокаПереплаты.СуммаОстаток);
				...
				Для Каждого ОстатокПоСчетамУСН Из ОстаткиСчетовУСНПоДокументу Цикл
					...
					СуммаВзаиморасчетовПоСчетамУСН = Окр(- ОстатокПоСчетамУСН.СуммаВзаиморасчетов * КоэффициентСписанияПереплаты, 2);
					СуммаРубПоСчетамУСН            = Окр(- ОстатокПоСчетамУСН.СуммаРуб * КоэффициентСписанияПереплаты, 2);
			...
			СуммаОплат = СуммаОплат - РаспределеннаяСумма;
...
Процедура ДобавитьПроводкиВспомогательныеРасчетыПоЭквайрингуЗачетПереплаты(СтрокаВзаиморасчетов, СтрокаВзаиморасчетовУСН, СуммаСписания, СчетаУСН, Реквизиты, Движения)
	
	КодЯзыка = ОбщегоНазначения.КодОсновногоЯзыка();
	
	// Расчеты по доходам комитента: счет УСН.02
	Если Реквизиты.УчитыватьЗадолженностьУСН
		И СтрокаВзаиморасчетов.СуммаВзаиморасчетовКомитента <> 0 Тогда
		...
		Проводка.Период      = Реквизиты.Период;
		Проводка.Организация = Реквизиты.Организация;
		Проводка.Содержание  = НСтр("ru = 'Расчеты за товары и услуги комитента'", КодЯзыка);
		
		Проводка.СчетДт = СтрокаВзаиморасчетовУСН.Счет;
		Проводка.СчетКт = ПланыСчетов.Хозрасчетный.УСНРасчетыСПокупателямиКомитент;
		
		ЗаполнитьАналитикуПоДтПроводкиВспомогательныхРасчетовПоЭквайрингу(Проводка, СтрокаВзаиморасчетовУСН);
		ЗаполнитьАналитикуПоКтПроводкиВспомогательныхРасчетовПоЭквайрингу(Проводка, СтрокаВзаиморасчетов, Реквизиты);
		
		Проводка.Сумма = СуммаСписания;
		
	КонецЕсли;
	
	// Расчеты по доходам на патенте: счет УСН.03
	Если Реквизиты.УчитыватьЗадолженностьУСНПатент 
		И СтрокаВзаиморасчетов.СуммаВзаиморасчетовПатент <> 0 Тогда
		...
		Проводка.Организация  = Реквизиты.Организация;
		Если Реквизиты.Период < УчетПСН.ДатаНачалаДействияПатентнойСистемы() Тогда
			Проводка.Содержание = НСтр("ru = 'Расчеты по деятельности УСН на патенте'", КодЯзыка);
		Иначе
			Проводка.Содержание = НСтр("ru = 'Расчеты по деятельности на патенте'", КодЯзыка);
		КонецЕсли;
	...
	// Расчеты по доходам от деятельности на торговом сборе: счет УСН.04
	Если Реквизиты.УчитыватьЗадолженностьУСН
		И СтрокаВзаиморасчетов.СуммаВзаиморасчетовТорговыйСбор <> 0 Тогда
		...
		Проводка.Период      = Реквизиты.Период;
		Проводка.Организация = Реквизиты.Организация;
		Проводка.Содержание  = НСтр("ru = 'Расчеты по деятельности, облагаемой торговым сбором'", КодЯзыка);
	...
	// Если нет вспомогательных расчетов, то просто спишем остаток
	Если Не Реквизиты.УчитыватьЗадолженностьУСН И Не Реквизиты.УчитыватьЗадолженностьУСНПатент
		Или (СтрокаВзаиморасчетов.СуммаВзаиморасчетовЕНВД = 0
			И СтрокаВзаиморасчетов.СуммаВзаиморасчетовКомитента = 0
			И СтрокаВзаиморасчетов.СуммаВзаиморасчетовПатент = 0
			И СтрокаВзаиморасчетов.СуммаВзаиморасчетовТорговыйСбор = 0) Тогда
		
		Если СчетаУСН.ЕНВД.Найти(СтрокаВзаиморасчетовУСН.Счет) <> Неопределено Тогда
			СодержаниеПроводки  = НСтр("ru = 'Расчеты по деятельности ЕНВД'", КодЯзыка);
		ИначеЕсли СчетаУСН.Комитент.Найти(СтрокаВзаиморасчетовУСН.Счет) <> Неопределено Тогда
			СодержаниеПроводки  = НСтр("ru = 'Расчеты за товары и услуги комитента'", КодЯзыка);
		ИначеЕсли СчетаУСН.ТорговыйСбор.Найти(СтрокаВзаиморасчетовУСН.Счет) <> Неопределено Тогда
			СодержаниеПроводки  = НСтр("ru = 'Расчеты по деятельности, облагаемой торговым сбором'", КодЯзыка);
		ИначеЕсли СчетаУСН.Патент.Найти(СтрокаВзаиморасчетовУСН.Счет) <> Неопределено Тогда
			СодержаниеПроводки = НСтр("ru = 'Расчеты по деятельности на патенте'", КодЯзыка);
		Иначе
			Возврат;
		КонецЕсли;
...
Процедура ЗаполнитьАналитикуПоДтПроводкиВспомогательныхРасчетовПоЭквайрингу(Проводка, СтрокаВзаиморасчетовУСН)
	
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,
		Проводка.СубконтоДт, "Контрагенты", СтрокаВзаиморасчетовУСН.Контрагент);
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,
		Проводка.СубконтоДт, "Договоры", СтрокаВзаиморасчетовУСН.Договор);
	БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт,
		Проводка.СубконтоДт, "ДокументыРасчетовСКонтрагентами", СтрокаВзаиморасчетовУСН.РасчетныйДокумент);
		
	СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетДт);
	Если СвойстваСчета.УчетПоПодразделениям Тогда
		Проводка.ПодразделениеДт = СтрокаВзаиморасчетовУСН.Подразделение;
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьАналитикуПоКтПроводкиВспомогательныхРасчетовПоЭквайрингу(Проводка, СтрокаВзаиморасчетов, Реквизиты)
...
Функция НоваяСтруктураКУДиРУСН()
	
	СтруктураКУДиР = Новый Структура;
	СтруктураКУДиР.Вставить("Пояснение", Новый Массив);
