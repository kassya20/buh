...
Процедура ЗаполнитьТаблицуОсновныеСредстваАмортизируемыеНеамортизируемые(ЗаполняемыеДанные, Классификаторы, КонтекстЗаполненияОтчета, МенеджерВременныхТаблиц, ВариантыУчета, ОбщиеПравилаУчета)
	...
	СпособОпределенияОбъектовПостоянные = СтрШаблон(
		...
		Метаданные.РегистрыСведений.НачислениеАмортизацииОСБухгалтерскийУчет.Представление(),
...
Процедура ЗаполнитьТаблицуОсновныеСредстваОграничения(ЗаполняемыеДанные, Классификаторы, КонтекстЗаполненияОтчета, МенеджерВременныхТаблиц, ВариантыУчета, ОбщиеПравилаУчета)
	...
	СпособОпределенияОбъектовВАренде = СтрШаблон(
		...
		Метаданные.РегистрыСведений.МестонахождениеОСБухгалтерскийУчет.Представление(),
		Метаданные.РегистрыСведений.МестонахождениеОСБухгалтерскийУчет.Ресурсы.Контрагент.Представление());
...
Процедура СоздатьСчетаУчетаОсновныхСредств(МенеджерВременныхТаблиц, КлассификацияСчетов, ОбщиеПравилаУчета)
	...
	|				КОНЕЦ
	|		КОГДА Хозрасчетный.Ссылка В ИЕРАРХИИ (&СчетаОбесценение)
	|			ТОГДА ""Обесценение""
	|		ИНАЧЕ ""Амортизация""
	|	КОНЕЦ КАК ВидСчета,
...
Процедура СоздатьСтоимостьОсновныхСредств(МенеджерВременныхТаблиц, КонтекстЗаполненияОтчета, ОбщиеПравилаУчета)
	...
	|ИЗ
	|	Справочник.ПрочиеДоходыИРасходы КАК Статьи
	|ГДЕ
	...
	|ВЫБРАТЬ
	|	""СписаноОбесценение"",
	|	СписаноОбесценение.Счет,
	|	СписаноОбесценение.Субконто1,
	|	СписаноОбесценение.Организация,
	|	0,
	|	0,
	|	-СписаноОбесценение.СуммаОборотДт,
	|	0,
	|	СписаноОбесценение.КорСчет
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В (&СчетаОбесценение), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), Организация В (&Организации), КорСчет В (&СчетаКПродаже), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства)) КАК СписаноОбесценение
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура СоздатьАмортизацияЗапрещена(МенеджерВременныхТаблиц, КонтекстЗаполненияОтчета, Классификаторы)
	...
	|						СтоимостьОсновныхСредств КАК Стоимость)) КАК Настройка
	|	ГДЕ
	|		НЕ Настройка.НачислятьАмортизацию) КАК Постоянные
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СоздатьОграниченияОсновныхСредств(МенеджерВременныхТаблиц, КонтекстЗаполненияОтчета, ОбщиеПравилаУчета, Классификаторы)
	...
	|	ГДЕ
	|		ВАрендеПоМестонахождениюНачало.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ВАрендеПоМестонахождениюКонец.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
