...
Функция ЗаголовокПереченьПрямыхЗатрат(Организация, Период) Экспорт
	
	СостояниеРегистра = РегистрыСведений.МетодыОпределенияПрямыхРасходовПроизводстваВНУ.СостояниеЗаполнения(Организация, Период);
	
	Если СостояниеРегистра = "ЗаполненПоУмолчанию" Тогда
		ТекстСостояние = НСтр("ru = 'заполнен в соответствии с рекомендациями ст. 318 НК РФ'");
	ИначеЕсли СостояниеРегистра = "НеЗаполнен" Тогда
		ТекстСостояние = НСтр("ru = 'не заполнен'");
	Иначе
		ТекстСостояние = НСтр("ru = 'заполнен'");
	КонецЕсли;
	
	Возврат СтрШаблон(НСтр("ru = 'Перечень прямых расходов: %1'"), ТекстСостояние);
