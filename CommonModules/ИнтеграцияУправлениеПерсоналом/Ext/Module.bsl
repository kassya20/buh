...
// Сохраняет ключи менеджера в безопасном хранилище.
// Вызывающий код должен самостоятельно устанавливать привилегированный режим.
//
// Параметры
// 		Приложение - ПеречислениеСсылка.ПриложенияДляИнтеграции
...
// Проверяет наличие ключей менеджера.
//
// Параметры
// 		Приложение - ПеречислениеСсылка.ПриложенияДляИнтеграции
//
...
// 		Булево
//
Функция ЕстьКлючиПриложения(Приложение) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Возврат (КлючиПриложения(Приложение) <> Неопределено);
	...
	Если Не Отказ Тогда
		...
		Если Ответ.Использовать = Истина Тогда
			...
			Если ЗначениеЗаполнено(ВерсииФормата.ВерсияDTO) И ЗначениеЗаполнено(ВерсииФормата.ВерсияAPI) Тогда
				...
				КонецЕсли;
				МенеджерОбмена.ВерсияAPI = ВерсияAPI;
				МенеджерОбмена.ВерсияDTO = ВерсияDTO;
				МенеджерОбмена.ОбновитьИнициализациюПриСмениВерсииAPI();
				МенеджерОбмена.ОбновитьВерсиюDTO();
			Иначе
				Отказ = Истина;
			КонецЕсли;
		Иначе
...
Функция ОбъектыДляРегистрацииПриЗаписиНовыхПравил(Приложение, ДанныеДляРегистрации)
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		...
		ИспользуютсяСИЗ = ДоступенРасширенныйФункционал И ИнтеграцияКабинетСотрудника.ДоступноИспользованиеСредствИндивидуальнойЗащиты();
		Для каждого ФизическоеЛицо Из НовыеФизическиеЛица Цикл
			НоваяСтрока = ИзмененияДляОбмена.Добавить();
			НоваяСтрока.Ссылка 			= ФизическоеЛицо;
	...
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ПодразделениеОрганизации);
	...
	Отбор = Новый Структура("УсловноВыгружается,Уволен,Удалить", Ложь, Ложь, Ложь);
	ФизическиеЛицаОрганизации = ТаблицаСотрудники.Скопировать(Отбор,"Организация,ФизическоеЛицо");
...
Функция ОбъектыДляРегистрацииОбмена(Приложение, ДанныеДляРегистрации)
	...
	Отбор = Новый Структура("УсловноВыгружается,Уволен,Удалить", Ложь, Ложь, Ложь);
	ФизическиеЛицаОрганизации = Сотрудники.Скопировать(Отбор,"Организация,ФизическоеЛицо");
	ФизическиеЛицаОрганизации.Свернуть("Организация,ФизическоеЛицо");
...
Функция ОбъектыПозицийШтатногоРасписанияПоСсылкам(Приложение, Позиции)
	...
	ПубликоватьСтруктуруЮридическихЛиц = ПубликоватьСтруктуруЮридическихЛиц();
	ВыгружатьПодразделенияОрганизаций = Ложь;
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
		ТипСсылки = Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.СсылкаПубликации1СПерсонал.Тип);
		ВыгружатьПодразделенияОрганизаций = Интеграция1СПерсонал.ИспользуетсяВерсияDTO("3.4");
	Иначе
		ТипСсылки = Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.СсылкаПубликацииКабинетСотрудника.Тип);
		ВыгружатьПодразделенияОрганизаций = ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.4");
	КонецЕсли;
	...
	ПодразделенияОрганизаций = Новый Массив;
	Если ВыгружатьПодразделенияОрганизаций Или ПубликоватьСтруктуруЮридическихЛиц Тогда
		ПодразделенияОрганизаций = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаПозиций, "Подразделение", Истина);
		РодителиПодразделений = РодителиПодразделений(ПодразделенияОрганизаций);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ПодразделенияОрганизаций, РодителиПодразделений, Истина);
	КонецЕсли;
	
	Подразделения = Новый Массив;
	Если ПубликоватьСтруктуруЮридическихЛиц Тогда
		Подразделения = ПодразделенияОрганизаций;
	Иначе
	...
	Если ВыгружатьПодразделенияОрганизаций Тогда
		Для каждого Подразделение Из ПодразделенияОрганизаций Цикл
			НоваяСтрока = ОбъектыПозиций.Добавить();
			НоваяСтрока.Ссылка = Подразделение;
			НоваяСтрока.ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ПодразделениеОрганизации;
		КонецЦикла;
	КонецЕсли;
...
Функция ТипОбъектовВыгружаемыеПоПравилам() Экспорт
	...
	ТипыОбъектов.Вставить(ПеречислениеМенеджер.ПодразделениеОрганизации, 		Истина);
	...
	ТипыОбъектов.Вставить(ПеречислениеМенеджер.ВидЛичногоКабинета, 					Истина); // ФизическоеЛицо
...
Процедура ОбновитьСведенияОПубликацииФизическихЛицИсключений(Приложение, ФизическиеЛица) Экспорт
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ИнтеграцияКабинетСотрудника.ЗарегистрироватьОбновлениеВидаЛичногоКабинета(ФизическиеЛица);
	КонецЕсли;
...
// 		Массив
//
Функция ВыгружаемыеСотрудники(Приложение) Экспорт
...
Функция ТипыОбъектовДляРучнойРегистрацииИзменений(Приложение) Экспорт
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		...
		КонецЕсли;
	ИначеЕсли Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
		Интеграция1СПерсонал.ТипыОбъектовДляРучнойРегистрацииИзменений(ТипыОбъектов);
...
Функция ТипОбъектаТипыЗначений()
	...
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.ПодразделениеОрганизации,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ПодразделенияОрганизаций")));
	...
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.МедицинскиеОсмотры,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ФизическиеЛица")));
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.ИнформацияОСредствахИЗ,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ФизическиеЛица")));
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.Антропометрия,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ФизическиеЛица")));
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.ВидДокументаЭДО,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ВидыДокументовЭДОБЗК")));
	...
	ТипОбъектаТипЗначения.Вставить(ПеречислениеМенеджер.ВидЛичногоКабинета,
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("СправочникСсылка.ФизическиеЛица")));
...
// Удаляет данные интеграции.
// Вызывающий код должен самостоятельно устанавливать привилегированный режим.
//
// Параметры
// 		Приложение - ПеречислениеСсылка.ПриложенияДляИнтеграции
// 		РегистрироватьСобытиеЖР - Булево, признак регистрации события отключения приложения.
//
Процедура ОчиститьДанныеПриОтключенииПриложения(Приложение, РегистрироватьСобытиеЖР) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки.УстановитьЗначение("Приложение", Приложение);
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОбработчикиОбменаУправлениеПерсоналом");
		ЭлементБлокировки.УстановитьЗначение("Приложение", Приложение);
	...
	Если РегистрироватьСобытиеЖР Тогда
		Если Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
...
Процедура ПроверитьРуководителейОрганизаций(БылиОшибки) Экспорт
	...
	ФизическиеЛицаДляОбновленияЛК = Новый Массив;
	Для каждого СтрокаТЗ Из ТекущиеРуководители Цикл
		...
		Если СтрокаТЗ.Руководитель <> ДанныеРуководителя.Руководитель Тогда
			...
			НоваяСтрока.Руководитель 	= ДанныеРуководителя.Руководитель;
			НоваяСтрока.Должность 		= ДанныеРуководителя.РуководительДолжность;
			Если ЗначениеЗаполнено(СтрокаТЗ.Руководитель) Тогда
				ФизическиеЛицаДляОбновленияЛК.Добавить(СтрокаТЗ.Руководитель);
			КонецЕсли;
			Если ЗначениеЗаполнено(ДанныеРуководителя.Руководитель) Тогда
				ФизическиеЛицаДляОбновленияЛК.Добавить(ДанныеРуководителя.Руководитель);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЗначениеЗаполнено(ФизическиеЛицаДляОбновленияЛК) И ИспользуетсяКабинетСотрудника() Тогда
		Для каждого ФизическоеЛицо Из ФизическиеЛицаДляОбновленияЛК Цикл
			НоваяСтрока = ИзмененияКС.Добавить();
			НоваяСтрока.Ссылка 			= ФизическоеЛицо;
...
Функция РуководителиПодразделений(Подразделения, ФизическиеЛица) Экспорт
...
#Область БлокировкаДоступа

Процедура ОбновитьСостояниеБлокировкиФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	
	Результат = Новый Структура("СостояниеБлокировкиПолучено, БлокировкаПользователейУстановлена", Ложь, Ложь);
	
	Приложение = Параметры.Приложение;
	УстановитьПривилегированныйРежим(Истина);
	МенеджерОбмена = ИнтеграцияУправлениеПерсоналомОбмен.НовыйМенеджерОбмена(Приложение, Истина);
	РезультатОбмена = МенеджерОбмена.ДоступностьПриложения();
	
	Если РезультатОбмена <> Неопределено Тогда
		Результат.СостояниеБлокировкиПолучено = Истина;
		Результат.БлокировкаПользователейУстановлена = РезультатОбмена.ДоступностьПользователей = "недоступно";
	КонецЕсли;
...
Процедура ИзменитьСостояниеБлокировкиПользователейФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	
	Результат = Новый Структура("СостояниеБлокировкиИзменено", Ложь);
	
	Приложение = Параметры.Приложение;
	УстановитьПривилегированныйРежим(Истина);
	МенеджерОбмена = ИнтеграцияУправлениеПерсоналомОбмен.НовыйМенеджерОбмена(Приложение, Истина);
	
	Результат.СостояниеБлокировкиИзменено = МенеджерОбмена.УстановитьДоступностьПриложения(Параметры.БлокировкаДоступаПользователей);
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
