...
// Выполняет поиск в массиве ВидыПродукции продукции, требующей прослеживания при перемещении между МОДами.
//
// Параметры:
//  ВидыПродукции - Массив Из ПеречислениеСсылка.ВидыПродукцииИС - Коллекция видов продукции ИС.
//  НаДату        - Дата - дата проверки контроля перемещений между МОД.
// Возвращаемое значение:
//  Булево - Истина, если найдена продукция, требующая прослеживания при перемещении между МОД.
Функция ПрисутствуетПрослеживаемаяПоМОДПродукция(ВидыПродукции, НаДату = Неопределено) Экспорт
	
	МассивПрослеживаемойПродукции = ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииПрослеживаемогоПеремещенияМеждуМОД();
	
	Для Каждого ВидПродукции Из МассивПрослеживаемойПродукции Цикл
...
Функция ВыполнитьПроверкиПередПолучениемДанныхПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		РезультатПроверки = ТребуетсяОткрытьФормуВводаКодаМаркировки(СтрокаДанных, ПараметрыСканирования, КешНаличияСвойств);
		Если ЗначениеЗаполнено(РезультатПроверки) Тогда
			СтрокаДанных.ОбработанСОшибками = Истина;
			РезультатПроверок.Вставить(СтрокаДанных.Штрихкод, РезультатПроверки);
			Продолжить;
...
Функция ТребуетсяОткрытьФормуВводаКодаМаркировки(СтрокаДанных, ПараметрыСканирования, КешНаличияСвойств = Неопределено) Экспорт
	...
	ЭтоЗапросКодаМаркировкиПослеСчитыванияRFID = ЕстьСвойствоСтрокиДанныхДанныеRFID
	...
	Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина)
		...
		ТребуетсяВводМарок = СтрокаДанных.ТребуетсяКонтрольМарок;
...
Процедура ЗаполнитьДанныеКодовМаркировкиВходящимиДанными(ДанныеПоШтрихкодам, СписокДанныхШтрихкода, ПараметрыСканирования)
	...
	Для Каждого ДанныеШтрихкода Из СписокДанныхШтрихкода Цикл
		...
		Если ТипЗнч(ДанныеШтрихкода) = Тип("Структура")
			И ДанныеШтрихкода.Свойство("ЗагруженИзЭлектронногоДокумента") Тогда
...
Функция ПроверкаОшибкиПриРазбореКодаМаркировки(СтрокаДанных, ПараметрыСканирования)
	...
	РезультатОбработки = ИнициализироватьРезультатОбработкиШтрихкода(ПараметрыСканирования, СтрокаДанных);
...
// Возвращаемое значение:
//   (См. ИнициализироватьРезультатОбработкиШтрихкода).
Функция РезультатТребуетсяУточнениеКоличества(ДанныеШтрихкода, ПараметрыСканирования) Экспорт
...
Функция ОбработатьДанныеПоШтрихкодам(ДанныеПоШтрихкодам, ПараметрыСканирования, ЕстьОшибки, ФормаУникальныйИдентификатор, РезультатыПроверок) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		ИначеЕсли (ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияМОТП(ВидПродукции)
			...
			РезультатОбработки = РезультатТребуетсяУточнениеКоличества(ДанныеШтрихкода, ПараметрыСканирования);
