...
Функция СчетЦифровойРубль() Экспорт
...
Функция УчетНа70ВедетсяСводно() Экспорт
	...
	Возврат БухгалтерскийУчетВызовСервераПовтИсп.НаСчетеВедетсяУчетПоРаботникам(СчетРасчетовСПерсоналом);
...
Функция СчетаУчета2017Кв3_ДебКср(ВидЗадолженности) Экспорт
	...
	ИначеЕсли ВидЗадолженности = "Платежи" Тогда
		СчетаЗапроса.Вставить("СчетаУчетаВидыПлатежей",
			"68.01, 68.02, 68.03, 68.06, 68.07, 68.08, 68.09, 68.11, 68.12, 68.13, 68.14, 68.15, 68.16, 68.21, 68.34, 68.52,
			|69.01, 69.02.1, 69.02.2, 69.02.3, 69.02.4, 69.02.5, 69.02.6, 69.02.7, 
			|69.03.1, 69.03.2, 69.04, 69.05.1, 69.05.2, 69.06.1, 69.06.2, 69.06.3, 69.06.4, 69.06.5, 69.06.6, 69.09, 69.11, 69.12");
...
Функция СчетаУчета2017Кв3_КредКср(ВидЗадолженности) Экспорт
	...
	Если ВидЗадолженности = "Расчеты" Тогда
		СчетаЗапроса.Вставить("СчетаУчетаКонтрагентыДоговоры",
			"60.01, 60.21, 60.31, 60.03, 76.05, 76.25, 76.35");
		СчетаЗапроса.Вставить("СчетаУчетаБезАналитики", "76.16");
	...
	ИначеЕсли ВидЗадолженности = "Платежи" Тогда
		СчетаЗапроса.Вставить("СчетаУчетаВидыПлатежей",
			...
			|69.04, 69.05.1, 69.05.2, 69.06.1, 69.06.2, 69.06.3, 69.06.4, 69.06.5, 69.06.6, 69.09, 69.11, 69.12");
		СчетаЗапроса.Вставить("СчетаУчетаВидыПлатежейУровни", "68.04.1, 68.10");
...
Функция ТекстЗапросаВТ_БанковскиеСчета()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	БанковскиеСчета.Банк.СВИФТБИК КАК СВИФТБИК,
	               |	БанковскиеСчета.Банк.Страна.Код КАК КодСтраны,
	               |	БанковскиеСчета.ВалютаДенежныхСредств.Код КАК КодВалюты,
	               |	БанковскиеСчета.ЦифровойСчет КАК ЭтоЦифровойСчет,
...
Функция ТекстЗапросаВТ_ДанныеПП()
	...
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	СписаниеСРасчетногоСчета.Ссылка КАК ДокументПлатежа,
		|	ПлатежноеПоручение.Ссылка КАК ДокументПП
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ПлатежныеПоручения.ДокументПлатежа КАК Списание,
		|	ВТ_ПлатежныеПоручения.ДокументПП КАК ПлатежноеПоручение,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПлатежноеПоручение КАК ПлатежноеПоручение
		|		ПО ВТ_ПлатежныеПоручения.ДокументПП = ПлатежноеПоручение.Ссылка";
...
Процедура УстановитьПараметрыСчета_75(ПараметрыСчета)
	...
	СтрокаПараметров = ДобавитьВидыСубконоПоСчету(ПараметрыСчета, "Учредители");
	СтрокаПараметров.РазвернутоеСальдо = Истина;
