...
Процедура УстановитьПодразделениеКт(Форма) Экспорт
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ОплатаПокупателя")
		...
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВозвратЗаймаКонтрагентом")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ПолучениеЗайма")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ПолучениеКредита")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВзносВУставныйКапитал") Тогда
...
Процедура ИзменитьПараметрыВыбораСчетаКассы(Форма) Экспорт
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.РозничнаяВыручка")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВозвратЗаймаРаботником")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВзносВУставныйКапитал") Тогда
...
Процедура УстановитьПараметрыВыбораДоговораСКонтрагентом(Форма) Экспорт
	...
	ВидыДоговора   = ПриходныйКассовыйОрдерФормыВызовСервера.ВидыДоговоровДокумента(Объект.ВидОперации);
...
Функция СвойстваСтрокРасшифровкиПлатежа(Знач ПараметрыОбъекта, Знач ПолучатьДоговор) Экспорт
	...
	Иначе
		...
		ПараметрыПлатежа = Новый Структура("ВидОперации, Организация, Дата, Контрагент, ДоговорКонтрагента, Сделка,
			...
			|СтатьяДвиженияДенежныхСредств, ПлательщикНДСПриУСН, СтавкаНДСПриУСН, ПорядокОтраженияАванса, Ссылка");
...
Функция ТекущиеПараметрыОбъекта(Форма) Экспорт
	...
	ПараметрыОбъекта = Новый Структура(
		"Дата, ВидОперации, Организация, Контрагент, ВалютаДокумента, СчетКасса, ВыручкаСНТТ, Ссылка");
	...
	СтатьяДДС = УчетДенежныхСредствВызовСервера.СтатьяДДСПоУмолчанию(Объект.ВидОперации, Объект.Дата);
...
Функция ДокументМожетБытьОформленВВалюте(ВидОперации) Экспорт
	...
	Возврат ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВозвратЗаймаРаботником")
		...
		И ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВзносВУставныйКапитал");
...
Процедура УстановитьПараметрыВыбораСтатьяДДС(Форма) Экспорт
	
	Если ФормаДокументаОднострочная(Форма) Тогда
		МассивЗначений = Новый Массив;
		МассивЗначений.Добавить(ПредопределенноеЗначение("Перечисление.ВидСтатьиДвиженияДенег.Доход"));
		МассивЗначений.Добавить(ПредопределенноеЗначение("Перечисление.ВидСтатьиДвиженияДенег.ДоходРасход"));
		
		НовыеПараметры = Новый Массив;
		НовыеПараметры.Добавить(Новый ПараметрВыбора("Отбор.ВидСтатьиДДС", Новый ФиксированныйМассив(МассивЗначений)));
		ПараметрыВыбора = Новый ФиксированныйМассив(НовыеПараметры);
		
		// Устанавливаем параметры выбора для статьей, которые добавлены как реквизиты формы
		// Для остальных статей параметры выбора настроены в реквизитах табличной части РасшифровкаПЛатежа.
		Элементы = Форма.Элементы;
		Элементы.РасшифровкаПлатежаСтатьяДвиженияДенежныхСредств.ПараметрыВыбора = ПараметрыВыбора;
		Элементы.РасшифровкаВыручкиСтатьяДвиженияДенежныхСредств.ПараметрыВыбора = ПараметрыВыбора;
		Элементы.СтатьяДвиженияДенежныхСредствПрочие.ПараметрыВыбора = ПараметрыВыбора;
		Элементы.РасшифровкаПлатежаСтатьяДвиженияДенежныхСредствБезДокументов.ПараметрыВыбора = ПараметрыВыбора;
		Элементы.РасшифровкаВыручкиСтатьяДвиженияДенежныхСредствБезДокументов.ПараметрыВыбора = ПараметрыВыбора;
	КонецЕсли;
