	...
	Если (Не МетаданныеДокумента.ТабличныеЧасти.Найти("Товары") = Неопределено) И (Не ВидОперации = "РеализацияУслуг") Тогда
		...
		Иначе
			...
			Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВидОперации") = Перечисления.ВидыОперацийРеализацияТоваров.ОтгрузкаБезПереходаПраваСобственности Тогда
				Текст = Текст + "
				|	ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				|	СУММА(КорректировкаРеализацииТовары.Количество - КорректировкаРеализацииТовары.КоличествоДоИзменения),
				|	КорректировкаРеализацииТовары.СчетДоходов,
				|	СУММА(КорректировкаРеализацииТовары.Сумма - КорректировкаРеализацииТовары.СуммаДоИзменения + 
				|		ВЫБОР
				|			КОГДА &ЕстьНДССверху
				|				ТОГДА КорректировкаРеализацииТовары.СуммаНДС - КорректировкаРеализацииТовары.СуммаНДСДоИзменения
				|			ИНАЧЕ 0
				|		КОНЕЦ),
				|	СУММА(КорректировкаРеализацииТовары.СуммаНДС - КорректировкаРеализацииТовары.СуммаНДСДоИзменения),
				|	КорректировкаРеализацииТовары.Номенклатура,
				|	КорректировкаРеализацииТовары.СчетУчета
				|ИЗ
				|	Документ.КорректировкаРеализации.Товары КАК КорректировкаРеализацииТовары
				|ГДЕ
				|	КорректировкаРеализацииТовары.Ссылка.ИсходныйДокументРеализации = &Ссылка
...
Процедура СформироватьДвиженияУСН(ДокументОбъект, СтруктураДопПараметров = Неопределено, ЭтоОбъект = Истина, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета")
		...
		Если ТипЗнч(ДокументСсылка) <> Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
			ОплатаПоДГПХПоВедомостям = Ложь;
			Если ЭтоОбъект Тогда
				Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
					ОплатаПоДГПХПоВедомостям = ДокументОбъект.ПеречислениеЗаработнойПлаты.Количество() > 0;
				ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
					ОплатаПоДГПХПоВедомостям = ДокументОбъект.ВыплатаЗаработнойПлаты.Количество() > 0;
				КонецЕсли;
				СтруктураШапкиДокумента.Вставить("ОплатаПоДГПХ",
					ЗначениеЗаполнено(ДокументОбъект.ПлатежнаяВедомость) Или ОплатаПоДГПХПоВедомостям);
			Иначе
				Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
					ОплатаПоДГПХПоВедомостям = Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ПеречислениеЗаработнойПлаты").Пустой();
				ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер") Тогда
					ОплатаПоДГПХПоВедомостям = Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ВыплатаЗаработнойПлаты").Пустой();
				КонецЕсли;
				СтруктураШапкиДокумента.Вставить("ОплатаПоДГПХ",
					ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ПлатежнаяВедомость"))
						Или ОплатаПоДГПХПоВедомостям);
			КонецЕсли;
		КонецЕсли;
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеСРасчетногоСчета") Тогда
			Если ЭтоОбъект Тогда
	...
	ИначеЕсли (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.НачислениеЗарплаты")
		...
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РегистрацияОблагаемойБазыРуководителя"))
		И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН Тогда
...
Процедура ОтразитьВУСН(ДокументСсылка, ВидОперации = Неопределено, Параметры = Неопределено, ЭтапПроведения = 0, СтруктураДопПараметров = Неопределено, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ВидОперации = "РеализацияКомиссионером"
		...
		Если ВидОперации = "Реализация" Тогда
			...
			УдерживаетсяВознаграждениеДокументом =
				ОбщегоНазначения.ЕстьРеквизитОбъекта("УдержатьВознаграждение", МетаданныеДокумента)
				И ДокументСсылка.УдержатьВознаграждение;
			Если УдерживаетсяВознаграждениеДокументом Тогда
			...
			УчестьВозвратыВВыручке = ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
				...
				И (ОтражатьДоходыНаДатуОтчетаКомиссионера(СтруктураШапкиДокумента.Организация, СтруктураШапкиДокумента.Дата)
					Или УдерживаетсяВознаграждениеДокументом);
			Если УчестьВозвратыВВыручке Тогда
				СуммаВозвратов = 0;
	...
	ИначеЕсли ВидОперации = "КорректировкаРеализации" Тогда
		...
		ЭтоОтгрузкаБезПереходаПраваСобственности = Параметры.Реквизиты[0].ЭтоОтгрузкаБезПереходаПраваСобственности;
		
		ОчищатьРасходыУСН = Параметры.Реквизиты[0].ЭтоКорректировкаПрошлогоПериода
			И СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы;
		СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН = Параметры.Реквизиты[0].ПрименяетсяУСНДоходыМинусРасходы;
		СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы = Не СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН;
			
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН 
			И НЕ ЭтоКомиссионер
			И Не ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
		...
		ИначеЕсли СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН
			И ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
			...
			ТаблицаПартий = ПодготовитьТаблицуПартий(СтруктураДопПараметров.ТаблицаВозвратОтгрузкаБезППС, "ВозвратСРеализации");
			СтатусПартии  = Перечисления.СтатусыПартийУСН.НаРеализации;
		...
		Если ТаблицаРасчетов <> Неопределено И ТаблицаРасчетов.Количество() > 0
			И Не ЭтоОтгрузкаБезПереходаПраваСобственности Тогда
			Если ТаблицаРасчетов.Итог("ОстатокВзаиморасчетов") > 0 Тогда
		...
		Если СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН 
			И НЕ (ЭтоКомиссионер Или ЭтоОтгрузкаБезПереходаПраваСобственности) Тогда
		...
		Если ДвиженияРегистровВх <> Неопределено И ОчищатьРасходыУСН Тогда
			ДвиженияРегистровВх.РасходыУСН.Очистить();
		КонецЕсли;
		
		Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН Тогда
			УчестьНДСВДоходахПриКорректировкеРеализации(СтруктураШапкиДокумента, КУДиР, Параметры.Реквизиты[0], ТаблицаРасчетов);
		КонецЕсли;
	...
	ИначеЕсли ВидОперации = "ОплатаОтЭквайераБезЗакрывающихДокументов" Тогда
		...
		Для Каждого СтрокаТаблицы Из ДокументСсылка.РасшифровкаПлатежа Цикл
			...
			Если СтрокаТаблицы.ПорядокОтраженияАванса = Перечисления.ПорядокОтраженияАвансов.ДоходУСН Тогда
				...
				Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН Тогда
					ИсключенныйНДС  = УчетНДСКлиентСервер.РассчитатьСуммуНДС(СуммаПлатежа + СуммаУслуг, Истина, 
						УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(СтруктураШапкиДокумента.СтавкаНДСПриУСН));
					...
					НоваяСтрокаРасшифровкаПоДоговорам.ИсключенныйНДС = ИсключенныйНДС;
					НоваяСтрокаРасшифровкаПоДоговорам.Графа4 = СуммаПлатежа;
					НоваяСтрокаРасшифровкаПоДоговорам.Графа5 = СуммаПлатежа + СуммаУслуг - ИсключенныйНДС;
				КонецЕсли;
...
Процедура СписаниеРасходовУСН(СтруктураШапкиДокумента, ТаблицаПартий, ДвиженияРегистров, Договор, СтатусПартии, ВидОперации = "Списание")
	...
	Если ТаблицаДляСписания.Количество() > 0
		...
		Если ВидОперации = "Реализация" Или ВидОперации = "Розница"
			Или (ВидОперации = "Списание" И СтруктураШапкиДокумента.ВидДокумента = "ТребованиеНакладная") Тогда
...
Процедура РасчетыСКонтрагентами_ЗачетАвансаОтПокупателяУСН(СтруктураШапкиДокумента, ТаблицаАвансов, ТаблицаДвижений, ДвиженияРегистровВх, ДвиженияРегистров, ДвиженияКУДиРПатент, КУДиР, Выручка)
	...
	Для Каждого СтрокаЗачета Из ТаблицаЗачетАвансаУСНПоДоговорам Цикл
		...
		Если СтруктураШапкиДокумента.ВидОперацииУСН = "КорректировкаРеализации" Тогда
			// Исключение и включение НДС в доход при корректировке реализации см. процедуру
			// УчестьНДСВДоходахПриКорректировкеРеализации
			ИсключитьНДС_УСН = 0;
			ВключитьНДС_УСН  = 0;
		Иначе
...
Процедура РозничнаяВыручкаЗачетАвансов(СтруктураШапкиДокумента, ТаблицаАвансов, ДвиженияКУДиРПатент, КУДиР, Выручка, ДоговорДляОтложенногоПроведения)
	...
	Для Каждого Строка Из ТаблицаВзаиморасчетов Цикл
		...
		Если СуммаКомиссии > 0 Тогда
			Коэффициент = СуммаКомиссии / СуммаВсего;
			СуммаКоррДоходов = Окр(Строка.СуммаРуб * Коэффициент, 2, 1);
			
			// Если аванс получен с НДС, то необходимо скорректировать сумму комисси на сумму НДС
			// полученного аванса
			Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
				И ТипЗнч(Строка.РасчетныйДокумент) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
				
				РеквизитыПредоплаты =
					ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Строка.РасчетныйДокумент, "ПодарочныеСертификаты, Предоплаты, Дата");
				ДатаДокументаПродажи = РеквизитыПредоплаты.Дата;
				
				СуммаКорректировкиНДС = 0;
				ПредоплатыКЗачету = Новый ТаблицаЗначений;
				Если Строка.ТипОплаты = Перечисления.ТипыОплат.РасчетыСРозничнымиПокупателями Тогда
					Предоплаты = РеквизитыПредоплаты.Предоплаты.Выгрузить();
					Отбор = Новый Структура("Контрагент, ДоговорКонтрагента");
					ЗаполнитьЗначенияСвойств(Отбор, Строка);
					ПредоплатыКЗачету = Предоплаты.НайтиСтроки(Отбор);
				ИначеЕсли Строка.ТипОплаты = Перечисления.ТипыОплат.ПодарочныйСертификатСобственный Тогда
					ПредоплатыКЗачету = РеквизитыПредоплаты.ПодарочныеСертификаты.Выгрузить();
					Для Каждого Сертификат Из ПредоплатыКЗачету Цикл
						ТипСертификата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Сертификат.ВидОплаты, "ТипОплаты");
						Если Не ТипСертификата = Строка.ТипОплаты Тогда
							ПредоплатыКЗачету.Удалить(Сертификат);
						КонецЕсли;
					КонецЦикла;
					ПредоплатыКЗачету.Колонки.Добавить("СтавкаНДС", Новый ОписаниеТипов("ПеречислениеСсылка.СтавкиНДС"));
					ВидСтавки = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(
						СтруктураШапкиДокумента.Организация, ДатаДокументаПродажи);
					ПредоплатыКЗачету.ЗаполнитьЗначения(ВидСтавки, "СтавкаНДС");
				КонецЕсли;
				
				Для Каждого ПредоплатаКЗачету Из ПредоплатыКЗачету Цикл
					СтавкаНДС = УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(ПредоплатаКЗачету.СтавкаНДС);
					СуммаНДСПредоплаты = УчетНДСКлиентСервер.РассчитатьСуммуНДС(ПредоплатаКЗачету.Сумма, Истина, СтавкаНДС);
					Если СуммаКоррДоходов >= ПредоплатаКЗачету.Сумма Тогда
						НДСИзТекущейПредоплаты = СуммаНДСПредоплаты;
					Иначе
						НДСИзТекущейПредоплаты = Окр(СуммаНДСПредоплаты * Коэффициент, 2);
					КонецЕсли;
					СуммаКорректировкиНДС = СуммаКорректировкиНДС + НДСИзТекущейПредоплаты;
				КонецЦикла;
				СуммаКоррДоходов = Макс(0, СуммаКоррДоходов - СуммаКорректировкиНДС);
			КонецЕсли;
		...
		// Зачет переплаты банка, отраженного в Прочих расчетах документом Поступление на расчетный счет.
		// В этом случае необходимо исключить НДС, который не исключен из дохода при поступлении денег.
		СуммаУСН = Макс(0,
			Строка.СуммаРуб - Строка.СуммаРуб_Комитента - Строка.СуммаРуб_Патент - Строка.СуммаРуб_ТорговыйСбор);
		Если СтруктураШапкиДокумента.ЭтоПлательщикНДСПрименяющийУСН
			И СуммаУСН > 0 И Строка.ТипОплаты = Перечисления.ТипыОплат.ПлатежнаяКарта Тогда
			СуммаИсключаемогоНДС = ИсключаемаяСуммаНДС(СтруктураШапкиДокумента, СтруктураШапкиДокумента.Ссылка, СуммаУСН);
			Если СуммаИсключаемогоНДС > 0 Тогда
				КУДиР.Графа5 = КУДиР.Графа5 - СуммаИсключаемогоНДС;
				СтрокаРасшифровки.Графа5 = СтрокаРасшифровки.Графа5 - СуммаИсключаемогоНДС;
				СодержаниеОперации = СтрШаблон(
					НСтр("ru = 'Предъявленный покупателям НДС в сумме %1 руб. исключен из доходов.'"),
					ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаИсключаемогоНДС, , , " "));
				ДополнитьСодержаниеКУДиРПояснениемПоИсключениюНДС(КУДиР, СуммаИсключаемогоНДС);
				СтрокаРасшифровки.Пояснение = СодержаниеОперации;
			КонецЕсли;
		КонецЕсли;
		...
		Если РаспределятьРасходыПоСпособамУчетаНДС Тогда
			...
			Если НДСВПартии > 0 И НДСРасход > 0 Тогда
				Для Каждого СтрокаСпособаУчетаНДС Из НайденныеСтрокиСпособУчетаНДС Цикл
					...
					СтрокаСпособаУчетаНДС.НДС = СтрокаСпособаУчетаНДС.НДС - СтрокаСпособаУчетаНДС.НДС * КоэффициентРасхода;
				КонецЦикла;
			Иначе
				ОбработатьИзменениеСтатусаРасхода(СтруктураШапкиДокумента, ПараметрыИзмененияСтатусаРасхода);
			КонецЕсли;
...
Процедура ЗаполнитьПризнакВыделятьНДСУСНДокумент(СтруктураШапкиДокумента, ТаблицаРасходов, ВыделятьНДСУСН)
	...
	Для Каждого Расход Из ТаблицаРасходов Цикл
		...
		Если Отказ Или (ДатаАнализируемогоДокумента <> Неопределено И ДатаАнализируемогоДокумента < ДатаПереходаНаУСН) Тогда
		...
		Иначе
			...
			Если ПорядокПризнания = Неопределено Тогда
				ПорядокПризнания = УчетнаяПолитика.ПорядокПризнанияРасходовПоНДС(
					СтруктураШапкиДокумента.Организация, ДатаАнализируемогоДокумента);
...
Функция ИсключаемаяСуммаНДС(СтруктураШапкиДокумента, Ссылка, СуммаДокумента)
	...
	Если СтруктураШапкиДокумента.ВидДокумента = "ПоступлениеНаРасчетныйСчет" Тогда
...
Процедура УчестьНДСВДоходахПриКорректировкеРеализации(СтруктураШапкиДокумента, КУДиР, РеквизитыДокумента, ТаблицаРасчетов)
	
	СуммаКорректировкиДохода = 0;
	СуммаДокументаПослеИзменения = 0;
	
	СчетаТоваровКомитентов = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ТоварыПринятыеНаКомиссию);
	Для Каждого СтрокаТовар Из СтруктураШапкиДокумента.Объект.Товары Цикл
		Если СчетаТоваровКомитентов.Найти(СтрокаТовар.СчетУчета) = Неопределено Тогда
			КоэффициентНДСДоИзменения =
				?(СтрокаТовар.СуммаДоИзменения = 0, 0, СтрокаТовар.СуммаНДСДоИзменения / СтрокаТовар.СуммаДоИзменения);
			КоэффициентНДС = ?(СтрокаТовар.Сумма = 0, 0, СтрокаТовар.СуммаНДС / СтрокаТовар.Сумма);
			СуммаДокументаПослеИзменения = СуммаДокументаПослеИзменения + СтрокаТовар.Сумма;
			Если КоэффициентНДСДоИзменения <> КоэффициентНДС Тогда
				СуммаКорректировкиДохода = СуммаКорректировкиДохода
					+ (Окр(СтрокаТовар.Сумма * КоэффициентНДСДоИзменения, 2) - СтрокаТовар.СуммаНДС);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаУслуга Из СтруктураШапкиДокумента.Объект.Услуги Цикл
		КоэффициентНДСДоИзменения =
			?(СтрокаУслуга.СуммаДоИзменения = 0, 0, СтрокаУслуга.СуммаНДСДоИзменения / СтрокаУслуга.СуммаДоИзменения);
		КоэффициентНДС = ?(СтрокаУслуга.Сумма = 0, 0, СтрокаУслуга.СуммаНДС / СтрокаУслуга.Сумма);
			СуммаДокументаПослеИзменения = СуммаДокументаПослеИзменения + СтрокаУслуга.Сумма;
		Если КоэффициентНДС <> КоэффициентНДСДоИзменения Тогда
			СуммаКорректировкиДохода = СуммаКорректировкиДохода
				+ (Окр(СтрокаУслуга.Сумма * КоэффициентНДСДоИзменения, 2) - СтрокаУслуга.СуммаНДС);
		КонецЕсли;
	КонецЦикла;
	
	Если СуммаКорректировкиДохода <> 0 Тогда
		
		// Получим таблицу взаиморасчетов
		Отказ = Ложь;
		РасчетыВВалюте = СтруктураШапкиДокумента.ВалютаДокумента <> СтруктураШапкиДокумента.ВалютаРегламентированногоУчета;
		Расшифровка = НоваяТаблицаРасшифровка();
		СтрокаДанныхРасшифровки = Расшифровка.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДанныхРасшифровки, СтруктураШапкиДокумента.Объект);
		СтрокаДанныхРасшифровки.ВидДоговора =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДанныхРасшифровки.ДоговорКонтрагента, "ВидДоговора");
		СтрокаДанныхРасшифровки.РасчетыВВалюте = РасчетыВВалюте;
		СтрокаДанныхРасшифровки.СпособПогашенияЗадолженности = Перечисления.СпособыПогашенияЗадолженности.Автоматически;
		СтрокаДанныхРасшифровки.СчетРасчетов = СтруктураШапкиДокумента.Объект.СчетУчетаРасчетовСКонтрагентом;
		
		Реквизиты = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(РеквизитыДокумента);
		Реквизиты.Вставить("УчитыватьЗадолженностьУСН", Реквизиты.ПрименяетсяУСН);
		Реквизиты.Вставить("УчитыватьЗадолженностьУСНПатент", Ложь);
		Реквизиты.Вставить("ЭтоВозврат", Ложь);
		Реквизиты.Вставить("НаправлениеДвижения", "");
		ТаблицаВзаиморасчеты = УчетВзаиморасчетов.ПолучитьОстаткиЗадолженности(Расшифровка, Реквизиты, Отказ);
		СтрокаРеализации = ТаблицаВзаиморасчеты.Найти(СтруктураШапкиДокумента.Объект.ДокументРеализации);
		СуммаПоРеализации = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
			СтруктураШапкиДокумента.Объект.ДокументРеализации, "СуммаДокумента");
		Если СтрокаРеализации <> Неопределено Тогда
			СуммаОплачено = Макс(0, СуммаПоРеализации - СтрокаРеализации.ОстатокВзаиморасчетов);
			КоэффициентНДС = ?(СуммаДокументаПослеИзменения = 0, 0, СуммаОплачено / СуммаДокументаПослеИзменения);
			СуммаКорректировкиДохода = Окр(СуммаКорректировкиДохода * КоэффициентНДС, 2);
		ИначеЕсли ТаблицаРасчетов <> Неопределено Тогда
			СтрокаЗадолженности = ТаблицаРасчетов.Найти(СтруктураШапкиДокумента.Ссылка);
			Если СтрокаЗадолженности <> Неопределено Тогда
				КоэффициентЗадолженности = ?(СуммаДокументаПослеИзменения = 0, 1,
					(СуммаДокументаПослеИзменения - СтрокаЗадолженности.СуммаВзаиморасчетов) / СуммаДокументаПослеИзменения);
				СуммаКорректировкиДохода = Окр(СуммаКорректировкиДохода * КоэффициентЗадолженности, 2);
			КонецЕсли;
		КонецЕсли;
		
		Если РасчетыВВалюте Тогда
			ДокументРеализацииСсылка = СтруктураШапкиДокумента.Объект.ДокументРеализации;
			МетаданныеДокумента = ДокументРеализацииСсылка.Метаданные();
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("КурсВзаиморасчетов", МетаданныеДокумента) Тогда
				КурсВзаиморасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументРеализацииСсылка, "КурсВзаиморасчетов");
			Иначе
				КурсВзаиморасчетов = 1;
			КонецЕсли;
			
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("КратностьВзаиморасчетов", МетаданныеДокумента) Тогда
				КратностьВзаиморасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ДокументРеализацииСсылка, "КратностьВзаиморасчетов");
			Иначе
				КратностьВзаиморасчетов = 1;
			КонецЕсли;
			
			ПараметрыТекущегоКурса = Новый Структура("Валюта, Курс, Кратность",
				СтруктураШапкиДокумента.ВалютаДокумента, КурсВзаиморасчетов, КратностьВзаиморасчетов);
			
			ПараметрыНовогоКурса = Новый Структура("Валюта, Курс, Кратность",
				СтруктураШапкиДокумента.ВалютаРегламентированногоУчета, 1, 1);
		
			СуммаКорректировкиДохода = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(
				СуммаКорректировкиДохода, ПараметрыТекущегоКурса, ПараметрыНовогоКурса);
		КонецЕсли;
		
		Если СуммаКорректировкиДохода <> 0 Тогда
			КУДиР.Графа5 = КУДиР.Графа5 + СуммаКорректировкиДохода;
			
			// Добавляем расшифровку по договорам.
			СтрокаРасшифровки = КУДиР.РасшифровкаПоДоговорам.Добавить();
			СтрокаРасшифровки.Контрагент = СтруктураШапкиДокумента.Объект.Контрагент;
			СтрокаРасшифровки.ДоговорКонтрагента = СтруктураШапкиДокумента.Объект.ДоговорКонтрагента;
			СтрокаРасшифровки.Графа5 = СуммаКорректировкиДохода;
				
			Если СуммаКорректировкиДохода < 0 Тогда
				ШаблонПояснениеНДС = НСтр("ru = 'Предъявленный покупателям НДС в сумме %1 руб. исключен из доходов.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
				СодержаниеОперации = СтрШаблон(ШаблонПояснениеНДС,
					ОбщегоНазначенияБПВызовСервера.ФорматСумм(- СуммаКорректировкиДохода, , , " "));
			Иначе
				ШаблонПояснениеНДС = НСтр(
					"ru = 'Исключенный ранее НДС, предъявленный покупателям, в сумме %1 руб. включен в доходы.'",
					ОбщегоНазначения.КодОсновногоЯзыка());
				СодержаниеОперации = СтрШаблон(ШаблонПояснениеНДС,
					ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаКорректировкиДохода, , , " "));
			КонецЕсли;
			СтрокаРасшифровки.Пояснение = СодержаниеОперации;
			КУДиР.ЧастиСодержания.Пояснение = СодержаниеОперации;
		КонецЕсли;
...
Функция НоваяТаблицаРасшифровка()
	
	Расшифровка = Новый ТаблицаЗначений;
	Расшифровка.Колонки.Добавить("СчетРасчетов", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	Расшифровка.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
	Расшифровка.Колонки.Добавить("ДоговорКонтрагента", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	Расшифровка.Колонки.Добавить("СпособПогашенияЗадолженности",
		Новый ОписаниеТипов("ПеречислениеСсылка.СпособыПогашенияЗадолженности"));
	Расшифровка.Колонки.Добавить("РасчетыВВалюте", Новый ОписаниеТипов("Булево"));
	Расшифровка.Колонки.Добавить("ВидДоговора", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыДоговоровКонтрагентов"));
