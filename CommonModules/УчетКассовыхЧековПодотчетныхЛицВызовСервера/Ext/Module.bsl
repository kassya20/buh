...
Функция ДанныеВыбораНоменклатуры(Параметры, НоменклатураНаименование) Экспорт
	...
	ДанныеВыбора = РаботаСНоменклатуройВызовСервераБП.ДанныеВыбораНоменклатуры(Параметры, Ложь, Истина);
...
// Получает данные выбора контрагента для события автоподбора поставщика.
//
// Параметры:
//  Параметры - Структура - см. УчетКассовыхЧековПодотчетныхЛицКлиент.ПараметрыАвтоПодбораПоставщикаНовый
//  Наименование - Строка - наименование поставщика из чека
//  ИНН - Строка - ИНН поставщика из чека
// 
// Возвращаемое значение:
//  СписокЗначений - варианты создания контрагента по ИНН
//
Функция ДанныеВыбораКонтратента(Параметры, Наименование, ИНН) Экспорт
	
	ДанныеВыбора = Справочники.Контрагенты.ПолучитьДанныеВыбора(Параметры);
	
	Если ДанныеВыбора.Количество() = 0 Тогда
		ДобавитьПоставщикаВДанныеВыбора(ДанныеВыбора, Наименование, ИНН)
	КонецЕсли;
...
Функция ПодключенныеИсточникиКассовыхЧеков() Экспорт
	...
	Возврат УчетКассовыхЧековПодотчетныхЛиц.ПодключенныеИсточникиКассовыхЧеков();
...
Функция СведенияПоставщиковКассовыхЧеков(КассовыеЧеки) Экспорт
	...
	Возврат УчетКассовыхЧековПодотчетныхЛиц.СведенияПоставщиковКассовыхЧеков(КассовыеЧеки);
...
Функция НачатьОбновлениеЧековКЗагрузке(Знач АдресДанныеФормы) Экспорт
	
	Возврат УчетКассовыхЧековПодотчетныхЛиц.ОбновитьЧекиКЗагрузке(АдресДанныеФормы);
...
Процедура ДобавитьПоставщикаВДанныеВыбора(ДанныеВыбора, Наименование, ИНН)
	
	Если Не ЗначениеЗаполнено(Наименование) Или Не ЗначениеЗаполнено(ИНН) Тогда
		Возврат;
	КонецЕсли;
	...
	Представление = Новый ФорматированнаяСтрока(
		Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru='Создать: %1, ИНН: '"), Наименование), ШрифтКоманды),
		Новый ФорматированнаяСтрока(СокрЛП(ИНН), ШрифтПодСтроки, ЦветПодстроки));
		
	ДанныеВыбора.Добавить(ИНН, Представление);
...
Функция НайтиОбъектыПоНаименованию(Знач Наименование, Знач МетаданныеОбъекта)
	...
	Запрос.УстановитьПараметр("Наименование", СтрокаДляПоискаВЗапросе(Наименование) + "%");
...
Функция СтрокаДляПоискаВЗапросе(Знач СтрокаПоиска) Экспорт
	
	// Если в качестве сервера БД используется PostgreSQL, необходимо убрать спец. символ,
	// который сервер БД не умеет экранировать в запросах с конструкцией ПОДОБНО.
