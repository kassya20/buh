...
// Управляет видимостью подсказки для поля выбора отражения аванса в доходах.
//
// Параметры:
//  Форма      - ФормаКлиентскогоПриложения - настраиваемая форма документа поступления денег.
//  НазваниеТабличнойЧасти - Строка, Неопределено - имя поля, отображающего данные табличной части РасшифровкаПлатежа.
//               См. например, ПриходныйКассовыйОрдерФормыКлиентСервер.НазваниеТаблицыРасшифровкиПлатежа
//
Процедура УстановитьВидимостьПодсказкиОтраженияАванса(Форма, НазваниеТабличнойЧасти) Экспорт
	...
	// Непосредственно на основной форме, без формы расшифровки
	НепосредственноеРедактирование = (Форма.Объект.РасшифровкаПлатежа.Количество() <= 1);
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		Форма, "ОтражениеАвансаПодсказкаВидимость") Тогда
		
		Если Форма.ОтражениеАвансаПодсказкаВидимость И НепосредственноеРедактирование Тогда  
			ОтображениеПодсказкиЭлемента = ОтображениеПодсказки.Кнопка;
		Иначе
			ОтображениеПодсказкиЭлемента = ОтображениеПодсказки.Нет;
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			СтрШаблон("%1%2", НазваниеТабличнойЧасти, "ОтражениеАвансаПредставление"),
...
// Возвращает признак видимости / доступности реквизита формы "КонтрагентЮрФизЛицо"
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма документа
//
// Возвращаемое значение:
//  Булево - Истина, если виден / доступен реквизит формы "КонтрагентЮрФизЛицо"
//
Функция ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма) Экспорт 
	
	Объект = Форма.Объект;
	
	Возврат ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "КонтрагентЮрФизЛицо")
		И (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДивидендов")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВзносВУставныйКапитал")
