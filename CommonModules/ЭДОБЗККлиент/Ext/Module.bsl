...
Процедура ПечатьДокументовВыполнитьКоманду(Форма, Команда, ПродолжитьВыполнениеНаСервере, ДополнительныеПараметры) Экспорт
	...
	ИнтеграцияСРаботаРоссииКлиент.ПечатьДокументовВыполнитьКоманду(Форма, Команда, ПродолжитьВыполнениеНаСервере, ДополнительныеПараметры);
...
Процедура ПечатьДокументовПослеОткрытия(Форма) Экспорт
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПечатнаяФормаНедоступна Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ПередатьПодписанныеPDFНаРаботаРоссии",
			"Доступность",
			Ложь);
...
Процедура ПодписатьПечатныеФомыПриПечатиПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		Если Результат.Значение = "ПередатьВсе"
			...
			Если ЗначениеЗаполнено(ПечатныеФормыОбъектов) Тогда
	...
	Если РезультатПодписания <> Неопределено Тогда
		...
		Иначе
			...
			Если ОбрабатываемыеФайлы.Количество() > 0 Тогда
				...
				ИначеЕсли ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСФайламиДокументовЭДОБЗК.ПередатьНаРаботаРоссии") Тогда
					
					ИнтеграцияСРаботаРоссииКлиент.ПередатьДокументы(ОбрабатываемыеФайлы);
...
Процедура НачатьПроверкуПодписей(ДанныеДляПроверкиПодписей, Индекс) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеЭлектронногоДокумента.ПодписиИБ) Тогда
		ПроверитьПодписи(ДополнительныеПараметры.СсылкаНаДвоичныеДанные, ДополнительныеПараметры);
...
Процедура ПроверитьПодписи(СсылкаНаДвоичныеДанные, ДополнительныеПараметры)
	
	Если Не ДополнительныеПараметры.Свойство("ИндексТекущейПодписиИБ") Тогда
		ДополнительныеПараметры.Вставить("ИндексТекущейПодписиИБ", 0);
	КонецЕсли;
	Если Не ДополнительныеПараметры.Свойство("ИндексТекущейПодписиСервиса") Тогда
		ДополнительныеПараметры.Вставить("ИндексТекущейПодписиСервиса", 0);
	КонецЕсли;
	
	Если ДополнительныеПараметры.ИндексТекущейПодписиИБ < ДополнительныеПараметры.ПодписиИБ.Количество() Тогда
		...
		ЭлектроннаяПодписьКлиент.ПроверитьПодпись(Оповещение, СсылкаНаДвоичныеДанные,
			ДополнительныеПараметры.ПодписиИБ[ДополнительныеПараметры.ИндексТекущейПодписиИБ].АдресПодписи, , ,
			ДополнительныеПараметры.ПараметрыПроверки);
	ИначеЕсли ДополнительныеПараметры.ИндексТекущейПодписиСервиса < ДополнительныеПараметры.ПодписиСервиса.Количество() Тогда
		ДополнительныеПараметры.ПараметрыПроверки.ПроверятьСертификат = ЭлектроннаяПодписьСлужебныйКлиентСервер.НеПроверятьСертификат();
		Оповещение = Новый ОписаниеОповещения("ПослеПроверкиПодписиСервиса", ЭтотОбъект, ДополнительныеПараметры);
		ЭлектроннаяПодписьКлиент.ПроверитьПодпись(Оповещение, СсылкаНаДвоичныеДанные,
			ДополнительныеПараметры.ПодписиСервиса[ДополнительныеПараметры.ИндексТекущейПодписиСервиса].АдресПодписи, , ,
			ДополнительныеПараметры.ПараметрыПроверки);
	КонецЕсли;
...
Процедура ПослеПроверкиПодписиИБ(Результат, ДополнительныеПараметры) Экспорт
	ПослеПроверкиПодписи(Результат, ДополнительныеПараметры, Истина);
	ДополнительныеПараметры.ИндексТекущейПодписиИБ = ДополнительныеПараметры.ИндексТекущейПодписиИБ + 1;
	Если ДополнительныеПараметры.ИндексТекущейПодписиИБ < ДополнительныеПараметры.ПодписиИБ.Количество() Тогда
		ПроверитьПодписи(ДополнительныеПараметры.СсылкаНаДвоичныеДанные, ДополнительныеПараметры)
	ИначеЕсли ДополнительныеПараметры.ИндексТекущейПодписиСервиса < ДополнительныеПараметры.ПодписиСервиса.Количество() Тогда
		ПроверитьПодписи(ДополнительныеПараметры.СсылкаНаДвоичныеДанные, ДополнительныеПараметры)
...
Процедура ПослеПроверкиПодписиСервиса(Результат, ДополнительныеПараметры) Экспорт
	ПослеПроверкиПодписи(Результат, ДополнительныеПараметры, Ложь);
	ДополнительныеПараметры.ИндексТекущейПодписиСервиса = ДополнительныеПараметры.ИндексТекущейПодписиСервиса + 1;
	Если ДополнительныеПараметры.ИндексТекущейПодписиСервиса < ДополнительныеПараметры.ПодписиСервиса.Количество() Тогда
		ПроверитьПодписи(ДополнительныеПараметры.СсылкаНаДвоичныеДанные, ДополнительныеПараметры)
	Иначе
		ПослеПроверкиПодписей(Результат, ДополнительныеПараметры);
	КонецЕсли;
...
Процедура ПослеПроверкиПодписи(Результат, ДополнительныеПараметры, ЭтоПодписьИБ)
	
	Если ЭтоПодписьИБ Тогда
		ДанныеСтроки = ДополнительныеПараметры.ПодписиИБ[ДополнительныеПараметры.ИндексТекущейПодписиИБ];
	Иначе
		ДанныеСтроки = ДополнительныеПараметры.ПодписиСервиса[ДополнительныеПараметры.ИндексТекущейПодписиСервиса];
	КонецЕсли;
