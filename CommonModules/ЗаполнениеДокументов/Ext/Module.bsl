...
Процедура Заполнить(ДокументОбъект, ДанныеЗаполнения = Неопределено, ЗаполнятьСчетаУчета = Ложь) Экспорт
	...
	ТипСсылкиДокумента = ТипЗнч(ДокументОбъект.Ссылка);
	...
	ЕстьРеквизитЭтоУниверсальныйДокумент = ОбщегоНазначения.ЕстьРеквизитОбъекта("ЭтоУниверсальныйДокумент", 
		МетаданныеДокумента);
	ЕстьРеквизитНДСНеВыделять        = ОбщегоНазначения.ЕстьРеквизитОбъекта("НДСНеВыделять",        МетаданныеДокумента);
	...
	Если ЕстьРеквизитДоговор
		...
		Если СвойстваДоговора <> Неопределено Тогда
			Если ЕстьРеквизитВалютаДокумента Тогда
				ДокументОбъект.ВалютаДокумента = ?(СвойстваДоговора.РасчетыВУсловныхЕдиницах = Истина
	...
	ПлательщикНДСПрименяющийУСН = ЕстьРеквизитОрганизация И УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ДокументОбъект.Организация, ДокументОбъект.Дата);
	ПлательщикНДСНаУСНСВычетами = ЕстьРеквизитОрганизация И УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(ДокументОбъект.Организация, ДокументОбъект.Дата);
	...
	Если ЕстьРеквизитВидОперации
		И ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитДокумента("ВидОперации", ДокументОбъект, МетаданныеДокумента) Тогда
		ВидОперацииМенеджер = Перечисления[ДокументОбъект.ВидОперации.Метаданные().Имя];
		Если СвойстваДоговора <> Неопределено
			И РаботаСДоговорамиКонтрагентовБПВызовСервера.ЭтоДокументСДоговоромПоВидуОперации(ТипСсылкиДокумента) Тогда
			ВидыОпераций = РаботаСДоговорамиКонтрагентовБПВызовСервера.ВидыОперацийДокумента(
				СвойстваДоговора.ВидДоговора, ТипСсылкиДокумента);
			Если ВидыОпераций.Количество() > 0 Тогда
				ДокументОбъект.ВидОперации = ВидыОпераций[0];
			КонецЕсли;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("НДСВключенВСтоимость", МетаданныеДокумента) Тогда
		ДокументОбъект.НДСВключенВСтоимость = НЕ ПлательщикНДС И НЕ ПлательщикНДСНаУСНСВычетами
			...
			Или (СвойстваДоговора <> Неопределено
				И (БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемТоваровУслуг(ДокументОбъект.Ссылка)
					Или БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемДопРасходов(ДокументОбъект.Ссылка))
				И Не УчетНДСБП.ВыделятьНДСПоУмолчанию(СвойстваДоговора.НалогообложениеКонтрагента));
	КонецЕсли;
	
	НеобходимоНачислениеНДС = ПлательщикНДС Или ПлательщикНДСПрименяющийУСН;
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокументБезНДС", МетаданныеДокумента)
		...
		ДокументОбъект.ДокументБезНДС = НЕ НеобходимоНачислениеНДС;
	КонецЕсли;
	
	Если СвойстваДоговора <> Неопределено И ЕстьРеквизитНДСНеВыделять
		И (БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемТоваровУслуг(ДокументОбъект.Ссылка)
		Или БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемДопРасходов(ДокументОбъект.Ссылка))
		И Не УчетНДСБП.ВыделятьНДСПоУмолчанию(СвойстваДоговора.НалогообложениеКонтрагента) Тогда
		НДСНеВыделять = Истина;
		ДокументОбъект.НДСНеВыделять = НДСНеВыделять;
	ИначеЕсли ЕстьРеквизитОрганизация И ЕстьРеквизитНДСНеВыделять
		И Не ТипДанныхЗаполнения = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
		И Не (ТипДанныхЗаполнения = Тип("Структура") И ДанныеЗаполнения.Свойство("Основание")
		И ТипЗнч(ДанныеЗаполнения.Основание) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")) Тогда
	...
	Если ЗначениеЗаполнено(ИмяРеквизитаСтавкаНДС) И НЕ ЗначениеЗаполнено(ДокументОбъект[ИмяРеквизитаСтавкаНДС]) Тогда
		Если МетаданныеДокумента.Имя = "ПлатежноеТребование" Тогда
			Если ПлательщикНДСПрименяющийУСН Тогда
				СтавкаНДСПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(ДокументОбъект.Организация, ДокументОбъект.Дата);
				Если СтавкаНДСПриУСН = Перечисления.ВидыСтавокНДС.Общая Тогда
					ДокументОбъект[ИмяРеквизитаСтавкаНДС] = УчетНДСКлиентСервер.СтавкаНДСПользователя(ДокументОбъект.Дата, Истина);
				Иначе
					ДокументОбъект[ИмяРеквизитаСтавкаНДС] = Перечисления.СтавкиНДС.СтавкаНДС(СтавкаНДСПриУСН, ДокументОбъект.Дата);
				КонецЕсли;
			Иначе
				ДокументОбъект[ИмяРеквизитаСтавкаНДС] = УчетНДСКлиентСервер.СтавкаНДСПользователя(ДокументОбъект.Дата, ПлательщикНДС);
			КонецЕсли;
		Иначе
			Если БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяРеализацией(ДокументОбъект.Ссылка) Тогда
				ДокументОбъект[ИмяРеквизитаСтавкаНДС] = УчетНДСКлиентСервер.СтавкаНДСПриРеализации(ДокументОбъект.Дата,
				ДокументОбъект.Организация);
			Иначе
				ДокументОбъект[ИмяРеквизитаСтавкаНДС] = УчетНДСКлиентСервер.СтавкаНДСПользователя(
				ДокументОбъект.Дата, (ПлательщикНДС ИЛИ ПлательщикНДСНаУСНСВычетами) И НЕ НДСНеВыделять);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ДеятельностьНаПатенте", МетаданныеДокумента) Тогда
		...
		Если ТипДанныхЗаполнения = Тип("Структура") И ДанныеЗаполнения.Свойство("ДеятельностьНаПатенте") Тогда
			...
			ИначеЕсли Не ДанныеЗаполнения.Свойство("Патент") И Не ЗначениеЗаполнено(ДокументОбъект.Патент) Тогда
				УчетПСН.УстановитьПатентПриРеализации(ДокументОбъект, Истина);
			КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиДвиженияДенежныхСредств")
		...
		ДокументОбъект.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ДокументОбъект.ВидОперации, ДокументОбъект.Дата);
	КонецЕсли;
	...
	Если ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитДокумента("ТипЦен", ДокументОбъект, МетаданныеДокумента) Тогда
		
		Ценообразование.ЗаполнитьТипЦенПоУмолчанию(ДокументОбъект, МетаданныеДокумента);
	...
	Если ОбщегоНазначенияБП.ЕстьТабЧастьДокумента("РасшифровкаПлатежа", МетаданныеДокумента) Тогда
		Если ДокументОбъект.РасшифровкаПлатежа.Количество() = 0 Тогда
			...
			Если ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитТабЧастиДокумента("СтавкаНДС", СтрокаПлатеж, МетаданныеДокумента, "РасшифровкаПлатежа") Тогда
				Если ЕстьРеквизитОрганизация И ЗначениеЗаполнено(ДокументОбъект.Организация)
					...
					ВидДоговораВзаиморасчетов = РаботаСДоговорамиКонтрагентовБПВызовСервера.ВидыДоговоровДокумента(ДокументОбъект.ВидОперации);
					ЗаполнятьНДС = ВидДоговораВзаиморасчетов.Количество() <> 0;
					Если Не ЗаполнятьНДС Тогда
						СтрокаПлатеж.СтавкаНДС = Неопределено;
					ИначеЕсли ПлательщикНДСПрименяющийУСН Тогда
						СтавкаНДСПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(ДокументОбъект.Организация, ДокументОбъект.Дата);
						Если СтавкаНДСПриУСН = Перечисления.ВидыСтавокНДС.Общая Тогда
							СтрокаПлатеж.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(ДокументОбъект.Дата, Истина);
						Иначе
							СтрокаПлатеж.СтавкаНДС = Перечисления.СтавкиНДС.СтавкаНДС(СтавкаНДСПриУСН, ДокументОбъект.Дата);
						КонецЕсли;
					Иначе
						СтрокаПлатеж.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(ДокументОбъект.Дата, ПлательщикНДС);
					КонецЕсли;
				Иначе
					СтрокаПлатеж.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(ДокументОбъект.Дата);
				КонецЕсли;
			КонецЕсли;
		Иначе
		...
		Если ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам Тогда
			...
			Если НЕ ЗначениеЗаполнено(СтрокаПлатеж.ДоговорКонтрагента)
				...
				Если ЕстьРеквизитДоговор И ЗначениеЗаполнено(ДокументОбъект.ДоговорКонтрагента) Тогда
					// Заполним договором из шапки, если не будет удовлетворть условиям документа, то будет заменен
					// в функции РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента() на договор "по умолчанию".
					СтрокаПлатеж.ДоговорКонтрагента = ДокументОбъект.ДоговорКонтрагента;
					УстановленДоговор = Истина;
				КонецЕсли;
				...
				ВидыДоговоровДокумента = РаботаСДоговорамиКонтрагентовБПВызовСервера.ВидыДоговоровДокумента(
					ДокументОбъект.ВидОперации, ТипСсылкиДокумента);
				УстановленДоговор = РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента(
						СтрокаПлатеж.ДоговорКонтрагента,
						ДокументОбъект.Контрагент,
						ДокументОбъект.Организация,
						ВидыДоговоровДокумента,
						ПараметрыПлатежа) Или УстановленДоговор;
				Если УстановленДоговор Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиДвиженияДенежныхСредств") Тогда
			...
			Если ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитТабЧастиДокумента(
				...
				СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ДокументОбъект.ВидОперации, ДокументОбъект.Дата);
...
Процедура ЗаполнитьПоОснованию(ДокументОбъект, ДокументОснование, КопироватьПодразделение = Истина) Экспорт
	...
	// Налог
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Налог", МетаданныеДокумента) Тогда
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Налог", МетаданныеДокументаОснования)
			И ОбщегоНазначения.ЕстьРеквизитОбъекта("ВидОперации", МетаданныеДокументаОснования)
			И ДокументОснование.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога Тогда
			ДокументОбъект.Налог = ДокументОснование.Налог;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриКопировании(Объект, ОбъектКопирования) Экспорт
	...
	Если Не ПлательщикНДС
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "НДССМежценовойРазницы")
		И Объект.НДССМежценовойРазницы Тогда
		// Неплательщикам НДС нельзя начислять НДС с межценовой разницы.
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "НДСНеВыделять")
		...
		СвойстваДоговора = Неопределено;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ДоговорКонтрагента")
			И ЗначениеЗаполнено(Объект.ДоговорКонтрагента) Тогда
			СвойстваДоговора = БухгалтерскийУчетПереопределяемый.ПолучитьРеквизитыДоговораКонтрагента(Объект.ДоговорКонтрагента);
		КонецЕсли;
		Если СвойстваДоговора <> Неопределено
			И (БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемТоваровУслуг(Объект.Ссылка)
			Или БухгалтерскийУчетКлиентСерверПереопределяемый.ДокументЯвляетсяПоступлениемДопРасходов(Объект.Ссылка))
			И Не УчетНДСБП.ВыделятьНДСПоУмолчанию(СвойстваДоговора.НалогообложениеКонтрагента) Тогда
...
//Определяет необходимость указания складов в табличной части документа
//Параметры:  
//			- Товары - Тип("ДокументТабличнаяЧасть")
//			- ТекстСообщения - Тип("Строка") - необяз.
//          - ИмяКолонкиДокументаОснования - Тип("Строка") - необяз.
//
//Возвращаемое значение - Тип("Булево")
//
Функция ПроверитьНеобходимостьУказанияСкладаВТабличнойЧасти(Товары, ТекстСообщения = "", ИмяКолонкиДокументаОснования = "ДокументПартии") Экспорт
	
	Если Не ПолучитьФункциональнуюОпцию("ПоступлениеНаНесколькоСкладов") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	КоличествоОшибок = 0;
	Для Каждого СтрокаТовары Из Товары Цикл
		Если ТипЗнч(СтрокаТовары[ИмяКолонкиДокументаОснования]) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
			И СтрокаТовары[ИмяКолонкиДокументаОснования].СкладВСписке 
			И Не ЗначениеЗаполнено(СтрокаТовары.Склад) Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ru = 'Не заполнено значение поля склад в строке '") + СтрокаТовары.НомерСтроки;
			КоличествоОшибок = КоличествоОшибок + 1;
		КонецЕсли;
	КонецЦикла;
	
	
	Возврат ?(КоличествоОшибок > 0, Истина, Ложь); 
