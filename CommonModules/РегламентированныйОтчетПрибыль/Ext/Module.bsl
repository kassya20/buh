...
Функция СтраницыРазделаКонтейнера(Контейнер, НомерЛиста, НомерПриложения, ИндексКомплектаСтраниц, ИмяРедакции) Экспорт
	...
	ИначеЕсли НомерПриложения = "5" Тогда
		ИндексПриложения = 4;
	ИначеЕсли НомерПриложения = "7Д" Тогда
		ИндексПриложения = 9;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
// Получает значение показателя многоуровнего раздела контейнера
//
// Параметры:
//  Контейнер	 - ДеревоЗначений, Структура - заполняемый контейнер, тип определяется редакцией отчета, см. ВерсияКонтейнера
//  ИмяРаздела	 - Строка - см. ИмяРаздела 
//  Показатель	 - Строка - Показатель контейнера 
// 
// Возвращаемое значение:
//   - Число
//
Функция ЗначениеМногоуровневогоРаздела(Контейнер, ИмяРаздела, Показатель) Экспорт

	Раздел = Контейнер.ДанныеМногоуровневыхРазделов[ИмяРаздела];
	Если Не ЗначениеЗаполнено(Раздел) Тогда
		Возврат Неопределено;	
	КонецЕсли;
	
	Значение = 0;
	Для Каждого Строка Из Раздел.Строки Цикл
		Данные = Строка.Данные;
		Если Не Данные.Свойство(Показатель) Тогда
			Возврат Неопределено;	
		КонецЕсли;	
		Значение = Значение + Данные[Показатель];			
	КонецЦикла;
...
Функция КодМножественногоЭлемента(КодТаблицыФНС, КодЭлементаФНС, ИмяРедакции) Экспорт
	...
	Если КодТаблицыФНС = "ОстУбытНачПерГод"
		Или КодТаблицыФНС = "СумНеиспВычГод"
		Или КодТаблицыФНС = "ОстНеиспВычГод" Тогда

		Если КодЭлементаФНС = "Год" Тогда
			Возврат "00001";
		КонецЕсли;

		Если КодЭлементаФНС = "ОстУбыт"
			Или КодЭлементаФНС = "СумНеиспВыч"
			Или КодЭлементаФНС = "ОстНеиспВыч" Тогда
			Возврат "00003";
		КонецЕсли;
...
Функция КодМногострочнойЧасти(КодЭлементаФНС)
	...
	ИначеЕсли КодЭлементаФНС = "СвЦелСред07" Тогда
		Возврат "П00007М1";
	ИначеЕсли КодЭлементаФНС = "СумНеиспВычГод" Тогда
		Возврат "П0027ДМ1";
	ИначеЕсли КодЭлементаФНС = "ОстНеиспВычГод" Тогда
		Возврат "П0027ДМ2";
	КонецЕсли;
