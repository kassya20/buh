...
Функция ПолучитьРасчетПоТранспортномуНалогу(Организация, ПериодРасчета, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ПараметрыРасчета.ДатаКонцаПериодаОтчета < РасчетИмущественныхНалоговКлиентСервер.НачалоПримененияНовогоРасчетаТранспортногоНалога() Тогда
		// Устаревший расчет оставлен для совместимости с периодами до 2020 года
		РезультатыРасчетаПоНалогу.Вставить("РасчетТранспортногоНалогаДо2020", ПустаяСправкаРасчет("РасчетТранспортногоНалогаДо2020"));
		ТаблицаРасчетНалога = РезультатыРасчетаПоНалогу.РасчетТранспортногоНалогаДо2020;
	Иначе
		РезультатыРасчетаПоНалогу.Вставить("РасчетТранспортногоНалога", ПустаяСправкаРасчет("РасчетТранспортногоНалога"));
	...
	РезультатРасчета = Новый Структура("РезультатыРасчетаПоНалогу, Платон", РезультатыРасчетаПоНалогу, Неопределено);
	
	Если Месяц(ПериодРасчета)%3 <> 0 Или Не ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		Возврат РезультатРасчета;
	КонецЕсли;
	
	Если ПараметрыРасчета.ДатаКонцаПериодаОтчета < РасчетИмущественныхНалоговКлиентСервер.НачалоПримененияНовогоРасчетаТранспортногоНалога() Тогда
		ДанныеДляРасчета = ПолучитьДанныеДляРасчетаТранспортногоНалогаДо2020(ПараметрыРасчета, ДополнительныеПараметры);
		ВыполнитьРасчетТранспортногоНалогаДо2020(
			ТаблицаРасчетНалога,
			ПараметрыРасчета,
			ДанныеДляРасчета,
	...
	Иначе
		...
		ВыполнитьРасчетТранспортногоНалога(
			ТаблицаРасчетНалога,
			ПараметрыРасчета,
			ДанныеДляРасчета,
			ДополнительныеПараметры);
	...
	// Вычет действовал в 2016 - 2018 годах.
	Если Год(ПериодРасчета) >= 2016 И Год(ПериодРасчета) <= 2018
		 И ТаблицаРасчетНалога.НайтиСтроки(
			 Новый Структура("ЗарегистрированоВРеестреСистемыПлатон", Истина)).Количество() > 0 Тогда
		РезультатРасчета.Платон = РасчетИмущественныхНалоговПереопределяемый.УчестьРасходыНаПлатон(
...
// Возвращаемое значение:
//  ТаблицаЗначений - Виды и категории транспортных средств.
//  	Колонки:
//  	*Вид - Строка - текстовое значение ПеречислениеСсылка.ВидыТранспортныхСредств
//  	*Категория - Строка - текстовое значение ПеречислениеСсылка.КатегорииТранспортныхСредств
//  	*ЕдиницаИзмерения - Строка
//  	*НомерКатегории - Строка
//
Функция ВидыИКатегорииТранспортныхСредств() Экспорт
	Возврат РасчетИмущественныхНалоговПереопределяемый.ВидыИКатегорииТранспортныхСредств();
КонецФункции

// Устарела.
// Оставлена для перехода на новую структуру регистра РегистрацияТранспортныхСредств.
// Возвращает коды видов и категории транспортных средств, а также виды транспортных средств.
// 
// Возвращаемое значение:
...
Процедура ДополнитьПараметрыДвиженийСправкиРасчеты(Параметры, Реквизиты, ТаблицыСправокРасчетов) Экспорт
	...
	Для Каждого ТаблицаСправкаРасчет Из ТаблицыСправокРасчетов Цикл
		...
		ИначеЕсли ТаблицаСправкаРасчет.Ключ = "РасчетТранспортногоНалогаДо2020" Тогда 
...
Функция ДанныеПоСреднейСтоимостиДвижимогоИмущества(Организации, ДатаНачала, ДатаОкончания) Экспорт 
	...
	Пока Период <= НачалоМесяца(ДатаОкончания) Цикл
		...
		// Данные из регистров сведений выбираем на день, предшествующий моменту выбора остатков. Например, если выбираются данные на 01.01,
		// то данные регистров сведений выбираем на 31.12 прошедшего года. Это обусловлено тем, что нам нужны данные на "границу дат",
		// и если срез выбрать на 01.01, то в результат попадут изменения за 01.01, а это неверно.
		Запрос.УстановитьПараметр("ПериодСреза", ?(Год(Период) = Год(ДатаОкончания),
			Период - 1, // берем срез на последний день прошлого месяца
			ДатаОкончания)); // берем срез на конец года
...
Функция ПолучитьДанныеДляРасчетаНалогаНаИмуществоПоСреднейСтоимости(ПараметрыРасчета, ДополнительныеПараметры)
	...
	Пока Период <= НачалоМесяца(ПараметрыРасчета.мДатаКонцаПериодаОтчета) Цикл
		...
		Если УчетнаяПолитика.ПрименяетсяУСН(ПараметрыРасчета.Организация, ПериодДляВыборкиДанных)
			 Или УчетнаяПолитика.ПрименяетсяАУСН(ПараметрыРасчета.Организация, ПериодДляВыборкиДанных) Тогда
			Продолжить;
		КонецЕсли;
		...
		// то данные регистров сведений выбираем на 31.12 прошедшего года. Это обусловлено тем, что нам нужны данные на "границу дат",
		// и если срез выбрать на 01.01, то в результат попадут изменения за 01.01, а это неверно.
		Запрос.УстановитьПараметр("ПериодСреза", ?(Год(Период) = Год(ПараметрыРасчета.ПериодРасчета),
			Период - 1, // берем срез на последний день прошлого месяца
			ПараметрыРасчета.мДатаКонцаПериодаОтчета)); // берем срез на конец года
			...
			Если УчитыватьАвансы И Месяц <> 0 И Месяц % 3 = 0 Тогда
				...
				Если НастройкиУчетнойПолитики = Неопределено Тогда
					ПрименяетсяУСН = УчетнаяПолитика.ПрименяетсяУСН(ПараметрыРасчета.Организация, ПериодУчетнойПолитики)
						Или УчетнаяПолитика.ПрименяетсяАУСН(ПараметрыРасчета.Организация, ПериодУчетнойПолитики);
				...
				// Чтобы суммы авансовых платежей, полученные здесь (при расчете за год), сошлись с теми, которые были получены при
				// расчетах за отчетные периоды, здесь округлим суммы до целых так же, как это сделано ниже для получения сумм за весь период.
				СреднегодоваяСтоимость = ?(ПрименяетсяУСН, 0, Окр(ДелимоеСреднегодоваяСтоимость / Счетчик));
				НеоблагаемаяСреднегодоваяСтоимость = ?(ПрименяетсяУСН, 0, Окр(ДелимоеНеоблагаемаяСреднегодоваяСтоимость / Счетчик));
	...
	Для Каждого СтрокаТаблицы Из ДанныеДляРасчета Цикл
		...
		Иначе
			...
			Если СтрокаТаблицы.УменьшениеСуммыНалогаНаСумму Тогда
				СуммаЛьготы = Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользования / 12);
				НоваяСтрока.СуммаЛьгот = НоваяСтрока.СуммаЛьгот + СуммаЛьготы;
				Если ПараметрыРасчета.ИностраннаяОрганизация Тогда
					НоваяСтрока.СуммаУменьшенияСуммыНалога = НоваяСтрока.СуммаУменьшенияСуммыНалога + СуммаЛьготы;
				КонецЕсли;
		...
		Если УчитыватьАвансы Тогда
			...
			ИначеЕсли ПараметрыРасчета.ИностраннаяОрганизация Тогда
				...
				Если СтрокаТаблицы.УменьшениеСуммыНалогаНаСумму Тогда
					СуммаАвансовыхПлатежейКв1 = Макс(СуммаАвансовыхПлатежейКв1
						- Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв1 / 12), 0);
					СуммаАвансовыхПлатежейКв2 = Макс(СуммаАвансовыхПлатежейКв2
						- Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв2 / 12), 0);
					СуммаАвансовыхПлатежейКв3 = Макс(СуммаАвансовыхПлатежейКв3
						- Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв3 / 12), 0);
				КонецЕсли;
			...
			Иначе
				...
				Если СтрокаТаблицы.УменьшениеСуммыНалогаНаСумму Тогда
					СуммаЛьготКв1 = СуммаЛьготКв1 + Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв1 / 12);
					СуммаЛьготКв2 = СуммаЛьготКв2 + Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв2 / 12);
					СуммаЛьготКв3 = СуммаЛьготКв3 + Окр(СтрокаТаблицы.СуммаУменьшения * СтрокаТаблицы.КоличествоМесяцевИспользованияКв3 / 12);
				КонецЕсли;
...
Функция ПолучитьДанныеДляРасчетаТранспортногоНалога(ПараметрыРасчета, ДополнительныеПараметры)
	...
	// Каждый отдельный месяц периода соотносится только с каким-то одним набором параметров расчета налога по ТС.
	...
	// периода владения (см. п.п. 3, 3.7 ст. 362 НК).
	// При этом место регистрации ТС в конкретном месяце расчета определяется на 1-е число этого месяца
	// (см. п. 3.6 ст. 362 НК).
	// В отношении периода действия льгот подход следующий: месяц начала и месяц окончания применения льготы 
	// считаются месяцами, когда льгота отсутствует (п. 3.3 ст. 362 НК). При этом льготу, у которой начало периода
	...
	Для Сч = ПервыйМесяцРасчета По Месяц(ПараметрыРасчета.ПериодРасчета) Цикл
		...
		ВременныеТаблицыНаВремяРасчета.Добавить(ИмяВременнойТаблицы);
		ПредельнаяДатаВозникновенияПрава = Дата(ГодРасчета, Сч, 15, 23, 59, 59);
		НачалоМесяца = Дата(ГодРасчета, Сч, 1);
		...
		СформироватьТаблицуРегистрацияТранспортныхСредствСрезПоследних(
			...
			НачалоМесяца, 
			ДополнительныеПараметры,
			"РегистрацияТранспортныхСредств_НаНачалоМесяца");
		
		// По конкретному основному средству в конкретный период времени может применяться только один набор параметров.
		// (см. РС РегистрацияТранспортныхСредств).
		// В сумме в течение отчетного периода (квартал или год) по одному ОС могут применяться несколько наборов параметров
		// расчета (с кратностью до месяца). Чтобы данные могли быть записаны в регистр расчета, применяется универсальный
		// разделитель (измерение) ДатаСведений - это дата начала применения конкретного набора параметров расчета.
		// При определении ДатаСведений учитываются периоды записей в РС РегистрацияТранспортныхСредств, а также периоды
		// действия льгот, указанные в записях РС (даты начала и даты окончания применения льгот также являются "граничными")
		// и даты изменения регистрационных данных (конкретного ТС или всей организации).
		// Дата сведений - это самая последняя среди дат, которые влияют на набор параметров расчета.
		// Особый случай расчета ДатаСведений - это изменение регистрации в данном месяце. В этом случае параметры расчета
		// берутся на середину месяца, а регистрационные данные - на начало. За дату сведений принимаем строго начало месяца,
		// чтобы не вызвать конфликт ни со сведениями предшествующих месяцев, ни с последующими.
		...
		|
		|ПОМЕСТИТЬ ВладениеТранспортнымиСредствами
		|
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВладениеТранспортнымиСредствами.ОсновноеСредство КАК ОсновноеСредство,
		|	МАКСИМУМ(ЕСТЬNULL(РегистрацияТранспортныхСредств.Период, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаПоследнегоСнятияСРегистрационногоУчета
		|
		|ПОМЕСТИТЬ ДатыПоследнегоСнятияСРегистрационногоУчета
		|
		|ИЗ
		|	ВладениеТранспортнымиСредствами КАК ВладениеТранспортнымиСредствами
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрацияТранспортныхСредств_Записи КАК РегистрацияТранспортныхСредств
		|		ПО ВладениеТранспортнымиСредствами.ОсновноеСредство = РегистрацияТранспортныхСредств.ОсновноеСредство
		|			И ВладениеТранспортнымиСредствами.Период > РегистрацияТранспортныхСредств.Период
		|			И (НЕ РегистрацияТранспортныхСредств.ВключатьВНалоговуюБазу)
		|
		|СГРУППИРОВАТЬ ПО
		|	ВладениеТранспортнымиСредствами.ОсновноеСредство
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
		...
		|	МИНИМУМ(РегистрацияТранспортныхСредств.Период) КАК ДатаРегистрацииТранспорта
		|
		|ПОМЕСТИТЬ ДатыРегистрацииТранспорта
		|
		|ИЗ
		|	ДатыПоследнегоСнятияСРегистрационногоУчета КАК ДатыПоследнегоСнятияСРегистрационногоУчета
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрацияТранспортныхСредств_Записи КАК РегистрацияТранспортныхСредств
		|		ПО ДатыПоследнегоСнятияСРегистрационногоУчета.ОсновноеСредство = РегистрацияТранспортныхСредств.ОсновноеСредство
		|			И ДатыПоследнегоСнятияСРегистрационногоУчета.ДатаПоследнегоСнятияСРегистрационногоУчета < РегистрацияТранспортныхСредств.Период
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(ДанныеРегистрации.Период, ВладениеТранспортнымиСредствами.Период) КАК Период,
		|	ВладениеТранспортнымиСредствами.ОсновноеСредство КАК ОсновноеСредство,
		|
		|	ВладениеТранспортнымиСредствами.ПостановкаНаУчетВНалоговомОргане КАК ПостановкаНаУчетВНалоговомОргане,
		|	ВладениеТранспортнымиСредствами.НалоговыйОрган КАК НалоговыйОрган,
		|	ВладениеТранспортнымиСредствами.КодПоОКТМО КАК КодПоОКТМО,
		|
		|	ДанныеРегистрации.ПостановкаНаУчетВНалоговомОргане КАК ПостановкаНаУчетВНалоговомОрганеНаНачалоМесяца,
		|	ДанныеРегистрации.НалоговыйОрган КАК НалоговыйОрганНаНачалоМесяца,
		|	ДанныеРегистрации.КодПоОКТМО КАК КодПоОКТМОНаНачалоМесяца,
		|
		|	ВЫБОР
		|		КОГДА ДанныеРегистрации.Период ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ВладениеТранспортнымиСредствами.ПостановкаНаУчетВНалоговомОргане <> ДанныеРегистрации.ПостановкаНаУчетВНалоговомОргане
		|			ИЛИ ВладениеТранспортнымиСредствами.НалоговыйОрган <> ДанныеРегистрации.НалоговыйОрган
		|			ИЛИ ВладениеТранспортнымиСредствами.ПостановкаНаУчетВНалоговомОргане = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ПоМестуНахожденияОрганизации)
		|				И &ОсновнойНалоговыйОрган <> &ОсновнойНалоговыйОрганНаНачалоМесяца
		|	КОНЕЦ КАК ИзмененаРегистрация
		|
		...
		|	ВладениеТранспортнымиСредствами КАК ВладениеТранспортнымиСредствами
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрацияТранспортныхСредств_НаНачалоМесяца КАК ДанныеРегистрации
		|		ПО ВладениеТранспортнымиСредствами.ОсновноеСредство = ДанныеРегистрации.ОсновноеСредство
		|		И (ДанныеРегистрации.ВключатьВНалоговуюБазу)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВладениеТранспортнымиСредствами.Период КАК Период,
		|	ВладениеТранспортнымиСредствами.ОсновноеСредство КАК ОсновноеСредство,
		|	ВладениеТранспортнымиСредствами.РегистрационныйЗнак КАК РегистрационныйЗнак,
		|	ВладениеТранспортнымиСредствами.ИдентификационныйНомер КАК ИдентификационныйНомер,
		|	ВладениеТранспортнымиСредствами.Марка КАК Марка,
		|
		|	ВЫБОР
		|		КОГДА ДанныеРегистрации.ИзмененаРегистрация
		|			ТОГДА ДанныеРегистрации.ПостановкаНаУчетВНалоговомОрганеНаНачалоМесяца
		|		ИНАЧЕ ДанныеРегистрации.ПостановкаНаУчетВНалоговомОргане
		|	КОНЕЦ КАК ПостановкаНаУчетВНалоговомОргане,
		|	
		|	ВЫБОР
		|		КОГДА ДанныеРегистрации.ИзмененаРегистрация
		|			ТОГДА ВЫБОР
		|				КОГДА ДанныеРегистрации.ПостановкаНаУчетВНалоговомОрганеНаНачалоМесяца = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане)
		|					ТОГДА ДанныеРегистрации.НалоговыйОрганНаНачалоМесяца
		|				ИНАЧЕ &ОсновнойНалоговыйОрганНаНачалоМесяца
		|			КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ДанныеРегистрации.ПостановкаНаУчетВНалоговомОргане = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане)
		|					ТОГДА ДанныеРегистрации.НалоговыйОрган
		|				ИНАЧЕ &ОсновнойНалоговыйОрган
		|			КОНЕЦ
		|	КОНЕЦ КАК ИФНС,
		|	
		...
		|			ТОГДА ВЫБОР
		|				КОГДА ДанныеРегистрации.ПостановкаНаУчетВНалоговомОрганеНаНачалоМесяца = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане)
		|					ТОГДА ДанныеРегистрации.КодПоОКТМОНаНачалоМесяца
		|				ИНАЧЕ &КодПоОКТМОНаНачалоМесяца
		|			КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ДанныеРегистрации.ПостановкаНаУчетВНалоговомОргане = ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ВДругомНалоговомОргане)
		|					ТОГДА ДанныеРегистрации.КодПоОКТМО
		|				ИНАЧЕ &КодПоОКТМО
		|			КОНЕЦ
		|	КОНЕЦ КАК КодПоОКТМО,
		|
		|	ДанныеРегистрации.ИзмененаРегистрация КАК ИзмененаРегистрация,
		|	ВладениеТранспортнымиСредствами.ВидТранспортногоСредства КАК ВидТранспортногоСредства,
		|	ВладениеТранспортнымиСредствами.НалоговаяБаза КАК НалоговаяБаза,
		|	ВладениеТранспортнымиСредствами.ЕдиницаИзмеренияНалоговойБазы КАК ЕдиницаИзмеренияНалоговойБазы,
		|	ВладениеТранспортнымиСредствами.НалоговаяСтавка КАК НалоговаяСтавка,
		|	ВладениеТранспортнымиСредствами.ОснованиеЛьготы КАК ОснованиеЛьготы,
		|
		|	ВЫБОР 
		|		КОГДА ВладениеТранспортнымиСредствами.НачалоДействияЛьготы > ВладениеТранспортнымиСредствами.Период
		|				И ВладениеТранспортнымиСредствами.НачалоДействияЛьготы > 
		|					НАЧАЛОПЕРИОДА(ВладениеТранспортнымиСредствами.НачалоДействияЛьготы, МЕСЯЦ)
		|			ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ВладениеТранспортнымиСредствами.НачалоДействияЛьготы, МЕСЯЦ, 1), МЕСЯЦ)
		|		КОГДА ВладениеТранспортнымиСредствами.НачалоДействияЛьготы > ВладениеТранспортнымиСредствами.Период
		|			ТОГДА ВладениеТранспортнымиСредствами.НачалоДействияЛьготы
		|		КОГДА ВладениеТранспортнымиСредствами.НачалоДействияЛьготы <=
		|				НАЧАЛОПЕРИОДА(ВладениеТранспортнымиСредствами.Период, МЕСЯЦ)
		|			ТОГДА ВладениеТранспортнымиСредствами.Период
		|		ИНАЧЕ НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ВладениеТранспортнымиСредствами.Период, МЕСЯЦ, 1), МЕСЯЦ)
		|	КОНЕЦ КАК НачалоДействияЛьготы,
		|
		|	ВЫБОР 
		|		КОГДА ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы = ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ДАТАВРЕМЯ(1,1,1)
		|		КОГДА КОНЕЦПЕРИОДА(ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы, ДЕНЬ) < 
		|				КОНЕЦПЕРИОДА(ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы, МЕСЯЦ)
		|			ТОГДА КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы, МЕСЯЦ, -1), МЕСЯЦ)
		|		ИНАЧЕ ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы
		|	КОНЕЦ КАК ОкончаниеДействияЛьготы,
		|
		|	ВладениеТранспортнымиСредствами.ОснованиеЛьготы <> ЗНАЧЕНИЕ(Справочник.ОснованияЛьготПоИмущественнымНалогам.ПустаяСсылка)
		|		И ВладениеТранспортнымиСредствами.НачалоДействияЛьготы <= &НачалоМесяца
		|		И (ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы = ДАТАВРЕМЯ(1, 1, 1)
		|			ИЛИ ВладениеТранспортнымиСредствами.ОкончаниеДействияЛьготы >= НАЧАЛОПЕРИОДА(&КонецМесяца, ДЕНЬ)) КАК ДействуетЛьгота,
		|
		|	ЕСТЬNULL(ВладениеТранспортнымиСредствами.ОснованиеЛьготы.ВидЛьготы, ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.ПустаяСсылка)) КАК ВидЛьготы,
		|	ВладениеТранспортнымиСредствами.ЛьготнаяСтавка КАК ЛьготнаяСтавка,
		|	ЕСТЬNULL(ВладениеТранспортнымиСредствами.ОснованиеЛьготы.ПроцентУменьшения, 0) КАК ПроцентУменьшенияСуммыНалога,
		|	ВладениеТранспортнымиСредствами.СуммаУменьшения КАК СуммаУменьшенияСуммыНалога,
		|	ВладениеТранспортнымиСредствами.ЭкологическийКласс КАК ЭкологическийКласс,
		|	ВладениеТранспортнымиСредствами.ДоляВПравеОбщейСобственностиЧислитель КАК ДоляВПравеОбщейСобственностиЧислитель,
		|	ВладениеТранспортнымиСредствами.ДоляВПравеОбщейСобственностиЗнаменатель КАК ДоляВПравеОбщейСобственностиЗнаменатель,
		|	ВладениеТранспортнымиСредствами.ПовышающийКоэффициент КАК ПовышающийКоэффициент,
		|	ВладениеТранспортнымиСредствами.СтавкаОпределяетсяАвтоматически КАК СтавкаОпределяетсяАвтоматически,
		|	ДатыРегистрацииТранспорта.ДатаРегистрацииТранспорта КАК ДатаРегистрацииТранспорта
		|
		...
		|	ВладениеТранспортнымиСредствами КАК ВладениеТранспортнымиСредствами
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыРегистрацииТранспорта КАК ДатыРегистрацииТранспорта
		|		ПО ВладениеТранспортнымиСредствами.ОсновноеСредство = ДатыРегистрацииТранспорта.ОсновноеСредство
		|
		...
		|	ГДЕ 
		|		НЕ ВТ_ПромежуточныеДанные.ИзмененаРегистрация
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		ВЫБОР
		|			КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|				ТОГДА ВТ_ПромежуточныеДанные.НачалоДействияЛьготы
		|			КОГДА ВТ_ПромежуточныеДанные.ОкончаниеДействияЛьготы <> ДАТАВРЕМЯ(1,1,1)
		|				И ВТ_ПромежуточныеДанные.ОкончаниеДействияЛьготы <= &КонецМесяца
		|				ТОГДА ВТ_ПромежуточныеДанные.ОкончаниеДействияЛьготы
		|			ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
		|		КОНЕЦ
		|	ИЗ
		|		ВТ_ПромежуточныеДанные КАК ВТ_ПромежуточныеДанные
		...
		|		ВТ_ПромежуточныеДанные КАК ВТ_ПромежуточныеДанные
		|	ГДЕ 
		|		ВТ_ПромежуточныеДанные.ПостановкаНаУчетВНалоговомОргане = 
		|			ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ПоМестуНахожденияОрганизации)
		|		И НЕ ВТ_ПромежуточныеДанные.ИзмененаРегистрация
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|		ВТ_ПромежуточныеДанные КАК ВТ_ПромежуточныеДанные
		|	ГДЕ 
		|		ВТ_ПромежуточныеДанные.ИзмененаРегистрация) КАК ДатыИзменений
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВТ_ПромежуточныеДанные.ОсновноеСредство КАК ОсновноеСредство,
		|	ВТ_ПромежуточныеДанные.РегистрационныйЗнак КАК РегистрационныйЗнак,
		|	ВТ_ПромежуточныеДанные.ИдентификационныйНомер КАК ИдентификационныйНомер,
		|	ВТ_ПромежуточныеДанные.Марка КАК Марка,
		|	ВТ_ПромежуточныеДанные.ИФНС КАК ИФНС,
		|	ВТ_ПромежуточныеДанные.КодПоОКТМО КАК КодПоОКТМО,
		|	ВТ_ПромежуточныеДанные.ВидТранспортногоСредства КАК ВидТранспортногоСредства,
		|	ВТ_ПромежуточныеДанные.НалоговаяБаза КАК НалоговаяБаза,
		|	ВТ_ПромежуточныеДанные.ЕдиницаИзмеренияНалоговойБазы КАК ЕдиницаИзмеренияНалоговойБазы,
		|	ВТ_ПромежуточныеДанные.ЭкологическийКласс КАК ЭкологическийКласс,
		...
		|	ВТ_ПромежуточныеДанные.ПовышающийКоэффициент КАК ПовышающийКоэффициент,
		|
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			ТОГДА ВТ_ПромежуточныеДанные.ОснованиеЛьготы
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ОснованияЛьготПоИмущественнымНалогам.ПустаяСсылка)
		|	КОНЕЦ КАК ОснованиеЛьготы,
		|
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			ТОГДА ВТ_ПромежуточныеДанные.ВидЛьготы
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.ПустаяСсылка)
		|	КОНЕЦ КАК ВидЛьготы,
		|
		|	ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.Освобождение) КАК ОсвобождениеОтНалогообложения,
		|
		|	ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСуммыНаПроцент) КАК УменьшениеСуммыНалогаНаПроцент,
		|
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСуммыНаПроцент)
		|			ТОГДА ВТ_ПромежуточныеДанные.ПроцентУменьшенияСуммыНалога
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ПроцентУменьшенияСуммыНалога,
		|
		|	ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСуммыНаСумму) КАК УменьшениеСуммыНалогаНаСумму,
		|
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСуммыНаСумму)
		...
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСтавкиДоУказанной)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ВТ_ПромежуточныеДанные.СтавкаОпределяетсяАвтоматически
		|	КОНЕЦ КАК СтавкаОпределяетсяАвтоматически,
		|
		|	ВЫБОР
		|		КОГДА ВТ_ПромежуточныеДанные.ДействуетЛьгота
		|			И ВТ_ПромежуточныеДанные.ВидЛьготы = ЗНАЧЕНИЕ(Перечисление.ВидыЛьготПоИмущественнымНалогам.СнижениеСтавкиДоУказанной)
		|			ТОГДА ВТ_ПромежуточныеДанные.ЛьготнаяСтавка
		|		КОГДА ВТ_ПромежуточныеДанные.СтавкаОпределяетсяАвтоматически
		|			ТОГДА 0
		|		ИНАЧЕ ВТ_ПромежуточныеДанные.НалоговаяСтавка
		|	КОНЕЦ КАК НалоговаяСтавка,
		|
		|	ВТ_ПромежуточныеДанные.ДатаРегистрацииТранспорта КАК ДатаРегистрацииТранспорта,
		|	1 КАК КоличествоМесяцевИспользования,
		|
		|	ВЫБОР
		|		КОГДА &НомерМесяца <= 3
		...
		|	КОНЕЦ КАК КоличествоМесяцевИспользованияКв3
		|
		|ПОМЕСТИТЬ ВладениеТранспортнымиСредствамиПериод
		|
		|ИЗ
		|	ВТ_ПромежуточныеДанные КАК ВТ_ПромежуточныеДанные
		|
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПОМЕСТИТЬ ВладениеТранспортнымиСредствамиПериод", "ПОМЕСТИТЬ " + ИмяВременнойТаблицы);
		
		ПараметрыРегистрацииВФНСНаНачалоМесяца = Новый Структура;
		ПараметрыРегистрацииВФНСНаНачалоМесяца.Вставить("ОсновнойНалоговыйОрган", Справочники.РегистрацииВНалоговомОргане.ПустаяСсылка());
		ПараметрыРегистрацииВФНСНаНачалоМесяца.Вставить("КодПоОКТМО", "");
		ПараметрыРегистрацииВФНСНаНачалоМесяца.Вставить("ДатаПоследнейРегистрации", '00010101');
	
		РасчетИмущественныхНалоговПереопределяемый.УточнитьПараметрыРегистрацииОрганизацииВФНС(
			ПараметрыРегистрацииВФНСНаНачалоМесяца,
			ПараметрыРасчета.Организация,
			НачалоМесяца);
		...
		Запрос.УстановитьПараметр("ОсновнойНалоговыйОрганНаНачалоМесяца", ПараметрыРегистрацииВФНСНаНачалоМесяца.ОсновнойНалоговыйОрган);
		Запрос.УстановитьПараметр("КодПоОКТМОНаНачалоМесяца", ПараметрыРегистрацииВФНСНаНачалоМесяца.КодПоОКТМО);
		Запрос.УстановитьПараметр("ОсновнойНалоговыйОрган", ПараметрыРегистрацииВФНС.ОсновнойНалоговыйОрган);
		Запрос.УстановитьПараметр("КодПоОКТМО", ПараметрыРегистрацииВФНС.КодПоОКТМО);
	...
	// ТС может быть несколько расчетов; каждый из них действует только часть периода (см. КоличествоМесяцевИспользования);
	// общая сумма налога по ТС будет равна сумме итогов по этим отдельным расчетам).
	// При этом нужно понимать, что каждый такой набор будет отличаться как минимум полем ДатаСведений.
	// Поэтому в дальнейшем при записи данных в РС РасчетТранспортногоНалога дублей не возникнет.
	// Теоретически могут быть несколько наборов, которые отличаются только полем ДатаСведений (пользователь ввел несколько
	// одинаковых записей по одному ОС в РС РегистрацияТранспортныхСредств). В этом случае берем самую раннюю ДатаСведений.
	// 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	МИНИМУМ(ВладениеТранспортнымиСредствами.ДатаСведений)КАК ДатаСведений,
	|	ВладениеТранспортнымиСредствами.ОсновноеСредство КАК ОсновноеСредство,
	|	ВладениеТранспортнымиСредствами.РегистрационныйЗнак КАК РегистрационныйЗнак,
	|	ВладениеТранспортнымиСредствами.ИдентификационныйНомер КАК ИдентификационныйНомер,
	|	ВладениеТранспортнымиСредствами.Марка КАК Марка,
	|	ВладениеТранспортнымиСредствами.ИФНС КАК ИФНС,
	|	ВладениеТранспортнымиСредствами.КодПоОКТМО КАК КодПоОКТМО,
	|	ВладениеТранспортнымиСредствами.ВидТранспортногоСредства КАК ВидТранспортногоСредства,
	|	ВладениеТранспортнымиСредствами.НалоговаяБаза КАК НалоговаяБаза,
	|	ВладениеТранспортнымиСредствами.ЕдиницаИзмеренияНалоговойБазы КАК ЕдиницаИзмеренияНалоговойБазы,
	...
	|	ВладениеТранспортнымиСредствами.ПовышающийКоэффициент КАК ПовышающийКоэффициент,
	|	ВладениеТранспортнымиСредствами.ОснованиеЛьготы КАК ОснованиеЛьготы,
	|	ВладениеТранспортнымиСредствами.ВидЛьготы КАК ВидЛьготы,
	|	ВладениеТранспортнымиСредствами.ОсвобождениеОтНалогообложения КАК ОсвобождениеОтНалогообложения,
	|	ВладениеТранспортнымиСредствами.УменьшениеСуммыНалогаНаПроцент КАК УменьшениеСуммыНалогаНаПроцент,
	|	ВладениеТранспортнымиСредствами.ПроцентУменьшенияСуммыНалога КАК ПроцентУменьшенияСуммыНалога,
	|	ВладениеТранспортнымиСредствами.УменьшениеСуммыНалогаНаСумму КАК УменьшениеСуммыНалогаНаСумму,
	|	ВладениеТранспортнымиСредствами.СуммаУменьшенияСуммыНалога КАК СуммаУменьшенияСуммыНалога,
	|	ВладениеТранспортнымиСредствами.СтавкаОпределяетсяАвтоматически КАК СтавкаОпределяетсяАвтоматически,
	|	ВладениеТранспортнымиСредствами.НалоговаяСтавка КАК НалоговаяСтавка,
	|	ВладениеТранспортнымиСредствами.ДатаРегистрацииТранспорта КАК ДатаРегистрацииТранспорта,
	|	СУММА(ВладениеТранспортнымиСредствами.КоличествоМесяцевИспользования) КАК КоличествоМесяцевИспользования,
	|	СУММА(ВладениеТранспортнымиСредствами.КоличествоМесяцевИспользованияКв1) КАК КоличествоМесяцевИспользованияКв1,
	|	СУММА(ВладениеТранспортнымиСредствами.КоличествоМесяцевИспользованияКв2) КАК КоличествоМесяцевИспользованияКв2,
	|	СУММА(ВладениеТранспортнымиСредствами.КоличествоМесяцевИспользованияКв3) КАК КоличествоМесяцевИспользованияКв3
	|ПОМЕСТИТЬ ВТ_ДанныеРасчета
	|ИЗ
	|	ВладениеТранспортнымиСредствами КАК ВладениеТранспортнымиСредствами
	|СГРУППИРОВАТЬ ПО
	|	ВладениеТранспортнымиСредствами.ОсновноеСредство,
	|	ВладениеТранспортнымиСредствами.РегистрационныйЗнак,
	|	ВладениеТранспортнымиСредствами.ИдентификационныйНомер,
	|	ВладениеТранспортнымиСредствами.Марка,
	|	ВладениеТранспортнымиСредствами.ИФНС,
	|	ВладениеТранспортнымиСредствами.КодПоОКТМО,
	|	ВладениеТранспортнымиСредствами.ВидТранспортногоСредства,
	|	ВладениеТранспортнымиСредствами.НалоговаяБаза,
	|	ВладениеТранспортнымиСредствами.ЕдиницаИзмеренияНалоговойБазы,
	|	ВладениеТранспортнымиСредствами.ЭкологическийКласс,
	|	ВладениеТранспортнымиСредствами.ДоляВПравеОбщейСобственностиЧислитель,
	|	ВладениеТранспортнымиСредствами.ДоляВПравеОбщейСобственностиЗнаменатель,
	|	ВладениеТранспортнымиСредствами.ПовышающийКоэффициент,
	|	ВладениеТранспортнымиСредствами.ОснованиеЛьготы,
	|	ВладениеТранспортнымиСредствами.ВидЛьготы,
	|	ВладениеТранспортнымиСредствами.ОсвобождениеОтНалогообложения,
	|	ВладениеТранспортнымиСредствами.УменьшениеСуммыНалогаНаПроцент,
	|	ВладениеТранспортнымиСредствами.ПроцентУменьшенияСуммыНалога,
	|	ВладениеТранспортнымиСредствами.УменьшениеСуммыНалогаНаСумму,
	|	ВладениеТранспортнымиСредствами.СуммаУменьшенияСуммыНалога,
	|	ВладениеТранспортнымиСредствами.СтавкаОпределяетсяАвтоматически,
	|	ВладениеТранспортнымиСредствами.НалоговаяСтавка,
	|	ВладениеТранспортнымиСредствами.ДатаРегистрацииТранспорта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ДанныеРасчета.ОсновноеСредство КАК ОсновноеСредство,
	|	ВТ_ДанныеРасчета.ДатаРегистрацииТранспорта КАК ДатаРегистрацииТранспорта,
	|	МИНИМУМ(РегистрацияТранспортныхСредств.Период) КАК ДатаСнятияСУчета
	|ПОМЕСТИТЬ БлижайшиеСнятияСУчета
	|ИЗ
	|	ВТ_ДанныеРасчета КАК ВТ_ДанныеРасчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрацияТранспортныхСредств_Записи КАК РегистрацияТранспортныхСредств
	|		ПО ВТ_ДанныеРасчета.ОсновноеСредство = РегистрацияТранспортныхСредств.ОсновноеСредство
	|		И ВТ_ДанныеРасчета.ДатаРегистрацииТранспорта < РегистрацияТранспортныхСредств.Период
	|ГДЕ
	|	НЕ РегистрацияТранспортныхСредств.ВключатьВНалоговуюБазу
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ДанныеРасчета.ОсновноеСредство,
	|	ВТ_ДанныеРасчета.ДатаРегистрацииТранспорта
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	ДатаРегистрацииТранспорта
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВТ_ДанныеРасчета.ОсновноеСредство КАК ОсновноеСредство,
	|	ВТ_ДанныеРасчета.РегистрационныйЗнак КАК РегистрационныйЗнак,
	|	ВТ_ДанныеРасчета.ИдентификационныйНомер КАК ИдентификационныйНомер,
	|	ВТ_ДанныеРасчета.Марка КАК Марка,
	|	ВТ_ДанныеРасчета.ИФНС КАК ИФНС,
	|	ВТ_ДанныеРасчета.КодПоОКТМО КАК КодПоОКТМО,
	|	ВТ_ДанныеРасчета.ВидТранспортногоСредства КАК ВидТранспортногоСредства,
	|	ВТ_ДанныеРасчета.НалоговаяБаза КАК НалоговаяБаза,
	|	ВТ_ДанныеРасчета.ЕдиницаИзмеренияНалоговойБазы КАК ЕдиницаИзмерения,
	|	ВТ_ДанныеРасчета.ЭкологическийКласс КАК ЭкологическийКласс,
	...
	|	ВТ_ДанныеРасчета.ПовышающийКоэффициент КАК ПовышающийКоэффициент,
	|	ВТ_ДанныеРасчета.ОснованиеЛьготы КАК ОснованиеЛьготы,
	|	ВТ_ДанныеРасчета.ВидЛьготы КАК ВидЛьготы,
	...
	|	ВТ_ДанныеРасчета.СтавкаОпределяетсяАвтоматически КАК СтавкаОпределяетсяАвтоматически,
	|	ВТ_ДанныеРасчета.НалоговаяСтавка КАК НалоговаяСтавка,
	|	ВТ_ДанныеРасчета.ДатаРегистрацииТранспорта КАК ДатаРегистрации,
	|	ЕСТЬNULL(БлижайшиеСнятияСУчета.ДатаСнятияСУчета, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСнятияСУчета,
	|	ВТ_ДанныеРасчета.ОсновноеСредство.ДатаВыпуска КАК ДатаВыпуска,
	...
	|		И ВТ_ДанныеРасчета.ДатаРегистрацииТранспорта = БлижайшиеСнятияСУчета.ДатаРегистрацииТранспорта
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НалоговыеОрганыСУстановленнойУплатойАвансов КАК ВТ_НалоговыеОрганыСУстановленнойУплатойАвансов
...
Процедура ВыполнитьРасчетТранспортногоНалога(ТаблицаРасчетНалога, ПараметрыРасчета, ДанныеДляРасчета, ДополнительныеПараметры)
	...
	Для Каждого СтрокаРасчета Из ДанныеДляРасчета Цикл
		...
		Если НоваяСтрока.ДоляВПравеОбщейСобственностиЧислитель <> 1
			 Или НоваяСтрока.ДоляВПравеОбщейСобственностиЗнаменатель <> 1 Тогда
			ДоляВПравеОбщейСобственности = 
		...
		Если НоваяСтрока.ПовышающийКоэффициент <> 0 Тогда
			ПовышающийКоэффициент = НоваяСтрока.ПовышающийКоэффициент;
		Иначе
			ПовышающийКоэффициент = 1;
		КонецЕсли;
		
		// Рассчитаем сумму налога без учета льгот (за исключением ставки - если установлена, то применяется льготная) -
		// требуется для сверки с ФНС по сообщению об исчисленных суммах налогов
		НоваяСтрока.СуммаНалогаБезЛьгот = СуммаТранспортногоНалога(
			НоваяСтрока.НалоговаяБаза,
			ДоляВПравеОбщейСобственности,
			НоваяСтрока.НалоговаяСтавка,
		...
		// Посчитаем сумму льгот и сумму налога с учетом льгот
		Если СтрокаРасчета.ОсвобождениеОтНалогообложения Тогда
			НоваяСтрока.СуммаЛьгот = НоваяСтрока.СуммаНалогаБезЛьгот;
		ИначеЕсли СтрокаРасчета.УменьшениеСуммыНалогаНаПроцент Тогда
			СуммаЛьготы = 
				Окр(Макс(НоваяСтрока.СуммаНалогаБезЛьгот - НоваяСтрока.СуммаЛьгот, 0) // сумма налога за вычетом всех других льгот
				* СтрокаРасчета.ПроцентУменьшенияСуммыНалога / 100);
			НоваяСтрока.СуммаЛьгот = НоваяСтрока.СуммаЛьгот + СуммаЛьготы;
		ИначеЕсли СтрокаРасчета.УменьшениеСуммыНалогаНаСумму Тогда
			СуммаЛьготы = Окр(СтрокаРасчета.СуммаУменьшенияСуммыНалога * СтрокаРасчета.КоличествоМесяцевИспользования / 12);
			НоваяСтрока.СуммаЛьгот = НоваяСтрока.СуммаЛьгот + СуммаЛьготы;
		КонецЕсли;
		...
		Если УчитыватьАвансы Тогда
			...
			Иначе
				...
				СуммаНалогаБезЛьготКв1 = СуммаТранспортногоНалога(
					НоваяСтрока.НалоговаяБаза,
					ДоляВПравеОбщейСобственности,
					НоваяСтрока.НалоговаяСтавка,
					ПовышающийКоэффициент,
				...
				СуммаНалогаБезЛьготКв2 = СуммаТранспортногоНалога(
					НоваяСтрока.НалоговаяБаза,
					ДоляВПравеОбщейСобственности,
					НоваяСтрока.НалоговаяСтавка,
					ПовышающийКоэффициент,
				...
				СуммаНалогаБезЛьготКв3 = СуммаТранспортногоНалога(
					НоваяСтрока.НалоговаяБаза,
					ДоляВПравеОбщейСобственности,
					НоваяСтрока.НалоговаяСтавка,
					ПовышающийКоэффициент,
				...
				Если СтрокаРасчета.УменьшениеСуммыНалогаНаСумму Тогда
					СуммаЛьготКв1 = СуммаЛьготКв1 + Окр(СтрокаРасчета.СуммаУменьшенияСуммыНалога * СтрокаРасчета.КоличествоМесяцевИспользованияКв1 / 12);
					СуммаЛьготКв2 = СуммаЛьготКв2 + Окр(СтрокаРасчета.СуммаУменьшенияСуммыНалога * СтрокаРасчета.КоличествоМесяцевИспользованияКв2 / 12);
					СуммаЛьготКв3 = СуммаЛьготКв3 + Окр(СтрокаРасчета.СуммаУменьшенияСуммыНалога * СтрокаРасчета.КоличествоМесяцевИспользованияКв3 / 12);
				КонецЕсли;
...
Функция СуммаТранспортногоНалога(НалоговаяБаза, ДоляВПравеОбщейСобственности, НалоговаяСтавка, ПовышающийКоэффициент,
			КоличествоМесяцевИспользования, КоличествоМесяцевВПериоде, ГодовойРасчет)
	
	// Расчет авансов отличается только тем, что сумму нужно разделить на 4:
	// "Налогоплательщики ... исчисляют суммы авансовых платежей по налогу ... в размере одной четвертой произведения
	// соответствующей налоговой базы и налоговой ставки с учетом повышающего коэффициента..." (п.2.1 ст. 362 НК)
	КоэффициентАвансов = ?(ГодовойРасчет, 1, 4); 
...
Процедура ДополнитьРасчетТранспортногоНалога(ТаблицаРасчетНалога, Знач ДатаКонцаПериодаОтчета)
	...
	Для Каждого СтрокаРасчета Из ТаблицаРасчетНалога Цикл
		...
		СведенияОВидеТС = СведенияОВидахТранспортныхСредств[СтрокаРасчета.ВидТранспортногоСредства];
		
		Если СведенияОВидеТС <> Неопределено Тогда
			СтрокаРасчета.КатегорияТС = Перечисления.КатегорииТранспортныхСредств[СведенияОВидеТС.Категория];
		КонецЕсли;
...
Функция ПолучитьДанныеДляРасчетаТранспортногоНалогаДо2020(ПараметрыРасчета, ДополнительныеПараметры)
...
Процедура ВыполнитьРасчетТранспортногоНалогаДо2020(ТаблицаРасчетНалога, ПараметрыРасчета, ДанныеДляРасчета, ДополнительныеПараметры)
	...
	Для Каждого СтрокаРасчета Из ДанныеДляРасчета Цикл
		...
		Если СтрокаРасчета.КоличествоЗаписейЛьгот = 1
		   И СтрокаРасчета.НалоговаяЛьгота = Перечисления.ВидыНалоговыхЛьготПоТранспортномуНалогуДо2020.НеПрименяется Тогда
			Продолжить;
		КонецЕсли;
		...
		Если НалоговаяЛьгота <> Перечисления.ВидыНалоговыхЛьготПоТранспортномуНалогуДо2020.НеПрименяется
			И НалоговаяЛьгота <> Перечисления.ВидыНалоговыхЛьготПоТранспортномуНалогуДо2020.УменьшениеСуммыНалогаНаСумму Тогда 
			...
			Если НалоговаяЛьгота = Перечисления.ВидыНалоговыхЛьготПоТранспортномуНалогуДо2020.ОсвобождениеОтНалогообложения Тогда 
			...
			ИначеЕсли НалоговаяЛьгота = Перечисления.ВидыНалоговыхЛьготПоТранспортномуНалогуДо2020.УменьшениеСуммыНалогаВПроцентах Тогда 
...
Функция ПолучитьДанныеДляРасчетаЗемельногоНалога(ПараметрыРасчета, ДополнительныеПараметры)
	...
	Для Сч = ПервыйМесяцРасчета По Месяц(ПараметрыРасчета.ПериодРасчета) Цикл
		...
		// См. врем. табл. ДатыСведений
		//
		// Если земельный участок имеет ВРИ для жилищного строительства, то это влияет на расчет: в период между получением
		// разрешения на строительство и до получения права собственности на построенный дом применяется повышающий коэффициент 2
		// (первые 3 года) или 4 (после 3 лет, пока не зарегистрировано право собственности).
		// Действие повышающих коэффициентов определяется так:
		// начинаем применять повышающий коэффициент с месяца регистрации, если участок сразу зарегистрирован для жилого строительства.
		// Если ВРИ изменен позже, то применям коэффициент со следующего месяца после получения разрешения на строительство.
		// Прекращаем применение повышенного коэффициента всегда со следующего месяца после получения права собственности.
		// (см. ДатыПолученияРазрешенияНаСтроительство, УчасткиПодЖилищноеСтроительство).
		// Границы периода строительства, а также граница изменения коэффициента с 2 до 4, также учитываются при определении ДатаСведений.
		...
		|ВЫБРАТЬ
		|	ДатыПоследнегоСнятияСРегистрационногоУчета.ОсновноеСредство КАК ОсновноеСредство,
		|	МИНИМУМ(РегистрацияЗемельныхУчастков.Период) КАК ДатаРазрешенияНаСтроительство
		|
		|ПОМЕСТИТЬ ДатыПолученияРазрешенияНаСтроительство
		|
		|ИЗ
		|	ДатыПоследнегоСнятияСРегистрационногоУчета КАК ДатыПоследнегоСнятияСРегистрационногоУчета
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрацияЗемельныхУчастков_Записи КАК РегистрацияЗемельныхУчастков
		|		ПО ДатыПоследнегоСнятияСРегистрационногоУчета.ОсновноеСредство = РегистрацияЗемельныхУчастков.ОсновноеСредство
		|			И ДатыПоследнегоСнятияСРегистрационногоУчета.ДатаПоследнегоСнятияСРегистрационногоУчета < РегистрацияЗемельныхУчастков.Период
		|			И (РегистрацияЗемельныхУчастков.ВключатьВНалоговуюБазу)
		|			И (РегистрацияЗемельныхУчастков.ЖилищноеСтроительство)
		|
		|СГРУППИРОВАТЬ ПО
		|	ДатыПоследнегоСнятияСРегистрационногоУчета.ОсновноеСредство
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДатыПолученияРазрешенияНаСтроительство.ОсновноеСредство КАК ОсновноеСредство,
		|	ДатыПолученияРазрешенияНаСтроительство.ДатаРазрешенияНаСтроительство КАК ДатаРазрешенияНаСтроительство,
		|
		|	ВЫБОР
		|		КОГДА ДатыПолученияРазрешенияНаСтроительство.ДатаРазрешенияНаСтроительство > ДатыРегистрацииЗемли.ДатаРегистрацииЗемли
		|			ТОГДА НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ДатыПолученияРазрешенияНаСтроительство.ДатаРазрешенияНаСтроительство, МЕСЯЦ, 1), МЕСЯЦ)
		|		ИНАЧЕ ДатыРегистрацииЗемли.ДатаРегистрацииЗемли
		|	КОНЕЦ КАК ДатаНачалаУчетаПериодаСтроительства,
		|
		|	ВЫБОР
		|		КОГДА ВладениеЗемельнымиУчастками.ДатаРегистрацииПравНаОбъектНедвижимости = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
		|		ИНАЧЕ КОНЕЦПЕРИОДА(ВладениеЗемельнымиУчастками.ДатаРегистрацииПравНаОбъектНедвижимости, МЕСЯЦ)
		|	КОНЕЦ КАК ДатаОкончанияУчетаПериодаСтроительства
		|
		|ПОМЕСТИТЬ УчасткиПодЖилищноеСтроительство
		|
		|ИЗ
		|	ДатыПолученияРазрешенияНаСтроительство КАК ДатыПолученияРазрешенияНаСтроительство
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВладениеЗемельнымиУчастками КАК ВладениеЗемельнымиУчастками
		|		ПО ДатыПолученияРазрешенияНаСтроительство.ОсновноеСредство = ВладениеЗемельнымиУчастками.ОсновноеСредство
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыРегистрацииЗемли КАК ДатыРегистрацииЗемли
		|		ПО ДатыПолученияРазрешенияНаСтроительство.ОсновноеСредство = ДатыРегистрацииЗемли.ОсновноеСредство
		|
		|ГДЕ
		|	ВладениеЗемельнымиУчастками.ЖилищноеСтроительство
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
		...
		|
		|	ВЫБОР
		|		КОГДА УчасткиПодЖилищноеСтроительство.ОсновноеСредство ЕСТЬ NULL
		|			ТОГДА """"
		|		КОГДА &КонецМесяца >= УчасткиПодЖилищноеСтроительство.ДатаНачалаУчетаПериодаСтроительства
		|			И (УчасткиПодЖилищноеСтроительство.ДатаОкончанияУчетаПериодаСтроительства = ДАТАВРЕМЯ(1, 1, 1)
		|				ИЛИ &КонецМесяца <= УчасткиПодЖилищноеСтроительство.ДатаОкончанияУчетаПериодаСтроительства)
		|			ТОГДА 
		|				ВЫБОР
		|					КОГДА ДОБАВИТЬКДАТЕ(УчасткиПодЖилищноеСтроительство.ДатаРазрешенияНаСтроительство, ГОД, 3) < &НачалоМесяца
		|						ТОГДА ""2""
		|					ИНАЧЕ ""1""
		|				КОНЕЦ
		|		ИНАЧЕ """"
		...
		|		КОГДА УчасткиПодЖилищноеСтроительство.ОсновноеСредство ЕСТЬ NULL
		|			ТОГДА ВладениеЗемельнымиУчастками.Период
		|		КОГДА УчасткиПодЖилищноеСтроительство.ДатаОкончанияУчетаПериодаСтроительства <> ДАТАВРЕМЯ(1, 1, 1)
		|				И УчасткиПодЖилищноеСтроительство.ДатаОкончанияУчетаПериодаСтроительства < &НачалоМесяца
		|			ТОГДА УчасткиПодЖилищноеСтроительство.ДатаОкончанияУчетаПериодаСтроительства
		|		КОГДА &КонецМесяца >= УчасткиПодЖилищноеСтроительство.ДатаНачалаУчетаПериодаСтроительства
		|				И ДОБАВИТЬКДАТЕ(УчасткиПодЖилищноеСтроительство.ДатаРазрешенияНаСтроительство, ГОД, 3) < &НачалоМесяца
		|			ТОГДА ДОБАВИТЬКДАТЕ(УчасткиПодЖилищноеСтроительство.ДатаРазрешенияНаСтроительство, ГОД, 3)
		|		КОГДА &КонецМесяца >= УчасткиПодЖилищноеСтроительство.ДатаНачалаУчетаПериодаСтроительства
		|			ТОГДА УчасткиПодЖилищноеСтроительство.ДатаНачалаУчетаПериодаСтроительства
		...
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ УчасткиПодЖилищноеСтроительство КАК УчасткиПодЖилищноеСтроительство
		|		ПО ВладениеЗемельнымиУчастками.ОсновноеСредство = УчасткиПодЖилищноеСтроительство.ОсновноеСредство
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ВТ_ПромежуточныеДанные КАК ВТ_ПромежуточныеДанные
		|	ГДЕ 
		|		ВТ_ПромежуточныеДанные.ПостановкаНаУчетВНалоговомОргане В 
		|			(ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.ПоМестуНахожденияОрганизации),
		|			ЗНАЧЕНИЕ(Перечисление.ПостановкаНаУчетВНалоговомОргане.СДругимКодомПоОКАТО))) КАК ДатыИзменений
		|
		|СГРУППИРОВАТЬ ПО
		|	ДатыИзменений.ОсновноеСредство
		|
	...
	// 
	// 2. БлижайшиеСнятияСУчета 
	// Уже в свобдных данных для каждой даты регистрации участка определяем следующую после нее дату снятия с учета (если есть).
	// Таком образом определяем границы периода владения (требуются для заполнения отчетных форм).
	//
	// 3. Итоговый запрос
	// Определяем, в каких ИФНС уплачиваются авансы
...
Процедура ВыполнитьРасчетЗемельногоНалогаДо2020(ТаблицаРасчетНалога, ПараметрыРасчета, ДанныеДляРасчета, ДополнительныеПараметры)
	...
	Если ГодовойРасчет Тогда
		...
		Если ЕстьОбъектыСУплатойАвансов Тогда
			...
			РасчетНалога1Кв = ПолучитьРасчетПоЗемельномуНалогу(
				ПараметрыРасчета.Организация,
				Дата(Год(ПараметрыРасчета.ПериодРасчета), 03, 31),
				ДополнительныеПараметры
			).РасчетЗемельногоНалогаДо2020;
								
			РасчетНалога3Кв = ПолучитьРасчетПоЗемельномуНалогу(
				ПараметрыРасчета.Организация,
				Дата(Год(ПараметрыРасчета.ПериодРасчета), 09, 30),
				ДополнительныеПараметры
			).РасчетЗемельногоНалогаДо2020;
