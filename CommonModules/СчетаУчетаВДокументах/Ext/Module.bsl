...
//  ЭлементыСчетов           - Массив из Строка - имена элементов формы, связанных со счетами
//  ЭлементыПриСкрытыхСчетах - Массив из Строка - имена элементов формы, отображаемых, когда счета учета скрыты
//                           - Неопределено - если элементы формы, отображаемые при скрытых счетах, отсутствуют
//
...
Функция ФункцияВключена(Роль)
	...
	Если Роль = "ВозвратнаяТараПереданная" Или Роль = "ВозвратнаяТараПолученная" Или Роль = "ВозвратнаяТараНаСкладе"
		Или Роль = "ВозвратнаяТараЗаБалансом" Или Роль = "ВозвратнаяТараУПокупателя" Тогда
...
Функция МетодЗаполнения(Правило)
	...
	ИначеЕсли Роль = "ВыплатыСамозанятым" Тогда
	...
	ИначеЕсли Роль = "СубконтоНоменклатурнаяГруппаЗатрат1"
		Или Роль = "СубконтоНоменклатурнаяГруппаЗатрат2"
		Или Роль = "СубконтоНоменклатурнаяГруппаЗатрат3" Тогда
		Возврат "СубконтоНоменклатурнаяГруппаЗатрат";
	...
	ИначеЕсли Роль = "НаличныеДеньги"
		...
		Или Роль = "Депозит"
		Или Роль = "БезналичныеДеньгиЦифровые"
		Или Роль = "Инкассация"
	...
	ИначеЕсли Роль = "СубконтоВозвратНалогов1" Или Роль = "СубконтоВозвратНалогов2" Или Роль = "СубконтоВозвратНалогов3" Тогда
		Возврат "СубконтоВозвратНалогов";
...
Функция ИсточникиДанныхПравила(Правило)
	...
	Если МетодЗаполнения = "Деньги" Тогда
		...
		ИсточникиДанных.Добавить("БанковскийСчет");
		ИсточникиДанных.Добавить("ДеятельностьПлатежногоАгента");
	КонецЕсли;
	...
	Если МетодЗаполнения = "Налоги" Или МетодЗаполнения = "ВозвратУплатыНалога" Тогда
	...
	Если МетодЗаполнения = "СубконтоВозвратаНалогов" Тогда
		ИсточникиДанных.Добавить("Налог");
	КонецЕсли;
...
Функция НастройкиДляЗаполнения(Правило)
	...
	ИначеЕсли МетодЗаполнения = "СубконтоНоменклатурнаяГруппаЗатрат" Тогда
		
		Настройки.Добавить("СубконтоНоменклатурнаяГруппаЗатрат");
...
Функция ИсточникиДанныхНастройки(ИмяНастройки)
	...
	ИначеЕсли ИмяНастройки = "СтатьяЗатратНоменклатуры" Тогда
		ИсточникиДанных.Вставить("Номенклатура");
	ИначеЕсли ИмяНастройки = "СубконтоНоменклатурнаяГруппаЗатрат" Тогда
		ИсточникиДанных.Вставить("Номенклатура");
	...
	ИначеЕсли ЭтоНастройкаУчетнойПолитики(ИмяНастройки) Тогда
		ИсточникиДанных.Вставить("Организация");
		ИсточникиДанных.Вставить("Дата");
	ИначеЕсли ИмяНастройки = "ЭтоДенежныйДокумент" Тогда
		ИсточникиДанных.Вставить("ВидЦенностей");
	КонецЕсли;
...
Функция ПроверитьУсловиеПоФункциональнымОпциям(Условие) // очередность, сложность - 0
	...
	ИначеЕсли Условие = "ОперацииСКомиссионером" Тогда
		Если Не ПолучитьФункциональнуюОпцию("ОсуществляетсяРеализацияТоваровУслугЧерезКомиссионеров")
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьПродажиЧерезМаркетплейс") Тогда
			Возврат Ложь;
		Иначе
			Возврат Неопределено; // проверим еще
		КонецЕсли;
...
Функция ПроверитьУсловиеПоНастройкам(ИмяУсловия, ПланВыполнения, НастройкиЭлемента) // очередность, сложность - 2
	...
	ИначеЕсли ИмяУсловия = "УчетТарыНаБалансе" Тогда
		
		Возврат НЕ ДанныеНастройкиЭлементаКоллекции("УчитыватьТаруЗаБалансом", ПланВыполнения, НастройкиЭлемента);
	...
	ИначеЕсли ИмяУсловия = "ЭтоДенежныйДокумент" Тогда 
		
		Возврат ДанныеНастройкиЭлементаКоллекции("ЭтоДенежныйДокумент", ПланВыполнения, НастройкиЭлемента);
...
Функция НастройкиДляПроверкиУсловия(ИмяУсловия)
	...
	ИначеЕсли ИмяУсловия = "УчетТарыНаБалансе" Тогда
		...
		Настройки.Добавить("УчитыватьТаруЗаБалансом");
	...
	ИначеЕсли ИмяУсловия = "ЭтоДенежныйДокумент" Тогда
		...
		Настройки.Добавить("ЭтоДенежныйДокумент");
...
Функция ИсточникиДанныхДляПроверкиУсловия(ИмяУсловия)
	...
	ИначеЕсли ИмяУсловия = "ИспользоватьПереводыВПути" Тогда
		ИсточникиДанных.Вставить("Организация");
		ИсточникиДанных.Вставить("Дата");
	ИначеЕсли ИмяУсловия = "ЦифровойСчетКор" Тогда
		ИсточникиДанных.Вставить("СчетКонтрагента");
	КонецЕсли;
...
Функция ДанныеНастройки(ИмяНастройки, ПредметыНастройки)
	...
	ИначеЕсли ИмяНастройки = "ВидДеятельностиНоменклатуры" Или ИмяНастройки = "СубконтоНоменклатурнаяГруппаЗатрат" Тогда
	...
	ИначеЕсли ИмяНастройки = "ЭтоДенежныйДокумент" Тогда
		...
		Возврат ДанныеНастройкиЭтоДенежныйДокумент(ПредметыНастройки);
...
Процедура ЗаполнитьРеквизит(Параметры)
	...
	ИначеЕсли МетодЗаполнения = "СубконтоНоменклатурнаяГруппаЗатрат" Тогда
		...
		ЗаполнитьЗначениеСубконтоНоменклатурнаяГруппаЗатрат(Параметры);
	...
	ИначеЕсли МетодЗаполнения = "ВыплатыСамозанятым" Тогда
		
		ЗаполнитьВыплатыСамозанятым(Параметры);
	...
	ИначеЕсли МетодЗаполнения = "Налоги" Или МетодЗаполнения = "ВозвратУплатыНалога" Тогда
		
		ЗаполнитьНалоги(Параметры);
		
	ИначеЕсли МетодЗаполнения = "СубконтоНалогов" Или МетодЗаполнения = "СубконтоВозвратНалогов" Тогда
	...
	ИначеЕсли МетодЗаполнения = "ОсновныеСредства" Тогда
		...
		ПараметрыЗаполнения = Новый Структура("ЕдиницаУчета", Неопределено);
		ЗаполнитьЗначенияСвойств(ПараметрыЗаполнения, Параметры.Элемент.ОсновноеСредство);
		
		Если ПараметрыЗаполнения.ЕдиницаУчета = Перечисления.ЕдиницыУчетаОС.ГрупповойОбъект Тогда
			Параметры.Значение = ПланыСчетов.Хозрасчетный.ГрупповыеОбъектыОС;
		ИначеЕсли Параметры.ПланВыполнения.ДанныеЗаполнения.ОбъектыПредназначеныДляСдачиВАренду = Ложь Тогда
	...
	ИначеЕсли МетодЗаполнения = "ОсновныеСредстваАмортизация" Тогда
		...
		Если ПараметрыЗаполнения.ЕдиницаУчета = Перечисления.ЕдиницыУчетаОС.ГрупповойОбъект Тогда
			Параметры.Значение = ПланыСчетов.Хозрасчетный.АмортизацияГрупповыхОбъектовОС;
		ИначеЕсли Параметры.ПланВыполнения.ДанныеЗаполнения.ОбъектыПредназначеныДляСдачиВАренду = Ложь Тогда
	...
	ИначеЕсли МетодЗаполнения = "ДенежныеДокументы" Тогда
		
		ЗаполнитьДенежныеДокументы(Параметры);
...
Функция РесурсНастройкиЗапасы(Роль)
	...
	Если Роль      = "Запасы"
		...
		Или Роль   = "ВозвратнаяТараЗаБалансом"
		Или Роль   = "ЗапасыЗатраты" Тогда
		
		Возврат "СчетУчета";
		
	ИначеЕсли Роль = "ЗапасыОтгруженные"
		Или Роль = "ВозвратнаяТараУПокупателя" Тогда
...
Функция СчетЗапрещенРольюЗапасыКромеПринятыхНаКомиссиюИлиВПереработку(Счет)
	...
	Возврат БухгалтерскийУчетПовтИсп.СчетВИерархии(Счет, ПланыСчетов.Хозрасчетный.МатериалыПринятыеВПереработку_)
		...
		Или БухгалтерскийУчетПовтИсп.СчетВИерархии(Счет, ПланыСчетов.Хозрасчетный.ВозвратнаяТара);
...
Функция ДанныеНастройкиУчетнойПолитики(ИмяНастройки, ПредметыНастройки)
	...
	Для Каждого ПредметНастройки Из ПредметыНастройки Цикл
		...
		ИначеЕсли ИмяНастройки = "УчитыватьТаруЗаБалансом" Тогда
			ЗначениеНастройки = УчетнаяПолитика.УчитыватьТаруЗаБалансом(ПредметНастройки.Организация, ПредметНастройки.Дата);
...
Функция ЭтоНастройкаУчетнойПолитики(ИмяНастройки)
	...
	Возврат ИмяНастройки = "СпособОценкиТоваровВРознице"
		Или ИмяНастройки = "УчитыватьТаруЗаБалансом"
...
Процедура ЗаполнитьЗатраты(Параметры)
	...
	ИначеЕсли Параметры.Правило.Роль = "РасходыУслугиБанка" Тогда
		...
		ВыделятьНДССКомиссииБанка = Ложь;
		БезЗакрывающихДокументов = Параметры.Элемент.БезЗакрывающихДокументов;
		Если Не БезЗакрывающихДокументов
			И Параметры.Элемент.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
			Дата = ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Дата");
			Организация = ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Организация");
			Контрагент = ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Контрагент");
			ДоговорКонтрагента = ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "ДоговорКонтрагента");
		...
		УчитыватьНДСПоКомиссии = Ложь;
		Если ВыделятьНДССКомиссииБанка Тогда
			Операция = Новый Структура;
			Операция.Вставить("НазначениеПлатежа", ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "НазначениеПлатежа"));
			...
			Если ЭтоСБП Тогда
				ТипОплаты = Перечисления.ТипыОплат.СБП;
			Иначе
				ТипОплаты = Перечисления.ТипыОплат.ПлатежнаяКарта;
			КонецЕсли;
			
			ВидОплаты = Справочники.ВидыОплатОрганизаций.ВидОплатыПоДоговоруИТипуОплаты(
				Организация, Контрагент, ДоговорКонтрагента, ТипОплаты);
			Если Не ЗначениеЗаполнено(ВидОплаты) Тогда
				ВидОплаты = Справочники.ВидыОплатОрганизаций.ВидОплатыПоДоговору(Организация, Контрагент, ДоговорКонтрагента);
			КонецЕсли;
			
			ПараметрыВидаОплаты = Справочники.ВидыОплатОрганизаций.ПараметрыВидОплаты(
				ВидОплаты,
				Организация,
				Контрагент,
				ДоговорКонтрагента,
			...
			УчитыватьНДСПоКомиссии = ПараметрыВидаОплаты.УчитыватьНДСПоКомиссии
				И ПараметрыВидаОплаты.УчитыватьНДСПоКомиссииЭталон;
		КонецЕсли;
		
		Если УчитыватьНДСПоКомиссии Тогда
			СчетаУчета = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаРасчетовСКонтрагентом(
				Организация, Контрагент, ДоговорКонтрагента);
			Если ЗначениеЗаполнено(СчетаУчета) Тогда
				Если ЗначениеЗаполнено(СчетаУчета.СчетАвансов) Тогда
					Параметры.Значение = СчетаУчета.СчетАвансов;
				ИначеЕсли ЗначениеЗаполнено(СчетаУчета.СчетРасчетов)Тогда
					Параметры.Значение = СчетаУчета.СчетРасчетов;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСубконтоЗатрат(Параметры)
	...
	Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
		...
		ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты Тогда
			Если СвойствоВладельцаПравила(Параметры.Правило, "Роль") = "РасходыУслугиБанка" Тогда
				Параметры.ЗначениеПоУмолчанию = ЗначениеРеквизитаЭлементаКоллекции(
					Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "Контрагент");
			КонецЕсли;
		...
		ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами Тогда
			Если СвойствоВладельцаПравила(Параметры.Правило, "Роль") = "РасходыУслугиБанка" Тогда
				Параметры.ЗначениеПоУмолчанию = Параметры.Элемент.Ссылка;
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьПодразделение(Параметры)
	...
	Если Параметры.Правило.Роль = "ПодразделениеБанковскогоСчета" Тогда
		...
		ИначеЕсли Параметры.ПланВыполнения.ДанныеЗаполнения.Свойство("БанковскийСчетЗаполнен") Тогда
			// Надо принудительно очистить подразделение
			Параметры.ЗначениеПоУмолчанию = Справочники.ПодразделенияОрганизаций.ПустаяСсылка();
			Параметры.Очистить = Истина;
		КонецЕсли;
...
Функция РесурсНастройкиВыручка(Роль)
	...
	ИначеЕсли Роль = "ВыручкаАкцизы" Тогда 
...
Процедура ЗаполнитьВидДеятельности(Параметры)
	...
	ИначеЕсли СвойстваСчета.ВидСубконто1 = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы Тогда
		...
		Если Параметры.Правило.Роль = "ВидДеятельностиРеализацияОсновныхСредств" Тогда
			Параметры.Значение = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.РеализацияОсновныхСредств");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
			Параметры.Значение = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПрочиеДоходыИРасходы.РеализацияПрочегоИмущества");
		КонецЕсли;
...
Процедура ЗаполнитьЗначениеСубконтоНоменклатурнаяГруппаЗатрат(Параметры)
	
	КоличествоСубконто = 3;
	
	// Определим НомерСубконто
	Для НомерСубконто = 1 По КоличествоСубконто + 1 Цикл
		Если Параметры.Правило.Роль = "СубконтоНоменклатурнаяГруппаЗатрат" + НомерСубконто Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы Тогда
		...
		Параметры.Значение = БухгалтерскийУчетВызовСервераПовтИсп.ОсновнаяНоменклатурнаяГруппа();
		Если Не ЗначениеЗаполнено(Параметры.Значение) Тогда
			Параметры.Значение = ДанныеНастройкиЭлементаКоллекции("СубконтоНоменклатурнаяГруппаЗатрат", Параметры);
		КонецЕсли;
...
Процедура ЗаполнитьДеньги(Параметры)
	...
	ПереводНаДругойСчет = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ПланВыполнения.ДанныеЗаполнения, "ПереводНаДругойСчет", Ложь);
	ВзносНаличнымиВБанк = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ПланВыполнения.ДанныеЗаполнения, "ВзносНаличнымиВБанк", Ложь);
	ПолучениеНаличныхВБанке = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ПланВыполнения.ДанныеЗаполнения, "ПолучениеНаличныхВБанке", Ложь);
	ЭтоДепозит = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ПланВыполнения.ДанныеЗаполнения, "Депозит", Ложь);
	ЦифровойСчетКор = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		Параметры.ПланВыполнения.ДанныеЗаполнения, "ЦифровойСчетКор", Ложь);
	Если ПереводНаДругойСчет
		Или ЭтоДепозит
		Или ЦифровойСчетКор
		Или ВзносНаличнымиВБанк
		Или ПолучениеНаличныхВБанке Тогда
		
		Если ЦифровойСчетКор Тогда
			БанковскийСчет = ЗначениеРеквизитаЭлементаКоллекции(
				Параметры.ПланВыполнения.ДанныеЗаполнения,
				Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта,
				"БанковскийСчет");
		Иначе
			БанковскийСчет = ЗначениеРеквизитаЭлементаКоллекции(
		...
		Если ЗначениеЗаполнено(БанковскийСчет) Тогда
			СчетБанк = Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(БанковскийСчет);
			Если ЗначениеЗаполнено(СчетБанк) Тогда
				Параметры.Значение = СчетБанк;
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	ДеятельностьПлатежногоАгента = Ложь;
	Если Параметры.ПланВыполнения.ДанныеЗаполнения.Свойство("ДеятельностьПлатежногоАгента", ДеятельностьПлатежногоАгента)
		И ДеятельностьПлатежногоАгента = Истина Тогда
		Параметры.Значение = ПланыСчетов.Хозрасчетный.ОсновнойСчет("КассаПлатежногоАгента");
		Возврат;
	КонецЕсли;
	...
	Если ОбщееЗначение(Параметры.КешОбщихЗначений, "ИспользоватьВалютныйУчет") Тогда
		...
		// В обработке "Ввод начальных остатков" реквизит "Валюта" в табличной части заполняется без переопределения источника данных.
		...
		Если ВалютаДокумента = Неопределено Тогда
			ВалютаДокумента = ЗначениеРеквизитаЭлементаКоллекции(Параметры.Элемент, Параметры.Правило.ИсточникиДанных.РеквизитыОбъекта, "ВалютаДокумента");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СчетОрганизации) Тогда
		СчетБанк = Справочники.БанковскиеСчета.СчетУчетаБанковскогоСчета(СчетОрганизации);
		Если ЗначениеЗаполнено(СчетБанк) Тогда
			Параметры.Значение = СчетБанк;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьДенежныеДокументы(Параметры)
	...
	Если ОбщееЗначение(Параметры.КешОбщихЗначений, "ИспользоватьВалютныйУчет") Тогда
		...
		ВалютаРегламентированногоУчета = ОбщееЗначение(Параметры.КешОбщихЗначений, "ВалютаРегламентированногоУчета");
		УчетВВалюте = ЗначениеЗаполнено(ВалютаДокумента) И ВалютаДокумента <> ВалютаРегламентированногоУчета;
		
		Если УчетВВалюте Тогда
			Параметры.Значение = ПланыСчетов.Хозрасчетный.ОсновнойСчет(Роль + "ВВалюте");
		Иначе
			Параметры.Значение = Параметры.ЗначениеПоУмолчанию;
		КонецЕсли;
...
Процедура ЗаполнитьНалоги(Параметры)
	...
	Если Роль = "ВозвратУплатыНалога" И Налог = Справочники.ВидыНалоговИПлатежейВБюджет.ПрочиеНалогиИСборы Тогда
		ЗаполнитьПрочиеДоходы(Параметры);
		Параметры.Значение = Параметры.ЗначениеПоУмолчанию;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСубконтоНалогов(Параметры)
	...
	// Определим НомерСубконто
	Для НомерСубконто = 1 По КоличествоСубконто + 1 Цикл
		Если Параметры.Правило.Роль = "СубконтоНалогов" + НомерСубконто
			Или Параметры.Правило.Роль = "СубконтоВозвратНалогов" + НомерСубконто Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	ВозвратУплатыНалога = СтрНайти(Параметры.Правило.Роль, "СубконтоВозвратНалогов") > 0;
	...
	Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыПлатежейВГосБюджет Тогда
		...
		Если ВозвратУплатыНалога И Не ЗначениеЗаполнено(ТекущееЗначение) Тогда
			ТекущееЗначение = Параметры.ЗначениеПоУмолчанию;
		КонецЕсли;
	ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.УровниБюджетов
		И МетодЗаполнения <> "СубконтоВозвратНалогов" Тогда
		
		Параметры.ЗначениеПоУмолчанию = ТипЗначения.ПривестиЗначение(Неопределено);
		
		ТекущееЗначение = ДанныеНастройкиЭлементаКоллекции("УровеньБюджета", Параметры);
		
	ИначеЕсли ВозвратУплатыНалога И ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы Тогда
		Параметры.ЗначениеПоУмолчанию = Справочники.ПрочиеДоходыИРасходы.ОсновнаяСтатьяПрочихРасходов();
		ТекущееЗначение = Параметры.ЗначениеПоУмолчанию;
		Параметры.Очистить = Истина;
	Иначе
...
#Область ЦенностиКасса

Функция ДанныеНастройкиЭтоДенежныйДокумент(ПредметыНастройки)
	
	Результат = НовыйДанныеНастройки(ПредметыНастройки);
	Для Каждого ПредметНастройки Из ПредметыНастройки Цикл
		
		ЗначениеНастройки = ТипЗнч(ПредметНастройки.ВидЦенностей) = Тип("СправочникСсылка.ДенежныеДокументы");
...
Процедура ЗаполнитьВыплатыСамозанятым(Параметры)
	
	Параметры.Значение = ПланыСчетов.Хозрасчетный.РасчетыССамозанятыми;
