...
Функция АдресаЭлектроннойПочты(Контрагент) Экспорт
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты")
		Или ТипЗнч(Контрагент) = Тип("СправочникСсылка.ФизическиеЛица")
		Или ТипЗнч(Контрагент) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
	...
	|	КонтактныеЛица.Фамилия + "" "" + КонтактныеЛица.Имя + "" "" + КонтактныеЛица.Отчество
	|ИЗ
	|	Справочник.КонтактныеЛица КАК КонтактныеЛица
	|ГДЕ
	|	КонтактныеЛица.Ссылка В(&Контрагенты)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА КонтактныеЛица.ОбъектВладелец ССЫЛКА Справочник.Контрагенты
	|			ТОГДА ВЫРАЗИТЬ(КонтактныеЛица.ОбъектВладелец КАК Справочник.Контрагенты).НаименованиеПолное
	|		КОГДА КонтактныеЛица.ОбъектВладелец ССЫЛКА Справочник.Пользователи
	|			ТОГДА ВЫРАЗИТЬ(КонтактныеЛица.ОбъектВладелец КАК Справочник.Пользователи).ФизическоеЛицо.Наименование
	|		ИНАЧЕ """"
	|	КОНЕЦ
	|ИЗ
	|	Справочник.КонтактныеЛица КАК КонтактныеЛица
	...
	|		ПО Контрагенты.Контрагент = КонтактныеЛицаКонтактнаяИнформация.Ссылка
