...
Процедура УстановитьУсловноеОформлениеСпискаДокументов(Список, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Оформление.Поля, "Контрагент");
...
Процедура ПриОпределенииСтатусаПроверкиКонтрагентаВБазеФНС(
	...
	Если ТипЗнч(ДанныеКонтрагента.Контрагент) <> Тип("СправочникСсылка.Контрагенты")
		Или ДанныеКонтрагента.Контрагент.ПустаяСсылка() Тогда
		// Это либо не контрагент, либо пустая ссылка, поэтому дальнейшие проверки бессмыслены.
		Возврат;
	КонецЕсли;
	
	Если Справочники.Контрагенты.ЭтоИностраннаяОрганизация(ДанныеКонтрагента.Контрагент) Тогда
		ОтключитьПроверкуФНС = Истина;
		ПричинаОтключения = РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСИностраннаяОрганизация();
	ИначеЕсли Справочники.Контрагенты.ЭтоФизическоеЛицо(ДанныеКонтрагента.Контрагент) Тогда
		ОтключитьПроверкуФНС = Истина;
		ПричинаОтключения = РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСФизическоеЛицоОтстуствуетВБазеФНС();
	КонецЕсли;
...
Процедура ДополнитьРеквизитыФормыКонтрагент(Форма, НовыеРеквизиты) Экспорт
	...
	ЭтоФизическоеЛицо = Справочники.Контрагенты.ЭтоФизическоеЛицо(Форма.Объект.Ссылка);
	НовыеРеквизиты.ОтключитьПроверкуФНС = (НовыеРеквизиты.ЭтоИностранныйКонтрагент Или ЭтоФизическоеЛицо);
	
	Если НовыеРеквизиты.ЭтоИностранныйКонтрагент Тогда
		НовыеРеквизиты.ПричинаОтключенияПроверкиФНС = РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСИностраннаяОрганизация();
	ИначеЕсли ЭтоФизическоеЛицо Тогда
		НовыеРеквизиты.ПричинаОтключенияПроверкиФНС = РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСФизическоеЛицоОтстуствуетВБазеФНС();
	КонецЕсли;
...
Процедура ДополнитьДанныеКонтрагентов(ДанныеКонтрагентов, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Запрос.УстановитьПараметр("ПричинаОтключенияПроверкиИностраннаяОрганизация",
			РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСИностраннаяОрганизация());
		Запрос.УстановитьПараметр("ПричинаОтключенияПроверкиФизическоеЛицо",
			РаботаСКонтрагентамиБП.ПричинаОтключенияПроверкиФНСФизическоеЛицоОтстуствуетВБазеФНС());
		...
		Запрос.Текст = 
			...
			|			ТОГДА """"
			|		КОГДА Контрагенты.СтранаРегистрации <> ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
			|			ТОГДА &ПричинаОтключенияПроверкиИностраннаяОрганизация
			|		КОГДА Контрагенты.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
			|				И (Контрагенты.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо)
			|					ИЛИ Контрагенты.ИндивидуальныйПредприниматель)
			|			ТОГДА """"
			|		ИНАЧЕ &ПричинаОтключенияПроверкиФизическоеЛицо
