...
Функция ЗагрузкаОтчетаИзФайла(ФормаОтчета, ПараметрыОтчета, ДанныеДляЗагрузки, ДеревоДляЗагрузки) Экспорт
	...
	РезультатДлительнойОперации = ДлительныеОперации.ВыполнитьВФоне(
		"РегламентированнаяОтчетностьЗагрузка.ЗагрузкаОтчетаИзФайлаДлительнаяОперация",
		ПараметрыВыполнения,
		ПараметрыЗапуска);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ЗагрузкаОтчетаИзФайлаДлительнаяОперация", Истина);
КонецПроцедуры
...
Функция ЗаполнитьДаннымиСохраненныйДокумент(СохраненныйДок, ДеревоДляЗагрузки, ПараметрыОтчета, ДанныеДляЗагрузки, ТекстыПравилЗагрузки)
	...
	ДанныеОтчета.Вставить("ДеревоДанныхРазделов");         // используется в бухотчетности
...
Процедура ДополнитьПараметрыШаблонамиДанныхМультиРазделов(ДанныеОтчета, ПараметрыОтчета)
	...
	ШаблоныДереваДанныхРазделов          = Новый Структура;
	...
	// Добавить в параметры отчета шаблоны из данных дерева данных разделов бухотчетности.
	Если ТипЗнч(ДеревоДанныхРазделов) = Тип("ДеревоЗначений") Тогда
		
		СкопированноеДерево = СкопироватьКолонкиДерева(ДеревоДанныхРазделов);
		
		Для Каждого СтрокаДерева Из ДеревоДанныхРазделов.Строки Цикл
			
			СкопированнаяСтрокаДерева = СкопированныйУзелМУР(СкопированноеДерево, СтрокаДерева, Ложь);
			
			Если СтрокаДерева.Тип = "Раздел" Тогда
			
				ШаблоныДереваДанныхРазделов.Вставить(СтрокаДерева.Идентификатор, СкопированнаяСтрокаДерева);
	...
	ПараметрыОтчета.Вставить("ШаблоныДереваДанныхРазделов",          ШаблоныДереваДанныхРазделов);
...
Процедура ЗаполнитьПоказательДокументаИзУзлаДерева(Узел, ДанныеОтчета, ПараметрыОтчета, НомерСтраницы = 0, НомерСтроки = 0)
	...
	Если ТипЗнч(НомерСтраницы) = Тип("Массив") Тогда // раздел многоуровневый (многостраничный)
		...
		Если ДанныеОтчета.ДанныеМногоуровневыхРазделов = Неопределено
		   И ДанныеОтчета.ДеревоДанныхРазделов = Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеОтчета.ДеревоДанныхРазделов) = Тип("ДеревоЗначений") Тогда // данные БО
			
			ЭтоДанныеРазделаБО = Истина;
			
			НайденныеРазделы = ДанныеОтчета.ДеревоДанныхРазделов.Строки.НайтиСтроки(
				Новый Структура("Тип, Идентификатор", "Раздел", ИмяРаздела));
			Если ЗначениеЗаполнено(НайденныеРазделы) Тогда
				МногоуровневыйРаздел = НайденныеРазделы[0];
			КонецЕсли;
			
		КонецЕсли;
		
		Если (ТипЗнч(МногоуровневыйРаздел) <> Тип("ДеревоЗначений")
			И ТипЗнч(МногоуровневыйРаздел) <> Тип("СтрокаДереваЗначений"))
				ИЛИ МногоуровневыйРаздел.Строки.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
		...
		Если ЭтоДанныеРазделаБО Тогда
			
			ВетвьШаблона = ПараметрыОтчета.ШаблоныДереваДанныхРазделов[ИмяРаздела];
			СтраницаРаздела = МногоуровневыйРаздел.Строки;
		...
		Если ЗначениеЗаполнено(НомерСтроки) Тогда // массив не пустой - определена многоуровневая многострочность в разделе
			...
			Если ЭтоДанныеРазделаБО Тогда // для бухотчетности с 2025 года новый формат хранения данных
				
				Если СтрНачинаетсяС(Узел.Ключ, "П") И СтрДлина(Узел.Ключ) < 13 Тогда
					Возврат;
				КонецЕсли;
				
				СтрокаМногострочнойЧастиПоказателяШаблон = Неопределено;
				
				МногострочныеЧасти = ВетвьШаблона.Строки.НайтиСтроки(Новый Структура("Тип", "МногострочнаяЧасть"), Истина);
				Для Каждого МногострочнаяЧасть Из МногострочныеЧасти Цикл
					Для Каждого СтрокаМногострочнойЧасти Из МногострочнаяЧасть.Строки Цикл
						Если СтрокаМногострочнойЧасти.Тип = "Строка" Тогда
							СтрокиПоказателей = СтрокаМногострочнойЧасти.Строки.НайтиСтроки(
								Новый Структура("Тип, Идентификатор", "Показатель", Узел.Ключ));
							Если ЗначениеЗаполнено(СтрокиПоказателей) Тогда
								СтрокаМногострочнойЧастиПоказателяШаблон = СтрокаМногострочнойЧасти;
							КонецЕсли;
						КонецЕсли;
					КонецЦикла;
					Если СтрокаМногострочнойЧастиПоказателяШаблон <> Неопределено Тогда
						Прервать;
					КонецЕсли;
				КонецЦикла;
				...
				Если СтрокаМногострочнойЧастиПоказателяШаблон = Неопределено Тогда
					ОбщегоНазначения.СообщитьПользователю(
						СтрШаблон(НСтр("ru = 'В разделе ""%1"" не найдена многострочная часть для показателя ""%2"".'"),
						Раздел, Узел.Ключ));
					Возврат;
				КонецЕсли;
				...
				СтраницаРаздела = СтрокаМногострочнойЧастиПоказателяБО(
					МногоуровневыйРаздел, СтрокаМногострочнойЧастиПоказателяШаблон, НомерСтроки).Строки;
				
			ИначеЕсли МногоуровневыйРаздел.Колонки.Найти("ДанныеМногострочныхЧастей") <> Неопределено Тогда
	...
	Если НомерСтроки = 0 ИЛИ ЭтоДанныеРазделаБО
	 ИЛИ(ТипЗнч(НомерСтроки) = Тип("Массив") И ЕстьДанныеМногострочныхЧастейМнУр <> Ложь) Тогда // обычные и многоуровневые разделы
		ПоказательНайден = Ложь;
		
		СтрокаПоказателяБО = Неопределено;
		Если ЭтоДанныеРазделаБО Тогда
			НайденныеПоказатели = СтраницаРаздела.НайтиСтроки(
				Новый Структура("Тип, Идентификатор", "Показатель", Узел.Ключ));
			Если ЗначениеЗаполнено(НайденныеПоказатели) Тогда
				ПоказательНайден = Истина;
				СтрокаПоказателяБО = НайденныеПоказатели[0];
				ЗначениеПоказателя = СтрокаПоказателяБО.Данные;
			КонецЕсли;
		Иначе
			ПоказательНайден = СтраницаРаздела.Свойство(Узел.Ключ, ЗначениеПоказателя);
		КонецЕсли;
		...
		Если ПоказательНайден Тогда
			...
			Если ЭтоДанныеРазделаБО Тогда
				Если Узел.Формат = "Q" Тогда
					СтрокаПоказателяБО.Данные = НовоеЗначениеПоказателя;
				Иначе
					СтрокаПоказателяБО.Данные = ОбработанноеЗначениеПоказателя(Узел.Ключ, ПараметрыОтчета, НовоеЗначениеПоказателя);
				КонецЕсли;
			Иначе
...
Функция СтрокаМногострочнойЧастиПоказателяБО(МногоуровневыйРазделБО, СтрокаМногострочнойЧастиШаблон, НомерСтроки);
	
	ВложенностьМнЧ = Новый Массив;
	
	РодительскаяСтрокаМнЧ = СтрокаМногострочнойЧастиШаблон.Родитель;
	Пока РодительскаяСтрокаМнЧ <> Неопределено И РодительскаяСтрокаМнЧ.Тип = "МногострочнаяЧасть" Цикл
		ВложенностьМнЧ.Вставить(0, РодительскаяСтрокаМнЧ);
		РодительскаяСтрокаМнЧ = РодительскаяСтрокаМнЧ.Родитель.Родитель;
	КонецЦикла;
	
	// Приведем массивы к одному размеру, если существует несоответствие между схемой выгрузки и данными отчета.
	Разница = НомерСтроки.Количество() - ВложенностьМнЧ.Количество();
	Пока НомерСтроки.Количество() <> ВложенностьМнЧ.Количество() Цикл
		Если Разница > 0 Тогда
			НомерСтроки.Удалить(0);
		Иначе
			НомерСтроки.Вставить(0, 1);
		КонецЕсли;
	КонецЦикла; 
	
	МногострочнаяЧастьВерхнегоУровня = МногоуровневыйРазделБО.Строки.НайтиСтроки(
		Новый Структура("Тип, Идентификатор", "МногострочнаяЧасть", ВложенностьМнЧ[0].Идентификатор))[0];
	
	СтрокаМногострочнойЧасти = МногострочнаяЧастьВерхнегоУровня.Строки.Найти("Строка", "Тип");
	
	ТекущаяМногострочнаяЧасть = МногострочнаяЧастьВерхнегоУровня;
	
	Для Каждого НомСтроки Из НомерСтроки Цикл
		СтрокиТекущейМнЧШаблона = ВложенностьМнЧ[0].Строки.НайтиСтроки(Новый Структура("Тип", "Строка"));
		СтрокиТекущейМнЧ = ТекущаяМногострочнаяЧасть.Строки.НайтиСтроки(Новый Структура("Тип", "Строка"));
		
		ВложенностьМнЧ.Удалить(0);
		
		КвоДобавляемыхСтрок = НомСтроки - СтрокиТекущейМнЧ.Количество();
		Для Сч = 1 По КвоДобавляемыхСтрок Цикл
			НоваяСтрокаТекущейМнЧ = СкопированныйУзелМУР(ТекущаяМногострочнаяЧасть, СтрокиТекущейМнЧШаблона[0], Ложь);
			СтрокиТекущейМнЧ.Добавить(НоваяСтрокаТекущейМнЧ);
		КонецЦикла;
		
		СтрокаМногострочнойЧасти = СтрокиТекущейМнЧ[НомСтроки - 1];
		
		Если ВложенностьМнЧ.Количество() > 0 Тогда
			ТекущаяМногострочнаяЧасть = СтрокаМногострочнойЧасти.Строки.НайтиСтроки(
				Новый Структура("Тип, Идентификатор", "МногострочнаяЧасть", ВложенностьМнЧ[0].Идентификатор))[0];
		КонецЕсли;
	КонецЦикла;
	
	Возврат СтрокаМногострочнойЧасти;
...
Процедура ВыполнитьМетодОбъекта(Объект, ИмяМетода, Параметры)
	
	Если ЗначениеЗаполнено(Параметры) Тогда
		
		Если ВРег(ИмяМетода) = ВРег("ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО") Тогда
			РегламентированнаяОтчетностьАЛКО.ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО(Параметры[0], Параметры[1]);
		КонецЕсли;
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры)
	
	Если ЗначениеЗаполнено(Параметры) Тогда
		
		Если ВРег(ИмяМетода) = ВРег("РегламентированнаяОтчетностьАЛКО.ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО") Тогда
			РегламентированнаяОтчетностьАЛКО.ПостОбработкаПриЗагрузкеИзФайлаВыгрузкиАЛКО(Параметры[0], Параметры[1]);
		КонецЕсли;
...
Процедура ДополнитьСтруктуруРеквизитовПараметрамиКопирования(СтруктураРеквизитовФормы) Экспорт 
	...
	СтруктураРеквизитовФормы.Вставить("ПредлагатьКонвертацию", Ложь);
	СтруктураРеквизитовФормы.Вставить("ЕстьАктуальныеФормы", Ложь);
	СтруктураРеквизитовФормы.Вставить("КонвертацияИзФормыОнлайнБлокировок", Ложь);
	СтруктураРеквизитовФормы.Вставить("ДоступныеДляКонвертацииФормы", Новый Массив);
	СтруктураРеквизитовФормы.Вставить("АктуальныеФормы", Новый Массив);
	СтруктураРеквизитовФормы.Вставить("МеханизмОнлайнСервисовВключен",
		ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.МеханизмОнлайнСервисовВключен());
КонецПроцедуры
