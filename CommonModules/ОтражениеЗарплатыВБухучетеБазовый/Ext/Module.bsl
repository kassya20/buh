...
Процедура СоздатьВТБухучетНачисленийБезДоговоровГПХ(Организация, ПериодРегистрации, МенеджерВременныхТаблиц)
	...
	// Получим сведения о настройке бухучета для первичного документа.
	СоздатьВТНастройкиБухучетаПервичныхДокументов(МенеджерВременныхТаблиц, "ВТНачисленияБезДоговоровГПХ");
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(БухучетДокументов.СпособОтраженияЗарплатыВБухучете, ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка)
	...
	|	ЕСТЬNULL(БухучетДокументов.ПодразделениеУчетаЗатрат, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК ПодразделениеУчетаЗатрат,
	|	ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНастройкиБухучетаНачисленийСотрудников КАК БухучетСотрудников
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияПоДокументамДляБухучета КАК БухучетДокументов
	|		ПО Начисления.ДокументОснование = БухучетДокументов.ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА СписокНачислений.ПодразделениеУчетаЗатрат = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
	|			ТОГДА СписокНачислений.Подразделение
	|		ИНАЧЕ СписокНачислений.ПодразделениеУчетаЗатрат
...
Функция СреднийЗаработокОбщийСотрудника(Сотрудник, ДатаНачалаСобытия)
	...
	ОтработанноеВремяДляСреднегоОбщий.Колонки.Добавить("ОтработаноДнейПятидневка", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(7, 2)));
	ОтработанноеВремяДляСреднегоОбщий.Колонки.Добавить("НормаДнейПроизводственныйКалендарь", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(7, 2)));
	...
	СреднийЗаработокОбщий.Колонки.Добавить("Год", Новый ОписаниеТипов("Число"));
	СреднийЗаработокОбщий.Колонки.Добавить("КоличествоМесяцев", Новый ОписаниеТипов("Число"));
	СреднийЗаработокОбщий.Колонки.Добавить("СоставнаяЧасть", Новый ОписаниеТипов("ПеречислениеСсылка.УчетНачисленийВСреднемЗаработкеОбщий"));
	СреднийЗаработокОбщий.Колонки.Добавить("ДатаНачалаБазовогоПериода", Новый ОписаниеТипов("Дата"));
...
// 			* Сумма - Число.
//
Функция НастройкиБухучетаДляРаспределенияПоУмолчанию(Организация, СотрудникиБезБухучета, Период) Экспорт
	
	БухучетБазовыхНачислений = ТаблицаБухучетБазовыхНачислений();
	
	КадровыеДанныеСтрока = "Подразделение";
	ТаблицаСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, СотрудникиБезБухучета, КадровыеДанныеСтрока, КонецМесяца(Период));
...
Процедура СоздатьВТНастройкиБухучетаПервичныхДокументов(МенеджерВТ, ИмяИсходнойВТ)
	...
	|ПОМЕСТИТЬ ВТДокументыДляПолученияНастроек
	|ИЗ
	|	ИмяИсходнойВТ КАК Таблица";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяИсходнойВТ", ИмяИсходнойВТ);
	Запрос.Выполнить();
	УдалитьВТ.Добавить("ВТДокументыДляПолученияНастроек");
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Таблица.ДокументОснование КАК ДокументОснование,
	|	ВЫБОР
	|		КОГДА НЕ БольничныйЛист.Ссылка ЕСТЬ NULL
	|			ТОГДА БольничныйЛист.СпособОтраженияЗарплатыВБухучете
	|		КОГДА НЕ ДокументПремия.Ссылка ЕСТЬ NULL
	|			ТОГДА ДокументПремия.СпособОтраженияЗарплатыВБухучете
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка)
	|	КОНЕЦ КАК СпособОтраженияЗарплатыВБухучете,
	|	ВЫБОР
	|		КОГДА ДокументПремия.Ссылка ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)
	|		ИНАЧЕ ДокументПремия.СтатьяФинансирования
	|	КОНЕЦ КАК СтатьяФинансирования,
	|	ВЫБОР
	|		КОГДА НЕ БольничныйЛист.Ссылка ЕСТЬ NULL
	|			ТОГДА БольничныйЛист.ПодразделениеУчетаЗатрат
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
	|	КОНЕЦ КАК ПодразделениеУчетаЗатрат,
	|	ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.НеЕНВД) КАК ОтношениеКЕНВД
	|ПОМЕСТИТЬ ВТНачисленияПоДокументамДляБухучета
	|ИЗ
	|	ИмяИсходнойВТ КАК Таблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Премия КАК ДокументПремия
	|		ПО Таблица.ДокументОснование = ДокументПремия.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.БольничныйЛист КАК БольничныйЛист
	|		ПО Таблица.ДокументОснование = БольничныйЛист.Ссылка";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяИсходнойВТ", ИмяИсходнойВТ);
	Запрос.Выполнить();
