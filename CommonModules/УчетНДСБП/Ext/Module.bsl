...
Функция ПечатьСводныхСправок(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСводнымСправкам, ПараметрыПечати) Экспорт
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		УправлениеПечатьюБП.ДоработатьФаксимильнуюПодписьИПечать(ТабДокумент, ОбъектыПечати,
			ВыборкаСФ.Организация, ВыборкаСФ.СчетФактура);
...
#Область РегистрацияСчетовФактур

// Подготавливает структуру начальных параметров заполнения 
// при анализе данных для заполнения таблиц обработки
//
// Возвращаемое значение:
//  - Структура
Функция ПараметрыЗаполненияРегистрацииСчетовФактур() Экспорт
	...
	Результат.Вставить("ПериодичностьРегистрации",       Перечисления.ДоступныеПериодыОтчета.Квартал);
