...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьКонстантуИспользуетсяКассоваяКнига");
...
Процедура ПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	ВызватьИсключениеЕслиИспользуетсяРежимЗамещения(НаборЗаписей, Замещение);
	
	ДополнительныеСвойства = НаборЗаписей.ДополнительныеСвойства;
	
	ДополнительныеСвойства.Вставить("ВестиУчетПоПодразделениям",
		БухгалтерскийУчетПереопределяемый.ВестиУчетПоПодразделениям());
	ДополнительныеСвойства.Вставить("ГоловныеОрганизации",
		Новый Массив);
	...
	Если ИмяРегистра = "УчетнаяПолитика"
		...
		Или ИмяРегистра = "НастройкиУчетаСтраховыхВзносовИП"
		Или ИмяРегистра = "НастройкиУчетаУСН" Тогда
...
Процедура ПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	Если ДополнительныеСвойства.ВестиУчетПоПодразделениям
		...
		Для каждого ГоловнаяОрганизация Из ДополнительныеСвойства.ГоловныеОрганизации Цикл
			СписокОП = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьСписокОбособленныхПодразделенийОрганизации(
				ГоловнаяОрганизация);
			ОбособленныеПодразделенияОрганизаций.Вставить(ГоловнаяОрганизация, СписокОП);
		КонецЦикла;
		
		ТаблицаЗаписей = НаборЗаписей.Выгрузить();
		ЕстьОтборПоПериоду = НаборЗаписей.Отбор.Найти("Период") <> Неопределено
			И НаборЗаписей.Отбор.Период.Использование;
	...
	Если ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Если НаборЗаписей.ВыгрузитьКолонку("ПрименяетсяУСНДоходыМинусРасходы").Найти(Истина) <> Неопределено
				...
				ВключитьВестиУчетРасчетовСКонтрагентами();
			КонецЕсли;
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаНДС" Тогда
		...
		Если НаборЗаписей.ВыгрузитьКолонку("СтавкаНалогообложенияПриУСН").Найти(Перечисления.ВидыСтавокНДС.Общая) <> Неопределено Тогда
			ВключитьВестиУчетРасчетовСКонтрагентами();
		КонецЕсли;
		
		Если ВключенныеНастройки.Свойство("РаздельныйУчетНДСДо2014Года") Тогда
			КлассификаторыДоходовРасходов.ОбеспечитьФункциональность(Справочники.СтатьиЗатрат,
				"РаздельныйУчетНДСДо2014Года");
		КонецЕсли;
	...
	ИначеЕсли ИмяРегистра = "НастройкиУчетаСтраховыхВзносовИП" Тогда
		...
		Для Каждого Запись Из НаборЗаписей Цикл
			РегистрыСведений.НастройкиУчетаСтраховыхВзносовИП.Подключаемый_ПриЗаписи(Запись);
		КонецЦикла;
		
	ИначеЕсли ИмяРегистра = "НастройкиУчетаТуристическогоНалога" Тогда
		
		Для Каждого Запись Из НаборЗаписей Цикл
			РегистрыСведений.НастройкиУчетаТуристическогоНалога.Подключаемый_ПриЗаписи(Запись);
		КонецЦикла;
	ИначеЕсли ИмяРегистра = "НастройкиУчетаНалогаНаПрибыль" Тогда
		
		УстановитьИспользуетсяПередачаИнвестиционногоВычета();
		
	ИначеЕсли ИмяРегистра = "НастройкиУчетаУСН" Тогда
		Если НаборЗаписей.Количество() > 0 Тогда
			ТекущаяЗапись = НаборЗаписей[0];
			МоментВремениПервогоДокумента = РаботаСПоследовательностями.МоментВремениПервогоДокументаВПоследовательности(
			...
			Если МоментВремениПервогоДокумента <> Неопределено Тогда
				РаботаСПоследовательностями.СброситьСостояниеПоследовательностиДокумента(
					МоментВремениПервогоДокумента.Ссылка,
					ТекущаяЗапись.Период,
					ТекущаяЗапись.Организация);
			КонецЕсли;
		КонецЕсли;
	...
	ПериодичностьРегистраСведений = НаборЗаписей.Метаданные().ПериодичностьРегистраСведений;
	Если НаборЗаписей.Количество() > 0
		И ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
		ТекущаяЗапись = НаборЗаписей[0];
		Параметры = Новый Структура;
		Параметры.Вставить("Организация", ТекущаяЗапись.Организация);
		Параметры.Вставить("Период", ТекущаяЗапись.Период);
		Параметры.Вставить("СостояниеПроведения",
			Перечисления.СостоянияПроведенияВПоследовательности.ПроведенСНарушениемПоследовательности);
		РаботаСПоследовательностями.АктуализироватьРегламентныеОперации(Параметры);
	КонецЕсли;
...
Процедура УстановкаПараметровУчетнойПолитикиПоУмолчанию(ДанныеЗаполнения, ТребуетсяСоздатьПоставляемыеВидыНалогов = Ложь) Экспорт
	...
	Если ДанныеЗаполнения.Свойство("СистемаНалогообложения", СистемаНалогообложения) Тогда
		ДатаНачалаПримененияСистемыНалогообложения =
			Перечисления.СистемыНалогообложения.ДатаНачалаПрименения(СистемаНалогообложения);
	...
	НастроитьПрименениеНДСПриУСН = ДанныеЗаполнения.Свойство("НастроитьНДСПриУСН")
		И ДанныеЗаполнения.НастроитьНДСПриУСН
		И ДанныеЗаполнения.Свойство("НачалоПримененияНДСПриУСН")
		И ЗначениеЗаполнено(ДанныеЗаполнения.НачалоПримененияНДСПриУСН);
	
	Если НастроитьПрименениеНДСПриУСН Тогда
		ДанныеЗаполнения.Вставить("ПлательщикНДСПрименяющийУСН", Истина);
		ДанныеЗаполнения.Вставить("Период", ДанныеЗаполнения.НачалоПримененияНДСПриУСН);
		ДанныеЗаполнения.Вставить("СтавкаНалогообложенияПриУСН", Перечисления.ВидыСтавокНДС.ПониженнаяУСН);
		СоздатьНастройкиУчетнойПолитикиПлательщикаНДСПриУСН(ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ЗаписатьУчетнуюПолитикуОбособленногоПодразделения(Организация) Экспорт
	...
	СкопироватьЗаписиУчетнойПолитикиДляОбособленногоПодразделения("НастройкиВариантаРасчетаКорректировокВВалюте",
		Организация);
...
Процедура СоздатьЗаписьУчетнойПолитикиНаНовуюДату(Организация, ДатаНачалаУчета, ДатаНастройки) Экспорт
	...
	СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("УчетнаяПолитика",
		Организация, ДатаНачалаУчета, ДатаНастройки);
	СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиСистемыНалогообложения",
		Организация, ДатаНачалаУчета, ДатаНастройки);
	СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаУСН",
		Организация, ДатаНачалаУчета, ДатаНастройки);
	СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаНДС",
		Организация, ДатаНачалаУчета, ДатаНастройки);
	
	Если ЭтоЮрЛицо Тогда
		СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаНалогаНаПрибыль",
			Организация, ДатаНачалаУчета, ДатаНастройки);
	Иначе
		СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаНДФЛ",
			Организация, ДатаНачалаУчета, ДатаНастройки);
		СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаСтраховыхВзносовИП",
			Организация, ДатаНачалаУчета, ДатаНастройки);
		СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС",
			Организация, ДатаНачалаУчета, ДатаНастройки);
	КонецЕсли;
...
//   *ПлательщикНДС                           - Булево - признак плательщика НДС.
//   *ПлательщикТорговогоСбора                - Булево - признак плательщика торгового сбора.
//   *ПлательщикНДСПрименяющийУСН             - Булево - признак плательщика НДС применяющего УСН.
//
Функция ОписаниеСтруктурыУчетнойПолитики() Экспорт
	...
	ПараметрыУчетнойПолитики.Вставить("СистемаНалогообложения",
		Перечисления.СистемыНалогообложения.ПустаяСсылка());
	...
	ПараметрыУчетнойПолитики.Вставить("ПрименяетсяАУСН",                         Ложь);
	...
	ПараметрыУчетнойПолитики.Вставить("ПрименяетсяНалогНаПрофессиональныйДоход", Ложь);
	ПараметрыУчетнойПолитики.Вставить("ОбъектНалогообложенияУСН",
		Перечисления.ОбъектыНалогообложенияПоУСН.ПустаяСсылка());
	...
	ПараметрыУчетнойПолитики.Вставить("СпособОценкиМПЗ",
		Перечисления.СпособыОценки.ПустаяСсылка());
	ПараметрыУчетнойПолитики.Вставить("ПлательщикЕНП",                           Ложь);
	ПараметрыУчетнойПолитики.Вставить("ПлательщикНДСПрименяющийУСН",             Ложь);
...
Процедура ВключитьПартионныйУчет() Экспорт
	...
	ВключитьРасширеннуюАналитику(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("ВестиПартионныйУчет"));
...
//
// Параметры:
//  ТребуемыеРазрезыУчета - Массив из Строка - требуемая аналитика: имена из ОбщегоНазначенияБП.ОпределитьПараметрыУчета
//
Процедура ВключитьРасширеннуюАналитику(ТребуемыеРазрезыУчета) Экспорт
	
	ПараметрыУчета = Новый Структура;
	Для Каждого ИмяРазреза Из ТребуемыеРазрезыУчета Цикл
		ПараметрыУчета.Вставить(ИмяРазреза, Истина);
	КонецЦикла;
...
Функция НайтиДатуОкончанияДействия(Знач Период, Знач Организация, ИмяРегистра) Экспорт
	...
	НастройкаУчетнаяПолитикиВсехОрганизаций =
		ОбщегоНазначенияБПВызовСервера.ЗаполнениеНастройкиУчетнойПолитики(ИмяРегистра);
	...
	Иначе
		Если ДатаОкончания = Период Тогда
			Возврат КонецГода(ДобавитьМесяц(ТекущаяДата, 1));
		Иначе
...
Процедура ПослеОбновленияИспользуемыхСистемНалогообложения() Экспорт
	...
	УчетЗарплаты.НастроитьСписокНачисленийПоСистемеНалогообложения();
	УчетЗарплаты.НастроитьСпособыОтраженияДополнительныхНачислений();
...
Процедура ВключитьВестиУчетРасчетовСКонтрагентами()

	УстановитьПривилегированныйРежим(Истина);
	Если Не Константы.ВестиУчетРасчетовСКонтрагентами.Получить() Тогда
		Константы.ВестиУчетРасчетовСКонтрагентами.Установить(Истина);
	КонецЕсли;

КонецПроцедуры

Процедура УстановитьИспользуетсяПередачаИнвестиционногоВычета()
	
	// Значение константы ИспользуетсяПередачаИнвестиционногоВычета синхронизировано с ресурсом ИспользуетсяПередачаИнвестиционногоВычета
	// регистра сведений НастройкиУчетаНалогаНаПрибыль.
	// При этом штатное редактирование настройки налога на прибыль не должно приводить к тому, что проведенные документы,
	// входящие в состав ФО ИспользуетсяПередачаИнвестиционногоВычета, окажутся недоступны пользователю.
	// Ручное изменение значения константы ИспользуетсяПередачаИнвестиционногоВычета не контролируется.
	
	УстановитьПривилегированныйРежим(Истина);
	
	ТекущееЗначение = Константы.ИспользуетсяПередачаИнвестиционногоВычета.Получить();
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ИспользуетсяПередачаИнвестиционногоВычета
	|ИЗ
	|	РегистрСведений.НастройкиУчетаНалогаНаПрибыль КАК НастройкиУчетаНалогаНаПрибыль
	|ГДЕ
	|	НастройкиУчетаНалогаНаПрибыль.ИспользуетсяПередачаИнвестиционногоВычета";
	
	НовоеЗначение = Не Запрос.Выполнить().Пустой();
	
	Если ТекущееЗначение = НовоеЗначение Тогда
		// Значение константы уже имеет нужное значение, ничего делать не нужно.
		Возврат;
	КонецЕсли;
	
	Если НовоеЗначение = Истина Тогда
		// Контроль не требуется, устанавливаем новое значение.
		Константы.ИспользуетсяПередачаИнвестиционногоВычета.Установить(Истина);
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК ЕстьПроведенныеДокументы
	|ИЗ
	|	Документ.ОперацияСИнвестиционнымВычетом КАК ОперацияСИнвестиционнымВычетом
	|ГДЕ
	|	ОперацияСИнвестиционнымВычетом.Проведен";
	
	ЕстьДокументыПередачиИнвестиционногоВычета = Не Запрос.Выполнить().Пустой();
	
	Если ЕстьДокументыПередачиИнвестиционногоВычета Тогда
		// Отключение константы приведет к недоступности проведенных документов из-за их вхождения
		// в состав функциональной опции ИспользуетсяПередачаИнвестиционногоВычета.
		Возврат;
	КонецЕсли;
	
	Константы.ИспользуетсяПередачаИнвестиционногоВычета.Установить(НовоеЗначение);
...
Процедура СинхронизацияНастроекУчета(ТекущаяЗапись, ИмяРегистра)
	...
	ЭтоЮрЛицо           = ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация);
	ЭтоИП               = ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоИП(Организация);
	...
	Если ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		...
		Если КлючЗаписиНастройки = Неопределено Тогда
			...
			НастройкиУчетнойПолитики =
				РегистрыСведений.УчетнаяПолитика.СрезПервых(Период, Новый Структура("Организация", Организация));
			...
			Иначе
				СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("УчетнаяПолитика",
					Организация, НастройкиУчетнойПолитики[0].Период, Период);
			КонецЕсли;
			
		Иначе
			
			КлючЗаписиПредыдущий = КлючЗаписиДействующейУчетнойПолитики("НастройкиСистемыНалогообложения",
				Организация, Период-1);
			КлючЗаписиТекущий    = КлючЗаписиДействующейУчетнойПолитики("НастройкиСистемыНалогообложения",
				Организация, Период);
			Если КлючЗаписиПредыдущий = Неопределено Или КлючЗаписиТекущий = Неопределено Тогда
				Если КлючЗаписиНастройки.Период <> Период Тогда
					СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("УчетнаяПолитика", Организация,
						КлючЗаписиНастройки.Период, Период);
				КонецЕсли;
			Иначе
				Если КлючЗаписиПредыдущий.Период <> КлючЗаписиТекущий.Период Тогда
					Отбор = Новый Структура("Организация",Организация);
					
					НастройкаПредыдущая = РегистрыСведений.НастройкиСистемыНалогообложения.Получить(
						КлючЗаписиПредыдущий.Период, Отбор);
					НастройкаТекущая    = РегистрыСведений.НастройкиСистемыНалогообложения.Получить(
						КлючЗаписиТекущий.Период, Отбор);
					...
					Если СистемаНалогообложенияПредыдущая <> СистемаНалогообложенияТекущая 
						...
						СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("УчетнаяПолитика",
							Организация, КлючЗаписиНастройки.Период, Период);
						КлючЗаписиНастройки = КлючЗаписиДействующейУчетнойПолитики("УчетнаяПолитика",
							Организация, Период);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	НастройкиНалога = РегистрыСведений.НастройкиСистемыНалогообложения.СрезПоследних(
	...
	Если ИмяРегистра = "УчетнаяПолитика" Тогда
		...
		Если НастройкиНалога.Количество() = 0 Тогда
			НастройкиНалога = РегистрыСведений.НастройкиСистемыНалогообложения.СрезПервых(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
			...
			Иначе
				СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиСистемыНалогообложения",
					Организация, НастройкиНалога[0].Период, Период);
			КонецЕсли;
			НастройкиНалога = РегистрыСведений.НастройкиСистемыНалогообложения.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
		КонецЕсли;
	...
	Если ЭтоЮрЛицо Тогда
		Если НастройкиСистемыНалогообложения.ПлательщикНалогаНаПрибыль Тогда
			НастройкиНалога = РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0
				Или НастройкиСистемыНалогообложения.ПлательщикЕНВД <>
					УчетнаяПолитика.ПлательщикЕНВД(НастройкиНалога[0].Организация, НастройкиНалога[0].Период) Тогда
				НастройкиНалога = РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.СрезПервых(
					Период, Новый Структура("Организация", Организация));
				Если НастройкиНалога.Количество() = 0 Тогда
			...
			НастройкиНалога = РегистрыСведений.СтавкиНалогаНаИмущество.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
				НастройкиНалога = РегистрыСведений.СтавкиНалогаНаИмущество.СрезПервых(
					Период, Новый Структура("Организация", Организация));
				Если НастройкиНалога.Количество() = 0 Тогда
					...
					Запись.Записать(Истина);
				Иначе
					СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("СтавкиНалогаНаИмущество",
						Организация, НастройкиНалога[0].Период, Период);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	Иначе
		НастройкиНалога = РегистрыСведений.НастройкиУчетаСтраховыхВзносовИП.СрезПоследних(
			Период, Новый Структура("Организация", Организация));
		Если НастройкиНалога.Количество() = 0 Тогда
			НастройкиНалога = РегистрыСведений.НастройкиУчетаСтраховыхВзносовИП.СрезПервых(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
		...
		Если НастройкиСистемыНалогообложения.ПлательщикНДФЛ Тогда
			...
			ВестиУчетПоВидамДеятельности =
				НастройкиСистемыНалогообложения.ПлательщикЕНВД Или НастройкиСистемыНалогообложения.ПрименяетсяУСНПатент;
			НастройкиНалога              = РегистрыСведений.НастройкиУчетаНДФЛ.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
				НастройкиНалога = РегистрыСведений.НастройкиУчетаНДФЛ.СрезПервых(
					Период, Новый Структура("Организация", Организация));
				Если НастройкиНалога.Количество() = 0 Тогда
			...
			Если СкопироватьНаНовуюДату Тогда
				СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("НастройкиУчетаНДФЛ",
					Организация, НастройкиНалога[0].Период, Период,
						Новый Структура("ВестиУчетПоВидамДеятельности",ВестиУчетПоВидамДеятельности));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиСистемыНалогообложения.ПлательщикНалогаНаПрибыль
		...
		НастройкиНалога = РегистрыСведений.НастройкиУчетаНДС.СрезПоследних(
			Период, Новый Структура("Организация", Организация));
		Если НастройкиНалога.Количество() = 0 Тогда
			НастройкиНалога = РегистрыСведений.НастройкиУчетаНДС.СрезПервых(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
	...
	ИначеЕсли НастройкиСистемыНалогообложения.ПрименяетсяУСН Тогда
		
		Если НастройкиСистемыНалогообложения.ПлательщикНДСПрименяющийУСН Тогда
			НастройкиНалога = РегистрыСведений.НастройкиУчетаНДС.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
				Запись = РегистрыСведений.НастройкиУчетаНДС.СоздатьМенеджерЗаписи();
		...
		НастройкиНалога = РегистрыСведений.НастройкиУчетаУСН.СрезПоследних(
			Период, Новый Структура("Организация", Организация));
		Запись = РегистрыСведений.НастройкиУчетаУСН.СоздатьМенеджерЗаписи();
		
		ДанныеЗаполнения.Вставить("ПрименяетсяУСНДоходыМинусРасходы",
		...
		ПредыдущийПериод = НачалоГода(НачалоГода(Период) - 1);
		ОтборНастроек =  Новый Структура("Организация", Организация);
		НастройкиНалогаПредыдущийПериод = РегистрыСведений.НастройкиСистемыНалогообложения.СрезПоследних(
			ПредыдущийПериод, ОтборНастроек);
		Если ЗначениеЗаполнено(НастройкиНалогаПредыдущийПериод) Тогда
			НастройкиНалогаПредыдущийПериод = НастройкиНалогаПредыдущийПериод[0];
			НапоминатьОВведенииОстатков = ((НастройкиСистемыНалогообложения.ПрименяетсяУСН
				И НастройкиНалогаПредыдущийПериод.ПрименяетсяУСН)
				И (НастройкиСистемыНалогообложения.ПрименяетсяУСНДоходыМинусРасходы =
					НастройкиНалогаПредыдущийПериод.ПрименяетсяУСНДоходы));
			Запись.НапоминатьОВведенииОстатков = НапоминатьОВведенииОстатков;
			Запись.Записать(Истина);
		КонецЕсли;
		
		Если НастройкиНалога.Количество() = 0 Тогда
			НастройкиНалога = РегистрыСведений.НастройкиУчетаУСН.СрезПервых(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0 Тогда
				Запись.Записать(Истина);
			Иначе
	...
	Если ЭтоЮрЛицо Или ЭтоИП Тогда
		Если Константы.УчетЗарплатыИКадровСредствамиБухгалтерии.Получить() Тогда
			
			ДанныеЗаполнения = Новый Структура("Период", Период);
			
			НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПоследних(
				Период, Новый Структура("Организация", Организация));
			Если НастройкиНалога.Количество() = 0
				Или НастройкиСистемыНалогообложения.ПрименяетсяАУСН Тогда
				НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПервых(
					Период, Новый Структура("Организация", Организация));
				Если НастройкиНалога.Количество() = 0
					Или НастройкиСистемыНалогообложения.ПрименяетсяАУСН Тогда
					ДанныеЗаполненияСтраховыеВзносы =
						РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.ЗаписьПоУмолчанию();
					Если НастройкиСистемыНалогообложения.ПрименяетсяАУСН Тогда
						ДанныеЗаполненияСтраховыеВзносы.Период = Период;
						ДанныеЗаполненияСтраховыеВзносы.Ставка = 0;
					КонецЕсли;
					Запись = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СоздатьМенеджерЗаписи();
					Запись.Организация = Организация;
					УчетЗарплаты.УстановкаНастроекПоУмолчанию("СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев",
						Запись, ДанныеЗаполненияСтраховыеВзносы);
					Запись.Записать(Истина);
				Иначе
					СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев",
						Организация, НастройкиНалога[0].Период, Период);
				КонецЕсли;
			КонецЕсли;
			
			НастройкиНалога = РегистрыСведений.ПрименяемыеТарифыСтраховыхВзносов.СрезПоследних(
				Период, Новый Структура("ГоловнаяОрганизация", ГоловнаяОрганизация));
			Если НастройкиНалога.Количество() = 0
				Или Не УчетЗарплаты.ТарифВзносовСоответствуетУчетнойПолитике(ГоловнаяОрганизация, Период) Тогда
			...
			Если НастройкиСистемыНалогообложения.ПрименяетсяАУСН Тогда
				УчетЗарплаты.ОтключитьНастройкиУчетаСтраховыхВзносов(Организация);
...
Процедура СоздатьНастройкиУчетнойПолитикиПлательщикаНДСПриУСН(ДанныеЗаполнения)
	...
	ЗаписатьНастройку(НастройкиСистемыНалогообложения);
	ЗаписатьНастройку(НастройкиУчетаНДС);
...
Процедура СоздатьНастройкиУчетнойПолитикиПоУмолчанию(ДанныеЗаполнения)
	...
	НастройкиВариантаРасчетаКорректировокВВалюте =
		РегистрыСведений.НастройкиВариантаРасчетаКорректировокВВалюте.СоздатьНаборЗаписей();
	НастройкиВариантаРасчетаКорректировокВВалюте.Отбор.Организация.Установить(ДанныеЗаполнения.Организация);
	НастройкиВариантаРасчетаКорректировокВВалюте.Прочитать();
	...
	НастройкиПоУмолчаниюВариантРасчетаКорректировокВВалюте(НастройкиВариантаРасчетаКорректировокВВалюте, ДанныеЗаполнения);
	...
	ЗаписатьНастройку(НастройкиУчетаУСН);
	ЗаписатьНастройку(НастройкиУчетаНДС);
	ЗаписатьНастройку(НастройкиУчетаНДФЛ);
	ЗаписатьНастройку(НастройкиБухгалтерскогоУчета);
	ЗаписатьНастройку(НастройкиУчетаНалогаНаПрибыль);
	ЗаписатьНастройку(НастройкиСистемыНалогообложения);
	ЗаписатьНастройку(НастройкиУчетаСтраховыхВзносовИП);
	ЗаписатьНастройку(НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС);
	ЗаписатьНастройку(НастройкиВариантаРасчетаКорректировокВВалюте);
...
Процедура НастройкиПоУмолчаниюУчетНалогаНаПрибыль(НастройкиУчетаНалогаНаПрибыль, ДанныеЗаполнения, СтруктураУчетнойПолитики)
	...
	// На 2017-2030 годы вводятся "нестандартные" ставки налога на прибыль - см. РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.ПрименяютсяВременныеСтавкиНалога
	// На протяжении этого периода ставки могут меняться, поэтому необходимо добавлять сразу несколько настроек.
	
	Период = '2025-01-01';
	// В 2025 году изменяются ставки налога на прибыль.
	// Если настройки записываются до 01.01.2025, то дополнительно запишем настройки с новыми ставками на 2025 год.
	Если ДанныеЗаполнения.Период < Период Тогда
		Если НастройкиУчетаНалогаНаПрибыль.ВыгрузитьКолонку("Период").Найти(Период) = Неопределено Тогда
			Запись = НастройкиУчетаНалогаНаПрибыль.Добавить();
			Запись.Период = Период;
			Запись.Организация = ДанныеЗаполнения.Организация;
		КонецЕсли;
...
Процедура НастройкиПоУмолчаниюУчетНДС(НастройкиУчетаНДС, ДанныеЗаполнения, СтруктураУчетнойПолитики)
	...
	Если Не СтруктураУчетнойПолитики.ПлательщикНДС
		И Не СтруктураУчетнойПолитики.ПлательщикНДСПрименяющийУСН Тогда
		Возврат;
	КонецЕсли;
...
Процедура СоздатьПрименяемыеТарифыСтраховыхВзносов(НаборЗаписей)
	...
	ВидТарифаОбщийНалоговыйРежим      =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим");
	ВидТарифаУпрощенныйНалоговыйРежим =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыТарифовСтраховыхВзносов.УпрощенныйНалоговыйРежим");
	ВидТарифаПатент                   =
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыТарифовСтраховыхВзносов.ИндивидуальныйПредпринимательПрименяющийПатент");
	ВидТарифаАУСН                     = 
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыТарифовСтраховыхВзносов.АУСН");
	
	Для каждого Строка Из НаборЗаписей Цикл
		
		ВидТарифа = Справочники.ВидыТарифовСтраховыхВзносов.ПустаяСсылка();
		...
		Если УчетЗарплаты.ТарифВзносовСоответствуетУчетнойПолитике(Строка.Организация, Строка.Период) Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли Строка.СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН Тогда
			
			Если Не ЗначениеЗаполнено(ВидТарифаАУСН) Тогда
				Продолжить;
			КонецЕсли;
		 
			ВидТарифа = ВидТарифаАУСН;
		...
		ЗаписатьНастройку(НаборЗаписейСтраховыхВзносов);
...
Процедура СоздатьНастройкиСтрахованиеОтНесчастныхСлучаев(НаборЗаписей)
	...
	Для каждого Строка Из НаборЗаписей Цикл
		...
		СформироватьЗаписьПоУмолчанию = Ложь;
		
		ПериодНастройки = Строка.Период;
		
		НаборЗаписейСтраховыхВзносов = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СоздатьНаборЗаписей();
		НаборЗаписейСтраховыхВзносов.Отбор.Период.Установить(ПериодНастройки);
		НаборЗаписейСтраховыхВзносов.Отбор.Организация.Установить(Строка.Организация);
		НаборЗаписейСтраховыхВзносов.Прочитать();
		НаборЗаписейСтраховыхВзносов.Очистить();
		
		ДанныеЗаполненияСтраховыеВзносы = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.ЗаписьПоУмолчанию();
		ДанныеЗаполненияСтраховыеВзносы.Период = ПериодНастройки;
		ДанныеЗаполненияСтраховыеВзносы.Ставка = ?(Строка.ПрименяетсяАУСН,
			УчетЗарплаты.НулеваяСтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев(),
			ДанныеЗаполненияСтраховыеВзносы.Ставка);
		
		ЭтоЮрЛицо = ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Строка.Организация);
		ЭтоИП = ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоИП(Строка.Организация);
		Если ЭтоЮрЛицо Или ЭтоИП Тогда
			Если Константы.УчетЗарплатыИКадровСредствамиБухгалтерии.Получить() Тогда
				ДанныеЗаполнения = Новый Структура("Период", Строка.Период);
				НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПоследних(
					ПериодНастройки, Новый Структура("Организация", Строка.Организация));
				Если НастройкиНалога.Количество() = 0 Тогда
					НастройкиНалога = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПервых(
						ПериодНастройки, Новый Структура("Организация", Строка.Организация));
					Если НастройкиНалога.Количество() = 0 Тогда
						СформироватьЗаписьПоУмолчанию = Истина;
					Иначе
						СкопироватьПараметрыУчетнойПолитикиНаНовуюДату("СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев",
							Строка.Организация, НастройкиНалога[0].Период, ПериодНастройки);
					КонецЕсли;
				КонецЕсли;
				
				Если Строка.ПрименяетсяАУСН Тогда
					// Переопределим ставку, в случае если настройки уже есть.
					СформироватьЗаписьПоУмолчанию = Истина;
				КонецЕсли;
		...
		Если СформироватьЗаписьПоУмолчанию Тогда
			Запись = НаборЗаписейСтраховыхВзносов.Добавить();
...
Процедура НастройкиПоУмолчаниюВариантРасчетаКорректировокВВалюте(НастройкиВариантаРасчетаКорректировокВВалюте, ДанныеЗаполнения)
	НастройкиВариантаРасчетаКорректировокВВалюте.Очистить();
	Запись = НастройкиВариантаРасчетаКорректировокВВалюте.Добавить();
	Запись.Период      = НачалоГода(ТекущаяДатаСеанса());
	Запись.Организация = ДанныеЗаполнения.Организация;
	РегистрыСведений.НастройкиВариантаРасчетаКорректировокВВалюте.УстановкаНастроекПоУмолчанию(Запись, ДанныеЗаполнения);

КонецПроцедуры

Процедура ЗаписатьНастройку(Объект, ОтключитьБизнесЛогикуПриЗаписи = Истина)
	
	Объект.ОбменДанными.Загрузка = ОтключитьБизнесЛогикуПриЗаписи;
	Объект.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
	Объект.Записать();
	
КонецПроцедуры

Процедура ВызватьИсключениеЕслиИспользуетсяРежимЗамещения(НаборЗаписей, Замещение)
	...
	Если ТипЗнч(Замещение) <> Тип("Булево") И ОтборПустой
		Или Замещение = ОбщегоНазначения.РежимУдаленияНабораЗаписей() Тогда
		
		СинонимРегистра = НаборЗаписей.Метаданные().Синоним;
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Регистр сведений %1 не поддерживает запись с использованием режимов замещения.'",
			ОбщегоНазначения.КодОсновногоЯзыка()), СинонимРегистра);
...
Процедура СоздатьНастройкиУчетаПоДаннымУчетнойПолитики() Экспорт
	...
	ДобавитьНастройкуБухгалтерскогоУчетаДляПБУ18(РСНастройкиБухгалтерскогоУчета);
...
Функция ТекстЗапросаУчетнойПолитики()
	...
	|	УдалитьУчетнаяПолитикаОрганизаций.СистемаНалогообложения КАК СистемаНалогообложения,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПлательщикНалогаНаПрибыль КАК ПлательщикНалогаНаПрибыль,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПлательщикНДС КАК ПлательщикНДС,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПлательщикНДФЛ КАК ПлательщикНДФЛ,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПлательщикЕНВД КАК ПлательщикЕНВД,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяУСН КАК ПрименяетсяУСН,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяУСНДоходы КАК ПрименяетсяУСНДоходы,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяУСНДоходыМинусРасходы КАК ПрименяетсяУСНДоходыМинусРасходы,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяУСНПатент КАК ПрименяетсяУСНПатент,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяОсобыйПорядокНалогообложения КАК ПрименяетсяОсобыйПорядокНалогообложения,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПоддержкаПБУ18 КАК ПоддержкаПБУ18,
	...
	|	УдалитьУчетнаяПолитикаОрганизаций.ОсновнойВидДеятельности КАК ОсновнойВидДеятельности,
	|	УдалитьУчетнаяПолитикаОрганизаций.ВестиУчетПоВидамДеятельностиИП КАК ВестиУчетПоВидамДеятельности,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПрименяетсяОсвобождениеОтУплатыНДС КАК ПрименяетсяОсвобождениеОтУплатыНДС,
	|	УдалитьУчетнаяПолитикаОрганизаций.СложныйУчетНДС КАК СложныйУчетНДС,
	|	УдалитьУчетнаяПолитикаОрганизаций.РаздельныйУчетНДСДо2014Года КАК РаздельныйУчетНДСДо2014Года,
	|	УдалитьУчетнаяПолитикаОрганизаций.РаздельныйУчетНДСНаСчете19 КАК РаздельныйУчетНДСНаСчете19,
	|	УдалитьУчетнаяПолитикаОрганизаций.УпрощенныйУчетНДС КАК УпрощенныйУчетНДС,
	|	УдалитьУчетнаяПолитикаОрганизаций.НачислятьНДСПоОтгрузке КАК НачислятьНДСПоОтгрузке,
	|	УдалитьУчетнаяПолитикаОрганизаций.НачислятьНДСПриПередачеНедвижимости КАК НачислятьНДСПриПередачеНедвижимости,
	|	УдалитьУчетнаяПолитикаОрганизаций.ФормироватьСчетФактуруПоУЕвРублях КАК ФормироватьСчетФактуруПоУЕвРублях,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокРегистрацииСчетовФактурНаАванс КАК ПорядокРегистрацииСчетовФактурНаАванс,
	|	УдалитьУчетнаяПолитикаОрганизаций.ДатаПереходаНаУСН КАК ДатаПереходаНаУСН,
	|	УдалитьУчетнаяПолитикаОрганизаций.УведомлениеНомер КАК УведомлениеНомер,
	|	УдалитьУчетнаяПолитикаОрганизаций.УведомлениеДата КАК УведомлениеДата,
	|	УдалитьУчетнаяПолитикаОрганизаций.НалоговыеКаникулыУСН КАК НалоговыеКаникулы,
	|	УдалитьУчетнаяПолитикаОрганизаций.ОбъектНалогообложенияУСН КАК ОбъектНалогообложенияУСН,
	...
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокПризнанияМатериальныхРасходов КАК ПорядокПризнанияМатериальныхРасходов,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокПризнанияРасходовПоТоварам КАК ПорядокПризнанияРасходовПоТоварам,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокПризнанияРасходовПоНДС КАК ПорядокПризнанияРасходовПоНДС,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокПризнанияДопРасходов КАК ПорядокПризнанияДопРасходов,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокПризнанияТаможенныхПлатежей КАК ПорядокПризнанияТаможенныхПлатежей,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПатентУСН КАК Патент,
	|	УдалитьУчетнаяПолитикаОрганизаций.РозничнаяТорговляОблагаетсяЕНВД КАК РозничнаяТорговляОблагаетсяЕНВД,
	|	УдалитьУчетнаяПолитикаОрганизаций.БазаРаспределенияКосвенныхРасходовПоВидамДеятельности КАК БазаРаспределенияКосвенныхРасходовПоВидамДеятельности,
	...
	|	УдалитьУчетнаяПолитикаОрганизаций.УплачиватьДобровольныеВзносыВФСС КАК УплачиватьДобровольныеВзносыВФСС,
	|	УдалитьУчетнаяПолитикаОрганизаций.СпособОценкиМПЗ КАК СпособОценкиМПЗ,
	|	УдалитьУчетнаяПолитикаОрганизаций.СпособОценкиТоваровВРознице КАК СпособОценкиТоваровВРознице,
	|	УдалитьУчетнаяПолитикаОрганизаций.ОсновнойСчетУчетаЗатрат КАК УдалитьОсновнойСчетУчетаЗатрат,
	|	УдалитьУчетнаяПолитикаОрганизаций.ВыпускПродукции КАК УдалитьВыпускПродукции,
	|	УдалитьУчетнаяПолитикаОрганизаций.ОказаниеУслуг КАК УдалитьОказаниеУслуг,
	|	УдалитьУчетнаяПолитикаОрганизаций.ДиректКостинг КАК УдалитьДиректКостинг,
	|	УдалитьУчетнаяПолитикаОрганизаций.ПорядокСписанияРасходовНаСебестоимостьУслуг КАК ПорядокСписанияРасходовНаСебестоимостьУслуг,
	|	УдалитьУчетнаяПолитикаОрганизаций.РассчитыватьСебестоимостьПолуфабрикатов КАК УдалитьРассчитыватьСебестоимостьПолуфабрикатов,
	|	УдалитьУчетнаяПолитикаОрганизаций.СпособРасчетаСебестоимостиПроизводства КАК СпособРасчетаСебестоимостиПроизводства,
	|	УдалитьУчетнаяПолитикаОрганизаций.РассчитыватьСебестоимостьУслугСобственнымПодразделениям КАК УдалитьРассчитыватьСебестоимостьУслугСобственнымПодразделениям,
	|	УдалитьУчетнаяПолитикаОрганизаций.СпособУчетаВыпускаГотовойПродукции КАК УдалитьСпособУчетаВыпускаГотовойПродукции,
	|	УдалитьУчетнаяПолитикаОрганизаций.УчитыватьРасходыНУАналогичноБУ КАК УчитыватьРасходыНУАналогичноБУ,
	|	УдалитьУчетнаяПолитикаОрганизаций.УчитыватьРасходыПоЭлементамЗатрат КАК УчитыватьРасходыПоЭлементамЗатрат,
	|	УдалитьУчетнаяПолитикаОрганизаций.УчитыватьРасходыПоСтатьямЗатрат КАК УчитыватьРасходыПоСтатьямЗатрат,
	|	УдалитьУчетнаяПолитикаОрганизаций.ФормироватьРезервыПоСомнительнымДолгамБУ КАК ФормироватьРезервыПоСомнительнымДолгамБУ,
	|	УдалитьУчетнаяПолитикаОрганизаций.ФормироватьРезервыПоСомнительнымДолгамНУ КАК ФормироватьРезервыПоСомнительнымДолгам,
	|	УдалитьУчетнаяПолитикаОрганизаций.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств КАК ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств
	|ИЗ
...
Функция ВидОрганизации(Организация)
	...
	СвойстваОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Организация, "ЮридическоеФизическоеЛицо, ГоловнаяОрганизация");
...
Функция НастройкиУчетаНалогаНаПрибыль()
	...
	ТаблицаНастройки.Колонки.Добавить("Период",
		Новый ОписаниеТипов("Дата",,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	ТаблицаНастройки.Колонки.Добавить("Организация",
		Новый ОписаниеТипов("СправочникСсылка.Организации"));
	ТаблицаНастройки.Колонки.Добавить("ПорядокУплатыАвансов",
		Новый ОписаниеТипов("ПеречислениеСсылка.ПорядокУплатыАвансовПоНалогуНаПрибыль"));
	ТаблицаНастройки.Колонки.Добавить("МетодНачисленияАмортизации",
		Новый ОписаниеТипов("ПеречислениеСсылка.МетодыНачисленияАмортизации"));
	ТаблицаНастройки.Колонки.Добавить("СпособПогашенияСтоимостиСпецодежды",
		Новый ОписаниеТипов("ПеречислениеСсылка.СпособыПогашенияСтоимостиНУ"));
	ТаблицаНастройки.Колонки.Добавить("БазаРаспределенияКосвенныхРасходовПоВидамДеятельности",
		Новый ОписаниеТипов("ПеречислениеСсылка.БазыРаспределенияКосвенныхРасходовПоВидамДеятельности"));
	ТаблицаНастройки.Колонки.Добавить("ФормироватьРезервыПоСомнительнымДолгам",
		Новый ОписаниеТипов("Булево"));
	ТаблицаНастройки.Колонки.Добавить("СтавкаФБ",
		Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаНастройки.Колонки.Добавить("СтавкаСубъектРФ",
		Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)));
	ТаблицаНастройки.Колонки.Добавить("ПрименяютсяРазныеСтавкиНалогаНаПрибыль",
		Новый ОписаниеТипов("Булево"));
...
Функция СтавкиНалогаНаПрибыльВСубъектахРФ()
	...
	Таблица.Колонки.Добавить("ПериодСтавки",
		Новый ОписаниеТипов(,,,,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	Таблица.Колонки.Добавить("СтавкаСубъектРФ",
		Новый ОписаниеТипов(,,, Новый КвалификаторыЧисла(15, 2, ДопустимыйЗнак.Неотрицательный)));
...
Функция ВариантыБухгалтерскойОтчетности()
	...
	Условия.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьОбщаяФорма());
	Условия.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьМалыеПредприятия());
	Условия.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьНекоммерческиеОрганизации());
	Условия.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьСоциальноОриентированные());
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Условия", Условия);
	Запрос.УстановитьПараметр("БухгалтерскаяОтчетностьМалыеПредприятия", 
		УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьМалыеПредприятия());
	Запрос.УстановитьПараметр("БухгалтерскаяОтчетностьНекоммерческиеОрганизации", 
		УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьНекоммерческиеОрганизации());
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВариантыПримененияТребованийЗаконодательства.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ВариантыПримененияТребованийЗаконодательства.Условие = &БухгалтерскаяОтчетностьМалыеПредприятия
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВариантыБухгалтерскойОтчетности.ДляМалыхПредприятий)
	|		КОГДА ВариантыПримененияТребованийЗаконодательства.Условие = &БухгалтерскаяОтчетностьНекоммерческиеОрганизации
...
Функция ПорядокУплатыАвансов()
	...
	УсловиеНалогНаПрибыльАвансыЕжеквартально =
		УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыЕжеквартально();
	УсловиеНалогНаПрибыльАвансыЕжемесячно =
		УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыЕжемесячно();
	УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли =
		УсловияПримененияТребованийЗаконодательства.УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли();
	Условия = Новый Массив;
	Условия.Добавить(УсловиеНалогНаПрибыльАвансыЕжеквартально);
	Условия.Добавить(УсловиеНалогНаПрибыльАвансыЕжемесячно);
	Условия.Добавить(УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Условия", Условия);
	Запрос.УстановитьПараметр("НалогНаПрибыльАвансыПоФактическойПрибыли",
		УсловиеНалогНаПрибыльАвансыПоФактическойПрибыли);
	Запрос.УстановитьПараметр("НалогНаПрибыльАвансыЕжемесячно",
		УсловиеНалогНаПрибыльАвансыЕжемесячно);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВариантыПримененияТребованийЗаконодательства.Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА ВариантыПримененияТребованийЗаконодательства.Условие = &НалогНаПрибыльАвансыПоФактическойПрибыли
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПорядокУплатыАвансовПоНалогуНаПрибыль.ПоФактическойПрибыли)
	|		КОГДА ВариантыПримененияТребованийЗаконодательства.Условие = &НалогНаПрибыльАвансыЕжемесячно
...
Функция УчетнаяПолитикаПоНалогуНаПрибыльОрганизаций(НастройкиУчетаНалогаНаПрибыль)
	...
	Для каждого Организация Из Организации Цикл
		...
		ТаблицаПериодов.Колонки.Добавить("ПериодСтавки",
			Новый ОписаниеТипов(,,,,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
		ТаблицаПериодов.Колонки.Добавить("ПериодУчПолитики",
			Новый ОписаниеТипов(,,,,, Новый КвалификаторыДаты(ЧастиДаты.Дата)));
	...
	ОбъединитьУчетнуюПолитикуИСтавки(НалогНаПрибыльОрганизаций,
		СтавкиНалогНаПрибыль,
		РегиональныеСтавки,
...
Процедура ЗаполнитьНастройкиУчетаНалогаНаПрибыль(НастройкиУчетаНалогаНаПрибыль, УчетнаяПолитика, СтруктураУчетнойПолитики)
	...
	МассивСтрок = НастройкиУчетаНалогаНаПрибыль.НайтиСтроки(
		Новый Структура("Период, Организация", Период, УчетнаяПолитика.Организация));
...
Процедура ЗаполнитьНастройкиУчетаУСН(РСНастройкиУчетаУСН, УчетнаяПолитика, СтруктураУчетнойПолитики)
	...
	МассивСтрок = РСНастройкиУчетаУСН.НайтиСтроки(
		Новый Структура("Период, Организация", Период, УчетнаяПолитика.Организация));
	Если МассивСтрок.Количество() = 0 Тогда
...
Процедура ЗаполнитьНастройкиУчетаНДФЛ(РСНастройкиУчетаНДФЛ, УчетнаяПолитика, СтруктураУчетнойПолитики)
	...
	Период = НачалоГода(УчетнаяПолитика.Период);
	МассивСтрок = РСНастройкиУчетаНДФЛ.НайтиСтроки(
		Новый Структура("Период, Организация", Период, УчетнаяПолитика.Организация));
	Если МассивСтрок.Количество() = 0 Тогда
...
Процедура ЗаполнитьНастройкиУчетаСтраховыхВзносовИП(РСНастройкиУчетаСтраховыхВзносовИП, УчетнаяПолитика, СтруктураУчетнойПолитики)
	...
	Период = НачалоГода(УчетнаяПолитика.Период);
	МассивСтрок = РСНастройкиУчетаСтраховыхВзносовИП.НайтиСтроки(
		Новый Структура("Период, Организация", Период, УчетнаяПолитика.Организация));
	Если МассивСтрок.Количество() = 0 Тогда
...
Процедура ЗаполнитьНастройкиБухгалтерскогоУчета(РСНастройкиБухгалтерскогоУчета, УчетнаяПолитика, ВариантыБухгалтерскойОтчетности)
	...
	Если УчетнаяПолитика.ПоддержкаПБУ18 И Запись.Период < НачалоСледующегоГода Тогда
		// Т.к. в БП 2.0 учет ведется только затратным методом, оставляем его для существующей политики.
		Запись.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.ЗатратныйМетод;
	ИначеЕсли УчетнаяПолитика.ПоддержкаПБУ18 И Запись.Период >= НачалоСледующегоГода Тогда
		// Может существовать настройка учетной политики "на будущее", для неё уже будет балансовый метод
		Запись.ВариантУчетаОтложенногоНалога =
			НалогНаПрибыльБухгалтерскийУчетКлиентСервер.ОсновнойМетодУчетаОтложенногоНалога(Запись.Период);
	Иначе
...
Процедура ДобавитьНастройкуБухгалтерскогоУчетаДляПБУ18(РСНастройкиБухгалтерскогоУчета)
	...
	Для Каждого НастройкаУчета Из РСНастройкиБухгалтерскогоУчета Цикл
		Если Организации.Найти(НастройкаУчета.Организация) = Неопределено Тогда
			Организации.Добавить(НастройкаУчета.Организация);
		КонецЕсли;
	КонецЦикла;
	
	// В БП 2.0 поддерживается ведение учета с применением ПБУ-18 только затратным методом.
	// При переходе на БП 3.0 применение балансового метода будет осуществляться только со следующего года.
	НачалоСледующегоГода = НачалоГода(ДобавитьМесяц(ТекущаяДатаСеанса(), 12));
	Для Каждого Организация Из Организации Цикл
		
		// Обработчик рассчитывает на упорядоченность записей набора по периоду.
		// Текст запроса формируется методом ТекстЗапросаУчетнойПолитики(), в нём результат выборки упорядочен по периоду.
		СоздаватьЗаписьНаСледующийГод = Истина;
		УчетнаяПолитикаНаКонецГода = Неопределено;
		Для Каждого НастройкаУчета Из РСНастройкиБухгалтерскогоУчета Цикл
			
			Если Организация <> НастройкаУчета.Организация Тогда
				Продолжить;
			КонецЕсли;
			
			Если НастройкаУчета.Период = НачалоСледующегоГода Тогда
				СоздаватьЗаписьНаСледующийГод = Ложь;
				Прервать;
			ИначеЕсли НастройкаУчета.Период > НачалоСледующегоГода Тогда
				Прервать;
			КонецЕсли;
			
			УчетнаяПолитикаНаКонецГода = НастройкаУчета;
		КонецЦикла;
		
		Если УчетнаяПолитикаНаКонецГода = Неопределено Или
			Не УчетнаяПолитикаНаКонецГода.ПоддержкаПБУ18 Или Не СоздаватьЗаписьНаСледующийГод Тогда
			
			Продолжить;
		КонецЕсли;
		
		НастройкаНаСледующийГод = РСНастройкиБухгалтерскогоУчета.Добавить();
		ЗаполнитьЗначенияСвойств(НастройкаНаСледующийГод, УчетнаяПолитикаНаКонецГода);
		НастройкаНаСледующийГод.Период = НачалоСледующегоГода;
		НастройкаНаСледующийГод.ВариантУчетаОтложенногоНалога =
			Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетодПостоянныеРазницы;
...
Процедура ОбъединитьУчетнуюПолитикуИСтавки(УчетныеПолитикиОрганизаций, СтавкиНалогНаПрибыль, РегиональныеСтавки, ПрименяютсяРазныеСтавкиНалогаНаПрибыль, ПорядокУплатыАвансовПоОрганизациям)
		...
		Для каждого ПериодСтавок Из СтавкиНалогНаПрибыль Цикл
			...
			Если ПрименяютсяРазныеСтавкиНалогаНаПрибыль Тогда
				...
				Если РегиональныеСтавкиОрганизации = Неопределено Тогда
					СтрокаТаблицы.СтавкаСубъектРФ =
						РегистрыСведений.НастройкиУчетаНалогаНаПрибыль.ЗначениеПоУмолчаниюРегиональныйБюджет(
	...
	Для каждого КлючИЗначение Из УчетныеПолитикиОрганизаций Цикл
		...
		ДанныеПоОрганизации.ЗаполнитьЗначения(ПрименяютсяРазныеСтавкиНалогаНаПрибыль,
			"ПрименяютсяРазныеСтавкиНалогаНаПрибыль");
		
		Если ПорядокУплатыАвансовПоОрганизациям[Организация] = Неопределено Тогда
			ДанныеПоОрганизации.ЗаполнитьЗначения(Перечисления.ПорядокУплатыАвансовПоНалогуНаПрибыль.Ежеквартально,
				"ПорядокУплатыАвансов");
		Иначе
			ДанныеПоОрганизации.ЗаполнитьЗначения(ПорядокУплатыАвансовПоОрганизациям[Организация],
				"ПорядокУплатыАвансов");
		КонецЕсли;
...
Функция ИспользуетсяКассоваяКнига()
	...
	ИспользуемыеСистемыНалогообложения =
