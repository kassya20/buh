...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗапуститьУниверсальныйОбменСБанкамиПоСервису", Истина);
	
	Методы.Вставить("СброситьВерсиюБанков");
	Методы.Вставить("СброситьВерсиюБанковИУслуг");
...
// Возвращает банк универсального обмена для сервиса.
//
// Параметры: 
//  ИНН - Строка - ИНН банка.
//  Сервис - ПеречислениеСсылка.СервисыОбменаСБанками - Тип сервиса обмена с банками.
//
// Возвращаемое значение:
//  СправочникСсылка.БанкиУниверсальногоОбмена.
//
Функция БанкУниверсальногоОбменаПоИНН(ИНН, Сервис) Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УслугиБанков.Банк КАК БанкУниверсальногоОбмена
		|ИЗ
		|	Справочник.УслугиБанков КАК УслугиБанков
		|ГДЕ
		|	УслугиБанков.Банк.ИНН = &ИНН
	...
	Если РезультатЗапроса.Пустой() Тогда
		Возврат Справочники.БанкиУниверсальногоОбмена.ПустаяСсылка();
	КонецЕсли;
	
	Выборка = РезультатЗапроса.Выбрать();
	Выборка.Следующий();
	Возврат Выборка.БанкУниверсальногоОбмена;
...
Процедура ЗаполнитьДеревоНовое(Сервис, ДеревоНовое, Автообмен = Ложь, ТолькоНепоказанные = Ложь, ВыводитьГруппуЕслиНетНовых = Ложь) Экспорт
	...
	Для каждого Группа Из ДанныеГрупп.Коллекция Цикл
		...
		// Поле Просмотрено.
		ОператорГруппы.ВыбираемыеПоля.Добавить("ИСТИНА", 4);
		// Поле Порядок.
		ОператорГруппы.ВыбираемыеПоля.Добавить(Строка(Группа.Порядок), 6);
		// Поле Группа.
		ОператорГруппы.ВыбираемыеПоля.Добавить("""" + Группа.Идентификатор + """", 9);
		// Поле Автообмен.
		ОператорГруппы.ВыбираемыеПоля.Добавить("ЛОЖЬ", 12);
	...
	Если НЕ ПустаяСтрока(ВыражениеПросмотрено) Тогда
		...
		ИндексПоля = 4; // Нумерация полей запроса начинается с нуля.
		Операторы[0].ВыбираемыеПоля[ИндексПоля] = Новый ВыражениеСхемыЗапроса(ВыражениеПросмотрено);
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ВыражениеПорядок) Тогда
		...
		ИндексПоля = 6; // Нумерация полей запроса начинается с нуля.
		Операторы[0].ВыбираемыеПоля[ИндексПоля] = Новый ВыражениеСхемыЗапроса(ВыражениеПорядок);
	КонецЕсли;
...
Процедура ПередЗаписьюДокументооборота(Объект, Отказ) Экспорт
	...
	Если Объект.Ссылка.Пустая() Тогда
		ПометкаУдаленияСсылки = Ложь;
	Иначе
		ПометкаУдаленияСсылки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "ПометкаУдаления");
	КонецЕсли;
	Объект.ИзмененаПометкаУдаления = (ПометкаУдаленияСсылки <> Объект.ПометкаУдаления);
...
Функция АвторизацияНаСервере(Сервис = Неопределено) Экспорт
	...
	ИмяСобытия = НСтр("ru = 'Ошибка авторизации на сервере обмена с банками'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ПолучитьИнформациюОБанкеССервера(Токен, ИдентификаторБанка) Экспорт
	...
	Если Банк <> Неопределено Тогда
		Версия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Банк, "Версия");
		ПараметрыЗапроса.Заголовки.Вставить("If-None-Match", Версия);
	КонецЕсли;
...
#Область ПолученияДанныхСервиса
	...
	Если НЕ ДанныеСервиса.Пустая() Тогда
		Версия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСервиса, "Версия");
		ПараметрыЗапроса.Заголовки.Вставить("If-None-Match", Версия);
	КонецЕсли;
...
Функция ОтправитьТранспортныйКонтейнерНаСервер(Параметры)
	...
	ДанныеТранзакции = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Транзакция, "Документооборот, ТипТранзакции, Идентификатор");
	Документооборот = ДанныеТранзакции.Документооборот;  
	ДанныеДокументооборота = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Документооборот, "Идентификатор, Сервис");
	ИдентификаторДокументооборота = ДанныеДокументооборота.Идентификатор;
	Сервис = ДанныеДокументооборота.Сервис;
	ТипТранзакции = ДанныеТранзакции.ТипТранзакции;
	ИдентификаторТранзакции = ДанныеТранзакции.Идентификатор;
...
Процедура ОбновитьДанныеСервиса(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'УниверсальныйОбменСБанками.ОбновитьДанныеСервиса'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
#Область ОтправкаСтатистикиНаСервер

// Выполняет отправку статистики на сервер БанкКоннект.
// Предназначена для вызова в качестве метода фонового задания.
//
// Параметры:
//	Сервис - ПеречислениеСсылка.СервисыОбменаСБанками - Сервис обмена.
//	Токен - Строка - Токен авторизации.
//	Активность - Строка - наименование активности .   
//	Контент - Строка - содержимое статистики.
//
Функция СохранитьЗаписьАктивностиСервиса(Сервис, Токен, Активность, Контент) Экспорт
	...
	Если Не ЗначениеЗаполнено(Токен) Тогда
		Токен = ТокенАвторизации(Сервис, Результат);
	КонецЕсли;
	
	РезультатВыполненияЗапроса = РезультатВыполненияЗапросаЗаписиАктивности(Токен, Активность, Контент);
	Если НЕ РезультатВыполненияЗапроса.Выполнено Тогда
		Результат.ОписаниеОшибки = РезультатВыполненияЗапроса.ОписаниеОшибки;  
		// Возможно просрочен токен, получим его
		// и выполним повторно запрос
		Токен = ТокенАвторизации(Сервис, Результат);
		РезультатВыполненияЗапроса = РезультатВыполненияЗапросаЗаписиАктивности(Токен, Активность, Контент);
		Если НЕ РезультатВыполненияЗапроса.Выполнено Тогда
			Возврат Результат;
	...
	Идентификатор = РезультатВыполненияЗапроса.Заголовки.Получить("user-activity-record");
...
Функция РезультатВыполненияЗапросаЗаписиАктивности(Токен, Активность, Контент)
	
	// Формируем параметры запроса.
	ПараметрыЗапроса = ПараметрыВыполнитьЗапросКСерверу(Токен, "user-activity-record");
			
	// Формируем тело запроса.
	ТелоЗапроса = Новый Структура;
	ТелоЗапроса.Вставить("activity", Активность);
	ТелоЗапроса.Вставить("activityContent", Контент);
	ПараметрыЗапроса.СтрокаТела = СтруктуруВСтрокуJSON(ТелоЗапроса);
	ПараметрыЗапроса.МетодЗапроса = "POST";
		
	// Запрос к серверу.
	Возврат ВыполнитьЗапросКСерверу(ПараметрыЗапроса);

КонецФункции

Функция ТокенАвторизации(Сервис, Результат)
	...
	Возврат РезультатАвторизации.Токен;
...
Функция СоздатьФайлОписанияТранзакции(Вложения, Контекст, Путь)
	...
	ДобавитьУзелXML(УзелТранспортнаяИнформация, "xmlns", Истина, "http://bankflow.1c.ru");
...
Функция ЗагрузитьИнформациюОБанкеИзСтруктуры(Данные)
	...
	Иначе 
		Версия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаБанк, "Версия");
		Если Версия >= Данные.version Тогда
...
Функция ЗагрузитьИнформациюОбУслугеБанкаИзСтруктуры(Данные)
	...
	Если УслугаБанка.Пустая() Тогда
		...
		УслугаБанка = Ссылка;
	Иначе
		Версия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УслугаБанка, "Версия");
		Если Версия >= Данные.version Тогда
