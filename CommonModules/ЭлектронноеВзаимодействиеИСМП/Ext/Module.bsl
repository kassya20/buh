...
Процедура ДобавитьСведенияОУчастникахОбмена_5_02(ДанныеШтрихкодовУпаковок, Источник, СтруктураДанных, ДеревоДанных, ДатаСведений, Отказ) Экспорт
	...
	Для Каждого ТипУчастника Из Участники Цикл
		...
		Если ТипУчастника.Ключ = "Продавец" И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя Тогда
			Продолжить;
		КонецЕсли;
		
		Если ТипУчастника.Ключ = "Покупатель" И Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Участник Из СписокУчастниковСтороны Цикл
			...
			Если Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДЮридическихЛиц
			...
			Если Не ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДИндивидуальныхПредпринимателей
...
Процедура ДополнитьРезультатКодамиОСУ(ТаблицаТоваров, ПараметрыСканирования, ЧастичноеСодержимоеРезультат, ИмяКолонкиКоличествоТаблицыТовары = "Количество") Экспорт
	...
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		...
		Если СтрокаТЧ.РазрешенОборотНемаркируемыхОстатков Тогда
			Продолжить;
		КонецЕсли;
		Если Не СтрокаТЧ.ДоступноУказаниеОСУКодов Тогда
			Продолжить;
		КонецЕсли;
...
// * ПроверятьМОДТолькоПоПодконтрольнымОбъектамВЕТИС              - Булево - Истина, если присутствуют только товарные группы, требующие при проверке МОД
// 		подконтрольности ВетИС
Функция КонструкторПараметрыУказанияМОДПоВидуПродукции() Экспорт
	...
	СтруктураВозврата.Вставить("ПроверятьМОДТолькоПоПодконтрольнымОбъектамВЕТИС",              Истина);
...
//  НеПроверятьДату - Булево - Истина, если требуется не контролировать даты наступления контроля перемещений между МОД
// 
// Возвращаемое значение:
...
Функция ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ВидыПродукции, НаДату = Неопределено, НеПроверятьДату = Ложь) Экспорт
	...
	Для Каждого ПроверяемыйВидПродукции Из МассивВидовПродукции Цикл
		...
		Если МассивПрослеживаемойПродукции.Найти(ПроверяемыйВидПродукции) <> Неопределено Тогда
			
			Если НеПроверятьДату Или ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПеремещенийМеждуМОД(ПроверяемыйВидПродукции, НаДату) Тогда
		...
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДЮридическихЛиц                 = МассивПродукцииТребующейУказаниеКодаФИАСПоМОДЮридическихЛиц.Найти(ПроверяемыйВидПродукции) <> Неопределено;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДИндивидуальныхПредпринимателей = МассивПродукцииТребующейУказаниеКодаФИАСПоМОДИндивидуальныхПредпринимателей.Найти(ПроверяемыйВидПродукции) <> Неопределено;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДОтправителя                    = МассивПродукцииТребующейУказаниеКодаФИАСОтправителя.Найти(ПроверяемыйВидПродукции) <> Неопределено;
		ПараметрыУказанияМОД.ТребуетсяУказаниеКодаФИАСПоМОДПолучателя                     = МассивПродукцииТребующейУказаниеКодаФИАСПолучателя.Найти(ПроверяемыйВидПродукции) <> Неопределено;
		ПараметрыУказанияМОД.ПроверятьМОДТолькоПоПодконтрольнымОбъектамВЕТИС              = МассивПродукцииТребующейПодконтрольностьВЕТИСПриПроверкеМОД.Найти(ПроверяемыйВидПродукции) <> Неопределено;
		
		СтруктураВозврата.ПараметрыУказанияМОДПоВидамПродукции.Вставить(ПроверяемыйВидПродукции, ПараметрыУказанияМОД);
		
		ТребуетсяУказаниеПеревозочныхДокументов = ТребуетсяУказаниеПеревозочныхДокументов Или МассивПродукцииТребующейУказаниеПеревозочныхДокументов.Найти(ПроверяемыйВидПродукции) <> Неопределено;
...
Процедура ЗаполнитьСтруктуруАдресовУчастниковОбменаПоПеремещениюМеждуМОД(ОбъектУчета, СтруктураДанных, СтруктураАдресов)
	...
	ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе = ПараметрыУказанияДанныхПоМОДВЭлектронномДокументеУПД_5_02(ДанныеШапки.ВидПродукции, ДанныеШапки.ДатаДокумента, Истина);
	
	ПараметрыУказанияМОД = ПараметрыУказанияДанныхПоМОДВЭлектронномДокументе.ПараметрыУказанияМОДПоВидамПродукции.Получить(ДанныеШапки.ВидПродукции);
