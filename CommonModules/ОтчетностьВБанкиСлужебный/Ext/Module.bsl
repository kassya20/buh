...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ОтправитьОтчетностьВБанк", Истина);
	Методы.Вставить("ОбновитьСтатусыОтчетов", Истина);
	Методы.Вставить("ОтправитьОтчетностьВБанкПринудительно", Истина);
	Методы.Вставить("СформироватьСодержимоеПакетаОтправки", Истина);
КонецПроцедуры
...
Функция ПараметрыЭлектронногоПредставления(Организация, ПараметрыОтчетов, ДатаВыгрузки) Экспорт
	...
	ПараметрыВыгрузки.Вставить("ВерсФорм", "5.13.02");
...
Процедура ВывестиУзел_ОперБанкСч(Компоновщик, ПараметрыВыгрузки)
	...
	СводныеСведения = СведенияОбОперациях.ЦифровойРубль.СводныеСведения;
	ДетальныеСведения = СведенияОбОперациях.ЦифровойРубль.ДетальныеСведения;
	ВывестиУзел_ОперБанкСч_СчетУчета(Компоновщик, СводныеСведения, ДетальныеСведения);
...
Процедура ВывестиУзел_ОперБанкСч_СчетУчета_БанкСч(Компоновщик, СтрокиСчетов, ДетальныеСведения)
	...
	Для Каждого СтрокаСчета Из СтрокиСчетов Цикл
		Если ОписаниеТипаБанковскиеСчета.СодержитТип(ТипЗнч(СтрокаСчета.Счет)) Тогда
			...
			Если СтрокаСчета.ЭтоЦифровойСчет Тогда
			...
			Если ВыборкаПоБанковскимСчетам.НайтиСледующий(СтруктураПоиска) Тогда
				...
				Пока ВыборкаОпераций.Следующий() Цикл
					...
					Если ЗначениеЗаполнено(СтрокаСекции.Контрагент) Тогда
						...
						Если СтрокаСекции.СчетКонтрагентаЭтоЦифровойСчет Тогда
...
Функция СведенияОбОперацияхПоБанковскимСчетам(Параметры)
	...
	СведенияОбОперациях.Вставить("ЦифровойРубль", Новый Структура("СчетУчета, СводныеСведения, ДетальныеСведения"));
