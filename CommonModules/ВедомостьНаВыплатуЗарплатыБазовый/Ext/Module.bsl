...
Процедура ДобавитьКомандыПечатиПриВыплатеНаКарточки(КомандыПечати) Экспорт
	...
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидМестаВыплаты", Перечисления.ВидыМестВыплатыЗарплаты.БанковскийСчет);
	...
	КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;
	УправлениеПечатью.ДобавитьУсловиеВидимостиКоманды(КомандаПечати, "ВидМестаВыплаты", Перечисления.ВидыМестВыплатыЗарплаты.ЗарплатныйПроект);
...
Функция ПараметрыЗаполненияПоОбъекту(Объект) Экспорт
	...
	ПараметрыЗаполнения.ОписаниеОперации.ВидДокументаОснования = СпособВыплаты.ВидДокументаОснования;
...
Функция ПараметрыПолученияСотрудниковПоШапкеВедомости(ОписаниеОперации, ОтборСотрудников)
	...
	ПараметрыПолученияСотрудников.КадровыеДанные   = "Подразделение, Организация";
...
Процедура СоздатьВТСотрудникиДляВедомостиПоОснованиям(МенеджерВременныхТаблиц, ОписаниеОперации, ИмяВТСотрудники = "")
	...
	|ГДЕ
	|	ЗарплатаКВыплатеАвансом.Регистратор В(&Основания)
	|	И ТИПЗНАЧЕНИЯ(ЗарплатаКВыплатеАвансом.Регистратор) = ТИП(Документ.НачислениеЗарплаты)";
...
Процедура СоздатьВТУсловияВозвратаЗадолженности(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТУсловияВозвратаЗадолженности";
	Запрос.Выполнить();
