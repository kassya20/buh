...
Функция НовыйКонтекстРасчета(Организация, КонецПериода) Экспорт
	...
	КонтекстРасчета.Вставить("УточнениеПериода",        0);
	...
	КонтекстРасчета.Вставить(
		"ПрименяетсяФСБУ25",
		УчетнаяПолитика.ПрименяетсяФСБУ25(Организация, КонецПериода));
	
	КонтекстРасчета.Вставить(
		"ПрименяетсяИнвестиционныйВычет",
		ИнвестиционныйНалоговыйВычет.МожноИспользоватьОперациюСИнвестиционнымВычетом(Организация, КонецПериода));
	
	ПоказательДляОпределенияДолиПрибыли = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ПоказательОпределенияДолиПрибыли(ПоказательДляОпределенияДолиПрибыли, Организация, КонецПериода);
	
	КонтекстРасчета.Вставить("ПоказательОпределенияДолиПрибыли",
		ПоказательДляОпределенияДолиПрибыли);
	...
	КонтекстРасчета.Вставить("НомерРасчета", 0);
...
Функция НачислитьНалогНаПрибыль(Движения, КонтекстРасчета, РасчетРаспределенияНалоговойБазы, БазаНалогаЗаГод, МенеджерВременныхТаблиц) Экспорт
	...
	НачислениеНалога = НачислениеНалогаНаПрибыль(
		...
		МенеджерВременныхТаблиц); // см. НовыйНачислениеНалогаНаПрибыль
	...
	ОтразитьПрименениеИнвестиционногоВычета(
		Движения,
		НачислениеНалога,
		БазаНалогаЗаГод,
		КонтекстРасчета,
	...
	Для Каждого ДанныеПроводки Из НачислениеНалога Цикл
		...
		Проводка.Содержание  = ДанныеПроводки.СодержаниеПроводки;

		Если КонтекстРасчета.ВариантУчетаОтложенногоНалога = Перечисления.ВариантыУчетаОтложенногоНалога.БалансовыйМетод
			И ДанныеПроводки.ЭтоИнвестиционныйВычет Тогда

			Проводка.Сумма       = - ДанныеПроводки.СуммаПроводки;
			Проводка.СчетКт      = ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль;

			Проводка.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыСБюджетом;

			СубконтоПроводкиРасчетыСБюджетом = Проводка.СубконтоДт;
		...
		Иначе
			...
			СубконтоПроводкиРасчетыСБюджетом = Проводка.СубконтоКт;
		КонецЕсли;

		БухгалтерскийУчет.УстановитьСубконто(
			ПланыСчетов.Хозрасчетный.РасчетыСБюджетом, 
			СубконтоПроводкиРасчетыСБюджетом,
...
//  СоответствиеПрежнихИТекущихНалоговыхОрганов - см. НалоговыйУчетОбособленныхПодразделений.ПодготовитьРасчетРаспределенияНалоговойБазы - содержит временную таблицу
//								     СоответствиеПрежнихИТекущихНалоговыхОрганов, которая используется в процедуре, если налог распределяется между налоговыми органами
// 
// Возвращаемое значение:
//  см. НовыйНачислениеНалогаНаПрибыль
//
Функция НачислениеНалогаНаПрибыль(КонтекстРасчета, НалоговаяБаза, СтавкаФедеральныйБюджет, СтавкаРегиональныйБюджет, РаспределениеНалогаНаПрибыль, СоответствиеПрежнихИТекущихНалоговыхОрганов)
	...
	ЕстьРаспределениеНалогаНаПрибыль = ЗначениеЗаполнено(РаспределениеНалогаНаПрибыль);
	
	ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, ЕстьРаспределениеНалогаНаПрибыль, СоответствиеПрежнихИТекущихНалоговыхОрганов);
	
	Возврат НачислениеНалога;
	
КонецФункции

Процедура ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, ЕстьРаспределениеНалогаНаПрибыль, МенеджерВременныхТаблиц)
	...
	Если КонтекстРасчета.НачалоМесяца > КонтекстРасчета.НачалоНалоговогоПериода Тогда
		...
		Запрос.УстановитьПараметр("СчетИнвестиционногоВычета", ПланыСчетов.Хозрасчетный.ИнвестиционныйВычетПоНалогуНаПрибыль);
		...
		Если ЕстьРаспределениеНалогаНаПрибыль Тогда
			...
			// Текст запроса модифицируется ниже с использованием СхемаЗапроса
			ТекстЗапросаАвансовыеПлатежи =
			"ВЫБРАТЬ
			...
			|	СУММА(ВЫБОР
			|			КОГДА ХозрасчетныйОбороты.КорСчет = &СчетИнвестиционногоВычета
			|				ТОГДА -ХозрасчетныйОбороты.СуммаОборотДт
			|			ИНАЧЕ ХозрасчетныйОбороты.СуммаОборотКт
			|		КОНЕЦ) КАК СуммаАвансовыхПлатежей,
			|	СУММА(ВЫБОР
			|			КОГДА ХозрасчетныйОбороты.Субконто2 = ЗНАЧЕНИЕ(Перечисление.УровниБюджетов.ФедеральныйБюджет)
			|				ТОГДА ХозрасчетныйОбороты.СуммаОборотДт
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК СуммаИнвестиционногоВычетаАвансовыхПлатежей
			|ПОМЕСТИТЬ ПрежниеНачисленияАвансовыеПлатежи
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.Обороты(
			...
			|ПОМЕСТИТЬ ПрежниеНачисленияПоРегистрациям
			|ИЗ
			|	ПрежниеНачисленияАвансовыеПлатежи КАК ПрежниеНачисления
		...
		ТекстыЗапроса.Добавить(ТекстЗапросаАвансовыеПлатежи);
		
		ТекстЗапросаИнвестиционныйВычет =
		"ВЫБРАТЬ
		|	ЦЕЛ(Хозрасчетный.СуммаНУОборотКт) КАК СуммаИнвестиционногоВычетаАвансовыхПлатежей
		|ПОМЕСТИТЬ ПрежниеНачисленияИнвестиционныйВычет
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоНалоговогоПериода, &КонецОтчетногоПериода, , СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка), , СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ФедеральныйИнвестиционныйНалоговыйВычет), , Организация = &Организация) КАК Хозрасчетный
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПрежниеНачисления.УровеньБюджета КАК УровеньБюджета,
		|	ПрежниеНачисления.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
		|	СУММА(ПрежниеНачисления.СуммаАвансовыхПлатежей) КАК СуммаАвансовыхПлатежей
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПрежниеНачисленияПоРегистрациям.УровеньБюджета КАК УровеньБюджета,
		|		ПрежниеНачисленияПоРегистрациям.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
		|		ПрежниеНачисленияПоРегистрациям.СуммаАвансовыхПлатежей КАК СуммаАвансовыхПлатежей
		|	ИЗ
		|		ПрежниеНачисленияПоРегистрациям КАК ПрежниеНачисленияПоРегистрациям
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		ПрежниеНачисленияИнвестиционныйВычет.СуммаИнвестиционногоВычетаАвансовыхПлатежей
		|	ИЗ
		|		ПрежниеНачисленияИнвестиционныйВычет КАК ПрежниеНачисленияИнвестиционныйВычет) КАК ПрежниеНачисления
		|
		|СГРУППИРОВАТЬ ПО
		|	ПрежниеНачисления.УровеньБюджета,
		|	ПрежниеНачисления.РегистрацияВНалоговомОргане
		|
		|УПОРЯДОЧИТЬ ПО
		|	УровеньБюджета,
		|	РегистрацияВНалоговомОргане";
		
		ТекстыЗапроса.Добавить(ТекстЗапросаИнвестиционныйВычет);
		...
		Пока ВыборкаАвансы.Следующий() Цикл
			...
			Иначе
				...
				Начисление = ДобавитьНачислениеНалогаНаПрибыль(НачислениеНалога, ВыборкаАвансы.УровеньБюджета, ВыборкаАвансы.РегистрацияВНалоговомОргане, 0);
				Начисление.СуммаАвансовыхПлатежей = ВыборкаАвансы.СуммаАвансовыхПлатежей;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
// Конструктор таблицы начисления налога на прибыль.
//
// Параметры:
...
Функция НовыйНачислениеНалогаНаПрибыль(ТочностьРасчета = 0)
	...
	НачислениеНалога.Колонки.Добавить("ЭтоИнвестиционныйВычет",       Новый ОписаниеТипов("Булево"));
	НачислениеНалога.Колонки.Добавить("СодержаниеПроводки",           ОбщегоНазначения.ОписаниеТипаСтрока(1000));
	
	Возврат НачислениеНалога;
	
КонецФункции

Функция ДобавитьНачислениеНалогаНаПрибыль(НачислениеНалога, УровеньБюджета, РегистрацияВНалоговомОргане, СуммаНалога)
	...
	Начисление.СодержаниеПроводки = НСтр("ru = 'Налог на прибыль'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ИнвестиционныйВычет

// Добавляет проводки уменьшения налога на сумму инвестиционного вычета.
// 
// Параметры:
//  Движения - КоллекцияДвижений
//  НачислениеНалога - см. НовыйНачислениеНалогаНаПрибыль
//  БазаНалогаЗаГод - Число
//  КонтекстРасчета - см. НовыйКонтекстРасчета
//  МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//
Процедура ОтразитьПрименениеИнвестиционногоВычета(Движения, НачислениеНалога, БазаНалогаЗаГод, КонтекстРасчета, МенеджерВременныхТаблиц)
	...
	ОтборФедеральныйБюджет = Новый Структура("УровеньБюджета", Перечисления.УровниБюджетов.ФедеральныйБюджет);
	НачислениеНалогаВФедеральныйБюджет =
		НачислениеНалога.Скопировать(ОтборФедеральныйБюджет, "РегистрацияВНалоговомОргане,СуммаНалога,СуммаПроводки");

	НачислениеНалогаВФедеральныйБюджет.Свернуть("РегистрацияВНалоговомОргане", "СуммаНалога,СуммаПроводки");

	Если НачислениеНалогаВФедеральныйБюджет.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("КонецМесяца", КонтекстРасчета.КонецПериода);
	Запрос.УстановитьПараметр("Период", Новый Граница(КонецМесяца(Период), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("СчетИнвестиционногоВычета",
		ПланыСчетов.Хозрасчетный.ФедеральныйИнвестиционныйНалоговыйВычет);
	Запрос.УстановитьПараметр("СубконтоСчетаИнвестиционногоВычета",
		ИнвестиционныйНалоговыйВычет.СубконтоСчетаИнвестиционныйВычет());

	Филиалы = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
	...
	// В регистре ДанныеПервичныхДокументов данные хранятся в разрезе организаций и документов. Поскольку в запросе могут
	// быть документы разных организаций, и в регистре технически один документ может относиться к разным организациям,
	// будем брать из регистра самую раннюю из дат регистраторов.
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ХозрасчетныйОстатки.Субконто1 КАК ВидРасходов,
	|	ХозрасчетныйОстатки.Субконто2 КАК АмортизируемоеИмущество,
	|	ХозрасчетныйОстатки.Субконто3 КАК ДокументВычета,
	|	ХозрасчетныйОстатки.СуммаНУОстатокДт КАК СуммаВычета,
	|	ЛОЖЬ КАК ТребуетсяОтразитьПередачуВычета
	|ПОМЕСТИТЬ ОстаткиВычета
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&Период, Счет = &СчетИнвестиционногоВычета, &СубконтоСчетаИнвестиционногоВычета, Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВычетФилиаловТекущегоПериода.Субконто1,
	|	ВычетФилиаловТекущегоПериода.Субконто2,
	|	ВычетФилиаловТекущегоПериода.Субконто3,
	|	ВычетФилиаловТекущегоПериода.СуммаНУОборотКт,
	|	ИСТИНА
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоМесяца, &КонецМесяца, , Счет = &СчетИнвестиционногоВычета, &СубконтоСчетаИнвестиционногоВычета, Организация В (&Филиалы), , ) КАК ВычетФилиаловТекущегоПериода
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументВычета
	|;
	...
	|	ХозрасчетныйОбороты.Субконто2 КАК АмортизируемоеИмущество,
	|	ХозрасчетныйОбороты.Субконто3 КАК ДокументВычета,
	|	СУММА(ХозрасчетныйОбороты.СуммаНУОборотКт) КАК СуммаВычета
	|ПОМЕСТИТЬ ПримененныйВычетТекущегоПериода
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоНалоговогоПериода, &КонецМесяца, Месяц, Счет = &СчетИнвестиционногоВычета, &СубконтоСчетаИнвестиционногоВычета, Организация = &Организация, , ) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Субконто1,
	|	ХозрасчетныйОбороты.Субконто2,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиВычета.ДокументВычета КАК ДокументВычета
	|ПОМЕСТИТЬ СписокДокументовВычета
	|ИЗ
	|	ОстаткиВычета КАК ОстаткиВычета
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПримененныйВычетТекущегоПериода.ДокументВычета
	|ИЗ
	|	ПримененныйВычетТекущегоПериода КАК ПримененныйВычетТекущегоПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписокДокументовВычета.ДокументВычета КАК ДокументВычета,
	|	МИНИМУМ(ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1, 1, 1))) КАК ДатаНачалаПримененияВычета
	|ПОМЕСТИТЬ ДанныеПервичныхДокументов
	|ИЗ
	|	СписокДокументовВычета КАК СписокДокументовВычета
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО СписокДокументовВычета.ДокументВычета = ДанныеПервичныхДокументов.Документ
	|
	|СГРУППИРОВАТЬ ПО
	|	СписокДокументовВычета.ДокументВычета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументВычета
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПримененныйВычетТекущегоПериода.АмортизируемоеИмущество КАК АмортизируемоеИмущество,
	|	ПримененныйВычетТекущегоПериода.ДокументВычета КАК ДокументВычета,
	|	ПримененныйВычетТекущегоПериода.ВидРасходов КАК ВидРасходов,
	|	ДанныеПервичныхДокументов.ДатаНачалаПримененияВычета КАК ДатаНачалаПримененияВычета,
	|	ПримененныйВычетТекущегоПериода.СуммаВычета КАК СуммаПримененногоВычета
	|ИЗ
	|	ПримененныйВычетТекущегоПериода КАК ПримененныйВычетТекущегоПериода
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ПримененныйВычетТекущегоПериода.ДокументВычета = ДанныеПервичныхДокументов.ДокументВычета
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаНачалаПримененияВычета УБЫВ,
	|	ПримененныйВычетТекущегоПериода.ДокументВычета УБЫВ,
	|	ПримененныйВычетТекущегоПериода.АмортизируемоеИмущество УБЫВ
	|ИТОГИ
	|	СУММА(СуммаПримененногоВычета)
	|ПО
	|	ОБЩИЕ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиВычета.АмортизируемоеИмущество КАК АмортизируемоеИмущество,
	|	ОстаткиВычета.ДокументВычета КАК ДокументВычета,
	|	ОстаткиВычета.ВидРасходов КАК ВидРасходов,
	|	ДанныеПервичныхДокументов.ДатаНачалаПримененияВычета КАК ДатаНачалаПримененияВычета,
	|	ЦЕЛ(ОстаткиВычета.СуммаВычета) КАК СуммаВычета,
	|	ОстаткиВычета.ТребуетсяОтразитьПередачуВычета КАК ТребуетсяОтразитьПередачуВычета
	|ИЗ
	|	ОстаткиВычета КАК ОстаткиВычета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОстаткиВычета.ДокументВычета = ДанныеПервичныхДокументов.ДокументВычета
	|ГДЕ
	|	ЦЕЛ(ОстаткиВычета.СуммаВычета) <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаНачалаПримененияВычета,
	|	ДокументВычета,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПередачаВычета.АмортизируемоеИмущество КАК АмортизируемоеИмущество,
	|	ПередачаВычета.ДокументВычета КАК ДокументВычета,
	|	ПередачаВычета.ВидРасходов КАК ВидРасходов,
	|	ДанныеПервичныхДокументов.ДатаНачалаПримененияВычета КАК ДатаНачалаПримененияВычета,
	|	ЦЕЛ(ПередачаВычета.СуммаВычета) КАК СуммаВычета
	|ИЗ
	|	ОстаткиВычета КАК ПередачаВычета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ПередачаВычета.ДокументВычета = ДанныеПервичныхДокументов.ДокументВычета
	|ГДЕ
	|	ЦЕЛ(ПередачаВычета.СуммаВычета) <> 0
	|	И ПередачаВычета.ТребуетсяОтразитьПередачуВычета
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаНачалаПримененияВычета,
	|	ДокументВычета,
	...
	ИнвестиционныйВычет = НовыйИнвестиционныйВычет();

	Начисление = НачислениеНалогаВФедеральныйБюджет[0];

	ПредельноеЗначениеВычета =
		Начисление.СуммаНалога - Макс(Окр(БазаНалогаЗаГод * МинимальныйРазмерНалогаФедеральныйБюджетВПроцентах(КонтекстРасчета.НачалоНалоговогоПериода) / 100), 0);
	ПредельноеЗначениеВычета = Макс(0, ПредельноеЗначениеВычета);

	ВыборкаРанееПримененныйВычет = Результат[Результат.ВГраница() - 2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	СуммаПримененногоВычетаТекущегоПериода = 0;
	Если ВыборкаРанееПримененныйВычет.Следующий() Тогда
		СуммаПримененногоВычетаТекущегоПериода = ВыборкаРанееПримененныйВычет.СуммаПримененногоВычета;
	КонецЕсли;
	
	Если ПредельноеЗначениеВычета < СуммаПримененногоВычетаТекущегоПериода Тогда
		СуммаВосстановления = СуммаПримененногоВычетаТекущегоПериода - ПредельноеЗначениеВычета;
		Выборка = ВыборкаРанееПримененныйВычет.Выбрать();
		Пока Выборка.Следующий() Цикл
			Если СуммаВосстановления <= 0 Тогда
				Прервать;
			КонецЕсли;
			СтрокаВычета = ИнвестиционныйВычет.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаВычета, Выборка);
			СтрокаВычета.СуммаВосстановленияВычета = Мин(СуммаВосстановления, Выборка.СуммаПримененногоВычета);
			СуммаВосстановления = СуммаВосстановления - СтрокаВычета.СуммаВосстановленияВычета;
		КонецЦикла;
	ИначеЕсли ПредельноеЗначениеВычета > СуммаПримененногоВычетаТекущегоПериода Тогда
		ВыборкаОстаткиВычета = Результат[Результат.ВГраница() - 1].Выбрать();
		ОстатокПредельногоЗначенияВычета = ПредельноеЗначениеВычета - СуммаПримененногоВычетаТекущегоПериода;
		Пока ВыборкаОстаткиВычета.Следующий() Цикл
			Если ОстатокПредельногоЗначенияВычета <= 0 Тогда
				Прервать;
			КонецЕсли;
			СтрокаВычета = ИнвестиционныйВычет.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаВычета, ВыборкаОстаткиВычета);
			СтрокаВычета.СуммаВычета = Мин(ОстатокПредельногоЗначенияВычета, ВыборкаОстаткиВычета.СуммаВычета);
			ОстатокПредельногоЗначенияВычета = ОстатокПредельногоЗначенияВычета - СтрокаВычета.СуммаВычета;
		КонецЦикла;
	КонецЕсли;
	
	// Получение вычета от обособленных подразделений
	ВыборкаПередачаВычета = Результат[Результат.ВГраница()].Выбрать();
	Пока ВыборкаПередачаВычета.Следующий() Цикл
		
		Содержание = НСтр("ru = 'Получение вычета от обособленного подразделения'", ОбщегоНазначения.КодОсновногоЯзыка());
		СуммаПроводки = ВыборкаПередачаВычета.СуммаВычета;
		
		ДобавитьПроводкуФедеральныйИнвестиционныйНалоговыйВычет(
			Движения.Хозрасчетный,
			КонтекстРасчета,
			ВидДвиженияБухгалтерии.Дебет,
			СуммаПроводки,
			Содержание,
			ВыборкаПередачаВычета);

	КонецЦикла;
	
	// Отразим уменьшение вычета на забалансовом счете.
	Для Каждого ИнвестиционныйВычетПоОбъекту Из ИнвестиционныйВычет Цикл

		Если ИнвестиционныйВычетПоОбъекту.СуммаВычета > 0 Тогда
			Содержание = НСтр("ru = 'Применение федерального инвестиционного налогового вычета'", ОбщегоНазначения.КодОсновногоЯзыка());
			СуммаПроводки = ИнвестиционныйВычетПоОбъекту.СуммаВычета;
		Иначе
			Содержание = НСтр("ru = 'Восстановление ранее примененного федерального инвестиционного налогового вычета'", ОбщегоНазначения.КодОсновногоЯзыка());
			СуммаПроводки = - ИнвестиционныйВычетПоОбъекту.СуммаВосстановленияВычета;
		КонецЕсли;
		...
		ДобавитьПроводкуФедеральныйИнвестиционныйНалоговыйВычет(
			...
			СуммаПроводки,
			Содержание,
			ИнвестиционныйВычетПоОбъекту);
		
	КонецЦикла;

	СуммаИнвестиционногоВычетаАвансовыхПлатежей = 0;
	Если КонтекстРасчета.НачалоМесяца > КонтекстРасчета.НачалоНалоговогоПериода Тогда

		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(ПрежниеНачисления.СуммаИнвестиционногоВычетаАвансовыхПлатежей), 0) КАК Сумма
		|ИЗ
		|	ПрежниеНачисленияИнвестиционныйВычет КАК ПрежниеНачисления";

		ВыборкаУменьшениеАвансовогоПлатежаПрошлогоПериода = Запрос.Выполнить().Выбрать();
		ВыборкаУменьшениеАвансовогоПлатежаПрошлогоПериода.Следующий(); // Всегда одна запись
		СуммаИнвестиционногоВычетаАвансовыхПлатежей = ВыборкаУменьшениеАвансовогоПлатежаПрошлогоПериода.Сумма;

	КонецЕсли;

	СуммаВычета = ИнвестиционныйВычет.Итог("СуммаВычета");
	Если ПредельноеЗначениеВычета < СуммаИнвестиционногоВычетаАвансовыхПлатежей Тогда
		СуммаВычета = ПредельноеЗначениеВычета - СуммаИнвестиционногоВычетаАвансовыхПлатежей;
	КонецЕсли;

	// Передадим сумму, на которую будет уменьшен (восстановлен) налог.
	Если СуммаВычета <> 0 Тогда

		НачислениеВычета = НачислениеНалога.Добавить();
		НачислениеВычета.ЭтоИнвестиционныйВычет = Истина;
		НачислениеВычета.УровеньБюджета = Перечисления.УровниБюджетов.ФедеральныйБюджет;
		НачислениеВычета.РегистрацияВНалоговомОргане = Начисление.РегистрацияВНалоговомОргане;
		НачислениеВычета.СуммаПроводки = - СуммаВычета;

		СодержаниеПроводки = НСтр("ru = 'Налог на прибыль уменьшен на сумму федерального инвестиционного вычета'",
			ОбщегоНазначения.КодОсновногоЯзыка());

		Если СуммаВычета < 0 Тогда
			СодержаниеПроводки =
				НСтр("ru = 'Налог на прибыль восстановлен на сумму ранее примененного инвестиционного налогового вычета'",
				ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;

		НачислениеВычета.СодержаниеПроводки = СодержаниеПроводки;

	КонецЕсли;

КонецПроцедуры

Процедура ДобавитьПроводкуФедеральныйИнвестиционныйНалоговыйВычет(Проводки, КонтекстРасчета, ВидДвижения, Сумма, Содержание, ЗначенияСубконто)
	
	Если Сумма = 0 Или Не ЗначениеЗаполнено(ВидДвижения) Тогда
		Возврат;
	КонецЕсли;
	
	Проводка = Проводки.Добавить(); // РегистрБухгалтерииЗапись.Хозрасчетный
	
	Проводка.Организация = КонтекстРасчета.Организация;
	Проводка.Период      = КонтекстРасчета.КонецПериода;
	Проводка.Содержание  = Содержание;
	
	СчетВычета = ПланыСчетов.Хозрасчетный.ФедеральныйИнвестиционныйНалоговыйВычет;
	...
	БухгалтерскийУчет.УстановитьСубконто(
		...
		ЗначенияСубконто.АмортизируемоеИмущество);
	БухгалтерскийУчет.УстановитьСубконто(
		СчетВычета,
		Субконто,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыИнвестиционногоВычета,
		ЗначенияСубконто.ДокументВычета);
	БухгалтерскийУчет.УстановитьСубконто(
		СчетВычета,
		Субконто,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРасходовНаАмортизируемоеИмущество,
		ЗначенияСубконто.ВидРасходов);
	
КонецПроцедуры

Функция МинимальныйРазмерНалогаФедеральныйБюджетВПроцентах(НалоговыйПериод)
	
	// абз. 3 п. 6 ст. 286.2 НК РФ
	// Сумма налога (авансового платежа), подлежащая зачислению в федеральный бюджет, в результате уменьшения на федеральный инвестиционный
	// налоговый вычет не может быть ниже суммы налога, исчисленной по ставке 2 процента (3 процента - в 2025 - 2030 годах).
	Если Год(НалоговыйПериод) <= 2030 Тогда 
		Возврат 3;
	КонецЕсли;
...
Функция НовыйИнвестиционныйВычет()
	
	ТипСумма = ТипСуммаНалога();
	
	ИнвестиционныйВычет = Новый ТаблицаЗначений;
	ИнвестиционныйВычет.Колонки.Добавить("АмортизируемоеИмущество");
	ИнвестиционныйВычет.Колонки.Добавить("ДокументВычета");
	ИнвестиционныйВычет.Колонки.Добавить("ВидРасходов");
	ИнвестиционныйВычет.Колонки.Добавить("СуммаВычета", ТипСумма);
	ИнвестиционныйВычет.Колонки.Добавить("СуммаВосстановленияВычета", ТипСумма);
	
	Возврат ИнвестиционныйВычет;
...
Процедура ОтражениеОНАиОНО(ПроводкиРасш, ВидАктивовОбязательств, ВременныеРазницы, СчетаАктивовИОбязательств = Неопределено)
	...
	// ОНА по виду "убытки текущего периода" переложим в "убытки прошлых лет"
	Если ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.УбыткиПрошлыхЛет И СуммаДт09 > 0 Тогда
		...
		Запись.СодержаниеПроводки = НСтр("ru = 'Убыток текущего периода'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
		Запись.ПоДебету           = "Д";
		Запись.УбыткиПрошлыхЛет   = Истина;
		Возврат;
	КонецЕсли;
	...
	Если СуммаДт09 <> 0 Тогда
		...
		Запись.СуммаЕНВД          = СуммаЕНВД;
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.СодержаниеПроводки = НСтр("ru = 'Признание отложенного налогового актива'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
		Запись.ПоДебету           = "Д";
	КонецЕсли;
	...
	Если СуммаКт09 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "К";
		Запись.СодержаниеПроводки = НСтр("ru = 'Погашение отложенного налогового актива'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
	...
	Если СуммаКт77 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "К";
		Запись.СодержаниеПроводки = НСтр("ru = 'Признание отложенного налогового обязательства'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
	...
	Если СуммаДт77 <> 0 Тогда
		...
		Запись.Субконто           = ВидАктивовОбязательств;
		Запись.ПоДебету           = "Д";
		Запись.СодержаниеПроводки = НСтр("ru = 'Погашение отложенного налогового обязательства'", ОбщегоНазначения.КодОсновногоЯзыка()); // строка записывается в ИБ
	КонецЕсли;
...
Функция НовыйПроводкиНалогНаПрибыль(РасширенныеДанные = Ложь)
	...
	Проводки.Колонки.Добавить("УбыткиПрошлыхЛет",   Новый ОписаниеТипов("Булево"));
...
Функция ТекстЗапросаХозрасчетныйОбороты(ФактическиИспользуемыеСубконтоИсточника)
	...
	|			И (ХозрасчетныйСубконто3.УточнениеПериода = 0)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ХозрасчетныйДвижения.ВидАктивовОбязательств,
	|	ВТ_ХозрасчетныйДвижения.ИндексСтрокиВидаАктивовОбязательства,
...
Процедура СоздатьОборотыПоКорСчетам(Процессор)
	...
	КлючеваяОперация = "ИндексТаблицыСубконто.РасчетНалогаНаПрибыль_ОборотыПоКорСчетам";
...
Процедура ПересчитатьОтложенныйНалогПоСтавкеСледующегоНалоговогоПериода(Движения, Период, Организация, Регистратор) Экспорт
	...
	Если Не УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Организация, Период) Тогда
		// Следует проверить, является ли организация плательщиком налога на прибыль,
		// так как сведения о варианте учета ПБУ 18 сохраняются при переходе организации на УСН
		Возврат;
	КонецЕсли;
...
Функция РассчитатьКорректировкуОтложенногоНалога(Период, Организация, СтараяСтавка, НоваяСтавка) Экспорт
	...
	ОтчетнаяДата = Новый Граница(КонтекстРасчета.КонецПериода, ВидГраницы.Включая);
	...
	Для каждого Запись Из ДанныеДляЗаписи Цикл
		...
		Если СуммаНалога <> 0 Тогда
			...
			Если Запись.ПоДебету = "Д" Тогда
				...
				Проводка.СчетКт  = ?(Запись.УбыткиПрошлыхЛет, Запись.КоррСчет, Счет68_04Кредит);
				БухгалтерскийУчет.УстановитьСубконто(
					Проводка.СчетДт, Проводка.СубконтоДт, 1, Запись.Субконто);
					
				Если Запись.УбыткиПрошлыхЛет Тогда
					БухгалтерскийУчет.УстановитьСубконто(
						Проводка.СчетКт, Проводка.СубконтоКт, 1, Перечисления.ВидыАктивовИОбязательств.УбытокТекущегоПериода);
				ИначеЕсли Запись.Субконто68_4 <> Неопределено Тогда
					БухгалтерскийУчет.УстановитьСубконто(
...
Процедура ИсключитьАвансовыеПлатежиДоОсвобожденияОтНалога(ОсвобождениеОтНалога, НачалоПериодаОсвобождения, Организация, ЕстьРаспределениеНалогаНаПрибыль)
	...
	ИсключитьРанееНачисленныеАвансы(ОсвобождениеОтНалога, КонтекстРасчета, ЕстьРаспределениеНалогаНаПрибыль, СоответствиеПрежнихИТекущихНалоговыхОрганов);
