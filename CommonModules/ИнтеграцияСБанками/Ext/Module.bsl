...
Функция ИмяМетодаФоновойАктуализации() Экспорт
	...
	Возврат "Обработки.ЗакрытиеМесяца.АктуализироватьСБлокировкойПараллельности";
...
// 
// Возвращаемое значение:
//  Число - 3 - Количество секунд ожидания
//
Функция ВремяОжиданияАктуализации() Экспорт
...
Процедура ЗапланироватьВосстановлениеАктуальностиУчета(Организация, Немедленно = Ложь)
	...
	ПараметрыЗадания = ЗакрытиеМесяцаКлиентСервер.НовыеПараметрыАктуализации();
	ПараметрыЗадания.Организация = Организация;
	КлючЗадания = ЗакрытиеМесяцаКлиентСервер.КлючЗаданияАктуализации(ПараметрыЗадания);
	...
	// Параметры метода Обработки.ЗакрытиеМесяца.АктуализироватьСБлокировкойПараллельности(ПараметрыЗадания);
	ПараметрыЗапуска = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыАктуализации);
...
Функция ЗаданиеАктуализацииВыполняется(Организация)
	...
	ОтборЗаданий.Вставить("Ключ",             ЗакрытиеМесяцаКлиентСервер.КлючЗаданияАктуализации(ПараметрыЗадания));
