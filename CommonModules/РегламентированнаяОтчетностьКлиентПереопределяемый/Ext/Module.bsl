...
Процедура ПередОткрытиемФормыУведомления(Организация, ВидУведомления, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ВидУведомления = ПредопределенноеЗначение(
		...
		СтандартнаяОбработка = Ложь;
	ИначеЕсли ВидУведомления = ПредопределенноеЗначение(
			"Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ИсчисленныеСуммыНалогов")
		Или ВидУведомления = ПредопределенноеЗначение(
			"Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ЗаявлениеОЗачетеНалога") Тогда
		ЕдиныйНалоговыйСчетКлиент.ПередОткрытиемФормыУведомления(Организация, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораКодаПоОКВЭД(ВыполняемоеОповещение, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ВыполняемоеОповещение.ДополнительныеПараметры.Свойство("ТекущийКод") Тогда
		ПараметрыФормы.Вставить("ТекущийКод", ВыполняемоеОповещение.ДополнительныеПараметры.ТекущийКод);
	Иначе
		ПараметрыФормы.Вставить("ТекущийКод", "");
	КонецЕсли;
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ВыполняемоеОповещение", ВыполняемоеОповещение);
	ПараметрыОповещения.Вставить("ИмяКлассификатора", ИмяКлассификатора);
...
Процедура ОткрытьФормуНастроекПередАвтозаполнениемРеглОтчета(ВыполняемоеОповещение) Экспорт
	...
	РегламентированнаяОтчетностьБПКлиент.ОткрытьФормуНастроекПередАвтозаполнениемРеглОтчета(ВыполняемоеОповещение);
...
Процедура ДобавитьПрикладныеФормыОтправкаОтчетов(ИменаФорм) Экспорт
	...
	УчетОбособленныхПодразделенийКлиент.ДобавитьПрикладныеФормыОтправкаОтчетов(ИменаФорм);
...
Процедура ОткрытьФормуПомощникаУплатыНалога(Организация, ДатаКонцаПериода, ПараметрыПравила) Экспорт
	...
	ПараметрыПомощника = ВыполнениеЗадачБухгалтераКлиентСервер.НовыйПараметрыФормыПомощника();
	ПараметрыПомощника.Организация = Организация;
	ПараметрыПомощника.ПериодСобытия = ДатаКонцаПериода;
	ПараметрыПомощника.Правило = ПараметрыПравила.Правило;
	ПараметрыПомощника.Описание = ПараметрыПравила.Описание;
	ПараметрыПомощника.ИдентификаторЗадачи = ПараметрыПравила.ИдентификаторЗадачи;
	ПараметрыПомощника.Срок = ПараметрыПравила.Срок;
	ПараметрыПомощника.ЕстьИнформацияНаИТС = ПараметрыПравила.ЕстьИнформацияНаИТС;
	
	ИмяФормыПомощника = "Обработка.ПомощникУплатыНалога.Форма";
...
Процедура ОткрытьФормуПодбораНоменклатуры(СтандартнаяОбработка, ОповещениеОЗакрытии, Владелец, ЗапрашиваемыеПоказатели) Экспорт
	РегламентированнаяОтчетностьБПКлиент.ОткрытьФормуПодбораНоменклатуры(СтандартнаяОбработка, ОповещениеОЗакрытии, Владелец, ЗапрашиваемыеПоказатели);
КонецПроцедуры
...
Процедура СверкаДекларацииНДСПриОткрытииОтчета(Форма, ИмяОтчета) Экспорт
	...
	СервисСверкиРасчетовСверкаДекларацийНДСКлиент.ПриОткрытииОтчета(Форма, ИмяОтчета);
...
Процедура СверитьДекларациюПоНДС(Форма) Экспорт
	...
	СервисСверкиРасчетовСверкаДекларацийНДСКлиент.СверитьДекларациюПоНДС(Форма);
...
Процедура ОткрытьОтчетПоРасхождениямВСверке(Форма) Экспорт
	...
	СервисСверкиРасчетовСверкаДекларацийНДСКлиент.ОткрытьОтчетПоРасхождениямВСверке(Форма);
