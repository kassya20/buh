...
Процедура ОпределитьСвойстваИсточниковДляЗаполненияСканированныхДокументовПоТребованиюФНС(СвойстваИсточников) Экспорт
	...
	ЗаполнитьСведения_АктСверкиРасчетов(СвойстваИсточников,                  Ложь);
...
Процедура ОпределитьСвойстваИсточниковДляРегистраДокументыПоТребованиюФНС(СвойстваИсточников) Экспорт
	...
	ЗаполнитьСведения_АктСверкиРасчетов(СвойстваИсточников,                  Истина);
	
КонецПроцедуры

Процедура ЗаполнитьСведения_ДоговорКонтрагента(СвойстваИсточников, ТолькоОсновныеСвойства) Экспорт
	
	ТипДокумента = Тип("СправочникСсылка.ДоговорыКонтрагентов");
	...
	|	ДоговорыКонтрагентов.Сумма КАК СуммаВсего
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		ВидПредставляемогоДокумента = ВидПредставляемогоДокумента(Выборка.Договор, СозданныеЭлектронныеДокументы,
		...
		ЗаполнитьДоговорнойДокумент(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства);
...
Процедура ЗаполнитьСведения_РеализацияТоваровУслуг(СвойстваИсточников, ТолькоОсновныеСвойства)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.СтроительныеРаботы Тогда
			ЗаполнитьПередачуСтроительныхРабот(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства);
		КонецЕсли;
...
Процедура ЗаполнитьСведения_АктСверкиРасчетов(СвойстваИсточников, ТолькоОсновныеСвойства)
	...
	ТипДокумента = Тип("ДокументСсылка.АктСверкиВзаиморасчетов");
	...
	|	ЕСТЬNULL(АктСверкиВзаиморасчетов.ДоговорКонтрагента.Номер, """") КАК НомерОснования,
	|	ЕСТЬNULL(АктСверкиВзаиморасчетов.ДоговорКонтрагента.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаОснования,
	|	ЕСТЬNULL(АктСверкиВзаиморасчетов.ДоговорКонтрагента.ОплатаВВалюте, ЛОЖЬ) КАК ОплатаВВалюте,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """") КАК Номер,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК Дата,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО АктСверкиВзаиморасчетов.Ссылка = ДанныеПервичныхДокументов.Документ
	|			И АктСверкиВзаиморасчетов.Организация = ДанныеПервичныхДокументов.Организация
	|ГДЕ
	|	АктСверкиВзаиморасчетов.Ссылка В(&Источники)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьАктСверкиВзаиморасчетов(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства);
...
Процедура ЗаполнитьПередачуСтроительныхРабот(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства)
	
	ВалютаРегламентированногоУчета = ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	
	Если Выборка.УслугиКоличество > 0 Тогда
		
		// Передача строительных работ
		Документ = СтруктураОсновныхСвойств(ОбъектИсточник);
		
		Документ.Вставить("ВидДокументаФНС", Перечисления.ВидыПредставляемыхДокументов.АктПриемкиСтроительныхРаботУслуг);
		УстановитьНаличиеЭлектронногоДокумента(Документ, ОбъектИсточник);
...
Процедура ЗаполнитьАктСверкиВзаиморасчетов(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства)
	...
	Документ.Вставить("ВидДокументаФНС", Перечисления.ВидыПредставляемыхДокументов.АктСверкиВзаиморасчетов);
	УстановитьНаличиеЭлектронногоДокумента(Документ, ОбъектИсточник);
...
Процедура ЗаполнитьДоговорнойДокумент(Выборка, ОбъектИсточник, СвойстваИсточников, ТолькоОсновныеСвойства)
	
	// Договорной документ
	Документ = СтруктураОсновныхСвойств(ОбъектИсточник);
	Документ.Вставить("ВидДокументаФНС", Перечисления.ВидыПредставляемыхДокументов.ДоговорнойДокумент);
	УстановитьНаличиеЭлектронногоДокумента(Документ, ОбъектИсточник);
	...
	Если ТолькоОсновныеСвойства Тогда
		// Если запрашиваются только основные свойства, возвращаем сумму договорного документа
		Документ.Вставить("СуммаВсего", Выборка.СуммаВсего);
	Иначе
		
		Документ.Вставить("СуммаВсего",  Выборка.СуммаВсего);
		Документ.Вставить("Участники",   Новый Массив);
		
		ДобавитьУчастника(Документ.Участники, Выборка.Организация, "Организация", Перечисления.РолиУчастниковСделкиДокументаПоТребованиюФНС.Отправитель);
		ДобавитьУчастника(Документ.Участники, Выборка.Продавец,  "Контрагент", Перечисления.РолиУчастниковСделкиДокументаПоТребованиюФНС.Получатель);
		
	КонецЕсли;
	
	СвойстваИсточников.Получить(Выборка.Договор).Добавить(Документ);
...
Процедура ОпределитьПодключаемыеНаправленияСдачиОтчетности(Организация, Направления) Экспорт
	...
	Для каждого Направление ИЗ Направления Цикл
		...
		ИначеЕсли Направление.Орган = Перечисления.ТипыКонтролирующихОрганов.УдалитьБанкРоссии Тогда
		...
		ИначеЕсли Направление.Орган = Перечисления.ТипыКонтролирующихОрганов.ФСГС Тогда
			УстановитьСвойстваНаправленияРосстат(Направление);
		ИначеЕсли Направление.Орган = Перечисления.ТипыКонтролирующихОрганов.ЦБ Тогда
			УстановитьСвойстваНаправленияЦентральныйБанк(Направление);
		Иначе
			УстановитьСвойстваНаправленияПоУмолчанию(Направление);
		КонецЕсли;
	КонецЦикла;
...
Процедура УстановитьСвойстваНаправленияФСС(РеквизитыОрганизации, Направление)
	...
	Направление.Доступен = Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком();
...
Процедура УстановитьСвойстваНаправленияБанкаРоссии(Направление)
	...
	ПодсистемаЭлектронныйДокументооборотСКонтролирующимиОрганами =
		Метаданные.Подсистемы.РегламентированнаяОтчетность.Подсистемы.ЭлектронныйДокументооборотСКонтролирующимиОрганами;
	
	Направление.Доступен = Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком()
		И ПодсистемаЭлектронныйДокументооборотСКонтролирующимиОрганами.Подсистемы.Найти("УдалитьСдачаОтчетностиВБанкРоссии") <> Неопределено;
	
	Направление.Подключить = Направление.Доступен;
	
КонецПроцедуры

Процедура УстановитьСвойстваНаправленияЦентральныйБанк(Направление)
	
	ПодсистемаЭлектронныйДокументооборотСКонтролирующимиОрганами =
		Метаданные.Подсистемы.РегламентированнаяОтчетность.Подсистемы.ЭлектронныйДокументооборотСКонтролирующимиОрганами;
	
	Направление.Доступен = Не ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком()
		И ПодсистемаЭлектронныйДокументооборотСКонтролирующимиОрганами.Подсистемы.Найти("СдачаОтчетностиВЦБ") <> Неопределено;
	Направление.Подключить = Направление.Доступен;
	
КонецПроцедуры

Процедура УстановитьСвойстваНаправленияРосстат(Направление)
...
Процедура УстановитьСвойстваНаправленияПоУмолчанию(Направление)
