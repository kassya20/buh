...
Процедура ВыполнитьАктивациюФизическихЛицКабинетСотрудника(МенеджерОбмена, РезультатВыгрузки) Экспорт
	...
	Если ФизическиеЛицаДляАктивации.Количество() > 0 Тогда
		...
		ИдентификаторСсылка = ИнтеграцияУправлениеПерсоналом.СоответствиеНаоборот(СсылкаПубличныйИдентификатор);
		...
		Для каждого Идентификатор Из Результат.Выгружено Цикл
			ФизическиеЛица.Добавить(ИдентификаторСсылка[Идентификатор]);
		КонецЦикла;
...
Функция ВыгрузитьИзмененияПрочихОбъектов(МенеджерОбмена) Экспорт
	...
	Если ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.2") Тогда
		ТипыОбъектов.Добавить(ПеречисленияМенеджер.ВидЛичногоКабинета);
	КонецЕсли;
	Если ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.3") Тогда
		ТипыОбъектов.Добавить(ПеречисленияМенеджер.ВидДокументаЭДО);
	КонецЕсли;
	Если ИнтеграцияУправлениеПерсоналом.ДоступенРасширенныйФункционал() Тогда
		...
		КонецЕсли;
		Если ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.2") Тогда
			ТипыОбъектов.Добавить(ПеречисленияМенеджер.ИнформацияОСредствахИЗ);
			ТипыОбъектов.Добавить(ПеречисленияМенеджер.Антропометрия);
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеМЧД) Тогда 
		...
		Для каждого СтрокаТЗ Из ДанныеМЧД Цикл
			...
			Для Каждого УстановленнаяПодпись Из УстановленныеПодписи Цикл
				...
				НаборСвойствПодписиЭДОБЗК = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(УстановленнаяПодпись, "НаборСвойствПодписиЭДОБЗК");
...
Функция ДанныеДляВыгрузкиСогласийЭДОБЗК(ТаблицаИзменений)
	...
	Для каждого СтрокаТЗ Из ДанныеСогласий Цикл
		...
		НоваяСтрока.ВидДокумента 			= ДанныеДокументаНаПодпись.ВидДокумента;
		...
		НоваяСтрокаСогласия.ТипСогласияЭДО 					= "Кадровый";
...
Функция ВыгрузитьГрафикиОтпусков(МенеджерОбмена)
	...
	ВыгружатьДанныеПоРабочимМестам = ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.2");
	...
	Если ЗначениеЗаполнено(Изменения) Тогда
		...
		Данные = ДанныеГрафиковОтпусков(Изменения, ВыгружатьДанныеПоРабочимМестам);
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Данные.ОтменитьРегистрацию, ОтменитьРегистрацию);
		
		Если ВыгружатьДанныеПоРабочимМестам Тогда
			
			ВыгруженныеФизическиеЛица 	= Новый Массив;
			НеВыгруженныеФизическиеЛица = Новый Массив;
			...
			Если ЗначениеЗаполнено(Данные.ДанныеГрафиков) Тогда
				...
				БылиОшибки = БылиОшибки Или РезультатВыгрузки.БылиОшибки;
				ФизическиеЛица = Данные.ДанныеГрафиков.ВыгрузитьКолонку("ФизическоеЛицо");
				ВыгруженныеФизическиеЛица = РезультатВыгрузки.Выгружено;
				НеВыгруженныеФизическиеЛица = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ФизическиеЛица, РезультатВыгрузки.Выгружено);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Данные.ДанныеПоРабочимМестам) Тогда
				
				Если НеВыгруженныеФизическиеЛица.Количество() = 0 Тогда
					ДанныеДляВыгрузки = Данные.ДанныеПоРабочимМестам;
				Иначе
					НеВыгружены = ОбщегоНазначенияБЗККлиентСервер.МассивВСоответствие(НеВыгруженныеФизическиеЛица);
					ДанныеДляВыгрузки = Данные.ДанныеПоРабочимМестам.СкопироватьКолонки();
					Для каждого СтрокаТЗ Из Данные.ДанныеПоРабочимМестам Цикл
						Если НеВыгружены[СтрокаТЗ.ФизическоеЛицо] = Истина Тогда
							Продолжить;
						КонецЕсли;
						ЗаполнитьЗначенияСвойств(ДанныеДляВыгрузки.Добавить(), СтрокаТЗ);
					КонецЦикла;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ДанныеДляВыгрузки) Тогда
					ТипОбъектаВыгрузки = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ГрафикОтпусковПоРабочимМестам;
					РезультатВыгрузки = МенеджерОбмена.РезультатВыгрузкиОбъектов(ДанныеДляВыгрузки, ТипОбъектаВыгрузки);
					БылиОшибки = БылиОшибки Или РезультатВыгрузки.БылиОшибки;
					ФизическиеЛица = ДанныеДляВыгрузки.ВыгрузитьКолонку("ФизическоеЛицо");
					НеВыгруженныеФизическиеЛица = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ФизическиеЛица, РезультатВыгрузки.Выгружено);
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВыгруженныеФизическиеЛица, РезультатВыгрузки.Выгружено, Истина);
					ВыгруженныеФизическиеЛица = ОбщегоНазначенияКлиентСервер.РазностьМассивов(ВыгруженныеФизическиеЛица, НеВыгруженныеФизическиеЛица);
				КонецЕсли;
...
Функция ДанныеГрафиковОтпусков(ТаблицаИзменений, ВыгружатьДанныеПоРабочимМестам)
	
	Данные = Новый Структура("ДанныеГрафиков,ДанныеПоРабочимМестам,ОтменитьРегистрацию");
	
	ДанныеГрафиков 			= НоваяТаблицаГрафиковОтпусков();
	ДанныеПоРабочимМестам 	= НоваяТаблицаГрафиковОтпусков();
	ОтменитьРегистрацию 	= ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
	...
	Если Не ЗначениеЗаполнено(ДанныеГрафиковОтпусков.ДанныеГрафиков) Тогда
		Для каждого СтрокаТЗ Из ТаблицаИзменений Цикл
			ЗаполнитьЗначенияСвойств(ОтменитьРегистрацию.Добавить(), СтрокаТЗ);
		КонецЦикла;
	Иначе
		
		Для каждого КлючИЗначение Из ДанныеГрафиковОтпусков.ДанныеГрафиков Цикл
		...
		Если ВыгружатьДанныеПоРабочимМестам Тогда
			Для каждого КлючИЗначение Из ДанныеГрафиковОтпусков.ДанныеГрафиковПоРабочимМестам Цикл
				ФизическоеЛицо = КлючИЗначение.Ключ;
				ПериодыГрафика = КлючИЗначение.Значение;
				ПериодыГрафикаОтпусков = НоваяТаблицаПериодыГрафикаОтпусковПоРабочимМестам();
				Для каждого ПериодГрафика Из ПериодыГрафика Цикл
					НовыйПериод = ПериодыГрафикаОтпусков.Добавить();
					НовыйПериод.Сотрудник 			= ПериодГрафика.Сотрудник;
					НовыйПериод.ВидОтпуска 			= ПериодГрафика.ВидОтпуска;
					НовыйПериод.ДатаНачала 			= ПериодГрафика.ДатаНачала;
					НовыйПериод.ДатаОкончания 		= ПериодГрафика.ДатаОкончания;
					НовыйПериод.КоличествоДней 		= ПериодГрафика.КоличествоДней;
					НовыйПериод.ИсходнаяДатаНачала 	= ПериодГрафика.ИсходнаяДатаНачала;
				КонецЦикла;
				НоваяСтрока = ДанныеПоРабочимМестам.Добавить();
				НоваяСтрока.ФизическоеЛицо = ФизическоеЛицо;
				НоваяСтрока.ПериодыГрафика = ПериодыГрафикаОтпусков;
			КонецЦикла;
		КонецЕсли;
	...
	Данные.ДанныеПоРабочимМестам 	= ДанныеПоРабочимМестам;
...
Функция ДанныеОбъектовДляВыгрузки(МенеджерОбмена, ТаблицаИзменений, ТипОбъекта)
	...
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.ИнформацияОСредствахИЗ Тогда
		
		ДанныеОбъектов = ДанныеСредствИндивидуальнойЗащиты(ТаблицаИзменений, Приложение);
		
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.Антропометрия Тогда
		
		ДанныеОбъектов = ДанныеАнтропометрии(ТаблицаИзменений, Приложение);
		
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.ВидЛичногоКабинета Тогда
		
		ДанныеОбъектов = ДанныеВидыЛичныхКабинетов(ТаблицаИзменений, Приложение);
		
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.ВидДокументаЭДО Тогда
		
		ДанныеОбъектов = ДанныеВидыДокументовЭДО(ТаблицаИзменений);
...
Функция ДанныеДоступныеФункцииФизическихЛиц(ТаблицаИзменений, Приложение)
	...
	ДетальноПоОрганизациям = ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.2");
	...
	Если ЗначениеЗаполнено(ФизическиеЛицаДляОбработки) Тогда
		Данные = ИнтеграцияУправлениеПерсоналомСлужебный.ДоступныеФункцииФизическихЛиц(ФизическиеЛицаДляОбработки, Приложение);
		Если ДетальноПоОрганизациям Тогда
			Для каждого СтрокаТЗ Из Данные Цикл
				СтрокаФизлица = ТаблицаДанных.Найти(СтрокаТЗ.ФизическоеЛицо, "ФизическоеЛицо");
				Если СтрокаФизлица = Неопределено Тогда
					СтрокаФизлица = ТаблицаДанных.Добавить();
					СтрокаФизлица.ФизическоеЛицо = СтрокаТЗ.ФизическоеЛицо;
					СтрокаФизлица.ДоступныеФункции = Новый Массив;
				КонецЕсли;
				ДоступныеФункции = Новый Структура("ИспользуетКЭДО,ИспользуетЭДО", СтрокаТЗ.ИспользуетЭДОБЗК, Ложь);
				ДоступныеФункцииВОрганизации = Новый Структура("Организация,ДоступныеФункции", СтрокаТЗ.Организация, ДоступныеФункции);
				СтрокаФизлица.ДоступныеФункции.Добавить(ДоступныеФункцииВОрганизации);
			КонецЦикла;
		Иначе
			Для каждого СтрокаТЗ Из Данные Цикл
				ЗаполнитьЗначенияСвойств(ТаблицаДанных.Добавить(), СтрокаТЗ);
...
Функция ДанныеСредствИндивидуальнойЗащиты(ТаблицаИзменений, Приложение)

	Результат = ИнтеграцияУправлениеПерсоналомОбмен.НовыеДанныеДляВыгрузки();
	
	Если Не ИнтеграцияКабинетСотрудника.ИспользоватьСредстваИндивидуальнойЗащиты() Тогда
		Возврат Результат;
	КонецЕсли;
	
	ТаблицаДанных 		= НоваяТаблицаИнформацияСИЗ();
	ОтменитьРегистрацию = ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
	...
	Если ЗначениеЗаполнено(ФизическиеЛицаДляОбработки) Тогда
		Данные = ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеСредствИндивидуальнойЗащиты(ФизическиеЛицаДляОбработки);
		Если Данные <> Неопределено Тогда
			...
			Для Каждого ФизическоеЛицо Из ФизическиеЛицаДляОбработки Цикл
				...
				Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
					ЗаполнитьЗначенияСвойств(ТаблицаДанных.Добавить(), НайденныеСтроки[0]);
				Иначе
					НоваяСтрока = ОтменитьРегистрацию.Добавить();
					НоваяСтрока.Ссылка 			= ФизическоеЛицо;
...
Функция ДанныеАнтропометрии(ТаблицаИзменений, Приложение)
	...
	ТаблицаДанных 		= НоваяТаблицаДанныеАнтропометрии();
	ОтменитьРегистрацию = ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
	...
	Если ЗначениеЗаполнено(ФизическиеЛицаДляОбработки) Тогда
		Данные = ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеАнтропометрии(ФизическиеЛицаДляОбработки);
		Если Данные <> Неопределено Тогда
			Данные.Индексы.Добавить("ФизическоеЛицо");
			Отбор = Новый Структура("ФизическоеЛицо");
			Для Каждого ФизическоеЛицо Из ФизическиеЛицаДляОбработки Цикл
...
Функция ДанныеВидыЛичныхКабинетов(ТаблицаИзменений, Приложение)

	Результат = ИнтеграцияУправлениеПерсоналомОбмен.НовыеДанныеДляВыгрузки();
	
	ТаблицаДанных 		= НоваяТаблицаВидыЛичныхКабинетов();
	ОтменитьРегистрацию = ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
	...
	Если ЗначениеЗаполнено(ФизическиеЛицаДляОбработки) Тогда
		ЗаполнитьТаблицуДанныхВидыЛичныхКабинетов(ФизическиеЛицаДляОбработки, ТаблицаДанных);
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуДанныхВидыЛичныхКабинетов(ФизическиеЛица, ТаблицаДанных)

	Данные = ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеВидыЛичныхКабинетов(ФизическиеЛица);
	Для каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		ВидКабинетаФизическогоЛица = Данные[ФизическоеЛицо];
		НоваяСтрока = ТаблицаДанных.Добавить();
		НоваяСтрока.ФизическоеЛицо 	= ФизическоеЛицо;
		НоваяСтрока.Сотрудник 		= ВидКабинетаФизическогоЛица.Сотрудник;
		НоваяСтрока.ДоговорГПХ 		= ВидКабинетаФизическогоЛица.ДоговорГПХ;
		НоваяСтрока.ИндивидуальныйПредприниматель = ВидКабинетаФизическогоЛица.ИндивидуальныйПредприниматель;
	КонецЦикла;

КонецПроцедуры

Функция ДанныеВидыДокументовЭДО(ТаблицаИзменений)
	
	Результат = ИнтеграцияУправлениеПерсоналомОбмен.НовыеДанныеДляВыгрузки();
	ТаблицаДанных = НоваяТаблицаВидовДокументовЭДО();
	ОтменитьРегистрацию = ТаблицаИзменений.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
	
	Ссылки = ТаблицаИзменений.ВыгрузитьКолонку("Ссылка");
	Данные = ИнтеграцияУправлениеПерсоналомСлужебный.ДанныеВидыДокументовЭДО(Ссылки);
	
	Для каждого СтрокаТЗ Из Данные Цикл
		НоваяСтрока = ТаблицаДанных.Добавить();
		НоваяСтрока.ВидДокументаЭДО 		= СтрокаТЗ.Ссылка;
...
Функция НовыйЭлектронныйДокумент()
	...
	Возврат Новый Структура("ИсходныйДокумент,ПредставленияДокумента,ОригиналВMXL,ВариантПодписания,ВидДокумента,ТипЭлектроннойПодписи");
...
Функция НоваяТаблицаПериодыГрафикаОтпусковПоРабочимМестам()
...
Функция НоваяТаблицаИнформацияСИЗ()
...
Функция НоваяТаблицаДанныеАнтропометрии()
...
Функция НоваяТаблицаВидыЛичныхКабинетов()
...
Функция НоваяТаблицаВидовДокументовЭДО()
...
Функция ЭлектронныйДокумент(ДанныеЭлектронногоДокумента)
	...
	ЭлектронныйДокумент.ВидДокумента = ДанныеЭлектронногоДокумента.ВидДокумента;
	ЭлектронныйДокумент.ТипЭлектроннойПодписи = ДанныеЭлектронногоДокумента.ТипЭлектроннойПодписи;
...
Функция РезультатыСогласованияЗаявкиСПодписью(ДанныеЗаявки, Комментарий)
	...
	Если ДанныеФайла.ПодписанЭП Тогда
		...
		Для каждого ПодписьДокумента Из ПодписиДокумента Цикл
			...
			НаборСвойствПодписиЭДОБЗК = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(ПодписьДокумента, "НаборСвойствПодписиЭДОБЗК");
			Если ЗначениеЗаполнено(НаборСвойствПодписиЭДОБЗК) Тогда
				Для Каждого ЭлементСвойств Из НаборСвойствПодписиЭДОБЗК Цикл
					НоваяСтрока = РезультатыСогласования.Добавить();
					НоваяСтрока.ИдентификаторОбъекта 				= Строка(Новый УникальныйИдентификатор);
...
Функция НовоеОписаниеПараметрыПубликацииРасчетныхЛистков() Экспорт
	
	ПараметрыПубликации = Новый Структура;
	ПараметрыПубликации.Вставить("ПовторнаяПубликация", 						Ложь);
	ПараметрыПубликации.Вставить("Организация", 								Неопределено);
	ПараметрыПубликации.Вставить("ПодготовитьДанныеДляТехПоддержки", 			Ложь);
	ПараметрыПубликации.Вставить("МесяцРасчетныхЛистов", 						Дата(1, 1, 1));
	ПараметрыПубликации.Вставить("СписокФизическихЛиц", 						Неопределено);
	ПараметрыПубликации.Вставить("ВариантОтчета", 								Неопределено);
	ПараметрыПубликации.Вставить("ПерваяПоловинаМесяца", 						Ложь);
	ПараметрыПубликации.Вставить("ФормироватьРасчетныеЛистыОтдельнымЗапросом", 	Ложь);
...
Функция РезультатВыгрузкиРасчетныхЛистков(МенеджерОбмена, ПараметрыВыгрузки)
	...
	Для каждого ПакетФизическихЛиц Из Пакеты Цикл
		...
		Для каждого ФизическоеЛицо Из ПакетФизическихЛиц Цикл
			...
			ОбъектЭлектронныйДокумент.ВидДокумента = ДокументЭДОБЗКОбъект.ВидДокумента;
			Если ЗначениеЗаполнено(ДокументЭДОБЗКОбъект.ВариантПодписания) Тогда
				ОбъектЭлектронныйДокумент.ВариантПодписания = ДокументЭДОБЗКОбъект.ВариантПодписания;
				ОбъектЭлектронныйДокумент.ТипЭлектроннойПодписи = ДокументЭДОБЗКОбъект.ТипЭлектроннойПодписи;
			Иначе
				СвойстваВидаДокументов = ЭДОБЗК.СвойстваВидаДокументов(ДокументЭДОБЗКОбъект.ВидДокумента);
				ОбъектЭлектронныйДокумент.ВариантПодписания = СвойстваВидаДокументов.ВариантПодписания;
				ОбъектЭлектронныйДокумент.ТипЭлектроннойПодписи = СвойстваВидаДокументов.ТипЭлектроннойПодписи;
			КонецЕсли;
			...
			НоваяСтрока.ВозможноОбновлениеПредставлений = Истина;
			НоваяСтрока.ОтправлятьУведомление 			= Ложь;
			НоваяСтрока.ВариантПодписания 				= ОбъектЭлектронныйДокумент.ВариантПодписания;
			НоваяСтрока.ВидДокумента 					= ОбъектЭлектронныйДокумент.ВидДокумента;
			НоваяСтрока.ТипЭлектроннойПодписи 			= ОбъектЭлектронныйДокумент.ТипЭлектроннойПодписи;
...
Функция ЗарегистрированныеИзмененияЗаявок()
	...
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявкаНаИзменениеГрафикаОтпуска);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявкаИсполнениеГособязанностей);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявкаВыходныеДляУходаЗаДетьмиИнвалидами);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявлениеНаКомандировку);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ОтчетОбИтогахКомандировки);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ОтчетОРасходахВКомандировке);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявлениеВПроизвольнойФорме);
	ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ЗаявлениеОСпособеВыплатыЗарплаты);
...
Процедура ОбновитьВидыЛичныхКабинетов(МенеджерОбмена, РезультатВыгрузки, БылиОшибки) Экспорт

	Если Не ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.2")
		Или Не ЗначениеЗаполнено(РезультатВыгрузки.Выгружено) Тогда
		Возврат;
	КонецЕсли;
	...
	|				И Изменения.ТипОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ВидЛичногоКабинета))
	|ГДЕ
	|	ФизическиеЛица.Ссылка В(&ФизическиеЛица)
	|	И НЕ ЕСТЬNULL(ВыгружаемыеОбъекты.ВыгружаетсяПоСсылке, ЛОЖЬ)";
	Таблица = Запрос.Выполнить().Выгрузить();
	ФизическиеЛицаДляОбновления = Таблица.ВыгрузитьКолонку("Ссылка");
	
	ФизическиеЛицаБезОбновления = ОбщегоНазначенияКлиентСервер.РазностьМассивов(РезультатВыгрузки.Выгружено, ФизическиеЛицаДляОбновления);
	
	ФизическиеЛица = Новый Массив;
	Если ФизическиеЛицаДляОбновления.Количество() > 0 Тогда
		
		ТаблицаДанных = НоваяТаблицаВидыЛичныхКабинетов();
		ЗаполнитьТаблицуДанныхВидыЛичныхКабинетов(ФизическиеЛицаДляОбновления, ТаблицаДанных);
		
		ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ВидЛичногоКабинета;
		Результат = МенеджерОбмена.РезультатВыгрузкиОбъектов(ТаблицаДанных, ТипОбъекта);
		
		// Отменим регистрацию выгруженных видов кабинетов.
		Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника;
		Изменения 			= ИнтеграцияКабинетСотрудника.НоваяТаблицаИзмененияДляОбмена();
		ОтменитьРегистрацию = Изменения.СкопироватьКолонки("Ссылка,ТипОбъекта,ВерсияДанных");
		ТаблицаОбработано 	= ИнтеграцияУправлениеПерсоналомОбмен.НоваяТаблицаОбработано();
		
		Для каждого СтрокаТЗ Из Таблица Цикл
			Если ЗначениеЗаполнено(СтрокаТЗ.ВерсияДанных) Тогда
				НоваяСтрока = Изменения.Добавить();
				НоваяСтрока.Ссылка 			= СтрокаТЗ.Ссылка;
				НоваяСтрока.ТипОбъекта 		= ТипОбъекта;
				НоваяСтрока.ВерсияДанных 	= СтрокаТЗ.ВерсияДанных;
			КонецЕсли;
		КонецЦикла;
		
		Для каждого ФизическоеЛицо Из Результат.Выгружено Цикл
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФизическиеЛица, ФизическиеЛицаБезОбновления);
