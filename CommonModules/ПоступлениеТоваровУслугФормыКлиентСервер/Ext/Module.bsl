...
Процедура УстановитьВидимостьКолонокРасхождения(Форма, Префиксы) Экспорт
	Элементы = Форма.Элементы;
	
	ЕстьРасхождения = Форма.Объект.ЕстьРасхождения;
	
	Элементы.АктОРасхождениях.Видимость = ЕстьРасхождения;
	
	СписокРеквизитовФормы = "ГруппаЗаголовкиСтрок,НадписьПоДокументу,НадписьПоФакту,
	|КоличествоПоДокументу,ЦенаПоДокументу,СуммаПоДокументу,СуммаНДСПоДокументу,
	|ВсегоПоДокументу,НадписьОтклонение,КоличествоОтклонение, КоличествоОтклонение,
	|ЦенаОтклонение,СуммаОтклонение,СуммаНДСОтклонение,ВсегоОтклонение";
	
	Если ПолучитьРеквизитФормы(Форма, "ИспользуютсяУпаковки") Тогда
		СписокРеквизитовФормы = СтрЗаменить(СписокРеквизитовФормы, "КоличествоПоДокументу"	, "КоличествоУпаковокПоДокументу");
		СписокРеквизитовФормы = СтрЗаменить(СписокРеквизитовФормы, "КоличествоОтклонение"	, "КоличествоУпаковокОтклонение");
		СписокРеквизитовФормы = СтрЗаменить(СписокРеквизитовФормы, "ЦенаПоДокументу"		, "ЦенаЗаУпаковкуПоДокументу");
		СписокРеквизитовФормы = СтрЗаменить(СписокРеквизитовФормы, "ЦенаОтклонение"			, "ЦенаЗаУпаковкуОтклонение");
	КонецЕсли;
	
	Для каждого Префикс Из СтрРазделить(Префиксы, ",") Цикл
		Для каждого ИмяРеквизита Из СтрРазделить(СписокРеквизитовФормы, ",") Цикл
			ИмяЭлемента = СокрЛП(Префикс) + СокрЛП(ИмяРеквизита);
			ОбщегоНазначенияклиентсервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяЭлемента, "Видимость", ЕстьРасхождения);
		КонецЦикла; 
	КонецЦикла;
...
Процедура УстановитьТребуетсяСчетФактура(Форма) Экспорт
	...
	// или агенского договора "Реализация медных полуфабрикатов п.8 ст.161 НК" требуем счет-фактуру.
	...
	ЭтоРеализацияП8Ст161 = 
		(Форма.ВидАгентскогоДоговора = ПредопределенноеЗначение("Перечисление.ВидыАгентскихДоговоров.РеализацияТоваров")
			ИЛИ Форма.ВидАгентскогоДоговора = ПредопределенноеЗначение("Перечисление.ВидыАгентскихДоговоров.РеализацияМедныхПолуфабрикатов"))
		И Форма.ОбратноеНачислениеНДС;
	
	Форма.ТребуетсяСчетФактура =
		Не Форма.ЭтоДоговорССамозанятым
		И Не Форма.УчетАгентскогоНДС
		И Не ЭтоКомиссия
...
Процедура ЗаполнитьДобавленныеКолонкиРасхождения(Форма) Экспорт
	...
	Для каждого ИмяТабличнойЧасти Из ТабличныеЧасти Цикл
		...
		Для каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
			...
			Если ИмяТабличнойЧасти = "Товары" И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИспользуютсяУпаковки") Тогда
				СтрокаТабличнойЧасти.КоличествоУпаковокПоДокументу 	= СтрокаТабличнойЧасти.КоличествоУпаковок;
				СтрокаТабличнойЧасти.ЦенаЗаУпаковкуПоДокументу 		= СтрокаТабличнойЧасти.Цена;
			КонецЕсли;
...
Функция ПолучитьРеквизитФормы(Форма, ИмяРеквизита) Экспорт
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ВидДоговора") Тогда
		// Признак поступления товаров на комиссию по продаже.
		Если ИмяФормы = "ФормаДокументаУслуги" Тогда
			Результат = Форма.ВидДоговора;
		Иначе
			Результат = Неопределено;
		КонецЕсли;
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ИспользуютсяУпаковки") Тогда
	    
		Если ИмяФормы = "ФормаДокументаОбщая" ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ИспользуютсяУпаковки;
		Иначе
			Результат = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьРеквизитФормы(Форма, ИмяРеквизита, ЗначениеРеквизита) Экспорт
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("НастроенОбменЭПД") Тогда
		// Признак того, что можно оформить ЭПД.
		Если ИмяФормы = "ФормаДокументаОбщая" ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Форма.НастроенОбменЭПД = ЗначениеРеквизита;
		КонецЕсли;
...
Процедура ПриИзмененииЕстьРасхождения(Форма, ИмяТабличнойЧасти) Экспорт
	УстановитьВидимостьКолонокРасхождения(Форма, ИмяТабличнойЧасти);
	ОбщегоНазначенияБПКлиентСервер.ЗаполнитьИдентификаторыСтрок(Форма.Объект, СтрРазделить(ИмяТабличнойЧасти, ","));
КонецПроцедуры
...
Процедура ПриИзмененииКоличество(Форма, ИмяТабличнойЧасти, ПрименяютсяСтавки4и2 = Ложь, ЗначениеПустогоКоличества = 0, Постфикс = "") Экспорт
	...
	Если НЕ Объект.ЕстьРасхождения Тогда
		...
		ОбработкаТабличныхЧастейКлиентСервер.ПриИзмененииКоличествоЦена(Форма, ИмяТабличнойЧасти, ЗначениеПустогоКоличества, 
			ПрименяютсяСтавки4и2, СоответствиеРеквизитов("ПоДокументу"));
	КонецЕсли; 
...
Процедура ПриИзмененииСумма(Форма, ИмяТабличнойЧасти, ПрименяютсяСтавки4и2 = Ложь, ЗначениеПустогоКоличества = 0, Постфикс = "") Экспорт
	...
	ОбработкаТабличныхЧастейКлиентСервер.ПриИзмененииСумма(Форма, ИмяТабличнойЧасти, ЗначениеПустогоКоличества, 
		ПрименяютсяСтавки4и2, СоответствиеРеквизитов(Постфикс));
...
Процедура ПриИзмененииСтавкаНДС(Форма, ИмяТабличнойЧасти, ПрименяютсяСтавки4и2 = Ложь) Экспорт
	...
	Если НЕ Объект.ЕстьРасхождения Тогда
		...
		ОбработкаТабличныхЧастейКлиентСервер.ПриИзмененииСтавкаНДС(Форма, ИмяТабличнойЧасти, ПрименяютсяСтавки4и2, 
			СоответствиеРеквизитов("ПоДокументу"));
	КонецЕсли; 
...
Процедура ПриИзмененииСуммаНДС(Форма, ИмяТабличнойЧасти, Постфикс = "") Экспорт
	...
	ОбработкаТабличныхЧастейКлиентСервер.ПриИзмененииСуммаНДС(Форма, ИмяТабличнойЧасти, 
		СоответствиеРеквизитов(Постфикс));
...
Функция НачатьЗаполнениеСчетовУчета(ПричиныИзменения, Объект = Неопределено, СтрокаСписка = Неопределено, КонтейнерОбъект = Неопределено, КонтейнерСтрокаСписка = Неопределено) Экспорт
	...
	Если ПараметрыЗаполнения.ПричиныИзменения.Найти("ВидОперации") <> Неопределено Тогда
		...
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "СчетУчетаРасчетовСКомиссионером");
	...
	Если ПараметрыЗаполнения.ПричиныИзменения.Найти("Организация") <> Неопределено Тогда
		...
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.СпособУчетаНДС");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Счет");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто1");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто2");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто3");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Подразделение");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.СчетУчетаНДС");
	КонецЕсли;
	...
	// Права.ОбъектПрава
	Если ПараметрыЗаполнения.ПричиныИзменения.Найти("Права.ОбъектПрава") <> Неопределено Тогда
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.СпособУчетаНДС");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Счет");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто1");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто2");
	...
	// Права.Счет
	Если ПараметрыЗаполнения.ПричиныИзменения.Найти("Права.Счет") <> Неопределено Тогда
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто1");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто2");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Субконто3");
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Права.Подразделение");
	...
	Если ПараметрыЗаполнения.Свойство("Контейнер") Тогда
		...
		Если ПараметрыЗаполнения.ПричиныИзменения.Найти("ВидОперации") <> Неопределено Тогда
			...
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Объект", "СчетУчетаРасчетовСКомиссионером");
		...
		Если ПараметрыЗаполнения.ПричиныИзменения.Найти("Организация") <> Неопределено Тогда
			...
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "СпособУчетаНДС");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "СпособУчетаНДС");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "ОбъектПрава");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "Счет");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "Субконто1");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "Субконто2");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "Субконто3");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "Подразделение");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "СчетУчетаНДС");
		КонецЕсли;
		...
		Если ПараметрыЗаполнения.ПричиныИзменения.Найти("ОсновныеСредства.ОсновноеСредство") <> Неопределено Тогда
			...
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "СчетУчетаНДС");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "СпособУчетаНДС");
			СчетаУчетаВДокументахКлиентСервер.ДополнитьДанныеЗаполнения(ПараметрыЗаполнения, "Строка", "ЕдиницаУчета");
		КонецЕсли;
