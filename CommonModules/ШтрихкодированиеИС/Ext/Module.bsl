...
Процедура ОбработатьШтрихкодыУпаковокПоДаннымБазы(ПараметрыСканирования, ДанныеПоШтрихкодам, ШтрихкодыМаркируемойПродукции, ТипыУпаковок, ЗаполнитьПоСсылкам) Экспорт
	...
	Если ШтрихкодированиеОбщегоНазначенияИС.ДопустимаТабачнаяПродукция(ПараметрыСканирования)
		...
		СканированиеФормыПроверкиИПодбора = МодульШтрихкодированиеИСМП.ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования);
	...
	Для Каждого НайденнаяУпаковка Из НайденныеУпаковки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанныхКодаМаркировки.ВидПродукции, Истина)
			И ШтрихкодированиеОбщегоНазначенияИС.ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина) Тогда
...
Процедура ЗаполнитьСведенияПоШтриходамДаннымПулаGTIN(ДанныеПоШтрихкодам, ПараметрыСканирования, ДанныеКодовМаркировки = Неопределено) Экспорт
	...
	Если ДопустимаПродукцияИСМП
		...
		СканированиеФормыПроверкиИПодбора = МодульШтрихкодированиеИСМПСлужебный.ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		МодульИнтеграцияИСМПКлиентСерверПовтИсп = ОбщегоНазначенияИСПовтИсп.ОбщийМодуль("ИнтеграцияИСМПКлиентСерверПовтИсп");
	...
	Для Каждого СтрокаДанных Из ДанныеКодовМаркировки Цикл
		...
		Если ОбщегоНазначенияИСПовтИсп.ЭтоПродукцияИСМП(СтрокаДанных.ВидПродукции, Истина)
			...
			И ШтрихкодированиеОбщегоНазначенияИС.ДопустимаПродукцияИСМП(ПараметрыСканирования, Истина)
			И ПараметрыСканирования.Свойство("ТребуетсяПроверкаСредствамиККТ")
...
Функция ТоварныеГруппыВидовПродукции() Экспорт
	...
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.НапиткиССодержаниемМолокаКакаоПрочие, Перечисления.ВидыПродукцииИС.СоковаяПродукция);
	...
	ТоварныеГруппыВидовПродукции.Вставить(Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026, Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность);
...
Функция ОбработатьУточнениеНоменклатуры(ФормаПараметрыСканирования, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	Если Не ДопустимаПродукцияИСМП
		И (ДанныеШтрихкода.ВидПродукции = Перечисления.ВидыПродукцииИС.Пиво
			Или ДанныеШтрихкода.ВидПродукции = Перечисления.ВидыПродукцииИС.ПивоВПотребительскихУпаковках) Тогда
		ДанныеШтрихкода.МаркируемаяПродукция = Ложь;
	КонецЕсли;
