...
Функция ТекстСообщенияОПриближенииКМаксимальноДопустимомуКоличествуРаботающихСотрудников() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизацийБухгалтерскийУчет") Тогда
		Возврат НСтр("ru='Учет больничных, отпусков, исполнительных документов и договоров ГПХ доступен,
...
Функция ТекстСообщенияОНевозможностиСоздаватьДокументыПриОтключеннойНастройке() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизацийБухгалтерскийУчет") Тогда
		Возврат НСтр("ru='Создавать документ ""%1"" можно при включенном режиме учета больничных, отпусков, исполнительных документов и договоров ГПХ.
...
Функция ТекстСообщенияОПревышенииМаксимальноДопустимогоКоличестваРаботающихСотрудников() Экспорт
	...
	Возврат НСтр(
		"ru='Учет больничных, отпусков, исполнительных документов и договоров ГПХ отключен.
...
// Рассчитывается количество дней компенсации отпуска и период оплаты с учетом специфики места внедрения
//
// Параметры:
//		Организация			- СправочникСсылка.Организации
//		Сотрудник			- СправочникСсылка.Сотрудники
//		Период				- Дата, дата на которую рассчитываются права на отпуск
//		СведенияПоОтпуску	- Структура:
//									- ДнейКомпенсацииУдержанияОтпуска - Число
//									- РабочийГодС                     - Дата
//									- РабочийГодПо                    - Дата
//
Процедура РассчитатьКоличествоДнейКомпенсацииУдержанияОтпуска(Организация, Сотрудник, Период, СведенияПоОтпуску) Экспорт
	
	УчетЗарплаты.РассчитатьКоличествоДнейОстаткаОтпуска(Организация, Сотрудник, Период, СведенияПоОтпуску);
	
КонецПроцедуры

// Возвращает текст сообщения, которое будет показано пользователю при попытке сформировать отчет,
// когда отключен режим расчета зарплаты для небольших организаций.
// В текст сообщения подставляются следующие параметры:
//		%1	- ограничение сотрудников.
//
// Возвращаемое значение:
//		Строка
//
Функция ТекстСообщенияОНевозможностиФормироватьОтчетыПриОтключеннойНастройке() Экспорт
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизацийБухгалтерскийУчет") Тогда
		Возврат НСтр("ru='Формировать отчеты по воинскому учету можно при включенном режиме учета больничных, отпусков, исполнительных документов и договоров ГПХ.
						 |Этот режим доступен, когда количество работающих сотрудников в каждой организации не превышает %1.'");
	Иначе
		Возврат НСтр("ru='Формировать отчеты по воинскому учету можно при включенном режиме учета больничных, отпусков, исполнительных документов и договоров ГПХ.
						 |Этот режим доступен, когда количество работающих сотрудников не превышает %1.'");
	КонецЕсли;
