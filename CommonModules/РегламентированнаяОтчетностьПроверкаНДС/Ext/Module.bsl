...
// декларации по НДС. Предназначена для редакций декларации по НДС, соответствующих формам "ФормаОтчета2026Кв1",
// "ФормаОтчета2025Кв1", "ФормаОтчета2023Кв1","ФормаОтчета2022Кв1", "ФормаОтчета2021Кв3", "ФормаОтчета2020Кв4",
...
Процедура ВыявитьОшибкиЗаполненияПрепятствующиеФормированиюЭлектронногоПредставления(Отчет, МакетОформления, НаименованиеДекларации, АдресХранилища) Экспорт
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) <> ВРег("ФормаОтчета2026Кв1")
		И Год(Отчет.ДатаОкончания) >= 2026 И Отчет.Вид = 0 И ТекущаяДатаСеанса() >= '20260329' Тогда
		
		ОписаниеОшибки = "С 29.03.2026 представление первичной декларации по НДС, начиная с отчета за период "
			+ "1 квартал 2026 года, доступно только по форме в редакции приказа ФНС России от 18.12.2025 № ЕД-7-3/1227@.";
		ЗафиксироватьОшибкуТитульногоЛиста(КонтекстВыполнения, ОписаниеОшибки, "Титульный", "НомерКорректировки");
	...
	Иначе
		...
		ПроверитьЗаполнение_Раздел3(КонтекстВыполнения);
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ВыявитьОшибкиЗаполненияПрепятствующиеФормированиюЭлектронногоПредставления_ФормаОтчета2015Кв1", Истина);
	Методы.Вставить("ВыявитьОшибкиЗаполненияПрепятствующиеФормированиюЭлектронногоПредставления", Истина);
КонецПроцедуры
...
Процедура ПроверитьЗаполнение_ТитульныйЛист(КонтекстВыполнения)
	...
	= "01,02,03,04,05,06,07,08,09,10,11,12,21,22,23,24";
	
	Если ПараметрыВыгрузки.ЭтоПБОЮЛ Тогда
		РазрешенныеКодыПериода = РазрешенныеКодыПериода + ",83,87,88,89";
	Иначе
		РазрешенныеКодыПериода = РазрешенныеКодыПериода + ",51,54,55,56,71,72,73,74,75,76,77,78,79,80,81,82";
	КонецЕсли;
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		...
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2019Кв1")
	...
	ИначеЕсли ВозможныеКодыПериода.Найти(ПараметрыВыгрузки.ПериодОтч) = Неопределено Тогда
		ОписаниеОшибки = "На титульном листе указано некорректное значение кода налогового периода.";
		ЗафиксироватьОшибкуТитульногоЛиста(КонтекстВыполнения, ОписаниеОшибки, ИмяРаздела, "Период");
...
Процедура ПроверитьЗаполнение_Раздел1(КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
		
		ЗначениеСтроки041 = Окр(ДанныеРаздела.П000100004103, 0);
		Если ЗначениеСтроки041 > ЗначениеСтроки040 Тогда
			ОписаниеОшибки = "В строке 041 раздела 1 значение не должно превышать сумму в строке 040.";
			ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100004103");
		КонецЕсли;
		
	КонецЕсли;
	
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1")
	...
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел1");
	КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
	ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеРаздела);
...
Процедура ПроверитьЗаполнение_Раздел2(КонтекстВыполнения)
	...
	Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2026Кв1")
		ИЛИ ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2025Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2023Кв1") Тогда
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		КонтекстРаздела = Новый Структура;
		КонтекстРаздела.Вставить("ИмяРаздела", "Раздел2");
		КонтекстРаздела.Вставить("НомерСтраницы", НомерСтраницы);
		КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
		ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеСтраницы);
		КонтекстВыполнения.Удалить("КонтекстРаздела");
...
Процедура ПроверитьЗаполнение_Раздел3(КонтекстВыполнения)
	
	ДанныеРаздела = КонтекстВыполнения.ДанныеОтчета.ПолеТабличногоДокументаРаздел3;
	
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел3");
	КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
	ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеРаздела);
	КонтекстВыполнения.Удалить("КонтекстРаздела");
...
Процедура ПроверитьЗаполнение_Раздел3Приложение1(КонтекстВыполнения)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
		...
		Иначе
			...
			Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2026Кв1") Тогда
				
				Если КодПериода <> "24" И КодПериода <> "56" И КодПериода <> "89" Тогда
					ОписаниеОшибки = "Приложение 1 к разделу 3 заполняется только если на титульном листе указан код налогового периода 24, 56 или 89.";
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310001001", "1");
					Прервать;
				КонецЕсли;
				
			Иначе// "ФормаОтчета2025Кв1" - "ФормаОтчета2017Кв1"
				
				Если КодПериода <> "24" И КодПериода <> "56" Тогда
					ОписаниеОшибки = "Приложение 1 к разделу 3 заполняется только если на титульном листе указан код налогового периода 24 или 56.";
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, "Раздел3_Прил1", "П000310001001", "1");
					Прервать;
				КонецЕсли;
		...
		КонтекстРаздела = Новый Структура;
		КонтекстРаздела.Вставить("ИмяРаздела", "Раздел3_Прил1");
		КонтекстРаздела.Вставить("НомерСтраницы", НомерСтраницы);
		КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
		ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеСтраницы);
		КонтекстВыполнения.Удалить("КонтекстРаздела");
...
Процедура ПроверитьЗаполнение_Раздел3Приложение2(КонтекстВыполнения)
	...
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел3_Прил2");
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(Раздел3Прил2, "П000320000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		КонтекстРаздела.Вставить("НомерСтроки", НомерСтроки);
		КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
		ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, Раздел3Прил2);
...
Процедура ПроверитьЗаполнение_Раздел4(КонтекстВыполнения)
	...
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел4");
	
	КонтекстРаздела.Вставить("ГруппаСтрок", "010-050");
	...
	КонтекстРаздела.Вставить("ГруппаСтрок", "060-080");
	...
	КонтекстРаздела.Вставить("ГруппаСтрок", "090-110");
...
Процедура ПроверитьЗаполнение_Раздел5(КонтекстВыполнения)
	...
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел5");
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела Цикл
			...
			Пока ДанныеСтраницы.Свойство("П000500003003_" + Формат(ИндексСтроки, "ЧГ=")) Цикл
				...
				Если ЗначениеСуммыЗаполнено(ДанныеСтраницы["П000500004003_" + СтрИндексСтроки])
				 ...
				 ИЛИ ЗначениеЗаполнено(ДанныеСтраницы["П000500003003_" + СтрИндексСтроки]) Тогда
					...
					КонтекстРаздела.Вставить("НомерСтроки", ИндексСтроки);
					КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
					ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеСтраницы);
		...
		КонтекстРаздела.Вставить("НомерСтроки", Неопределено);
		КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
		ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеСтраницы);
...
Процедура ПроверитьЗаполнение_Раздел6(КонтекстВыполнения)
	...
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел6");
	
	КонтекстРаздела.Вставить("ГруппаСтрок", "010-040");
	...
	КонтекстРаздела.Вставить("ГруппаСтрок", Неопределено);
	КонтекстРаздела.Вставить("НомерСтроки", Неопределено);
	КонтекстВыполнения.Вставить("КонтекстРаздела", КонтекстРаздела);
	ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеРаздела);
	
	КонтекстРаздела.Вставить("ГруппаСтрок", "070-100");
	...
	КонтекстРаздела.Вставить("ГруппаСтрок", "110-150");
...
Процедура ПроверитьЗаполнение_Раздел7(КонтекстВыполнения)
	...
	КонтекстРаздела = Новый Структура;
	КонтекстРаздела.Вставить("ИмяРаздела", "Раздел7");
...
Процедура ПроверитьРазделыНДС(КонтекстВыполнения)
	...
	Если Окр(Раздел1.П000100003003) > 0 И ДанныеРаздел12 = Неопределено Тогда
	...
	Иначе
		...
		Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2026Кв1")
			...
			Если Окр(Раздел1.П000100003003) <= 0 И ДанныеРаздел12 <> Неопределено Тогда
				ОписаниеОшибки = "Значение в строке 030 раздела 1 должно быть больше 0, если в декларации заполнен раздел 12";
				ЗафиксироватьОшибкуОтчета(КонтекстВыполнения, ОписаниеОшибки, "Раздел1", "П000100003003");
			КонецЕсли;
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			СтрокиСегментаРаздела = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
				Отчет, "Раздел8", НомерПервойСтрокиСекции);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел8(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	ЕстьИсправлениеСчетаФактуры
		= ЗначениеЗаполнено(СтрокаДанных.НомИспрСчФ) ИЛИ ЗначениеЗаполнено(СтрокаДанных.ДатаИспрСчФ);
	...
	ЕстьИсправлениеКорректировочногоСчетаФактуры
		= ЗначениеЗаполнено(СтрокаДанных.НомИспрКСчФ) ИЛИ ЗначениеЗаполнено(СтрокаДанных.ДатаИспрКСчФ);
	...
	ПроверитьСведенияОСчФПриОтгрузкеВСчетРанееПолучОплаты(СтрокаДанных, КонтекстВыполнения);
	...
	ПроверитьСведенияОПрослеживаемомТоваре(СтрокаДанных, КонтекстВыполнения);
	
	ПроверитьРегНомерТаможеннойДекларации(СтрокаДанных, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнение_Раздел9(КонтекстВыполнения, ДанныеРаздела)
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			СтрокиСегментаРаздела = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
				Отчет, "Раздел9", НомерПервойСтрокиСекции);
...
Процедура ПроверитьЗаполнение_Раздел10(КонтекстВыполнения, ДанныеРаздела)
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			СтрокиСегментаРаздела = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
				Отчет, "Раздел10", НомерПервойСтрокиСекции);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел10(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ЗначениеЗаполнено(СтрокаДанных.СвСчФОтПрод) Тогда
		...
		Для Каждого СведенияОПосредническойДеятельности Из СтрокаДанных.СвСчФОтПрод Цикл
			...
			Если ЗначениеЗаполнено(СведенияОПосредническойДеятельности.ОКВ)
				И СтрДлина(СокрЛП(СведенияОПосредническойДеятельности.ОКВ)) <> 3 Тогда
				ЗафиксироватьОшибку812(КонтекстВыполнения, "НеКодВалюты3");
			КонецЕсли;
			...
			ПроверитьСведенияОПрослеживаемомТоваре(СведенияОПосредническойДеятельности, КонтекстВыполнения);
...
Процедура ПроверитьЗаполнение_Раздел11(КонтекстВыполнения, ДанныеРаздела)
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			СтрокиСегментаРаздела = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
				Отчет, "Раздел11", НомерПервойСтрокиСекции);
...
Процедура ПроверитьЗаполнение_Раздел12(КонтекстВыполнения, ДанныеРаздела)
	...
	Если ТипЗнч(ДанныеРаздела) = Тип("Массив") Тогда
		...
		Для Каждого НомерПервойСтрокиСекции Из ДанныеРаздела Цикл
			СтрокиСегментаРаздела = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
				Отчет, "Раздел12", НомерПервойСтрокиСекции);
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел12(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1")
		ИЛИ ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1") Тогда
...
Процедура СформироватьПредставлениеОшибок(КонтекстВыполнения)
	...
	Иначе
		...
		Для Каждого ГруппаОшибокПоТипу Из ВыявленныеТипыОшибок Цикл
			ОшибкиПоТипу = КонтекстВыполнения.ОшибкиРазделов812.НайтиСтроки(
				Новый Структура("ТипОшибки", ГруппаОшибокПоТипу.ТипОшибки));
...
Процедура ПроверитьПоказателиРазделов17НаНеотрицательность(КонтекстВыполнения, ДанныеРаздела)
	
	КонтекстРаздела = КонтекстВыполнения.КонтекстРаздела;
	
	ИмяРаздела = КонтекстРаздела.ИмяРаздела;
	
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
		
		Если ИмяРаздела = "Раздел1" Тогда
			ИменаПоказателей = "П000100003003,П000100004103,П000100005603";
		ИначеЕсли ИмяРаздела = "Раздел2" Тогда
			ИменаПоказателей = "П000200006003,П000200008003,П000200009003,П000200010003";
		ИначеЕсли ИмяРаздела = "Раздел3" Тогда
			ИменаПоказателей = "П000300000303,П000300000305,П000300000503,П000300000505,П000300000703,П000300000705"
				+ ",П000300001003,П000300001005,П000300002003,П000300002005,П000300002103,П000300002105"
				+ ",П000300002203,П000300002205,П000300003003,П000300003005,П000300003103,П000300003105"
				+ ",П000300003203,П000300003205,П000300003303,П000300003305,П000300003403,П000300003405"
				+ ",П000300004003,П000300004005,П000300004103,П000300004105,П000300004203,П000300004205"
				+ ",П000300004303,П000300004305,П000300004403,П000300004405,П000300005003,П000300005005"
				+ ",П000300006003,П000300006005,П000300007003,П000300007005,П000300008005,П000300009005"
				+ ",П000300010005,П000300010503,П000300010505,П000300011005,П000300011505,П000300011805"
				+ ",П000300012005,П000300012505,П000300013005,П000300013505,П000300014005,П000300015005"
				+ ",П000300016005,П000300017005,П000300018005,П000300018505,П000300019005,П000300020005"
				+ ",П000300021005";
		ИначеЕсли ИмяРаздела = "Раздел3_Прил1" Тогда
			ИменаПоказателей = "П000310005001,П000310006001,П000310007003_1,П000310007003_2,П000310007003_3,П000310007003_4"
				+ ",П000310007003_5,П000310007003_6,П000310007003_7,П000310007003_8,П000310007003_9,П000310007003_10"
				+ ",П000310007004_1,П000310007004_2,П000310007004_3,П000310007004_4,П000310007004_5,П000310007004_6"
				+ ",П000310007004_7,П000310007004_8,П000310007004_9,П000310007004_10";
		ИначеЕсли ИмяРаздела = "Раздел3_Прил2" Тогда
			ИменаПоказателей = "П000320000102,П000320000103";
		ИначеЕсли ИмяРаздела = "Раздел4" Тогда
			Если КонтекстРаздела.ГруппаСтрок = "010-050" Тогда
				ИменаПоказателей = "П0004М1002003,П0004М1003003,П0004М1004003,П0004М1005003";
			ИначеЕсли КонтекстРаздела.ГруппаСтрок = "060-080" Тогда
				ИменаПоказателей = "П0004М2007003,П0004М2008003";
			ИначеЕсли КонтекстРаздела.ГруппаСтрок = "090-110" Тогда
				ИменаПоказателей = "П0004М3010003,П0004М3011003";
			КонецЕсли;
		ИначеЕсли ИмяРаздела = "Раздел5" Тогда
			Если КонтекстРаздела.НомерСтроки <> Неопределено Тогда
				ИменаПоказателей = "П000500004003,П000500005003,П000500006003,П000500007003";
			Иначе
				ИменаПоказателей = "П000500008003,П000500009003";
			КонецЕсли;
		ИначеЕсли ИмяРаздела = "Раздел6" Тогда
			Если КонтекстРаздела.ГруппаСтрок = "010-040" Тогда
				ИменаПоказателей = "П0006М1002003,П0006М1003003,П0006М1004003";
			ИначеЕсли КонтекстРаздела.ГруппаСтрок = "070-100" Тогда
				ИменаПоказателей = "П0006М2008003,П0006М2009003,П0006М2010003";
			ИначеЕсли КонтекстРаздела.ГруппаСтрок = "110-150" Тогда
				ИменаПоказателей = "П0006М3012003,П0006М3013003,П0006М3014003,П0006М3015003";
			ИначеЕсли КонтекстРаздела.НомерСтроки = Неопределено Тогда
				ИменаПоказателей = "П000600005003,П000600006003";
			КонецЕсли;
		ИначеЕсли ИмяРаздела = "Раздел7" Тогда
			Если КонтекстРаздела.НомерСтроки <> Неопределено Тогда
				ИменаПоказателей = "П000700000102,П000700000103,П000700000104";
			Иначе
				ИменаПоказателей = "П000700001001";
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого ИмяПоказателя Из ИменаПоказателей Цикл
			...
			Если ИмяРаздела = "Раздел3_Прил2" ИЛИ ИмяРаздела = "Раздел4"
				ИЛИ (ИмяРаздела = "Раздел5" И КонтекстРаздела.НомерСтроки <> Неопределено)
				ИЛИ (ИмяРаздела = "Раздел6" И КонтекстРаздела.НомерСтроки <> Неопределено)
				ИЛИ (ИмяРаздела = "Раздел7" И КонтекстРаздела.НомерСтроки <> Неопределено) Тогда
				ИмяПоказателя = ИмяПоказателя + "_" + Формат(КонтекстРаздела.НомерСтроки, "ЧГ=");
			КонецЕсли;
			
			Если ДанныеРаздела.Свойство(ИмяПоказателя) И ТипЗнч(ДанныеРаздела[ИмяПоказателя]) = Тип("Число") Тогда
				
				Если ИмяРаздела = "Раздел3_Прил1" И СтрНачинаетсяС(ИмяПоказателя, "П000310007003") Тогда
					ЗначПоказателя = Окр(ДанныеРаздела[ИмяПоказателя], 1);
				Иначе
					ЗначПоказателя = Окр(ДанныеРаздела[ИмяПоказателя]);
				КонецЕсли;
				
				Если ЗначПоказателя < 0 Тогда
					Если ИмяРаздела = "Раздел1" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='В строке %1 раздела %2 значение не должно быть отрицательным.'"),
							Сред(ИмяПоказателя, 9, 3), Прав(ИмяРаздела, 1));
					ИначеЕсли ИмяРаздела = "Раздел2" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='В разделе %1 на странице %2 в строке %3 значение не должно быть отрицательным.'"),
							Прав(ИмяРаздела, 1), КонтекстРаздела.НомерСтраницы, Сред(ИмяПоказателя, 9, 3));
					ИначеЕсли ИмяРаздела = "Раздел3" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='В строке %1 по графе %2 раздела %3 значение не должно быть отрицательным.'"),
							Сред(ИмяПоказателя, 9, 3), Сред(ИмяПоказателя, 13, 1), Прав(ИмяРаздела, 1));
					ИначеЕсли ИмяРаздела = "Раздел3_Прил1" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='На странице %1 приложения 1 к разделу 3 в строке %2 значение не должно быть отрицательным.'"),
							КонтекстРаздела.НомерСтраницы, Сред(ИмяПоказателя, 9, 3)
							+ ?(СтрНачинаетсяС(ИмяПоказателя, "П0003100070"), " по графе " + Сред(ИмяПоказателя, 13, 1), ""));
					ИначеЕсли ИмяРаздела = "Раздел3_Прил2" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='В приложении 2 к разделу 3 в строке %1 по графе %2 значение не должно быть отрицательным.'"),
							Формат(КонтекстРаздела.НомерСтроки, "ЧГ="), Сред(ИмяПоказателя, 13, 1));
					ИначеЕсли ИмяРаздела = "Раздел4" Тогда
						ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
							"ru='В разделе 4 в строке %1 группы строк %2 значение в строке %3 не должно быть отрицательным.'"),
							Формат(КонтекстРаздела.НомерСтроки, "ЧГ="), КонтекстРаздела.ГруппаСтрок, Сред(ИмяПоказателя, 9, 3));
					ИначеЕсли ИмяРаздела = "Раздел5" Тогда
						Если КонтекстРаздела.НомерСтроки <> Неопределено Тогда
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 5 на странице %1 в строке %2 значение в строке %3 не должно быть отрицательным.'"),
								КонтекстРаздела.НомерСтраницы, Формат(КонтекстРаздела.НомерСтроки, "ЧГ="), Сред(ИмяПоказателя, 9, 3));
						Иначе
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 5 на странице %1 значение в строке %2 не должно быть отрицательным.'"),
								КонтекстРаздела.НомерСтраницы, Сред(ИмяПоказателя, 9, 3));
						КонецЕсли;
					ИначеЕсли ИмяРаздела = "Раздел6" Тогда
						Если КонтекстРаздела.НомерСтроки <> Неопределено Тогда
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 6 в строке %1 группы строк %2 значение в строке %3 не должно быть отрицательным.'"),
								Формат(КонтекстРаздела.НомерСтроки, "ЧГ="), КонтекстРаздела.ГруппаСтрок, Сред(ИмяПоказателя, 9, 3));
						Иначе
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 6 в строке %1 значение не должно быть отрицательным.'"), Сред(ИмяПоказателя, 9, 3));
						КонецЕсли;
					ИначеЕсли ИмяРаздела = "Раздел7" Тогда
						Если КонтекстРаздела.НомерСтроки <> Неопределено Тогда
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 7 в строке %1 по графе %2 значение не должно быть отрицательным.'"),
								Формат(КонтекстРаздела.НомерСтроки, "ЧГ="), Сред(ИмяПоказателя, 13, 1));
						Иначе
							ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
								"ru='В разделе 7 в строке %1 значение не должно быть отрицательным.'"), Сред(ИмяПоказателя, 9, 3));
						КонецЕсли;
					КонецЕсли;
					ЗафиксироватьОшибкуРазделов17(КонтекстВыполнения, ОписаниеОшибки, ИмяРаздела, ИмяПоказателя);
				КонецЕсли;
...
Процедура ПроверитьСведенияОСчФПриОтгрузкеВСчетРанееПолучОплаты(СтрокаДанных, КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
		...
		Для Каждого Док Из СтрокаДанных.СвЧастСчФ Цикл
			ПроверяемаяДата = Док.ДатаЧастСчФ;
			
			Если НЕ ЗначениеЗаполнено(Док.НомЧастСчФ)
			   И НЕ ЗначениеЗаполнено(ПроверяемаяДата) Тогда
				ОшибкиСпискаДокументов.Вставить("НеУказаныНомерДата", Истина);
			ИначеЕсли НЕ ЗначениеЗаполнено(Док.НомЧастСчФ) Тогда
				ОшибкиСпискаДокументов.Вставить("НеУказанНомер", Истина);
			ИначеЕсли НЕ ЗначениеЗаполнено(ПроверяемаяДата) Тогда
				ОшибкиСпискаДокументов.Вставить("НеУказанаДата", Истина);
			КонецЕсли;
		...
		Если ОшибкиСпискаДокументов.НеУказаныНомерДата Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказаныНомерДатаЧастСчФ");
		КонецЕсли;
		
		Если ОшибкиСпискаДокументов.НеУказанНомер Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанНомерЧастСчФ");
		КонецЕсли;
		
		Если ОшибкиСпискаДокументов.НеУказанаДата Тогда
			ЗафиксироватьОшибку812(КонтекстВыполнения, "НеУказанаДатаЧастСчФ");
		КонецЕсли;
...
Процедура ПроверитьСведенияОПрослеживаемомТоваре(СтрокаДанных, КонтекстВыполнения)
...
Процедура ПроверитьРегНомерТаможеннойДекларации(СтрокаДанных, КонтекстВыполнения)
...
Функция КодыОпераций(ИмяРаздела, КонтекстВыполнения)
	...
	Если ИмяРаздела = "Раздел8" ИЛИ ИмяРаздела = "Раздел8_Прил1" Тогда
		...
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
			...
			Возврат "01,02,06,13,15,16,17,18,19,20,22,23,24,25,26,27,28,32,34,36,41,42,43,44,45,46,54,55,57,58,59,60,61";
			
		ИначеЕсли ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2025Кв1") Тогда
	...
	ИначеЕсли ИмяРаздела = "Раздел9" ИЛИ ИмяРаздела = "Раздел9_Прил1" Тогда
		...
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
			...
			Возврат "01,02,06,10,13,14,15,18,21,26,27,28,29,30,31,33,34,35,37,38,39,40,41,42,43,44,47,52,53,56,57,58,59,60,61";
			
		Иначе// "ФормаОтчета2025Кв1" и ранее
	...
	ИначеЕсли ИмяРаздела = "Раздел10" ИЛИ ИмяРаздела = "Раздел11" Тогда
		...
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2026Кв1") Тогда
			...
			Возврат "01,02,13,15,18,19,20,27,28,29,30,33,34,37,38,39,40,56,57";
...
Функция СписокСекцийРазделаИзРегистра(Отчет, ИмяРаздела)
	...
	Если ВсегоСтраниц > 0 Тогда
		...
		ДанныеСегмента = РегламентированнаяОтчетность.СегментДанныхРазделаДекларацииНДС(
			Отчет, ИмяРаздела, НомерПервойСтрокиПоследнегоСегмента);
...
Процедура ОпределитьОшибкиИННКПП(СведенияОКонтрагенте, КонтекстВыполнения)
	...
	ПредставлениеКонтрагента = СведенияОКонтрагенте.КонтрагентНаименование + ?(ЭтоФизЛицо,
		", ИНН " + СведенияОКонтрагенте.ИННФЛ, ", ИНН " + СведенияОКонтрагенте.ИННЮЛ + ", КПП " + СведенияОКонтрагенте.КПП);
...
Функция КонтрольноеЧислоИНН(ИНН, ВесаРазрядов = Неопределено)
	...
	ИначеЕсли СтрДлина(СокрЛП(ИНН)) = 12 Тогда
		...
		КонтрольноеЧисло = СтрокаЧГ0(КонтрольнаяСумма11) + СтрокаЧГ0(КонтрольнаяСумма12);
