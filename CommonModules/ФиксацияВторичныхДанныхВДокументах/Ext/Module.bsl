...
Функция ИзменитьЗначениеРеквизита(ТекущееЗначение, НовоеЗначение) Экспорт
	Если Не ЗначениеЗаполнено(ТекущееЗначение) И Не ЗначениеЗаполнено(НовоеЗначение) Тогда
		Возврат Ложь; // Оба значения - пустые.
	КонецЕсли;
	СтароеЗначение = ТекущееЗначение;
	ТекущееЗначение = НовоеЗначение;
	Если СтароеЗначение <> ТекущееЗначение Тогда
		Возврат Истина; // ОбъектМодифицирован = Истина.
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции

Функция ИзменитьЗначенияРеквизитов(Объект, НовыеЗначения) Экспорт
	ОбъектМодифицирован = Ложь;
	Для Каждого КлючИЗначение Из НовыеЗначения Цикл
		Если ИзменитьЗначениеРеквизита(Объект[КлючИЗначение.Ключ], КлючИЗначение.Значение) Тогда
			ОбъектМодифицирован = Истина;
		КонецЕсли;
	КонецЦикла;
	Возврат ОбъектМодифицирован;
...
Функция ЗагрузитьТаблицу(ТабличнаяЧасть, ТаблицаЗначений) Экспорт
	ЗначениеДоИзменения = ТабличнаяЧасть.Выгрузить();
	ТабличнаяЧасть.Загрузить(ТаблицаЗначений);
	ЗначениеПослеизменения = ТабличнаяЧасть.Выгрузить();
	Возврат Не ОбщегоНазначенияБЗК.ЗначенияСовпадают(ЗначениеДоИзменения, ЗначениеПослеизменения);
КонецФункции
