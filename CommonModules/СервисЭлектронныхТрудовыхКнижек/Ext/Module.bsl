...
Процедура ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	...
	ОписаниеОбъекта.ИмяОсновногоМакетаДляПечати        = "ПФ_MXL_ФормаЕФС_1";
...
Процедура ПолучитьНазванияРегламентированныхОтчетовСервисаЭТК(СписокРегламентированныхОтчетов) Экспорт
	...
	СписокРегламентированныхОтчетов.Добавить(НаименованиеВидаРегламентированногоОтчета(), "");
	
КонецПроцедуры

Функция НаименованиеВидаРегламентированногоОтчета()
	
	Возврат "Сведения о трудовой деятельности, ЕФС-1 подраздел 1.1";
...
//  Команда.Картинка = БиблиотекаКартинок.ПолеВводаОткрыть;
...
Процедура ФормаРегламентированнойОтчетности_ПриСозданииНаСервере(Форма) Экспорт
	...
	Если НЕ Форма.ЭлектронныйДокументооборотДоступен Тогда
		...
		ТекстБаннера1.Заголовок                         = НСтр("ru=' Подготовьте отчет ЕФС-1 подр. 1.1'");
	...
	ВидОтчета = НаименованиеВидаРегламентированногоОтчета();
	Форма.ВидОтчета = ВидОтчета;
	МассивВидовОтчетов = Новый Массив;
	МассивВидовОтчетов.Добавить(ВидОтчета);
	МассивВидовОтчетов.Добавить("Сведения о трудовой деятельности работников, СЗВ-ТД");
...
Процедура СведенияОТрудовойДеятельностиРаботникаСТД_Р_ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Для Каждого ПараметрВыбора ИЗ Элементы.Сотрудник.СвязиПараметровВыбора Цикл
		Если ПараметрВыбора.Имя = "Отбор.Организация"
			Или ПараметрВыбора.Имя = "Отбор.ГоловнаяОрганизация" Тогда
...
Процедура УстановитьРежимКомандногоИнтерфейса() Экспорт
	...
	ОбменСКонтрагентами.УстановитьИспользованиеЭДО(Ложь);
...
Процедура УстановитьОтборыПоОтчетам(Форма)
	...
	СписокОтбораВидОтчета = Форма.Элементы.ОтборВидОтчета.СписокВыбора.Скопировать();
