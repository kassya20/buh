...
//  ТаблицаЗначений - см. НовыйВыручкаПоВидамДеятельности()
//
Функция ВыручкаИППоВидамДеятельности(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо 
		...
		Если ТипЗнч(ОсновнойВидДеятельности) = Тип("СправочникСсылка.ВидыДеятельностиПредпринимателей")
			...
			Если ЗначениеЗаполнено(ОКВЭДОсновногоВидаДеятельностиНДФЛ) Тогда
				НаименованиеОсновногоВидаДеятельностиНДФЛ = ОрганизацииФормыКлиентСервер.ПрочитатьОКВЭД2(ОКВЭДОсновногоВидаДеятельностиНДФЛ);
			КонецЕсли;
			
			Если Не ЗначениеЗаполнено(НаименованиеОсновногоВидаДеятельностиНДФЛ) Тогда
		...
		Если УчетнаяПолитика.ВестиУчетПоВидамДеятельностиИП(Организация, КонецПериода) Тогда
			...
			Пока ВыборкаВидов.Следующий() Цикл 
				...
				Если ВыборкаОбщие.Следующий() Тогда
					...
					Если ЗначениеЗаполнено(ВыборкаВидов.КодОКВЭД) Тогда
						ВыручкаПоВидуДеятельности.Аналитика = ОрганизацииФормыКлиентСервер.ПрочитатьОКВЭД2(ВыборкаВидов.КодОКВЭД);
					КонецЕсли;
					
					Если Не ЗначениеЗаполнено(ВыручкаПоВидуДеятельности.Аналитика) Тогда
	...
	ВыручкаПоВидуДеятельности           = ВыручкаПоВидамДеятельности.Добавить();
	ВыручкаПоВидуДеятельности.Выручка   = УчетАУСН.СуммаДоходов(Организация, НачалоПериодаИлиРегистрации, КонецПериода);
	ВыручкаПоВидуДеятельности.Аналитика = НаименованиеОсновногоВидаДеятельности;
	ВыручкаПоВидуДеятельности.КодОКВЭД  = ОКВЭДОсновногоВидаДеятельности;
	...
	ПатентыИП = УчетПСН.ВидыДеятельностиПоПатентуОрганизации(Организация, КонецПериода);
