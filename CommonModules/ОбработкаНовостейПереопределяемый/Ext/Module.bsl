			...
			Если Лев(ИмяКлассов, 2) = "FO" Тогда
				...
				Для ИндексКласса = 0 По МассивКлассов.Количество() - 1 Цикл
					...
					ИначеЕсли ИмяФО = "ЭтоБизнесСтарт"
						...
						СкрыватьЗадачу = Истина;
					ИначеЕсли ИмяФО = "ЭтоНеБизнесСтарт"
						И ОбщегоНазначения.РазделениеВключено()
						И ОбщегоНазначенияБП.ЭтоВитринаБизнесСтарта() Тогда
						СкрыватьЗадачу = Истина;
...
Процедура ДополнительноОбработатьНовостьПослеПолученияПослеЗаписи(НовостьСсылка) Экспорт
	...
	Если ПродуктыОснования.Найти(ТекущийОтраслевойПродукт) = Неопределено Тогда
		...
		РеквизитыНовостей = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НовостьСсылка, "ЛентаНовостей, УИННовости");
		Если ИнтернетПоддержкаПользователейБП.ЭтоЛентаНовостейОбновления(РеквизитыНовостей.ЛентаНовостей) Тогда
			...
			Запрос.УстановитьПараметр("ЛентаНовостей", РеквизитыНовостей.ЛентаНовостей);
			Запрос.УстановитьПараметр("УИННовости", РеквизитыНовостей.УИННовости);
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					Запись = РегистрыСведений.КатегорииНовостейИнтервалыВерсийНеобработанные.СоздатьМенеджерЗаписи();
						...
						Запись.ЛентаНовостей           = РеквизитыНовостей.ЛентаНовостей;
						Запись.УИННовости              = РеквизитыНовостей.УИННовости;
...
Процедура ДополнительноОбработатьФормуНовостиПриСозданииНаСервере(
	...
	Если ИнтернетПоддержкаПользователейБП.ЭтоЛентаНовостейОбновления(Объект.ЛентаНовостей) Тогда
...
Процедура ЗначениеПредопределеннойКатегории(Категория, Значение) Экспорт
	...
	Если ВРег(КатегорияКод) = ВРег("ProductName") Тогда // В плане видов характеристик "КатегорииНовостей" такого элемента нет
		...
		ИначеЕсли ВРег(Метаданные.Имя) = ВРег("БухгалтерияПредприятияУчебная") Тогда
			Значение = "AccountingEduc";
...
Процедура РассчитатьОтборПоКатегории(
	...
	Если ЗначениеКатегорииНовостей = "БП.ПрименяетсяПовышеннаяСтавкаНалогаУСН"
		Или ЗначениеКатегорииНовостей = "БП.ПрименяетсяУСНСНДС" Тогда
		...
		Если ЗначениеКатегорииНовостей = "БП.ПрименяетсяПовышеннаяСтавкаНалогаУСН" Тогда
			ЕстьПереходНаПовышеннуюСтавку = УчетУСН.ЕстьПереходНаПовышеннуюСтавкуНалогаСПонижениемНалоговойБазы();
		ИначеЕсли ЗначениеКатегорииНовостей = "БП.ПрименяетсяУСНСНДС" Тогда
			ЕстьОрганизацииНаУСНСНДС = УчетУСН.ЕстьОрганизацииНаУСНСНДС();
		КонецЕсли;
		
		Если (ВРег(КодКатегорииНовостей) = ВРег("FnOptionDataAreaOn")) Тогда
			СтандартнаяОбработка = Ложь;
			Если ЗначениеКатегорииНовостей = "БП.ПрименяетсяПовышеннаяСтавкаНалогаУСН" Тогда
				РезультатРасчета = ЕстьПереходНаПовышеннуюСтавку;
			ИначеЕсли ЗначениеКатегорииНовостей = "БП.ПрименяетсяУСНСНДС" Тогда
				РезультатРасчета = ЕстьОрганизацииНаУСНСНДС;
			КонецЕсли;
		ИначеЕсли (ВРег(КодКатегорииНовостей) = ВРег("FnOptionDataAreaOff")) Тогда
			СтандартнаяОбработка = Ложь;
			Если ЗначениеКатегорииНовостей = "БП.ПрименяетсяПовышеннаяСтавкаНалогаУСН" Тогда
				РезультатРасчета = Не ЕстьПереходНаПовышеннуюСтавку;
			ИначеЕсли ЗначениеКатегорииНовостей = "БП.ПрименяетсяУСНСНДС" Тогда
				РезультатРасчета = Не ЕстьОрганизацииНаУСНСНДС;
			КонецЕсли;
		КонецЕсли;
