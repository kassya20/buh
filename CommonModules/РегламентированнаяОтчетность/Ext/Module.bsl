...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.Минпромторг Тогда
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СформироватьСпискиВыбораВФоне", Истина);
	Методы.Вставить("Автозаполнение_РеестрПоНДС_Приложение5", Истина);
	Методы.Вставить("Автозаполнение_РеестрПоНДС_Приложение1", Истина);
	Методы.Вставить("Автозаполнение_ПрослеживаемыеТоварыОтчетОбОперациях", Истина);
	Методы.Вставить("ОтключитьВнешнийМодульДокументооборотаСФНС");
	Методы.Вставить("Автозаполнение_ОбученныеПоОхранеТрудаЛица", Истина);
	Методы.Вставить("ЗаполнитьОтчетВФоне_Уведомления", Истина);
	Методы.Вставить("ОбновитьШаблоныЧерезВебСервисНаСервереВФоне", Истина);
	Методы.Вставить("ОбновитьОтправкиПослеЗаменыПредопределенныхЗначенийДляФСРАР");
	Методы.Вставить("ОбновитьРеквизитВидОбменаСКонтролирующимиОрганамиСправочникаОрганизации");
	Методы.Вставить("ВыполнитьПроверку", Истина);
	Методы.Вставить("ВыполнитьОбновлениеИнформационнойБазы");
	Методы.Вставить("ЗаполнитьОтчетВФоне", Истина);
	Методы.Вставить("ПереносДанныхЭДОПриОбновлении10158");
	Методы.Вставить("ОбновитьШаблоныСпискомЧерезВебСервисНаСервереВФоне", Истина);
	Методы.Вставить("ЗаменитьСсылкиРазделенныйСпрУдалитьРеглОтчетыНаНеРазделенныйСпрРеглОтчеты");
	Методы.Вставить("ВыполнитьДобавлениеВРегистр", Истина);
	Методы.Вставить("Автозаполнение_РеестрПоНДС_КНД1155221", Истина);
	Методы.Вставить("ПроверитьВозможностьВыгрузкиВФоне", Истина);
	Методы.Вставить("ПроверитьКонтрольныеСоотношенияДлительнаяОперация", Истина);
КонецПроцедуры
...
// Возвращает все строки дерева значений на всех уровнях подчиненности для итеративной обработки.
// 
// Параметры:
//  ИсходноеДеревоЗначений - ДеревоЗначений
// 
// Возвращаемое значение:
//  Массив Из СтрокаДереваЗначений
// 
Функция ВсеСтрокиДереваЗначений(ИсходноеДеревоЗначений) Экспорт
	
	Строки = Новый Массив();
	
	ОчередьОбхода = Новый Массив();
	ОчередьОбхода.Добавить(ИсходноеДеревоЗначений);
	
	Пока ОчередьОбхода.Количество() > 0 Цикл
		УзелДерева = ОчередьОбхода[0];
		ОчередьОбхода.Удалить(0);
		Для Каждого СтрокаДерева Из УзелДерева.Строки Цикл
			Строки.Добавить(СтрокаДерева);
			ОчередьОбхода.Добавить(СтрокаДерева);
		КонецЦикла;
	КонецЦикла;
...
Функция ВидКонтролирующегоОргана(Знач ИмяОтчета) Экспорт
	...
	Иначе
		...
		ИначеЕсли СтрНайти(ИмяОтчета, "ОкрСреда") > 0
			...
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.РПН;
		ИначеЕсли СтрНайти(ИмяОтчета, "Минпромторг") > 0 ИЛИ СтрНайти(ИмяОтчета, "Отчет1604") > 0
			ИЛИ СтрНайти(ИмяОтчета, "Форма1604") > 0 ИЛИ СтрНайти(ИмяОтчета, "ОтчетФЭО") > 0
			ИЛИ СтрНайти(ИмяОтчета, "ФормаФЭО") > 0 Тогда
			ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.Минпромторг;
...
Функция ВывестиПоказательМинпромторгВXML(Узел, ЗначениеПоказателя, ФорматПредставленияНуля = "") Экспорт 
...
Функция ПолучитьДанныеИнициализацииПоследнегоОтчета(Организация) Экспорт 
	Результат = Новый Структура();
	Результат.Вставить("IDOrg");
	Результат.Вставить("NaimenovanieSubektaDeyatelnostiVSferePromyshlennostiZnachenie");
	Результат.Вставить("NaimenovanieSubektaDeyatelnostiVSferePromyshlennostiPrimechanie");
	Результат.Вставить("OGRNZnachenie");
	Результат.Вставить("OGRNPrimechanie");
	Результат.Вставить("INNZnachenie");
	Результат.Вставить("INNPrimechanie");
	Результат.Вставить("AdresOficialnogoSajtaZnachenie");
	Результат.Вставить("AdresOficialnogoSajtaiPrimechanie");
	Результат.Вставить("NaimenovanieGolovnojOrganizaciiZnachenie");
	Результат.Вставить("NaimenovanieGolovnojOrganizaciiPrimechanie");
	Результат.Вставить("FakticheskijAdresZnachenie");
	Результат.Вставить("FakticheskijAdresPrimechanie");
	Результат.Вставить("OtraslZnachenie");
	Результат.Вставить("OtraslID");
	Результат.Вставить("OtraslPrimechanie");
	Результат.Вставить("Familiya");
	Результат.Вставить("Imya");
	Результат.Вставить("Otchestvo");
	...
	|	И РегламентированныйОтчет.ИсточникОтчета В(&ИсточникОтчета)
	|	И НЕ РегламентированныйОтчет.ПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаПодписи УБЫВ";
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ИсточникОтчета", РегламентированнаяОтчетностьПовтИсп.ПолучитьОтборДляМинпромторга());
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда 
		ДанныеОтчета = Выборка.ДанныеОтчета.Получить();
		
		Если Не ДанныеОтчета.Свойство("ПоказателиОтчета")
			Или ТипЗнч(ДанныеОтчета.ПоказателиОтчета) <> Тип("Структура") Тогда 
			
			Возврат Результат;
		КонецЕсли;
		
		Если ДанныеОтчета.ПоказателиОтчета.Свойство("ПолеТабличногоДокументаТитульный")
			И ТипЗнч(ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаТитульный) = Тип("Структура") Тогда 
			
			ЗаполнитьЗначенияСвойств(Результат, ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаТитульный);
			Возврат Результат;
		КонецЕсли;
		Если ДанныеОтчета.ПоказателиОтчета.Свойство("ПолеТабличногоДокументаФормаОтчета")
			И ТипЗнч(ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаФормаОтчета) = Тип("Структура") Тогда 
			
			ЗаполнитьЗначенияСвойств(Результат, ДанныеОтчета.ПоказателиОтчета.ПолеТабличногоДокументаФормаОтчета);
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	Возврат Результат;
