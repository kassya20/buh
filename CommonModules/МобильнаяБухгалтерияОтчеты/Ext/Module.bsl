...
Функция СформироватьОтчет(СтруктураПараметров, СообщениеОбОшибке) Экспорт
	...
	ИначеЕсли ИмяОтчета = "КнигаУчетаДоходовПатент" Тогда
		...
		|	ОперацияСПатентом.ДатаНачала КАК НачалоПериода,
		|	КОНЕЦПЕРИОДА(ОперацияСПатентом.ДатаОкончания, ДЕНЬ) КАК КонецПериода
		|ИЗ
		|	Документ.ОперацияСПатентом КАК ОперацияСПатентом
		|ГДЕ
		|	ОперацияСПатентом.Организация = &Организация
		|	И ОперацияСПатентом.Проведен
		|	И НАЧАЛОПЕРИОДА(ОперацияСПатентом.ДатаНачала, ГОД) = &Период";
		...
		Пока ДанныеПатента.Следующий() Цикл
			...
			Для Каждого ПечатнаяФорма Из Отчет.СписокСформированныхЛистов Цикл
				ПечатныеФормы.Добавить(ПечатнаяФорма.Значение);
			КонецЦикла;
	...
	ИначеЕсли ИмяОтчета = "ОборотноСальдоваяВедомостьПоСчету"
		...
		ВходящиеПараметры.Вставить("ВидПериода",          0);
...
Функция ПараметрыОСВ(ВходящиеПараметры)
	...
	ПараметрыОтчета.Вставить("ВидПериода"                       , 0);
