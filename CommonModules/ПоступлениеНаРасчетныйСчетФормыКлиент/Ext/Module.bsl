...
Функция ТребуетсяВызовСервераПриИзмененииСчетаКонтрагента(Форма) Экспорт
	
	Возврат Форма.ПрименяетсяАУСН
		И Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя");
...
Процедура ПослеЗаписи(Форма, ПараметрыЗаписи) Экспорт
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал") Тогда 
		Оповестить("Запись_РасчетыСУчредителями");
	КонецЕсли;
...
Процедура ОбработкаНавигационнойСсылки(Форма, НавигационнаяСсылка, СтандартнаяОбработка) Экспорт
	...
	ИнтеграцияАУСНКлиент.ОбработкаНавигационнойСсылки(Форма, НавигационнаяСсылка, СтандартнаяОбработка);
...
Процедура ОбработкаОповещения(Форма, ИмяСобытия, Параметр, Источник, ОбновитьФорму) Экспорт
	
	Объект   = Форма.Объект;
	Элементы = Форма.Элементы;
	Если ИмяСобытия = "Запись_ВидОплаты" И Не Объект.БезЗакрывающихДокументов
		...
		Если Не ЗначениеЗаполнено(Источник.Объект.Ссылка)
			Или ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Форма.СвойстваПлатежа, "ВидОплаты")
			<> Источник.Объект.Ссылка Тогда
			Возврат;
		КонецЕсли;
		
		Если Форма.СвойстваПлатежа.Свойство("ВидОплатыРеквизиты")
			И Форма.СвойстваПлатежа.ВидОплатыРеквизиты <> Неопределено Тогда
			Форма.СвойстваПлатежа.ВидОплатыРеквизиты.Вставить("УчитыватьНДСПоКомиссии", Источник.Объект.УчитыватьНДСПоКомиссии);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Изменения) Тогда
			...
			Если ПараметрыВыбораПолейСубконто.Объект <> Неопределено Тогда
				ПараметрыУстановки = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПараметрыУстановкиСвойствСубконто(
					Форма,
...
Процедура ОтражениеВУСНПриИзменении(Форма, Элемент) Экспорт
	
	Объект = Форма.Объект;
	
	Если Объект.ОтражениеВУСН = ПредопределенноеЗначение("Перечисление.ОтражениеВУСН.Принимаются") Тогда
		
		Если Форма.ОплатаВВалюте Тогда
			СуммаДохода = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(
		...
		Иначе
			СуммаДохода = Объект.СуммаДокумента;
		КонецЕсли;
		
		Объект.Графа5_УСН = СуммаДохода;
		Объект.НДС_УСН    = 0;
...
Процедура РасшифровкаПлатежаПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если НоваяСтрока И Не Копирование Тогда
		ЗаполнитьЗначенияСвойств(СтрокаПлатеж, Форма.СвойстваПлатежа, , "СуммаПлатежа, СуммаУслуг, СуммаВозврата");
...
Процедура РасшифровкаПлатежаПриИзменении(Форма, Элемент) Экспорт
	...
	ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ВидимостьДоступностьСуммыДокумента(Форма);
...
Процедура РасшифровкаПлатежаДоговорКонтрагентаСоздание(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Объект = Форма.Объект;
	ТекущиеДанные = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма);
...
Процедура РасшифровкаПлатежаДоговорКонтрагентаАвтоПодбор(Форма, Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка) Экспорт
	...
	РаботаСДоговорамиКонтрагентовБПКлиент.ЗаполнитьСписокВыбора(
		Элемент, Текст, Форма.ПредлагатьНовыйДоговор, СтандартнаяОбработка, Форма.ПредлагатьНовыйДоговорСНумерацией);
...
Процедура СделкаНачалоВыбора(Форма, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	ПараметрыОбъекта.Вставить("ТипыДокументов",
		ПоступлениеНаРасчетныйСчетФормыВызовСервера.ОписаниеТипаРеквизитаСделка());
	Если ЗначениеЗаполнено(СчетУчета) Тогда
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам") Тогда
			ПараметрыОбъекта.Вставить("РежимОтбораДокументов", ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоОборотам"));
		Иначе
...
Процедура РасшифровкаПлатежаОтражениеАвансаПредставлениеПриИзменении(Форма, Элемент) Экспорт
	...
	Если СтрокаПлатеж.ОтражениеАвансаПредставление <> "" Тогда
		БанкИКассаФормыКлиент.ОтражениеДоходаПриИзменении(Форма,
			СтрокаПлатеж.ПорядокОтраженияАванса,
			СтрокаПлатеж.ОтражениеАвансаПредставление,
			Форма.СвойстваПлатежа.ПорядокОтраженияАванса);
...
Процедура РасшифровкаПлатежаПокупкаВалютыПриНачалеРедактирования(Форма, Элемент, НоваяСтрока, Копирование) Экспорт
	...
	Если НоваяСтрока И НЕ Копирование Тогда
		...
		СписокВидовДоговоров = ПоступлениеНаРасчетныйСчетФормыВызовСервера.ВидыДоговоровДокумента(Объект.ВидОперации);
...
Процедура РасшифровкаПлатежаПлатежныеКартыПриОкончанииРедактирования(Форма, Элемент, НоваяСтрока, ОтменаРедактирования) Экспорт
	...
	Если НоваяСтрока И ОтменаРедактирования Тогда
		...
		ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПлатежныеКартыОбработатьИзменениеСуммыВозврата(Форма);
	КонецЕсли;
...
Процедура ПлатежныеКартыСуммаВозвратаПриИзменении(Форма, Элемент, ЕстьТабличнаяЧасть = Истина) Экспорт
	
	ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПлатежныеКартыОбработатьИзменениеСуммыВозврата(Форма, ЕстьТабличнаяЧасть);
...
Процедура РасшифровкаПлатежаПокупкаВалютыКурсВзаиморасчетовНачалоВыбораЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтруктураКурсаИКратности) = Тип("Структура") Тогда
		СтрокаПлатеж.КурсВзаиморасчетов      = СтруктураКурсаИКратности.КурсВалюты;
		СтрокаПлатеж.КратностьВзаиморасчетов = СтруктураКурсаИКратности.КратностьВалюты;
	КонецЕсли;
