...
Процедура НачатьПроверкуКодовМаркировкиСредствамиККТ(ПараметрыНачалаПроверки) Экспорт
	...
	Для Каждого ЭлементПроверки Из ПараметрыПроверки.ЭлементыПроверки Цикл
		...
		Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияИСМП(ЭлементПроверки.ВидПродукции, Истина)
			...
			КонецЕсли;
			Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(ЭлементПроверки.ВидПродукции) Тогда
	...
	Для Каждого ЭлементПроверки Из ПараметрыПроверки.ЭлементыПроверки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(ЭлементПроверки.ЧастичноеВыбытиеКоличество) Тогда
			...
			ИначеЕсли ЭлементПроверки.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Пиво")
				...
				Или ЭлементПроверки.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.БезалкогольноеПиво")
				Или ЭлементПроверки.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МоторныеМасла") Тогда
...
Процедура ПроверкаМРЦПоЦенеПродажи(ПараметрыПроверки)
	...
	Для Каждого ЭлементПроверки Из ЭлементыПроверки Цикл
		...
		Если Не ЭлементПроверки.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.НикотиносодержащаяПродукция") Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ДанныеОтвета.ДанныеИдентификаторов Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеПоКМ, "КонтролиРазрешительногоРежима") Тогда
			...
			ИначеЕсли Не ДействуетАварийныйРежим Тогда
				...
				Если ДанныеПоКМ.ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.НикотиносодержащаяПродукция") Тогда
...
Процедура ДополнительныеПроверкиПередЗавершениемОбработкиШтрихкода(Форма, ПараметрыСканирования, РезультатОбработкиШтрихкода) Экспорт
	...
	ПараметрыЗапроса.Параметры               = Новый Структура("НеВыводитьОкноОжидания, НомерФискальногоНакопителя, ККТ", Истина, ПараметрыСканирования.НомерФискальногоНакопителя, ПараметрыСканирования.ККТФФД12ИСМП);
