...
Процедура ЗаполнитьСведенияОбОперацияхПоБанковскимСчетам(Параметры, СведенияОбОперациях) Экспорт
	...
	СведенияОбОперациях.ЦифровойРубль.СчетУчета = ЗаполнениеОтчетностиВБанкиПереопределяемый.СчетЦифровойРубль();
	...
	ПараметрыЗапроса.Вставить("СчетБанковскихОпераций", СведенияОбОперациях.ЦифровойРубль.СчетУчета);
	СведенияОбОперациях.ЦифровойРубль.СводныеСведения = СводныеСведенияПоБанковскимОперациям2017Кв3(ПараметрыЗапроса);
	СведенияОбОперациях.ЦифровойРубль.ДетальныеСведения = ДетальныеСведенияПоБанковскимОперациям2017Кв3(ПараметрыЗапроса);
...
Функция СчетаОСВПоСчетам() Экспорт
	...
	СписокСчетов =
		"01, 02, 03, 04, 05, 07, 08, 09, 10, 11, 14, 15, 16, 19,
		|20, 21, 23, 25, 26, 28, 29,
		|40, 41, 42, 43, 44, 45, 46, 50, 57, 58, 59,
		|60, 62, 63, 66, 67, 68, 69, 70, 71, 73, 75, 76, 77, 79,
		|80, 81, 82, 83, 84, 86, 90, 91, 94, 96, 97, 98, 99,
		|001, 003, 007, 008, 009";
...
Функция СчетаАнализовСчетов() Экспорт
	...
	СписокСчетов =
		...
		|20, 23, 25, 26, 41, 43, 44, 45, 50, 57, 58, 59,
...
Процедура ЗаполнитьОтчетБухгалтерскаяОтчетностьВБанкФормаОтчета2017Кв3(ПараметрыОтчета, Контейнер)
	...
	Заполнить2017Кв3_Операции53(ПараметрыОтчета);
...
Процедура Заполнить2017Кв3_ОСВПоСчету_ЗаПериод_5_11_01(ПараметрыЗаполнения)
	...
	ИтогиОСВ = Новый Структура;
	ИтогиОСВ.Вставить("СНД", 0);
	ИтогиОСВ.Вставить("СНК", 0);
	ИтогиОСВ.Вставить("ДО", 0);
	ИтогиОСВ.Вставить("КО", 0);
	ИтогиОСВ.Вставить("СКД", 0);
	ИтогиОСВ.Вставить("СКК", 0);
	ИтогиОСВ.Вставить("КНОД", 0);
	ИтогиОСВ.Вставить("КНОК", 0);
	ИтогиОСВ.Вставить("КДО", 0);
	ИтогиОСВ.Вставить("ККО", 0);
	ИтогиОСВ.Вставить("ККОД", 0);
	ИтогиОСВ.Вставить("ККОК", 0);
	...
	Пока ВыборкаПоСчетам.Следующий() Цикл
		...
		ИтогиОСВ.КНОД = ИтогиОСВ.КНОД + ВыборкаПоСчетам.КНОД;
		ИтогиОСВ.КНОК = ИтогиОСВ.КНОК + ВыборкаПоСчетам.КНОК;
		ИтогиОСВ.КДО  = ИтогиОСВ.КДО  + ВыборкаПоСчетам.КДО;
		ИтогиОСВ.ККО  = ИтогиОСВ.ККО  + ВыборкаПоСчетам.ККО;
		ИтогиОСВ.ККОД = ИтогиОСВ.ККОД + ВыборкаПоСчетам.ККОД;
		ИтогиОСВ.ККОК = ИтогиОСВ.ККОК + ВыборкаПоСчетам.ККОК;
	...
	// Сброс итогов в 0-сегмент.
	ПараметрыЗаписи.Вставить("Данные", ИтогиОСВ);
	ПараметрыЗаписи.Вставить("ВидДополнительногоФайла", ПрефиксСегментов + "Итоги" + "." + Счет.УникальныйИдентификатор() + ".0");
	ПараметрыЗаписи.Вставить("ИмяФайла", "Итоги ОСВ по счету " + Счет.Код);
	ПараметрыЗаписи.Вставить("КоличествоЗаписей", 1);
	ЗаписатьДанныеВРегистрДополнительныхФайлов(ПараметрыЗаписи);
		   ...
		   И СчетОтмеченВСписке(СчетОтчета, ПараметрыОтчета[ИмяРаздела + "_Счета"]) Тогда
			...
			Если ДействуетСВерсииФормата("5.11.01", ПараметрыОтчета.ВерсияФормата) Тогда
				...
				Для Каждого ПериодЗаполнения Из ПериодыЗаполнения Цикл
					...
					ДополнительныеДанные = Новый Структура("НачалоПериода, КонецПериода",
						ПараметрыЗаполнения.НачалоПериода, ПараметрыЗаполнения.КонецПериода);
					СтраницаРаздела.ДополнительныеДанные = ДополнительныеДанные;
...
Процедура Заполнить2017Кв3_Операции53(ПараметрыОтчета)
...
Функция Заполнить2017Кв3_Операции_НовыеСводныеДанные()
	...
	СводныеДанные.Колонки.Добавить("ЭтоЦифровойСчет", Новый ОписаниеТипов("Булево"));
	СводныеДанные.Колонки.Добавить("ИдентификаторЦифровогоСчета");
...
Процедура Заполнить2017Кв3_Операции_СводныеСведенияПоСубсчету(СтрокиПоСубсчетам, ВыборкаПоСубсчетам, Запрос)
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	БанковскиеСчета.ЭтоЦифровойСчет КАК ЭтоЦифровойСчет,
...
Процедура Заполнить2017Кв3_Операции_ДетальныеСведения(ПараметрыОтчета, СчетВерхнегоУровня)
	...
	ТаблицаОпераций.Колонки.Добавить("СчетКонтрагентаЭтоЦифровойСчет", Новый ОписаниеТипов("Булево"));
	ТаблицаОпераций.Колонки.Добавить("СчетКонтрагентаИдЦифровогоСчета", Новый ОписаниеТипов("Строка"));
...
Функция ЗапросАнализСчета_5_11_01(ПараметрыЗаполнения)
	...
	Для Инд = 1 По ПараметрыСчета.Количество() - 1 Цикл
		...
		Если ПараметрыСчета[Инд].Субконто.Количество() < 2 Тогда
			// Группировка в итогах по полю со значением NULL теряет группируемые данные, поэтому используется строковый литерал.
			ТекстСегментаЗапроса = СтрЗаменить(ТекстСегментаЗапроса, "ПРЕДСТАВЛЕНИЕ(ХозрасчетныйОстаткиИОбороты.Субконто2)", "NULL");
			ТекстСегментаЗапроса = СтрЗаменить(ТекстСегментаЗапроса, "ПРЕДСТАВЛЕНИЕ(ХозрасчетныйОбороты.Субконто2)", "NULL");
			ТекстСегментаЗапроса = СтрЗаменить(ТекстСегментаЗапроса, "ХозрасчетныйОстаткиИОбороты.Субконто2", """NULL""");
			ТекстСегментаЗапроса = СтрЗаменить(ТекстСегментаЗапроса, "ХозрасчетныйОбороты.Субконто2", """NULL""");
...
Функция ДетальныеСведенияПоБанковскимОперациям2017Кв3(Параметры)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВТ_БанковскиеСчета.ЭтоЦифровойСчет, ЛОЖЬ) КАК СчетКонтрагентаЭтоЦифровойСчет,
		|	ЕСТЬNULL(ВТ_БанковскиеСчета.ИдентификаторЦифровогоСчета, """") КАК СчетКонтрагентаИдЦифровогоСчета
		|ИЗ
