...
Функция ДанныеДляНачисленияРезерва(ОтчетнаяДата, Организации, МенеджерВременныхТаблиц, ПолучатьСостояниеРезерва = Ложь, ПолучатьСостояниеРезерваНачалоГода = Ложь)
	...
	|	ВЫБОР
	|		КОГДА СрокиОплатыДокументов.СрокОплаты ЕСТЬ NULL
	|				ИЛИ СрокиОплатыДокументов.СрокОплаты = ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК СрокОплатыУстановленДокументом,
	...
	|	ЕСТЬNULL(СрокиОплатыДокументов.СрокОплаты, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаОплатыПоДокументу,
	...
	|		ПО ДанныеРегистраХозрасчетный.Организация = СрокиОплатыДокументов.Организация
	|			И ДанныеРегистраХозрасчетный.ДокументРасчетовСКонтрагентом = СрокиОплатыДокументов.Документ
...
Функция ДанныеВстречнойЗадолженности(ОтчетнаяДата, МенеджерВременныхТаблиц)
	...
	// Задолженность сначала выбирается в разрезе счетов, чтобы исключить счета с остатком по Дт.
	// Затем задолженность суммируется в целом по контрагенту.
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Задолженность.Организация КАК Организация,
	|	Задолженность.Счет КАК Счет,
	|	Задолженность.Субконто1 КАК Контрагент,
	|	ЕСТЬNULL(Задолженность.СуммаОстатокКт, 0) КАК Задолженность,
	|	ЕСТЬNULL(Задолженность.СуммаНУОстатокКт, 0) КАК ЗадолженностьНУ
	|ПОМЕСТИТЬ ВстречнаяЗадолженность
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(
	|			&ОтчетнаяДата,
	|			Счет В (&СчетаРасчетов),
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВстречнаяЗадолженность.Организация КАК Организация,
	|	ВстречнаяЗадолженность.Контрагент КАК Контрагент,
	|	СУММА(ВстречнаяЗадолженность.Задолженность) КАК Задолженность,
	|	СУММА(ВстречнаяЗадолженность.ЗадолженностьНУ) КАК ЗадолженностьНУ
	|ИЗ
	|	ВстречнаяЗадолженность КАК ВстречнаяЗадолженность
	|
	|СГРУППИРОВАТЬ ПО
	|	ВстречнаяЗадолженность.Организация,
	|	ВстречнаяЗадолженность.Контрагент
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	Контрагент";
...
Функция РассчитатьРезерв(ДанныеДляНачисленияРезерва, ОтчетнаяДата, МенеджерВременныхТаблиц)
	...
	Пока ДанныеДляНачисленияРезерва.Следующий() Цикл
		...
		Если Запись.СрокОплатыУстановленДоговором Или Запись.СрокОплатыУстановленДокументом Тогда
			
			Если ЗначениеЗаполнено(Запись.ДатаОплатыПоДокументу) Тогда
				Запись.СрокОплаты = (Запись.ДатаОплатыПоДокументу - Запись.ДатаПризнанияЗадолженности) / ПродолжительностьСуток;
			ИначеЕсли ЗначениеЗаполнено(Запись.ДатаОплатыПоДоговору) Тогда
			...
			Запись.СрокОплаты = ?(Запись.СрокОплаты < 0, 0, Запись.СрокОплаты);
		Иначе
...
Функция НовыйРасчетРезерваПоСомнительнымДолгам()
	...
	РасчетРезерва.Колонки.Добавить("СрокОплатыУстановленДокументом",         Новый ОписаниеТипов("Булево"));
	...
	РасчетРезерва.Колонки.Добавить("ДатаОплатыПоДокументу",                  ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
