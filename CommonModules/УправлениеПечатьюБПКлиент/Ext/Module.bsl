...
Функция ВыполнитьКомандуПечатиТранспортнойНакладной(ОписаниеКоманды) Экспорт
	Если НЕ РеализацияТоваровУслугФормыКлиентСервер.ПолучитьРеквизитФормы(ОписаниеКоманды.Форма, "НастроенОбменЭПД")
		ИЛИ УправлениеПечатьюБПВызовСервера.ЭлектроннаяНакладнаяБольшеНеСпрашивать() Тогда
		
		ВыполнитьКомандуПечати(ОписаниеКоманды);
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьКомандуПечатиТранспортнойНакладнойЗавершение", ЭтотОбъект, ОписаниеКоманды);

		ОткрытьФорму("Обработка.ПечатьТранспортнойНакладной.Форма.ФормаЭлектроннаяТранспортнаяНакладная",,ОписаниеКоманды.Форма,,,,ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли; 
КонецФункции 

Функция ВыполнитьКомандуПечатиТранспортнойНакладнойЗавершение(Результат, ОписаниеКоманды) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		РеализацияТоваровУслугФормыКлиент.СоздатьЭтРННаОсновании(ОписаниеКоманды.Форма);
...
Функция ВыполнитьКомандуПечатиРеквизитовОрганизаций(ОписаниеКоманды) Экспорт
	...
	ОткрытьФорму("Справочник.Организации.Форма.ПечатьРеквизитовОрганизации", ПараметрыФормы, ОписаниеКоманды.Форма);
...
Функция ВыполнитьКомандуПечатиРеестраДокументов(ОписаниеКоманды) Экспорт
	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		МобильныйКлиентБПКлиент.ПоказатьФормуНедоступнаяФункциональность(
			НСтр("ru = 'Сформировать реестр документов'"));
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВыполнитьКомандуПечатиИнвентаризационныхЭтикеток(ОписаниеКоманды) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОбъектыПечати", ОписаниеКоманды.ОбъектыПечати);
	ПараметрыФормы.Вставить("Вид", "ИнвентарнаяЭтикетка");
	
	ОткрытьФорму("Обработка.ПечатьИнвентарныхЭтикеток.Форма.ФормаНастройкиПечати", ПараметрыФормы, ОписаниеКоманды.Форма, , , , ,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработкаНавигационнойСсылки(Форма, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки =
		УправлениеПечатьюБПКлиентСервер.ГиперссылкаФормыНастройкиПечатиАртикулов() Тогда
		
		СтандартнаяОбработка = Ложь;
		ОткрытьФорму("Обработка.ПанельАдминистрированияБП.Форма.ПечатьАртикулов");
...
// Выполняет команду печати признания расходов УСН.
// 
// Параметры:
//	ОписаниеКоманды - структура, содержащая ключ, соответствующие таблице значений
//						создаваемой функций БСП УправлениеПечатью.СоздатьКоллекциюКомандПечати(),
//						и ключ Форма - управляемая форма, из которой вызвана команда печати.
//
Функция ВыполнитьКомандуПечатиРасходовУСН(ОписаниеКоманды) Экспорт
	...
	ФормаДокумента = ОписаниеКоманды.Форма;
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("ОбъектОтбора", ОбъектПечати);
	ОткрытьФорму("Отчет.РасходыУСН.Форма.ФормаРасходыПоДокументу", ПараметрыФормы, ФормаДокумента);
