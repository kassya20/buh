...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ПодключениеПриложения1СПерсонал.ИмяМетода);
...
Процедура ВыполнитьДействияПриПереходеНаВерсиюAPI(ТекущаяВерсияAPI, НоваяВерсияAPI)
	...
	Если ИнтеграцияУправлениеПерсоналом.ВерсияФорматаМеньшеВерсии(ТекущаяВерсияAPI, "2.4")
		И ИнтеграцияУправлениеПерсоналом.ВерсияФорматаБольшеИлиРавнаВерсии(НоваяВерсияAPI, "2.4") Тогда
...
Процедура ВключитьНастройкиПрограммыПослеПодключенияПриложения() Экспорт
	...
	Если ИспользуетсяВерсияAPI("2.4") Тогда
...
Процедура ТипыОбъектовДляРучнойРегистрацииИзменений(ТипыОбъектов) Экспорт
	...
	Интеграция1СПерсоналВнутренний.ТипыОбъектовДляРучнойРегистрацииИзменений(ТипыОбъектов);
...
Процедура ВыполнитьОбработчикиОбмена(МенеджерОбмена) Экспорт
	...
	|	И Обработчики.Приложение = ЗНАЧЕНИЕ(Перечисление.ПриложенияДляИнтеграции.УправлениеПерсоналом)";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если Выборка.Обработчик = ИмяОбработчикаЗарегистрироватьОбновлениеВидовОтпусков() Тогда
			ЗарегистрироватьОбновлениеВидовОтпусков();
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗарегистрироватьОбработчикОбмена(ИмяОбработчика, ПроверятьРегистрацию = Истина) Экспорт
	...
	Если ПроверятьРегистрацию Тогда
		...
		|	И ОбработчикиОбмена.Приложение = ЗНАЧЕНИЕ(Перечисление.ПриложенияДляИнтеграции.УправлениеПерсоналом)";
	...
	МенеджерЗаписи.Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом;
