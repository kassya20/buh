...
// Определяет период, за который можно получить записи учетной политики.
// Важно использовать при получении кешируемых значений.
//
// Параметры:
//  Период - Дата
// 
...
//  Дата
//
Функция ПериодКеша(Период) Экспорт
...
Функция СуществуетСистемаНалогообложения(Организация, Период, ВыводитьСообщениеОбОтсутствииУчетнойПолитики = Ложь, ДокументСсылка = Неопределено) Экспорт
	...
	ПрименяетсяТолькоЕНВД = СистемаНалогообложения = Перечисления.СистемыНалогообложения.ОсобыйПорядок
		И ТолькоЕНВДПоДаннымУчетнойПолитики(Организация, Период);
...
Функция ПлательщикЕНП(Организация, Период) Экспорт
	...
	Если Период < НастройкиУчетаВызовСервера.ДатаНачалаПримененияЕНП() Тогда
		Результат = Ложь;
	ИначеЕсли Период >= НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж() Тогда
		Результат = Истина;
	Иначе
		Результат = ПолучитьФункциональнуюОпцию("ПлательщикЕНП", 
			ПараметрыФункциональнойОпцииНастройкиСистемыНалогообложения(Организация, Период));
	КонецЕсли;
...
#Область УчетнаяПолитикаНалогНаПрибыль
...
// Возвращает значение настройки учета налога на прибыль ФормироватьСтоимостьПоПравиламБУ
//
// Параметры:
//   Организация - СправочникСсылка.Организации
//   Период - Дата - период
//
// Возвращаемое значение:
//   Булево - Истина, если стоимость в налоговом учете формируется по правилам бухгалтерского учета.
//
Функция ФормироватьСтоимостьПоПравиламБУ(Знач Организация, Период) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Настройки.ФормироватьСтоимостьПоПравиламБУ КАК ФормироватьСтоимостьПоПравиламБУ
		|ИЗ
		|	РегистрСведений.НастройкиУчетаНалогаНаПрибыль.СрезПоследних(&Период, Организация = &Организация) КАК Настройки
		|ГДЕ
		|	Настройки.ФормироватьСтоимостьПоПравиламБУ";
...
Функция ОсновнойСчетУчетаЗатрат(Организация, Период) Экспорт
	...
	Возврат БухгалтерскийУчетПовтИсп.ОсновнойСчетУчетаЗатрат(
...
Функция СпособПогашенияСтоимостиСпецодеждыНУ(Организация, Период) Экспорт
	...
	Если Период < '20150101' Или Период > УчетМатериаловВЭксплуатации.ДатаОтменыСпецодежды() Тогда
...
Функция ПоддерживаетсяДисконтированиеОбязательств() Экспорт

	УчетПоддерживается = Ложь;
	УчетПроцентовПоОбязательствам.ОпределитьПоддержкуДисконтированияОбязательств(УчетПоддерживается);
...
// В соответствии с Рекомендацией БМЦ Р-175 организация может отражать в ОДДС суммы единого налогового платежа общей суммой,
// либо распределять его между отдельными показателями отчета.
//
// Параметры:
//  Период      - Дата
//  Организация - СправочникСсылка.Организации
// 
// Возвращаемое значение:
//  Булево - Истина, если выбран вариант, предполагающий распределение ЕНП
//
Функция РаспределятьЕдиныйНалоговыйПлатеж(Период, Организация) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УчетнаяПолитика.РаспределятьЕдиныйНалоговыйПлатеж КАК РаспределятьЕдиныйНалоговыйПлатеж
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &ГоловнаяОрганизация) КАК УчетнаяПолитика
	|ГДЕ
	|	УчетнаяПолитика.РаспределятьЕдиныйНалоговыйПлатеж";
...
Функция ПрименяетсяНалогНаПрофессиональныйДоходЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
// Возвращает признак того, что в базе есть организации, применяющие налог на профессиональный доход
// и уплачивающие добровольные взносы в ПФР после даты перехода на НПД
//
// Возвращаемое значение:
//  Булево
//
Функция ЕстьСамозанятыеПлательщикиДобровольныхВзносовПФР() Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	ИСТИНА КАК РезультатВыполнения
		|ИЗ
		|	ВТ_ПлательщикиНПД КАК ВТ_ПлательщикиНПД
		...
		|		ПО ВТ_ПлательщикиНПД.Организация = НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС.Организация
		|			И ВТ_ПлательщикиНПД.ДатаИзменения <= НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС.Период
		|			И (НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС.ПлательщикСтраховыхВзносовПФР_ФФОМС = ИСТИНА)";
...
// Определяет применяется ли ФСБУ 6 для этой организации на эту дату
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация, для которой выполянется проверка
//  Период - Дата - Дата, на которую проверяем
//
// Возвращаемое значение:
//  Булево
//
...
// Определяет дату начала применения ФСБУ 6 для организации
// 
// Параметры:
//  Организация - СправочникСсылка.Организации - Организация, для которой выполянется проверка
//
...
// Определяет алгоритм периода организации, соответствующий решению, принятому организацией на основании п. 33 ФСБУ 6
// и заданный в учетной политике бухгалтерского учета.
// Некорректные значения приводятся к допустимым.
//
// Параметры:
//  Период      - Дата
//  Организация - СправочникСсылка.Организации
...
//
Функция АлгоритмПериодаАмортизации(Период, Организация) Экспорт
	
	// Обращения не должны быть частыми, поэтому кеширование не используется
	
	Если Не Перечисления.АлгоритмыПериодаАмортизации.ПоддерживаетсяВыборАлгоритма() Тогда
		Возврат Перечисления.АлгоритмыПериодаАмортизации.ЗначениеПоУмолчанию();
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УчетнаяПолитика.ПериодАмортизации КАК ПериодАмортизации
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &ГоловнаяОрганизация) КАК УчетнаяПолитика";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Не Выборка.Следующий() Или Не ЗначениеЗаполнено(Выборка.ПериодАмортизации) Тогда
		Возврат Перечисления.АлгоритмыПериодаАмортизации.ЗначениеПоУмолчанию();
	КонецЕсли;
...
Функция ПлательщикНДСПрименяющийУСНЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
Функция ДатаНачалаУплатыНДСНаУСН(Организация, Период) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаНачалаПримененияНДСнаУСН", УчетУСНКлиентСервер.ДатаНачалаПримененияНДС());
	Запрос.Текст =
	"ВЫБРАТЬ
...
Функция НачислятьНДСПоОтгрузке(Организация, Период) Экспорт
	...
	ОтгрузкаНеплательщикомНДС = УчетНДС.ВедетсяУчетНДСПоФЗ134(Период) 
								И НЕ ПлательщикНДС(Организация, Период)
								И НЕ ПлательщикНДСПрименяющийУСН(Организация, Период);   
...
Функция УчитыватьТаруЗаБалансом(Организация, Период) Экспорт

	Если Период >= '2022-01-01' Тогда
		Возврат Истина;
	ИначеЕсли Период < '2021-01-01' Тогда
		Возврат Ложь;
	Иначе
		ПараметрыОпции = Новый Структура("Организация", Организация);
		Возврат ПолучитьФункциональнуюОпцию("УчитыватьТаруЗаБалансом2021", ПараметрыОпции);
	КонецЕсли;

КонецФункции 

Функция ПрименяетсяФСБУ27(Период) Экспорт
	
	Возврат Период >= БухгалтерскийУчетКлиентСервер.НачалоПримененияФСБУ27();
	
КонецФункции

Функция ИзменениеСпособаРасчетаСебестоимостиСФИФОНаУчетПоСредней(Организация, Период) Экспорт
	Запрос = Новый Запрос();
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УчетнаяПолитикаСрезПоследних.СпособОценкиМПЗ КАК СпособОценкиМПЗ,
	|	УчетнаяПолитикаСрезПоследних.Организация КАК Организация
	|ПОМЕСТИТЬ УчетнаяПолитикаСтарая
	|ИЗ
	...
	|	УчетнаяПолитикаСрезПоследних.СпособОценкиМПЗ КАК СпособОценкиМПЗ,
	|	УчетнаяПолитикаСрезПоследних.Организация КАК Организация
	|ПОМЕСТИТЬ УчетнаяПолитикаНовая
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&ПериодНовый, Организация = &Организация) КАК УчетнаяПолитикаСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА УчетнаяПолитикаНовая.СпособОценкиМПЗ = УчетнаяПолитикаСтарая.СпособОценкиМПЗ
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА УчетнаяПолитикаНовая.СпособОценкиМПЗ = ЗНАЧЕНИЕ(Перечисление.СпособыОценки.ПоСредней)
	|						И УчетнаяПолитикаСтарая.СпособОценкиМПЗ = ЗНАЧЕНИЕ(Перечисление.СпособыОценки.ФИФО)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|	КОНЕЦ КАК БылоПереключениеСУчетаПоФИФО
	|ИЗ
	|	УчетнаяПолитикаСтарая КАК УчетнаяПолитикаСтарая
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ УчетнаяПолитикаНовая КАК УчетнаяПолитикаНовая
	|		ПО УчетнаяПолитикаСтарая.Организация = УчетнаяПолитикаНовая.Организация";
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Период", Период);
	Запрос.УстановитьПараметр("ПериодНовый", ДобавитьМесяц(Период, 1));
	Результат = Запрос.Выполнить().Выбрать();
	Если Результат.Следующий() Тогда
		Возврат Результат.БылоПереключениеСУчетаПоФИФО;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Функция ПлательщикНДСНаУСНСВычетами(Организация, Период, ДатаИзменения = Неопределено) Экспорт
	...
	Возврат ПлательщикНДСПрименяющийУСН(Организация, Период, ДатаИзменения)
		И РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(Организация, Период) = Перечисления.ВидыСтавокНДС.Общая;
	
КонецФункции

Функция ПлательщикТуристическогоНалога(Организация, Период, ДатаИзменения = Неопределено) Экспорт
	
	Результат = ПолучитьФункциональнуюОпцию("ПлательщикТуристическогоНалога", 
		ПараметрыФункциональнойОпцииНастройкиСистемыНалогообложения(Организация, Период, ДатаИзменения));
...
Функция РаздельныйУчетНДСБезВычетов(Организация, Период) Экспорт

	Результат = ПолучитьФункциональнуюОпцию("РаздельныйУчетНДСБезВычетов", 
		Новый Структура("Организация, Период", Организация, НачалоМесяца(Период)));
...
// Возвращает границы периода записей
// 
// Параметры:
//  Организация - СправочникСсылка.Организации
// 
// Возвращаемое значение:
//  Структура - Период записей:
//    * Начало - Дата, Неопределено - Начало периода
//    * Конец - Дата, Неопределено - Конец периода
//
Функция ГраницыПериодаЗаписей(Организация) Экспорт
	...
	ГраницыПериода.Вставить("Начало", ДатаНачала);
	ГраницыПериода.Вставить("Конец", ДатаКонца);
	
	Возврат ГраницыПериода;
	
КонецФункции

Функция ПлательщикНДССВычетами(Организация, Период, ДатаИзменения = Неопределено) Экспорт
	
	Возврат ПлательщикНДС(Организация, Период, ДатаИзменения)
		Или ПлательщикНДСНаУСНСВычетами(Организация, Период, ДатаИзменения);
	
КонецФункции

Функция ВыделятьНДССКомиссииБанка(Организация, Период) Экспорт
	
	Результат =УчетДенежныхСредствПовтИсп.ВыделятьНДССКомиссииБанка(Организация, ПериодКеша(Период));
	
	Возврат Результат;
	
КонецФункции

// Возвращает значение настройки классификации активов и обязательств по данным инвентаризации расчетов
//
// Параметры:
//  Период      - Дата
//  Организация - СправочникСсылка.Организации
...
//  Булево
//
Функция КлассифицироватьАктивыОбязательстваПоСрокам(Период, Организация) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УчетнаяПолитика.КлассифицироватьАктивыОбязательстваПоСрокам КАК КлассифицироватьАктивыОбязательстваПоСрокам
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &ГоловнаяОрганизация) КАК УчетнаяПолитика
	|ГДЕ
	|	УчетнаяПолитика.КлассифицироватьАктивыОбязательстваПоСрокам";
	
	Возврат Не Запрос.Выполнить().Пустой();
	
КонецФункции

Функция ПериодичностьРегистрацииСводныхСправок(Организация, Период) Экспорт

	Результат = ПолучитьФункциональнуюОпцию("ПериодичностьРегистрацииСводныхСправок", 
		Новый Структура("Организация, Период", Организация, НачалоМесяца(Период)));
...
#Область УстаревшиеСвойстваУчетнойПолитики

#Область СовместимостьРедакция2

Функция НакапливатьЗатратыДоВыпуска(ЗаписьУчетнойПолитики, ИспользоватьПлановуюСебестоимость) Экспорт
	
	Если Не ЗначениеЗаполнено(ЗаписьУчетнойПолитики.ПорядокСписанияРасходовНаСебестоимостьУслуг) Тогда
		
		Возврат Ложь;
		
	ИначеЕсли Не ЗаписьУчетнойПолитики.УдалитьОказаниеУслуг Тогда
		
		Возврат Истина;
		
	ИначеЕсли ИспользоватьПлановуюСебестоимость Тогда
		
		Возврат ЗаписьУчетнойПолитики.ПорядокСписанияРасходовНаСебестоимостьУслуг =
			Перечисления.ПорядокСписанияРасходовНаСебестоимостьУслуг.ТолькоПоПлановымЦенам;
...
Функция СпособНастройкиВстречногоВыпуска(Запись, Период, Организация) Экспорт
	
	НеНастроен = Перечисления.СпособыРасчетаСебестоимостиПродукции.ПустаяСсылка();
	
	// Здесь проверяются устаревшие настройки, так как предполагается,
	// что СпособРасчетаСебестоимостиПроизводства не может включаться
	// после отказа от этих устаревших настроек
	Если Не Запись.УдалитьРассчитыватьСебестоимостьПолуфабрикатов
		И Не Запись.УдалитьРассчитыватьСебестоимостьУслугСобственнымПодразделениям Тогда
		// Производство одноэтапное, настройка не имеет смысла
		Возврат НеНастроен;
	КонецЕсли;
	
	СпособНастройки = Запись.СпособРасчетаСебестоимостиПроизводства;
	Если Не ПолучитьФункциональнуюОпцию("ВестиУчетЗатратПоПодразделениям") Тогда
		СпособНастройки = Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПеределам;
	КонецЕсли;
	...
	Если СпособНастройки = Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПеределам Тогда
		
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Настройка.Период КАК Период
		|ИЗ
		...
		Если Не Запрос.Выполнить().Пустой() Тогда
			Возврат Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПеределам;
		КонецЕсли;
		
	ИначеЕсли СпособНастройки = Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПодразделениям Тогда
		
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Настройка.Период КАК Период
		|ИЗ
		|	РегистрСведений.ПорядокЗакрытияПодразделенийОрганизаций.СрезПоследних(&Период, Организация = &Организация) КАК Настройка";
		
		Если Не Запрос.Выполнить().Пустой() Тогда
			Возврат Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПодразделениям;
		КонецЕсли;
...
#Область ДляОбработчиковОбновления

Функция ЕстьУсловияБазыРаспределенияВыручкаОтРеализацииУслуг(Запись) Экспорт
	
	ТолькоУслугиЗаказчикам = Не Запись.УдалитьВыпускПродукции
		И Не Запись.УдалитьРассчитыватьСебестоимостьПолуфабрикатов
		И Не Запись.УдалитьРассчитыватьСебестоимостьУслугСобственнымПодразделениям;
		
	Если Не ТолькоУслугиЗаказчикам Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат Запись.ПорядокСписанияРасходовНаСебестоимостьУслуг =
		Перечисления.ПорядокСписанияРасходовНаСебестоимостьУслуг.БезУчетаВыручки;
	
КонецФункции

Функция УчетНезавершенногоПроизводстваВедетсяБезИнвентаризации(Запись) Экспорт
	
	Если Запись.УдалитьОказаниеУслуг Тогда
	...
	Иначе
		...
		Возврат УчетПроизводства.ИспользоватьВыпускПродукции() И Запись.УдалитьВыпускПродукции;
...
#Область МалоценныеВнеоборотныеАктивы

// Возвращает ссылку на справочник НастройкиСущественностиВнеоборотныхАктивов, установленный в учетной политике
// 
// Параметры:
//  Период - Дата
//  Организация - СправочникСсылка.Организации
...
Функция НастройкиСущественностиВНА(Период, Организация) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	УчетнаяПолитика.НастройкиСущественностиВнеоборотныхАктивов КАК Настройки
	|ИЗ
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &Организация) КАК УчетнаяПолитика";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Не Выборка.Следующий() Тогда
		Возврат Справочники.НастройкиСущественностиВнеоборотныхАктивов.ПустаяСсылка();
	КонецЕсли;
	
	Возврат Выборка.Настройки;
...
// Определяет ссылку на настройку учета затрат на указанную дату.
// Для прикладных задач следует читать содержимое настроек с помощью методов
// Справочники.НастройкиУчетаЗатрат.БазовыеНастройки, Справочники.НастройкиУчетаЗатрат.ИсполняемыеНастройки.
// 
// Параметры:
//  Период      - Дата
//  Организация - СправочникСсылка.Организации
// 
// Возвращаемое значение:
//  СправочникСсылка.НастройкиУчетаЗатрат - значение ссылки.
//  Может быть возвращена пустая ссылка.
//
Функция НастройкиУчетаЗатрат(Период, Организация) Экспорт
	...
	Если Не Выборка.Следующий() Тогда
		Возврат Справочники.НастройкиУчетаЗатрат.ПустаяСсылка();
	КонецЕсли;
	
	Возврат Выборка.НастройкиУчетаЗатрат;
...
Функция ПрименяетсяАУСН(Организация, Период, ДатаИзменения = Неопределено) Экспорт
	
	Результат = ПолучитьФункциональнуюОпцию("ПрименяетсяАУСН", 
		ПараметрыФункциональнойОпцииНастройкиСистемыНалогообложения(Организация, Период, ДатаИзменения));
...
Функция ПрименяетсяАУСНДоходы(Организация, Период) Экспорт
	
	Результат = ПрименяетсяАУСН(Организация, Период)
		И ОбъектНалогообложенияУСН(Организация, Период) = Перечисления.ОбъектыНалогообложенияПоУСН.Доходы;
	
	Возврат Результат;
	
КонецФункции

Функция ПрименяетсяАУСНДоходыМинусРасходы(Организация, Период) Экспорт
	
	Результат = ПрименяетсяАУСН(Организация, Период)
		И ОбъектНалогообложенияУСН(Организация, Период) = Перечисления.ОбъектыНалогообложенияПоУСН.ДоходыМинусРасходы;
...
Функция ПрименяетсяАУСНЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
Функция ПорядокПризнанияРасходовПоНДС(Организация, Период) Экспорт
	...
	Если Период >= УчетУСНКлиентСервер.ДатаНачалаПримененияНДС() Тогда
		Результат = Перечисления.ПорядокПризнанияРасходовПоНДС.ВключатьВСтоимость;
	Иначе
...
Функция ПризнаватьДоходОтКомиссионераНаДатуОтчетаУСН(Организация, Период) Экспорт
	
	Результат = ПолучитьФункциональнуюОпцию("ПризнаватьДоходОтКомиссионераНаДатуОтчета",
...
Функция ИсключатьНДСИзДоходаПоАвансамБезНДС(Организация, Период) Экспорт
	
	Результат = ПолучитьФункциональнуюОпцию("ИсключатьНДСИзДоходаПоАвансамБезНДС",
		Новый Структура("Организация, Период", Организация, Период));
...
Функция ПорядокПризнанияДоходаОтПродажЧерезКомиссионераИП(Организация, Период) Экспорт
	
	Результат = ПолучитьФункциональнуюОпцию("ПорядокПризнанияДоходаОтПродажЧерезКомиссионераИП",
		Новый Структура("Организация, Период", Организация, Период));
	
	Если Результат = Ложь Тогда
		Результат = Перечисления.ПорядокПризнанияДоходаОтПродажЧерезКомиссионера.ПоФактуОплатыПокупателем;
	КонецЕсли;
...
Функция ПлательщикФиксированныхВзносовЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	ЗначенияНастройки = НастройкиУчета.ЗначенияРесурсаУчетнойПолитикиЗаПериод(
	...
	Результат = Ложь;
	Для Каждого Значение Из ЗначенияНастройки Цикл
		Результат = Результат Или Значение;
	КонецЦикла;
...
// Возвращает признак того, что в заданном периоде упачивались взносы в ФСС
//
// Параметры:
//  Организация - СправочникСсылка.Организации
//  НачалоПериода - Дата
...
//  Булево
//
Функция ПлательщикВзносовВФССзаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
// Возвращает признак того, что в заданном периоде индивидуальный предприниматель
// платил только налог на профессиональный доход
//
// Параметры:
//  Организация - СправочникСсылка.Организации
//  НачалоПериода - Дата
...
//
// Возвращаемое значение:
//  Булево - Истина, если ИП платил только налог на профессиональный доход в указанном периоде
//
Функция УплачивалсяТолькоНПДЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
// Возвращает признак того, что в заданном периоде предприниматель уплачивает НПД
//
// Параметры:
//  Организация - СправочникСсылка.Организации
//  НачалоПериода - Дата
...
//  Булево
//
Функция ПлательщикНПДЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
...
Функция ЗначениеУчетнойПолитики(ИмяРесурса, Организация, Период)
	...
	Параметры.Вставить("Период",      ПериодКеша(Период));
...
Функция УчетВыпускаПродукцииПоДаннымПродаж(Организация, Период) Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ИспользоватьВыпускПродукции")
		И ЗначениеУчетнойПолитики("УчетВыпускаПродукцииПоДаннымПродаж", Организация, Период);
...
Функция СпособУчетаВыпускаГотовойПродукции(Организация, Период) Экспорт
	...
	Возврат Справочники.НастройкиУчетаЗатрат.ВариантУчетаСтоимостиВыпуска(Период, Организация);

КонецФункции

// Устарела. Следует использовать Справочники.НастройкиУчетаЗатрат.ПлановаяСебестоимость
//
Функция ПлановаяСебестоимость(Организация, Период) Экспорт
	...
	|	РегистрСведений.УчетнаяПолитика.СрезПоследних(&Период, Организация = &Организация) КАК УчетнаяПолитика
	|ГДЕ
	|	УчетнаяПолитика.УдалитьИспользоватьПлановуюСебестоимость";
...
// Для регулярного применения следует использовать Справочники.НастройкиУчетаЗатрат.ВариантУчетаСтоимостиВыпуска
//
Функция ВариантУчетаСтоимостиВыпуска(Запись) Экспорт
	
	Если Не ПлановаяСебестоимость(Запись.Организация, Запись.Период) Тогда
		Возврат Перечисления.ВариантыУчетаСтоимостиВыпуска.ФактическаяСтоимость;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Запись.УдалитьСпособУчетаВыпускаГотовойПродукции)
		Или Запись.УдалитьСпособУчетаВыпускаГотовойПродукции = Перечисления.ВариантыУчетаСтоимостиВыпуска.ФактическаяСтоимость Тогда
		Возврат Перечисления.ВариантыУчетаСтоимостиВыпуска.ПредварительнаяСтоимость;
	КонецЕсли;
	
	Если Запись.УдалитьСпособУчетаВыпускаГотовойПродукции = Перечисления.ВариантыУчетаСтоимостиВыпуска.ПлановаяСтоимость Тогда
		Поддерживается = Ложь;
		ОценкаПродукцииПоПлановойСтоимости.ОпределитьФункциональностьПоддерживается(Поддерживается);
		Если Не Поддерживается Тогда
			Возврат Перечисления.ВариантыУчетаСтоимостиВыпуска.ОтклоненияСтоимости;
		КонецЕсли;
	КонецЕсли;
	
	Возврат Запись.УдалитьСпособУчетаВыпускаГотовойПродукции;
	
КонецФункции

Функция ПрименяетсяАУСНДоходыЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
	
	МассивЗначений = НастройкиУчета.ЗначенияРесурсаУчетнойПолитикиЗаПериод(
		"НастройкиСистемыНалогообложения",
		"ПрименяетсяАУСНДоходы",
		Организация,
		ПериодНастройкиСистемыНалогообложения(Организация, НачалоПериода),
		ПериодНастройкиСистемыНалогообложения(Организация, КонецПериода));
