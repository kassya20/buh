...
Функция ПодготовитьТаблицуСтатусовОтгрузкиПоСчетам(ТаблицаСчетовНаОплату, ТаблицаТоварыУслуги, ТаблицаВозвратнаяТара, ТаблицаРеквизитов) Экспорт
	...
	РеализацииПоСчетам = Документы.СчетНаОплатуПокупателю.РеализацииПоСчетам(МассивСчетовНаОплату);
	
	Для каждого СчетНаОплатуПокупателю Из МассивСчетовНаОплату Цикл
		
		// документы "Реализация товаров" в которых были продажи по анализируемым счетам на оплату
		РеализацииПоСчету = РеализацииПоСчетам.Скопировать(Новый Структура("СчетНаОплату", СчетНаОплатуПокупателю));
		// товары реализованные документом из которого происхолдит пересчет статусов
		РеализованныеТоварыПоСчету = ТоварыУслуги.Скопировать(Новый Структура("СчетНаОплатуПокупателю", СчетНаОплатуПокупателю));
		
		СтрокиКУдалению = РеализацииПоСчету.НайтиСтроки(Новый Структура("Реализация", Реквизиты.Регистратор));
		...
		Для Каждого СтрокаТаблицы Из РеализованныеТоварыПоСчету Цикл
		...
		Если ИнтеграцияCRMПовтИсп.ИнтеграцияВИнформационнойБазеВключена() Тогда
			СостояниеИнтеграции = 
				РегистрыСведений.ДокументыИнтеграцииCRM.СостояниеИнтеграцииДокумента(СчетНаОплатуПокупателю);
			НоваяСтрока.ОтправитьОповещениеОбИзмененииСтатуса = (СостояниеИнтеграции <> Неопределено);
		КонецЕсли;
...
Процедура ВходящийДокументПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("СтатусДокумента",
		СтатусВходящегоДокументаПеречислением(Форма.ОригиналПолучен));
	...
	// Обязательно надо проверять сначала условие Не Форма.ТребуетсяСчетФактура, так как это необходимо для 
	// правильного определения статуса в документе "Поступления доп. расходов" по операции без НДС
	...
	НалогообложениеКонтрагента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Объект.ДоговорКонтрагента, "НалогообложениеКонтрагента");
	Если НалогообложениеКонтрагента = Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход Тогда
		Если Форма.ПолученЧекНПД Тогда
			СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.Получен;
		Иначе
			ДатаПроверки = ?(ЗначениеЗаполнено(Объект.ДатаВходящегоДокумента),
				Объект.ДатаВходящегоДокумента, Объект.Дата);
			СтатусСамозанятого = РегистрыСведений.СтатусыСамозанятых.ПлательщикНПД(Объект.Контрагент, ДатаПроверки);
			Если СтатусСамозанятого <> Неопределено
				И Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтатусСамозанятого, "ПлательщикНПД", Ложь) Тогда
				СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.УтраченСтатусПлательщикаНПД;
			Иначе
				СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.НеПолучен;
			КонецЕсли;
		КонецЕсли;
	Иначе
		СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.НеТребуется;
	КонецЕсли;
	
	ТекущийОбъект.ДополнительныеСвойства.Вставить(ЧекиНПДКлиентСервер.ИмяСтатусаЧекаНПД(), СтатусЧекаНПД);
	
КонецПроцедуры

Функция СтатусВходящегоДокументаПеречислением(ОригиналПолучен) Экспорт
	
	Возврат ?(ОригиналПолучен = Истина,
		Перечисления.СтатусыДокументовПоступления.ОригиналПолучен,
		Перечисления.СтатусыДокументовПоступления.ОригиналНеПолучен);
