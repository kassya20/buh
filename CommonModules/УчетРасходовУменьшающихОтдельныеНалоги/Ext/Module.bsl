...
//  ТаблицаЗначений, Неопределено - Таблица с расходами, см. ПустаяТаблицаРасходов()
//
Функция РасчетРасходовУменьшающихОтдельныеНалоги(ТаблицаРеквизитов) Экспорт
	...
	Если Не Реквизиты.ПрименяетсяУСНДоходы И Не Реквизиты.ПлательщикЕНВД И Не Реквизиты.ПрименяетсяПСН Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	УпрощенныйУчетБольничных =
		Реквизиты.Период >= УчетУСНКлиентСервер.ДатаНачалаИспользованияУпрощенногоУчетаРасходовПоБольничным();
	...
	Если УпрощенныйУчетБольничных Тогда
		Если Реквизиты.ВедетсяУчетБольничных Тогда
			ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыСредствамиБухгалтерии(
				ТаблицыДанных.РасходыПоБольничным, Реквизиты, ТаблицаРасходов);
		Иначе
			ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыВоВнешнейПрограмме(
				ТаблицыДанных.РасходыПоБольничным, Реквизиты, ТаблицаРасходов);
		КонецЕсли;
	Иначе
...
Процедура СформироватьДвиженияРасходыУменьшающиеОтдельныеНалоги(ТаблицаРасходов, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаЗаписи Из ТаблицаДвижений Цикл
		...
		КорректировкаСуммыРасхода = СтрокаЗаписи.СуммаРасхода < 0
			И ТипЗнч(СтрокаЗаписи.ДокументРасхода) = Тип("ДокументСсылка.СведенияОбУплатеНалогов");
		КорректировкаСуммыРасходаСпецРежим = СтрокаЗаписи.СуммаРасходаСпецРежим < 0
			И ТипЗнч(СтрокаЗаписи.ДокументРасхода) = Тип("ДокументСсылка.СведенияОбУплатеНалогов");
		
		Если СтрокаЗаписи.СуммаРасхода > 0 Или СтрокаЗаписи.СуммаРасходаСпецРежим > 0
			Или КорректировкаСуммыРасхода Или КорректировкаСуммыРасходаСпецРежим Тогда
			...
			ЗаполнитьЗначенияСвойств(Запись, Реквизиты, , "Период");
...
//   *СчетаУчета - Массив из ПланСчетовСсылка.Хозрасчетный - счета, относящиеся к данной группировке вида расходов.
//
Функция СчетаРасходовУменьшающихНалог() Экспорт
	...
	Для Каждого СчетУчета Из СчетаСтраховыхВзносов Цикл
		...
		СтрокаВидаРасходов.Заголовок = Нстр("ru = 'Страховые взносы за сотрудников'");
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП);        // 69.06.6 - Страховые взносы единый тариф ИП
	...
	Для Каждого СчетУчета Из СчетаСтраховыхВзносовИП Цикл
		...
		СтрокаВидаРасходов.Заголовок = Нстр("ru = 'Страховые взносы в фиксированном размере уплаченные'");
		СтрокаВидаРасходов.СчетУчета = СчетУчета;
	КонецЦикла;
...
Функция ПодготовитьПараметрыРасходовУменьшающихНалог(ТаблицаРеквизитов) 
	...
	СписокОбязательныхКолонок = ""
		...
		+ "ПлательщикЕНП,"                     // <Булево> - признак использования уплаты ЕНП
		...
		+ "ПрименяетсяТолькоПатент,"           // <Булево> - признак применения только патентной системы налогообложения
		+ "ПлательщикНДФЛ,"                    // <Булево> - признак применения индивидуальным предпринимателем общей системы налогообложения
...
Процедура ДобавитьРасходыПоВзносамСФОТ(РасходыПоВзносам, Реквизиты, ТаблицаРасходов)
	...
	ПериодыЕНП = РасходыПоВзносам.ВыгрузитьКолонку("ДатаПризнанияЕНП");
	ПериодыЕНП = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ПериодыЕНП);
	...
	Если ЗначениеЗаполнено(ПроизводственныйКалендарь) Тогда
		ДатыОплаты = КалендарныеГрафики.ДатыБлижайшихРабочихДней(
			...
			Истина);
	Иначе
		ДатыОплаты = Новый Соответствие;
		Для Каждого Элемент Из ПериодыЕНП Цикл
			ДатыОплаты.Вставить(Элемент, Элемент);
		КонецЦикла;
	КонецЕсли;
	...
	Для каждого СтрокаВзноса Из РасходыПоВзносам Цикл
		...
		ИначеЕсли Реквизиты.ПрименяетсяТолькоПатент Тогда
		...
		// Если все расходы ИП были отнесены на ОСНО, то такие расходы не нужно отражать
		Если Реквизиты.ПлательщикНДФЛ И ДоляНачисленияСпецРежим = 0 Тогда
			СуммаРасхода = 0;
		КонецЕсли;
		
		КорректировкаСуммыРасхода = СуммаРасхода < 0
			И ТипЗнч(СтрокаВзноса.ДокументРасхода) = Тип("ДокументСсылка.СведенияОбУплатеНалогов");
		КорректировкаСуммыРасходаСпецРежим = СуммаРасходаСпецРежим < 0
			И ТипЗнч(СтрокаВзноса.ДокументРасхода) = Тип("ДокументСсылка.СведенияОбУплатеНалогов");
		
		Если СуммаРасхода > 0 Или СуммаРасходаСпецРежим > 0
			Или КорректировкаСуммыРасхода Или КорректировкаСуммыРасходаСпецРежим Тогда
			
			Если ЗначениеЗаполнено(СтрокаВзноса.ДатаПризнанияЕНП) Тогда
				ДатаСУчетомПереноса = ДатыОплаты[СтрокаВзноса.ДатаПризнанияЕНП];
				Если Не ЗначениеЗаполнено(ДатаСУчетомПереноса) Тогда
					ДатаСУчетомПереноса = СтрокаВзноса.ДатаПризнанияЕНП;
				КонецЕсли;
				ПросроченныйПлатеж = ДатаСУчетомПереноса < СтрокаВзноса.ДокументДата;
				ПериодЗаписи = ?(ПросроченныйПлатеж, СтрокаВзноса.ДокументДата, ДатаСУчетомПереноса);
			Иначе
				ПериодЗаписи = СтрокаВзноса.ПериодРегистрации;
			КонецЕсли;
			
			СтрокаРасхода = ТаблицаРасходов.Добавить();
			СтрокаРасхода.Период                = ПериодЗаписи;
...
Процедура ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыВоВнешнейПрограмме(РасходыПоБольничным, Реквизиты, ТаблицаРасходов)
	
	Если РасходыПоБольничным = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СчетЗарплаты = ПланыСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда;
	
	РасходыПоБольничным.Сортировать("ПериодРегистрации УБЫВ");
	
	ИменаКолонок = "Сотрудник, ПериодНачисления, БольничныйЗаСчетРаботодателя, БольничныйЗаСчетРаботодателяСпецРежим,
		|РанееПризнано, РанееПризнаноСпецРежим";
	НачислениеБольничных = РасходыПоБольничным.Скопировать(, ИменаКолонок);
	НачислениеБольничных.Свернуть(ИменаКолонок);
	
	РасходыПоБольничным.Индексы.Добавить("Сотрудник, ПериодНачисления");
	
	Для Каждого СтрокаНачисления Из НачислениеБольничных Цикл
		
		РасходУчтенВОСНО = Реквизиты.ПрименяетсяУСНДоходы И Не Реквизиты.ЭтоИндивидуальныйПредприниматель
			И СтрокаНачисления.ПериодНачисления < Реквизиты.ДатаПереходаНаУСН;
		
		ВсегоНачислено = СтрокаНачисления.БольничныйЗаСчетРаботодателя
			+ СтрокаНачисления.БольничныйЗаСчетРаботодателяСпецРежим;
		
		РанееПризнано = СтрокаНачисления.РанееПризнано + СтрокаНачисления.РанееПризнаноСпецРежим;
		
		СуммаКПризнанию = ВсегоНачислено - РанееПризнано;
		
		Если СуммаКПризнанию <= 0 Тогда
			Прервать;
		КонецЕсли;
		
		Отбор = Новый Структура("Сотрудник, ПериодНачисления",
			СтрокаНачисления.Сотрудник, СтрокаНачисления.ПериодНачисления);
		СтрокиРасходаПоБольничным = РасходыПоБольничным.НайтиСтроки(Отбор);
		
		Для Каждого СтрокаБольничного Из СтрокиРасходаПоБольничным Цикл
			...
			Если СуммаКПризнанию > СуммаВыплаты Тогда
				Продолжить;
			КонецЕсли;
			
			СуммаРасхода = СтрокаБольничного.БольничныйЗаСчетРаботодателя - СтрокаБольничного.РанееПризнано;
			СуммаРасходаСпецРежим = СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим - СтрокаБольничного.РанееПризнаноСпецРежим;
			
			// Если в периоде начисления больничного применялась ОСНО с учетом расходов методом начисления - уменьшать налог на такой больничный нельзя,
			// поскольку расходы уже учтены в налоговом учете при начислении.
			Если РасходУчтенВОСНО Тогда
				СуммаРасхода = 0;
			КонецЕсли;
			...
			Если СуммаРасхода > 0 Или СуммаРасходаСпецРежим > 0 Тогда
				...
				СтрокаРасхода.ДокументРасходаДата   = СтрокаБольничного.ПериодРегистрации;
				СтрокаРасхода.ПериодНачисления      = СтрокаБольничного.ПериодНачисления;
				СтрокаРасхода.СчетУчета             = СчетЗарплаты;
				СтрокаРасхода.СуммаОплаты           = СуммаВыплаты;
				СтрокаРасхода.Задолженность         = СтрокаБольничного.ВсегоКВыплате;
				СтрокаРасхода.НачисленоВсего        = СтрокаБольничного.БольничныйЗаСчетРаботодателя;
				СтрокаРасхода.НачисленоСпецРежим    = СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим;
...
Процедура ДобавитьРасходыПоВыплатеБольничныхПриУчетеЗарплатыСредствамиБухгалтерии(РасходыПоБольничным, Реквизиты, ТаблицаРасходов)
	...
	Для Каждого СтрокаБольничного Из РасходыПоБольничным Цикл
		...
		СуммаВыплаты = СтрокаБольничного.СуммаВыплачено;
		
		ВсегоНачислено = СтрокаБольничного.БольничныйЗаСчетРаботодателя + СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим;
		
		Если ВсегоНачислено <> 0 Тогда
			СуммаРасхода = СуммаВыплаты * СтрокаБольничного.БольничныйЗаСчетРаботодателя / ВсегоНачислено;
			СуммаРасходаСпецРежим = СуммаВыплаты * СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим / ВсегоНачислено;
		Иначе
			СуммаРасхода = 0;
			СуммаРасходаСпецРежим = 0;
		КонецЕсли;
		...
		// поскольку расходы уже учтены в налоговом учете при начислении.
		РасходУчтенВОСНО = Реквизиты.ПрименяетсяУСНДоходы И Не Реквизиты.ЭтоИндивидуальныйПредприниматель
			И СтрокаБольничного.ПериодНачисления < Реквизиты.ДатаПереходаНаУСН;
		Если РасходУчтенВОСНО Тогда
			СуммаРасхода = 0;
		КонецЕсли;
		...
		Если СуммаРасхода > 0 Или СуммаРасходаСпецРежим > 0 Тогда
			...
			СтрокаРасхода.СчетУчета             = СчетЗарплаты;
			СтрокаРасхода.СуммаОплаты           = СуммаВыплаты;
			СтрокаРасхода.Задолженность         = ВсегоНачислено;
			СтрокаРасхода.НачисленоВсего        = СтрокаБольничного.БольничныйЗаСчетРаботодателя;
			СтрокаРасхода.НачисленоСпецРежим    = СтрокаБольничного.БольничныйЗаСчетРаботодателяСпецРежим;
			СтрокаРасхода.СуммаРасхода          = СуммаРасхода;
			СтрокаРасхода.СуммаРасходаСпецРежим = СуммаРасходаСпецРежим;
...
Процедура ДобавитьРасходыПоВзносамИП(Реквизиты, ТаблицаРасходов)
	...
	Если УменьшатьПСН Тогда
		...
		Иначе
			...
			КоэффициентСпецРежим = УчетПСН.ДоляДоходовПатентнойСистемыНалогообложения(
				Реквизиты.Организация, Реквизиты.Период, Периодичность, Истина);
		КонецЕсли;
	...
	ДатыПринятияЕНП = ТаблицаРасходовНаУплатуСтраховыхВзносов.ВыгрузитьКолонку("ДатаПринятияЕНП");
	ДатыПринятияЕНП = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДатыПринятияЕНП);
	ПроизводственныйКалендарь = КалендарныеГрафики.ОсновнойПроизводственныйКалендарь();
	Если ЗначениеЗаполнено(ПроизводственныйКалендарь) Тогда
		ДатыОплаты = КалендарныеГрафики.ДатыБлижайшихРабочихДней(
			ПроизводственныйКалендарь,
	...
	Иначе
		ДатыОплаты = Новый Соответствие;
		Для Каждого Элемент Из ДатыПринятияЕНП Цикл
			ДатыОплаты.Вставить(Элемент, Элемент);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Расход Из ТаблицаРасходовНаУплатуСтраховыхВзносов Цикл
		...
		Если ЗначениеЗаполнено(Расход.ДатаПринятияЕНП) Тогда
			ДатаСУчетомСдвига = ДатыОплаты[Расход.ДатаПринятияЕНП];
			Если Не ЗначениеЗаполнено(ДатаСУчетомСдвига) Тогда
				ДатаСУчетомСдвига = Расход.ДатаПринятияЕНП;
			КонецЕсли;
			ПросроченныйПлатеж = НачалоМесяца(ДатаСУчетомСдвига) < НачалоМесяца(Расход.ДатаДокументаОплаты);
			ПериодЗаписи = ?(ПросроченныйПлатеж, Расход.ДатаДокументаОплаты, ДатаСУчетомСдвига);
		Иначе
			ПериодЗаписи = Реквизиты.Период;
		КонецЕсли;
...
Процедура УстановитьПараметрыЗапросаРасходыУменьшающиеНалог(Запрос, Реквизиты)
	...
	Запрос.УстановитьПараметр("ПлательщикЕНП",             Реквизиты.ПлательщикЕНП);
	Запрос.УстановитьПараметр("ВидДвижения",               ВидДвиженияНакопления.Расход);
	Запрос.УстановитьПараметр("ДатаПереходаНаЕНП",         НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж());
	...
	МассивСчетов = Новый Массив;
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ФСС_НСиПЗ); // 69.11 - взносы травматизм
	Запрос.УстановитьПараметр("СчетаТравматизм",
		БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(МассивСчетов, УсловияОтбораСубсчетов));
	...
	Если Реквизиты.ПлательщикЕНВД Или Реквизиты.ПрименяетсяПСН Тогда
...
Функция ТекстЗапросаОперацииПоВзносамСФОТ(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА ХозрасчетныйОстаткиИОбороты.Период >= &ДатаПереходаНаЕНП
	|				И НЕ ХозрасчетныйОстаткиИОбороты.Счет В (&СчетаТравматизм)
	|			ТОГДА 0
	|		ИНАЧЕ ХозрасчетныйОстаткиИОбороты.СуммаНачальныйОстатокДт
	|	КОНЕЦ КАК НачальнаяПереплата,
	|	ХозрасчетныйОстаткиИОбороты.СуммаНачальныйОстатокКт КАК НачальнаяЗадолженность,
	|	ВЫБОР
	|		КОГДА &ПлательщикЕНП
	|			ТОГДА ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ХозрасчетныйОстаткиИОбороты.Регистратор) = ТИП(Документ.СписаниеСРасчетногоСчета)
	|								И ХозрасчетныйОстаткиИОбороты.Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф)
	|							ИЛИ ХозрасчетныйОстаткиИОбороты.Счет В (&СчетаТравматизм)
	...
	|	КОНЕЦ КАК Уплата,
	|	ВЫБОР
	|		КОГДА ХозрасчетныйОстаткиИОбороты.Регистратор ССЫЛКА Документ.ОперацияПоЕдиномуНалоговомуСчету
	|			ТОГДА 0
	...
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаПризнанияЕНП
	...
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(РасчетыПоНалогамНаЕдиномНалоговомСчете.СрокУплаты, МЕСЯЦ, -1), МЕСЯЦ),
	|	0,
	|	0,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СрокиУплаты КАК ВТ_СрокиУплаты
	|		ПО (НАЧАЛОПЕРИОДА(РасчетыПоНалогамНаЕдиномНалоговомСчете.СрокУплаты, МЕСЯЦ) = ВТ_СрокиУплаты.МесяцУплаты)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО РасчетыПоНалогамНаЕдиномНалоговомСчете.Организация = ДанныеПервичныхДокументов.Организация
	|			И РасчетыПоНалогамНаЕдиномНалоговомСчете.ПлатежныйДокумент = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	|	РасчетыПоНалогамНаЕдиномНалоговомСчете.Период МЕЖДУ &НачалоПериодаДата И &КонецПериодаДата
	|	И РасчетыПоНалогамНаЕдиномНалоговомСчете.Организация = &Организация
	|	И РасчетыПоНалогамНаЕдиномНалоговомСчете.СчетУчета В(&СчетаВзносовСФОТ)
	...
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(СведенияОбУплатеНалогов.Ссылка.Дата, МЕСЯЦ, -1), МЕСЯЦ),
	|	0,
	|	0,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО СведенияОбУплатеНалогов.Ссылка.Организация = ДанныеПервичныхДокументов.Организация
	|			И СведенияОбУплатеНалогов.Ссылка = ДанныеПервичныхДокументов.Документ
	|ГДЕ
	|	СведенияОбУплатеНалогов.Ссылка.Дата МЕЖДУ &НачалоПериодаДата И &КонецПериодаДата
	|	И СведенияОбУплатеНалогов.Ссылка.Организация = &Организация
	|	И СведенияОбУплатеНалогов.СчетУчета В(&СчетаВзносовСФОТ)
...
Функция ТекстЗапросаОперацииВыплатыЗарплаты(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	Возврат УчетЗарплаты.ТекстЗапросаОперацииВыплатыЗарплаты(Реквизиты.ВедетсяУчетПоРаботникам);
...
Функция ТекстЗапросаРасходыПоВзносам(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	|	ВТ_ОперацииПоВзносам.СчетВзноса КАК СчетВзноса,
	|	ВТ_ОперацииПоВзносам.ДокументРасхода КАК ДокументРасхода,
	|	ВТ_ОперацииПоВзносам.ПериодНачисления КАК ПериодНачисления,
	...
	|	ВТ_ОперацииПоВзносам.ДокументНомер КАК ДокументНомер,
	|	ВТ_ОперацииПоВзносам.ДокументДата КАК ДокументДата,
	...
	|	ВТ_ОперацииПоВзносам.ДатаПризнанияЕНП КАК ДатаПризнанияЕНП
	|ИЗ
	|	ВТ_ОперацииПоВзносам КАК ВТ_ОперацииПоВзносам
...
Функция ТекстЗапросаРасходыПоБольничным(НомераТаблиц, ТаблицыДанных, Реквизиты)
	...
	РасчетПослеОтменыЕНВД = Реквизиты.НачалоПериодаАнализаРасходов >= УчетЕНВДКлиентСервер.ДатаОтменыЕНВД();
	УпрощенныйУчетБольничных =
		Реквизиты.Период >= УчетУСНКлиентСервер.ДатаНачалаИспользованияУпрощенногоУчетаРасходовПоБольничным();
	...
	Если УпрощенныйУчетБольничных Тогда
		...
		Если Реквизиты.ВедетсяУчетБольничных Тогда
			...
			НомераТаблиц.Вставить("ВременнаяТаблицаНачисленияБольничных", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаВыплатыБольничных", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаНачисленияИВыплатыБольничных", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("РасходыПоБольничным", НомераТаблиц.Количество());
			
			ТекстЗапроса = УчетЗарплаты.ТекстЗапросаРасходыПоБольничнымПриУчетеЗарплатыСредствамиБухгалтерии(
				Реквизиты.ВедетсяУчетПоРаботникам);
			
		Иначе
			
			НомераТаблиц.Вставить("ВременнаяТаблицаСтатьиЗатратНаБольничные", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаНачисленияБольничных", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаВыплатыИтоги", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаНачисленияИВыплаты", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("ВременнаяТаблицаРанееПризнанныеРасходы", НомераТаблиц.Количество());
			НомераТаблиц.Вставить("РасходыПоБольничным", НомераТаблиц.Количество());
			
			ТекстЗапроса = УчетЗарплаты.ТекстЗапросаРасходыПоБольничнымПриУчетеЗарплатыВоВнешнейПрограмме();
			
		КонецЕсли;
		
	ИначеЕсли Реквизиты.ВедетсяУчетБольничных Или РасчетПослеОтменыЕНВД Тогда
		...
		|				Организация = &Организация
		|					И СубконтоДт1 ССЫЛКА Справочник.СтатьиЗатрат
		|					И &УсловиеПоСтатьеЗатрат) КАК ХозрасчетныйОборотыДтКт
		|	ГДЕ
		...
		|				Организация = &Организация
		|					И СубконтоДт1 ССЫЛКА Справочник.ПрочиеДоходыИРасходы
		|					И &УсловиеПоПрочимДоходамИРасходам) КАК ХозрасчетныйОборотыДтКт
		|	ГДЕ
		|		&УсловиеПоРегистратору) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.ПериодНачисления,
		...
		УсловиеПоРегистратору = ?(Реквизиты.ВедетсяУчетБольничных,
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоРегистратору", УсловиеПоРегистратору);
		
		УсловиеПоСтатьеЗатрат = ?(Реквизиты.ВедетсяУчетБольничных,
			"ВЫРАЗИТЬ(СубконтоДт1 КАК Справочник.СтатьиЗатрат).ВидДеятельностиДляНалоговогоУчетаЗатрат В (&ВидыДеятельностиУчетаЗатрат)",
			"ВЫРАЗИТЬ(СубконтоДт1 КАК Справочник.СтатьиЗатрат) В
				|(ВЫБРАТЬ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоСтатьеЗатрат", УсловиеПоСтатьеЗатрат);
		
		УсловиеПоПрочимДоходамИРасходам = ?(Реквизиты.ВедетсяУчетБольничных,
			"ВЫРАЗИТЬ(СубконтоДт1 КАК Справочник.ПрочиеДоходыИРасходы).ВидДеятельностиДляНалоговогоУчетаЗатрат В (&ВидыДеятельностиУчетаЗатрат)",
			"ВЫРАЗИТЬ(СубконтоДт1 КАК Справочник.ПрочиеДоходыИРасходы) В
				|(ВЫБРАТЬ
				|	СтатьиЗатратПоЗарплате.СтатьяЗатрат КАК СтатьяЗатрат
				|ИЗ
				|	РегистрСведений.СтатьиЗатратПоЗарплате КАК СтатьиЗатратПоЗарплате
				|ГДЕ
				|	СтатьиЗатратПоЗарплате.ВидСтатьиЗатрат = ЗНАЧЕНИЕ(Перечисление.ВидыДополнительныхСтатейЗатрат.ПособияЗаСчетРаботодателя))");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоПрочимДоходамИРасходам", УсловиеПоПрочимДоходамИРасходам);
