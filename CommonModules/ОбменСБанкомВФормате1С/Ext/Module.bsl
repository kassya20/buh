...
Функция ПрочитатьФайл(ИмяФайла, Кодировка, ОдиночныйРазделительСтрок)
	...
	Пока Истина Цикл
		...
		ИначеЕсли Не ПустаяСтрока(Секция.ПредыдущийЭлемент) Тогда
			...
			Если Не Секция.Данные.Свойство(Секция.ПредыдущийЭлемент)
				И Секция.Описание.МногострочныеЭлементы[Секция.ПредыдущийЭлемент] <> Неопределено Тогда
				Секция.Данные.Вставить(Секция.ПредыдущийЭлемент, ТекстСтрокиФайла);
			Иначе
...
Функция НовыйДанныеИзБанка(ВерсияФормата = "1.03") Экспорт
	...
	ТипНомерСчета     = БанковскиеПравила.ТипЦифровойНомерСчета();
	ТипНомерКорСчета  = БанковскиеПравила.ТипЦифровойНомерСчета(); // Допускается передавать номер счета контрагента в национальном, международном, цифровом формате
	...
	ДанныеИзБанка.Документы.Колонки.Добавить("ИдентификаторДокумента", ОбщегоНазначения.ОписаниеТипаСтрока(36));
	
	// Используется при загрузке операций по счету цифрового рубля
	ДанныеИзБанка.Документы.Колонки.Добавить("ВидОперацииЦР", ТипСтрока);
	
	// Используется при обмене с сервисом АУСН
	ДанныеИзБанка.Документы.Колонки.Добавить("ИдентификаторАУСН", ОбщегоНазначения.ОписаниеТипаСтрока(64));
	ДанныеИзБанка.Документы.Колонки.Добавить("РазметкаАУСН", Новый ОписаниеТипов("ТаблицаЗначений")); // См. НовыйРазметкаАУСН()
	ДанныеИзБанка.Документы.Колонки.Добавить("ТипДокументаАУСН", Новый ОписаниеТипов("ПеречислениеСсылка.ТипыДокументовАУСН"));
	ДанныеИзБанка.Документы.Колонки.Добавить("РасчетыВВалюте", Новый ОписаниеТипов("Структура")); // См. НовыйРасчетыВВалюте()
...
//
Функция НовыйУчастники() Экспорт
...
Функция ОписаниеФайлаОбмена() Экспорт
...
Функция ЭлементыРеквизитовУчастника() Экспорт
...
Функция ЭлементыРеквизитовСчетаУчастника() Экспорт
	...
	ЭлементыРеквизитовСчетаУчастника.Добавить("НомерСчетаЦифровой");
	
	Возврат ЭлементыРеквизитовСчетаУчастника;
	
КонецФункции

Функция ЭлементыРеквизитовПлатежаВБюджет() Экспорт
...
// Данные разметки, загружаемой из банка при подключении к сервису АУСН.
//
// Возвращаемое значение:
//  ТаблицаЗначений - данные разметки.
Функция НовыйРазметкаАУСН() Экспорт
	
	ТипСумма = Метаданные.ОпределяемыеТипы.ДенежнаяСуммаЛюбогоЗнака.Тип;
	
	РазметкаАУСН = Новый ТаблицаЗначений;
	РазметкаАУСН.Колонки.Добавить("ПризнакНалоговойБазы", Новый ОписаниеТипов("ПеречислениеСсылка.ПризнакиНалоговойБазыАУСН"));
	РазметкаАУСН.Колонки.Добавить("КатегорияОперации", Новый ОписаниеТипов("ПеречислениеСсылка.КатегорииОперацийАУСН"));
	РазметкаАУСН.Колонки.Добавить("Сумма", ТипСумма);
	РазметкаАУСН.Колонки.Добавить("ВалютнаяСумма", ТипСумма);
	Возврат РазметкаАУСН;
	
КонецФункции

// Конструктор таблицы, содержащей информацию о расчетах в валюте, используемой для последующего
// заполнения табличной части РасшифровкаПлатежа документов ПоступлениеНаРасчетныйСчет и СписаниеСРасчетногоСчета
//
// Возвращаемое значение:
//  ТаблицаЗначений
//
Функция НовыйРасчетыВВалюте() Экспорт
	
	РасчетыВВалюте = Новый Структура;
	
	РасчетыВВалюте.Вставить("ВалютнаяСумма", 0);
	РасчетыВВалюте.Вставить("Сумма", 0);
	РасчетыВВалюте.Вставить("Валюта", Справочники.Валюты.ПустаяСсылка());
	
	Возврат РасчетыВВалюте;
	...
	ИначеЕсли Секция.Описание.Имя = "Документ" Тогда
		...
		Для Каждого Участник Из Участники Цикл
			...
			Если РеквизитыУчастника.Свойство("НаименованиеДетально") И Не ПустаяСтрока(РеквизитыУчастника.НаименованиеДетально) Тогда
				...
				Если ЭлементыНаименования.Количество() > 1
					И Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ЭлементыНаименования[1]) Тогда
			...
			ИсправитьОшибкиУчастникаСчет(СтрокаДанныхИзБанка, Участник);
...
Процедура РазделитьСтрокуФайла(Строка, Тег, Значение)
	...
	Если Не ПустаяСтрока(Значение) Тогда
		...
		Если ОбработкаСтрокиXML.НайтиНедопустимыеСимволыXML(Значение) <> 0 Тогда
...
Процедура ОчиститьТолькоНулиВСтроке(Значение) Экспорт
...
Процедура ИсправитьОшибкиУчастникаСчет(СтрокаДанныхИзБанка, Участник) Экспорт
	...
	Счет = СтрокаДанныхИзБанка[Участник + "Счет"];
	Если Не ПустаяСтрока(Счет)
		И (Не БанковскиеПравила.СтрокаСоответствуетФорматуБанковскогоСчета(Счет)
		Или Не СтрокаДанныхИзБанка[Участник + "Реквизиты"].Свойство("Наименование")) Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли БанковскиеПравила.ЭтоСчетПереводовФизическимЛицам(БалансовыйСчет)
		И СтрЗаканчиваетсяНа(СтрокаДанныхИзБанка[Участник + "Счет"], "0") Тогда
		Проблема = "ПредположительноФиктивныйСчетФизЛица";
	ИначеЕсли ПустаяСтрока(Счет) Тогда
		Проблема = "СчетНеЗаполнен";
	Иначе
		Возврат;
	КонецЕсли;
	
	НаименованиеУчастника = "";
	Если СтрокаДанныхИзБанка[Участник + "Реквизиты"].Свойство("Наименование") Тогда
		НаименованиеУчастника = СтрокаДанныхИзБанка[Участник + "Реквизиты"].Наименование;
	КонецЕсли;
	
	ПризнакРС            = "р/с";
	ПризнакРСБезСлеша    = "рс";
	ПризнакРСлат         = "p/c";
	ПризнакРСлатБезСлеша = "pc";
	НаименованиеУчастника = СтрЗаменить(НаименованиеУчастника, ПризнакРС,    " " +  ПризнакРСБезСлеша   + " ");
	НаименованиеУчастника = СтрЗаменить(НаименованиеУчастника, ПризнакРСлат, " " + ПризнакРСлатБезСлеша + " ");
	
	СловаУчастника = СтрРазделить(НаименованиеУчастника, " .:()/'", Ложь);
	
	НомерСчета = "";
	ПредыдущееСлово = "";
	ПлатежОтНерезидента = Ложь;
	Для Каждого ВозможныйНомерСчета Из СловаУчастника Цикл
		
		Если Проблема = "РасчетыФилиалаБанкаСКлиентамиБанка" Или Проблема = "РасчетыЧерезБанкКорреспондент"
			Или Проблема = "ПредположительноФиктивныйСчетФизЛица"
			Или Проблема = "СчетНеЗаполнен"
			Или Проблема = "РасчетыЧерезБанкНерезидент" Тогда
			
			НомерСчетаВНижнемРегистре  = НРег(ВозможныйНомерСчета);
			НомерСчетаВВерхнемРегистре = ВРег(НомерСчетаВНижнемРегистре);
			
			Если БанковскиеПравила.СтрокаСоответствуетФорматуIBAN(НомерСчетаВВерхнемРегистре)
				И БанковскиеПравила.ПроверитьКонтрольныйКлючIBAN(НомерСчетаВВерхнемРегистре) Тогда
				ПлатежОтНерезидента = Проблема = "РасчетыЧерезБанкНерезидент";
				НомерСчета = НомерСчетаВНижнемРегистре;
				Прервать;
			ИначеЕсли БанковскиеПравила.СтрокаСоответствуетФорматуБанковскогоСчета(НомерСчетаВВерхнемРегистре) Тогда
				НомерСчета = НомерСчетаВНижнемРегистре;
				Прервать;
			ИначеЕсли Проблема <> "СчетНеЗаполнен"
				И БанковскиеПравила.СтрокаСоответствуетФорматуНациональногоНомераСчета(НомерСчетаВВерхнемРегистре)
				И (ПредыдущееСлово = ПризнакРСБезСлеша Или ПредыдущееСлово = ПризнакРСлатБезСлеша
					Или СтрНачинаетсяС(ПредыдущееСлово, "acc")) Тогда
				НомерСчета = НомерСчетаВНижнемРегистре;
				Прервать;
			КонецЕсли;
			
			ПредыдущееСлово = НРег(ВозможныйНомерСчета);
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(НомерСчета) И Проблема <> "СчетНеЗаполнен" Тогда
		НаборШаблонов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПризнакРС);
		Если СтрНайти(НаименованиеУчастника, ПризнакРС) > 0 Тогда
			НомерСчета = ЗагрузкаВыпискиПоБанковскомуСчету.ВыделитьНомерСчетаИзТекста(НаименованиеУчастника, НаборШаблонов);
		ИначеЕсли СтрНайти(СтрокаДанныхИзБанка.НазначениеПлатежа, ПризнакРС) > 0 Тогда
			НомерСчета = ЗагрузкаВыпискиПоБанковскомуСчету.ВыделитьНомерСчетаИзТекста(
				СтрокаДанныхИзБанка.НазначениеПлатежа, НаборШаблонов);
		КонецЕсли;
		
		НаборШаблонов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПризнакРСлат);
		Если СтрНайти(НаименованиеУчастника, ПризнакРСлат) > 0 Тогда
			НомерСчета = ЗагрузкаВыпискиПоБанковскомуСчету.ВыделитьНомерСчетаИзТекста(НаименованиеУчастника, НаборШаблонов);
		ИначеЕсли СтрНайти(СтрокаДанныхИзБанка.НазначениеПлатежа, ПризнакРСлат) > 0 Тогда
			НомерСчета = ЗагрузкаВыпискиПоБанковскомуСчету.ВыделитьНомерСчетаИзТекста(
				СтрокаДанныхИзБанка.НазначениеПлатежа, НаборШаблонов);
		КонецЕсли;
	КонецЕсли;
	...
	СтрокаДанныхИзБанка[Участник + "Счет"] = ВРег(НомерСчета);
	...
	СтрокаДанныхИзБанка[Участник + "Реквизиты"].Удалить("БИК");
	
	ИННУчастника = СтрокаДанныхИзБанка[Участник + "ИНН"];
	Если ПлатежОтНерезидента И ЗначениеЗаполнено(ИННУчастника) Тогда
		Результат = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(ИННУчастника, Истина);
		Если Результат.СоответствуетТребованиям Тогда
			КодИФНС = Лев(ИННУчастника, 4);
			КодПричиныПостановки = 0;
			РеквизитыУчастника = СтрокаДанныхИзБанка[Участник + "Реквизиты"];
			ЕстьКПП = РеквизитыУчастника.Свойство("КПП") И СтрДлина(СокрЛП(РеквизитыУчастника.КПП)) = 9;
			Если ЕстьКПП Тогда
				КодПричиныПостановки = Число(Сред(РеквизитыУчастника.КПП, 5, 2));
			КонецЕсли;
			
			// С 01.01.2005 ИНН иностранного юридического лица всегда начинается с "9909"
			// см. https://www.nalog.gov.ru/rn77/fl/interest/inn/
			ИНННерезидента = КодИФНС = "9909"
				Или СтрДлина(ИННУчастника) = 5; // это код иностранной организации (КИО);
			КППНерезидента = КодПричиныПостановки > 50; // для иностранной организации от 51 до 99
			
			Если Не ИНННерезидента Тогда
				// Платеж от нерезидента через корр.счет в российском банке и в ИНН контрагента указан ИНН банка.
				СтрокаДанныхИзБанка[Участник + "ИНН"] = "";
			КонецЕсли;
			
			Если ЕстьКПП И Не КППНерезидента Тогда
...
Функция НовыйПротоколЧтенияФайла() Экспорт
...
Процедура ЗаписатьВПротокол(Протокол, НомерСтрокиФайла, КодСобытия) Экспорт
