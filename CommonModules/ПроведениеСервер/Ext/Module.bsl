...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчисткаРегистраПроверенныхДокументов", Истина);
	Методы.Вставить("ПереключитьОтложенноеПроведениеВФоне", Истина);
	Методы.Вставить("ПереключитьОтложенноеПроведениеПоПериодамВФоне", Истина);
...
Процедура ПодготовитьНаборыЗаписейКПроведению(Объект, ВыборочноОчищатьРегистры = Истина) Экспорт
	...
	НеВыполнятьКонтрольУдаляемых = Объект.ДополнительныеСвойства.Свойство("НеВыполнятьКонтрольУдаляемых");
...
Процедура УстановитьЗаписьОчищаемыхНаборовЗаписей(Объект) Экспорт 
	...
	ЗапомнитьПризнакПроверкиРеквизитов(Объект);
...
//
Функция РегистрыТребующиеОчисткиДляТипа(РегистрыТребующиеОчисткиПоДокументам, МетаданныеДокумента) Экспорт
...
Функция СтруктураВидовУчета() Экспорт
	...
	СтруктураВидовУчета.Вставить("СтраховыеВзносы", Ложь);
	...
	Если Движения = Неопределено Тогда
		...
		Если СтруктураВидовУчета.ДанныеДляРасчетаСреднего Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
				МодульУчетСреднегоЗаработка = ОбщегоНазначения.ОбщийМодуль("УчетСреднегоЗаработка");
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивРегистров, МодульУчетСреднегоЗаработка.РегистрыСреднегоЗаработка(), Истина);
			КонецЕсли;
		...
		Если СтруктураВидовУчета.СтраховыеВзносы Тогда
			...
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивРегистров, УчетСтраховыхВзносов.РегистрыУчетаСтраховыхВзносов(), Истина);
			//Зависимые регистры других подсистем
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивРегистров, УчетПособийСоциальногоСтрахования.РегистрыУчетаПособийСоциальногоСтрахования(), Истина);
								
		КонецЕсли;
		
		СтруктураВидовУчета.МассивРегистров = МассивРегистров;
...
Функция ИспользуетсяОтложенноеПроведение(Организация, Период, ПроверятьЭквайрингПриУСН = Истина) Экспорт
	...
	Если УчетнаяПолитика.ПрименяетсяАУСН(Организация, Период) Тогда
		// При применении Автоматизированной УСН отложенное проведение не поддерживается.
		Возврат Ложь;
	КонецЕсли;
	...
	Если УчетнаяПолитика.ПлательщикТуристическогоНалога(Организация, Период) Тогда
		// Для плательщиков туристического налога отложенное проведение не поддерживается.
		Возврат Ложь;
	КонецЕсли;
...
Функция ПериодыИспользованияОтложенногоПроведения(Организации = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(НастройкиСистемыНалогообложения.ПрименяетсяУСНДоходыМинусРасходы, ЛОЖЬ) КАК
	...
	|	ЕСТЬNULL(НастройкиСистемыНалогообложения.ПрименяетсяАУСН, ЛОЖЬ) КАК ПрименяетсяАУСН,
	|	ЛОЖЬ КАК СложныйУчетНДС,
	|	НЕОПРЕДЕЛЕНО КАК СпособОценкиМПЗ,
	|	НЕОПРЕДЕЛЕНО КАК СпособОценкиТоваровВРознице,
	|	ЕСТЬNULL(НастройкиСистемыНалогообложения.ПрименяетсяОсобыйПорядокНалогообложения, ЛОЖЬ) КАК
	...
	|	МАКСИМУМ(ВТ_Настройки.ПрименяетсяАУСН) КАК ПрименяетсяАУСН,
	...
	|	ВТ_НастройкиУчета.ПрименяетсяАУСН,
	...
	|	ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ВТ_ПериодыДействияУчетнойПолитики.ПериодСледующий, ДАТАВРЕМЯ(2999, 12, 31)),
	|		ДЕНЬ), СЕКУНДА, -1) КАК ДатаОкончания
	|ИЗ
	...
	|	И
	|		ВТ_ПериодыДействияУчетнойПолитики.СпособОценкиТоваровВРознице = ЗНАЧЕНИЕ(Перечисление.СпособыОценкиТоваровВРознице.ПоСтоимостиПриобретения)
	|	И ВТ_ПериодыДействияУчетнойПолитики.ПрименяетсяАУСН = ЛОЖЬ)
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	ДатаНачала
	|ИТОГИ
...
Функция ПричиныНепримененияОтложенногоПроведения() Экспорт
	...
	|
	|ВЫБРАТЬ
	|	НастройкиУчетаТуристическогоНалога.Период,
	|	НастройкиУчетаТуристическогоНалога.Организация
	|ИЗ
	|	РегистрСведений.НастройкиУчетаТуристическогоНалога КАК НастройкиУчетаТуристическогоНалога
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	ВТ_Периоды.Период,
	|	ВТ_Периоды.Организация,
	|	МАКСИМУМ(НастройкиУчетаТуристическогоНалога.Период) КАК ПериодБлижайший
	|ПОМЕСТИТЬ ВТ_НастройкиУчетаТуристическогоНалога
	|ИЗ
	|	ВТ_Периоды КАК ВТ_Периоды
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаТуристическогоНалога КАК НастройкиУчетаТуристическогоНалога
	|		ПО ВТ_Периоды.Организация = НастройкиУчетаТуристическогоНалога.Организация
	|		И ВТ_Периоды.Период >= НастройкиУчетаТуристическогоНалога.Период
	|СГРУППИРОВАТЬ ПО
	|	ВТ_Периоды.Период,
	|	ВТ_Периоды.Организация
	|;
	...
	|	ЕСТЬNULL(НастройкиСистемыНалогообложения.ПрименяетсяУСНПатент, ЛОЖЬ) КАК ПрименяетсяУСНПатент,
	|	ЕСТЬNULL(НастройкиСистемыНалогообложения.ПрименяетсяАУСН, ЛОЖЬ) КАК ПрименяетсяАУСН,
	|	ЛОЖЬ КАК ПлательщикТуристическогоНалога,
	|	ЛОЖЬ КАК СложныйУчетНДС,
	|	НЕОПРЕДЕЛЕНО КАК СпособОценкиМПЗ,
	|	НЕОПРЕДЕЛЕНО КАК СпособОценкиТоваровВРознице
	|ПОМЕСТИТЬ ВТ_Настройки
	...
	|
	|ВЫБРАТЬ
	|	ВТ_НастройкиУчетаТуристическогоНалога.Период,
	|	ВТ_НастройкиУчетаТуристическогоНалога.Организация,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЛОЖЬ,
	|	ЕСТЬNULL(НастройкиУчетаТуристическогоНалога.ПлательщикТуристическогоНалога, ЛОЖЬ),
	|	ЛОЖЬ,
	|	НЕОПРЕДЕЛЕНО,
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	ВТ_НастройкиУчетаТуристическогоНалога КАК ВТ_НастройкиУчетаТуристическогоНалога
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиУчетаТуристическогоНалога КАК НастройкиУчетаТуристическогоНалога
	|		ПО ВТ_НастройкиУчетаТуристическогоНалога.Организация = НастройкиУчетаТуристическогоНалога.Организация
	|		И ВТ_НастройкиУчетаТуристическогоНалога.ПериодБлижайший = НастройкиУчетаТуристическогоНалога.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(ВТ_Настройки.ПрименяетсяУСНПатент) КАК ПрименяетсяУСНПатент,
	|	МАКСИМУМ(ВТ_Настройки.ПрименяетсяАУСН) КАК ПрименяетсяАУСН,
	|	МАКСИМУМ(ВТ_Настройки.ПлательщикТуристическогоНалога) КАК ПлательщикТуристическогоНалога,
	|	МАКСИМУМ(ВТ_Настройки.СложныйУчетНДС) КАК СложныйУчетНДС,
	|	МАКСИМУМ(ВТ_Настройки.СпособОценкиМПЗ) КАК СпособОценкиМПЗ,
	|	МАКСИМУМ(ВТ_Настройки.СпособОценкиТоваровВРознице) КАК СпособОценкиТоваровВРознице
	|ИЗ
	...
	|	ВТ_Настройки.Период,
	|	ВТ_Настройки.Организация,
	|	ВТ_Настройки.Организация.Наименование
	|УПОРЯДОЧИТЬ ПО
	...
	ТаблицаПериоды.Колонки.Добавить("ПрименяетсяАУСН",                       ОписаниеТипаБулево);
	ТаблицаПериоды.Колонки.Добавить("СпособОценкиМПЗФИФО",                   ОписаниеТипаБулево);
	ТаблицаПериоды.Колонки.Добавить("ПлательщикТуристическогоНалога",        ОписаниеТипаБулево);
	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ПлательщикНДФЛ
				...
				ИЛИ Выборка.ПрименяетсяАУСН
				ИЛИ СпособОценкиМПЗФИФО
				ИЛИ Выборка.ПлательщикТуристическогоНалога
				...
				СтрокаТаблицы.ПрименяетсяАУСН = СтрокаТаблицы.ПрименяетсяАУСН
					Или Выборка.ПрименяетсяАУСН;
				...
				СтрокаТаблицы.ПлательщикТуристическогоНалога = СтрокаТаблицы.ПлательщикТуристическогоНалога
					ИЛИ Выборка.ПлательщикТуристическогоНалога;
		...
		Если ТаблицаПериоды.Количество() > 0 Тогда
			...
			Для Каждого СтрокаТаблицы Из ТаблицаПериоды Цикл
				...
				Если СтрокаТаблицы.ПрименяетсяАУСН Тогда
					МассивСтрок.Добавить(ТекстОтступ + НСтр("ru = 'Применяет Автоматизированную УСН'"));
				КонецЕсли;
				...
				Если СтрокаТаблицы.ПлательщикТуристическогоНалога Тогда
					МассивСтрок.Добавить(ТекстОтступ + НСтр("ru = 'Плательщик туристического налога'"));
				КонецЕсли;
...
#Область СчитатьРеквизитыПроверенными

// Устанавливает доп.свойство-признак, чтобы была выполнена проверка реквизитов документа.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - записываемый документ.
//  ПараметрыЗаписи - Структура - стандартные ключи одноименного параметра из обработчика ПередЗаписьюНаСервере у формы.
//
Процедура УстановитьПризнакПроверкиРеквизитов(Отказ, ДокументОбъект, ПараметрыЗаписи) Экспорт

	Если Отказ Или ПараметрыЗаписи.РежимЗаписи <> РежимЗаписиДокумента.Проведение Тогда
		// Если сейчас отменяем проведение или только записываем, то факт выполнения проверок не имеет значения.
		Возврат;
...
// Очищает запись о факте проведения документа.
//
// Параметры:
//  Источник     - ДокументОбъект - записываемый документ.
//  Отказ        - Булево - признак отказа от записи.
//
Процедура ОчиститьРегистрПроверенныхДокументовПриЗаписиДокумента(Источник, Отказ) Экспорт
	
	// Нужно обеспечить отсутствие записи в регистре ПроверенныеДокументы, если последнее проведение выполнялось без проверки заполнения
	// реквизитов документа.
	
	Если Не Источник.Проведен Тогда
		// Если сейчас отменяем проведение или только записываем, то наличие записи в регистре не имеет значения.
		Возврат;
	КонецЕсли;
	
	Если Не Источник.ОбменДанными.Загрузка Тогда
		// Если сейчас идет обычное проведение, то добавлять или нет запись мы решаем в ОбработкаПроведения().
		// Если документ записывается в режиме "Загрузка=Истина", то необходимо удалить запись
		// в регистре, чтобы документ был проверен при следующем перепроведении в текущей базе.
		Возврат;
	КонецЕсли;
	
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Источник.ДополнительныеСвойства, "СчитатьРеквизитыПроверенными", Ложь) Тогда
		// Если установлено доп.свойство СчитатьРеквизитыПроверенными, то очищать запись не нужно.
		Возврат;
	КонецЕсли;
	
	ПроверенныеДокументы = РегистрыСведений.ПроверенныеДокументы.СоздатьНаборЗаписей();
	ПроверенныеДокументы.Отбор.Организация.Установить(Источник.Организация);
	ПроверенныеДокументы.Отбор.Документ.Установить(Источник.Ссылка);
	ПроверенныеДокументы.Записать(Истина);
	
КонецПроцедуры

// Запоминает факт проведения документа, при котором выполнялась проверка реквизитов.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - проводимый документ.
//
Процедура ЗапомнитьПризнакПроверкиРеквизитов(ДокументОбъект) Экспорт
	
	Если ДокументОбъект.Метаданные().Реквизиты.Найти("Организация") = Неопределено Тогда
		// Документ не связан с организацией, поэтому проверка заполнения при групповом перепроведении будет вызываться всегда.
		Возврат;
	КонецЕсли;
	
	// Смотрим как изменилось состояние проверенности документа, анализируя как было и как стало.
	// Как было = есть ли ранее добавленная запись в регистре.
	ПроверкаЗапрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ИСТИНА КАК СчитатьРеквизитыПроверенными
	|ИЗ
	|	РегистрСведений.ПроверенныеДокументы КАК ПроверенныеДокументы
	|ГДЕ
	|	ПроверенныеДокументы.Организация = &Организация
	|	И ПроверенныеДокументы.Документ = &ПроверенныйДокумент");
	ПроверкаЗапрос.УстановитьПараметр("Организация",         ДокументОбъект.Организация);
	ПроверкаЗапрос.УстановитьПараметр("ПроверенныйДокумент", ДокументОбъект.Ссылка);
	СчитатьРеквизитыПровереннымиРанее = Не ПроверкаЗапрос.Выполнить().Пустой();
	
	// Как стало = происходит ли текущая запись в результате действия, инициированного пользователем.
	СчитатьРеквизитыПровереннымиСейчас = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДокументОбъект.ДополнительныеСвойства, "СчитатьРеквизитыПроверенными", Ложь);
		
	Если СчитатьРеквизитыПровереннымиСейчас И СчитатьРеквизитыПровереннымиРанее
	 Или Не СчитатьРеквизитыПровереннымиСейчас И Не СчитатьРеквизитыПровереннымиРанее Тогда
		// Состояние не изменилось.
		Возврат;
	КонецЕсли;
	
	ПроверенныеДокументы = РегистрыСведений.ПроверенныеДокументы.СоздатьНаборЗаписей();
	Если Не ПроверенныеДокументы.Отбор.Документ.ТипЗначения.СодержитТип(ТипЗнч(ДокументОбъект.Ссылка)) Тогда
		// Документ не добавлен в список типов измерения Документ регистра сведений ПроверенныеДокументы.
		Возврат;
	КонецЕсли;
	
	// Вызывается при проведении документа, когда для документа и всех его движений уже отработала проверка запрета изменения.
	ПроверенныеДокументы.ДополнительныеСвойства.Вставить("ПропуститьПроверкуЗапретаИзменения", Истина);
	
	ПроверенныеДокументы.Отбор.Организация.Установить(ДокументОбъект.Организация);
	ПроверенныеДокументы.Отбор.Документ.Установить(ДокументОбъект.Ссылка);
	
	Если СчитатьРеквизитыПровереннымиСейчас Тогда
		// Установить признак.
		ФактПроверки = ПроверенныеДокументы.Добавить();
		ФактПроверки.Организация       = ДокументОбъект.Организация;
		ФактПроверки.Документ          = ДокументОбъект.Ссылка;
		ФактПроверки.ПериодРегистрации = НачалоМесяца(ДокументОбъект.Дата);
		ПроверенныеДокументы.Записать(Ложь);
...
// Обработчик подписки на событие ОбработкаПроведения() документов библиотеки "Зарплата и кадры (базовая)".
//
Процедура ЗапомнитьПризнакПроверкиРеквизитовОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	
	ЗапомнитьПризнакПроверкиРеквизитов(Источник);
...
//
Процедура ОчисткаРегистраПроверенныхДокументов(НачалоНепроверенногоПериода, Организация = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	НачалоПериодаНепроверенных = Макс(
		НачалоМесяца(НачалоНепроверенногоПериода),
		ОбщегоНазначенияБПСобытия.КорректныйПериодВводаДокументов().НачалоКорректногоПериода);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачалоПериодаНепроверенных", НачалоПериодаНепроверенных);
	
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ПроверенныеДокументы.ПериодРегистрации КАК ПериодРегистрации
	|ИЗ
	|	РегистрСведений.ПроверенныеДокументы КАК ПроверенныеДокументы
	|ГДЕ
	|	ПроверенныеДокументы.ПериодРегистрации >= &НачалоПериодаНепроверенных
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ОчищаемыеОрганизации = Справочники.Организации.ВсяОрганизация(Организация, НачалоНепроверенногоПериода);
		Запрос.УстановитьПараметр("Организация", ОчищаемыеОрганизации);
		УсловиеПоОрганизации = "ПроверенныеДокументы.Организация В(&Организация)";
		
	Иначе
		
		ОчищаемыеОрганизации = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.Организации.ПустаяСсылка());
	...
	// В другом узле могут быть записи за те месяцы, которых нет в текущем узле. Поэтому делаем очищающие записи по всем организациям
	// и по всем месяцам до текущего месяца. Следующие месяцы очищаем, если по ним есть записи в регистре.
	КонецПериодаНепроверенных = ?(Результат.Пустой(), '00010101', Результат.Выгрузить().Получить(0).ПериодРегистрации);
	КонецПериодаНепроверенных = НачалоМесяца(Макс(НачалоПериодаНепроверенных, ТекущаяДатаСеанса(), КонецПериодаНепроверенных));
	
	ВсегоМесяцев = Год(КонецПериодаНепроверенных) * 12 + Месяц(КонецПериодаНепроверенных)
		- Год(НачалоПериодаНепроверенных) * 12 - Месяц(НачалоПериодаНепроверенных) + 1;
	НомерМесяца = ВсегоМесяцев;
	
	ИмяСобытия = НСтр("ru = 'Считать реквизиты непроверенными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ИмяСобытия = ЗакрытиеМесяцаКлиентСервер.СобытиеЖурналаРегистрации(ИмяСобытия);
	ШаблонСообщенияПрогресса = НСтр("ru = 'Подготовка к перепроведению документов за %1'");
	
	ПроверенныеДокументы = РегистрыСведений.ПроверенныеДокументы.СоздатьНаборЗаписей();
	
	Попытка
		ПериодРегистрации = КонецПериодаНепроверенных;
		Пока ПериодРегистрации >= НачалоПериодаНепроверенных Цикл
			
			ПредставлениеТекущегоМесяца = Формат(ПериодРегистрации, "ДФ='ММММ гггг'");
			
			ПроцентВыполнения = 100 - Цел((НомерМесяца * 2 - 1) / (ВсегоМесяцев * 2) * 100);
			ДлительныеОперации.СообщитьПрогресс(ПроцентВыполнения, СтрШаблон(ШаблонСообщенияПрогресса, ПредставлениеТекущегоМесяца));
			
			ПроверенныеДокументы.Отбор.ПериодРегистрации.Установить(ПериодРегистрации);
			
			Для Каждого ОчищаемаяОрганизация Из ОчищаемыеОрганизации Цикл
				
				Если ЗначениеЗаполнено(ОчищаемаяОрганизация) Тогда
					
					ПроверенныеДокументы.Отбор.Организация.Установить(ОчищаемаяОрганизация);
					ТекстСообщения = НСтр("ru = 'Для документов за %1 по %2 включена проверка заполнения реквизитов при следующем групповом перепроведении'");
					ТекстСообщения = СтрШаблон(ТекстСообщения, ПредставлениеТекущегоМесяца, Строка(ОчищаемаяОрганизация));
					
				Иначе
					
					ТекстСообщения = НСтр("ru = 'Для документов за %1 включена проверка заполнения реквизитов при следующем групповом перепроведении'");
					ТекстСообщения = СтрШаблон(ТекстСообщения, ПредставлениеТекущегоМесяца);
				...
				ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация,
					Метаданные.РегистрыСведений.ПроверенныеДокументы, , ТекстСообщения);
	...
	Исключение
		...
		Если НачалоПериодаНепроверенных = КонецПериодаНепроверенных Тогда
			ТекстСообщения = НСтр("ru = 'За %1 не удалось включить проверку заполнения реквизитов документов при групповом перепроведении.
									|%2'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, 
				ПредставлениеТекущегоМесяца,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Иначе
			ТекстСообщения = НСтр("ru = 'За период с %1 по %2 не удалось включить проверку заполнения реквизитов документов при групповом перепроведении.
									|%3'");
			ТекстСообщения = СтрШаблон(ТекстСообщения,
				Формат(НачалоПериодаНепроверенных, "ДФ='ММММ гггг'"),
				ПредставлениеТекущегоМесяца,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецЕсли;
