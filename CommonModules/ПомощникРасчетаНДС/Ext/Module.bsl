...
Функция ДоступенАвтоматическийРасчетНДС(Организация, Период) Экспорт
	...
	Если РассчитыватьНДСВручную(Организация)
		...
		ИЛИ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС(Организация, Период)
		ИЛИ Обработки.ПодготовкаКВычетуНДСУчтенногоВСтоимости.НеобходимЗапускПомощника(Организация, Период) Тогда
...
Функция ФормироватьДекларациюВПомощнике(Организация, Период) Экспорт
	...
	Возврат (УчетнаяПолитика.ПлательщикНДС(Организация, КонецКвартала(Период))
		ИЛИ УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, КонецКвартала(Период)))
		И Период >= '20180101'
...
Функция ПричиныНедоступностиАвтоматическогоРасчета(Организация, Период) Экспорт
	...
	Если Обработки.ПодготовкаКВычетуНДСУчтенногоВСтоимости.НеобходимЗапускПомощника(Организация, Период) Тогда
		ПричиныНедоступности.Добавить("ЕстьНДСУчтенныйВСтоимости");
	КонецЕсли;
...
Процедура ОбновитьРасчет(Параметры) Экспорт
	...
	СоздаватьФЗКПродаж = Истина;
	Если УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(ПараметрыРасчетаНалога.Организация, ПараметрыРасчетаНалога.Период)
		И РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(ПараметрыРасчетаНалога.Организация, ПараметрыРасчетаНалога.Период) <> Перечисления.ВидыСтавокНДС.Общая Тогда
		СоздаватьФЗКПродаж = Ложь;
		ПараметрыРасчетаНалога.КоличествоОперацийКВыполнению = ПараметрыРасчетаНалога.КоличествоОперацийКВыполнению - 1;
	КонецЕсли;
	РегламентныеДокументы = РегламентныеДокументыЗаПериод(ПараметрыРасчетаНалога, СоздаватьФЗКПродаж);
	
	СформироватьСчетаФактурыНаАванс(ПараметрыРасчетаНалога);
	
	СформироватьСводныеСправки(ПараметрыРасчетаНалога);
...
Процедура ЗагрузитьРезультат(Форма, АдресХранилища) Экспорт
	...
	Если СоставРазделов.Уплата Тогда
		...
		КонецЕсли;
		Форма.Расчет.Очистить();
		ТаблицаРасчетов = Результат.ПоказателиДляУплаты.ТаблицаРасчет;
		Если ТаблицаРасчетов <> Неопределено Тогда
			Форма.Расчет.Загрузить(ТаблицаРасчетов);
		КонецЕсли;
		Форма.НалоговыйАгентНДС.Очистить();
		ТаблицаНалоговыйАгент = Результат.ПоказателиДляУплаты.ТаблицаНалоговыйАгент;
		Если ТаблицаНалоговыйАгент <> Неопределено Тогда
			Форма.НалоговыйАгентНДС.Загрузить(ТаблицаНалоговыйАгент);
		КонецЕсли;
		Форма.НалоговыйАгентНДСПоОтдельнымВидамТоваров.Очистить();
		ТаблицаНалоговыйАгентПоОтдельнымВидамТоваров = Результат.ПоказателиДляУплаты.ТаблицаНалоговыйАгентПоОтдельнымВидамТоваров;
		Если ТаблицаНалоговыйАгентПоОтдельнымВидамТоваров <> Неопределено Тогда
			Форма.НалоговыйАгентНДСПоОтдельнымВидамТоваров.Загрузить(ТаблицаНалоговыйАгентПоОтдельнымВидамТоваров);
		КонецЕсли;
		Форма.ТаможенныйСоюзНДС.Очистить();
		ТаблицаТаможенныйСоюз = Результат.ПоказателиДляУплаты.ТаблицаТаможенныйСоюз;
		Если ТаблицаТаможенныйСоюз <> Неопределено Тогда
			Форма.ТаможенныйСоюзНДС.Загрузить(ТаблицаТаможенныйСоюз);
		КонецЕсли;
		Форма.СуммаНалоговыйАгентНДС        = Форма.НалоговыйАгентНДСПоОтдельнымВидамТоваров.Итог("Сумма")
			+ Форма.НалоговыйАгентНДС.Итог("Сумма")
			+ Результат.ПоказателиДляУплаты.НалогНАКУплате; 
		Форма.СуммаТаможенныйСоюзНДС        = Форма.ТаможенныйСоюзНДС.Итог("Сумма");
		НалогКУплатеКоллекция = Новый Структура(
			"НалогКУплатеПошаговыйПомощник",
			Результат.ПоказателиДляУплаты.НалогКУплате + Результат.ПоказателиДляУплаты.НалогНАКУплате);
		ЗаполнитьЗначенияСвойств(Форма, НалогКУплатеКоллекция);
	КонецЕсли;
	
	Если СоставРазделов.Уведомление Тогда
		Форма.Уведомления.Очистить();
		ТаблицаУведомлений = Результат.ПоказателиДляУведомления.ТаблицаУведомления;
		Если ТаблицаУведомлений <> Неопределено Тогда
			Форма.Уведомления.Загрузить(ТаблицаУведомлений);
		КонецЕсли;
	КонецЕсли; 
	
	Если СоставРазделов.СверкаСНалогомНаПрибыль Тогда
		
		// см. Обработки.ПомощникРасчетаНДС.ПоказателиДляРасчета, СверкаСНалогомНаПрибыль
		СверкаНП = Результат.ПоказателиСверкаСНалогомНаПрибыль; 
		
		Если ТипЗнч(СверкаНП.Данные) = Тип("ТаблицаЗначений") Тогда
			ОшибкиНомер = 3300;
			Форма.ОшибкаСверкиСНалогомНаПрибыль = СверкаНП.Данные.Найти(ОшибкиНомер, "Номер").Значение;		
			СверкаНП.Данные = ПоместитьВоВременноеХранилище(СверкаНП.Данные, Форма.УникальныйИдентификатор);
			Форма.СверкаСНалогомНаПрибыль = СверкаНП;
		КонецЕсли;
...
Функция ПараметрыФормыПлатежногоДокументаПоСтроке(Форма, Идентификатор) Экспорт
	...
	ЕНП = Справочники.ВидыНалоговИПлатежейВБюджет.ЕдиныйНалоговыйПлатеж;
	КБКЕНП = Справочники.ВидыНалоговИПлатежейВБюджет.КБК(ЕНП);
	
	ОписаниеПлатежа = ВыполнениеЗадачБухгалтера.НовыйОписаниеПлатежа();
	ОписаниеПлатежа.Период                     = КонецКвартала(Форма.Объект.Период);
	ОписаниеПлатежа.Налог                      = ?(Форма.ПлательщикЕНП, ЕНП, СтрокаПлатежа.Налог);
	...
	ОписаниеПлатежа.ПоказательПериода          = ?(Форма.ПлательщикЕНП,
		ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение(), Форма.ПоказательПериода);
	ОписаниеПлатежа.Аванс                      = ?(Форма.ПлательщикЕНП, Ложь, СтрокаПлатежа.Аванс);
	ОписаниеПлатежа.КБК                        = ?(Форма.ПлательщикЕНП, КБКЕНП, СтрокаПлатежа.КБК);
	...
	ОписаниеПлатежа.ДатаДекларации             = ?(Форма.ПлательщикЕНП,
		ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение(), Форма.ДатаДекларации);
	ОписаниеПлатежа.ОснованиеПлатежа           = ?(Форма.ПлательщикЕНП,
		ПлатежиВБюджетКлиентСервер.НезаполненноеЗначение(), "");
...
Процедура НастроитьБлокОплата(Форма) Экспорт
	...
	Для НомерПлатежа = 1 По 3 Цикл
		Элементы["ДекорацияПлатеж" + НомерПлатежа].Заголовок = ПредставлениеПлатежа(Платежи[НомерПлатежа - 1]);
	...
	ТребуетсяОтправитьДокументыОплатыПоБанку = ТребуетсяОтправитьДокументыОплатыПоДиректБанку(Форма);
	ТребуетсяВыгрузитьДокументыОплаты = Не ТребуетсяОтправитьДокументыОплатыПоБанку И ТребуетсяВыгрузитьДокументыОплаты(Форма);
...
Процедура НастроитьБлокУведомление(Форма) Экспорт
	
	Элементы        = Форма.Элементы;
	Уведомления     = Форма.Уведомления;
	КоличествоСтрок = Уведомления.Количество();
	...
	УведомлениеПодготовлено = Ложь;
	
	Для НомерУведомления = 1 По 3 Цикл
		
		Если НомерУведомления > КоличествоСтрок Тогда
			ВидимостьУведомления  = Ложь;
		Иначе
			Уведомление = Уведомления[НомерУведомления - 1].Уведомление;
			Если ТипЗнч(Уведомление) = Тип("ДокументСсылка.ОперацияПоЕдиномуНалоговомуСчету") Тогда
				ПредставлениеУведомления = Документы.ОперацияПоЕдиномуНалоговомуСчету.ОписаниеДокумента(Уведомление);
			Иначе
				ПредставлениеУведомления = Документы.УведомлениеОбИсчисленныхСуммахНалогов.ОписаниеДокумента(Уведомление);
			КонецЕсли;
		
			Элементы["ДекорацияУведомление" + НомерУведомления].Заголовок      = ПредставлениеУведомления.Наименование;
			Элементы["ДекорацияСуммаУведомления" + НомерУведомления].Заголовок = ПредставлениеУведомления.Сумма;
			ВидимостьУведомления = ЗначениеЗаполнено(Уведомление);
		КонецЕсли;
		
		Элементы["ГруппаУведомление" + НомерУведомления].Видимость = ВидимостьУведомления;
		Если ВидимостьУведомления Тогда
			УведомлениеПодготовлено = Истина;
		КонецЕсли;
		
	КонецЦикла;
	
	Если Форма.Расчет.Количество() > Форма.ИндексОчередногоПлатежа
		И Форма.Уведомления.Количество() > Форма.ИндексОчередногоПлатежа Тогда
		ВидимостьКнопки = Не УведомлениеПодготовлено
		И ((Форма.Расчет[Форма.ИндексОчередногоПлатежа].Сумма <> 0
			И Не ЗначениеЗаполнено(Форма.Уведомления[Форма.ИндексОчередногоПлатежа].Уведомление))
			Или Форма.НалоговыйАгентНДС.Количество() <> 0);
	Иначе
		ВидимостьКнопки = Ложь;
	КонецЕсли;
	
	Форма.УведомлениеПодготовлено = УведомлениеПодготовлено;
	
	Элементы.ВыполнитьДействиеСформироватьУведомление.Видимость   = ВидимостьКнопки И ПодаетсяУведомлениеПоНалогуЗаПериод;
	Элементы.ВыполнитьДействиеСформироватьУведомление.Доступность = Форма.МожноСоздаватьУведомления;
	Элементы.ВыполнитьДействиеСформироватьОперациюПоЕНС.Видимость = ВидимостьКнопки И Не ПодаетсяУведомлениеПоНалогуЗаПериод;
	Элементы.ВыполнитьДействиеСформироватьОперациюПоЕНС.Доступность = Форма.МожноСоздаватьУведомления;
	
	Если ПодаетсяУведомлениеПоНалогуЗаПериод Тогда
		ТекстИнформацияУведомление = НСтр("ru = 'Подготовка уведомления'");
		Если Не ВидимостьКнопки И Не УведомлениеПодготовлено Тогда
			ТекстИнформацияУведомление  = НСтр("ru = 'Нет сумм налога к уплате. Уведомление не требуется.'");
		КонецЕсли;
	Иначе
		ТекстИнформацияУведомление = НСтр("ru = 'По НДС формировать уведомление по исчисленным налогам не нужно,
												|но для корректного ведения расчетов по налогам нужно
												|отразить их на едином налоговом счете.'");
		Если Не ВидимостьКнопки И Не УведомлениеПодготовлено Тогда
...
Функция ОписаниеТаблицыНалоги(Форма, Идентификатор) Экспорт
	
	СтрокаПлатежа = Форма.Расчет[Идентификатор];
	...
	НачалоПериода = ИнтерфейсыВзаимодействияБРОКлиентСервер.НачалоПериода(Форма.ПравилоУплаты.Периодичность, Форма.Срок);
	КонецПериода = ИнтерфейсыВзаимодействияБРОКлиентСервер.КонецПериода(Форма.ПравилоУплаты.Периодичность, Форма.Срок);
	
	РегистрацияВНалоговомОргане  = Справочники.Организации.РегистрацияВНалоговомОргане(Форма.Объект.Организация, КонецПериода);
	...
	Если СтрокаПлатежа.Сумма <> 0 Тогда
		...
		НоваяСтрока.СрокУплаты                  = ПредельныйСрокУплаты;
	...
	ОкруглятьДОЦелого = Форма.Срок < НастройкиУчетаКлиентСервер.ДатаПереходаНаЕдиныйНалоговыйПлатеж();
	
	СоответствиеТаблиц = Новый Соответствие;
	СоответствиеТаблиц.Вставить("НалоговыйАгентНДС", Новый Структура("СчетУчета, ТаблицаДанных", ПланыСчетов.Хозрасчетный.НДСНалоговогоАгента, ТаблицаНалоговыйАгентНДС));
	СоответствиеТаблиц.Вставить("ТаможенныйСоюзНДС", Новый Структура("СчетУчета, ТаблицаДанных", ПланыСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате, ТаблицаТаможенныйСоюзНДС));
	
	Для Каждого ЗаписьСоответствия Из СоответствиеТаблиц Цикл
		Если Форма[ЗаписьСоответствия.Ключ].Количество() <> 0 Тогда
			
			СчетУчета     = ЗаписьСоответствия.Значение.СчетУчета;
			ТаблицаДанных = ЗаписьСоответствия.Значение.ТаблицаДанных;
			
			ОбщаяТаблицаРасчетов = Форма[ЗаписьСоответствия.Ключ].Выгрузить(Новый Структура("СчетУчета", СчетУчета));
	...
	Если Форма.НалоговыйАгентНДСПоОтдельнымВидамТоваров.Количество() <> 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из Форма.НалоговыйАгентНДСПоОтдельнымВидамТоваров Цикл
			НоваяСтрока = ТаблицаНалоги.Добавить();
			НоваяСтрока.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане;
			НоваяСтрока.КодПоОКТМО                  = Справочники.РегистрацииВНалоговомОргане.КодТерритории(РегистрацияВНалоговомОргане);
			НоваяСтрока.Налог                       = Налог;
			НоваяСтрока.КодБК                       = КодБК;
	...
	ТаблицыНалогов.ТаблицаНалоговыйАгентНДС = ТаблицаНалоговыйАгентИтоговая;
	
	АдресХранилищаТаблицыНалоги = ПоместитьВоВременноеХранилище(ТаблицыНалогов, Форма.УникальныйИдентификатор);
	
	Возврат АдресХранилищаТаблицыНалоги;
	
КонецФункции

Функция ПлатежныеДокументыДляВыгрузкиВФайл(Форма) Экспорт
...
Функция ПлатежныеДокументыДляОтправкиПоДиректБанку(Форма) Экспорт
...
Функция НеОтправленныеПлатежи(Форма) Экспорт
	
	ДокументыОплаты = Новый Массив;
	
	Для Каждого СтрокаОплаты Из Форма.Платежи Цикл
		Если ТипЗнч(СтрокаОплаты.ПлатежныеПоручения) <> Тип("Массив")
			Или ТипЗнч(СтрокаОплаты.Состояния) <> Тип("Массив") Тогда
			Продолжить;
		КонецЕсли;
		КоличествоПоручений = СтрокаОплаты.ПлатежныеПоручения.ВГраница();
		КоличествоСостояний = СтрокаОплаты.Состояния.ВГраница();
		Для ИндексДокумента = 0 По КоличествоПоручений Цикл
			Документ = СтрокаОплаты.ПлатежныеПоручения[ИндексДокумента];
			Состояние = ?(ИндексДокумента <= КоличествоСостояний,
				СтрокаОплаты.Состояния[ИндексДокумента], Неопределено);
			Если ТипЗнч(Документ) = Тип("ДокументСсылка.ПлатежноеПоручение")
				И Не ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеУспешнойОтправки(Состояние)
				И Не ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОшибки(Состояние)
				И Не ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОплачено(Состояние) Тогда
				ДокументыОплаты.Добавить(Документ);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция РегламентныеДокументыЗаПериод(Параметры, СоздаватьФЗКПродаж)
...
Процедура СформироватьРегламентныеДокументы(Параметры, РегламентныеДокументы)
	...
	Если РегламентныеДокументы.ФЗКПродаж <> Неопределено Тогда
...
Процедура СформироватьСводныеСправки(Параметры)
	...
	ПериодичностьРегистрацииСводныхСправок = 
		УчетнаяПолитика.ПериодичностьРегистрацииСводныхСправок(Параметры.Организация, Параметры.НачалоПериода);
	
	СтруктураПараметров.ПериодичностьРегистрации = ПериодичностьРегистрацииСводныхСправок;
	
	Обработки.РегистрацияСводныхСправок.СформироватьСводныеСправкиВПомощнике(СтруктураПараметров);
...
Функция НеИсполненныеПлатежи(Форма)
	...
	Для Каждого СтрокаОплаты Из Форма.Платежи Цикл
		Если ТипЗнч(СтрокаОплаты.ПлатежныеПоручения) <> Тип("Массив") Тогда
			Продолжить;
		КонецЕсли;
		КоличествоПоручений = СтрокаОплаты.ПлатежныеПоручения.ВГраница();
		КоличествоСостояний = СтрокаОплаты.Состояния.ВГраница();
		Для ИндексДокумента = 0 По КоличествоПоручений Цикл
			...
			Состояние = ?(ИндексДокумента <= КоличествоСостояний,
				СтрокаОплаты.Состояния[ИндексДокумента], Неопределено);
			Если ТипЗнч(Документ) = Тип("ДокументСсылка.ПлатежноеПоручение")
				И Не ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОплачено(Состояние)
				И Не ПомощникиПоУплатеНалоговИВзносовКлиентСервер.СостояниеОшибки(Состояние) Тогда
				ДокументыОплаты.Добавить(Документ);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ТребуетсяВыгрузитьДокументыОплаты(Форма)
	
	ДокументыОплатыВФайл = ПлатежныеДокументыДляВыгрузкиВФайл(Форма);
	
	Возврат ЗначениеЗаполнено(ДокументыОплатыВФайл);
	
КонецФункции

Функция ТребуетсяОтправитьДокументыОплатыПоДиректБанку(Форма)
	
	ДокументыОплатыПоБанку = ПлатежныеДокументыДляОтправкиПоДиректБанку(Форма);
...
Функция ПредставлениеПлатежа(СтрокаКоллекцииПлатежей)
	...
	СвойстваПлатежа = Новый Структура("ПлатежныеПоручения, Состояния, ПредставлениеПлатежногоПоручения");
	ЗаполнитьЗначенияСвойств(СвойстваПлатежа, СтрокаКоллекцииПлатежей);
	
	Если Не ЗначениеЗаполнено(СвойстваПлатежа.ПлатежныеПоручения) Тогда
		Возврат СвойстваПлатежа.ПредставлениеПлатежногоПоручения;
	КонецЕсли;
	
	ПредставлениеПлатежа = Новый Массив;
	
	Если ТипЗнч(СвойстваПлатежа.Состояния) = Тип("Массив")
		И СвойстваПлатежа.Состояния.Количество() = 1 Тогда
		СостояниеПлатежа = СвойстваПлатежа.Состояния[0];
		
		ПредставлениеПлатежа.Добавить(Новый ФорматированнаяСтрока(СтрШаблон("%1 (%2)",
			СвойстваПлатежа.ПредставлениеПлатежногоПоручения, Строка(СостояниеПлатежа)), , , , "ОткрытьДокумент"));
		...
		Если СостояниеПлатежа <> Перечисления.СостоянияБанковскихДокументов.Оплачено
			...
			И ПравоДоступа("Изменение", Метаданные.Документы.ПлатежноеПоручение) Тогда
	...
	Иначе
		ПредставлениеПлатежа.Добавить(Новый ФорматированнаяСтрока(СтрШаблон("%1",
			СвойстваПлатежа.ПредставлениеПлатежногоПоручения), , , , "ОткрытьДокумент"));
	КонецЕсли;
