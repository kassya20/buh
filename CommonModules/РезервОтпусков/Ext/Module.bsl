...
Функция НастройкиРезервовОтпусков(Организация, Период) Экспорт
	...
	Настройки.Вставить("ОпределятьИзлишкиЕжемесячно",
		Ложь);
	Настройки.Вставить("ОпределятьИзлишкиЕжемесячноБУ",
		Ложь);
	...
	Если НастройкиРасчетаРезервовОтпусков.Количество() > 0 Тогда
		...
		Настройки.Вставить("ОпределятьИзлишкиЕжемесячноБУ", ТекущаяНастройка.ОпределятьИзлишкиЕжемесячно);
...
Функция ДокументНачислениеОценочныхОбязательствПоОтпускам(ПериодРегистрации, Организация, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Иначе
		...
		ДокументОбъект.Дата = КонецМесяца(ПериодРегистрации);
...
Процедура ПеречитатьОценочныеОбязательства(Объект, ПересчитатьПолностью) Экспорт
	...
	РезервыПоОплатеТруда.ДополнитьПеренесеннымиОстатками(ОценочныеОбязательстваПоСотрудникам, СтруктураПараметровРасчета);
...
Процедура СобратьДанныеДляНачислениеРезерваОтпусков(Регистратор, Организация, ПериодРегистрации, РасчетРезерваОтпусков, ОценочныеОбязательства, ОценочныеОбязательстваПоСотрудникам) Экспорт
	...
	СтруктураПараметровРасчета.Вставить("ОпределятьИзлишкиЕжемесячноБУ",                ОпределятьИзлишкиЕжемесячно);
	...
	РезервыПоОплатеТруда.ДополнитьПеренесеннымиОстатками(ПараметрыРасчетаОбязательств, СтруктураПараметровРасчета);
...
Процедура ОпределитьСуммуКорректировкиОстатков(ПараметрыРасчетаОбязательств, СтруктураПараметровРасчета)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКорректировок Цикл
		...
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерва",                      "СуммаРезерваИсчислено");
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерваСтраховыхВзносов",      "СуммаРезерваСтраховыхВзносовИсчислено");
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерваФССНесчастныеСлучаи",   "СуммаРезерваФССНесчастныеСлучаиИсчислено");
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерваНУ",                    "СуммаРезерваИсчисленоНУ");
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерваСтраховыхВзносовНУ",    "СуммаРезерваСтраховыхВзносовИсчисленоНУ");
		РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаТаблицы, МассивСтрок, "СуммаРезерваФССНесчастныеСлучаиНУ", "СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ");
	...
	Если МассивОбщихОстатков.Количество() <> 0 Тогда
		...
		Для Каждого СтрокаМассива Из МассивОбщихОстатков Цикл
			...
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерва",                      "СуммаРезерваИсчислено");
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерваСтраховыхВзносов",      "СуммаРезерваСтраховыхВзносовИсчислено");
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерваФССНесчастныеСлучаи",   "СуммаРезерваФССНесчастныеСлучаиИсчислено");
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерваНУ",                    "СуммаРезерваИсчисленоНУ");
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерваСтраховыхВзносовНУ",    "СуммаРезерваСтраховыхВзносовИсчисленоНУ");
			РезервыПоОплатеТруда.РаспределитьПоМассиву(СтрокаМассива, МассивСтрок, "СуммаРезерваФССНесчастныеСлучаиНУ", "СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ");
...
Функция ТаблицаКорректировокТекущегоМесяца(СтруктураПараметровРасчета, Регистратор) Экспорт
	...
	Возврат РезервыПоОплатеТруда.ТаблицаКорректировокТекущегоМесяца(СтруктураПараметровРасчета, Регистратор);
...
Функция РаспределитьПропорционально(Знач ИсхСумма, МассивКоэф, Знач Точность = 2) Экспорт
	...
	Возврат РезервыПоОплатеТруда.РаспределитьПропорционально(
...
Процедура РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяСумма, НаборСтрок, ИмяКолонки, ИмяКолонкиБазы = "") Экспорт
	...
	РезервыПоОплатеТруда.РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяСумма, НаборСтрок, ИмяКолонки, ИмяКолонкиБазы);
