...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьДанныеМонитораВФоне", Истина);
	Методы.Вставить("ПерезаписатьДанныеМонитораВФоне", Истина);
	Методы.Вставить("ОбновитьДанныеМонитораВФонеДляВнешнегоПрограммногоИнтерфейса", Истина);
	Методы.Вставить("ЗаполнитьДанныеМонитораРуководителя");
	Методы.Вставить("УстановитьРазрешенныеРазделыМонитораРуководителя");
...
Функция ЗапуститьОбновлениеМонитораВФоне(ПараметрыОбновления, УникальныйИдентификатор, СУчетомАктуальности, ОжидатьВыполненияЗадания) Экспорт
	...
	Если СУчетомАктуальности Тогда
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"МониторРуководителя.ОбновитьДанныеМонитораВФоне",
			ПараметрыОбновления,
			ПараметрыВыполненияВФоне);
	Иначе
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"МониторРуководителя.ПерезаписатьДанныеМонитораВФоне",
			ПараметрыОбновления,
			ПараметрыВыполненияВФоне);
	КонецЕсли;
...
Процедура ОбновитьДанныеМонитораВФоне(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	СрокХраненияТоваров = Неопределено;
	
	Если Параметры.Свойство("СрокХраненияТоваров") Тогда
		СрокХраненияТоваров = Параметры.СрокХраненияТоваров;
	КонецЕсли;
	
	ОбновитьДанныеМонитора(Параметры.Организация, Истина, Параметры.РазделыМонитора, Параметры.ПолучатьПрошлыйПериод, СрокХраненияТоваров);
...
Процедура ПерезаписатьДанныеМонитораВФоне(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	ОбновитьДанныеМонитора(Параметры.Организация, Ложь, Параметры.РазделыМонитора, Параметры.ПолучатьПрошлыйПериод, СрокХраненияТоваров);
...
Процедура ЗаполнитьДанныеМонитораРуководителя() Экспорт
	...
	ОбновитьДанныеМонитораПоВсемОрганизациям(ТекущаяДатаСеанса(), ИнтервалОбновления);
...
Функция СписокРазделовМонитораРуководителяПоУмолчанию() Экспорт
	...
	// Для совместимости старой и новой версий монитора
	НеиспользуемыеРазделы.Добавить(Перечисления.РазделыМонитораРуководителя.Доходы);
	НеиспользуемыеРазделы.Добавить(Перечисления.РазделыМонитораРуководителя.Расходы);
	НеиспользуемыеРазделы.Добавить(Перечисления.РазделыМонитораРуководителя.Прибыль);
...
Процедура ОбновитьДанныеМонитораВФонеДляВнешнегоПрограммногоИнтерфейса(Параметры, ВременноеХранилищеРезультата) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ОбновитьДанныеМонитора(Организация, Ложь, РазделыМонитора, Параметры.ПолучатьПрошлыйПериод, Параметры.СрокХраненияТоваров);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ЗапросКДаннымРазделаМонитора(ПолучатьПрошлыйПериод, ОтбиратьПоОрганизации, ПолучатьДанныеПодразделов)
	...
	Если ОбщегоНазначенияБП.ИспользуетсяАрхив() Тогда
		Условия.Добавить("НЕ ДанныеМонитораРуководителя.Организация.ВАрхиве");
	КонецЕсли;
...
//
// Параметры:
//  Организация - Справочники.Организация - организация по которой нужно обновить ланные монитора
//  УчитыватьАктуальностьДанных - Булево - Истина - перед обновлением проверить что данные неактуальны, Ложь - не проверять
//  РазделыМонитора - Массив - список разделов, которые необходимо обновить, если передано Неопределено, то будет обновлен список разделов по умолчанию
//  ПолучатьПрошлыйПериод - Булево - признак, которые говорит о том, что необходимо обновить данные за прошлый период
//  СрокХраненияТоваров - Число - количество месяцев, по прошествию который товар считается залежалым.
//
Процедура ОбновитьДанныеМонитора(Организация, УчитыватьАктуальностьДанных, РазделыМонитора, ПолучатьПрошлыйПериод, СрокХраненияТоваров)
	...
	Если СрокХраненияТоваров = Неопределено Тогда
		СрокХраненияТоваров = 6;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Если СписокДоступныхОрганизаций.Найти(Организация) <> Неопределено Тогда
			РегистрыСведений.ДанныеМонитораРуководителя.ОбновитьДанныеМонитора(Организация, Дата, ИнтервалОбновления,
				РазделыМонитора, ПолучатьПрошлыйПериод, СрокХраненияТоваров);
		КонецЕсли;
	Иначе
		ОбновитьДанныеМонитораПоВсемОрганизациям(Дата, ИнтервалОбновления, РазделыМонитора, ПолучатьПрошлыйПериод, СрокХраненияТоваров);
	КонецЕсли;
...
//  СрокХраненияТоваров - Число - количество месяцев, по прошествию который товар считается залежалым.
//
Процедура ОбновитьДанныеМонитораПоВсемОрганизациям(Дата, ИнтервалОбновления, РазделыМонитора = Неопределено, ПолучатьПрошлыйПериод = Ложь, СрокХраненияТоваров = 6)
