...
Процедура СформироватьНадписьЦеныИВалюта(Форма) Экспорт
	...
	Объект = Форма.Объект;
	СтруктураНадписи = Новый Структура("ВалютаДокумента, СуммаВключаетНДС, ДокументБезНДС");
	СтруктураНадписи.Вставить("НДССМежценовойРазницы", Ложь);
...
Функция ПолучитьРеквизитФормы(Форма, ИмяРеквизита) Экспорт
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ЭтоВыкупТоваровКомиссионером") Тогда
		// Признак того, что счета при поступлении в розницу номенклатура в строках не указывается,
		// вводятся общие цифры.
		Если ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ЭтоВыкупТоваровКомиссионером;
		Иначе
			Результат = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("НастроенОбменЭПД") Тогда
		// Признак того, что для между организацией и покупателем настроен обмнг ЭПД
		Если ИмяФормы = "ФормаДокументаОбщая" 
			ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.НастроенОбменЭПД;
		Иначе
			Результат = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ВедетсяУчетПрослеживаемыхТоваров") Тогда
		...
		Иначе
			Результат = Ложь;
		КонецЕсли;
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ЕстьУчетПодакцизныхТоваров") Тогда
		Если ИмяФормы = "ФормаДокументаОбщая" 
			ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ЕстьУчетПодакцизныхТоваров;
		Иначе
			Результат = Ложь;
		КонецЕсли;
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ЭкспортПриРеализации") Тогда
		Если ИмяФормы = "ФормаДокументаОбщая" 
			ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ЭкспортПриРеализации;
		Иначе
			Результат = Ложь;   
		КонецЕсли;
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ВидыПроизведенныхПодакцизныхТоваров") Тогда
		Если ИмяФормы = "ФормаДокументаОбщая" 
			ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ВидыПроизведенныхПодакцизныхТоваров;
		Иначе
			Результат = Ложь;
		КонецЕсли;
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ПроизводствоПодакцизныхТоваров") Тогда
		Если ИмяФормы = "ФормаДокументаОбщая" 
			ИЛИ ИмяФормы = "ФормаДокументаТовары" Тогда
			Результат = Форма.ПроизводствоПодакцизныхТоваров;
		Иначе
			Результат = Ложь;
		КонецЕсли;
...
Процедура УстановитьРеквизитФормы(Форма, ИмяРеквизита, ЗначениеРеквизита) Экспорт
	...
	Если ИмяРеквизитаВРег = ВРег("ЭтоКомиссия") Тогда
		...
		КонецЕсли;
	ИначеЕсли ИмяРеквизитаВРег = ВРег("ЭтоВыкупТоваровКомиссионером") Тогда
		// Признак того, что реализация выполняется со счета товары переданные.
		Если ИмяФормы = "ФормаДокументаТовары" Тогда
			Форма.ЭтоВыкупТоваровКомиссионером = ЗначениеРеквизита;
		КонецЕсли;
...
Процедура ПроверитьВозможностьОформленияЭПД(Форма) Экспорт
	Объект = Форма.Объект;
	
	Грузополучатель = ?(ЗначениеЗаполнено(Объект.Грузополучатель), Объект.Грузополучатель, Объект.Контрагент);
	
	Форма.НастроенОбменЭПД = Форма.ИспользоватьОбменЭД 
		И ЗначениеЗаполнено(Объект.Организация) 
		И ЗначениеЗаполнено(Грузополучатель) 
		И ЗначениеЗаполнено(Объект.Перевозчик) 
		И РеализацияТоваровУслугФормыВызовСервера.НастроенОбменЭПД(Объект.Организация, Грузополучатель, Объект.Перевозчик);
		
КонецПроцедуры

// Устанавливает значения заявления о ввозе полученном и ссылку на отчет "Статистическая форма учета перемещения товаров"
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма, для которой определяется реквизит.
//
Процедура УстановитьРеквизитыОтчетностиЭкспортеров(Форма) Экспорт
	
	Если НЕ Форма.Элементы.СтатформаПросмотр.Видимость
		И НЕ Форма.Элементы.ЗаявлениеОВвозеТоваровПолученноеПросмотр.Видимость Тогда
		Возврат;
	КонецЕсли;
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		Если ЭтоУчастникЕАЭС Тогда
			...
			Если Элементы.СтатформаПросмотр.Видимость Тогда
				
				Элементы.ДекорацияСтатформа.Видимость = Истина;
				
				Если Не ЗначениеЗаполнено(Форма.ОтчетностьВФТС)
					...
					Форма.ОтчетностьВФТС = УчетНДСВызовСервера.ОпределитьСсылкуНаСуществующуюСтатформу(Реквизиты);
					
				КонецЕсли;
				
			КонецЕсли;
			
			Если Элементы.ЗаявлениеОВвозеТоваровПолученноеПросмотр.Видимость Тогда
			
				Отбор = Новый Структура("СтавкаНДС", ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС0"));
				РезультатПоиска = Объект.Товары.НайтиСтроки(Отбор);
				
				Если РезультатПоиска.Количество() > 0 Тогда
				
					УчетНДСКлиентСервер.НастроитьПоляЗаявленияОВвозеТоваровПолученное(
						Элементы.ЗаявлениеОВвозеТоваровПолученноеКнопка,
						Элементы.ЗаявлениеОВвозеПолученноеСсылка,
						Форма.ЗаявлениеОВвозеТоваровПолученное);
					
					УчетНДСКлиентСервер.ЗаполнитьРеквизитыФормыПроЗаявлениеОВвозеТоваровПолученное(Форма,
						Форма.ЗаявлениеОВвозеТоваровПолученное);
				Иначе
					
					Элементы.ЗаявлениеОВвозеТоваровПолученноеКнопка.Видимость = Ложь;
					Элементы.ЗаявлениеОВвозеПолученноеСсылка.Видимость = Ложь;
		...
		Иначе
			...
			Элементы.ДекорацияСтатформа.Видимость = Ложь;
			Элементы.ЗаявлениеОВвозеТоваровПолученноеКнопка.Видимость = Ложь;
			Элементы.ЗаявлениеОВвозеПолученноеСсылка.Видимость = Ложь;
