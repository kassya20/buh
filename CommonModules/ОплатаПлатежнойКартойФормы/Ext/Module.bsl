...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ЭтоПлатежПоРеестру =
		Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ПлатежПоРеестру");
	ОплатаСчетаЧерезСБП =
		Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаСчетаЧерезСБП");
	
	// Запись реестра не может меняться ни на какой другой вид
	Форма.Элементы.ГруппаВидОперации.Видимость = Не ЭтоПлатежПоРеестру;
	Форма.Элементы.ГруппаНомерДатаРеестра.Видимость = ЭтоПлатежПоРеестру;
	Форма.Элементы.ГруппаВидаОплаты.Доступность = Не ОплатаСчетаЧерезСБП;
	
	Если ЭтоПлатежПоРеестру Или ОплатаСчетаЧерезСБП Тогда
	
		ТекстЗаголовка = ?(ОплатаСчетаЧерезСБП, НСтр("ru = 'Платеж через СБП'"), НСтр("ru = 'Платеж по реестру'"));
		
		Если ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда
			ТекстЗаголовка = ТекстЗаголовка + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru=' %1 от %2'"), Форма.Объект.Номер, Форма.Объект.Дата);
		Иначе
	...
	ТипыОборудования.ЭквайринговыйТерминал = Истина;
...
Процедура ДатаПриИзменении(Форма) Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя
		...
		Если ОплатаПлатежнойКартойКлиентСервер.ФормаДокументаОднострочная(Форма)
			...
			Если СтрокаПлатежа.ПорядокОтраженияАванса = ПредопределенноеЗначение("Перечисление.ПорядокОтраженияАвансов.ДоходПатент")
				Или ТипЗнч(СтрокаПлатежа.ПорядокОтраженияАванса) = Тип("СправочникСсылка.Патенты") Тогда
				Форма.РасшифровкаПлатежаСтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			Иначе
				Форма.РасшифровкаПлатежаСтавкаНДС = Форма.СвойстваПлатежа.СтавкаНДС;
			КонецЕсли;
			СтрокаПлатежа.СтавкаНДС    = Форма.РасшифровкаПлатежаСтавкаНДС;
			СтрокаПлатежа.СуммаПлатежа = Форма.РасшифровкаПлатежаСуммаВзаиморасчетов;
			ОплатаПлатежнойКартойКлиентСервер.ПересчитатьСуммуНДС(СтрокаПлатежа);
			Форма.РасшифровкаПлатежаСуммаНДС = СтрокаПлатежа.СуммаНДС;
		КонецЕсли;
...
Процедура ОрганизацияПриИзменении(Форма) Экспорт
	...
	Если ОплатаПлатежнойКартойКлиентСервер.ФормаДокументаОднострочная(Форма)
		И Объект.РасшифровкаПлатежа.Количество() = 1 Тогда
		СтрокаПлатежа = Объект.РасшифровкаПлатежа[0];
		Форма.РасшифровкаПлатежаСтавкаНДС = НовыеПараметры.СтавкаНДС;
		СтрокаПлатежа.СтавкаНДС    = Форма.РасшифровкаПлатежаСтавкаНДС;
		СтрокаПлатежа.СуммаПлатежа = Форма.РасшифровкаПлатежаСуммаВзаиморасчетов;
		ОплатаПлатежнойКартойКлиентСервер.ПересчитатьСуммуНДС(СтрокаПлатежа);
		Форма.РасшифровкаПлатежаСуммаНДС = СтрокаПлатежа.СуммаНДС;
	Иначе
		Для Каждого СтрокаПлатеж Из Объект.РасшифровкаПлатежа Цикл
			СтрокаПлатеж.СтавкаНДС    = НовыеПараметры.СтавкаНДС;
			ОплатаПлатежнойКартойКлиентСервер.ПересчитатьСуммуНДС(СтрокаПлатеж);
		КонецЦикла;
	КонецЕсли;
...
Функция НовыеПараметрыПриИзмененииОрганизации(Знач ПараметрыОбъекта)
	...
	Иначе
		НовыеПараметры.Вставить("СтавкаНДС", УчетНДСКлиентСервер.СтавкаНДСПользователя(ПараметрыОбъекта.Дата, ПараметрыОбъекта.ПлательщикНДС));
	КонецЕсли;
...
Процедура ПерезаполнитьРасшифровкуПлатежа(Форма, ЭтоИзменениеОрганизации = Ложь)
	...
	Для Каждого СтрокаШаблон Из ШаблонЗаполнения Цикл
		...
		Иначе
			Если Форма.ПлательщикНДСПриУСН Тогда
				СтрокаПлатеж.СтавкаНДС = Форма.СтавкаНДСПриУСН;
			Иначе
...
Процедура ПодготовитьФормуНаСервере(Форма)
	...
	Форма.ЭтоВозвратПоСБП = ЭтоВозвратПоСБП(Объект);
	Если Форма.ЭтоВозвратПоСБП Тогда
		ИнициализироватьФормуВозвратПоСБП(Форма);
	Иначе
		Форма.ЭтоВозвратОнлайнЗаказы = ЭтоВозвратОнлайнЗаказы(Объект, СпособОплаты);
		Если Форма.ЭтоВозвратОнлайнЗаказы Тогда
			Форма.СпособОплатыОнлайнЗаказы = СпособОплаты;
			ИнициализироватьФормуВозвратОнлайнЗаказы(Форма);
		КонецЕсли;
	КонецЕсли;
	
	ТипыПО = МенеджерОборудованияКлиентСерверБП.ТипыКонтрольноКассовойТехники();
	ТипыПО.ЭквайринговыйТерминал = Истина;
	
	Форма.ЕстьПравоПодключаемоеОборудование = ЗначениеЗаполнено(МенеджерОборудованияВызовСервера.РабочееМестоКлиента())
		И МенеджерОборудованияБП.ИспользуетсяОборудование(ТипыПО);
	...
	// Добавим дополнительные реквизиты записи реестра
	Если Объект.РеквизитыРеестра.Количество() > 0 Тогда
		РеквизитыQRКода = Справочники.РеквизитыКодаДляОплаты.СписокРеквизитов();
		
		ОписаниеРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ОбщегоНазначения.ВыгрузитьКолонку(Объект.РеквизитыРеестра, "РеквизитРеестра"), "Код");
		
		Для НомерСтроки = 0 По Объект.РеквизитыРеестра.Количество()-1 Цикл
			ИмяЭлемента  = СтрШаблон("РеквизитQRКода_%1", НомерСтроки);
			
			Если Элементы.Найти(ИмяЭлемента) <> Неопределено Тогда
				Продолжить;
			КонецЕсли; 
			
			РеквизитРеестра = Объект.РеквизитыРеестра[НомерСтроки].РеквизитРеестра;
			
			Если НЕ ЗначениеЗаполнено(РеквизитРеестра) 
				ИЛИ ОписаниеРеквизита[РеквизитРеестра] = Неопределено
				ИЛИ РеквизитыQRКода[ОписаниеРеквизита[РеквизитРеестра]] = Неопределено Тогда
				
				Продолжить;
			КонецЕсли; 
			
			ЗаголовокЭлемента = РеквизитыQRКода[ОписаниеРеквизита[РеквизитРеестра]].Представление;
			
			НовыйЭлемент = Элементы.Добавить(ИмяЭлемента, Тип("ПолеФормы"), Элементы.ГруппаРеквизитыРеестра);
			НовыйЭлемент.Вид = ВидПоляФормы.ПолеНадписи;
			НовыйЭлемент.Заголовок = ЗаголовокЭлемента;
			НовыйЭлемент.ПутьКДанным = СтрШаблон("Объект.РеквизитыРеестра[%1].Значение", НомерСтроки);
			НовыйЭлемент.ВысотаЗаголовка = Окр(СтрДлина(ЗаголовокЭлемента)/15);
			НовыйЭлемент.Шрифт = Новый Шрифт(Новый Шрифт(),,, Истина);
		КонецЦикла;
	КонецЕсли; 
...
Процедура УстановитьПараметрыВыбораВидаОплаты(Форма)
	...
	ТипыОплатБезналичныеОплаты.Добавить(Перечисления.ТипыОплат.СБП);
...
Процедура УстановитьУсловноеОформление(Форма) Экспорт
	...
	Если ЭлементУО <> Неопределено Тогда
		СписокОперацийОплатаПокупателя = Новый СписокЗначений;
		СписокОперацийОплатаПокупателя.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя);
		СписокОперацийОплатаПокупателя.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.ПлатежПоРеестру);
		СписокОперацийОплатаПокупателя.Добавить(Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаСчетаЧерезСБП);
		
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
			ЭлементУО.Отбор,
			НачалоПутиКДаннымТЧ + "ВидОперации",
			ВидСравненияКомпоновкиДанных.НеВСписке,
			СписокОперацийОплатаПокупателя);
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	КонецЕсли;
	...
	ЭлементУО = Неопределено;
	ДобавитьПолеВУсловноеОформление(СтрШаблон("%1ДоговорКонтрагента", ИмяТабличнойЧасти),
		ЭлементУО, Элементы, УсловноеОформление);
...
Процедура УстановитьФункциональныеОпцииФормы(Форма)
	...
	Если Форма.ПрименяетсяУСНПатент Тогда
		Форма.ПрименяетсяНесколькоПатентов = УчетПСН.ИспользуетсяНесколькоПатентов(
			Объект.Организация, Объект.Дата);
	КонецЕсли;
...
Функция ЭтоВозвратПоСБП(Объект)
	
	Если Объект.ВидОперации <> Перечисления.ВидыОперацийОплатаПлатежнойКартой.ВозвратПокупателю
		Или Не ЗначениеЗаполнено(Объект.ДокументОснование)
		Или ТипЗнч(Объект.ДокументОснование) <> Тип("ДокументСсылка.ОплатаПлатежнойКартой") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование, "ВидОперации, СсылочныйНомер");
	
	Если Не ЗначениеЗаполнено(ДанныеОснования.СсылочныйНомер)
		Или ДанныеОснования.ВидОперации <> Перечисления.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя Тогда
		Возврат Ложь;
	КонецЕсли;
	
	СчетНаОплату = Документы.ОплатаПлатежнойКартой.СчетНаОплату(Объект.ДокументОснование);
	
	Если Не ЗначениеЗаполнено(СчетНаОплату) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Результат = ИнтеграцияССБПБП.СчетОплачен(СчетНаОплату, ДанныеОснования.СсылочныйНомер);
	
	Возврат Результат;
	
КонецФункции

Процедура ИнициализироватьФормуВозвратПоСБП(Форма)
...
Функция ЭтоВозвратОнлайнЗаказы(Объект, СпособОплаты)
	...
	Результат = ОнлайнЗаказыБП.СчетОплачен(СчетНаОплату, СпособОплаты);
	
	Возврат Результат;
	
КонецФункции

Процедура ИнициализироватьФормуВозвратОнлайнЗаказы(Форма)
	...
	Результат = ОнлайнЗаказыБП.ДанныеВозвратОплаты(ДанныеДокумента);
...
Процедура МобильныйКлиентАдаптацияФормыПриСоздании(Форма)
	...
	Исключения = "РозничнаяВыручкаНДСУСН,РозничнаяВыручкаРасходыУСН,РасшифровкаПлатежаСуммаУслуг";
