...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи) Экспорт
	...
	ИначеЕсли СтруктураНадписи.Свойство("СуммаВключаетНДС") Тогда
		Если СтруктураНадписи.Свойство("НДССМежценовойРазницы") И СтруктураНадписи.НДССМежценовойРазницы Тогда
			ТекстНадписи = ТекстНадписи + ?(ПустаяСтрока(ТекстНадписи), НСтр("ru = 'НДС с межценовой разницы'"), НСтр("ru = '(НДС с межценовой разницы)'")); 
		ИначеЕсли СтруктураНадписи.СуммаВключаетНДС Тогда
...
Функция ФормаМножественногоЧисла(Слово1, Слово2, Слово3, Знач ЦелоеЧисло) Экспорт
	...
	ДесятокОкончанияЧисла = ЦелоеЧисло%100; // определяем в какой десяток входит окончание числа
	Если (ДесятокОкончанияЧисла > 10) И (ДесятокОкончанияЧисла < 20) Тогда
...
// Возвращает строку с переносами по количеству символов.
//
// Параметры:
//  ИсходнаяСтрока     - Строка - строка, которую необходимо преобразовать.
//  КоличествоСимволов - Число  - количество символов, после которого должен быть перенос.
//
// Возвращаемое значение:
//   Строка      - строка с переносами по словам, по количеству символов.
//
Функция СтрокаСПереносомПоКоличествуСимволов(ИсходнаяСтрока, КоличествоСимволов) Экспорт
	
	МассивСлов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(ИсходнаяСтрока, " ");
	Результат = "";
	ДлинаСтроки = КоличествоСимволов;
	
	Для Каждого ТекущееСлово Из МассивСлов Цикл 
		
		ПредыдущаяСтрока = Результат;
		Результат = Результат + " " + ТекущееСлово; 
		ДлинаИсходнойСтроки = СтрДлина(Результат);
		ДлинаПредыдущейСтроки = СтрДлина(ПредыдущаяСтрока);
		
		Если ДлинаИсходнойСтроки = ДлинаСтроки Тогда
			Результат = Результат + Символы.ПС;
			ДлинаСтроки = ДлинаИсходнойСтроки + КоличествоСимволов;
		ИначеЕсли ДлинаИсходнойСтроки > ДлинаСтроки Тогда
			Результат = ПредыдущаяСтрока + Символы.ПС + " " + ТекущееСлово;
			ДлинаСтроки = ДлинаПредыдущейСтроки + КоличествоСимволов;
		КонецЕсли;
...
// Удаляет лишние пробелы и переводы строки, которые могли добавиться к паролю или токену при копировании через буфер обмена
//
Функция УдалитьАртефактыВставкиИзБуфера(Строка) Экспорт
...
// Возвращает представление финансового периода.
//
// Параметры:
...
//
// Возвращаемое значение:
//   Строка - представление финансового периода
//
Функция ПредставлениеФинансовогоПериода(ДатаНачала, ДатаОкончания) Экспорт
	
	ПредставлениеЗаОдинДень    = НачалоДня(ДатаНачала)   = НачалоДня(ДатаОкончания);
	
	ДатаНачалаНачалоМесяца     = НачалоДня(ДатаНачала)   = НачалоМесяца(ДатаНачала);
	ДатаНачалаНачалоКвартала   = НачалоДня(ДатаНачала)   = НачалоКвартала(ДатаНачала);
	ДатаНачалаНачалоГода       = НачалоДня(ДатаНачала)   = НачалоГода(ДатаНачала);
	ДатаОкончанияКонецМесяца   = КонецДня(ДатаОкончания) = КонецМесяца(ДатаОкончания);
	ДатаОкончанияКонецКвартала = КонецДня(ДатаОкончания) = КонецКвартала(ДатаОкончания);
	ДатаОкончанияКонецГода     = КонецДня(ДатаОкончания) = КонецГода(ДатаОкончания);
	
	ГодДатаНачала              = Год(ДатаНачала);
	ГодДатаОкончания           = Год(ДатаОкончания);
	НомерМесяцаДатаНачала      = Месяц(ДатаНачала);
	НомерМесяцаДатаОкончания   = Месяц(ДатаОкончания);
	НомерКварталаДатаНачала    = НомерКвартала(ДатаНачала);
	НомерКварталаДатаОкончания = НомерКвартала(ДатаОкончания);
	
	ПериодЗаОдинГод            = ГодДатаНачала = ГодДатаОкончания;
	
	ПредставлениеЗаКварталСНачалаГода = ПериодЗаОдинГод И ДатаНачалаНачалоГода И ДатаОкончанияКонецКвартала;
	ПредставлениеЗаКвартал = ПериодЗаОдинГод
		И ДатаНачалаНачалоКвартала И ДатаОкончанияКонецКвартала И НомерКварталаДатаНачала = НомерКварталаДатаОкончания;
	ПредставлениеЗаПолугодие = ПериодЗаОдинГод
		И ДатаНачалаНачалоКвартала И ДатаОкончанияКонецКвартала И НомерКварталаДатаНачала = 3 И ДатаОкончанияКонецГода;
	ПредставлениеЗаМесяц = ПериодЗаОдинГод
		И ДатаНачалаНачалоМесяца И ДатаОкончанияКонецМесяца И НомерМесяцаДатаНачала = НомерМесяцаДатаОкончания;
	ПредставлениеЗаМесяцСНачалаГода = ПериодЗаОдинГод
		И ДатаНачалаНачалоМесяца И ДатаОкончанияКонецМесяца И НомерМесяцаДатаНачала <> НомерМесяцаДатаОкончания;
	
	Если ПредставлениеЗаОдинДень Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), Формат(ДатаНачала, "ДЛФ=D"));
	ИначеЕсли ПредставлениеЗаКварталСНачалаГода Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), ПредставлениеЗаКварталСНачалаГода(ДатаОкончания));
	ИначеЕсли ПредставлениеЗаКвартал Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), ПредставлениеЗаКвартал(ДатаОкончания));
	ИначеЕсли ПредставлениеЗаПолугодие Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), ПредставлениеЗаВтороеПолугодие(ДатаОкончания));
	ИначеЕсли ПредставлениеЗаМесяц Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), ПредставлениеЗаМесяц(ДатаОкончания));
	ИначеЕсли ПредставлениеЗаМесяцСНачалаГода Тогда
		Возврат СтрШаблон(НСтр("ru = 'за %1'"), ПредставлениеЗаМесяцСНачалаГода(ДатаОкончания));
	Иначе // произвольная дата
...
// Возвращает сокращенное наименование организации по полному.
//
// Параметры:
//   НаименованиеПолное - Строка - Полное наименование организации, полученное из сокращенного.   
//
...
//   СокращенноеНаименование - Строка - Сокращенное наименование организации.
//
Функция СокращенноеНаименованиеПоПолномуНаименованию(НаименованиеПолное) Экспорт
	
	СтруктураНаименования = РазложитьСтрокуНаименования(НаименованиеПолное);

	Если Не ЗначениеЗаполнено(СтруктураНаименования.ОрганизационноПравоваяФормаСокращенно) Тогда
		НаименованиеСокращенное = СтруктураНаименования.Наименование;
	ИначеЕсли СтруктураНаименования.ВыводитьКавычки Тогда
		НаименованиеСокращенное = СтрШаблон(НСтр("ru = '%1 ""%2""'"), 
			СтруктураНаименования.ОрганизационноПравоваяФормаСокращенно, СтруктураНаименования.Наименование);
	Иначе
		НаименованиеСокращенное = СтрШаблон(НСтр("ru = '%1 %2'"),
			СтруктураНаименования.ОрганизационноПравоваяФормаСокращенно, СтруктураНаименования.Наименование);
	КонецЕсли;
...
// Возвращает сокращенное наименование по ФИО физического лица
//
// Параметры:
//   Фамилия  - Строка
//   Имя      - Строка
//   Отчество - Строка
//   ЗарегистрированВКачествеИП - Булево
//
// Возвращаемое значение: 
//   Строка
//
Функция СокращенноеНаименованиеФизическогоЛица(Фамилия, Имя, Отчество, ЗарегистрированВКачествеИП = Истина) Экспорт
	...
	Если ЗарегистрированВКачествеИП Тогда
		Шаблон = НСтр("ru = 'ИП %1%2%3'");
	КонецЕсли;
...
// Возвращает полное наименование по ФИО физического лица
//
// Параметры:
//   Фамилия  - Строка
//   Имя      - Строка
...
//   Строка
//
Функция ПолноеНаименованиеФизическогоЛица(Фамилия, Имя, Отчество, ЗарегистрированВКачествеИП = Истина) Экспорт
	...
	Шаблон = НСтр("ru = '%1%2%3'");
	Если ЗарегистрированВКачествеИП Тогда
		Шаблон = НСтр("ru = 'Индивидуальный предприниматель %1%2%3'");
	КонецЕсли;
...
// Возвращает наименование по ФИО физического лица
//
// Параметры:
//   Фамилия  - Строка
//   Имя      - Строка
...
//   Строка
//
Функция НаименованиеФизическогоЛица(Фамилия, Имя, Отчество, ЗарегистрированВКачествеИП = Истина) Экспорт
...
// Возвращает ФИО в PAM-формате: Иванов Иван Иванович -> Иван Иванович И.
//
// Параметры:
//   ФИО  - Строка
//
...
//   Строка
//
Функция PAMФорматФИО(ФИО) Экспорт
	...
	Результат = ЧастиФИО.Имя
		+ ?(ЗначениеЗаполнено(ЧастиФИО.Отчество), " "+ ЧастиФИО.Отчество, "")
		+ " " + Лев(ЧастиФИО.Фамилия, 1) + ".";
...
// Преобразует двоичные данные в строку Base64, удаляя переносы строк
//
// Параметры:
//  ДвоичныеДанные - ДвоичныеДанные
//
// Возвращаемое значение:
//  Строка - строка Base64
//
Функция СтрокаBase64(ДвоичныеДанные) Экспорт
...
// Преобразует число в строку, при необходимости подставляя лидирующий ноль
//
// Параметры:
//  Значение - число - преобразуемое числовое значение
//
// Возвращаемое значение:
//  Строка
//
Функция ВремяЧислом(Значение) Экспорт
	Если Значение < 10 Тогда
		Возврат СтрШаблон("0%1", Строка(Значение));
	Иначе
		Возврат Строка(Значение);
	КонецЕсли;
КонецФункции

// Возвращает длину универсального уникального идентификатора (UUID)
//
// Возвращаемое значение:
//  Число
//
Функция ДлинаИдентификатора() Экспорт
	
	Возврат 36;
	
КонецФункции

// Возвращает описание типа для универсального уникального идентификатора (UUID)
//
// Возвращаемое значение:
//  ОписаниеТипов
//
Функция ОписаниеТипаИдентификатор() Экспорт
	...
	Квалификатор = Новый КвалификаторыСтроки(ДлинаИдентификатора(), ДопустимаяДлина.Переменная);
	ТипИдентификатор = Новый ОписаниеТипов("Строка", , Квалификатор);
	Возврат ТипИдентификатор;
	
КонецФункции

// Возвращает длину имени реквизита в 1С
//
// Возвращаемое значение:
//  Число
//
Функция ДлинаИмениРеквизита() Экспорт
	
	Возврат 254;
	
КонецФункции

// Возвращает строковое представление незаполненного значения, необязательного для заполнения, принятое в конфигурации.
//
// Возвращаемое значение:
//  Строка
//
Функция ПредставлениеНезаполненногоЗначенияЗаполнить() Экспорт
...
// Функция формирует список представлений документов.
//
// Параметры:
//  ДобавитьДокументПоступлениеДопРасходов - Булево - признак, что нужно добавить представление документа
//  "Поступление доп. расходов".
//
// Возвращаемое значение:
//  Список - СписокЗначений, содержащий представления документов.
//
Функция ПредставленияДокументов(ДобавитьДокументПоступлениеДопРасходов = Истина) Экспорт
	
	Список = Новый СписокЗначений;
	Список.Добавить("ПредставлениеДокументаАктРеализация", НСтр("ru='Акт (реализация услуг)'"));
	Список.Добавить("ПредставлениеДокументаУПДРеализацияУслуг", НСтр("ru='УПД (реализация услуг)'"));
	Список.Добавить("ПредставлениеДокументаНакладнаяРеализация", НСтр("ru='Накладная (реализация товаров)'"));
	Список.Добавить("ПредставлениеДокументаУПДРеализацияТоваров", НСтр("ru='УПД (реализация товаров)'"));
	Список.Добавить("ПредставлениеДокументаРеализация", НСтр("ru='Реализация'"));
	Список.Добавить("ПредставлениеДокументаАктПоступление", НСтр("ru='Акт (поступление услуг)'"));
	Список.Добавить("ПредставлениеДокументаУПДПоступлениеУслуг", НСтр("ru='УПД (поступление услуг)'"));
	Список.Добавить("ПредставлениеДокументаАктПередачиПрав", НСтр("ru='Акт на передачу прав'"));
	Список.Добавить("ПредставлениеДокументаУПДПриобретениеПрав", НСтр("ru='УПД (приобретение прав)'"));
	Список.Добавить("ПредставлениеДокументаНакладнаяПоступление", НСтр("ru='Накладная (поступление товаров)'"));
	Список.Добавить("ПредставлениеДокументаУПДПоступлениеТоваров", НСтр("ru='УПД (поступление товаров)'"));
	Список.Добавить("ПредставлениеДокументаНакладнаяТопливо", НСтр("ru='Накладная (топливо)'"));
	Список.Добавить("ПредставлениеДокументаУПДТопливо", НСтр("ru='УПД (топливо)'"));
	Список.Добавить("ПредставлениеДокументаПоступление", НСтр("ru='Поступление'"));
	
	Если ДобавитьДокументПоступлениеДопРасходов Тогда
		Список.Добавить("ПредставлениеДокументаПоступлениеДопРасходов", НСтр("ru='Поступление (доп. расходы)'"));
	КонецЕсли;
...
Функция ОжидаетсяДлительноеОткрытиеФормы(ИмяФормы) Экспорт
	...
	#ИначеЕсли Сервер Тогда
		//Открытие форм на мобильном клиенте не должно учитываться в статистике
...
// Устанавливает заголовок и ширину сворачиваемой группы.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма, в которой расположена группа
//  НазваниеГруппы - Строка - имя группы в форме
//  ЗаголовокТекст - Строка - заголовок, который требуется установить
//
Процедура УстановитьЗаголовокСворачиваемойГруппы(Форма, НазваниеГруппы, ЗаголовокТекст) Экспорт
	
	Форма.Элементы[НазваниеГруппы].ЗаголовокСвернутогоОтображения = ЗаголовокТекст;
	
	Если СтрНайти(ЗаголовокТекст, " ") > 0 Тогда
		ШиринаГруппы = 1000;
	Иначе
		ШиринаГруппы = 0;
	КонецЕсли;
	Форма.Элементы[НазваниеГруппы].Ширина = ШиринаГруппы;
	
КонецПроцедуры

#КонецОбласти

// Выгружает в массив ключи соответствия или структуры.
// 
// Параметры:
// 	Коллекция - Структура из КлючИЗначение
// 	          - Соответствие из КлючИЗначение:
// 	              * Ключ - Произвольный
// 	              * Значение - Произвольный
// 
// Возвращаемое значение:
// 	Массив из Произвольный - ключи структуры или соответствия.
//
...
// Преобразует текст в формате csv в массив значений
//
// Параметры:
//  Текст - Строка - исходный текст в формате csv (формат, в котором значения заключены в кавычки и отделены друг от друга
//          символом-разделителем)
//  Разделитель - Строка - символ, разделяющий значения в строке
//  Начало - Число - номер строки, с которой нужно начинать чтение файла (для исключения шапки и технической информации)
//
// Возвращаемое значение:
//  Массив из Массив из Строка
//
Функция РазобратьТекстВФорматеЗначенийСРазделителями(Текст, Разделитель, Начало = 1) Экспорт
	
	ПрочитанныеЗначения = Новый Массив;
	КоличествоСтрок = СтрЧислоСтрок(Текст);
	Если Не ЗначениеЗаполнено(Текст) Или Начало > КоличествоСтрок Тогда
	...
	Для Сч = Начало По КоличествоСтрок Цикл
		...
		КавычкиОткрыты = Ложь;
		Пока Не СтрокаСформирована И Сч <= КоличествоСтрок Цикл
			Строка = СтрПолучитьСтроку(Текст, Сч);
			СоставнаяСтрока.Добавить(Строка);
			КоличествоКавычекВСтроке = СтрЧислоВхождений(Строка, Символ(КодСимволаДвойнойКавычки()));
		...
		Подстроки = РазбитьНаПодстрокиБезУчетаРазделителейВнутриКавычек(Строка, Разделитель);
		ПрочитанныеЗначения.Добавить(Подстроки);
	КонецЦикла;
...
Функция ПредставлениеЗаКварталСНачалаГода(Период)
	
	НомерКвартала = НомерКвартала(Период);
	
	Если НомерКвартала = 1 Тогда
		ПредставлениеПериода = СтрШаблон(НСтр("ru = '1 квартал %1 года'"), Формат(Год(Период), "ЧГ="));
	ИначеЕсли НомерКвартала = 2 Тогда
		ПредставлениеПериода = СтрШаблон(НСтр("ru = 'полугодие %1 года'"), Формат(Год(Период), "ЧГ="));
	ИначеЕсли НомерКвартала = 3 Тогда
		ПредставлениеПериода = СтрШаблон(НСтр("ru = '9 месяцев %1 года'"), Формат(Год(Период), "ЧГ="));
	ИначеЕсли НомерКвартала = 4 Тогда
		ПредставлениеПериода = СтрШаблон(НСтр("ru = '%1 год'"), Формат(Год(Период), "ЧГ="));
	КонецЕсли;
...
Функция ПредставлениеЗаВтороеПолугодие(Период)
	
	ПредставлениеПериода = СтрШаблон(НСтр("ru = '2 полугодие %1 года'"), Формат(Год(Период), "ЧГ="));
	
	Возврат ПредставлениеПериода;
	
КонецФункции

Функция ПредставлениеЗаКвартал(Период)
	
	НомерКвартала = НомерКвартала(Период);
	ПредставлениеПериода = СтрШаблон(НСтр("ru = '%1 квартал %2 года'"), НомерКвартала, Формат(Год(Период), "ЧГ="));
	
	Возврат ПредставлениеПериода;
	
КонецФункции

Функция ПредставлениеЗаМесяц(Период)
	
	ПредставлениеМесяц = Формат(Период, "ДФ=MMMM");
	ПредставлениеМесяц = НРег(ПредставлениеМесяц);
	
	ПредставлениеПериода = СтрШаблон(НСтр("ru = '%1 %2 года'"), Формат(Период, "ДФ=MMMM"), Формат(Год(Период), "ЧГ="));
	
	Возврат ПредставлениеПериода;
	
КонецФункции

Функция ПредставлениеЗаМесяцСНачалаГода(Период)
	
	НачалоПериода = Формат(НачалоГода(Период), "ДФ=MMMM");
	КонецПериода  = Формат(Период, "ДФ=MMMM");
	НачалоПериода = НРег(НачалоПериода);
	КонецПериода  = НРег(КонецПериода);
	
	ПредставлениеПериода = СтрШаблон(НСтр("ru = '%1 - %2 %3 года'"),
		НачалоПериода, КонецПериода, Формат(Год(Период), "ЧГ="));
	
	Возврат ПредставлениеПериода;
	
КонецФункции

Функция РазбитьНаПодстрокиБезУчетаРазделителейВнутриКавычек(Строка, Разделитель, Подстроки = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		Возврат Подстроки;
	Иначе
		ОткрывающаяКавычка = СтрНайти(Строка, Символ(КодСимволаДвойнойКавычки()));
		...
		Если Не ОткрывающаяКавычка = 1 Тогда
			НовыеПодстроки = СтрРазделить(Результат, Разделитель, Истина);
	...
	Для Сч = 1 По СтрДлина(Строка) Цикл
		Если КодСимвола(Строка, Сч) = КодСимволаДвойнойКавычки() Тогда // Нашли кавычку "
			...
			Иначе
				...
				// Две двойные кавычки - экранирующий символ. Меняем на одну двойную кавычку
				Подстроки.Добавить(СтрЗаменить(Результат, """""", """"));
				ОставшаясяСтрока = Сред(Строка, Сч + 2);
				Возврат РазбитьНаПодстрокиБезУчетаРазделителейВнутриКавычек(
					ОставшаясяСтрока, Разделитель, Подстроки);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Подстроки.Добавить(СтрЗаменить(Строка, """""", """"));
	Возврат Подстроки;
