...
Функция НастройкиЭДОБЗК() Экспорт
	...
	Настройки.Вставить("ИспользуетсяЭДОБЗКРаботаРоссии", Константы.ИспользоватьИнтеграциюСРаботаРоссии.Получить());
...
Процедура ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.Параметры.ПараметрКоманды) Или ЭтоОбъектСПечатнымиФормами(Форма.Параметры.ПараметрКоманды[0]) Тогда
		Если ЭлектроннаяПодпись.ИспользоватьЭлектронныеПодписи() Тогда
			...
			Если Не Форма.Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("СторонняяПечатнаяФорма")
			...
			Если ТребуетсяПечатнаяФормаБезПодписи Тогда
				...
				КомандаФормы = ИнтеграцияСРаботаРоссии.КомандаПодписатьФормыПечатьДокументов(Форма);
...
Процедура ВывестиКомандыПередачиПечатныхФормВоВнешниеПриложения(Форма, Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияСРаботаРоссии.ПечатьДокументовПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
	Если Не ИнтеграцияСРаботаРоссии.ДоступнаПередачаДокументовНаРаботаРоссии() Тогда
	...
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользуетсяЭДОБЗККабинетСотрудника") Тогда
		...
		Если КомандаФормы <> Неопределено Тогда
			...
			Если СтрНайти(КомандаФормы.Заголовок, НСтр("ru = '1С:Кабинет сотрудника'")) = 0 Тогда
				СвойстваКоманды = СвойстваКомандыПередачиНаПорталРаботаРоссииИВКабинетСотрудника();
...
Функция СвойстваКомандыПередачиНаПорталРаботаРоссииИВКабинетСотрудника() Экспорт
	Возврат Новый Структура("Заголовок,Подсказка,Картинка",
		НСтр("ru = 'Передать на ""Работа России"" и ""1С:Кабинет сотрудника""'"),
		НСтр("ru = 'Подписать и передать на порталы ""Работа России"" и ""1С:Кабинет сотрудника""'"),
...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Настройки) Экспорт
	...
	Настройка = Настройки.Добавить();
	Настройка.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии;
	Настройка.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьИнтеграциюСРаботаРоссии;
	Настройка.ДоступноВАвтономномРабочемМесте = Ложь;
	Настройка.РаботаетСВнешнимиРесурсами = Истина;
...
Процедура ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны) Экспорт
	...
	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии.Имя);
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ПолучениеДокументовРаботаРоссии.ИмяМетода);
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если Источники.Строки.Найти(Метаданные.Документы.ДокументЭДОБЗК, "Метаданные") <> Неопределено Тогда
		...
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(Команда, "ФорматДокументаСоответствуетТребованиям", Ложь);
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера(Настройки, Метаданные.Отчеты.КонтрольОзнакомленияСРасчетнымиЛистками);
...
Функция ПечатнаяФормаСоШтампами(ФайлПечатнойФормы, Оригинал, ВыводитьОжиданияПодписи = Ложь, ВыводитьКомментарииДокументов = Ложь) Экспорт
	...
	Возврат ПечатнаяФормаСоШтампамиПоОригиналу(
		...
		ЭлектроннаяПодписьЭДОБЗК.ВсеУстановленныеПодписи(ФайлПечатнойФормы),
		Направления,
		Комментарии);
	
КонецФункции

Функция ПечатнаяФормаСоШтампамиПоОригиналу(ОригиналПечатнойФормы, УстановленныеПодписи, Направления = Неопределено, Комментарии = Неопределено, РазбиватьНаСтраницы = Истина) Экспорт
	...
	Для Каждого ДанныеСертификата Из УстановленныеПодписи Цикл
		Если ДанныеСертификата.Свойство("ИдентификаторПриложения") Тогда
			Штамп = Новый ТабличныйДокумент;
			Макет = ПолучитьОбщийМакет("ПФ_MXL_МакетШтампаПростойПодписи");
			Область = Макет.ПолучитьОбласть("Бланк|ШиринаБланка");
			Область.Параметры.ИдПриложения = ДанныеСертификата.ИдентификаторПриложения;
			Область.Параметры.ИмяВладельцаПодписи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеСертификата.ФизическоеЛицо, "ФИО");
			Область.Параметры.ИдВладельцаПодписи = ДанныеСертификата.ИдентификаторВладельцаПодписи;
			Область.Параметры.ДатаВремяПодписи = Формат(ДанныеСертификата.ДатаПодписи, "ДФ='дд.ММ.гггг ЧЧ:мм:сс'") + " " + UTCСеанса();
			Штамп.Вывести(Область);
		Иначе
	...
	ДобавитьШтампВТабличныйДокумент(ПечатнаяФорма, ОписаниеШтампов, РазбиватьНаСтраницы);
...
Функция UTCСеанса()
	СмещенияВремени = СмещениеСтандартногоВремени();
	ЧасыСмещения      = Цел(СмещенияВремени / 3600);
	Результат         = "UTC" + ?(ЧасыСмещения >= 0, "+", "-") + Формат(ЧасыСмещения, "ЧЦ=2; ЧН=00; ЧВН=;");
...
Процедура ДобавитьШтампВТабличныйДокумент(ПечатнаяФорма, ТабличныеДокументыШтампов, РазбиватьНаСтраницы)
	...
	Для НомерИтерации = 0 По КоличествоИтераций - 1 Цикл
		...
		Если РазбиватьНаСтраницы И Не ПечатнаяФорма.ПроверитьВывод(ТаблицаШтампов) Тогда
...
Процедура ПриПодписанииФайловПечатныхФорм(Источник, Отказ, Замещение) Экспорт
	...
	Если ЗначениеЗаполнено(ПодписанныеОбъекты) Тогда
		ЭДОБЗКВызовСервера.УдалитьФайлыИзОбработкиПользователя(ПодписанныеОбъекты);
	КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Инициализация новых шаблонов документов пособий.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.35.36";
	Обработчик.Процедура = "Справочники.ВидыДокументовЭДОБЗК.ЗаполнитьЗначенияЗаполненияДокументов";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("c353cfee-e097-4f1e-952a-fc4be98b1b66");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение значений заполнения документов ЭДО.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.30";
	...
	Обработчик.Процедура = "РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК.ЗаполнитьОрганизации";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("96f3c5ca-9c9a-4644-a49e-b6ed709b9ea2");
	Обработчик.Комментарий = НСтр("ru = 'Заполняет организации физических лиц, присоединенных к КЭДО.'");;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.31";
	Обработчик.Процедура = "РегистрыСведений.НастройкиПечатныхФормБЗК.ЗаполнитьНастройки";
	Обработчик.РежимВыполнения = "Монопольно";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.38";
	Обработчик.Процедура = "РегистрыСведений.СостоянияДокументовЭДОБЗК.ОбновитьСостоянияДокументовСПЭП";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("044782af-52e3-4f25-9b9b-e57bd719a21d");
	Обработчик.Комментарий = НСтр("ru = 'Обновляет состояния документов ЭДО.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.38";
	Обработчик.Процедура = "Справочники.ВидыДокументовЭДОБЗК.ЗаполнитьВариантПодписанияИТипПодписи";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("32a2f081-cd8e-4563-b64c-f32c2b077d3d");
	Обработчик.Комментарий = НСтр("ru = 'Заполнение вариантов подписания видов документов ЭДО.'");
	...
	Обработчик.Процедура = "Справочники.ШаблоныДокументов.ОбновитьПредопределенныеШаблоны_3_2_0";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("0c73e1a8-8315-4a4f-a63a-243f3de1daaa");
	Обработчик.Комментарий = НСтр("ru = 'Инициализация новых шаблонов документов.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.68";
	Обработчик.Процедура = "Справочники.ШаблоныДокументов.ОбновитьПредопределенныеШаблоны_3_4_0";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("b594156d-9b37-4850-8faa-832cb5cf23f4");
	Обработчик.Комментарий = НСтр("ru = 'Инициализация новых шаблонов документов.'");
...
// Обработчик подписки на событие ОбновитьРасписаниеОбработкиДокументовИнтеграцииСРаботаРоссии.
Процедура ОбновитьРасписаниеОбработкиДокументовИнтеграцииСРаботаРоссии(Источник, Отказ) Экспорт
	
	Если ЗарплатаКадры.ОтключитьБизнесЛогикуПриЗаписи(Источник) Тогда
		Возврат;
	КонецЕсли;
	
	ОбновитьРасписаниеЗаданийДляРаботаРоссии();
	
КонецПроцедуры

Процедура ОбновитьРасписаниеЗаданийДляРаботаРоссии() Экспорт
	
	ОбновитьРасписаниеЗаданияОбработкиДокументовПечатныхФорм();
	ОбновитьРасписаниеЗаданияОбновленияСостоянийДокументовЭДОБЗК();
	ИнтеграцияСРаботаРоссии.ОбновитьРасписаниеРегламентныхЗаданий();
...
Функция ОбъектЗаблокирован(СсылкаНаОбъект, ПометкаУдаления) Экспорт
...
Функция ИспользуютсяЭлектронныеДокументы(ПоДаннымИБ = Ложь)
	...
	Возврат ПолучитьФункциональнуюОпцию("ИспользуетсяЭДОБЗККабинетСотрудника")
		Или ИнтеграцияСРаботаРоссии.ДоступнаПередачаДокументовНаРаботаРоссии();
...
Функция РезультатЗапросаТребуетсяСогласиеНаПрисоединениеКЭДОБЗК(ТаблицаФизическихЛиц)
	...
	Если ИспользоватьШтатноеРасписание Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "КадровыеДанныеДругихСотрудников.Должность", "КадровыеДанныеДругихСотрудников.ДолжностьПоШтатномуРасписанию");
		...
		Если ДоступныРасширенныеПодсистемы Тогда
			Если Не ИнтеграцияУправлениеПерсоналом.ПубликоватьСтруктуруЮридическихЛиц() Тогда
				...
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "КадровыеДанныеДругихСотрудников.Подразделение", "КадровыеДанныеДругихСотрудников.МестоВСтруктуреПредприятия");
			КонецЕсли;
		КонецЕсли;
	Иначе
		ТекстыЗапросов = Новый Массив;
...
Процедура СформироватьСогласияНаПрисоединениеКЭДОБЗК(ТаблицаФизическихЛиц, РучноеФормирование = Ложь) Экспорт
	...
	ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК = Новый Соответствие;
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если Выборка.УсловиеДопуска = Перечисления.УсловияДопускаСотрудниковКЭДОБЗК.СогласияНаЭДОБЗКСобраныРанее Тогда
				ДобавитьФизическоеЛицоОрганизации(
					Выборка.ФизическоеЛицо, Выборка.Организация, ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК);
			Иначе
	...
	КонецЕсли;
	Для Каждого ФизическиеЛицаДляВключенияКЭДО Из ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК Цикл
		РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК.УстановитьПризнакПолученияСогласияЭДОБЗКРанее(
			ФизическиеЛицаДляВключенияКЭДО.Значение, ФизическиеЛицаДляВключенияКЭДО.Ключ, Истина);
	КонецЦикла;
...
Функция ФизическиеЛицаПрисоединенныеКЭДОБЗК(ФизическиеЛица, Приложение) Экспорт
	...
	Возврат РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК.ФизическиеЛицаПрисоединенныеКЭДОБЗК(ФизическиеЛица, Приложение);
...
Процедура ДобавитьФизическоеЛицоОрганизации(ФизическоеЛицо, Организация, ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК)
	ФизическиеЛицаОрганизации = ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК.Получить(Организация);
	Если ФизическиеЛицаОрганизации = Неопределено Тогда
		ФизическиеЛицаОрганизации = Новый Массив;
		ФизическиеЛицаОрганизацийДляВключенияЭДОБЗК.Вставить(Организация, ФизическиеЛицаОрганизации);
	КонецЕсли;
	ФизическиеЛицаОрганизации.Добавить(ФизическоеЛицо);
КонецПроцедуры
...
Процедура УстановитьИспользованиеЭДОБЗКПриВключенииИнтеграцииСРаботаРоссии(Источник, Отказ) Экспорт
...
Процедура УстановитьИспользованиеЭДОБЗК(ИспользоватьИнтеграциюСРаботаРоссии = Неопределено, ИспользуетсяЭДОБЗККабинетСотрудника = Неопределено) Экспорт
	
	Если ИспользоватьИнтеграциюСРаботаРоссии = Неопределено Тогда
		ИспользоватьИнтеграциюСРаботаРоссии = ИнтеграцияСРаботаРоссии.ДоступнаПередачаДокументовНаРаботаРоссии();
	КонецЕсли;
	...
	НовоеЗначение = (ИспользоватьИнтеграциюСРаботаРоссии Или ИспользуетсяЭДОБЗККабинетСотрудника);
	УстановитьПривилегированныйРежим(Истина);
...
Функция РезультатыПроверкиДокументовЭДОБЗК(ДатаНачалаПроверки, ПроверяемыеОбъекты)
	...
	Запрос.Текст =
		...
		|	ДокументЭДОБЗК.ВидДокумента КАК ВидДокумента,
...
Функция РезультатыПроверкиЧтоВсеСотрудникиОзнакомлены(Знач ДатаНачалаПроверки, Знач ПроверяемыеОбъекты, Знач ЛимитПроблем, РезультатыПроверки = Неопределено)
	...
	Запрос.Текст =
		...
		|	И НЕ ДокументЭДОБЗКВнешниеПодписанты.Ссылка.ВидДокумента.ВариантПодписания В (ЗНАЧЕНИЕ(Перечисление.ВариантыПодписанияДокументовЭДОБЗК.НеТребуется), ЗНАЧЕНИЕ(Перечисление.ВариантыПодписанияДокументовЭДОБЗК.НеобходимаСобственноручнаяПодпись))
...
Функция ДобавитьПолеСведенияОНаличииКабинетаСотрудника(ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
	
	ДобавленоПолеСведений = Ложь;
	Если НеобходимыСведенияОНаличииКабинетаСотрудника(ИмяПоля) Тогда
		
		ДобавленоПолеСведений = Истина;
		ИсточникиДанных.Вставить("СведенияОНаличииКабинетаСотрудника", Истина);
		
		ПутьКДанным = ПутьКДаннымСведенияОНаличииКабинетаСотрудника(ИмяПоля);
...
Функция НеобходимыСведенияОНаличииКабинетаСотрудника(Знач ИмяПоля) Экспорт
	
	ИмяПоля = ВРег(ИмяПоля);
	Возврат ИмяПоля = ВРег("ИмеетКабинетСотрудника");
	
КонецФункции

Функция ПутьКДаннымСведенияОНаличииКабинетаСотрудника(ИмяПоля)
	
	ИмяПоляВВерхнемРегистре = ВРег(ИмяПоля);
	ПутьКДанным = "";
	
	Если ИмяПоляВВерхнемРегистре = ВРег("ИмеетКабинетСотрудника") Тогда
		Если ПолучитьФункциональнуюОпцию("ИспользуетсяСервисКабинетСотрудника") Тогда
			ПутьКДанным = "	СведенияОНаличииКабинетаСотрудника.ИмеетКабинетСотрудника";
		Иначе
			ПутьКДанным = "	ЛОЖЬ";
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьТекстЗапросаВТСведенияОНаличииКабинетаСотрудника(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных) Экспорт
	
	Если ИсточникиДанных.Получить("СведенияОНаличииКабинетаСотрудника") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользуетсяСервисКабинетСотрудника") Тогда
		...
		ЗапросСведенийОНаличииКабинетаСотрудника = Новый Запрос;
		ЗапросСведенийОНаличииКабинетаСотрудника.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	ВЫБОР
			|		КОГДА ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ЕСТЬ NULL
			|			ТОГДА ЛОЖЬ
			|		ИНАЧЕ ИСТИНА
			|	КОНЕЦ КАК ИмеетКабинетСотрудника,
			|	ТаблицаОтборов.ФизическоеЛицо КАК ФизическоеЛицо
			|ПОМЕСТИТЬ ВТСведенияОНаличииКабинетаСотрудника
			|ИЗ
			|	ВТОтборовФизическихЛиц КАК ТаблицаОтборов
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
			|		ПО ТаблицаОтборов.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка";
		
		ЗапросСведенийОНаличииКабинетаСотрудника.Текст = СтрЗаменить(ЗапросСведенийОНаличииКабинетаСотрудника.Текст, "ВТОтборовФизическихЛиц",
		...
		КадровыйУчет.УстановитьВТекстеЗапросаИмяПоляФизическоеЛицо(
			ЗапросСведенийОНаличииКабинетаСотрудника.Текст, "ТаблицаОтборов.ФизическоеЛицо", ОписательВременнойТаблицыОтборов.ИмяПоляФизическоеЛицо);
		
		ЗарплатаКадрыОбщиеНаборыДанных.УстановитьВыборкуТолькоРазрешенныхДанных(ЗапросСведенийОНаличииКабинетаСотрудника.Текст, ТолькоРазрешенные);
		
		ЗарплатаКадрыОбщиеНаборыДанных.СкопироватьПараметрыЗапроса(Запрос, ЗапросСведенийОНаличииКабинетаСотрудника);
		
		ЧастиЗапроса.Добавить(ЗапросСведенийОНаличииКабинетаСотрудника.Текст);
		ЧастиЗапроса.Добавить(ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов());
		ЧастиЗапроса.Добавить(Запрос.Текст);
		
		ЧастиЗапроса.Добавить(
			"	{ЛЕВОЕ СОЕДИНЕНИЕ ВТСведенияОНаличииКабинетаСотрудника КАК СведенияОНаличииКабинетаСотрудника
			|	ПО " + КадровыйУчет.ПутьКПолюФизическоеЛицо("ТаблицаОтборов", ОписательВременнойТаблицыОтборов.ИмяПоляФизическоеЛицо) + " = СведенияОНаличииКабинетаСотрудника.ФизическоеЛицо}");
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	ОписаниеПоля.ИмяПоля = "ИмеетКабинетСотрудника";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Имеет кабинет сотрудника'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ИСТИНА";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".ИмеетКабинетСотрудника";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);

КонецПроцедуры

Функция ДобавитьКритерийПоискаСведенияОНаличииКабинетаСотрудника(КритерииПоиска, УсловиеПоиска) Экспорт
	
	ИмяПоля = ВРег(УсловиеПоиска.ЛевоеЗначение);
	
	КритерийДобавлен = Ложь;
	Если НеобходимыСведенияОНаличииКабинетаСотрудника(УсловиеПоиска.ЛевоеЗначение) Тогда
		Если ИмяПоля = ВРег("ИмеетКабинетСотрудника") Тогда
			УсловиеПоиска.ЛевоеЗначение = "ВЫБОР КОГДА Т.Ссылка ЕСТЬ NULL ТОГДА ЛОЖЬ ИНАЧЕ ИСТИНА КОНЕЦ";
			УсловиеПоиска.ПравоеЗначение = "ИСТИНА";
			УсловиеПоиска.ОтносительныйПуть = Ложь;
		КонецЕсли;
		КритерийДобавлен = Истина;
		КадровыйУчет.ДобавитьКритерийПоискаСотрудников(КритерииПоиска, "РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника", УсловиеПоиска);
		ДополнительноеУсловие = Новый Структура;
		ДополнительноеУсловие.Вставить("ЛевоеЗначение", "ТИПЗНАЧЕНИЯ(Т.Ссылка)");
		ДополнительноеУсловие.Вставить("ВидСравнения", "=");
		ДополнительноеУсловие.Вставить("ПравоеЗначение", "ТИП(Справочник.ФизическиеЛица)");
		ДополнительноеУсловие.Вставить("ОтносительныйПуть", Ложь);
		КадровыйУчет.ДобавитьКритерийПоискаСотрудников(КритерииПоиска, "РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника", ДополнительноеУсловие);
	КонецЕсли;
...
Процедура УстановитьЗапросДинамическогоСпискаФормыВыбораПубликуемыхСотрудников(УправляемаяФорма) Экспорт
	...
	ЭДОБЗКВнутренний.УстановитьЗапросДинамическогоСпискаФормыВыбораПубликуемыхСотрудников(УправляемаяФорма);
	
КонецПроцедуры

Функция ДанныеРукводителейПодразделений(Подразделения, ДатаСведений) Экспорт
	
	Результат = Новый Структура;
	
	Результат.Вставить("ПодразделенияРуководители",
		ИнтеграцияУправлениеПерсоналом.РуководителиПодразделений(
			Подразделения, ИнтеграцияУправлениеПерсоналом.ВыгружаемыеФизическиеЛица(Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника)));
	
	Результат.Вставить("ДанныеРуководителей", Новый Соответствие());
	ТаблицаРуководителей = Новый ТаблицаЗначений;
	ТаблицаРуководителей.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций,СправочникСсылка.СтруктураПредприятия"));
	ТаблицаРуководителей.Колонки.Добавить("Руководитель", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
	
	Для Каждого ДанныеРуководителя Из Результат.ПодразделенияРуководители Цикл
		НоваяСтрока = ТаблицаРуководителей.Добавить();
		НоваяСтрока.Подразделение = ДанныеРуководителя.Ключ;
		НоваяСтрока.Руководитель = ДанныеРуководителя.Значение;
	КонецЦикла;
	
	Руководители = ОбщегоНазначения.ВыгрузитьКолонку(Результат.ПодразделенияРуководители, "Значение", Истина);
	ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(Руководители, Неопределено);
	ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(Руководители, Справочники.ФизическиеЛица.ПустаяСсылка());
	
	Если ЗначениеЗаполнено(Руководители) Тогда
		ПараметрыПолучения = КадровыйУчет.ПараметрыПолученияСотрудниковОрганизацийПоСпискуФизическихЛиц();
		ПараметрыПолучения.НачалоПериода = НачалоДня(ДатаСведений);
		ПараметрыПолучения.ОкончаниеПериода = КонецДня(ДатаСведений);
		ПараметрыПолучения.СписокФизическихЛиц = Руководители;
		ПараметрыПолучения.КадровыеДанные = "ГоловнаяОрганизация,ВидЗанятости,Должность";
		
		Запрос = Новый Запрос;
		Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		Запрос.УстановитьПараметр("ТаблицаРуководителей", ТаблицаРуководителей);
		Запрос.УстановитьПараметр("Подразделения", Подразделения);
		
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ТаблицаРуководителей.Подразделение КАК Подразделение,
			|	ТаблицаРуководителей.Руководитель КАК Руководитель
			|ПОМЕСТИТЬ ВТТаблицаРуководителей
			|ИЗ
			|	&ТаблицаРуководителей КАК ТаблицаРуководителей
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ТаблицаРуководителей.Подразделение КАК Подразделение,
			|	ПодразделенияОрганизаций.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
			|	ТаблицаРуководителей.Руководитель КАК Руководитель
			|ПОМЕСТИТЬ ВТОрганизацииПодразделений
			|ИЗ
			|	ВТТаблицаРуководителей КАК ТаблицаРуководителей
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПодразделенияОрганизаций КАК ПодразделенияОрганизаций
			|		ПО ТаблицаРуководителей.Подразделение = ПодразделенияОрганизаций.Ссылка
			|ГДЕ
			|	ПодразделенияОрганизаций.Ссылка В(&Подразделения)";
		
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.ЭДОБЗКРасширенный") Тогда
			Запрос.Текст = Запрос.Текст +
				"
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				|	ТаблицаРуководителей.Подразделение,
				|	СтруктураПредприятия.Источник.ГоловнаяОрганизация,
				|	ТаблицаРуководителей.Руководитель
				|ИЗ
				|	ВТТаблицаРуководителей КАК ТаблицаРуководителей
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СтруктураПредприятия
				|		ПО ТаблицаРуководителей.Подразделение = СтруктураПредприятия.Ссылка
				|ГДЕ
		...
		ВыгружаемыеСотрудники = ИнтеграцияУправлениеПерсоналом.ВыгружаемыеСотрудники(Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника);
		Запрос.УстановитьПараметр("ВыгружаемыеСотрудники", ВыгружаемыеСотрудники);
		
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ОрганизацииПодразделений.Подразделение КАК Подразделение,
			|	МАКСИМУМ(СотрудникиОрганизации.Должность) КАК Должность
			|ИЗ
			|	ВТОрганизацииПодразделений КАК ОрганизацииПодразделений
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиОрганизации КАК СотрудникиОрганизации
			|		ПО ОрганизацииПодразделений.Руководитель = СотрудникиОрганизации.ФизическоеЛицо
			|			И (ОрганизацииПодразделений.ГоловнаяОрганизация = НЕОПРЕДЕЛЕНО
			|				ИЛИ ОрганизацииПодразделений.ГоловнаяОрганизация = СотрудникиОрганизации.ГоловнаяОрганизация)
			|ГДЕ
			|	СотрудникиОрганизации.Сотрудник В(&ВыгружаемыеСотрудники)
			...
			|
			|СГРУППИРОВАТЬ ПО
			|	ОрганизацииПодразделений.Подразделение
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	ОрганизацииПодразделений.Подразделение,
			|	МАКСИМУМ(СотрудникиОрганизации.Должность)
			|ИЗ
			|	ВТОрганизацииПодразделений КАК ОрганизацииПодразделений
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиОрганизации КАК СотрудникиОрганизации
			|			ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиОрганизации КАК СотрудникиОрганизацииОсновные
			|			ПО СотрудникиОрганизации.ФизическоеЛицо = СотрудникиОрганизацииОсновные.ФизическоеЛицо
			|				И СотрудникиОрганизации.ГоловнаяОрганизация = СотрудникиОрганизацииОсновные.ГоловнаяОрганизация
			|				И (СотрудникиОрганизацииОсновные.ВидЗанятости В (ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы), ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.Совместительство)))
			|				И (СотрудникиОрганизацииОсновные.Сотрудник В (&ВыгружаемыеСотрудники))
			|		ПО ОрганизацииПодразделений.Руководитель = СотрудникиОрганизации.ФизическоеЛицо
			|			И (ОрганизацииПодразделений.ГоловнаяОрганизация = НЕОПРЕДЕЛЕНО
			|				ИЛИ ОрганизацииПодразделений.ГоловнаяОрганизация = СотрудникиОрганизации.ГоловнаяОрганизация)
			|ГДЕ
			|	СотрудникиОрганизации.Сотрудник В(&ВыгружаемыеСотрудники)
			|	И СотрудникиОрганизации.ВидЗанятости В (ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ВнутреннееСовместительство))
			|	И СотрудникиОрганизацииОсновные.ФизическоеЛицо ЕСТЬ NULL
			|
			|СГРУППИРОВАТЬ ПО
			|	ОрганизацииПодразделений.Подразделение";
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Результат.ДанныеРуководителей.Вставить(Выборка.Подразделение, Выборка.Должность);
		КонецЦикла;
...
Функция СвойстваВидаДокументов(ВидДокумента) Экспорт
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидДокумента, "КодКадровогоДокумента,ВариантПодписания,ТипЭлектроннойПодписи");
КонецФункции
