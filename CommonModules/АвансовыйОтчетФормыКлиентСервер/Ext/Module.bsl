...
// Устанавливает текст настроек учета НДС в табличных частях авансового отчета
// 
// Параметры:
// 	ДанныеСтроки - ДанныеФормыЭлементКоллекции - данные строки, в которой необходимо установить текст настроек учета НДС
// 	ИмяТЧ - наименование таблицы авансого отчета, в строке которой устанавливается текст настройки учета НДС
// 	РаздельныйУчетНДСНаСчете19 - Булево - Истина, если ведется раздельный учет НДС
Процедура УстановитьТекстУчетНДС(ДанныеСтроки, ИмяТЧ, РаздельныйУчетНДСНаСчете19) Экспорт
	
	Если Не ДанныеСтроки.ПредъявленСФ Тогда
		// НДС не принимается к вычету
		ДанныеСтроки.УчетНДС = НСтр("ru='Не принимается к вычету'");
		Возврат;
	КонецЕсли;		
	
	ЧастиОписания = Новый Массив;
	
	Если РаздельныйУчетНДСНаСчете19 И СчетаУчетаВДокументахВызовСервераПовтИсп.ПользовательУправляетСчетамиУчета() Тогда 
		// НДС требуется учесть, а способ учета НДС указан в отдельном поле
		ЧастиОписания.Добавить(ДанныеСтроки.СпособУчетаНДС);
	Иначе	
		// Принимается к вычету
		ЧастиОписания.Добавить(НСтр("ru='Принимается к вычету'"));
	КонецЕсли;	
		
	Если ИмяТЧ = "Билеты" Тогда		
		
		ЧастиОписания.Добавить(
			СтрШаблон(НСтр("ru='документ оплаты %1 от %2'"),
				ДанныеСтроки.НомерДокументаОплаты,
				Формат(ДанныеСтроки.ДатаДокументаОплаты, "ДЛФ=D;")));
				
		Если ЗначениеЗаполнено(ДанныеСтроки.Перевозчик) Тогда
			ЧастиОписания.Добавить(ДанныеСтроки.Перевозчик);				
		КонецЕсли;		
		
	Иначе	
		
		Если ДанныеСтроки.ВидДокументаРасхода = ПредопределенноеЗначение("Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.ДругойДокумент") Тогда
			ЧастиОписания.Добавить(
				СтрШаблон(НСтр("ru='чек %1 от %2'"),
					ДанныеСтроки.НомерДокументаОплаты,
					Формат(ДанныеСтроки.ДатаДокументаОплаты, "ДЛФ=D;")));
		КонецЕсли;
		
		Если ДанныеСтроки.ВидДокументаРасхода = ПредопределенноеЗначение("Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.Чек") 
			 Или ДанныеСтроки.ВидДокументаРасхода = ПредопределенноеЗначение("Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.ДругойДокумент") Тогда			
			ЧастиОписания.Добавить(
				СтрШаблон(НСтр("ru='СФ %1 от %2'"),
					ДанныеСтроки.НомерСФ,
					Формат(ДанныеСтроки.ДатаСФ, "ДЛФ=D;")));						
		КонецЕсли;		
		
		Если ДанныеСтроки.ВидДокументаРасхода = ПредопределенноеЗначение("Перечисление.ВидыДокументовПодтверждающихКомандировочныеРасходы.Билет") Тогда
			ЧастиОписания.Добавить(
				СтрШаблон(НСтр("ru='чек %1 от %2'"),
					ДанныеСтроки.НомерДокументаОплаты,
					Формат(ДанныеСтроки.ДатаДокументаОплаты, "ДЛФ=D;")));
		...
		Если ЗначениеЗаполнено(ДанныеСтроки.Поставщик) Тогда
			ЧастиОписания.Добавить(ДанныеСтроки.Поставщик);				
		КонецЕсли;
					
	КонецЕсли;		
	
	ДанныеСтроки.УчетНДС = СтрСоединить(ЧастиОписания, ", ");
...
Функция НачатьЗаполнениеСчетовУчета(ПричиныИзменения, Объект = Неопределено, СтрокаСписка = Неопределено, КонтейнерОбъект = Неопределено, КонтейнерСтрокаСписка = Неопределено) Экспорт
	...
	// Билеты.ПредъявленСФ
	Если ПараметрыЗаполнения.ПричиныИзменения.Найти("Билеты.ПредъявленСФ") <> Неопределено Тогда
		СчетаУчетаВДокументахКлиентСервер.НачатьЗаполнениеРеквизита(ПараметрыЗаполнения, "Билеты.СпособУчетаНДС");
	КонецЕсли;
