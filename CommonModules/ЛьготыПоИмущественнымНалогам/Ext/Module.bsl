...
Функция ТекстПодсказкиЛьготы(Льгота, Организация, Период, ОсновноеСредство = Неопределено) Экспорт 
	...
	СостояниеДокументовПоЛьготе = Справочники.ДокументыПодтверждающиеЛьготыПоИмущественнымНалогам.СостояниеДокументовПоЛьготе(
		Льгота,
		Организация,
		Период,
		ОсновноеСредство);
			
	Если СостояниеДокументовПоЛьготе.КоличествоДействующихВсего = 0 И СостояниеДокументовПоЛьготе.КоличествоВсего > 0 Тогда
		Подсказка.Добавить(Нстр("ru='Закончился срок действия подтверждающих документов'"));
		ЕстьОшибки = Истина;
	ИначеЕсли СостояниеДокументовПоЛьготе.КоличествоВсего > 0 Тогда
		КоличествоДокументовСтрокой = СтрокаСЧислом(";%1 подтверждающий документ;;%1 подтверждающих документа;%1 подтверждающих документов;%1 подтверждающих документа",
		    СостояниеДокументовПоЛьготе.КоличествоВсего,
		    ВидЧисловогоЗначения.Количественное,
		    "L=ru");
		Подсказка.Добавить(КоличествоДокументовСтрокой);
	Иначе
		Подсказка.Добавить(Нстр("ru='Укажите подтверждающие документы'"));
		ЕстьОшибки = Истина;
	КонецЕсли;	
