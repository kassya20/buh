...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьНачальныеНастройкиПользователей");
	Методы.Вставить("ПроизвестиЗаменуСсылокНаНовыйПредопределенныйЭлементГруппыПользователей");
	Методы.Вставить("ПереносОсновнойОрганизацииПриОбновленииС20");
	Методы.Вставить("ЗаполнениеРеквизитовОрганизацииПриОбновленииС20");
	Методы.Вставить("ЗаполнитьИспользуемыеСистемыНалогообложения");
	Методы.Вставить("ОбработатьСсылкиНаВедомости");
	Методы.Вставить("ВосстановитьЗначенияДополнительныхСведений");
	Методы.Вставить("УдалитьСубконтоОбъектыСтроительстваСоСчетаНДСприСтроительствеОсновныхСредств");
	Методы.Вставить("ПроизвестиЗаменуСсылокНаНовыйПредопределенныйЭлементСтранМира");
	Методы.Вставить("ОбновитьРИБПриПереходе20На30");
	Методы.Вставить("КонвертироватьДокументыФизическихЛицС20");
	Методы.Вставить("ОчиститьТиповыеОперацииОперацииБухПриПереходеС20");
	Методы.Вставить("УстановитьНеобходимостьЗаполненияПоследовательностей");
	Методы.Вставить("УстановитьКонстантыПриПереходеС20");
	Методы.Вставить("СброситьНеактуальностьРегламентныхОпераций");
	Методы.Вставить("ПеренестиПрисоединенныеФайлы");
	Методы.Вставить("ПереносКИиДопСвойствПриОбновленииС20");
	Методы.Вставить("ПодготовитьДанныеУчетнойПолитики");
	Методы.Вставить("ЗаполнитьСтраныМира");
	Методы.Вставить("ОбработатьУплатыТорговогоСбораНаУСНДоходыПриПереходеС20");
...
Процедура ПереносОсновнойОрганизацииПриОбновленииС20() Экспорт
	...
	НастройкаОсновнаяСтавкаНДС = ПланыВидовХарактеристик.УдалитьНастройкиПользователей["ОсновнаяСтавкаНДС"]; 
	...
	Запрос.УстановитьПараметр("НастройкаОсновнаяСтавкаНДС",    НастройкаОсновнаяСтавкаНДС);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	НастройкиБП20.Пользователь КАК Пользователь,
	...
	|		РегистрСведений.УдалитьНастройкиПользователей КАК РегистрЗначениеПрав
	|	ГДЕ
	|		РегистрЗначениеПрав.Настройка = &НастройкаВидимостиСчетовУчета
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		РегистрСведений.УдалитьНастройкиПользователей КАК РегистрЗначениеПрав
	|	ГДЕ
	|		РегистрЗначениеПрав.Настройка = &НастройкаОсновнаяСтавкаНДС) КАК НастройкиБП20
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекущаяНастройкаОсновнойСтавкиНДС =ОбщегоНазначенияБПВызовСервера.ПолучитьЗначениеПоУмолчанию("ОсновнаяСтавкаНДС", ИмяПользователяИБ);
		...
		ТекущаяНастройкаОсновнойСтавкиНДС = ?(НЕ ЗначениеЗаполнено(ТекущаяНастройкаОсновнойСтавкиНДС), Выборка.ОсновнаяСтавкаНДС, ТекущаяНастройкаОсновнойСтавкиНДС);
...
Процедура ПереносКИПриОбновленииС20()
	...
	Пока ВыборкаОбъект.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаОбъект.Объект)
			Или Не ОбщегоНазначения.СсылкаСуществует(ВыборкаОбъект.Объект) Тогда
			Продолжить;
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(Выборка.Вид)
				Или Не ОбщегоНазначения.СсылкаСуществует(Выборка.Вид) Тогда
				Продолжить;
			КонецЕсли;
			...
			ИначеЕсли Выборка.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
				...
				ПоляТелефона = УправлениеКонтактнойИнформацией.СведенияОТелефоне();
...
Процедура ЗаполнитьСтраныМира() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УчастникЕАЭС
			И ЗначениеЗаполнено(Выборка.МеждународноеНаименование)
			И ЗначениеЗаполнено(Выборка.КодАльфа3) Тогда
			Продолжить;
		КонецЕсли;
		...
		СтрокаКлассификатора = ТаблицаКлассификатора.Найти(Выборка.Код, "Код");
		Если СтрокаКлассификатора = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтранаМираОбъект = Выборка.Ссылка.ПолучитьОбъект();
		Если СтранаМираОбъект = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтранаМираОбъект.УчастникЕАЭС = СтрокаКлассификатора.УчастникЕАЭС;
		
		Если Не ЗначениеЗаполнено(СтранаМираОбъект.МеждународноеНаименование) Тогда
			СтранаМираОбъект.МеждународноеНаименование = СтрокаКлассификатора.МеждународноеНаименование;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтранаМираОбъект.КодАльфа3) Тогда
			СтранаМираОбъект.КодАльфа3 = СтрокаКлассификатора.КодАльфа3;
		КонецЕсли;
		
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(СтранаМираОбъект);
