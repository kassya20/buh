...
Процедура ПроверитьИВыгрузитьПлатежныеДокументы(Организация, ДокументыОплаты, Оповещение, УникальныйИдентификатор) Экспорт
	
	Если Не ЗначениеЗаполнено(ДокументыОплаты) Тогда
		Возврат;
	КонецЕсли;
	
	ПлатежноеПоручение = Неопределено;
	Если ДокументыОплаты.Количество() = 1 Тогда
		ПлатежноеПоручение = ДокументыОплаты[0];
		АдресФайла = ПомощникиПоУплатеНалоговИВзносовВызовСервера.ПроверитьИВыгрузитьПлатежныйДокумент(
			Организация, ПлатежноеПоручение, УникальныйИдентификатор);
		ВыгружатьСразу = АдресФайла <> Неопределено;
	Иначе
		ВыгружатьСразу = Ложь;
	КонецЕсли;
	...
	Если ВыгружатьСразу Тогда
		Попытка
			РезультатВыгрузки = ПолучитьФайл(АдресФайла, "1c_to_kl.txt");
			Если РезультатВыгрузки = Истина Или ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
				ВыполнитьОбработкуОповещения(Оповещение, ДокументыОплаты);
			КонецЕсли;
		Исключение
			ВыгружатьСразу = Ложь;
			...
			ПомощникиПоУплатеНалоговИВзносовВызовСервера.ЗаписатьОшибкуВыгрузкиПлатежаВЖурналРегистрации(ТекстОшибки);
		КонецПопытки;
	КонецЕсли;
	
	Если Не ВыгружатьСразу Тогда
		СписокДокументов = Новый СписокЗначений;
		Если ЗначениеЗаполнено(ПлатежноеПоручение) Тогда
			СписокДокументов.Добавить(ПлатежноеПоручение);
			РеквизитыДокумента = ПомощникиПоУплатеНалоговИВзносовВызовСервера.РеквизитыПлатежногоДокументаДляОтправки(
				ПлатежноеПоручение);
			НачалоПериода = РеквизитыДокумента.Дата;
			КонецПериода = РеквизитыДокумента.Дата;
			БанковскийСчет = РеквизитыДокумента.СчетОрганизации;
		Иначе
			СписокДокументов.ЗагрузитьЗначения(ДокументыОплаты);
			ПериодДокументов = ПомощникиПоУплатеНалоговИВзносовВызовСервера.ПериодПлатежныхДокументов(ДокументыОплаты);
			НачалоПериода = ПериодДокументов.НачалоПериода;
			КонецПериода = ПериодДокументов.КонецПериода;
		...
		СтруктураПараметров.Вставить("УплатаНалогов", Истина);
		
		ОткрытьФорму("Обработка.КлиентБанк.Форма.Форма", СтруктураПараметров, , , , , Оповещение);
	КонецЕсли;
