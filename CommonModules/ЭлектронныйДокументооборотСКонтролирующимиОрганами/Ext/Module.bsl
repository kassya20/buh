...
Процедура ОпределитьТаблицуОписанияОбъектовРегламентированнойОтчетности(ТаблицаОписания) Экспорт
	...
	Иначе
		НаименованиеУПУП = НСтр("ru = 'Уведомление о предоставлении полномочий представителю (ПФР, не применяется с 04.04.2026)'");
		НаименованиеУПРУП = НСтр("ru = 'Уведомление о прекращении полномочий представителя (ПФР, не применяется с 04.04.2026)'");
	КонецЕсли;
...
Процедура ОпределитьТаблицуОписанияОбъектовЗаявленияПоЭлДокументооборотуСПФР(ТаблицаОписания) Экспорт
	...
	ДополнениеНаименования =
		?(ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ЗПЭД"),
		"", " (" + НСтр("ru = 'не применяется с 04.04.2026'") + ")");
	
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.Наименование = Строка(Перечисления.ВидыЗаявленийНаЭДОВПФР.НаПодключение) + ДополнениеНаименования;
	...
	НоваяСтрока = ТаблицаОписания.Добавить();
	НоваяСтрока.Наименование = Строка(Перечисления.ВидыЗаявленийНаЭДОВПФР.НаОтключение) + ДополнениеНаименования;
	НоваяСтрока.ТипОбъекта = Тип("ДокументСсылка.ЗаявленияПоЭлДокументооборотуСПФР");
	НоваяСтрока.ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ПФР;
	НоваяСтрока.ГруппаВДереве = ГруппаВДереве;
	НоваяСтрока.ВидДокумента = Перечисления.СтраницыЖурналаОтчетность.Уведомления;
...
Функция ПолучитьГруппуНовогоСобытияДокументооборотаСКО(Запись)
	...
	ЗавершенныеСостояния = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		+ "Зарегистрировано, не пройдена проверка госоргана;Зарегистрировано, погашено;"
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("СравнитьТабличныеДокументыЗаявлений", Истина);
	Методы.Вставить("ПолучитьКонтактыПФР", Истина);
	Методы.Вставить("ФоноваяЗаменаВОрганизацияхИПодразделенияхКодОрганаФСГС", Истина);
	Методы.Вставить("СконвертироватьРеквизитыЗаявленияПо1СОтчетности");
	Методы.Вставить("ЗаполнитьРегистрДокументыПоТребованиюФНС");
	Методы.Вставить("НачатьОтправкуОблачногоЗаявленияВБазуЗаявок", Истина);
	Методы.Вставить("ЗапуститьПереносТранспортныхСообщенийВПрисоединенныеФайлы");
	Методы.Вставить("ЗавершитьНастройкуЗаявлений");
	Методы.Вставить("ЗаполнитьДатыОтправки");
	Методы.Вставить("ИсправитьНекорректныеСостоянияОтправок2НДФЛ");
	Методы.Вставить("ПеренестиПризнакНепрочтенности");
	Методы.Вставить("ИсправитьНекорректныеСостоянияОтправок2НДФЛ_2");
	Методы.Вставить("ДобавитьНовыеДокументыВРегистрДокументыПоТребованиюФНС");
	Методы.Вставить("ВключитьОбменСФССПриНеобходимости");
	Методы.Вставить("СброситьПризнакПодпискиСтрахователяСЭДОФСС");
	Методы.Вставить("ЗаполнитьТипСверкиИОН");
	Методы.Вставить("ЗаменитьВОрганизацияхИПодразделенияхКодОрганаФСГСОтложенно");
	Методы.Вставить("ИсправитьНекорректныеСостоянияОтправок2НДФЛ_1110307");
	Методы.Вставить("ЗаполнитьСпособПолученияСертификата");
	Методы.Вставить("ОбновитьНастройкиПодключенияСЭДО");
	Методы.Вставить("ПолучитьИдентификаторАдресаФИАСФоновоеЗадание", Истина);
	Методы.Вставить("ДобавитьНовыеДокументыВРегистрДокументыПоТребованиюФНС_2");
	Методы.Вставить("ЗаменитьВОрганизацияхИПодразделенияхКодОрганаФСГС");
	Методы.Вставить("НачатьОпределениеТекстаЗаявления", Истина);
	Методы.Вставить("ОтослатьПисьмоПартнеру", Истина);
	Методы.Вставить("ОпределениеИзмененныхПараметровПодключенияВФоне", Истина);
	Методы.Вставить("ЗаполнитьПериодЗапросовИОН");
	Методы.Вставить("РасшифроватьТранспортныеСообщенияВФоне", Истина);
	Методы.Вставить("ОтправитьОбратнуюСвязьСервер", Истина);
	Методы.Вставить("ЗаполнитьСлужебныеРегистрыОбменаСЭДО");
	Методы.Вставить("ПолучитьДанныеССервераКАФоновоеЗадание", Истина);
КонецПроцедуры
