...
//  МакетКомпоновки - МакетКомпоновкиДанных - макет компоновки данных, содержащий внешние наборы данных
// 
...
Функция ВнешниеДанные(ИмяОбъекта, МакетКомпоновки) Экспорт
	...
	ИначеЕсли ИмяОбъекта = "БуквенныеКодыРазделовОКВЭД2" Тогда
		...
		Возврат КлассификаторОКВЭДБП.РазделыКодов();
...
Процедура ЧисленностьПерсонала(Показатели, Формула, ПараметрыОтчета) Экспорт
	...
	ДанныеЧисленности = УчетЗарплаты.ПоказателиЧисленностиФормСтатистики(
		ПараметрыОтчета.Организация,
	...
	Для Каждого Показатель Из ПоказателиЧисленности Цикл
		...
		Если ДанныеЧисленности.Колонки.Найти(Характеристика) <> Неопределено Тогда
			Показатель.Значение = ДанныеЧисленности.Итог(Характеристика);
		КонецЕсли;
