...
Функция ПараметрыФормыВыбораРегламентированногоОтчета() Экспорт
	...
	Результат.Вставить("МаксВерсияФормыРеглОтчета", "");
	...
	Результат.Вставить("ПодсказкаДляВыбораРеглОтчета",      "");
...
//
// Параметры:
//  Версия1  - Строка - номер версии в формате БРО, например, "ФормаОтчета2024Кв1".
//  Версия2  - Строка - второй сравниваемый номер версии.
//
// Возвращаемое значение:
//   Число   - больше 0, если Версия1 > Версия2; 0, если версии равны.
//	 Неопределено - если исходные версии не являются корректными.
//
Функция СравнитьВерсииФормРеглОтчета(Знач Версия1, Знач Версия2) Экспорт

	Версия1 = ВРег(СокрЛП(Версия1));
	Версия2 = ВРег(СокрЛП(Версия2));
    ...
    Если СтрНачинаетсяС(Версия1, "ФОРМАОТЧЕТА") Тогда
   		Год1 = Сред(Версия1, 12, 4);
    КонецЕсли;

    Если СтрНачинаетсяС(Версия2, "ФОРМАОТЧЕТА") Тогда
		Год2 = Сред(Версия2, 12, 4);
	КонецЕсли;
	
	// Иногда в течение квартала действуют несколько редакций форм, в этом случае БРО именует их по типу ФормаОтчета2020Кв4_1,
	// тогда для расчета веса такой формы добавляем число после "_" к номеру квартала.
	ПозицияКвартала = СтрНайти(Версия1, "КВ");
	Если ПозицияКвартала > 0 Тогда
		Квартал1 = Сред(Версия1, ПозицияКвартала + 2, 1);
		
		ПозицияДопНомера = СтрНайти(Версия1, "_", , ПозицияКвартала);
		Если ПозицияДопНомера > 0 Тогда
			ДопНомер1 = Сред(Версия1, ПозицияДопНомера + 1, СтрДлина(Версия1) - ПозицияДопНомера);
		КонецЕсли;
	КонецЕсли;


	ПозицияКвартала = СтрНайти(Версия2, "КВ");
	Если ПозицияКвартала > 0 Тогда
		Квартал2 = Сред(Версия2, ПозицияКвартала + 2, 1);
		
		ПозицияДопНомера = СтрНайти(Версия2, "_", , ПозицияКвартала);
		Если ПозицияДопНомера > 0 Тогда
			ДопНомер2 = Сред(Версия2, ПозицияДопНомера + 1, СтрДлина(Версия2) - ПозицияДопНомера);
		КонецЕсли;
	КонецЕсли;
	
	// Преобразуем строки к числам.
	Год1 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Год1);
	Год2 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Год2);
	
	Квартал1 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Квартал1);
	Квартал2 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Квартал2);
	
	ДопНомер1 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ДопНомер1);
	ДопНомер2 = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(ДопНомер2);
	
	Если НЕ ЗначениеЗаполнено(Год1)
		ИЛИ НЕ ЗначениеЗаполнено(Год2)
		ИЛИ НЕ ЗначениеЗаполнено(Квартал1)
		ИЛИ НЕ ЗначениеЗаполнено(Квартал2) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// Добавим допонительный номер к кварталу для увеличения его веса.
	Если ЗначениеЗаполнено(ДопНомер1) Тогда
		Квартал1 = Квартал1 + ДопНомер1;
	КонецЕсли;

	Если ЗначениеЗаполнено(ДопНомер2) Тогда
		Квартал2 = Квартал2 + ДопНомер2;
	КонецЕсли;
	
	Если Год1 = Год2 Тогда
		Результат = Квартал1 - Квартал2;
	Иначе
		Результат = Год1 - Год2;
	КонецЕсли;
