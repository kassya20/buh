...
Функция ЗаголовокГиперссылкиЗаявления(СсылкаОснования) Экспорт
	...
	Если Заявление = Неопределено Тогда
		Если ТекущаяДатаСеанса() >= ДатаВступленияВСилуФорм2021Года() Тогда
			Возврат НСтр("ru = 'Ввести сведения для реестра прямых выплат СФР'");
		Иначе
...
Процедура ОбновитьНадписьВБольничном(Надпись, Больничный, РазрешенВводНовыхСведений) Экспорт
	...
	Если Заявление = Неопределено Тогда
		...
		ИначеЕсли ТекущаяДатаСеанса() >= ДатаВступленияВСилуФорм2021Года() Тогда
			ЗаголовокНадписи = НСтр("ru = '<a href=""СоздатьСведения"">Ввести сведения для реестра прямых выплат СФР</a>'");
		Иначе
	...
	Для Каждого Пособие Из Пособия Цикл
		...
		Если КатегорииПрямыхВыплат.Найти(ОписаниеНачисления.КатегорияНачисленияИлиНеоплаченногоВремени) <> Неопределено
			...
			Если ПособиеПлатитУчастникПилотногоПроекта Тогда
				...
				Иначе
					ТекстОшибки = НСтр("ru = 'С %1 пособие %2 не может быть начислено. Выплата пособий передана СФР.'");
				КонецЕсли;
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					...
					Пособие.Начисление);
			Иначе
				ТекстОшибки = НСтр("ru = 'Пособие, начисляемое в карантине по коронавирусу, не может быть начислено страхователем. Выплата пособий передана СФР.'");
			КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение сотрудников и идентификаторов СФР в реестрах прямых выплат.'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Поддержка формата прямых выплат СФР 1.7.7 в документе ""Уведомление о прекращении отпуска по уходу за ребенком"".'");
	...
	Обработчик.Комментарий     = НСтр("ru = 'Заполнение сведений для прямых выплат СФР.'");
...
Функция ДатаВступленияВСилуФорм2025() Экспорт
	Возврат '20250615';
КонецФункции
...
Процедура ОбновитьДанныеИсправленийОплатДляВозмещенияВыплатРодителямДетейИнвалидов(СтруктураПараметров) Экспорт
	Модуль = МодульРасширеннойРеализации();
	Если Модуль <> Неопределено Тогда
		Модуль.ОбновитьДанныеИсправленийОплатДляВозмещенияВыплатРодителямДетейИнвалидов(СтруктураПараметров);
	КонецЕсли;
КонецПроцедуры
...
Функция ОбновитьДанныеСреднегоЗаработкаДляОплатыОтпускаСФР(ДокументОбъект, ПараметрыФиксации) Экспорт
	...
	Если Модуль = Неопределено Тогда
		Возврат Ложь;
	Иначе
		Возврат Модуль.ОбновитьДанныеСреднегоЗаработкаДляОплатыОтпускаСФР(ДокументОбъект, ПараметрыФиксации);
	КонецЕсли;
КонецФункции
