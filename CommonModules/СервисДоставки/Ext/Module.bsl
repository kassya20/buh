	...
	Если ЗначениеЗаполнено(ТипГрузоперевозки) И ПараметрыФормы.ДокументыОснования.Количество() > 0 Тогда
		...
		Если СтрНачинаетсяС(ПараметрыФормы.ИмяФормы, "КарточкаЗаказа") И ПараметрыФормы.РежимМастера = 0 Тогда
			...
			ИначеЕсли ПараметрыФормы.ИмяФормы = "КарточкаЗаказаСДЭК" Тогда
				ПараметрыЗаказа.ТипЗаказа = СервисДоставкиКлиентСервер.ТипыЗаказовСДЭК().ИнтернетМагазин;
			КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить(СервисДоставкиКлиентСервер.ИмяРегламентногоЗаданияПолучениеНаложенныхПлатежей(), Истина);
