...
// Сохраняет реквизиты организаций в формате CML в файлы
//
// Параметры:
//   Организации - Массив из СправочникСсылка.Организации - Список организаций, реквизиты которых нужно сохранить
// 
Процедура СохранитьРеквизитыОрганизацииВXML(Знач Организации) Экспорт
	
	МассивФайлов = ОрганизацииФормыВызовСервера.КарточкиОрганизацийВXML(Организации);
	ПараметрыСохранения = файловаяСистемаКлиент.ПараметрыСохраненияФайла();
	ПараметрыСохранения.Диалог.Заголовок = НСтр("ru = 'Сохранение реквизитов организации'");
	ПараметрыСохранения.Диалог.Фильтр = НСтр("ru = 'Реквизиты организации (*.xml)|*.xml'");
	СохранитьФайлы(МассивФайлов, ПараметрыСохранения);
...
Функция ФИОПриИзменении(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ФамилияИП) Или ЗначениеЗаполнено(Объект.ИмяИП) Или ЗначениеЗаполнено(Объект.ОтчествоИП) Тогда
		...
		ЗарегистрированВКачествеИП =
			Объект.СтатусФизическогоЛица = ПредопределенноеЗначение(
				"Перечисление.СтатусыФизическихЛиц.ИндивидуальныйПредприниматель");
		
		Объект.НаименованиеСокращенное = ОбщегоНазначенияБПКлиентСервер.СокращенноеНаименованиеФизическогоЛица(
			Объект.ФамилияИП, Объект.ИмяИП, Объект.ОтчествоИП, ЗарегистрированВКачествеИП);
		
		Объект.НаименованиеПолное = ОбщегоНазначенияБПКлиентСервер.ПолноеНаименованиеФизическогоЛица(
			Объект.ФамилияИП, Объект.ИмяИП, Объект.ОтчествоИП, ЗарегистрированВКачествеИП);
...
Процедура СохранитьФайлы(МассивФайлов, ПараметрыСохранения = Неопределено)
	
	Оповещение = Новый ОписаниеОповещения("СохранитьФайлыЗавершение", ЭтотОбъект);
	Если МассивФайлов.Количество() = 1 Тогда
		ФайловаяСистемаКлиент.СохранитьФайл(Оповещение, МассивФайлов[0].Хранение, МассивФайлов[0].Имя, ПараметрыСохранения);
	Иначе
...
Процедура СохранитьФайлыЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(ПолученныеФайлы) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого ОписаниеФайла Из ПолученныеФайлы Цикл
