...
Функция ПеречислениеПоИсполнительномуЛисту(Объект) Экспорт
	
	Возврат Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника;
	...
	Если Параметры.ЗначенияЗаполнения.Свойство("КодБК")
		...
		Если ЗначениеЗаполнено(ВидГосударственногоОргана)
			...
			ПолучательПлатежа = Документы.ПлатежноеПоручение.ПолучательДляПлатежаГосударственномуОргануПоПравиламУплаты(
				...
				Параметры.ЗначенияЗаполнения.КодНалоговогоОргана,
				Объект.Налог);
...
Функция ПлатежныеРеквизиты(Форма) Экспорт
	...
	ПлатежныеРеквизиты.Вставить("ПлательщикЕНП", Форма.ПлательщикЕНП);
...
Функция ИсточникДанныхКонтекстаПлатежногоДокумента(Форма) Экспорт
	...
	ИсточникДанныхКонтекста.Вставить("ПеречислениеВБюджет",         Объект.ПеречислениеВБюджет);
	...
	КонецЕсли;
	ИсточникДанныхКонтекста.Вставить("ПеречислениеПоИсполнительномуЛисту", Объект.ПеречислениеПоИсполнительномуЛисту);
...
Процедура УстановитьПараметрыВыбораБанковскихСчетов(Форма) Экспорт
	...
	Если Не ДокументМожетБытьОформленВЦР Тогда
		НовыйМассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ЦифровойСчет", Ложь));
	КонецЕсли;
...
Процедура ЗаполнитьРегистрациюВНалоговомОрганеИзПараметровФормы(Форма, Параметры) Экспорт
	
	// При создании платежки из списка задач бухгалтера заполняем регистрацию в налоговом органе
	Если Параметры.Свойство("РегистрацияВНалоговомОргане") Тогда
		Форма.РегистрацияВНалоговомОргане = Параметры.РегистрацияВНалоговомОргане;
	КонецЕсли;
...
Процедура ПроверитьИзменитьКПППлательщика(Форма) Экспорт
	...
	Объект = Форма.Объект;
	ПеречислениеПоИсполнительномуЛистуРаботника =
		Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеПоИсполнительномуЛистуРаботника;
	...
	Если НЕ ЗначениеЗаполнено(Объект.Налог) ИЛИ ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(Объект.КодБК) Тогда
		Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
			...
			Если СведенияОКонтрагенте.ГосударственныйОрган 
				...
				КонецЕсли;
			ИначеЕсли ПеречислениеПоИсполнительномуЛистуРаботника
				И СведенияОКонтрагенте.ГосударственныйОрган Тогда
...
Функция АвтоЗначенияРеквизитов(Форма) Экспорт
	...
	ПеречислениеПоИсполнительномуЛисту = ПеречислениеПоИсполнительномуЛисту(Объект);
	...
	Получатель = ?(ПереводМеждуСчетами, Объект.Организация,
		?(Форма.ПеречислениеФизическомуЛицу И Не Форма.ПеречислениеНаЛичныйСчет, Объект.Банк, Объект.Контрагент));
...
Функция РеквизитыГосударственногоОрганаПоКППОрганизации(Организация, КППОрганизации, Налог, Период) Экспорт
	Перем ГосударственныйОрган;
	
	Если ЗначениеЗаполнено(Налог) Тогда
		ВидИКодГосОргана = ДанныеГосударственныхОрганов.ВидИКодГосударственногоОрганаПоНалогу(
			Налог, Организация, Период);
		РеквизитыМИДол = ЕдиныйНалоговыйСчет.РеквизитыМИДол();
		Если ВидИКодГосОргана.Код = РеквизитыМИДол.Код Тогда
...
Процедура ОпределитьОрганизацияЮрФизЛицо(Форма) Экспорт
...
Функция СохранятьКПППлательщика(Форма) Экспорт
	
	СохранятьКПППлательщика = Истина;
	
	Объект = Форма.Объект;
	Если Объект.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога Тогда
		Если Объект.Налог = Справочники.ВидыНалоговИПлатежейВБюджет.ЕдиныйНалоговыйПлатеж
			Или Не ПлатежиВБюджетКлиентСервер.ПлатежАдминистрируетсяНалоговымиОрганами(Объект.КодБК) Тогда
			СохранятьКПППлательщика = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Возврат СохранятьКПППлательщика;
...
Процедура НастройкаДиректБанк(ЕстьНастройкаДиректБанк, Знач Организация, Знач БанковскийСчет, СписокВыбора) Экспорт
	...
	Если ИспользуетсяСогласованиеПлатежей Тогда
		СписокВыбора.Добавить(Перечисления.СостоянияБанковскихДокументов.Согласовано);
	КонецЕсли; 
