...
//  КодыНалоговыхОрганов - Массив - коды налоговых органов, в которых зарегистрированы объекты, попавшие в сверку
//  РегистрацииВНалоговомОргане - СправочникСсылка.РегистрацииВНалоговомОргане
//  Сообщение - ДокументСсылка.ДокументРеализацииПолномочийНалоговыхОрганов - входящее сообщение от ФНС с расчетом налога
Процедура ИнициализироватьПараметрыНавигационныхСсылок(Форма, Организация, Налог, НалоговыйПериод, КодыНалоговыхОрганов, РегистрацияВНалоговомОргане, Сообщение = Неопределено) Экспорт 
	...
	ОбщиеПараметры.КодНалога =
		Перечисления.ВидыИмущественныхНалогов.КодНалогаВЗадачахБухгалтера(Налог);
...
// Инициализирует структуру ПараметрыСверки со свойствами:
//   * КодОтправителя - Строка - код налогового органа - отправителя сообщения с расчетом налога, с которым нужно свериться.
//                      В общем случае может не совпадать с кодом текущей регистрации в налоговом органе.
//                      См. РегистрыСведений.СообщенияСРасчетомИмущественныхНалогов.КодОтправителя().
//   * КодыНалоговыхОрганов - Массив - коды налоговых органов, в которых зарегистрированы объекты, по которым 
//                      отправитель прислал расчет налога.
//
Процедура ИнициализироватьПараметрыСверки(
		Форма,
	...
	ПараметрыСверки.Цвета.Вставить("РасчетыНеСошлись", ЦветаСтиля.ЦветВыделенияКонтрагентаСОшибкой);
	ПараметрыСверки.Цвета.Вставить("РучнаяСверка", Новый Цвет);
	ПараметрыСверки.Цвета.Вставить("РасчетОтсутствует", ЦветаСтиля.ЦветВыделенияКонтрагентаСОшибкой);
...
// Возвращает 2 суммы налога: рассчитанную и фактически уплаченную за указанный налоговый период.
// 
// Параметры:
//  Организация - СправочникСсылка.Организации 
//  Налог - ПеречислениеСсылка.ВидыИмущественныхНалогов
//  НалоговыйПериод - Дата - любая дата за налоговый период
//  КодыНалоговыхОрганов - Массив - массив кодов налогового органа (строка 4 символа),
//                         по которым нужно выбрать рассчитанные суммы налога
// 
// Возвращаемое значение:
//  Структура -
// * СуммаНалога - Число - рассчитанная сумма налога за налоговый период
// * СуммаНалогаУплачено - Число - уплаченная сумма налога за налоговый период
Функция СуммыНалога(Организация, Налог, НалоговыйПериод, КодыНалоговыхОрганов) Экспорт
	
	СуммыНалога = Новый Структура;
	
	СуммыНалога.Вставить("СуммаНалога", 0);
	СуммыНалога.Вставить("СуммаНалогаУплачено", 0);
	...
	Если Налог = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(РасчетТранспортногоНалога.СуммаНалога) КАК СуммаНалога
		|ИЗ
	...
	ИначеЕсли Налог = Перечисления.ВидыИмущественныхНалогов.ЗемельныйНалог Тогда 
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(РасчетЗемельногоНалога.СуммаНалога) КАК СуммаНалога
		|ИЗ
	...
	СуммыНалога.СуммаНалога = Выборка.СуммаНалога;
	// Считаем, что уплачено ровно столько, сколько рассчитано в программе
	СуммыНалога.СуммаНалогаУплачено = Выборка.СуммаНалога;
...
// 
// Возвращаемое значение:
//  Массив - наименования полей расчета для конкретного налога за конкретный период
//
...
//  СверяемыеПоляСтрокРасчета - Массив- см. ПоляСтрокРасчета()
// 
// Возвращаемое значение:
...
Функция СравнитьРасчетыПоОбъекту(Расчет, РасчетФНС, СуммаЛьгот, СуммаЛьготФНС, СуммаНалога, СуммаНалогаФНС, СверяемыеПоляСтрокРасчета) Экспорт 
	...
	Для каждого СтрокаРасчета Из Расчет Цикл
		...
		Для каждого СтрокаРасчетаФНС Из РасчетФНС Цикл
			...
			НоваяСтрокаСранения = Сравнение.Добавить();
			НоваяСтрокаСранения.СтрокаРасчета = СтрокаРасчета;
			НоваяСтрокаСранения.ИндексСтрокиРасчета = Расчет.Индекс(СтрокаРасчета);
			НоваяСтрокаСранения.СтрокаРасчетаФНС = СтрокаРасчетаФНС;
			НоваяСтрокаСранения.ИндексСтрокиРасчетаФНС = РасчетФНС.Индекс(СтрокаРасчетаФНС);
			Для каждого ПолеРасчета Из СверяемыеПоляСтрокРасчета Цикл
				НоваяСтрокаСранения[ПолеРасчета] = 
					?(СтрокаРасчета[ПолеРасчета] = СтрокаРасчетаФНС[ПолеРасчета], 1, 0);
				НоваяСтрокаСранения.ВсегоСошлось = НоваяСтрокаСранения.ВсегоСошлось + НоваяСтрокаСранения[ПолеРасчета];
			КонецЦикла;
	...
	Иначе
		...
		// В расчете может быть техногенная ошибка, когда показатели сходятся, а сумма налога различается. Чтобы корректно
		// обработать такую ситуацию, запишем, что расхождение в производном поле суммы:
...
Функция НовыеПараметрыНавигационныхСсылок()
	...
	ОбщиеПараметры.Вставить("КодНалога", "");
