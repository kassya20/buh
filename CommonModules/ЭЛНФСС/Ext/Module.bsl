...
Функция ФизическоеЛицо(ДанныеЭЛН) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатПоиска.ТекстОшибки) Тогда
		СообщенияБЗК.СообщитьОПроблеме(РезультатПоиска.ТекстОшибки);
	КонецЕсли;
	Возврат ДанныеЭЛН.ФизическоеЛицо;
КонецФункции
...
Функция КадровыеДанныеСотрудникаБезКэша(ФизическоеЛицо, ДатаНачалаСобытия, ГоловнаяОрганизация, ПредполагаемаяОрганизация = Неопределено) Экспорт
	...
	Результат.Вставить("ВсеСотрудники", Новый ТаблицаЗначений);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СотрудникиОрганизации.ДатаУвольнения КАК ДатаУвольненияФакт,
	|	СотрудникиОрганизации.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	|	СотрудникиОрганизации.Организация КАК Организация,
	|	СотрудникиОрганизации.ВидЗанятости КАК ВидЗанятости,
	...
	Результат.ВсеСотрудники = Пакет[ВсегоПакетов - 2].Выгрузить();
	ВсеОрганизации = Пакет[ВсегоПакетов - 1].Выгрузить();
...
Функция РазрешенаЗагрузкаЭЛН(СтатусЭЛНВФСС, НомерЛН) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтатусЭЛНВФСС)
		Или СтатусЭЛНВФСС = Перечисления.СостоянияЭЛНВФСС.Аннулирован
		Или СтатусЭЛНВФСС = Перечисления.СостоянияЭЛНВФСС.ВозвращенМО Тогда
		Возврат Ложь;
	КонецЕсли;
