...
//
Функция ПредопределенныеРазрезыРаспределения() Экспорт
	...
	// Этап распределения позволяет расширить пространство вершин учета затрат в рамках одного расчета,
	// с тем, чтобы последовательно выполнить несколько правил распределения
	РазрезыРаспределения.Вставить("ЭтапРаспределения",    ОбщегоНазначения.ОписаниеТипаСтрока(255));
	
	РазрезыРаспределения.Вставить("ВидРасходовНУ",        Новый ОписаниеТипов("ПеречислениеСсылка.ВидыРасходовНУ"));
	
	// Назначение аналогично ЭтапРаспределения. Возможно, в будущем будет заменен на него
...
Функция РазрезыРаспределения(МенеджерВременныхТаблиц) Экспорт
	...
	РазрезыРаспределения = ОписаниеПредопределенныхРазрезовРаспределения();
	
	РасширеннаяАналитикаЗатрат.ДополнитьРазрезыРаспределения(РазрезыРаспределения, МенеджерВременныхТаблиц);
...
// Определяет имена разрезов распределения по ключам аналитики.
// Имена используются, в частности, в модуле ПравилаРаспределения;
// ключи - при необходимости сохранения в информационной базе (настроек, значений субконто и т.п.)
//
// Параметры:
//  Ключи - Массив Из УчетЗатрат.ТипЗначенияАналитикиЗатрат
//  РазрезыРаспределения - ТаблицаЗначений - см. НовыйРазрезыРаспределения, описание разрезов распределения,
//      полученное с помощью РазрезыРаспределения
// 
// Возвращаемое значение:
//  Массив из Строка - имена разрезов распределения.
//
Функция ИменаРазрезовРаспределения(Ключи, РазрезыРаспределения) Экспорт
	
	Имена = Новый Массив;
	
	Для Каждого Ключ Из Ключи Цикл
		
		ОписаниеРазрезаРаспределения = РазрезыРаспределения.Найти(Ключ, "Ключ");
		
		Если ОписаниеРазрезаРаспределения = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Имена.Добавить(ОписаниеРазрезаРаспределения.Имя);
...
// Определяет разрез (ключ) аналитики по его имени.
// Имена используются, в частности, в модуле ПравилаРаспределения;
// ключи - при необходимости сохранения в информационной базе (настроек, значений субконто и т.п.)
//
// Параметры:
//  Имя - Строка - имя разреза распределения
//  РазрезыРаспределения - ТаблицаЗначений - см. НовыйРазрезыРаспределения, описание разрезов распределения,
//      полученное с помощью РазрезыРаспределения
// 
// Возвращаемое значение:
//  УчетЗатрат.ТипЗначенияАналитикиЗатрат - ключ аналитики
//  Неопределено - неизвестное имя
//
Функция РазрезРаспределения(Имя, РазрезыРаспределения) Экспорт
	
	ОписаниеРазрезаРаспределения = РазрезыРаспределения.Найти(Имя, "Имя");
	Если ОписаниеРазрезаРаспределения = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ОписаниеРазрезаРаспределения.Ключ;
	
КонецФункции

// Возвращает разрезы, в которых распределяются расходы.
// При использовании Расширенной функциональности перечень включает субконто, добавленные пользователем на счета затрат.
//
// Параметры:
//  СчетаУчета - ПланСчетовСсылка.Хозрасчетный, Массив - Ссылка на счет учета или массив счетов
// 
// Возвращаемое значение:
//  ТаблицаЗначений - см. НовыйРазрезыРаспределения()
//
Функция РазрезыРаспределенияСчетовУчета(СчетаУчета) Экспорт
	...
	|ГДЕ
	|	Хозрасчетный.Ссылка В (&СчетУчета)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет";
	...
	РасширеннаяАналитикаЗатрат.ДополнитьРазрезыРаспределения(РазрезыРаспределения, МенеджерВременныхТаблиц, Ложь);
...
Функция ПредопределенныеРазрезыРаспределенияОпределяемыеПоСубконто()
	...
	РазрезыРаспределения.Вставить("ВидРемонтаОС",         ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРемонтовОС);
	РазрезыРаспределения.Вставить("ОсновноеСредство",     ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства);
	РазрезыРаспределения.Вставить("НИОКР",                ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.РасходыНаНИОКР);
...
Функция НовыйРазрезыРаспределения()
	...
	РазрезыРаспределения.Колонки.Добавить("Ключ",        Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные, Строка, Тип"));
	...
	РазрезыРаспределения.Индексы.Добавить("Имя");
	РазрезыРаспределения.Индексы.Добавить("Ключ");
...
Функция СоздатьБазыРаспределения(МенеджерВременныхТаблиц, ПравилаРаспределения, Период, Организация)
	...
	Для Каждого Правило Из ПравилаРаспределения Цикл
		...
		Для Каждого ПакетВременныхТаблиц Из ПравилаРаспределенияРасходов.ПакетыВременныхТаблицПравила(Правило.БазаРаспределения) Цикл
		
			Если ВыполненныеПакетыВременныхТаблиц.Свойство(ПакетВременныхТаблиц.Имя) Тогда
				Продолжить;
			КонецЕсли;
			
			ВыполненныеПакетыВременныхТаблиц.Вставить(ПакетВременныхТаблиц.Имя);
			
			// У каждого пакета могут быть свои параметры, поэтому для каждого пакета - свой запрос
			Для Каждого Параметр Из ПакетВременныхТаблиц.ПараметрыЗапроса Цикл
				Запрос.УстановитьПараметр(Параметр.Ключ, Параметр.Значение);
			КонецЦикла;
				
			Запрос.Текст = ПакетВременныхТаблиц.ТекстЗапроса;
			Запрос.Выполнить();
	...
	Для Каждого Правило Из ПравилаРаспределения Цикл
		...
		// Если для распределения затрат между наименованиями продукции используются данные о прямых затратах,
		// то для материальных и иных затрат используются разные формулы распределения. Сохраненные данные будут
		// использованы в справке-расчете "Себестоимость выпущенной продукции".
		СохранитьРасчетБазы(СозданныеВременныеТаблицы, Правило);
...
Процедура СохранитьРасчетБазы(СозданныеВременныеТаблицы, Правило)

	Если Не Правило.БазаРаспределения.Свойство("РасшифровкаРасчета") 
		Или Не ЗначениеЗаполнено(Правило.БазаРаспределения.РасшифровкаРасчета) Тогда
		Возврат;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(
		СозданныеВременныеТаблицы,
...
Функция ОтобратьРасходы(МенеджерВременныхТаблиц, ПравилоРаспределения, ИндексыТаблицыБазыРаспределения, РазрезыРаспределения)
	...
	ЗаполнитьПараметрыОтбораРасходов(Запрос, ПравилоРаспределения.ОтборРасходов);
...
Функция СоздатьРезультатРаспределения(МенеджерВременныхТаблиц, ПравилоРаспределения, РазрезыРаспределения)
	...
	Если ПравилоРаспределения.Действие = "РаспределитьНаСубконто" Тогда
		...
		// Первоначально все поля берем из базы, но можем уточнить в ДополнитьВыраженияПолейВнешнейАналитики
		...
		ПараметрыТекста.Вставить("СоединениеСчетаЗатрат", "");
		ПараметрыТекста.Вставить("Подразделение", "БазаРаспределения.Подразделение");
		ПараметрыТекста.Вставить("Субконто1",     "БазаРаспределения.Субконто1");
		ПараметрыТекста.Вставить("Субконто2",     "БазаРаспределения.Субконто2");
		ПараметрыТекста.Вставить("Субконто3",     "БазаРаспределения.Субконто3");
		ПараметрыТекста.Вставить("Номенклатура",  "БазаРаспределения.Номенклатура");
		
		ДополнитьВыраженияПолейВнешнейАналитики(ПараметрыТекста, ПараметрыЗапроса, ПравилоРаспределения);
...
Процедура ДополнитьОтборРасходовПравиломРаспределения(ОператорЗапроса, ПравилоРаспределения)
	...
	Для Каждого ПолеОтбора Из ПравилоРаспределения.ОтборРасходов Цикл
		...
		ИначеЕсли ТипЗнч(ПолеОтбора.Значение) = Тип("ОписаниеТипов") Тогда
			Шаблон = "Расходы.%1 <> &%1";
		КонецЕсли;
...
Процедура ЗаполнитьПараметрыОтбораРасходов(Запрос, ОтборРасходов)
	
	Для Каждого ПолеОтбора Из ОтборРасходов Цикл
		
		ЗначениеОтбора = ПолеОтбора.Значение;
		
		Если ТипЗнч(ЗначениеОтбора) = Тип("ОписаниеТипов") Тогда
			ЗначениеОтбора = ЗначениеОтбора.ПривестиЗначение(Неопределено);
		КонецЕсли;
		
		Запрос.УстановитьПараметр(ПолеОтбора.Ключ, ЗначениеОтбора);
...
Процедура ДополнитьВыраженияПолейВнешнейАналитики(ПараметрыТекста, ПараметрыЗапроса, Правило)
	
	Если Не ЗначениеЗаполнено(Правило.ПоляПриемника) Тогда
		Возврат;
	КонецЕсли;
	
	Соединения = Новый Массив;
	Соединения.Добавить("ЛЕВОЕ СОЕДИНЕНИЕ СчетаЗатрат ПО БазаРаспределения.Счет = СчетаЗатрат.Счет");
	
	ШаблонСоединениеВидыСубконто =
		"ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Хозрасчетный.ВидыСубконто КАК ОписаниеСубконто%1
			|	ПО БазаРаспределения.Счет = ОписаниеСубконто%1.Ссылка И ОписаниеСубконто%1.НомерСтроки = %1";
	Для НомерСубконто = 1 По БухгалтерскийУчет.МаксимальноеКоличествоСубконто() Цикл
		Соединения.Добавить(СтрШаблон(ШаблонСоединениеВидыСубконто, XMLСтрока(НомерСубконто)));
	КонецЦикла;
	
	ПараметрыТекста.СоединениеСчетаЗатрат = СтрСоединить(Соединения, Символы.ПС);
	
	ПараметрыВидСубконто = Новый Структура; // Ключ - имя поля приемника, Значение - имя параметра запроса
	
	Для Каждого ОписаниеПоля Из Правило.ПоляПриемника Цикл
		
		Если ТипЗнч(ОписаниеПоля.Значение) = Тип("Строка") Тогда
			
			ПараметрыТекста[ОписаниеПоля.Значение] = СтрШаблон(
				"ВЫБОР
				|	КОГДА НЕ СчетаЗатрат.Счет ЕСТЬ NULL
				|ТОГДА
		...
		Иначе // Вид субконто
			...
			ИмяВидаСубконто = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОписаниеПоля.Значение, "ИмяПредопределенныхДанных");
			Если Не ЗначениеЗаполнено(ИмяВидаСубконто) Тогда
				ИмяВидаСубконто = ИмяСубконтоПользователя(ОписаниеПоля.Значение);
			КонецЕсли;
			ИмяПараметраВидСубконто = СтрШаблон("ВидСубконто%1", ИмяВидаСубконто);
			
			ПараметрыЗапроса.Вставить(ИмяПараметраВидСубконто, ОписаниеПоля.Значение);
			
			ПараметрыВидСубконто.Вставить(ОписаниеПоля.Ключ, ИмяПараметраВидСубконто);
			
			Если Правило.Выпуск И ОписаниеПоля.Значение = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура Тогда
				...
				ПараметрыТекста.Номенклатура = СхемыЗапросов.ВыражениеПриведениеТипа(ТекстВыражения, Тип("СправочникСсылка.Номенклатура"));
	...
	Если Не ЗначениеЗаполнено(ПараметрыВидСубконто) Тогда
		Возврат;
	КонецЕсли;
	
	// %1 - ид (номер) субконто, %2 - имя параметра, %3 - имя поля расходов
	ЭлементыШаблонаСубконто = Новый Массив;
	ЭлементыШаблонаСубконто.Добавить("ВЫБОР КОГДА НЕ СчетаЗатрат.Счет ЕСТЬ NULL ТОГДА БазаРаспределения.Субконто%1");
	ШаблонВидыСубконто = "КОГДА ОписаниеСубконто%1.ВидСубконто = &%2 ТОГДА Расходы.%3";
	Для Каждого ОписаниеПараметраВидСубконто Из ПараметрыВидСубконто Цикл
		ЭлементыШаблонаСубконто.Добавить(СтрШаблон(
			ШаблонВидыСубконто,
			"%1", // чтобы избежать %%1 и %1 в одной строке
			ОписаниеПараметраВидСубконто.Значение,
			ОписаниеПараметраВидСубконто.Ключ));
	КонецЦикла;
	ЭлементыШаблонаСубконто.Добавить("ИНАЧЕ БазаРаспределения.Субконто%1 КОНЕЦ");
	
	ШаблонСубконто = СтрСоединить(ЭлементыШаблонаСубконто, Символы.ПС);
	
	Для НомерСубконто = 1 По БухгалтерскийУчет.МаксимальноеКоличествоСубконто() Цикл
		
		ИдентификаторСубконто = XMLСтрока(НомерСубконто);
		
		ИмяПоля = СтрШаблон("Субконто%1", ИдентификаторСубконто);
		
		ПараметрыТекста[ИмяПоля] = СтрШаблон(ШаблонСубконто, ИдентификаторСубконто);
...
Функция ОписаниеПредопределенныхРазрезовРаспределения()
	...
	Для Каждого ОписаниеПредопределенногоРазреза Из ПредопределенныеРазрезыРаспределения() Цикл
		...
		ИначеЕсли ОписаниеРазреза.Имя = "Счет" Тогда
			ОписаниеРазреза.Ключ = УчетЗатрат.РазрезАналитикиСчет();
		Иначе
