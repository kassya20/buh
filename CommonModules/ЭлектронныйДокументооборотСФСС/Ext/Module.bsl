...
Функция ДобавитьПанельПереходаНаОбменЧерезОператора(Форма,
	...
	Если Облачная1СО.ЭтоБизКуб()
		ИЛИ Организации.Количество() = 0 Тогда
		Возврат Панель;
	КонецЕсли;
	...
	Если НЕ РезультатАнализа.ЕстьОрганизацииОбменНапрямую
		...
		// Скрываем баннер, если он отображается.
		ИмяЭлементаБаннера = ЭлектронныйДокументооборотСФССКлиентСервер.ИмяГруппыПанелиПереходНаОператораСЭДО();
		Если НЕ Форма.Элементы.Найти(ИмяЭлементаБаннера) = Неопределено
			И Форма.Элементы[ИмяЭлементаБаннера].Видимость Тогда
			Форма.Элементы[ИмяЭлементаБаннера].Видимость = Ложь;
		КонецЕсли;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ФоноваяОтправкаЗапросовСЭДО", Истина);
	Методы.Вставить("ВыполнитьОбменСФСС", Истина);
КонецПроцедуры
...
Функция СоздатьПанельРекомендацийПоПереходуНаОператораСЭДО(Форма,
	...
	Если НЕ ПустаяСтрока(ДействияПанели) И ВыводитьДействия Тогда
		...
		НадписьДействия.Шрифт = Новый Шрифт(НадписьТекст.Шрифт, , , , , , , 100 + ДельтаДляШрифтаДействий);
