...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.Справочники.НастройкиЗагрузкиДанныхВнешнихСистем);
	Типы.Добавить(Метаданные.Справочники.СеансыЗагрузкиДанныхВнешнихСистем);
	
	Типы.Добавить(Метаданные.РегистрыСведений.ОбъектыВнешнихСистем);
	Типы.Добавить(Метаданные.РегистрыСведений.ПропущенныеПакетыДанныхВнешнихСистем);
...
Функция ИнформацияОВнешнихСистемах()
	...
	РезультатОтправки = ВыполнитьЗапросСервиса(URLОперации, ПараметрыЗапросаJSON);
...
Функция ЗагрузитьФайлОбмена(ИдентификаторНастройки, ОписаниеФайлаОбмена)
	...
	// За формат содержимого отвечает вызывающий код, поэтому полученное тело ответа передается "как есть"
	СодержимоеДвоичныеДанные = 2;
	РезультатОтправки = ВыполнитьЗапросСервиса(URLОперации, ПараметрыЗапросаJSON, СодержимоеДвоичныеДанные);
...
// Взывает к сервису передачи данных
//
// Параметры:
//  URL - Строка - URL вызываемой операции (в терминах БИП)
//  ТелоЗапроса - Строка - сериализованное тело запроса к сервису
//  ФорматОтвета - Число - определяет формат возврата содержимого ответа операции (в термиинах БИП):
//    - 0 - имя файла ответа;
//    - 1 - как строка;
//    - 2 - как двоичные данные;
// 
// Возвращаемое значение:
//  Структура, см. ИнтернетПоддержкаПользователей.ЗагрузитьСодержимоеИзИнтернет - результат запроса
//
Функция ВыполнитьЗапросСервиса(URL, ТелоЗапроса, ФорматОтвета = 1)
	...
	// Для логирования на сервисе; позволяет заподозрить ошибочные действия администратора,
	// в результате которых один идентификатор участника обмена одновременно
	// используется двумя копиями информационной базы.
	Заголовки.Вставить("X-Tenant-Instance-Id", ИдентификаторЭкземпляраИнформационнойБазы());
	
	ПараметрыОтправки = Новый Структура;
	ПараметрыОтправки.Вставить("Метод"                   , "POST");
	ПараметрыОтправки.Вставить("ФорматОтвета"            , ФорматОтвета);
	ПараметрыОтправки.Вставить("Заголовки"               , Заголовки);
	ПараметрыОтправки.Вставить("ДанныеДляОбработки"      , ТелоЗапроса);
...
Функция ИдентификаторЭкземпляраИнформационнойБазы()
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	
	// Собираем информацию с учетом того, что база может быть, в частности,
	// на виртуальной машине (строка соединения может быть одинаковой у разных копий) 
	// или во фреше (данные о процессоре нерелевантны).
	// Для этого соединяем хэши, а не рассчитываем хэш от всех полей.
	ЗначенияИдентификатора = Новый Массив;
	ЗначенияИдентификатора.Добавить(ВРег(СтрокаСоединенияИнформационнойБазы()));
	ЗначенияИдентификатора.Добавить(СистемнаяИнформация.Процессор);
	ЗначенияИдентификатора.Добавить(XMLСтрока(СистемнаяИнформация.ИдентификаторКлиента));
	
	ЭлементыИдентификатора = Новый Массив;
	Для Каждого Значение Из ЗначенияИдентификатора Цикл
		ЭлементыИдентификатора.Добавить(ЭлементИдентификатораЭкземпляраИнформационнойБазы(Значение));
	КонецЦикла;
	
	Возврат СтрСоединить(ЭлементыИдентификатора, ".");
	
КонецФункции

Функция ЭлементИдентификатораЭкземпляраИнформационнойБазы(Значение)
	
	Хеш = Новый ХешированиеДанных(ХешФункция.CRC32);
	Хеш.Добавить(Значение);
	Возврат XMLСтрока(Хеш.ХешСумма);
