...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьНеобходимостьНастройкиАвтоматическогоПриемаПриглашенийЭДО");
	Методы.Вставить("ВключитьЭДОВБазовойВерсииОбновлениеИБ");
...
Процедура НайтиСоздатьСчетНаОплату101(ДеревоДанных, ДокументУчета, СпособОбработки, ОписаниеОшибки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДокументУчета) Тогда
		...
		Если НДСИсчисляетсяНалоговымАгентом Тогда
			...
			СтруктураПараметров.Вставить("ВидАгентскогоДоговора", ОтборПоВидуАгентскогоДоговораВСписке());
		Иначе
...
Процедура ЗаполнитьДанныеАктОРасхождениях_ИнформацияПокупателя(СсылкаНаОбъект, Настройки, ДеревоДанных, Описание, Отказ) Экспорт
	...
	// Номер документа.
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
		ДеревоДанных, "НомерДокумента", ВыборкаШапка.НомерВходящегоДокумента);

	// Дата документа.
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
		ДеревоДанных, "ДатаДокумента", ВыборкаШапка.ДатаВходящегоДокумента);
	...
	Если ЗначениеЗаполнено(ВыборкаШапка.Грузополучатель) Тогда
		...
		ЗаполнитьДанныеУчастникаУПД(ДеревоДанных, СведенияОГрузополучателе, "Грузополучатель", ВидыАдресов.Почтовый, ВыборкаШапка.ДатаДокумента);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ВыборкаШапка.Грузоотправитель) Тогда
		...
		ЗаполнитьДанныеУчастникаУПД(ДеревоДанных, СведенияОГрузоотправителе, "Грузоотправитель", ВидыАдресов.Почтовый, ВыборкаШапка.ДатаДокумента);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеУчастникаУПД(ДеревоДанных, СведенияОбУчастнике, ВидУчастника, ВидАдреса, ДатаСведений = '00010101')
	...
	Иначе
		...
		Если СведенияОбУчастнике.ЭтоИндивидуальныйПредприниматель Тогда
			ПутьВДереве = ВидУчастника + ".ТипУчастника.ИП";
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, ПутьВДереве + ".СвидетельствоОГосРегистрации", 
				БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(СведенияОбУчастнике));
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, ПутьВДереве + ".ОГРН", 
				СведенияОбУчастнике.ОГРН);
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, ПутьВДереве + ".ДатаОГРН", 
				СведенияОбУчастнике.ДатаПрисвоенияОГРН);
		КонецЕсли;
...
Процедура ЗаполнитьДанныеУчастниковУПД(ДеревоДанных, ПараметрыУчастников, Отказ = Ложь)
	...
	Для Каждого СведенияОбУчастнике Из ПараметрыУчастников.СведенияОбУчастниках Цикл
		...
		ДанныеУчастника = ДанныеУчастникаЭД(ПараметрыУчастника, Отказ);
...
Функция НовыйПараметрыУчастников()
	...
	ПараметрыУчастников.Вставить("ДокументУчета", Неопределено);
...
Процедура ЗаполнитьСведенияОПоставщикахУПД(ДеревоДанных, ДанныеДляФормированияЭД, Отказ = Ложь)
	...
	ПараметрыУчастников.ДокументУчета = ДанныеШапки.СсылкаНаПервичныйДокумент;
	
	ЗаполнитьДанныеУчастниковУПД(ДеревоДанных, ПараметрыУчастников, Отказ);
...
Процедура ЗаполнитьОсновнуюЧастьУПД_5_02(ДанныеДляФормированияЭД, ДеревоДанных, Отказ)
	...
	ЗаполнитьСведенияОПоставщикахУПД(ДеревоДанных, ДанныеДляФормированияЭД, Отказ);
...
Процедура ЗаполнитьРасширеннуюЧастьУПД_5_02(ДанныеДляФормированияЭД, ДеревоДанных, Отказ)
	...
	Для Каждого ДокументОснование Из ДанныеДокументаУчета.ДокументыОснования Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "ДокументыОснованияДокументаОтгрузки", ДокументыОснования);
...
Процедура ЗаполнитьУКД_2020(СсылкаНаОбъект, Настройки, ДеревоДанных, Описание, Отказ)
	...
	ЗаполнитьОсновнуюЧастьУКД_2020(ДанныеДляФормированияЭД, Настройки, ДеревоДанных, Отказ);
...
Процедура ЗаполнитьКСЧ_2020(СсылкаНаОбъект, Настройки, ДеревоДанных, Отказ)
	...
	Если ДанныеДокументаИСчетаФактуры.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.КорректировочныйНаАванс Тогда
		ТекстСообщения = НСтр("ru = 'Формат корректировочного счета-фактуры на аванс не поддержан ФНС. Используйте бумажный формат документа.'");
		СообщитьОбОшибкеПриФормированииЭД(СсылкаНаОбъект, ТекстСообщения, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОсновнуюЧастьУКД_2020(ДанныеДляФормированияЭД, Настройки, ДеревоДанных, Отказ = Ложь)
...
Функция ПодготовитьСтруктуруДляКорректировкиПоступленияУКД_2020(ДеревоДанных, ОписаниеОшибки = "")
	...
	Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
		...
		Номенклатура = НоменклатураИБ(СведенияОТоваре, "СведенияОТоварах.НомерСтроки.Сопоставление");
		ЭтоСтрокаТовары = Ложь; //для упаковок они есть только в товарах
		...
		НоваяСтрока.Сумма = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре, "СведенияОТоварах.НомерСтроки.СтоимостьТоваровБезНалога");
		//Заполняем колонку Упаковка для табличной части товары
		Если ЭтоСтрокаТовары И ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			КодЕдиницы = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(СведенияОТоваре, "СведенияОТоварах.НомерСтроки.ЕдиницаИзмеренияКод");
			ЕдиницаПоУмолчанию = УпаковкиВызовСервера.ЕдиницаПоУмолчанию(Номенклатура);
			Если ЕдиницаПоУмолчанию.Код <> КодЕдиницы Тогда
				НоваяСтрока.Упаковка = РаботаСУпаковкамиНоменклатуры.НайтиУпаковкуПоЕдиницеИзмерения(Номенклатура, КодЕдиницы);
			Иначе
				НоваяСтрока.Упаковка = ЕдиницаПоУмолчанию;
			КонецЕсли;
			Коэффициент = УпаковкиВызовСервера.КоэффициентУпаковки(НоваяСтрока.Упаковка);
			НоваяСтрока.КоличествоДоКорректировки = Окр(НоваяСтрока.КоличествоДоКорректировки * Коэффициент, 3);
			НоваяСтрока.Количество = Окр(НоваяСтрока.Количество * Коэффициент, 3);
			НоваяСтрока.ЦенаДоКорректировки = Окр(НоваяСтрока.ЦенаДоКорректировки / Коэффициент, 2); 
			НоваяСтрока.Цена = Окр(НоваяСтрока.Цена / Коэффициент, 2);
		КонецЕсли;
		//Конец
...
Процедура ЗаполнитьДокументПоступленияТоваровУслуг(Документ, ДанныеДокумента, СпособОбработки = Неопределено, ОписаниеОшибки = "")
	...
	Для Каждого СтрокаНакладной Из ДанныеТабличныхЧастей.Товары Цикл
		...
		//Если используются Упаковки и в ЭД единица номенклатуры не совпадает с нашей
		Если ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") И ЗначениеЗаполнено(СтрокаНакладной.Номенклатура) Тогда
			Если СтрокаНакладной.ЕдиницаИзмерения <> УпаковкиВызовСервера.ЕдиницаПоУмолчанию(СтрокаНакладной.Номенклатура) Тогда
				Упаковка = РаботаСУпаковкамиНоменклатуры.НайтиУпаковкуПоЕдиницеИзмерения(СтрокаНакладной.Номенклатура, СтрокаНакладной.ЕдиницаИзмерения);
				СтрокаТЧ.Упаковка = Упаковка;
				Если ТипЗнч(Упаковка) = Тип("СправочникСсылка.УпаковкиНоменклатуры") Тогда
					КоэффициентУпаковки = ?(Упаковка.Коэффициент = 0, 1, Упаковка.Коэффициент);
					СтрокаТЧ.Количество = Окр(СтрокаТЧ.Количество * КоэффициентУпаковки, 3);
					СтрокаТЧ.Цена 		= Окр(СтрокаТЧ.Цена / КоэффициентУпаковки, 2);
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;
...
Функция НовыйПараметрыУчастника()
	...
	ПараметрыУчастника.Вставить("ДокументУчета", Неопределено);
	
	Возврат ПараметрыУчастника;
	
КонецФункции

Функция ДанныеУчастникаЭД(ПараметрыУчастника, Отказ = Ложь)
	...
	Иначе
		...
		Если ТипУчастника = "ИП" Тогда
			...
			ДанныеТипУчастника.Вставить("ДатаОГРН", СведенияОбУчастнике.ДатаПрисвоенияОГРН);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОсновнуюЧастьУПД(ДанныеДляФормированияЭД, СтруктураЭД, ДеревоДанных)
	...
	НомерСтроки = 1;
	ТолькоУслуги = Истина;
	УчетАгентскогоНДСПокупателем = Ложь; // используется для реализации металлолома, сырых шкур животных и медных полуфабрикатов.
...
Функция ОтборПоВидуАгентскогоДоговораВСписке()
	
	ОтборПоВидуАгентскогоДоговора = Новый Структура();
	ОтборПоВидуАгентскогоДоговора.Вставить("ВидСравненияОтбора", ВидСравнения.ВСписке);
	ОтборПоВидуАгентскогоДоговора.Вставить("ЗначениеОтбора",
		Перечисления.ВидыАгентскихДоговоров.МассивВидовДоговоровРеализацииТоваров());
	
	Возврат ОтборПоВидуАгентскогоДоговора;
