...
// Возвращает описание таблицы налоги по переданным параметрам.
//
// Параметры:
//  Организация - СправочникСсылка.Организации - организация.
//   Правило - СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов - правило уплаты налогов.
//   ПериодСобытия - Дата - период, за который начисляются взносы.
//   Срок - Дата - срок уплаты взносов.
//   СтраховыеВзносыКУплате - Структура - см. Обработка.РасчетСтраховыхВзносовИП.НовыеСтраховыеВзносыКУплате().
//   ВидНалога - ПеречислениеСсылка.ВидыНалогов - ссылка на вид налога, по которому формируется таблица налогов.
// 
// Возвращаемое значение:
//   ТаблицаЗначений - возвращаемые данные.
//
Функция ОписаниеТаблицыНалоги(Организация, Правило, ПериодСобытия, Срок, СтраховыеВзносыКУплате, ВидНалога = Неопределено) Экспорт
	
	ПараметрыТаблицы = Новый Структура;
	ПараметрыТаблицы.Вставить("Организация",             Организация);
	ПараметрыТаблицы.Вставить("Правило",                 Правило);
	ПараметрыТаблицы.Вставить("ПериодСобытия",           ПериодСобытия);
	ПараметрыТаблицы.Вставить("Срок",                    Срок);
	ПараметрыТаблицы.Вставить("СуммаВзносаПФР",          0);
	ПараметрыТаблицы.Вставить("СуммаВзносаПФРсДоходов",  0);
	ПараметрыТаблицы.Вставить("СуммаВзносаФФОМС",        0);
	ПараметрыТаблицы.Вставить("СуммаВзносаЕдиныйТариф",  0);

	Если ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_СтраховаяЧасть Тогда
		ПараметрыТаблицы.СуммаВзносаПФР = СтраховыеВзносыКУплате.СуммаВзносаПФР;
		ПараметрыТаблицы.СуммаВзносаПФРсДоходов = СтраховыеВзносыКУплате.СуммаВзносаПФРсДоходов;
	...
	ИначеЕсли ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_ФФОМС Тогда
		ПараметрыТаблицы.СуммаВзносаФФОМС = СтраховыеВзносыКУплате.СуммаВзносаФФОМС;
	ИначеЕсли ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф Тогда
		ПараметрыТаблицы.СуммаВзносаЕдиныйТариф =  СтраховыеВзносыКУплате.СуммаВзносаЕдиныйТариф;
	Иначе
		ПараметрыТаблицы.СуммаВзносаПФР = СтраховыеВзносыКУплате.СуммаВзносаПФР;
		ПараметрыТаблицы.СуммаВзносаПФРсДоходов = СтраховыеВзносыКУплате.СуммаВзносаПФРсДоходов;
		ПараметрыТаблицы.СуммаВзносаФФОМС = СтраховыеВзносыКУплате.СуммаВзносаФФОМС;
		ПараметрыТаблицы.СуммаВзносаЕдиныйТариф = СтраховыеВзносыКУплате.СуммаВзносаЕдиныйТариф;
	КонецЕсли;
	ПараметрыТаблицы.Вставить("УникальныйИдентификатор", Новый УникальныйИдентификатор);
	
	Возврат Обработки.РасчетСтраховыхВзносовИП.ОписаниеТаблицыНалоги(ПараметрыТаблицы);
...
Функция РеквизитыПлатежаДляПубликации(Организация, НалоговыйПериод, ВидНалога, ВидНалоговогоОбязательства, ОплатаЗадолженности)
	...
	СрокУплаты = КонецГода(НалоговыйПериод);
	Если ВидНалоговогоОбязательства = Перечисления.ВидыПлатежейВГосБюджет.ВзносыСвышеПредела
	...
	ДанныеАктуализированы = ИнтеграцияСБанками.ОбеспечитьАктуальностьПубликуемыхДанных(Организация, СрокУплаты, СообщениеОбОшибке);
	...
	Если Не ДанныеАктуализированы И Не ПустаяСтрока(СообщениеОбОшибке) Тогда
		ДобавитьОшибку(Ошибки, СообщениеОбОшибке);
	КонецЕсли;
	...
	Если ВидНалога = Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж Тогда
		// Необходимо получить сумму к доплате из Монитора с учетом всех налогов
		СуммаЕНП = Обработки.МониторНалоговИОтчетности.СуммаЕдиногоНалоговоПлатежа(Организация);
		Результат.Вставить("СуммаПлатежа", СуммаЕНП);
	Иначе
...
Функция НовоеПлатежноеПоручениеВБюджет(Организация, НалоговыйПериод, ВидНалога, ВидНалоговогоОбязательства, ОплатаЗадолженности)
	...
	Если ПлатежиВБюджетКлиентСервер.ДействуетПереходНаЕНП(Дата) Тогда
			...
			ОписаниеПлатежа.Сумма        = ДанныеЗаполнения.СуммаДокумента;
			...
			Если ЗначениеЗаполнено(Организация.РегистрацияВНалоговомОргане) Тогда
				РеквизитыРегистрации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация.РегистрацияВНалоговомОргане,
					"Код, КПП, КодПоОКТМО");
				ОписаниеПлатежа.КПППлательщика      = РеквизитыРегистрации.КПП;
				ОписаниеПлатежа.ОКАТО               = СокрЛП(РеквизитыРегистрации.КодПоОКТМО);
			...
			ДокументПлатежноеПоручение.Заполнить(ЗначенияЗаполнения);
		Иначе
...
Функция СуммаПлатежаПоНалогу(Организация, НалоговыйПериод, ВидНалога, ВидНалоговогоОбязательства, ОплатаЗадолженности)
	...
	Если ВидНалога = Перечисления.ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф
		И УчетСтраховыхВзносовИПКлиентСервер.ЕдиныйТарифВключенВЕдиныйНалоговыйПлатеж(НалоговыйПериод) Тогда
		ИмяФормыПомощника = "Обработка.РасчетСтраховыхВзносовИП.Форма";
		ПомощникВыполненияЗадачи = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяФормыПомощника);
	ИначеЕсли ОписаниеДействия <> Неопределено И ОписаниеДействия.Свойство("ИмяФормы") Тогда
