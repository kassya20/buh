...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаменитьСтарыеПоследовательностиНаДокументыОрганизаций");
	Методы.Вставить("ОчиститьОбщуюПоследовательность20Отложенно");
	Методы.Вставить("ОчиститьПоследовательностьВзаиморасчеты20Отложенно");
	Методы.Вставить("СброситьГраницыПоследовательностьОбщаяПоследовательность20Отложенно");
	Методы.Вставить("СброситьГраницыПоследовательностьВзаиморасчеты20Отложенно");
...
Процедура ЗарегистрироватьВПоследовательности(Источник, Отказ, ТолькоКИсключениюИзПоследовательности) Экспорт
	...
	Если НаборЗаписей.Модифицированность() Тогда
		НаборЗаписей.ДополнительныеСвойства.Вставить("ЗарегистрироватьВПоследовательности", Истина);
...
Функция ЗарегистрироватьОтложенныеРасчетыВПоследовательности(
	...
	Если Не НужноЗарегистрироватьВПоследовательности И Товары.Количество() > 0 Тогда
		...
		СчетаРассчитываемыеОперативно = УчетТоваров.СчетаРассчитываемыеОперативно(Объект.Организация, Объект.Дата);
...
Функция ДатаРегистрацииДокументаВПоследовательности(ДокументСсылка, Дата, Организация) Экспорт
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ОперацияБух") И КонецДня(Дата) = КонецГода(Дата)
		И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "МежотчетныйПериод") Тогда
		// Операция с признаком "Межотчетный период" не должна влиять на последовательность текущего месяца.
		Результат = КонецМесяца(Дата) + 1;
...
// Проверяет наличие документа в составе последовательности ДокументыОрганизаций, а также исключение из регистрации в ней.
//
// Параметры:
//  ДокументОбъектМетаданных - ОбъектМетаданных - документ как объект метаданных, который требуется проверить.
// 
// Возвращаемое значение:
//  Булево - регистрация выполняется.
//
Функция ДокументВходитВПоследовательностьИРегистрируется(ДокументОбъектМетаданных) Экспорт
	
	РегистрируетсяВПоследовательности = Ложь;
	
	ЕстьВПоследовательности = Метаданные.Последовательности.ДокументыОрганизаций.Документы.Содержит(ДокументОбъектМетаданных);
	Если ЕстьВПоследовательности Тогда
		...
		ТипРегистратора = Тип("ДокументСсылка." + ДокументОбъектМетаданных.Имя);
		РегистрируетсяВПоследовательности = РаботаСПоследовательностями.ДокументРегистрируетсяВПоследовательности(ТипРегистратора);
	КонецЕсли;
	
	Результат = ЕстьВПоследовательности И РегистрируетсяВПоследовательности;
...
// Актуализирует регламентные операции для расчета налогов.
//
// Параметры:
//  Параметры - ПоследовательностьЗапись.ДокументыОрганизаций - строка набора записей;
//            - Структура - структура параметров:
//               * Период - дата проверки
//               * Организация - проверяемая организация
//               * СостояниеПроведения - ПеречислениеСсылка.СостоянияПроведенияВПоследовательности - 
//               *  состояние проведения документа.
//
Процедура АктуализироватьРегламентныеОперации(Параметры) Экспорт
	
	Организация = Параметры.Организация;
	Период = Параметры.Период;
	СостоянияПроведения = Новый Массив;
	СостоянияПроведения.Добавить(
		Перечисления.СостоянияПроведенияВПоследовательности.ПроведенСНарушениемПоследовательности);
	СостоянияПроведения.Добавить(
		Перечисления.СостоянияПроведенияВПоследовательности.ИсключенИзПоследовательности);
	ИспользуетсяАктуализацияДляРасчетаНалогов = УчетнаяПолитика.ПрименяетсяУСН(Организация, Период)
		Или УчетнаяПолитика.ПлательщикНДФЛ(Организация, Период);
	
	Если ИспользуетсяАктуализацияДляРасчетаНалогов
		И СостоянияПроведения.Найти(Параметры.СостояниеПроведения) <> Неопределено
		И Не ЗакрытиеМесяца.ЕстьНеактуальныеОперацииДляРасчетаНалога(Организация, КонецДня(Период)) Тогда
		
		ВидОперации = ЗакрытиеМесяца.ОперацииДляРасчетаНалога()[0];
		НомерГруппы = 2;
		РегистрыСведений.НеактуальныеРегламентныеОперации.СдвинутьГраницуАктуальностиНазад(
			Организация,
			НачалоМесяца(Период),
