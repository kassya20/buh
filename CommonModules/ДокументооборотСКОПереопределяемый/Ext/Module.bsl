...
Процедура ПослеЗаписиДокументаРеализацииПолномочийНалоговыхОрганов(Ссылка, Файлы) Экспорт
	...
	УчетПСН.ПослеЗаписиДокументаРеализацииПолномочийНалоговыхОрганов(Ссылка, Файлы);
КонецПроцедуры
...
Процедура ОрганизацияОбязанаСдаватьОтчет(Организация, Отчет, Обязана) Экспорт
	...
	Если Отчет = "Форма6_НДФЛ" Тогда
		МассивПравил = ВыполнениеЗадачБухгалтера.ПравилаОтчетности6НДФЛ();
	ИначеЕсли Отчет = "ПерсонифицированныеСведенияОФизическихЛицах" Тогда
		МассивПравил = ВыполнениеЗадачБухгалтераБП.ПравилаОтчетностиПерсонифицированныеСведения();
	ИначеЕсли Отчет = "РасчетПоСтраховымВзносам" Тогда
		МассивПравил = ВыполнениеЗадачБухгалтераБП.ПравилаОтчетностиРасчетПоСтраховымВзносам();
	ИначеЕсли Отчет = "ЖурналУчетаСчетовФактур" Тогда
		МассивПравил = ВыполнениеЗадачБухгалтераБП.ПравилаОтчетностиЖурналУчетаСчетовФактур();
	КонецЕсли;
	...
	Для Каждого Правило Из МассивПравил Цикл
		МассивКодов.Добавить(Сред(Правило, СтрНайти(Правило, ".") + 1));
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Наименования", МассивКодов);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ПравилаПредставленияОтчетовУплатыНалоговУсловия.Ссылка.Код В(&Наименования)
	|	И НЕ ПравилаПредставленияОтчетовУплатыНалоговУсловия.Ссылка.ПометкаУдаления";
	...
	ВариантыПериодичности = Новый Массив;
	ВариантыПериодичности.Добавить(Перечисления.Периодичность.Квартал);
	ВариантыПериодичности.Добавить(Перечисления.Периодичность.Год);
	
	УсловияПримененияПравил = Справочники.ПравилаПредставленияОтчетовУплатыНалогов.УсловияПримененияПравил(Организация);
	...
	Для Каждого Периодичность Из ВариантыПериодичности Цикл
		Для Каждого СтрокаТаблицы Из ТаблицаРезультат Цикл
			УсловиеВыполняется = УсловияПримененияТребованийЗаконодательства.УсловиеВыполняется(
				УсловияПримененияПравил,
