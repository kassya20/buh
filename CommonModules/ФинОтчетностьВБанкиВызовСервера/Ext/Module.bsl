...
Функция ИнициализацияФинОтчетностиВБанки() Экспорт
		
	ПараметрыОбработчика = ФинОтчетностьВБанкиКлиентСервер.НовыеПараметрыОбработчикаОжиданияВходящихСообщений();
	ХранилищеОбщихНастроекСохранить("ПараметрыОбработчикаОжиданияВходящихСообщений", "", ПараметрыОбработчика);
	
	Возврат ЕстьЗаписиЖурналСтатусовФинОтчетностиВБанки();
...
Функция ЕстьЗаписиЖурналСтатусовФинОтчетностиВБанки()
		
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ЖурналСтатусовФинОтчетностиВБанки.Статус КАК Статус
		|ИЗ
