...
Функция ПолучитьДатыПериодаДокумента(Ссылка) Экспорт
	...
	Заполнено = Ложь;
	УчетОбособленныхПодразделений.ЗаполнитьДатыПериодаДокумента(ПериодОтчета, Ссылка, Заполнено);
	Если Заполнено = Истина Тогда
...
//
// Результат:
//  Структура, массив структур или Неопределено, если не удалось сформировать файл выгрузки,
//    в случае массива первый файл - основной, остальные - приложения.
//  Ключи структуры:
//    - АдресФайлаВыгрузки - адрес двоичных данных файла выгрузки во временном хранилище,
//                         - Неопределено - может возвращаться при заполнении ключа "Ошибки",
//    - ТипФайлаВыгрузки - строка,
//    - ИмяФайлаВыгрузки - короткое имя файла выгрузки (с расширением),
//    - КодировкаФайлаВыгрузки - перечисление КодировкаТекста, для приложений можно передать Неопределено,
...
Функция ВыгрузитьДокумент(Ссылка, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ВыгруженныеДокументы = Неопределено;
	УчетОбособленныхПодразделений.ВыгрузитьДокумент(ФайлДляОтправки, ВыгруженныеДокументы, Ссылка, УникальныйИдентификатор);
	Если ЗначениеЗаполнено(ВыгруженныеДокументы) Тогда
		Возврат ВыгруженныеДокументы;
	КонецЕсли;
...
Функция ПолучитьДанныеИсполнителя(СсылкаФизЛицо, ОрганизацияСсылка) Экспорт
	...
	АдресПоПропискеСтруктурой = УправлениеКонтактнойИнформациейБП.АдресСтруктурой(СсылкаФизЛицо, ВидКонтактнойИнформации, ТекущаяДатаСеанса());
	Если ЗначениеЗаполнено(АдресПоПропискеСтруктурой.ЗначениеJSON) Тогда
		Структура.Вставить("АдресПоПрописке_JSON", АдресПоПропискеСтруктурой.ЗначениеJSON);
	КонецЕсли;
	
	ВидКонтактнойИнформации = Справочники.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица;
	АдресМестаПроживанияСтруктурой = УправлениеКонтактнойИнформациейБП.АдресСтруктурой(СсылкаФизЛицо, ВидКонтактнойИнформации, ТекущаяДатаСеанса());
	Если ЗначениеЗаполнено(АдресМестаПроживанияСтруктурой.ЗначениеJSON) Тогда
		Структура.Вставить("АдресМестаПроживания_JSON", АдресМестаПроживанияСтруктурой.ЗначениеJSON);
	КонецЕсли;
...
Функция ПолучитьКодИФНСПолучателяПоСсылке(ОбъектСсылка) Экспорт
	...
	КодИФНСПолучателя = "";
	УчетОбособленныхПодразделений.ЗаполнитьКодИФНСПолучателяПоСсылке(ОбъектСсылка, КодИФНСПолучателя);
	Если ЗначениеЗаполнено(КодИФНСПолучателя) Тогда
		Возврат КодИФНСПолучателя;
	КонецЕсли;
