...
//	 * НайденыДубли - Булево
//   * ТекстПредупрежденияПриОткрытии - Строка - текст предупреждения, которое будет показано при открытии формы похожих
//     физических лиц.
//   * ТекстПодсказки - Строка
//   * ЗаголовокКнопкиНетЭтоДругойЧеловек - Строка
Функция ПараметрыОткрытияФормыФизическиеЛицаСПохожимиДанными(РезультатПоиска = Неопределено) Экспорт
	...
	ПараметрыОткрытия.Вставить("ТекстПредупрежденияПриОткрытии", "");
	ПараметрыОткрытия.Вставить("ТекстПодсказки", "");
	ПараметрыОткрытия.Вставить("ЗаголовокКнопкиНетЭтоДругойЧеловек", "");
	...
	Если НЕ Форма.ФизическоеЛицоЗаблокировано Тогда
		...
		Если НЕ СотрудникиВызовСервера.ЗаблокироватьФизическоеЛицоПриРедактированииНаСервере(Форма.ФизическоеЛицоСсылка, Форма.ФизическоеЛицоВерсияДанных, Форма.УникальныйИдентификатор) Тогда
			...
			Если СообщатьОНевозможностиБлокировки Тогда
				...
				Иначе
					СведенияОБлокировке = СотрудникиВызовСервера.СформироватьСведенияОБлокировке(Форма.ФизическоеЛицоСсылка, Форма.ФизическоеЛицоВерсияДанных, Форма.УникальныйИдентификатор);
					ПоказатьПредупреждение(, СтрШаблон(НСтр("ru = 'Не удается внести изменения в личные данные сотрудника. Уже открыта форма с личными данными или личные данные редактируются другим пользователем %1'"), СведенияОБлокировке));
				КонецЕсли;
...
Функция РолиФизическихЛицПолучателейДохода() Экспорт
	РолиФизическихЛиц = Новый Массив;
	СотрудникиКлиентВнутренний.ДополнитьРолиФизическихЛицПолучателейДохода(РолиФизическихЛиц);
...
Процедура ОткрытьДополнительнуюФорму(ОписаниеДополнительнойФормы, Форма, ИзФормыСотрудника = Ложь, ОписаниеОповещения = Неопределено, РежимОткрытияОкна = Неопределено) Экспорт
	
	ПараметрыФормы = ПараметрыДополнительнойФормы(ОписаниеДополнительнойФормы, Форма, ИзФормыСотрудника);
	ОткрытьФорму(ОписаниеДополнительнойФормы.ИмяФормы, ПараметрыФормы, Форма, , , , ОписаниеОповещения, РежимОткрытияОкна);
