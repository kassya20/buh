...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПересчитатьИтоги", Истина);
...
//
Функция СчетаУчетаТоваров(ИсключаемыеСчета = Неопределено, ИзПомощникаИнвентаризации = Ложь) Экспорт
	...
	БухгалтерскиеОтчетыПереопределяемый.СчетаУчетаТоваров(СчетаУчетаТоваров, ИзПомощникаИнвентаризации);
...
// Для случая, когда установлены отборы по доп.свойствам подразделений, будет произведена замена в тексте схемы
// компоновки данных. Отборы по дополнительным свойствам невозможны в параметрах виртуальной таблицы.
// Поэтому в текст набора данных Проводки в СКД к запросам по остаткам добавляется секция ГДЕ.
//
// Параметры:
...
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - связь настроек компоновки данных и схемы компоновки.
//
Процедура ИзменитьСхемуДляОтборовПоДопСвойствамПодразделения(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	
	Если Не ЕстьОтборПоДопСвойствуПодразделения(КомпоновщикНастроек.Настройки.Отбор.Элементы) Тогда 
		 Возврат;
	КонецЕсли;
	...
	Для Каждого ЗапросПакета Из ПакетЗапросов Цикл
		...
		// При отборе, используя секцию ГДЕ, возможно появление строк, где есть только остатки
		// и строк, где есть только обороты (период/регистратор/номер одинаковые). Требуется сгруппировать такие строки.
		Если ЗапросПакета.ТаблицаДляПомещения = "ХозрасчетныйОстаткиИОбороты" Тогда
			...
			Для каждого ВыбираемоеПоле Из ВыбираемыеПоля Цикл
				...
				// Группировка была добавлена ранее
				Если СтрНачинаетсяС(ВыбираемоеПоле, "СУММА(") Тогда
					Продолжить;
				КонецЕсли;
				ИмяРесурса = Сред(ВыбираемоеПоле, 29); // убираем имя таблицы
				Если СтрНачинаетсяС(ИмяРесурса, "Сумма")
...
Процедура ИзменитьСхемуДляОтборовПоСубконтоСубсчетов(ПараметрыОтчета, КомпоновщикНастроек, Схема, ИмяОсновногоНабора) Экспорт
	...
	НетОтборовПоСубконто = (НомераОтбираемыхСубконто.Количество() = 0);
	...
	// Если нет отборов по субконто, то проверяем дополнительное условие (наличие видов
	// субконто субсчетов, которые не совпадают с видами субконто счета-группы) 
	// и, если оно не выполняется, схему не меняем.
	// Если есть отборы по субконто, то дополнительное условие не проверяем, схему меняем безусловно.
	Если НетОтборовПоСубконто Тогда
...
//                         * Направление     - Строка - направление поиска в кросс-таблице (ТаблицаКомпоновкиДанных):
//                                             ** "Строки"  - только в строках таблицы
//                                             ** "Колонки" - только в колонках таблицы
//                                             Иное или не указанное значение означает, что поиск выполняется везде:
//                                             в строках и в колонках.
...
// Приводит колонки показателей отчета к одинаковой (максимальной) ширине.
// Предполагается, что колонки показателей отчета находятся в конце таблицы.
//
// Параметры:
//  ТабличныйДокумент - ТабличныйДокумент - результат отчета.
//  КоличествоКолонок - Число - количество колонок числовых показателей отчета.
//
Процедура НастроитьШиринуКолонкиИтогов(ТабличныйДокумент, КоличествоКолонок) Экспорт
	
	МаксимальнаяШирина = СтандартнаяШиринаПолейПоказателейОтчета();
	
	НомерПоследнейКолонки = ТабличныйДокумент.ШиринаТаблицы;
	
	Для НомерКолонки = НомерПоследнейКолонки - КоличествоКолонок + 1 По НомерПоследнейКолонки Цикл
		ШиринаТекущейКолонки = ТабличныйДокумент.Область("C" + XMLСтрока(НомерКолонки)).ШиринаКолонки;
		МаксимальнаяШирина = Макс(ШиринаТекущейКолонки, МаксимальнаяШирина);
	КонецЦикла;
	
	Для НомерКолонки = НомерПоследнейКолонки - КоличествоКолонок + 1 По НомерПоследнейКолонки Цикл
		ТабличныйДокумент.Область("C" + XMLСтрока(НомерКолонки)).ШиринаКолонки = МаксимальнаяШирина;
	КонецЦикла;
...
Процедура ПересчитатьИтоги(МестоРасхождения, АдресРезультата) Экспорт
	...
	Попытка
		...
		ЦелевойМинимальныйПериодРассчитанныхИтогов = НачалоГода(ДобавитьМесяц(Сейчас, -24));
		...
		КонецЕсли;
		ИсходныйПериодРассчитанныхИтогов = РегистрСИтогами.ПолучитьМинимальныйПериодРассчитанныхИтогов();
		Если КорректныйПериод.НачалоКорректногоПериода > ИсходныйПериодРассчитанныхИтогов Тогда
			ИсходныйПериодРассчитанныхИтогов = КорректныйПериод.НачалоКорректногоПериода;
		КонецЕсли;
		Если ИсходныйПериодРассчитанныхИтогов > ЦелевойМинимальныйПериодРассчитанныхИтогов Тогда
			ЦелевойМинимальныйПериодРассчитанныхИтогов = НачалоМесяца(ИсходныйПериодРассчитанныхИтогов);
		КонецЕсли;
		
		// Определение конца интервала.
		ЦелевойМаксимальныйПериодРассчитанныхИтогов = КонецМесяца(Сейчас);
		...
		КонецЕсли;
		ИсходныйПериодРассчитанныхИтогов = РегистрСИтогами.ПолучитьМаксимальныйПериодРассчитанныхИтогов();
		Если Не ЗначениеЗаполнено(ИсходныйПериодРассчитанныхИтогов)
		 Или КорректныйПериод.КонецКорректногоПериода < ИсходныйПериодРассчитанныхИтогов
		 Или ЦелевойМинимальныйПериодРассчитанныхИтогов > ИсходныйПериодРассчитанныхИтогов Тогда
			ИсходныйПериодРассчитанныхИтогов = КорректныйПериод.КонецКорректногоПериода;
		КонецЕсли;
		Если ЦелевойМаксимальныйПериодРассчитанныхИтогов > ИсходныйПериодРассчитанныхИтогов Тогда
			ЦелевойМаксимальныйПериодРассчитанныхИтогов = КонецМесяца(ИсходныйПериодРассчитанныхИтогов);
		КонецЕсли;
		
		// Очистка данных в таблицах итогов.
		РегистрСИтогами.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
				'19800101', '19800131235959');
				
		// Установка последних двух лет как периода расчитанных итогов.
		РегистрСИтогами.УстановитьМинимальныйИМаксимальныйПериодыРассчитанныхИтогов(
...
// Обрабатывает результат фонового задания после завершения формирования стандартного отчета.
//
// Параметры:
//  РезультатВыполнения - Структура - см. описание параметра ОповещениеОЗавершении в ДлительныеОперацииКлиент.ОжидатьЗавершение()
//  Форма        - ФормаКлиентскогоПриложения - откуда было вызвано фоновое задание.
//
Процедура ЗагрузитьПодготовленныеДанные(РезультатВыполнения, Форма) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РезультатВыполнения, "Статус", "Отменено") = "Отменено" Тогда
		...
		Форма.Результат = Новый ТабличныйДокумент;
		Форма.ДанныеРасшифровки = Неопределено;
	...
	ИначеЕсли РезультатВыполнения.Статус = "Ошибка" Тогда
		...
		ПозицияТочки = СтрНайти(Форма.ИмяФормы, ".", ,, 2);
		ПолноеИмяМетаданного = ?(ПозицияТочки = 0, Форма.ИмяФормы, Лев(Форма.ИмяФормы, ПозицияТочки - 1));
		ОтчетМетаданные = Метаданные.НайтиПоПолномуИмени(ПолноеИмяМетаданного);
		ОтчетОрганизация = ?(Форма.Отчет.Свойство("Организация"), Форма.Отчет.Организация, Неопределено);
	...
	Форма.ОписаниеЗаданияФормированияОтчета = Неопределено;
	...
	Форма.Результат = ДанныеОтчета.Результат;
	ДанныеОтчета.Результат = Неопределено;
	Форма.ДанныеРасшифровки = ДанныеОтчета.ДанныеРасшифровки;
	ДанныеОтчета.ДанныеРасшифровки = Неопределено;
		
	БухгалтерскийУчетПереопределяемый.ОбработатьРезультатПроверкиИтогов(Форма, ДанныеОтчета);
	
КонецПроцедуры

// Завершает ранее запущенное фоновое задание в форме отчета, если оно было.
//
// Параметры:
//  Форма        - ФормаКлиентскогоПриложения - форма отчета.
//
Процедура ОтменитьВыполнениеЗадания(Форма) Экспорт
	...
	Если Форма.ОписаниеЗаданияФормированияОтчета <> Неопределено Тогда
		
		ДлительныеОперации.ОтменитьВыполнениеЗадания(Форма.ОписаниеЗаданияФормированияОтчета.ИдентификаторЗадания);
		Форма.ОписаниеЗаданияФормированияОтчета = Неопределено;
...
// Запускает фоновое задание в форме отчета для формирования табличного документа отчета.
//
// Параметры:
//  Форма        - ФормаКлиентскогоПриложения - форма отчета.
//  ПараметрыОтчета - Структура - входящие параметры для алгоритма формирования отчета.
//
// Возвращаемое значение:
...
Функция ЗапуститьПодготовкуОтчета(Форма, ПараметрыОтчета) Экспорт
	...
	ПараметрыЗадания.НаименованиеФоновогоЗадания = БухгалтерскиеОтчетыКлиентСервер.ПолучитьНаименованиеЗаданияВыполненияОтчета(Форма);
...
Функция СтандартнаяШиринаПолейПоказателейОтчета()
...
Функция СчетаУчетаРасчетов(ИсключенныеСчета)
	...
	|	И ХозрасчетныйВидыСубконто_Контрагенты.Ссылка <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСТаможенныйСоюзКУплате)
	|	И ХозрасчетныйВидыСубконто_Контрагенты.Ссылка <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНалоговогоАгента)
	|	И ХозрасчетныйВидыСубконто_Контрагенты.Ссылка <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам)
...
Функция ЕстьОтборПоДопСвойствуПодразделения(Отбор) 

	ИмяИскомогоПоля = "Подразделение";
	ДлинаИскомогоПоля = СтрДлина(ИмяИскомогоПоля);
	...
	Для Каждого ЭлементОтбора Из Отбор Цикл
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
			...
			Возврат ЕстьОтборПоДопСвойствуПодразделения(ЭлементОтбора.Элементы);
			
		Иначе
			
			ИмяПоля = Строка(ЭлементОтбора.ЛевоеЗначение);
			Если СтрНачинаетсяС(ИмяПоля, ИмяИскомогоПоля) Тогда
				Если СтрНайти(Сред(ИмяПоля, ДлинаИскомогоПоля + 1), ".[") > 0 Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЕсли;
...
// (даже если это поле одно из нескольких полей группиовки)
//
// Параметры:
