...
// ВОЗВРАТ ОТ ПОДОТЧЕТНОГО ЛИЦА В ВАЛЮТЕ

Функция ПодготовитьТаблицуВозвратОтПодотчетногоЛицаВВалюте(ТаблицаРеквизитыВозврата) Экспорт
	
	Если Не ЗначениеЗаполнено(ТаблицаРеквизитыВозврата) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Реквизиты = ТаблицаРеквизитыВозврата[0];
	
	ТаблицаРезультат = ТаблицаРеквизитыВозврата.СкопироватьКолонки();
	
	// Получем остатки выданных подотчетному лицу авансов
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Остатки.Субконто1 КАК ПодотчетноеЛицо,
	...
	|					ИЛИ Подразделение ЕСТЬ NULL)) КАК Остатки
	|ГДЕ
	|	Остатки.ВалютнаяСуммаОстаток > 0";
	Запрос.УстановитьПараметр("МоментДокумента", Реквизиты.Регистратор.МоментВремени());
	Запрос.УстановитьПараметр("Счет7121", ПланыСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицамиВал);
	Запрос.УстановитьПараметр("РаботникиОрганизаций", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.РаботникиОрганизаций);
	...
	Если Выборка.Следующий() Тогда
		АвансВВалюте = Мин(Выборка.ВалютнаяСумма, Реквизиты.ВалютнаяСумма);
		АвансБУ = ?(АвансВВалюте = Выборка.ВалютнаяСумма, 
			Выборка.Сумма, 
			Окр(АвансВВалюте / Выборка.ВалютнаяСумма * Выборка.Сумма, 2));
		АвансНУ = ?(АвансВВалюте = Выборка.ВалютнаяСумма, 
			Выборка.СуммаНУ, 
			Окр(АвансВВалюте / Выборка.ВалютнаяСумма * Выборка.СуммаНУ, 2));
	Иначе
		АвансВВалюте = 0;
		АвансБУ = 0;
		АвансНУ = 0;
	...
	Если АвансВВалюте > 0 Тогда
		НоваяСтрока = ТаблицаРезультат.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Реквизиты);
		НоваяСтрока.ВалютнаяСумма = АвансВВалюте;
		НоваяСтрока.СуммаБУ = АвансБУ;
		НоваяСтрока.СуммаНУ = АвансНУ;
	КонецЕсли;
	
	ЗадолженностьВВалюте = Реквизиты.ВалютнаяСумма - АвансВВалюте;
	Если ЗадолженностьВВалюте > 0 Тогда
		НоваяСтрока = ТаблицаРезультат.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Реквизиты);
		НоваяСтрока.ВалютнаяСумма = ЗадолженностьВВалюте;
		ЗадолженностьВРублях = Окр(ЗадолженностьВВалюте * Реквизиты.КоэффициентРуб, 2);
		НоваяСтрока.СуммаБУ = ЗадолженностьВРублях;
		НоваяСтрока.СуммаНУ = ЗадолженностьВРублях;
	КонецЕсли;
...
Процедура СформироватьДвиженияВозвратОтПодотчетногоЛицаВВалюте(ТаблицаВозврата, Движения, Отказ) Экспорт
	
	Если Не ЗначениеЗаполнено(ТаблицаВозврата) Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из ТаблицаВозврата Цикл
		...
		Проводка.СчетКт = ПланыСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицамиВал; // 71.21
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, 
			"РаботникиОрганизаций", СтрокаТаблицы.ПодотчетноеЛицо);
		...
		НалоговыйУчет.ЗаполнитьНалоговыеСуммыПроводки(СтрокаТаблицы.СуммаНУ, СтрокаТаблицы.СуммаНУ, , , , , Проводка);
