...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Документы.УведомлениеОПрекращенииОтпускаПоУходуЗаРебенком.ПриДобавленииОбработчиковОбновления(Обработчики);
...
Процедура ПриОпределенииОбработчиковТекущихДел(Обработчики) Экспорт
	...
	Обработчики.Добавить(Документы.ЗаявлениеВФССОВозмещенииВыплатРодителямДетейИнвалидов);
...
Процедура ПослеРасшифровкиСообщенияСЭДО(Страхователь, Сообщение, Результат) Экспорт
	...
	ИначеЕсли ТипСообщения = 105 Тогда
		...
		РегистрыСведений.РегистрацииОтветовНаЗапросыФССДляРасчетаПособий.ЗагрузитьСообщение105(
			...
			Результат,
			Кэш);
	ИначеЕсли ТипСообщения = 106 Или ТипСообщения = 10610 Тогда
		// Результат уведомления о прекращении отпуска по уходу за ребенком до полутора лет.
		РегистрыСведений.РегистрацииУведомленийОПрекращенииОтпускаПоУходуЗаРебенком.ЗагрузитьСообщение(
			ТипСообщения,
			Страхователь,
			ИдентификаторСообщения,
			Сообщение.ТекстСообщения,
...
Процедура ПослеПолученияОшибокЛогическогоКонтроляСЭДО(Страхователь, ИсходноеСообщение, ТекстОшибки, Результат) Экспорт
	...
	ИначеЕсли ТипСообщения = 104 Тогда
		...
		РегистрыСведений.РегистрацииУведомленийОПрекращенииОтпускаПоУходуЗаРебенком.ЗагрузитьОшибкуСообщения104(
			...
			ТекстОшибки,
			Результат);
	ИначеЕсли ТипСообщения = 10410 Тогда
		// Уведомление о прекращении отпуска по уходу за ребенком до полутора лет.
		РегистрыСведений.РегистрацииУведомленийОПрекращенииОтпускаПоУходуЗаРебенком.ЗагрузитьОшибкуСообщения10410(
			Страхователь,
			ИсходноеСообщение.Идентификатор,
			ТекстОшибки,
			Результат);
...
Процедура ЗагрузитьРезультатДоставкиСообщенияСЭДО(Страхователь, Сообщение, Результат)
	...
	ИначеЕсли Сообщение.ТипЧислом = 104 Тогда
		...
		Документы.УведомлениеОПрекращенииОтпускаПоУходуЗаРебенком.ЗагрузитьРезультатДоставкиСообщения104(
			...
			Сообщение,
			Результат);
	ИначеЕсли Сообщение.ТипЧислом = 10410 Тогда
		// Уведомление о прекращении отпуска по уходу за ребенком до полутора лет.
		Документы.УведомлениеОПрекращенииОтпускаПоУходуЗаРебенком.ЗагрузитьРезультатДоставкиСообщения10410(
			Страхователь,
			Сообщение,
			Результат);
...
Функция ДатаСообщения(ИдентификаторСообщения, Кэш = Неопределено, ДатаИзСообщенияXML = Неопределено) Экспорт
	Если ДатаИзСообщенияXML = Неопределено Тогда
		ДополнительнаяДата = Неопределено;
	Иначе
		ДополнительнаяДата = СериализацияБЗК.ДатаИзXML(ДатаИзСообщенияXML);
	КонецЕсли;
	Если Кэш <> Неопределено Тогда
		...
		КонецЕсли;
		Если ДополнительнаяДата <> Неопределено
			И ОбновитьДатуСообщения(ДатаСообщения, НачалоДня(ДатаСообщения), ДополнительнаяДата) Тогда
			Кэш.Вставить("ДатаСообщения", ДатаСообщения);
		КонецЕсли;
	...
	Выборка = Запрос.Выполнить().Выбрать();
	НайденнаяДата = ?(ДополнительнаяДата = Неопределено, ТекущаяДатаСеанса(), ДополнительнаяДата);
...
Функция ОбновитьДатуСообщения(ДатаСообщения, НачалоДняСообщения, НоваяДата) Экспорт
...
// Возвращает дату перехода на ЕЦП в части уведомления о прекращении отпуска по уходу за ребенком.
Функция ДатаПереходаПособия5НаЕЦП() Экспорт
	Возврат '20260512090000';
КонецФункции

// Возвращает Истина, если указанный вид пособия обрабатывается в ЕЦП.
Функция ВыплатаПособияПереведенаНаЕЦП(Документ, ВидПособия) Экспорт
	
	Если ВидПособия = Перечисления.ПособияНазначаемыеФСС.ЕдиновременноеПособиеПриРожденииРебенка Тогда
		
		Возврат Истина;
		
	ИначеЕсли ВидПособия = Перечисления.ПособияНазначаемыеФСС.ПособиеПоБеременностиИРодам
		Или ВидПособия = Перечисления.ПособияНазначаемыеФСС.ПособиеПоБеременностиИРодамВставшимНаУчетВРанниеСроки
		Или ВидПособия = Перечисления.ПособияНазначаемыеФСС.ЕжемесячноеПособиеПоУходуЗаРебенком Тогда
		
		Возврат Истина;
		
	Иначе
		
		Возврат ТекущаяДатаФонда() >= '20260521000000';
...
Функция ЗаписатьФИОXML(МенеджерXML, УзелФИО, Фамилия, Имя, Отчество) Экспорт
	Если ЗначениеЗаполнено(Фамилия) Тогда
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:lastName", Фамилия);
	Иначе
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:noLastName", Истина);
	КонецЕсли;
	Если ЗначениеЗаполнено(Имя) Тогда
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:firstName", Имя);
	Иначе
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:noFirstName", Истина);
	КонецЕсли;
	Если ЗначениеЗаполнено(Отчество) Тогда
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:middleName", Отчество);
	Иначе
		МенеджерXML.ДобавитьУзел(УзелФИО, "person:noMiddleName", Истина);
	КонецЕсли;
КонецФункции
...
Функция ПредставлениеСпискаОшибокXML(СписокОшибокDOM, ЗаголовокСписка = "") Экспорт
	...
	Иначе
		...
		Иначе
			Возврат ЗаголовокСписка + " (" + Формат(Количество, "ЧН=; ЧГ=") + "): " + Представление;
		КонецЕсли;
	КонецЕсли;
КонецФункции
