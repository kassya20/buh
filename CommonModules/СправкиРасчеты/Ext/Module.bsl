...
Процедура ДобавитьПримечание(ТабличныйДокумент, ТекстПримечания, СчетчикПримечаний = 0) Экспорт
	...
	ОбластьСноска.Шрифт = Новый Шрифт(, 8, Ложь, Ложь, Ложь);
...
// Определяет, отключено ли для отчета использование расширенного налогового периода в параметрах исполнения отчета.
//
// Параметры:
//  ОписаниеОтчета - Строка - имя отчета
//                 - Структура - см. БухгалтерскиеОтчеты.ПараметрыИсполненияОтчета
//
// Возвращаемое значение:
//  Булево - Истина, если отключено.
//
Функция ОтключеноИспользованиеНалоговогоПериода(ОписаниеОтчета) Экспорт
...
#Область БазовыеПараметрыОтчета

Процедура УстановитьБазовыеПараметрыОтчета(Контекст, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.НачалоПериода) Тогда
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
			КомпоновщикНастроек,
			"НачалоПериода",
			НачалоДня(Контекст.НачалоПериода));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Контекст.КонецПериода) Тогда
		БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
			КомпоновщикНастроек,
			"КонецПериода",
			КонецДня(Контекст.КонецПериода));
	КонецЕсли;
...
Процедура ДобавитьГруппировкиТаблицыКомпоновкиДанныхПоШаблону(
	...
	Для каждого ПолеВыбраннойГруппировки Из НастройкаГруппировок Цикл
		Если ПолеВыбраннойГруппировки.Использование Тогда
			...
			СтруктураШаблон = БухгалтерскиеОтчеты.НайтиСтрокуТаблицыРекурсивно(ГруппировкаШаблон, ПолеВыбраннойГруппировки.Поле);
...
Функция СформироватьОтчет(Форма, СобытияФормированияОтчета = Неопределено) Экспорт
	...
	ИспользуетсяНоваяВерсияБСП = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(
		Форма, "ОписаниеЗаданияФормированияОтчета");
	
	Если ИспользуетсяНоваяВерсияБСП Тогда
		РезультатВыполнения = СформироватьОтчетНовыеФункцииФоновыхБСП(Форма, СобытияФормированияОтчета);
...
Функция НайтиМакетыЗаголовкаТаблицы(МакетКомпоновки) Экспорт
	...
	Для Каждого ТелоТаблицы Из МакетКомпоновки.Тело Цикл
		...
		ЗаполнитьМакетыЗаголовкаТаблицы(МакетыЗаголовка, ТелоТаблицы);
...
// Заполняет имена макетов областей таблицы компоновки данных.
//
// Параметры:
//  МакетыЗаголовка - Соответствие - описание макетов:
//   * Ключ - Строка - имя макета;
//   * Значение - Строка - имя колонки или служебное имя для заголовка строк.
//  ТелоТаблицы - ТаблицаМакетаКомпоновкиДанных
//
Процедура ЗаполнитьМакетыЗаголовкаТаблицы(МакетыЗаголовка, ТелоТаблицы) Экспорт
...
Процедура НастроитьГрафыОтчета(МакетКомпоновки, ГрафыОтчета, РазмещениеМакетовЗаголовкаТаблицы = Неопределено) Экспорт
	...
	Для Каждого ОписаниеМакетаОбласти Из РазмещениеМакетовЗаголовкаТаблицы Цикл
		...
		ДобавитьНомераКолонок(МакетЗаголовка, ИменаГрафОтчета);
	
	КонецЦикла;
	
КонецПроцедуры

Процедура ДобавитьНомераКолонок(МакетЗаголовка, Знач ИменаГрафОтчета = Неопределено, АвтоматическаяНумерация = Ложь) Экспорт
	...
	Для Каждого ЭталоннаяЯчейка Из ЭталоннаяСтрока.Ячейки Цикл
		...
		ИначеЕсли ИндексГрафыВКолонке > ИменаГрафОтчета.ВГраница() Тогда
...
Процедура ДополнитьОформлениеЯчейкиТаблицыОбластиКомпоновкиДанных(НоваяЯчейка, ЭталоннаяЯчейка) Экспорт
...
Функция КонтекстФормированияОтчета(Форма)
	...
	Контекст.Вставить("ВыводитьЕдиницуИзмерения",          Истина); // Соответствует флагу "Единица измерения" в форме справки-расчета на закладке "Оформление".
	Контекст.Вставить("ВыводитьЗаголовок",                 Истина); // Соответствует флагу "Заголовок" в форме справки-расчета на закладке "Оформление".
	Контекст.Вставить("ВыводитьПодвал",                    Истина); // Соответствует флагу "Подписи" в форме справки-расчета на закладке "Оформление".
	...
	Контекст.Вставить("ВидРегламентнойОперации", Неопределено);
	...
	ДополнительныеНастройкиФормы = Новый Структура("МакетОформления, ВыводитьЕдиницуИзмерения, ВыводитьЗаголовок, ВыводитьПодвал");
	...
	Если ДоступныеНаборыСуммовыхПоказателей <> Неопределено Тогда
		...
		Если Контекст.НаборПоказателей.Количество() > 1 Тогда
			...
			// В общей форме СправкаРасчет набор передается через свойство формы.
			// В зависимости от применяемой системы налогообложения и учетной политики некоторые наборы могут быть отключены.
			Если СправкиРасчетыКлиентСервер.ОбщаяФорма(Форма) Тогда
				Контекст.НомерНабораСуммовыхПоказателей = Форма.НаборПоказателейОтчета;
			ИначеЕсли ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Отчет, "НаборПоказателейОтчета") Тогда
	...
	Если Форма.Параметры.Свойство("ВидРегламентнойОперации") Тогда
		Контекст.ВидРегламентнойОперации = Форма.Параметры.ВидРегламентнойОперации;
	КонецЕсли;
...
Функция СформироватьОтчетНовыеФункцииФоновыхБСП(Форма, СобытияФормированияОтчета)
	...
	Настройки.Порядок.ИдентификаторПользовательскойНастройки            = "";
	Настройки.УсловноеОформление.ИдентификаторПользовательскойНастройки = "";
	Настройки.ДополнительныеСвойства.Вставить("ВыводитьЗаголовок", Форма.ВыводитьЗаголовок);
	Настройки.ДополнительныеСвойства.Вставить("ВыводитьПодвал"   , Форма.ВыводитьПодвал);
	...
	БухгалтерскиеОтчеты.ОтменитьВыполнениеЗадания(Форма);
	
	РезультатВыполнения = БухгалтерскиеОтчеты.ЗапуститьПодготовкуОтчета(Форма, Контекст);
	Форма.ОписаниеЗаданияФормированияОтчета = РезультатВыполнения;
