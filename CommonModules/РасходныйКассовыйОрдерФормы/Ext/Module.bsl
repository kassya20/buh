...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Форма.ПредупреждениеВыдачаЗПНаличнымиАУСН =
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Если ЭлементТипКонтрагента <> Неопределено Тогда
		ЭлементТипКонтрагента.Видимость = БанкИКассаФормыКлиентСервер.ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма);
	КонецЕсли;
	...
	Если Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма) Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратПокупателю И Форма.ПлательщикНПД Тогда
			...
			Элементы.ГруппаЧекНПД.Видимость = ЧекАннулирован
				Или ЧекиНПДВызовСервера.ВключенаИнтеграцияСПлатформойСамозанятые();
	...
	// Для вида операции "Выплата сотруднику по договору ГПХ", если выбрана ведомость, то
	// сумму документа править нельзя, необходимо редактировать исходные ведомости на выплату.
	...
	ЭлементГруппаПредупреждениеВыдачаЗПНаличнымиАУСН = Элементы.Найти("ГруппаПредупреждениеВыдачаЗПНаличнымиАУСН");
	Если ЭлементГруппаПредупреждениеВыдачаЗПНаличнымиАУСН <> Неопределено Тогда
		ЭлементГруппаПредупреждениеВыдачаЗПНаличнымиАУСН.Видимость = 
			Перечисления.ВидыОперацийРКО.ЭтоВыплатаЗарплаты(Объект.ВидОперации);
	КонецЕсли;
...
Процедура ПодготовитьОднострочнуюФорму(Форма)
	...
	ПроверитьЕстьБюджетНаДатуНаСервере(Форма);
	ЗаполнитьБыстрыйВыборСтатьиРасходов(Форма);
...
Процедура УстановитьВидимостьСтраниц(Форма)
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаСотрудникуПоДоговоруПодряда Тогда
		...
		Элементы.ВедомостьПоДоговоруПодряда.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьНачисленияПоДоговорам")
			Или (Форма.УчетЗарплатыИКадровВоВнешнейПрограмме И Не Форма.ВедетсяУчетРасчетовПоЗарплатеСводно);
...
Процедура ВидОперацииОбработатьИзменение(Форма, ПричиныИзменения)
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВзносНаличнымиВБанк Тогда
		...
		НовыйПараметр = Новый ПараметрВыбора("Отбор.ЦифровойСчет", Ложь);
	...
	Элементы.СчетОрганизации.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметрыВыбора);
...
Процедура СчетОрганизацииПриИзмененииСервер(Форма) Экспорт
	
	ПричиныИзменения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве("СчетОрганизации");
...
Функция НовыеПараметрыПриИзмененииКонтрагента(ПараметрыОбъекта, УчетЗарплатыИКадровВоВнешнейПрограмме, ВедетсяУчетРасчетовПоЗарплатеСводно)
	...
	Если ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратРозничномуПокупателю Тогда
		...
		ОсобыйНалоговыйРежим = ПараметрыОбъекта.ПрименяетсяОсобыйНалоговыйРежим
		...
		НовыеПараметры.Вставить("СтавкаНДС", УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(
			ПараметрыОбъекта.Дата, ПараметрыОбъекта.ПлательщикНДС И Не ОсобыйНалоговыйРежим));
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(ПараметрыПлатежа, ПолучатьДоговор) Экспорт
	...
	Если ПараметрыПлатежа.БезЗакрывающихДокументов Тогда
		УчетПСН.УстановитьПорядокОтраженияАванса(СвойстваПлатежа.ПорядокОтраженияДохода, ПараметрыПлатежа);
	КонецЕсли;
...
Функция СвойстваСтрокРасшифровкиПлатежа(ПараметрыОбъекта, ПолучатьДоговор) Экспорт
	...
	Иначе
		...
		ПараметрыПлатежа = Новый Структура("ВидОперации, Организация, Дата, Контрагент, ДоговорКонтрагента, Сделка, 
			...
			|ПлательщикНДСПриУСН, СтавкаНДСПриУСН, Ссылка");
...
Процедура УстановитьОграничениеТипаКонтрагента(Форма) Экспорт
	...
	Если БанкИКассаФормыКлиентСервер.ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма) Тогда 
		...
		КонецЕсли;
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ОплатаПоставщику")
...
Процедура УстановитьВидимостьКомандПечати(Форма) Экспорт
	...
	// БСП за один проход может установить только Доступность или Видимость команды.
	// При этом то, что будет изменяться зависит от вида подменю Печать.
	// При этом БСП расчитывает на то, вид подменю в объекте никогда не меняется, но у нас это не так.
	// Поэтому, если вид не равен "Подменю", а Видимость и Доступность команды выключены,
	// то для их включения требуется 2 вызова метода БСП.
	Если ПодменюПечать.Вид <> ВидГруппыФормы.Подменю Тогда
		ПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(Форма, Форма.Объект);
	КонецЕсли;
...
Процедура ПроверитьЕстьБюджетНаДатуНаСервере(Форма)
...
Процедура ЗаполнитьБыстрыйВыборСтатьиРасходов(Форма) Экспорт
	
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЕстьБюджетНаПериод")
		Или Не Форма.ЕстьБюджетНаПериод Тогда
		Возврат;
	КонецЕсли;
	Объект = Форма.Объект;
	...
	Форма.ДоступныеСтатьиРасходов.ЗагрузитьЗначения(МассивСтатейРасходов);
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц(Форма) Экспорт
	...
	Если Форма.ПрименениеУСН И Не Форма.ПрименениеУСНДоходы Тогда
		Для каждого СтрокаПлатеж Из РасшифровкаПлатежа Цикл
			РасходныйКассовыйОрдерФормыКлиентСервер.ЗаполнитьНадписиУСНВРасшифровке(СтрокаПлатеж);
		КонецЦикла;
	КонецЕсли;
...
Процедура ПерезаполнитьРасшифровкуПлатежа(Форма)
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийРКО.ВозвратРозничномуПокупателю Тогда
		ПрименяетсяТолькоОсобыйРежим = УчетнаяПолитика.ПрименяетсяОсобыйПорядокНалогообложения(Объект.Организация, Объект.Дата);
		ПлательщикНДС = Форма.ПлательщикНДС И Не(Форма.ПрименениеУСН Или ПрименяетсяТолькоОсобыйРежим
...
Процедура ПересчитатьСуммыВзаиморасчетов(Форма)
	...
	Если РасходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
		...
		СтрокаПлатеж = РасходныйКассовыйОрдерФормыКлиентСервер.ДанныеРасшифровкиПлатежаОднострочнойФормы(Форма);
...
Процедура ЗаполнитьОтражениеВУСННаСервере(Форма, ЗаполнитьСуммы = Истина, ЗаполнитьСодержание = Ложь) Экспорт
	...
	Если ЗаполнитьСуммы Тогда
		ОбновлятьРеквизитыФормы = РасходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
			И РасходныйКассовыйОрдерФормыКлиентСервер.ЕстьРасшифровкаПлатежа(Объект.ВидОперации)
		...
		Если ОбновлятьРеквизитыФормы Тогда
			РасходныйКассовыйОрдерФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма);
		КонецЕсли;
...
Процедура УправлениеЭлементамиОтраженияВУСНСервер(Форма) Экспорт
	...
	Если ЭлементРасшифровкаПлатежаОтражениеВУСН <> Неопределено Тогда
		ЭлементРасшифровкаПлатежаОтражениеВУСН.Видимость = ВидимостьРасходыУСН;
	КонецЕсли;
	...
	Если ЭлементРасшифровкаПлатежаЗаголовокРасходыУСН <> Неопределено Тогда
		ЭлементРасшифровкаПлатежаЗаголовокРасходыУСН.Видимость = ВидимостьРасходыУСН;
	КонецЕсли;
...
Процедура УстановитьЗаголовкиКолонокУСН(Форма)
	...
	Если Не (Форма.ПрименениеУСН И Не Форма.ПрименениеУСНДоходы) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСпискиВыбораОтражениеВУСН(Форма)
	...
	ПрименяетсяУСНДоходыРасходы = Форма.ПрименениеУСН И Не Форма.ПрименениеУСНДоходы;
	
	Если Не ПрименяетсяУСНДоходыРасходы
...
Процедура УстановитьУсловноеОформлениеРасшифровкаПлатежа(Форма) Экспорт
	...
	// РасшифровкаПлатежаРасходыУСН, РасшифровкаПлатежаЗаголовокРасходыУСН
