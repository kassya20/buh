...
Функция ВидНалогаПоСчетуУчета(СчетУчета, Организация = Неопределено, Знач Период = Неопределено, УровеньБюджета = Неопределено) Экспорт
	...
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		...
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.НДС
			...
			Или СчетУчета = ПланыСчетов.Хозрасчетный.НДСНалоговогоАгентаПоОтдельнымВидамТоваров
		...
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.НДФЛ Тогда
			...
			ВидНалога = Перечисления.ВидыНалогов.НДФЛ;
		...
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.ПФР_ОПС Тогда
			...
			Если Период >= УчетЗарплаты.ДатаПереходаНаЕдиныйТарифСтраховыхВзносов() Тогда
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ОПС_ИностранныеРаботники;
			Иначе
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ПФР_СтраховаяЧасть;
			КонецЕсли;
			
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.ФФОМС Тогда
			
			Если Период >= УчетЗарплаты.ДатаПереходаНаЕдиныйТарифСтраховыхВзносов() Тогда
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ОМС_ИностранныеРаботники;
			Иначе
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ФФОМС;
			КонецЕсли;
			
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.ФСС Тогда
			
			Если Период >= УчетЗарплаты.ДатаПереходаНаЕдиныйТарифСтраховыхВзносов() Тогда
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ОСС_ИностранныеРаботники;
			Иначе
				ВидНалога = Перечисления.ВидыНалогов.СтраховыеВзносы_ФСС;
			КонецЕсли;
		...
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.НалогНаИмущество Тогда
			...
			ВидНалога = Перечисления.ВидыНалогов.НалогНаИмущество;
			
		ИначеЕсли СчетУчета = ПланыСчетов.Хозрасчетный.ЗемельныйНалог Тогда
			
			ВидНалога = Перечисления.ВидыНалогов.ЗемельныйНалог_ВнутригородскойОкруг;
...
Функция ВидНалогаПоКодуЗадачи(КодЗадачи, Организация, Знач Период = Неопределено) Экспорт
	...
	ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
		...
		КонецЕсли;
	ИначеЕсли КодЗадачи = "СтраховыеВзносыАУСН" Тогда
...
Функция НалоговыйАгент(ВидНалога) Экспорт
	...
	Если Справочники.ВидыНалоговИПлатежейВБюджет.ЭтоНДФЛНАлоговогоАгента(ВидНалога) Тогда
...
Функция НеобходимыеВидыНалогов() Экспорт
	...
	Если УчетЗарплаты.ВключенаПодсистемаУчетаЗарплатыИКадров() Тогда
		...
		Если ИзмененныеОпцииПодсистемы.ИспользуетсяТрудЧленовЛетныхЭкипажей Или УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
			ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ДополнительныеВзносы_ПФР_ЛетныеЭкипажи);
		КонецЕсли;
		Если ИзмененныеОпцииПодсистемы.ИспользуетсяТрудШахтеров Или УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
			ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ДополнительныеВзносы_ПФР_Шахтеры);
		КонецЕсли;
		Если ИзмененныеОпцииПодсистемы.ИспользуютсяРаботыСДосрочнойПенсией Или УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
		...
		КонецЕсли;
		Если ИзмененныеОпцииПодсистемы.УдерживаютсяДобровольныеВзносыВПФР Или УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
			ВидыНалогов.Добавить(Перечисления.ВидыНалогов.СтраховыеВзносы_ПФР_Добровольные);
		КонецЕсли;
		Если ИзмененныеОпцииПодсистемы.ИспользуетсяТрудИностранцевСОсобымиВзносами Или УчетЗарплатыИКадровВоВнешнейПрограмме Тогда
	...
	Если ИспользуемыеСистемыНалогообложения.УплачиваетсяЕдиныйНалоговыйПлатеж Тогда
	...
	Если ИспользуемыеСистемыНалогообложения.ИспользуетсяОСНО
		Или ИспользуемыеСистемыНалогообложения.ИспользуетсяНДФЛИП
		Или ИспользуемыеСистемыНалогообложения.ИспользуетсяУСНСНДС Тогда
	...
	Если ИспользуемыеСистемыНалогообложения.ИспользуетсяАУСН Тогда
	...
	Если ИспользуемыеСистемыНалогообложения.ИспользуетсяНалогНаПрофессиональныйДоход Тогда
		...
		Если УчетнаяПолитика.ЕстьСамозанятыеПлательщикиДобровольныхВзносовПФР() Тогда
	...
	Если Константы.ВестиУчетИндивидуальногоПредпринимателя.Получить() Тогда
		...
		Если РегистрыСведений.НастройкиУчетаСтраховыхВзносовИП_ПФР_ФФОМС.УплачиваютсяСтраховыеВзносы() Тогда
	...
	Если Константы.ВестиУчетЮридическогоЛица.Получить() И Константы.ВедетсяУчетОсновныхСредств.Получить() Тогда
		...
		Если УсловияПримененияТребованийЗаконодательства.ЕстьИмуществоЕСГС() Тогда
			ВидыНалогов.Добавить(Перечисления.ВидыНалогов.НалогНаИмуществоЕСГС);
		КонецЕсли;
		
		Если РегистрыСведений.РегистрацияТранспортныхСредств.ЕстьТранспортныеСредства() Тогда
			ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ТранспортныйНалог);
		КонецЕсли;
		
		КатегорииПодчинения = УсловияПримененияТребованийЗаконодательства.КатегорииПодчиненияЗемельныхУчастковПоВидамНалогов();
		Для каждого КатегорияПодчинения Из КатегорииПодчинения Цикл
	...
	Если ИспользуемыеСистемыНалогообложения.ИспользуетсяТорговыйСбор Тогда
	...
	Если Константы.УплачиваетсяТуристическийНалог.Получить() Тогда
		ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ТуристическийНалог);
	КонецЕсли;
...
// Возвращает доступные виды налогов для ИБ
//
Функция ВидыНалогов() Экспорт
	...
	СписокДоступныхЗначений = Перечисления.ВидыНалогов.ПолучитьСписокДоступныхЗначений(Новый Структура);
	
	Для Индекс = 0 По КоллекцияМетаданных.Количество() - 1 Цикл
		ЗначениеПеречисления = Перечисления.ВидыНалогов.Получить(Индекс);
		Если СписокДоступныхЗначений.НайтиПоЗначению(ЗначениеПеречисления) <> Неопределено Тогда
			Результат.Вставить(КоллекцияМетаданных[Индекс].Имя, ЗначениеПеречисления);
		КонецЕсли;
	КонецЦикла;
...
Функция ВидыНалоговФизическихЛиц() Экспорт
	...
	ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ПСН_МуниципальныйОкруг);
	ВидыНалогов.Добавить(Перечисления.ВидыНалогов.НалогНаПрофессиональныйДоход);
	ВидыНалогов.Добавить(Перечисления.ВидыНалогов.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС);
...
// Возвращает признак того, что в ИБ ведется учет ИП
//
Функция ВедетсяУчетПредпринимателя() Экспорт
	
	ВедетсяУчетПредпринимателя = Константы.ВестиУчетИндивидуальногоПредпринимателя.Получить();
	
	Возврат ВедетсяУчетПредпринимателя;
	
КонецФункции

// Возвращает признак того, что в ИБ ведется учет ЮЛ м выплатой дивидендов
//
Функция ВедетсяУчетДивидендов() Экспорт
	
	Возврат УчетЗарплаты.ВедетсяУчетДивидендов();
	
КонецФункции

// Возвращает признак того, что в ИБ ведется учет РК и СН
//
Функция ВедетсяУчетРКиСН() Экспорт
	
	Возврат УчетЗарплаты.ВедетсяУчетРКиСН();
...
Функция ИзмененныеОпцииПодсистемыУчетаЗарплатыИКадров(ПроверяемыеОпцииПодсистемы = Неопределено) Экспорт
	...
	Если ПроверяемыеОпцииПодсистемы = Неопределено Тогда
		...
		ПроверяемыеОпцииПодсистемы.Вставить("ИспользуетсяТрудИностранцевСОсобымиВзносами", Ложь);
		ПроверяемыеОпцииПодсистемы.Вставить("ПрименятьСевернуюНадбавку",                   Ложь);
		ПроверяемыеОпцииПодсистемы.Вставить("ПрименятьРайонныйКоэффициент",                Ложь);
	КонецЕсли;
	...
	КонецЕсли;
	Если ПроверяемыеОпцииПодсистемы.ИспользуетсяТрудИностранцевСОсобымиВзносами Тогда
		ВключаемыеОпцииПодсистемы.Вставить("ИспользуетсяТрудИностранцевСОсобымиВзносами", Ложь);
	Иначе
		ВключаемыеОпцииПодсистемы.Вставить("ИспользуетсяТрудИностранцевСОсобымиВзносами", УчетЗарплаты.ИспользуетсяТрудИностранцевСОсобымиВзносами());
	КонецЕсли;
	Если ПроверяемыеОпцииПодсистемы.ПрименятьСевернуюНадбавку Тогда
		ВключаемыеОпцииПодсистемы.Вставить("ПрименятьСевернуюНадбавку", Ложь);
	Иначе
		ВключаемыеОпцииПодсистемы.Вставить("ПрименятьСевернуюНадбавку", УчетЗарплаты.ПрименятьСевернуюНадбавку());
	КонецЕсли;
	Если ПроверяемыеОпцииПодсистемы.ПрименятьРайонныйКоэффициент Тогда
		ВключаемыеОпцииПодсистемы.Вставить("ПрименятьРайонныйКоэффициент", Ложь);
	Иначе
		ВключаемыеОпцииПодсистемы.Вставить("ПрименятьРайонныйКоэффициент", УчетЗарплаты.ПрименятьРайонныйКоэффициент());
	КонецЕсли;
...
Процедура ПроверитьАктуальностьНалога(ДанныеОбъекта, РезультатПроверки) Экспорт
	...
	НеактуальныеНалоги = ПлатежиВБюджетКлиентСерверПереопределяемый.НеактуальныеНалоги(ДанныеОбъекта.Дата);
	НеактуальныйНалог = НеактуальныеНалоги[ДанныеНалога.ВидНалога];
	Если НеактуальныйНалог <> Неопределено Тогда
		ДатаАктуальностиКлассификатора = Справочники.ВидыНалоговИПлатежейВБюджет.ДатаАктуальностиКлассификатора();
		ДатаНеактуальности = НеактуальныйНалог.ДатаНеактуальности;
		НаименованиеНалога = ДанныеНалога.Наименование;
		АктуальныйНалог = Неопределено;
		Если НеактуальныйНалог.Свойство("АктуальныйНалог", АктуальныйНалог) Тогда
			НаименованиеАктуальногоНалога = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоВиду(АктуальныйНалог), "Наименование");
			ДатаНеактуальностиСтрокой = Формат(ДатаНеактуальности, "ДЛФ=DD");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не применяется с %2
				|Следует использовать %3.'"), НаименованиеНалога, ДатаНеактуальностиСтрокой, НаименованиеАктуальногоНалога);
		ИначеЕсли ДатаНеактуальности < НачалоГода(ДанныеОбъекта.Дата) Тогда
			ДатаНеактуальности = КонецГода(ДатаНеактуальности) + 1;
			ДатаНеактуальностиСтрокой = Формат(ДатаНеактуальности, "ДЛФ=DD");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 не применяется с %2'"), НаименованиеНалога, ДатаНеактуальностиСтрокой);
		КонецЕсли;
	КонецЕсли;
...
Функция ТаблицаСоответствияСчетовУчета()
	...
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.НДС, ПланыСчетов.Хозрасчетный.НДСНалоговогоАгентаПоОтдельнымВидамТоваров);
	...
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.АУСН, ПланыСчетов.Хозрасчетный.НалогПриАУСН);
	...
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.НалогНаПрофессиональныйДоход, ПланыСчетов.Хозрасчетный.НалогНаПрофессиональныйДоход);
	...
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.СтраховыеВзносыЕдиныйТариф,              ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф);
	
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.СтраховыеВзносы_ОПС_ИностранныеРаботники, ПланыСчетов.Хозрасчетный.ПФР_ОПС);
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.СтраховыеВзносы_ОМС_ИностранныеРаботники, ПланыСчетов.Хозрасчетный.ФФОМС);
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.СтраховыеВзносы_ОСС_ИностранныеРаботники, ПланыСчетов.Хозрасчетный.ФСС);
	
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.ФиксированныеВзносы_СтраховыеВзносыЕдиныйТариф,
		ПланыСчетов.Хозрасчетный.СтраховыеВзносыЕдиныйТариф_ИП);
	
	ДобавитьСчетУчета(
		ТаблицаЗначений,
		Перечисления.ВидыНалогов.ФиксированныеВзносы_ПФР_Добровольные,
		ПланыСчетов.Хозрасчетный.ПФР_ДОБР_орг);
	
	ДобавитьСчетУчета(
		ТаблицаЗначений,
		Перечисления.ВидыНалогов.СтраховыеВзносы_ПФР_Добровольные,
		ПланыСчетов.Хозрасчетный.ПФР_ДОБР_сотр);
	
	ДобавитьСчетУчета(
		ТаблицаЗначений,
		Перечисления.ВидыНалогов.ФиксированныеВзносы_СвышеПредела,
		ПланыСчетов.Хозрасчетный.ПФР_ОПС_ИП);
	...
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.ТуристическийНалог,                         ПланыСчетов.Хозрасчетный.ТуристическийНалог);
	
	ДобавитьСчетУчета(ТаблицаЗначений, Перечисления.ВидыНалогов.ЕдиныйНалоговыйПлатеж, ПланыСчетов.Хозрасчетный.ЕдиныйНалоговыйСчет);
	
	ДобавитьСчетУчета(
		ТаблицаЗначений,
		Перечисления.ВидыНалогов.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС,
		ПланыСчетов.Хозрасчетный.ОбеспечительныйПлатежПриВвозеТоваровИзЕАЭС);
