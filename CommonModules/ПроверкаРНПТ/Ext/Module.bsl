...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверитьРНПТВДокументе", Истина);
	Методы.Вставить("ПолучитьСписокДокументовСОшибками", Истина);
	Методы.Вставить("ПроверитьРНПТРегламентноеЗадание", Истина);
...
Функция ПроверкаРНПТВключена() Экспорт
	...
	Возврат (ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров") И ЗначениеКонстантыИспользоватьПроверкуРНПТ());
...
Процедура ЗаполнитьРезультатПроверкиРНПТВФормеДокумента(Форма, ДанныеРНПТ = Неопределено, ПриОткрытии = Ложь)
	...
	Для Каждого ТабличнаяЧастьНаименование Из ТабличныеЧастиДокументаСПрослеживаемостью Цикл 
		Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект[ТабличнаяЧастьНаименование] Цикл
			Если ТабличнаяЧастьНаименование = "Услуги" И Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.РНПТ) Тогда
				СтрокаТабличнойЧасти.СостояниеПроверкиРНПТ = 4;
				Продолжить;
				
			ИначеЕсли НЕ ТабличнаяЧастьНаименование = "Услуги"
					И (НЕ СтрокаТабличнойЧасти.ПрослеживаемыйТовар ИЛИ Не ЗначениеЗаполнено(СтрокаТабличнойЧасти.РНПТ)) Тогда
				СтрокаТабличнойЧасти.СостояниеПроверкиРНПТ = 4;
				Продолжить;
