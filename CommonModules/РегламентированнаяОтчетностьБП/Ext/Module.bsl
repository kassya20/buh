...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если СписокПоказателей = Неопределено Тогда
		...
		СписокПоказателей.Добавить("КодКатегорииСтрахователя_ФизическогоЛица");
		...
		СписокПоказателей.Добавить("ПериодичностьПредставленияДекларацииПоНалогуНаПрибыль");
		СписокПоказателей.Добавить("ЭтоБюджетополучатель");
		СписокПоказателей.Добавить("ЭтоФизическоеЛицо");
		СписокПоказателей.Добавить("ДатаЗакрытия");
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "ПериодичностьПредставленияДекларацииПоНалогуНаПрибыль" Тогда
			
			Значение = НалоговыйУчет.ПериодичностьОтчетногоПериода(ДатаЗначения, Организация.Ссылка);
		...
		ИначеЕсли ИмяПоказателя = "РегНомерСФР" Тогда
			...
			РегНомПФР = УчетЗарплаты.РегистрационныйНомерПФР(Организация.Ссылка, ДатаЗначения);
			РегНомСФР = УчетЗарплаты.РегистрационныйНомерСФР(Организация.Ссылка, ДатаЗначения);
			Если Не ЗначениеЗаполнено(РегНомПФР)
				Или УчетЗарплаты.ИспользованиеРегистрационногоНомераСФР() Тогда
				Значение = СокрЛП(РегНомСФР);
			Иначе
				Значение = "";
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "РегНомерСФРЗаСебя" Тогда
			...
			РегНомСФР = УчетЗарплаты.РегистрационныйНомерСФР(Организация.Ссылка, ДатаЗначения);
			Если ПБОЮЛ 
					И (Не ЗначениеЗаполнено(РегНомПФР) 
					Или УчетЗарплаты.ИспользованиеРегистрационногоНомераСФР()) Тогда
				Значение = СокрЛП(РегНомСФР); 
			Иначе
				Значение = "";
		...
		ИначеЕсли ИмяПоказателя = "РегНомерСФРГоловнойОрганизации" Тогда
			...
			Если Организация.ГоловнаяОрганизация <> Организация.Ссылка Тогда
				РегНомПФР = УчетЗарплаты.РегистрационныйНомерПФР(Организация.ГоловнаяОрганизация, ДатаЗначения);
				РегНомСФР = УчетЗарплаты.РегистрационныйНомерСФР(Организация.ГоловнаяОрганизация, ДатаЗначения);
			Иначе
				РегНомПФР = УчетЗарплаты.РегистрационныйНомерПФР(Организация.Ссылка, ДатаЗначения);
				РегНомСФР = УчетЗарплаты.РегистрационныйНомерСФР(Организация.Ссылка, ДатаЗначения);
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "КодКатегорииСтрахователя_ФизическогоЛица" Тогда
			
			Значение = "";
			
			Если ПБОЮЛ Тогда
				КатегорияСтрахователя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация.Ссылка, "КатегорияСтрахователяФизическогоЛица");
				Если ЗначениеЗаполнено(КатегорияСтрахователя) Тогда
					Значение = Метаданные.Перечисления.КатегорииСтрахователейФизическихЛицСФР.ЗначенияПеречисления.Получить(
						Перечисления.КатегорииСтрахователейФизическихЛицСФР.Индекс(КатегорияСтрахователя)).Имя;
				КонецЕсли;
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "РегистрационныйНомерФСС" Тогда
			...
			Значение = Лев(СокрЛП(Организация.РегистрационныйНомерФСС), 10);
		...
		ИначеЕсли ИмяПоказателя = "НомерСтрахователяФСС" Тогда
			...
			Если ЭтоГоловнаяОрганизация Или ДатаЗначения < '20150121' Тогда
				Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо
					Или УчетЗарплаты.ИПИспользуетТрудНаемныхРаботников(Организация) Тогда
		...
		ИначеЕсли ИмяПоказателя = "НаимГоловнОрг" Тогда
			...
			Если Организация.ГоловнаяОрганизация = Организация.Ссылка Тогда
				Значение = "";
			Иначе
				Значение = Справочники.Организации.НаименованияНаДату(Организация.ГоловнаяОрганизация, ДатаЗначения).ПолноеНаименование;
			КонецЕсли;
			...
			Если УчетныеПолитики.Количество() > 0 Тогда
				...
				Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
					...
					ИначеЕсли ДействующаяУчетнаяПолитика.СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная Тогда
						Значение = "УСН";
					ИначеЕсли ДействующаяУчетнаяПолитика.СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН Тогда
						Значение = "АУСН";
					КонецЕсли;
				...
				ИначеЕсли Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
					...
					ИначеЕсли ДействующаяУчетнаяПолитика.СистемаНалогообложения = Перечисления.СистемыНалогообложения.АУСН Тогда
						Значение = "АУСН";
					ИначеЕсли  ДействующаяУчетнаяПолитика.СистемаНалогообложения = Перечисления.СистемыНалогообложения.НалогНаПрофессиональныйДоход Тогда
						Значение = "НПД";
		...
		ИначеЕсли ИмяПоказателя = "ЭтоФизическоеЛицо" Тогда
			
			СтатусФизическогоЛица = Справочники.Организации.СтатусФизическогоЛицаНаДату(Организация.Ссылка, ДатаЗначения);
			
			Значение = СтатусФизическогоЛица = Перечисления.СтатусыФизическихЛиц.Самозанятый;
			
		ИначеЕсли ИмяПоказателя = "ДатаЗакрытия" Тогда
			
			Значение = Организация.ДатаЗакрытия;
	...
	УчетОбособленныхПодразделений.ПолучитьСведенияОбОрганизации(ОргСведения, Организация, ДатаЗначения, СписокПоказателей);
...
Функция РеквизитыОбязательныеДляОтчета(ИмяОтчета, Организация, ПериодОтчета) Экспорт
	...
	Если ИмяОтчета = "РегламентированныйОтчет3НДФЛ" Или ИмяОтчета = "РегламентированныйОтчет3_НДФЛ" Тогда
...
Функция СоздатьОбновитьРегламентированныйОтчет(Организация, ИсточникОтчета, ДатаНачала, ДатаОкончания, Ошибки = Неопределено) Экспорт
	
	Если Не МожноСоздаватьОтчет(Организация, ИсточникОтчета, ДатаОкончания, Ошибки) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция Используется1СОтчетность() Экспорт
	...
	// В режиме интеграции с банком 1С:Отчетность используется всегда
	Возврат Истина;
	
КонецФункции

Функция Подключена1СОтчетность(Организация) Экспорт
	
	Подключена1СОтчетность = ИнтерфейсыВзаимодействияБРО.ОрганизацияИмеетУчетнуюЗапись(Организация);
	Возврат Подключена1СОтчетность;
	
КонецФункции

Функция ДоступнаСтатистическаяОтчетностьБанкРоссии() Экспорт
	
	ПодсистемаСтатистическаяОтчетность =
		Метаданные.Подсистемы.РегламентированнаяОтчетность.Подсистемы.СтатистическаяОтчетность;
	Возврат ПодсистемаСтатистическаяОтчетность.Подсистемы.Найти("ОтчетностьВБанкРоссии") <> Неопределено;
	
КонецФункции

// Возвращает действующее полное имя регламентированного отчета
//
// Параметры:
//   ИмяОтчета - Строка - Имя регламентированного отчета, например, РегламентированноеУведомлениеФормаР11001
//   Период    - Дата - Дата, на которую нужно получить действующую форму отчета
//
// Возвращаемое значение:
//   Строка - Например, Отчет.РегламентированноеУведомлениеФормаР11001.Форма.Форма2020_4
//
Функция ПолноеИмяФормыПоУмолчанию(ИмяОтчета, Знач Период = Неопределено) Экспорт
	...
	Для Каждого Строка Из ДействующиеФормы Цикл
		Если Не ЗначениеЗаполнено(Строка.ДатаКонца) Или ТекущаяДатаПользователя <= Строка.ДатаКонца Тогда
			Результат = Строка.ПутьФормы;
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		// Форма отчета одна, но еще не вступила в силу
		Результат = ДействующиеФормы[0].ПутьФормы;
	КонецЕсли;
...
Функция МожноСоздаватьОтчет(Организация, ИсточникОтчета, Период, Ошибки = Неопределено)
	
	ДатаПроверки = КонецГода(Период);
	
	// Дополнительная проверка требуется только перед созданием декларации УСН начиная с периода,
	// в котором появилось требование заполнять обоснование льготной ставки налога в декларации.
	Если ИсточникОтчета <> "РегламентированныйОтчетУСН"
		Или Не УчетУСНКлиентСервер.ТребуетсяОснованиеЛьготнойСтавкиНалога(ДатаПроверки) Тогда
		Возврат Истина;
	КонецЕсли;
	
	ПараметрыЛьготнойСтавки = УчетУСН.ПараметрыПримененияЛьготнойСтавкиНалога(Организация, ДатаПроверки);
	
	Если ПараметрыЛьготнойСтавки.ПрименяласьЛьготнаяСтавка
		И ПустаяСтрока(ПараметрыЛьготнойСтавки.ОснованиеЛьготнойСтавки) Тогда
		
		Если Ошибки = Неопределено Тогда
			Ошибки = Новый Массив;
		КонецЕсли;
		
		ТекстСообщения = НСтр("ru = 'Не заполнено основание льготной ставки налога УСН. Перейдите в 1С и укажите в настройках норму закона субъекта РФ, на основании которого применяется льготная ставка.'");
...
Функция ДействующиеФормы(ИмяОтчета, Знач Период)
	...
	ТаблицаФормОтчета.Сортировать("ДатаНачала Убыв");
	Для Каждого Строка Из ТаблицаФормОтчета Цикл
		Если (Строка.ДатаНачала <= Период) И (Период <= Строка.ДатаКонца Или Не ЗначениеЗаполнено(Строка.ДатаКонца)) Тогда
