...
Функция РассчитатьСебестоимость(Период, Организация, Ссылка, РегламентнаяОперация) Экспорт
	...
	Настройки = НастройкиУчетаЗатрат(Период, Организация, РегламентнаяОперация);
	...
	Протокол = НовыйПротокол(ОписаниеСумм, ВариантыСтруктурыЗатрат, Настройки);
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	СоздатьСчетаЗатрат(МенеджерВременныхТаблиц, Период, Организация, Режим, Настройки);
	...
	СоздатьНачальноеСальдо(МенеджерВременныхТаблиц, Период, Организация, РаспределитьРасходыНаПроизводство, ВариантыСтруктурыЗатрат);
	
	// При получении начального сальдо в случае, если НЗП ведется не с детализацией до продукции, субконто "Продукция"
	// было определено как оборотное (см. СоздатьСчетаЗатрат()). Далее такая замена становится ненужной.
	ПереопределитьОборотныеСубконто(МенеджерВременныхТаблиц);
	
	// Особые виды движений: ОборотыФикс, ПредварительныеПроводкиВыпуск, ФиксированнаяСтоимость, Пересортица
	...
	СоздатьПересортица(МенеджерВременныхТаблиц, Период, Организация);
	СоздатьПересортицаИзлишкиНУ(МенеджерВременныхТаблиц, Период, Организация);
	...
	// Существуют два варианта оценки незавершенного производства - с суммовой и количественной оценкой.
	// Для суммовой оценки используется вспомогательная таблица графа затрат ЦелевоеСальдо, которая заполняется
	// после применения правил распределения и создания графа затрат. Исключение сумм целевого сальдо происходит
	// в процессе основного алгоритма оценки затрат. В процедуре СоздатьНезавершенноеПроизводство() будут
	// получены данные документов ИнвентаризацияНЗП для заполнения таблицы ЦелевоеСальдо.
	// Для количественной оценки незавершенного производства используются правила распределения, данные для расчета
	// будут получены с процессе создания схемы распределения, по аналогии с остальными затратами.
	// Алгоритм количественной оценки использует данные в разрезе статей затрат, в том числе и остатков затрат предыдущего месяца.
	// Поскольку в регистре бухгалтерии не хранятся остатки в разрезе статей затрат, то для получения информации о статьях затрат
	// предыдущего месяца используются данные регистра сведений РасчетКалькуляцииСебестоимости. Дополнение таблицы начального сальдо
	// статьями затрат происходит в процедуре СоздатьНачальноеСальдо().
	...
	// Обновляет:
	// КлючиВершин, ИдентификаторыВершин
	// Использует:
	// Поступления
	...
	ВариантИспользользованияРазниц =
		БухгалтерскийУчет.ИспользоватьСуммыНалогНаПрибыль(Организация, Период, БухгалтерскийУчетВызовСервераПовтИсп.ПользователюДоступныСуммыНалогНаПрибыль());
	Затраты.КешПараметровРасчета.ИспользоватьСуммыРазницПБУ18 = БухгалтерскийУчетКлиентСервер.ИспользоватьСуммыРазниц(ВариантИспользользованияРазниц);
	...
	Если РаспределитьРасходыНаПроизводство Тогда
		...
		ПрименитьНастройкуПорядкаПеределов(Затраты, МенеджерВременныхТаблиц, Настройки);
		
		// - разрушим встречный выпуск
		ПрименитьНастройкуВстречногоВыпуска(Затраты, МенеджерВременныхТаблиц, Настройки);
	...
	Если РаспределитьРасходыНаПроизводство ИЛИ СписатьРасходыНаПродажу Тогда
		...
		Если РаспределитьРасходыНаПроизводство Тогда
			...
			Результат.Вставить("РасчетБазыРаспределенияЗатратЭквивалентныеЕдиницы", Протокол.БазаРаспределенияВЭквивалентныхЕдиницах);
			...
			РасчетНачальногоСальдоПрямыхЗатрат = ПодготовитьСправкаРасчетНачальногоСальдоПрямыхЗатрат(МенеджерВременныхТаблиц);
			Результат.Вставить("РасчетНачальногоСальдоПрямыхЗатрат", РасчетНачальногоСальдоПрямыхЗатрат);
...
// Проверяет, нужно ли в данном месяце по данной организации выполнять регламентную операцию. Если результат проверки
// для различных филиалов отличается, возвращает список филиалов, требующих выполнение регламентной операции.
//
// Параметры:
//  Период - Дата - любая дата, относящаяся к проверяемому месяцу
//  Организация - СправочникСсылка.Организации
//  РегламентнаяОперация - ПеречислениеСсылка.ВидыРегламентныхОпераций
// 
// Возвращаемое значение:
//  Булево, Массив из СправочникСсылка.Организации
//
Функция ТребуетсяРасчет(Период, Организация, РегламентнаяОперация) Экспорт
	
	ВсяОрганизация = Справочники.Организации.ВсяОрганизация(Организация, Период);
	СписокОрганизацийТребуетсяРасчет = Новый Массив;
	ТребуетсяРасчет = Ложь;
	
	Для Каждого ПроверяемаяОрганизация Из ВсяОрганизация Цикл
		
		Настройки = НастройкиУчетаЗатрат(Период, ПроверяемаяОрганизация, РегламентнаяОперация);
		Если ЗначениеЗаполнено(Настройки.СчетаУчета) Тогда
			ТребуетсяРасчет = Истина;
			СписокОрганизацийТребуетсяРасчет.Добавить(ПроверяемаяОрганизация);
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не ТребуетсяРасчет Или ВсяОрганизация.Количество() = СписокОрганизацийТребуетсяРасчет.Количество() Тогда
		Возврат ТребуетсяРасчет;
	Иначе
		Возврат СписокОрганизацийТребуетсяРасчет;
	КонецЕсли;
...
Функция ОписаниеГрафа(Затраты, ПравилаРаспределения) Экспорт
	...
	Для Каждого Ребро Из Затраты.Ребра Цикл
		...
		ИначеЕсли Ребро.ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.Выпуск Тогда
			ВидСтрелки = "open";
		ИначеЕсли Ребро.ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие Тогда
			ВидСтрелки = "icurve";
		ИначеЕсли Ребро.ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.ПересортицаПоступление Тогда
			ВидСтрелки = "curve";
		КонецЕсли;
...
Функция НастройкиУчетаЗатрат(Период, Организация, РегламентнаяОперация)
	
	Если РегламентнаяОперация = Перечисления.ВидыРегламентныхОпераций.КорректировкаСтоимостиНоменклатуры Тогда
		Возврат Справочники.НастройкиУчетаЗатрат.НовыйИсполняемыеНастройкиУчетаЗатрат();
	КонецЕсли;
	
	Настройки = Справочники.НастройкиУчетаЗатрат.ИсполняемыеНастройки(Период, Организация);
	
	ОграничитьНастройкиУчетаЗатратКонтекстом(Настройки, Период, Организация, РегламентнаяОперация);
	
	Возврат Настройки;
	
КонецФункции

Процедура ОграничитьНастройкиУчетаЗатратКонтекстом(Настройки, Период, Организация, РегламентнаяОперация = Неопределено)
	...
	СчетаРасходов = БухгалтерскийУчет.СформироватьМассивСубсчетов(СчетаРасходов);
	
	КоличествоСчетов = Настройки.СчетаУчета.Количество();
	Для НомерСКонца = 1 По КоличествоСчетов Цикл
		
		ИндексСчета = КоличествоСчетов - НомерСКонца;
...
// СчетаРасходов
// СубконтоЗатрат
Процедура СоздатьСчетаЗатрат(МенеджерВременныхТаблиц, Период, Организация, Режим, Настройки = Неопределено)
	...
	Если ЗначениеЗаполнено(Настройки) Тогда
		...
		СоздатьОборотныеСубконто(МенеджерВременныхТаблиц, Настройки);
		
		СоздатьСчетаНезавершенноеПроизводствоПоПродукции(МенеджерВременныхТаблиц, Настройки);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура СоздатьСчетаНезавершенноеПроизводствоПоПродукции(МенеджерВременныхТаблиц, Настройки)

	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	СчетаНезавершенноеПроизводствоПоПродукции =
		Справочники.НастройкиУчетаЗатрат.СчетаВедетсяНезавершенноеПроизводствоПоПродукции(Настройки);
		
	Запрос.УстановитьПараметр("СчетаНезавершенноеПроизводствоПоПродукции", СчетаНезавершенноеПроизводствоПоПродукции);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СчетаРасходов.Счет КАК Счет
	|ПОМЕСТИТЬ СчетаНезавершенноеПроизводствоПоПродукции
	|ИЗ
	|	СчетаРасходов КАК СчетаРасходов
	|ГДЕ
	|	СчетаРасходов.Счет В (&СчетаНезавершенноеПроизводствоПоПродукции)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет";
...
Процедура СоздатьСчетаЗапасов(МенеджерВременныхТаблиц, Период, Организация, Пустые = Ложь)
	...
	Иначе
		...
		СчетаЗапасовИсключения.Добавить(ПланыСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов);     // 10.21.2 - технический, регулирующий, суммы всегда совпадают с 10.21.1
...
Процедура СоздатьСчетаРасходов(МенеджерВременныхТаблиц, Режим = "РасходыНаПроизводство")
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВложенияВРемонтыОсновныхСредств),
	|	""Выпуск""
	|ГДЕ
	|	&ДобавитьСчетВыпуска
	|
...
Процедура СоздатьСубконтоЗатрат(МенеджерВременныхТаблиц)
	...
	|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ХозрасчетныйВидыСубконто
	|ГДЕ
	|	ХозрасчетныйВидыСубконто.Ссылка = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ЗатратыНаРемонтОС)
	|	И ХозрасчетныйВидыСубконто.Суммовой
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.ТипЗначения) = Тип("ОписаниеТипов") Тогда
...
Процедура СоздатьОборотныеСубконто(МенеджерВременныхТаблиц, Настройки)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	Запрос.УстановитьПараметр("СчетаИгнорироватьАналитикуПродукцияНачальногоСальдо",
		Справочники.НастройкиУчетаЗатрат.СчетаИгнорироватьАналитикуПродукцияНачальногоСальдо(Настройки));
	...
	|			ИЛИ ХозрасчетныйВидыСубконто.Ссылка В (&СчетаИгнорироватьАналитикуПродукцияНачальногоСальдо)
...
// Создает временные таблицы, содержащие данные о начальном сальдо
//
Процедура СоздатьНачальноеСальдо(МенеджерВременныхТаблиц, Период, Организация, РаспределитьРасходыНаПроизводство, ВариантыСтруктурыЗатрат)
	...
	// Для учета малоценных объектов используется пара счетов 10.21.1 (Активный) и 10.21.2 (Пассивный).
	// Суммы на счете 10.21.2 равны суммам на 10.21.1, поэтому они не важны для целей учета затрат.
	...
	// Поэтому в контуре учета затрат на 10.21.1 учитываются только суммы налогового учета,
	// а для бухгалтерского используется 10.21.2
	// При формировании проводок он будет обратно заменен на 10.21.1.
	
	// Временная таблица НачальноеСальдоПрямыхРасходов может быть пересоздана (дополнена статьями затрат
	// из регистра сведений РасчетКалькуляцииСебестоимости), если среди счетов затрат есть
	// счета с количественной оценкой незавершенного производства.
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	...
	|	КОНЕЦ КАК Субконто3,
	|	СУММА(НачальноеСальдо.КоличествоОстаток) КАК Количество,
	|	СУММА(НачальноеСальдо.СуммаОстаток) КАК Сумма,
	|	СУММА(НачальноеСальдо.СуммаНУОстаток) КАК СуммаНУ,
	|	СУММА(НачальноеСальдо.СуммаПРОстаток) КАК СуммаПР,
	|	СУММА(НачальноеСальдо.СуммаВРОстаток) КАК СуммаВР,
	|	ЛОЖЬ КАК НезавершенноеПроизводствоПоПродукции
	|ПОМЕСТИТЬ НачальноеСальдоПрямыхРасходов
	|ИЗ
	...
	// Для расчета стоимости незавершенного производства с количественной оценкой необходима информация о начальном
	// сальдо продукции в разрезе статей затрат. Это нужно для классификации сальдо затрат на материальные затраты и затраты на обработку.
	// При этом в регистре бухгалтерии такой информации нет: субконто "Статьи затрат" - это оборотное субконто, в отличие от субконто "Продукция",
	// если включена количественная оценка НЗП.
	// Добавим данные по статьям затрат в таблицу НачальноеСальдоПрямыхРасходов из регистра сведений РасчетКалькуляцииСебестоимости.
	// При этом следует учитывать, что данные в регистре бухгалтерии и в регистре сведений
	// могут быть рассинхронизированы, поскольку для расчета данных для регистра сведений используется отдельный алгоритм (см., например,
	// СтруктураЗатрат.ДополнитьГрафЗатрат, СтруктураЗатрат.РассчитатьДолю, СтруктураЗатрат.УстановитьОсновныеЭлементы). Если суммы БУ и НУ
	// в регистре сведений окажутся меньше, чем в регистре бухгалтерии, отнесем их на пустую статью затрат. Фактически такие затраты будут считаться
	// затратами на обработку. Если суммы в регистре сведений окажутся больше, то распределим остатки по продукции в регистре бухгалтерии
	// пропорционально, а погрешность округления будем считать затратами на обработку (отнесем на пустую статью затрат). Чтобы начальное сальдо
	// по статьям затрат не имело отрицатальных сумм там, где их изначально не было, при распределении будем использовать округление в меньшую сторону.
	// Для этого используется конструкция ЦЕЛ(СуммаПропорции * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле: берем целую часть суммы в копейках,
	// а затем переводим обратно в рубли.
	// Сумма ПР регистра бухгалтерии при этом распределяется на суммы ПР регистра сведений только "своего" знака. Это нужно, чтобы по сумма ПР
	// для каждой из статей затрат по модулю не стала больше остатка в регистре бухгалтерии. А сумма ВР тогда получается расчетным путем,
	// по формуле СуммаВР = СуммаБУ - СуммаНУ - СуммаПР.
	// Количество материала делится пропорционально СуммаБУ. Нераспределенное количество так же относим на пустую статью затрат.
	// Если же суммы в обоих регистрах синхронизированы (сумма остатков НЗП по продукции совпадает), то суммы и количество материала по статьям затрат
	// берем из регистра сведений без каких-либо преобразований и округлений.
	
	Если ИспользуетсяВнешнийИсточникСтатьиКалькуляцииНачальноеСальдо(ВариантыСтруктурыЗатрат) Тогда
		
		ВидИспользованияСумм = БухгалтерскийУчет.ИспользоватьСуммыНалогНаПрибыль(Организация, Период);
		Запрос.УстановитьПараметр("ИспользоватьСуммыРазниц", БухгалтерскийУчетКлиентСервер.ИспользоватьСуммыРазниц(ВидИспользованияСумм));
		Запрос.УстановитьПараметр("КоличествоКопеекВРубле", Pow(10, БухгалтерскийУчетКлиентСервер.РазрядностьДробнойЧастиСумм()));
		Запрос.УстановитьПараметр("РазрядностьКоличества",
			Pow(10, Метаданные.РегистрыСведений.РасчетКалькуляцииСебестоимости.Ресурсы.КоличествоМатериала.Тип.КвалификаторыЧисла.РазрядностьДробнойЧасти));
			
		Запрос.Текст =
		"ВЫБРАТЬ
		|	СтатьиКалькуляцииНачальноеСальдо.Счет КАК Счет,
		|	СтатьиКалькуляцииНачальноеСальдо.Подразделение КАК Подразделение,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто1 КАК Субконто1,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто2 КАК Субконто2,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто3 КАК Субконто3,
		|	ВЫБОР
		|		КОГДА ОборотныеСубконто.Субконто1
		|			ТОГДА ОборотныеСубконто.ЗначениеПоУмолчанию1
		|		ИНАЧЕ ЕСТЬNULL(СтатьиКалькуляцииНачальноеСальдо.Субконто1, НЕОПРЕДЕЛЕНО)
		|	КОНЕЦ КАК АналитикаРегистраБухгалтерииСубконто1,
		|	ВЫБОР
		|		КОГДА ОборотныеСубконто.Субконто2
		|			ТОГДА ОборотныеСубконто.ЗначениеПоУмолчанию2
		|		ИНАЧЕ ЕСТЬNULL(СтатьиКалькуляцииНачальноеСальдо.Субконто2, НЕОПРЕДЕЛЕНО)
		|	КОНЕЦ КАК АналитикаРегистраБухгалтерииСубконто2,
		|	ВЫБОР
		|		КОГДА ОборотныеСубконто.Субконто3
		|			ТОГДА ОборотныеСубконто.ЗначениеПоУмолчанию3
		|		ИНАЧЕ ЕСТЬNULL(СтатьиКалькуляцииНачальноеСальдо.Субконто3, НЕОПРЕДЕЛЕНО)
		|	КОНЕЦ КАК АналитикаРегистраБухгалтерииСубконто3,
		|	ИСТИНА КАК ДетальнаяЗаписьНачальногоСальдоПоСтатьямЗатрат,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.Сумма) КАК Сумма,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаНУ) КАК СуммаНУ,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаПР) КАК СуммаПР,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаВР) КАК СуммаВР
		|ПОМЕСТИТЬ ВТ_НачальноеСальдоПоОборотнымСубконто
		|ИЗ
		|	СтатьиКалькуляцииНачальноеСальдо КАК СтатьиКалькуляцииНачальноеСальдо
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаНезавершенноеПроизводствоПоПродукции КАК ОтборСчетов
		|		ПО СтатьиКалькуляцииНачальноеСальдо.Счет = ОтборСчетов.Счет
		|		ЛЕВОЕ СОЕДИНЕНИЕ ОборотныеСубконто КАК ОборотныеСубконто
		|		ПО СтатьиКалькуляцииНачальноеСальдо.Счет = ОборотныеСубконто.Счет
		|ГДЕ
		|	СтатьиКалькуляцииНачальноеСальдо.Сумма >= 0
		|	И СтатьиКалькуляцииНачальноеСальдо.СуммаНУ >= 0
		|
		|СГРУППИРОВАТЬ ПО
		|	СтатьиКалькуляцииНачальноеСальдо.Счет,
		|	СтатьиКалькуляцииНачальноеСальдо.Подразделение,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто1,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто2,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто3,
		|	ВЫБОР
		|		КОГДА ОборотныеСубконто.Субконто1
		|			ТОГДА ОборотныеСубконто.ЗначениеПоУмолчанию1
		|		ИНАЧЕ ЕСТЬNULL(СтатьиКалькуляцииНачальноеСальдо.Субконто1, НЕОПРЕДЕЛЕНО)
		...
		|	Счет,
		|	Подразделение,
		|	АналитикаРегистраБухгалтерииСубконто1,
		|	АналитикаРегистраБухгалтерииСубконто2,
		|	АналитикаРегистраБухгалтерииСубконто3
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НачальноеСальдоПрямыхРасходов.Счет КАК Счет,
		|	НачальноеСальдоПрямыхРасходов.Подразделение КАК Подразделение,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто1 КАК Субконто1,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто2 КАК Субконто2,
		|	СтатьиКалькуляцииНачальноеСальдо.Субконто3 КАК Субконто3,
		|	СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто1 КАК АналитикаРегистраБухгалтерииСубконто1,
		|	СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто2 КАК АналитикаРегистраБухгалтерииСубконто2,
		|	СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто3 КАК АналитикаРегистраБухгалтерииСубконто3,
		|	НачальноеСальдоПрямыхРасходов.Количество КАК ОстатокКоличество,
		|	НачальноеСальдоПрямыхРасходов.Сумма КАК СводнаяСумма,
		|	НачальноеСальдоПрямыхРасходов.СуммаНУ КАК СводнаяСуммаНУ,
		|	НачальноеСальдоПрямыхРасходов.СуммаПР КАК СводнаяСуммаПР,
		|	НачальноеСальдоПрямыхРасходов.СуммаВР КАК СводнаяСуммаВР,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.Сумма) КАК ДетальнаяСумма,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаНУ) КАК ДетальнаяСуммаНУ,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаПР) КАК ДетальнаяСуммаПР,
		|	СУММА(СтатьиКалькуляцииНачальноеСальдо.СуммаВР) КАК ДетальнаяСуммаВР,
		|	СУММА(ВЫБОР
		|			КОГДА НачальноеСальдоПрямыхРасходов.Сумма * СтатьиКалькуляцииНачальноеСальдо.СуммаПР >= 0
		|				ТОГДА СтатьиКалькуляцииНачальноеСальдо.СуммаПР
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК УчетЗнакаДетальнаяСуммаПР,
		|	ВЫБОР
		|		КОГДА СГРУППИРОВАНОПО(СтатьиКалькуляцииНачальноеСальдо.ДетальнаяЗаписьНачальногоСальдоПоСтатьямЗатрат)
		|			ТОГДА ""Детальная""
		|		ИНАЧЕ ""Сводная""
		|	КОНЕЦ КАК ТипЗаписи
		|ПОМЕСТИТЬ ВТ_ДетальныеСводныеЗаписи
		|ИЗ
		|	НачальноеСальдоПрямыхРасходов КАК НачальноеСальдоПрямыхРасходов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаНезавершенноеПроизводствоПоПродукции КАК СчетаНезавершенноеПроизводствоПоПродукции
		|		ПО НачальноеСальдоПрямыхРасходов.Счет = СчетаНезавершенноеПроизводствоПоПродукции.Счет
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НачальноеСальдоПоОборотнымСубконто КАК СтатьиКалькуляцииНачальноеСальдо
		|		ПО НачальноеСальдоПрямыхРасходов.Счет = СтатьиКалькуляцииНачальноеСальдо.Счет
		|			И НачальноеСальдоПрямыхРасходов.Подразделение = СтатьиКалькуляцииНачальноеСальдо.Подразделение
		|			И НачальноеСальдоПрямыхРасходов.Субконто1 = СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто1
		|			И НачальноеСальдоПрямыхРасходов.Субконто2 = СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто2
		|			И НачальноеСальдоПрямыхРасходов.Субконто3 = СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто3
		|ГДЕ
		|	НачальноеСальдоПрямыхРасходов.Количество >= 0
		|	И НачальноеСальдоПрямыхРасходов.Сумма >= 0
		|	И НачальноеСальдоПрямыхРасходов.СуммаНУ >= 0
		|
		|СГРУППИРОВАТЬ ПО ГРУППИРУЮЩИМ НАБОРАМ
		|(	
		|	(НачальноеСальдоПрямыхРасходов.Счет,
		|	 НачальноеСальдоПрямыхРасходов.Подразделение,
		|	 СтатьиКалькуляцииНачальноеСальдо.ДетальнаяЗаписьНачальногоСальдоПоСтатьямЗатрат,
		|	 НачальноеСальдоПрямыхРасходов.Количество,
		|	 НачальноеСальдоПрямыхРасходов.Сумма,
		|	 НачальноеСальдоПрямыхРасходов.СуммаВР,
		|	 НачальноеСальдоПрямыхРасходов.СуммаНУ,
		|	 НачальноеСальдоПрямыхРасходов.СуммаПР,
		|	 СтатьиКалькуляцииНачальноеСальдо.Субконто1,
		|	 СтатьиКалькуляцииНачальноеСальдо.Субконто2,
		|	 СтатьиКалькуляцииНачальноеСальдо.Субконто3,
		|	 СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто1,
		|	 СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто2,
		|	 СтатьиКалькуляцииНачальноеСальдо.АналитикаРегистраБухгалтерииСубконто3),
		|	(НачальноеСальдоПрямыхРасходов.Счет,
		|	 НачальноеСальдоПрямыхРасходов.Подразделение,
		|	 НачальноеСальдоПрямыхРасходов.Количество,
		|	 НачальноеСальдоПрямыхРасходов.Сумма,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДетальныеЗаписи.Счет КАК Счет,
		|	ДетальныеЗаписи.Подразделение КАК Подразделение,
		|	ДетальныеЗаписи.Субконто1 КАК Субконто1,
		|	ДетальныеЗаписи.Субконто2 КАК Субконто2,
		|	ДетальныеЗаписи.Субконто3 КАК Субконто3,
		|	ВЫБОР
		|		КОГДА СводныеЗаписи.ДетальнаяСумма <= СводныеЗаписи.СводнаяСумма
		|			ТОГДА ДетальныеЗаписи.ДетальнаяСумма
		|		ИНАЧЕ ЦЕЛ(ДетальныеЗаписи.ДетальнаяСумма * СводныеЗаписи.СводнаяСумма / СводныеЗаписи.ДетальнаяСумма * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|	КОНЕЦ КАК Сумма,
		|	ВЫБОР
		|		КОГДА СводныеЗаписи.ДетальнаяСуммаНУ <= СводныеЗаписи.СводнаяСуммаНУ
		|			ТОГДА ДетальныеЗаписи.ДетальнаяСуммаНУ
		|		ИНАЧЕ ЦЕЛ(ДетальныеЗаписи.ДетальнаяСуммаНУ * СводныеЗаписи.СводнаяСуммаНУ / СводныеЗаписи.ДетальнаяСуммаНУ * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|	КОНЕЦ КАК СуммаНУ,
		|	ВЫБОР
		|		КОГДА НЕ &ИспользоватьСуммыРазниц
		|			ТОГДА 0
		|		КОГДА СводныеЗаписи.ДетальнаяСуммаПР = ДетальныеЗаписи.СводнаяСуммаПР
		|			ТОГДА ДетальныеЗаписи.ДетальнаяСуммаПР
		|		КОГДА СводныеЗаписи.УчетЗнакаДетальнаяСуммаПР = 0
		|			ТОГДА 0
		|		КОГДА ДетальныеЗаписи.СводнаяСуммаПР > 0
		|			ТОГДА ЦЕЛ(ДетальныеЗаписи.УчетЗнакаДетальнаяСуммаПР * СводныеЗаписи.СводнаяСуммаПР / СводныеЗаписи.УчетЗнакаДетальнаяСуммаПР * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|		ИНАЧЕ -ЦЕЛ(-ДетальныеЗаписи.УчетЗнакаДетальнаяСуммаПР * СводныеЗаписи.СводнаяСуммаПР / СводныеЗаписи.УчетЗнакаДетальнаяСуммаПР * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|	КОНЕЦ КАК СуммаПР,
		|	ВЫБОР
		|		КОГДА НЕ &ИспользоватьСуммыРазниц
		|			ТОГДА 0
		|		КОГДА СводныеЗаписи.ДетальнаяСумма = СводныеЗаписи.СводнаяСумма
		|				И СводныеЗаписи.ДетальнаяСуммаНУ = СводныеЗаписи.СводнаяСуммаНУ
		|				И СводныеЗаписи.ДетальнаяСуммаПР = СводныеЗаписи.СводнаяСуммаПР
		|				И СводныеЗаписи.ДетальнаяСуммаВР = СводныеЗаписи.СводнаяСуммаВР
		|			ТОГДА ДетальныеЗаписи.ДетальнаяСуммаВР
		|		ИНАЧЕ ВЫБОР
		|				КОГДА СводныеЗаписи.ДетальнаяСумма <= СводныеЗаписи.СводнаяСумма
		|						ИЛИ СводныеЗаписи.ДетальнаяСумма = 0
		|					ТОГДА ДетальныеЗаписи.ДетальнаяСумма
		|				ИНАЧЕ ЦЕЛ(ДетальныеЗаписи.ДетальнаяСумма * СводныеЗаписи.СводнаяСумма / СводныеЗаписи.ДетальнаяСумма * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|			КОНЕЦ - ВЫБОР
		|				КОГДА СводныеЗаписи.ДетальнаяСуммаНУ <= СводныеЗаписи.СводнаяСуммаНУ
		|						ИЛИ СводныеЗаписи.ДетальнаяСуммаНУ = 0
		|					ТОГДА ДетальныеЗаписи.ДетальнаяСуммаНУ
		|				ИНАЧЕ ЦЕЛ(ДетальныеЗаписи.ДетальнаяСуммаНУ * СводныеЗаписи.СводнаяСуммаНУ / СводныеЗаписи.ДетальнаяСуммаНУ * &КоличествоКопеекВРубле) / &КоличествоКопеекВРубле
		|			КОНЕЦ - ВЫБОР
		|				КОГДА НЕ &ИспользоватьСуммыРазниц
		...
		|	ВЫБОР
		|		КОГДА СводныеЗаписи.ДетальнаяСумма <= СводныеЗаписи.СводнаяСумма
		|			ТОГДА ДетальныеЗаписи.ОстатокКоличество
		|		ИНАЧЕ ЦЕЛ(ДетальныеЗаписи.ОстатокКоличество * СводныеЗаписи.СводнаяСумма / СводныеЗаписи.ДетальнаяСумма * &РазрядностьКоличества) / &РазрядностьКоличества
		|	КОНЕЦ КАК Количество,
		|	ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто1 КАК АналитикаРегистраБухгалтерииСубконто1,
		|	ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто2 КАК АналитикаРегистраБухгалтерииСубконто2,
		|	ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто3 КАК АналитикаРегистраБухгалтерииСубконто3
		|ПОМЕСТИТЬ ВТ_РаспределенныеСуммы
		|ИЗ
		|	ВТ_ДетальныеСводныеЗаписи КАК ДетальныеЗаписи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДетальныеСводныеЗаписи КАК СводныеЗаписи
		|		ПО ДетальныеЗаписи.Счет = СводныеЗаписи.Счет
		|			И ДетальныеЗаписи.Подразделение = СводныеЗаписи.Подразделение
		|			И ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто1 = СводныеЗаписи.АналитикаРегистраБухгалтерииСубконто1
		|			И ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто2 = СводныеЗаписи.АналитикаРегистраБухгалтерииСубконто2
		|			И ДетальныеЗаписи.АналитикаРегистраБухгалтерииСубконто3 = СводныеЗаписи.АналитикаРегистраБухгалтерииСубконто3
		|			И (СводныеЗаписи.ТипЗаписи = ""Сводная"")
		|			И (ДетальныеЗаписи.ТипЗаписи = ""Детальная"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	НЕ СчетаНезавершенноеПроизводствоПоПродукции.Счет ЕСТЬ NULL КАК НезавершенноеПроизводствоПоПродукции
		|ПОМЕСТИТЬ ВТ_НачальноеСальдоПрямыхРасходов
		|ИЗ
		|	НачальноеСальдоПрямыхРасходов КАК ИсходныеДанные
		|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаНезавершенноеПроизводствоПоПродукции КАК СчетаНезавершенноеПроизводствоПоПродукции
		|		ПО ИсходныеДанные.Счет = СчетаНезавершенноеПроизводствоПоПродукции.Счет
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ИсходнаяАналитикаДетальныхДанных.Счет,
		|	ИсходнаяАналитикаДетальныхДанных.Подразделение,
		|	ИсходнаяАналитикаДетальныхДанных.АналитикаРегистраБухгалтерииСубконто1,
		|	ИсходнаяАналитикаДетальныхДанных.АналитикаРегистраБухгалтерииСубконто2,
		|	ИсходнаяАналитикаДетальныхДанных.АналитикаРегистраБухгалтерииСубконто3,
		|	-ИсходнаяАналитикаДетальныхДанных.Сумма,
		|	-ИсходнаяАналитикаДетальныхДанных.СуммаНУ,
		|	-ИсходнаяАналитикаДетальныхДанных.СуммаПР,
		|	-ИсходнаяАналитикаДетальныхДанных.СуммаВР,
		|	-ИсходнаяАналитикаДетальныхДанных.Количество,
		|	ИСТИНА
		|ИЗ
		|	ВТ_РаспределенныеСуммы КАК ИсходнаяАналитикаДетальныхДанных
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ДетальныеДанные.Счет,
		|	ДетальныеДанные.Подразделение,
		|	ДетальныеДанные.Субконто1,
		|	ДетальныеДанные.Субконто2,
		|	ДетальныеДанные.Субконто3,
		|	ДетальныеДанные.Сумма,
		|	ДетальныеДанные.СуммаНУ,
		|	ДетальныеДанные.СуммаПР,
		|	ДетальныеДанные.СуммаВР,
		|	ДетальныеДанные.Количество,
		|	ИСТИНА
		|ИЗ
		|	ВТ_РаспределенныеСуммы КАК ДетальныеДанные
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ НачальноеСальдоПрямыхРасходов
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НачальноеСальдоПрямыхРасходов.Счет КАК Счет,
		|	НачальноеСальдоПрямыхРасходов.Подразделение КАК Подразделение,
		|	НачальноеСальдоПрямыхРасходов.Субконто1 КАК Субконто1,
		|	НачальноеСальдоПрямыхРасходов.Субконто2 КАК Субконто2,
		|	НачальноеСальдоПрямыхРасходов.Субконто3 КАК Субконто3,
		|	СУММА(НачальноеСальдоПрямыхРасходов.Сумма) КАК Сумма,
		|	СУММА(НачальноеСальдоПрямыхРасходов.СуммаНУ) КАК СуммаНУ,
		|	СУММА(НачальноеСальдоПрямыхРасходов.СуммаПР) КАК СуммаПР,
		|	СУММА(НачальноеСальдоПрямыхРасходов.СуммаВР) КАК СуммаВР,
		|	СУММА(НачальноеСальдоПрямыхРасходов.Количество) КАК Количество,
		|	НачальноеСальдоПрямыхРасходов.НезавершенноеПроизводствоПоПродукции КАК НезавершенноеПроизводствоПоПродукции
		|ПОМЕСТИТЬ НачальноеСальдоПрямыхРасходов
		|ИЗ
		|	ВТ_НачальноеСальдоПрямыхРасходов КАК НачальноеСальдоПрямыхРасходов
		|
		|СГРУППИРОВАТЬ ПО
		|	НачальноеСальдоПрямыхРасходов.НезавершенноеПроизводствоПоПродукции,
		|	НачальноеСальдоПрямыхРасходов.Подразделение,
		|	НачальноеСальдоПрямыхРасходов.Субконто1,
		|	НачальноеСальдоПрямыхРасходов.Субконто2,
		|	НачальноеСальдоПрямыхРасходов.Субконто3,
		|	НачальноеСальдоПрямыхРасходов.Счет
		|
		|ИМЕЮЩИЕ
		|	(СУММА(НачальноеСальдоПрямыхРасходов.Сумма) <> 0
		|		ИЛИ СУММА(НачальноеСальдоПрямыхРасходов.СуммаНУ) <> 0
		|		ИЛИ СУММА(НачальноеСальдоПрямыхРасходов.СуммаПР) <> 0
		|		ИЛИ СУММА(НачальноеСальдоПрямыхРасходов.СуммаВР) <> 0)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Счет,
		|	Подразделение,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_НачальноеСальдоПоОборотнымСубконто
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_ДетальныеСводныеЗаписи
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|УНИЧТОЖИТЬ ВТ_РаспределенныеСуммы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
	...
	|ИЗ
	|	НачальноеСальдоПрямыхРасходов КАК НачальноеСальдо
	|
	|СГРУППИРОВАТЬ ПО
	|	НачальноеСальдо.Счет,
	|	НачальноеСальдо.Подразделение,
...
Процедура СоздатьВТ_СальдоКосвенныхРасходов(МенеджерВременныхТаблиц, Период, Организация)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СальдоКосвенныхРасходов.Счет КАК Счет,
	|	СальдоКосвенныхРасходов.Подразделение КАК Подразделение,
	|	СальдоКосвенныхРасходов.Субконто1 КАК Субконто1,
	|	СальдоКосвенныхРасходов.Субконто2 КАК Субконто2,
	|	СальдоКосвенныхРасходов.Субконто3 КАК Субконто3,
	|	СУММА(СальдоКосвенныхРасходов.Сумма) КАК Сумма,
	|	СУММА(СальдоКосвенныхРасходов.СуммаНУ) КАК СуммаНУ,
	|	СУММА(СальдоКосвенныхРасходов.СуммаПР) КАК СуммаПР,
	|	СУММА(СальдоКосвенныхРасходов.СуммаВР) КАК СуммаВР
	|ПОМЕСТИТЬ ВТ_СальдоКосвенныхРасходов
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|				,
	|				Организация = &Организация) КАК ХозрасчетныйОстатки) КАК СальдоКосвенныхРасходов
	|
	|СГРУППИРОВАТЬ ПО
	|	СальдоКосвенныхРасходов.Счет,
	|	СальдоКосвенныхРасходов.Подразделение,
	|	СальдоКосвенныхРасходов.Субконто1,
	|	СальдоКосвенныхРасходов.Субконто2,
	|	СальдоКосвенныхРасходов.Субконто3
	|
	|ИМЕЮЩИЕ
	|	(СУММА(СальдоКосвенныхРасходов.Сумма) <> 0
	|		ИЛИ СУММА(СальдоКосвенныхРасходов.СуммаНУ) <> 0
	|		ИЛИ СУММА(СальдоКосвенныхРасходов.СуммаПР) <> 0
	|		ИЛИ СУММА(СальдоКосвенныхРасходов.СуммаВР) <> 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура СоздатьОборотыФикс(МенеджерВременныхТаблиц, Период, Организация)
	...
	// Затраты на приобретение прав, если они относятся к малоценным НМА, учитываются в составе расходов текущего периода,
	// относятся на счет финансового результата в момент проведения документа и не должны корректироваться.
	ИменаДокументовФиксированнойСтоимости.Добавить("ПоступлениеТоваровУслуг");
	// Некоторые счета капитальных вложений включены в счета затрат, например, счет "Вложения в ремонты основных средств".
	// Если такой счет указан в документе "МодернизацияОС", то сумма в проводках
	// документа - фиксированная и не должна корректироваться.
	ИменаДокументовФиксированнойСтоимости.Добавить("МодернизацияОС");
	УчетОбособленныхПодразделений.ДополнитьМассивИменамиДокументовАвизо(ИменаДокументовФиксированнойСтоимости, Ложь);
	УчетПроцентовПоОбязательствам.ДополнитьМассивИменамиДокументовПоРассрочке(ИменаДокументовФиксированнойСтоимости);
	...
	|	И Движения.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Движения.Организация = &Организация
	|	И &УсловиеТипаДокумента
	|	И Движения.КоличествоКт < 0";
	
	ТекстыЗапросаОбъединить = Новый Массив;
	ТекстыЗапросаОбъединить.Добавить(ТекстОбъединитьДокументыФикс);

	Для Каждого ИмяДокумента Из ИменаДокументовЗапасыФиксированнойСтоимости Цикл
		
		ТекстЗапроса = СтрЗаменить(ШаблонТекста, "&УсловиеТипаДокумента", "Движения.Регистратор ССЫЛКА Документ." + ИмяДокумента);
		ТекстыЗапросаОбъединить.Добавить(ТекстЗапроса);
	
	КонецЦикла;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"//ОбъединитьДокументыФикс",
		СтрСоединить(ТекстыЗапросаОбъединить, " ОБЪЕДИНИТЬ "));
...
// Создает временную таблицу Пересортица с данными проводок документа ПересортицаТоваров.
// Здесь и далее учтены особенности расчета для случая, когда в документе установлено, что стоимость излишков рассчитывается автоматически.
// Если установлено, что стоимость задается в документе явно, то особенностей расчета нет.
//
// В целом, движения запасов при пересортице подобны комплектации - из некоторого объема товара A получается некоторый объем товара B.
// Однако, движения запасов этого документа нужно обрабатывать обособленно, так как они приводят к раздельному отражению дохода и расхода
// в налоговом учете.
// То есть стоимость этого движения оценивается исходя из стоимости запаса - как в бухгалтерском, так и в налоговом учете.
// Но формула проводок в налоговом учете "разрывает" граф затрат через доходы и расходы,
// например, Дт 91.02 Кт 41 и Дт 41 Кт 91.01, где сумма второй проводки определяется суммой первой
// (в отличие от бухгалтерского учета, где формула проводки указывает на движение внутри графа затрат, например, Дт 41 Кт 41).
//
// Для обработки таких движений в конфигурации принято, что документ ПересортицаТоваров не формирует проводки по доходам и расходам,
// т.е. отражает только Дт 41 Кт 41.
// Проводки по доходам и расходам формируются в результате расчета себестоимости на всю сумму налоговой оценки движения по пересортице.
// См. ДобавитьПроводкиПересортица
// Такие движения выделяются видом движения.
//
// В то же время, проводки по доходам и расходам могут быть сформированы до выполнения регламентной операции.
// Например, для продукции, оставшейся с предыдущего месяца и производимой в текущем, оценка может быть дана
// как операцией КорректировкаСтоимостиНоменклатуры, так и операцией ЗакрытиеСчетов20_23_25_26,
// соответственно каждая из операций формирует движения по доходам и расходам.
// Поэтому ранее сформированные движения по доходам и расходам нужно учитывать при расчете - они выбираются во временные таблицы
// ПересортицаИзлишкиНУ и ПересортицаНедостачиНУ.
//
// Поскольку расчет не предусматривает кратных ребер, чтобы отличить движения по пересортице от комплектаций, вводятся особые вершины учета запасов.
// То есть, движение по пересортице разбивается на два этапа:
// - из исходной вершины A в промежуточную вершину B*
// - из промежуточной вершины B* в финальную вершину B.
// Для идентификации промежуточных вершин (B*) в ней заполняется поле КорСчет значением, равным Счет.
// См. обращения к таблице в СоздатьВыбытия, СоздатьПоступления
//
Процедура СоздатьПересортица(МенеджерВременныхТаблиц, Период, Организация)
	...
	// Группировка при создании таблицы Пересортица не выполняется, чтобы снизить риск переполнения (и снижения точности) суммируемых полей.
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Проводки.СчетДт КАК СчетДт,
	|	ЕСТЬNULL(Проводки.ПодразделениеДт, &ПустоеПодразделение) КАК ПодразделениеДт,
	|	ЕСТЬNULL(Проводки.СубконтоДт1, НЕОПРЕДЕЛЕНО) КАК СубконтоДт1,
	|	ЕСТЬNULL(Проводки.СубконтоДт2, НЕОПРЕДЕЛЕНО) КАК СубконтоДт2,
	|	ЕСТЬNULL(Проводки.СубконтоДт3, НЕОПРЕДЕЛЕНО) КАК СубконтоДт3,
	|	Проводки.СчетКт КАК СчетКт,
	|	ЕСТЬNULL(Проводки.ПодразделениеКт, &ПустоеПодразделение) КАК ПодразделениеКт,
	|	ЕСТЬNULL(Проводки.СубконтоКт1, НЕОПРЕДЕЛЕНО) КАК СубконтоКт1,
	|	ЕСТЬNULL(Проводки.СубконтоКт2, НЕОПРЕДЕЛЕНО) КАК СубконтоКт2,
	|	ЕСТЬNULL(Проводки.СубконтоКт3, НЕОПРЕДЕЛЕНО) КАК СубконтоКт3,
	|	СУММА(ЕСТЬNULL(Проводки.КоличествоДт, 0)) КАК КоличествоДт,
	|	СУММА(ЕСТЬNULL(Проводки.КоличествоКт, 0)) КАК КоличествоКт,
	|	СУММА(ЕСТЬNULL(Проводки.Сумма, 0)) КАК Сумма,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаНУДт, 0)) КАК СуммаНУДт,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаПРДт, 0)) КАК СуммаПРДт,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаВРДт, 0)) КАК СуммаВРДт,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаНУКт, 0)) КАК СуммаНУКт,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаПРКт, 0)) КАК СуммаПРКт,
	|	СУММА(ЕСТЬNULL(Проводки.СуммаВРКт, 0)) КАК СуммаВРКт
	|ПОМЕСТИТЬ ВТ_ПроводкиПересортица
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(
	|			&НачалоПериода,
	|			&КонецПериода,
	...
	|				И Регистратор ССЫЛКА Документ.ПересортицаТоваров
	|				И ВЫРАЗИТЬ(Регистратор КАК Документ.ПересортицаТоваров).ПриходоватьТоварыПоСебестоимостиСписания
	...
	|СГРУППИРОВАТЬ ПО
	|	Проводки.СчетДт,
	|	ЕСТЬNULL(Проводки.ПодразделениеДт, &ПустоеПодразделение),
	|	ЕСТЬNULL(Проводки.СубконтоДт1, НЕОПРЕДЕЛЕНО),
	|	ЕСТЬNULL(Проводки.СубконтоДт2, НЕОПРЕДЕЛЕНО),
	|	ЕСТЬNULL(Проводки.СубконтоДт3, НЕОПРЕДЕЛЕНО),
	|	Проводки.СчетКт,
	|	ЕСТЬNULL(Проводки.ПодразделениеКт, &ПустоеПодразделение),
	|	ЕСТЬNULL(Проводки.СубконтоКт1, НЕОПРЕДЕЛЕНО),
	|	ЕСТЬNULL(Проводки.СубконтоКт2, НЕОПРЕДЕЛЕНО),
	|	ЕСТЬNULL(Проводки.СубконтоКт3, НЕОПРЕДЕЛЕНО)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		КОНЕЦ, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК НоменклатураДт,
	|	Проводки.СчетКт КАК СчетКт,
	|	Проводки.ПодразделениеКт КАК ПодразделениеКт,
	|	Проводки.СубконтоКт1 КАК СубконтоКт1,
	|	Проводки.СубконтоКт2 КАК СубконтоКт2,
	...
	|				ТОГДА ВЫРАЗИТЬ(Проводки.СубконтоКт1 КАК Справочник.Номенклатура)
	|			КОГДА СчетаЗапасовКт.НомерСубконтоНоменклатура = 2
	|				ТОГДА ВЫРАЗИТЬ(Проводки.СубконтоКт2 КАК Справочник.Номенклатура)
	|			КОГДА СчетаЗапасовКт.НомерСубконтоНоменклатура = 3
	|				ТОГДА ВЫРАЗИТЬ(Проводки.СубконтоКт3 КАК Справочник.Номенклатура)
	|		КОНЕЦ, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) КАК НоменклатураКт,
	...
	|ИЗ
	|	ВТ_ПроводкиПересортица КАК Проводки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаЗапасов КАК СчетаЗапасовДт
	|		ПО Проводки.СчетДт = СчетаЗапасовДт.Счет
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаЗапасов КАК СчетаЗапасовКт
	|		ПО Проводки.СчетКт = СчетаЗапасовКт.Счет
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ПроводкиПересортица";
	
	Запрос.Выполнить();
	
КонецПроцедуры

Процедура СоздатьПересортицаИзлишкиНУ(МенеджерВременныхТаблиц, Период, Организация)
	
	// Таблица ПересортицаРасходыНУ создается непосредственно в СоздатьВыбытия для оптимизации (там уже выбраны потребные данные регистра)
	...
	ОписаниеПересортицаИзлишки = СвойстваПроводкиПересортицаДоходы();
	Запрос.УстановитьПараметр("СтатьяИзлишковПересортица",           ОписаниеПересортицаИзлишки.ЗначениеСубконто);
	Запрос.УстановитьПараметр("СчетаИзлишковПересортица",            БухгалтерскийУчетПовтИсп.СчетаВИерархии(ОписаниеПересортицаИзлишки.Счет));
	Запрос.УстановитьПараметр("ВидСтатьиИзлишковПересортица",        ОписаниеПересортицаИзлишки.ВидСубконто);
	Запрос.УстановитьПараметр("ОпределенаСтатьяИзлишковПересортица", ЗначениеЗаполнено(ОписаниеПересортицаИзлишки.ЗначениеСубконто));
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Проводки.Регистратор КАК Регистратор,
	|	Проводки.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ ВТ_ПроводкиПересортицаИзлишки
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный КАК Проводки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Субконто КАК Субконто
	|		ПО Проводки.Регистратор = Субконто.Регистратор
	|			И Проводки.НомерСтроки = Субконто.НомерСтроки
	|ГДЕ
	|	&ОпределенаСтатьяИзлишковПересортица
	|	И Проводки.Активность
	|	И Проводки.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Проводки.Организация = &Организация
	|	И ЕСТЬNULL(Проводки.КоличествоДт, 0) = 0
	|	И ЕСТЬNULL(Проводки.КоличествоКт, 0) = 0
	|	И НЕ Проводки.НеКорректироватьСтоимостьАвтоматически
	|	И Проводки.СчетКт В(&СчетаИзлишковПересортица)
	|	И Субконто.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Кредит)
	|	И Субконто.Вид = &ВидСтатьиИзлишковПересортица
	|	И Субконто.Значение = &СтатьяИзлишковПересортица
	|	И Субконто.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Регистратор,
	|	НомерСтроки
	...
	|	СУММА(ЕСТЬNULL(Обороты.СуммаПРКт, 0)) КАК СуммаПР,
	|	СУММА(ЕСТЬNULL(Обороты.СуммаВРКт, 0)) КАК СуммаВР
	|ПОМЕСТИТЬ ВТ_ОборотыПересортицаИзлишки
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(
	|			&НачалоПериода,
	|			&КонецПериода,
	...
	|				ИЗ
	|					ВТ_ПроводкиПересортицаИзлишки КАК Проводки),
	|			,
	|			) КАК Обороты
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Излишки.СчетДт КАК СчетДт,
	|	Излишки.ПодразделениеДт КАК ПодразделениеДт,
	|	Излишки.СубконтоДт1 КАК СубконтоДт1,
	|	Излишки.СубконтоДт2 КАК СубконтоДт2,
	|	Излишки.СубконтоДт3 КАК СубконтоДт3
	|ПОМЕСТИТЬ ВТ_АналитикаПересортицаИзлишки
	|ИЗ
	|	Пересортица КАК Излишки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетДт,
	|	ПодразделениеДт,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Поступления.СчетДт КАК СчетДт,
	|	Поступления.ПодразделениеДт КАК ПодразделениеДт,
	|	Поступления.СубконтоДт1 КАК СубконтоДт1,
	|	Поступления.СубконтоДт2 КАК СубконтоДт2,
	|	Поступления.СубконтоДт3 КАК СубконтоДт3,
	|	Поступления.СуммаНУ КАК СуммаНУ,
	|	ВЫБОР
	|		КОГДА Поступления.СуммаПР <> 0
	|			ТОГДА Поступления.СуммаПР + Поступления.Сумма
	|		ИНАЧЕ Поступления.СуммаПР
	|	КОНЕЦ КАК СуммаПР,
	|	ВЫБОР
	|		КОГДА Поступления.СуммаПР = 0
	|			ТОГДА Поступления.СуммаВР + Поступления.Сумма
	|		ИНАЧЕ Поступления.СуммаВР
	|	КОНЕЦ КАК СуммаВР
	|ПОМЕСТИТЬ ПересортицаИзлишкиНУ
	|ИЗ
	|	ВТ_ОборотыПересортицаИзлишки КАК Поступления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АналитикаПересортицаИзлишки КАК ОтборДт
	|		ПО Поступления.СчетДт = ОтборДт.СчетДт
	|			И Поступления.ПодразделениеДт = ОтборДт.ПодразделениеДт
	|			И Поступления.СубконтоДт1 = ОтборДт.СубконтоДт1
	|			И Поступления.СубконтоДт2 = ОтборДт.СубконтоДт2
	|			И Поступления.СубконтоДт3 = ОтборДт.СубконтоДт3
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетДт,
	|	ПодразделениеДт,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ПроводкиПересортицаИзлишки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ОборотыПересортицаИзлишки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_АналитикаПересортицаИзлишки";
...
Процедура СоздатьПоступления(МенеджерВременныхТаблиц, Период, Организация)
	...
	//   ОборотДтЗапасы
	// + ОборотДтРасходы,
	// при этом в них может заполняться поле КорСчет.
	// В ОборотДтРасходы КорСчет заполняется последовательно, это нужно для определения элементов затрат,
	// а также, для целей налогового учета - прямые расходы налогового учета задаются с точностью до КорСчета.
	...
	// В ОборотДтЗапасы КорСчет заполняется в особых случаях, по разным правилам для разных случаев:
	// - для пересортицы см. комментарий к СоздатьПересортица
	// - для розничной наценки - см. создание и использование СчетаТорговойНаценкиАТТ, комментарий в СоздатьНачальноеСальдо
	...
	//
	// Из тех же соображений из Выбытия выбираются сведения о введенных промежуточных вершинах для отражения пересортицы (B* в терминах из СоздатьПересортица).
	// Здесь вычитание не требуется, потому что промежуточные вершины "виртуальные": в регистре бухгалтерии отсутствуют проводки, ведущие в них.
	...
	// Об использовании пар счетов 41.11/42.01 и 10.21.1/10.21.2 см. комментарий в СоздатьНачальноеСальдо
	...
	|	
	|	ВЫБРАТЬ
	|		Промежуточные.СчетДт,
	|		Промежуточные.ПодразделениеДт,
	|		Промежуточные.СубконтоДт1,
	|		Промежуточные.СубконтоДт2,
	|		Промежуточные.СубконтоДт3,
	|		Промежуточные.КорСчетДт,
	|		Промежуточные.НоменклатураДт,
	|		0,
	|		Промежуточные.Количество,
	|		Промежуточные.Сумма,
	|		Промежуточные.СуммаНУ,
	|		Промежуточные.СуммаПР,
	|		Промежуточные.СуммаВР
	|	ИЗ
	|		Выбытия КАК Промежуточные
	|	ГДЕ
	|		Промежуточные.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		Излишки.СчетДт,
	|		Излишки.ПодразделениеДт,
	|		Излишки.СубконтоДт1,
	|		Излишки.СубконтоДт2,
	|		Излишки.СубконтоДт3,
	|		ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка),
	...
	|		-Излишки.СуммаВР
	|	ИЗ
	|		ПересортицаИзлишкиНУ КАК Излишки) КАК Поступления
	|
	|СГРУППИРОВАТЬ ПО
...
// Создает временную таблицу движений Выбытия, заодно создается ПересортицаНедостачиНУ
Процедура СоздатьВыбытия(МенеджерВременныхТаблиц, Период, Организация)
	...
	// Об использовании пар счетов 41.11/42.01 и 10.21.1/10.21.2 см. комментарий в СоздатьНачальноеСальдо
	// Выбытие со счета 10.21.1 в налоговом учете - всегда в рамках контура учета затрат.
	// В бухгалтерском учете может быть
	// - внутри контура затрат, если речь идет о движении в рамках счета 10.21.1
	...
	// что таких записей достаточно для формирования проводок по этому счету
	...
	// Для пересортицы "разрываем" проводку по превращению излишков в недостачи на две, через промежуточную вершину,
	// выделяемую заполненным КорСчетом. См. комментарии в СоздатьПересортица
	...
	ОписаниеПересортицаНедостачи = СвойстваПроводкиПересортицаРасходы();
	Запрос.УстановитьПараметр("СчетНедостачПересортица",             ОписаниеПересортицаНедостачи.Счет);
	Запрос.УстановитьПараметр("ВидСтатьиНедостачПересортица",        ОписаниеПересортицаНедостачи.ВидСубконто);
	Запрос.УстановитьПараметр("СтатьяНедостачПересортица",           ОписаниеПересортицаНедостачи.ЗначениеСубконто);
	Запрос.УстановитьПараметр("ОпределенаСтатьяНедостачПересортица", ЗначениеЗаполнено(ОписаниеПересортицаНедостачи.ЗначениеСубконто));
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Недостачи.СчетКт КАК СчетКт,
	|	Недостачи.ПодразделениеКт КАК ПодразделениеКт,
	|	Недостачи.СубконтоКт1 КАК СубконтоКт1,
	|	Недостачи.СубконтоКт2 КАК СубконтоКт2,
	|	Недостачи.СубконтоКт3 КАК СубконтоКт3,
	|	Недостачи.НоменклатураКт КАК НоменклатураКт
	|ПОМЕСТИТЬ ВТ_АналитикаПересортицаНедостачи
	|ИЗ
	|	Пересортица КАК Недостачи
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетКт,
	|	ПодразделениеКт,
	...
	|	&СтатьяНедостачПересортица КАК ЗначениеСубконто
	|ПОМЕСТИТЬ ВТ_МаркерПроводокНедостачиНУ
	|ИЗ
	|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ВидыСубконто
	|ГДЕ
	|	ВидыСубконто.Ссылка В ИЕРАРХИИ(&СчетНедостачПересортица)
	|	И ВидыСубконто.ВидСубконто = &ВидСтатьиНедостачПересортица
	|	И &ОпределенаСтатьяНедостачПересортица
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Выбытия.СчетДт КАК СчетДт,
	|	Выбытия.ПодразделениеДт КАК ПодразделениеДт,
	|	Выбытия.СубконтоДт1 КАК СубконтоДт1,
	|	Выбытия.СубконтоДт2 КАК СубконтоДт2,
	|	Выбытия.СубконтоДт3 КАК СубконтоДт3,
	|	ОтборКт.СчетКт КАК СчетКт,
	|	ОтборКт.ПодразделениеКт КАК ПодразделениеКт,
	|	ОтборКт.СубконтоКт1 КАК СубконтоКт1,
	|	ОтборКт.СубконтоКт2 КАК СубконтоКт2,
	|	ОтборКт.СубконтоКт3 КАК СубконтоКт3,
	|	ОтборКт.НоменклатураКт КАК НоменклатураКт,
	|	Выбытия.СуммаНУ КАК СуммаНУ,
	|	ВЫБОР
	|		КОГДА Выбытия.СуммаПР <> 0
	|			ТОГДА Выбытия.СуммаПР + Выбытия.Сумма
	|		ИНАЧЕ Выбытия.СуммаПР
	|	КОНЕЦ КАК СуммаПР,
	|	ВЫБОР
	|		КОГДА Выбытия.СуммаПР = 0
	|			ТОГДА Выбытия.СуммаВР + Выбытия.Сумма
	|		ИНАЧЕ Выбытия.СуммаВР
	|	КОНЕЦ КАК СуммаВР
	|ПОМЕСТИТЬ ПересортицаНедостачиНУ
	|ИЗ
	|	ВТ_Выбытия КАК Выбытия
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_АналитикаПересортицаНедостачи КАК ОтборКт
	|		ПО Выбытия.СчетКт = ОтборКт.СчетКт
	|			И Выбытия.ПодразделениеКт = ОтборКт.ПодразделениеКт
	|			И Выбытия.СубконтоКт1 = ОтборКт.СубконтоКт1
	|			И Выбытия.СубконтоКт2 = ОтборКт.СубконтоКт2
	|			И Выбытия.СубконтоКт3 = ОтборКт.СубконтоКт3
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_МаркерПроводокНедостачиНУ КАК ОтборДт
	|		ПО Выбытия.СчетДт = ОтборДт.Счет
	|			И (ОтборДт.НомерСубконто = 1
	|					И Выбытия.СубконтоДт1 = ОтборДт.ЗначениеСубконто
	|				ИЛИ ОтборДт.НомерСубконто = 2
	|					И Выбытия.СубконтоДт2 = ОтборДт.ЗначениеСубконто
	|				ИЛИ ОтборДт.НомерСубконто = 3
	|					И Выбытия.СубконтоДт3 = ОтборДт.ЗначениеСубконто)
	|ГДЕ
	|	НЕ Выбытия.ФиксированнаяСтоимость
	|	И НЕ Выбытия.ДанныеЗабалансовогоУчета
	|	И НЕ Выбытия.ПереходНаЗабалансовыйУчет
	|	И Выбытия.Количество = 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетКт,
	|	ПодразделениеКт,
	...
	|		ЛОЖЬ КАК Выпуск,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПустаяСсылка) КАК ВидДвижения,
	|		Обороты.ФиксированнаяСтоимость КАК ФиксированнаяСтоимость,
	...
	|		КОНЕЦ,
	|		ЛОЖЬ,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПустаяСсылка),
	...
	|	
	|	ВЫБРАТЬ
	|		РазорванныеСторно.СчетДт,
	|		РазорванныеСторно.ПодразделениеДт,
	|		РазорванныеСторно.СубконтоДт1,
	|		РазорванныеСторно.СубконтоДт2,
	|		РазорванныеСторно.СубконтоДт3,
	|		РазорванныеСторно.НоменклатураДт,
	|		РазорванныеСторно.СчетКт,
	|		РазорванныеСторно.ПодразделениеКт,
	|		РазорванныеСторно.СубконтоКт1,
	|		РазорванныеСторно.СубконтоКт2,
	|		РазорванныеСторно.СубконтоКт3,
	|		РазорванныеСторно.НоменклатураКт,
	|		ЛОЖЬ,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПустаяСсылка),
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		-РазорванныеСторно.КоличествоКт,
	|		-РазорванныеСторно.Сумма,
	|		-РазорванныеСторно.СуммаНУКт,
	|		-РазорванныеСторно.СуммаПРКт,
	|		-РазорванныеСторно.СуммаВРКт
	|	ИЗ
	|		Пересортица КАК РазорванныеСторно
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		РазорванныеКт.СчетДт,
	|		РазорванныеКт.ПодразделениеДт,
	|		РазорванныеКт.СубконтоДт1,
	|		РазорванныеКт.СубконтоДт2,
	|		РазорванныеКт.СубконтоДт3,
	|		РазорванныеКт.НоменклатураДт,
	|		РазорванныеКт.СчетКт,
	|		РазорванныеКт.ПодразделениеКт,
	|		РазорванныеКт.СубконтоКт1,
	|		РазорванныеКт.СубконтоКт2,
	|		РазорванныеКт.СубконтоКт3,
	|		РазорванныеКт.НоменклатураКт,
	|		ЛОЖЬ,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие),
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		РазорванныеКт.КоличествоКт,
	|		РазорванныеКт.Сумма,
	|		РазорванныеКт.СуммаНУКт,
	|		РазорванныеКт.СуммаПРКт,
	|		РазорванныеКт.СуммаВРКт
	|	ИЗ
	|		Пересортица КАК РазорванныеКт
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		РазорванныеДт.СчетДт,
	|		РазорванныеДт.ПодразделениеДт,
	|		РазорванныеДт.СубконтоДт1,
	|		РазорванныеДт.СубконтоДт2,
	|		РазорванныеДт.СубконтоДт3,
	|		РазорванныеДт.НоменклатураДт,
	|		РазорванныеДт.СчетДт,
	|		РазорванныеДт.ПодразделениеДт,
	|		РазорванныеДт.СубконтоДт1,
	|		РазорванныеДт.СубконтоДт2,
	...
	|		РазорванныеДт.НоменклатураДт,
	|		ЛОЖЬ,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаПоступление),
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	|		РазорванныеДт.КоличествоДт,
	|		РазорванныеДт.Сумма,
	|		РазорванныеДт.СуммаНУДт,
	|		РазорванныеДт.СуммаПРДт,
	|		РазорванныеДт.СуммаВРДт
	|	ИЗ
	|		Пересортица КАК РазорванныеДт
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		Недостачи.СчетДт,
	|		Недостачи.ПодразделениеДт,
	|		Недостачи.СубконтоДт1,
	|		Недостачи.СубконтоДт2,
	|		Недостачи.СубконтоДт3,
	|		ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка),
	|		Недостачи.СчетКт,
	|		Недостачи.ПодразделениеКт,
	|		Недостачи.СубконтоКт1,
	|		Недостачи.СубконтоКт2,
	|		Недостачи.СубконтоКт3,
	|		Недостачи.НоменклатураКт,
	|		ЛОЖЬ,
	|		ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПустаяСсылка),
	|		ЛОЖЬ,
	|		ЛОЖЬ,
	...
	|		-Недостачи.СуммаВР
	|	ИЗ
	|		ПересортицаНедостачиНУ КАК Недостачи) КАК Обороты
	|
	|СГРУППИРОВАТЬ ПО
	|	Обороты.СчетДт,
	|	Обороты.ПодразделениеДт,
	...
	|	ВЫБОР
	|		КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаПоступление)
	...
	|				И НЕ Движения.ПереходНаЗабалансовыйУчет
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов)
	|		ИНАЧЕ ЕСТЬNULL(СчетаТорговойНаценкиДт.СчетНаценки, Движения.СчетДт)
	|	КОНЕЦ КАК СчетДт,
	...
	|	ВЫБОР
	|		КОГДА Движения.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_АналитикаПересортицаНедостачи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_МаркерПроводокНедостачиНУ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура СоздатьНезавершенноеПроизводство(МенеджерВременныхТаблиц, Период, Организация, Пустая = Ложь)
	...
	// Обработчик обновления, заполняющий вид операции, может быть еще не выполнен. В предыдущих версиях программы
	// все документы были с суммовой оценкой, поэтому считаем, что все документы с незаполненным видом операции имеют
	// суммовую оценку и в отбор вида операции добавляем пустую ссылку.
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаНезавершенноеПроизводствоПоПродукции КАК СчетаНезавершенноеПроизводствоПоПродукции
	|		ПО СуммыНЗП.Ссылка.СчетЗатрат = СчетаНезавершенноеПроизводствоПоПродукции.Счет
	|ГДЕ
	...
	|	И СуммыНЗП.Ссылка.ВидОперации В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИнвентаризацияНЗП.СуммоваяОценка), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийИнвентаризацияНЗП.ПустаяСсылка))
	|	И СчетаНезавершенноеПроизводствоПоПродукции.Счет ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура СоздатьСтатьиКалькуляцииНачальноеСальдо(МенеджерВременныхТаблиц, Период, Организация, ВариантыСтруктурыЗатрат)
	...
	НайденИсточник = ИспользуетсяВнешнийИсточникСтатьиКалькуляцииНачальноеСальдо(ВариантыСтруктурыЗатрат);
	...
	// Поля Сальдо1, Сальдо2, Сальдо3 в общем случае заполняются значениями номенклатурных групп и продукции,
	// если эти субконто являются необоротными. Для количественной оценки незавершенного производства всегда требуются
	// остатки в разрезе номенклатурных групп, статей затрат и продукции, несмотря на то, что СтатьиЗатрат - оборотное субконто.
	// По данным временной таблицы СтатьиКалькуляцииНачальноеСальдо в процедуре СоздатьНачальноеСальдо() остатки регистра бухгалтерии
	// могут дополняться данными о статьях затрат.
    ...
    |	КОНЕЦ КАК Подразделение,
    |	ВЫБОР
    |		КОГДА НЕ СчетаНезавершенноеПроизводствоПоПродукции.Счет ЕСТЬ NULL
    |			ТОГДА ВЫБОР
    |					КОГДА СубконтоЗатрат.ВидСубконто1 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы)
    ...
    |	СтруктураСтоимости.Продукция <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка) КАК ПрямыеЗатраты
    ...
    |		ЛЕВОЕ СОЕДИНЕНИЕ ОборотныеСубконто КАК ОборотныеСубконто
    |		ПО СтруктураСтоимости.СчетВыпуска = ОборотныеСубконто.Счет
    |		ЛЕВОЕ СОЕДИНЕНИЕ СчетаНезавершенноеПроизводствоПоПродукции КАК СчетаНезавершенноеПроизводствоПоПродукции
    |		ПО СтруктураСтоимости.СчетВыпуска = СчетаНезавершенноеПроизводствоПоПродукции.Счет
    |ГДЕ
...
Функция ИспользуетсяВнешнийИсточникСтатьиКалькуляцииНачальноеСальдо(Знач ВариантыСтруктурыЗатрат)
	...
	Возврат НайденИсточник;
	
КонецФункции

Процедура ПереопределитьОборотныеСубконто(МенеджерВременныхТаблиц)

	Запрос = Новый Запрос("УНИЧТОЖИТЬ ОборотныеСубконто");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Выполнить();
	СоздатьОборотныеСубконто(МенеджерВременныхТаблиц, Неопределено);
...
Процедура ДополнитьВыбытияИдентификаторамиВершин(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Выбытия.ВнутреннееДвижение КАК ВнутреннееДвижение,
	|	Выбытия.БезИзмененияКоличества КАК БезИзмененияКоличества,
	|	Выбытия.Выпуск КАК Выпуск,
	|	Выбытия.ВидДвижения КАК ВидДвижения,
	|	Выбытия.СчетКт КАК СчетКт,
	|	Выбытия.ПодразделениеКт КАК ПодразделениеКт,
	|	Выбытия.СубконтоКт1 КАК СубконтоКт1,
	|	Выбытия.СубконтоКт2 КАК СубконтоКт2,
	|	Выбытия.СубконтоКт3 КАК СубконтоКт3,
	|	Выбытия.НоменклатураКт КАК НоменклатураКт,
	|	Выбытия.КорСчетКт КАК КорСчетКт,
	...
	|	Выбытия.СубконтоДт2 КАК СубконтоДт2,
	|	Выбытия.СубконтоДт3 КАК СубконтоДт3,
	|	Выбытия.НоменклатураДт КАК НоменклатураДт,
	|	Выбытия.КорСчетДт КАК КорСчетДт,
	|	Выбытия.ФиксированнаяСтоимость КАК ФиксированнаяСтоимость,
	|	Выбытия.Количество КАК Количество,
	...
	|	Выбытия.РазделУчетаПриемник КАК РазделУчетаПриемник,
	|	Выбытия.СчетДт КАК СчетДт,
	|	Выбытия.ПодразделениеДт КАК ПодразделениеДт,
	|	Выбытия.СубконтоДт1 КАК СубконтоДт1,
	|	Выбытия.СубконтоДт2 КАК СубконтоДт2,
...
Процедура ЗаполнитьГрафЗатрат(Затраты, МенеджерВременныхТаблиц)
	...
	//                        Источник - из раздела "Расходы"
	//                        Вес - не определен
	// - "ПересортицаВыбытие", "ПересортицаПоступление" - выделяют движения, которые для целей налогового учета
	//                        должны быть "разорваны" через расходы и доходы: выбывают недостачи, поступают излишки.
	//                        Подробнее см. комментарий к СоздатьПересортица.
	//                        Вес - количество номенклатуры.
	...
	|	(ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА Ребра.ВидДвижения <> ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПустаяСсылка)
	|				ТОГДА Ребра.ВидДвижения
	|			КОГДА Ребра.БезИзмененияКоличества
	|				ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.Перемещение)
	|			ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.Другое)
...
Процедура ИндексироватьГрафЗатрат(Затраты)
	...
	Затраты.Контуры.Индексы.Добавить("Вершина");
...
Функция РазрушитьКонтурПоискВГлубинуРекурсивно(Затраты, ОписаниеВершины, Путь, МетодРазрушения, Статусы)
	...
	Для Каждого Ребро Из Приемники Цикл
		...
		// Движения пересортицы приравниваем к перемещениям, несмотря на то, что в их ходе изменяется номенклатура:
		// эта номенклатура по определению однородная, неотличимая на практике.
		Если МетодРазрушения = "Перемещения"
			И Ребро.ВидДвижения <> Перечисления.ВидыДвиженийРасчетСебестоимости.Перемещение
			И Ребро.ВидДвижения <> Перечисления.ВидыДвиженийРасчетСебестоимости.ПересортицаПоступление
			И Ребро.ВидДвижения <> Перечисления.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие Тогда
			Продолжить;
		КонецЕсли;
...
Функция РазрушитьКонтур(Затраты, ОписаниеВершины, Путь, МетодРазрушения)
	...
	ИначеЕсли МетодРазрушения = "Запасы" Тогда
		
		// Условно размыкаем ребро, ведущее из вершины с наименьшим ожидаемым влиянием на другие вершины контура.
		// Условность размыкания заключается в том, что один раз пройдем по этому ребру
		
		// Влияние оцениваем с учетом того, что веса разных ребер в общем случае могут быть не сопоставимы между собой.
		// Для этого рассчитываем коэффициенты самостоятельности вершин - т.е. степень их "неучастия" в контуре:
		// отношение "свободного" (не участвующего в контуре) веса вершины к весу ребра, участвующего в контуре.
		// Размыкаем то, что ведет из вершины с наибольшим коэффициентом.
		// Следствием такого подхода будет то, что не размыкается единственный исход вершины
		// (т.е. единственное ребро вершины, не имеющей ни остатка, ни внешнего выбытия).
		
		НаибольшийКоэффициентСамостоятельности = 0;
		НаилучшееРебро = Контур[0]; // с наибольшим коэффициентом самостоятельности
		...
		Для Каждого Ребро Из Контур Цикл
			...
			ОписаниеИсточника = ОписаниеВершины(Затраты, Ребро.Источник);
			СвободныйВесИсточника = ОписаниеИсточника.Вес - Ребро.Вес;
			КоэффициентСамостоятельности = СвободныйВесИсточника / Ребро.Вес;
			
			Если КоэффициентСамостоятельности > НаибольшийКоэффициентСамостоятельности Тогда
				НаибольшийКоэффициентСамостоятельности = КоэффициентСамостоятельности;
				НаилучшееРебро = Ребро;
			КонецЕсли;
			
		КонецЦикла;
		
		НаилучшееРебро.СлабоеЗвено = Истина;
...
Функция ВыделитьНалоговыйУчет(ВидДвижения, СуммыИсточника, Затраты)
	...
	// В случае когда сумма БУ = 0 и ведется учет разниц, выделение временных разниц при наличии постоянных, может приводить
	// к тому, что разницы ПР и ВР будут "ходить" вслед за отсутствующей суммой БУ отдельно от налогового учета.
	// Такая ситуация может возникать, например, в случае нормируемых расходов, а также при применении инвестиционного вычета
	// или повышающего коэффициента на НИОКР, НМА, лицензии на ПО.
	ВременныеРазницыУжеВыделены = ВременныеРазницыУжеВыделены(СуммыИсточника, Затраты, ВидДвижения);
	...
	Для Каждого КоллекцияНалоговыйУчет Из Затраты.КешПараметровРасчета.ИменаСвойствНалоговыйУчетПеременныхНаборовСумм Цикл
		...
		НаборСуммРебра = НаборСуммИсточника.Скопировать();
		...
		Если ВременныеРазницыУжеВыделены Тогда
			НаборСуммРебра.ЗагрузитьКолонку(НаборСуммИсточника.ВыгрузитьКолонку("СуммаПР"), "СуммаПР");
			НаборСуммРебра.ЗагрузитьКолонку(НаборСуммИсточника.ВыгрузитьКолонку("СуммаВР"), "СуммаВР");
		КонецЕсли;
	КонецЦикла;
	...
	Если НалогНаПрибыльБухгалтерскийУчет.ЕстьРазницыПоНалогуНаПрибыль(Затраты.ОписаниеСумм) Тогда
		...
		ИначеЕсли ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.ВременныеРазницы Тогда
			...
			Если ВременныеРазницыУжеВыделены Тогда
				ЗаполнитьЗначенияСвойств(СуммыРебра, СуммыИсточника, "СуммаПР, СуммаВР");
			Иначе
...
Функция ВременныеРазницыУжеВыделены(СуммыИсточника, Затраты, ВидДвижения)
	
	Возврат НалогНаПрибыльБухгалтерскийУчет.ЕстьРазницыПоНалогуНаПрибыль(Затраты.ОписаниеСумм)
		И ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.ВременныеРазницы
		И СуммыИсточника.Сумма = 0 И СуммыИсточника.СуммаВР <> 0
		И СуммыИсточника.СуммаНУ + СуммыИсточника.СуммаПР + СуммыИсточника.СуммаВР = 0;
...
Процедура ИнициализироватьКешНалоговогоУчета(Затраты)
	...
	Затраты.КешПараметровРасчета.Вставить("ИспользоватьСуммыРазницПБУ18",         Истина);
...
Процедура ПрименитьНастройкуПорядкаПеределов(Затраты, МенеджерВременныхТаблиц, Настройки)
	...
	Если Настройки.ВстречныйВыпуск <> Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПодразделениям Тогда
...
Процедура ПрименитьНастройкуВстречногоВыпуска(Затраты, МенеджерВременныхТаблиц, Настройки)
	
	Если Настройки.ВстречныйВыпуск <> Перечисления.СпособыРасчетаСебестоимостиПродукции.ПоПеределам Тогда
		// Настройка не используется
		Возврат;
	КонецЕсли;
...
Функция НайтиВстречныйВыпускБезНастройки(Знач Контур, Затраты)
	...
	// Стараемся выбрать то ребро, которое соответствует выпуску продукции/услуг, перемещению запасов либо отражает списание материалов в производство.
	...
	Для Каждого Ребро Из Контур Цикл
		Если Ребро.ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.Выпуск
			Или Ребро.ВидДвижения = Перечисления.ВидыДвиженийРасчетСебестоимости.Перемещение
...
Процедура СоздатьДвижения(МенеджерВременныхТаблиц, Затраты, Протокол)
	...
	Запрос.УстановитьПараметр("Ребра",                   Затраты.Ребра);
	Запрос.УстановитьПараметр("ИспользоватьСуммыРазниц", Затраты.КешПараметровРасчета.ИспользоватьСуммыРазницПБУ18);
	...
	|			ИЛИ &ИспользоватьСуммыРазниц
	|				И НЕ(ВнутренниеДвижения.СуммаПР = 0
...
Процедура ДобавитьПроводку(ТаблицаПроводок, Знач ОписаниеПроводки)
	...
	Если Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов Тогда
		...
		ИначеЕсли Проводка.СчетДт = ПланыСчетов.Хозрасчетный.ИнвентарьИХозяйственныеПринадлежностиВЭксплуатации Тогда
			...
			Для НомерСубконто = 1 По 3 Цикл
				ОписаниеПроводки.АналитикаДт["Субконто" + НомерСубконто] = ОписаниеПроводки.АналитикаКт["Субконто" + НомерСубконто];
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьПроводкиВнутренниеДвижения(Проводки, МенеджерВременныхТаблиц)
	...
	|		ОкончательныеПроводки.СуммаНУ - ВЫБОР
	|			КОГДА ОкончательныеПроводки.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие)
	|				ТОГДА ОкончательныеПроводки.СуммаНУ
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК СуммаНУ,
	|		ОкончательныеПроводки.СуммаПР КАК СуммаПР,
	|		ОкончательныеПроводки.СуммаВР + ВЫБОР
	|			КОГДА ОкончательныеПроводки.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие)
	|				ТОГДА ОкончательныеПроводки.СуммаНУ
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК СуммаВР,
...
Процедура ДобавитьПроводкиПересортица(Проводки, МенеджерВременныхТаблиц)
	
	// В налоговом учете при пересортице отражаются доходы и расходы.
	// Однако, в отличие от всех остальных доходов и расходов, отражаемых по результатам расчета себестоимости,
	// эти порождаются внутренними движениями.
	// Можно сказать, что "внутренняя проводка" (внутри счетов запасов, Дт 41 Кт 41) в налоговом учете приводит в доходам и расходам.
	
	// Также для этих проводок отличается источник информации о предварительных проводках - это не сумма внутренних движений,
	// а сумма ранее сформированных проводок по доходам и расходам.
	...
	|ИЗ
	|	ВнутренниеДвижения КАК Проводки
	|ГДЕ
	|	Проводки.ВидДвижения = ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийРасчетСебестоимости.ПересортицаВыбытие)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Проводки.СчетДт КАК Счет,
	|	Проводки.ПодразделениеДт КАК Подразделение,
	|	Проводки.СубконтоДт1 КАК Субконто1,
	|	Проводки.СубконтоДт2 КАК Субконто2,
	|	Проводки.СубконтоДт3 КАК Субконто3,
	|	СУММА(Проводки.СуммаНУ) КАК СуммаНУ,
	|	МАКСИМУМ(Проводки.Порядок) КАК Порядок
	|ПОМЕСТИТЬ ВТ_ОкончательныеПроводкиДт
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Проводки.СчетКт КАК Счет,
	|	Проводки.ПодразделениеКт КАК Подразделение,
	|	Проводки.СубконтоКт1 КАК Субконто1,
	|	Проводки.СубконтоКт2 КАК Субконто2,
	|	Проводки.СубконтоКт3 КАК Субконто3,
	|	СУММА(Проводки.СуммаНУ) КАК СуммаНУ,
	|	МАКСИМУМ(Проводки.Порядок) КАК Порядок
	|ПОМЕСТИТЬ ВТ_ОкончательныеПроводкиКт
	|ИЗ
	|	ВТ_ОкончательныеПроводки КАК Проводки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ВидДвиженияБухгалтерии.Дебет) КАК НаправлениеДвиженияЗапасов,
	|	Корректировки.Счет КАК Счет,
	|	Корректировки.Подразделение КАК Подразделение,
	|	Корректировки.Субконто1 КАК Субконто1,
	|	Корректировки.Субконто2 КАК Субконто2,
	|	Корректировки.Субконто3 КАК Субконто3,
	|	МАКСИМУМ(Корректировки.Порядок) КАК Порядок,
	|	0 КАК Сумма,
	|	СУММА(Корректировки.СуммаНУ) КАК СуммаНУ,
	|	0 КАК СуммаПР,
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОкончательныеПроводки.Счет КАК Счет,
	|		ОкончательныеПроводки.Подразделение КАК Подразделение,
	|		ОкончательныеПроводки.Субконто1 КАК Субконто1,
	|		ОкончательныеПроводки.Субконто2 КАК Субконто2,
	|		ОкончательныеПроводки.Субконто3 КАК Субконто3,
	|		ОкончательныеПроводки.СуммаНУ КАК СуммаНУ,
	|		-ОкончательныеПроводки.СуммаНУ КАК СуммаВР,
	|		ОкончательныеПроводки.Порядок КАК Порядок
	|	ИЗ
	|		ВТ_ОкончательныеПроводкиДт КАК ОкончательныеПроводки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		NULL
	|	ИЗ
	|		ПересортицаИзлишкиНУ КАК ПредварительныеПроводки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ОкончательныеПроводкиДт КАК ОкончательныеПроводки
	|			ПО ПредварительныеПроводки.СчетДт = ОкончательныеПроводки.Счет
	|				И ПредварительныеПроводки.ПодразделениеДт = ОкончательныеПроводки.Подразделение
	|				И ПредварительныеПроводки.СубконтоДт1 = ОкончательныеПроводки.Субконто1
	|				И ПредварительныеПроводки.СубконтоДт2 = ОкончательныеПроводки.Субконто2
	|				И ПредварительныеПроводки.СубконтоДт3 = ОкончательныеПроводки.Субконто3) КАК Корректировки
	|
	|СГРУППИРОВАТЬ ПО
	|	Корректировки.Счет,
	|	Корректировки.Подразделение,
	|	Корректировки.Субконто1,
	|	Корректировки.Субконто2,
	|	Корректировки.Субконто3
	|
	|ИМЕЮЩИЕ
	|	(СУММА(Корректировки.СуммаНУ) <> 0
	|		ИЛИ СУММА(Корректировки.СуммаВР) <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	Корректировки.Субконто2,
	|	Корректировки.Субконто3,
	|	МАКСИМУМ(Корректировки.Порядок),
	|	0,
	|	СУММА(Корректировки.СуммаНУ),
	|	0,
	|	СУММА(Корректировки.СуммаВР)
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОкончательныеПроводки.Счет КАК Счет,
	|		ОкончательныеПроводки.Подразделение КАК Подразделение,
	...
	|		ОкончательныеПроводки.Порядок КАК Порядок
	|	ИЗ
	|		ВТ_ОкончательныеПроводкиКт КАК ОкончательныеПроводки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		NULL
	|	ИЗ
	|		ПересортицаНедостачиНУ КАК ПредварительныеПроводки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ОкончательныеПроводкиКт КАК ОкончательныеПроводки
	|			ПО ПредварительныеПроводки.СчетКт = ОкончательныеПроводки.Счет
	|				И ПредварительныеПроводки.ПодразделениеКт = ОкончательныеПроводки.Подразделение
	|				И ПредварительныеПроводки.СубконтоКт1 = ОкончательныеПроводки.Субконто1
	|				И ПредварительныеПроводки.СубконтоКт2 = ОкончательныеПроводки.Субконто2
	|				И ПредварительныеПроводки.СубконтоКт3 = ОкончательныеПроводки.Субконто3) КАК Корректировки
	|
	|СГРУППИРОВАТЬ ПО
	|	Корректировки.Счет,
	|	Корректировки.Подразделение,
	...
	|	Субконто2,
	|	Субконто3,
	|	НаправлениеДвиженияЗапасов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ОкончательныеПроводки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ОкончательныеПроводкиДт
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТ_ОкончательныеПроводкиКт";
	...
	СвойстваДоходы  = СвойстваПроводкиПересортицаДоходы();
	СвойстваРасходы = СвойстваПроводкиПересортицаРасходы();
	ПоляДоходы  = ПоляПроводкиПересортица(СвойстваДоходы);
	ПоляРасходы = ПоляПроводкиПересортица(СвойстваРасходы);
	
	Пока Выборка.Следующий() Цикл
		
		Если Выборка.НаправлениеДвиженияЗапасов = ВидДвиженияБухгалтерии.Дебет Тогда
			
			// Излишки отражаются в НУ как доход
			
			ОписаниеПроводки.АналитикаДт = НоваяСтруктураПолейПроводки(Выборка);
			ОписаниеПроводки.АналитикаКт = ПоляДоходы;
			ОписаниеПроводки.Содержание  = СвойстваДоходы.СодержаниеПроводки;
			
		Иначе
			
			// Недостачи отражаются в НУ как расход
			
			ОписаниеПроводки.АналитикаДт = ПоляРасходы;
			ОписаниеПроводки.АналитикаКт = НоваяСтруктураПолейПроводки(Выборка);
			ОписаниеПроводки.Содержание  = СвойстваРасходы.СодержаниеПроводки;
			
		КонецЕсли;
			
		ОписаниеПроводки.СчетДт = ОписаниеПроводки.АналитикаДт.Счет;
		ОписаниеПроводки.СчетКт = ОписаниеПроводки.АналитикаКт.Счет;
		ОписаниеПроводки.Суммы  = Выборка;
...
Процедура ДобавитьПроводкиСчетМЦ04(Проводки, МенеджерВременныхТаблиц, Период, Организация)
	...
	// ещё неизвестна, за исключением счетов, возвращаемых методом УчетТоваров.СчетаРассчитываемыеОперативно.
	// Поэтому как проводки Дт 20 Кт 10, так и проводки Дт МЦ.04 имеют нулевую сумму. После расчёта
	...
	// Счета материалов, суммы по которым в момент проведения не известны.
	СчетаМатериалов = ОбщегоНазначенияКлиентСервер.РазностьМассивов(
		БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.Материалы),
		УчетТоваров.СчетаРассчитываемыеОперативно(Организация, Период));
	...
	Запрос.УстановитьПараметр("Счет10",                  СчетаМатериалов);
...
#Область Пересортица // "Разорванные" проводки налогового учета по пересортице

Функция НовыйСвойстваПроводкиПересортица()
	
	Описание = Новый Структура;
	
	Описание.Вставить("Счет",               ПланыСчетов.Хозрасчетный.ПустаяСсылка());
	Описание.Вставить("ВидСубконто",        ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы);
	Описание.Вставить("ЗначениеСубконто",   Справочники.ПрочиеДоходыИРасходы.ПустаяСсылка());
	Описание.Вставить("СодержаниеПроводки", "");
	
	Возврат Описание;
	
КонецФункции

Функция СвойстваПроводкиПересортицаДоходы()
	
	Описание = НовыйСвойстваПроводкиПересортица();
	
	Описание.Счет = ПланыСчетов.Хозрасчетный.ПрочиеДоходы;
	Описание.ЗначениеСубконто = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("ПересортицаТоваровОприходование");
	Описание.СодержаниеПроводки = НСтр("ru = 'Оприходование излишков по результатам инвентаризации'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	Возврат Описание;
	
КонецФункции

Функция СвойстваПроводкиПересортицаРасходы()
	
	Описание = НовыйСвойстваПроводкиПересортица();
	
	Описание.Счет = ПланыСчетов.Хозрасчетный.ПрочиеРасходы;
	Описание.ЗначениеСубконто = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("ПересортицаТоваровСписание");
	Описание.СодержаниеПроводки = НСтр("ru = 'Списание недостач по результатам инвентаризации'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	Возврат Описание;
	
КонецФункции

Функция ПоляПроводкиПересортица(СвойстваПроводки)
	
	Аналитика = Новый Соответствие;
	Аналитика.Вставить(Тип("ПланСчетовСсылка.Хозрасчетный"), СвойстваПроводки.Счет);
	Аналитика.Вставить(СвойстваПроводки.ВидСубконто,         СвойстваПроводки.ЗначениеСубконто);
	
	АналитикаНомераСубконто = УчетЗатрат.АналитикаЗатратНомераСубконто(Аналитика);
	
	Возврат НоваяСтруктураПолейПроводки(АналитикаНомераСубконто);
...
Функция НовыйПротокол(ОписаниеСумм, ВариантыСтруктурыЗатрат, Настройки)
	...
	Протокол.Вставить("Настройки",            Настройки);
	...
	Протокол.РаспределениеНаУслуги.Колонки.Добавить("ПравилоРаспределения",         ПравилаРаспределенияРасходов.ТипИмени());
	...
	Протокол.Вставить("БазаРаспределенияВЭквивалентныхЕдиницах", Новый ТаблицаЗначений);
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("СчетВыпуска",              Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("Подразделение",            БухгалтерскийУчетКлиентСерверПереопределяемый.ОписаниеТиповПодразделения());
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("НоменклатурнаяГруппа",     Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("СтатьяЗатрат",             Новый ОписаниеТипов("СправочникСсылка.СтатьиЗатрат"));
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("Продукция",                Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("ВидБазыРаспределения",     Новый ОписаниеТипов("ПеречислениеСсылка.БазыРаспределенияКосвенныхРасходов"));
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("БазаВыпуск",               Метаданные.РегистрыСведений.РасчетБазыРаспределенияЗатратЭквивалентныеЕдиницы.Ресурсы.БазаВыпуск.Тип);
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Колонки.Добавить("БазаЭквивалентныеЕдиницы", Метаданные.РегистрыСведений.РасчетБазыРаспределенияЗатратЭквивалентныеЕдиницы.Ресурсы.БазаЭквивалентныеЕдиницы.Тип);
	
	Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Индексы.Добавить(РегистрыСведений.РасчетБазыРаспределенияЗатратЭквивалентныеЕдиницы.КлючАналитикиБазыРаспределения());
...
Процедура ПодготовитьСправкаРасчетКалькуляцияСебестоимости(Протокол, Затраты, МенеджерВременныхТаблиц, Период)
	...
	// Тем не менее упорядочиваем результат запроса, так как до группировки он используется для формирования других справок-расчетов
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	// и представим их в виде аналитики расходов (Счет, Подразделение, НоменклатурнаяГруппа, Продукция)
...
Функция СправкаРасчетСебестоимостиПродукции(Протокол, МенеджерВременныхТаблиц, ОписаниеСумм)
	...
	// 1. Возьмем из калькуляции сгруппированные данные о выпуске
	//    (за исключением позиций незавершенного производства без указания продукции).
	...
	ОтборКУдалению = Новый Структура;
	ОтборКУдалению.Вставить("НезавершенноеПроизводство", Истина);
	ОтборКУдалению.Вставить("Продукция",                 Справочники.Номенклатура.ПустаяСсылка());
	
	Для Каждого Запись Из Выпуск.НайтиСтроки(ОтборКУдалению) Цикл
		
		Выпуск.Удалить(Запись);
	...
	// Применим их.
	КешРасчетаБазыРаспределения = Неопределено;
	Для Каждого Распределение Из Протокол.РаспределениеНаУслуги Цикл
		...
		ЗаполнитьРасчетБазыВЭквивалентныхЕдиницах(Протокол, МенеджерВременныхТаблиц, Распределение, КешРасчетаБазыРаспределения);
	КонецЦикла;
	...
	|	МАКСИМУМ(Распределение.ПравилоРаспределения) КАК ПравилоРаспределения,
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьРасчетБазыВЭквивалентныхЕдиницах(Протокол, МенеджерВременныхТаблиц, Выборка, КешРасчетаБазыРаспределения);
	КонецЦикла;
...
Функция ПодготовитьСправкаРасчетНачальногоСальдоПрямыхЗатрат(МенеджерВременныхТаблиц)

	Записи = ЗакрытиеМесяца.НовыйЗаписиВспомогательногоРегистраСведений(
		Метаданные.РегистрыСведений.РасчетНачальногоСальдоПрямыхЗатрат);
	...
	|	ВЫБОР
	|		КОГДА СубконтоЗатрат.ВидСубконто1 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы)
	|			ТОГДА НачальноеСальдоПрямыхРасходов.Субконто1
	|		КОГДА СубконтоЗатрат.ВидСубконто2 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы)
	|			ТОГДА НачальноеСальдоПрямыхРасходов.Субконто2
	|		КОГДА СубконтоЗатрат.ВидСубконто3 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы)
	|			ТОГДА НачальноеСальдоПрямыхРасходов.Субконто3
	|		ИНАЧЕ ЕСТЬNULL(СубконтоЗатрат.ЗначениеПоУмолчанию1, НЕОПРЕДЕЛЕНО)
	|	КОНЕЦ КАК НоменклатурнаяГруппа,
	|	ВЫБОР
	|		КОГДА СубконтоЗатрат.ВидСубконто1 = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат)
	...
	|		ИНАЧЕ ЕСТЬNULL(СубконтоЗатрат.ЗначениеПоУмолчанию3, НЕОПРЕДЕЛЕНО)
	|	КОНЕЦ КАК Продукция,
	|	НачальноеСальдоПрямыхРасходов.Сумма КАК Сумма,
	|	НачальноеСальдоПрямыхРасходов.СуммаНУ КАК СуммаНУ,
	|	НачальноеСальдоПрямыхРасходов.СуммаПР КАК СуммаПР,
	|	НачальноеСальдоПрямыхРасходов.СуммаВР КАК СуммаВР
	|ИЗ
	|	НачальноеСальдоПрямыхРасходов КАК НачальноеСальдоПрямыхРасходов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СубконтоЗатрат КАК СубконтоЗатрат
	|		ПО НачальноеСальдоПрямыхРасходов.Счет = СубконтоЗатрат.Счет
	|ГДЕ
	|	НачальноеСальдоПрямыхРасходов.НезавершенноеПроизводствоПоПродукции
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ЗаполнитьРасчетБазыВЭквивалентныхЕдиницах(Протокол, МенеджерВременныхТаблиц, Выборка, КешРасчетаБазыРаспределения)
	
	ПоляРасчетаБазыРаспределения = "СчетВыпуска, Подразделение, НоменклатурнаяГруппа, Продукция";
	Если КешРасчетаБазыРаспределения = Неопределено Тогда
		КешРасчетаБазыРаспределения = Новый ТаблицаЗначений();
		КешРасчетаБазыРаспределения.Колонки.Добавить("СчетВыпуска");
		КешРасчетаБазыРаспределения.Колонки.Добавить("Подразделение");
		КешРасчетаБазыРаспределения.Колонки.Добавить("НоменклатурнаяГруппа");
		КешРасчетаБазыРаспределения.Колонки.Добавить("Продукция");
		КешРасчетаБазыРаспределения.Индексы.Добавить(ПоляРасчетаБазыРаспределения);
	Иначе
		Отбор = Новый Структура(ПоляРасчетаБазыРаспределения);
		ЗаполнитьЗначенияСвойств(Отбор, Выборка);
		Если КешРасчетаБазыРаспределения.НайтиСтроки(Отбор).Количество() > 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	
	СтрокаКеша = КешРасчетаБазыРаспределения.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаКеша, Выборка);
	
	ОтборПравила = Новый Структура("Имя", Выборка.ПравилоРаспределения);
	СтрокиПравил = Протокол.ПравилаРаспределения.НайтиСтроки(ОтборПравила);
	
	Если СтрокиПравил.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ИмяТаблицыРасчетаБазы = СтрокиПравил[0].БазаРаспределения.РасшифровкаРасчета;
	
	Если МенеджерВременныхТаблиц.Таблицы.Найти(ИмяТаблицыРасчетаБазы) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	БазаРаспределения.Счет КАК СчетВыпуска,
	|	БазаРаспределения.Подразделение КАК Подразделение,
	|	БазаРаспределения.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	|	БазаРаспределения.СтатьяЗатрат КАК СтатьяЗатрат,
	|	БазаРаспределения.Продукция КАК Продукция,
	|	БазаРаспределения.ВидБазыРаспределения КАК ВидБазыРаспределения,
	|	БазаРаспределения.БазаВыпуск КАК БазаВыпуск,
	|	БазаРаспределения.БазаЭквивалентныеЕдиницы КАК БазаЭквивалентныеЕдиницы
	|ИЗ
	|	&ИмяТаблицы КАК БазаРаспределения
	|ГДЕ
	|	БазаРаспределения.Продукция <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицы", ИмяТаблицыРасчетаБазы);
	Результат = Запрос.Выполнить();
	ВыборкаБаза = Результат.Выбрать();
	Пока ВыборкаБаза.Следующий() Цикл
		Отбор = Новый Структура(РегистрыСведений.РасчетБазыРаспределенияЗатратЭквивалентныеЕдиницы.КлючАналитикиБазыРаспределения());
		ЗаполнитьЗначенияСвойств(Отбор, ВыборкаБаза);
		Если Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.НайтиСтроки(Отбор).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		СтрокаПротокола = Протокол.БазаРаспределенияВЭквивалентныхЕдиницах.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаПротокола, ВыборкаБаза);
	КонецЦикла;
