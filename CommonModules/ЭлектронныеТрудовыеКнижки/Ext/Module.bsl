...
Функция ДанныеТрудовойДеятельностиСотрудников(ТолькоРазрешенные, Организация, ФизическоеЛицо = Неопределено, НачалоПериода = Неопределено, ОкончаниеПериода = Неопределено, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
		...
		Для Каждого КадровыеДанныеФизическогоЛица Из КадровыеДанные Цикл
			...
			ДанныеСотрудника.КодКатегории = ПерсонифицированныйУчет.КодКатегорииЗарегистрированногоЛицаЕФС1(
				КадровыеДанныеФизическогоЛица.ВидЗастрахованногоЛица, ОкончаниеПериода);
		КонецЦикла;
			...
			Если ПараметрыФормирования.ДополнитьСведениямиОКодахПоОКЗ
				...
				Если ЗначениеЗаполнено(ДанныеСотрудника.ТрудоваяФункция) Тогда
					ДанныеСотрудника.КодПоОКЗ = ЭлектронныеТрудовыеКнижкиПовтИсп.КодПоОКЗТрудовойФункции(
						ДанныеСотрудника.ТрудоваяФункция, ДанныеСотрудника.ДатаМероприятия);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.36.27";
	Обработчик.Процедура = "ЭлектронныеТрудовыеКнижки.ЗаполнитьКодыКатегорияТрудовыхФункций";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("d4d0abba-3bf7-47b4-8e38-ebfa4827f894");
	Обработчик.Комментарий = НСтр("ru = 'Обновление кодов категорий трудовых функций.'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.31";
	Обработчик.Процедура = "ЭлектронныеТрудовыеКнижки.ЗаполнитьКодыКлассификатора2026годаВТрудовыхФункциях";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("bb0c91e7-2c87-49e2-8b3f-900ad4d3f3c9");
	Обработчик.Комментарий = НСтр("ru = 'Обновление кодов ОКПДТР, классификтора 2026 года.'");
...
Функция ДобавитьПолеСведенийЗаявленийОВеденииТрудовойКнижки(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Функция ДобавитьПолеСведенийОМероприятияхТрудовойДеятельности(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Процедура ДобавитьТекстЗапросаВТСведенийОМероприятияхТрудовойДеятельности(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных) Экспорт
	...
	Если ИсточникиДанных.Получить("СведенияОПоследнихМероприятияхТрудовойДеятельности") <> Неопределено Тогда
		...
		Если ПоляОтбораПериодическихДанных <> Неопределено Тогда
			...
			ИначеЕсли ПоляОтбораПериодическихДанных.Свойство("МероприятияТрудовойДеятельности") Тогда
				Для Каждого ОписаниеОтбора Из ПоляОтбораПериодическихДанных.МероприятияТрудовойДеятельности Цикл
					Если ОписаниеОтбора.ЛевоеЗначение = "Регистратор" Тогда
						ИсключаемыеРегистраторы = ОписаниеОтбора.ПравоеЗначение;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
			...
			ТекстЗапросаПоследнихМероприятий =
				...
				|	И НЕ ЕСТЬNULL(МероприятияОтмененные.Отменено, ЛОЖЬ)
				|	И &УсловияОтбора
				|
				|СГРУППИРОВАТЬ ПО
				|	ТаблицаОтбора.Период,
		...
		Запрос.УстановитьПараметр("ДатаВсутпленияВСилуОКПДТР2026", ЭлектронныеТрудовыеКнижкиКлиентСервер.ДатаВсутпленияВСилуОКПДТР2026());
		...
		ТекстЗапросаПоследнихМероприятий =
			...
			+	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			 	...
			 	|	ВЫБОР
			 	|		КОГДА ИдентификаторыПоследнихМероприятийТрудовойДеятельности.Период < &ДатаВсутпленияВСилуОКПДТР2026
			 	|			ТОГДА Мероприятия.КодПоОКЗ
			 	|		ИНАЧЕ ЕСТЬNULL(Мероприятия.ТрудоваяФункция.КодПоОКЗ2026, ЗНАЧЕНИЕ(Справочник.КлассификаторЗанятий.ПустаяСсылка))
			 	|	КОНЕЦ КАК КодПоОКЗ,
	...
	Если ПроверяемыйОбъект[ИмяРеквизитаДатаМероприятия] >= '20210701' Тогда
		...
		Если ЗначениеЗаполнено(ПроверяемыйОбъект.ТрудоваяФункция) Тогда
			...
			КодПоОКЗ = ЭлектронныеТрудовыеКнижкиПовтИсп.КодПоОКЗТрудовойФункции(
				ПроверяемыйОбъект.ТрудоваяФункция, ПроверяемыйОбъект[ИмяРеквизитаДатаМероприятия]);
...
Функция ПустаяТаблицаСотрудникиДатыДляПроверкиПоследовательностиМероприятий() Экспорт
	...
	СотрудникиДаты.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
...
Процедура ДобавитьСотрудникаВТаблицуСотрудникиДаты(СотрудникиДаты, Сотрудник, ДатаСобытия, НомерСтроки = 0, Возвратное = Ложь) Экспорт
	...
	СтрокаСотрудниковДат.ФизическоеЛицо = КадровыйУчетПовтИсп.ФизическоеЛицоСотрудника(СтрокаСотрудниковДат.Сотрудник);
...
Функция ДокументыВозвратныхМероприятий(СотрудникиДаты, ИсключаемыеРегистраторы) Экспорт
	...
	Запрос.Текст =
		...
		|	СотрудникиДаты.ФизическоеЛицо КАК ФизическоеЛицо,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.МероприятияТрудовойДеятельности КАК Мероприятия
		|		ПО СотрудникиДаты.ФизическоеЛицо = Мероприятия.ФизическоеЛицо
		|			И СотрудникиДаты.Сотрудник = Мероприятия.Сотрудник
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МероприятияТрудовойДеятельности КАК МероприятияОтмененные
		|		ПО (Мероприятия.Организация = МероприятияОтмененные.Организация)
		|			И (Мероприятия.ИдМероприятия = МероприятияОтмененные.ИдМероприятия)
		|			И (МероприятияОтмененные.Отменено)
		...
		|			И Регистраторы.ФизическоеЛицо = Мероприятия.ФизическоеЛицо
...
Процедура ДобавитьЗаписьДляПрежнегоФилиала(ДвиженияДокумента, Запись, ДанныеФилиалов, Период, ОрганизацияДокумента, ОрганизацияПеревода, ВозвратноеМероприятие = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Запись.КодПоОКЗ)
		...
		Запись.КодПоОКЗ = ЭлектронныеТрудовыеКнижкиПовтИсп.КодПоОКЗТрудовойФункции(Запись.ТрудоваяФункция, Период);
	КонецЕсли;
...
Функция ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(МенеджерВременныхТаблиц, ТолькоРазрешенные, Организации, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено, ПараметрыЗаполнения = Неопределено)
	...
	Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
		...
		ДанныеСотрудника.КодКатегории = ПерсонифицированныйУчет.КодКатегорииЗарегистрированногоЛицаЕФС1(
			Выборка.ВидЗастрахованногоЛица, Выборка.ПериодКадровыхДанных);
...
Функция ПредставлениеУсловийРаботы(ВыборкаДанных) Экспорт
	Если Не ВыборкаДанных.ЯвляетсяСовместителем Тогда
		Если ВыборкаДанных.ВидМероприятия <> Перечисления.ВидыМероприятийТрудовойДеятельности.Увольнение Тогда
			...
			ИначеЕсли ВыборкаДанных.Дата < ПерсонифицированныйУчет.ДатаПостановленияЕФС1_2026() Тогда
				Если ВыборкаДанных.УдаленностьРаботы = Перечисления.УдаленностиРаботы.Надомная Тогда
...
Функция ЗапросВТМероприятияТрудовойДеятельности(ТолькоРазрешенные, ИсключаемыйРегистратор, ИмяВТМероприятияТрудовойДеятельности)
	...
	ТекстЗапроса =
		...
		|	ЕСТЬNULL(МероприятияПереданные.КодПоОКЗ, МероприятияПрочие.КодПоОКЗ) КАК КодПоОКЗ,
		|	МероприятияПрочие.ТерриториальныеУсловия КАК ТерриториальныеУсловия,
		|	ЕСТЬNULL(МероприятияПереданные.ПричинаУвольненияПФР, МероприятияПрочие.ПричинаУвольненияПФР) КАК ПричинаУвольненияПФР,
		|	ЕСТЬNULL(МероприятияПереданные.СрочностьТрудовогоДоговора, МероприятияПрочие.СрочностьТрудовогоДоговора) КАК СрочностьТрудовогоДоговора,
		|	ЕСТЬNULL(МероприятияПереданные.УдаленностьРаботы, МероприятияПрочие.УдаленностьРаботы) КАК УдаленностьРаботы,
		|	ЕСТЬNULL(МероприятияПереданные.СокращенностьГрафика, МероприятияПрочие.СокращенностьГрафика) КАК СокращенностьГрафика,
...
Функция ЗапросВТВсеМероприятияДоНачалаУчета(ТолькоРазрешенные, ИсключаемыйРегистратор, ИмяВТВсеМероприятияДоНачалаУчета)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(МероприятияПереданные.КодПоОКЗ, МероприятияТрудовойДеятельности.КодПоОКЗ) КАК КодПоОКЗ,
		|	МероприятияТрудовойДеятельности.ТерриториальныеУсловия КАК ТерриториальныеУсловия,
		|	ЕСТЬNULL(МероприятияПереданные.ПричинаУвольненияПФР, МероприятияТрудовойДеятельности.ПричинаУвольненияПФР) КАК ПричинаУвольненияПФР,
		|	ЕСТЬNULL(МероприятияПереданные.СрочностьТрудовогоДоговора, МероприятияТрудовойДеятельности.СрочностьТрудовогоДоговора) КАК СрочностьТрудовогоДоговора,
		|	ЕСТЬNULL(МероприятияПереданные.УдаленностьРаботы, МероприятияТрудовойДеятельности.УдаленностьРаботы) КАК УдаленностьРаботы,
		|	ЕСТЬNULL(МероприятияПереданные.СокращенностьГрафика, МероприятияТрудовойДеятельности.СокращенностьГрафика) КАК СокращенностьГрафика,
...
Процедура ЗаполнитьКодыКлассификатора2026годаВТрудовыхФункциях(ПараметрыОбновления = Неопределено) Экспорт
	
	ЭлектронныеТрудовыеКнижкиВнутренний.ЗаполнитьКодыКлассификатора2026годаВТрудовыхФункциях(ПараметрыОбновления);
	
КонецПроцедуры

Процедура ЗаполнитьКодыКатегорияТрудовыхФункций(ПараметрыОбновления = Неопределено) Экспорт
	
	ЭлектронныеТрудовыеКнижкиВнутренний.ЗаполнитьКодыКатегорияТрудовыхФункций(ПараметрыОбновления);
