...
Функция ЭтоОшибочноМногострочноеВыражение(Контекст, Выражение)
	Если СтрЧислоВхождений(Выражение, "[") > 3 Тогда
		...
		СтрокаЧислом = Контекст.ОТЧ.ПривестиЗначение(АргСкобок);
		Если СтрокаЧислом > 0 И Цел(СтрокаЧислом) = СтрокаЧислом Тогда
			Контекст.СтруктураПоискаСекцияСтрока.row = Прав("00000" + АргСкобок, 5);
			Для Каждого Стр Из Контекст.ОсновнойПериод_ДанныеОтчета.НайтиСтроки(Контекст.СтруктураПоискаСекцияСтрока) Цикл 
				Если ЗначениеЗаполнено(Стр.s1) Или Стр.s1_IsPresent
					Или ЗначениеЗаполнено(Стр.s2) Или Стр.s2_IsPresent
					Или ЗначениеЗаполнено(Стр.s3) Или Стр.s3_IsPresent Тогда 
...
Функция ЭтоСкалярноеУсловие(Контекст, Условие)
	Если ЭтоСравнениеСуммированияСОтборомПоСпецифике(Контекст, Условие) Тогда 
		Возврат Ложь;
	КонецЕсли;
...
Функция СтрокаВходитВДиапазон(Контекст, Выборка, Стр, СмотретьСпецифики = Истина)
...
Функция ЭтоСравнениеСуммированияСОтборомПоСпецифике(Контекст, Условие)
	Аргументы = ПолучитьВсеАргументыКромеСуммированийВВыражении(Контекст, Условие);
	Если Аргументы.Количество() <> 1  Тогда 
		Возврат Ложь;
	КонецЕсли;
	Аргумент = ПервыйКлючСоответсвия(Аргументы);
	Если СтрЧислоВхождений(Аргумент, "[") <> 4 Или ПолучитьАргументСкобки(Аргумент, 4) <> "*" Тогда 
		Возврат Ложь;
	КонецЕсли;
	Суммирования = ПолучитьВсеАргументыСуммированияВВыражении(Условие);
	Если Суммирования.Количество() = 0 Тогда 
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого АргСуммирования Из Суммирования Цикл 
		Если СтрЧислоВхождений(АргСуммирования.Ключ, "[") <> 5 
			Или ПолучитьАргументСкобки(АргСуммирования.Ключ, 4) <> "*"
			Или ПолучитьАргументСкобки(АргСуммирования.Ключ, 5) <> "*" Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ВычислитьСравнениеСуммированияСОтборомПоСпецифике(Контекст, Условие, РазрядностьОкругления, Строки)
	КлючСравнения = ПолучитьПервыйКлючСоответствия(ПолучитьВсеАргументыКромеСуммированийВВыражении(Контекст, Условие));
	Контекст.СтруктураПоискаСекцияСтрока.section = Прав("000" + ПолучитьАргументСкобки(Условие, 1), 3);
	Контекст.СтруктураПоискаСекцияСтрока.row = Прав("00000" + ПолучитьАргументСкобки(Условие, 2), 5);
	СпецификиОтбора = Новый Соответствие;
	Для Каждого Стр Из Строки.НайтиСтроки(Контекст.СтруктураПоискаСекцияСтрока) Цикл 
		Если Стр.s1_isPresent И ЗначениеЗаполнено(Стр.s1) Тогда 
			СпецификиОтбора.Вставить(Стр.s1);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого КЗ Из СпецификиОтбора Цикл 
		НаЧтоЗаменяем = "";
		Для Каждого Стр Из Строки Цикл 
			Если Стр.row = Контекст.СтруктураПоискаСекцияСтрока.row Тогда 
				Продолжить;
			КонецЕсли;
			
			Если Стр.s1_IsPresent И Стр.s1 = КЗ.Ключ Тогда
				НаЧтоЗаменяем = "[" + КЗ.Ключ + "][*]";
			ИначеЕсли Стр.s2_IsPresent И Стр.s2 = КЗ.Ключ Тогда 
				НаЧтоЗаменяем = "[*][" + КЗ.Ключ + "]";
			КонецЕсли;
		КонецЦикла;
		
		Условие = СтрЗаменить(Условие, КлючСравнения, СтрЗаменить(КлючСравнения, "[*]", "[" + КЗ.Ключ + "]"));
		Для Каждого АргСум Из ПолучитьВсеАргументыСуммированияВВыражении(Условие) Цикл 
			Условие = СтрЗаменить(Условие, АргСум.Ключ, СтрЗаменить(АргСум.Ключ, "[*][*]", НаЧтоЗаменяем));
		КонецЦикла;
		
		Если Не ВычислитьСкалярноеУсловие(Условие, Контекст, РазрядностьОкругления) Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ВычислитьУсловиеПоСтрокам(Условие, Контекст, РазрядностьОкругления = 2, Строки = Неопределено)
	Если ЭтоСравнениеСуммированияСОтборомПоСпецифике(Контекст, Условие) Тогда 
		Возврат ВычислитьСравнениеСуммированияСОтборомПоСпецифике(Контекст, Условие, РазрядностьОкругления, Строки);
	КонецЕсли;
...
Процедура ПреобразоватьСравнениеРазныхСтрокСоСкаляром(Контекст)
	КСДляЗамены = Новый Соответствие;
	
	Для Каждого Стр Из Контекст.ТаблицаКонтрольныхСоотношений Цикл 
		Аргументы = ПолучитьВсеАргументыКромеСуммированийВВыражении(Контекст, Стр.rule);
		Если Аргументы.Количество() <> 1  Тогда 
			Продолжить;
		КонецЕсли;
		Аргумент = ПервыйКлючСоответсвия(Аргументы);
		СтрокаЧислом = Контекст.ОТЧ.ПривестиЗначение(ПолучитьАргументСкобки(Аргумент, 2));
		Если ЗначениеЗаполнено(СтрокаЧислом) И СтрокаЧислом = Цел(СтрокаЧислом) Тогда 
			Продолжить;
		КонецЕсли;
		Суммирования = ПолучитьВсеАргументыСуммированияВВыражении(Стр.rule);
		Если Суммирования.Количество() = 0  Тогда 
			Продолжить;
		КонецЕсли;
		
		УсловиеПоГрафам = Новый Соответствие;
		УсловиеПоГрафам.Вставить(ПолучитьАргументСкобки(Аргумент, 3));
		Для Каждого АргСум Из Суммирования Цикл 
			УсловиеПоГрафам.Вставить(ПолучитьАргументСкобки(АргСум.Ключ, 3));
		КонецЦикла;
		Если УсловиеПоГрафам.Количество() <> 1  Тогда 
			Продолжить;
		КонецЕсли;
		
		КСДляЗамены.Вставить(Стр, Новый Структура("АргументДляРаспределения", Аргумент));
	КонецЦикла;
	
	ПоискПоСекции = Новый Структура("section");
	КСКУдалению = Новый Соответствие;
	Для Каждого КЗ Из КСДляЗамены Цикл 
		АргументДляРаспределения = КЗ.Значение.АргументДляРаспределения;
		ПоискПоСекции.section = Прав("000" + ПолучитьАргументСкобки(АргументДляРаспределения, 1), 3);
		Строки = ПолучитьАргументСкобки(АргументДляРаспределения, 2);
		
		АргументДляРаспределенияНачало = Лев(АргументДляРаспределения,
			СтрНайти(АргументДляРаспределения, "[", НаправлениеПоиска.СНачала, 1, 2));
		АргументДляРаспределенияКонец = Сред(АргументДляРаспределения,
			СтрНайти(АргументДляРаспределения, "]", НаправлениеПоиска.СНачала, 1, 2));
		УИД_КС = Новый УникальныйИдентификатор;
		Для Каждого ОтборПоСекции Из Контекст.ТаблицаТиповСтрок.НайтиСтроки(ПоискПоСекции) Цикл 
			Если СтрокаВходитВДиапазон(Контекст, АргументДляРаспределения, ОтборПоСекции, Ложь) Тогда
				НовоеУсловие = Контекст.ТаблицаКонтрольныхСоотношений.Добавить();
				ЗаполнитьЗначенияСвойств(НовоеУсловие, КЗ.Ключ);
				НовоеУсловие.uuid = УИД_КС;
				
				НовыйАргументДляРаспределения = АргументДляРаспределенияНачало 
					+ Формат(Контекст.ОТЧ.ПривестиЗначение(ОтборПоСекции.row), "ЧГ=") 
					+ АргументДляРаспределенияКонец;
				НовоеУсловие.rule = СтрЗаменить(КЗ.Ключ.rule, АргументДляРаспределения, НовыйАргументДляРаспределения);
				КСКУдалению.Вставить(КЗ.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого КЗ Из КСКУдалению Цикл
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки)
	...
	ПреобразоватьСравнениеРазныхСтрокСоСкаляром(Контекст);
	Для Каждого Стр Из Контекст.ТаблицаКонтрольныхСоотношений Цикл
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ПроверитьКонтрольныеСоотношенияВОтчетеВФоне", Истина);
КонецПроцедуры
