	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.ПолучитьЭлементы() Цикл
		...
		ИначеЕсли СтрокаДерева.ТипУпаковки = ПредопределенноеЗначение("Перечисление.ПрочиеЗоныПересчетаПродукцииИСМП.БлокиБезКоробки") Тогда
			...
			Для Каждого ГрупповаяУпаковкаИлиНабор Из СтрокаДерева.ПолучитьЭлементы() Цикл
				ОпределитьТипУпаковки(ГрупповаяУпаковкаИлиНабор, Истина, ПараметрыЗаполнения, МодульКонтекста);
			КонецЦикла;
...
Процедура УстановитьИндексКартинкиСостояниеПодбораКодаМаркировки(ТекущаяСтрока, СвойстваВидаПродукции) Экспорт
	...
	ИначеЕсли РазрешенОборотНемаркируемыхОстатков Тогда
		...
		Если ТекущаяСтрока.КоличествоПодобрано < ТекущаяСтрока.Количество
			...
			ТекущаяСтрока.ИндексКодаМаркировки = 7;
			
		Иначе
			
			ТекущаяСтрока.ИндексКодаМаркировки = 6;
...
Функция ДопустимыеВложенияАгрегатовПоВидамПродукции() Экспорт
	...
	ВложенияПоТипуШтрихкодаSSCC.Добавить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18"));
	ВложенияПоТипуШтрихкодаSSCC.Добавить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20"));
	...
	ВложенияПоТипуШтрихкодаGS1.Добавить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18"));
	ВложенияПоТипуШтрихкодаGS1.Добавить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20"));
	...
	ДопустимыеВложенияАгрегатов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC18"),        ВложенияПоТипуШтрихкодаSSCC);
	ДопустимыеВложенияАгрегатов.Вставить(ПредопределенноеЗначение("Перечисление.ТипыШтрихкодов.SSCC20"),        ВложенияПоТипуШтрихкодаSSCC);
	...
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.СпортивноеПитание"), ДопустимыеВложенияАгрегатов);
	...
	Результат.Вставить(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.НапиткиССодержаниемМолокаКакаоПрочие"), ДопустимыеВложенияАгрегатов);
...
Процедура ОпределитьТипУпаковки(СтрокаДерева, ВключаяПодчиненные, ПараметрыЗаполнения, МодульКонтекста)
	...
	ЭтоОднородныйНабор = ЭтоНабор
		...
		И (ПодчиненныеСтроки.Количество() = 0
			Или (СтрокаДерева.Номенклатура = ПодчиненнаяСтрока.Номенклатура
...
Функция РазрешительнаяДокументацияТребуетПроверкиПоДатеПроизводства(СтрокаДанных, ПараметрыПроверкиКодовМаркировки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыПроверкиКодовМаркировки.ДатаКонтроляРазрешительнойДокументации)
	...
	Иначе
		...
		ДатаНачалаКонтроляРазрешительнойДокументации = ПараметрыПроверкиКодовМаркировки.ДатаКонтроляРазрешительнойДокументации;
