...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗарплатаКадрыОбновлениеС20");
	Методы.Вставить("ЗаполнитьГоловныхСотрудниковВКадровойИстории");
...
Процедура ЗарплатаКадрыОбновлениеС20()Экспорт
	...
	КонвертироватьСправочникКлассификаторЗанятий(СоответствиеКонвертированныхОбъектов);
	ТрудовыеФункцииКонвертация(СоответствиеКонвертированныхОбъектов);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		МодульРасчетЗарплатыДляНебольшихОрганизаций.УстановитьФОРасчетЗарплатыДляНебольшихОрганизаций();
	КонецЕсли;
...
Функция КонвертироватьСсылочныеДанные(СсылкаНаКонверируемоеЗначение, КонвертируемыеДанные, СоответствиеКонвертированныхОбъектов, СсылкаНаПриемник)
	...
	ИначеЕсли ТипЗначения = Тип("СправочникСсылка.УдалитьКлассификаторЗанятий") Тогда 
		ВозвращаемоеЗначение = КонвертироватьЭлементКлассификатораЗанятий(КонвертируемыеДанные, СоответствиеКонвертированныхОбъектов, СсылкаНаПриемник);
...
Процедура КонвертироватьСправочникКлассификаторЗанятий(СоответствиеКонвертированныхОбъектов)
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьКлассификаторЗанятий.Ссылка КАК Ссылка,
	|	УдалитьКлассификаторЗанятий.ВерсияДанных КАК ВерсияДанных,
	|	УдалитьКлассификаторЗанятий.ПометкаУдаления КАК ПометкаУдаления,
	|	УдалитьКлассификаторЗанятий.Предопределенный КАК Предопределенный,
	|	УдалитьКлассификаторЗанятий.Код КАК Код,
	|	УдалитьКлассификаторЗанятий.КонтрольноеЧисло КАК КонтрольноеЧисло,
	|	УдалитьКлассификаторЗанятий.НаименованиеГруппЗанятий КАК НаименованиеГруппЗанятий,
	|	КлассификаторЗанятий.Ссылка КАК ПриемникСсылка
	|ИЗ
	|	Справочник.УдалитьКлассификаторЗанятий КАК УдалитьКлассификаторЗанятий
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлассификаторЗанятий КАК КлассификаторЗанятий
	|		ПО УдалитьКлассификаторЗанятий.Код = КлассификаторЗанятий.Код";
...
Функция КонвертироватьЭлементКлассификатораЗанятий(КонвертируемыеДанные, СоответствиеКонвертированныхОбъектов, СсылкаНаПриемник = Неопределено)
	
	Элемент = ПолучитьСсылкуНаНовыйЭлемент(КонвертируемыеДанные, СоответствиеКонвертированныхОбъектов, СсылкаНаПриемник);
	
	Если Элемент = Неопределено Тогда
		Справочники.КлассификаторЗанятий.ПустаяСсылка();
	Иначе
		Возврат Элемент.Ссылка;
	КонецЕсли;
...
//
Процедура ТрудовыеФункцииКонвертация(СоответствиеКонвертированныхОбъектов)
	
	ВыборкаТрудовыхФункций= Справочники.ТрудовыеФункции.Выбрать();
	
	Пока ВыборкаТрудовыхФункций.Следующий() Цикл 
		Если НЕ ВыборкаТрудовыхФункций.ЭтоГруппа Тогда
			ТрудовыеФункцииОбъект = ВыборкаТрудовыхФункций.ПолучитьОбъект();
			ТрудовыеФункцииОбъект.КодПоОКЗ = КонвертироватьДанные(ВыборкаТрудовыхФункций.УдалитьКодПоОКЗ, СоответствиеКонвертированныхОбъектов);
			ТрудовыеФункцииОбъект.Записать();
		КонецЕсли;	
	КонецЦикла;	
КонецПроцедуры	
		...
		Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("БИК") Цикл  
				...
				Если НЕ ЗначениеЗаполнено(ЗарплатныйПроект) Тогда
					Банк = РаботаСБанкамиБП.СсылкаПоКлассификатору(Выборка.БИК);
...
Процедура КонверитроватьДанныеОВидеЗанятостиСотрудникаДокумента(ДанныеВыгружаемогоДокумента, СоответствиеКонвертированныхОбъектов, НовыйРегистратор = Неопределено)
	...
	КлючПорцииДанных = Новый Структура("Регистратор, Сотрудник", ДанныеВыгружаемогоДокумента.Ссылка, ДанныеВыгружаемогоДокумента.Сотрудник);
	ДанныеКадровойИстории = ДанныеКадровойИстории(КлючПорцииДанных, Ложь);
	Если ДанныеКадровойИстории.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗарегистрироватьВзаиморасчеты(ВыборкаПоВзаиморасчетам, СоответствиеКонвертируемыхОбъектов, СотрудникиОрганизаций,  НовыйРегистратор = Неопределено)
	...
	СтатьяОплатаТруда = Справочники.СтатьиРасходовЗарплата.НайтиПоКоду("ОТ");
	...
	Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("Организация") Цикл
		Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("Период") Цикл
			Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("ПериодВзаиморасчетов") Цикл	
				...
				ТаблицаНачислений.Колонки.Добавить("ВидДоходаИсполнительногоПроизводства");
				ТаблицаНачислений.Колонки.Добавить("СтатьяРасходов");
				...
				ТаблицаУдержаний.Колонки.Добавить("ВидДоходаИсполнительногоПроизводства");
				ТаблицаУдержаний.Колонки.Добавить("СтатьяРасходов");
				...
				Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
				    Пока ВыборкаПоВзаиморасчетам.СледующийПоЗначениюПоля("ВидДвижения") Цикл 
						...
						Пока ВыборкаПоВзаиморасчетам.Следующий() Цикл 
							...
							Если Не ЗначениеЗаполнено(СтрокаТаблицы.ВидДоходаИсполнительногоПроизводства) Тогда
								СтрокаТаблицы.ВидДоходаИсполнительногоПроизводства = ВидДоходаЗарплата;
							КонецЕсли;
							Если Не ЗначениеЗаполнено(СтрокаТаблицы.СтатьяРасходов) Тогда
								СтрокаТаблицы.СтатьяРасходов = СтатьяОплатаТруда;
							КонецЕсли;
...
Процедура ЗарегистрироватьНачисления(ВыборкаПоНачислениям, Регистратор, СоответствиеКонвертированныхОбъектов = Неопределено)
	...
	Если ТаблицаНачислений.Количество() > 0 Тогда
		...
		ДанныеДляПроведенияУчетЗарплаты.ПланируемаяДатаВыплаты	= ВыборкаПоНачислениям.ПериодРегистрации;
	...
	НаборЗаписей = Регистратор.Движения.Найти("ВзаиморасчетыССотрудниками");
	Если НаборЗаписей <> Неопределено Тогда
		НаборЗаписей.Очистить();
	КонецЕсли;
	НаборЗаписей = Регистратор.Движения.Найти("ЗарплатаКВыплате");
	Если НаборЗаписей <> Неопределено Тогда
		НаборЗаписей.Очистить();
	КонецЕсли;
	...
	Пока ДанныеПоРасчетамНДФЛ.СледующийПоЗначениюПоля("Период") Цикл
		Пока ДанныеПоРасчетамНДФЛ.СледующийПоЗначениюПоля("Организация") Цикл
			...
			Если ТаблицаУдержанийНДФЛ.Количество() > 0 Тогда
				...
				УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛПоСотрудникам(ДанныеДляПроведенияУчетЗарплаты, Ложь, ТаблицаУдержанийНДФЛ);
			КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				МодульРасчетЗарплатыДляНебольшихОрганизаций.СформироватьТабличнуюЧастьСотрудники(ДокументОбъект);
