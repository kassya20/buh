...
Процедура СформироватьДвиженияНачисленийПоДоговорам(Движения, Отказ, Организация, ПериодРегистрации, Начисления) Экспорт
	...
	РасчетЗарплатыВнутренний.СформироватьДвиженияНачисленийПоДоговорам(Движения, Отказ, Организация, ПериодРегистрации, Начисления);
...
//
// Параметры:
//		КатегорииУдержаний - Массив элементов типа Перечисление.КатегорииУдержаний
...
//
Функция УдержанияПоКатегориям(КатегорииУдержаний, Отбор = Неопределено) Экспорт
...
Функция УдержанияПоКатегории(КатегорияУдержания, Отбор = Неопределено) Экспорт
	...
	Если ТипЗнч(КатегорияУдержания) = Тип("Массив") Тогда
		МассивКатегорий = КатегорияУдержания;
	Иначе
		МассивКатегорий = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(КатегорияУдержания);
	КонецЕсли;
	
	Результат = УдержанияПоКатегориям(МассивКатегорий, Отбор);
	Возврат Результат;
...
Функция РекомендуемыеКорректировкиВыплаты(Налоги, ВозвратНДФЛПрошлыхЛет, ТекущиеКорректировкиВыплаты, ГодНалоговогоПериода = НеОпределено) Экспорт
	...
	ПредыдущееФизическоеЛицо = Неопределено;
	ПредыдущаяНалоговаяБаза = Неопределено;
	Для Каждого СтрокаНалога Из Налоги Цикл
		Если СтрокаНалога.ФизическоеЛицо <> ПредыдущееФизическоеЛицо Или СтрокаНалога.НалоговаяБаза <> ПредыдущаяНалоговаяБаза Тогда
			ПредыдущееФизическоеЛицо = СтрокаНалога.ФизическоеЛицо;
			ПредыдущаяНалоговаяБаза = СтрокаНалога.НалоговаяБаза;
			ПараметрыОтбораСтрокКорректировки = Новый Структура("ФизическоеЛицо,НалоговаяБаза");
			ЗаполнитьЗначенияСвойств(ПараметрыОтбораСтрокКорректировки,СтрокаНалога);
			ТекущаяКорректировка = 0;
			ОстатокВзаиморасчетов = 0;
			СтрокиКорректировок = ТекущиеКорректировкиВыплаты.Скопировать(ПараметрыОтбораСтрокКорректировки);
			Если СтрокаНалога.НалоговыйПериод = ГодНалоговогоПериода И СтрокиКорректировок.Количество() > 0 Тогда
				ПараметрыОтбораСтрокКорректировки.Удалить("НалоговаяБаза");
		...
		Если СтрокаНалога.ИсчисленоНалога < 0 ИЛИ ТекущаяКорректировка < 0 Тогда
			...
			КонецЕсли;
			ТекущаяКорректировка = ТекущаяКорректировка - НоваяСтрока.КорректировкаВыплаты;
		ИначеЕсли СтрокаНалога.ИсчисленоНалога < ТекущаяКорректировка Тогда
			...
			Для Каждого СтрокаКорректировки Из СтрокиКорректировок Цикл
				...
				СтрокаКорректировки.СуммаКорректировки = СтрокаКорректировки.СуммаКорректировки - НоваяСтрока.КорректировкаВыплаты;
				ТекущаяКорректировка = ТекущаяКорректировка - НоваяСтрока.КорректировкаВыплаты;
				...
				Если МаксимальныйВесСтроки <= ВесСтроки Тогда
					...
					СамаяТяжелаяСтрокаКорректировки = СтрокаКорректировки;
				КонецЕсли;
			КонецЦикла;
			
			Если МаксимальныйВесСтроки > 0 Тогда
				...
				СамаяТяжелаяСтрокаКорректировки.СуммаКорректировки = СамаяТяжелаяСтрокаКорректировки.СуммаКорректировки - (КорректировкаВыплаты - Распределено);
				ТекущаяКорректировка = ТекущаяКорректировка - (КорректировкаВыплаты - Распределено);
			КонецЕсли;
		...
		Иначе
			...
			Для Каждого СтрокаКорректировки Из СтрокиКорректировок Цикл
				...
				ТекущаяКорректировка = ТекущаяКорректировка - НоваяСтрока.КорректировкаВыплаты;
				СтрокаКорректировки.СуммаКорректировки = 0;
			КонецЦикла;
		КонецЕсли;
...
Процедура ДополнитьВТНачисленияЕдиноличныйРуководительКоммерческойОрганизации(Запрос, ДанныеДляПроведения, Организация, ПериодРегистрации) Экспорт

	Если Не УчетСтраховыхВзносов.АвтоматическиДополнятьБазуРуководителя(Организация, ПериодРегистрации) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Выполнить();
	ОписаниеФильтра = ЗарплатаКадрыПериодическиеРегистры.ОписаниеФильтраДляСоздатьВТИмяРегистраПоВременнойТаблице("ВТФильтрСотрудников", "Сотрудник");
	
	ЗарплатаКадрыПериодическиеРегистры.СоздатьВТИмяРегистра("КадроваяИсторияСотрудников", ДанныеДляПроведения.МенеджерВременныхТаблиц, Истина, ОписаниеФильтра, ПараметрыПостроения, "ВТКадроваяИстория");
		
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КадроваяИстория.Сотрудник КАК Сотрудник,
	|	МАКСИМУМ(Должности.ЯвляетсяДолжностьюЕдиноличногоРуководителяКоммерческойОрганизации) КАК ЕдиноличныйРуководительКоммерческойОрганизации
	|ПОМЕСТИТЬ ВТСотрудникиПризнакЕдиноличногоРуководителя
	|ИЗ
	|	ВТКадроваяИстория КАК КадроваяИстория
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Должности КАК Должности
	|		ПО КадроваяИстория.Должность = Должности.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ВЫБРАТЬ
	|	&ИменаПолей,
	|	СотрудникиПризнакЕдиноличногоРуководителя.ЕдиноличныйРуководительКоммерческойОрганизации КАК ЕдиноличныйРуководительКоммерческойОрганизации
	|ПОМЕСТИТЬ ВТНачисленияПредварительно
	|ИЗ
	|	ВТНачисления КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиПризнакЕдиноличногоРуководителя КАК СотрудникиПризнакЕдиноличногоРуководителя
	|		ПО Начисления.Сотрудник = СотрудникиПризнакЕдиноличногоРуководителя.Сотрудник";
	
	ЗарплатаКадры.ЗаполнитьИменаПолейВТ(Запрос.Текст, ДанныеДляПроведения.МенеджерВременныхТаблиц, "ВТНачисления", "Начисления", Новый Массив);
	Запрос.Выполнить();
	...
	ЗарплатаКадры.ЗаполнитьИменаПолейВТ(Запрос.Текст, ДанныеДляПроведения.МенеджерВременныхТаблиц, "ВТНачисленияПредварительно", "Начисления", Новый Массив);
	Запрос.Выполнить();
	
	УдалитьВТ = Новый Массив;
	УдалитьВТ.Добавить("ВТКадроваяИстория");
	УдалитьВТ.Добавить("ВТФильтрСотрудников");
	УдалитьВТ.Добавить("ВТНачисленияПредварительно");
...
Функция ТаблицаКорректировкиВыплаты()
	...
	КорректировкиВыплаты.Колонки.Добавить("НалоговаяБаза", Новый ОписаниеТипов("ПеречислениеСсылка.КатегорииДоходовНДФЛ"));
...
Процедура СоздатьВТДанныеКорректировкиВыплатыПоДокументу(МенеджерВременныхТаблиц, ДокументСсылка, ОписаниеТаблицДокумента = Неопределено, ФизическиеЛица = Неопределено) Экспорт
	...
	|	ТаблицаКорректировкиВыплаты.НалоговаяБаза
...
//		
Процедура ЗаполнитьДокументНачисленияЗарплаты(ДанныеНачисленияЗарплаты, ПерваяПоловинаМесяца = Ложь, ПланируемаяДатаВыплаты = '00010101') Экспорт
	РасчетЗарплатыВнутренний.ЗаполнитьДокументНачисленияЗарплаты(ДанныеНачисленияЗарплаты, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
КонецПроцедуры	
...
//		Результат
//
Функция РезультатНачисленияРасчетаЗарплаты(Организация, МесяцНачисления, Документ, Подразделение = Неопределено, Сотрудники = Неопределено, ПерваяПоловинаМесяца = Ложь, ПланируемаяДатаВыплаты = '00010101') Экспорт
	Возврат РасчетЗарплатыВнутренний.РезультатНачисленияРасчетаЗарплаты(Организация, МесяцНачисления, Документ, Подразделение, Сотрудники, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
КонецФункции
...
Процедура ПриПримененииТарифаСтраховыхВзносов(Источник, Отказ, Замещение) Экспорт
	...
	РасчетЗарплатыВнутренний.ПриПримененииТарифаСтраховыхВзносов(Источник, Отказ, Замещение);
