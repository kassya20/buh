...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьОтображениеПомеченныхНаУдаление");
...
Процедура СкрытьПомеченныеНаУдаление(Форма, ИмяСписка = "Список") Экспорт
	...
	Если Не СкрыватьПомеченныеНаУдаление() Или (Форма.Параметры.Свойство("Отбор")
		И Форма.Параметры.Отбор.Свойство("ПометкаУдаления")) Тогда
		Возврат;
	КонецЕсли;
