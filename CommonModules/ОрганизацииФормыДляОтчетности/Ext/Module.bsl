...
Функция СведенияОНалоговомОрганеПоАдресу(Знач ЗначенияАдресаJSON, Знач ЮридическоеФизическоеЛицо)
	...
	СведенияОНалоговомОрганеПоАдресу = АдресныйКлассификатор.КодыАдреса(ЗначенияАдресаJSON, АдресныйКлассификаторБП.ИсточникПолученияСведенийОбАдресныхСведениях());
...
Процедура ЗаписатьРегистрациюВНалоговомОргане(Объект, ЗначенияПолейРегистрации, РеквизитыНалоговогоОргана = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НаборЗаписей) Тогда
		...
		СтрокаНабораЗаписей.СтруктурнаяЕдиница = Объект.Ссылка;
		СтрокаНабораЗаписей.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане.Ссылка;
		НаборЗаписей.Записать();
	ИначеЕсли НаборЗаписей.Выгрузить().Найти(РегистрацияВНалоговомОргане.Ссылка, "РегистрацияВНалоговомОргане") = Неопределено Тогда
		МенеджерЗаписи = РегистрыСведений.ИсторияРегистрацийВНалоговомОргане.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Период = РегистрыСведений.ИсторияРегистрацийВНалоговомОргане.ДатаОтсчетаПериодическихСведений();
		МенеджерЗаписи.СтруктурнаяЕдиница = Объект.Ссылка;
		МенеджерЗаписи.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане.Ссылка;
...
//
// Параметры:
//  ОтветДанныхЕГР - Структура - см. ОрганизацииФормыДляОтчетностиКлиентСервер.НовыйОтветДанныхЕГР(), результат запроса к сервису, предоставляющему данные госреестров
...
Процедура ЗаполнитьДаннымиЕГР(ОтветДанныхЕГР, Форма) Экспорт
	...
	Если ОтветДанныхЕГР.КраткиеДанные.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если ДанныеЕГР.Свойство("Телефон") Тогда
	...
	Если ЗначениеЗаполнено(ДанныеЕГР.РегистрацияВНалоговомОргане) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.КодНалоговогоОргана) Тогда
			Объект.КодНалоговогоОргана = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ДанныеЕГР.РегистрацияВНалоговомОргане,
				"Код");
			Объект.НаименованиеНалоговогоОргана = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ДанныеЕГР.РегистрацияВНалоговомОргане,
				"Наименование");
		...
		Если Не ЗначениеЗаполнено(Форма.КодПоОКТМО) Тогда
			Форма.КодПоОКТМО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЕГР.РегистрацияВНалоговомОргане, "ОКТМО");
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеЕГР.РегистрацияВПенсионномФонде) Тогда
		...
		РегистрационныйНомерПФР = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ДанныеЕГР.РегистрацияВПенсионномФонде,
		...
		Объект.КодОрганаПФР = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ДанныеЕГР.РегистрацияВПенсионномФонде,
			"КодОрганаПФР");
	...
	Если ЗначениеЗаполнено(ДанныеЕГР.РегистрацияВСФР) Тогда
		РегистрационныйНомерСФР = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			ДанныеЕГР.РегистрацияВСФР, "РегистрационныйНомерСФР");
		Если ОтветДанныхЕГР.КраткиеДанные.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
	...
	Если ЗначениеЗаполнено(ДанныеЕГР.РегистрацияВФСС) Тогда
		...
		КодПодчиненности = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеЕГР.РегистрацияВФСС, "КодПодчиненности");
...
Функция ПоказыватьФормуСокращенно(Форма)
	...
	// Если ИНН заполнен или статус физического лица Самозанятый, форма сокращается до обязательных незаполненных реквизитов
	Возврат ЗначениеЗаполнено(Форма.Объект.ИНН) Или
		Форма.Объект.СтатусФизическогоЛица = Перечисления.СтатусыФизическихЛиц.Самозанятый;
...
Функция ПроверяемыеДанныеОрганизации(Организация, ПроверяемыеРеквизиты)
	...
	|	Организации.ИПРегистрационныйНомерФСС КАК ИПРегистрационныйНомерФСС,
