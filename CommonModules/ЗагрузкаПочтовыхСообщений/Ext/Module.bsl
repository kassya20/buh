...
Функция ПрочитатьПисьма(ПараметрыЧтенияПисем) Экспорт
	...
	Если ПараметрыЧтенияПисем.Свойство("МаксимальныйРазмерПисьма") Тогда
		МаксимальныйРазмерПисьма = ПараметрыЧтенияПисем.МаксимальныйРазмерПисьма;
	...
	УчетнаяЗапись = ПараметрыЧтенияПисем.УчетнаяЗапись;
	...
	Если ПараметрыЧтенияПисем.Свойство("ГлубинаПоиска") И ПараметрыЧтенияПисем.ГлубинаПоиска > 0 Тогда
		ПослеДатыОтправления = КонецДня(ТекущаяДатаСеанса()) - 60 * 60 * 24 * ПараметрыЧтенияПисем.ГлубинаПоиска + 1;
	Иначе
	...
	ВидОперации = "";
	ПараметрыЧтенияПисем.Свойство("ВидОперации", ВидОперации);
...
// Конструктор таблицы для загружаемых почтовых сообщений.
//
// Возвращаемое значение:
//	ТаблицаЗначений - загружаемые почтовые сообщения:
//		* Вложения - Соответствие из КлючИЗначение - с полями:
//			** Ключ - Строка - Имя файла.
//			** Значение - ДвоичныеДанные - Данные файла.
//		* ДатаОтправления - Дата - Дата и время отправления письма.
//		* Идентификатор   - Массив из Строка - Массив строк с идентификаторами письма.
...
//
Функция СоздатьАдаптированноеОписаниеПисьма()
	
	Результат = Новый ТаблицаЗначений;
	
	Колонки = Результат.Колонки;
	Колонки.Добавить("Вложения",        Новый ОписаниеТипов("Соответствие"));
	Колонки.Добавить("ДатаОтправления", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.ДатаВремя));
	Колонки.Добавить("Идентификатор",   Новый ОписаниеТипов("Массив"));
	Колонки.Добавить("Размер",          ОбщегоНазначения.ОписаниеТипаЧисло(10, 0, ДопустимыйЗнак.Неотрицательный));
	Колонки.Добавить("ПисьмоЗагружено", Новый ОписаниеТипов("Булево"));
