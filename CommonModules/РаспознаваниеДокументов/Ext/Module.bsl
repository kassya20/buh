...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СостоянияПоБалансу", Истина);
	Методы.Вставить("УдалениеУстаревшихРаспознанныхДокументов", Истина);
	Методы.Вставить("ОбменССервисомРаспознаванияДокументов", Истина);
	Методы.Вставить("ЗагрузитьРаспознанныеДокументы", Истина);
	
	Методы.Вставить("УстановитьКорректировкаНераспознанныхДокументовРазрешена");
...
// Возвращает распознанный текст документа по заданным координатам.
//
// Параметры:
//   ИдентификаторДокумента - Строка - Идентификатор документа для распознавания
//   Координаты - Структура - Координаты области документа для распознавания текста
//
// Возвращаемое значение:
//   Строка - Распознанный текст в заданной области документа
//
Функция РаспознанныйТекстПоКоординатам(ИдентификаторДокумента, Координаты) Экспорт
	
	Попытка
		Результат = РаспознаваниеДокументовSDK.РаспознанныйТекстПоКоординатам(ИдентификаторДокумента, Координаты);
		РаспознанныйТекст = Результат.РаспознанныйТекст;
	Исключение
		РаспознанныйТекст = "";
	КонецПопытки;
	
	Возврат РаспознанныйТекст;
...
Функция СоздатьЗаданиеДляРаспознавания(
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru = 'Отправлены файлы для распознавания:'") + Символы.ПС 
		+ ФайлыСтрокой
	);
...
Процедура ЗаполнитьСписокВыбораРеквизита(ДокументОбъект, ИмяРеквизита) Экспорт
	
	НаборЗаписей = РегистрыСведений.СписокВыбораРеквизитовРаспознаваниеДокументов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.РаспознанныйДокумент.Установить(ДокументОбъект.Ссылка);
	НаборЗаписей.Отбор.ИмяРеквизита.Установить(ИмяРеквизита);
	
	НайденныеСтроки = ДокументОбъект.РеквизитыДокумента.НайтиСтроки(Новый Структура("ИмяРеквизита", ИмяРеквизита));
	Если НайденныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ПроверяемыйТип = СтрокаРеквизита.Значение;
	Если Не РаспознаваниеДокументовКлиентСервер.ЭтоПримитивныйТип(ТипЗнч(ПроверяемыйТип)) Тогда
		//@skip-check query-in-loop
		мЗначений = ВыполнитьНечеткийПоиск(СтрокаРеквизита.РаспознанныйТекст, ПроверяемыйТип);
		Ид = 0;
		Для Каждого СтрВыбор Из мЗначений Цикл
			Ид = Ид + 1;
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.РаспознанныйДокумент = ДокументОбъект.Ссылка;
			НоваяЗапись.ИмяРеквизита = СтрокаРеквизита.ИмяРеквизита;
			НоваяЗапись.ПорядковыйНомер = Ид;
			НоваяЗапись.Значение = СтрВыбор[0];
			НоваяЗапись.Уверенность = СтрВыбор[1];
			НоваяЗапись.НайденВТаблицеСоответствий = СтрВыбор[2];
			Если Ид = 1 Тогда
				СтрокаРеквизита.НайденноеЗначение = НоваяЗапись.Значение;
				СтрокаРеквизита.УверенностьНайденногоЗначения = НоваяЗапись.Уверенность;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	РаспознаваниеДокументовСлужебный.ЗаполнитьСписокВыбораРеквизитаДокумента(ДокументОбъект, ИмяРеквизита);
	
КонецПроцедуры

Процедура ЗаполнитьСписокВыбораРеквизитаТабличнойЧасти(ДокументОбъект, ИмяРеквизита, НомерСтрокиТЧ) Экспорт
	...
	НайденныеСтроки = ДокументОбъект.РеквизитыТабличныхЧастей.НайтиСтроки(Новый Структура("ИмяРеквизита, НомерСтрокиТЧ", ИмяРеквизита, НомерСтрокиТЧ));
	Если НайденныеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не РаспознаваниеДокументовКлиентСервер.ЭтоПримитивныйТип(ТипЗнч(ПроверяемыйТип)) Тогда
		...
		Для Каждого СтрВыбор Из мЗначений Цикл
			...
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.РаспознанныйДокумент = ДокументОбъект.Ссылка;
			НоваяЗапись.ТабличнаяЧасть = СтрокаРеквизита.ТабличнаяЧасть;
			НоваяЗапись.НомерСтрокиТЧ = СтрокаРеквизита.НомерСтрокиТЧ;
			НоваяЗапись.ИмяРеквизита = СтрокаРеквизита.ИмяРеквизита;
			НоваяЗапись.ПорядковыйНомер = Ид;
			НоваяЗапись.Значение = СтрВыбор[0];
			НоваяЗапись.Уверенность = СтрВыбор[1];
	...
	РаспознаваниеДокументовСлужебный.ЗаполнитьСписокВыбораРеквизитаДокумента(ДокументОбъект, ИмяРеквизита, НомерСтрокиТЧ);
...
Процедура ОбновитьАдресЭлектроннойПочты()
	...
	Если Не ПустаяСтрока(ТекущиеНастройки.АдресЭлПочты) Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Предупреждение, , , 
				НСтр("ru = 'Обновление адреса электронной почты не удалось, адрес электронной почты очищен.'", ОбщегоНазначения.КодОсновногоЯзыка())
