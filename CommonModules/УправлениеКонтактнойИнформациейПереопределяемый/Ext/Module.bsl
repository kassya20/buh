...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	Настройки.ШиринаПоляКомментарий = 15;

	Если Взаимодействия.ИспользуютсяПрочиеВзаимодействия()
		И ПравоДоступа("Редактирование", Метаданные.Документы.ТелефонныйЗвонок) Тогда
		Телефон = Перечисления.ТипыКонтактнойИнформации.Телефон;
		Настройки.ОписаниеКоманд[Телефон].ПозвонитьПоТелефону.Заголовок  = НСтр("ru = 'Позвонить'");
		Настройки.ОписаниеКоманд[Телефон].ПозвонитьПоТелефону.Подсказка  = НСтр("ru = 'Позвонить по телефону'");
		Настройки.ОписаниеКоманд[Телефон].ПозвонитьПоТелефону.Картинка   = БиблиотекаКартинок.Позвонить;
		Настройки.ОписаниеКоманд[Телефон].ПозвонитьПоТелефону.Действие   = "ВзаимодействияКлиентБП.ПодготовитьПараметрыИПозвонить";
	КонецЕсли;
	
	Если Взаимодействия.ИспользуетсяПочтовыйКлиент()
		И ПравоДоступа("Редактирование", Метаданные.Документы.ЭлектронноеПисьмоИсходящее) Тогда
		АдресЭлектроннойПочты = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
		Настройки.ОписаниеКоманд[АдресЭлектроннойПочты].НаписатьЭлектронноеПисьмо.Заголовок  = НСтр("ru = 'Написать письмо'");
		Настройки.ОписаниеКоманд[АдресЭлектроннойПочты].НаписатьЭлектронноеПисьмо.Подсказка  = 
			НСтр("ru = 'Написать письмо на указанный адрес'");
		Настройки.ОписаниеКоманд[АдресЭлектроннойПочты].НаписатьЭлектронноеПисьмо.Картинка =
			БиблиотекаКартинок.КонтактнаяИнформацияОтправитьЭлектронноеПисьмо;
		Настройки.ОписаниеКоманд[АдресЭлектроннойПочты].НаписатьЭлектронноеПисьмо.Действие =
			"ВзаимодействияКлиентБП.ПодготовитьПараметрыИНаписатьПисьмо";
	КонецЕсли;
	
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		Телефон = Перечисления.ТипыКонтактнойИнформации.Телефон;
		//Для взаимодействий команда звонка в форме звонка
		Если Не Взаимодействия.ИспользуютсяПрочиеВзаимодействия() Тогда
			Настройки.ОписаниеКоманд[Телефон].ПозвонитьПоТелефону.Действие   = "МобильныйКлиентБПКлиент.ПодготовитьПараметрыИПозвонить";
		КонецЕсли;
		Если Не Взаимодействия.ИспользуетсяПочтовыйКлиент()
			И Не РаботаСПочтовымиСообщениямиВызовСервера.ЕстьДоступныеУчетныеЗаписиДляОтправки() Тогда
			АдресЭлектроннойПочты = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты;
			Настройки.ОписаниеКоманд[АдресЭлектроннойПочты].НаписатьЭлектронноеПисьмо.Действие = "МобильныйКлиентБПКлиент.ПодготовитьПараметрыИНаписатьПисьмо";
		КонецЕсли;
		//SMS во взаимодействиях не используется
		Настройки.ОписаниеКоманд[Телефон].ОтправитьSMS.Действие   = "МобильныйКлиентБПКлиент.ПодготовитьПараметрыИОтправитьSMS";
		//В мобильном интент SMS может принимать WhatsApp
		Настройки.ОписаниеКоманд[Телефон].ОтправитьSMS.Заголовок  = НСтр("ru = 'Отправить сообщение'");
	КонецЕсли;
...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	УправлениеКонтактнойИнформациейБП.ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти);
	
	// ЗарплатаКадры
	ЗарплатаКадры.ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти);
	// Конец ЗарплатаКадры	
