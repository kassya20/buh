...
// Находит, а при необходимости - создает поставляемый элемент.
// Следует использовать в алгоритмах обновления информационной базы для исправления ошибок.
// 
// Выполнение связанных настроек не гарантируется.
// 
// Для регулярных задач следует использовать ОбеспечитьФункциональность, ПредопределенныйЭлемент.
// 
// Параметры:
//  Классификатор - СправочникМенеджер - классификатор, содержащий элементы
//  ИмяПредопределенногоЭлемента - Строка - имя поставляемого элемента.
//                  Описание создаваемых элементов приведено в ЗаполнитьОписанияПоставляемыхЭлементов
//                  модуля менеджера классификатора.
// 
// Возвращаемое значение:
//  СправочникСсылка.ПрочиеДоходыИРасходы,
//  СправочникСсылка.СтатьиЗатрат
//        - найденный (созданный) элемент.
//          Конкретный тип определяется переданным классификатором.
//  Неопределено - ошибка при создании элемента
//
Функция ПоставляемыйЭлемент(Классификатор, ИмяПредопределенногоЭлемента) Экспорт
	
	ПоставляемыеЭлементы = ПоставляемыеЭлементы(Классификатор);
	ОписаниеЭлемента = ПоставляемыеЭлементы.Найти(ИмяПредопределенногоЭлемента, "Имя");
	
	Если ОписаниеЭлемента = Неопределено Тогда
		ШаблонТекста = НСтр("ru = 'Неизвестное имя элемента %1'", ОбщегоНазначения.КодОсновногоЯзыка());
		ВызватьИсключение СтрШаблон(ШаблонТекста, ИмяПредопределенногоЭлемента);
	КонецЕсли;
	
	ПотребныеЭлементы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеЭлемента);
	
	НайденныеЭлементы = НазначитьПодходящиеЭлементы(Классификатор, ПотребныеЭлементы);
	Если НайденныеЭлементы.Свойство(ИмяПредопределенногоЭлемента) Тогда
		Возврат НайденныеЭлементы[ИмяПредопределенногоЭлемента];
	КонецЕсли;
	
	Созданные = СоздатьПоставляемыеЭлементы(Классификатор, ПотребныеЭлементы);
	Если Созданные.Свойство(ИмяПредопределенногоЭлемента) Тогда
		Возврат Созданные[ИмяПредопределенногоЭлемента];
	КонецЕсли;
