...
Процедура СоставВыбор(Форма, Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтроки) Тогда
		...
		ИначеЕсли СтрНачинаетсяС(Поле.Имя, "СоставНДФЛ") Тогда
			Форма.РедактироватьНДФЛСтроки(ТекущиеДанные);
			СтандартнаяОбработка = Ложь;
		ИначеЕсли СтрНачинаетсяС(Поле.Имя, "СоставВзысканнаяСумма") Тогда
			Форма.РедактироватьВзысканныеСуммыСтроки(ТекущиеДанные, Поле.Имя);
			СтандартнаяОбработка = Ложь;
		ИначеЕсли Поле.ГиперссылкаЯчейки Или Поле.Вид = ВидПоляФормы.ПолеНадписи Или Поле.ТолькоПросмотр Тогда
			Форма.РедактироватьЗарплатуСтроки(ТекущиеДанные, Поле.Имя);
			СтандартнаяОбработка = Ложь;
		КонецЕсли;
	КонецЕсли;
...
Процедура Подобрать(Форма) Экспорт
	...
	Отбор = Новый Структура("ДатаПримененияОтбора", КонецМесяца(Форма.Объект.ПериодРегистрации));

	Если ЗначениеЗаполнено(Форма.Объект.Подразделение) Тогда
		Отбор.Вставить("Подразделение", Форма.Объект.Подразделение);
	КонецЕсли;
...
Процедура РедактироватьЗарплатуСтроки(Форма, ДанныеСтроки, ИмяПоля = "") Экспорт
	...
	ПараметрыОткрытия.Вставить("ЗарплатныйПроектДляПеречисленияНаКартыМир", Ложь);
	ЗаполнитьЗначенияСвойств(ПараметрыОткрытия, Форма);
	...
	ПараметрыОткрытия.Вставить("ИмяПоля", ИмяПоля);
	
	ПараметрыОткрытия.Вставить("Заголовок", "");
	Если СтрНачинаетсяС(ИмяПоля, "СоставВзысканнаяСумма") Тогда
		ПараметрыОткрытия.Заголовок = НСтр("ru = 'взыскано'");
	Иначе
		ПараметрыОткрытия.Заголовок = НСтр("ru = 'зарплата'");
	КонецЕсли;
