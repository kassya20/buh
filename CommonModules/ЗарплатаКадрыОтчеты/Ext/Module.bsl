...
Функция НоваяСтруктураНастроекПечатнойФормы() Экспорт
	НоваяСтруктура = Новый Структура("Настройки,ПользовательскиеНастройки,СхемаКомпоновкиДанных,Макет");
...
Процедура ЗапомнитьНастройкиВариантаОтчета(ВариантОтчета, НастройкиВарианта) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	НаборЗаписей = РегистрыСведений.НастройкиВариантовОтчетовПечатныхФормБЗК.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ВариантОтчета.Установить(ВариантОтчета);
	
	Если НастройкиВарианта <> Неопределено Тогда
		Запись = НаборЗаписей.Добавить();
		Запись.ВариантОтчета = ВариантОтчета;
		Запись.Настройки = Новый ХранилищеЗначения(НастройкиВарианта, Новый СжатиеДанных(9));
	КонецЕсли;
...
Процедура ОбновитьМакетДокументаНастроекВариантовОтчета(ВариантОтчета, МакетДокумента) Экспорт
	
	Настройки = НастройкиВариантаОтчета(ВариантОтчета);
	Настройки.Макет = МакетДокумента;
	ЗапомнитьНастройкиВариантаОтчета(ВариантОтчета, Настройки);
				...
				Если СтрокаГруппы <> Неопределено Тогда
					...
					Для каждого СтрокаСправочно Из СтрокаГруппы.Строки Цикл
						...
						Если СтрокаКоллекции = Неопределено Тогда
							...
							Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаСправочно, "ДокументОснование")
								И ЗначениеЗаполнено(СтрокаСправочно.ДокументОснование) 
								И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаСправочно.ДокументОснование, "НаименованиеНачисленияДляРасчетногоЛиста") Тогда								
								
								ПредставлениеНачисленияДляРасчетногоЛиста = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСправочно.ДокументОснование, "НаименованиеНачисленияДляРасчетногоЛиста");
								Если ЗначениеЗаполнено(ПредставлениеНачисленияДляРасчетногоЛиста) Тогда      
									СтрокаКоллекции.ВидРасчета = ПредставлениеНачисленияДляРасчетногоЛиста;
								КонецЕсли;
...
Процедура СвернутьСтрокиВидовРасчета(СтрокиВидовРасчета, СворачиваемыеКолонки, СуммируемыеКолонки, НастройкиВывода, СвернутьСотрудников)
	...
	ДанныеСворачиваемойСтроки = Неопределено;
	...
	Для Каждого СтрокаВидаРасчета Из СтрокиВидовРасчета Цикл
		...
		Если НастройкиВывода.ВыводитьПоказателиНачисленийИУдержаний
			...
			Если ДанныеСворачиваемойСтроки = Неопределено
				Или ДанныеСтрокРазличны(ДанныеСворачиваемойСтроки, СтрокаВидаРасчета, СворачиваемыеКолонки) Тогда
				
				ДанныеСворачиваемойСтроки = Новый Структура(СворачиваемыеКолонки);
				ЗаполнитьЗначенияСвойств(ДанныеСворачиваемойСтроки, СтрокаВидаРасчета);
				СворачиваемыеСтроки = Новый Массив;
...
Функция ДанныеСтрокРазличны(ДанныеСворачиваемойСтроки, СтрокаВидаРасчета, СворачиваемыеКолонки)
	
	Для Каждого ИмяКолонки Из СтрРазделить(СворачиваемыеКолонки, ",", Ложь) Цикл
		Если ИмяКолонки = "ПриоритетВидаРасчета"
			Или ИмяКолонки = "Показатель"
			Или ИмяКолонки = "Значение"
			Или ИмяКолонки = "ПорядокПоказателей" Тогда
			
			Продолжить;
		КонецЕсли;
		
		Если ДанныеСворачиваемойСтроки[ИмяКолонки] <> СтрокаВидаРасчета[ИмяКолонки] Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция ДанныеДокументаФизическихЛиц(Форма, ФизическиеЛица, ТабличныеЧасти, МесяцНачисленияИмя = "МесяцНачисления", ИмяРеквизитаВидРасчета = "") Экспорт
	...
	Для Каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		...
		Для Каждого ОписаниеТаблицы Из ТабличныеЧасти.ТаблицыНачислений Цикл
			...
			Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
				...
				НоваяСтрока.Период = Объект[МесяцНачисленияИмя];
				...
				Если ПустаяСтрока(ОписаниеТаблицы.ИмяРеквизитаДатаНачала) Тогда
					НоваяСтрока.ДатаНачала = Объект[МесяцНачисленияИмя];
				Иначе
				...
				Если ПустаяСтрока(ОписаниеТаблицы.ИмяРеквизитаДатаОкончания) Тогда
					НоваяСтрока.ДатаОкончания = НачалоДня(КонецМесяца(Объект[МесяцНачисленияИмя]));
				Иначе
					НоваяСтрока.ДатаОкончания = СтрокаТаблицы[ОписаниеТаблицы.ИмяРеквизитаДатаОкончания];
				КонецЕсли;
				
				Если Не ПустаяСтрока(ИмяРеквизитаВидРасчета) Тогда
					НоваяСтрока.НачислениеУдержание = Объект[ИмяРеквизитаВидРасчета];
				ИначеЕсли Не ПустаяСтрока(ОписаниеТаблицы.ИмяРеквизитаВидРасчета) Тогда
...
Функция НастройкиВариантаОтчета(ВариантОтчета)
	...
	Если НаборЗаписей.Количество() = 0 Тогда
		...
		Если ТипЗнч(НастройкиВарианта) = Тип("Структура") Тогда
			ЗапомнитьНастройкиВариантаОтчета(ВариантОтчета, НастройкиВарианта);
		...
		КонецЕсли;
	Иначе
		НастройкиВарианта = НаборЗаписей[0].Настройки.Получить();
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, ОбъектОтчета, КлючВарианта, УправляемаяФорма = Неопределено) Экспорт
	...
	Если НастройкиПечатнойФормы.Свойство("СхемаКомпоновкиДанных")
		И НастройкиПечатнойФормы.СхемаКомпоновкиДанных  <> Неопределено Тогда
		
		СхемаКомпоновкиДанных = НастройкиПечатнойФормы.СхемаКомпоновкиДанных
	Иначе
		СхемаКомпоновкиДанных = ОбъектОтчета.СхемаКомпоновкиДанных
	КонецЕсли;
	
	Если УправляемаяФорма <> Неопределено Тогда
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УправляемаяФорма, "АдресСхемы") Тогда
			УправляемаяФорма.АдресСхемы = ПоместитьВоВременноеХранилище(
				СхемаКомпоновкиДанных, УправляемаяФорма.УникальныйИдентификатор);
		КонецЕсли;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(УправляемаяФорма, "МакетПечатнойФормы")
			И НастройкиПечатнойФормы.Макет <> Неопределено Тогда
			
			УправляемаяФорма.МакетПечатнойФормы = НастройкиПечатнойФормы.Макет;
		КонецЕсли;
	КонецЕсли;
	
	ИнициализироватьИЗагрузитьНастройкиВКомпоновщикКД(КомпоновщикНастроекКД, НастройкиПечатнойФормы, СхемаКомпоновкиДанных);
...
Процедура ДобавитьПредставленияПараметраВМассив(СхемаКомпоновкиДанных, КомпоновщикНастроек, Параметры, ИмяПараметра)
	...
	Если Не Параметр = Неопределено Тогда
		...
		ПараметрКомпоновки = НайтиПараметр(КомпоновщикНастроек, ИмяПараметра);
