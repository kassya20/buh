	...
	Если Не БылиОшибки Тогда
		...
		Попытка
			...
			Если КонтекстныеОбсуждения.Количество() > 0 Тогда
				...
				ДокументыЭДОБЗК = ДокументыЭДОБЗКПоИдентификатору(ИдентификаторыДокументов);
				...
				Для каждого СтрокаТЗ Из КонтекстныеОбсуждения Цикл
					...
					ДанныеДокумента = ДокументыЭДОБЗК.Найти(СтрокаТЗ.ИдентификаторДокумента, "ИдентификаторДокумента");
...
Процедура ЗавершитьОбработкуОбсуждения(ОбсуждениеДляОбработки, ПолучателиСообщения, Сообщение, ФизическоеЛицо, ДокументЭДОБЗК = Неопределено)
	...
	Если ЗначениеЗаполнено(ДокументЭДОБЗК) Тогда
		Действие = Новый Структура;
		Действие.Вставить("ВыполняемоеДействие", "ОткрытьНавигационнуюСсылкуКабинетСотрудника");
		Действие.Вставить("НавигационнаяСсылка", ПолучитьНавигационнуюСсылку(ДокументЭДОБЗК));
		Сообщение.Действия.Добавить(Новый ФиксированнаяСтруктура(Действие), Строка(ДокументЭДОБЗК));
	КонецЕсли;
	Сообщение.Записать();
...
Функция ДокументыЭДОБЗКПоИдентификатору(Идентификаторы)
	...
	|	ДокументЭДО.ОснованиеДокумента КАК Документ,
	|	ВЫБОР
	|		КОГДА НЕ РасчетныеЛистки.Ответственный ЕСТЬ NULL
	|			ТОГДА РасчетныеЛистки.Ответственный
	|		ИНАЧЕ РасчетныеЛистки.ДокументЭДОБЗК.Ответственный
	|	КОНЕЦ КАК Ответственный
	|ИЗ
	|	Документ.ДокументЭДОБЗК КАК ДокументЭДО
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РасчетныеЛисткиКабинетСотрудника КАК РасчетныеЛистки
	|		ПО ДокументЭДО.Ссылка = РасчетныеЛистки.ДокументЭДОБЗК
	|ГДЕ
	|	ДокументЭДО.ИдентификаторДокумента В(&ИдентификаторыДокументов)";
