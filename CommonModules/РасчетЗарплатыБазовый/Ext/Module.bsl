...
//
Функция РезультатНачисленияРасчетаЗарплаты(Организация, МесяцНачисления, Документ, Подразделение, Сотрудники, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты = '00010101') Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		Если УчетЗарплаты.ИспользоватьРасчетПервойПоловиныМесяца(Организация, МесяцНачисления) Тогда
			...
			ИначеЕсли МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп <> Неопределено Тогда
				МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
	...
	Если Сотрудники = Неопределено Тогда
		...
		ПараметрыПолученияСотрудниковОрганизаций.ОкончаниеПериода				= КонецДня(КонецПериодаНачисления);
	...
	ВзаиморасчетыССотрудникамиПереопределяемый.ДополнитьЗапросПлановымАвансом(Запрос, Организация);
	
	Запрос.Выполнить();
	
	Возврат ТаблицаЗаполненияНачислений(Запрос.МенеджерВременныхТаблиц, Организация, МесяцНачисления, ПерваяПоловинаМесяца);
	
КонецФункции

Функция ТаблицаЗаполненияНачислений(МенеджерВременныхТаблиц, Организация, МесяцНачисления, ПерваяПоловинаМесяца = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		МодульРасчетЗарплатыДляНебольшихОрганизаций.СоздатьВТМежрасчетныхВыплат(Организация, МесяцНачисления, Запрос.МенеджерВременныхТаблиц, ПерваяПоловинаМесяца);
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|
		|ВЫБРАТЬ
		|	ДанныеПоАвансамСотрудников.Сотрудник,
		|	ДанныеПоАвансамСотрудников.ОсновноеПодразделение,
		|	ДанныеПоАвансамСотрудников.ЕдиноличныйРуководительКоммерческойОрганизации,
		|	ДанныеПоАвансамСотрудников.ПланируемаяДатаВыплаты,
		|	ДанныеПоАвансамСотрудников.ДоляНеполногоРабочегоВремени,
		|	ДанныеПоАвансамСотрудников.Начисление.КатегорияДохода,
		|	СУММА(ДанныеПоАвансамСотрудников.Результат)
		|ИЗ
		|	ВТМежрасчетныеВыплаты КАК ДанныеПоАвансамСотрудников
		|ГДЕ
		|	ДанныеПоАвансамСотрудников.ВходитВБазуРКиСН
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеПоАвансамСотрудников.Сотрудник,
		|	ДанныеПоАвансамСотрудников.ОсновноеПодразделение,
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ
		|	ДанныеПоАвансамСотрудников.Сотрудник,
		|	ДанныеПоАвансамСотрудников.Подразделение,
		|	ДанныеПоАвансамСотрудников.ЕдиноличныйРуководительКоммерческойОрганизации,
		|	ДанныеПоАвансамСотрудников.Начисление,
		|	ДанныеПоАвансамСотрудников.КодВычета,
		|	ДанныеПоАвансамСотрудников.Результат,
		|	ДанныеПоАвансамСотрудников.ВходитВБазуРКиСН,
		|	0,
		|	ДанныеПоАвансамСотрудников.ПланируемаяДатаВыплаты,
		|	ДанныеПоАвансамСотрудников.ДоляНеполногоРабочегоВремени,
		|	ДанныеПоАвансамСотрудников.СуммаВычета,
		|	ДанныеПоАвансамСотрудников.ОтработаноДней,
		|	ДанныеПоАвансамСотрудников.ОтработаноЧасов,
		|	ИСТИНА
		|ИЗ
		|	ВТМежрасчетныеВыплаты КАК ДанныеПоАвансамСотрудников
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСотрудникиОрганизации КАК КадровыеДанныеСотрудников
		|		ПО ДанныеПоАвансамСотрудников.Сотрудник = КадровыеДанныеСотрудников.Сотрудник
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция СоздатьДанныеДляПроведенияНачисленияЗарплаты() Экспорт
	...
	|ИсчисленныеВзносы,
	|НачисленияДляСреднегоЗаработка";
...
Процедура ЗаполнитьНачисления(ДанныеДляПроведенияНачисленияЗарплаты, Документ, ТаблицаНачислений, ПолеДатыДействия, ПолеВидаНачисления = Неопределено, ПолеВидаНачисленияВШапке = Неопределено, ФизическиеЛица) Экспорт
	...
	Для Каждого ИмяТаблицыНачислений Из ИменаТаблицНачислений Цикл
		...
		ТекстЗапроса = ТекстЗапроса 
			...
			|	0 КАК СуммаВычетаНДФЛ,
			|	0 КАК СуммаВычетаВзносы,
			|	Неопределено КАК КодВычетаНДФЛ,
		...
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("СуммаВычета") <> Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "0 КАК СуммаВычетаНДФЛ", "ТаблицаНачислений.СуммаВычета КАК СуммаВычетаНДФЛ");
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "0 КАК СуммаВычетаВзносы", "ТаблицаНачислений.СуммаВычета КАК СуммаВычетаВзносы");
		КонецЕсли;
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("КодВычета") <> Неопределено Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Неопределено КАК КодВычетаНДФЛ", "ТаблицаНачислений.КодВычета КАК КодВычетаНДФЛ");
		КонецЕсли;
		...
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("Начисление") = Неопределено И ЗначениеЗаполнено(ПолеВидаНачисленияВШапке) Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТаблицаНачислений.Начисление", "ТаблицаНачислений."+ ПолеВидаНачисленияВШапке);
		КонецЕсли;
		
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.Премия") Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Неопределено КАК ДокументОснование", "&Ссылка КАК ДокументОснование");
		КонецЕсли;
	...
	Для Каждого ИмяТаблицыНачислений Из ИменаТаблицНачислений Цикл
		...
		ТекстЗапросаПолей =
			...
			|	НАЧАЛОПЕРИОДА(ДАТАВРЕМЯ(1, 1, 1), МЕСЯЦ) КАК ПериодДействия,
			|	Неопределено КАК ДокументОснование
			|ИЗ
			|	#ТаблицаНачислений КАК Начисления
			|ГДЕ
		...
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("Начисление") = Неопределено И ЗначениеЗаполнено(ПолеВидаНачисленияВШапке) Тогда
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "Начисления.Начисление", "Начисления."+ ПолеВидаНачисленияВШапке);
		КонецЕсли;
		
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("ОтработаноДней") <> Неопределено Тогда
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "0 КАК ОтработаноДней", "Начисления.ОтработаноДней КАК ОтработаноДней");
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "0 КАК ОплаченоДней", "Начисления.ОтработаноДней КАК ОплаченоДней");
		КонецЕсли;
		
		Если МетаданныеДокумента.ТабличныеЧасти[ИмяТаблицыНачислений].Реквизиты.Найти("ОтработаноЧасов") <> Неопределено Тогда
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "0 КАК ОтработаноЧасов", "Начисления.ОтработаноЧасов КАК ОтработаноЧасов");
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "0 КАК ОплаченоЧасов", "Начисления.ОтработаноЧасов КАК ОплаченоЧасов");
		КонецЕсли;
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.Премия") Тогда
			ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "Неопределено КАК ДокументОснование", "&Ссылка КАК ДокументОснование");
		КонецЕсли;
		
		ТекстЗапросаПолей = СтрЗаменить(ТекстЗапросаПолей, "НАЧАЛОПЕРИОДА(ДАТАВРЕМЯ(1, 1, 1), МЕСЯЦ) КАК ПериодДействия", "НАЧАЛОПЕРИОДА(Начисления." + ПолеДатыДействия + ", МЕСЯЦ) КАК ПериодДействия");
	...
	ТекстЗапроса = ТекстЗапроса + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов() 
		+ "ВЫБРАТЬ
		  ...
		  |	НачисленияИОтработанноеВремя.ПериодДействия КАК ПериодДействия,
		  |	НачисленияИОтработанноеВремя.ДокументОснование КАК ДокументОснование
		  |ИЗ
...
Процедура ЗаполнитьДокументНачисленияЗарплаты(Объект, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты = '00010101') Экспорт
	
	ЗаполнитьРезультатНачисления(Объект, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
	ЗаполнитьРезультатНачисленияПоДоговорам(Объект, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
...
Процедура ЗаполнитьДокументПремия(Объект) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		МодульРасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьСписокСотрудниковНаПремию(Объект);
		МодульРасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьРезультатУдержаний(Объект, , "ВидПремии");
		МодульРасчетЗарплатыДляНебольшихОрганизаций.СформироватьТабличнуюЧастьСотрудникиПремии(Объект);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьРезультатНачисления(Объект, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты = '00010101')
	
	Объект.Начисления.Очистить();
	
	РезультатНачисления = РасчетЗарплаты.РезультатНачисленияРасчетаЗарплаты(Объект.Организация, Объект.МесяцНачисления,
		Объект.Ссылка, Объект.Подразделение, , ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты);
	ТаблицаИсточник = Объект.Начисления.Выгрузить();
	Для Каждого КолонкаТаблицы Из ТаблицаИсточник.Колонки Цикл
		Если РезультатНачисления.Колонки.Найти(КолонкаТаблицы.Имя) = Неопределено Тогда
			РезультатНачисления.Колонки.Добавить(КолонкаТаблицы.Имя, КолонкаТаблицы.ТипЗначения);
		КонецЕсли;
	КонецЦикла;
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаИсточник, РезультатНачисления);
	
	ЗаполнитьОтработанноеВремя(РезультатНачисления, Объект.МесяцНачисления, Объект.Организация, ПерваяПоловинаМесяца);
...
Процедура ЗаполнитьРезультатНачисленияПоДоговорам(Объект, ПерваяПоловинаМесяца, ПланируемаяДатаВыплаты = '00010101')
	...
	НачисленияПоДоговорам   = РезультатНачисленияПоДоговорам(Объект.Организация, Объект.МесяцНачисления, Объект.Ссылка, Объект.Подразделение, СотрудникиДляНачислений, ПланируемаяДатаВыплаты);
...
Функция РезультатНачисленияПоДоговорам(Организация, МесяцНачисления, ДокументСсылка, Подразделение, Сотрудники, ПланируемаяДатаВыплаты = '00010101') Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		Если ЗначениеЗаполнено(ПланируемаяДатаВыплаты) Тогда
			ПланируемаяДатаВыплатыЗарплаты = ПланируемаяДатаВыплаты;
		Иначе
			МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизацийПовтИсп");
...
Функция ТаблицаЗависимыеНачисления()
	...
	ЗависимыеНачисления.Колонки.Добавить("Подразделение", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
	ЗависимыеНачисления.Колонки.Добавить("Начисление", Новый ОписаниеТипов("ПланВидовРасчетаСсылка.Начисления"));
	ЗависимыеНачисления.Колонки.Добавить("КодВычета", Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"));
	ЗависимыеНачисления.Колонки.Добавить("СуммаВычета", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
	ЗависимыеНачисления.Колонки.Добавить("СуммаВычетаВзносы", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
	ЗависимыеНачисления.Колонки.Добавить("Результат", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
	ЗависимыеНачисления.Колонки.Добавить("ПланируемаяДатаВыплаты", Новый ОписаниеТипов("Дата"));
	ЗависимыеНачисления.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(18,0));
	ЗависимыеНачисления.Колонки.Добавить("ЕдиноличныйРуководительКоммерческойОрганизации", Новый ОписаниеТипов("Булево"));
...
Процедура ЗаполнитьОтработанноеВремя(СтрокиНачислений, МесяцНачисления, Организация, ПерваяПоловинаМесяца, КадровыеДанные = Неопределено) Экспорт
	...
	Для Каждого Строка Из СтрокиНачислений Цикл
		...
		Если КатегорииНачислений[Строка.Начисление] = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПовременнаяОплатаТруда Тогда
			...
			Если ТипЗнч(Строка) = Тип("ДанныеФормыЭлементКоллекции")
				Или ТипЗнч(Строка) = Тип("СтрокаТаблицыЗначений") Тогда
				РабочихДнейЧасов =
					Новый Структура("РабочихДней, РабочихЧасов",
						КоличествоДней, КоличествоЧасов * ДоляНеполногоРабочегоВремени);
				ЗаполнитьЗначенияСвойств(Строка, РабочихДнейЧасов);
			КонецЕсли;
	...
	Для Каждого Строка Из СтрокиНачислений Цикл
		...
		Если Не ПерваяПоловинаМесяца И Строка.ПерваяПоловинаМесяца Тогда
			Продолжить;
		КонецЕсли;
...
//	только за месяц в целом, а не за произвольный период.
//
Функция РезультатНачисленияРасчетаПремии(Организация, МесяцНачисления, Документ, Подразделение, Сотрудники) Экспорт
	...
	КадровыеДанныеДокумента = "ГоловнаяОрганизация,Организация,Подразделение,ПроцентСевернойНадбавки,ДоляНеполногоРабочегоВремени,ДатаРождения";
	КадровыеДанные = КадровыеДанныеДокумента;
	Если ЗарплатаКадры.ДополнитьКадровымиДаннымиНастройкиПорядкаСписка(КадровыеДанные) Тогда
		ПоляСортировки = Сред(КадровыеДанные, СтрДлина(КадровыеДанныеДокумента) + 2);
	Иначе
	...
	Возврат ТаблицаЗаполненияНачисленийПремии(Запрос.МенеджерВременныхТаблиц, Организация, МесяцНачисления);
	
КонецФункции

Функция ТаблицаЗаполненияНачисленийПремии(МенеджерВременныхТаблиц, Организация, МесяцНачисления) Экспорт
...
Процедура СоздатьВТДляРасчетаНДФЛиВзносов(МенеджерВременныхТаблиц, Объект, Сотрудники = Неопределено, МесяцНачисленияИмя = "МесяцНачисления", РеквизитНачислениеИмя = "") Экспорт
	...
	|	ЗависимыеНачисления.Начисление КАК Начисление,
	|	&МесяцНачисления КАК ПериодДействия,
	|	&МесяцНачисления КАК ДатаНачала,
	|	НАЧАЛОПЕРИОДА(КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ), ДЕНЬ) КАК ДатаОкончания,
	|	ЗависимыеНачисления.Результат КАК СуммаДохода,
	|	ЗависимыеНачисления.КодВычета КАК КодВычетаНДФЛ,
	|	ЗависимыеНачисления.СуммаВычета КАК СуммаВычетаНДФЛ,
	|	ЗависимыеНачисления.СуммаВычета КАК СуммаВычетаВзносы,
	|	ЗависимыеНачисления.Подразделение КАК Подразделение,
	|	ЗависимыеНачисления.ПланируемаяДатаВыплаты КАК ПланируемаяДатаВыплаты,
	|	ЗависимыеНачисления.ЕдиноличныйРуководительКоммерческойОрганизации КАК ЕдиноличныйРуководительКоммерческойОрганизации
	|ПОМЕСТИТЬ ВТВсеЗависимыеНачисленияя
	|ИЗ
	|	&ЗависимыеНачисления КАК ЗависимыеНачисления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Начисления.ЕдиноличныйРуководительКоммерческойОрганизации
	|ИЗ
	|	ВТВсеЗависимыеНачисленияя КАК Начисления
	|ГДЕ
	|	Начисления.Сотрудник В(&Сотрудники)
	|	И &ПоВсемСотрудникам = ЛОЖЬ
	|
	...
	КонецЕсли;
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Начисления.Колонки, "Начисление") Тогда
		Начисления.Колонки.Добавить("Начисление", Новый ОписаниеТипов("ПланВидовРасчетаСсылка.Начисления"));
		Начисления.ЗаполнитьЗначения(Объект[РеквизитНачислениеИмя], "Начисление");
	КонецЕсли;
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Начисления.Колонки, "КодВычета") Тогда
		...
		Начисления.Колонки.Добавить("СуммаВычетаВзносы", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ЗависимыеНачисления") Тогда
		ЗависимыеНачисления = Объект.ЗависимыеНачисления.Выгрузить();
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЗависимыеНачисления.Колонки, "ПланируемаяДатаВыплаты") Тогда
			ЗависимыеНачисления.Колонки.Добавить("ПланируемаяДатаВыплаты", Новый ОписаниеТипов("Дата"));
			ЗависимыеНачисления.ЗаполнитьЗначения(Объект.ПланируемаяДатаВыплаты, "ПланируемаяДатаВыплаты");
		КонецЕсли;
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЗависимыеНачисления.Колонки, "ЕдиноличныйРуководительКоммерческойОрганизации") Тогда
			ЗависимыеНачисления.Колонки.Добавить("ЕдиноличныйРуководительКоммерческойОрганизации", Новый ОписаниеТипов("Булево"));
		КонецЕсли;
		Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЗависимыеНачисления.Колонки, "КодВычета") Тогда
			ЗависимыеНачисления.Колонки.Добавить("КодВычета", Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"));
			ЗависимыеНачисления.Колонки.Добавить("СуммаВычета", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
			ЗависимыеНачисления.Колонки.Добавить("СуммаВычетаВзносы", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
		КонецЕсли;
	Иначе
		ЗависимыеНачисления = ТаблицаЗависимыеНачисления();
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ЗависимыеНачисления",   ЗависимыеНачисления);
...
Функция РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц, Объект, ДатаОперации, ПланируемаяДатаВыплатыВТаблицеНачислений = Ложь, МесяцНачисленияИмя = "МесяцНачисления") Экспорт
	...
	ПланируемаяДатаВыплаты = КонецМесяца(Объект[МесяцНачисленияИмя]);
	ОкончательныйРасчет = Истина;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "ПланируемаяДатаВыплаты") Тогда
		ПланируемаяДатаВыплаты = Объект.ПланируемаяДатаВыплаты;
	ИначеЕсли ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизацийПовтИсп");
		ДатыВыплаты = МодульРасчетЗарплатыДляНебольшихОрганизацийПовтИсп.ДатыВыплатыОрганизации(Объект.Организация, Объект[МесяцНачисленияИмя]);
	...
	УчетНДФЛ.СформироватьСоциальныеВычетыПоВременнойТаблице(Объект.Ссылка, НаборыЗаписей,  Неопределено, Объект.Организация, ДатаОперации, Объект[МесяцНачисленияИмя], МенеджерВременныхТаблиц, , ОкончательныйРасчет);
	УчетНДФЛ.СформироватьДоходыНДФЛПоНачислениям(НаборыЗаписей, Неопределено, Объект.Организация, ДатаОперации, ?(ПланируемаяДатаВыплатыВТаблицеНачислений, Неопределено, ПланируемаяДатаВыплаты), МенеджерВременныхТаблиц, Объект[МесяцНачисленияИмя], , ОкончательныйРасчет);
	...
	// Расчет НДФЛ
	РезультатРасчетаНДФЛ = УчетНДФЛ.РезультатРасчетаНДФЛ(МенеджерВременныхТаблиц, Объект.Ссылка, Объект.Организация, Объект[МесяцНачисленияИмя], ОкончательныйРасчет, МаксимумДатыПолученияДохода, НаборыЗаписей);
...
#Область СлужебныеПроцедурыИФункцииПриемника

#Область АктуализацияДанныхФормыДокументов
