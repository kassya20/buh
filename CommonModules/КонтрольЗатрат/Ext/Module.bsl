...
Процедура ОписатьОшибкиВыявленныеПриРасчетеСебестоимости(Ошибки, ДетальныеДанные, ПротоколРасчета, Период, Организация) Экспорт
...
Процедура ОписатьРасходыНеУдалосьРаспределить(Ошибки, ВсеДетальныеДанные, ПротоколРасчета, Период, Организация)
	...
	КешНастроекУчетаЗатрат = Неопределено; // Будут получены при необходимости
	
	ОтраженаВыручка = Неопределено;
	...
	Для Каждого ГруппаОшибокСчет Из ВсеДетальныеДанные.Строки Цикл
		...
		Для Каждого ГруппаОшибокПравилоРаспределения Из ГруппаОшибокСчет.Строки Цикл
			...
			Если МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.Выручка И ОтраженаВыручка = Неопределено Тогда
				ОтраженаВыручка = ОтраженаВыручка(Период, Организация);
			КонецЕсли;
			
			ПереченьПрямыхЗатрат             = Новый Массив;
			ОшибкаПустойПереченьПрямыхЗатрат = Ложь;
			Если МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ОтдельныеСтатьиПрямыхЗатрат Тогда
				ПереченьПрямыхЗатрат = ПереченьПрямыхЗатрат(
					ГруппаОшибокСчет.Счет,
					ГруппаОшибокПравилоРаспределения.Настройка,
					ПротоколРасчета.Настройки,
					КешНастроекУчетаЗатрат);
				ОшибкаПустойПереченьПрямыхЗатрат = Не ЗначениеЗаполнено(ПереченьПрямыхЗатрат);
			КонецЕсли;
			...
			ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ВыручкаОтРеализацииУслуг Тогда
				
				Причина.Добавить(НСтр("ru = 'Выручка от реализации услуг не отражена.'"));
				
			ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.Выручка Тогда
				
				Если Не ОтраженаВыручка Тогда
			...
			Иначе // База основана на стоимости затрат
				...
				Если ОшибкаПустойПереченьПрямыхЗатрат Тогда
					Причина.Добавить(НСтр("ru = 'Однако, не задан перечень статей затрат, образующих базу.'"));
				Иначе
			...
			Иначе // База основана на стоимости затрат
				...
				ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ПрямыеЗатраты Тогда
					...
					ПараметрыЧтоНадоЗнать.Вставить("КакОтражаютсяЗатратыОбразующиеБазу", ВидыДокументовОтраженияЗатрат(Организация));
				ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ОтдельныеСтатьиПрямыхЗатрат Тогда
					...
					Иначе
						...
						ПоКакимСтатьям = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'по следующим статьям затрат:
	                              |%1'"),
							СтрСоединить(ПереченьПрямыхЗатрат, Символы.ПС));
			...
			ИначеЕсли МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ОтдельныеСтатьиПрямыхЗатрат
				И Не ОшибкаПустойПереченьПрямыхЗатрат Тогда
				ПроверьтеИсходныеДанные = НСтр("ru = 'Проверьте, что в документах отражения прямых затрат указаны счета 20 и 23 и статьи затрат из списка.'");
			КонецЕсли;
			...
			Если МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.ВыручкаОтРеализацииУслуг
				Или (МетодРасчетаБазы = Перечисления.БазыРаспределенияКосвенныхРасходов.Выручка И Не ОтраженаВыручка) Тогда
				
				ВыводСообщенийОбОшибках.ДобавитьСекцию(Секция, "Текст", ТекстРекомендацииОтразитьРеализациюУслуг());
			...
			Если ОшибкаПустойПереченьПрямыхЗатрат Тогда
				...
				НастройтеРаспределение = (НСтр("ru = 'Укажите в настройке распределения перечень статей прямых затрат, пропорционально суммам которых должны распределяться косвенные затраты.'"));
				ВыводСообщенийОбОшибках.ДобавитьСекцию(Секция, "Текст", НастройтеРаспределение);
		...
		КонецЦикла; // По ГруппаОшибокСчет.Строки - правилам распределения
		
	КонецЦикла; // По счетам затрат
	
КонецПроцедуры

Функция ОтраженаВыручка(Период, Организация)
...
Функция ДокументыОтраженияВыручки()
	
	ДокументыОтраженияВыручки = Новый Массив;
	ДокументыОтраженияВыручки.Добавить(Метаданные.Документы.РеализацияТоваровУслуг);
	ДокументыОтраженияВыручки.Добавить(Метаданные.Документы.ОказаниеУслуг);
	ДокументыОтраженияВыручки.Добавить(Метаданные.Документы.АктОбОказанииПроизводственныхУслуг);
	
	Возврат ДокументыОтраженияВыручки;
	
КонецФункции

Функция ТекстРекомендацииОтразитьРеализациюУслуг()
	
	ПредставленияДокументов = Новый Массив;
	Для Каждого ДокументВыручки Из ДокументыОтраженияВыручки() Цикл
		
		Если Не ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(ДокументВыручки) Тогда
			Продолжить;
		КонецЕсли;
		
		ПредставленияДокументов.Добавить(ДокументВыручки.Синоним);
		
	КонецЦикла;
	
	КоличествоДокументов = ПредставленияДокументов.Количество();
	Если КоличествоДокументов = 0 Тогда
		
		Возврат НСтр("ru = 'Отразите реализацию услуг.'");
		
	ИначеЕсли КоличествоДокументов = 1 Тогда
		
		Возврат СтрШаблон(НСтр("ru = 'Отразите реализацию услуг документом %1.'"), ПредставленияДокументов[0]);
		
	Иначе
		
		ПредставлениеСписка = ОбщегоНазначенияБПКлиентСервер.ПредставлениеСписка(ПредставленияДокументов, "ИЛИ");
		
		Возврат СтрШаблон(НСтр("ru = 'Отразите реализацию услуг документами %1.'"), ПредставлениеСписка);
...
Функция ПереченьПрямыхЗатрат(Счет, Ссылка, Настройки, КешНастроекУчетаЗатрат)
	
	Если КешНастроекУчетаЗатрат = Неопределено Тогда
		КешНастроекУчетаЗатрат = Новый Структура;
		КешНастроекУчетаЗатрат.Вставить("СтатьиЗатрат", Новый Соответствие); // Ключ - Ссылка, Значение - Массив - перечень статей затрат
	КонецЕсли;
	
	СтатьиЗатрат = КешНастроекУчетаЗатрат.СтатьиЗатрат[Ссылка];
	Если СтатьиЗатрат <> Неопределено Тогда
		Возврат СтатьиЗатрат;
	КонецЕсли;
	
	НастройкаРаспределения = Справочники.НастройкиУчетаЗатрат.НастройкаРаспределения(Настройки, Счет, Ссылка);
	
	Если НастройкаРаспределения <> Неопределено Тогда
		СтатьиЗатрат = Справочники.НастройкиУчетаЗатрат.ОтборБазыРаспределения(
			НастройкаРаспределения.БазаРаспределения,
			НастройкаРаспределения.Модификатор);
	Иначе
		СтатьиЗатрат = Новый Массив;
	КонецЕсли;
	
	КешНастроекУчетаЗатрат.СтатьиЗатрат.Вставить(Ссылка, СтатьиЗатрат);
