...
Процедура ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
		МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
		МодульРасчетЗарплатыДляНебольшихОрганизаций.ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений);
	КонецЕсли;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.33.6";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Процедура = "ЗарплатаКадрыДляНебольшихОрганизаций.ЗаполнитьПериодРегистрацииДокументаУвольнение";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("43efa5d1-faf8-4cd4-b55f-6c46308dcd85");

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.37.60";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Процедура = "ЗарплатаКадрыДляНебольшихОрганизаций.ЗаполнитьУчитыватьМРОТПриОплатеПоСреднемуЗаработкуДокументаУвольнение";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("0180dee8-9e09-40e7-b692-1d8e723c05d6");
...
Процедура ЗаполнитьКоличествоСтавокВКадровомУчете(ПараметрыОбновления = Неопределено) Экспорт
	...
	Иначе
		...
		ОписаниеБлокировкиДокументаОперацияБух     = ОбновлениеИБ.ОписаниеБлокируемыхДанных(Метаданные.Документы.ОперацияБух);
		...
		Если НЕ ОбновляемыеДанные.Пустой() Тогда
			...
			Пока ВыборкаОбновляемыхДанных.Следующий() Цикл
				...
				ИначеЕсли ТипЗнч(ВыборкаОбновляемыхДанных.Регистратор) = Тип("ДокументСсылка.ОперацияБух") Тогда
					ОписаниеБлокировкиДокумента = ОписаниеБлокировкиДокументаОперацияБух;
				Иначе
					Продолжить;
				КонецЕсли;
				...
				Если Не ОбновлениеИБ.НачатьОбновлениеДанных(ОписаниеБлокировкиДокумента, ПараметрыОбновления) Тогда
					Возврат
				КонецЕсли;
				Если ТипЗнч(ВыборкаОбновляемыхДанных.Регистратор) <> Тип("ДокументСсылка.Увольнение")
					И ТипЗнч(ВыборкаОбновляемыхДанных.Регистратор) <> Тип("ДокументСсылка.ОперацияБух") Тогда
...
Процедура ЗаполнитьПериодРегистрацииДокументаУвольнение(ПараметрыОбновления = Неопределено) Экспорт
	...
	|ГДЕ
	|	Увольнение.ПериодРегистрации = ДАТАВРЕМЯ(1, 1, 1)";
	
	ОписаниеБлокировкиДокумента = ОбновлениеИБ.ОписаниеБлокируемыхДанных(Метаданные.Документы.Увольнение);
	...
	Если НЕ ОбновляемыеДанные.Пустой() Тогда
		...
		Пока ВыборкаОбновляемыхДанных.Следующий() Цикл
			...
			ОбъектДокумента = ВыборкаОбновляемыхДанных.Ссылка.ПолучитьОбъект();
			Если ЗначениеЗаполнено(ОбъектДокумента.ДатаУвольнения) Тогда
				ПериодРегистрации = НачалоМесяца(ОбъектДокумента.ДатаУвольнения);
			Иначе
				ПериодРегистрации = НачалоМесяца(ОбъектДокумента.Дата);
			КонецЕсли;
			ОбъектДокумента.ПериодРегистрации = ПериодРегистрации;
			ОбъектДокумента.ДополнительныеСвойства.Вставить("ОтключитьПроверкуДатыЗапретаИзменения", Истина);
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ОбъектДокумента);
...
Процедура ЗаполнитьУчитыватьМРОТПриОплатеПоСреднемуЗаработкуДокументаУвольнение(ПараметрыОбновления = Неопределено) Экспорт
	...
	|	Документ.Увольнение КАК Увольнение
	|ГДЕ
	|	Увольнение.УчитыватьМРОТПриОплатеПоСреднемуЗаработку";
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	Увольнение.Ссылка КАК Ссылка,
	|	Увольнение.ДатаУвольнения КАК ДатаСобытия,
	...
	|	И НЕ Увольнение.УчитыватьМРОТПриОплатеПоСреднемуЗаработку";
	...
	Если НЕ ОбновляемыеДанные.Пустой() Тогда
		...
		Пока ВыборкаОбновляемыхДанных.Следующий() Цикл
			...
			КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, ВыборкаОбновляемыхДанных.Сотрудник, "ВидЗанятости, ДоляНеполногоРабочегоВремени", ВыборкаОбновляемыхДанных.ДатаСобытия);
			Если Не (КадровыеДанные.Количество() > 0
				И ((КадровыеДанные[0].ВидЗанятости = Перечисления.ВидыЗанятости.ОсновноеМестоРаботы
				Или КадровыеДанные[0].ВидЗанятости = Перечисления.ВидыЗанятости.Совместительство)
					И (ЗначениеЗаполнено(КадровыеДанные[0].ДоляНеполногоРабочегоВремени) И КадровыеДанные[0].ДоляНеполногоРабочегоВремени >= 1))) Тогда
				Продолжить;
			КонецЕсли;
			...
			ОбъектДокумента = ВыборкаОбновляемыхДанных.Ссылка.ПолучитьОбъект();
			ОбъектДокумента.УчитыватьМРОТПриОплатеПоСреднемуЗаработку = Истина;
			ОбъектДокумента.ДополнительныеСвойства.Вставить("ОтключитьПроверкуДатыЗапретаИзменения", Истина);
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ОбъектДокумента);
