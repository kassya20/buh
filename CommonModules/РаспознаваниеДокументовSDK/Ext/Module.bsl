...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("АктивироватьПромокод", Истина);
	Методы.Вставить("ТекущиеДопустимыеДействияПриАвторизации", Истина);
...
// Возвращает распознанный текст по заданным координатам в документе.
//
// Параметры:
//   ИдентификаторДокумента - Строка - Идентификатор документа, для которого выполняется распознавание
//   Координаты - Структура
//
// Возвращаемое значение:
//   Строка - Распознанный текст в указанной области документа
//
Функция РаспознанныйТекстПоКоординатам(ИдентификаторДокумента, Координаты) Экспорт
	
	Пакет = Новый Структура;
	Пакет.Вставить("field_type", Координаты.ИмяЭлемента);
	Пакет.Вставить("x0", Координаты.КоординатаX0);
	Пакет.Вставить("y0", Координаты.КоординатаY0);
	Пакет.Вставить("x1", Координаты.КоординатаX1);
	Пакет.Вставить("y1", Координаты.КоординатаY1);
	
	Параметры = РаспознаваниеДокументовHTTP.НовыеПараметры();
	Параметры.Json = Пакет;
	Параметры.Заголовки.Вставить("Charset", "utf-8");
	
	ДобавитьЗаголовокАвторизации(Параметры.Заголовки);
	
	Ответ = РаспознаваниеДокументовHTTP.Post(
		АдресСервисаРаспознавания() + "/api/v1/ocr/models/5/reprocess_bbox?doc_id=" + ИдентификаторДокумента,
		,
		Параметры);
	...
	Если Ответ.КодСостояния <> РаспознаваниеДокументовHTTP.КодыСостоянияHTTP().ОК_200 Тогда
		ВызватьИсключение РаспознаваниеДокументовHTTP.КакИсключение(Ответ,
			НСтр("ru='Ошибка распознавания фрагмента.'")
		);
	КонецЕсли;
	...
	Результат = Новый Структура;
	Результат.Вставить("РаспознанныйТекст", ОтветJson.Получить("text"));
...
Функция НоваяСущность() Экспорт
	...
	Сущность.Вставить("ОбластьТекста", Неопределено);
...
Функция ПрочитатьСвойстваСущности(Ключ, СущностьJson)
	...
	Сущность.ОбластьТекста = ПрочитатьОбласть(СущностьJson["refined_bbox"]);
