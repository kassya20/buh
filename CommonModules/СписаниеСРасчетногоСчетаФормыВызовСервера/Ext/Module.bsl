...
Функция ВидыДоговоровДокумента(ВидОперации) Экспорт
	...
	Возврат РаботаСДоговорамиКонтрагентовБПВызовСервера.ВидыДоговоровДокумента(
		ВидОперации, Тип("ДокументСсылка.СписаниеСРасчетногоСчета"));
	
КонецФункции

Функция ОписаниеТипаРеквизитаСделка() Экспорт
	
	Возврат Метаданные.Документы.СписаниеСРасчетногоСчета.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.Сделка.Тип;
...
Функция НастроитьСтатьяДДСДляОперации(СтатьяДДС, КонтекстОперации, Период) Экспорт
	
	Если Не ПравоДоступа("Изменение", Метаданные.Справочники.СтатьиДвиженияДенежныхСредств) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат Не БанкИКассаФормы.СтатьяДДСДляОперацииНастроена(СтатьяДДС, КонтекстОперации, Период);
