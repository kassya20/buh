	...
	Если Не Отказ Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(ИдентификаторЗаявки) Тогда
				ИдентификаторЗаявки = Ответ.РезультатПодключения.ОписаниеПриложения.Идентификатор;
			КонецЕсли;
...
Процедура ПроверитьСостояниеПодключенияФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Если Не ЗначениеЗаполнено(СостояниеПодключения.ИдентификаторЗаявки) Тогда
		Результат.ОшибкаСозданияПриложения = Истина;
		СообщениеОбОшибке = НСтр("ru='Не указан идентификатор заявки.'");
		Результат.СообщениеОбОшибке = СообщениеОбОшибке;
...
Процедура ИнформацияОПриложенииФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Результат = Новый Структура("ОписаниеПриложения,Абонент,ПриложениеЗаблокировано,СообщениеОбОшибке");
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		...
		Если ИСДоступна <> Неопределено Тогда
			Результат.ПриложениеЗаблокировано = Не ИСДоступна;
		КонецЕсли;
...
Процедура ИзменитьАбонентаИППФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	// - получение состояния ИС для КабинетСотрудника.
	
	Отказ = Ложь;
	Результат = Новый Структура("Абонент,ПриложениеЗаблокировано,СообщениеОбОшибке");
	...
	// Получение состояния ИС.
	Если Не Отказ и Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ИСДоступна = ИнтеграцияКабинетСотрудникаОбмен.ИнформационнаяСистемаДоступна();
		УстановитьПривилегированныйРежим(Ложь);
		Если ИСДоступна <> Неопределено Тогда
			Результат.ПриложениеЗаблокировано = Не ИСДоступна;
...
Процедура ОтключитьПриложенияФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	
	Результат = Новый Структура("УдалитьПриложение");
	Результат.УдалитьПриложение = Параметры.УдалитьПриложение;
	
	Приложение = Параметры.Приложение;
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если Параметры.УдалитьПриложение Тогда
	...
	Если Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ЗаявкиСотрудников.АннулироватьНеОбработанныеЗаявки();
	КонецЕсли;
...
Процедура ОбновитьСвойстваПриложенияФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.Приложение <> Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника Тогда
		ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Параметры.ИмяПриложения) Тогда
		Ответ = МенеджерИнтеграции.ОбновитьСвойстваПриложения(Параметры.Идентификатор, Параметры.ИмяПриложения);
		Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
			Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
			Отказ = Истина;
		Иначе
	...
	Если Не Отказ И ЗначениеЗаполнено(Параметры.Ответственный) Тогда
		Отказ = Не ИнтеграцияКабинетСотрудника.ЗарегистрироватьОпубликоватьОтветственного(Параметры.Ответственный);
		Если Не Отказ Тогда
			ОписаниеАдминистратора = ОписаниеАдминистратора(Параметры.Ответственный);
			Ответ = МенеджерИнтеграции.ОбновитьАдминистратораПриложения(Параметры.Идентификатор, ОписаниеАдминистратора);
			Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
				Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
			ИначеЕсли Ответ.АдминистраторИзменен Тогда
				Результат.ОписаниеАдминистратора = Ответ.ОписаниеАдминистратора;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновлениеСведенийОбОплатеФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Ответ = МенеджерИнтеграции.ОбновитьДанныеТарифа();
	Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
		Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПолучениеКодаАктивацииФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	МенеджерИнтеграции = НовыйМенеджерИнтеграции(Приложение, Истина);
	Ответ = МенеджерИнтеграции.ПолучениеКодаАктивации(ИдентификаторИС);
	Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
		Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
	Иначе
...
Процедура ПодключениеПоКодуАктивацииФоновоеЗадание(Параметры, АдресХранилища) Экспорт
	...
	Если Не Отказ Тогда
		ИнтеграцияУправлениеПерсоналом.СохранитьКлючиМенеджера(Приложение, КлючиМенеджера);
		ИнтеграцияУправлениеПерсоналом.УдалитьТокенМенеджера(Приложение);
		Ответ = МенеджерИнтеграции.ПодключениеИСПоКодуАктивации(ИдентификаторИС, КодАктивации);
		Если Ответ.СообщениеОбОшибке <> Неопределено Тогда
			Результат.СообщениеОбОшибке = Ответ.СообщениеОбОшибке;
...
Функция ОписаниеОбъектаДанныеДляРазвертывания(Данные, Приложение)
	...
	Описание.ФизическоеЛицо = Данные.Администратор;
