...
Функция ПараметрыЗаполненияТиповойНаОсновеФормыТОРГ12(Знач НашДокумент, Знач ВидОперации, Знач ТипДокументаСтрокой) Экспорт
	...
	СтрокиТаблицыОборудование = Новый Массив;
	...
	ЭтоВидОперацииОборудование = (ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование);
	Если ЭтоВидОперацииОборудование Тогда
		Номенклатура = ТаблицаДокумента.ВыгрузитьКолонку("Номенклатура");
		ВидыНоменклатурыДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Номенклатура, "ВидНоменклатуры");
		ОсновнойВидНоменклатуры = Справочники.ВидыНоменклатуры.ПустаяСсылка();
		
		ВидыНоменклатурыОборудование = РаботаСНоменклатуройПовтИспБП.ВидыНоменклатурыДляВыбора(
			ОсновнойВидНоменклатуры, "Оборудование");
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицыДокумента Из ТаблицаДокумента Цикл
		...
		Если ЭтоВидОперацииОборудование Тогда
			ВидНоменклатуры = ВидыНоменклатурыДокумента.Получить(СтрокаТаблицыДокумента.Номенклатура);
			Если ВидыНоменклатурыОборудование.НайтиПоЗначению(ВидНоменклатуры) <> Неопределено Тогда
				СтрокиТаблицыОборудование.Добавить(ДанныеСтрокиТаблицы);
				Продолжить;
			КонецЕсли;
		КонецЕсли;
	...
	ПараметрыЗаполнения.Вставить("Оборудование", СтрокиТаблицыОборудование);
