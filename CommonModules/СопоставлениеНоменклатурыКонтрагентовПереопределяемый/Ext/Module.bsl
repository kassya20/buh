...
Процедура ПриСозданииНаСервере_ФормаВыбораНоменклатурыКонтрагентов(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтеграцияС1СДокументооборотом") Тогда
		...
		МодульИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриСозданииНаСервере(Форма);
	КонецЕсли;
...
Процедура ПриОпределенииПредставленийСопоставленияНоменклатуры(Представления) Экспорт
	...
	Представления.НоменклатураПредставлениеОбъекта = НСтр("ru = 'Номенклатура / Основное средство / Объект строительства'");
...
Процедура ПриСохраненииСопоставленияНоменклатурыКонтрагентов(Знач НоменклатураКонтрагента, Знач НоменклатураИБ, Отказ = Ложь, ТекстОшибки = "") Экспорт
	...
	ОбменСКонтрагентамиБП.ПриСохраненииСопоставленияНоменклатурыКонтрагентов(НоменклатураКонтрагента, НоменклатураИБ);
...
Процедура ПриОтраженииВУчетеЭлектронногоДокументаСНоменклатурой(Знач Документ, Настройки) Экспорт
	...
	СпособОбработкиПервичногоДокумента = ОбменСКонтрагентамиБП.СпособОбработкиПервичногоДокумента(Документ.СпособОбработки);
	...
	Если СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеУслуг
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеДопРасходов
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеИзПереработки
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.СчетФактураПолученный
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеУслугАренды
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеУслугЛизинга
			Или СпособОбработкиПервичногоДокумента = СпособыОбработки.ПоступлениеУслугФакторинга Тогда
		Настройки.ОтражатьТолькоСопоставленные = Ложь;
	КонецЕсли;
	
	// ЭлектронноеВзаимодействие.ЭлектронноеАктированиеЕИС
	Если СпособОбработкиПервичногоДокумента = "ГосударственныеКонтрактыЕИС" Тогда
		Настройки.ОтражатьТолькоСопоставленные = Ложь;
	КонецЕсли;
...
Процедура ЗначенияНаименованийДанныхСопоставленияНоменклатурыБЭД(Знач НаборСсылокНаОбъекты, СоответствиеЗначенийНаименований) Экспорт
	...
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивНоменклатуры, "Наименование, НаименованиеПолное");
	Для Каждого ЭлементСоответствия Из ДанныеНоменклатуры Цикл
		ЗначениеСоответствия = ЭлементСоответствия.Значение;
		СоответствиеЗначенийНаименований.Вставить(ЭлементСоответствия.Ключ,
			?(ЗначениеЗаполнено(ЗначениеСоответствия.НаименованиеПолное), ЗначениеСоответствия.НаименованиеПолное, ЗначениеСоответствия.Наименование));
	КонецЦикла;
...
Процедура ОпределитьНаименованиеТипаОбъектаСопоставленияНоменклатурыБЭД(Знач СсылкаНаОбъект, НаименованиеТипаОбъекта) Экспорт
	...
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ОсновныеСредства") Тогда
		НаименованиеТипаОбъекта = "Основное средство";
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ОбъектыСтроительства") Тогда
		НаименованиеТипаОбъекта = "Объект строительства";
	КонецЕсли;
...
Процедура ИнициализацияТекстаЗапросаПоискаСопоставленияПоНатуральнымКлючам(ТекстЗапроса) Экспорт
	...
	|	И НЕ НоменклатураКонтрагента.ВАрхиве
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ИдентификаторНоменклатуры,
	|	Номенклатура";
...
Процедура ПодобратьВариантыСопоставленияПоНатуральнымКлючам(ТаблицаНоменклатурыКонтрагентов, ДополнительныеПараметры) Экспорт
	...
	СопоставлениеНоменклатурыКонтрагентовБП.ПодобратьВариантыСопоставленияПоНатуральнымКлючам(ТаблицаНоменклатурыКонтрагентов, ДополнительныеПараметры);
