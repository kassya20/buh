...
// Возвращает признак того, что отчет предназначен для подготовки заявления на получение патента
// 
// Параметры:
...
// Возвращаемое значение:
//  Булево
Функция ЭтоЗаявлениеНаПолучениеПатента(ИмяОтчета) Экспорт
	
	Возврат ИмяОтчета = "РегламентированноеУведомлениеПолучениеПатента";
...
Функция РасчетПлатежейПоПатенту(СуммаНалогаКУплате, НачалоПатента, ОкончаниеПатента) Экспорт
	...
	// пп. "б" п. 69 ст. 2 Федерального закона от 08.08.2024 № 259-ФЗ изменил срок уплаты налога по патенту (п. 2 ст. 346.51 НК РФ).
	// Ранее налог уплачивался не позднее срока окончания действия патента.
	// После вступления в силу новой редакции НК, если срок окончания действия патента приходится на 31 декабря,
	// то уплата налога производится не позднее 28 декабря.
	// Изменения вступают в силу с 1 января 2025 года.
	ДатаПлатежа = ОкончаниеПатента;
	Если НачалоПатента >= ДатаНачалаПримененияИзмененийСрокаУплатыНалога()
		И КонецДня(ОкончаниеПатента) = КонецГода(ОкончаниеПатента) Тогда
		ДатаПлатежа = Дата(Год(ОкончаниеПатента), 12, 28);
	КонецЕсли;
	...
	Если УплачиваетсяОдинПлатеж(НачалоПатента, ОкончаниеПатента) Тогда
		...
		// Если окончание патента приходится на 31 декабря, то уплата производится не позднее 28 декабря.
		Расчет.СуммаПервогоПлатежа = СуммаНалогаКУплате;
		Расчет.ДатаПервогоПлатежа = УчетПСНВызовСервера.УточнитьСрокОплатыПатента(ДатаПлатежа);
	...
	Иначе
		...
		//  - второй платеж 2/3 суммы не позднее срока окончания патента, при этом если срок окончания приходится на 31 декабря,
		//    то уплата производится не позднее 28 декабря;
		...
		Расчет.ДатаВторогоПлатежа = УчетПСНВызовСервера.УточнитьСрокОплатыПатента(ДатаПлатежа);
	КонецЕсли;
...
// Возвращает дату вступления в силу Федерального закона от 08.08.2024 № 259-ФЗ,
// согласно которому, если срок окончания действия патента приходится на 31 декабря,
// то уплата налога производится не позднее 28 декабря.
// 
// Возвращаемое значение:
//   Дата - 1 января 2025 года
//
Функция ДатаНачалаПримененияИзмененийСрокаУплатыНалога() Экспорт
	
	Возврат Дата(2025, 1, 1);
	
КонецФункции

// Возвращает дату вступления в силу новой формы заявления на патент по приказу ФНС от 18.12.2025 г. № ЕД-7-3/1226@,
// согласно которой можно подать заявление на получение патента взамен ранее выданного.
// 
// Возвращаемое значение:
//   Дата - 2 марта 2026 года
//
Функция ДатаНачалаДействияВозможностиВнесенияИзмененийВПатент() Экспорт
	
	Возврат Дата(2026, 3, 2);
