...
Процедура ЗаполнитьСоответствиеПолейДокументовОснованийИПроизводственнойОперации(СоответствиеПолей) Экспорт
	...
	СоответствиеПолей["ОтчетПроизводстваЗаСмену"].Вставить("Предприятие", "ПодразделениеЗатрат");
	
	//ТребованиеНнакладная
	СоответствиеПолей.Вставить("ТребованиеНакладная",Новый Соответствие);
	СоответствиеПолей["ТребованиеНакладная"].Вставить("ХозяйствующийСубъект", "Организация");
	СоответствиеПолей["ТребованиеНакладная"].Вставить("Предприятие", "ПодразделениеЗатрат");
...
Процедура ЗаполнитьСвойстваЭлементовПроизводственнойТранзакцииПоДокументуОснованию(Свойства, ДокументОснование) Экспорт
	...
	Если ИнтеграцияВЕТИСБПВызовСервера.ИспользоватьРаздельноеСписаниеМатериаловИВыпускПродукцииВЕТИС() Тогда
		СвойстваПоУмолчанию = Новый Структура("Видимость, Доступность, ТолькоПросмотр", Истина, Истина, Ложь);
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементаЗавершениеПроизводственнойТранзакцииПоДокументуОснованию(Форма) Экспорт
	
	Если ИнтеграцияВЕТИСБПВызовСервера.ИспользоватьРаздельноеСписаниеМатериаловИВыпускПродукцииВЕТИС() Тогда
			Форма.Элементы.ЗавершениеПроизводственнойТранзакции.Видимость = 
				ИнтеграцияВЕТИСБПВызовСервера.ПолучитьВидимостьЭлементаЗавершенияПроизводственнойТранзакцииПоОснованию(Форма.Объект);
	КонецЕсли;
