...
Асинх Функция ПроверитьВозрастАсинх(ЦифровойID, ТокенОрганизации) Экспорт
	...
	НастройкиСоединения = МенеджерОборудованияКлиентСервер.НастройкиСоединенияПроверкиВозраста();
	ПараметрыСоединения = НовыеПараметрыСоединения();
	ЗаполнитьЗначенияСвойств(ПараметрыСоединения, НастройкиСоединения);
	ПараметрыСоединения.АдресРесурса = СтрШаблон(НастройкиСоединения.АдресРесурса, ЦифровойID);
	...
	Если РезультатВызова.КодОтвета = 200 Тогда
		Попытка 
			ЗначениеJSON = ПрочитатьЗначениеJSON(РезультатВызова.ТелоОтвета);
			Если ЗначениеJSON.is_adult Тогда
				Результат.ВремяЗапроса = ОбщегоНазначенияБПОКлиентСервер.TimestampВДату(ЗначениеJSON.timestamp);
		...
		КонецПопытки;
	ИначеЕсли РезультатВызова.КодОтвета = 400 Тогда
		Результат.ОписаниеОшибки = НСтр("ru = 'Ошибка подтверждения возраста. Обновите QR код в MAX и попробуйте еще раз.'");
