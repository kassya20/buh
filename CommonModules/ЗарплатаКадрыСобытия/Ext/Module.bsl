...
Процедура ЗапретитьРедактированиеОбщихДанныхНаборЗаписейПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	Если Источник.Количество() = 0 Или РегистрыБЗК.ЭтоУдалениеНабораЗаписей(Замещение) Тогда
		...
		ИмяФлажкаРазрешено = "_УдалениеРазрешено";
		Если ТаблицаОтборов.Колонки.Найти(ИмяФлажкаРазрешено) = Неопределено Тогда
			ТаблицаОтборов.Колонки.Добавить(ИмяФлажкаРазрешено, Новый ОписаниеТипов("Булево"));
			Для Каждого Измерение Из Источник.Метаданные().Измерения Цикл
				Если Измерение.Ведущее Тогда
					Фильтр = Новый Структура(ИмяФлажкаРазрешено, Ложь);
					Фильтр.Вставить(Измерение.Имя + "_Использование", Истина);
					Найденные = ТаблицаОтборов.НайтиСтроки(Фильтр);
					Для Каждого СтрокаОтбора Из Найденные Цикл
						ЗначениеОтбора = СтрокаОтбора[Измерение.Имя];
						Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ЗначениеОтбора))
							И Не ОбщегоНазначения.СсылкаСуществует(ЗначениеОтбора) Тогда
							СтрокаОтбора[ИмяФлажкаРазрешено] = Истина;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		Если ТаблицаОтборов.Найти(Ложь, ИмяФлажкаРазрешено) = Неопределено Тогда
			Возврат; // Удаление всех строк разрешено.
		КонецЕсли;
	КонецЕсли;
