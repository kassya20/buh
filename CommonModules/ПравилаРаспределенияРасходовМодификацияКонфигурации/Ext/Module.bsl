...
//
// Параметры:
//  Настройки - Структура - см. Справочники.НастройкиУчетаЗатрат.НовыйИсполняемыеНастройкиУчетаЗатрат, заполненные настройки
//
...
//               Для изменения состава переиспользуемых временных таблиц следует использовать
...
Процедура ЗакрыватьОбщехозяйственныеРасходыНаКалькуляционныйСчет(Настройки)  Экспорт // для обхода проверки конфигурации
	...
	НастройкиОбщехозяйственныеРасходы = Настройки.Закрытие[ПланыСчетов.Хозрасчетный.ОбщехозяйственныеРасходы];
	
	Если НастройкиОбщехозяйственныеРасходы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	НастройкиОбщехозяйственныеРасходы.Роль = Перечисления.РолиСчетовЗатрат.КосвенныеЗатраты;
