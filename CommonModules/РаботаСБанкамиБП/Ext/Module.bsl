...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СброситьВерсиюКлассификатораБанков");
	Методы.Вставить("ОбновитьБанковскиеСчетаГосударственныхОрганов");
	Методы.Вставить("ДобавитьКазначейскийСчетЕНПДляНалоговыхОрганов");
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры) Экспорт
	...
	Если Обработан И Не ОбщегоНазначения.РазделениеВключено() Тогда
		Обработан = РаботаСБанкамиБП.ОбновитьБанкиИзКлассификатора();
	КонецЕсли;
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Для Каждого Классификатор Из Классификаторы Цикл
		Если Классификатор.Идентификатор = "BIC" Тогда
			Классификатор.ОбработкаРазделенныхДанных = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
#Область РаботаСКонтрагентамиБанков

Функция ПолучитьДанныеКонтрагентаБанка(ДанныеКонтрагентаБанка, СоздаватьКонтрагента = Истина) Экспорт
	
	Если ТипЗнч(ДанныеКонтрагентаБанка) = Тип("Структура") Тогда
		ДанныеБанка = ОбщегоНазначения.СкопироватьРекурсивно(ДанныеКонтрагентаБанка);
	Иначе
		ДанныеБанка = ПолучитьДанныеБанка(ДанныеКонтрагентаБанка);
	КонецЕсли;
	
	Контрагент = КонтрагентБанка(ДанныеБанка);
	Если СоздаватьКонтрагента И Не ЗначениеЗаполнено(Контрагент) Тогда
		Контрагент = СоздатьКонтрагентБанка(ДанныеБанка);
	КонецЕсли;
	
	ДанныеБанка.Вставить("Ссылка", Контрагент);
	
	Возврат ДанныеБанка;
	
КонецФункции

Функция ДоговорКонтрагентБанка(Организация, Контрагент, СоздаватьНовыеЭлементы = Истина) Экспорт
	...
	РаботаСДоговорамиКонтрагентовБП.УстановитьДоговорКонтрагента(
		ДоговорКонтрагента, Контрагент, Организация, ВидыДоговоров, ПараметрыПлатежа);
	
	Если Не ЗначениеЗаполнено(ДоговорКонтрагента) И СоздаватьНовыеЭлементы Тогда
...
Функция КлассификаторБанкаПоКонтрагенту(Контрагент) Экспорт
	...
	РеквизитыКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, "ИНН, НаименованиеПолное");
	Наименование = ОбщегоНазначенияБПКлиентСервер.СокращенноеНаименованиеПоПолномуНаименованию(
		РеквизитыКонтрагента.НаименованиеПолное);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИНН", РеквизитыКонтрагента.ИНН);
	Запрос.УстановитьПараметр("НаименованиеПолное", ВРег(РеквизитыКонтрагента.НаименованиеПолное));
	Запрос.УстановитьПараметр("Наименование", ВРег(Наименование));
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	КлассификаторБанков.Ссылка КАК Ссылка,
	|	ВРЕГ(КлассификаторБанков.Наименование) = &Наименование
	|		ИЛИ ВРЕГ(КлассификаторБанков.Наименование) = &НаименованиеПолное КАК Приоритет
	|ИЗ
	|	Справочник.КлассификаторБанков КАК КлассификаторБанков
	|ГДЕ
	|	КлассификаторБанков.ИНН = &ИНН
	|	И НЕ КлассификаторБанков.ДеятельностьПрекращена
	|	И НЕ КлассификаторБанков.ПометкаУдаления
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ДобавитьКазначейскийСчетЕНПДляНалоговыхОрганов(Параметры) Экспорт
	
	// Создаем у налоговых органов новый казначейский счет для уплаты прочих "неналоговых" платежей, не входящих в ЕНП.
	// Но не делаем его "основным", т.к. обработка обновления может выполняться до всеобщего перехода на ЕНП.
	
	ПлатежныеРеквизиты = ЕдиныйНалоговыйСчет.ЗначенияРеквизитовЕдиногоНалоговогоСчета();
	
	Банк = БанкПоПлатежнымРеквизитам(ПлатежныеРеквизиты);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НомерСчетаЕНП", ПлатежныеРеквизиты.РасчетныйСчет);
	Запрос.УстановитьПараметр("БИКЕНП",        ПлатежныеРеквизиты.БИК);
	Запрос.УстановитьПараметр("КоррСчетЕНП",   ПлатежныеРеквизиты.КоррСчет);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Контрагенты.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТ_КонтрагентыССчетомЕНП
	|ИЗ
	...
	|ГДЕ
	|	БанковскиеСчета.НомерСчета = &НомерСчетаЕНП
	|	И БанковскиеСчета.Банк.Код = &БИКЕНП
	|	И БанковскиеСчета.Банк.КоррСчет = &КоррСчетЕНП
	...
	|	ЕСТЬNULL(Контрагенты.ОсновнойБанковскийСчет.ТекстКорреспондента, """") КАК ТекстКорреспондента
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КонтрагентыССчетомЕНП КАК ВТ_КонтрагентыССчетомЕНП
	|		ПО Контрагенты.Ссылка = ВТ_КонтрагентыССчетомЕНП.Ссылка
	|ГДЕ
	...
	|	И ВТ_КонтрагентыССчетомЕНП.Ссылка ЕСТЬ NULL";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КонтрагентОбъект = Неопределено Или Не ЗначениеЗаполнено(КонтрагентОбъект.Ссылка) Тогда
			Продолжить;
		КонецЕсли;
		
		ПлатежныеРеквизиты.ТекстКорреспондента = Выборка.ТекстКорреспондента;
		СоздатьКазначейскийСчет(Контрагент, ПлатежныеРеквизиты, Банк)
	КонецЦикла;
	
КонецПроцедуры

Процедура СоздатьКазначейскийСчет(Владелец, ПлатежныеРеквизиты, Банк = Неопределено) Экспорт
	...
	БанковскийСчет.Наименование = БанковскиеСчетаФормыКлиентСервер.НаименованиеБанковскогоСчета(
		БанковскийСчет, БанкОбъект.Наименование);
	Если ЗначениеЗаполнено(ПлатежныеРеквизиты.ТекстКорреспондента) Тогда
		БанковскийСчет.ТекстКорреспондента = СокрЛП(ПлатежныеРеквизиты.ТекстКорреспондента);
	ИначеЕсли ЕдиныйНалоговыйСчет.ЭтоРеквизитыЕдиногоНалоговогоСчета(ПлатежныеРеквизиты) Тогда
		...
		Если ЗначениеЗаполнено(ОсновнойБанковскийСчет) Тогда
			БанковскийСчет.ТекстКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
	...
	НачатьТранзакцию();
	Попытка
		Если ЗаписыватьБанк Тогда
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(БанкОбъект);
			Банк = БанкОбъект.Ссылка;
		КонецЕсли;
		
		БанковскийСчет.Владелец = Владелец;
	...
	Исключение
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка создания казначейского счета у контрагента %1:'"), Владелец)
			+ Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		ЗаписьЖурналаРегистрации(ОбщегоНазначенияБП.СобытиеЖурналаРегистрацииОшибкаЗаписиОбъекта(),
			УровеньЖурналаРегистрации.Ошибка,
...
Процедура ОбновитьБанковскиеСчетаГосударственныхОрганов(Параметры) Экспорт
	...
	|			И БанковскиеСчета.Банк.Код = ВТ_ТаблицаСчетов.БИК
	|ГДЕ
	...
	|	И ТаблицаСоответствияСчетовКонтрагентов.Владелец.Ссылка ЕСТЬ НЕ NULL ";
...
Функция ПолучитьДанныеБанка(Банк)
	
	РеквизитыБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Банк, "Код, КоррСчет, Наименование");
	БанкПоКлассификатору = РаботаСБанкамиБП.СсылкаПоКлассификатору(РеквизитыБанка.Код, РеквизитыБанка.КоррСчет);
	Если ЗначениеЗаполнено(БанкПоКлассификатору) Тогда
		ДанныеБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(БанкПоКлассификатору, "ИНН, Наименование");
	Иначе
		ДанныеБанка = Новый Структура("ИНН, Наименование");
		ЗаполнитьЗначенияСвойств(ДанныеБанка, РеквизитыБанка);
	КонецЕсли;
...
Функция КонтрагентБанка(ДанныеБанка)
	
	Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	Если Не ЗначениеЗаполнено(ДанныеБанка.ИНН) Тогда
		Если ДанныеБанка.Свойство("Наименование") И ЗначениеЗаполнено(ДанныеБанка.Наименование) Тогда
			Контрагент = КонтрагентБанкаПоНаименованию(ДанныеБанка);
		КонецЕсли;
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИНН", ДанныеБанка.ИНН);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 2
	|	Контрагенты.Ссылка КАК Ссылка
	|ИЗ
...
Функция КонтрагентБанкаПоНаименованию(ДанныеБанка)
	
	Контрагент = Справочники.Контрагенты.ПустаяСсылка();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Наименование", ДанныеБанка.Наименование);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 2
	|	Контрагенты.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.НаименованиеПолное = &Наименование
	|	И НЕ Контрагенты.ПометкаУдаления";
...
Функция СоздатьКонтрагентБанка(ДанныеБанка)
	
	Если Не ЗначениеЗаполнено(ДанныеБанка.ИНН) Тогда
		Возврат Справочники.Контрагенты.ПустаяСсылка();
	КонецЕсли;
	
	ДанныеКонтрагента = Новый Структура("НаименованиеПолное, Наименование, ИНН, КПП, ОГРН, ОКВЭД, КодПоОКПО");
	ЗаполнитьЗначенияСвойств(ДанныеКонтрагента, ДанныеБанка);
	
	НаименованиеИФилиал = Справочники.Контрагенты.ВыделитьФилиалИзНаименованияКонтрагента(ДанныеБанка.Наименование);
	ЭтоФилиалБанка = ЗначениеЗаполнено(НаименованиеИФилиал.ТекстФилиал);
	
	РезультатПроверки = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(
		ДанныеБанка.ИНН, Истина);
	Попытка
		// Для филиала банка заполнение по ЕГРЮЛ не выполняем, т.к. данные по ИНН в этом случае будут не валидны.
		Если Не ЭтоФилиалБанка И РезультатПроверки.СоответствуетТребованиям И РезультатПроверки.ЭтоЮрЛицо Тогда
			СведенияОКонтрагенте = РаботаСКонтрагентами.СведенияОЮридическомЛицеПоИНН(ДанныеБанка.ИНН);
			Если ЗначениеЗаполнено(СведенияОКонтрагенте.ЕГРЮЛ) Тогда
				ЗаполнитьЗначенияСвойств(ДанныеКонтрагента, СведенияОКонтрагенте.ЕГРЮЛ);
			КонецЕсли
		КонецЕсли;
	Исключение
	...
	Если Не ЗначениеЗаполнено(ДанныеКонтрагента.НаименованиеПолное) Тогда
		ДанныеКонтрагента.НаименованиеПолное = ДанныеКонтрагента.Наименование;
	КонецЕсли;
	
	ДанныеКонтрагента.Наименование =
		Справочники.Контрагенты.ПолучитьКраткоеНаименованиеКонтрагента(ДанныеКонтрагента.НаименованиеПолное);
