...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДобавитьОбновитьПолучательКазначейство");
	Методы.Вставить("ДобавитьМИДолПолучателяЕНП");
	Методы.Вставить("ПроверитьПлатежныеРеквизитыКонтрагентаВФоне", Истина);
...
//
// Возвращаемое значение:
//   Строка - код региона по адресному классификатору (без лидирующего нуля) - см. КодСубъектаРФ
//            в РегистрыСведений.АдресныеОбъекты.КлассификаторСубъектовРФ()
//
Функция КодРегионаПоКодуНалоговогоОргана(Знач КодНалоговогоОргана) Экспорт
	...
	Если КодНалоговогоОргана = КодРегионаФедеральнойТерриторииСириус() Тогда
		Возврат КодНалоговогоОргана;
	КонецЕсли;
	
	КодРегиона = Лев(КодНалоговогоОргана, 2);
	Пока СтрНачинаетсяС(КодРегиона, "0") Цикл
		КодРегиона = Сред(КодРегиона, 2);
	КонецЦикла;
...
// Возвращает служебный код региона федеральной территории Сириус.
// Это код единственной ИФНС в Сириусе.
// 
// Возвращаемое значение:
//  Строка
Функция КодРегионаФедеральнойТерриторииСириус() Экспорт
	
	Возврат "2378";
	
КонецФункции

// Возвращает служебный код региона федеральной территории Сириус в числовом формате.
// См. КодРегионаФедеральнойТерриторииСириус()
// 
// Возвращаемое значение:
//  Число
Функция КодРегионаФедеральнойТерриторииСириусЧислом() Экспорт
	
	Возврат Число(КодРегионаФедеральнойТерриторииСириус());
	
КонецФункции

// Возвращает наименование федеральной территории Сириус для формирования представления
// в списках выбора, полях ввода и т.п..
// 
// Возвращаемое значение:
//  Строка
Функция НаименованиеФедеральнойТерриторииСириус() Экспорт
	
	Возврат НСтр("ru='Федеральная территория ""Сириус""'");
...
Процедура ОбновитьДанныеГосударственногоОргана(Реквизиты, ОбработкаОбновления = Ложь) Экспорт
	...
	РезультатПроверкиИНН = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямИНН(Объект.ИНН, Истина);
	РезультатПроверкиКПП = ИдентификационныеНомераНалогоплательщиков.ПроверитьСоответствиеТребованиямКПП(Объект.КПП, Истина, Ложь);
	
	Объект.ИННВведенКорректно = ПустаяСтрока(РезультатПроверкиИНН.ОписаниеОшибки) И РезультатПроверкиИНН.СоответствуетТребованиям;
	Объект.КППВведенКорректно = ПустаяСтрока(РезультатПроверкиКПП.ОписаниеОшибки) И РезультатПроверкиКПП.СоответствуетТребованиям;
...
//
Функция КодГосударственногоОрганаОрганизации(Организация, ВидГосударственногоОргана, ТолькоОсновнаяРегистрация = Ложь) Экспорт
	...
	Если ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
		Если ВидГосударственногоОргана = Перечисления.ВидыГосударственныхОрганов.НалоговыйОрган Тогда
			...
			Если Не ТолькоОсновнаяРегистрация Тогда
				УчетОбособленныхПодразделений.УточнитьКодНалоговогоОрганаОрганизации(Код, Организация, ВидГосударственногоОргана);
			КонецЕсли;
...
Функция БанковскийСчетПоРеквизитам(Контрагент, РасчетныйСчет, БИК, КоррСчет, Знач НаименованиеБанка = "", СоздаватьНовый = Ложь) Экспорт
	...
	БанковскийСчетОбъект.ВсегдаУказыватьКПП = Истина;
	...
	БанковскийСчетОбъект.Наименование = БанковскиеСчетаФормыКлиентСервер.НаименованиеБанковскогоСчета(
