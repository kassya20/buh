...
Процедура СоставПриИзмененииНаСервере(Форма, ИмяТабличнойЧасти = "Зарплата") Экспорт
	
	// Из Состав в табличные части оплат ведомостей переносятся значения имеющихся реквизитов
	
	РеквизитыЗарплата = Форма.Объект.Ссылка.Метаданные().ТабличныеЧасти[ИмяТабличнойЧасти].Реквизиты;
	...
	ИзменяемыеСтроки = Форма.Объект[ИмяТабличнойЧасти].НайтиСтроки(Новый Структура("ИдентификаторСтроки", СтрокаСостава.ИдентификаторСтроки));
...
Процедура ПриПолученииДанныхСтрокиСостава(Форма, СтрокаСостава) Экспорт
	...
	ПриПолученииДанныхСтрокиСоставаВзысканнаяСумма(Форма, СтрокаСостава);
...
Процедура ПриПолученииДанныхСтрокиСоставаВзысканнаяСумма(Форма, СтрокаСостава)
	
	МетаданныеТаблицы = Метаданные.НайтиПоТипу(ТипЗнч(Форма.Объект.Ссылка)).ТабличныеЧасти.Зарплата;
	Если Не ОбщегоНазначения.ЕстьРеквизитОбъекта("ВзысканнаяСумма", МетаданныеТаблицы) Тогда
		Возврат;
	КонецЕсли;
	...
	СтрокаСостава.ВзысканнаяСумма = Форма.Объект.Зарплата.Выгрузить(СтрокиЗарплатыРаботника, "ВзысканнаяСумма").Итог("ВзысканнаяСумма");
	
	// Для формирования расшифровки использовать только строки с ненулевыми взысканными суммами.
	Зарплата = Форма.Объект.Зарплата.Выгрузить().СкопироватьКолонки();
	Для каждого Строка Из СтрокиЗарплатыРаботника Цикл
		Если Строка.ВзысканнаяСумма <> 0 Тогда
			НоваяСтрока = Зарплата.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		КонецЕсли;
	КонецЦикла;
	
	РабочиеМеста = Зарплата.Скопировать(, "Сотрудник, Подразделение");
	...
	ПериодыВзаиморасчетов = Зарплата.Скопировать(, "ПериодВзаиморасчетов");
	ПериодыВзаиморасчетов.Свернуть("ПериодВзаиморасчетов");
	
	РасшифровкаПериодов = "";
	Если ПериодыВзаиморасчетов.Количество() = 0
		Или (ПериодыВзаиморасчетов.Количество() = 1 И ПериодыВзаиморасчетов[0].ПериодВзаиморасчетов = ПериодРегистрацииВедомости) Тогда
	...
	СтрокаСостава.ВзысканнаяСуммаРасшифровка = "";
	Если ЗначениеЗаполнено(РасшифровкаПериодов) Тогда
		СтрокаСостава.ВзысканнаяСуммаРасшифровка = СтрокаСостава.ВзысканнаяСуммаРасшифровка + РасшифровкаПериодов;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(РасшифровкаРабочихМест) Тогда
		СтрокаСостава.ВзысканнаяСуммаРасшифровка = 
			СтрокаСостава.ВзысканнаяСуммаРасшифровка
			+ ?(ЗначениеЗаполнено(СтрокаСостава.ВзысканнаяСуммаРасшифровка), "; " + Символы.ПС, "")
			+ РасшифровкаРабочихМест;
	КонецЕсли;
...
Процедура ПриПолученииДанныхСтрокСостава(Форма) Экспорт
	...
	ПриПолученииДанныхСтрокСоставаВзысканнаяСумма(Форма);
...
Процедура ПриПолученииДанныхСтрокСоставаВзысканнаяСумма(Форма)
	...
	// Для формирования расшифровки использовать только строки с ненулевыми взысканными суммами.
	Зарплата = Форма.Объект.Зарплата.Выгрузить().СкопироватьКолонки();
	Для каждого Строка Из Форма.Объект.Зарплата Цикл
		Если Строка.ВзысканнаяСумма <> 0 Тогда
			НоваяСтрока = Зарплата.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
		КонецЕсли;
	...
	ЗарплатаВзыскано = Зарплата.Скопировать(, "ИдентификаторСтроки, ВзысканнаяСумма");
	ЗарплатаВзыскано.Свернуть("ИдентификаторСтроки", "ВзысканнаяСумма");
	ЗарплатаВзыскано.Индексы.Добавить("ИдентификаторСтроки");
	
	ЗарплатаРабочиеМеста = Зарплата.Скопировать(,"ИдентификаторСтроки, Сотрудник, Подразделение");
	ЗарплатаРабочиеМеста.Свернуть("ИдентификаторСтроки, Сотрудник, Подразделение");
	ЗарплатаРабочиеМеста.Индексы.Добавить("ИдентификаторСтроки");
	
	ЗарплатаПериодыВзаиморасчетов = Зарплата.Скопировать(,"ИдентификаторСтроки, ПериодВзаиморасчетов");
	ЗарплатаПериодыВзаиморасчетов.Свернуть("ИдентификаторСтроки, ПериодВзаиморасчетов");
	ЗарплатаПериодыВзаиморасчетов.Индексы.Добавить("ИдентификаторСтроки");
	...
	Для Каждого СтрокаСостава Из СтрокиСостава Цикл
		...
		СтрокаЗарплатаВзысканнаяСумма = ЗарплатаВзыскано.Найти(СтрокаСостава.ИдентификаторСтроки,"ИдентификаторСтроки");
		...
		Если ЗначениеЗаполнено(РасшифровкаРабочихМест) Тогда
			СтрокаСостава.ВзысканнаяСуммаРасшифровка = СтрокаСостава.ВзысканнаяСуммаРасшифровка
				+ ?(ЗначениеЗаполнено(СтрокаСостава.ВзысканнаяСуммаРасшифровка), "; " + Символы.ПС, "")
				+ РасшифровкаРабочихМест;
		КонецЕсли;
	КонецЦикла;
...
Функция СписокОплат(Форма)
	...
	Для Каждого Регистратор Из Регистраторы Цикл
		...
		НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(РеквизитыРегистратора[Регистратор].Номер, Истина, Ложь);
		ДатаДокумента  = Формат(РеквизитыРегистратора[Регистратор].Дата, "ДЛФ=Д");
...
Процедура ЗаполнитьНаСервере(Форма) Экспорт
	...
	Ведомость.ЗагрузитьВыплаты(Зарплата, НДФЛ, ПараметрыЗаполнения.Финансирование);
...
Процедура ДополнитьНаСервере(Форма, ФизическиеЛица)
	...
	Ведомость.ДобавитьВыплаты(Зарплата, НДФЛ, ПараметрыЗаполнения.Финансирование);
...
Процедура РедактироватьЗарплатуСтрокиЗавершениеНаСервере(Форма, РезультатыРедактирования) Экспорт
	...
	Для Каждого РеквизитСостава Из Форма.Объект.Ссылка.Метаданные().ТабличныеЧасти.Состав.Реквизиты  Цикл
		...
		КонецЕсли;
		Если РеквизитСостава.Имя <> "ВзысканнаяСумма" Тогда
			ПоляСостава.Добавить(РеквизитСостава.Имя);
		КонецЕсли;
	КонецЦикла;
	СписокСвойств = СтрСоединить(ПоляСостава, ", ");
