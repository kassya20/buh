...
Функция УстановитьСоединениеССерверомМеханизмаОнлайнСервисовРО(ОписаниеОшибки = Неопределено, Таймаут = 120) Экспорт
	...
	Исключение 
		...
		Исключение
			...
			Шаблон = НСтр("ru='Ошибка установки соединения с сервером %1:'") + Символы.ПС + "%2";
...
Функция ПолучитьРесурсССервера(Соединение, ИмяРесурсаНаСервере, ОписаниеОшибки = Неопределено) Экспорт
	...
	Исключение
		...
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Шаблон = НСтр("ru='Ошибка получения ресурса %1 с сервера %2:'") + Символы.ПС + "%3";
		КраткоеПредставлениеОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
		ОписаниеОшибки = СтрШаблон(Шаблон,
	...
	Если НЕ Ответ.КодСостояния = 200 Тогда
		...
		Если НЕ ПустаяСтрока(ТекстОтвета) Тогда
			ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + НСтр("ru='Текст ответа:'") + Символы.ПС + ТекстОтвета;
		КонецЕсли;
		ЗаписатьСобытиеВЖурналОнлайнСервисы(УровеньЖурналаРегистрации.Ошибка, ОписаниеОшибки);
		Возврат Неопределено;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("ВключитьМеханизмОнлайнСервисовРО");
	Методы.Вставить("АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРОРегламентноеЗадание", Истина);
	Методы.Вставить("АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО", Истина);
КонецПроцедуры
