...
Процедура УстановитьВалютуПодсказкуСчета(БанковскийСчет, Форма, КодБанка, Цвет, ЯвляетсяБанкомРФ) Экспорт
	...
	Если НомерСчетаКорректен И ЯвляетсяБанкомРФ Тогда
		...
		Иначе
			...
			АлфавитныйСимвол = Сред(БанковскийСчет.НомерСчета, 6, 1);
		...
		Если ЭтоКлиринговаяВалюта Тогда
			ТекстВалютаНезаполнена = НСтр("ru = 'Укажите клиринговую валюту'");
		Иначе
			ТекстВалютаНезаполнена = НСтр("ru = 'Валюта счета неизвестна. Проверьте номер счета'");
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(БанковскийСчет.ВалютаДенежныхСредств) Тогда
			ПодсказкаНомерСчета = Новый ФорматированнаяСтрока(ТекстВалютаНезаполнена,, Цвет);
		КонецЕсли;
