...
Процедура ПроверитьАлгоритмШифрования(Форма, МастерДалее = Истина, ВыводитьСообщения = Истина) Экспорт
	
	Проверить = Форма.СдаватьВФНС И Форма.АлгоритмШифрованияОбязателен И Форма.АлгоритмШифрованияУстарел;
	
	Если Проверить Тогда
		
		МастерДалее = Ложь;
		ТекстОшибки = "Обновите криптопровайдер";
		ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстОшибки,,,"УказательГОСТ2018");
