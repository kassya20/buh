...
Процедура ПараметрыЭтапаПодготовкиКДокументообороту(ТекущиеПараметры) Экспорт
	...
	ТекущиеПараметры.ВыводитьПодтверждениеПриМассовомФормировании = Ложь;
...
Процедура ПриПроверкеГотовностиКДокументообороту(ОбъектыУчета, ОбработчикПродолжения, СтандартнаяОбработка = Истина) Экспорт
	...
	ОбменСКонтрагентамиБПКлиент.ПриПроверкеГотовностиКДокументообороту(ОбъектыУчета, ОбработчикПродолжения, СтандартнаяОбработка);	
...
Процедура ПриПодбореУчетногоДокумента(Знач Настройки, Знач ОповещениеОВыборе, СтандартнаяОбработка = Истина) Экспорт
	...
	ПараметрыФормы.Вставить("СпособОбработки", Настройки.СпособОбработки);
	ПараметрыФормы.Вставить("ИмяОбъекта"     , Настройки.ИмяОбъектаМетаданных);
	ПараметрыФормы.Вставить("Организация"    , Настройки.Организация);
	ПараметрыФормы.Вставить("Контрагент"     , Настройки.Контрагент);
	
	ОткрытьФорму("ОбщаяФорма.ПодборДокументовДляЭДО", ПараметрыФормы,,,,, ОповещениеОВыборе,
...
Процедура ПослеПодбораУчетногоДокумента(ЭлектронныйДокумент, ДокументУчета) Экспорт
	...
	Если ТипЗнч(ДокументУчета) = Тип("ДокументСсылка.АктСверкиВзаиморасчетов") Тогда
		ОбменСКонтрагентамиБПВызовСервера.ПослеПодбораУчетногоДокументаАктСверкиВзаиморасчетов(ЭлектронныйДокумент, ДокументУчета);
	ИначеЕсли ТипЗнч(ДокументУчета) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		ОбменСКонтрагентамиБПВызовСервера.ПослеПодбораУчетногоДокументаДоговорыКонтрагентов(ЭлектронныйДокумент, ДокументУчета);
	КонецЕсли;
...
Процедура ПриОпределенииВариантовВыбораФайлаДляДоговорногоДокументаПДФ_А3(ВариантыВыбораФайла,
	...
	ВариантыВыбораФайла.СДиска             = Истина;
	ВариантыВыбораФайла.ПрисоединенныйФайл = Истина;
...
Процедура ПриФормированииНовогоФайлаОбменаДоговорногоДокументаXML(ОбъектУчета, ИдентификаторОсновногоФайла,
	...
	ДвоичныеДанныеФайла = ПечатьДоговоровВызовСервера.ЭлектронныйФорматДоговорногоДокумента(ОбъектУчета, ИдентификаторОсновногоФайла);   
	
	Если ТипЗнч(ДвоичныеДанныеФайла) = Тип("Структура") Тогда
		
		//Если возвращена структура данных, значит возникли ошибки при выгрузке
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ДвоичныеДанныеФайла.СодержаниеОшибок);
...
Процедура ПриОпределенииИнтерфейсаПомощникаПодключения(ИспользоватьПомощникЛегкогоИнтерфейса) Экспорт
	...
	Если ОбщегоНазначенияБПВызовСервера.ЭтоПростойИнтерфейс() Тогда
		ИспользоватьПомощникЛегкогоИнтерфейса = Истина;
	КонецЕсли;
