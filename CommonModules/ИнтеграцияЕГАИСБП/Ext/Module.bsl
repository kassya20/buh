...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьНовыеРеквизитыТТНИсходящаяЕГАИС");
	Методы.Вставить("ПеренестиДанныеОрганизацияСкладЕГАИС");
	Методы.Вставить("УстановитьКонстантыЕГАИС");
	Методы.Вставить("ЗаполнитьНоменклатуруЕГАИС");
...
Процедура ЗаполнениеДокументовЕГАИС(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.АктСписанияЕГАИС") Тогда
		...
		ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
			...
			РеквизитыОснования          = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, "Ссылка, Организация, Склад");
			Источник.ДокументОснование  = РеквизитыОснования.Ссылка;
	...
	ИначеЕсли ТипЗнч(Источник) =  Тип("ДокументОбъект.ОтчетОПроизводствеЕГАИС") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетПроизводстваЗаСмену") Тогда
			Источник.ДокументОснование = ДанныеЗаполнения;
			Источник.Идентификатор     = Строка(ДанныеЗаполнения.УникальныйИдентификатор());
			Источник.ДатаПроизводства  = ДанныеЗаполнения.Дата;
			Если НЕ ЗначениеЗаполнено(Источник.ОрганизацияЕГАИС) Тогда
			...
			ЗаполнитьПродукциюЕГАИС(Источник, ДанныеЗаполнения);
			ЗаполнитьСырьеЕГАИС(Источник, ДанныеЗаполнения);
		КонецЕсли;
...
Процедура ЗаполнитьТоварыЕГАИС(Источник, ДанныеЗаполнения, ИсключитьМаркируемые = Ложь)
	...
	|		ПО ТоварыКОформлению.Номенклатура = СведенияОбАлкогольнойПродукции.Номенклатура
	|ГДЕ
	|	НЕ(СведенияОбАлкогольнойПродукции.Номенклатура ЕСТЬ NULL
	|				И ВидыАлкогольнойПродукции.Ссылка ЕСТЬ NULL)
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ЗаполнитьПродукциюЕГАИС(Источник, ДанныеЗаполнения, ИсключитьМаркируемые = Ложь)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыЕГАИС КАК СоответствиеНоменклатурыЕГАИС
	|		ПО ОтчетПроизводстваЗаСменуПродукция.Номенклатура = СоответствиеНоменклатурыЕГАИС.Номенклатура
	|ГДЕ
	...
	|	Документ.ОтчетПроизводстваЗаСмену.Продукция КАК ОтчетПроизводстваЗаСменуПродукция
	|		ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеТоваров КАК СоответствиеТоваров
	|		ПО ОтчетПроизводстваЗаСменуПродукция.Номенклатура = СоответствиеТоваров.Номенклатура
	|ГДЕ
	|	ОтчетПроизводстваЗаСменуПродукция.Ссылка = &ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	ИмяДокумента         = МетаданныеДокумента.Имя;
	ЕстьСуммаВключаетНДС = ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаВключаетНДС", МетаданныеДокумента);
	ЕстьСумма            = (МетаданныеДокумента.ТабличныеЧасти["Продукция"].Реквизиты.Найти("Сумма") <> Неопределено);
...
Процедура ЗаполнитьСырьеЕГАИС(Источник,ДанныеЗаполнения, ИсключитьМаркируемые = Ложь)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеНоменклатурыЕГАИС КАК СоответствиеНоменклатурыЕГАИС
	|		ПО ОтчетПроизводстваЗаСменуМатериалы.Номенклатура = СоответствиеНоменклатурыЕГАИС.Номенклатура
	|ГДЕ
	|	ОтчетПроизводстваЗаСменуМатериалы.Ссылка = &ДокументОснование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Документ.ОтчетПроизводстваЗаСмену.Материалы КАК ОтчетПроизводстваЗаСменуМатериалы
	|		ЛЕВОЕ СОЕДИНЕНИЕ СоответствиеТоваров КАК СоответствиеТоваров
	|		ПО ОтчетПроизводстваЗаСменуМатериалы.Номенклатура = СоответствиеТоваров.Номенклатура
	|ГДЕ
	|	ОтчетПроизводстваЗаСменуМатериалы.Ссылка = &ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	ИмяДокумента         = МетаданныеДокумента.Имя;
	ЕстьСуммаВключаетНДС = ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаВключаетНДС", МетаданныеДокумента);
	ЕстьСумма            = (МетаданныеДокумента.ТабличныеЧасти["Материалы"].Реквизиты.Найти("Сумма") <> Неопределено);
	
	ЕстьКоличествоУпаковок = (Источник.Метаданные().ТабличныеЧасти["Сырье"].Реквизиты.Найти("КоличествоУпаковок") <> Неопределено);
	ЕстьЦена               = (Источник.Метаданные().ТабличныеЧасти["Сырье"].Реквизиты.Найти("Цена") <> Неопределено);
...
Процедура ОбработатьДокументЕГАИС(МетаданныеОбъекта, ПоследнееОбработанноеЗначение)
	...
	Запрос.УстановитьПараметр("ПустаяНоменклатура", ОбщегоНазначенияИС.ПустоеЗначениеОпределяемогоТипа(Метаданные.ОпределяемыеТипы.Номенклатура.Имя));
