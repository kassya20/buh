	...
	Для Каждого ВариантУчета Из Классификаторы.ДебиторскаяЗадолженностьНаличиеДвижение.ВариантУчета Цикл
		...
		Для Каждого СтрокаГруппы Из ГруппыРаздела Цикл
			...
			Если СчетаГруппыРаздела.МеткаШаблонПорядокРасчета = "Аренда" Тогда
				ДобавитьСлагаемыеРасчетыАрендыДебиторская(НаличиеДвижение, ТаблицыРасчета.РасчетыПоАренде, КонтекстРаздела, ПараметрыТекстовРасшифровки);
...
Процедура ДобавитьСлагаемыеРасчетыАрендыДебиторская(ВыводТаблицы, РасчетыПоАренде, КонтекстРаздела, ПараметрыТекстов)
	
	ИмяОтчета = "Отчет.СправкаРасчетКлассификацияПоСрокамАрендныхОбязательств.ФормаОбъекта";
	Для Каждого СтрокаТаблицы Из РасчетыПоАренде Цикл
		Если СтрокаТаблицы.ТипСлагаемого = "Авансы" Тогда
			ШаблонОписания = НСтр("ru = 'Авансы по аренде'");
			ИмяКолонки = "НачалоНоминал";
			Значение = СтрокаТаблицы.Начало;
			Если Значение <> 0 Тогда
				НастройкиОтчета = ЗначенияНастроекРасшифровкиПоказателейБалансаПоАренде(ВыводТаблицы.КонтекстЗаполненияОтчета, КонтекстРаздела.КонецПредыдущегоПериода);
			...
			ИмяКолонки = "ПоступилоНоминал";
			Значение = СтрокаТаблицы.Поступило;
			Если Значение <> 0 Тогда
				Описание = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонОписания, ПараметрыТекстов);
			...
			ИмяКолонки = "СписаноНоминал";
			Значение = СтрокаТаблицы.Списано;
			Если Значение <> 0 Тогда
				Описание = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонОписания, ПараметрыТекстов);
				ЗаполнениеБухгалтерскойОтчетностиТабличное.ДобавитьСлагаемое(ВыводТаблицы, ИмяКолонки, Описание, Значение);
			КонецЕсли;
			...
			ИмяКолонки = "КонецНоминал";
			Значение = СтрокаТаблицы.Конец;
			Если Значение <> 0 Тогда
				НастройкиОтчета = ЗначенияНастроекРасшифровкиПоказателейБалансаПоАренде(ВыводТаблицы.КонтекстЗаполненияОтчета, ВыводТаблицы.КонтекстЗаполненияОтчета.Период.Конец);
				Отчет = ЗаполнениеБухгалтерскойОтчетности.НастройкиРасшифровкиСправкиРасчетаКлассификацииЗадолженности(НастройкиОтчета, ИмяОтчета);
				Описание = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонОписания, ПараметрыТекстов);
				ЗаполнениеБухгалтерскойОтчетностиТабличное.ДобавитьСлагаемое(ВыводТаблицы, ИмяКолонки, Описание, Значение, , Отчет);
			КонецЕсли;
		ИначеЕсли СтрокаТаблицы.ТипСлагаемого = "КорректировкаНДС" Тогда
			ШаблонОписания = НСтр("ru = 'Корректировка НДС'");
			ИмяКолонки = "НачалоНоминал";
			Значение = СтрокаТаблицы.Начало;
			Если Значение <> 0 Тогда
				Описание = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонОписания, ПараметрыТекстов);
...
Функция ТаблицыРасчетаДебиторская(КонтекстЗаполненияОтчета, КонтекстРаздела)
	...
	// Расчеты по аренде
	ТаблицыРасчета.Вставить("РасчетыПоАренде", РасчетыПоАренде(КонтекстЗаполненияОтчета, КонтекстРаздела, НДСАвансов));
...
Функция КонтекстРазделаДебиторская(КонтекстЗаполненияОтчета)
	...
	КонтекстРаздела.Вставить("СчетаАвансовПоАренде", ПланыСчетов.Хозрасчетный.СчетаАрендныхПлатежей());
	...
	РазделятьАвансыПоВидамАктивов = РегламентированнаяОтчетность.АвансыРазделеныПоВидамАктивов(Организация, КонтекстЗаполненияОтчета.Период.КонецДата);
...
Процедура ЗаполнитьПояснениеКредиторскаяЗадолженностьНаличиеДвижение(ЗаполняемыеДанные, Классификаторы, КонтекстЗаполненияОтчета)
	...
	Для Каждого ВариантУчета Из Классификаторы.КредиторскаяЗадолженностьНаличиеДвижение.ВариантУчета Цикл
		...
		Для Каждого СтрокаГруппы Из ГруппыРаздела Цикл
			...
			// Показатели по аренде (лизингу) рассчитываются в разрезе договоров без разбивки на организации
			Если СчетаГруппыРаздела.МеткаШаблонПорядокРасчета = "Аренда" Тогда
				ОтборАренда = Новый Структура("ДолгосрочнаяЗадолженность", ДолгосрочнаяЗадолженность);
				МассивРасчетыПоАренде = ТаблицыРасчета.РасчетыПоАренде.НайтиСтроки(ОтборАренда);
				ДобавитьСлагаемыеРасчетыАрендыКредиторская(НаличиеДвижение, МассивРасчетыПоАренде, КонтекстРаздела, ПараметрыТекстовРасшифровки, ДолгосрочнаяЗадолженность);
				ЗаполнениеБухгалтерскойОтчетностиТабличное.ЗакончитьДобавлениеСтрокиТаблицыОтчета(НаличиеДвижение, ШаблоныЗаголовков);
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьСлагаемыеРасчетыАрендыКредиторская(ВыводТаблицы, РасчетыПоАренде, КонтекстРаздела, ПараметрыТекстов, ДолгосрочнаяЗадолженность)
	
	ИмяОтчета = "Отчет.СправкаРасчетКлассификацияПоСрокамАрендныхОбязательств.ФормаОбъекта";

	ШаблонОписания = НСтр("ru = 'Краткосрочное обязательство по аренде'");
	Если ДолгосрочнаяЗадолженность Тогда
		ШаблонОписания = НСтр("ru = 'Долгосрочное обязательство по аренде'");
	КонецЕсли;
...
Функция ТаблицыРасчетаКредиторская(КонтекстЗаполненияОтчета, КонтекстРаздела)
	...
	// Расчеты по аренде
	ТаблицыРасчета.Вставить("РасчетыПоАренде", РасчетыПоАренде(КонтекстЗаполненияОтчета, КонтекстРаздела));
...
Функция КонтекстРазделаКредиторская(КонтекстЗаполненияОтчета)
	...
	КонтекстРаздела.Вставить("СчетаПроцентовПоРассрочке", ПланыСчетов.Хозрасчетный.СчетаПроцентовПоРассрочке());
	КонтекстРаздела.Вставить("ВидПДРНачисление", Перечисления.ВидыПрочихДоходовИРасходов.ШтрафыПениНеустойкиКПолучениюУплате);
	КонтекстРаздела.Вставить("ВидПДРСписание", Перечисления.ВидыПрочихДоходовИРасходов.СписаниеДебиторскойКредиторскойЗадолженности);
...
#Область НачалоНоминал
	...
	Если КонтекстРаздела.ЕстьДисконтирование Тогда
		РасчетПоказателейБалансаПоОбязательствамГрафики.ЗаполнитьШаблонРасшифровкиПоАрендеПояснениеБалансаОбязательстваНачалоПериода(ШаблонАренда);
	КонецЕсли;
...
#Область КонецНоминал
	...
	ШаблонАренда = НСтр("ru = 'Отрицательное сальдо по [СчетуАренды] за вычетом относящегося к нему НДС (сальдо по кредиту [СчетаНДСАвансовВыданных]). Включаются суммы со сроком действия договора не ранее [СрокПогашенияКонец]
			|Расчет выполняется отдельно по каждому договору аренды (лизинга)[СуммарноПоФилиалам].'");
	Если КонтекстРаздела.ЕстьДисконтирование Тогда
		РасчетПоказателейБалансаПоОбязательствамГрафики.ЗаполнитьШаблонРасшифровкиПоАрендеПояснениеБалансаОбязательстваКонецПериода(ШаблонАренда);
	КонецЕсли;
...
Функция ТекстЗапросаСчетаРасчетовДебиторская(НаборыСчетов, КонтекстРаздела)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РасходыБудущихПериодов КАК СправочникРБП
...
Функция ТекстЗапросаРезервыПоСомнительнымДолгам()
	...
	|	ВЫБОР
	|		КОГДА УНИКАЛЬНЫЙИДЕНТИФИКАТОР(ХозрасчетныйОстаткиИОбороты.Субконто3) = &ПустойУникальныйИдентификатор
	|			ТОГДА НЕОПРЕДЕЛЕНО
	...
	|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(ХозрасчетныйОстаткиИОбороты.Субконто3) = &ПустойУникальныйИдентификатор КАК Субконто3ПустаяСсылка
	|ПОМЕСТИТЬ ВсеРезервы
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, Период, , Счет В (&СчетаРезервов), &СубконтоКонтрагентыДоговорыДокументы, Организация В (&Организации)) КАК ХозрасчетныйОстаткиИОбороты
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Субконто1,
...
Функция ТекстЗапросаСписаниеНаРасходы(НаборыСчетов, КонтекстРаздела)
	...
	|		ПО РасчетыПоСтрахованию.Субконто2 = СправочникРасходыБудущихПериодов.Ссылка
	|ГДЕ
	|	ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов = &ВидПДРСписание
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РасходыБудущихПериодов КАК СправочникРасходыБудущихПериодов
	|		ПО РасходыБудущихПериодов.Субконто1 = СправочникРасходыБудущихПериодов.Ссылка
	|ГДЕ
	|	ПрочиеДоходыИРасходы.ВидПрочихДоходовИРасходов = &ВидПДРСписание
	|	И СправочникРасходыБудущихПериодов.ВидАктива = &ВидАктиваДебиторскаяЗадолженность
	|
...
Функция ТекстЗапросаСрокиПогашения()
	...
	|		КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.ВидДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ПустаяСсылка)) В (&ВидыДоговоровПоставщиков)
	|				И &ИспользуютсяСрокиОплатыПоставщики
	|			ТОГДА &СрокОплатыПоставщикам
	|		КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.ВидДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровКонтрагентов.ПустаяСсылка)) В (&ВидыДоговоровПокупатели)
	|				И &ИспользуютсяСрокиОплатыПокупатели
	|			ТОГДА &СрокОплатыПокупателей
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СрокОплаты,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СрокиОплатыДокументов.СрокОплаты, ДАТАВРЕМЯ(1, 1, 1)) <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЕСТЬNULL(СрокиОплатыДокументов.СрокОплаты, ДАТАВРЕМЯ(1, 1, 1))
	|		КОГДА ЕСТЬNULL(ДоговорыКонтрагентов.УстановленСрокОплаты, ЛОЖЬ)
	|				И ЕСТЬNULL(ДоговорыКонтрагентов.ДатаОплаты, ДАТАВРЕМЯ(1, 1, 1)) <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЕСТЬNULL(ДоговорыКонтрагентов.ДатаОплаты, ДАТАВРЕМЯ(1, 1, 1))
	|		КОГДА ДоговорыКонтрагентов.УстановленСрокОплаты <> ИСТИНА
	|				И (ДоговорыКонтрагентов.ВидДоговора В (&ВидыДоговоровПоставщиков)
	|						И НЕ &ИспользуютсяСрокиОплатыПоставщики
	|					ИЛИ ДоговорыКонтрагентов.ВидДоговора В (&ВидыДоговоровПокупатели)
	...
	|		ПО Аналитика.Организация = СрокиОплатыДокументов.Организация
	|			И Аналитика.Субконто3 = СрокиОплатыДокументов.Документ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	|	Субконто1,
...
Процедура ЗаполнитьГруппыДебиторскойЗадолженности(Группы, ТаблицаСчетов, КонтекстРаздела)
	...
	СчетаГруппы.РазделятьАвансыПоВидамАктивов = Истина;
	...
	СчетаГруппы.СчетаИсключения = "76.01.2, 76.01.9, 76.05, 76.25, 76.35, 76.06, 76.26, 76.36, 76.07, 76.27, 76.37, 76.13, 76.08, 76.28, 76.38, 76.АВ, 76.НА, 76.ОТ";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);

	Группа = Группы.Добавить();
	Группа.НаименованиеГруппы = НСтр("ru = 'Авансы по аренде (лизингу)'");
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаРасчетов = "76.07, 76.27, 76.37";
	СчетаГруппы.СПодчиненными = Ложь;
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
	Группа.МеткаШаблонПорядокРасчета = "Аренда";
	...
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаПоСрочности = "76.13";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
...
Процедура ЗаполнитьГруппыКредиторскойЗадолженности(Группы, ТаблицаСчетов, КонтекстРаздела)
	...
	СчетаГруппы.СчетаИсключения = "76.05, 76.25, 76.35, 76.06, 76.26, 76.36, 76.07, 76.27, 76.37, 76.ВА, 76.НА";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);

	Группа = Группы.Добавить();
	Группа.НаименованиеГруппы = НСтр("ru = 'Расчеты по аренде'");
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаРасчетов = "76.07, 76.27, 76.37";
	СчетаГруппы.СПодчиненными = Ложь;
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	...
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаРасчетов = "66.03, 66.05, 66.23, 66.25";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
	...
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаРасчетов = "66.02, 66.04, 66.06, 66.22, 66.24, 66.26, 67.02, 67.04, 67.06, 67.22, 67.24, 67.26";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
	...
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаДолгосрочные = "67.03, 67.05, 67.23, 67.25";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
	...
	Группа.НаименованиеГруппы = НСтр("ru = 'Расчеты по НДС, не отраженные на ЕНС'");
	СчетаГруппы = НовыйСчетаГруппы();
	СчетаГруппы.СчетаРасчетов = "68.02, 76.НА";
	СтрокиСчетов = ДобавитьДанныеСчетов(ТаблицаСчетов, Группа, СчетаГруппы, КонтекстРаздела);
	ДополнитьГруппуДаннымиСчетов(Группа, СтрокиСчетов);
...
Процедура ДополнитьКонтекстРаздела(КонтекстРаздела, КонтекстЗаполненияОтчета)
	...
	КонтекстРаздела.Вставить("ИспользуютсяСрокиОплатыПоставщики", ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПлатежейПоставщикам"));
	КонтекстРаздела.Вставить("ИспользуютсяСрокиОплатыПокупатели", ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПлатежейОтПокупателей"));
	КонтекстРаздела.Вставить("ВидыДоговоровПоставщиков", БухгалтерскиеОтчеты.ВидыДоговоровПоставщиков());
	КонтекстРаздела.Вставить("ВидыДоговоровПокупатели",  БухгалтерскиеОтчеты.ВидыДоговоровПокупателей());
	КонтекстРаздела.Вставить("СрокОплатыПоставщикам", Константы.СрокОплатыПоставщикам.Получить());
	КонтекстРаздела.Вставить("СрокОплатыПокупателей", Константы.СрокОплатыПокупателей.Получить());
	...
	КонтекстРаздела.Вставить("СчетаРасчетовПоАренде", ПланыСчетов.Хозрасчетный.СчетаРасчетовПоАренде());
	КонтекстРаздела.Вставить("ЕстьДисконтирование",  УчетнаяПолитика.ПоддерживаетсяДисконтированиеОбязательств());
...
Функция РасчетыПоАренде(КонтекстЗаполненияОтчета, КонтекстРаздела, НДСАвансов = Неопределено)
	
	Контекст = РасчетПоказателейБалансаПоОбязательствам.НовыйКонтекстРасчета();

	Контекст.КонецПериода = КонтекстРаздела.КонецПредыдущегоПериода;
	Контекст.КонецПериодаДата = КонтекстРаздела.КонецПредыдущегоПериодаДата;
	Контекст.Организации = КонтекстЗаполненияОтчета.Организации;
	Контекст.ИспользуетсяВалютныйУчет = КонтекстРаздела.ИспользуетсяВалютныйУчет;
	Контекст.ЕстьДисконтирование = КонтекстРаздела.ЕстьДисконтирование;
	
	НачалоРасчетыПоАренде = РасчетПоказателейБалансаПоОбязательствам.ИсходныеДанныеПоАренднымОбязательствам(Контекст).АрендныеОбязательства;
	
	Контекст.КонецПериода = КонтекстЗаполненияОтчета.Период.Конец;
	Контекст.КонецПериодаДата = КонтекстЗаполненияОтчета.Период.КонецДата;
	
	КонецРасчетыПоАренде = РасчетПоказателейБалансаПоОбязательствам.ИсходныеДанныеПоАренднымОбязательствам(Контекст).АрендныеОбязательства;

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("НачалоРасчетыПоАренде", НачалоРасчетыПоАренде);
	Запрос.УстановитьПараметр("КонецРасчетыПоАренде", КонецРасчетыПоАренде);
	Запрос.УстановитьПараметр("СчетаРасчетовПоАренде", КонтекстРаздела.СчетаРасчетовПоАренде);
	...
	|	РасчетыПоАренде.АвансыПоАренде КАК НачалоАвансы,
	|	РасчетыПоАренде.НДСсПлатежей КАК НачалоНДС,
	|	РасчетыПоАренде.КраткосрочноеОбязательство КАК НачалоКраткосрочное,
	|	РасчетыПоАренде.ДолгосрочноеОбязательство КАК НачалоДолгосрочное
	|ПОМЕСТИТЬ НачалоРасчетыПоАренде
	|ИЗ
	|	&НачалоРасчетыПоАренде КАК РасчетыПоАренде
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасчетыПоАренде.Контрагент КАК Контрагент,
	|	РасчетыПоАренде.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	РасчетыПоАренде.АвансыПоАренде КАК КонецАвансы,
	|	РасчетыПоАренде.НДСсПлатежей КАК КонецНДС,
	|	РасчетыПоАренде.КраткосрочноеОбязательство КАК КонецКраткосрочное,
	|	РасчетыПоАренде.ДолгосрочноеОбязательство КАК КонецДолгосрочное
	|ПОМЕСТИТЬ КонецРасчетыПоАренде
	|ИЗ
	|	&КонецРасчетыПоАренде КАК РасчетыПоАренде
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасчетыПоАрендеОбъединение.Контрагент КАК Контрагент,
	|	РасчетыПоАрендеОбъединение.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	СУММА(РасчетыПоАрендеОбъединение.НачалоАвансы) КАК НачалоАвансы,
	|	СУММА(РасчетыПоАрендеОбъединение.НачалоНДС) КАК НачалоНДС,
	|	СУММА(РасчетыПоАрендеОбъединение.НачалоКраткосрочное) КАК НачалоКраткосрочное,
	|	СУММА(РасчетыПоАрендеОбъединение.НачалоДолгосрочное) КАК НачалоДолгосрочное,
	|	СУММА(РасчетыПоАрендеОбъединение.КонецАвансы) КАК КонецАвансы,
	|	СУММА(РасчетыПоАрендеОбъединение.КонецНДС) КАК КонецНДС,
	|	СУММА(РасчетыПоАрендеОбъединение.КонецКраткосрочное) КАК КонецКраткосрочное,
	|	СУММА(РасчетыПоАрендеОбъединение.КонецДолгосрочное) КАК КонецДолгосрочное
	|ПОМЕСТИТЬ РасчетыПоАрендеВсе
	|ИЗ
	|	(ВЫБРАТЬ
	|		НачалоРасчетыПоАренде.Контрагент КАК Контрагент,
	|		НачалоРасчетыПоАренде.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|		НачалоРасчетыПоАренде.НачалоАвансы КАК НачалоАвансы,
	|		НачалоРасчетыПоАренде.НачалоНДС КАК НачалоНДС,
	|		НачалоРасчетыПоАренде.НачалоКраткосрочное КАК НачалоКраткосрочное,
	|		НачалоРасчетыПоАренде.НачалоДолгосрочное КАК НачалоДолгосрочное,
	|		0 КАК КонецАвансы,
	|		0 КАК КонецНДС,
	|		0 КАК КонецКраткосрочное,
	|		0 КАК КонецДолгосрочное
	|	ИЗ
	|		НачалоРасчетыПоАренде КАК НачалоРасчетыПоАренде
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		КонецРасчетыПоАренде.Контрагент,
	|		КонецРасчетыПоАренде.ДоговорКонтрагента,
	|		0,
	|		0,
	|		0,
	|		0,
	|		КонецРасчетыПоАренде.КонецАвансы,
	|		КонецРасчетыПоАренде.КонецНДС,
	|		КонецРасчетыПоАренде.КонецКраткосрочное,
	|		КонецРасчетыПоАренде.КонецДолгосрочное
	|	ИЗ
	|		КонецРасчетыПоАренде КАК КонецРасчетыПоАренде) КАК РасчетыПоАрендеОбъединение
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыПоАрендеОбъединение.Контрагент,
	|	РасчетыПоАрендеОбъединение.ДоговорКонтрагента");
	Если КонтекстРаздела.ВидЗадолженности = "ДебиторскаяЗадолженность" Тогда
		ТекстыЗапросов.Добавить(
		"ВЫБРАТЬ
		|	НДСАвансов.СНК КАК СНК,
		|	НДСАвансов.КО КАК КО,
		|	НДСАвансов.ДО КАК ДО,
		|	НДСАвансов.СКК КАК СКК
		|ПОМЕСТИТЬ НДС
		|ИЗ
		|	&НДСАвансов КАК НДСАвансов
		|ГДЕ
		|	НДСАвансов.СчетРасчетов В(&СчетаРасчетовПоАренде)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(РасчетыПоАрендеВсе.НачалоНДС) КАК НачалоНДС,
		|	СУММА(ВЫБОР
		|			КОГДА РасчетыПоАрендеВсе.КонецНДС - РасчетыПоАрендеВсе.НачалоНДС > 0
		|				ТОГДА РасчетыПоАрендеВсе.КонецНДС - РасчетыПоАрендеВсе.НачалоНДС
		|			ИНАЧЕ 0
		...
		|				ТОГДА РасчетыПоАрендеВсе.КонецНДС - РасчетыПоАрендеВсе.НачалоНДС
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК СписаноНДС,
		|	СУММА(РасчетыПоАрендеВсе.КонецНДС) КАК КонецНДС
		|ПОМЕСТИТЬ КорректировкаНДС
		|ИЗ
		|	РасчетыПоАрендеВсе КАК РасчетыПоАрендеВсе
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	""Авансы"" КАК ТипСлагаемого,
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.НачалоАвансы), 0) КАК Начало,
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|				КОГДА РасчетыПоАрендеВсе.КонецАвансы - РасчетыПоАрендеВсе.НачалоАвансы > 0
		|					ТОГДА РасчетыПоАрендеВсе.КонецАвансы - РасчетыПоАрендеВсе.НачалоАвансы
		|				ИНАЧЕ 0
		...
		|				ИНАЧЕ 0
		|			КОНЕЦ), 0) КАК Списано,
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.КонецАвансы), 0) КАК Конец
		|ИЗ
		|	РасчетыПоАрендеВсе КАК РасчетыПоАрендеВсе
		|
		|ОБЪЕДИНИТЬ ВСЕ
		...
		|ВЫБРАТЬ
		|	""КорректировкаНДС"",
		|	ЕСТЬNULL(СУММА(КорректировкаНДС.НачалоНДС), 0),
		|	ЕСТЬNULL(СУММА(КорректировкаНДС.ПоступилоНДС), 0),
		|	ЕСТЬNULL(СУММА(КорректировкаНДС.СписаноНДС), 0),
		|	ЕСТЬNULL(СУММА(КорректировкаНДС.КонецНДС), 0)
		|ИЗ
		|	КорректировкаНДС КАК КорректировкаНДС");
	Запрос.УстановитьПараметр("НДСАвансов", НДСАвансов);
	Иначе
		ТекстыЗапросов.Добавить(
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.НачалоКраткосрочное), 0) КАК Начало,
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|			КОГДА РасчетыПоАрендеВсе.КонецКраткосрочное - РасчетыПоАрендеВсе.НачалоКраткосрочное > 0
		|				ТОГДА РасчетыПоАрендеВсе.КонецКраткосрочное - РасчетыПоАрендеВсе.НачалоКраткосрочное
		|			ИНАЧЕ 0
		|		КОНЕЦ), 0) КАК Поступило,
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|			КОГДА РасчетыПоАрендеВсе.КонецКраткосрочное - РасчетыПоАрендеВсе.НачалоКраткосрочное < 0
		...
		|			ИНАЧЕ 0
		|		КОНЕЦ), 0) КАК Списано,
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.КонецКраткосрочное), 0) КАК Конец,
		|	ЛОЖЬ КАК ДолгосрочнаяЗадолженность
		|ИЗ
		|	РасчетыПоАрендеВсе КАК РасчетыПоАрендеВсе
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.НачалоДолгосрочное), 0),
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|			КОГДА РасчетыПоАрендеВсе.КонецДолгосрочное - РасчетыПоАрендеВсе.НачалоДолгосрочное > 0
		|				ТОГДА РасчетыПоАрендеВсе.КонецДолгосрочное - РасчетыПоАрендеВсе.НачалоДолгосрочное
		|			ИНАЧЕ 0
		|		КОНЕЦ), 0),
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|			КОГДА РасчетыПоАрендеВсе.КонецДолгосрочное - РасчетыПоАрендеВсе.НачалоДолгосрочное < 0
		...
		|			ИНАЧЕ 0
		|		КОНЕЦ), 0),
		|	ЕСТЬNULL(СУММА(РасчетыПоАрендеВсе.КонецДолгосрочное), 0),
		|	ИСТИНА
		|ИЗ
		|	РасчетыПоАрендеВсе КАК РасчетыПоАрендеВсе");
	КонецЕсли;
...
Функция ЗначенияНастроекРасшифровкиПоказателейБалансаПоАренде(КонтекстЗаполненияОтчета, КонецПериода)
	
	ЗначенияНастроек = Новый Структура;
	ЗначенияНастроек.Вставить("Организация",   КонтекстЗаполненияОтчета.Организации[0].ГоловнаяОрганизация);
...
Функция СчетаОбъектовРезервов(ВидЗадолженности)
	...
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочке);
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочкеВал);
	СчетаИсключения.Добавить(ПланыСчетов.Хозрасчетный.ПроцентыПоРассрочкеУЕ);
	...
	СчетаРасчетовПоАренде.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАрендеВал);
	СчетаРасчетовПоАренде.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАрендеУЕ);
