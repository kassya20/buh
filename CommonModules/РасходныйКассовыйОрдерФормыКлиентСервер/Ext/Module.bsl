...
Процедура УправлениеЭлементамиОтраженияВУСН(Форма) Экспорт
	...
	Если БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма)
		Или РасходныйКассовыйОрдерФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		Возврат;
	КонецЕсли;
	...
	ОтражениеПринимаютсяВУСН = ПредопределенноеЗначение("Перечисление.ОтражениеВУСН.Принимаются");
	
	Элементы.РасходыУСН.Видимость = (Объект.ОтражениеВУСН = ОтражениеПринимаютсяВУСН);
	Элементы.НДСУСН.Видимость = (Объект.ОтражениеВУСН = ОтражениеПринимаютсяВУСН);
	
	Элементы.НДСУСН.Доступность = (Объект.Графа7_УСН <> 0);
	
	Если ФормаДокументаОднострочная(Форма) И ЕстьРасшифровкаПлатежа(Объект.ВидОперации) Тогда
		Элементы.РасшифровкаПлатежаРасходыУСН.Видимость = (Форма.РасшифровкаПлатежаОтражениеВУСН = ОтражениеПринимаютсяВУСН);
	КонецЕсли;
...
Функция ИсточникДанныхКонтекстаПлатежногоДокумента(Форма) Экспорт
	...
	ИсточникДанныхКонтекста.Вставить("ПеречислениеВБюджет",                 Истина);
	ИсточникДанныхКонтекста.Вставить("ПеречислениеПоИсполнительномуЛисту",  Ложь);
...
Функция ЕстьРасшифровкаПлатежа(ВидОперации) Экспорт
	...
	ЕстьРасшифровка = (ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ОплатаПоставщику")
		...
		ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРКО.ВозвратРозничномуПокупателю")
...
Функция ТекущиеПараметрыОбъекта(Форма) Экспорт
	...
	ПараметрыОбъекта = Новый Структура("Дата, ВидОперации, Организация, Налог, Контрагент, СчетКонтрагента, 
		|ВалютаДокумента, СчетКасса, БезЗакрывающихДокументов, Ссылка");
	...
	ПараметрыОбъекта.Вставить("ПрименяетсяОсобыйНалоговыйРежим", Форма.ПрименяетсяОсобыйНалоговыйРежим);
...
Процедура ВидимостьДоступностьПолейПриИзмененииСпособаПогашения(Форма) Экспорт
	...
	Элементы.РасшифровкаПлатежаСделка.Видимость =
