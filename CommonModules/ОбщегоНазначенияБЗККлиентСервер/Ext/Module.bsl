...
//
Функция ИдентификаторСтрокиДереваПоЗначениюПоля(Дерево, ИмяПоля, СтрокаДляПоиска, ИскатьНачало = Ложь) Экспорт
...
Функция НаименованиеПериода(НачалоПериода, ОкончаниеПериода, ВключатьГод = Неопределено) Экспорт
	...
	Если НачалоПериодаГод = ОкончаниеПериодаГод Тогда
		...
		Если ВключатьГод = Неопределено Тогда
			ВключатьГод = (НачалоПериодаГод <> Год(ТекущаяДата()));
		КонецЕсли;
		
		Если Не ВключатьГод Тогда // АПК:143 Для вычисления года не важно какую дату взять.
...
Функция СледующийИдентификаторСтрокиКоллекцииЭлементовДереваПоЗначениюПоля(ИдентификаторСтроки, ЭлементыРодителя, ИмяПоля, 
	СтрокаДляПоиска, СтартовыйИндекс = 0, ИскатьНачало = Ложь)
	
	Для Индекс = СтартовыйИндекс По ЭлементыРодителя.Количество() - 1 Цикл
		
		СтрокаДерева = ЭлементыРодителя.Получить(Индекс);
		
		Если ИскатьНачало Тогда
			// Ищем по совпадению символов с начала строки
			Если СтрНачинаетсяС(НРег(СтрокаДерева[ИмяПоля]), НРег(СтрокаДляПоиска)) Тогда
				ИдентификаторСтроки = СтрокаДерева.ПолучитьИдентификатор();
				Возврат Истина;
			КонецЕсли;
		Иначе
			// Ищем по совпадении символов в теле строки
	...
	Если ИскатьНачало Тогда
		// Продолжим поиск по совпадению символов в теле строки
		Если СледующийИдентификаторСтрокиКоллекцииЭлементовДереваПоЗначениюПоля(
				ИдентификаторСтроки,
				ЭлементыРодителя, 
				ИмяПоля, 
				СтрокаДляПоиска,
				СтартовыйИндекс) Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
