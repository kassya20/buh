...
Процедура ПерезаполнитьРасшифровкуПлатежа(Форма, ИзменениеОрганизации = Ложь) 
	...
	Если ПриходныйКассовыйОрдерФормыКлиентСервер.ЕстьРасшифровкаПлатежа(Объект.ВидОперации) Тогда
		...
		Для Каждого СтрокаШаблон Из ШаблонЗаполнения Цикл
			...
			ЗаполнитьЗначенияСвойств(СтрокаПлатеж, Форма.СвойстваПлатежа, , "УслугаНПД, СтавкаНДС");
			Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Или Объект.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка Тогда
				СтрокаПлатеж.СтавкаНДС = ?(ЗначениеЗаполнено(Форма.СвойстваПлатежа.СтавкаНДС), Форма.СвойстваПлатежа.СтавкаНДС, СтрокаПлатеж.СтавкаНДС);
			Иначе
...
Функция СтавкаНДСпоУмолчанию(Форма)
	...
	Иначе
		...
		Возврат УчетНДСКлиентСервер.СтавкаНДСПоУмолчанию(Объект.Дата, Форма.ПлательщикНДС);
...
Процедура ПодготовитьФормуНаСервере(Форма) Экспорт
	...
	Если Форма.ПрименяетсяУСНПатент Тогда
		Форма.ПрименяетсяНесколькоПатентов = УчетПСН.ИспользуетсяНесколькоПатентов(Объект.Организация, Объект.Дата);
	КонецЕсли;
	...
	Если Объект.БезЗакрывающихДокументов Тогда
		...
		Если ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
	...
	ПриходныйКассовыйОрдерФормыКлиентСервер.УстановитьПараметрыВыбораСтатьяДДС(Форма);
...
Функция СтавкаНДСРозничнойВыручки(Организация, Склад, Период, ВыручкаСНТТ) Экспорт
	
	ПараметрыОбъекта = Новый Структура;
	ПараметрыОбъекта.Вставить("Организация", Организация);
	ПараметрыОбъекта.Вставить("Контрагент", Склад);
	ПараметрыОбъекта.Вставить("Дата", Период);
	ПараметрыОбъекта.Вставить("ПлательщикЕНВД", УчетнаяПолитика.ПлательщикЕНВД(Организация, Период));
	ПараметрыОбъекта.Вставить("РозничнаяТорговляОблагаетсяЕНВД",
		УчетнаяПолитика.РозничнаяТорговляОблагаетсяЕНВД(Организация, Период));
	ПараметрыОбъекта.Вставить("ВыручкаСНТТ", ВыручкаСНТТ);
	
	НТТНаЕНВД = ПараметрыОбъекта.ВыручкаСНТТ И НТТнаЕНВДилиПатенте(ПараметрыОбъекта);
	
	Если УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, Период) Тогда
		СтавкаНДСПриУСН = РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(
			Организация, Период);
		Возврат Перечисления.СтавкиНДС.СтавкаНДС(СтавкаНДСПриУСН, Период);
	Иначе
		ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(Организация, Период);
		Возврат УчетНДСКлиентСервер.СтавкаНДСПользователя(
			Период, ПлательщикНДС И Не НТТНаЕНВД);
	КонецЕсли;
...
Процедура УстановитьВидимостьСтраницНаСервере(Форма)
	...
	Если НЕ ФормаРасшифровкиПлатежа Тогда
		...
		Если Элементы.Контрагент.Видимость Тогда
			Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда
...
Функция ВидимостьАналитики(ВидОперации)
	...
	Возврат (ВидОперации <> Перечисления.ВидыОперацийПКО.ПолучениеНаличныхВБанке
		...
		И ВидОперации <> Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал);
...
Процедура УстановитьПараметрыВыбораСчетовУчетаДенежныхСредств(Форма)
	...
	СчетаБанка = УчетДенежныхСредствПовтИсп.ПолучитьСчетаССубконтоБанковскиеСчета(ПланыСчетов.Хозрасчетный.ЦифровойРубль);
...
Процедура ВидОперацииОбработатьИзменение(Форма, ПричиныИзменения)
	...
	Если ПриходныйКассовыйОрдерФормыКлиентСервер.ЕстьРасшифровкаПлатежа(Объект.ВидОперации)
		...
		ПараметрыОбъекта.ПорядокОтраженияАванса =
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НовыеПараметры, "ПорядокОтраженияАванса");
		...
		Если Не ЗначениеЗаполнено(ПараметрыОбъекта.ПорядокОтраженияАванса)
			И (ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
			Или ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка) Тогда
			УчетПСН.УстановитьПорядокОтраженияАванса(Форма.СвойстваПлатежа.ПорядокОтраженияАванса, ПараметрыОбъекта);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОтражениеВУСН(Форма, ЗаполнитьСуммы = Истина, ЗаполнитьСодержание = Ложь) Экспорт
	...
	Если Не (Форма.ПрименениеУСН Или Форма.ПрименяетсяОсобыйПорядокНалогообложения) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОрганизацияПриИзмененииСервер(Форма) Экспорт
	...
	ОбработатьИзмененияВОрганизацииНаСервере(Форма);
...
Процедура ДатаПриИзмененииСервер(Форма) Экспорт
	...
	Если Не Форма.СвойстваПлатежа = Неопределено Тогда
		Форма.СвойстваПлатежа.ПорядокОтраженияАванса = Неопределено;
	КонецЕсли;
	...
	Если (Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
		...
		Если ПриходныйКассовыйОрдерФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
			...
			ПриходныйКассовыйОрдерФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Ложь, СтрокаПлатеж);
		КонецЕсли;
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя И Форма.СвойстваПлатежа <> Неопределено Тогда
		НовыйПорядок = Форма.СвойстваПлатежа.ПорядокОтраженияАванса;
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	ЭлементКонтрагентЮрФизЛицо = Элементы.Найти("КонтрагентЮрФизЛицо");
	Если ЭлементКонтрагентЮрФизЛицо <> Неопределено Тогда 
		ЭлементКонтрагентЮрФизЛицо.Видимость = БанкИКассаФормыКлиентСервер.ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма);
	КонецЕсли;
...
Функция ПринятоОтПоПлательщику(Плательщик) Экспорт
	
	Если Не ЗначениеЗаполнено(Плательщик) Тогда
		Возврат "";
	КонецЕсли;
	
	СвойстваКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Плательщик,
		"Наименование, НаименованиеПолное");
	Возврат ?(ПустаяСтрока(СвойстваКонтрагента.НаименованиеПолное),
...
Процедура ВидимостьДоступностьЭлементовОднострочнойФормы(Форма)
	...
	Элементы.РасшифровкаПлатежаСчетНаОплатуБезДокументов.Видимость = Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя;
	...
	Элементы.РасшифровкаПлатежаДоговорКонтрагента.Доступность = ЗначениеЗаполнено(Объект.Организация)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЕстьБюджетНаПериод") Тогда
		ОперацияУчитываетсяВРасходах = БюджетДоходовРасходовКлиентСервер.ОперацияУчитываетсяВБюджетеРасходы(Объект.ВидОперации);
	
		ВидимостьНаправленияДеятельности = ОперацияУчитываетсяВРасходах И Форма.ЕстьБюджетНаПериод;
...
Функция НовыеПараметрыПриИзмененииВидаОперации(ПараметрыОбъекта)
	...
	Если ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка Тогда
		...
		НовыеПараметры.Вставить("СтавкаНДС", СтавкаНДСРозничнойВыручки(ПараметрыОбъекта.Организация,
			ПараметрыОбъекта.Контрагент, ПараметрыОбъекта.Дата, ПараметрыОбъекта.ВыручкаСНТТ));
...
Функция НовыеПараметрыПриИзмененииОрганизации(ПараметрыОбъекта, ПричиныИзменения)
	...
	ИначеЕсли ПараметрыОбъекта.ПрименениеУСН ИЛИ ПараметрыОбъекта.ПрименяетсяОсобыйПорядокНалогообложения
	...
	Если ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
		УчетПСН.УстановитьПорядокОтраженияАванса(НовыйПорядок, ПараметрыОбъекта);
	КонецЕсли;
	НовыеПараметры.Вставить("ПорядокОтраженияАвансов", НовыйПорядок);
...
Функция НовыеПараметрыПриИзмененииКонтрагента(ПараметрыОбъекта) 
	...
	ИначеЕсли ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПКО.ВзносВУставныйКапитал Тогда
		...
		Если ТипЗнч(ПараметрыОбъекта.Контрагент) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			ЗаполнитьЗначенияСвойств(НовыеПараметры, НовыеПараметрыПриВыбореПринятоОт(ПараметрыОбъекта));
		Иначе
			НовыеПараметры.ПринятоОт = ПринятоОтПоПлательщику(ПараметрыОбъекта.Контрагент);
		КонецЕсли;
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(Знач ПараметрыПлатежа, Знач ПолучатьДоговор) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыПлатежа.ДоговорКонтрагента) Тогда
		...
		Если ПараметрыПлатежа.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
			...
			Иначе
				СвойстваПлатежа.СтавкаНДС = УчетНДСКлиентСервер.СтавкаНДСПользователя(
	...
	Если Не ЗначениеЗаполнено(СвойстваПлатежа.ПорядокОтраженияАванса)
		И ПараметрыПлатежа.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
...
Процедура ИнициализироватьСвойстваПлатежа(Форма, ТолькоСоздатьКлючи = Ложь, ПолучатьДоговор = Ложь) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.СвойстваПлатежа.ПорядокОтраженияАванса)
		И Объект.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
		УчетПСН.УстановитьПорядокОтраженияАванса(Форма.СвойстваПлатежа.ПорядокОтраженияАванса,
			ПриходныйКассовыйОрдерФормыКлиентСервер.ТекущиеПараметрыОбъекта(Форма));
	КонецЕсли;
...
Функция ГруппаРасшифровкаПлатежа(ВидОперации, БезЗакрывающихДокументов)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПКО.РозничнаяВыручка Тогда
		Если БезЗакрывающихДокументов Тогда
			ГруппаРасшифровкаПлатежаТаблица = "ГруппаРозничнаяВыручкаБезЗакрывающихДокументов";
		Иначе
			ГруппаРасшифровкаПлатежаТаблица = "ГруппаРозничнаяВыручкаАналитика";
		КонецЕсли;
