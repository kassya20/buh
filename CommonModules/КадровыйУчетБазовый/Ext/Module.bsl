...
Процедура СформироватьКадровыеПриказы(ПараметрыЗадания = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если Не Константы.ИспользоватьКадровыйУчет.Получить() Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ДанныеСотрудника Из ДанныеСотрудников Цикл
		...
		Если ЗначениеЗаполнено(ДанныеСотрудника.ДатаПриема)
			И Не ЗначениеЗаполнено(ДанныеСотрудника.ПриказОПриеме) Тогда
			
			ДокументПриемНаРаботу = Документы.ПриемНаРаботу.СоздатьДокумент();
			
			ДокументПриемНаРаботу.Дата = ДанныеСотрудника.ДатаПриема;
			ДокументПриемНаРаботу.ДатаПриема = ДанныеСотрудника.ДатаПриема;
			ДокументПриемНаРаботу.Организация = ДанныеСотрудника.ТекущаяОрганизация;
			ДокументПриемНаРаботу.Сотрудник = ДанныеСотрудника.Сотрудник;
			ДокументПриемНаРаботу.Подразделение = ДанныеСотрудника.ТекущееПодразделение;
			ДокументПриемНаРаботу.Должность = ДанныеСотрудника.ТекущаяДолжность;
			ДокументПриемНаРаботу.ВидЗанятости = ДанныеСотрудника.ТекущийВидЗанятости;
			ДокументПриемНаРаботу.КоличествоСтавок = ДанныеСотрудника.КоличествоСтавок;
			ДокументПриемНаРаботу.СпособРасчетаАванса = ДанныеСотрудника.ТекущийСпособРасчетаАванса;
			ДокументПриемНаРаботу.Аванс = ДанныеСотрудника.ТекущийАванс;
			ДокументПриемНаРаботу.Комментарий = НСтр("ru='Документ создан автоматически при включении кадрового учета.'");
			...
			Если ЗначениеЗаполнено(НачислениеОклад) Тогда
				...
				Начисление.Размер = ДанныеСотрудника.ТекущаяТарифнаяСтавка;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеСотрудника.ДатаУвольнения)
			И Не ЗначениеЗаполнено(ДанныеСотрудника.ПриказОбУвольнении) Тогда
			
			ДокументУвольнение = Документы.Увольнение.СоздатьДокумент();
			
			ДокументУвольнение.Дата = ДанныеСотрудника.ДатаУвольнения;
			ДокументУвольнение.ДатаУвольнения = ДанныеСотрудника.ДатаУвольнения;
			ДокументУвольнение.Организация = ДанныеСотрудника.ТекущаяОрганизация;
			ДокументУвольнение.Сотрудник = ДанныеСотрудника.Сотрудник;
			ДокументУвольнение.ПериодРегистрации = НачалоМесяца(ДокументУвольнение.ДатаУвольнения);
			ДокументУвольнение.Комментарий = НСтр("ru='Документ создан автоматически при включении кадрового учета.'");
			
			ДанныеСотрудников.НайтиСтроки(Новый Структура("ГоловнаяОрганизация, ФизическоеЛицо", ДанныеСотрудника.ГоловнаяОрганизация, ДанныеСотрудника.ФизическоеЛицо));
			ДокументУвольнение.ПрименятьВычетыПослеУвольнения = ДанныеСотрудников.Количество() > 1;
...
Процедура ПриОпределенииДоступныхДляПереводаМакетов(Макеты) Экспорт
	...
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т1);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т5);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т6);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_Т8);
	Макеты.Добавить(Метаданные.ОбщиеМакеты.ПФ_MXL_ТрудовойДоговорМикропредприятий);
	Макеты.Добавить(Метаданные.Отчеты.УнифицированнаяФормаТ2.Макеты.ПФ_MXL_Т2);
...
Функция КритерииПоискаСотрудниковПоКоллекцииОтборов(Отборы) Экспорт
	...
	Для каждого УсловиеПоиска Из Отборы Цикл
		...
		ИначеЕсли ДобавитьКритерийПоискаСправочникаФизическиеЛица(КритерииПоиска, УсловиеПоиска)
			...
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоДокументамВоинскогоУчета(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОВодительскомУдостоверении(КритерииПоиска, УсловиеПоиска)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОСверкеСДокументамиВоинскогоУчета(КритерииПоиска, УсловиеПоиска) 
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОСверкеСВоеннымКомиссариатом(КритерииПоиска, УсловиеПоиска) 
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьКритерийПоискаПоСведениямОБронированииГражданОрганизациями(КритерииПоиска, УсловиеПоиска)
			Или ЭДОБЗК.ДобавитьКритерийПоискаСведенияОНаличииКабинетаСотрудника(КритерииПоиска, УсловиеПоиска) Тогда
		...
		Если ИмяПоля <> ВРег("ДатаНачала")
			...
			ТекстСообщенияИсключения = СтрШаблон(НСтр("ru='Неизвестное условие поиска - %1'"), ИмяПоля);
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ИнвалидностьДатаУстановления";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ДАТАВРЕМЯ(1, 1, 1)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".СведенияОбИнвалидности.ДатаУстановления";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ПодключенКЭДОБЗК";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Подключен к ЭДО'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ИСТИНА";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямЛичныхДанных + ".ПодключенКЭДОБЗК";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	...
	ЭДОБЗК.ЗаполнитьПоляПредставленийКадровыхДанныхФизическихЛиц(ДополнительныеПоляПредставлений, ПутьКПолямЛичныхДанных, ИсключаемыеПоля);
...
Процедура ЗаполнитьПоляПредставленийКадровыхДанныхСотрудников(ДополнительныеПоляПредставлений, ПутьКПолямСотрудника, ПутьКПолямЛичныхДанных, ИсключаемыеПоля) Экспорт
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "ВыявленыВредныеОпасныеФакторы";
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЛОЖЬ";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".ВыявленыВредныеОпасныеФакторы";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	...
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеКЭДОБЗК";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Согласие на присоединение к ЭДО'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЗНАЧЕНИЕ(Документ.СогласиеНаПрисоединениеКЭДОБЗК.ПустаяСсылка)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеКЭДОБЗК";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеКЭДОБЗКСостояние";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Состояние'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ЗНАЧЕНИЕ(Перечисление.СостоянияСогласийНаПрисоединениеКЭДОБЗК.ПустаяСсылка)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеКЭДОБЗК.Состояние";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеКЭДОБЗКСостояниеПриема";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Состояние приема'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ИСТИНА";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеКЭДОБЗК.СостояниеПриема";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеКЭДОБЗКДатаНачала";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Начало'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ДАТАВРЕМЯ(1, 1, 1)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеКЭДОБЗК.ДатаНачала";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
	
	ОписаниеПоля = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеДополнительногоПоляПредставлений();
	ОписаниеПоля.ИмяПоля = "СогласиеНаПрисоединениеКЭДОБЗКДатаОкончания";
	ОписаниеПоля.ЗаголовокПоляСКД = НСтр("ru = 'Окончание'");
	ОписаниеПоля.ПустоеЗначениеНаЯзыкеЗапросов = "ДАТАВРЕМЯ(1, 1, 1)";
	ОписаниеПоля.ПутьПоляСКД = ПутьКПолямСотрудника + ".СогласиеНаПрисоединениеКЭДОБЗК.ДатаОкончания";
	ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьПолеПредставления(ДополнительныеПоляПредставлений, ОписаниеПоля, ИсключаемыеПоля);
...
Функция ДополнительныеСведенияУнифицированнойФормыТ2(СтрокиДанных, ДатаОтчета, КлючВарианта) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		МодульСемейныеПоложенияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ОбразованияФизическихЛиц") Тогда
		МодульОбразованияФизическихЛиц = ОбщегоНазначения.ОбщийМодуль("ОбразованияФизическихЛиц");
		МодульОбразованияФизическихЛиц.ДополнитьСведенияУнифицированнойФормыТ2(
			ДополнительныеСведения, СтрокиДанных, ДатаОтчета, КлючВарианта);
	КонецЕсли;
...
Функция ЗапросВТПлановыеНачисленияСотрудников(ТолькоРазрешенные, ИмяВТПлановыеНачисленияСотрудников, ИмяВременнойТаблицыОтборовСотрудников, ПоляОтбораСотрудников, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень) Экспорт
	...
	Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций.РасчетЗарплаты") Тогда
			МодульРасчетЗарплатыДляНебольшихОрганизаций = ОбщегоНазначения.ОбщийМодуль("РасчетЗарплатыДляНебольшихОрганизаций");
			ЗапросНачислений = МодульРасчетЗарплатыДляНебольшихОрганизаций.ЗапросВТНачисленияОрганизаций(ИмяВременнойТаблицыОтборовСотрудников, ПоляОтбораСотрудников);
		Иначе
			...
			ЗапросНачислений.Текст =
				...
				|	Начисления.Ссылка КАК Начисление
				|ПОМЕСТИТЬ ВТНачисленияОрганизаций
				|ИЗ
				|	ВТСотрудникиОрганизации КАК КадровыеДанныеСотрудников
				|		ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления КАК Начисления
				|		ПО (ИСТИНА)
				|ГДЕ
				|	НачисленияАУСН.НачислениеАУСН ЕСТЬ NULL";
			ЗапросНачислений.Текст = СтрЗаменить(ЗапросНачислений.Текст, "ВТСотрудникиОрганизации", ИмяВременнойТаблицыОтборовСотрудников);
			ЗапросНачислений.Текст = СтрЗаменить(ЗапросНачислений.Текст, "КадровыеДанныеСотрудников.Сотрудник", "КадровыеДанныеСотрудников." + ПоляОтбораСотрудников.Сотрудник);
		КонецЕсли;
		
		ЗарплатаКадрыОбщиеНаборыДанных.СкопироватьПараметрыЗапроса(Запрос, ЗапросНачислений);
		МассивТекстов.Добавить(ЗапросНачислений.Текст);
		...
		ТекстЗапроса =
			...
			|	НачисленияОрганизаций.Начисление,
			|	ВЫБОР
			|		КОГДА НачисленияОрганизаций.Начисление.КодДоходаНДФЛ.ИмеетЕдинственныйВычет
			|			ТОГДА НачисленияОрганизаций.Начисление.КодДоходаНДФЛ.ВычетПоУмолчанию
			|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
			|	КОНЕЦ,
			...
			|ИЗ
			|	ВТДанныеПоКоэффициентамСотрудников КАК ДанныеПоКоэффициентамСотрудников
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияОрганизаций КАК НачисленияОрганизаций
			|		ПО ДанныеПоКоэффициентамСотрудников.Сотрудник.ГоловнаяОрганизация = НачисленияОрганизаций.ГоловнаяОрганизация
			|			И (НачисленияОрганизаций.Начисление.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.РайонныйКоэффициент))
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНачисленияОрганизаций КАК НачисленияОрганизаций
			|		ПО ДанныеПоКоэффициентамСотрудников.Сотрудник.ГоловнаяОрганизация = НачисленияОрганизаций.ГоловнаяОрганизация
			|			И (НачисленияОрганизаций.Начисление.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.СевернаяНадбавка))
...
Функция ОписанияСоставаНачисленийПоВременнойТаблице(МенеджерВременныхТаблиц, ИмяВТСотрудникиПериоды, ИмяПоляПериод, ИмяПоляСотрудник, ИмяПоляДатаОкончания, ИспользоватьКраткиеПредставления, ВыводитьДенежныеПоказателиСВалютой, ФормироватьСПериодичностьДень, ИмяПоляНачисление) Экспорт
	...
	Если Не Результат Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
			...
			Результат = МодульВоинскийУчет.НеобходимыСведенияОВоинскомУчетеФизическихЛиц(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияДокументовВоинскогоУчета(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияОВодительскомУдостоверении(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияОСверкеСДокументамиВоинскогоУчета(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияОСверкеСВоеннымКомиссариатом(ИмяПоля)
				Или МодульВоинскийУчет.НеобходимыСведенияОПредполагаемойДатеСнятияСВоинскогоУчета(ИмяПоля)
...
Функция НеобходимыПостоянныеКадровыеДанныеФизическихЛиц(Знач ИмяПоля) Экспорт
	...
	Результат = КадровыйУчет.НеобходимыЗначенияРеквизитовСправочникаФизическиеЛица(ИмяПоля)
		...
		Или Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеКЭДОБЗК"] <> Неопределено
				И ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК").
					НеобходимыДанныеОПодключенииКЭДОБЗК(ИмяПоля)
		Или ЭДОБЗК.НеобходимыСведенияОНаличииКабинетаСотрудника(ИмяПоля);
...
Функция ДобавитьПолеРеквизитаСправочникаФизическиеЛица(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеКонтактнойИнформацииФизическихЛиц(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимаКонтактнаяИнформацияФизическогоЛица(Знач ИмяПоля)
...
Функция ДобавитьПолеСведенийОВозрасте(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных, ОписательВременнойТаблицыОтборов)
...
Функция ДобавитьПолеСведенийОДокументахУдостоверяющихЛичность(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийОбИнвалидности(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимыСведенияОбИнвалидности(Знач ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("ИнвалидностьПериодРегистрации")
		...
		Или ИмяПоля = ВРег("ИнвалидностьДатаУстановления");
...
Функция ДобавитьКритерийПоискаПоСведениямОбИнвалидности(КритерииПоиска, УсловиеПоиска)
	...
	Если НеобходимыСведенияОбИнвалидности(УсловиеПоиска.ЛевоеЗначение)
		...
		ИначеЕсли ИмяПоля <> ВРег("Период") Тогда
			...
			ИначеЕсли ИмяПоля = ВРег("ИнвалидностьДатаУстановления") Тогда
				УсловиеПоиска.ЛевоеЗначение = "ДатаУстановления";
...
Функция ПутьКДаннымСведенийОбИнвалидности(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ИнвалидностьДатаУстановления") Тогда
		ПутьКДанным = "	СведенияОбИнвалидности.ДатаУстановления";
	Иначе
...
Функция ДобавитьПолеСведенийОГражданстве(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимыСведенияОГражданстве(Знач ИмяПоля)
	...
	Возврат ВРег(ИмяПоля) = ВРег("ГражданствоПериодРегистрации")
		...
		Или ВРег(ИмяПоля) = ВРег("СтранаПолноеНаименование")
...
Функция ПутьКДаннымСведенийОГражданстве(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("СтранаПолноеНаименование") Тогда
		ПутьКДанным =
			"	(ВЫБОР
				|	КОГДА ГражданствоФизическихЛиц.Страна ЕСТЬ NULL
				|		ТОГДА ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
				|	ИНАЧЕ ГражданствоФизическихЛиц.Страна
				|КОНЕЦ).НаименованиеПолное";
...
Функция ДобавитьПолеСведенийОФИОФизическихЛиц(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных, ИмяПоляФизическоеЛицоТаблицыОтборов)
...
Функция ЗапросВТКадровыеДанныеСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, КадровыеДанные, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных, ИмяВТКадровыеДанныеСотрудников, ТаблицаОтборовСодержитПолеФизическоеЛицо, УничтожитьВспомогательныеТаблицы) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из ИменаКадровыхДанных Цикл
		...
		Иначе
			...
			КадровыйУчетПереопределяемый.ПриДобавленииПоляПериодическихКадровыхДанныхСотрудников(
				ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных, СтандартнаяОбработка);
	...
	Иначе
		...
		Если Метаданные.РегистрыСведений["СостояниеСогласияНаПрисоединениеКЭДОБЗК"] <> Неопределено Тогда
			ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.СостояниеСогласияНаПрисоединениеКЭДОБЗК").
				ДобавитьТекстЗапросаВТСведенияОСогласияхПодключенийКЭДОБЗК(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
		КонецЕсли;
		...
		КадровыйУчетПереопределяемый.ПриДобавленииТекстовЗапросовПериодическихКадровыхДанныхСотрудников(
			Запрос, ТолькоРазрешенные, ОписательВТОтборов, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных);
...
Функция ДобавитьПолеСведенийПостоянныхКадровыхДанныхФизическихЛиц(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийТекущихКадровыхДанныхСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийТекущихТарифныхСтавокСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимыСведенияПриказаОПриеме(Знач ИмяПоля) Экспорт
	...
	Результат = ИмяПоля = ВРег("ПриказОПриеме")
		...
		Или ИмяПоля = ВРег("ПриказОПриемеДлительностьИспытательногоСрока")
...
Функция ПутьКДаннымСведенийПриказаОПриемеОбУвольнении(Знач ИмяПоля) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ПриказОПриемеДлительностьИспытательногоСрока") Тогда
			ПутьКДанным = "	ПриказыОПриеме.ДлительностьИспытательногоСрока";
...
Функция ДобавитьПолеПостоянныхКадровыхДанныхСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеКадровыхДанныхФизическихЛиц(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийОДатахСмертиСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимыДатыСмертиСотрудников(Знач ИмяПоля)
...
Функция ПутьКДаннымСведенийОДатахСмертиСотрудников(Знач ИмяПоля)
...
Функция ПутьКДаннымСведенийЗависящихОтФункциональныхОпцийОрганизаций(Знач ИмяПоля) Экспорт
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ЕдиноличныйРуководительКоммерческойОрганизации") Тогда
		
		ПутьКДанным =
			"	ЕСТЬNULL(" + ПутьКДолжности + ".ЯвляетсяДолжностьюЕдиноличногоРуководителяКоммерческойОрганизации, ЛОЖЬ)";
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ЯвляетсяПрокурором")
		...
		Или ИмяПоляВВерхнемРегистре = ВРег("РасчетОтпускаБезУчетаПраздничныхДней")
...
Функция ДобавитьПолеСведенийКадровойИсторииСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
	
	ДобавленоПолеСведений = Ложь;
	Если (ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Или КадровыйУчет.НастройкиКадровогоУчета().ИспользуютсяДанныеСотрудниковВнешнихИсточников)
			И КадровыйУчет.НеобходимыСведенияКадровойИсторииСотрудников(ИмяПоля) Тогда
...
Функция ДобавитьПолеСведенийОбОплатеТруда(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийОВидахЗанятостиСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ПутьКДаннымСведенийОВидахЗанятостиСотрудников(Знач ИмяПоля)
...
Функция ДобавитьПолеСведенийОДолеНеполногоРабочегоВремениСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ДобавитьПолеСведенийОПлановыхАвансах(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ПутьКДаннымСведенийОПлановыхАвансах(Знач ИмяПоля) Экспорт
...
Функция ДобавитьПолеСведенийОКлассахУсловийТруда(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция НеобходимыСведенияОКлассахУсловийТруда(Знач ИмяПоля)
	...
	Возврат ИмяПоля = ВРег("КлассУсловийТрудаПериодРегистрации")
		...
		Или ИмяПоля = ВРег("ВыявленыВредныеОпасныеФакторы");
...
Функция ПутьКДаннымСведенийОКлассахУсловийТруда(Знач ИмяПоля) Экспорт
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("ВыявленыВредныеОпасныеФакторы") Тогда
		...
		ПутьКДанным =
			"	ВЫБОР
			|		КОГДА ФункциональныеОпции.ПрименятьРезультатыСпециальнойОценкиУсловийТруда = ИСТИНА
			|			ТОГДА КлассыУсловийТрудаПоДолжностям.ВыявленыВредныеОпасныеФакторы
			|		ИНАЧЕ ЛОЖЬ
			|	КОНЕЦ";
...
Функция ДобавитьПолеСведенийОбУсловияхДоговоровОпеки(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных)
...
Функция ПутьКДаннымСведенийОбУсловияхДоговоровОпеки(Знач ИмяПоля) Экспорт
...
Функция ДобавитьПолеСведенийМестРаботыСотрудников(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
Функция НеобходимыСведенияЗависящиеОтФункциональныхОпцийОрганизации(Знач ИмяПоля) Экспорт
	...
	Возврат ИмяПоля = ВРег("ЯвляетсяЧленомЛетногоЭкипажа")
		...
		Или ИмяПоля = ВРег("ЕдиноличныйРуководительКоммерческойОрганизации")
		...
		Или ИмяПоля = ВРег("РасчетОтпускаБезУчетаПраздничныхДней")
...
Процедура КадроваяИсторияСотрудниковПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчет.ЗапомнитьСотрудниковДляОбновленияКадровыхДанных(НаборЗаписей, Замещение);
	
КонецПроцедуры

Процедура КадроваяИсторияСотрудниковПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет")
		...
		КадровыйУчет.ОбновитьВторичныеКадровыеДанныеСотрудников(НаборЗаписей, Замещение);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура ПлановыеНачисленияПередЗаписью(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчет.ЗапомнитьСотрудниковДляОбновленияДанныхОбОплатеТруда(НаборЗаписей, Замещение);
	
КонецПроцедуры

Процедура ПлановыеНачисленияПриЗаписи(НаборЗаписей, Отказ, Замещение) Экспорт
	
	КадровыйУчет.ОбновитьВторичныеКадровыеДанныеСотрудников(НаборЗаписей, Замещение);
	
КонецПроцедуры

Процедура ОбновитьВторичныеКадровыеДанныеСотрудников(НаборЗаписей, Замещение) Экспорт
...
Функция ТекстыОписанияПолейЗапросПоляКадровыхДанныхФизическихЛиц(КадровыеДанные, ИсточникиДанных, ОписательВТОтборов)
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийОДокументахУдостоверяющихЛичность(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОДокументахВоинскогоУчета(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОВодительскомУдостоверении(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОСверкеСДокументамиВоинскогоУчета(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОСверкеСВоеннымКомиссариатом(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
				И МодульВоинскийУчет.ДобавитьПолеСведенийОПредполагаемойДатеСнятияСВоинскогоУчета(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаВоинскийУчет
		...
		Иначе
			...
			СтандартнаяОбработка = Истина;
			КадровыйУчетПереопределяемый.ПриДобавленииПоляПериодическихКадровыхДанныхФизическихЛиц(
				ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных, СтандартнаяОбработка);
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиКадровыхДанныхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных, ИмяВТКадровыеДанныеФизическихЛиц) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КадровыйУчет.ВоинскийУчет") Тогда
		...
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТДокументыВоинскогоУчета(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТВодительскоеУдостоверение(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТДанныеСверкиСДокументамиВоинскогоУчета(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТДанныеСверкиСВоеннымКомиссариатом(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
		МодульВоинскийУчет.ДобавитьТекстЗапросаВТСведенияОПредполагаемойДатеСнятияСВоинскогоУчета(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
	...
	КадровыйУчетПереопределяемый.ПриДобавленииТекстовЗапросовПериодическихКадровыхДанныхФизическихЛиц(
		Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных);
...
Процедура ДобавитьВЗапросПоляПостоянныхКадровыхДанныхФизическихЛиц(Запрос, КадровыеДанные, ИсточникиДанных, ИмяВТПостоянныеКадровыеДанныеФизическихЛиц) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		ИначеЕсли ДобавитьПолеРеквизитаСправочникаФизическиеЛица(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеКЭДОБЗК"] <> Неопределено
				И ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК").
					ДобавитьПолеСведенийОПодключенииКЭДОБЗК(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЕстьПодсистемаОбразованияФизическихЛиц
			...
			Или ИнтеграцияКабинетСотрудника.ДобавитьПолеСведенийОДатахОтключенияФизическихЛиц(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			Или ЭДОБЗК.ДобавитьПолеСведенияОНаличииКабинетаСотрудника(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
		...
		Иначе
			...
			СтандартнаяОбработка = Истина;
			КадровыйУчетПереопределяемый.ПриДобавленииПоляПостоянныхКадровыхДанныхФизическихЛиц(
				ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных, СтандартнаяОбработка);
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиПостоянныхКадровыхДанныхФизическихЛиц(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных) Экспорт
	...
	Если Метаданные.РегистрыСведений["ФизическиеЛицаПрисоединенныеКЭДОБЗК"] <> Неопределено Тогда
		ОбщегоНазначения.ОбщийМодуль("РегистрыСведений.ФизическиеЛицаПрисоединенныеКЭДОБЗК").
			ДобавитьТекстЗапросаВТСведенияОПодключенииКЭДОБЗК(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	КонецЕсли;
	...
	КонецЕсли;
	ЭДОБЗК.ДобавитьТекстЗапросаВТСведенияОНаличииКабинетаСотрудника(Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
	
	КадровыйУчетПереопределяемый.ПриДобавленииТекстовЗапросовПостоянныхКадровыхДанныхФизическихЛиц(
		Запрос, ТолькоРазрешенные, ОписательВТОтборов, ИсточникиДанных);
...
Функция ТекстыОписанияПолейЗапросПоляПостоянныхКадровыхДанныхСотрудников(КадровыеДанные, ИсточникиДанных) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		Иначе
			...
			СтандартнаяОбработка = Истина;
			КадровыйУчетПереопределяемый.ПриДобавленииПоляПостоянныхКадровыхДанныхСотрудников(
				ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных, СтандартнаяОбработка);
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиПостоянныхКадровыхДанныхСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных, ФормироватьТаблицуРазличных) Экспорт
	...
	КадровыйУчетПереопределяемый.ПриДобавленииТекстовЗапросовПостоянныхКадровыхДанныхСотрудников(
		Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных);
