...
Функция ЭтоИдентификаторTXID(Идентификатор, УчастникиОпераций) Экспорт
	...
	Возврат ОписаниеИдентификатора.Схема = "TXID";
	
КонецФункции

Функция ЭтоИдентификаторBBAN(Идентификатор, УчастникиОпераций) Экспорт
	...
	Возврат ОписаниеИдентификатора.Схема = "BBAN";
	
КонецФункции

Функция ЭтоИдентификаторPNNM(Идентификатор, УчастникиОпераций) Экспорт
	...
	Возврат ОписаниеИдентификатора.Схема = "PNNM";
	
КонецФункции

Функция ОтчествоСлужебное(Отчество) Экспорт
	
	// Исторический вариант преобразования отчества.
	// Требуется для корректной работы с идентификаторами, сформированными при его использовании.
...
Функция СведенияОбОбъектеИнформационнойБазы(Тип)
	...
	ИначеЕсли Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		...
		НаборСведений.Вставить("СчетБанк",                  Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
		НаборСведений.Вставить("ЭтоСчетОрганизации",        Новый ОписаниеТипов("Булево"));
		НаборСведений.Вставить("ЭтоСчетКонтрагента",        Новый ОписаниеТипов("Булево"));
		НаборСведений.Вставить("ЭтоСчетДепозита",           Новый ОписаниеТипов("Булево"));
		НаборСведений.Вставить("ЭтоЦифровойСчет",           Новый ОписаниеТипов("Булево"));
	ИначеЕсли Тип = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		НаборСведений.Вставить("ЭтоСБП",                    Новый ОписаниеТипов("Булево"));
		НаборСведений.Вставить("УчитыватьНДСПоКомиссии",    Новый ОписаниеТипов("Булево"));
		НаборСведений.Вставить("ЗаполненПроцентКомиссии",   Новый ОписаниеТипов("Булево"));
	КонецЕсли;
...
Функция СвязиМеждуТипамиОбъектовИнформационнойБазы() Экспорт
...
Функция РеквизитыНовыхОбъектовИнформационнойБазы(Тип)
	...
	ИначеЕсли Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		...
		ТипыНомерСчета = Новый ОписаниеТипов(
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(БанковскиеПравила.ТипЦифровойНомерСчета())); // Передаем номер счета максимальной длины.
		НаборРеквизитов.Вставить("НомерСчета",                  ТипыНомерСчета);
		НаборРеквизитов.Вставить("ИдентификаторЦифровогоСчета", ТипыНомерСчета);
		...
		НаборРеквизитов.Вставить("ЦифровойСчет",        Новый ОписаниеТипов("Булево"));
		НаборРеквизитов.Вставить("ВыпискаЗагружаетсяПоДепозитномуСчету", Новый ОписаниеТипов("Булево"));
...
Функция ИсточникиКлючевыхРеквизитовНовыхОбъектовИнформационнойБазы(Тип, Схема)
	...
	ИначеЕсли Схема = "BBAN" Тогда
		Если Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			...
			ИсточникиРеквизитов.Вставить("ИдентификаторЦифровогоСчета", "ИдентификаторЦифровогоСчета");
			...
			ИсточникиРеквизитов.Вставить("ВыпискаЗагружаетсяПоДепозитномуСчету", "ЭтоСчетДепозита");
		КонецЕсли;
	ИначеЕсли Схема = "IBAN" Тогда
		...
		КонецЕсли;
	ИначеЕсли Схема = "DBAN" Тогда
		Если Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
			ИсточникиРеквизитов.Вставить("НомерСчета",                  "НомерБанковскогоСчета");
			ИсточникиРеквизитов.Вставить("БИК",                         "БИКПриведенныйКСтандарту");
			ИсточникиРеквизитов.Вставить("ИдентификаторЦифровогоСчета", "ИдентификаторЦифровогоСчета");
			ИсточникиРеквизитов.Вставить("ЦифровойСчет", "ЭтоЦифровойСчет");
		КонецЕсли;
	ИначеЕсли Схема = "PNNM" Или Схема = "PAM" Тогда
...
Функция ОписаниеИдентификатораПоСхеме(СведенияОбОбъектеИнформационнойБазы, УчастникиОпераций, Схема)
	...
	ИначеЕсли Схема = "DBAN" Тогда
		БИК = ?(ЗначениеЗаполнено(СведенияОбОбъектеИнформационнойБазы.БИК),
			СведенияОбОбъектеИнформационнойБазы.БИК,
			ЦифровойРубль.БИКПлатформыЦифровогоРубля());
		Идентификатор = ИдентификаторПоИдентификаторуЦифровогоРубля(
			УчастникиОпераций,
			СведенияОбОбъектеИнформационнойБазы.ИдентификаторЦифровогоСчета,
			СведенияОбОбъектеИнформационнойБазы.НомерБанковскогоСчета,
			БИК);
	ИначеЕсли Схема = "BKNM" Тогда
	...
	ИначеЕсли Схема = "PAM" Тогда
		Идентификатор = ИдентификаторПоPAM(
			УчастникиОпераций,
			СведенияОбОбъектеИнформационнойБазы.Наименование);
...
Функция ИдентификаторПоНомеруБанковскогоСчета(УчастникиОпераций,
											  НомерБанковскогоСчета,
											  БИК,
											  ИспользуетсяБанкДляРасчетов = Ложь,
											  ИдентификаторВладельца = Неопределено) Экспорт
	...
	Если БанковскиеСчетаСервер.ЭтоИдентификаторЦифровогоСчета(НомерБанковскогоСчета) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИдентификаторВладельца) Тогда
		Состав.Вставить("Владелец", ИдентификаторВладельца);
	КонецЕсли;
	...
	ИначеЕсли Не ИспользуетсяБанкДляРасчетов И ЗначениеЗаполнено(БИК) Тогда
		...
		КонецЕсли;
	ИначеЕсли ИспользуетсяБанкДляРасчетов И Не ЗначениеЗаполнено(БИК) Тогда
		НомерСчетаСоответствуетПравиламЦБР = Ложь;
	Иначе
		НомерСчетаСоответствуетПравиламЦБР = Истина;
	КонецЕсли;
	...
	Иначе
		...
		Если БанковскиеПравила.ЭтоСчетИндивидуальногоПредпринимателя(БалансовыйСчет) Тогда
		...
		Если БанковскиеПравила.ЭтоСчетДепозитаБанка(БалансовыйСчет) Тогда
			Состав.Вставить("ЭтоСчетДепозита", Истина);
			Состав.Вставить("СвязанныеСвойства", "ЭтоСчетДепозита_ЭтоСчетОрганизации"); // отобразим связь через "_"
		КонецЕсли;
	КонецЕсли;
...
Функция ИдентификаторПоМеждународномуНомеруБанковскогоСчета(УчастникиОпераций, НомерБанковскогоСчета, SWIFT) Экспорт
	...
	СтрокаСоответствуетФорматуIBAN = БанковскиеПравила.СтрокаСоответствуетФорматуIBAN(НомерБанковскогоСчета);
	КонтрольныйКлючВалиден         = БанковскиеПравила.ПроверитьКонтрольныйКлючIBAN(НомерБанковскогоСчета);
	
	Если Не СтрокаСоответствуетФорматуIBAN Или Не КонтрольныйКлючВалиден Тогда
		// Вместо платежей по SWIFT могут использоваться платежи через иные сервисы, например, Wise.com.
		// В этом случае может использоваться счет:, который по формату будет похож на IBAN, но при этом им не являющийся.
		// Например, китайские банки могут открывать для иностранных компаний счета нерезидентов
		// (non-resident account, сокращенно NRA).
		Если Не БанковскиеПравила.ЭтоКитайскийСчетNRA(НомерБанковскогоСчета, SWIFT) Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ИдентификаторПоИдентификаторуЦифровогоРубля(УчастникиОпераций, НомерСчета, НомерСчетаЦифровой, БИК) Экспорт
	
	// DBAN - digital BAN
	
	Если ТипЗнч(НомерСчета) <> Тип("Строка") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЦифровойСчет = БанковскиеСчетаСервер.ЭтоИдентификаторЦифровогоСчета(НомерСчета);
	...
	Состав = Новый Структура;
	Состав.Вставить("ИдентификаторЦифровогоСчета", НомерСчета);
	Состав.Вставить("БИК",                         БИК);
	Состав.Вставить("НомерБанковскогоСчета",       НомерСчетаЦифровой);
	Состав.Вставить("ЭтоЦифровойСчет", Истина);
	Состав.Вставить("СвязанныеСвойства", "ЭтоЦифровойСчет_ЭтоСчетКонтрагента"); // отобразим связь через "_"
	
	Возврат УстановитьИдентификатор(УчастникиОпераций, "DBAN", НомерСчета, Состав, Истина, "");
	
КонецФункции

Функция ИдентификаторПоИНН(УчастникиОпераций, Знач ИНН, КПП) Экспорт
...
Функция ИдентификаторПоPAM(УчастникиОпераций, PAM) Экспорт
	...
	Если ТипЗнч(PAM) <> Тип("Строка") Или ПустаяСтрока(PAM) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ЕстьТочкаВКонце = Ложь;
	БезТочки = PAM;
	Если СтрЗаканчиваетсяНа(БезТочки, ".") Тогда
		БезТочки = Сред(БезТочки, 1, СтрДлина(БезТочки) - 1);
		ЕстьТочкаВКонце = Истина;
	КонецЕсли;
	
	ФорматИмени = "Имя,Отчество,Фамилия";
	СловаФИО = СтрРазделить(БезТочки, " ", Ложь);
	Если ЕстьТочкаВКонце И СловаФИО.Количество() = 2 Тогда
		ФорматИмени = "Имя,Фамилия";
	КонецЕсли;
	
	ЧастиФИО = ФизическиеЛицаКлиентСервер.ЧастиИмени(БезТочки, ФорматИмени);
	Имя      = ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЧастиФИО, "Имя",      ""));
	Фамилия  = ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЧастиФИО, "Фамилия",  ""));
	Отчество = ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(
		ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ЧастиФИО, "Отчество", ""));
	Если ПустаяСтрока(Фамилия) И Не ПустаяСтрока(Отчество) Тогда
		Если СтрДлина(Отчество) = 1 Тогда
			// В этом случае в свойстве Отчество содержится Фамилия (1-я буква)
			НаименованиеСлужебное = Отчество + "% " + Имя;
		Иначе // в строке содержится только имя и отчество, без фамилии
			НаименованиеСлужебное = Имя + Отчество + "%";
		КонецЕсли;
	Иначе
	...
	Состав   = Новый Структура;
	Состав.Вставить("Имя",                       PAM);
	Состав.Вставить("НаименованиеСлужебное",     НаименованиеСлужебное);
	Состав.Вставить("ЮридическоеФизическоеЛицо", Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо);
	
	Код = ВРег(БезТочки);
	
	Возврат УстановитьИдентификатор(УчастникиОпераций, "PAM", Код, Состав, Ложь, "");
...
Функция СвойстваСоставаИдентификатора(Схема)
	...
	ИначеЕсли Схема = "DBAN" Тогда
		...
		ОписаниеСвойства = ИменаСвойств.Добавить();
		ОписаниеСвойства.Имя           = "ИдентификаторЦифровогоСчета";
		ОписаниеСвойства.Представление = НСтр("ru = 'идентификатор кошелька цифрового рубля'");
		...
		ОписаниеСвойства = ИменаСвойств.Добавить();
		ОписаниеСвойства.Имя           = "ЭтоЦифровойСчет";
		ОписаниеСвойства.Представление = НСтр("ru = 'Цифровой счет'");
	...
	ИначеЕсли Схема = "PAM" Тогда
		...
		ОписаниеСвойства = ИменаСвойств.Добавить();
		ОписаниеСвойства.Имя           = "Имя";
		ОписаниеСвойства.Представление = НСтр("ru = 'ФИО в формате PAM'");
...
Процедура НайтиВИнформационнойБазе(УчастникиОпераций, СведенияОВладельцеБанковскогоСчетаИзКонтекста) Экспорт
	...
	Для Каждого МетодПоиска Из ОписаниеМетодовПоиска() Цикл
		...
		Для Каждого ОписаниеИдентификатора Из УчастникиОпераций.Идентификаторы.НайтиСтроки(Отбор) Цикл
			...
			Если МетодПоиска.Схема = "PAM" Тогда
				Если ОписаниеИдентификатора.Состав.Свойство("НаименованиеСлужебное")
					И СтрНайти(ОписаниеИдентификатора.Состав.НаименованиеСлужебное, "%") > 0 Тогда
					НоваяСтрока.ШаблонПоиска = ОписаниеИдентификатора.Состав.НаименованиеСлужебное;
				КонецЕсли;
				
				Если ОписаниеИдентификатора.Состав.Свойство("Имя") Тогда
					НоваяСтрока.PAM = ОписаниеИдентификатора.Состав.Имя;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		ИначеЕсли МетодПоиска.Схема = "PAM" Тогда
			Запрос = ЗапросМетодаПоискаPAM(ЗначенияПоискаЗапросом)
		Иначе
			Запрос = Новый Запрос;
...
Функция ОписаниеМетодовПоиска()
	...
	МетодыПоиска.Добавить(ОписаниеМетодаПоискаDBAN());
	...
	МетодыПоиска.Добавить(ОписаниеМетодаПоискаPAM());
...
Функция МераСоответствияИдентификаторов(ОписаниеИдентификатора1, ОписаниеИдентификатора2, ИмяСхемы)
	...
	ИначеЕсли ИмяСхемы = "TXZN" Или ИмяСхемы = "PNNM" Или ИмяСхемы = "AIIN" Или ИмяСхемы = "PAM" Тогда
...
Функция ОписаниеМетодаПоискаBBAN()
	...
	|	БанковскиеСчета.Владелец ССЫЛКА Справочник.Организации КАК ЭтоСчетОрганизации,
	|	БанковскиеСчета.Владелец ССЫЛКА Справочник.Контрагенты КАК ЭтоСчетКонтрагента,
	...
	|	ЕСТЬNULL(ЗарплатныеПроекты.Банк.СВИФТБИК, """"),
	|	"""",
	...
	|	БанковскиеСчета.БанкДляРасчетов.СВИФТБИК,
	|	ВЫБОР
	|		КОГДА БанковскиеСчета.Владелец ССЫЛКА Справочник.ФизическиеЛица
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо)
	|		ИНАЧЕ ЕСТЬNULL(БанковскиеСчета.Владелец.ЮридическоеФизическоеЛицо, ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо))
...
Функция ОписаниеМетодаПоискаDBAN()
	
	// Digital BAN
	
	МетодПоиска = НовыйОписаниеМетодаПоиска("DBAN");
	
	МетодПоиска.ПоляПоискаЗапросом.Вставить("ИдентификаторЦифровогоСчета", БанковскиеПравила.ТипЦифровойНомерСчета());
	
	МетодПоиска.ТекстЗапросаСсылок =
	"ВЫБРАТЬ
	|	ЗначенияПоискаЗапросом.Идентификатор КАК Идентификатор,
	|	ЗначенияПоискаЗапросом.ИдентификаторЦифровогоСчета КАК ИдентификаторЦифровогоСчета
	|ПОМЕСТИТЬ ЗначенияПоискаЗапросом
	|ИЗ
	|	&ЗначенияПоискаЗапросом КАК ЗначенияПоискаЗапросом
	|
	...
	|	"""" КАК СВИФТБИК,
	|	"""" КАК БИКБанкаДляРасчетов,
	|	ЕСТЬNULL(БанковскиеСчета.Владелец.ЮридическоеФизическоеЛицо, ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо)) КАК ЮридическоеФизическоеЛицо,
	|	БанковскиеСчета.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств,
	|	БанковскиеСчета.Владелец ССЫЛКА Справочник.Организации КАК ЭтоСчетОрганизации,
	|	БанковскиеСчета.Владелец ССЫЛКА Справочник.Контрагенты КАК ЭтоСчетКонтрагента,
	|	ЛОЖЬ КАК ВыпискаЗагружаетсяПоДепозитномуСчету,
	|	БанковскиеСчета.ЦифровойСчет КАК ЦифровойСчет,
	|	ЛОЖЬ КАК УстановленОсновным
	|ИЗ
	|	Справочник.БанковскиеСчета КАК БанковскиеСчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗначенияПоискаЗапросом КАК ПараметрыПоиска
	|		ПО БанковскиеСчета.ИдентификаторЦифровогоСчета = ПараметрыПоиска.ИдентификаторЦифровогоСчета
	|ИТОГИ ПО
	|	Идентификатор,
	|	ТипВладельца,
	|	Тип";
...
Функция ОписаниеМетодаПоискаPAM()
	
	МетодПоиска = НовыйОписаниеМетодаПоиска("PAM");
	
	ТипНаименованияКонтрагента = Метаданные.Справочники.Контрагенты.Реквизиты.НаименованиеПолное.Тип;
	МетодПоиска.ПоляПоискаЗапросом.Вставить("ШаблонПоиска", ТипНаименованияКонтрагента);
	МетодПоиска.ПоляПоискаЗапросом.Вставить("PAM", ТипНаименованияКонтрагента);
	
	// Текст запроса определяется отдельно - см. ЗапросМетодаПоискаPAM()
	Возврат МетодПоиска;
	
КонецФункции

Функция ЗапросМетодаПоискаPAM(ЗначенияПоискаЗапросом)
	...
	Для Каждого ОписаниеЗапроса Из ЗначенияПоискаЗапросом Цикл
		...
		Запрос.УстановитьПараметр("PAM", ОписаниеЗапроса.PAM);
		НомерСтроки = Формат(ЗначенияПоискаЗапросом.Индекс(ОписаниеЗапроса) + 1, "ЧГ=0");
		ИмяПараметраИдентификатор = "Идентификатор" + НомерСтроки;
		ИмяПараметраШаблонПоиска  = "ШаблонПоиска"  + НомерСтроки;
...
Процедура НайтиДоговорыКонтрагентовВИнформационнойБазе(УчастникиОпераций, ОтборКонтрагент = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(ВидыОплатОрганизацийКомиссияБанка.ПроцентБанковскойКомиссии, 0) > 0 КАК ЗаполненПроцентКомиссии,
	|	ВЫБОР
	|		КОГДА ВидыОплатОрганизаций.Наименование ПОДОБНО ""%СБП%""
	|				ИЛИ ВидыОплатОрганизаций.ТипОплаты = ЗНАЧЕНИЕ(Перечисление.ТипыОплат.СБП)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоСБП,
	|	ВидыОплатОрганизаций.УчитыватьНДСПоКомиссии КАК УчитыватьНДСПоКомиссии,
	|	ЕСТЬNULL(ВидыОплатОрганизаций.Ссылка, НЕОПРЕДЕЛЕНО) КАК ВидОплаты,
	|	ЕСТЬNULL(ВидыОплатОрганизаций.ПометкаУдаления, ЛОЖЬ) КАК ВидОплатыПометкаУдаления,
	...
	|		ПО ДоговорыКонтрагентов.ВалютаВзаиморасчетов = Валюты.Ссылка
	...
	|				И (ВидыОплатОрганизацийКомиссияБанка.НомерСтроки = 1)
	...
	|	УчитыватьНДСПоКомиссии УБЫВ,
	|	ЭтоСБП УБЫВ,
	|	ЗаполненПроцентКомиссии УБЫВ,
	|	ВидДоговора,
	|	ПометкаУдаления,
	|	ВидОплатыПометкаУдаления,
	|	ВидОплаты УБЫВ,
	|	Ссылка
	|ИТОГИ ПО
	|	Идентификатор";
...
Функция НайтиСсылки(Идентификаторы, УчастникиОпераций, Отбор, ПротоколВыбора = Неопределено, КешИдентификаторов = Неопределено) Экспорт
	...
	Если КешИдентификаторов <> Неопределено И ЗначениеЗаполнено(КешИдентификаторов.ВсеСсылкиПоТипам) Тогда
...
Функция СсылкаНового(Идентификаторы, ЗначенияЗаполнения, Тип, УчастникиОпераций, КешИдентификаторов = Неопределено) Экспорт
	...
	Если СвойстваНового.Свойство("ЭтоСчетКонтрагента") И ЗначенияЗаполнения.Свойство("Тип")
		И ОписаниеСсылки.Владелец().Колонки.Найти("ЭтоСчетКонтрагента") <> Неопределено Тогда
		ОписаниеСсылки.ЭтоСчетКонтрагента = ЗначенияЗаполнения.Тип = Тип("СправочникСсылка.Контрагенты")
			Или ТипЗнч(ЗначенияЗаполнения.Владелец) = Тип("СправочникСсылка.Контрагенты");
	КонецЕсли;
	...
	Если ОписаниеНовогоОбъекта <> Неопределено Тогда
		...
		Для Каждого Идентификатор Из ИдентификаторыНового Цикл
			...
			Для Каждого ИсточникЗначенияРеквизита Из ИсточникиКлючевыхРеквизитовНовыхОбъектовИнформационнойБазы(Тип, ОписаниеИдентификатора.Схема) Цикл
				Если ЗначениеЗаполнено(ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ])
					И Не ТипЗнч(ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ]) = Тип("Булево") Тогда
					Продолжить;
				КонецЕсли;
				...
				Если ТипЗнч(ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ]) = Тип("Булево") Тогда
					СвязанныеСвойства = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
						ОписаниеИдентификатора.Состав, "СвязанныеСвойства");
					Если ИсточникЗначенияРеквизита.Ключ = ИсточникЗначенияРеквизита.Значение Тогда
						ЗначениеСвойства = Новый Структура(ИсточникЗначенияРеквизита.Ключ,
							ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ]);
						ЗаполнитьЗначенияСвойств(ЗначениеСвойства, ОписаниеСсылки);
						Если ТипЗнч(ЗначениеСвойства[ИсточникЗначенияРеквизита.Ключ]) = Тип("Булево") Тогда
							// Данное свойство нужно установить из ОписаниеСсылки
							ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ] = ЗначениеСвойства[ИсточникЗначенияРеквизита.Ключ];
						КонецЕсли;
					ИначеЕсли СвязанныеСвойства <> Неопределено Тогда
						Свойства = СтрРазделить(СвязанныеСвойства, "_");
						Если Свойства.Количество() = 2 И Свойства[0] = ИсточникЗначенияРеквизита.Значение Тогда
							ЗначениеСвойства = Новый Структура(Свойства[1], ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ]);
							ЗаполнитьЗначенияСвойств(ЗначениеСвойства, ОписаниеСсылки);
							Если ТипЗнч(ЗначениеСвойства[Свойства[1]]) = Тип("Булево") Тогда
								// Данное свойство нужно установить с учетом связанного свойства из ОписаниеСсылки
								ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ] = ОписаниеНовогоОбъекта[ИсточникЗначенияРеквизита.Ключ]
									И ЗначениеСвойства[Свойства[1]];
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
	...
	Если ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком()
		И Тип = Тип("СправочникСсылка.БанковскиеСчета") Тогда
		БалансовыйСчет = БанковскиеПравила.БалансовыйСчет(ОписаниеНовогоОбъекта.НомерСчета);
		ЭтоСчетДепозитаБанка = БанковскиеПравила.ЭтоСчетДепозитаБанка(БалансовыйСчет);
		ОписаниеНовогоОбъекта["ВыпискаЗагружаетсяПоДепозитномуСчету"] = ЭтоСчетДепозитаБанка;
	КонецЕсли;
	...
	Для Каждого ТипСсылки Из ПорядокТипов Цикл
		...
		Иначе
			...
			Для Каждого ЭлементОтбора Из ОтборПоТипу Цикл
				...
				Если ОписаниеНовогоОбъекта = Неопределено Или Не ЭлементОтбора.Значение Тогда
					Продолжить;
				КонецЕсли;
		...
		Для НомерСКонца = 1 По КоличествоОбъектов Цикл
			...
			Если РезультатСозданияЭлемента.Создан Тогда
				...
				Если МожноУстановитьОсновным И ТипСсылки = Тип("СправочникСсылка.БанковскиеСчета") Тогда
					...
					Если Не ЗначениеЗаполнено(РезультатСозданияЭлемента.Объект.Банк)
						Или (ЗначениеЗаполнено(РезультатСозданияЭлемента.Объект.Ссылка)
							И РезультатСозданияЭлемента.Объект.ЦифровойСчет) Тогда
...
Функция СоздатьЭлемент(ОписаниеНовогоОбъекта, ОписаниеСсылки, ШаблонДанныхЗаполнения, Ошибки, МенеджерОбъекта, ТипСсылки, КешОбработкиЗаполнения)
	...
	Если ТипСсылки = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		...
		КонецЕсли;
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.БанковскиеСчета")
		И (ОписаниеСсылки = Неопределено Или ТипЗнч(ОписаниеСсылки.Владелец) = Тип("СправочникСсылка.Организации")) Тогда
		
		РазрешеноСозданиеБанковскиеСчетаОрганизации = ОписаниеСсылки <> Неопределено
			И ПравоДоступа("Редактирование", Метаданные.Справочники.БанковскиеСчета)
			И ПравоДоступа("Редактирование", Метаданные.Справочники.Организации);
		
		Если Не РазрешеноСозданиеБанковскиеСчетаОрганизации Тогда
			Ошибка = Ошибки.Добавить();
			Ошибка.Ссылка           = ОписаниеНовогоОбъекта.Ссылка;
			Ошибка.ДанныеЗаполнения = ДанныеЗаполнения;
			Ошибка.ТекстИсключения  = СтрШаблон(НСтр("ru = 'Ошибка доступа: создание банковского счета организации %1'"),
				ОписаниеНовогоОбъекта.НомерСчета);
			
			Если ЗначениеЗаполнено(ОписаниеСсылки) Тогда
				ОписаниеСсылки.Ссылка = Справочники.БанковскиеСчета.ПустаяСсылка();
			КонецЕсли;
