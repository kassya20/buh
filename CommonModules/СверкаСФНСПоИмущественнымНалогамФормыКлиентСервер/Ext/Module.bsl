...
// Определяет дату, которая указана в регистре расчета налога за соотвествующий налоговый период (год).
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Дата - дата, после которой измененения в состоянии объектов требуют особого контроля
...
// Возвращает представление налоговой базы транспортного налога по переданнному коду единицы измерения.
// 
// Параметры:
//  ЕдиницаИзмерения - Строка - код единицы измерения
// 
...
//          Если не указан, то возвращаются все возможные варианты, действующие в разные периоды
// Возвращаемое значение:
//  Массив - массив строк возможного начала имени файла с расчетом налога из ФНС
//
Функция ВозможныеНачалаИмениФайлаРасчета(Налог = Неопределено, НалоговыйПериод = '00010101') Экспорт 

	ВозможныеНачалаИмениФайла = Новый Массив;

	Если ЗначениеЗаполнено(НалоговыйПериод) И НалоговыйПериод >= '20220101' Тогда
		// За 2022 год и далее применяется общее сообщение с тремя имущественными налогами
		ВозможныеНачалаИмениФайла.Добавить(СверкаСФНСПоИмущественнымНалогамФормыКлиентСервер.КНДРасчетаИмущественныхНалогов());
	ИначеЕсли Не ЗначениеЗаполнено(Налог) Тогда
		// Возможен любой вариант файла
		ВозможныеНачалаИмениФайла.Добавить(СверкаСФНСПоИмущественнымНалогамФормыКлиентСервер.КНДРасчетаТранспортногоНалога());
		ВозможныеНачалаИмениФайла.Добавить(СверкаСФНСПоИмущественнымНалогамФормыКлиентСервер.КНДРасчетаЗемельногоНалога());
	...
	Если Не ЗначениеЗаполнено(НалоговыйПериод) Тогда
		// Возможно, уже применяется общее сообщение с тремя имущественными налогами
		ВозможныеНачалаИмениФайла.Добавить(СверкаСФНСПоИмущественнымНалогамФормыКлиентСервер.КНДРасчетаИмущественныхНалогов());
	КонецЕсли;
...
// Вовзращает крайний срок, до которого ФНС должна прислать расчет налога за конкретный налоговый период
//
// Параметры:
//  НалоговыйПериод - Дата - любой день налогового периода (года)
// 
