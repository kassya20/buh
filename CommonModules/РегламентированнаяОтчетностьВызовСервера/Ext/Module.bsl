...
// Возвращает менеджер отчета, встроенного в конфигурацию, или объект внешнего отчета,
// зарегистрированного в системе. Применяется для бесшовной работы с внешними отчетами,
// замещающими исходные отчеты в конфигурации. Через возвращенную сущность доступно:
// - получение форм;
// - получение макетов;
// - обращение к методам модуля менеджера, во внешнем отчете требуется их дублирование
//   в модуле объекта.
// 
// Параметры:
//  ИсточникОтчета - Строка - имя отчета в конфигурации или имя внешнего отчета.
// 
// Возвращаемое значение:
//  ОтчетМенеджер, ОтчетОбъект -
// 
...
// Возвращает объект отчета, объект внешнего отчета или документа по полному имени формы.
// Применяется для обращения к методам из модулей форм отчетов и документов.
// 
// Параметры:
//  ПолноеИмяФормы - Строка - полное имя формы.
// 
// Возвращаемое значение:
//  ОтчетОбъект, ДокументОбъект, Неопределено, ОтчетМенеджер - Объект отчета
// 
Функция ОбъектОтчета(ПолноеИмяФормы) Экспорт
	
	Если СтрНачинаетсяС(ПолноеИмяФормы, "Внешний") Тогда
		Попытка
			// Объект внешнего отчета, зарегистрированного в системе.
			Возврат ВнешниеОтчеты.Создать(СтрРазделить(ПолноеИмяФормы, ".")[1]);
		Исключение
			// Объект внешнего отчета, не зарегистрированного в системе, например, при вызове в фоновом задании.
			Возврат ВнешнийОтчетВФоновомЗадании(СтрРазделить(ПолноеИмяФормы, ".")[1]);
		КонецПопытки;
		
	ИначеЕсли СтрНачинаетсяС(ПолноеИмяФормы, "Документ") Тогда
		// Объект документа.
		Возврат Документы[СтрРазделить(ПолноеИмяФормы, ".")[1]].СоздатьДокумент();
		
	Иначе
		// Объект отчета из конфигурации.
		Возврат Отчеты[СтрРазделить(ПолноеИмяФормы, ".")[1]].Создать();
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьПараметрыВнешнихРегламентированныхОтчетов", Истина);
КонецПроцедуры
...
Функция НеотправляемыйРеглОтчетИмеетВозможностьИзмененияСтатусаОтправкиИзФормыОтчетаВручную(ИмяОбъекта)
	...
	МассивИмен.Добавить("РегламентированныйОтчетОценочнаяКартаУБ");
	МассивИмен.Добавить("РегламентированныйОтчетГОЗФорма1д");
	МассивИмен.Добавить("РегламентированныйОтчетГОЗФорма2д");
	МассивИмен.Добавить("РегламентированныйОтчетГОЗФорма3д");
