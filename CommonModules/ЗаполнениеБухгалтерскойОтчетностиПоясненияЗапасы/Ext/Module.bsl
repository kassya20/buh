...
Процедура ЗаполнитьГруппыЗапасов(ГруппыЗапасов, СчетаУчетаЗапасов, КонтекстЗаполненияОтчета)
	...
	Группа = ДобавитьГруппуЗапасов(ГруппыЗапасов, НСтр("ru = 'Незавершенное производство'"), ПланыСчетов.Хозрасчетный.ОсновноеПроизводство_);
	Группа.СчетаЗатрат.Добавить(ПланыСчетов.Хозрасчетный.Полуфабрикаты);
	Группа.СчетаЗатрат.Добавить(ПланыСчетов.Хозрасчетный.ВспомогательныеПроизводства_);
...
Функция НовыйСчетаУчетаЗапасов()
	...
	СчетаУчетаЗапасов.ВнеоборотныеАктивы.Добавить(ПланыСчетов.Хозрасчетный.ВыполнениеРемонтовОС);
...
Функция СчетаУчетаРБП()
	Возврат ЗаполнениеБухгалтерскойОтчетностиТабличное.СчетаПоСубконто(
		БухгалтерскийУчет.СчетаУчетаРасходовБудущихПериодов(),
...
Функция ОписаниеСтатейЗатрат(ВидСтатьи)
	...
	|ГДЕ
	|	СтатьиЗатрат.ВидРасходовНУ = &ВидСтатьи
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ВыбытиеОбесцененияОпределяетсяПоСправкеРасчету(КонтекстЗаполненияОтчета)
	...
	Возврат УчетнаяПолитика.ОбесценениеЗапасов(КонтекстЗаполненияОтчета.Организации[0], КонтекстЗаполненияОтчета.Период.КонецДата);
...
Процедура СоздатьАвансыВыданные(МенеджерВременныхТаблиц, КонтекстЗаполненияОтчета, СчетаАвансы)
	...
	ЗаполнитьАвансыВыданные(АвансыВыданные, КонтекстЗаполненияОтчета, СчетаАвансы, Перечисления.ВидыАктивовДляАвансов.Запасы);
...
Процедура ЗаполнитьАвансыВыданные(АвансыВыданные, КонтекстЗаполненияОтчета, СчетаАвансы, ВидАктива)
	
	Для Каждого Организация Из КонтекстЗаполненияОтчета.Организации Цикл
		
		ПараметрыИтогов = ЗаполнениеБухгалтерскойОтчетностиИтоги.НовыеПараметрыСчетовУчетаАвансовПоВидамАктива();
		ПараметрыИтогов.Организация   = Организация;
		ПараметрыИтогов.НачалоПериода = КонтекстЗаполненияОтчета.Период.Начало;
		ПараметрыИтогов.КонецПериода  = КонтекстЗаполненияОтчета.Период.Конец;
		ПараметрыИтогов.СчетаУчетаАвансов    = СчетаАвансы;
		ПараметрыИтогов.СчетаУчетаНДСАвансов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПланыСчетов.Хозрасчетный.НДСпоАвансамИПредоплатам);
		ПараметрыИтогов.РазделятьАвансыПоВидамАктивов = Истина;
		
		ВсеАвансыВыданные = ЗаполнениеБухгалтерскойОтчетностиИтоги.ИтогиСчетовУчетаАвансовПоВидамАктива(ПараметрыИтогов);
		
		Для Каждого АвансВыданный Из ВсеАвансыВыданные.НайтиСтроки(Новый Структура("ВидАктива", ВидАктива)) Цикл
			
			ДобавитьЗаписьАвансыВыданные(АвансыВыданные, Организация, АвансВыданный.Счет, "Начало",   АвансВыданный.СНД + АвансВыданный.СНК);
			ДобавитьЗаписьАвансыВыданные(АвансыВыданные, Организация, АвансВыданный.Счет, "АвансыДт", АвансВыданный.ДО);
			ДобавитьЗаписьАвансыВыданные(АвансыВыданные, Организация, АвансВыданный.Счет, "АвансыКт", -АвансВыданный.КО);
			ДобавитьЗаписьАвансыВыданные(АвансыВыданные, Организация, АвансВыданный.Счет, "Конец",    АвансВыданный.СКД + АвансВыданный.СКК);
...
Процедура ДобавитьЗаписьАвансыВыданные(АвансыВыданные, Организация, Счет, Характеристика, Сумма)
	
	Если Сумма = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Запись = АвансыВыданные.Добавить();
	Запись.Организация    = Организация;
	Запись.Счет           = Счет;
...
Процедура УстановитьОтборПоАналитике(ЗначенияНастроекОтчета, Выборка)
	...
	Если ЗначениеЗаполнено(Выборка.ОтборПоАналитике) Тогда
		...
		Если Выборка.ОтборПоАналитике = "ТранспортныеРасходы" Тогда
			...
			ШаблонПоляОтбора = "Субконто%1.ВидРасходовНУ";
			ВидСубконтоОтбора = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат;
		ИначеЕсли Выборка.ОтборПоАналитике = "Авансы" Тогда
			ЗначениеОтбора = Перечисления.ВидыАктивовДляАвансов.Запасы;
			ШаблонПоляОтбора = "Субконто%1.ВидАктива";
			ВидСубконтоОтбора = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры;
