...
Процедура ОповеститьОбУдачнойОтправке(Параметры = Неопределено) Экспорт
	...
	Оповестить(РегламентированнаяОтчетностьКлиент.ИмяСобытияУспешнаяОтправка(), Параметры);
...
Процедура ЗапомнитьКонтекстДлительнойОперации(
	...
	Если СтрЗаканчиваетсяНа(ИмяПараметраПриложения, "Массив") Тогда
		МассивКонтекстов = ПараметрыПриложения[ИмяПараметраПриложения];
		Если МассивКонтекстов = Неопределено Тогда
			МассивКонтекстов = Новый Массив;
		КонецЕсли;
		МассивКонтекстов.Добавить(ВходящийКонтекст);
		ПараметрыПриложения[ИмяПараметраПриложения] = МассивКонтекстов;
...
Функция КонтекстДлительнойОперации(ИмяПараметраПриложения = "БРО.ПараметрыОбновленияОтправкиИзПанелиОтправки") Экспорт
	...
	Если СтрЗаканчиваетсяНа(ИмяПараметраПриложения, "Массив") Тогда
		...
		МассивКонтекстов = ПараметрыПриложения[ИмяПараметраПриложения];
		Если МассивКонтекстов <> Неопределено Тогда
			Если МассивКонтекстов.Количество() <> 0 Тогда
				ДополнительныеПараметры = МассивКонтекстов[0];
				МассивКонтекстов.Удалить(0);
			КонецЕсли;
			Если МассивКонтекстов.Количество() <> 0 Тогда
				ПараметрыПриложения[ИмяПараметраПриложения] = МассивКонтекстов;
			Иначе
...
Функция ЕстьКонтекстДлительнойОперации(
		ИмяПараметраПриложения = "БРО.ПараметрыОбновленияОтправкиИзПанелиОтправки") Экспорт
	
	Возврат ПараметрыПриложения[ИмяПараметраПриложения] <> Неопределено;
...
Процедура ПоказатьОшибки(ПутьКОбъекту, ВходящийКонтекст) Экспорт
	...
	Если ЕстьОшибкаРазмераПисьмаФНС(ПараметрыФормы) Тогда
		...
		Ошибка = ПисьмаВГосОрганыКлиент.ОшибкаРазмераПисьмаФНС(ПараметрыФормы.Ошибки);
...
Функция ЕстьОшибкаРазмераПисьмаФНС(ПараметрыФормы)
	...
	Ошибка = ПисьмаВГосОрганыКлиент.ОшибкаРазмераПисьмаФНС(Ошибки);
