...
Функция БазаДляРаспределенияНДФЛ(ПараметрыПолученияБазы) Экспорт
	...
	Если ЗарплатаКадры.ВТСуществует(МенеджерВременныхТаблиц, "ВТСведенияОДоходахЗачетАванса")
		И ПараметрыПолученияБазы.РегистраторыНДФЛОбновленияБухучета = Неопределено Тогда
		...
		|ГДЕ
		|	НЕ Начисления.НатуральныйДоход
		|
		|СГРУППИРОВАТЬ ПО
	...
	Иначе
		...
		Если ЗарплатаКадры.ВТСуществует(МенеджерВременныхТаблиц, "ВТСведенияОДоходахЗачетАванса")
			...
			|	СведенияОДоходах.ДоходМежрасчетногоПериода КАК ДоходМежрасчетногоПериода,
			|	СведенияОДоходах.КатегорияДохода КАК КатегорияДохода,
			|	СведенияОДоходах.ДанныеТекущегоДокумента КАК ДанныеТекущегоДокумента,
			|	СУММА(СведенияОДоходах.Сумма) КАК Сумма,
			|	СведенияОДоходах.ЗарегистрированоПриНачисленииАванса КАК ЗарегистрированоПриНачисленииАванса
			|ИЗ
			|	ВТСведенияОДоходахДляРаспределенияНДФЛ КАК СведенияОДоходах
			|ГДЕ
			|	НЕ СведенияОДоходах.СторноТекущегоПериода
			|	И &ДополнительноеУсловие
			|
			...
			|	СведенияОДоходах.Сотрудник,
			|	СведенияОДоходах.Начисление,
			|	СведенияОДоходах.ДанныеТекущегоДокумента,
			|	СведенияОДоходах.Подразделение,
			|	СведенияОДоходах.ДоходМежрасчетногоПериода,
			|	СведенияОДоходах.ФизическоеЛицо,
			|	СведенияОДоходах.Территория,
			...
			ДоходыАванс = РезультатЗапроса[0].Выгрузить();
			
			БазаЗачетАванса = РезультатЗапроса[1].Выгрузить();
			Для каждого СтрокаТЗ Из БазаЗачетАванса Цикл
				СтрокаТЗ.ВидДоходаИсполнительногоПроизводства = ВидыДоходовИсполнительногоПроизводства[СтрокаТЗ.Начисление];
			КонецЦикла;
...
Функция ТаблицаНачислениеУдержаниеВидОперации(ВыделятьНДФЛСПревышения = Истина) Экспорт
	...
	|				ИЛИ ВидыОсобыхНачислений.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.ДополнительнаяОблагаемаяБазаРуководителя)
	|					И ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.НачисленоДоход))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ДобавитьКолонкиСторно(Таблица) Экспорт
	...
	Если Таблица.Колонки.Найти("ИсходныйДокумент") = Неопределено Тогда
		Таблица.Колонки.Добавить("ИсходныйДокумент", Таблица.Колонки.Регистратор.ТипЗначения);
	КонецЕсли;
...
Функция НоваяТаблицаНачислений() Экспорт
	...
	ТипыИсходныхДокументов = ОтражениеЗарплатыВУчетеВнутренний.ТипыИсходныхДокументов();
	Если ТипыИсходныхДокументов = Неопределено Тогда
		ТипыИсходныхДокументов = ТипыРегистраторов;
	КонецЕсли;
	...
	Таблица.Колонки.Добавить("ИсходныйДокумент", Новый ОписаниеТипов(ТипыИсходныхДокументов));
...
Процедура СоздатьВТНачисленияПоТаблицеЗначений(МенеджерВременныхТаблиц, НачисленияУдержания)
	...
	|	НачисленияУдержания.ИсходныйДокумент КАК ИсходныйДокумент
	|ПОМЕСТИТЬ ВТНачисленияУдержания
	|ИЗ
	|	&НачисленияУдержания КАК НачисленияУдержания
	|;
	...
	|	НачисленияУдержания.ИсходныйДокумент
	|ИЗ
	|	ВТНачисленияУдержания КАК НачисленияУдержания
	|ГДЕ
...
Процедура СоздатьВТНачисления(МенеджерВременныхТаблиц, Организации, ПериодРегистрации, Сотрудники = Неопределено)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоСотрудникам", УсловиеПоСотрудникам);
	ОписаниеПоляИсходныйДокумент = "ЗНАЧЕНИЕ(Документ.НачислениеЗарплаты.ПустаяСсылка)";
	Если Метаданные.РегистрыНакопления.НачисленияУдержанияПоСотрудникам.Реквизиты.Найти("ИсходныйДокумент") <> Неопределено Тогда
		ОписаниеПоляИсходныйДокумент = "Начисления.ИсходныйДокумент";
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИсходныйДокумент", ОписаниеПоляИсходныйДокумент);
...
Функция ТаблицаВзносовДляОтражениеВУчете(Взносы, Период)
	...
	ДополнительныеПоля = "ФизическоеЛицо,Сотрудник,Подразделение,Начисление,ОблагаетсяЕНВД,ДатаНачала,БюджетныйТарифНСиПЗ";
...
Функция ТаблицаСтраховыхВзносовПоВидамОпераций(Взносы, Период, ДополнительныеПоля) Экспорт
	...
	Если ТаблицаВзносов.Колонки.Найти("БюджетныйТарифНСиПЗ") = Неопределено Тогда
		ТаблицаВзносов.Колонки.Добавить("БюджетныйТарифНСиПЗ", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	Если Взносы.Колонки.Найти("БюджетныйТарифНСиПЗ") = Неопределено Тогда
		Взносы.Колонки.Добавить("БюджетныйТарифНСиПЗ", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
...
Функция НоваяТаблицаСтраховыеВзносы() Экспорт
	...
	Таблица.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
	Таблица.Колонки.Добавить("ИдентификаторСтроки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(7, 0)));
	Таблица.Колонки.Добавить("БюджетныйТарифНСиПЗ", Новый ОписаниеТипов("Булево"));
