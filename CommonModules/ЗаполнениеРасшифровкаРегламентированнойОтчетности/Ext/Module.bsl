...
//
// Расшифровки могут предоставляться в двух видах
// - "нечеткий" - применяется в большинстве отчетов, по умолчанию;
//                позволяет указать часть выводимых сведений,
//                а другая (например, итоги, арифметические выражения, группировка по организациям и т.п.) будет доопределена при выводе
// - "линейный" - позволяет вывести расшифровку именно так, как нужно конкретному отчету.
// В рамках одного отчета (одной коллекции) можно совмещать оба варианта - часть показателей могут расшифровываться "нечетко", а часть - "линейно".
//
// В зависимости от вида расшифровки используются все или часть полей коллекции.
// 
// Для работы с линейной расшифровкой см. методы области ЛинейнаяРасшифровка
// 
...
//
// Рекомендуется для добавления записей вида "нечеткий".
// Для линейной расшифровки см. УстановитьЛинейнаяРасшифровка
//
...
//                     По умолчанию в отчет включаются и организации-обособленные подразделения
//                     (филиалы на отдельном балансе). Чтобы их исключить см. ИсключитьОбособленныеПодразделения
//  НачалоПериода	 - Дата	 - начало периода, за который формируется отчет
//  КонецПериода     - Дата, МоментВремениСУточнениемПериода - конец периода, за который формируется отчет.
//                     Уточнение периода может указывать на то, что в отчет нужно включить межотчетный период.
...
Функция НовыйНастройкиОтчетаРасходыПоСтатьямЗатрат(Организация, НачалоПериода, КонецПериода, Счет) Экспорт
	...
	Настройки.Отчет.ПараметрыФормы.Вставить("СформироватьПриОткрытии", Истина);
	Настройки.Отчет.ПараметрыФормы.Вставить("Счета", Счет);
	...
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("Счета", Счет);
	Если Справочники.Организации.ВсяОрганизация(Организация).Количество() > 1 Тогда
		ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ВключатьОбособленныеПодразделения", Истина);
	КонецЕсли;
...
//  Отбор        - ОтборКомпоновкиДанных - настройки отбора
// 
// Возвращаемое значение:
//	ЭлементОтбораКомпоновкиДанных - Добавленный элемент отбора
//
Функция ДобавитьОтборВНастройкиОтчета(Настройки, ИмяПоля, Значение, ВидСравнения = Неопределено, Отбор = Неопределено) Экспорт
...
// Добавляет в отбор настройки компоновки новую группировку.
//
// Параметры:
//  Отбор - ОтборКомпоновкиДанных - настройки отбора
//  ТипГруппы - ТипГруппыЭлементовОтбораКомпоновкиДанных - тип добавляемой группировки
//
// Возвращаемое значение:
//  ГруппаЭлементовОтбораКомпоновкиДанных - Добавленная группировка
//
Функция ДобавитьГруппуВОтборВНастройкиОтчета(Отбор, ТипГруппы) Экспорт
...
// Для отчета "АнализСубконто" следует использовать ОписаниеСубконтоОтчетаАнализСубконто
//
// Параметры:
//  ПолеВиртуальнойТаблицы - Строка - имя поля виртуальной таблицы, лежащей в основе отчета, например "Субконто", "КорСубконто"
//  Счет                   - ПланСчетовСсылка.Хозрасчетный - счет, по которому формируется отчет
...
// 
// Возвращаемое значение:
//  - ОписаниеСубконто - описание субконто
//  - Неопределено - указанное субконто отсутствует на счете
//
