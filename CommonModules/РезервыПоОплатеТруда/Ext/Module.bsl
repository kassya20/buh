...
Функция СтруктураНастроекРезервов() Экспорт
	...
	Настройки.Вставить("УчитыватьМРОТПриОплатеПоСреднемуЗаработку", Ложь);
	Настройки.Вставить("РаспределятьВыявленныеИзлишки", Ложь);
	Настройки.Вставить("ИспользоватьВсеДоступныеОстаткиПоСтатьямФинансирования", Ложь);
	
	Настройки.Вставить("Ссылка", Справочники.НастройкиРасчетаРезервовПоОплатеТруда.ПустаяСсылка());
...
Процедура ПеречитатьОценочныеОбязательства(Объект, ПересчитатьПолностью) Экспорт
	...
	Если ПересчитатьПолностью Тогда
		...
		НастройкаРезерва = НастройкиРезерва(Объект.Организация, Объект.Резерв, Объект.ПериодРегистрации);
		РаспределятьВыявленныеИзлишки = НастройкаРезерва.РаспределятьВыявленныеИзлишки
			И (Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Начисление
				Или Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация);
		
		СформироватьТаблицуОценочныеОбязательства(ОценочныеОбязательстваПоСотрудникам, Объект.ОценочныеОбязательства, РаспределятьВыявленныеИзлишки);
	КонецЕсли;
...
Процедура ОбновитьОтражениеВУчете(Объект) Экспорт
	...
	НастройкаРезерва = НастройкиРезерва(Объект.Организация, Объект.Резерв, Объект.МесяцНачисления);
	РаспределятьВыявленныеИзлишки = НастройкаРезерва.РаспределятьВыявленныеИзлишки
		И (Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Начисление
			Или Объект.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация);
	
	СформироватьТаблицуОценочныеОбязательства(ТаблицаРаспределения, Объект.ОценочныеОбязательства, РаспределятьВыявленныеИзлишки);
	...
	Для Каждого ВидУчета Из МассивВидовУчета Цикл
		Для Каждого ЗначениеПоказателя Из МассивПоказателей Цикл
			Для Каждого СтрокаТаблицы Из ТаблицаРасчетРезервов Цикл
				Коэфффициент = 0;
				Если Направление = "Списание"
					И (СтрокаТаблицы.Перемещен Или СтрокаТаблицы[ЗначениеПоказателя + "Накоплено" + ВидУчета] > 0) Тогда
					Коэфффициент = 1;
				КонецЕсли;
				Если Направление = "Корректировка" Тогда
					Если СтрокаТаблицы.Перемещен Или СтрокаТаблицы[ЗначениеПоказателя + "Накоплено" + ВидУчета] < 0 Тогда
						Коэфффициент = -1;
					ИначеЕсли СтрокаТаблицы.Зачислен Тогда
						Коэфффициент = 1;
					КонецЕсли;
				КонецЕсли;
...
Функция ТаблицаОценочныеОбязательстваПоСотрудникам() Экспорт
	...
	ОценочныеОбязательстваПоСотрудникам.Колонки.Добавить("Перемещен", Новый ОписаниеТипов("Булево"));
	ОценочныеОбязательстваПоСотрудникам.Колонки.Добавить("Зарезервирован", Новый ОписаниеТипов("Булево"));
	ОценочныеОбязательстваПоСотрудникам.Колонки.Добавить("Зачислен", Новый ОписаниеТипов("Булево"));
	ОценочныеОбязательстваПоСотрудникам.Колонки.Добавить("Уволен", Новый ОписаниеТипов("Булево"));
...
Процедура РассчитатьСуммыРезерваПоСотрудникам(СтруктураТаблиц, Организация, Резерв, МесяцНачисления, Регистратор, ВидОперации) Экспорт
	...
	ПереопределитьСпособОтраженияСтатьюРасходов(РасчетРезервов, Организация, Резерв, МесяцНачисления, ВидОперации);
...
Процедура РассчитатьСуммыРезерва(СтруктураТаблиц, Организация, Резерв, МесяцНачисления, Регистратор, ВидОперации) Экспорт
	...
	НастройкаРезерва = НастройкиРезерва(Организация, Резерв, МесяцНачисления);
	РаспределятьВыявленныеИзлишки = НастройкаРезерва.РаспределятьВыявленныеИзлишки
		И (ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Начисление
			Или ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Инвентаризация);
	СформироватьТаблицуОценочныеОбязательства(ОценочныеОбязательстваПоСотрудникам, ОценочныеОбязательства, РаспределятьВыявленныеИзлишки);
...
Функция ПечатьСправкаРасчет(МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ВидУчета = "") Экспорт
	...
	ОбластьМакетаМСФОЗаголовокНачисление  = МакетМСФО.ПолучитьОбласть("ЗаголовокТаблицы" + ВидУчета + "|ТаблицаНачисление");
	ОбластьМакетаМСФОЗаголовокРасчет1     = МакетМСФО.ПолучитьОбласть("ЗаголовокТаблицы" + ВидУчета + "|ТаблицаПоказатель1");
	...
	МассивЗаголовковРасчет.Добавить(ОбластьМакетаМСФОЗаголовокРасчет1);
	...
	ОбластьМакетаМСФООсновнаяСтрокаНачисление       = МакетМСФО.ПолучитьОбласть("СтрокаОсновная" + ВидУчета + "|ТаблицаНачисление");
	ОбластьМакетаМСФООсновнаяСтрокаРасчет1          = МакетМСФО.ПолучитьОбласть("СтрокаОсновная" + ВидУчета + "|ТаблицаПоказатель1");
	...
	МассивЗначенийОсновнойСтрокиРасчет.Добавить(ОбластьМакетаМСФООсновнаяСтрокаРасчет1);
	...
	ОбластьМакетаМСФОДополнительнаяСтрокаФизическиеЛицаНачисление  = МакетМСФО.ПолучитьОбласть("СтрокаДополнительнаяФизическиеЛица" + ВидУчета + "|ТаблицаНачисление");
	ОбластьМакетаМСФОДополнительнаяСтрокаФизическиеЛицаРасчет1     = МакетМСФО.ПолучитьОбласть("СтрокаДополнительнаяФизическиеЛица" + ВидУчета + "|ТаблицаПоказатель1");
	...
	МассивЗначенийДополнительнойСтрокиФизическиеЛицаРасчет.Добавить(ОбластьМакетаМСФОДополнительнаяСтрокаФизическиеЛицаРасчет1);
	...
	ОбластьМакетаМСФОДополнительнаяСтрокаСотрудникиНачисление  = МакетМСФО.ПолучитьОбласть("СтрокаДополнительнаяСотрудники" + ВидУчета + "|ТаблицаНачисление");
	ОбластьМакетаМСФОДополнительнаяСтрокаСотрудникиРасчет1     = МакетМСФО.ПолучитьОбласть("СтрокаДополнительнаяСотрудники" + ВидУчета + "|ТаблицаПоказатель1");
	...
	МассивЗначенийДополнительнойСтрокиСотрудникиРасчет.Добавить(ОбластьМакетаМСФОДополнительнаяСтрокаСотрудникиРасчет1);
	...
	ОбластьМакетаМСФОИтогиНачисление    = МакетМСФО.ПолучитьОбласть("Итоги" + ВидУчета + "|ТаблицаНачисление");
	ОбластьМакетаМСФОИтогиСтрокаРасчет1 = МакетМСФО.ПолучитьОбласть("Итоги" + ВидУчета + "|ТаблицаПоказатель1");
	...
	МассивЗначенийСтрокиИтоги.Добавить(ОбластьМакетаМСФОИтогиСтрокаРасчет1);
	...
	Для Каждого ДанныеПечати Из ДанныеПечатиОбъектов Цикл
		...
		// Требуется ли расшифровка наичислений
		ЕстьРасшифровкаНачислений = ДанныеДокумента.ЕстьРасшифровкаНачислений;
		...
		Иначе
			...
			Если ЕстьРасшифровкаНачислений Тогда
				ДокументРезультат.Присоединить(ОбластьМакетаМСФОЗаголовокНачисление);
			КонецЕсли;
			Для ИндексПоказателя = 0 По ДанныеДокумента.КоличествоПоказателей - 1 Цикл
			...
			Для каждого ДанныеСотрудника Из ДанныеДокумента.ТабличнаяЧастьДокумента Цикл
				...
				Если ДанныеСотрудника.ДополнительнаяСтрокаСотрудники Тогда
					...
					ОбластьМакетаМСФОСтрокаНачисление  = ОбластьМакетаМСФОДополнительнаяСтрокаСотрудникиНачисление;
				...
				ИначеЕсли ДанныеСотрудника.ДополнительнаяСтрокаФизическиеЛица Тогда
					...
					ОбластьМакетаМСФОСтрокаНачисление  = ОбластьМакетаМСФОДополнительнаяСтрокаФизическиеЛицаНачисление;
				...
				Иначе
					...
					ОбластьМакетаМСФОСтрокаНачисление  = ОбластьМакетаМСФООсновнаяСтрокаНачисление;
				...
				Если ЕстьРасшифровкаНачислений Тогда
					ОбластьМакетаМСФОСтрокаНачисление.Параметры.Заполнить(ДанныеСотрудника);
					ДокументРезультат.Присоединить(ОбластьМакетаМСФОСтрокаНачисление);
				КонецЕсли;
			...
			Если ЕстьРасшифровкаНачислений Тогда
				ДокументРезультат.Присоединить(ОбластьМакетаМСФОИтогиНачисление);
			КонецЕсли;
...
Процедура СформироватьТаблицуОценочныеОбязательства(ТаблицаОбязательств, ОценочныеОбязательства, РаспределятьВыявленныеИзлишки = Ложь) Экспорт
	...
	РаспределитьВыявленныеИзлишки(ТаблицаРезервов, РаспределятьВыявленныеИзлишки);
...
Процедура СформироватьТаблицуОценочныеОбязательстваПоСотрудникам(ТаблицаОбязательств, ОценочныеОбязательстваПоСотрудникам, Очищать = Истина) Экспорт
	...
	|Перемещен, Зарезервирован, Зачислен, Уволен",
...
Процедура РаспределитьВыявленныеИзлишки(ТаблицаИсточник, РаспределятьВыявленныеИзлишки) Экспорт
	
	Если Не РаспределятьВыявленныеИзлишки Тогда
		Возврат;
	КонецЕсли;
	
	ДополнитьНедостающимиКолонками(ТаблицаИсточник);
	
	ТаблицаИсточникВременная = ТаблицаИсточник.СкопироватьКолонки();
	ТаблицаИсточникНовая = ТаблицаИсточник.СкопироватьКолонки();
	ТаблицаИсточникСписания = ТаблицаИсточник.СкопироватьКолонки();
	
	СтатьиФинансирования = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточник, "СтатьяФинансирования", Истина);
	
	СписокКолонок = "СуммаРезерва,СуммаРезерваНУ,
		|СуммаРезерваСтраховыхВзносов,СуммаРезерваСтраховыхВзносовНУ,
		|СуммаРезерваФССНесчастныеСлучаи,СуммаРезерваФССНесчастныеСлучаиНУ";
	
	МассивИменПоказателей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СписокКолонок,,Истина, Истина);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаИсточник.Скопировать(Новый Структура("Перемещен", Истина)), ТаблицаИсточникНовая);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаИсточник.Скопировать(Новый Структура("Зачислен", Истина)), ТаблицаИсточникНовая);
	
	Для Каждого СтатьяФинансирования Из СтатьиФинансирования Цикл
		
		ТаблицаИсточникВременная.Очистить();
		ТаблицаИсточникСписания.Очистить();
		
		ТаблицаИсточникПоСтатьям = ТаблицаИсточник.Скопировать(Новый Структура("СтатьяФинансирования, Перемещен, Зачислен", СтатьяФинансирования, Ложь, Ложь));
		
		СтруктураЗначенийРаспределения = Новый Структура;
		СтруктураЗначенийБазы = Новый Структура;
		СтруктураЗначенийРазницы = Новый Структура;
		Для Каждого ИмяПоказателя Из МассивИменПоказателей Цикл
			СтруктураЗначенийРаспределения.Вставить(ИмяПоказателя, 0);
			СтруктураЗначенийБазы.Вставить(ИмяПоказателя, 0);
			СтруктураЗначенийРазницы.Вставить(ИмяПоказателя, 0);
		КонецЦикла;
		
		ЕстьДанныеДляРаспределения = Ложь;
		Для Каждого СтрокаТаблицы Из ТаблицаИсточникПоСтатьям Цикл
			Для Каждого ИмяПоказателя Из МассивИменПоказателей Цикл
				Если СтрокаТаблицы[ИмяПоказателя] < 0 Тогда
					СтруктураЗначенийРаспределения.Вставить(ИмяПоказателя, СтруктураЗначенийРаспределения[ИмяПоказателя] + СтрокаТаблицы[ИмяПоказателя]);
					НоваяСтрока = ТаблицаИсточникСписания.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, , СписокКолонок);
					НоваяСтрока[ИмяПоказателя] = СтрокаТаблицы[ИмяПоказателя];
					СтрокаТаблицы[ИмяПоказателя] = 0;
					ЕстьДанныеДляРаспределения = Истина;
				Иначе
					СтруктураЗначенийБазы.Вставить(ИмяПоказателя, СтруктураЗначенийБазы[ИмяПоказателя] + СтрокаТаблицы[ИмяПоказателя]);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		Если Не ЕстьДанныеДляРаспределения Тогда
			Возврат;
		КонецЕсли;
		
		Для Каждого ИмяПоказателя Из МассивИменПоказателей Цикл
			СтруктураЗначенийРазницы.Вставить(ИмяПоказателя, Мин(0, СтруктураЗначенийБазы[ИмяПоказателя] + СтруктураЗначенийРаспределения[ИмяПоказателя]));
		КонецЦикла;
		
		Для Каждого ИмяПоказателя Из МассивИменПоказателей Цикл
			МассивЗначений = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточникСписания, ИмяПоказателя);
			МассивНовыхСумм = РаспределитьПропорционально(СтруктураЗначенийРазницы[ИмяПоказателя], МассивЗначений);
			Если МассивНовыхСумм <> Неопределено Тогда
				ИндексСтроки = 0;
				КоличествоСтрок = МассивНовыхСумм.Количество();
				Пока ИндексСтроки < КоличествоСтрок Цикл
					НоваяСтрока = ТаблицаИсточникВременная.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТаблицаИсточникСписания[ИндексСтроки], , СписокКолонок);
					НоваяСтрока[ИмяПоказателя] = МассивНовыхСумм[ИндексСтроки];
					ИндексСтроки = ИндексСтроки + 1;
				КонецЦикла;
			КонецЕсли;
			СтруктураЗначенийРаспределения.Вставить(ИмяПоказателя, СтруктураЗначенийРаспределения[ИмяПоказателя] - СтруктураЗначенийРазницы[ИмяПоказателя]);
			Если СтруктураЗначенийРаспределения[ИмяПоказателя] <> 0 Тогда
				МассивЗначений = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаИсточникПоСтатьям, ИмяПоказателя);
				МассивНовыхСумм = РаспределитьПропорционально(СтруктураЗначенийРаспределения[ИмяПоказателя], МассивЗначений);
				Если МассивНовыхСумм <> Неопределено Тогда
					ИндексСтроки = 0;
					КоличествоСтрок = МассивНовыхСумм.Количество();
					Пока ИндексСтроки < КоличествоСтрок Цикл
						НоваяСтрока = ТаблицаИсточникВременная.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, ТаблицаИсточникПоСтатьям[ИндексСтроки], , СписокКолонок);
						НоваяСтрока[ИмяПоказателя] = ТаблицаИсточникПоСтатьям[ИндексСтроки][ИмяПоказателя] + МассивНовыхСумм[ИндексСтроки];
						ИндексСтроки = ИндексСтроки + 1;
					КонецЦикла;
				КонецЕсли;
			ИначеЕсли СтруктураЗначенийРазницы[ИмяПоказателя] = 0 Тогда
				Для Каждого СтрокаТаблицыИсточника Из ТаблицаИсточникПоСтатьям Цикл
					НоваяСтрока = ТаблицаИсточникВременная.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицыИсточника, , СписокКолонок);
					НоваяСтрока[ИмяПоказателя] = СтрокаТаблицыИсточника[ИмяПоказателя];
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
		
		МасивСтрокКУдалению = Новый Массив;
		Для Каждого СтрокаИсточник Из ТаблицаИсточникВременная Цикл
			
			Если СтрокаИсточник.СуммаРезерва = 0
				И СтрокаИсточник.СуммаРезерваНУ = 0
				И (СтрокаИсточник.СуммаРезерва - СтрокаИсточник.СуммаРезерваНУ) = 0
				И СтрокаИсточник.СуммаРезерваСтраховыхВзносов = 0
				И СтрокаИсточник.СуммаРезерваСтраховыхВзносовНУ = 0
				И (СтрокаИсточник.СуммаРезерваСтраховыхВзносов - СтрокаИсточник.СуммаРезерваСтраховыхВзносовНУ) = 0
				И СтрокаИсточник.СуммаРезерваФССНесчастныеСлучаи = 0
				И СтрокаИсточник.СуммаРезерваФССНесчастныеСлучаиНУ = 0
				И (СтрокаИсточник.СуммаРезерваФССНесчастныеСлучаи - СтрокаИсточник.СуммаРезерваФССНесчастныеСлучаиНУ) = 0 Тогда
				
				МасивСтрокКУдалению.Добавить(СтрокаИсточник);
		...
		Если МасивСтрокКУдалению.Количество() > 0 Тогда
			Для Каждого СтрокаМассива Из МасивСтрокКУдалению Цикл
				ТаблицаИсточникВременная.Удалить(СтрокаМассива);
			КонецЦикла;
		КонецЕсли;
		
		ТаблицаИсточникВременная.Свернуть("МестоВСтруктуреПредприятия, Подразделение, СпособОтраженияЗарплатыВБухУчете, СтатьяФинансирования, ОблагаетсяЕНВД",	
			"СуммаРезерва,СуммаРезерваНУ,СуммаРезерваСтраховыхВзносов,СуммаРезерваСтраховыхВзносовНУ,СуммаРезерваФССНесчастныеСлучаи,СуммаРезерваФССНесчастныеСлучаиНУ");
		
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаИсточникВременная, ТаблицаИсточникНовая);
		
	КонецЦикла;
	
	ТаблицаИсточникНовая.Свернуть("МестоВСтруктуреПредприятия, Подразделение, СпособОтраженияЗарплатыВБухУчете, СтатьяФинансирования, ОблагаетсяЕНВД",	
		"СуммаРезерва,СуммаРезерваНУ,СуммаРезерваСтраховыхВзносов,СуммаРезерваСтраховыхВзносовНУ,СуммаРезерваФССНесчастныеСлучаи,СуммаРезерваФССНесчастныеСлучаиНУ");
	
	ТаблицаИсточник = ТаблицаИсточникНовая.Скопировать();
...
Функция ПолучитьВходящиеОстатки(СтруктураПараметровРасчета, ПараметрыРасчетаОбязательств, ИспользоватьДетализацию = Ложь) Экспорт
	...
	|ПОМЕСТИТЬ ВТ_ОстаткиДляГруппировки
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ОстаткиДляГруппировки.Сотрудник КАК Сотрудник,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваОстаток) КАК СуммаРезерваОстаток,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваСтраховыхВзносовОстаток) КАК СуммаРезерваСтраховыхВзносовОстаток,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваФССНесчастныеСлучаиОстаток) КАК СуммаРезерваФССНесчастныеСлучаиОстаток,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваОстатокНУ) КАК СуммаРезерваОстатокНУ,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваСтраховыхВзносовОстатокНУ) КАК СуммаРезерваСтраховыхВзносовОстатокНУ,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваФССНесчастныеСлучаиОстатокНУ) КАК СуммаРезерваФССНесчастныеСлучаиОстатокНУ,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваОстаток + ВТ_ОстаткиДляГруппировки.СуммаРезерваСтраховыхВзносовОстаток + ВТ_ОстаткиДляГруппировки.СуммаРезерваФССНесчастныеСлучаиОстаток) КАК СуммаРезервовВсегоБУ,
	|	СУММА(ВТ_ОстаткиДляГруппировки.СуммаРезерваОстатокНУ + ВТ_ОстаткиДляГруппировки.СуммаРезерваСтраховыхВзносовОстатокНУ + ВТ_ОстаткиДляГруппировки.СуммаРезерваФССНесчастныеСлучаиОстатокНУ) КАК СуммаРезервовВсегоНУ,
	|	ВТ_ОстаткиДляГруппировки.СтатьяФинансирования КАК СтатьяФинансирования
	|ПОМЕСТИТЬ ВТ_Остатки
	|ИЗ
	|	ВТ_ОстаткиДляГруппировки КАК ВТ_ОстаткиДляГруппировки
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ОстаткиДляГруппировки.Сотрудник,
	|	ВТ_ОстаткиДляГруппировки.СтатьяФинансирования
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ВТ_Остатки.СуммаРезервовВсегоБУ) КАК СуммаРезервовВсегоБУ,
	|	СУММА(ВТ_Остатки.СуммаРезервовВсегоНУ) КАК СуммаРезервовВсегоНУ,
	|	ВТ_Остатки.СтатьяФинансирования КАК СтатьяФинансирования
	|ИЗ
	|	ВТ_Остатки КАК ВТ_Остатки
	|ГДЕ
	|	(ВТ_Остатки.СуммаРезерваОстаток <> 0
	|			ИЛИ ВТ_Остатки.СуммаРезерваОстатокНУ <> 0
	|			ИЛИ ВТ_Остатки.СуммаРезерваСтраховыхВзносовОстаток <> 0
	|			ИЛИ ВТ_Остатки.СуммаРезерваСтраховыхВзносовОстатокНУ <> 0
	|			ИЛИ ВТ_Остатки.СуммаРезерваФССНесчастныеСлучаиОстаток <> 0
	|			ИЛИ ВТ_Остатки.СуммаРезерваФССНесчастныеСлучаиОстатокНУ <> 0)
	|
	|СГРУППИРОВАТЬ ПО
	...
	ДополнитьЗапросТаблицыВходящихОстатков(Запрос);
	
	Результат = Запрос.ВыполнитьПакет();
	РезультатПоСотрудникам = Результат[Результат.Количество()-2].Выгрузить();
	РезультатСводный       = Результат[Результат.Количество()-1].Выгрузить();
...
Функция ТаблицаКорректировокТекущегоМесяца(СтруктураПараметровРасчета, Регистратор) Экспорт
	...
	ДополнитьЗапросТаблицыКорректировок(Запрос);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции

Процедура ДополнитьПеренесеннымиОстатками(ПараметрыРасчетаОбязательств, СтруктураПараметровРасчета) Экспорт
...
Процедура ДополнитьОстаткамиЗачисленных(ОценочныеОбязательстваПоСотрудникам, РасчетРезервов)
	...
	Для Каждого СтрокаТаблицы ИЗ ТаблицаОстатковЗачисленных Цикл
		...
		Если СтрокаТаблицы.СуммаРезерваНакоплено = 0
			И СтрокаТаблицы.СуммаРезерваНакопленоНУ = 0
			И СтрокаТаблицы.СуммаРезерваСтраховыхВзносовНакоплено = 0
			И СтрокаТаблицы.СуммаРезерваСтраховыхВзносовНакопленоНУ = 0
			И СтрокаТаблицы.СуммаРезерваФССНесчастныеСлучаиНакоплено = 0
			И СтрокаТаблицы.СуммаРезерваФССНесчастныеСлучаиНакопленоНУ = 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		НоваяСтрока.СпособОтраженияЗарплатыВБухучете  = СтрокаТаблицы.СпособОтраженияЗарплатыВБухучете;
		НоваяСтрока.СтатьяФинансирования              = СтрокаТаблицы.СтатьяФинансирования;
...
Процедура ДополнитьНедостающимиКолонками(Таблица) Экспорт
	...
	Если Таблица.Колонки.Найти("Перемещен") = Неопределено Тогда
		Таблица.Колонки.Добавить("Перемещен", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	
	Если Таблица.Колонки.Найти("Зарезервирован") = Неопределено Тогда
		Таблица.Колонки.Добавить("Зарезервирован", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	
	Если Таблица.Колонки.Найти("Зачислен") = Неопределено Тогда
		Таблица.Колонки.Добавить("Зачислен", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	
	Если Таблица.Колонки.Найти("Уволен") = Неопределено Тогда
		Таблица.Колонки.Добавить("Уволен", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
...
Процедура ЗаполнитьСпособОтражения(ТаблицаРасчетРезервов, Реквизиты)
	...
	ЭтоКорректировка = Реквизиты.ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Корректировка;
...
Процедура ПереопределитьСпособОтраженияСтатьюРасходов(ТаблицаРасчетРезервов, Организация, Резерв, Период, ВидОперации)
	
	РезервыПоОплатеТрудаВнутренний.ПереопределитьСпособОтраженияСтатьюРасходов(ТаблицаРасчетРезервов, Организация, Резерв, Период, ВидОперации);
	
КонецПроцедуры

Функция ОписаниеТаблицыПриоритетов() Экспорт
	
	ТаблицаПриоритетов = Новый ТаблицаЗначений;
	ТаблицаПриоритетов.Колонки.Добавить("СтатьяФинансирования", Новый ОписаниеТипов("СправочникСсылка.СтатьиФинансированияЗарплата"));
	ТаблицаПриоритетов.Колонки.Добавить("СтатьяФинансированияРезервовПоОплатеТруда", Новый ОписаниеТипов("СправочникСсылка.СтатьиФинансированияЗарплата"));
	ТаблицаПриоритетов.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число"));
	ТаблицаПриоритетов.Колонки.Добавить("Приоритет", Новый ОписаниеТипов("Число"));
...
Функция ДанныеПечатиДокументов(МассивОбъектов)
	...
	|	ВТ_Документ.ЕстьРасшифровкаНачислений КАК ЕстьРасшифровкаНачислений
	|ИЗ
	|	ВТ_Документ КАК ВТ_Документ
	...
	|	МАКСИМУМ(ЕстьРасшифровкаНачислений)
	|ПО
	|	Документ";
	...
	Пока ГруппировкаПоДокументу.Следующий() Цикл
		...
		// Ограничение вывода начислений
		ДанныеПечати.Вставить("ЕстьРасшифровкаНачислений", ГруппировкаПоДокументу.ЕстьРасшифровкаНачислений);
		...
		Пока ГруппировкаПоСотрудникам.Следующий() Цикл
			...
			// Вид отпуска, вид премии и т.п.
			СтрокаДанныхПечати.Вставить("ВидРасчетаРезерва", ГруппировкаПоСотрудникам.ВидРасчетаРезерва);
...
Процедура ДополнитьЗапросТаблицыВходящихОстатков(Запрос)
	
	РезервыПоОплатеТрудаВнутренний.ДополнитьЗапросТаблицыВходящихОстатков(Запрос);
	
КонецПроцедуры

Процедура ДополнитьЗапросТаблицыКорректировок(Запрос)
	
	РезервыПоОплатеТрудаВнутренний.ДополнитьЗапросТаблицыКорректировок(Запрос);
