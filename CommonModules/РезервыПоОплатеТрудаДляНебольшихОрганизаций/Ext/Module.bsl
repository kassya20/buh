...
Процедура ПроверитьЗаполнитьВидРасчетаРезерва(НастройкиРасчетаРезервовПоОплатеТруда, ВидыНачислений) Экспорт
	...
	Если ЭтоРасчетОтпуска Тогда
		...
		ВидыНачислений.Добавить(ПланыВидовРасчета.Начисления.НачислениеПоУмолчанию(Отбор));
	Иначе
		СреднийЗаработокОбщий = Новый Массив;
		СреднийЗаработокОбщий.Добавить(Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяФиксированнойСуммой);
		СреднийЗаработокОбщий.Добавить(Перечисления.УчетНачисленийВСреднемЗаработкеОбщий.ПремияГодоваяПроцентом);
		
		Отбор = Новый Структура();
		Отбор.Вставить("КатегорияНачисленияИлиНеоплаченногоВремени",
			Перечисления.КатегорииНачисленийИНеоплаченногоВремени.Премия);
		Отбор.Вставить("СреднийЗаработокОбщий",
			Новый ФиксированныйМассив(СреднийЗаработокОбщий));
		НачисленияПоОтбору = ПланыВидовРасчета.Начисления.НачисленияПоОтбору(Отбор);
		Если НачисленияПоОтбору.Количество() < 2 Тогда
			УстановитьПривилегированныйРежим(Истина);
			ПланыВидовРасчета.Начисления.СоздатьПремии();
			УстановитьПривилегированныйРежим(Ложь);
			НачисленияПоОтбору = ПланыВидовРасчета.Начисления.НачисленияПоОтбору(Отбор);
		КонецЕсли;
		ВидыНачислений.ЗагрузитьЗначения(НачисленияПоОтбору);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьВидыРасчетовРезервовПоОплатеТруда(НастройкиРасчетаРезервовПоОплатеТруда, ВидыНачислений) Экспорт
	
	ЭтоРасчетОтпуска = НастройкиРасчетаРезервовПоОплатеТруда.ВидРезерва = Перечисления.ВидыРезервовПоОплатеТруда.Отпуск;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидыРасчета",      ВидыНачислений);
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО Начисления.Ссылка = ВидыРасчетовРезервовПоОплатеТруда.ВидРасчета
	               |			И (ВидыРасчетовРезервовПоОплатеТруда.Организация = &Организация)
	               |			И (ВидыРасчетовРезервовПоОплатеТруда.Резерв = &Резерв)
	               |			И (ВидыРасчетовРезервовПоОплатеТруда.ВидРасчета В (&ВидыРасчета))
	...
	Пока Выборка.Следующий() Цикл
		ВидРасчетаРезерва = Выборка.ВидРасчетаРезерва;
		Если Не ЗначениеЗаполнено(ВидРасчетаРезерва) Тогда
			ВидРасчетаРезерва = НовыйВидРасчетаРезерва(Выборка.ВидРасчета,
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.РежимВыполнения = "Монопольно";
	Обработчик.Процедура = "РезервыПоОплатеТрудаДляНебольшихОрганизаций.ОбновитьНастройкуРезервовПремий";
	Обработчик.НачальноеЗаполнение = Истина;
...
Процедура ОбновитьНастройкуРезервовПремий() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиРасчетаРезервовПоОплатеТрудаВидыРасчетовРезерва.Ссылка КАК Ссылка
	|ИЗ
	...
	|ГДЕ
	|	НастройкиРасчетаРезервовПоОплатеТрудаВидыРасчетовРезерва.Ссылка.ВидРезерва = ЗНАЧЕНИЕ(Перечисление.ВидыРезервовПоОплатеТруда.ГодоваяПремия)
	|
	|СГРУППИРОВАТЬ ПО
	|	НастройкиРасчетаРезервовПоОплатеТрудаВидыРасчетовРезерва.Ссылка
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(НастройкиРасчетаРезервовПоОплатеТрудаВидыРасчетовРезерва.ВидРасчетаРезерва) = 1";
	...
	Пока Выборка.Следующий() Цикл
		...
		ПроверитьЗаполнитьВидРасчетаРезерва(СправочникОбъект, ВидыНачислений);
		СохраненныеВидыРасчетовРезерва = ОбщегоНазначения.ВыгрузитьКолонку(СправочникОбъект.ВидыРасчетовРезерва, "ВидРасчетаРезерва", Истина);
		НачисленияСохраненныхВидовРасчета = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СохраненныеВидыРасчетовРезерва, "ВидРасчета");
		Для Каждого ВидНачисления Из НачисленияСохраненныхВидовРасчета Цикл
			Если ВидыНачислений.НайтиПоЗначению(ВидНачисления.Значение) = Неопределено Тогда
				ВидыНачислений.Добавить(ВидНачисления.Значение);
			КонецЕсли;
		КонецЦикла;
		
		УстановитьВидыРасчетовРезервовПоОплатеТруда(СправочникОбъект, ВидыНачислений);
...
Функция ДанныеДляЗаполненияТаблицДокумента(ОписаниеДокумента, Организация, Резерв, МесяцНачисления, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Сотрудники = Неопределено Тогда
		...
		УчитыватьУволенных = Истина;
		Если ДопПараметрыОтбора <> Неопределено Тогда
			УточнитьСписокСотрудников(Организация, СотрудникиДляНачислений, ДопПараметрыОтбора.Период, ДопПараметрыОтбора.ИмяПоказателяДопОтбора);
			УчитыватьУволенных = ДопПараметрыОтбора.ИмяПоказателяДопОтбора = "Уволен";
		КонецЕсли;
		Если УчитыватьУволенных
			И (ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Списание
			Или ВидОперации = Перечисления.ВидыОперацийРезервовПоОплатеТруда.Корректировка) Тогда
			ДополнитьСписокСотрудниковРанеееУволенными(СотрудникиДляНачислений, Организация, Подразделение, Резерв, ОкончаниеПериода, ДокументСсылка);
		КонецЕсли;
...
Процедура ДополнитьСписокСотрудниковРанеееУволенными(СотрудникиДляНачислений, Организация, Подразделение, Резерв, Период, ДокументСсылка) Экспорт
	...
	Запрос.УстановитьПараметр("Сотрудники",     ОбщегоНазначения.ВыгрузитьКолонку(СотрудникиДляНачислений, "Сотрудник", Истина));
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Для Каждого СтрокаТаблицы Из КадровыеДанныеСотрудников Цикл
		...
		Если ЗначениеЗаполнено(Подразделение)
			...
			Продолжить;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(СотрудникиДляНачислений.Добавить(), СтрокаТаблицы);
	КонецЦикла;
...
Функция СоздатьНастройкуРезерваПоВиду(Организация, ВидРезерва, Период) Экспорт
	...
	ВидыНачислений = Новый СписокЗначений;
	ПроверитьЗаполнитьВидРасчетаРезерва(Настройка, ВидыНачислений);
	УстановитьВидыРасчетовРезервовПоОплатеТруда(Настройка, ВидыНачислений);
