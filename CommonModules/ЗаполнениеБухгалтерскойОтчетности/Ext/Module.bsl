...
Функция ПараметрыАвтозаполненияБухОтчетности(
	...
	Если ПараметрыОтчета.Свойство("ПрименениеЕНВД") Тогда
		ПрименениеЕНВД = ПлательщикЕНВД(Организация, КонецПериодаОтчета);
		ПараметрыОтчета.ПрименениеЕНВД = ПрименениеЕНВД;
	КонецЕсли;
...
#Область НастройкиРасшифровкиПоказателейБаланса

Функция НастройкиРасшифровкиСправкиРасчетаКлассификацииЗадолженности(ОбщиеНастройки, ИмяОтчета, Отборы = Неопределено) Экспорт

	НастройкиРасшифровки = Новый Структура;

	ВключатьОбособленныеПодразделения = Ложь;
	Если ОбщиеНастройки.Свойство("ВключатьОбособленныеПодразделения") Тогда
		ВключатьОбособленныеПодразделения = ОбщиеНастройки.ВключатьОбособленныеПодразделения;
	КонецЕсли;

	Если ОбщиеНастройки.Свойство("ВидПериода") Тогда
		ВидПериода = ОбщиеНастройки.ВидПериода;
	Иначе
		ВидПериода = ВидПериодаСтандартногоОтчетаСМОП(ОбщиеНастройки.НачалоПериода,
			ОбщиеНастройки.КонецПериода);
	КонецЕсли;
		
	НастройкиРасшифровки.Вставить("Отчет", Новый Структура);
	НастройкиРасшифровки.Отчет.Вставить("ИмяОтчета", ИмяОтчета);
	...
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("Организация",   ОбщиеНастройки.Организация);
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("НачалоПериода", НачалоМесяца(ПривестиКДате(ОбщиеНастройки.КонецПериода)));
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("КонецПериода",  ПривестиКДате(ОбщиеНастройки.КонецПериода));
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ВидПериода",    ВидПериода);
	ПользовательскиеНастройки.ДополнительныеСвойства.Вставить("ВключатьОбособленныеПодразделения", ВключатьОбособленныеПодразделения);
	...
	Если Отборы <> Неопределено Тогда
		...
		Для Каждого ЭлементОтбора Из Отборы Цикл
			...
			ЭлементОтбора.Значение.ВидСравненияКомпоновкиДанных);
		КонецЦикла;
	КонецЕсли;
	
	НастройкиРасшифровки.Отчет.ПараметрыФормы.Вставить("ПользовательскиеНастройки", ПользовательскиеНастройки);
...
Функция ОписаниеОбъектовВПДР(ИмяВидаПрочихДоходовИРасходов) Экспорт
	...
	Возврат "/объекты со значением реквизита ""Вид прочих доходов и расходов"" - """
	        + Строка(Перечисления.ВидыПрочихДоходовИРасходов[ИмяВидаПрочихДоходовИРасходов]) + """/";
...
Функция СчетаИзСписка(Знач СписокСчетов, ИмяСпискаСчетов = Неопределено) Экспорт
	...
	КодыСчетов = СтрРазделить(СписокСчетов, ",", Ложь);
	Для Каждого КодСчета Из КодыСчетов Цикл
		Если ЗначениеЗаполнено(КодСчета) Тогда
			МассивСчетов.Добавить(Счет(СокрЛП(КодСчета)));
		КонецЕсли;
	КонецЦикла;
...
Функция ПрименяетсяАУСН(Организация, КонецПериода) Экспорт
	
	ПрименяетсяАУСН = Ложь;
	
	ЗаполнениеБухгалтерскойОтчетностиПереопределяемый.ОпределитьУчетнуюПолитикуПрименяетсяАУСН(
		ПрименяетсяАУСН, Организация, КонецПериода);
...
Функция ПрименяетсяНПД(Организация, КонецПериода) Экспорт
	...
	ЗаполнениеБухгалтерскойОтчетностиПереопределяемый.ОпределитьУчетнуюПолитикуПрименяетсяНПД(
		ПрименяетсяНПД, Организация, КонецПериода);
	
	Возврат ПрименяетсяНПД;
	
КонецФункции

Функция ПлательщикЕНВД(Организация, КонецПериода) Экспорт
	
	ПлательщикЕНВД = Ложь;
	
	ЗаполнениеБухгалтерскойОтчетностиПереопределяемый.ОпределитьУчетнуюПолитикуПлательщикЕНВД(
		ПлательщикЕНВД, Организация, КонецПериода);
	
	Возврат ПлательщикЕНВД;
	
КонецФункции

Функция СпособОценкиТоваровВРознице(Организация, КонецПериода) Экспорт
	
	СпособОценкиТоваровВРознице = Ложь;
	
	ЗаполнениеБухгалтерскойОтчетностиПереопределяемый.ОпределитьУчетнуюПолитикуСпособОценкиТоваровВРознице(
		СпособОценкиТоваровВРознице, Организация, КонецПериода);
	
	Возврат СпособОценкиТоваровВРознице;
