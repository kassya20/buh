...
Процедура СформироватьДвиженияПоступлениеТоваровУслугОтПодотчетногоЛица(ТаблицаТовары, ТаблицаУслуги, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Не УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Реквизиты.Организация, Реквизиты.Период)
		И Не УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодготовитьПараметрыПоступлениеТоваровУслугОтПодотчетногоЛица(Параметры, ТаблицаТовары, ТаблицаУслуги)
	...
	ЕстьМалоценка = ТаблицаТовары <> Неопределено И ТаблицаТовары.Колонки.Найти("Малоценка") <> Неопределено;
	Если ЕстьМалоценка Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Малоценка";
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",СчетЗатрат";
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",ПодразделениеЗатрат";
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Субконто1";
...
Функция ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПодотчетногоЛица(Товары, Услуги, Реквизиты)
	...
	СписокКолонок = "СчетУчета,Подразделение,Номенклатура,Склад,Партия,Контрагент,ПредъявленСчетФактура,
		|СчетФактура,ВидЦенности,СчетУчетаНДС,СтавкаНДС,СпособУчетаНДС,
		|Количество,СуммаБезНДСРуб,СуммаНДСРуб";
	
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "Малоценка");
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "СчетЗатрат");
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "ПодразделениеЗатрат");
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "Субконто1");
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "Субконто2");
	ДобавитьНеобязательнуюКолонкуВСписок(Товары, СписокКолонок, "Субконто3");
	
	ДанныеДвижений = Товары.Скопировать(,СписокКолонок);
	
	Если Товары.Колонки.Найти("Малоценка") <> Неопределено Тогда
		ДанныеДвижений.Колонки.СчетЗатрат.Имя          = "МалоценкаСчетЗатрат";
		ДанныеДвижений.Колонки.ПодразделениеЗатрат.Имя = "МалоценкаПодразделение";
		ДанныеДвижений.Колонки.Субконто1.Имя = "МалоценкаСубконто1";
		ДанныеДвижений.Колонки.Субконто2.Имя = "МалоценкаСубконто2";
		ДанныеДвижений.Колонки.Субконто3.Имя = "МалоценкаСубконто3";
	КонецЕсли;
...
Функция ПолучитьТаблицуПартийПоРегиструНДСРаздельныйУчет(Реквизиты)
	...
	|	ВЫБОР
	|		КОГДА Аналитика.СчетЗатрат В (&Счета45)
	|			ТОГДА Аналитика.Субконто2
	|		ИНАЧЕ Аналитика.Субконто1
	...
	Для каждого СтрокаТаблицы Из ДанныеДвижений Цикл
		...
		Для Ном = 1 По СвойстваСчета.КоличествоСубконто Цикл
			...
			ИначеЕсли СвойстваСчета["ВидСубконто" + Ном] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты Тогда
				Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.ТоварыНаСкладКомиссионера Тогда
					СтрокаТаблицы["Субконто" + Ном] = Реквизиты.Комиссионер;
				Иначе
...
//   ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу - ТаблицаЗначений - остатки в РН РаздельныйУчетНДС,
//       если возврат в Отчете о розничных продажах по текущему документу, тогда для возврата требуются другие остатки
//       если движения формируются не для Отчета о розничных продажах, тогда значение Неопределено 
Процедура СформироватьДвиженияПартииРаздельныйУчет(ТаблицаПартий, ТаблицаВозвращаемыеТовары, Реквизиты, Движения, ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу = Неопределено)
	...
	Если ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу <> Неопределено Тогда
		ТаблицаПартийТоварыПоТекущемуДокументу     = 
			ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу.Скопировать(Новый Структура("ЕстьКоличество", Истина));
		ТаблицаПартийДопРасходыПоТекущемуДокументу = 
			ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу.Скопировать(Новый Структура("ЕстьКоличество", Ложь));
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВозвращаемыеТовары Цикл
		...
		Если ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу <> Неопределено
			И СтрокаТаблицы.ВозвратПоТекущемуДокументу Тогда
			ПартииСписания = ТаблицаПартийТоварыПоТекущемуДокументу.НайтиСтроки(СтруктураПоиска);
		Иначе
		...
		Для Каждого ПартияСписания Из ПартииСписания Цикл
			...
			Если ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу <> Неопределено
				И СтрокаТаблицы.ВозвратПоТекущемуДокументу Тогда
				ПартииДопРасходов = ТаблицаПартийДопРасходыПоТекущемуДокументу.НайтиСтроки(СтруктураПоиска);
			Иначе
...
Процедура СформироватьДвиженияВозвратТоваровВРознице(ТаблицаВозвращаемыеТовары, ТаблицаПартииВозвратовРаздельныйУчет, Реквизиты, Движения, Отказ, ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу = Неопределено) Экспорт
	...
	Если ТаблицаПартииВозвратовРаздельныйУчет.Количество() <> 0 
		Или (ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу <> Неопределено И ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу.Количество() <> 0)Тогда
		СформироватьДвиженияПартииРаздельныйУчет(
			ТаблицаПартииВозвратовРаздельныйУчет, 
			ТаблицаВозвращаемыеТовары, 
			Реквизиты, 
			Движения, 
			ВозвратТоваровРаздельныйУчетНДСПоТекущемуДокументу);
	КонецЕсли; 
...
Процедура СформироватьДвиженияУплатаНДСнаТаможне(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Не УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Реквизиты.Организация, Реквизиты.Период)
		И Не РаздельныйУчетНДСБезВычетов Тогда
		Возврат;
	КонецЕсли;
	...
	ДанныеДвижений = ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПоставщика(
		Параметры.Товары, Неопределено, Реквизиты);
...
Процедура СформироватьДвиженияКомплектацияТоваров(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Если Не (УчетнаяПолитика.ПлательщикНДС(Реквизиты.Организация, Реквизиты.Период)
		...
		Или (Не УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Реквизиты.Организация, Реквизиты.Период)
		И Не УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период)) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьТаблицуДляИзмененияСпособаУчетаНДСКомплектацияТоваров(СписанныеПартииНДС, Реквизиты)
	...
	Для Каждого СтрокаТаблицы Из СписанныеПартииНДС Цикл
		...
		Если РаздельныйУчетНДСБезВычетов Тогда
			НоваяСтрока.НовыйСпособУчетаНДС     = Перечисления.СпособыУчетаНДС.ПустаяСсылка(); 
			НоваяСтрока.СпособУчетаНДСИзменился = Ложь;
		КонецЕсли;
...
Функция ПодготовитьТаблицуДляИзмененияСпособаУчетаНДСРазукомплектацияТоваров(Комплектующие, СписанныеПартииНДС, Реквизиты)
	...
	"СуммаБезНДС,НДС,СуммаБезНДСПринятоКВычету,НДСПринятоКВычету, БлокироватьДоПодтвержденияСтавки0");
	...
	Для каждого СтрокаТаблицы Из СписанныеПартииНДС Цикл
		...
		Если СуммаБезНДС > 0 ИЛИ НДС > 0 Тогда
			...
			ТаблицаЦенностейСуммы.ЗаполнитьЗначения(СтрокаТаблицы.БлокироватьДоПодтвержденияСтавки0, "БлокироватьДоПодтвержденияСтавки0");
	...
	БлокируетсяДляПодтверждения0 = Перечисления.СпособыУчетаНДС.ДляОперацийПо0;
	Для Каждого СтрокаТаблицы Из ТаблицаЦенностей Цикл
		Если СтрокаТаблицы.НовыйСпособУчетаНДС = БлокируетсяДляПодтверждения0 Тогда
			СтрокаТаблицы.БлокироватьДоПодтвержденияСтавки0 = Истина;
		КонецЕсли;
...
Процедура ОтразитьКорректировкуПоступленияВКнигеПродаж(Реквизиты, СтруктураТаблицДокумента, Движения, Отказ)
	...
	Если Не Реквизиты.ЕстьНДС Тогда
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТаблицы Из ТаблицаУменьшениеВычета Цикл
		...
		Если НДС > 0 
		 ИЛИ СуммаБезНДС > 0 Тогда
			...
			Если Реквизиты.ЭтоКорректировкаИсправления Тогда
				НоваяСтрока.ИсправленныйСчетФактура = Реквизиты.КорректируемыйДокументИсправления;
			КонецЕсли;
			НоваяСтрока.СуммаБезНДС = -СуммаБезНДС;
			НоваяСтрока.НДС = -НДС;
...
Процедура ДобавитьЗаписьВНДСРаздельныйУчет(ДвиженияНДСРаздельныйУчет, СтрокаТаблицы, Реквизиты, НДС, СуммаБезНДС, Количество, СтавкаНДСДоИзменения = Ложь)
	...
	Иначе
		НоваяСтрока.Партия              =
		?(СуммаБезНДС < 0 Или Количество < 0 Или НДС < 0, Реквизиты.ДокументПоступленияСсылка, Реквизиты.Регистратор);
	КонецЕсли;
...
Процедура СформироватьДвиженияКорректировкиОтгрузкаБезПереходаПраваСобственности(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	
	Если Не (ЗначениеЗаполнено(ТаблицаТовары) Или ЗначениеЗаполнено(ТаблицаСписанныеТоварыБухУчет))
		Или Не ЗначениеЗаполнено(ТаблицаРеквизиты) Тогда
		Возврат;
	КонецЕсли;
	...
	Параметры = ПодготовитьПараметрыКорректировкиОтгрузкаБезПереходаПраваСобственности(
		ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
...
Функция ПодготовитьПараметрыКорректировкиОтгрузкаБезПереходаПраваСобственности(ТаблицаТовары, ТаблицаСписанныеТоварыБухУчет, ТаблицаРеквизиты)
...
Процедура СформироватьДвиженияПоступлениеДопРасходовОтПоставщика(ТаблицаТовары, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ДанныеДвижений = ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПоставщика(
		Параметры.Товары, Услуги, Реквизиты);
	СформироватьДвиженияПоступлениеЦенностей(ДанныеДвижений, Реквизиты, Движения, Отказ);
...
Функция ПодготовитьДанныеДвиженийПоступлениеНематериальныхАктивов(НематериальныеАктивы, Реквизиты)
	...
	ДанныеДвижений = НематериальныеАктивы.Скопировать(,
		...
		|ПодразделениеЗатрат, Субконто1, Субконто2, Субконто3, Регистратор,Партия,СуммаБезНДСРуб,СуммаНДСРуб");
	ДанныеДвижений.Свернуть(
		"СчетУчета,НематериальныйАктив,СчетФактура,ВидЦенности,СтавкаНДС,СчетУчетаНДС,СпособУчетаНДС,
		|ПодразделениеЗатрат, Субконто1, Субконто2, Субконто3, Регистратор,Партия",
...
Процедура ПодготовитьПараметрыПоступлениеТоваровУслугОтПоставщика(Параметры, ТаблицаТовары, ТаблицаУслуги)
	...
	Если ТаблицаТовары <> Неопределено И НЕ ТаблицаТовары.Колонки.Найти("Склад") = Неопределено Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ",Склад";
	КонецЕсли;
	...
	//Склад берем Из шапки документа поступления если склада нет в табличной части
	Если Параметры.Товары.Колонки.Найти("Склад") = Неопределено Тогда
		Параметры.Товары.Колонки.Добавить("Склад", Новый ОписаниеТипов("СправочникСсылка.Склады"));
		Параметры.Товары.ЗаполнитьЗначения(Реквизиты.Склад, "Склад");
	КонецЕсли;
	// Подразделение берем из шапки документа поступления
	...
	Если ТаблицаУслуги <> Неопределено Тогда
		ЕстьРасхождения = ТаблицаУслуги.Колонки.Найти("СуммаПодтвержденнаяРуб") <> Неопределено
	...
	ДобавитьНеобязательныеКолонки(ТаблицаУслуги, СписокОбязательныхКолонок, НеобязательныеКолонкиПриобретениеПрав());
...
Процедура ДобавитьНеобязательныеКолонки(Таблица, СписокОбязательныхКолонок, НеобязательныеКолонки)
	
	Если Таблица = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого НеобязательнаяКолонка Из НеобязательныеКолонки Цикл
		
		Если Таблица.Колонки.Найти(НеобязательнаяКолонка) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + "," + НеобязательнаяКолонка;

	КонецЦикла;
	
КонецПроцедуры

Функция НеобязательныеКолонкиПриобретениеПрав()

	НеобязательныеКолонки = Новый Массив;
	НеобязательныеКолонки.Добавить("СчетОтражениеБУ");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеБУ1");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеБУ2");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеБУ3");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеБУ1");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеБУ2");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеБУ3");
	НеобязательныеКолонки.Добавить("СчетОтражениеНУ");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеНУ1");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеНУ2");
	НеобязательныеКолонки.Добавить("ВидСубконтоОтражениеНУ3");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеНУ1");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеНУ2");
	НеобязательныеКолонки.Добавить("СубконтоОтражениеНУ3");
	НеобязательныеКолонки.Добавить("ЭтоМалоценныйНМА");
	НеобязательныеКолонки.Добавить("ПрименяетсяПовышающийКоэффициент");
	НеобязательныеКолонки.Добавить("ДатаНачалаСписания");
	Возврат НеобязательныеКолонки;
КонецФункции
...
Функция ПодготовитьДанныеДвиженийПоступлениеТоваровУслугОтПоставщика(Товары, Услуги, Реквизиты)
	...
	Если Товары <> Неопределено Тогда
		...
		ДанныеДвижений.Колонки.Удалить("Номенклатура");
		Если Не БухгалтерскийУчетПереопределяемый.СкладВТабличнойЧасти(Реквизиты) Тогда
			ДанныеДвижений.Колонки.Удалить("Склад");
		КонецЕсли;
		...
		Если Услуги <> Неопределено Тогда
			...
			КонецЕсли;
			ДобавитьКолонкиПриобретениеПрав(ДанныеДвижений);
			ОбщегоНазначенияБПВызовСервера.ЗагрузитьВТаблицуЗначений(Услуги, ДанныеДвижений);
		КонецЕсли;
	Иначе
	...
	ДанныеДвижений.Колонки.Добавить("СпособУчетаНДСИзменился",
		Новый ОписаниеТипов("Булево"));
	ДанныеДвижений.ЗаполнитьЗначения(НЕ РаздельныйУчетНДСБезВычетов, "СпособУчетаНДСИзменился");
...
Процедура ДобавитьКолонкиПриобретениеПрав(ДанныеДвижений)
	
	ДанныеДвижений.Колонки.Добавить("СчетОтражениеБУ");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеБУ1");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеБУ2");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеБУ3");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеБУ1");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеБУ2");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеБУ3");
	ДанныеДвижений.Колонки.Добавить("СчетОтражениеНУ");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеНУ1");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеНУ2");
	ДанныеДвижений.Колонки.Добавить("ВидСубконтоОтражениеНУ3");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеНУ1");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеНУ2");
	ДанныеДвижений.Колонки.Добавить("СубконтоОтражениеНУ3");
	ДанныеДвижений.Колонки.Добавить("ЭтоМалоценныйНМА");
	ДанныеДвижений.Колонки.Добавить("ПрименяетсяПовышающийКоэффициент");
	ДанныеДвижений.Колонки.Добавить("ДатаНачалаСписания");
...
Процедура СформироватьДвиженияРаспределениеНДС(ТаблицаРеквизиты, ТаблицаРаспределяемыйНДС, ТаблицаВключенныйВСтоимостьНДС, Движения, Отказ) Экспорт
	...
	Если Не (УчетнаяПолитика.ПлательщикНДС(Реквизиты.Организация, Реквизиты.Период)
		ИЛИ УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Реквизиты.Организация, Реквизиты.Период)) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПодготовитьДанныеПроводокВключениеВСтоимостьРаспределениеНДС(ПартииНДС, Реквизиты)
	...
	ДанныеПроводокПрочиеЦенности = ПартииНДС.СкопироватьКолонки("
		|Организация,Период,Партия,
		|СчетЗатрат,Подразделение,Субконто1,Субконто2,Субконто3,
		|СчетУчетаНДС,СчетФактура,Поставщик,
		|НДС,Модернизация");
	
	ДанныеПроводокПрочиеЦенности.Колонки.Добавить("Содержание",
		Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(150)));
	ДанныеПроводокПрочиеЦенности.Колонки.Добавить("НовыйСпособУчетаНДС",
		Новый ОписаниеТипов("ПеречислениеСсылка.СпособыУчетаНДС"));
	...
	Если ДанныеПроводокПрочиеЦенности.Количество() > 0 Тогда
		...
		ДобавитьАналитикуСписанияМалоценки(ДанныеПроводокПрочиеЦенности, Реквизиты);
...
Процедура СформироватьДвиженияКурсовыеРазницыНалоговыйАгент(ТаблицаРеквизитов, ТаблицаВзаиморасчетов, Движения, Отказ) Экспорт
	...
	Если Не ЗначениеЗаполнено(ТаблицаРеквизитов) ИЛИ Не ЗначениеЗаполнено(ТаблицаВзаиморасчетов) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИзменениеАналитикиПоПартиямПриПереходеСФИФОНаУчетПоСредней(Реквизиты, Движения, Отказ) Экспорт
	
	Если Реквизиты = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	РаздельныйУчетНДСНаСчете19 = УчетнаяПолитика.РаздельныйУчетНДСНаСчете19(Реквизиты.Организация, Реквизиты.Дата)
	 ИЛИ УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Дата);
	Если Не РаздельныйУчетНДСНасчете19 Тогда
		Возврат;
	КонецЕсли;
	
	СчетаЗапасов = БухгалтерскийУчет.СчетаОчисткиПартийПриИзмененииСпособаУчетаСебестоимости();
	...
	|ПОМЕСТИТЬ ДанныеОСчетах
	|ИЗ
	|	ПланСчетов.Хозрасчетный.ВидыСубконто КАК ХозрасчетныйВидыСубконто
	|ГДЕ
	|	ХозрасчетныйВидыСубконто.Ссылка В(&СчетаЗапасов)
	...
	|	НДСРаздельныйУчетОстатки.СуммаБезНДСОстаток КАК СуммаБезНДС,
	|	НДСРаздельныйУчетОстатки.НДСОстаток КАК НДС
	|ПОМЕСТИТЬ ОстаткиПоРУ
	|ИЗ
	|	РегистрНакопления.НДСРаздельныйУчет.Остатки(&ГраницаОстатков, Организация = &Организация) КАК НДСРаздельныйУчетОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АналитикаЗатрат.СчетЗатрат КАК СчетЗатрат,
	|	АналитикаЗатрат.Подразделение КАК Подразделение,
	|	АналитикаЗатрат.Субконто1 КАК Субконто1,
	|	АналитикаЗатрат.Субконто2 КАК Субконто2,
	|	АналитикаЗатрат.Субконто3 КАК Субконто3,
	|	АналитикаЗатрат.КлючАналитики КАК КлючАналитики,
	|	ДанныеОСчетах.НомерСтроки КАК НомерСубконто
	|ПОМЕСТИТЬ ДанныеПоАналитике
	|ИЗ
	|	РегистрСведений.АналитикаУчетаЗатрат КАК АналитикаЗатрат
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеОСчетах КАК ДанныеОСчетах
	|		ПО АналитикаЗатрат.СчетЗатрат = ДанныеОСчетах.Ссылка
	|ГДЕ
	|	АналитикаЗатрат.Организация = &Организация
	|	И АналитикаЗатрат.СчетЗатрат В(&СчетаЗапасов)
	|	И ВЫБОР
	|			КОГДА ДанныеОСчетах.НомерСтроки = 1
	|				ТОГДА АналитикаЗатрат.Субконто1 <> НЕОПРЕДЕЛЕНО
	|			КОГДА ДанныеОСчетах.НомерСтроки = 2
	|				ТОГДА АналитикаЗатрат.Субконто2 <> НЕОПРЕДЕЛЕНО
	|			КОГДА ДанныеОСчетах.НомерСтроки = 3
	|				ТОГДА АналитикаЗатрат.Субконто3 <> НЕОПРЕДЕЛЕНО
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	КлючАналитики
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеПоАналитике.СчетЗатрат КАК СчетЗатрат,
	|	ДанныеПоАналитике.Подразделение КАК Подразделение,
	|	ДанныеПоАналитике.Субконто1 КАК Субконто1,
	|	ДанныеПоАналитике.Субконто2 КАК Субконто2,
	|	ДанныеПоАналитике.Субконто3 КАК Субконто3,
	|	ДанныеПоАналитике.НомерСубконто КАК НомерСубконто,
	|	ОстаткиПоРУ.Организация КАК Организация,
	|	ОстаткиПоРУ.АналитикаУчетаЗатрат КАК АналитикаУчетаЗатрат,
	|	ОстаткиПоРУ.АналитикаУчетаНДС КАК АналитикаУчетаНДС,
	|	ОстаткиПоРУ.Партия КАК Партия,
	|	ОстаткиПоРУ.СпособУчетаНДС КАК СпособУчетаНДС,
	|	ОстаткиПоРУ.Количество КАК Количество,
	|	ОстаткиПоРУ.СуммаБезНДС КАК СуммаБезНДС,
	|	ОстаткиПоРУ.НДС КАК НДС
	|ИЗ
	|	ОстаткиПоРУ КАК ОстаткиПоРУ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПоАналитике КАК ДанныеПоАналитике
	|		ПО ОстаткиПоРУ.АналитикаУчетаЗатрат = ДанныеПоАналитике.КлючАналитики";
	
	Запрос.УстановитьПараметр("СчетаЗапасов", 					СчетаЗапасов);
	Запрос.УстановитьПараметр("ГраницаОстатков", 				Реквизиты.КонГраница);
	Запрос.УстановитьПараметр("Организация", 					Реквизиты.Организация);
	...
	Пока Выборка.Следующий() Цикл
		Проводка = Движения.НДСРаздельныйУчет.Добавить();
		...
		Проводка.Период      = Реквизиты.Дата;
		ЗаполнитьЗначенияСвойств(Проводка, Выборка);
		ЗаполнитьЗначенияСвойств(СтруктураАналитикУчетаЗатрат, Выборка);
		Если Выборка.НомерСубконто = 3 Тогда
			СтруктураАналитикУчетаЗатрат.Субконто3 = Неопределено;
		ИначеЕсли Выборка.НомерСубконто = 2 Тогда
			СтруктураАналитикУчетаЗатрат.Субконто2 = Неопределено;
		Иначе
			СтруктураАналитикУчетаЗатрат.Субконто1 = Неопределено;
		КонецЕсли;
		Проводка.АналитикаУчетаЗатрат = Справочники.КлючиАналитикиУчетаЗатрат.КлючиАналитикиУчетаЗатратДокумента(СтруктураАналитикУчетаЗатрат);
	КонецЦикла;
...
Процедура СформироватьДвиженияПоступлениеЦенностей(ДанныеДвижений, Реквизиты, Движения, Отказ)
	...
	Если ЕстьВключениеВСтоимость Тогда
		...
		ИначеЕсли ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
			И Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Права Тогда
			СформироватьПроводкиВключениеНДСВСтоимостьПриобретениеПрав(ТаблицаВключениеВСтоимость, Реквизиты, Движения);
		Иначе
...
Функция ПодготовитьДанныеПроводокПоступлениеЦенностей(ДанныеПроводок, Реквизиты)
	...
	СписокКолонок = "Организация,Период,
		|СчетЗатрат,Подразделение,Субконто1,Субконто2,Субконто3,
		|СчетУчетаНДС,СчетФактура,Поставщик,НовыйСпособУчетаНДС";
	
	Если ДанныеПроводок.Колонки.Найти("Малоценка") <> Неопределено Тогда
		СписокКолонок = СписокКолонок + ",Малоценка,МалоценкаСчетЗатрат,
		|МалоценкаПодразделение,МалоценкаСубконто1,МалоценкаСубконто2,МалоценкаСубконто3";
	Иначе
		СтрокиМалоценка = ДанныеПроводок.НайтиСтроки(Новый Структура("СчетЗатрат", ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов));
		Если СтрокиМалоценка.Количество() > 0 Тогда
			ПолучитьАналитикуСписанияНДСПоМалоценке(ДанныеПроводок, Реквизиты);
			СписокКолонок = СписокКолонок + ",Малоценка,МалоценкаСчетЗатрат,
			|МалоценкаПодразделение,МалоценкаСубконто1,МалоценкаСубконто2,МалоценкаСубконто3";
		КонецЕсли;
	КонецЕсли;
	
	ДанныеПроводок.Свернуть(СписокКолонок, "НДС");
...
Функция ПодготовитьДанныеДвиженийНДСПредъявленныйРеализация0ПоступлениеЦенностей(ДанныеДвижений, Реквизиты)
	...
	Если ДанныеДвиженийНДСПредъявленныйРеализация0.Колонки.Найти("ИсправленныйСчетФактура") = Неопределено Тогда
		ДанныеДвиженийНДСПредъявленныйРеализация0.Колонки.Добавить("ИсправленныйСчетФактура");
	КонецЕсли;
...
Функция ПодготовитьДанныеДвиженийНДСРаздельныйУчетПоступлениеЦенностей(ДанныеДвижений, Реквизиты)
	...
	ЭтоПриобретениеПрав =  ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
		И Реквизиты.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Права;
	...
	Для Каждого СтрокаТаблицы Из ДанныеДвижений Цикл
		...
		//СкладВСписке
		Если БухгалтерскийУчетПереопределяемый.СкладВТабличнойЧасти(Реквизиты) Тогда
			Если СтрокаТаблицы.Склад <> Неопределено Тогда
				ПолучательНТТПоПродажнойСтоимости =  
					ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы.Склад, "ТипСклада") = Перечисления.ТипыСкладов.НеавтоматизированнаяТорговаяТочка;
			Иначе
				ПолучательНТТПоПродажнойСтоимости = Ложь;	
			КонецЕсли;
		КонецЕсли;
		...
		Если ЭтоПриобретениеПрав И НачалоКвартала(СтрокаТаблицы.ДатаНачалаСписания) > НачалоКвартала(Реквизиты.Период)
			И (СтрокаТаблицы.СчетОтражениеБУ = ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов)
			И (СтрокаТаблицы.СчетОтражениеНУ = ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов)
			И Не СтрокаТаблицы.ЭтоМалоценныйНМА Тогда
			НоваяСтрока.СчетЗатрат = СтрокаТаблицы.СчетОтражениеБУ;
			НоваяСтрока.Субконто1 = СтрокаТаблицы.СубконтоОтражениеБУ1;
			НоваяСтрока.Субконто2 = СтрокаТаблицы.СубконтоОтражениеБУ2;
			НоваяСтрока.Субконто3 = СтрокаТаблицы.СубконтоОтражениеБУ3;
		КонецЕсли;
		...
		Если РаздельныйУчетНДСБезВычетов Тогда
			НоваяСтрока.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПустаяСсылка();
		ИначеЕсли СтрокаТаблицы.СпособУчетаНДСИзменился Тогда
			НоваяСтрока.СпособУчетаНДС = СтрокаТаблицы.НовыйСпособУчетаНДС;
		Иначе
			НоваяСтрока.СпособУчетаНДС = СтрокаТаблицы.СпособУчетаНДС;
		КонецЕсли;
...
Процедура СформироватьДвиженияВыбытиеТоваров(СписанныеПартииНДС, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ) Экспорт
	...
	Если ЕстьИсключениеИзСтоимости Тогда
		...
		Иначе
			ДобавитьАналитикуСписанияМалоценки(СписанныеПартииНДСИсключениеИзСтоимости, Реквизиты);
	...
	Если Не РаздельныйУчетНДСБезВычетов Тогда 
	...
	Если НЕ РаздельныйУчетНДСБезВычетов
		И ЕстьРаспределяемыеПартииНДС Тогда
...
Функция НовыйТаблицаДокументыПоНеоблагаемымНДСОперациям()
	...
	ТаблицаДокументыПоНеоблагаемымНДСОперациям.Колонки.Добавить("ТипПодтверждающегоДокумента", 
		Новый ОписаниеТипов("СправочникСсылка.ТипыДокументов"));
...
Процедура ДобавитьАналитикуСписанияМалоценки(ДанныеПроводок, Реквизиты)
...
Функция ПодтверждающиеДокументыПоРегистратору(ОперацииКПодтверждению, ДокументРегистратор)
	...
	Если ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
		...
		Если ОбменСКонтрагентами.ДокументооборотНастроен(ДокументРегистратор)
			...
			Если ИспользуетсяУПД Тогда 
				ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.УПД;
			Иначе
				ТипПодтверждающегоДокумента = 
					Справочники.ТипыДокументов.ТипПодтверждающегоДокументаПоВидуЭД(
	...
	ИначеЕсли ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.АктОбОказанииПроизводственныхУслуг") Тогда 
		...
		ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.Акт;
	...
	ИначеЕсли ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда 
		...
		ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.СправкаРасчет;
	...
	ИначеЕсли ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		...
		ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.Накладная;
	...
	ИначеЕсли ТипЗнч(ДокументРегистратор) = Тип("ДокументСсылка.РеализацияОтгруженныхТоваров") Тогда
		...
		Если ОбменСКонтрагентами.ДокументооборотНастроен(ДокументИсточник)
			И ЗначениеЗаполнено(РеквизитыДокумента.ВидЭлектронногоДокумента) Тогда
	...
	Если ОпределятьТипДокументаПоТЧ Тогда
		...
		ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.ПустаяСсылка();
	...
	Для Каждого Операция Из ОперацииКПодтверждению Цикл
		...
		Пока ВыборкаДокументы.Следующий() Цикл
			...
			Если ТипЗнч(ВыборкаДокументы.ПодтверждающийДокументИсточник) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда 
				НоваяСтрока.ТипПодтверждающегоДокумента = Справочники.ТипыДокументов.Договор;
			Иначе
...
Функция ПолучитьТаблицуСписанныеПартииНДС(Товары, СписанныеТоварыБухУчет, Реквизиты, Отказ) Экспорт
	...
	ПодготовитьТаблицуТовары(Товары, Реквизиты);
	...
	// Для организаций на УСН с НДС 5 и 7% остатки РУ не критичны,
	// используются только для целей заполнения 7 раздела декларации.
	КонтролироватьОстаток = НЕ БухгалтерскийУчетПереопределяемый.ОтключитьКонтрольОтрицательныхОстатков()
		И НЕ УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Реквизиты.Организация, Реквизиты.Период);
	...
	СчетаИсключенияДляКонтроляОстатков.Добавить(ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов); 		  //10.21.1
	СчетаИсключенияДляКонтроляОстатков.Добавить(ПланыСчетов.Хозрасчетный.ТопливоВБаке); 									  //10.03.2
	СчетаИсключенияДляКонтроляОстатков.Добавить(ПланыСчетов.Хозрасчетный.ПроизводствоИзДавальческогоСырья); 				  // 20.02
	
	СубСчета = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ГотоваяПродукция);                            //43
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СчетаИсключенияДляКонтроляОстатков, СубСчета);
	...
	Для каждого СтрокаДокумента Из Товары Цикл
		...
		Для каждого СписаннаяПартияБухУчет Из СписанныеТоварыПоСтрокеБухУчет Цикл
			...
			Если ОтсутствуетКоличественныйУчет Тогда
				...
				Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.МодернизацияОС") Тогда
						
					СуммаОсталосьПогасить = СписаннаяПартияБухУчет.Сумма;
					
					Для каждого СтрокаОстаткаДопРасхода Из ОстаткиДопРасходов Цикл
						
						Если СуммаОсталосьПогасить <= 0 Тогда
							Прервать;
						ИначеЕсли СтрокаОстаткаДопРасхода.СуммаБезНДС <= 0 Тогда
							Продолжить;
						КонецЕсли;
						
						Сумма = Мин(СтрокаОстаткаДопРасхода.СуммаБезНДС, СуммаОсталосьПогасить);
						
						СуммаБезНДС	= Сумма;
						НДС	= Окр((Сумма * СтрокаОстаткаДопРасхода.НДС) / СтрокаОстаткаДопРасхода.СуммаБезНДС, 2);
						
						СуммаОсталосьПогасить = СуммаОсталосьПогасить - Сумма;
						
						СтрокаОстаткаДопРасхода.СуммаБезНДС  = СтрокаОстаткаДопРасхода.СуммаБезНДС - СуммаБезНДС;
						СтрокаОстаткаДопРасхода.НДС          = СтрокаОстаткаДопРасхода.НДС - НДС;
						
						ДобавитьСтрокуПоСписаннымПартиям(СписанныеПартииНДС, СтрокаДокумента, СтрокаОстаткаДопРасхода,
							СписаннаяПартияБухУчет.КорАналитикаУчетаЗатрат, СуммаБезНДС, НДС);
				...
				Иначе
					...
					Для каждого СтрокаОстаткаДопРасхода Из ОстаткиДопРасходов Цикл
						...
						ДобавитьСтрокуПоСписаннымПартиям(СписанныеПартииНДС, СтрокаДокумента, СтрокаОстаткаДопРасхода,
							СписаннаяПартияБухУчет.КорАналитикаУчетаЗатрат, СписаннаяСуммаБезНДС, СписанныйНДС);
			...
			Для каждого ОстатокПартии Из ОстаткиПартийНДСПоСтроке Цикл
				...
				ДобавитьСтрокуПоСписаннымПартиям(СписанныеПартииНДС, СтрокаДокумента, ОстатокПартии,
							СписаннаяПартияБухУчет.КорАналитикаУчетаЗатрат, СуммаБезНДС, НДС, Количество);
				...
				ИтогиПоОстаткамДопРасходов = ТаблицыОстатковПартийНДС.ДопРасходы.Скопировать(ОтборДопРасходов);
...
Процедура ПодготовитьТаблицуТовары(Товары, Реквизиты)
	
	ПараметрыДокумента = Новый Структура("ЭлектроннаяТорговаяПлощадка", Ложь);
	ЗаполнитьЗначенияСвойств(ПараметрыДокумента, Реквизиты);
	...
	Если Товары.Колонки.Найти("СтавкаНДСДокумента") <> Неопределено Тогда
		...
		Для Каждого СтрокаТаблицы Из Товары Цикл
			...
			Если СтрокаТаблицы.СтавкаНДСДокумента = Перечисления.СтавкиНДС.БезНДС
				И Не ПараметрыДокумента.ЭлектроннаяТорговаяПлощадка Тогда
			...
			ИначеЕсли Не ЗначениеЗаполнено(СтрокаТаблицы.СтавкаНДСДокумента) Тогда
				СтрокаТаблицы.НовыйСпособУчетаНДС = Неопределено;
			Иначе
...
Процедура ПодготовитьТаблицуСписанныеТоварыБухУчет(СписанныеТоварыБухУчет, Реквизиты)
	...
	ЭтоПродажаТоваровСоСкладаКомиссионера = Реквизиты.Владелец().Колонки.Найти("Комиссионер") <> Неопределено
		И Реквизиты.Владелец().Колонки.Найти("ВидОперации") <> Неопределено
		И Реквизиты.ВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаТоваровСоСкладаКомиссионера;
		...
		Для Ном = 1 По СвойстваСчета.КоличествоСубконто Цикл
	        ...
	        ИначеЕсли СвойстваСчета["ВидСубконто" + Ном] = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты Тогда
				Если ЭтоПродажаТоваровСоСкладаКомиссионера Тогда
					АналитикаУчетаЗатрат["Субконто" + Ном] = Реквизиты.Комиссионер;
				Иначе
...
Функция ПолучитьОстаткиПартийНДС(СписанныеТовары, Реквизиты)
	...
	ТипЗначенияРегистратор = ТипЗнч(Реквизиты.Регистратор);
	
	// В этих документах возможна продажа товаров возвращенных тем же документом
	Если ТипЗначенияРегистратор = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
		Или (ТипЗначенияРегистратор = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") И Реквизиты.ЕстьВозвраты) Тогда
...
Процедура ЗаполнитьЗначениеЕстьНДСПредъявленный(СписанныеПартии)
	...
	|	ВЫБОР
	|		КОГДА НДСПредъявленный.СчетФактура ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
...
Процедура ДобавитьСтрокуПоСписаннымПартиям(СписанныеПартииНДС, СтрокаДокумента, СтрокаОстатка, 
		КорАналитикаУчетаЗатрат, СуммаБезНДС, НДС, Количество = 0)
		
	Если СуммаБезНДС = 0 И НДС = 0 И Количество = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ЗаполнитьЗначенияСвойств(НоваяСтрока, КорАналитикаУчетаЗатрат);
...
Процедура СформироватьДвиженияНДСЗаписиКнигиПродажВосстановлениеНДС(ТоварыУслуги, Реквизиты, Движения, Отказ)
	...
	СтавкаНДС0 = Перечисления.СтавкиНДС.НДС0; 
	ВосстановлениеНДС = Перечисления.СобытияПоНДСПродажи.ВосстановлениеНДС;
	ВосстановлениеНДС0 = Перечисления.СобытияПоНДСПродажи.ВосстановлениеНДС0;
	Для каждого СтрокаТаблицы Из ТоварыУслуги Цикл
		НоваяСтрока = ТаблицаЗаписиКнигиПродаж.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы);
		Если СтрокаТаблицы.СтавкаНДСДокумента = СтавкаНДС0 Тогда
			НоваяСтрока.Событие = ВосстановлениеНДС0;
		Иначе
			НоваяСтрока.Событие = ВосстановлениеНДС;
		КонецЕсли;
	КонецЦикла;
...
Процедура СформироватьДвиженияНДСРаздельныйУчетВыбытиеЦенностей(СписанныеПартииНДС, Движения, Отказ)
	...
	ЕстьНовыйСпособУчетаНДС = СписанныеПартииНДС.Колонки.Найти("НовыйСпособУчетаНДС") <> Неопределено;
	Распределен = Перечисления.СпособыУчетаНДС.Распределен;
	...
	Для каждого СтрокаТаблицы Из СписанныеПартииНДС Цикл
		...
		Если ЕстьНовыйСпособУчетаНДС И СтрокаТаблицы.СпособУчетаНДС <> Распределен Тогда
			ЗаписьРасход.СпособУчетаНДСПриВыбытии = СтрокаТаблицы.НовыйСпособУчетаНДС;
		КонецЕсли;
...
Процедура СформироватьДвиженияНДСРаздельныйУчетОприходованиеТоваров(Товары, Реквизиты, Движения, Отказ) Экспорт
	...
	СформироватьДвиженияНДСРаздельныйУчет(Товары, Реквизиты, ВидДвиженияНакопления.Приход, Движения);
	
КонецПроцедуры

Процедура СформироватьДвиженияНДСРаздельныйУчет(Товары, Реквизиты, ВидДвижения, Движения) Экспорт
	...
	Для каждого СтрокаТаблицы Из Товары Цикл
		...
		Запись = Движения.НДСРаздельныйУчет.Добавить();
		...
		Запись.Партия = Реквизиты.Регистратор;  
		Если РаздельныйУчетНДСБезВычетов Тогда
			Запись.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.ПустаяСсылка();
		Иначе
			Запись.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Распределен;
		КонецЕсли;
		Запись.Количество = СтрокаТаблицы.Количество;
...
Процедура СформироватьПроводкиВключениеНДСВСтоимость(ДанныеПроводок, Движения, Отказ) Экспорт
	...
	СчетаУчетаОС = СчетаУчетаОС();
	ЕстьМалоценка = ДанныеПроводок.Колонки.Найти("Малоценка") <> Неопределено;
	
	ПоддержкаПБУ18 = УчетнаяПолитика.ПоддержкаПБУ18(ДанныеПроводок[0].Организация, ДанныеПроводок[0].Период);
	...
	Для Каждого СтрокаТаблицы Из ДанныеПроводок Цикл
		...
		СвойстваСчета = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаТаблицы.СчетЗатрат);
		ВалютаСчетаЗатрат = Константы.ВалютаРегламентированногоУчета.Получить();
		Для Ном = 1 По СвойстваСчета.КоличествоСубконто Цикл
			...
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
				...
				Если СвойстваСчета.Валютный 
					И ТипЗнч(СтрокаТаблицы["Субконто" + Ном]) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
					ВалютаСчетаЗатрат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы["Субконто" + Ном], "ВалютаВзаиморасчетов");
				КонецЕсли;
		...
		Если СвойстваСчета.Валютный Тогда
			Проводка.ВалютаДт = ВалютаСчетаЗатрат;
		КонецЕсли;
		...
		Если ЕстьМалоценка и ЗначениеЗаполнено(СтрокаТаблицы.Малоценка) и СтрокаТаблицы.Малоценка Тогда
			...
			Проводка.Сумма       = СтрокаТаблицы.НДС;
			Проводка.Содержание  = СтрокаТаблицы.Содержание;
			Проводка.СчетДт      = СтрокаТаблицы.МалоценкаСчетЗатрат;
			
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 1, СтрокаТаблицы.МалоценкаСубконто1);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 2, СтрокаТаблицы.МалоценкаСубконто2);
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, 3, СтрокаТаблицы.МалоценкаСубконто3);
			
			СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(Проводка.СчетДт);
			
			Если СвойстваСчетаДт.УчетПоПодразделениям Тогда
				Проводка.ПодразделениеДт = СтрокаТаблицы.МалоценкаПодразделение;
			КонецЕсли;
			...
			Если СвойстваСчетаДт.НалоговыйУчет И ПоддержкаПБУ18 Тогда
				Проводка.СуммаВРДт = Проводка.Сумма;
			КонецЕсли;
			
			Если СвойстваСчетаКт.НалоговыйУчет И ПоддержкаПБУ18 Тогда
				Проводка.СуммаВРКт = Проводка.Сумма;
			КонецЕсли;
...
Процедура СформироватьПроводкиВключениеНДСВСтоимостьПриобретениеПрав(ДанныеПроводок, Реквизиты, Движения) Экспорт
	
	Если ДанныеПроводок.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Содержание = НСтр("ru = 'НДС включен в стоимость ценностей'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	СвойстваСчетаНМА = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СчетУчетаНМА);
	...
	// Проводки по включению НДС в стоимость.
	// Отражение в учете расходов на приобретение прав на результаты интеллектуальной деятельности может происходить в один или два этапа,
	// в зависимости от того, является ли приобретаемый объект права нематериальным активом в бухгалтерском и налоговом учете.
	// Включение НДС в стоимость происходит по такому же принципу, что и отражение в учете стоимости без суммы НДС.
	// См. УчетНМА.СформироватьДвиженияПриобретениеПрав.
	Для Каждого СтрокаТаблицы Из ДанныеПроводок Цикл
		...
		Для НомерСубконто = 1 По СвойстваСчета.КоличествоСубконто Цикл
			...
			БухгалтерскийУчет.УстановитьСубконто(
				...
				Если СвойстваСчета.Валютный 
					И ТипЗнч(ЗначениеСубконто) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
					ВалютаСчетаЗатрат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначениеСубконто, "ВалютаВзаиморасчетов");
				КонецЕсли;
			
		КонецЦикла;
		
		Если СвойстваСчета.УчетПоПодразделениям Тогда
			Если ЭтоСчетНМА(Проводка.СчетДт) Тогда
		...
		БухгалтерскийУчет.УстановитьСубконто(
			...
			Проводка.СубконтоКт,
			"СпособыУчетаНДС",
			СтрокаТаблицы.НовыйСпособУчетаНДС);

		СвойстваСчетаНУ = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(СтрокаТаблицы.СчетОтражениеНУ);
		
		НужнаОтдельнаяПроводкаНУ = СвойстваСчетаНУ.НалоговыйУчет
			И ЗначениеЗаполнено(СтрокаТаблицы.СчетОтражениеНУ) И СтрокаТаблицы.СчетОтражениеБУ <> СтрокаТаблицы.СчетОтражениеНУ;

		Если Не НужнаОтдельнаяПроводкаНУ Тогда
			Если СвойстваСчета.НалоговыйУчет Тогда
				Проводка.СуммаНУДт = Проводка.Сумма;
			КонецЕсли;
			ПроводкаНУ = Проводка;
		Иначе
			...
			ПроводкаНУ.СчетДт = СтрокаТаблицы.СчетОтражениеНУ;

			Для НомерСубконто = 1 По БухгалтерскийУчет.МаксимальноеКоличествоСубконто() Цикл
				
				ВидСубконто = СтрокаТаблицы["ВидСубконтоОтражениеНУ" + НомерСубконто];
				
				ЗначениеСубконто = СтрокаТаблицы["СубконтоОтражениеНУ" + НомерСубконто];
				
				БухгалтерскийУчет.УстановитьСубконто(
					ПроводкаНУ.СчетДт,
					ПроводкаНУ.СубконтоДт,
					ВидСубконто,
					ЗначениеСубконто);

			КонецЦикла;
			
			Если СвойстваСчетаНУ.УчетПоПодразделениям Тогда
				Если ЭтоСчетНМА(ПроводкаНУ.СчетДт) Тогда
					ПроводкаНУ.ПодразделениеДт = Реквизиты.Подразделение;
				Иначе
					ПроводкаНУ.ПодразделениеДт = СтрокаТаблицы.Подразделение;
				КонецЕсли;
			КонецЕсли;
	
			ПроводкаНУ.СчетКт = СтрокаТаблицы.СчетУчетаНДС;
			
			БухгалтерскийУчет.УстановитьСубконто(
				ПроводкаНУ.СчетКт,
				ПроводкаНУ.СубконтоКт,
				"Контрагенты",
				СтрокаТаблицы.Поставщик);
			...
			БухгалтерскийУчет.УстановитьСубконто(ПроводкаНУ.СчетКт, ПроводкаНУ.СубконтоКт,
				"СпособыУчетаНДС", СтрокаТаблицы.НовыйСпособУчетаНДС);
			ПроводкаНУ.СуммаНУДт = СтрокаТаблицы.НДС;
			ПроводкаНУ.СуммаВРДт = - СтрокаТаблицы.НДС;
		КонецЕсли;
		
		УчетНМА.ПрименитьПовышающийКоэффициент(ПроводкаНУ, Реквизиты.Период, СтрокаТаблицы.ПрименяетсяПовышающийКоэффициент);
			
		НужнаПроводкаБУ = СтрокаТаблицы.СчетОтражениеБУ = ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов;
		НужнаПроводкаНУ = СтрокаТаблицы.СчетОтражениеНУ = ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов;
		
		ПроводкиСовпадают = (СтрокаТаблицы.СчетОтражениеБУ = СтрокаТаблицы.СчетОтражениеНУ) И Не СтрокаТаблицы.ЭтоМалоценныйНМА;
		
		Если Не НужнаПроводкаБУ И Не НужнаПроводкаНУ Тогда
			Продолжить;
		КонецЕсли;
		
		Если НужнаПроводкаБУ Тогда
			ПроводкаНМА = Движения.Хозрасчетный.Добавить();
			УчетНМА.СкопироватьДанныеПроводки(ПроводкаНМА, Проводка, "Кт", "Дт");
			
			Если СтрокаТаблицы.ЭтоМалоценныйНМА Тогда
				ПроводкаНМА.СчетДт = СтрокаТаблицы.СчетЗатрат;
				СвойстваСчетаДт = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ПроводкаНМА.СчетДт);

				Если СвойстваСчетаДт.УчетПоПодразделениям Тогда
					Если ЭтоСчетНМА(ПроводкаНМА.СчетДт) Тогда
						ПроводкаНМА.ПодразделениеДт = Реквизиты.Подразделение;
					Иначе
						ПроводкаНМА.ПодразделениеДт =  СтрокаТаблицы.Подразделение;
					КонецЕсли;
				КонецЕсли;
				
				БухгалтерскийУчет.УстановитьСубконто(ПроводкаНМА.СчетДт, ПроводкаНМА.СубконтоДт, 1, СтрокаТаблицы.Субконто1);
				БухгалтерскийУчет.УстановитьСубконто(ПроводкаНМА.СчетДт, ПроводкаНМА.СубконтоДт, 2, СтрокаТаблицы.Субконто2);
				БухгалтерскийУчет.УстановитьСубконто(ПроводкаНМА.СчетДт, ПроводкаНМА.СубконтоДт, 3, СтрокаТаблицы.Субконто3);
			Иначе
				
				ПроводкаНМА.Период = СтрокаТаблицы.ДатаНачалаСписания;
				ПроводкаНМА.СчетДт = СчетУчетаНМА;

				БухгалтерскийУчет.УстановитьСубконто(
					ПроводкаНМА.СчетДт,
					ПроводкаНМА.СубконтоДт,
					СтрокаТаблицы.ВидСубконтоОтражениеБУ1,
					СтрокаТаблицы.СубконтоОтражениеБУ1);
					
				БухгалтерскийУчет.УстановитьСубконто(
					ПроводкаНМА.СчетДт,
					ПроводкаНМА.СубконтоДт,
					СтрокаТаблицы.ВидСубконтоОтражениеБУ2,
					СтрокаТаблицы.СубконтоОтражениеБУ2);
					
				БухгалтерскийУчет.УстановитьСубконто(
					ПроводкаНМА.СчетДт,
					ПроводкаНМА.СубконтоДт,
					СтрокаТаблицы.ВидСубконтоОтражениеБУ3,
					СтрокаТаблицы.СубконтоОтражениеБУ3);
					
				Если СвойстваСчетаНМА.УчетПоПодразделениям Тогда
					Если ЭтоСчетНМА(ПроводкаНМА.СчетДт) Тогда
						ПроводкаНМА.ПодразделениеДт = Реквизиты.Подразделение;
					Иначе
						ПроводкаНМА.ПодразделениеДт =  СтрокаТаблицы.Подразделение;
			...
			ПроводкаНМА.Сумма = Проводка.Сумма;
			
			Если ПроводкиСовпадают Тогда
				ПроводкаНМА.СуммаНУКт = Проводка.СуммаНУДт;
				ПроводкаНМА.СуммаВРКт = Проводка.СуммаВРДт;
				ПроводкаНМА.СуммаНУДт = СтрокаТаблицы.НДС;
			Иначе
				ПроводкаНМА.СуммаНУКт = 0;
				ПроводкаНМА.СуммаВРКт = СтрокаТаблицы.НДС;
				ПроводкаНМА.СуммаНУДт = 0;
				ПроводкаНМА.СуммаВРДт = СтрокаТаблицы.НДС;
			КонецЕсли;
			
			УчетНМА.ДобавитьПроводкуМалоценныйНМАСчетФинансовогоРезультата(Движения, ПроводкаНМА, СтрокаТаблицы, СтрокаТаблицы.Подразделение, Настройки)
				
		КонецЕсли;
		
		Если Не ПроводкиСовпадают И НужнаПроводкаНУ Тогда
			
			ПроводкаНМА = Движения.Хозрасчетный.Добавить();
			
			УчетНМА.СкопироватьДанныеПроводки(ПроводкаНМА, ПроводкаНУ, "Кт", "Дт");
			...
			БухгалтерскийУчет.УстановитьСубконто(
				ПроводкаНМА.СчетДт,
				ПроводкаНМА.СубконтоДт,
				СтрокаТаблицы.ВидСубконтоОтражениеНУ1,
				СтрокаТаблицы.СубконтоОтражениеНУ1);
				
			БухгалтерскийУчет.УстановитьСубконто(
				ПроводкаНМА.СчетДт,
				ПроводкаНМА.СубконтоДт,
				СтрокаТаблицы.ВидСубконтоОтражениеНУ2,
				СтрокаТаблицы.СубконтоОтражениеНУ2);
				
			БухгалтерскийУчет.УстановитьСубконто(
				ПроводкаНМА.СчетДт,
				ПроводкаНМА.СубконтоДт,
				СтрокаТаблицы.ВидСубконтоОтражениеНУ3,
				СтрокаТаблицы.СубконтоОтражениеНУ3);
			...
			ПроводкаНМА.СуммаНУКт = СтрокаТаблицы.НДС;
			ПроводкаНМА.СуммаВРКт = - СтрокаТаблицы.НДС;
			ПроводкаНМА.СуммаНУДт = СтрокаТаблицы.НДС;
			ПроводкаНМА.СуммаВРДт = - СтрокаТаблицы.НДС;
...
Функция ЭтоСчетНМА(СчетУчета)
	Возврат БухгалтерскийУчетПовтИсп.СчетВИерархии(СчетУчета,  ПланыСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов)
		Или БухгалтерскийУчетПовтИсп.СчетВИерархии(СчетУчета,  ПланыСчетов.Хозрасчетный.НематериальныеАктивы);
КонецФункции
...
Процедура СформироватьПроводкиИсключениеНДСИзСтоимости(ТаблицаИсключениеНДСИзСтоимости, СписанныеТоварыБухУчет, Реквизиты, Движения, Отказ)
	...
	ЕстьМалоценка            = ТаблицаИсключениеНДСИзСтоимости.Колонки.Найти("Малоценка") <> Неопределено;
	...
	Для каждого СтрокаТаблицы Из ТаблицаИсключениеНДСИзСтоимости Цикл
		...
		Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.КомплектацияНоменклатуры") Тогда
			Если Реквизиты.ЭтоКомплектация Тогда
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Номенклатура", Реквизиты.Комплект);
			Иначе
				СубконтоНоменклатура = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура;
				Если СтрокаСписанияБухУчет.ВидКорСубконто1 = СубконтоНоменклатура Тогда
					Номенклатура = СтрокаСписанияБухУчет.КорСубконто1;
				ИначеЕсли СтрокаСписанияБухУчет.ВидКорСубконто2 = СубконтоНоменклатура Тогда
					Номенклатура = СтрокаСписанияБухУчет.КорСубконто2;
				ИначеЕсли СтрокаСписанияБухУчет.ВидКорСубконто3 = СубконтоНоменклатура Тогда
					Номенклатура = СтрокаСписанияБухУчет.КорСубконто3;
				Иначе
					Номенклатура = СтрокаСписанияБухУчет.Номенклатура;
				КонецЕсли;
				БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Номенклатура", Номенклатура);
			КонецЕсли;
		Иначе
		...
		Если ЕстьМалоценка и СтрокаТаблицы.Малоценка Тогда
...
Процедура ВключитьНДСВРасходыНУ(СписанныеПартииНДС, ТаблицаЗатратПоВключениюНДСВСоставРасходов,
	...
	ТаблицаЗатрат = ТаблицаЗатратПоВключениюНДСВСоставРасходов.СкопироватьКолонки("ОбъектУчета, ПодразделениеЗатрат,
		|Подразделение,Субконто1, Субконто2, Субконто3, СуммаБУ, СуммаНУ, СуммаПР, СуммаВР, СчетЗатрат, СчетАмортизации, СодержаниеОперации");
	...
	Если ДанныеПроводокПоАренднымОбязательствам.Количество() > 0 Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			КолонкиДляСвертки = 
				...
				|СчетЗатрат,Подразделение,Субконто1,Субконто2,Субконто3,
				|СчетУчетаНДС,СчетФактура,Поставщик,НовыйСпособУчетаНДС";
			Если ДанныеПроводок.Колонки.Найти("Партия") <> Неопределено Тогда
				КолонкиДляСвертки = КолонкиДляСвертки +", Партия";
			КонецЕсли;
			ДанныеПроводок.Свернуть(КолонкиДляСвертки, "НДС");
...
Процедура ПолучитьАналитикуСписанияНДСПоМалоценке(ДанныеПроводок, Реквизиты)
	
	ДанныеПроводок.Колонки.Добавить("Малоценка",  Новый ОписаниеТипов("Булево"));
	ДанныеПроводок.Колонки.Добавить("МалоценкаСчетЗатрат");
	ДанныеПроводок.Колонки.Добавить("МалоценкаПодразделение");
	ДанныеПроводок.Колонки.Добавить("МалоценкаСубконто1");
	ДанныеПроводок.Колонки.Добавить("МалоценкаСубконто2");
	ДанныеПроводок.Колонки.Добавить("МалоценкаСубконто3");
	
	СтрокиМалоценка    = ДанныеПроводок.НайтиСтроки(Новый Структура("СчетЗатрат", ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов));
	СчетУчетаМалоценки = ПланыСчетов.Хозрасчетный.ПриобретениеМалоценногоОборудованияИЗапасов;
	
	ДанныеПроводокПоМалоценке = Новый Массив;
	...
	Для каждого СтрокаТаблицы Из ДанныеПроводок Цикл
		...
		Если НЕ СтрокаТаблицы.СчетЗатрат = СчетУчетаМалоценки Тогда
			Продолжить;
		КонецЕсли;
		СтрокаТаблицы.Малоценка = Истина;
		ДанныеПроводокПоМалоценке.Добавить(СтрокаТаблицы);
		
		Номенклатура.Добавить(СтрокаТаблицы.Субконто1);
		Партии.Добавить(СтрокаТаблицы.Партия);
		
	КонецЦикла;
	
	Если ДанныеПроводокПоМалоценке.Количество() > 0 Тогда
	
		Запрос = Новый Запрос;
		
		Запрос.УстановитьПараметр("Организация", Реквизиты.Организация);
		Запрос.УстановитьПараметр("Номенклатура", 
			ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(Номенклатура));
		Запрос.УстановитьПараметр("Партии", 
			ОбщегоНазначенияБПВызовСервера.УдалитьПовторяющиесяЭлементыМассива(Партии));
		...
		|	И ХозрасчетныйДвиженияССубконто.Регистратор В(&Партии)
		|	И ХозрасчетныйДвиженияССубконто.СубконтоКт1 В(&Номенклатура)
		|	И ХозрасчетныйДвиженияССубконто.СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВыбытиеМалоценногоОборудованияИЗапасов)";
		 
		Результат = Запрос.Выполнить();
		
		Если НЕ Результат.Пустой() Тогда
			
			ТаблицаАналитикиМалоценки = Результат.Выгрузить();
			
			Для Каждого Проводка Из ДанныеПроводокПоМалоценке Цикл
				
				АналитикаМалоценки = ТаблицаАналитикиМалоценки.НайтиСтроки(Новый Структура("Регистратор, СубконтоКт1" , Проводка.Партия, Проводка.Субконто1));
				
				Если АналитикаМалоценки.Количество() > 0 Тогда
					Проводка.МалоценкаСчетЗатрат    = АналитикаМалоценки[0].СчетДт;
					Проводка.МалоценкаПодразделение = АналитикаМалоценки[0].ПодразделениеДт;
					Проводка.МалоценкаСубконто1     = АналитикаМалоценки[0].СубконтоДт1;
					Проводка.МалоценкаСубконто2     = АналитикаМалоценки[0].СубконтоДт2;
					Проводка.МалоценкаСубконто3     = АналитикаМалоценки[0].СубконтоДт3;
				Иначе
					Проводка.Малоценка = Ложь;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция ЕстьУчетнаяПолитикаСРаздельнымУчетомНДС() Экспорт
	...
	|	НастройкиУчетаНДС.РаздельныйУчетНДСБезВычетов
	|ИЗ
	|	РегистрСведений.НастройкиУчетаНДС КАК НастройкиУчетаНДС
	|ГДЕ
	|	НастройкиУчетаНДС.РаздельныйУчетНДСНаСчете19
	|	ИЛИ НастройкиУчетаНДС.РаздельныйУчетНДСДо2014Года
	|	ИЛИ НастройкиУчетаНДС.РаздельныйУчетНДСБезВычетов";
