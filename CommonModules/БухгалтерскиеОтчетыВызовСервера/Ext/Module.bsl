...
Процедура ДобавитьГруппировкуПоПериоду(ПараметрыОтчета, Структура) Экспорт
	...
	ИначеЕсли ПараметрыОтчета.Периодичность >= 6 Тогда
		...
		Для Каждого ИмяПоказателя Из ПараметрыОтчета.НаборПоказателей Цикл
			...
			ПрефиксПоказателяНаличияОборота = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПараметрыОтчета, 
				"ПрефиксПоказателяНаличияОборота", "");
			ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОборотыЗаПериод." + ПрефиксПоказателяНаличияОборота + ИмяПоказателя + "ОборотДт");
			ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
			ЭлементОтбора.Использование = Истина;
			
			ЭлементОтбора = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбора.ЛевоеЗначение = 
				Новый ПолеКомпоновкиДанных("ОборотыЗаПериод." + ПрефиксПоказателяНаличияОборота + ИмяПоказателя + "ОборотКт");
			ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
			ЭлементОтбора.Использование = Истина;
...
Функция ОписаниеАналитикиСчета(Счет, ДополнитьПодразделением, ВключенныеСубконто = Неопределено) Экспорт
	...
	// см. БухгалтерскиеОтчетыКлиент.ТабличноеПолеПоСчетамПредставлениеНачалоВыбора
	ДоступнаяАналитика           = Новый Массив; 
	ЭлементыПредставления        = Новый Массив; // представление содержит включенную аналитику
	ВключеннаяАналитикаПоНомерам = Новый Соответствие; // ключ - строка с номером субконто, значение - вид субконто
	
	Для НомерСубконто = 0 По СвойстваСчета.КоличествоСубконто Цикл
		
		КлючСубконто = XMLСтрока(НомерСубконто);
		
		// Условный номер 0 - у подразделения
		Если НомерСубконто = 0 Тогда
			
			Если Не ДополнитьПодразделением Или Не СвойстваСчета.УчетПоПодразделениям Тогда
				Продолжить;
			КонецЕсли;
			
			ВидСубконто           = "Подразделение";
			ПредставлениеСубконто = НСтр("ru = 'Подразделения'");
		Иначе
			
			ВидСубконто           = СвойстваСчета["ВидСубконто" + КлючСубконто];
			ПредставлениеСубконто = СвойстваСчета["ВидСубконто" + КлючСубконто + "Наименование"];
			
		КонецЕсли;
		
		ДоступнаяАналитика.Добавить(КлючСубконто);
		
		Если ВключенныеСубконто <> Неопределено
			И ВключенныеСубконто[ВидСубконто] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ВключеннаяАналитикаПоНомерам.Вставить(КлючСубконто, ВидСубконто);
		ЭлементыПредставления.Добавить(ПредставлениеСубконто);
		
	КонецЦикла;
	
	ОписаниеАналитики.КоличествоВключеннойАналитики = ВключеннаяАналитикаПоНомерам.Количество();
	ОписаниеАналитики.Представление                 = СтрСоединить(ЭлементыПредставления, ", ");
	
	// Закодируем позиционно включенную аналитику
	ЭлементыВключеннаяАналитика = Новый Массив;
	Для Каждого КлючСубконто Из ДоступнаяАналитика Цикл
		
		Если ВключеннаяАналитикаПоНомерам[КлючСубконто] = Неопределено Тогда
			Включена = "-";
		Иначе
			Включена = "+";
		КонецЕсли;
		
		ЭлементыВключеннаяАналитика.Добавить(Включена + КлючСубконто);
		
	КонецЦикла;
	
	ОписаниеАналитики.ШифрАналитики = СтрСоединить(ЭлементыВключеннаяАналитика);
...
Процедура ИзменитьРазмерДиаграмм(ТабличныйДокумент, Знач КоэффициентИзмененияШирины, Знач КоэффициентИзмененияВысоты = Неопределено) Экспорт
	...
	Для Каждого Рисунок Из ТабличныйДокумент.Рисунки Цикл
		...
		ИзменитьРазмерДиаграммы(ТабличныйДокумент, Рисунок, КоэффициентИзмененияШирины, КоэффициентИзмененияВысоты);
...
Функция ПодготовитьОтчет(ПараметрыОтчета) Экспорт
	...
	Попытка
		...
		Если ПараметрыИсполненияОтчета.ИспользоватьВременныеТаблицыЗапроса Тогда
			МенеджерВременныхТаблицЗапроса = МенеджерОтчета.МенеджерВременныхТаблицОтчета(ПараметрыОтчета, МакетКомпоновки);
		КонецЕсли;
	...
	Если ПараметрыИсполненияОтчета.НастроитьШиринуКолонкиИтогов Тогда
		
		Если ПараметрыИсполненияОтчета.КоличествоКолонокПоказателей > 0 Тогда
			КоличествоКолонокПоказателейОтчета = ПараметрыИсполненияОтчета.КоличествоКолонокПоказателей;
		Иначе
			КоличествоКолонокПоказателейОтчета = МенеджерОтчета.КоличествоКолонокПоказателей(ПараметрыОтчета, Результат);
		КонецЕсли;

		БухгалтерскиеОтчеты.НастроитьШиринуКолонкиИтогов(Результат, КоличествоКолонокПоказателейОтчета);
...
// Процедура уменьшает размер переданной диаграммы табичного документа до заданной величины.
//
// Параметры:
//   ТабличныйДокумент - ТабличныйДокумент - табличный документ, содержащий диаграммы
//   Рисунок - Картинка - Изменяемая диаграмма
//   КоэффициентИзмененияШирины - Число - коэффициент (больше 0), на который нужно изменить ширину диаграмм.
//                         К примеру, 0.75 - уменьшить ширину диаграмм до 75% от исходной.
//   КоэффициентИзмененияВысоты - Число - коэффициент, на который нужно изменить высоту диаграмм.
//                         К примеру, 0.75 - уменьшить высоту диаграмм до 75% от исходной.
//                         Если не указан, то высота будет изменена на тоже значение, что и ширина.
//
Процедура ИзменитьРазмерДиаграммы(ТабличныйДокумент, Рисунок, Знач КоэффициентИзмененияШирины, Знач КоэффициентИзмененияВысоты = Неопределено) Экспорт
...
// Возвращает Имя области табличного документа по переданному значению верхней и нижней границы
//
// Параметры:
//   ТабличныйДокумент - ТабличныйДокумент - табличный документ
//   Верх - Число - верхняя граница области.
//   Низ - Число - нижняя граница области.
// 
// Возвращаемое значение:
//  Строка - название области.
//
Функция ИмяОбласти(ТабличныйДокумент, Знач Верх, Знач Низ) Экспорт
	
	ПерваяСтрока = НайтиСтрокуДляПозицииВМиллиметрах(ТабличныйДокумент, Верх);
	ПоследняяСтрока = НайтиСтрокуДляПозицииВМиллиметрах(ТабличныйДокумент, Низ);
	
	Возврат СтрШаблон("R%1:R%2", ПерваяСтрока, ПоследняяСтрока);
...
Функция СохраняемыеРеквизитыФормы()
	...
	ИменаРеквизитов.Добавить("НаборПоказателейОтчета");
	
	//В мобильном клиенте настройка скрывается по умолчанию, запоминать это не следует
	Если Не ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		ИменаРеквизитов.Добавить("ПоказыватьБыстрыеНастройки");
	КонецЕсли;
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, ЗагружатьТолькоРеквизиты = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() 
		И ФормаОтчета.Отчет.Свойство("ПоказыватьБыстрыеНастройки") Тогда
		ФормаОтчета.Отчет.ПоказыватьБыстрыеНастройки = Ложь;
	КонецЕсли;
...
Функция ПолучитьПараметрыРасшифровкиОтчета(Адрес, ИдентификаторОбъекта, Расшифровка) Экспорт
	...
	Иначе
		...
		Если ПоляРасшифровки.Количество() > 0 И ЗначениеЗаполнено(ПоляРасшифровки[0].Значение) 
			И Не БухгалтерскиеОтчетыКлиентСервер.ПростойТип(ПоляРасшифровки[0].Значение) Тогда
			ПараметрыРасшифровки.Вставить("ОткрытьОбъект", Истина);
...
// Заполняет описание типов видов субконто.
//
// Параметры:
//  СтруктураОписания - Структура - Структура с описанием типов.
//  ОписаниеСубконто - Строка - ключ в структуре описания.
//  НаименованиеСубконто - Строка - Наименование субконто.
//
Процедура ВставитьОписанияТиповВидовСубконто(СтруктураОписания, ОписаниеСубконто, НаименованиеСубконто)
	
	ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные[НаименованиеСубконто];
	ТипЗначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидСубконто, "ТипЗначения");
	СтруктураОписания.Вставить(ОписаниеСубконто, ТипЗначения);
...
Процедура ИнициализацияФормыОтчета(Отчет, РеквизитыФормы, Форма)
	...
	Если РеквизитыФормы.Свойство("ОписанияТиповВидовСубконто") Тогда
		...
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "Номенклатура", "Номенклатура");
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "Склад", "Склады");
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "Контрагент", "Контрагенты");
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "ДоговорКонтрагента", "Договоры");
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "Партия", "Партии");
		ВставитьОписанияТиповВидовСубконто(Форма.ОписанияТиповВидовСубконто, "ДокументРасчетовСКонтрагентами", 
	...
	Если РеквизитыФормы.Свойство("СоответствиеОрганизаций")
		...
		Если РеквизитыФормы.Свойство("ИсключитьФилиалы") Тогда
			ИсключитьФилиалы = Форма.ИсключитьФилиалы;
		КонецЕсли;
		ОбщегоНазначенияБПВызовСервера.ЗаполнитьСписокОрганизаций(
			Форма.Элементы.ПолеОрганизация, Форма.СоответствиеОрганизаций, , ИсключитьФилиалы);
	КонецЕсли;
...
Процедура ЗаполнитьГруппировкиИзНастроек(КомпоновщикНастроек, Структура, Группировка)
	...
	Если (НРег(Поле) <> "подразделение" Или БухгалтерскийУчетПереопределяемый.ВестиУчетПоПодразделениям())
		И (НРег(Поле) <> "договор" Или БухгалтерскийУчетПереопределяемый.ВестиУчетПоДоговорам())
		И (НРег(Поле) <> "ответственный" Или ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОтветственных")) Тогда
...
Процедура УстановитьОрганизацию(Форма)
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Отчет, "Организация") Тогда
		ДанныеОтчета = Форма.Отчет;
	...
	Если Справочники.Организации.ИспользуетсяНесколькоОрганизаций() Тогда
		...
		Если ЗначениеЗаполнено(ОсновнаяОрганизация) И ОсновнаяОрганизация <> ДанныеОтчета.Организация Тогда
			ДанныеОтчета.Организация = ОсновнаяОрганизация;
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеОтчета, "ВключатьОбособленныеПодразделения") Тогда
				ДанныеОтчета.ВключатьОбособленныеПодразделения = Ложь;
			КонецЕсли;
...
Процедура ВывестиЗаголовокОтчетаПоМакету(ОбластиЗаголовка, ПараметрыОтчета, КомпоновщикНастроек, Результат)
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		...
		Если СтрДлина(ТекстОрганизация) < КоличествоСимволов Тогда    
			ТекстОрганизация = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(ТекстОрганизация, КоличествоСимволов, " ", "Справа");
		КонецЕсли;
