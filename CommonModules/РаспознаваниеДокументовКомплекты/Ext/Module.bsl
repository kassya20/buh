...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеКомплектов", Истина);
...
Функция НайтиВсеВозможныеКомплекты(ВсеДанныеДокументов, ДанныеОбязательногоДокумента = Неопределено)
	...
	// Рассчитываем ошибки только для документов, входящих в комплекты
	Если ТаблицаРезультата.Количество() > 0 Тогда
		СсылкиДокументовКомплекта = Новый Массив;
		Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
			Для Каждого СтрокаОбщейТаблицы Из СтрокаРезультата.СтрокиОбщейТаблицы Цикл
				Если СсылкиДокументовКомплекта.Найти(СтрокаОбщейТаблицы.Ссылка) = Неопределено Тогда
					СсылкиДокументовКомплекта.Добавить(СтрокаОбщейТаблицы.Ссылка);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		
		Если СсылкиДокументовКомплекта.Количество() > 0 Тогда
			ТаблицаОшибок = ОшибкиПоДокументам(СсылкиДокументовКомплекта);
			...
			Для Каждого СтрокаРезультата Из ТаблицаРезультата Цикл
				...
				Для Каждого СтрокаОбщейТаблицы Из СтрокаРезультата.СтрокиОбщейТаблицы Цикл
					СтрокаОшибки = ТаблицаОшибок.Найти(СтрокаОбщейТаблицы.Ссылка, "Ссылка");
					Если СтрокаОшибки <> Неопределено Тогда
						ВсегоОшибок = ВсегоОшибок + СтрокаОшибки.КоличествоОшибок;
						ВсегоОшибокТЧ = ВсегоОшибокТЧ + СтрокаОшибки.КоличествоОшибокВТаблицах;
					КонецЕсли;
				КонецЦикла;
				
				СтрокаРезультата.ОшибокВсего = ВсегоОшибок;
				СтрокаРезультата.ОшибокТЧ = ВсегоОшибокТЧ;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	
	ТаблицаРезультата.Сортировать("ДокументовВКомплекте Убыв, РазностьДат, Печатей Убыв, СтрокТЧ Убыв, ОшибокТЧ, ОшибокВсего");
