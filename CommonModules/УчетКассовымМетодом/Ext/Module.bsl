...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьФункциональнуюОпциюРасчеты");
...
// Устарела. Следует использовать функцию УчетПСН.УстановитьПорядокОтраженияАванса.
...
Функция БезЗакрывающихДокументов(Организация, Дата, ВидОперации) Экспорт
	...
	БезЗакрывающихДокументов = УчетБезЗакрывающихДокументовВозможен(Организация, Дата)
		И ((УчетДенежныхСредствКлиентСервер.ЕстьРасчетыСПокупателями(ВидОперации)
			...
			И Не ПолучитьФункциональнуюОпцию("ВестиУчетРасчетовСКонтрагентами")
...
Функция УчетБезЗакрывающихДокументовВозможен(Организация, Дата) Экспорт
	...
	ПрименяетсяАУСН = УчетнаяПолитика.ПрименяетсяАУСН(Организация, Дата);
	ПлательщикНДСУСНСВычетами = УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Организация, Дата);
	
	УСНБезЗакрывающих =
		СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная И ПрименяетсяУСНДоходы И Не ПлательщикНДСУСНСВычетами;
	УчетВозможен = УСНБезЗакрывающих
		Или ПрименяетсяАУСН
...
Процедура УстановитьФункциональнуюОпциюРасчеты() Экспорт
	...
	РасчетыВключены = Константы.ВестиУчетРасчетовСКонтрагентами.Получить();
	
	Если Не ВозможноИзменитьФункциональностьРасчеты Или РасчетыВключены Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	И НастройкиСистемыНалогообложенияСрезПоследних.ОбъектНалогообложенияУСН = ЗНАЧЕНИЕ(Перечисление.ОбъектыНалогообложенияПоУСН.Доходы)";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось установить функциональность Расчеты по причине:
				|%1'"), ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Константы.ВестиУчетРасчетовСКонтрагентами,
				,
				ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Функция ПорядокОтраженияДоходовПоУмолчанию(Организация, Знач Период)
	...
	ИначеЕсли УчетнаяПолитика.ПрименяетсяОсобыйПорядокНалогообложения(Организация, Период) Тогда
		...
		Если УчетнаяПолитика.ПрименяетсяУСНПатент(Организация, Период) И Не УчетнаяПолитика.ПлательщикЕНВД(Организация, Период) Тогда
			...
			ПатентПоУмолчанию = УчетПСН.ВидДеятельностиПоПатентуПоУмолчанию(Организация, Период);
