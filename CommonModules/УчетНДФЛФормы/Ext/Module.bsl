	...
	Иначе
		...
		Попытка
			...
			Если Организация <> ГоловнаяОрганизация Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Отбор, "Организация", ВидСравненияКомпоновкиДанных.ВСписке, МассивОрганизаций);
			Иначе
...
Процедура ЗаполнитьПредставлениеВычетовКДоходамСтрокиНДФЛ(Форма, СтрокаНДФЛ, ОписаниеПанелиВычеты, СоответствиеНачисленийРеквизитам)
	...
	Если ВычетыКДоходам <> Неопределено Тогда
		...
		ЕстьКолонкаДопВычет = ОписаниеПанелиВычеты.ЕстьКолонкаДопВычет;
		...
		Для каждого СтрокаДанныхВычетов Из ДанныеПоСтрокеНДФЛ Цикл
			Если СоответствиеНачисленийРеквизитам = Неопределено ИЛИ СоответствиеНачисленийРеквизитам.Количество() = 0 ИЛИ
				...
				УчетНДФЛКлиентСервер.ДополнитьПредставлениеВычетов(СтрокаНДФЛ.ПредставлениеВычетовКДоходам,
									СтрокаДанныхВычетов.КодВычета, СтрокаДанныхВычетов.СуммаВычета);
				Если ЕстьКолонкаДопВычет Тогда
					УчетНДФЛКлиентСервер.ДополнитьПредставлениеВычетов(СтрокаНДФЛ.ПредставлениеВычетовКДоходам,
									СтрокаДанныхВычетов.КодВычетаДополнительный, СтрокаДанныхВычетов.СуммаВычетаДополнительная);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
