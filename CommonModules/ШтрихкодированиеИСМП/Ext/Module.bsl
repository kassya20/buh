...
// * ИспользоватьПолноеРаспределениеЕГАИС - Булево - если Ложь, распределение штрихкодов алкогольной продукции добавляется только при наличии ошибок
Функция НовыеПараметрыРаспределенияШтрихкодовУпаковок() Экспорт
	...
	Параметры.Вставить("ИспользоватьПолноеРаспределениеЕГАИС", Истина);
...
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Для Каждого СтрокаТовары Из НайденныеСтрокиТовары Цикл
			...
			Иначе
				...
				Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(СтрокаШтрихкод.ВидПродукции) Тогда
	...
	Для Каждого СтрокаШтрихкод Из ТаблицаШтрихкоды Цикл
		...
		Если СтрокаШтрихкод.Количество <= 0
			Или Не ПараметрыРаспределения.ИспользоватьПолноеРаспределениеЕГАИС
				И СтрокаШтрихкод.ВидПродукции = Перечисления.ВидыПродукцииИС.Алкогольная Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбработкаИтоговойСтрокиРаспределения(РезультатСписания, СтрокаШтрихкод, ПараметрыНормализацииКМ, ПараметрыНормализацииМОТП, ПараметрыНормализацииИСМП)
	...
	Если ЗначениеЗаполнено(РезультатСписания.ПолныйКодМаркировки)
		И ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияИСМП(РезультатСписания.ВидПродукции, Истина) Тогда
		...
		Если ДанныеРазбора <> Неопределено Тогда
			...
			Если ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(РезультатСписания.ВидПродукции) Тогда
...
Функция ТребуетсяПоискТоваровПриРаспределенииСУчетомЦены(СтрокаШтрихкод, ПараметрыСканирования)
	Возврат ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(СтрокаШтрихкод.ВидПродукции)
...
Процедура ЗаполнениеСохраняемойСтрокиШтрихкодовУпаковкиПоДаннымРазрешительнойСистемыГИСМТ(СтрокаШтрихкод, ДанныеШтрихкода, ПараметрыСканирования)
	...
	Если Не ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияМОТП(ДанныеШтрихкода.ВидПродукции) Тогда
		Возврат;
	КонецЕсли;
