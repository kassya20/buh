...
// Возвращает строковый код основания применения льготной налоговой ставки в формате заявления о применении льготы
...
//
Функция ОснованиеМестнойЛьготыСтрокой(Знач Статья, Знач Часть, Знач Пункт, Знач Подпункт, Знач Абзац, Знач Иное) Экспорт
...
//                                См. также ОснованиеМестнойЛьготыСтрокой().
...
// Формирует по переданному сокращенному коду основания льготы (12 символов) полный код (24 символа) - фактически
// заполняет только номер статьи, пункта и подпункта. Часть, абзац и иное заполняются нулями.
//
// Параметры:
//  КодОснованияЛьготы  - Строка - код основания применения льготной ставки налога в формате, применявшемся в
//                                 декларациях (длина 12).
//                                 См. также ОснованиеМестнойЛьготыСтрокой().
//
// Возвращаемое значение:
//  Строка - сокращенный код основания применения льготной ставки налога (длина - 24).
//
Функция ПолныйКодОснованияЛьготы(Знач КодОснованияЛьготы) Экспорт 
	...
	Возврат ОснованиеМестнойЛьготыСтрокой(СтатьяПоФормату, "", ПунктПоФормату, ПодпунктПоФормату, "", "");
