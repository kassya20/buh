...
Функция НеактивныеВидыДокументов() Экспорт
	...
	НачалоПредыдущегоГода = ДобавитьМесяц(НачалоГодаСеанса, -12);
	...
	Для каждого ВидОтправляемыхДокументов Из ВидыОтправляемыхДокументов Цикл
		...
		Если СтрНачинаетсяС(ВидОтправляемыхДокументов.Источник, "РегламентированныйОтчет")
			...
			Неактивный = ДатаНачала <> Неопределено И ДатаНачала >= НачалоГодаЧерезГод
				ИЛИ ДатаКонца <> Неопределено И ДатаКонца < НачалоПредыдущегоГода;
	...
	Запрос = Новый Запрос(
		...
		|		И ВидыДокументовГосОргановДляПоиска.ДатаКонцаДействия < &НачалоПредыдущегоГода");
	
	Запрос.УстановитьПараметр("НачалоПредыдущегоГода", НачалоПредыдущегоГода);
