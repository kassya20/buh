...
//   НеобработанныеУзлы - Неопределено - Не искать прочие узлы.
//       - Массив - Если ИменаРеквизитов указаны, то в НеобработанныеУзлы добавляются все прочие узлы,
//           которые не описаны в ИменаРеквизитов.
//   ТребуетсяАдаптацияИмен - Булево - Если Истина, то из имен узлов удаляются пробелы и тире.
//       Обязательно передавать Истина в случае, если имена узлов не известны, или в именах могут встречаться пробелы или тире.
//
// Возвращаемое значение:
...
//
Функция УзлыЭлементаDOM(ЭлементDOM, ИменаУзлов = "", НеобработанныеУзлы = Неопределено, ТребуетсяАдаптацияИмен = Ложь) Экспорт
	Результат = Новый Структура;
	ЕстьОжидаемыеУзлы = Не ПустаяСтрока(ИменаУзлов);
	ОжидаемыеУзлы = Новый Структура(ИменаУзлов);
	Если ЭлементDOM = Неопределено Тогда
		Возврат ОжидаемыеУзлы;
	КонецЕсли;
	Для Каждого ВложенныйЭлементDOM Из ЭлементDOM.ДочерниеУзлы Цикл
		Если ВложенныйЭлементDOM.ТипУзла <> ТипУзлаDOM.Элемент Тогда
			Продолжить;
		КонецЕсли;
		ИмяУзла = ЛокальноеИмя(ВложенныйЭлементDOM);
		Если Не ЗначениеЗаполнено(ИмяУзла) Тогда
			Продолжить;
		КонецЕсли;
		Если ТребуетсяАдаптацияИмен Тогда // В целях оптимизации спрятано за флажок.
			ИмяУзла = СтроковыеФункцииБЗККлиентСервер.УбратьПробелы(ИмяУзла, Истина);
		КонецЕсли;
		Если ЕстьОжидаемыеУзлы И Не ОжидаемыеУзлы.Свойство(ИмяУзла) Тогда
			Если НеобработанныеУзлы <> Неопределено Тогда
				НеобработанныеУзлы.Добавить(ЭлементDOM);
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		ЭтоПервыйУзелСТакимИменем = Не Результат.Свойство(ИмяУзла);
		Если ЭтоПервыйУзелСТакимИменем Тогда
		...
		КонецЕсли;
	КонецЦикла;
	Если ЕстьОжидаемыеУзлы Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Результат, ОжидаемыеУзлы, Ложь);
	КонецЕсли;
	Возврат Результат;
КонецФункции

Функция ЛокальноеИмя(ЭлементDOM) Экспорт
	Возврат ?(ЗначениеЗаполнено(ЭлементDOM.ЛокальноеИмя),
		ЭлементDOM.ЛокальноеИмя,
		СтроковыеФункцииБЗККлиентСервер.СтрПрав(ЭлементDOM.ИмяУзла, ":"));
КонецФункции

Функция ЛокальныйПуть(ЭлементDOM) Экспорт
	МассивИмен = Новый Массив;
	РодительDOM = ЭлементDOM.РодительскийУзел;
	ЛокальноеИмя = ?(РодительDOM = Неопределено, "", СериализацияБЗК.ЛокальноеИмя(РодительDOM));
	Пока ЗначениеЗаполнено(ЛокальноеИмя) Цикл
		МассивИмен.Вставить(0, ЛокальноеИмя);
		РодительDOM = РодительDOM.РодительскийУзел;
		ЛокальноеИмя = ?(РодительDOM = Неопределено, "", СериализацияБЗК.ЛокальноеИмя(РодительDOM));
	КонецЦикла;
...
//   Массив
//
Функция СписокDOM(ЭлементDOM, ПутьКСвойству, НеобработанныеУзлы = Неопределено) Экспорт
	Если ЭлементDOM = Неопределено Тогда
		Возврат Новый Массив;
	КонецЕсли;
	Если ТипЗнч(ЭлементDOM) = Тип("Структура") Тогда
		УзлыЭлементаDOM = ЭлементDOM;
	Иначе
		УзлыЭлементаDOM = УзлыЭлементаDOM(ЭлементDOM, ПутьКСвойству, НеобработанныеУзлы);
	КонецЕсли;
	Значение = Неопределено;
	Если Не УзлыЭлементаDOM.Свойство(ПутьКСвойству, Значение) Или Значение = Неопределено Тогда
		Возврат Новый Массив;
...
Функция УзлыЭлементаDOMСКонтролем(ЭлементDOM, ИменаРеквизитов) Экспорт
	ПредставленияНеобработанныхУзловDOM = Новый Массив;
	...
	Для Каждого ВложенныйЭлементDOM Из ЭлементDOM.ДочерниеУзлы Цикл
		Если ВложенныйЭлементDOM.ТипУзла = ТипУзлаDOM.Элемент Тогда
			...
			Иначе
				ПредставленияНеобработанныхУзловDOM.Добавить(СтрШаблон(
					НСтр("ru = 'Узел ""%1"", содержимое: ""%3"" (тип: ""%2"").'"),
		...
		КонецЕсли;
	КонецЦикла;
	Результат.Вставить("ПредставленияНеобработанныхУзловDOM", ПредставленияНеобработанныхУзловDOM);
	Возврат Результат;
КонецФункции
...
//   НеобработанныеУзлы - Неопределено - Не искать прочие атрибуты.
//       - Массив - Если ИменаАтрибутов указаны, то в НеобработанныеУзлы добавляются все прочие атрибуты,
//           которые не описаны в ИменаАтрибутов.
//
// Возвращаемое значение:
...
//
Функция АтрибутыЭлементаDOM(ЭлементDOM, ИменаАтрибутов = "", НеобработанныеУзлы = Неопределено) Экспорт
	...
	Для Каждого АтрибутDOM Из ЭлементDOM.Атрибуты Цикл
		...
		ИначеЕсли НеобработанныеУзлы <> Неопределено Тогда
			НеобработанныеУзлы.Добавить(АтрибутDOM);
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции
