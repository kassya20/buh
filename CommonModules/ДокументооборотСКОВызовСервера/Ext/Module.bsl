...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	Методы.Вставить("УстановитьПараметрСеансаРезультатыОбработкиМЧДСКО", Истина);
	Методы.Вставить("УстановитьПараметрСеансаОрганизацииПринятыхЗПЭД", Истина);
	Методы.Вставить("УстановитьПараметрСеансаТекущиеУчетныеЗаписиНалогоплательщика", Истина);
	Методы.Вставить("УстановитьПараметрСеансаРезультатАвторизацииНаСервереМЧДРРСКО", Истина);
	Методы.Вставить("ЗаполнитьРегистрЖурналОтправокВКонтролирующиеОрганы");
КонецПроцедуры
...
Процедура СброситьДанныеАутентификацииНаПорталеПоддержки()
	...
	Если МодульИнтернетПоддержкаПользователей.ДоступноПодключениеИнтернетПоддержки() Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБИП, "2.9.1.51") >= 0 Тогда
			МодульИнтернетПоддержкаПользователей.Отключить();
		Иначе
...
Функция СобытиеНаступилоНовыйФорматПояснДеклНДС(ВерсияФормата = Неопределено) Экспорт
	
	Результат = Ложь;
	
	ПроверяемаяВерсияФормата = "5.04";
	Если Не ЗначениеЗаполнено(ВерсияФормата) Или ВерсияФормата = ПроверяемаяВерсияФормата Тогда
		Результат =	ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("ПояснДеклНДС_Формат_5.04", '20260512');
		Если Результат Или ЗначениеЗаполнено(ВерсияФормата) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	ПроверяемаяВерсияФормата = "5.03";
	Если Не ЗначениеЗаполнено(ВерсияФормата) Или ВерсияФормата = ПроверяемаяВерсияФормата Тогда
		Результат =	ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило(
		"ПояснДеклНДС_Формат_5.03",	"ДокументооборотСКонтролирующимиОрганами_ПоясненияКДекларацииНДС_СобытиеПояснДеклНДС_Формат_5.03");
		Если Результат Или ЗначениеЗаполнено(ВерсияФормата) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
