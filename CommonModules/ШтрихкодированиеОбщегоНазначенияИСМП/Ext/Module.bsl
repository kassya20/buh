...
Процедура ПроверитьНаОшибкиПоПравилам(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Если ПравилаПроверкиНаОшибки.Свойство("КонтролироватьСканированиеКодовОСУ")
		И ПравилаПроверкиНаОшибки.КонтролироватьСканированиеКодовОСУ.Количество() > 0 Тогда
		
		Для Каждого ЭлементДанных Из ПравилаПроверкиНаОшибки.КонтролироватьСканированиеКодовОСУ Цикл
			...
			Если ПараметрыСканирования.РасширеннаяВерсияГосИС Тогда
				...
				Если ЭлементДанных.ПроверитьДатыДействияОСУ Тогда
					...
					ВидДополнения = Модуль.ПараметрыРасширенногоОписанияОшибки();
					ВидДополнения.ВидОперацииИСМП     = ПараметрыСканирования.ВидОперацииИСМП;
					ВидДополнения.НеПоддерживаетсяОСУ = Истина;
					ВидДополнения.ВидПродукцииИС      = ЭлементДанных.СтрокаДанных.ВидПродукции;
					Модуль.ДополнитьПредставлениеОшибки(ЭлементДанных.СтрокаДанных, ВидДополнения);
		...
		ПравилаПроверкиНаОшибки.КонтролироватьСканированиеКодовОСУ.Очистить();
...
Процедура ОписаниеОшибокКонтроляЧастичногоВыбытия(ПравилаПроверкиНаОшибки, ПараметрыСканирования, ОбработкаСтрокиТаблицы = Ложь) Экспорт
	...
	Если ПравилаПроверкиНаОшибки.Свойство("ПолноеВыбытиеНевозможноРанееВыбывалЧастично")
		...
		Для Каждого СтрокаДанных Из ПравилаПроверкиНаОшибки.ПолноеВыбытиеНевозможноРанееВыбывалЧастично Цикл
			...
			ИначеЕсли СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы
				Или СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.БАДы2025
				Или СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.СпортивноеПитание Тогда
	...
	Если ПравилаПроверкиНаОшибки.Свойство("КодМаркировкиВыбывалЧастичноОперацияНевозможна")
		...
		Для Каждого СтрокаДанных Из ПравилаПроверкиНаОшибки.КодМаркировкиВыбывалЧастичноОперацияНевозможна Цикл
			...
			ИначеЕсли СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.СпортивноеПитание Тогда
				ОписаниеПроблемы = НСтр("ru = 'Операция не поддерживается. Спортивное питание ранее продавалось поштучно.'");
			ИначеЕсли СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.КреслаКоляски
				Или СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия
				Или СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.МедицинскиеИзделия20 Тогда
...
Процедура ЗаполнитьДанныеИдентификатораРозничнойПродажиПоОбязательнымТоварнымГруппам(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	Если Не ОбщегоНазначенияИСМПКлиентСервер.ЗначениеПараметраСканированияЗапрашиватьДанныеСервисаИСМП(ПараметрыСканирования)
	...
	Если РезультатОнлайнЗапроса.КодыМаркировкиБезДанныхГИСМТ.Количество() Тогда
		...
		Для Каждого СтрокаДанных Из РезультатОнлайнЗапроса.КодыМаркировкиБезДанныхГИСМТ Цикл
			...
			ПолучатьОффлайнДанныеГИСМТ = ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОффлайнПроверкеПередРозничнойПродажей(
				СтрокаДанныхПоШтрихкодам.ВидПродукции, ПараметрыСканирования.ВидОперацииИСМП, ПараметрыСканирования.ТребуетсяПроверкаСредствамиККТ);
...
Процедура УстановитьПризнакОтказаОтПроверкиКодовМаркировки(ДанныеПоШтрихкодам, ПараметрыСканирования) Экспорт
	...
	СканированиеФормыПроверкиИПодбора = ЭтоСканированиеВФормеПроверкиИПодбора(ПараметрыСканирования);
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
...
Процедура ПроверитьНаОшибкиТаблицуДанныеКодовМаркировки(ДанныеПоШтрихкодам, ПравилаПроверкиНаОшибки, ПараметрыСканирования, ЕстьОшибки)
	...
	Для Каждого СтрокаДанных Из ДанныеПоШтрихкодам.ДанныеКодовМаркировки Цикл
		...
		Если ПравилаПроверкиНаОшибки.Свойство("КонтролироватьСканированиеКодовОСУ") Тогда
			
			Если СтрокаДанных.ВидУпаковки = Перечисления.ВидыУпаковокИС.ОбъемноСортовойУчет Тогда
				
				Если Не ПараметрыСканирования.ПоддерживаетсяОбъемноСортовойУчет Тогда
					
					ЕстьОшибки = Истина;
					ПравилаПроверкиНаОшибки.КонтролироватьСканированиеКодовОСУ.Добавить(
					Новый Структура(
						"СтрокаДанных, ПроверитьДатыДействияОСУ, ТекстОшибки",
							СтрокаДанных,
							Ложь,
							НСтр("ru = 'По документу не поддерживается указание кодов ОСУ'")));
							
				ИначеЕсли Не ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоОСУ(СтрокаДанных.ВидПродукции, ПараметрыСканирования.ДатаДокумента) Тогда
					...
					Новый Структура(
						"СтрокаДанных, ПроверитьДатыДействияОСУ, ТекстОшибки",
							СтрокаДанных,
							Истина,
							СтрШаблон(НСтр("ru = 'По виду продукции %1 не доступно указание кодов ОСУ.'"), СтрокаДанных.ВидПродукции)));
...
Функция ПравилаПроверкиНаОшибки(ПараметрыСканирования, ДанныеПоШтрихкодам)
	...
	ПравилаПроверкиНаОшибки.Вставить("КонтролироватьСканированиеКодовОСУ", Новый Массив);
