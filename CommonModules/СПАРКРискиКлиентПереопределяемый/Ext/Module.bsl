...
Процедура ОбработкаОповещения(
	...
	Если Форма.ИмяФормы = "Документ.ПлатежноеПоручение.Форма.ФормаДокумента" 
			Или Форма.ИмяФормы = "Справочник.Контрагенты.Форма.ФормаЭлемента" Тогда
		// При обновлении индексов СПАРК сбрасывается вводимый в поле ввода текст. Например поле "Назначение платежа" в Платежном поручении.
		Форма.ПредотвратитьСбросРедактируемогоЗначения();
	КонецЕсли;
