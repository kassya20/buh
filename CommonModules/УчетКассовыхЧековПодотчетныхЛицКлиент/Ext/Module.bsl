...
Процедура ПодключитьОбновлениеЧековКЗагрузке(Форма, ОписаниеФормы) Экспорт
	...
	Интервал = УчетКассовыхЧековПодотчетныхЛицКлиентСервер.ИнтервалПроверкиЧековПодотчетныхЛиц();
...
Процедура ОбновитьЧекиКЗагрузке(Форма, ОписаниеФормы) Экспорт
	...
	Результат = УчетКассовыхЧековПодотчетныхЛицВызовСервера.НачатьОбновлениеЧековКЗагрузке(АдресДанныеФормы);
	
	ЗавершитьОбновлениеЧековКЗагрузке(Результат, Форма, ОписаниеФормы);
...
Процедура НачатьЗагрузкуВДокумент(Форма, ПодотчетноеЛицо, ОписаниеФормы, ОповещениеЗаполнениеДокумента) Экспорт
	...
	Если Действие = "Сканировать" Тогда
		...
		ИсточникиЧеков = УчетКассовыхЧековПодотчетныхЛицВызовСервера.ПодключенныеИсточникиКассовыхЧеков();
		ИмяПриложенияСканерЧеков = НСтр("ru = '""1С:Сканер чеков""'");
		ИмяПриложенияСканерДокументов = НСтр("ru = '""1С:Сканер документов""'");
		
		ДоступныеПриложения = ИмяПриложенияСканерЧеков;
		Если ИсточникиЧеков.ВключенСканерЧеков И ИсточникиЧеков.ВключенСканерДокументов Тогда
			ДоступныеПриложения = СтрШаблон(НСтр("ru = '%1 или %2'"), ИмяПриложенияСканерЧеков, ИмяПриложенияСканерДокументов);
		ИначеЕсли ИсточникиЧеков.ВключенСканерДокументов Тогда
			ДоступныеПриложения = ИмяПриложенияСканерДокументов;
		КонецЕсли;
		
		ТекстПредупреждения = СтрШаблон(НСтр("ru = 'Нет чеков к загрузке. Отсканируйте чеки в мобильном приложении %1'"), ДоступныеПриложения);
...
// Обрабатывает событие автоподбора поставщика.
//
// Параметры:
//  ДанныеВыбора - СписокЗначений - см. описание параметра обработчика АвтоПодбор
//  СтандартнаяОбработка - Булево - см. описание параметра обработчика АвтоПодбор
//  Параметры - Структура - см. ПараметрыАвтоПодбораПоставщикаНовый
//
Процедура ПоставщикАвтоПодбор(ДанныеВыбора, СтандартнаяОбработка, Параметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Параметры.Поставщик)
		И ПустаяСтрока(Параметры.Текст)
		И Не ПустаяСтрока(Параметры.ПоставщикНаименование)
		И Не ПустаяСтрока(Параметры.ПоставщикИНН)Тогда
		
		СтандартнаяОбработка = Ложь;
		Параметры.ПараметрыПолученияДанных.СтрокаПоиска = Параметры.ПоставщикИНН;
		ДанныеВыбора = УчетКассовыхЧековПодотчетныхЛицВызовСервера.ДанныеВыбораКонтратента(
			Параметры.ПараметрыПолученияДанных,
			Параметры.ПоставщикНаименование,
			Параметры.ПоставщикИНН);
...
// Конструктор параметров процедуры ПоставщикАвтоПодбор
// 
// Возвращаемое значение:
//  Структура - см. тело функции
//
Функция ПараметрыАвтоПодбораПоставщикаНовый() Экспорт
	...
	ПараметрыПроцедуры.Вставить("ПараметрыПолученияДанных", Новый Структура);
	ПараметрыПроцедуры.Вставить("Ожидание", 0);
	ПараметрыПроцедуры.Вставить("Поставщик", ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка"));
	ПараметрыПроцедуры.Вставить("ПоставщикНаименование", "");
	ПараметрыПроцедуры.Вставить("ПоставщикИНН", "");
...
Процедура ОбработатьОтветСканера(Форма, ОписаниеФормы, ИмяСобытия, Параметр, ИмяСписка, СуммаВключаетНДС) Экспорт
	...
	Если Форма.ВводДоступен() И ИмяСобытия = "ScanData"
		...
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("QRКод", ОбщегоНазначенияКлиентСервер.УдалитьНедопустимыеСимволыXML(ТекущийКод));
...
//                                     должна соответствовать описанию
//  ОписаниеФормы					 - Структура - см. УчетКассовыхЧековПодотчетныхЛицКлиентСервер.НовыйОписаниеФормы
//  ШтрихКод						 - Текст - строка с данными штрихкода
//  ИмяСписка						 - Строка - имя заполняемой табличной части
//  СуммаВключаетНДС				 - Булево - признак из документа СуммаВключаетНДС
//
Процедура ОбработатьОтветСканированиеШтрихкодаКамерой(Форма, ОписаниеФормы, ШтрихКод, ИмяСписка, СуммаВключаетНДС) Экспорт
	
	Если ЗначениеЗаполнено(ШтрихКод)
		И Не ПустаяСтрока(Форма[ОписаниеФормы.РеквизитСледующееДействие]) Тогда
		
		ПараметрыФормы = Новый Структура();
		ПараметрыФормы.Вставить("QRКод", ОбщегоНазначенияКлиентСервер.УдалитьНедопустимыеСимволыXML(ШтрихКод));
		ПараметрыФормы.Вставить("ИмяСписка", ИмяСписка);
		ПараметрыФормы.Вставить("СуммаВключаетНДС", СуммаВключаетНДС);
		ПараметрыФормы.Вставить("АдресДанныеФормы", Форма[ОписаниеФормы.РеквизитДанныеФормы]);
		ОповещениеОЗакрытииФормы = Новый ОписаниеОповещения(
...
// Обработчик добавления кассовых чеков в документ
//
// Параметры:
//  Форма         - УправляемаяФорма - настраиваемая форма документа, в которую можно загружать чеки подотчетных лиц,
//                                     должна соответствовать описанию
//  ОписаниеФормы   - Структура - см. УчетКассовыхЧековПодотчетныхЛицКлиентСервер.НовыйОписаниеФормы
//  ОписаниеОповещения - ОписаниеОповещения - обработчик оповещения, размещенный в форме.
//      В качестве дополнительного параметра обработчика следует передать структуру из двух элементов:
//          Строка - имя заполняемой табличной части,
//          Булево - признак из документа СуммаВключаетНДС.
//  Чеки - Массив - массив ссылок документов КассовыйЧекПодотчетногоЛица
//
Процедура ДобавитьЧекиВДокумент(Форма, ОписаниеФормы, ОписаниеОповещения, Чеки) Экспорт
	
	КонтекстЗавершенияПодбора = Новый Структура;
	КонтекстЗавершенияПодбора.Вставить("ОписаниеФормы", ОписаниеФормы);
	КонтекстЗавершенияПодбора.Вставить("АдресДанныеФормы", Форма[ОписаниеФормы.РеквизитДанныеФормы]);
	КонтекстЗавершенияПодбора.Вставить("ОповещениеЗаполнениеДокумента", ОписаниеОповещения);
	
	УчетКассовыхЧековПодотчетныхЛицКлиент.ЗавершитьПодборВДокумент(Чеки, КонтекстЗавершенияПодбора);
	
КонецПроцедуры

#КонецОбласти

#Область ПомощникПодключенияСканераЧеков

// Открывает форму ПомощникУстановкиМобильногоПриложенияСканированиеЧеков, если подключена
// Интернет-поддержка, иначе - предлагает подключить Интернет-поддержку.
//
Процедура ОткрытьПомощникПодключенияСканераЧеков() Экспорт
	
	Если УчетКассовыхЧековПодотчетныхЛицВызовСервера.ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки() Тогда
		ОткрытьФормуПомощникаПодключенияСканераЧеков();
		Возврат;
	КонецЕсли;
	...
	Если ИнтернетПоддержкаПользователейКлиент.ДоступноПодключениеИнтернетПоддержки() Тогда
		ОбработчикПродолжения = Новый ОписаниеОповещения(
			"ПодключитьИнтернетПоддержку", ЭтотОбъект);
		ТекстВопроса = НСтр("ru='Для подключения 1С:Сканер чеков
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(
		НСтр("ru = 'Интернет-поддержка не подключена. Использование Сканера чеков недоступно.'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка()));
	
КонецПроцедуры

Процедура ОткрытьФормуПомощникаПодключенияСканераЧеков(Окно = Неопределено) Экспорт
	
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		ИмяФормыПомощника = "ОбщаяФорма.ПомощникУстановкиМобильногоПриложенияСканированиеЧековДляМК";
	Иначе
		ИмяФормыПомощника = "ОбщаяФорма.ПомощникУстановкиМобильногоПриложенияСканированиеЧеков";
	КонецЕсли;
	
	ОткрытьФорму(ИмяФормыПомощника,,,,Окно);
...
Процедура ПодключитьИнтернетПоддержку(Ответ, ДополнительныеПараметры) Экспорт
	...
	ОбработчикПродолжения = Новый ОписаниеОповещения(
		"ПродолжитьОткрытьФормуПомощника", ЭтотОбъект);
	ИнтернетПоддержкаПользователейКлиент.ПодключитьИнтернетПоддержкуПользователей(ОбработчикПродолжения);
	
КонецПроцедуры

Процедура ПродолжитьОткрытьФормуПомощника(РезультатПодключения, ДополнительныеПараметры) Экспорт
...
Процедура ЗавершитьОбновлениеУсловийИспользования(Результат, ПараметрыЗавершения) Экспорт // Обработчик оповещения
	...
	ДанныеБлока = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	...
	ОбновитьБлокУсловияИспользования(Форма, ДанныеБлока);
	Форма.ОбновитьОтображениеДанных(Элементы.ГруппаУсловияИспользования);
...
// Заполняет готовыми данными форму документа.
// Если данные о чеках к загрузке изменились, обновляет сведения для подбора и баннер на форме.
//
// Параметры:
//  АдресРезультата - Строка - адрес данных во временном хранилище
//  Форма - УправляемаяФорма - настраиваемая форма документа, в которую можно загружать чеки подотчетных лиц,
//                             должна соответствовать описанию
//  ОписаниеФормы - Структура - см. УчетКассовыхЧековПодотчетныхЛицКлиентСервер.НовыйОписаниеФормы
//
Процедура ЗавершитьОбновлениеЧековКЗагрузке(АдресРезультата, Форма, ОписаниеФормы)
	
	Форма.ПодключитьОбработчикОжидания(
		ОписаниеФормы.ОбработчикОжидания,
		УчетКассовыхЧековПодотчетныхЛицКлиентСервер.ИнтервалПроверкиЧековПодотчетныхЛиц(),
		Истина);
	...
	УчетКассовыхЧековПодотчетныхЛицКлиентСервер.ОбновитьБаннерЧекиКЗагрузке(
		...
		РезультатОбновления.ВыводитьКартинкуБаннера);
...
#Область ОбщиеМетодыПомощникаПодключения

Процедура ОбновитьБлокУсловияИспользования(Форма, ПараметрыЗаполнения) Экспорт
	...
	Если Форма.РазделениеВключено Тогда
		// В режиме сервиса необходимо вывести информацио об обслуживающей организации.
		 НастроитьОбслуживающуюОрганизацию(Форма, ПараметрыЗаполнения.ОбслуживающаяОрганизация);
	КонецЕсли;
	
КонецПроцедуры

Процедура НастроитьОбслуживающуюОрганизацию(Форма, ОбслуживающаяОрганизация)
