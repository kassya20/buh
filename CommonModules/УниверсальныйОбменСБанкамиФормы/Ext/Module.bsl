...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("СинхронизироватьПрисоединенныеФайлы", Истина);
...
Процедура ЗаполнитьНаименованияБанков(СведенияОБанках) Экспорт
	...
	Если СведенияОБанках.Колонки.Найти("СтепеньСоответствия") <> Неопределено Тогда
...
Процедура ЗаполнитьОписаниеДополнительногоФайла(ОписаниеФайла, ДополнительныйФайл, Банк, НаименованиеИдентификатора,
	ИдентификаторПродукта = "") Экспорт
	...
	Если Не ПустаяСтрока(ИдентификаторПродукта) Тогда
		ОписаниеФайла[НаименованиеИдентификатора] = ИдентификаторПродукта;
	КонецЕсли;
...
Функция ЗаполнитьТекстСогласияВФоне(ПараметрыСогласия, ОбъектыСогласия, УникальныйИдентификатор) Экспорт
	...
	Если ИмяСервиса = "ЗаявкиНаОткрытиеСчета" Тогда
		ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = 
			НСтр("ru = 'Заполнение согласия на обработку данных из заявки на открытие счета'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"ЗаявкиНаОткрытиеСчета.ЗаполнитьТекстСогласияВФоне",
			ПараметрыЗаполнения,
			ПараметрыВыполненияВФоне);
	Иначе
		ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = 
			НСтр("ru = 'Заполнение согласия на обработку данных из заявки на кредит'");
		
		ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
			"ЗаявкиНаКредит.ЗаполнитьТекстСогласияВФоне",
			ПараметрыЗаполнения,
			ПараметрыВыполненияВФоне);
	КонецЕсли;
...
Функция ТекстСогласия(ПараметрыСогласия, РеквизитыОбъектов, ПараметрыФорматирования) Экспорт
	...
	ТекстыСогласий.Колонки.Добавить("ВидСогласия", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыСогласий"));

	ТекстыСогласий.Индексы.Добавить("Банк, ИдентификаторПродукта, ВидСогласия");
	...
	Для каждого ИнформацияОбОбъекте Из РеквизитыОбъектов Цикл
		...
		Для каждого ПараметрМакета Из ИнформацияОбОбъекте.ПараметрыМакета Цикл
			...
			НеВыделятьЗначенияПараметров = СтрНайти(ПараметрМакета.ЗначениеПараметра, "<li>") > 0;

			Если Не НеВыделятьЗначенияПараметров И ПараметрыФорматирования.ВыделятьЗначенияПараметров Тогда
		...
		КонецЕсли; 
		Если РеквизитыОбъектов.Колонки.Найти("ВидСогласия") <> Неопределено Тогда
			НоваяСтрока.ВидСогласия = ИнформацияОбОбъекте.ВидСогласия;
		КонецЕсли;
...
Функция ДанныеФизлицаДляВыгрузки(ДанныеЗаявки, ФизЛицо) Экспорт
...
Функция РеквизитыФизическогоЛица(ФизЛицо, Дата) Экспорт
...
Процедура ДобавитьКонтактнуюИнформацияФизЛица(ДанныеФизЛица, ФизЛицоОтКлиента, ВидыКонтактнойИнформации, ДатаЗаявки) Экспорт
	...
	КонтактнаяИнформация = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъектов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ФизЛицоОтКлиента),
		,
		ВидыКИ,
		ДатаЗаявки);
...
Функция ЗначениеПоказателяИзКэша(Дата, Организация, ИмяПоказателя) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(КэшПоказателейФинУслуги.Значение, 0) КАК Значение
	|ИЗ
	|	РегистрСведений.КэшПоказателейФинУслуги КАК КэшПоказателейФинУслуги
	|ГДЕ
	|	НАЧАЛОПЕРИОДА(КэшПоказателейФинУслуги.ДатаАктуальности, ДЕНЬ) = &ДатаАктуальности
	|	И КэшПоказателейФинУслуги.Организация = &Организация
	|	И КэшПоказателейФинУслуги.ИмяПоказателя = &ИмяПоказателя";
	Запрос.УстановитьПараметр("ДатаАктуальности", НачалоДня(Дата));
...
Функция ДатаФактическогоНачалаВеденияБизнеса(Организация, Дата) Экспорт
	...
	СчетаУчетаДС.Добавить(ПланыСчетов.Хозрасчетный.ЦифровойРубль);  // 53
...
Функция ВыручкаОрганизации(Дата, Организация, КоличествоМесяцев) Экспорт
...
Функция ПриближениеСрокаПлатежа(Дата, Организация) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		Возврат 0;
	КонецЕсли;
	
	ИмяПоказателя = "ФинДир.СкороПлатежНерез";
	
	Значение = ЗначениеПоказателяИзКэша(Дата, Организация, ИмяПоказателя);
	...
	|ПОМЕСТИТЬ ИноКонтрагенты
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.СтранаРегистрации.Код <> ""643""
	...
	|				И Субконто1 В
	|					(ВЫБРАТЬ
	|						ИноКонтрагенты.Ссылка
	|					ИЗ
	|						ИноКонтрагенты КАК ИноКонтрагенты)
	|				И ТИПЗНАЧЕНИЯ(Субконто2) = ТИП(Документ.РеализацияТоваровУслуг)) КАК ХозрасчетныйОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	...
	|			ТОГДА РАЗНОСТЬДАТ(&ТекущаяДата, СрокиОплатыДокументов.СрокОплаты, ДЕНЬ)
	|		КОГДА ДоговорыКонтрагентов.УстановленСрокОплаты
	|			ТОГДА ВЫБОР
	|					КОГДА ДоговорыКонтрагентов.ДатаОплаты <> ДАТАВРЕМЯ(1, 1, 1)
	|						ТОГДА РАЗНОСТЬДАТ(&ТекущаяДата, ДоговорыКонтрагентов.ДатаОплаты, ДЕНЬ)
	|					КОГДА ДоговорыКонтрагентов.СрокОплаты > 0
	|						ТОГДА РАЗНОСТЬДАТ(&ТекущаяДата, ДОБАВИТЬКДАТЕ(РеализацияТоваровУслуг.Дата, ДЕНЬ, ДоговорыКонтрагентов.СрокОплаты), ДЕНЬ)
	|					КОГДА ДоговорыКонтрагентов.СрокОплаты = 0
	|						ТОГДА 0
	|				КОНЕЦ
	|		ИНАЧЕ 0
	...
	|		ПО Остатки.ДокументРасчетов = РеализацияТоваровУслуг.Ссылка
	|			И Остатки.Организация = РеализацияТоваровУслуг.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СрокиОплатыДокументов КАК СрокиОплатыДокументов
	|		ПО Остатки.Организация = СрокиОплатыДокументов.Организация
	|			И Остатки.ДокументРасчетов = СрокиОплатыДокументов.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов КАК ДоговорыКонтрагентов
	|		ПО (РеализацияТоваровУслуг.ДоговорКонтрагента = ДоговорыКонтрагентов.Ссылка)
	|ГДЕ
	|	РеализацияТоваровУслуг.Организация В(&Организация)
	|	И ВЫБОР
	|			КОГДА РеализацияТоваровУслуг.ВалютаДокумента = Остатки.Валюта
	|				ТОГДА РеализацияТоваровУслуг.СуммаДокумента = Остатки.ВалютнаяСуммаОстаток
	|			ИНАЧЕ РеализацияТоваровУслуг.СуммаДокумента = Остатки.СуммаОстаток
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ СрокиОплатыДокументов.СрокОплаты ЕСТЬ NULL
	|				ТОГДА РАЗНОСТЬДАТ(&ТекущаяДата, СрокиОплатыДокументов.СрокОплаты, ДЕНЬ)
	...
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСПокупателямиИЗаказчиками); // 62
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСРазнымиДебиторамиИКредиторами); // 76
	
	Запрос.УстановитьПараметр("ТекущаяДата", Дата);
	Запрос.УстановитьПараметр("Организация", ОбщегоНазначенияБПВызовСервераПовтИсп.ВсяОрганизация(Организация));
	Запрос.УстановитьПараметр("НаДату", Дата);
	Запрос.УстановитьПараметр("Счет", БухгалтерскийУчет.СформироватьМассивСубсчетов(МассивСчетов));
	...
	СрокОплаты = 99999; // Данное значение записывается в регистр КэшПоказателейФинУслуги при пустом запросе.
	
	Если Выборка.Следующий() Тогда
		Если Выборка.СрокОплаты >= 0 Тогда
			СрокОплаты = Выборка.СрокОплаты;
		КонецЕсли;
	КонецЕсли;
	
	РегистрыСведений.КэшПоказателейФинУслуги.ЗаписатьВРегистрПоказателиФинУслуги(СрокОплаты,
		Дата, Организация, ИмяПоказателя);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция РасходыПереоценка(Дата, Организация) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Организация) Тогда
		Возврат 0;
	КонецЕсли;

	ИмяПоказателя = "ФинДир.РасходыПереоценка";
	...
	|				КОГДА ХозрасчетныйОбороты.Субконто1 = &ДоходыРасходыПриПродажеПокупкеВалюты
	|					ТОГДА ВЫБОР
	|							КОГДА ХозрасчетныйОбороты.СуммаОборотДт - ХозрасчетныйОбороты.СуммаОборотКт > 0
	...
	|			,
	|			Организация В (&Организация)
	|				И Субконто1 В (&МассивСтатейПрочихРасходов),
	|			КорСчет В (&КорСчет),
	|			) КАК ХозрасчетныйОбороты";
	
	МассивСчетов = Новый Массив;
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.ПрочиеДоходыИРасходы); // 91
	
	МассивСтатейПрочихРасходов = Новый Массив;
	МассивСтатейПрочихРасходов.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.ДоходыРасходыПриПродажеПокупкеВалюты"));
	МассивСтатейПрочихРасходов.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.КурсовыеРазницы"));
	МассивСтатейПрочихРасходов.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.КурсовыеРазницыПоРасчетамВУЕ"));
	МассивСтатейПрочихРасходов.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.ОтклонениеКурсаПродажиПокупкиВалюты"));
	
	МассивКорСчетов = Новый Массив;
	МассивКорСчетов.Добавить(ПланыСчетов.Хозрасчетный.ВалютныеСчета); // 52
	МассивКорСчетов.Добавить(ПланыСчетов.Хозрасчетный.СпециальныеСчета); // 55
	МассивКорСчетов.Добавить(ПланыСчетов.Хозрасчетный.ПереводыВПути_); // 57
	МассивКорСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоКраткосрочнымКредитамИЗаймам); // 66
	МассивКорСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоДолгосрочнымКредитамИЗаймам); // 67
	
	Запрос.УстановитьПараметр("Организация", ОбщегоНазначенияБПВызовСервераПовтИсп.ВсяОрганизация(Организация));
	Запрос.УстановитьПараметр("ДатаНачала", НачалоМесяца(ДобавитьМесяц(Дата, -1)));
	Запрос.УстановитьПараметр("ДатаОкончания", КонецМесяца(ДобавитьМесяц(Дата, -1)));
	Запрос.УстановитьПараметр("Счет", БухгалтерскийУчет.СформироватьМассивСубсчетов(МассивСчетов)); 
	Запрос.УстановитьПараметр("КорСчет", БухгалтерскийУчет.СформироватьМассивСубсчетов(МассивКорСчетов));
	Запрос.УстановитьПараметр("МассивСтатейПрочихРасходов", МассивСтатейПрочихРасходов);
	Запрос.УстановитьПараметр("ДоходыРасходыПриПродажеПокупкеВалюты", ОбщегоНазначения.ПредопределенныйЭлемент(
		"Справочник.ПрочиеДоходыИРасходы.ДоходыРасходыПриПродажеПокупкеВалюты"));
	...
	СуммаОборот = 0;
	Если Результат.Количество() > 0 Тогда
		СуммаОборот = Результат[0].СуммаОборот;
	КонецЕсли;
	
	РегистрыСведений.КэшПоказателейФинУслуги.ЗаписатьВРегистрПоказателиФинУслуги(СуммаОборот,
		Дата, Организация, ИмяПоказателя);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ТаможеннаяСтоимость(Дата, Организация) Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ВедетсяУчетИмпортаТоваров") Тогда
		Возврат 0;
	КонецЕсли;
	...
	ИмяПоказателя = "ТаможеннаяСтоимость";
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СУММА(ГТДИмпорт.ТаможеннаяСтоимость) КАК ТаможеннаяСтоимость
	|ИЗ
	|	Документ.ГТДИмпорт.Разделы КАК ГТДИмпорт
	|ГДЕ
	|	ГТДИмпорт.Ссылка.Проведен
	|	И ГТДИмпорт.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	|	И ГТДИмпорт.Ссылка.Организация В (&Организация)
	|
	|ИМЕЮЩИЕ
	|	НЕ СУММА(ГТДИмпорт.ТаможеннаяСтоимость) ЕСТЬ NULL";
	
	Запрос.УстановитьПараметр("ДатаНачала", НачалоМесяца(ДобавитьМесяц(Дата, -6)));
	Запрос.УстановитьПараметр("ДатаОкончания", КонецМесяца(ДобавитьМесяц(Дата, -1)));
	Запрос.УстановитьПараметр("Организация", ОбщегоНазначенияБПВызовСервераПовтИсп.ВсяОрганизация(Организация));
	...
	Если Выборка.Следующий() Тогда
		ТаможеннаяСтоимость = Выборка.ТаможеннаяСтоимость;
	КонецЕсли;
	
	РегистрыСведений.КэшПоказателейФинУслуги.ЗаписатьВРегистрПоказателиФинУслуги(ТаможеннаяСтоимость,
		Дата, Организация, ИмяПоказателя);
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат ТаможеннаяСтоимость;
