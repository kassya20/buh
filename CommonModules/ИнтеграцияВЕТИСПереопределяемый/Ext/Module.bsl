...
Процедура ОбработкаПроверкиЗаполнения(ДокументОбъект, Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов) Экспорт
	...
	ИнтеграцияВЕТИСБПВызовСервера.ПроверитьЗаполнениеТранзакцииВЕТИС(ДокументОбъект, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, Отказ);
...
Процедура ПередЗаписью(ДокументОбъект, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПроизводственнаяОперацияВЕТИС") Тогда
		...
		//Производственные транзакции
		ИнтеграцияВЕТИСБПВызовСервера.ПроверитьИЗаписатьПроизводственнуюТранзакцию(ДокументОбъект, РежимЗаписи, Отказ);
...
Процедура УстановитьПараметрыВыбораНоменклатуры(Форма, ИмяПоляВвода = "ТоварыНоменклатура") Экспорт
	...
	ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.ВидМаркировки", Перечисления.ВидыПродукцииИС.ПодконтрольнаяПродукцияВЕТИС));
...
Процедура ЗаполнитьКоличествоНоменклатурыПоКоличествуВЕТИС(ТекущаяСтрока, Суффикс = "") Экспорт
	...
	ИмяРеквизитаКоличествоВетис = "Количество" + Суффикс + "ВЕТИС";
	
	СтруктураРеквизитов = Новый Структура;
	СтруктураРеквизитов.Вставить("Номенклатура");
	СтруктураРеквизитов.Вставить(ИмяРеквизитаКоличествоВетис);
	...
	Количество = ИнтеграцияВЕТИСКлиентСерверПереопределяемый.ПересчитатьКоличествоЕдиниц(
		СтруктураРеквизитов[ИмяРеквизитаКоличествоВетис], 
...
Процедура ПриОпределенииЗапросаМаркируемойПродукции(Шаблон, ТаблицаИсточник, ИмяПоляНоменклатура, ТаблицаПриемник) Экспорт
	...
	|	ПО СправочникНоменклатура.Ссылка = %1.%2
	|ГДЕ
	|	СправочникНоменклатура.ВидМаркировки = ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.МолочнаяПродукцияПодконтрольнаяВЕТИС)
	|;
	|
	|//////////////////////////////////////////////////
	|";
...
Процедура ПриОпределенииРеквизитовТранспортногоСредства(Реквизиты, ТранспортноеСредство) Экспорт
	...
	Если Реквизиты.Свойство("НомерТранспортногоСредства") Тогда
		Реквизиты.НомерТранспортногоСредства = ТранспортноеСредство.РегистрационныйЗнак;
	КонецЕсли;
	
	Если Реквизиты.Свойство("НомерАвтомобильногоПрицепа") Тогда
		Реквизиты.НомерАвтомобильногоПрицепа = ТранспортноеСредство.РегистрационныйЗнакПрицепа;
    КонецЕсли;
...
Процедура ПриСохраненииЗначенияРеквизита(ИмяРеквизита, ЗначениеРеквизита) Экспорт
	
	ИскомаяКонстанта = Метаданные.Константы.Найти(ИмяРеквизита);
	Если ИскомаяКонстанта <> Неопределено Тогда
		Если ИскомаяКонстанта.Тип.СодержитТип(ТипЗнч(ЗначениеРеквизита)) Тогда
			Константы[ИскомаяКонстанта.Имя].Установить(ЗначениеРеквизита);
		КонецЕсли;
	КонецЕсли;
