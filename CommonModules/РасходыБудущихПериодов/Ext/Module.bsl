...
Функция РаспределитьЧастьРасходовЗаМесяц(Период, Организация, Отказ, Ошибки = Неопределено) Экспорт
	...
	СчетаУчетаНазначенияИспользованияНМА = БухгалтерскийУчет.СчетаУчетаНазначенияИспользованияНМА();
	
	// Исключим счета без субконто статей РБП
	МассивИсключаемыхСчетов = Новый Массив;
	МассивИсключаемыхСчетов.Добавить(ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС);
	МассивИсключаемыхСчетов.Добавить(ПланыСчетов.Хозрасчетный.УбыткиПрошлыхЛет);
	УсловияОтбора = БухгалтерскийУчет.НовыеУсловияОтбораСубсчетов();
	УсловияОтбора.СчетаИсключения = МассивИсключаемыхСчетов;
	
	СчетаУчетаРБП = БухгалтерскийУчет.СформироватьМассивСубсчетовПоОтбору(СчетаУчетаРБП, УсловияОтбора);   // добавляем субсчета    
	...
	Запрос.УстановитьПараметр("КонецМесяцаГраница",              Новый Граница(КонецМесяца(Период), ВидГраницы.Включая));
	...
	Запрос.УстановитьПараметр("СчетаУчетаНазначенияИспользованияНМА", СчетаУчетаНазначенияИспользованияНМА);
	...
	|ПОМЕСТИТЬ Сальдо
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецМесяцаГраница, Счет В (&Счета), , Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто1 КАК Справочник.НазначенияИспользованияНМА)
	|		КОГДА ХозрасчетныйОстатки.Субконто2 ССЫЛКА Справочник.НазначенияИспользованияНМА
	|			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто2 КАК Справочник.НазначенияИспользованияНМА)
	|		КОГДА ХозрасчетныйОстатки.Субконто3 ССЫЛКА Справочник.НазначенияИспользованияНМА
	|			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйОстатки.Субконто3 КАК Справочник.НазначенияИспользованияНМА)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.НазначенияИспользованияНМА.ПустаяСсылка)
	|	КОНЕЦ КАК НазначениеИспользования
	|ПОМЕСТИТЬ СальдоРезультатыИнтеллектуальнойДеятельности
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&КонецМесяцаГраница, Счет В (&СчетаУчетаНазначенияИспользованияНМА), , Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОстатки.Счет,
	|	ХозрасчетныйОстатки.Субконто1,
	...
	|	СУММА(ХозрасчетныйОстатки.СуммаПРОстаток) КАК СуммаПР,
	|	СУММА(ХозрасчетныйОстатки.СуммаВРОстаток) КАК СуммаВР
	|ПОМЕСТИТЬ СальдоОсновныеСредства
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&НачалоМесяца, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.УбыткиОтРеализацииОС), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства), Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОстатки.Счет,
	|	ХозрасчетныйОстатки.Субконто1,
	...
	|	МАКСИМУМ(СведенияОС.ДатаПринятияКУчету) КАК ДатаСостоянияПринятоКУчету
	|ПОМЕСТИТЬ ОсновныеСредстваДатаПринятияКУчету
	|ИЗ
	|	(ВЫБРАТЬ
	|		СостоянияОСОрганизацийПринятоКУчету.ОсновноеСредство КАК ОсновноеСредство,
	|		СостоянияОСОрганизацийПринятоКУчету.ДатаСостояния КАК ДатаПринятияКУчету
	|	ИЗ
	|		РегистрСведений.СостоянияОСОрганизаций.СрезПоследних(
	|				&НачалоМесяца,
	|				ОсновноеСредство В
	|						(ВЫБРАТЬ
	|							СальдоОсновныеСредства.ОсновноеСредство КАК ОсновноеСредство
	|						ИЗ
	|							СальдоОсновныеСредства КАК СальдоОсновныеСредства)
	|					И Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету)
	|					И Организация = &Организация) КАК СостоянияОСОрганизацийПринятоКУчету
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|						ИЗ
	|							СальдоОсновныеСредства КАК СальдоОсновныеСредства)
	|					И Событие.ВидСобытияОС = ЗНАЧЕНИЕ(Перечисление.ВидыСобытийОС.ПереходПраваСобственности)
	|					И Организация = &Организация) КАК СобытияОСОрганизаций) КАК СведенияОС
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СостоянияОСОрганизацийСнятоСУчета.ОсновноеСредство КАК ОсновноеСредство,
	|	СостоянияОСОрганизацийСнятоСУчета.ДатаСостояния КАК ДатаСостоянияСнятоСУчета
	|ПОМЕСТИТЬ ОсновныеСредстваДатаСнятияСУчета
	|ИЗ
	|	РегистрСведений.СостоянияОСОрганизаций.СрезПоследних(
	|			&НачалоМесяца,
	|			ОсновноеСредство В
	...
	|				И Организация = &Организация) КАК СостоянияОСОрганизацийСнятоСУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	ЕСТЬNULL(СпециальныйКоэффициентНалоговыйУчет.СпециальныйКоэффициент, 0) КАК Коэффициент
	|ПОМЕСТИТЬ ОсновныеСредстваСрокПолезногоИспользования
	|ИЗ
	...
	|					ИЗ
	|						СальдоОсновныеСредства КАК СальдоОсновныеСредства)
	|				И Организация = &Организация) КАК ПараметрыАмортизацииОС
	...
	|						ИЗ
	|							СальдоОсновныеСредства КАК СальдоОсновныеСредства)
	|					И Организация = &Организация) КАК СпециальныйКоэффициентНалоговыйУчет
	|		ПО ПараметрыАмортизацииОС.ОсновноеСредство = СпециальныйКоэффициентНалоговыйУчет.ОсновноеСредство
	|			И ПараметрыАмортизацииОС.Организация = СпециальныйКоэффициентНалоговыйУчет.Организация
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ХозрасчетныйОбороты.СуммаНУОборотДт КАК СуммаУбытка,
	|	ХозрасчетныйОбороты.Субконто1 КАК ОсновноеСредство
	|ПОМЕСТИТЬ ОсновныеСредстваСуммаУбытка
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(
	|			,
	|			&НачалоМесяца,
	|			,
	|			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.УбыткиОтРеализацииОС),
	...
	|ВЫБРАТЬ
	|	Сальдо.Счет,
	|	НазначенияИспользования.Подразделение,
	|	Сальдо.Субконто1,
	|	Сальдо.Субконто2,
	|	Сальдо.Субконто3,
	|	Сальдо.Сумма,
	...
	|	Сальдо.СуммаПР,
	|	Сальдо.СуммаВР,
	|	Сальдо.НазначениеИспользования,
	...
	|	НазначенияИспользования.Счет,
	|	НазначенияИспользования.Субконто1,
	|	НазначенияИспользования.Субконто2,
	|	НазначенияИспользования.Субконто3,
	|	0,
	|	ВЫБОР
	|		КОГДА СчетаГдеОбязательныПодразделения.Счет ЕСТЬ NULL
	|				ИЛИ НазначенияИспользования.Подразделение <> &ПустоеПодразделение
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ,
	...
	|ИЗ
	|	СальдоРезультатыИнтеллектуальнойДеятельности КАК Сальдо
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НазначенияИспользованияНМА КАК НазначенияИспользования
	|			ЛЕВОЕ СОЕДИНЕНИЕ СчетаГдеОбязательныПодразделения КАК СчетаГдеОбязательныПодразделения
	|			ПО НазначенияИспользования.Счет = СчетаГдеОбязательныПодразделения.Счет
	|			ЛЕВОЕ СОЕДИНЕНИЕ СчетаГдеОбязательныНоменклатурныеГруппы КАК СчетаГдеОбязательныНоменклатурныеГруппы
	|			ПО НазначенияИспользования.Счет = СчетаГдеОбязательныНоменклатурныеГруппы.Счет
	|			ЛЕВОЕ СОЕДИНЕНИЕ СчетаГдеОбязательныСтатьиЗатрат КАК СчетаГдеОбязательныСтатьиЗатрат
	|			ПО НазначенияИспользования.Счет = СчетаГдеОбязательныСтатьиЗатрат.Счет
	|		ПО Сальдо.НазначениеИспользования = НазначенияИспользования.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Сальдо.СуммаВР,
	|	Сальдо.ОсновноеСредство,
	|	ЗНАЧЕНИЕ(Перечисление.СпособыПризнанияРасходов.ПоМесяцам),
	|	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ОсновныеСредстваДатаСнятияСУчета.ДатаСостоянияСнятоСУчета, МЕСЯЦ, 1), МЕСЯЦ),
	|	КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(ОсновныеСредстваДатаПринятияКУчету.ДатаСостоянияПринятоКУчету, МЕСЯЦ, ВЫБОР
	|				КОГДА ОсновныеСредстваСрокПолезногоИспользования.Коэффициент = 0
	|					ТОГДА ОсновныеСредстваСрокПолезногоИспользования.СрокПолезногоИспользования
	|				ИНАЧЕ ВЫБОР
	|						КОГДА ОсновныеСредстваСрокПолезногоИспользования.СрокПолезногоИспользования / ОсновныеСредстваСрокПолезногоИспользования.Коэффициент = (ВЫРАЗИТЬ(ОсновныеСредстваСрокПолезногоИспользования.СрокПолезногоИспользования / ОсновныеСредстваСрокПолезногоИспользования.Коэффициент КАК ЧИСЛО(10, 0)))
	|							ТОГДА ОсновныеСредстваСрокПолезногоИспользования.СрокПолезногоИспользования / ОсновныеСредстваСрокПолезногоИспользования.Коэффициент
	|						ИНАЧЕ ОКР(ОсновныеСредстваСрокПолезногоИспользования.СрокПолезногоИспользования / ОсновныеСредстваСрокПолезногоИспользования.Коэффициент + 0.5, 0)
	|					КОНЕЦ
	|			КОНЕЦ), МЕСЯЦ),
	...
	|	Сальдо.ОсновноеСредство,
	|	НЕОПРЕДЕЛЕНО,
	|	ОсновныеСредстваСуммаУбытка.СуммаУбытка,
	|	ЛОЖЬ,
	|	0,
	|	0
	|ИЗ
	|	СальдоОсновныеСредства КАК Сальдо
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОсновныеСредстваДатаПринятияКУчету КАК ОсновныеСредстваДатаПринятияКУчету
	|		ПО Сальдо.ОсновноеСредство = ОсновныеСредстваДатаПринятияКУчету.ОсновноеСредство
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОсновныеСредстваДатаСнятияСУчета КАК ОсновныеСредстваДатаСнятияСУчета
	|		ПО Сальдо.ОсновноеСредство = ОсновныеСредстваДатаСнятияСУчета.ОсновноеСредство
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОсновныеСредстваСрокПолезногоИспользования КАК ОсновныеСредстваСрокПолезногоИспользования
	|		ПО Сальдо.ОсновноеСредство = ОсновныеСредстваСрокПолезногоИспользования.ОсновноеСредство
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОсновныеСредстваСуммаУбытка КАК ОсновныеСредстваСуммаУбытка
	|		ПО Сальдо.ОсновноеСредство = ОсновныеСредстваСуммаУбытка.ОсновноеСредство
	|
	|УПОРЯДОЧИТЬ ПО
	|	Счет,
	|	Субконто1,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если КоллекцииСумм.Пустая(СуммыРасходов, ОписаниеСумм) Тогда
			Если ТипЗнч(Выборка.СтатьяРБП) = Тип("СправочникСсылка.ОсновныеСредства")
				И Выборка.Счет = ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС
				И Период > Выборка.ДатаОкончанияСписания И Выборка.СуммаНУ <> 0 Тогда
				// Обнаружены несписанные суммы НУ убытка ОС, например при переходе на расчет по коэффицентам (сокращения СПИ)
				СуммыРасходов.СуммаНУ = Выборка.СуммаНУ;
				СуммыРасходов.СуммаВР = Выборка.СуммаВР;
			Иначе
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если Выборка.Счет = ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС Тогда
			СтрокаЗаписьРасчетаРБП.СубконтоЗатрат1 = Справочники.ПрочиеДоходыИРасходы.ПредопределенныйЭлемент("РеализацияОсновныхСредств");
		Иначе
...
Процедура СформироватьДвиженияРасчетСписанияРБП(РасчетСписанияРБП, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого СтрокаРасчетРБП Из Параметры.ТаблицаРасчетСписанияРБП Цикл
		...
		ЗапрещеноУчитыватьРасход = НалоговыйУчет.ЗапрещеноУчитыватьРасход(Проводка);
		Если ЗапрещеноУчитыватьРасход Тогда
...
// Возвращает описание расхода будущих периодов.
//
// Параметры:
...
// 
// Возвращаемое значение:
//  Структура - см. НовыйОписаниеРасходаБудущихПериодов
//
Функция ОписаниеРасходаБудущихПериодов(РасходБудущихПериодов) Экспорт
	
	Описание = НовыйОписаниеРасходаБудущихПериодов();
	...
	|ГДЕ
	|	РасходыБудущихПериодов.Ссылка = &РасходБудущихПериодов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НазначенияИспользованияНМА.ДатаНачалаСписания,
	|	НазначенияИспользованияНМА.ДатаОкончанияСписания,
	|	ЗНАЧЕНИЕ(Перечисление.СпособыПризнанияРасходов.ПоКалендарнымДням)
	|ИЗ
	|	Справочник.НазначенияИспользованияНМА КАК НазначенияИспользованияНМА
	|ГДЕ
	|	НазначенияИспользованияНМА.Ссылка = &РасходБудущихПериодов";
...
Функция НовыйРасчетСписанияРБП()
	...
	МассивСоставногоТипа.Добавить(Новый ОписаниеТипов("СправочникСсылка.РасходыБудущихПериодов"));
	МассивСоставногоТипа.Добавить(Новый ОписаниеТипов("СправочникСсылка.ОсновныеСредства"));
	МассивСоставногоТипа.Добавить(Новый ОписаниеТипов("СправочникСсылка.НазначенияИспользованияНМА"));
	
	Расчет.Колонки.Добавить("Статья",                   Новый ОписаниеТипов(МассивСоставногоТипа));
...
// Конструктор описания расхода будущих периодов.
// 
// Возвращаемое значение:
//  Структура:
...
//   * СпособПризнанияРасходов - ПеречислениеСсылка.СпособыПризнанияРасходов
//
Функция НовыйОписаниеРасходаБудущихПериодов()

	Описание = Новый Структура;
	Описание.Вставить("ДатаНачалаСписания", Дата(1, 1, 1));
	Описание.Вставить("ДатаОкончанияСписания", Дата(1, 1, 1));
	Описание.Вставить("СпособПризнанияРасходов", Перечисления.СпособыПризнанияРасходов.ПустаяСсылка());
