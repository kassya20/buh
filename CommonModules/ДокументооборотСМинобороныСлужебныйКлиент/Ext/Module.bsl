...
Процедура ПолучитьСвойстваПрограммыМенеджераКриптографии(ОповещениеОбратногоВызова, Сертификат = Неопределено, УниверсальныйКриптопровайдер=Ложь) Экспорт
	...
	ЭтоLinux = ЭтоКриптопровайдерLinux();
	ПутьМодуляКриптографии = "";
	Если ЭтоLinux Тогда
		
		ПутьМодуляКриптографии = ДокументооборотСМинобороныКлиентСервер.ПутьМодуляКриптографии();
		Если НЕ ЗначениеЗаполнено(ПутьМодуляКриптографии) Тогда
			...
			Возврат;
		КонецЕсли;
		Криптопровайдер = ДокументооборотСМинобороныКлиентСервер.КриптопровайдерCryptoPro(, ЭтоLinux, , ПутьМодуляКриптографии);
