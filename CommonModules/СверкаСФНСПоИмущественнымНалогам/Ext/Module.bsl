...
//
// Параметры:
//  Сообщение - ДокументСсылка.ДокументРеализацииПолномочийНалоговыхОрганов - Входящее сообщение от ФНС
...
Процедура ПослеЗаписиДокументаРеализацииПолномочийНалоговыхОрганов(Сообщение, Файлы) Экспорт
	...
	ТекстОшибки = "";
	СведенияОРасчитанныхНалогах = СведенияОРасчитанныхНалогахИзФайлов(Файлы,, ТекстОшибки);
	
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Загрузка сообщения об исчисленных суммах имущественных налогов'"),
			УровеньЖурналаРегистрации.Предупреждение,
			Метаданные.Справочники.ДокументыРеализацииПолномочийНалоговыхОрганов, Сообщение, ТекстОшибки);
	КонецЕсли;
...
// Собирает подробности о содержимом файлах с расчетом налогов.
// Используется как первая итерация обработки содержимого файлов. 
//
// Параметры:
//  Файлы - Массив структур - массив описаний вложений документа - структура 
//       * Адрес - Строка - адрес файла во временном хранилище
//         или Данные - ДвоичныеДанные или ВременноеХранилище - данные файла
//       * ИмяФайла - Строка - Имя файла с расширением
//  Отбор - Структура - отбор данных из файлов. 
//          Возможные свойства:
//          * Организация
//          * Налог
//          * НалоговыйПериод 
...
//
Функция СведенияОРасчитанныхНалогахИзФайлов(Файлы, Отбор = Неопределено, ТекстОшибки = "") Экспорт 
	
	СведенияОРасчитанныхНалогах = Новый Структура;
	СведенияОРасчитанныхНалогах.Вставить("РассчитанныеНалоги", Новый ТаблицаЗначений); // данные, сгруппированные по налогам и периодам
	СведенияОРасчитанныхНалогах.Вставить("ИмяПервогоФайла", ""); // имя файла, записанное в атрибут ИдФайл первого файла
	
	РазобранныеСведенияОРасчетеНалога = Новый ТаблицаЗначений; // "сырые" данные
	РассчитанныеНалоги = СведенияОРасчитанныхНалогах.РассчитанныеНалоги; 
	...
	ВозможныеНачалаИмениФайла = СверкаСФНСПоИмущественнымНалогамФормыКлиентСервер.ВозможныеНачалаИмениФайлаРасчета(
		ОтборНалог,
		?(ОтборНалоговыйПериод = Неопределено, '00010101', ОтборНалоговыйПериод));
	...
	Для Каждого Файл Из Файлы Цикл
		...
		Если ПустаяСтрока(СведенияОРасчитанныхНалогах.ИмяПервогоФайла) Тогда
			СведенияОРасчитанныхНалогах.ИмяПервогоФайла = ИдФайл;
		КонецЕсли;	
		...
		Если ЧастиИмени.Количество() > 2 Тогда
			...
			Организация = Справочники.Организации.НайтиОрганизацию(ИННПолучателя);
		КонецЕсли; 
	...
	Если РазобранныеСведенияОРасчетеНалога.Количество() = 0 Тогда
		Возврат СведенияОРасчитанныхНалогах;
	КонецЕсли;	
	...
	СведенияОРасчитанныхНалогах.РассчитанныеНалоги = РазобранныеСведенияОРасчетеНалога.Скопировать(, ИзмеренияКонтроляУникальности);
	РассчитанныеНалоги = СведенияОРасчитанныхНалогах.РассчитанныеНалоги;
...
//                   см. УведомлениеОСпецрежимахНалогообложения.СформироватьДеревоЗагрузки()
//  Налог - ПеречислениеСсылка.ВидыИмущественныхНалогов - налог, который нужно выбрать из файла
// 
// Возвращаемое значение:
//  Массив - массив строк дерева значений с расчетом налога 
//
...
//
// Параметры:
//  Организация - СправочникСсылка.Организации - владелец объекта налогобложения
...
//  * ОсновноеСредство - СправочникСсылка.ОсновныеСредства - объект налогообложения
//  * Идентификатор - Строка - регистрационный номер ТС или кадастровый номер недвижимости (участка)
//  * ВидТС - ПеречислениеСсылка.ВидыТранспортныхСредств - вид ТС (только для транспортного налога)
//
Функция ИдентификаторыОбъектовНалогообложения(Организация, Налог, ПоискПоVIN = Ложь) Экспорт 
	...
	Если Налог = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		...
		|	РегистрацияТранспортныхСредств.ВидТранспортногоСредства КАК ВидТС,
	...
	ИначеЕсли Налог = Перечисления.ВидыИмущественныхНалогов.НалогНаИмущество Тогда
		...
		|ИЗ
		|	РегистрСведений.СтавкиНалогаНаИмуществоПоОтдельнымОсновнымСредствам КАК РегистрацияИмущества
		|ГДЕ
...
//  ЗначенияКлюча - Структура - значения полей, по которым сопоставляются объекты налогобложения при сверке расчетов
//  Налог - ПеречислениеСсылка.ВидыИмущественныхНалогов - налог, которым облагаются основные средства
//  ПоискПоVIN - Булево - нужно искать ТС по VIN (имеет смысл только для транспортного налога)
// 
// Возвращаемое значение:
...
Процедура УстановитьОсновноеСредствоПоИдентификатору(СтрокаСверки, ИдентификаторыОбъектов, ЗначенияКлюча, Налог, ПоискПоVIN = Ложь) Экспорт 
	...
	Если СтрокаОС = Неопределено Тогда
		СтрокаСверки.ОсновноеСредство = Справочники.ОсновныеСредства.ПустаяСсылка();
		Возврат;
	КонецЕсли;

	Если Налог = Перечисления.ВидыИмущественныхНалогов.ТранспортныйНалог Тогда
		СтрокаСверки.ВидТранспортногоСредства = СтрокаОС.ВидТС;
	Иначе
