...
Асинх Процедура ВыбратьСтатусОтправкиИзСписка(ТекущиеДанные, ТекСтатус, СсылкаНаОбъект, ЭтоОтчет, Форма) Экспорт
	...
	Если ВыбСтатусЭлемент <> Неопределено И ВыбСтатусЭлемент <> ТекЭлемСпискаСтатусов Тогда
		...
		Если ВыбСтатусЭлемент.Значение = НСтр("ru = 'Сдано'")
			...
			ТекстВопроса = ?(ЭтоОтчет, НСтр("ru='Вы уверены, что отчет уже сдан?'"), НСтр("ru='Вы уверены, что уведомление уже сдано?'"));
...
Функция ВыполнениеКомандыОтменено(Форма, НаименованиеОтчета, Организация, НаименованиеКоманды = "") Экспорт
	...
	Если (НаименованиеОтчета = "Бухгалтерская отчетность (с 2011 года)"
		...
		КонецЕсли;
	ИначеЕсли НаименованиеОтчета = "Форма 2-ТП (отходы)" Тогда 
		Предупреждение = НСтр("ru = 'Формат выгрузки не опубликован, для сдачи отчета следует воспользоваться личным кабинетом природопользователя'");
		ПоказатьИВернутьОшибку(Форма, Предупреждение);
		Возврат Истина;
	ИначеЕсли СтрНачинаетсяС(НаименованиеОтчета, Строка(ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаПодключение")))
		ИЛИ СтрНачинаетсяС(НаименованиеОтчета, Строка(ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаОтключение"))) Тогда
...
Процедура ОтправитьОтчетИзФормыСписка(Форма, ИмяКоманды, КонтекстЭДОКлиент, ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Декларация по налогу на прибыль"
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2019Кв4" И ОписаниеРегОтчета.Свойство("КорректирующаяФорма")
			...
			Возврат;
		ИначеЕсли ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2025Кв4"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.прибыль 25к4", '20260101') Тогда
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Декларация по налогу на имущество"
		...
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв4" И ОписаниеРегОтчета.Свойство("КорректирующаяФорма")
			...
			Возврат;
		ИначеЕсли ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2025Кв4"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.имущество 25к4", '20260101') Тогда
			ПоказатьИВернутьОшибку(Форма,
				НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
						   |законодательством. Форма сдается в бумажном виде'"));
			Возврат;
...
Процедура НастроитьЗаполнение(Форма) Экспорт
	
	ИДОтчета = ИДОтчета(Форма);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Организация", Форма.СтруктураРеквизитовФормы.Организация);
	ПараметрыФормы.Вставить("ИДОтчета", ИДОтчета.Отчет);
	ПараметрыФормы.Вставить("ИДРедакцииОтчета", ИДОтчета.Редакция);
	ПараметрыФормы.Вставить("ДатаНачала", Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	ПараметрыФормы.Вставить("ДатаОкончания", Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	РегламентированнаяОтчетностьКлиентПереопределяемый.ОткрытьФормуНастройкиАвтозаполнения(ПараметрыФормы);
	
КонецПроцедуры

Процедура НастроитьЗаполнениеТекущегоПоказателя(Форма, ИмяЭлемента = "ПолеТабличногоДокументаФормаОтчета") Экспорт
	...
	ТекущаяОбластьИмя = Форма.Элементы[ИмяЭлемента].ТекущаяОбласть.Имя;
...
Процедура ОбработкаВыбораРаспределитьВыручкуПоОКВЭД(Форма, Область, ОбработчикЗавершения) Экспорт
	
	Если Область.Имя = "РаспределитьВыручкуПоОКВЭД" Тогда
		...
		РегламентированнаяОтчетностьКлиентПереопределяемый.ОбработкаВыбораРаспределитьВыручкуПоОКВЭД(ПараметрыФормы, Форма, ОбработчикЗавершения);
	КонецЕсли;
...
Процедура СпрятатьСсылкуУдалитьСтраницу(Форма) Экспорт
	Если Форма.Элементы.РазделыОтчета.ТекущиеДанные <> Неопределено
		И Форма.Элементы.РазделыОтчета.ТекущиеДанные.ПолучитьРодителя() <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ИмяСобытияЗаписьУведомления() Экспорт 
	Возврат "Запись_УведомлениеОСпецрежимахНалогообложения";
КонецФункции
...
Асинх Процедура ИзменитьСтатусОтправки(ПараметрыИзменения) Экспорт
	...
	ТекСтатус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1'"), ПараметрыИзменения.Форма.ТекущийЭлемент.Заголовок);
	ТекЭлемСпискаСтатусов = СписокСтатусов.НайтиПоЗначению(ТекСтатус);
...
Функция ЭтоСобытиеЗавершениеОтправки(ИмяСобытия) Экспорт 
	Возврат (ИмяСобытия = ИмяСобытияЗавершениеОтправки()
		Или ИмяСобытия = ИмяСобытияЗавершениеОтправкиВКонтролирующийОрган()
		Или ИмяСобытия = ИмяСобытияУспешнаяОтправка());
КонецФункции

Функция ИмяСобытияЗавершениеОтправки() Экспорт 
	Возврат "Завершение отправки";
КонецФункции

Функция ИмяСобытияЗавершениеОтправкиВКонтролирующийОрган() Экспорт 
	Возврат "Завершение отправки в контролирующий орган";
КонецФункции

Функция ИмяСобытияУспешнаяОтправка() Экспорт 
	Возврат "Успешная отправка";
КонецФункции
...
Процедура ПроверитьКонтрольныеСоотношения_СКП_Продолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПередаватьПараметры = ДополнительныеПараметры.ИсточникОтчета + ДополнительныеПараметры.ИмяФормы = 
		...
		ИЛИ 
		ДополнительныеПараметры.ИсточникОтчета + ДополнительныеПараметры.ИмяФормы = 
		"РегламентированныйОтчетРасчетПоСтраховымВзносамФормаОтчета2026Кв1";
...
Функция ИДОтчета(Форма)
	
	ИДОтчета = Новый Структура;
	
	ИмяФормы = Форма.ИмяФормы;
	Если СтрЧислоВхождений(ИмяФормы, "ВнешнийОтчет.") > 0 Тогда
		ИмяФормы = СтрЗаменить(ИмяФормы, "ВнешнийОтчет.", "");
	ИначеЕсли СтрЧислоВхождений(ИмяФормы, "Отчет.") > 0 Тогда
		ИмяФормы = СтрЗаменить(ИмяФормы, "Отчет.", "");
	КонецЕсли;
	ИДОтчета.Вставить("Отчет", Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1));
	ИДОтчета.Вставить("Редакция", Сред(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") + 7));
...
Процедура ОткрытьФормуКонвертацииОтчета(Форма) Экспорт 
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("СтруктураРеквизитовФормы",
		ПоместитьВоВременноеХранилище(Форма.СтруктураРеквизитовФормы, Новый УникальныйИдентификатор));
	ПараметрыОткрытия.Вставить("ИмяФормыОтчета", Форма.ИмяФормы);
	ОткрытьФорму("Обработка.ОбщиеОбъектыРеглОтчетности.Форма.ДействияПриОткрытииНеактуальногоОтчета", ПараметрыОткрытия, Форма,
		, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

#КонецОбласти

// Открыть замещающую форму отчета.
// 
// Параметры:
//  ИмяОтчета - Строка
//  ПараметрыОткрытия - Структура
//  ИсходнаяфФорма - ФормаКлиентскогоПриложения - форма отчета, из которой происходит замещение
//  ОбработчикОтсутствияЗамещающейФормы - Неопределено, ОписаниеОповещения - Обработчик отсутствия замещающей формы
// 
Процедура ОткрытьЗамещающуюФормуОтчета(ИмяОтчета, ПараметрыОткрытия, ИсходнаяфФорма,
	ОбработчикОтсутствияЗамещающейФормы = Неопределено) Экспорт
	
	ФормаЗамещена = Ложь;
	РегламентированнаяОтчетностьКлиентПереопределяемый.ПриОткрытииЗамещающейФормыОтчета(ИмяОтчета, ПараметрыОткрытия,
		ФормаЗамещена);
	
	Если НЕ ФормаЗамещена И ОбработчикОтсутствияЗамещающейФормы <> Неопределено Тогда
		ВыполнитьОбработкуОповещения(ОбработчикОтсутствияЗамещающейФормы);
	Иначе
		ИсходнаяфФорма.Закрыть();
	КонецЕсли;
