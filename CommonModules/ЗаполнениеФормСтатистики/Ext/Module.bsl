...
// Инициализирует компоновщик настроек для получения состава пользовательских настроек.
//
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - компоновщик настроек, который необходимо инициализировать
//  ОбъектНаблюдения - СправочникСсылка.ОбъектыСтатистическогоНаблюдения
//  УникальныйИдентификаторФормы - Строка Или Неопределено - идентификатор формы из которой осуществляется вызов
//
...
// Параметры:
//  ПараметрыОтчета - Структура - структура параметров отчета
//  Контейнер - Структура - данные отчета которые нужно заполнить
//
Процедура ЗаполнитьПодтверждениеВидаДеятельности(ПараметрыОтчета, Контейнер) Экспорт
	
	Показатели = ПоказателиОтчетаПодтверждениеВидаДеятельности(
		ПараметрыОтчета.Организация,
	...
	РассчитатьЗначенияПоказателей(Показатели, Неопределено, ПараметрыОтчета);
	
	Контейнер.Приложение2_ДанныеДопСтрок.Очистить();
	Для Каждого СтрокаПоказателя Из Показатели Цикл
		НоваяСтрока = Контейнер.Приложение2_ДанныеДопСтрок.Добавить();
		НоваяСтрока["П000020000101"] = СтрокаПоказателя.АналитикаКод;
		НоваяСтрока["П000020000102"] = СтрокаПоказателя.АналитикаНаименование;
		НоваяСтрока["П000020000103"] = СтрокаПоказателя.Значение;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Показатели) Тогда
...
Функция ПоказателиОтчетаПодтверждениеВидаДеятельности(Организация, НачалоПериода, КонецПериода)
	...
	|	ОбъектыСтатистическогоНаблюдения.ИсточникДанных КАК ИсточникДанных,
	|	ОбъектыСтатистическогоНаблюдения.Настройка КАК ОбщаяНастройка,
	|	Настройки.Настройка КАК Настройка,
	|	Настройки.Детализация КАК Аналитика,
	|	""ВыручкаБезНДС"" КАК Характеристика,
	|	Настройки.Детализация.Код КАК ЗначениеУпорядочиванияАналитики,
	|	ИСТИНА КАК Детализировать,
	|	ИСТИНА КАК Используется,
	|	Настройки.Детализация.Код КАК АналитикаКод,
	|	Настройки.Детализация.Наименование КАК АналитикаНаименование
	|ИЗ
	...
	|	И ОбъектыСтатистическогоНаблюдения.Ссылка = &ОбъектНаблюдения
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЗначениеУпорядочиванияАналитики";
	...
	Показатели = ЗаполнениеФормСтатистики.НовыйОписаниеПоказателей();
	Показатели.Колонки.Добавить("АналитикаКод", ОбщегоНазначения.ОписаниеТипаСтрока(8));
	Показатели.Колонки.Добавить("АналитикаНаименование", ОбщегоНазначения.ОписаниеТипаСтрока(500));
...
Процедура ЗаполнитьКонтейнерФормыСтатистики(Контейнер, Показатели, ТаблицаРасшифровки)
	...
	Для Каждого Показатель Из Показатели Цикл
		...
		Если МногострочныеРазделыКонтейнера[ИмяРаздела] = Неопределено Тогда
			...
			Если Показатель.РасшифровкаЗаполнена Или Не ЗначениеЗаполнено(Показатель.ИсточникДанных) Тогда
				Продолжить;
			КонецЕсли;
