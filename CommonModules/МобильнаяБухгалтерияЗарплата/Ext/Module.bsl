...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РассчитатьЗарплату", Истина);
	Методы.Вставить("ПересчитатьЗарплатуПоСотрудникам", Истина);
...
Функция РассчитатьЗарплатуВФоне(ХранилищеПараметров, СообщениеОбОшибке) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Расчет зарплаты в мобильной бухгалтерии'");
	
	РезультатДлительнойОперации = ДлительныеОперации.ВыполнитьВФоне(
		"МобильнаяБухгалтерияЗарплата.РассчитатьЗарплату",
		ПараметрыРасчета,
		ПараметрыВыполнения);
...
Функция ПересчитатьЗарплатуПоСотрудникамВФоне(ХранилищеПараметров, СообщениеОбОшибке) Экспорт
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Неопределено);
	ПараметрыВыполнения.ОжидатьЗавершение = 0;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Перерасчет зарплаты по сотрудникам в мобильной бухгалтерии'");
	
	РезультатДлительнойОперации = ДлительныеОперации.ВыполнитьВФоне(
		"МобильнаяБухгалтерияЗарплата.ПересчитатьЗарплатуПоСотрудникам",
		ПараметрыРасчета,
		ПараметрыВыполнения);
...
Процедура ПерезаполнитьНачислениеЗарплаты(МесяцНачисления, Организация, ДокументНачисления, Сотрудники)
	...
	Если РезультатНачисления.Количество() > 0 Тогда
		...
		РасчетЗарплатыБазовый.ЗаполнитьОтработанноеВремя(РезультатНачисления, Объект.МесяцНачисления, Организация, Объект.ПерваяПоловинаМесяца);
