...
Функция ПроверитьНастройкуПлатежаВБюджет(РеквизитыОбъекта, РеквизитыВБюджетПоУмолчанию, РегистрацияВНалоговомОргане) Экспорт
	...
	Для каждого КлючИЗначение Из ИзмененныеНастройки Цикл
		...
		ИначеЕсли КлючИЗначение.Ключ = "РеквизитыПолучателя" И ПравилаЗаполнения.СохранятьПолучателя Тогда
		...
		ИначеЕсли КлючИЗначение.Ключ = "КодТерритории"
			И (ПравилаЗаполнения.ЗаполнятьКодТерриторииПоРегистрации
				Или ПравилаЗаполнения.СохранятьКодТерриторииПриЗаписи) Тогда
	...
	Если Не НалоговыйУчет.УчетВРазрезеНалоговыхОрганов()
		Или ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоУплатаВСФР(РеквизитыОбъекта.Налог.ВидНалога)
		Или Справочники.РегистрацииВНалоговомОргане.ПолучитьКоличествоПодчиненныхЭлементовПоВладельцу(
			РеквизитыОбъекта.Организация) = 1 Тогда
...
Функция ПравилаЗаполненияРеквизитовПлатежа(Налог) Экспорт
	...
	ИначеЕсли ПлатежиВБюджетКлиентСерверПереопределяемый.ЭтоУплатаВСФР(ВидНалога) Тогда
		
		// При уплате в СФР (бывший ФСС) ОКТМО указывается не от налогового органа, а от данных ОСФР.
		Правила.СохранятьКодТерриторииПриЗаписи = Истина;
...
Функция ПроверитьСтатьяДДС(РеквизитыОбъекта, НастройкаЗаполнения)
	...
	Иначе
		...
		КонецЕсли;
		СтатьяДДСПоУмолчанию = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(КонтекстОперации, РеквизитыОбъекта.Дата);
