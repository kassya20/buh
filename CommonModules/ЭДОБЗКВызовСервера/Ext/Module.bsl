...
//
Функция ПечатныеФормыОбъектов(ОписанияПечатныхФорм, ОбъектыПечати, ПараметрыПечати, ИдентификаторХранилища,
	РеквизитыОбъектов = Неопределено, ТолькоДляВыгружаемыхСотрудников = Ложь) Экспорт
	...
	Если РеквизитыОбъектов = Неопределено Тогда
	...
	Для Каждого ОписаниеПечатнойФормы Из ОписанияПечатныхФорм Цикл
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Если ТолькоДляВыгружаемыхСотрудников Тогда
				Если Не ЗначениеЗаполнено(РеквизитыОбъекта.ВыгружаемостьСотрудников) Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
			...
			Если Не ЕстьДетальныеОбласти Тогда
				...
				Если ЗначениеЗаполнено(РеквизитыОбъекта) Тогда
					...
					Если ТолькоДляВыгружаемыхСотрудников Тогда
						ДанныеПечатнойФормы.ФизическоеЛицо = ОбщегоНазначения.ВыгрузитьКолонку(
							РеквизитыОбъекта.ВыгружаемостьСотрудников, "Ключ");
					КонецЕсли;
			...
			Иначе
				...
				Для Каждого СоответствиеОбъектаДетальнойПечатнойФорме Из ДетальныеПечатныеФормыПоОбъектам Цикл
					...
					Иначе
						ДанныеПечатнойФормы.Название = ОписаниеПечатнойФормы.Название;
					КонецЕсли;
					ЗаполнитьЗначенияСвойств(ДанныеПечатнойФормы, РеквизитыОбъекта, "Организация,Дата");
					...
					Если ТолькоДляВыгружаемыхСотрудников Тогда
						Если Не ЗначениеЗаполнено(ДанныеПечатнойФормы.ФизическоеЛицо)
							...
							ДанныеПечатнойФормы.ФизическоеЛицо = КадровыйУчетПовтИсп.ФизическоеЛицоСотрудника(ДанныеПечатнойФормы.Сотрудник);
						КонецЕсли;
						Если Не ЗначениеЗаполнено(ДанныеПечатнойФормы.ФизическоеЛицо)
							Или РеквизитыОбъекта.ВыгружаемостьСотрудников.Получить(ДанныеПечатнойФормы.ФизическоеЛицо) <> Истина Тогда
...
Функция ОтветНаВопросОПередачеДокументовТолькоВСуществующиеКабинеты() Экспорт
	Возврат ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"ЭлектронныйКадровыйДокументооборот", "ОтветНаВопросОПередачеДокументовТолькоВСуществующиеКабинеты")
КонецФункции

Процедура ЗапомнитьОтветНаВопросОПередачеДокументовТолькоВСуществующиеКабинеты(Результат) Экспорт
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ЭлектронныйКадровыйДокументооборот",
		"ОтветНаВопросОПередачеДокументовТолькоВСуществующиеКабинеты", Результат);
...
Процедура ОчиститьНастройкиПодписанияПечатныхФорм() Экспорт
	...
	ЗапомнитьОтветНаВопросОПередачеДокументовТолькоВСуществующиеКабинеты(Неопределено);
КонецПроцедуры
...
Функция ЭлектронныеДокументыПоСсылкамНаДокументыЭДОБЗК(ДокументыЭДОБЗК) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ПодписанныеПечатныеФормы.ИдентификаторПечатнойФормы, """") КАК ИдентификаторПечатнойФормы
		|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.Организации.Вставить(Выборка.Организация, Истина);
		Результат.ГоловныеОрганизации.Вставить(ЗарплатаКадрыПовтИсп.ГоловнаяОрганизация(Выборка.Организация), Истина);
		Если ПустаяСтрока(Выборка.ИдентификаторПечатнойФормы) Тогда
			Результат.ПрисоединенныеФайлы.Добавить(Выборка.ЭлектронныйДокумент);
		Иначе
			Результат.ПечатныеФормы.Добавить(Выборка.ЭлектронныйДокумент);
		КонецЕсли;
		ДокументыКатегории = Результат.ДокументыПоКатегориям.Получить(Выборка.КатегорияДокумента);
		Если ДокументыКатегории = Неопределено Тогда
			ДокументыКатегории = Новый Массив();
			Результат.ДокументыПоКатегориям.Вставить(Выборка.КатегорияДокумента, ДокументыКатегории);
		КонецЕсли;
		ДокументыКатегории.Добавить(Выборка.Ссылка);
	КонецЦикла;
...
Функция ДанныеФайловНаПодпись(ПрисоединенныеФайлы, Знач ИдентификаторХранилища = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ДокументЭДОБЗК.Организация КАК Организация,
		|	ДокументЭДОБЗК.ЭлектронныйДокумент КАК ЭлектронныйДокумент,
		|	ДокументЭДОБЗК.Ссылка КАК Владелец
		|ИЗ
		|	Документ.ДокументЭДОБЗК КАК ДокументЭДОБЗК
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеФайла.Владелец = Выборка.Владелец;
...
Функция РеквизитыАдресацииОбъектов(ОбъектыПечати, ПутьКПодписантам = "") Экспорт
		...
		Если ОбщегоНазначения.ЭтоДокумент(МетаданныеТипа) Тогда
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Сотрудник", МетаданныеТипа)
				...
				ТекстЗапрос =
					...
					|		ПО ТаблицаДокумента.Сотрудник.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|		ПО ТаблицаДокумента.Сотрудник.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
					|ГДЕ
					|	ТаблицаДокумента.Ссылка В(&Объекты)";
				
				Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(МетаданныеТипа, "ТабличныеЧасти")
					И МетаданныеТипа.ТабличныеЧасти.Найти("Сотрудники") <> Неопределено
					И ОбщегоНазначения.ЕстьРеквизитОбъекта("Сотрудник", МетаданныеТипа.ТабличныеЧасти.Сотрудники) Тогда
					...
					ТекстЗапрос = ТекстЗапрос +
						...
						|	ТаблицаДокумента.Сотрудник.ФизическоеЛицо КАК ФизическоеЛицо,
						|	ТаблицаДокумента.Сотрудник.Наименование КАК ФИО,
						|	ТаблицаДокумента.Сотрудник.ФизическоеЛицо КАК Детализация,
						|	ВЫБОР
						|		КОГДА ВыгружаемыеОбъекты1СПерсонал.Ссылка ЕСТЬ NULL
						|				И ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ЕСТЬ NULL
						|			ТОГДА ЛОЖЬ
						...
						|	КОНЕЦ КАК Выгружается
						|ИЗ
						|	#ТаблицаДокумента.Сотрудники КАК ТаблицаДокумента
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
						|		ПО ТаблицаДокумента.Сотрудник.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
						|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
			...
			ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("СовмещающийСотрудник", МетаданныеТипа)
				...
				ТекстЗапрос =
					...
					|	#ТаблицаДокумента КАК ТаблицаДокумента
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
					|		ПО ТаблицаДокумента.СовмещающийСотрудник.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|		ПО ТаблицаДокумента.СовмещающийСотрудник.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
					|ГДЕ
					|	ТаблицаДокумента.Ссылка В(&Объекты)";
			...
			ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("Сотрудник", МетаданныеТипа)
				...
				ТекстЗапрос =
					...
					|	#ТаблицаДокумента КАК ТаблицаДокумента
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
					|		ПО ТаблицаДокумента.Сотрудник = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|		ПО ТаблицаДокумента.Сотрудник = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
					|ГДЕ
					|	ТаблицаДокумента.Ссылка В(&Объекты)";
			...
			ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("ФизическоеЛицо", МетаданныеТипа) Тогда
				...
				ТекстЗапрос =
					...
					|	#ТаблицаДокумента КАК ТаблицаДокумента
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
					|		ПО ТаблицаДокумента.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|		ПО ТаблицаДокумента.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
					|ГДЕ
					|	ТаблицаДокумента.Ссылка В(&Объекты)";
			...
			Иначе
				...
				ТекстЗапрос =
					...
					|	СоставДокументовЗарплатаКадры.ФизическоеЛицо КАК СотрудникФизическоеЛицо,
					|	"""" КАК ФИО,
					|	СоставДокументовЗарплатаКадры.ФизическоеЛицо КАК Детализация,
					|	ВЫБОР
					|		КОГДА ВыгружаемыеОбъекты1СПерсонал.Ссылка ЕСТЬ NULL
					|				И ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ЕСТЬ NULL
					|			ТОГДА ЛОЖЬ
					...
					|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
					|			ПО СоставДокументовЗарплатаКадры.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|				И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|			ПО СоставДокументовЗарплатаКадры.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|				И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|				И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
			...
			Если ДобавитьДетальныеЗаписи Тогда
				...
				ТекстЗапрос = СтрЗаменить(ТекстЗапрос, "ИСТИНА КАК Выгружается",
					"ВЫБОР
					|		КОГДА ВыгружаемыеОбъекты1СПерсонал.Ссылка ЕСТЬ NULL
					|				И ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ЕСТЬ NULL
					|			ТОГДА ЛОЖЬ
				...
				ТекстЗапрос = СтрЗаменить(ТекстЗапрос, "#ТаблицаДокумента КАК ТаблицаДокумента",
					...
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
					|		ПО " + ПсевдонимТаблицыДетализации + "." + ЧастиПутиКРеквизиту[1] + " = ВыгружаемыеОбъекты1СПерсонал.Ссылка
					|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
					|		ПО " + ПсевдонимТаблицыДетализации + "." + ЧастиПутиКРеквизиту[1] + " = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
					|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
					|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)");
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("НомерПервичногоДокумента", МетаданныеТипа) Тогда
				...
				ТекстЗапрос = СтрЗаменить(ТекстЗапрос, "ТаблицаДокумента.Ссылка.НомерПриказа", "ТаблицаДокумента.Ссылка.НомерПервичногоДокумента");
			ИначеЕсли Не ОбщегоНазначения.ЕстьРеквизитОбъекта("НомерПриказа", МетаданныеТипа) Тогда
				...
				ТекстЗапрос = СтрЗаменить(ТекстЗапрос, "ТаблицаДокумента.Ссылка.НомерПриказа", "НЕОПРЕДЕЛЕНО");
			КонецЕсли;
			...
			Если МетаданныеТипа = Метаданные.Справочники.Сотрудники Тогда
				...
				Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
					...
					ТекстЗапрос =
						...
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
						|		ПО ТаблицаСправочника.ФизическоеЛицо = ВыгружаемыеОбъекты1СПерсонал.Ссылка
						|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
						|		ПО ТаблицаСправочника.ФизическоеЛицо = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
						|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
						|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
						|ГДЕ
			...
			ИначеЕсли МетаданныеТипа = Метаданные.Справочники.ФизическиеЛица Тогда
				...
				Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
					...
					ТекстЗапрос =
						...
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъекты1СПерсонал КАК ВыгружаемыеОбъекты1СПерсонал
						|		ПО ТаблицаСправочника.Ссылка = ВыгружаемыеОбъекты1СПерсонал.Ссылка
						|			И (ВыгружаемыеОбъекты1СПерсонал.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
						|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыгружаемыеОбъектыКабинетСотрудника КАК ВыгружаемыеОбъектыКабинетСотрудника
						|		ПО ТаблицаСправочника.Ссылка = ВыгружаемыеОбъектыКабинетСотрудника.Ссылка
						|			И (ВыгружаемыеОбъектыКабинетСотрудника.Ссылка ССЫЛКА Справочник.ФизическиеЛица)
						|			И (НЕ ВыгружаемыеОбъектыКабинетСотрудника.УсловноВыгружается)
						|ГДЕ
						|	ТаблицаСправочника.Ссылка В(&Объекты)";
	...
	Если ТекстыЗапроса.Количество() > 0 Тогда
		...
		Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
			...
			РеквизитыОбъекта.Вставить("ВыгружаемостьСотрудников", Новый Соответствие);
			...
			Если Не ЗначениеЗаполнено(РеквизитыОбъекта.ФизическоеЛицо) Тогда
				...
				ВыгружаемостьСотрудников = Новый Соответствие;
				...
				Пока Выборка.Следующий() Цикл
					Если ЗначениеЗаполнено(Выборка.Детализация) Тогда
						...
						Если Выборка.Выгружается Тогда
							ВыгружаемостьСотрудников.Вставить(Выборка.Детализация, Истина);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
				...
				КонецЕсли;
				РеквизитыОбъекта.ВыгружаемостьСотрудников = ВыгружаемостьСотрудников;
			Иначе
				Если Выборка.Выгружается Тогда
					Если ЗначениеЗаполнено(РеквизитыОбъекта.ФизическоеЛицо) Тогда
						РеквизитыОбъекта.ВыгружаемостьСотрудников.Вставить(РеквизитыОбъекта.ФизическоеЛицо, Истина);
					ИначеЕсли ЗначениеЗаполнено(РеквизитыОбъекта.Сотрудник) Тогда
						РеквизитыОбъекта.ВыгружаемостьСотрудников.Вставить(
							КадровыйУчетПовтИсп.ФизическоеЛицоСотрудника(РеквизитыОбъекта.Сотрудник), Истина);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция PDFПредставлениеMXLФайлаСоШтампамиПодписей(Оригинал, АдресВХранилище, ИмяФайла, ИдентификаторФормы)
	...
	УстановленныеПодписи = ЭлектроннаяПодписьЭДОБЗК.ВсеУстановленныеПодписи(Оригинал);
...
Функция СвойстваВыводимогоФайла(ДокументЭДОБЗК)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументЭДОБЗК);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДокументЭДОБЗК.ВидДокумента КАК ВидДокумента,
		|	ДокументЭДОБЗК.ВариантПодписания КАК ВариантПодписания,
		|	ДокументЭДОБЗК.ТипЭлектроннойПодписи КАК ТипЭлектроннойПодписи,
		|	ДокументЭДОБЗК.ВидДокумента.ВариантПодписания КАК ВидДокументаВариантПодписания,
		|	ДокументЭДОБЗК.ВидДокумента.ТипЭлектроннойПодписи КАК ВидДокументаТипЭлектроннойПодписи
		|ИЗ
		|	Документ.ДокументЭДОБЗК КАК ДокументЭДОБЗК
		|ГДЕ
		|	ДокументЭДОБЗК.Ссылка = &Ссылка";
	...
	Если РезультатЗапроса.Пустой() Тогда
		СвойстваФайла.Вставить("ВидДокумента");
		СвойстваФайла.Вставить("ВариантПодписания");
		СвойстваФайла.Вставить("ТипЭлектроннойПодписи");
	Иначе
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		СвойстваФайла.Вставить("ВидДокумента", Выборка.ВидДокумента);
		Если ЗначениеЗаполнено(Выборка.ВариантПодписания) Тогда
			СвойстваФайла.Вставить("ВариантПодписания", Выборка.ВариантПодписания);
			СвойстваФайла.Вставить("ТипЭлектроннойПодписи", Выборка.ТипЭлектроннойПодписи);
		Иначе
			СвойстваФайла.Вставить("ВариантПодписания", Выборка.ВидДокументаВариантПодписания);
			СвойстваФайла.Вставить("ТипЭлектроннойПодписи", Выборка.ВидДокументаТипЭлектроннойПодписи);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьФайлОписания(ДанныеВыводимогоФайла, ИдентификаторХранилища)
	...
	Если ДанныеВыводимогоФайла.ВариантПодписания <> Перечисления.ВариантыПодписанияДокументовЭДОБЗК.Требуется Тогда
		Возврат;
	КонецЕсли;
...
Функция PDFПредставлениеPDFФайлаСоШтампамиПодписей(Оригинал, АдресВХранилище, ИмяФайла, ИдентификаторФормы)
	...
	Попытка
		...
		ТаблицаСоШтампами = ЭДОБЗК.ПечатнаяФормаСоШтампамиПоОригиналу(
			Новый ТабличныйДокумент, УстановленныеПодписи, , , Ложь);
		
		// Подготовка файла со штампами
		ДокументНаПечать = Новый ДокументPDF();
		ДокументНаПечать.Прочитать(ПолноеИмяФайла);
		КоличествоСтраницPDF = ДокументНаПечать.КоличествоСтраниц();
		
		ВысотаСтраницы = 42;
		СтраницСоШтампами = Цел(ТаблицаСоШтампами.ВысотаТаблицы / ВысотаСтраницы);
		Если ТаблицаСоШтампами.ВысотаТаблицы / ВысотаСтраницы - СтраницСоШтампами > 0 Тогда
			СтраницСоШтампами = СтраницСоШтампами + 1;
		КонецЕсли;
		Если СтраницСоШтампами > 1 И  КоличествоСтраницPDF >= СтраницСоШтампами Тогда
			Для НомерСтраницы = 0 По СтраницСоШтампами -1 Цикл
				
				ОбластьСтраницы = ТаблицаСоШтампами.ПолучитьОбласть(
					ВысотаСтраницы * НомерСтраницы + 1,
					,
					Мин(ВысотаСтраницы * НомерСтраницы + ВысотаСтраницы, ТаблицаСоШтампами.ВысотаТаблицы) + 1,
					);
				
				// Добавление отображаемого объекта
				ОписаниеОбъекта = Новый ОписаниеОтображаемогоОбъектаPDF;
				ОписаниеОбъекта.Объект = ОбластьСтраницы;
				ОписаниеОбъекта.НомерСтраницы = КоличествоСтраницPDF - СтраницСоШтампами + НомерСтраницы + 1;
				ОписаниеОбъекта.Верх = 5;
				ОписаниеОбъекта.Лево = 5;
				ДокументНаПечать.ДобавитьОтображаемыйОбъект(ОписаниеОбъекта);
			КонецЦикла;
		Иначе
			// Добавление отображаемого объекта
			ОписаниеОбъекта = Новый ОписаниеОтображаемогоОбъектаPDF;
		...
		ДокументНаПечать.Записать(ПолноеИмяФайла);
