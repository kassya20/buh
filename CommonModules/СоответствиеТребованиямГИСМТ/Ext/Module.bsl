...
//   * Ссылка - ОпределяемыйТип.ПрикладныеДокументыГИСМТ - Ссылка на документ
Процедура ПриПолученииДанныхНаСервереДокумента(Строки) Экспорт
	...
	Для Каждого Строка Из Строки Цикл
		...
		Если ДанныеСтроки.Свойство("Ссылка") И ДанныеСтроки.Свойство("СостояниеГИСМТ") Тогда
			Прервать;
		Иначе
	...
	Для Каждого Строка Из Строки Цикл
		ДанныеДокументов.Добавить(Строка.Значение.Данные["Ссылка"]);
	КонецЦикла;
...
Функция СостояниеКомандыСоответствиеТребованиямГИСМТ(Документ, РезультатПроверкиПоДокументу = Неопределено) Экспорт
	...
	СостояниеКоманды.Вставить("ЦветТекста",        ИнтеграцияИСПовтИсп.ЦветСтиля("ЦветГиперссылкиГосИС"));
	...
	Если РезультатПроверкиПоДокументу.ПроверкаОтключена Тогда
		СостояниеКоманды.ЦветТекста = ИнтеграцияИСПовтИсп.ЦветСтиля("ЦветТекстаНеТребуетВниманияГосИС");
	ИначеЕсли РезультатПроверкиПоДокументу.Статус = Перечисления.СтатусыОбработкиДокументовИСМП.ЗавершеноСОшибкой
		...
		СостояниеКоманды.ЦветТекста = ИнтеграцияИСПовтИсп.ЦветСтиля("ЦветТекстаПроблемаГосИС");
	ИначеЕсли РезультатПроверкиПоДокументу.Статус = Перечисления.СтатусыОбработкиДокументовИСМП.Завершено Тогда
		СостояниеКоманды.ЦветТекста     = ИнтеграцияИСПовтИсп.ЦветСтиля("ЦветТекстаПоложительногоЗначения");
...
Функция ПолучитьИРазобратьШтрихкодыДокумента(Документ, ПараметрыСканирования, НаДату) Экспорт
	...
	СвойстваТаблицыПродукции = ИнтеграцияИС.СвойстваШтрихкодовУпаковок(ШтрихкодыУпаковокПоДокументу);
	...
	Для Каждого СтрокаТаблицы Из ШтрихкодыУпаковокПоДокументу Цикл
		...
		Иначе
			...
			СтруктураОтбора = Новый Структура("ШтрихкодУпаковки", СтрокаТаблицы.ШтрихкодУпаковки);
			СтрокиДанныхПоШтрихкодуУпаковки = СвойстваТаблицыПродукции.НайтиСтроки(СтруктураОтбора);
			
			Если СтрокиДанныхПоШтрихкодуУпаковки.Количество() Тогда
				ДанныеШтрихкода.ВидПродукции = СтрокиДанныхПоШтрихкодуУпаковки[0].ВидПродукции;
			КонецЕсли;
...
Функция ДокументСодержитМаркируемуюПродукцию(Документ, ПараметрыСканирования, ЭлектронныйДокумент, ТаблицаМаркируемойПродукции)
	...
	Если ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО") Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "СодержитМаркируемыеТовары");
	КонецЕсли;
...
Функция ДокументСодержитПродукциюПроверкиПоМОД(Документ, ПараметрыСканирования = Неопределено, ТаблицаМаркируемойПродукции = Неопределено) Экспорт
	...
	Если ТаблицаМаркируемойПродукции.Количество() > 0 Тогда
		...
		ВидыПродукцииВДокументе = ИнтеграцияИСМП.ВидыПродукцииПоТаблицеНоменклатуры(ТаблицаМаркируемойПродукции);
	...
	Возврат ШтрихкодированиеОбщегоНазначенияИС.ПрисутствуетПрослеживаемаяПоМОДПродукция(ВидыПродукцииВДокументе, НаДату);
...
Процедура ДобавитьВРегистрДляОтслеживанияСоответствияТребованиямГИСМТ(Знач МассивДокументов)
	...
	Для Каждого Документ Из МассивДокументов Цикл
		...
		ПараметрыСканирования = ПараметрыСканирования(Документ);
		ДокументСодержитМаркируемуюПродукцию = ДокументСодержитМаркируемуюПродукцию(Документ, ПараметрыСканирования, ЭлектронныйДокумент, ТаблицаМаркируемойПродукции);
