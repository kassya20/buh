...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверитьДоступностьСинхронизацииВФоне", Истина);
	Методы.Вставить("ВключитьСинхронизациюВФоне", Истина);
	Методы.Вставить("ЗаписатьНастройкиНапоминанийВФоне", Истина);
...
Функция redirect_uri() Экспорт
	
	// В качестве URL для редиректа используем localhost и случайный порт 23451
	// После подтверждения доступа гугл переадресует код подтверждения на эту страницу и мы ее сможем поймать.
	Возврат "http://127.0.0.1:23451";
...
Функция ИдентификаторПриложения()
	...
	ИдентификаторПриложения.redirect_uri  = redirect_uri();
