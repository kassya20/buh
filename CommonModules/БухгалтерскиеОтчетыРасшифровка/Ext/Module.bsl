...
Процедура ДобавитьВНастройкиДанныеРасшифровки(ДанныеРасшифровки, ИдентификаторРасшифровки, Настройки, РеквизитыРасшифровки) Экспорт
	...
	Для Каждого РеквизитРасшифровки Из РеквизитыРасшифровки Цикл
		...
		Если Настройки.Свойство(РеквизитРасшифровки.Ключ, Настройка) Тогда
			...
			ОбработатьИзменениеСчета(Настройки, РеквизитРасшифровки.Ключ, Настройка, РеквизитРасшифровки.Значение);
	...
	Если ЭлементыОтбораПодЗамену.Количество() > 0 Тогда
		...
		Для Каждого ЭлементЗамены Из ЭлементыОтбораПодЗамену Цикл
			...
			Для Каждого ВидСубконто Из ВидыСубконто Цикл
				...
				Если ВидСубконто.Ключ.ТипЗначения.СодержитТип(ТипЗнч(ЭлементЗамены.ПравоеЗначение)) 
					И ЭлементЗамены.ПравоеЗначение <> Неопределено Тогда
...
// Записывает новое значение настройки в коллекцию настроек. Если исходный отчет содержал суммы с разных бухгалтерских счетов,
// то набор субконто будет преобразован от исходного к видам субконто счета расшифровки.
//
// Параметры:
...
//
Процедура ОбработатьИзменениеСчета(Настройки, ИмяНастройки, СтароеЗначение, НовоеЗначение)
	
	Если ИмяНастройки <> "Счет" Или СтароеЗначение = НовоеЗначение
	 Или ТипЗнч(НовоеЗначение) <> Тип("ПланСчетовСсылка.Хозрасчетный") Тогда
		// Настройка не является счетом бухгалтерского учета.
...
Функция ВидСравненияДляПоляРасшифровки(ПолеРасшифровки)
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(ЗначениеПоляРасшифровки) Тогда
		Если ТипЗнч(ЗначениеПоляРасшифровки) = Тип("Неопределено") Тогда
