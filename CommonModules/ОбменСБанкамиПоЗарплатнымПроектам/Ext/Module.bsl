...
Процедура УстановитьДоступностьСпособовЗачисленияПоЗарплатномуПроекту(СпособыЗачисления, ЗарплатныйПроект) Экспорт
	...
	ЗарплатныйПроектДляПеречисленийНаКартыМир = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗарплатныйПроект, "ДляПеречисленийНаКартыМир");

	Если ЗарплатныйПроектДляПеречисленийНаКартыМир Тогда
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоЛицевомуСчету] = Ложь;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруКарты] = Истина;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоНомеруТелефона] = Ложь;
		СпособыЗачисления[Перечисления.СпособыЗачисленияВыплат.ПоСНИЛС] = Ложь;
	Иначе
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	ТипыДокументовВыгружаемыхВБанк = ТипыДокументовВыгружаемыхВБанк();
	ТипПлатежноеПоручение = Тип("ДокументСсылка.ПлатежноеПоручение");
	ИндексОбъекта = ТипыДокументовВыгружаемыхВБанк.Найти(ТипПлатежноеПоручение);
	Если ИндексОбъекта <> Неопределено Тогда
		ТипыДокументовВыгружаемыхВБанк.Удалить(ИндексОбъекта);
	КонецЕсли;
	...
	Если ЗарплатныеПроектыИспользующиеВыгрузкуФайла.Количество() > 0 Тогда
		// Команда и для формы списка и для формы документа
		...
		Команда.Подменю  = "ПодменюВыгрузкаФайловОбменаСБанком";
		...
		Команда.Важность = "Обычное";
		Команда.Подменю  = "ПодменюЗагрузкаФайловОбменаСБанком";
		Команда.ОтображениеКнопки = ОтображениеКнопки.КартинкаИТекст;
		...
		// Команда и для формы документа
		Команда = Команды.Добавить();
		Команда.Вид = "ЗагрузкаФайловОбменаСБанками";
		Команда.Идентификатор = "ЗагрузитьФайлОбменаСБанкомФормаДокумента";
		Команда.Представление = НСтр("ru = 'Загрузить подтверждения банка'");
		Команда.Важность = "Обычное";
		Команда.Подменю  = "ПодменюЗагрузкаФайловОбменаСБанком";
		Команда.ОтображениеКнопки = ОтображениеКнопки.КартинкаИТекст;
...
Функция ДобавитьПолеСведенийОЗарплатныхПроектах(Знач ИмяПоля, ТекстыОписанияПолей, ИсточникиДанных) Экспорт
...
// Возвращает предназначен ли зарплатный проект для перечислений на карты Мир через Казначейство
//
// Параметры:
//  ЗарплатныйПроект - СправочникСсылка.ЗарплатныеПроекты - зарплатный проект.
//
Функция ЗарплатныйПроектДляПеречисленийНаКартыМир(ЗарплатныйПроект) Экспорт
	Возврат ?(ЗначениеЗаполнено(ЗарплатныйПроект),
...
Функция СоответствиеПолейФайла(ВидОперации, Знач ФорматФайла = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(ФорматФайла) Тогда
		ФорматФайла = ПредопределенноеЗначение("Перечисление.ФорматыФайловОбменаПоЗарплатномуПроекту.Версия37");
	КонецЕсли;
...
Функция ПутьКДаннымСведенийОЗарплатныхПроектах(Знач ИмяПоля) Экспорт
