...
Процедура ВыполнитьОбработчикиОбмена(МенеджерОбмена)
	...
	ИначеЕсли МенеджерОбмена.Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом Тогда
		Интеграция1СПерсонал.ВыполнитьОбработчикиОбмена(МенеджерОбмена);
	КонецЕсли;
...
Функция ВыгрузитьИзмененияОбъектов(МенеджерОбмена)
	...
	Если ИнтеграцияУправлениеПерсоналом.ДоступенРасширенныйФункционал() Тогда
		Если МенеджерОбмена.Приложение = Перечисления.ПриложенияДляИнтеграции.КабинетСотрудника И ИнтеграцияКабинетСотрудника.ИспользуетсяВерсияDTO("3.0")
			Или МенеджерОбмена.Приложение = Перечисления.ПриложенияДляИнтеграции.УправлениеПерсоналом И Интеграция1СПерсонал.ИспользуетсяВерсияAPI("2.4") Тогда
			ТипыОбъектов.Добавить(Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ВидОтпуска);
		КонецЕсли;
	КонецЕсли;
	...
	Для Индекс = 0 По ТипыОбъектов.ВГраница() Цикл
		...
		Если ЗначениеЗаполнено(ДанныеОбъектов.ТаблицаДанных) Тогда
			...
			Если ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ФизическоеЛицо
				...
				ИнтеграцияКабинетСотрудникаОбмен.ВыполнитьАктивациюФизическихЛицКабинетСотрудника(МенеджерОбмена, РезультатВыгрузки);
				ИнтеграцияКабинетСотрудникаОбмен.ОбновитьВидыЛичныхКабинетов(МенеджерОбмена, РезультатВыгрузки, БылиОшибки);
			КонецЕсли;
			ДополнитьТаблицуРезультатамиВыгрузки(ТаблицаОбработано, РезультатВыгрузки, ТипОбъекта);
		КонецЕсли;
...
Процедура ОбработатьЗагрузкуЗаявок(МенеджерОбмена, ОбъектВсеИзменения, РезультатОбработки, ПовторнаяЗагрузка, ПовторнаяОбработка)
	...
	Иначе
		...
		ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявлениеВПроизвольнойФорме);
		ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявлениеОСпособеВыплатыЗарплаты);
		Если ИнтеграцияУправлениеПерсоналом.ДоступенРасширенныйФункционал() Тогда
			...
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявкаНаИзменениеГрафикаОтпуска);
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявкаИсполнениеГособязанностей);
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявкаВыходныеДляУходаЗаДетьмиИнвалидами);
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ЗаявлениеНаКомандировку);
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ОтчетОбИтогахКомандировки);
			ОбрабатываемыеЗаявки.Добавить(ПеречислениеТипОбъекта.ОтчетОРасходахВКомандировке);
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого ТипОбъекта Из ОбрабатываемыеЗаявки Цикл
		...
		Иначе
			...
			Объекты = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(ОбъектВсеИзменения, ИмяКоллекции);
		КонецЕсли;
	...
	Для каждого СтрокаТЗ Из ТаблицаЗаявок Цикл
		...
		ИдентификаторЭлектронногоДокумента = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(Объект, "ИдентификаторЭлектронногоДокумента");
...
Процедура ОбработатьОфферы(МенеджерОбмена, ОбъектВсеИзменения, РезультатОбработки, ПовторнаяЗагрузка, ПовторнаяОбработка)
	...
	Для каждого СтрокаТЗ Из ТаблицаОбъектов Цикл
		...
		ПубличныйИдентификатор = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(Объект, "ФизическоеЛицо");
...
Функция ЗагрузитьФайл(МенеджерОбмена, ОбъектФайл)
	...
	Если ЗначениеЗаполнено(Ответ.ДвоичныеДанныеФайла) Тогда
		ОписаниеФайла = Новый Структура("ИмяФайла,РасширениеФайла,ДвоичныеДанные,ХешСумма,ИдентификаторФайла");
		...
		ОписаниеФайла.ИдентификаторФайла 	= ОбъектФайл.ИдентификаторФайла;
...
Функция ЗагрузитьЭлектронныйДокумент(МенеджерОбмена, ЭлектронныйДокумент)
	...
	ОригиналВMXL = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(ЭлектронныйДокумент, "ОригиналВMXL");
...
Процедура ПолучитьВложенияДанныхДляОформленияНаРаботу(МенеджерОбмена, Объект, Вложения, ОписаниеОшибокПолученияФайлов)
	...
	Для каждого ЭлементКоллекции Из ТипДанныхИмяСвойства Цикл
		...
		ЗначениеСвойства = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(Объект, ИмяСвойства);
	...
	Если ЗначениеЗаполнено(СостояниеВБраке) Тогда
		ДокументыОСемейномПоложении = ИнтеграцияУправлениеПерсоналом.ЗначениеСвойстваОбъекта(СостояниеВБраке, "ДокументыОСемейномПоложении");
...
Функция ДанныеОбъектовДляВыгрузки(МенеджерОбмена, ТаблицаИзменений, ТипОбъекта)
	...
	ИначеЕсли ТипОбъекта = ПеречисленияМенеджер.ПодразделениеОрганизации Тогда
		
		ДанныеОбъектов = ДанныеПодразделенийОрганизации(Приложение, ТаблицаИзменений);
...
Процедура ЗарегистрироватьНеобработанныйОбъект(Идентификатор, Объект, ТипОбъекта, Описание, МенеджерОбмена)
	...
	ИначеЕсли ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.Оффер Тогда
		...
		СвойстваОбъекта.ФизическоеЛицо = СвойстваОффера.ФизическоеЛицо;
	КонецЕсли;
...
Функция ДанныеПодразделенийОрганизации(Приложение, ТаблицаИзменений)
	
	Результат = НовыеДанныеДляВыгрузки();
	ТаблицаДанных = НоваяТаблицаДанныеПодразделенийОрганизации();
...
Функция НоваяТаблицаДанныеПодразделенийОрганизации()
...
Функция НоваяТаблицаДанныеСотрудников()
	...
	|Организация,Подразделение,Должность,ДолжностьПоШтатномуРасписанию,ПодразделениеОрганизации,
		...
		Для каждого Ссылка Из Ссылки Цикл
			...
			Если Не ЗначениеЗаполнено(Идентификатор) Тогда
				...
				ИначеЕсли ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ДанныеДляОформленияНаРаботу
					...
					СсылкиБезИдентификатора.Добавить(Ссылка);
					Продолжить;
				ИначеЕсли ТипОбъекта = Перечисления.ТипыОбъектовИнтеграцияУправлениеПерсоналом.ПодразделениеОрганизации Тогда
