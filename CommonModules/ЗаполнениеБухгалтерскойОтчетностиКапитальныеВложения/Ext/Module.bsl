...
Процедура ЗаполнитьПояснениеКапитальныеВложения(ПолучениеДанныхТаблицОтчета, ИмяТаблицы, Классификаторы, КонтекстЗаполненияОтчета) Экспорт
	...
	Если ВидКапитальныхВложений = ВидКапитальныхВложенийНематериальныеАктивы() Тогда
		ГруппаПоУмолчанию = Перечисления.ВидыНематериальныхАктивов.ДругиеНМА;
	КонецЕсли;
...
Процедура НастроитьОтчетОсновныеСредства(ЗначенияНастроекОтчета, ВыборкаСчет, СчетаУчетаПоВидуАктивов, ВидыСубконто, КлассификаторСтадий)
	...
	Если ЭтоСчетМодернизации(ВыборкаСчет, СчетаУчетаПоВидуАктивов) Тогда
		ЗаполнениеБухгалтерскойОтчетностиРасшифровка.НастроитьОтборПоВидуСубконто(
			...
			ВыборкаСчет.Группа);
		Если СчетаУчетаПоВидуАктивов.Ремонты.Найти(ВыборкаСчет.Счет) <> Неопределено Тогда
			ЗаполнениеБухгалтерскойОтчетностиРасшифровка.НастроитьОтборПоВидуСубконто(
				ЗначенияНастроекОтчета,
				"Субконто%1.ЗатратыКапитализируются",
				ЗначенияНастроекОтчета.Счет,
				ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРемонтовОС,
				Истина);
		КонецЕсли;
...
Функция СчетаУчетаПоВидуАктивов(ВидКапитальныхВложений)
	...
	ИначеЕсли ВидКапитальныхВложений = ВидКапитальныхВложенийОсновныеСредства() Тогда
		...
		СчетаУчетаПоВидуАктивов.ЗатратыПриобретение =  СчетаУчетаПоКоду("07, 08", "08.03, 08.04.5, 08.05, 08.08, 08.11, 08.12");
		СчетаУчетаПоВидуАктивов.ЗатратыПоследующиеВложения = СчетаУчетаПоКоду("23.05, 08.04.5");
		СчетаУчетаПоВидуАктивов.ВсеЗатраты = СчетаУчетаПоКоду("07, 08","08.05, 08.08, 08.11, 08.12");
		СчетаУчетаПоВидуАктивов.ВнеоборотныеАктивы = СчетаУчетаПоКоду("01", "01.11");
		...
		СчетаУчетаПоВидуАктивов.Ремонты = СчетаУчетаПоКоду("23.05");
		СчетаУчетаПоВидуАктивов.ВнеоборотныеАктивыИерархия =
...
Процедура СоздатьСтоимостьКапитальныхВложений(МенеджерВременныхТаблиц, КонтекстЗаполненияОтчета, ИмяТаблицы, СчетаУчетаПоВидуАктивов, ВидыСубконто, КлассификаторСтадий)
	...
	ВидСубконтоРемонты =           ВидыСубконто.Ремонты;
	...
	СчетаРемонты                    = СчетаУчетаПоВидуАктивов.Ремонты;
	...
	Запрос.УстановитьПараметр("СчетаРемонты",                           СчетаРемонты);
	...
	Запрос.УстановитьПараметр("ВидСубконтоРемонты",           ВидСубконтоРемонты);
	...
	|				И ВЫРАЗИТЬ(Субконто2 КАК Справочник.ВидыРемонтовОС).ЗатратыКапитализируются) КАК ХозрасчетныйОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ИЛИ ХозрасчетныйОбороты.Субконто1 = ХозрасчетныйОбороты.КорСубконто3)
	|		И ВЫБОР
	|			КОГДА ХозрасчетныйОбороты.КорСчет В (&СчетаРемонты)
	|				ТОГДА ХозрасчетныйОбороты.Субконто2 = ХозрасчетныйОбороты.КорСубконто1
	|						ИЛИ ХозрасчетныйОбороты.Субконто2 = ХозрасчетныйОбороты.КорСубконто2
	|						ИЛИ ХозрасчетныйОбороты.Субконто2 = ХозрасчетныйОбороты.КорСубконто3
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ,
	|	&ВидСубконтоВнеоборотныйАктив
	|ИЗ
	...
	|			&ВидСубконтоРемонты,
	|			Организация В (&Организации)
	|				И ВЫРАЗИТЬ(Субконто2 КАК Справочник.ВидыРемонтовОС).ЗатратыКапитализируются,
	|			,
	...
	Если ВидКапитальныхВложений = ВидКапитальныхВложенийНематериальныеАктивы() Тогда
		Запрос.Текст = СтрЗаменить(
			...
			|			ТОГДА ВЫРАЗИТЬ(СальдоЗатраты.Субконто КАК Справочник.НематериальныеАктивы).ВидНематериальногоАктива
			|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыНематериальныхАктивов.ДругиеНМА)
			|	КОНЕЦ");
	Иначе
		Запрос.Текст = СтрЗаменить(
			Запрос.Текст,
			"&ВыражениеГруппа",
			"	ВЫБОР
			|		КОГДА СальдоЗатраты.Субконто ССЫЛКА Справочник.ОсновныеСредства
			|			ТОГДА ВЫРАЗИТЬ(СальдоЗатраты.Субконто КАК Справочник.ОсновныеСредства).ГруппаОС
...
Функция ВнеоборотныеАктивыНаНачалоПоГруппам(МенеджерВременныхТаблиц, ВидКапитальныхВложений)
	...
	|			ТОГДА ВЫРАЗИТЬ(ВнеоборотныеАктивыСальдоНачало.Субконто КАК Справочник.НематериальныеАктивы).ВидНематериальногоАктива
	|		ИНАЧЕ ВЫРАЗИТЬ(ВнеоборотныеАктивыСальдоНачало.Субконто КАК Справочник.ОсновныеСредства).ГруппаОС
	|	КОНЕЦ КАК Группа
	|ИЗ
...
Функция ВидыСубконтоПоВидуАктивов(ВидАктивов)
	...
	Если ВидАктивов = ВидКапитальныхВложенийОсновныеСредства() Тогда
		...
		СубконтоРемонты.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРемонтовОС);
