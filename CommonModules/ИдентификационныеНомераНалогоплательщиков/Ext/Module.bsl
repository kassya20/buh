...
// Проверяет, относится ли указанный КПП к обособленному подразделению.
//
// Параметры:
//   КПП - Строка - Код причины постановки на учет
//
// Возвращаемое значение:
//   Булево - Истина, если КПП относится к обособленному подразделению
Функция ЭтоКППОбособленногоПодразделения(КПП) Экспорт
	
	КодПричиныПостановкиНаУчет = КодПричиныПостановкиНаУчет(КПП);
	Возврат ЭтоПричинаПостановкиНаУчетОбособленногоПодразделения(КодПричиныПостановкиНаУчет);
...
Функция ПроверитьСоответствиеТребованиямКПП(Знач КПП, Знач ЭтоЮрЛицо, Знач ОбособленноеПодразделение, Знач ЭтоКонтрагент = Истина, Знач ОписатьДействие = Истина, Знач КрупнейшийНалогоплательщик = Ложь) Экспорт
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(КодПричиныПостановкиНаУчет) Тогда
		...
		ПризнакКрупнейшегоНалогоплательщика = ЭтоПричинаПостановкиНаУчетКрупнейшегоНалогоплательщика(КодПричиныПостановкиНаУчет);
		...
		Если ОбособленноеПодразделение и ПризнакГоловногоПодразделения Тогда 
			...
			РезультатПроверки.ОписаниеОшибки = НСтр("ru = 'Возможно, введен КПП юридического лица'");
			Если ОписатьДействие Тогда
				РезультатПроверки.ОписаниеОшибки = СтрШаблон(
					НСтр("ru = '%1. Измените вид %2'"),
		...
		ИначеЕсли НЕ ОбособленноеПодразделение И ПризнакОбособленногоПодразделения Тогда
			РезультатПроверки.СоответствуетТребованиям = Истина;
			РезультатПроверки.ОписаниеОшибки = НСтр("ru = 'Возможно, введен КПП обособленного подразделения'");
			Если ЭтоКонтрагент Тогда
				Если ОписатьДействие Тогда
					РезультатПроверки.ОписаниеОшибки = СтрШаблон(
						НСтр("ru = '%1. Измените вид контрагента'"),
						РезультатПроверки.ОписаниеОшибки);
				КонецЕсли;
			Иначе
				Если ОписатьДействие И БухгалтерскийУчетВызовСервераПовтИсп.ВестиУчетПоПодразделениям() Тогда
					РезультатПроверки.ОписаниеОшибки = СтрШаблон(
						НСтр("ru = '%1. Измените вид организации'"),
						РезультатПроверки.ОписаниеОшибки);
				КонецЕсли;
			КонецЕсли;
			Возврат РезультатПроверки;
		ИначеЕсли НЕ ПризнакГоловногоПодразделения и НЕ ПризнакОбособленногоПодразделения Тогда
			...
			Возврат РезультатПроверки;
		ИначеЕсли Не ЭтоКонтрагент И Не КрупнейшийНалогоплательщик И ПризнакКрупнейшегоНалогоплательщика Тогда
			РезультатПроверки.СоответствуетТребованиям = Истина;
			РезультатПроверки.ОписаниеОшибки = НСтр("ru = 'Введен КПП крупнейшего налогоплательщика'");
			Если ОписатьДействие Тогда
				РезультатПроверки.ОписаниеОшибки = СтрШаблон(
					НСтр("ru = '%1. Укажите КПП по месту основной регистрации'"),
					РезультатПроверки.ОписаниеОшибки);
			КонецЕсли;
			Возврат РезультатПроверки;
		ИначеЕсли Не ЭтоКонтрагент И КрупнейшийНалогоплательщик И Не ПризнакКрупнейшегоНалогоплательщика Тогда
			РезультатПроверки.СоответствуетТребованиям = Истина;
			РезультатПроверки.ОписаниеОшибки = НСтр("ru = 'Введен КПП по месту основной регистрации'");
			Если ОписатьДействие Тогда
				РезультатПроверки.ОписаниеОшибки = СтрШаблон(
					НСтр("ru = '%1. Укажите КПП крупнейшего налогоплательщика'"),
					РезультатПроверки.ОписаниеОшибки);
			КонецЕсли;
			Возврат РезультатПроверки;
		КонецЕсли;
