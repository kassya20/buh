...
// Помещает таблицу значений во временную таблицу запроса и возвращает созданную схему запроса
//
// Параметры:
//  ПомещаемаяТаблицаЗначений	 - ТаблицаЗначений
//  ИмяВременнойТаблицы			 - Строка
//  СхемаЗапроса                 - СхемаЗапроса, Неопределено - если указана схема запроса, в пакет запросов схемы добавляется новый запрос,
//                                 если не указана, создается новая схема запроса.
// 
// Возвращаемое значение:
//  Структура - см. НовыйТочкаЗапроса() - точка запроса, содержащая временную таблицу.
//
Функция ПоместитьТаблицуЗначений(ПомещаемаяТаблицаЗначений, ИмяВременнойТаблицы, СхемаЗапроса = Неопределено) Экспорт

	Если СхемаЗапроса = Неопределено Тогда
		ТочкаЗапроса = НайтиЗапросРезультата(Новый СхемаЗапроса);
	Иначе
		ОписаниеЗапроса = СхемаЗапроса.ПакетЗапросов.Добавить();
		ТочкаЗапроса = НовыйТочкаЗапроса(СхемаЗапроса, ОписаниеЗапроса);
	КонецЕсли;
	
	Оператор = ТочкаЗапроса.Запрос.Операторы[0];
	Оператор.Источники.Добавить(Тип("ОписаниеВременнойТаблицыСхемыЗапроса"), "&" + ИмяВременнойТаблицы, ИмяВременнойТаблицы);
	
	Для Каждого Колонка Из ПомещаемаяТаблицаЗначений.Колонки Цикл
		УстановитьВыражение(ТочкаЗапроса, Колонка.Имя, ИмяВременнойТаблицы + "." + Колонка.Имя);
	КонецЦикла;
	
	ПоместитьРезультаты(ТочкаЗапроса.Схема, ИмяВременнойТаблицы);
