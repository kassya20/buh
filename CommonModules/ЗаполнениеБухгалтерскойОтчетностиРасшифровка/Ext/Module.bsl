...
// Параметры:
//  ИмяКолонки - Строка
//  ПараметрыРасшифровки - см. ЗаполнениеБухгалтерскойОтчетностиПоясненияНематериальныеАктивы.ПараметрыРасшифровкиНематериальныеАктивы
//  Выборка - ВыборкаИзРезультатаЗапроса
// 
// Возвращаемое значение:
//  Структура - см. НовыйЗначенияНастроекОтчета
//
Функция ЗначенияНастроекОтчетаНематериальныеАктивы(ИмяКолонки, ПараметрыРасшифровки, Выборка) Экспорт
	...
	ИначеЕсли Выборка.Расшифровка = "СтоимостьДАП" Тогда
		ЗначенияНастроекОтчета.Счет = СчетаРасшифровки.СчетДолгосрочныеАктивыКПродаже.Ссылка;
	ИначеЕсли Выборка.Расшифровка = "СтоимостьАмортизацияОбесценение" Тогда
		ЗначенияНастроекОтчета.Счет = СчетаРасшифровки.СчетАмортизацияОбесценение.Ссылка;
		Если ИмяКолонки = "Обесценение" Тогда
			ЗначенияНастроекОтчета.Счет = СчетаРасшифровки.СчетОбесценение.Ссылка;
		ИначеЕсли ИмяКолонки = "Амортизация" Тогда
			ИсключитьСчетОбесценения(ЗначенияНастроекОтчета, СчетаРасшифровки);
		КонецЕсли;
	...
	ИначеЕсли Выборка.Расшифровка = "Переклассификация" Тогда
		...
		Если ИмяКолонки = "ПереклассифицированоПервоначальная" Или ИмяКолонки = "СписаноПервоначальная" Тогда
		...
		ЗначенияНастроекОтчета.ГруппировкаСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НематериальныеАктивы);
	ИначеЕсли Выборка.Расшифровка = "МежфилиальныйОборот" Тогда
		...
		НастроитьОтборПоВидуСубконто(
			...
			"КорСубконто%1",
			ЗначенияНастроекОтчета.КорСчет,
			ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ОбособленныеПодразделения,
			ПараметрыРасшифровки.Организации,
			ВидСравненияКомпоновкиДанных.ВСписке);
		Если ИмяКолонки = "Амортизация" Тогда
			ИсключитьСчетОбесценения(ЗначенияНастроекОтчета, СчетаРасшифровки);
		ИначеЕсли ИмяКолонки = "Обесценение" Тогда
			ЗначенияНастроекОтчета.Счет = СчетаРасшифровки.СчетОбесценение.Ссылка;
		КонецЕсли;
	КонецЕсли;
...
Процедура НастроитьОтборПоВидуСубконто(ЗначенияНастроекОтчета, ШаблонПоляОтбора, Счет, ВидСубконтоОтбора, ЗначениеОтбора, ВидСравнения = Неопределено) Экспорт
	...
	Если НомерСубконто <> Неопределено Тогда
		...
		Иначе
			ОтборЗначение = ЗначениеОтбора;
			// Компоновка не умеет работать с массивом.
			Если ТипЗнч(ЗначениеОтбора) = Тип("Массив") Тогда
				ОтборЗначение = Новый СписокЗначений;
				ОтборЗначение.ЗагрузитьЗначения(ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЗначениеОтбора));
			КонецЕсли;
			...
			ЭлементОтбора.Значение = ОтборЗначение;
...
// Значения настроек отчета таблиц 5.1 "Наличие и движение Финансовых вложений".
// 
// Параметры:
//  Выборка - ВыборкаИзРезультатаЗапроса
// 
...
//  Структура - см. НовыйЗначенияНастроекОтчета
//
Функция ЗначенияНастроекОтчетаФинансовыеВложения(Источник) Экспорт
	
	ЗначенияНастроекОтчета = НовыйЗначенияНастроекОтчета();
	ЗаполнитьЗначенияСвойств(ЗначенияНастроекОтчета, Источник);
...
Процедура ИсключитьСчетОбесценения(ЗначенияНастроекОтчета, СчетаРасшифровки)
	
	ЗначениеОтбора = ЗаполнениеРасшифровкаРегламентированнойОтчетности.НовыйЗначениеЭлементаОтбора();
	ЗначениеОтбора.ВидСравненияКомпоновкиДанных = ВидСравненияКомпоновкиДанных.НеРавно;
	ЗначениеОтбора.Значение = СчетаРасшифровки.СчетОбесценение.Ссылка;
	ЗначенияНастроекОтчета.Отбор.Вставить("Счет", ЗначениеОтбора);
