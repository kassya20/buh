...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьСрокиОплатыСчетов");
...
// Устарела. Следует использовать СрокиОплатыДокументов.СрокОплатыДокументаРасчетов
...
Функция СрокОплатыДокументаРасчетовАвторасчет(Ссылка, ДатаДокумента, ДоговорКонтрагента) Экспорт
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) 
		И ОбщегоНазначения.СсылкаСуществует(ДоговорКонтрагента) Тогда
		
		РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента, "УстановленСрокОплаты, ДатаОплаты, СрокОплаты");
	КонецЕсли;
	
	Если РеквизитыДоговора <> Неопределено И РеквизитыДоговора.УстановленСрокОплаты Тогда
		Если ЗначениеЗаполнено(РеквизитыДоговора.ДатаОплаты) Тогда
			СрокОплаты = РеквизитыДоговора.ДатаОплаты;
		Иначе
			СрокОплаты = НачалоДня(ДатаДокумента) + РеквизитыДоговора.СрокОплаты*ДлинаСуток;
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		СрокОплаты = СрокОплатыНовогоСчетаПокупателю(ДатаДокумента);
	Иначе
...
Функция СрокОплатыНовогоСчетаПокупателю(ДатаДокумента) Экспорт
	...
	// Процедура СрокОплатыНовогоСчетаПокупателю() обращается к неразделенным данным регистра ДанныеПроизводственногоКалендаря,
	// И если счет создается через мезанизм OData, то у пользователя штатно нет доступа к неразделенным данным.
	// Поэтому нужно устанавливать привилегированный режим, чтобы не было ошибки доступа к данным.
	УстановитьПривилегированныйРежим(Истина);
...
//   ТаблицаЗначений - см. ПросроченнаяЗадолженность().
//
Функция ПросроченнаяЗадолженностьПокупателей(Организация, ДатаЗадолженности, ВключатьОбособленныеПодразделения = Ложь, РазрешеноИспользоватьТекущиеИтоги = Ложь, 
	ВРазрезеОтветственных = Ложь) Экспорт
...
//
Функция ПросроченнаяЗадолженность(Тип, Организация, ДатаЗадолженности, ВключатьОбособленныеПодразделения, РазрешеноИспользоватьТекущиеИтоги, ВРазрезеОтветственных)
	
	СписокОрганизаций = ОбщегоНазначенияБП.СписокДоступныхОрганизаций(Организация, ВключатьОбособленныеПодразделения);
	
	Запрос = НовыйЗапросПросроченнаяЗадолженность(Тип, СписокОрганизаций, ДатаЗадолженности, РазрешеноИспользоватьТекущиеИтоги, ВРазрезеОтветственных);
...
Функция НовыйЗапросПросроченнаяЗадолженность(Тип, СписокОрганизаций, ДатаЗадолженности, РазрешеноИспользоватьТекущиеИтоги, ВРазрезеОтветственных)
	...
	ТекстЗапроса = ТекстЗапросаДолгиПоДокументамИСрокиДолговБезДокументов(Тип);
	
	Если ВРазрезеОтветственных Тогда
	...
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстПоляВзаиморасчетыОстаткиОтветственный", ТекстПоляВзаиморасчетыОстаткиОтветственный);
...
Функция ТекстЗапросаДолгиПоДокументамИСрокиДолговБезДокументов(Тип)
	...
	|	&ТекстПоляВзаиморасчетыОстаткиОтветственный КАК Ответственный, 
	...
	|	ОстаткиДолгаПоДокументам.Ответственный КАК Ответственный,
	...
	|	ОстаткиДолгаПоДокументам.Ответственный,
	|	ОстаткиДолгаПоДокументам.ОстатокДолга
	|ИЗ
	|	ОстаткиДолгаПоДокументам КАК ОстаткиДолгаПоДокументам
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СрокиОплатыДокументов КАК СрокиОплатыДокументов
