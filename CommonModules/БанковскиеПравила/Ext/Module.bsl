...
Функция СчетПозволяетИдентифицироватьКонтрагента(НомерСчета) Экспорт
	...
	Если ЭтоМеталлический(БалансовыйСчет) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если ЭтоБрокерский(БалансовыйСчет) Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если ЭтоСчетДепозитаБанка(БалансовыйСчет) Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ЭтоСредстваДоВостребования(БалансовыйСчет) Экспорт
	
	// Средства, предоставленные негосударственным коммерческим организациям
	
	Возврат БалансовыйСчет = "47101"; //до востребования
...
Функция ЭтоСчетВнутрибанковскиеОбязательстваПоПереводамКлиентов(БалансовыйСчет) Экспорт
...
Функция ЭтоСчетРасчетовФилиалаБанкаСКлиентамиБанка(БалансовыйСчет) Экспорт
	...
	Возврат ЭтоСчетВнутрибанковскиеОбязательстваПоПереводамКлиентов(БалансовыйСчет)
...
Функция ЭтоКорсчетБанкаНерезидента(БалансовыйСчет) Экспорт
	...
	Возврат БалансовыйСчет = "30111"
		Или БалансовыйСчет = "30114";
	
КонецФункции

Функция ЭтоСчетОбязательстваПоУплатеПроцентов(БалансовыйСчет) Экспорт
	
	Возврат БалансовыйСчет = "47426"; // Обязательства по уплате процентов
	
КонецФункции

Функция ЭтоСчетРасходы(БалансовыйСчет) Экспорт
	
	Возврат БалансовыйСчет = "70606"; // Расходы
	
КонецФункции

Функция ЭтоСчетДепозитаБанка(БалансовыйСчет) Экспорт
	
	НазначениеСчета = БанковскиеСчетаКлиентСервер.НазначениеСчета(БалансовыйСчет);
	
	Возврат НазначениеСчета = "Депозиты";
...
Функция ЭтоСчетКредитаБанкаНаСрокСвыше3Лет(БалансовыйСчет) Экспорт
	
	// Кредиты, предоставленные негосударственным коммерческим организациям на срок свыше 3 лет.
	Возврат БалансовыйСчет = "45208";
...
Функция ЭтоСчетПогашенияПриобретенныхПравТребования(БалансовыйСчет) Экспорт
	
	// Выбытие (реализация) и погашение приобретенных прав требования.
	Возврат БалансовыйСчет = "61212";
...
Функция ЭтоМеталлический(БалансовыйСчет) Экспорт
	
	// Отдельные счета раздела 3 предназначены не для учета средств в распоряжении банка,
	// а для учета средств клиентов, которыми они могут распоряжаться с некоторым набором ограничений.
	// На таких счетах в аналитическом учете ведутся лицевые счета по каждому договору,
	// что позволяет идентифицировать контрагента, заключившего договор.
	
	// Счет N 20309 "Счета клиентов (кроме кредитных организаций) в драгоценных металлах"
	
	Возврат БалансовыйСчет = "20309";
	
КонецФункции

Функция ЭтоБрокерский(БалансовыйСчет) Экспорт
...
Функция ПроверитьКонтрольныйКлючВНомереБанковскогоСчета(Знач НомерСчета,
														Знач БИК,
														Знач СтрокаСоответствуетФормату = Неопределено) Экспорт
	...
	Если ЭтоБИКТОФК(БИК) Или ЭтоКазначейскийСчет(НомерСчета) Тогда
	...
	КоличествоРазрядовУсловногоНомера =3 ;
	Если ЭтоОтделениеЦентральногоБанка(БИК) Тогда
		УсловныйНомер = "0" + Сред(БИК, 5, КоличествоРазрядовУсловногоНомера - 1);
	Иначе
		УсловныйНомер = Сред(БИК, 7, КоличествоРазрядовУсловногоНомера);
	КонецЕсли;
	...
	КоличествоРазрядов = ДлинаНомераСчета() + КоличествоРазрядовУсловногоНомера;
	ПроверяемыйНаборЦифр = Новый Массив(КоличествоРазрядов);
	Для НомерСимвола = 1 По КоличествоРазрядов Цикл
	...
	ПорядковыйНомерАлфавитногоСимволаВСчете = 6;
	АлфавитныйСимвол = Сред(НомерСчета, ПорядковыйНомерАлфавитногоСимволаВСчете, 1);
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(АлфавитныйСимвол) Тогда
		ЦифраШестогоРазряда = СтрНайти(ДопустимыеАлфавитныеСимволыНомераБанковскогоСчета(), АлфавитныйСимвол);
		Если ЦифраШестогоРазряда = 0 Тогда
			Возврат Ложь;
		КонецЕсли;
		ЦифраШестогоРазряда = ЦифраШестогоРазряда - 1;
		ИндексАлфавитногоСимвола = КоличествоРазрядовУсловногоНомера + ПорядковыйНомерАлфавитногоСимволаВСчете - 1;
		ПроверяемыйНаборЦифр[ИндексАлфавитногоСимвола] = ЦифраШестогоРазряда;
	КонецЕсли;
	...
	Сумма = 0;
	Для Разряд = 0 По КоличествоРазрядов - 1 Цикл
...
// Возвращает длину банковского идентификационного кода согласно Приложению 5
// к Положению Банка России от 24 сентября 2020 года N 732-П "О платежной системе Банка России"
//
// Возвращаемое значение:
//  Число
//
Функция ДлинаБИК() Экспорт
...
//
// Параметры:
//   КодБанка - Строка - БИК банка
//
// Возвращаемое значение:
//   Булево - Истина, если это код Территориального органа Федерального казначейства
//
...
// Проверяет, что переданы реквизиты Банка УФК (Управления Федерального Казначейства).
//
// Параметры:
//   КодБанка - Строка - БИК банка
//   ЕКС - Строка - Единый казначейский счет (ЕКС) банка (он же Корр.счет)
//
// Возвращаемое значение:
//   Булево - Истина, если это Банк УФК (Управления Федерального Казначейства)
//
Функция ЭтоБанкУФК(КодБанка, ЕКС = "") Экспорт
	
	БалансовыйСчет = БалансовыйСчет(ЕКС);
	ГоловнойСчет   = Лев(БалансовыйСчет, 3);
	
	Результат = ЭтоБИКТОФК(КодБанка)
		Или ГоловнойСчет = "401"  // Средства федерального бюджета
		Или ГоловнойСчет = "402"  // Средства бюджетов субъектов РФ и местных бюджетов
		Или ГоловнойСчет = "403"  // Средства государственных и других внебюджетных фондов
		Или ГоловнойСчет = "404"  // Счета организаций, находящихся в федеральной собственности
		Или ГоловнойСчет = "405"; // Счета организаций, находящихся в государственной (кроме федеральной) собственности
...
// (утв. Банком России 29.06.2021 N 762-П)
...
Функция ДлинаИдентификатораПорученияНаПереводЦР() Экспорт
	
	// см. альбом распоряжений для платформы цифрового рубля версия 2025.1.
...
Функция ЭтоКитайскийСчетNRA(Счет, SWIFT = "") Экспорт
	...
	КодNRA = КодКитайскогоСчетаДляИностранцев();
	Если Не СтрНачинаетсяС(Счет, КодNRA) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(SWIFT) И Не СтрокаСоответствуетФорматуSWIFT(SWIFT) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ДлинаСтроки = СтрДлина(Счет);
	Если ДлинаСтроки > МаксимальнаяДлинаМеждународногоНомераСчета() Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ТипЦифровойНомерСчета() Экспорт
	
	Возврат Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(МаксимальнаяДлинаНомераСчета()));
...
Функция МаксимальнаяДлинаНомераСчета() Экспорт
...
Функция МинимальнаяДлинаНациональногоНомераСчета() Экспорт
	
	Возврат 4;
	
КонецФункции

Функция МаксимальнаяДлинаНациональногоНомераСчета() Экспорт
	
	Возврат 20;
	
КонецФункции

Функция ПроверитьДлинуНациональногоНомераСчета(НомерСчета) Экспорт
	
	Возврат СтрДлина(НомерСчета) >= МинимальнаяДлинаНациональногоНомераСчета()
		И СтрДлина(НомерСчета) <= МаксимальнаяДлинаНациональногоНомераСчета();
	
КонецФункции

Функция СтрокаСоответствуетФорматуНациональногоНомераСчета(Строка) Экспорт
	
	Если Не ПроверитьДлинуНациональногоНомераСчета(Строка) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Строка) Тогда
		Возврат Ложь;
	КонецЕсли;
...
#Область ЦифровойСчет

Функция ЭтоСчетаКредитныхОрганизацийПоОперациямСЦифровымиРублями(БалансовыйСчет) Экспорт
	
	Возврат БалансовыйСчет = "30502"  // Счет цифрового рубля кредитной организации – участника платформы цифрового рубля
		Или БалансовыйСчет = "30504"  // Счет цифрового рубля кредитной организации – пользователя платформы цифрового рубля
		Или БалансовыйСчет = "30506"; // Счет цифрового рубля иностранного банка – участника платформы цифрового рубля
...
Функция КодКитайскогоСчетаДляИностранцев()
