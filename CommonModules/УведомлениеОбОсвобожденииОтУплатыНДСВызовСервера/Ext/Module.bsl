...
//
Процедура ЗаполнитьУведомление(ПараметрыОтчета, Контейнер, ИДРедакцияОтчета) Экспорт
	Организация = ПараметрыОтчета.Организация;
	Если ИДРедакцияОтчета = "ФормаОсвобождениеНДС" Тогда
	...
	Если ИДРедакцияОтчета = "ФормаОсвобождениеНДС" Тогда
		ДанныеОВыручке = ДанныеПоВыручкеИДляВыписокСКнигойПродаж(Организация, Дата);
		ЗаполнитьУведомлениеДаннымиПоКнигеПродажИВыручке(Организация, Дата, Контейнер, ДанныеОВыручке);
	Иначе
		ДанныеОВыручке = ДанныеПоВыручкеИДляВыписок(Организация, Дата);
		ЗаполнитьУведомлениеДаннымиПоВыручке(Контейнер, ДанныеОВыручке);
	КонецЕсли;
...
Функция ПолучитьСведенияОПоказателях(ПоказателиОтчета) Экспорт
	
	Для Инд = 1 По 3 Цикл 
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "СовВыр3МесяцаМесяц" + Инд, Истина, Ложь);
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "СовВыр3МесяцаГод"   + Инд, Истина, Ложь);
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "СовВыр3МесяцаСумма" + Инд, Истина, Ложь);
	КонецЦикла;
	Для Инд = 1 По 12 Цикл 
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "Выр12МесяцевМесяц" + Инд, Истина, Ложь);
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "Выр12МесяцевГод"   + Инд, Истина, Ложь);
		РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "Выр12МесяцевСумма" + Инд, Истина, Ложь);
	КонецЦикла;
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "ПрСообщ",              Истина, Ложь);
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "ПрСообщДатаНачИспОсв", Истина, Ложь);
...
Процедура ЗаполнитьУведомлениеДаннымиПоВыручке(Контейнер, ДанныеОВыручке)
	
	Контейнер.Титульная.ПрСообщ       = ДанныеОВыручке.ТитульнаяСтраница.ПризнакЗаполнения;
	Контейнер.Титульная.ДатаНачИспОсв = ДанныеОВыручке.ТитульнаяСтраница.ДатаНачалаИспОсв;
	Инд = 1;
	Если ДанныеОВыручке.ВыручкаЗа3Месяца.Количество() > 0 Тогда
		Для Каждого СтрокаМесяца Из ДанныеОВыручке.МассивПериодов Цикл
			ВыручкаЗаМесяц = ДанныеОВыручке.ВыручкаЗа3Месяца.Найти(СтрокаМесяца, "Период");
			Контейнер.СовВыр3Месяца["СовВыр3МесяцаМесяц" + Инд] = Формат(Месяц(СтрокаМесяца), "ЧЦ=2; ЧВН=");
			Контейнер.СовВыр3Месяца["СовВыр3МесяцаГод" + Инд]   = Формат(Год(СтрокаМесяца), "ЧГ = 0");
			Контейнер.СовВыр3Месяца["СовВыр3МесяцаСумма" + Инд] = ?(ВыручкаЗаМесяц = Неопределено, 0, ВыручкаЗаМесяц.Выручка);
			Инд = Инд + 1;
		КонецЦикла;
		Контейнер.СовВыр3Месяца.ВыручкаВсего = Контейнер.СовВыр3Месяца["СовВыр3МесяцаСумма1"]
		+ Контейнер.СовВыр3Месяца["СовВыр3МесяцаСумма2"] + Контейнер.СовВыр3Месяца["СовВыр3МесяцаСумма3"];
	КонецЕсли;
	
	Если ДанныеОВыручке.ВыручкаЗаГод.Количество() > 0 Тогда
		Для Каждого СтрокаМесяца Из ДанныеОВыручке.МассивПериодов Цикл
			ВыручкаЗаМесяц = ДанныеОВыручке.ВыручкаЗаГод.Найти(СтрокаМесяца, "Период");
			Контейнер.Выр12Месяцев["Выр12МесяцевМесяц" + Инд] = Формат(Месяц(СтрокаМесяца), "ЧЦ=2; ЧВН=");
			Контейнер.Выр12Месяцев["Выр12МесяцевГод" + Инд]   = Формат(Год(СтрокаМесяца), "ЧГ = 0");
			Контейнер.Выр12Месяцев["Выр12МесяцевСумма" + Инд] = ?(ВыручкаЗаМесяц = Неопределено, 0, ВыручкаЗаМесяц.Выручка);
			Инд = Инд + 1;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьУведомлениеДаннымиПоКнигеПродажИВыручке(Организация, Дата, Контейнер, ДанныеОВыручке)
	...
	// Для раздела 5_01
	Выручка.Вставить("ДатаНачИспОсв", Дата + 1);
	Выручка.Вставить("ВыручкаВсего",  0);
	Выручка.Вставить("Сумма3",        0);
	Выручка.Вставить("Сумма2",        0);
	Выручка.Вставить("Сумма1",        0);
	Выручка.Вставить("Дата3",         ДобавитьМесяц(Дата + 1, - 3));
	Выручка.Вставить("Дата2",         ДобавитьМесяц(Дата + 1, - 2));
	Выручка.Вставить("Дата1",         ДобавитьМесяц(Дата + 1, - 1));
	...
	Если ДанныеОВыручке.ВыручкаПоУСН.Количество() > 0 Тогда
		...
		ДанныеДля5_01(ДанныеОВыручке.ВыручкаПоУСН, Выручка, Разрядность);
	...
	Если ДанныеОВыручке.ВыручкаПоОСН.Количество() > 0 Тогда
		...
		ДанныеДля5_01(ДанныеОВыручке.ВыручкаПоОСН, Выручка, Разрядность);
...
Функция ВыручкаОСНИП(Организация, НачалоПериода, КонецПериода)
	...
	ПараметрыЗаполненияОтчета.Вставить("НалоговаяБазаПоПериодам", Истина);
	...
	Если ВыборкаОбщие.Следующий() Тогда 
		ВыборкаПериоды = ВыборкаОбщие.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Период");
		Пока ВыборкаПериоды.Следующий() Цикл
			НоваяСтр         = ВыручкаИП.Добавить();
			НоваяСтр.Период  = ВыборкаПериоды.Период;
			НоваяСтр.Выручка = ВыборкаПериоды.ИтогоДоходов;
		КонецЦикла;
	КонецЕсли;
...
Функция ДанныеПоВыручкеИДляВыписок(Организация, Дата)
	...
	ТитульнаяСтраница.Вставить("ПризнакЗаполнения", ?(КоличествоМесяцевДляВыручки = 3, "1", "2"));
	ТитульнаяСтраница.Вставить("ДатаНачалаИспОсв",  ПериодыВыписок.ДатаОсвобожденияОтУплатыНДС);
	Если ПериодыВыписок.УСНДатаС > ПустаяДата Тогда
		Если ПериодыВыписок.ОСНДатаС > ПустаяДата Тогда
			НачальнаяДатаВыручки = Мин(ПериодыВыписок.УСНДатаС, ПериодыВыписок.УСНДатаС);
		Иначе
			НачальнаяДатаВыручки = ПериодыВыписок.УСНДатаС;
		КонецЕсли;
	ИначеЕсли ПериодыВыписок.ОСНДатаС > ПустаяДата Тогда
		НачальнаяДатаВыручки = ПериодыВыписок.ОСНДатаС;
	Иначе
		НачальнаяДатаВыручки = ПустаяДата;
	КонецЕсли;
	ТитульнаяСтраница.Вставить("НачальнаяДатаВыручки", НачальнаяДатаВыручки);
	
	ВыручкаЗа3Месяца = Новый ТаблицаЗначений();
	ВыручкаЗаГод     = Новый ТаблицаЗначений();
	МассивПериодов   = Новый Массив();
	ЕстьВыручкаПоУСН = ВыручкаПоУСН.Количество() > 0;
	ЕстьВыручкаПоОСН = ВыручкаПоОСН.Количество() > 0;
	Если КоличествоМесяцевДляВыручки = 3 Тогда
		
		Если ЕстьВыручкаПоУСН И ЕстьВыручкаПоОСН Тогда
			ВыручкаЗа3Месяца = ВыручкаПоУСН;
			Для Каждого Строка Из ВыручкаПоОСН Цикл
				СтрокаВыручки = ВыручкаЗа3Месяца.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаВыручки, Строка);
			КонецЦикла;
			ВыручкаЗа3Месяца.Сортировать("Период");
		ИначеЕсли ЕстьВыручкаПоУСН Тогда
			ВыручкаЗа3Месяца = ВыручкаПоУСН;
		Иначе
			ВыручкаЗа3Месяца = ВыручкаПоОСН;
		КонецЕсли;
		Для Инд = 1 По 3 Цикл
			Если Инд =1 Тогда
				МассивПериодов.Добавить(НачальнаяДатаВыручки);
			Иначе
				МассивПериодов.Добавить(ДобавитьМесяц(НачальнаяДатаВыручки, Инд-1));
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли КоличествоМесяцевДляВыручки = 12 Тогда
		Если ЕстьВыручкаПоУСН И ЕстьВыручкаПоОСН Тогда
			ВыручкаЗаГод = ВыручкаПоУСН;
			Для Каждого Строка Из ВыручкаПоОСН Цикл
				СтрокаВыручки = ВыручкаЗаГод.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаВыручки, Строка);
			КонецЦикла;
			ВыручкаЗаГод.Сортировать("Период");
		ИначеЕсли ЕстьВыручкаПоУСН Тогда
			ВыручкаЗаГод = ВыручкаПоУСН;
		Иначе
			ВыручкаЗаГод = ВыручкаПоОСН;
		КонецЕсли;
	...
	ДанныеПоВыручкеИВыпискам.Вставить("ВыручкаЗа3Месяца",  ВыручкаЗа3Месяца);
	ДанныеПоВыручкеИВыпискам.Вставить("ВыручкаЗаГод",      ВыручкаЗаГод);
	ДанныеПоВыручкеИВыпискам.Вставить("МассивПериодов",    МассивПериодов);
	ДанныеПоВыручкеИВыпискам.Вставить("ТитульнаяСтраница", ТитульнаяСтраница);
	
	Возврат ДанныеПоВыручкеИВыпискам;
	
КонецФункции

Функция ДанныеПоВыручкеИДляВыписокСКнигойПродаж(Организация, Дата)
...
Функция ПериодыФормированияВыписок(Организация, Дата)
	...
	ПараметрыФормированияВыписок.Вставить("ДатаОсвобожденияОтУплатыНДС", '00010101');
	...
	ПараметрыФормированияВыписок.ДатаОсвобожденияОтУплатыНДС  = ДатаОсвобожденияОтУплатыНДС;
...
Процедура ДанныеДля5_01(ТЧВыручки, Выручка, Разрядность)
	
	Сч =1;
	Для Сч = 1 по 3 Цикл
		НайденнаяСтрока = ТЧВыручки.Найти(Выручка["Дата"+Сч], "Период");
		Если НайденнаяСтрока <> Неопределено Тогда
			Выручка["Сумма"+Сч] = НайденнаяСтрока.Выручка;
			Выручка.ВыручкаВсего = Выручка.ВыручкаВсего + НайденнаяСтрока.Выручка;
		КонецЕсли;
	КонецЦикла;
