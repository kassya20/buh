...
#Область РаботаСКоллекциейОписанийОшибок
...
#Область ВерсткаТабличногоДокумента
...
#Область ВыводСообщенийОбОшибкахБезИспользованияТабличногоДокумента

// Процедура формирует сообщение об учетных ошибках, обнаруженных при выполнении рег. операции.
// 
// Параметры:
//  Ошибки - см. НовыйДетальнаяИнформацияОбОшибках
//  Регистратор - ДокументСсылка.РегламентнаяОперация
//              - ДокументОбъект.РегламентнаяОперация
//  Отказ - Булево
//
Процедура СообщитьПользователюОбОшибках(Ошибки, Регистратор, Отказ) Экспорт
	
	Для Каждого СтрокаОшибок Из Ошибки.Строки Цикл
		Если Не ЗначениеЗаполнено(СтрокаОшибок.Данные.Локализация) Тогда
			Продолжить;
		КонецЕсли;
		БухгалтерскийУчетПереопределяемый.СообщитьОбОшибкеРегОперацииСНавигацией(
			СтрокаОшибок.Данные.Локализация,
			СтрокаОшибок.Данные.Ссылка,
			Отказ,
			Регистратор);
	КонецЦикла;
