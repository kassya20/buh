...
#Область ФункцииОбеспеченияРаботыФормДокументовСИмуществомВЭксплуатации
...
#Область ИзменениеЭлементовАмортизации

// Определяет дату, начиная с которой амортизация рассчитывается с учетом изменений элементов амортизации,
// введенных документом ИзменениеПараметровНачисленияАмортизацииОС
//
// Параметры:
//  Дата                      - Дата - дата документа
//  НакоплениеИнформацииЗаГод - Булево - значение одноименного реквизита документа
//  ЗадерживатьПериодДействия - Булево - см. Перечисления.АлгоритмыПериодаАмортизации.ЗадерживатьАмортизацию
// 
// Возвращаемое значение:
//  Дата - дата, начиная с которой применяются изменения
//
Функция ДатаНачалаПримененияИзмененийЭлементовАмортизации(Дата, НакоплениеИнформацииЗаГод, ЗадерживатьПериодДействия) Экспорт
	
	Если НакоплениеИнформацииЗаГод Тогда
		Возврат НачалоГода(Дата);
	ИначеЕсли ЗадерживатьПериодДействия Тогда
		// Изменения параметров учетной политики могут действовать со следующего месяца,
		// поэтому явно об этом сообщим пользователю
		Возврат КонецМесяца(Дата) + 1;
	Иначе
		Возврат Дата;
	КонецЕсли;
