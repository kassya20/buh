...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ВыполнитьОтложенныеРасчетыВФоне", Истина);
...
Процедура ЗарегистрироватьОтложенныеРасчетыСКонтрагентами(Объект, Отказ, ТаблицаОтложенныеРасчетыСКонтрагентами = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Объект.ДополнительныеСвойства, "НеВыполнятьКонтрольУдаляемых", Ложь) Тогда
...
Процедура ЗарегистрироватьОтложенныеРасчетыСКонтрагентамиПриВключенииПоОрганизации(Организация, Знач ДатаНачала, ДатаОкончания, МенеджерВременныхТаблиц, Отказ) Экспорт
	...
	КлючеваяОперация = "ИндексТаблицыСубконто.УчетВзаиморасчетов_ОтложенныеРасчетыСКонтрагентами";
...
Функция НовыеПараметрыРасчета() Экспорт
	...
	Результат.Вставить("ГрупповоеПерепроведение", Ложь); // Если Истина, то проведение выполняется в режиме "ГрупповоеПерепроведение"
...
Функция ВыполнитьОтложенныеРасчеты(ПараметрыРасчета) Экспорт
	...
	Параметры.Вставить("ГрупповоеПерепроведение", 	ПараметрыРасчета.ГрупповоеПерепроведение);
	...
	Параметры.Вставить("КонтрагентПредставление", "");                                              // наименование текущего контрагента
	Параметры.Вставить("ДоговорКонтрагента", 	  Справочники.ДоговорыКонтрагентов.ПустаяСсылка()); // текущий договор
	Параметры.Вставить("ДоговорКонтрагентаПредставление", "");                                      // наименование текущего договора
...
Процедура ПодготовитьДоговорыДляРасчетов(Параметры)
	...
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_Проводки.Контрагент) КАК КонтрагентПредставление,
	|	ВТ_Проводки.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_Проводки.ДоговорКонтрагента) КАК ДоговорКонтрагентаПредставление,
	...
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_Проводки.Контрагент),
	|	ВТ_Проводки.ДоговорКонтрагента,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_Проводки.ДоговорКонтрагента),
	...
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_ДоговорыДляРасчета.Контрагент),
	|	ВТ_ДоговорыДляРасчета.ДоговорКонтрагента,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(ВТ_ДоговорыДляРасчета.ДоговорКонтрагента),
	|	ДоговорыКонтрагентов.Владелец,
	|	РасчетыСКонтрагентамиОтложенноеПроведение.Дата,
	|	РасчетыСКонтрагентамиОтложенноеПроведение.Документ,
	|	РасчетыСКонтрагентамиОтложенноеПроведение.СостояниеРасчетов
...
Процедура ПриИзмененииДоговора(Параметры, Отказ)
	...
	Параметры.КонтрагентПредставление         = ВыборкаДокументы.КонтрагентПредставление;
	Параметры.ДоговорКонтрагента 	          = ВыборкаДокументы.ДоговорКонтрагента;
	Параметры.ДоговорКонтрагентаПредставление = ВыборкаДокументы.ДоговорКонтрагентаПредставление;
...
Процедура ОбработатьСтарыеДвиженияЗачетАвансов(Параметры, Знач СчетаРасчетов, Знач СчетаАвансов)
	...
	Если Параметры.ЭтоВозврат Тогда
		...
		Иначе
			...
			Если ТипЗнч(Параметры.Регистратор) <> Тип("ДокументСсылка.КорректировкаПоступления") Тогда
				СчетаРасчетов = СчетаАвансов;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Пока Счетчик > 0 Цикл
		...
		Иначе
			...
			// Дт 60.01 - Кт 76.02
			Если Параметры.ЭтоВозврат
			   И СчетаРасчетов.Найти(Проводка.СчетДт) <> Неопределено
			   И КонтрагентДт = Контрагент
			   И ДоговорКонтрагентаДт = ДоговорКонтрагента
			...
			// или корректировка аванса поставщику вида Дт 60.02 - Кт 60.01
			ИначеЕсли СчетаРасчетов.Найти(Проводка.СчетДт) <> Неопределено
			   И КонтрагентДт = Контрагент
			   И ДоговорКонтрагентаДт = ДоговорКонтрагента
...
Процедура ЗаполнитьПлатежноРасчетныеДокументыСчетаФактуры(Параметры, ТаблицаВзаиморасчетов)
	...
	Попытка
		...
		Если Параметры.Свойство("ГрупповоеПерепроведение") И Параметры.ГрупповоеПерепроведение Тогда
	...
	Исключение
		...
		ТекстПричины = СтрШаблон(
			...
			Параметры.ДоговорКонтрагентаПредставление,
			Параметры.КонтрагентПредставление);
...
Функция ПодготовитьПлатежноРасчетныеДокументыЗачетАвансов(Параметры, ТаблицаВзаиморасчетов)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ДокументыРасчетов.ДокументРасчетов = ДанныеПервичныхДокументов.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК СчетФактураДокументыОснования
	|		ПО ДокументыРасчетов.ДокументРасчетов = СчетФактураДокументыОснования.ДокументОснование
	|			И (&Период >= СчетФактураДокументыОснования.Ссылка.Дата)
