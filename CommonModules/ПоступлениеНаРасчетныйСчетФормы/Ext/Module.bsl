...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		...
		// Если документ скопирован и есть расчеты в у.е. или валюте, пересчитаем суммы взаиморасчетов на дату документа
...
Процедура ПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	ПлательщикНДССВычетами = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПлательщикНДССВычетами(Форма);
	Документы.ПоступлениеНаРасчетныйСчет.ПроверитьСоздатьВидОплаты(
		ТекущийОбъект, Форма.СвойстваПлатежа, ПлательщикНДССВычетами);
...
Процедура СуммаДокументаПриИзменении(Форма) Экспорт
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Инкассация
		...
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносНаличными
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал Тогда
		
		СтрокаПлатеж = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма, Неопределено);
		СтрокаПлатеж.СуммаПлатежа = Объект.СуммаДокумента;
		Если (Форма.ПрименениеУСН Или Форма.ПрименяетсяАУСН)
			И Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
			Или Форма.ПрименяетсяАУСН
				И Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога Тогда
...
&НаСервере
Процедура ВторойСчетОрганизацииПриИзменении(Форма) Экспорт
	...
	Если Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма)
		И Элементы.СчетБанк.Видимость <> СчетБанкаВиден(Форма) Тогда
...
Процедура ВидОперацииОбработатьИзменение(Форма, ПричиныИзменения)
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
		Объект.Контрагент      = Справочники.Контрагенты.ПустаяСсылка();
		ЦифровойСчетКор        = ЗначениеЗаполнено(Объект.СчетКонтрагента)
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетКонтрагента, "ЦифровойСчет");
		Если ЦифровойСчетКор = Неопределено Или ЦифровойСчетКор Тогда
			Объект.СчетКонтрагента = Справочники.БанковскиеСчета.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ДокументМожетБытьОформленВЦифровыхРублях(Объект.ВидОперации) Тогда
		ЦифровыеДеньгиПараметр = Ложь;
	Иначе
		ЦифровыеДеньгиПараметр = Форма.ЦифровыеДеньги;
	КонецЕсли;
	...
	УстановитьПараметрыВыбораСтатейДДС(Форма);
	...
	Если Документы.ПоступлениеНаРасчетныйСчет.ЕстьРасшифровкаПлатежа(Объект.ВидОперации) Тогда
		...
		ПараметрыОбъекта.ПорядокОтраженияАванса = Неопределено;
	...
	Если Объект.ВидОперации =
		...
		СтрокаПлатеж = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма);
		ЕстьТабличнаяЧасть = Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма);
		
		ПлатежныеКартыДоговорКонтрагентаПриИзмененииНаСервере(СтрокаПлатеж, Форма, ЕстьТабличнаяЧасть, ПричиныИзменения);
	КонецЕсли;
	
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит
...
Процедура ДатаПриИзмененииСервер(Форма) Экспорт
	...
	Если Форма.ПлательщикНДСПриУСН Тогда
		...
		Форма.СвойстваПлатежа.Вставить("СтавкаНДС", Форма.СтавкаНДСПриУСН);
	ИначеЕсли Не Форма.ПлательщикНДС Тогда
		ИнициализироватьСвойстваПлатежа(Форма);
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		И (Не Форма.ПлательщикНДС ИЛИ Форма.ПлательщикНДСПриУСН) Тогда
		Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
			И Объект.РасшифровкаПлатежа.Количество() = 1 Тогда
			СтрокаПлатеж = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.РасшифровкаПлатежаТекущиеДанные(Форма);
			СтрокаПлатеж.СуммаПлатежа = Объект.СуммаДокумента;
			Если СтрокаПлатеж.ПорядокОтраженияАванса = ПредопределенноеЗначение("Перечисление.ПорядокОтраженияАвансов.ДоходПатент")
				Или ТипЗнч(СтрокаПлатеж.ПорядокОтраженияАванса) = Тип("СправочникСсылка.Патенты") Тогда
				СтрокаПлатеж.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			Иначе
				СтрокаПлатеж.СтавкаНДС    = Форма.СвойстваПлатежа.СтавкаНДС;
			КонецЕсли;
			ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПересчитатьСуммуНДС(СтрокаПлатеж);
			ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Истина, СтрокаПлатеж);
			ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Ложь, СтрокаПлатеж);
	...
	УстановитьПорядокОтраженияАванса(Форма);
	УправлениеЭлементамиОтраженияВУСННаСервере(Форма);
...
Процедура ОрганизацияПриИзмененииСервер(Форма) Экспорт
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ЛичныеСредстваПредпринимателя
		...
		Объект.СчетКонтрагента = Справочники.БанковскиеСчета.ПустаяСсылка();
		УстановитьВладельцаСчетаКонтрагента(Форма);
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
	...
	БанкИКассаФормы.УстановитьРасчетыПриОплате(Форма);
	УстановитьВидимостьСтраниц = БезЗакрывающихДокументов <> Объект.БезЗакрывающихДокументов;
	...
	Если УстановитьВидимостьСтраниц Тогда
		УстановитьВидимостьСтраницНаСервере(Форма);
	КонецЕсли;
...
Процедура ПроверитьКурсЦБРФ(Форма)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПриобретениеИностраннойВалюты Тогда
		...
		Если Объект.КурсНаДатуПриобретенияРеализацииВалюты <> СтруктураКурса.Курс Тогда
			ШаблонСообщения =
				НСтр("ru = 'Курс ЦБ РФ ""%1"" на дату операции %2 с валютой %3 не соответствует установленному курсу ""%4"".
...
Процедура ПерезаполнитьРасшифровкуПлатежа(Форма)
	...
	Для Каждого СтрокаШаблон Из ШаблонЗаполнения Цикл
		...
		Если ВидОперацииСТабличнойЧастью(Объект.ВидОперации) Тогда
			...
			СтрокаПлатеж.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(Объект.ВидОперации, Объект.Дата);
...
Процедура ПерезаполнитьСтатьюДДС(Форма)
	...
	Если НЕ ВидОперацииСТабличнойЧастью(Объект.ВидОперации) Тогда
		Объект.СтатьяДвиженияДенежныхСредств = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(Объект.ВидОперации, Объект.Дата);
	КонецЕсли;
...
Процедура УстановитьВидимостьСтраницНаСервере(Форма) Экспорт
	...
	Если НЕ ФормаРасшифровкиПлатежа Тогда
		...
		Если Элементы.Контрагент.Видимость Тогда
			Если Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма)
				И Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал Тогда
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
		Если ФормаДокументаОднострочная Тогда
			ТекущаяГруппа = ?(Объект.БезЗакрывающихДокументов
				И Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма),
		...
		КонецЕсли;
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
		Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога Тогда
...
Процедура ПодготовитьФормуНаСервере(Форма) Экспорт
	...
	Форма.ЦифровыеДеньги = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ДокументМожетБытьОформленВЦифровыхРублях(Объект.ВидОперации)
	...
	ЭтоФормаРасшифровкиПлатежа = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма);
	
	Если ЭтоФормаРасшифровкиПлатежа Тогда
	...
	Если Объект.БезЗакрывающихДокументов Тогда
		...
		Если Не ЗначениеЗаполнено(Форма.СвойстваПлатежа.ПорядокОтраженияАванса)
			И (ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
			Или ПараметрыОбъекта.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам) Тогда
			УчетПСН.УстановитьПорядокОтраженияАванса(Форма.СвойстваПлатежа.ПорядокОтраженияАванса, ПараметрыОбъекта);
		КонецЕсли;
	...
	МобильныйКлиентАдаптацияФормыПриСоздании(Форма);
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЦветПредупрежденияОНастройке") Тогда
		Форма.ЦветОтсутствияПредупреждения = ЦветаСтиля.ЦветФонаФормы;
		Форма.ЦветПредупрежденияОНастройке = ЦветаСтиля.ЦветБаннераБежевый;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма) Экспорт
	...
	Если Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		Элементы.ГруппаПереводСДругогоСчета.Видимость =
			Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
				Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит;
		
		Элементы.ВторойСчетОрганизации.РасширеннаяПодсказка.Заголовок = "";
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
			ВыпискаЗагружаетсяПоДепозитномуСчету = ЗначениеЗаполнено(Объект.СчетОрганизации)
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетОрганизации, "ВыпискаЗагружаетсяПоДепозитномуСчету");
			ВыпискаЗагружаетсяНаДепозитныйСчет = ЗначениеЗаполнено(Объект.СчетКонтрагента)
				И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетКонтрагента, "ВыпискаЗагружаетсяПоДепозитномуСчету");
			
			Элементы.ВторойСчетОрганизации.РасширеннаяПодсказка.Заголовок =
				Документы.ПоступлениеНаРасчетныйСчет.ТекстОтсутствияДвиженийПоДепозиту(
					ВыпискаЗагружаетсяНаДепозитныйСчет, ВыпискаЗагружаетсяПоДепозитномуСчету);
			Элементы.ВторойСчетОрганизации.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
		КонецЕсли;
		...
		КонецЕсли;
		Если Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма) Тогда
			Элементы.КонтрагентЮрФизЛицо.Видимость =
				БанкИКассаФормыКлиентСервер.ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма);
		КонецЕсли;
	КонецЕсли;
	...
	Если БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма) Тогда
		...
		ЭтоВозвратЗайма =
			Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаКонтрагентом;
		ЭтоПрочиеРасчетыСКонтрагентами =
			Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочиеРасчетыСКонтрагентами;
		ЭтоПрочееПоступление =
			Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление;
		
		Форма.ВсяСуммаЯвляетсяДоходом = Объект.СуммаДокумента = Объект.Графа5_УСН;
		
		Элементы.ВозвратЗаймаСуммаПроцентов.Видимость = ЭтоВозвратЗайма
			Или ((ЭтоПрочиеРасчетыСКонтрагентами
			Или ЭтоПрочееПоступление) И Не Форма.ВсяСуммаЯвляетсяДоходом);
		
		Элементы.ВсяСуммаЯвляетсяДоходом.Видимость = ЭтоПрочиеРасчетыСКонтрагентами Или ЭтоПрочееПоступление;
		
		Если ЭтоПрочиеРасчетыСКонтрагентами Или ЭтоПрочееПоступление Тогда
			Элементы.ВозвратЗаймаСуммаПроцентов.Заголовок = ТекстЗаголовкаДоход();
		Иначе
			Элементы.ВозвратЗаймаСуммаПроцентов.Заголовок = ТекстЗаголовкаПроценты();
		КонецЕсли;
		
	ИначеЕсли ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаКонтрагентом Тогда
			Элементы.ДоходыУСН.Заголовок = ТекстЗаголовкаПроценты();
		Иначе
			Элементы.ДоходыУСН.Заголовок = ТекстЗаголовкаДоходыУСН();
		КонецЕсли;
	Иначе
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаКонтрагентом Тогда
			Элементы.РасшифровкаПлатежаДоходыУСН.Заголовок = ТекстЗаголовкаПроценты();
		Иначе
			Элементы.РасшифровкаПлатежаДоходыУСН.Заголовок = ТекстЗаголовкаДоходыУСН();
		КонецЕсли;
	КонецЕсли;
	...
	АдаптироватьЭлементыМКФормыДокумента(Форма);
...
Процедура НастроитьЭлементыДляНПД(Форма)
	...
	ВключенаИнтеграция = ЧекиНПДВызовСервера.ВключенаИнтеграцияСПлатформойСамозанятые();
	...
	Иначе
		...
		Если ВидимостьЭлементовНПД Тогда
			...
			Элементы.ГруппаЧекНПДКнопка.Видимость = Не ЧекСформирован И ВключенаИнтеграция;
...
Процедура УстановитьПодразделениеКт(Форма)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
		// При переводе на счет депозита значение заполнения для ПодразделениеКт зависит от подразделения расчетного счета,
		...
		Если Не ЗначениеЗаполнено(Объект.СчетКонтрагента) Тогда
			Объект.ПодразделениеКт = Объект.ПодразделениеОрганизации;
		КонецЕсли;
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
...
Процедура ВидимостьДоступностьЭлементовФормы(Форма) Экспорт
	...
	ЭтоВозвратУплатыНалога = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога;
	ЭлементНалог = Элементы.Найти("Налог");
	Если ЭлементНалог <> Неопределено Тогда
		ЭлементНалог.Видимость = ЭтоВозвратУплатыНалога;
	КонецЕсли;
	...
	Если НазваниеТабличнойЧасти = "РасшифровкаПлатежа" Или НазваниеТабличнойЧасти = "РасшифровкаПлатежаПлатежныеКарты" Тогда
		...
		Если Не БанкИКассаФормыКлиентСервер.ФормаДокументаИнтеграцииСБанком(Форма) Тогда
			Элементы.СчетНаОплатуБезДокументов.Видимость = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя;
		КонецЕсли;
		...
		Если Документы.ПоступлениеНаРасчетныйСчет.ЕстьРасшифровкаПлатежа(Объект.ВидОперации) Тогда
			...
			БанкИКассаФормыКлиентСервер.УстановитьВидимостьПодсказкиОтраженияАванса(Форма, НазваниеТабличнойЧасти);
		КонецЕсли;
	...
	УстановитьВидимостьНаправленияДеятельности(Форма);
	УстановитьДоступностьДоговораОднострочнойФормы(Форма);
...
Процедура ОтобразитьГиперссылкуПерейтиВТаблицу(Форма) Экспорт
	...
	Элементы.СуммаВозврата.Видимость = Ложь;
	Элементы.ПлатежныеКартыСуммаВозврата.Видимость = Ложь;
	...
	Если КоличествоСтрок = 1 Тогда
		...
		Элементы.ПлатежныеКартыСуммаВозврата.Видимость = ПлатежныеКартыИКредиты;
		
		Если ПлатежныеКартыИКредиты Тогда
			Элементы.ГруппаПлатежныеКартыПогашениеЗадолженности.Видимость = Истина;
		КонецЕсли;
	...
	Иначе
		...
		Элементы.СуммаВозврата.Видимость = ПлатежныеКартыИКредиты;
		
		Если ПлатежныеКартыИКредиты Тогда
			Элементы.ГруппаПлатежныеКартыПогашениеЗадолженности.Видимость = Ложь;
		КонецЕсли;
...
Процедура УстановитьФункциональныеОпцииФормы(Форма) Экспорт
	...
	Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) 
		...
		Форма.ЕстьБюджетНаПериод = Документы.Бюджет.ЕстьБюджетНаПериод(Объект.Организация, Объект.Дата);
	КонецЕсли;
...
Функция СчетаБанка(Объект) Экспорт
	...
	Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ДокументМожетБытьОформленВЦифровыхРублях(Объект.ВидОперации) Тогда
	...
	ИначеЕсли Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ДокументМожетБытьОформленВВалюте(Объект.ВидОперации) Тогда
		ОтбиратьПоВалюте = Истина;
...
Процедура УстановитьПараметрыВыбораДоговораСКонтрагентом(Форма)
	...
	ВидыДоговора   = ПоступлениеНаРасчетныйСчетФормыВызовСервера.ВидыДоговоровДокумента(Объект.ВидОперации);
...
Функция НовыеПараметрыПриИзмененииСчетаОрганизации(Знач ПараметрыОбъекта)
	...
	Если ПараметрыОбъекта.ВалютаДокумента = НоваяВалюта Тогда
		...
		НовыеПараметры.Вставить("НазначениеПлатежа",        ПараметрыОбъекта.НазначениеПлатежа);
		НовыеПараметры.Вставить("СчетКонтрагента",          ПараметрыОбъекта.СчетКонтрагента);
		НовыеПараметры.Вставить("КурсДокумента",            ПараметрыОбъекта.КурсДокумента);
		НовыеПараметры.Вставить("КратностьДокумента",       ПараметрыОбъекта.КратностьДокумента);
	...
	НовыеПараметры.Вставить("ЦифровыеДеньги",           НовыйЦифровыеДеньги);
	НовыеПараметры.Вставить("ОплатаВВалюте", НоваяВалюта <> ПараметрыОбъекта.ВалютаРегламентированногоУчета);
...
Функция НовыеПараметрыПриИзмененииОрганизации(Знач ПараметрыОбъекта)
	...
	Если ЗначениеЗаполнено(ПараметрыОбъекта.Организация) Тогда
		...
		УчетДенежныхСредствБП.УстановитьБанковскийСчет(
			...
			ПараметрыОбъекта.ВалютаРегламентированногоУчета, НЕ ПараметрыОбъекта.ОплатаВВалюте, УчитыватьВалюту,,,
	...
	Если ПараметрыОбъекта.ПлательщикНДСПриУСН Тогда
		НовыеПараметры.Вставить("СтавкаНДС", ПараметрыОбъекта.СтавкаНДСПриУСН);
	ИначеЕсли Не ПараметрыОбъекта.ПлательщикНДС Тогда
...
Функция НовыеПараметрыПриИзменении(Знач ПараметрыОбъекта, Знач ОплатаВВалюте, Знач ЦифровыеДеньги)
...
Функция ТекущиеПараметрыОбъекта(Форма)
	...
	ПараметрыОбъекта = Новый Структура(
		...
		|СуммаДокумента, СуммаПлатежа, СуммаУслуг, СуммаВозврата,
		|Контрагент, СчетКонтрагента, СчетБанк, ОтражатьРазницуВКурсеВСоставеОперационныхРасходов, Ссылка,
		|НазначениеПлатежа,
		|ПлательщикНДСПриУСН, СтавкаНДСПриУСН");
	...
	Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		ПараметрыОбъекта.Вставить("ПорядокОтраженияАванса", Форма.РасшифровкаПлатежаПорядокОтраженияАванса);
	Иначе
		ПараметрыОбъекта.Вставить("ПорядокОтраженияАванса",
			?(Форма.СвойстваПлатежа = Неопределено, Неопределено, Форма.СвойстваПлатежа.ПорядокОтраженияАванса));
	КонецЕсли;
	
	Если Форма.СвойстваПлатежа <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ПараметрыОбъекта, Форма.СвойстваПлатежа,
			"ДоговорКонтрагента, УслугаНПД, СтатьяДвиженияДенежныхСредств, СуммаПлатежа, СуммаУслуг, СуммаВозврата");
	КонецЕсли;
...
Функция ЭтоПеремещениеДенежныхСредств(ВидОперации)
	...
	Возврат ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета
		ИЛИ ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит
...
Функция ВидимостьАналитики(ВидОперации)
	...
	Возврат НЕ ЭтоПеремещениеДенежныхСредств(ВидОперации)
		...
		И ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал
		И ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога
...
Процедура УстановитьОграничениеТипаКонтрагента(Форма) Экспорт
	...
	Если БанкИКассаФормыКлиентСервер.ВидимостьДоступностьКонтрагентаЮрФизЛицо(Форма) Тогда 
		...
		КонецЕсли;
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаРаботником
...
Процедура УстановитьДоступностьДоговораОднострочнойФормы(Форма)
	...
	ТабличнаяЧасть = ?(ПустаяСтрока(НазваниеТабличнойЧасти) Или Элементы.Найти(НазваниеТабличнойЧасти) = Неопределено,
	...
	Элементы[ТабличнаяЧасть + "ДоговорКонтрагента"].Доступность = ЗначениеЗаполнено(Объект.Организация)
		И ЗначениеЗаполнено(Объект.Контрагент);

КонецПроцедуры

Функция ТекстЗаголовкаДоходыУСН()
	Возврат НСтр("ru = 'Доходы УСН'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции

Функция ТекстЗаголовкаПроценты()
	Возврат НСтр("ru = 'в т.ч. проценты'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции

Функция ТекстЗаголовкаДоход()
	Возврат НСтр("ru = 'в т.ч. доход'", ОбщегоНазначения.КодОсновногоЯзыка());
КонецФункции

Процедура УстановитьПорядокОтраженияАванса(Форма)
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		...
		УчетПСН.УстановитьПорядокОтраженияАванса(НовыйПорядок, Объект);
		Форма.СвойстваПлатежа.ПорядокОтраженияАванса = НовыйПорядок;
		ПредставлениеДохода = БанкИКассаФормыКлиентСервер.ВариантОтраженияДоходовПредставление(
			НовыйПорядок, БанкИКассаФормыКлиентСервер.ВариантыОтраженияДоходов(Форма));
		Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
			Форма.РасшифровкаПлатежаОтражениеАвансаПредставление = ПредставлениеДохода;
			Форма.РасшифровкаПлатежаОтражениеДоходаПредставление = ПредставлениеДохода;
			Форма.РасшифровкаПлатежаПорядокОтраженияАванса = НовыйПорядок;
		Иначе
			Для Каждого СтрокаПлатеж Из Объект.РасшифровкаПлатежа Цикл
				СтрокаПлатеж.ПорядокОтраженияАванса = НовыйПорядок;
...
Функция НовыйРеквизитыПодсказки()
	...
	РеквизитыПодсказки.Вставить("ИмяРеквизитаДоговорКонтрагента", "ДоговорКонтрагента");
	РеквизитыПодсказки.Вставить("ИмяРеквизитаДоговорКонтрагентаРасширеннаяПодсказка", 
		"ДоговорКонтрагентаРасширеннаяПодсказка");
...
Процедура УстановитьПараметрыВыбораСтатейДДС(Форма)
	
	Элементы = Форма.Элементы;
	Объект = Форма.Объект;
	
	НовыйМассивПараметров = Новый Массив;
	МассивВидовСтатей = Новый Массив;
	
	Если БюджетДоходовРасходовКлиентСервер.ОперацияУчитываетсяВБюджетеРасходы(Объект.ВидОперации) Тогда
		МассивВидовСтатей.Добавить(Перечисления.ВидСтатьиДвиженияДенег.Расход);
	Иначе
		МассивВидовСтатей.Добавить(Перечисления.ВидСтатьиДвиженияДенег.Доход);
	КонецЕсли;
	МассивВидовСтатей.Добавить(Перечисления.ВидСтатьиДвиженияДенег.ДоходРасход);
	НовыйМассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ВидСтатьиДДС", МассивВидовСтатей));
	
	МассивИменЭлементов = Новый Массив;
	МассивИменЭлементов.Добавить("СтатьяДвиженияДенежныхСредствРасшифровкаПлатежа");
	МассивИменЭлементов.Добавить("СтатьяДвиженияДенежныхСредствИнкассация");
	МассивИменЭлементов.Добавить("ПокупкаВалютыСтатьяДвиженияДенежныхСредств");
	МассивИменЭлементов.Добавить("ПродажаВалютыСтатьяДвиженияДенежныхСредств");
	МассивИменЭлементов.Добавить("ПлатежныеКартыСтатьяДвиженияДенежныхСредств");
	МассивИменЭлементов.Добавить("РасшифровкаПлатежаСтатьяДвиженияДенежныхСредствБезДокументов");
	МассивИменЭлементов.Добавить("РасшифровкаПлатежаПлатежныеКартыСтатьяДвиженияДенежныхСредствБезДокументов");
	
	Для Каждого ИмяЭлемента Из МассивИменЭлементов Цикл
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы, ИмяЭлемента, "ПараметрыВыбора", Новый ФиксированныйМассив(НовыйМассивПараметров));
	КонецЦикла;
		
КонецПроцедуры

Процедура УстановитьВидимостьНаправленияДеятельности(Форма)
	
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ЕстьБюджетНаПериод") Тогда
		Возврат;
	КонецЕсли;
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	ОперацияУчитываетсяВРасходах = БюджетДоходовРасходовКлиентСервер.ОперацияУчитываетсяВБюджетеРасходы(Объект.ВидОперации);
		
	ВидимостьНаправленияДеятельности = Ложь;
	Если Форма.ЕстьБюджетНаПериод Тогда
		ВидимостьНаправленияДеятельности = ОперацияУчитываетсяВРасходах;
	КонецЕсли;
	
	ЭлементНаправлениеДеятельности = Элементы.Найти("НаправлениеДеятельности");
	Если ЭлементНаправлениеДеятельности <> Неопределено Тогда
		ЭлементНаправлениеДеятельности.Видимость = ВидимостьНаправленияДеятельности;
	КонецЕсли;
...
// Настраивает видимость и заголовки элементов формы, связанных с отражением документа
// в учете при упрощенной системе налогообложения (включая Автоматизированную УСН)
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма документа "ПоступлениеНаРасчетныйСчет"
//
Процедура УправлениеЭлементамиОтраженияВУСННаСервере(Форма) Экспорт
	...
	БанкИКассаФормы.НастроитьЭлементыИсточникаРазметкиАУСН(Форма,
		СтрРазделить("ИсточникРазметкиАУСН,ПрочееИсточникРазметкиАУСН", ","));
	
	ВидимостьИсточникаРазметкиАУСН = Форма.ПрименяетсяАУСН;
	ОтображатьСтатусОбменаАУСН = Не ЭтоФормаРасшифровкиПлатежа И Форма.ПрименяетсяАУСН И Форма.ПодключенСервисАУСН;
	...
	Если НазваниеТабличнойЧасти = "РасшифровкаПлатежа" Или НазваниеТабличнойЧасти = "РасшифровкаПлатежаПлатежныеКарты" Тогда
		...
		Если (Форма.ПрименениеУСН Или Форма.ПрименяетсяАУСН)
			И УчетУСН.ТребуетсяНастройкаДоходовПоПрочимРасчетам(Объект.ВидОперации) Тогда
		...
		Элементы[Префикс + "ДоходыУСН"].Видимость = ОтображатьДоходыУСН И Не ПрименяетсяРазметкаАУСНБанка;
		
		Если Не ЭтоФормаРасшифровкиПлатежа Тогда
			Элементы.ГруппаИсточникРазметкиАУСН.Видимость = ОтображатьДоходыУСН И ВидимостьИсточникаРазметкиАУСН;
			Элементы.СтатусАУСН.Видимость = ОтображатьДоходыУСН И ОтображатьСтатусОбменаАУСН;
		КонецЕсли;
	...
	Если Не ЭтоФормаРасшифровкиПлатежа Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление
			Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога
				И Форма.ПрименяетсяАУСН Тогда
			ПрименяетсяАУСН = Форма.ПрименяетсяАУСН;
			Элементы.ГруппаОтражениеВУСН.Видимость = Форма.ПрименениеУСН Или ПрименяетсяАУСН;
			Элементы.ГруппаОтражениеВУСН.ОтображатьЗаголовок = Истина;
			Элементы.ОтражениеВУСН.Видимость = ПрименяетсяАУСН И Не ПрименяетсяРазметкаАУСНБанка;
			ПризнаватьПроцентПоВкладам =
				НалоговыйУчетУСН.ПризнаватьДоходомПроцентыПоВкладам(Объект.Организация, Объект.Дата, Объект.СубконтоКт1);
			Элементы.СодержаниеУСН.Видимость = Не ПрименяетсяАУСН И ПризнаватьПроцентПоВкладам;
			ВидимостьСуммыДоходовУСН = (Не ПрименяетсяАУСН Или Объект.ОтражениеВУСН = Перечисления.ОтражениеВУСН.Принимаются)
				 И Не ПрименяетсяРазметкаАУСНБанка И ПризнаватьПроцентПоВкладам;
			Элементы.ПрочееДоходыУСН.Видимость = ВидимостьСуммыДоходовУСН;
			Элементы.НадписьВалютаРеглПрочие.Видимость = ВидимостьСуммыДоходовУСН;
			Элементы.ПрочееТекстДоходыУСН.Видимость = ВидимостьСуммыДоходовУСН;
			Если ПрименяетсяАУСН Тогда
				Элементы.ГруппаОтражениеВУСН.Заголовок = НСтр("ru = 'Отражение в АУСН'");
				Элементы.ПрочееДоходыУСН.ФорматРедактирования = "ЧЦ=15; ЧДЦ=2; ЧРД=,; ЧРГ=' '; ЧГ=3,0";
			Иначе
				Элементы.ГруппаОтражениеВУСН.Заголовок = НСтр("ru = 'Отражение в УСН'");
				Элементы.ПрочееДоходыУСН.ФорматРедактирования = "ЧЦ=15; ЧДЦ=2; ЧРД=,; ЧРГ=' '; ЧН='<Не принимаются>'; ЧГ=3,0";
			КонецЕсли;
			Элементы.ГруппаПрочееИсточникРазметкиАУСН.Видимость = ВидимостьИсточникаРазметкиАУСН;
			Элементы.ПрочееСтатусАУСН.Видимость = ОтображатьСтатусОбменаАУСН;
		ИначеЕсли Форма.ПрименяетсяАУСН
				И (Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
					Или Объект.ВидОперации = ПоступлениеПоПлатежнымКартам) Тогда
			...
			Элементы.ПрочееДоходыУСН.Видимость = Ложь;
			Элементы.НадписьВалютаРеглПрочие.Видимость = Ложь;
			Элементы.ПрочееТекстДоходыУСН.Видимость = Ложь;
			Элементы.ГруппаПрочееИсточникРазметкиАУСН.Видимость = ВидимостьИсточникаРазметкиАУСН;
			Элементы.ПрочееСтатусАУСН.Видимость = ОтображатьСтатусОбменаАУСН;
		ИначеЕсли ВидимостьИсточникаРазметкиАУСН
			И (Не ОтображатьДоходыУСН
				Или ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
					И Объект.РасшифровкаПлатежа.Количество() > 1) Тогда
			Элементы.ГруппаОтражениеВУСН.Видимость = Истина;
			Элементы.ГруппаОтражениеВУСН.ОтображатьЗаголовок = Ложь;
			Элементы.СодержаниеУСН.Видимость = Ложь;
	...
	Если Форма.ПрименениеУСН
		И ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратЗаймаКонтрагентом Тогда
			Элементы.ДоходыУСН.ФорматРедактирования = "ЧЦ=15; ЧДЦ=2; ЧРД=,; ЧРГ=' '; ЧГ=3,0";
		Иначе
			Элементы.ДоходыУСН.ФорматРедактирования = "ЧЦ=15; ЧДЦ=2; ЧРД=,; ЧРГ=' '; ЧН='<Не принимаются>'; ЧГ=3,0";
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗаголовкиКолонокУСН(Форма)
	...
	ТребуетсяНастроитьУСН = Форма.ПрименениеУСН
		И УчетУСН.ТребуетсяНастройкаДоходовПоПрочимРасчетам(Объект.ВидОперации);
	
	ТребуетсяНастроитьАУСН = Форма.ПрименяетсяАУСН
		И РазметкаОперацийАУСН.РучнаяНастройкаДоходов(Объект.ВидОперации);
	
	Если Не ТребуетсяНастроитьУСН И Не ТребуетсяНастроитьАУСН Тогда
		Возврат;
	КонецЕсли;
	...
	ПрочиеРасчетыСКонтрагентами = УчетУСН.ТребуетсяНастройкаДоходовПоПрочимРасчетам(Объект.ВидОперации);
	
	Если ПрочиеРасчетыСКонтрагентами Тогда
		
		Если Форма.ОплатаВВалюте Тогда
			ЗаголовокДоходыУСН = СтрШаблон(НСтр("ru = 'Доходы %1, %2'"),
				НалоговыйРежим,
				Форма.ВалютаРегламентированногоУчета);
		Иначе
			ЗаголовокДоходыУСН = СтрШаблон(НСтр("ru = 'Доходы %1'"), НалоговыйРежим);
		КонецЕсли;
	...
	Иначе
		...
		Если ЭлементОтражениеВУСН <> Неопределено Тогда
			Элементы.ОтражениеВУСН.Заголовок = СтрШаблон(НСтр("ru = 'Доходы %1'"), НалоговыйРежим);
		КонецЕсли;
		
		Если Форма.ОплатаВВалюте Тогда
			ЗаголовокДоходыУСН = СтрШаблон(НСтр("ru = 'Сумма дохода, %1'"), Форма.ВалютаРегламентированногоУчета);
		Иначе
			ЗаголовокДоходыУСН = НСтр("ru = 'Сумма дохода'");
		КонецЕсли;
		
		Элементы[ТабличнаяЧасть + "ДоходыУСН"].Заголовок = ЗаголовокДоходыУСН;
		
		ЭлементПрочееДоходыУСН = Элементы.Найти("ПрочееДоходыУСН");
		Если ЭлементПрочееДоходыУСН <> Неопределено Тогда
			ЭлементПрочееДоходыУСН.Заголовок = ЗаголовокДоходыУСН;
		КонецЕсли;
...
Процедура ЗаполнитьОтражениеВУСННаСервере(Форма, ЗаполнитьСуммы = Истина, ЗаполнитьСодержание = Ложь) Экспорт
	...
	Если Объект.Графа5_УСН > 0 Тогда
...
Процедура ЗаполнитьСпискиВыбораОтражениеВУСН(Форма)
	...
	ЭлементОтражениеВУСН = Форма.Элементы.Найти("ОтражениеВУСН");
	
	Если ЭлементОтражениеВУСН <> Неопределено Тогда
		
		ЭтоВозвратНалога        = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога;
		ВозможенВозвратРасходов = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступление;
		
		ДоступныеОтражения = Перечисления.ОтражениеВУСН.СписокВыбора(Ложь, Ложь, ВозможенВозвратРасходов, ЭтоВозвратНалога);
		
		ЭлементОтражениеВУСН.СписокВыбора.Очистить();
		Для Каждого ВариантОтражения Из ДоступныеОтражения Цикл
			ЭлементОтражениеВУСН.СписокВыбора.Добавить(ВариантОтражения.Значение, ВариантОтражения.Представление);
		КонецЦикла;
		
		// При оплате от покупателя и эквайринге вариант "Не принимаются" имеет особое представление.
		ПоступлениеПоКартам =
			Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам;
		Если Форма.Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
			Или Форма.Объект.ВидОперации = ПоступлениеПоКартам Тогда
			ЭлементСпискаНеПринимаются = ЭлементОтражениеВУСН.СписокВыбора.НайтиПоЗначению(Перечисления.ОтражениеВУСН.НеПринимаются);
			Если ЭлементСпискаНеПринимаются <> Неопределено Тогда
				ЭлементСпискаНеПринимаются.Представление = НСтр("ru = 'Не принимаются (отражены кассовым чеком)'");
			КонецЕсли;
		КонецЕсли;
...
Процедура НалогПриИзмененииНаСервере(Форма, ПричиныИзменения) Экспорт
...
Процедура ПлатежныеКартыДоговорКонтрагентаПриИзмененииНаСервере(СтрокаПлатеж,
																Форма,
																Знач ЕстьТабличнаяЧасть = Истина,
																ПричиныИзменения = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПричиныИзменения) Тогда
		ПричиныИзменения = Новый Массив;
	КонецЕсли;
	
	ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ЗаполнитьРеквизитыРасшифровкаПлатежа(Форма, Ложь, СтрокаПлатеж);
	
	Если Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
		И Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		Объект.ДоговорКонтрагента = СтрокаПлатеж.ДоговорКонтрагента;
	КонецЕсли;
	
	Если Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		ПричинаИзменения = "ДоговорКонтрагента";
		ПричиныИзменения.Добавить(ПричинаИзменения);
		ПараметрыЗаполнения = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.НачатьЗаполнениеСчетовУчета(
			ПричиныИзменения, Объект, Форма.СвойстваПлатежа);
		СчетаУчетаВДокументах.ЗаполнитьОбъектПриИзменении(ПараметрыЗаполнения);
		БухгалтерскийУчетКлиентСервер.УстановитьСвойстваСубконтоШапкиПриИзмененииСчета(
			Форма,
			Форма.Объект,
			ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ПараметрыУстановкиСвойствСубконто(Форма,, "УслугиБанка"));
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(Знач ПараметрыПлатежа, Знач ПолучатьДоговор) Экспорт
	...
	СтатьяДДСПоУмолчанию = УчетДенежныхСредствБП.СтатьяДДСПоУмолчанию(ПараметрыПлатежа.ВидОперации, ПараметрыПлатежа.Дата);
	...
	Если ЗначениеЗаполнено(ПараметрыПлатежа.ДоговорКонтрагента) Тогда
		...
		Если ПараметрыПлатежа.ВидОперации
			...
			Операция.Вставить("НазначениеПлатежа", ПараметрыПлатежа.НазначениеПлатежа);
			Операция.Вставить("РазделителиИзНазначения");
			Операция.Вставить("СловаНазначениеПлатежа");
			ЭтоСБП = ЗагрузкаВыпискиПоБанковскомуСчету.НазначениеПлатежаУказываетНаСБП(Операция);
			ТипОплаты = ?(ЭтоСБП, Перечисления.ТипыОплат.СБП, Перечисления.ТипыОплат.ПлатежнаяКарта);
			СвойстваПлатежа.ВидОплаты = Справочники.ВидыОплатОрганизаций.ВидОплатыПоДоговоруИТипуОплаты(
				ПараметрыПлатежа.Организация, ПараметрыПлатежа.Контрагент, ПараметрыПлатежа.ДоговорКонтрагента, ТипОплаты);
			Если Не ЗначениеЗаполнено(СвойстваПлатежа.ВидОплаты) Тогда
			...
			КонецЕсли;
			СвойстваПлатежа.ВидОплатыРеквизиты = Справочники.ВидыОплатОрганизаций.ПараметрыВидОплаты(
				СвойстваПлатежа.ВидОплаты,
				ПараметрыПлатежа.Организация,
				ПараметрыПлатежа.Контрагент,
				ПараметрыПлатежа.ДоговорКонтрагента,
				ТипОплаты,
				ПараметрыПлатежа.Дата);
		КонецЕсли;
	Иначе
	...
	СтавкаНДС = СтавкаНДСРасшифровкиПлатежа(ПараметрыПлатежа.Дата, ПараметрыПлатежа.ВидОперации,
		...
		ПараметрыПлатежа.ПлательщикНДСПриУСН, ПараметрыПлатежа.СтавкаНДСПриУСН);
	Если СтавкаНДС <> Неопределено Тогда
		СвойстваПлатежа.СтавкаНДС = СтавкаНДС;
	КонецЕсли;
	
	// Порядок отражения аванса по умолчанию
	СвойстваПлатежа.ПорядокОтраженияАванса = ПараметрыПлатежа.ПорядокОтраженияАванса;
	Если Не ЗначениеЗаполнено(СвойстваПлатежа.ПорядокОтраженияАванса)
		И (ПараметрыПлатежа.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
		Или ПараметрыПлатежа.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам) Тогда
	...
	СвойстваПлатежа.СуммаВозврата = ПараметрыПлатежа.СуммаВозврата;
	...
	Если ПолучатьДоговор Тогда
		Если ПараметрыПлатежа.ВидОперации
			...
			ПересчитатьСуммуУслугЭквайринга(
				...
				ВидОплатыПредыдущий);
			Если ЗначениеЗаполнено(СвойстваПлатежа.ВидОплаты) Тогда
				СвойстваПлатежа.СчетУчетаРасчетовСКонтрагентом = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					СвойстваПлатежа.ВидОплаты, "СчетУчетаРасчетов");
			КонецЕсли;
		Иначе
...
Функция СтавкаНДСРасшифровкиПлатежа(Дата, ВидОперации, Договор, ПлательщикНДС, ПлательщикНДСПриУСН, СтавкаНДСПриУСН) Экспорт
	...
	Если ЗначениеЗаполнено(Договор) Тогда
		...
		Если ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя
			...
			Иначе
				Возврат УчетНДСКлиентСервер.СтавкаНДСПользователя(Дата, ПлательщикНДС);
			КонецЕсли;
	...
	Если Форма.СвойстваПлатежа = Неопределено Тогда
		...
		Иначе
			...
			Если Не ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
				...
				ПараметрыОбъекта.Вставить("СуммаВозврата",                 РасшифровкаПлатежа[0].СуммаВозврата);
			КонецЕсли;
			
			Форма.СвойстваПлатежа = ПоступлениеНаРасчетныйСчетФормыКлиентСервер.СвойстваСтрокРасшифровкиПлатежа(
				ПараметрыОбъекта, ПолучатьДоговор);
			Если Не ЗначениеЗаполнено(Форма.СвойстваПлатежа.ПорядокОтраженияАванса) Тогда
				УчетПСН.УстановитьПорядокОтраженияАванса(Форма.СвойстваПлатежа.ПорядокОтраженияАванса, ПараметрыОбъекта);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПересчитатьСуммыВзаиморасчетов(Форма) Экспорт
	...
	Иначе
		...
		ИначеЕсли Форма.ОплатаВВалюте Тогда
			ПлатежиКПересчету = Объект.РасшифровкаПлатежа;
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеЭлементамиВалютногоКонтроля(Форма)
	...
	Видимость = ИностранныйБанк
		Или ПолучитьФункциональнуюОпцию("УчетПоКодамВалютныхОперацийИНомерамВалютныхДоговоров");
...
Процедура УстановитьВидимостьСчетовУчета(Форма) Экспорт
	...
	Если НЕ ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаРасшифровкиПлатежа(Форма) Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета Тогда
			ЦифровойСчет = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетКонтрагента, "ЦифровойСчет");
			Элементы.ГруппаСтатьяДДС.Видимость =
				УчетнаяПолитика.ИспользоватьПереводыВПутиПриПеремещенияДенежныхСредств(Объект.Организация, Объект.Дата)
					Или ЗначениеЗаполнено(ЦифровойСчет) И ЦифровойСчет;
		ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
			Элементы.ГруппаСтатьяДДС.Видимость = Не ЗначениеЗаполнено(Объект.СчетКонтрагента)
				Или Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.СчетКонтрагента, "ВыпискаЗагружаетсяПоДепозитномуСчету");
	...
	Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		РасчетыПоКредитамИЗаймам = ЭтоРасчетыПоКредитамИЗаймам(Объект.ВидОперации);
		Если НазваниеТабличнойЧасти = "РасшифровкаПлатежа"
			Или НазваниеТабличнойЧасти = "РасшифровкаПлатежаПлатежныеКарты" Тогда
...
Функция СчетБанкаВиден(Форма) Экспорт
	...
	СвойстваСчета = Новый Структура("ВыпискаЗагружаетсяПоДепозитномуСчету, ЦифровойСчет", Ложь, Ложь);
	Если ЗначениеЗаполнено(Объект.СчетКонтрагента) Тогда
		СвойстваСчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Объект.СчетКонтрагента, "ВыпискаЗагружаетсяПоДепозитномуСчету, ЦифровойСчет");
	КонецЕсли;
	
	Если СвойстваСчета.ЦифровойСчет = Неопределено Тогда
		Возврат СчетаУчетаВДокументахВызовСервераПовтИсп.ПользовательУправляетСчетамиУчета();
	КонецЕсли;
	
	Если Не Используется57Счет И Не СвойстваСчета.ЦифровойСчет
		И (Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносНаличными
			Или Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПереводСДругогоСчета) Тогда
		Возврат Ложь;
	ИначеЕсли СвойстваСчета.ВыпискаЗагружаетсяПоДепозитномуСчету
		И Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.Депозит Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОбновитьУсловноеОформление(Форма) Экспорт
	...
	ИначеЕсли Не Форма.НастройкиУсловногоОформления.Свойство("РасшифровкаПлатежаПлатежныеКартыПроинициализировано")
		И Элементы.ГруппаПлатежныеКарты.Видимость
		И Элементы.Найти("РасшифровкаПлатежаПлатежныеКарты") <> Неопределено Тогда
		УстановитьУсловноеОформлениеРасшифровкаПлатежаПлатежныеКарты(Форма, НачалоПутиКДаннымТЧ);
...
Процедура УстановитьУсловноеОформлениеШапкаИВидимость(Форма, НачалоПутиКДаннымТЧ, ФормаДокументаОднострочная)
	...
	// РасшифровкаПлатежаПлатежныеКартыОтражениеАвансаПредставление
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыОтражениеАвансаПредставление");
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(
		ЭлементУО.Отбор,
		"ОтражениеАвансаВидимость",
		ВидСравненияКомпоновкиДанных.Равно,
...
Процедура УстановитьУсловноеОформлениеРасшифровкаПлатежаПлатежныеКарты(Форма, НачалоПутиКДаннымТЧ)
	
	УсловноеОформление           = Форма.УсловноеОформление;
	НастройкиУсловногоОформления = Форма.НастройкиУсловногоОформления;
	
	НастройкиУсловногоОформления.Вставить("РасшифровкаПлатежаПлатежныеКартыПроинициализировано", Истина);
	
	// РасшифровкаПлатежаПлатежныеКартыДоговорКонтрагента
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыДоговорКонтрагента");
	...
	// РасшифровкаПлатежаПлатежныеКартыСделка
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыСделка");
	...
	// РасшифровкаПлатежаПлатежныеКартыСпособПогашенияЗадолженности
	
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасшифровкаПлатежаПлатежныеКартыСпособПогашенияЗадолженности");
...
Процедура МобильныйКлиентАдаптацияФормыПриСоздании(Форма)
	...
	МобильныйКлиентБПФормаОбъекта.УстановитьИсключенияИзАдаптации(Форма, Исключения);
	
КонецПроцедуры

Процедура АдаптироватьЭлементыМКФормыДокумента(Форма) Экспорт
