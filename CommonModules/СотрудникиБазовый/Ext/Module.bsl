...
Функция ДанныеТрудовыхДоговоров(МассивОбъектов, ДистанционныеРаботы, ДоговорСНадомником)
	...
	Если ИспользоватьКадровыйУчет Тогда
		...
		Если ВыводПоСсылкеНаСотрудника Тогда
			...
			Запрос.Текст =
				...
				|	ПриказыОПриеме.ТрудовойДоговорНомер,
				|	ПриказыОПриеме.ТрудовойДоговорДата
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	NULL КАК ТрудовойДоговорНомер,
			|	NULL КАК ТрудовойДоговорДата
			|ПОМЕСТИТЬ ВТДанныеПриказовОПриеме
			|ИЗ
			|	ВТСотрудникиСДаннымиОПриеме КАК СотрудникиСДаннымиОПриеме";
	...
	Запрос.Текст =
		...
		|	ДанныеПриказовОПриеме.ТрудовойДоговорНомер КАК РаботаТрудовойДоговорНомерНаПечать,
		|	ДанныеПриказовОПриеме.ТрудовойДоговорДата КАК РаботаТрудовойДоговорДатаОформления,
		...
		|			ТОГДА 1
		|		ИНАЧЕ КадровыеДанныеСотрудников.КоличествоСтавок
		|	КОНЕЦ КАК КоличествоСтавок,
		|	НастройкиУчетаЗарплаты.ПродолжительностьОтпуска КАК ПродолжительностьОтпуска
		|ИЗ
		|	ВТСотрудникиСДаннымиОПриеме КАК СотрудникиСДаннымиОПриеме
		...
		|				ПО ДанныеОрганизаций.Организация = НастройкиУчетаЗарплаты.Организация
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.РаботаТрудовойДоговорНомерНаПечать) Тогда
			ДанныеДоговора.РаботаТрудовойДоговорНомерНаПечать = Выборка.РаботаТрудовойДоговорНомерНаПечать;
		Иначе
			ДанныеДоговора.РаботаТрудовойДоговорНомерНаПечать = "__________";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.РаботаТрудовойДоговорДатаОформления) Тогда
			ДанныеДоговора.РаботаТрудовойДоговорДатаОформления = Выборка.РаботаТрудовойДоговорДатаОформления;
		КонецЕсли;
		...
		Если Выборка.КоличествоСтавок < 1 Тогда
			ПродолжительностьРабочегоДня    = Окр(8 * Выборка.КоличествоСтавок, 2);
			ПродолжительностьРабочейНедели = ПродолжительностьРабочегоДня * 5;
			ДанныеДоговора.РаботаГрафикРаботыПродолжительностьРабочегоДня = ПродолжительностьРабочегоДня;
			ДанныеДоговора.РаботаГрафикРаботыОписаниеДлительностиРабочейНедели = СтрШаблон(НСтр("ru='%1 часов в неделю'"), ПродолжительностьРабочейНедели);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.ПродолжительностьОтпуска) Тогда
			ДанныеДоговора.РаботаПраваНаОтпускКоличествоДнейОбщегоОтпуска = Макс(28, Выборка.ПродолжительностьОтпуска);
		КонецЕсли;
...
Процедура ТрудовойДоговорВывестиРаздел(ИмяОбласти, ДокументРезультат, Макет, ДанныеДоговора, ПараметрыНумерации)
	...
	Для каждого ВложеннаяОбласть Из ОбластьРаздела.Области Цикл
		...
		Если ВложеннаяОбласть.Имя = "Подписи_05_СогласияНаОбработкуПерсональныхДанных"
			И (Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеДоговора, "РаботаТрудовойДоговорДатаОформления")
				Или Не ЗначениеЗаполнено(ДанныеДоговора.РаботаТрудовойДоговорДатаОформления)
				Или ДанныеДоговора.РаботаТрудовойДоговорДатаОформления >= '20250901') Тогда
...
Процедура ПечатьТ1ПоДокументам(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если СписокПриказов.Количество() > 0 Тогда
		ЭДОБЗК.ПечатьСтороннейПечатнойФормы(
...
Процедура ПечатьТ1ПоСправочнику(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	ДанныеСсылок.Колонки.Добавить("РаботаСведенияОбОплатеТрудаНадбавкаНаПечать");
...
Процедура ПечатьТ2(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) 
	...
	НазваниеПечатнойформы = НСтр("ru='Личная карточка (Т-2)'");
	ИмяФайлаПечатнойФормы = Неопределено;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ИнтеграцияС1САрхивомЗарплатаКадры") Тогда
		...
		МодульИнтеграцияС1САрхивом.ЗадатьИмяФайлаПечатнойФормы(ИмяФайлаПечатнойФормы, НазваниеПечатнойформы, МассивОбъектов);
	КонецЕсли;
	
	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_MXL_Т2", НазваниеПечатнойформы,
		ДокументРезультат,,,ИмяФайлаПечатнойФормы);
