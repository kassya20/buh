...
Процедура ФизическиеЛицаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	УстановитьВидимостьГруппыИсторияРаботы(Форма);
...
Функция ТаблицаПохожихФизическихЛиц(ПараметрыПоиска)
	...
	Запрос.Текст =
		...
		|	Справочник.ФизическиеЛица КАК ФизическиеЛица
		|ГДЕ
		|	ФизическиеЛица.Ссылка НЕ В(&ИсключаемыеФизическиеЛица)
		|	И &ФИО";
	...
	ИсключаемыеФизическиеЛица = ОбщегоНазначения.СкопироватьРекурсивно(ПараметрыПоиска.ИсключаемыеФизическиеЛица);
	ИсключаемыеФизическиеЛица.Добавить(ПараметрыПоиска.Ссылка);
...
Функция ИзменяемыеПоляФизическогоЛица() Экспорт
	...
	ИзменяемыеПоля.Вставить("ДатаВременнойРегистрации", "ИзмененаДатаВременнойРегистрации");
	ИзменяемыеПоля.Вставить("ДатаФактическогоПроживания", "ИзмененаДатаФактическогоПроживания");
...
Процедура УстановитьВидимостьГруппыИсторияРаботы(Форма)
	
	Если Не ПравоДоступа("Чтение", Метаданные.РегистрыСведений.КадроваяИсторияСотрудниковИнтервальный) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы, "ГруппаИстория", "Видимость", Ложь);
	КонецЕсли;
