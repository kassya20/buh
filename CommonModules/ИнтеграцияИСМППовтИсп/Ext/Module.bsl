...
Функция ОбъектМетаданныхДоступен(ПолноеИмяМетаданных) Экспорт
	...
	ИначеЕсли ОбъектМетаданных = Метаданные.Документы.ОтгрузкаТоваровИСМП Тогда
		...
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.ТоварыЛичнойГигиены);
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.БритвыИЛезвия);
		...
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.РастворимыеЗавариваемыеНапитки);
		...
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.АвтозапчастиИКомплектующиеТранспортныхСредств);
	...
	ИначеЕсли ОбъектМетаданных = Метаданные.Документы.УточнениеСведенийОКодахМаркировкиИСМП Тогда
		...
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.СпортивноеПитание);
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.МясоПодконтрольноеВЕТИС);
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей);
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.РадиоэлектроннаяПродукция);
		НедоступныеВидыПродукции.Добавить(Перечисления.ВидыПродукцииИС.ТелефоныИНоутбуки);
...
Функция ВидыПродукцииТоварныхГрупп() Экспорт
	
	ВозвращаемоеЗначение           = Новый Соответствие();
	ВидыПродукцииПоТоварнымГруппам = Новый Соответствие();
	
	Для Каждого ВидПродукции Из Перечисления.ВидыПродукцииИС Цикл
		...
		ВидыПродукцииПоТоварнойГруппе = ВидыПродукцииПоТоварнымГруппам[ИмяТоварнойГруппы];
		
		Если ВидыПродукцииПоТоварнойГруппе = Неопределено Тогда
			ВидыПродукцииПоТоварнойГруппе = Новый Массив();
			ВидыПродукцииПоТоварнымГруппам[ИмяТоварнойГруппы] = ВидыПродукцииПоТоварнойГруппе;
		КонецЕсли;
		
		ВидыПродукцииПоТоварнойГруппе.Добавить(ВидПродукции);
		
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ВидыПродукцииПоТоварнымГруппам Цикл
		
		Если КлючИЗначение.Значение.Количество() < 2 Тогда
			Продолжить;
		КонецЕсли;
		
		ОсновнойВидПродукции = Неопределено;
		
		Для Каждого ВидПродукции Из КлючИЗначение.Значение Цикл
			
			ВидыПродукцииТоварнойГруппыЗначение = ОбщегоНазначения.СкопироватьРекурсивно(КлючИЗначение.Значение);
			
			Для Каждого ВидПродукцииВложенный Из ВидыПродукцииТоварнойГруппыЗначение Цикл
				Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукцииВложенный)
					И ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияПодконтрольнаяВЕТИС(ВидПродукцииВложенный) Тогда
					ОсновнойВидПродукции = ВидПродукцииВложенный;
				КонецЕсли;
			КонецЦикла;
			
			Если ОсновнойВидПродукции = Неопределено Тогда
				Для Каждого ВидПродукцииВложенный Из ВидыПродукцииТоварнойГруппыЗначение Цикл
					Если ИнтеграцияИСМПКлиентСерверПовтИсп.ВестиУчетМаркируемойПродукции(ВидПродукцииВложенный) Тогда
						ОсновнойВидПродукции = ВидПродукцииВложенный;
						Прервать;
					КонецЕсли;
				КонецЦикла;
				Если ОсновнойВидПродукции = Неопределено Тогда
					ОсновнойВидПродукции = ВидыПродукцииТоварнойГруппыЗначение[0];
				КонецЕсли;
			КонецЕсли;
			
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ВидыПродукцииТоварнойГруппыЗначение, ВидПродукции);
			
			Значение = Новый Структура();
			Значение.Вставить("ВидыПродукции",        ВидыПродукцииТоварнойГруппыЗначение);
			Значение.Вставить("ИмяТоварнойГруппы",    КлючИЗначение.Ключ);
			Значение.Вставить("ОсновнойВидПродукции", ОсновнойВидПродукции);
			
			ВозвращаемоеЗначение.Вставить(ВидПродукции, Значение);
