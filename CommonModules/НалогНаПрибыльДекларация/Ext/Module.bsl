...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьСтатусыДекларацийПоОбособленнымПодразделениям", Истина);
	Методы.Вставить("ФормированиеДекларацийПоОбособленнымПодразделениям", Истина);
	Методы.Вставить("ИсправлениеДекларацийПоОбособленнымПодразделениям", Истина);
	Методы.Вставить("ОбновитьДекларацииПоОбособленнымПодразделениям", Истина);
...
// См. НалоговыйУчетОбособленныхПодразделений.ФормированиеДекларацийПоОбособленнымПодразделениям
Функция ФормированиеДекларацийПоОбособленнымПодразделениям(Параметры) Экспорт
	
	РезультатВыполнения = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ФормированиеДекларацийПоОбособленнымПодразделениям(РезультатВыполнения, Параметры);
	Возврат РезультатВыполнения;
	
КонецФункции

// См. НалоговыйУчетОбособленныхПодразделений.ИсправлениеДекларацийПоОбособленнымПодразделениям
Функция ИсправлениеДекларацийПоОбособленнымПодразделениям(Параметры) Экспорт
	
	РезультатВыполнения = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ИсправлениеДекларацийПоОбособленнымПодразделениям(РезультатВыполнения, Параметры);
	Возврат РезультатВыполнения;
	
КонецФункции

// См. НалоговыйУчетОбособленныхПодразделений.ОбновитьДекларацииПоОбособленнымПодразделениям
Функция ОбновитьДекларацииПоОбособленнымПодразделениям(Параметры) Экспорт
	
	РезультатВыполнения = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ОбновитьДекларацииПоОбособленнымПодразделениям(РезультатВыполнения, Параметры);
	Возврат РезультатВыполнения;
	
КонецФункции

// См. НалоговыйУчетОбособленныхПодразделений.ОбновитьСтатусыДекларацийПоОбособленнымПодразделениям
Функция ОбновитьСтатусыДекларацийПоОбособленнымПодразделениям(Параметры) Экспорт
	
	РезультатВыполнения = Неопределено;
	НалоговыйУчетОбособленныхПодразделений.ОбновитьСтатусыДекларацийПоОбособленнымПодразделениям(РезультатВыполнения, Параметры);
	Возврат РезультатВыполнения;
	
КонецФункции

// Рассчитывает показатели декларации 
//
// Параметры:
//  КонтекстРасчета - см. НовыйКонтекстРасчета() - содержит сведения о том, какая декларация заполняется: по какой организации, за какой период и т.п.
// 	Показатели      - Массив строк - выбранные показатели для расчета, см. СхемаРасчета
// Возвращаемое значение:
// Соответствие из КлючИЗначение:
// * Ключ - Строка - Показатель отчета
// * Значение - Число - Значение показателя
Функция РассчитатьПоказатели(КонтекстРасчета, Показатели) Экспорт
	...
	Для Каждого Показатель Из Показатели Цикл
		...
		ЭлементОтбора.Имя = Показатель;
	КонецЦикла;         
	
	РезультатРасчета = Рассчитать(СхемаРасчета, Отбор);
	Результат = Новый Соответствие;
	
	Для Каждого РассчитанноеЗначение Из РезультатРасчета.РассчитанныеЗначения Цикл
		Результат.Вставить(РассчитанноеЗначение.Имя, РассчитанноеЗначение.СуммаНУ);		
	КонецЦикла;
...
Функция СхемаРасчета(КонтекстРасчета, ОписаниеСумм)
	...
	ДобавитьРасчет(СхемаРасчета, "ФедеральныйИнвестиционныйВычет");
	ДобавитьСлагаемое(СхемаРасчета, "УменьшениеНалога");
	...
	ИсточникДанных.ОтборДанных.Показатель = "УменьшениеНалога";
	...
	Для Каждого ВидРасхода Из Перечисления.ВидыПрочихДоходовИРасходов.ВнереализационныеРасходы() Цикл
		ДобавитьСлагаемоеПрочиеРасходы(СхемаРасчета, ВидРасхода);
	КонецЦикла;
	...
	// Отдельные из перечисленных выше раскрываются отдельными строками
	...
	// Расходы.Внереализационные.Убытки - см. Перечисления.ВидыПрочихДоходовИРасходов.ВнереализационныеУбытки
	...
	ДобавитьОписание(СхемаРасчета,  "02", "2", "301", "Убытки прошлых налоговых периодов, выявленные в отчетном периоде");
	...
	ДобавитьОписание(СхемаРасчета,  "02", "2", "302", "Суммы безнадежных долгов");
	ДобавитьСлагаемоеПрочиеРасходы(СхемаРасчета, Перечисления.ВидыПрочихДоходовИРасходов.ПрочиеУбыткиПриравненныеКВнереализационнымРасходам);
	...
	ДобавитьКолонкиГодВозникновения(СписокУбыткиПрошлыхЛет, "ОстУбыт", ПредставлениеКолонкиГод, ПредставлениеКолонкиСумма);
	...
	ПараметрыСписка = НовыйПараметрыСпискаПоГодамВозникновения();
	
	ПараметрыСписка.НомерЛиста = "02";
	ПараметрыСписка.НомерПриложения = "4";
	ПараметрыСписка.ПредставлениеКолонкиГод = ПредставлениеКолонкиГод;
	...
	ДобавитьРасчетСписокПоГодамВозникновения(СхемаРасчета, ИмяСписка, ПараметрыСписка);
	...
	ДобавитьКолонкиГодВозникновения(СписокИнвестиционныйВычет, "СумНеиспВыч", ПредставлениеКолонкиГод, ПредставлениеКолонкиСумма);
	...
	ДобавитьРасчетСписокПоГодамВозникновения(СхемаРасчета, ИмяСпискаНачальноеСальдо, ПараметрыСписка);
	...
	ДобавитьКолонкиГодВозникновения(СписокИнвестиционныйВычет, "ОстНеиспВыч", ПредставлениеКолонкиГод, ПредставлениеКолонкиСумма);
	...
	ДобавитьРасчетСписокПоГодамВозникновения(СхемаРасчета, ИмяСпискаКонечноеСальдо, ПараметрыСписка);
...
Функция ОписаниеФормыОтчета(ИмяРедакции, СхемаРасчета)
	...
	// 4. ФНС РФ, приказ N ЕД-7-3/830@  от 02.10.2024                            - "ФормаОтчета2025Кв1"
	// 5. ФНС РФ, приказ N ЕД-7-3/830@  от 02.10.2024 в ред. от 03.10.2025       - "ФормаОтчета2025Кв4"
	//
...
Функция НовыйУсловиеПолученияДанныхИнформационнойБазы(ИмяАлгоритма)
	...
	ИначеЕсли ИмяАлгоритма = "УбыткиПрошлыхЛет.ПериодыОбразования" Тогда
		...
		Условие.Вставить("Счет",                ПланыСчетов.Хозрасчетный.ПустаяСсылка());
		Условие.Вставить("ВидСубконто",         ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПустаяСсылка());
		Условие.Вставить("ВидСтатьиРБП"); // не несет прикладного смысла; передается для оптимизации
...
Функция НастройкиОтчетаРасшифровкаЗначения(ЗаписьПротокола, СхемаРасчета)
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = "УбыткиПрошлыхЛет.ПериодыОбразования" Тогда
		...
		Настройки = НастройкиОтчетаУбыткиПрошлыхЛет(
			ЗаписьПротокола.Условие.ПериодВозникновения,
			ЗаписьПротокола.Условие.Счет,
			ЗаписьПротокола.Условие.ВидСубконто,
			ЗаписьПротокола.Условие.ВидСтатьиРБП,
			СхемаРасчета);
	...
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = "ФедеральныйИнвестиционныйВычет" Тогда
		Настройки = НастройкиОтчетаФедеральныйИнвестиционныйВычетРасходы(ЗаписьПротокола.Условие, СхемаРасчета);
	ИначеЕсли ЗаписьПротокола.ТипЗаписи = "ФедеральныйИнвестиционныйВычет.НачальноеСальдо.ПериодыОбразования"
		...
		Настройки = НастройкиОтчетаФедеральныйИнвестиционныйВычетСальдо(ЗаписьПротокола.Условие.ПериодВозникновения, СхемаРасчета);
	КонецЕсли;
...
Функция СтатьяПрочихДоходовРасходовПрименима(ВидСтатьи, Показатель, ЭтапРасчета)
	...
	Если ЭтапРасчета = "РаспределенныеНаСпециальныеРежимы" Тогда
		...
		Если Показатель = "ПрочиеДоходы" Или Перечисления.ВидыПрочихДоходовИРасходов.ИспользуетсяДляРеализацииАмортизируемогоИмущества(ВидСтатьи) Тогда
...
Функция ХарактеристикиУбыткиОтРеализацииОсновныхСредств()
	
	Параметры = Новый Структура;
	// Счета учета
	Параметры.Вставить("СчетРасходов", ПланыСчетов.Хозрасчетный.ПрочиеРасходы);
	Параметры.Вставить("СчетБудущихУбытков", ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС);
	Параметры.Вставить("СчетТекущихУбытков", ПланыСчетов.Хозрасчетный.СальдоПрочихДоходовИРасходов);
	// Аналитика на этих счетах
	Параметры.Вставить("ВидыСтатей", Новый ТаблицаЗначений);
	Параметры.ВидыСтатей.Колонки.Добавить("ВидСтатьи", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПрочихДоходовИРасходов"));
	// Описание расчета
	Параметры.Вставить(
		"ШаблонОписания",
		НСтр("ru = 'Оборот по дебету счета [СчетРасходов] по статьям вида ""[ВидСтатьи]"" в корреспонденции со счетами [СчетБудущихУбытков], [СчетТекущихУбытков][ЭтапРасчета]'"));
	Параметры.Вставить("ПараметрыОписания", Новый Структура);
	Параметры.ПараметрыОписания.Вставить("СчетРасходов", Параметры.СчетРасходов);
	Параметры.ПараметрыОписания.Вставить("СчетБудущихУбытков", Параметры.СчетБудущихУбытков);
	Параметры.ПараметрыОписания.Вставить("СчетТекущихУбытков", Параметры.СчетТекущихУбытков);
	Параметры.ПараметрыОписания.Вставить("ВидСтатьи"); // должен быть установлен при подготовке текста описания
	Параметры.ПараметрыОписания.Вставить("ЭтапРасчета"); // должен быть установлен при подготовке текста описания
...
Функция РезультатЗапросаУбыткиОтРеализацииАмортизируемыхОсновныхСредств(ХарактеристикиРасходов, ПроцессорПолученияДанных, СчетУбытка)
	...
	Запрос.УстановитьПараметр("СчетУбытка", СчетУбытка);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ХозрасчетныйОбороты.Субконто1 КАК СтатьяДоходов,
	|	ЕСТЬNULL(СУММА(ХозрасчетныйОбороты.СуммаНУОборотДт), 0) КАК СуммаНУ,
	|	ХозрасчетныйОбороты.Субконто2 КАК РеализованныйОбъект
	|ПОМЕСТИТЬ ВТ_Обороты
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В (&СчетаРасходов), &ВидыСубконтоСчетаРасходов, Организация В (&Организации), КорСчет = &СчетУбытка) КАК ХозрасчетныйОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ХозрасчетныйОбороты.Субконто1,
	|	ХозрасчетныйОбороты.Субконто2";
	
	РезультатЗапроса = ВыполнитьЗапрос(Запрос, ПроцессорПолученияДанных.ВременныеТаблицы); // ВТ_Обороты
	...
	|	СтатьиПрочихДоходовРасходов.ВидДеятельности КАК ВидДеятельности,
	|	СтатьиПрочихДоходовРасходов.Учитываются КАК Учитываются,
	|	Обороты.РеализованныйОбъект КАК РеализованныйОбъект,
	|	ЕСТЬNULL(СУММА(Обороты.СуммаНУ), 0) КАК СуммаНУ
	|ИЗ
	|	ВТ_Обороты КАК Обороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СтатьиПрочихДоходовРасходов КАК СтатьиПрочихДоходовРасходов
	|		ПО Обороты.СтатьяДоходов = СтатьиПрочихДоходовРасходов.Ссылка
...
Процедура ПолучитьРеализацияАмортизируемогоИмущества(ПроцессорПолученияДанных)
	...
	КонецЦикла;
	ВидыСтатей = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ВидыСтатей);
	...
	// 3а. Расходы после реализации
	// Расходы могут отражаться по счету 91 трижды:
	...
	// - третий раз - в периоде получения убытков, в корреспонденции со счетом 91.09 (после реализации в текущем периоде).
	...
	// 3б. Убытки от реализации основных средств по счету 97

	ШагРасчета = ШагиРасчета.Добавить();
	ШагРасчета.Имя = "РасходыПослеРеализации";
	ШагРасчета.ПоДаннымИБ = Истина;
	
	СчетУбытка = ПланыСчетов.Хозрасчетный.УбыткиОтРеализацииОС;
	ХарактеристикиРасходов = ХарактеристикиУбыткиОтРеализацииОсновныхСредств();
	НоваяСтрока = ХарактеристикиРасходов.ВидыСтатей.Добавить();
	НоваяСтрока.ВидСтатьи = Перечисления.ВидыПрочихДоходовИРасходов.РеализацияОсновныхСредств;
	
	РезультатЗапроса = РезультатЗапросаУбыткиОтРеализацииАмортизируемыхОсновныхСредств(
		ХарактеристикиРасходов,
		ПроцессорПолученияДанных,
		ХарактеристикиРасходов.СчетБудущихУбытков);
	...
	ШагРасчета.Условие.Вставить("СчетДоходовРасходов", ХарактеристикиРасходов.СчетРасходов);
	ШагРасчета.Условие.Вставить("СчетУбытков", ХарактеристикиРасходов.СчетБудущихУбытков);
	
	// 3в. Убытки от реализации основных средств по счету 91.09
	// Если срок полезного использования ОС при продажи был нулевой, то все убытки
	// были учтены в расходах общей суммой в текущем периоде (Дт 91.02 - Кт 91.09).
	...
	РезультатЗапроса = РезультатЗапросаУбыткиОтРеализацииАмортизируемыхОсновныхСредств(
		ХарактеристикиРасходов,
		ПроцессорПолученияДанных,
		ХарактеристикиРасходов.СчетТекущихУбытков);
	...
	ШагРасчета.Условие.Вставить("СчетДоходовРасходов", ХарактеристикиРасходов.СчетРасходов);
	ШагРасчета.Условие.Вставить("СчетУбытков", ХарактеристикиРасходов.СчетТекущихУбытков);
	...
	Для Каждого Задача Из ПроцессорПолученияДанных.Задачи Цикл
		СтрокиШагиРасчета = ШагиРасчета.НайтиСтроки(Новый Структура("Имя", Задача.ОтборДанных.Показатель));
		Для Каждого ШагРасчета Из СтрокиШагиРасчета Цикл
...
Процедура ПолучитьФедеральныйИнвестиционныйВычетРеквизиты(ПроцессорПолученияДанных, Алгоритм)
	...
	Запрос.УстановитьПараметр("СубконтоДокументыВычета", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыИнвестиционногоВычета);
	Запрос.УстановитьПараметр("ВидОперацииПолучение", Перечисления.ВидыОперацийСИнвестиционнымВычетом.Получение);
	
	// Приказ ФНС от 02.10.2024 N ЕД-7-3/830@ (ред. от 3 октября 2025 г. N ЕД-7-3/855@) - п. 184, 185:
	// - код "1", если есть расходы налогоплательщика; ИНН не указывается
	// - код "2", если все расходы получены из группы; указывается ИНН одного из лиц, фактически осуществивших расходы ... по выбору налогоплательщика

	ПериодНачалаПримененияПриказа855Собака = '2025-12-31'; // "начиная с представления налоговой декларации по налогу на прибыль организаций за налоговый период 2025 года"
	
	// Приведенные выше правила установились не сразу: ранее следовало указать код "2", если одновременно есть
	// и расходы налогоплательщика, и расходы, полученные из группы.
	//
	...
	Запрос.УстановитьПараметр(
		"ПрименяетсяВременнаяИнформацияФНС",
		ПроцессорПолученияДанных.КонтекстРасчета.КонецПериода < ПериодНачалаПримененияПриказа855Собака);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ХозрасчетныйОстаткиИОбороты.Субконто1 КАК ДокументОперацияСВычетом
	|ПОМЕСТИТЬ ДанныеПоОперациям
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, , , Счет = &СчетВычета, &СубконтоДокументыВычета, Организация В (&Организации)) КАК ХозрасчетныйОстаткиИОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОперацияСИнвестиционнымВычетом КАК ОперацияСИнвестиционнымВычетом
	|		ПО ДанныеПоОперациям.ДокументОперацияСВычетом = ОперацияСИнвестиционнымВычетом.Ссылка
	|			И (ОперацияСИнвестиционнымВычетом.ВидОперации = &ВидОперацииПолучение)
	|ГДЕ
	|	ОперацияСИнвестиционнымВычетом.Ссылка ЕСТЬ NULL
	|	И НЕ &ПрименяетсяВременнаяИнформацияФНС
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	1,
	|	2,
	|	СОКРЛП(ОперацияСИнвестиционнымВычетом.Контрагент.ИНН)
	|ИЗ
	|	ДанныеПоОперациям КАК ДанныеПоОперациям
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОперацияСИнвестиционнымВычетом КАК ОперацияСИнвестиционнымВычетом
	|		ПО ДанныеПоОперациям.ДокументОперацияСВычетом = ОперацияСИнвестиционнымВычетом.Ссылка
	|ГДЕ
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Задача.СуммаНУ = Реквизиты[Показатель];
...
Процедура ПолучитьФедеральныйИнвестиционныйВычетСальдо(ПроцессорПолученияДанных, Алгоритм)
	...
	// Если нет данных в регистре ДанныеПервичныхДокументов, тогда заполняем предыдущим годом: это наиболее вероятный случай.
	Запрос.УстановитьПараметр("ПериодВозникновенияПоУмолчанию", НачалоГода(ПроцессорПолученияДанных.КонтекстРасчета.НачалоПериода - 1));

	Запрос.Текст =
	"ВЫБРАТЬ
	|	ХозрасчетныйОстатки.Субконто1 КАК ДокументВычета,
	|	ХозрасчетныйОстатки.СуммаНУОстатокДт КАК СуммаНУ
	|ПОМЕСТИТЬ ОстаткиПоДокументам
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ПериодОстатков, Счет = &СчетВычета, &СубконтоДокументыВычета, Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументВычета
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, &ПериодВозникновенияПоУмолчанию), ГОД)) КАК ПериодВозникновения,
	|	ОстаткиПоДокументам.ДокументВычета КАК ДокументВычета
	|ПОМЕСТИТЬ ДанныеПервичныхДокументов
	|ИЗ
	|	ОстаткиПоДокументам КАК ОстаткиПоДокументам
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОстаткиПоДокументам.ДокументВычета = ДанныеПервичныхДокументов.Документ
	|
	|СГРУППИРОВАТЬ ПО
	|	ОстаткиПоДокументам.ДокументВычета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеПервичныхДокументов.ПериодВозникновения КАК ПериодВозникновения,
	|	СУММА(ОстаткиПоДокументам.СуммаНУ) КАК СуммаНУ
	|ИЗ
	|	ОстаткиПоДокументам КАК ОстаткиПоДокументам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	|		ПО ОстаткиПоДокументам.ДокументВычета = ДанныеПервичныхДокументов.ДокументВычета
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеПервичныхДокументов.ПериодВозникновения
	|
	|УПОРЯДОЧИТЬ ПО
	|	ПериодВозникновения";
...
Процедура ПолучитьФедеральныйИнвестиционныйВычет(ПроцессорПолученияДанных)
	...
	Запрос.УстановитьПараметр("СубконтоВидыРасходов", ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРасходовНаАмортизируемоеИмущество);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ХозрасчетныйОбороты.Субконто1 КАК ВидРасходов,
	|	ХозрасчетныйОбороты.СуммаНУОборотДт КАК СуммаВычетаРасходы,
	|	ХозрасчетныйОбороты.СуммаНУОборотКт КАК СуммаВычетаУменьшениеНалога
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет = &СчетИнвестиционногоВычета, &СубконтоВидыРасходов, Организация = &Организация, , ) КАК ХозрасчетныйОбороты";
	...
	Для Каждого Задача Из МассивЗадач Цикл
		...
		Показатель = Задача.ОтборДанных.Показатель;
		Если Показатель = "УменьшениеНалога" Тогда
			
			Задача.СуммаНУ = ТаблицаВычетов.Итог("СуммаВычетаУменьшениеНалога");
		...
		ВидРасходов = Задача.ОтборДанных.ВидРасходов;

		СтрокаВычетаПоВидуРасходов = ТаблицаВычетов.Найти(ВидРасходов, "ВидРасходов");
		
		СуммаВычетаРасходы = 0;
		Если СтрокаВычетаПоВидуРасходов <> Неопределено Тогда
			СуммаВычетаРасходы = СтрокаВычетаПоВидуРасходов.СуммаВычетаРасходы;
		КонецЕсли;
		
		Задача.СуммаНУ = СуммаВычетаРасходы;
		...
		ЗаписьПротокола.Условие.ВидРасходов = ВидРасходов;

	КонецЦикла;
	
КонецПроцедуры

Функция НастройкиОтчетаФедеральныйИнвестиционныйВычетРасходы(Условие, СхемаРасчета)
	...
	// Если расходы осуществил филиал, то в оборотно-сальдовой ведомости отобразятся задвоенные суммы вычета:
	// по головной организации и филиалу. Чтобы в отчете отобразилась сумма, указанная в ячейке декларации, делаем
	// не отбор, а группировку по организации, чтобы была возможность расшифровать сумму до первичного документа (по филиалу).
	Если СхемаРасчета.КонтекстРасчета.Организации.Количество() > 0 Тогда
	...
	Если ЗначениеЗаполнено(Условие.ВидРасходов) Тогда
		
		ОписаниеПоляВидРасходов = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
			"Субконто",
			Счет,
			ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ВидыРасходовНаАмортизируемоеИмущество);

		Если ОписаниеПоляВидРасходов <> Неопределено Тогда

			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
				Настройки, ОписаниеПоляВидРасходов.Имя, Условие.ВидРасходов);
...
Функция НастройкиОтчетаФедеральныйИнвестиционныйВычетСальдо(Период, СхемаРасчета)
	...
	ОписаниеПоляДокументВычета = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
		"Субконто",
		Счет,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыИнвестиционногоВычета);
		
	// Отбор по Документ.Дата
	Если ОписаниеПоляДокументВычета <> Неопределено Тогда
		
		ИмяПоляДата = СтрШаблон("%1.Дата", ОписаниеПоляДокументВычета.Имя);

		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
			Настройки, ИмяПоляДата, НачалоГода(Период), ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
		
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
			Настройки, ИмяПоляДата, КонецГода(Период), ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);

	КонецЕсли;
	
	// Группировка Объект
	ОписаниеПоляОбъектВычета = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОписаниеСубконтоСчета(
		"Субконто",
		Счет,
		ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.АмортизируемоеИмущество);

	Если ОписаниеПоляОбъектВычета <> Неопределено Тогда
		
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьГруппировкуВНастройкиОтчета(
			Настройки,
			ОписаниеПоляОбъектВычета.Имя,
			ОписаниеПоляОбъектВычета.Представление);
	...
	Если ОписаниеПоляДокументВычета <> Неопределено Тогда
		...
		ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьГруппировкуВНастройкиОтчета(
			Настройки,
			ОписаниеПоляДокументВычета.Имя,
			ОписаниеПоляДокументВычета.Представление);
...
Функция СчетаУчетаАмортизации()
	...
	СчетаУчета.Вставить("НематериальныеАктивы", ПланыСчетов.Хозрасчетный.АмортизацияНематериальныхАктивов_);
	Возврат СчетаУчета;
...
Процедура ПолучитьУбыткиПрошлыхЛет(ПроцессорПолученияДанных)
	...
	Запрос.УстановитьПараметр("НачалоПериодаУбытков",  НачалоПериодаУбытков);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	НАЧАЛОПЕРИОДА(УбыткиПрошлыхЛет.ГодУбытка, ГОД)
	|ИЗ
	|	Справочник.УбыткиПрошлыхЛет КАК УбыткиПрошлыхЛет
	|ГДЕ
	|	УбыткиПрошлыхЛет.ГодУбытка >= &НачалоПериодаУбытков
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ШаблоныОписаний.Вставить(
			...
			НСтр(
				"ru = 'Годы, в которых возник убыток, начиная с %1.
		         ...
		         |Анализируется начальное сальдо по дебету счета %2 по статьям убытка за %1 и более поздние годы'",
				ОбщегоНазначения.КодОсновногоЯзыка()));
				
		ШаблоныОписаний.Вставить(
			"Сумма",
			НСтр(
				"ru = 'Убытки, возникшие с %1 года: начальное сальдо по дебету счета %2 по статьям убытка за %1 и более поздние годы'",
				ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Для Каждого Задача Из ПроцессорПолученияДанных.Задачи Цикл
			...
			Для Каждого Счет Из ПорядокУчета.СчетаУбыткиПрошлыхЛет Цикл
				ЗаписьПротокола = ДобавитьЗаписьПротоколаПолученияДанныхИнформационнойБазы(ПроцессорПолученияДанных, Задача);
				ЗаписьПротокола.Описание = СтрШаблон(
					ШаблоныОписаний[Задача.ОтборДанных.Показатель],
					ПредставлениеНомераГода(НачалоПериодаУбытков),
					ПорядокУчета.ПредставлениеСчетовУбытков[Счет]);
				ЗаписьПротокола.Условие.Счет = Счет;
				ЗаписьПротокола.Условие.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.УбыткиПрошлыхЛет;
			КонецЦикла;
	...
	ШаблонДетальногоОписанияСтатьиУбытка = НСтр(
		"ru = 'Начальное сальдо по дебету счета %2 по статьям убытка за %1 год'",
		ОбщегоНазначения.КодОсновногоЯзыка());
		
	ШаблонДетальногоОписанияСтатьиРБП = НСтр(
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		СтрокаСписка = ПроцессорПолученияДанных.ДанныеСписка.Добавить();
		СтрокаСписка.ПериодВозникновения = ВыборкаПериод.ПериодВозникновения;
		СтрокаСписка.НомерГодаСтрокой    = ПредставлениеНомераГода(СтрокаСписка.ПериодВозникновения);
		СтрокаСписка.Сумма               = КоллекцииСумм.НовыйКоллекцияСумм(ПроцессорПолученияДанных.ОписаниеСумм, Истина);
		КоллекцииСумм.Заполнить(СтрокаСписка.Сумма, ВыборкаПериод, ПроцессорПолученияДанных.ОписаниеСумм);
		...
		// Расшифровку в терминах статей убытка (субконто УбыткиПрошлыхЛет) выводим в разрезе субсчетов,
		// чтобы корректно сформировать стандартные отчетв по субконто, не заданному для счета-группы в целом.
		// Расшифровку в терминах статей РБП выводим при наличии данных, так как это устаревший подход.
		СуммыПоСчетам = Новый Соответствие;
		ВыборкаСчет = ВыборкаПериод.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаСчет.Следующий() Цикл
			СуммыПоСчетам.Вставить(ВыборкаСчет.Счет, ВыборкаСчет.СуммаНУ);
		КонецЦикла;
			
		Для Каждого Счет Из ПорядокУчета.СчетаУбыткиПрошлыхЛет Цикл
			
			// протоколирование списка предполагает привязку к строке списка, сумму уточним ниже
			ЗаписьПротокола = ДобавитьЗаписьПротоколаПолученияДанныхИнформационнойБазы(
				ПроцессорПолученияДанных,
				"Сумма",
				СтрокаСписка);
			ЗаписьПротокола.СуммаНУ = СуммыПоСчетам[Счет];
			ЗаписьПротокола.Описание = СтрШаблон(
				ШаблонДетальногоОписанияСтатьиУбытка,
				ПредставлениеНомераГода(ВыборкаПериод.ПериодВозникновения),
				ПорядокУчета.ПредставлениеСчетовУбытков[Счет]);
			ЗаписьПротокола.Условие.ПериодВозникновения = ВыборкаПериод.ПериодВозникновения;
			ЗаписьПротокола.Условие.Счет        = Счет;
			ЗаписьПротокола.Условие.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.УбыткиПрошлыхЛет;
			
		КонецЦикла;
		
		// В терминах статей РБП выводим при наличии, так как это устаревший вариант
		СуммаПоСтатьямРБП = СуммыПоСчетам[ПланыСчетов.Хозрасчетный.ПустаяСсылка()];
		Если ЗначениеЗаполнено(СуммаПоСтатьямРБП) Тогда
			ЗаписьПротокола = ДобавитьЗаписьПротоколаПолученияДанныхИнформационнойБазы(
				ПроцессорПолученияДанных,
				"Сумма",
				СтрокаСписка);
			ЗаписьПротокола.СуммаНУ = СуммаПоСтатьямРБП;
			ЗаписьПротокола.Описание = СтрШаблон(
				ШаблонДетальногоОписанияСтатьиРБП,
				ПредставлениеНомераГода(ВыборкаПериод.ПериодВозникновения),
				ПорядокУчета.ПредставлениеСчетаУбытков,
				ПорядокУчета.ПредставлениеВидаСтатьи);
			ЗаписьПротокола.Условие.ПериодВозникновения = ВыборкаПериод.ПериодВозникновения;
			ЗаписьПротокола.Условие.Счет        = ПорядокУчета.СчетУчетаУбытков;
			ЗаписьПротокола.Условие.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.РасходыБудущихПериодов;
			ЗаписьПротокола.Условие.ВидСтатьиРБП = ПорядокУчета.ВидСтатьиРБП;
		КонецЕсли;
...
Функция НастройкиОтчетаУбыткиПрошлыхЛет(ПериодВозникновенияУбытка, Счет, ВидСубконто, ВидСтатьиРБП, СхемаРасчета)
	...
	Если ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.УбыткиПрошлыхЛет Тогда
		
		Если ЗначениеЗаполнено(ПериодВозникновенияУбытка) Тогда
			ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьОтборВНастройкиОтчета(
				Настройки,
				ОписаниеПоляСтатьяУбытка.Имя + "." + "ГодУбытка",
				ПериодВозникновенияУбытка);
		КонецЕсли;
		
	ИначеЕсли ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.РасходыБудущихПериодов Тогда
...
Процедура ПолучитьПереносУбытковПрошлыхЛет(ПроцессорПолученияДанных)
	...
	|				И СубконтоКт1 В
	|					(ВЫБРАТЬ
	|						СтатьиУбытковРБП.Ссылка
	|					ИЗ
	|						СтатьиУбытковРБП КАК СтатьиУбытковРБП)) КАК УменьшениеПрибыли
	|
	|СГРУППИРОВАТЬ ПО
	...
	|ВЫБРАТЬ
	|	УменьшениеПрибыли.СубконтоКт1,
	|	СУММА(УменьшениеПрибыли.СуммаНУОборотДт)
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ОборотыДтКт(&НачалоПериода, &КонецПериода, , СчетДт = &СчетУчетаПрибыли, , СчетКт В (&СчетаУчетаУбытков), ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.УбыткиПрошлыхЛет), Организация В (&Организации)) КАК УменьшениеПрибыли
	|
	|СГРУППИРОВАТЬ ПО
	|	УменьшениеПрибыли.СубконтоКт1
	|;
	...
	ШаблонОписания = НСтр(
		"ru = 'Оборот по дебету счета %1 в корреспонденции со счетом %2 по статьям вида ""%3"" и счетом %4'",
		ОбщегоНазначения.КодОсновногоЯзыка());
			
	ТекстОписания = СтрШаблон(
		ШаблонОписания,
		ПорядокУчета.ПредставлениеСчетаПрибыли,
		ПорядокУчета.ПредставлениеСчетовУбытков.Получить(ПланыСчетов.Хозрасчетный.ПрочиеРасходыБудущихПериодов),
		ПорядокУчета.ПредставлениеВидаСтатьи,
		ПорядокУчета.ПредставлениеСчетовУбытков.Получить(ПланыСчетов.Хозрасчетный.УбыткиПрошлыхЛет));
...
Функция НастройкиОтчетаПереносУбытковПрошлыхЛет(СхемаРасчета)
	...
	Отбор = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ОтборНастроекОтчета(Настройки);
	ГруппаИЛИ = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьГруппуВОтборВНастройкиОтчета(
		Отбор,
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИЛИ);
	
	ГруппаИ = ЗаполнениеРасшифровкаРегламентированнойОтчетности.ДобавитьГруппуВОтборВНастройкиОтчета(
		ГруппаИЛИ,
...
Функция ПрочитатьИдентификатор(ХранилищеСумм, ОписаниеСумм)
	
	// См. также ЗаписатьИдентификатор
	
	Если Не ЗначениеЗаполнено(ХранилищеСумм) Тогда
		Возврат "";
	КонецЕсли;
	...
	Идентификатор = ХранилищеСумм[ИменаСвойств[0]];
...
Процедура ДобавитьКолонкиГодВозникновения(Список, КодФНС, ПредставлениеКолонкиГод, ПредставлениеКолонкиСумма)
...
Процедура ДобавитьРасчетСписокПоГодамВозникновения(СхемаРасчета, ИмяСписка, ПараметрыСписка)
	...
	ШаблонСуммаУбытка = "%1.Сумма.%2-";
	ШаблонНомерГода   = "%1.НомерГода.%2";
	СуффиксСлужебногоНомераСтрокГода = "01";
	
	Для ИндексПериода = 0 По ПараметрыСписка.КоличествоСтрок - 1 Цикл
		
		ИдентификаторПериода = XMLСтрока(ИндексПериода);
		ИмяСуммаУбытка = СтрШаблон(ШаблонСуммаУбытка, ИмяСписка, ИдентификаторПериода);
		ИмяНомерГода   = СтрШаблон(ШаблонНомерГода,   ИмяСписка, ИдентификаторПериода);
		
		ЧисловойНомерСтроки = ПараметрыСписка.НомерПервойСтроки + ИндексПериода * ПараметрыСписка.ШагКодовСтрок;
		...
		ДобавитьОписание(
			СхемаРасчета, ПараметрыСписка.НомерЛиста, ПараметрыСписка.НомерПриложения, "", ПараметрыСписка.ПредставлениеКолонкиГод, "Идентификатор", СлужебныйНомерСтроки);
...
Функция НовыйПараметрыСпискаПоГодамВозникновения()
	...
	ПараметрыСписка.Вставить("ПредставлениеКолонкиГод", "");
...
Процедура ПередатьРассчитанныеЗначенияБРО(Контейнер, РезультатРасчета, ОписаниеФормыОтчета, ИмяРедакции, СхемаРасчета)
	...
	Для Каждого Показатель Из ОписаниеФормыОтчета Цикл
		...
		Если Показатель.ОсобенностьПредставления = "Идентификатор" Тогда
			...
			Значение = ПрочитатьИдентификатор(Значение, СхемаРасчета.ОписаниеСумм);
		Иначе
