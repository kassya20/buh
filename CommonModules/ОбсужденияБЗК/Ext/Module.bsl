...
// Возвращает негрупповое обсуждение между двумя пользователями.
// Если обсуждение уже существует, тогда возвращает существующее обсуждение.
// Если обсуждение еще не существует, тогда создает новое и возвращает его.
// Параметры:
...
//
Функция НеГрупповоеОбсуждениеМеждуПользователями(Автор, Участник) Экспорт
...
// Создает новое личное обсуждение в системе взаимодействия.
//
// Параметры:
//  Ключ - Строка - Уникальный ключ обсуждения
//  Заголовок - Строка - Заголовок обсуждения
//  Участник - ИдентификаторПользователяСистемыВзаимодействия
//
// Возвращаемое значение:
//  УникальныйИдентификатор - Идентификатор созданного обсуждения
//
Функция ИдентификаторНовогоЛичногоОбсуждения(Ключ, Заголовок, Участник) Экспорт
...
// Возвращает идентификатор внешнего пользователя системы взаимодействия.
//
// Параметры:
//  ИмяПолучателя - Строка - имя получателя.
//  АдресЭлектроннойПочты - Строка - содержащая адрес электронной почты.
//
// Возвращаемое значение:
//  УникальныйИдентификатор - Идентификатор внешнего пользователя системы взаимодействия.
//
Функция ВнешнийПользовательСистемыВзаимодействия(ИмяПолучателя, АдресЭлектроннойПочты) Экспорт
	
	Отбор = Новый ОтборПользователейСистемыВзаимодействия;
	Отбор.АдресЭлектроннойПочтыДляАутентификации = АдресЭлектроннойПочты;
	
	ВнешниеПользователиСВ = СистемаВзаимодействия.ПолучитьПользователей(Отбор);
	
	Если ВнешниеПользователиСВ.Количество() <> 0 Тогда
		Идентификатор = ВнешниеПользователиСВ[0].Идентификатор;
	Иначе
		ПользовательСВ = СистемаВзаимодействия.СоздатьПользователя();
		ПользовательСВ.Внешний = Истина;
		ПользовательСВ.Имя = ИмяПолучателя;
		ПользовательСВ.ПолноеИмя = ИмяПолучателя;
		ПользовательСВ.АдресЭлектроннойПочтыДляАутентификации = АдресЭлектроннойПочты;
...
Функция Ассистент(Знач ИмяДляВхода, Знач Картинка, Служебный = Истина) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПользовательАссистент) Тогда
		...
		Попытка
			...
			ПользовательАссистент.Служебный = Служебный;
...
Процедура ОбновитьУведомленияОНеобходимостиПодписанияФайловДокументовЭДОБЗК() Экспорт
	...
	Если ЗначениеЗаполнено(Исполнители) Тогда
		...
		РегистрыСведений.ЗапланированныеДействияСФайламиДокументовЭДОБЗК.ОбновитьУведомленияОНеобходимостиПодписанияФайловДокументовЭДОБЗК(Исполнители);
