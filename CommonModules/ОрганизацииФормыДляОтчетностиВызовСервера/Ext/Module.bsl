...
Функция ЗапроситьДанныеЕГР(Знач Запрос) Экспорт
	...
	Если РезультатПроверки.ЭтоЮрЛицо Тогда
		...
		Иначе
			...
			Если ОшибкаПодключения1СКонтрагент(СведенияОбОрганизации.ОписаниеОшибки) Тогда
				Ответ.Ошибка.Код = СведенияОбОрганизации.ОписаниеОшибки;
			КонецЕсли;
	...
	Если ЗначениеЗаполнено(РеквизитыОрганизации.ОписаниеОшибки) Тогда
		...
		Если ОшибкаПодключения1СКонтрагент(РеквизитыОрганизации.ОписаниеОшибки) Тогда
...
Функция НаименованиеПоКлассификатору(Знач Классификатор, Знач КодПоКлассификатору) Экспорт
	...
	Если Классификатор = "ОКВЭД2" Тогда
		Возврат КлассификаторОКВЭДБП.НаименованиеПоКоду(КодПоКлассификатору);
	ИначеЕсли Классификатор = "ОКОПФ" Тогда
		Возврат РегистрыСведений.КлассификаторОКОПФ.НаименованиеПоКоду(КодПоКлассификатору);
	ИначеЕсли Классификатор = "ОКФС" Тогда
		Возврат РегистрыСведений.КлассификаторОКФС.НаименованиеПоКоду(КодПоКлассификатору);
	КонецЕсли;
...
Функция ОшибкаПодключения1СКонтрагент(КодОтвета)
	
	ВозможныеКодыОтветов = Новый Массив;
	ВозможныеКодыОтветов.Добавить("НеУказаныПараметрыАутентификации");
	ВозможныеКодыОтветов.Добавить("НеУказанПароль");
	ВозможныеКодыОтветов.Добавить("Сервис1СКонтрагентНеПодключен");
	
	Возврат ВозможныеКодыОтветов.Найти(КодОтвета) <> Неопределено;
