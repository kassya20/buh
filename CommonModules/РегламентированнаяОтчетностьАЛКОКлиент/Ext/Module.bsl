...
// Возвращает Истина, если текущий сеанс выполняется на сервере, работающим под управлением ОС Linux Х64.
//
// Возвращаемое значение:
//  Булево - Истина, если сервер работает под управлением ОС Linux Х64.
//
Функция ЭтоLinuxКлиентХ64() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Возврат СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Linux_x86_64
...
//  Булево - Истина, если сервер работает под управлением ОС Linux Х64.
//
Функция ЭтоWindowsКлиентХ64() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Возврат СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86_64
	
КонецФункции

// Возвращает размер оперативной памяти  в мегабайтах компьютера, на котором выполняется клиентская часть кода.
//
// Возвращаемое значение:
//  Число
//
Функция ОперативнаяПамятьКлиента() Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Возврат СистемнаяИнформация.ОперативнаяПамять
...
Процедура ВыполнитьКомандуФормыПоИмениАЛКО(Форма, ИмяКоманды) Экспорт
	...
	ИначеЕсли ИмяКоманды = "Просуммировать" Тогда
		...
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
	ИначеЕсли ИмяКоманды = "МасштабФормы" Тогда
		ВыбратьУстановитьМасштабАЛКО(Форма);
		ВыполнитьСледующееДействиеИзОчередиАЛКО(Форма);
...
Процедура ОбработкаОповещенияАЛКО(Форма, ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если Источник = Форма.УникальностьФормы Тогда
		...
		ИначеЕсли НРег(ИмяСобытия) = НРег("ПриАктивизацииСтрокиНаКлиенте") Тогда
			ПриАктивизацииСтрокиАЛКО(Форма);
		ИначеЕсли НРег(ИмяСобытия) = НРег("ОткрытиеСтраницы") Тогда
			ВыполнитьСМинимальнойЗадержкой(Форма, "ОткрытиеСтраницыНаКлиенте");
		ИначеЕсли НРег(ИмяСобытия) = НРег("ОтменаОперации") Тогда
		...
		ИначеЕсли НРег(ИмяСобытия) = НРег("ОтменаОперацииПриОшибке") Тогда
			ОтменаДлительнойОперацииАЛКО(Форма);
		ИначеЕсли НРег(ИмяСобытия) = НРег("ПересчетИтогов") Тогда
		...
		ИначеЕсли НРег(ИмяСобытия) = НРег("ПоказыватьПредупреждениеПослеПереходаПоСсылке") Тогда
			Форма.ПоказыватьПредупреждениеПослеПереходаПоСсылке = Ложь;
		ИначеЕсли НРег(ИмяСобытия) = НРег("ОткрытаФормаЗаписиРегистра") Тогда
			Форма.ФормаЗаписиРегистра = Параметр;
			Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = Параметр;
		ИначеЕсли НРег(ИмяСобытия) = НРег("ЗакрытаФормаЗаписиРегистра") Тогда
			...
			Если (Форма.ТекущееСостояние = "Копирование")
				...
				ТаблицаФормыРазделаПриИзмененииАЛКО(Форма, Элемент);
				Форма.ТекущееСостояние = "";
				
			КонецЕсли;

		ИначеЕсли НРег(ИмяСобытия) = НРег("ОткрытьФормуНавигацииПоОшибкам") Тогда
		...
		ИначеЕсли НРег(ИмяСобытия) = НРег("ИзменениеМасштаба") Тогда
			УстановитьМасштабАЛКО(Параметр.Масштаб, Параметр.ИмяТаблицы, Форма);			
		КонецЕсли;
...
Процедура РазделыОтчетаПриАктивизацииСтрокиАЛКО(Форма, Элемент) Экспорт
	...
	ЭтоРазделЛицензии = (
							СтрНайти(Нрег(Форма.ТекущееИмяРаздела), "лиценз") > 0
							ИЛИ
							СтрНайти(Нрег(Форма.ТекущееИмяРаздела), "адрес") > 0
						);
	
	ЭтоТитульный = (СтрНайти(Нрег(Форма.ТекущееИмяРаздела), "титульный") > 0);
	
	ДекорацияВспомогательная = Форма.Элементы.Найти("ДекорацияВспомогательная");
	Если ДекорацияВспомогательная <> Неопределено Тогда
		
		Если НЕ (ЭтоРазделЛицензии ИЛИ ЭтоТитульный) Тогда
			ДекорацияВспомогательная.Заголовок = Форма.ТекущееИмяРаздела;
		КонецЕсли;
	...
	// Заполним Заголовок Свернутого Отображения для строки навигации.
	ГруппаТаблицаНавигации = Форма.Элементы["ГруппаТаблицаНавигации"];
	Форма["ЗаголовокГруппыНавигации"] = "Раздел / " + Форма.ТекущееИмяРаздела 
		+ ?(Форма.ЭтоРазделОтчета, "", 
			" / Стр. " + РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(НомерСтраницы + 1)
			);
	ГруппаТаблицаНавигации.Заголовок = Форма["ЗаголовокГруппыНавигации"];
	ГруппаТаблицаНавигации.ЗаголовокСвернутогоОтображения = Форма["ЗаголовокГруппыНавигации"];
	ГруппаТаблицаНавигации.Скрыть();
...
Процедура ТаблицаФормыРазделаПриИзмененииАЛКО(Форма, Элемент) Экспорт
	...
	ТаблицаФормыРазделаЛицензииПриИзмененииАЛКО(Форма, Элемент);
...
Функция ТекущееОбособленноеПодразделение(Форма) Экспорт
	...
	Возврат РегламентированнаяОтчетностьАЛКОКлиентСервер.ТекущееОбособленноеПодразделение(Форма);
...
Функция ОбработкаДействияСАлкоОтчетом(ОписаниеОтчета, Действие,
	...
	Алко25.ФормаОтчета2025Кв1.Вставить("Утверждена", "Приказом ФСКАТР от 26.11.2024 г. № 461");
	...
	Алко40.ФормаОтчета2025Кв1.Вставить("Утверждена", "Приказом ФСКАТР от 26.11.2024 г. № 461");
...
Процедура АктивизироватьПолеАктивнойСтраницыАЛКО(Форма) Экспорт
			...
			Если Форма.ВходВРедактированиеПриАктивизации Тогда
				...
				Если ТекущийЭлемент.Вид = ВидПоляФормы.ПолеНадписи Тогда
					...
					Если ЗначениеЗаполнено(ИмяПроцедурыНажатие) Тогда
						...
						ВыполнитьОбработкуОповещения(Оповещение, ТекущийЭлемент);
						
					Иначе
						
						ИмяПроцедурыОбработкаСсылки = Форма.ПолучитьДействиеПоляФормы(
													Форма.ИмяЯчейкиАктивизации, "ОбработкаНавигационнойСсылки");
						
						Если ЗначениеЗаполнено(ИмяПроцедурыОбработкаСсылки) Тогда
							
							ИмяПроцедурыОбработкаСсылки = ИмяПроцедурыОбработкаСсылки + "ДляОповещения";
							Оповещение = Новый ОписаниеОповещения(ИмяПроцедурыОбработкаСсылки, Форма.ЭтотОбъект);
							ВыполнитьОбработкуОповещения(Оповещение, ТекущийЭлемент);
				...
				ИначеЕсли ТекущийЭлемент.Вид = ВидПоляФормы.ПолеВвода Тогда
					...
					Если ТекущийЭлемент.КнопкаВыбора <> Неопределено Тогда
						...
						Если ТекущийЭлемент.КнопкаВыбора Тогда
							...
							ИмяПроцедурыНачалоВыбора = Форма.ПолучитьДействиеПоляФормы(
														Форма.ИмяЯчейкиАктивизации, "НачалоВыбора");
                            
							Если ЗначениеЗаполнено(ИмяПроцедурыНачалоВыбора) Тогда
								
								ИмяПроцедурыНачалоВыбора = ИмяПроцедурыНачалоВыбора + "ДляОповещения";
	...
	Иначе
		...
		ПереходНаСтрокуТаблицыФормыАЛКО(Форма, ИдГруппы, Форма.НомерСтрокиАктивизации, 
														Форма.ИмяКолонкиАктивизации);
...
Процедура АктивизироватьЯчейкуПослеЗакрытияОткрытыхФормАЛКО(Форма) Экспорт
	...
	ЭлементДереваРаздел = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(
		КоллекцияЭлементовДереваРазделыОтчета, "КолонкаРазделыОтчетаСокрНаим", КолонкаРазделыОтчетаСокрНаим);
	...
	// Многостраничные разделы.		
		...
		ЭлементСтраницыРаздела = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(
							ЭлементДереваРаздел.ПолучитьЭлементы(), "КолонкаНомерСтраницыРазделаОтчета", 
							КолонкаНомерСтраницыРазделаОтчета);
		...
		// Отчет уже открыт на нужной странице,
			...
			ЭлементДереваРазделов = РегламентированнаяОтчетностьКлиентСервер.НайтиЭлементВДанныхФормыДерево(
							КоллекцияЭлементовДереваРазделыОтчета, "КолонкаРазделыОтчетаСокрНаим", "Титульный");

			// Чтобы не отработал ПриАктивизацииСтроки() Таблицы разделов 
			// при активизации строчки Титульного листа, 
			// заранее присваем нужное значение в СтруктураРеквизитовФормы.ТекущаяСтрокаРазделовОтчета.
			Форма.СтруктураРеквизитовФормы.ТекущаяСтрокаРазделовОтчета = 
											ЭлементДереваРазделов.ПолучитьИдентификатор();
			Форма.Элементы.РазделыОтчета.ТекущаяСтрока = 
											Форма.СтруктураРеквизитовФормы.ТекущаяСтрокаРазделовОтчета;
...
Процедура АктивизироватьПолеФормыРегистраСведений(
					Форма, АктивноеПоле, ИмяАктивногоПоля, ТекущееСостояниеВладельца) Экспорт
	...
	Если НЕ АктивноеПоле = Неопределено Тогда
		...
		Если АктивноеПоле.Вид = ВидПоляФормы.ПолеНадписи Тогда
			...
			ИмяПроцедурыНажатие = Форма.ПолучитьДействиеПоляФормы(ИмяАктивногоПоля, "Нажатие");
			
			Если ЗначениеЗаполнено(ИмяПроцедурыНажатие) Тогда
				
				Оповещение = Новый ОписаниеОповещения(ИмяПроцедурыНажатие, Форма.ЭтотОбъект);
				ВыполнитьОбработкуОповещения(Оповещение, АктивноеПоле);
				
			Иначе
				
				ИмяПроцедурыОбработкаСсылки = Форма.ПолучитьДействиеПоляФормы(
												ИмяАктивногоПоля, "ОбработкаНавигационнойСсылки");
				
				Если ЗначениеЗаполнено(ИмяПроцедурыОбработкаСсылки) Тогда
					
					ИмяПроцедурыОбработкаСсылки = ИмяПроцедурыОбработкаСсылки + "ИзОповещения";
					Оповещение = Новый ОписаниеОповещения(ИмяПроцедурыОбработкаСсылки, Форма.ЭтотОбъект);
					ВыполнитьОбработкуОповещения(Оповещение, АктивноеПоле);
		...
		ИначеЕсли АктивноеПоле.Вид = ВидПоляФормы.ПолеВвода Тогда
			...
			Если АктивноеПоле.КнопкаВыбора <> Неопределено Тогда
				...
				Если АктивноеПоле.КнопкаВыбора Тогда
					...
					ИмяПроцедурыНачалоВыбора = Форма.ПолучитьДействиеПоляФормы(
														ИмяАктивногоПоля, "НачалоВыбора");
                    
					Если ЗначениеЗаполнено(ИмяПроцедурыНачалоВыбора) Тогда
						
						ИмяПроцедурыНачалоВыбора = ИмяПроцедурыНачалоВыбора + "ИзОповещения";
						Оповещение = Новый ОписаниеОповещения(ИмяПроцедурыНачалоВыбора, Форма.ЭтотОбъект);
						ВыполнитьОбработкуОповещения(Оповещение, АктивноеПоле);
...
Процедура ВызватьФормуВыбораЗначенийАЛКО(Форма, ФлажокОтклАвтоВыборКодов,
	...
	Если НЕ ФлажокОтклАвтоВыборКодов Тогда
		...
		Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = ОткрытьФорму(
			"ОбщаяФорма.АЛКОФормаВыбораЗначенияИзТаблицы", 
...
Процедура ВыполнитьСЗадержкойНеИзОтчета(Форма, ИмяПроцедуры, Таймаут = 0.1) Экспорт
	...
	Задержка = Таймаут;
	#Если ВебКлиент Тогда
		Задержка = ?(Таймаут < 0.2, 0.2, Таймаут);
	#Иначе
		Задержка = ?(Таймаут < 0.1, 0.1, Таймаут);
	#КонецЕсли
	
    Если ЗначениеЗаполнено(ИмяПроцедуры)
			и (ТипЗнч(ИмяПроцедуры) = Тип("Строка")) Тогда
		Форма.ПодключитьОбработчикОжидания(ИмяПроцедуры, Задержка, Истина);
	КонецЕсли;
...
Функция НайтиОтчетВладелецПодчиненнойФормы(ПодчиненнаяФорма) Экспорт

	ТекущийРодитель = ПодчиненнаяФорма.ВладелецФормы;
	...
	ИмяФормыВладельца 	= ТекущийРодитель.ИмяФормы;
	
	СчетчикЗащитыОтЗацикливания = 0;
	
	Пока 
		(
			СтрНайти(ИмяФормыВладельца, "РегламентированныйОтчетАлко") = 0
			ИЛИ
			СтрНайти(ИмяФормыВладельца, "ФормаОтчета") = 0
		)
		И СчетчикЗащитыОтЗацикливания < 11
		Цикл
	
		ТекущийРодитель = ТекущийРодитель.ВладелецФормы;
		...
		СчетчикЗащитыОтЗацикливания = СчетчикЗащитыОтЗацикливания + 1;
			
	КонецЦикла;
	
	Возврат ?(СчетчикЗащитыОтЗацикливания < 11, ТекущийРодитель, Неопределено);

КонецФункции

// Меняет внешний вид подчиненной управляемой формы отчета в соответствии с настройками
// отчета из подсистемы ОтчетностьПоАлкогольнойПродукции.
//
// Параметры
//		ПодчиненнаяФорма	- подчиненная управлемая форма отчета, 
//							владельцем которой прямо или косвенно является отчет.
//		ОтчетВладелец		- в параметре возвращается управляемая форма отчета владельца.
//
Процедура ИнициализироватьUIПодчиненнойФормыОтчетаАЛКО(ПодчиненнаяФорма, ОтчетВладелец = Неопределено, НеМенятьМасштаб = Ложь) Экспорт
	
	ОтчетВладелец = НайтиОтчетВладелецПодчиненнойФормы(ПодчиненнаяФорма);
	
	НастройкаUI = Неопределено;
	
	Если ОтчетВладелец <> Неопределено Тогда
		
		ОтчетВладелец.СтруктураРеквизитовФормы.Свойство("НастройкаUI", НастройкаUI);
		
		Если НЕ НеМенятьМасштаб Тогда
		
			ПодчиненнаяФорма.Масштаб = ОтчетВладелец.Масштаб;
		
		КонецЕсли;
		
	КонецЕсли;
	
	ПодчиненнаяФорма.ИнициализироватьUIФормы(НастройкаUI);
	
КонецПроцедуры

Процедура ВыбратьУстановитьМасштабАЛКО(Форма, УникальностьФормыМасштаб = Неопределено) Экспорт
	
	Уникальность = ?(УникальностьФормыМасштаб = Неопределено, Форма.УникальностьФормы, УникальностьФормыМасштаб);
	
	ЭлементТаблицаФормы = Форма.ТекущийЭлемент;
	Пока 	ТипЗнч(ЭлементТаблицаФормы) <> Тип("ТаблицаФормы") // Кнопка меню таблицы формы.
		    И
			ЭлементТаблицаФормы.Имя <> "ГруппаКнопки" // Кнопка меню формы.
		Цикл
	
		ЭлементТаблицаФормы = ЭлементТаблицаФормы.Родитель;
	...
	КнопкаМасштаб = Форма.Элементы.Масштаб;
	ВиднаКнопкаМасштабаВМенюОтчета = КнопкаМасштаб.Видимость;
	
	Если 	ТипЗнч(ЭлементТаблицаФормы) = Тип("ТаблицаФормы")
			И
			(НЕ ВиднаКнопкаМасштабаВМенюОтчета)
		Тогда
		
		МасштабТаблицы = 0;
		Форма["МасштабТаблиц"].Свойство(ЭлементТаблицаФормы.Имя, МасштабТаблицы);
		МасштабТаблицы = ?(МасштабТаблицы = Неопределено, 100, МасштабТаблицы);
		
		
		ПараметрыФормы.Вставить("Масштаб", МасштабТаблицы);
		ПараметрыФормы.Вставить("ИмяТаблицы", ЭлементТаблицаФормы.Имя);
		
	ИначеЕсли ВиднаКнопкаМасштабаВМенюОтчета Тогда
		
		Масштаб = Форма["МасштабФормы"];
		Масштаб = ?(Масштаб = Неопределено, 100, Масштаб);
		ПараметрыФормы.Вставить("Масштаб", Масштаб);
	...
	ПараметрыФормы.Вставить("УникальностьФормы", Уникальность);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеВыбораМасштабаАЛКО", ЭтотОбъект, ДопПараметры);
	
	Форма.Активизировать();
	Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = ОткрытьФорму(
		"ОбщаяФорма.АЛКОВыборМасштаба", 
		ПараметрыФормы, Форма, Форма.КлючУникальности, , , ОписаниеОповещения, 
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ЗавершениеВыбораМасштабаАЛКО(РезультатВыбора, ДопПараметры) Экспорт
	
	Форма = ДопПараметры.Форма;
	
	ИмяТаблицы = "";
	ДопПараметры.Свойство("ИмяТаблицы", ИмяТаблицы);
	
	Форма.ОткрытаяФормаПотомокСБлокировкойВладельца = Неопределено;
	
	УстановитьМасштабАЛКО(РезультатВыбора, ИмяТаблицы, Форма);

КонецПроцедуры

Процедура УстановитьМасштабАЛКО(Масштаб, ИмяТаблицы, Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
		...
		Форма["МасштабТаблиц"].Вставить(ИмяТаблицы, Масштаб);
		РегламентированнаяОтчетностьАЛКОВызовСервера.СохранитьМасштаб(Форма.ИмяФормы, Форма["МасштабТаблиц"]);
		СписокМасштабируемыхИменЭлементов = ИмяТаблицы;
		
	Иначе
		
		Форма["МасштабФормы"] = Масштаб;
		СписокМасштабируемыхИменЭлементов = Форма["СписокИменТаблиц"];
		РегламентированнаяОтчетностьАЛКОВызовСервера.СохранитьМасштаб(Форма.ИмяФормы, Масштаб);
		
	КонецЕсли;
	
	Форма.МасштабироватьОтчетАЛКОНаКлиенте(СписокМасштабируемыхИменЭлементов);
...
Процедура ДобавитьСтраницыПоОПвДекларации(Форма, 
					НаименованиеОП, ПредставлениеАдреса, АдресXML, КПП, 
	...
	СтруктураНовогоОП = Новый Соответствие;
	СтруктураНовогоОП["НаименованиеОП"] 		= НаименованиеОП;
	СтруктураНовогоОП["ПредставлениеАдреса"] 	= ПредставлениеАдреса;
	СтруктураНовогоОП["АдресXML"] 				= АдресXML;
	СтруктураНовогоОП["КПП"] 					= КПП;
	СтруктураНовогоОП["ВидРозничнойПродажи"] 	= ВидРозничнойПродажи;
	СтруктураНовогоОП["ОтсутствуетИнтернет"] 	= ОтсутствуетИнтернет;
	
	Форма.ОбщийПараметрПроцедурНаКлиенте.Вставить("СтруктураНовогоОП", СтруктураНовогоОП); 
	
	ВыполнитьСМинимальнойЗадержкой(Форма, "ДобавитьСтраницыПоОПвДекларации");

КонецПроцедуры

Процедура ТаблицаФормыРазделаЛицензииПриИзмененииАЛКО(Форма, Элемент)
	...
	Если Раздел = "Лицензии" Тогда
		...
		КПП = СокрЛП(ТекущиеДанные.П000000000201);
		АдресXML = СокрЛП(ТекущиеДанные.П0000000VNUTR);
		ПредставлениеАдреса = СокрЛП(ТекущиеДанные.П0000000Адрес);
		
		ВидРозничнойПродажи = 0;
		ОтсутствуетИнтернет = Ложь;
		
		Если 
			ЗначениеЗаполнено(ПредставлениеАдреса)
			И
			(
				(ЗначениеЗаполнено(КПП) И СтрДлина(КПП) = 9)
				ИЛИ (НЕ ЗначениеЗаполнено(КПП) И Форма.ЭтоПБОЮЛ)
			)
			
			Тогда
			
			НуженРеквизитОтсутствуетИнтернет = 
				РегламентированнаяОтчетностьАЛКОКлиентСервер.НуженРеквизитОтсутствуетИнтернет(Форма);
			Если НуженРеквизитОтсутствуетИнтернет Тогда
			
				ВидРозничнойПродажи = ТекущиеДанные.П000000000209;
				ОтсутствуетИнтернет = ТекущиеДанные.П000000000208;
				
			КонецЕсли;
			
			КоличествоОП = 0;
			Если НЕ РегламентированнаяОтчетностьАЛКОКлиентСервер.ЕстьОПвДекларации(
						Форма, КоличествоОП, ПредставлениеАдреса, КПП, 
						ВидРозничнойПродажи, ОтсутствуетИнтернет) Тогда
			
				// В разделе Лицензии введено ОП, которого нет в Декларации.
				// Нужно добавить.
				НомерОП = КоличествоОП + 1;
				НаименованиеОП = "Обособленное подразделение №" 
								+ РегламентированнаяОтчетностьАЛКОКлиентСервер.СтрокаЧГ0(НомерОП);
								
				ДобавитьСтраницыПоОПвДекларации(
					Форма, 
					НаименованиеОП, ПредставлениеАдреса, АдресXML, КПП, 
					ВидРозничнойПродажи, ОтсутствуетИнтернет);
...
Процедура ПослеВыполненияДлительнойОперацииАЛКО(Форма,
	...
	Форма.ВладелецФормыОшибок = РегламентированнаяОтчетностьАЛКОКлиентСервер.СкопироватьРекурсивноАЛКО(Форма.ВладелецФормыДлитОпер);
	Форма.ВладелецФормыДлитОпер = Неопределено;
...
Процедура ВыборДеятельностиПоЛицензииЗавершение(РезультатВыбора,
	...
	// Если выбран код больше 7 - т.е нелицензируемая деятельность
		...
		Иначе
			...
			Форма.Модифицированность = Истина;
			Форма.ЭтоЛицензируемаяДеятельность = КодТребуетЛицензию;
			...
			Форма.Элементы.РазделыОтчетаКонтекстноеМенюУдалитьСтраницу.Видимость = (КоличествоСтраницЛицензий > 1)
				и (Родитель <> Неопределено);
			Форма.Элементы.РазделыОтчетаУдалитьСтраницу.Доступность = 
				Форма.Элементы.РазделыОтчетаКонтекстноеМенюУдалитьСтраницу.Видимость;
			...
			Если (Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20131101')
				...
				// т.к. Лицензия может быть только одна.
				Форма.Элементы.РазделыОтчетаКонтекстноеМенюДобавитьСтраницу.Видимость = Ложь;
		        Форма.Элементы.РазделыОтчетаДобавитьСтраницу.Доступность = Ложь;
				
			Иначе
				
				Форма.Элементы.РазделыОтчетаКонтекстноеМенюДобавитьСтраницу.Видимость = Истина;
		        Форма.Элементы.РазделыОтчетаДобавитьСтраницу.Доступность = Истина;
...
Процедура ВопросВыборДеятельностиПоЛицензииЗавершение(РезультатВопроса,
	...
	Форма.ЭтоЛицензируемаяДеятельность = Ложь;
...
Процедура ПечатьПослеСохраненияАЛКО(Отказ, Форма) Экспорт
	...
	ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Отчет не сохранен, продолжение %1 невозможно!'"), ТекстПечати);
	...
	Если ВидПечати = "Экспорт" Тогда
		...
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1. Экспорт...'"), Форма.Заголовок), , , 
			КартинкаСостояния);
			
	Иначе
		
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1. Формируется печатная форма...'"), Форма.Заголовок), , , 
...
Процедура ОткрытьФормуНавигацииПоОшибкамБезПараметровАЛКО(Форма) Экспорт
	...
	ФормаНавигацииПоОшибкам = ОткрытьФорму("ОбщаяФорма.АЛКОФормаСообщенийОбОшибках", 
		ПараметрыФормы, , КлючУникальности, , , , РежимОткрытияОкнаФормы.Независимый);
