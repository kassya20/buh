...
// Возвращает или статусы документа о предстоящей поставке, или обновляет статус ДОПП, или рассчитывает по массиву
// квитанций о присвоении статуса ДОПП.
// Параметры:
//   ТекущийСтатус - Строка или Неопределено,
//   НовыйСтатус - Строка или Неопределено,
//   КраткийВариант - Булево,
//   МассивКвитанцийОПрисвоенииСтатусаДОПП - Массив, возвращаемый ДокументыРеализацииПолномочийНО(СсылкаНаУведомление),
//     при заданном параметре рассчитывает актуальный статус и возвращает его.
//
// Возвращаемое значение:
//   Строка - наиболее актуальный статус из переданных в параметрах "ТекущийСтатус", "НовыйСтатус", если хотя бы один
//     из них задан,
//   Структура - если параметры "ТекущийСтатус", "НовыйСтатус", не заданы:
//     * Зарегистрировано - Строка - "1",
//     * Обеспечено - Строка - "2",
//     * ПройденаПроверкаКО - Строка - "3",
//     * НеПройденаПроверкаКО - Строка - "4",
//     * НаПроверкеКНП - Строка - "5",
//     * Погашено - Строка - "6".
//
Функция СтатусыДОПП(
		...
		КраткийВариант = Ложь,
		МассивКвитанцийОПрисвоенииСтатусаДОПП = Неопределено) Экспорт
	
	Если МассивКвитанцийОПрисвоенииСтатусаДОПП <> Неопределено Тогда
		Результат = "";
		КоличествоДокументовСтатусовДОПП = Новый Соответствие;
		МаксимальноеКоличествоДокументовСтатусовДОПП = 0;
		Для каждого СведенияКвитанции Из МассивКвитанцийОПрисвоенииСтатусаДОПП Цикл
			Если ЗначениеЗаполнено(СведенияКвитанции.СтатусДОПП) Тогда
				КоличествоДокументовСтатусовДОПП[СведенияКвитанции.СтатусДОПП] =
					?(ЗначениеЗаполнено(КоличествоДокументовСтатусовДОПП[СведенияКвитанции.СтатусДОПП]),
					КоличествоДокументовСтатусовДОПП[СведенияКвитанции.СтатусДОПП] + 1, 1);
				Если КоличествоДокументовСтатусовДОПП[СведенияКвитанции.СтатусДОПП] >
					МаксимальноеКоличествоДокументовСтатусовДОПП Тогда
					МаксимальноеКоличествоДокументовСтатусовДОПП =
						КоличествоДокументовСтатусовДОПП[СведенияКвитанции.СтатусДОПП];
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Для каждого СведенияКоличестваДокументовСтатусовДОПП Из КоличествоДокументовСтатусовДОПП Цикл
			Если ЗначениеЗаполнено(СведенияКоличестваДокументовСтатусовДОПП.Значение)
				И СведенияКоличестваДокументовСтатусовДОПП.Значение >= МаксимальноеКоличествоДокументовСтатусовДОПП Тогда
				Результат = СтатусыДОПП(Результат, СведенияКоличестваДокументовСтатусовДОПП.Ключ);
			КонецЕсли;
		КонецЦикла;
		
		Возврат Результат;
		
	ИначеЕсли ТекущийСтатус = Неопределено И НовыйСтатус = Неопределено Тогда
		Результат = Новый Структура;
		Результат.Вставить("Зарегистрировано", "1");
		Результат.Вставить("Обеспечено", "2");
		Результат.Вставить("ПройденаПроверкаКО", "3");
		Результат.Вставить("НеПройденаПроверкаКО", "4");
		Результат.Вставить("НаПроверкеКНП", "5");
		Результат.Вставить("Погашено", "6");
		
		Возврат Результат;
		
	ИначеЕсли НовыйСтатус = Неопределено Тогда
		Статусы = СтатусыДОПП();
		
		Если ТекущийСтатус = Статусы.Зарегистрировано Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'Зарегистрирован'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано'");
			КонецЕсли;
		ИначеЕсли ТекущийСтатус = Статусы.Обеспечено Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'Обеспечен'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано, обеспечено'");
			КонецЕсли;
		ИначеЕсли ТекущийСтатус = Статусы.ПройденаПроверкаКО Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'Пройдена проверка госоргана'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано, пройдена проверка госоргана'");
			КонецЕсли;
		ИначеЕсли ТекущийСтатус = Статусы.НеПройденаПроверкаКО Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'Не пройдена проверка госоргана'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано, не пройдена проверка госоргана'");
			КонецЕсли;
		ИначеЕсли ТекущийСтатус = Статусы.НаПроверкеКНП Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'На проверке КНП'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано, на проверке КНП'");
			КонецЕсли;
		ИначеЕсли ТекущийСтатус = Статусы.Погашено Тогда
			Если КраткийВариант Тогда
				Возврат НСтр("ru = 'Погашен'");
			Иначе
				Возврат НСтр("ru = 'Зарегистрировано, погашено'");
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(ТекущийСтатус) Тогда
			Если КраткийВариант Тогда
				Возврат СтрШаблон(
					"%1",
					СокрЛП(ТекущийСтатус));
			Иначе
				Возврат СтрШаблон(
					НСтр("ru = 'Зарегистрировано, код состояния: ""%1""'"),
					СокрЛП(ТекущийСтатус));
			КонецЕсли;
		Иначе
			Возврат "";
	...
	Иначе
		...
		Если (НовыйСтатус = Статусы.Зарегистрировано
				И НЕ ЗначениеЗаполнено(ТекущийСтатус))
			ИЛИ (НовыйСтатус = Статусы.Обеспечено
				И (НЕ ЗначениеЗаполнено(ТекущийСтатус) ИЛИ ТекущийСтатус = Статусы.Зарегистрировано))
			ИЛИ (НовыйСтатус = Статусы.ПройденаПроверкаКО
				И (НЕ ЗначениеЗаполнено(ТекущийСтатус) ИЛИ ТекущийСтатус = Статусы.Зарегистрировано
				ИЛИ ТекущийСтатус = Статусы.Обеспечено ИЛИ ТекущийСтатус = Статусы.НеПройденаПроверкаКО))
			ИЛИ (НовыйСтатус = Статусы.НеПройденаПроверкаКО
				И (НЕ ЗначениеЗаполнено(ТекущийСтатус) ИЛИ ТекущийСтатус = Статусы.Зарегистрировано
				ИЛИ ТекущийСтатус = Статусы.Обеспечено))
			ИЛИ (НовыйСтатус = Статусы.НаПроверкеКНП
				И (ЗначениеЗаполнено(ТекущийСтатус) ИЛИ ТекущийСтатус = Статусы.Зарегистрировано
				ИЛИ ТекущийСтатус = Статусы.Обеспечено ИЛИ ТекущийСтатус = Статусы.ПройденаПроверкаКО
				ИЛИ ТекущийСтатус = Статусы.НеПройденаПроверкаКО))
			ИЛИ (НовыйСтатус = Статусы.Погашено
				И (НЕ ЗначениеЗаполнено(ТекущийСтатус) ИЛИ ТекущийСтатус = Статусы.Зарегистрировано
				ИЛИ ТекущийСтатус = Статусы.Обеспечено ИЛИ ТекущийСтатус = Статусы.ПройденаПроверкаКО
				ИЛИ ТекущийСтатус = Статусы.НеПройденаПроверкаКО ИЛИ ТекущийСтатус = Статусы.НаПроверкеКНП)) Тогда
...
// Предназначена для получения квитанции о присвоении статуса документа о предстоящей поставке.
// Параметры:
//   СсылкаНаОтчетИлиДанныеКвитанции - Ссылка из определяемого типа ПредметЦиклаОбмена - например, ссылка, возвращенная
//                                     ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервераПереопределяемый.ПриИзмененииСтатусаОтправкиДокумента
//                                   - ДвоичныеДанные - данные квитанции ДОПП с КНД 1160303
//   СтатусОтправки                  - Перечисления.СтатусыОтправки - например, статус, возвращаемый
//                                     ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервераПереопределяемый.ПриИзмененииСтатусаОтправкиДокумента
//                                   - Неопределено - не используется при предаче двоичных данных квитанции.
//
// Возвращаемое значение:
//   Структура:
//     * ЭтоКвитанцияОПрисвоенииСтатусаДОПП - Булево - при значении Истина заполняются остальные поля,
//     * НомерУведомления - Строка - номер уведомления,
//     * ДатаУведомления - Дата - дата уведомления,
//     * ДанныеКвитанции - ДволичныеДанные - данные квитанции о присвоении статуса ДОПП с КНД 1167011,
//     * Квитанция - Структура - содержимое квитанции о присвоении статуса ДОПП с КНД 1167011:
//       ** ИдентификаторФайла - Строка,
//       ** ВерсияПрограммы - Строка,
//       ** ВерсияФормат - Строка,
//       ** КНД - Строка,
//       ** ПолучательЭтоОрганизация - Булево,
//       ** ПолучательНаименованиеОрганизации - Строка,
//       ** ПолучательИНН - Строка,
//       ** ПолучательКПП - Строка,
//       ** ПолучательФамилия - Строка,
//       ** ПолучательИмя - Строка,
//       ** ПолучательОтчество - Строка,
//       ** ПолучательКодНО - Строка,
//       ** ДатаПринятияДокумента - Дата,
//       ** КНДДокумента - Строка - КНД 1160303 ДОПП,
//       ** ДатаНаправленияДокумента - Дата,
//       ** ВремяНаправленияДокумента - Дата,
//       ** ДатаВремяНаправленияДокумента - Дата - сумма ДатаНаправленияДокумента и ВремяНаправленияДокумента,
//       ** ДатаПоступленияДокумента - Дата,
//       ** ИмяПринятогоФайла - Строка,
//       ** НомерДОПП - Строка,
//       ** ДатаРегистрацииДокумента - Дата,
//       ** Статус - Строка - см. функцию "СтатусыДОПП",
//       ** КодПричиныОшибки - Строка - "21" – QR-код прошел проверку, выявлены нарушения, возврат,
//                                      "22" – QR-код не прошел проверку, QR-код не найден, возврат,
//                                      "23" – QR-код прошел проверку, товар другой, возврат,
//                                      "24" – QR-код прошел проверку, товара больше, чем в ДОПП, возврат,
//                                      "25" – QR-код прошел проверку, товар передан в ФОИВ, изъят или уничтожен,
//                                      "26" – QR-код не прошел проверку, подтверждено убытие транспортного средства,
//       ** QRКод - Строка,
//       ** Примечание - Строка,
//       ** ОтправительЭтоОрганизация - Булево,
//       ** ОтправительНаименованиеОрганизации - Строка,
//       ** ОтправительИНН - Строка,
//       ** ОтправительКПП - Строка,
//       ** ОтправительФамилия - Строка,
//       ** ОтправительИмя - Строка,
//       ** ОтправительОтчество - Строка,
//       ** ОтправительКодНО - Строка,
//       ** ПромежуточныйКодНО - Строка,
//       ** НалогоплательщикЭтоОрганизация - Булево,
//       ** НалогоплательщикНаименованиеОрганизации - Строка,
//       ** НалогоплательщикИНН - Строка,
//       ** НалогоплательщикКПП - Строка,
//       ** НалогоплательщикФамилия - Строка,
//       ** НалогоплательщикИмя - Строка,
//       ** НалогоплательщикОтчество - Строка,
//       ** НалогоплательщикКодНО - Строка,
//       ** ДолжностноеЛицоНОФамилия - Строка,
//       ** ДолжностноеЛицоНОИмя - Строка,
//       ** ДолжностноеЛицоНООтчество - Строка,
//     * ОписаниеОшибки - Строка - описание ошибки извлечения данных квитанции, например, из-за
//                                 недостаточности прав на документооборот с гос.органами.
//
Функция СведенияКвитанцииОПрисвоенииСтатусаДОПП(СсылкаНаОтчетИлиДанныеКвитанции, СтатусОтправки = Неопределено) Экспорт
	
	Квитанция = Новый Структура;
	Квитанция.Вставить("ИдентификаторФайла", "");
	Квитанция.Вставить("ВерсияПрограммы", "");
	Квитанция.Вставить("ВерсияФормат", "");
	Квитанция.Вставить("КНД", "");
	
	Квитанция.Вставить("ПолучательЭтоОрганизация", Ложь);
	Квитанция.Вставить("ПолучательНаименованиеОрганизации", "");
	Квитанция.Вставить("ПолучательИНН", "");
	Квитанция.Вставить("ПолучательКПП", "");
	Квитанция.Вставить("ПолучательФамилия", "");
	Квитанция.Вставить("ПолучательИмя", "");
	Квитанция.Вставить("ПолучательОтчество", "");
	Квитанция.Вставить("ПолучательКодНО", "");
	
	Квитанция.Вставить("ДатаПринятияДокумента", Неопределено);
	Квитанция.Вставить("КНДДокумента", "");
	Квитанция.Вставить("ДатаНаправленияДокумента", Неопределено);
	Квитанция.Вставить("ВремяНаправленияДокумента", Неопределено);
	Квитанция.Вставить("ДатаВремяНаправленияДокумента", Неопределено);
	Квитанция.Вставить("ДатаПоступленияДокумента", Неопределено);
	Квитанция.Вставить("ИмяПринятогоФайла", "");
	Квитанция.Вставить("НомерДОПП", "");
	Квитанция.Вставить("ДатаРегистрацииДокумента", Неопределено);
	Квитанция.Вставить("СтатусДОПП", "");
	Квитанция.Вставить("КодПричиныОшибки", "");
	Квитанция.Вставить("QRКод", "");
	Квитанция.Вставить("Примечание", "");
	
	Квитанция.Вставить("ОтправительЭтоОрганизация", Ложь);
	Квитанция.Вставить("ОтправительНаименованиеОрганизации", "");
	Квитанция.Вставить("ОтправительИНН", "");
	Квитанция.Вставить("ОтправительКПП", "");
	Квитанция.Вставить("ОтправительФамилия", "");
	Квитанция.Вставить("ОтправительИмя", "");
	Квитанция.Вставить("ОтправительОтчество", "");
	Квитанция.Вставить("ОтправительКодНО", "");
	
	Квитанция.Вставить("ПромежуточныйКодНО", "");
	
	Квитанция.Вставить("НалогоплательщикЭтоОрганизация", Ложь);
	Квитанция.Вставить("НалогоплательщикНаименованиеОрганизации", "");
	Квитанция.Вставить("НалогоплательщикИНН", "");
	Квитанция.Вставить("НалогоплательщикКПП", "");
	Квитанция.Вставить("НалогоплательщикФамилия", "");
	Квитанция.Вставить("НалогоплательщикИмя", "");
	Квитанция.Вставить("НалогоплательщикОтчество", "");
	Квитанция.Вставить("НалогоплательщикКодНО", "");
	
	Квитанция.Вставить("ДолжностноеЛицоНОФамилия", "");
	Квитанция.Вставить("ДолжностноеЛицоНОИмя", "");
	Квитанция.Вставить("ДолжностноеЛицоНООтчество", "");
	
	Результат = Новый Структура;
	Результат.Вставить("ЭтоКвитанцияОПрисвоенииСтатусаДОПП", Ложь);
	Результат.Вставить("НомерУведомления", "");
	Результат.Вставить("ДатаУведомления", Неопределено);
	Результат.Вставить("ДанныеКвитанции", Неопределено);
	Результат.Вставить("Квитанция", Квитанция);
	...
	Если ТипЗнч(СсылкаНаОтчетИлиДанныеКвитанции) = Тип("СправочникСсылка.ОписиВходящихДокументовИзНалоговыхОрганов")
		...
		Для каждого ВходящийДокумент Из СсылкаНаОтчетИлиДанныеКвитанции.ВходящиеДокументы Цикл
			...
			Если ТипЗнч(ДокументРеализацииПолномочийНО) =
				...
				Если ДокументРеализацииПолномочийНО.ВидДокумента =
					Перечисления.ВидыНалоговыхДокументов.КвитанцияОПрисвоенииСтатусаДОПП Тогда
					
					Результат.ЭтоКвитанцияОПрисвоенииСтатусаДОПП = Истина;
					Результат.НомерУведомления = ДокументРеализацииПолномочийНО.НомерДокумента;
					Результат.ДатаУведомления = ДокументРеализацииПолномочийНО.ДатаДокумента;
	...
	Если Результат.ДанныеКвитанции <> Неопределено Тогда
		...
		Исключение
			ПодробноеПредставление = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ИмяСобытия = НСтр("ru = 'Электронный документооборот с контролирующими органами. Квитанция о присвоении статуса ДОПП'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(
				ИмяСобытия ,
				УровеньЖурналаРегистрации.Ошибка,,,
		...
		Если КорневойУзелОбъекта <> Неопределено И КорневойУзелОбъекта.Свойства().Получить("ИдФайл") <> Неопределено Тогда
			Результат.Квитанция.ИдентификаторФайла = XMLЗначение(Тип("Строка"), КорневойУзелОбъекта.ИдФайл);
		КонецЕсли;
		
		Если КорневойУзелОбъекта <> Неопределено И КорневойУзелОбъекта.Свойства().Получить("ВерсПрог") <> Неопределено Тогда
			Результат.Квитанция.ВерсияПрограммы = XMLЗначение(Тип("Строка"), КорневойУзелОбъекта.ВерсПрог);
		КонецЕсли;
		
		Если КорневойУзелОбъекта <> Неопределено И КорневойУзелОбъекта.Свойства().Получить("ВерсФорм") <> Неопределено Тогда
			Результат.Квитанция.ВерсияФормат = XMLЗначение(Тип("Строка"), КорневойУзелОбъекта.ВерсФорм);
		КонецЕсли;
		
		Если КорневойУзелОбъекта <> Неопределено И КорневойУзелОбъекта.Свойства().Получить("Документ") <> Неопределено Тогда
			УзелДокумент = КорневойУзелОбъекта.Документ;
			
			Если УзелДокумент.Свойства().Получить("КНД") <> Неопределено Тогда
				Результат.Квитанция.КНД = XMLЗначение(Тип("Строка"), УзелДокумент.КНД);
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("СвПолДок") <> Неопределено Тогда
				УзелСвПолДок = УзелДокумент.СвПолДок;
				
				Если УзелСвПолДок.Свойства().Получить("ПолучЮЛ") <> Неопределено Тогда
					УзелПолучЮЛ = УзелСвПолДок.ПолучЮЛ;
					
					Результат.Квитанция.ПолучательЭтоОрганизация = Истина;
					
					Если УзелПолучЮЛ.Свойства().Получить("НаимОрг") <> Неопределено Тогда
						Результат.Квитанция.ПолучательНаименованиеОрганизации = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.НаимОрг);
					КонецЕсли;
					
					Если УзелПолучЮЛ.Свойства().Получить("ИННЮЛ") <> Неопределено Тогда
						Результат.Квитанция.ПолучательИНН = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.ИННЮЛ);
					КонецЕсли;
					
					Если УзелПолучЮЛ.Свойства().Получить("КПП") <> Неопределено Тогда
						Результат.Квитанция.ПолучательКПП = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.КПП);
					КонецЕсли;
					
				ИначеЕсли УзелСвПолДок.Свойства().Получить("ПолучИП") <> Неопределено Тогда
					УзелПолучИП = УзелСвПолДок.ПолучИП;
					
					Если УзелПолучИП.Свойства().Получить("ИННИП") <> Неопределено Тогда
						Результат.Квитанция.ПолучательИНН = XMLЗначение(Тип("Строка"), УзелПолучИП.ИННИП);
					КонецЕсли;
					
					Если УзелПолучИП.Свойства().Получить("ФИО") <> Неопределено Тогда
						УзелФИО = УзелПолучИП.ФИО;
						
						Если УзелФИО.Свойства().Получить("Фамилия") <> Неопределено Тогда
							Результат.Квитанция.ПолучательФамилия = XMLЗначение(Тип("Строка"), УзелФИО.Фамилия);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Имя") <> Неопределено Тогда
							Результат.Квитанция.ПолучательИмя = XMLЗначение(Тип("Строка"), УзелФИО.Имя);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Отчество") <> Неопределено Тогда
							Результат.Квитанция.ПолучательОтчество = XMLЗначение(Тип("Строка"), УзелФИО.Отчество);
						КонецЕсли;
					КонецЕсли;
					
				ИначеЕсли УзелСвПолДок.Свойства().Получить("ОтпрНО") <> Неопределено Тогда
					УзелОтпрНО = УзелСвПолДок.ОтпрНО;
					
					Если УзелОтпрНО.Свойства().Получить("КодНО") <> Неопределено Тогда
						Результат.Квитанция.ПолучательКодНО = XMLЗначение(Тип("Строка"), УзелОтпрНО.КодНО);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("СвКвит") <> Неопределено Тогда
				УзелСвКвит = УзелДокумент.СвКвит;
				
				Если УзелСвКвит.Свойства().Получить("ДатаПрин") <> Неопределено И ЗначениеЗаполнено(УзелСвКвит.ДатаПрин) Тогда
					ДатаСтрокой = XMLЗначение(Тип("Строка"), УзелСвКвит.ДатаПрин);
					Результат.Квитанция.ДатаПринятияДокумента = ДатаВремяИзСтрокиXML(ДатаСтрокой);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("КНД") <> Неопределено Тогда
					Результат.Квитанция.КНДДокумента = XMLЗначение(Тип("Строка"), УзелСвКвит.КНД);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ДатаНапр") <> Неопределено И ЗначениеЗаполнено(УзелСвКвит.ДатаНапр) Тогда
					ДатаСтрокой = XMLЗначение(Тип("Строка"), УзелСвКвит.ДатаНапр);
					Результат.Квитанция.ДатаНаправленияДокумента = ДатаВремяИзСтрокиXML(ДатаСтрокой);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ВремНапр") <> Неопределено И ЗначениеЗаполнено(УзелСвКвит.ВремНапр) Тогда
					ВремяСтрокой = XMLЗначение(Тип("Строка"), УзелСвКвит.ВремНапр);
					ВремяСтрокой = СтрЗаменить(ВремяСтрокой, ".", ":");
					Результат.Квитанция.ВремяНаправленияДокумента = ДатаВремяИзСтрокиXML("01.01.0001 " + ВремяСтрокой);
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Результат.Квитанция.ДатаНаправленияДокумента)
					ИЛИ ЗначениеЗаполнено(Результат.Квитанция.ВремяНаправленияДокумента) Тогда
					
					Результат.Квитанция.ДатаВремяНаправленияДокумента =
						?(ЗначениеЗаполнено(Результат.Квитанция.ДатаНаправленияДокумента),
							Результат.Квитанция.ДатаНаправленияДокумента, '00010101')
						+ (?(ЗначениеЗаполнено(Результат.Квитанция.ВремяНаправленияДокумента),
							Результат.Квитанция.ВремяНаправленияДокумента, '00010101') - '00010101');
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ДатаПост") <> Неопределено И ЗначениеЗаполнено(УзелСвКвит.ДатаПост) Тогда
					ДатаСтрокой = XMLЗначение(Тип("Строка"), УзелСвКвит.ДатаПост);
					Результат.Квитанция.ДатаПоступленияДокумента = ДатаВремяИзСтрокиXML(ДатаСтрокой);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ИмяПринятФайла") <> Неопределено Тогда
					Результат.Квитанция.ИмяПринятогоФайла = XMLЗначение(Тип("Строка"), УзелСвКвит.ИмяПринятФайла);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("НомерДОПП") <> Неопределено Тогда
					Результат.Квитанция.НомерДОПП = XMLЗначение(Тип("Строка"), УзелСвКвит.НомерДОПП);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ДатаРегистр") <> Неопределено
					И ЗначениеЗаполнено(УзелСвКвит.ДатаРегистр) Тогда
					ДатаСтрокой = XMLЗначение(Тип("Строка"), УзелСвКвит.ДатаРегистр);
					Результат.Квитанция.ДатаРегистрацииДокумента = ДатаВремяИзСтрокиXML(ДатаСтрокой);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("Статус") <> Неопределено Тогда
					Результат.Квитанция.СтатусДОПП = XMLЗначение(Тип("Строка"), УзелСвКвит.Статус);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("ПричОшибк") <> Неопределено Тогда
					Результат.Квитанция.КодПричиныОшибки = XMLЗначение(Тип("Строка"), УзелСвКвит.ПричОшибк);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("QRКод") <> Неопределено Тогда
					Результат.Квитанция.QRКод = XMLЗначение(Тип("Строка"), УзелСвКвит.QRКод);
				КонецЕсли;
				
				Если УзелСвКвит.Свойства().Получить("Примечание") <> Неопределено Тогда
					Результат.Квитанция.Примечание = XMLЗначение(Тип("Строка"), УзелСвКвит.Примечание);
				КонецЕсли;
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("СвОтпрДок") <> Неопределено Тогда
				УзелСвОтпрДок = УзелДокумент.СвПолДок;
				
				Если УзелСвОтпрДок.Свойства().Получить("ПолучЮЛ") <> Неопределено Тогда
					УзелПолучЮЛ = УзелСвОтпрДок.ПолучЮЛ;
					
					Результат.Квитанция.ОтправительЭтоОрганизация = Истина;
					
					Если УзелПолучЮЛ.Свойства().Получить("НаимОрг") <> Неопределено Тогда
						Результат.Квитанция.ОтправительНаименованиеОрганизации = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.НаимОрг);
					КонецЕсли;
					
					Если УзелПолучЮЛ.Свойства().Получить("ИННЮЛ") <> Неопределено Тогда
						Результат.Квитанция.ОтправительИНН = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.ИННЮЛ);
					КонецЕсли;
					
					Если УзелПолучЮЛ.Свойства().Получить("КПП") <> Неопределено Тогда
						Результат.Квитанция.ОтправительКПП = XMLЗначение(Тип("Строка"), УзелПолучЮЛ.КПП);
					КонецЕсли;
					
				ИначеЕсли УзелСвОтпрДок.Свойства().Получить("ПолучИП") <> Неопределено Тогда
					УзелПолучИП = УзелСвОтпрДок.ПолучИП;
					
					Если УзелПолучИП.Свойства().Получить("ИННИП") <> Неопределено Тогда
						Результат.Квитанция.ОтправительИНН = XMLЗначение(Тип("Строка"), УзелПолучИП.ИННИП);
					КонецЕсли;
					...
					Если УзелПолучИП.Свойства().Получить("ФИО") <> Неопределено Тогда
						...
						Если УзелФИО.Свойства().Получить("Фамилия") <> Неопределено Тогда
							Результат.Квитанция.ОтправительФамилия = XMLЗначение(Тип("Строка"), УзелФИО.Фамилия);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Имя") <> Неопределено Тогда
							Результат.Квитанция.ОтправительИмя = XMLЗначение(Тип("Строка"), УзелФИО.Имя);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Отчество") <> Неопределено Тогда
							Результат.Квитанция.ОтправительОтчество = XMLЗначение(Тип("Строка"), УзелФИО.Отчество);
						КонецЕсли;
					КонецЕсли;
					
				ИначеЕсли УзелСвОтпрДок.Свойства().Получить("ОтпрНО") <> Неопределено Тогда
					УзелОтпрНО = УзелСвОтпрДок.ОтпрНО;
					
					Если УзелОтпрНО.Свойства().Получить("КодНО") <> Неопределено Тогда
						Результат.Квитанция.ОтправительКодНО = XMLЗначение(Тип("Строка"), УзелОтпрНО.КодНО);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("НОПром") <> Неопределено Тогда
				УзелНОПром = УзелДокумент.НОПром;
				
				Если УзелНОПром.Свойства().Получить("КодНО") <> Неопределено Тогда
					Результат.Квитанция.ПромежуточныйКодНО = XMLЗначение(Тип("Строка"), УзелНОПром.КодНО);
				КонецЕсли;
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("СвАдр") <> Неопределено Тогда
				УзелСвАдр = УзелДокумент.СвАдр;
				
				Если УзелСвАдр.Свойства().Получить("ЮЛ") <> Неопределено Тогда
					УзелЮЛ = УзелСвАдр.ЮЛ;
					
					Результат.Квитанция.НалогоплательщикЭтоОрганизация = Истина;
					
					Если УзелЮЛ.Свойства().Получить("НаимОрг") <> Неопределено Тогда
						Результат.Квитанция.НалогоплательщикНаименованиеОрганизации = XMLЗначение(Тип("Строка"), УзелЮЛ.НаимОрг);
					КонецЕсли;
					
					Если УзелЮЛ.Свойства().Получить("ИННЮЛ") <> Неопределено Тогда
						Результат.Квитанция.НалогоплательщикИНН = XMLЗначение(Тип("Строка"), УзелЮЛ.ИННЮЛ);
					КонецЕсли;
					
					Если УзелЮЛ.Свойства().Получить("КПП") <> Неопределено Тогда
						Результат.Квитанция.НалогоплательщикКПП = XMLЗначение(Тип("Строка"), УзелЮЛ.КПП);
					КонецЕсли;
					
				ИначеЕсли УзелСвАдр.Свойства().Получить("ИП") <> Неопределено Тогда
					УзелИП = УзелСвАдр.ИП;
					
					Если УзелИП.Свойства().Получить("ИННИП") <> Неопределено Тогда
						Результат.Квитанция.НалогоплательщикИНН = XMLЗначение(Тип("Строка"), УзелИП.ИННИП);
					КонецЕсли;
					
					Если УзелИП.Свойства().Получить("ФИО") <> Неопределено Тогда
						УзелФИО = УзелИП.ФИО;
						
						Если УзелФИО.Свойства().Получить("Фамилия") <> Неопределено Тогда
							Результат.Квитанция.НалогоплательщикФамилия = XMLЗначение(Тип("Строка"), УзелФИО.Фамилия);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Имя") <> Неопределено Тогда
							Результат.Квитанция.НалогоплательщикИмя = XMLЗначение(Тип("Строка"), УзелФИО.Имя);
						КонецЕсли;
						
						Если УзелФИО.Свойства().Получить("Отчество") <> Неопределено Тогда
							Результат.Квитанция.НалогоплательщикОтчество = XMLЗначение(Тип("Строка"), УзелФИО.Отчество);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			
			Если УзелДокумент.Свойства().Получить("ФИОДолж") <> Неопределено Тогда
				УзелФИОДолж = УзелДокумент.ФИОДолж;
				
				Если УзелФИОДолж.Свойства().Получить("Фамилия") <> Неопределено Тогда
					Результат.Квитанция.ДолжностноеЛицоНОФамилия = XMLЗначение(Тип("Строка"), УзелФИОДолж.Фамилия);
				КонецЕсли;
				
				Если УзелФИОДолж.Свойства().Получить("Имя") <> Неопределено Тогда
					Результат.Квитанция.ДолжностноеЛицоНОИмя = XMLЗначение(Тип("Строка"), УзелФИОДолж.Имя);
				КонецЕсли;
				
				Если УзелФИОДолж.Свойства().Получить("Отчество") <> Неопределено Тогда
					Результат.Квитанция.ДолжностноеЛицоНООтчество = XMLЗначение(Тип("Строка"), УзелФИОДолж.Отчество);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// сейчас возвращает квитанции о присвоении РНПТ, квитанции о присвоении статуса ДОПП.
// Параметры:
...
//   РасширениеПолучаемогоПриложения - Строка, Массив или Неопределено - ".png" для отбора приложения квитанций
//     о присвоении статуса ДОПП с QR-кодом.
//   ОтборПолучаемогоПриложения - Структура - Новый Структура("СтатусДОПП", "2") для отбора квитанций о присвоении
//     статуса ДОПП с QR-кодом.
//
// Возвращаемое значение:
//   Массив из Структур - квитанции о присвоении РНПТ, КНД 1169012, присвоении статуса ДОПП, КНД 1167011, по убыванию
//     даты, возвращаются при значении параметра "РасширениеПолучаемогоПриложения" равном Неопределено:
//     * Ссылка - СправочникСсылка.ДокументыРеализацииПолномочийНалоговыхОрганов,
//     * РНПТ - Строка,
//     * ВидДокумента - Строка - "Уведомление об имеющихся остатках товаров, подлежащих прослеживаемости"
//         или "Уведомление о ввозе товаров, подлежащих прослеживаемости, с территории другого государства-члена
//         Евразийского экономического союза на территорию Российской Федерации и иные территории, находящиеся под ее
//         юрисдикцией",
//     * НомерУведомления - Строка,
//     * ДатаУведомления - Дата,
//     * НаименованиеСобственника - Строка - наименование собственника товара, если это организация,
...
//     * СтатусДОПП - Строка - см. функцию "СтатусыДОПП",
//     * СсылкаНаQRКод - Строка - ссылка на QR-код для ДОПП
//     * ОшибкиНСП - Массив:
//        ** КодОшибки - Строка,
//        ** ТекстОшибки - Строка,
...
//   ДвоичныеДанные или Неопределено - данные приложения с расширением имени файла, указанном в параметре
//     "РасширениеПолучаемогоПриложения", если найдено
//
Функция ДокументыРеализацииПолномочийНО(
		СсылкаНаУведомление,
		ВозвращатьСостояние = Ложь,
		РасширениеПолучаемогоПриложения = Неопределено,
		ОтборПолучаемогоПриложения = Неопределено) Экспорт
	
	Если РасширениеПолучаемогоПриложения <> Неопределено Тогда
		МассивРасширений = Неопределено;
		
		МассивДокументов = ДокументыРеализацииПолномочийНО(СсылкаНаУведомление);
		
		Для каждого ДокументРеализацииПолномочийНО Из МассивДокументов Цикл
			СоответствуетОтбору = Истина;
			Если ОтборПолучаемогоПриложения <> Неопределено Тогда
				Для каждого ЭлементОтбора Из ОтборПолучаемогоПриложения Цикл
					Если НЕ ДокументРеализацииПолномочийНО.Свойство(ЭлементОтбора.Ключ)
						ИЛИ ДокументРеализацииПолномочийНО[ЭлементОтбора.Ключ] <> ЭлементОтбора.Значение Тогда
			...
			Если СоответствуетОтбору Тогда
				СведенияКвитанций = СведенияКвитанцийОтчета(
					ДокументРеализацииПолномочийНО.Ссылка,
					Перечисления.ТипыТранспортныхСообщений.ДокументНО);
				Если СведенияКвитанций.Выполнено И СведенияКвитанций.Сведения <> Неопределено Тогда
					Для каждого СтрокаТаблицы Из СведенияКвитанций.Сведения Цикл
						Если МассивРасширений = Неопределено Тогда
							МассивРасширений = ?(ТипЗнч(РасширениеПолучаемогоПриложения) = Тип("Массив")
									ИЛИ ТипЗнч(РасширениеПолучаемогоПриложения) = Тип("ФиксированныйМассив"),
								ОбщегоНазначения.СкопироватьРекурсивно(РасширениеПолучаемогоПриложения, Ложь),
								ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(РасширениеПолучаемогоПриложения));
							Для ИндексЭлемента = 0 По МассивРасширений.Количество() - 1 Цикл
								МассивРасширений[ИндексЭлемента] = нрег(МассивРасширений[ИндексЭлемента]);
							КонецЦикла;
						КонецЕсли;
						
						РасширениеФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(СтрокаТаблицы.ИмяФайла).Расширение;
						РасширениеФайла = нрег(РасширениеФайла);
						Если МассивРасширений.Найти(РасширениеФайла) <> Неопределено И СтрокаТаблицы.Данные <> Неопределено Тогда
							Возврат СтрокаТаблицы.Данные.Получить();
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
	...
	Если ТипЗнч(СсылкаНаУведомление) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		РеквизитыУведомления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			СсылкаНаУведомление,
			"Организация, ИмяОтчета",
			Истина);
	ИначеЕсли ТипЗнч(СсылкаНаУведомление) = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов") Тогда
		РеквизитыУведомления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			СсылкаНаУведомление,
			"Организация, ВидОтчета",
			Истина);
	КонецЕсли;
	ЭтоДОПП = (ТипЗнч(СсылкаНаУведомление) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения")
			И РеквизитыУведомления.ИмяОтчета = "РегламентированноеУведомлениеДокументОПредстоящейПоставке")
		ИЛИ (ТипЗнч(СсылкаНаУведомление) = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов")
			И РеквизитыУведомления.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.ДокументОПредстоящейПоставке);
	
	Если ЭтоУведомлениеОбОстаткахПрослеживаемыхТоваров ИЛИ ЭтоУведомлениеОВвозеПрослеживаемыхТоваров Тогда
		РеквизитыУведомления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			СсылкаНаУведомление,
		...
		Запрос.Текст =
			...
			|	ДокументыРеализацииПолномочийНалоговыхОрганов.СтатусДОПП КАК СтатусДОПП,
			|	ДокументыРеализацииПолномочийНалоговыхОрганов.СсылкаНаQRКод КАК СсылкаНаQRКод,
	...
	ИначеЕсли ЭтоДОПП Тогда
		...
		Запрос = Новый Запрос(
			...
			|ГДЕ
			|	ДокументыРеализацииПолномочийНалоговыхОрганов.Организация = &Организация
			|	И ДокументыРеализацииПолномочийНалоговыхОрганов.ДокументОснование = &ДокументОснование
			|УПОРЯДОЧИТЬ ПО
			|	ДатаДокумента УБЫВ");
		
		Запрос.УстановитьПараметр("Организация", РеквизитыУведомления.Организация);
		Запрос.УстановитьПараметр("ДокументОснование", СсылкаНаУведомление);
...
Функция УчетнаяЗаписьОрганизации(Организация, ДопустимыеОператоры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДопустимыеОператоры) И ЗначениеЗаполнено(УчетнаяЗапись) Тогда
...
Функция СведенияДляБакета(ИмяФайлаВыгрузки = "", КороткоеИмяФайлаВыгрузки = "") Экспорт
	...
	Если ЭтоРегламентированныйОтчетНДС Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаВыгрузки) Тогда
			Если ВерсФорм = "5.12" ИЛИ ВерсФорм = "5.11" Тогда
				...
				Попытка
					...
					Пока ОбъектЧтениеXML.Прочитать() Цикл
						...
						Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
							...
							ИначеЕсли ВерсФорм = "5.12"
								И (ОбъектЧтениеXML.Имя = "РеалТов22"
								ИЛИ ОбъектЧтениеXML.Имя = "РеалТов10"
								ИЛИ ОбъектЧтениеXML.Имя = "РеалТов122"
								ИЛИ ОбъектЧтениеXML.Имя = "РеалТов18.03"
							...
							ИначеЕсли ВерсФорм = "5.11"
								И (ОбъектЧтениеXML.Имя = "РеалТов20"
								ИЛИ ОбъектЧтениеXML.Имя = "РеалТов10"
								ИЛИ ОбъектЧтениеXML.Имя = "ОплПредПост") Тогда
							...
							ИначеЕсли ВерсФорм = "5.11"
								И ОбъектЧтениеXML.Имя = "РеалТов120" Тогда
	...
	ИначеЕсли ЭтоРегламентированныйОтчетРасчетПоСтраховымВзносам Тогда
		...
		Если ЗначениеЗаполнено(ИмяФайлаВыгрузки) Тогда
			Если ВерсФорм = "5.08" ИЛИ ВерсФорм = "5.07" Тогда
