...
Функция РасходыНаПриобретениеККТ(Период, Организация, РегистрацияВНалоговомОргане = Неопределено)
	...
	|				И &УсловиеПоРегистрацииВНалоговомОргане
	|				И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговУменьшаемыхНаРасходыККТ.ЕНВД)) КАК РасходыНаОнлайнКассыУменьшающиеЕНВДОбороты
...
Процедура ОтменитьНачислениеЕНВД(Организация, Период) Экспорт
	...
	Если СостояниеОперации = Перечисления.ВидыСостоянийРегламентныхОпераций.Выполнено
	 Или СостояниеОперации = Перечисления.ВидыСостоянийРегламентныхОпераций.Выполняется Тогда
...
Функция ПериодУменьшенияЕНВДПослеОтмены(Организация) Экспорт
	...
	Иначе
		...
		Результат.КонецПериода = ?(КонецКвартала(СрокиУплатыВзносовЗаСебя.КонецПериода) > ДобавитьМесяц(ДатаОтменыЕНВД - 1, 9),
			ДобавитьМесяц(ДатаОтменыЕНВД - 1, 9), КонецКвартала(СрокиУплатыВзносовЗаСебя.КонецПериода));
...
Процедура ПрименитьВычетПоОнлайнКассам(КонтекстРасчета, РасчетСуммНалога)
	...
	|			&ГраницаОстатков,
	|			Организация = &Организация
	|				И РегистрацияВНалоговомОргане В (&РегистрацииВНалоговомОргане)
	|				И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговУменьшаемыхНаРасходыККТ.ЕНВД)) КАК РасходыНаОнлайнКассыОстатки"
	;
...
Процедура СформироватьДвиженияНачислениеЕНВД(ТаблицаПроводок, ТаблицаРасходовНаОнлайнКассы, ТаблицаРеквизитов, Движения, Отказ) Экспорт
	...
	Для Каждого УчтенныйРасход Из Параметры.РасходыНаОнлайнКассы Цикл
		...
		Движение.СистемаНалогообложения = Перечисления.ВидыНалоговУменьшаемыхНаРасходыККТ.ЕНВД;
