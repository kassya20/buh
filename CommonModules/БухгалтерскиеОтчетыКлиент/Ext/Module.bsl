...
Процедура ПередЗакрытием(Форма, Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка, РежимРасшифровки = Ложь) Экспорт
	...
	БухгалтерскийУчетКлиентПереопределяемый.ПередЗакрытиемФормыСтандартногоОтчета(Форма);
...
Процедура ОтчетСохранитьКак(Форма, ИмяТабличногоДокумента = "Результат", ТипПоУмолчанию = Неопределено) Экспорт
	...
	Контекст.Вставить("ТипПоУмолчанию",      ТипПоУмолчанию);
	
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		ДанныеФайла = МобильныйКлиентБПВызовСервера.ДвоичныеДанныеИзТабличногоДокумента(
			Контекст.СохраняемыйДокумент,
			ТипПоУмолчанию);
		
		Каталог = МобильныйКлиентБПКлиент.ПолучитьВременныйКаталогДокументов();
	
		ИмяФайлаБезТочек = СтрЗаменить(Контекст.Заголовок,"."," ");
		ИмяФайлаСРасширением = ОбщегоНазначенияКлиентСервер.ЗаменитьНедопустимыеСимволыВИмениФайла(ИмяФайлаБезТочек, "_")
			+ "." + ДанныеФайла.РасширениеФайла;
		ПолноеИмяФайла = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(Каталог, ИмяФайлаСРасширением);
		
		ПолучитьФайл(ДанныеФайла.АдресДвоичныхДанных, ПолноеИмяФайла, Ложь);
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботы1СПредприятия(ОповещениеОРаботеСФайлами,
...
Процедура ОтчетСохранитьКакПриПодключенииРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	ТипФайла = Контекст.ТипПоУмолчанию;
...
// Приводит свойство отчета ВидПериода к значению, соответствующему значению свойств НачалоПериода и КонецПериода.
//
// Параметры:
//	Отчет - ОтчетОбъект - обрабатываемый отчет
//
Процедура СброситьВидПериода(Отчет) Экспорт
	...
	Если КонецДня(Отчет.КонецПериода) <> КонецГода(Отчет.КонецПериода) Тогда
		Отчет.ВидПериода = ВидыПериода.ОтчетныйПериод;
	КонецЕсли;
	
	Если КонецДня(Отчет.НачалоПериода) <> КонецГода(Отчет.НачалоПериода)
		И Отчет.ВидПериода = ВидыПериода.МежотчетныйПериод Тогда
		Отчет.ВидПериода = ВидыПериода.ОтчетныйПериод;
	КонецЕсли;
	
КонецПроцедуры

// Устанавливает свойство отчета ВидПериода по имени команды.
//
// Параметры:
//	ИмяКоманды - Строка - Имя команды выбора вида периода.
//	             Должно быть сформировано по шаблонку "ВидПериода<Вариант>",
//	             где вариант в ("ОтчетныйПериод", "МежотчетныйПериод", "ОтчетныйИМежотчетныйПериод")
//	Форма - Форма клиентского приложения - форма отчета
//
Процедура УстановитьВидПериодаПоИмениКоманды(ИмяКоманды, Форма) Экспорт
	
	Отчет = Форма.Отчет;
	
	ВидыПериода = БухгалтерскиеОтчетыКлиентСервер.ВидыПериода();
	
	Вид = СтрЗаменить(ИмяКоманды, "ВидПериода", "");
	
	Если ВидыПериода.Свойство(Вид) Тогда
		Отчет.ВидПериода = ВидыПериода[Вид];
	КонецЕсли;
