...
Процедура ПроверитьИзменениеГражданстваФизическогоЛицаПриЗаписиНабораЗаписей(Источник, Отказ, Замещение) Экспорт
	...
	ФизическиеЛица = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(Источник, Замещение, "ФизическоеЛицо", Истина);
	...
	Для Каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		Если ФизическоеЛицо = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ПроверкаИзмененияГражданстваДоступна(ФизическоеЛицо, СистемаБронирования) Тогда
...
Процедура ПроверитьИзменениеДокументаФизическогоЛицаПриЗаписиНабораЗаписей(Источник, Отказ, Замещение) Экспорт
	...
	ФизическиеЛица = РегистрыБЗК.ЗначенияОтбораЗаписейНабора(Источник, Замещение, "Физлицо", Истина);
	...
	Для Каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		Если ФизическоеЛицо = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ПроверкаИзмененияДокументаДоступна(ФизическоеЛицо, СистемаБронирования) Тогда
...
Процедура ПроверитьИзмененияКадровыхДанныхИЗарегистрироватьНеобходимостьОтправки(СистемаБронирования, Источник)
	...
	НастройкаКонтактнойИнформации = РегистрыСведений.НастройкиКонтактнойИнформацииФизическихЛицБронированияКомандировок.Настройки();
	МобильныйТелефон = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(
		Источник, НастройкаКонтактнойИнформации.ВидКИМобильныйТелефон);
	ЭлектроннаяПочта = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформацииОбъекта(
		Источник, НастройкаКонтактнойИнформации.ВидКИАдресЭлектроннойПочты);
...
Функция ПроверкаИзмененияГражданстваДоступна(ФизическоеЛицо, СистемаБронирования)

	Если Не СотрудникОтправляетсяВСистемуБронирования(СистемаБронирования, ФизическоеЛицо) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПроверитьИзменениеГражданстваИЗарегистрироватьНеобходимостьОтправки(СистемаБронирования, Источник)
	...
	ИначеЕсли ОбщегоНазначения.КоллекцииИдентичны(ПрежнийНабор, Источник, "Страна") Тогда
		РегистрыСведений.ИзмененияФизическихЛицБронированияКомандировок.ЗарегистрироватьСотрудниковВСпискеОтправки(
			СистемаБронирования, Источник[0].ФизическоеЛицо);
	КонецЕсли;
...
Функция ПроверкаИзмененияДокументаДоступна(ФизическоеЛицо, СистемаБронирования)
...
Процедура ПроверитьИзменениеДокументаИЗарегистрироватьНеобходимостьОтправки(СистемаБронирования, Источник)
	...
	ИначеЕсли Не ОбщегоНазначения.КоллекцииИдентичны(ПрежнийНабор, Источник, "Серия, Номер") Тогда
		РегистрыСведений.ИзмененияФизическихЛицБронированияКомандировок.ЗарегистрироватьСотрудниковВСпискеОтправки(
			СистемаБронирования, Источник[0].Физлицо);
	КонецЕсли;
...
Функция ДанныеСтатусаОтправкиСотрудникаПоУсловию(СистемаБронирования, ФизическоеЛицо)
	...
	ПоСтруктуреПредприятия = Не ТипЗнч(ТаблицаВсехПодразделений[0].Подразделение) = Тип("СправочникСсылка.ПодразделенияОрганизаций");

	ТаблицаСотрудников = МестаРаботыСотрудников(ФизическоеЛицо, Неопределено, Неопределено, Ложь, ПоСтруктуреПредприятия).ТаблицаСотрудников;
	Если ТаблицаСотрудников.Количество() = 0 Тогда
		ДанныеСтатуса.Уволен = Истина;
		Возврат ДанныеСтатуса;
	КонецЕсли;

	Если ТаблицаСотрудников.Колонки.Найти("МестоВСтруктуреПредприятия") <> Неопределено Тогда
		ПодразделенияСотрудника = ТаблицаСотрудников.ВыгрузитьКолонку("МестоВСтруктуреПредприятия");
	Иначе
		ПодразделенияСотрудника = ТаблицаСотрудников.ВыгрузитьКолонку("Подразделение");
	КонецЕсли;

	ИспользуетсяОтборПоПодразделениям = РегистрыСведений.НастройкиСистемыБронированияКомандировок.ИспользуетсяОтборПоПодразделениям(СистемаБронирования);

	Если ИспользуетсяОтборПоПодразделениям Тогда
		ПодразделенияОтбора = СписокПодразделенийСотрудникаВходящихВНастройкуОтбораПоПодразделениям(
			СистемаБронирования, ПодразделенияСотрудника, ТаблицаВсехПодразделений);
...
Функция МестаРаботыСотрудников(ФизическоеЛицо, МассивКадровыхПолей, СписокПодразделений, 
	ЭтоОтправка, ПоСтруктуреПредприятия = Неопределено)
	...
	Если ПоСтруктуреПредприятия = Неопределено Тогда
		ПоСтруктуреПредприятия = Ложь;
		Если СписокПодразделений <> Неопределено И СписокПодразделений.Количество() > 0 Тогда
			ПоСтруктуреПредприятия = Не ТипЗнч(СписокПодразделений[0]) = Тип("СправочникСсылка.ПодразделенияОрганизаций");
		КонецЕсли;
	КонецЕсли;
	
	Если ПоСтруктуреПредприятия Тогда
	...
	Иначе
		...
		Если СписокПодразделений <> Неопределено Тогда
			ЗарплатаКадрыОбщиеНаборыДанных.ДобавитьВКоллекциюОтбор(Параметры.Отборы, "Подразделение", "В", СписокПодразделений);
		КонецЕсли;
	КонецЕсли;
	...
	Если СписокПодразделений <> Неопределено И ПоСтруктуреПредприятия Тогда
	...
	КонтактнаяИнформация = КонтактнаяИнформацияФизическихЛиц(ТаблицаСотрудников.ВыгрузитьКолонку("ФизическоеЛицо"));
	УстановитьПривилегированныйРежим(Ложь);

	ДанныеСотрудников = Новый Структура("ТаблицаСотрудников", ТаблицаСотрудников);
	ДанныеСотрудников.Вставить("КонтактнаяИнформация", КонтактнаяИнформация);
...
Функция СписокПодразделенийСотрудникаВходящихВНастройкуОтбораПоПодразделениям(
	Знач СистемаБронирования, Знач СписокПодразделенийСотрудника, Знач ПодразделенияБронированияКомандировок)
	...
	Если ПодразделенияБронированияКомандировок = Неопределено Тогда

		Возврат ВходятСписок;
	КонецЕсли;
	СписокВсехПодразделений = ПодразделенияБронированияКомандировок.ВыгрузитьКолонку("Подразделение");
...
Функция ПодразделенияБронированияКомандировок(СистемаБронирования) Экспорт
	...
	Если МассивВключатьПодчиненные.Количество() > 0 Тогда
		ПоСтруктуреПредприятий = Не ТипЗнч(МассивВключатьПодчиненные[0]) = Тип("СправочникСсылка.ПодразделенияОрганизаций");
		Если ПоСтруктуреПредприятий Тогда
			МодульОрганизационнаяСтруктура = ОбщегоНазначения.ОбщийМодуль("ОрганизационнаяСтруктура");
			ДобавленыВСписок = МодульОрганизационнаяСтруктура.ПодчиненныеПодразделения(МассивВключатьПодчиненные);
		Иначе
			ДобавленыВСписок = ПодчиненныеПодразделенияПодразделенийБронирования(МассивВключатьПодчиненные);
		КонецЕсли;
...
Функция ПодразделенияБронированияКомандировокПоНастройкам(СписокЗначенийПодразделений)
	
	МассивВключатьПодчиненные = Новый Массив;
	МассивНеВключатьПодчиненные = Новый Массив;
	Для Каждого Значение Из СписокЗначенийПодразделений Цикл
		Если Значение.Пометка Тогда
			МассивВключатьПодчиненные.Добавить(Значение.Значение)
		Иначе
			МассивНеВключатьПодчиненные.Добавить(Значение.Значение)
		КонецЕсли;
	КонецЦикла;
...
Функция ПодчиненныеПодразделенияПодразделенийБронирования(МассивПодразделенийУзла) Экспорт
...
Функция ПодготовитьМассивыИзмененныхСотрудниковИзИсключаемыхСписков(НовыеНастройки) Экспорт
...
Функция ПодготовитьМассивыИзмененныхСотрудниковОтправляемыхПоУсловию(НовыеНастройки) Экспорт
	...
	Если НовыеНастройки.ИспользоватьОтборПоПодразделениям Тогда
		ТаблицаВсехПодразделений = ПодразделенияБронированияКомандировокПоНастройкам(НовыеНастройки.ПодразделенияОтбора);
		...
		Иначе
			...
			ТаблицаФизическихЛиц = МестаРаботыСотрудников(Неопределено, Неопределено, СписокПодразделений, Ложь).ТаблицаСотрудников;
		КонецЕсли;
	Иначе
		ТаблицаФизическихЛиц = МестаРаботыСотрудников(Неопределено, Неопределено, Неопределено, Ложь).ТаблицаСотрудников;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеОтправляемыхСотрудников(СистемаБронирования, ДанныеФизическиЛиц, ТаблицаФизическихЛиц)
	...
	ДанныеСотрудников = МестаРаботыСотрудников(МассивФизическихЛиц, МассивПолейКадровыхДанных(), Неопределено, Истина);
	ТаблицаМестРаботыСКадровымиДанными = ДанныеСотрудников.ТаблицаСотрудников;
	...
	Для Каждого ЭлементФизическоеЛицо Из МассивФизическихЛиц Цикл
		...
		НовоеФизическоеЛицо.Телефон 			= НомерТелефонаСтрокой(ДанныеСотрудников.КонтактнаяИнформация.МобильныеТелефоны[ЭлементФизическоеЛицо]);
		НовоеФизическоеЛицо.EMail 				= ДанныеСотрудников.КонтактнаяИнформация.АдресаПочты[ЭлементФизическоеЛицо];
