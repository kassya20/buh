...
Функция СписокМестОсуществленияДеятельности(Организация, ПараметрыОтбора = Неопределено) Экспорт
	...
	Если ПараметрыОтбора <> Неопределено Тогда
		...
		ИначеЕсли ПараметрыОтбора.Свойство("ВидПродукции") И ЗначениеЗаполнено(ПараметрыОтбора.ВидПродукции) Тогда
			
			ИдентификаторТГ = ТоварнаяГруппа(ПараметрыОтбора.ВидПродукции);
			МассивИдентификаторовТГ.Добавить(ИдентификаторТГ);
...
Функция ПараметрыКодаМаркировкиМОТП(ЭлементДанных = Неопределено, ВидПродукции = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Значение = ИсточникДанных["mcGtinCop"];
	Если Значение <> Неопределено Тогда
		
		ПараметрыКодаМаркировки.МинимальнаяРозничнаяЦена = Значение;
		ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ПривестиЗначениеМРЦ(ПараметрыКодаМаркировки, "МинимальнаяРозничнаяЦена");
...
Функция ТоварнаяГруппа(Знач ЗначениеПоиска, ВидПродукции = Неопределено) Экспорт
	...
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") Тогда
		...
		Если ЗначениеПоиска = "lp" Или ЗначениеПоиска = "clothes" Тогда
			...
			ИначеЕсли ВидПродукции = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026 Тогда
				Возврат Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026;
			Иначе
		...
		ИначеЕсли ЗначениеПоиска = ТоварыИзНатуральногоМехаИмяТоварнойГруппы() Тогда
			Возврат Перечисления.ВидыПродукцииИС.ТоварыИзНатуральногоМеха;
		Иначе
			Возврат Перечисления.ВидыПродукцииИС.ПустаяСсылка();
		КонецЕсли;
	ИначеЕсли ТипЗнч(ЗначениеПоиска) = Тип("ПеречислениеСсылка.ВидыПродукцииИС") Тогда
		Если ЗначениеПоиска = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность
			Или ЗначениеПоиска = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2025
			Или ЗначениеПоиска = Перечисления.ВидыПродукцииИС.ЛегкаяПромышленность2026 Тогда
			Возврат "lp";
		...
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.СладостиИКондитерскиеИзделия Тогда
			Возврат "sweets";
		ИначеЕсли ЗначениеПоиска = Перечисления.ВидыПродукцииИС.ТоварыИзНатуральногоМеха Тогда
