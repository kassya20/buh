...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПроверитьАналитикуСчетовУчетаКапитала");
	Методы.Вставить("ПеренестиОстаткиРасчетовСУчредителями");
	Методы.Вставить("ЗаменитьСчетУчетаРасчетовСУчредителями");
...
// Формирует проводки документа по взносу в уставный капитал
//
// Параметры:
//  ТаблицаВзносВУставныйКапитал - ТаблицаЗначений - данные об уставном капитале, 
//   описание колонок см. в ПодготовитьПараметрыВзносаВУставныйКапитал()
...
//
Процедура СформироватьДвиженияВзносВУставныйКапитал(ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты, Движения, Отказ) Экспорт 
	
	Если Не ЗначениеЗаполнено(ТаблицаВзносВУставныйКапитал) Тогда
		Возврат;
	КонецЕсли;
	
	Параметры = ПодготовитьПараметрыВзносаВУставныйКапитал(ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты);
	Реквизиты = Параметры.Реквизиты[0];
	
	ВзносВУставныйКапитал = Параметры.ВзносВУставныйКапитал;
	
	СформироватьПроводкиВзносаВУставныйКапитал(ВзносВУставныйКапитал, Реквизиты, Движения, Отказ);

КонецПроцедуры

// Возвращает статус оплаты уставного капитала по данным документа
//
// Параметры:
//  ТаблицаВзносВУставныйКапитал - ТаблицаЗначений - данные об уставном капитале, 
//   описание колонок см. в ПодготовитьПараметрыВзносаВУставныйКапитал()
...
//  Строка - см. СтатусОплатыУставногоКапитала()
//
Функция СтатусОплатыУставногоКапиталаПоДаннымДокумента(ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты) Экспорт 
	
	Если Не ЗначениеЗаполнено(ТаблицаВзносВУставныйКапитал) Тогда 
		Возврат Неопределено;
	КонецЕсли;
		
	Параметры = ПодготовитьПараметрыСтатусаОплатыУставногоКапиталаПоДаннымДокумента(
		ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты);
	
	Реквизиты = Параметры.Реквизиты[0];
	
	ВзносВУставныйКапитал = Параметры.ВзносВУставныйКапитал;
	СуммаВзносаПоДокументу = ВзносВУставныйКапитал.Итог("СуммаВзноса");
	
	СтатусОплаты = СтатусОплатыУставногоКапитала(Реквизиты.Регистратор, СуммаВзносаПоДокументу);
	
	Возврат СтатусОплаты;
	
КонецФункции

// Возвращает статус оплаты уставного капитала по данным учета, включая сумму взноса по документу
//
// Параметры:
//  Регистратор - ДокументСсылка - документ, регистрирующий оплату уставного капитала
//  СуммаВзносаПоДокументу - сумма взноса в качестве оплаты уставного капитала по документу
//
// Возвращаемое значение:
//  Строка - статус оплаты
//
Функция СтатусОплатыУставногоКапитала(Регистратор, СуммаВзносаПоДокументу = 0) Экспорт 
	
	// Определяем статус оплаты по оплате за весь период: до момента проведения документа и после
	// с учетом оплаченной суммы по документу
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ПозицияРегистратора, Счет = &СчетРасчетыПоВкладамВУставныйКапитал, , Организация = &Организация) КАК ХозрасчетныйОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ПозицияРегистратора, , , Счет = &СчетРасчетыПоВкладамВУставныйКапитал, , Организация = &Организация, , ) КАК ХозрасчетныйОбороты
	|ИТОГИ
	|	СУММА(Задолженность)
	|ПО
	|	ОБЩИЕ";
	...
	Запрос.УстановитьПараметр("ПозицияРегистратора", 
		Новый Граница(Новый МоментВремени(Реквизиты.Дата, Регистратор), ВидГраницы.Исключая));
	...
	Если Выборка.Следующий() Тогда 
		ТекущаяЗадолженность = Выборка.Задолженность;
	КонецЕсли;
	
	ТекущаяЗадолженность = ТекущаяЗадолженность - СуммаВзносаПоДокументу;
	
	СтатусОплаты = ?(ТекущаяЗадолженность <= 0, "Оплачено", "");
...
Функция ПодготовитьПараметрыВзносаВУставныйКапитал(ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты)
	...
	// Подготовка таблицы Параметры.ВзносВУставныйКапитал
	
	СписокОбязательныхКолонок = ""
	+ "Подразделение,"                 // <Ссылка на справочник подразделений> - подразделение для отражения на счете учета денежных средств
	+ "Контрагент,"                    // <СправочникСсылка.ФизическиеЛица>, <СправочникСсылка.Контрагенты> - учредитель, делающий взнос
	+ "СуммаВзноса,"                   // <Число,15,2> - сумма выплаты
	+ "БанковскийСчет,"                // <СправочникСсылка.БанковскиеСчета>
	+ "СтатьяДвиженияДенежныхСредств," // <СправочникСсылка.СтатьиДвиженияДенежныхСредств>
	+ "СчетУчета";                     // <ПланСчетовСсылка.Хозрасчетный> - счет учета денежных средств
	
	Параметры.Вставить("ВзносВУставныйКапитал", ОбщегоНазначенияБПВызовСервера.ПолучитьТаблицуПараметровПроведения(
		ТаблицаВзносВУставныйКапитал, СписокОбязательныхКолонок));
	
	Возврат Параметры;

КонецФункции

Процедура СформироватьПроводкиВзносаВУставныйКапитал(ТаблицаВзноса, Реквизиты, Движения, Отказ)
	
	Если Не ЗначениеЗаполнено(ТаблицаВзноса) Тогда 
		Возврат;
	КонецЕсли;
	
	СчетРасчетовПоВзносам = ПланыСчетов.Хозрасчетный.РасчетыПоВкладамВУставныйКапитал;
	
	Для Каждого СтрокаТаблицы Из ТаблицаВзноса Цикл
		
		Если СтрокаТаблицы.СуммаВзноса = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстСодержания = НСтр("ru = 'Оплата доли в уставном капитале'");
		...
		Проводка.Сумма = СтрокаТаблицы.СуммаВзноса;
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.БанковскийСчет) Тогда
			БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "БанковскиеСчета", 
		...
		Проводка.СчетКт = СчетРасчетовПоВзносам;
		БухгалтерскийУчет.УстановитьСубконто(Проводка.СчетКт, Проводка.СубконтоКт, "Учредители", СтрокаТаблицы.Контрагент);
...
Функция ПодготовитьПараметрыСтатусаОплатыУставногоКапиталаПоДаннымДокумента(ТаблицаВзносВУставныйКапитал, ТаблицаРеквизиты)
...
Процедура ПроверитьАналитикуСчетовУчетаКапитала() Экспорт
	...
	КлючеваяОперация = "ИндексТаблицыСубконто.АналитикаСчетовУчетаКапитала";
...
Процедура ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2019Кв4(ПараметрыОтчета, Контейнер) Экспорт
	...
	// заполняем данными
	Запрос = Новый Запрос;
	УстановитьПараметрыЗапросаРасчетПоНалогуНаПрибыль(Запрос, ПараметрыОтчета, НачалоТекущегоПериода);
	
	Если Не ЕстьРасчетыСУчредителями(Запрос) Или Не ЕстьДокументыВыплатыДивидендов(Запрос) Тогда
		// Первая и единственная страница - шаблон
	...
	КатегорияНалоговогоАгента = СуммыВыплаченныхДивидендов(Запрос);
	Пока КатегорияНалоговогоАгента.Следующий() Цикл
										...
										Пока ДетальныеДанные.Следующий() Цикл
											...
											Если ЗначениеЗаполнено(ДатаУдержания)
												...
												Если ВидДивидендов.П0003А0012003 <> 0 Тогда
													...
													Если ПлатежиВБюджетКлиентСервер.ДействуетПереходНаЕНП(ДатаУдержания) Тогда
														СрокУплаты = ДобавитьМесяц(Дата(Год(ДатаУдержания), Месяц(ДатаУдержания), 28), 1);
													Иначе
														СрокУплаты = НачалоДня(ДатаУдержания + 86400);
													КонецЕсли;
...
Процедура ОписаниеПоказателей_РасчетПоНалогуНаПрибыль_2025Кв1(ПоказателиОтчета) Экспорт
	
	ОписаниеПоказателей_РасчетПоНалогуНаПрибыль_2019Кв4(ПоказателиОтчета);
	
	// Изменения Раздела 1.3
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П000130001003", Истина, Ложь);
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П00013М104003", Истина, Ложь);
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П00013М105003", Истина, Ложь);
	РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(ПоказателиОтчета, "П00013М106003", Истина, Ложь);
	
КонецПроцедуры

Процедура ЗначенияПоказателей_РасчетПоНалогуНаПрибыль_2025Кв1(ПараметрыОтчета, Контейнер) Экспорт
	...
	// Порядок хранения данных (только значимые показатели)
	// Раздел1_3 1 - дерево значений с колонками:
	//   Данные - Структура с показателями ПризнакВключения и ОКТМО
	//   ДанныеМногострочныхЧастей - Структура с деревом значений
	//      П00013М1 - Дерево значений с колонками
	//         Данные                    - Структура (показатели)
	//         ДанныеМногострочныхЧастей - пустая структура
	...
	ТаблицаСроковРаздела1_3 = Новый ТаблицаЗначений;
	ТаблицаСроковРаздела1_3.Колонки.Добавить("НомерСрока", ОбщегоНазначения.ОписаниеТипаЧисло(1));
	// Формат выгрузки N(15)
	ТаблицаСроковРаздела1_3.Колонки.Добавить("СуммаУдержанногоНалога", ОбщегоНазначения.ОписаниеТипаЧисло(17, 2));
	...
	Если Не ЕстьРасчетыСУчредителями(Запрос) Или Не ЕстьДокументыВыплатыДивидендов(Запрос) Тогда
		...
		НоваяСтрокаП00013М1                            = ДеревоП00013М1Раздел1_3.Строки.Добавить();
		НоваяСтрокаП00013М1.Данные                     = ШаблонДанныеП00013М1Раздел1_3;
		НоваяСтрокаП00013М1.ДанныеМногострочныхЧастей = Новый Структура();
	...
	// п. 71 Приложения N 2 к приказу ФНС России от 02.10.2024 N ЕД-7-3/830@:
	// "В строке 020 "Вид дохода (код)" налогоплательщик (налоговый агент) указывает код дохода, который отражен в Листе 03, 04 или 09
	// для соответствующего вида дохода, налог с которого отражается в подразделе 1.3 Раздела 1."
	// Раздел 1.3 автоматически заполняется только данными Листа 03.
	// Вид дохода "020" для Листа 03 указан  как единственно допустимый в Приложении N 1 (Форма декларации)
	// и Приложении N 3 (Формат представления декларации в электронной форме) к приказу ФНС России от 02.10.2024 N ЕД-7-3/830@
	ВидДохода = "20";
		...
		Пока ОтчетныйГод.Следующий() Цикл
			...
			Пока ОтчетныйКвартал.Следующий() Цикл
				...
				Пока ВидДивидендов.Следующий() Цикл
					...
					СтруктураДаныхЛиста03А = ОбщегоНазначения.СкопироватьРекурсивно(ШаблонДанныеЛист03А);
					
					ЗаполнитьЗначенияСвойств(СтруктураДаныхЛиста03А, ВидДивидендов); // данные
					СтруктураДаныхЛиста03А.П0003А0000500 = Формат(СтруктураДаныхЛиста03А.П0003А0000500, "ЧГ=0");
					СтруктураДаныхЛиста03А.ПризнакВключения = "V";
					СтруктураДаныхЛиста03А.П0003А0000600 = ВидДохода;
									...
									Пока ДатаВыплаты.Следующий() Цикл
										...
										Пока ДетальныеДанные.Следующий() Цикл
											...
											Если ЗначениеЗаполнено(ДатаУдержания)
												...
												СтруктураДаныхП0003ВМ1Листа03А = ОбщегоНазначения.СкопироватьРекурсивно(ШаблонДанныеП0003ВМ1Лист03А);
												...
												Если ВидДивидендов.П0003А0012003 <> 0 Тогда
													...
													НоваяСтрока1_3 = ТаблицаСроковРаздела1_3.Добавить();
													// п. 4 ст. 287 НК РФ. По доходам, выплачиваемым налогоплательщикам в виде дивидендов, а также процентов по государственным
													// и муниципальным ценным бумагам, налог, удержанный при выплате дохода, перечисляется в бюджет налоговым агентом,
													// осуществившим выплату, не позднее 28-го числа месяца, следующего за месяцем выплаты.
													НоваяСтрока1_3.НомерСрока = (Месяц(ДатаУдержания) - 1) % 3 + 1;
													НоваяСтрока1_3.СуммаУдержанногоНалога = ДетальныеДанные.П0003ВМ106003; // СуммаУдержанногоНалога
													//											
													Если ПараметрыОтчета.Периодичность <> Перечисления.Периодичность.Квартал Тогда
														// Налогоплательщики (налоговые агенты), для которых отчетным периодом признаются месяц, два месяца,
														// три месяца и так далее до окончания календарного года, отражают сумму налога, подлежащую уплате
														// (зачислению) в федеральный бюджет по первому сроку, путем заполнения строки 040
														НоваяСтрока1_3.НомерСрока = 1;
													КонецЕсли;
	...
	ТаблицаСроковРаздела1_3.Свернуть("НомерСрока", "СуммаУдержанногоНалога");
	ТаблицаСроковРаздела1_3.Сортировать("НомерСрока");
	
	ИменаЯчеекСуммыСрока = ИменаЯчеекСуммыСрокаРаздел1_3();
	Если ТаблицаСроковРаздела1_3.Количество() = 0 Тогда
		// первая и единственная строка - шаблон
	...
	Иначе
		...
		ДеревоП00013М1Раздел1_3 = ШаблонДеревоП00013М1Раздел1_3.Скопировать();
		НоваяСтрокаП00013М1 = ДеревоП00013М1Раздел1_3.Строки.Добавить();
		СтруктураДаных00013М1Раздел1_3 = ОбщегоНазначения.СкопироватьРекурсивно(ШаблонДанныеП00013М1Раздел1_3);
		
		Для Каждого СтрокаТаблицы ИЗ ТаблицаСроковРаздела1_3 Цикл
			
			ИмяЯчейкиСуммаСрока = ИменаЯчеекСуммыСрока[СтрокаТаблицы.НомерСрока];
			Если Не ЗначениеЗаполнено(ИмяЯчейкиСуммаСрока) Тогда
				Продолжить;
			КонецЕсли;
			СтруктураДаных00013М1Раздел1_3[ИмяЯчейкиСуммаСрока] = СтрокаТаблицы.СуммаУдержанногоНалога;
			
		КонецЦикла;

		СтруктураДаных00013М1Раздел1_3.П00013М102003 = ВидДохода;
		СтруктураДаных00013М1Раздел1_3.П00013М103003 = "18210101040011000110";
		
		НоваяСтрокаП00013М1.Данные = СтруктураДаных00013М1Раздел1_3;
		...
		Для Каждого ИмяПоказателя ИЗ ШаблонДанныеРаздел1_3 Цикл
			СтруктураДаныхРаздел1_3.Вставить(ИмяПоказателя.Ключ, ИмяПоказателя.Значение);
		КонецЦикла;
		СтруктураДаныхРаздел1_3.П000130001003 = СведенияОбОрганизации.ОКТМО;
		СтруктураДаныхРаздел1_3.ПризнакВключения = "V";
...
Функция ИменаЯчеекСуммыСрокаРаздел1_3() Экспорт
	
	// П00013М104003, П00013М105003, П00013М106003 - первый, второй и третий срок
	...
	Возврат ИменаЯчеекСуммыСрока;
	
КонецФункции

// Получает выборку с данными для заполнения декларации по налогу на прибыль.
// 
// Параметры:
//  Запрос - Запрос
// 
...
//
Функция СуммыВыплаченныхДивидендов(Запрос)
	...
	Возврат Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "П0003А0000100");
	
КонецФункции

Функция ЕстьДокументыВыплатыДивидендов(Запрос)
	...
	|	И РасходныйКассовыйОрдер.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.ВыплатаДивидендов)
	|	И РасходныйКассовыйОрдер.НачислениеДивидендов <> ЗНАЧЕНИЕ(Документ.НачислениеДивидендов.ПустаяСсылка)
	|	И ВЫРАЗИТЬ(РасходныйКассовыйОрдер.Контрагент КАК Справочник.Контрагенты).ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	И СписаниеСРасчетногоСчета.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДивидендов)
	|	И СписаниеСРасчетногоСчета.НачислениеДивидендов <> ЗНАЧЕНИЕ(Документ.НачислениеДивидендов.ПустаяСсылка)
	|	И ВЫРАЗИТЬ(СписаниеСРасчетногоСчета.Контрагент КАК Справочник.Контрагенты).ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо)";
	
	Возврат Не Запрос.Выполнить().Пустой();
	
КонецФункции

// Возвращает Истина, если есть остатки или обороты по счетам расчетов с учредителями.
// 
// Параметры:
//  Запрос - Запрос
// 
...
//  Булево
//
Функция ЕстьРасчетыСУчредителями(Запрос)
...
Процедура УстановитьПараметрыЗапросаРасчетПоНалогуНаПрибыль(Запрос, ПараметрыОтчета, НачалоТекущегоПериода)
	...
	// Но сначала проверим, надо ли вообще заполнять
	// Если в составе учредителей только физ.лица - то не заполняем
	МассивСчетов = Новый Массив;
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыСУчредителями);
	МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоВкладамВУставныйКапитал);
