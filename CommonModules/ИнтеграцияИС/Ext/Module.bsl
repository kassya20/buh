...
// Возвращает таблицу свойств маркируемой продукции по входящей таблице штрихкодов упаковок.
// 
// Параметры:
//  Таблица - ТаблицаЗначений - входящая таблица товаров.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Описание:
//   * ШтрихкодУпаковки - СправочникСсылка.ШтрихкодыУпаковокТоваров - ссылка на штрихкод упаковки
//	 * Номенклатура - ОпределяемыйТип.Номенклатура - Номенклатура.
//	 * МаркируемаяПродукция - Булево - Истина, если продукция маркируемая.
//	 * ВидПродукции - ПеречислениеСсылка.ВидыПродукцииИС - Вид маркируемой продукции.
Функция СвойстваШтрихкодовУпаковок(Таблица) Экспорт
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Т.ШтрихкодУпаковки КАК ШтрихкодУпаковки
	|ПОМЕСТИТЬ ВТ_ШтрихкодовУпаковок
	|ИЗ
	|	&Таблица КАК Т
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ШтрихкодовУпаковок.ШтрихкодУпаковки КАК ШтрихкодУпаковки,
	|	СписокШтрихкодыУпаковокТоваров.Номенклатура КАК Номенклатура
	|ПОМЕСТИТЬ ДанныеШтрихкодовУпаковок
	|ИЗ
	|	ВТ_ШтрихкодовУпаковок КАК ВТ_ШтрихкодовУпаковок
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтрихкодыУпаковокТоваров КАК СписокШтрихкодыУпаковокТоваров
	|		ПО ВТ_ШтрихкодовУпаковок.ШтрихкодУпаковки = СписокШтрихкодыУпаковокТоваров.Ссылка
	|;
	|
	|/////////////////////////////////////////////
	|"
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваМаркируемойПродукции КАК ДанныеПоВидуПродукции
	|			ПО ДанныеШтрихкодовУпаковок.Номенклатура = ДанныеПоВидуПродукции.Номенклатура
	|";
...
Функция УказываетсяТранспортноеСредство() Экспорт
	...
	ИнтеграцияИСПереопределяемый.УказываетсяТранспортноеСредство(УказываетсяТранспортноеСредство);
