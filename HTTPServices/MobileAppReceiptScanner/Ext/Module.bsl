...
Процедура УстановитьМобильноеПриложениеПодключено()
	...
	Приложение = РегистрыСведений.КлиентскоеПриложениеСканированиеЧеков.ИмяМобильногоПриложения();
	Регистрация = ШинаМобильныхПриложенийКлиентскоеПриложение.Регистрация(Приложение);
	Если Не ЗначениеЗаполнено(Регистрация) Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось зарегистрировать клиентское приложение.'");
	КонецЕсли;
...
Функция ЗагрузитьКассовыеЧеки(QRКоды)
	...
	ЕстьОжидающиеЧеки = Ложь;
	СтатусЧекаОжидает = Перечисления.СтатусыКассовыхЧековПодотчетныхЛиц.Ожидает;
	...
	Для Каждого ДанныеДокумента Из ДанныеСозданныхДокументов Цикл
		...
		Если Не ЗначениеЗаполнено(Статус) Тогда
			Статус = СтатусЧекаОжидает;
		КонецЕсли;
		
		Если Статус = СтатусЧекаОжидает Тогда
			ЕстьОжидающиеЧеки = Истина;
		КонецЕсли;
	...
	Если ЕстьОжидающиеЧеки Тогда
		// Наличие чеков в состоянии "Ожидает" указывает на то, что пользователь не заходил в
		// форму списка Авансовых отчетов, как следствие - не получал детализацию чеков от Шины.
		// Инициализируем обмен данных с Шиной.
