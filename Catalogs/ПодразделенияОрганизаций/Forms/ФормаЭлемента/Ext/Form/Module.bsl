...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КодРегионаРегистрации = XMLСтрока(РегистрацияВНалоговомОргане.КодРегиона);
...
Процедура ОписаниеПредставителейПодписанияОтчетностиНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьПредставителейПодписанияОтчетности();
...
&НаКлиенте
Процедура ОткрытьПредставителейПодписанияОтчетности()
	
	ОписаниеОповещенияОВыбореПредставителей = Новый ОписаниеОповещения(
		"Подключаемый_ПослеВыбораПредставителейПодписанияОтчетности", ЭтотОбъект);
	
	АдресВременногоХранилищаПредставителей = АдресВременногоХранилищаПредставителейПодписанияОтчетности();
...
&НаСервере
Функция АдресВременногоХранилищаПредставителейПодписанияОтчетности()
	
	СведенияОПодписантахПоРегистрации = РегистрацияВНОСервер.СведенияОПодписантахПоРегистрации(РегистрацияВНалоговомОргане);
...
Процедура Подключаемый_ПослеВыбораПредставителейПодписанияОтчетности(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьПредставителейПодписанияОтчетностиИзВременногоХранилища(Результат);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПредставителейПодписанияОтчетностиИзВременногоХранилища(АдресВременногоХранилищаПредставителей)
...
Процедура АдресИТелефонУстановитьЗаголовок(Форма)
	...
	Если ЗначениеЗаполнено(Форма.РегистрацияВНалоговомОргане.КодПоОКТМО) Тогда
		АдресЗаголовок = АдресЗаголовок + ?(АдресЗаполнен, НСтр("ru=', ОКТМО: '"), ": ");
		АдресЗаголовок = АдресЗаголовок + Форма.РегистрацияВНалоговомОргане.КодПоОКТМО;
		ОКТМОЗаполнен = Истина;
	КонецЕсли;
	...
	Если Телефоны.Количество() > 0 
		...
		Если АдресЗаполнен Или ОКТМОЗаполнен Тогда
...
Процедура ПроверитьКодПоОКТМО()
	...
	ПараметрыПроверки.КодПоОКТМО.ВидКонтактнойИнформации =
		ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций");
...
Функция СтатическиеРеквизиты()
	...
	РазмещеныНаФорме.Вставить(Справочники.ВидыКонтактнойИнформации.ФактическийАдресПодразделенияОрганизаций, Истина);
