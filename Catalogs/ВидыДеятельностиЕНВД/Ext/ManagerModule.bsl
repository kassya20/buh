...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьКонтактнуюИнформациюЗначениямиJSON");
	
КонецПроцедуры

// Функция проверяет, уплачивается ли налог более, чем в одном регионе.
// Регион определяется по первым 2 символам кода ИФНС.
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации	- организация, для которой выполняется проверка
//  Период		 - Дата - дата, на которую считываются настройки уплаты налога
// 
// Возвращаемое значение:
//   - Булево - Истина - налог уплачивается более, чем в одном регионе
//
Функция НалогУплачиваетсяВРазныхРегионах(Организация, Период) Экспорт 

	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ПОДСТРОКА(ВидыДеятельностиЕНВД.РегистрацияВНалоговомОргане.Код, 1, 2)) КАК КоличествоРегионов
	|ИЗ
	...
	|	И ЕСТЬNULL(ВидыДеятельностиЕНВД.РегистрацияВНалоговомОргане.Код, """") <> """"
	|	И ВидыДеятельностиЕНВД.ДатаНачала <= &Период
	|	И (ВидыДеятельностиЕНВД.ДатаПрекращения = ДАТАВРЕМЯ(1, 1, 1)
	|			ИЛИ ВидыДеятельностиЕНВД.ДатаПрекращения >= &Период)";
	
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Период", Период);
		
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий(); // всегда 1 запись
	
	Возврат Выборка.КоличествоРегионов > 1;
