...
Процедура ОткрытьФормуНовойОперации(ВидОперации)
	...
	Если ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийСБилетами.Покупка") Тогда
		ЗначенияЗаполнения.Вставить("Билет", Элементы.Список.ТекущаяСтрока);
	КонецЕсли;	
