...
Процедура УстановитьВидимостьБИКТОФК(Форма)
	...
	Элементы.ДекорацияБИКТОФК.Видимость = БанковскиеПравила.ЭтоБанкУФК(Объект.Код, Объект.КоррСчет);
...
Процедура ИзменитьРеквизитыЗависимыеОтСтраны(Форма)
	...
	Если ЯвляетсяБанкомРФ Тогда
		...
		Форма.Элементы.Код.АвтоОтметкаНезаполненного = Неопределено;
	Иначе
		Форма.Элементы.Код.Заголовок = НСтр("ru = 'Национальный код'");
		Форма.Элементы.Код.АвтоОтметкаНезаполненного = Ложь;
	КонецЕсли;
...
Функция ПоказыватьИнформациюНадежностьБанков(Форма)
	...
	Если Форма.Объект.Страна <> Форма.СтранаРФ Тогда
		Возврат Ложь;
	КонецЕсли;
