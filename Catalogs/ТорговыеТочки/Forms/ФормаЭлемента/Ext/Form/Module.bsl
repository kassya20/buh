...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "РегистрСведений.СоответствиеСкладовТорговымТочкам.Форма.ВыборЗначенийИзСписка" Тогда
		...
		СкладыПредставление = ПредставлениеГиперссылкиСклад(СкладыТорговойТочки, УменьшатьНалогУСННаТорговыйСбор);
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Склада"
		И НЕ ЭтоНоваяТорговаяТочка Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если УменьшатьНалогУСННаТорговыйСбор И СкладыТорговойТочки.Количество() = 0 Тогда
...
Процедура ЗаполнитьОрганизацию()
	...
	ПараметрыТорговыхТочек.Организация = Организация;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	ПараметрыТорговыхТочек.Организация = Объект.Организация;
...
Процедура ЗаполнитьСкладыТорговойТочки()
	...
	СкладыТорговойТочки.Очистить();;
	
	Если НЕ ЭтоНоваяТорговаяТочка Тогда
...
Процедура ЗаписатьСкладыТорговойТочки()
	...
	Если ЭтоНоваяТорговаяТочка И СкладыТорговойТочки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПредставлениеГиперссылкиСклад(СкладыТорговойТочки, УменьшатьНалогУСННаТорговыйСбор)
	...
	Иначе
		...
		МассивСтрок.Добавить(Новый ФорматированнаяСтрока(
			Нстр("ru = 'Выбрать'"), , ЦветГиперссылкиВыбратьСклад(УменьшатьНалогУСННаТорговыйСбор), , "ВыбратьИзСписка"));
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЦветГиперссылкиВыбратьСклад(УменьшатьНалогУСННаТорговыйСбор)
	
	Если УменьшатьНалогУСННаТорговыйСбор Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	УстановитьДоступныеЛьготыПоТорговойТочке(ЭтоНоваяТорговаяТочка);
	УстановитьПредставлениеУведомления();
...
Процедура УстановитьФункциональныеОпцииФормы()
	...
	УменьшатьНалогУСННаТорговыйСбор
		= УчетУСН.ЕстьПравоУменьшатьНалогНаТорговыйСбор(Объект.Организация, ТекущаяДатаСеанса());
