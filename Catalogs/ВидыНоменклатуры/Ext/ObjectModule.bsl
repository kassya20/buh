...
Процедура ВыполнитьПроверкуКлючевыхРеквизитов(Отказ)
	...
	Если НЕ Запрос.Выполнить().Пустой() Тогда
		ТекстСообщения = НСтр("ru = 'Нельзя изменять признак ""Услуга"" если есть номенклатурные позиции данного вида.'"); 
...
Процедура ПередЗаписью(Отказ)
	...
	Если ПодакцизныйТовар И Не ЗначениеЗаполнено(ВидПодакцизногоТовара) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, "ЗАПОЛНЕНИЕ",
		НСтр("ru = 'вид подакцизного товара'"),,, ТекстСообщения);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка, 
		"ВидПодакцизногоТовара", "Объект", Отказ); 
	КонецЕсли;
	Если Не ПодакцизныйТовар Тогда 
		ВидПодакцизногоТовара = Справочники.КодыВидовПодакцизныхТоваров.ПустаяСсылка();
	КонецЕсли;
