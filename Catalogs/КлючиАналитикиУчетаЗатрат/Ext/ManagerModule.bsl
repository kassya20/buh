...
Процедура ПриПоискеЗаменыСсылок(ПарыЗамен, НеобработанныеЗначенияОригиналов) Экспорт
	...
	Для Каждого НеобработанныйДубль Из НеобработанныеЗначенияОригиналов Цикл
		...
		ПарыЗамен.Вставить(НеобработанныйДубль.ЗаменяемоеЗначение, КлючиАналитикиУчетаЗатратДокумента(ПараметрыКлюча));
...
// Выполняет замену дублей в справочнике "Ключи аналитики учета затрат".
//
// Параметры:
//  СписокКлючей - Неопределено
//               - Массив из СправочникСсылка.КлючиАналитикиУчетаЗатрат
//
Процедура ЗаменитьДублиКлючейАналитики(СписокКлючей = Неопределено) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаЗатрат КАК ДанныеРегистра
	...
	|	И &УсловиеСписокКлючей";
	
	Если СписокКлючей = Неопределено Тогда
		Запрос.УстановитьПараметр("УсловиеСписокКлючей", Истина);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеСписокКлючей", "ДанныеСправочника.Ссылка В (&СписокКлючей)");
