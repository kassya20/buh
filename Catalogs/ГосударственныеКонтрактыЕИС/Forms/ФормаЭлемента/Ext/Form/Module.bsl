...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИспользуетсяНесколькоОрганизаций И НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Объект.Организация = Справочники[ИмяСправочникаОрганизации].ОрганизацияПоУмолчанию();
	КонецЕсли;
...
&НаКлиенте
Процедура ГоловнаяОрганизацияПоставщикаОчистка(Элемент, СтандартнаяОбработка)

	ВладелецКонтрактаОчисткаНаСервере(Истина);
...
Процедура КонтрагентОчистка(Элемент, СтандартнаяОбработка)
	...
	Очищать = Объект.ГоловнаяОрганизацияПоставщика.Пустая();
	ВладелецКонтрактаОчисткаНаСервере(Очищать);
...
Процедура ДеревоОбъектовПередНачаломИзменения(Элемент, Отказ)
	...
	ЗаполнитьОграничениеТипаНоменклатуры(Родитель.Тип);
...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ Объект.ГоловнаяОрганизацияПоставщика.Пустая() Тогда
		Элементы.ГоловнаяОрганизацияПоставщика.Видимость	= Истина;
	Иначе
		Элементы.ГоловнаяОрганизацияПоставщика.Видимость	= Ложь;
	КонецЕсли;
	...
	Элементы.Контрагент.ТолькоПросмотр							= НЕ ОбъектыЗакупкиЗаполнены;
	Элементы.ВладелецКонтракта.ТолькоПросмотр					= НЕ ОбъектыЗакупкиЗаполнены;
	Элементы.СоздатьДоговор.Доступность							= ОбъектыЗакупкиЗаполнены;
	Элементы.ГоловнаяОрганизацияПоставщика.Доступность			= ОбъектыЗакупкиЗаполнены;
...
&НаСервере
Процедура ЗаполнитьОграничениеТипаНоменклатуры(ТипОбъектаЗакупки)
	
	ЭлектронноеАктированиеЕИС.ЗаполнитьОграничениеТипаНоменклатуры(
		Элементы.ДеревоОбъектовНаименование, ТипНоменклатура, ТипОбъектаЗакупки);
...
Процедура ЗаполнитьДеревоОбъектовЗакупки()
	...
	ЭлектронноеАктированиеЕИС.ЗаполнитьОграничениеТипаНоменклатуры(
		Элементы.ДеревоОбъектовНаименование, ТипНоменклатура, Перечисления.ТипыОбъектовЗакупкиЕИС.Товар);
...
&НаСервере
Процедура ВладелецКонтрактаОчисткаНаСервере(Очищать)
	
	Если Очищать Тогда
		Объект.ВладелецКонтракта = Метаданные.ОпределяемыеТипы.ДоговорСКонтрагентомЭДО.Тип.ПривестиЗначение();
