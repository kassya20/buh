...
Процедура Сохранить(Команда)
	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		РезультатВыбора.Вставить("ПапкаДляСохранения", МобильныйКлиентБПКлиент.ПутьКВременномуКаталогуДокументов());
	КонецЕсли;
...
Процедура МобильныйКлиентАдаптацияФормы()
	...
	Элементы.ПапкаДляСохраненияФайлов.Видимость = Ложь;
	ВариантСохранения = "СохранитьВПапку";
	Заголовок = НСтр("ru = 'Выбор формата файла'"); 
	Элементы.КнопкаСохранить.Заголовок = НСтр("ru = 'Выбрать'");
