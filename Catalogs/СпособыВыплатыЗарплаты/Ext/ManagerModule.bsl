...
// Возвращает способ выплаты зарплаты для договора подряда
//
// Возвращаемое значение:
//  СправочникСсылка.СпособыВыплатыЗарплаты - способ выплаты аванса
//
Функция ДоговорПодряда() Экспорт
	...
	ЗапросПоискаОбъекта = Новый Запрос;
	ЗапросПоискаОбъекта.УстановитьПараметр("СтатьяРасходов", ДоговорыГПХ);
	...
	ЗапросПоискаОбъекта.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	                            ...
	                            |	И СпособыВыплатыЗарплаты.Поставляемый
	                            |	И СпособыВыплатыЗарплаты.СтатьяРасходов = &СтатьяРасходов";
...
//
// Возвращаемое значение:
// Массив значений СправочникСсылка.СпособыВыплатыЗарплаты - с характером выплаты Межрасчет
//
Функция МежрасчетныеВыплаты() Экспорт
	
	ЗапросПоискаОбъекта = Новый Запрос;
	
	ЗапросПоискаОбъекта.Текст = "ВЫБРАТЬ
	                            |	СпособыВыплатыЗарплаты.Ссылка КАК Ссылка
	                            |ИЗ
	                            |	Справочник.СпособыВыплатыЗарплаты КАК СпособыВыплатыЗарплаты
	                            |ГДЕ
	...
	Результат = ЗапросПоискаОбъекта.Выполнить().Выгрузить();
