...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ЗаголовокСчетаПоУмолчанию = Справочники.ЗаголовкиСчетовПокупателям.ЗаголовокСчетаПоУмолчанию();
...
Процедура ПередЗаписью(Отказ)
	...
	ЕстьОбособленныеПодразделения = БухгалтерскийУчетПереопределяемый.ВестиУчетПоПодразделениям()
	...
	ДополнительныеСвойства.Вставить("ИННДоЗаписи", "");
	ДополнительныеСвойства.Вставить("КодПоОКПОДоЗаписи", "");
	Если Не ЭтоНовый() Тогда
		РеквизитыДоЗаписи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "ИНН, КодПоОКПО");
		ДополнительныеСвойства.Вставить("ИННДоЗаписи", РеквизитыДоЗаписи.ИНН);
		ДополнительныеСвойства.Вставить("КодПоОКПОДоЗаписи", РеквизитыДоЗаписи.КодПоОКПО);
	КонецЕсли;
	...
	Если ПустаяСтрока(Наименование) Или Наименование = Справочники.Организации.НаименованиеПоУмолчанию() Тогда
		...
		Иначе
			НовоеНаименование = ОбщегоНазначенияБПКлиентСервер.НаименованиеФизическогоЛица(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
	...
	Если ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		...
		Если ПустаяСтрока(НаименованиеСокращенное) Тогда
			НаименованиеСокращенное = ОбщегоНазначенияБПКлиентСервер.СокращенноеНаименованиеФизическогоЛица(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
		
		Если ПустаяСтрока(НаименованиеПолное) Тогда
			НаименованиеПолное = ОбщегоНазначенияБПКлиентСервер.ПолноеНаименованиеФизическогоЛица(ФамилияИП, ИмяИП, ОтчествоИП);
		КонецЕсли;
	...
	Если Не ЭтоНовый() Тогда
		...
		РеквизитыДоЗаписи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка,
			"Наименование, НаименованиеПолное, РегистрацияВНалоговомОргане,
			| КрупнейшийНалогоплательщикРегистрацияВНалоговомОргане, ОбособленноеПодразделение, ДатаЗакрытия,
			| КПП, СтатусФизическогоЛица, ВАрхиве");
		...
		ДополнительныеСвойства.Вставить("КрупнейшийНалогоплательщикРегистрацияВНалоговомОрганеДоЗаписи", РеквизитыДоЗаписи.КрупнейшийНалогоплательщикРегистрацияВНалоговомОргане);
		ДополнительныеСвойства.Вставить("ОбособленноеПодразделениеДоЗаписи", РеквизитыДоЗаписи.ОбособленноеПодразделение);
		ДополнительныеСвойства.Вставить("ДатаЗакрытияДоЗаписи", РеквизитыДоЗаписи.ДатаЗакрытия);
		ДополнительныеСвойства.Вставить("КППДоЗаписи", РеквизитыДоЗаписи.КПП);
		ДополнительныеСвойства.Вставить("СтатусФизическогоЛицаДоЗаписи", РеквизитыДоЗаписи.СтатусФизическогоЛица);
		ДополнительныеСвойства.Вставить("ВАрхивеДоЗаписи", РеквизитыДоЗаписи.ВАрхиве);
	...
	Если ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		ДобавитьЗаписьИсторииИзмененийСтатусаФизическогоЛица();
	КонецЕсли;
	...
	Если СтатусФизическогоЛица = Перечисления.СтатусыФизическихЛиц.Самозанятый Тогда
		// Обнуляем реквизиты ИП для самозанятого
	...
	Если ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо
			И РегистрацияОрганизацииКлиентСервер.ПрименяетсяОГРНИП(ДатаРегистрации) Тогда
		// Если дата регистрации ИП > 01.01.2004, то дата присвоения ОГРН равна дате регистрации.
		// Элемент ДатаПрисвоенияОГРН не отображаем на форме карточки организации, дату присвоения очищаем.
		ДатаПрисвоенияОГРН = Неопределено;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	МожноИзменятьНачалоРаботы = Не (ДополнительныеСвойства.Свойство("НеИзменятьНачалоРаботы")
		И ДополнительныеСвойства.НеИзменятьНачалоРаботы = Истина);
	...
	Если НЕ Отказ
		...
		И МожноИзменятьНачалоРаботы Тогда
	...
	Справочники.Организации.ПроверитьИспользуемыеВидыОрганизаций(Отказ);
	Справочники.Организации.ОбновитьИспользуемыеВидыОрганизаций(ТребуетсяСоздатьПоставляемыеВидыНалогов);
	
	РегистрыСведений.ИспользованиеФункциональностиПоОрганизациям.УстановитьЗначенияПоОрганизации(Ссылка);
	...
	ДатаЗакрытияДоЗаписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "ДатаЗакрытияДоЗаписи");
	Если ДатаЗакрытияДоЗаписи <> ДатаЗакрытия И ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		
		// Изменяем действие настроек учета страховых взносов ИП
		УчетСтраховыхВзносовИП.ПриИзмененииДатыЗакрытия(Ссылка, ДатаЗакрытия, ДатаЗакрытияДоЗаписи);
		
		Если ЗначениеЗаполнено(ДатаЗакрытия) Тогда
			// Если есть патенты, необходимо их пересчитать
			УчетПСН.ОбновитьСведенияПоПатентамПриЗакрытииИП(Ссылка);
		КонецЕсли;
		
	КонецЕсли;
	
	СтатусФизическогоЛицаДоЗаписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеСвойства, "СтатусФизическогоЛицаДоЗаписи");
	Если ЗначениеЗаполнено(СтатусФизическогоЛицаДоЗаписи)
		И ЗначениеЗаполнено(СтатусФизическогоЛица)
		И СтатусФизическогоЛица <> СтатусФизическогоЛицаДоЗаписи Тогда
		// ЭДО по умолчанию выключен для Самозанятых и включен для ИП
		ОбменСКонтрагентамиБП.ВключитьЭДОВБазовойВерсии();
	КонецЕсли;
	
	ВАрхивеДоЗаписи = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеСвойства, "ВАрхивеДоЗаписи", Ложь);
	Если Не Отказ И ВАрхиве И ВАрхиве <> ВАрхивеДоЗаписи Тогда
		Если БизнесСеть.ОрганизацияПодключена(Ссылка) Тогда
			СписокОрганизаций = Новый Массив;
			СписокОрганизаций.Добавить(Ссылка);
			БизнесСеть.ОтключитьОрганизации(СписокОрганизаций);
			ТекстСообщения = НСтр(
				"ru = 'Отключен обмен от сервиса 1С:Бизнес-сеть по причине установки флага ""Скрывать в списках, все операции по организации завершены""'",
		...
		// У пользователя может не быть учетки 1С-Отчетности, но быть подключен обмен СФР
		ОтключенОбмен1СОтчетность = Ложь;
		Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(Ссылка) Тогда
			ОрганизацияОбъект = Ссылка.ПолучитьОбъект();
			ОрганизацияОбъект.ВидОбменаСКонтролирующимиОрганами = Перечисления.ВидыОбменаСКонтролирующимиОрганами.ОбменОтключен;
			ОрганизацияОбъект.Записать();
			ОтключенОбмен1СОтчетность = Истина;
		КонецЕсли;
		ЭлектронныйДокументооборотСКонтролирующимиОрганами.ОтключитьОбменыСКонтролирующимиОрганами(Ссылка, ОтключенОбмен1СОтчетность);
		
		Если ОтключенОбмен1СОтчетность Тогда
			ТекстСообщения = НСтр(
				"ru = 'Отключен обмен с контролирующими органами по причине установки флага ""Скрывать в списках, все операции по организации завершены""'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Организация'"),
				УровеньЖурналаРегистрации.Предупреждение,
		...
		РегистрыСведений.НастройкиАвтоматическогоЗакрытияМесяца.ОтключитьАвтоматическоеЗакрытиеМесяцаПоОрганизации(Ссылка);
			
		// Подчиненным организациям также устанавливаем признак неактуальной организации.
		СписокОП = БухгалтерскийУчетПереопределяемый.ПолучитьСписокОбособленныхПодразделенийОрганизации(Ссылка);
		Для Каждого ОП Из СписокОП Цикл
			ОПОбъект = ОП.Значение.ПолучитьОбъект();
			ОПОбъект.ВАрхиве = Истина;
			ОПОбъект.Записать();
		КонецЦикла
...
Функция РеквизитыСервисаПроверокКонтролирующимиОрганами()
	...
	Реквизиты.Вставить("ИзменилсяИНН", ДополнительныеСвойства.Свойство("ИННДоЗаписи")
		И ИНН <> ДополнительныеСвойства.ИННДоЗаписи);
	...
	Реквизиты.Вставить("СтарыйИНН", ДополнительныеСвойства.ИННДоЗаписи);
	...
	Реквизиты.Вставить("КорректныйСтарыйИНН", РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(
		ДополнительныеСвойства.ИННДоЗаписи, ЭтоЮридическоеЛицо, ""));
	Возврат Реквизиты;
...
Процедура ДобавитьЗаписьИсторииИзмененийСтатусаФизическогоЛица()

	ДобавитьЗаписьСтатуса = Ложь;
	ПоследняяЗапись = Неопределено;
	
	Если ИсторияСтатусовФизическогоЛица.Количество() > 0 Тогда
		ИсторияСтатусовФизическогоЛица.Сортировать("Период");
		КоличествоЗаписей = ИсторияСтатусовФизическогоЛица.Количество();
		ПоследняяЗапись = ИсторияСтатусовФизическогоЛица[КоличествоЗаписей - 1];
		Если ПоследняяЗапись.СтатусФизическогоЛица <> СтатусФизическогоЛица Тогда
			ДобавитьЗаписьСтатуса = Истина;
		КонецЕсли;
	Иначе
		ДобавитьЗаписьСтатуса = Истина;
	КонецЕсли;
	
	Если ДобавитьЗаписьСтатуса Тогда
		
		Если ПоследняяЗапись <> Неопределено Тогда
			НоваяЗапись = ПоследняяЗапись;
		Иначе
			НоваяЗапись = ИсторияСтатусовФизическогоЛица.Добавить();
			НоваяЗапись.Период = '00010101';
		КонецЕсли;
		
		НоваяЗапись.СтатусФизическогоЛица = СтатусФизическогоЛица;
