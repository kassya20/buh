...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнениеТиповыхДополнительныхУсловийВСчетахНаОплатуПокупателю");
	Методы.Вставить("ЗаполнениеТиповыхДополнительныхУсловийВОрганизации");
	Методы.Вставить("ЗаполнениеТиповыхДополнительныхУсловий");
...
Функция ПолучитьТиповыеУсловия() Экспорт
	...
	Иначе
		// Если ранее типовые условия не были созданы монопольным обработчиком обновления,
		// то создадим их здесь в отложенном обработчике.
		ТиповыеУсловия = СоздатьИПолучитьТиповыеУсловия();
	КонецЕсли;
...
Процедура ЗаполнениеТиповыхДополнительныхУсловийВСчетахНаОплатуПокупателю(Параметры) Экспорт
	...
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда // В подчиненных узлах РИБ не выполняется, чтобы не было дублей
		...
		Возврат;
	КонецЕсли;
	Если Параметры.Свойство("ТиповыеУсловия") И ЗначениеЗаполнено(Параметры.ТиповыеУсловия) Тогда
...
Функция СоздатьИПолучитьТиповыеУсловия()
	
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда 
		// В подчиненных узлах РИБ если не созданы дополнительные условия, то возвращаем пустую ссылку
		Возврат Справочники.ДополнительныеУсловия.ПустаяСсылка();
	КонецЕсли;
	
	// Пытаемся создать типовые условия
	ЗаполнениеТиповыхДополнительныхУсловий();
	
	// Возвращаем типовые условия, если они созданы
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДополнительныеУсловия.Ссылка
