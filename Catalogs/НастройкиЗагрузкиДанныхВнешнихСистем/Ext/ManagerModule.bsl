...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьАвтоматическоеСозданиеНенайденныхСотрудников");
	
	Методы.Вставить("ЗагрузитьВФоне", Истина);
	Методы.Вставить("ОбновитьПоставляемоеОписаниеВнешнейСистемыВФоне", Истина);
	Методы.Вставить("ПодключитьВнешнююСистемуВФоне", Истина);
	Методы.Вставить("ЗапроситьАктивностьВнешнейСистемы", Истина);
	Методы.Вставить("ЗапроситьОтСервисаАктивностьВнешнейСистемы", Истина);
	Методы.Вставить("ЗапроситьОтключение", Истина);
	Методы.Вставить("ЗапроситьПоставляемоеОписаниеВнешнейСистемы", Истина);
...
Функция ВнешняяСистемаАктивна(Настройка) Экспорт
	...
	ОтветСервиса = ВнешняяСистемаАктивнаЗапросСервиса(Настройка);
	Если Не ОтветСервиса = Истина Тогда
...
// Проверяет активность внешней системы без учета записей справочника СеансыЗагрузкиДанныхВнешнихСистем
//
// Параметры:
//  Параметры        - Структура -
//                      * Настройка - настройка загрузки из внешней системы, активность которой нужно проверить
...
//                     Содержимое временного хранилища - см. ВнешняяСистемаАктивна()
//
Процедура ЗапроситьОтСервисаАктивностьВнешнейСистемы(Параметры, АдресРезультата) Экспорт
	
	ОтветСервиса = ВнешняяСистемаАктивнаЗапросСервиса(Параметры.Настройка);
	ПоместитьВоВременноеХранилище(ОтветСервиса, АдресРезультата);
...
Функция ВнешняяСистемаАктивнаЗапросСервиса(Настройка)
	
	Если Не ИнтернетПоддержкаПользователей.ЗаполненыДанныеАутентификацииПользователяИнтернетПоддержки() Тогда
		Возврат "НетИнтернетПоддержки";
	КонецЕсли;
	
	КодРазрешения = КодРазрешения(Настройка);
	ОписаниеФайла = ФайлОбменаДанными(КодРазрешения);
...
// Обработчик обновления, заполняет реквизит АвтоматическоеСозданиеНенайденныхСотрудников.
//
// Параметры:
//  ПараметрыОбработчика - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке
//
Процедура ЗаполнитьАвтоматическоеСозданиеНенайденныхСотрудников(ПараметрыОбработчика) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиЗагрузкиДанныхВнешнихСистем.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.НастройкиЗагрузкиДанныхВнешнихСистем КАК НастройкиЗагрузкиДанныхВнешнихСистем
	|ГДЕ
	|	НЕ НастройкиЗагрузкиДанныхВнешнихСистем.АвтоматическоеСозданиеНенайденныхСотрудников";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Попытка
			СправочникОбъект = Выборка.Ссылка.ПолучитьОбъект();
			СправочникОбъект.АвтоматическоеСозданиеНенайденныхСотрудников = Истина;
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(СправочникОбъект);
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при выполнении обработчика НастройкиЗагрузкиДанныхВнешнихСистем.ЗаполнитьАвтоматическоеСозданиеНенайденныхСотрудников по причине:
				|%1'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Справочники.НастройкиЗагрузкиДанныхВнешнихСистем,, ТекстСообщения);
		КонецПопытки
	КонецЦикла;
