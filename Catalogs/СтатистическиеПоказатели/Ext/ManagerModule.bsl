...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьПоставляемымиПравилами");
...
Процедура ЗагрузитьФормыСтатистики(ПравилаЗаполнения)
	...
	Для Каждого ПоставляемаяФорма Из ПравилаЗаполнения Цикл
		...
		Если ОписаниеЗагруженногоРанее = Неопределено Тогда
			// Запись объекта может быть отменена ниже
		...
		Объект.Владелец      = Справочники.РегламентированныеОтчеты.НайтиПоРеквизиту(
								"ИсточникОтчета",
		...
		// Если регламетированный отчет не найден, то не создаем элемент справочника Формы статистики и все подчиненные поля
		Если Не ЗначениеЗаполнено(Объект.Владелец) Тогда
			Объект = Неопределено;
			ТекстСообщения = НСтр("ru = 'Не найден регламентированный отчет по источнику ""%1"".'");
			ТекстСообщения = СтрШаблон(ТекстСообщения, ПоставляемаяФорма.РегламентированныйОтчет);
		...
		Для Каждого ПоставляемоеПоле Из ПоставляемаяФорма.Поля Цикл
			...
			ОписаниеЗагруженногоРанее = ЗагруженныйРанееЭлемент(
				ПоставляемоеПоле.СтатистическийПоказатель,
