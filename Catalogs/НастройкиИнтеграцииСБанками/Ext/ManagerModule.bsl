...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьПрефиксы");
	Методы.Вставить("ОбновитьПраваПользователяИнтеграции");
	Методы.Вставить("ПереходНаНовыйИнтерфейсИнтеграцияСБанком");
...
//                      * ДополнительнаяИнформация - Неопределено, Структура - дополнительная информация из банка для обработки
...
Процедура ОбработатьПодтверждения(УчетнаяСистема, Подтверждения, СтандартнаяОбработка) Экспорт
	...
	Для Каждого Подтверждение Из Подтверждения Цикл
		...
		Иначе
			РегистрыСведений.ДокументыИнтеграцииСБанком.ОбработатьДополнительнуюИнформацию(УчетнаяСистема, Подтверждение);
...
Функция ИнтеграцияВИнформационнойБазеВключена() Экспорт
	...
	// У роли УдаленныйДоступOData нет прав на чтение справочника НастройкиИнтеграцииСБанками,
	// поэтому при создании документов через OData нужно получать эти данные в привилегированном режиме.
...
// Возвращает массив всех БИК банков, с которыми настроена интеграция.
//
// Возвращаемое значение:
//   Массив - массив БИК банков.
//
Функция БИКВсехБанковИнтеграции() Экспорт
	...
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("БИК");
...
Функция НастройкаИнтеграцииРегламентированнойОтчетности(Организация) Экспорт
	...
	Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(Организация) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция АдресСервисаВыполненияКомандыИнтеграции() Экспорт
	...
	ЧастиАдреса.Добавить(ИнтеграцияОбъектовВСервисеБП.ИдентификаторХранилищаИнтеграцияОбъектов());
	ЧастиАдреса.Добавить(ИнтеграцияОбъектовВСервисеБП.КомандаВыполнить());
...
// Возвращает параметры настройки для создания Уведомления об исчисленных суммах налога
//
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, для которой необходимо вернуть параметры настройки
//   Период      - Дата - Период, за который необходимо сформировать уведомление по ЕНС
//
// Возвращаемое значение:
//   Структура - См. НовыеПараметрыМетодаУведомленияОСуммахНалога()
//
Функция ВходящиеНастройкиСозданияУведомленияОСуммахНалога(Организация, Период, КБК, СуммаНалога) Экспорт
	
	Результат = НовыеПараметрыМетодаУведомленияОСуммахНалога();
	Результат.method = ИмяМетодаУведомлениеОбИсчисленныхСуммахНалогов();
	
	РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация, "ИНН, КПП");
	
	Результат.parameters.organization.inn = РеквизитыОрганизации.ИНН;
	Результат.parameters.organization.kpp = РеквизитыОрганизации.КПП;
	Результат.parameters.date = Период;
	Результат.parameters.sum = СуммаНалога;
	Результат.parameters.kbk = КБК;
	
	Возврат Результат;
	
КонецФункции

// Возвращает параметры настройки для создания Заявления на зачет
//
// Параметры:
//   Организация  - СправочникСсылка.Организации - Организация, для которой необходимо вернуть параметры настройки
//   Период       - Дата - Период, за который необходимо сформировать уведомление по ЕНС
//   СуммыКУплате - см. Обработка.РасчетСтраховыхВзносовИП.НовыеПоказателиРасчетаВзносовЗаСебя()
//
// Возвращаемое значение:
//   Структура - См. НовыеПараметрыЗаявлениеОЗачете()
//
Функция ВходящиеНастройкиСозданияЗаявленияНаЗачет(Организация, Период, СуммыКУплате) Экспорт
	
	Результат = НовыеПараметрыЗаявлениеОЗачете();
	Результат.method = ИмяМетодаЗаявлениеОЗачетеВСчетПредстоящейОбязанности();
	...
	Результат.parameters.organization.kpp = РеквизитыОрганизации.КПП;
	Результат.parameters.date = Период;
	Результат.parameters.sum_flat = СуммыКУплате.СуммаВзносаЕдиныйТариф;
	Результат.parameters.sum_float = СуммыКУплате.СуммаВзносаПФРсДоходов;
...
// При загрузке выписки по депозитному счету находит и заполняет реквизит СчетКонтрагента 
// в парных операциях по расчетному счету.
//
// Параметры:
//   ПараметрыСозданныхДокументовПоВыписке - Соответствие - параметры созданных документов по выписке:
//     Ключ - идентификатор выписки по банковским счетам,
//      см. ЗагрузкаВыпискиПоБанковскомуСчету.ВыпискиПоБанковскимСчетам(). 
//     Значение - Соответствие - параметры созданных документов,
//      см.ЗагрузкаВыпискиПоБанковскомуСчету.СоздатьДокументы().
Процедура ЗаполнитьСчетКонтрагентаВОперацияхПоДепозитам(ПараметрыСозданныхДокументовПоВыписке) Экспорт
	
	ДатаОкончанияВнесенияИзменений = '20251231235959';
	Если Не (ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком()
		И ТекущаяДатаСеанса() <= ДатаОкончанияВнесенияИзменений) Тогда
		Возврат;
	КонецЕсли;
	
	СозданныеДокументы = Новый Массив;
	Для Каждого ПараметрыДокументовВыписки Из ПараметрыСозданныхДокументовПоВыписке Цикл
		Для Каждого ПараметрыДокумента Из ПараметрыДокументовВыписки.Значение Цикл
			Ссылка = ПараметрыДокумента.Значение.Ссылка;
	...
	СозданныеДокументы = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СозданныеДокументы);
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ПоступлениеНаРасчетныйСчет.Ссылка В (&СозданныеДокументы)
		|	И ПоступлениеНаРасчетныйСчет.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеДенежныхСредств.Депозит)
		...
		|ГДЕ
		|	СписаниеСРасчетногоСчета.Ссылка В (&СозданныеДокументы)
		|	И СписаниеСРасчетногоСчета.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.Депозит)
		...
		|	СозданныеДокументы.СчетОрганизации КАК ДепозитныйСчет
		|ИЗ
		...
		|		ПО СозданныеДокументы.НомерВходящегоДокумента = ДанныеПервичныхДокументов.Номер
		|		И СозданныеДокументы.ДатаВходящегоДокумента = ДанныеПервичныхДокументов.Дата
		|ГДЕ
		|	ДанныеПервичныхДокументов.Документ ССЫЛКА Документ.ПоступлениеНаРасчетныйСчет
		|	И ДанныеПервичныхДокументов.Документ <> СозданныеДокументы.Ссылка
		|	И ДанныеПервичныхДокументов.Документ.СуммаДокумента = СозданныеДокументы.СуммаДокумента
		|	И ДанныеПервичныхДокументов.Документ.СчетКонтрагента = ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		И СозданныеДокументы.ДатаВходящегоДокумента = ДанныеПервичныхДокументов.Дата
		|ГДЕ
		|	ДанныеПервичныхДокументов.Документ ССЫЛКА Документ.СписаниеСРасчетногоСчета
		|	И ДанныеПервичныхДокументов.Документ <> СозданныеДокументы.Ссылка
		|	И ДанныеПервичныхДокументов.Документ.СуммаДокумента = СозданныеДокументы.СуммаДокумента
		|	И ДанныеПервичныхДокументов.Документ.СчетКонтрагента = ЗНАЧЕНИЕ(Справочник.БанковскиеСчета.ПустаяСсылка)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Объект.СчетКонтрагента = ВыборкаДетальныеЗаписи.ДепозитныйСчет;
...
Функция СоздатьЕслиОтсутствует() Экспорт
	
	Если Не ОбщегоНазначенияБП.ИнтеграцияСБанкомДоступна() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ИнтеграцияВключена = ИнтеграцияВИнформационнойБазеВключена();
	
	Если ИнтеграцияВключена Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Не удалось включить интеграцию с банком: не определена организация по умолчанию'"));
		Возврат ИнтеграцияВключена;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(СчетОрганизации) Тогда
		ЗаписатьОшибкуВЖурналРегистрации(
			НСтр("ru = 'Не удалось включить интеграцию с банком: не указан основной банковский счет'"));
		Возврат ИнтеграцияВключена;
	КонецЕсли;
	
	Банк = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СчетОрганизации, "Банк");
	
	БанковскиеСчета = Справочники.БанковскиеСчета.БанковскиеСчетаОрганизации(
		Организация,
		ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета(),
		Банк);
	БанковскиеСчетаСсылка = ОбщегоНазначения.ВыгрузитьКолонку(БанковскиеСчета, "Ссылка");
	...
	Попытка
		...
		Объект.Наименование = Строка(Банк);
		НоваяСтрока = Объект.Банки.Добавить();
		НоваяСтрока.Банк = Банк;
		Объект.Записать();
	
		Если БанковскиеСчетаСсылка.Количество() > 0 Тогда
			ДобавитьБанковскиеСчетаВНастройкеИнтеграции(
				Объект.Ссылка, Организация, БанковскиеСчетаСсылка);
		КонецЕсли;
		
		ЗафиксироватьТранзакцию();
		
		ИнтеграцияВключена = Истина;
		
	Исключение
		ОтменитьТранзакцию();
		ЗаписатьОшибкуВЖурналРегистрации(
			СтрШаблон(НСтр("ru = 'Не удалось включить интеграцию с банком: %1'")), ОписаниеОшибки());
	КонецПопытки;
...
Процедура ЗаполнитьПрефиксы() Экспорт
	...
	|ГДЕ
	|	НастройкиИнтеграцииСБанками.Префикс = """"
	|	И НЕ НастройкиИнтеграцииСБанками.ПометкаУдаления";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НастройкиИнтеграцииСБанками = Выборка.Ссылка.ПолучитьОбъект();
		НастройкиИнтеграцииСБанками.Префикс = НовыйПрефикс();
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(НастройкиИнтеграцииСБанками);
	КонецЦикла;
...
Процедура ВыполнитьОбработкуДанных(УчетнаяСистема, ИдентификаторОбъекта, ПотокДанных, Обработчик, КодВозврата, Ошибка, СообщениеОбОшибке)
	...
	Если НРег(Обработчик) = "bank_account_statement_1c_txt" Тогда
		...
		Если КодВозврата <> ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаВыполнено() Тогда
			Ошибка = Истина;
		КонецЕсли;
	...
	ИначеЕсли НРег(Обработчик) = "initial_balances_bank_account_statement" Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
			КодВозврата = ИнтеграцияОбъектовВСервисеБП.КодСостоянияВыполненоСПредупреждениями();
		КонецЕсли;
	...
	ИначеЕсли ЭтоОбработчикСменыСтатусаУведомления(Обработчик) Тогда
		...
		Если Не Ошибка Тогда
			ИзменитьСтатусУведомления(УчетнаяСистема, ИдентификаторОбъекта, ДанныеОтчета, Ошибка, СообщениеОбОшибке);
		КонецЕсли;
	...
	ИначеЕсли НРег(Обработчик) = "insurance_tariff" Тогда
		
		ДанныеТарифаСтраховыхВзносов = СтруктураИзПотокаJSON(ПотокДанных);
		
		ПроверитьКорректностьДанныхДляТарифовСтраховыхВзносов(ДанныеТарифаСтраховыхВзносов, Ошибка, СообщениеОбОшибке);
		
		Если Не Ошибка Тогда
			УстановитьТарифСтраховыхВзносов(ДанныеТарифаСтраховыхВзносов, КодВозврата, СообщениеОбОшибке);
			Если КодВозврата <> ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаВыполнено() Тогда
				Ошибка = Истина;
			КонецЕсли;
		Иначе
			КодВозврата = ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаОшибкаДанных();
		КонецЕсли;
	ИначеЕсли НРег(Обработчик) = "usn_rate" Тогда
		
		ДанныеСтавокУСН = СтруктураИзПотокаJSON(ПотокДанных);
		
		ПроверитьКорректностьДанныхДляСтавокУСН(ДанныеСтавокУСН, Ошибка, СообщениеОбОшибке);
		
		Если Не Ошибка Тогда
			УстановитьСтавкуУСН(ДанныеСтавокУСН, КодВозврата, СообщениеОбОшибке);
			Если КодВозврата <> ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаВыполнено() Тогда
				Ошибка = Истина;
			КонецЕсли;
		Иначе
...
Процедура ПроверитьКорректностьДанныхДляТарифовСтраховыхВзносов(ДанныеТарифовСтраховыхВзносов, Ошибка, СообщениеОбОшибке)
	
	ПроверитьКорректностьДанныхДляСчетов(ДанныеТарифовСтраховыхВзносов, Ошибка, СообщениеОбОшибке);
	
	Если Не ДанныеТарифовСтраховыхВзносов.Свойство("date")
		Или Не ЗначениеЗаполнено(ДанныеТарифовСтраховыхВзносов.date) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле date'");
	КонецЕсли; 
	
	Если Не ДанныеТарифовСтраховыхВзносов.Свойство("tariff")
		Или Не ЗначениеЗаполнено(ДанныеТарифовСтраховыхВзносов.tariff) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле tariff'");
	КонецЕсли;

КонецПроцедуры 

Процедура ПроверитьКорректностьДанныхДляСтавокУСН(ДанныеСтавокУСН, Ошибка, СообщениеОбОшибке)
	
	ПроверитьКорректностьДанныхДляСчетов(ДанныеСтавокУСН, Ошибка, СообщениеОбОшибке);
	
	Если Не ДанныеСтавокУСН.Свойство("date")
		Или Не ЗначениеЗаполнено(ДанныеСтавокУСН.date) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле date'");
	КонецЕсли;
	
	Если Не ДанныеСтавокУСН.Свойство("rate") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле rate'");
	КонецЕсли;

	Если Не ДанныеСтавокУСН.Свойство("article") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле article'");
	КонецЕсли;

	Если Не ДанныеСтавокУСН.Свойство("paragraph") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле paragraph'");
	КонецЕсли;

	Если Не ДанныеСтавокУСН.Свойство("subparagraph") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле subparagraph'");
	КонецЕсли;
	
	Если Не ДанныеСтавокУСН.Свойство("tax_holiday") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле tax_holiday'");
	КонецЕсли;
...
Процедура СоздатьЗаполнитьОрганизацию(НастройкаИнтеграции, ДанныеОрганизации, КодВозврата, СообщениеОбОшибке)
	...
	ЗаполнитьТарифСтраховыхВзносов(ПараметрыСозданияОрганизации, ДанныеОрганизации);
	...
	Если Не ПроверитьРеквизитыДляДекларацииУСН(Организация, СообщениеОбОшибке, ВыводитьОписаниеВсехРеквизитов)
		...
		КодВозврата = ИнтеграцияОбъектовВСервисеБП.КодСостоянияВыполненоСПредупреждениями();
	Иначе
		КодВозврата = ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаВыполнено();
	КонецЕсли;
...
Процедура ЗаполнитьТарифСтраховыхВзносов(ПараметрыСозданияОрганизации, ДанныеОрганизации)
	
	Если ДанныеОрганизации.Свойство("insurance_tariff")
		И ЗначениеЗаполнено(ДанныеОрганизации.insurance_tariff) Тогда
		
		Тариф = Строка(ДанныеОрганизации.insurance_tariff);
		ТарифСуществует =
			Метаданные.Перечисления.ИПТарифыФиксированныхВзносов.ЗначенияПеречисления.Найти(Тариф) <> Неопределено;
		
		Если ТарифСуществует Тогда
			
			ПараметрыСозданияОрганизации.ТарифСтраховыхВзносов = 
				Перечисления.ИПТарифыФиксированныхВзносов[Тариф];
		КонецЕсли;
	КонецЕсли;
	...
	Если ДанныеОрганизации.Свойство("accounting_policy")
		...
		Если ДанныеОрганизации.accounting_policy.Свойство("tax_system")
			...
			Если НРег(СистемаНалогообложенияJSON) = "usn_profit" Тогда
				...
				Налогообложение.СведенияОСтавкеНалогаУСНДоходы = СведенияОСтавкеНалогаУСНДоходы(ДанныеОрганизации);
			ИначеЕсли НРег(СистемаНалогообложенияJSON) = "usn" Тогда
				Налогообложение.СистемаНалогообложения = "УпрощеннаяДоходыМинусРасходы";
			ИначеЕсли НРег(СистемаНалогообложенияJSON) = "ausn_profit" Тогда
				Налогообложение.СистемаНалогообложения = "АУСНДоходы";
			ИначеЕсли НРег(СистемаНалогообложенияJSON) = "ausn" Тогда
				Налогообложение.СистемаНалогообложения = "АУСНДоходыМинусРасходы";
...
Функция НалогообложениеПоУмолчанию()
	...
	Налогообложение.СистемаНалогообложения = "УпрощеннаяДоходы";
	Налогообложение.Вставить("СведенияОСтавкеНалогаУСНДоходы", НовыйСведенияОСтавкеНалогаУСНДоходыПоУмолчанию());
	
	Возврат Налогообложение;
	
КонецФункции

Функция СведенияОСтавкеНалогаУСНДоходы(ДанныеОрганизации)
	
	СведенияОСтавкеНалога = НовыйСведенияОСтавкеНалогаУСНДоходыПоУмолчанию();
	
	ДанныеОСтавкеНалога = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОрганизации, "usn_rate", Неопределено);
	
	Если ДанныеОСтавкеНалога <> Неопределено Тогда
		СведенияОСтавкеНалога.СтавкаНалога = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОСтавкеНалога, "rate", 6);
		
		СведенияОСтавкеНалога.НомерСтатьи = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОСтавкеНалога, "article", "");
		
		СведенияОСтавкеНалога.Пункт = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОСтавкеНалога, "paragraph", "");
		
		СведенияОСтавкеНалога.Подпункт = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОСтавкеНалога, "subparagraph", "");

		СведенияОСтавкеНалога.НалоговыеКаникулы = 
			ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДанныеОСтавкеНалога, "tax_holiday", Ложь);
	КонецЕсли;
	
	Возврат СведенияОСтавкеНалога;
	
КонецФункции

Функция НовыйСведенияОСтавкеНалогаУСНДоходыПоУмолчанию()
	
	СведенияОСтавкеНалога = Новый Структура;
	СведенияОСтавкеНалога.Вставить("СтавкаНалога", 6);
	СведенияОСтавкеНалога.Вставить("НомерСтатьи", "");
	СведенияОСтавкеНалога.Вставить("Пункт", "");
	СведенияОСтавкеНалога.Вставить("Подпункт", "");
	СведенияОСтавкеНалога.Вставить("НалоговыеКаникулы", Ложь);
	...
	Если ДанныеЗаполнения.Свойство("bank_accounts") И ЗначениеЗаполнено(ДанныеЗаполнения.bank_accounts) Тогда
		Для Каждого account Из ДанныеЗаполнения.bank_accounts Цикл
			...
			Если ЗначениеЗаполнено(НомерРасчетногоСчета) И ЗначениеЗаполнено(БИК) Тогда
				...
				БалансовыйСчет = БанковскиеПравила.БалансовыйСчет(НомерРасчетногоСчета);
				ЭтоСчетДепозитаБанка = БанковскиеПравила.ЭтоСчетДепозитаБанка(БалансовыйСчет);
				Если ЭтоСчетДепозитаБанка Тогда
					Если БанковскийСчетОбъект = Неопределено Тогда
						БанковскийСчетОбъект = БанковскийСчет.ПолучитьОбъект();
					КонецЕсли;
					БанковскийСчетОбъект.ВыпискаЗагружаетсяПоДепозитномуСчету = Истина;
				КонецЕсли;
...
Процедура УстановитьТарифСтраховыхВзносов(ДанныеТарифовСтраховыхВзносов, КодВозврата, СообщениеОбОшибке)
	
	ОрганизацияНеНайдена = Ложь;
	Организация = НайтиОрганизацию(ДанныеТарифовСтраховыхВзносов, ОрганизацияНеНайдена, СообщениеОбОшибке);

	Дата = XMLЗначение(Тип("Дата"), ДанныеТарифовСтраховыхВзносов.date);
	Тариф = ТарифИзПараметров(ДанныеТарифовСтраховыхВзносов, ОрганизацияНеНайдена, СообщениеОбОшибке);

	Если ОрганизацияНеНайдена Или Тариф = Неопределено Тогда
		КодВозврата = ИнтеграцияОбъектовОбластейДанныхСловарь.КодВозвратаОшибкаДанных(); 
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Запись.Период = Дата;
		Если ДанныеТарифовСтраховыхВзносов.Свойство("payment")
			И ЗначениеЗаполнено(ДанныеТарифовСтраховыхВзносов.payment) Тогда
			Запись.ПлательщикСтраховыхВзносовПФР_ФФОМС = ДанныеТарифовСтраховыхВзносов.payment;
		Иначе
...
Процедура УстановитьСтавкуУСН(ДанныеСтавокУСН, КодВозврата, СообщениеОбОшибке)
	
	ОрганизацияНеНайдена = Ложь;
	Организация = НайтиОрганизацию(ДанныеСтавокУСН, ОрганизацияНеНайдена, СообщениеОбОшибке);
	...
	Дата              = XMLЗначение(Тип("Дата"), ДанныеСтавокУСН.date);
	
	НомерСтатьи       = ДанныеСтавокУСН.article;
	Пункт             = ДанныеСтавокУСН.paragraph;
	Подпункт          = ДанныеСтавокУСН.subparagraph;
	НалоговыеКаникулы = ДанныеСтавокУСН.tax_holiday;
	
	Если НалоговыеКаникулы Тогда
		СтавкаНалога  = 0;
	Иначе
		СтавкаНалога  = ДанныеСтавокУСН.rate;
	КонецЕсли;
	...
	Попытка
		...
		Запись.ОснованиеЛьготнойСтавки = УчетУСНКлиентСервер.ПолныйКодОснованияЛьготы(НомерСтатьи, Пункт, Подпункт);
		Запись.НалоговыеКаникулы = НалоговыеКаникулы;
		Запись.Записать();
...
Процедура ВыделитьОстаткиПоСчетамДругихБанков(ОстаткиПоРасчетнымСчетам,
	...
	Для каждого БанковскийСчет Из Отбор.БанковскийСчетОрганизации Цикл
		...
		Если Не ВыделитьОстатокНаДатуНачала(ОстаткиПоРасчетнымСчетам, Остатки, ОтборПоСчету, ДатаНачалаВеденияУчета) Тогда
			// Если нет начального остатка на нужную дату,
...
Процедура ПроверитьКорректныеОстаткиПоСчетамВРежимеИнтеграции(ОстаткиПоРасчетнымСчетам, Ошибка, СообщениеОбОшибке)
	...
	Иначе
		...
		Если ДатыОстатков.Количество() = 1 Тогда
			...
			Если СтрокаОстатка.ДатаОстатка <= СтрокаОстатка.ДатаРегистрации Тогда
				Если СтрокаОстатка.СуммаОстатка = 0 Тогда
					// Дату "нулевых" остатков устанавливаем на месяц, предыдущий регистрации.
					СтрокаОстатка.ДатаОстатка = НачалоМесяца(СтрокаОстатка.ДатаРегистрации);
				...
				Иначе
					...
					СообщениеОбОшибке = СтрШаблон(
						НСтр("ru = 'Период начальных остатков не должен быть раньше даты регистрации организации %1'"),
						Формат(СтрокаОстатка.ДатаРегистрации, "ДФ=dd.MM.yyyy"));
					ОстаткиПоРасчетнымСчетам = Неопределено;
				КонецЕсли;
			ИначеЕсли СтрокаОстатка.СуммаОстатка = 0 Тогда
				// Дату "нулевых" остатков устанавливаем на месяц, предыдущий месяцу даты остатка.
				СтрокаОстатка.ДатаОстатка = НачалоМесяца(СтрокаОстатка.ДатаОстатка);
			КонецЕсли;
		Иначе
			Ошибка = Истина;
...
Процедура ВыполнитьКомандуУчетнойСистемы(УчетнаяСистема, ДанныеКоманды, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	...
	ИначеЕсли ИмяМетода = ИмяМетодаУведомлениеОбИсчисленныхСуммахНалогов() Тогда
		
		ПроверитьПараметрыКомандыСозданияУведомленияОбИсчисленныхСуммахНалогов(Параметры, Ошибка, СообщениеОбОшибке);
		Если Не Ошибка Тогда
			
			СоздатьУведомлениеОбИсчисленныхСуммахНалогов(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке);
			
		КонецЕсли;
		
	ИначеЕсли ИмяМетода = ИмяМетодаЗаявлениеОЗачетеВСчетПредстоящейОбязанности() Тогда
		
		ПроверитьПараметрыКомандыСозданияЗаявлениеОЗачетеВСчетПредстоящейОбязанности(Параметры, Ошибка, СообщениеОбОшибке);
		Если Не Ошибка Тогда
			
			СоздатьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке);
...
Процедура ВыполнитьКомандуСозданияРегламентированногоОтчета(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	...
	Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(Организация) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В информационной базе подключена 1С-Отчетность. Удаленное создание регламентированных отчетов не поддерживается.'");
		Возврат;
	КонецЕсли;
	...
	Если Параметры.type = "РегламентированныйОтчетУСН" Тогда
		...
		РегламентированныйОтчет = РегламентированнаяОтчетностьБП.СоздатьОбновитьРегламентированныйОтчет(Организация,
			Параметры.type, НачалоПериода, КонецПериода, Ошибки);
		
		Если Не ЗначениеЗаполнено(РегламентированныйОтчет) Тогда
			
			Если Ошибки.Количество() > 0 Тогда
				Ошибки.Вставить(0, СтрШаблон(НСтр("ru = 'Ошибка при создании регламентированного отчета %1:'"), Параметры.type));
				СообщениеОбОшибке = СтрСоединить(Ошибки, Символы.ПС);
			Иначе
		...
		УстановитьСтатусПодготовленоДляОтчета(РегламентированныйОтчет);
		
		Если Параметры.Свойство("format")
			И Параметры.format = "pdf" Тогда
			
			ЗаполнитьРезультатВыполненияКомандыВыгрузкиВPDF(
				УчетнаяСистема,
...
Процедура УстановитьСтатусПодготовленоДляОтчета(РегламентированныйОтчет)
...
Процедура УстановитьСтатусПодготовленоДляУведомления(РегламентированноеУведомление)
	
	Статус = РегламентированнаяОтчетность.СохраненныйСтатусОтправкиУведомления(РегламентированноеУведомление);
	
	Если Не ЗначениеЗаполнено(Статус)
		Или Статус = ИнтерфейсыВзаимодействияБРОКлиентСервер.СтатусВРаботеСтрокой() Тогда
		
		ИнтерфейсыВзаимодействияБРО.СохранитьСтатусОтправкиУведомления(
			РегламентированноеУведомление,
...
Функция АдресПечатногоБланкаДекларацииВФорматеPDF(Декларация, АдресПараметровПроксиВоВременномХранилище, ВозможноПолучитьМашиночитаемуюФорму)
	
	Перем АдресПечатногоБланка, ПараметрыСформированногоОтчета;
	
	Если ВозможноПолучитьМашиночитаемуюФорму
		И МашиночитаемаяФормаОтчетаСформирована(
			АдресПараметровПроксиВоВременномХранилище,
			ПараметрыСформированногоОтчета,
			Декларация) Тогда
		
		АдресПечатногоБланка = ПараметрыСформированногоОтчета.АдресОтчетаВоВременномХранилище;
		
	Иначе
		
		РезультатПолученияОбычнойФормыОтчетаВPDF =
			ИнтерфейсыВзаимодействияБРО.ПечатныйБланкРегламентированногоОтчета(Декларация);
		Если РезультатПолученияОбычнойФормыОтчетаВPDF = "" Тогда
			Возврат АдресПечатногоБланка;
		КонецЕсли;
		
		АдресПечатногоБланка = РезультатПолученияОбычнойФормыОтчетаВPDF.АдресПечатногоБланка;
...
Функция МашиночитаемаяФормаОтчетаСформирована(АдресПараметровПроксиВоВременномХранилище, ПараметрыСформированногоОтчета, РегламентированныйОтчет)
	
	РезультатВыгрузки = ИнтерфейсыВзаимодействияБРО.ВыгрузитьРегламентированныйОтчет(РегламентированныйОтчет);
	
	Если Не РезультатВыгрузки.РеглОтчетВыгружен
		Или Не ЭтоАдресВременногоХранилища(РезультатВыгрузки.АдресФайлаВыгрузки) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	ФайлВыгрузкиВоВременномХранилище.Вставить("АдресФайлаВыгрузкиВоВременномХранилище",
		АдресФайлаВыгрузкиВоВременномХранилище);
	
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		РегламентированныйОтчет,
		"ИсточникОтчета, ВыбраннаяФорма");
	ИмяФормыОтчета = РегламентированнаяОтчетностьВызовСервера.ПолныйПутьКФорме(ЗначенияРеквизитов.ИсточникОтчета,
		ЗначенияРеквизитов.ВыбраннаяФорма);
	
	ВывестиМашиночитаемуюФорму = РегламентированнаяОтчетностьВызовСервера.ВывестиМашиночитаемуюФормуЧерезВебСервис(
		ИмяФормыОтчета,
		Неопределено,
	...
	Если Не ВывестиМашиночитаемуюФорму Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПараметрыСформированногоОтчета.Вставить("АдресПараметровПроксиВоВременномХранилище",
		АдресПараметровПроксиВоВременномХранилище);
	
	Возврат МашиночитаемаяФормаПолученаСВебСервиса(ПараметрыСформированногоОтчета);
...
#Область Тариф
Функция ТарифИзПараметров(Параметры, Ошибка, СообщениеОбОшибке)

	Дата = XMLЗначение(Тип("Дата"), Параметры.date);
	ТарифСуществует =
		Метаданные.Перечисления.ИПТарифыФиксированныхВзносов.ЗначенияПеречисления.Найти(Параметры.tariff) <> Неопределено;
		
	Если ТарифСуществует Тогда
		Тариф = Перечисления.ИПТарифыФиксированныхВзносов[Параметры.tariff];
		ДатаНачалаПримененияТарифаВоенныйПенсионер = УчетСтраховыхВзносовИП.ДатаНачалаПримененияТарифаВоенныйПенсионер();
		Если Тариф = Перечисления.ИПТарифыФиксированныхВзносов.ВоенныйПенсионер 
				И Дата < ДатаНачалаПримененияТарифаВоенныйПенсионер Тогда
			Ошибка = Истина;
			СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Перейти на тариф %1 возможно не ранее %2'"),
				Тариф, Формат(ДатаНачалаПримененияТарифаВоенныйПенсионер, "ДФ='dd MMMM yyyy ""года""'"));
		Иначе
			Возврат Тариф;
		КонецЕсли;
	Иначе
		Ошибка = Истина;
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Тариф %1 не найден'"), Параметры.tariff);
	КонецЕсли;
...
Процедура ИзменитьСтатусОтчета(УчетнаяСистема, ИдентификаторОбъекта, ДанныеОтчета, Ошибка, СообщениеОбОшибке)
	...
	Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(Организация) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'Нельзя изменять статусы регламентированных отчетов, если подключена 1С-Отчетность.'");
		Возврат;
	КонецЕсли;
...
#Область УведомленияОбИсчисленныхСуммахНалогов

Функция НовыеПараметрыМетодаУведомленияОСуммахНалога()
	
	РеквизитыОрганизации = Новый Структура;
	РеквизитыОрганизации.Вставить("inn", "");
	РеквизитыОрганизации.Вставить("kpp", "");
	
	ПараметрыМетода = Новый Структура;
	ПараметрыМетода.Вставить("organization", РеквизитыОрганизации);
	ПараметрыМетода.Вставить("date", '00010101');
	ПараметрыМетода.Вставить("sum", 0);
	ПараметрыМетода.Вставить("kbk", "");
	
	Результат = Новый Структура;
	Результат.Вставить("method", "");
	Результат.Вставить("parameters", ПараметрыМетода);
	
	Возврат Результат;
	
КонецФункции

Функция ИмяМетодаУведомлениеОбИсчисленныхСуммахНалогов()
	
	Возврат "Документ/УведомлениеОбИсчисленныхСуммахНалогов/Создать";
	
КонецФункции

Процедура СоздатьУведомлениеОбИсчисленныхСуммахНалогов(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	...
	Если Не ОбщегоНазначенияБПВызовСервераПовтИсп.ЭтоЮрЛицо(Организация) Тогда
		ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоКБК(
			Параметры.kbk,
			Ложь,
			ПлатежиВБюджетКлиентСервер.СтатусНалогоплательщикаФизЛицо());
	Иначе
		ВидНалога = Справочники.ВидыНалоговИПлатежейВБюджет.НалогПоКБК(Параметры.kbk, Ложь);
	КонецЕсли;
	
	ПорядокОтчетаУплаты = Обработки.ПомощникРасчетаНалогаУСН.ПорядокОтчетаУплатыНалога(Организация, Параметры.date);
	УведомлениеСсылка = НайтиУведомлениеОбИсчисленныхСуммахНалога(Организация,
	...
	ПараметрыЗаполнения = Новый Структура("Дата, Организация, ВидОперации, Налоги");
	ПараметрыЗаполнения.Организация = Организация;
	ПараметрыЗаполнения.Дата        = ОбщегоНазначения.ТекущаяДатаПользователя();
	
	ДокументОбъект.Заполнить(ПараметрыЗаполнения);
	
	Если Не ЗначениеЗаполнено(ВидНалога) Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка при создании уведомления об исчисленных суммах налога:
		|По переданному КБК не найден вид налога'");
		Если Ошибки.Количество() > 0 Тогда
	...
	ПараметрыОперации = Обработки.ПомощникРасчетаНалогаУСН.НовыеПараметрыОперацииЕНС();
	ПараметрыОперации.Организация = Организация;
	ПараметрыОперации.Период = Параметры.date;
	ПараметрыОперации.Правило = ПорядокОтчетаУплаты.ПравилоУплаты;
	ПараметрыОперации.Налог = ВидНалога;
	ПараметрыОперации.Срок = ПорядокОтчетаУплаты.СрокОплаты;
	ПараметрыОперации.НалогКУплате = Параметры.sum;
	
	Налоги = Обработки.ПомощникРасчетаНалогаУСН.НалогиОперацииЕНС(ПараметрыОперации);
	ДокументОбъект.Налоги.Загрузить(Налоги);
	
	ВыполнениеЗадачБухгалтера.УстановитьСвойстваУведомленияПриРегистрации(
		ДокументОбъект,
		ПорядокОтчетаУплаты.ПравилоУплаты,
		Параметры.date);
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка при создании уведомления об исчисленных суммах налога:
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Уведомление) Тогда
		
		УстановитьСтатусПодготовленоДляУведомления(ДокументОбъект.Уведомление);
...
// Возвращает ссылку на сформированное уведомление об исчисленных суммах налога
//
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, для которой необходимо получить уведомление
//   СрокОплаты  - Дата - Срок оплаты налога
//   Налог       - СправочникСсылка.ВидыНалоговИПлатежейВБюджет - Вид налога
//
// Возвращаемое значение:
//   Неопределено, ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов
//
Функция НайтиУведомлениеОбИсчисленныхСуммахНалога(Организация, СрокОплаты, Налог) Экспорт
	...
	|ГДЕ
	|	НЕ УведомлениеОбИсчисленныхСуммахНалогов.ПометкаУдаления
	|	И УведомлениеОбИсчисленныхСуммахНалогов.Организация = &Организация
	|	И УведомлениеОбИсчисленныхСуммахНалогов.Налоги.Налог = &Налог
	|	И УведомлениеОбИсчисленныхСуммахНалогов.Налоги.СрокУплаты = &СрокУплаты");
	
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Налог", Налог);
	Запрос.УстановитьПараметр("СрокУплаты", СрокОплаты);
...
Процедура ИзменитьСтатусУведомления(УчетнаяСистема, ИдентификаторОбъекта, ДанныеОтчета, Ошибка, СообщениеОбОшибке)
	...
	ТипОбъекта = ТипЗнч(СсылкаНаОбъект);
	ТипРегламентированноеУведомление = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения");
	Если ТипОбъекта <> ТипРегламентированноеУведомление Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'По идентификатору %1 найден объект типа %2. Ожидается тип %3.'"),
			ИдентификаторОбъекта,
			ТипОбъекта,
			ТипРегламентированноеУведомление);
		Возврат;
	КонецЕсли;
	...
	Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(Организация) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'Нельзя изменять статусы регламентированных уведомлений, если подключена 1С-Отчетность.'");
		Возврат;
	КонецЕсли;
	
	ТекущийСтатус = РегламентированнаяОтчетность.СохраненныйСтатусОтправкиУведомления(СсылкаНаОбъект);
	НовыйСтатус = ДанныеОтчета.status;
	...
	Если Не ЗначениеЗаполнено(ТекущийСтатус) Или ТекущийСтатус <> НовыйСтатус Тогда
		...
		Попытка
			ИнтерфейсыВзаимодействияБРО.СохранитьСтатусОтправкиУведомления(СсылкаНаОбъект, НовыйСтатус);
		Исключение
			ЗаписьЖурналаРегистрации(ИнтеграцияСБанкамиКлиентСервер.ИмяСобытияЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				УчетнаяСистема.Метаданные(),
				УчетнаяСистема,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение;
		КонецПопытки
...
Процедура ПроверитьПараметрыКомандыСозданияУведомленияОбИсчисленныхСуммахНалогов(Параметры, Ошибка, СообщениеОбОшибке)
	
	ПроверитьПараметрыКомандыСозданияРегламентированногоУведомления(Параметры, Ошибка, СообщениеОбОшибке);
	
	Если Не Параметры.Свойство("sum") Или Не ЗначениеЗаполнено(Параметры.sum) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле sum'");
		Возврат;
	КонецЕсли;
	
	Если Не Параметры.Свойство("kbk") Или Не ЗначениеЗаполнено(Параметры.kbk) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле kbk'");
		Возврат;
	КонецЕсли;
	
КонецПроцедуры

Функция АдресПечатногоБланкаУведомленияВФорматеPDF(Уведомление, АдресПараметровПроксиВоВременномХранилище, ВозможноПолучитьМашиночитаемуюФорму)
	
	Перем АдресПечатногоБланка, ПараметрыСформированногоУведомления;
	
	Если ВозможноПолучитьМашиночитаемуюФорму
		И МашиночитаемаяФормаУведомленияСформирована(
			АдресПараметровПроксиВоВременномХранилище,
			ПараметрыСформированногоУведомления,
			Уведомление) Тогда
		
		АдресПечатногоБланка = ПараметрыСформированногоУведомления.АдресОтчетаВоВременномХранилище;
		
	Иначе
		АдресПечатногоБланка = АдресПечатногоБланкаУведомленияВФорматеPDFНеМЧД(Уведомление);
	КонецЕсли;
...
Функция МашиночитаемаяФормаУведомленияСформирована(АдресПараметровПроксиВоВременномХранилище, ПараметрыСформированногоУведомления, Уведомление)
	
	УведомлениеОбъект = Уведомление.ПолучитьОбъект();
	СтруктураВыгрузки = Отчеты[УведомлениеОбъект.ИмяОтчета].СформироватьВыгрузкуИПолучитьДанные(УведомлениеОбъект);
	Если Не ЗначениеЗаполнено(СтруктураВыгрузки) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	АдресФайлаВыгрузкиВоВременномХранилище = РегламентированнаяОтчетностьВызовСервера.ПоместитьФайлВыгрузкиВоВременноеХранилищеНаСервере(
		СтруктураВыгрузки.ТестВыгрузки,
		Строка(Новый УникальныйИдентификатор),
		СтруктураВыгрузки.КодировкаВыгрузки);
	
	ФайлВыгрузкиВоВременномХранилище = Новый Структура;
	ФайлВыгрузкиВоВременномХранилище.Вставить("КодировкаФайлаВыгрузки", СтруктураВыгрузки.КодировкаВыгрузки);
	ФайлВыгрузкиВоВременномХранилище.Вставить("ИмяФайлаВыгрузки", СтруктураВыгрузки.ИмяФайла);
	ФайлВыгрузкиВоВременномХранилище.Вставить("АдресФайлаВыгрузкиВоВременномХранилище",
		АдресФайлаВыгрузкиВоВременномХранилище);
	
	ВывестиМашиночитаемуюФорму = РегламентированнаяОтчетностьВызовСервера.ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахЧерезВебСервис(
		ФайлВыгрузкиВоВременномХранилище,
		ПараметрыСформированногоУведомления,
		АдресПараметровПроксиВоВременномХранилище,
		СтруктураВыгрузки.Данные);
	
	Если Не ВывестиМашиночитаемуюФорму Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ПараметрыСформированногоУведомления.Вставить("АдресПараметровПроксиВоВременномХранилище",
		АдресПараметровПроксиВоВременномХранилище);
	
	Возврат МашиночитаемаяФормаПолученаСВебСервиса(ПараметрыСформированногоУведомления);
	
КонецФункции

Функция АдресПечатногоБланкаУведомленияВФорматеPDFНеМЧД(Уведомление)
	
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("pdf");
	ПакетОтображаемыхДокументов = Новый ПакетОтображаемыхДокументов;
	СписокПечатаемыхЛистов = УведомлениеОСпецрежимахНалогообложенияВызовСервера.ПечатьУведомленияБРО(Уведомление);
		
	Для Каждого Лист Из СписокПечатаемыхЛистов Цикл
		
		АдресВоВременномХранилище = Лист.Значение[0];
		ПакетОтображаемыхДокументов.Состав.Добавить(АдресВоВременномХранилище);
...
#Область ЗаявлениеОЗачетеВСчетПредстоящейОбязанности

Функция ИмяМетодаЗаявлениеОЗачетеВСчетПредстоящейОбязанности()
	
	Возврат "Документ/ЗаявлениеОЗачетеВСчетПредстоящейОбязанности/Создать";
	
КонецФункции

Процедура СоздатьЗаявлениеОЗачетеВСчетПредстоящейОбязанности(УчетнаяСистема, Параметры, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	...
	ПериодСобытия = НачалоДня(КонецКвартала(Параметры.date));
	
	ПорядокОтчетаУплаты = Обработки.РасчетСтраховыхВзносовИП.ПорядокУплатыСтраховыхВзносов(Организация, ПериодСобытия);
	
	ЗаявлениеСсылка = НайтиЗаявлениеОЗачетеВСчетПредстоящейОбязанности(
		Организация,
		ПериодСобытия,
		ПорядокОтчетаУплаты.Правило);
	
	Если ЗначениеЗаполнено(ЗаявлениеСсылка) Тогда
		ДокументОбъект = ЗаявлениеСсылка.ПолучитьОбъект();
	Иначе
		ДокументОбъект = Документы.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности.СоздатьДокумент();
	КонецЕсли;
	
	ПараметрыЗаполнения = Новый Структура("Дата, Организация");
	ПараметрыЗаполнения.Организация = Организация;
	ПараметрыЗаполнения.Дата        = Параметры.date; // На дату документа проверяется доступный остаток по ЕНС
	ДокументОбъект.Дата             = Параметры.date;
	
	ДокументОбъект.Заполнить(ПараметрыЗаполнения);
	
	ПараметрыНалогов = Обработки.РасчетСтраховыхВзносовИП.НовыеПоказателиРасчетаВзносовЗаСебя();
	ПараметрыНалогов.Вставить("Организация", Организация);
	ПараметрыНалогов.Вставить("Период", Параметры.date);
	ПараметрыНалогов.СуммаВзносаЕдиныйТариф = Параметры.sum_flat;
	ПараметрыНалогов.СуммаВзносаПФРсДоходов = Параметры.sum_float;
	
	АдресТаблицыНалоги = АсинхронноеПолучениеДанныхБППлатежиВБюджет.ОписаниеТаблицыНалоги(
		Организация,
		ПорядокОтчетаУплаты.Правило,
		ПериодСобытия,
		ПорядокОтчетаУплаты.Срок,
		ПараметрыНалогов);
	
	Если ЭтоАдресВременногоХранилища(АдресТаблицыНалоги) Тогда
		
		ТаблицыЗаявления = ПолучитьИзВременногоХранилища(АдресТаблицыНалоги);
		ДокументОбъект.Налоги.Загрузить(ТаблицыЗаявления.ТаблицаНалоги);
		УдалитьИзВременногоХранилища(АдресТаблицыНалоги);
		
	Иначе
		
		ТекстОшибки = НСтр("ru = 'Ошибка при создании заявления о зачете в счет предстоящей обязанности:
			|не удалось заполнить таблицу Налоги.'");
		Если Ошибки.Количество() > 0 Тогда
			Ошибки.Вставить(0, ТекстОшибки);
			СообщениеОбОшибке = СтрСоединить(Ошибки, Символы.ПС);
		Иначе
	...
	РегистрацияВНалоговомОргане = Справочники.Организации.РегистрацияВНалоговомОрганеНаДату(Организация, Параметры.date);
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Ошибка при создании заявления о зачете в счет предстоящей обязанности:
			|%1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Если Ошибки.Количество() > 0 Тогда
			Ошибки.Вставить(0, ТекстОшибки);
			СообщениеОбОшибке = СтрСоединить(Ошибки, Символы.ПС);
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru = 'Ошибка при выгрузке заявления о зачете в счет предстоящей обязанности'");
		Если Ошибки.Количество() > 0 Тогда
			Ошибки.Вставить(0, ТекстОшибки);
			СообщениеОбОшибке = СтрСоединить(Ошибки, Символы.ПС);
		Иначе
...
// Возвращает ссылку на сформированное заявление о зачете
//
// Параметры:
//   Организация - СправочникСсылка.Организации - Организация, для которой необходимо получить уведомление
//   СрокОплаты  - Дата - Срок оплаты налога
...
//   Неопределено, ДокументСсылка.УведомлениеОбИсчисленныхСуммахНалогов
//
Функция НайтиЗаявлениеОЗачетеВСчетПредстоящейОбязанности(Организация, Период, Правило)
	
	МассивПравил = Новый Массив;
	МассивПравил.Добавить(Правило);
	МассивПравил.Добавить(ВыполнениеЗадачБухгалтера.ПравилоУведомленияПоПравилуУплаты(Правило, Организация, Период));
	...
	Если ТаблицаУведомления.Количество() = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ДокументСсылка = ТаблицаУведомления[0].Уведомление;
	
	Если ТипЗнч(ДокументСсылка) <> Тип("ДокументСсылка.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция НовыеПараметрыЗаявлениеОЗачете()
	...
	ПараметрыМетода.Вставить("organization", РеквизитыОрганизации);
	ПараметрыМетода.Вставить("date", '00010101');
	ПараметрыМетода.Вставить("sum_flat", 0);
	ПараметрыМетода.Вставить("sum_float", 0);
...
Процедура ПроверитьПараметрыКомандыСозданияЗаявлениеОЗачетеВСчетПредстоящейОбязанности(Параметры, Ошибка, СообщениеОбОшибке)
	
	ПроверитьПараметрыКомандыСозданияРегламентированногоУведомления(Параметры, Ошибка, СообщениеОбОшибке);
	
	Если Не Параметры.Свойство("sum_flat") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует обязательное поле sum_flat'");
		Возврат;
	КонецЕсли;
	
	Если Не Параметры.Свойство("sum_float") Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует обязательное поле sum_float'");
		Возврат;
	КонецЕсли;
...
Функция ЭтоОбработчикСменыСтатусаУведомления(ИмяОбработчика)
	
	ОбработчикиСтатусаУведомления = Новый Массив;
	ОбработчикиСтатусаУведомления.Добавить("tax_notification_status");
	ОбработчикиСтатусаУведомления.Добавить("insurance_statement_status");
	
	Возврат ОбработчикиСтатусаУведомления.Найти(НРег(ИмяОбработчика)) <> Неопределено;
	
КонецФункции

Процедура ПроверитьПараметрыКомандыСозданияРегламентированногоУведомления(Параметры, Ошибка, СообщениеОбОшибке)
	
	Если Не Параметры.Свойство("date") Или Не ЗначениеЗаполнено(Параметры.date) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'В структуре parameters отсутствует или не заполнено обязательное поле date'");
		Возврат;
	КонецЕсли;
	...
	Если Параметры.Свойство("organization") Тогда
		...
		Если СтрДлина(СвойстваОрганизации.inn) = 10 Тогда
			Если Не СвойстваОрганизации.Свойство("kpp")
				Или Не ЗначениеЗаполнено(СвойстваОрганизации.kpp)
				Или СтрДлина(СвойстваОрганизации.kpp) <> 9 Тогда
...
Процедура ЗаписатьОшибкуВЖурналРегистрации(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки = Неопределено)
...
Процедура ЗаполнитьРезультатВыполненияКомандыВыгрузкиВPDF(УчетнаяСистема, Документ, РезультатВыполнения, Ошибка, СообщениеОбОшибке)
	
	АдресПечатногоБланка = АдресПечатногоБланкаВФорматеPDF(Документ);
	Если Не ЗначениеЗаполнено(АдресПечатногоБланка) Тогда
		Ошибка = Истина;
		СообщениеОбОшибке = НСтр("ru = 'Не удалось сформировать печатную форму в формате PDF.'");
		Возврат;
	КонецЕсли;
	
	ДвоичныеДанныеДокумента = ПолучитьИзВременногоХранилища(АдресПечатногоБланка);
	Идентификатор = Документ.УникальныйИдентификатор();
	ИмяОбработчика = ИмяОбработчикаВыгрузкиВФорматеPDF(Документ);
	...
	Результат = РезультатВыполнения.Добавить();
	Результат.ИдентификаторОбъекта = Идентификатор;
	Результат.Обработчик = ИмяОбработчика;
	
КонецПроцедуры

Функция АдресПечатногоБланкаВФорматеPDF(Документ)
	
	Перем АдресПараметровПроксиВоВременномХранилище, АдресПечатногоБланка;
	
	ВозможноПолучитьМашиночитаемуюФорму = РегламентированнаяОтчетностьВызовСервера.ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(
		АдресПараметровПроксиВоВременномХранилище);
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		АдресПечатногоБланка = АдресПечатногоБланкаДекларацииВФорматеPDF(Документ,
			АдресПараметровПроксиВоВременномХранилище,
			ВозможноПолучитьМашиночитаемуюФорму);
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		АдресПечатногоБланка = АдресПечатногоБланкаУведомленияВФорматеPDF(Документ,
			АдресПараметровПроксиВоВременномХранилище,
			ВозможноПолучитьМашиночитаемуюФорму);
	КонецЕсли;
...
Функция ИмяОбработчикаВыгрузкиВФорматеPDF(Документ)
	
	ИмяОбработчика = "";
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		ИмяОбработчика = "tax_return_1c_pdf";
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		ИмяОбработчика = "notification_return_1c_pdf";
	КонецЕсли;
	
	Возврат ИмяОбработчика;
