...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьВидыРегистровУчета");
	Методы.Вставить("ОбновитьВидыРегистровУчета");
...
Процедура ЗаполнитьВидыРегистровУчета() Экспорт
	...
	ВидРегистра = Справочники.ВидыРегистровУчета.СправкаРасчетАмортизационнойПремии.ПолучитьОбъект();
	ВидРегистра.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.СправкаРасчетАмортизационнойПремии);
	ВидРегистра.ВариантОтчета = "ВариантНастройкиАмортизационнаяПремия";
	ВидРегистра.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидРегистра.Отчет, "Синоним");
	ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидРегистра, Истина);
	
	ВидРегистра = Справочники.ВидыРегистровУчета.СправкаРасчетПризнаниеРасходовПоОСПоступившимВАренду.ПолучитьОбъект();
	ВидРегистра.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.СправкаРасчетПризнаниеРасходовПоОСПоступившимВАренду);
	ВидРегистра.ВариантОтчета = "ПризнаниеРасходовПоОСПоступившимВАренду";
	ВидРегистра.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидРегистра.Отчет, "Синоним");
	ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидРегистра, Истина);
...
// Меняет существующие или дополняет виды регистров учета при обновлении конфигурации.
//
Процедура ОбновитьВидыРегистровУчета() Экспорт
	
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	// Новая справка-расчет земельного налога: обновляем ссылку на объект метаданных
	ВидРегистра = СправкаРасчетЗемельногоНалога.ПолучитьОбъект();
	Если ВидРегистра <> Неопределено Тогда
		ВидРегистра.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.СправкаРасчетЗемельногоНалога);
		ВидРегистра.ВариантОтчета = "РасчетЗемельногоНалога";
		ВидРегистра.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидРегистра.Отчет, "Синоним");
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидРегистра, Истина);
	...
	// Прежняя (устаревшая) справка-расчет земельного налога
	ВидРегистра = СправкаРасчетЗемельногоНалогаДо2020.ПолучитьОбъект();
	Если ВидРегистра <> Неопределено Тогда
		ВидРегистра.Отчет = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Метаданные.Отчеты.СправкаРасчетЗемельногоНалогаДо2020);
		ВидРегистра.ВариантОтчета = "РасчетЗемельногоНалога";
		ВидРегистра.Наименование = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидРегистра.Отчет, "Синоним");
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ВидРегистра, Истина);
	КонецЕсли;
