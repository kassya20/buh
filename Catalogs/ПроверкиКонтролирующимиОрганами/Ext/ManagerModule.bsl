...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьРеквизитыСервисаПроверокПриОбновлении");
	Методы.Вставить("УстановитьВремяОбновленияПроверок");
...
Процедура ОбновитьПроверки() Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РеквизитыСервиса.Организация КАК Организация,
	|	РеквизитыСервиса.ИНН КАК ИНН,
	|	РеквизитыСервиса.ИНН КАК Код,
	|	РеквизитыСервиса.ОбластьДанныхВспомогательныеДанные КАК ОбластьДанныхВспомогательныеДанные
	|ИЗ
	...
	Если ОбщегоНазначенияБП.ИспользуетсяАрхив() Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПоОрганизации", "НЕ РеквизитыСервиса.Организация.ВАрхиве");
	Иначе
...
Процедура ОбновитьПроверкиПоТаблицеОрганизаций(ТаблицаРеквизиты, СоздаватьРегламентноеЗадание = Ложь) Экспорт
	...
	ПараметрыПроверки = НовыйПараметрыПроверки(ТаблицаРеквизиты);
	
	ПолучитьСписокОбластейДанныхИИнформациюОСтарыхПроверках(ПараметрыПроверки, ТаблицаРеквизиты);
	
	Для каждого СтрокаОбласти Из ПараметрыПроверки.ОбластиДанных Цикл
		Если РазделениеВключено Тогда
			РаботаВМоделиСервиса.УстановитьРазделениеСеанса(Истина, СтрокаОбласти.ОбластьДанных);
			Если Не ПодключенаУслугаИнформацияОПроверкахКонтролирующихОрганов() Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаРеквизитыОбласти Цикл
			...
			Если ИННКорректен(ИНН) И ПараметрыПроверки.ДанныеОПроверках[ИНН] = Неопределено Тогда
				ЗаполнитьЗначенияСвойств(ПараметрыПроверки.ТаблицаИННДляПроверки.Добавить(), СтрокаТаблицы);
			КонецЕсли;
			
			// Получим данные порциями по 100 ИНН.
			Если ПараметрыПроверки.ТаблицаИННДляПроверки.Количество() = 100
				Или НомерСтроки = ТаблицаРеквизитыОбласти.Количество() Тогда
				ЗапроситьИнформациюОПроверках(ПараметрыПроверки);
			КонецЕсли;
		КонецЦикла;
	...
	Если СоздаватьРегламентноеЗадание И ПараметрыПроверки.ТаблицаОжиданиеОтвета.Количество() > 0 Тогда
		СоздатьРегламентноеЗаданиеДляОжиданияОтвета(ПараметрыПроверки.ТаблицаОжиданиеОтвета);
	КонецЕсли;
...
// Устанавливает случайное время (в пределах от 00:00 до 06:00) для запуска 
...
Процедура УстановитьВремяОбновленияПроверок() Экспорт
	...
	ЧислоЧасов = ГенераторЧисел.СлучайноеЧисло(0, 5);
	ЧислоМинут = ГенераторЧисел.СлучайноеЧисло(0, 59);
	ЧислоСекунд = ГенераторЧисел.СлучайноеЧисло(0, 59);
	Задание.Расписание.ВремяНачала = Дата(1, 1, 1, ЧислоЧасов, ЧислоМинут, ЧислоСекунд);
	Задание.Записать();
...
Функция ИННКорректен(ИНН, ОписаниеОшибки = "")

	Если ЗначениеЗаполнено(ИНН) Тогда 
		ЮридическоеЛицо = СтрДлина(ИНН) = 10;
		
		Возврат РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(ИНН,
			ЮридическоеЛицо,
			ОписаниеОшибки);
	Иначе
...
Процедура СоздатьРегламентноеЗаданиеДляОжиданияОтвета(ТаблицаОжиданиеОтвета)
...
Процедура ЗапроситьИнформациюОПроверках(ПараметрыПроверки)

	ИнформацияОПроверках = РаботаСКонтрагентами.ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИНН(
		ПараметрыПроверки.ТаблицаИННДляПроверки.ВыгрузитьКолонку("ИНН"), Истина, Истина, Ложь, Истина); // Незавершенные проверки
		
	Если ЗначениеЗаполнено(ИнформацияОПроверках.ОписаниеОшибки) Тогда
		ПараметрыПроверки.ТаблицаИННДляПроверки.Очистить();
		Возврат;
	ИначеЕсли ИнформацияОПроверках.ОжиданиеОтвета Тогда
		Для Каждого СтрокаИННДляПроверки Из ПараметрыПроверки.ТаблицаИННДляПроверки Цикл
			ЗаполнитьЗначенияСвойств(ПараметрыПроверки.ТаблицаОжиданиеОтвета.Добавить(), СтрокаИННДляПроверки);
		КонецЦикла;
		ПараметрыПроверки.ТаблицаИННДляПроверки.Очистить();
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаИННДляПроверки Из ПараметрыПроверки.ТаблицаИННДляПроверки Цикл
		ДанныеОПроверкахПоИНН = ИнформацияОПроверках.Проверки[СтрокаИННДляПроверки.ИНН];
		ПараметрыПроверки.ДанныеОПроверках.Вставить(СтрокаИННДляПроверки.ИНН, ДанныеОПроверкахПоИНН);
		
		ЗаполнитьЗначенияСвойств(ОтборПроверкиПоИдентификатору, СтрокаИННДляПроверки);
		Для каждого СтрокаПроверки Из ДанныеОПроверкахПоИНН Цикл
			ЭтоПроверкаПрошлогоПериода = СтрокаПроверки.ПериодПроверки < ПараметрыПроверки.НачалоТекущегоМесяца;
			Если ЭтоПроверкаПрошлогоПериода Тогда
				Продолжить;
			КонецЕсли;
			ОтборПроверкиПоИдентификатору.Идентификатор = СтрокаПроверки.Идентификатор;
			НайденныеПроверки = ПараметрыПроверки.СтарыеПроверки.НайтиСтроки(ОтборПроверкиПоИдентификатору);
			...
			Если ЭтоНоваяПроверка Тогда
				...
				ЗаполнитьЗначенияСвойств(НоваяПроверка, СтрокаИННДляПроверки);
				НоваяПроверка.Наименование = КраткоеНаименованиеПроверки(НоваяПроверка.КонтролирующийОрган, ПараметрыПроверки.НаименованияОргановИСлова);
				НоваяПроверка.Записать();
				// Новая запись регистра
				МенеджерЗаписи = РегистрыСведений.НовыеПроверкиКонтролирующимиОрганами.СоздатьМенеджерЗаписи();
				ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СтрокаИННДляПроверки);
...
Функция НовыйПараметрыПроверки(ТаблицаРеквизиты)

	Результат = Новый Структура;
	Результат.Вставить("НаименованияОргановИСлова", НаименованияОргановИСлова());
	Результат.Вставить("ТаблицаИННДляПроверки"    , ТаблицаРеквизиты.СкопироватьКолонки());
	Результат.Вставить("ТаблицаОжиданиеОтвета"    , ТаблицаРеквизиты.СкопироватьКолонки());
	Результат.Вставить("ДанныеОПроверках"         , Новый Соответствие);
	Результат.Вставить("СтарыеПроверки"           , Новый ТаблицаЗначений);
	Результат.Вставить("ОбластиДанных"            , Новый ТаблицаЗначений);
	Результат.Вставить("НачалоТекущегоМесяца"     , НачалоМесяца(ТекущаяДатаСеанса()));
	
	Возврат Результат;

КонецФункции

Функция ПодключенаУслугаИнформацияОПроверкахКонтролирующихОрганов()

	Возврат ИнтернетПоддержкаПользователей.УслугаПодключена("1c-counteragent-organization-audit");

КонецФункции

Процедура ПолучитьСписокОбластейДанныхИИнформациюОСтарыхПроверках(ПараметрыПроверки, ТаблицаРеквизиты)
	...
	Запрос.УстановитьПараметр("НачалоТекущегоМесяца", ПараметрыПроверки.НачалоТекущегоМесяца);
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	ПараметрыПроверки.СтарыеПроверки = РезультатыЗапроса[2].Выгрузить();
	ПараметрыПроверки.СтарыеПроверки.Индексы.Добавить("ИНН,ОбластьДанныхВспомогательныеДанные,Идентификатор");
	ПараметрыПроверки.ОбластиДанных = РезультатыЗапроса[3].Выгрузить();
