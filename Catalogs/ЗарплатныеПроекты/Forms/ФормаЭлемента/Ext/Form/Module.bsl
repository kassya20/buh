...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьВидимостьПризнакаПеречисленийНаКартыМир();
	УстановитьДоступностьЭлементовПриИзмененииПризнакаПеречисленийНаКартыМир(ЭтотОбъект);
...
&НаКлиенте
Процедура ДляПеречисленийНаКартыМирПриИзменении(Элемент)
	УстановитьДоступностьЭлементовПриИзмененииПризнакаПеречисленийНаКартыМир(ЭтотОбъект);
КонецПроцедуры
...
&НаКлиенте
Процедура НастройкаВидовЗачисленийНажатие(Элемент)
	
	Обработчик = Новый ОписаниеОповещения("ПослеНастройкиВидовЗачислений", ЭтотОбъект);
	
	ОткрытьФорму("Справочник.ЗарплатныеПроекты.Форма.ФормаНастройкиВидовЗачисления",
		ПараметрыФормыНастройкиВидовЗачислений(),
		ЭтотОбъект,
...
&НаСервере
Функция ПараметрыФормыНастройкиВидовЗачислений()
	
	АдресТаблицыНастройкиВидовЗачислений = ПоместитьВоВременноеХранилище(, Новый УникальныйИдентификатор);
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЗарплатныйПроект",
		Объект.Ссылка);
	ПараметрыФормы.Вставить("АдресТаблицыНастройкиВидовЗачислений",
		ПоместитьВоВременноеХранилище(Объект.ВидыЗачисленийПоСпособамВыплат.Выгрузить(), АдресТаблицыНастройкиВидовЗачислений));
...
&НаКлиенте
Процедура ПослеНастройкиВидовЗачислений(Результат, Параметры) Экспорт
	
	Если Результат <> КодВозвратаДиалога.ОК Тогда
		Возврат;
	КонецЕсли;
	
	ПослеНастройкиВидовЗачисленийНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ПослеНастройкиВидовЗачисленийНаСервере()
	
	ТаблицаЗначений = ПолучитьИзВременногоХранилища(АдресТаблицыНастройкиВидовЗачислений);
	Объект.ВидыЗачисленийПоСпособамВыплат.Загрузить(ТаблицаЗначений);
...
Процедура НастроитьПолеНомерДоговора(Форма)
	...
	Иначе
		...
		Подсказка = НСтр("ru = 'Допускается длина номера договора не более 40 символов.'");
	КонецЕсли;
...
&НаСервере
Процедура НастроитьСписокФорматовФайла()
	
	Элементы.ФорматФайла.РежимВыбораИзСписка = Истина;
	Элементы.ФорматФайла.СписокВыбора.ЗагрузитьЗначения(Перечисления.ФорматыФайловОбменаПоЗарплатномуПроекту.АктуальныеФорматы())
...
Процедура УстановитьДоступностьЭлементовЭДО(Форма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "НастройкаВидовЗачислений", "Видимость",
...
&НаСервере
Процедура УстановитьВидимостьПризнакаПеречисленийНаКартыМир()
	
	ВидимостьПризнакаПеречисленийНаКартыМир = Ложь;
	
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		ВидимостьПризнакаПеречисленийНаКартыМир = Истина;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
		"ДляПеречисленийНаКартыМир", "Видимость", ВидимостьПризнакаПеречисленийНаКартыМир);
...
&НаСервере
Функция БанкЗарплатногоПроекта()
	
	Банк = Неопределено;
	ОбменСБанкамиПоЗарплатнымПроектамПереопределяемый.ОпределитьБанкЗарплатногоПроекта(Объект.Ссылка, Банк);
	Возврат Банк;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьЭлементовПриИзмененииПризнакаПеречисленийНаКартыМир(Форма)
	
	ЗарплатныйПроектСБанком = Не Форма.Объект.ДляПеречисленийНаКартыМир;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "Банк", "Видимость", ЗарплатныйПроектСБанком);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ИспользоватьЭДО", "Видимость", ЗарплатныйПроектСБанком);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "ЭлектронныйДокументооборотСБанком", "Видимость", ЗарплатныйПроектСБанком);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы, "НастройкаВидовЗачисленийГруппа", "Видимость", ЗарплатныйПроектСБанком);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.КоманднаяПанель.ПодчиненныеЭлементы, 
		"ФормаРегистрСведенийНастройкиПрямыхВыплатФССОрганизацийИспользоватьПроектДляПрямыхВыплатФСС",
		"Видимость", ЗарплатныйПроектСБанком);
