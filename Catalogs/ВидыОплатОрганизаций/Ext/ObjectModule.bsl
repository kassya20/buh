...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоГруппа Тогда
		...
		Если ТипОплаты <> Перечисления.ТипыОплат.ПлатежнаяКарта И ТипОплаты <> Перечисления.ТипыОплат.СБП Тогда
...
Функция ИзменениеКлючевыхРеквизитовЗапрещено(РеквизитыСсылки)
	...
	Если Контрагент <> РеквизитыСсылки.Контрагент
		...
		Запрос.УстановитьПараметр("ИзменилсяТипОплаты", ТипОплаты <> РеквизитыСсылки.ТипОплаты);
		
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ДокументыПоВидуОплаты.Ссылка КАК Ссылка
		|ИЗ
		...
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	СоответствиеНастроекИнтеграции.ТорговаяТочка
		|ИЗ
		|	РегистрСведений.СоответствиеНастроекИнтеграции КАК СоответствиеНастроекИнтеграции
		|ГДЕ
		|	СоответствиеНастроекИнтеграции.ВидОплаты = &Элемент
		|	И &ИзменилсяТипОплаты
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	СоответствиеНастроекИнтеграцииСБПb2b.ТорговаяТочка
		|ИЗ
		|	РегистрСведений.СоответствиеНастроекИнтеграцииСБПb2b КАК СоответствиеНастроекИнтеграцииСБПb2b
		|ГДЕ
		|	СоответствиеНастроекИнтеграцииСБПb2b.ВидОплаты = &Элемент
		|	И &ИзменилсяТипОплаты";
