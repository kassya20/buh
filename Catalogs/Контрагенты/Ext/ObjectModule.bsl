...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЭтоГруппа Тогда
		...
		ИННСоответствуетТребованиям = ПроверкаИНН.СоответствуетТребованиям;
		Если Не ИННСоответствуетТребованиям Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Проверка реквизитов. ИНН не соответствует требованиям ФНС'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Информация,
				Метаданные.Справочники.Контрагенты,
				,
				ПроверкаИНН.ОписаниеОшибки);
		КонецЕсли;
			
		
		Если ИННСоответствуетТребованиям И ЗначениеЗаполнено(КПП) И НЕ КППВведенКорректно Тогда
		...
		// Проверяем заполнение КПП только в паре с ИНН
		Если ИННСоответствуетТребованиям И ЗначениеЗаполнено(ИНН) И ЭтоЮрЛицо 
...
Процедура ПередЗаписью(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("ДанныеКонтактногоЛица")
		И ТипЗнч(ДополнительныеСвойства.ДанныеКонтактногоЛица) = Тип("Структура")
		И ОсновноеКонтактноеЛицо.Пустая() Тогда
...
Процедура ОбработкаЗаполненияПоОрганизации(Организация)
	...
	СтатусФизическогоЛица = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "СтатусФизическогоЛица");
	...
	Если СтатусФизическогоЛица = Перечисления.СтатусыФизическихЛиц.ИндивидуальныйПредприниматель Тогда
		...
		РегистрационныйНомер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "ОГРН");
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Организация, "ДатаРегистрации, СвидетельствоДатаВыдачи, СвидетельствоСерияНомер");
	КонецЕсли;
...
Процедура ОбработкаЗаполненияПоСтруктуре(ДанныеЗаполнения)
	...
	Если Не ДанныеЗаполнения.Свойство("Наименование") 
		...
		ДанныеЗаполнения.Вставить("Наименование",
			Справочники.Контрагенты.ПолучитьКраткоеНаименованиеКонтрагента(ДанныеЗаполнения.НаименованиеПолное));
	КонецЕсли;
