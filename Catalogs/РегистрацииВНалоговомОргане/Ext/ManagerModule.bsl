...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьКодыРегионовИСлужебныеНаименования");
	Методы.Вставить("ЗаполнитьКодРегионаФедеральнойТерриторииСириус");
	Методы.Вставить("ОбработатьРеквизитыКрупнейшегоНалогоплательщика");
	Методы.Вставить("ЗаполнитьКодыРегионов");
	Методы.Вставить("УстановитьОтборНовостейТорговыйСбор");
...
Функция РегистрацияВНалоговомОргане(Организация, КПП, КодНалоговогоОргана) Экспорт
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	РегистрацииВНалоговомОргане.ПометкаУдаления,
...
Функция НайтиРегистрациюВНалоговомОргане(Организация, НалоговыйОрган) Экспорт
	...
	Запрос.Текст = 
		...
		|	И РегистрацииВНалоговомОргане.Владелец = &ГоловнаяОрганизация";
...
Функция НаименованиеСлужебное(РегистрацияВНалоговомОргане) Экспорт
	...
	|		ИНАЧЕ 3
	|	КОНЕЦ,
	|	ПодразделенияОрганизаций.РеквизитДопУпорядочиванияИерархического
	|ИЗ
...
Функция ПолучитьПоКодамРегистрациюВИФНС(РегистрацияВНалоговомОргане, Организация, КодНО, КПП, КодТерритории, ПометкаУдаления = Ложь) Экспорт
	
	ЭтоФизическоеЛицо = НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Организация);
	...
	Запрос.УстановитьПараметр("Ссылка", РегистрацияВНалоговомОргане);
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("КодНО", КодНО);
	Запрос.УстановитьПараметр("УсловиеПометкаУдаления", Истина);
	
	Если ТипЗнч(ПометкаУдаления) = Тип("Булево") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПометкаУдаления", "РегистрацииВНалоговомОргане.ПометкаУдаления = &ПометкаУдаления");
		Запрос.УстановитьПараметр("ПометкаУдаления", ПометкаУдаления);
	КонецЕсли;
	
	Если ЭтоФизическоеЛицо Тогда
		ТекстЗапроса = ТекстЗапроса + " И РегистрацииВНалоговомОргане.КодПоОКТМО = &КодПоОКТМО";
		Запрос.УстановитьПараметр("КодПоОКТМО", КодТерритории);
	Иначе
...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|	ПО ОбособленныеПодразделения.ГоловнаяОрганизация = ЭтотСписок.Владелец
	|;
...
// Процедура обновляет реквизит КодРегиона (по коду налогового органа) для регистраций на федеральной территории "Сириус".
// В "Сириусе" только одна ИНФС (2378), поэтому у всех регистраций с таким кодом налогового органа меняем код региона.
// Обрабатываем все элементы справочника одной порцией, так как обрабатываемых элементов ожидается совсем немного.
Процедура ЗаполнитьКодРегионаФедеральнойТерриторииСириус(Параметры) Экспорт
	...
	|	И РегистрацииВНалоговомОргане.КодРегиона <> &КодРегионаСириус";
	...
	ВыборкаРегистраций = РезультатЗапроса.Выбрать();
	ЕстьОшибки = Ложь;
	...
	Пока ВыборкаРегистраций.Следующий() Цикл
		...
		Попытка
			...
			РегистрацияВНалоговомОрганеОбъект.КодРегиона = КодРегионаСириусЧисло;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(РегистрацияВНалоговомОрганеОбъект);
		Исключение
			ЕстьОшибки = Истина;
			...
			Описание = СтрШаблон(
				НСтр("ru = 'Не удалось обработать регистрацию в налоговом органе на федеральной территории ""Сириус"" (%1) по причине:'"),
				ВыборкаРегистраций.Ссылка);
	...
	НалоговыйУчетОбособленныхПодразделений.ОбновитьНастройкиОтветственныхПодразделенийВФедеральнойТерриторииСириус(ЕстьОшибки);
	
	Параметры.ОбработкаЗавершена = Не ЕстьОшибки;
	
КонецПроцедуры

Процедура УстановитьОтборНовостейТорговыйСбор() Экспорт
	
	КатегорияНовостей = ПланыВидовХарактеристик.КатегорииНовостей.НайтиПоКоду("TaxesAndFees");
	Если НЕ ЗначениеЗаполнено(КатегорияНовостей) Тогда
		Возврат;
	КонецЕсли; 

	Запрос = Новый Запрос;
	
	КодыРегионовУплатыТС = Новый Массив;
	Для каждого КодРегиона Из ТорговыйСбор.КодыРегионовУплатыТорговогоСбора() Цикл
		КодыРегионовУплатыТС.Добавить(Число(КодРегиона));
	КонецЦикла; 
	
	Запрос.УстановитьПараметр("КодыРегионовУплатыТС", КодыРегионовУплатыТС);
	...
	|	Справочник.РегистрацииВНалоговомОргане КАК РегистрацииВНалоговомОргане
	|ГДЕ
	|	РегистрацииВНалоговомОргане.ДатаСнятияСУчета = &ПустаяДата
	|	И РегистрацииВНалоговомОргане.КодРегиона В (&КодыРегионовУплатыТС)";
	
	Результат = Запрос.Выполнить();
	
	// Показываем новости всем кто зарегистрирован в регионе введения ТС, или не вводил регион регистрации
	// Обрабатываем сценарий, когда пользователь только начал вести учет и еще не полностью заполнил реквизиты организации
	Если НЕ Результат.Пустой() Тогда
		ЗначениеОтбора = "ТорговыйСбор";
	Иначе
		// Фиктивное значение отбора чтобы срабатывали новости с пустым значение категории.
		ЗначениеОтбора = "*";
	КонецЕсли; 
	
	ДанныеОтбора = Новый Структура;
	ДанныеОтбора.Вставить("КатегорияНовостей"        , КатегорияНовостей);
	ДанныеОтбора.Вставить("ЗначениеКатегорииНовостей", ЗначениеОтбора);
...
// Процедура заполняет новый реквизит справочника РегистрацииВНалоговомОргане:
// - КрупнейшийНалогоплательщик (по коду налогового органа)
// Обрабатываем все элементы справочника одной порцией, так как элементов в справочнике, как правило, совсем немного
Процедура ОбработатьРеквизитыКрупнейшегоНалогоплательщика() Экспорт
	
	// КрупнейшийНалогоплательщик определяется по КПП, если 5-6 знак "50"
	...
	|	Справочник.РегистрацииВНалоговомОргане КАК РегистрацииВНалоговомОргане
	|ГДЕ
	|	РегистрацииВНалоговомОргане.КПП ПОДОБНО &Шаблон
	|	И НЕ РегистрацииВНалоговомОргане.КрупнейшийНалогоплательщик";
	...
	Пока ВыборкаРегистраций.Следующий() Цикл
		...
		Попытка
			...
			РегистрацияВНалоговомОрганеОбъект = ВыборкаРегистраций.Ссылка.ПолучитьОбъект();
			РегистрацияВНалоговомОрганеОбъект.КрупнейшийНалогоплательщик = Истина;
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(РегистрацияВНалоговомОрганеОбъект);
