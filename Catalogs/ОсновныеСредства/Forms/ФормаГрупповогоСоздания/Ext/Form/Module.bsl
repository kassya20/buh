...
Процедура ГруппаОСПриИзменении(Элемент)
	...
	УправлениеВидимостьюПрослеживаемости(ЭтаФорма);
...
Процедура КодТНВЭДПриИзменении(Элемент)
	...
	ЗаполнитьПолеНаименованияКода("Код", "ТНВЭД");
...
Процедура ПрослеживаемыйТоварПриИзменении(Элемент)
	...
	Элементы.ТНВЭД.Видимость = ПрослеживаемыйТовар;
...
Процедура Создать(Команда)
	...
	Если ПрослеживаемыйТовар И Не ЗначениеЗаполнено(КодТНВЭД) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ru = 'Код ТН ВЭД'"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Код ТН ВЭД");
		ЕстьОшибкиЗаполнения = Истина;
	КонецЕсли;
...
Функция СоздатьЭлементыСправочника()
	...
	Для НомерЭлемента = 1 По Количество Цикл
		...
		НовыйЭлемент.ПрослеживаемыйТовар   = ПрослеживаемыйТовар;
		НовыйЭлемент.КодТНВЭД              = КодТНВЭД;
		НовыйЭлемент.ЕдиницаУчета          = Перечисления.ЕдиницыУчетаОС.ИнвентарныйОбъект;
...
Процедура УправлениеВидимостьюПрослеживаемости(Форма)
	...
	Если Не Форма.ПрослеживаемыйТовар Тогда
		Элементы.ПрослеживаемыйТовар.Видимость = Форма.ВедетсяУчетПрослеживаемыхТоваров 
		И ЗначениеЗаполнено(Форма.ГруппаОС)
		И Форма.СписокГруппУчетаБезПрослеживаемости.НайтиПоЗначению(Форма.ГруппаОС) = Неопределено;
	КонецЕсли;
	
	Элементы.ТНВЭД.Видимость = Форма.ПрослеживаемыйТовар;
