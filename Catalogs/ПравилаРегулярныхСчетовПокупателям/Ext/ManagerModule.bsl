...
Функция ПравилоПоСчету(СчетПокупателю) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегулярныеСчетаПокупателям КАК РегулярныеСчетаПокупателям
	|		ПО ПравилаРегулярныхСчетовПокупателям.Ссылка = РегулярныеСчетаПокупателям.Правило
	|ГДЕ
