...
Процедура НастроитьСписокВыбора()
	...
	ПервыйПериод = Мин(ДатаАктуальностиКлассификатора,  ДатаВторогоКлассификатора);
	ВторойПериод = Макс(ДатаАктуальностиКлассификатора, ДатаВторогоКлассификатора);
	
	СписокВыбора.Добавить(Формат(Год(ПервыйПериод), "ЧГ="), Формат(ПервыйПериод, "ДФ='yyyy ""г.""'"));
	СписокВыбора.Добавить(Формат(Год(ВторойПериод), "ЧГ="), Формат(ВторойПериод, "ДФ='yyyy ""г.""'"));
...
Процедура ОпределитьНомерПриказа()
	...
	ГодКлассификатораМакс = Год(Макс(Справочники.ВидыНалоговИПлатежейВБюджет.ДатаАктуальностиКлассификатора(),
		Справочники.ВидыНалоговИПлатежейВБюджет.ДатаВторогоКлассификатора()));
	
	Если Период >= ГодКлассификатораМакс Тогда
		// Указывается новый классификатор
		ИмяМакета      = Справочники.ВидыНалоговИПлатежейВБюджет.ИмяМакетаКБКНовый();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 10 июня 2025 г. № 70н
			|(в редакции приказа Минфина от 01.12.2025 № 171н)'");
	Иначе
		// Указывается текущий, или старый классификатор
		ИмяМакета      = Справочники.ВидыНалоговИПлатежейВБюджет.ИмяМакетаКБКБезГода();
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 10 июня 2024 г. № 85н
			|(в редакции приказа Минфина от 15.10.2025 № 145н)'");
	КонецЕсли;
