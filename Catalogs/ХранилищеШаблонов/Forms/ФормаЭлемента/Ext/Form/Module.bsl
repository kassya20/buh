...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// для немаркированный продукции этот признак переопределяется в форме печати этикеток
	Элементы.ВыводитьРамки.Видимость = Объект.МаркированнаяПродукция;
КонецПроцедуры
...
Процедура РедактироватьШаблон(Команда)
	...
	ОборудованиеПринтерыЭтикетокКлиент.НачатьРедактированиеМакета(
		Оповещение, ПараметрыШаблонаЭтикеток.XMLОписаниеМакета, ПараметрыШаблонаЭтикеток.АдресХранилищаСКД, ПоместитьВоВременноеХранилище(Неопределено));
КонецПроцедуры
