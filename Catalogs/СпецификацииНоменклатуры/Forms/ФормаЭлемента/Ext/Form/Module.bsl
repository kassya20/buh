...
Процедура ПодобратьМатериалы(Команда)
	...
	ОткрытьФорму(
		...
		Новый Структура("ЭтоФормаСправочника", Истина), 
		ЭтотОбъект,
...
Процедура ЗагрузитьИзФайла(Команда)
	...
	ПараметрыЗагрузки.ИмяТабличнойЧасти = "Справочник.СпецификацииНоменклатуры.ИсходныеКомплектующие";
...
&НаКлиенте
Процедура ИсходныеКомплектующиеНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	ПараметрыПолученияДанных.Вставить("ВидыНоменклатуры", ВидыНоменклатурыДляВыбора());
...
&НаКлиенте
Процедура ИсходныеКомплектующиеНоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
...
Процедура НастроитьЗаголовокИВидимостьЭлементовФормы(ОткрытоИзКарточкиНоменклатуры, ОтображатьПродукцию)
	...
	Если ОткрытоИзКарточкиНоменклатуры И ЗначениеЗаполнено(Объект.Наименование) Тогда
		Заголовок = СтрШаблон(НСтр("ru = '%1 (%2)'"),НСтр("ru = 'Материалы'"), Объект.Наименование);
	Иначе
		Заголовок = Метаданные.Справочники.СпецификацииНоменклатуры.РасширенноеПредставлениеОбъекта;
	КонецЕсли;
	
	// Наименование новых объектов может быть заполнено в ОбработкеЗаполнения модуля объекта. Если этого не произошло,
	// разрешим редактировать наименование. В противном случае элемент справочника не получится записать.
	Элементы.Наименование.Видимость = Не ОткрытоИзКарточкиНоменклатуры Или Не ЗначениеЗаполнено(Объект.Наименование);
...
&НаКлиенте
Функция ВидыНоменклатурыДляВыбора()
