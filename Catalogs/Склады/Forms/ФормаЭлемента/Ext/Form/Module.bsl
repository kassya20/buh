...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступноРедактированиеКасс = ПравоДоступа("Редактирование", Метаданные.Справочники.ПодключаемоеОборудование)
		ИЛИ ПравоДоступа("Редактирование", Метаданные.ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат);
		
	Элементы.ГруппаКассы.Видимость = РольДоступна("ДобавлениеИзменениеДанныхБухгалтерии") ИЛИ РольДоступна("ПолныеПрава");
	Элементы.ПодключитьКассу.Видимость = ДоступноРедактированиеКасс;
...
&НаКлиенте
Процедура КонтактнаяИнформацияПолеАдресТорговойТочкиНажатие(Элемент, СтандартнаяОбработка)
	Если Элементы.КонтактнаяИнформацияПолеАдресСклада.ТолькоПросмотр Тогда
...
Процедура ПрочитатьТорговыеТочкиСклада()
	...
	Если НЕ (ПолучитьФункциональнуюОпцию("УплачиваетсяТорговыйСбор") 
		И ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.СоответствиеСкладовТорговымТочкам)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПодключитьКассу(Команда)
	...
	ПараметрыФормы = Новый Структура("Склад", Объект.Ссылка);
	ОткрытьФорму("Обработка.ПодключенныеКассы.Форма.ПомощникПодключенияКассы", ПараметрыФормы, ЭтаФорма, , , , ОбработкаОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокКасс(Команда)
	
	ОбработкаОповещения = Новый ОписаниеОповещения("ДобавлениеКассы_Завершение", ЭтотОбъект);
	ПараметрыФормы = Новый Структура("Склад", Объект.Ссылка);
	ОткрытьФорму("Обработка.ПодключенныеКассы.Форма.ПодключенныеКассы", ПараметрыФормы, ЭтаФорма, , , , ОбработкаОповещения);
...
Процедура УстановитьЦветГиперссылкиАдрес()
	...
	Элементы.КонтактнаяИнформацияПолеАдресСклада.ЦветТекста = ЦветТекста;
...
Функция АдресЗаполнен()
	...
	Возврат НЕ КонтактнаяИнформацияПолеАдресСклада = УправлениеКонтактнойИнформациейКлиентСервер.ТекстПустогоАдресаВВидеГиперссылки();
...
Процедура РазместитьКонтактнуюИнформацию()
	...
	РазмещеныНаФорме.Вставить(Справочники.ВидыКонтактнойИнформации.АдресСклада, Истина);
	...
	Если Не ЗначениеЗаполнено(КонтактнаяИнформацияПолеАдресСклада) Тогда
		КонтактнаяИнформацияПолеАдресСклада = УправлениеКонтактнойИнформациейКлиентСервер.ТекстПустогоАдресаВВидеГиперссылки();
	КонецЕсли;
