...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаЗаполнения.Видимость = Ложь;
...
Процедура ПроизводственныйКалендарьПриИзменении(Элемент)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.РезультатЗаполненияСтраница Тогда
...
Процедура РасширеннаяНастройка(Команда)
	...
	Элементы.ФормаРасширеннаяНастройка.Пометка = РасширеннаяНастройка;
	Если РасширеннаяНастройка Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.НастройкаЗаполненияСтраница;
	Иначе
		Элементы.Страницы.ТекущаяСтраница = Элементы.РезультатЗаполненияСтраница;
	КонецЕсли;
	Элементы.НастройкаЗаполнения.Видимость = РасширеннаяНастройка;
...
Процедура ЗаполнитьПоШаблонуНаСервере(СохранятьРучноеРедактирование = Ложь)
	...
	Для НомерДня = 1 По 7 Цикл
		...
		Объект.ШаблонЗаполнения.Добавить().ДеньВключенВГрафик = НомерДня <= 5;
