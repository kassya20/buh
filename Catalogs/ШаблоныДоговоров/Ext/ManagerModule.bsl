...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнениеТиповыхШаблоновДоговоровОтложенно");
...
//
Процедура ЗаполнениеТиповыхШаблоновДоговоровОтложенно(Параметры) Экспорт
	...
	ТиповыеМакеты = Новый Соответствие;
	МетаданныеМакеты = Метаданные.Справочники.ШаблоныДоговоров.Макеты;
	Для Каждого МетаМакета Из МетаданныеМакеты Цикл
		Если СтрНачинаетсяС(МетаМакета.Имя, "Договор")
			Или СтрНачинаетсяС(МетаМакета.Имя, "ДополнительноеСоглашение") Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТиповыеМакеты[Выборка.ИмяМакета] <> Неопределено Тогда
...
//
Процедура ОбновлениеТиповыхШаблоновДоговоровОтложенно(Параметры) Экспорт
	...
	НоваяСтрока = ТаблицаШаблонов.Добавить();
	НоваяСтрока.ИмяМакета 						= "ДоговорПоставки";
	НоваяСтрока.ПредыдущееКоличествоСимволов 	= 20709;
	...
	НоваяСтрока = ТаблицаШаблонов.Добавить();
	НоваяСтрока.ИмяМакета 						= "ДоговорОказанияУслуг";
	НоваяСтрока.ПредыдущееКоличествоСимволов 	= 12656;
	...
	НоваяСтрока = ТаблицаШаблонов.Добавить();
	НоваяСтрока.ИмяМакета 						= "ДоговорПодряда";
	НоваяСтрока.ПредыдущееКоличествоСимволов 	= 15174;

	ОбновлениеТиповыхШаблоновДоговоров(ТаблицаШаблонов, Параметры);
...
// Процедура обновляет элементы справочника при обновлении ИБ
//
Процедура ОбновлениеТиповыхШаблоновДоговоров(ТаблицаШаблонов, Параметры)
...
Процедура ВключитьУчетПоДоговорамПриНаличииПользовательскихШаблоновДоговоров() Экспорт
	
	Если Не ПолучитьФункциональнуюОпцию("ВестиУчетПоДоговорам") И СуществуютПользовательскиеШаблоныДоговоров() Тогда
...
Функция СуществуютПользовательскиеШаблоныДоговоров()
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 4
	|	ШаблоныДоговоров.Наименование КАК Наименование,
	|	ШаблоныДоговоров.ИмяМакета КАК ИмяМакета,
	|	ШаблоныДоговоров.ШаблонХранилище КАК ШаблонХранилище
	|ИЗ
	|	Справочник.ШаблоныДоговоров КАК ШаблоныДоговоров
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если Выборка.Количество() = 3 Тогда
		Пока Выборка.Следующий() Цикл
			Если НЕ СоответствуетПоставляемомуШаблону(Метаданные.Справочники.ШаблоныДоговоров.Макеты,
					Выборка.Наименование, Выборка.ИмяМакета, Выборка.ШаблонХранилище) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
...
Функция СоответствуетПоставляемомуШаблону(Макеты, Наименование, ИмяМакета, ШаблонХранилище)
	
	Если НЕ ЗначениеЗаполнено(ИмяМакета) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	НаименованиеМакета = Макеты[ИмяМакета];
	Если СокрЛП(Наименование) <> СокрЛП(НаименованиеМакета) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Макет = СтроковыеФункцииКлиентСервер.ИзвлечьТекстИзHTML(
		Справочники.ШаблоныДоговоров.ПолучитьМакет(ИмяМакета).ПолучитьТекст());
	Шаблон = СтроковыеФункцииКлиентСервер.ИзвлечьТекстИзHTML(
		ШаблонХранилище.Получить().ТекстHTML);
	Если Шаблон <> Макет Тогда
		Возврат Ложь;
	КонецЕсли;
