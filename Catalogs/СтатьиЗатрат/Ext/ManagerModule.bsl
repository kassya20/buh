...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УстановитьСтатьюЗатратДляМалоценки");
	Методы.Вставить("НазначитьПредопределенныйЭлементАмортизация");
	Методы.Вставить("ВключитьНастройкуПорядкаЭлементов");
	Методы.Вставить("УстановитьТорговыйСбор");
...
Процедура ЗаполнитьОписанияПоставляемыхЭлементов(ОписаниеЭлементов) Экспорт
	...
	// Уточнение названия статьи с особым порядком налогообложения
	// в зависимости от используемого функционала: ЕНВД или Патент.
	// В момент вызова функциональные опции еще могут быть
	// не обновлены, поэтому читаем значения констант.
	УточнениеНазванияСтатьи = "";
	Если Константы.ИспользуетсяЕНВД.Получить()
		И Константы.ИспользуетсяУСНПатент.Получить() Тогда
		УточнениеНазванияСтатьи = НСтр("ru = '(особый порядок налогообложения)'");
	ИначеЕсли Константы.ИспользуетсяЕНВД.Получить() Тогда
		УточнениеНазванияСтатьи = НСтр("ru = '(ЕНВД)'");
	ИначеЕсли Константы.ИспользуетсяУСНПатент.Получить() Тогда
		УточнениеНазванияСтатьи = НСтр("ru = '(патент)'");
	КонецЕсли;
	...
	// Оплата труда (особый порядок налогообложения)
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"ОплатаТрудаЕНВД",
		Перечисления.ВидыРасходовНУ.ОплатаТруда,
		СтрЗаменить("ru = 'Оплата труда %1'", "%1", УточнениеНазванияСтатьи),
		Истина,
		"ru = 'Основное отражение начисления зарплаты по деятельности с особым порядком налогообложения'");
	...
	// Страховые взносы (особый порядок налогообложения)
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"СтраховыеВзносыЕНВД",
		Перечисления.ВидыРасходовНУ.СтраховыеВзносы,
		СтрЗаменить("ru = 'Страховые взносы %1'", "%1", УточнениеНазванияСтатьи));
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяЕНВД");
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяУСНПатент");
	ОписаниеЭлемента.Реквизиты.Вставить(
		"ВидДеятельностиДляНалоговогоУчетаЗатрат",
	...
	// Взносы в ФСС от НС и ПЗ (особый порядок налогообложения)
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"ФСС_НС_ЕНВД",
		Перечисления.ВидыРасходовНУ.ПрочиеРасходы,
		СтрЗаменить("ru = 'Взносы в ФСС от НС и ПЗ %1'", "%1", УточнениеНазванияСтатьи));
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяЕНВД");
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяУСНПатент");
	ОписаниеЭлемента.Реквизиты.Вставить(
		"ВидДеятельностиДляНалоговогоУчетаЗатрат",
	...
	// Оплата больничного (особый порядок налогообложения)
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"ОплатаБольничногоЕНВД",
		Перечисления.ВидыРасходовНУ.ПрочиеРасходы,
		СтрЗаменить("ru = 'Оплата больничного %1'", "%1", УточнениеНазванияСтатьи));
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяЕНВД");
	ОписаниеЭлемента.Функциональность.Добавить("ИспользуетсяУСНПатент");
	ОписаниеЭлемента.Реквизиты.Вставить(
		"ВидДеятельностиДляНалоговогоУчетаЗатрат",
	...
	ОписаниеЭлемента.Функциональность.Добавить("ИспользоватьВыпускПродукции");
	...
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"ПриобретениеМалоценногоОборудованияИЗапасов",
		Перечисления.ВидыРасходовНУ.МатериальныеРасходы,
		"ru = 'Малоценное оборудование и запасы'",
		Истина,
		"ru = 'Документ ""Поступление товаров"" (малоценные объекты)'");
	ОписаниеЭлемента.Функциональность.Добавить("ВедетсяУчетОсновныхСредств");
	ОписаниеЭлемента.Функциональность.Добавить("ВедетсяУчетСпецодеждыСпецоснастки");
	...
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		...
		"РемонтОС",
		Перечисления.ВидыРасходовНУ.ПрочиеРасходы,
		"ru = 'Ремонт основных средств'",
		Истина);
		
	ОписаниеЭлемента.СоздаватьЗаранее = Ложь;
	
	ОписаниеЭлемента.Функциональность.Добавить("ВедетсяУчетОсновныхСредств");
	ОписаниеЭлемента.Функциональность.Добавить("КомпонентыАмортизацииОС");
	...
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		...
		"ru = 'Туристический налог'",
		Истина,
		"ru = 'Туристический налог'");
	ОписаниеЭлемента.Функциональность.Добавить("УплачиваетсяТуристическийНалог");
	
	// Приобретение прав на компьютерные программы из реестра отечественного ПО
	ОписаниеЭлемента = ДобавитьОписаниеПоставляемогоЭлемента(
		ОписаниеЭлементов,
		"ПриобретениеКомпьютерныхЛицензийСПовышающимКоэффициентом",
		Перечисления.ВидыРасходовНУ.РасходыНаКомпьютерныеЛицензииСПовышающимКоэффициентом,
		"ru = 'Приобретение компьютерных лицензий с применением повышающего коэффициента (п. 1.26 ст. 264 НК РФ)'",
		Истина);
	ОписаниеЭлемента.СоздаватьЗаранее = Ложь;
	ОписаниеЭлемента.Функциональность.Добавить("ИспользоватьДокументыПоступления");
...
// Сопоставляет предопределенный элемент "Амортизация" с подходящим элементом справочника. 
// Если элемент не найден, то при необходимости создает его.
//
Процедура НазначитьПредопределенныйЭлементАмортизация() Экспорт
	...
	КлассификаторыДоходовРасходов.ПоставляемыйЭлемент(Справочники.СтатьиЗатрат, "Амортизация");
...
// Обрабатывает добавление новой предопределенной статьи для учета малоценки.
// Сама статья в данных могла быть создана и ранее, но не была предопределенной.
//
Процедура УстановитьСтатьюЗатратДляМалоценки() Экспорт
	...
	КлассификаторыДоходовРасходов.ОбеспечитьФункциональность(Справочники.СтатьиЗатрат, "ВедетсяУчетСпецодеждыСпецоснастки");
