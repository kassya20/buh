...
&НаКлиенте
Процедура ВидыНачисленийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ТекущиеДанные = Элементы.ВидыНачислений.ТекущиеДанные;
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Если Объект.ВидыРасчетовРезерва.Количество() > 0 Тогда
		ВидыРачетаРезерва = ОбщегоНазначения.ВыгрузитьКолонку(Объект.ВидыРасчетовРезерва,"ВидРасчетаРезерва", Истина);
		Если ЗначениеЗаполнено(ВидыРачетаРезерва) Тогда
			ВидыНачисления = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ВидыРачетаРезерва, "ВидРасчета");
			Для Каждого ВидРачетаРезерва Из ВидыРачетаРезерва Цикл
				ВидНачисления = ВидРачетаРезерва.ВидРасчета;
...
Процедура ВидНачисленияПриИзмененииНаСервере()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций") Тогда
		...
		Модуль.УстановитьВидыРасчетовРезервовПоОплатеТруда(Объект, ВидыНачислений);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ВидНачисления.Видимость  = Форма.ВидыНачислений.Количество() <= 1;
	Элементы.ВидыНачислений.Видимость = Форма.ВидыНачислений.Количество() > 1;
...
Процедура ПроверитьЗаполнитьВидРасчетаРезерва()
	...
	ВидыНачислений.Очистить();
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ЗарплатаКадрыДляНебольшихОрганизаций") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("РезервыПоОплатеТрудаДляНебольшихОрганизаций");
		Модуль.ПроверитьЗаполнитьВидРасчетаРезерва(Объект, ВидыНачислений);
	КонецЕсли;
	ВидНачисления = ?(ВидыНачислений.Количество() = 1, ВидыНачислений[0].Значение, "");
	ВидНачисленияПриИзмененииНаСервере();
	ЗаполнитьБазовыеНачисления();
