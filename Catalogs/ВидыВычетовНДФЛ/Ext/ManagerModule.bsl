...
// 
// Параметры:
//		ИмяПредопределенныхДанных - Строка, СправочникСсылка.ВидыДоходовНДФЛ - элемент для описания.
//  	Код - Строка.
//  	Наименование - Строка.
//		ВычетКДоходу - Булево.
...
//  	ПорядковыйНомерВРасчетеНалога - Число.
//  	Код2023 - Строка.
//  	
Процедура ОписатьКодВычетаНДФЛ(ИмяПредопределенныхДанных, Код, Наименование, ВычетКДоходу = Истина, ГруппаВычета = Неопределено, Код2010 = "", Код2011 = "", Код2015 = "", Код2016 = "", НеПредоставляетсяНерезидентам = Ложь, ПорядковыйНомерВРасчетеНалога = 0, Код2023 = "") Экспорт 
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Код) Тогда
			СсылкаНаЭлемент = Справочники.ВидыВычетовНДФЛ.НайтиПоКоду(Код);
		КонецЕсли;
...
Процедура ЗаполнитьКодыВычетовНДФЛ() Экспорт
	...
	ОписатьВычеты2025();
	ОписатьВычеты2026();
	ПереименоватьВычеты2026();
	
	// АПК:1297-вкл
	
	УчетЗарплаты.ЗаполнитьКодыВычетовНДФЛНепредопределенных();
...
Процедура ОписатьВычеты2026() Экспорт 

	ГруппыВычетов = Перечисления.ГруппыВычетовПоНДФЛ;
	// АПК:1297-выкл Не локализуется, т.к. является частью регламентированной формы, применяемой в РФ.
	
	ОписатьКодВычетаНДФЛ("Код333", "333", "Сумма, уплаченная налогоплательщиком в налоговом периоде за свое обучение в образовательных учреждениях иностранного государства, за обучение брата (сестры) в возрасте до 24 лет, супруга (супруги) по очной форме обучения в образовательных учреждениях иностранного государства - в размере фактически произведенных расходов на обучение с учетом ограничения, установленного пунктом 2 статьи 219 НК РФ", 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО,
	"-", "-", "-", "-", , 32, "333");
	
	ОписатьКодВычетаНДФЛ("Код334", "334", "Сумма, уплаченная налогоплательщиком-родителем за обучение своих детей в возрасте до 24 лет, налогоплательщиком-опекуном (налогоплательщиком-попечителем) за обучение своих подопечных в возрасте до 18 лет по очной форме обучения в образовательных учреждениях иностранного государства, налогоплательщиком, осуществляющим обязанности опекуна или попечителя над гражданами, бывшими их подопечными, после прекращения опеки или попечительства в случаях оплаты налогоплательщиком обучения граждан в возрасте до 24 лет по очной форме обучения в образовательных учреждениях иностранного государства - в размере фактически произведенных расходов на обучение, но с учетом ограничения, установленного подпунктом 2 пункта 1 статьи 219 НК РФ", 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО,
	"-", "-", "-", "-", , 32, "334");
	
	ОписатьКодВычетаНДФЛ("Код518", "518", "Вычет из сумм выплат по кодам дохода 1241 и 1243 в пределах 30 миллионов рублей по каждому договору и 30 миллионов рублей за каждый налоговый период", 
	, ,
	"-", "-", "-", "-", , , "518");
	// АПК:1297-вкл
...
Процедура ПереименоватьВычеты2026() Экспорт
	
	ГруппыВычетов = Перечисления.ГруппыВычетовПоНДФЛ;
	// АПК:1297-выкл Не локализуется, т.к. является частью регламентированной формы, применяемой в РФ.
	
	ОписатьКодВычетаНДФЛ("Код320о", "320", "Сумма, уплаченная налогоплательщиком в налоговом периоде за свое обучение в образовательных учреждениях РФ, за обучение брата (сестры) в возрасте до 24 лет, супруга (супруги) по очной форме обучения в образовательных учреждениях РФ - в размере фактически произведенных расходов на обучение с учетом ограничения, установленного пунктом 2 статьи 219 НК РФ", 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО,
	"-", "-", "320", "320", , 32);
	
	ОписатьКодВычетаНДФЛ("Код321", "321", "Сумма, уплаченная налогоплательщиком-родителем за обучение своих детей в возрасте до 24 лет, налогоплательщиком-опекуном (налогоплательщиком-попечителем) за обучение своих подопечных в возрасте до 18 лет по очной форме обучения в образовательных учреждениях РФ, налогоплательщиком, осуществляющим обязанности опекуна или попечителя над гражданами, бывшими их подопечными, после прекращения опеки или попечительства в случаях оплаты налогоплательщиком обучения граждан в возрасте до 24 лет по очной форме обучения в образовательных учреждениях РФ - в размере фактически произведенных расходов на обучение, но с учетом ограничения, установленного подпунктом 2 пункта 1 статьи 219 НК РФ", 
	Ложь, ГруппыВычетов.СоциальныеПоУведомлениюНО, 
	"-", "-", "321", "321", , 31);
	// АПК:1297-вкл
