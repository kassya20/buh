...
Процедура ДобавитьФайлПриложенияОтчетностиЗавершениеПослеПомещенияФайла(
	...
	Если ФайлыБылиВыбраны Тогда
		...
		Если ДополнительныеПараметры.ТипФайлаОтчетности =
					ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ПояснительнаяЗаписка")
				И НЕ СтрНачинаетсяС(СвойстваФайла.Имя, "NO_BUHPZ_")
			ИЛИ ДополнительныеПараметры.ТипФайлаОтчетности =
					ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.АудиторскоеЗаключение")
				И НЕ СтрНачинаетсяС(СвойстваФайла.Имя, "NO_BUHAZ_")
			ИЛИ ДополнительныеПараметры.ТипФайлаОтчетности =
					ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ЗаявлениеСоюзаСельхозПроизводителей")
				И НЕ СтрНачинаетсяС(СвойстваФайла.Имя, "NO_BUHZSHP_") Тогда
			
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Неверное имя файла'"));
			Возврат;
		КонецЕсли;
...
Функция ОпределитьТипФайлаУведомления(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов)
	...
	ИмяФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолноеИмяФайла).ИмяБезРасширения;
	ИмяФайлаПриведенное = ВРЕГ(ИмяФайла);
	Если Лев(ИмяФайлаПриведенное, 11) = "ON_UVNPDUS_"
		ИЛИ Лев(ИмяФайлаПриведенное, 12) = "UT_DOKUVKSD_"
		ИЛИ Лев(ИмяФайлаПриведенное, 16) = "NO_UVNPOKODOKNO_"
		ИЛИ Лев(ИмяФайлаПриведенное, 14) = "NO_UVEDAUDZAK_" Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности");
	ИначеЕсли ЗагружаетсяНесколькоФайлов И Лев(ИмяФайлаПриведенное, 3) = "PR_"
		ИЛИ ЗагружаетсяНесколькоФайлов И Лев(ИмяФайлаПриведенное, 5) = "0300_"
		ИЛИ ЗагружаетсяНесколькоФайлов И Лев(ИмяФайлаПриведенное, 5) = "9996_"
		ИЛИ ЗагружаетсяНесколькоФайлов И Лев(ИмяФайлаПриведенное, 10) = "NO_MFSOAZ_" Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыСодержимогоТранспортногоКонтейнера.Приложение");
	КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиФСС(ПолноеИмяФайла, ПреобразуемыйАдрес)
	...
	ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникамГосслужащим = (Лев(КороткоеИмяФайла, 2) = "G_");
	...
	ИначеЕсли ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникам
		ИЛИ ЭтоРеестрСтимулирующихВыплатМедицинскимИСоциальнымРаботникамГосслужащим Тогда
...
Функция ПроанализироватьФайлВыгрузкиЦБ(ПолноеИмяФайла)
	...
	Если Расширение <> "xml" Тогда
		Если Расширение = "pdf" ИЛИ Расширение = "doc" ИЛИ Расширение = "docx" Тогда 
			Если СтрНайти(КороткоеИмяФайла, "1-ПИ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ПИ") > 0
			...
			ИначеЕсли СтрНайти(КороткоеИмяФайла, "1-ИЦБ") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "1ИЦБ") > 0
				...
				ИЛИ СтрНайти(КороткоеИмяФайла, "ICB-1") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "ICB1") > 0 Тогда 
...
Функция ЭтоФайлВыгрузкиФСС(ОсновнойФайл)
	...
	Если нрег(КороткоеИмяФайла) = "9999999999_9999.xml" 			// 4аФСС
		...
		ИЛИ Лев(КороткоеИмяФайла, 18) = "C_9999999999_9999_" 		// реестр стимулирующих выплат мед. и соц. работникам
		ИЛИ Лев(КороткоеИмяФайла, 18) = "G_9999999999_9999_" Тогда 	// реестр стим. выплат мед. и соц. работникам госслужащим
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = 
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДокументацияПоКонтролируемымСделкам")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.АудиторскоеЗаключениеФинОтчетность")
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС") Тогда
		...
		Пока ИндексФайла >= 1 Цикл
			СвойстваФайла = Форма.СвойстваФайлов[ИндексФайла];
			// "Документация по контролируемым сделкам" может отправляться с приложениями
			ПриведенноеИмяФайла = ВРег(СвойстваФайла.ИмяФайла);
			Если Лев(ПриведенноеИмяФайла, 12) <> "UT_DOKUVKSD_" И Лев(ПриведенноеИмяФайла, 5) <> "0300_" Тогда
				ТекстСообщения = СтрШаблон(
	...
	ИначеЕсли ЗагружаетсяНесколькоФайлов
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ЖалобаФНС")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ДокументацияПоКонтролируемымСделкам")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.АудиторскоеЗаключениеФинОтчетность")) Тогда
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если РасширениеВыбранногоФайла = ".txt"
			...
			РазрешеныНетекстовыеВложения = (Лев(ПриведенноеИмяФайла, 11) = "ON_UVNPDUS_"
				ИЛИ Лев(ПриведенноеИмяФайла, 12) = "UT_DOKUVKSD_"
				ИЛИ Лев(ПриведенноеИмяФайла, 16) = "NO_UVNPOKODOKNO_"
				ИЛИ Лев(ПриведенноеИмяФайла, 14) = "NO_UVEDAUDZAK_"
