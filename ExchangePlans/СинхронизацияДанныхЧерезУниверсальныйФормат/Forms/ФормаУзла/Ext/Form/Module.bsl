...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтправлятьНСИ_Касса = Объект.ПравилаОтправкиСправочников = "АвтоматическаяСинхронизация";
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если Объект.ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
		
		Если Объект.Склады.Количество() = 1 Тогда
			РозничныйСклад = Объект.Склады[0].Склад;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(ПроверяемыеРеквизиты, "РозничныйСклад");
	
	Если Объект.ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
		
		Если РозничныйСклад.Пустая() Тогда
...
Процедура ПереключательОтправлятьНСИНикогдаПриИзменении(Элемент)
	...
	УстановитьНикогдаНеОтправлятьНСИНаСервере();
...
&НаКлиенте
Процедура ФлагОтправлятьНСИ_КассаПриИзменении(Элемент)
	
	УстановитьОтправкуНСИ_КассаНаСервере();
	
КонецПроцедуры

&НаКлиенте
Процедура РозничныйСкладПриИзменении(Элемент)
	
	Объект.Склады.Очистить();
	
	Если НЕ РозничныйСклад.Пустая() Тогда
		
		Объект.Склады.Добавить().Склад = РозничныйСклад;
		
	КонецЕсли;
	
	РозничныйСкладПриИзмененииСервер();
...
Процедура УстановитьВидимостьНаСервере()
	...
	ЭтоОбменСКассой =
		Объект.ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСКассой()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ГруппаНастройкаОтправкиНСИ_Касса",
		"Видимость",
		ЭтоОбменСКассой);
	...
	Если ЭтоОбменСКассой Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			"ПереключательОтправлятьНСИПоНеобходимости",
			"Видимость",
			Ложь);
		...
		Если Объект.ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
			...
			Элементы.СтраницыВыгрузкаКассаРозничныеСклады.ТекущаяСтраница = Элементы.СтраницаРозничныйСклад;
			
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы,
				"РозничныйСкладТипЦенРозничнойТорговли",
				"Видимость",
				НЕ РозничныйСклад.Пустая());
			
		КонецЕсли;
		
	ИначеЕсли Объект.ВариантНастройки = "ОбменЗУПБП" Тогда
...
&НаСервере
Процедура РозничныйСкладПриИзмененииСервер()
...
&НаСервере
Процедура УстановитьНикогдаНеОтправлятьНСИНаСервере()
...
&НаСервере
Процедура УстановитьОтправкуНСИ_КассаНаСервере()
	
	Если Объект.ВариантНастройки <> ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСКассой()
		И Объект.ВариантНастройки <> ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ПравилаОтправкиСправочников = ?(ОтправлятьНСИ_Касса, "АвтоматическаяСинхронизация", "СинхронизироватьПоНеобходимости");
