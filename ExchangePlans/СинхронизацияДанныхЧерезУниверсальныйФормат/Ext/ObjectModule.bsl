...
Процедура ПередЗаписью(Отказ)
	...
	Если ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
		...
		НастройкиИзОжиданиеПодключенияМК();
...
Процедура ПриЗаписи(Отказ)
	...
	Если ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСМобильнойКассой() Тогда
		УдалитьНастройкиИзОжиданиеПодключенияМК();
	КонецЕсли;
...
Процедура НастроитьОтборыПоУмолчанию()
	...
	ИначеЕсли ВариантНастройки = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ИдентификаторОбменаСДокументооборотом() Тогда
		ВерсияФорматаОбмена = "1.16";
...
Процедура НастройкиИзОжиданиеПодключенияМК()
	
	НачалоИдентификатора = СтрНайти(Наименование, "(");
	КонецИдентификатора = СтрНайти(Наименование, ")");
	
	Если КонецИдентификатора - НачалоИдентификатора > 1
		И НачалоИдентификатора > 0 Тогда
		
		ИдентификаторМК = Сред(Наименование, НачалоИдентификатора + 1, КонецИдентификатора - НачалоИдентификатора - 1);
		...
		Если ПодключенияМК.Выбран() Тогда
			ТипЦенДляИзмененияЦен = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПодключенияМК.Склад, "ТипЦенРозничнойТорговли");
			
			Организации.Очистить();
			НоваяСтрока = Организации.Добавить();
			НоваяСтрока.Организация = ПодключенияМК.Организация;
			ИспользоватьОтборПоОрганизациям = Истина;
			
			Склады.Очистить();
			НоваяСтрока = Склады.Добавить();
			НоваяСтрока.Склад = ПодключенияМК.Склад;
...
Процедура УдалитьНастройкиИзОжиданиеПодключенияМК()
