...
Функция ВидыОперацийДоступныеПриИнтеграцииСБанком() Экспорт
	...
	НовыйРезультат = Результат.Добавить();
	НовыйРезультат.ВидОперации = Менеджер.ВозвратНалога;
	НовыйРезультат.ОблагаетсяНалогом = "Нет";
	...
	НовыйРезультат = Результат.Добавить();
	НовыйРезультат.ВидОперации = Менеджер.ВозвратОтПодотчетногоЛица;
	НовыйРезультат.ОблагаетсяНалогом = "Нет";
	...
	НовыйРезультат = Результат.Добавить();
	НовыйРезультат.ВидОперации = Менеджер.Депозит;
	НовыйРезультат.ОблагаетсяНалогом = "Нет";
...
Функция ПолучитьДоступныеЗначения(Отбор, СтрокаПоиска)
	...
	Если ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком() Тогда
		Исключения.Добавить(ВозвратОтПодотчетногоЛица);
	...
	Если Не ЭтоЮрЛицо Или Валютный Тогда
		Исключения.Добавить(Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВзносВУставныйКапитал);
	КонецЕсли;
	
	Если Валютный Тогда
		Исключения.Добавить(Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратНалога);
	КонецЕсли;
