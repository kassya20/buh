...
Функция ДопустимыеЗначения(Отбор = Неопределено, Знач СтрокаПоиска = Неопределено) Экспорт
	...
	ИначеЕсли ТипОтбора = Тип("ФиксированныйМассив") Тогда
		...
		Запрос.УстановитьПараметр("Отбор", Отбор);
	ИначеЕсли ТипОтбора = Тип("Структура") И Отбор.Свойство("Исключения") Тогда
		ТекстыОтбора.Добавить("ВидыОперацийОплатаПлатежнойКартой.Ссылка НЕ В(&Исключения)");
		Запрос.УстановитьПараметр("Исключения", Отбор.Исключения);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка = Перечисления.ВидыОперацийОплатаПлатежнойКартой.РозничнаяВыручка И Не РозничнаяВыручкаДоступна(Отбор) Тогда
...
Функция РозничнаяВыручкаДоступна(Отбор)
	...
	Возврат РозницаВключена Или УчетБезЗакрывающих;
