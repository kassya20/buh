...
// 
// В том же источнике данных СведенияОбОрганизации в наборе данных ОсуществлениеХозДеятельности
// первый подзапрос объединения использует таблицу справочника организаций лишь с той целью, чтобы при
// выключенных ФО ИспользоватьДокументыРеализации и ВедетсяПроизводственнаяДеятельность 
// система компоновки данных не отключала целиком этот набор данных и таким образом не вызывала ошибку
// при обращении к показателям, поставляемым данным источником данных. 
...
Функция РазделИсточникаДанных(ИсточникДанных, Знач Характеристика) Экспорт
	...
	Если ИсточникДанных = ФинансовоеСостояние 
		...
		Если Характеристика = "ПрибыльУбыток"
			...
			Или Характеристика = "ВыручкаВЦеляхНУ"
...
Функция ИсточникПоддерживаетСписокОрганизаций(ИсточникДанных) Экспорт 
	...
	Если ИсточникДанных = СведенияОбОрганизации
		...
		Или ИсточникДанных = ВложенияВоВнеоборотныеАктивы
		Или ИсточникДанных = НематериальныеАктивы Тогда
		Возврат Ложь;
	КонецЕсли;
