...
// Определяет, относится ли переданный вид агентского договора договором реализации товаров
// Параметры:
//     ВидАгентскогоДоговора - ПеречислениеСсылка.ВидыАгентскихДоговоров - вид договора налогового агента, 
//                             для которого необходимо определить признак.
// Возвращаемое значение:
//     Булево - Истина - договор с нерезидентом,
//              Ложь - договор с резидентом.
Функция ЭтоРеализацияТоваров (ВидАгентскогоДоговора) Экспорт
	
	ВидыДоговоровРеализацииТоваров = МассивВидовДоговоровРеализацииТоваров();
	
	Если ВидыДоговоровРеализацииТоваров.Найти(ВидАгентскогоДоговора) = Неопределено Тогда 
		Возврат Ложь;
	Иначе 
		Возврат Истина;
	КонецЕсли;
...
// На основании переданного вида агентского договора определяет, нужно ли исчисляеть НДС налоговому агенту.
// Параметры:
//     ВидАгентскогоДоговора - ПеречислениеСсылка.ВидыАгентскихДоговоров - вид договора налогового агента, 
//                             для которого необходимо определить признак.
// Возвращаемое значение:
//     Булево - Истина - договор с нерезидентом,
//              Ложь - договор с резидентом.
Функция НДСИсчисляетсяНалоговымАгентом (ВидАгентскогоДоговора) Экспорт
	
	Возврат ЭтоРеализацияТоваров(ВидАгентскогоДоговора);
	
КонецФункции

// Возвращает массив видов договоров налогового агента по которым требуется округление налога.
// Параметры:
//     ОкруглятьПоВсемДоговорамНА - Булево - признак того, что нужно округлять НДС по всем договорам налогового агента.
// Возвращаемое значение:
//     ВидыДоговоров - Массив - массив значений виды ПеречислениеСсылка.ВидыАгентскихДоговоров.
Функция МассивВидовДоговоровТребующихОкругленияНалога(ОкруглятьПоВсемДоговорамНА = Ложь) Экспорт
	...
	Если ОкруглятьПоВсемДоговорамНА Тогда
		ВидыДоговоров.Добавить(Перечисления.ВидыАгентскихДоговоров.Аренда);
	КонецЕсли;
...
// Возвращает массив видов договоров налогового агента по которым происходит обратное начисление НДС.
// Возвращаемое значение:
//     ВидыДоговоров - Массив - массив значений виды ПеречислениеСсылка.ВидыАгентскихДоговоров.
Функция МассивВидовДоговоровРеализацииТоваров() Экспорт
	
	ВидыДоговоров = Новый Массив;
	ВидыДоговоров.Добавить(Перечисления.ВидыАгентскихДоговоров.РеализацияТоваров);
	ВидыДоговоров.Добавить(Перечисления.ВидыАгентскихДоговоров.РеализацияМедныхПолуфабрикатов);
...
Функция ПолучитьДоступныеЗначения(Отбор, СтрокаПоиска)
	...
	Если НЕ БухгалтерскийУчетПереопределяемый.ИспользуетсяОбратноеНачислениеНДС() Тогда
		Для Каждого ВидДоговораРеализацииТоваров Из МассивВидовДоговоровРеализацииТоваров() Цикл 
			Исключения.Добавить(ВидДоговораРеализацииТоваров);
		КонецЦикла;
	КонецЕсли;
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИсполняютсяОбязанностиНалоговогоАгентаПоНДС")
		ИЛИ (ТипЗнч(Отбор) = Тип("Структура") И Отбор.Свойство("ДоговорРеализации"))Тогда
