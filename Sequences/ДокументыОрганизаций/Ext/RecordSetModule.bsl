...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если Отказ
		...
		Или ДополнительныеСвойства.Свойство("ОтключитьБизнесЛогикуРИБ") Тогда
		Возврат;
	КонецЕсли;
	
	Если ЭтотОбъект.Количество() > 0
		И ТипЗнч(Регистратор) <> Тип("ДокументСсылка.УстановкаЦенНоменклатуры")
		И ДополнительныеСвойства.Свойство("ЗарегистрироватьВПоследовательности") Тогда
		
		РаботаСПоследовательностями.АктуализироватьРегламентныеОперации(ЭтотОбъект[0]);
