...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоВводНаОсновании Тогда
		...
		Если ТипыДокументовРазрешенные = Неопределено Тогда // доступен полный набор типов
			...
			ИначеЕсли ТипЗнч(Параметры.Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
				Если Параметры.Основание.ВидОперации = Перечисления.ВидыОперацийСчетаПокупателю.ОсновныеСредства Тогда
					Если ТипыДокументов.НайтиПоЗначению("АктОбОказанииПроизводственныхУслуг") <> Неопределено Тогда
						ТипыДокументов.Удалить(ТипыДокументов.НайтиПоЗначению("АктОбОказанииПроизводственныхУслуг"));
					КонецЕсли;
					Если ТипыДокументов.НайтиПоЗначению("РеализацияТоваровУслуг") <> Неопределено Тогда
						ТипыДокументов.Удалить(ТипыДокументов.НайтиПоЗначению("РеализацияТоваровУслуг"));
					КонецЕсли;
					Если ТипыДокументов.НайтиПоЗначению("РозничнаяПродажа") <> Неопределено Тогда
						ТипыДокументов.Удалить(ТипыДокументов.НайтиПоЗначению("РозничнаяПродажа"));
					КонецЕсли;
				Иначе
					Если ТипыДокументов.НайтиПоЗначению("ПередачаОС") <> Неопределено Тогда
						ТипыДокументов.Удалить(ТипыДокументов.НайтиПоЗначению("ПередачаОС"));
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		...
		Для Каждого ОбъектМетаданных Из ДокументыЖурнала Цикл
			Если ПравоДоступа("Изменение", ОбъектМетаданных) Тогда
				ТипыДокументовРазрешенные.Добавить(ОбъектМетаданных);
			КонецЕсли;
		КонецЦикла;
		Если Параметры.КлючНазначенияИспользования = "ДокументыПоДоговоруКонтрагента" Тогда
			ТипыДокументовРазрешенные = КритерииОтбора.ДокументыПоДоговоруКонтрагента.ОтобратьДокументыПоВидуДоговора(ТипыДокументовРазрешенные,
				Параметры.ВидДоговораКонтрагента);
		КонецЕсли;
		Для Каждого ОбъектМетаданных Из ТипыДокументовРазрешенные Цикл
			ТипыДокументов.Добавить(ОбъектМетаданных.Имя, ОбъектМетаданных.Синоним);
		КонецЦикла;
