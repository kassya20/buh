...
Функция ПредупреждениеДляДокумента(ПараметрыДокумента) Экспорт
	...
	Предупреждение = РасширенноеПредупреждениеДляДокумента(ПараметрыДокумента);
	
	Если ЗначениеЗаполнено(Предупреждение) Тогда
		Возврат Предупреждение.ТекстСообщения;
	КонецЕсли;
...
// Возвращает расширенное предупреждение при загрузке для документа.
//
// Параметры:
//  ПараметрыДокумента - ДокументСсылка - Документ, для которого нужно сформировать предупреждение.
// 
// Возвращаемое значение:
//   - Неопределено, Структура - Если предупреждения для документа нет - возвращается Неопределено.
//     * ТекстСообщения - Строка - Форматированная строка с предупреждением
//     * АвтоматическаяВыписка - Булево - признак получения ошибки при загрузке автоматической выписки
//
Функция РасширенноеПредупреждениеДляДокумента(ПараметрыДокумента) Экспорт
	...
	|	ПредупрежденияПриЗагрузкеВыписки.ТекстСообщенияАвтоматическойВыписки КАК ТекстСообщенияАвтоматическойВыписки
	|ИЗ
	|	РегистрСведений.ПредупрежденияПриЗагрузкеВыписки КАК ПредупрежденияПриЗагрузкеВыписки
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		Предупреждение = Новый Структура("ТекстСообщения,АвтоматическаяВыписка");
		Если ЗначениеЗаполнено(Выборка.ТекстСообщенияАвтоматическойВыписки) Тогда
			Предупреждение.ТекстСообщения = Выборка.ТекстСообщенияАвтоматическойВыписки;
			Предупреждение.АвтоматическаяВыписка = Истина;
		ИначеЕсли ЗначениеЗаполнено(Выборка.ТекстСообщенияПользователю) Тогда
			Предупреждение.ТекстСообщения = Выборка.ТекстСообщенияПользователю;
			Предупреждение.АвтоматическаяВыписка = Ложь;
		Иначе
			Предупреждение.ТекстСообщения = ТекстПредупрежденияПоСтрокеВыписки(СтрокаВыписки, ПараметрыДокумента);
			Предупреждение.АвтоматическаяВыписка = Ложь;
		КонецЕсли;
	КонецЕсли;
...
// Создает новую запись регистра, заполненную согласно переданным параметрам
//
// Параметры:
//  Организация - СправочникСсылка.Организации
//  Документ - ДокументСсылка.СписаниеСРасчетногоСчета, ДокументСсылка.ПоступлениеНаРасчетныйСчет
//  ТекстПредупреждения - Строка - новое значение ресурса ТекстСообщенияПользователю
//
Процедура ЗаписатьПредупреждение(Организация, Документ, ТекстПредупреждения = "") Экспорт
	...
	Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
		...
		Запись.ТекстСообщенияПользователю = ТекстПредупреждения;
	КонецЕсли;
	Набор.Записать();
