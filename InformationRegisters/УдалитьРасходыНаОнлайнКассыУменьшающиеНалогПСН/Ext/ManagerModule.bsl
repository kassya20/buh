...
Функция ОтразитьРасходыНаОнлайнКассы(КонтекстОперации, ДанныеКРаспределению, Движения) Экспорт
	...
	Для Каждого ПлатежПоПатенту Из УменьшаемыеПлатежи Цикл
		Если ОстатокВычета = 0 Тогда
			Прервать;
		КонецЕсли;
		НеоплаченнаяСумма = Макс(ПлатежПоПатенту.СуммаПлатежа - ПлатежПоПатенту.Оплачено
			- ПлатежПоПатенту.ВычетНаСтраховыеВзносы - ПлатежПоПатенту.ТекущийВычетККТ, 0);
...
Функция ПлатежиПодлежащиеУменьшению(Организация, РегистрацияВНалоговомОргане, Период)
	...
	|		КОНЕЦ
	|	И ОперацияПоПатенту.ДатаОкончания <= &КонецГода
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	КОНЕЦ КАК Оплачено,
	|	ЕСТЬNULL(РасходыНаОнлайнКассыУменьшающиеНалогПСН.НалоговыйВычет, 0) КАК ТекущийВычетККТ,
