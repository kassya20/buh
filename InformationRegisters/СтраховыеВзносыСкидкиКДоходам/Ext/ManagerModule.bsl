...
Процедура ЗаполнитьСкидкиКДоходамПоСтраховымВзносам()
	...
	// ---------------------------------------------------
	// ЗНАЧЕНИЯ СКИДОК К ДОХОДАМ НА 01.01.2010 и 01.01.2026
	...
	МассивДатСведений = Новый Массив;
	ДатаСведений = '20100101';
	ДатаСведений2026 = '20260101';
	...
	НаборЗаписей.Отбор.Период.Значение = ДатаСведений2026;
	НаборЗаписей.Отбор.ВидДохода.Значение = СпрВидов.МатпомощьПриРожденииРебенка;
	НаборЗаписей.Записать();
	...
	НоваяЗапись.Период			= ДатаСведений;
	НоваяЗапись.ВидДохода		= СпрВидов.МатпомощьПриРожденииРебенка;
	НоваяЗапись.НормативЗатрат	= 1000000;
...
Процедура ОписатьДополнительныеСкидкиКДоходамПоСтраховымВзносам()
	...
	НаборЗаписей.Записать();
	НаборЗаписей.Очистить();
	НаборЗаписей.Отбор.ВидДохода.Значение = СпрВидов.МатпомощьПриРожденииРебенкаСтудентовИзСтудотряда;
 	НоваяЗапись = НаборЗаписей.Добавить();
	НоваяЗапись.Период			= ДатаСведений;
	НоваяЗапись.ВидДохода		= СпрВидов.МатпомощьПриРожденииРебенкаСтудентовИзСтудотряда;
	НоваяЗапись.НормативЗатрат	= 1000000;
	НаборЗаписей.Записать();
