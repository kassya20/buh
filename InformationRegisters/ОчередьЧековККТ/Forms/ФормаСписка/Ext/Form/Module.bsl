...
Процедура ВыбратьФискальноеУстройство()
	...
	РаспределеннаяФискализацияКлиентПереопределяемый.ПриНачалеВыбораФискальногоУстройства(ПоддерживаемыеТипы, СтандартнаяОбработка);
...
Процедура НачатьФискализациюЧеков()
	...
	Если НомерСтроки < ЧекиДляФискализации.Количество() Тогда
		...
		Если Данные.СтатусЧека = ПредопределенноеЗначение("Перечисление.СтатусЧекаККТВОчереди.Новый") 
			...
			СтандартнаяОбработка = Истина;
			РаспределеннаяФискализацияКлиентПереопределяемый.ПриНачалеФискализацииЧекаВОчереди(ОбщиеПараметры, ИдентификаторУстройстваККТ, СтандартнаяОбработка);
