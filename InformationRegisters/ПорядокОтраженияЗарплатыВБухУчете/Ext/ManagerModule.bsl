...
// Заполняет регистр на основании переданных значений аналитики учета.
//
// Параметры:
//  СпособОтраженияЗарплатыВБухучете - СправочникСсылка.СпособыОтраженияЗарплатыВБухУчете - способ отражения, устанавливаемый
//                                                                                          в новых записях.
//  СчетУчета - ПланСчетовСсылка.Хозрасчетный - счет учета, устанавливаемый в новых записях.
//  СтатьяРасходов - СправочникСсылка.СтатьиЗатрат, Справочники.ПрочиеДоходыИРасходы - статья затрат для заполнения в новых записях.
//                                                                                     Если не передана, будет заполнена по умолчанию.
//  ОсобыйПорядок - Булево - Истина для создания записей для особого порядка налогообложения.
//  ОтражениеВУСН - ПеречислениеСсылка.ОтражениеВУСН - отражение расходов в УСН и АУСН, заданное для новых записей.
//
Процедура СоздатьПоАналитике(СпособОтраженияЗарплатыВБухучете, СчетУчета, Знач СтатьяРасходов = Неопределено, ОсобыйПорядок, ОтражениеВУСН) Экспорт
	
	Если СтатьяРасходов = Неопределено Тогда
		ИмяСтатьи = СтрШаблон("ОплатаТруда%1", ?(ОсобыйПорядок, "ЕНВД", ""));
		СтатьяРасходов = Справочники.СтатьиЗатрат.СтатьяЗатратПоНазначению(ИмяСтатьи);
	КонецЕсли;
	...
	АналитикаЗатрат = Новый Соответствие;
	АналитикаЗатрат.Вставить(Тип("ПланСчетовСсылка.Хозрасчетный"), СчетУчета);
	
	Если ЗначениеЗаполнено(СтатьяРасходов) Тогда
		Если ТипЗнч(СтатьяРасходов) = Тип("СправочникСсылка.ПрочиеДоходыИРасходы") Тогда
			ВидСубконтоРасходов = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ПрочиеДоходыИРасходы;
		Иначе
			ВидСубконтоРасходов = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.СтатьиЗатрат;
		КонецЕсли;
		АналитикаЗатрат.Вставить(
			ВидСубконтоРасходов,
			СтатьяРасходов);
	...
	Запись.СпособОтраженияЗарплатыВБухУчете = СпособОтраженияЗарплатыВБухучете;
	Запись.ЕНВД = ОсобыйПорядок;
	
	Если ОсобыйПорядок Тогда
		Запись.ОтражениеВУСН = Перечисления.ОтражениеВУСН.НеПринимаются;
	Иначе
		Запись.ОтражениеВУСН = ОтражениеВУСН;
	КонецЕсли;
