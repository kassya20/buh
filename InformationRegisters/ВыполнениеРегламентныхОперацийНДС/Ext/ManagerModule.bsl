...
Процедура ОпределитьНеобходимостьКонтроляОпераций(Организация, ПериодРегистрации, ДеревоОпераций) Экспорт
	...
	УСНщикНаНДС = УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Организация, ПериодРегистрации);
	ОрганизацияПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(Организация, ПериодРегистрации) ИЛИ УСНщикНаНДС;
	УСНщикНаНДСБезВычетов = УСНщикНаНДС 
		И РегистрыСведений.НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН(Организация, ПериодРегистрации) <> Перечисления.ВидыСтавокНДС.Общая;
	РаздельныйУчетНДС               = УчетнаяПолитика.РаздельныйУчетНДС(Организация, ПериодРегистрации);
	РаздельныйУчетНДСБезВычетов     = УчетнаяПолитика.РаздельныйУчетНДСБезВычетов(Организация, ПериодРегистрации);
	...
	Запрос.УстановитьПараметр("РаздельныйУчетНДСБезВычетов", РаздельныйУчетНДСБезВычетов);
	...
	// Проверка наличия авансов и необходимости формирования сводных справок
	Если НЕ ОрганизацияПлательщикНДС Тогда
		НаличиеАвансов = Ложь;
		НеобходимостьФормированияСводныхСправок = Ложь;
	Иначе
		ПараметрыПоиска = УчетНДСБП.ПараметрыЗаполненияРегистрацииСчетовФактур();
		ПараметрыПоиска.Организация     = Организация;
		ПараметрыПоиска.НачалоПериода   = ПериодРегистрации;
		ПараметрыПоиска.КонецПериода    = КонецКвартала(ПериодРегистрации);
		ПараметрыПоиска.ЗаТекущийПериод = Истина;
	
		ТаблицаАвансов = Обработки.РегистрацияСчетовФактурНаАванс.ЗаполнитьСтроки(ПараметрыПоиска);
		НаличиеАвансов = ТаблицаАвансов.Количество() > 0;
		
		ИнформацияПоСводнымСправкам = Обработки.РегистрацияСводныхСправок.ЗаполнитьСтроки(ПараметрыПоиска);
		НеобходимостьФормированияСводныхСправок = ИнформацияПоСводнымСправкам.СписокПродажи.Количество() > 0 
			ИЛИ ИнформацияПоСводнымСправкам.СписокВозвраты.Количество() > 0;
	КонецЕсли;
	КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.РегистрацияСчетовФактурНаАванс, НаличиеАвансов);
	КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ФормированиеСводнойСправки, НеобходимостьФормированияСводныхСправок);
	...
	НаличиеНДСКРаспределению = НЕ УСНщикНаНДСБезВычетов И ЕстьНДСКРаспределению(Организация, ПериодРегистрации);
	...
	КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ПодтверждениеОплатыНДСВБюджет, ЕстьЗаявленияКПодтверждению И НЕ УСНщикНаНДСБезВычетов);
	...
	Если ДоступенАвтоматическийРасчетНДС Тогда
		...
		КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ФормированиеЗаписейКнигиПродаж,
			ОрганизацияПлательщикНДС И НЕ УСНщикНаНДСБезВычетов);
	Иначе
		...
		КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ФормированиеЗаписейКнигиПродаж,
			ОрганизацияПлательщикНДС И ЕстьРеализацияКОтражениюВКнигеПродаж И НЕ УСНщикНаНДСБезВычетов);
	КонецЕсли;

	// Необходимость подготовки к вычету НДС, включенного в стоимость
	Если УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Организация, ПериодРегистрации) Тогда 
		Если Обработки.ПодготовкаКВычетуНДСУчтенногоВСтоимости.НеобходимЗапускПомощника(Организация, ПериодРегистрации) Тогда 
			КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости, Истина);
			КонтролируемыеОперации.Вставить(Перечисления.РегламентныеОперации.ФормированиеЗаписейКнигиПокупок, Истина);
		КонецЕсли;
	КонецЕсли;
...
Функция СуществуютДокументыВводаОстатковПоРУ(Организация, Период) Экспорт
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Период", Период);
	Запрос.Текст =
	...
	|	РегламентныеОперацииНДС.Организация = &Организация
	|	И РегламентныеОперацииНДС.ПериодРегистрации <= &Период
	|	И РегламентныеОперацииНДС.РегламентнаяОперация = ЗНАЧЕНИЕ(Перечисление.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС)";
...
Функция УжеЗаполнялисьОстаткиПоРУ(Организация, Период) Экспорт
	...
	Запрос.Текст = 	
		...
		|		ПО ВыполнениеРегламентныхОперацийНДС.РегламентныйДокумент = ВводНачальныхОстатков.Ссылка
		|ГДЕ
		|	ВыполнениеРегламентныхОперацийНДС.ПериодРегистрации = &Период
		|	И ВыполнениеРегламентныхОперацийНДС.Организация = &Организация
		|	И ВыполнениеРегламентныхОперацийНДС.РегламентнаяОперация = ЗНАЧЕНИЕ(Перечисление.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС)
		|	И НЕ ВыполнениеРегламентныхОперацийНДС.РегламентныйДокумент = НЕОПРЕДЕЛЕНО
		...
		|		ПО ВыполнениеРегламентныхОперацийНДС.РегламентныйДокумент = ОперацияБух.Ссылка
		|ГДЕ
...
// 
Процедура СброситьФактВыполненияВсехРегламентныхОпераций(Организация, ПериодРегистрации) Экспорт
	...
	|	И НЕ ВыполнениеРегламентныхОперацийНДС.РегламентнаяОперация = ЗНАЧЕНИЕ(Перечисление.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС)";
	
	Набор = РегистрыСведений.ВыполнениеРегламентныхОперацийНДС.СоздатьНаборЗаписей();
	
	ВводОстатковПоРУ = Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС;
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			Для Каждого Движение Из Набор Цикл
				Если Движение.Актуальность И Движение.РегламентнаяОперация <> ВводОстатковПоРУ Тогда
...
Процедура СброситьФактВыполненияРегламентнойОперации(Организация, ПериодРегистрации, РегламентнаяОперация) Экспорт
	...
	|	ВыполнениеРегламентныхОперацийНДС.Счет КАК Счет
	|ИЗ
	|	РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК ВыполнениеРегламентныхОперацийНДС
	|ГДЕ
	|	ВыполнениеРегламентныхОперацийНДС.Организация = &Организация
...
Функция ВсеРегламентныеОперацииНДСВыполнены(Организация, ПериодРегистрации)
	...
	// Необходимо ввести остатки по РУ  
	СтруктураКонтроля = НеобходимостьДействийПоВводуОстатковПоРУ(Организация, ПериодРегистрации);
	Если СтруктураКонтроля.ТребуетсяВыполнитьВводОстатковПоРУ Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ОпределитьСостояниеОпераций(Организация, ПериодРегистрации, ДеревоОпераций)
	...
	// Ввод остатков по раздельному учету 
	СтруктураКонтроля = НеобходимостьДействийПоВводуОстатковПоРУ(Организация, ПериодРегистрации);
	
	ТребуетсяКонтрольПоВводуОстатковПоРУ  = СтруктураКонтроля.ТребуетсяКонтрольПоВводуОстатковПоРУ;
	ТребуетсяВыполнитьВводОстатковПоРУ    = СтруктураКонтроля.ТребуетсяВыполнитьВводОстатковПоРУ;
	...
	ТребуетсяПодготовкаКВычетуНДСУчтенногоВСтоимости = 
		Обработки.ПодготовкаКВычетуНДСУчтенногоВСтоимости.НеобходимЗапускПомощника(Организация, ПериодРегистрации);
	...
	Для Каждого СтрокаУзла ИЗ ДеревоОпераций Цикл
		...
		Если РегламентнаяОперация = Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС Тогда
			
			СтрокаУзла.Контроль = ТребуетсяКонтрольПоВводуОстатковПоРУ;
			
			Если ТребуетсяКонтрольПоВводуОстатковПоРУ Тогда
				
				СтрокаУзла.Выполнена = НЕ ТребуетсяВыполнитьВводОстатковПоРУ;
				СтрокаУзла.Актуальна = НЕ ТребуетсяВыполнитьВводОстатковПоРУ;
		...
		ИначеЕсли РегламентнаяОперация = Перечисления.РегламентныеОперации.ПереходНаРаздельныйУчетНДСНаСчете19 Тогда
		...
		ИначеЕсли РегламентнаяОперация = Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости
			И ТребуетсяПодготовкаКВычетуНДСУчтенногоВСтоимости Тогда 
			
			Запрос.Текст = 
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	ВыполнениеРегламентныхОперацийНДС.Актуальность КАК Актуальность
			|ИЗ
			|	РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК ВыполнениеРегламентныхОперацийНДС
			|ГДЕ
			|	ВыполнениеРегламентныхОперацийНДС.РегламентнаяОперация = ЗНАЧЕНИЕ(Перечисление.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости)
			|	И ВыполнениеРегламентныхОперацийНДС.Организация = &Организация
			|	И ВыполнениеРегламентныхОперацийНДС.ПериодРегистрации = &ДатаПерехода
			|
			|УПОРЯДОЧИТЬ ПО
			|	Актуальность УБЫВ";
			Запрос.УстановитьПараметр("ДатаПерехода",
			Обработки.ПодготовкаКВычетуНДСУчтенногоВСтоимости.ДатаПереходаНаУСНСВычетами(Организация, ПериодРегистрации));
			
			Выборка = Запрос.Выполнить().Выбрать();
			ОперацияВыполнена = Выборка.Следующий();
			СтрокаУзла.Выполнена = ОперацияВыполнена;
			СтрокаУзла.Актуальна = ОперацияВыполнена И Выборка.Актуальность;
			Продолжить;
...
Процедура ЗаполнитьЗначенияДереваОперацийНДС(ДеревоОпераций)
	...
	СтрокаУзла = ДеревоОпераций.Добавить();
	СтрокаУзла.Узел = Перечисления.РегламентныеОперации.ФормированиеНачальныхОстатковДляВеденияРаздельногоУчетаНДС;
	СписокПодчиненных.Очистить();
	СтрокаУзла.Связи = СписокПодчиненных.Скопировать();
	...
	СписокПодчиненных.Очистить();
	СписокПодчиненных.Добавить(Перечисления.РегламентныеОперации.ФормированиеДекларации);
	СтрокаУзла.Связи = СписокПодчиненных.Скопировать();
	...
	СтрокаУзла = ДеревоОпераций.Добавить();
	СтрокаУзла.Узел = Перечисления.РегламентныеОперации.ПодготовкаКВычетуНДСУчтенногоВСтоимости;
	СписокПодчиненных.Очистить();
	СписокПодчиненных.Добавить(Перечисления.РегламентныеОперации.ФормированиеЗаписейКнигиПокупок);
	СписокПодчиненных.Добавить(Перечисления.РегламентныеОперации.ФормированиеДекларации);
	СтрокаУзла.Связи = СписокПодчиненных.Скопировать();
...
Функция НеобходимостьДействийПоВводуОстатковПоРУ(Организация, Период)
	...
	Если НЕ РаздельныйУчетНаСчете19 Тогда
		Возврат Новый Структура(
			"ТребуетсяКонтрольПоВводуОстатковПоРУ,
			|ТребуетсяВыполнитьВводОстатковПоРУ",
			Ложь, Ложь);
	КонецЕсли;
	...
	Если НЕ ПереходНаУчетНДСПоСпособамВТекущемКвартале(Организация, Период, Ложь) Тогда
		Возврат Новый Структура(
		"ТребуетсяКонтрольПоВводуОстатковПоРУ,
		|ТребуетсяВыполнитьВводОстатковПоРУ",
		Ложь, Ложь);  		
	...
	ТребуетсяВыполнить = НеобходимоАктуализироватьОстаткиПоРаздельномуУчету(Организация, Период);
...
Функция НеобходимостьДействийПоПереходуНаРаздельныйУчетНДСНаСчете19(Организация, Период)
	...
	Если НЕ ПереходНаУчетНДСПоСпособамВТекущемКвартале(Организация, Период, Истина) Тогда 
		Возврат Новый Структура(
			"ТребуетсяКонтрольПереходаНаРаздельныйУчетНДСНаСчете19,
			|ТребуетсяВыполнитьПереходНаРаздельныйУчетНДСНаСчете19",
			Ложь, Ложь); 
...
Функция ПереходНаУчетНДСПоСпособамВТекущемКвартале(Организация, Период, ПереходСоСтарогоНаНовыйУчет)
	...
	|	И НастройкиУчетаНДС.РаздельныйУчетНДСДо2014Года";
	
	Результат = Запрос.ВыполнитьПакет();
	
	Если ПереходСоСтарогоНаНовыйУчет Тогда 
		ПереходНаУчетНДСПоСпособамВТекущемКвартале = НЕ Результат[0].Пустой() И Результат[1].Пустой() И Не Результат[2].Пустой();
	Иначе
	// Учетная политика есть и переход производится в текущем квартале.
		ПереходНаУчетНДСПоСпособамВТекущемКвартале = НЕ Результат[0].Пустой() И Результат[1].Пустой() И  Результат[2].Пустой();
	КонецЕсли;
...
Функция ТекстЗапросаНеобходимостьРегламентныхОпераций()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ЗНАЧЕНИЕ(Перечисление.РегламентныеОперации.РегистрацияСчетовФактурНалоговогоАгента) КАК РегламентнаяОперация
	|ИЗ
	...
	|				И НЕ ВЫРАЗИТЬ(СубконтоДт2 КАК Справочник.ДоговорыКонтрагентов).ВидАгентскогоДоговора В (&ВидыДоговоровРеализацииТоваров)) КАК УсловиеПоОборотам
	|ГДЕ
	...
	|					ИЛИ &РаздельныйУчетНДСБезВычетов)) КАК НДСНеоблагаемыеОперацииОстатки
	|ГДЕ
	...
	|			Организация = &Организация
	|				И (&РаздельныйУчетНДСНаСчете19
	|					ИЛИ &РаздельныйУчетНДСБезВычетов)) КАК НДСНеоблагаемыеОперацииОбороты
	|ГДЕ
	...
	|ГДЕ
	|	НДСЗаписиКнигиПродажОбороты.Событие В (ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.Реализация), ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ПодтвержденаСтавка0))
	|	И (НДСЗаписиКнигиПродажОбороты.СчетФактура ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|			ИЛИ НДСЗаписиКнигиПродажОбороты.СчетФактура ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|			ИЛИ НДСЗаписиКнигиПродажОбороты.СчетФактура ССЫЛКА Документ.ОплатаПлатежнойКартой)";
	
КонецФункции

Функция НеобходимоАктуализироватьОстаткиПоРаздельномуУчету(Организация, Период) Экспорт
	
	
	// Проверим, сдвинута ли граница последовательности
	ГраницаАктуальности = РаботаСПоследовательностями.МоментНачалаПерепроведения(Организация, Период);
	
	Если ГраницаАктуальности <> Неопределено Тогда
		ГраницаСдвинутаПоДате = ?(ГраницаАктуальности.Дата > Период, Ложь, Истина);
		Если ГраницаСдвинутаПоДате И НЕ ЭтоРегламентнаяОперацияНДС(ГраницаАктуальности.Ссылка) Тогда 
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	...
	// Проверка, есть ли пустое субконто на счете 19
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Организация",                 Организация);
	Запрос.УстановитьПараметр("Период",                      Новый Граница(Период - 1, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("МассивСубСчетов19НДСКВычету", БухгалтерскийУчет.МассивСубСчетов19НДСКВычету());
	...
	// Проверим актуальность операций
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Период",      Период);
	Запрос.Текст=
	...
	Если Результат.Следующий() Тогда
		НужноАктуализировать = НЕ Результат.Актуальность;
	Иначе
		НужноАктуализировать = Ложь;
	КонецЕсли;
	
	Возврат НужноАктуализировать;
		
	
	
КонецФункции

Функция ЭтоРегламентнаяОперацияНДС(РегламентныйДокумент) Экспорт
	...
	|	РегистрСведений.ВыполнениеРегламентныхОперацийНДС КАК ВыполнениеРегламентныхОперацийНДС
	|ГДЕ
	|	ВыполнениеРегламентныхОперацийНДС.РегламентныйДокумент = &РегламентныйДокумент";
	Запрос.УстановитьПараметр("РегламентныйДокумент", РегламентныйДокумент);
...
Процедура ЗафиксироватьФактВыполненияРегламентнойОперации(ПериодРегистрации,
														  ...
														  Актуальность = Истина,
														  Счет = Неопределено) Экспорт
	
	Ответственный = Пользователи.ТекущийПользователь();
	
	Счет = ?(Счет=Неопределено, ПланыСчетов.Хозрасчетный.ПустаяСсылка(), Счет);
