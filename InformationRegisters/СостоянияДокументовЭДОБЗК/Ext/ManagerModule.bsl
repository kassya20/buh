...
Функция СостояниеДокумента(СсылкаНаДокумент) Экспорт
	Если ЗначениеЗаполнено(СсылкаНаДокумент) Тогда
		...
		Возврат ОбщегоНазначения.СтруктураПоМенеджеруЗаписи(
			МенеджерЗаписи,
			Метаданные.РегистрыСведений.СостоянияДокументовЭДОБЗК);
	КонецЕсли;
	Возврат ОбщегоНазначения.СтруктураПоМенеджеруЗаписи(
		РегистрыСведений.СостоянияДокументовЭДОБЗК,
		Метаданные.РегистрыСведений.СостоянияДокументовЭДОБЗК);
КонецФункции
...
Процедура ЗаполнитьСостояниеДокументаЭДОБЗК(Запись, ДанныеДокумента)
	...
	Если ДанныеДокумента.ВариантПодписания = Перечисления.ВариантыПодписанияДокументовЭДОБЗК.НеТребуется Тогда
		Запись.СостояниеПодписей = НСтр("ru = 'Подпись не требуется'");
	Иначе
		СостояниеПодписей = "";
