...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВозможноПродлениеСроковНалоговОтчетов = (НастройкиУчетаУСН.Период >= ПериодПереносаСроков.Начало И НастройкиУчетаУСН.Период <= ПериодПереносаСроков.Окончание);
	РежимБезЗакрывающихДокументов = НЕ ПолучитьФункциональнуюОпцию("ВестиУчетРасчетовСКонтрагентами");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзменениеПатента" Тогда
		...
		Если НастройкиУчетаУСН.Организация = Параметр.Организация Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ПроведениеКлиент.НачатьОчисткуРегистраПроверенныхДокументов(
		ОповещениеОЗавершении, НастройкиУчетаУСН.Период, НастройкиУчетаУСН.Организация);
...
Процедура ДатаУплатыУСНЗа2019ГодПриИзменении(Элемент)
	НастройкиУчетаВызовСервера.ЗаписатьПродленныйСрокУплатыНалога(
		НастройкиУчетаУСН.Организация, 
		"ДатаУплатыУСНЗа2019Год", 
		ДатаУплатыУСНЗа2019Год);
КонецПроцедуры
