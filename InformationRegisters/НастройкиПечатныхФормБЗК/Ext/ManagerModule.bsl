...
Функция Настройки() Экспорт
	...
	Запрос.Текст =
		...
		|	НастройкиПечатныхФорм.ВидДокумента.ВариантПодписания КАК ВариантПодписания,
		|	НастройкиПечатныхФорм.ВидДокумента.ТипЭлектроннойПодписи КАК ТипЭлектроннойПодписи,
		...
		Для Каждого НастройкиПечатнойФормы Из НастройкиПечатныхФормПоУмолчанию Цикл
			...
			Иначе
				Для Каждого Колонка Из НастройкиПечатныхФормПоУмолчанию.Колонки Цикл
					Если Колонка.Имя = "ВариантПодписания" Тогда
						Если ЗаписьИдентификатора.УдалитьВариантПодписания <> НастройкиПечатнойФормы.ВариантПодписания Тогда
							ЗаписатьНабор = Истина;
							ЗаписьИдентификатора.УдалитьВариантПодписания = НастройкиПечатнойФормы.ВариантПодписания;
						КонецЕсли;
...
Процедура ЗаполнитьВариантПодписанияПользовательскихПечатныхФорм()
	...
	Попытка
		...
		Запрос.Текст = 
			...
			|ГДЕ
			|	НастройкиПечатныхФормБЗК.УдалитьВариантПодписания = ЗНАЧЕНИЕ(Перечисление.ВариантыПодписанияДокументовЭДОБЗК.ПустаяСсылка)";
		...
		Пока Выборка.Следующий() Цикл
			...
			НаборЗаписей.Прочитать();
			НаборЗаписей[0].УдалитьВариантПодписания = Перечисления.ВариантыПодписанияДокументовЭДОБЗК.Требуется;
			ОбновлениеИнформационнойБазы.ЗаписатьНаборЗаписей(НаборЗаписей);
...
Процедура УстановитьВариантПодписанияПечатнойФормы(ИдентификаторПечатнойФормы, ВариантПодписания) Экспорт
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		НаборЗаписей[0].УдалитьВариантПодписания = ВариантПодписания;
