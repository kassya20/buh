...
Процедура ОбновитьДанные(Организация, Дата = Неопределено, ИнтервалОбновления = 0, РазделыПерсонализированныхДанных = Неопределено) Экспорт
	...
	Для каждого Раздел из РазделыПерсонализированныхДанных Цикл
		...
		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.НастройкаПечатиАртикулов Тогда
			ТаблицаДанных = ПерсонализированныеПредложенияСервисов.ПоказательНастройкиПечатиАртикулов();
		...
		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.ЗаявкаНаЛизинг Тогда
			ПерсонализированныеПредложенияСервисов.ОбновитьДанныеДляЗаявкаНаЛизинг(ДатаПолученияДанных);
			Продолжить;

		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.ФинОтчетность Тогда
			ПерсонализированныеПредложенияСервисов.ОбновитьДанныеФинОтчетность(Организация, ДатаПолученияДанных);
			Продолжить;
			
		ИначеЕсли Раздел = Перечисления.РазделыПерсонализированныхДанных.Маркетплейс Тогда
			ТаблицаДанных = ПерсонализированныеПредложенияСервисов.ПоказательЕстьПродажиЧерезМаркетплейсВайлдберриз(Организация);
	...
	Если ДанныеКОбновлению.Количество() = 0 Тогда
		// Данных к обновлению нет, поэтому выходим.
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(Организация) Тогда
			// Блокировку устанавливаем только в случае если передана организация
