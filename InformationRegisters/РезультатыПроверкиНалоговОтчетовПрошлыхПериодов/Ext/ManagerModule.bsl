...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьПериодСобытия");
...
// Параметры:
//  Организация   - СправочникСсылка.Организации - организация, по которой сдается отчетность
//  Правило       - Массив значений типа СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов.
// 
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Период        - Дата - дата внутри границ отчетного периода
...
//                              Значение Ложь возможно только тогда, когда есть более ранние периоды, за которые отчеты нужны.
//
Функция ДобавленныеПериодыГруппыПравил(Организация, Правила) Экспорт
...
//
Функция ПроверкаВыполнена(Организация, ГосударственныйОрган = Неопределено) Экспорт
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Соответствия.ИмяИсточника КАК ИмяИсточника,
	...
	|	ВТСоответствия.ИмяИсточника КАК ИмяИсточника,
	...
	|		ПО ВТСоответствия.ИмяИсточника = ВидыОтправляемыхДокументов.Источник
	|ГДЕ
	|	&УсловиеГосударственныйОрган
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	РезультатыТеста.Организация КАК Организация
	|ИЗ
	|	РегистрСведений.РезультатыПроверкиНалоговОтчетовПрошлыхПериодов КАК РезультатыТеста
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПолучателиОтчета КАК ВТПолучателиОтчета
	|		ПО (РезультатыТеста.Правило.Код = ВТПолучателиОтчета.ИдентификаторПравила
	|				ИЛИ РезультатыТеста.Правило.Владелец.Код = ВТПолучателиОтчета.ИдентификаторЗадачи
	|					И ВТПолучателиОтчета.ИдентификаторПравила = """")
	|ГДЕ
	|	РезультатыТеста.Организация = &Организация
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если ЗначениеЗаполнено(ГосударственныйОрган) Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеГосударственныйОрган", "ВидыОтправляемыхДокументов.ТипПолучателя = &ГосударственныйОрган");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеГосударственныйОрган", "Истина");
	КонецЕсли;
...
Функция НовыйРезультатыТеста() Экспорт
	...
	Результат.Вставить("УпрощеннаяНулеваяОтчетность", Ложь);
...
Процедура ЗарегистрироватьИзменения(РезультатТеста) Экспорт
	...
	ТекущееПравило              = РезультатТеста.Правило;
	ТекущиеПравила              = РезультатТеста.Правила;
	ВсеОтчетныеПериоды          = РезультатТеста.ОтчетныеПериоды;
	УпрощеннаяНулеваяОтчетность = РезультатТеста.УпрощеннаяНулеваяОтчетность;
	
	НачалоОбзора = РегистрыСведений.ПерваяОтчетнаяКампания.ПервыйПериодКампании(Организация);
	
	Если ТекущиеПравила = Неопределено Тогда
		МассивПравил = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущееПравило);
	Иначе
		МассивПравил = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ТекущиеПравила.ВыгрузитьКолонку("Правило"));
	КонецЕсли;
	...
	Для Каждого Правило ИЗ МассивПравил Цикл
		...
		Если УпрощеннаяНулеваяОтчетность Тогда
			...
			ОтчетныеПериоды = ВсеОтчетныеПериоды.СкопироватьКолонки();
			
			Для Каждого СтрокаОтчетногоПериода ИЗ ВсеОтчетныеПериоды Цикл
				Если (ПериодичностьПравила = Перечисления.Периодичность.Год
					И НачалоГода(СтрокаОтчетногоПериода.Период) < НачалоГода(НачалоОбзора))
					ИЛИ (ПериодичностьПравила = Перечисления.Периодичность.Квартал
					И НачалоГода(СтрокаОтчетногоПериода.Период) < НачалоКвартала(НачалоОбзора))
					ИЛИ (ПериодичностьПравила = Перечисления.Периодичность.Месяц
					И НачалоГода(СтрокаОтчетногоПериода.Период) < НачалоМесяца(НачалоОбзора)) Тогда
					ЗаполнитьЗначенияСвойств(ОтчетныеПериоды.Добавить(), СтрокаОтчетногоПериода);
				КонецЕсли;
			КонецЦикла;
		Иначе
			ОтчетныеПериоды = ВсеОтчетныеПериоды;
		КонецЕсли;
		...
		НовыйСписокЗадач = Обработки.ПодготовкаОтчетностиПрошлыхПериодов.АктуальноеРасписаниеПоОтчетам(Организация);
		ЗаполнитьНовыеРезультатыПроверки(Организация, ОтчетныеПериоды, НовыеРезультатыПроверки, Правило, ПериодичностьПравила, НовыйСписокЗадач);
...
Процедура ЗаполнитьНовыеРезультатыПроверки(Организация, ОтчетныеПериоды, НовыеРезультатыПроверки, Правило, ПериодичностьПравила, ОбновленныйСписок)
	
	Для каждого СтрокаПериода Из ОтчетныеПериоды Цикл
		
		Если (ПериодичностьПравила = Перечисления.Периодичность.Год И НачалоГода(СтрокаПериода.Период) <> СтрокаПериода.Период)
			ИЛИ (ПериодичностьПравила = Перечисления.Периодичность.Квартал И НачалоКвартала(СтрокаПериода.Период) <> СтрокаПериода.Период)
			ИЛИ (ПериодичностьПравила = Перечисления.Периодичность.Месяц И НачалоМесяца(СтрокаПериода.Период) <> СтрокаПериода.Период) Тогда
			Продолжить;
		КонецЕсли;
		
		ПериодСобытия = РегистрыСведений.ЗадачиБухгалтера.ПериодСобытияПоНалогамИСборам(ПериодичностьПравила, СтрокаПериода.Период);
		
		ОбновленноеПравило = ОбновленныйСписок.Найти(ПериодСобытия);
		Если ЗначениеЗаполнено(ОбновленноеПравило)
			И ОбновленноеПравило.Правило <> СтрокаПериода.Правило Тогда
			СтрокаПериода.Правило = ОбновленноеПравило.Правило;
		КонецЕсли;
		
		Запись = НовыеРезультатыПроверки.Добавить();
		Запись.Организация = Организация;
		Запись.Правило = СтрокаПериода.Правило;
...
Функция ИзмененияЗадач(Организация, ОтчетныеПериоды, Правило, ПериодичностьПравила, РезультатыПроверкиДоИзменения)
	...
	Для каждого СтрокаПериода Из ОтчетныеПериоды Цикл
		...
		Если ЗначениеЗаполнено(ПредыдущаяЗапись) Тогда
			...
			Если СтрокаПериода.Требуется <> ПредыдущаяЗапись.ТребуетсяНалогОтчет Тогда
				ДобавитьИзменениеЗадач(ИзменениеЗадач, Организация, ПериодСобытия, Правило, ПериодичностьПравила, СтрокаПериода.Требуется, Ложь);
			КонецЕсли;
	...
	Для каждого УдаляемыйПериод Из РезультатыПроверки Цикл
		Если ЗначениеЗаполнено(УдаляемыйПериод.ПериодСобытия) Тогда
			ДобавитьИзменениеЗадач(ИзменениеЗадач, Организация, УдаляемыйПериод.ПериодСобытия, Правило, ПериодичностьПравила, Ложь, Истина);
		КонецЕсли;
	КонецЦикла;
...
Функция НовыйТаблицаПериодов()
	...
	ТаблицаПериодов.Колонки.Добавить("Правило", Новый ОписаниеТипов("СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов"));
...
Процедура ЗарегистрироватьИзменениеЗадачДляНулевойОтчетности(ИзменениеЗадач)
	...
	Правила = ИзменениеЗадач.Скопировать(,"Правило, НачалоПериода");
	Правила.Колонки.НачалоПериода.Имя = "ФинансовыйПериод";
	
	// Дополняем в список задач задачи прошлого периода, добавленного по опроснику и только по конкретному набору правил
	РегистрыСведений.ЗадачиБухгалтера.ОбновитьИДополнитьРегулярныеЗадачи(Организация, НачалоОбновления, Правила);
	
	// Обновляем список задач полностью но только по состоянию на текущую дату
	// т.к. возможнотребуется добавить ранее не учтенные задачи
	РегистрыСведений.ЗадачиБухгалтера.ОбновитьИЗаполнитьРегулярныеЗадачи(Организация);
...
Процедура УдалитьИлиПоместитьВАрхивЛишниеЗадачи(ИзменениеЗадач)
	...
	|	ИзменениеЗадач.Правило КАК Правило,
	...
	|			И ВТ_ИзменениеЗадач.Правило = ЗадачиБухгалтера.Правило";
...
Процедура ДобавитьИзменениеЗадач(ТаблицаИзменений, Организация, Период, Правило, Периодичность, ЗадачиТребуются, Удалить)
...
Функция НоваяТаблицаИзмененияЗадач()
	...
	ТаблицаИзменений.Колонки.Добавить("Правило",         Новый ОписаниеТипов("СправочникСсылка.ПравилаПредставленияОтчетовУплатыНалогов"));
