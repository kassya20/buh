...
Процедура ПриЗаписиБольничного(БольничныйОбъект) Экспорт
	...
	Если ПустаяСтрока(БольничныйОбъект.НомерЛисткаНетрудоспособности)
		И ПустаяСтрока(ЗначенияРеквизитовДоЗаписи.НомерЛисткаНетрудоспособности) Тогда
		Возврат;
	КонецЕсли;
	...
	ТаблицаОтбора.Колонки.Добавить("НомерЛистка", Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(12)));
	ТаблицаОтбора.Колонки.Добавить("ГоловнаяОрганизация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	
	Если Не ПустаяСтрока(БольничныйОбъект.НомерЛисткаНетрудоспособности) Тогда
		НоваяСтрока = ТаблицаОтбора.Добавить();
		НоваяСтрока.НомерЛистка = БольничныйОбъект.НомерЛисткаНетрудоспособности;
		НоваяСтрока.ГоловнаяОрганизация = ГоловнаяОрганизация;
	КонецЕсли;
	Если Не ПустаяСтрока(ЗначенияРеквизитовДоЗаписи.НомерЛисткаНетрудоспособности)
		И (ЗначенияРеквизитовДоЗаписи.НомерЛисткаНетрудоспособности <> БольничныйОбъект.НомерЛисткаНетрудоспособности 
			Или ЗначенияРеквизитовДоЗаписи.ГоловнаяОрганизация <> ГоловнаяОрганизация) Тогда
		НоваяСтрока = ТаблицаОтбора.Добавить();
		НоваяСтрока.НомерЛистка = ЗначенияРеквизитовДоЗаписи.НомерЛисткаНетрудоспособности;
		НоваяСтрока.ГоловнаяОрганизация = ЗначенияРеквизитовДоЗаписи.ГоловнаяОрганизация;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаНомеров.НомерЛистка КАК НомерЛистка,
	|	ТаблицаНомеров.ГоловнаяОрганизация КАК ГоловнаяОрганизация
	|ПОМЕСТИТЬ ТаблицаОтбора
	|ИЗ
	|	&ТаблицаОтбора КАК ТаблицаНомеров
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО СведенияОбЭЛН.НомерЛисткаНетрудоспособности = ТаблицаОтбора.НомерЛистка
	|			И СведенияОбЭЛН.ГоловнаяОрганизация = ТаблицаОтбора.ГоловнаяОрганизация";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
...
Процедура ПриЗаписиДанныхСФРДляРасчетаПособия(ДокументОбъект) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	
	ЗначенияДоЗаписи = ДокументОбъект.ДополнительныеСвойства.ЗначенияРеквизитовДоЗаписи;
	
	НомерЛН             = ДокументОбъект.НомерЛН;
	ГоловнаяОрганизация = ДокументОбъект.ГоловнаяОрганизация;
	...
	Если ЗначениеЗаполнено(НомерЛНДоЗаписи)
		...
		Если НаборЗаписей <> Неопределено Тогда
			...
			Иначе
				Запись = НаборЗаписей[0];
				ОбновитьСсылкуНаДанныеСФРДляРасчетаПособия(Запись);
				НаборЗаписей.ДополнительныеСвойства.Вставить("Событие", "ПриЗаписиДанныхСФРДляРасчетаПособия");
				ЗавершитьЗаписьНабора(НаборЗаписей);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьФлажокИзмененияПринятыИПричинуИсправленияИзПоследнегоРеестра(Запись)
	...
	Если Запись.СостояниеФСС = Перечисления.СостоянияЭЛНВФСС.Аннулирован
		Или Запись.СостояниеФСС = Перечисления.СостоянияЭЛНВФСС.ВозвращенМО Тогда
		// Действия прекращены.
...
Процедура ОбновитьСсылкуНаДанныеСФРДляРасчетаПособия(Запись)
	ДанныеСообщения = ПоследнийДокументДанныеСФРДляРасчетаПособия(
		Запись.НомерЛисткаНетрудоспособности,
		Запись.ГоловнаяОрганизация);
	Если ДанныеСообщения = Неопределено Тогда
		Запись.ДанныеСФРДляРасчетаПособия = Неопределено;
	Иначе
		Запись.ДанныеСФРДляРасчетаПособия = ДанныеСообщения.Ссылка;
	КонецЕсли;
	
	Если Не Запись.ДоступенИсходныйXML
		И Не ЗначениеЗаполнено(Запись.Больничный)
		И Не ЗначениеЗаполнено(Запись.ВходящийЗапрос)
		И Не ЗначениеЗаполнено(Запись.СообщениеОСтраховомСлучае)
		И ЗначениеЗаполнено(Запись.ДанныеСФРДляРасчетаПособия)
		И ДанныеСообщения <> Неопределено Тогда
		Запись.Организация         = ДанныеСообщения.Организация;
		Запись.Страхователь        = ДанныеСообщения.Страхователь;
		Запись.Сотрудник           = ДанныеСообщения.Сотрудник;
...
Функция ПоследнийДокументДанныеСФРДляРасчетаПособия(НомерЛН, ГоловнаяОрганизация)
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Шапка.Ссылка КАК Ссылка,
	|	Шапка.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	|	Шапка.НомерЛН КАК НомерЛН,
