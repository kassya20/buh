...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОчиститьРеквизитыЗаписейПоВсемНалогамИВзносам");
...
Функция ДанныеЗаполнения(КлючЗаписи, Период, Организация, ВидНалоговогоОбязательства = Неопределено, ПоказателиПериода = Неопределено, ЗаполнятьСумму = Ложь) Экспорт
	...
	// При получении настроек по налогу, необходимо получить записи в т.ч. и по "пустой" организации - которые означают,
	// что такая настройка подходит для всех организаций. Для этого выполним запрос привилегированном режиме.
	// Недоступная для пользователя информация получена не будет, т.к. в запрос,
	// в качестве параметра, всегда передается Организация, которая разрешена пользователю, либо пустая.
	РезультатЗапроса = Запрос.Выполнить();
	УстановитьПривилегированныйРежим(Ложь);
	
	Настройка = РезультатЗапроса.Выбрать();
	...
	ПлатежиВБюджетКлиентСервер.ПроверитьИсправитьНедопустимыеРеквизитыПлатежаВБюджет(
		Настройка.ВидПеречисленияВБюджет, Период, ДанныеЗаполнения.СтатусСоставителя, ДанныеЗаполнения.ПоказательОснования);
...
Функция КлючНастройкиУплатыНалога(Налог, Организация, РегистрацияВНалоговомОргане) Экспорт
	...
	// что такая настройка подходит для всех организаций. Для этого выполним запрос привилегированном режиме.
	// Недоступная для пользователя информация получена не будет, т.к. в запрос,
	// в качестве параметра, всегда передается Организация, которая разрешена пользователю.
	РезультатЗапроса = Запрос.Выполнить();
	УстановитьПривилегированныйРежим(Ложь);
