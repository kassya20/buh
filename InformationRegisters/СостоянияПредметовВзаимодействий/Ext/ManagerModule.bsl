...
// Обработчик обновления БП.
// Очищает записи, у которых измерение Предмет имеет тип Строка.
Процедура ОчиститьЗаписиПоПредметуТипаСтрока(Параметры) Экспорт
	...
	|	РегистрСведений.СостоянияПредметовВзаимодействий КАК СостоянияПредметовВзаимодействий
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(СостоянияПредметовВзаимодействий.Предмет) = ТИП(СТРОКА)";
	...
	Пока Истина Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Исключение
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось очистить запись по предмету %1 по причине:
						|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Выборка.Предмет,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
					УровеньЖурналаРегистрации.Предупреждение,
					Метаданные.РегистрыСведений.СостоянияПредметовВзаимодействий,
					,
					ТекстСообщения);
			КонецПопытки;
		КонецЦикла;
		...
		Если ОбработаноОбъектов > 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ОчиститьЗаписиПоПредметуТипаСтрока: удалено записей: %1'", 
		...
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ОчиститьЗаписиПоПредметуТипаСтрока: не удалось удалить записей: %1'", 
					ОбщегоНазначения.КодОсновногоЯзыка()), 
				Выборка.Количество());
