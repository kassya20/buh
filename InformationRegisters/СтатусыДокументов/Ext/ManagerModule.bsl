...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаполнитьСтатусыСчетовФактурПоДокументамПоступленияДопРасходов");
	Методы.Вставить("ПерезаполнитьСтатусыЧековНПД");
	Методы.Вставить("ЗаполнитьСтатусыСчетовФактурПоДокументамПоступления");
	Методы.Вставить("ЗаполнитьСтатусыОплатыСчетов");
	Методы.Вставить("ЗаполнитьСтатусыСчетовФактурПоДокументамРеализация");
	Методы.Вставить("ЗаполнитьДополнительныеСтатусыСчетовНаОплату");
	Методы.Вставить("ЗаполнитьДополнительныеСтатусыСчетовОтПоставщиков");
	Методы.Вставить("ЗаполнитьНомераСчетовФактурПоДокументамРеализация");
	Методы.Вставить("ИнформацияПоНепредоставленнымЧекамНПД", Истина);
...
Функция УстановитьСтатусыДокументов(МассивДокументов, СтатусыДокумента) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ИзменитьСтатус Тогда
			УчетОригиналовПервичныхДокументовБП.ИзменитьСостояниеОригиналовДокументовПриИзмененииСтатуса(
				Выборка.Документ, СтатусыДокумента.Статус);
		КонецЕсли;
...
Функция ПолучитьСтатусыДокументов(МассивДокументов, Организация = Неопределено) Экспорт
	...
	|	СтатусыДокументов.СтатусОригиналаСчетаФактуры КАК СтатусОригиналаСчетаФактуры,
	|	СтатусыДокументов.СтатусЧекаНПД КАК СтатусЧекаНПД
	|ИЗ
...
Функция НовыеСтатусыДокумента() Экспорт
	...
	СтатусыДокумента.Вставить("СтатусЧекаНПД", Неопределено);
	
	Возврат СтатусыДокумента;
	
КонецФункции

// Возвращает информацию по документам с непредоставленными чеками НПД
//
// Параметры:
//  Контрагент - СправочникСсылка.Контрагенты - документы ищутся с отбором по переданному контрагенту
//  Период - Дата, Неопределено - поиск выполняется за последний год от переданной даты, но не позже конца прошлого
//           месяца от даты сеанса
//  ВыполнятьТолькоПроверкуНаличияДокументов - Булево - если Истина, то возвращается только информация о том,
//           есть ли хоть один документ с непредоставленным чеком НПД         
//
// Возвращаемое значение:
//  Структура - см. НовыйИнформацияПоНеПредоставленнымЧекамНПД
//
Функция ИнформацияПоНепредоставленнымЧекамНПД(Контрагент, ВыполнятьТолькоПроверкуНаличияДокументов = Истина) Экспорт
	
	Информация = НовыйИнформацияПоНеПредоставленнымЧекамНПД();
	
	Если Не ПравоДоступа("Чтение", Метаданные.Документы.ПоступлениеТоваровУслуг)
		Или Не ПравоДоступа("Чтение", Метаданные.Документы.ПоступлениеДопРасходов) Тогда
		Возврат Информация;
	КонецЕсли;
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ИСТИНА КАК ЕстьДокументыБезЧеков
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|		ИЛИ СтатусыДокументов.СтатусЧекаНПД = ЗНАЧЕНИЕ(Перечисление.СтатусыЧековНПД.УтраченСтатусПлательщикаНПД))
		...
		|		И СтатусыДокументов.Организация В (&ДоступныеОрганизации)
		...
		|		КОНЕЦ МЕЖДУ &НачалоПериода И &КонецПериода
		|		И СтатусыДокументов.Организация В (&ДоступныеОрганизации)
		|		И ПоступлениеДопРасходов.Проведен) КАК ВложенныйЗапрос";
	
	СхемаЗапроса = СхемыЗапросов.Создать(ТекстЗапроса);
	Если Не ВыполнятьТолькоПроверкуНаличияДокументов Тогда
		ОсновнойОператор = СхемыЗапросов.НайтиТаблицуПоПсевдониму(СхемаЗапроса, "ВложенныйЗапрос").Оператор;
		ВложенныйЗапрос = ОсновнойОператор.Источники[0].Источник.Запрос;
		Для Каждого Оператор Из ВложенныйЗапрос.Операторы Цикл
			Оператор.ВыбираемыеПоля.Добавить("СтатусыДокументов.Организация");
			Оператор.ВыбираемыеПоля.Удалить(0);
		КонецЦикла;
		ВложенныйЗапрос.Колонки[0].Псевдоним = "Организация";
		ОсновнойОператор.КоличествоПолучаемыхЗаписей = Неопределено;
		ОсновнойОператор.ВыбиратьРазличные = Истина;
		ОсновнойОператор.ВыбираемыеПоля.Добавить("Организация");
		ОсновнойОператор.ВыбираемыеПоля.Удалить(0);
	КонецЕсли;
	...
	Информация.ЕстьДокументыБезЧеков = Не РезультатЗапроса.Пустой();
	Информация.НачалоПериода = ПериодПроверки.ДатаНачала;
	Информация.КонецПериода = ПериодПроверки.ДатаОкончания;
	Если Не ВыполнятьТолькоПроверкуНаличияДокументов И Информация.ЕстьДокументыБезЧеков Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			Информация.Организации.Добавить(Выборка.Организация);
		КонецЦикла;
	КонецЕсли; 
...
// Отложенный обработчик обновления: Заполняет ресурс СтатусЧекаНПД новым типом значения
// ПеречислениеСсылка.СтатусыЧековНПД вместо устаревшего типа ПеречислениеСсылка.СтатусыДокументовПоступления,
// в случае, если значение ресурса было ОригиналПолучен
//
// Параметры:
...
//
Процедура ПерезаполнитьСтатусыЧековНПД(Параметры) Экспорт
	
	Курсор = Неопределено;
	Параметры.Свойство("Курсор", Курсор);
	
	ШаблонТекстаЗапроса =
		"ВЫБРАТЬ ПЕРВЫЕ 1000
		|	ВложенныйЗапрос.Организация,
		|	ВложенныйЗапрос.Документ,
		|	ВложенныйЗапрос.Курсор КАК Курсор
		|ИЗ
		|	(ВЫБРАТЬ
		|		СтатусыДокументов.Организация,
		|		СтатусыДокументов.Документ,
		|		СтатусыДокументов.Документ.МоментВремени КАК Курсор
		|	ИЗ
		|		РегистрСведений.СтатусыДокументов КАК СтатусыДокументов
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
		|			ПО СтатусыДокументов.Документ = ПоступлениеТоваровУслуг.Ссылка
		|			И
		|				ПоступлениеТоваровУслуг.ДоговорКонтрагента.НалогообложениеКонтрагента = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрофессиональныйДоход)
		|	ГДЕ
		|		СтатусыДокументов.Документ ССЫЛКА Документ.ПоступлениеТоваровУслуг
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|	ВЫБРАТЬ
		...
		|			ПО СтатусыДокументов.Документ = ПоступлениеДопРасходов.Ссылка
		|			И
		|				ПоступлениеДопРасходов.ДоговорКонтрагента.НалогообложениеКонтрагента = ЗНАЧЕНИЕ(Перечисление.СистемыНалогообложения.НалогНаПрофессиональныйДоход)
		|	ГДЕ
		|		СтатусыДокументов.Документ ССЫЛКА Документ.ПоступлениеДопРасходов) КАК ВложенныйЗапрос
		|
		|УПОРЯДОЧИТЬ ПО
		|	Курсор УБЫВ";
	...
	Иначе
		...
		ВложенныйЗапрос = ОсновнойОператор.Источники[0].Источник.Запрос;
		Для Каждого Оператор Из ВложенныйЗапрос.Операторы Цикл
			Оператор.Отбор.Добавить("СтатусыДокументов.Документ.МоментВремени < &Курсор");
		КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		...
		Параметры.Вставить("Курсор", Курсор);
		...
		Попытка
			...
			ЭлементБлокировкиДанных.УстановитьЗначение("Документ", Выборка.Документ);
			...
			Для Каждого Запись Из НаборЗаписей Цикл
				Если Запись.УдалитьСтатусЧекаНПД = Перечисления.СтатусыДокументовПоступления.ОригиналПолучен Тогда
					Запись.СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.Получен;
				Иначе
					Запись.СтатусЧекаНПД = Перечисления.СтатусыЧековНПД.НеПолучен;
				КонецЕсли;
			КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать записи при выполнении процедуры РегистрыСведений.СтатусыДокументов.ПерезаполнитьСтатусыЧековНПД() по причине:
					|%2'"),
					ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				РегистрыСведений.СтатусыДокументов,
				,
				ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
	
	ИмяПроцедурыОбновления = НСтр("ru = 'РегистрыСведений.СтатусыДокументов.ПерезаполнитьСтатусыЧековНПД'");
...
Функция СтатусНовогоДокумента(Документ)
	...
	Результат = Новый Структура("Статус, ДополнительныйСтатус, СтатусОригиналаСчетаФактуры, СтатусЧекаНПД");
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Или ТипДокумента = Тип("ДокументСсылка.АктСверкиВзаиморасчетов") Тогда
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ВыплатыСамозанятым") Тогда
		
		Результат.Статус = Перечисления.СтатусыДокументовВыплатыСамозанятым.СтатусНовогоДокумента();
		
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.КоммерческоеПредложение") Тогда
		
		Результат.Статус = Перечисления.СтатусыКоммерческогоПредложения.СтатусНовогоДокумента();
...
// Обработка обновления ИБ. Заполняет номера счетов-фактур по документам "Реализация".
//
Процедура ЗаполнитьНомераСчетовФактурПоДокументамРеализация(Параметры) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("СтатусПоУмолчанию", Перечисления.СтатусыДокументовРеализации.СтатусНовогоДокумента());
	Запрос.Параметры.Вставить("ДополнительныйСтатусПоУмолчанию",  Перечисления.СтатусыОтгрузки.СтатусНовогоДокумента());
	Запрос.Параметры.Вставить("СтатусСчетаФактурыПоУмолчанию", Перечисления.СтатусыСчетаФактуры.ПустаяСсылка());
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	РеализацияТоваровУслуг.Организация КАК Организация,
	|	РеализацияТоваровУслуг.Ссылка КАК Документ,
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заполнить номер счета-фактуры для документа %1 по причине:
				|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				Выборка.Документ,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедура ЗаполнитьНомераСчетовФактурПоДокументамРеализация: не удалось обработать документов: %1'", 
			ОбщегоНазначения.КодОсновногоЯзыка()), 
			Выборка.Количество());
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Функция НовыйИнформацияПоНеПредоставленнымЧекамНПД()
	
	Результат = Новый Структура();
	Результат.Вставить("ЕстьДокументыБезЧеков", Ложь);
