...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустой() Тогда
		УстановитьСрокУплатыНалогаАвансов(ЭтотОбъект);
	ИначеЕсли ВозможенПереносСроковУплатыАвансов Тогда
		ПрочитатьСрокиУплатыАвансов();
	КонецЕсли;
	
	УстановитьГоловнуюОрганизацию(ЭтотОбъект);
	НастроитьСрокУплатыНалогаМесяцев(ЭтотОбъект);
	ОбновитьОписаниеСрокаУплаты(ЭтотОбъект);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если КодНалога = ЗадачиБухгалтераКлиентСервер.КодЗадачиТранспортныйНалог() Тогда 
		ПереходНаНовыеНастройки = РегистрыСведений.ПорядокУплатыНалоговПоРегионам.НачалоИспользованияНастроек(КодНалога);
		Если Запись.Период >= ПереходНаНовыеНастройки Тогда
			ТекстСообщения =  СтрШаблон(НСтр("ru = 'С %1 года данные настройки по транспортному налогу не используются.
				|Используйте настройку ""Уплата авансовых платежей"" (Налоги и отчеты - Транспортный налог)'"),
				Формат(ПереходНаНовыеНастройки, "ДФ=yyyy"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Запись.Период", "", Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СрокУплатыНалога.Видимость = Форма.НастраиваетсяСрокУплатыНалога;
	Элементы.ОбычныйСрокУплатыАванса.Видимость = Форма.НастраиваетсяСрокУплатыАвансов;
	Элементы.ОбычныйСрокУплатыАванса.Доступность = Запись.УплачиваютсяАвансы;
	Элементы.ОписаниеСрокаУплатыАванса.Видимость = Не Запись.СрокУплатыУстанавливаетсяДляКаждогоАванса;
	
	НастраиваетсяСрокУплатыКаждогоАванса = Форма.НастраиваетсяСрокУплатыАвансов И Форма.ВозможенПереносСроковУплатыАвансов;
	
	Элементы.НадписьОбщийСрокУплатыАвансов.Видимость = НастраиваетсяСрокУплатыКаждогоАванса; 
	Элементы.СрокУплатыУстанавливаетсяДляКаждогоАванса.Видимость = НастраиваетсяСрокУплатыКаждогоАванса; 
	Элементы.СрокУплатыУстанавливаетсяДляКаждогоАванса.Доступность = Запись.УплачиваютсяАвансы; 
	Элементы.ПереносСроковУплатыАвансов.Видимость = НастраиваетсяСрокУплатыКаждогоАванса; 
