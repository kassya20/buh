...
//
Процедура ЗаписатьРасчетСтавокНалога(Движения, СтавкиНалога, ПротоколРасчетаСтавокНалога, ПериодРасчета, Организация, ОтчетнаяДата = '0001-01-01', НомерРасчета = 0) Экспорт
	...
	Для Каждого МетаданныеВидСтавки Из Метаданные.Перечисления.ВидыСтавокОтложенногоНалога.ЗначенияПеречисления Цикл
		...
		Если Не ЗначениеЗаполнено(СтавкаНалога) И Не ЗначениеЗаполнено(АлгоритмРасчета) Тогда
			Продолжить;
		КонецЕсли;
		...
		КонецЕсли;
		Запись.НомерРасчета = НомерРасчета;
...
Функция ПрочитатьСтавки(ОтчетныйПериод, Организация, ФормаПредставленияРезультата = "Доля", ОтборВидСтавки = Неопределено) Экспорт
	...
	ОбязательныеСтавки = Новый Соответствие;
	ОбязательныеСтавки.Вставить(Перечисления.ВидыСтавокОтложенногоНалога.ПредыдущаяОценка, Истина);
	ОбязательныеСтавки.Вставить(Перечисления.ВидыСтавокОтложенногоНалога.ТекущийНалог,     Истина);
	ОбязательныеСтавки.Вставить(Перечисления.ВидыСтавокОтложенногоНалога.БудущийНалог,     Истина);
	...
	Для Каждого МетаданныеВидСтавок Из Метаданные.Перечисления.ВидыСтавокОтложенногоНалога.ЗначенияПеречисления Цикл
		...
		Если СтавкиНалога[ВидСтавки] = Неопределено И ОбязательныеСтавки[ВидСтавки] <> Неопределено Тогда
