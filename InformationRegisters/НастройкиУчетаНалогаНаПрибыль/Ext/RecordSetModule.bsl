...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		ИнвестиционныйНалоговыйВычет.ПроверитьКорректностьПериода(Запись, Запись.Период, Отказ);
	КонецЦикла;
...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если Количество() <> 0 Тогда
		Если ЭтотОбъект[0].ФормироватьСтоимостьПоПравиламБУ Тогда
			ОбщегоНазначенияБП.ЗаписатьОперациюБизнесСтатистики(
				"СтатистикаБП.НастройкиУчетаНалогаНаПрибыль.ФормироватьСтоимостьПоПравиламБУ");
		Иначе
			ОбщегоНазначенияБП.ЗаписатьОперациюБизнесСтатистики(
				"СтатистикаБП.НастройкиУчетаНалогаНаПрибыль.РучноеЗаполнениеСпискаПрямыхРасходов");
		КонецЕсли;
	КонецЕсли;
