...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("УдалитьВремяИзДатыИОчиститьГруппу");
...
Процедура ОбновитьГраницуАктуальности(Организация, Период, ВидОперации, ПропуститьПроверкуСтатусаПредыдущих = Ложь) Экспорт
	...
	Если Не УжеБылаУстаревшей Или ПропуститьПроверкуСтатусаПредыдущих Тогда
        // Помечаем устаревшими все, начиная с текущей операции.
	...
	Если УжеБылаУстаревшей И (ПропуститьПроверкуСтатусаПредыдущих
		Или ЗакрытиеМесяца.ПредыдущиеОперацииВыполнены(Период, Организация, ВидОперации)) Тогда
		// Помечаем текущую операцию как актуальную, чтобы было видно, что действие над ней успешно.
...
Процедура СдвинутьГраницуАктуальностиНазад(Организация, Период, НомерГруппы, ВидОперации, ТекущаяОперацияУстарела = Ложь) Экспорт
	...
	|			ИЛИ РегламентнаяОперация.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийРегламентныхОпераций.Выполняется))
	...
	НеАктуальныеУстаревшие = РегистрыСведений.НеАктуальныеРегламентныеОперации.СоздатьНаборЗаписей();
	// Допускаем ситуацию, что мы записываем "битую ссылку" на документ, который параллельно будет физически удален.
	НеАктуальныеУстаревшие.ДополнительныеСвойства.Вставить("НеВыполнятьКонтрольУдаляемых", Истина);
	
	ОтборЗаписи = НеАктуальныеУстаревшие.Отбор;
	Пока Выборка.Следующий() Цикл
		
		ОтборЗаписи.Организация.Установить(Выборка.Организация);
		// Группа в записях регистра всегда 0. Некое число будет только в блокировках.
		ОтборЗаписи.Дата.Установить(Выборка.Дата);
		ОтборЗаписи.ВидОперации.Установить(Выборка.ВидОперации);
		
		ЗаписьНабора = НеАктуальныеУстаревшие.Добавить();
		ЗаписьНабора.Организация = Выборка.Организация;
		ЗаписьНабора.Дата        = Выборка.Дата;
		ЗаписьНабора.ВидОперации = Выборка.ВидОперации;
		
		НеАктуальныеУстаревшие.Записать(Истина);
		НеАктуальныеУстаревшие.Очистить();
