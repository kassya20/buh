...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Если форма была открыта не из помощника установки мобильного приложения, отобразим ссылку для перехода в помощник.
	Если Не Параметры.СкрыватьКомандуПереходаВПомощник Тогда
		УстановитьСсылкуПереходаВПомощникУстановкиМобильногоПриложения();
	КонецЕсли;
...
&НаСервере
Процедура УстановитьСсылкуПереходаВПомощникУстановкиМобильногоПриложения()
	
	Команда = Неопределено;
	
	Если ПолучитьФункциональнуюОпцию("УстановкаМобильногоПриложенияСканированиеЧеков") Тогда
		Команда = СтрШаблон(
			"e1cib/command/ОбщаяКоманда.%1",
			Метаданные.ОбщиеКоманды.УстановкаМобильногоПриложенияСканированиеЧеков.Имя);
	ИначеЕсли ПолучитьФункциональнуюОпцию("УстановкаМобильногоПриложенияСканированиеЧековПростойИнтерфейс") Тогда
		Команда = СтрШаблон(
			"e1cib/command/Обработка.ПанелиПростойИнтерфейс.Команда.%1",
			Метаданные.Обработки.ПанелиПростойИнтерфейс.Команды.МобильноеПриложениеСканированиеЧеков.Имя);
	КонецЕсли;
	...
	ШаблонПодсказки = НСтр("ru = 'Сотрудники, не имеющие доступа в %1, могут получить коды доступа для использования
		|<a href=''%2''>мобильного приложения 1С:Сканер чеков</a>'");
