...
// Определяет, разрешено ли мобильному приложению 1С:Сканер чеков подключение по логину и паролю.
// 
// Возвращаемое значение:
//  Булево - Истина, если разрешено подключение по логину и паролю.
//
Функция ДоступноПодключениеПоЛогинуИПаролю() Экспорт
	
	// Подключение по логину и паролю доступно всем пользователям Fresh.
	Возврат ОбщегоНазначения.РазделениеВключено() И
		РегистрыСведений.КассовыеЧекиПодотчетныхЛицЗапросыДетальныхДанных.МенеджерСервисаНастроен();
	
КонецФункции

// Вызывается при подключении мобильного приложения 1С:Сканер чеков.
...
// Определяет наличие подключения с мобильным приложением 1С:Сканер чеков у текущего пользователя.
// 
// Возвращаемое значение:
...
// Определяет наличие подключения с мобильным приложением 1С:Сканер чеков у любого из пользователей.
// 
// Возвращаемое значение:
...
// Отправляет на Шину запрос на получение сведений тарификации.
//
Процедура ЗапроситьБаланс() Экспорт
	
	Если ПроверятьСообщения() Тогда
		ТарификацияПриложенияСканированияЧеков.ЗапроситьСостояниеБаланса(ИмяМобильногоПриложения());
	КонецЕсли;
	
КонецПроцедуры

// Возвращает текущее состояние баланса пользователя.
// 
// Возвращаемое значение:
//  Структура - см. Тарификация.НовыйДанныеТарификации()
//
Функция СостояниеБаланса() Экспорт
	
	Возврат ТарификацияПриложенияСканированияЧеков.Прочитать();
	
КонецФункции

#Область ШинаМобильныхПриложений

// Использование шины мобильных приложений для приложения 1С:Сканер чеков

#Область ПодключениеПодсистемы

// Методы раздела - это обязательный API для подключения к шине

// Имя мобильного приложения 1С:Сканер чеков в шине мобильных приложений
// 
// Возвращаемое значение:
//  Строка
//
...
// Обрабатывает сообщение шины мобильных приложений -
// 1) сохраняет содержащийся в нем QR-код кассового чека подотчетного лица, если сообщение получено
// от мобильного приложения
// 2) загружает сведения о тарификации, если сообщение получено от Шины
//
// Параметры:
//  Отправитель - Строка - имя сервиса тарификации или код доступа пользователя, отсканировавшего QR-код
...
Процедура ОбработатьСообщениеШиныМобильныхПриложений(Отправитель, Сообщение) Экспорт
	...
	Если Отправитель = ТарификацияПриложенияСканированияЧеков.ИмяСервисаТарификации() Тогда
		// Сообщение от сервиса тарификации.
		ТарификацияПриложенияСканированияЧеков.ОбработатьСообщение(Сообщение);
		Возврат;
	КонецЕсли;
	
	// Сообщение от мобильного приложения.
	ОбработатьСообщение(Отправитель, Сообщение);
...
Функция КодДоступа(Пользователь = Неопределено) Экспорт
	...
	КодДоступа = ШинаПрикладныхПриложений.КодДоступа(ИмяМобильногоПриложения(), НачальныеДанные);
...
Функция УдалитьКодДоступа(Пользователь) Экспорт
	...
	Для Каждого Идентификатор Из ИдентификаторыПользователя Цикл
		...
		КодУдален = ШинаПрикладныхПриложений.УдалитьКодДоступа(ИмяМобильногоПриложения(), Запись.КодДоступа);
		Если Не КодУдален Тогда
			Продолжить;
		КонецЕсли;
...
// Инициирует получение сообщений из Шины при активности пользователя, связанной с чеками подотченых лиц.
//
// Параметры:
//  ВыполнитьАсинхронно - Булево - если синхронно, то проверка сообщений выполняется с запуском фонового задания.
// 
// Возвращаемое значение:
//  Булево - Истина, если регулярная проверка сообщений имеет смысл.
//
Функция НачатьПроверкуСообщений(ВыполнитьАсинхронно = Истина, ПроверятьТаймаут = Истина) Экспорт
	...
	Если ПроверятьТаймаут Тогда
		ДатаСледующегоОбновления = УчетКассовыхЧековПодотчетныхЛиц.ДатаСледующегоОбновленияКассовыхЧеков();
		ПроверятьНовыеСообщения = ТекущаяУниверсальнаяДата() >= ДатаСледующегоОбновления;
		
		Если Не ПроверятьНовыеСообщения Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
	
	Если ВыполнитьАсинхронно Тогда
		ПолучитьСообщенияОтложенно();
		Возврат Истина;
	КонецЕсли;
	
	// Синхронное получение сообщений.
	ТаймаутСоединения = 5; // небольшой таймаут для синхронного получения данных
	ПолучитьСообщения(ТаймаутСоединения);
	
	Возврат Истина;
	
КонецФункции

// Проверяет, можем ли ожидать, что получим какие-либо данные с Шины.
//
Функция ПроверятьСообщения() Экспорт
...
Процедура ПолучитьСообщения(ТаймаутСоединения = Неопределено) Экспорт
	
	Результат = ШинаПрикладныхПриложений.ПолучитьСообщенияШины(ИмяМобильногоПриложения(), ТаймаутСоединения);
	СохранитьДатуСледующегоОбновленияКассовыхЧеков(Результат.ТаймаутСледующегоОпроса);
...
Процедура ОбработатьСообщение(Отправитель, Сообщение)
	
	Приложение = ИмяМобильногоПриложения();
	
	ДанныеПользователя = ПользовательПоКодуДоступа(Отправитель);
	...
	Если Не ДанныеПользователя.ЭтоПользовательКлиентскогоПриложения Тогда
		УстановитьМобильноеПриложениеПодключено(ДанныеПользователя.Пользователь);
	КонецЕсли;
	...
	Если Сообщение.Свойство("QRКод") Тогда
		ЗагрузитьQRКодыИзШиныМобильныхПриложений(Сообщение, ДанныеПользователя.Пользователь);
		СодержитДанные = Истина;
	КонецЕсли;
	
	Если Сообщение.Свойство("КассовыйЧек") Тогда
		ЗагрузитьКассовыйЧекИзШиныМобильныхПриложений(Сообщение, ДанныеПользователя.Пользователь);
		СодержитДанные = Истина;
	КонецЕсли;
...
Функция ПользовательПоКодуДоступа(КодДоступа)
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	МобильноеПриложение.Пользователь КАК Пользователь,
	|	ЛОЖЬ КАК ЭтоПользовательКлиентскогоПриложения
	|ИЗ
	|	РегистрСведений.МобильноеПриложениеСканированиеЧеков КАК МобильноеПриложение
	|ГДЕ
	...
	|	РегистрСведений.КлиентскоеПриложениеСканированиеЧеков КАК КлиентскоеПриложение
	|ГДЕ
	|	КлиентскоеПриложение.КодДоступа = &КодДоступа
	|
	|УПОРЯДОЧИТЬ ПО
	|	Пользователь";
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		...
		Возврат Новый Структура("Пользователь, ЭтоПользовательКлиентскогоПриложения",
			Выборка.Пользователь, Выборка.ЭтоПользовательКлиентскогоПриложения);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВладелецТалонаИТС()
	...
	Возврат "1c-check-scan";
	
КонецФункции

Процедура ПолучитьСообщенияОтложенно()
	...
	ДлительныеОперации.ВыполнитьПроцедуру(
		...
		"РегистрыСведений.МобильноеПриложениеСканированиеЧеков.ПолучитьСообщения");
	
КонецПроцедуры

Процедура СохранитьДатуСледующегоОбновленияКассовыхЧеков(Таймаут)
	
	ТаймаутПоУмолчанию = УчетКассовыхЧековПодотчетныхЛицКлиентСервер.ИнтервалПроверкиЧековПодотчетныхЛиц();
	ДатаСледующегоЗапроса = ТекущаяУниверсальнаяДата();
	ТаймаутОпроса = ?(Таймаут = Неопределено, ТаймаутПоУмолчанию, Число(Таймаут));
	ДатаСледующегоЗапроса = ДатаСледующегоЗапроса + ТаймаутОпроса;
	
	УчетКассовыхЧековПодотчетныхЛиц.СохранитьДатуСледующегоОбновленияКассовыхЧеков(ДатаСледующегоЗапроса);
	
КонецПроцедуры

Функция ИмяСобытияЖурналаРегистрации()
	
	Возврат НСтр("ru = 'Мобильное приложение Сканер чеков'");
