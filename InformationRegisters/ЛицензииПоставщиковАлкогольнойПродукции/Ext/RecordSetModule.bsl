	...
	Если Не Отказ
	   ...
	   И Не ДополнительныеСвойства.Свойство("ОтключитьПроверкуВидаДеятельности") Тогда
		...
		Если Не Результат.Пустой() Тогда
			...
			ТекстСообщения = НСтр("ru = 'Для ""%1"" ранее введена лицензия %2 %3.
										|Одновременное использование лицензий с ограничением и без него не поддерживается.'");
			
			Если ПустаяСтрока(ТекущаяЛицензия.КодВидаПродукции) Тогда
				ВидДеятельностиБыл = НСтр("ru = 'с ограничением по виду продукции'");
			Иначе
				ВидДеятельностиБыл = НСтр("ru = 'без ограничения по виду продукции'");
			КонецЕсли;
