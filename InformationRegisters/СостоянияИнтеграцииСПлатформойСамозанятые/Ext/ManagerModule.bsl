...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗаменитьСсылкиНаНеактуальныеСостояния");
...
Процедура ЗаписатьСостояниеИнтеграцииСПлатформойСамозанятые(Организация, Ответ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СостояниеИнтеграции) И Ответ.Результат.Свойство("Разрешения") Тогда
		...
		ИначеЕсли ЕстьВсеТребуемыеРазрешения(Ответ.Результат.Разрешения) Тогда
	...
	// Если сервис подключен, то идентификатор заявки больше не нужен
	Если СостояниеИнтеграции = Перечисления.СостоянияИнтеграцииСПлатформойСамозанятые.Подключено Тогда
		ИдентификаторЗаявки = "";
	КонецЕсли;
...
Функция ИнформацияОСобытияхПлатформыСамозанятые(Организация) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВключенаИнтеграцияСПлатформойСамозанятые") Тогда
...
// Возвращает признак того, что заявка на подключение просрочена.
// По умолчанию, если заявка отправлена более 1 дня назад, то она просрочена.
//
// Параметры:
//   Организация - СправочникСсылка.Организации.
//
...
//     Булево
//
Функция ЗаявкаНаПодключениеКСервисуПросрочена(Организация) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 1
	|	СостоянияИнтеграцииСПлатформойСамозанятые.ДатаВремяСобытия КАК ДатаВремяСобытия
	|ИЗ
	|	РегистрСведений.СостоянияИнтеграцииСПлатформойСамозанятые КАК СостоянияИнтеграцииСПлатформойСамозанятые
	|ГДЕ
	|	СостоянияИнтеграцииСПлатформойСамозанятые.Организация = &Организация
	|	И СостоянияИнтеграцииСПлатформойСамозанятые.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияИнтеграцииСПлатформойСамозанятые.ЗапросОтправлен)
	|	И СостоянияИнтеграцииСПлатформойСамозанятые.ДатаВремяСобытия < &НачалоТекущаяДня";
	
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("НачалоТекущаяДня", НачалоДня(ТекущаяДатаСеанса()));
...
Процедура ПолучитьНовыеСобытияИзСервиса(Организация)
	...
	Если Не ИнтеграцияСПлатформойСамозанятыеВызовСервера.ПриложениеПодключено(Организация) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Повтор = 1 По МаксимальноеЧислоПовторов Цикл
...
Функция ЕстьВсеТребуемыеРазрешения(ТекущиеРазрешения)
	
	РазрешенияСтруктура = Новый Структура;
	
	ПереченьРазрешений = ИнтеграцияСПлатформойСамозанятые.ПереченьРазрешенийНаДействияПартнеромОтИмениНП();
	Для Каждого Разрешение Из ПереченьРазрешений Цикл
		РазрешенияСтруктура.Вставить(Разрешение.Код, ?(ТекущиеРазрешения.Найти(Разрешение.Код) <> Неопределено, Истина, Ложь));
	КонецЦикла;
	
	ЕстьВсеРазрешения = РазрешенияСтруктура.ОтражениеДохода
		И РазрешенияСтруктура.ПолучениеCводнойИнформацииОДоходе
		И РазрешенияСтруктура.ПолучениеИнформацииПоДоходам
		И РазрешенияСтруктура.ПолучениеИнформацииПоНалоговымНачислениям
		И (РазрешенияСтруктура.КорректировкаСведенийОДоходахПоданныхПартнером
		ИЛИ РазрешенияСтруктура.КорректировкаСведенийОДоходахБезОграничений);
	
	Возврат ЕстьВсеРазрешения;
