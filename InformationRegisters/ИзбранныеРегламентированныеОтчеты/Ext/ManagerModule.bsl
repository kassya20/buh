...
//   *Ссылка                           - Строка (см. общий модуль УсловияПримененияТребованийЗаконодательства),
...
Процедура ОбновитьИЗаполнитьИзбранныеРегламентированныеОтчеты(Организация, ДеревоЗадач) Экспорт
	...
	Для Каждого Задача ИЗ ВключенныеЗадачи Цикл
		...
		ИначеЕсли Задача.Родитель <> Неопределено
			...
			Если Задача.Ссылка = Перечисления.ВидыПодакцизныхТоваров.Табак Тогда
				МассивАкцизы.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеАкцизыФормаТабак());
			ИначеЕсли Задача.Ссылка = Перечисления.ВидыПодакцизныхТоваров.НефтяноеСырье Тогда
				МассивАкцизы.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеАкцизыФормаНефтяноеСырье());
			Иначе
				МассивАкцизы.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеАкцизыОбщаяФорма());
				Если Задача.Ссылка = Перечисления.ВидыПодакцизныхТоваров.АлкогольнаяПродукция Тогда
					МассивАкцизы.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеАвансыПоАкцизам());
				КонецЕсли;
			КонецЕсли;
		Иначе
	...
	// Бух.отчетность
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьОбщаяФорма());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьМалыеПредприятия());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьНекоммерческиеОрганизации());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеБухгалтерскаяОтчетностьСоциальноОриентированные());
	// Декларации по алкоголю
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеРозничнаяПродажаАлкоголя());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеРозничнаяПродажаПива());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеОптоваяТорговляАлкоголем());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеПроизводствоАлгоколя());
	МассивУсловий_Исключение.Добавить(УсловияПримененияТребованийЗаконодательства.УсловиеПеревозкаАлкоголя());
...
Процедура ДополнитьИзбранныеРегламентированныеОтчеты(ТаблицаПравил, Организация) Экспорт
	...
	Если ТекущаяТаблицаОтчетов.Количество() <> 0 Тогда
		...
		СоответствиеОтчетов = Неопределено;
		Если МенеджерЗаписи.Выбран() Тогда
			СоответствиеОтчетов = МенеджерЗаписи.Отчеты.Получить();
		КонецЕсли;
		
		Если СоответствиеОтчетов = Неопределено Тогда
			СоответствиеОтчетов = Новый Соответствие;
		КонецЕсли;
...
//   ДеревоВсехИспользуемыхОтчетовПоКатегориям - заполненное дерево отчетов
//     * см.ОбщиеОбъектыРеглОтчетности.ФормаСозданияОтчета.ДеревоВсехИспользуемыхОтчетов()
//
	...
	Для Каждого Отчет Из ТаблицаВсехРанееСозданныхОтчетов Цикл
		...
		Если МассивСтрок.Количество() = 0 Тогда
			...
			Если МасивСсылок.Количество() <> 0 Тогда
				СтрокаТаблицыСсылка = МасивСсылок[0].СсылкаВидОтчета;
			ИначеЕсли Отчет.ПолноеИмя = "Документ.РегламентированныйОтчет" Тогда
				Продолжить;
			КонецЕсли;
			НоваяСтрокаТаблицы = РанееСозданныеОтчеты.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТаблицы, Отчет);
			НоваяСтрокаТаблицы.Ссылка = СтрокаТаблицыСсылка;
		КонецЕсли;
	КонецЦикла;
	
	ДеревоВсехИспользуемыхОтчетовПоКатегориям = Неопределено;
	Если ПараметрыОтбора.Свойство("ДеревоВсехИспользуемыхОтчетовПоКатегориям", ДеревоВсехИспользуемыхОтчетовПоКатегориям) Тогда
	
		МассивСтрокКУдалению = Новый Массив;
		Для Каждого СтрокаТаблицы Из РанееСозданныеОтчеты Цикл
			НайденныеОтчеты = ДеревоВсехИспользуемыхОтчетовПоКатегориям.Строки.НайтиСтроки(Новый Структура("Наименование", СтрокаТаблицы.Наименование), Истина);
		...
		Для Каждого СтрокаКУдалению Из МассивСтрокКУдалению Цикл
			РанееСозданныеОтчеты.Удалить(СтрокаКУдалению);
		КонецЦикла;
	КонецЕсли;
