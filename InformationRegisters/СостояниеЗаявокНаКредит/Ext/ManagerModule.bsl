...
Процедура ОбновитьСостояниеЗаявкиПриПолученииСообщения(ЗаявкаНаКредит, РеквизитыТранзакции) Экспорт 
	...
	ИначеЕсли РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.ПоложительноеКредитноеРешение Тогда
		НовоеСостояние = Перечисления.СостоянияЗаявокНаКредит.Одобрено;
	ИначеЕсли РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.ОтрицательноеКредитноеРешение Тогда
		НовоеСостояние = Перечисления.СостоянияЗаявокНаКредит.Отказано;
	ИначеЕсли РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.ИзвещениеОПолученииБанкомАкцепта Тогда
...
Процедура ОбновитьСостояниеЗаявкиПриОтправкеСообщения(ЗаявкаНаКредит, РеквизитыТранзакции) Экспорт 
	...
	ТипыТранзакций = ЗаявкиНаКредитКлиентСервер.ТипыТранзакцийСервисаОбменаСБанками(РеквизитыТранзакции.Сервис);
	...
	ИначеЕсли РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.Акцепт Тогда
...
Процедура УстановитьСостояниеНеотправленнойЗаявки(ЗаявкаНаКредит) Экспорт 
	...
	РеквизитыЗаявки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаявкаНаКредит,
		"СервисОбменаСБанками, Организация, СуммаДокумента, СрокКредита, ПометкаУдаления");
	...
	Запись.СервисОбменаСБанками = РеквизитыЗаявки.СервисОбменаСБанками;
...
//	Банк - СправочникСсылка.БанкиУниверсальногоОбмена - Банк-получатели.
...
Функция ПодготовитьАкцептДляОтправки(ЗаявкаНаКредит, Банк, ЭтоЭлектроннаяПодписьВМоделиСервиса) Экспорт
	...
	Вложение.Сервис           = СведенияОбАкцепте.ДанныеЗаявки.СервисОбменаСБанками;
	...
	Если СведенияОбАкцепте.ДанныеЗаявки.СервисОбменаСБанками = Перечисления.СервисыОбменаСБанками.ЗаявкиНаЛизинг Тогда
		КлючеваяОперация = "ПодготовкаФайловДляОтправкиАкцептаЗаявкаНаЛизинг";
	Иначе
		КлючеваяОперация = "ПодготовкаФайловДляОтправкиАкцептаЗаявкаНаКредит";
	КонецЕсли;
...
// Возвращает XDTO объект из пакета ЗаявкиНаОткрытиеСчета или ЗаявкиНаКредит с ответом банка по умолчанию
...
//	ТипТранзакции - ПеречисленияСсылка.ТипыТранзакцийОбменаСБанкаимЗаявкиНаОткрытиеСчета,
//                  ПеречисленияСсылка.ТипыТранзакцийОбменаСБанкаимЗаявкиНаКредит - Тип ответной транзакции.
//	ДатаАктуальности - Дата - Дата, по состоянию на которую искать ответ банка.
//
// Возвращаемое значение:
//	ОбъектXDTO, Неопределено - Объект ОтветБанка из пакета XDTO или Неопределено, если ответ банка по умолчанию отсутствует.
//
Функция ОтветБанкаПоУмолчанию(СервисОбменаСБанками, Банк, ТипТранзакции, Знач ДатаАктуальности) Экспорт
	...
	// Найдем условия обслуживания банка с текстами ответов по умолчанию.
	...
	УсловияОбслуживания = ЗаявкиНаКредит.ОбъектXDTOХарактеристики(СервисОбменаСБанками, ХарактеристикиСервиса[0].Значение);
	...
	Для каждого ОтветБанка Из УсловияОбслуживания.ОтветБанка Цикл
...
Функция НовыеСообщенияОтБанков(ПовторятьЗапрос) Экспорт  
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СостояниеЗаявокНаКредит.СервисОбменаСБанками КАК СервисОбменаСБанками,
	|	СостояниеЗаявокНаКредит.Организация КАК Организация,
	|	СостояниеЗаявокНаКредит.ЗаявкаНаКредит КАК ЗаявкаНаКредит,
	|	СостояниеЗаявокНаКредит.Банк КАК Банк,
...
Функция ПользовательскиеПрисоединенныеФайлы(Транзакция) Экспорт
	...
	ТипыТранзакцийСервиса = ЗаявкиНаКредитКлиентСервер.ТипыТранзакцийСервисаОбменаСБанками(
	...
	ВходящиеТипыТранзакций.Добавить(ТипыТранзакцийСервиса.ОтрицательноеКредитноеРешение);
	ВходящиеТипыТранзакций.Добавить(ТипыТранзакцийСервиса.ПоложительноеКредитноеРешение);
	ВходящиеТипыТранзакций.Добавить(ТипыТранзакцийСервиса.ИзвещениеОПолученииБанкомАкцепта);
	...
	Отбор.Вставить("ТипДокумента", Перечисления.ТипыДокументовОбменаСБанкамиЗаявкиНаКредит.ПроизвольныйФайл);
...
Процедура ЗаполнитьНовыеДанныеПриДобавленииСервиса1СЛизинг() Экспорт
	...
	// Во всех ранее существующих записях в регистре устанавливаем сервис обмена данными 1С:Кредит.
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СостояниеЗаявокНаКредит.Организация,
	...
	|	РегистрСведений.СостояниеЗаявокНаКредит КАК СостояниеЗаявокНаКредит
	|ГДЕ
	|	СостояниеЗаявокНаКредит.СервисОбменаСБанками = ЗНАЧЕНИЕ(Перечисление.СервисыОбменаСБанками.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		// Записываем через набор записей, чтобы выполнялось в режиме ОбменДанными.Загрузка
		НачатьТранзакцию();
		Попытка

			НаборЗаписей.Отбор.Организация.Установить(Выборка.Организация);
			НаборЗаписей.Отбор.ЗаявкаНаКредит.Установить(Выборка.ЗаявкаНаКредит);
			НаборЗаписей.Отбор.Банк.Установить(Выборка.Банк);
			
			НаборЗаписей.Прочитать();
			
			Для каждого Запись Из НаборЗаписей Цикл
				Запись.СервисОбменаСБанками = Перечисления.СервисыОбменаСБанками.ЗаявкиНаКредит;
			КонецЦикла;
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать запись регистра СостояниеЗаявокНаКредит: организация: %1 %2 банк: %3 по причине:
					|%4'"),
					Выборка.Организация,
					Выборка.ЗаявкаНаКредит,
					Выборка.Банк,
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
				УровеньЖурналаРегистрации.Предупреждение,
				Метаданные.Документы.ЗаявкаНаКредит, Выборка.ЗаявкаНаКредит, ТекстСообщения);
...
Процедура ЗаписатьНачальноеСостояниеОтправки(ЗаявкаНаКредит, РеквизитыТранзакции)
	...
	Запрос.Текст = 
		...
		|	ЗаявкаНаКредитБанки.Ссылка.СервисОбменаСБанками КАК СервисОбменаСБанками,
...
Процедура УстановитьСрокАктуальностиЗаписи(Запись)
	...
	ИначеЕсли Запись.Состояние = Перечисления.СостоянияЗаявокНаКредит.Одобрено 
		...
		// В течение указанного срока действия или 3 месяца с момента изменения состояния
...
Процедура ПрочитатьПодробностиОтвета(Запись, РеквизитыТранзакции)
	...
	Попытка
		...
		Пока ЧтениеXML.Прочитать() Цикл
			...
			ИначеЕсли ТипУзла = ТипУзлаXML.НачалоЭлемента И ИмяУзла = "Продукт" Тогда
				...
				СловарьАтрибутовПродукта.Вставить("СуммаАванса",	 Новый Структура("ИмяРесурса, Тип", "СуммаАванса", 	"Число"));
				СловарьАтрибутовПродукта.Вставить("СуммаПлатежа",	 Новый Структура("ИмяРесурса, Тип", "СуммаПлатежа", "Число"));
			...
			ИначеЕсли ТипУзла = ТипУзлаXML.НачалоЭлемента И ИмяУзла = "ДопСвед" Тогда
				...
				// Установим признак, что в составе ответа от банка переданы дополнительные сведения.
				ЕстьДопСведенияВОтветеБанка = Истина;
				...
				Если РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.ИзвещениеОПолученииБанкомАкцепта Тогда
			...
			ИначеЕсли ТипУзла = ТипУзлаXML.НачалоЭлемента И ИмяУзла = "Свед" Тогда
				...
				Если ЗначениеЗаполнено(ВидСведений)
					И РеквизитыТранзакции.ТипТранзакции = ТипыТранзакций.ИзвещениеОПолученииБанкомАкцепта Тогда
		...
		Если НЕ ЕстьДопСведенияВОтветеБанка Тогда
			// Если банк в ответе никаких дополнительных сведений не передал,
			// то проверим, возможно, заданы тексты по умолчанию для конкретного банка.
			ДополнительныеСведения = ДополнительныеСведенияИзОтветаБанкаПоУмолчанию(Запись.Банк, РеквизитыТранзакции, Запись.ДатаИзменения);
			Запись.ПодробностиРешения = Новый ХранилищеЗначения(ДополнительныеСведения);
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ЗаявкиНаКредит.ПрочитатьПодробностиОтвета'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ДополнительныеСведенияИзОтветаБанкаПоУмолчанию(Банк, РеквизитыТранзакции, ДатаАктуальности)

	Результат = НоваяТаблицаДополнительныхСведений();

	// Найдем ответ банка по умолчанию для текущего типа транзакции.
	ОтветБанка = ОтветБанкаПоУмолчанию(
		РеквизитыТранзакции.Сервис,
		Банк,
		РеквизитыТранзакции.ТипТранзакции,
		ДатаАктуальности);
		
	Если ОтветБанка = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	Для каждого ДопСведения Из ОтветБанка.ДополнительныеСведения Цикл
		НоваяСтрока = Результат.Добавить();
		НоваяСтрока.ВидСведений      = ДопСведения.ВидСведений;
		НоваяСтрока.ЗначениеСведений = ДопСведения.Содержание;
	КонецЦикла;
...
Функция СведенияОбАкцепте(ЗаявкаНаКредит, Банк)
	...
	Если НЕ ЗначениеЗаполнено(ЗаявкаНаКредит) Тогда
		ВызватьИсключение СтрШаблон(ОписаниеОшибки, НСтр("ru='Не указана заявка'"));
	КонецЕсли;

	Если НЕ ЗначениеЗаполнено(Банк) Тогда
		ВызватьИсключение СтрШаблон(ОписаниеОшибки, НСтр("ru='Не указан получатель акцепта'"));
	КонецЕсли;
	...
	ДанныеЗаявки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЗаявкаНаКредит,
		"Ссылка, Дата, Организация, СервисОбменаСБанками");
	...
	Если НЕ Запись.Выбран() Тогда
		ВызватьИсключение СтрШаблон(ОписаниеОшибки, НСтр("ru='Не найдены сведения по заявке'"));
	КонецЕсли; 
...
Функция ВыгрузитьАкцепт(СведенияОбАкцепте)
	...
	Возврат ЗаявкиНаКредит.ЭлектронноеПредставлениеСообщения("Акцепт", ПараметрыВыгрузки);
