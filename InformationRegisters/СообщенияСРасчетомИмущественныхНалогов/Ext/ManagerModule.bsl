...
//   - Структура 
//   * Сообщение - СправочникСсылка.ДокументыРеализацииПолномочийНалоговыхОрганов - сообщение с расчетом налога
...
Функция СведенияОСообщении(Организация, Налог, Период, КодНалоговогоОргана, КодОтправителя = Неопределено) Экспорт
	...
	// о коде налогвого органа - отправителя, а также о кодах налоговых органов, на территории которых находятся объекты. Таким образом,
...
// Определяет налоговые органы, в которых учтены объекты налогобложения, попавшие в расчет налога, присланный отправителем
//
// Параметры:
//  Организация - СправочникСсылка.Организации
...
// 
// Возвращаемое значение:
//  Массив - массив кодов налоговых органов, в которых учтены объекты, попавшие в расчет налога,
...
// Коррективроки связаны с тем, что после получения сообщения из ФНС срок передачи поясенений ограничен.
//
// Параметры:
//  Организация - СправочникСсылка.Организации
// 
...
Функция ОсобыеСрокиСверкиИмущественныхНалогов(Организация) Экспорт
	...
	|	СообщенияСРасчетомИмущественныхНалогов.КодНалоговогоОргана КАК КодНалоговогоОргана,
	|	МАКСИМУМ(ЕСТЬNULL(СообщенияСРасчетомИмущественныхНалогов.Сообщение.ДатаДокумента, ДАТАВРЕМЯ(1, 1, 1))) КАК Срок
	|ИЗ
	|	РегистрСведений.СообщенияСРасчетомИмущественныхНалогов КАК СообщенияСРасчетомИмущественныхНалогов
	|ГДЕ
	|	СообщенияСРасчетомИмущественныхНалогов.Организация = &Организация
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(ЕСТЬNULL(СообщенияСРасчетомИмущественныхНалогов.Сообщение.ДатаДокумента, ДАТАВРЕМЯ(1, 1, 1))) <> ДАТАВРЕМЯ(1, 1, 1)";
	...
	Пока Выборка.Следующий() Цикл
		...
		ОсобыйСрок.Налог = Перечисления.ВидыИмущественныхНалогов.КодНалогаВЗадачахБухгалтера(Выборка.Налог);
		...
		НоваяДата = КалендарьБухгалтера.РабочаяДатаПоКалендарю(ПроизводственныйКалендарь, Выборка.Срок, 20);
		Если НоваяДата <> Неопределено Тогда
			ОсобыйСрок.Срок = НоваяДата;
		Иначе
