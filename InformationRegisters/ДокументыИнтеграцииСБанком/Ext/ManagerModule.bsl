...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеБаннераИнтеграцииСБанкомВФоне", Истина);
	Методы.Вставить("ОтправитьОповещениеОбИзмененииПоДокументам", Истина);
...
//
// Параметры:
//  Документ - ДокументСсылка.ПлатежноеПоручение, ДокументСсылка.РегламентированныйОтчет, ДокументСсылка.УведомлениеОСпецрежимахНалогообложения - Документ, который требуется отправить в банк.
//
...
//
// Параметры:
//  Документ - ДокументСсылка.РегламентированныйОтчет, ДокументСсылка.УведомлениеОСпецрежимахНалогообложения - Документ, который требуется отправить в банк.
//
Процедура ПриИзмененииСтатусаРегламентированногоОтчета(Документ) Экспорт
	
	Если Не РазрешаетсяОтправкаВыгрузкиДокумента(Документ) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДанныеБаннераИнтеграцииСБанкомДляПомощников(СостоянияИнтеграцииДокументов) Экспорт
	...
	ОписаниеДокументовПоТипам.Вставить("РегламентированноеУведомление",
		ОписаниеДокументовПоСостояниюИнтеграции(СостоянияИнтеграцииДокументов,
...
// Возвращает рузультат проверки отправки документа в банк по состоянию интеграции.
//
// Параметры:
//  СостоянияИнтеграцииДокументов - ТаблицаЗначений - результат выполнения функции СостоянияИнтеграцииДокументов().
//  Документ - ДокументСсылка.ПлатежноеПоручение - проверяемый документ.
//
// Возвращаемое значение:
//   - Булево - Истина, если документ отправлен в банк, Ложь если еще не отправлен.
//
Функция ДокументОтправленВБанк(СостоянияИнтеграцииДокументов, Документ) Экспорт
	
	ОписаниеДокумента = ОписаниеДокументовПоСостояниюИнтеграции(СостоянияИнтеграцииДокументов, ТипЗнч(Документ));
	
	Возврат ОписаниеДокумента.Документ = Документ И ОписаниеДокумента.Отправленные = 1;
...
// Обрабатывает дополнительную информацию переданную из банка
// в составе пакета данных, подтверждающих доставку объекта.
//
// Параметры:
//  НастройкаИнтеграции - СправочникСсылка.НастройкиИнтеграцииСБанками - Настройка интеграции, для которой нужно возвратить двоичные данные.
//  ДанныеИзБанка       - Структура
...
//
Процедура ОбработатьДополнительнуюИнформацию(НастройкаИнтеграции, ДанныеИзБанка) Экспорт
	
	ИдентификаторОбъекта = ДанныеИзБанка.Ключ;
	ПакетДанных          = ДанныеИзБанка.Значение;
	
	ДополнительнаяИнформация = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПакетДанных, "ДополнительнаяИнформация");
	Если ДополнительнаяИнформация = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Номер = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительнаяИнформация, НомерПлатежногоПоручения());
	Если ЗначениеЗаполнено(Номер) Тогда
		УстановитьНомерПлатежногоПоручения(НастройкаИнтеграции, ИдентификаторОбъекта, Номер);
	КонецЕсли;
...
Функция НастройкаИнтеграцииДляОтправкиДокумента(Документ)
	...
	ИначеЕсли РазрешаетсяОтправкаВыгрузкиДокумента(Документ) Тогда
...
Функция НастройкаИнтеграцииДляОтправкиРегламентированногоОтчета(Документ)
	...
	Если РегламентированнаяОтчетностьБП.Подключена1СОтчетность(РеквизитыДокумента.Организация) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция ВыгрузитьДокумент(Документ)
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		...
		ВыгрузкаДокумента = Документ.ПолучитьОбъект().ВыгрузитьДокумент();
		
		Если ВыгрузкаДокумента.Количество() > 0
			И ЭтоАдресВременногоХранилища(ВыгрузкаДокумента[0].АдресФайлаВыгрузки) Тогда

			СтруктураВыгрузки.ДвоичныеДанные = ПолучитьИзВременногоХранилища(ВыгрузкаДокумента[0].АдресФайлаВыгрузки);
		Иначе
			ТекстОшибки = НСтр("ru = 'Не удалось выгрузить данные уведомления'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			СтруктураВыгрузки.Ошибка = Истина;
			СтруктураВыгрузки.СообщениеОбОшибке = ТекстОшибки;
		КонецЕсли;
	Иначе
...
Функция ИмяОбработчикаДляОтправки(Документ)
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
		Основание = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "Основание");
		Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаявлениеОЗачетеВСчетПредстоящейОбязанности") Тогда
			Возврат ОбработчикДанныхОтправкиЗаявлениеОЗачетеВзносов();
		Иначе
			Возврат ОбработчикДанныхОтправкиУведомленияОбИсчисленныхСуммах();
		КонецЕсли;
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Выгрузка объекта типа ""%1"" в банк не поддерживается'"), ТипДокумента);
	КонецЕсли;
...
Функция ОбработчикДанныхОтправкиУведомленияОбИсчисленныхСуммах()
	
	Возврат НРег("tax_notification_1c_zip");
	
КонецФункции

Функция ОбработчикДанныхОтправкиЗаявлениеОЗачетеВзносов()
	
	Возврат НРег("insurance_statement_1c_zip");
...
Функция РазрешаетсяОтправкаВыгрузкиДокумента(Документ)
	
	РазрешенныеТипыДокументов = Новый Массив;
	РазрешенныеТипыДокументов.Добавить(Тип("ДокументСсылка.РегламентированныйОтчет"));
	РазрешенныеТипыДокументов.Добавить(Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения"));
	
	Возврат РазрешенныеТипыДокументов.Найти(ТипЗнч(Документ)) <> Неопределено;
	
КонецФункции

#Область ОбработкаДополнительнойИнформации

// Устанавливает платежному поручению номер, переданный из банка
//
// Параметры:
//  НастройкаИнтеграции  - СправочникСсылка.НастройкиИнтеграцииСБанками - Настройка интеграции.
//  Идентификатор        - Строка - Идентификатор документа, в котором нужно установить номер.
//  Номер                - Строка - Номер, который нужно установить в документе.
//
Процедура УстановитьНомерПлатежногоПоручения(НастройкаИнтеграции, Идентификатор, Номер)
	...
	ТекстОшибкиБлокировки = СтрШаблон(НСтр("ru = 'Платежное поручение №%1 от %2 заблокировано'"),
		Номер, ДокументОбъект.Дата);
	Если ДокументОбъект.Заблокирован() Тогда
		ВызватьИсключение ТекстОшибкиБлокировки;
	Иначе
		Попытка
	...
	Попытка
		...
		ДокументОбъект.Номер =
			НомерОбъектаСПрефиксомБанкаИнтеграции(Номер, ДокументОбъект.Организация, ДокументОбъект.СчетОрганизации);
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ДокументОбъект, Истина);
		ЗафиксироватьТранзакцию();
		ДокументОбъект.Разблокировать();
	Исключение
		ОтменитьТранзакцию();
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Для платежного поручения №%1 от %2 не удалось установить номер'"),
			Номер, ДокументОбъект.Дата);
	КонецПопытки;
	
КонецПроцедуры

// Возвращает идентификатор номера платежного поручения.
//
Функция НомерПлатежногоПоручения()
