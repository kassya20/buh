...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыЗаписи.Вставить("УплачиваетсяЗемельныйНалогПередЗаписью",
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыЗаписи.Вставить("УплачиваетсяЗемельныйНалогПослеЗаписи",
		ПолучитьФункциональнуюОпцию("УплачиваетсяЗемельныйНалог"));
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Перем УплачиваетсяЗемельныйНалогПередЗаписью, УплачиваетсяЗемельныйНалогПослеЗаписи;
	
	Если ПараметрыЗаписи.Свойство("УплачиваетсяЗемельныйНалогПередЗаписью", УплачиваетсяЗемельныйНалогПередЗаписью)
		И ПараметрыЗаписи.Свойство("УплачиваетсяЗемельныйНалогПослеЗаписи", УплачиваетсяЗемельныйНалогПослеЗаписи) Тогда
		
		Если УплачиваетсяЗемельныйНалогПередЗаписью <> УплачиваетсяЗемельныйНалогПослеЗаписи Тогда
			ОбновитьИнтерфейс();
		КонецЕсли;
...
Процедура ТекстНалоговойЛьготыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыЛьготы.Вставить("Организация");
	ПараметрыЛьготы.Вставить("ОсновноеСредство");
	ПараметрыЛьготы.Вставить("НалоговаяСтавка");
	...
	ОткрытьФорму("РегистрСведений.РегистрацияЗемельныхУчастков.Форма.ФормаНастройкиЛьготы", ПараметрыФормы, ЭтотОбъект);
...
Процедура ЗаполнитьПараметрыПоОсновномуСредству()
	...
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОбщаяСобственность КАК ОбщаяСобственность,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ДоляВПравеОбщейСобственностиЧислитель КАК
	|		ДоляВПравеОбщейСобственностиЧислитель,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ДоляВПравеОбщейСобственностиЗнаменатель КАК
	...
	|	РегистрацияЗемельныхУчастковСрезПоследних.ПостановкаНаУчетВНалоговомОргане КАК ПостановкаНаУчетВНалоговомОргане,
	|	РегистрацияЗемельныхУчастковСрезПоследних.НалоговыйОрган КАК НалоговыйОрган,
	|	РегистрацияЗемельныхУчастковСрезПоследних.КодПоОКТМО КАК КодПоОКТМО,
	...
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОснованиеЛьготыПоНалоговойБазе.ВидЛьготы КАК ВидЛьготыПоНалоговойБазе,
	...
	|	РегистрацияЗемельныхУчастковСрезПоследних.ДоляНеОблагаемойНалогомПлощадиЧислитель КАК
	|		ДоляНеОблагаемойНалогомПлощадиЧислитель,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ДоляНеОблагаемойНалогомПлощадиЗнаменатель КАК
	|		ДоляНеОблагаемойНалогомПлощадиЗнаменатель,
	|	РегистрацияЗемельныхУчастковСрезПоследних.НеОблагаемаяНалогомСумма КАК НеОблагаемаяНалогомСумма,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОснованиеЛьготыСнижениеСуммыНалога КАК ОснованиеЛьготыСнижениеСуммыНалога,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОснованиеЛьготыСнижениеСуммыНалога.ВидЛьготы КАК
	...
	|	РегистрацияЗемельныхУчастковСрезПоследних.СуммаУменьшенияСуммыНалога КАК СуммаУменьшенияСуммыНалога,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОснованиеЛьготыСнижениеСтавки КАК ОснованиеЛьготыСнижениеСтавки,
	|	РегистрацияЗемельныхУчастковСрезПоследних.ОснованиеЛьготыСнижениеСтавки.ВидЛьготы КАК ВидЛьготыСнижениеСтавки,
	...
	|ИЗ
	|	РегистрСведений.РегистрацияЗемельныхУчастков.СрезПоследних(&Период, Организация = &Организация
	...
	Если Выборка.Количество() > 0 Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка); // виды льгот
...
Процедура ВыполнитьИнициализацию()
	...
	Если ЗначениеЗаполнено(Запись.ОснованиеЛьготыПоНалоговойБазе) Тогда
		ВидЛьготыПоНалоговойБазе = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.ОснованиеЛьготыПоНалоговойБазе, "ВидЛьготы");
	КонецЕсли;
	Если ЗначениеЗаполнено(Запись.ОснованиеЛьготыСнижениеСуммыНалога) Тогда
		РеквизитыЛьготы = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Запись.ОснованиеЛьготыСнижениеСуммыНалога, "ВидЛьготы, ПроцентУменьшения");
		ВидЛьготыСнижениеСуммыНалога = РеквизитыЛьготы.ВидЛьготы;
		ПроцентУменьшенияСуммыНалога = РеквизитыЛьготы.ПроцентУменьшения;
	КонецЕсли;
	Если ЗначениеЗаполнено(Запись.ОснованиеЛьготыСнижениеСтавки) Тогда
		ВидЛьготыСнижениеСтавки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.ОснованиеЛьготыСнижениеСтавки, "ВидЛьготы");
	КонецЕсли;
...
Процедура УстановитьТекстНалоговойЛьготы(Форма)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыЛьготы, Форма.Запись);
	ЗаполнитьЗначенияСвойств(ПараметрыЛьготы, Форма);
	...
	Форма.Элементы.ГруппаИнформацияЗаявлениеОЛьготе.Видимость = Не ПустаяСтрока(ТекстНалоговойЛьготы); 
