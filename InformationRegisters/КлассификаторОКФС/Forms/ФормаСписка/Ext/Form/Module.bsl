...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ТекущийКод) Тогда
		...
		КлючЗаписи = РегистрыСведений.КлассификаторОКФС.СоздатьКлючЗаписи(СтруктураКлюча);
...
Процедура ОбновитьДанныеКлассификатора(Команда)
	...
	Если ТребуетсяПодключениеИнтернетПоддержки Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для автоматического обновления классификатора ОКФС необходимо подключить Интернет-поддержку пользователей.'");
...
Функция НачатьПроверкуОбновленияНаСервере()
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		...
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			РегистрыСведений.КлассификаторОКФС.ИдентификаторВСервисеКлассификаторов()));
...
Процедура НачатьОбновлениеДанныхСписка()
	...
	Если ПроверкаАктуальностиДанныхСписка.ДоступныеВерсии.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Загружен актуальный классификатор ОКФС'"));
		Возврат;
	КонецЕсли;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Обновление классификатора ОКФС'");
	
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
		"РегистрыСведений.КлассификаторОКФС.ЗагрузитьДанныеИзСервисаКлассификаторов",
		Новый Структура(),
		ПараметрыВыполнения);
...
Процедура ОжидатьЗавершениеОбновленияДанныхСписка()
	...
	ПараметрыОжидания.ТекстСообщения = НСтр("ru='Идет загрузка классификатора ОКФС.'");
...
Процедура ОбновлениеДанныхСпискаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Успешно Тогда
		ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.КлассификаторОКФС"));
