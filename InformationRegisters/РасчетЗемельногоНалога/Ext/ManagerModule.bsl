...
Процедура ЗаполнитьДаннымиПрежнегоРегистра(Параметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(Параметры.ПрогрессВыполнения.ВсегоОбъектов) Тогда
		...
		|ГДЕ
		|	РасчетЗемельногоНалогаДо2020.Активность
	...
	Иначе
		...
		Льготы.Колонки.Добавить("КодРегиона", ОбщегоНазначения.ОписаниеТипаЧисло(4, 0, ДопустимыйЗнак.Неотрицательный));
	...
	Пока ОстатокПорции > 0 Цикл  // может быть прерван и раньше
		...
		|	РасчетЗемельногоНалогаДо2020.Активность
		|	И РасчетЗемельногоНалогаДо2020.ПериодРасчета >= &ДатаПерехода
		|	И РасчетЗемельногоНалога.Регистратор ЕСТЬ NULL
		|
		|УПОРЯДОЧИТЬ ПО
		...
		Попытка
			...
			|ИЗ
			|	РегистрСведений.РегистрацияЗемельныхУчастков.СрезПоследних(&ПредельнаяДатаВозникновенияПрава,
