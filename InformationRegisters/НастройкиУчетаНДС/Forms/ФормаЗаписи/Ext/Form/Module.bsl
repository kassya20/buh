...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтавкаНДСПриУСН = НастройкиСистемыНалогообложенияФормы.ИндексСтавкиНДСПоЗначению(НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ОрганизацияВелаРаздельныйУчет 
		...
		И ЭтотОбъект.НастройкиУчетаНДС.Период <> НачалоКвартала(ЭтотОбъект.НастройкиУчетаНДС.Период) Тогда
		...
		Отказ = Истина;
	ИначеЕсли РежимБезЗакрывающихДокументов И НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН = ПредопределенноеЗначение("Перечисление.ВидыСтавокНДС.Общая") Тогда
		Отказ = Истина;
		Таймаут = 30;
...
Процедура ПодготовитьФормуНаСервере()
	...
	Период = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(
		...
		Истина);
	ПлательщикНДС   = УчетнаяПолитика.ПлательщикНДС(НастройкиУчетаНДС.Организация, НастройкиУчетаНДС.Период);
	СтавкаНДСПриУСН = НастройкиСистемыНалогообложенияФормы.ИндексСтавкиНДСПоЗначению(НастройкиУчетаНДС.СтавкаНалогообложенияПриУСН);
