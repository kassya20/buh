...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьСтавкиТорговогоСбора");
	Методы.Вставить("ОбновитьСтавкиТорговыеАвтоматы");
...
Функция НовыйПараметрыТорговойТочки() Экспорт
	...
	ПараметрыТорговойТочки.Вставить("КоэффициентДефлятор",       1);
...
Процедура РассчитатьСуммуСбора(ПараметрыТорговойТочки, СтавкиСбора, Территории) Экспорт
	...
	СтавкаСбора = Неопределено;
	Для Каждого СтрокаТаблицыСтавок Из ТаблицаСтавок Цикл
		Если (СтрокаТаблицыСтавок.ДействуетС <= ТорговаяТочкаПериод И ТорговаяТочкаПериод <= СтрокаТаблицыСтавок.ДействуетПо)
			И (НЕ ЕстьТорговыйЗал 
				ИЛИ (СтрокаТаблицыСтавок.НижняяГраница < ПлощадьТорговогоЗала И  ПлощадьТорговогоЗала <= СтрокаТаблицыСтавок.ВерхняяГраница)) Тогда
			
			СтавкаСбора = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаТаблицыСтавок);
			Прервать;
			
		КонецЕсли;
	КонецЦикла;
	
	Если СтавкаСбора = Неопределено Тогда
	...
	СтавкаСбора.Вставить("КоэффициентДефлятор", 1);
	Если ПараметрыТорговойТочки.Период >= Дата("20250101") 
		...
		СтавкаСбора.Вставить("КоэффициентДефлятор", КоэффициентДефляторНаДату(ПараметрыТорговойТочки.Период));
	КонецЕсли;
	...
	Если ПараметрыТорговойТочки.ТипТорговойТочки = Перечисления.ТипыТорговыхТочек.РозничныйРынок Тогда
		...
		ПараметрыТорговойТочки.СуммаСбора = Окр(СтавкаСбора.Превышение * ПараметрыТорговойТочки.ПлощадьТорговогоЗала * СтавкаСбора.КоэффициентДефлятор);
		ПараметрыТорговойТочки.Ставка = ПараметрыТорговойТочки.СуммаСбора;
	Иначе
		Если ЕстьТорговыйЗал И СтавкаСбора.Превышение > 0 Тогда
			// Торговый сбор считается в полных рублях
			ПараметрыТорговойТочки.Ставка = Окр(СуммаТорговогоСбора(ПараметрыТорговойТочки, СтавкаСбора));
		Иначе
			ПараметрыТорговойТочки.Ставка = Окр(СтавкаСбора.БазоваяСтавка * СтавкаСбора.КоэффициентДефлятор);
		КонецЕсли;
...
Процедура СоздатьРасшифровкуРасчетаСуммыСбора(ПараметрыТорговойТочки, СтавкаСбора)
	...
	Если ПараметрыТорговойТочки.ТипТорговойТочки = Перечисления.ТипыТорговыхТочек.ТорговыйАвтомат Тогда
		...
		Иначе
			...
			РасшифровкаРасчета = СтрШаблон(РасшифровкаРасчета, ОбщегоНазначенияБПВызовСервера.ФорматСумм(ПараметрыТорговойТочки.Ставка));
		КонецЕсли;
	...
	ИначеЕсли ПараметрыТорговойТочки.ВидТорговойДеятельности =
		...
		РасчетнаяСтавкаЗаМетр = ОбщегоНазначенияБПВызовСервера.ФорматСумм(РасчетнаяСтавкаЗаМетр(ПараметрыТорговойТочки, СтавкаСбора));
		РасшифровкаРасчета = СтрШаблон(РасшифровкаРасчета,
			ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтавкаСбора.БазоваяСтавка * СтавкаСбора.КоэффициентДефлятор),
			ПревышениеБазовойПлощади(ПараметрыТорговойТочки.ПлощадьТорговогоЗала, СтавкаСбора.НижняяГраница),
			ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтавкаСбора.Превышение * СтавкаСбора.КоэффициентДефлятор),
			ПараметрыТорговойТочки.ПлощадьТорговогоЗала,
			РасчетнаяСтавкаЗаМетр,
			РасчетнаяСтавкаЗаМетр,
			ПараметрыТорговойТочки.ПлощадьТорговогоЗала,
			ОбщегоНазначенияБПВызовСервера.ФорматСумм(ПараметрыТорговойТочки.Ставка),
	...
	ИначеЕсли ПараметрыТорговойТочки.ВидТорговойДеятельности =
		...
		РасшифровкаРасчета = СтрШаблон(РасшифровкаРасчета, ОбщегоНазначенияБПВызовСервера.ФорматСумм(ПараметрыТорговойТочки.Ставка), СтавкаСбора.ВерхняяГраница);
...
Функция СуммаТорговогоСбора(ПараметрыТорговойТочки, СтавкаСбора)

	СуммаТорговогоСбора = СтавкаСбора.БазоваяСтавка + ПревышениеБазовойПлощади(ПараметрыТорговойТочки.ПлощадьТорговогоЗала, СтавкаСбора.НижняяГраница) * СтавкаСбора.Превышение;
	
	Возврат СуммаТорговогоСбора * СтавкаСбора.КоэффициентДефлятор;

КонецФункции 


Функция РасчетнаяСтавкаЗаМетр(ПараметрыТорговойТочки, СтавкаСбора)
	РасчетнаяСтавкаЗаМетр = СуммаТорговогоСбора(ПараметрыТорговойТочки, СтавкаСбора) / ПараметрыТорговойТочки.ПлощадьТорговогоЗала;
