...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущаяДатаПользователя = КонецДня(ОбщегоНазначения.ТекущаяДатаПользователя());
...
Процедура НаборЗаписейПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ТекущиеДанные.Период < КонецДня('20150701') Тогда
		ТекущиеДанные.Период = КонецДня('20150701');
...
Процедура УстановитьОтборСтрок()
	...
	Для Каждого Запись Из НаборЗаписей Цикл
		...
		Запись.ПоказыватьЗапись = Запись.ВидОперации <> Перечисления.ВидыОперацийТорговыеТочки.СнятиеСУчета 
			И Запись.ВидОперации <> Перечисления.ВидыОперацийТорговыеТочки.ИзменениеСтавки;
	КонецЦикла;
