...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПолучитьПроцедурыДекларированияФТСССервера", Истина);
...
Процедура ДобавитьАктуализироватьСведенияПоДекларациям(Знач ДекларацииНаТовары, Знач Организация)
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьСтоимостьКакЧисло(СведенияДекларацийНаТовары);
...
Процедура ЗаполнитьСтоимостьКакЧисло(СведенияДекларацийНаТовары)
	
	СтоимостьСтрока = СокрЛП(СведенияДекларацийНаТовары.ОбщаяСтоимость);
	НаименованиеВалюты = Прав(СтоимостьСтрока, 4);
	СтоимостьСтрока = СтрЗаменить(СтоимостьСтрока, НаименованиеВалюты, "");
