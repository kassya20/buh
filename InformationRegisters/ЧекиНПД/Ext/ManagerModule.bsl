...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ОбновитьЧекиПоСпискуДоходовВФоне", Истина);
	Методы.Вставить("ПроверитьРезультатФормированияЧекаВФоне", Истина);
	Методы.Вставить("ОтправитьЧекВФоне", Истина);
	Методы.Вставить("ПолучитьФайлЧекаВФоне", Истина);
	Методы.Вставить("АннулироватьЧекВФоне", Истина);
	Методы.Вставить("ПроверитьРезультатАннулированияЧекаВФоне", Истина);
	Методы.Вставить("СформироватьЧекВФоне", Истина);
	
	
	Методы.Вставить("ЗаполнитьПричиныОтменыЧека");
...
Функция СведенияОЧеке(ДокументОснование) Экспорт
	...
	|	ЧекиНПД.НалоговыйПериод КАК НалоговыйПериод,
	|	ЧекиНПД.КодПоОКТМО КАК КодПоОКТМО,
	|	ЧекиНПД.СуммаНалога КАК СуммаНалога,
	|	ЧекиНПД.ДатаОтправки КАК ДатаОтправки,
	|	ЧекиНПД.АвтоматическаяФискализация КАК АвтоматическаяФискализация,
	|	ЧекиНПД.ОтправлятьEmail КАК ОтправлятьEmail,
	|	ЧекиНПД.ОтправлятьSMS КАК ОтправлятьSMS,
	|	ЧекиНПД.СообщениеОбОшибке КАК СообщениеОбОшибке
	|ИЗ
...
//
// Параметры:
//  СтруктураПараметров - Структура - Структура параметров формирования чека. См. ЧекиНПД.НовыйПараметрыФормированияЧека().
...
// Формирует чек в сервисе Мой налог и отправляет покупателю по Email и/или SMS.
// Вызывается при автоматической загрузке документа оплаты.
//
// Параметры:
//  ПараметрыЧека - Структура - параметры формирования чека. См. ЧекиНПД.НовыйПараметрыФормированияЧека().
//
Процедура СформироватьЧекАвтоматически(ПараметрыЧека) Экспорт
	
	НачатьТранзакцию();
	Попытка
		
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ЧекиНПД");
		ЭлементБлокировкиДанных.УстановитьЗначение("ДокументОснование", ПараметрыЧека.Ссылка);
		
		БлокировкаДанных.Заблокировать();
		
		Результат = СформироватьЧек(ПараметрыЧека);
		...
		РезультатЗапроса = НовыйРезультатОперацииСЧеком();
		РезультатЗапроса.СтатусЗапроса = СтатусыЗапросов.Выполняется;
		РезультатЗапроса.ВремяОжидания = Результат.ВремяОжидания;
		
		ПараметрыОжиданияЧека = ЧекиНПДКлиентСервер.НовыйПараметрыОжиданияФормированияЧека();
		ПараметрыОжиданияЧека.Ссылка                      = ПараметрыЧека.Ссылка;
		ПараметрыОжиданияЧека.ПараметрыОжиданияРезультата = Результат.ПараметрыОжиданияРезультата;
		ПараметрыОжиданияЧека.СведенияОЧеке               = Результат.СведенияОЧеке;
		
		Пока РезультатЗапроса.СтатусЗапроса = СтатусыЗапросов.Выполняется И ВремяОжидания < МаксимальнаяДлительность Цикл
			ОбщегоНазначенияБТС.Пауза(РезультатЗапроса.ВремяОжидания);
			ВремяОжидания = ВремяОжидания + РезультатЗапроса.ВремяОжидания;
			РезультатЗапроса = ПроверитьРезультатФормированияЧека(ПараметрыОжиданияЧека);
		КонецЦикла;
		
		Если РезультатЗапроса.СтатусЗапроса = СтатусыЗапросов.Выполнено
			И РезультатЗапроса.СведенияОЧеке.Состояние = Перечисления.СостоянияЧековНПД.Зарегистрирован Тогда
			
			ОтправитьЧекАвтоматически(РезультатЗапроса.СведенияОЧеке, ПараметрыЧека.Контрагент);
		
		ИначеЕсли РезультатЗапроса.СтатусЗапроса = СтатусыЗапросов.Выполняется
			И Не РезультатЗапроса.Ошибка Тогда
			
			СообщениеОбОшибке = СтрШаблон(
				НСтр("ru = 'Превышено время ожидания при проверке формирования чека: %1 с'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ВремяОжидания);
			ЗаписатьСведенияОЧеке(РезультатЗапроса.СведенияОЧеке, СообщениеОбОшибке);
	...
	Исключение
		...
		КомментарийЖурналаРегистрации = СтрШаблон(
			НСтр("ru = 'Не удалось сформировать чеки автоматически: %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьОшибкуВЖурналРегистрации(КомментарийЖурналаРегистрации);
...
//
// Параметры:
//  ПараметрыЧека - Структура - Структура параметров формирования чека. См. ЧекиНПД.НовыйПараметрыФормированияЧека().
// 
// Возвращаемое значение:
...
Функция СформироватьЧек(ПараметрыЧека) Экспорт
	...
	Если Ответ.Ошибка Тогда
		...
		ОписаниеОперации = НСтр("ru = 'При формировании чека возникла ошибка'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписатьОшибкуИКодВЖурналРегистрации(СтрШаблон(НСтр("ru = '%1:'"), ОписаниеОперации), Ответ);
		
		РезультатФормирования.Ошибка = Истина;
		РезультатФормирования.КодОшибки = КодОшибки(Ответ);
		...
		Если ПараметрыЧека.АвтоматическаяФискализация Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1. %2.'", ОбщегоНазначения.КодОсновногоЯзыка()), ОписаниеОперации, РезультатФормирования.ТекстОшибки);
		КонецЕсли;
	Иначе
		СтатусыЗапросов = ИнтеграцияСПлатформойСамозанятыеКлиентСервер.СтатусыЗапросов();
	...
	ЗаписатьСведенияОЧеке(СведенияОЧеке, СообщениеОбОшибке);
...
Функция ПроверитьРезультатФормированияЧека(ПараметрыЧека) Экспорт
	...
	СведенияОЧеке = ПараметрыЧека.СведенияОЧеке;
	Если СведенияОЧеке = Неопределено Тогда
		СведенияОЧеке = СведенияОЧеке(ДокументОснование);
		ПараметрыЧека.СведенияОЧеке = СведенияОЧеке;
	КонецЕсли;
	РезультатФормирования.СведенияОЧеке = СведенияОЧеке;
	...
	Если Ответ.Статус = СтатусыЗапросов.Выполнено Тогда
		...
		СведенияОЧеке.СуммаНалога = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ.Результат, "СуммаНалога", 0);
		СведенияОЧеке.НалоговыйПериод = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ.Результат, "НалоговыйПериод", "");
		СведенияОЧеке.КодПоОКТМО = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ.Результат, "КодПоОКТМО", "");
	...
	ИначеЕсли Ответ.Статус = СтатусыЗапросов.Ошибка
		...
		ОписаниеОперации = НСтр("ru = 'При проверке формирования чека возникла ошибка'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписатьОшибкуИКодВЖурналРегистрации(СтрШаблон(НСтр("ru = '%1:'"), ОписаниеОперации), Ответ);
		...
		СообщениеОбОшибке = "";
		Если СведенияОЧеке.АвтоматическаяФискализация Тогда
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1. %2.'"), ОписаниеОперации, РезультатФормирования.ТекстОшибки);
		КонецЕсли;
		
		ЗаписатьСведенияОЧеке(СведенияОЧеке, СообщениеОбОшибке);
		
		РегистрыСведений.СостоянияИнтеграцииСПлатформойСамозанятые.ЗаписатьСостояниеИнтеграцииСПлатформойСамозанятые(
			СведенияОЧеке.Организация, Ответ);
...
Функция АннулироватьЧек(ПараметрыЧека) Экспорт
	...
	Если Ответ.Ошибка Тогда
		...
		РезультатАннулирования.Ошибка = Истина;
		РезультатАннулирования.КодОшибки = КодОшибки(Ответ);
...
Функция ОтправитьЧек(ПараметрыОтправки) Экспорт
	...
	Если Не ЗначениеЗаполнено(ДокументОснование) Тогда
		ВызватьИсключение НСтр("ru = 'Не указано основание для отправки чека'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Если Не МожноРаспечататьЧек(СведенияОЧеке) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нельзя отправлять чек в статусе %1.'", ОбщегоНазначения.КодОсновногоЯзыка()), СведенияОЧеке.Состояние);
	КонецЕсли;
	...
	ОбработатьОтправкуЧека(РезультатОтправки, СведенияОЧеке);
...
Функция ЗапуститьАннулированиеЧекаВФоне(ПараметрыАннулированияЧека, Идентификатор) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыАннулированияЧека.ДокументОснование) Тогда
		СведенияОЧеке = СведенияОЧеке(ПараметрыАннулированияЧека.ДокументОснование);
	КонецЕсли;
...
// Обновляет файл чека в фоне по данным сервиса Мой налог.
// При вызове на клиенте должна выполняться асинхронно.
//
// Параметры:
//  СведенияОЧеке - Структура - см. НовыйСведенияОЧеке()
// 
// Возвращаемое значение:
//  Структура, Неопределено - новые сведения о чеке, см. НовыйСведенияОЧеке(), или Неопределено, если файл не был получен
Функция ПолучитьФайлЧекаВФоне(СведенияОЧеке) Экспорт
	
	Перем ДвоичныеДанныеЧека, ТипФайла;
	
	Если Не МожноПолучитьФайлЧека(СведенияОЧеке) Тогда
		Возврат СведенияОЧеке;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СведенияОЧеке.АдресЧекаНаСайте) Тогда
		КоличествоПопыток = 3;
		Для Повтор = 1 По КоличествоПопыток Цикл
			ФайлЧека = ПолучитьФайлЧека(СведенияОЧеке.АдресЧекаНаСайте);
			Если ФайлЧека <> Неопределено Тогда
				Статус = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ФайлЧека, "Статус", Ложь);
				Путь = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ФайлЧека, "Путь", Неопределено);
				Заголовки = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ФайлЧека, "Заголовки", Новый Соответствие());
				Если Статус И ЗначениеЗаполнено(Путь) Тогда
					ДвоичныеДанныеЧека = Новый ДвоичныеДанные(Путь);
					ТипФайла = Заголовки["Content-Type"];
	...
	Если ДвоичныеДанныеЧека <> Неопределено Тогда
		ПрисоединитьИзображениеЧекаКДокументуОснованию(СведенияОЧеке, ДвоичныеДанныеЧека, РасширениеФайла(ТипФайла));
		// Сохраняем ошибку автоматической фискализации, чтобы показать пользователю
		ЗаписатьСведенияОЧеке(СведенияОЧеке, СведенияОЧеке.СообщениеОбОшибке);
		Возврат СведенияОЧеке;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
//   Организация - СправочникСсылка.Организации - ссылка на организацию.
//   ДопустимыеСостояния - Неопределено, Массив из ПеречислениеСсылка.СостоянияЧековНПД - состояния чеков
//
// Возвращаемое значение:
...
//
Функция НайтиДокументПоНомеруЧека(НомерЧека, Организация, ДопустимыеСостояния = Неопределено) Экспорт
	...
	Если ДопустимыеСостояния <> Неопределено Тогда
		СхемаЗапроса = Новый СхемаЗапроса;
		СхемаЗапроса.УстановитьТекстЗапроса(Запрос.Текст);
		СхемаЗапроса.ПакетЗапросов[0].Операторы[0].Отбор.Добавить("ЧекиНПД.Состояние В(&ДопустимыеСостояния)");
		Запрос.Текст = СхемаЗапроса.ПолучитьТекстЗапроса();
...
// Обновляет сведения офлайн чека в фоне.
// При вызове на клиенте должна выполняться асинхронно.
//
// Параметры:
//   Организация - СправочникСсылка.Организации - ссылка на организацию.
...
//
Процедура ОбновитьОфлайнЧекВФоне(Организация, НовыеСведенияОЧеке) Экспорт
	
	Перем ДвоичныеДанныеЧека, РасширениеФайлаЧека;
	
	ДокументОснование = НайтиДокументПоНомеруЧека(
		НовыеСведенияОЧеке.ИдентификаторЧека,
		Организация,
	...
	// Если чек был сформирован средствами 1С, то его необходимо скачать из сервиса Мой налог
	Если ЧекХранитсяВТаблице(НовыеСведенияОЧеке.ТипДанных)
		...
		Для Повтор = 1 По КоличествоПопыток Цикл
			...
			Если ФайлЧека <> Неопределено Тогда
				...
				Если Статус И ЗначениеЗаполнено(Путь) Тогда
					ДвоичныеДанныеЧека = Новый ДвоичныеДанные(Путь);
					ТипФайла = Заголовки["Content-Type"];
					РасширениеФайлаЧека = РасширениеФайла(ТипФайла);
					ФайловаяСистема.УдалитьВременныйФайл(Путь);
					Прервать;
				Иначе
					ОбщегоНазначенияБТС.Пауза(1);
	...
	Если ДвоичныеДанныеЧека = Неопределено Тогда
		ДвоичныеДанныеЧека = ПолучитьДвоичныеДанныеИзBase64Строки(НовыеСведенияОЧеке.Base64Строка);
		РасширениеФайлаЧека = НовыеСведенияОЧеке.ТипДанных;
	КонецЕсли;
	
	Если ДвоичныеДанныеЧека <> Неопределено Тогда
		ПрисоединитьИзображениеЧекаКДокументуОснованию(СведенияОЧеке, ДвоичныеДанныеЧека, РасширениеФайлаЧека);
		ЗаписатьСведенияОЧеке(СведенияОЧеке);
		
		ОтправитьЧекАвтоматически(СведенияОЧеке);
	КонецЕсли;
...
// Обновляет сведения о чеках, аннулированных в сервисе Мой налог.
// При вызове на клиенте должна выполняться асинхронно.
//
// Параметры:
//  ТаблицаДоходов - ТаблицаЗначений - см. ИнтеграцияСПлатформойСамозанятые.НовыйСписокЧеков()
//  Организация - СправочникСсылка.Организации
//
// Возвращаемое значение:
//  Массив из Строка - список номеров обновленных чеков.
Функция ОбновитьЧекиПоСпискуДоходовВФоне(ТаблицаДоходов, Организация) Экспорт
	
	ОбновленныеЧеки = Новый Массив;
	
	Если Не ЗначениеЗаполнено(ТаблицаДоходов) Или Не ЗначениеЗаполнено(Организация) Тогда
		Возврат ОбновленныеЧеки;
	КонецЕсли;
	
	АннулированныеЧеки = Новый Массив;
	НомераЧеков = Новый Массив;
	
	Для Каждого Доход Из ТаблицаДоходов Цикл
		Если Доход.ЧекОтменен Тогда
			НовыеСведенияОЧеке = НовыйСведенияОбАннулированномЧеке();
			НовыеСведенияОЧеке.НомерЧека = Доход.ИдентификаторЧека;
			НовыеСведенияОЧеке.ДатаАннулированияЧека = Доход.ДатаОтмены;
			НовыеСведенияОЧеке.АдресЧекаНаСайте = Доход.Ссылка;
			АннулированныеЧеки.Добавить(НовыеСведенияОЧеке);
			НомераЧеков.Добавить(Доход.ИдентификаторЧека);
		КонецЕсли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(АннулированныеЧеки) Тогда
		Возврат ОбновленныеЧеки;
	КонецЕсли;
	
	Выборка = ВыборкаЧековКАннулированию(Организация, НомераЧеков);
	Пока Выборка.Следующий() Цикл
		СведенияОЧеке = НовыйСведенияОЧеке();
		ЗаполнитьЗначенияСвойств(СведенияОЧеке, Выборка);
		
		ИндексЧека = НомераЧеков.Найти(Выборка.НомерЧека);
		НовыеСведенияОЧеке = АннулированныеЧеки[ИндексЧека];

		СведенияОбновлены = ОбновитьЧекВФоне(СведенияОЧеке, НовыеСведенияОЧеке);
		Если Не СведенияОбновлены Тогда
			Продолжить;
		КонецЕсли;
		ОбновленныеЧеки.Добавить(СведенияОЧеке.НомерЧека);
		
		Если Не ЗначениеЗаполнено(Выборка.Комментарий) Тогда
			// В списке доходов, возвращаемых сервисом Мой налог, нет причины аннулирования чека.
			// Поэтому в комментарии документа отражаем, что чек аннулирован в сервисе Мой налог.
			Попытка
				ДокументОбъект = СведенияОЧеке.ДокументОснование.ПолучитьОбъект();
				ДокументОбъект.Заблокировать();
				ДокументОбъект.Комментарий = НСтр("ru = 'Чек аннулирован в сервисе Мой налог'");
				ДокументОбъект.Записать();
				ДокументОбъект.Разблокировать();
			Исключение
				ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось записать причину аннулирования чека НПД в документе %1'"),
					СведенияОЧеке.ДокументОснование);
				ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки);
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
...
// Обновляет сведения о чеке по данным сервиса Мой налог.
// При вызове на клиенте должна выполняться асинхронно.
//
// Параметры:
//  СведенияОЧеке - Структура - см. НовыйСведенияОЧеке()
//  НовыеСведенияОЧеке - Структура - см. НовыйСведенияОбАннулированномЧеке()
// 
// Возвращаемое значение:
//  Булево - Истина, если чек обновлен, иначе - Ложь.
Функция ОбновитьЧекВФоне(СведенияОЧеке, НовыеСведенияОЧеке) Экспорт
	...
	Попытка
		...
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("РегистрСведений.ЧекиНПД");
		ЭлементБлокировкиДанных.УстановитьЗначение("ДокументОснование", СведенияОЧеке.ДокументОснование);
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			// Проверим, что обновляемые сведения о чеке не поменялись
			Если Запись.АдресЧекаНаСайте <> СведенияОЧеке.АдресЧекаНаСайте
				Или Запись.ДатаАннулированияЧека <> СведенияОЧеке.ДатаАннулированияЧека
				Или Запись.Состояние <> СведенияОЧеке.Состояние
				Или Запись.ПричинаОтменыЧека <> СведенияОЧеке.ПричинаОтменыЧека Тогда
				ОтменитьТранзакцию();
				Возврат Результат;
			КонецЕсли;
		Иначе
		...
		СведенияДляОбновления = Новый Массив;
		СведенияДляОбновления.Добавить("Состояние");
		СведенияДляОбновления.Добавить("АдресЧекаНаСайте");
		СведенияДляОбновления.Добавить("ДатаАннулированияЧека");
		
		СведенияОЧеке.Состояние = Перечисления.СостоянияЧековНПД.Аннулирован;
		СведенияОЧеке.АдресЧекаНаСайте = НовыеСведенияОЧеке.АдресЧекаНаСайте;
		СведенияОЧеке.ДатаАннулированияЧека = НовыеСведенияОЧеке.ДатаАннулированияЧека;
		СведенияОЧеке.ПредставлениеПричиныОтменыЧека = НовыеСведенияОЧеке.ПредставлениеПричиныОтменыЧека;
		Если ЗначениеЗаполнено(НовыеСведенияОЧеке.ПредставлениеПричиныОтменыЧека) Тогда
			СведенияОЧеке.ПричинаОтменыЧека = Перечисления.ПричиныОтменыЧекаНПД.ПричинаПоПредставлению(
				НовыеСведенияОЧеке.ПредставлениеПричиныОтменыЧека);
			СведенияДляОбновления.Добавить("ПричинаОтменыЧека");
		КонецЕсли;
		СведенияОЧеке.ОписаниеЧека = ОписаниеЧека(СведенияОЧеке);
			...
			Для Повтор = 1 По КоличествоПопыток Цикл
				...
				Если ФайлЧека <> Неопределено Тогда
					...
					Если Статус И ЗначениеЗаполнено(Путь) Тогда
						...
						ПрисоединитьИзображениеЧекаКДокументуОснованию(СведенияОЧеке, ДвоичныеДанныеЧека, РасширениеФайла(ТипФайла));
						ФайловаяСистема.УдалитьВременныйФайл(Путь);
						СведенияДляОбновления.Добавить("ПрисоединенныйФайлЧека");
						Прервать;
					Иначе
						ОбщегоНазначенияБТС.Пауза(1);
					КонецЕсли;
		...
		ЗаполнитьЗначенияСвойств(Запись, СведенияОЧеке, СтрСоединить(СведенияДляОбновления, ","));
	...
	Исключение
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось обновить сведения о чеке НПД в документе %1 по причине:
			|%2'"),
			СведенияОЧеке.ДокументОснование,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписатьОшибкуВЖурналРегистрации(ТекстОшибки);
...
// Очищает сообщение об ошибке автоматически сформированного чека
//
// Параметры:
//  СведенияОЧеке - Структура - см. НовыйСведенияОЧеке()
//
Процедура ОчиститьСообщениеОбОшибке(СведенияОЧеке) Экспорт
	...
	Для Каждого Запись Из НаборЗаписей Цикл
		Если Не ПустаяСтрока(Запись.СообщениеОбОшибке) Тогда
...
Функция ОбновитьЧекиПоСпискуАннулированныхВФоне(АннулированныеЧеки, Организация) Экспорт
	
	ОбновленныеЧеки = Новый Массив;
	
	Если Не ЗначениеЗаполнено(АннулированныеЧеки) Или Не ЗначениеЗаполнено(Организация) Тогда
		Возврат ОбновленныеЧеки;
	КонецЕсли;
	
	НомераЧеков = АннулированныеЧеки.ВыгрузитьКолонку("ИдентификаторЧека");
	...
	Пока Выборка.Следующий() Цикл
		...
		СведенияОбАннулировании = АннулированныеЧеки.Найти(СведенияОЧеке.НомерЧека, "ИдентификаторЧека");
		
		НовыеСведенияОЧеке = НовыйСведенияОбАннулированномЧеке();
		НовыеСведенияОЧеке.НомерЧека = СведенияОбАннулировании.ИдентификаторЧека;
		НовыеСведенияОЧеке.ДатаАннулированияЧека = СведенияОбАннулировании.ДатаАннулирования;
		НовыеСведенияОЧеке.ПредставлениеПричиныОтменыЧека = СведенияОбАннулировании.ПричинаАннулирования;
		НовыеСведенияОЧеке.АдресЧекаНаСайте = СведенияОбАннулировании.Ссылка;
		...
		Если Не ЗначениеЗаполнено(Выборка.Комментарий)
			И ЗначениеЗаполнено(СведенияОбАннулировании.ПричинаАннулирования) Тогда
			Попытка
				ДокументОбъект = СведенияОЧеке.ДокументОснование.ПолучитьОбъект();
				ДокументОбъект.Заблокировать();
				ДокументОбъект.Комментарий = СведенияОбАннулировании.ПричинаАннулирования;
				ДокументОбъект.Записать();
				ДокументОбъект.Разблокировать();
			Исключение
				ТекстОшибки = СтрШаблон(НСтр("ru = 'Не удалось записать причину аннулирования чека НПД в документе %1'"),
...
Функция НовыйСведенияОбАннулированномЧеке() Экспорт
...
Функция НовыйСведенияОЧеке()
	...
	СведенияОЧеке.Вставить("НалоговыйПериод", "");
	СведенияОЧеке.Вставить("КодПоОКТМО", "");
	СведенияОЧеке.Вставить("СуммаНалога", 0);
	СведенияОЧеке.Вставить("ДатаОтправки", '00010101');
	СведенияОЧеке.Вставить("АвтоматическаяФискализация", Ложь);
	СведенияОЧеке.Вставить("ОтправлятьEmail", Ложь);
	СведенияОЧеке.Вставить("ОтправлятьSMS", Ложь);
	СведенияОЧеке.Вставить("СообщениеОбОшибке", "");
...
Функция НовыйРезультатОперацииСЧеком()
	...
	РезультатФормированияЧека.Вставить("КодОшибки", "");
...
Функция ОписаниеЧека(СведенияОЧеке, ВключатьБлокОжидаетОтправки = Истина)
	...
	НуженБлокДатаОтправки = ЗначениеЗаполнено(СведенияОЧеке.ДатаОтправки);
	...
	ИначеЕсли НуженБлокДата И НуженБлокДатаОтправки Тогда
		
		ТекстОписания = СтрШаблон(НСтр("ru = 'Чек %1 от %2, отправлен %3'"),
			СведенияОЧеке.НомерЧека,
			Формат(СведенияОЧеке.ДатаЧека, "ДЛФ=D"),
			Формат(СведенияОЧеке.ДатаОтправки, "ДЛФ=D"));
...
Функция СведенияОЧекеНачалоФормирования(ПараметрыЧека);
	...
	СведенияОЧеке.ОписаниеЧека = ОписаниеЧека(СведенияОЧеке);
	СведенияОЧеке.АвтоматическаяФискализация = ПараметрыЧека.АвтоматическаяФискализация;
	СведенияОЧеке.ОтправлятьEmail = ПараметрыЧека.ОтправлятьEmail;
	СведенияОЧеке.ОтправлятьSMS = ПараметрыЧека.ОтправлятьSMS;
	Возврат СведенияОЧеке;
	
КонецФункции

Процедура ЗаписатьСведенияОЧеке(СведенияОЧеке, СообщениеОбОшибке = "")
...
Функция РасширениеФайла(ТипФайлаСтрокой)
	
	Если ЗначениеЗаполнено(ТипФайлаСтрокой) И ТипЗнч(ТипФайлаСтрокой) = Тип("Строка") Тогда
		ТипФайла = СтрРазделить(НРег(ТипФайлаСтрокой), "/");
		Если ЗначениеЗаполнено(ТипФайла) Тогда
			РасширениеФайла = ТипФайла[ТипФайла.ВГраница()];
		КонецЕсли;
	КонецЕсли;
...
Функция ПрисоединитьИзображениеЧекаКДокументуОснованию(СведенияОЧеке, ДвоичныеДанныеИзображенияЧека, РасширениеФайла)
	...
	ПараметрыФайла.Вставить("ИмяБезРасширения", СведенияОЧеке.ОписаниеЧека);
...
Функция ТекстSMS(СведенияОЧеке) 
	...
	ЧастиТекста.Добавить(ПредставлениеЧека(СведенияОЧеке));
	
	АдресЧекаНаСайте = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОЧеке, "АдресЧекаНаСайте");
	Если ЗначениеЗаполнено(АдресЧекаНаСайте) Тогда
		ЧастиТекста.Добавить(АдресЧекаНаСайте);
	КонецЕсли;
...
Функция ПредставлениеЧека(СведенияОЧеке)  
	
	НомерЧека = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОЧеке, "НомерЧека", "");
	СуммаЧека = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОЧеке, "СуммаЧека", 0);
	ДатаЧека = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СведенияОЧеке, "ДатаЧека", Дата(1, 1, 1));
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Чек %1 на %2 %3 от %4'"),
		СокрЛП(СведенияОЧеке.НомерЧека),
		Формат(СведенияОЧеке.СуммаЧека, "ЧЦ=15; ЧДЦ=2; ЧРД=-; ЧН=0-00; ЧГ="),
		СокрЛП(ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета()),
		Формат(СведенияОЧеке.ДатаЧека, "ДФ='dd.MM.yyyy HH:mm'")
	);
...
Функция КодОшибки(Ответ)
	...
	Результат = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Ответ, "Результат");
	
	Если Результат = Неопределено Или ТипЗнч(Результат) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "Код");
...
//  Неопределено - если получение файла завершилось ошибкой
//
Функция ПолучитьФайлЧека(АдресЧекаНаСайте) Экспорт
	...
	Попытка
		...
		Если Не РезультатФайлаЧека.Статус Тогда
...
Функция ВыборкаЧековКАннулированию(Организация, НомераЧеков)
	
	// Отбираем чеки по документам, из которых пробит чек: те, которые имеют
	// состояние "Зарегистрирован" или "Ошибка при аннулировании" в случае, если чек
	// был аннулирован в сервисе Мой налог на тот момент, когда
	// пользователь попытался повторно аннулировать его в программе.
	...
	|	ЧекиНПД.ДокументОснование КАК ДокументОснование,
	|	ВЫБОР
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ПоступлениеНаРасчетныйСчет
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.ПоступлениеНаРасчетныйСчет).Комментарий
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.СписаниеСРасчетногоСчета
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.СписаниеСРасчетногоСчета).Комментарий
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.ПриходныйКассовыйОрдер).Комментарий
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.РасходныйКассовыйОрдер
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.РасходныйКассовыйОрдер).Комментарий
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ОплатаПлатежнойКартой
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.ОплатаПлатежнойКартой).Комментарий
	|		КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|			ТОГДА ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.ОтчетОРозничныхПродажах).Комментарий
	...
	|ГДЕ
	|	ЧекиНПД.Организация = &Организация
	|	И ЧекиНПД.НомерЧека В(&НомераЧеков)
	|	И НЕ ЧекиНПД.ПроизведенВозвратПоЧеку
	|	И (ЧекиНПД.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЧековНПД.Зарегистрирован)
	|			ИЛИ ВЫБОР
	|				КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|						ИЛИ ЧекиНПД.ДокументОснование ССЫЛКА Документ.ПоступлениеНаРасчетныйСчет
	|					ТОГДА ЧекиНПД.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЧековНПД.ОшибкаПриАннулировании)
	|				КОГДА ЧекиНПД.ДокументОснование ССЫЛКА Документ.ОплатаПлатежнойКартой
	|						И ВЫРАЗИТЬ(ЧекиНПД.ДокументОснование КАК Документ.ОплатаПлатежнойКартой).ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийОплатаПлатежнойКартой.ОплатаПокупателя)
	|					ТОГДА ЧекиНПД.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияЧековНПД.ОшибкаПриАннулировании)
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ)
	|
	...
	Запрос.УстановитьПараметр("НомераЧеков", НомераЧеков);
	Запрос.УстановитьПараметр("Организация", Организация);
...
Процедура ОтправитьЧекАвтоматически(СведенияОЧеке, Контрагент = Неопределено)
	
	Если (СведенияОЧеке.ОтправлятьEmail Или СведенияОЧеке.ОтправлятьSMS)
		И МожноРаспечататьЧек(СведенияОЧеке) Тогда
		Если Контрагент = Неопределено Тогда
			Контрагент = КонтрагентПоЧеку(СведенияОЧеке);
		КонецЕсли;
		КонтактнаяИнформация = ЧекиНПД.ПараметрыОтправкиЧека(Контрагент);
		ПараметрыОтправкиЧека = ЧекиНПДКлиентСервер.НовыйПараметрыОтправкиЧека();
		ПараметрыОтправкиЧека.Ссылка = СведенияОЧеке.ДокументОснование;
		Если СведенияОЧеке.ОтправлятьEmail И КонтактнаяИнформация.Свойство("АдресЭлектроннойПочты") Тогда
			ПараметрыОтправкиЧека.ОтправлятьEmail = Истина;
			ПараметрыОтправкиЧека.АдресЭлектроннойПочты = КонтактнаяИнформация.АдресЭлектроннойПочты;
		КонецЕсли;
		Если СведенияОЧеке.ОтправлятьSMS И КонтактнаяИнформация.Свойство("НомерТелефона") Тогда
			ПараметрыОтправкиЧека.ОтправлятьSMS = Истина;
			ПараметрыОтправкиЧека.НомерТелефона = КонтактнаяИнформация.НомерТелефона;
		КонецЕсли;
		Если ПараметрыОтправкиЧека.ОтправлятьEmail Или ПараметрыОтправкиЧека.ОтправлятьSMS Тогда
			ОтправитьЧек(ПараметрыОтправкиЧека);
		Иначе
			СообщениеОбОшибке = НСтр("ru = 'Чек не отправлен по причине: не задан email и телефон контрагента'");
...
Процедура ОбработатьОтправкуЧека(Результат, СведенияОЧеке)
	...
	Если Не ЗначениеЗаполнено(ЧастиТекстаОшибки) Тогда
		СведенияОЧеке.ДатаОтправки = ТекущаяДатаСеанса();
	КонецЕсли;
	
	СообщениеОбОшибке = "";
	Если ЗначениеЗаполнено(ЧастиТекстаОшибки)
		И СведенияОЧеке.АвтоматическаяФискализация Тогда
		ОписаниеОперации = НСтр("ru = 'При отправке чека возникла ошибка'", ОбщегоНазначения.КодОсновногоЯзыка());
		ТекстОшибки = СтрСоединить(ЧастиТекстаОшибки, Символы.ПС);
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1. %2.'"), ОписаниеОперации, ТекстОшибки);
	КонецЕсли;
...
Функция КонтрагентПоЧеку(СведенияОЧеке)
	
	Если ЗначениеЗаполнено(СведенияОЧеке.ДокументОснование)
		И ОбщегоНазначения.ЕстьРеквизитОбъекта("Контрагент", СведенияОЧеке.ДокументОснование.Метаданные()) Тогда
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СведенияОЧеке.ДокументОснование, "Контрагент");
	КонецЕсли;
