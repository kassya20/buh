...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ПерезаполнитьЗначенияПоУмолчанию");
	Методы.Вставить("УстановитьПоддержкуСовместительства");
	Методы.Вставить("ЗаполнитьПорядокПримененияВычетовПоНДФЛ");
...
Процедура ПерезаполнитьЗначенияПоУмолчанию() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		// Синхронизация дат выплаты
		ПорядокВыплатыЗарплатыПоУмолчанию = УчетЗарплаты.ПорядокВыплатыЗарплатыПоУмолчанию();
		НастройкиЗарплатаКадры.Организация = Выборка.Организация;
		НастройкиЗарплатаКадры.Прочитать();
		НастройкиЗарплатаКадры.Организация = Выборка.Организация;
		ЗаполнитьЗначенияСвойств(НастройкиЗарплатаКадры, ПорядокВыплатыЗарплатыПоУмолчанию);
		
		Попытка
			НастройкиЗарплатаКадры.Записать();
		Исключение
			ШаблонСообщения	= НСтр("ru = 'Не выполнено обновление записей регистра сведений ""Настройки зарплата кадры""
                                    |%1'");
			ТекстСообщения = СтрШаблон(ШаблонСообщения, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), 
...
Процедура ЗначенияНастройкиПоУмолчанию(Запись) Экспорт
	...
	РезервЕжегодныхОтпусков = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.Резервы.РезервЕжегодныхОтпусков");
