...
Процедура ПолучитьПодразделения(Форма, Организация, Период)
	...
	|	ТерриториальныеУсловияПФРСрезПоследних.ПрименятьСевернуюНадбавку КАК ПрименятьСевернуюНадбавку
	|ИЗ
...
Процедура ПолучитьЛьготныеТарифы(Форма, Организация)
	...
	ТаблицаПодразделений = УчетЗарплаты.ПодразделенияПрименяющиеЛьготныеТарифы(Организация);
	
	ЗначениеВДанныеФормы(ТаблицаПодразделений, Форма.ЛьготныеТарифы);
	ЗначениеВДанныеФормы(ТаблицаПодразделений, Форма.ЛьготныеТарифыПрежняя);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПравоРедактированияНастроек = Истина;
	...
	Иначе
		...
		Элементы.ГруппаКадровыеДанные.Видимость       = Вид = "Кадры";
		...
		ИначеЕсли Вид = "Подразделения" Тогда
			ПравоРедактированияНастроек = ПравоДоступа("Изменение", Метаданные.Справочники.ПодразделенияОрганизаций);
			...
			Элементы.ОбособленныеПодразделенияПрименятьСевернуюНадбавку.Видимость = ПрименятьСевернуюНадбавку;
			ПолучитьПодразделения(ЭтаФорма, Организация, Период);
		ИначеЕсли Вид = "ЛьготныеТарифы" Тогда
			ПравоРедактированияНастроек = ПравоДоступа("Изменение", Метаданные.Справочники.ПодразделенияОрганизаций);
			Организация =  Параметры.Организация;
		...
		ИначеЕсли Вид = "Должности" Тогда
			ПравоРедактированияНастроек = ПравоДоступа("Изменение", Метаданные.Справочники.Должности);
		...
		ИначеЕсли Вид = "Кадры" Тогда
			ЭтаФорма.Заголовок = НСтр("ru = 'Кадровые классификаторы'");
