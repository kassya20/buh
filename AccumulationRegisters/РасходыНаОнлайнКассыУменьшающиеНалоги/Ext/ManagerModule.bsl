...
//
Функция СуммыРасходовЕНВДЗаПериод(Организация, Период, РегистрацияВНалоговомОргане = Неопределено) Экспорт
	...
	|					И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговУменьшаемыхНаРасходыККТ.ЕНВД)) КАК РасходыНаОнлайнКассыЗаКвартал
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				Организация = &Организация
	|					И &УсловиеРегистрацияВНалоговомОргане
	|					И СистемаНалогообложения = ЗНАЧЕНИЕ(Перечисление.ВидыНалоговУменьшаемыхНаРасходыККТ.ЕНВД)) КАК РасходыНаКассовуюТехникуУчтенныеРанее) КАК ВложенныйЗапрос"
	;
...
//
// Возвращаемое значение:
//  Структура - см. НовыйИнформацияПоСуммамРасходов
//
Функция ИнформацияПоСуммамРасходовЗаПериод(Организация, ДатаНачала, ДатаОкончания, ВидНалога) Экспорт
	...
	Если Не ВычетыНаОнлайнКассыКлиентСервер.ВычетыПрименяются(Организация, ДатаНачала, ВидНалога) Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	РасходыНаОнлайнКассыУменьшающиеНалогиОстаткиИОбороты.СуммаКонечныйОстаток КАК КонечныйОстаток,
		|	РасходыНаОнлайнКассыУменьшающиеНалогиОстаткиИОбороты.СуммаПриход КАК Приход,
		|	РасходыНаОнлайнКассыУменьшающиеНалогиОстаткиИОбороты.СуммаРасход КАК Расход
		|ИЗ
		|	РегистрНакопления.РасходыНаОнлайнКассыУменьшающиеНалоги.ОстаткиИОбороты(
		|			&ДатаНачала,
		|			&ДатаОкончания,
	...
	Если Выборка.Следующий() Тогда
		Результат.ВсегоРасходов = Выборка.НачальныйОстаток + Выборка.Приход;
		Результат.ИспользованныйВычет = Выборка.Расход;
		Результат.Остаток = Выборка.КонечныйОстаток;
	КонецЕсли;
