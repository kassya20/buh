...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если НЕ Параметры.ЗапретитьКорректироватьКурсВалютыВФорме Тогда
	...
	Если ДоговорЕстьРеквизит И НЕ КурсДокументаЕстьРеквизит Тогда	
		...
		Если Параметры.ЗапретитьКорректироватьКурсВалютыВФорме Тогда
		...
		ИначеЕсли РасчетыВУЕ И ВалютаУчета <> ВалютаРасчетов Тогда // у.е.
	...
	ИначеЕсли КурсДокументаЕстьРеквизит Тогда
		...
		Для Каждого КурсНаДату Из СписокКурсовВалют Цикл
			Если ЭтоАвансовыйОтчет Тогда
				Элементы.КурсАвансовогоОтчета.СписокВыбора.Добавить(КурсНаДату.Значение, КурсНаДату.Представление);
			Иначе
	...
	КурсАвансовогоОтчета(ЭтотОбъект);
...
Процедура ВалютаПриИзменении(Элемент)
	...
	ЭлементКурс = ?(ЭтоАвансовыйОтчет, "КурсАвансовогоОтчета", "Курс");
	
	Элементы[ЭлементКурс].СписокВыбора.Очистить();
	...
	Для Каждого КурсНаДату Из СписокКурсовВалют Цикл
		Элементы[ЭлементКурс].СписокВыбора.Добавить(КурсНаДату.Значение, КурсНаДату.Представление);
	КонецЦикла;
	
	Если ЭтоАвансовыйОтчет Тогда
		Если ВалютаУчета = ВалютаДокумента Тогда
			КурсРассчитывается = 0;
		КонецЕсли;
...
Процедура ВариантРасчетаНДСПриИзменении(Элемент)
	...
	Если ВариантРасчетаНДС = ПредопределенноеЗначение("Перечисление.ВариантыРасчетаНДС.НДССМежценовойРазницы") Тогда
		ДокументБезНДС        = Ложь;
		НДСНеВыделять         = Ложь;
		СуммаВключаетНДС      = Истина;
		НДССМежценовойРазницы = Истина;
	ИначеЕсли ВариантРасчетаНДС = ПредопределенноеЗначение("Перечисление.ВариантыРасчетаНДС.ДокументБезНДС") Тогда 
...
&НаКлиенте
Процедура КурсРассчитываетсяПриИзменении(Элемент)
...
Процедура КомандаПересчитатьИЗакрыть(Команда)
	...
	Если НЕ Отказ Тогда
		...
		СтруктураРеквизитовФормы.Вставить("БылиВнесеныИзменения",  ЭтотОбъект.Модифицированность);
		...
		СтруктураРеквизитовФормы.Вставить("НДССМежценовойРазницы", НДССМежценовойРазницы);
		...
		СтруктураРеквизитовФормы.Вставить("КурсРассчитывается",    КурсРассчитывается);
...
Процедура КурсАвансовогоОтчета(Форма)
	
	Элементы = Форма.Элементы;
	
	Если Не Форма.ЭтоАвансовыйОтчет Тогда
		Элементы.ГруппаВалютаАвансовогоОтчета.Видимость = Ложь;
		Форма.КлючСохраненияПоложенияОкна = "КлючСохраненияПоложенияОкна";
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаВалютаАвансовогоОтчета.Видимость = Истина;
	Элементы.ГруппаПрочее.Видимость = Ложь;
	Элементы.ЗаголовокВалюта.Видимость = Ложь;
	
	ЭтоВалюта = Форма.ВалютаУчета <> Форма.ВалютаДокумента;
	
	Элементы.КурсАвансовогоОтчетаРуб.Видимость      = Не ЭтоВалюта;
	Элементы.ГруппаКурсАвансовогоОтчета.Видимость   = ЭтоВалюта;
	Элементы.КурсРассчитывается.Видимость = ЭтоВалюта;
	
	Элементы.КурсАвансовогоОтчета.ТолькоПросмотр = ?(Форма.КурсРассчитывается = 0, Ложь, Истина);
	
	Элементы.ГруппаКурс.ОтображениеПодсказки = ?(ЭтоВалюта, ОтображениеПодсказки.ОтображатьСнизу, ОтображениеПодсказки.Нет);
	
	Форма.КлючСохраненияПоложенияОкна = ?(ЭтоВалюта, "КлючАвансовыйОтчетВВалюте", "КлючАвансовыйОтчетВРублях");
...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	Если Параметры.Свойство("Организация") И Параметры.Свойство("ДатаДокумента") Тогда
		Если Параметры.Свойство("НДСНеВыделять") Тогда
			// Признак "НДСНеВыделять" завязан на доступность вычетов у плательшиков НДС.
			ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(Параметры.Организация, Параметры.ДатаДокумента)
				Или УчетнаяПолитика.ПлательщикНДСНаУСНСВычетами(Параметры.Организация, Параметры.ДатаДокумента);
		Иначе
			ПлательщикНДС = УчетнаяПолитика.ПлательщикНДС(Параметры.Организация, Параметры.ДатаДокумента)
				Или УчетнаяПолитика.ПлательщикНДСПрименяющийУСН(Параметры.Организация, Параметры.ДатаДокумента);
		КонецЕсли;
	КонецЕсли;
	
	ПрименяетсяОсвобождениеОтУплатыНДС = Ложь;
	Если Параметры.Свойство("ПрименяетсяОсвобождениеОтУплатыНДС") Тогда
		ПрименяетсяОсвобождениеОтУплатыНДС = Параметры.ПрименяетсяОсвобождениеОтУплатыНДС;
	КонецЕсли; 
	...
	Если Параметры.Свойство("НДССМежценовойРазницы") Тогда
		НДССМежценовойРазницы = Параметры.НДССМежценовойРазницы;
	Иначе
		НДССМежценовойРазницы = Ложь;
	КонецЕсли;
	...
	ИначеЕсли НДССМежценовойРазницы Тогда
		ВариантРасчетаНДС = Перечисления.ВариантыРасчетаНДС.НДССМежценовойРазницы;
	...
	Если Параметры.Свойство("Договор")
		...
		Если (ВалютаРасчетов <> ВалютаУчета) И НЕ Параметры.ЗапретитьКорректироватьКурсВалютыВФорме Тогда
		...
		ИначеЕсли Параметры.ЗапретитьКорректироватьКурсВалютыВФорме Тогда
	...
	Если ЗначениеЗаполнено(ВалютаДокумента)  Тогда
		...
		Если Параметры.Свойство("КурсРассчитывается") Тогда
			ЭтоАвансовыйОтчет = Истина;
			КурсРассчитывается = Параметры.КурсРассчитывается;
		КонецЕсли;
		
	КонецЕсли;
	
	Если Параметры.Свойство("ИспользоватьНачислениеНДССМежценовойРазницы")
	   И Параметры.ИспользоватьНачислениеНДССМежценовойРазницы = Истина
	   И ПлательщикНДС
	   И ВалютаРасчетов = ВалютаРегламентированногоУчета Тогда
		СписокВыбораВариантовРасчетаНДС.Добавить(Перечисления.ВариантыРасчетаНДС.НДССМежценовойРазницы);
	КонецЕсли;
...
Процедура ЗаполнитьКурсКратностьВалютыДокумента(ПересчитатьВалютуДокумента = Ложь)
	...
	Если ЗначениеЗаполнено(ВалютаДокумента) И (НЕ КурсДокументаЕстьРеквизит ИЛИ ПересчитатьВалютуДокумента) Тогда
		...
		Если ВалютаДокумента = ВалютаРасчетов
			...
			// Проверим, существенное ли изменение курса
			МинимальныйКурс = Мин(КурсРасчетов, Курс);
			МаксимальныйКурс = Макс(КурсРасчетов, Курс);
			Если (МинимальныйКурс/МаксимальныйКурс) < 0.5 Тогда
				// Необходимо актуализирвоать кратность расчетов
				КратностьРасчетов = КрастностьРасчетовПоКурсу(КурсРасчетов, КратностьРасчетов, ВалютаДокумента);
			КонецЕсли;
			Курс = КурсРасчетов;
			Кратность = КратностьРасчетов;
...
&НаСервереБезКонтекста
Функция КрастностьРасчетовПоКурсу(Курс, Кратность, Валюта)
	...
	|	И КурсыВалют.Курс = &Курс";
	Результат = Запрос.Выполнить().Выбрать();
	Если Результат.Следующий() Тогда
		Возврат Результат.Кратность;
	Иначе
		Возврат Кратность;
	КонецЕсли;
...
// Состав полей определяется при каждом открытии формы исходя из параметров документа, из которого форма вызывается.
//
&НаСервере
Процедура СброситьРазмерыИПоложениеОкна()
	...
	Если ПравоДоступа("СохранениеДанныхПользователя", Метаданные) Тогда
		ХранилищеСистемныхНастроек.Удалить("ОбщаяФорма.ФормаЦеныИВалюта", "", ИмяПользователя);
	КонецЕсли;
