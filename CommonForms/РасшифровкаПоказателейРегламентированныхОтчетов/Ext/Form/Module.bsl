...
Процедура ОткрытьФормуРасшифровки(Расшифровка, НаименованиеСтроки, НаименованиеПоказателя)
	...
	// До версии 3.0.195 отчет "СправкаРасчетКлассификацияПоСрокамАрендныхОбязательств" назывался "РасшифровкаПоказателейБалансаПоАренде".
	// Путь с предыдущим именем мог быть сохранен в параметрах расшифровок баланса и пояснений к балансу
	// в реквизите "ДанныеОтчета" документа "РегламентированныйОтчет".
	// Для корректного открытия расшифровки в формах отчетности меняем путь с учетом нового имени.
	Если ИмяФормыОтчета = "Отчет.РасшифровкаПоказателейБалансаПоАренде.Форма.ФормаОтчета" Тогда
		ИмяФормыОтчета = "Отчет.СправкаРасчетКлассификацияПоСрокамАрендныхОбязательств.ФормаОбъекта";
	КонецЕсли;
...
Процедура ВывестиЗаписьЛинейнойРасшифровки(Запись, КешВывода)
	...
	ИначеЕсли ТипЗаписи = "РазделительТаблиц" Тогда
		ИмяСекции = "РазделительТаблиц";
		ПодготовитьВыводЗначения(ВыводимыеЗначения, "РазделительТаблицТекст", Запись.НаименованиеСлагаемого);
	Иначе
		ИмяСекции = "Слагаемое";
...
Процедура ВывестиЛинейнуюСекцию(ИмяСекции, ВыводимыеЗначения, Запись, Отступ, КешВывода)
	...
	ГруппироватьСтроки = Свернуть Или (ИмяСекции = "ПорядокРасчета");
	
	Если ГруппироватьСтроки Тогда
		КешВывода.Результат.НачатьГруппуСтрок(, Не Свернуть);
	КонецЕсли;
