		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент()
					И МобильныйКлиентБПКлиент.ДоступенМобильныйПочтовыйКлиент() Тогда
					ДоступныУчетныеЗаписиДляОтправкиМК = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(
						ЭтотОбъект,
						"ДоступныУчетныеЗаписиДляОтправкиМК");
					Если Не ДоступныУчетныеЗаписиДляОтправкиМК Тогда
						ОтправкаПочтовыхСообщенийКлиент.ОтправитьНовоеПисьмоМК(ПараметрыОтправки);
						Возврат;
					КонецЕсли;
				КонецЕсли;
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег(МобильныйКлиентБПКлиент.ИмяФормыСохраненияПечатнойФормы()) Тогда
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			ФайлыВоВременномХранилище = ПоместитьТабличныеДокументыВоВременноеХранилище(ВыбранноеЗначение);
			ФайлыВоВременномХранилище = ПоместитьФайлыВАрхив(ФайлыВоВременномХранилище, ВыбранноеЗначение);
			МобильныйКлиентБПКлиент.ОтправитьФайлыЧерезПоделиться(ФайлыВоВременномХранилище);
		КонецЕсли;
	КонецЕсли;
...
Функция ЕстьПодписиИПечатиДляОбъектовПечати()
	...
	Если ПараметрыВывода.Свойство("ОбъектыПечатиДляФаксимилеБП") Тогда
		...
		ПодписиИПечатиОбъектов = УправлениеПечатью.ПодписиИПечатиОбъектов(ОбъектыПечатиОбъединенный, ПараметрыВывода);
	Иначе
		ПодписиИПечатиОбъектов = УправлениеПечатью.ПодписиИПечатиОбъектов(ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
...
Процедура ОтправитьПечатныеФормыПоПочте()
	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент()
		...
		ДоступныУчетныеЗаписиДляОтправкиМК = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(
			...
			"ДоступныУчетныеЗаписиДляОтправкиМК");
		Если Не ДоступныУчетныеЗаписиДляОтправкиМК Тогда
			ОтправитьПечатныеФормыПоПочтеНастройкаУчетнойЗаписиПредложена(Истина, Неопределено);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьНаКомпьютерПриПодключенииРасширения(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если МобильныйКлиентБПКлиент.ЭтоМобильныйКлиент() Тогда
		ИмяФормыСохраненияПечатнойФормы = МобильныйКлиентБПКлиент.ИмяФормыСохраненияПечатнойФормы();
		ОткрытьФорму(ИмяФормыСохраненияПечатнойФормы, ПараметрыФормы, ЭтотОбъект);
	Иначе
