...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.СвойстваКонтекста.Свойство("ПеречислениеПоИсполнительномуЛисту", ПеречислениеПоИсполнительномуЛисту);
	Параметры.СвойстваКонтекста.Свойство("РедактироватьНазначенияПлатежа",     РедактироватьНазначенияПлатежа);
	Если РедактироватьНазначенияПлатежа Тогда
		КлючСохраненияПоложенияОкна = "РежимРедактированияИсполнительногоДокумента";
	КонецЕсли;
	...
	ЗаполнитьСписокОчередностьПлатежа(); // должно идти позже заполнения ПериодДокумента
...
Процедура КБКПриИзменении(Элемент)
	...
	НастроитьФорму("ИдентификаторПлатежа");
	ВывестиСообщениеОбОшибкахУИН();
...
&НаКлиенте
Процедура ПериодПлатежаПолугодиеПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПериодПлатежаКварталПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПериодПлатежаМесяцПриИзменении(Элемент)
...
Функция РезультатРедактирования(Форма, ЗначениеМодифицированности = Ложь)
	...
	Если Форма.ПеречислениеПоИсполнительномуЛисту
		И ПустаяСтрока(Форма.ИдентификаторПлатежа) Тогда
	...
	Результат.Вставить("ПеречислениеПоИсполнительномуЛисту", Форма.ПеречислениеПоИсполнительномуЛисту);
...
Процедура ПеренестиВДокумент(ПроверитьЗаполнение = Истина)
	...
	Если ПеречислениеВБюджет Или РедактироватьНазначенияПлатежа Тогда
...
Функция Проверить()
	...
	Если РезультатПроверки.Ошибки.Количество() > 0 Тогда
		...
		ШаблонСообщения = НСтр("ru = 'Указаны неверные значения.
			|%1'");
		Если РедактироватьНазначенияПлатежа Тогда
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("ru = 'Сохранить изменения?'"));
		Иначе
			ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("ru = 'Перенести их в документ?'"));
		КонецЕсли;
	...
	ИначеЕсли ИнформацияДублиУИН.Количество() > 0 Тогда
		...
		Если РедактироватьНазначенияПлатежа Тогда
			ШаблонТекстаВопроса = СтрШаблон(ШаблонТекстаВопроса, НСтр("ru = 'Сохранить изменения?'"));
		Иначе
			ШаблонТекстаВопроса = СтрШаблон(ШаблонТекстаВопроса, НСтр("ru = 'Перенести указанные значения в документ?'"));
		КонецЕсли;
...
Процедура НастроитьФорму(Знач ЧастьФормы, Знач Реквизиты = Неопределено)
	...
	ИначеЕсли ЧастьФормы = "ОснованиеПлатежа" Тогда
		ЕНП = ПлатежиВБюджетКлиентСервер.ЭтоЕдиныйНалоговыйПлатеж(КБК);
		Комментарий = "";
		РедактированиеОграничено = Не ПлатежиВБюджетКлиентСервер.ИспользуетсяОснованиеПлатежа(
			ВидПеречисления, Контекст.Период, Комментарий, ПеречислениеПоИсполнительномуЛисту, ЕНП);
		
		ОснованияПлатежа = ПлатежиВБюджетКлиентСервер.ОснованияПлатежа(ВидПеречисления, Контекст.Период);
		Если ПеречислениеПоИсполнительномуЛисту Тогда
			ПлатежиВБюджетКлиентСервер.ДополнитьОснованиямиИсполнительногоЛистаРаботника(ОснованияПлатежа, Контекст.Период);
		КонецЕсли;
		НастроитьСписокВыбора(
	...
	ИначеЕсли ЧастьФормы = "ИдентификаторПлатежа" Тогда
		АвтоОтметкаНезаполненного =
			СтатусПлательщика = ПлатежиВБюджетКлиентСервер.СтатусПлательщикаПоИсполнительномуЛистуПоУИН();
		УстановитьОтметкуНезаполненного(Элементы.ИдентификаторПлатежа, АвтоОтметкаНезаполненного);
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ГруппаНазначениеПлатежа.Видимость = Форма.РедактироватьНазначенияПлатежа И Форма.ПеречислениеПоИсполнительномуЛисту;
...
Процедура НастроитьПоДатеДокумента(Форма, ЗаполнитьВспомогательныеПоля = Истина)
	...
	ИныеСлучаи            = ПлатежиВБюджетКлиентСервер.ИныеСлучаи();
	...
	Если ЗаполнитьВспомогательныеПоля Тогда
		...
		Если ТипЗнч(Форма.ДатаДокумента_Детально) <> Тип("Дата") 
			...
			Если Форма.ДатаДокумента = ИныеСлучаи Тогда
				Форма.ДатаДокумента_Детально = ИныеСлучаи;
			Иначе
	...
	Если Не ПолеВРежимеДаты Тогда
		...
		ПредставлениеИныеСлучаи     = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = '%1 - иные случаи'"),
			ИныеСлучаи);
		
		Если Форма.ВидПеречисления <> ПредопределенноеЗначение("Перечисление.ВидыПеречисленийВБюджет.ТаможенныйПлатеж")
			Или Не ПлатежиВБюджетКлиентСервер.ДействуетПриказ2020_199н_Отложенный(Форма.ПериодДокумента) Тогда
			Поле.СписокВыбора.Добавить(НезаполненноеЗначение,   ПредставлениеНезаполненного);
		КонецЕсли;
		
		Если Форма.ВидПеречисления = ПредопределенноеЗначение("Перечисление.ВидыПеречисленийВБюджет.ТаможенныйПлатеж")
			И ПлатежиВБюджетКлиентСервер.ДействуетПриказ2020_199н_Отложенный(Форма.ПериодДокумента) Тогда
			Поле.СписокВыбора.Добавить(ИныеСлучаи,          ПредставлениеИныеСлучаи);
		КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтметкуНезаполненного(Элемент, АвтоОтметкаНезаполненного)
	
	Элемент.АвтоОтметкаНезаполненного = АвтоОтметкаНезаполненного;
	Если Не АвтоОтметкаНезаполненного Тогда
		Элемент.ОтметкаНезаполненного = АвтоОтметкаНезаполненного;
	КонецЕсли;
