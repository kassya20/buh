...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзмененыКорректировкиВыплаты" И Источник.ВладелецФормы = ЭтаФорма Тогда 
		ОбработатьИзменениеКорректировкиВыплаты(Параметр);
	КонецЕсли;
...
Функция ПоместитьИзмененныеДанныеВоВременноеХранилище()
	...
	Если НЕ СпециализированныйДокумент Тогда
		Для Каждого СтрокаТаблицы Из КорректировкиВыплаты Цикл
			СтрокаТаблицы.ФизическоеЛицо = ФизическоеЛицо;
			СтрокаТаблицы.ФиксРасчет     = КорректироватьРасчет;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьФормуНаСервере(АдресСведенийОбНДФЛ)
	...
	Если СведенияОбНДФЛ.Свойство("СериализованныйОбъект") Тогда
		ДесериализованныйОбъект = ЗарплатаКадры.ДесериализоватьОбъектИзДвоичныхДанных(СведенияОбНДФЛ.СериализованныйОбъект);
		ИмяТипа = ДесериализованныйОбъект.Метаданные().Имя;
		ДобавитьРеквизитФормыОбъект();
		ЗначениеВРеквизитФормы(ДесериализованныйОбъект, "Объект");
	КонецЕсли;
	
	СпециализированныйДокумент = ПустаяСтрока(ИмяТипа);
	...
	Если НЕ СпециализированныйДокумент Тогда
		КорректировкаВыплаты = КорректировкиВыплаты.Итог("КорректировкаВыплаты");
		Если Не КорректироватьРасчет Тогда
			Если КорректировкиВыплаты.НайтиСтроки(Новый Структура("ФиксРасчет", Истина)).Количество() > 0 Тогда
				КорректироватьРасчет = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ДанныеКорректировкиВыплаты()
	
	Возврат ПоместитьВоВременноеХранилище(КорректировкиВыплаты.Выгрузить(), УникальныйИдентификатор);
...
&НаСервере
Процедура ОбработатьИзменениеКорректировкиВыплаты(АдресДанныхКорректировкиВХранилище)
	
	ДанныеКорректировкиВыплаты = ПолучитьИзВременногоХранилища(АдресДанныхКорректировкиВХранилище);
...
Процедура ОбновитьНДФЛСотрудника()
	...
	Если ИмяТипа = "Отпуск"
		ИЛИ ИмяТипа = "БольничныйЛист"
		ИЛИ ИмяТипа = "Увольнение" Тогда
...
Процедура ЗаполнитьВторичныеДанныеТабличныхЧастей()
	...
	ДанныеВычетовКДоходам = ТекущийОбъект.Начисления.Выгрузить();
	Если ИмяТипа = "Премия" Тогда
		ДанныеВычетовКДоходам.Колонки.Добавить("Начисление", Новый ОписаниеТипов("ПланВидовРасчетаСсылка.Начисления"));
		ДанныеВычетовКДоходам.ЗаполнитьЗначения(ТекущийОбъект.ВидПремии, "Начисление");
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТекущийОбъект.ЗависимыеНачисления.Выгрузить(), ДанныеВычетовКДоходам);
	КонецЕсли;
	
	Сотрудники = ОбщегоНазначения.ВыгрузитьКолонку(ДанныеВычетовКДоходам, "Сотрудник");
	Начисления = ОбщегоНазначения.ВыгрузитьКолонку(ДанныеВычетовКДоходам, "Начисление");
	
	УстановитьПривилегированныйРежим(Истина);
	ФизическиеЛицаСотрудников = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Сотрудники,
																			"ФизическоеЛицо");
...
Процедура СформироватьОтчет()
	...
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Отчет.РегистрНалоговогоУчетаПоНДФЛ",
		...
		РегистрНалоговогоУчетаПоНДФЛ,
		ЭтотОбъект,
		Новый Структура("ЗаголовокФормы", НСтр("ru='Регистр налогового учета по НДФЛ'")));
...
&НаКлиенте
Процедура КорректировкаВыплатыНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ФизическоеЛицо", ФизическоеЛицо);
	ПараметрыФормы.Вставить("ТолькоПросмотр", НЕ ДоступностьДляРедактирования);
	ПараметрыФормы.Вставить("ДанныеКорректировкиВыплаты", ДанныеКорректировкиВыплаты());
	ОткрытьФорму("ОбщаяФорма.ПодробнееОРасчетеНДФЛКорректировкиВыплаты", ПараметрыФормы, ЭтаФорма);
