...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Параметры.ОбъектыСогласия) Тогда
		ВызватьИсключение НСтр("ru = 'Не указан параметр ""ОбъектыСогласия""'");
	КонецЕсли;
	
	ТекстСогласияHTML = ТекстПросьбыПодождать();
	ТекстСогласия.УстановитьHTML(ТекстСогласияHTML, Новый Структура);
	
	ДлительнаяОперация = УниверсальныйОбменСБанкамиФормы.ЗаполнитьТекстСогласияВФоне(
		Параметры.ПараметрыСогласия, Параметры.ОбъектыСогласия, УникальныйИдентификатор);
