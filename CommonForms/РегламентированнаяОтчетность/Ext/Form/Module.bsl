...
&НаКлиенте
Перем ПараметрыЦиклаРазделения Экспорт; // Для разбивки писем ФНС
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
			ИспользуетсяРаботаВМоделиСервиса = ПолучитьФункциональнуюОпцию("РаботаВМоделиСервиса");
			Если ИспользуетсяРаботаВМоделиСервиса Тогда
				Элементы.СвязыванияБазДанныхСОблаком.Видимость = Ложь;
				Элементы.НастройкиРаботыСФайлами.Видимость = Ложь;
				Элементы.ПереносДанныхПриложения.Видимость = Истина;
				Элементы.ЗагрузкаДанныхИзСервиса.Видимость = Ложь;
				Элементы.ПерейтиВОблачныйСервис.Видимость = Ложь;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	#КонецОбласти 
	...
	НаличиеЕГРЮЛ = (ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.СообщенияВКонтролирующиеОрганы.РегистрационныеФормы")
		Или ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС"));
		
	Элементы.ГиперссылкаЕГРЮЛ.Видимость = Элементы.ГиперссылкаЕГРЮЛ.Видимость И НаличиеЕГРЮЛ;
	Элементы.СтраницаЕГРЮЛ.Видимость = Элементы.СтраницаЕГРЮЛ.Видимость И НаличиеЕГРЮЛ;
	...
	Если ТипЗнч(КомандаУПУП) = Тип("КомандаФормы") Тогда
		Если ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.УПУП") Тогда
			ЗаголовокУПУП = НСтр("ru = 'Уведомление о предоставлении полномочий представителю - МЧД (ПФР)'");
		Иначе
			ЗаголовокУПУП = НСтр("ru = 'Уведомление о предоставлении полномочий представителю - МЧД (ПФР, не применяется с 04.04.2026)'");
		КонецЕсли;
	...
	Если ТипЗнч(КомандаУПРУП) = Тип("КомандаФормы") Тогда
		...
		Иначе
			ЗаголовокУПРУП = НСтр("ru = 'Уведомление о прекращении полномочий представителя - МЧД (ПФР, не применяется с 04.04.2026)'");
		КонецЕсли;
...
Процедура ПриОткрытии()
	...
	ПрорисоватьПанельГОСТ34122018ДляФНС();
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОбновлениеОтчетностиКалендарьОтчетности" Тогда
		Если НЕ ДлительнаяОтправкаКлиент.ФормаДлительнойОтправкиЕстьСредиОткрытых(Истина) Тогда
	...
	ИначеЕсли ИмяСобытия = "ПредупредитьОГОСТ34122018ДляФНС" Тогда
		ПрорисоватьПанельГОСТ34122018ДляФНС();
	...
	ИначеЕсли ТипЗнч(Источник) <> Тип("ФормаКлиентскогоПриложения")
		...
		Иначе			
			Если КонтекстЭДОКлиент <> Неопределено И Источник <> Неопределено
				И ОбщегоНазначенияЭДКОКлиент.ЭтоЗначениеТипаСОтображениемНаСервере(Источник) Тогда
...
&НаКлиенте
Процедура ПанельГОСТ34122018ДляФНСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПредупредитьОГОСТ34122018ДляФНС" Тогда
		СтандартнаяОбработка = Ложь;
		
		ПредупредитьОГОСТ34122018ДляФНС = ПараметрыПриложения["БРО.ПредупредитьОГОСТ34122018ДляФНС"];
		ТипКриптопровайдераИнициализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПредупредитьОГОСТ34122018ДляФНС,
			"ТипКриптопровайдераИнициализации");
		ПараметрыФормыПредупреждения = Новый Структура("ТипКриптопровайдера", ТипКриптопровайдераИнициализации);
		ОткрытьФорму(
			"ОбщаяФорма.ПредупредитьОГОСТ34122018ДляФНС",
			ПараметрыФормыПредупреждения,
			ЭтотОбъект,,,,,
...
Процедура ПриПереключенииРаздела(Элемент)
	...
	ИначеЕсли Элемент.Имя = "ГиперссылкаНовое" Тогда
		ОтметитьДеревоПросмотренным(Истина);
...
Процедура ОтправитьЗапросНаВыпискуИзЕГРЮЛ_ЕГРИП(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда
			ТекстСообщения = НСтр("ru = 'Для формирования пакета Р13014 или Р24001 зайдите в форму отчета. Отправка других форм не поддерживается.'");
...
Процедура ЗарегистрироватьНовуюОрганизацию(Команда)
	...
	СоздатьУведомлениеЕГРЮЛ(
		ПредопределенноеЗначение("Перечисление.ВидыУведомленийОСпецрежимахНалогообложения.ПустаяСсылка"),
		ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка"));
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ЕщеНеОтправлялся(ОтправляемыйОбъект) Тогда
			...
			Если ТипЗнч(ОтправляемыйОбъект) = Тип("СправочникСсылка.ПерепискаСКонтролирующимиОрганами") Тогда
				...
				ПисьмаВГосОрганыКлиент.ОтправитьПисьмо(ЭтотОбъект, ОтправляемыйОбъект);
...
Процедура ОтправитьПисьмаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбработкаОповещения(РегламентированнаяОтчетностьКлиент.ИмяСобытияЗавершениеОтправкиВКонтролирующийОрган(), , Результат);
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаявленияПоЭлДокументооборотуСПФР")
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ЗПЭД") Тогда
			
			ТекстСообщения = ?(ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(
				Ссылка, "Вид") = ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаОтключение"),
				НСтр("ru = 'Отправка заявления на отключение от ЭДО СФР не применяется'"),
				НСтр("ru = 'Отправка заявления на подключение к ЭДО СФР не применяется, подключение выполняется СФР автоматически'"));
			ПоказатьПредупреждение( , ТекстСообщения);
			Возврат;
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиРаспределенныйРеестр")
			...
			Если ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Исп.МЧД един.фор.для ФНС", Истина) Тогда
				ПоказатьПредупреждение(,
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Уведомление об освобождении от НДС для участников ВИТ Эра МО РФ (КНД 1150153)"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило(НаименованияОтчетаБезПрефикса,
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Аудиторское заключение о консолидированной финансовой отчетности"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Аудиторское заключение о")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Запрос выписки из Единого государственного реестра налогоплательщиков"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Запрос выписки из Единого")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Документ о предстоящей поставке"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ДОПП", '20260401') Тогда
				
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Отправка уведомления в электронном виде не предусмотрена законодательством.
				|Уведомление сдается в бумажном виде'"), Ссылка);
			Возврат;
		...
		ИначеЕсли (ТипЗнч(Ссылка) = Тип("ДокументСсылка.УведомлениеОПредоставленииПолномочийПредставителю")
			ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.УведомлениеОПрекращенииПолномочийПредставителя"))
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.УПУП") Тогда
			
			ТекстСообщения = ?(ТипЗнч(Ссылка) = Тип("ДокументСсылка.УведомлениеОПрекращенииПолномочийПредставителя"),
				НСтр("ru = 'Отправка уведомления о прекращении полномочий представителя не применяется'"),
				НСтр("ru = 'Отправка уведомления о предоставлении полномочий представителю не применяется.
					|Используйте машиночитаемую доверенность (СФР)'"));
			ПоказатьПредупреждение( , ТекстСообщения);
			Возврат;
...
&НаКлиенте
Процедура ПроизводственныеКалендари(Команда)
...
&НаКлиенте
Процедура ПерейтиВОблачныйСервис(Команда)
...
&НаКлиенте
Процедура ЗагрузкаДанныхИзСервиса(Команда)
...
&НаКлиенте
Процедура ПереносДанныхПриложения(Команда)
...
Процедура СкрытьЛишниеЭлементыВрежимеВыбора(Параметры)
	...
	Иначе
		СкрытьЕГРЮЛ = Не (ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.СообщенияВКонтролирующиеОрганы.РегистрационныеФормы")
			Или ОбщегоНазначения.ПодсистемаСуществует("РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ГосударственнаяРегистрацияВФНС"));
	КонецЕсли;
...
&НаСервере
Процедура ОтметитьДеревоПросмотренным(ПроверитьСобытие = Ложь)
	
	Если НЕ ПроверитьСобытие
		ИЛИ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отмечать новое просмотр.") Тогда
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Декларация по водному налогу"
		И ОписаниеРегОтчета(Ссылка).ВыбраннаяФорма = "ФормаОтчета2026Кв2" Тогда
		ПоказатьПредупреждение(, НСтр(
...
Процедура УправлениеВидимостьюЛичныхКабинетовСпецоператоров()
	...
	// Для Бизкуба не отображаем личные кабинеты спец.операторов, пользователи не могут их подключить.
	Если НЕ ЭтоБизКуб Тогда
...
Процедура ЗадатьДатуПодтверждения(СтрокаСписка, Сегодня)
	...
	Если Данные.СостояниеСдачиОтчетности = Перечисления.СостояниеСдачиОтчетности.ТребуетсяПодтверждениеПриема Тогда 
		...
		Ответили = ТребованияФНСВызовСервера.ОтветилиНаВопросПроЭДО(Данные.Ссылка);
		
		Если НЕ ЗначениеЗаполнено(Данные.ПодтвердитьДо) ИЛИ НЕ Ответили Тогда
...
Процедура ЗадатьДатуОтветаНаТребование(СтрокаСписка, Сегодня, ПоддерживаетсяОплата, Ответы)
	...
	ИначеЕсли Данные.ДатаОтвета = '00010101' 
		...
		И (НЕ Данные.ОтветилиНаВопросПроЭДО // не знаем откуда отсчитывать дату ответа - от даты требования или подтверждения
		ИЛИ НЕ Данные.ЭДООбязателен) Тогда // ответили что ЭДО не обязателен и подтверждение еще не отправлено, поэтому не можем рассчитать срок ответа
...
Процедура ДобавитьОтборПоТребованиямБезПодтверждения(ГруппаОтбора, Знач Использование)
	...
	Список.Добавить(ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ТребуетсяПодтверждениеПриема"));
	Список.Добавить(ПредопределенноеЗначение("Перечисление.СостояниеСдачиОтчетности.ОбязанностьПодтвержденияНеОпределена"));
...
Функция ПометитьНаУдалениеРассылки()
	...
	Для каждого Ссылка Из ПредметыРассылок Цикл
		...
		Если ЦиклОбмена <> Неопределено И ЦиклОбмена <> Справочники.ЦиклыОбмена.ПустаяСсылка() Тогда 
...
Асинх Процедура ВыбратьОрганизациюИСоздатьУведомление(ВидыУведомления)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда 
		ВыбраннаяОрганизация = Ждать ВвестиЗначениеАсинх(ПредопределенноеЗначение("Справочник.Организации.ПустаяСсылка"), 
			НСтр("ru = 'Выберите организацию'"), Тип("СправочникСсылка.Организации"));
		...
		Если Элементы.ГруппаОтправкиВЕГРЮЛ.Видимость Тогда
			...
			ИначеЕсли НЕ ЭтоВеб Тогда
				Элементы.ПредупреждениеОбОтправкеЕГРЮЛ.Заголовок = Новый ФорматированнаяСтрока(
					...
					НСтр("ru = ' на изменение в ЕГРЮЛ/ЕГРИП'"));
			КонецЕсли;
...
&НаКлиенте
Процедура ПрорисоватьПанельГОСТ34122018ДляФНС()
	
	ПредупредитьОГОСТ34122018ДляФНС = ПараметрыПриложения["БРО.ПредупредитьОГОСТ34122018ДляФНС"];
	ТипКриптопровайдераИнициализации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ПредупредитьОГОСТ34122018ДляФНС,
		"ТипКриптопровайдераИнициализации");
	
	ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018 =
		КриптографияЭДКОСлужебныйКлиентСервер.ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018(
		ТипКриптопровайдераИнициализации,
		ГОСТ34122018ViPNetCSPДоступен,
		Истина);
	
	Если ЭтоРежимОблачной1СО Тогда
		Элементы.ПанельГОСТ34122018ДляФНС1СО.Заголовок = ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018;
	Иначе
		Элементы.ПанельГОСТ34122018ДляФНС.Заголовок = ОписаниеТребуетсяКриптопровайдерСПоддержкойГОСТ34122018;
	КонецЕсли;
	
	Если ЭтоРежимОблачной1СО Тогда
		Элементы.ГруппаГОСТ34122018ДляФНС1СО.Видимость = ПредупредитьОГОСТ34122018ДляФНС <> Неопределено;
	Иначе
		Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = ПредупредитьОГОСТ34122018ДляФНС <> Неопределено;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗакрытьПанельГОСТ34122018ДляФНСНажатие(Элемент)
	
	ПараметрыПриложения["БРО.ПредупредитьОГОСТ34122018ДляФНС"] = Неопределено;
	Если ЭтоРежимОблачной1СО Тогда
		Элементы.ГруппаГОСТ34122018ДляФНС1СО.Видимость = Ложь;
	Иначе
		Элементы.ГруппаГОСТ34122018ДляФНС.Видимость = Ложь;
	КонецЕсли;
