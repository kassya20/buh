...
Функция КлючНастройки()
	...
	// Приведем ключ к верхнему регистру без пробелов и спецсимволов, кроме разделителя-слеш "/".
	Спецсимволы = ОбменСБанкомКлиентСервер.Спецсимволы() + " ";
...
Функция ПредставлениеОпераций()
	...
	Если ЭтоНастройкаСтатьиДДС(КонтекстНастройки.Настройка) Тогда
		...
		СоответствиеСтатьейДДСОперациям = Справочники.СтатьиДвиженияДенежныхСредств.СоответствиеСтатейДДСОперациям(
...
Функция СформироватьТекстВопросаОсновнаяСтавкаНДС()
	...
	Если ПростойИнтерфейс Тогда
		ПутьКРазделу = НСтр("ru = '""Настройки"" - ""Персональные настройки""'");
	Иначе
...
Процедура МобильныйКлиентАдаптацияФормы()
	...
	Элементы.КартинкаКот.Видимость = Ложь;
