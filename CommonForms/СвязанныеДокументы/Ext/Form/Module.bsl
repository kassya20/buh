...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ОсновнойОбъект)
		И Не ЭтоСсылкаНаДоговор(ТипЗнч(ОсновнойОбъект)) Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоСсылкаНаДоговор(ТипЗнч(ОсновнойОбъект)) Тогда
		ОткрытьДокументыПоДоговору(ОсновнойОбъект);
		Отказ = Истина;
	КонецЕсли;
...
Процедура ВывестиДляТекущего(Команда)
	...
	Если ЭтоСсылкаНаДоговор(ТипЗнч(ТекущийОбъект)) Тогда
		ОткрытьДокументыПоДоговору(ТекущийОбъект);
		Возврат;
	КонецЕсли;
...
Процедура ВывестиПредставлениеИКартинку(СтрокаДерева, Макет, ЭтоТекущийОбъект = Ложь, ЭтоПодчиненный = Неопределено)
	...
	ЭтоДокумент       = ОбщегоНазначения.ЭтоДокумент(МетаданныеОбъекта) Или ЭтоДоговор(МетаданныеОбъекта)
		Или ЭтоЗадача(МетаданныеОбъекта);
...
Функция ЗапросПоРеквизитамОбъектов(СписокОбъектов)
	...
	Для каждого ТипОбъекта Из ОбъектыПоТипу Цикл
		...
		ИначеЕсли ЭтоДоговор(МетаданныеОбъекта) Тогда
			ИмяРеквизитаСумма    = "Сумма";
			ИмяРеквизитаВалюта   = "ВалютаВзаиморасчетов";
...
Функция МетаданныеТипаРеквизита(ТипРеквизита)
	...
	Результат.ЭтоДокумент = Метаданные.Документы.Содержит(МетаданныеРеквизита) Или ЭтоДоговор(МетаданныеРеквизита)
		Или ЭтоЗадача(МетаданныеРеквизита);
...
&НаКлиенте
Процедура ОткрытьДокументыПоДоговору(Договор)
	
	Если Не ДоступныДокументыПоДоговору() Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Документы по договору %1'"), Договор));
	ПараметрыФормы.Вставить("Отбор", Новый Структура("ДоговорКонтрагента", Договор));
...
&НаСервере
Функция ПредставлениеОсновногоОбъекта()
	
	ПредставлениеОбъекта = Неопределено;
	
	Выборка = ЗапросПоРеквизитамОбъектов(ОсновнойОбъект).Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ПредставлениеОбъекта = ПредставлениеОбъектаДляВывода(Выборка);
		Если ПредставлениеОбъекта <> Неопределено Тогда
			ПозицияСуммы = СтрНайти(ПредставлениеОбъекта, НСтр("ru = ' на сумму'"));
			Если ПозицияСуммы <> Неопределено Тогда
				ПредставлениеОбъекта = Лев(ПредставлениеОбъекта, ПозицияСуммы - 1);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если ПредставлениеОбъекта = Неопределено Тогда
		ПредставлениеОбъекта = ОбщегоНазначения.ПредметСтрокой(ОсновнойОбъект);
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЭтоДоговор(ОбъектМетаданных)
	
	Возврат ОбъектМетаданных = Метаданные.Справочники.ДоговорыКонтрагентов;
	
КонецФункции

&НаСервереБезКонтекста
Функция ЭтоЗадача(ОбъектМетаданных)
	
	Возврат ОбъектМетаданных = Метаданные.Задачи.ЗадачаИсполнителя;
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЭтоСсылкаНаДоговор(ТипОбъекта)
	
	Возврат ТипОбъекта = Тип("СправочникСсылка.ДоговорыКонтрагентов");
	
КонецФункции

&НаСервереБезКонтекста
Функция ДоступныДокументыПоДоговору()
	
	МетаданныеЖурнала = Метаданные.ЖурналыДокументов.ЖурналОпераций;
	Возврат ОбщегоНазначения.ОбъектМетаданныхДоступенПоФункциональнымОпциям(МетаданныеЖурнала)
		И ПравоДоступа("Просмотр", МетаданныеЖурнала);
