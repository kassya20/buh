...
Процедура УстановитьПризнакВыполненаУОтмененнойЗадачи() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СтатусЗадачи", Перечисления.СтатусЗадачи.Отменена);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ЗадачаИсполнителя.СтатусЗадачи = &СтатусЗадачи";
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'При записи задачи произошла ошибка: %1'"),
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Задачи.ЗадачаИсполнителя,
				Выборка.Ссылка,
				ТекстСообщения);
				
			ТекстСообщенияПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось записать задачу. Возможно, уже открыта форма этой задачи.'"),
				Выборка.Ссылка);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияПользователю, Выборка.Ссылка);
		КонецПопытки;
	КонецЦикла;
